...
[EventProcessorHost] представляет собой класс .NET, который упрощает прием событий от концентраторов событий путем управления постоянными контрольными точками и одновременно принимает сообщения от этих концентраторов событий в параллельном режиме. С помощью класса [EventProcessorHost] можно разделить события между несколькими получателями даже в том случае, если они размещены в разных узлах. В этом примере показано, как использовать [EventProcessorHost] для одного получателя. [Пример обработки масштабируемого события] показывает, как использовать [EventProcessorHost] для нескольких получателей.

Дополнительные сведения о шаблонах получения концентраторов событий см. в разделе [Обзор концентраторов событий].

Для использования класса [EventProcessorHost] необходимо настроить [учетную запись хранилища Azure]:

1. Войдите на [портал управления Azure] и щелкните элемент **СОЗДАТЬ** в нижней части экрана.

2. Выберите **Службы данных**, **Хранилище**, **Быстрое создание**, а затем введите имя для своей учетной записи. Выберите нужный регион и нажмите кнопку **Создать учетную запись хранилища**.

   	![][11]

3. Выберите созданную запись и нажмите кнопку **Управление ключами доступа**:

   	![][12]

	Скопируйте ключ доступа для последующего использования.

4. Создайте в Visual Studio новый проект приложения C# для настольных ПК с помощью шаблона **Консольное приложение**. Присвойте проекту имя **Receiver**.

   	![][14]

5. В обозревателе решений щелкните правой кнопкой мыши проект **Управление пакетами NuGet**. 

	Откроется диалоговое окно **Управление пакетами NuGet**.

6. Найдите "Концентратор событий Microsoft Azure Service Bus - EventProcessorHost", а затем выберите **Установка** и примите условия использования. 

	![][13]

Будет загружена, установлена и добавлена ссылка на <a href="https://www.nuget.org/packages/Microsoft.Azure.ServiceBus.EventProcessorHost">пакет концентратора событий Azure Service Bus - EventProcessorHost NuGet</a> со всеми его зависимостями.
...
<!-- Links -->
[Обзор концентраторов событий]: http://msdn.microsoft.com/ru-ru/library/azure/dn821413.aspx
...
[Руководство по программированию концентраторов событий]: http://msdn.microsoft.com/ru-ru/library/azure/dn789972.aspx
