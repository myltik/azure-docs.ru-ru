...
> [AZURE.NOTE] Убедитесь, что вы установили [пакет SDK для Azure](https://azure.microsoft.com/downloads/) для Visual Studio, используемый для развертывания веб-сайта.
...
4. Теперь создадим новый файл класса, представляющий подключение к концентратору уведомлений, который используется для отправки уведомлений. В обозревателе решений щелкните правой кнопкой мыши папку **Модели**, щелкните **Добавить**, а затем выберите **Класс**. Назовите новый класс **Notifications.cs** и нажмите кнопку **Добавить** для создания класса.
