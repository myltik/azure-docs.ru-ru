...
На странице [Общие сведения о виртуальных сетях](https://azure.microsoft.com/documentation/articles/virtual-networks-overview/) есть таблица решений, с помощью которой можно подобрать лучший вариант сети.
...
- Портал управления. См. статью [Управление свойствами виртуальной сети](virtual-networks-settings.md).

- Файл конфигурации сети (netcfg). См. статью [Настройка виртуальной сети с помощью файла конфигурации сети](virtual-networks-using-network-configuration-file.md).
...
Да. Дополнительную информацию о диапазонах общедоступных IP-адресов см. в статье [Пространство общедоступных IP-адресов в виртуальной сети](virtual-networks-public-ip-within-vnet.md). Имейте в виду, что ваши общедоступные IP-адреса не будут доступны непосредственно из Интернета.
...
Да. Вы можете использовать пользовательскую маршрутизацию. Дополнительную информацию о пользовательской маршрутизации см. в статье [Пользовательские маршруты и перенаправление IP-адресов](virtual-networks-udr-overview.md).
...
Да. Вы можете организовать обмен данными между виртуальными сетями с помощью REST API или Windows PowerShell. См. статью [Настройка подключения между виртуальными сетями](virtual-networks-configure-vnet-to-vnet-connection.md).
...
> [AZURE.NOTE]В настоящее время имеется следующее ограничение: разрешение имен между клиентами с помощью DNS, предоставленной Azure, выполняется только для первых 100 облачных служб в виртуальной сети. Если вы используете собственный DNS-сервер, то это ограничение не применяется.
...
Да. Дополнительную информацию см. [здесь](virtual-networks-move-vm-role-to-subnet.md).
...
Нет. Развернуть веб-приложение Azure в виртуальной сети невозможно. Тем не менее, веб-приложения могут безопасно подключаться к виртуальной сети Azure и получать доступ к ее ресурсам, если для нее настроено подключение типа «точка-сеть». Дополнительную информацию см. в следующих разделах.

- [Интеграция виртуальной сети веб-приложений Azure](http://azure.microsoft.com/blog/
...
- [Интеграция веб-приложения с виртуальной сетью Azure](web-sites-integrate-with-vnet.md)
