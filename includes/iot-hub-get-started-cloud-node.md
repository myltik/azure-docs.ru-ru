...
4. Добавьте следующие инструкции `require` в начало файла **ReadDeviceToCloudMessages.js**:
    ...
    ```
5. Добавьте следующие объявления переменных, заменив заполнители ранее записанными значениями. Для заполнителя **{your event hub-compatible namespace}** будет использоваться значение поля **Конечная точка, совместимая с концентраторами событий** на портале в формате **namespace.servicebus.windows.net** (без префикса **sb://*).
   ...
   > [!NOTE]
   > Предполагается, что вы создали центр IoT на уровне F1 (Бесплатный). Бесплатный центр IoT содержит два раздела с именами 0 и 1. Если вы создали центр IoT с помощью одной из ценовых категорий, измените код, чтобы создать **MessageReceiver** для каждого раздела.
   > 
   > 
