<properties 
   ...
   description="Описана настройка и регистрация вашего устройства в StorSimple с помощью Windows PowerShell."
   services="storsimple"
...
10. Пароль Диспетчера моментальных снимков StorSimple также задается в этом разделе. Этот пароль используется только при проверке подлинности устройства на узле Windows, на котором запущен Диспетчер моментальных снимков StorSimple. При появлении запроса введите пароль длиной 14–15 символов. Пароль должен содержать сочетание из трех следующих типов символов: строчные и прописные буквы, цифры и специальные символы.
    ...
    Сбросить пароль Диспетчера моментальных снимков StorSimple можно с помощью интерфейса службы Диспетчера StorSimple.

11. На последнем шаге мастер установки выполняет регистрацию устройства в службе Диспетчера StorSimple. Для этого потребуется ключ регистрации службы, полученный на шаге 2. После ввода ключа регистрации, возможно, необходимо будет подождать 2–3 минуты до регистрации устройства.
...
14. Вернитесь на портал управления и выполните следующие действия.
  1. Дважды щелкните службу Диспетчера StorSimple, чтобы открыть страницу **Быстрый запуск**.
  ...
  \>[AZURE.NOTE]Если состояние устройства — **Вне сети**, подождите несколько минут, чтобы устройство могло перейти в оперативный режим.
