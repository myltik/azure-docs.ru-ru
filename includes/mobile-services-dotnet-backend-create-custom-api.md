...
2. Разверните **Мобильные службы Azure** и выберите элемент **Настраиваемый контроллер мобильных служб Azure**, нажмите кнопку **Добавить**, введите в поле **Имя контроллера** значение `CompleteAllController`, а затем еще раз нажмите кнопку **Добавить**.
	...
	![Диалоговое окно веб-API Add Scaffold (Добавление шаблона)](./media/mobile-services-dotnet-backend-create-custom-api/add-custom-api-controller.png)
...
3. В **CompleteAllController.cs** добавьте следующие операторы **using**: 	Замените `todolistService` пространством имен своего проекта мобильных служб, которое должно быть именем мобильной службы с добавлением `Service`.
