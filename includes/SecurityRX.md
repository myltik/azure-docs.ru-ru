...
В облачных приложениях на плечи разработчиков приложений ложится больше нагрузки, связанной с проектированием, разработкой и поддержкой облачных приложений в соответствии со стандартами безопасности высокого уровня для защиты от злоумышленников. Рассмотрим следующую диаграмму (из документа [Заметки о безопасности Azure (PDF)](http://blogs.msdn.com/b/jmeier/archive/2010/08/03/now-available-azure-security-notes-pdf.aspx), автор Дж. Д. Майер (J. D. Meier)): обратите внимание, как к компоненту инфраструктуры обращается поставщик облачных служб, в нашем случае это Azure, из-за чего на долю разработчиков приложений выпадает больше работы, связанной с безопасностью:
...
Полный список угроз, атак, уязвимостей и контрмер см. в документах [Памятка: механизм безопасности веб-приложения](http://msdn.microsoft.com/library/ff649461.aspx) и [Указатель руководства по безопасности для приложений](http://msdn.microsoft.com/library/ff650760.aspx).
...
Атаки — использование таких уязвимостей для причинения вреда приложению. Например, межсайтовые сценарии (XSS) — это атаки, использующие непроверенные выходные данные. Другим примером служит перехват данных по сети для получения учетных данных, передаваемых в незашифрованном виде. Эти атаки также могут привести к подделке удостоверений. Для простоты считайте угрозы, уязвимости и атаки плохими вещами. Рассмотрим следующие схемы как общее представление плохих вещей, связанных с веб-приложением, развернутым в Azure (из документа [Заметки о безопасности Azure (PDF)](http://blogs.msdn.com/b/jmeier/archive/2010/08/03/now-available-azure-security-notes-pdf.aspx), автор Дж. Д. Майер (J. D. Meier)):
...
-   **Из веб-приложения ASP.NET в службу REST WCF с использованием общего токена SWT.** В этом сценарии имеется распределенное приложение, состоящее из интерфейсного веб-приложения ASP.NET и подчиненной службы REST. Вам требуется передавать контекст пользователя через физические уровни.
...
Используйте удостоверение службы Azure AD Access Control. Это похоже на учетную запись пула приложений IIS, которая используется при развертывании приложений в Windows Server и службах IIS. Настройте контроль доступа Azure AD для выдачи токенов SAML, которые будут обрабатываться WIF в службе WCF (SOAP).
...
Использование ClaimsAuthorizationManager в качестве точки расширения WIF. ClaimsAuthorizationManager позволяет выполнять вызовы проверки внешнего доступа, поэтому код приложения будет более понятным, чем при реализации проверки доступа в коде приложения.
...
-   [Рекомендации и ограничения системы безопасности (база данных SQL)](http://msdn.microsoft.com/library/windowsazure/ff394108.aspx#authentication)
-   [Инструкции по подключению к базе данных SQL Azure с использованием sqlcmd](http://msdn.microsoft.com/library/windowsazure/ee336280.aspx)
-   [Инструкции по подключению к базе данных SQL Azure с использованием ADO.NET](http://msdn.microsoft.com/library/windowsazure/ee336243.aspx)
-   [Инструкции по подключению к базе данных SQL Azure с помощью ASP.NET](http://msdn.microsoft.com/library/windowsazure/ee621781.aspx)
-   [Инструкции по подключению к базе данных SQL с помощью служб данных WCF](http://msdn.microsoft.com/library/windowsazure/ee621789.aspx)
-  [Инструкции по подключению к базе данных SQL с помощью PHP](http://msdn.microsoft.com/library/windowsazure/ff394110.aspx)
-   [Инструкции по подключению к базе данных SQL с помощью JDBC](http://msdn.microsoft.com/library/windowsazure/gg715284.aspx)
-   [Инструкции по подключению к базе данных SQL с помощью ADO.NET Entity Framework](http://msdn.microsoft.com/library/windowsazure/ff951633.aspx)
...
Ниже приведены основные средства безопасности WIF. Приведенные ниже сведения являются выдержкой из документов [Вопросы проектирования WIF](http://msdn.microsoft.com/library/ee517298.aspx) и [Безопасность Windows Identity Foundation (WIF) для веб-приложений ASP.NET — угрозы и контрмеры](http://blogs.msdn.com/b/alikl/archive/2010/12/02/windows-identity-foundation-wif-security-for-asp-net-web-applications-threats-amp-countermeasures.aspx).
...
-   **requestValidation** и **httpRuntime requestValidationType**. Включает или отключает функцию проверки ASP.NET. См. указания в документе [Windows Identity Foundation (WIF): обнаружено потенциально опасное значение Request.Form от клиента](http://social.technet.microsoft.com/wiki/contents/articles/1725.windows-identity-foundation-wif-a-potentially-dangerous-request-form-value-was-detected-from-the-client-wresult-t-requestsecurityto.aspx).

###Контроль доступа Azure AD

Рассмотрим следующие меры безопасности в развертывании контроля доступа Azure AD. Приведенные ниже сведения являются выдержкой из документов [Рекомендации по безопасности ACS](http://msdn.microsoft.com/library/gg185962.aspx) и [Рекомендации по управлению сертификатами и ключами](http://msdn.microsoft.com/library/hh204521.aspx).
