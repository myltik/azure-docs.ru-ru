...
](http://blogs.msdn.com/b/jmeier/archive/2010/08/03/now-available-azure-security-notes-pdf.aspx)): обратите внимание, как к компоненту инфраструктуры обращается поставщик облачных служб, в нашем случае это Azure, из-за чего на долю разработчиков приложений выпадает больше работы, связанной с безопасностью.
...
**Windows Identity Foundation (WIF).** WIF - это библиотека среды выполнения .NET, входящая в состав .NET Framework 4.5 (она также доступна для отдельного скачивания для .NET 3.5 и 4.0). WIF выполняет всю сложную работу по обработке протоколов, таких как WS-Federation и WS-Trust, и управляет маркерами, такими как обработка
Security Assertion Markup Language (SAML), благодаря чему не нужно прописывать очень сложный код безопасности в приложении. Следующие ресурсы содержат подробные сведения о службах WIF.
...
-   [Примеры Windows Identity Foundation 3.5/4.0 и шаблоны Visual Studio 2008/2010 templates](http://www.microsoft.com/download/details.aspx?displaylang=en&id=4451) на портале MSDN.
...
STS) и обеспечивает формирование федерации с различными поставщиками удостоверений (IdPs), например корпоративной службой Active Directory или интернет-поставщиками, такими как
Windows Live ID или учетная запись Майкрософт, Facebook, Google, Yahoo! и Open
ID 2.0. Следующие ресурсы содержат подробные сведения о контроле доступа Azure AD.
...
Службы федерации Active Directory (AD FS) 2.0 поддерживают решения удостоверений на основе положений, которые используют технологии Windows Server и Active Directory. Службы AD FS 2.0 поддерживают протоколы WS-Trust, WS-Federation и SAML. Следующие ресурсы содержат подробные сведения о службах AD
FS:
...
-   [Управление доступом к BLOB-объектам и контейнерам](http://msdn.microsoft.com/library/ee393343.aspx)
...
-   [Практическое руководство. Создание первого приложения ASP.NET с поддержкой утверждений с помощью ACS](http://msdn.microsoft.com/library/gg429779.aspx)
-   [Практическое руководство. Размещение страниц входа в веб-приложении ASP.NET](http://msdn.microsoft.com/library/gg185926.aspx)
-   [Практическое руководство. Реализация авторизации на основе утверждений в приложении ASP.NET с поддержкой утверждений с помощью WIF и ACS](http://msdn.microsoft.com/library/gg185907.aspx)    
-   [Практическое руководство. Реализация управления доступом на основе ролей (RBAC) в приложении ASP.NET с поддержкой утверждений с помощью WIF и ACS](http://msdn.microsoft.com/library/gg185914.aspx)
-   [Практическое руководство. Настройка доверия между службой ACS и веб-приложениями ASP.NET с помощью сертификатов X.509](http://msdn.microsoft.com/library/gg185947.aspx)
...
Это похоже на учетную запись пула приложений IIS, которая используется при развертывании приложений в Windows Server и службах IIS. Настройте службу контроля доступа Azure
...
-   [Практическое руководство. Добавление удостоверений службы с сертификатом X.509, паролем или симметричным ключом](http://msdn.microsoft.com/library/gg185924.aspx)
-   [Практическое руководство. Проверка подлинности с помощью сертификата клиента в службе WCF, защищенной ACS](http://msdn.microsoft.com/library/hh289316.aspx)
-   [Практическое руководство. Проверка подлинности с помощью имени пользователя и пароля в службе WCF, защищенной ACS](http://msdn.microsoft.com/library/gg185954.aspx)
-   [Пример кода: проверка подлинности с помощью сертификата WCF](http://msdn.microsoft.com/library/gg185952.aspx)
-   [Пример кода: проверка подлинности с помощью имени пользователя WCF](http://msdn.microsoft.com/library/gg185927.aspx)
...
-   [Практическое руководство. Добавление удостоверений службы с сертификатом X.509, паролем или симметричным ключом](http://msdn.microsoft.com/library/gg185924.aspx)
-   [Практическое руководство. Настройка служб AD FS 2.0 как поставщика удостоверений](http://msdn.microsoft.com/library/gg185961.aspx)
-   [Практическое руководство. Использование службы управления для настройки служб AD FS 2.0 как корпоративного поставщика удостоверений](http://msdn.microsoft.com/library/gg185905.aspx)
...
-   [Практическое руководство. Настройка доверия между службой ACS и службой WCF с помощью симметричных ключей](http://msdn.microsoft.com/library/gg185958.aspx)
-   [Практическое руководство. Проверка подлинности REST-службы WCF, развернутой в Azure, с помощью ACS](http://msdn.microsoft.com/library/hh289317.aspx)
...
-   [Практическое руководство. Проверка подлинности REST-службы WCF, развернутой в Azure, с помощью ACS](http://msdn.microsoft.com/library/hh289317.aspx)
-   [Практическое руководство. Настройка Google как поставщика удостоверений](http://msdn.microsoft.com/library/gg185976.aspx)
-   [Практическое руководство. Настройка Facebook как поставщика удостоверений](http://msdn.microsoft.com/library/gg185919.aspx)
-   [Практическое руководство. Настройка Yahoo! как поставщика удостоверений](http://msdn.microsoft.com/library/gg185977.aspx)
-  [ Пример кода: приложение Windows Phone 7](http://msdn.microsoft.com/library/gg983271.aspx)
-   [Служба REST WCF с токеном SWT, выданным службой контроля доступа Azure (ACS)](http://code.msdn.microsoft.com/REST-WCF-With-SWT-Token-123d93c0)
...
веб-приложением ASP.NET и подчиненной службой REST, и вам требуется сохранить контекст пользователя на физических уровнях. Иногда это необходимо при реализации логики авторизации или ведения журнала на основе удостоверения конечного пользователя в подчиненной службе REST.
...
Существует несколько способов реализации этого сценария. Вы можете использовать 
службу контроля доступа Azure AD, диспетчер проверки подлинности утверждений WIF, сопоставление samlSecurityTokenRequirement или диспетчер ролей клиента.

WIF используется во всех случаях. WIF поддерживает метод IPrincipal.IsInRole(MyRole). В большинстве случаев необходимо убедиться, что в токене есть утверждение о типе роли с универсальным кодом ресурса http://schemas.microsoft.com/ws/2008/06/identity/claims/role, чтобы служба WIF могла успешно проверить членство в роли при 
вызове метода IsInRole.

**Контроль доступа Azure AD**. В этой реализации 
используется модуль правил преобразования утверждений AD Access Control. С помощью этого модуля можно преобразовать любое входящее утверждение в утверждение типа роли, чтобы после поступления токена в приложение или службу WIF могла проанализировать это утверждение роли для того, чтобы
успешно выполнить вызов метода IsInRole.

![][09]

**WIF ClaimsAuthenticationManager**. В этой реализации используйте
ClaimsAuthenticationManager как точку расширения WIF. При таком подходе вы преобразуете все входящие утверждения в тип утверждения роли в приложении. Сложность преобразования ограничивается только написанным кодом.
...
RoleManger. WIF используется для проверки входящих утверждений при реализации пользовательских методов интерфейса RoleManager, например GetAllRoles().
...
-   [Практическое руководство. Реализация управления доступом на основе ролей (RBAC) в приложении ASP.NET с поддержкой утверждений с помощью WIF и ACS](http://msdn.microsoft.com/library/gg185914.aspx)
-   [Практическое руководство. Реализация логики преобразования токенов с помощью правил](http://msdn.microsoft.com/library/gg185955.aspx)
...
-   [Практическое руководство. Подключение к базе данных SQL с помощью sqlcmd](http://msdn.microsoft.com/library/windowsazure/ee336280.aspx)
-   [Практическое руководство. Подключение к базе данных SQL с помощью ADO.NET](http://msdn.microsoft.com/library/windowsazure/ee336243.aspx)
-   [Практическое руководство. Подключение к базе данных SQL через ASP.NET](http://msdn.microsoft.com/library/windowsazure/ee621781.aspx)
-   [Практическое руководство. Подключение к базе данных SQL через службы данных WCF](http://msdn.microsoft.com/library/windowsazure/ee621789.aspx)
-  [Практическое руководство. Подключение к базе данных SQL с помощью PHP](http://msdn.microsoft.com/library/windowsazure/ff394110.aspx)
-   [Практическое руководство. Подключение к базе данных SQL с помощью JDBC](http://msdn.microsoft.com/library/windowsazure/gg715284.aspx)
-   [Практическое руководство. Подключение к базе данных SQL с помощью ADO.NET Entity Framework](http://msdn.microsoft.com/library/windowsazure/ff951633.aspx)
...
-   [Проверка подлинности и авторизация служебной шины с помощью службы контроля доступа](http://msdn.microsoft.com/library/hh403962.aspx)

## Сценарии идентификации и контроля доступа для кэша в памяти

Служба кэша в памяти (ранее - кэш Azure) использует
Azure AD Access Control для проверки подлинности. Он использует общие ключи, доступные через портал управления. Используйте ключи в коде или файлах конфигурации при обращении к кэшу. Обязательно храните ключи в безопасном расположении, чтобы избежать раскрытия информации.
...
-   [Практическое руководство. Настройка кэша клиента программным способом для службы кэша Azure](http://msdn.microsoft.com/library/windowsazure/gg618003.aspx)
-   [Практическое руководство. Настройка клиента кэша с помощью файла конфигурации приложения для службы кэша Azure](http://msdn.microsoft.com/library/windowsazure/gg278346.aspx)
-   [Примеры использования служебной шины и службы кэша Azure](http://msdn.microsoft.com/library/ee706741.aspx) (раздел "Примеры службы кэша")
...
Marketplace API. Для успешного выполнения вызовов к API Marketplace необходимо пройти проверку подлинности. Приложение развернуто в
Azure Marketplace.
...
контроль доступа Azure AD. Используйте его как базовый контрольный список для этих технологий при разработке и развертывании приложения.
...
](http://msdn.microsoft.com/library/gg185962.aspx) и [Рекомендации по управлению сертификатами и ключами](http://msdn.microsoft.com/library/hh204521.aspx).
...
[Проектирование единого входа в Интернете]: http://technet.microsoft.com/library/dd807033(WS.10).aspx
[Единый вход для интернет-решений]: http://technet.microsoft.com/library/dd807033(WS.10).aspx
[Проектирование федеративного единого входа в Интернете]: http://technet.microsoft.com/library/dd807050(WS.10).aspx
[Единый вход федерации для интернет-решений]: http://technet.microsoft.com/library/dd807050(WS.10).aspx
