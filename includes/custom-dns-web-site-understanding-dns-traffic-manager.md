...
При создании веб-сайта Azure DNS-имя автоматически назначается веб-сайту. Это имя имеет форму **&lt;имя_сайта&gt;.azurewebsites.net**. При добавлении веб-сайта в качестве конечной точки диспетчера трафика Azure ваш веб-сайт становится доступным через домен **&lt;профиль_диспетчера_трафика&gt;.trafficmanager.net**.

> [AZURE.NOTE] Если веб-сайт настроен в качестве конечной точки диспетчера трафика, будет использоваться адрес **.trafficmanager.net** при создании записей DNS.

> [AZURE.NOTE] Можно использовать записи CNAME только с помощью диспетчера трафика
...
Запись CNAME сопоставляет *specific* DNS-имя, такое как **mail.contoso.com** или **www.contoso.com**, с другим (каноническим) доменным именем. В случае веб-сайтов Azure, использующих диспетчер трафика, каноническим доменным именем является имя домена **&lt;мое_приложение>.trafficmanager.net** из профиля вашего диспетчера трафика. После создания запись CNAME создает псевдоним для доменного имени **&lt;мое_приложение>.trafficmanager.net**. Запись CNAME будет автоматически разрешаться в IP-адрес имени вашего домена **&lt;мое_приложение>.trafficmanager.net**, поэтому при изменении IP-адреса веб-сайта не нужно предпринимать никаких действий.
