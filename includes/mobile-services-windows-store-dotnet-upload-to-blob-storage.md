...
2. Откройте файл MainPage.xaml и замените элемент **StackPanel** непосредственно после первого элемента **Задача** следующим кодом:
...
3. Замените элемент **StackPanel** в **DataTemplate** на следующий код:
...
4. Откройте файл проекта MainPage.xaml.cs и добавьте следующие операторы **using**:
...
5. В классе TodoItem добавьте следующие свойства:
       ...
       >[AZURE.NOTE]Чтобы добавить новые свойства для объекта TodoItem во внутренней мобильной службе JavaScript, необходимо включить в ней динамическую схему. После включения динамической схемы в таблицу TodoItem автоматически добавляются новые столбцы, которые соответствуют этим новым свойствам. Сведения о внутренней мобильной службе .NET см. в разделе [Изменение модели данных в серверной мобильной службе для .NET](../articles/mobile-services/mobile-services-dotnet-backend-how-to-use-code-first-migrations.md).
6. Добавьте в класс MainPage приведенный ниже код:
...
7. Замените существующий метод `InsertTodoItem` следующим кодом:
