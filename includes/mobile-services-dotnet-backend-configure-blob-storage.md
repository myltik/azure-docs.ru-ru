...
Чтобы иметь возможность создавать учетные данные SAS для отправки изображений в хранилище больших двоичных объектов, необходимо добавить пакет NuGet, который устанавливает клиентскую библиотеку хранилища в проект мобильной службы. 
...
2. В левой области щелкните категорию **В сети**, выберите **Только стабильные**, выполните поиск **WindowsAzure.Storage**, нажмите кнопку **Установить** для пакета **Служба хранения Azure**, затем примите условия лицензионных соглашений. 
...
##<a name="update-scripts"></a>Обновление контроллера TodoItem для создания подписанного URL-адреса 
...
7.  В папке Controllers откройте файл TodoItemController.cs и добавьте следующие директивы **using**:
...
8.  Замените существующий метод **PostTodoItem** следующим кодом:
   	...
   	Этот метод POST создает новый SAS для вставленного элемента, действительный в течение 5 минут, и присваивает значение созданного SAS свойству  `sasQueryString` возвращенного элемента. Свойство  `imageUri` также задается для пути ресурса нового большого двоичного объекта, чтобы отображать изображение в пользовательском интерфейсе клиента во время привязки.
