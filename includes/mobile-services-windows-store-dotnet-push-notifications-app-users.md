...
3.  Откройте файл проекта MainPage.xaml.cs и замените переопределение метода **OnNavigatedTo** следующим:
    ...
    В этом коде следует заменить автоматически созданное имя класса push (`todolistPush`) именем класса, созданным мастером, обычно в формате `mobile_servicePush`.
...
2.  Измените диапазон доступа метода **InitNotificationsAsync** с `private` на `public` и добавьте модификатор `static`.
