...
1. В Visual Studio измените определение метода `PostTodoItem` с помощью следующего кода:  
    ...
    Этот код отправляет шаблонное уведомление только для регистраций с тегом, содержащим SID текущего пользователя. Обратите внимание, что значение SID можно изменить, поскольку он формируется на основе маркера поставщика проверки подлинности.
2. Повторная публикация серверного проекта
