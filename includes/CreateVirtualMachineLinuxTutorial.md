...
Вы можете легко создать виртуальную машину, работающую под управлением ОС Linux, с помощью коллекции образов на портале управления Azure. Это руководство разработано для читателей, не имеющих опыта использования платформы Azure. Вы сможете создать виртуальную машину под управлением ОС Linux в облаке, к которой также сможете получать доступ и настраивать ее.
...
- Создание и передача VHD-файла с образом в Azure и затем создание виртуальной машины с помощью образа. Дополнительные сведения о создании и передаче пользовательского образа см. в разделе [Создание и передача виртуального жесткого диска, содержащего операционную систему Linux](/ru-ru/manage/linux/common-tasks/upload-a-vhd/).
...
2. Щелкните **Виртуальная машина** и выберите **Из коллекции**.
...
8. В разделе "Проверка подлинности" установите флажок **Предоставьте пароль**. Затем введите требуемые сведения и нажмите стрелку для продолжения.
...
14.  В разделе **Агент ВМ** укажите, следует ли установить агент ВМ. Этот агент предоставляет среду для установки расширений, которые помогут вам взаимодействовать с виртуальной машиной. Дополнительные сведения см. в разделе [Использование расширений](http://go.microsoft.com/FWLink/p/?LinkID=390493). 
...
3. Введите **имя узла** и **данные порта**, полученные с панели мониторинга, и щелкните **Открыть**.
...
Диски данных хранятся в хранилище Azure и могут использоваться для постоянного хранения файлов и данных приложений.
...
В системе Linux диск ресурсов обычно управляется агентом Linux Azure, и он автоматически подключается к **/mnt/resource** (или **/mnt** на образах Ubuntu). Дополнительные сведения см. в [руководстве пользователя агента Linux Azure](http://www.windowsazure.com/ru-ru/manage/linux/how-to-guides/linux-agent-guide/).
...
3. В окне SSH введите следующую команду для создания нового устройства, а затем введите пароль учетной записи:
	...
	>[WACOM.NOTE] В данном примере можно использовать `sudo -i` для некоторых дистрибутивов, если /sbin или /usr/sbin не расположены по пути `$PATH`.
...
8. Необходимо создать файловую систему в новом разделе. Например, введите следующую команду для создания новой файловой системы, а затем введите пароль учетной записи:
	...
	>[WACOM.NOTE] Обратите внимание, что системы SUSE Linux Enterprise 11 предоставляют доступ только для чтения в файловых системах ext4.  Для этих систем рекомендуется форматировать новую файловую систему ext3, а не ext4.
...
10. Введите следующую команду, чтобы подключить диск:
...
11. Добавьте новый диск в /etc/fstab:
	...
	>[WACOM.NOTE] Для blkid может не требоваться доступ sudo во всех случаях, однако может быть проще выполнить `sudo -i` в некоторых дистрибутивах, если /sbin или /usr/sbin не находятся в пути `$PATH`.
	...
	**Внимание!** Некорректное изменение файла /etc/fstab может привести к невозможности загрузить систему. Если не уверены, обратитесь к документации дистрибутива за сведениями о том, как правильно изменить этот файл. Также рекомендуется перед внесением изменений создать резервную копию файла /etc/fstab.
	...
	Теперь можно проверить, правильно ли установлена файловая система, просто отключив и повторно установив файловую систему, т. е. используя типовую точку установки `/datadrive`, созданную на предыдущих этапах: 
	...
	>[WACOM.NOTE] Последующее удаление диска данных без редактирования fstab может привести к тому, что ВМ не загрузится. Если это происходит часто, то большинство дистрибутивов предоставляют параметры `nofail` или `nobootwait`, позволяющие загрузить систему, даже если диск отсутствует. Дополнительные сведения об этих параметрах см. в документации по вашему дистрибутиву.
