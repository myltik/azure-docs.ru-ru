...
6. Добавьте реализацию  `completeAll` в **QSTodoService.m** перед строкой `@end`. iOS, как и JavaScript, не поддерживает сериализацию JSON произвольных типов. Таким образом, здесь имеет место довольно простой API для вызова пользовательских API, состоящий из метода  `invokeAPI`. 
