# <a name="data-management-and-business-analytics"></a>Управление данными и бизнес-аналитика
...
* [Использование СУБД на виртуальной машине](#dbinvm)
* [База данных SQL](#sqldb)
  * [Синхронизация данных SQL](#datasync)
  * [Создание отчетов на основе данных SQL с использованием виртуальных машин](#datarpt)
* [Хранилище таблиц](#tblstor)
* [Hadoop](#hadoop)

## <a name="a-nameblobablob-storage"></a><a name="blob"></a>Хранилище BLOB-объектов
Слово BLOB является сокращением от фразы "Binary Large OBject" (большой двоичный объект) и точно отражает значение этого термина: набор двоичных данных. Несмотря на то, что BLOB-объекты весьма просты, они достаточно полезны. [На рис. 1](#Fig1) продемонстрированы основные принципы хранилища BLOB-объектов в Windows Azure.

<a name="Fig1"></a>![Схема BLOB-объектов][blobs]

**Рис. 1. В хранилище BLOB-объектов Azure в контейнерах хранятся двоичные данные (BLOB-объекты)**
...
&lt;*StorageAccount*&gt; — уникальный идентификатор, назначаемый при создании учетной записи хранения, а &lt;*Container*&gt; и &lt;*BlobName*&gt; — имена определенного контейнера и BLOB-объекта в этом контейнере. 
...
* — Непосредственно через протокол доступа RESTful (т. е. на основе протокола HTTP). Этот способ доступен для приложений Azure и внешних приложений, включая приложения, работающие локально.
...
## <a name="a-namedbinvmarunning-a-dbms-in-a-virtual-machine"></a><a name="dbinvm"></a>Использование СУБД на виртуальной машине
...
### <a name="a-namedatasyncasql-data-sync"></a><a name="datasync"></a>Синхронизация данных SQL
...
### <a name="a-namedatarptasql-data-reporting-using-virtual-machines"></a><a name="datarpt"></a>Создание отчетов на основе данных SQL с использованием виртуальных машин
...
## <a name="a-nametblstoratable-storage"></a><a name="tblstor"></a>Хранилище таблиц
...
Каждый из разделов таблицы содержит определенное число *объектов*, каждый из которых включает 255 *свойств*. Каждое свойство имеет имя, тип (например, Binary, Bool, DateTime, Int или String) и значение. В отличие от реляционного хранилища, эти таблицы не имеют фиксированной схемы, поэтому другие объекты в той же таблице могут содержать свойства с различными типами. Например, один из объектов может содержать только свойство типа String с именем, в то время как другой объект в той же таблице будет иметь два свойства типа Int, содержащие идентификационный номер клиента и кредитный рейтинг.
...
Приложение Hadoop, обычно называемое *заданием*, использует модель программирования *MapReduce*. Как показано на рисунке, логика для задания MapReduce выполняется одновременно на нескольких виртуальных машинах. При помощи параллельной обработки данных Hadoop может анализировать данные гораздо быстрее, чем решения для одного компьютера.

В Azure данные, с которыми работает задание MapReduce, обычно хранятся в хранилище BLOB-объектов. А в Hadoop задания MapReduce ожидают, что данные будут храниться в *распределенной файловой системе Hadoop (HDFS)*. В чем-то система HDFS схожа с хранилищем BLOB-объектов; например, она реплицирует данные на нескольких физических серверах. Вместо того чтобы дублировать эту функциональность, Hadoop в Azure предоставляет хранилища BLOB-объектов через API-интерфейс HDFS, как показано на рисунке. В то время как логика задания MapReduce предполагает, что работа ведется с обычными файлами HDFS, задание на самом деле обращается к данным, которые передаются потоком из BLOB-объектов. Кроме того, для поддержки ситуаций, когда по одним данным выполняется несколько заданий, Hadoop в Azure также допускает копирование данных из BLOB-объектов в полноценную систему HDFS на виртуальных машинах. 
