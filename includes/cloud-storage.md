...
**Рисунок 2. Виртуальные машины Azure разрешают выполнять запуск СУБД на виртуальной машине с хранением данных в BLOB-объектах.**
...
Как показано на [Рисунке 2](#Fig2), базы данных, по виду, хранятся на локальном диске виртуальной машины, на которой выполняется сервер. Тем не менее, на базовом уровне каждый из этих дисков хранится в BLOB-объекте Azure. (Это похоже на использование сети хранения данных в собственном центре обработки данных, при этом BLOB-объект во многом схож с логическим номером устройства.) Как в любом BLOB-объекте Azure, содержащиеся в нем данные реплицируются трижды в пределах центра обработки данных, а по запросу дополнительно выполняется гео-репликация в другой центр обработки данных в том же регионе. Для повышения надежности можно также использовать такие функции, как зеркальное отображение баз данных SQL Server. 
...
Если база данных содержит данные, скорее всего, потребуется создавать отчеты на основе этих данных. Azure способна выполнять службы отчетности SQL Server (SSRS) на виртуальных машинах Azure, что является функциональным эквивалентом запуску служб отчетности SQL Server локальным образом. Затем службы SSRS можно использовать для построения отчетов на основе данных, хранящихся в базе данных SQL Azure.  [На Рисунке 5](#Fig5) показано, как работает этот процесс.

<a name="Fig5"></a>![Схема создания отчетов SQL][SQL-report]
 
**Рисунок 5. Службы отчетов SQL Server на виртуальных машинах Azure обеспечивают создание отчетности по данным из базы данных SQL. .**
...
**Рисунок 6. Хранилище таблиц Azure представляет собой хранилище пар ключ-значение, которое обеспечивает быстрый и простой доступ к большим объемам данных.**
...
**Рисунок 7. Hadoop в Azure выполняет задания MapReduce, которые обрабатывают данные в параллельном режиме на нескольких виртуальных машинах.**
