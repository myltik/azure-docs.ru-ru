...
1. Отправить файл запроса Hive (HQL) в папку **script** в контейнере **adfgetstarted**.
2. Отправить входной файл в папку **inputdata** в контейнере **adfgetstarted**.
...
1. Чтобы подготовить хранилище Azure для использования с учебником:
   ...
   3. Перейдите в папку c:\\adfgetstarted и выполните команду ниже. Эта команда отправляет файл **input.log** в учетную запись хранилища (контейнер **adfgetstarted**, папка **inputdata**). Замените значения **StorageAccountName** и **Storage Key** именем и ключом своей учетной записи хранения соответственно.
      ...
      > 
      > 
   4. После успешной передачи файла отображаются примерно следующие выходные данные из AzCopy.
