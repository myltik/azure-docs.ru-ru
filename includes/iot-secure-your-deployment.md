...
[Центр Интернета вещей поддерживает такие протоколы, как MQTT, AMQP и HTTP][lnk-protocols]. Каждый из этих протоколов по-разному используют маркеры безопасности, передаваемые из устройства Интернета вещей в Центр Интернета вещей:
...
Во время установления безопасного подключения TLS к Центру Интернета вещей устройство Интернета вещей аутентифицирует Центр Интернета вещей с помощью корневого сертификата, который является частью пакета SDK для устройств. Сертификат клиентского пакета SDK для C находится в папке \\c\\certs в корне репозитория. Хотя эти корневые сертификаты действуют продолжительное время, срок их действия может истечь или они могут быть отозваны. Если нет возможности обновить сертификат на устройстве, вследствие этого устройство не сможет подключиться к Центру Интернета вещей (или другой облачной службе). Наличие средств обновления корневого сертификата после развертывания устройства IoT позволит эффективно уменьшить этот риск.
