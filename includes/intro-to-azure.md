...
Azure — это платформа приложений корпорации Майкрософт для общедоступного облака. Вы можете использовать данную платформу множеством разных способов.

-   Вы можете создать веб-приложение, которое выполняет и хранит свои данные в центрах данных Azure.
-   Вы можете просто хранить данные. При этом использующие их приложения могут выполняться локально (то есть за пределами общедоступного облака).
-   Вы можете использовать Azure, чтобы создавать виртуальные машины для разработки и тестирования или для запуска приложений SharePoint и других приложений.
-   Вы можете использовать Azure для создания приложений со значительными возможностями масштабирования и огромным количеством пользователей.

Все это — и многое другое — возможно благодаря обширному диапазону служб, предоставляемому платформой. Однако для выполнения любой из указанных задач необходимо разобраться в основах. Даже если вам ничего не известно об облачных вычислениях, данная статья поможет вам ознакомиться с основными понятиями и принципами работы Azure. Ее задачей является сформировать у вас основу для понимания и использования этой облачной платформы.

**Как читать эту статью**

В Azure представлено множество служб и постоянно появляются новые, поэтому можно легко запутаться. Основные службы указаны ранее в документе. Начните с них. Затем ознакомьтесь с дополнительными службами. Это не означает, что вы не можете использовать только дополнительные службы, однако базовые службы лежат в основе приложения, запущенного в Azure.

**Отправка отзывов**

В этой статье предоставляются полезные общие сведения об Azure. Если они недостаточно исчерпывающие, сообщите об этом в разделе комментариев внизу страницы. Укажите подробнее о том, что вы ожидаете увидеть и как можно улучшить статью. Ваши отзывы очень важны для нас.
...
-   [Помощь в вычислении][]
-   [Производительность][]
-   [Большие вычисления и данные][]
-   [Служба мультимедиа][]
-   [Приложения для коммерции][]
-   [Приступая к работе][]

## <span id="components"></span></a>Компоненты Azure

Azure группирует службы по категориям на портале управления и по разным вспомогательным визуальным элементам, таким как [Инфографика «Что такое Windows Azure»][]. Портал управления используется для управления большинством служб в Azure (но не всеми). В этом документе будет использоваться **другая организация** для предоставления сведений о службах на основе аналогичных функций и для вызова важных подслужб, которые являются частью более крупных служб.

![Компоненты Azure][1]

 **Рис. 1. Azure предоставляет службы приложений, доступных в Интернете, выполняющихся в центрах данных Azure.**

## <span id="portal"></span></a>Портал управления

Azure содержит веб-интерфейс, который называется портал управления. Он позволяет администраторам получить доступ к большинству функций Azure (но не всем) и управлять ими. Обычно перед прекращением действия портала Майкрософт выпускает бета-версию нового портала пользовательского интерфейса. Более новый портал называется «портал (предварительная версия)». Обычно в течение некоторого времени можно использовать оба портала.

## <span id="compute"></span></a> Среда выполнения приложений
...
**Рис. 2. Виртуальные машины Azure предоставляют полный контроль над экземплярами виртуальных машин в облаке.**
 ...
 **Веб-сайты Azure запускают приложение веб-сайта в облаке без управления базовым веб-сервером.**

Одна из самых распространенных задач, которые люди выполняют в облаке, — запуск веб-сайтов и веб-приложений. Модель виртуальных машин Azure допускает это, однако требует от вас заниматься администрированием одной или нескольких виртуальных машин и базовых операционных систем. Это могут выполнять веб-роли облачных служб, но их развертывание и обслуживание все еще требует административной работы. А что если вы просто хотите получить веб-сайт, всю работу по администрированию которого выполняет кто-то другой?
...
Для ведения разработки веб-сайты поддерживают .NET, PHP, Node.js и Python, а также базу данных SQL и MySQL (от компании ClearDB — партнера корпорации Майкрософт) для реляционного хранилища. Она также имеет встроенную поддержку нескольких популярных приложений, включая WordPress, Joomla и Drupal. Задача заключается в предоставлении экономичной, масштабируемой и универсальной платформы для создания веб-сайтов и веб-приложений в общедоступном облаке.

**Сценарии веб-сайтов**

Веб-сайты полезны для корпораций, разработчиков и агентств, занимающихся веб-дизайном. Для корпораций это легко управляемое, масштабируемое, весьма доступное решение с высоким уровнем безопасности для работы веб-сайтов. Если вам нужно настроить веб-сайт, лучше всего начать с веб-сайтов Azure и продолжить облачными службами, пока не потребуется функция, которой нет в веб-сайтах. Дополнительные ссылки, которые могут помочь выбрать нужный вариант, см. в разделе «Вычисления».

### Облачные службы

![Облачная служба Azure][]

 **Облачные службы Azure предоставляют место для запуска высокомасштабируемого пользовательского кода в среде PaaS**
...
![База данных SQL хранилища Azure ][]

**База данных SQL Azure предоставляет управляемую службу реляционной базы данных в облаке.**
...
**Таблицы Azure предоставляют неструктурированный способ NoSQL хранения данных.**
...
**Рис. BLOB-объекты Azure предоставляют неструктурированные двоичные данные.**
...
**Рис. Импорт и экспорт Azure предоставляет возможность передавать физический жесткий диск в службу Azure и из нее, используя более быстрый и более дешевый импорт и экспорт пакетных данных.**
...
-   **Миграция крупных данных**. Если вам требуется отправить большое количество данных (терабайты) в Azure, служба импорта и экспорта часто гораздо быстрее и дешевле, чем передача по Интернету. После того как данные появятся в BLOB-объектах, можно обработать их в другие формы, например хранилище таблиц или база данных SQL.

-   **Восстановление заархивированных данных**. Вы можете использовать службу импорта и экспорта, чтобы Майкрософт перемещала большие объемы данных из хранилища BLOB-объектов Azure на устройство хранилища, которое вы отправите и получите обратно в нужное расположение. Поскольку это занимает некоторое время, данный вариант не подходит для аварийного восстановления. Лучше всего архивировать данные, к которым не потребуется быстрый доступ.

### Служба файлов Azure

![Служба файлов Azure][]

**Рис. Службы файлов Azure предоставляют пути \\\\server\\share SMB для приложений, запущенных в облаке.**
...
## <span id="networking"></span></a>Сети
 ...
 **Рис. Виртуальные сети предоставляют частную сеть в облаке, чтобы разные службы могли взаимодействовать друг с другом или с локальными ресурсами, если вы настроите VPN-подключение или подключение между организациями.**
...
Дополнительные сведения о планировании и создании виртуальной сети, подходящей вам, см. в разделе [Виртуальная сеть][].
...
Дополнительные сведения об ExpressRoute см. в разделе [Общие технические сведения об ExpressRoute][].
...
## <span id="DevService"></span></a>Службы разработки
...
### Azure SDK и инструменты Azure для Visual Studio
...
Visual Studio Online также предлагает службу, называющуюся Application Insights, которая предоставляет анализ всего приложения. Она показывает статистику производительности и сведения об использовании приложения. Если вы уже используете System Center Operations Manager, она может подключиться и отправлять оповещения при возникновении проблем.
...
## <span id="identity"></span></a>Удостоверение и доступ
...
**Рис.**
Безопасность всегда важна. Multi-Factor Authentication (MFA) гарантирует, что только сами пользователи могут получить доступ к своей учетной записи. MFA (также известная как двухфакторная проверка подлинности, или «2FA») запрашивает два из трех методов проверки удостоверений пользователей для входа и транзакций.
...
**Сценарии MFA**
Защита входа в учетные записи с конфиденциальными сведениями, такими как банковские имена пользователей и доступ к исходному коду, где неавторизованный доступ может привести к высоким финансовым затратам или затратам интеллектуальной собственности.

## <span id="Mobile"></span></a>Мобильные приложения
 ...
 **Рис. Мобильные службы предоставляют функции, обычно необходимые приложениям, предназначенным для мобильных устройств.**

Мобильные службы Azure предоставляют множество полезных функций, которые могут сэкономить ваше время при создании серверной части для мобильного приложения. Они позволяют выполнять простую подготовку данных, хранящихся в базе данных SQL, и управление ими. С серверным кодом вы можете легко использовать дополнительные варианты хранилища данных, такие как хранилище BLOB-объектов или MongoDB. Мобильные службы предоставляют поддержку уведомлений, однако в некоторых случаях вместо этого можно использовать концентраторы уведомлений, как описано далее. Служба также включает интерфейс API REST, который может вызвать мобильное приложение. Мобильные службы также предоставляют возможность проверки подлинности пользователей в Майкрософт и Active Directory, а также у других известных поставщиков удостоверений, например Facebook, Twitter и Google.
 ...
 **Рис. Концентраторы уведомлений предоставляют функции, обычно необходимые приложениям, предназначенным для мобильных устройств.**

Хотя вы можете написать код для отправки уведомлений в мобильных службах Azure, концентраторы уведомлений оптимизированы для передачи миллионов персонализированных push-уведомлений за минуты. Вам не нужно беспокоиться о таких деталях, как мобильный оператор или изготовитель устройства. Вы можете указать одного или миллионы пользователей с помощью одного вызова API.

Концентраторы уведомлений предназначены для работы с любой серверной частью. Вы можете использовать мобильные службы Azure, пользовательскую серверную часть в облаке, запущенную на любом поставщике или на локальном сервере.

**Сценарии концентраторов уведомлений**
Если бы вы создавали мобильную игру, где игроки ходят по очереди, вам потребовалось бы уведомить второго игрока о том, что первый игрок закончил ход. Если это все, что требуется, можно просто использовать мобильные службы. Но если в игре находятся 100 000 пользователей и вы хотите отправить бесплатное, зависимое от времени предложение всем, лучше выбрать концентраторы уведомлений.
...
## <span id="Backup"></span></a>Резервное копирование
...
Azure Site Recovery (ранее диспетчер восстановления Hyper-V) может помочь защитить важные приложения, координируя репликацию и восстановление образов Hyper-V на сайтах. Вы можете выполнить резервное копирование на вторичный сайт, сайт поставщика услуг размещения или использовать Azure и избежать затрат и сложностей в создании и управлении собственным вторичным расположением. Azure шифрует данные и сообщения; кроме того, можно включить шифрование для хранящихся данных.
...
Site Recovery работает с существующими технологиями, такими как реплика Hyper-V, System Center и SQL Server AlwaysOn.

### Служба архивации Azure

![Служба архивации Azure][]

**Рис. Служба архивации Azure выполняет резервное копирование данных из локальных Windows Server в облако**
Служба архивации Azure выполняет резервное копирование данных из локальных серверов, запускающих Windows Server в облаке. Вы можете управлять резервными копиями непосредственно в средствах резервного копирования в Windows Server 2012, Windows Server 2012 Essentials или System Center 2012 — Data Protection Manager. Кроме того, вы можете использовать специальный агент резервного копирования.
...
Если вы уже используете Windows Server или System Center, служба архивации Azure — подходящее решение для резервного копирования файловой системы сервера, виртуальных машин и баз данных SQL Server. Она работает с зашифрованными, разреженными и сжатыми файлами. Существуют некоторые ограничения, поэтому следует сначала [проверить предварительные требования службы архивации Azure][].

## <span id="messaging"></span></a>Обмен сообщениями и интеграция
...
Идея организации очереди довольно проста: одно приложение помещает в очередь сообщение, которое в итоге считывается другим приложением. Если вашему приложению достаточно такого прямолинейного механизма, оптимальным выбором может стать такой компонент Azure, как Очереди.

Поскольку служба Azure со временем выросла, служба очередей хранилища Azure и служба очередей служебной шины предоставляют аналогичные службы очередей. Причины использования той или иной службы охватываются в техническом документе [<link>][]. В большинстве сценариев можно использовать обе службы.
...
![Ретранслятор служебной шины Azure][]

**Рис. Ретранслятор служебной шины разрешает взаимодействие между приложениями с разных сторон брандмауэра.**
Служебная шина разрешает непосредственное взаимодействие в реляционной службе, предоставляя безопасный способ взаимодействия между брандмауэрами. Ретрансляции Service Bus позволяют приложениям взаимодействовать путем обмена сообщениями через конечную точку, размещенную в облаке, а не локально.
 ...
 **Рис. Разделы служебной шины позволяют нескольким приложениям публиковать сообщения и другие приложения, чтобы подписаться на получение сообщений, соответствующих определенным критериям.**
...
**Сценарии ретранслятора служебной шины**
 ...
 **Рис. Службы BizTalk предоставляют возможность преобразования форматов XML-сообщений в облаке.**
...
**Планировщик Azure предоставляет способ планирования заданий в определенное время и в течение заданной длительности.**
...
## <span id="Performance"></span></a>Производительность
 ...
 **Рис. Приложение Azure может кэшировать данные в памяти и даже разделять их на несколько рабочих ролей**
 ...
 **Рис. Копии BLOB-объекта можно кэшировать на сайтах по всему миру.**
...
**Сценарии CDN**
CDN обычно используют со службами мультимедиа для поставки видео по всему миру. Видео обычно большие и требуют большой пропускной способности. Сведения о службах мультимедиа см. далее на этой странице.

## <span id="BigStuff"></span></a>Большие данные и большие вычисления

### HDInsight (Hadoop)

![HDInsight][]

 **Рис. HDInsight помогает с массовой обработкой больших объемов данных**
В течение многих лет массовый анализ данных выполнялся в реляционных данных, сохраненных в хранилище данных, которое было создано с помощью реляционной СУБД. Этот тип бизнес-аналитики до сих пор сохраняет свою важность и будет применяться еще очень долго. Но что делать, если объем анализируемых данных настолько велик, что реляционные базы данных не способны обработать его? И предположим, что эти данные не являются реляционными? Это могут быть серверные журналы в центре обработки данных, полученные с датчиков данные об имевших место событиях или что-то еще. В таких случаях вы сталкиваетесь с так называемой проблемой большого объема данных. Вам требуется использовать иной подход.
...
Один из самых привлекательных способов использования облачной платформы — это запустить высокопроизводительные вычислительные системы (HPC) и другие приложения больших вычислений. Примеры включают специальные инженерные приложения, созданные для использования стандартного MPI, а также так называемые «приложения с усложненным параллелизмом», например модели финансового риска.
...
## <span id="media"></span></a>Службы мультимедиа

![Службы мультимедиа Azure][]

 **Рис. Службы мультимедиа — это платформа для приложений, которые предоставляют клиентам по всему миру видео и мультимедиа.**
...
Чтобы более наглядно ознакомиться с работой служб мультимедиа, загрузите [афишу служб мультимедиа Azure][].

## <span id="commerce"></span></a>Коммерческие предложения

Возникновение службы Software отражает трансформацию подхода к созданию приложений. Она также изменяет то, как мы продаем приложения. Поскольку приложение SaaS живет в облаке, разумно, что потенциальные клиенты будут искать решения в Интернете. И это изменение касается не только приложений, но и данных. Почему бы людям не обращаться в облако за наборами данных, доступными на коммерческой основе? Майкрософт решает обе эти проблемы с помощью [Azure Marketplace][] и [Магазина Azure][].

![Коммерческие предложения Azure][]

 **Рис. В Azure Marketplace и Магазине Azure можно найти и купить приложения Azure и коммерческие наборы данных, чтобы использовать их в рамках приложений Azure.**
...
## <span id="start"></span></a>Начало работы

Теперь, когда вы получили общее представление, следующим шагом будет написание вашего первого приложения Azure. Выберите язык, [получите соответствующий SDK][] и начинайте работу. Облачные вычисления предоставляют новый вариант по умолчанию — начинать работу сразу.

  [Компоненты Azure]: #components
  [Портал управления]: #portal
  [Среда выполнения приложений]: #compute
  [Управление данными]: #data
  [Работа в сети]: #networking
  [Службы разработки]: #DevService
  [Удостоверение и доступ]: #identity
  [Мобильные приложения]: #mobile
  [Архивация]: #backup
  [Обмен сообщениями и интеграция]: #messaging
  [Помощь в вычислении]: #ComputeAssist
  [Производительность]: #Performance
  [Большие вычисления и данные]: #BigStuff
  ...
  [Облачная служба Azure]: ./media/intro-to-azure/CloudServicesIntroNew.png
  [База данных SQL хранилища Azure ]: ./media/intro-to-azure/StorageAzureSQLDatabaseIntroNew.png
  [Таблицы хранилища Azure ]: ./media/intro-to-azure/StorageTablesIntroNew.png
  [BLOB-объекты хранилища Azure]: ./media/intro-to-azure/StorageBlobsIntroNew.png
  [Служба импорта и экспорта Azure]: ./media/intro-to-azure/ImportExportIntroNew.png
  [Служба файлов Azure]: ./media/intro-to-azure/FileServiceIntroNew.png
  [VirtualNetwork]: ./media/intro-to-azure/VirtualNetworkIntroNew.png
  [Виртуальная сеть]: http://msdn.microsoft.com/library/azure/jj156007.aspx
  [ExpressRoute]: ./media/intro-to-azure/ExpressRouteIntroNew.png
  [Общие технические сведения об ExpressRoute]: http://msdn.microsoft.com/en-us/library/azure/dn606309.aspx
  ...
  [Служба архивации Azure]: ./media/intro-to-azure/AzureBackupIntroNew.png
  [проверить предварительные требования службы архивации Azure]: http://technet.microsoft.com/en-us/library/dn296608.aspx
  [<link>]: http://msdn.microsoft.com/library/azure/hh767287.aspx "Очереди Azure и очереди служебной шины — сравнение и противопоставление"
  [Ретранслятор служебной шины Azure]: ./media/intro-to-azure/ServiceBusRelayIntroNew.png
  [Разделы служебной шины Azure]: ./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png
  [Службы BizTalk]: ./media/intro-to-azure/BizTalkServIntroNew2.png
  [Планировщик Azure ]: ./media/intro-to-azure/SchedulerIntroNew2.png
  [Кэширование Azure]: ./media/intro-to-azure/AzureCacheIntroNew.png
  ...
  [получите соответствующий SDK]: /en-us/downloads/
