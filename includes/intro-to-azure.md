...
-   [Компоненты Azure][Компоненты Azure]
-   [Портал управления][Портал управления]
-   [Среда выполнения приложений][Среда выполнения приложений]
-   [Управление данными][Управление данными]
-   [Работа в сети][Работа в сети]
-   [Службы разработки][Службы разработки]

**Дополнительные службы**

-   [Удостоверение и доступ][Удостоверение и доступ]
-   [Мобильные приложения][Мобильные приложения]
-   [Архивация][Архивация]
-   [Обмен сообщениями и интеграция][Обмен сообщениями и интеграция]
-   [Помощь в вычислении][Помощь в вычислении]
-   [Производительность][Производительность]
-   [Большие вычисления и данные][Большие вычисления и данные]
-   [Служба мультимедиа][Служба мультимедиа]
-   [Приложения для коммерции][Приложения для коммерции]

[Приступая к работе][Приступая к работе]

## <span id="components"></span></a>Компоненты Azure

Azure группирует службы по категориям на портале управления и по разным вспомогательным визуальным элементам, таким как [Инфографика «Что такое Windows Azure»][Инфографика «Что такое Windows Azure»]. Портал управления используется для управления большинством служб в Azure (но не всеми).
...
![Компоненты Azure][1]
 **Рис. Azure предоставляет службы приложений, доступных в Интернете, выполняющихся в центрах данных Azure.**

## <span id="portal"></span></a>Портал управления

Azure содержит веб-интерфейс, который называется [Портал управления][2]. Он позволяет администраторам получить доступ к большинству функций Azure (но не ко всем) и управлять ими. Обычно перед прекращением действия портала Майкрософт выпускает бета-версию нового портала пользовательского интерфейса. Более новый портал называется [портал предварительной версии Azure][портал предварительной версии Azure].
...
**Рис. Виртуальные машины Azure предоставляют полный контроль над экземплярами виртуальных машин в облаке.**
 ...
 **Рис. Веб-сайты Azure запускают приложение веб-сайта в облаке без необходимости управления базовым веб-сервером.**
...
![Облачная служба Azure Azure][Облачная служба Azure Azure]
**Рис. Облачные службы Azure предоставляют место для запуска высокомасштабируемого пользовательского кода в среде PaaS**
...
![База данных SQL Database хранилища Azure][База данных SQL Database хранилища Azure]
**Рис. В базе данных Azure SQL функционирует управляемая служба реляционной базы данных в облаке.**
...
![Таблицы хранилища Azure][Таблицы хранилища Azure]
**Рис. Таблицы Azure обеспечивают неструктурированный способ хранения данных NoSQL.**
...
![BLOB-объекты хранилища Azure][BLOB-объекты хранилища Azure]
...
![Служба импорта и экспорта Azure][Служба импорта и экспорта Azure]
...
-   **Миграция крупных данных.** Если вам требуется отправить большое количество данных (терабайты) в Azure, служба импорта и экспорта может оказаться более быстрым и дешевым способом, чем передача данных по Интернету. После того как данные появятся в BLOB-объектах, можно обработать их в другие формы, например хранилище таблиц или база данных SQL.

-   **Восстановление заархивированных данных.** Вы можете использовать службу импорта и экспорта Майкрософт для перемещения больших объемов данных из хранилища BLOB-объектов Azure на предоставленное вами устройство хранения, которое вы получите обратно по указанному вами адресу. Поскольку это занимает некоторое время, данный вариант не подходит для аварийного восстановления. Лучше всего архивировать данные, к которым не потребуется быстрый доступ.

### Служба файлов Azure

![Служба файлов Azure][Служба файлов Azure]
...
Дополнительные сведения о планировании и создании виртуальной сети, подходящей вам, см. в разделе [Виртуальная сеть][3].
...
Дополнительные сведения об ExpressRoute см. в разделе [Общие технические сведения об ExpressRoute][Общие технические сведения об ExpressRoute].
...
![Многофакторная проверка подлинности Azure][Многофакторная проверка подлинности Azure]
**Рис. Многофакторная проверка подлинности позволяет вашим приложениям проверять несколько видов идентификации.**
...
![Служба архивации Azure][Служба архивации Azure]
**Рис. Служба архивации Azure позволяет выполнять резервное копирование данных с локально размещенных серверов Windows в облачную среду.**
...
Если вы уже используете Windows Server или System Center, служба архивации Azure — подходящее решение для резервного копирования файловой системы сервера, виртуальных машин и баз данных SQL Server. Она работает с зашифрованными, разреженными и сжатыми файлами. Существуют некоторые ограничения, поэтому следует сначала [проверить предварительные требования службы архивации Azure][проверить предварительные требования службы архивации Azure].
...
![Ретранслятор служебной шины Azure][Ретранслятор служебной шины Azure]
**Рис. Очереди создают слабые взаимозависимости между частями приложения и облегчают масштабирование.**
...
Поскольку служба Azure со временем выросла, служба очередей хранилища Azure и служба очередей служебной шины предоставляют аналогичные службы очередей. Причины использования той или иной службы охватываются в техническом документе [<link>][<link>]. В большинстве сценариев можно использовать обе службы.
...
![Ретранслятор служебной шины Azure][4]
**Рис. Ретранслятор служебной шины позволяет осуществлять обмен данными между приложениями, разделенными брандмауэром.**

Service Bus обеспечивает прямую связь через службу ретрансляции, предоставляя защищенный метод взаимодействия через брандмауэры. Ретрансляции Service Bus позволяют приложениям взаимодействовать путем обмена сообщениями через конечную точку, размещенную в облаке, а не локально.
...
![Разделы служебной шины Azure][Разделы служебной шины Azure]
...
![Планировщик Azure][Планировщик Azure]
**Рис. Планировщик Azure позволяет планировать задания в определенное время и на определенный период.**
...
![Кэширование Azure][Кэширование Azure]
...
![Службы мультимедиа Azure][Службы мультимедиа Azure]
...
Теперь, когда вы получили общее представление, следующим шагом будет написание вашего первого приложения Azure. Выберите язык, [получите соответствующий SDK][получите соответствующий SDK] и начинайте работу. Облачные вычисления предоставляют новый вариант по умолчанию — начинать работу сразу.
  ...
  [Облачная служба Azure Azure]: ./media/intro-to-azure/CloudServicesIntroNew.png
  [База данных SQL Database хранилища Azure]: ./media/intro-to-azure/StorageAzureSQLDatabaseIntroNew.png
  ...
  [3]: http://msdn.microsoft.com/library/azure/jj156007.aspx
  [ExpressRoute]: ./media/intro-to-azure/ExpressRouteIntroNew.png
  [Общие технические сведения об ExpressRoute]: http://msdn.microsoft.com/ru-ru/library/azure/dn606309.aspx
  [TrafficManager]: ./media/intro-to-azure/TrafficManagerIntroNew.png
  [www.microsoftazure.com]: http://azure.microsoft.com/ru-ru/downloads/
  [Многофакторная проверка подлинности Azure]: ./media/intro-to-azure/MFAIntroNew.png
  ...
  [проверить предварительные требования службы архивации Azure]: http://technet.microsoft.com/ru-ru/library/dn296608.aspx
  [Ретранслятор служебной шины Azure]: ./media/intro-to-azure/QueuesIntroNew.png
  ...
  [4]: ./media/intro-to-azure/ServiceBusRelayIntroNew.png
  [Разделы служебной шины Azure]: ./media/intro-to-azure/ServiceBusTopicsSubsIntroNew.png
  [Службы BizTalk]: ./media/intro-to-azure/BizTalkServicesIntroNew.png
  [Планировщик Azure]: ./media/intro-to-azure/SchedulerIntroNew.png
  ...
  [получите соответствующий SDK]: /ru-ru/downloads/
