...
Так как Azure предусматривает множество разных ресурсов и их типов, эталонную архитектуру можно развернуть различными способами. Дополнительные сведения о развертывании архитектуры, как показано на рисунке выше, см. в разделе [Развертывание решения](#solution-deployment) в конце этой статьи. Если у вас нет определенных противоречащих требований, следуйте этим рекомендациям при создании собственной эталонной архитектуры.

### <a name="vm-recommendations"></a>Рекомендации по виртуальным машинам

Azure предлагает виртуальные машины различных размеров, но мы советуем использовать виртуальные машины серий DS и GS, так как они поддерживают [хранилище класса Premium][premium-storage]. Если отсутствует специализированная рабочая нагрузка, например высокопроизводительные вычисления, выберите виртуальную машину одного из этих размеров. Дополнительные сведения см. в статье [Размеры виртуальных машин в Azure][virtual-machine-sizes]. 

При перемещении имеющейся рабочей нагрузки в Azure выберите начальный размер виртуальной машины, который больше всего соответствует вашим локальным серверам. Затем измерьте производительность фактической рабочей нагрузки по таким ресурсам, как потребление ЦП, памяти и дисковых операций ввода-вывода в секунду, и при необходимости измените размер виртуальной машины. Если требуется несколько сетевых интерфейсов для виртуальной машины, помните, что их максимально возможное количество зависит от [размера виртуальной машины][vm-size-tables].   
...
Подготовьте достаточно учетных записей хранения для виртуальных машин, которые планируется развернуть. Каждой виртуальной машине требуется диск ОС и временный диск. Исходя из [ограничений для дисков учетной записи хранения][storage-account-limits], одна учетная запись уровня "Премиум" может поддерживать от 1 до 20 виртуальных машин. Диски данных, добавляемые к виртуальной машине во время создания, не форматируются. Чтобы отформатировать эти диски, необходимо выполнить вход в виртуальную машину.
...
* Зарезервируйте [статический IP-адрес][static-ip], если вам нужен постоянный IP-адрес (например, для создания записи А в DNS или добавления IP-адреса в список надежных адресов).
...
В репозитории GitHub доступен [шаблон развертывания][github-folder] эталонной архитектуры, реализующей эти рекомендации. В состав этой эталонной архитектуры входит виртуальная сеть (VNet), группа безопасности сети (NSG) и одна виртуальная машина.

Ее можно развернуть несколькими способами. Проще всего выполнить следующие действия. 
...
Дополнительные сведения о других методах развертывания эталонной архитектуры см. в файле сведений в папке [guidance-single-vm][github-folder]] в репозитории GitHub. 
