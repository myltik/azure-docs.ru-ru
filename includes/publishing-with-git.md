...
<h2><a id="Step1"></a>Шаг 1. Установка Git</h2>
...
Веб-приложения поддерживают приложения, созданные с использованием различных языков программирования. В этом примере будет использоваться статический HTML-файл. Информацию о публикации в Azure веб-приложений, созданных с использованием других языков программирования, см. в [Центре разработчиков Azure].
...
Чтобы включить репозиторий Git для веб-приложения, на [портале Azure](http://go.microsoft.com/fwlink/?LinkId=529715) выполните следующие действия:
...
4. Если репозиторий в Azure настраивается вами впервые, потребуется создать для него учетную запись. Вы будете использовать данные этой учетной записи для входа в репозиторий Azure и отправки изменений из локального репозитория Git. В колонке веб-приложения щелкните **Настройка учетных данных развертывания**, а затем настройте имя пользователя и пароль развертывания. Закончив, нажмите кнопку **OK**.
...
2. Используйте удаленный репозиторий Git для добавления внешней ссылки, указанной в пункте **URL-адрес Git** на шаге 1. Команда будет выглядеть примерно следующим образом:
