...
В этой статье описывается использование Git для выполнения публикации непосредственно с локального компьютера на веб-сайт Azure (в Azure такой способ публикации называется **Локальный репозиторий Git**). Вы также узнаете, как включить непрерывное развертывание с таких веб-сайтов репозиториев, как BitBucket, CodePlex, Dropbox, GitHub или Mercurial. Информацию об использовании TFS для непрерывного развертывания см. в разделе [Непрерывная доставка в Azure с помощью Visual Studio Online][Непрерывная доставка в Azure с помощью Visual Studio Online].

> [WACOM.NOTE] Многие команды Git, описанные в этой статье, автоматически выполняются при создании веб-сайта с помощью [средств командной строки Azure для Mac и Linux][средств командной строки Azure для Mac и Linux].

Эта задача включает следующие шаги:

-   [Установка Git][Установка Git]
-   [Создание локального репозитория][Создание локального репозитория]
-   [Добавление веб-страницы][Добавление веб-страницы]
-   [Включение репозитория веб-сайта][Включение репозитория веб-сайта]
-   [Развертывание проекта][Развертывание проекта]

    -   [Принудительная отправка локальных файлов в Azure (локальный репозиторий Git)][Принудительная отправка локальных файлов в Azure (локальный репозиторий Git)]
    -   [Развертывание файлов с веб-сайта репозиториев, например BitBucket, CodePlex, Dropbox, GitHub или Mercurial][Развертывание файлов с веб-сайта репозиториев, например BitBucket, CodePlex, Dropbox, GitHub или Mercurial]
    -   [Развертывание решения Visual Studio из BitBucket, CodePlex, Dropbox, GitHub или Mercurial][Развертывание решения Visual Studio из BitBucket, CodePlex, Dropbox, GitHub или Mercurial]
-   [Устранение неполадок][Устранение неполадок]

## <span id="Step2"></span></a>Установка Git

Действия, необходимые для установки Git, отличаются в разных операционных системах. Сведения о дистрибутивах для разных операционных систем и руководство по установке см. в разделе [Установка Git][1].

> [WACOM.NOTE] В некоторых операционных системах доступны обе версии Git: для командной строки и с графическим интерфейсом. В инструкциях, приведенных в данной статье, используется версия для командной строки.

## <span id="Step2"></span></a>Создание локального репозитория
...
## <span id="Step3"></span></a>Добавление веб-страницы

Веб-сайты Azure поддерживают приложения, созданные с использованием различных языков программирования. В этом примере будет использоваться статический HTML-файл. Сведения о публикации веб-сайтов, созданных с использованием других языков программирования в Azure, см. в [Центре разработчиков Azure][Центре разработчиков Azure].
...
3.  Проверьте в командной строке свое нахождение в корневой папке репозитория Git. Затем выполните следующую команду для добавления файла **index.html** в репозиторий:
    ...
    > [WACOM.NOTE] Справку по любой команде Git можно получить по расширениям -help или --help после строки команды. Например, для получения справочных сведений о параметрах для команды add введите "git add -help" или "git add --help" для подробной справки.
...
## <span id="Step4"></span></a>Включение репозитория веб-сайта
...
3.  Дождитесь окончания процесса создания веб-сайта в представлении **Веб-сайты** и выберите этот веб-сайт.

    ![Изображение, на котором показан выбранный веб-сайт][Изображение, на котором показан выбранный веб-сайт]

4.  Выберите вкладку **ПАНЕЛЬ МОНИТОРИНГА**.

5.  В разделе **Сводка** выберите **Настроить развертывания в системе управления версиями**. Появится диалоговое окно **НАСТРОЙКА РАЗВЕРТЫВАНИЯ**.

    ![git-WhereIsYourSourceCode][git-WhereIsYourSourceCode]

6.  Выберите **Локальный репозиторий Git**, а затем щелкните стрелку **Далее**.
...
## <span id="Step5"></span></a>Развертывание проекта

### <span id="Step6"></span></a>Принудительная отправка локальных файлов в Azure (локальный репозиторий Git)
...
3.  Введите следующую команду в командной строке для передачи содержимого текущего репозитория из локального репозитория в удаленный репозиторий "azure":
    ...
    > [WACOM.NOTE] Репозиторий, созданный для вашего веб-сайта Azure, ожидает push-запросы для **главной** ветви репозитория, который затем будет использоваться в качестве контента данного веб-сайта.

4.  На портале щелкните ссылку **ОБЗОР** в нижней части портала, чтобы убедиться, что файл **index.html** был развернут. Появится страница с текстом "Hello Git!".

    ![Веб-страница с текстом "Hello Git!"][Веб-страница с текстом "Hello Git!"]

5.  С помощью текстового редактора измените файл **index.html**, чтобы он содержал текст "Yay!", а затем сохраните его.

6.  Используйте следующие команды в командной строке, чтобы **добавить** и **применить** изменения, а затем **передать** эти изменения в удаленный репозиторий:
    ...
    ![Веб-страница с текстом "Yay!"][Веб-страница с текстом "Yay!"]

### <span id="Step7"></span></a>Развертывание файлов с веб-сайта репозиториев, например BitBucket, CodePlex, Dropbox, GitHub или Mercurial
...
Хотя оба способа обеспечивают развертывание вашего проекта на веб-сайте Azure, однако непрерывное развертывание лучше, когда в работе над проектом участвуют несколько человек и требуется поддержка актуальной версии независимо от того, кто внес последнее обновление. Непрерывное развертывание удобно также при использовании одного из вышеупомянутых средств в качестве центрального репозитория для вашего приложения.

Развертывание файлов из GitHub, CodePlex или BitBucket требует, чтобы локальный проект был опубликован в одной из этих служб. Дополнительные сведения о публикации проекта в этих службах см. в разделах [Создание репозитория (GitHub)][Создание репозитория (GitHub)], [Использование Git с CodePlex][Использование Git с CodePlex], [Создание репозитория (BitBucket)][Создание репозитория (BitBucket)], [Использование Dropbox для общего доступа к репозиториям Git][Использование Dropbox для общего доступа к репозиториям Git] или [Краткое руководство — Mercurial][Краткое руководство — Mercurial]
...
3.  Выберите метод управления версиями, который будет использоваться для непрерывного развертывания.

4.  В ответ на запрос введите ваши учетные данные для выбранной службы.

5.  После авторизации Azure и входа в учетную запись появится запрос со списком репозиториев.

    ![git-ChooseARepositoryToDeploy][git-ChooseARepositoryToDeploy]

6.  Выберите репозиторий, который требуется связать с вашим веб-сайтом Azure. Установите флажок для продолжения.

    > [WACOM.NOTE] При включении непрерывного развертывания с использованием GitHub или BitBucket будут отображаться проекты и с публичным и с частным доступом.

7.  Azure создаст связь с выбранным репозиторием и получит файлы из главной ветви. После завершения этого процесса в **истории развертывания** на странице **Развертывания** будет отображено сообщение **Активное развертывание**, аналогичное следующему:

    ![git-githubdeployed][git-githubdeployed]

8.  Теперь ваш проект развернут из выбранного репозитория на вашем веб-сайте Azure. Чтобы убедиться, что веб-сайт активен, щелкните ссылку **Обзор** в нижней части портала. Браузер должен перейти на этот веб-сайт.

9.  Для проверки непрерывного развертывания внесите изменение в проект, а затем перешлите это обновление в репозиторий, связанный с данным веб-сайтом. Ваш веб-сайт должен обновиться в соответствии с этими изменениями вскоре после их пересылки в репозиторий. Проверить получение сайтом данного обновления можно на странице **Развертывания** вашего веб-сайта.

    ![git-GitHubDeployed-Updated][git-GitHubDeployed-Updated]

### <span id="Step75"></span></a>Развертывание решения Visual Studio из BitBucket, CodePlex, Dropbox, GitHub или Mercurial

Переслать решение Visual Studio на веб-сайт Azure так же просто, как отправить простой файл index.html. Процесс развертывания веб-сайтов Azure упрощает выполнение всех операций, включая восстановление зависимостей NuGet и создание двоичных файлов приложения. Вы можете воспользоваться практическими рекомендациями по управлению версиями в репозитории Git, оставив все остальные операции средствам развертывания веб-сайтов Azure.

Операции по пересылке решения Visual Studio на веб-сайт Azure такие же, как и описанные в [предыдущем разделе][Развертывание файлов с веб-сайта репозиториев, например BitBucket, CodePlex, Dropbox, GitHub или Mercurial], при условии, что решение и репозиторий настроены следующим образом.

-   В корневую папку репозитория добавьте файл `.gitignore`, а затем укажите все файлы и папки, которые нужно исключить из репозитория, например папки `Obj`, `Bin` и `packages` (см. информацию о формате в [документации по gitignore][документации по gitignore]). Например:
    ...
    > [WACOM.NOTE] При использовании GitHub при создании репозитория по необходимости можно создать специфичный для Visual Studio файл .gitignore, куда будут включены все обычные временные файлы, результаты построения и т. д. Этот файл можно будет изменить, приспособив под конкретные нужды.

-   Добавьте в репозиторий все дерево каталогов решения с файлом .sln в корневой папке репозитория.

-   В решении Visual Studio [включите восстановление пакетов NuGet][включите восстановление пакетов NuGet], чтобы Visual Studio автоматически восстанавливала недостающие пакеты.
...
#### Как реализуется непрерывное развертывание
...
Дополнительные сведения о механизме процесса развертывания Git для веб-сайтов Azure см. на сайте [Project Kudu][Project Kudu].

#### Указание ветви для использования
...
**Симптом**. Ошибка — изменения в удаленном репозитории внесены, но веб-сайт не обновился.
...
-   **Собственный модуль, не имеющий двоичного дистрибутива для Windows**:
    ...
    -   npm ERR! [<modulename@version>] preinstall (предустановка [имя\_модуля@версия][имя\_модуля@версия]]): \`make || gmake\`

## Дополнительные ресурсы

-   [Использование PowerShell для Azure][Использование PowerShell для Azure]
-   [Использование средств командной строки Azure для Mac и Linux][средств командной строки Azure для Mac и Linux]
-   [Документация по Git][Документация по Git]
-   [Project Kudu][Project Kudu]

  [Непрерывная доставка в Azure с помощью Visual Studio Online]: http://www.windowsazure.com/ru-ru/develop/net/common-tasks/publishing-with-tfs/
  [средств командной строки Azure для Mac и Linux]: /ru-ru/develop/nodejs/how-to-guides/command-line-tools/
  [Установка Git]: #Step1
  ...
  [Центре разработчиков Azure]: http://www.windowsazure.com/ru-ru/develop/overview/
  ...
  [документации по gitignore]: http://git-scm.com/docs/gitignore
  [включите восстановление пакетов NuGet]: http://docs.nuget.org/docs/workflows/using-nuget-without-committing-packages
  [git-DeploymentTrigger]: ./media/publishing-with-git/git-DeploymentTrigger.png
  [Project Kudu]: https://github.com/projectkudu/kudu/wiki
  [git-DisconnectFromGitHub]: ./media/publishing-with-git/git-DisconnectFromGitHub.png
  [имя\_модуля@версия]: mailto:modulename@version
