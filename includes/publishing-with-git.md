[Служба приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714) поддерживает непрерывное развертывание из таких инструментов управления исходным кодом и репозиториев, как GitHub, BitBucket и Visual Studio Team Services. Вы можете использовать эти инструменты для хранения содержимого и кода своего приложения, а затем быстро и легко отправлять изменения в приложение службы приложений.

В этой статье описывается использование Git для выполнения публикации непосредственно с локального компьютера в веб-приложения (в Azure такой способ публикации называется **локальным репозиторием Git**). Вы также узнаете, как включить непрерывное развертывание с таких сайтов репозиториев, как BitBucket и GitHub. Сведения об использовании VSTS для непрерывного развертывания см. в статье [Непрерывная доставка в Azure с помощью Visual Studio Team Services].

> [!NOTE]
> Многие команды Git, описанные в этой статье, автоматически выполняются при создании веб-приложения с помощью [программ командной строки Azure для Mac и Linux](/develop/nodejs/how-to-guides/command-line-tools/).
> 
> 
...
3. Проверьте в командной строке свое нахождение в корневой папке репозитория Git. Затем выполните следующую команду для добавления файла **index.html** в репозиторий:
   ...
   > [!NOTE]
   > Справку по любой команде Git можно найти, введя "-help" или "--help" после команды. Например, для получения справочных сведений о параметрах для команды add введите "git add -help" для вызова справки в командной строке или "git add --help" для подробной справки.
   > 
   > 
...
## <a id="Step4"></a>Включение репозитория для веб-приложения
Чтобы включить репозиторий Git для веб-приложения, сделайте следующее:

1. Войдите на [портал Azure].
2. В колонке веб-приложения щелкните **Параметры > Непрерывное развертывание**. Щелкните **Выбор источника**, затем щелкните **Локальный репозиторий Git** и нажмите кнопку **OK**.
   
    ![Локальный репозиторий Git](./media/publishing-with-git/azure1-local-git.png)
3. Если репозиторий в Azure настраивается вами впервые, потребуется создать для него учетную запись. Вы будете использовать данные этой учетной записи для входа в репозиторий Azure и отправки изменений из локального репозитория Git. В колонке веб-приложения щелкните **Параметры > Учетные данные развертывания**, а затем настройте имя пользователя и пароль развертывания. Закончив, нажмите кнопку **OK**.
...
1. В колонке веб-приложения щелкните **Параметры > Свойства** для элемента **URL-адрес Git**.
...
2. В командной строке убедитесь, что находитесь в корневой папке локального репозитория Git, которая содержит ранее созданный файл index.html.
3. Используйте `git remote` для добавления внешней ссылки, указанной в пункте **URL-адрес Git** на шаге 1. Команда будет выглядеть примерно следующим образом:
   ...
   > 
   > 
4. Введите следующую команду в командной строке для передачи содержимого текущего репозитория из локального репозитория в удаленный репозиторий "azure":
   ...
   > [!NOTE]
   > Репозиторий, созданный для вашего веб-приложения, ожидает push-запросы к <strong>главной</strong> ветви репозитория, которая затем будет использоваться в качестве содержимого данного веб-сайта.
   > 
   > 
5. Перейдите к колонке вашего веб-приложения на портале Azure. Вместо сообщения **Развертывание не найдено** должно отображаться сообщение **Активное развертывание **и запись журнала с последним push-запросом.
   
    ![](./media/publishing-with-git/azure4-deployed.png)
6. В верхней части колонки веб-приложения щелкните ссылку под пунктом **URL-адрес**, чтобы убедиться, что **index.html** был развернут. Появится страница с текстом "Hello Git!".
   
    ![Веб-страница с текстом "Hello Git!"][hello-git]
7. С помощью текстового редактора измените файл **index.html**, чтобы он содержал текст "Yay!", а затем сохраните его.
8. Используйте следующие команды в командной строке, чтобы **добавить** и **применить** изменения, а затем **передать** эти изменения в удаленный репозиторий:
...
### <a id="Step7"></a>Развертывание файлов с веб-сайта репозитория, например BitBucket, CodePlex, Dropbox, GitHub или Mercurial
...
2. В колонке веб-приложения на портале щелкните **Параметры > Непрерывная доставка**. Щелкните **Выбор источника** и выберите, например, источник **GitHub**.
   
    ![](./media/publishing-with-git/azure6-setup-github.png)
3. В колонке **Непрерывное развертывание** последовательно щелкните **Авторизация** и **Авторизовать**. Вы будете перенаправлены с портала Azure на сайт репозитория для выполнения авторизации.
4. После этого вернитесь на портал Azure и нажмите кнопку **ОК** в колонке **Авторизация**.
...
1. Теперь ваш проект развернут из выбранного репозитория в вашем веб-приложении. Чтобы убедиться, что веб-приложение активно, щелкните **URL-адрес** в верхней части портала. Браузер должен перейти на это веб-приложение.
2. Чтобы убедиться, что непрерывное развертывание выполняется из выбранного репозитория, отправьте в этот репозиторий изменения. Ваше веб-приложение должно обновиться в соответствии с этим изменением вскоре после выполнения его отправки в репозиторий. Вы можете убедиться в том, что данное обновление получено приложением, в колонке **Развертывания** веб-приложения.
...
* В корневую папку репозитория добавьте файл `.gitignore`, а затем укажите все файлы и папки, которые нужно исключить из репозитория, например папки `Obj`, `Bin` и `packages` (см. информацию о формате в [документации по gitignore](http://git-scm.com/docs/gitignore)). Например:
  ...
  > [!NOTE]
  > В случае использования GitHub при создании репозитория при необходимости можно создать GITIGNORE-файл для Visual Studio, куда будут включены все обычные временные файлы, результаты сборки и т. д. Этот файл можно будет изменить, приспособив под конкретные нужды.
  > 
  > 
...
Непрерывное развертывание можно отключить в колонке **Развертывания**. В колонке веб-приложения щелкните **Параметры > Непрерывное развертывание**. Нажмите кнопку **Отключить**.

![git-DisconnectFromGitHub](./media/publishing-with-git/azure5-disconnect.png)

Ответив **Да** на сообщение о подтверждении, вы можете вернуться к колонке веб-приложения и щелкнуть **Параметры > Непрерывное развертывание**, если требуется настроить публикацию из другого источника.
...
[Непрерывная доставка в Azure с помощью Visual Studio Team Services]: ../articles/cloud-services/cloud-services-continuous-delivery-use-vso.md
