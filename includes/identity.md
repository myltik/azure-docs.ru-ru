...
-   Служба Windows Server Active Directory может работать в облаке на виртуальных машинах Azure. Такой подход имеет смысл при использовании Azure для переноса локального центра обработки данных в облако.
...
-   [Выполнение Windows Server Active Directory на виртуальных машинах][Выполнение Windows Server Active Directory на виртуальных машинах]

-   [Использование Azure Active Directory][Использование Azure Active Directory]

-   [Использование Azure Active Directory Access Control][Использование Azure Active Directory Access Control]

## <a name="adinvm"></a>Выполнение Windows Server Active Directory на виртуальных машинах

Выполнение Windows Server AD на виртуальных машинах Azure очень похоже на выполнение в локальных средах. [На рисунке 1][На рисунке 1] показан типичный пример того, как это выглядит.

![Azure Active Directory на виртуальной машине][Azure Active Directory на виртуальной машине]
...
[На рис. 2][На рис. 2] показан первый из этих вариантов, в котором Windows Azure Active Directory обеспечивает все необходимые возможности.

![Azure Active Directory на виртуальной машине][1]
...
Чтобы использовать Azure AD, пользователь выполняет вход в локальный домен Active Directory, как обычно (шаг 1). Когда пользователь пытается получить доступ к приложению SaaS (шаг 2), в результате действия процесса федерации Azure AD выдает ему маркер для этого приложения (шаг 3). (Дополнительные сведения о действии федерации см. в документе [Удостоверение на основе утверждений для Windows: технологии и сценарии][Удостоверение на основе утверждений для Windows: технологии и сценарии].) Как и раньше, этот маркер содержит данные, идентифицирующие пользователя, а также цифровую подпись Azure AD. Этот маркер отправляется в приложение SaaS (шаг 4), которое проверяет его подпись и использует его содержимое (шаг 5). Как и в предыдущем сценарии, приложение SaaS может использовать Graph API для получения дополнительных сведений о пользователе, если это необходимо (шаг 6).
...
## <a name="ac"></a>Использование Azure Active Directory Access Control
...
Но так ли это необходимо? Почему бы не создать посредника, который генерирует единый формат маркера с общим представлением сведений об удостоверении? Такой подход облегчит жизнь разработчиков, которые создают приложения, так как им нужно будет обрабатывать только один вид маркеров. Именно это и делает Azure Active Directory Access Control, предоставляя облачного посредника для работы с различными маркерами. [На рисунке 4][На рисунке 4] показано, как это работает

![Azure Active Directory in Virtual Machine][2]
<span id="fig4"></span></a>Рис. 4: Azure Active Directory Access Control упрощает для приложений прием маркеров удостоверений, выпускаемых различными поставщиками.
...
Как показано на рисунке, Access Control поддерживает целый ряд облачных поставщиков удостоверений, включая учетные записи, созданные службами Google, Yahoo, Facebook, Майкрософт (ранее известные как Windows Live ID) и любыми поставщиками OpenID. Также поддерживаются удостоверения, созданные с помощью Azure Active Directory, и, посредством федерации с AD FS, Windows Server Active Directory. Целью является поддержка самых распространенных в настоящее время удостоверений, выпускаемых поставщиками в облаке или локально.
...
Дэвид Чаппел (David Chappell) является главой компании Chappell &amp; Associates [www.davidchappell.com][www.davidchappell.com], расположенной в Сан-Франциско, штат Калифорния. Своими выступлениями, публикациями и консультациями

  [Выполнение Windows Server Active Directory на виртуальных машинах]: #adinvm
  [Использование Azure Active Directory]: #ad
  [Использование Azure Active Directory Access Control]: #ac
  [На рисунке 1]: #fig1
  [Azure Active Directory на виртуальной машине]: ./media/identity/identity_01_ADinVM.png
  [см. здесь]: http://msdn.microsoft.com/ru-ru/library/windowsazure/jj156090.aspx
  [На рис. 2]: #fig2
  [1]: ./media/identity/identity_02_AD.png
  [Введение в OData]: http://download.microsoft.com/download/E/5/A/E5A59052-EE48-4D64-897B-5F7C608165B8/IntroducingOData.pdf
  [рис. 3]: #fig3
  [Azure Active Directory in Virtual Machine]: ./media/identity/identity_03_AD.png
  [Удостоверение на основе утверждений для Windows: технологии и сценарии]: http://www.davidchappell.com/writing/white_papers/Claims-Based_Identity_for_Windows_v3.0--Chappell.docx
  [На рисунке 4]: #fig4
