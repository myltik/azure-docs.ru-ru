В этом разделе мы будем изменять модель нашей базы данных посредством добавления нового поля timestamp с именем **CompleteDate**. В этом поле будет записано последнее время, когда был завершен элемент todo. Entity Framework будет обновлять базу данных, на основании изменений нашей модели, используя инициализатор класса базы данных по умолчанию, выводимый из [DropCreateDatabaseIfModelChanges](http://go.microsoft.com/fwlink/?LinkId=394621)

1. В обозревателе решений для Visual Studio разверните папку **App\_Start** в проекте службы списка дел. Откройте файл WebApiConfig.cs.
...
3. В обозревателе решений для Visual Studio разверните папку **DataObjects** в проекте службы списка дел. Откройте файл TodoItem.cs и обновите класс TodoItem для включения поля CompleteDate следующим образом. Затем сохраните файл TodoItem.cs.
