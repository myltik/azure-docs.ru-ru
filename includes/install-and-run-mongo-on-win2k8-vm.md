...
2. Нажмите кнопку **Сервис** в правом верхнем углу.  В окне **Свойства браузера** выберите вкладку **Безопасность**, щелкните значок **Надежные сайты**, а затем нажмите кнопку **Сайты**. Добавьте _http://\*.mongodb.org_ в список надежных сайтов.
...
> [AZURE.NOTE] По умолчанию приложение MongoDB использует порт 27017. Вы можете изменить порт по умолчанию, указав параметр `--port` при запуске сервера mongod.exe. В брандмауэре укажите тот же номер порта, что и для конечной точки Mongo в инструкциях выше.
