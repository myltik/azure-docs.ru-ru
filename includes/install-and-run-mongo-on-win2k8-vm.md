Выполните следующие действия, чтобы установить и запустить MongoDB на виртуальной машине Windows Server 2008 R2.
...
3. Перейдите на страницу [Загрузки — MongoDB] [MongoDownloads].
4. Найдите последний выпуск в разделе **Рабочая версия (рекомендуется)** и щелкните ссылку ***2008+** в столбце "Windows 64-bit".  Нажмите кнопку **Сохранить как** и сохраните ZIP-файл на рабочем столе.
5. Щелкните правой кнопкой мыши ZIP-файл и выберите **Извлечь все...**  Введите "C:\" и нажмите **Извлечь**.  После извлечения файлов папку установки можно переименовать.  Например, в "MongoDB".
...
8. Чтобы запустить консоль управления MongoDB, откройте еще одно окно командной строки из меню **Пуск** и введите:
...
12. После установки MongoDB необходимо настроить конечную точку для удаленного доступа к MongoDB. На портале управления щелкните **Виртуальные машины** и выберите имя новой виртуальной машины, а затем щелкните **Конечные точки**.
