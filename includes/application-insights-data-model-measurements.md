...
```
 ...
 > [!NOTE]
 > Пользовательские измерения, связанные с элементом телеметрии, к которому они принадлежат. Они должны быть доступны для выборки с использованием элемента телеметрии, содержащего эти измерения. Используйте [телеметрию метрик](#metric-telemetry) для отслеживания измерения со значением, которое не зависит от другого типа телеметрии. 
