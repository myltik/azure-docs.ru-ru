1. Войдите на [портал Azure](http://manage.windowsazure.com). Воспользуйтесь [бесплатной пробной версией](http://azure.microsoft.com/pricing/free-trial/), если у вас пока нет подписки.
...
5. На странице **Конфигурация виртуальной машины** выполните следующие действия:
	...
	- Проверьте значение параметра **Уровень** и выберите значение параметра **Размер**. Размеры, доступные для выбора, зависят от указанного уровня. Размер определяет расходы на виртуальную машину, а также возможности настройки, например число подключаемых дисков с данными. Дополнительную информацию см. в статье [Размеры виртуальных машин](../articles/virtual-machines-size-specs.md).
...
6. На следующей странице **Конфигурация виртуальной машины** выполните следующие действия:
	...
	- В разделе **Конечные точки** сохраните конечную точку SSH. Вы можете добавить другие точки прямо сейчас либо добавить, изменить или удалить их после создания виртуальной машины.
