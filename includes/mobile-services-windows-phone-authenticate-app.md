1. Откройте файл проекта mainpage.xaml.cs и добавьте следующий фрагмент кода в класс MainPage:
    ...
    >[AZURE.NOTE]Если используется поставщик удостоверений, отличный от Facebook, измените значение <strong>MobileServiceAuthenticationProvider</strong> выше на значение для вашего поставщика.</p>
    </div>

2. Удалите или закомментируйте существующее переопределение метода **OnNavigatedTo** и замените его следующим методом, который обрабатывает событие **Loaded** для страницы. 
