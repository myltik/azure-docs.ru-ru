...
3.	Откроется диалоговое окно **Подключение пустого диска**.
	...
	![Подключите пустой диск](./media/howto-attach-disk-window-linux/AttachEmptyDetail.png)
...
Для подключения существующего диска требуется VHD-файл в учетной записи хранения. Используйте командлет [Add-AzureVhd](http://go.microsoft.com/FWLink/p/?LinkID=391684) для отправки VHD-файла в учетную запись хранения. После создания и отправки VHD-файла можно подключить его к виртуальной машине. 
...
2. На панели команд нажмите кнопку **Присоединить**, а затем выберите **Присоединить диск**.
	...
	![Подключите диск данных](./media/howto-attach-disk-window-linux/AttachExistingDisk.png)
