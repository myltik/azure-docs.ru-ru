## <a name="connect-to-azure-sql-database-as-a-user"></a>Подключение пользователя к базе данных SQL Azure
...
* **Тип сервера**: по умолчанию здесь указан компонент "database engine", не изменяйте это значение.
  ...
  * **Имя сервера**: укажите имя сервера, на котором размещена ваша база данных SQL, в формате *&lt;имя_сервера>*.**database.windows.net**.
...
Наиболее распространенными причинами сбоев подключения являются ошибки при вводе имени сервера (например, <*имя_сервера*> — это имя логического сервера, а не базы данных), имени пользователя или пароля, а также запрет на подключение к серверу из соображений безопасности. 
