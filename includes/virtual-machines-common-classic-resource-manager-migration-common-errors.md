...
| Миграция не разрешена для развертывания {имя_развертывания} в размещенной службе {имя_облачной_службы}, так как она не входит в область работ для региона. Сведения о добавлении виртуальной сети в область работ для региона см. на странице http://aka.ms/regionalscope. | В 2014 г. было объявлено, что сетевые ресурсы в Azure будут перемещены из области работ для кластера на региональный уровень. Дополнительные сведения см. здесь: [http://aka.ms/regionalscope] \(http://aka.ms/regionalscope). Эта ошибка возникает при миграции развертывания без выполнения обновления, которое автоматически перемещает такое развертывание в область работ для региона. Лучшим решением будет добавить для виртуальной машины конечную точку или диск данных, а затем повторить попытку миграции. <br> См. дополнительные сведения о [настройке конечных точек для классической виртуальной машины Windows в Azure](../articles/virtual-machines/windows/classic/setup-endpoints.md#create-an-endpoint) и [подключении диска данных к виртуальной машине Windows, созданной с помощью классической модели развертывания](../articles/virtual-machines/windows/classic/attach-disk.md).|
...
После завершения миграции может потребоваться переместить виртуальную машину в другую подписку. Тем не менее, если на виртуальной машине имеется секрет или ключ, который ссылается на ресурс Key Vault, то переместить ее в настоящее время не удастся. Ниже приведены инструкции, позволяющие обойти эту проблему. 
