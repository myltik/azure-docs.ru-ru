...
Проще всего подключиться к эмулятору хранения из приложения можно путем настройки строки подключения в файле конфигурации приложения, который ссылается на ярлык `UseDevelopmentStorage=true`. Ниже приведен пример строки подключения к эмулятору хранения в файле app.config:
...
Это значение полностью совпадает с ярлыком, указанным выше (`UseDevelopmentStorage=true`).
