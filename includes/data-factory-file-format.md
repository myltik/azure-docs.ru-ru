...
> [AZURE.IMPORTANT] Если используется копирование данных из локального в облачное хранилище или наоборот в формате ORC, а не копирование файлов ORC из источника в приемник в исходном виде, необходимо установить JRE 8 (среду выполнения Java) на компьютере шлюза, который будет использоваться для преобразования данных в правильный формат. Обратите внимание, что для 64-разрядного шлюза требуется 64-разрядная версия JRE, а для 32-разрядного шлюза — 32-разрядная версия JRE. Выберите подходящую для своего шлюза версию [здесь](http://go.microsoft.com/fwlink/?LinkId=808605).

Обратите внимание на следующее.

-	Данные сложных типов (STRUCT, MAP, LIST, UNION) не поддерживаются.
-	Файл ORC имеет три [варианта сжатия](http://hortonworks.com/blog/orcfile-in-hdp-2-better-compression-better-performance/): NONE, ZLIB, SNAPPY. Фабрика данных поддерживает чтение данных из файла ORC в любом из указанных выше форматов сжатия. Для чтения данных используется кодек сжатия из метаданных. Однако при записи в файл ORC фабрика данных по умолчанию выбирает ZLIB. В настоящее время изменить это поведение нельзя.
