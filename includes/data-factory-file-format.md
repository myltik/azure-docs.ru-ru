...
| --- | --- | --- | --- |
| columnDelimiter |Знак, используемый для разделения столбцов в файле. |Допускается только один знак. Значение по умолчанию — запятая (,). <br/><br/>Чтобы использовать [символы Юникода](https://en.wikipedia.org/wiki/List_of_Unicode_characters), необходимо получить соответствующий код. Например, вы можете использовать редкий непечатаемый символ, который обычно отсутствует в данных: укажите \u0001 (символ начала заголовка). |Нет |
...
* [Сложные типы данных](http://avro.apache.org/docs/current/spec.html#schema_complex) не поддерживаются (записи, перечисления, массивы, сопоставления, объединения и фиксированные данные).

### <a name="specifying-jsonformat"></a>Указание JsonFormat
Сведения об импорте и экспорте файлов в формате JSON в базу данных DocumentDB и из нее см. в [этой статье](../articles/data-factory/data-factory-azure-documentdb-connector.md#importexport-json-documents).
...
#### <a name="setofobjects-file-pattern"></a>Шаблон файла setOfObjects
...
* Каждый объект в коллекции пар "имя-значение" сопоставлен с одной строкой данных в табличном формате. Объекты могут быть вложенными, и вы можете определить, как свести структуру в наборе данных с разделителем вложения (.) по умолчанию. в разделе [Пример JsonFormat](#jsonformat-example) выше.  
