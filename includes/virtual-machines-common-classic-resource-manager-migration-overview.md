...
| Сеть |Виртуальная сеть с двумя шлюзами (шлюз ExpressRoute и VPN-шлюз)  | Удалите VPN-шлюз до начала миграции и повторно создайте его по ее завершении. Дополнительные сведения о [миграции ExpressRoute](../articles/expressroute/expressroute-migration-classic-resource-manager.md).|
| Сеть |ExpressRoute со ссылками авторизации  | Удалите подключение канала ExpressRoute к виртуальной сети до начала миграции, а затем повторно создайте подключение после ее завершения. Дополнительные сведения о [миграции ExpressRoute](../articles/expressroute/expressroute-migration-classic-resource-manager.md). |
...
| Среда выполнения приложений |Расширения центра безопасности Azure с виртуальной сетью, использующей VPN-шлюз для транзитных подключений или шлюз ExpressRoute с локальным DNS-сервером |Центр безопасности Azure автоматически устанавливает расширения на виртуальные машины, чтобы наблюдать за их безопасности и создавать оповещения. Обычно эти расширения устанавливается автоматически, если для подписки включена политика центра безопасности Azure. Сейчас перенос шлюза ExpressRoute не поддерживается, и VPN-шлюзы с транзитными подключениями лишаются локального доступа. Если продолжать выполнение миграции, то удаление шлюза ExpressRoute или перенос VPN-шлюза с транзитными подключениями приведет к потере доступа к Интернету для учетной записи хранения виртуальной машины. После этого миграция не продолжится, так как будет невозможно заполнить большой двоичный объект состояния гостевого агента. Рекомендуется отключить политику центра безопасности Azure в подписке за 3 часа до выполнения миграции. |
