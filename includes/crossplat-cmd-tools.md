...
В этом руководстве рассматривается, как использовать программы командной строки Mac и Linux для создания и управления службами в Windows Azure. Рассмотрены следующие сценарии: **Установка средств**, **Импорт параметров публикации**, **Создание и управление веб-сайтами Windows Azure** и **Создание и управление виртуальными машинами Windows Azure**. Полную справочную документацию см. в [документации средства командной строки Windows Azure для Linux и Mac][reference-docs]. 

##Оглавление
* [Программы командной строки Windows Azure для Mac и Linux](#Overview)
* [Установка программ командной строки Windows Azure для Mac и Linux](#Download)
* [Создание учетной записи Windows Azure](#CreateAccount)
* [Загрузка и импорт параметров публикации](#Account)
* [Создание и управление веб-сайтом Windows Azure](#WebSites)
* [Создание и управление виртуальной машиной Windows Azure](#VMs)


<h2><a id="Overview"></a>Программы командной строки Windows Azure для Mac и Linux</h2>

Программы командной строки Windows Azure для Mac и Linux — это программы командной строки для развертывания служб Windows Azure и управления ими.
 
Ниже перечислены поддерживаемые задачи:

* Параметры импорта публикации.
* Создание и управление веб-сайтами Windows Azure.
* Создание и управление виртуальными машинами Windows Azure.
...
<h2><a id="Download">Установка программ командной строки Windows Azure для Mac и Linux</a></h2>

Следующий список содержит сведения по установке программ командной строки в зависимости от операционной системы:

* **Mac**: загрузите [установщик Windows Azure SDK][mac-installer]. Откройте загруженный файл .pkg и выполните установку.
...
* **Windows**: запустите установщика Windows (файл .msi), который доступен здесь: [программы командной строки Windows Azure][windows-installer].
...
<h2><a id="CreateAccount"></a>Создание учетной записи Windows Azure</h2>

Чтобы использовать программы командной строки Windows Azure для Mac и Linux, будет необходима учетная запись Windows Azure.
...
![Веб-сайт Windows Azure][Windows Azure Web Site]
...
<b>Примечание</b>. 
<p>При импорте параметров публикации учетные данные для доступа к подписке Windows Azure хранятся внутри вашей папки <code>user</code>. Папка <code>user</code> защищена операционной системой. Тем не менее, рекомендуется принять дополнительные меры для шифрования папки <code>user</code>. Это можно сделать одним из следующих способов:</p>
...
Теперь можно приступать к созданию и управлению веб-сайтами Windows Azure и виртуальными машинами Windows Azure.  

<h2><a id="WebSites"></a>Создание и управление веб-сайтом Windows Azure</h2>

###Создание веб-сайта

Чтобы создать веб-сайт Windows Azure, необходимо сначала создать пустой каталог с именем "MySite" и перейти в этот каталог.
...
Затем можно добавить содержимое в каталог веб-сайта. Используйте обычный поток git ("git add", "git commit") для применения контента. Используйте следующую команду git для передачи контента веб-сайта на Windows Azure: 
...
<h2><a id="VMs"></a>Создание и управление виртуальной машиной Windows Azure</h2>

Виртуальная машина Windows Azure создается из образа виртуальной машины (VHD-файл), предоставленного пользователем или доступного в коллекции образов. Чтобы просмотреть доступные образы, используйте команду "vm image list":
...
Вместо создания образа из локального VHD-файла можно создать образ из VHD-файла, хранящегося в хранилище BLOB-объектов Windows Azure. Это можно сделать с помощью параметра "blob-url":
