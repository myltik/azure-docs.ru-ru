# <a name="how-to-use-the-azure-command-line-tools-for-mac-and-linux"></a>Использование средств командной строки Azure для Mac и Linux
В этом руководстве рассматривается, как использовать программы командной строки Mac и Linux для создания и управления службами в Azure. Рассмотрены следующие сценарии: **установка средств**, **импорт параметров публикации**, **создание и администрирование веб-сайтов Azure** и **создание и администрирование виртуальных машин Azure**. См. полную справочную документацию по [средствам командной строки Azure для Mac и Linux][reference-docs]. 
...
Чтобы получить полный список поддерживаемых команд введите `azure -help` в командной строке после установки средств или обратитесь к [справочной документации][reference-docs].
...
* **Linux**: установите последнюю версию [Node.js][nodejs-org] (см. сведения об [установке Node.js с помощью диспетчера пакетов][install-node-linux]), а затем выполните следующую команду:
  
        npm install azure-cli -g
  
    **Примечание**. Возможно, вам потребуется выполнить эту команду с повышенными привилегиями:
  
        sudo npm install azure-cli -g
* **Windows**: запустите установщик Windows (MSI-файл), который доступен здесь: [Программы командной строки Microsoft Azure][windows-installer].
...
Откройте браузер, перейдите по адресу [http://www.windowsazure.com][windowsazuredotcom] и щелкните **Бесплатная учетная запись** в правом верхнем углу.

![Веб-сайт Azure][Azure Web Site]

Следуйте инструкциям по созданию учетной записи.

<h2><a id="Account"></a>Скачивание и импорт параметров публикации</h2>
...
Все сведения, сохраненные с помощью команды <code>import</code>, можно удалить с помощью команды <code>account clear</code>:
...
> [!NOTE]
> Когда вы импортируете параметры публикации, учетные данные для доступа к подписке Microsoft Azure хранятся в папке `user`. Папка `user` защищена операционной системой. Тем не менее рекомендуется принять дополнительные меры для шифрования папки `user` . Это можно сделать одним из следующих способов:    
> 
...
<h2><a id="WebSites"></a>Создание и администрирование веб-сайта Azure</h2>
...
### <a name="set-up-publishing-from-github"></a>Настройка публикации из GitHub
...
### <a name="list-and-show-sites"></a>Отображение сайтов
...
### <a name="stop-start-or-restart-a-site"></a>Остановка, запуск и перезапуск сайта
...
<h2><a id="VMs"></a>Создание виртуальной машины Azure и управление ей</h2>
