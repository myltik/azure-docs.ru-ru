...
> [!NOTE]
> При импорте параметров публикации учетные данные для доступа к подписке Azure хранятся внутри папки `user`. Папка `user` защищена операционной системой. Тем не менее рекомендуется принять дополнительные меры для шифрования папки `user`. Это можно сделать одним из следующих способов:
> 
