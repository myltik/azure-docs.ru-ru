...
Обмен данными между веб-приложением и браузером можно защитить с помощью протокола HTTPS, использующего шифрование Secure Socket Layer (SSL). Это наиболее часто используемый способ защиты данных, передаваемых через Интернет. Он гарантирует безопасность взаимодействия посетителя с вашим приложением. В этой статье описывается настройка протокола HTTPS для веб-приложения в службе приложений Azure. В данной статье не рассматриваются вопросы проверки подлинности сертификата клиента. Сведения об этом можно найти в статье [Настройка взаимной проверки подлинности TLS для веб-приложений](../articles/app-service-web/app-service-web-configure-tls-mutual-auth.md).
