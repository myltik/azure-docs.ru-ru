...
По умолчанию Azure уже обеспечивает HTTP для вашего приложения с помощью шаблонного сертификата для домена *.azurewebsites.net. Если вы не планируете настраивать пользовательский домен, можно воспользоваться сертификатом HTTPS по умолчанию. Однако, как и [все домены с подстановочными знаками](https://casecurity.org/2014/02/26/pros-and-cons-of-single-domain-multi-domain-and-wildcard-certificates/), он не так безопасен, как личный домен с собственным сертификатом.
...
2. [Настройка стандартной ценовой категории](#bkmk_standardmode)
...
## 2\. Настройка стандартной ценовой категории

Включение HTTPS для пользовательского домена возможно только на уровне **Стандартный** службы приложений Azure. Выполните следующие шаги для переключения плана службы приложений на уровень **Стандартный**.
