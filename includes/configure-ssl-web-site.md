...
> [AZURE.NOTE] Если вы добавите **SSL на основе IP** и включите для веб-приложения, в котором уже есть **привязка SNI** к другому сертификату, имя узла сайта будет повторно сопоставлено с этим с IP-адресом. Поэтому если каноническим именем для имени узла сайта является другое имя узла, оно тоже будет получать трафик по IP-адресу в рамках SSL-подключения. В таких случаях мы создаем еще одну запись DNS: sni.&lt;nameofyourWebApp&gt;.azurewebsites.net, в которой &lt;nameofyourWebApp&gt; — это имя вашего веб-приложения службы приложений Azure. Поэтому вам нужно изменить свои записи DNS, указывающие на используемое в привязке SNI имя, чтобы они указывали на имя sni.&lt;nameofyourWebApp&gt;.azurewebsites.net.
