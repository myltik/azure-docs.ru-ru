...
Обмен данными между веб-приложением и браузером можно защитить с помощью протокола HTTPS, использующего шифрование Secure Socket Layer (SSL). Это наиболее часто используемый способ защиты данных, передаваемых через Интернет. Он гарантирует безопасность взаимодействия посетителя с вашим приложением. В этой статье описывается настройка протокола HTTPS для веб-приложения в службе приложений Azure. 

##<a name="bkmk_azurewebsites"></a>HTTPS для домена \*.azurewebsites.net

Если вы не планируете использовать личное доменное имя и вместо него думаете использовать домен \*.azurewebsites.net, назначенный для вашего веб-приложения платформой Azure (например, contoso.azurewebsites.net), то ваш сайт уже защищен сертификатом Майкрософт. Для доступа к своему приложению вы можете использовать ссылку **https://mywebsite.azurewebsites.net**. Однако \*.azurewebsites.net - это домен с подстановочным знаком. Как и [все домены с подстановочными знаками](https://casecurity.org/2014/02/26/pros-and-cons-of-single-domain-multi-domain-and-wildcard-certificates/), он не так безопасен, как личный домен с собственным сертификатом. 
...
##<a name="bkmk_domainname"></a>Включение SSL для личного домена

Чтобы включить протокол HTTPS для личного домена, например **contoso.com**, необходимо сначала зарегистрировать имя этого домена у регистратора доменных имен. Дополнительные сведения о том, как настроить доменное имя для веб-приложения см. в разделе [Настройка имени личного домена для веб-приложения Azure](/ru-ru/develop/net/common-tasks/custom-dns-web-site/). После регистрации личного доменного имени и настройки веб-приложения в ответ на личное имя необходимо запросить SSL-сертификат для домена. 

> [AZURE.NOTE] Чтобы активировать протокол HTTPS для личных доменных имен, необходимо настроить веб-приложение на **стандартный** режим. Если в настоящий момент используется бесплатный режим или общий режим, это может повлечь за собой дополнительные затраты. Дополнительные сведения о стоимости общего и **стандартного** режимов см. в разделе [Сведения о ценах][цены]. 

##<a name="bkmk_getcert"></a>Получение SSL-сертификата

Перед запросом SSL-сертификата необходимо сначала определить, какие доменные имена будут защищены с помощью этого сертификата. От этого зависит требуемый тип сертификата. Если необходимо защитить одно доменное имя, например **contoso.com** или **www.contoso.com**, вам будет достаточно базового сертификата. Если необходимо защитить несколько доменных имен, таких как **contoso.com**, **www.contoso.com** и **mail.contoso.com**, то можно использовать [групповой сертификат](http://en.wikipedia.org/wiki/Wildcard_certificate) или сертификат с [именем альтернативной темы](http://en.wikipedia.org/wiki/SubjectAltName) (subjectAltName).
...
* Имя субъекта сертификата должно совпадать с доменным именем, которое используется для обращения к веб-приложению. Если требуется обслуживать несколько доменов с этим сертификатом, необходимо использовать подстановочные значения или указать значения subjectAltName, как обсуждалось ранее.
...
Возможно, вам также понадобятся **[промежуточные сертификаты](http://en.wikipedia.org/wiki/Intermediate_certificate_authorities)** (также известные как сертификаты цепочки), если ваш центр сертификации использует их. Использование промежуточных сертификатов считается более безопасным, чем  'unchained certificates', поэтому ЦС в большинстве случаев используют такие сертификаты. Промежуточные сертификаты зачастую нужно отдельно загружать с веб-сайта ЦС. Действия, приведенные в данном разделе, описывают процедуру объединения всех промежуточных сертификатов с сертификатом, отправленным в веб-приложения. 
...
1. Откройте **Блокнот** и создайте новый документ со следующим содержимым. Замените **mysite.com** в строке темы на личное доменное имя вашего веб-приложения. Например, тема = "CN=www.contoso.com".
...
Теперь можно отправить экспортированный PFX-файл в веб-приложение Azure.
...
2. При появлении запроса введите соответствующую информацию. Например:
		...
		Укажите следующие атрибуты  'extra', отправляемые вместе с запросом сертификата
...
5. Из сеанса командной строки, Bash или в сеансе терминала используйте следующую команду для преобразования файлов **myserver.key** и **myserver.crt** в **myserver.pfx**; для службы приложений Azure требуется файл в таком формате:
	...
	> 
	> Следующая команда демонстрирует процедуру создания PFX-файла, включающего в себя промежуточные сертификаты, содержащиеся в файле **intermediate-cets.pem**:  
	>
	>
	`````
...
1. С помощью диспетчера IIS создайте файл запроса подписи сертификата (CSR) и отправьте его в центр сертификации. Дополнительные сведения о создании запроса CSR см. в разделе [Запрос сертификата службы IIS 7][iiscsr].
...
3. Для запроса CSR укажите сертификат, предоставленный центром сертификации. Дополнительные сведения о завершении работы с CSR см. в разделе [Установка служб IIS 7][installcertiis].
...
4. Экспортируйте сертификат из диспетчера IIS. Дополнительные сведения об экспорте сертификата см. в разделе [Экспорт сертификата служб IIS 7][exportcertiis]. Экспортированный файл на следующих этапах будет отправляться в Azure для использования с веб-приложением.
	...
	> [AZURE.NOTE] Во время экспорта убедитесь, что выбран параметр **Включить все сертификаты в путь сертификации** и **Экспортировать все расширенные свойства**. Это позволит включить в экспортированный сертификат все промежуточные сертификаты.


###<a name="bkmk_subjectaltname"></a> Получение сертификата SubjectAltName с помощью OpenSSL
...
2. В командной строке для разработчика используйте следующую команду для создания нового самозаверяющего сертификата. Необходимо заменить **serverdnsname** на DNS вашего веб-приложения.
...
3. Воспользуйтесь следующей командой, чтобы экспортировать сертификат в PFX-файл, который можно отправить в веб-приложение Azure:

		openssl pkcs12 -export -out myserver.pfx -inkey myserver.key -in myserver.crt

	При появлении запроса введите пароль для защиты файла .pfx.

	Созданный этой командой файл **myserver.pfx** может использоваться для защиты веб-приложения при выполнении тестирования.

<a name="bkmk_standardmode"></a><h2>Настройка стандартного режима</h2>

Включение протокола HTTPS для пользовательского домена возможно только в режиме веб-приложения **Стандартный**. Чтобы перейти в режим **Стандартный**, выполните следующие действия.

> [AZURE.NOTE] Прежде чем перевести веб-приложение из режима **Бесплатный** в режим **Стандартный**, необходимо снять существующие ограничения расходов, установленные для вашей подписки на веб-приложение. В противном случае существует риск недоступности веб-приложения при превышении ограничений до завершения расчетного периода. Дополнительные сведения о ценах на режим общего доступа и **Стандартный** см. в разделе [Сведения о ценах][pricing].

1.	Откройте в браузере [Портал Azure](http://go.microsoft.com/fwlink/?LinkId=529715).
...
6.	Щелкните пункт **Масштаб**
	![The scale tab][scale]
7.	В разделе **Масштаб** выберите режим для плана сервиса приложений, щелкнув команду **Выбрать**.
...
##<a name="bkmk_configuressl"></a>Настройка SSL

Прежде чем выполнять действия, описанные в этом разделе, необходимо связать личное доменное имя с веб-приложением. Дополнительные сведения см. в разделе [Настройка личного доменного имени для веб-приложения][customdomain].
...
8.	С использованием диалогового окна **загрузки сертификата** выберите файл сертификата .pfx, созданный ранее с помощью диспетчера IIS или OpenSSL. Укажите пароль, используемый для защиты файла .pfx (при наличии). В конце нажмите кнопку **Сохранить** для отправки сертификата.
	![ssl upload][ssluploadcert]
9. В разделе **привязка ssl** вкладки **Параметры SSL** в раскрывающихся списках выберите имя домена для защиты с применением SSL, а также используемый сертификат. Можно также выбрать использование подключение с [Указанием имени сервера][sni] (SNI) или SSL на основе IP-адреса.
...
На этом этапе веб-приложение должно открываться с использованием `HTTPS://`, а не `HTTP://`, что подтверждает правильность настройки сертификата.

##<a name="bkmk_enforce"></a>Принудительное использование HTTPS для веб-приложения
...
В случае приложений .NET измените файл web.config для приложения, добавив раздел **&lt;rewrite>** из [примера](#example) в раздел **&lt;system.WebServer>**.

Если файл web.config уже содержит раздел **&lt;rewrite>**, добавьте **&lt;rule>** из [примера](#example) в качестве первой записи в разделе **&lt;rules>**.

###PHP

Для приложений PHP просто сохраните [пример](#example) как файл web.config в корневом каталоге приложения, а затем заново разверните приложение в веб-приложении.
...
Чтобы получить этот автоматически созданный файл с веб-приложения и изменить его, выполните следующие действия.
