...
Все виртуальные машины, созданные в Azure, могут автоматически взаимодействовать с другими виртуальными машинами в той же облачной службе или виртуальной сети, используя канал частной сети. Все другие входящие связи, таких как трафик от узлов Интернета или виртуальные машины в других облачных службах или виртуальных сетях, требуют конечной точки.

Конечные точки могут использоваться для различных целей. Конечные точки на виртуальной машине, создаваемой в портале управления Azure, по умолчанию используются и настраиваются для протокола удаленного рабочего стола (RDP) и трафика удаленного сеанса Windows PowerShell. Эти конечные точки позволяют удаленно управлять виртуальной машиной через Интернет. 

Другое применение конечных точек — конфигурация средства балансировки нагрузки Azure для распределения определенного типа трафика между несколькими виртуальными машинами или службами. Например, можно распределить нагрузку от трафика веб-запросов на несколько веб-серверов или веб-ролей.
...
При настройке балансировки нагрузки трафика среди нескольких виртуальных машин или служб Azure предоставляет случайное распределение входящего трафика.
...
Чтобы создать набор балансировки нагрузки из виртуальных машин Azure, выполните следующие действия:
...
Войдите на [портал управления Azure](http://manage.windowsazure.com), если вы еще этого не сделали. Можно создать первую виртуальную машину из коллекции или методом быстрого создания. 
...
1. В портале управления Azure нажмите **Виртуальные машины** и выберите имя первой виртуальной машины.
...
6. На странице "Настроить набор балансировки нагрузки" введите имя для набора балансировки нагрузки, а затем присвойте значения для пробного поведения подсистемы балансировки нагрузки Azure. Подсистема балансировки нагрузки использует пробы для определения, доступны ли виртуальные машины в наборе балансировки нагрузки для приема входящего трафика.
...
[Начало работы с Azure PowerShell]:http://msdn.microsoft.com/ru-ru/library/jj156055.aspx

[Обзор виртуальной сети Azure]: http://go.microsoft.com/fwlink/p/?LinkID=294063
