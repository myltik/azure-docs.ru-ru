...
Временный диск создается автоматически. На виртуальных машинах Windows этот диск обозначается буквой D и используется для хранения файла подкачки pagefile.sys. На виртуальных машинах Linux этот диск обычно является диском /dev/sdb, а форматируется и монтируется в /mnt/resource агентом Linux для Azure.

>[AZURE.WARNING] Мы советуем не хранить данные на временном диске. Он обеспечивает временное хранилище для приложений и процессов и предназначен только для хранения данных, таких как файлы подкачки. Сведения о переназначении этого диска для виртуальной машины Windows см. в статье [Изменение буквы диска для временного диска Windows](../articles/virtual-machines/virtual-machines-windows-classic-change-drive-letter.md).
...
>[AZURE.NOTE] Дополнительные сведения о емкости виртуальных машин см. в статье [Размеры виртуальных машин](../articles/virtual-machines/virtual-machines-linux-sizes.md).
