...
3. В Visual Studio откройте файл applicationhost.config для IIS Express. Этот файл расположен в следующем подкаталоге каталога профиля пользователя.
        ...
        C:\Users\<имя вашего профиля>\Documents\IISExpress\config\applicationhost.config
...
5. Откройте консоль брандмауэра Windows и создайте новое правило для порта, чтобы разрешить подключения к порту. Дополнительные сведения о создании нового правила для порта в брандмауэре Windows см. в разделе [Добавление нового правила для порта в брандмауэре Windows].

    >[WACOM.NOTE] Если тестовый компьютер присоединен к домену, исключения брандмауэра могут регулироваться политикой домена. В этом случае необходимо связаться со своим администратором домена, чтобы получить исключение для порта на вашем компьютере.
