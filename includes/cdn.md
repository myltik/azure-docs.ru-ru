...
Сеть доставки содержимого Windows Azure (CDN) предлагает разработчикам глобальное решение для доставки большого объема содержимого с возможностью кэширования BLOB-объектов и статического содержимого на физических узлах в США, Европе, Азии, Австралии и Южной Америке. Для получения текущего списка местоположений узлов CDN см. [Расположение узлов сети CDN для Windows Azure]
...
Преимущества использования сети CDN для кэширования данных Windows Azure включают следующее:
...
Существующие клиенты сети CDN теперь могут использовать сеть Windows Azure CDN в [портале управления Windows Azure]. CDN является функцией расширения для подписки и имеет отдельный [план выставления счетов]

<a id="Step1"> </a>
<h2>Шаг 1. Создание учетной записи хранения</h2>

Чтобы создать новую учетную запись хранения для подписки Windows Azure, воспользуйтесь следующей процедурой. Учетная запись хранения предоставляет доступ к службам хранения Windows Azure. Учетная запись хранения представляет собой высший уровень пространства имен для доступа ко всем компонентам службы хранения Windows Azure: службам BLOB-объекта, службам очередей и службам таблиц. Дополнительные сведения о службах хранения Windows Azure см. в разделе [Использование служб хранения Windows Azure].
...
<p>Сведения о выполнении этой операции с помощью API управления службой Windows Azure см. в справочном разделе <a href="http://msdn.microsoft.com/ru-ru/library/windowsazure/hh264518.aspx">Создание учетной записи хранения</a>.</p>
</div>

**Чтобы создать учетную запись хранения для подписки Windows Azure**
...
4. В поле **URL-адрес** введите имя поддомена. Запись может содержать от 3 до 24 строчных букв и цифр.
    ...
    **Важно.** Метка URL-адреса образует поддомен URI учетной записи хранения и должна быть уникальной на уровне всех размещенных служб в Windows Azure.
...
5.  В раскрывающемся списке **Регион или территориальная группа** выберите географическое расположение для этой учетной записи хранения. Можно также использовать территориальную группу. Инструкции по созданию территориальной группы см. в разделе [Создание территориальной группы в Windows Azure].
...
1. В [портале управления Windows Azure] на панели управления щелкните **CDN**.
...
Если кэширование объекта в сети CDN Windows Azure больше не требуется, можно выполнить одно из следующих действий:

-   Для BLOB-объекта Windows Azure можно удалить этот объект из открытого контейнера.
...
В настоящее время средство явной "очистки" для сети Windows Azure CDN недоступно.

## Дополнительные ресурсы

-   [Создание территориальной группы в Windows Azure]
-   [Управление учетной записи хранения для подписки Windows Azure]
...
-   [Сопоставление содержимого CDN с пользовательским доменом]
  ...
  [Расположение узлов сети CDN Windows Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/gg680302.aspx
  ...
  [Использование служб хранения Windows Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/ee924681.aspx
  [Регистрация имени пользовательского поддомена для доступа к BLOB-объектам в Windows Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/ee795179.aspx
  [Создание территориальной группы в Windows Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/hh531560.aspx
  [Обзор Windows Azure CDN]: http://msdn.microsoft.com/ru-ru/library/windowsazure/ff919703.aspx
  [Управление учетной записью хранения для подписки Windows Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/hh531567.aspx
