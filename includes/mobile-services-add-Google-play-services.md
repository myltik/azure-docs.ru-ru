...
3. Установите пакет SDK служб Google Play в вашем проекте. В Eclipse щелкните **Файл**, а затем - **Импорт**. Выберите **Android**, затем - **Существующий код Android в рабочую область** и нажмите кнопку **Далее**. Нажмите кнопку **Обзор**, перейдите в папку, где расположен пакет SDK для Android (обычно это папка с именем adt-bundle-windows-x8664 `adt-bundle-windows-x86_64` inside the folder that contains Eclipse), then go to the `\extras\google\), а затем перейдите во вложенную папку google_play_services\extras\google\google\playlibprojectservices\libproject`, выберите в ней папку google-play-services-lib и нажмите кнопку **ОК**. Установите флажок **Копировать проекты в рабочую область** и нажмите кнопку **Готово**.
...
8. В разделе **Библиотека** щелкните **Добавить**, выберите проект служб Google Play (*google-play-services-lib*) и нажмите кнопку **ОК**.
