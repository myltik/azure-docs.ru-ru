...
3. В окне SSH введите следующую команду для создания нового устройства, а затем введите пароль учетной записи:
	...
	>[AZURE.NOTE] В этом примере может потребоваться использовать `sudo -i` на некоторых дистрибутивах, если/sbin или/usr/sbin не находятся в вашей `$PATH`.
...
11. Добавьте новый диск в /etc/fstab:
	...
	>[AZURE.NOTE] Для blkid может не требоваться доступ sudo во всех случаях, однако может быть проще выполнить `sudo -i` на некоторых дистрибутивах, если/sbin или/usr/sbin не находятся в вашей `$PATH`.
