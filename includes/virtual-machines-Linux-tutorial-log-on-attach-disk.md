...
## <a id="logon"> </a>Вход на виртуальную машину после ее создания ##
...
## <a id="attachdisk"> </a>Подключение диска данных к новой виртуальной машине ##

Приложению может потребоваться сохранять данные. Чтобы выполнить такую настройку, подключите диск данных к ранее созданной виртуальной машине. Для этого проще всего подключить пустой диск данных к машине.
...
4. Значения **Имя виртуальной машины**, **Расположение хранилища** и **Имя файла** уже определены. Достаточно ввести размер диска. Введите **5** в поле **Размер**.
	...
	**Примечание.** Все диски создаются из файла виртуального жесткого диска в службе хранилища Azure. Вы можете указать имя для файла виртуального жесткого диска, который добавляется в хранилище, но имя диска создается автоматически.
...
3. В окне SSH введите следующую команду для создания нового устройства, а затем введите пароль учетной записи:
	...
	>[AZURE.NOTE] В данном примере можно использовать `sudo -i` для некоторых дистрибутивов, если /sbin или /usr/sbin не расположены по пути "$PATH".
...
11. Добавьте новый диск в /etc/fstab:
	...
	>[AZURE.NOTE] Программе blkid может не требоваться доступ sudo во всех случаях; однако на некоторых дистрибутивах может быть проще выполнить `sudo -i`, если /sbin или /usr/sbin не в вашем пути "$PATH".

	**Внимание!** Некорректное изменение файла /etc/fstab может привести к невозможности загрузить систему. Если вы не уверены в своих действиях, обратитесь к документации дистрибутива для получения сведений о правильном редактировании файла. Также рекомендуется перед внесением изменений создать резервную копию файла /etc/fstab.
