...
Независимо от того, работает ли программное обеспечение в облаке или локально, ему часто бывает необходимо взаимодействовать с другим программным обеспечением. Для обеспечения этой возможности Azure предоставляет служебную шину. В данной статье приводится обзор, описание и сценарии ее использования.

## Оглавление

-   [Базовые сведения о служебной шине][Базовые сведения о служебной шине]
-   [Очереди][Очереди]
-   [Разделы][Разделы]
-   [Ретрансляторы][Ретрансляторы]
...
Service Bus — это мультитенантная облачная служба, т. е. служба, совместно используемая несколькими пользователями. Каждый пользователь, например, разработчик приложений, создает *пространство имен* и определяет механизмы взаимодействия, которые должны использоваться в нем. [Рисунок 1][Рисунок 1] демонстрирует, как это выглядит.

<a name="Fig1"></a>![Схема служебной шины Azure][Схема служебной шины Azure]
...
-   *Очереди*, которые поддерживают однонаправленное взаимодействие. Каждая очередь действует как посредник (иногда называемый *брокером*), который хранит отправленные сообщения, пока они не будут получены.
-   *Разделы*, которые обеспечивают однонаправленное взаимодействие посредством *подписок*. Подобно очереди, раздел действует как посредник, но позволяет каждой подписке видеть только те сообщения, которые соответствуют определенным условиям.
...
Важно понимать, что, хотя сама служебная шина работает в облаке (т. е. в центрах обработки данных Microsoft Azure), исполняемые по ней приложения могут работать где угодно. С помощью служебной шины можно связать приложения, выполняемые, например, в Azure и в локальном центре обработки данных. Эту службу также можно использовать для связи приложения, работающего в Azure или на другой облачной платформе, с локальным приложением или с планшетами и телефонами. Можно даже связывать бытовые приборы, датчики и другие устройства с центральным приложением или друг с другом. Service Bus — это универсальный механизм взаимодействия в облаке, доступный практически везде. Способ ее использования зависит от целей приложения.

## <a name="queues"></a>Очереди

Предположим, что необходимо связать два приложения с помощью очереди Service Bus. Эта ситуация показана на [рисунке 2][рисунке 2].

<a name="Fig2"></a>![Схема очередей Service Bus][Схема очередей Service Bus]

**Рис. 2: Очереди служебной шины обеспечивают однонаправленное асинхронное взаимодействие.**

Процесс прост: отправитель отсылает сообщение в очередь служебной шины, а получатель извлекает это сообщение позднее. У очереди может быть только один получатель, как показано на [рисунке 2][рисунке 2], или же несколько приложений могут считывать сообщения из одной очереди. В последнем случае каждое сообщение, как правило, считывается только одним получателем — очереди не предоставляют многоадресное обслуживание.

Каждое сообщение состоит из двух частей: набора свойств (пар "ключ/значение") и двоичного тела сообщения. Способ их использования зависит от целей приложения. Например, приложение, отправляющее сообщение о недавней продаже, может включить в него свойства *Продавец="Ava"* и *Сумма=10 000*. Тело сообщения может содержать отсканированное изображение подписанного контракта или может остаться пустым.
...
Хотя очереди полезны, они не всегда являются правильным решением. Иногда лучше использовать разделы Service Bus. [На рисунке 3][На рисунке 3] показан принцип работы разделов.

<a name="Fig3"></a>![Схема разделов и подписок Service Bus][Схема разделов и подписок Service Bus]
...
Раздел во многом похож на очередь. Отправители отправляют сообщения в раздел так же, как в очередь, и эти сообщения выглядят в разделе так же, как в очереди. Существенное отличие заключается в том, что разделы позволяют каждому принимающему приложению создать собственную подписку, определив *фильтр*. Подписчик будет видеть только те сообщения, которые соответствуют фильтру. Например, [на рисунке 3][На рисунке 3] показан отправитель и раздел с тремя подписчиками, у каждого из которых имеется собственный фильтр:
...
Связь приложений всегда была частью комплексных решений, и эта задача вряд ли когда-нибудь потеряет актуальность. При предоставлении облачных технологий посредством очередей, разделов и ретрансляторов используется служебная шина, которая упрощает эту важную функцию и делает ее более доступной.
  ...
  [Рисунок 1]: #Fig1
  [Схема служебной шины Azure]: ./media/hybrid-solutions/SvcBus_01_architecture.png
  [рисунке 2]: #Fig2
  [Схема очередей Service Bus]: ./media/hybrid-solutions/SvcBus_02_queues.png
  [На рисунке 3]: #Fig3
  [Схема разделов и подписок Service Bus]: ./media/hybrid-solutions/SvcBus_03_topicsandsubscriptions.png
  [рисунке 4]: #Fig4
  [Схема ретранслятора Service Bus]: ./media/hybrid-solutions/SvcBus_04_relay.png
