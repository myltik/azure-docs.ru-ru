...
Независимо от того, работает ли программное обеспечение в облаке или локально, ему часто бывает необходимо взаимодействовать с другим программным обеспечением. Для обеспечения этой возможности Windows Azure предлагает Service Bus. В данной статье приводится обзор, описание и сценарии использования этой технологии.

##Оглавление      
- [Service Bus — основы](#fundamentals)
- [Очереди](#queues)
- [Разделы](#topics)
- [Ретрансляторы](#relays)


## <a name="fundamentals"></a>Service Bus — основы
...
Service Bus — это мультитенантная облачная служба, т. е. служба, совместно используемая несколькими пользователями. Каждый пользователь, например, разработчик приложений, создает *пространство имен* и определяет механизмы взаимодействия, которые должны использоваться в нем. [На рисунке 1](#Fig1) показано, как это выглядит.

<a name="Fig1"></a>![Схема Service Bus в Windows Azure][svc-bus]
 
**Рисунок 1. Service Bus предоставляет мультитенантную службу для связи приложений через облако.**
...
Важно понимать, что хотя сама Service Bus работает в облаке (т. е. в центрах обработки данных Microsoft Windows Azure), использующие ее приложения могут выполняться где угодно. С помощью Service Bus можно связать приложения, выполняемые, например, в Windows Azure или в локальном центре обработки данных. Эту службу также можно использовать для связи приложения, работающего в Windows Azure или на другой облачной платформе, с локальным приложением или с планшетами и телефонами. Можно даже связывать бытовые приборы, датчики и другие устройства с центральным приложением или друг с другом. Service Bus — это универсальный механизм взаимодействия в облаке, доступный практически везде. Способ ее использования зависит от целей приложения.


## <a name="queues"></a>Очереди

Предположим, что необходимо связать два приложения с помощью очереди Service Bus. Эта ситуация показана на [рисунке 2](#Fig2).

<a name="Fig2"></a>![Схема очередей Service Bus][queues]
 
**Рисунок 2. Очереди Service Bus обеспечивают однонаправленное асинхронное взаимодействие.**
...
- Если получатель успешно обрабатывает сообщение, он вызывает функцию Complete и очередь удаляет сообщение. 
- Если получатель не может обработать сообщение, он вызывает функцию Abandon. Тогда очередь разблокирует сообщение и делает его доступным для других получателей.
...
Хотя очереди полезны, они не всегда являются правильным решением. Иногда лучше использовать разделы Service Bus. [На рисунке 3](#Fig3) показан принцип работы разделов.

<a name="Fig3"></a>![Схема разделов и подписок Service Bus][topics-subs]
 
**Рисунок 3. Подписанное приложение определяет фильтр, на основании которого оно может получать некоторые или все сообщения, отправленные в раздел Service Bus.**
...
- Подписчик 1 получает только сообщения, содержащие свойство *Продавец="Ava"*.
- Подписчик 2 получает сообщения, содержащие свойство *Продавец="Ruby"* и/или свойство *Сумма* со значением больше 100 000. Возможно, Ruby — это продавец, который хочет видеть собственные продажи и крупные продажи, независимо от того, кто их осуществил.
...
**Рисунок 4. Ретранслятор Service Bus обеспечивает синхронное двунаправленное взаимодействие между приложениями.**
...
Связь приложений всегда была частью комплексных решений, и эта задача вряд ли когда-нибудь потеряет актуальность. Предоставляя облачные технологии для решения этой задачи (очереди, разделы и ретрансляторы), Service Bus упрощает эту важную функцию и делает ее более доступной.
