...
Независимо от того, работает ли программное обеспечение в облаке или локально, оно часто должно взаимодействовать с другим программным обеспечением. Чтобы обеспечить такую возможность, Azure предоставляет служебную шину. В данной статье приводится ее обзор, описание и сценарии использования.
...
В рамках пространства имен можно использовать один или несколько экземпляров трех механизмов взаимодействия, по-разному связывающих приложения. Вот доступные варианты:
...
- *разделы*, которые обеспечивают однонаправленное взаимодействие с использованием *подписок*. Подобно очереди, раздел действует как посредник, но позволяет каждой подписке видеть только те сообщения, которые соответствуют определенным условиям;
...
Важно понимать, что хотя Service Bus и работает в облаке (т. е. в центрах обработки данных Microsoft Azure), использующие ее приложения могут работать где угодно. С помощью служебной шины можно связать приложения, выполняемые, например, в Azure и в локальном центре обработки данных. Эту службу также можно использовать для связи приложения, работающего в Azure или на другой облачной платформе, с локальным приложением или с планшетами и телефонами. Можно даже связывать бытовые приборы, датчики и другие устройства с центральным приложением или друг с другом. Service Bus - это универсальный механизм взаимодействия в облаке, доступный практически везде. Способ его использования зависит от целей приложения.


## <a name="queues"></a>Очереди

Предположим, вам необходимо связать два приложения с помощью очереди Service Bus. Эта ситуация показана на [рисунке 2](#Fig2).
...
- если получатель успешно обрабатывает сообщение, вызывается функция "Завершено" и очередь удаляет сообщение; 
- если получатель не может обработать сообщение, вызывается функция "Отменить", очередь разблокирует сообщение и делает его доступным для других получателей;
...
Очереди удобны во многих ситуациях. Они позволяют приложениям взаимодействовать, даже если эти приложения работают не одновременно. Это особенно удобно в случае пакетных и мобильных приложений. Очередь с несколькими получателями обеспечивает автоматическую балансировку нагрузки, так как отправленные сообщения распределяются между получателями.


## <a name="topics"></a>Разделы

Хотя очереди удобны, они не всегда являются правильным выбором. Иногда лучше использовать разделы Service Bus. [Рисунок 3](#Fig3) иллюстрирует принцип работы разделов.
...
- Подписчик 2 получает сообщения, содержащие свойство *Seller="Ruby"* и/или свойство *Amount* со значением больше 100 000. Возможно, Ruby - продавец, который хочет видеть собственные продажи и все крупные продажи, независимо от того, кто их осуществил.
- Подписчик 3 задал для фильтра значение *True*. Это означает, что он получает все сообщения. Это приложение может, к примеру, вести журнал аудита, и ему нужно видеть все сообщения.
...
Связь приложений всегда была частью комплексных решений, и эта задача вряд ли когда-нибудь потеряет актуальность. Предоставляя облачные технологии для связи приложений посредством очередей, разделов и ретрансляторов, Service Bus делает эту важнейшую функцию проще и доступнее.
