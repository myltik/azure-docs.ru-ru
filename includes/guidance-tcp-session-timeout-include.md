## <a name="tcp-settings-for-azure-vms"></a>Параметры TCP для виртуальных машин Azure
...
Для [Linux][linux] следует изменить переменные ядра.
net.ipv4.tcp_keepalive_time = 120 net.ipv4.tcp_keepalive_intvl = 30 net.ipv4.tcp_keepalive_probes = 8

А для [Windows][windows] следует изменить значения реестра.
