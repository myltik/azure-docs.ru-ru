С помощью диспетчера ресурсов Azure можно определить параметры значений, которые должны указываться на этапе развертывания шаблона. В шаблоне есть раздел "Параметры", содержащий все значения параметров. Для этих значений необходимо определить параметры, которые будут зависеть от развертываемого проекта либо от среды, в которой выполняется развертывание. Не задавайте параметры для значений, которые не меняются. Значение каждого параметра в шаблоне определяет развертываемые ресурсы.

Далее следует описание каждого параметра в шаблоне.

### gatewayName

Имя шлюза. Приложение API регистрируется в этом шлюзе.

    "gatewayName": {
      "type": "string"
    }

### apiAppName

Имя создаваемого приложения API. Имя должно иметь длину не менее 8 и не более 50 символов.
    
    "apiAppName": {
      "type": "string"
    }

### apiAppSecret

Секрет приложения API. Это значение должно быть строкой с кодировкой Base64. Это должна быть случайная 64-символьная строка, содержащая только целые числа и символы нижнего регистра.

    "apiAppSecret": {
      "type": "securestring"
    }

### location

Расположение нового приложения API. Допустимые расположения можно получить, выполнив команду PowerShell `Get-AzureLocation` или команду интерфейса командной строки Azure `azure location list`.

    "location": {
      "type": "string"
    }

<!---HONumber=August15_HO6-->