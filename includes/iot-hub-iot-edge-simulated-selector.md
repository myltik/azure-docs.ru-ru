...
Модуль связи Центра Интернета вещей открывает одиночное HTTP-подключение от шлюза к Центру Интернета вещей. HTTP — один из трех протоколов, понятных для центра IoT. Этот модуль позволяет избежать открытия отдельного подключения для каждого устройства путем мультиплексирования подключений со всех устройств через одно подключение. Такой подход позволяет подключить много устройств при помощи одного шлюза. 

В примере этот модуль:

1. Принимает сообщения со свойством ключа устройства Центра Интернета вещей, назначенным предыдущим модулем. 
1. Отправляет содержимое сообщения в Центр Интернета вещей при помощи протокола HTTP. 
