...
1. Следуя процедурам, приведенным в разделе [HDInsight Storm - приступая к работе](http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-storm-getting-started/), создайте новый кластер HDInsight и подключитесь к нему через удаленный рабочий стол.
...
3. Для установки пакета в локальный репозиторий Maven выполните следующую команду. Это позволит добавить в качестве ссылки в проекте Storm позднее.

		mvn install:install-file -Dfile=target\eventhubs-storm-spout-0.9-jar-with-dependencies.jar -DgroupId=com.microsoft.eventhubs -DartifactId=eventhubs-storm-spout -Dversion=0.9 -Dpackaging=jar

4. В Eclipse создайте новый проект Maven (щелкните **File**(Файл), затем **New**(Создать), затем **Project**(Проект).

   	![][12]

5. Выберите **Использовать расположение рабочей области по умолчанию**, затем нажмите **Next**(Далее)

6. Выберите архетип **maven-archetype-quickstart**, а затем нажмите **Next**(Далее)

7. Вставьте **GroupId** и **ArtifactId**, затем щелкните **Finish**(Готово)

8. В **pom.xml** добавьте следующие зависимости в узел "<dependency>".
...
9. В папке **src** создайте файл с именем **Config.properties** и скопируйте следующее содержимое, подставляя следующие значения:
	...
	Значение для **eventhub.receiver.credits** определяет, сколько событий являются пакетными перед их выпуском в конвейер Storm. Для простоты в данном примере это значение равно 10. В рабочей среде обычно следует задавать более высокие значения; например, 1024.
