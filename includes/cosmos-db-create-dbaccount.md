...
2. В области слева щелкните **Создать**, далее — **Базы данных**, затем выберите **Azure Cosmos DB** > **Создать**.
...
3. В колонке **Новая учетная запись** укажите необходимую конфигурацию учетной записи Azure Cosmos DB. 
    ...
    API|SQL (DocumentDB)|Мы будем программировать [API DocumentDB](../articles/cosmos-db/documentdb-introduction.md) далее в этой статье.|
...
6.  Если открылось окно уведомлений, это означает, что развертывание прошло успешно. Закройте это окно и откройте новую учетную запись, щелкнув плитку **Все ресурсы** на панели мониторинга. 
    ...
    ![Учетная запись Cosmos DB на плитке "Все ресурсы"](./media/cosmos-db-create-dbaccount/all-resources.png)
