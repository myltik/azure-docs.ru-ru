...
5. Если вы хотите выполнить локальную отладку аутентификации, откройте папку App_Start, откройте файл проекта WebApiConfig.cs и добавьте следующий код в метод **Register**:
	...
	Благодаря этому локальный проект будет выполняться так, словно он размещен в Azure (включая использование параметров AuthorizeLevel). Без этого параметра все направляемые в *localhost* HTTP-запросы разрешаются без аутентификации, несмотря на параметр AuthorizeLevel.  
