...
При создании приложения в Azure служба Azure предоставляет поддомен в домене cloudapp.net, чтобы пользователи могли получить доступ к приложению по такому URL-адресу, как http://&lt;*myapp*>.cloudapp.net. Однако вы можете предоставить собственное доменное имя, например contoso.com.

> [WACOM.NOTE]
> Процедуры в этом задании применяются для облачных служб Azure. Для учетных записей хранения см. [Настройка пользовательского доменного имени для учетной записи хранения Azure][]. Сведения о веб-сайтах см. в разделе [Настройка пользовательского доменного имени для веб-сайта Azure][].
...
>
> Удобно, что IP-адрес данного слота развертывания (производственного или промежуточного) сохраняется при переходе от промежуточного к производственному развертыванию или при выполнении обновления на месте существующего развертывания. Дополнительные сведения о том, как производить эти действия, см. в разделе [Управление облачными службами][].
...
1.  Чтобы найти доменное имя **.cloudapp.net**, назначенное для вашей облачной службы, используйте один из этих методов.
	...
	-   Установите и настройте [Azure Powershell][], а затем используйте следующую команду:
...
-   [Сопоставление содержимого CDN с пользовательским доменом][]
  ...
  [Сопоставление содержимого CDN с пользовательским доменом]: http://msdn.microsoft.com/en-us/library/windowsazure/gg680307.aspx
