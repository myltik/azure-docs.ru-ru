# Настройка пользовательского имени домена для облачной службы Azure
...
> [WACOM.NOTE] 
> Процедуры в этом задании применяются для облачных служб Azure. Для учетных записей хранения см. [Настройка пользовательского имени домена для учетной записи хранения Azure](../storage-custom-domain-name/). Для веб-сайтов см. [Настройка пользовательского имени домена для веб-сайта Azure](../web-sites-custom-domain-name/).
...
Запись CNAME сопоставляет *конкретный* домен, такой как **contoso.com** или **www.contoso.com**, с каноническим именем домена. В этом случае каноническим именем домена является имя домена вашего размещенного приложения в Azure **&lt;myapp>.cloudapp.net**. После того как создано имя CNAME, оно создает псевдоним для **&lt;myapp>.cloudapp.net**. Запись CNAME будет автоматически разрешаться в IP-адрес вашей службы **&lt;myapp>.cloudapp.net**, поэтому при изменении IP-адреса облачной службы не нужно предпринимать никаких действий.
...
Запись A сопоставляет домен, такой как **contoso.com** или **www.contoso.com**, *или домен с подстановочными знаками*, такой как **\*.contoso.com**, с IP-адресом. В случае облачной службы Azure – виртуальный IP-адрес службы. Так что основным преимуществом записи А перед записью CNAME является то, что у вас может быть одна запись, использующая подстановочные знаки, например ***.contoso.com**, которая будет обрабатывать запросы для нескольких поддоменов, таких как **mail.contoso.com**, **login.contoso.com** или **www.contso.com**.

> [WACOM.NOTE]
> Поскольку запись А сопоставляется со статическим IP-адресом, она не может автоматически разрешаться в IP-адрес вашей облачной службы. IP-адрес, используемый службой облака, выделяется при первом развертывании в пустую область (производственном или промежуточном). При удалении развертывания для гнезда Azure освобождает IP-адрес, и все будущие развертывания в область могут получить новый IP-адрес.
> 
...
1. Чтобы найти имя домена **.cloudapp.net**, назначенное для вашей облачной службы, используйте один из этих методов.
  ...
  * Войдите на [портал управления Azure], выберите вашу облачную службу, выберите **панель мониторинга** и найдите **URL-адрес сайта** в разделе **краткое описание**.

  		  ![Раздел краткого описания, в котором отображается URL-адрес][csurl]

  * Установите и настройте [Powershell Azure](../install-configure-powershell/), а затем используйте следующую команду:
...
1. Чтобы получить IP-адрес для своей облачной службы, используйте один из следующих способов.
  ...
  * Войдите на [портал управления Azure], выберите вашу облачную службу, выберите **панель мониторинга** и найдите **Общедоступный виртуальный IP-адрес (VIP-адрес)** в разделе **краткое описание**.
