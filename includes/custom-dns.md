# Настройка пользовательского имени домена для облачной службы Windows Azure

При создании приложения в Windows Azure система Windows Azure предоставляет дочерний домен в домене cloudapp.net, поэтому пользователи могут получить доступ к приложению по URL-адресу, такому как http://&lt;*myapp*>.cloudapp.net. Однако вы можете сделать свое приложение доступным на своем собственном имени домена, например contoso.com.

> [WACOM.NOTE] 
> Процедуры в этом задании применяются для облачных служб Windows Azure. Для учетных записей хранения см. [Настройка пользовательского имени домена для учетной записи хранения Windows Azure](../storage-custom-domain-name/). Для веб-сайтов см. [Настройка пользовательского имени домена для веб-сайта Windows Azure](../web-sites-custom-domain-name/).

Содержание:

-   [Что такое записи CNAME и A](#access-app)
-   [Добавление записи CNAME для своего пользовательского домена](#add-cname)
-   [Добавление записи A для своего пользовательского домена](#add-aname)

<h2><a name="access-app"></a>Что такое записи CNAME и A</h2>

И записи CNAME (или записи псевдонимов), и записи А позволяют связать имя домена с конкретным сервером (или службой в данном случае), но они работают по-разному. При использовании записей А с облачными службами Windows Azure существуют также некоторые особые соображения, которые следует учесть, принимая решение, какие именно записи использовать.

###Запись CNAME или запись псевдонима

Запись CNAME сопоставляет *конкретный* домен, такой как **contoso.com** или **www.contoso.com**, с каноническим именем домена. В этом случае каноническим именем домена является имя домена вашего размещенного приложения в Windows Azure **&lt;myapp>.cloudapp.net**. После того как создано имя CNAME, оно создает псевдоним для **&lt;myapp>.cloudapp.net**. Запись CNAME будет автоматически разрешаться в IP-адрес вашей службы **&lt;myapp>.cloudapp.net**, поэтому при изменении IP-адреса облачной службы не нужно предпринимать никаких действий.

> [WACOM.NOTE] 
> Некоторые регистраторы доменов позволяют сопоставлять дочерние домены только при использовании записи CNAME, например www.contoso.com, а не корневых имен, таких как contoso.com. Дополнительные сведения о записи CNAME см. в документации вашего регистратора, <a href="http://en.wikipedia.org/wiki/CNAME_record">информации в Википедии о записи CNAME</a> или <a href="http://tools.ietf.org/html/rfc1035">в документе "Имена доменов IETF – реализация и спецификация"</a>.

###Запись А

Запись A сопоставляет домен, такой как **contoso.com** или **www.contoso.com**, *или домен с подстановочными знаками*, такой как **\*.contoso.com**, с IP-адресом. В случае облачной службы Windows Azure – виртуальный IP-адрес службы. Так что основным преимуществом записи А перед записью CNAME является то, что у вас может быть одна запись, использующая подстановочные знаки, например ***.contoso.com**, которая будет обрабатывать запросы для нескольких поддоменов, таких как **mail.contoso.com**, **login.contoso.com** или **www.contso.com**.

> [WACOM.NOTE]
> Поскольку запись А сопоставляется со статическим IP-адресом, она не может автоматически разрешаться в IP-адрес вашей облачной службы. IP-адрес, используемый службой облака, выделяется при первом развертывании в пустую область (производственном или промежуточном). При удалении развертывания для гнезда Windows Azure освобождает IP-адрес, и все будущие развертывания в область могут получить новый IP-адрес.
> 
> Удобно, что IP-адрес данной области развертывания (производственного или промежуточного) сохраняется при переходе от промежуточного к производственному развертыванию или при выполнении обновления на месте существующего развертывания. Дополнительные сведения о том, как производить эти действия, см. в разделе [Управление облачными службами](../cloud-services-how-to-manage/).
...
1. Чтобы найти имя домена **.cloudapp.net**, назначенное для вашей облачной службы, используйте один из этих методов.

  * Войдите на [портал управления Windows Azure], выберите вашу облачную службу, выберите **панель мониторинга** и найдите **URL-адрес сайта** в разделе **краткое описание**.

  		  ![Раздел краткого описания, в котором отображается URL-адрес][csurl]

  * Установите и настройте [Powershell Windows Azure](../install-configure-powershell/), а затем используйте следующую команду:
...
Например, следующая запись CNAME пересылает весь трафик из **www.contoso.com** в **contoso.cloudapp.net**, имя пользовательского домена вашего развернутого приложения:
...
Посетитель из **www.contoso.com** никогда не увидит настоящий хост (contoso.cloudapp.net), поэтому процесс пересылки невидим для конечного пользователя.

> [WACOM.NOTE]
> Приведенный выше пример применим только к трафику дочернего домена <strong>www</strong>. Поскольку с записями CNAME нельзя использовать подстановочные знаки, необходимо создать один CNAME для каждого домена или поддомена. Если вы хотите направить трафик из поддоменов, таких как *.contoso.com, на адрес cloudapp.net, можно настроить элемент <strong>Перенаправление URL-адреса</strong> или <strong>Переадресация URL</strong> в настройках DNS или создать A-запись.
...
1. Чтобы получить IP-адрес для своей облачной службы, используйте один из следующих способов.
  ...
  * Войдите на [портал управления Windows Azure], выберите вашу облачную службу, выберите **панель мониторинга** и найдите **Общедоступный виртуальный IP-адрес (VIP-адрес)** в разделе **краткое описание**.
...
-   [Сопоставление содержимого CDN с пользовательским доменом][]
  ...
  [Переключение VIP]: http://msdn.microsoft.com/ru-ru/library/ee517253.aspx
   [Создание записи CNAME, связывающей дочерний домен с учетной записью хранилища]: #create-cname
  [Портал управления Windows Azure]: https://manage.windowsazure.com
  [Диалоговое окно "Проверить пользовательский домен"]: http://i.msdn.microsoft.com/dynimg/IC544437.jpg
