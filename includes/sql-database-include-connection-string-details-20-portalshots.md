...
5. Запишите имя, указанное в параметрах **База данных SQL** и **Имя сервера**.  Именем пользователя будет yourusername@yourserver.
   
    ![Получение сведений о подключении][3-get-connection-details]
6. Вставьте данные подключения в код клиентской программы.  Текст {your_password_here} необходимо заменять на фактический пароль.
