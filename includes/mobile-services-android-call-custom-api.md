...
5. В обозревателе проектов щелкните правой кнопкой мыши имя проекта *src* в папке (`com.example.{your projects name}`), выберите пункт **Создать**, а затем **Классы**. В диалоговом окне введите **MarkAllResult** в поле имя класса, нажмите кнопку "ОК" и замените результирующее определение класса на следующий код:
