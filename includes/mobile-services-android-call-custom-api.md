...
6.  В файле **ToDoActivity.java** найдите метод **refreshItemsFromTable** и убедитесь в том, что первая строка кода начинается следующим образом:

        mToDoTable.where().field("complete").eq(false)

    Это приводит к фильтрации элементов, так что завершенные элементы не возвращаются в результатах запроса.

7.  Убедитесь, что **ToDoActivity.java** импортирует ApiOperationCallback в блок импорта в начале файла:
