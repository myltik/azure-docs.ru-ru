...
##<a name="update-app"></a>Обновите приложение, чтобы вызвать настраиваемый API

1. Мы добавим кнопку с именем "Выполнить все" рядом с кнопкой с существующей кнопкой и переместим обе кнопки на одну строку вниз. Откройте в Eclipse файл *res\layout\activity_to_do.xml* быстрой настройки проекта, найдите элемент **LinearLayout**, содержащий элемент **Button** с именем `buttonAddToDo`. Скопируйте элемент **LinearLayout** и вставьте его непосредственно рядом с оригиналом. Удалить элемент **Button** из первого элемента **LinearLayout**.
...
4. Откройте файл res\values\string.xml и добавьте следующую строку кода:
    	...
    	<string name="complete_button_text">Выполнить все</string>
...
6. Найдите метод **refreshItemsFromTable** в файле **ToDoActivity.java** и убедитесь в том, что первая строка кода начинается следующим образом:

        mToDoTable.where().field("complete").eq(false)

	Это приводит к фильтрации элементов, так что завершенные элементы не возвращаются в результатах запроса.

7. В файле **ToDoActivity.java** добавьте следующий метод:
...
1. В меню **Пуск** щелкните **Выполнить**, чтобы запустить проект в эмуляторе Android.
...
2. В приложении введите какой-либо текст в поле **Вставить TodoItem**, а затем нажмите кнопку **Добавить**.

3. Повторяйте предыдущий шаг, пока в список не будут добавлены несколько элементов todo списка дел.
