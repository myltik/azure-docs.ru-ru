...
1. Мы добавим кнопку с подписью "Выполнить все" рядом с существующей кнопкой и переместим обе кнопки на одну строку вниз. Откройте в Android Studio файл *res\layout\activity_to_do.xml* проекта быстрого запуска, найдите элемент **LinearLayout**, содержащий элемент **Button** с именем `buttonAddToDo`. Скопируйте элемент **LinearLayout** и вставьте его непосредственно рядом с оригиналом. Удалить элемент **Button** из первого элемента **LinearLayout**.
