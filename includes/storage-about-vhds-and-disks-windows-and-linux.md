### <a name="disk-persistence"></a>Сохранение данных на диске 

В следующей таблице показаны различные состояния для разных дисков на виртуальной машине Azure, а также указано, сохраняются ли данные в каждом из этих состояний.

| Диск | Начало | Остановить/<br>Отменить выделение | Приостановить | Перезагрузить<br>boot | Завершить<br>работу | Удалить | Сбой | Изменить размер | 
| ---- | ----- | ---- | ---- | ---- | ----  | ------ | ------- | ------ | 
| Диск ОС | Да | Да  | Да | Да | Да  | Нет | Нет  | Да | 
| ОЗУ  | Да | Да | Да | Да | Нет   | Нет | Нет | Нет | 
| Локальный временный диск | Да | Нет | Да | Нет | Нет  | Нет | Нет | Нет | 
| Подключенный диск данных | Да | Да | Да | Да | Да  | Да | Да | Да | 
