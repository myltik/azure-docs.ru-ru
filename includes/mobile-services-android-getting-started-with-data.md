...
1. Убедитесь, что в теге **зависимостей** в файле *build.gradle (Module app)* присутствуют следующие строки, если они отсутствуют - добавьте их. Этот код добавляет ссылки на пакет SDK клиента Android мобильных служб.
...
6. Мы удалим расположенный в памяти список, используемый приложением в данный момент, чтобы заменить его мобильной службой. В классе **ToDoActivity** закомментируйте следующую строку кода, которая определяет существующий список **toDoItemList**.

		public List<ToDoItem> toDoItemList = new ArrayList<ToDoItem>();

7. Сохраните файл, и проект укажет ошибки сборки. Найдите три оставшихся места, где используется переменная `toDoItemList`, и закомментируйте указанные разделы. Это полностью удалит список, расположенный в памяти. 
...
9. Найдите класс *ProgressFilter* в нижней части файла и раскомментируйте его. Этот класс служит для отображения индикатора 'loading', когда *MobileServiceClient* выполняет сетевые операции.
...
12. В методе **onCreate** раскомментируйте следующие строки кода, в которых определяется переменная **MobileServiceClient**:
  	...
  	Этот код создает новый экземпляр *MobileServiceClient*, который используется для получения доступа к мобильной службе. Он также создает экземпляр *MobileServiceTable*, который используется для хранения данных прокси-сервера в мобильной службе.

13. В приведенном выше коде замените `MobileServiceUrl` и `AppKey` URL-адресом и ключом приложения из мобильной службы соответственно.
