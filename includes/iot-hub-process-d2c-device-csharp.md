...
2. Добавьте в метод **Main** прямо перед строкой `Console.ReadLine()` следующий метод:
    ...
    > [AZURE.NOTE]Для упрощения в этом руководстве не реализуются какие-либо политики повтора. В рабочем коде следует реализовать политику повтора (например, экспоненциальную задержку), как предлагается в статье MSDN [Обработка временного сбоя].

<!-- Links -->
[Обработка временного сбоя]: https://msdn.microsoft.com/ru-RU/library/hh675232.aspx
