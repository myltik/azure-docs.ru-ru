...
6. Добавьте следующий метод в класс **Program**.
	...
	Этот метод создаст новое удостоверение устройства с идентификатором **myFirstDevice** (если удостоверение с таким идентификатором уже существует, он просто извлечет его). Затем приложение отображает первичный ключ для данного удостоверения. Этот ключ будет использоваться имитацией устройства для подключения к центру IoT.
...
1. В текущем решении Visual Studio щелкните **Файл -> Новый -> Проект**, чтобы создать новый проект классического приложения Visual C# с помощью шаблона проекта **Консольное приложение**. Присвойте проекту имя **ReadDeviceToCloudMessages**.
