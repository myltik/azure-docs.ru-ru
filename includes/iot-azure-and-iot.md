...
> [AZURE.NOTE] Подробное рассмотрение архитектуры IoT см. в статье [Службы IoT Microsoft Azure: эталонная архитектура][lnk-refarch].
...
Устройство может взаимодействовать непосредственно с конечной точкой облачного шлюза или (если устройство не может использовать протоколы связи, поддерживаемые облачным шлюзом) подключаться через промежуточный шлюз, например [шлюз протокола центра IoT][lnk-protocol-gateway], выполняющий преобразование протоколов. Пример:
