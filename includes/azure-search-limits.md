| Ресурс | Бесплатно | Базовый (предварительная версия) <sup>6</sup> | S1 | S2 <sup>7</sup> |
| --- | --- | --- | --- | --- |
| Максимальное количество служб поиска <sup>8</sup> |1 на подписку |12 на подписку |12 на подписку |1 на подписку |
| Максимальный размер хранилища <sup>1</sup> |50 МБ или 10 000 документов |2 ГБ на службу |25 ГБ на секцию или 300 ГБ документов на службу |100 ГБ на секцию или 1,2 ТБ на службу |
| Максимальное число документов |10 000 документов в целом |1 млн на службу |15 млн на секцию (до 180 млн документов на службу) |60 млн на секцию (до 720 млн документов на службу) |
| Максимальное число индексов |3 |5 |50 |200 |
| Максимальное число индексаторов |3 |5 |50 |200 |
| Максимальное число источников данных для индексаторов |3 |5 |50 |200 |
| Индекс: максимальное число полей на индекс |1000 |100 <sup>5</sup> |1000 |1000 |
| Максимальное число профилей оценки на индекс |16 |16 |16 |16 |
| Индекс: максимальное число функций на профиль |8 |8 |8 |8 |
| Индексаторы: максимальная нагрузка индексирования на вызов |10 000 документов |Ограничивается только максимальным числом документов |Ограничивается только максимальным числом документов |Ограничивается только максимальным числом документов |
| Индексаторы: максимальное время выполнения |3 минуты |24 часа |24 часа |24 часа |
| Индексатор BLOB-объектов: максимальный размер BLOB-объектов в МБ |16 |16 |128 |256 |
| Индексатор BLOB-объектов: максимальное число символов в содержимом, извлеченном из BLOB-объекта |32 000 |64 000 |4 млн |4 млн |
| Число запросов в секунду <sup>2</sup> |Недоступно |~3 на реплику |~15 на реплику |~60 на реплику |
| Масштабирование: максимальное число единиц поиска <sup>3</sup> |Недоступно |До 3 единиц (3 реплики и 1 секция) |36 единиц |36 единиц |
| Цены <sup>4</sup> |Недоступно |75 долл. на единицу поиска в месяц |250 долл. на единицу поиска в месяц |1000 долл. на единицу поиска в месяц |

<sup>1</sup> Размер хранилища представляет собой фиксированный размер или число документов на службу, в зависимости от того, какая из этих величин меньше.

<sup>2</sup> QPS определяется приблизительно, в зависимости от эвристики, с использованием имитированных и фактических нагрузок клиента для получения предполагаемых значений. Точная пропускная способность QPS будет зависеть от характера данных и природы запроса.

<sup>3</sup> Единицы поиска — это тарифицируемые единицы для реплики или секции. Они необходимы для хранилища, индексирования и операций запроса. Допустимые сочетания реплик и разделов, позволяющие оставаться в допустимых пределах (3 и 36 единиц для уровней "Базовый" и "Стандартный", соответственно), см. в разделе [Планирование емкости](../articles/search/search-capacity-planning.md).

<sup>4</sup> Цена — на рынке США, иллюстрирующая относительные затраты между альтернативными уровнями. Различные рынки имеют различные цены. Для цен в других валютах обратитесь к [странице цен](https://azure.microsoft.com/pricing/details/search/). Цена определяется за одну единицу поиска (SU). На уровне S1 конфигурация из 3 единиц поиска (например 3 реплики и 1 раздел) будет стоить в среднем 750 долларов в месяц. При масштабировании до меньшего количества единиц поиска в месяц счет пересчитывается так, что вы платите только за то, что фактически используете.

<sup>5</sup> Уровень "Базовый" — единственный уровень с нижним пределом, составляющим 100 полей на индекс.

<sup>6</sup> [Уровень "Базовый"](http://aka.ms/azuresearchbasic) доступен по вводной цене в размере 50 % от полной цены на этапе предварительной версии.

<sup>7</sup> Для S2 требуется билет службы. Он не может быть подготовлен на портале. Обратитесь за помощью в службу поддержки или отправьте письмо по адресу azuresearch_contact@microsoft.com.

<sup>8</sup> Максимальное число служб на подписку можно увеличить для любого уровня, кроме бесплатного. Обратитесь за помощью в службу поддержки или отправьте письмо по адресу azuresearch_contact@microsoft.com.

<!---HONumber=AcomDC_0608_2016-->