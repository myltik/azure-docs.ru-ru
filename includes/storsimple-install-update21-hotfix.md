...
3. В поле поиска каталога Центра обновления Майкрософт введите номер необходимого исправления в базе знаний, например **3179904**, а затем нажмите кнопку **Найти**.

    Появится список исправлений, например **Совокупное обновление пакета программного обеспечения версии 2.2 для StorSimple серии 8000**.
...
> [AZURE.IMPORTANT] В случае установки обновления 2.2 установите только двоичный файл, имя которого начинается с all-hcsmdssoftwareudpate. Не устанавливайте обновление агента Cis и MDS, имя которого начинается с all-cismdsagentupdatebundle. В противном случае произойдет сбой.
...
1.  Отслеживайте ход выполнения установки с помощью команды `Get-HcsUpdateStatus`. Обновление завершится, когда `RunInProgress` поменяется на `False`.
