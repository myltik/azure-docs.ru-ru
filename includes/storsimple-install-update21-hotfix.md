...
3. В поле поиска каталога Центра обновления Майкрософт введите номер необходимого исправления в базе знаний, например **3162954**, а затем нажмите кнопку **Найти**.

    Появится список исправлений, например **Совокупное обновление пакета программного обеспечения версии 2.1 для StorSimple серии 8000**.
...
5. Найдите дополнительные исправления, перечисленные в приведенной выше таблице (**3103616**, **3146621**), и добавьте каждое из них в корзину.
...
Выполните следующие действия для установки и проверки обычных исправлений. Если вы уже установили их с помощью портала Azure, перейдите к разделу об [установке и проверке исправлений режима обслуживания](#to-install-and-verify-maintenance-mode-hotfixes).
...
4. Выберите **Вариант 1**, чтобы войти на устройство с правами на полный доступ. Рекомендуем сначала установить исправление на пассивный контроллер.
...
7. Проверьте обновление с помощью командлета `Get-HcsUpdateStatus`. Сначала обновится пассивный контроллер. После этого будет выполнена отработка отказа. Затем обновление будет применено на другом контроллере. Установка обновления завершится, когда оба контроллера будут обновлены.
...
8. Когда установка обновления будет завершена, проверьте версии программного обеспечения системы. Тип:
...
9. Повторите шаги 3–5 для установки оставшихся обычных исправлений.
	...
	> [AZURE.IMPORTANT] Прежде чем применить оставшиеся обновления, необходимо перезапустить активный контроллер с помощью командлета `Restart-HcsController`.
	
	
	- обновление KB3146621 для iSCSI;
	- обновление KB3103616 для WMI.
	

10. Этот шаг можно пропустить, если обновление выполняется с обновления 2. При обновлении с более ранней версии, чем обновление 2, также необходимо скачать:


	- обновление KB3121900 для драйвера LSI;

	- обновление KB3090322 для Spaceport;
	
	- обновление KB3080728 для Storport.
...
Обратите внимание, что если встроенное ПО диска уже обновлено, устанавливать эти обновления не требуется. Запустите командлет `Get-HcsUpdateAvailability` из последовательной консоли устройства, чтобы проверить, доступны ли обновления и являются ли они критическими (режим обслуживания) или некритическими (обычный режим).
...
2.  После завершения установки контроллер, на котором установлено исправление режима обслуживания, будет перезагружен. В качестве варианта 1 войдите в систему с полным доступом и проверьте версию встроенного ПО диска. Тип:
	...
	Результат выполнения команды показан ниже.
	 ...
	 Запустите команду `Get-HcsFirmwareVersion` на втором контроллере, чтобы проверить обновление версии программного обеспечения. Затем можно выйти из режима обслуживания. Для этого введите следующую команду для каждого контроллера устройства:
