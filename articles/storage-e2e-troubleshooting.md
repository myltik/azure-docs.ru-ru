...
В этом учебнике содержатся практические исследования сквозного сценария устранения неполадок. Подробное концептуальное руководство по устранению неполадок в приложениях хранилища Azure см. в разделе [Мониторинг, диагностика и устранение неполадок хранилища](../articles/storage-monitoring-diagnosing-troubleshooting/).
...
- **Портал управления Azure**. Настроить метрики и ведение журнала для учетной записи хранения можно на портале. Можно также просматривать диаграммы и графики, которые показывают, как производительность приложения изменяется со временем, и настроить оповещения на портале для уведомления в случае, если производительность выполняемого приложения отличается от ожидаемой для указанной метрики.
	
	В этом учебнике показано, как отслеживать учетную запись хранения с помощью [портала управления Azure](https://manage.windowsazure.com/). В разделе [Мониторинг учетной записи хранения](storage-monitor-storage-account.md) приведены сведения о настройке мониторинга на портале.

	Можно также использовать более новую [предварительную версию портала Azure](https://portal.azure.com/), но обратите внимание, что она по-прежнему является предварительным выпуском.
...
> [AZURE.NOTE]Поминутные метрики невозможно задавать с помощью портала управления Azure. Тем не менее рекомендуется установить их для целей данного учебника, а также для изучения проблем производительности в приложении. Поминутные метрики можно задать с помощью PowerShell, как показано ниже, программно либо через портал предварительной версии Azure.
>
> Обратите внимание, что на портале управления Azure не удастся отобразить поминутные метрики, доступны только почасовые метрики.
...
## Просмотр данных метрик на портале

После запуска приложения в течение определенного времени можно просмотреть графики метрик, отображаемых на портале, чтобы проверить выполнение операций службой. Во-первых, добавим метрику **процента успешных операций** на странице "Мониторинг".

1. Перейдите на панель мониторинга для учетной записи на портале управления, а затем выберите "Мониторинг", чтобы просмотреть страницу мониторинга.
2. Щелкните **Добавить метрики** для отображения диалогового окна **Выбор метрик**.
3. Прокрутите окно вниз и найдите группу **Процент успешных операций**, разверните ее, а затем выберите **Статистические**, как показано на рисунке ниже. Эта метрика собирает данные о проценте успешно выполненных из всех операций BLOB-объекта.

![Выбор метрики](./media/storage-e2e-troubleshooting/choose-metrics-portal-1.png)

На портале появится график наблюдения **процента успешных операций**, а также любые другие показатели, которые могли быть добавлены ранее \(можно одновременно отобразить до шести графиков\). На приведенном ниже рисунке видно, что процент успешных операций немного ниже 100 %, поэтому по нашему сценарию далее проведем исследование путем анализа журналов в анализаторе сообщений:

![Диаграмма метрик на портале](./media/storage-e2e-troubleshooting/portal-metrics-chart-1.png)
...
> [AZURE.NOTE]После включения метрики хранилища может потребоваться некоторое время, чтобы данные метрики отобразились на портале. Это вызвано тем, что почасовые метрики для предыдущего часа не отображаются на портале, пока не истечет текущий час. Кроме того, в настоящее время на портале не отображаются поминутные показатели. Поэтому в зависимости от времени включения метрик может потребоваться до двух часов, чтобы увидеть данные метрик на портале.
...
AzCopy можно загрузить на странице [Загрузки Azure](http://azure.microsoft.com/downloads/). Дополнительные сведения об использовании AzCopy см. в разделе [Использование AzCopy с хранилищем Microsoft Azure](storage-use-azcopy.md).
...
![Начальная страница анализатора сообщений](./media/storage-e2e-troubleshooting/mma-start-page-1.png)
...
После применения фильтра исключаются строки журнала клиента, так как журнал клиента не содержит столбец \*\*StatusCode\*\*. Для начала просмотрим журналы сервера и трассировки сети, чтобы найти ошибки 404, а затем вернемся в журнал клиента для проверки операций клиента, которые приводят к их возникновению.
...
>
> <code>&\#42;StatusCode \>= 400 или !&\#42;StatusCode</code>
>
...
| Увеличение HTTP в PercentThrottlingError | HTTP.Response.StatusCode == 500 &\#124;&\#124; HTTP.Response.StatusCode == 503 | Сеть |
