...
> [AZURE.NOTE] В этом разделе предполагается, что у вас нет этих ключей. Вы должны создать файлы **`myPrivateKey.pem`** и **`myCert.pem`** для внесения ясности. Если у вас уже есть открытый и закрытый ключи, сохраненные в **`~/.ssh/id_rsa`**, вы можете просто ввести `openssl req -x509 -key ~/.ssh/id_rsa -nodes -days 365 -newkey rsa:2048 -out myCert.pem`, чтобы получить PEM-файл, который вам необходим для передачи в Azure.
...
<!--Каждый раздел должен иметь описание дальнейших действий и ссылки следующие по смыслу разделы для поддержания у клиента интереса к материалу.-->

1. Установите [межплатформенного интерфейса Azure (xplat-cli)], если вы еще не сделали это, и либо войдите в систему, используя идентификатор организации или школы, либо скачайте PUBLISHSETTINGS-файл и импортируйте его в вашу учетную запись.
...
Замечание. Если вы посмотрите на текущее состояние вашей работы в [портале](https://portal.azure.com), то увидите, что имя вашей облачной службы является одновременно группой ресурсов и доменом, как показано на следующем изображении:
