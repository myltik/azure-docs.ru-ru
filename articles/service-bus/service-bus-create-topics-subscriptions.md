...
Вернемся к сценарию розничной торговли. В этом сценарии очередь заменяется разделом и добавляется подписка, которая будет использоваться компонентом системы управления запасами. Теперь система выглядит следующим образом:
