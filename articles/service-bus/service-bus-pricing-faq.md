...
- **Квоты на размер сообщения**
	- **Очередь, раздел и подписка**
		- **Размер сообщения**. Каждое сообщение ограничено суммарным размером 256 КБ, включая заголовки.
	...
	- **Ретрансляторы http и NetTcp**. В служебной шине отсутствует значение максимального размера этих сообщений.
