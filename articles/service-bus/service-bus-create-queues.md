...
Рассмотрим сценарий из мира розничной продажи, в котором данные о продажах из отдельных точек продаж (POS) должны направляться в систему управления запасами, которая использует эти данные для определения момента пополнения запасов. В этом решении используется обмен сообщениями служебной шины для обмена данными между терминалами и системой управления запасами, как показано на следующем рисунке:
...
### Регистрация для учетной записи и подписки служебной шины

Чтобы начать работу со служебной шиной, вам потребуется учетная запись Azure. Если у вас ее нет, вы можете зарегистрироваться для получения бесплатной пробной версии [здесь](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A85619ABF).

### Создание пространства имен службы

После получения подписки можно создать новое пространство имен. Пространству имен необходимо присвоить имя, которое будет уникальным среди всех учетных записей служебной шины. Каждое пространство имен выступает в качестве контейнера для набора сущностей служебной шины. Дополнительные сведения см. в разделе [Руководство по созданию и изменению пространства имен служебной шины](https://msdn.microsoft.com/library/azure/hh690931.aspx).

### Установка пакета NuGet

Чтобы использовать пространство имен службы служебной шины, приложение должно сослаться на сборку служебной шины Microsoft.ServiceBus.dll. Эту сборка является частью пакета SDK Microsoft Azure, который можно загрузить на [странице загрузки пакета SDK Azure](https://azure.microsoft.com/downloads/). Однако проще всего получить интерфейс API служебной шины и настроить свое приложение с учетом всех зависимостей служебной шины с помощью пакета NuGet для служебной шины. Дополнительные сведения об использовании пакета NuGet для служебной шины см. в разделе [Использование пакета NuGet для служебной шины](https://msdn.microsoft.com/library/dn741354.aspx).
...
Теперь когда вы ознакомились с основами использования очередей, обратитесь к разделу [Создание приложений, использующих разделы и подписки служебной шины](service-bus-create-topics-subscriptions.md). В этом разделе описаны возможности публикаций и подписок для обмена сообщения через посредника служебной шины.
