...
Вновь созданной политике назначается *первичный ключ* и *вторичный ключ*. Это криптографически строгие ключи. Их нельзя потерять — они всегда будут доступны на [классическом портале Azure][]. Вы можете использовать любой из созданных ключей и создавать их повторно в любое время. Однако в случае повторного создания или изменения первичного ключа в политике любая созданная на его основе подпись общего доступа станет недействительной.
...
Ниже будет показано, как отправить маркер SAS с помощью протокола AMQP, используя библиотеку [AMQP.Net Lite](http://amqpnetlite.codeplex.com), если нельзя использовать официальный пакет SDK служебной шины (например, в WinRT, .Net Compact Framework, .Net Micro Framework и Mono) при разработке на C&#35;. Разумеется, эта библиотека полезна для ознакомления с работой защиты на основе утверждений на уровне AMQP, так как вы видели, как работает она на уровне HTTP (с отправлением запроса HTTP POST и маркера SAS в заголовке Authorization). Тем не менее, не волнуйтесь! Если вам не требуются такие глубокие знания о AMQP, можно использовать официальный пакет SDK служебной шины для приложений .Net Framework, который сделает все за вас, или библиотеку [Azure SB Lite](http://azuresblite.codeplex.com) для всех других платформ (см. выше).
...
Дополнительные сведения об аутентификации с помощью служебной шины см. в статье [Проверка подлинности и авторизация в служебной шине](service-bus-authentication-and-authorization.md).
