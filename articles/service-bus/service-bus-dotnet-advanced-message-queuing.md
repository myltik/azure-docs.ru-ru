...
В этой статье предполагается, что вы уже создали пространство имен служебной шины, содержащее очередь с именем queue1. Если это не так, можно создать пространство имен и в очередь, используя [портал Azure](http://manage.windowsazure.com). Дополнительную информацию о создании пространства имен и очередей служебной шины см. в статье [Как использовать очереди служебной шины](service-bus-dotnet-how-to-use-queues.md).
...
Где `[namespace]` и `[SAS key]` получены на портале Azure. Дополнительные сведения см. в статье [Использование очередей служебной шины][].
...
Дополнительные сведения о межплатформенном обмене сообщениями с помощью служебной шины и AMQP 1.0 см. в [обзоре AMQP 1.0 для служебной шины](service-bus-amqp-overview.md).
