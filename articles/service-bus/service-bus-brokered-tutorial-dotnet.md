...
1. Запишите значение первичного ключа, связанного с политикой **RootManagerSharedAccessKey**, или скопируйте его в буфер обмена. Этот ключ потребуется позже.
...
1. Сначала создайте очередь. Назовите ее, например, `myQueue` и объявите ее сразу после операций управления, добавленных на предыдущем этапе в метод `Queue()`:
...
1. В методе `Queue()` создайте объект фабрики обмена сообщениями, указав только что созданный URI служебной шины в качестве аргумента. Добавьте следующий код сразу после операций управления, добавленных на предыдущем этапе. Не забудьте заменить `<yourNamespace>` на имя пространства имен вашей службы:
