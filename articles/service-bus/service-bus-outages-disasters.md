...
> [AZURE.NOTE] Этот пример хорошо работает в сценариях IaaS (инфраструктура как услуга), когда локальный диск или диск для MSMQ сопоставлен учетной записью хранения и сообщения надежно хранятся в MSMQ. Это не подходит для сценариев PaaS (платформа как услуга), например облачных служб и веб-приложений.
