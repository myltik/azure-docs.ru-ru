...
6. 	Оставьте в остальных полях диалогового окна значения по умолчанию (**Обмен сообщениями** и **Уровень Standard**), а затем щелкните значок галочки. Теперь система создает пространство имен службы и включает его. Вероятно, придется подождать несколько минут, пока система не выделит ресурсы для вашей учетной записи.
...
3.  Нажмите кнопку **Настроить** для просмотра политик общего доступа пространства имен. ![](./media/service-bus-java-how-to-use-topics-subscriptions/sb-queues-14.png)
...
Методы **TopicInfo** позволяют настраивать свойства раздела (например, задавать значение срока жизни (Time to Live, TTL) по умолчанию, применяемое к сообщениям, отправленным в раздел). Следующий пример показывает, как создать раздел с именем `TestTopic` и размером не более 5 ГБ.
