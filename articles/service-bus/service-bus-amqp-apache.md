...
Первое действие выполняется на [портале Azure][]. На следующем снимке экрана показано, как создается виртуальная машина CentOS с именем scott-centos:
...
Чтобы войти на компьютер, нужно знать порт конечной точки для SSH. Это значение можно получить на портале, выбрав вновь созданную виртуальную машину и открыв вкладку **Конечные точки**. На следующем снимке экрана показано, что открытым портом SSH для этого компьютера является порт 57146.
