...
В этом учебнике рассматривается создание клиентского приложения и службы служебной шины с использованием возможностей обмена сообщениями с ретрансляцией. Соответствующий учебник по [обмену сообщениями через брокер](service-bus-messaging-overview.md#Brokered-messaging) в служебной шине см. в статье [Руководство по обмену сообщениями через брокер в служебной шине для .NET](service-bus-brokered-tutorial-dotnet.md).
...
Прежде всего необходимо создать пространство имен службы служебной шины и получить подписанный URL-адрес (SAS). Пространство имен определяет границы каждого приложения, предоставляемого через служебную шину. Пространство имен и ключ SAS — это учетные данные, на основе которых служебная шина осуществляет проверку подлинности и предоставляет доступ к приложению.

1. Чтобы создать пространство имен, перейдите на [классический портал Azure][]. В левой части щелкните **Служебная шина** и нажмите кнопку **Создать**. Введите имя пространства имен, примите значения по умолчанию для всех остальных значений и нажмите кнопку "ОК".

	>[AZURE.NOTE] Для клиентского приложения и службы можно использовать разные пространства имен.

	![][4]

1. В главном окне портала щелкните пространство имен службы, созданное на предыдущем шаге.

2. Перейдите на вкладку **Настройка**, чтобы просмотреть стандартные политики общего доступа и ключи для пространства имен.
...
3. Установка пакета NuGet для служебной шины. Этот пакет автоматически добавляет ссылки на библиотеки служебной шины, а также элемент WCF **System.ServiceModel**. [System.ServiceModel](https://msdn.microsoft.com/library/system.servicemodel.aspx) — это пространство имен, которое предоставляет программный доступ к основным функциям WCF. Служебная шина использует множество объектов и атрибутов WCF для определения контрактов службы.
	...
	В обозревателе решений щелкните решение правой кнопкой мыши и выберите пункт **Управление пакетами NuGet для решения**. Щелкните вкладку **Обзор** и выполните поиск по строке `Microsoft Azure Service Bus`. Убедитесь, что имя проекта указано в поле **Версии**. Щелкните **Установить** и примите условия использования.

	![][3]

3. В обозревателе решений дважды щелкните файл Program.cs, чтобы открыть его в редакторе.
...
1. В интерфейсе `IEchoContract` объявите метод для отдельной операции, которую контракт `IEchoContract` предоставляет в интерфейсе. Затем примените атрибут `OperationContractAttribute` к методу, который требуется предоставить как часть открытого контракта служебной шины.
...
1. В меню **Сборка** щелкните **Собрать решение** или нажмите клавиши **CTRL+SHIFT+B**, чтобы проверить правильность выполнения действий.
...
1. Примените атрибут [ServiceBehaviorAttribute](https://msdn.microsoft.com/library/system.servicemodel.servicebehaviorattribute.aspx) к интерфейсу `IEchoContract`. Атрибут определяет имя службы и пространство имен. После этого класс `EchoService` будет выглядеть так:
...
2. В элементе `<appSettings>` замените заполнители именем пространства имен службы и ключом SAS, скопированным на предыдущем шаге.
...
1. Создайте в `Main()` две переменные для хранения пространства имен и ключа SAS, которые считываются из окна консоли.
	...
	Далее ключ SAS потребуется для доступа к проекту служебной шины. Пространство имен передается в `CreateServiceUri` в качестве параметра для создания универсального кода ресурса (URI) службы.
...
1. Установите [пакет NuGet для служебной шины](https://www.nuget.org/packages/WindowsAzure.ServiceBus). В обозревателе решений щелкните правой кнопкой мыши проект **EchoClient** и выберите **Управление пакетами NuGet**. Щелкните вкладку **Обзор** и выполните поиск по строке `Microsoft Azure Service Bus`. Щелкните **Установить** и примите условия использования.
...
1. Скомпилируйте клиент, нажав клавиши **CTRL+SHIFT+B**.
...
1. Установите режим подключения **AutoDetect**. Добавьте следующий код в метод `Main()` клиентского приложения **EchoClient**.
...
1. Скомпилируйте решение, нажав клавиши **CTRL+SHIFT+B**. Будут скомпилированы клиентский проект и проект службы, созданный на предыдущих этапах.

2. Прежде чем запустить клиентское приложение, убедитесь, что приложение службы работает. В обозревателе решений в Visual Studio щелкните правой кнопкой мыши решение **EchoService** и выберите пункт **Свойства**.

3. В диалоговом окне свойств решения щелкните **Запускаемый проект** и нажмите кнопку **Несколько запускаемых проектов**. Убедитесь, что проект **EchoService** отображается первым в списке.
