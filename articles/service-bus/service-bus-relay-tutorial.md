...
1. В главном окне портала Azure щелкните имя пространства имен службы, созданного на предыдущем шаге.
...
1. Создайте хост службы с помощью универсального кода ресурса (URI), созданного на предыдущем шаге.
	...
	Хост службы представляет собой объект WCF, который создает экземпляры службы. На этом шаге нужно передать тип службы, которую требуется создать (тип `EchoService`), и адрес, по которому служба должна быть доступна.
...
1. Замените имя пространства имен `EchoClient` (имя по умолчанию) именем `Microsoft.ServiceBus.Samples`.
...
В этом учебнике мы рассмотрели создание приложения клиента и службы служебной шины с использованием обмена сообщениями с ретрансляцией через служебную шину. Аналогичный учебник по [обмену сообщениями через посредника](service-bus-messaging-overview.md/#Brokered-messaging) в служебной шине см. в статье [Руководство по обмену сообщениями через посредника в служебной шине для .NET](https://msdn.microsoft.com/library/hh367512.aspx).
