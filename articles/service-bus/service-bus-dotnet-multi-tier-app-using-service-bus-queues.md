...
4. 	Когда отобразится запрос на выполнение или сохранение файла установки, нажмите кнопку **Выполнить**.
...
> [AZURE.NOTE]Чтобы управлять пространствами имен и сущностями обмена сообщениями служебной шины, вы также можете использовать обозреватель серверов Visual Studio. Однако новые пространства имен можно создавать только с помощью портала Azure.
...
4.  На странице **Добавление нового пространства имен** введите имя пространства имен. Система немедленно проверяет, доступно ли оно.
...
5.  Убедившись, что имя пространства имен доступно, выберите страну или регион, где будет размещено ваше пространство имен (необходимо указать страну и регион развертывания своих вычислительных ресурсов). Кроме того, убедитесь, что выбраны настройки **Обмен сообщениями** в поле **Тип** и **Стандартный** в поле **Уровень обмена сообщениями**.
    ...
    > [AZURE.IMPORTANT]Выберите **тот же регион**, который вы собираетесь выбрать для развертывания приложения. Это обеспечит наилучшую производительность.
...
6.  В **обозревателе решений** щелкните **Ссылки** правой кнопкой мыши и выберите элемент **Управление пакетами NuGet** или **Добавить ссылку на пакет библиотеки**.
...
9.  В **обозревателе решений** щелкните **Модели** правой кнопкой мыши, а затем последовательно щелкните **Добавить** и **Класс**. В поле **Имя** введите имя **OnlineOrder.cs**. Нажмите кнопку **Добавить**.
...
2.  В **Обозревателе решений** дважды щелкните **Controllers\\HomeController.cs**. Добавьте в начало файла инструкцию **using**, чтобы включить пространства имен в созданную модель и служебную шину.
...
4.  В меню **Сборка** щелкните **Собрать решение**, чтобы проверить правильность кода.

5.  Теперь необходимо создать представление для ранее созданного метода **Submit()**. Щелкните метод **Submit()** правой кнопкой и выберите пункт **Добавить представление**.
...
Далее вам необходимо добавить код, реализующий отправку элементов в очередь. Сначала требуется создать класс, содержащий информацию о подключении к очереди служебной шины. Далее следует инициализировать подключение из Global.aspx.cs. Наконец, вам нужно изменить код отправки, ранее созданный в файле HomeController.cs, чтобы реализовать фактическую отправку элементов в очередь служебной шины.

1.  В **обозревателе решений** щелкните элемент **FrontendWebRole** правой кнопкой мыши (проект, не роль). Нажмите кнопку **Добавить** и выберите **Класс**.

2.  Присвойте классу имя QueueConnector.cs. Нажмите кнопку **Добавить**, чтобы создать класс.

3.  Теперь необходимо добавить код, который инкапсулирует сведения о подключении и инициализирует подключение к очереди Service Bus. Добавьте в файл QueueConnector.cs следующий код и введите значения для элементов **Namespace** (пространство имен службы) и **yourKey** (ключ SAS, полученный ранее на [портале управления Azure][Azure portal]).
    ...
    Далее в этом учебнике вы узнаете, как сохранить имя **пространства имен** и значение ключа SAS в файле конфигурации.
...
7.  Внесите следующие изменения в метод **Submit()**, чтобы получить количество сообщений очереди.
...
Метод **GetSettings** в классе **Microsoft.WindowsAzure.Configuration.CloudConfigurationManager** позволяет считывать параметры конфигурации из хранилища конфигураций для вашей платформы. Например, если код выполняется в веб-роли или рабочей роли, метод **GetSettings** обращается к файлу ServiceConfiguration.cscfg, а если код выполняется в стандартном консольном приложении — к файлу app.config.

Если сохранить строку подключения для пространства имен Service Bus в файле конфигурации, можно использовать метод **GetSettings** для чтения строки подключения, с помощью которой можно создать экземпляр объекта **NamespaceMananger**. Для программной настройки пространства имен служебной шины вы можете использовать экземпляр **NamespaceMananger**. С помощью той же строки подключения можно создавать экземпляры клиентских объектов (таких как **QueueClient**, **TopicClient** и **EventHubClient**), которые можно использовать для выполнения операций времени выполнения, например отправки и получения сообщений.
...
4.  В диалоговом окне **Добавление нового проекта роли** щелкните элемент **Рабочая роль с очередью служебной шины**.

	![][23]

5.  В поле **Имя** введите **OrderProcessingRole**. Нажмите кнопку **Добавить**.

6.  В **обозревателе сервера** щелкните правой кнопкой имя пространства имен службы, а затем щелкните **Свойства**. В области **Свойства** Visual Studio первая запись определяет строку подключения, которая содержит конечную точку пространства имен с необходимыми для авторизации учетными данными. См. пример на следующем снимке экрана. Дважды щелкните **ConnectionString**, а затем нажмите клавиши **Ctrl+C**, чтобы скопировать строку в буфер обмена.
...
8.  На вкладке **Настройки** диалогового окна **Свойства** щелкните поле **Значение** для элемента **Microsoft.ServiceBus.ConnectionString** и вставьте значение конечной точки, скопированное на этапе 6.
...
11. В файле **WorkerRole.cs** замените значение переменной **QueueName** с `"ProcessingQueue"` на `"OrdersQueue"`, как показано в следующем коде.
...
В некоторых случаях требуется реализовать внешний интерфейс многоуровневого приложения на веб-сайте Azure, а не в облачной службе Azure. Дополнительную информацию о различиях между веб-узлами и облачными службами см. в статье [Модели выполнения Azure][executionmodels].
...
Дополнительные сведения о развертывании приложения на веб-сайте Azure см. в статье [Развертывание веб-приложения ASP.NET на веб-сайте Azure](http://azure.microsoft.com/develop/net/tutorials/get-started/). Дополнительные сведения о развертывании серверной части в облачной службе Azure см. в статье [Многоуровневое приложение .NET, использующее таблицы, очереди и BLOB-объекты хранилища][mutitierstorage].
