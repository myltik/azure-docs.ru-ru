...
API Engagement предоставляется в классе `EngagementAgent`. Экземпляр этого класса можно получить, вызвав статический метод `[EngagementAgent shared]` (обратите внимание, что возвращенный объект `EngagementAgent` представлен в одном экземпляре).
 ...
 *Действие*, как правило, связано с одним экраном приложения, т. е. *действие* начинается при отображении экрана и завершается при его закрытии. Именно в таком случае интегрируется пакет SDK для Engagement с помощью класса `EngagementViewController`.
...
Длина дополнений ограничена **1024** знаками на один вызов (одно кодирование в JSON-файле агентом Engagement).
...
Так же, как и дополнения событий, класс `NSDictionary` используется для абстрагирования данных о приложении. Обратите внимание, что массивы или вложенные словари будут считаться неструктурированными строками (в которых используется сериализация JSON).
...
Данные приложения ограничены **1024** знаками на один вызов (одно кодирование в JSON-файле агентом Engagement).
