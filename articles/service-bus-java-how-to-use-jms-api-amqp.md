...
В этом практическом руководстве описывается использование функций обмена сообщениями, выполняемых посредством шины обслуживания (очередей и разделов публикации/подписки), из приложений Java, использующих популярный стандарт API JMS. В связанном руководстве объясняется, как выполнить те же действия, используя API служебной шины .NET. Эти два руководства можно использовать совместно для изучения обмена сообщениями между различными платформами с помощью AMQP 1.0.
...
**Примечание**. Необходимо создать очередь с отключенным секционированием, так как поддержка протокола AMQP еще не добавлена в секционированные очереди и разделы. Дополнительную информацию см. в разделе [Секционированные сущности обмена сообщениями](http://msdn.microsoft.com/library/azure/dn520246.aspx).

## Загрузка клиентской библиотеки AMQP 1.0 JMS

Информацию о том, где загрузить последнюю версию клиентской библиотеки Apache Qpid JMS AMQP 1.0, см. на странице [http://people.apache.org/~rgodfrey/qpid-java-amqp-1-0-client-jms.html](http://people.apache.org/~rgodfrey/qpid-java-amqp-1-0-client-jms.html).
...
<table>
  <tr>
    <td>[namespace]</td>
    <td>Пространство имен служебной шины, полученное с портала управления Azure.</td>
  </tr>
  <tr>
    <td>[username]</td>
    <td>Имя издателя служебной шины, полученное с портала управления Azure.</td>
  </tr>
  <tr>
    <td>[password]</td>
...
Для того чтобы определить **ConnectionFactory** под названием SBCF, строка конфигурации должна иметь следующий вид:
...
<table>
  ...
  <tr>
    <td>[physical_name]</td>
    <td>Имя сущности служебной шины, которой приложение отправляет сообщения или от которой оно получает сообщения.</td>
  </tr>
</table>

**Примечание**. При получении данных из подписки раздела служебной шины физическое имя, указанное в JNDI, должно быть именем раздела. Имя подписки предоставляется при создании устойчивой подписки в коде приложения JMS. В [руководстве разработчика служебной шины AMQP 1.0](http://msdn.microsoft.com/library/jj841071.aspx) содержатся дополнительные сведения о работе с подписками разделов служебной шины в JMS.

### Написание приложения JMS

Не существует специальных API-интерфейсов или параметров для использования JMS со служебной шиной. Однако существует несколько ограничений, которые будут рассмотрены ниже. Как и для любого приложения JMS, первое, что необходимо, - это конфигурация среды JNDI, позволяющая разрешать **ConnectionFactory** и назначения.
...
В этом руководстве мы рассмотрели, как отправлять и получать сообщения служебной шины с помощью JMS. Однако одно из основных преимуществ использования AMQP 1.0 состоит в том, что этот протокол позволяет приложениям, созданным из компонентов, написанных на разных языках, надежно и точно обмениваться сообщениями.

Используя приведенный выше пример приложения JMS и аналогичное приложение .NET, взятое из связанного руководства [Использование AMQP 1.0 с API .NET служебной шины](http://aka.ms/lym3vk), можно организовать обмен сообщениями между .NET и Java. 
...
При использовании JMS по протоколу AMQP 1.0 со служебной шиной действуют следующие ограничения:
...
* Временные назначения, т. е. **TemporaryQueue**, **TemporaryTopic**, в данный момент не поддерживаются, как и интерфейсы API **QueueRequestor** и **TopicRequestor**, которые используют их.
...
Протокол AMQP 1.0 служебной шины можно также использовать из других языков, в числе которых .NET, C, Python и PHP. Компоненты, созданные с помощью этих различных языков, могут надежно и точно обмениваться сообщениями, используя поддержку AMQP 1.0 в служебной шине. Дополнительную информацию см. в [Руководстве разработчика AMQP 1.0 для служебной шины](http://msdn.microsoft.com/library/jj841071.aspx).
