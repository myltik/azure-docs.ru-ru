...
Каждый уровень производительности имеет соответствующий предел скорости, измеряемый в [единицах запросов \(RU\)](http://go.microsoft.com/fwlink/?LinkId=735027). Это значение пропускной способности, которое будет зарезервировано для коллекции на основе ее уровня производительности и доступно для использования исключительно этой коллекцией.
...
> [AZURE.NOTE] Уровень производительности коллекции можно настраивать с помощью API-интерфейсов или на [портале Microsoft Azure](https://portal.azure.com/). Изменение уровня производительности должно занимать не более трех минут.
...
> [AZURE.NOTE] Если приложение превышает уровни производительности для одной или нескольких коллекций, будет выполнено регулирование запросов для каждой коллекции. Это значит, что некоторые запросы приложения могут выполняться успешно, а к другим может быть применено регулирование. Рекомендуется добавить
...
Портал Azure — один из доступных вариантов для управления уровнями производительности коллекции. Выполните следующие действия, чтобы от использования предварительно определенных уровней производительности перейти к использованию определенных пользователем уровней производительности на портале Azure, или посмотрите 75-секундное [видео Channel 9](https://channel9.msdn.com/Blogs/AzureDocumentDB/ChangeDocumentDBCollectionPerformance). Дополнительные сведения об изменении вариантов оплаты см. в записи блога [DocumentDB: Everything you need to know about using the new pricing options](https://azure.microsoft.com/blog/documentdb-use-the-new-pricing-options-on-your-existing-collections/).

1. Перейдите на [**портал Azure**](https://portal.azure.com) в браузере.
...
7. В колонке **Управление коллекцией** щелкните **Ценовая категория**.

    ![Снимок экрана колонок "Управление коллекцией" и "Выбор ценовой категории" для Azure DocumentDB, на котором показано, где можно изменить ценовую категорию для коллекции][1]

8. В колонке **Выбор ценовой категории** выберите **Стандартный**.

9. В колонке **Выбор ценовой категории** щелкните **Выбрать**.

10. В колонке **Управление коллекцией** значение **Ценовая категория** изменилось на **Стандартная** и появилось поле **Пропускная способность \(единицы запросов в секунду\)**.

    Измените значение в поле **Пропускная способность**, указав значение от 400 до 10 000 [единиц запроса](documentdb-request-units.md) в секунду. **Сводка по ценам** в нижней части страницы автоматически обновляется, показывая оценку ежемесячных расходов.

    ![Снимок экрана колонки "Управление коллекцией", на котором показано, где можно изменить значение пропускной способности для коллекции][2]

9. В колонке **Управление коллекцией** нажмите кнопку **ОК**, чтобы применить к коллекции определенные пользователем параметры производительности.
