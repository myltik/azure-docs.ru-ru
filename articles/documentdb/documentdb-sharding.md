---
title: "Как реализовать секционирование на стороне клиента с помощью пакетов SDK | Документация Майкрософт"
...
---
# <a name="how-to-partition-data-using-client-side-support-in-documentdb"></a>Как секционировать данные с помощью поддержки на стороне клиента в DocumentDB
...
## <a name="client-side-partitioning-with-the-documentdb-sdk"></a>Секционирование на стороне клиента с помощью пакета SDK для DocumentDB
...
Образцы содержат открытый исходный код, и мы рекомендуем вам отправлять запросы с материалами, которые могут принести пользу другим разработчикам DocumentDB. Инструкции о том, как принять участие, см. [здесь](https://github.com/Azure/azure-documentdb-net/blob/master/Contributing.md).  

> [!NOTE]
> Операции создания коллекции ограничены DocumentDB по скорости, поэтому для выполнения некоторых примеров методов, показанных здесь, может потребоваться несколько минут.
> 
> 
