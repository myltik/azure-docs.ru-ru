...
2. **Политика подключения: использование протокола TCP**
   ...
   * HTTPS
     ...
     При использовании протокола TCP в режиме шлюза TCP-порт 443 используется Cosmos DB, а TCP-порт 10250 — API MongoDB. Если вы используете протокол TCP в режиме прямого подключения, то в дополнение к портам шлюза необходимо открыть диапазон портов 10 000–20 000, так как Cosmos DB использует динамические TCP-порты. Используя протокол TCP при закрытых портах, вы получите ошибку 503 "Служба недоступна".
    ...
    ![Изображение: политика подключения Azure Cosmos DB](./media/documentdb-performance-tips/azure-documentdb-connection-policy.png)
...
4. **Оптимизация производительности за счет размещения клиентов в одном регионе Azure**
    ...
    ![Изображение: политика подключения Azure Cosmos DB](./media/documentdb-performance-tips/azure-documentdb-same-region.png)
...
Пример приложения, используемого в сценариях оценки производительности Cosmos DB на нескольких клиентских компьютерах, см. в статье [Проверка производительности и масштабирования с помощью Azure Cosmos DB](documentdb-performance-testing.md).

Дополнительные сведения о создании собственного масштабируемого приложения с высокой производительностью см. в статье [Секционирование, ключи секции и масштабирование в DocumentDB](documentdb-partition-data.md).
