---
title: "Советы по улучшению производительности DocumentDB | Документация Майкрософт"
...
2. **Политика подключения: использование протокола TCP**
   ...
   * HTTPS
     ...
     При использовании протокола TCP в режиме шлюза TCP-порт 443 используется DocumentDB, а TCP-порт 10250 — API MongoDB. При использовании протокола TCP в режиме прямого подключения в дополнение к портам шлюза необходимо будет открыть диапазон портов 10000–20000, так как DocumentDB использует динамические TCP-порты. Если эти порты не открыты и вы попытаетесь использовать TCP, возникнет ошибка 503 "Служба недоступна".
...
1. **Измерение и настройка расхода единиц запроса/повторного использования**
    ...
    Затраты на запрос, возвращенные в этом заголовке, — это часть подготовленной пропускной способности (например, 2000 единиц запроса в секунду). Например, если после выполнения приведенного выше запроса возвращается 1000 документов размером 1 КБ каждый, затраты на операцию составят 1000 единиц. Таким образом, перед регулированием последующих запросов сервер за одну секунду выполняет только два таких запроса. Чтобы узнать больше, ознакомьтесь с [единицами запроса](documentdb-request-units.md) и [калькулятором единиц запроса](https://www.documentdb.com/capacityplanner).
