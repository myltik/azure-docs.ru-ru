---
title: "Советы по повышению производительности. NoSQL в Azure DocumentDB | Документация Майкрософт"
...
2. **Политика подключения: использование протокола TCP**
   ...
   * HTTPS
     ...
     При использовании протокола TCP в режиме шлюза TCP-порт 443 используется DocumentDB, а TCP-порт 10250 — API MongoDB. Если вы используете протокол TCP в режиме прямого подключения, то в дополнение к портам шлюза необходимо открыть диапазон портов 10000–20000, так как DocumentDB использует динамические TCP-порты. Используя протокол TCP при закрытых портах, вы получите ошибку 503 "Служба недоступна".
...
4. **Оптимизация производительности за счет размещения клиентов в одном регионе Azure**
    ...
    ![Изображение: политика подключения DocumentDB](./media/documentdb-performance-tips/azure-documentdb-same-region.png)
...
1. **Установка последней версии пакета SDK**
    ...
    Пакеты SDK для DocumentDB постоянно улучшаются для оптимизации производительности. Сведения о последней версии пакета SDK и улучшениях см. на страницах о [пакете SDK для DocumentDB](documentdb-sdk-dotnet.md).
...
1. **Измерение и настройка расхода единиц запроса/повторного использования**
    ...
    Затраты на запрос, возвращенные в этом заголовке, — это часть подготовленной пропускной способности (например, 2000 единиц запроса в секунду). Например, если приведенный выше запрос вернет 1000 документов размером по 1 КБ каждый, затраты на операцию составят 1000 единиц. Таким образом, перед регулированием последующих запросов сервер за одну секунду выполняет только два таких запроса. Чтобы узнать больше, ознакомьтесь с [единицами запроса](documentdb-request-units.md) и [калькулятором единиц запроса](https://www.documentdb.com/capacityplanner).
<a id="429"></a>
