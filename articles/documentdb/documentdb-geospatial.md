---
title: Работа с геопространственными данными в Azure DocumentDB | Microsoft Docs
...
> [!NOTE]
> DocumentDB поддерживает автоматическое индексирование Points, Polygons (закрытая предварительная версия) и LineStrings (закрытая предварительная версия). Чтобы получить доступ к предварительной версии, свяжитесь с нами по адресу электронной почты askdocdb@microsoft.com или через службу поддержки Azure.
> 
> 
...
> [!NOTE]
> Если значение расположения GeoJSON в документе сформировано неверно или является недействительным, оно не будет индексироваться для пространственных запросов. Проверить значение расположения можно с помощью ST\_ISVALID и ST\_ISVALIDDETAILED.
> 
