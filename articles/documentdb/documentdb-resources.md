...
> [!NOTE]
> Cosmos DB предлагает весьма эффективный протокол TCP, который также является API-интерфейсом RESTful в своей коммуникационной модели, доступной с помощью [клиентского пакета SDK для .NET](https://msdn.microsoft.com/library/azure/dn781482.aspx).
> 
> 
...
Вы можете [создавать учетные записи базы данных Cosmos DB и управлять ими](documentdb-create-account.md) с помощью портала Azure на сайте [http://portal.azure.com/](https://portal.azure.com/). Создание и управление учетной записью базы данных требует наличия административного доступа и может быть выполнено только в рамках вашей подписки Azure. 
...
Документы в коллекции могут иметь произвольные схемы, и вы можете запрашивать их без предварительного предоставления любой схемы или вторичных индексов. Вы можете запрашивать коллекцию, используя [синтаксис SQL API DocumentDB](https://msdn.microsoft.com/library/azure/dn782250.aspx). Он предоставляет богатый выбор иерархических, реляционных и пространственных операторов, а также возможность расширения с использованием определяемых пользователем функций на основе JavaScript. Грамматика JSON позволяет моделировать документы JSON в форме деревьев с метками в качестве узлов. Этот подход используется для автоматического индексирования API DocumentDB, а также лежит в основе диалекта SQL API DocumentDB. Язык запросов API DocumentDB построен на следующих трех основных принципах.   
