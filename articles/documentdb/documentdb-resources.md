...
|Ресурс |Описание
|-----------|-----------
|Учетная запись базы данных |Учетная запись базы данных связана с набором баз и определенным местом в хранилище больших двоичных объектов для хранения вложений (это предварительная версия функции). Вы можете создать одну или несколько учетных записей базы данных с помощью подписки Azure. Каждой стандартной учетной записи базы данных выделяется минимум одна коллекция уровня производительности S1. Дополнительные сведения см. на нашей [странице цен](https://azure.microsoft.com/pricing/details/documentdb/).
...
---|---|---
_\_rid| Генерируемое системой| Генерируемый системой уникальный иерархический идентификатор ресурса. \_etag| Генерируемое системой |etag ресурса, необходимый для управления оптимистичным параллелизмом. \_ts| Генерируемое системой | Метка времени последнего обновления ресурса. \_self| Генерируемое системой| Уникальный адресуемый URI ресурса. id| Устанавливаемое пользователем| Определяемое пользователем уникальное имя ресурса. Если пользователь не укажет идентификатор, идентификатор будет создан системой.
...
|Значение свойства \_self |Описание|-------------------|----------- |/dbs |Канал баз данных в учетной записи|/dbs/{\_rid-db} |База данных, идентификатор которой имеет значение {\_rid-db} |/dbs/{\_rid-db}/colls/ |Поток коллекций в базе данных |/dbs/{\_rid-db}/colls/{\_rid-coll} |Коллекция, идентификатор которой имеет значение {\_rid-coll} |/dbs/{\_rid-db}/colls/{\_rid-coll}/docs |Канал документов в коллекции |/dbs/{\_rid-db}/colls/{\_rid-coll}/docs/{\_rid-doc} |Документ, идентификатор которого имеет значение {\_rid-doc} |/dbs/{\_rid-db}/users/ |Канал пользователей в базе данных |/dbs/{\_rid-db}/users/{\_rid-user} |Пользователь, идентификатор которого имеет значение {\_rid-user} |/dbs/{\_rid-db}/users/{\_rid-user}/permissions |Канал разрешений для пользователя |/dbs/{\_rid-db}/users/{\_rid-user}/permissions/{\_rid-permission} |Разрешение, идентификатор которого имеет значение {\_rid-permission}
...
Каждый ресурс также имеет иерархический идентификатор ресурсов, генерируемый системой (также известный как RID), который доступен через свойство \_rid. RID кодирует всю иерархию данного ресурса, и это очень удобное внутреннее представление используется для обеспечения ссылочной целостности распределенным образом. RID является уникальным в пределах учетной записи базы данных и используется службой DocumentDB для эффективной маршрутизации без перекрестного поиска по разделам.
...
[Создавать учетные записи базы данных DocumentDB и управлять ими](documentdb-create-account.md) можно с помощью классического портала Azure на сайте [http://portal.azure.com/](https://portal.azure.com/). Создание и управление учетной записью базы данных требует наличия административного доступа и может быть выполнено только для вашей подписки Azure.
...
---|---
Политика согласованности|Установите это свойство, чтобы настроить уровень согласованности по умолчанию для всех коллекций под вашей учетной записью базы данных. Вы можете переопределить уровень согласованности для каждого запроса, используя заголовок запроса x-ms-consistency-level. <p><p>Обратите внимание, что это свойство применяется только к <i>ресурсам, определяемым пользователем</i>. Все системные ресурсы настроены для поддержки чтения и запросов с сильной согласованностью.
...
Обратите внимание, что, помимо подготовки, настройки учетной записи базы данных на классическом портале Azure и управления ею, можно также программно создавать учетные записи базы данных DocumentDB и управлять ими через [интерфейс API REST Azure DocumentDB](https://msdn.microsoft.com/library/azure/dn781481.aspx) или [клиентские пакеты SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx).
...
Вы можете изменить политику индексирования, выполнив запрос PUT для коллекции. Это можно сделать с помощью [клиентского пакета SDK](https://msdn.microsoft.com/library/azure/dn781482.aspx), [классического портала Azure](https://portal.azure.com) или [интерфейсов API REST Azure DocumentDB](https://msdn.microsoft.com/library/azure/dn781481.aspx).

### Запросы к коллекции
Документы в коллекции могут иметь произвольные схемы, и вы можете запрашивать их без предварительного предоставления любой схемы или вторичных индексов. Вы можете создавать запросы к коллекции, используя [синтаксис DocumentDB SQL](https://msdn.microsoft.com/library/azure/dn782250.aspx), который предоставляет богатый выбор иерархических и реляционных операторов JavaScript в формате определяемых пользователем функций. Грамматика JSON позволяет моделировать документы JSON в форме деревьев с метками в качестве узлов. Этот подход используется для автоматического индексирования DocumentDB, а также лежит в основе диалекта SQL DocumentDB. Язык запросов DocumentDB построен на следующих трех основных принципах.
