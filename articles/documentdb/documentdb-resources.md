...
<td valign="top" ><p><b>Устанавливается пользователем или генерируется системой</b></p></td>
...
###Адресация ресурса
Все ресурсы можно адресовать по URI. Значение свойства **__self** представляет относительный URI ресурса. Формат URI состоит из сегментов пути /<feed>/{_rid}: 
...
Каждый ресурс также имеет иерархический идентификатор ресурсов, который генерируется системой (он также называется RID) и доступен через свойство _rid. RID кодирует всю иерархию данного ресурса, и это очень удобное внутреннее представление используется для обеспечения ссылочной целостности распределенным образом. RID — это уникальный в пределах учетной записи базы данных идентификатор, который используется внутри DocumentDB для эффективной маршрутизации без необходимости в перекрестных выборках разделов. 
...
<tr>
<td valign="top" ><p>MediaStorageUsageInMB (ЧТЕНИЕ)</p></td>
...
Обратите внимание, что примеры используют мнемонические идентификаторы для передачи иерархии ресурсов. Ресурсы доступны через REST API по уникальным идентификаторам ресурсов.
