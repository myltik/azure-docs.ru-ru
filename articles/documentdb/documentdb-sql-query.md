<properties 
	...
	keywords="База данных запросов, запросы sql, sql-запрос, язык sql, documentdb, azure, Microsoft azure"
	services="documentdb" 
	documentationCenter="" 
	authors="arramac" 
	manager="jhubbard" 
...
В предыдущем примере создается определяемая пользователем функция с именем `REGEX_MATCH`. Она принимает два строковых значения JSON, `input` и `pattern`, а затем проверяет, совпадает ли значение первого поля с шаблоном, указанным во втором поле, с помощью функции string.match() JavaScript.
