<properties 
	pageTitle="SQL-запрос в DocumentDB, базе данных NoSQL | Microsoft Azure" 
	description="Узнайте, как использовать инструкции SQL-запросов для выполнения запросов к DocumentDB, базе данных NoSQL. Как и язык запросов JSON, SQL-запросы могут использоваться для анализа больших объемов данных." 
	keywords="Запрос sql, запросы sql, синтаксис sql, язык запросов json, понятия базы данных и запросы sql"
	services="documentdb" 
	documentationCenter="" 
	authors="arramac" 
	manager="jhubbard" 
...
-	Вместо того чтобы изобретать новый язык запросов, мы хотели поддержать SQL. SQL — это один из самых популярных и хорошо известных языков формирования запросов. DocumentDB SQL предоставляет формальную модель программирования для выполнения многофункциональных запросов над документами JSON.
...
Давайте посмотрим, какую роль играет `$1`. Предложение `SELECT` необходимо для создания объекта JSON, и, так как ни один из ключей не предоставлен, мы используем имена неявных переменных аргументов, начиная с `$1`. Например, этот запрос возвращает две неявные переменные аргументов, помеченные как `$1` и `$2`.
...
### Псевдонимы
Теперь давайте расширим пример выше с явным использованием псевдонимов. AS – ключевое слово, используемое для создания псевдонимов. Обратите внимание, что он не является обязательным при отображении второго значения, как показано `NameInfo`.
...
### Итерация
Чтобы обеспечить поддержку перебора массивов JSON, мы добавили новую конструкцию с помощью ключевого слова **IN** в SQL DocumentDB. Исходное выражение FROM поддерживает итерацию. Начнем с такого примера:
...
При включении пространственного индексирования в политику индексирования «запросы для определения расстояния» будут эффективно обрабатываться с помощью индекса. Дополнительные сведения о пространственном индексировании см. в следующем разделе. При отсутствии пространственного индекса для указанных путей вы все равно сможете выполнять пространственные запросы, указывая заголовок запроса `x-ms-documentdb-query-enable-scan` со значением "true". В .NET это можно сделать, передав необязательный аргумент **FeedOptions** в запросы со свойством [EnableScanInQuery](https://msdn.microsoft.com/library/microsoft.azure.documents.client.feedoptions.enablescaninquery.aspx#P:Microsoft.Azure.Documents.Client.FeedOptions.EnableScanInQuery), имеющим значение "true".
...
Аргументы многоугольника в ST\_WITHIN могут содержать только одно кольцо, т. е. в многоугольниках не должно быть «дыр». Ознакомьтесь с [ограничениями DocumentDB](documentdb-limits.md) на максимально допустимое количество точек многоугольника в запросе ST\_WITHIN.
...
На рисунке ниже показана архитектура поддержки запросов LINQ при использовании DocumentDB. Используя клиент DocumentDB, разработчики могут создать объект **IQueryable**, который будет направлять запросы к поставщику запросов DocumentDB, который затем преобразует запросы LINQ в запросы DocumentDB. Затем запрос передается на сервер DocumentDB для получения набора результатов в формате JSON. Возвращенные результаты десериализуются в поток объектов .NET на стороне клиента.
