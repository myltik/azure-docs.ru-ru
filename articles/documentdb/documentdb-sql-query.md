---
title: "SQL-запросы и синтаксис SQL в Azure Cosmos DB | Документация Майкрософт"
...
---
# <a name="sql-query-and-sql-syntax-in-azure-cosmos-db"></a>SQL-запросы и синтаксис SQL в Azure Cosmos DB
...
Прежде чем мы перейдем к знакомству с синтаксисом SQL в API DocumentDB, стоит рассмотреть принципы индексирования в API Cosmos DB. 
...
Статистические вычисления также могут быть выполнены над результатом итерации массива. Дополнительные сведения об итерации массива в запросах см. в [здесь](#Iteration).
...
В реляционных базах данных возможность соединения таблиц является чрезвычайно важной. Это всего логическое следствие проектирования нормализованных схем. В противоположность этому, API DocumentDB работает с денормализованной моделью данных документов без схемы. Это является логическим аналогом «автообъединения».
...
В SQL API DocumentDB, в отличие от традиционного SQL, типы значений зачастую неизвестны до тех пор, пока значения не будут действительно извлечены из базы данных. Для того чтобы эффективно выполнять запросы, большинство операторов имеют строгие требования к типам. 

SQL в API DocumentDB не выполняет неявные преобразования, в отличие от JavaScript. Например, запрос `SELECT * FROM Person p WHERE p.Age = 21` соответствует документам, содержащим свойство "Возраст" со значением 21. Любой документ, свойство "возраст" в котором равно строке "21" или другим вариациям, таким как "021", "21.0", "0021", "00021" и т. д. не будут возвращены. Это вступает в противоречие с JavaScript, в котором значения строк приведены к цифрам (в зависимости от оператора, например: ==). Этот выбор имеет решающее значение для эффективного согласования индексов в SQL API DocumentDB. 
...
> [!NOTE]
> Для выполнения статистических запросов необходимы пакеты SDK версии 1.12.0 или выше. Для статистических функций поддержка LINQ не осуществляется, однако она будет доступна в пакете SDK версии 1.13.0 для .NET.
>
