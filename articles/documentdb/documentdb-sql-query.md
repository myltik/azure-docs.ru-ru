...
-	<strong>Воспользоваться SQL</strong> — вместо того чтобы изобретать новый язык запросов, мы хотели использовать существующий язык SQL. Ведь SQL – один из самых популярных и хорошо известных языков формирования запросов. DocumentDB SQL предоставляет формальную модель программирования для выполнения многофункциональных запросов над документами JSON.
-	<strong>Extend SQL</strong> — мы хотели использовать модель программирования JavaScript в качестве основы для нашего языка запросов, так как ядро базы способно выполнять JavaScript непосредственно для документов JSON. DocumentDB SQL основывается на системе типов, вычисления выражений и вызовов функций JavaScript. Это, в свою очередь, обеспечивает естественную модель программирования для реляционных проекций, иерархической навигации через документы JSON, внутренних соединений, вызовов определяемых пользователем функций (UDF), написанных полностью на JavaScript, среди других особенностей. 
...
-	DocumentDB поддерживает только документы, строго соответствующие JSON. Это означает, что система типов и выражения могут обрабатывать только типы JSON. Ознакомьтесь со статьей [Спецификации JSON](http://www.json.org/) для получения дополнительных сведений.  
...
Прежде чем мы перейдем к знакомству с грамматикой DocumentDB SQL, стоит рассмотреть принципы индексирования в DocumentDB.
...
Ознакомьтесь с [Примерами DocumentDB](https://github.com/Azure/azure-documentdb-net) на сайте MSDN, чтобы увидеть, как настроить политику индексации для коллекции. Давайте теперь перейдем к деталям грамматики SQL DocumentDB.
...
-	Обратите внимание, что после создания псевдонима оригинальный источник связывать нельзя. Например, выражение `SELECT Families.id FROM Families f` является синтаксически неправильным, так как идентификатор Families уже не может быть разрешен.
...
В настоящее время поддерживаются и могут использоваться в запросах следующие бинарные операторы, как показано в примерах: <table> <tr> <td>Арифметические</td> <td>+,-,*,/,%</td> </tr> <tr> <td>Битовые</td> <td>|, &, ^, <<, >>, >>> (сдвиг вправо с заполнением нулями) </td> </tr> <tr> <td>Логические</td> <td>AND, OR</td> </tr> <tr> <td>Сравнения</td> <td>=, !=, >, >=, <, <=, <></td> </tr> <tr> <td>Строковые</td> <td>|| (сцепление)</td> </tr> </table>
...
Ниже приведена таблица, в которой собраны результаты сравнения равенства в SQL для DocumentDB между любыми двумя типами JSON. <table style = "width:300px"> <tbody> <tr> <td valign="top"> <strong>Оператор</strong> </td> <td valign="top"> <strong>Неопределенный</strong> </td> <td valign="top"> <strong>Нулевой</strong> </td> <td valign="top"> <strong>Логический</strong> </td> <td valign="top"> <strong>Числовой</strong> </td> <td valign="top"> <strong>Строковый</strong> </td> <td valign="top"> <strong>Объектный</strong> </td> <td valign="top"> <strong>Массив</strong> </td> </tr> <tr> <td valign="top"> <strong>Неопределенный<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Нулевой<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>Не определено</strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Логический<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>ОК</strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Числовой<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>ОК</strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Строковый<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>ОК</strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Объектный<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>ОК</strong> </td> <td valign="top"> Не определено </td> </tr> <tr> <td valign="top"> <strong>Массив<strong> </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> Не определено </td> <td valign="top"> <strong>ОК</strong> </td> </tr> </tbody> </table>
...
Слово IN эквивалентно цепочке из нескольких предложений OR, однако поскольку их можно обслужить с помощью одного индекса, DocumentDB поддерживает более высокое [ограничение](documentdb-limits.md) на количество аргументов, указанных в предложении IN.
...
Давайте посмотрим, какую роль здесь играет `$1`. Предложению `SELECT` необходимо создать объект JSON, но поскольку ни один из ключей не предоставлен, мы используем имена неявных переменных аргументов, начиная с `$1`. Например, этот запрос возвращает две неявные переменные аргументов, помеченных как `$1` и `$2`.
...
Синтаксисом, который поддерживает язык, является ОБЪЕДИНЕНИЕ <from_source1> JOIN <from_source2> JOIN ... ОБЪЕДИНИТЕ <from_sourceN>. В целом будет возвращаться набор **N**-кортежей (кортежей с **N** значениями). Каждый кортеж будет иметь значения, полученные путем итерации всех псевдонимов коллекции среди их наборов. Другими словами, это полное векторное произведение множеств, участвующих в соединении.
...
-	Наконец, получите проекцию только свойства имени корневого объекта **f**. 

Первый документ (`AndersenFamily`) содержит только один дочерний элемент, поэтому результирующий набор содержит только один соответствующий этому документу объект. Второй документ (`WakefieldFamily`) имеет два дочерних объекта. Так, векторное произведение создает отдельный объект для каждого дочернего, в результате получаются два объекта, по одному для каждого дочернего, соответствующего этому документу. Обратите внимание, что корневые поля в обоих этих документах будут такими же, как и следовало ожидать при векторном произведении.
...
В предыдущем примере создается определяемая пользователем функция с именем `REGEX_MATCH`. Она принимает два строковых значения JSON `input` и `pattern`, а затем проверяет совпадает ли значение первого поля с шаблоном, указанным во втором поле, с помощью функции string.match(), написанной на JavaScript.
...
>[AZURE.NOTE]До 17 марта 2015 года компонент DocumentDB поддерживал вызовы определяемой пользователем функции без префикса "udf.", например SELECT REGEX_MATCH(). Теперь этот метод выполнения вызовов устарел.
...
SQL в DocumentDB не выполняет неявные преобразования, в отличие от JavaScript. Например, запрос `SELECT * FROM Person p WHERE p.Age = 21` соответствует документам, содержащим свойство Age (возраст) со значением 21. Любой документ, свойство "возраст" в котором равно строке "21" или другим вариациям, таким как "021", "21.0", "0021", "00021" и т. д. не будут возвращены. Это вступает в противоречие с JavaScript, в котором значения строк приведены к цифрам (в зависимости от оператора, например: ==). Этот выбор имеет решающее значение для эффективного согласования индексов в SQL DocumentDB.
...
<tr>
  ...
  <td>Возвращает число элементов массива, указанного в выражении.</td>
</tr>
<tr>
  ...
  <td>Возвращает массив, который является результатом объединения значений двух или более массивов.</td>
</tr>
<tr>
  <td><a href="https://msdn.microsoft.com/library/azure/dn782250.aspx#bk_array_contains">ARRAY_CONTAINS (arr_expr, expr)</a></td>
  <td>Возвращает логическое значение, указывающее, содержит ли массив указанное значение.</td>
</tr>
<tr>
  ...
  <td>Возвращает часть выражения массива.</td>
</tr>
</table>
...
Он связывает встроенные функции и грамматику SQL для DocumentDB. Теперь давайте рассмотрим схему работы LINQ и взаимодействие с грамматикой, которую мы уже видели.
...
На рисунке ниже показана архитектура поддержки запросов LINQ при использовании DocumentDB. Используя клиент DocumentDB, разработчики могут создать объект **IQueryable**, который будет направлять запросы к поставщику запросов DocumentDB, транслирующему запросы LINQ в запросы DocumentDB. Затем запрос передается на сервер DocumentDB для получения набора результатов в формате JSON. Возвращенные результаты десериализуются в поток объектов .NET на стороне клиента.
...
-	Постоянные значения. Включают в себя постоянные значения примитивных типов данных, которые вычисляются на момент оценки запроса.
