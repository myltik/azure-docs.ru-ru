<properties 
	pageTitle="Формирование запросов с помощью DocumentDB SQL | Microsoft Azure" 
...
Синтаксисом, который поддерживает язык, является ОБЪЕДИНЕНИЕ <from_source1> JOIN <from_source2> JOIN ... JOIN <from_sourceN>. В целом будет возвращаться набор **N**-кортежей (кортежей, у которых число значений равно **N**). Каждый кортеж будет иметь значения, полученные путем итерации всех псевдонимов коллекции среди их наборов. Другими словами, это полное векторное произведение множеств, участвующих в соединении.
...
####Оператор Where
Синтаксис: `input.Where(x => f(x))`, где `f` — скалярное выражение, возвращающее логический тип.
...
9.	Технология оценки запроса для больших баз данных — [http://dl.acm.org/citation.cfm?id=152611](http://dl.acm.org/citation.cfm?id=152611)
