...
> [AZURE.TIP] Чтобы достичь полной пропускной способности коллекции, необходимо выбрать ключ секции, который позволяет равномерно распределять запросы по нескольким разным значениям ключа секции.
...
Кроме того, поскольку ключи секций можно задать только во время создания коллекции, необходимо экспортировать и повторно импортировать данные с помощью [средства переноса данных DocumentDB](http://www.microsoft.com/downloads/details.aspx?FamilyID=cda7703a-2774-4c07-adcc-ad02ddc1a44d) для создания секционированной коллекции.
...
-   Если вы хотите настроить секционирование, выполняемое приложением, то можете подключить собственную реализацию секционирования на стороне клиента. См. статью [Поддержка секционирования на стороне клиента](documentdb-sharding.md).
