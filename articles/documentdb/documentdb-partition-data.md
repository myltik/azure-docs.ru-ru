...
> [!NOTE]
> Мы рекомендуем иметь ключ раздела с большим количеством уникальных значений (как минимум сотней и тысячей). Многие клиенты используют DocumentDB в качестве хранилища значений ключей, в котором уникальный идентификатор представляет ключ раздела среди огромного множества других ключей.
>
...
DocumentDB поддерживает [статистические функции] ([Статистические функции](documentdb-sql-query.md#Aggregates) `COUNT`, `MIN`, `MAX`, `SUM` и `AVG`) для секционированных коллекций с помощью SQL с пакетом SDK 1.12.0 и выше. Запросы должны содержать один статистический оператор и одно значение в проекции.
