...
7. Выберите одну из указанных ниже **Политик индексации**.
	...
	- **Хэш**. Эта политика лучше всего подходит при использовании запросов предложения ORDER BY, диапазонов и сравнения на равенство и для чисел и для строк. Эта политика обладает более высокими затратами на хранение индекса по сравнению с политикой **По умолчанию** и включает геопространственную индексацию.
...
Коллекции не обязательно создавать на портале. Помимо этого, вы можете создавать их с помощью [пакетов SDK для DocumentDB](documentdb-sdk-dotnet.md). Примеры кода C#, в которых показано, как работать с коллекциями, используя пакет SDK для DocumentDB .NET, см. в [примерах коллекции C#](documentdb-dotnet-samples.md#collection-examples). Примеры кода Node.js, в которых показано, как работать с коллекциями, используя пакет SDK для DocumentDB Node.js, см. в [примерах коллекции Node.js](documentdb-nodejs-samples.md#collection-examples).
...
- Либо можно добавить документы с помощью одного из [пакетов SDK для DocumentDB](documentdb-sdk-dotnet.md). DocumentDB содержит пакеты SDK для .NET, Java, Python, Node.js и JavaScript API. Примеры кода C#, в которых показано, как работать с документами, используя пакет SDK для DocumentDB .NET, см. в [примерах документов C#](documentdb-dotnet-samples.md#document-examples). Примеры кода Node.js, в которых показано, как работать с документами, используя пакет SDK для DocumentDB Node.js, см. в [примерах документов Node.js](documentdb-nodejs-samples.md#document-examples).
