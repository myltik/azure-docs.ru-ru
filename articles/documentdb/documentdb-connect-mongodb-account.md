...
Узнайте, как подключиться к учетной записи DocumentDB с поддержкой протокола MongoDB, используя стандартный формат URI для строки подключения MongoDB.

## Получение строки подключения для учетной записи

1. В новом окне войдите на [портал Azure](https://portal.azure.com).
2. В колонке **Все параметры** щелкните **Строка подключения**. Чтобы перейти к разделу **Все параметры**, на панели быстрых переходов нажмите кнопку **Обзор**, затем нажмите учетные записи **DocumentDB** и выберите нужную учетную запись DocumentDB с поддержкой протокола MongoDB.
