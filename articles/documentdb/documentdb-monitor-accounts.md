...
Вы можете осуществлять мониторинг своей учетной записи DocumentDB на [портале предварительной версии Azure](https://portal.azure.com/). Для каждой учетной записи DocumentDB доступны метрики производительности (такие как запросы и ошибки сервера) и использования (такие как потребление памяти).
...
2.	В области **Мониторинг** по умолчанию отображается следующее:
...
3.	при щелчке раздела **Всего запросов** или **Среднее число запросов в секунду** открывается подробная выноска **Метрика**.
...
1.	Чтобы настроить метрики, отображаемые в определенном разделе, щелкните правой кнопкой мыши диаграмму с метриками и выберите пункт **Изменить диаграмму**.  
	![](./media/documentdb-monitor-accounts/madocdb3.png)

2.	На выноске **Изменение диаграммы** присутствуют параметры для изменения метрик, отображаемых в данном разделе, и их временного диапазона.  
...
2.	В области **Использование**  по умолчанию отображается следующее:
	...
	*	Выделение единиц измерения емкости.
...
2.	В области **Операции** щелкните раздел **Правила оповещения**.  
	![](./media/documentdb-monitor-accounts/madocdb10.png)

3.	На выноске "Правила оповещения" щелкните элемент **Добавить оповещение**.  
	![](./media/documentdb-monitor-accounts/madocdb11.png)

4.	На выноске **Добавление правила оповещения** укажите:
...
Дополнительные сведения о емкости и производительности DocumentDB см. в разделе [Управление емкостью и производительностью DocumentDB](documentdb-manage.md). 

<!--Anchors-->
[Просмотр метрик производительности для учетной записи DocumentDB]: #How-to-view-performance-metrics-for-a-DocumentDB-account
[Настройка представлений метрик производительности для учетной записи DocumentDB]: #Customize-performance-metric-views-for-a-DocumentDB-account
[Создание параллельных диаграмм метрик производительности]: #How-to-create-side-by-side-performance-metric-charts
[Просмотр метрик использования для учетной записи DocumentDB]: #How-to-view-usage-metrics-for-a-DocumentDB-account
[Настройка оповещений метрик производительности для учетной записи DocumentDB]: #How-to-setup-performance-metric-alerts-for-a-DocumentDB-account
