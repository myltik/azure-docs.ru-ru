...
В следующем разделе описывается, как получить доступ к веб-каналу изменений с помощью REST API и пакетов SDK DocumentDB. Для приложений .NET рекомендуется использовать [библиотеку обработчика веб-канала изменений](), чтобы обрабатывать события из веб-канала изменений.
...
Чтобы начать обработку событий, следует создать экземпляр ChangeFeedProcessorHost, указав соответствующие параметры для коллекции DocumentDB. Затем вызовите метод `RegisterObserverAsync` для регистрации вашей реализации `IChangeFeedObserver` в среде выполнения. На этом этапе узел попытается получить аренду для каждого диапазона ключей секций в коллекции DocumentDB, используя "жадный" алгоритм. Эти аренды будут продолжаться в течение заданного промежутка времени, после чего их необходимо продлить. По мере перехода новых узлов (в нашем случае это рабочие экземпляры) в оперативный режим они размещают резервирования аренды и со временем нагрузка смещается между узлами при каждой попытке получения дополнительной аренды.

![Использование узла обработчика веб-канала изменений DocumentDB](./media/documentdb-change-feed/changefeedprocessor.png)

Со временем устанавливается равновесие. Такая динамическая возможность позволяет применить к потребителям автоматическое масштабирование как вверх, так и вниз на основе использования ресурсов ЦП. Если события становятся доступными в DocumentDB быстрее, чем могут обработать потребители, то увеличение использования ресурсов ЦП потребителями можно использовать для автоматического масштабирования на основе числа экземпляров исполнителей.

Класс `ChangeFeedProcessorHost` также реализует механизм создания контрольных точек с помощью отдельной коллекции аренд DocumentDB. Этот механизм хранит смещение для каждой секции, чтобы каждый потребитель мог определить последнюю контрольную точку от предыдущего потребителя. По мере перехода секций между узлами перенос нагрузки осуществляется за счет механизма синхронизации.
...
Следующий фрагмент кода показывает, как зарегистрировать новый узел для ожидания передачи изменений из коллекции DocumentDB. Здесь мы настраиваем отдельную коллекцию, чтобы управлять арендами для секций между несколькими потребителями:
