...
**Ограниченная с запаздыванием**: ограниченная согласованность с запаздыванием гарантирует общий порядок распространения операций записи, при чтении возможно возникновение задержек от записи не более чем на К префиксов. Чтение всегда признается большинством необходимого набора реплик. Ответ на запрос чтения определяет ее относительную «свежесть» (в пересчете на К). Благодаря ограниченной согласованности с запаздыванием вы можете задать для операций чтения настраиваемое пороговое значение запаздывания (в качестве префиксов или времени), чтобы уравновесить задержку и согласованность в устойчивом состоянии.

Ограниченная согласованность с запаздыванием обеспечивает более предсказуемое поведение согласованности чтения, предлагая самые низкие задержки для записи. Так как операции чтения подтверждаются кворумом большинства, время задержки чтения не самое низкое их тех, что предлагаются системой. Ограниченная согласованность с запаздыванием подходит для сценариев, где требуется строгая согласованность, однако ее применение нецелесообразно. Если вы настроите, что «интервал запаздывания» для ограниченной согласованности с запаздыванием может быть сколь угодно большим, это позволит сохранить общий глобальный порядок операций записи. Этот вариант обеспечивает большую уверенность по сравнению с окончательной согласованностью или согласованностью сеанса.
...
3. Если колонка **Параметры** еще не открыта, в колонке учетной записи щелкните значок **Параметры** на панели команд вверху.
...
5. В колонке **Согласованность по умолчанию** выберите новый уровень согласованности и щелкните **Сохранить**.

	![Снимок экрана, на котором показан уровень согласованности и кнопка "Сохранить"](./media/documentdb-consistency-levels/database-consistency-level-2.png)
