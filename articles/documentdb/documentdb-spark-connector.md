---
title: "Подключение Apache Spark к Azure DocumentDB | Документация Майкрософт"
description: "В этом руководстве приведены сведения о соединителе Azure DocumentDB Spark, который позволяет подключать Apache Spark к Azure DocumentDB для выполнения распределенного агрегирования и обработки данных в мультитенантной глобально распределенной системе баз данных Майкрософт, разработанной для использования в облачной среде."
...
# <a name="accelerate-real-time-big-data-analytics-with-the-spark-to-documentdb-connector"></a>Ускорение аналитики больших данных в реальном времени с помощью соединителя Spark для DocumentDB

Соединитель Spark для DocumentDB позволяет использовать Azure DocumentDB в качестве источника входных данных или приемника выходных данных для заданий Apache Spark. Подключение [Spark](http://spark.apache.org/) к [DocumentDB](https://azure.microsoft.com/services/documentdb/) ускоряет возможность решения проблем анализа данных. Это также позволяет быстро сохранять и запрашивать данные с помощью DocumentDB. Соединитель Spark для DocumentDB эффективно использует собственные, управляемые индексы DocumentDB, а также позволяет использовать обновляемые столбцы при выполнении анализа, применять фильтры предиката к быстро меняющимся глобально распределенным данным, начиная от Интернета вещей, обработки и анализа данных и до сценариев аналитики. 

## <a name="download"></a>Загрузить

Сначала скачайте соединитель Spark для DocumentDB (предварительная версия) из репозитория [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark/) на сайте GitHub.

## <a name="connector-components"></a>Компоненты соединителя

Соединитель использует следующие компоненты:

* [DocumentDB](http://documentdb.com) — мультитенантная [глобально распределенная система баз данных](documentdb-distribute-data-globally.md) от корпорации Майкрософт, разработанная для использования в облачной среде. Эта система позволяет клиентам подготавливать и эластично масштабировать пропускную способность и ресурсы хранилища в любом количестве географических регионов. DocumentDB гарантирует низкую задержку на уровне 99-го процентиля, высокий уровень доступности в 99,99 % и [несколько четко определенных моделей согласованности](documentdb-consistency-levels.md) для разработчиков.
...
Существует два способа подключения Apache Spark и Azure DocumentDB:
...
Текущий [пакет SDK для pyDocumentDB](https://github.com/Azure/azure-documentdb-python) позволяет подключить Spark к DocumentDB, как показано на следующей схеме:

![Передача данных между Spark и DocumentDB с помощью pyDocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-pydocumentdb.png)
...
1. Между главным узлом Spark и узлом шлюза DocumentDB устанавливается подключение через pyDocumentDB.  Обратите внимание, что пользователь указывает только подключения между Spark и DocumentDB. Тот факт, что подключение устанавливается между соответствующим главным узлом и узлом шлюза, очевиден.
2. К DocumentDB (через узел шлюза) выполняется запрос, в впоследствии чего отправляется запрос к разделам коллекции в узлах данных. Ответ на эти запросы возвращается в узел шлюза, а результирующий набор — в главный узел Spark.
3. Все последующие запросы (например, к таблице данных Spark) отправляются в рабочие узлы Spark для обработки.

Важным является то, что взаимодействие между Spark и DocumentDB происходит в пределах главного узла Spark и узла шлюза DocumentDB.  Скорость запросов зависит от транспортного уровня между двумя этими узлами.
...
### <a name="connecting-spark-to-documentdb-via-pydocumentdb"></a>Подключение Spark к DocumentDB с помощью pyDocumentDB 
Благодаря простоте обмена данными выполнение запроса из Spark в DocumentDB с помощью pyDocumentDB относительно простое.
...
Подключение к DocumentDB с помощью pyDocumentDB Spark в основном предназначено для следующих сценариев:
...
## <a name="spark-to-documentdb-connector"></a>Соединитель Spark для DocumentDB

Соединитель Spark для DocumentDB использует [пакет SDK для Java в Azure DocumentDB](https://github.com/Azure/azure-documentdb-java) и перемещает данные между рабочими узлами Spark и DocumentDB, как показано на следующей схеме:

![Поток данных в соединителе Spark для DocumentDB](./media/documentdb-spark-connector/azure-documentdb-spark-connector.png)

Путь потока данных выглядит следующим образом:

1. Чтобы получить схему секционирования, между главным узлом Spark и узлом шлюза DocumentDB устанавливается соединение.  Обратите внимание, что пользователь указывает только подключение между Spark и DocumentDB. Тот факт, что подключение устанавливается между соответствующим главным узлом и узлом шлюза, очевиден.
2. Эта информация возвращается в главный узел Spark.  На этом этапе вы можете проанализировать запрос, чтобы определить разделы (и их расположение) в DocumentDB, к которым требуется получить доступ.
3. Эта информация передается в рабочие узлы Spark.
4. Таким образом, рабочие узлы Spark могут подключаться непосредственно к разделам DocumentDB для извлечения необходимых данных и переносить эти данные обратно в разделы рабочих узлов Spark.

Важным является то, что взаимодействие между Spark и DocumentDB значительно быстрее, так как данные перемещаются между рабочими узлами Spark и узлами данных DocumentDB (разделами).

### <a name="building-the-spark-to-documentdb-connector"></a>Создание соединителя Spark для DocumentDB
...
### <a name="connecting-spark-to-documentdb-using-the-connector"></a>Подключение Spark и DocumentDB с помощью соединителя
Хотя обмен данными происходит сложнее, выполнение запроса из Spark в DocumentDB с помощью соединителя выполняется значительно быстрее.
...
- Импортируйте необходимые библиотеки и настройте главный ключ и узел для создания клиента DocumentDB.
...
### <a name="why-use-the-spark-to-documentdb-connector-implementation"></a>Зачем использовать соединитель Spark для DocumentDB?

Подключение к DocumentDB с помощью соединителя Spark в основном предназначено для следующих сценариев:
...
* Необходимо передать большой объем данных между Apache Spark и DocumentDB.
...
В этом разделе приведены некоторые примеры того, как можно выполнить распределенное агрегирование и аналитику с помощью Apache Spark и Azure DocumentDB.  Обратите внимание, что в Azure DocumentDB уже реализована поддержка агрегирования, как описано в статье блога [Planet scale aggregates with Azure DocumentDB](https://azure.microsoft.com/blog/planet-scale-aggregates-with-azure-documentdb/) (Глобальное агрегирование с помощью Azure DocumentDB). Таким образом с помощью Apache Spark этот процесс можно выполнить на новом уровне.

Обратите внимание, что эти агрегирования относятся к [соединителю Spark для DocumentDB](https://github.com/Azure/azure-documentdb-spark/blob/master/samples/notebooks/Spark-to-DocumentDB_Connector.ipynb).

### <a name="connecting-to-flights-sample-data"></a>Подключение к примеру данных о полетах
В этих примерах мы обратимся к данным о выполнении рейсов, которые хранятся в базе данных DocumentDB **DoctorWho**.  Чтобы подключиться к ней, используйте следующий фрагмент кода:
...
С помощью этого фрагмента кода мы также выполним базовый запрос, который передаст отфильтрованный набор данных из DocumentDB в Spark (где в последствии можно выполнить распределенные агрегирования).  В этом случае мы запрашиваем сведения об авиарейсах, вылетающих из Сиэтла (SEA).
...
В этом наборе можно легко выполнять запросы **GROUP BY** к базе данных DocumentDB:
...
Скачайте соединитель DocumentDB для Spark из репозитория [azure-documentdb-spark](https://github.com/Azure/azure-documentdb-spark) на сайте GitHub и изучите дополнительные ресурсы в этом репозитории:
