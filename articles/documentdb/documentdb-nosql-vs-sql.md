---
title: "Решения NoSQL и SQL в службе Azure Cosmos DB | Документация Майкрософт"
description: "Сравните преимущества использования нереляционных решений NoSQL и решений SQL в Azure Cosmos DB. Узнайте, какие преимущества NoSQL и SQL обеспечивает Azure Cosmos DB."
...
---
# <a name="nosql-vs-sql-and-azure-cosmos-db"></a>Решения NoSQL и SQL в службе Azure Cosmos DB
...
Теперь для хранения данных и обращения к ним с помощью объединений вы можете использовать реляционные решения, например SQL Server, так как SQL поддерживает динамические данные [в формате JSON](https://msdn.microsoft.com/library/dn921897.aspx). Но есть еще один вариант, который упрощает подход при этом конкретном сценарии. Это база данных NoSQL. Когда вы используете один документ, как показано ниже, сохраняя его в Azure Cosmos DB, службе базы данных документов Azure NoSQL, вы можете повысить производительность и получить запись полностью с помощью одного запроса и без объединений. Это гораздо более простой и эффективный способ.
...
Такая социальная сеть, представленная в виде сайта, является всего лишь одним из примеров использования базы данных NoSQL в качестве модели данных. Дополнительные сведения об этом сценарии и о моделировании данных для использования Azure Cosmos DB в приложениях социальных сетей см. [в этой статье](documentdb-social-media-apps.md). 
...
![NoSQL и SQL: схема, описывающая использование предложений NoSQL в Microsoft Azure (Azure Cosmos DB, хранилище таблиц, HBase как часть HDInsight, кэш Redis)](./media/documentdb-nosql-vs-sql/nosql-vs-sql-documentdb-storage-hbase-hdinsight-redis-cache.png)

Если для вашего приложения можно использовать одну или несколько служб, ознакомьтесь со следующими ресурсами. 

* [Распространенные варианты использования Azure Cosmos DB](documentdb-use-cases.md)
