...
1. Войдите на [классический портал Azure][azure-classic-portal].
	...
	> [AZURE.NOTE]В настоящее время Azure HDInsight поддерживается на классическом портале Azure, тогда как служба Azure DocumentDB реализована только на портале Microsoft Azure.
