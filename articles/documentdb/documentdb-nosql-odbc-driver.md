...
3. Откройте приложение **Администратор источников данных ODBC** на компьютере. Для этого в поле поиска Windows введите **Источники данных ODBC**. 
    ...
    ![Администратор источников данных ODBC для Azure Cosmos DB](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver.png)

## <a id="connect"></a>Шаг 2. Подключение к базе данных Azure Cosmos DB

1. [Установив драйвер ODBC для Azure Cosmos DB](#install), в окне **Администратор источников данных ODBC** щелкните **Добавить**. Вы можете создать пользовательское или системное имя источника данных. В этом примере мы создаем пользовательское имя источника данных.
...
3. В окне **Azure Cosmos DB ODBC Driver SDN Setup** (Настройка имени источника данных драйвера ODBC для Azure Cosmos DB) укажите следующие сведения: 
    ...
    ![Окно настройки имени источника данных драйвера ODBC для Azure Cosmos DB](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-dsn-setup.png)
    ...
    - **Узел** — универсальный код ресурса (URI) для учетной записи Azure Cosmos DB. Его можно получить в колонке ключей Azure Cosmos DB на портале Azure, как показано на следующем снимке экрана. 
    - **Ключ доступа** — первичный или вторичный ключ для чтения и записи или только для чтения из колонки ключей Azure Cosmos DB на портале Azure, как показано на следующем снимке экрана. Мы рекомендуем использовать ключ только для чтения, если имя источника данных используется для обработки данных, предназначенных только для чтения, и создания отчетов на их основе.
    ![Колонка ключей Azure Cosmos DB](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-keys.png)
...
6. Когда вы завершите настройку и закроете окно **настройки имени источника данных драйвера ODBC для Azure Cosmos DB**, новое пользовательское имя источника данных будет добавлено на соответствующую вкладку.
    ...
    ![Новое имя источника данных ODBC для Azure Cosmos DB на вкладке "DSN пользователя"](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-user-dsn.png)
...
1. Выполнив шаги 1–4 в разделе о [подключении к базе данных Azure Cosmos DB](#connect), щелкните **Редактор схем** в окне **настройки имени источника данных драйвера ODBC для Azure Cosmos DB**.
    ...
    ![Кнопка "Редактор схем" в окне настройки имени источника данных драйвера ODBC для Azure Cosmos DB](./media/documentdb-nosql-odbc-driver/documentdb-nosql-odbc-driver-schema-editor.png)
...
Дополнительные сведения о службе Azure Cosmos DB см. в статье [Знакомство со службой Azure Cosmos DB. API DocumentDB](documentdb-introduction.md).
