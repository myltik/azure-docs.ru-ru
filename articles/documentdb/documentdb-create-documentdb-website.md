...
- Учетная запись хранения Azure Инструкции см. в разделе [Сведения об учетных записях хранения Azure](../storage/storage-create-storage-account.md).
- Рабочая станция с Azure PowerShell 0.9.8. Инструкции см. в разделе [Установка и настройка Azure PowerShell](../powershell-install-configure.md). В это руководство еще не были внесены данные о предварительной версии Azure PowerShell 1.0. 
...
7. Далее мы создадим несколько задач, а затем откроем [портал Microsoft Azure](https://portal.azure.com).
...
8. Выберите просмотр групп ресурсов и выберите созданную во время развертывания группу ресурсов (в приведенном выше примере это myDemoResourceGroup).
	...
	![Снимок экрана на классическом портале Azure с выделенной группой myDemoResourceGroup](./media/documentdb-create-documentdb-website/image3.png)
...
5. Прежде чем перейти к входу на портал Azure, давайте разберемся, как осуществляется развертывание шаблона.
...
6. Давайте откроем [портал Azure](https://portal.azure.com), перейдем в режим просмотра групп ресурсов, а затем выберем созданную во время развертывания группу (в примере выше это myOtherDemoResourceGroup).
...
9. Вы можете продолжить знакомство с порталом Azure либо воспользоваться одним из [примеров](http://go.microsoft.com/fwlink/?LinkID=402386) DocumentDB для создания собственного приложения DocumentDB.
