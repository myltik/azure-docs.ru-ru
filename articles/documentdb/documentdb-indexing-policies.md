...
- **Хэш-индекс** поддерживает эффективные запросы JOIN. Для большинства случаев использования хэш-индексов не требуется более высокая точность, чем разрядность по умолчанию, т. е. 3 байта.
...
- Начинаете использовать новые функции индексирования для своих текущих коллекций DocumentDB, например запросы ORDER BY и запросы диапазона строки, для которых требуется новый тип индекса диапазона строки.
