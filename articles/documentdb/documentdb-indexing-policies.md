<properties 
    ...
    description="В этом документе объясняется, как работает индексирование для базы данных DocumentDB и как настроить и изменить политику индексирования. Настройте политику индексирования в DocumentDB для автоматического индексирования и повышения производительности." 
    keywords="принцип работы индексирования, автоматическое индексирование, индексирование базы данных, documentdb, azure, Microsoft azure"
    services="documentdb" 
    documentationCenter="" 
    authors="arramac" 
    manager="jhubbard" 
...
# <a name="documentdb-indexing-policies"></a>Политики индексации DocumentDB

Хотя многие клиенты с удовольствием позволяют Azure DocumentDB автоматически регулировать [все аспекты индексирования](documentdb-indexing.md), DocumentDB также поддерживает задание пользовательской **политики индексирования** для коллекций во время их создания. Политики индексирования DocumentDB обладают большей гибкостью и возможностями, чем дополнительные индексы, предлагаемые на других платформах баз данных, так как они позволяют разрабатывать и настраивать формы индекса без ущерба для гибкости схемы. Чтобы узнать, как индексация работает в DocumentDB, необходимо понять, почему управляя политикой индексирования, можно найти точный компромисс между дополнительными затратами на хранение индекса, пропускной способностью записи и запросов и согласованностью запросов.  
...
##<a name="<a-id="customizingindexingpolicy"></a>-customizing-the-indexing-policy-of-a-collection"></a><a id="CustomizingIndexingPolicy"></a> Настройка политики индексирования коллекции
...
### <a name="database-indexing-modes"></a>Режимы индексирования базы данных
...
<table border="0" cellspacing="0" cellpadding="0">
    <tbody>
        ...
        <tr>
            ...
            <td valign="top">
                <p>
Уровень согласованности Strong (сильная) </p>
            </td>
            <td valign="top">
                <p>
Уровень согласованности Eventual (в конечном счете) </p>
            </td>            
        </tr>       
        <tr>
            <td valign="top">
                <p>
                    <strong>Ограниченная с запаздыванием</strong>
                </p>
            </td>
            <td valign="top">
                <p>
Ограниченное устаревание </p>
            </td>
            <td valign="top">
                <p>
Уровень согласованности Eventual (в конечном счете) </p>
...
<table border="0" cellspacing="0" cellpadding="0">
    <tbody>
        <tr>
            ...
            <td valign="top">
                <p>
                    <strong>Описание или вариант использования</strong>
                </p>
            </td>
        </tr>
        <tr>
...
/prop/?
            ...
            <td valign="top">
                <p>
Путь индекса, необходимый для обслуживания следующих запросов (с типами хэш, диапазон, соответственно): </p>
                <p>
...
/prop/* </p>
            ...
            <td valign="top">
                <p>
Путь индекса для всех путей по заданной метке. Работает со следующими запросами </p>
                <p>
SELECT * FROM collection c WHERE c.prop = "value" </p>
                <p>
...
/props/[]/?
            ...
            <td valign="top">
                <p>
Путь индекса, необходимый для обслуживания запросов с итерацией и соединения, отправляемых в массивы скалярных выражений, например, ["a", "b", "c"]: </p>
                <p>
...
/props/[]/subprop/?
            ...
            <td valign="top">
                <p>
Путь индекса, необходимый для обслуживания запросов с итерацией и соединения, отправляемых в массивы объектов, например, [{subprop: "a"}, {subprop: "b"}]: </p>
                <p>
...
/prop/subprop/?
            ...
            <td valign="top">
                <p>
Путь индекса, необходимый для обслуживания запросов (с типами хэш, диапазон, соответственно): </p>
                <p>
...
- Тип данных: **String**, **Number** или **Point** (может содержать только одну запись для каждого типа данных пути). Типы данных **Polygon** и **LineString** поддерживаются только в закрытой предварительной версии.
...
DocumentDB также поддерживает вид пространственного индекса для каждого пути, который может быть указан для типа данных точки. Значение по указанному пути должно быть действительным значением точки GeoJSON, например `{"type": "Point", "coordinates": [0.0, 10.0]}`.
...
>[AZURE.NOTE] DocumentDB поддерживает автоматическое индексирование Points, Polygons (закрытая предварительная версия) и LineStrings (закрытая предварительная версия). Чтобы получить доступ к предварительной версии, свяжитесь с нами по электронному адресу askdocdb@microsoft.com, или через службу поддержки Azure.
...
Хэш </p>
            ...
            <td valign="top">
                <p>
Хэш-индекс по over/prop/? (или /*) позволяет эффективно обрабатывать следующие запросы: SELECT * FROM collection c WHERE c.prop = "value". Хэш-индекс по /props/[]/? (или /*, или /props/*) позволяет эффективно обрабатывать следующие запросы: SELECT tag FROM collection c JOIN tag IN c.props WHERE tag = 5 </p>
            </td>
        </tr>
        <tr>
            <td valign="top">
                <p>
Диапазонный индекс </p>
            </td>
            <td valign="top">
                <p>
Диапазонный индекс по /prop/? (или /*) позволяет эффективно обрабатывать следующие запросы: SELECT * FROM collection c WHERE c.prop = "value" SELECT * FROM collection c WHERE c.prop > 5 SELECT * FROM collection c ORDER BY c.prop </p>
            </td>
        </tr>
        <tr>
            <td valign="top">
                <p>
пространственный индекс </p>
            </td>
            <td valign="top">
                <p>
Диапазонный индекс по /prop/? (или /*) позволяет эффективно обрабатывать следующие запросы: SELECT * FROM collection c WHERE ST_DISTANCE(c.prop, {"type": "Point", "coordinates": [0.0, 10.0]}) < 40 SELECT * FROM collection c WHERE ST_WITHIN(c.prop, {"type": "Polygon", ... }) </p>
            </td>
        </tr>        
    </tbody>
</table>

По умолчанию возвращается сообщение об ошибке для запросов с операторами диапазона, например > =, если отсутствует диапазонный индекс (любой точности), чтобы указать, что для обслуживания запроса может потребоваться проверка. Диапазонные запросы можно выполнять без диапазонного индекса с помощью заголовка x-ms-documentdb-enable-scans в REST API или параметра запроса EnableScanInQuery посредством пакета SDK для .NET. Если в запросе есть какие-либо другие фильтры, по которым DocumentDB может отфильтровать индекс, сообщение об ошибке не появится.
...
Так, следующий пример показывает, как добавить документ явным образом с помощью [пакета SDK .NET для DocumentDB](https://github.com/Azure/azure-documentdb-java) и свойства [RequestOptions.IndexingDirective](http://msdn.microsoft.com/library/microsoft.azure.documents.client.requestoptions.indexingdirective.aspx).
...
![Как работает индексирование — преобразование индексов DocumentDB в сети](media/documentdb-indexing-policies/index-transformations.png)
