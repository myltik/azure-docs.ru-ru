...
---
# <a name="storage-and-predictable-performance-provisioning-in-azure-cosmos-db"></a>Подготовка хранилища и прогнозируемой производительности в Azure Cosmos DB
Azure Cosmos DB — это полностью управляемая масштабируемая служба базы данных NoSQL для документов JSON. Благодаря Azure Cosmos DB вам не нужно арендовать виртуальные машины, развертывать программное обеспечение или отслеживать работу баз данных. Система Azure Cosmos DB обслуживается и непрерывно контролируется инженерами корпорации Майкрософт, что позволяет добиться доступности, производительности и защиты данных мирового уровня.  

Вы можете приступить к работе с Azure Cosmos DB, [создав учетную запись базы данных](documentdb-create-account.md), а затем — [коллекцию и базу данных Azure Cosmos DB](documentdb-create-collection.md) на [портале Azure](https://portal.azure.com/). Базы данных Azure Cosmos DB предлагаются в виде единиц хранения на основе твердотельных накопителей (SSD) с определенной пропускной способностью. Эти единицы хранения предоставляются при создании коллекций баз данных. Для каждой коллекции резервируется определенная пропускная способность, которую в любое время можно уменьшать или увеличивать в соответствии с требованиями вашего приложения.
...
Как подписчик Azure вы можете подготовить для управления ресурсами баз данных одну или несколько учетных записей базы данных Azure Cosmos DB. Каждая подписка связана с одной подпиской Azure.

Учетные записи Azure Cosmos DB можно создавать с помощью [портала Azure](documentdb-create-account.md), [шаблона ARM или Azure CLI](documentdb-automation-resource-manager-cli.md).

## <a name="databases"></a>Базы данных
Каждая база данных Azure Cosmos DB может содержать практически неограниченное хранилище документов, разбитое на коллекции. Коллекции обеспечивают изоляцию производительности — каждую коллекцию можно подготовить с объемом пропускной способности, который не используется совместно с другими коллекциями в той же базе данных или учетной записи. База данных Azure Cosmos DB является эластичной, позволяя использовать SSD-хранилище документов размером от нескольких гигабайт до нескольких терабайт и подготовленную пропускную способность. В отличие от традиционных реляционных СУБД, база данных в Azure Cosmos DB не ограничена одним узлом и может охватывать несколько компьютеров или кластеров.  

Благодаря Azure Cosmos DB по мере роста вашего приложения вы можете создать больше коллекций или баз данных либо и то, и другое. Базы данных можно создавать на [портале Azure](documentdb-create-database.md) или с помощью любого [пакета SDK для Azure Cosmos DB](documentdb-dotnet-samples.md).   

## <a name="database-collections"></a>Коллекции базы данных
Каждая база данных Azure Cosmos DB может содержать одну или несколько коллекций. Коллекции выступают в качестве высокодоступных разделов для хранения и обработки данных. В каждой коллекции могут храниться документы с разнородными схемами. Автоматическое индексирование Azure Cosmos DB и возможность выполнять запросы позволяют с легкостью фильтровать и извлекать документы. Коллекция обеспечивает область для хранения документов и выполнения запросов. Коллекция также является доменом транзакций для всех входящих в ее состав документов. Коллекции определяют выделяемую пропускную способность на основе значения, заданного на портале Azure или с помощью пакетов SDK.

Azure Cosmos DB автоматически секционирует коллекции на один или несколько физических серверов. При создании коллекции можно указать подготовленную пропускную способность в единицах запросов в секунду и свойство ключа раздела. Служба Azure Cosmos DB учитывает это значение при распределении документов между разделами и перенаправлении запросов. Значение ключа раздела тоже выступает в качестве границы транзакции для хранимых процедур и триггеров. Каждой коллекции назначается зарезервированный объем пропускной способности, который не используется совместно с другими коллекциями в данной учетной записи. Таким образом, для приложения можно масштабировать как объем хранилища, так и пропускную способность.
...
На расход единиц запросов на выполнение операций в учетной записи базы данных Azure Cosmos DB влияет несколько факторов. К ним относятся:
...
Дополнительные сведения см. в статье [Единицы запросов в базе данных Azure Cosmos DB](documentdb-request-units.md).
...
Azure Cosmos DB поддерживает создание односекционных и секционированных коллекций. Каждый раздел в Azure Cosmos DB поддерживает SSD-хранилище размером до 10 ГБ. В эти 10 ГБ включено как место для хранения собственно документов, так и место для хранения индекса. По умолчанию коллекция Azure Cosmos DB настроена на автоматическое индексирование всех документов без явного указания вторичных индексов или схем. В зависимости от приложения, использующего Azure Cosmos DB, обычные затраты на индекс составляют от 2 до 20 %. Технология индексирования, используемая Azure Cosmos DB, гарантирует, что независимо от значений свойств, показатель накладных расходов не превышает 80 % от размера документов с параметрами по умолчанию.

По умолчанию все документы индексируются Azure Cosmos DB автоматически. Но если нужно уменьшить расходы на индексирование, вы можете отменить индексацию некоторых документов. Это можно сделать при вставке или замене документа, как описано в статье [Как индексируются данные в Azure Cosmos DB](documentdb-indexing-policies.md). Вы можете настроить коллекцию Azure Cosmos DB, чтобы не индексировать все документы в коллекции. А также можете настроить выборочную индексацию коллекции Azure Cosmos DB, например только определенных свойств или путей в документах JSON (используются подстановочные знаки). Описание настройки политики индексации для коллекции см. в [этом разделе](documentdb-indexing-policies.md#CustomizingIndexingPolicy). Исключение свойств или документов их индексирования также улучшает пропускную способность записи, это значит, вы будете потреблять меньшее количество единиц запросов.   

## <a name="next-steps"></a>Дальнейшие действия
Дополнительные сведения о работе Azure Cosmos DB можно найти в статье [Секционирование в базе данных Azure Cosmos DB с помощью API DocumentDB](documentdb-partition-data.md).

Мониторинг уровней производительности на портале Azure описан в статье [Monitor Azure Cosmos DB requests, usage, and storage](documentdb-monitor-accounts.md) (Мониторинг запросов, использования и хранилища Azure Cosmos DB). Дополнительные сведения о выборе уровней производительности коллекций Azure Cosmos DB см. в [этой статье](documentdb-performance-levels.md).
