...
Существуют специальные графовые базы данных, которые можно [использовать в Azure](http://neo4j.com/developer/guide-cloud-deployment/#_windows_azure) , но они не дешевые, и для них требуются службы IaaS (инфраструктура как служба, в основном это виртуальные машины) и обслуживание. Я собираюсь посвятить эту статью более дешевому решению, которое подойдет для большинства сценариев и выполняется в базе данных NoSQL для Azure, [Azure Cosmos DB](https://azure.microsoft.com/services/documentdb/). Используя [NoSQL](https://en.wikipedia.org/wiki/NoSQL), храня данные в формате JSON и применяя [денормализацию](https://en.wikipedia.org/wiki/Denormalization), нашу запись, которая ранее представлялась сложной, можно преобразовать в один [документ](https://en.wikipedia.org/wiki/Document-oriented_database).
...
Дополнительные сведения о вариантах использования Cosmos DB см. в [этой статье](documentdb-use-cases.md).
