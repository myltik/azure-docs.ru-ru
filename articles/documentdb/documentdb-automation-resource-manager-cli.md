...
# Автоматизация создания учетной записи базы данных DocumentDB с помощью шаблонов диспетчера ресурсов Azure и Azure CLI

> [AZURE.SELECTOR]
- [Azure Portal](documentdb-create-account.md)
- [Azure CLI and ARM](documentdb-automation-resource-manager-cli.md)

В этой статье показано, как создать учетную запись DocumentDB с помощью шаблонов диспетчера ресурсов Azure или интерфейса командной строки (CLI Azure).
...
В командной строке введите команду `azure --version`, чтобы узнать, установлена ли у вас версия 0.9.11 или более поздняя версия.

	azure --version
    0.9.11 (node: 0.12.7)

Если у вас не установлена версия 0.9.11 или более поздняя, [установите интерфейс командной строки Azure](../xplat-cli-install.md) либо выполните обновление c помощью одного из встроенных установщиков или команды **npm**, введя `npm update -g azure-cli` для обновления или `npm install -g azure-cli` для установки.
...
Откройте [https://aka.ms/devicelogin](https://aka.ms/devicelogin) в браузере и введите код, предоставленный в выходных данных команды.
...
Помимо описанного здесь метода интерактивного входа в систему, доступны дополнительные методы входа Azure CLI. Дополнительные сведения о других методах и сведения о работе с нескольким подписками см. в разделе [Подключение к подписке Azure в интерфейсе командной строки Azure (Azure CLI)](../xplat-cli-connect.md).
...
## <a id="quick-create-documentdb-account"></a>Задача: создание учетной записи DocumentDB с помощью CLI
...
После возврата команды учетная запись несколько минут будет в состоянии **Создание**, потом состояние изменится на **Online**, в котором она будет готова к использованию. Вы можете проверить состояние учетной записи на [портале Azure](https://portal.azure.com), в колонке **Учетные записи DocumentDB**.
...
В файле azuredeploy.parameters.json обновите значение samplearmacct имени базы данных, которую вы хотите использовать, а затем сохраните файл.

> [AZURE.TIP]В именах учетных записей могут использоваться только строчные буквы, цифры и символ "-". Его длина должна быть от 3 до 50 символов.

### Шаг 2. Создание или получение группы ресурсов

Чтобы создать учетную запись DocumentDB, сначала необходима группа ресурсов. Если известно имя группы ресурсов, которую вы хотите использовать, перейдите к [шагу 3](#create-account-from-template).
...
Вот пример (который показывает строки и запись для учетной записи базы данных с именем new\_db\_acct):
...
При появлении ошибки, например `Deployment provisioning state was not successful`, при создании группы ресурсов или учетной записи базы данных используйте следующую команду для просмотра журнала группы ресурсов.
...
Затем см. раздел [Устранение неполадок развертываний группы ресурсов в Azure](../resource-group-deploy-debug.md) для получения дополнительных сведений.

Помните, что в именах учетных записей могут использоваться только строчные буквы, цифры и символ "-". Длина имени должна быть от 3 до 50 символов.
...
Дополнительные шаблоны см. в статье [Шаблоны быстрого запуска Azure](http://azure.microsoft.com/documentation/templates/).
