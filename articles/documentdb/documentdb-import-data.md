...
1. "Включить внутренние поля": указывает, следует ли включать системные свойства документа DocumentDB в экспорт (например, _rid, _ts). 2. "Число повторных попыток в случае сбоя": указывает количество повторных попыток подключения к DocumentDB при временном сбое (например, прерывания сетевого подключения).
