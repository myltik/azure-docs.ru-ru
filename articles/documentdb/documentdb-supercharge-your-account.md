...
6. В колонке **Параметры** значение **Ценовая категория** изменилось на **Стандартная** и появилось поле **Пропускная способность (единицы запросов в секунду)** со значением 400 (единиц запросов в секунду) по умолчанию. Нажмите кнопку **ОК**, чтобы сохранить изменения.
    ...
    > [AZURE.NOTE] Вы можете задать пропускную способность от 400 до 10 000 [единиц запросов](../articles/documentdb/documentdb-request-units.md) в секунду. **Сводка по ценам** в нижней части страницы автоматически обновляется, показывая оценку ежемесячных расходов.
