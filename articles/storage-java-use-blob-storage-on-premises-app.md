<properties urlDisplayName="Image Gallery w/ Storage" pageTitle="Локальное приложение с хранилищем BLOB-объектов (Java) &mdash; Microsoft Azure" metaKeywords="Azure blob storage, Azure blob Java, Azure blob example, Azure blob tutorial" description="Вы узнаете, как использовать консольное приложение для загрузки изображения на Azure, а затем отобразить изображение в вашем браузере. Примеры кода написаны на Java." metaCanonical="" services="storage" documentationCenter="Java" title="Локальное приложение с хранилищем больших двоичных объектов" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
-   [Использование хранилища BLOB-объектов Azure для отправки файла][Использование хранилища BLOB-объектов Azure для отправки файла]
...
3.  JAR-файл для библиотек Azure для Java и JAR-файлы для любых применимых
    ...
    компилятором Java. Сведения об установке библиотек Azure для Java см. в разделе [Загрузка
    Azure SDK для Java][Загрузка
    Azure SDK для Java].
4.  Настроенная учетная запись хранения Azure. Имя учетной записи
    ...
    коде. См. раздел [Создание учетной записи хранения][Создание учетной записи хранения] для получения информации о создании учетной записи
    и [Управление учетной записью][Управление учетной записью] для получения сведений об извлечении
    ключа учетной записи.
5.  Был создан локальный файл изображения, сохраненный в
    c:\\\\myimages\\\\image1.jpg. Можно также изменить конструктор
    **FileInputStream** в данном примере, чтобы использовать другой путь и другое имя
...
Здесь представлены пошаговые процедуры; если вы хотите пропустить их,
 весь код представлен далее в этом разделе.

Начните работу с кодом, включив операции импорта для базовых классов хранилища
Azure, классов клиента BLOB-объектов Azure, классов ввода-вывода Java и
...
**your\_account\_key** именем и ключом учетной записи,
соответственно.
...
включит необходимые открывающие скобки, **{**.
...
Объявите переменные следующего типа (описания их
использования приведены в этом примере):
...
Присвойте значение переменной **container**. Мы получим ссылку на контейнер с
именем **gettingstarted**.
...
Отправьте локальный файл посредством вызова метода **CloudBlockBlob.upload**
. Первым параметром метода **CloudBlockBlob.upload** является объект
**FileInputStream**, который представляет локальный файл, предназначенный для отправки
 в хранилище Azure. Вторым параметром является размер
файла в байтах.
...
учетной записи хранения Azure. (Код
**MakeHTMLPage** будет обсуждаться далее в этом разделе.)
...
для выполнения итерации по списку отправленных BLOB-объектов. Этот метод создает
исключения типа **FileNotFoundException**, которые создает конструктор
**FileOutputStream** и **URISyntaxException**,
 создаваемое методом **ListBlobItem.getUri**. Вставьте
открывающую скобку, **{**.
...
Выполните итерацию по списку отправленных BLOB-объектов. Для каждого blob-объекта страница HTML
создает элемент **\<img\>**, который имеет собственный атрибут **src**, отправленный в
URI blob-объекта, так как он существует в учетной записи хранения Azure.
Хотя в этот пример было добавлено всего одно изображение, при добавлении нескольких изображений
код будет выполнять итерации по всем этим изображениям.

Для простоты в этом примере предполагается, что каждый отправленный BLOB-объект является изображением. Если
 вы обновили BLOB-объекты, отличные от изображений или являющиеся не
BLOB-блоками,а BLOB-страницами, скорректируйте код соответствующим образом.
...
**your\_account\_key** на имя и ключ учетной записи,
соответственно.
...
Кроме отправки локального файла изображения в хранилище Azure,в этом
примере кода создается локальный файл с именем index.html, который можно
открыть в браузере для просмотра отправленного изображения.

Поскольку код содержит имя учетной записи и ключ учетной записи, обеспечьте
безопасность исходного кода.

## <a name="bkmk_deletecontainer"> </a>Удаление контейнера

Поскольку за хранилище начисляется плата, вам может потребоваться удалить контейнер
**gettingstarted** после окончания экспериментов с этим
примером. Чтобы удалить контейнер, используйте метод **CloudBlobContainer.delete**
:

    container = serviceClient.getContainerReference("gettingstarted");
    container.delete();

Чтобы вызвать метод **CloudBlobContainer.delete** выполняется тот же процесс инициализации
объектов **CloudStorageAccount**, **ClodBlobClient**,
**CloudBlobContainer**, как было показано для метода
**createIfNotExist**. Ниже приведен полный пример, который
...
Обзор других классов и методов хранилища BLOB-объектов см. в разделе [Использование
службы хранилища BLOB-объектов из Java][Использование
службы хранилища BLOB-объектов из Java].
  ...
  [Управление учетной записью]: http://www.windowsazure.com/ru-ru/manage/services/storage/how-to-manage-a-storage-account/
