...
### <a name="0--what-is-the-resource-structure-of-notification-hubs"></a>0.  Какую структуру ресурсов используют Центры уведомлений?

Центры уведомлений имеют два уровня ресурсов — это узлы и пространства имен. Узел — это отдельный ресурс для отправки, который содержит сведения об операциях отправки между платформами для одного приложения. Пространство имен содержит несколько узлов одного региона.

В общем случае мы рекомендуем создавать одно пространство имен для одного приложения. В пространство имен можно включить рабочий концентратор, который работает с основным приложением, тестовый концентратор для работы с тестовым приложением и т. д.

### <a name="1--what-is-the-price-model-for-notification-hubs"></a>1.  Как определяется цена на концентраторы уведомлений?
Последние сведения можно найти на странице [ценами на Центры уведомлений] . Центры уведомлений оплачиваются на уровне пространства имен (определение пространства имен дано выше, в описании структуры ресурсов) по одному из трех уровней тарификации:

* **Бесплатный**. Это хороший выбор для изучения возможностей отправки, но мы не рекомендуем использовать его для рабочих приложений. На каждое пространство имен поддерживается до 500 устройств и до 1 млн уведомлений в месяц, без соглашений об уровне обслуживания.
* **Базовый**. Для небольших рабочих приложений можно выбрать этот уровень или уровень "Стандартный". На каждое пространство имен изначально поддерживается до 200 тысяч устройств и до 10 млн операций отправки в месяц, также предусмотрены возможности для увеличения квот.
* **Стандартный**. Рекомендуемый вариант для средних и крупных рабочих приложений. На каждое пространство имен изначально поддерживается до 10 млн устройств и до 10 млн операций отправки в месяц. Также предусмотрены возможности для увеличения квот и предоставляются широкие возможности для телеметрии.

Вот лишь некоторые из возможностей, доступных для уровня "Стандартный".
...
* *Многопользовательский режим*. Вы можете управлять учетными данными для системы отправки уведомлений платформы (PNS) на уровне пространства имен. Это позволяет легко распределять между клиентами узлы в одном пространстве имен.
...
Для уровней Центров уведомлений **Базовый** и **Стандартный** мы гарантируем, что как минимум 99,9 % времени правильно настроенные приложения смогут отправлять push-уведомления и выполнять операции управления регистрацией через Центр уведомлений, развернутый на поддерживаемом уровне. Дополнительные сведения о соглашении об уровне обслуживания см. на [этой странице](https://azure.microsoft.com/support/legal/sla/notification-hubs/).

> [!NOTE]
> Так как push-уведомления зависят от систем отправки уведомлений платформы сторонних поставщиков (Apple APNS, Google FCM и т. д.), в соглашениях об уровне обслуживания не предусмотрено никаких гарантий по доставке этих сообщений. После того, как Центр уведомлений передаст пакет сторонней PNS (с поддержкой соглашений об уровне обслуживания), непосредственная доставка push-уведомлений возлагается на эту PNS (без поддержки соглашений об уровне обслуживания).

### <a name="3--which-customers-are-using-notification-hubs"></a>3)  Какие клиенты используют концентраторы уведомлений
Наши Центры уведомлений используются множеством клиентов. Ниже перечислены несколько самых значимых из них.
...
### <a name="4-how-do-i-upgrade-or-downgrade-my-hub-or-namespace-to-a-different-tier"></a>4. Как мне повысить или понизить уровень для узла или пространства имен?
На [портале Azure] выберите пункт "Пространства имен Центров уведомлений" или "Центры уведомлений", затем щелкните ресурс, для которого нужно изменить параметры, и перейдите в навигации к разделу "Ценовая категория". Вы можете выбрать любой требуемый уровень с учетом нескольких замечаний.
* Новая ценовая категория будет применяться ко *всем* узлам в пространстве имен, с которым вы работаете.
* Если при снижении уровня будет превышен лимит, установленный для нового уровня, такое изменение вступит в силу только тогда, когда вы удалите все лишние устройства.
...
У нас есть серверные пакеты SDK для .NET, Java, Node.js, PHP и Python. Кроме того, API Центров уведомлений основан на интерфейсах REST, поэтому вы можете работать непосредственно с API REST. Это удобно, если вы используете сразу несколько платформ или хотите избежать дополнительных зависимостей. Дополнительные сведения см. на странице [Использование API REST Центров уведомлений].

### <a name="2--which-client-platforms-do-you-support"></a>2.  Какие клиентские платформы вы поддерживаете?
Мы поддерживаем отправку push-уведомлений на платформы [iOS](notification-hubs-ios-apple-push-notification-apns-get-started.md), [Android](notification-hubs-android-push-notification-google-gcm-get-started.md), [Windows Universal](notification-hubs-windows-store-dotnet-get-started-wns-push-notification.md), [Windows Phone](notification-hubs-windows-mobile-push-notifications-mpns.md), [Kindle](notification-hubs-kindle-amazon-adm-push-notification.md), [Android China (через Baidu)](notification-hubs-baidu-china-android-notifications-get-started.md), Xamarin ([iOS](xamarin-notification-hubs-ios-push-notification-apns-get-started.md)  & и [Android](xamarin-notification-hubs-push-notifications-android-gcm.md)), [Chrome Apps](notification-hubs-chrome-push-notifications-get-started.md) и [Safari](https://github.com/Azure/azure-notificationhubs-samples/tree/master/PushToSafari). Дополнительные сведения см. на странице [Руководства по началу работы].
...
Центры уведомлений Microsoft Azure позволяют просматривать данные телеметрии на [классическом портале Azure]. Дополнительные сведения о доступных метриках см. в статье [Метрики].
...
[Использование API REST Центров уведомлений]: https://msdn.microsoft.com/library/azure/dn530746.aspx
...
[мобильных приложений Azure]: https://azure.microsoft.com/services/app-service/mobile/
[ценами на службу приложений]: https://azure.microsoft.com/pricing/details/app-service/
