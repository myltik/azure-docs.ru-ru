...
<ol start="5">
<li><p>Выберите <b>Notification Services</b> > <b>Windows (WNS)</b>, а затем введите секретный пароль приложения в поле <b>Ключ безопасности</b>. В поле <b>Идентификатор безопасности пакета</b> введите значение, полученное из WNS при работе с предыдущим разделом, и нажмите кнопку <b>Сохранить</b>.</p>
</li>
</ol>
...
2. В поле поиска введите **WindowsAzure.Messaging.Managed**, затем выберите **установить** и примите условия использования.
   
    ![Окно "Управление пакетами NuGet"][20]
   
    После этого будут выполнены скачивание, установка и добавление ссылки на библиотеку обмена сообщениями Azure для Windows с помощью [пакета NuGet WindowsAzure.Messaging.Managed](http://nuget.org/packages/WindowsAzure.Messaging).
...
Для отправки уведомлений с помощью консольного приложения .NET сделайте следующее: 

1. Щелкните решение правой кнопкой мыши, выберите пункты **Добавить** > **Создать проект**. Затем в разделе **Visual C#** выберите **Windows** и **Консольное приложение**, а затем нажмите кнопку **ОК**.
   
    В решение будет добавлено новое консольное приложение Visual C#. Можно также добавить проект в отдельном решении.

2. В Visual Studio последовательно выберите элементы **Сервис**, **Диспетчер пакетов NuGet** и **Консоль диспетчера пакетов**.
   
    В консоли диспетчера пакетов откроется Visual Studio.

3. В окне консоли диспетчера пакетов выберите **Проект по умолчанию** для нового проекта консольного приложения, а затем в окне консоли выполните следующую команду:
    ...
    ![Имя "Проект по умолчанию"](./media/notification-hubs-windows-store-dotnet-get-started/notification-hub-package-manager.png)
...
5. Добавьте в класс **Program** следующий метод:
    ...
    >[!NOTE]
    >* Замените заполнитель **hub name** именем концентратора уведомлений, которое отображается на портале Azure. 
    >* Замените заполнитель строки подключения строкой подключения **DefaultFullSharedAccessSignature**, полученной на странице **Политики доступа** (Политики доступа) Центра уведомлений при работе с разделом "Настройка концентратора уведомлений".
    >* Используйте строку подключения с *полным* доступом, а не c доступом на *прослушивание*. У строки с доступом к прослушиванию отсутствуют разрешения для отправки уведомлений.
   > 
   > 
...
7. В Visual Studio щелкните проект консольного приложения правой кнопкой мыши и выберите пункт **Назначить запускаемым проектом**. Нажмите клавишу **F5**, чтобы запустить приложение.
   
    На всех зарегистрированных устройствах будет получено всплывающее уведомление. Если выбрать баннер уведомления, приложение будет загружено.
