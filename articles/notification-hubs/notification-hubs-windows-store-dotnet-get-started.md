...
2. Выполните поиск по запросу `WindowsAzure.Messaging.Managed` и щелкните **Установить**, выберите все проекты решения и примите условия использования.
	...
	После этого будут выполнены загрузка и установка всех проектов. Кроме того, во все проекты будут добавлены ссылки на библиотеку обмена сообщениями Azure для Windows с помощью <a href="http://nuget.org/packages/WindowsAzure.Messaging.Managed/">пакета NuGet WindowsAzure.Messaging.Managed</a>.
...
В этом учебнике мы пойдем по простому пути и продемонстрируем тестирование клиентского приложения, отправляя уведомления с помощью пакета SDK для .NET для центров уведомлений не в серверную службу, а в консольное приложение. В качестве следующего шага по отправке уведомлений с серверной части ASP.NET рекомендуем ознакомиться с руководством [Уведомление пользователей посредством концентраторов уведомлений с помощью серверной части .NET]. Можно использовать следующие способы отправки уведомлений.

* **Интерфейс REST**. [Интерфейс REST](http://msdn.microsoft.com/library/windowsazure/dn223264.aspx) поддерживает работу с уведомлениями на любой серверной платформе.
...
3. В окне консоли диспетчера пакетов задайте для нового проекта консольного приложения свойство **Проект по умолчанию**, а затем в окне консоли выполните следующую команду:
...
5. Добавьте в класс **Program** следующий метод:
   	...
   	Обязательно замените заполнитель hub name именем центра уведомлений, отображаемым на [классическом портале Azure] на вкладке **Центры уведомлений**. Кроме того, замените заполнитель строки подключения строкой подключения с именем **DefaultFullSharedAccessSignature**, полученной в разделе «Настройка центра уведомлений».
