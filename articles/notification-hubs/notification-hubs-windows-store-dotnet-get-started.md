...
+ Активная учетная запись Azure <br/>Если у вас нет учетной записи Azure, вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](http://azure.microsoft.com/pricing/free-trial/?WT.mc_id=A0E0E5C02&amp;returnurl=http%3A%2F%2Fazure.microsoft.com%2Fja-jp%2Fdocumentation%2Farticles%2Fnotification-hubs-windows-store-dotnet-get-started%2F).
...
1. Если вы еще не зарегистрировали свое приложение, перейдите в <a href="http://go.microsoft.com/fwlink/p/?LinkID=266582" target="_blank">Центр разработки для Windows</a>, выполните вход с использованием своей учетной записи Майкрософт и затем нажмите кнопку **Создать приложение**.
...
1. Войдите на [портал Azure], а затем нажмите кнопку **СОЗДАТЬ** в нижней части экрана.

2. Щелкните элемент **Службы приложений**, выберите пункт **Служебная шина**, затем щелкните элемент **Центр уведомлений** и нажмите кнопку **Быстрое создание**.

   	![][7]

3. Введите имя для центра уведомлений, выберите нужный регион и нажмите кнопку **Создать центр уведомлений**.

   	![][8]

4. Выберите недавно созданное пространство имен (обычно это ***имя центра уведомлений*-ns**), а затем щелкните расположенную сверху вкладку **Настройка**.

   	![][9]

5. Перейдите на расположенную сверху вкладку **Центры уведомлений** и выберите недавно созданный центр уведомлений.
...
7. Перейдите на расположенную сверху вкладку **Панель мониторинга** и нажмите кнопку **Сведения о подключении** внизу страницы. Запишите две строки подключения.
...
1. В Visual Studio щелкните правой кнопкой мыши решение, а затем выберите элемент **Управление пакетами NuGet**.
...
2. Выполните поиск по запросу `WindowsAzure.Messaging.Managed` и щелкните **Установить**, выберите все проекты решения и примите условия использования.
	...
	После этого будут выполнены загрузка и установка всех проектов, а также добавление во все проекты ссылок на библиотеку обмена сообщениями Azure для Windows с помощью <a href="http://nuget.org/packages/WindowsAzure.Messaging.Managed/">пакета WindowsAzure.Messaging.Managed NuGet</a>.
...
* **Java или PHP**: примеры отправки уведомлений с использованием API REST см. в статье «Использование центров уведомлений из Java и PHP» ([Java](notification-hubs-java-backend-how-to.md) | [PHP](notification-hubs-php-backend-how-to.md)).
...
1. Щелкните правой кнопкой мыши решение, выберите элементы **Добавить** и **Новый проект...**. После этого в разделе **Visual C#** последовательно выберите элементы **Windows** и **Консольное приложение**, а затем нажмите кнопку **ОК**.

   	![][13]

	При этом в решение добавляется новое консольное приложение Visual C#. Это можно сделать также и в отдельном решении.

4. В Visual Studio последовательно выберите элементы **Инструменты**, **Диспетчер пакетов NuGet** и **Консоль диспетчера пакетов**.

	В результате в Visual Studio откроется консоль диспетчера пакетов.

6. В окне консоли диспетчера пакетов установите свойство **Проект по умолчанию** для вашего нового проекта консольного приложения, затем в окне консоли выполните следующую команду:
...
8. Щелкните проект консольного приложения в Visual Studio правой кнопкой мыши и выберите параметр **Назначить запускаемым проектом**. Нажмите клавишу **F5**, чтобы запустить консольное приложение.
