...
9. Переопределите метод **OnRegistered()** в классе **PushHandlerService** с помощью следующего кода:
	...
	> [AZURE.NOTE]В коде **OnRegistered()** выше обратите внимание на возможность задавать теги, чтобы зарегистрироваться на определенные каналы обмена сообщениями.
...
- Пакет SDK для центров уведомлений Azure для Java: сведения об отправке уведомлений Java см. в статье [Использование центров уведомлений в Java](notification-hubs-java-backend-how-to.md). Было протестировано в Eclipse для разработки для Android.
...
4. Создав задание, щелкните его имя. На верхней панели перейдите на вкладку **Скрипт**.
