...
1. В Xamarin Studio последовательно щелкните элементы **New Solution** (New Solution), **Android App** (Приложение Android) и **Next** (Далее).

   	![](./media/partner-xamarin-notification-hubs-android-get-started/notification-hub-create-xamarin-android-project1.png)

2. Введите значения в поля **App Name** (Имя приложения) и **Identifier** (Идентификатор). В разделе **Target Plaforms** (Целевые платформы) выберите платформы, на которых должно работать приложение, и последовательно нажмите кнопки **Next** (Далее) и **Create** (Создать).
...
1. Щелкните правой кнопкой мыши папку Components (Компоненты) в приложении Xamarin.Android и выберите пункт **Get More Components (Получить дополнительные компоненты)**.
...
4. Создайте новый класс **MyBroadcastReceiver**.
	...
	> [AZURE.NOTE]Здесь мы рассмотрим создание класса **BroadcastReceiver** с нуля. Быстрой альтернативой созданию файла **MyBroadcastReceiver.cs** вручную является использование ссылки на файл **GcmService.cs**, находящийся в образце проекта Xamarin.Android на [NotificationHubs][GitHub]. Кроме того, вы можете скопировать файл **GcmService.cs** и переименовать классы.

5. Добавьте следующие операторы using в файл **MyBroadcastReceiver.cs** (ссылаясь на компонент и сборку, которые были добавлены ранее):
...
8. **GcmServiceBase** реализует методы **OnRegistered()**, **OnUnRegistered()**, **OnMessage()**, **OnRecoverableError()** и **OnError()**. Наш класс реализации **PushHandlerService** должен переопределить эти методы, и они будут срабатывать в ответ на взаимодействие с центром уведомлений.
...
> [AZURE.IMPORTANT]Чтобы получать push-уведомления, необходимо настроить учетную запись Google на виртуальном устройстве Google Android (в эмуляторе выберите пункт **Settings** (Параметры) и нажмите кнопку **Add Account** (Добавить учетную запись)). Кроме того, убедитесь, что эмулятор подключен к Интернету.

>[AZURE.NOTE]Структура уведомлений в версии платформы Android 5.0 и более поздних версиях существенно отличается от структуры в предыдущих версиях. Дополнительную информацию см. в статье [Уведомления Android](http://go.microsoft.com/fwlink/?LinkId=615880).
...
2. В поле **Target** (Цель) выберите **Google APIs** (API-интерфейсы Google), а затем нажмите кнопку **ОК**.
...
- ASP.NET: см. статью [Уведомление пользователей посредством Центров уведомлений].
- Пакет SDK Центров уведомлений Azure для Java: см. статью [Использование Центров уведомлений из Java](notification-hubs-java-backend-how-to.md) об отправке уведомлений из Java. Было протестировано в Eclipse для разработки для Android.
...
5. Добавьте в класс `Program` следующий метод: Замените текст заполнителя на значение вашей строки подключения *DefaultFullSharedAccessSignature* и имя концентратора на портале Azure.
...
1. Следуйте инструкциям в статье [Приступая к работе с мобильными службами].
...
[Уведомление пользователей посредством Центров уведомлений]: /manage/services/notification-hubs/notify-users-aspnet
