...
Полный код для этого руководства можно найти на портале [GitHub](https://github.com/Azure/azure-notificationhubs-samples/tree/master/dotnet/Xamarin/GetStartedXamarinAndroid).
...
2. Откройте свойства проекта, щелкнув правой кнопкой мыши новый проект в представлении решения и выбрав пункт **Options** (Параметры). Выберите элемент **Приложение Android** в разделе **Сборка**.
...
1. Подготовьте следующую информацию для вашего приложения Android и концентратора уведомлений.
	...
	- **Прослушивание строки подключения**. На портале Azure на панели мониторинга щелкните **Просмотреть строки подключения**. Для этого значения скопируйте строку подключения *DefaultListenSharedAccessSignature*.
...
3. Добавьте переменную экземпляра в класс `MainActivity` для отображения диалогового окна оповещения при выполнении приложения.
...
4. Создайте новый класс **MyBroadcastReceiver**.
	...
	> [AZURE.NOTE]Здесь мы рассмотрим создание класса **BroadcastReceiver** с нуля. Быстрой альтернативой созданию файла **MyBroadcastReceiver.cs** вручную является использование ссылки на файл **GcmService.cs** из образца проекта Xamarin.Android в [примерах NotificationHubs][GitHub]. Вы также можете скопировать файл **GcmService.cs** и изменить имена классов.
...
5. В файле **MyBroadcastReceiver.cs** добавьте следующие разрешения запросов между инструкциями **using** и объявлением **namespace**:
...
9. Переопределите метод **OnRegistered()** в классе **PushHandlerService** с помощью следующего кода:
	...
	> [AZURE.NOTE]В коде **OnRegistered()** выше обратите внимание на возможность задать теги для регистрации на определенные каналы обмена сообщениями.
...
- PHP: см. статью [Использование концентраторов уведомлений из PHP](notification-hubs-php-backend-how-to.md).
...
4. Создав задание, щелкните его имя. На панели вверху перейдите на вкладку **Скрипт**.
...
[Практическое руководство. Концентраторы уведомлений Azure (приложения Android)]: http://msdn.microsoft.com/library/dn282661.aspx
