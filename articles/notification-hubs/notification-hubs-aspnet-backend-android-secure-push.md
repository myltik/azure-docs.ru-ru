...
В этом руководстве описывается, как отправлять безопасные push-уведомления. Так как это руководство продолжает начатый цикл, вам сначала нужно ознакомиться с предыдущими материалами — см. руководство [Уведомление пользователей Android через центры уведомлений с помощью серверной части .NET](notification-hubs-aspnet-backend-android-notify-users.md).
...
Вы изменили серверную часть приложения для отправки только *идентификатора* push-уведомления. Теперь вам нужно изменить приложение Android для обработки этого уведомления и осуществления обратного вызова к серверной части. Это необходимо для извлечения защищенного сообщения, которое будет показано пользователю. Чтобы выполнить эту задачу, убедитесь, что ваше приложение Android умеет идентифицировать себя с серверной частью, когда получает push-уведомления.
