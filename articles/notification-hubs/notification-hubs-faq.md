...
(Функции экспорта и импорта данных регистрации, доступные для категории «Стандартный», описаны здесь: [Экспорт и импорт регистраций].)
...
Концентраторы уведомлений Azure предоставляют несколько функциональных возможностей для стандартного устранения неполадок, особенно в наиболее распространенном случае удаления уведомлений. Дополнительную информацию см. в этом техническом документе: [Центры уведомлений Azure — рекомендации по диагностике].

###2\. Какие телеметрические функции доступны?
Центры уведомлений Microsoft Azure позволяют просматривать данные телеметрии на [классическом портале Azure]. Дополнительную информацию о доступных метриках см. здесь: [Метрики]. Обратите внимание, что успешная доставка уведомлений означает только то, что уведомления доставлены во внешнюю службу push-уведомлений (например, в APNS для Apple, GCM для Google и т. д.), после чего эта служба должна доставить их на устройства. Кроме того, служба push-уведомлений не предоставляет нам такие метрики. Концентраторы уведомлений также предоставляют возможность экспортировать данные телеметрии программным путем (на уровне Standard). Дополнительную информацию см. в этом [примере метрик центра уведомлений].
...
[второй части руководства по регистрации в серверной части]: https://msdn.microsoft.com/library/azure/dn530747.aspx
...
[примере метрик центра уведомлений]: https://github.com/Azure/azure-notificationhubs-samples/tree/master/FetchNHTelemetryInExcel
