...
Во-вторых, чтобы обеспечить доставку данных соответствующим участникам, можно отслеживать их путем [добавления тегов](notification-hubs-routing-tag-expressions.md).

В представленном выше решении описывается сценарий, предполагающий использование множества целевых платформ, поэтому определение геозон не ограничено возможностями конкретной системы. Тем не менее, универсальная платформа Windows предоставляет готовые возможности для [определения геозон](msdn.microsoft.com/ru-RU/windows/uwp/maps-and-location/set-up-a-geofence).
