...
1. Войдите на [портал Azure] и щелкните **+СОЗДАТЬ** в нижней части экрана.
...
5. Скачайте и распакуйте [Пакет Android SDK для мобильных служб], откройте папку **notificationhubs**, скопируйте файл **notification-hubs-x.y.jar** в папку **libs** проекта Eclipse и обновите папку *libs*.
...
10. Добавьте в класс следующий код:
	...
	Присвойте параметру **API\_KEY** значение, которое вы получили ранее из облачного проекта Baidu, параметру **NotificationHubName** — имя вашего концентратора уведомлений на портале Azure, а параметру **NotificationHubConnectionString** — значение DefaultListenSharedAccessSignature на портале Azure.
...
Чтобы протестировать это приложение с помощью эмулятора, в верхней панели инструментов Eclipse нажмите кнопку **Run** (Запуск) и выберите приложение. Это действие запускает эмулятор и выполняет приложение.
