<properties
	pageTitle="Форматированные push-уведомления на основе концентраторов уведомлений Azure"
...
1. В Visual Studio откройте проект **AppBackend**, созданный в учебнике [Уведомление пользователей](notification-hubs-aspnet-backend-ios-notify-users.md).
...
9. Выберите в качестве цели публикации веб-сайт Azure. Войдите с помощью учетной записи Azure, выберите существующий или новый веб-сайт и запишите свойство **URL-адрес назначения** во вкладке **Подключение**. Далее в учебнике этот URL-адрес будет называться *конечная точка сервера*. Щелкните **Опубликовать**.
...
3. Перейдите к файлу **Main.storyboard** и убедитесь, что у вас есть контроллер представления (который в этом учебнике называется контроллером домашнего представления) из раздела[Уведомление пользователя](notification-hubs-aspnet-backend-ios-notify-users.md).
...
14. Чтобы применить изменения интерфейса пользователя раскадровки, в следующей реализации добавьте **application:didRegisterForRemoteNotificationsWithDeviceToken**:
...
15. Затем добавьте следующие методы в файл **AppDelegate.m** для получения изображения из конечной точки и отправки локального уведомления после получения. Обязательно замените заполнитель `{backend endpoint}` на конечную точку серверной части:
