...
Вы можете осуществлять потоковую передачу набора MP4-файлов с адаптивной скоростью, создавая указатель потоковой передачи OnDemand и формируя URL-адрес потоковой передачи. [Кодирование актива](media-services-rest-encode-asset.md) статье показано, как кодировать в набор MP4 с адаптивной скоростью. При шифровании контента настроить политику доставки актива (как описано в [это](media-services-rest-configure-asset-delivery-policy.md) статьи) до создания локатора. 
...
В этой статье демонстрируется создание OnDemand потоковую передачу указателя для публикации актива и построения Smooth, MPEG DASH и HLS URL-адреса потоковой передачи. В нем также показывается, как создать URL-адреса последовательного скачивания.
...
Для создания по запросу, указатель потоковой передачи и получения URL-адреса, необходимо сделать следующее:
...
5. Невозможно создать указатель потоковой передачи с помощью AccessPolicy, который включает запись или удаление разрешений.
