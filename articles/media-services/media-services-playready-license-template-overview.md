...
Службы мультимедиа также определяют набор классов .NET, которые могут использоваться для сериализации в XML и десериализации из XML. Описания основных классов, используемых для настройки шаблонов лицензий, см. в статье [Классы .NET служб мультимедиа](media-services-playready-license-template-overview.md#classes).
