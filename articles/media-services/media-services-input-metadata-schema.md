...
В этой статье рассматриваются элементы и типы XML-схемы, на основе которой создаются входные метаданные (&lt;ИД_ресурса-контейнера&gt;_metadata.xml).  Сведения о файле, который содержит метаданные выходного контейнера-ресурса, см. в статье [Output Metadata](media-services-output-metadata-schema.md) (Выходные метаданные).  
