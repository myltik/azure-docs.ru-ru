...
Если для служб мультимедиа требуется шифрование ресурса, необходимо связать ключ шифрования (**CommonEncryption** или **EnvelopeEncryption**) с ресурсом (как описано [здесь](media-services-dotnet-create-contentkey.md)), а также конфигурация политики авторизации ключа (как описано в этой статье). 
...
- Чтобы применять динамическую упаковку и динамическое шифрование, у вас должна быть хотя бы одна зарезервированная единица потоковой передачи. Дополнительную информацию см. в разделе [Масштабирование службы мультимедиа](media-services-manage-origins.md#scale_streaming_endpoints). 
...
Для получения маркера тестирования на основе маркера ограничения, который использовался для политики авторизации ключа, см. [этот](#test) раздел. 
