...
Если необходимо, чтобы службы мультимедиа зашифровали ресурс-контейнер, требуется сопоставить ключ шифрования (**CommonEncryption** или **EnvelopeEncryption**) с ресурс-контейнером (как описано [здесь](media-services-dotnet-create-contentkey.md)), а также настроить политики авторизации ключа (как описано в этой статье).
