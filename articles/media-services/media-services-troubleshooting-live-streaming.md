<properties 
    pageTitle="Руководство по устранению неполадок потоковой передачи в реальном времени | Microsoft Azure" 
...
###<a name="problem:-unable-to-playback-the-published-stream."></a>Проблема. Не удается воспроизвести опубликованный поток.
...
- **Потенциальная причина**. Не запущена конечная точка потоковой передачи или не выделены единицы потоковой передачи (единицы масштабирования). 
    ...
    **Шаги по устранению неполадок**. В AMSE откройте вкладку "Конечная точка потоковой передачи" и убедитесь, что запущена одна конечная точка потоковой передачи с одной единицей потоковой передачи. 
    


>[AZURE.NOTE] Если после выполнения этих действий воспроизвести поток по-прежнему не получается, отправьте запрос в службу поддержки с помощью портала Azure.
