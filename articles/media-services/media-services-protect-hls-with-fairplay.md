...
Пользователи могут разрабатывать приложения проигрывателя с помощью пакета SDK для iOS. Для воспроизведения содержимого FairPlay клиентам необходимо реализовать протокол обмена лицензиями. Протокол обмена лицензиями не указывается компанией Apple. Каждое приложение может само выбирать, как отправлять запросы доставки ключей. Служба доставки ключей FairPlay AMS ожидает, что SPC поступит как сообщение публикации www-form-url следующего вида:
