...
- **Незащищенный ключ конверта AES-128**: весь фрагмент шифруется в режиме **AES-128 CBC**. Проигрыватели iOS и OSX поддерживают расшифровку потока по умолчанию. Дополнительные сведения см. в [этой статье](media-services-protect-with-aes128.md).
...
Пользователи могут разрабатывать приложения проигрывателя с помощью пакета SDK для iOS. Для воспроизведения содержимого FairPlay клиентам необходимо реализовать протокол обмена лицензиями. Apple не определяет протокол обмена лицензиями, поэтому каждое приложение само выбирает формат передачи запросов о доставке ключей. Служба доставки ключей FairPlay AMS ожидает, что SPC поступит как сообщение публикации www-form-url следующего вида:
