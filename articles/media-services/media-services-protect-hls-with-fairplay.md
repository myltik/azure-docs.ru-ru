...
#Использование служб мультимедиа Azure для потоковой передачи содержимого HLS, защищенного с помощью Apple FairPlay 

Службы мультимедиа Azure позволяют использовать динамическое шифрование содержимого HTTP Live Streaming (HLS), используя следующие форматы:

- **Незащищенный ключ конверта AES-128**: весь фрагмент шифруется в режиме **AES-128 CTR**. Проигрыватели iOS и OSX поддерживают расшифровку потока по умолчанию. Дополнительные сведения см. в [этой статье](media-services-protect-with-aes128.md).

- **Apple FairPlay**: отдельные образцы видео и аудиоданных шифруются в режиме **AES-128 CBC**. **Потоковая передача FairPlay** (FPS): интегрируется в операционные системы устройств и поддерживается iOS и Apple TV по умолчанию. Safari в OS X включает FPS, используя поддержку интерфейса расширений зашифрованных носителей (EME).

	>[AZURE.NOTE]
	В настоящее время применение AMS для доставки HLS, зашифрованного с помощью FairPlay, доступен в предварительной версии.
...
- При использовании AMS для доставки HLS, зашифрованного с помощью FairPlay, и лицензий FairPlay необходимо следующее:
	...
	- Пакет SDK служб мультимедиа Azure для .NET обновлен до версии **3.6.0**.
...
- На стороне доставки ключей AMS должны быть заданы следующие параметры:
	...
	- **Идентификатор пароля к сертификату приложения**: клиент должен передать пароль таким же образом, как при передаче других ключей AMS и использовании значения перечисления **ContentKeyType.FairPlayPfxPassword** В результате он получает идентификатор AMS, который нужно использовать в параметре политики доставки ключей.
	...
	- **ASK**: ASK (секретный ключ приложения) принимается при формировании сертификата с использованием портала для разработчиков Azure. Каждая группа разработчиков получает свой, уникальный ASK. Сохраните копию ASK в безопасном месте. Впоследствии ASK необходимо настроить как FairPlayAsk для служб мультимедиа Azure. 
	-  **Идентификатор ASK**: предоставляется Apple. Клиент должен загрузить ASK таким же образом, как при передаче других ключей AMS и использовании значения перечисления **ContentKeyType.FairPlayASk**. В результате он получает идентификатор WAMS, который нужно использовать в параметре политики доставки ключей.
...
- Для воспроизведения зашифрованного потока FairPlay необходимо получить реальный ASK, а затем создать реальный сертификат. Этот процесс создает все три части:

	-  DER-файл, 
	-  PFX-файл и 
	-  пароль для PFX-файла.
 
- Клиенты, поддерживающие HLS с шифрованием **AES-128 CBC**: Safari на OS X, Apple TV, iOS.
...
1. Настройте политику авторизации для ключа содержимого. При создании политики авторизации ключа содержимого необходимо указать следующее: 
	...
	- Конфигурация параметров политики FairPlay. Дополнительные сведения о настройке FairPlay см. ниже в описании метода ConfigureFairPlayPolicyOptions().
