...
- На стороне доставки ключей AMS должны быть заданы следующие параметры:
	...
	- **ASK** — секретный ключ приложения, принимается при формировании сертификата с использованием портала для разработчиков Azure. Каждая группа разработчиков получает свой, уникальный ASK. Сохраните копию ASK в безопасном месте. Впоследствии ASK необходимо настроить как FairPlayAsk для служб мультимедиа Azure. 
...
1. Настройте политику авторизации для ключа содержимого. При создании политики авторизации ключа содержимого необходимо указать следующее: 
	...
	- Конфигурация параметров политики FairPlay. Дополнительные сведения о настройке FairPlay см. ниже в примере метода ConfigureFairPlayPolicyOptions().
		...
		>[AZURE.NOTE] В большинстве случаев настраивать параметры политики FairPlay необходимо только один раз, так как у вас будет один набор сертификации и ASK.
...
>[AZURE.NOTE] В проигрывателе мультимедиа Azure предварительно не настроена поддержка воспроизведения FairPlay. Чтобы получить возможность воспроизведения FairPlay в Mac OS X, необходимо получить образец проигрывателя из учетной записи разработчика Apple.
>
>Кроме того, можно разрабатывать приложения с помощью пакета SDK для iOS.
