...
**DRM**|**Собственная ОС/собственное приложение**|**Поддерживаемые браузеры для настольных систем**|**Поддерживаемые браузеры для мобильных систем**|**Поддерживаемые форматы потоковой передачи**
----|------|----|-----|----
**PlayReady**|Windows, Windows Phone, Xbox|IE11 на Windows 8.1+, Microsoft Edge|Windows Phone 8.1+|DASH, Smooth Streaming
**Widevine**|Android|Chrome 34+ на всех компьютерах ПК и Mac|Android 4.3+|DASH
...
------|-------
**Проигрыватель**|[Azure Media Player](http://azure.microsoft.com/services/media-services/media-player/)
...
1. Создайте проигрыватель с помощью [API мультимедиапроигрывателя Azure](http://amp.azure.net/libs/amp/latest/docs/). [API ProtectionInfo мультимедиапроигрывателя Azure ](http://amp.azure.net/libs/amp/latest/docs/) позволяет указать, какие технологии DRM использовать на разных платформах DRM.
1. Тестовая матрица:
...
Джордж Трифонов из команды служб мультимедиа Azure ведет блог, где вы найдете подробное описание шагов для настройки Azure Active Directory для приложения проигрывателя ASP.NET MVC: [интегрируйте приложение на основе OWIN MVC служб мультимедиа Azure с Azure Active Directory и ограничьте доставку ключей содержимого на основе утверждений JWT](http://gtrifonov.com/2015/01/24/mvc-owin-azure-media-services-ad-integration/).
...
##Дополнительные вопросы в нашем проектировании и реализации

Далее мы рассмотрим некоторые дополнительные вопросы в нашем проектирования и реализации.
...
1. приобретение лицензий DRM клиентом, который должен находиться в разделе HTTPS.
...
Если взглянуть на то, как веб-приложение вызывает приложение API в разделе [Удостоверение приложения с предоставлением учетных данных клиента OAuth 2.0](active-directory-authentication-scenarios.md#web-application-to-web-api), поток проверки подлинности выглядит так:
...
В этом потоке "удостоверения приложения" веб-интерфейс API доверяет тому, что веб-приложение проверило подлинность пользователя. По этой причине данная схема называется доверенной (надежной) подсистемой. [Схема на этой странице](http://msdn.microsoft.com/library/azure/dn645542.aspx) описывает, как работает поток предоставления кода авторизации.
...
**Учетная запись домена пользовательского клиента Azure AD**: в этом случае вы видите пользовательскую страницу входа домена пользовательского клиента Azure AD.
...
###Использование расширения зашифрованных носителей (EME) для PlayReady

В современных браузерах с поддержкой EME или PlayReady, таких как IE 11 в Windows 8.1 и выше и Microsoft Edge в Windows 10, PlayReady будет базовой DRM для EME.
...
![Пользователи без прав](./media/media-services-cenc-with-multidrm-access-control/media-services-unentitledusers.png.png)

###Использование пользовательской службы маркеров безопасности (STS)

В сценариях с использованием пользовательской STS маркер JWT будет выдаваться пользовательской STS с использованием симметричного или асимметричного ключа.
...
Вильям Цанг, Ян Мингфей, Килрой Хьюс, Юлия Корнич
