...
Рекомендуется использовать стандартный кодировщик мультимедиа. Однако он в настоящее время недоступен на классическом портале Azure.
...
Дополнительные сведения см. в статье [Усечение видео с помощью стандартного кодировщика служб мультимедиа](media-services-custom-mes-presets-with-dotnet.md#trim_video).

###Создание наложений

Дополнительные сведения см. в статье [Создание наложений с помощью стандартного кодировщика служб мультимедиа](media-services-custom-mes-presets-with-dotnet.md#overlay).
...
ErrorParsingConfiguration|Категория ошибок, в которых задача <see cref="MediaTask.PrivateData"/> (конфигурация) является недопустимой, например, конфигурация не является допустимой конфигурации системы или содержит недопустимый XML.
...
Для получения помощи от команды **служб мультимедиа** откройте [запрос в службу поддержки](https://ms.portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade).
