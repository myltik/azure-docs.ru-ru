...
- Можно считывать данные непосредственно из хранилища таблиц Azure (например, с помощью пакета SDK для хранилища). Описание таблиц хранилища для данных телеметрии см. в подразделе **Использование данные телеметрии** в [этом](https://msdn.microsoft.com/library/mt742089.aspx) разделе.
...
Из таблиц телеметрии можно запрашивать приведенные ниже данные метрик. Далее в этом разделе показан пример того, как использовать пакет SDK служб мультимедиа для .NET для запроса метрик.
