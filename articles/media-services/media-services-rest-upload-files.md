...
> * Существует ограничение на максимальный размер файла, который могут обработать службы мультимедиа. В разделе [это](media-services-quotas-and-limitations.md) статьи приведены сведения о ограничения размера файла.
> 
...
AMS также позволяет передавать ресурсы в пакетном режиме. Дополнительные сведения см. в [этом разделе](media-services-rest-upload-files.md#upload_in_bulk).
...
* **Нет** = **0**: шифрование не используется. Это значение по умолчанию. При использовании этого параметра содержимое не защищено при передаче и хранении.
    Используйте этот параметр, если MP4-файл планируется доставить с помощью поэтапного скачивания. 
* **StorageEncrypted** = **1**. Укажите, если нужно использовать 256-разрядное шифрование AES для хранения и передачи файлов.
...
> [!NOTE]
> Если актива использует шифрование, необходимо создать **ContentKey** и связать его с ресурсами, как описано в следующей статье: [как создать ContentKey](media-services-rest-create-contentkey.md). После отправки файлов в актив, необходимо обновить свойства шифрования на **AssetFile** сущности со значениями, полученный во время **активов** шифрования. Сделать это можно с помощью HTTP-запроса **MERGE** . 
> 
> 
...
После отправки файла мультимедиа в контейнер больших двоичных объектов, используется **СЛИЯНИЯ** HTTP-запроса для обновления сведений о файле мультимедиа AssetFile (как показано далее в этой статье). 
...
### <a name="create-the-ingestmanifest"></a>Создание сущности IngestManifest
...
Прежде чем создать манифест IngestManifestAsset, необходимо создать ресурс, который будет выполняться с помощью массового приема. Ресурс — это контейнер, состоящий из нескольких наборов объектов или объектов разного типа в службах мультимедиа, в том числе видео, аудио, изображения, коллекции отпечатков, текстовые каналы и файлы скрытых субтитров. В API REST создания ресурса необходимо отправить запрос HTTP POST для службы мультимедиа Microsoft Azure и поместить все сведения о свойствах ресурса в тексте запроса. В этом примере ресурс создается с помощью параметра StorageEncrption(1), включенных в тексте запроса.
...
### <a name="create-the-ingestmanifestfiles-for-each-asset"></a>Создание сущности IngestManifestFiles для каждого ресурса
IngestManifestFile представляет фактический видео- или большой двоичный объект объекта, который загружен при массовой передаче актива. Свойства, связанные с шифрованием не требуются, если ресурс не использует шифрование. В примере, который приводится в этом разделе, демонстрируется создание сущности IngestManifestFile, которая использует для созданного ранее ресурса параметр StorageEncryption.
...
### <a name="upload-the-files-to-blob-storage"></a>Отправка файлов в хранилище BLOB-объектов
Можно использовать любое высокоскоростное клиентское приложение, может загрузить файлы ресурса в контейнер хранилища BLOB-объекта, Uri, предоставленному свойству BlobStorageUriForUpload объекта IngestManifest. Одна из важных высокоскоростной передачи служб будет [Aspera On Demand for Azure приложения](http://go.microsoft.com/fwlink/?LinkId=272001).

### <a name="monitor-bulk-ingest-progress"></a>Контроль за ходом выполнения массового приема
Ход выполнения массового приема операций для сущности IngestManifest можно отслеживать путем опроса свойства Statistics сущности IngestManifest. Это свойство комплексного типа, [IngestManifestStatistics](https://docs.microsoft.com/rest/api/media/operations/ingestmanifeststatistics). Для опроса свойства Statistics, отправьте запрос HTTP GET, передав код IngestManifest.

## <a name="create-contentkeys-used-for-encryption"></a>Создание сущности ContentKeys, используемой для шифрования
Если актива использует шифрование, необходимо создать ContentKey, используемый для шифрования перед созданием файлов актива. Для шифрования хранилища в текст запроса необходимо включить указанные ниже свойства.

| Свойство текста запроса | ОПИСАНИЕ |
| --- | --- |
| Идентификатор |Код ContentKey, что мы создаем сами в следующем формате «nb:kid:UUID:<NEW GUID>». |
...
| EncryptedContentKey |Создадим новое значение ключа контента, имеющий значение 256-разрядный (32 байта). Ключ шифруется с использованием сертификата хранилища шифрования X.509, который получен из службы мультимедиа Microsoft Azure, выполнив запрос HTTP GET для GetProtectionKeyId и GetProtectionKey методы. |
...
### <a name="link-the-contentkey-to-the-asset"></a>Привязка ключа содержимого к ресурсу
ContentKey связывается с одним или несколькими ресурсами, отправив запрос HTTP POST. Приведенный ниже запрос представляет собой пример привязки ключа содержимого к ресурсу по идентификатору.
