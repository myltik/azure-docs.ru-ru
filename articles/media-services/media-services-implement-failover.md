<properties 
	pageTitle="Реализация сценария потоковой передачи отработки отказа | Microsoft Azure" 
...
#Реализация сценария потоковой передачи отработки отказа

В этом пошаговом руководстве показано, как скопировать содержимое (большие двоичные объекты) из одного ресурса в другой для обработки избыточности потокового воспроизведения по запросу. Этот сценарий пригодится клиентам, которым нужно настроить CDN на переключение между двумя центрами обработки данных в случае, если один из наших центров выдаст сбой. В этом пошаговом руководстве пакет SDK и API REST служб мультимедиа Microsoft Azure, а также пакет SDK хранилища Azure используются для демонстрации следующих задач:

1. настройка учетной записи для служб мультимедиа в "Центре обработки данных A";
1. отправка мезонинного файла в исходный ресурс;
1. кодирование ресурса в мультискоростные MP4-файлы; 
1. создание доступного только для чтения указателя SAS, который обеспечит исходному ресурсу доступ к контейнеру в учетной записи хранения, связанной с этим исходным ресурсом;
1. получение имени контейнера исходного ресурса из указателя SAS, доступного только для чтения, который вы создали на предыдущем этапе. Эти сведения необходимы для копирования больших двоичных объектов из одной учетной записи хранения в другую (эта процедура описана далее в этом разделе).
1. Создание указателя источника для ресурса, созданного задачей кодирования. 

А также задачи для обработки отказа:

1. настройка учетной записи для служб мультимедиа в "Центре обработки данных B";
1. создание пустого целевого ресурса в целевой учетной записи служб мультимедиа;
1. создание доступного для записи указателя SAS для пустого целевого ресурса, чтобы обеспечить доступ на запись к контейнеру в целевой учетной записи хранения, связанной с целевым ресурсом;
1. использование пакета SDK хранилища Azure для копирования больших двоичных объектов (файлов ресурсов) из исходной учетной записи хранения в "Центре обработки данных A" в целевую учетную запись хранения в "Центре обработки данных Б" и обратно (эти учетные записи хранения связаны с интересующими нас ресурсами);
1. связывание больших двоичных объектов (файлов ресурсов), скопированных в целевой контейнер больших двоичных объектов в целевом ресурсе; 
1. создание указателя источника для ресурса в "Центре обработки данных Б" и определение идентификатора указателя, созданного для ресурса в "Центре обработки данных А". 
1. Это позволяет получить URL-адреса потоковой передачи, если относительные пути в URL-адресах совпадают (отличаются только базовые URL-адреса). 
 
Затем для обработки сбоев поверх указателей источников можно создать CDN.

Действительны следующие условия.

- Текущая версия пакета SDK служб мультимедиа не поддерживает создание указателей с заданным идентификатором. Для выполнения этой задачи будет использоваться API REST служб мультимедиа.
- Текущая версия пакета SDK служб мультимедиа не поддерживает программное создание сведений IAssetFile, связывающих ресурс с файлами ресурса. Для выполнения этой задачи мы будем использовать API REST служб мультимедиа CreateFileInfos. 
- Зашифрованные ресурсы хранилища (AssetCreationOptions.StorageEncrypted) нельзя использовать для репликации (поскольку ключ шифрования в учетных записях служб мультимедиа будет разным). 
- Чтобы воспользоваться всеми преимуществами динамической упаковки, необходимо получить по крайней мере один из зарезервированных модулей потоковой передачи по требованию. Дополнительные сведения см. в статье [Динамическая упаковка ресурсов](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Вместо того чтобы реализовать сценарий потоковой передачи отработки отказа вручную, можно воспользоваться [средством репликации](http://replicator.codeplex.com/) служб мультимедиа. Оно позволит вам реплицировать ресурсы между двумя учетными записями служб мультимедиа.

##Предварительные требования
 
- Две учетные записи служб мультимедиа в новой или существующей подписке Azure. См. статью [Создание учетной записи служб мультимедиа](media-services-create-account.md).
...
1. Создайте папку SupportFiles на том же уровне, где находится файл проекта HandleRedundancyForOnDemandStreaming.csproj. В папке SupportFiles создайте папки OutputFiles и MP4Files. Скопируйте MP4-файл в папку MP4Files (в этом примере используется файл BigBuckBunny.mp4). 
...
1. Добавьте другие ссылки, необходимые для этого проекта: System.Configuration, System.Runtime.Serialization и System.Web.
...
##Добавление кода, обрабатывающего избыточность потокового воспроизведения по запросу.
...
1. Замените определение метода Main по умолчанию на следующий:
...
1. Определения методов, вызываемых из метода Main, описаны ниже.
		                                        ...
		                                        bool autoRedirect = true)
		{
		    ...
		    if (startTime != DateTime.MinValue)
		        sb.Append(", "StartTime" : "" + startTime.ToString("G", CultureInfo.CreateSpecificCulture("ru-RU")) + """);
