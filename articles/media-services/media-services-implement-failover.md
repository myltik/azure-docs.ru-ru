...
4. создание доступного только для чтения указателя SAS, который обеспечит исходному ресурсу доступ к контейнеру в учетной записи хранения, связанной с этим исходным ресурсом;
5. получение имени контейнера исходного ресурса из указателя SAS, доступного только для чтения, который вы создали на предыдущем этапе. Эти сведения необходимы для копирования больших двоичных объектов из одной учетной записи хранения в другую (эта процедура описана далее в этом разделе).
...
3. создание доступного для записи указателя SAS для пустого целевого ресурса, чтобы обеспечить доступ на запись к контейнеру в целевой учетной записи хранения, связанной с целевым ресурсом;
4. использование пакета SDK хранилища Azure для копирования больших двоичных объектов (файлов ресурсов) из исходной учетной записи хранения в "Центре обработки данных A" в целевую учетную запись хранения в "Центре обработки данных Б" и обратно (эти учетные записи хранения связаны с интересующими нас ресурсами);
...
7. Это позволяет получить URL-адреса потоковой передачи, если относительные пути в URL-адресах совпадают (отличаются только базовые URL-адреса). 
...
> [!NOTE]
> Вместо того чтобы реализовать сценарий потоковой передачи отработки отказа вручную, можно воспользоваться [средством репликации](http://replicator.codeplex.com/) служб мультимедиа. Оно позволит вам реплицировать ресурсы между двумя учетными записями служб мультимедиа.
> 
> 
...
2. Замените определение метода Main по умолчанию на следующий:
