...
3. Щелкните **Create Channel** (Создать канал). ![wirecast](./media/media-services-wirecast-live-encoder/media-services-wirecast2.png)
...
4. На странице параметров для поля **Destination** (Назначение) задайте значение **Azure Media Services** (Службы мультимедиа Azure).
	...
	В поле «Encoding profile» (Профиль кодировки) будет выбран профиль **Azure H.264 720p 16:9 (1280x720)**. Чтобы настроить эти параметры, выберите значок шестеренки справа от раскрывающегося списка и затем выберите **New Preset** (Новая предустановка).
