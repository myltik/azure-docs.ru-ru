...
1. Чтобы остановить потоковую передачу и архивацию контента мероприятия, завершите программу.
...
Вы можете определить IP-адреса, с которых разрешено публиковать видео в этом канале. Разрешенные IP-адреса можно указать в виде одного IP-адреса (например, 10.0.0.1), диапазона IP-адресов, заданного с помощью IP-адреса и маски подсети CIDR (например, 10.0.0.1/22), или диапазона IP-адресов, заданного с помощью IP-адрес и маски подсети в десятичной записи (например, 10.0.0.1(255.255.252.0)).
...
необязательный параметр. Дает динамическому кодировщику сигнал переключиться во время рекламной паузы на статическое изображение по умолчанию, чтобы скрыть входящий видеопоток. Звук на это время также выключается. Значение по умолчанию — **false**.
...
###Идентификатор ресурса листа по умолчанию
...
**Примечание**. Перед созданием канала необходимо добавить изображение листа в формате JPEG с максимальным разрешением 1920x1080 и размером не более 3 МБ в качестве выделенного ресурса (в нем не должно быть других файлов). Имя файла должно иметь расширение *.jpg, и этот файл AssetFile должен быть помечен как основной файл ресурса. Файл должен храниться в незашифрованном виде.
...
Когда вы будете готовы начать потоковую передачу и архивацию, запустите программу. Чтобы остановить потоковую передачу и архивацию контента мероприятия, завершите программу.
...
###Публикации и доставка ресурсов

**Общие сведения**:

- [Общие сведения о динамической упаковке](../media-services-dynamic-overview.md)
- [Обзор доставки содержимого](media-services-deliver-content-overview.md)

Настройте политику доставки ресурсов с помощью **.NET** или **REST API**.
