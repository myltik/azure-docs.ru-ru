...
В следующей таблице показано, как состояния канала соотносятся с состояниями выставления счетов в API и на портале. Обратите внимание, что состояния для API и портала немного отличаются. Как только канал переходит в состояние «Работает» с помощью API-интерфейса или в состояние «Готово» или «Потоковая передача» на портале Azure, начинается начисление оплаты. Чтобы прекратить начисление оплаты за канал, необходимо остановить канал через API или на портале. Именно вы несете ответственность за остановку каналов после завершения кодирования в реальном времени. Если вы не остановите работу канала, начисление оплаты будет продолжено.
