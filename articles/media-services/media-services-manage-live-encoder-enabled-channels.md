...
- **Нет** — задайте это значение, если вы собираетесь использовать локальный динамический кодировщик, который будет выдавать многоскоростной поток. В этом случае входящий поток передается на выход без кодирования. Таким образом каналы работали до выпуска 2.10. Более подробную информацию о работе с каналами такого типа см. в разделе [Работа с каналами, принимающими многоскоростные потоки в реальном времени от локальных кодировщиков](media-services-manage-channels-overview.md).

- **Стандартный** (предварительная версия) — выберите это значение, если собираетесь использовать службы мультимедиа для кодирования односкоростного потока в многоскоростной.

	Функция кодирования потока в реальном времени с помощью служб мультимедиа в настоящее время находится в состоянии предварительной версии.

>[AZURE.NOTE]В этом разделе рассматриваются атрибуты канала, с помощью которых обеспечивается кодирование в реальном времени (тип кодирования **Стандартный**). Информацию о работе с каналами, которые не выполняют кодирование в реальном времени, см. в разделе [Работа с каналами, принимающими многоскоростные потоки в реальном времени от локальных кодировщиков](media-services-manage-channels-overview.md).
...
>[AZURE.NOTE]Кодирование в реальном времени с помощью служб мультимедиа Azure поддерживается не во всех центрах обработки данных.
>
>При использовании портала управления Azure для создания каналов будут доступны два варианта типов кодирования каналов: **Нет** и **Стандартный**. Если отображается только вариант **Нет**, значит центр обработки данных не поддерживает кодирование в реальном времени с помощью AMS.
>
>Если вы используете пакет SDK .NET или REST API, для проверки сделайте следующее:
>
>1. Попробуйте создать канал с типом кодирования «Стандартный». 
>2. Если возвращается результат HTTP с кодом ошибки 412 (условие ложно) с сообщением *Кодирование в реальном времени не поддерживается в этом регионе. Для EncodingType необходимо задать значение «Нет»*, ваш центр обработки данных не поддерживает кодирование в реальном времени.
...
- [Задачи, связанные с потоковой передачей в реальном времени](media-services-manage-live-encoder-enabled-channels.md#tasks)

##<a id="scenario"></a>Стандартный сценарий потоковой передачи в реальном времени

Ниже описываются основные этапы создания стандартных приложений, предназначенных для потоковой передачи контента в реальном времени.

1. Подключите видеокамеру к компьютеру. Запустите и настройте локальный динамический кодировщик, который может выдавать **односкоростной** поток по одному из следующих протоколов: RTMP, Smooth Streaming или RTP (MPEG-TS). Подробнее см. в записи блога [Поддержка протокола RTMP службами мультимедиа Azure и динамические кодировщики](http://go.microsoft.com/fwlink/?LinkId=532824).
...
Подробнее см. в записи блога [Поддержка протокола RTMP службами мультимедиа Azure и динамические кодировщики](http://go.microsoft.com/fwlink/?LinkId=532824).
...
- Настоятельно рекомендуется использовать на входе однопрограммный транспортный поток (SPTS). Однако использование звуковых дорожек на нескольких языках также поддерживается.
...
Как только канал начнет принимать данные, вы можете просмотреть поток.

**Примечание**. В настоящее время поток предварительного просмотра может доставляться только в формате фрагментированного MP4 (Smooth Streaming) вне зависимости от указанного входного типа. Для проверки потока Smooth Streaming можно использовать проигрыватель [http://smf.cloudapp.net/healthmonitor](http://smf.cloudapp.net/healthmonitor). Кроме того, для просмотра потока можно воспользоваться проигрывателем, размещенным на портале управления Azure.
...
<table border="1">
<tr><th>Скорость</th><th>Ширина</th><th>Высота</th><th>Макс. кадров/с</th><th>Профиль</th><th>Имя выходного потока</th></tr>
<tr><td>3500</td><td>1280</td><td>720</td><td>30</td><td>Высокий</td><td>Video_1280x720_30fps_3500kbps</td></tr>
<tr><td>2200</td><td>960</td><td>540</td><td>30</td><td>Главная</td><td>Video_960x540_30fps_2200kbps</td></tr>
<tr><td>1350</td><td>704</td><td>396</td><td>30</td><td>Главная</td><td>Video_704x396_30fps_1350kbps</td></tr>
<tr><td>850</td><td>512</td><td>288</td><td>30</td><td>Главная</td><td>Video_512x288_30fps_850kbps</td></tr>
<tr><td>550</td><td>384</td><td>216</td><td>30</td><td>Главная</td><td>Video_384x216_30fps_550kbps</td></tr>
<tr><td>350</td><td>340</td><td>192</td><td>30</td><td>Базовый</td><td>Video_340x192_30fps_350kbps</td></tr>
<tr><td>200</td><td>340</td><td>192</td><td>30</td><td>Базовый</td><td>Video_340x192_30fps_200kbps</td></tr>
</table>
...
необязательный параметр. Дает динамическому кодировщику сигнал переключиться во время рекламной паузы на изображение [листа по умолчанию](media-services-manage-live-encoder-enabled-channels.md#default_slate), чтобы скрыть входящий видеопоток. Звук на это время также выключается. Значение по умолчанию — **false**.
...
###<a id="default_slate"></a>Идентификатор актива листа по умолчанию
...
- Имя файла должно иметь расширение *.jpg.- Изображение должно быть загружено в актив (Asset) в качестве единственного содержащегося в этом активе файла (AssetFile), и этот AssetFile должен быть помечен как основной файл. Актив должен храниться в незашифрованном виде.
...
<table border="1">
<tr><th>Состояние канала</th><th>Индикаторы в пользовательском интерфейсе портала</th><th>Выставление счетов</th></tr>
<tr><td>Запуск</td><td>Запуск</td><td>Нет (переходное состояние)</td></tr>
<tr><td>Выполняется</td><td>Готово (нет запущенных программ)<br/>или<br/>Потоковая передача (запущена по крайней мере одна программа)</td><td>Да</td></tr>
<tr><td>Остановка</td><td>Остановка</td><td>Нет (переходное состояние)</td></tr>
<tr><td>Остановлена</td><td>Остановлена</td><td>Нет</td></tr>
</table>


>[AZURE.NOTE]В текущей предварительной версии запуск канала может занять до 20 минут и более. Сброс канала может занимать до 5 минут.
...
- Запуск канала может занять более 20 минут.
- Поддержка RTP адаптируется к профессиональным вещателям. Просмотрите заметки о RTP в [этом](http://azure.microsoft.com/blog/2015/04/13/an-introduction-to-live-encoding-with-azure-media-services/) блоге.
- Изображения листов должны соответствовать ограничениям, описанным [здесь](media-services-manage-live-encoder-enabled-channels.md#default_slate). При попытке создать канал с листом по умолчанию, разрешение которого больше 1920 x 1080, запрос в конечном итоге завершится ошибкой.


##<a id="tasks"></a>Задачи, связанные с потоковой передачей в реальном времени
...
Общие сведения о конечных точках потоковой передачи и управлении ими см. в разделе [Управление конечными точками потоковой передачи в учетной записи служб мультимедиа](media-services-manage-origins.md).
...
###Создание каналов, выполняющих кодирование односкоростного потока в реальном времени в поток с адаптивной скоростью 
...
Если вы хотите зашифровать ресурс, связанный с программой, с помощью алгоритма AES (на основе 128-битных ключей шифрования) или технологии управления цифровыми правами PlayReady, вам нужно создать ключ содержимого.
...
- [Общие сведения о динамической упаковке](../media-services-dynamic-overview.md)
...
Информацию о масштабировании единиц потоковой передачи см. в разделе [Как масштабировать единицы потоковой передачи](media-services-manage-origins.md#scale_streaming_endpoints.md).
