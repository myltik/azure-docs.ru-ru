---
title: Потоковая трансляция с использованием служб мультимедиа Azure для создания потоков с несколькими скоростями на портале Azure | Microsoft Docs
...
1. Настройка конечных точек потоковой передачи данных.
2. Создание канала, поддерживающего динамического кодирование.
3. Получение URL-адреса приема для динамического кодировщика. Динамический кодировщик будет использовать этот URL-адрес для приема потока в канале.
4. Создание события или программы (и ресурса)
5. Публикация ресурса и получение URL-адресов для потоковой передачи.
...
4. Чтобы указать число единиц потоковой передачи, передвиньте ползунок **Единицы потоковой передачи**.
