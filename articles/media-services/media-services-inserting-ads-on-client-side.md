...
Этот образец использует AdSchedulerPlugin для определения времени отображения рекламы. В этом примере рекламная вставка в процессе показа запланирована к воспроизведению через 5 секунд. Рекламная вставка (группа рекламных объявлений, отображаемых по порядку) задается в VAST-файле, возвращаемом с сервера рекламы. Код URI VAST-файла указывается в элементе <RemoteAdSource>.
...
В этом примере также используется подключаемый модуль AdSchedulerPlugin. Он планирует воспроизведение трех рекламных объявлений: до, во время и после показа видео. Код URI VAST-файла каждого из объявлений указывается в элементе <RemoteAdSource>.
...
В этом примере используется AdSchedulerPlugin для планирования линейной и нелинейной рекламы. Расположение VAST-файла указывается с помощью элемента <RemoteAdSource>.
...
В этом примере используется VmapSchedulerPlugin для планирования рекламных объявлений с помощью VMAP-файла. Код URI VMAP-файла указывается в атрибуте Source элемента <VmapSchedulerPlugin>.
