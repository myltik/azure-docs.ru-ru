<properties 
	pageTitle="Вставка рекламы на стороне клиента" 
...
Этот раздел содержит сведения о том, как вставлять рекламу различных типов на стороне клиента.

Сведения о поддержке субтитров и рекламы в динамических потоковых видео см. в разделе [Поддерживаемые стандарты субтитров и рекламы](https://msdn.microsoft.com/library/azure/dn783466.aspx#caption_ad).
...
Каждая платформа проигрывателя работает по-разному, и каждой из них посвящен отдельный раздел. В этом разделе описываются базовые механизмы, используемые для вставки рекламы. Приложения видеопроигрывателей запрашивают рекламу с сервера рекламы. Сервер рекламы может ответить несколькими способами:
...
###Использование файла Video Ad Service Template (VAST)
...
**<IFrameResource>** описывает ресурс HTML, который может быть отображен в IFrame. **<HTMLResource>** описывает фрагмент HTML-кода, который можно вставить в веб-страницу. **<TrackingEvents>** указывают события отслеживания и URI для запроса при возникновении события. В этом образце отслеживаются события acceptInvitation (принятие приглашения) и collapse (сворачивание). Дополнительные сведения об элементе **<NonLinearAds>** и его дочерних элементах см. в разделе IAB.NET/VAST. Обратите внимание, что элемент **<TrackingEvents>** располагается внутри элемента \*\*<NonLinearAds>\*\*, а не элемента **<NonLinear>**.
...
###Использование файла Digital Video Multiple Ad Playlist (VMAP)
...
1. Процент в формате n%, где n — процент временной шкалы видео, который будет воспроизведен до начала рекламы.
1. Начало/конец — указывает, что реклама должна воспроизводиться до или после отображения видео.
1. Позиция — указывает порядок рекламных пауз, когда время их показа неизвестно, например при прямой потоковой трансляции. Порядок каждой рекламной паузы задается в формате #n, где n — целое число 1 или больше. 1 означает, что рекламу следует воспроизводить при первой же возможности, 2 — при второй возможности и т. д.
...
1. allowMultipleAds — логическое значение, указывающее, могут ли отображаться несколько рекламных объявлений во время паузы
1. followRedirects — необязательное логическое значение, указывающее, следует ли видеопроигрывателю учитывать перенаправления в ходе отклика на рекламу
...
1. breakEnd — отслеживает завершение рекламной паузы;
1. error — отслеживает ошибку, возникшую во время рекламной паузы.
...
Дополнительные сведения об элементе <\*\*TrackingEvents\*\*> и его дочерних элементах см. в разделе http://iab.org/VMAP.pdf

###Использование файла Media Abstract Sequencing Template (MAST)
...
1. **type** — указывает тип условия, событие (event) или свойство (property);
1. **name** — имя свойства или события, которое будет использоваться при вычислении;
1. **value** — значение, по которому будет вычисляться свойство;
1. **operator** — операция, которая будет использоваться при вычислении: EQ (равно), NEQ (не равно), GTR (больше), GEQ (больше или равно), LT (меньше), LEQ (меньше или равно), MOD (остаток от деления)

В **<endConditions>** также содержатся элементы <condition>. Если условие принимает значение true, триггер сбрасывается. В элементе <trigger> также содержится элемент <sources>, который содержит один или несколько элементов <source>. Элементы <source> определяют URI отклика на рекламу и тип отклика. В этом примере для URI задан отклик VAST.
...
##Реализация проигрывателя Windows или Windows Phone 8 с поддержкой рекламы

Платформа проигрывателя Microsoft Media Platform для Windows 8 и Windows Phone 8 содержит набор образцов приложений, в которых демонстрируется реализация приложения видеопроигрывателя с использованием данной платформы. Платформу проигрывателя и образцы можно загрузить со страницы [Платформа проигрывателя для Windows 8 и Windows Phone 8](https://playerframework.codeplex.com/releases/view/105214).

Дополнительные сведения о вставке рекламы см. в разделе [Вставка рекламы в мультимедиа](media-services-inserting-ads-on-client-side.md#insert_ads_into_media).
...
Этот образец использует AdSchedulerPlugin для определения времени отображения рекламы. В этом примере рекламная вставка в процессе показа запланирована к воспроизведению через 5 секунд. Рекламная вставка (группа рекламных объявлений, отображаемых по порядку) задается в VAST-файле, возвращаемом с сервера рекламы. URI VAST-файла указывается в элементе <RemoteAdSource>.
...
Дополнительные сведения о AdSchedulerPlugin см. в разделе [Реклама на платформе проигрывателя в Windows 8 и Windows Phone 8](http://playerframework.codeplex.com/wikipage?title=Advertising&referringTitle=Windows%208%20Player%20Documentation)
...
В этом примере используется подключаемый модуль FreeWheelPlugin, который задает исходный атрибут, указывающий URI файла SmartXML, который определяет содержимое рекламы, а также сведения о ее расписании.
...
Файл ProgrammaticAdPage.xaml.cs создает AdHandlerPlugin, добавляет TimelineMarker для указания времени, когда должна отображаться реклама, а затем добавляет обработчик для события MarkerReached, который загружает RemoteAdSource, указывающий URI VAST-файла, а затем воспроизводит рекламу.
...
##Реализация видеопроигрывателя с поддержкой рекламы для iOS


Платформа проигрывателя Microsoft Media Platform для iOS содержит набор образцов приложений, в которых демонстрируется реализация приложения видеопроигрывателя с использованием данной платформы. Платформу проигрывателя и образцы можно загрузить со страницы [Платформа мультимедиапроигрывателя Azure](https://github.com/Azure/azure-media-player-framework). На странице github содержится ссылка на вики-сайт с дополнительными сведениями о платформе проигрывателя и общими сведениями об образце проигрывателя: [Вики-сайт мультимедиапроигрывателя Azure](https://github.com/Azure/azure-media-player-framework/wiki/How-to-use-Azure-media-player-framework).

Дополнительные сведения о вставке рекламы см. в разделе [Вставка рекламы в мультимедиа](media-services-inserting-ads-on-client-side.md#insert_ads_into_media).

###Планирование рекламы с помощью VMAP
...
###Планирование рекламы с помощью VAST
...
Следующий пример демонстрирует планирование рекламы VAST с поздней привязкой.
   ...
   Следующий пример демонстрирует планирование рекламы VAST с ранней привязкой. //Пример:4 Планирование рекламы VAST с ранней привязкой //Загрузка VAST-файла if (![framework.adResolver downloadManifest:&manifest withURL:[NSURL URLWithString:@"http://portalvhdsq3m25bf47d15c.blob.core.windows.net/vast/PlayerTestVAST.xml"]]) { [self logFrameworkError]; } else { adLinearTime.startTime = 7; adLinearTime.duration = 0;
...
В следующем примере показано, как запланировать многоразовую рекламу во время показа видео. Многоразовая реклама будет отображаться каждый раз при достижении заданной точки на временной шкале видео.
...
[Разработка приложений видеопроигрывателя](media-services-develop-video-players.md) [Общие сведения о платформе мультимедиапроигрывателя Azure для iOS](https://channel9.msdn.com/Series/Windows-Azure-Media-Services-Tutorials/An-introduction-to-Azure-Media-Player-Framework-for-IOS)
