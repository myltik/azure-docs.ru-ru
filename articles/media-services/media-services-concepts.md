...
С помощью [указателей](https://msdn.microsoft.com/library/azure/hh974308.aspx) обозначаются точки входа для доступа к файлам, содержащимся в ресурсе. Политика доступа используется для определения разрешений и длительности доступа клиента к определенному активу. Указатели могут иметь множество или одну связь с политикой доступа таким образом, чтобы разные указатели могли обеспечивать разное время запуска и типы подключения к различным клиентам с использованием одних параметров разрешения и длительности. Однако из-за ограничений общей политики доступа, установленных службами хранилища Azure, невозможно одновременно связать с определенным активом более пяти указателей. 
