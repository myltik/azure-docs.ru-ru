...
Эта статья полезна для клиентов, имеющих **конечную точку потоковой передачи** типа **Премиум**. Дополнительные сведения о типах конечных точек потоковой передачи и конфигурации сети CDN см. в статье [Управление конечными точками потоковой передачи с помощью портала Azure](media-services-portal-manage-streaming-endpoints.md).
 
При использовании типа **Премиум** вы получаете 1 единицу потоковой передачи по умолчанию. При необходимости масштабирования конечной точки потоковой передачи следуйте инструкциям в этой статье.
...
Чтобы создать единицы потоковой передачи и изменить их число, сделайте следующее:
