...
В этом разделе показано, как использовать классический портал Azure для управления содержимым в учетной записи служб мультимедиа.
...
4. В диалоговом окне **Обработка** выберите обработчик **кодировщика мультимедиа Azure**.
5. Выберите одну из **конфигураций кодирования**.

![Process2][process2]


В разделе [Предустановки задачи для кодировщика мультимедиа Azure](https://msdn.microsoft.com/library/azure/dn619392.aspx) описывается значение каждой предустановки в категориях **Предустановки для адаптивной потоковой передачи (динамическое упаковывание)**, **Предустановки для поэтапного скачивания**, **Устаревшие предустановки для адаптивной потоковой передачи**.
...
Служба лицензий PlayReady служб мультимедиа включена по умолчанию. Чтобы указать другую службу, из которой клиенты могут получить лицензию на воспроизведение зашифрованного контента PlayReady, используйте API-интерфейсы REST или пакета SDK .NET для служб мультимедиа. Для получения дополнительной информации ознакомьтесь с разделом [Использование статического шифрования для защиты контента]() и установите свойство **licenseAcquisitionUrl** в предустановке Media Encryptor. Также можно использовать динамическое шифрование и установить свойство **PlayReadyLicenseAcquisitionUrl**, как описано в разделе [Использование динамического шифрования PlayReady и службы доставки лицензий](http://go.microsoft.com/fwlink/?LinkId=507720). + **Воспроизведение на ПК или компьютерах Mac (через Flash или Silverlight)**. Эта предустановка позволяет получить ресурс-контейнер Smooth Streaming со следующими характеристиками: стереозвук 44,1 кГц с разрядностью 16 бит на выборку, постоянной скоростью 96 Кбит/с и кодировкой AAC, а также видео с 6 постоянными скоростями от 400 Кбит/с до 3400 Кбит/с, в формате 720p с использованием основного профиля H.264 и двумя дополнительными группами GOP. + **Воспроизведение через HTML5 (IE, Chrome, Safari)**. Эта предустановка позволяет получить отдельный MP4-файл со следующими характеристиками: стереозвук 44,1 кГц с разрядностью 16 бит на выборку, постоянной скоростью 128 Кбит/с и кодировкой AAC, а также видео с постоянной скоростью 4500 Кбит/с в формате 720p с использованием основного профиля H.264. + **Воспроизведение на устройствах iOS, а также ПК и компьютерах Mac**. Эта предустановка позволяет получить актив с теми же характеристиками, что и у актива с бесперебойной потоковой передачей (описан выше), но в формате, который может использоваться для доставки потоков Apple HLS на устройства iOS.
