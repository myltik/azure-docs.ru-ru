...
Чтобы зарегистрировать и настроить приложение Azure Active Directory (AAD), а также получить ключи для вызова конечной точки API REST служб мультимедиа Azure, ознакомьтесь со статьей [Приступая к работе с аутентификацией Azure AD с помощью портала Azure](media-services-portal-get-started-with-aad.md).
...
Маркер обновления предназначен не для обновления маркера доступа. Он позволяет обойти аутентификацию пользователя и все равно получить действительный маркер доступа, когда срок предыдущего маркера истечет. Более понятным названием маркера обновления может быть что-то вроде "маркер обхода повторного входа пользователя".

При использовании последовательности предоставления авторизации OAuth 2.0 (имя пользователя и пароль, предоставляемые от имени пользователя) маркер обновления помогает продлить срок действия маркера доступа без необходимости вмешательства пользователя. Тем не менее в последовательности предоставления учетных данных клиента OAuth 2.0, рассматриваемой в этой статье, клиент действует от собственного имени. Никакое вмешательство пользователя не требуется, и серверу авторизации не требуется выдавать вам маркер обновления. Если выполнить отладку метода **GetUrlEncodedJWT** то вы заметите, что ответ конечной точки выдачи маркеров содержит маркер доступа, но не маркер обновления.
