...
>
> Для расчета затрат используется наибольшее число единиц, указанных для 24-часового периода. Дополнительные сведения о ценах см. на странице [Сведения о ценах на службы мультимедиа](http://go.microsoft.com/fwlink/?LinkId=275107).
...
Метод **UploadFile**, определенный ниже, вызывает метод **CreateFromFile** (этот метод определен в расширениях пакета SDK для .NET). **CreateFromFile** создает новый ресурс-контейнер, в который отправляется указанный исходный файл.
...
Метод **CreateFromFile** также позволяет указывать метод обратного вызова, чтобы сообщать о ходе отправки файла.

В следующем примере для параметров ресурса-контейнера используется значение **None**.
