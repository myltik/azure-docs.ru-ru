...
В этом кратком руководстве рассматриваются следующие задачи.
...
9.  Воспроизведение содержимого.


##Создание учетной записи служб мультимедиа с помощью портала

1. На портале Azure последовательно выберите **Создать**, **Служба мультимедиа**, **Быстрое создание**.
...
2. Используйте пакет NuGet [windowsazure.mediaservices.extensions](https://www.nuget.org/packages/windowsazure.mediaservices.extensions), чтобы установить **расширения пакета SDK для .NET служб мультимедиа Azure**. Расширения пакета SDK .NET служб мультимедиа — это набор методов расширения и вспомогательных функций, которые упростят код и разработку с помощью служб мультимедиа. При установке этого пакета также устанавливается **пакет SDK служб мультимедиа для .NET** и добавляются все остальные необходимые зависимости.
...
5. Замените существующие операторы **using** в начале файла Program.cs следующим кодом.
...
В службах мультимедиа цифровые файлы отправляются (или принимаются) в актив. Сущность **Asset** может содержать видео, звуковые файлы, изображения, коллекции эскизов, текстовые каналы и файлы скрытых субтитров (а также метаданные этих файлов). После отправки этих файлов содержимое сохраняется в безопасном расположении в облаке для дальнейшей обработки и потоковой передачи. Файлы в ресурсе называются **файлами ресурса**.

Метод **UploadFile**, определенный ниже, вызывает **CreateFromFile** (этот метод определен в расширениях пакета SDK для .NET). Метод **CreateFromFile** создает новый ресурс-контейнер, в который отправляется указанный исходный файл.

Метод **CreateFromFile** принимает параметры **AssetCreationOptions**, которые позволяют указать один из таких параметров создания ресурса-контейнера:
...
Метод **CreateFromFile** позволяет также указывать метод обратного вызова, чтобы сообщать о ходе отправки файла.

В следующем примере в качестве параметров ресурса-контейнера указывается **None**.
...
В следующем коде показано, как отправить задание кодирования. Задание содержит одну задачу, которая указывает необходимость перекодировать мезонинный файл в набор MP4-файлов с переменной скоростью с помощью **кодировщика мультимедиа Azure**. Код отправляет задание и ждет его выполнения.
