...
Скачать и запустить пример можно [здесь](http://azure.microsoft.com/documentation/samples/media-services-dotnet-on-demand-encoding-with-media-encoder-standard/).
...
Если вы используете службы мультимедиа с помощью .NET, для большинства задач программирования этих служб следует использовать класс **CloudMediaContext**. Это могут быть такие задачи: подключение к учетной записи служб мультимедиа, а также создание, обновление и удаление различных объектов и получение доступа к этим объектам (ресурсам-контейнерам, файлам ресурсов-контейнеров, заданиям, политикам доступа, указателям и т. д.).
...
В следующем примере мы используем значение **None** для параметров ресурса-контейнера.
