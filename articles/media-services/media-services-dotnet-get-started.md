<properties
	pageTitle="Краткое руководство. Доставка видео по запросу с помощью пакета SDK для .NET"
...
# Краткое руководство. Доставка видео по запросу с помощью пакета SDK для .NET
...
- Операционные системы: Windows 7, Windows 2008 R2, Windows 8 или более поздние.
...
Если вы используете службы мультимедиа с помощью .NET, то для большинства задач программирования этих служб вам нужно использовать класс **CloudMediaContext**. Это могут быть такие задачи: подключение к учетной записи служб мультимедиа, а также создание, обновление и удаление разных объектов и получение доступа к этим объектам (активам, файлам активов, заданиям, политикам доступа, указателям).
...
В службах мультимедиа цифровые файлы отправляются (или принимаются) в актив. Сущность **Asset** может содержать видео, аудио, изображения, коллекции эскизов, текстовые каналы и файлы скрытых субтитров (и метаданные этих файлов). После отправки этих файлов содержимое сохраняется в безопасном расположении в облаке для дальнейшей обработки и потоковой передачи. Файлы в ресурсе называются **файлами ресурса**.

Метод **UploadFile**, определенный ниже, вызывает **CreateFromFile** (этот объект определен в расширениях пакета SDK .NET). **CreateFromFile** создает новый актив, в который отправляется указанный исходный файл.

Метод **CreateFromFile** получает параметры **AssetCreationOptions**, которые позволяют указать один из таких параметров создания актива:
...
Метод **CreateFromFile** позволяет также указать метод обратного вызова, чтобы сообщать о ходе отправки файла.
...
В следующем коде показано, как отправить задание кодирования. Задание содержит одну задачу, которая указывает необходимость перекодировать с помощью **кодировщика мультимедиа Azure** мезонинный файл в набор MP4-файлов с адаптивным битрейтом. Код отправляет задание и ждет его выполнения.
...
Следующий метод **ConfigureClearAssetDeliveryPolicy** указывает, что не следует применять динамическое шифрование и что следует осуществлять доставку потока с помощью любого из следующих протоколов: MPEG DASH, HLS и Smooth Streaming.
