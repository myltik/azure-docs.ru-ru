...
1. **Ключ SSH и пароль ИЛИ только пароль** - виртуальная машина была подготовлена с использованием сертификата (файл `CER`) или ключа SSH и пароля либо только имени пользователя и пароля. В этом случае `sudo` запросит пароль пользователя перед выполнением команды.
...
В этом случае пользователю **не будет** предложено ввести пароль. После нажатия клавиши `< ВВОД >` `sudo` выполнит команду с привилегиями `root`.
