...
---
# <a name="connect-to-azure-storage-in-your-xamarinforms-app"></a>Подключение к службе хранилища Azure в приложении Xamarin.Forms
...
3. Перейдите к серверной части мобильного приложения. Выберите **Все параметры** -> **Параметры приложения** -> **Строки подключения**, создайте ключ с именем `MS_AzureStorageAccountConnectionString` и используйте значение, скопированное в учетной записи хранения. В качестве типа ключа выберите **Настраиваемый** .

## <a name="add-a-storage-controller-to-the-server"></a>Добавление контроллера хранилища в сервер
...
### <a name="a-nameadd-controller-codeaadd-a-storage-controller-to-your-server-project"></a><a name="add-controller-code"></a>Добавление контроллера хранилища в серверный проект
...
2. В Visual Studio откройте серверный проект .NET. Щелкните правой кнопкой мыши папку **Контроллеры**, а затем выберите **Добавить** -> **Контроллер** -> **Контроллер веб-API 2 — пустой**. Присвойте контроллеру имя `TodoItemStorageController`.
...
### <a name="a-nameroutes-registeredaroutes-registered-by-the-storage-controller"></a><a name="routes-registered"></a>Маршруты, регистрируемые с помощью контроллера хранилища
...
### <a name="a-nameclient-communicationaclient-and-server-communication"></a><a name="client-communication"></a>Взаимодействие клиента и сервера
...
## <a name="update-your-client-app-to-add-image-support"></a>Обновление клиентского приложения для поддержки изображений
...
### <a name="a-nameadd-nugetaadd-nuget-packages"></a><a name="add-nuget"></a>Добавление пакетов NuGet
...
### <a name="a-nameadd-iplatformaadd-iplatform-interface"></a><a name="add-iplatform"></a>Добавление интерфейса IPlatform
...
### <a name="a-nameadd-filehelperaadd-filehelper-class"></a><a name="add-filehelper"></a>Добавление класса FileHelper
...
### <a name="a-namefile-sync-handlera-add-a-file-sync-handler"></a><a name="file-sync-handler"></a> Добавление обработчика синхронизации файлов
...
### <a name="a-nameupdate-todoitemmanageraupdate-todoitemmanager"></a><a name="update-todoitemmanager"></a>Обновление TodoItemManager
...
### <a name="a-nameadd-details-viewaadd-a-details-view"></a><a name="add-details-view"></a>Добавление подробного представления
...
4. Щелкните правой кнопкой мыши проект переносимой библиотеки и выберите **Добавить** -> **Новый элемент** -> **Кроссплатформенный** -> **Forms Xaml Page** (Страница форм Xaml). Присвойте представлению имя `TodoItemDetailsView`.
...
### <a name="a-nameupdate-main-viewaupdate-the-main-view"></a><a name="update-main-view"></a>Обновление главного представления
Обновите главное представление, чтобы при выборе элемента списка задач открывалось подробное представление.

В файле **TodoList.xaml.cs** замените реализацию метода `OnSelected` следующим кодом:
...
### <a name="a-nameupdate-androidaupdate-the-android-project"></a><a name="update-android"></a>Обновление проекта Android
...
### <a name="a-nameupdate-iosaupdate-the-ios-project"></a><a name="update-ios"></a>Обновление проекта iOS
...
### <a name="a-nameupdate-windowsaupdate-the-windows-project"></a><a name="update-windows"></a>Обновление проекта Windows
1. Установите расширение Visual Studio [SQLite для Windows 8.1](http://go.microsoft.com/fwlink/?LinkID=716919). 
   Дополнительные сведения см. в руководстве [Включение автономной синхронизации для приложения для Windows](app-service-mobile-windows-store-dotnet-get-started-offline-data.md). 
...
* Создайте учетную запись хранения и добавьте строку подключения в серверную часть мобильного приложения. Ключ к службе хранилища Azure есть только в серверной части. Мобильный клиент запрашивает маркер SAS при каждой попытке доступа к службе хранилища Azure. Дополнительные сведения о маркерах SAS в службе хранилища Azure см. в статье [Использование подписанных URL-адресов (SAS)].
...
* Пакет SDK для клиента мобильных приложений Azure не хранит данные файлов. Вместо этого пакет вызывает ваш класс `IFileSyncHandler`, который определяет, какие файлы и каким образом будут храниться на локальном устройстве. Следующий код предназначен для регистрации обработчика синхронизации:
      ...
      + `IFileSyncHandler.GetDataSource` вызывается, когда пакету SDK для клиента мобильных приложений Azure нужны данные файла (например, в процессе отправки). Это позволяет управлять тем, как (и нужно ли) хранить файлы на локальном устройстве, и возвращать эти сведения, когда необходимо.
  
      + `IFileSyncHandler.ProcessFileSynchronizationAction` вызывается в потоке синхронизации файлов. Чтобы можно было определить, что должно происходить в приложении при возникновении такого события (например, автоматическое скачивание файла при его создании или обновлении либо удаление этого файла на локальном устройстве при его удалении на сервере), можно указать ссылку на файл и значение перечисления FileSynchronizationAction.
...
[Использование подписанных URL-адресов (SAS)]: ../storage/storage-dotnet-shared-access-signature-part-1.md
