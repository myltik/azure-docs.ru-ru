...
В этом учебнике вы подключите мобильное приложение к корпоративному решению SaaS. Вы будете обновлять приложение из раздела [Проверка подлинности приложения с помощью единого входа библиотеки проверки подлинности Active Directory], чтобы создавать документ Word в SharePoint Online при каждом добавлении нового элемента TodoItem.
...
* Введите SP_Authority в качестве конечной точки центра для своего клиента AAD. Это значение должно совпадать со значением центра, используемым для вашего клиентского приложения. Значение будет представлено в формате `https://login.windows.net/contoso.onmicrosoft.com`.

* Введите SP_ClientSecret в качестве значения секрета клиента, которое вы получили ранее.

* Введите SP_SharePointURL в качестве URL-адреса для своего сайта SharePoint. Это значение должно быть представлено в формате `https://contoso-my.sharepoint.com`.
