...
Атрибут [JsonPropertyAttribute] используется для определения сопоставления *PropertyName* между типом клиента и таблицей.

Чтобы научиться создавать таблицы в серверной части мобильных приложений, обратитесь к [статье, посвященной пакету SDK для сервера .NET](app-service-mobile-dotnet-backend-how-to-use-server-sdk.md#define-table-controller) или [статье, посвященной пакету SDK для сервера Node.js](app-service-mobile-node-backend-how-to-use-server-sdk.md#howto-dynamicschema). При создании серверной части мобильного приложения на портале Azure с помощью краткого руководства можно использовать параметр **Простые таблицы** на [портале Azure].

###Практическое руководство. Установка пакета SDK для управляемого клиента

Используйте один из следующих методов установки пакета SDK для управляемого клиента для мобильных приложений с сайта [NuGet](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Client/).
...
Символы для пространства имен Microsoft.Azure.Mobile доступны в [SymbolSource]. Ознакомьтесь с [инструкциями по SymbolSource], чтобы интегрировать SymbolSource с Visual Studio.
...
Если при вставке в `todoItem` не добавлено значение уникального пользовательского идентификатора, то GUID создается сервером. Созданный идентификатор можно получить, проверив объект после возвращения вызова.
...
###<a name="clientflow"></a>Управляемая клиентом аутентификация
...
####<a name="client-livesdk"></a>Единый вход с использованием учетной записи Майкрософт и Live SDK
...
Дополнительные сведения см. в [документации по Windows Live SDK].

###<a name="serverflow"></a>Управляемая сервером аутентификация
...
При использовании серверного потока служба приложений Azure управляет потоком аутентификации OAuth, отображая страницу входа выбранного поставщика. После получения ответа от поставщика удостоверений служба приложений Azure создает маркер аутентификации службы приложений. Метод [LoginAsync] возвращает ответ [MobileServiceUser], в котором содержится как [userId] авторизованного пользователя, так и [MobileServiceAuthenticationToken] в качестве веб-маркера JSON (JWT). Этот маркер можно поместить в кэш и повторно использовать, пока не истечет срок его действия. Дополнительные сведения см. в разделе [Кэширование маркера проверки подлинности](#caching).
...
Приложения Xamarin используют интерфейсы API [Xamarin.Auth] для безопасного хранения учетных данных в объекте **Account**. Пример использования этих API см. в файле кода [AuthStore.cs], который включен в [пример совместного использования фотографий ContosoMoments].
...
При отправке push-уведомлений в WNS необходимо получить SID пакета Магазина Windows (см. ниже). Дополнительные сведения о приложениях Windows, включая способ регистрации для выполнения регистраций шаблонов, см. в разделе [Добавление push-уведомлений в приложение].

Запрос тегов от клиента не поддерживается. Запросы тегов автоматически отбрасываются из регистрации. Если вы хотите зарегистрировать устройство с тегами, создайте пользовательский интерфейс API, который использует API центров уведомлений для выполнения регистрации от вашего имени. [Используйте настраиваемый API](#customapi) вместо метода `RegisterNativeAsync()`.

###<a name="package-sid"></a>Практическое руководство. Получение SID пакета Магазина Windows

Для включения push-уведомлений для приложений Магазина Windows необходим SID пакета. Для получения ИД безопасности пакета зарегистрируйте приложение в Магазине Windows.
...
Во многих случаях ИД безопасности пакета рассматривается как универсальный код ресурса (URI). В этом случае необходимо использовать _ms-app://_ в качестве схемы. Запишите версию ИД безопасности пакета, получаемую путем сцепления этого значения в качестве префикса.
...
[документации по Windows Live SDK]: https://msdn.microsoft.com/ru-RU/library/bb404787.aspx
...
[пакета SDK для серверной части .NET]: app-service-mobile-dotnet-backend-how-to-use-server-sdk.md
[пакета SDK для серверной части Node.js]: app-service-mobile-node-backend-how-to-use-server-sdk.md
...
[инструкциями по SymbolSource]: http://www.symbolsource.org/Public/Wiki/Using
