...
Чтобы научиться создавать новые таблицы в серверной части мобильных приложений, обратитесь к [статье, посвященной пакету SDK для серверной части .NET](app-service-mobile-dotnet-backend-how-to-use-server-sdk.md#define-table-controller) или [статье, посвященной пакету SDK для серверной части Node.js](app-service-mobile-node-backend-how-to-use-server-sdk.md#howto-dynamicschema). При создании серверной части мобильного приложения на портале Azure с помощью краткого руководства можно использовать **Простые таблицы** в [портале Azure].

###Практическое руководство. Установка пакета SDK для управляемого клиента

Используйте один из следующих методов для установки пакета SDK для управляемого клиента для мобильных приложений с [NuGet](https://www.nuget.org/packages/Microsoft.Azure.Mobile.Client/):
...
Символы для пространства имен Microsoft.Azure.Mobile доступны на [SymbolSource]. См. [инструкции по SymbolSource], чтобы интегрировать SymbolSource с Visual Studio.
...
Обратите внимание, что при создании запроса на удаление необходимо указать идентификатор. Другие свойства не передаются в службу или игнорируются ею. В результате вызова функции `DeleteAsync` обычно возвращается значение `null`. Идентификатор для передачи можно получить в результате вызова метода `InsertAsync`. При попытке удалить элемент без указания поля `id` выдается исключение `MobileServiceInvalidOperationException`.
...
По умолчанию мобильные приложения Azure выдают не больше 50 элементов на запрос. Это значение можно изменить, увеличив максимальный размер страницы на сервере и размер запрашиваемой страницы на стороне клиента. Чтобы увеличить размер запрашиваемой страницы, используйте перезагрузку `PullAsync`, которая позволит указать `PullOptions`:
...
Например, если установить на сервере значение размера страницы, равное 100 элементам или больше, каждый запрос будет возвращать до 100 элементов.
...
1. Настройте серверную часть мобильного приложения для входа с помощью AAD, следуя инструкциям учебника [Настройка приложения службы приложений для использования службы входа Azure Active Directory]. Обязательно выполните дополнительный этап регистрации собственного клиентского приложения.
...
3. Добавьте в приложение код, соответствующий используемой платформе. Код находится ниже. В каждом коде выполните следующие замены.
	...
	* Замените текст **INSERT-REDIRECT-URI-HERE** HTTPS-адресом конечной точки сайта для _проверки подлинности/авторизации/подтверждения_. Это значение должно быть аналогично https://contoso.azurewebsites.net/.auth/login/done_.
...
Приложения из Магазина Windows и UWP могут использовать [PasswordVault] для сохранения текущего маркера аутентификации после успешного входа в систему:
...
Еще один пример работы с ошибками можно найти в разделе [Mobile Apps Files Sample] \(Файлы примеров для мобильных приложений). Пример [LoggingHandler] представляет собой делегат-обработчик ведения журналов (см. ниже) запросов к серверной части приложения. Это позволяет удобно отлаживать приложения Xamarin вместо использования Fiddler.
...
[инструкции по SymbolSource]: http://www.symbolsource.org/Public/Wiki/Using
