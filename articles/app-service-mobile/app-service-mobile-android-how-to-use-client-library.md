...
2. Добавьте следующий код в файл *build.gradle* уровня **Module app** внутри тега *dependencies*.
    ...
    Сейчас последней версией является версия 3.1.0. Поддерживаемые версии см. [здесь][14].
...
См. дополнительные сведения о создании дополнительных таблиц в серверной части мобильных приложений, включая сведения об [определении контроллера таблиц][15] (серверная часть .NET) и [определении таблиц с помощью динамической схемы][16] (серверная часть Node.js). Для серверной части Node.js можно использовать параметр **Простые таблицы** на [портале Azure].
...
См. полный пример [кэширования маркеров аутентификации][7].
...
Подробные сведения см. в [этом обзоре][6], где объясняется, как центры уведомлений Microsoft Azure поддерживают разные виды push-уведомлений.  В [этом руководстве][5] показано, как настроить отправку push-уведомлений при каждой вставке записи.
...
Сопоставьте имя клиентской таблицы с именем таблицы разных мобильных служб, используя переопределение метода [getTable()][4].
