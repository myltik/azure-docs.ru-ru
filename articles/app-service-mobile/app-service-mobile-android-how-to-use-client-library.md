...
Навыки [быстрого запуска мобильных приложений](app-service-mobile-android-get-started.md), необходимые для установки Android Studio (вам нужно будет настроить учетную запись и создать серверную часть мобильного приложения). Если вы умеете это делать, вы можете пропустить оставшуюся часть этого раздела.
...
2. Добавьте следующий код в файл **build.gradle** уровня *Module app* внутри тега *dependencies*:
	...
	Сейчас последней версией является версия 3.0. Поддерживаемые версии перечислены [здесь](http://go.microsoft.com/fwlink/p/?LinkID=717034).
...
Самый простой способ запросить или изменить данные в серверной части — воспользоваться *типизированной моделью программирования*, так как Java является строго типизированным языком (позже мы расскажем о *нетипизированной* модели). Эта модель обеспечивает простую сериализацию и десериализацию в формате JSON с помощью библиотеки [gson](http://go.microsoft.com/fwlink/p/?LinkId=290801) при передаче данных между объектами клиента и таблицами серверной части. В итоге разработчику не нужно ничего делать, платформа сама занимается обработкой.

Для доступа к таблице, во-первых, создайте объект [MobileServiceTable](http://go.microsoft.com/fwlink/p/?LinkId=296835), вызвав метод **getTable** объекта [**MobileServiceClient**](http://dl.windowsazure.com/androiddocs/com/microsoft/windowsazure/mobileservices/MobileServiceClient.html). У этого метода две перегрузки.
...
В нашем примере кода мы возвращаем данные из таблицы *ToDoItem* мобильных приложений SQL Azure в массив. Это очень распространенный сценарий для приложений, работающих с данными: запросы к базе данных обычно возвращают набор строк, которые клиент получает в списке или массиве. В этом примере массив является источником данных.
...
Следующий код возвращает все элементы из таблицы *ToDoItem*, упорядоченные по полю *text*. *mToDoTable* — это ссылка на таблицу серверной части, которую вы создали ранее.
...
Первый пример показывает, как выбрать 5 верхних элементов из таблицы. Следующий запрос возвращает элементы из таблицы *ToDoItem*. *mToDoTable* — это ссылка на таблицу серверной части, которую вы создали ранее.
...
В следующем фрагменте кода показано, как получить все элементы из таблицы *ToDoItems* с отображением только полей *complete* и *text*. *mToDoTable* — это ссылка на таблицу серверной части, которую мы создали ранее.
...
Мобильные службы поддерживают [проверку подлинности и авторизацию пользователей](mobile-services-android-get-started-users.md) с помощью разных внешних поставщиков удостоверений: Facebook, Google, учетной записи Майкрософт, Twitter и Azure Active Directory. Можно задать разрешения таблиц, чтобы предоставить доступ к определенным операциям только пользователям, прошедшим проверку подлинности. Удостоверения пользователей, прошедших проверку подлинности, также можно применять для реализации правил авторизации в серверной части.
