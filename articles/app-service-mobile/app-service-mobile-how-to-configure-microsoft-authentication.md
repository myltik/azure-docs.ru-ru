...
6. В разделе "URI перенаправления" укажите конечную точку для вашего приложения, а затем щелкните **Сохранить**.
   ...
   > [!NOTE]
   > Ваш универсальный код ресурса (URI) перенаправления — это URL-адрес приложения, дополненный путем */.auth/login/microsoftaccount/callback*. Например, `https://contoso.azurewebsites.net/.auth/login/microsoftaccount/callback`. Убедитесь, что используете схему HTTPS.
   > 
   > 
