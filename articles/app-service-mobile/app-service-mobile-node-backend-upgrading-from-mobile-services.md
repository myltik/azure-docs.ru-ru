...
- Redis можно использовать с собственными модулями, например [hiredis](https://www.npmjs.com/package/hiredis). А поскольку служба приложений будет устанавливать пакеты NPM во время развертывания автоматически, нет необходимости включать двоичные файлы в пакеты развертывания.
...
>[AZURE.TIP]Прежде чем начинать обновление, рекомендуем полностью ознакомиться с этой статьей. Запишите все используемые функции, которые вызываются ниже.
...
4. Удаление исходного перенесенного приложения мобильной службы (необязательно).
...
Каждая серверная часть на Node.js для мобильного приложения службы приложений Azure запускается как приложение ExpressJS. Вот как создать простое приложение [Express](http://expressjs.com/en/index.html):
...
Чтобы начать перемещение бизнес-логики из мобильной службы в мобильные приложения, нужно создать файл с именем таблицы (с расширением JS) в каталоге `./tables`. Полный пример таблицы мобильного приложения можно просмотреть на портале [GitHub](https://github.com/Azure/azure-mobile-apps-node/blob/master/samples/todo/tables/TodoItem.js). Самая простая версия приведена ниже.
...
Просматривая код, вы увидите большую часть параметров функции.
...
Чтобы ограничить доступ к таблице, его можно установить на уровне таблицы с помощью параметра `table.access = 'authenticated';`. Полный пример см. на портале [GitHub](https://github.com/Azure/azure-mobile-apps-node/blob/master/samples/authentication/tables/TodoItem.js).

Доступ к данным удостоверения пользователя можно получить с помощью метода `user.getIdentity`, описанного [здесь](http://azure.github.io/azure-mobile-apps-node/module-azure-mobile-apps_auth_user.html#~getIdentity).
...
[пакет SDK для сервера мобильных приложений]: https://www.npmjs.com/package/azure-mobile-apps
