...
4. В левой панели навигации щелкните **API & Auth**. Затем щелкните **Consent screen**. Укажите **Email address** и введите **Product name**. Нажмите кнопку **Save**.

5. Также в разделе **API & Auth** выберите **API** и включите **Google+ API**. Он находится в разделе **Social APIs**. Можно также просто выполнить поиск **Google+ API**.

6. В том же разделе **API & Auth** выберите пункт **Credentials**, а затем выберите команду **Create new client ID**.

7. Выберите **Web application**. Вставьте **URL-адрес**, скопированный ранее, в поле **Authorized JavaScript Origins** и замените созданный URL-адрес в поле **Authorized Redirect URI** значением **URI перенаправления** мобильного приложения, скопированным ранее. Этот универсальный код ресурса (URI) — шлюз мобильного приложения, дополненный путем _/signin-twitter_. Пример: `https://contosogateway.azurewebsites.net/signin-google`. Убедитесь, что используете схему HTTPS. Затем щелкните **Create client ID**.
