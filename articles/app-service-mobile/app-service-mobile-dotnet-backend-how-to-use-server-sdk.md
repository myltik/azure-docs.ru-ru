...
Вернитесь в колонку _Приступая к работе_ и в разделе **Создание API таблицы** выберите в поле **Язык серверной части** значение **C#**. Щелкните **Загрузить**, извлеките сжатые файлы проекта на локальный компьютер и откройте решение в Visual Studio.
...
- Пакет [Microsoft.Azure.Mobile.Server.Login] обеспечивает поддержку пользовательской системы проверки подлинности с использованием метода AppServiceLoginHandler.CreateToken(). Это статический метод. Его не нужно включать в конфигурации.
...
##<a name="push-user"></a>Практическое руководство. Отправка push-уведомлений пользователю, прошедшему проверку подлинности.
