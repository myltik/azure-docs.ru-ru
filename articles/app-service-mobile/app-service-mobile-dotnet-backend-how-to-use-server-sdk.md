...
Для этого выполните описанные ниже действия либо инструкции из руководства [Создание мобильного приложения](app-service-mobile-ios-get-started.md), где описано одновременное создание клиента и сервера. В этом учебнике представлена упрощенная версия этих инструкций, что лучше всего подходит для проектов, предназначенных для подтверждения концепции. В учебнике описывается только создание серверной части Node.js.

[AZURE.INCLUDE [app-service-mobile-dotnet-backend-create-new-service-classic](../../includes/app-service-mobile-dotnet-backend-create-new-service-classic.md)]

Вернитесь в колонку _Приступая к работе_ и в разделе **Создание API таблицы** выберите в поле **Язык серверной части** значение **C#**. Щелкните **Скачать**, извлеките сжатые файлы проекта на локальный компьютер и откройте решение в Visual Studio.
...
Обратите внимание, что `MapApiControllers` обозначает только контроллеры с атрибутом `[MobileAppController]`. Чтобы сопоставить другие контроллеры, используйте метод [MapHttpAttributeRoutes].
...
- Пакет [Microsoft.Azure.Mobile.Server.Login] обеспечивает поддержку пользовательской аутентификации с помощью метода AppServiceLoginHandler.CreateToken(). Это статический метод. Его не нужно включать в конфигурации.
...
### Практическое руководство: изменение размера постраничного представления таблиц

По умолчанию мобильные приложения Azure выдают по 50 записей на запрос. Это гарантирует, что клиент не связывает ни поток пользовательского интерфейса, ни сервер надолго, обеспечивая таким образом удобство работы пользователей. Чтобы изменить размер постраничного представления таблиц, необходимо увеличить допустимый размер запроса на стороне сервера и размер страницы на стороне клиента. Чтобы увеличить размер постраничного представления, откорректируйте контроллер таблиц с помощью этой строки:

    [EnableQuery(PageSize = 500)]

Значение параметра PageSize должно быть больше или равно размеру, который будет запрошен клиентом. Сведения об изменении размера страницы для изменения клиента см. в методических указаниях по соответствующему клиенту.
...
Обратите внимание, что этот контроллер ApiController **не должен** использовать атрибут `[MobileAppController]`, так как это приведет к сбою клиентских запросов на вход. Для атрибута `[MobileAppController]` требуется заголовок запроса [ZUMO-API-VERSION](app-service-mobile-client-and-server-versioning.md), а клиентский пакет SDK для маршрутов входа **не** отправляет этот заголовок.
...
Вы также можете упростить код клиента, воспользовавшись методом `loginAsync()` (имя может отличаться для разных платформ) вместо отправки HTTP-запроса POST вручную. В этом случае используется перегруженный метод, который принимает дополнительный параметр маркера, соответствующий объекту утверждения, который был бы отправлен в запросе POST. В качестве поставщика в этом случае нужно указать любое имя по своему выбору. На сервере действие входа должно находиться в расположении _/.auth/login/{пользовательское\_имя\_поставщика}_ (путь, который включает это имя). Чтобы разместить свой контроллер по этому пути, добавьте маршрут к HttpConfiguration перед применением конфигурации MobileAppConfiguration.
...
SID является производным от идентификатора пользователя, предоставленного поставщиком, и является статическим для данного пользователя и поставщика входа в систему. Когда пользователь осуществляет анонимный доступ к конечной точке, свойство User возвращает значение NULL.
...
В зависимости от конкретного сценария вам также может потребоваться создать таблицы пользователей или ролей для отслеживания более подробных сведений авторизации пользователей, например конечных точек, к которым данному пользователю разрешен доступ.
...
Центры уведомлений позволяют отправлять целевые уведомления в определенные регистрации с помощью тегов. Тег, который создается автоматически, — это идентификатор установки, который относится к экземпляру приложения на данном устройстве. Регистрация с помощью идентификатора установки называется также *установкой*. Идентификатор установки используется для управления установкой, например при добавлении тегов. Доступ к идентификатору установки можно получить из свойства **installationId** в **MobileServiceClient**.

В примере ниже показано, как использовать идентификатор установки для добавления тега в конкретную установку в центрах уведомлений.
...
Обратите внимание, что все теги, предоставленные клиентом во время регистрации push-уведомлений, игнорируются серверной частью при создании установки. Чтобы включить клиент для добавления тегов в установку, необходимо создать настраиваемый API, который добавляет теги, с помощью шаблона выше. Пример контроллера настраиваемого API, который позволяет клиентам добавлять теги в установку, см. в разделе [Добавляемые клиентом теги push-уведомлений](https://github.com/Azure-Samples/app-service-mobile-dotnet-backend-quickstart/blob/master/README.md#client-added-push-notification-tags) в примере краткого руководства по мобильным приложениям службы приложений для серверной части .NET.
...
При регистрации для работы с push-уведомлениями на клиенте, прошедшем проверку подлинности, убедитесь, что проверка подлинности завершена, и только после этого начинайте регистрацию. Дополнительные сведения см. в разделе [Отправка push-уведомлений пользователям](https://github.com/Azure-Samples/app-service-mobile-dotnet-backend-quickstart/blob/master/README.md#push-to-users) в полном примере краткого руководства по мобильным приложениям службы приложений для серверной части .NET.
