...
> [AZURE.NOTE] Сейчас доступна предварительная версия этого пакета SDK. Поэтому мы не рекомендуем использовать этот пакет SDK в рабочей среде. В примерах в этом документе используется пакет [azure-mobile-apps] версии v2.0.0-rc2.
...
### <a name="howto-enable-homepage"></a>Практическое руководство. Настройка домашней страницы в приложении
...
Можно также добавить параметр Swagger в файл `azureMobile.js`, если поддержка Swagger требуется только при локальной разработке.
