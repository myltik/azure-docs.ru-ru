...
2. В разделе **Параметры** щелкните **Проверка подлинности мобильного приложения**, а затем щелкните **Azure Active Directory**. Скопируйте указанные на странице **URL-адрес приложения** и **URL-адрес ответа**. Они понадобятся вам позже. Убедитесь, что в **URL-адресе приложения** и **URL-адресе ответа** используется схема HTTPS.
