...
## <a name="why-not-migrate"></a>Почему не следует переносить сайт

Есть несколько причин, по которым вам не следует переносить мобильные службы Azure в данный момент.

  *  Мобильная служба очень загружена, и сейчас вы не можете себе позволить перезапуск сайта.
  *  Вы не хотите, чтобы перенос отразился на рабочем сайте до тестирования процесса миграции.
  *  Несколько ваших сайтов находятся в ценовых категориях «Бесплатный» и «Базовый», и вы не хотите переносить все сайты одновременно.
  *  Вы запланировали задания в соответствии с требованиями переноса.

Если вы очень загружены, запланируйте перенос на время обслуживания по графику. В процессе переноса ваш сайт будет перезапущен и пользователи могут заметить кратковременный перерыв в доступности.

Для большинства пунктов в этом списке есть обходные пути. Дополнительные сведения см. в разделе [Перед началом работы](#before-you-begin) ниже.

## <a name="before-you-begin"></a>Перед началом работы

Прежде чем переносить сайт, выполните следующие действия:

  *  [Создайте резервную копию] сценариев и базы данных SQL мобильной службы.
  *  (Необязательно) Повысьте категорию мобильной службы до уровня «Стандартный».

Если вы хотите протестировать процесс переноса перед переносом рабочего сайта, продублируйте свою рабочую мобильную службу Azure (заполните ее копией источника данных) и протестируйте перенос на новом URL-адресе. Чтобы правильно протестировать перенесенный сайт, вам также нужно протестировать реализацию клиента, которая указывает на тестовый сайт.

### <a name="opt-raise-service-tier"></a>(Необязательно) Повышение категории мобильной службы до уровня «Стандартный»

Все сайты мобильных служб, которые используют один план размещения, переносятся одновременно. Мобильные службы в ценовых категориях «Бесплатный» и «Базовый» используют тот же план размещения, что и другие службы в той же ценовой категории и том же [регионе Azure]. Если ваша мобильная служба работает в ценовой категории «Стандартный», она использует собственный план размещения. Если вы хотите перенести сайты в ценовых категориях «Бесплатный» и «Базовый» по отдельности, временно повысьте ценовую категорию мобильной службы до уровня «Стандартный». Это можно сделать в меню «Масштаб» для мобильной службы.

  1.  Войдите на [классический портал Azure].
  2.  Выберите мобильную службу.
  3.  Откройте вкладку **Масштаб**.
  4.  В разделе **Уровень мобильной службы** выберите уровень **Стандартный**. Щелкните значок **Сохранить** в нижней части страницы.

Не забудьте задать соответствующее значение ценовой категории после переноса.

## <a name="migrating-site"></a>Перенос сайта
...
Если вы переносите мобильную службу в ценовых категориях «Бесплатный» или «Базовый», все мобильные службы в этой ценовой категории будут перенесены одновременно. Чтобы этого не произошло, вы можете на время переноса [повысить категорию мобильной службы, которую вы переносите](#opt-raise-service-tier), до уровня «Стандартный».
...
### <a name="on-demand-jobs"></a>Запланированные задания по требованию

Запланированные задания по требованию активируются с помощью веб-запроса. Рекомендуется использовать HTTP-клиент, например [Postman], [Fiddler] или [curl]. Если сайт называется contoso, то для активации задания по требованию нужно использовать конечную точку https://contoso.azure-mobile.net/jobs/_yourjobname_. Необходимо отправить дополнительный заголовок **X-ZUMO-MASTER** с главным ключом.

Чтобы получить главный ключ, выполните следующие действия:

  1. Войдите на [портал Azure].
  2. Выберите **Все ресурсы** или **Службы приложений**, а затем щелкните имя перенесенной мобильной службы.
  3. По умолчанию откроется колонка «Параметры». Если это не произошло, щелкните **Параметры**.
  4. В меню «Общие» щелкните элемент **Параметры приложения**.
  5. Найдите параметр приложения **MS\_MasterKey**

Вы можете вырезать и вставить главный ключ в сеанс Postman. Ниже приведен пример активации задания по требованию в перенесенной мобильной службе.

  ![Активация задания по требованию с помощью Postman][2]

Сохраните параметры.

  * Метод: **POST**
  * URL-адрес: https://_yoursite_.azure-mobile.net/jobs/_yourjobname_
  * Заголовки: X-ZUMO-MASTER: _ваш главный ключ_

Кроме того, вы можете использовать [curl] для активации задания по требованию в командной строке.

    curl -H 'X-ZUMO-MASTER: yourmasterkey' --data-ascii '' https://yoursite.azure-mobile.net/jobs/yourjob

Ваши задания по требованию находятся в папке `App_Data/config/scripts/scheduler post-migration`. Мы рекомендуем преобразовывать все задания по требованию в [веб-задания].
...
[Создайте резервную копию]: ../mobile-services/mobile-services-disaster-recovery.md
