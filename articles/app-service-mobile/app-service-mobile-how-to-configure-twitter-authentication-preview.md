...
4. Введите в **Name** и **Description** для нового приложения. Вставьте **URL-адрес мобильного приложения** в поле **Website**. Затем в поле **Callback URL** вставьте **Callback URL**, скопированный ранее. Это шлюз мобильного приложения, дополненный путем _/signin-twitter_. Пример: `https://contosogateway.azurewebsites.net/signin-twitter`. Убедитесь, что используете схему HTTPS.

    ![][0]

3.  В нижней части страницы прочтите и примите условия. Затем щелкните **Create your Twitter application**. После этого приложение будет зарегистрировано и появятся сведения о приложении.

4. Откройте вкладку **Settings**, установите флажок **Allow this application to be used to sign in with Twitter**, затем нажмите **Update Settings**.

5. Откройте вкладку **Keys and Access Tokens**. Запишите значения полей **Consumer Key (API Key)** и **Consumer secret (API Secret)**.
