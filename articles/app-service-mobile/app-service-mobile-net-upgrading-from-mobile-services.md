...
В этом разделе приводится процедура обновления существующего серверного приложения .NET мобильных служб Azure до нового мобильного приложения службы приложений. Во время обновления существующее приложение мобильных служб может продолжать работать.
...
>[AZURE.TIP]Перед обновлением рекомендуется [выполнить миграцию](app-service-mobile-dotnet-backend-migrating-from-mobile-services.md). Таким образом, к обеим версиям приложения будет применен один тарифный план службы, и дополнительные затраты будут исключены.
...
>[AZURE.TIP]Перед началом обновления рекомендуется полностью ознакомиться с остальной частью этого раздела. Запишите все используемые функции, которые вызываются ниже.

Пакеты SDK клиента для мобильных служб **не совместимы** с новым пакетом SDK сервера для мобильных приложений. Чтобы обеспечить непрерывность обслуживания приложения, не следует публиковать изменения на сайте, который в настоящее время обслуживает опубликованных клиентов. Вместо этого нужно создать новое мобильное приложение, которое является дубликатом. Это приложение можно включить в тот же план службы приложений, чтобы избежать дополнительных финансовых затрат.

После этого у вас будут две версии приложения: одна, которая остается без изменений и обслуживает опубликованные приложения на практике, и вторая, которую можно обновить и ориентировать на новый выпуск клиента. Вы можете перенести и протестировать код в удобное для вас время, но необходимо убедиться, что внесенные исправления применяются к обеим версиям. Если вы считаете, что до последней версии уже обновлено требуемое количество клиентских приложений, при желании можно удалить исходное перенесенное приложение.

Полная схема процесса обновления выглядит следующим образом.

1. Создание нового мобильного приложения
2. Обновление проекта для использования новых пакетов SDK сервера
...
Скорее всего, вы захотите использовать ту же базу данных и концентратор уведомлений, которые использовали в мобильных службах. Вы можете скопировать эти значения, открыв [портал Azure], перейдя к исходному приложению и выбрав **Параметры** > **Параметры приложения**. В разделе **Строки подключения** скопируйте `MS_NotificationHubConnectionString` и `MS_TableConnectionString`. Перейдите на свой новый сайт обновления и вставьте эти значения, перезаписав все существующие значения. Повторите эту процедуру для других необходимых параметров приложения. Если перенесенная служба не используется, строки подключения и параметры приложения можно просмотреть на вкладке **Настройка** раздела "Мобильные службы" на [классическом портале Azure].
...
Затем выполняется добавление ссылок на пакет SDK для мобильных приложений. Для данного обновления большинство разработчиков будет загружать и устанавливать пакет `Micrsoft.Azure.Mobile.Server.Quickstart`, тем самым извлекая весь необходимый набор.
...
2. В Visual Studio щелкните правой кнопкой мыши проект и последовательно выберите **Добавить** -> **Новый элемент**. Выберите **Интернет** -> **Общие** -> **Класс запуска OWIN**. 
...
В этом случае проверьте, что задан параметр MS\_MobileServiceName. Если имя схемы в приложении было настроено ранее, можно также указать другое имя.
...
Основные проблемы при использовании CORS заключаются в необходимости разрешить заголовки `eTag` и `Location` для правильного функционирования пакета SDK клиента.
...
Компоненты проверки подлинности мобильных служб были перемещены в функцию проверки подлинности и авторизации службы приложений. Сведения о включении этой функции для сайта см. в статье [Добавление проверки подлинности в мобильное приложение](app-service-mobile-ios-get-started-users.md).

При работе с некоторыми поставщиками, например AAD, Facebook и Google, требуется возможность использования существующей регистрации из копии приложения. Нужно просто перейти на портал поставщика удостоверений и добавить в систему регистрации новый URL-адрес перенаправления. Затем настройте проверку подлинности и авторизацию службы приложений с идентификатором клиента и секретом.

### Авторизация действий контроллера
Теперь все экземпляры атрибута `[AuthorizeLevel(AuthorizationLevel.User)]` необходимо изменить для использования стандартного атрибута ASP.NET `[Authorize]`. Кроме того, теперь контроллеры являются анонимными по умолчанию, как и в других приложениях ASP.NET. Если вы использовали один из других параметров AuthorizeLevel, например Admin или Application, обратите внимание, что их больше нет. Вместо этого можно настроить атрибут AuthorizationFilters для общих секретов или настроить участника службы AAD для безопасного выполнения вызовов между службами.
...
Если ваше приложение использует решение для настраиваемой проверки подлинности, необходимо убедиться, что обновленный сайт имеет доступ к системе. Инструкции по интеграции решения см. в статье [Обзор пакета SDK сервера для .NET]. Обратите внимание, что компоненты настраиваемой проверки подлинности все еще находятся на стадии предварительной версии.

##<a name="updating-clients"></a>Обновление клиентов
После получения рабочего внутреннего сервера мобильных приложений вы можете работать с новой версией клиентского приложения, которое использует это мобильное приложение. Мобильные приложения также содержат новую версию клиентского пакета SDK, и, аналогично приведенному выше процессу обновления сервера, перед установкой версий мобильных приложений потребуется удалить все ссылки на пакеты SDK мобильных служб.

Одно из основных различий между версиями заключается в том, что конструкторам больше не требуется ключ приложения. Просто передайте URL-адрес мобильного приложения. Например, в клиентах .NET `MobileServiceClient` теперь конструктор выглядит следующим образом:
...
- [iOS версии 3.0.0 или более поздней версии](app-service-mobile-ios-how-to-use-client-library.md)
- [.NET/Windows (Xamarin) версии 2.0.0 или более поздней версии](app-service-mobile-dotnet-how-to-use-client-library.md) 

Если приложение использует push-уведомления, обратите внимание на особые инструкции по регистрации для каждой платформы, поскольку в них были внесены некоторые изменения.

Когда новая версия клиента будет готова, испробуйте ее в новом обновленном серверном проекте. Проверив ее работу, вы можете предоставить новую версию приложения клиентам. После того как клиенты смогут получить эти обновления, вы можете удалить версию мобильных служб своего приложения. Этот шаг завершает обновление до мобильного приложения службы приложений с помощью последнего пакета SDK сервера мобильных приложений.
