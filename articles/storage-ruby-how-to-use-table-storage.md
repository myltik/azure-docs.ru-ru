<properties 
	pageTitle="Использование табличного хранилища из Ruby в Microsoft Azure" 
	description="Вы узнаете, как использовать службу табличного хранилища в Azure. Примеры написаны с помощью API Ruby." 
	services="storage" 
...
Ruby API. Здесь описаны такие сценарии, как **создание и удаление таблицы, вставка и запрос сущностей в таблице**.
...
Модуль Azure считывает переменные среды **AZURE_STORAGE_ACCOUNT** и **AZURE_STORAGE_ACCESS_KEY**для получения сведений, необходимых для подключения к учетной записи хранения Azure. Если эти переменные среды не заданы, необходимо указать сведения об учетной записи перед использованием **Azure::TableService** с помощью следующего кода:
...
* **merge_entity():** обновление сущности посредством объединения новых значений свойств с имеющейся сущностью.
* **insert_or_merge_entity():** обновление имеющейся сущности путем ее замены. Если сущность не существует, будет вставлена новая:
* **insert_or_replace_entity():** обновление сущности посредством объединения новых значений свойств с имеющейся сущностью. Если сущность не существует, будет вставлена новая.
...
Чтобы запросить сущность в таблице, используйте метод **get_entity()**, передав имя таблицы, **PartitionKey** и **RowKey**.
...
- Обратитесь к репозиторию [Azure SDK для Ruby](http://github.com/WindowsAzure/azure-sdk-for-ruby) (Пакет SDK Azure для Ruby) на веб-сайте GitHub
