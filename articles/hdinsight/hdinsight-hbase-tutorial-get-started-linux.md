...
# Руководство по HBase. Приступая к работе с Apache HBase на Hadoop в HDInsight (Linux)
...
> [AZURE.NOTE]Информация, приведенная в этом документе, относится только к кластерам HDInsight на платформе Linux. Сведения о кластерах под управлением Windows см. в статье [Приступая к работе с Apache HBase на Hadoop в HDInsight (Windows)](hdinsight-hbase-tutorial-get-started.md).
...
**Создание кластера HBase с помощью портала предварительной версии Azure**


1. Выполните вход на [портал предварительной версии Azure][azure-portal].
2. Щелкните **Создать** в левом верхнем углу и выберите **Данные + аналитика**, **HDInsight**.
3. Введите следующие значения.


	- **Имя кластера** — введите имя для определения этого кластера.
	- **Тип кластера** — выберите **HBase**.
	- **Операционная система кластера** — выберите **Linux**. Сведения о кластерах HBase под управлением Windows см. в статье [Руководство по HBase. Приступая к работе с Apache HBase на Hadoop в HDInsight (Windows)](hdinsight-hbase-tutorial-get-started.md).
	...
	- **Учетные данные**. При появлении запроса введите пароль для пользователя веб-службы HTTP. Имя пользователя по умолчанию — **admin**. Необходимо также ввести **имя пользователя SSH** и **пароль** или **открытый ключ**, которые будут использоваться для проверки подлинности пользователя SSH. Мы рекомендуем использовать открытый ключ. Дополнительные сведения об использовании SSH с HDInsight см. в следующих статьях.
		...
		- [Использование SSH с Hadoop на основе Linux в HDInsight в Windows](hdinsight-hadoop-linux-use-ssh-windows.md). Нажмите кнопку **Выбрать**, чтобы сохранить изменения.
