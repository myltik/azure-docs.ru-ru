...
3. Введите следующие значения.
	...
	- **Data Source** — укажите существующую или создайте новую учетную запись хранения Azure, которая используется для кластера в качестве файловой системы по умолчанию. Расположение учетной записи хранения определяет расположение кластера. Кластер HDInsight и зависимая учетная запись хранения Azure должны находиться в одном и том же центре обработки данных. По умолчанию в качестве имени **контейнера по умолчанию** используется имя кластера.  
...
5. В разделе **Категория** в левой части диалогового окна последовательно разверните узлы **Подключение** и **SSH**, а затем выберите **Туннели**.
