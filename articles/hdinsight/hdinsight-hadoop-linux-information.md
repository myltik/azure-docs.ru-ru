...
Замените параметр **PASSWORD** паролем учетной записи администратора, а **CLUSTERNAME** — именем своего кластера. Эта команда возвращает документ JSON, который содержит список узлов в кластере. Программа jq извлекает значение элемента `host_name` для каждого узла.
...
> [!NOTE]
> Эта команда возвращает первую конфигурацию, применяемую к серверу (`service_config_version=1`), который содержит эти сведения. Может потребоваться перечислить все версии конфигурации, чтобы найти последнюю.
...
Действия сценариев — это сценарии Bash. Они выполняются при подготовке кластера, и их можно использовать для установки и настройки дополнительных компонентов в кластере. Доступны примеры скриптов для установки следующих компонентов:
