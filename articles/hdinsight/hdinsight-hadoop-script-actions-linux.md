...
Действия сценариев служат для настройки кластеров Azure HDInsight. Для этого либо задаются параметры конфигурации кластера, либо в кластере устанавливаются дополнительные службы, инструменты или другое программное обеспечение. Действия сценария можно использовать во время создания кластера или в работающем кластере.
...
Пользователям нужно сделать так, чтобы все сценарии и ресурсы, используемые сценарием, были доступны в период существования кластера и чтобы версии этих файлов не изменялись на протяжении данного периода. Эти ресурсы необходимы, если во время операций масштабирования в кластер добавляются новые узлы.
...
> [AZURE.NOTE] Веб-интерфейс Ambari доступен только в том случае, если кластер успешно создан. Если во время создания кластера используется действие сценария и создание завершается ошибкой, обратитесь к разделу по устранению неполадок [Настройка кластеров HDInsight с помощью действия сценария](hdinsight-hadoop-customize-cluster-linux.md#troubleshooting), чтобы узнать другие способы доступа к данным журнала.
...
Действия сценариев можно использовать для настройки кластеров HDInsight с помощью портала Azure, Azure PowerShell, шаблонов Azure Resource Manager (ARM) или пакета SDK .NET для HDInsight. Указания см. в разделе [Как использовать действие сценария](hdinsight-hadoop-customize-cluster-linux.md).
