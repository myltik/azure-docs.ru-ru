...
Чтобы сократить время выполнения сценария, избегайте операций, компилирующих ресурсы из исходного кода. Предварительно компилируйте ресурсы и сохраняйте их в хранилище BLOB-объектов Azure, чтобы их можно было быстро загружать.
...
Вспомогательные методы действий сценариев — это служебные программы, которые можно использовать при создании пользовательских скриптов. Эти методы содержатся в сценарии [https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh](https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh). Чтобы скачать и использовать их как часть сценария, используйте следующую команду:
...
Замените `INFILE` на файл с меткой порядка байтов. `OUTFILE` должно быть новым именем файла, который будет содержать скрипт без метки порядка байтов.
