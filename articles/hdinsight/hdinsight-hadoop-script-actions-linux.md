---
title: Разработка действий сценариев с помощью HDInsight на основе Linux | Microsoft Docs
...
> [!NOTE]
> Информация, приведенная в этом документе, относится только к кластерам HDInsight на платформе Linux. Сведения об использовании действий сценариев в кластерах на платформе Windows см. в статье [Разработка действий сценариев с помощью HDInsight (Windows)](hdinsight-hadoop-script-actions.md).
> 
> 
...
Параметры, передаваемые в скрипт, называются *позиционными параметрами*, т. е. `$1` соответствует первому параметру, `$2` — второму и т. д. Значение `$0` содержит имя самого сценария.
...
В приведенной выше команде замените **INFILE** на файл с меткой порядка байтов. Для **OUTFILE** укажите новое имя файла, который будет содержать скрипт без метки порядка байтов.
