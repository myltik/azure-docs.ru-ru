...
Действие сценария можно вызвать при подготовке кластера из __портала предварительной версии Azure__, __Azure PowerShell__ или из __пакета SDK для HDInsight .NET__.
...
Эта команда перенаправляет данные из STDOUT (значение 1, которое является значением по умолчанию, поэтому не указано здесь) в STDERR (2). Дополнительные сведения о перенаправлении данных см. на странице [http://www.tldp.org/LDP/abs/html/io-redirection.html](http://www.tldp.org/LDP/abs/html/io-redirection.html).
...
Вспомогательные методы действий скриптов представляют собой служебные программы, которые можно использовать при создании пользовательских скриптов. Их определение приведено на странице [https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh](https://hdiconfigactions.blob.core.windows.net/linuxconfigactionmodulev01/HDInsightUtilities-v01.sh). Эти методы могут быть включены в сценарии с помощью следующей команды.
...
Параметры, передаваемые в сценарий, называются _позиционными параметрами_, т. е. `$1` соответствует первому параметру, `$2` — второму и т. д. Значение `$0` содержит имя самого сценария.
...
> [AZURE.NOTE]Приведенные выше документы относятся только к кластерам HDInsight под управлением Linux. Сведения о сценариях, предназначенных для HDInsight под управлением Windows, см. в статье [Разработка действий сценариев с помощью HDInsight (Windows)](hdinsight-hadoop-script-actions.md) или по ссылкам, приведенным в начале каждой статьи.
...
_Причина._ Эта ошибка возникает, если сценарий сохранен в кодировке UTF-8 с меткой порядка байтов (BOM).
...
В приведенной выше команде замените __INFILE__ на файл с меткой порядка байтов. Для __OUTFILE__ укажите новое имя файла, который будет содержать сценарий без метки порядка байтов.
