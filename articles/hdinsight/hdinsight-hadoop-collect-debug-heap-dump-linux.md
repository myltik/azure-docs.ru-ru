...
4. Найдите запись *** _OPTS** для службы, для которой нужно включить дампы кучи, и добавьте параметры, которые требуется включить. На следующем рисунке я добавил `-XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/` в запись **HADOOP_NAMENODE_OPTS**:
