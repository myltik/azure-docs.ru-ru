...
> [AZURE.NOTE] Воронка и сито были переданы для включения в проект Apache Storm. Дополнительные сведения см. на странице портала GitHub [STORM-583: Initial check-in for storm-event hubs](https://github.com/apache/storm/pull/336/files) (STORM 583: первичная регистрация для концентраторов событий storm).
...
1. На [классическом портале Azure](https://manage.windowsazure.com) выберите **Создать** > __Службы приложений__ > **Служебная шина** > **Концентратор событий** > **Настраиваемое создание**.
...
2. В диалоговом окне **Настройка концентраторов событий** введите значения параметров **Количество разделов** и **Хранение сообщений**. В этом примере числу разделов присвойте значение 8, а хранению сообщений — 1. Запомните количество разделов, поскольку это значение понадобится позже.
...
4. Откройте Visual Studio. В меню __Вид__ щелкните __Cloud Explorer__. В __Cloud Explorer__ разверните элемент __Учетные записи хранения__, а затем разверните созданную ранее учетную запись хранения.

    ![Cloud explorer](./media/hdinsight-storm-develop-csharp-event-hub-topology/createtablestorage.png)

5. Щелкните правой кнопкой мыши элемент __Таблицы__ для своей учетной записи хранения и выберите пункт __Новая таблица__. Присвойте таблице имя **events**, когда появится соответствующий запрос. Сохраните это имя, оно понадобится на следующих этапах.
...
> [AZURE.NOTE] По умолчанию эта топология создает один рабочий процесс, которого достаточно для примера. Если вы адаптируете эту топологию для рабочего кластера, добавьте следующие строки, чтобы изменить число рабочих процессов.
