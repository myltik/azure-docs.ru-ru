---
title: Обработка событий из службы концентраторов событий Azure с помощью Storm в HDInsight | Microsoft Docs
...
> [!NOTE]
> Шаги, описанные в этом документе, можно применять только к кластеру HDInsight под управлением Windows. Java-версию этого проекта, которая работает с кластером под управлением Linux или Windows, можно найти в разделе [Обработка событий из службы концентраторов событий Azure с помощью Storm в HDInsight (Java)](hdinsight-storm-develop-java-event-hub-topology.md).
> 
> 
...
> [!NOTE]
> При использовании завершенного проекта следует воспользоваться **диспетчером пакетов NuGet**, чтобы восстановить пакеты, необходимые для этого решения.
> 
> 
...
Последняя версия файла **eventhubs-storm-spout-0.9-jar-with-dependencies.jar** содержится в проекте с [примерами HDInsight Storm](https://github.com/hdinsight/hdinsight-storm-examples), который размещен в папке **lib**. Чтобы скачать файл, используйте один из следующих методов.

> [!NOTE]
> Воронка и сито были переданы для включения в проект Apache Storm. Дополнительные сведения см. на странице портала GitHub [STORM-583: Initial check-in for storm-event hubs](https://github.com/apache/storm/pull/336/files) (STORM 583: первичная регистрация для концентраторов событий storm).
> 
> 

* **Загрузка ZIP-файла**: на странице с [примерами HDInsight Storm](https://github.com/hdinsight/hdinsight-storm-examples) нажмите на правой панели кнопку **Download ZIP** (Загрузить ZIP-файл), чтобы скачать ZIP-файл с проектом.
...
## Настройка концентраторов событий Azure
...
1. На [классическом портале Azure](https://manage.windowsazure.com) выберите **Создать** > **Службы приложений** > **Служебная шина** > **Концентратор событий** > **Настраиваемое создание**.
2. В диалоговом окне **Добавление нового концентратора событий**введите **Имя концентратора событий**, выберите **Регион** для создания в нем концентратора и создайте новое пространство имен или выберите существующее. Щелкните **стрелку** для продолжения.
   ...
   > 
   > 
3. В диалоговом окне **Настройка концентраторов событий** введите значения параметров **Количество разделов** и **Хранение сообщений**. В этом примере числу разделов присвойте значение 8, а хранению сообщений — 1. Запомните количество разделов, поскольку это значение понадобится позже.
...
5. Выберите **Настроить** и создайте две новые политики доступа, используя следующую информацию.
   ...
   | Имя | Разрешения |
   | --- | --- |
   | Модуль записи |Отправка |
   | читатель. |Прослушивание |
...
1. На [классическом портале Azure](https://manage.windowsazure.com) выберите **СОЗДАТЬ** > **Службы данных** > **Хранилище** > **Быстрое создание**.
...
2. Введите **Имя** учетной записи хранения, выберите **Расположение** и установите **флажок** для создания учетной записи хранения.
   ...
   > [!NOTE]
   > Для уменьшения задержек и затрат в поле **Расположение** следует выбрать то же расположение, в котором находится служба концентраторов событий и сервер Storm в HDInsight.
   > 
   > 
3. После создания учетной записи хранения выберите эту учетную запись, а затем нажмите ссылку **Управление ключами доступа** в нижней части страницы, чтобы получить **имя учетной записи хранения** и **первичный ключ доступа**. Сохраните эти сведения для использования в дальнейшем.
   
    ![ключи доступа](./media/hdinsight-storm-develop-csharp-event-hub-topology/managekeys.png)
4. Откройте Visual Studio. В меню **Вид** щелкните **Cloud Explorer**. В **Cloud Explorer** разверните элемент **Учетные записи хранения**, а затем разверните созданную ранее учетную запись хранения.
   
    ![Cloud explorer](./media/hdinsight-storm-develop-csharp-event-hub-topology/createtablestorage.png)
5. Щелкните правой кнопкой мыши элемент **Таблицы** для своей учетной записи хранения и выберите пункт **Новая таблица**. Присвойте таблице имя **events**, когда появится соответствующий запрос. Сохраните это имя, оно понадобится на следующих этапах.
...
3. Введите следующие параметры. Используйте сведения для концентратора событий, созданного ранее, в столбце **Значение**.
   ...
   | Имя | Тип | Область |
   | --- | --- | --- |
   | EventHubPolicyName |string |Приложение |
   | EventHubPolicyKey |string |Приложение |
   | EventHubNamespace |string |Приложение |
   | EventHubName |string |Приложение |
   | EventHubPartitionCount |int |Приложение |
...
2. Откройте файл **Program.cs** и добавьте следующий код непосредственно после строки `TopologyBuilder topologyBuilder = new TopologyBuilder("EventHubWriter" + DateTime.Now.ToString("yyyyMMddHHmmss"));`.
...
4. Сразу после предыдущего кода добавьте следующий код:
...
> [!NOTE]
> По умолчанию эта топология создает один рабочий процесс, которого достаточно для примера. Если вы адаптируете эту топологию для рабочего кластера, добавьте следующие строки, чтобы изменить число рабочих процессов.
> 
> 
...
1. В **обозревателе решений** откройте файл **Spout.cs** и добавьте следующий код в его начало:
   ...
   > [!NOTE]
   > Должен быть уже установлен пакет JSON.NET, поскольку он необходим для платформы SCP.NET, которая используется для топологий C# Storm.
   > 
   > 
2. Найдите следующий код:
    ...
    Теперь для определения данных, созданных воронкой, будет использоваться **строковый** тип данных и сериализатор **CustomizedInteropJSONSerializer**, объявленный в топологии ранее (в файле program.cs).
3. Замените метод **NextTuple** на следующий код:
...
1. Откройте Visual Studio, выберите последовательно **Файл** > **Создать** > **Проект**.
2. В диалоговом окне **Новый проект** разверните **Установленные** > **Шаблоны** и выберите **HDInsight**. В списке шаблонов выберите **Приложение Storm**. В нижней части диалогового окна, введите имя приложения **EventHubReader**.

### Конфигурация
1. В **обозревателе решений** щелкните правой кнопкой мыши **EventHubReader** и выберите пункт **Свойства**.
2. Выберите элемент **Сервис**, затем **Диспетчер пакетов NuGet** и **Консоль диспетчера пакетов**. Когда появится консоль, выполните следующую команду, чтобы установить пакеты хранилища Azure.
   
        NuGet install WindowsAzure.Storage
3. В свойствах проекта выберите **Параметры** и выберите **Этот проект не содержит файл параметров по умолчанию. Щелкните здесь, чтобы создать такой файл.**
4. Введите следующие параметры. Используйте сведения для концентратора событий и учетной записи хранения, созданные ранее, в столбце **Значение**.
   ...
   | EventHubName |string |Приложение |
   | EventHubPartitionCount |int |Приложение |
   | StorageConnection |(строка подключения) |Приложение |
   | TableName |string |Приложение |
...
5. Сохраните изменения и закройте страницу **Свойства**.
...
2. Откройте файл **Program.cs** и добавьте следующий код непосредственно после строки`TopologyBuilder topologyBuilder = new TopologyBuilder("EventHubReader" + DateTime.Now.ToString("yyyyMMddHHmmss"));`.
...
4. Добавьте следующий код сразу после предыдущего кода:
...
> [!NOTE]
> По умолчанию эта топология создает один рабочий процесс, которого достаточно для примера. Если вы адаптируете эту топологию для рабочего кластера, следует добавить следующее для изменения количества рабочих процессов:
> 
> 
...
1. В **обозревателе решений** щелкните правой кнопкой мыши проект **EventHubReader**, выберите пункт **Добавить**, а затем **Класс**. Присвойте новому классу имя **Devices.cs**.
2. Откройте файл **Devices.cs** и замените существующий код приведенным ниже кодом.
...
4. Сразу после предыдущего кода добавьте следующий код:
    ...
    Здесь выполняется подключение к созданной ранее таблице хранилища Azure с помощью строки подключения, которая хранится в переменной `TableName`.
5. Найдите метод **Execute** и замените его следующим кодом:
   ...
   > [!IMPORTANT]
   > Компонент EventHubSpout ожидает ACK для каждого кортежа от нижестоящих компонентов, как от этого сита, например. Если не получены ACK, EventHubSpout предполагает, что в обработке для кортежа произошел сбой.
   > 
   > 

На этом этапе у вас есть полная топология, которая будет считывать данные из концентратора событий и сохранять их в созданной ранее таблице в хранилище таблиц.
...
5. В диалоговом окне **Отправка топологии** выберите свой **кластер Storm**. Разверните узел **Дополнительные конфигурации**, выберите **Пути к файлам Java**, элемент **...** и укажите каталог, содержащий загруженный ранее файл **eventhubs-storm-spout-0.9-jar-with-dependencies.jar**. Теперь нажмите кнопку **Отправить**.
