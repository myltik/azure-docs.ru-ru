---
title: "Установка GNU R в HDInsight на основе Linux | Документация Майкрософт"
...
---
# <a name="install-and-use-gnu-r-on-linux-based-hdinsight-hadoop-clusters"></a>Установка и использование GNU R в кластерах HDInsight Hadoop на платформе Linux

GNU R можно установить на любой тип кластера Hadoop в HDInsight с помощью параметра кластера **Действие сценария**. Специалисты по обработке и анализу данных могут использовать R для обработки больших объемов данных в кластерах Hadoop, развернутых в HDInsight.

> [!WARNING]
> В этом документе приведены инструкции по установке GNU R в HDInsight в качестве настраиваемого действия. GNU R __не__ предоставляет те же возможности, что и __Microsoft R Server в HDInsight__. Вместо выполнения действий, описанных в этом документе, рекомендуется использовать R Server в HDInsight.
>
> Сведения о том, как использовать R Server в HDInsight, см. в разделе [Приступая к работе с R Server в HDInsight](hdinsight-hadoop-r-server-get-started.md). 
...
Этот раздел содержит указания по использованию сценария при создании кластера с помощью портала Azure. 
...
2. В колонке **Необязательная настройка** выберите **Действия сценария** и введите в форму следующие сведения.
...
1. Подключитесь к кластеру HDInsight с помощью протокола SSH:
   ...
   * [Использование SSH с HDInsight (Hadoop) на платформе Windows 10, Linux, Unix или OS X](hdinsight-hadoop-linux-use-ssh-unix.md)
...
3. Введите следующую программу R: Этот код генерирует числа от 1 до 100 и умножает их на 2.
...
4. Затем используйте следующую команду для просмотра временного пути, по которому были сохранены выходные данные MapReduce:
    ...
    Эта команда возвращает значение следующего вида: `/tmp/file5f615d870ad2`. Чтобы просмотреть выходные данные, используйте следующую команду.
...
5. Для выхода из R введите следующую команду.
