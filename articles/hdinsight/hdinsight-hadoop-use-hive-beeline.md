...
1. После подключения получите имя головного узла, выполнив следующую команду:

        hostname -f
    
    Сохраните полученное имя узла, так как оно понадобится при подключении к HiveServer2 из Beeline.
    
2. Запустите интерфейс командной строки Hive с помощью следующей команды:

        beeline

2. В командной строке `beeline>` введите следующую команду, чтобы подключиться к службе HiveServer2: Замените __HOSTNAME__ полученным ранее именем головного узла:
...
3. Команды Beeline обычно начинаются с символа `!`. Например, `!help` выводит справку. Однако `!` часто можно опускать. Например, `help` также будет работать.
...
3. Чтобы сохранить файл, нажмите клавиши __CONTROL__+___\_X__, затем введите __Y__ и __ВВОД__.

4. Запустите файл с помощью Beeline, используя следующую команду: Замените __HOSTNAME__ полученным ранее именем головного узла, а __PASSWORD__ — паролем учетной записи администратора:
