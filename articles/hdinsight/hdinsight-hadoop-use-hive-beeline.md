...
1. После подключения запустите Beeline с помощью такой команды:
    ...
    Клиент Beeline будет запущен, и будет выполнен переход по URL-адресу JDBC. В нашем случае указан адрес `localhost`, так как сервер HiveServer2 работает на обоих головных узлах кластера, а мы запускаем Beeline непосредственно на головном узле 0.
