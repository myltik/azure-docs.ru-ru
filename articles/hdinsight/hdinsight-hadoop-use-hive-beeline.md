...
2. В командной строке `beeline>` введите следующую команду, чтобы подключиться к службе HiveServer2: Замените __HOSTNAME__ полученным ранее именем головного узла:
    ...
    Это указывает Beeline на необходимость подключиться к порту __10001__ на указанном узле __HOSTNAME__ и использовать в качестве метода транспорта __HTTP__. Для проверки подлинности подключения используется учетная запись __администратора__.
