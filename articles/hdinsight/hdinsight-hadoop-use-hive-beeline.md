<properties
   ...
   description="Узнайте, как использовать SSH для подключения к кластеру Hadoop в HDInsight и интерактивно отправлять запросы Hive с помощью инструмента Beeline."
   services="hdinsight"
   documentationCenter=""
   authors="Blackmist"
   manager="paulettm"
...
> [AZURE.NOTE]Для подключения к Hive в Beeline используется драйвер JDBC. Дополнительные сведения об использовании JDBC с Hive см. в статье [Подключение к Hive в Azure HDInsight с помощью драйвера Hive JDBC](hdinsight-connect-hive-jdbc-driver.md).
...
Windows не предоставляет встроенный клиент SSH. Рекомендуется использовать клиент **PuTTY**, который можно скачать по адресу: [http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html).
...
2. В командной строке `beeline>` введите следующую команду для подключения к службе HiveServer2:

        !connect jdbc:hive2://headnode0:10001/;transportMode=http admin

    При появлении соответствующего запроса введите пароль учетной записи администратора кластера HDInsight. После установления соединения вы увидите следующее:
    
        jdbc:hive2://headnode0:10001/>

3. Команды Beeline обычно начинаются с символа `!`. Например, `!help` выводит справку. Однако `!` часто можно опускать: `help` также будет работать.

    При просмотре справки можно заметить текст `!sql`, который используется для выполнения инструкций HiveQL. Однако они настолько распространены, что `!sql` тоже можно опустить. Следующие две инструкции дадут идентичный результат (будут выведены таблицы, доступные в Hive):
...
5. Введите следующие инструкции, чтобы создать таблицу с именем **log4jLogs** с помощью примера данных, предоставляемого с кластером HDInsight:
    ...
    * **INPUT\_\_FILE\_\_NAME LIKE '%.log'**: указывает Hive, что нужно вернуть только данные из файлов с расширением LOG. Как правило, возвращаемые запросом Hive данные с одной и той же схемой хранятся в одной и той же папке. Однако в этом примере файл журнала хранится с другими форматами данных.
...
2. Когда откроется редактор, добавьте в файл следующее содержимое: Запрос создаст новую таблицу internal с именем **errorLogs**:
    ...
    * **INSERT OVERWRITE ... SELECT**: выбирает из таблицы **log4jLogs** строки, которые содержат **[ERROR]**, а затем вставляет данные в таблицу **errorLogs**.
    
    > [AZURE.NOTE]В отличие от внешних таблиц, удаление внутренних таблиц приводит также к удалению данных.
    
3. Чтобы сохранить файл, нажмите клавиши __Ctrl__+___\_X__, затем введите __Y__ и __ВВОД__.

4. Используйте следующую команду для запуска файла с использованием Beeline:

        beeline -u 'jdbc:hive2://headnode0:10001/;transportMode=http' -n admin -p GiantR0b0! -f query.hql

5. Чтобы убедиться, что таблица **errorLogs** создана, запустите Beeline и подключитесь к HiveServer2. Затем выполните следующую инструкцию, чтобы вывести все строки из таблицы **errorLogs**:

        SELECT * from errorLogs;

    Должны быть возвращены три строки данных, каждая из которых содержит **[ERROR]** в столбце t4:
