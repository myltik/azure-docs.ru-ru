...
Начиная с этого выпуска изменена политика исправлений в гостевой ОС для кластеров HDInsight под управлением Linux. Цель этой новой политики — значительно сократить число перезагрузок, выполняемых в процессе исправления. Согласно новой политике исправления виртуальных машин в кластерах Linux выполняются каждый понедельник или четверг, начиная с 12:00 (UTC), поочередно обслуживая узлы в каждом данном кластере. Но перезагрузка каждой виртуальной машины по причине исправлений в гостевой ОС выполняется не чаще одного раза в 30 дней. Кроме того, первая перезагрузка только что созданного кластера выполняется не ранее, чем через 30 дней с даты создания кластера.

> [!NOTE]
> Эти изменения распространяются только на только что созданные кластеры, версия которых совпадает с версией этого выпуска или выпущена после нее.
>
>
...
| Нерекомендованные старые версии Spark |Обслуживание старой версии Spark 1.3.1 прекращено с 31 мая. |служба |Кластеры Spark в Windows |Недоступно |
...
| Изменения в формате имени виртуальных машин для развертывания нескольких кластеров HDInsight на базе Linux в одной виртуальной сети |В этом выпуске добавлена поддержка развертывания нескольких кластеров HDInsight на базе Linux в одной виртуальной сети. С учетом этого обновления формат имен виртуальных машин в кластере изменен с headnode\*, workernode\* и zookeepernode\* на hn\*, wn\* и zk\* соответственно. Не рекомендуется использовать прямую зависимость от формата имен виртуальных машин, так как он может меняться. Используйте hostname -f на локальном компьютере или Ambari API, чтобы определить список узлов и сопоставление компонентов с узлами. Дополнительные сведения можно найти на страницах [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/hosts.md) и [https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md](https://github.com/apache/ambari/blob/trunk/ambari-server/docs/api/v1/host-components.md). |служба |Кластеры HDInsight на базе Linux |Недоступно |
...
| Название | Описание | Затронутая область (например, служба, компонент или пакет SDK) | Тип кластера (например, Hadoop, HBase или Storm) | JIRA (если применимо) |
| --- | --- | --- | --- | --- |
| Исправление процесса повторного создания образа узла кластера Spark |Исправлена ошибка, вызывавшая блокировку восстановления узлов кластера Spark после повторного создания образа. |служба |Spark |Недоступно |
...
<td>В этом обновлении объем доступной памяти по умолчанию для контейнеров YARN (yarn.nodemanager.resource.memory-mb и yarn.scheduler.maximum-allocation-mb), запускаемых в диспетчере узлов, увеличен до 5632 МБ. Ранее он был уменьшен до 4608 МБ, но с учетом различных выполняемых заданий новый объем обеспечит больше надежности и производительности для большинства заданий, поэтому это более оптимальное значение по умолчанию. Если эта настройка памяти критически важна, то установите ее явным образом при создании кластера.</td>
<td>HDP</td>
<td>Все типы кластеров</td>
<td>Недоступно</td>
</tr>
...
<td>В этом выпуске HDI 3.2 включены исправления для Apache TEZ 2214 и TEZ 1923. Они необходимы для определенных запросов Hive в Tez, для которых требуется перемешать значительное количество данных в случайном порядке.
</td>
<td>HDP</td>
<td>Hadoop</td>
...
Данный выпуск содержит следующие обновления компонентов:
...
> [!NOTE]
> Чтобы продемонстрировать новые возможности, ставшие доступными благодаря настройке кластера, были задокументированы процедуры использования действий сценария для установки на кластере модулей Spark и R. Дополнительные сведения можно найти в разделе 
