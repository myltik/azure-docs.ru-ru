...
Используйте следующую команду для создания туннеля SSH с помощью команды `ssh` . Замените __USERNAME__ именем пользователя SSH для кластера HDInsight, а __CLUSTERNAME__ — именем кластера HDInsight.
...
4. Выберите вкладку **URL Patterns** (Шаблоны URL-адреса), а затем щелкните **Add New Pattern** (Добавить новый шаблон). Используйте следующие значения параметров, чтобы определить шаблон, а затем нажмите кнопку **ОК**:
    ...
    * __Имя шаблона__ — headnode
    * __URL pattern__ (Шаблон URL-адреса) - \*headnodehost\*.
...
5. В верхней части диалогового окна FoxyProxy из раскрывающегося списка **Выбрать режим** выберите пункт **Use proxies based on their pre-defined patterns and priorities** (Использовать прокси-серверы на основе предопределенных шаблонов и приоритетов), а затем нажмите кнопку **Закрыть**.
...
3. После появления информации о службе HDFS выберите __Быстрые ссылки__. Появится список головных узлов кластера. Выберите один из головных узлов, а затем выберите __NameNode UI__(Пользовательский интерфейс NameNode).

    ![Изображение с развернутым меню "Быстрые ссылки"](./media/hdinsight-linux-ambari-ssh-tunnel/namenodedropdown.png)

    > [AZURE.NOTE] Если подключение к Интернету медленное или головной узел занят, при выборе меню __Быстрые ссылки__вместо меню отображается индикатор ожидания. В этом случае подождите одну-две минуты для получения данных с сервера, а затем повторите попытку.
    >
...
4. Откроется страница, аналогичная следующей:
    ...
    > [AZURE.NOTE] Обратите внимание на URL-адрес этой страницы. Он должен иметь вид __http://hn1-<имя_кластера>.randomcharacters.cx.internal.cloudapp.net:8088/cluster__. Этот адрес использует полное внутреннее доменное имя узла (FQDN), и он недоступен без использования туннеля SSH.
