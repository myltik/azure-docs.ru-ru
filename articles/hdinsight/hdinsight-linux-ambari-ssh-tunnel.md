...
4. Выберите вкладку **Шаблоны URL-адреса**, а затем нажмите кнопку **Добавить новый шаблон**. Используйте следующие значения параметров, чтобы определить шаблон, а затем нажмите кнопку **ОК**:
	...
	* **Шаблон URL** — ***headnode*** — определяет шаблон, которому соответствует любой URL-адрес со словом **headnode**.
	...
	> [AZURE.NOTE]При использовании кластера __HBase__ также необходимо добавить следующий шаблон, так как веб-интерфейс размещается на узлах zookeeper кластера:
	>
	> * __Имя шаблона__ — __zookeeper__
	> * __Шаблон URL-адреса__ — __* zookeeper *__
	>
	> При использовании кластера __Storm__ необходимо добавить следующие шаблоны, так как пользовательский интерфейс Storm ссылается на IP-адреса узлов рабочей роли при получении журналов. Мы работаем над тем, чтобы в будущих обновлениях здесь использовалось имя домена.
	>
	> * __Имя шаблона__ — __nodesbyip__
	> * __Шаблон URL-адреса__ — __* 10.0.0.*__
...
3. После появления информации о службе YARN выберите __Быстрые ссылки__. Появится список головных узлов кластера. Выберите один из головных узлов, а затем выберите __Пользовательский интерфейс ResourceManager__.
	...
	> [AZURE.NOTE]Если подключение к Интернету медленное или головной узел занят, вы можете увидеть индикатор ожидания вместо меню при выборе меню __Быстрые ссылки__. В этом случае подождите одну-две минуты для получения данных с сервера, а затем повторите попытку.
...
4. Откроется страница, аналогичная следующей:
	...
	> [AZURE.TIP]Обратите внимание на URL-адрес этой страницы; он должен быть похож на __http://headnode1.CLUSTERNAME-ssh.j8.internal.cloudapp.net:8088/cluster__. Этот адрес использует полное внутреннее доменное имя узла (FQDN), и он недоступен без использования туннеля SSH.
