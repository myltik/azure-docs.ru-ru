---
title: Использование Apache Storm с Power BI | Microsoft Docs
...
> [!WARNING]
> Перед запуском топологии в кластере HDInsight следует сначала создать таблицу в базе данных SQL.
> 
> 
...
3. В обозревателе объектов щелкните базу данных правой кнопкой мыши и **создайте запрос**. Вставьте в окно запроса содержимое из файла **IISLogsTable.sql**, входящего в состав скачанного проекта, а затем нажмите клавиши Ctrl+Shift+E, чтобы выполнить этот запрос. Должно появиться сообщение о том, что выполнение команд успешно завершено.
   
    После этого в базе данных появится новая таблица с именем **IISLOGS**.

## Настройка примера
1. На [портале Azure](https://portal.azure.com) выберите свою базу данных SQL. В разделе **Основные компоненты** колонки базы данных SQL выберите элемент **Показать строки подключения к базе данных**. В появившемся списке скопируйте сведения об **ADO.NET (проверка подлинности SQL)**.
2. Откройте пример в Visual Studio. В **обозревателе решений** откройте файл **App.config** и найдите в нем следующий элемент:
    ...
    Замените значение **##TOBEFILLED##** строкой подключения к базе данных, которую вы скопировали на предыдущем шаге. Замените строки **{your\_username}** и **{your\_password}** именем пользователя и паролем для базы данных.
...
6. В области **Поля** разверните запись **IISLOGS**. Установите флажок напротив **URISTEM**. Немедленно будет создан отчет, в котором перечислены все ресурсы URI (/foo, /bar и т. д.), сохраненные в базе данных.
...
10. Теперь отчет имеет тот вид, который нам нужен. Щелкните пункт меню **Сохранить** и введите имя отчета, чтобы сохранить отчет.
