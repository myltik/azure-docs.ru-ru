...
Microsoft R Server в кластере Azure HDInsight предоставляет новейшие возможности для анализа на основе R. Это решение использует данные, хранящиеся в контейнере на файловой системе HDFS в вашей учетной записи хранения [BLOB-объектов Azure](../storage/storage-introduction.md "BLOB-объектов Azure storage") или локальной файловой системе Linux. Так как R Server основывается на R с открытым кодом, вы сможете использовать в своих приложениях на основе R любые из более чем 8000 пакетов R с открытым кодом. Также вы можете использовать подпрограммы [ScaleR](http://www.revolutionanalytics.com/revolution-r-enterprise-scaler "ScaleR от компании Revolution Analytics")из пакета аналитики корпорации Майкрософт для больших объемов данных, который предоставляется вместе с R Server.  
