...
Сейчас HDInsight предоставляет 4 различных типа кластера, каждый из которых имеет набор компонентов для предоставления определенных функциональных возможностей.
...
С помощью [действий сценария](#customize-clusters-using-script-action) к этим базовым типам вы можете добавить другие компоненты, например Hue или R.
...
- **Источник данных**
    ...
    Наряду с хранилищем BLOB-объектов Azure в качестве учетной записи хранения по умолчанию для кластера HBase в HDInsight, а также в качестве связанного хранилища для всех 4 типов кластера HDInsight можно использовать [хранилище озера данных Azure](data-lake-store-overview.md). Инструкции см. в разделе [Создание кластера HDInsight с хранилищем озера данных с помощью портала Azure](../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md).
...
- **Ценовые категории узлов**
	...
	При настройке кластера на портале Azure размер узла можно выбрать в колонке __Ценовая категория узла__. В ней также отображаются цены для узлов разных размеров. На следующем рисунке показаны варианты для кластера Hadoop под управлением Linux:
    ...
    Рекомендации по развертыванию, которые нужно учитывать, если вы планируете использовать эти ресурсы, приведены в разделе [Размеры виртуальных машин](../virtual-machines/virtual-machines-size-specs.md). Сведения о ценах для различных размеров см. в разделе [Расценки для HDInsight](https://azure.microsoft.com/pricing/details/hdinsight).
...
Дополнительную информацию об использовании вторичных хранилищ больших двоичных объектов см. в статье [Использование хранилища больших двоичных объектов Azure с HDInsight](hdinsight-hadoop-use-blob-storage.md). Дополнительные сведения об использовании дополнительного хранилища озера данных см. в разделе [Создание кластеров HDInsight с хранилищем озера данных на портале Azure](../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md).
...
Дополнительные сведения об использовании HDInsight в виртуальной сети, включая требования к конфигурации виртуальной сети, см. в разделе [Расширение возможностей HDInsight с помощью виртуальной сети Azure](hdinsight-extend-hadoop-virtual-network.md).
...
| Используйте этот метод для создания кластеров... | Используйте веб-браузер... | Используйте командную строку | Используйте API REST | Используйте пакет SDK | В ОС Linux, Mac OS X и Unix | В ОС Windows |
| ------------------------------- |:----------------------:|:--------------------:|:------------------:|:------------:|:-----------------------------:|:------------:|
