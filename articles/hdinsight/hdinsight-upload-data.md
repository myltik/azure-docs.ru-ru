---
title: Отправка данных для заданий Hadoop в HDInsight | Microsoft Docs
...
1. Запустите обозреватель хранилищ Azure. Если вы запускаете Storage Explorer в первый раз, вам будет предложено ввести ***имя учетной записи хранения*\_ и __ключ учетной записи хранения**. Если вы уже запускали его раньше, нажмите кнопку *_Добавить\*\_, чтобы добавить новые имя и ключ учетной записи хранения.
   
    Введите имя и ключ учетной записи хранения, используемые вашим кластером HDinsight, а затем нажмите **СОХРАНИТЬ И ОТКРЫТЬ**.
...
4. Укажите файл для отправки и щелкните **Открыть**. При появлении запроса выберите **Отправить** и отправьте файл в корневой каталог контейнера хранилища. Если требуется передать файл по конкретному пути, введите путь в поле **Место назначения**, а затем нажмите **Отправить**.
