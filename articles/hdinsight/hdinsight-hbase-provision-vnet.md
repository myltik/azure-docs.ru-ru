...
3. В разделе **Выбор модели развертывания** щелкните **Классическая** и нажмите кнопку **Создать**.
	...
	>[AZURE.NOTE]Кластер HDInsight на основе Windows можно развернуть только в классической виртуальной сети.
...
3. Введите или выберите следующие значения:
  ...
  - **Data Source** — укажите существующую или создайте новую учетную запись хранения Azure, которая будет использована в качестве файловой системы по умолчанию для кластера. По умолчанию имя контейнера по умолчанию является именам кластера. Расположение учетной записи хранения также определяет расположение кластера.
  - **Ценовая категория узла** — для целей обучения или тестирования выберите узел региона 1, чтобы уменьшить стоимость.
