...
- **Secure Shell** — отображение сведений, необходимых для подключения к кластеру по протоколу SSH.
	...
	- **Теги** (![значок "теги"](./media/hdinsight-administer-use-portal-linux/tags.png)). Теги позволяют задавать пары «ключ-значение» для определения пользовательской таксономии облачных служб. Например, можно создать ключ с именем __project__, а затем использовать общее значение для всех служб, связанных с определенным проектом.
	- **Просмотры Ambari** — отображение ссылок на веб-интерфейс Ambari.
...
5. Щелкните **Параметры**.
    ...
    - **Действия скриптов** — выполнение bash-скриптов в кластере.
	...
	- **Пользователи** — здесь можно задать разрешения на доступ к _порталу управления_ этого кластера для других пользователей в своей подписке Azure.
	- **Теги** — с помощью тегов можно задать пары "ключ-значение" для определения пользовательской таксономии облачных служб. Например, можно создать ключ с именем __project__, а затем использовать общее значение для всех служб, связанных с определенным проектом.
...
1. Войдите в веб-интерфейс Ambari с помощью учетных данных пользователя кластера HDInsight. Имя пользователя по умолчанию — **admin**. URL-адрес — **https://<HDInsight Cluster Name>azurehdinsight.net**.
...
4. В колонке __Действия скрипта__ выберите __Отправить новое__. Когда появится колонка __Отправка действия скрипта__, введите следующие сведения.
...
**Поиск идентификаторов подписки**

1. Перейдите на [портал][azure-portal].
2. В меню слева щелкните **Просмотреть все** и выберите пункт **Подписки**. У всех подписок есть имя и идентификатор.

Каждый кластер привязывается к подписке Azure. Идентификатор подписки показан на элементе кластера **Базовая**. См. раздел [Отображение кластеров](#list-and-show-clusters).
...
В колонке кластера HDInsight в разделе __Использование__ отображаются сведения о количестве ядер, доступных для HDInsight в рамках вашей подписки, о количестве ядер, выделенных для этого кластера, и их распределении между узлами в кластере. См. раздел [Отображение кластеров](#list-and-show-clusters).
