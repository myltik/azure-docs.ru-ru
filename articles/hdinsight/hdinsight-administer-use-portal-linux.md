...
9. Выберите __Необязательная конфигурация__. В этой колонке можно настроить следующие элементы.
	...
	* __Действия скрипта__. Позволяет указывать скрипты Bash, используемые для настройки кластера HDInsight во время подготовки его к работе. Например, есть [скрипт, выполняющий установку Hue](hdinsight-hadoop-hue-linux.md) (графический клиент для работы с Hadoop). Дополнительные сведения о действиях скрипта см. в статье [Настройка кластеров HDInsight с помощью действия скрипта](hdinsight-hadoop-customize-cluster-linux.md).
...
10. Обязательно выберите параметр __Закрепить на начальной панели__, а затем — __Создать__. После этого кластер будет создан, а на начальную панель портала Azure будет добавлена его плитка. Значок указывает, что выполняется подготовка кластера. После завершения подготовки вместо него будет отображаться значок HDInsight.
	...
	> [AZURE.NOTE]Обычно создание кластера занимает около 15 минут. Отслеживать процесс подготовки к работе можно с помощью плитки на начальной панели или записи __Уведомления__ в левой части страницы.
...
* __Параметры__ и __Все параметры__. Отображают колонку __Параметры__ кластера, которая содержит подробные сведения о его конфигурации.

* __Панель мониторинга__, __Панель мониторинга кластера__ и __URL-адрес__ — три способа доступа к панели мониторинга кластера, которая представляет собой веб-интерфейс Ambari для кластеров под управлением Linux.
...
* __Быстрый старт__ (![значок облака и молнии = быстрый запуск](./media/hdinsight-administer-use-portal-linux/quickstart.png)). Отображает сведения, необходимые для начала работы с HDInsight.

* __Пользователи__ (![значок "пользователи"](./media/hdinsight-administer-use-portal-linux/users.png)). Здесь можно указать разрешения на доступ к _порталу управления_ этого кластера для других пользователей в вашей подписке Azure.
...
Дополнительные сведения об операциях масштабирования см. в статье [Сведения об использовании HDInsight в Linux](hdinsight-hadoop-linux-information.md#scaling).

## Мониторинг кластера

В разделе __Использование__ колонки кластера HDInsight отображаются сведения о количестве ядер, доступных для HDInsight в рамках вашей подписки, а также о количестве ядер, выделенных для этого кластера, и их распределении по узлам кластера.
