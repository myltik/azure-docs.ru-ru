...
9. Выберите __Необязательная конфигурация__. В этой колонке можно настроить следующие элементы.
	...
	* __Виртуальная сеть__. Позволяет разместить кластер HDInsight в виртуальной сети вместе с другими ресурсами, такими как база данных SQL или виртуальная машина Azure. Размещение ресурсов в виртуальной сети позволяет им взаимодействовать друг с другом напрямую, в обход открытых шлюзов, обрабатывающих входящий трафик из Интернета.
	...
	* __Действия сценариев__. Позволяют указывать сценарии Bash, используемые для настройки кластера HDInsight во время его подготовки к работе. Например, есть [скрипт, который устанавливает Hue](hdinsight-hadoop-hue-linux.md) (графический клиент для работы с Hadoop). Подробнее о действиях сценариев см. в статье [Настройка кластеров HDInsight с помощью действия сценария](hdinsight-hadoop-customize-cluster-linux.md).
