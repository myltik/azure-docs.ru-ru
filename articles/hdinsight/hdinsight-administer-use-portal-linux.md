...
7. Щелкните запись __Источник данных__, чтобы выбрать существующий или создать новый источник данных.

	![Колонка "Источник данных"](./media/hdinsight-administer-use-portal-linux/datasource.png)

	В настоящее время в качестве источника данных для кластера HDInsight можно выбрать учетную запись хранения Azure. Далее приводится описание элементов интерфейса, которое поможет вам лучше понять записи в колонке __Источник данных__.
...
8. Выберите __Ценовые категории узла__, чтобы отобразить сведения об узлах, которые будут созданы для этого кластера. По умолчанию выбрано __4__ рабочих узла.
...
9. Выберите __Необязательная конфигурация__. В этой колонке можно настроить следующие элементы.
	...
	* __Внешние метахранилища__. Здесь вы можете выбрать базу данных SQL, в которой будут храниться сведения о конфигурации для Oozie и Hive. Благодаря метахранилищу вы сможете повторно использовать конфигурацию после удаления или повторного создания кластера, а не создавать ее каждый раз заново.

	* __Виртуальная сеть__. Позволяет размещать кластер HDInsight в виртуальной сети вместе с другими ресурсами, такими как база данных SQL или виртуальная машина Azure. Размещение ресурсов в виртуальной сети позволяет им взаимодействовать друг с другом напрямую, в обход открытых шлюзов, обрабатывающих входящий трафик из Интернета. Дополнительные сведения о преимуществах использования виртуальных сетей Azure в HDInsight см. в статье [Расширение возможностей HDInsight с помощью виртуальной сети Azure](hdinsight-extend-hadoop-virtual-network.md).
	...
	* __Действия скриптов__. Позволяет указывать скрипты Bash, используемые для настройки кластера HDInsight во время его подготовки. Например, есть [скрипт, который устанавливает Hue](hdinsight-hadoop-hue-linux.md) (графический клиент для работы с Hadoop). Дополнительную информацию о действиях скриптов см. в статье [Настройка кластеров HDInsight с помощью действия скрипта](hdinsight-hadoop-customize-cluster-linux.md).

	* __Ключи хранилища Azure__. Позволяет связать дополнительные учетные записи хранения с сервером HDInsight.
...
10. Убедитесь, что флажок __Закрепить на начальной панели__ установлен, а затем нажмите кнопку __Создать__. После этого кластер будет создан, а на начальную панель портала Azure будет добавлена его плитка. Значок указывает, что выполняется подготовка кластера. После завершения подготовки вместо него будет отображаться значок HDInsight.
	...
	> [AZURE.NOTE]Обычно создание кластера занимает около 15 минут. Отслеживать процесс подготовки можно с помощью плитки на начальной панели или записи в разделе __Уведомления__ в левой части страницы.
...
Ниже описаны значки в верхней части этой колонки, а также в разделах __Основные компоненты__ и __Быстрые ссылки__.

* __Параметры__ и __Все параметры__. Отображают колонку __Параметры__ кластера, в которой представлены подробные сведения о конфигурации кластера.

* __Панель мониторинга__, __Панель мониторинга кластера__ и __URL-адрес__ — три пути доступа к панели мониторинга кластера, которая представляет собой веб-интерфейс Ambari для кластеров под управлением Linux.

* __Secure Shell__ — сведения, необходимые для доступа к кластеру с помощью протокола SSH.

* __Масштабировать кластер__. Позволяет изменить количество рабочих узлов для этого кластера.

* __Удалить__. Удаляет кластер HDInsight.
...
* __Пользователи__ (![значок "пользователи"](./media/hdinsight-administer-use-portal-linux/users.png)). Здесь можно задать разрешения на доступ к _порталу управления_ этим кластером для других пользователей в вашей подписке Azure.
...
Для масштабирования кластера с помощью портала выберите кластер HDInsight, а затем нажмите кнопку __Масштабировать кластер__. Введите __количество рабочих узлов__, которое вы хотите задать для кластера, и нажмите кнопку __Сохранить__.

![колонка масштабирования](./media/hdinsight-administer-use-portal-linux/scaling.png)

Дополнительные сведения о масштабировании операций см. в разделе [Сведения об использовании HDInsight в Linux](hdinsight-hadoop-linux-information.md#scaling).

## Мониторинг кластера

В разделе __Использование__ колонки кластера HDInsight отображаются сведения о количестве ядер, доступных для использования с HDInsight в рамках вашей подписки, а также о количестве ядер, выделенных для этого кластера, и их распределении по узлам кластера.
