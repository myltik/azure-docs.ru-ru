...
7. Выберите __Ценовые категории узла__, чтобы отобразить сведения об узлах, которые будут созданы для этого кластера. По умолчанию для количества рабочих узлов устанавливается значение __4__.
...
8. Выберите __Необязательная конфигурация__. В этой колонке можно настроить следующие элементы.
	...
	* __Внешние метахранилища__: здесь вы можете выбрать Базу данных SQL для хранения сведений о конфигурации Oozie и Hive. Так вы сможете повторно использовать конфигурацию после удаления или повторного создания кластера, а не создавать ее заново каждый раз. **__Ключи хранилища Azure__: здесь вы можете связать дополнительные учетные записи хранения с сервером HDInsight.
...
* __Параметры__ и __Все параметры__: отображение колонки __Параметры__ кластера, в которой представлены подробные сведения о конфигурации кластера.

* __Панель мониторинга__, __Панель мониторинга кластера__ и __URL-адрес__: три способа доступа к панели мониторинга кластера, которая представляет собой интерфейс Ambari Web для кластеров под управлением Linux.

* __Secure Shell__: сведения, необходимые для доступа к кластеру с помощью протокола SSH.

* __Масштабировать кластер__: здесь можно изменить число рабочих узлов для этого кластера.

* __Удалить__: удаление кластера HDInsight.

* __Быстрый запуск (![значок облака и молнии = быстрый запуск](./media/hdinsight-administer-use-portal-linux/quickstart.png))__: отображение сведений, необходимых для начала работы с HDInsight.

* __Пользователи (![значок "пользователи"](./media/hdinsight-administer-use-portal-linux/users.png))__: здесь можно задать разрешения на _управление_ этим кластером для других пользователей в подписке Azure.

	> [AZURE.IMPORTANT]Этот параметр влияет _только_ на доступ к этому кластера и соответствующие разрешения на портале предварительной версии Azure. Он не определяет пользователей, которые могут подключаться к кластеру HDInsight или отправлять в него задания.

* __Теги (![значок тега](./media/hdinsight-administer-use-portal-linux/tags.png))__: здесь можно задать пары "ключ — значение" для определения пользовательской таксономии облачных служб. Например, можно создать ключ с именем __project__, а затем использовать общее значение для всех служб, связанных с определенным проектом.
...
В разделе __Использование__ в колонке кластера HDInsight отображаются сведения о количестве ядер, доступных в вашей подписке для использования с HDInsight, а также количество ядер, выделенных для этого кластера, и их распределение на узлах кластера.
