...
Чтобы выполнить действия, описанные в этой статье, необходима **подписка Azure**. Ознакомьтесь с [бесплатной пробной версией Azure](https://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).
...
4. Щелкните кластер в списке, чтобы просмотреть страницу с общими сведениями:
    ...
    * **SSH + Cluster login** (SSH и вход в кластер): отображение сведений, необходимых для подключения к кластеру по протоколу SSH. Ознакомьтесь с дополнительными сведениями в статье [Подключение к HDInsight (Hadoop) с помощью SSH](hdinsight-hadoop-linux-use-ssh-unix.md) и сбросьте учетные данные для входа в кластер.
...
6. Щелкните **Свойства**.
   ...
   * **Виртуальная сеть.** Имя виртуальной сети и подсети, в которой развернут кластер, если она была выбрана во время развертывания.
