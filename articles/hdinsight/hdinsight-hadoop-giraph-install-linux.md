...
5. После завершения задания результаты будут сохранены в каталоге __wasb:///example/out/shotestpaths__. Созданные файлы будут начинаться с __part-m-\_\_ и заканчиваться числом, указывающим номер файла (первый, второй и т. д.). Для просмотра выходных данных используйте следующую команду:
