---
title: "Установка RStudio с R Server в кластере HDInsight | Документация Майкрософт"
...
Сегодня для R существует несколько интегрированных сред разработки (IDE), включая недавно заявленное корпорацией Майкрософт решение [R Tools for Visual Studio](https://www.visualstudio.com/en-us/features/rtvs-vs.aspx) (RTVS), семейство локальных и серверных инструментов от [RStudio](https://www.rstudio.com/products/rstudio-server/) или [StatET](http://www.walware.de/goto/statet) на основе Eclipse от Walware. Чаще всего в Linux используется [RStudio Server](https://www.rstudio.com/products/rstudio-server/) — IDE на основе браузера, предназначенная для удаленных клиентов.  Установка RStudio Server на граничном узле кластера HDInsight предоставляет доступ ко всем функциональным возможностям IDE для разработки и выполнения R-скриптов с использованием R Server в кластере и может значительно повысить производительность по сравнению со стандартным применением R-консоли.

> [!NOTE]
> Процедура, описанная в этой статье, подходит только в том случае, если вы не выбрали установку выпуска RStudio Server для сообщества при подготовке кластера.  Если вы добавили сервер RStudio Server во время подготовки, то для доступа к нему можете щелкнуть элемент **Панели мониторинга R Server** в записи портала Azure для своего кластера, а затем щелкнуть элемент **R Studio Server**. 
...
> [!NOTE]
> Для выполнения действий, описанных в этом документе, требуется сервер R Server в кластере HDInsight. Они не будут работать в кластере HDInsight со средой R, установленной на основе действий, описанных в статье [Установка и использование R на кластерах HDInsight Hadoop](hdinsight-hadoop-r-scripts-linux.md).
>
> 
...
7. Создайте туннель SSH для кластера, сопоставив `ssh -L localhost:8787:localhost:8787 USERNAME@CLUSTERNAME-ed-ssh.azurehdinsight.net` в кластере HDInsight с клиентским компьютером. Туннель SSH следует создать перед открытием нового сеанса браузера.
