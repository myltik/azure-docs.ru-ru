...
> [!NOTE]
> Если сбой узла происходит в процессе анализа данных, ход выполнения этого задания будет утрачен. Задание, выполняемое неисправным узлом, будет передано другому узлу.
> 
> 

В следующих разделах рассматриваются отдельные типы узлов, которые используются с HDInsight. Не все типы узлов используются для определенного типа кластера. Например, тип кластера Hadoop не будет содержать узлы Nimbus. Дополнительные сведения об узлах, используемых типами кластера HDInsight, см. в разделе "Типы кластеров" статьи [Создание кластеров Hadoop под управлением Linux в HDInsight](hdinsight-hadoop-provision-linux-clusters.md#cluster-types).
...
> [!IMPORTANT]
> Числовое значение не зависит от того, каким является узел: основным или дополнительным. Это значение служит только для определения уникального имени каждого узла.
> 
> 
...
[ZooKeeper](http://zookeeper.apache.org/) (ZK) используются для выбора лидера основных служб на головных узлах, а также для уведомления служб, узлов данных (рабочих узлов) и шлюзов о том, на каком головном узле активна основная служба. По умолчанию HDInsight предоставляет три узла ZooKeeper.
...
При подключении к кластеру с помощью SSH через порт 22 (порт SSH по умолчанию) выполняется подключение к основному головному узлу, а при подключении через порт 23 — к дополнительному головному узлу. Например, `ssh username@mycluster-ssh.azurehdinsight.net` подключится к основному головному узлу кластера с именем **mycluster**.

> [!NOTE]
> Это также касается протоколов, основанных на SSH, например протокола передачи файлов SSH (SFTP).
> 
> 

К граничному узлу с R Server в кластерах HDInsight также можно получать непосредственный доступ с помощью SSH через порт 22. Например, `ssh username@RServer.mycluster.ssh.azurehdinsight.net` подключится к граничному узлу для R Server в кластере HDInsight с именем **mycluster**. 
...
* **SSH.** После подключения к головному узлу с помощью SSH можно затем использовать SSH с головного узла для подключения к другим узлам в кластере.
* **Туннель SSH.** Если нужен доступ к веб-службе, размещенной на неподключенном к Интернету узле, следует [использовать туннель SSH](hdinsight-linux-ambari-ssh-tunnel.md).
* **Виртуальная сеть Azure.** Если кластер HDInsight входит в состав виртуальной сети Azure, любой ресурс в той же виртуальной сети может напрямую получать доступ ко всем узлам в кластере.
