---
title: Функции высокой доступности кластера HDInsight (Hadoop) под управлением Linux | Microsoft Docs
...
* **Туннель SSH**: если необходим доступ к веб-службе, размещенной на неподключенном к Интернету узле, следует [использовать туннель SSH](hdinsight-linux-ambari-ssh-tunnel.md).
* **Виртуальная сеть Azure**: если кластер HDInsight является частью виртуальной сети Azure, любой ресурс в той же виртуальной сети может напрямую получать доступ ко всем узлам в кластере.
