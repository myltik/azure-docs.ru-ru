...
1. Выберите кластер Spark на портале Azure, затем на панели задач портала в нижней экрана щелкните элемент **Панель мониторинга Spark**.

2. В верхней области панели мониторинга щелкните вкладку **Диспетчер ресурсов**.

##<a name="scenariosrm"></a>Как устранить возникающие проблемы, используя диспетчер ресурсов?
...
### Я использую с кластером Spark только IPython. Как мне выделить все ресурсы для этого интерфейса?
...
2. На панели мониторинга HDInsight Spark щелкните вкладку **Диспетчер ресурсов** и задайте значения для параметров **Число ядер приложения по умолчанию** и **Память исполнителя по умолчанию на каждый рабочий узел**. Для других свойств задайте значение 0.
