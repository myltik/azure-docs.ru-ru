---
title: Управление кластерами Hadoop в HDInsight с помощью портала Azure | Microsoft Docs
...
4. Дважды щелкните кластер в списке, чтобы просмотреть подробные сведения о нем.
   ...
   * **Панель мониторинга**, **панель мониторинга кластера** и **URL-адрес: это все способы доступа к панели мониторинга кластера, являющейся веб-интерфейсом Ambari для кластеров на основе Linux. **Secure Shell**: отображает инструкции для подключения к кластеру с помощью подключения SSH (Secure Shell).
   ...
   * **Пользователи** (![значок "пользователи"](./media/hdinsight-administer-use-portal-linux/users.png)). Здесь можно указать разрешения на доступ к *порталу управления* этого кластера для других пользователей в своей подписке Azure.
     ...
     > 
     > 
   * **Теги** (![значок "теги"](./media/hdinsight-administer-use-portal-linux/tags.png)). Теги позволяют задавать пары «ключ-значение» для определения пользовательской таксономии облачных служб. Например, вы можете создать ключ с именем **project**, а затем использовать общее значение для всех служб, связанных с определенным проектом.
   * **Просмотры Ambari**: ссылки на веб-интерфейс Ambari.
     
     > [!IMPORTANT]
     > Для управления службами, предоставляемыми кластером HDInsight, необходимо использовать интерфейс Ambari Web или API REST Ambari. Дополнительные сведения об использовании Ambari см. в статье [Управление кластерами HDInsight с помощью Ambari (предварительная версия)](hdinsight-hadoop-manage-ambari.md).
     > 
     > 
...
> [!NOTE]
> Поддерживаются только кластеры HDInsight версии 3.1.3 или более поздней. Если вы не знаете версию кластера, см. страницу «Свойства». См. раздел [Отображение кластеров](hdinsight-adminster-use-management-portal.md#list-and-show-clusters).
> 
> 
...
> [!IMPORTANT]
> Для мониторинга служб, предоставляемых кластером HDInsight, необходимо использовать интерфейс Ambari Web или API REST Ambari. Дополнительные сведения об использовании Ambari см. в статье [Управление кластерами HDInsight с помощью Ambari (предварительная версия)](hdinsight-hadoop-manage-ambari.md).
> 
> 
