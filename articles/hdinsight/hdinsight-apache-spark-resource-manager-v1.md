...
# Управление ресурсами кластера Apache Spark в HDInsight на платформе Windows (предварительная версия)
...
2. В колонке кластера Spark нажмите **Панель мониторинга**. При появлении запроса введите учетные данные администратора для кластера Spark.
	...
	![Запуск диспетчера ресурсов](./media/hdinsight-apache-spark-resource-manager-v1/hdispark.cluster.launch.dashboard.png "Начало работы с диспетчером ресурсов")
...
1. В разделе **Панель мониторинга Spark** щелкните вкладку **Пользовательский интерфейс Spark**, чтобы определить максимальное количество ядер и максимальный объем ОЗУ, которые можно выделить для приложений.
	...
	![Выделение ресурсов](./media/hdinsight-apache-spark-resource-manager-v1/hdispark.ui.resource.png "Поиск ресурсов, выделенных для кластера Spark")
	...
	Следует также учитывать любые выполняющиеся приложения. Просмотреть запущенные приложения вы можете на вкладке **Пользовательский интерфейс Spark**.

	![Выполняющиеся приложения](./media/hdinsight-apache-spark-resource-manager-v1/hdispark.ui.running.apps.png "приложения, выполняемые на кластере;")
...
2. На панели мониторинга Spark в HDInsight щелкните вкладку **Диспетчер ресурсов** и задайте значения для параметров **Число ядер приложения по умолчанию** и **Память исполнителя по умолчанию на каждый рабочий узел**. Для других свойств задайте значение 0.
	...
	![Выделение ресурсов](./media/hdinsight-apache-spark-resource-manager-v1/hdispark.ui.allocate.resources.png "Выделение ресурсов для приложений")
...
![Выделение ресурсов](./media/hdinsight-apache-spark-resource-manager-v1/hdispark.ui.no.thrift.png "Серверу Thrift не выделены ресурсы")
