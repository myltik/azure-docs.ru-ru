...
При создании кластера с использованием командлетов HDInsight Windows PowerShell или пакета SDK HDInsight .NET можно выбрать версию для кластера HDInsight Hadoop с помощью раскрывающегося списка **Версия HDInsight** в колонке **Необязательная конфигурация** на портале Azure.
