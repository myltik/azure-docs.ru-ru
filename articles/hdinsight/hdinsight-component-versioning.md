---
title: "Что представляют собой различные компоненты, доступные в кластере HDInsight? | Документация Майкрософт"
...
---
# <a name="what-are-the-different-hadoop-components-available-with-hdinsight"></a>Что представляют собой различные компоненты Hadoop, доступные в HDInsight?
...
### <a name="cluster-types-supported-for-premium"></a>Типы кластеров, поддерживаемые для предложения "Премиум"
...
| --- | --- | --- |
| Hadoop |Да |Да (только HDInsight 3.5) |
| Spark |Да |Нет |
| HBase |Да |Нет |
| Storm |Да |Нет |
| Interactive Hive (предварительная версия) |Да |Нет |
| R Server (предварительная версия) |Да |Нет |
...
> [!NOTE]
> Версия по умолчанию из службы может быть изменена без уведомления. Рекомендуется указать версию при создании кластеров с использованием пакета SDK для .NET, Azure PowerShell и интерфейса командной строки Azure, если используется зависимость версии. 
> 
> 

| Компонент | HDInsight версии 3.5 | HDInsight версии 3.4 (по умолчанию) | HDInsight версия 3.3 | HDInsight версия 3.2 | HDInsight версия 3.1 | HDInsight версия 3,0 |
| --- | --- | --- | --- | --- | --- | --- |
...
| Apache Spark |1.6.2 + 2.0 (только для Linux) |1.6.0 (только для Linux) |1.5.2 (только для Linux, экспериментальная сборка) |1.3.1 (только для Windows) | | |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HDI 3.5 |HDP 2.5 |Ubuntu 16 |Да |30.09.2016 |Да | | |
...
##<a name="hdi-version-32-and-33-nearing-deprecation-date"></a>Приближается дата прекращения сопровождения HDI версий 3.2 и 3.3
Поддержка кластеров HDI 3.2 истекла 1.03.2016, и они будут считаться нерекомендуемыми с 1.04.2017. Поддержка кластеров HDI 3.3 истекла 27.06.2016, и они будут считаться нерекомендуемыми с 31.07.2017. Если вы используете кластер HDI 3.2 или HDI 3.3, то в ближайшее время обновите его до кластера HDI 3.5 (последняя версия). 
...
##<a name="hdinsight-deprecation-on-windows"></a>Нерекомендуемые версии HDInsight в Windows
Начиная с HDI версии 3.4, мы выпускали HDInsight только в операционной системе Linux. Некоторые предложения для HDInsight доступны только для Linux: Apache Ranger, приложения HDInsight, Azure Data Lake Store в качестве основной FS и т. д. Это дает клиентам несколько преимуществ.

* Мы можем быстрее вывести на рынок технологию обработки больших данных с открытым кодом через службу HDInsight.
* Существуют большое сообщество и экосистема, где можно получить поддержку.
* Активная разработка сообществом разработчиков ПО с открытым кодом для Hadoop и новые технологии обработки больших данных. 
* Служба HDInsight может уделять больше внимания технологии обработки больших данных с открытым кодом. 

В целях дальнейшего инвестиции в технологии обработки больших данных с открытым кодом будущие выпуски HDInsight будут доступны только в операционной системе Linux. В дальнейшем не будет выпусков HDInsight для ОС Windows. Последний выпуск HDInsight на платформе Windows — HDI 3.3. Поддержка выпуска HDI 3.3 истекла 27.06.2016, и он будет считаться нерекомендуемым с 31.07.2017. Чтобы выполнить миграцию из кластера HDInsight под управлением Windows на кластер под управлением Linux, ознакомьтесь с этой [информацией](https://docs.microsoft.com/en-gb/azure/hdinsight/hdinsight-migrate-from-windows-to-linux).
