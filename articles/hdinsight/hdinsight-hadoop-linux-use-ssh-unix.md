---
title: Использование ключей SSH с Hadoop на платформе Linux из Linux, Unix или OS X | Microsoft Docs
...
* **Адрес SSH**. К кластеру можно подключиться, используя один из двух адресов SSH.
  ...
  * **Подключение к граничному узлу**. Если используется кластер сервера R в HDInsight, кластер также будет содержать граничный узел, доступ к которому можно получить с помощью адреса **RServer.CLUSTERNAME.ssh.azurehdinsight.net**, где **CLUSTERNAME** — имя кластера.
