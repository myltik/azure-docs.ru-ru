...
---
# <a name="use-ssh-with-hdinsight-hadoop-from-bash-on-windows-10-linux-unix-or-os-x"></a>Использование SSH с HDInsight (Hadoop) из Bash на платформе Windows 10, Linux, Unix или OS X
...
Windows 10 Anniversary Edition предоставляет Bash в качестве компонента разработчика. Он предоставляет `ssh`, `scp` и другие команды Linux. Дополнительные сведения о Bash Windows 10 см. в разделе [Bash on Ubuntu on Windows](https://msdn.microsoft.com/commandline/wsl/about) (Bash для Ubuntu в Windows).
...
* **Закрытый ключ** — это ключ, который передается кластеру HDInsight, чтобы идентифицироваться, при входе с использованием SSH-клиента. Его нужно защищать и нельзя никому предоставлять.
    ...
    Вы можете повысить уровень безопасности, создав парольную фразу для закрытого ключа. Если вы используете парольную фразу, то необходимо ввести ее при аутентификации с помощью SSH.
...
* __id\_rsa__ — здесь содержится закрытый ключ.
    ...
    > [!WARNING]
    > Ограничьте доступ к этому файлу, чтобы предотвратить несанкционированное использование служб, защищенным открытым ключом.
...
* Новых пользователей SSH необходимо добавить в каждый узел в кластере.
...
##<a id="connect"></a>Подключение к HDInsight
...
Рабочие узлы и узлы ZooKeeper недоступны напрямую за пределами кластера, но к ним можно получить доступ из головного или граничного узла кластера. Шаги ниже описывают общую процедуру подключения к другим узлам.
...
1. Подключитесь к присоединенному к домену кластеру HDInsight через SSH. Например, следующая команда устанавливает подключение к кластеру HDInsight __myhdi__, используя учетную запись SSH с именем __sshuser__.
...
Протокол SSH может использоваться для туннелирования локальных запросов, например веб-запросов, к кластеру HDInsight. Запрос перенаправляется в кластер, а затем разрешается в нем.
