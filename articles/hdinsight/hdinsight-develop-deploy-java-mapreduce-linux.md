...
##<a name="<a-name="prerequisites"></a>prerequisites"></a><a name="prerequisites"></a>Предварительные требования
...
1. Отредактируйте файл __pom.xml__ и добавьте в раздел `<dependencies>` следующий код:
...
2. Добавьте в файл __pom.xml__ следующий код: Эти строки должны находиться в файле внутри тегов `<project>...</project>`; например между `</dependencies>` и `</project>`.
...
3. После завершения выполнения команды в каталог __wordcountjava/target__ будет содержать файл с именем __wordcountjava-1.0-SNAPSHOT.jar__.
...
##<a name="<a-id="upload"></a>upload-the-jar"></a><a id="upload"></a>Загрузка файла JAR
...
##<a name="<a-name="run"></a>run-the-mapreduce-job"></a><a name="run"></a>Выполнение задания MapReduce
...
2. В сеансе SSH используйте следующую команду для запуска приложения MapReduce:
    ...
    Она будет использовать приложение WordCount MapReduce, чтобы подсчитать слова в файле davinci.txt и сохранить результаты в __wasbs:///example/data/wordcountout__. Файлы ввода и вывода хранятся в хранилище кластера по умолчанию.
