...
3. Так как записная книжка была создана с помощью ядра PySpark, задавать контексты явно необязательно. Контексты Spark, SQL и Hive будут созданы автоматически при выполнении первой ячейки кода. Вы можете приступить к созданию своего приложения машинного обучения, импортировав типы, необходимые для этого сценария. Для этого наведите курсор на ячейку и нажмите клавиши **SHIFT+ВВОД**.
...
4. Теперь у нас есть *таблица данных*, `df`, на основе которой можно выполнять анализ. У нас также есть временный табличный вызов **CountResults**. Она состоит из 4 столбцов: **id**, **name**, **results** и **violations**.
...
2. Простой способ понять, каким образом распределены результаты, — применить визуализацию. Во временной таблице **CountResults** уже есть данные. Чтобы лучше понять, как распределяются результаты, можно выполнить следующий SQL-запрос к таблице.
	...
	Волшебное слово `%%sql`, за которым следует `-o countResultsdf`, гарантирует, что вывод запроса сохраняется локально на сервере Jupyter (обычно это головной узел кластера). Выходные данные сохраняются в таблице данных [Pandas](http://pandas.pydata.org/) с именем **countResultsdf**.
...
3. Также можно создать диаграмму с помощью Matplotlib, библиотеки, используемой для визуализации данных. Поскольку диаграмма должна создаваться из локально сохраненной таблицы данных **countResultsdf**, фрагмент кода должен начинаться с волшебного слова `%%local`. Это гарантирует, что код будет выполняться локально на сервере Jupyter.
...
1. Приведенный ниже фрагмент кода создает новую таблицу данных, **predictionsDf**, содержащую сформированный моделью прогноз. Фрагмент кода также создает временную таблицу **Predictions** на основе таблицы данных.
...
Мы можем визуализировать итоговые данные, чтобы наглядно увидеть результаты теста.

1. Начнем с извлечения различных прогнозов и результатов из временной таблицы **Прогнозы**, созданной ранее.

		%%sql -o predictionstable
		SELECT prediction, results FROM Predictions

2. В приведенном выше фрагменте кода **predictionstable** — локальная таблица данных на сервере Jupyter, в которой сохраняются выходные данные SQL-запроса. Теперь вы можете использовать волшебное слово `%%local` для запуска последующих фрагментов кода для хранящейся локально таблицы данных.
...
3. Наконец, используйте следующий фрагмент кода для создания диаграммы.
