...
1. На начальной панели [портала предварительной версии Azure](https://portal.azure.com/) щелкните элемент кластера Spark (если он закреплен на начальной панели). Кроме того, можно перейти к кластеру, выбрав пункты **Просмотреть все** и **Кластеры HDInsight**.   

2. В колонке кластера Spark щелкните **Быстрые ссылки**, а затем в колонке **Панель мониторинга кластера** выберите **Записная книжка Jupyter**. При появлении запроса введите учетные данные администратора для кластера.
...
2. Создайте новую записную книжку. Щелкните **New** («Создать»), а затем выберите **Python 2**.
...
3. Приступайте к созданию приложения машинного обучения. Сначала нужно настроить среду Pyspark. Для этого наведите курсор на ячейку и нажмите клавиши **SHIFT+ВВОД**.
...
У нас уже есть SQLContext, который можно использовать для преобразования структурированных данных. Сначала нужно загрузить пример данных (**Food\_Inspections1.csv**) в *таблицу данных* SQL Spark. В фрагментах кода ниже предполагается, что данные уже отправлены в контейнер хранилища по умолчанию, который связан с кластером Spark.
...
4. Теперь у нас есть *таблица данных*, `df`, на основе которой можно выполнять анализ. Она состоит из 4 столбцов: **id**, **name**, **results** и **violations**. Выполните следующую команду, чтобы получить небольшой пример данных:
...
Давайте подробно рассмотрим содержимое набора данных. Например, что представляют собой различные значения в столбце **results**?
...
Простой способ понять, каким образом распределены результаты, — применить визуализацию.
...
Результаты проверки делятся на 5 различных категорий:
...
Приведенный ниже фрагмент кода создает новую таблицу данных, **predictionsDf**, содержащую сформированный моделью прогноз.
...
Чтобы просмотреть пример прогноза, выполните этот фрагмент кода:

	predictionsDf.take(1)

Появится прогноз для первой записи в тестовом наборе данных.

Метод `model.transform()` таким же образом преобразует новые данные и определит, как их классифицировать. Можно выполнить некоторые статистические расчеты, чтобы увидеть, насколько точны полученные прогнозы:
...
Используя логистическую регрессию Spark, мы создали точную модель, которая прогнозирует вероятность прохождения контроля качества пищевых продуктов для определенного предприятия на основе описаний нарушений на английском языке. Мы можем визуализировать итоговые данные, чтобы наглядно увидеть результаты теста:
...
На этой диаграмме «положительный» результат представляет собой непройденную проверку, а отрицательный — пройденную. Таким образом, ложно отрицательные результаты составляют (приблизительно) 12,6 %, а ложно положительные — 16,0 %.

## Завершение работы записной книжки

Завершив работу с приложением, следует закрыть записную книжку, чтобы освободить ресурсы. Для этого в записной книжке в меню **File** («Файл») выберите пункт **Close and Halt** («Закрыть и остановить»). Это позволит завершить работу и закрыть записную книжку.
