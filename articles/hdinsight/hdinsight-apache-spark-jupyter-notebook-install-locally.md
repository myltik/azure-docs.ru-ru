...
В этой статье вы узнаете, как установить записную книжку Jupyter с пользовательскими ядрами PySpark (для Python) и Spark (для Scala) с помощью волшебной команды Spark и как подключить записную книжку к кластеру HDInsight.
...
Дополнительные сведения о пользовательских ядрах и волшебной команде Spark для записных книжек Jupyter с кластерами HDInsight см. в статье [Ядра, доступные для записных книжек Jupyter с кластерами Apache Spark Linux в HDInsight](hdinsight-apache-spark-jupyter-notebook-kernels.md).
...
1. Скачайте [установщик Anaconda](https://www.continuum.io/downloads) для вашей платформы и запустите программу установки. В мастере установки укажите параметр для добавления Anaconda в переменную PATH.
...
1. Скачайте последнюю общедоступную предварительную версию волшебной команды Spark из [Github](https://github.com/jupyter-incubator/sparkmagic/archive/publicpreview0.5.zip).
...
2. Перейдите в домашний каталог и создайте папку с именем **.sparkmagic**, если ее нет.

3. В этой папке создайте файл **config.json** и добавьте в него следующий код JSON.
