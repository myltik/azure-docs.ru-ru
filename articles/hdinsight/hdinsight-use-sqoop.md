...
В этом разделе показано, как создать кластер и схемы базы данных SQL для выполнения заданий учебника с помощью портала Azure и шаблона Azure Resource Manager. Если вы предпочитаете использовать Azure PowerShell, см. [приложение A](#appendix-a---a-powershell-sample).
...
2. В колонке "Параметры" заполните следующие поля:
   ...
   * **Имя для входа на сервер базы данных SQL Azure и пароль**.
     ...
     | Имя учетной записи хранения по умолчанию | <Имя\_кластера>store |
     | --- | --- |
     | Имя сервера базы данных SQL Azure |<Имя\_кластера>dbserver |
     | Имя базы данных SQL Azure |<Имя\_кластера>db |
...
* **SQL Server**: если ваш кластер HDInsight находится в той же виртуальной сети Azure, что и SQL Server, эта статья поможет вам разобраться с импортом данных в базу данных SQL Server и их экспортом из нее.
  ...
  * Сведения о создании и настройке виртуальной сети см. в статье [Задачи настройки виртуальной сети](../services/virtual-machines.md).
