...
# Приступая к работе с Apache Kafka (предварительная версия) в HDInsight
...
6. В колонке __Приложения (необязательно)__ щелкните __Далее__. Для этого примера приложения не требуются.

7. В колонке __Размер кластера__ щелкните __Далее__.
...
8. В колонке __Дополнительные параметры__ щелкните __Далее__.

9. В колонке **Сводка** просмотрите конфигурацию для кластера. Используйте ссылки __Изменить__, чтобы изменить неправильные параметры. Наконец, нажмите кнопку "Создать" для создания кластера.
...
2. Используйте следующие команды, чтобы определить переменные среды на основе информации, полученной из Ambari. Замените __CLUSTERNAME__ именем кластера Kafka. Замените __PASSWORD__ паролем администратора, который использовался при создании кластера.
...
4. Когда команда `scp` закончит копирование файла, подключитесь к кластеру с помощью SSH. Для записи в тестовый раздел используйте следующую команду:
    ...
    Также появится счетчик, отображающий число сохраненных записей.
