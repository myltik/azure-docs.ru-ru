...
3. Введите __имя кластера__, а затем для поля __Тип кластера__ выберите значение __Storm__. Рядом с полем __Имя кластера__ (если оно доступно) отобразится зеленый флажок.
...
6. В колонке __Источник данных__ выберите запись, чтобы указать существующий источник данных или создать новый.

	![Колонка "Источник данных"](./media/hdinsight-apache-storm-tutorial-get-started/datasource.png)

	В настоящее время в качестве источника данных для кластера HDInsight можно выбрать учетную запись хранения Azure. Ниже описаны элементы интерфейса, доступные для настройки в колонке __Источник данных__.
