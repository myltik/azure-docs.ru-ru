...
5. Щелкните __Группа ресурсов__, чтобы просмотреть список существующих групп ресурсов и выбрать ту, в которой будет создан кластер. Или выберите __Создать__ и введите имя новой группы ресурсов. Если имя новой группы доступно, то рядом с ним отобразится зеленый флажок.
...
7. Выберите __Ценовые категории узла__, чтобы отобразить сведения об узлах, которые будут созданы для этого кластера. По умолчанию выбрано __4__ рабочих узла. Выберите значение __1__. Этого будет достаточно для работы с данным учебником. Кроме того, это позволит снизить стоимость кластера. Оценочная стоимость кластера отображается в нижней части этой колонки.
...
1. Выберите **StormStarter - WordCount** (StormStarter — Подсчет слов) из раскрывающегося списка **Jar File** (JAR-файл). После этого действия поля **Имя класса** и **Дополнительные параметры** будут заполнены соответствующими параметрами, использующимися в данном примере.
