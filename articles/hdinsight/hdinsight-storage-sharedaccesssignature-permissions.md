...
* Хранимая политика доступа. Хранимая политика доступа определяется в контейнере ресурсов (контейнере больших двоичных объектов, таблице, очереди или общей папке). Она может использоваться для управления ограничениями одного или нескольких подписанных URL-адресов. При сопоставлении подписи общего доступа с хранимой политикой доступа эта подпись наследует ограничения (время начала, время окончания и разрешения), определенные для данной хранимой политики доступа.
...
3. Удаляется хранимая политика доступа, на которую ссылается подпись, что является другим способом отозвать подпись общего доступа. При повторном создании хранимой политики доступа с таким же именем все маркеры SAS для предыдущей политики снова станут действительными (если время окончания срока действия для подписи SAS еще не прошло). Если планируется отозвать подпись SAS, следует использовать другое имя при повторном создании политики доступа, у которой время окончания срока действия относится к будущему.
4. Повторно создается ключ учетной записи, который был использован для создания подписи общего доступа. Это приведет к неспособности всех компонентов приложения, которые используют предыдущий ключ, пройти проверку подлинности до тех пор, пока они не будут обновлены для использования нового ключа.
...
1. Откройте файл SASToken.py и измените следующие значения:
   ...
   * example\_file\_path: путь к файлу для отправки в контейнер.
...
3. В командной строке выполните следующую команду для проверки подлинности в подписке Azure:
...
* Для подключения к кластерам HDInsight **под управлением Linux** используйте протокол SSH. Сведения об использовании SSH с кластерами под управлением Linux см. в одной из следующих статей.
  ...
  * [Использование SSH с Hadoop на основе Linux в HDInsight на платформе Linux, OS X, Unix или Bash в Windows 10](hdinsight-hadoop-linux-use-ssh-unix.md)
...
1. Когда появится запрос, введите следующую команду: Замените **SASCONTAINER** именем контейнера, созданного для учетной записи хранения SAS. Замените **SASACCOUNTNAME** именем учетной записи хранения, используемой для SAS.
    ...
    После выполнения команды появится список содержимого контейнера, который должен включать файл, загруженный при создании контейнера и SAS.
