...
> [AZURE.NOTE]Для выполнения действий, описанных в этом документе, необходим кластер HDInsight под управлением Linux. Сведения об использовании Solr с кластером под управлением Windows см. в статье [Установка и использование Solr на кластерах HDInsight Hadoop](hdinsight-hadoop-solr-install.md).
...
4. Продолжите подготовку кластера, как описано в статье [Подготовка кластеров HDInsight под управлением Linux](hdinsight-provision-linux-clusters.md#portal).
...
1. В адресной строке браузера введите \_\___http://headnode0:8983/solr/#/__. Этот трафик должен перенаправляться через туннель SSH на узел headnode0 вашего кластера HDInsight. Вы должны увидеть страницу, аналогичную показанной ниже:

	![Изображение панели мониторинга Solr](./media/hdinsight-hadoop-solr-install-linux/solrdashboard.png)

2. В левой области в раскрывающемся списке **Core Selector** (Выбор ядра) выберите **collection1**. В результате под заголовком __collection1__ отобразятся несколько записей.

3. В записях под __collection1__ выберите __Query__ (Запрос). На странице поиска введите следующие значения:
	...
	* В текстовом поле **wt** выберите формат выходных данных. По умолчанию используется **JSON**.
	...
	Наконец, нажмите **Execute Query** (Выполнить запрос) в нижней части панели поиска.
...
1. Подключитесь к кластеру с помощью SSH, а затем используйте следующую команду, чтобы создать моментальный снимок индексированных данных:
...
2. Затем перейдите в каталог __/usr/hdp/current/solr/example/solr__. Здесь будут размещаться подкаталоги для каждой коллекции. Каждый каталог коллекции содержит каталог __data__, в котором хранится моментальный снимок этой коллекции.
	...
	Например, если вы создали индекс для образцов документов с помощью описанных выше действий, то в каталоге __/usr/hdp/current/solr/example/solr/collection1/data__ будет создан каталог с именем __snapshot.###########__, где вместо символов # указаны дата и время моментального снимка.
