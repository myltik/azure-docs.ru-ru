...
# Руководство по HBase. Приступая к работе с Apache HBase на Hadoop в HDInsight (Windows)
...
> [AZURE.NOTE]Информация, приведенная в этом документе, относится только к кластерам HDInsight под управлением Windows. Сведения о кластерах под управлением Linux см. в статье [Учебник по HBase: приступая к работе с Apache HBase на Hadoop в HDInsight](hdinsight-hbase-tutorial-get-started-linux.md).
>
...
>[AZURE.NOTE]После удаления кластера HBase можно создать другой кластер HBase, использовав ту же учетную запись хранения и контейнер BLOB-объектов, используемые по умолчанию. Таблицы HBase, созданные в исходном кластере, получит новый кластер.
...
- [Анализ мнений пользователей Twitter с помощью HBase в HDInsight][hbase-twitter-sentiment]. Узнайте, как в режиме реального времени проводить [анализ мнений](http://en.wikipedia.org/wiki/Sentiment_analysis) на основе больших данных с помощью HBase в кластере Hadoop в HDInsight.
