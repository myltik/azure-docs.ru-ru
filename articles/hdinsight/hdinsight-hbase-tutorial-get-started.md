---
title: "Руководство по HBase. Приступая к работе с кластерами HBase под управлением Windows в Hadoop | Документация Майкрософт"
...
---
# <a name="hbase-tutorial-get-started-using-apache-hbase-with-windows-based-hadoop-in-hdinsight"></a>Руководство по HBase. Приступая к работе с Apache HBase на Hadoop под управлением Windows в HDInsight

Узнайте, как создавать кластеры HBase на HDInsight, создавать таблицы HBase и запрашивать таблицы с помощью Apache Hive. Для получения общих сведений по HBase обратитесь к разделу [Что такое HBase в HDInsight][hdinsight-hbase-overview].

> [!IMPORTANT]
> Linux — единственная операционная система, используемая для работы с HDInsight 3.4 или более поздней версии. См. дополнительные сведения о [нерекомендуемых версиях HDInsight в Windows](hdinsight-component-versioning.md#hdi-version-32-and-33-nearing-deprecation-date). Информация, приведенная в этом документе, относится только к кластерам HDInsight под управлением Windows. Сведения о кластерах под управлением Linux см. в статье [Руководство по HBase. Приступая к работе с Apache HBase на Hadoop под управлением Linux в HDInsight](hdinsight-hbase-tutorial-get-started-linux.md).
...
* **Подписка Microsoft Azure**. Ознакомьтесь с [бесплатной пробной версией Azure](https://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).
...
3. Введите следующие значения.
   ...
   * **Операционная система кластера**: выберите **Windows**.  Инструкции по созданию кластера HBase под управлением Linux описаны в статье [Руководство по HBase. Приступая к работе с Apache HBase на Hadoop под управлением Linux в HDInsight](hdinsight-hbase-tutorial-get-started-linux.md).
   * **Версия** — выберите версию HBase.
   * **Подписка** — выберите подписку Azure, используемую для создания этого кластера.
   * **Группа ресурсов** — создайте новую группу ресурсов Azure или выберите существующую. Дополнительные сведения см. в статье [Общие сведения о диспетчере ресурсов Azure](../azure-resource-manager/resource-group-overview.md).
...
![Большие данные HDInsight HBase][img-hbase-sample-data-bigtable]

Смысл операции прояснится после завершения следующей процедуры.  

**Использование оболочки HBase**

1. Используйте RDP для подключения к кластеру HBase на HDInsight. Инструкции для RDP см. в статье [Управление кластерами Hadoop в HDInsight с помощью портала Azure][hdinsight-manage-portal].
...
6. Получите одну строку
    ...
    Дополнительные сведения о схеме таблицы HBase см. в документе [Introduction to HBase Schema Design][hbase-schema] (Общие сведения о проектировании схемы HBase). Дополнительные команды HBase см. в [справочнике по Apache HBase][hbase-quick-start].
...
## <a name="use-hive-to-query-hbase-tables"></a>Использование HiveQL для формирования запросов к таблицам HBase
...
4. Щелкните **Пользователь**, а затем — **Администратор**. (Это имя пользователя Hadoop.)
...
## <a name="use-the-net-hbase-rest-api-client-library"></a>Использование клиентской библиотеки .NET HBase REST API
...
* [Анализ мнений пользователей Twitter в режиме реального времени с использованием HBase в HDInsight][hbase-twitter-sentiment].
  Узнайте, как в режиме реального времени проводить [анализ мнений](http://en.wikipedia.org/wiki/Sentiment_analysis) на основе больших данных с помощью HBase в кластере Hadoop в HDInsight.
