...
3. В колонке __Отправка действия скрипта__ введите следующие сведения:
    ...
    * __Имя__: понятное имя, используемое для идентификации этого скрипта.
    ...
    * __Persist this script…__ (Сохранить этот скрипт…): флажок должен быть __установлен__.
    ...
    >
    >Дополнительные сведения о разработке действий скрипта см. в статье [Разработка действий сценариев с помощью HDInsight](hdinsight-hadoop-script-actions-linux.md).
    
    ![Добавление действия сценария](./media/hdinsight-getting-started-with-r/scriptaction.png)

4. Выберите __Создать__, чтобы выполнить скрипт. После завершения выполнения пакеты R будут доступны на всех рабочих узлах.
