...
5. Выберите **Учетные данные** и заполните поля **Имя пользователя для входа в кластер** и **Пароль для входа в кластер**.
...
6. Щелкните **Источник данных**, чтобы выбрать источник данных в качестве первичного расположения файловой системы HDFS, используемой кластером. Выберите новую или существующую учетную запись хранения Azure или существующую учетную запись Data Lake Store.
   ...
   1. Если вы выбрали учетную запись хранения Azure, вы можете выбрать существующую учетную запись в разделе **Выбрать учетную запись хранения**. Также вы можете создать новую учетную запись, используя ссылку **Создать** в разделе **Выбрать учетную запись хранения**.
...
9. После просмотра выбранных параметров можно приступить к созданию кластера. Для этого выберите **Закрепить на начальной панели** и нажмите кнопку **Создать**. После этого кластер будет создан, а на начальную панель портала Azure будет добавлен его элемент.

   Вы также обратите внимание на ссылку на **Параметры автоматизации**. Если щелкнуть ее, будут отображены сценарии, которые можно использовать для автоматизации создания кластера с выбранной конфигурацией. Эти скрипты также доступны в записи кластера на портале Azure после создания кластера.
...
* Выберите рабочие узлы (которые нужно вывести из эксплуатации), выберите Actions > Selected Hosts > Hosts (Действия > Выбранные узлы > Узлы), а затем щелкните Turn ON Maintenance Mode (Включить режим обслуживания). Например, на следующем снимке экрана для вывода из эксплуатации выбраны узлы wn3 и wn4.  
