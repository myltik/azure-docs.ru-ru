---
title: Анализ полученных с датчиков данных с помощью Apache Storm и HBase | Microsoft Docs
...
2. В колонке **Создать пространство имен** выполните следующие задачи:
   ...
   6. Выберите **Закрепить на панели мониторинга** и нажмите кнопку **Создать**.
...
2. Откройте файл **app.js** в текстовом редакторе и добавьте полученные ранее сведения о концентраторе событий.
   ...
   > [!NOTE]
   > В этом примере предполагается, что **sensordata** использовалось в качестве имени концентратора событий, а **devices** — в качестве имени политики с утверждением **Send** (Отправка).
   > 
   > 
...
2. Не закрывая панель мониторинга, отправьте новые данные в концентраторы событий с помощью команды `node app.js` (использовалась на предыдущих шагах). Так как значения температуры генерируются случайным образом, граф будет изменяться, отображая значительные изменения температуры.
   ...
   > [!NOTE]
   > Выполнять команду `node app.js` необходимо из каталога **hdinsight-eventhub пример/SendEvents/Nodejs**.
   > 
   > 
...
2. В колонке **Параметры** задайте следующие параметры.
   ...
   * **BASECLUSTERNAME**: это значение будет использоваться в качестве базового имени для кластеров Storm и HBase. Например, если ввести **hdi**, то будет создан кластер Storm **storm-hdi** и кластер HBase **hbase-hdi**.
...
5. Выберите **Юридические условия**, а затем выберите **Создать**.
