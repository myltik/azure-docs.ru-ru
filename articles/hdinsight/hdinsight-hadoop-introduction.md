---
title: "Знакомство с Azure HDInsight, технологической платформой Hadoop и кластерами | Документация Майкрософт"
...
Azure HDInsight является облачным распределением компонентов Hadoop из **платформы данных Hortonworks Data Platform (HDP)**. Первоначально [Apache Hadoop](http://hadoop.apache.org/) представлял платформу с открытым кодом для распределенной обработки и анализа наборов больших данных в кластерах компьютеров. 

Благодаря HDInsight технологии Hadoop просты в использовании. Теперь вам доступно следующее:

*    Упрощенная настройка и установка. Дополнительные сведения см. в статье [Создание кластеров Hadoop в HDInsight](hdinsight-hadoop-provision-linux-clusters.md).
*    Максимальная доступность и надежность. Дополнительные сведения см. в статье [Доступность и надежность кластеров Hadoop в HDInsight](hdinsight-high-availability-linux.md).
*    Функции безопасности и управления за счет интеграции с Active Directory. Дополнительные сведения см. в статье [Основные сведения о безопасности Hadoop в присоединенных к домену кластерах HDInsight (предварительная версия)](hdinsight-domain-joined-introduction.md).
*    Динамическое масштабирование без прерывания заданий.
*   Обновление компонентов и доступность текущих версий. Дополнительные сведения см. в статье [Что представляют собой различные компоненты и версии Hadoop, доступные в HDInsight?][component-versioning].
*   Интеграция с другими службами Azure, включая [веб-приложения](https://docs.microsoft.com/azure/app-service-web/) и [базу данных SQL](https://docs.microsoft.com/azure/sql-database/).

Технологическая платформа Hadoop включает связанные программное обеспечение и служебные программы, включая Apache Hive, HBase, Spark, Kafka и т. д. Дополнительные сведения о Hadoop в HDInsight см. на странице [возможностей HDInsight в Azure](https://azure.microsoft.com/services/hdinsight/).

## <a name="what-is-a-hadoop-cluster-and-when-do-you-use-it"></a>Что такое кластер Hadoop и для чего его используют
Термин *Hadoop* также относится к типу кластера, который включает:
...
В большинстве случаев кластеры Hadoop используются для пакетной обработки хранимых данных. Другие типы кластеров в HDInsight включают дополнительные возможности, например ускоренную обработку в памяти или обработку для потоковой передачи очередей сообщений. Дополнительные сведения см. в разделе [Обзор экосистемы Hadoop в HDInsight](#overview).
...
Большие данные могут быть историческими (т. е. сохраненными за прошлый период) или данными в реальном времени (т. е. транслироваться прямо из источника). Большие данные в различных форматах объединяются в большие тома с постоянно увеличивающейся скоростью.

## <a name="overview"></a>Типы кластеров в HDInsight
HDInsight — это облачный дистрибутив быстро развертывающегося стека технологий Apache Hadoop для анализа больших данных. Он включает определенные типы кластера и возможности его настройки, например добавление компонентов, служебных программ или языков.
...
* **[Apache Hadoop](https://wiki.apache.org/hadoop)** использует управление ресурсами [HDFS](#hdfs) и [YARN](#yarn), а также простую модель программирования [MapReduce](#mapreduce) для обработки и анализа данных в параллельном режиме.
...
* [Подключение Excel к HDInsight с помощью Power Query](hdinsight-connect-excel-power-query.md): узнайте о подключении Excel к учетной записи хранения Azure, в которой хранятся данные, связанные с кластером HDInsight, с помощью Microsoft Power Query для Excel. Требуется рабочая станция Windows. 
