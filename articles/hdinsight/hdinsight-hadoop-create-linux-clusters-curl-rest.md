...
2. Создайте новый маркер с помощью Azure REST API.
    ...
    Документ JSON, возвращаемый этим запросом, будет содержать элемент с именем __access_token__. Значением этого элемента является маркер доступа, который необходимо использовать для проверки подлинности запросов, используемых в следующих разделах этого документа.
