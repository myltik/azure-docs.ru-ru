...
6. Выберите запись __Источник данных__, чтобы выбрать существующий источник данных или создать новый.
    ...
    - __Удостоверение кластера AAD__. Применяется для выбора удостоверения Azure Active Directory, которое будет использоваться кластером для доступа к хранилищу озера данных Azure.
    
        > [AZURE.NOTE] Оно не используется в этом документе, поэтому можно оставить значение по умолчанию. Сведения о применении этой записи и хранилища озера данных Azure с HDInsight см. в статье [Создание кластера HDInsight, который использует хранилище озера данных Azure](data-lake-store-hdinsight-hadoop-use-portal.md).
