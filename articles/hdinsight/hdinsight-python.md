---
title: Использование Python с Hive и Pig в HDInsight | Microsoft Docs
...
Чтобы определить, какой интерпретатор будет использовать Pig (Jython или CPython) для выполнения сценария, используйте команду **register** для ссылки на сценарий Python из Pig Latin. Эта команда сообщает Pig, какой интерпретатор следует использовать и какой псевдоним создавать для сценария. Следующие примеры регистрируют сценарии с Pig в качестве **myfuncs**.
...
> [!IMPORTANT]
> При использовании Jython путь к файлу pig\_jython может быть локальным путем или путем WASB://. Но при использовании CPython необходимо указать ссылку на файл в локальной файловой системе узла, который используется для отправки задания Pig.
> 
> 
...
Фактический файл сценария Python также одинаков для CPython и Jython. Единственное реальное различие заключается в том, что его необходимо импортировать из **pig\_util** при использовании интерпретатора CPython. Вот сценарий **pig\_python.py**:
...
> [!NOTE]
> Вам не нужно устанавливать pig\_util. Эта служебная программа автоматически доступна для сценария.
> 
> 
...
#### Hive
Следующий сценарий запустит сценарий **streaming.py**. Перед запуском он предложит вам ввести сведения о HTTPS/учетной записи администратора для кластера HDInsight.
...
#### Pig (Jython)
В следующем примере используется сценарий **pig\_python.py** с помощью интерпретатора Jython. Перед запуском он предложит вам ввести сведения о HTTPS/учетной записи администратора для кластера HDInsight.
