...
---
# <a name="use-python-with-hive-and-pig-in-hdinsight"></a>Использование Python с Hive и Pig в HDInsight
...
## <a name="a-namepythonapython-on-hdinsight"></a><a name="python"></a>Python в HDInsight
...
## <a name="a-namehivepythonahive-and-python"></a><a name="hivepython"></a>Hive и Python
...
> [!NOTE]
> В кластерах HDInsight под управлением Windows оператор **USING** должен задавать полный путь к python.exe. Это всегда `D:\Python27\python.exe`.
...
2. Инструкция **SELECT TRANSFORM ... USING** выбирает данные из **hivesampletable** и передает параметры clientid, devicemake и devicemodel в скрипт **streaming.py**.
...
5. Все это происходит в цикле `while`, который повторяется до тех пор, пока считывается `line`, после чего `break` выходит из цикла и сценарий завершает выполнение.
...
## <a name="a-namepigpythonapig-and-python"></a><a name="pigpython"></a>Pig и Python
...
Чтобы определить, какой интерпретатор будет использовать Pig (Jython или CPython) для выполнения скрипта, используйте команду **register** при выполнении скрипта Python из Pig Latin. Эта команда сообщает Pig, какой интерпретатор следует использовать и какой псевдоним создавать для сценария. Следующие примеры регистрируют скрипты с Pig в качестве **myfuncs**.
...
1. Первая строка загружает файл с демонстрационными данными **sample.log** в **LOGS**. Так как этот файл журнала не имеет согласованной схемы, она также объявляет каждую запись (в этом случае **LINE**) как массив символов **chararray**. По существу, массив строк является строкой.
...
Фактический файл скрипта Python также аналогичен для CPython и Jython. Единственное реальное различие заключается в том, что при использовании интерпретатора CPython его необходимо импортировать из **pig\_util**. Ниже приведен пример скрипта **pig\_python.py**.
...
4. Затем, команда **split** используется для разделения данных по первым четырем символам пробела. В результате получаем пять значений, присвоенных параметрам **date**, **time**, **classname**, **level** и **detail**.
...
## <a name="a-namerunningarunning-the-examples"></a><a name="running"></a>Выполнение примеров
...
### <a name="ssh"></a>SSH
Дополнительную информацию об использовании SSH см. в разделе [Использование SSH с Hadoop на основе Linux в HDInsight из Linux, Unix или OS X](hdinsight-hadoop-linux-use-ssh-unix.md) или [Использование SSH с Hadoop на основе Linux в HDInsight из Windows](hdinsight-hadoop-linux-use-ssh-windows.md).
...
Для этих действий используется Azure PowerShell. Если вы еще не установили и не настроили его на компьютере для разработки, см. статью [Установка и настройка Azure PowerShell](/powershell/azureps-cmdlets-docs) перед выполнением следующих действий.
...
#### <a name="hive"></a>Hive
Следующий скрипт запустит скрипт **streaming.py**. Перед запуском он предложит вам ввести сведения о HTTPS/учетной записи администратора для кластера HDInsight.
...
В следующем примере используется скрипт **pig_python.py** с интерпретатором Jython. Перед запуском он предложит вам ввести сведения о HTTPS/учетной записи администратора для кластера HDInsight.
