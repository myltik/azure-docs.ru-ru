...
* Подписка Azure
* Storm в кластере HDInsight. Сведения о создании кластера, который может использовать хранилище озера данных Azure, содержатся в этом документе.
...
* __TickSpout__: создает данные, используемые компонентами в топологии.
...
Проект, содержащий эту топологию, можно загрузить на странице [https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store](https://github.com/Azure-Samples/hdinsight-storm-azure-data-lake-store).
...
Поскольку для записи в хранилище озера данных применяется HdfsBolt и изменен только URL-адрес, можно воспользоваться любой существующей топологией, которая записывает данные в HDFS или WASB с помощью HdfsBolt, и легко изменить для использования хранилища озера данных Azure.

##Создание кластера HDInsight и хранилища озера данных.

Создайте новый Storm в кластере HDInsight, следуя инструкциям в документе [Использование HDInsight с хранилищем озера данных с помощью Azure](../data-lake-store/data-lake-store-hdinsight-hadoop-use-portal.md). Действия, описанные в этом документе, помогут вам создать новый кластер HDInsight и хранилище озера данных Azure.

> [AZURE.IMPORTANT] При создании кластера HDInsight необходимо выбрать __Storm__ как тип кластера. Операционной системой может быть Windows или Linux.
...
3. С помощью командной строки, терминала или сеанса работы с оболочкой измените каталоги на корень загруженного проекта и выполните следующие команды для сборки и упаковки топологии.
    ...
    По завершении сборки и упаковки будет создан новый каталог с именем `target`, который содержит файл с именем `StormToDataLakeStore-1.0-SNAPSHOT.jar`. Этот файл содержит скомпилированную топологию.
...
1. Для копирования топологии в кластер HDInsight воспользуйтесь следующей командой. Замените __USER__ именем пользователя SSH, которое вы использовали при создании кластера. Замените __CLUSTERNAME__ именем кластера.
    ...
    При появлении запроса введите пароль, который применялся при создании пользователя SSH для кластера. Если используется открытый ключ вместо пароля, может потребоваться параметр `-i` с указанием пути к соответствующему закрытому ключу.
    
    > [AZURE.NOTE] При использовании клиента Windows для разработки у вас может не быть команды `scp`. В этом случае вы можете применить `pscp`, доступную на странице [http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html](http://www.chiark.greenend.org.uk/~sgtatham/putty/download.html).
...
2. После завершения отправки используйте следующую команду для подключения к кластеру HDInsight с помощью SSH. Замените __USER__ именем пользователя SSH, которое вы использовали при создании кластера. Замените __CLUSTERNAME__ именем кластера.
    ...
    > [AZURE.NOTE] Если вы используете клиент Windows для разработки, следуйте указаниям в статье [Подключение к HDInsight под управлением Linux с помощью SSH из Windows](hdinsight-hadoop-linux-use-ssh-windows.md) для использования клиента PuTTY для подключения к кластеру.
...
1. Откройте веб-браузер и перейдите по адресу HTTPS://CLUSTERNAME.azurehdinsight.net, где __CLUSTERNAME__ — это имя вашего кластера HDInsight. При появлении запроса укажите имя пользователя администратора (`admin`) и пароль, используемый для этой учетной записи во время создания кластера.

2. На панели мониторинга Storm выберите __Обзор__ из раскрывающегося списка __Jar-файл__, а затем выберите файл StormToDataLakeStore-1.0-SNAPSHOT.jar из каталога `target`. Примените следующие значения для других операций в форме:
...
3. Выберите __Отправить__, чтобы отправить и запустить топологию. После запуска топологии в поле результатов под кнопкой __Отправить__ будет выведена информация, аналогичная приведенной ниже:
...
Существует несколько способов просмотреть данные. В этом разделе мы используем портал Azure и команду `hdfs` для просмотра данных.
...
* __На [портале Azure](https://portal.azure.com)__: выберите хранилище озера данных Azure, которое используется с HDInsight.
    ...
    > [AZURE.NOTE] Если вы не закрепили хранилище озера данных Azure панели мониторинга портала, его можно найти, выбрав __Обзор__ в нижней части списка, расположенного слева, затем __Хранилище озера данных__, и, наконец, выбрав хранилище.
    
    Из значков в верхней части хранилища озера данных выберите __Обозреватель данных__.
...
* __Из кластера__: если вы подключились к кластеру HDInsight с использованием SSH (кластера Linux) или удаленного рабочего стола (кластера Windows), данные можно просмотреть следующим образом. Замените __DATALAKE__ именем озера хранилища данных
...
3. В разделе __Действия с топологией__ выберите __Удалить__ и нажмите кнопку ОК в появившемся диалоговом окне.
