...
3. С помощью командной строки, терминала или сеанса работы с оболочкой измените каталоги на корень загруженного проекта и выполните следующие команды для сборки и упаковки топологии.
    ...
    По завершении сборки и упаковки будет создан новый каталог `target`, который содержит файл `StormToDataLakeStore-1.0-SNAPSHOT.jar`. Этот файл содержит скомпилированную топологию.
...
1. Для копирования топологии в кластер HDInsight воспользуйтесь следующей командой. Замените __USER__ именем пользователя SSH, которое вы использовали при создании кластера. Замените __CLUSTERNAME__ именем кластера.
    ...
    При появлении запроса введите пароль, который применялся при создании пользователя SSH для кластера. Если вместо пароля используется открытый ключ, может потребоваться параметр `-i` с указанием пути к соответствующему закрытому ключу.
...
2. После завершения отправки используйте следующую команду для подключения к кластеру HDInsight с помощью SSH. Замените __USER__ именем пользователя SSH, которое вы использовали при создании кластера. Замените __CLUSTERNAME__ именем кластера.
    ...
    > [AZURE.NOTE] Если вы используете клиент Windows для разработки, следуйте указаниям в статье [Подключение к HDInsight под управлением Linux с помощью SSH из Windows](hdinsight-hadoop-linux-use-ssh-windows.md), чтобы использовать клиент PuTTY для подключения к кластеру.
...
3. Выберите __Отправить__, чтобы передать и запустить топологию. После запуска топологии в поле результатов под кнопкой __Отправить__ будет выведена информация, аналогичная приведенной ниже.
...
* __На [портале Azure](https://portal.azure.com)__: выберите хранилище озера данных Azure, которое используется с HDInsight.
    ...
    > [AZURE.NOTE] Если вы не закрепили хранилище озера данных Azure на панели мониторинга портала, его можно найти, выбрав __Обзор__ в нижней части списка, расположенного слева, затем — __Хранилище озера данных__, и, наконец, выбрав хранилище.
