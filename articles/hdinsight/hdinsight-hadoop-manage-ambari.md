...
Для подключения к Ambari в кластере HDInsight требуется использовать протокол HTTPS. Также необходимо выполнить вход в Ambari с помощью имени учетной записи администратора (по умолчанию используется __admin__) и пароля, указанных при создании кластера.
...
Сведения о создании туннеля SSH для работы с Ambari см. в статье [Использование туннелирования SSH для доступа к веб-интерфейсу Ambari, ResourceManager, JobHistory, NameNode, Oozie и другим веб-интерфейсам](hdinsight-linux-ambari-ssh-tunnel.md).
...
>
> Сведения об использовании туннеля SSH с HDInsight см. в статье [Использование туннелирования SSH для доступа к веб-интерфейсу Ambari, ResourceManager, JobHistory, NameNode, Oozie и другим веб-интерфейсам](hdinsight-linux-ambari-ssh-tunnel.md).
...
> [AZURE.WARNING]Несмотря на то что в этом меню есть пункт __Добавить службу__, не следует использовать его для добавления служб в кластер HDInsight. Для добавления новых служб следует использовать действие сценария во время подготовки кластера. Дополнительные сведения об использовании действий сценария см. в статье [Настройка кластеров HDInsight с помощью действия сценария](hdinsight-hadoop-customize-cluster-linux.md).
...
* **Некоторые функциональные возможности не включены** — некоторые функциональные возможности Ambari (например, добавление узлов в кластер, удаление их из него или добавление новых служб) отключены, так как им управляет облачная служба HDInsight. При этом другие функциональные возможности в предварительной версии HDInsight на платформе Linux могут быть реализованы не полностью.
