...
Для оценки с помощью веб-службы машинного обучения Azure используйте [пакет R с открытым кодом для машинного обучения Azure](http://www.inside-r.org/blogs/2015/11/18/enhancements-azureml-package-connect-r-azureml-studio), чтобы [опубликовать модель как веб-службу Azure](http://www.r-bloggers.com/deploying-a-car-price-model-using-r-and-azureml/). Используйте средства машинного обучения, чтобы создать пользовательский интерфейс для веб-службы. Затем вызовите веб-службу, чтобы выполнить оценку.

При выборе этого варианта объекты модели ScaleR необходимо конвертировать в эквивалентные объекты модели с открытым кодом. Для этого можно использовать функции приведения ScaleR (такие как `as.randomForest()`) для моделей на основе ансамблей.
...
Более сложное средство для разработки сценариев R — среда IDE на базе R для использования на рабочем столе. Примером является недавно анонсированная корпорацией Майкрософт среда разработки [R Tools for Visual Studio](https://www.visualstudio.com/ru-RU/features/rtvs-vs.aspx) (RTVS). Это семейство настольных и серверных средств [RStudio](https://www.rstudio.com/products/rstudio-server/). Вы также можете использовать [StatET](http://www.walware.de/goto/statet) Walware на базе Eclipse.
