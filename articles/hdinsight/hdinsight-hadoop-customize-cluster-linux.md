...
3. Выполните приведенную далее команду, чтобы применить действие сценария к работающему кластеру.
    ...
    Допустимыми значениями __nodetypes__ являются __headnode__, __workernode__ и __zookeeper__. Если сценарий должен быть применен к нескольким типам узлов, укажите типы, разделив их точкой с запятой. Например, `-n headnode;workernode`.
