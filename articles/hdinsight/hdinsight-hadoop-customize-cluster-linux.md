...
При использовании подписки Azure без прав администратора или владельца (например, подписка организации) для входа в группу ресурсов Azure, в которой содержится кластер HDInsight, требуется по крайней мере роль **участника**.
...
Действие скрипта — это просто скрипт Bash, в который вы вводите универсальный код ресурса (URI) и параметры, а он затем выполняется на узлах кластера HDInsight. Ниже приведены характеристики и возможности действий скриптов.
...
* Они должны храниться в URI, доступном из кластера HDInsight. Возможные места хранения:
    ...
    * **Учетная запись хранилища BLOB-объектов**, которая служит основной или дополнительной учетной записью хранения для кластера HDInsight. При создании кластера HDInsight получает доступ к обоим видам учетных записей хранения, что позволяет использовать действия сценариев, которые не являются общедоступными.

    * Универсальный код ресурса (URI), такой как большой двоичный объект Azure, GitHub, OneDrive, Dropbox и т. д.
...
Когда скрипт применяется в кластере, состояние кластера изменяется с **Работает** на **Принято**, затем на **Конфигурация HDInsight** и снова на **Работает**, если выполнение скрипта завершается успешно. Так как состояние скрипта регистрируется в журнале действий скриптов, вы можете определить успешность его выполнения. Например, командлет PowerShell `Get-AzureRmHDInsightScriptActionHistory` можно использовать для просмотра состояния сценария. Он возвращает следующую информацию:
...
4. В колонке "Добавление действия скрипта" введите следующие сведения.
   ...
   * **Сохраненное** — установите флажок для этой записи, если необходимо сохранить скрипт, чтобы он применялся на новых рабочих узлах при увеличении масштаба кластера.
...
> [!WARNING]
> Не изменяйте пароль модуля наблюдения Ambari (hdinsightwatchdog) в кластере HDInsight под управлением Linux. Это не позволит выполнять новые действия скрипта в кластере HDInsight.
