---
title: Настройка кластеров HDInsight с помощью действий сценария | Microsoft Docs
...
* Могут быть ограничены **запуском только на узлах определенного типа**, например головных или рабочих.
  
  > [!NOTE]
  > При использовании HDInsight уровня "Премиум" можно указать использование сценария на граничном узле.
  > 
  > 
* Могут быть **сохраняемыми** или **специализированными**.
  
    **Сохраняемые** — это сценарии, которые применяются на рабочих узлах и автоматически запускаются на новых узлах, созданных при масштабировании кластера.
  ...
  > 
    ...
    **Специализированные** сценарии не сохраняются, но такой сценарий можно впоследствии сделать сохраняемым (и наоборот).
  ...
  > 
  > 
* Могут принимать **параметры**, используемые в процессе выполнения сценария.
* Выполняются с **использованием прав корневой учетной записи** на узлах кластера.
* Могут использоваться с помощью **портала Azure**, **Azure PowerShell**, **Azure CLI** или **пакета SDK HDInsight для .NET**.
...
* **Ошибка** в сценарии может вызвать сбой при создании кластера.
...
2. В колонке кластера HDInsight выберите **Действия скрипта**.
   
    ![Элемент "Действия сценариев"](./media/hdinsight-hadoop-customize-cluster-linux/scriptactionstile.png)
   
   > [!NOTE]
   > Также можно выбрать **Все параметры**, а затем **Действия сценариев** в колонке "Параметры".
   > 
   > 
...
4. В колонке "Добавление действия скрипта" введите следующие сведения.
   ...
   * **Имя**: понятное имя для этого действия сценария. В этом примере — `Add Storage account`.
   * **Универсальный код ресурса скрипта**: универсальный код ресурса (URI) для сценария. В этом примере — `https://hdiconfigactions.blob.core.windows.net/linuxaddstorageaccountv01/add-storage-account-v01.sh`.
   * **Головной**, **Рабочий** и **Zookeeper**: отметьте флажками узлы, на которых должен применяться этот сценарий. В этом примере отмечены узлы "Головной", "Рабочий" и "Zookeeper".
   * **Параметры**: если сценарий принимает параметры, укажите их здесь. В этом примере введите имя учетной записи хранения и ключ учетной записи хранения:
...
3. Выполните приведенную далее команду, чтобы применить действие сценария к работающему кластеру.
    ...
    Допустимыми значениями **nodetype** являются **headnode**, **workernode** и **zookeeper**. Если сценарий должен быть применен к нескольким типам узлов, укажите типы, разделив их точкой с запятой. Например, `-n headnode;workernode`.
...
6. Можно также использовать элемент **…** справа от записей в колонке "Действия скрипта" для выполнения таких действий, как повторное выполнение, сохранение или (для сохраняемых действий) удаление.
