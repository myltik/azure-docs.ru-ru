...
* Могут быть __сохраняемыми__ или __специальными__.

    __Сохраняемые__ — это скрипты, которые применяются на рабочих узлах и автоматически запускаются на новых узлах, созданных при масштабировании кластера.
    ...
    __Специальные__ скрипты не сохраняются; но такой скрипт можно впоследствии сделать сохраняемым (и наоборот).
...
* Могут принимать __параметры__, используемые в процессе выполнения скрипта.

* Выполняются с __использованием прав корневой учетной записи__ на узлах кластера.

* Могут использоваться с помощью __портала Azure__, __Azure PowerShell__ или __пакета SDK HDInsight для .NET__.
...
* Скрипт __сохраняется автоматически__.

* __Ошибка__ в скрипте может вызвать сбой при создании кластера.
...
Когда скрипт применяется в кластере, состояние кластера изменяется с __Работает__ на __Принято__, затем на __Настройка HDInsight__ и снова на __Работает__, если выполнение скрипта завершается успешно. Так как состояние скрипта регистрируется в журнале действий скриптов, вы можете определить успешность его выполнения. Например, командлет `Get-AzureRmHDInsightScriptActionHistory` PowerShell можно использовать для просмотра состояния скрипта. Он возвращает следующую информацию:
...
Действия сценария можно использовать из портала Azure, Azure PowerShell или пакета SDK .NET HDInsight. В HDInsight доступны скрипты для установки следующих компонентов в кластерах HDInsight.
...
| **Предварительная загрузка библиотек Hive** | https://hdiconfigactions.blob.core.windows.net/linuxsetupcustomhivelibsv01/setup-customhivelibs-v01.sh. См. статью [Добавление библиотек Hive во время создания кластера HDInsight](hdinsight-hadoop-add-hive-libraries.md). |
...
В этом разделе описывается, как применять действия скриптов в работающем кластере HDInsight: с помощью портала Azure, командлетов PowerShell и пакета SDK для .NET.
...
3. В колонке "Параметры" выберите __Действия скриптов__.

    ![Ссылка на действия скриптов](./media/hdinsight-hadoop-customize-cluster-linux/settings.png)

4. В верхней части колонки "Действия скриптов" выберите __Отправить новое__.
...
5. В колонке "Добавление действия скрипта" введите следующие сведения.
    ...
    * __Имя__: понятное имя для этого действия скрипта. В этом примере — `R`.
    * __URI скрипта__: универсальный код ресурса (URI) для скрипта. В этом примере — `https://hdiconfigactions.blob.core.windows.net/linuxrconfigactionv01/r-installer-v01.sh`
    * __Головной узел __, __Рабочий узел__ и __Узел Zookeeper__: отметьте флажками узлы, на которых должен применяться этот скрипт. В этом примере отмечены головной и рабочий узлы.
    * __Параметры__: если скрипт принимает параметры, укажите их здесь.
    * __Сохраняемый__: установите флажок для этой записи, если необходимо сохранить скрипт, чтобы он применялся на новых рабочих узлах при увеличении масштаба кластера.

6. Наконец, нажмите кнопку __Создать__, чтобы применить скрипт в кластере.
...
Пример использования пакета SDK для .NET для применения скриптов в кластере см. на странице [https://github.com/Azure-Samples/hdinsight-dotnet-script-action](https://github.com/Azure-Samples/hdinsight-dotnet-script-action).
...
6. Можно также использовать элемент __…__ справа от записей в колонке действий скриптов для выполнения таких действий, как повторное выполнение, сохранение или (для сохраняемых действий) удаление.
...
> [AZURE.IMPORTANT] Использование `Remove-AzureRmHDInsightPersistedScriptAction` не отменяет действия, выполняемые скриптом, — удаляется только сохраняемый флаг, в результате чего скрипт не будет выполняться на новых рабочих узлах, добавляемых в кластер.
...
Пример использования пакета SDK для .NET для получения журнала скриптов из кластера и изменения типа скрипта см. в [этой статье]() (подлежит уточнению).
