---
title: "Использование различных ядер с записными книжками Jupyter в кластерах Azure Spark | Документация Майкрософт"
description: "Узнайте о ядрах PySpark и Spark, которые можно использовать с записной книжкой Jupyter, доступной в кластерах Spark в HDInsight под управлением Linux."
services: hdinsight
documentationcenter: 
author: nitinme
manager: jhubbard
...
Кластеры HDInsight Spark предоставляют два ядра, которые можно использовать с записными книжками Jupyter для тестирования приложений Spark. Ядра — это программа, которая выполняет и интерпретирует ваш код. Ниже приведены два ядра.
...
3. Щелкните **Записная книжка Jupyter**. При появлении запроса введите учетные данные администратора для кластера.
   ...
   > 
   > 
3. Щелкните **Создать**, а затем — **Pyspark** или **Spark**, чтобы создать записную книжку. Для приложений Scala следует использовать ядро Spark, а для приложений Python — ядро PySpark.
   
    ![Создание записной книжки Jupyter](./media/hdinsight-apache-spark-jupyter-notebook-kernels/jupyter-kernels.png "Создание записной книжки Jupyter") 

4. Записная книжка должна открыться с выбранным ядром.

## <a name="choose-between-the-kernels"></a>Выбор ядра
Ниже приведены несколько преимуществ использования новых ядер.

- **Предустановленные контексты**. Благодаря ядрам **PySpark** и **Spark** вам не требуется явно настраивать контексты Spark или Hive перед началом работы с приложением. Они доступны по умолчанию. а именно:
...
| -n |`-n <MAXROWS>` |**MAXROWS** должно быть выражено целым числом. Число выходных строк для параметра **MAXROWS**ограничивается ядром. Если значение параметра **MAXROWS** выражено отрицательным числом, например **-1**, то число строк в результирующем наборе ограничиваться не будет. |
...
Какое бы ядро вы ни использовали (PySpark или Spark), работающие записные книжки будут потреблять ресурсы кластера.  При использовании этих ядер (так как контексты заданы предварительно) при простом выходе из записных книжек контекст не завершается, а значит ресурсы кластера будут продолжать использоваться. При работе с ядрами PySpark и Spark рекомендуется использовать параметр **Close and Halt** (Закрыть и остановить) в меню **Файл** записной книжки. Программа аннулирует контекст и закроет записную книжку.     
...
Записные книжки Jupyter хранятся в учетной записи хранения, связанной с кластером в папке **/HdiNotebooks** .  Доступ к записным книжкам, текстовым файлам и папкам, создаваемым в Jupyter, можно получить через WASB.  Например, если Jupyter используется для создания папки **myfolder** и записной книжки **myfolder/mynotebook.ipynb**, то доступ к этой записной книжке можно получить по адресу `wasbs:///HdiNotebooks/myfolder/mynotebook.ipynb`.  Верно и обратное: если вы передаете записную книжку непосредственно в свою учетную запись в `/HdiNotebooks/mynotebook1.ipynb`, то эта записная книжка также отображается в Jupyter.  Записные книжки хранятся в учетной записи хранения даже после удаления кластера.
