...
Узнайте, как подключить HDInsight к локальной сети с помощью виртуальной сети Azure и VPN-шлюза. В этом документе содержатся следующие сведения:

* Как создать виртуальную сеть Azure, подключаемую к локальной сети.

* Как включить разрешение DNS-имен между виртуальной и локальной сетью.

* Как ограничить доступ к HDInsight из Интернета с использованием групп безопасности сети.

* Как обнаружить порты, предоставляемые HDInsight в виртуальной сети.
...
2. В колонке __Основные сведения__ задайте следующие параметры.
...
3. В колонке __Выбор размера__ выберите размер виртуальной машины. Для этого руководства выберите вариант с наименьшим размером и стоимостью. Чтобы продолжить, щелкните __Выбрать__.

4. В колонке __Параметры__ задайте следующие параметры:
...
5. В колонке __Приобретение__ нажмите кнопку __Приобрести__, чтобы создать виртуальную машину.

6. После создания виртуальной машины для нее отобразится колонка __Обзор__. Из списка слева выберите __Свойства__. Сохраните значения __Общедоступный IP-адрес__ и __Частный IP-адрес__. Они будут использоваться в следующем разделе.
...
1. Используйте SSH для подключения к __общедоступному IP-адресу__ виртуальной машины. В следующем примере устанавливается подключение к виртуальной машине по адресу 40.68.254.142:
    ...
    > [!NOTE]
    > Есть несколько способов получить служебную программу `ssh`. В Linux, Unix и macOS она обычно предоставляется как часть операционной системы. Если вы используете Windows, рассмотрите один из следующих вариантов:
    >
