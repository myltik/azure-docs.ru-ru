...
Дополнительные сведения об использовании собственного DNS-сервера с виртуальной сетью Azure см. в разделе __Разрешение имен с помощью собственного DNS-сервера__ статьи [Разрешение имен для ВМ и экземпляров ролей](../virtual-network/virtual-networks-name-resolution-for-vms-and-role-instances.md#name-resolution-using-your-own-dns-server).
...
> [AZURE.IMPORTANT] HDInsight не поддерживает ограничение исходящего трафика, ограничить можно только входящий трафик. При определении правил группы безопасности сети для подсети, содержащей HDInsight, используйте только правила для входящего трафика.
...
1. Используйте следующую команду, чтобы создать новую группу безопасности сети с именем `hdisecure`. Замените __RESOURCEGROUPNAME__ и __LOCATION__ на имя группы ресурсов, которая содержит виртуальную сеть Azure, и расположение (регион), в котором была создана группа.
