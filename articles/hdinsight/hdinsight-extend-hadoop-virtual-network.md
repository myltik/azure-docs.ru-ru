...
* HDInsight не поддерживается в виртуальных сетях Azure, для которых явным образом ограничен доступ в Интернет или из Интернета. Например, в сетях, использующих группы безопасности сети или ExpressRoute для блокировки трафика из Интернета к ресурсам в виртуальной сети. Служба HDInsight — это управляемая служба, и во время подготовки и запуска ей необходим доступ к Интернету, чтобы Azure могла наблюдать за работоспособностью кластера, инициировать отработку отказа ресурсов кластера и выполнять другие автоматизированные задачи управления.
    ...
    2. С помощью следующих инструкций PowerShell можно убедиться, что для этой подсети не настроены группы безопасности сети и таблицы маршрутизации. Замените __VIRTUALNETWORKNAME__ на имя вашей виртуальной сети, __GROUPNAME__ — на имя группы ресурсов, которая содержит виртуальную сеть, а __SUBNET__ — на имя подсети.
        ...
        Обратите внимание, что в результатах __NetworkSecurityGroup__ и __RouteTable__ равны `null`.
