...
* HDInsight не поддерживается в виртуальных сетях Azure, для которых явным образом ограничен доступ в Интернет или из Интернета. Например, в сетях, использующих группы безопасности сети или ExpressRoute для блокировки трафика из Интернета к ресурсам в виртуальной сети. Служба HDInsight — это управляемая служба, и во время подготовки и запуска ей необходим доступ к Интернету, чтобы Azure могла наблюдать за работоспособностью кластера, инициировать отработку отказа ресурсов кластера и выполнять другие автоматизированные задачи управления.
    ...
    1.	Создайте новую подсеть в виртуальной сети. Эта подсеть будет использоваться HDInsight.

    2.	Определите таблицу маршрутизации и создайте определяемый пользователем маршрут (UDR) для подсети, которая разрешает входящие и исходящие подключения к Интернету. Это можно сделать с помощью маршрутов *. Это обеспечит подключение к Интернету только для ресурсов, расположенных в этой подсети. Дополнительные сведения о работе с определяемыми пользователями маршрутами см. в разделах https://azure.microsoft.com/ru-RU/documentation/articles/virtual-networks-udr-overview/ и https://azure.microsoft.com/ru-RU/documentation/articles/virtual-networks-udr-how-to/.
    
    3.	При создании кластера HDInsight выберите подсеть, созданную на шаге 1. Кластер будет развернут в подсети, которая имеет доступ к Интернету.
