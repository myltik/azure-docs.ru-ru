...
HDInsight не поддерживается в виртуальных сетях Azure, для которых явным образом ограничен доступ в Интернет или из Интернета. Например, в сетях, использующих группы безопасности сети или ExpressRoute для блокировки трафика из Интернета к ресурсам в виртуальной сети. Служба HDInsight — это управляемая служба, и во время подготовки и запуска ей необходим доступ к Интернету, чтобы Azure могла наблюдать за работоспособностью кластера, инициировать отработку отказа ресурсов кластера и выполнять другие автоматизированные задачи управления.

Если вы хотите использовать HDInsight в виртуальной сети, которая блокирует интернет-трафик, можно сделать следующее:

1. Создайте новую подсеть в виртуальной сети. По умолчанию новая подсеть сможет обмениваться данными через Интернет. Это позволяет установить HDInsight в этой подсети. Так как новая подсеть находится в той же виртуальной сети, что и защищенные подсети, она также может обмениваться данными с установленными там ресурсами.
...
Если вы не уверены, применены ли к подсети, в которую вы хотите установить HDInsight, ограничения, или вам требуется удалить ограничения подсети, сделайте следующее:

1. Откройте [портал Azure](https://portal.azure.com).

2. Выберите виртуальную сеть.

3. Выберите __Свойства__.

4. Щелкните __Подсети__, а затем выберите конкретную подсеть. Если ограничения не применены, в колонке подсети для записей __Группа безопасности сети__ и __Таблица маршрутов__ будет задано значение __Нет__.

    Чтобы снять ограничения, выберите запись __Группа безопасности сети__ или __Таблица маршрутов__, а затем щелкните __Нет__. Чтобы сохранить изменения, щелкните __Сохранить__ в колонке подсети.
    
    ![Изображение колонки подсети и выбор группы безопасности сети](./media/hdinsight-extend-hadoop-virtual-network/subnetnsg.png)

Дополнительные сведения о группах безопасности сети см. в статье [Группа безопасности сети](../virtual-network/virtual-networks-nsg.md). Сведения об управлении маршрутизацией в виртуальной сети Azure см. в статье [Определяемые пользователем маршруты и IP-пересылка](../virtual-network/virtual-networks-udr-overview.md).
