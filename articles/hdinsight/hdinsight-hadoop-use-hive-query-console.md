---
title: Использование Hadoop Hive в консоли запросов в HDInsight | Microsoft Docs
...
1. Откройте веб-браузер и перейдите по адресу **https://CLUSTERNAME.azurehdinsight.net**, где *_CLUSTERNAME\*\_ — это имя вашего кластера HDInsight. При появлении соответствующего запроса введите имя пользователя и пароль, использованные при создании кластера.
...
4. После изменения значения поля **Состояние** на **Завершено** выберите **Показать подробности** для задания. На странице подробностей **выходные данные задания** будут содержать `[ERROR]    3`. Чтобы скачать файл, содержащий выходные данные задания, можно нажать кнопку **Скачать** под этим полем.
