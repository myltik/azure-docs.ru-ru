...
3.	Разверните пункт **Azure**, а затем — **HDInsight**.
	...
	>[AZURE.NOTE]Должно открыться окно **Список задач HDInsight**. Если оно не открылось, щелкните **Другие окна** в меню **Вид** и выберите пункт **Окно списка задач HDInsight**.  
...
4. Щелкните **Отправка** или **Отправка (расширенная)**. С помощью расширенной отправки можно настроить **Имя задания**, **Аргументы**, **Дополнительные конфигурации** и **Каталог состояния** сценария:
	...
	После отправки задания отобразится окно **Сводка по заданию Hive**.
...
2. Щелкните правой кнопкой мыши кластер HDInsight, а затем — **Просмотр заданий**. Появится список заданий Hive, запущенных на кластере.
