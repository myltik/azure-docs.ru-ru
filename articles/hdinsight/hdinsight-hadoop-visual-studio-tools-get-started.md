---
title: Узнайте, как использовать инструменты Hadoop для Visual Studio для HDInsight | Microsoft Docs
...
* Рабочая станция со следующим ПО:
  ...
  * Одна из следующих версий Visual Studio:
    ...
    * Visual Studio 2015 (Community/Enterprise)
    ...
    > [!NOTE]
    > Сейчас доступна только английская версия инструментов HDInsight для Visual Studio.
    > 
    > 
...
3. Разверните пункт **Azure**, а затем — **HDInsight**.
   ...
   > [!NOTE]
   > Должно открыться окно **Список задач HDInsight**. Если оно не открылось, в меню **Вид** щелкните пункт **Другие окна**, а затем — **Окно списка задач HDInsight**.
   > 
   > 
...
2. Щелкните **hivesampletable** правой кнопкой мыши, а затем — **Просмотреть первые 100 строк** для просмотра строк. Это эквивалентно выполнению следующего запроса Hive с помощью драйвера Hive ODBC:
...
5. Щелкните **Отправка** или **Отправка (расширенная)**. С помощью расширенной отправки можно настроить **Имя задания**, **Аргументы**, **Дополнительные конфигурации** и **Каталог состояния** сценария:
...
> [!NOTE]
> Эта функция доступна для кластеров HDInsight начиная с версии 3.2.4.593. Она работает только для выполненных заданий (если задание отправлено через WebHCat). Показанная ниже диаграмма будет отображаться, если запрос выполняется через HiveServer2. Она работает для кластеров на основе Windows и на основе Linux.
> 
> 
