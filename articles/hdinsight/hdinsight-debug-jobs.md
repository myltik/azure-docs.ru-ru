---
title: 'Отладка Hadoop в среде HDInsight: просмотр журналов и интерпретация сообщений об ошибках | Microsoft Docs'
...
Имена файлов таблиц следующие: **u<имя\_кластера>ДДМесГГГГatЧЧММССsss<имя\_таблицы>**.
...
5. Щелкните правой кнопкой мыши таблицу hadoopservicelog в области **Навигатор** и выберите **Изменить**. Вы увидите 4 столбца. При необходимости удалите столбцы **Partition Key**, **Row Key** и **Timestamp**, выделив их, а затем щелкнув **Удалить столбцы** в меню на ленте.
...
4. Разверните **Учетные записи хранения**, выберите учетную запись хранения по умолчанию для кластера и затем выберите **Таблицы**.
...
* **Описание**. В подписке с идентификатором *ИД\_вашей\_подписки* не осталось ядер для создания кластера *имя\_вашего\_кластера*. Требуется: *требуемыеРесурсы*, доступно: *доступныеРесурсы*.
...
* **Описание**. В подписке с идентификатором *ИД\_вашей\_подписки* нет квоты на новую размещенную службу для создания кластера *имя\_вашего\_кластера*.
* **Устранение**. Освободите ресурсы в своей подписке или увеличьте количество ресурсов, доступных для подписки, а затем попробуйте создать кластер снова.
...
* **Описание**. Используемый идентификатор подписки *ИД\_вашей\_подписки* не имеет достаточных разрешений для выполнения операции удаления для кластера *имя\_вашего\_кластера*.
...
* **Описание**. Недоступна поддержка региона *имя\_вашего\_региона* и идентификатора подписки *ИД\_вашей\_подписки*.
...
* **Описание**. Указанный идентификатор подписки *ИД\_вашей\_подписки* неактивен.
* **Устранение**. Повторно активируйте свою подписку или получите новую действующую подписку.

### <a id="SubscriptionIdNotFound"></a>SubscriptionIdNotFound
* **Описание**. Не удалось найти идентификатор подписки *ИД\_вашей\_подписки*.
...
* **Устранение**. Укажите допустимый URL-адрес большого двоичного объекта. Этот URL-адрес ДОЛЖЕН быть полностью допустимым, начинаться с *http://* и заканчиваться на *.com*.
...
* **Описание**. Недоступна поддержка версии для версии *указанная\_версия* и идентификатора подписки *ИД\_вашей\_подписки*.
