...
В ситуациях, когда ошибка связана с Azure HDInsight, может быть полезно понять, что означает сообщение об ошибке. Сведения о различных кодах ошибок и о том, как устранить причины ошибок, см. в статье [Коды ошибок в HDInsight](#hdi-error-codes). В некоторых ситуациях вам может потребоваться доступ к журналам Hadoop. Это можно сделать непосредственно на портале предварительной версии Azure.

## Просмотр журналов заданий и сведений о работоспособности кластеров

* **Получение доступа к пользовательскому интерфейсу Hadoop**. На портале предварительной версии Azure щелкните имя кластера HDInsight, чтобы открыть колонку кластера. В колонке кластера щелкните **Панель мониторинга**.
...
* **Получение доступа к пользовательскому интерфейсу Yarn**. На портале предварительной версии Azure щелкните имя кластера HDInsight, чтобы открыть колонку кластера. В колонке кластера щелкните **Панель мониторинга**. При появлении соответствующего запроса введите учетные данные администратора кластера. В открывшейся консоли запроса щелкните **Пользовательский интерфейс YARN**
...
* **Получение доступа к пользовательскому интерфейсу HBase**. На портале предварительной версии Azure щелкните имя кластера HDInsight HBase, чтобы открыть колонку кластера. В колонке кластера щелкните **Панель мониторинга**. При появлении соответствующего запроса введите учетные данные администратора кластера. В открывшейся консоли запроса щелкните **Пользовательский интерфейс HBase**.

## <a id="hdi-error-codes"></a>Коды ошибок HDInsight

Ошибки, с которыми может столкнуться пользователь при использовании Azure PowerShell или портала предварительной версии, перечислены ниже в алфавитном порядке. В свою очередь, ошибки связаны с записью в разделе [Описание и устранение ошибок](#discription-mitigation-errors), где приводятся указанные ниже сведения для них.
