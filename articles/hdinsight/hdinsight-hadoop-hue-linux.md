...
2. В колонке **Необязательная конфигурация** выберите **Действия сценария** и введите следующие сведения:
	...
	* __ПАРАМЕТРЫ__: для сценария нужен **пароль администратора кластера** в качестве параметра. Это пароль, указанный при подготовке кластера. Важные особенности задания пароля
...
2. На вкладке **Assist** (Помощь) в разделе **Database** (База данных) отобразится элемент **hivesampletable**. Это пример таблицы, входящей в состав всех кластеров Hadoop в HDInsight. Введите запрос в поле в правой части окна и просмотрите выходные данные на нижней вкладке **Results** (Результаты), как показано на снимке экрана ниже.
...
3. Щелкните правой кнопкой мыши файл или папку, чтобы отобразились доступные операции. Отправить файлы в текущую папку можно с помощью кнопки **Upload** (Отправить) в правом углу. Создать новые файлы и папки можно с помощью кнопки **New** (Создать).
...
5.	Hue поддерживает WebHDFS, если кластеры HDInsight используют хранилище Azure с приставкой `wasb://` в начале пути. Таким образом, пользовательский сценарий, используемый со сценарием действия, устанавливает службу WebWasb, совместимую с WebHDFS и предназначенную для обмена данными с WASB. Поэтому, хотя в на портале Hue иногда отображается HDFS (например, при наведении указателя мыши на элемент **File Browser** (Обозреватель файлов)), на самом деле это означает WASB.
