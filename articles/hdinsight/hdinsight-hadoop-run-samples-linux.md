...
Кластеры HDInsight предоставляют набор примеров MapReduce, при помощи которых можно познакомиться с процедурой запуска заданий Hadoop MapReduce. В этом документе рассказывается о доступных примерах и пошаговом выполнении некоторых из них.
...
**Расположение**. Примеры находятся в кластере HDInsight по адресу **/usr/hdp/current/hadoop-mapreduce-client/hadoop-mapreduce-examples.jar**.
...
**Исходный код**. Исходный код этих примеров хранится в кластере HDInsight по адресу **/usr/hdp/2.2.4.9-1/hadoop/src/hadoop-mapreduce-project/hadoop-mapreduce-examples**.

> [!NOTE]
> Ключ `2.2.4.9-1` в пути соответствует версии платформы данных Hortonworks для кластера HDInsight, которое может измениться при обновлении HDInsight.
> 
> 
...
2. В командной строке `username@#######:~$` введите следующую команду, чтобы вывести список примеров:
    ...
    В результате создается список примеров из предыдущего раздела этого документа.
3. Чтобы получить справку по конкретному примеру, используйте следующую команду: В данном случае запускается пример **wordcount**.
    ...
    Это означает, что можно указать несколько входных путей для исходных документов. Окончательный путь — это место, где сохраняются выходные данные (число слов в исходных документах).
4. Для подсчета всех слов в книге «Записи Леонардо да Винчи», которая поставляется с кластером и служит примером исходных данных, используйте следующую команду:
    ...
    Входные данные для этого задания считываются из файла **wasbs:///example/data/gutenberg/davinci.txt**.
   
    Выходные данные в этом примере сохраняются в расположение **wasbs:///example/data/davinciwordcount**.
...
5. По завершении задания воспользуйтесь следующей командой, чтобы просмотреть результат:
    ...
    Все файлы, созданные в процессе выполнения задания, объединяются и выводятся на экран. В этом несложном примере был создан только один файл, однако при наличии нескольких файлов эта команда выполнит их перебор.
...
> [!NOTE]
> Ключ `2.2.4.9-1` в пути может меняться при обновлении кластера HDInsight.
> 
> 
...
## <a name="10gb-greysort"></a>Graysort&10;GB
...
1. Создайте 10 ГБ данных, которые будут храниться в хранилище кластера HDInsight по умолчанию: **wasbs:///example/data/10GB-sort-input**:
   
        yarn jar /usr/hdp/current/hadoop-mapreduce-client/hadoop-mapreduce-examples.jar teragen -Dmapred.map.tasks=50 100000000 /example/data/10GB-sort-input
   
    Ключ `-Dmapred.map.tasks` говорит Hadoop о том, сколько задач сопоставления будет использоваться в этом задании. Последние два параметра означают, что задание создаст 10 ГБ данных и сохранит их в расположении **wasbs:///example/data/10GB-sort-input**.
