...
4. Для подсчета всех слов в книге «Записи Леонардо да Винчи», которая поставляется с кластером и служит примером исходных данных, используйте следующую команду:
    ...
    Исходные данные для этого задания считываются по следующему пути: **wasbs:///example/data/gutenberg/davinci.txt**.

    Выходные данные для этого задания сохраняются по следующему пути: **wasbs:///example/data/davinciwordcount**.
...
1. Создание 10 ГБ данных, которые будут храниться в хранилище кластера HDInsight по умолчанию по адресу **wasbs:///example/data/10GB-sort-input**:

        yarn jar /usr/hdp/current/hadoop-mapreduce-client/hadoop-mapreduce-examples.jar teragen -Dmapred.map.tasks=50 100000000 /example/data/10GB-sort-input

	Ключ `-Dmapred.map.tasks` говорит Hadoop о том, сколько задач сопоставления будет использоваться в этом задании. Последние два параметра означают, что задание создаст 10 ГБ данных и сохранит их по адресу **wasbs:///example/data/10GB-sort-input**.
