...
* Инструменты Hadoop для Visual Studio. Пошаговые указания по установке и настройке инструментов см. в разделе [Начало работы со средствами HDInsight Hadoop для Visual Studio](hdinsight-hadoop-visual-studio-tools-get-started.md).
...
5. Дважды щелкните **контейнер по умолчанию** для кластера. После этого откроется новое окно с содержимым контейнера по умолчанию.
...
7. После завершения отправки можно будет использовать приложение с помощью запроса Hive.
...
4. В качестве запроса Hive используйте следующее:
...
1. После ввода последней строки запустится задание. В итоге возвратятся выходные данные следующего вида:
