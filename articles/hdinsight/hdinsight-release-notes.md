...
| Microsoft R Server | HDInsight Премиум предоставляет сервер Microsoft R Server, который можно включать в кластеры Hadoop и Spark на платформе Linux. Дополнительные сведения см. в статье [Общие сведения о сервере R Server в HDInsight](hdinsight-hadoop-r-server-overview.md).| служба | Hadoop и Spark на платформе Linux| Недоступно
