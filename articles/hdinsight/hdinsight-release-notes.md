...
## Заметки о выпуске R Server в HDinsight от 17.08.2016

• R Server 8.0.5 — это, главным образом, выпуск для исправления ошибок. Дополнительные сведения см. на странице [R Server Release Notes](https://msdn.microsoft.com/microsoft-r/notes/r-server-notes) (Заметки о выпуске R Server). • Пакет AzureML на граничном узле — [пакет R](https://cran.r-project.org/web/packages/AzureML/vignettes/getting_started.html), позволяющий публиковать модели R и предлагать их пользователям в качестве веб-службы Azure ML. Дополнительные сведения см. в разделе [Ввод модели в эксплуатацию](hdinsight-hadoop-r-server-overview.md#operationalize-a-model) статьи [Общие сведения об R Server в HDInsight (предварительная версия)](hdinsight-hadoop-r-server-overview.md). • Зависимости Linux для [100 наиболее популярных пакетов R](https://github.com/metacran/cranlogs) — теперь эти зависимости пакета Linux предустановлены. • Возможность использования репозитория CRAN при добавлении пакетов R в узлы данных. Дополнительные сведения см. в разделе [Установка пакетов R](hdinsight-hadoop-r-server-get-started.md#install-r-packages) статьи [Приступая к работе с R Server в HDInsight (предварительная версия)](hdinsight-hadoop-r-server-get-started.md). • Повышена надежность подготовки R Server при создании кластеров.


## Заметки о выпуске HDinsight от 01.08.2016
...
## Заметки о выпуске HDinsight от 14.07.2016
...
## Заметки о выпуске HDInsight от 07.07.2016
...
>[AZURE.NOTE] Эти изменения распространяются только на вновь созданные кластеры, версия которых равна или выше версии данного выпуска.

## Заметки о выпуске HDInsight от 06.06.2016
...
| Инструменты HDInsight для IntelliJ | Это подключаемый модуль IntelliJ IDEA для кластеров Spark в HDInsight. Он обеспечивает следующие возможности.<ul><li>Простое создание и программирование приложения Spark в Scala и Java с первоклассной поддержкой разработки для IntelliSense, автоформатированием, проверкой ошибок и т. д.</li><li>Локальное тестирование приложения Spark.</li><li>Отправка заданий в кластер HDInsight Spark и получение результатов.</li><li>Вход в Azure и получение доступа ко всем кластерам Spark, связанным с вашими подписками Azure.</li><li>Обзор всех ресурсов хранилища для кластера HDInsight Spark.</li><li>Просмотр всех журналов заданий и информации о заданиях для кластера HDInsight Spark.</li><li>Удаленная отладка заданий Spark с помощью настольного компьютера.</li></ul>| Средства | Spark| Недоступно

## Заметки о выпуске HDInsight от 13.05.2016
...
## Заметки о выпуске HDinsight 1 от 11.04.2016
...
## Заметки о выпуске HDinsight от 29.03.2016
...
## Заметки о выпуске HDinsight от 22.03.2016
...
* При использовании конечной точки Ambari https://{clusterDns}.azurehdinsight.net/ambari/api/v1/clusters/{clusterDns}.azurehdinsight.net/services/{servicename}/components/{componentname} поле *host\_name* в настоящее время возвращает полное доменное имя (FQDN), а не только имя узла. Например, вместо возврата "**headnode0**" поле теперь возвращает полное доменное имя “**headnode0.{ClusterDNS}.azurehdinsight.net**”. Это изменение было продиктовано необходимостью использовать сценарии, в которых кластеры различного типа, например кластеры HBase и Hadoop, можно было бы размещать в одной виртуальной сети. Такая необходимость может возникнуть, например, при использовании HBase в качестве вспомогательной платформы для Hadoop.
