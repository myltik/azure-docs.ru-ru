...
# Начало работы. Создание кластера Apache Spark в HDInsight Windows и выполнение интерактивных запросов с помощью SQL Spark (предварительная версия)
...
3. Введите имя в поле **Имя кластера**, выберите значение **Spark** для параметра **Тип кластера** и из раскрывающегося меню **Операционная система кластера** выберите **Центр обработки данных Windows Server 2012 R2**. Рядом с именем кластера (если оно доступно) отобразится зеленый флажок.
