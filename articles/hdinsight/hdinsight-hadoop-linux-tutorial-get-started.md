...
- **Подписку на Azure**: См. [Получение бесплатной пробной версии Azure](https://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).
...
Чтобы открыть представления Ambari на портале Azure, выберите в разделе __Быстрые ссылки__ свой кластер HDInsight, а затем — элемент __Просмотры Ambari__.
...
4. Справа от окна редактора запросов отображается ряд значков. Выберите значок, который выглядит как цепочка.
    ...
    * **Параметры**: значок шестеренки позволяет менять настройки Hive, например параметр `hive.execution.engine` или параметры Tez.
    * **Tez**: отображает направленный ациклический граф (DAG), который использовался в Tez для выполнения запроса. Чтобы просмотреть DAG для ранее выполненных запросов, используйте __представление Tez__.
    * **Уведомления**: отображает уведомления об отправке запроса или о возникновении ошибки при его выполнении.
