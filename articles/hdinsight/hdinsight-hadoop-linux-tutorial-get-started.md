...
6. Щелкните **Учетные данные** и введите пароль для пользователя с правами администратора. Также нужно ввести **Имя пользователя SSH** и **ПАРОЛЬ** или **ОТКРЫТЫЙ КЛЮЧ**, который будет использоваться для проверки подлинности пользователя SSH. Мы рекомендуем использовать открытый ключ. Щелкните **Выбрать** внизу страницы, чтобы сохранить конфигурацию учетных данных.
    ...
    > [AZURE.NOTE]SSH используется для удаленного доступа к кластеру HDInsight с помощью командной строки. Заданные здесь имя пользователя и пароль или открытый ключ будут использоваться при подключении к кластеру через SSH. Имя пользователя SSH должно быть уникальным, так как на его основе создается учетная запись пользователя на всех узлах кластера HDInsight. Вот некоторые имена учетных записей, которые зарезервированы для служб в кластере и не должны использоваться в качестве имени пользователя SSH:
    >
...
8. Щелкните **Ценовые категории узла**, чтобы отобразить сведения об узлах, которые будут созданы для этого кластера. Задайте количество рабочих узлов, необходимых для кластера. Оценочная стоимость кластера отобразится в колонке.
	...
	Нажмите **Выбрать**, чтобы сохранить ценовую категорию узла.
...
- [Управление кластерами HDInsight с помощью Ambari](hdinsight-hadoop-manage-ambari.md). Кластеры HDInsight на основе Linux используют Ambari для управления службами Hadoop и их мониторинга. Веб-интерфейс Ambari доступен для каждого кластера по адресу https://CLUSTERNAME.azurehdinsight.net.

	> [AZURE.IMPORTANT]Хотя многие разделы веб-интерфейса Ambari доступны напрямую через Интернет, веб-интерфейс для таких служб Hadoop, как диспетчер ресурсов или журнал заданий, требует использования туннеля SSH. Дополнительную информацию об использовании туннеля SSH с HDInsight см. в следующих статьях.
	>
...
- [Подготовка HDInsight в Linux с использованием настраиваемых параметров](hdinsight-hadoop-provision-linux-clusters.md). Из этой статьи вы узнаете, как подготовить кластеры HDInsight.

- [Работа с HDInsight в Linux](hdinsight-hadoop-linux-information.md). Если вы уже использовали Hadoop на платформах Linux, из этого документа вы больше узнаете об особенностях работы с Azure, например:
...
- Дополнительные сведения о Hive, Pig и MapReduce см. в следующих статьях:

	- [Использование MapReduce с HDInsight][hdinsight-use-mapreduce]
	- [Использование Hive с HDInsight][hdinsight-use-hive]
	- [Использование Pig с HDInsight][hdinsight-use-pig]

- Дополнительные сведения о работе с хранилищем Azure, которое используется вашим кластером HDInsight, см. в следующих статьях:
