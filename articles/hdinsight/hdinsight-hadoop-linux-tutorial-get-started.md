...
4. Справа от окна редактора запросов отображается ряд значков. Выберите значок, который выглядит как цепочка.
	...
	* Параметры. Значок шестеренки позволяет менять настройки Hive, например `hive.execution.engine` или параметры Tez.
	* Tez. Отображает направленный ациклический граф, который использовался в Tez для выполнения запроса. Чтобы просмотреть DAG для ранее выполненных запросов, используйте __представление Tez__.
