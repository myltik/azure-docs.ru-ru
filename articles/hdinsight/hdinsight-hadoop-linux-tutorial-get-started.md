...
- **Ключи SSH** (необязательно): для защиты учетной записи SSH, которая используется для подключения к кластеру с использованием пароля или открытого ключа. Рекомендуется использовать ключ. Этот метод более безопасен, но требует дополнительной настройки. Инструкции по созданию и использованию ключей SSH в HDInsight см. в следующих статьях:
