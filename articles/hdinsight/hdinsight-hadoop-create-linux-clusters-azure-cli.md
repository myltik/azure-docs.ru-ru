...
5. Откройте файл __azuredeploy.parameters.json__ в редакторе и укажите значения для элементов в разделе `parameters`.
    ...
    * __sshUserName__: имя первого пользователя SSH, созданного для этого кластера. SSH будет использоваться для удаленного доступа к кластеру с использованием этой учетной записи. Имя должно быть уникальным; использовать имя учетной записи, которая уже используется в кластере, нельзя. В число недопустимых имен входят root, storm и hbase.
