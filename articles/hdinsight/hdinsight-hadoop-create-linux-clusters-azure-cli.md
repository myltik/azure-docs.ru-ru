...
В этом документе пошагово описан процесс создания нового кластера HDInsight с использованием шаблона и Azure CLI.

##Предварительные требования

- **Подписка Azure.**. См. [Бесплатная пробная версия Azure](http://azure.microsoft.com/documentation/videos/get-azure-free-trial-for-testing-hadoop-in-hdinsight/).

- __Azure CLI__. Сведения об установке интерфейса командной строки см. в разделе [Установка Azure CLI](xplat-cli-install.md).
...
5. Откройте файл __azuredeploy.parameters.json__ в редакторе и укажите значения для элементов в разделе `parameters`.
    ...
    * __sshUserName__: имя первого пользователя SSH, которого необходимо создать для этого кластера. SSH будет использоваться для удаленного доступа к кластеру с использованием этой учетной записи. Имя должно быть уникальным; использовать имя учетной записи, которая уже используется в кластере, нельзя. В число недопустимых имен входят root, storm и hbase.
    * __sshPublicKey__: при использовании шаблона, который требует открытого ключа SSH, необходимо добавить открытый ключ в эту строку. Дополнительные сведения о создании открытых ключей и работе с ними см. в следующих статьях:
...
7. Развертывание может занять определенное время, примерно 15 минут. Чтобы просмотреть информацию о развертывании, выполните указанную ниже команду. Замените __RESOURCEGROUPNAME__ именем группы ресурсов, созданной на предыдущем этапе.
    ...
    После завершения развертывания поле __Состояние__ будет содержать значение __Успешно__. Если во время развертывания произойдет сбой, получить дополнительные сведения об этом сбое можно будет с помощью следующей команды:
