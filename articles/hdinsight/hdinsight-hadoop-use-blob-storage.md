...
В этом учебном материале вы узнаете, как использовать недорогие хранилища BLOB-объектов Azure с HDInsight, создавать учетную запись хранилища Azure и контейнер хранилища BLOB-объектов и затем обращаться к хранимым ими данным.
...
> [AZURE.NOTE]Синтаксис **asv://* не поддерживается в кластерах HDInsight версии 3.0. Это означает, что все задания, отправляемые в кластер HDInsight версии 3.0, в которых явно используется синтаксис **asv://*, завершатся сбоем. Вместо этого следует использовать синтаксис **wasb://*. Кроме того, сбоем будут завершаться задания, отправляемые в кластеры HDInsight версии 3.0, если эти кластеры созданы с существующим метахранилищем, которое содержит явные ссылки на ресурсы с использованием синтаксиса "asv://". Для правильного обращения к ресурсам эти метахранилища необходимо создать повторно, используя синтаксис wasb://.
...
- **Контейнеры в учетных записях хранения, подключенные к кластеру:** Поскольку имя учетной записи и ключ связываются с кластером во время настройки, вы получаете полный доступ к BLOB-объектам в этих контейнерах.
...
## <a id="preparingblobstorage"></a>Создание контейнера BLOB-объектов
...
При подготовке кластера HDInsight на портале предварительной версии вы можете использовать существующую учетную запись хранения или создать новую.

![источник данных для подготовки hadoop в hdinsight](./media/hdinsight-hadoop-use-blob-storage/hdinsight.provision.data.source.png)
...
> [AZURE.NOTE]Параметр `--type` указывает, как будут реплицированы учетные записи хранения. Дополнительные сведения см. в статье [Репликация хранилища Azure](../storage-redundancy.md).
...
Если у вас [установлен и настроен Azure PowerShell][powershell-install], можно выполнить следующие действия в командной строке Azure PowerShell для создания учетной записи хранения и контейнера:
...
## <a id="azurecli"></a>Доступ к BLOB-объектам с помощью Azure CLI
...
## <a id="powershell"></a>Доступ к BLOB-объектам с помощью Azure PowerShell
...
**Пример использования Azure PowerShell для выполнения запроса Hive с использованием неопределенной учетной записи хранения**
