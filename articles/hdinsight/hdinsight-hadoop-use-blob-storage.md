---
title: Запрос данных из HDFS-совместимых BLOB-хранилищ | Microsoft Docs
...
> [!NOTE]
> В версиях HDInsight до 3.0 вместо префикса `asv://` использовался `wasb://`. Префикс `asv://` не следует использовать с кластерами HDInsight версии 3.0 и выше, так как это приведет к ошибке.
> 
> 
...
Помимо этой учетной записи хранения в процессе создания или после создания кластера можно добавить дополнительные учетные записи хранения из той же или других подписок Azure. Инструкции по добавлению дополнительных учетных записей хранения см. в статье [Создание кластеров HDInsight][hdinsight-creation].
...
> [!NOTE]
> Параметр `--type` указывает, как будут реплицированы учетные записи хранения. Дополнительные сведения см. в статье [Репликация хранилища Azure](../storage/storage-redundancy.md). Не используйте хранилище, избыточное в пределах зоны (ZRS), поскольку оно не поддерживает страничный BLOB-объект, файл, таблицу или очереди.
> 
> 
...
В этом пример показано, как получить список папки из учетной записи хранения, которая не была задана при создании.
    $clusterName = "<имя\_кластера\_HDInsight>"
