...
> [AZURE.NOTE] Этот пример доступен по адресу [https://github.com/Azure-Samples/hdinsight-python-storm-wordcount](https://github.com/Azure-Samples/hdinsight-python-storm-wordcount) в каталоге __JavaTopology__. Это проект на основе Maven. Если вы не знакомы с Maven, ознакомьтесь со статьей [Разработка топологий на основе Java с помощью Apache Storm в HDInsight](hdinsight-storm-develop-java-topology.md), чтобы получить дополнительные сведения о создании проекта Maven для топологии Storm.
...
2. Загрузите JAR-файл в кластер Hadoop с помощью одного из следующих методов.
    ...
    * Для кластеров HDInsight __под управлением Windows__: подключитесь к панели мониторинга Storm, перейдя по адресу HTTPS://<имя_кластера>.azurehdinsight.net/ в браузере. Замените CLUSTERNAME именем кластера HDInsight и укажите имя администратора и пароль в ответ на запрос.
