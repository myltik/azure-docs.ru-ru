...
> [!NOTE]
> При использовании PowerShell параметры `-D` необходимо заключить в кавычки. Ниже приведен пример команды с использованием PowerShell.
>
> `mvn archetype:generate "-DarchetypeArtifactId=maven-archetype-quickstart" "-DgroupId=com.microsoft.example" "-DartifactId=WordCount" "-DinteractiveMode=false"`

В результате в текущем расположении создается каталог с именем `WordCount`, содержащий базовый проект Maven.
...
> [!NOTE]
> Обратите внимание на строку `<scope>provided</scope>` в этом разделе. Она указывает Maven, что **storm-core** нужно исключить из всех созданных JAR-файлов, так как этот элемент предоставит система. Это позволяет немного уменьшить размер создаваемых пакетов и обеспечить использование битов **storm-core**, предусмотренных в Storm в кластере HDInsight.
...
> [!NOTE]
> Обратите внимание, что в записи `<mainClass>` используется `${storm.topology}`. В разделе свойств `pom.xml` это значение не определяется. Нам потребуется задать это значение из командной строки при выполнении топологии в среде разработки позже.

Другим полезным подключаемым модулем является  [Apache Maven Compiler](http://maven.apache.org/plugins/maven-compiler-plugin/), который используется для изменения параметров компиляции. Он нам нужен в основном для того, чтобы изменить версию Java, используемую Maven для исходного и целевого объектов назначения приложения.
...
Раздел ресурсов позволяет включать не связанные с кодом ресурсы, такие как файлы конфигурации, необходимые для компонентов топологии. Для этого примера добавьте следующий фрагмент в раздел `<resources>` файла pom.xml:
...
В корневую папку проекта (`${basedir}`) будет добавлен каталог ресурсов как расположение, содержащее ресурсы и файл с именем `log4j2.xml`. Этот файл используется для настройки того, какая информация записывается топологией.
...
Создайте для элемента spout в каталоге `src\main\java\com\microsoft\example` файл с именем `RandomSentenceSpout.java` и используйте следующий код в качестве содержимого этого файла:
...
В каталоге `src\main\java\com\microsoft\example` создайте два файла — `SplitSentence.java` и `WordCount.Java`. Измените содержимое файлов следующим образом:
...
Для реализации топологии создайте в каталоге `src\main\java\com\microsoft\example` файл с именем `WordCountTopology.java`. Измените содержимое файла следующим образом:
...
Storm использует Apache Log4j для записи информации в журнал. Если не настроить ведение журнала, то топология будет выдавать много диагностических сведений, которые могут быть трудными для чтения. Чтобы управлять записываемыми сведениями, создайте в каталоге `resources` файл с именем `log4j2.xml`. Добавьте в этот файл следующее содержимое.
...
2. В каталоге `resources` создайте файл с именем `topology.yaml`. Используйте следующее в качестве содержимого этого файла:
