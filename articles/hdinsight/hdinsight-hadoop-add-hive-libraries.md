...
* Путь WASB к контейнеру необходимо указать в виде параметра для действия сценария. Например, если JAR-файлы хранятся в контейнере __libs__ в учетной записи хранения __mystorage__, параметром будет \_\___wasb://libs@mystorage.blob.core.windows.net/__.
...
2. В колонке **Необязательная конфигурация** выберите **Действия скрипта** и введите следующие сведения:
    ...
    * __ПАРАМЕТРЫ__: введите адрес WASB к контейнеру и учетной записи хранения, содержащий JAR-файлы. Например: \_\___wasb://libs@mystorage.blob.core.windows.net/__.
