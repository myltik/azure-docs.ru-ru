...
2. После создания проекта удалите каталог __exampleudf/src/test__, который создан как часть проекта. Этот каталог не нужен для нашего примера.

3. Откройте файл __exampleudf/pom.xml__ и замените текущую запись `<dependencies>` следующей:
