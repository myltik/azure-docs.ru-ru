...
> [AZURE.IMPORTANT] Хотя имя кластера в полном доменном имени (FQDN) в URI (CLUSTERNAME.azurehdinsight.net) указывается без учета регистра, другие вхождения этого URI учитывают регистр. Например, если кластер называется MyCluster, допустимые URI имеют следующий вид:
>
...
Подключение к Ambari в HDInsight выполняется по протоколу HTTPS. Также необходимо выполнить вход в Ambari с помощью имени учетной записи администратора (по умолчанию используется __admin__) и пароля, указанных при создании кластера.
...
Так как это сведения в формате JSON, для получения данных удобнее использовать средство синтаксического анализа JSON. Например, если требуется получить сведения о состоянии работоспособности кластера, то можно использовать следующее.

    curl -u admin:PASSWORD -G "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME" | jq '.Clusters.health_report'
    
Эта команда получает документ JSON, а затем передает выходные данные в jq. `.Clusters.health_report` указывает в JSON-документе элемент, который требуется получить.
...
1. Получите текущую конфигурацию, которую Ambari хранит в виде "требуемой конфигурации":
    ...
    Эта команда возвращает документ JSON с текущей конфигурацией (идентифицируется по значению _tag_) для компонентов, установленных в кластере. Например, ниже приведен отрывок данных, возвращаемых из типа кластера Spark.
...
2. Извлеките конфигурацию для компонента и значение tag с помощью следующей команды: Замените __spark-thrift-sparkconf__ и __INITIAL__ на компонент и значение tag, по которым хотите получить конфигурацию.
    ...
    Curl извлекает документ JSON, а затем используется jq, чтобы внести некоторые изменения для создания шаблона, который можно применить для добавления и изменения значений конфигурации. В частности, он выполняет следующие действия:
    ...
    * Добавляет новый элемент __tag__ и задает для него значение __version#################__, где числовая часть основана на текущей дате. Каждая конфигурация должна иметь уникальное значение tag.
    
    Наконец, данные сохраняются в документе __newconfig.json__. Структура документа будет выглядеть следующим образом:
...
3. Откройте документ __newconfig.json__ и измените или добавьте значения в объекте __properties__. Например, измените значение __"spark.yarn.am.memory"__ с __"1g"__ на __"3g"__ и добавьте новый элемент для __"spark.kryoserializer.buffer.max"__ со значением __"256m"__.
...
4. Используйте следующую команду для отправки измененной конфигурации в Ambari:

        cat newconfig.json | curl -u admin:PASSWORD -H "X-Requested-By: ambari" -X PUT -d "@-" "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME"
        
    Эта команда передает содержимое файла __newconfig.json__ в cURL-запрос, который отправляет его в кластер в качестве новой требуемой конфигурации. При этом возвращается документ JSON. Элемент __versionTag__ в этом документе должен совпадать с отправленной версией, а объект __configs__ содержит запрошенные изменения конфигурации.
...
1. Используйте следующую команду для перевода службы Spark в режим обслуживания.

        echo '{"RequestInfo": {"context": "turning on maintenance mode for SPARK"},"Body": {"ServiceInfo": {"maintenance_state":"ON"}}}' | curl -u admin:PASSWORD -H "X-Requested-By: ambari" -X PUT -d "@-" "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME/services/SPARK"

    Она отправляет документ JSON на сервер (содержится в операторе `echo`) для включения режима обслуживания. Проверить, находится ли служба в этом режиме, можно с помощью следующего запроса.
    
        curl -u admin:PASSWORD -H "X-Requested-By: ambari" "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME/services/SPARK" | jq .ServiceInfo.maintenance_state
        
    При этом возвращается значение `"ON"`.

3. Затем используйте следующую команду для отключения службы.

        echo '{"RequestInfo": {"context" :"Stopping the Spark service"}, "Body": {"ServiceInfo": {"state": "INSTALLED"}}}' | curl -u admin:PASSWORD -H "X-Requested-By: ambari" -X PUT -d "@-" "https://CLUSTERNAME.azurehdinsight.net/api/v1/clusters/CLUSTERNAME/services/SPARK"
        
    Вы получаете ответ, аналогичный показанному ниже.
...
4. После завершения предыдущего запроса используйте следующую команду для запуска службы.
    ...
    После перезапуска службы она использует новые параметры конфигурации.
