...
Чтобы получить IP-адрес, необходимо знать полное доменное имя узла. Если у вас есть полное доменное имя, вы можете получить IP-адрес узла. В следующих примерах у Ambari сначала запрашивается полное доменное имя для всех узлов, а затем — IP-адрес каждого узла.
...
2. Извлеките конфигурацию для компонента и значение tag с помощью следующих команд. Замените **spark-thrift-sparkconf** и **INITIAL** на компонент и значение tag, по которым хотите получить конфигурацию.
