...
2. Введите имя приложения в поле **Description** \(Описание\) и введите значение в поле **Bundle Identifier** \(Идентификатор набора\), выберите вариант Push Notifications \(Push-уведомления\) в разделе App Services \(Службы приложений\), а затем щелкните **Continue** \(Продолжить\).
   ...
   ![][108]
	...
	При этом создается идентификатор вашего приложения и появляется запрос об отправке информации. Нажмите **Submit**.

   ![][109]

	После нажатия кнопки **Submit** (Отправить) вы увидите экран **Registration complete** (Регистрация выполнена), представленный ниже. Нажмите кнопку **Done**.

   ![][110]

	> [AZURE.NOTE] Если вы хотите указать значение **Bundle Identifier** (Идентификатор набора), отличное от , необходимо также обновить значение идентификатора набора в проекте Xcode.  
...
1. В Xamarin Studio создайте новый проект iOS и выберите шаблон **Unified API \> Single View Application** \(Единый API \> Приложение одного представления\).

   ![][31]

2. Сначала добавьте ссылку на компонент обмена сообщениями Azure. В представлении "Решение" щелкните правой кнопкой мыши папку **Компоненты** для вашего проекта и выберите **Получить больше компонентов**. Найдите компонент **Мобильные службы Azure** и добавьте его в проект.
...
9. Создайте следующий метод **ProcessNotification\(\)** в **AppDelegate.cs**:
    ...
    > [AZURE.NOTE]Вы можете переопределить метод **FailedToRegisterForRemoteNotifications\(\)** для обработки различных ситуаций, таких как отсутствие сетевого подключения и т. д.
