<properties 
	pageTitle="Настройка смоделированной гибридной облачной среды для тестирования" 
...
#Настройка смоделированной гибридной облачной среды для тестирования
...
[Настройка гибридной облачной среды для тестирования](http://azure.microsoft.com/documentation/articles/virtual-networks-setup-hybrid-cloud-environment-testing/) в случае отсутствия прямого подключения к Интернету и доступного общего IP-адреса. Это конфигурация, которая получается в результате.

![](./media/virtual-networks-set-up-simulated-hybrid-cloud-environment-for-testing/CreateSimHybridCloud_4.png)

Они позволяет смоделировать гибридную облачную рабочую среду. Состав конфигурации:
...
Используйте указания в документе [Руководство по TestLab: базовая конфигурация в Azure](http://www.microsoft.com/download/details.aspx?id=41684) для настройки компьютеров DC1, APP1 и CLIENT1 в виртуальной сети Azure с именем TestLab. 
...
Далее следуйте инструкциям раздела [Установка и настройка Azure PowerShell для установки Azure PowerShell на локальном компьютере](../install-configure-powershell/).
...
4.	Щелкните **Сохранить** на панели задач.
...
8.	Щелкните **Сохранить** на панели задач.
...
Далее необходимо настроить общий ключ для обоих шлюзов, чтобы они использовали одно и то же значение ключа, которое было определено с помощью портала управления Azure для виртуальной сети TestLab. Выполните следующие команды из командной строки Azure PowerShell с правами администратора на локальном компьютере, указав значение общего ключа TestLab.
...
2.	В панели содержимого в группе **Диски** щелкните **диск 2** (с параметром **Раздел**, имеющим значение **Неизвестно**).
...
- [Ферма интрасети SharePoint](../virtual-networks-setup-sharepoint-hybrid-cloud-testing/)
- [Веб бизнес-приложение](../virtual-networks-setup-lobapp-hybrid-cloud-testing/)
- [Сервер синхронизации каталога (DirSync) Office 365](../virtual-networks-setup-dirsync-hybrid-cloud-testing/)

##Дополнительные ресурсы

[Настройка гибридной облачной среды для тестирования](../virtual-networks-setup-hybrid-cloud-environment-testing/)

[Настройка подключения типа "VNet-to-VNet"](http://msdn.microsoft.com/library/azure/dn690122.aspx)


##<a id="costs"></a>Минимизация текущих расходов данной среды

Чтобы свести к минимуму затраты на работающие виртуальные машины в этой среде, следует как можно быстрее выполнить необходимое тестирование и демонстрацию, а затем удалить или выключить виртуальные машины, если они не используются. Например, можно использовать автоматизацию Azure и Runbook для автоматического выключения виртуальных машин в виртуальных сетях TestLab и Test_VNET в конце каждого рабочего дня. Подробнее см. раздел [Начало работы с автоматизацией Azure](../automation-create-runbook-from-samples/). При повторном запуске виртуальных машин в подсети Corpnet следует сначала запустить DC1.
