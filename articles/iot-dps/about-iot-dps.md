---
title: "Общие сведения о службе подготовки устройств для Центра Интернета вещей Azure (предварительная версия) | Документация Майкрософт"
...
# <a name="provisioning-devices-with-azure-iot-hub-device-provisioning-service-preview"></a>Подготовка устройств с помощью службы подготовки устройств для Центра Интернета вещей Azure (предварительная версия)
...
На этом этапе производитель также предоставляет специалисту по развертыванию или оператору устройств сведения об идентифицирующем ключе. По сути, процедура может заключаться в подтверждении того, что все устройства имеют сертификат X.509, созданный из корневого ЦС, который предоставил специалист по развертыванию или оператор устройств, и извлечения открытой части ключа подтверждения доверенного платформенного модуля (TPM) из каждого устройства TPM. Сегодня эти службы предлагают многие кремниевые производители.
...
Служба подготовки устройств, как и все службы Интернета вещей Azure, работает на нескольких платформах с различными операционными системами. Общедоступная предварительная версия поддерживает ограниченный набор языков и протоколов. С выходом общедоступной версии их список значительно расширится. В общедоступной предварительной версии служба подготовки устройств поддерживает только HTTPS-подключения для операций устройства и службы. Пакет SDK для устройства написан на языке C, а пакет SDK для службы — на языке C#.

## <a name="regions"></a>регионы
Общедоступная предварительная версия службы подготовки устройств доступна в восточной части США, Западной Европе и Юго-Восточной Азии. Список имеющихся и новых регионов обновляется для всех служб.

* [Регионы Azure](https://azure.microsoft.com/regions/)

## <a name="availability"></a>Доступность
Мы поддерживаем оптимальный уровень доступности службы на этапе общедоступной предварительной версии. На этапе общедоступной предварительной версии соглашение об уровне обслуживания не действует. В полном [соглашении об уровне обслуживания Azure](https://azure.microsoft.com/support/legal/sla/) поясняется гарантированная доступность Azure в целом.
...
Служба подготовки устройств автоматизирует подготовку устройств с помощью Центра Интернета вещей. См. дополнительные сведения о [Центре Интернета вещей](https://docs.microsoft.com/en-us/azure/iot-hub/).
