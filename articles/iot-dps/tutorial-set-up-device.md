...
1. Библиотека должна реализовывать функции, определенные в следующем файле заголовка: a. Для пользовательских доверенных платформенных модулей реализуются функции, определенные в [соответствующей документации](https://github.com/Azure/azure-iot-sdk-c/blob/master/provisioning_client/devdoc/using_custom_hsm.md#hsm-tpm-api).
    Б. Для пользовательских устройств X.509 реализуются функции, определенные в [соответствующей документации](https://github.com/Azure/azure-iot-sdk-c/blob/master/provisioning_client/devdoc/using_custom_hsm.md#hsm-x509-api). 

### <a name="integrate-with-the-device-provisioning-service-client"></a>Интеграция с клиентом службы подготовки устройств

Как только библиотека будет автоматически успешно создана, можно перейти к клиентскому пакету SDK для Центра Интернета вещей, чтобы можно было использовать свою библиотеку:
