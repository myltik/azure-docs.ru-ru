...
>[WACOM.NOTE] В Visual Studio Online есть [более ранняя версия Application Insights](http://msdn.microsoft.com/ru-ru/library/dn481095.aspx), которая поддерживает больше типов приложений. Мы полностью перерабатываем ее для Microsoft Azure, и это как раз та новая версия, о которой идет речь здесь.
