...
## <a name="event-hub-client"></a>Клиент концентратора событий
...
### <a name="create-an-event-hub-client"></a>Создание клиента концентратора событий
...
Для получения событий из концентраторов событий рекомендуется использовать узел [**EventProcessorHost**](##Event-Processor-Host-APIs), который предоставляет возможность автоматически отслеживать данные о смещении и секциях. Однако, существуют определенные ситуации, в которых для получения событий может потребоваться гибкость основной библиотеки концентраторов событий.
