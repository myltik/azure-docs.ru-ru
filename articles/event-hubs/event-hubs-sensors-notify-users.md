...
Проблема в том, как получать критически важную информацию при наступлении определенных условий не в процессе проверки статических отчетов. Если для получения данных от устройств или корпоративных приложений, таких как [Dynamics AX][], используется [концентратор событий Azure][] или [Центр Интернета вещей][], то вы можете использовать различные способы обработки этих данных. Вы можете просматривать данные на веб-сайте, анализировать их, сохранять и использовать для активации определенных команд. Для этого вы можете использовать такие мощные инструменты, как [веб-сайты Azure][], [SQL Azure][], [HDInsight][], [Cortana Intelligence Suite][], [IoT Suite][], [Logic Apps][] или [центры уведомлений Azure][]. Однако в некоторых случаях требуется только отправка данных с минимальной дополнительной нагрузкой на систему. Чтобы показать вам, как это сделать, используя лишь небольшой фрагмент кода, мы возьмем новый образец — [AppToNotifyUsers][]. В число доступных параметров входя электронная почта (SMTP), SMS и телефон.
