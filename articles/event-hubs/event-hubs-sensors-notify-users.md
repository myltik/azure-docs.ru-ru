---
title: Уведомление пользователей о данных, полученных с датчиков или из других систем | Microsoft Docs
...
Проблема в том, как получать критически важную информацию при наступлении определенных условий не в процессе проверки статических отчетов. Если для получения данных от устройств или корпоративных приложений, таких как [Dynamics AX][], используется [концентратор событий Azure][концентратор событий Azure] или [Центр Интернета вещей][], то вы можете использовать различные способы обработки этих данных. Вы можете просматривать данные на веб-сайте, анализировать их, сохранять и использовать для активации определенных команд. Для этого вы можете использовать такие мощные инструменты, как [веб-сайты Azure][веб-сайты Azure], [SQL Azure][SQL Azure], [HDInsight][HDInsight], [Cortana Intelligence Suite][Cortana Intelligence Suite], [IoT Suite][IoT Suite], [Logic Apps][Logic Apps] или [центры уведомлений Azure][центры уведомлений Azure]. Однако в некоторых случаях требуется только отправка данных с минимальной дополнительной нагрузкой на систему. Чтобы показать вам, как это сделать, используя лишь небольшой фрагмент кода, мы возьмем новый образец — [AppToNotifyUsers][AppToNotifyUsers]. В число доступных параметров входя электронная почта (SMTP), SMS и телефон.
