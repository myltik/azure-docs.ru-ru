...
В этой статье перечислены некоторые исключения, создаваемые API библиотеку обмена сообщениями Azure Service Bus, которые включают API концентраторов событий. Этот справочник может измениться, поэтому следите за обновлениями.
...
1. Нагрузка не распределяется равномерно между всех разделов в концентраторе событий и один раздел попаданий в ограничение единицы локальной пропускной способности.
...
2. Пространство имен концентраторы событий не имеет достаточно единиц производительности (можно проверить **метрики** событий экрана окна имен концентраторы в [портал Azure](https://portal.azure.com) для подтверждения). Обратите внимание, что на портале отображаются обобщенные (за 1 минуту) сведения, но мы измеряем пропускную способность в режиме реального времени, то есть это только оценка.

    Способ устранения: увеличить число единиц пропускной способности для пространства имен. Это можно сделать на портале, в **шкалы** окна экрана имен концентраторов событий.
