...
Самый простой способ начать работу с концентраторами событий — использовать поведение по умолчанию. Если вы создаете объект `EventHubClient` и используете функцию отправки, то события автоматически распределяются между секциями в концентраторе событий. Такое поведение обеспечивает наивысший показатель времени непрерывной работы.
