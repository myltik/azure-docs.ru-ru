<properties linkid="manage-services-how-to-deploy-websites" urlDisplayName="Развертывание веб-сайта Azure" pageTitle="Развертывание веб-сайта Azure" metaKeywords="развертывание публикация веб-сайта Azure" description="Узнайте, какие методы доступны для развертывания контента на веб-сайте Azure." metaCanonical="" services="web-sites" documentationCenter="" title="Развертывание веб-сайта Azure" authors="tdykstra" solutions="" manager="wpickett" editor="mollybos" />

#Развертывание веб-сайта Azure

Имеется множество вариантов развертывания вашего контента на веб-сайте Azure. В этом разделе приводится краткое описание каждого варианта и даются ссылки на дополнительные сведения.
...
[Visual Studio Online](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#gittfs) (ранее Team Foundation Service) — это облачные решение корпорации Майкрософт для управления версиями и совместной работы. Служба предоставляется бесплатно для группы, включающей до 5 разработчиков. VSO можно настроить для выполнения непрерывной доставки на веб-сайт Azure, а в репозитории можно использовать [Git или TFVC](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#gittfs).

Для получения дополнительных сведений см. следующие ресурсы:

* [Непрерывная доставка в Azure](http://www.visualstudio.com/ru-ru/learn/continuous-delivery-in-vs). Пошаговый учебник, в котором показано, как настроить непрерывную доставку из VSO на веб-сайт Azure, используя TFVC. TFVC — это централизованный компонент системы управления версиями в VSO в противовес Git, который является распределенным компонентом системы управления версиями.
* [Непрерывная доставка в Azure с использованием Visual Studio Online](/ru-ru/documentation/articles/cloud-services-continuous-delivery-use-vso/). Регистрация для использования VSO, возвращение проекта в систему управления версиями и настройка непрерывной доставки (автоматическое развертывание) в Azure. Создан для облачных служб, но большая часть процесса настройки развертывания в Visual Studio Online подходит и для веб-сайтов.
...
* [Развертывание в Azure с использованием Dropbox](http://blogs.msdn.com/b/windowsazure/archive/2013/03/19/new-deploy-to-windows-azure-web-sites-from-dropbox.aspx). Использование портала управления Azure для настройки развертывания в DropBox.
* [DropBox и веб-сайты Azure](http://channel9.msdn.com/Series/Windows-Azure-Web-Sites-Tutorials/Dropbox-Deployment-to-Windows-Azure-Web-Sites). В этом видео показан пошаговый процесс подключения папки DropBox к веб-сайту Azure и продемонстрировано, как быстро можно активировать веб-сайт или осуществить его обслуживание, используя простое развертывание путем перетаскивания.
* [Форум Azure для Git, Mercurial и DropBox](http://social.msdn.microsoft.com/Forums/windowsazure/ru-ru/home?forum=azuregit).
...
[Visual Studio](http://www.visualstudio.com/ru-ru/downloads/download-visual-studio-vs.aspx) и [WebMatrix](http://www.microsoft.com/web/webmatrix/) — это интегрированные среды разработки (IDE) корпорации Майкрософт, которые можно использовать для разработки веб-приложений. Оба среды предоставляют встроенные функции, упрощающие развертывание на веб-сайтах Azure.  Обе среды могут использовать [веб-развертывание](http://www.iis.net/downloads/microsoft/web-deploy) для автоматизации дополнительных задач, связанных с развертыванием, таких как развертывание базы данных и изменение строки подключения. В обеих средах можно выполнять развертывание с использованием [FTP и FTPS](http://en.wikipedia.org/wiki/File_Transfer_Protocol). 

WebMatrix можно быстро установить и легко освоить, но Visual Studio предлагает значительно больше возможностей для работы с веб-сайтами Azure. В Visual Studio IDE можно создавать, останавливать, запускать и удалять веб-сайты Azure, можно просматривать журналы по мере их создания в режиме реального времени, можно выполнять удаленную отладку и многое другое. Visual Studio также интегрируется с такими системами управления версиями, как [Visual Studio Online](#vso), [Team Foundation Server](#tfs) и [репозитории Git](#git).

###<a name="vs"></a>Visual Studio

Для получения сведений о выполнении развертывания на веб-сайты Azure из Visual Studio см. следующие ресурсы:
...
Для получения сведений о выполнении развертывания на веб-сайты Azure из WebMatrix см. следующие ресурсы:

* [Разработка и развертывание веб-сайта с помощью Microsoft WebMatrix](http://www.windowsazure.com/ru-ru/documentation/articles/web-sites-dotnet-using-webmatrix/). Создание простого веб-сайта ASP.NET с использованием шаблона WebMatrix и развертывание его на веб-сайте Azure с использованием WebMatrix и веб-развертывания.
...
* [Управление веб-сайтами](http://www.windowsazure.com/ru-ru/documentation/articles/web-sites-manage/). Содержит дополнительные сведения, не включенные в учебник по PHP, о том, как задать учетные данные для FTP. В комментариях, расположенных в нижней части страницы, можно найти сведения о получении учетных данных, отсутствующие в самом документе.
...
* [Непрерывная доставка для облачных служб в Azure](/ru-ru/develop/net/common-tasks/continuous-delivery/). Этот документ предназначен для облачной службы Azure, но некоторое его содержимое относится к веб-сайтам.

###<a name="gitmercurial"></a>Локальные репозитории Git или Mercurial

В Azure можно ввести URL-адрес любого репозитория, использующего Git или Mercurial, чтобы выполнить развертывание из этого расположения. Можно также принудительно отправить контент прямо из локального репозитория Git на веб-сайт Azure. 
...
Вы легко можете создать учетные данные [FTP или FTPS](http://en.wikipedia.org/wiki/File_Transfer_Protocol) для веб-сайта Azure, а затем использовать эти учетные данные с пакетными скриптами FTP.
...
Можно выполнять функции MSBuild или FTP из [Windows PowerShell](http://msdn.microsoft.com/ru-ru/library/dd835506.aspx). В этом случае можно также использовать набор командлетов Windows PowerShell, упрощающих вызов API-интерфейса управления Azure REST.
...
Дополнительные сведения см. в разделе, посвященном развертыванию, в [документации по веб-сайтам Azure](/ru-ru/documentation/services/web-sites/).
