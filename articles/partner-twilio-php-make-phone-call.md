...
Следующий код HTML показывает, как построить веб-страницу (** callform.html**), которая получает данные пользователя для выполнения звонка:
...
В следующем коде показано, как создать веб-страницу (**makecall.php**), которая вызывается, когда пользователь отправляет форму, отображаемую элементом **callform.html**. Показанный ниже код создает сообщение звонка и выполняет его. Вместо заполнителей **$sid** и **$token** в приведенном ниже коде следует указать вашу учетную запись Twilio и маркер проверки подлинности.
