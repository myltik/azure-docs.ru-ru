<properties title="Выполнение телефонных звонков из Twilio (PHP) - Azure" pageTitle="Выполнение телефонных звонков из Twilio (PHP) - Azure" metaKeywords="Azure PHP Twilio, Azure Twilio, телефонные звонки Azure, Azure twilio, Azure SMS, Azure SMS, голосовые вызовы Azure, голосовые вызовы azure, текстовые сообщения Azure, текстовые сообщения Azure, PHP twilio Azure" description="Узнайте, как осуществлять телефонные вызовы и отправлять SMS-сообщения с использованием службы API Twilio для Azure. Примеры, для PHP-приложения." documentationCenter="PHP" services="" solutions="" videoId="" scriptId="" authors="waltpo" manager="bjsmith" editor="mollybos" />

# Осуществление телефонных звонков с использованием Twilio в PHP-приложении Azure 
...
В следующем коде показано, как построить веб-страницу (**makecall.php**), которая вызывается при отправке пользователем формы, отображаемой на странице **callform.html**. Показанный ниже код создает сообщение звонка и выполняет его. Вместо заполнителей **$sid** и **$token** в приведенном ниже коде следует указать вашу учетную запись Twilio и маркер проверки подлинности.
...
## Запуск приложения
На следующем шаге выполняется развертывание приложения на веб-сайтах Azure. В следующих статьях приводятся сведения о создании веб-сайта и развертывании кода с помощью Git, FTP или WebMatrix (некоторая информация в этих статьях нерелевантна):
...
## Дальнейшие действия
В этом коде демонстрируются базовые функциональные возможности службы Twilio в PHP в Azure. Возможно, перед развертыванием в рабочей среде Azure потребуется добавить в него дополнительные обработчики ошибок и другие функции. Например:
...
[howto_blob_storage_php]: http://msdn.microsoft.com/ru-ru/library/windowsazure/hh674502(v=vs.103).aspx
[howto_sql_azure_php]: http://msdn.microsoft.com/ru-ru/library/windowsazure/hh674500(v=vs.103).aspx
