...
Предложение SELECT (**SELECT <список_для_выбора>**) является обязательным. Оно указывает значения, которые будут получены из запроса. Здесь задаются значения JSON, которые используются для создания новых объектов JSON.
...
| hexadecimal_literal |Число, представленное строкой&0;x, за которой следует строка с шестнадцатеричными цифрами. |
...
| Функция | Описание |
| -------- | ----------- |
| CONCAT(x, …) | Возвращает строку, являющуюся результатом объединения двух или более строковых значений. |
