...
Прямые методы реализуются на устройстве. Чтобы процесс создания экземпляра прошел удачно, может потребоваться указать входные данные в полезных данных метода или не указывать их. Вызов прямого метода осуществляется через обращенный к службе URI (`{iot hub}/twins/{device id}/methods/`). Устройство получает непосредственные методы через раздел MQTT конкретного устройства (`$iothub/methods/POST/{method name}/`) или с помощью ссылки AMQP (`IoThub-methodname` и `IoThub-status` свойства приложения). 
...
### <a name="amqp"></a>AMQP
#### <a name="method-invocation"></a>Вызов метода
Устройство получает прямых запросов, создав получить ссылку на адрес`amqps://{hostname}:5671/devices/{deviceId}/methods/deviceBound`

Ссылку receive, представляющий метод запрос приходит сообщение AMQP. Он содержит следующие элементы:
* Свойство идентификатора корреляции, который содержит идентификатор запроса, который должен быть передан обратно с соответствующим ответом метод
* Свойства приложения с именем `IoThub-methodname`, который содержит имя метода, вызываемого
* Тело сообщения AMQP, содержащий метод полезных данных в JSON
...
Метод ответ возвращается отправку ссылку и имеет следующую структуру:
* Свойство идентификатора корреляции, который содержит идентификатор запроса, переданного в сообщении запроса метода
* Свойства приложения с именем `IoThub-status`, содержащий пользователя указано состояние метод
* Тело сообщения AMQP, содержащий метод ответ в виде JSON
