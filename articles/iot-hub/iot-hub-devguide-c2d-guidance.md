---
title: "Руководство по обмену данными между облаком и устройством в Центре Интернета вещей Azure | Документация Майкрософт"
description: "Руководство разработчика по Центру Интернета вещей Azure — это рекомендации по использованию прямых методов, требуемых свойств двойника устройства или сообщений, отправляемых из облака на устройство."
services: iot-hub
documentationcenter: 
author: fsautomata
manager: timlt
...
Центр Интернета вещей предоставляет следующие три варианта отправки сведений из приложения устройства в серверную часть.
...
| Поток данных | Двусторонняя связь. Приложение устройства может немедленно отвечать на метод. Серверная часть получает результат в зависимости от контекста запроса. | Односторонняя связь. Приложение устройства получает уведомление при изменении свойства. | Односторонняя связь. Приложение устройства получает сообщение.
...
| Размер | Запросы и ответы до 8 КБ. | Максимальный размер требуемого свойства — 8 КБ. | Сообщения до 256 КБ. |
...
| Протокол | Доступно при использовании для MQTT и AMQP. | В настоящее время доступно только при использовании MQTT. | Доступно при использовании всех протоколов. При использовании HTTP устройству необходимо выполнять опрос. |
...
* [Настройка устройств с помощью требуемых свойств][lnk-twin-properties] — сведения о требуемых свойствах двойника. 
