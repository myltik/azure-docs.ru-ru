---
title: Руководство разработчика. Управление доступом к Центру Интернета вещей | Microsoft Docs
...
> [!NOTE]
> [Пакеты SDK Центра Интернета вещей Azure][lnk-sdks] автоматически создают маркеры при подключении к службе. В некоторых случаях пакеты SDK поддерживают не все протоколы или не все методы проверки подлинности.
> 
> 
...
> [!NOTE]
> Можно создать маркер безопасности с помощью инструмента .NET [Обозреватель устройств][lnk-device-explorer].
> 
> 
...
> [!NOTE]
> Центр IoT не требует и не хранит весь клиентский сертификат X.509, а только его отпечаток.
> 
> 
