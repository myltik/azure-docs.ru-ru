...
- node
- npm
- CMake (версии 2.8 или более поздней). Это средство можно установить со страницы <https://cmake.org/download/>. Установите соответствующий флажок, чтобы добавить CMake в переменную PATH текущего пользователя;
...
3.  Из корневой папки, куда клонирован репозиторий **azure-iot-sdks**, перейдите в каталог **azure-iot-sdks/node/service/samples** и выполните сценарий, заменив значение заполнителя своей строкой подключения из предыдущего раздела:
...
Используя терминал, в каталоге **azure-iot-sdks/node/service/samples** выполните следующее:

  ```
  simulate.sh
  ```

Этот скрипт выводит команды, которые необходимо выполнить для запуска **iotdm\_simple\_sample** для каждого устройства, указанного в файле **devicecreds.txt**. Запустите команды отдельно в отдельных окнах терминала для каждого виртуального устройства. Виртуальное устройство будет работать, пока командное окно не будет закрыто.
