...
После добавления устройств в реестр устройств можно запустить виртуальные управляемые устройства. Для каждого удостоверения устройства, подготовленного к работе в центре IoT Azure, нужно запустить одно виртуальное устройство.
...
Этот сценарий выводит команды, которые необходимо выполнить, чтобы запустить приложение **iotdm\_simple\_sample** для каждого устройства, указанного в файле **devicecreds.txt**. Запустите команды отдельно в отдельных окнах терминала для каждого виртуального устройства. Виртуальное устройство будет работать, пока командное окно не будет закрыто.
...
8. Когда в командной строке появится сообщение "Службы запущены", откройте веб-браузер и перейдите к приложению управления устройствами по URL-адресу <http://127.0.0.1:3003>, чтобы просмотреть виртуальные устройства.
