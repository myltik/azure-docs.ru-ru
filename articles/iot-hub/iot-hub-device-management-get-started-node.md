---
title: Приступая к работе с функцией управления устройствами в центре IoT | Microsoft Docs
...
---
# Приступая к работе с функцией управления устройствами центра IoT Azure с использованием Node.js (предварительная версия)
...
> [!NOTE]
> Чтобы обеспечить возможность управления устройствами, необходимо создать центр IoT, даже если он у вас уже есть, так как ваш имеющийся центр не обладает этой возможностью. После того, как функция управления устройствами станет общедоступной, все существующие центры IoT будут обновлены для поддержки возможностей управления устройствами.
> 
> 

## Предварительные требования
В этом руководстве предполагается, что вы используете компьютер разработки под управлением Ubuntu Linux.

Чтобы выполнить приведенные здесь шаги, необходимо установить следующее программное обеспечение:

* Git
* gcc (версии 4.9 или более поздней). Чтобы узнать версию, установленную в вашей среде, выполните команду `gcc --version`. Дополнительные сведения о том, как обновить версию gcc на Ubuntu 14.04, см. на странице <http://askubuntu.com/questions/466651/how-do-i-use-the-latest-gcc-4-9-on-ubuntu-14-04>.
* [cmake](https://cmake.org/download/) (версии 2.8 или более поздней). Чтобы узнать версию, установленную в вашей среде, выполните команду `cmake --version`.
...
3. В колонке **Центр IoT** выберите конфигурацию центра IoT.
   ...
   * Установите флажок **Enable Device Management** (Включить управление устройствами). Если не установить флажок **Enable Device Management** (Включить управление устройствами), примеры не будут работать. Устанавливая флажок **Enable Device Management** (Включить управление устройствами), вы создаете экземпляр ознакомительной версии центра IoT, поддерживаемый только в восточной части США, Северной Европе и Восточной Азии и не предназначенный для рабочих сценариев. Вы не можете переносить устройства в центры, в которых управление устройствами включено (или переносить устройства из таких центров).
...
4. Выбрав параметры конфигурации центра IoT, нажмите кнопку **Создать**. Создание центра IoT в Azure может занять несколько минут. Чтобы проверить состояние, следите за ходом выполнения на **начальной панели** или **панели уведомлений**.
   ...
   .![][img-monitor]
5. После создания центра IoT откройте соответствующую колонку, запишите **имя узла**, а затем щелкните **Политики общего доступа**.
   
   .![][img-keys]
6. Выберите политику **iothubowner**, а затем скопируйте и запишите строку подключения в колонке **iothubowner**. Скопируйте политику в расположение, где ее можно будет просмотреть позже, так как она понадобится, чтобы завершить оставшуюся часть этого руководства.
   
   > [!NOTE]
   > Не используйте учетные данные **iothubowner** в рабочих сценариях.
   > 
   > 
...
В этом разделе выполняется сценарий, который создает виртуальное устройство и примеры, а также подготавливает набор удостоверений новых устройств к работе в реестре устройств центра IoT. Устройство может подключиться к центру IoT, только если в реестре устройств есть соответствующая запись.
...
2. Клонируйте репозиторий GitHub. **Выполните эту операцию в каталоге, в имени которого нет пробелов.**
     ...
     ```
3. Чтобы установить необходимые пакеты и зависимые библиотеки, в корневой папке, куда клонирован репозиторий **azure-iot-sdks**, перейдите в каталог **azure-iot-sdks/c/build\_all/linux** и выполните следующую команду:
   
     ```
     ./setup.sh
     ```
4. В корневой папке, куда клонирован репозиторий **azure-iot-sdks**, перейдите в каталог **azure-iot-sdks/node/service/samples** и выполните следующую команду, заменив значение заполнителя своей строкой подключения из предыдущего раздела.
...
1. Запускает **cmake**, чтобы создать файлы, необходимые для создания виртуального устройства. Исполняемый файл находится в папке **azure-iot-sdks/node/service/samples/cmake/iotdm\_client/samples/iotdm\_simple\_sample**. Обратите внимание, что исходные файлы находятся в папке **azure-iot-sdks\\c\\iotdm\_client\\samples\\iotdm\_simple\_sample**.
2. Создает исполняемый файл виртуального устройства **iotdm\_simple\_sample**.
3. Выполняет `npm install`, чтобы установить необходимые пакеты.
4. Выполняет файл `node generate_devices.js`, чтобы подготовить удостоверения устройств к работе в центре IoT. Устройства описаны в файле **sampledevices.json**. После подготовки устройств к работе учетные данные сохраняются в файле **devicecreds.txt** (в каталоге **azure-iot-sdks/node/service/samples**).

## Запуск виртуальных устройств
После добавления устройств в реестр устройств можно запустить виртуальные управляемые устройства. Для каждого удостоверения устройства, подготовленного к работе в центре IoT Azure, необходимо запустить одно виртуальное устройство.

Используя оболочку, перейдите в каталог **azure-iot-sdks/node/service/samples** и выполните следующее:

  ```
  ./simulate.sh
  ```

Этот скрипт выводит команды, которые необходимо выполнить, чтобы запустить приложение **iotdm\_simple\_sample** для каждого устройства, указанного в файле **devicecreds.txt**. Запустите команды в отдельных окнах терминала для каждого виртуального устройства. Виртуальное устройство работает, пока командное окно не закрыто.

Приложение **iotdm\_simple\_sample** создается с использованием клиентской библиотеки управления устройствами центра IoT Azure для C, позволяющей создавать устройства IoT, которыми можно управлять в центре IoT Azure. С помощью этой библиотеки изготовители устройств могут предоставлять сведения о свойствах устройства и реализовывать выполнимые действия, необходимые для заданий устройства. Эта библиотека поставляется в составе пакетов SDK с открытым кодом для центра IoT Azure.

При выполнении **simulate.sh** в окне вывода появляется поток данных. Здесь отображается входящий и исходящий трафик, а также инструкции **printf** в функциях обратного вызова приложения. Таким образом вы можете следить за входящим и исходящим трафиком, а также за тем, как пример приложения обрабатывает декодированные пакеты. При подключении устройства к центру IoT служба автоматически начинает отслеживать ресурсы на устройстве. Затем клиентская библиотека DM центра IoT осуществляет обратные вызовы устройства, чтобы получить из него последние значения.
...
> [!NOTE]
> Чтобы получить менее подробный вывод, создайте и запустите розничную конфигурацию.
> 
> 
...
3. Клонируйте репозиторий GitHub пользовательского интерфейса управления устройствами центра IoT Azure, выполнив следующую команду в оболочке:
...
4. Чтобы получить зависимые пакеты, в корневой папке, куда клонирована копия репозитория пользовательского интерфейса управления устройствами центра IoT Azure, выполните следующую команду:
    ...
    npm install
    ```
5. После завершения команды npm install выполните в оболочке следующую команду, чтобы создать код.
...
7. Чтобы запустить пользовательское приложение управления устройствами, в оболочке выполните следующую команду:
   
    ```
    npm run start
    ```
8. Когда в командной строке появится сообщение Services have started (Службы запущены), откройте браузер и перейдите к приложению управления устройствами по URL-адресу <http://127.0.0.1:3003>, чтобы просмотреть виртуальные устройства.
