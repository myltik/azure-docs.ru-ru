---
title: Обработка сообщений, отправляемых с устройства центра IoT в облако (.Net) | Microsoft Docs
...
---
# <a name="tutorial-how-to-process-iot-hub-devicetocloud-messages-using-net"></a>Учебник: как обрабатывать сообщения, отправляемые с устройства центра IoT в облако, с помощью .Net
...
8. В классе **Program** добавьте в начало следующие инструкции **using**:
    ...
    using Microsoft.ServiceBus.Messaging;
    ```
9. Измените метод **Main** в классе **Program**, как показано ниже. Замените **{строка подключения Центра Интернета вещей}** строкой подключения **iothubowner** из руководства [Начало работы с Центром Интернета вещей]. Замените строку подключения к хранилищу строкой подключения, которую вы записали в начале данного раздела. Замените строку подключения служебной шины разрешениями **Отправка** для очереди **d2ctutorial**, записанными в начале этого раздела:
...
В этом разделе вам предстоит создать консольное приложение для Windows, которое принимает интерактивные сообщения из очереди служебной шины. Дополнительные сведения о том, как разработать решение с использованием служебной шины, см. в статье [Многоуровневое приложение .NET, использующее очереди служебной шины Azure][Многоуровневое приложение .NET, использующее очереди служебной шины Azure].
...
[Многоуровневое приложение .NET, использующее очереди служебной шины Azure]: ../service-bus-messaging/service-bus-dotnet-multi-tier-app-using-service-bus-queues.md
