...
# Учебник: как обрабатывать сообщения, отправляемые с устройства центра IoT в облако, с помощью .Net
...
3. Найдите **WindowsAzure.ServiceBus**, щелкните **Установить** и примите условия использования. После этого будут выполнены скачивание, установка и добавление ссылки на [пакет NuGet служебной шины Azure](https://www.nuget.org/packages/WindowsAzure.ServiceBus) со всеми зависимостями.

4. Найдите **Концентратор событий служебной шины Microsoft Azure — EventProcessorHost**, а затем щелкните **Установить** и примите условия использования. Будет скачана, установлена и добавлена ссылка на [пакет NuGet концентратора событий служебной шины Azure — EventProcessorHost](https://www.nuget.org/packages/Microsoft.Azure.ServiceBus.EventProcessorHost) со всеми его зависимостями.
...
7. Замените следующий код текстом класса:
    ...
    Метод **ProcessEventsAsync** получает пакет сообщений из центра IoT и обрабатывает их следующим образом: интерактивные сообщения отправляются в очередь служебной шины, а сообщения точки данных добавляются в буфер памяти с именем **toAppend**. Если буфер памяти достигает предельного размера в 4 МБ или заканчивается период дедупликации (в данном учебнике — через один час после создания контрольной точки), то приложение активирует создание контрольной точки.
...
9. Измените метод **Main** в классе **Program**, как показано ниже. Замените **{iot hub connection string}** строкой подключения **iothubowner** из учебника [Приступая к работе с центром Azure IoT с использованием .NET]. Замените строку подключения к хранилищу строкой подключения, которую вы записали в начале данного раздела. Замените строку подключения служебной шины разрешениями **Отправка** для очереди **d2ctutorial**, записанными в начале этого раздела.
