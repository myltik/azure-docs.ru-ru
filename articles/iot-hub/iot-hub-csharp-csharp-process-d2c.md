...
description: "Обработка сообщений Центра Интернета вещей, отправляемых с устройства в облако, с использованием маршрутов для перенаправления сообщений в другие серверные службы."
services: iot-hub
...
В этой статье используется код, представленный в руководстве [Приступая к работе с Центром Интернета вещей Azure (Java)]. Из статьи вы узнаете, как можно использовать маршрутизацию для перенаправления сообщений от устройства в облако. Это простой способ, реализуемый через настройки системы. Руководство демонстрирует, как выделять для дополнительной обработки сообщения, которые требуют немедленных действий от серверной части решения. Например, устройство может отправить аварийный сигнал, который активирует вставку билета в системе CRM. При этом обычные сообщения от точек данных просто поступают в модуль аналитики. Например, телеметрическое сообщение о температуре с устройства, которое должно быть сохранено для последующего анализа, является сообщением точки данных.
...
В этом руководстве вы узнали, как надежно перенаправлять сообщения, отправляемые от устройства в облако, с использованием функции маршрутизации сообщений Центра Интернета вещей.
