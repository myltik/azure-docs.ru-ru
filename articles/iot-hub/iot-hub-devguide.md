...
Реестр удостоверений устройств в центре IoT не содержит метаданные приложений, к нему можно получить доступ как к словарю (используя в качестве ключа свойство **deviceId**), и он не поддерживает выразительные запросы.
...
Задание записывает выходные данные в указанный контейнер больших двоичных объектов в файл с именем **job\_{job\_id}\_devices.txt**. Этот файл содержит удостоверения устройств, сериализованные в формате JSON (как указано в разделе [Свойства удостоверений устройств](#deviceproperties)). Для материалов безопасности устанавливается значение **null**, если для параметра **includeKeysInExport** задано значение **false**.
...
**Пример.** В типичном решении IoT компонент управления устройствами использует политику *registryReadWrite*, компонент обработчика событий — политику *service*, компонент бизнес-логики устройства среды выполнения — также политику *service*, а отдельные устройства подключаются с помощью учетных данных, хранящихся в реестре удостоверений центра IoT.
...
Каждый поддерживаемый протокол, например AMQP и HTTP, передает маркеры разными способами.

Протокол HTTP реализует проверку подлинности посредством включения действительного маркера в заголовок запроса **Authorization**. Маркер может передаваться и с помощью параметра запроса **Авторизация**.
...
Чтобы определить область действия для политик безопасности на уровне концентратора, создайте маркеры с помощью универсального кода (URI) ограниченного ресурса. Например, конечная точка для отправки сообщений с устройства в облако — **/devices/{deviceId}/events**. Кроме того, можно использовать политику общего доступа на уровне концентратора с разрешениями **DeviceConnect**. С ее помощью можно подписать маркер, значением resourceURI которого является **/devices/{deviceId}**. В результате будет создан маркер, который можно использовать только для отправки устройств от имени устройства с кодом **deviceId**.

Этот механизм похож на [политику издателя концентраторов событий][lnk-event-hubs-publisher-policy]. Он позволяет реализовывать методы настраиваемой проверки подлинности (как объясняется в разделе о безопасности статьи [Разработка решения][lnk-guidance-security]).

## Обмен сообщениями

Центр IoT предоставляет примитивы обмена сообщениями, с помощью которых обеспечивается обмен данными [из облака на устройство](#c2d) (из серверной части приложения — *служба* или *облако*) и [из устройства в облако](#d2c) (из устройства в серверную часть приложения).
...
| Кому | Используется в сообщениях, отправляемых [из облака в устройство](#c2d), чтобы указать поле назначения.|
...
На высоком уровне следует использовать протокол AMQP всегда, когда это возможно, и использовать только протокол HTTP/1, если ресурсы устройства или конфигурация сети не поддерживают протокол AMQP. Кроме того, при использовании HTTP/1 нужно задать такую частоту опроса, чтобы он выполнялся реже одного раза в 25 минут для каждого устройства. Во время разработки допускается большая частота опроса.

И наконец, следует рассмотреть [шлюз протокола IoT Azure][lnk-azure-protocol-gateway], который позволяет развернуть высокопроизводительный шлюз MQTT, напрямую взаимодействующий с центром IoT. Протокол MQTT поддерживает отправку сообщений по инициативе сервера (что позволяет мгновенно доставлять сообщения, отправляемые из облака в устройство). Кроме того, этот шлюз доступен для устройств с очень малым объемом ресурсов. Основной недостаток этого подхода — необходимость самостоятельно размещать шлюз протокола и управлять им.

### Отправка сообщений с устройства в облако<a id="d2c"></a>

Как описано в разделе [Конечные точки](#endpoints), сообщения, отправляемые с устройства в облако, отправляются через конечную точку, обращенную к устройству (**/devices/{deviceId}/messages/events**). Устройства получают их через конечную точку, обращенную к службе (**/messages/events**), то есть совместимую с [концентраторами событий][lnk-event-hubs]. Таким образом, чтобы получать сообщения, отправляемые из устройства в облако, можно использовать стандартную интеграцию концентраторов событий и пакетов SDK.
...
> [AZURE.NOTE] При отправке сообщений с устройства в облако по HTTP указанная ниже строка может содержать только символы ASCII — значения системных свойств, а также имена и значения свойств приложений.

#### Трафик, передающийся без использования телеметрии

Часто устройства отправляют не только точки данных телеметрии, но и *интерактивные* сообщения и запросы, выполнение и обработку которых осуществляет та часть приложения, которая отвечает за бизнес-логику. Например, устройство отправляет критические оповещения, которые должны активировать определенное действие в серверной части, или отвечает на команды, отправленные из серверной части.

Дополнительные сведения об оптимальных методах обработки таких сообщений см. в разделе [Обработка сообщений, отправляемых с устройства в облако][lnk-guidance-d2c-processing].
...
Как подробно описано в разделе [Конечные точки](#endpoints), сообщения из облака можно отправлять в устройство через конечную точку, обращенную к службе (**/messages/devicebound**), а устройства могут получить их через конечную точку конкретного устройства (**/devices/{deviceId}/messages/devicebound**).

Каждое сообщение, отправляемое с облака в устройство, адресовано отдельному устройству, то есть для свойства **to** задается значение **/devices/{deviceId}/messages/devicebound**.
...
> [AZURE.NOTE] При отправке сообщений из облака на устройство указанная ниже строка может содержать только символы ASCII — значения системных свойств, а также имена и значения свойств приложений.
...
Когда служба отправляет сообщение, считается, что оно *поставлено в очередь*. Когда устройству нужно *получить* сообщение, центр IoT *блокирует* сообщение (устанавливает состояние **Невидимое**), чтобы другие потоки на том же устройстве начали получать другие сообщения. Когда поток устройства завершает обработку устройства, он уведомляет центр IoT, *завершая* сообщение.

Устройство также может *отклонить* сообщение, в результате чего центр IoT задает состояние **Не доставлено**, а также *отменить* отправление сообщения, в результате чего центр IoT размещает сообщение обратно в очередь с состоянием **Поставлено в очередь**.

При обработке сообщения потоком может произойти ошибка без уведомления центра IoT. В этом случае состояние сообщения автоматически меняется с **Невидимое** обратно на **Поставлено в очередь** после *истечения срока видимости (или блокировки)* (значение по умолчанию — одна минута). Состояние сообщения может изменяться с **Поставлено в очередь** на **Невидимое** и наоборот столько раз, сколько указано в свойстве *Максимальное количество доставок* центра IoT. После выполнения этого количества изменений центр IoT устанавливает для сообщения состояние **Не доставлено**. Таким же образом центр IoT устанавливает для сообщения состояние **Не доставлено** после истечения срока действия сообщения (см. раздел [Срок жизни](#ttl)).
...
Каждый центр IoT подготавливается с определенным количеством единиц в определенной единице хранения (SKU). Дополнительные сведения см. на странице [Центр IoT —цены][lnk-pricing]. SKU и количество единиц определяют максимальную дневную квоту сообщений, которые вы можете отправить, и максимальное количество удостоверений устройств в реестре удостоверений. Количество удостоверений в реестре ограничивает количество одновременно подключенных устройств.

Кроме того, значение SKU определяет лимиты регулирования, которые центр IoT применяет по отношению к операциям.
...
| Передачи с устройства в облако | 120/с/единица (для S2), 12/с/единица (для S1). Минимум 100/с |
| Операции из облака в устройство (отправки, приемы, отзывы) | 100/мин/единица |
