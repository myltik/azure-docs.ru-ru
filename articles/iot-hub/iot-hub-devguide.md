...
> [AZURE.NOTE] При использовании HTTP для отправки сообщений с устройства в облако имена и значения свойств могут содержать только алфавитно-цифровые символы ASCII и символы ``{'!', '#', '$', '%, '&', "'", '*', '*', '+', '-', '.', '^', '_', '`', '|', '~'}``.
...
Дополнительные сведения об оптимальных методах обработки таких сообщений см. в статье [Обработка сообщений, отправляемых с устройства в облако][lnk-guidance-d2c-processing].
...
Как подробно описано в статье [Конечные точки](#endpoints), сообщения из облака на устройство можно отправлять через конечную точку, обращенную к службе (**/messages/devicebound**), а устройства могут получить их через конечную точку конкретного устройства (**/devices/{идентификатор\_устройства}/messages/devicebound**).
...
> [AZURE.NOTE] При отправке сообщений из облака на устройство имена и значения свойств могут содержать только алфавитно-цифровые символы ASCII и символы ``{'!', '#', '$', '%, '&', "'", '*', '*', '+', '-', '.', '^', '_', '`', '|', '~'}``.
