---
title: "Преобразование данных в шлюзе Интернета вещей с помощью пакета SDK для шлюза Azure IoT | Документация Майкрософт"
description: "Шлюз Интернета вещей можно использовать для преобразования формата данных датчиков с помощью настраиваемого модуля из пакета SDK для шлюза Azure IoT."
services: iot-hub
documentationcenter: 
author: shizn
manager: timtl
...
---
# <a name="use-iot-gateway-for-sensor-data-transformation-with-azure-iot-gateway-sdk"></a>Использование шлюза IoT для преобразования данных датчиков с помощью пакета SDK для шлюза Azure IoT
...
Одна из задач шлюза Интернета вещей — обработка собранных данных перед их отправкой в облако. Пакет SDK для шлюза Azure IoT позволяет создавать и собирать модули для формирования рабочего процесса обработки данных. Модуль получает сообщение, выполняет с ним определенное действие, а затем передает его для обработки другими модулями.
...
* Скомпилируем модуль.
* Добавим модуль в пример приложения BLE из пакета SDK для шлюза Azure IoT.
...
1. Создайте клоны исходных файлов модуля преобразования из GitHub в домашнем каталоге шлюза Интернета вещей, выполнив следующие команды:
   ...
   Это собственный модуль пакета SDK для шлюза Azure, написанный на языке программирования C. Модуль преобразует формат полученных сообщений в следующий:
