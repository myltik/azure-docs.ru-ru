...
В рамках этого руководства мы подключим плату Adafruit Feather HUZZAH ESP8266 к созданному Центру Интернета вещей Azure. Затем запустим пример приложения на ESP8266, чтобы собрать данные о температуре и влажности, зафиксированные датчиком DHT22. И наконец, отправим данные датчика в Центр Интернета вещей.
...
Теперь работающий датчик должен быть подключен к Adafruit Feather HUZZAH ESP8266.
...
При использовании Ubuntu убедитесь, что обычный пользователь имеет разрешения для работы с USB-портом платы Feather HUZZAH ESP826. Чтобы добавить разрешения для работы с последовательным портом, сделайте следующее:
