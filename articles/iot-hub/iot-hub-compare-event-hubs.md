...
| Масштаб |Оптимизирован для поддержки миллионов одновременно подключенных устройств. |Измеряет подключения согласно [квоте служебной шины][Azure Service Bus quotas]. С другой стороны, концентраторы событий позволяют пользователям указывать раздел для каждого отправляемого сообщения. |
