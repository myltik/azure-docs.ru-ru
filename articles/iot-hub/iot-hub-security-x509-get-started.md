...
ИЛИ
- Создайте собственные сертификаты X.509 с помощью стороннего инструмента, например [OpenSSL](https://www.openssl.org/). Это способ хорошо подходит для разработки и тестирования. Сведения о примере сценария PowerShell для создания сертификатов с помощью OpenSSL см. в разделах [Создание сертификатов X.509](iot-hub-security-x509-create-certificates.md#createcerts) и [Создание цепочки сертификатов X.509](iot-hub-security-x509-create-certificates.md#createcertchain). В остальной части этого руководства будет использоваться среда OpenSSL, настроенная в этом *практическом руководстве*, чтобы получить подробные сведения о комплексной защите в Центре Интернета вещей Azure с использованием сертификата X.509.
