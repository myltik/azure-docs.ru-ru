...
Чтобы запустить пример, необходимо выполнить `registry_queryDevices.js`. При этом выполняются сразу несколько разных запросов. Чтобы запустить пример, выполните следующие действия.
...
2.  В корневой папке, куда был клонирован репозиторий **azure-iot-sdks**, перейдите к папке **azure-iot-sdks/node/service/samples**.

3.  Откройте файл **registry\_queryDevices.js** и замените заполнитель строкой подключения центра IoT.

4.  Запустите `node registry_queryDevices.js`.
...
Составленное выражение запроса JSON можно использовать для запроса "двойников" устройств. Вызовите **queryDevices** и проверьте объект **result** в случае статистических запросов и объект **devices** в случае всех прочих запросов.
