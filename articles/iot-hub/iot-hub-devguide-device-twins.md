...
*Двойники устройств* — это документы JSON, хранящие сведения о состоянии устройства, в том числе метаданные, конфигурации и условия. Центр Интернета вещей Azure сохраняет двойник устройства для каждого устройства, подключаемого к Центру Интернета вещей. Содержание статьи
...
* **Свойства идентификации устройства**. Корень документа JSON двойных устройство содержит свойства только для чтения, из соответствующего удостоверение устройства, хранящиеся в [реестра удостоверений][lnk-identity].
...
В корневой объект устройства Свойства identity, а контейнер объектов для `tags` и `reported` и `desired` свойства. Контейнер `properties` содержит некоторые элементы только для чтения (`$metadata`, `$etag` и `$version`), описанные в разделах [Метаданные двойника устройства][lnk-twin-metadata] и [Оптимистичный параллелизм][lnk-concurrency].
...
Центр IoT применяет ограничение на размер 8 КБ на каждом из соответствующих значений общее `tags`, `properties/desired`, и `properties/reported`, за исключением элементы, доступные только для чтения.
