<properties
 pageTitle="Руководство разработчика. Общие сведения о двойниках устройств | Microsoft Azure"
...
> [AZURE.NOTE] Фрагменты выше являются примерами возможного способа кодирования конфигурации устройства и его состояния, оптимизированные для удобства чтения. Центр Интернета вещей не использует специальную схему для требуемых и сообщаемых свойств в двойниках устройств.
...
> [AZURE.NOTE] На данный момент доступ к двойникам устройств можно получить только на устройствах, подключающихся к Центру Интернета вещей по протоколу MQTT.
...
> [AZURE.NOTE] Эта логика уже реализована в [пакетах SDK для устройств Azure IoT][lnk-sdks]. Это описание полезно, только если приложение устройства не может использовать пакеты SDK для устройств Azure IoT и должно программировать интерфейс MQTT напрямую.
