...
* Библиотека **IoTHubClient** зависит от других библиотек с открытым кодом:
   * [общей библиотеки Azure C](https://github.com/Azure/azure-c-shared-utility), которая предоставляет основные функции для решения базовых задач (например, манипуляции со строками и списками, ввод и вывод данных и т. д.), требуемых в различных связанных с Azure пакетах SDK для C;
...
Выполнив все действия, описанные на странице [Prepare your development environment](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md) (Подготовка среды разработки), вы будете готовы к компиляции примеров приложений.
...
Введите в первом поле **строку подключения центра IoT** и щелкните **Обновить**. Теперь средство настроено для обмена данными с центром IoT.
...
Когда устройство получает сообщение, которое соответствует сигнатуре, вызывается соответствующая функция. Таким образом, кроме включения стереотипного кода из **IoTHubMessage**, получение сообщений заключается в определении простой функции для каждого действия, заданного в модели.
