...
-   **serializer**. Папка **serializer** содержит реализацию библиотеки **serializer**. В частности, библиотека **serializer** добавляет возможности моделирования к функциям, которые предоставляет **IoTHubClient**. Если вы используете библиотеку **serializer**, начните с определения модели, которая описывает события, отправляемые в центр IoT, а также сообщения, получаемые из него. После определения модели пакет SDK предоставит поверхность API, которая упрощает работу с событиями и сообщениями и избавляет вас от необходимости выполнять задачи, связанные с сериализацией.
...
Во-первых, вам нужно загрузить копию пакета SDK из GitHub, а затем выполнить сборку исходного кода. Копию исходного кода следует извлечь из ветви **master** репозитория GitHub:

<https://github.com/Azure/azure-iot-sdks>

После загрузки копии исходного кода вам потребуется выполнить еще несколько шагов, прежде чем запускать свой код. В статье [Подготовка среды разработки](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md) можно найти сводную информацию по этим шагам. Большая часть работы, которую следует выполнить для среды Windows, заключается в [подготовке библиотек Qpid Proton](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md#preparing-qpid-proton-libraries-in-windows) и [проверке среды](https://github.com/Azure/azure-iot-sdks/blob/master/c/doc/devbox_setup.md#verify-your-environment). Но вы можете воспользоваться следующими советами, которые помогут выполнить процедуру, описанную в руководстве по подготовке.
...
- Добавьте каталог Python в системную переменную среды **PATH**. В ОС Windows это будет выглядеть следующим образом: ![](media/iot-hub-device-sdk-c-intro/16-PythonPath.PNG)
...
-   Прежде чем открыть **командную строку разработчика для VS2015**, установите *средства командной строки Git*. Чтобы установить эти средства, выполните следующие действия:
	...
	2. Убедитесь, что в установщике включена функция **Git для Windows**. Для обеспечения интеграции IDE также необходимо проверить наличие в установщике параметра **Расширение GitHub для Visual Studio**: ![](media/iot-hub-device-sdk-c-intro/10-GitTools.PNG)
	3. Завершите работу мастера установки, чтобы установить инструменты.
	4. Добавьте каталог **bin** средств Git в системную переменную среды **PATH**. В ОС Windows это будет выглядеть следующим образом: ![](media/iot-hub-device-sdk-c-intro/11-GitToolsPath.PNG)

При запуске сценария **build\_proton.cmd** из командной строки исходный код Proton будет извлечен из репозитория GitHub, после чего будет выполнена сборка библиотек Proton.
...
Чтобы создать устройство, нажмите кнопку **Создать**. Откроется диалоговое окно с заполненным набором ключей (первичными и вторичными). Все, что вам нужно сделать, — ввести **идентификатор устройства** и нажать кнопку **Создать**.
...
Выполнив описанные выше шаги, вы будете готовы к запуску кода. В обоих примерах в верхней части основного исходного файла имеется константа для ввода строки подключения. Например, вот соответствующая строка из приложения **iothub\_client\_sample\_amqp**.
...
Это решение содержит один проект: Однако следует отметить, что в этом решении установлено четыре пакета GitHub.

  ![](media/iot-hub-device-sdk-c-intro/17-iothub-client-sample-amqp-githubpackages.PNG)

Для работы с пакетом SDK всегда необходим пакет **Microsoft.Azure.IoTHub.Common**. Поскольку этот пример использует AMQP, также необходимо включить пакеты **Apache.QPID.Proton.AzureIoT** и **Microsoft.Azure.IoTHub.AmqpTransport** (для HTTP есть эквивалентные пакеты). Кроме того, поскольку в рассматриваемом примере используется библиотека **IoTHubClient**, нам также нужно включить в решение проект **Microsoft.Azure.IoTHub.IoTHubClient**.
...
Большинство из них мы видели в предыдущем примере, но **Microsoft.Azure.IoTHub.Serializer** — это новый пакет. Наличие нового пакета обязательно, если используется библиотека **serializer**.
...
Функция **serializer\_init** вызывается один раз. Она инициализирует базовую библиотеку. Затем вызывается функция **IoTHubClient\_CreateFromConnectionString**, с которой мы уже знакомы по примеру **IoTHubClient**. Вызов задает строку подключения устройства (на этом этапе вы также можете выбрать используемый протокол). Обратите внимание, что в этом примере для транспорта используется протокол AMQP, но также можно использовать и HTTP.

Наконец, вызывается функция **CREATE\_MODEL\_INSTANCE**. Обратите внимание, что **WeatherStation** — это пространство имен модели, а **ContosoAnemometer** — имя модели. Создав экземпляр модели, вы сможете использовать его для отправки событий и получения сообщений. Но сначала нужно понять, что такое модель.
...
Второй параметр — это указатель на контекст пользователя. Такой же указатель мы передавали в **IoTHubClient\_SendEventAsync**. В этом случае контекстом является простой счетчик. Но это может быть любой другой объект.
...
Когда устройство получает сообщение, которое соответствует сигнатуре, вызывается соответствующая функция. Следовательно, кроме включения стандартного кода из **IoTHubMessage**, получение сообщений заключается в определении простой функции для каждого действия, определенного в модели.
