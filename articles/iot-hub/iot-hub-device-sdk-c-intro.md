...
Вы передаете этой функции копию строки подключения устройства, полученную из обозревателя устройств. Также вы указываете протокол связи, который будет использоваться. В этом примере используется протокол MQTT, но вы также можете использовать протоколы AMQP и HTTP.
...
Очевидно, что с помощью библиотеки **IoTHubClient** вы можете легко отправлять и получать сообщения. Библиотека обеспечивает обмен данными с центром IoT, включая выбор используемого протокола (что с точки зрения разработчика является простым параметром конфигурации).
