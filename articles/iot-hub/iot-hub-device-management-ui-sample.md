---
title: Использование пользовательского интерфейса управления устройствами центра IoT | Microsoft Docs
...
Чтобы перейти к этому подробному представлению на своей первой имитации устройства (**Device11 7ce4a850**), выберите соответствующую строку устройства. Затем нажмите кнопку **Edit** (Изменить) (можно также дважды щелкнуть строку или ввести идентификатор устройства в поле поиска).

Теперь вы видите полное представление разделов "двойника" устройства, где можно обновить записываемые свойства и выполнить другие операции с устройством, как описано ниже.
...
Обновите этот экран через несколько секунд, чтобы увидеть, как состояние родительского и трех дочерних заданий изменилось на **завершено**. Новые значения состояний показывают, что операции перезагрузки были успешно выполнены и подтверждены имитациями устройств. Используйте столбец **Device ID** (Идентификатор устройства), чтобы определить, какие задания и устройства связаны.

> [!NOTE]
> Если дочерние задания вернут состояние "failed" (Сбой), проверьте, выполняются ли ваши имитации устройств в тестовой системе. Если это не так, еще раз выполните файл simulate.bat или сценарий simulate.sh и повторите процесс перезагрузки устройств с помощью задания, описанный в предыдущем разделе.
> 
> 
