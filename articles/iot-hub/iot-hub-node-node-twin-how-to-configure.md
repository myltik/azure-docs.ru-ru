<properties
    pageTitle="Использование свойств двойников | Microsoft Azure"
...
4. Добавьте следующий код в файл **SimulateDeviceConfiguration.js** и замените заполнитель **{строка подключения устройства}** на строку подключения, скопированную при создании удостоверения устройства **myDeviceId**:
    ...
    > [AZURE.IMPORTANT] События изменения требуемых свойств всегда создаются единожды при подключении устройства. Прежде чем что-либо делать, обязательно убедитесь, что требуемые свойства действительно изменились.
...
5. Добавьте следующий код перед осуществлением вызова `registry.getDeviceTwin()` для реализации функции **queryTwins**:
...
8. Запустив файл **SimulateDeviceConfiguration.js**, запустите приложение с помощью следующей команды:
    ...
    > [AZURE.IMPORTANT] Результаты запроса выводятся после операции сообщения устройства с задержкой (не более минуты). Таким образом обеспечивается работа инфраструктуры запросов в очень широком масштабе. Для получения согласованных представлений одного двойника используйте метод **getDeviceTwin** в классе **Registry**.
