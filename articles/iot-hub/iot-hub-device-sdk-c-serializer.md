...
Модели содержат определение событий, которые вы можете отправить в центр IoT (*данные*), и сообщений, которые вы можете получить из Центра Интернета вещей (*действия*). Как видно из примера, для событий указываются тип и имя; а для действий — имя и необязательные параметры (каждый с указанием типа).
...
Фактически мы присваиваем значение каждому элементу структуры **Test**, а затем вызываем **SendAsync** для отправки события **Test** в облако. **SendAsync** является вспомогательной и служит для отправки одного события, инициируемого изменением данных, в центр IoT:
...
Если вы клонировали репозиторий Azure-iot-sdks с сайта GitHub с помощью параметра --recursive, то эту общедоступную библиотека служебных программ можно найти здесь:
