...
Чтобы использовать новую версию macro\_utils.h, необходимо будет удалить пакет NuGet **serializer** из решения, а на его место поместить проект **serializer** Visual Studio. Это позволяет вашему коду выполнить компиляцию для исходного кода библиотеки serializer, содержащего обновленный macro\_utils.h. Предположим, что это нужно сделать для **simplesample\_amqp**. Следует начать с удаления пакета NuGet для библиотеки serializer из решения:
