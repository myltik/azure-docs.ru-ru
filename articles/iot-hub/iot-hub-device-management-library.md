...
4.	Разработка на базе проекта с открытым исходным кодом [Eclipse Wakaama][lnk-Wakaama]; это позволяет использовать накопленный опыт и поддерживать развитие сообщества.
...
Кроме этих понятий важно иметь представления о правилах передачи данных от устройства к службе. Для этой задачи в LWM2M определена модель наблюдения и уведомления. Когда физическое устройство подключается к центру IoT, служба запускает наблюдение за некоторыми свойствами этого устройства. Затем физическое устройство уведомляет центр IoT об изменениях в этих свойствах устройства.
...
> [AZURE.NOTE] В текущей версии системы не поддерживаются пользовательские свойства устройства, несколько экземпляров ресурса и несколько экземпляров объекта.
...
| ModelNumber | чтение | Строка | | Идентификатор модели (строка, определяемая производителем). |
| DeviceType | чтение | Строка | | Тип устройства (строка, определяемая производителем).<br/>Примечание: сопоставляется со свойством **SystemPropertyNames.DeviceDescription** серверного API. |
| SerialNumber | чтение | Строка | | Серийный номер устройства. |
| FirmwareVersion | чтение | Строка | | Текущая версия встроенного ПО на устройстве. |
...
| Часовой пояс | Чтение<br/>Запись | Строка | | Указывает, в каком часовом поясе находится устройство. |
...
| PackageURI | запись | Строка | 0–255 байт | URI-адрес, по которому устройство может получить пакет встроенного ПО.<br/>Сопоставляется с параметром **SystemPropertyNames.FirmwarePackageUri** API службы. |
...
| Значение | Чтение Запись | Строка | | Однозначно определяет значение конфигурации для чтения или обновления. |
