...
Сначала на устройстве Edison необходимо обновить версию программного обеспечения BlueZ. Обратите внимание, что даже если у вас уже установлена версия 5.37, необходимо выполнить следующие действия, чтобы убедиться в завершении установки.
...
2. Скачайте и извлеките [исходный код](http://www.kernel.org/pub/linux/bluetooth/bluez-5.37.tar.xz) для BlueZ версии 5.37.
...
3. Выполните сборку и установите BlueZ.
...
4. Измените конфигурацию службы *systemd* для Bluetooth, чтобы она указывала на новую управляющую программу Bluetooth, отредактировав файл **/lib/systemd/system/bluetooth.service**. Замените значение атрибута **ExecStart**, чтобы он выглядел следующим образом:
    
    ```
    ExecStart=/usr/local/libexec/bluetooth/bluetoothd -E
    ```

5. Перезагрузите устройство Edison.

Теперь необходимо убедиться, что плата Edison имеет возможность подключаться к устройству SensorTag.
...
2. Выполните команду **bluetoothctl**. Должен отобразиться примерно такой результат:
    
    ```
    [NEW] Controller 98:4F:EE:04:1F:DF edison [default]
    ```

3. Теперь вы находитесь в интерактивной оболочке Bluetooth. Введите команду **scan on**, чтобы выполнить поиск устройств Bluetooth. Должен отобразиться примерно такой результат:
...
4. Сделайте устройство SensorTag доступным для обнаружения, нажав маленькую кнопку (должен загореться зеленый светодиодный индикатор). Плата Edison должна обнаружить устройство SensorTag:
...
5. Отключите поиск, введя команду **scan off**.
...
6. Подключитесь к устройству SensorTag с помощью его MAC-адреса, введя команду **connect <MAC address>**. Обратите внимание, что приведенный ниже пример выходных данных является сокращенным:
...
7. Теперь можно отключиться от устройства с помощью команды **disconnect** и выйти из оболочки Bluetooth с помощью команды **quit**:
...
- Добавьте в центр IoT одно устройство с именем **SensorTag\_01** и запишите его идентификатор и ключ устройства. Для добавления устройства в центр IoT, созданный при выполнении предыдущего шага, и получения его ключей можно использовать [обозреватель устройств или инструмент iothub-explorer][lnk-explorer-tools]. Сопоставление этого устройства с устройством SensorTag будет выполнено при настройке шлюза.
...
- Вариант 1. Клонировать репозиторий [Пакет SDK для шлюза Microsoft Azure IoT][lnk-sdk] на устройстве Edison, а затем вручную клонировать репозиторий для каждого подмодуля.
- Вариант 2. Клонировать репозиторий [Пакет SDK для шлюза Microsoft Azure IoT][lnk-sdk] на классическом устройстве, на котором **git** поддерживает подмодули, а затем скопировать полный репозиторий с подмодулями на устройство Edison.

Если вы выбрали вариант 2, используйте следующие команды **git**, чтобы клонировать пакет SDK для шлюза и все его подмодули:
...
Весь локальный репозиторий следует запаковать в один файл архива, прежде чем копировать его на устройство Edison. Для копирования файла архива на устройство Edison можно воспользоваться служебной программой, например **pscp**, которая входит в состав **Putty** . Например:
...
Для начальной загрузки и запуска примера необходимо настроить каждый модуль, который участвует в шлюзе. Необходимая конфигурация предоставляется в файле JSON, и требуется настроить все пять участвующих модулей. В репозитории есть пример файла JSON с именем **gateway\_sample.json**, который можно использовать в качестве отправной точки для создания собственного файла конфигурации. Этот файл находится в папке **samples/ble\_gateway\_hl/src** в локальной копии репозитория пакета SDK для шлюза.

В следующих разделах описывается, как изменить этот файл конфигурации для примера BLE, и предполагается, что репозиторий пакета SDK для шлюза находится в папке **/home/root/azure-iot-gateway-sdk/** на устройстве Edison. Если репозиторий находится в другом месте, необходимо соответствующим образом изменить пути.


#### Конфигурация средства ведения журнала

Предполагая, что репозиторий шлюза находится в папке **/home/root/azure-iot-gateway-sdk/**, настройте модуль ведения журнала следующим образом:
...
Для запуска примера запустите двоичный файл **ble\_gateway\_hl**, передав путь к файлу конфигурации JSON. Если использовался файл **gateway\_sample.json**, то выполняемая команда выглядит следующим образом:
...
При запуске примера для мониторинга сообщений, которые шлюз пересылает с устройства SensorTag, можно использовать [обозреватель устройств или инструмент iothub-explorer][lnk-explorer-tools].
