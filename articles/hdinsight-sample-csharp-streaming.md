...
В приведенном примере процедура map и процедура reduce являются исполняемыми файлами, которые считывают входные данные из [stdin][stdin] (построчно) и выводят выходные данные в [stdout][stdin]. Программа подсчитывает все слова в тексте.
...
2.  [Код C\# для потоковой передачи Hadoop][Код C\# для потоковой передачи Hadoop]
3.  [Сводка][Сводка]
...
Код процедуры reduce в файле wc.cs использует объект [StreamReader][StreamReader] для чтения символов из стандартного входного потока, который является выходным потоком процедуры mapper в cat.exe. Поскольку для чтения символов используется метод [Console.Writeline][Console.Writeline], подсчет слов осуществляется путем подсчета символов пробела и конца строки в конце каждого слова, а затем суммарное значение записывается в стандартный выходной поток с помощью метода [Console.Writeline][Console.Writeline].
...
-   [Документация по пакету Azure HDInsight SDK][Документация по пакету Azure HDInsight SDK]
  ...
  [stdin]: http://msdn.microsoft.com/ru-ru/library/3x292kth(v=vs.110).aspx
