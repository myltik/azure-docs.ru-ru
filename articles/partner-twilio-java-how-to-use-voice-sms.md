<properties 
	pageTitle="Использование Twilio для поддержки голосовых вызовов и SMS (Java) - Azure" 
...
Twilio - это API веб службы телефонии, позволяющий использовать существующие языки веб-программирования и навыки для построения голосовых и SMS приложений. Twilio - это сторонняя служба (не компонент Azure и не продукт Майкрософт).
...
1. Получите JAR-файл Twilio и добавьте его к пути построения Java и к WAR-сборке. Можно загрузить исходные коды с GitHub по адресу [https://github.com/twilio/twilio-java][twilio_java] и собрать собственный файл JAR, либо загрузить уже готовый файл JAR (вместе с зависимостями или без них).
2. Убедитесь, что в хранилище ключей **cacerts** JDK содержится сертификат Equifax Secure Certificate Authority с отпечатком MD5 67:CB:9D:C0:13:24:8A:82:9B:B2:17:1E:D1:1B:EC:D4 (серийный номер 35:DE:F4:CF, отпечаток SHA1 D2:32:09:AD:23:D3:14:23:21:74:E4:0D:7F:9D:62:13:97:86:63:3A). Это сертификат центра сертификации (ЦС) для службы [https://api.twilio.com][twilio_api_service], которая вызывается при использовании интерфейсов API Twilio. Дополнительные сведения о том, как убедиться, что в хранилище ключей JDK **cacerts** содержится правильный сертификат ЦС, см. в разделе [Добавление сертификата в хранилище сертификатов ЦС Java][add_ca_cert].

Подробные инструкции по использованию клиентской библиотеки Twilio для Java доступны в [осуществление телефонных вызовов с использованием Twilio в Java-приложении Azure][howto_phonecall_java].
...
Далее показано, как осуществлять исходящий вызов с использованием класса **CallFactory**. Этот код также использует сайт из Twilio для выдачи ответа на языке разметки Twilio (TwiML). Замените значения **из** и **для** номера телефонов и проверьте **из** телефонный номер для учетной записи Twilio до выполнения кода.
...
Параметр **ApiVersion** доступен в голосовых запросах Twilio (но не в SMS-запросах). Доступные параметры запроса для Twilio voice и SMS-запросов см. в разделе <https://www.twilio.com/docs/api/twiml/twilio_request>и <https://www.twilio.com/docs/api/twiml/sms/twilio_request>соответственно. Переменная среды **RoleName** доступна как часть развертывания Azure. (Если необходимо добавить пользовательские переменные среды, чтобы их можно было взять из **System.getenv**, см. раздел по переменным среды в [Прочие параметры конфигурации ролей][misc_role_config_settings].)
...
Помимо примеров, перечисленных здесь Twilio предлагает веб интерфейсы API, можно использовать дополнительные функции Twilio из вашего приложения Azure. Подробные сведения см. в [документации по интерфейсу API Twilio] [twilio_api_documentation].
