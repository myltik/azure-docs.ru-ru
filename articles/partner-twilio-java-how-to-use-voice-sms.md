<properties linkid="develop-java-how-to-twilio-sms-service" urlDisplayName="Служба Twilio Voice/SMS" pageTitle="Использование Twilio для поддержки голосовых возможностей и SMS (Java) - Azure" metaKeywords="Twilio, Twilio API, телефонные вызовы, SMS-сообщение, ответы TwiML, Azure Twilio Java" description="Узнайте, как осуществить телефонный вызов и отправить SMS-сообщение с помощью службы Twilio API в Azure. Примеры кода, написанного на Java." metaCanonical="" services="" videoId="" scriptId="" documentationCenter="Java" title="Использования Twilio для поддержки голосовых возможностей и SMS в Java" authors="waltpo" solutions="" manager="bjsmith" editor="mollybos" />
...
1. Получите JAR-файл Twilio и добавьте его к пути построения Java и к WAR-сборке. По адресу [https://github.com/twilio/twilio-java][twilio_java] можно загрузить источники GitHub и создать свои собственные JAR-файлы или загрузить готовый JAR-файл (с зависимостями или без зависимостей).
...
Помимо примеров, перечисленных здесь, Twilio предлагает веб-интерфейсы API, с помощью которых вы можете использовать дополнительные функции Twilio из вашего приложения Azure. Дополнительные сведения см. в [документации по интерфейсу API Twilio] [twilio_api_documentation].
