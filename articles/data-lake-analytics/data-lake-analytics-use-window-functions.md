...
В этом руководстве мы продемонстрируем вам несколько вариантов использования оконных функций на примере двух типовых наборов данных. Дополнительные сведения см. в [справке по U-SQL](http://go.microsoft.com/fwlink/p/?LinkId=691348).
...
- Изучите два руководства:
    ...
    - [Начало работы со средствами озера данных Azure для Visual Studio](data-lake-analytics-use-data-lake-tools.md);
    - [Начало работы с языком U-SQL для заданий в службе аналитики озера данных Azure](data-lake-analytics-u-sql-get-started.md).
- Создайте учетную запись в службе аналитики озера данных, как описано в статье [Начало работы со средствами озера данных Azure для Visual Studio](data-lake-analytics-use-data-lake-tools.md).
...
- Есть 5 строк, значение которых меньше или равно 500, поэтому CUME\_DIST равно 5/6=0,83.
- Есть 4 строки, значение которых меньше или равно 500, поэтому CUME\_DIST равно 4/6=0,67.
- Есть 3 строки, значение которых меньше или равно 500, поэтому CUME\_DIST равно 3/6=0,50. Существуют две строки с одинаковым значением задержки.
- Есть 1 строка, значение которой меньше или равно 500, поэтому CUME\_DIST равно 1/6=0,17. 
...
WITHIN GROUP (ORDER BY <identifier> [ ASC | DESC ]). Указывает список числовых значений для сортировки и вычисления процентиля. Допускается только один идентификатор столбца. Выражение должно возвращать числовое значение. Другие типы данных не допускаются. По умолчанию значения сортируются по возрастанию.

OVER ([ PARTITION BY <identifier,>...[n] ] ). Делит входной набор строк на секции (на основе ключей секций), к которым применяется функция вычисления процентиля. Дополнительные сведения см. в этой статье в разделе «Ранжирующие функции». Примечание. Все значения NULL в наборе данных игнорируются.
...
- [Обзор службы аналитики озера данных Microsoft Azure](data-lake-analytics-overview.md)
...
- [Управление службой аналитики озера данных Azure с помощью портала предварительной версии Azure](data-lake-analytics-use-portal.md)
- [Управление службой аналитики озера данных Azure с помощью Azure PowerShell](data-lake-analytics-use-powershell.md)
- [Мониторинг заданий и устранение связанных с ними неполадок в службе аналитики озера данных Azure с помощью портала предварительной версии Azure](data-lake-analytics-monitor-and-troubleshoot-jobs-tutorial.md)
