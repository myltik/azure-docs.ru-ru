...
* Журнал запросов. 
    ...
    Скопируйте и вставьте в проект U-SQL следующий скрипт, который создает набор строк QueryLog.
    ...
    Обычно эти данные хранятся в файле. Данные из файла с текстом, разделенного табуляциями, извлекаются с помощью следующего кода. 
...
* Сотрудники.
    ...
    Скопируйте и вставьте в проект U-SQL следующий скрипт, который создает набор строк Employees с данными о сотрудниках.
...
Добавьте следующую инструкцию, которая выводит результирующий набор строк в файл данных.
...
> [!NOTE]
> Инструкции по использованию запроса и проверке выходных данных см. в статье [Приступая к работе с языком U-SQL для Azure Data Lake Analytics](data-lake-analytics-u-sql-get-started.md).
> 
> 
...
> [!NOTE]
> Если вы раньше не работали с оконными функциями, запоминайте числа в выходных данных.  
> 
> 
...
Чтобы увидеть самую высокую зарплату по каждому отделу, измените MIN на MAX и повторно выполните запрос.
...
* Если количество строк в секции не делится на значение numgroups, будут созданы группы двух размеров с разницей в единицу. Сначала идут большие группы, а затем — маленькие. Порядок определяется предложением OVER. 
...
Функция CUME_DIST вычисляет относительное положение указанного значения в группе значений. Она вычисляет процент запросов, задержка которых меньше или равна задержке текущего запроса в той же вертикали. Если упорядочение выполняется по возрастанию, для каждой строки R функция CUME_DIST возвращает число строк, значения в которых не превышают значение в строке R, деленное на общее число строк в секции. CUME_DIST возвращает числа в диапазоне 0 < x < = 1.
...
В секции есть шесть строк с ключом секции Web (4-я строка и ниже):
...
WITHIN GROUP (ORDER BY <identifier> [ ASC | DESC ]). Указывает список числовых значений, по которым выполняется сортировка и вычисление процентиля. Допускается только один идентификатор столбца. Выражение должно возвращать числовое значение. Другие типы данных не допускаются. По умолчанию значения сортируются по возрастанию.

OVER ([ PARTITION BY <идентификатор>…[n] ] ). Делит входной набор строк на секции (на основе ключей секций), к которым применяется функция вычисления процентиля. Дополнительные сведения см. в этой статье в разделе «Ранжирующие функции».
