...
3. В разделе __Параметры диагностики__ установите для состояния значение __Вкл.__ и выберите параметры ведения журнала.
   ...
   * Укажите, хотите ли вы получить журналы аудита или журналы запросов, либо и те, и другие.  В журнале запросов регистрируются все запросы API, а в журнале аудита — все операции, активируемые подобным запросом.
...
> [!NOTE]
> Свойства **SubmitTime**, **StartTime**, **EndTime** и **Parallelism** содержат сведения об операции. У них есть значение, только если операция началась или уже завершена. Например, у свойства **SubmitTime** значение появляется только после того, как свойство **operationName** получает значение **JobSubmitted**.
