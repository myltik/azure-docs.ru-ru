...
* OUTPUT — это ключевое слово принимает набор строк и сериализует его. Средство Outputters.Csv() записывает файл с разделителями-запятыми в указанное расположение. Можно также разработать пользовательские средства вывода.
 ...
 >     adl://\<учетная_запись_хранения_ADL>.azuredatalakestore.net:443/Samples/Data/SearchLog.tsv
 >
 >Для доступа к файлам в связанных учетных записях хранения необходимо использовать абсолютный путь.  Для файлов, хранящихся в связанной учетной записи хранения Azure, используется следующий синтаксис: wasb://\<контейнер_больших_двоичных_объектов>@\<учетная_запись_хранения>.blob.core.windows.net/Samples/Data/SearchLog.tsv
