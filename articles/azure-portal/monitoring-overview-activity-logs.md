---
title: Общие сведения о журнале действий Azure | Microsoft Docs
...
Журнал действий отличается от [журналов диагностики](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md), которые включают все журналы, генерируемые ресурсом. Эти журналы содержат данные об операциях самих ресурсов, а не об операциях с ресурсами.
...
* [Создание оповещений электронной почты или веб-перехватчика, активируемых событием журнала действий.](../monitoring-and-diagnostics/insights-auditlog-to-webhook-email.md)
* [Сохранение журнала в **учетную запись хранения** для архивации или проверки вручную](../monitoring-and-diagnostics/monitoring-archive-activity-log.md). В **профилях журнала** вы также можете задать время хранения (в днях).
...
* [Потоковая передача журнала в **концентратор событий**](../monitoring-and-diagnostics/monitoring-stream-activity-logs-event-hubs.md) для обработки в сторонней службе или пользовательском аналитическом решении, например, PowerBI.
...
3. Откроется колонка, в которой вы можете выбрать следующие элементы: регионы, связанные с экспортируемыми событиями; учетную запись хранения для хранения событий (а также количество дней, в течение которых эти события будут храниться; если указать здесь 0 дней, журналы будут храниться неограниченно долго); пространство имен служебной шины, в котором будет создан концентратор событий для потоковой передачи этих событий.
