...
|---|---|
|\\Процессор(\_Всего)\\% загруженности процессора |Процент|
|\\Процессор(\_Всего)\\% времени в привилегированном режиме |Процент|
|Процессор(\_Всего)\\% работы в пользовательском режиме |Процент|
|\\Информация о процессоре(\_Всего)\\Частота процессора |Число|
|\\Система\\Процессы |Число|
|\\Процесс(\_Всего)\\Счетчик потоков |Число|
|\\Процесс(\_Всего)\\Счетчик дескрипторов |Число|
|\\Память\\% используемой выделенной памяти в байтах |Процент|
|\\Память\\Доступно байт |Байты|
|\\Память\\Байт выделенной памяти |Байты|
|\\Память\\Предел выделенной памяти |Байты|
|\\Память\\Байт в выгружаемом пуле |Байты|
|\\Память\\Байт в невыгружаемом страничном пуле |Байты|
|\\Физический диск(\_Всего)\\% активности диска |Процент|
|\\Физический диск(\_Всего)\\% активности диска при чтении |Процент|
|\\Физический диск(\_Всего)\\% активности диска при записи |Процент|
|\\Физический диск(\_Всего)\\Обращений к диску/с |Число/с|
|\\Физический диск(\_Всего)\\Обращений чтения с диска/с |Число/с|
|\\Физический диск(\_Всего)\\Обращений записи на диск/с |Число/с|
|\\Физический диск(\_Всего)\\Скорость обмена с диском (байт/с) |Байт/с|
|\\Физический диск(\_Всего)\\Скорость чтения с диска (байт/с) |Байт/с|
|\\Физический диск(\_Всего)\\Скорость записи на диск (байт/с) |Байт/с|
|\\Физический диск(\_Всего)\\Средняя длина очереди диска |Число|
|\\Физический диск(\_Всего)\\Средняя длина очереди чтения диска |Число|
|\\Физический диск(\_Всего)\\Средняя длина очереди записи на диск |Число|
|\\Логический диск(\_Всего)\\% свободного пространства |Процент|
|\\Логический диск(\_Всего)\\Свободно мегабайт |Число|
...
Эти значения можно настроить на портале Azure в колонке **Параметры**. Для наборов масштабирования виртуальных машин можно настроить параметр автоматического масштабирования в шаблоне ARM таким образом, чтобы для параметра *metricName* использовалось значение *ApproximateMessageCount* и в *metricResourceUri* передавался идентификатор очереди хранилища.
...
Для наборов масштабирования виртуальных машин в шаблоне ARM можно настроить параметр автоматического масштабирования. Для этого для параметра *metricName* нужно указать значение *ApproximateMessageCount* и передать идентификатор очереди хранилища как *metricResourceUri*.
