...
Политику управления доступом на основе IP-адресов можно задать на портале Azure или программно с помощью [Azure CLI](cli-samples.md), [Azure PowerShell](powershell-samples.md) или [REST API](/rest/api/documentdb/), обновив свойство **ipRangeFilter**. 

Чтобы настроить политику управления доступом на основе IP-адресов на портале Azure, перейдите на страницу учетной записи Azure Cosmos DB, в меню навигации щелкните **Брандмауэр**, а затем измените значение параметра **Включить управление доступом к IP-адресам** на **Вкл.** 
...
Включив политику управления доступом на основе IP-адресов, вы можете предоставить доступ к порталу Azure и другим службам Azure, а также добавить текущий IP-адрес. Дополнительные сведения об этих параметрах см. в разделах ниже.
...
Чтобы разрешить доступ к порталу Azure, **включите** параметр **Разрешить доступ к порталу Azure** (чтобы отобразить параметр **Разрешить доступ к порталу Azure** и изменить его значение, **включите** параметр **Включить управление доступом к IP-адресам**).
...
В Azure такие службы PaaS, как Azure Stream Analytics, Функции Azure и Служба приложений Azure, используются в сочетании с Azure Cosmos DB. Чтобы разрешить доступ к учетной записи базы данных Azure Cosmos DB из этих служб (их IP-адреса не общедоступные), программным способом добавьте IP-адрес 0.0.0.0 в список разрешенных IP-адресов, связанных с учетной записью базы данных Azure Cosmos DB. Кроме того, вы можете включить на портале Azure параметр **Разрешить доступ к службам Azure** (чтобы отобразить параметр **Разрешить доступ к службам Azure** и изменить его значение, **включите** параметр **Включить управление доступом к IP-адресам**). Так вы предоставите службам PaaS в Azure доступ к учетной записи Azure Cosmos DB. 
