...
## <a name="connections-from-azure-paas-service"></a>Подключения из службы PaaS Azure 
В Azure такие службы PaaS, как Azure Stream Analytics и Функции Azure, используются в сочетании с Azure Cosmos DB. Вы можете разрешить доступ к учетной записи базы данных Azure Cosmos DB из служб, IP-адрес которых не является общедоступным. Для этого добавьте IP-адрес 0.0.0.0 в список разрешенных IP-адресов, связанных с учетной записью базы данных Azure Cosmos DB, [настроив политику управления доступом на основе IP-адресов](#configure-ip-policy).  Так вы предоставите службам PaaS Azure доступ к учетной записи Azure Cosmos DB, в которой настроено это правило. 
