...
* **[MongoDB API](mongodb-introduction.md)**. Средство миграции данных экспортирует данные базы данных MongoDB в Azure Cosmos DB для использования с помощью API DocumentDB. Но если вы хотите продолжить использование API-интерфейсов MongoDB, имеет смысл использовать API MongoDB для Azure Cosmos DB. Кроме того, следует использовать mongoimport.exe или mongorestore.exe для импорта данных. Дополнительные сведения см. в статье [Azure Cosmos DB: импорт данных MongoDB](mongodb-migrate.md).
...
Данные, импортированные из хранилища таблиц Azure, можно выводить в таблицы и сущности Azure Cosmos DB для использования с API таблицы или в коллекции и документы для использования с API DocumentDB.  
...
## <a id="DocumentDBSource"></a>Импорт из коллекции DocumentDB API
...
## <a id="DocumentDBBulkTarget"></a>Импорт в API DocumentDB (массовый импорт)
...
Если вы разрешили средству миграции создавать коллекции API Azure Cosmos DB DocumentDB во время импорта, можно указать политику индексирования коллекций. В разделе дополнительных параметров массового импорта Azure Cosmos DB и параметров последовательной записи Azure Cosmos DB перейдите в раздел "Политика индексации".
...
## <a id="tableapibulkexport"></a>Экспорт в API таблицы (массовый импорт)

Средство экспорта API таблицы Azure Cosmos DB позволяет экспортировать данные из источника хранилища таблиц Azure в базу данных API таблицы Azure Cosmos DB. 

Строку подключения для учетной записи API таблицы Azure Cosmos DB для экспорта можно получить на портале Azure на странице строки подключения. Нажмите кнопку "Копировать" ![Снимок экрана со строкой подключения на портале Azure](./media/import-data/copy-button.png) в правой части экрана, чтобы скопировать всю строку.

![Снимок экрана со строкой подключения на портале Azure](./media/import-data/connection-string.png)

## <a id="tableapiseqtarget"></a>Экспорт в API таблицы (последовательный импорт записей)

Средство экспорта API таблицы Azure Cosmos DB позволяет экспортировать данные из источника хранилища таблиц Azure в базу данных API таблицы Azure Cosmos DB.

Строку подключения для учетной записи API таблицы Azure Cosmos DB для экспорта можно получить на портале Azure на странице строки подключения, как показано на рисунке в разделе [Экспорт в API таблицы (массовый импорт)](#tableapibulkexport).
