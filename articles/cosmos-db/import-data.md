...
В этом руководстве показано, как использовать средство миграции данных Azure Cosmos DB, с помощью которого можно импортировать данные из различных источников в коллекции и таблицы Azure Cosmos DB. Можно импортировать из JSON файлов CSV файлы, SQL, MongoDB, таблицы Azure хранилища, Amazon DynamoDB и даже коллекции базы данных SQL Azure Cosmos API и миграцию, что данные коллекции и таблицы для использования с Azure Cosmos DB. Также можно использовать средство переноса данных, при миграции из коллекции один раздел в коллекцию несколько секций для API-Интерфейсы SQL.

Какой программный интерфейс вы собираетесь использовать с помощью Azure Cosmos DB? 
* **[API-Интерфейсы SQL](documentdb-introduction.md)**  -параметры источника, в средство переноса данных можно использовать для импорта данных.
...
* **[MongoDB API](mongodb-introduction.md)**  -средство переноса данных сейчас не поддерживает базы данных MongoDB Azure Cosmos API как источник или цели. Если вы хотите перенести данные в или из MongoDB API коллекций в базе данных Azure Cosmos, ссылаются на [Azure Cosmos DB: как для переноса данных MongoDB интерфейсов API](mongodb-migrate.md) инструкции. По-прежнему можно использовать средство переноса данных для экспорта данных из MongoDB коллекциям базы данных SQL Azure Cosmos API для работы с API-Интерфейсы SQL. 
...
## <a id="SQLSource"></a>Импорт из коллекции SQL API
...
## <a id="SQLBulkTarget"></a>Импорт в API SQL (массового импорта)
...
## <a id="SQLSeqTarget"></a>Импорт в API SQL (Импорт последовательной записи)
...
Если средство миграции для создания базы данных SQL Azure Cosmos API коллекций во время импорта, можно указать политике индексирования коллекций. В разделе дополнительных параметров массового импорта Azure Cosmos DB и параметров последовательной записи Azure Cosmos DB перейдите в раздел "Политика индексации".
