...
* **Разные модели данных и популярные API-интерфейсы для доступа к данным и создания запросов к ним**.
    ...
    * API-интерфейсы поддерживаются пакетами SDK, доступными на нескольких языках, для следующих моделей данных:
        * [DocumentDB API](documentdb-introduction.md) — ядро СУБД с поддержкой бессхемного формата JSON и возможностью отправки SQL-запросов.
        * [MongoDB API](mongodb-introduction.md) — служба базы данных MongoDB, созданная на основе Cosmos DB. Совместима с существующими библиотеками, драйверами, средствами и приложениями MongoDB.
        * [API таблицы](table-introduction.md) — служба базы данных пар "ключ — значение", созданная для предоставления возможностей уровня "Премиум" приложениям хранилища таблиц Azure.
        * [API Graph (Gremlin)](graph-introduction.md) — служба базы данных графа, созданная по [спецификации Apache TinkerPop](http://tinkerpop.apache.org/).
        * [API Cassandra](cassandra-introduction.md) — хранилище пар "ключ — значение", основанное на реализации [Apache Cassandra](https://cassandra.apache.org/). 
...
* **Создание высокопроизводительных и критически важных приложений**.
    ...
    * Для обычного элемента размером в 1 КБ Cosmos DB обеспечивает сквозную задержку операций чтения менее 10 мс, а для индексированных операций записи — менее 15 мс на уровне 99-го процентиля в рамках одного региона Azure. Среднее значение задержки значительно ниже (менее 5 мс).

* **Гарантированная доступность**.
    * Доступность на уровне 99,99 % в соответствии с соглашением об уровне обслуживания для всех учетных записей в пределах одного и нескольких регионов с нестрогой согласованностью и доступность для чтения на уровне 99,999 % для всех учетных записей базы данных в пределах нескольких регионов.
...
* **Низкая стоимость владения**.
    * Это решение в пять-десять раз [экономичнее](https://aka.ms/cosmos-db-tco-paper), чем неуправляемое решение.
