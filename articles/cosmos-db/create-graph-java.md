...
3. В окне терминала git выполните команду `mvn exec:java -D exec.mainClass=GetStarted.Program`, чтобы открыть приложение Java.
    ...
    В окне терминала появятся вершины, добавляемые в граф. По завершении работы программы снова откройте в веб-браузере портал Azure. 
...
11. Нажмите кнопку **Применить фильтр** со значением фильтра `g.V()` по умолчанию, чтобы отобразить все значения графа. Все пользователи теперь отображаются в списке **Результаты**. 
