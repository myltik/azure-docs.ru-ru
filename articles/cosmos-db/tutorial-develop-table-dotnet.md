...
API Azure Cosmos DB поддерживает ряд возможностей, которые недоступны в хранилище таблиц Azure. Эти новые возможности можно включить, используя приведенные ниже значения конфигурации `appSettings`. В API таблиц не были добавлены какие-либо новые подписи или перегрузки, которых нет в пакете SDK для службы хранилища Azure. Это позволяет подключаться к таблицам в хранилище таблиц Azure и Azure Cosmos DB, а также работать с другими службами в службе хранилища Azure, такими как большие двоичные объекты и очереди. 
