...
В этом кратком руководстве показано, как создать учетную запись Azure Cosmos DB для API Graph (предварительная версия), базу данных и граф с использованием портала Azure. Затем вы можете создать и запустить консольное приложение, используя драйвер [Gremlin Node.js](https://aka.ms/gremlin-node) OSS.  
...
2. Скопируйте значение универсального кода ресурса (URI) на портале (с помощью кнопки копирования) и добавьте его в качестве значения параметра ключа config.endpoint в файле config.js.

    `config.endpoint = "GRAPHENDPOINT";`

3. Замените часть универсального кода ресурса (URI) documents.azure.com на graphs.azure.com.
...
2. Запустите `npm install gremlin`, чтобы установить необходимые модули npm.

3. Замените содержимое папки `node_modules\gremlin` исходным кодом из [разветвления Cosmos DB Gremlin](https://github.com/CosmosDB/gremlin-javascript), поддерживающего SSL и SASL. Эти протоколы требуются для Azure Cosmos DB, но сейчас драйвер не поддерживает их.

4. Запустите `node app.js` в окне терминала, чтобы запустить приложение Node.
