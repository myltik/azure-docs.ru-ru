...
## <a name="ttl-and-index-interaction"></a>Взаимодействие TTL и индекса
При добавлении или изменении значения TTL меняется и основной индекс. Если значение TTL не указано и вы задаете допустимое значение, это приведет к повторной индексации. Если индекс согласованный, пользователь не увидит изменений в состоянии индекса. Если индекс асинхронный, он прежде всего всегда находится в режиме наверстывания и с учетом этого изменения в TTL он создается заново. В результате этого запросы, выполненные при перестроении индекса, не вернут полных или правильных результатов. Не меняйте TTL для асинхронного индекса, если вам нужно точное количество данных и т. д., так как сам режим индексирования является асинхронным.  По возможности всегда выбирайте согласованный индекс. 
