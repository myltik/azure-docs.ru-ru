...
## <a name="ttl-and-index-interaction"></a>Срок ЖИЗНИ и индекс взаимодействия
Добавление или изменение параметра TTL на коллекцию изменения базового индекса. Если значение срока ЖИЗНИ была изменена с Off на On, коллекцию блоков повторное индексирование. При внесении изменений в политике индексирования при согласованный режим индексирования, пользователи не заметят изменения в индекс. Когда режим индексирования является устанавливается с отложенной индекс всегда Запаздывание и если изменяется значение срока ЖИЗНИ, индекс создается заново с самого начала. Если изменяется значение TTL, индекс режим отложенной запросы, выполненные во время перестроения индекса не возвращают полный или правильные результаты.

Если требуется точное возвращаемых данных, не изменяйте значение срока ЖИЗНИ при индексации режим отложенной. В идеале согласованное индекса следует всегда выбирать обеспечение согласованности результатов запроса. 
