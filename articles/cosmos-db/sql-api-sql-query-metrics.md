---
title: "Метрики SQL для базы данных SQL Azure Cosmos API | Документы Microsoft"
...
| `MaxDegreeOfParallelism` | Возвращает или задает число параллельных операций, которые выполняются на стороне клиента во время выполнения параллельных запросов в службу Azure Cosmos DB базы данных. Положительное значение свойства ограничивает число параллельных операций до заданного значения. Если оно имеет значение меньше 0, система автоматически определяет число параллельных операций для выполнения. |
...
Сведения об API-интерфейса REST заголовки запросов и параметры в разделе [запросы к ресурсам с помощью API-интерфейса REST](https://docs.microsoft.com/rest/api/documentdb/querying-documentdb-resources-using-the-rest-api).
...
Заметки о выпуске пакета SDK, и сведения о реализованные классы и методы [SDK SQL](sql-api-sdk-dotnet.md)
