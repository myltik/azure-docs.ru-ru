...
> [!IMPORTANT] 
> Для подключения Azure HDInsight к Azure Cosmos DB рекомендуем использовать соединитель между Spark и Azure Cosmos DB. Дополнительные сведения см. в статье [Ускорение аналитики больших данных в реальном времени с помощью соединителя Spark для Cosmos DB](spark-connector.md).
...
<table border='1'>
    ...
    <tr><th>Журнал изменений</th>
        <td>Пакет SDK для Java для Azure Cosmos DB обновлен до версии 1.6.0.</br>
...
5. Щелкните **Выберите тип кластера** и задайте для следующих свойств соответствующие значения.
    ...
    <table border='1'>
        ...
        <tr><td>Version (версия)</td><td>Последняя версия</td></tr>
    </table>
...
## <a name="RunHive"></a>Шаг 3. Выполнение задания Hive с помощью Azure Cosmos DB и HDInsight
> [!IMPORTANT]
...
2. <p>Начнем создавать строку запроса. Будет написан запрос Hive, который принимает все системные метки времени документа (_ts) и уникальные идентификаторы (_rid) из коллекции Azure Cosmos DB, подсчитывает все документы поминутно, а затем сохраняет результаты в новую коллекцию Azure Cosmos DB.</p>

    <p>Сначала создадим таблицу Hive из коллекции Azure Cosmos DB. Добавьте следующий фрагмент кода в область сценариев PowerShell <strong>после</strong> фрагмента кода с шага 1. Убедитесь, что включен дополнительный параметр запроса для обрезки документов до вида _ts и _rid.</p>
...
8. Проверьте результаты. Войдите на [портал Azure][azure-portal].
   ...
   4. Затем найдите свою <strong>учетную запись Azure Cosmos DB</strong>, <strong>базу данных Azure Cosmos DB</strong> и <strong>коллекцию Azure Cosmos DB</strong>, связанную с выходной коллекцией, указанной в запросе Hive.</br>
...
2. <p>Начнем создавать строку запроса. Будет написан запрос Pig, который принимает все системные метки времени документа (_ts) и уникальные идентификаторы (_rid) из коллекции Azure Cosmos DB, подсчитывает все документы поминутно, а затем сохраняет результаты в новую коллекцию Azure Cosmos DB.</p>
    <p>Сначала загрузите документы из Cosmos DB в HDInsight. Добавьте следующий фрагмент кода в область сценариев PowerShell <strong>после</strong> фрагмента кода с шага 1. Добавьте запрос в дополнительный параметр запроса DocumentDB для обрезки документов до вида _ts и _rid.</p>
...
10. Проверьте результаты. Войдите на [портал Azure][azure-portal].
    ...
    2. В правой верхней части панели обзора выберите <strong>Все ресурсы</strong>. </br>
    3. Найдите и щелкните <strong>Учетные записи Azure Cosmos DB</strong>. </br>
    4. Затем найдите свою <strong>учетную запись Azure Cosmos DB</strong>, <strong>базу данных Azure Cosmos DB</strong> и <strong>коллекцию Azure Cosmos DB</strong>, связанную с выходной коллекцией, указанной в запросе Pig.</br>
    5. И, наконец, щелкните <strong>Обозреватель документов</strong> в разделе <strong>Средства разработчика</strong>.</br></p>
...
2. Будет выполняться задание MapReduce, которое подсчитывает число вхождений для каждого свойства документа из коллекции Azure Cosmos DB. Добавьте этот фрагмент сценария **после** приведенного выше фрагмента.
...
6. Проверьте результаты. Войдите на [портал Azure][azure-portal].
   ...
   4. Затем найдите свою <strong>учетную запись Azure Cosmos DB</strong>, <strong>базу данных Azure Cosmos DB</strong> и <strong>коллекцию Azure Cosmos DB</strong>, связанную с выходной коллекцией, указанной в задании MapReduce.
...
* [Создание веб-приложения Java с использованием Azure Cosmos DB][sql-api-java-application]
