...
После настройки проекта необходимо добавить необходимые пакеты термосе проекта, включая pydocumentdb пакета Python для базы данных SQL Azure Cosmos API.
...
2. Добавьте следующий код config.py, вам потребуется изменить значения **COSMOSDB\_узла** и **COSMOSDB\_ключ** на следующем шаге.
    ...
    ```
3. На [портале Azure](https://portal.azure.com/) перейдите на страницу **Ключи**, щелкните **Обзор**, **Azure Cosmos DB Accounts** (Учетные записи Azure Cosmos DB), дважды щелкните имя учетной записи и нажмите кнопку **Ключи** в области **Основные компоненты**. На **ключей** скопируйте **URI** значение и вставьте его в **config.py** файл в качестве значения для **COSMOSDB\_узла**свойство. 
4. Обратно на портале Azure на **ключей** скопируйте значение **первичного ключа** или **вторичный ключ**и вставьте его в **config.py**файл в качестве значения для **COSMOSDB\_ключ** свойство.
