...
> [!NOTE]
> В настоящее время Azure Cosmos DB триггер, привязок ввода и привязок выходного работать с Graph API и API-Интерфейсы SQL только для учетных записей.
...
В играх при создании пользователя можно выполнить поиск других пользователей, которые могут быть знакомы с ним, с помощью [API Graph Azure Cosmos DB](graph-introduction.md). После этого можно написать результаты [Cosmos DB базы данных SQL Azure] для облегчения поиска.
...
4. Затем эти данные хранятся в Azure DB Cosmos, который можно затем легко получить любым приложением переднего плана, показывающий подключенных друзей нового пользователя.
