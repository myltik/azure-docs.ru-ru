...
> [!NOTE]
> В настоящее время триггер, входные и выходные привязки Azure Cosmos DB работают только с учетными записями DocumentDB, хранилищем таблиц и API Graph.
...
В играх при создании пользователя можно выполнить поиск других пользователей, которые могут быть знакомы с ним, с помощью [API Graph Azure Cosmos DB](graph-introduction.md). Затем можно записать результаты в [базу данных таблиц Azure Cosmos DB](table-introduction.md) для облегчения поиска.
...
4. Затем эти данные сохраняются в [базе данных таблиц](table-introduction.md) Azure Cosmos DB как набор пар "ключ-значение", которые затем может легко получить любое внешнее приложение, показывающее новому пользователю подключенных друзей.
...
1. Можно создать несколько функций Azure и добавить в каждую из них триггеры Azure Cosmos DB, которые ожидают передачи данных из одного канала изменений данных корзины для покупок. 
...
Если вы собираетесь интегрировать службу "Функцию Azure" для хранения данных и не нуждаетесь в глубоком индексировании, или если вам необходимо сохранять вложения и файлы мультимедиа, то [триггер хранилища BLOB-объектов Azure](../azure-functions/functions-bindings-storage-blob.md) может быть более подходящим вариантом.
...
* [Привязки и триггеры Azure Cosmos DB](../azure-functions/functions-bindings-documentdb.md)
