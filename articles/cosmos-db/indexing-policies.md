...
**Асинхронное индексирование.** Индекс обновляется асинхронно, когда коллекция Azure Cosmos DB не загружена, т. е. когда пропускная способность коллекции не используется полностью для обслуживания запросов пользователя. Для рабочих нагрузок типа "прием сейчас, запрос позже", требующих приема документов, может подойти асинхронный режим индексирования. Обратите внимание, что результаты могут быть противоречивыми, так как данные медленно принимаются и индексируются. Это означает, что все или некоторые подсчитанные результаты запросов, не будут гарантированно правильными или повторяемыми, пока данные индексируются. Индекс обычно находится в режиме наверстывания. При асинхронном индексировании WRT изменение срока жизни приводит к удалению индекса и его воссозданию, что может повлечь за собой непредвиденные результаты. Большинству клиентов следует использовать согласованное индексирование.
