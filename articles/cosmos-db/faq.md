...
API DocumentDB является одним из поддерживаемых API-интерфейсов и моделей данных для Azure Cosmos DB. Кроме того, Azure Cosmos DB предоставляет поддержку для предварительной версии API Graph, API таблиц, а также API MongoDB. См. дополнительные сведения в разделе с [вопросами клиентов DocumentDB](#moving-to-cosmos-db).
...
Служба Azure Cosmos DB доступна на портале Azure. Во-первых, вам нужно зарегистрироваться для получения подписки Azure. После этого вы сможете добавить учетную запись API DocumentDB, предварительной версии API Graph, API таблиц, а также API MongoDB в свою подписку Azure.
...
* Вызовы CreateIfNotExists регулируются регулятором управления, который фиксирован и отделен от других операций таблицы, которые покрываются единицами ЕЗ. Это означает, что пользователи, создающие большое количество запросов CreateIfNotExists, будут регулироваться и не смогут ничего с этим поделать, потому что лимит обусловлен ЕЗ.
...
| OPTIONS | /<table-resource-name> | [Preflight Table Request](https://docs.microsoft.com/en-us/rest/api/storageservices/preflight-table-request) (Предварительные запросы таблиц) | Этот метод является частью CORS и сейчас не поддерживается в Azure Cosmos DB. |
...
Вы можете использовать [обозреватель хранилищ Azure](https://docs.microsoft.com/en-us/azure/vs-azure-tools-storage-manage-with-storage-explorer).
...
[AzCopy](https://docs.microsoft.com/en-us/azure/storage/common/storage-use-azcopy) и [средство миграции данных DB Cosmos Azure](import-data.md) поддерживаются.
...
### <a name="what-is-the-protocol-version-supported-in-the-preview-do-you-plan-to-support-other-protocols"></a>Какая версия протокола поддерживается в предварительной версии? Вы планируете поддержку других протоколов?
API Cassandra Apache для Azure Cosmos DB в настоящее время поддерживает CQL версии 4. Чтобы отправить отзыв о поддержке дополнительных моделей, создайте запросы, используя [портал uservoice для отзывов](https://feedback.azure.com/forums/263030-azure-cosmos-db), или отправьте электронное письмо по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 
...
Azure Cosmos DB — это система, управляемая ресурсами, как для операций с данными, так и для действий уровня управления. Контейнеры, такие как коллекции и таблицы, представляют собой сущности среды выполнения, подготовленные для определенной пропускной способности. Быстрое создание этих контейнеров не предусмотрено и может регулироваться. Если у вас есть тесты, которые немедленно удаляют или создают таблицы, попробуйте их исключить или повторно использовать имеющиеся таблицы после удаления данных.

### <a name="what-is-maximum-number-of-tables-that-can-be-created"></a>Какое максимальное число таблиц можно создать?
Физическое ограничение на количество таблиц, которые могут быть созданы, отсутствует. Отправьте электронное письмо по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com), если у вас очень много таблиц (общий размер которых превышает 10 ТБ), которые необходимо создать из обычных 10 или 100. 

### <a name="what-is-the-maximum-number-of-keyspaces-that-can-be-created"></a>Какое максимальное число пространств ключей можно создать? 
Физическое ограничение на количество создаваемых пространств ключей отсутствует, так как они являются контейнерами метаданных. Отправьте электронное письмо по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com), если необходимо создать очень много пространств ключей. 

### <a name="is-it-possible-to-bring-in-a-lot-of-data-after-starting-from-a-normal-table"></a>Можно ли добавить большое количество данных после начала работы с обычной таблицей? 
Объем памяти контролируется автоматически и увеличивается при увеличении объема данных. Это обеспечивает достаточную пропускную способность для поддержки большого количества данных, поэтому вы можете уверенно импортировать нужный объем данных.
...
Apache Cassandra API для Azure Cosmos DB — это служба платформы. Она предоставляет совместимость на уровне протокола для выполнения операций, а также упрощает управление, мониторинг и конфигурацию. Разработчику и пользователю не нужно беспокоиться о доступности, отметках полного удаления, кеше ключей, кеше строк, фильтрах Блума и множестве других настроек. API Apache Cassandra для Azure Cosmos DB ориентирован на обеспечение требуемой производительности операций чтения и записи без увеличения расходов.
...
Apache Cassandra API — это платформа, обеспечивающая простое планирование мощности и отвечающая требованиям к эластичности для высокой пропускной способности и хранения. С помощью Azure Cosmos DB можно обеспечить требуемую пропускную способность. Затем можно увеличивать и уменьшать масштаб множество раз в течение дня, не беспокоясь о добавлении узлов, их удалении и управлении ими. Это также означает, что вам не нужно использовать инструмент управления узлом или кластером. 

### <a name="what-happens-with-respect-to-various-config-settings-for-keyspace-creation"></a>Что происходит с различными настройками конфигурации для создания пространств ключей?
Azure Cosmos DB обеспечивает готовое глобальное распределение для обеспечения высокой доступности и низкой задержки. Настройка реплики не требуется. Все операции записи всегда надежно фиксируются кворумом в любом регионе, в котором выполняется запись или распределение данных, при этом обеспечивается высокая производительность. По этой причине параметры конфигурации, такие как `Simple Strategy` и `Network Topology Strategy`, не учитываются. 

### <a name="what-happens-with-respect-to-various-settings-for-table-metadata-like-bloom-filters-caching-read-repair-changes-gcgrace-and-compression-memtableflushperiod"></a>Что происходит с различными настройками метаданных таблицы, такими как фильтры Блума, кеширование, изменение разрешения конфликтов при чтении gc_grace и сжатие memtable_flush_period?
Azure Cosmos DB обеспечивает высокую производительность операций чтения и записи и пропускную способность без необходимости настроек каких-либо параметров конфигурации и риска их случайного изменения. Это позволяет исключить набор параметров конфигурации, которыми трудно управлять.

### <a name="what-is-the-default-consistency-of-an-apache-cassandra-api-account"></a>Что такое согласованность учетной записи Apache Cassandra API по умолчанию?
Azure Cosmos DB поддерживает пять уровней согласованности, которые описаны в статье [Настраиваемые уровни согласованности данных в Azure Cosmos DB](consistency-levels.md). В настоящее время Apache Cassandra API поддерживает только согласованность сеансов. 

### <a name="is-time-to-live-ttl-supported-for-cassandra-tables"></a>Поддерживается ли срок жизни (TTL) для таблиц Cassandra? 
Да, TTL поддерживается для таблиц. 

### <a name="what-is-the-default-throughput-of-a-table-when-created-through-cql-what-if-i-need-to-change-it"></a>Что представляет собой пропускная способность по умолчанию при создании таблицы с помощью CQL? Что делать, если требуется ее изменить?
Azure Cosmos DB использует единицы запросов в секунду (ЕЗ) в качестве валюты для обеспечения пропускной способности. Для таблиц, созданных с помощью CQL, по умолчанию подготавливается 400 ЕЗ/с. Вы можете изменить пропускную способность таблицы на портале Azure и увеличить ее до 10 000 ЕЗ. Если вам нужно более 10 000 ЕЗ/с на время действия предварительной версии, создайте свою таблицу, используя портал Azure. Если вам нужна эта возможность для CQL, свяжитесь с нами по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

###  <a name="is-it-possible-to-monitor-node-status-replica-status-gc-and-os-parameters-what-should-i-monitor"></a>Возможно ли контролировать состояние узла, состояние реплики, сборку мусора и параметры ОС? Что следует отслеживать?
Служба платформы Azure Cosmos DB помогает повысить производительность и не беспокоиться об управлении инфраструктурой и ее мониторинге. Метрикой, которую требуется контролировать, является пропускная способность, доступная на вкладке "Пропускная способность" на странице метрик Azure Cosmos DB на портале Azure. На этих диаграммах указаны данные, по которым можно определить, выполняется ли регулирование и нужно ли изменить пропускную способность. Дополнительные сведения см. в статье [Мониторинг Azure Cosmos DB](monitor-accounts.md) и [Мониторинг и отладка с помощью метрик в Azure Cosmos DB](use-metrics.md).
...
Apache Cassandra API был протестирован с C#, Java, Python и драйверами узла Apache Cassandra. Если вы используете другие драйверы и нуждаетесь в помощи, напишите нам по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com).

### <a name="how-do-i-get-ru-capacity-planning-for-the-apache-cassandra-api"></a>Как получить планирование емкости по ЕЗ для API Apache Cassandra?
Azure Cosmos DB предоставляет [планировщик ресурсов](https://www.documentdb.com/capacityplanner). Укажите представление сущности json, и планировщик получит необходимое количество ЕЗ для операций чтения и записи. Для запросов и других операций имеющиеся драйверы возвращают метаданные со сведениями о потреблении ЕЗ. Вы можете использовать эти сведения для точного планирования. 

### <a name="is-composite-primary-key-supported"></a>Поддерживается ли составной первичный ключ?
Да, можно использовать обычный синтаксис для создания составного ключа раздела. 
...
### <a name="can-an-on-premise-cassandra-cluster-be-paired-with-the-azure-cosmos-db-apache-cassandra-api"></a>Может ли локальный кластер Cassandra быть сопряжен с Apache Cassandra API для Azure Cosmos DB?
Пишите нам о своих сценариях по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). Служба оптимизирована для работы с облачной средой без дополнительных расходов на операции.

### <a name="do-i-need-to-take-snapshots-and-incremental-backups"></a>Зачем мне делать моментальные снимки и добавочные резервные копии? 
Azure Cosmos DB предоставляет две бесплатные полные резервные копии, которые создаются каждые четыре часа во всех API. Это гарантирует, что вам не понадобится создавать расписание резервного копирования. Если вы хотите изменить срок хранения и частоту резервного копирования, отправьте письмо по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

### <a name="where-can-i-provide-feedback-if-a-feature-in-the-regular-cassandra-api-does-not-work"></a>Куда я могу отправить отзыв о сбое функции в обычном API Cassandra?
Отправьте сообщение электронной почты по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com).

### <a name="where-can-i-suggest-new-features-for-the-apache-cassandra-api"></a>Где можно предложить новые функции для Apache Cassandra API?
Отправьте сообщение электронной почты по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). Кроме того, вы можете предоставить отзыв на сайте [отзывов uservoice](https://feedback.azure.com/forums/263030-azure-cosmos-db). 

### <a name="why-do-i-need-to-choose-a-throughput-level"></a>Зачем необходимо выбирать уровень пропускной способности?
...
Azure Cosmos DB предоставляет гарантии производительности и задержки с верхними границами для операций. Это возможно, если обработчик может применить управление операциями клиента. Настраивая значение пропускной способности, вы гарантированно получаете требуемые показатели пропускной способности и задержки, так как платформа резервирует эту емкость и обеспечивает бесперебойное выполнение операций. См. дополнительные сведения о [единицах запросов в базе данных Azure Cosmos DB](request-units.md).

Изменяя спецификации, вы можете гибко управлять пропускной способностью в соответствии со своими потребностями, обеспечивая поддержку сезонности приложений и позволяя минимизировать затраты.

### <a name="i-never-get-a-quota-full-notification-indicating-that-a-partition-is-full-when-i-ingest-data-into-regular-cassandra-with-the-azure-cosmos-db-cassandra-api-i-do-get-this-message-is-this-offering-limiting-me-and-forcing-me-to-change-my-existing-application"></a>При приеме данных в обычный API Cassandra мне не приходили уведомления о достижении квоты, информирующие о заполнении секции. При работе с Cassandra API для Azure Cosmos DB я получаю такое сообщение. Значит ли это, что предложение связано с определенными ограничениями, и поэтому мне необходимо изменить приложение?

Azure Cosmos DB — это система на основе соглашений об уровне обслуживания, которая предоставляет неограниченные возможности масштабирования, а также гарантирует согласованность, низкую задержку, высокую пропускную способность и доступность. Объем данных и индекс должны быть управляемыми и масштабируемыми, чтобы получить гарантированную производительность уровня "Премиум". Ограничение в 10 ГБ для числа сущностей и элементов на ключ секции требуется для повышения скорости поиска и производительности запросов Azure Cosmos DB. Кроме того, для обеспечения достаточной масштабируемости приложения для службы хранилища Azure мы рекомендуем *не* создавать секцию с высокой нагрузкой, чтобы хранить в ней все сведения и выполнять запрос этих данных. 

### <a name="how-does-the-cassandra-api-handle-failover-if-a-region-goes-down"></a>Как Cassandra API выполняет отработку отказа, если регион становится недоступен? 
Cassandra API для Azure Cosmos DB использует глобально распределенную платформу Azure Cosmos DB. Чтобы обеспечить нормальную работу приложения при простое центра обработки данных, вам нужно использовать несколько регионов для учетной записи на портале Azure Cosmos DB. Вы можете задать приоритет регионов. Дополнительные сведения см. в статье [Автоматическая отработка отказа между регионами для обеспечения непрерывности бизнес-процессов в DocumentDB](regional-failover.md). 

Вы можете не только добавить любое нужное число регионов для учетной записи, но и управлять тем, в какой регион будет выполняться отработка отказа, настроив приоритет для этой процедуры. Конечно, чтобы использовать базу данных, вам также нужно предоставить приложение. Но этот простой не отразится на работе клиентов.  

### <a name="is-the-apache-cassandra-api-enabled-for-backups"></a>Включено ли резервное копирование в Apache Cassandra API?
Да, в Cassandra API для Azure Cosmos DB предусмотрены такие же резервные копии, что и в Azure Cosmos DB. Резервные копии создаются автоматически. См. дополнительные сведения об [оперативном резервном копировании и восстановлении с помощью Azure Cosmos DB](online-backup-and-restore.md).
...
### <a name="can-i-change-the-indexing-policy"></a>Можно ли изменить политику индексирования?
Да, вы можете изменить политику индексирования, предоставив определение индекса. См. дополнительные сведения о [возможностях Azure Cosmos DB](../cosmos-db/tutorial-develop-table-dotnet.md#azure-cosmos-db-capabilities). Вам нужно правильно закодировать эти параметры, а также экранировать их. Чтобы узнать больше, отправьте электронное письмо по адресу [askcosmosdbcassandra@microsoft.com](mailto:askcosmosdbcassandra@microsoft.com). 

В строке формата JSON в файле app.config приведен пример:
...
[Локальный эмулятор](local-emulator.md) в настоящее время не поддерживает Cassandra API. 
...
![Страница "Коллекции Azure Cosmos DB"](./media/faq/cosmos-db-data-explorer.png)
