...
* Один контейнер для каждой подписки SQL (API DocumentDB), Gremlin (API Graph) и учетных записей API таблиц.
...
Предварительная версия API таблиц в Azure Cosmos DB — это расширенное предложение Azure Cosmos DB для поддержки модели данных таблиц. О нем было объявлено на конференции Microsoft Build 2017. 
...
Да, [пакет SDK WindowsAzure.Storage-PremiumTable (предварительная версия)](https://www.nuget.org/packages/WindowsAzure.Storage-PremiumTable) доступен в NuGet. Он необходим для использования API таблиц Azure Cosmos DB. См. дополнительные сведения на странице [API таблицы .NET для базы данных Azure Cosmos DB. Скачивание и заметки о выпуске](https://github.com/Microsoft/azure-docs-pr/cosmos-db/table-sdk-dotnet.md). 
...
### <a name="are-there-any-changes-for-customers-who-are-using-the-existing-azure-table-storage-sdk"></a>Изменилось ли что-либо для клиентов, использующих существующий пакет SDK для хранилища таблиц Azure?
...
### <a name="can-i-connect-to-the-azure-table-storage-and-the-azure-cosmos-db-table-api-preview-side-by-side-in-the-same-application"></a>Возможно ли параллельное подключение к хранилищу таблиц Azure и API таблиц Azure Cosmos DB (предварительная версия) в одном и том же приложении? 
...
Чтобы воспользоваться преимуществами нового предложения API таблиц Azure Cosmos DB в существующем источнике данных хранилища таблиц, отправьте электронное сообщение по адресу [askcosmosdb@microsoft.com](mailto:askcosmosdb@microsoft.com). 
...
Отслеживать запросы и использование хранилища можно на портале Azure с помощью области **Метрики** API таблиц (предварительная версия). 
...
### <a name="can-my-existing-azure-table-storage-application-work-with-the-table-api-preview"></a>Совместимо ли мое существующее приложение для хранилища таблиц Azure с API таблиц (предварительная версия)? 
Контактная зона API таблиц (предварительная версия) совместима с существующим пакетом SDK для хранилища таблиц Azure. Вы можете создавать, удалять, обновлять и выполнять запросы с помощью API .NET. У вас должен быть ключ строки, так как для работы с предварительной версией API таблиц требуется как ключ секции, так и ключ строки. Мы также планируем добавить дополнительную поддержку пакетов SDK в рамках подготовки общедоступной версии этого предложения службы.

### <a name="do-i-need-to-migrate-my-existing-azure-table-storage-applications-to-the-new-sdk-if-i-do-not-want-to-use-the-table-api-preview-features"></a>Необходимо ли переводить существующие приложения для хранилища таблиц Azure на новый пакет SDK, если я не планирую использовать возможности API таблиц (предварительная версия)?
Нет, вы можете создавать и использовать существующие ресурсы хранилища таблиц Azure, не прерывая работу. Но если вы не работаете с новой предварительной версией API таблиц, вы не сможете воспользоваться преимуществами автоматической индексации, дополнительных способов обеспечения согласованности и глобального распределения. 

### <a name="how-do-i-add-replication-of-the-data-in-the-table-api-preview-across-multiple-regions-of-azure"></a>Как добавить репликацию данных в API таблиц (предварительная версия) для нескольких регионов Azure?
...
### <a name="how-do-i-change-the-primary-write-region-for-the-account-in-the-table-api-preview"></a>Как изменить основной регион записи для учетной записи в API таблиц (предварительная версия)?
...
### <a name="does-the-azure-cosmos-db-table-api-offer-more-consistency-levels-than-azure-table-storage"></a>Обеспечивает ли API таблиц DB Cosmos Azure дополнительные уровни согласованности по равнению с хранилищем таблиц Azure?
...
### <a name="is-the-table-api-preview-enabled-for-backups"></a>Предусмотрена ли в API таблиц (предварительная версия) поддержка резервных копий?
Да, API таблиц Azure Cosmos DB (предварительная версия) использует платформу Azure Cosmos DB для резервного копирования. Резервные копии создаются автоматически. См. дополнительные сведения об [оперативном резервном копировании и восстановлении с помощью Azure Cosmos DB](online-backup-and-restore.md).
...
Да, по умолчанию Azure Cosmos DB индексирует все атрибуты сущности. См. дополнительные сведения о [политиках индексации Azure Cosmos DB](indexing-policies.md). 
...
### <a name="is-there-any-change-of-pricing-for-existing-customers-of-azure-table-storage"></a>Изменились ли какие-либо цены для клиентов хранилища таблиц Azure?
...
Стоимость использования API таблиц Azure Cosmos DB (предварительная версия) зависит от выделенного значения TableThroughput. 

### <a name="how-do-i-handle-any-throttling-in-table-api-preview-offering"></a>Как обрабатывать регулирование в API таблиц (предварительная версия)? 
...
### <a name="why-do-i-need-to-choose-a-throughput-apart-from-partitionkey-and-rowkey-to-take-advantage-of-the-azure-cosmos-db-table-api-preview"></a>Зачем при использовании API таблиц Azure Cosmos DB (предварительная версия) кроме значений PartitionKey и RowKey нужно выбирать еще и пропускную способность?
...
Так как эта предварительная версия совместима с хранилищем таблиц Azure, большинство ошибок будет сопоставимо с ошибками таблицы уровня "Стандартный". 
