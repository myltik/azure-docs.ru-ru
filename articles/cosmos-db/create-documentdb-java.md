...
3. В окне терминала git выполните команду `mvn exec:java -D exec.mainClass=GetStarted.Program`, чтобы открыть приложение Java.

    В окне терминала отобразится уведомление о создании базы данных FamilyDB. Нажмите любую клавишу, чтобы создать коллекцию. Затем перейдите в обозреватель данных, и убедитесь, что в нем теперь содержится база данных FamilyDB.
    
    Продолжайте нажимать клавиши, чтобы создать документы и выполнить запрос.
