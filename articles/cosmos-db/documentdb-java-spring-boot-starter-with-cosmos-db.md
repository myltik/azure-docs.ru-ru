...
1. В колонке **Azure Cosmos DB** введите следующие сведения:
...
1. При создании база данных указывается на **панели мониторинга** Azure, а также в колонках **Все ресурсы** и **Azure Cosmos DB**. Вы можете выбрать свою базу данных в любом из этих расположений, чтобы открыть колонку свойств кэша.

   ![Портал Azure][AZ04]

1. Когда откроется колонка свойств базы данных, щелкните **Ключи доступа** и скопируйте URI и ключи доступа для базы данных. Эти значения будут использоваться в приложении Spring Boot.
...
1. Откройте файл основного приложения Java в текстовом редакторе и добавьте в него следующие строки:
   ...
   > [!NOTE]
   >
   > Класс приложение использует два класса, *UserRepository* и *User*, которые будут определены и реализованы позже.
   >
...
### <a name="define-a-basic-class-for-storing-data"></a>Определение базового класса для хранения данных
...
1. НЕОБЯЗАТЕЛЬНО. На портале Azure можно просмотреть содержимое Azure Cosmos DB в колонке свойств для базы данных. Для этого нужно щелкнуть **обозреватель документов** и выбрать элемент из списка для просмотра содержимого.
