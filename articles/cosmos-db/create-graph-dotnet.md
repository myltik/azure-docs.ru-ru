...
1. В Visual Studio 2017 откройте файл appsettings.json. 
...
3. Скопируйте значение **URI** на портале и добавьте его в качестве значения параметра ключа конечной точки в файле appsettings.json. Как показано на предыдущем снимке экрана, вы можете использовать кнопку "Копировать" для копирования значения.
