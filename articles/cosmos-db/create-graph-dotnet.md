...
1. На портале Azure в учетной записи Azure Cosmos DB щелкните **Обзор** в левой области навигации. Вы скопируете значение **Gremlin URI** в файл App.config на следующем шаге. 

    ![Просмотр и копирование ключа доступа на портале Azure, колонка "Ключи"](./media/create-graph-dotnet/gremlin-uri.png)
...
3. Скопируйте значение **Gremlin URI** из портала и добавьте его в качестве значения параметра ключа конечной точки в файле App.config. 

    `<add key="Endpoint" value="FILLME.graphs.azure.com:443" />`

4. На портале Azure в области навигации слева щелкните **Ключи**, скопируйте значение параметра **Первичный ключ** из портала и добавьте его в качестве значения параметра ключа AuthKey в файле App.config. Затем сохраните изменения. 
