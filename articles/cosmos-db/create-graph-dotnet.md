...
Теперь вы можете добавить данные в созданный граф с помощью обозревателя данных.

1. В обозревателе данных разверните **sample-database**, **sample-graph**, щелкните **График**, затем **New Vertex** (Новая вершина) и **New Edge** (Новое ребро), чтобы добавить элементы в граф. В обозревателе данных вы можете масштабировать пропускную способность, а также добавлять хранимые процедуры, определенные пользователем функции и триггеры в свой контейнер.

    ![Добавление узлов и ребер в граф с помощью обозревателя данных](./media/create-graph-dotnet/azure-cosmos-db-graph-sample-data.png)

2. После добавления элементов нажмите кнопку **Применить фильтр** или щелкните правой кнопкой мыши **График** и щелкните **New Graph Query** (Новый запрос графа) для просмотра графа данных. Вы можете изменить внешний вид и подписи данных, нажав кнопку **Стиль** и изменив параметры. Ниже приведен пример графа в обозревателе данных. Подписи, цвета и данные графа можно изменить.

    ![Визуальный обозреватель графа в обозревателе данных на портале Azure](./media/create-graph-dotnet/azure-cosmos-db-graph-explorer.png)
