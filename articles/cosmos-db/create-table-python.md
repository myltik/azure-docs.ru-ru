...
Azure Cosmos DB — это глобально распределенная многомодельная служба базы данных Майкрософт. Вы можете быстро создавать и запрашивать документы, пары "ключ — значение", базы данных с широкими столбцами и базы данных графов, используя преимущества возможностей глобального распределения и горизонтального масштабирования Azure Cosmos DB. 
...
1. На [портале Azure](http://portal.azure.com/) выберите элемент **Строка подключения**. 

    ![Просмотр и копирование строки подключения на панели строки подключения](./media/create-table-python/connection-string.png)

2. Откройте файл config.py и скопируйте необходимые свойства строки подключения в файл конфигурации.
...
> [Import data for use with the Azure Cosmos DB Table API](table-import.md) (Импорт данных для использования с API таблицы Azure Cosmos DB)
