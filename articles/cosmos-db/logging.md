...
* Регистрируются всех запросов проверки подлинности REST SQL API, включающий невыполненных запросов, в результате разрешения на доступ, системных ошибок или неправильных запросов. Сейчас MongoDB, Graph и API таблицы не поддерживаются.
...
2. На странице **Параметры диагностики** сделайте следующее: 
    ...
    * **Log DataPlaneRequests** (Журнал DataPlaneRequests). Выберите этот параметр, чтобы журнала диагностики для учетных записей SQL, диаграммы и таблицы API. Если выполняется архивация в учетную запись хранения, можно выбрать период удержания для журналов диагностики. По окончании периода хранения журналы удаляются автоматически.
