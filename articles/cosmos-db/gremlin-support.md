...
Здесь представлено краткое руководство по языку Gremlin с перечислением функций, доступных для этого языка, и действий, поддерживаемых в предварительной версии поддержки API Graph.
...
| --- | --- | --- |
| Функции графа | В предварительной версии обеспечивается сохраняемость и одновременный доступ. Разработаны для поддержки транзакций. | Вычислительные методы могут применяться через соединитель Spark. |
