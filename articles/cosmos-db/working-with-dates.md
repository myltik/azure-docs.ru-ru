...
Пакет SDK для .NET SQL автоматически поддерживает запросы к данным, хранящимся в базе данных Azure Cosmos через LINQ. Например следующий фрагмент кода демонстрирует запрос LINQ, который отбирает все заказы, доставленные за последние три дня.
...
* Дополнительные сведения о [SQL-запросов](sql-api-sql-query.md)
