...
---
# <a name="azure-cosmos-db-c-console-application-tutorial-for-the-sql-api"></a>Azure Cosmos DB: Учебник C++ консоли приложения для API-Интерфейсы SQL
...
Добро пожаловать в учебник C++, для базы данных SQL Azure Cosmos API пакета SDK одобрены для C++! По завершении работы с этим руководством у вас будет консольное приложение, которое создает ресурсы Azure Cosmos DB и выполняет запросы этих ресурсов, включая базу данных C++.

Рассматриваются краткого руководства:
...
У вас нет времени? Не беспокойтесь! Полное решение доступно на [GitHub](https://github.com/stalker314314/sql-apiCpp). Краткие инструкции см. в разделе [Получение полного решения для руководства по Node.js](#GetSolution).
...
2. Далее добавьте следующий код к функции main, а затем замените настройки учетной записи и первичный ключ соответствующими параметрами Azure Cosmos DB, скопированными на шаге 3. 
    ...
    Теперь, когда у вас есть код, чтобы инициализировать клиент, давайте рассмотрим работы с ресурсами Azure Cosmos DB.
