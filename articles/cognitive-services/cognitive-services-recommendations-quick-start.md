...
1. Выберите продукт **Cognitive Services APIs**. Этот продукт дает возможность запустить подписку с любым API когнитивных служб (для распознавания лиц, анализа текста, машинного распознавания образов и т. д.). Сегодня мы уделим внимание API рекомендаций.
...
1. Выберите план для параметра **Ценовая категория**. Вы можете выбрать уровень **Бесплатный** с 10 000 транзакций в месяц**. Это бесплатный план, хорошо подходящий для того, чтобы ознакомиться с системой. Как только вы перейдете к рабочей среде, мы рекомендуем запросить дополнительный объем и соответствующим образом изменить тип плана.
...
1. Обычно эта информация запрашивается из базы данных транзакций. На случай, если у вас нет их под рукой, мы предоставили примеры данных, полученных на основе данных транзакций Microsoft Store.
 ...
 > ** Примечание. ** Пример кода, который вы скачаете и выполните для задачи 3, уже содержит встроенные примеры данных, поэтому данная операция не обязательна. С другой стороны, эта задача позволит скачать более реалистичные наборы данных и даст возможность лучше понять, какие входные данные используются для API рекомендаций.
...
1.	Теперь рассмотрим файл каталога. Перейдите к расположению, в которое были скопированы данные. Откройте файл каталога в **Блокноте**.
 ...
 Следует отметить, что файл каталога может быть намного содержательнее. Например, в него можно добавить метаданные о продуктах (мы называем их *признаками элементов*). Чтобы больше узнать о формате каталога, ознакомьтесь с разделом о [формате каталога](http://go.microsoft.com/fwlink/?LinkID=760716) в справочнике по API.

1. Сделаем то же самое с данными об использовании. Вы обратите внимание, что данные об использовании имеют формат `<User Id>,<Item Id>`.

  > 00030000A11B024B,GZA-00202<br> 0003BFFD93B934B7,P2W-00004<br> 000300009C01C881,W6F-00121<br> 00060000AF0D2B04,QR2-00011<br>

 Это минимум информации, необходимой для действительного файла использования. Более сложный файл использования может содержать дополнительную информацию о каждой транзакции, включая отметку времени операции и тип произошедшего события (щелчок, покупка и т. д.). Чтобы больше узнать об этой теме, вы можете ознакомиться с [форматом данных об использовании](http://go.microsoft.com/fwlink/?LinkID=760712).
...
1. Прежде всего следует принять к сведению [справочник по API рекомендаций](http://go.microsoft.com/fwlink/?LinkId=759348).
...
1. Замените значения полей **accountEmail** и **accountKey** своим электронным адресом и ключом из задачи 1.
