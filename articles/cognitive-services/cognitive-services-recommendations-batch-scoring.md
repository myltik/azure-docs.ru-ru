---
title: "Получение пакетов рекомендаций: API рекомендаций для машинного обучения | Документация Майкрософт"
...
---
# <a name="get-recommendations-in-batches"></a>Получение пакетов рекомендаций
> [!NOTE]
> Процесс получения пакетов рекомендаций сложнее, чем получение рекомендаций по одной. Здесь вы можете получить информацию о том, как получить рекомендацию для одного запроса:
> 
> [Рекомендации "элемент — элемент"](https://westus.dev.cognitive.microsoft.com/docs/services/Recommendations.V4.0/operations/56f30d77eda5650db055a3d4)<br>
> [Рекомендации "пользователь — элемент"](https://westus.dev.cognitive.microsoft.com/docs/services/Recommendations.V4.0/operations/56f30d77eda5650db055a3dd)
> 
...
## <a name="create-a-storage-container-if-you-dont-have-one-already"></a>Создание контейнера хранилища
Перейдите на [портал Azure](https://portal.azure.com) и создайте новую учетную запись хранения, если у вас ее нет. Для этого выберите **Создать** > **Данные** + **Хранилище** > **Учетная запись хранения**.
...
Отправьте входной файл, в котором описываются все запросы рекомендаций к хранилищу BLOB-объектов Azure. В этом примере он будет называться input.json.
После создания контейнера необходимо отправить из службы рекомендаций файл, содержащий сведения о каждом запросе, который необходимо выполнить.
...
## <a name="kick-start-the-batch-job"></a>Запуск пакетного задания
...
* В настоящее время параметр **apiName** поддерживает только значение **ItemRecommend**, используемое для рекомендаций "элемент — элемент". Пакетная обработка пока не поддерживает рекомендации "пользователь — элемент".

## <a name="wait-for-the-asynchronous-operation-to-finish"></a>Ожидание завершения асинхронной операции
После запуска пакетной операции в ответе вернется заголовок Operation-Location, в котором содержатся все необходимые сведения для отслеживания операции.
Для отслеживания пакетной операции (как и операции создания) используется [API получения состояния операции](https://westus.dev.cognitive.microsoft.com/docs/services/Recommendations.V4.0/operations/56f30d77eda5650db055a3da).

## <a name="get-the-results"></a>Получение результатов
...
## <a name="learn-about-the-limitations"></a>Дополнительные сведения об ограничениях
