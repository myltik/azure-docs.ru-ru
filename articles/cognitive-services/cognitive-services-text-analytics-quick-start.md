...
Это руководство предназначено для версии 2 API. Дополнительные сведения о версии 1 интерфейсов API см. в [этом документе](../machine-learning-apps-text-analytics/).
...
Для этого API необходимо отправить **как минимум 100 текстовых записей**. Он предназначен для обнаружения тем в сотнях и тысячах записей. Записи на любом языке, кроме английского, и записи, которые содержат менее 3 слов, будут отклонены и не будут назначены темам. При определении темы максимальный размер одного отправленного документа составляет 30 КБ. Максимальный объем всех входных данных — 30 МБ.
