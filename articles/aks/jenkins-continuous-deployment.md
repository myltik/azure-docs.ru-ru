...
Если возникают проблемы при входе в Jenkins, создайте сеанс SSH с виртуальной машиной Jenkins и перезапустите службу Jenkins. IP-адрес виртуальной машины имеет тот же адрес, который был указан в скрипте сборки. Имя пользователя администратора виртуальной машины — `azureuser`.
...
Обновите окно браузера, чтобы появилась форма входа Jenkins.
...
После завершения процесса можно щелкнуть **build #1** (Сборка № 1) в журнале сборки и выбрать **Console Output** (Вывод в консоль), чтобы просмотреть все выходные данные процесса сборки. Последняя строка должна указывать, что сборка выполнена успешно. 
...
В каталоге **/azure-vote/azure-vote** можно найти файл **config_file.cfg**. Обновите значения для голосования в этом файле, указав что-нибудь иное вместо кошек и собак. 
