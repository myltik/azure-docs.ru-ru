<properties linkid="develop-net-tutorials-website-with-mongodb-mongolab" urlDisplayName="Веб-сайт с MongoDB с MongoLab" pageTitle="Создание веб-сайта, использующего MongoDB с MongoLab (.NET)" metaKeywords="" description="Узнайте, как создать веб-сайт Azure, который хранит данные в базе данных MongoDB, размещенной в MongoLab." metaCanonical="" services="web-sites" documentationCenter=".NET" title="Создание приложения ASP.NET на C# в Azure с MongoDB с использованием надстройки MongoLab" authors="" solutions="" manager="" editor="" />
...
1. [Подготавливать базу данных][provision] - надстройка [MongoLab](http://mongolab.com) в Магазине Azure предоставляет доступ к базе данных MongoDB, которая размещена в облаке Azure и управляется облачной платформой базы данных MongoLab.
1. [Создавать приложение][create] - это будет простое приложение ASP.NET MVC на C# для создания заметок.
1. [Развертывать приложение][deploy] - объединяя вместе несколько обработчиков конфигурации, мы значительно упрощаем перемещение кода.
...
1. Для поля **Имя** введите значение MONGOLAB\_URI.
...
1. В Visual Studio установите драйвер Mongo C#, выбрав **Средства > Диспетчер пакетов библиотеки > Консоль диспетчера пакетов**. В консоли PM введите команду **Install-Package mongocsharpdriver** и нажмите клавишу **ВВОД**.
1. Настройте обработчик в коде, чтобы получить URI подключения MongoLab из переменной среды:
...
В этом разделе мы рассмотрим создание проекта C# ASP.NET в Visual Studio и использование драйвера C# MongoDB для разработки простого приложения создания заметок. Вы сможете посетить ваш веб-сайт, создать заметку и просмотреть всех оставшиеся заметки.
...
1. Выберите **.NET Framework 4** в раскрывающемся меню версий .NET (*Примечание. Framework 4.5 в данный момент не работает*).
...
1. Выберите **Интернет-приложение** и нажмите кнопку **ОК**. Выполнится построение проекта.
...
Для получения дополнительных сведений об использовании драйвера MongoDB для C# посетите [Краткое руководство по драйверу CSharp](http://www.mongodb.org/display/DOCS/CSharp+Driver+Quickstart "CSharp Driver Quickstart") на сайте mongodb.org.

### Добавление представления создания
Теперь добавим представление для создания заметки.

1. Щелкните правой кнопкой запись **Представления > Главная страница** в обозревателе решений и выберите **Добавить > Представление**. Задайте для представления имя **Create** и нажмите кнопку **Добавить**
...
### Создание веб-сайта и получение файла параметров публикации
...
![Опубликовать][dialog-mongolab-vspublish]
...
1. После успешного завершения проверки нажмите кнопку **Опубликовать**. После завершения публикации откроется новая вкладка браузера с вашим веб-сайтом.
