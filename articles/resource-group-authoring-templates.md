...
Сведения, которые помогут вам определить, какие типы ресурсов доступны для развертывания, какие регионы поддерживаются для данного типа, а также какие версии API доступны для каждого типа, можно найти в разделе [Поставщики диспетчера ресурсов, регионы, версии API и схемы](resource-manager-supported-services.md). В этом разделе приводятся примеры и ссылки, которые помогут вам определить значения, которые нужно указать в шаблоне.

Если ресурс должен быть развернут после другого ресурса, можно пометить его как зависимый от другого ресурса. Эта процедура подробно описана в разделе [Ресурсы](#resources) ниже.

Результаты развертывания шаблона можно изменять, указывая значения параметров во время выполнения. Эта процедура описана в разделе [Параметры](#parameters) ниже.

Значения из развертывания можно возвращать в разделе [Результаты](#outputs).
...
| location | Нет | Поддерживаемые географические расположения указанного ресурса. Чтобы определить доступные расположения, см. раздел [Поддерживаемые регионы](../resource-manager-supported-services/#supported-regions).
