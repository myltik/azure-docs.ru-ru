...
Azure группирует службы по категориям на портале Azure и на различные визуальные средства, такие как [возможности Azure Инфографикой](https://azure.microsoft.com/documentation/infographics/azure/). [Портал Azure](http://portal.azure.com) является то, что используется для управления службами в Azure.

В этой статье используется **другой организации** говорить о служб, основанных на аналогичные функции и выполнить вызов важные вложенные служб, которые являются частью большим.  
...
Azure имеет веб-интерфейс называется [портал Azure](http://portal.azure.com) , позволяет администраторам доступ и права администрирования Azure компонентов и служб.  
...
2. **Перемещение приложений в Azure (извлечение и перемещение)**. Извлечение и перемещение приложения похоже на перемещение большого объекта с помощью погрузчика.  Вы "извлекаете" виртуальный жесткий диск из локального центра данных, "перемещаете" его в Azure и запускаете.  Обычно необходимо выполнить некоторые действия, чтобы удалить зависимости от других систем. Если зависимостей слишком много, можно вместо этого выбрать вариант 3.  
...
Именно это и предоставляют веб-приложения. Эта модель вычислений предлагает управляемого веб-среду с помощью портала Azure, а также API-интерфейсы. Вы можете переместить существующее приложение веб-сайта в модель веб-приложений без каких-либо изменений или можете создать новый веб-сайт прямо в облаке. После запуска веб-сайта вы можете добавлять или удалять экземпляры динамически, при этом модель веб-приложений Azure осуществляет балансировку нагрузки по запросам для них. Модель приложений Azure поддерживает два варианта: общий вариант — запуск веб-сайта в виртуальной машине с другими сайтами, и стандартный вариант, предполагающий работу сайта в отдельной виртуальной машине. Стандартная возможность также позволяет увеличить размер своих экземпляров (вычислительной мощности).

В целях осуществления разработок служба веб-приложений поддерживает такие языки и платформы, применяемые в работе с реляционными хранилищами: .NET, PHP, Node.js, Java, Python, База данных SQL и База данных Azure для MySQL. Она также имеет встроенную поддержку нескольких популярных приложений, включая WordPress, Joomla и Drupal. Задача заключается в предоставлении экономичной, масштабируемой и универсальной платформы для создания веб-сайтов и веб-приложений в общедоступном облаке.
...
На странице [Сравнение веб-приложений, облачных служб и виртуальных машин Azure](app-service/choose-web-site-cloud-service-vm.md) можно найти более подробную информацию о том, как выбрать модель вычислений.
...
Azure предлагает целый ряд средств, чтобы помочь разработчикам и ИТ-специалистам создавать и поддерживать приложения в облаке.  
...
Visual Studio Team Services является именем маркетинга, охватывающие число служб, которые позволяют разрабатывать приложения в Azure.
...
Visual Studio Team Services предоставляют службу нагрузочного тестирования. Вы можете выполнять нагрузочные тесты, созданные в Visual Studio, в виртуальных машинах в облаке. Можно указать общее число пользователей, необходимо загрузить тест с и Visual Studio Team Services автоматически определяет, сколько агенты необходимы запустили необходимые виртуальные машины и выполнения нагрузочных тестов. Если вы являетесь подписчиком MSDN, то получите тысячи бесплатных минут пользователей нагрузочного тестирования в месяц.
...
Предположим, что у вашей компании есть устройства, которым необходимо обращаться в центральную службу для получения данных. Например, вы транспортная компания, а в каждом вашем грузовом автомобиле имеется такое устройство.  Наверняка компании требуется настроить систему отслеживать свой грузовики, поэтому можно надежно предсказать и обновить время доставки. Это позволит узнать количество грузовиков и планировать в соответствии с этими сведениями.  Каждый грузовик должен устройство, которое выполняет обратный вызов в центральном расположении с его расположение и скорость данных и может быть несколько.

Клиент транспортной компании также может воспользоваться преимуществом полученных данных о расположении.  Клиент может использовать их, чтобы узнать, насколько далеко находятся продукты, попал ли грузовик в пробку, сколько составляет плата за определенные маршруты (если это связано с оплатой перевозки). Если транспортная компания уже собирает эти данные, многие клиенты могут заплатить за это.  Но в этом случае транспортной компании нужен способ предоставления данных клиентам. После предоставления доступа клиентам, возможно, не удастся контролировать частоту запросов данных. Они должны предоставить правила, определяющие, кто имеет доступ к данных. Все эти правила придется встраивать во внешний интерфейс API. И в этом случае управление API может помочь.  
...
![Многофакторная идентификация Azure](./media/fundamentals-introduction-to-azure/MFAIntroNew.png)   

*Рис. Служба Многофакторной идентификации Azure предоставляет вашему приложению возможность выполнять проверку с помощью нескольких способов идентификации*
...
Если вы уже используете Windows Server или System Center, служба архивации Azure — это естественное решение для резервное копирование файловых серверов, виртуальных машин и баз данных SQL Server.  Она работает с зашифрованными, разреженными и сжатыми файлами. Существуют некоторые ограничения, поэтому следует сначала [проверить предварительные требования службы архивации Azure](http://technet.microsoft.com/library/dn296608.aspx) .
...
Поскольку служба Azure со временем выросла, служба очередей хранилища Azure и служба очередей служебной шины предоставляют аналогичные службы очередей. Рекомендации по использованию той или иной службы см. в техническом документе [Очереди Azure и очереди служебной шины: сходства и различия](http://msdn.microsoft.com/library/azure/hh767287.aspx).  Во многих сценариях либо работает.
...
Иногда приложения нужно включать только в определенное время. В Azure вы можете сэкономить деньги с помощью этого типа приложения, вместо того, чтобы разрешить приложению работать круглосуточно в ожидании данных для обработки. Планировщик Azure позволяет планировать, когда приложение следует запускать на основании интервала времени и календаря. Он надежен и проверит, что процесс запускается даже при сбое сети, компьютера и центра обработки данных. Можно использовать планировщик API REST для управления этими действиями.
...
В Azure существует несколько разных технологий кэша, которые изменились со временем. В порядке представления существует общий кэш, кэш роли, управляемый кэш и кэш Redis. Общий кэш — это более старая технология, с ее помощью не следует создавать новые внедрения. Управляемый кэш имеет те же функции кэша в роли, но в качестве управляемой службы за пределами портала Azure. В реализации Redis больше всего функций, и рекомендуется использовать ее при записи нового кода кэша.
...
Один из самых привлекательных способов использования облачной платформы — это запустить высокопроизводительные вычислительные системы (HPC) и другие приложения для "больших вычислений". Примеры включают специализированные проектирования приложения, созданные для интерфейса передачи стандартных сообщений (MPI), а также так называемые неудобно параллельные приложения, такие как финансовое моделей.
...
Для распространения своего продукта приложение может использовать сеть доставки содержимого Azure CDN, другую сеть CDN или просто отправить биты непосредственно пользователям. Тем не менее когда он существует, видео, созданные с помощью служб мультимедиа может использоваться различных клиентских систем, включая Windows, Macintosh, HTML 5, iOS, Android, Windows Phone, Flash и Silverlight. Цель состоит в том, чтобы упростить создание современных мультимедийных приложений.
...
Различие между этими двумя — Marketplace находится за пределами портала Azure, что хранилище может осуществляться на портале. Потенциальные покупатели могут искать приложения Azure, соответствующие их требованиям. Клиенты также могут искать коммерческие наборы данных, включая демографические, финансовые, географические данные и многое другое. При их найти то, что угодно, их можно получить доступ к его из поставщика, напрямую через веб-сайтах Marketplace или хранилища или в некоторых случаях на портале Azure. Приложения также могут использовать в Marketplace интерфейс Bing Search, что даст им доступ к результатам поиска в Интернете.
...
Теперь, когда вы получили общее представление, следующим шагом будет написание вашего первого приложения Azure. Выберите язык, [получите соответствующий SDK](https://azure.microsoft.com/en-us/downloads/)и начинайте работу. Облачные вычисления предоставляют новый вариант по умолчанию — начинать работу сразу.
