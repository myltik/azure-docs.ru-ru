<properties urlDisplayName=".NET Client Library" pageTitle="Создание приложения списка лидеров с помощью внутреннего сервера .NET мобильных служб Azure" metaKeywords="Azure Mobile Services, Mobile Service .NET client, .NET client" description="Узнайте, как создать приложение магазина Windows с помощью мобильных служб Azure с серверной частью .NET." documentationCenter="Mobile" title="Creating a Leaderboard App with Azure Mobile Services .NET Backend" authors="mwasson" solutions="" manager="dwrede" editor="" />
...
В этом учебнике используется [Visual Studio 2013 с обновлением 3](http://go.microsoft.com/fwlink/p/?LinkID=390465). 
...
- Player. Содержит идентификатор и имя игрока.
- PlayerRank. Содержит баллы и рейтинг игрока.

PlayerRank содержит внешний ключ к Player. У каждого игрока может быть один параметр PlayerRank или не может быть ни одного.
...
В Visual Studio 2013 с обновлением 3 в проект веб-приложения ASP.NET будет включен шаблон для мобильной службы Microsoft Azure. Выберите этот шаблон и нажмите кнопку **OK**.
...
Для определения таблиц баз данных будет использоваться [EF Code First](http://msdn.microsoft.com/en-US/data/ee712907#codefirst). В папке DataObjects добавьте класс с именем `Player`.
...
Обратите внимание, что оба класса наследуются из класса **EntityData**. Наследование из класса **EntityData** упрощает использование данных приложением с помощью кроссплатформенной клиентской библиотеки для мобильных служб Azure. **EntityData** также упрощает [обработку конфликтов записи базы данных](http://azure.microsoft.com/ru-ru/documentation/articles/mobile-services-windows-store-dotnet-handle-database-conflicts/).
...
Далее вы добавите контроллеры веб-API для `Player` и `PlayerRank`. Вместо простых контроллеров веб-интерфейса API будет добавлен контроллер с именем *контроллер таблиц*, разработанный специально для мобильных служб Azure.

Щелкните правой кнопкой мыши папку "Контроллеры", нажмите кнопку "Добавить" и выберите "Создать элемент Scaffolded".

![][6] 

В диалоговом окне**Добавление Scaffold** откройте **Общие** слева и выберите **Мобильные службы Windows Azure**. Затем выберите **Контроллер таблиц мобильных служб Microsoft Azure**. Щелкните **Добавить**.
...
Помните, что `PlayerRank` имеет связанную сущность `Player`: 
...
В классе `PlayerRankController` мы будем использовать метод LINQ **Select** для преобразования экземпляров `PlayerRank` в экземпляры `PlayerRankDto`. Обновите методы контроллера `GetAllPlayerRank` и `GetPlayerRank` следующим образом.
...
С этими изменениями два метода GET вернут объекты `PlayerRankDto` в клиент. Имени игрока назначено свойство `PlayerRankDto.PlayerName`. Вот пример ответа после внесения этого изменения:
...
Сущность `PlayerRank` включает свойство `Рейтинг`. Это значение рассчитывает сервер, и не нужно, чтобы клиенты задавали его. Вместо этого клиенты будут использовать пользовательский API для отправки баллов игрока.  При получении сервером нового количества баллов он обновит все рейтинги игрока.

Сначала добавьте класс с именем `PlayerScore` в папку DataObjects.
...
В классе `PlayerRankController` переместите переменную `MobileServiceContext` из конструктора в класс переменной:
...
Затем добавьте в `PlayerRankController` следующий код:
...
Метод `PostPlayerScore` принимает экземпляр `PlayerScore` в качестве входных данных. (Клиент отправит `PlayerScore` в HTTP-запрос POST.) Метод делает следующее.
...
Можно также поместить метод в отдельный контроллер. В любом случае нет определенного преимущества, все зависит от способа организации кода.
Дополнительные сведения об атрибуте **[Route]** см. в [Маршрутизация атрибута в веб-API](http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2).
...
Добавьте еще один класс с именем `LeaderboardViewModel`.
...
Откройте файл App.xaml.cs и добавьте экземпляр **MobileServiceClient** в класс `Приложение`.
...
В диалоговом окне **Публикация** щелкните **Мобильные службы Windows Azure**.

![][13]
 
Если вы еще не вошли в учетную запись Azure, щелкните **Sign In**(Вход).

![][14]


Выберите существующую мобильную службу или нажмите кнопку **Создать**, чтобы создать новую. Затем нажмите кнопку **OK** для публикации.
...
* [Дополнительные сведения о веб-интерфейсах API]
...
[Дополнительные сведения о веб-интерфейсах API]: http://asp.net/web-api
