...
Для простоты приложение не связано с реальной игрой. Вместо этого вы можете добавлять игроков и отправлять баллы для каждого из них. При отправке баллов мобильная служба подсчитывает новые рейтинги. В серверной части мобильная служба создает базу данных с двумя таблицами::
...
Загрузите весь проект [здесь](http://code.msdn.microsoft.com/Leaderboard-App-with-Azure-9acf63af).
...
В Visual Studio 2013 с обновлением 3 в проект веб-приложения ASP.NET будет включен шаблон для мобильной службы Microsoft Azure. Выберите этот шаблон и нажмите кнопку **ОК**.
...
Далее будет показано, как добавить контроллеры веб-API для `Player` и `PlayerRank`.. Вместо простых контроллеров веб-API будет добавлен контроллер с именем *контроллер таблиц*, разработанный специально для мобильных служб Azure.
...
С этими изменениями два метода GET вернут в клиент объекты `PlayerRankDto`. Свойство `PlayerRankDto.PlayerName` задается для имени игрока. Вот пример ответа после внесения этого изменения:
...
Вместо операторов Select LINQ можно использовать AutoMapper. Этот параметр возвращает дополнительный код настройки, но включает автоматическое сопоставление из сущностей домена в DTO. Дополнительные сведения см. в разделе [Сопоставление между типами базы данных и типами клиентов в серверной части .NET с помощью AutoMapper](http://blogs.msdn.com/b/azuremobile/archive/2014/05/19/mapping-between-database-types-and-client-type-in-the-net-backend-using-automapper.aspx).

## Определение пользовательского API для отправки оценок

Сущность `PlayerRank` включает в себя свойство `Rank`. Это значение рассчитывает сервер, и не нужно, чтобы клиенты задавали его. Вместо этого клиенты будут использовать пользовательский API для отправки баллов игрока.  При получении сервером нового количества баллов он обновит все рейтинги игрока.
...
Дополнительные сведения об атрибуте **[Route]** см. в разделе [Маршрутизация атрибута в веб-API](http://www.asp.net/web-api/overview/web-api-routing-and-actions/attribute-routing-in-web-api-2).

## Создание приложения Магазина Windows

В этом разделе описывается приложение Магазина Windows, использующее мобильную службу. Однако здесь не будут подробно рассматриваться XAML или пользовательский интерфейс. Вместо этого будут приведены сведения о логике приложения. Загрузить весь проект можно [здесь](http://code.msdn.microsoft.com/Leaderboard-App-with-Azure-9acf63af).
...
В обозревателе решений щелкните правой кнопкой мыши проект "Список лидеров" и выберите **Опубликовать**.
...
Если вы еще не вошли в учетную запись Azure, нажмите кнопку **Войти**.
...
[Добавление контроллеров веб-API]: #add-web-api-controllers
