<properties urlDisplayName="Compute Intensive .NET Task" pageTitle="Ресурсоемкая задача .NET на виртуальной машине - Azure" metaKeywords="deploying compute .NET application, vm .NET application, Service Bus queue monitoring, remote monitoring" description="Узнайте, как развернуть и запустить ресурсоемкое приложение .NET на виртуальной машине Azure и использовать очереди служебной шины для удаленного мониторинга хода выполнения." metaCanonical="" services="virtual-machines" documentationCenter=".NET" title="How to run a compute-intensive task in .NET on an Azure virtual machine" authors="wpickett" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
1. На компьютере разработки создайте консольное приложение .NET, используя **TSPClient** в качестве имени проекта. Убедитесь, что установлена целевая платформа .**NET Framework 4** или ее более поздняя версия (а не **Клиентский профиль .NET Framework 4**). Вот как можно задать целевую платформу после создания проекта: В меню Visual Studio выберите пункт **Проекты**, щелкните **Свойства**, откройте вкладку **Приложение**, а затем установите значение для параметра **Целевая платформа**.
...
3. Скопируйте **TSPClient.exe** и Microsoft.ServiceBus.dll, которые находятся в папке **bin** проекта TSPClient, в папку c:\TSP.
