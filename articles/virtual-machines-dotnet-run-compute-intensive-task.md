<properties urlDisplayName="Compute Intensive .NET Task" pageTitle="Ресурсоемкая задача .NET на виртуальной машине &mdash; Azure" metaKeywords="deploying compute .NET application, vm .NET application, Service Bus queue monitoring, remote monitoring" description="Узнайте, как развернуть и запустить ресурсоемкое приложение .NET на виртуальной машине Azure и использовать очереди служебной шины для удаленного мониторинга хода выполнения." metaCanonical="" services="virtual-machines" documentationCenter=".NET" title="Выполнение ресурсоемкой задачи в .NET на виртуальной машине Azure" authors="wpickett" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
![Средство поиска решения для задачи коммивояжера][Средство поиска решения для задачи коммивояжера]

Ниже приведен пример приложения .NET, отслеживающего выполнение ресурсоемкой задачи:

![Клиент задачи коммивояжера][Клиент задачи коммивояжера]
...
1.  Выполните вход в [портал управления][портал управления Azure].
...
3.  В нижней части портала управления нажмите кнопку **Создать**.
    ...
    ![Создание новой шины Service Bus][Создание новой шины Service Bus]
...
4.  В диалоговом окне **Создать пространство имен** введите имя пространства имен. Система немедленно проверяет, доступно ли это имя, так как оно должно быть уникальным.
    ...
    ![Диалоговое окно "Создать пространство имен"][Диалоговое окно "Создать пространство имен"]
...
5.  Проверив доступность имени пространства имен, выберите регион, в котором будет размещаться пространство имен (убедитесь, что используется тот же регион, где размещается виртуальная машина).
    ...
    **Важно!**
    Выберите **тот же регион**, который вы используете или собираетесь использовать для своей виртуальной машины. Это обеспечит наилучшую производительность.
...
7.  Щелкните значок галочки. Теперь система создает пространство имен службы и включает его. Возможно, вам придется подождать несколько минут, пока система выделит ресурсы для вашей учетной записи.
    ...
    ![Снимок экрана с элементом создания][Снимок экрана с элементом создания]
...
1.  В левой области навигации щелкните узел **Service Bus**, чтобы
    отобразить список доступных пространств имен:
    ![Снимок экрана доступных пространств имен][Снимок экрана доступных пространств имен]
2.  Выберите из отображенного списка пространство имен, которое вы только что создали:
    ![Снимок экрана списка пространств имен][Снимок экрана списка пространств имен]
3.  Нажмите кнопку **Ключ доступа**.
    ![Кнопка «Ключ доступа»][Кнопка «Ключ доступа»]
...
1.  На компьютере для разработки (который не обязательно должен быть созданной вами виртуальной машиной) скачайте [пакет Azure SDK для .NET][пакет Azure SDK для .NET].
2.  Создайте консольное приложение .NET с проектом, имеющим имя **TSPSolver**. Убедитесь, что установлена целевая платформа **NET Framework 4** (а не **Клиентский профиль .NET Framework 4**). Вот как можно задать целевую платформу после создания проекта: В меню Visual Studio выберите пункт **Проекты**, щелкните **Свойства**, откройте вкладку **Приложение**, а затем установите значение для параметра **Целевая платформа**.
...
1.  На компьютере для разработки создайте консольное приложение .NET, используя **TSPClient** в качестве имени проекта. Убедитесь, что установлена целевая платформа **NET Framework 4** (а не **Клиентский профиль .NET Framework 4**). Вот как можно задать целевую платформу после создания проекта: В меню Visual Studio выберите пункт **Проекты**, щелкните **Свойства**, откройте вкладку **Приложение**, а затем установите значение для параметра **Целевая платформа**.
...
6.  Вам необходимо создать очередь Service Bus перед запуском перестановок средства поиска решения для задачи коммивояжера. Выполните следующую команду, чтобы создать очередь Service Bus:
...
Для создания или удаления очереди вместо TSPSolver можно использовать [портал управления Azure][портал управления Azure]. Посетите раздел Service Bus портала управления для доступа к пользовательским интерфейсам для создания или удаления очереди, а также для получения строки подключения, издателя и ключа доступа. Можно также использовать панель мониторинга очередей Service Bus, что позволяет просматривать метрики для входящих и исходящих сообщений.

  [Средство поиска решения для задачи коммивояжера]: ./media/virtual-machines-dotnet-run-compute-intensive-task/WA_dotNetTSPSolver.png
  [Клиент задачи коммивояжера]: ./media/virtual-machines-dotnet-run-compute-intensive-task/WA_dotNetTSPClient.png
  [портал управления Azure]: https://manage.windowsazure.com
  [Создание новой шины Service Bus]: ./media/virtual-machines-dotnet-run-compute-intensive-task/ServiceBusCreateNew.png
  [Диалоговое окно "Создать пространство имен"]: ./media/virtual-machines-dotnet-run-compute-intensive-task/CreateNameSpaceDialog.png
  [Снимок экрана с элементом создания]: ./media/virtual-machines-dotnet-run-compute-intensive-task/ClickCreate.png
  [Снимок экрана доступных пространств имен]: ./media/virtual-machines-dotnet-run-compute-intensive-task/AvailableNamespaces.png
  [Снимок экрана списка пространств имен]: ./media/virtual-machines-dotnet-run-compute-intensive-task/NamespaceList.png
  [Кнопка «Ключ доступа»]: ./media/virtual-machines-dotnet-run-compute-intensive-task/AccessKey.png
  [пакет Azure SDK для .NET]: http://www.windowsazure.com/ru-ru/develop/net/
