<properties pageTitle="Ресурсоемкая задача .NET на виртуальной машине - Azure" description="Узнайте, как развернуть и запустить ресурсоемкое приложение .NET на виртуальной машине Azure и использовать очереди служебной шины для удаленного мониторинга хода выполнения." services="virtual-machines" documentationCenter=".net" authors="" manager="wpickett" editor="mollybos"/>
...
3.  В нижней части портала управления щелкните **Создать**.
...
7. Щелкните значок галочки. Система создаст ваше пространство имен службы и активирует его. Вероятно, придется подождать несколько минут, пока система не выделит ресурсы для вашей учетной записи.
...
4.  В диалоговом окне найдите записи **Издатель по умолчанию** и **Ключ по умолчанию**. Запишите эти значения, так как эти данные будут использованы ниже для выполнения операций с пространством имен. 

<h2>Создание приложения .NET, выполняющего ресурсоемкую задачу</h2>

1. На компьютере для разработки (который не обязательно должен быть созданной вами виртуальной машиной) скачайте [пакет Azure SDK для .NET](http://www.windowsazure.com/ru-ru/develop/net/).
2. Создайте консольное приложение .NET с проектом **TSPSolver**. Убедитесь, что установлена целевая платформа **.NET Framework 4** или ее более поздняя версия (а не **клиентский профиль .NET Framework 4**). Вот как можно задать целевую платформу после создания проекта: В меню Visual Studio выберите пункт **Проекты**, щелкните **Свойства**, откройте вкладку **Приложение**, а затем установите значение для параметра **Целевая платформа**.
3. Добавьте библиотеку Microsoft ServiceBus. В обозревателе решений Visual Studio щелкните правой кнопкой мыши **TSPSolver**, выберите **Добавить ссылку**, щелкните вкладку **Обзор**, перейдите к пакету SDK Azure .NET (например, в папку **C:\Program Files\Microsoft SDKs\Azure\.NET SDK\v2.5\ToolsRef**) и укажите в качестве ссылки **Microsoft.ServiceBus.dll**.
...
1. На компьютере для разработки создайте консольное приложение .NET, используя **TSPClient** в качестве имени проекта. Убедитесь, что установлена целевая платформа **.NET Framework 4** или ее более поздняя версия (а не **клиентский профиль .NET Framework 4**). Вот как можно задать целевую платформу после создания проекта: В меню Visual Studio выберите пункт **Проекты**, щелкните **Свойства**, откройте вкладку **Приложение**, а затем установите значение для параметра **Целевая платформа**.
2. Добавьте библиотеку Microsoft ServiceBus. В обозревателе решений Visual Studio щелкните правой кнопкой мыши **TSPSolver**, выберите **Добавить ссылку**, щелкните вкладку **Обзор**, перейдите к пакету SDK Azure .NET (например, в папку **C:\Program Files\Microsoft SDKs\Azure\.NET SDK\v2.5\ToolsRef**) и укажите в качестве ссылки **Microsoft.ServiceBus.dll**.
...
1. Войдите на компьютер, где будет выполняться клиентское приложение. Это не обязательно должен быть именно компьютер с приложением **TSPSolver**.
