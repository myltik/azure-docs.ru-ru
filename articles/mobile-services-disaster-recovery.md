...
+ **Создание резервной копии данных в базе данных SQL мобильных служб Azuree**
...
+ **Создание резервной копии скриптов мобильных служб**
...
+ Состояние мобильной службы на [портале Azure] отображается как **Неработоспособная**.
+ Баннер **Неработоспособная** отображается в верхней части каждой вкладки мобильные службы на портале Azure, и операции управления создают сообщения об ошибках.
...
3. Настройте программы командной строки Azure для работы с подпиской, как описано в разделе [Автоматизация мобильных служб с помощью программ командной строки].

4. Теперь вы можете использовать дополнительную службу для восстановления своей основной службы.

    > [AZURE.IMPORTANT] При выполнении команды на этом шаге дополнительная служба удаляется таким образом, чтобы ее емкость могла использоваться для восстановления основной службы. Мы рекомендуем создавать резервные копии сценариев и параметров перед выполнением данной команды, если вы хотите их сохранить.
...
6. Убедитесь в том, что восстановленная служба взаимодействует с базой данных SQL Azure. Команда восстановления восстанавливает услуг мобильную службу, но сохраняет соединение с исходной базой данных. Если проблема в основной области Azure также влияет на базу данных, восстановленная служба может по-прежнему не выполняться правильно. Панель мониторинга служб Azure можно использовать для проверки состояния базы данных для данной области. Если исходная база данных не работает, ее можно восстановить:
	+ Восстановите базу данных SQL Azure в области Azure, где вы только что восстановили свою мобильную службу, как описано в [руководстве по обеспечению непрерывности бизнеса на основе базы данных SQL]
...
[функции управления версиями]: http://www.windowsazure.com/develop/mobile/tutorials/store-scripts-in-source-control/
[с помощью средства командной строки Azure]: http://www.windowsazure.com/develop/mobile/tutorials/command-line-administration/
...
[Автоматизация мобильных служб с помощью программ командной строки]: http://www.windowsazure.com/develop/mobile/tutorials/command-line-administration/
