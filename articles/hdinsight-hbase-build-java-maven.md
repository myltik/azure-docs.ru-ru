<properties 
	pageTitle="Создание приложения HBase с помощью Maven" 
	description="Узнайте, как использовать Apache Maven для создания Java-приложения Apache HBase и его последующего развертывания в Azure HDInsight" 
	services="hdinsight" 
	documentationCenter="" 
	authors="blackmist" 
	manager="paulettm" 
...
2. Используйте команду __mvn__, которая будет установлена вместе с Maven, для формирования шаблона проекта.

		mvn archetype:generate -DgroupId=com.microsoft.examples -DartifactId=hbaseapp -DarchetypeArtifactId=maven-archetype-quickstart -DinteractiveMode=false

	При этом в текущем каталоге будет создан новый каталог с именем, указанным в параметре __artifactID__ (в данном случае **hbaseapp**). В этом каталоге будут находиться следующие элементы:
	...
	* __src__ - каталог, содержащий каталог __main\java\com\microsoft\examples__, в котором будет создаваться приложение.
...
1. Откройте для редактирования файл __pom.xml__ и добавьте следующие строки в раздел `<dependencies>`.
	...
	Они указывают Maven, что для проекта требуется __hbase-client__ версии __0.98.4-hadoop2__. При компиляции он будет загружено из репозитория Maven по умолчанию. Вы можете воспользоваться [поиском по репозиторию Maven](http://search.maven.org/#artifactdetails%7Corg.apache.hbase%7Chbase-client%7C0.98.4-hadoop2%7Cjar), чтобы получить дополнительную информацию об этой зависимости.
...
1. Перейдите в каталог __hbaseapp\src\main\java\com\microsoft\examples__ и переименуйте файл app.java__ в __CreateTable.java__.
...
3. После завершения выполнения команды в каталоге __hbaseapp\target__ будет находиться файл с именем __hbaseapp-1.0-SNAPSHOT.jar__.
	...
	> [AZURE.NOTE] Файл __hbaseapp-1.0-SNAPSHOT.jar__ относится к типу uberjar (другое название - fatjar) и содержит все зависимости, необходимые для работы приложения.
...
1. После установки и настройки [Azure PowerShell](install-configure-powershell.md) создайте новый файл с именем __hbase-runner.psm1__. Используйте следующее в качестве содержимого этого файла:
	...
	* __Add-HDInsightFile__ - используется для передачи файлов в HDInsight;
...
3. Откройте окно Azure PowerShell, измените каталоги на каталог __hbaseapp__, затем выполните следующую команду.
	...
	Измените путь на место расположения созданного ранее файла __hbase-runner.psm1__. При этой модуль будет зарегистрирован для текущего сеанса PowerShell.

2. Воспользуйтесь следующей командой, чтобы загрузить __hbaseapp-1.0-SNAPSHOT.jar__ на свой кластер HDInsight.
...
3. После передачи файлов используете следующую команду для создания новой таблицы с __hbaseapp__.

		Start-HBaseExample -className com.microsoft.examples.CreateTable -clusterName hdinsightclustername

	Замените __hdinsightclustername__ на имя своего кластера HDInsight.

	__Эта команда создаст новую таблицу с именем __ в кластере HDInsight. Эта команда не отображает какие-либо выходные данные в окне консоли.
...
2. Для осуществления поиска по записям таблицы используйте следующую команду.
	...
	Будет использован класс SearchByEmail для поиска всех строк, у которых значение столбцов family __contactinformation__ - __email__ - содержит строку __contoso.com__. Вы получите следующие результаты:
	...
	При использовании __fabrikam.com__ для значения `-emailRegex` возвращается список пользователей, у которых имеется строка __fabrikam.com__ в поле электронной почты. Так как поиск реализован на базе фильтра регулярных выражений, также можно использовать регулярные выражения, например __^r__, в этом случае будут возвращены записи, у которых электронная почта начинается с буквы "r".

##Удаление таблицы

После завершения работы с примером используйте следующую команду в сеансе PowerShell для удаления таблицы __people__, использовавшейся в данном примере.
