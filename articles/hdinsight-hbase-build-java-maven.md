...
Вы узнаете, как создавать и собирать приложение [Apache HBase][Apache HBase] на Java с использованием Apache Maven. Затем приложение будет использоваться с Azure HDInsight (Hadoop).

[Maven][Maven] — это средство для управления и повышения обозримости проектов программного обеспечения, позволяющее создавать ПО, документацию и отчеты для проектов Java Из данной статьи вы узнаете, как использовать его для создания базового приложения Java, которое создает, формирует запросы и удаляет таблицу HBase в кластере Azure HDInsight.

## Требования

-   [Пакет JDK][Пакет JDK] 7 или выше для платформы Java

-   [Maven][Maven]

-   [Кластер Azure HDInsight с HBase][Кластер Azure HDInsight с HBase]
...
3.  Удалите файл **src\\test\\java\\com\\microsoft\\examples\\apptest.java**, он не используется в этом примере.
...
1.  Откройте для редактирования файл **pom.xml** и добавьте следующие строки в раздел `<dependencies>`.
    ...
    Они указывают Maven, что для проекта требуется **hbase-client** версии **0.98.4-hadoop2**. При компиляции он будет загружено из репозитория Maven по умолчанию. Можно воспользоваться [поиском по репозиторию Maven][поиском по репозиторию Maven], чтобы получить дополнительную информацию об этой зависимости.
...
4.  Создайте новый каталог с именем **conf** в каталоге **hbaseapp**. В каталоге **conf** создайте новый файл с именем **hbase-site.xml** и используйте следующее в качестве содержимого этого файла:
    ...
    > [WACOM.NOTE] Это минимально возможный файл hbase-site.xml, содержащий лишь самые минимальные настройки для кластера HDInsight. Чтобы получить полную версию файла конфигурации hbase-site.xml, используемого HDInsight, [подключитесь с помощью удаленного рабочего стола к кластеру HDInsight][подключитесь с помощью удаленного рабочего стола к кластеру HDInsight] и найдите файл hbase-site.xml в каталоге C:\\apps\\dist\\hbase-\<номер\_версии\>-hadoop2\\conf. Часть пути с номером версии изменяется при обновлении версии HBase в кластере.

5.  Сохраните файл **hbase-site.xml**.

## Создание приложения

1.  Перейдите в каталог **hbaseapp\\src\\com\\microsoft\\examples** и переименуйте файл app.java\_\_ в **CreateTable.java**.
...
4.  В каталоге **hbaseapp\\com\\microsoft\\examples** создайте новый файл с именем **SearchByEmail.java**. Используйте следующее в качестве содержимого этого файла:
...
6.  В каталоге **hbaseapp\\com\\microsoft\\examples** dсоздайте новый файл с именем **DeleteTable.java**. Используйте следующее в качестве содержимого этого файла:
...
7.  Сохраните файл **CreateTable.java**.
...
> [WACOM.NOTE] Существует множество способов загрузки файла в ваш кластер HDInsight, они описаны в документе [Отправка данных для заданий Hadoop в HDInsight][Отправка данных для заданий Hadoop в HDInsight]. Для описанных ниже действий используется [Azure PowerShell][Azure PowerShell].

1.  После установки и настройки [Azure PowerShell][Azure PowerShell] создайте новый файл с именем **hbase-runner.psm1**. Используйте следующее в качестве содержимого этого файла:
...
4.  Воспользуйтесь следующей командой, чтобы загрузить **hbaseapp-1.0-SNAPSHOT.jar** на ваш кластер HDInsight.

        Add-HDInsightFile -localPath target\hbaseapp-1.0-SNAPSHOT.jar -destinationPath example/jars/hbaseapp-1.0-SNAPSHOT.jar -clusterName hdinsightclustername

    Замените **hdinsightclustername** на имя своего кластера HDInsight. Затем команда загрузит **hbaseapp-1.0-SNAPSHOT.jar** в каталог **example/jars**, расположенный на основном хранилище для вашего кластера HDInsight.

5.  После загрузки файлов используете следующую команду для создания новой таблицы с **hbaseapp**

        Start-HBaseExample -className com.microsoft.examples.CreateTable -clusterName hdinsightclustername

    Замените **hdinsightclustername** на имя своего кластера HDInsight.

    Эта команда создаст новую таблицу с именем **people** в кластере HDInsight.

6.  Для осуществления поиска по записям таблицы используйте следующую команду.

        Start-HBaseExample -className com.microsoft.examples.SearchByEmail -clusterName hdinsightclustername -emailRegex contoso.com

    Замените **hdinsightclustername** на имя своего кластера HDInsight.

    Будет использован класс SearchByEmail для поиска всех строк, у которых значение столбцов family **contactinformation**, **email** содержит строку **contoso.com**. Вы получите следующие результаты:
...
### Отсутствуют или получены неожиданные результаты при использовании **Start-HBaseExample**

Используйте параметр `-showErr` для просмотра STDERR, выдаваемого при выполнении задания.

  [Apache HBase]: http://hbase.apache.org/
  [Maven]: http://maven.apache.org/
  [Пакет JDK]: http://www.oracle.com/technetwork/java/javase/downloads/index.html
  ...
  [поиском по репозиторию Maven]: http://search.maven.org/#artifactdetails%7Corg.apache.hbase%7Chbase-client%7C0.98.4-hadoop2%7Cjar
  [maven-shade-plugin]: http://maven.apache.org/plugins/maven-shade-plugin/
  [подключитесь с помощью удаленного рабочего стола к кластеру HDInsight]: http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-administer-use-management-portal/#rdp
