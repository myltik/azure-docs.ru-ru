...
5. В некоторых случаях в запросе аутентификации необходимо передать идентификатор клиента. Идентификатор клиента можно узнать, выбрав **Просмотреть конечные точки** и отобразив идентификатор, как показано ниже.
...
Необходимо назначить приложение роли, чтобы предоставить ему разрешения для выполнения действий. Можно использовать [портал предварительной версии](https://portal.azure.com), чтобы назначить приложение Active Directory роли с соответствующими разрешениями.

Чтобы приступить к работе с контролем доступа на портале предварительной версии, щелкните значок **Доступ**.

![Выбор пользователей](./media/resource-group-create-service-principal-portal/select-users.png)

Выберите роль, которую хотите назначить приложению, и выполните поиск приложения.
...
- Подробнее о настройке политик безопасности см. в разделе [Управление доступом к ресурсам и его аудит](resource-group-rbac.md).  
