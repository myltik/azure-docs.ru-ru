<properties
   pageTitle="Создание приложения Active Directory на портале | Microsoft Azure"
...
# Создание приложения Active Directory с доступом к ресурсам с помощью портала
...
Пояснения к терминам Active Directory см. в статье [Объекты приложений и объекты участников-служб](./active-directory/active-directory-application-objects.md). Дополнительные сведения об аутентификации Active Directory см. в статье [Сценарии проверки подлинности в Azure AD](./active-directory/active-directory-authentication-scenarios.md).
...
5. Выберите тип приложения, которое вы хотите создать. Чтобы открыть инструкции, выберите пункт **Добавить приложение, разрабатываемое моей организацией**.

     ![новое приложение](./media/resource-group-create-service-principal-portal/what-do-you-want-to-do.png)

6. Укажите имя и тип приложения. Для работы с данным учебником создайте **ВЕБ-ПРИЛОЖЕНИЕ И/ИЛИ ВЕБ-API** и нажмите кнопку "Далее". При выборе параметра **СОБСТВЕННОЕ КЛИЕНТСКОЕ ПРИЛОЖЕНИЕ** дальнейшие действия будут отличаться от описанных в статье.
...
2. Скопируйте **ИДЕНТИФИКАТОР КЛИЕНТА**.
  
     ![идентификатор клиента](./media/resource-group-create-service-principal-portal/client-id.png)

3. Если приложение выполняется под собственными учетными данными, прокрутите содержимое до раздела **Ключи** и выберите срок действия пароля.
...
При программном входе в запросе проверки подлинности необходимо передавать идентификатор клиента. Чтобы получить идентификатор клиента для веб-приложений и приложений веб-API, внизу экрана нужно выбрать **Просмотр конечных точек** и найти идентификатор, как показано ниже.
...
3. В раскрывающемся списке делегированных разрешений выберите пункт **Доступ к управлению службами Azure в качестве организации**.
...
Вы готовы журнал к программному входу в приложение.

- Примеры для .NET см. в статье [Пакет SDK .NET для диспетчера ресурсов Azure](resource-manager-net-sdk.md).
- Примеры для Java см. в статье [Пакет SDK Java для диспетчера ресурсов Azure](resource-manager-java-sdk.md).
- Примеры для Python см. в статье [Проверка подлинности при управлении ресурсов для Python](https://azure-sdk-for-python.readthedocs.io/en/latest/resourcemanagementauthentication.html).
- Примеры для REST см. в статье [API REST диспетчера ресурсов](resource-manager-rest-api.md).
