<properties 
   pageTitle="Настройка служб, необходимых для публикации облачной службы из Visual Studio"
...
Чтобы опубликовать облачную службу в Azure, сначала необходимо создать облачную службу, которая выполняет роли в среде Azure. Облачную службу можно создать на портале управления Azure. Как это сделать, описано [здесь](#to-create-a-cloud-service-by-using-the-azure-management-portal). Ее также можно создать в Visual Studio с помощью мастера публикации.
...
### Создание облачной службы на портале управления Azure

1. Войдите на [портал управления Azure](http://go.microsoft.com/fwlink/?LinkId=253103) на веб-сайте Майкрософт.
...
Учетная запись хранения предоставляет доступ к службам BLOB-объектов, очередей и таблиц. Создать учетную запись хранения можно с помощью Visual Studio или [портала управления Azure](http://go.microsoft.com/fwlink/?LinkId=253103).
...
1. Когда все будет готово, нажмите **Создать**. Новая учетная запись хранения появится в списке **Хранилище** обозревателя решений.

### Создание учетной записи хранения на портале управления Azure
...
1. В списке **Конфигурация службы** выберите имя конфигурации службы, которую требуется изменить. Если вы хотите изменить все конфигурации служб для этой роли, выберите **Все конфигурации**. Дополнительные сведения об изменении конфигураций служб см. в разделе [Управление строками подключения для учетных записей хранения](https://msdn.microsoft.com/library/azure/8cda8963-ef0e-4f64-8d29-5eac467e5f53#ConnectionStrings).
...
1. В списке **Имя учетной записи** выберите имя своей учетной записи хранения. Инструменты Azure автоматически получат учетные данные хранилища (используется файл PUBLISHSETTINGS). Чтобы указать учетные данные хранилища вручную, выберите параметр **Учетные данные, введенные вручную** и перейдите к следующему шагу. Имя учетной записи хранения и первичный ключ можно получить на [портале управления](http://go.microsoft.com/fwlink/p/?LinkID=213885). Если вы не хотите указывать параметры учетной записи хранения вручную, нажмите кнопку **ОК**, чтобы закрыть диалоговое окно.

1. Выберите ссылку **Ввод учетных данных хранилища**.

1. В поле **Имя учетной записи** введите имя своей учетной записи хранения.

    >[AZURE.NOTE]Войдите на портал управления и нажмите кнопку **Хранилище**. На портале отобразится список учетных записей хранения. Выбрав учетную запись, вы попадете на ее страницу, где можно скопировать имя записи. В предыдущей версии портала управления имя учетной записи хранения можно найти в представлении **Учетные записи хранения**. Чтобы скопировать это имя, выделите его в окне **Свойства** и нажмите клавиши CTRL+C. Чтобы вставить скопированное имя в Visual Studio, щелкните в текстовом поле **Имя учетной записи** и нажмите клавиши CTRL+V.

1. В поле **Ключ учетной записи** укажите свой первичный ключ. Его можно ввести вручную или скопировать и вставить из [портала управления](http://go.microsoft.com/fwlink/?LinkID=213885). Чтобы скопировать этот ключ на портале управления, сделайте вот что:
