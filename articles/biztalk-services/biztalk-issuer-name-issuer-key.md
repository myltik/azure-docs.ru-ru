...
> [AZURE.IMPORTANT]Если при создании пространства имен службы контроля доступа выбран параметр <strong>Active Directory</strong>, удостоверение службы **не** создается автоматически. При подготовке службы BizTalk автоматически создаются пространство имен контроля доступа, удостоверение службы «Владелец» (имя издателя), пароль (ключ издателя) и симметричный ключ.<br /> В разделе [Инструкция по использованию службы управления ACS для настройки удостоверений службы](http://go.microsoft.com/fwlink/p/?LinkID=303942) представлены дополнительные данные об удостоверениях службы контроля доступа.
