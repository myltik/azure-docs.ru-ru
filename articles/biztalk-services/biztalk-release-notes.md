...
"Не удается скопировать файл <путь к DLL-файлу> в bin\\Debug\\FileName.dll". Процесс не может получить доступ к файлу «bin\\Debug\\FileName.dll», так как он используется другим процессом».
...
Вы увидите, что несмотря на то, что имя соответствующего файла .brigeconfig изменено на новое, в области конструктора имя сущности будет прежним. Если вы попытаетесь открыть конфигурацию моста, дважды щелкнув компонент моста, возникнет следующая ошибка: "  
Файл <старое имя>.bridgeconfig, связанный с сущностью <старое имя>, не существует".  
Чтобы избежать этой ситуации, сохраняйте изменения после переименования сущностей в проекте службы BizTalk.
