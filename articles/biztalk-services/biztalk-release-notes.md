...
* AS2 поддерживает новые алгоритмы MIC для подписи сообщений и новые алгоритмы шифрования. См. раздел [Создание соглашения AS2 в службах Azure BizTalk](https://msdn.microsoft.com/library/azure/hh689890.aspx).
## Известные проблемы.
...
Построение решения в Visual Studio выполняется успешно. Затем вы выполняете повторное построение решения или очищаете его. После этого при попытке перестроения или очистки возникает следующая ошибка: "Не удается скопировать файл <путь к DLL-файлу> в bin\\Debug\\FileName.dll". Процесс не может получить доступ к файлу «bin\\Debug\\FileName.dll», так как он используется другим процессом».
...
**Возможное решение**. Эту проблему можно обойти, удалив TBD-файлы в папке <системный диск>: \\Users<пользователь>\\AppData\\Local\\Microsoft\\VisualStudio\\11.0.
