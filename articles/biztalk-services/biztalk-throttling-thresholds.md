...
||Описание|Нижнее пороговое значение|Верхнее пороговое значение| |---|---|---|---| |Память|% от общей доступной системной памяти/байты файла подкачки. <p><p>Общий объем доступных байтов файла подкачки приблизительно в 2 раза больше ОЗУ в системе. |60%|70%| |Обработка сообщения|Количество одновременно обрабатываемых сообщений|40 * количество ядер|100 * количество ядер|
