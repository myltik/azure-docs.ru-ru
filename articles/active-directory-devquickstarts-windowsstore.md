...
# Вызов веб-интерфейса API из приложения Windows Phone
...
Классическим приложениям для Магазина Windows, которым необходим доступ к защищенным ресурсам, Azure AD предоставляет библиотеку проверки подлинности Active Directory (ADAL). Единственная задача ADAL — упрощение процесса получения приложением маркеров доступа. Чтобы продемонстрировать, насколько простой является данная процедура, мы выполним сборку приложения DirectorySearcher для Магазина Windows, которое:
...
-	Выберите клиента для регистрации приложения.
...
- Теперь найдите метод `Search(...)`, который будет вызываться при нажатии кнопки "Поиск" в пользовательском интерфейсе приложения. Этот метод отправляет запрос GET в интерфейс Graph API Azure AD для поиска пользователей, чьи имена UPN начинаются с заданного искомого значения. Но для отправки запросов в Graph API необходимо включить access_token в заголовок `Authorization` запроса — именно отсюда ADAL начинает свою работу.
...
[Защита веб-интерфейса API .NET с помощью Azure AD >>](active-directory-devquickstarts-webapi-dotnet.md)
