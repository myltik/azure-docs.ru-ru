<properties urlDisplayName="details" pageTitle="Сведения о Video Center" metaKeywords ="" description="" metaCanonical="" services="" documentationCenter="" title="How to Use ASP.NET Web Forms Output Caching with Azure Websites" authors="sdanie" solutions="" manager="wpickett" editor="" />
...
>После того как экземпляр службы управляемого кэша создан командлетом PowerShell, его можно просмотреть и настроить на [портале управления Azure][].
...
Вызовите командлет [New-AzureManagedCache][] и укажите имя, регион, предложение кэша и размер кэша.
...
Выберите **Sku** и **Объем памяти**, удовлетворяющий требованиям вашего приложения. Обратите внимание, что некоторые возможности кэша, например уведомления и высокая доступность, предоставляются только в некоторых предложениях. Дополнительную информацию о выборе предложения кэша и его размера, которые лучше всего подходят для вашего приложения, см. в разделе [Предложения кэша][].
...
После вызова командлета PowerShell cоздание кэша может занять несколько минут. После создания новый кэш имеет состояние `Запущен` и готов для использования со значениями по умолчанию, его можно просмотреть и настроить на [портале управления Azure][].
...
[Командлеты управляемого кэша Azure]: http://go.microsoft.com/fwlink/?LinkID=398555
