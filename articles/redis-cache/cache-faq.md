...
ConnectRetry|Количество повторных попыток подключения во время первоначального подключения.| Рекомендации приведены ниже. |
ConnectTimeout|Время ожидания в миллисекундах для операций подключения.| Рекомендации приведены ниже. |
...
-	**Значения времени ожидания**
	...
	-	Установите для `AbortOnConnectFail` значение false и позвольте StackExchange.Redis восстанавливать соединение.
...
Одним из основных преимуществ Redis является то, что существует множество клиентов, поддерживающих множество языков разработки. Текущий список клиентов Redis см. по [этой ссылке](http://redis.io/clients). Учебники по нескольким различных языкам и клиентам см. в статье [Как использовать кэш Redis для Azure](cache-dotnet-how-to-use-azure-redis-cache.md). Выберите нужный язык, используя переключатель языков в верхней части статьи.
...
-	Щелкните **Новый запрос на поддержку**, чтобы создать запрос по своему кэшу.
