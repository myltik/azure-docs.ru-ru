...
-	**Максимальное число подключений клиентов**: уровень Премиум обеспечивает максимальное число клиентов, которые могут подключаться к Redis, причем чем больше размер кэша, тем больше число подключений. [Подробные сведения приведены на странице с ценами](TODO).
...
Из этой таблицы можно сделать следующие выводы. — Пропускная способность для кэша одинакового размера для уровня Премиум выше, чем для уровня Стандартный. Например, для кэша размером 6 ГБ пропускная способность P1 — 140 тысяч запросов в секунду, а пропускная способность С3 — 49 тысяч запросов в секунду. — С кластеризацией Redis пропускная способность увеличивается по мере увеличения числа сегментов (узлов) в кластере. Например, если создать кластер P4 из 10 сегментов, то доступная пропускная способность составит 250 тысяч * 10 = 2,5 миллиона запросов в секунду — пропускная способность для большего размера ключа для уровня Премиум выше, чем для уровня Стандартный.
