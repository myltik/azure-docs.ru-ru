...
Для создания кэша войдите на [портал предварительной версии Microsoft Azure][], щелкните**Создать** > **Данные + хранилище** > **Кэш Redis**.
...
Используйте **Уровень цен** для выбора желаемого размера и компонентов кэша. Для кэша уровня **Basic** предлагается один узел с различными размерами (до 53 ГБ). Для кэша уровня **Standard** предлагается конфигурация на базе двух узлов (основной и реплика) с соглашением об уровне обслуживания 99,9 % и различными вариантами размеров до 53 ГБ.
...
Для подключения к кэшу Redis для Azure и получения экземпляра подключенного класса `ConnectionMultiplexer` вызовите статический метод `Connect` и передайте ему конечную точку кэша и ключ, как показано в примере ниже. Используйте в качестве параметра пароля ключ Azure, созданный на портале.
...
Задайте для атрибутов значения из колонки своего кэша на портале и настройте другие параметры.
