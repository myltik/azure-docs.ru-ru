<properties 
   pageTitle="Поставщик состояний сеансов ASP.NET кэша"
...
![Управление пакетами NuGet с помощью кэша Redis для Azure](./media/cache-asp.net-session-state-provider/IC729541.png)
...
Задайте для атрибутов значения из колонки своего кэша на портале предварительной версии Microsoft Azure и настройте другие параметры по своему усмотрению. Инструкции по доступу к свойствам кэша см. в разделе [Настройка параметров кэша Redis](cache-configure.md#configure-redis-cache-settings).
...
- Распределенный в памяти поставщик состояний сеансов, например поставщик состояний сеансов для кэша Redis, обладает преимуществами предыдущих двух поставщиков. Веб-приложение может использовать простой, быстрый и масштабируемый поставщик состояний сеансов. Но необходимо учитывать, что этот поставщик сохраняет состояние сеанса в кэше, поэтому ваше приложение должно принимать в расчет все характеристики, связанные с распределенным кэшем в памяти, например временные неполадки в сети. Рекомендации по использованию кэша см. в статье [Руководство по кэшированию](https://github.com/mspnp/azure-guidance/blob/master/Caching.md) из коллекции шаблонов и рекомендаций Майкрософт [Руководство по разработке и реализации облачного приложения Azure](https://github.com/mspnp/azure-guidance).
