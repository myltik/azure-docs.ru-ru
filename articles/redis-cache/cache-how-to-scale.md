...
>
>-	Нельзя выполнить масштабирование кэша с уровня **Standard** до уровня **Basic**.
>-	Выполнить масштабирование кэша с уровня **Basic** до уровня **Standard** возможно, но одновременно изменить размер нельзя. Если требуется изменить размер, можно выполнить последующую операцию масштабирования до нужного размера.
>-	Выполнить масштабирование от большего размера к **C0 (250 МБ)** невозможно.
...
При масштабировании кэша уровня **Basic** до другого размера кэш отключается и подготавливается новый кэш с новым размером. В течение этого времени кэш недоступен и все данные в кэше теряются.

При масштабировании кэша уровня **Basic** до уровня **Standard** подготавливается реплика кэша и данные копируются из основного кэша в реплику кэша. Кэш остается доступным в процессе масштабирования.

При масштабировании кэша уровня **Standard** до другого размера одна из реплик отключается и подготавливается повторно в новом размере, после чего в нее передаются данные. Затем другая реплика выполняет отработку отказа перед повторной подготовкой. Процесс отработки отказа аналогичен отработке отказа, возникающей при сбое одного из узлов кэша.

## Сохранятся ли данные кэша при масштабировании?

При масштабировании кэша уровня **Basic** до другого размера теряются все данные. Во время операции масштабирования кэш недоступен.
...
Кэши уровня **Basic** находятся в автономном режиме во время операций масштабирования до другого размера, но остаются доступными при масштабировании с уровня **Basic** до уровня **Standard**.
