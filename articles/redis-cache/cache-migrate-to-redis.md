...
>[AZURE.NOTE] Чтобы использовать кэш Redis для Azure, необходима учетная запись Azure. Если ее нет, вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=redis_cache_hero).
