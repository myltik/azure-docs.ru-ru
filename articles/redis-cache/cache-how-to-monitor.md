...
Чтобы просмотреть метрики кэша, [перейдите](https://msdn.microsoft.com/library/azure/cbe6d113-7bdc-4664-a59d-ff0df6f4e214#CacheSettings) к своему экземпляру кэша на [портале предварительной версии Azure](https://portal.azure.com). Метрики экземпляров кэша Redis для Azure доступны в колонке **Кэш Redis**.

![Монитор][redis-cache-monitor-overview]

>[AZURE.IMPORTANT]Если на портале предварительной версии отображается показанное ниже сообщение, включите диагностику кэша с помощью действий, описанных в разделе [Включение диагностики кэша](#enable-cache-diagnostics).
>
...
В кэше Redis для Azure можно настроить сохранение диагностических данных в учетной записи хранения, после чего с ними можно непосредственно работать с использованием любого подходящего инструмента. Для сбора, хранения и отображения диагностических данных кэша на портале предварительной версии Azure необходимо настроить учетную запись хранения. Для экземпляров кэша в одном регионе и в рамках одной подписки используется общая учетная запись хранения диагностических данных, и в случае изменения конфигурации новые настройки применяются ко всем кэшам в подписке, входящим в данный регион.
...
Выберите из раскрывающегося списка **Хранение (дни)** период хранения диагностических данных. Количество дней также можно ввести в текстовое поле вверху списка.
...
Просмотреть сохраненные метрики можно в таблицах в учетной записи хранения, названия которых начинаются со строки `WADMetrics`. Дополнительные сведения о доступе к сохраненным показателям из-за пределов портала предварительной версии см. в образце [Доступ к данным мониторинга кэша Redis](https://github.com/rustd/RedisSamples/tree/master/CustomMonitoring).

>[AZURE.NOTE]На портале предварительной версии отображаются только метрики, которые хранятся в выбранной учетной записи хранения. В случае ее смены данные из прежней записи можно будет скачать, однако они не отображаются на портале предварительной версии и не удаляются по истечении периода хранения.
...
| Используемая память | Объем используемой памяти кэша (в МБ) за указанный интервал отчетности. Это значение соответствует INFO-команде `used_memory` Redis. |
...
| Чтение из кэша | Объем данных (в КБ/с), считанных из кэша за указанный интервал отчетности. Это значение рассчитывается на основе трафика сетевых адаптеров, поддерживающих виртуальную машину, на которой размещен кэш, и не связано непосредственно с системой Redis. |
| Запись в кэш | Объем данных (в кБ/с), записанных в кэш за указанный интервал отчетности. Это значение рассчитывается на основе трафика сетевых адаптеров, поддерживающих виртуальную машину, на которой размещен кэш, и не связано непосредственно с системой Redis. |
