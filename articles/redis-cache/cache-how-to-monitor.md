...
Чтобы просмотреть метрики кэша, [перейдите](cache-configure.md) к своему экземпляру кэша на [портале предварительной версии Azure](https://portal.azure.com). Метрики экземпляров кэша Redis для Azure доступны в колонке **Кэш Redis**.
...
Просмотреть сохраненные метрики можно в таблицах в учетной записи хранения, названия которых начинаются со строки `WADMetrics`. Дополнительные сведения о доступе к сохраненным метрикам извне портала предварительной версии см. в примере [Доступ к данным мониторинга кэша Redis](https://github.com/rustd/RedisSamples/tree/master/CustomMonitoring).

>[AZURE.NOTE]На портале предварительной версии отображаются только метрики, которые хранятся в выбранной учетной записи хранения. В случае ее смены данные из прежней учетной записи хранения можно будет скачать, однако они не будут отображаться на портале предварительной версии.
