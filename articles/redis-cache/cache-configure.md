...
В экземплярах кэша Redis для Azure вы можете безопасно выполнять команды с помощью **консоли Redis**, доступной для кэшей категорий "Стандартный" и "Премиум".
...
>-	[Кластеризация.](cache-how-to-premium-clustering.md) Консоль Redis использует клиент redis-cli.exe, который на данный момент не поддерживает кластеризацию. Служебная программа redis-cli в ветви [нестабильных версий](http://redis.io/download) репозитория Redis в GitHub реализует базовую поддержку, если запустить ее с параметром `-c`. Дополнительные сведения см. в разделе [Эксперименты с кластером](http://redis.io/topics/cluster-tutorial#playing-with-the-cluster) [учебника по кластерам Redis](http://redis.io/topics/cluster-tutorial) на сайте [http://redis.io](http://redis.io).
...
Список команд Redis, отключенных в кэше Redis для Azure, см. в предыдущем разделе [Команды Redis, которые не поддерживаются в кэше Redis для Azure](#redis-commands-not-supported-in-azure-redis-cache). Дополнительные сведения о командах Redis см. по ссылке [http://redis.io/commands](http://redis.io/commands).
