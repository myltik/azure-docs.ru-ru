...
>[AZURE.NOTE] В настоящее время служба работоспособности ресурсов не поддерживает отправку сведений о работоспособности экземпляров кэша Redis для Azure, размещенных в виртуальной сети. Дополнительные сведения см. в разделе [Do all cache features work when hosting a cache in a VNET?](cache-how-to-premium-vnet.md#do-all-cache-features-work-when-hosting-a-cache-in-a-vnet) (Все ли функции кэша работают, когда он размещен в виртуальной сети?)
...
При возникновении во время работы кэша любого из таких условий, как интенсивное использование памяти, высокая пропускная способность сети или сервера, в колонке **Кэш Redis** отображается предупреждение.
...
В колонке **Schedule updates** (Планирование обновлений) можно указать период обслуживания для обновлений сервера кэша Redis.
...
Щелкните **Метрики Redis**, чтобы [просмотреть метрики](cache-how-to-monitor.md#how-to-view-metrics-and-customize-charts) кэша, либо **Правила оповещения**, чтобы [настроить правила оповещения](cache-how-to-monitor.md#operations-and-alerts).
...
-	Кэши уровня Premium
	...
	-   Для всех кэшей категории "Премиум" включен кластер Redis. Кластер Redis поддерживает только базу данных 0, поэтому предельное значение `databases` для любого кэша категории "Премиум" с включенным кластером Redis равно 1, а использование команды [Select](http://redis.io/commands/select) не допускается. Дополнительные сведения см. в разделе [Нужно ли вносить изменения в клиентское приложение, чтобы использовать кластеризацию?](#do-i-need-to-make-any-changes-to-my-client-application-to-use-clustering)
...
>-	Консоль Redis устанавливает новое подключение к базе данных 0 при каждой отправке команды. Нельзя использовать команду `SELECT`, чтобы выбрать другую базу данных, так как при каждом выполнении команды база данных 0 сбрасывается. Сведения о выполнении команд Redis, включая смену базы данных, см. в разделе [Как выполнять команды Redis?](cache-faq.md#how-can-i-run-redis-commands).
