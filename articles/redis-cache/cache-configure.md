...
Щелкните **Ценовая категория**, чтобы просмотреть или изменить ценовую категорию для своего кэша. Дополнительную информацию о масштабировании см. в разделе [Масштабирование кэша Redis для Azure](cache-how-to-scale.md).
...
Щелкните **Политика максимальной памяти**, чтобы настроить политики памяти для кэша. Параметр **maxmemory-policy** определяет порядок удаления данных для кэша, а параметр **maxmemory-reserved** определяет объем памяти, зарезервированный для процессов, не связанных с кэшем.
...
В разделе **Пользователи** предусматривается поддержка контроля доступа на основе ролей (RBAC) в портале предварительной версии, чтобы помочь организациям просто и точно выполнять требования к управлению доступом. Дополнительные сведения см. в разделе [Контроль доступа на основе ролей в портале предварительной версии Azure](http://go.microsoft.com/fwlink/?LinkId=512803).

Раздел **Теги** поможет организовать ресурсы. Дополнительные сведения см. в статье [Использование тегов для организации ресурсов Azure](../resource-group-using-tags.md).
...
В экземплярах кэша Redis для Azure можно безопасно выполнять команды с помощью **консоли Redis**, которая доступна для кэшей уровня Standard. Для доступа к консоли Redis щелкните **Консоль** в колонке **Кэш Redis**.
