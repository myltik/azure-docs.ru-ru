...
Прежде чем пользователи смогут видеть и использовать приложения в RemoteApp, необходимо предоставить им доступ. Это просто: на вкладке **Доступ пользователей** введите данные учетной записи пользователя, чтобы предоставить ему доступ к этой службе.
...
Azure RemoteApp поддерживает два метода развертывания: облачный и гибридный. В каждом из них для доступа пользователей к приложениям используются различные удостоверения пользователей.
...
|Сторонние поставщики удостоверений, поддерживаемые службой Azure (например, Ping) |Да |Нет|	
