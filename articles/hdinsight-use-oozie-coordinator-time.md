...
##<a id="prerequisites"></a>Предварительные условия
...
##<a id="defineworkflow"></a>Определение рабочего процесса Oozie и связанного с ним сценария HiveQL
...
1. Создайте текстовый файл со следующим содержимым:
	...
	| ${coordStart}     | Время начала задания. |
	...
	| ${wfPath}         | Путь к workflow.xml.  Если имя файла рабочего процесса не является именем файла по умолчанию (workflow.xml), необходимо его указать. |
	
2. Сохраните файл как **C:\Tutorials\UseOozie\coordinator.xml** в кодировке ANSI (ASCII) (если текстовый редактор не позволяет сделать это, используйте Блокнот).
...
HDInsight использует для хранения данных хранилище BLOB-объектов Azure.  Это хранилище называется WASB или хранилище Microsoft Azure больших двоичных объектов. WASB - это реализация HDFS на базе хранилища больших двоичных объектов Azure от корпорации Майкрософт. Дополнительную информацию см. в разделе [Использование хранилища BLOB-объектов Azure с HDInsight][hdinsight-storage]. 
...
> [WACOM.NOTE] Путь WASB является виртуальным.  Дополнительную информацию см. в разделе [Использование хранилища BLOB-объектов Azure с HDInsight][hdinsight-storage]. 
...
3. Скопируйте следующий сценарий в область сценарниев и задайте первые четырнадцать переменных (пропустите шестую - $storageUri). 
...
7. Для выполнения сценария нажмите кнопку **Запустить сценарий** или нажмите клавишу **F5**. Результат должен быть аналогичен приведенному ниже:
