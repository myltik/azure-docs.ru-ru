<properties title="Hybrid Connections Step-by-Step: Connect to on-premises SQL Server from an Azure website" pageTitle="Пошаговое создание гибридного подключения: подключение к локальному SQL Server с веб-сайта Azure" description="Создание веб-сайта в Microsoft Azure и его подключение к локальной базе данных SQL Server" metaKeywords="" services="web-sites" solutions="web" documentationCenter="" authors="cephalin" manager="wpickett" editor="mollybos" videoId="" scriptId="" />
...
- **Microsoft .NET Framework 3.5 с пакетом обновления 1 (SP1)**. Если у вас операционная система Windows 8.1, Windows Server 2012 R2, Windows 8, Windows Server 2012, Windows 7 или Windows Server 2008 R2, ее можно включить на панели управления, выбрав "Программы и компоненты > Включение и выключение компонентов Windows". В противном случае программу можно скачать в [Центре загрузки Майкрософт](http://www.microsoft.com/download/en/details.aspx?displaylang=en&id=22).

- **SQL Server 2014, экспресс-выпуск с инструментами** - скачайте экспресс-выпуск Microsoft SQL Server бесплатно на [странице базы данных веб-платформы Майкрософт](http://www.microsoft.com/web/platform/database.aspx). Выберите **экспресс-выпуск** (не LocalDB). Версия **экспресс-выпуск с инструментами** включает в себя SQL Server Management Studio, который будет использоваться в этом учебнике.

- **SQL Server Management Studio Express**. Сюда включается SQL Server 2014, экспресс-выпуск с инструментами, указанный выше, но если требуется установить его отдельно, вы можете скачать и установить его на [странице скачивания экспресс-выпуска SQL Server](http://www.microsoft.com/web/platform/database.aspx).
...
> <table border="1">
    ...
    <tr>
        <td>80</td>
        <td><strong>Обязательно</strong> для порта HTTP при проверке сертификатов и (необязательно) для подключения к данным.</td>
    </tr>
    <tr>
        <td>443</td>
...
[2. Создание веб-сайта на портале Azure (предварительная версия)](#CreateSite)
...
2. В колонке **Гибридные подключения** в столбце **Состояние** для недавно добавленных конечных точек показано **Не подключено**. Щелкните подключение, чтобы настроить его.
...
2. В разделе **Шаблоны** диалогового окна **Новый проект** выберите **Интернет**, затем **Веб-приложение ASP.NET**, а затем нажмите кнопку **ОК**.
...
> [WACOM.NOTE] Чтобы убедиться, что приложение использует базу данных, созданную в экспресс-выпуске SQL Server, а не LocalDB Visual Studio по умолчанию, важно выполнить это действие до запуска проекта.
...
1. Вы можете скачать профиль публикации для веб-сайта на портале Azure. Выберите на выноске своего веб-сайта **Скачать профиль публикации** и сохраните файл на компьютере.
