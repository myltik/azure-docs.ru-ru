...
|Коллекция	|Коллекция представляет собой контейнер документов JSON и связанной логики приложения на JavaScript.
...
|Вложение	|Вложение - это специальный документ, содержащий ссылки и связанные с ним метаданные к внешним BLOB-объектам/носителям. Разработчик может выбрать, управлять ли BLOB-объектами в DocumentDB или хранить их с помощью внешних поставщиков услуг хранения BLOB-объектов, таких как OneDrive, Dropbox и т. д. 
...
Определяемая пользователем функция (UDF) может быть указана как часть SQL-запроса и может использоваться в качестве способа расширения [языка запросов SQL DocumentDB](https://msdn.microsoft.com/library/azure/dn782250.aspx).   
...
##Разрешения
С точки зрения контроля доступа, ресурсы, такие как учетные записи баз данных, базы данных, пользователи и разрешения, рассматриваются как *administrative* ресурсы, так как они требуют наличия прав администратора. С другой стороны, ресурсы, включая коллекции, документы, вложения, хранимые процедуры, триггеры и определяемые пользователем функции, находятся в области видимости данной базы данных и рассматриваются как *application resources*. В соответствии с двумя типами ресурсов и ролями, которые обращаются к ним (администратора и пользователей), модель авторизации определяет два типа *access keys*: *master key* и *resource key*. Мастер-ключ является частью учетной записи базы данных и предоставляется разработчику (или администратору), который подготавливает учетную запись базы данных. Этот мастер-ключ имеет семантику администратора, в том смысле, что он может быть использован для того, чтобы разрешить доступ как к административным, так и к прикладным ресурсам. В отличие от этого, ключ ресурса является разделенным ключом доступа, который позволяет получить доступ к *specific* ресурсу приложения. Таким образом, он охватывает отношения между пользователем базы данных и разрешениями пользователя, которые он имеет к определенному ресурсу (например, коллекции, документу, вложению, хранимой процедуре, триггеру или пользовательской функции).   
