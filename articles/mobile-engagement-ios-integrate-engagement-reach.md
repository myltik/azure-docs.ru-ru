...
-   Добавьте пакет SDK для рекламных кампаний в проект Xcode. В проекте Xcode выберите **Проект \> Добавить в проект** и выберите папку `EngagementReach`.
...
-   Измените строку **icon.png** на имя изображения, которое нужно использовать в качестве значка уведомления.
-   Если вы хотите использовать параметр *Обновить значение индикатора событий* в рекламных кампаниях или кампании системных push-уведомлений \</SaaS/Reach API/Формат кампаний/Системные push-уведомления\>, следует разрешить модулю обработки рекламных кампаний управлять значком индикатора событий (он автоматически очистит индикатор событий приложения и будет сбрасывать значение, хранимое службой Engagement, при каждом запуске приложения или когда оно будет становится активным). Для этого после инициализации модуля обработки рекламных кампаний необходимо добавить следующую строку:

			[reach setAutoBadgeEnabled:YES];

-   Если требуется обрабатывать отправку данных рекламных кампаний, делегат приложения должен соответствовать протоколу `AEReachDataPushDelegate`. После инициализации модуля обработки рекламных кампаний добавьте следующую строку:
...
Следуйте указаниям в этом руководстве: Как подготовить приложение для работы с push-уведомлениями Apple

### Добавление необходимого клиентского кода

*На этом этапе в приложении в интерфейсной части Engagement должен быть зарегистрирован сертификат push-уведомлений Apple.*

Если он не зарегистрирован, необходимо зарегистрировать приложение для получения push-уведомлений. Добавьте следующую строку при запуске приложения (обычно в методе `application:didFinishLaunchingWithOptions:`):
...
Наконец необходимо уведомить пакет SDK для службы Engagement о получении приложением удаленного уведомления. Для этого просто вызовите метод `applicationDidReceiveRemoteNotification:` в делегате приложения:
...
После выполнения этих шагов приложение будет готово к получению push-сообщений Engagement в любое время.

Однако вам может потребоваться, чтобы что-то отображалось для конечного пользователя при запуске приложения в ответ на получение push-сообщения Engagement. Для этого мы предоставляем три дополнительных метода делегата в протоколе `AEPushDelegate`:
...
> Когда пользователь запускает приложение в ответ на получение push-сообщения, этот метод вызывается сразу после запуска приложения, чтобы указать, что получено полное сообщение. Это хорошая возможность для отображения сообщения о загрузке конечному пользователю.

`didFailToRetrieveLaunchMessage`

> Этот метод вызывается при сбое получения сообщения. На этом этапе, возможно, следует отобразить для конечного пользователя сообщение, которое указывает о сбое.

`didReceiveLaunchMessage:`

> Этот метод вызывается при получении push-сообщения, которое вызвало запуск приложения. Используйте эту возможность, чтобы скрыть все сообщения о загрузке и отобразить соответствующее содержимое для конечного пользователя.

После инициализации службы Engagement следует задать делегат для push-уведомлений:
...
Чтобы изменить внешний вид уведомлений в приложении, можно просто изменить файл `AENotificationView.xib` в соответствии со своими потребностями, но при этом следует сохранить значения тегов и типы существующих вложенных представлений.

По умолчанию уведомления в приложении представлены в нижней части экрана. Если нужно, чтобы они отображались в верхней части экрана, измените предоставляемый файл `AENotificationView.xib` и измените свойство `AutoSizing` главного представления, чтобы оно располагалось в верхней части суперпредставления.
...
В этом простом примере категории предполагается, что в основном наборе приложений есть файл с именем `MyNotificationView.xib`. Если методу не удается найти соответствующий файл `.xib`, уведомление не будет отображаться и служба Engagement выведет сообщение в консоль.
...
Если вам требуется расширенная настройка уведомлений (например, если вам нужно загрузить представление непосредственно из кода), мы советуем просмотреть предоставленную документацию по исходному коду и классу для `Protocol ReferencesDefaultNotifier` и `AENotifier`.
...
Если ваша реализация `AENotifier` обходит поведение по умолчанию, необходимо вызвать эти методы жизненного цикла самостоятельно. В следующих примерах показаны некоторые случаи обхода поведения по умолчанию.

-    `AEDefaultNotifier` не был расширен, например, если обработка категории была выполнена с нуля.
-    `prepareNotificationView:forContent:` был переопределен. Обязательно сопоставьте хотя бы `onNotificationActioned` или `onNotificationExited` с одним из элементов управления пользовательского интерфейса.
...
1.  Добавьте представление уведомлений с помощью конструктора интерфейса:
	...
	-   Разместите `UIView` размером 320x60 (или 768x60 для iPad), в котором следует отображать уведомление.
...
> [AZURE.NOTE] По каждому щелчку уведомления для объявления с категорией "my_category" будет выполняться инициализация зарегистрированного контроллера представлений (в этом случае `MyCustomAnnouncementViewController`) путем вызова метода `initWithAnnouncement:` и представление будет добавлено в текущее окно приложения.
...
В этом случае предоставленный `MyCustomPollViewController` должен расширить `AEPollViewController`. Или же можно расширить его с помощью контроллера по умолчанию `AEDefaultPollViewController`.
