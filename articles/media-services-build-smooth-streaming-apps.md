<properties linkid="develop-media-services-tutorials-smooth-streaming-windows-store-app" urlDisplayName="Приложение для магазина Windows с бесперебойной потоковой передачей" pageTitle="Приложение для магазина Windows с бесперебойной потоковой передачей — учебные курсы Azure .NET" metaKeywords="" description="Узнайте, как с помощью служб носителей Azure создать на C# приложение для магазина Windows с элементом управления XML MediaElement для воспроизведения контента с бесперебойной потоковой передачей." metaCanonical="" services="media-services" documentationCenter="" title="Создание приложения для магазина Windows с бесперебойной потоковой передачей" authors="" solutions="" manager="" editor="" />
...
- Windows 8 (32-разрядная или 64-разрядная). Можно получить [оценку Windows 8 Enterprise](http://msdn.microsoft.com/ru-ru/evalcenter/jj554510.aspx) из MSDN.
- Visual Studio 2012 и Visual Studio Express 2012 для Windows 8, установленные под управлением Windows 8. Можно получить бесплатную пробную версию [здесь](http://www.microsoft.com/visualstudio/11/ru-ru/downloads).
...
Готовое решение для каждого урока можно загрузить из раздела образцов кода для разработчиков MSDN (библиотека кода): [Урок 1](http://code.msdn.microsoft.com/Smooth-Streaming-Client-0bb1471f "Простой проигрыватель мультимедиа Windows 8 с бесперебойной потоковой передачей"), [Урок 2](http://code.msdn.microsoft.com/A-simple-Windows-8-Smooth-ee98f63a "Простой проигрыватель мультимедиа Windows 8 с бесперебойной потоковой передачей с ползунком"), [Урок 3](http://code.msdn.microsoft.com/A-Windows-8-Smooth-883c3b44 "Проигрыватель мультимедиа Windows 8 с бесперебойной потоковой передачей с возможностью выбора потока") и [Урок 4](http://code.msdn.microsoft.com/A-Windows-8-Smooth-aa9e4907 "Проигрыватель мультимедиа Windows 8 с бесперебойной потоковой передачей и возможностью выбора дорожки").
...
Дополнительные сведения о разработке приложений для магазина Windows см. в разделе [Разработка замечательных приложений для Windows 8](http://msdn.microsoft.com/ru-ru/windows/apps/br229512.aspx). 
...
5.	Добавьте в конец класса **MainPage** следующий код:
	...
	Здесь определяется обработчик событий SliderProgress_PointerPressed.  Для его запуска требуется выполнить ряд дополнительных действий, которые будут рассматриваться на следующем уроке этого учебного курса.
...
3.	В классе **MainPage** добавьте следующие члены данных:
...
5.	Добавьте в конец класса **MainPage** следующий код:
		...
		// При воспроизведении контента в режиме реального времени необходимо обновлять время начала и окончания ползунка.
...
2. Изменение кода файла
3. Компиляция и тестирование приложения


**Изменение XAML-файла**

1. В обозревателе решений щелкните правой кнопкой мыши файл **MainPage.xaml** и выберите команду **Просмотреть конструктор**.
...
1. В обозревателе решений щелкните правой кнопкой мыши **MainPage.xaml** и выберите команду **Просмотреть код**.
2. В пространстве имен SSPlayer добавьте следующий класс:
		#region class Stream
...
5. Найдите метод элемент mediaElement_ManifestReady и добавьте следующий код в конце этой функции:
