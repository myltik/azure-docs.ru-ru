...
- Windows 8 (32-разрядная или 64-разрядная). Вы можете получить [ознакомительную версию Windows 8 Enterprise](http://msdn.microsoft.com/ru-ru/evalcenter/jj554510.aspx) из MSDN.
- Visual Studio 2012 или Visual Studio Express 2012 для Windows 8, установленные в Windows 8. Пробную версию можно получить [здесь](http://www.microsoft.com/visualstudio/11/ru-ru/downloads).
- [Клиентский пакет Microsoft SDK для потоковой передачи Smooth Streaming для Windows 8](http://visualstudiogallery.msdn.microsoft.com/04423d13-3b3e-4741-a01c-1ae29e84fea6?SRC=Homehttp://visualstudiogallery.msdn.microsoft.com/04423d13-3b3e-4741-a01c-1ae29e84fea6?SRC=Home).

Готовое решение для каждого урока можно загрузить из раздела образцов кода для разработчиков MSDN (коллекция кодов): [Урок 1](http://code.msdn.microsoft.com/Smooth-Streaming-Client-0bb1471f "A Simple Windows 8 Smooth Streaming Media Player"), [урок 2](http://code.msdn.microsoft.com/A-simple-Windows-8-Smooth-ee98f63a "A Simple Windows 8 Smooth Streaming Media Player with a Slider Bar Control"), [урок 3](http://code.msdn.microsoft.com/A-Windows-8-Smooth-883c3b44 "A Windows 8 Smooth Streaming Media Player with Stream Selection") и [урок 4](http://code.msdn.microsoft.com/A-Windows-8-Smooth-aa9e4907 "A Windows 8 Smooth Streaming Media Player with Track Selection").
...
Дополнительную информацию о разработке приложений для магазина Windows см. в разделе [Разработка замечательных приложений для Windows 8](http://msdn.microsoft.com/ru-ru/windows/apps/br229512.aspx). 
...
**Добавление ссылки на клиентский пакет SDK для потоковой передачи Smooth Streaming**
...
**Разработка пользовательского интерфейса проигрывателя**

1.	В обозревателе решений дважды щелкните файл **MainPage.xaml**, чтобы открыть его в режиме конструктора.
...
3.	В начале класса **MainPage** добавьте следующий элемент данных:
...
5.	В конце класса **MainPage** добавьте следующий код:
...
1.	В меню **СБОРКА** выберите команду **Диспетчер конфигураций**.
...
**Регистрация обработчика потока байтов для потоковой передачи Smooth Streaming и передача набора свойств propertyset**
...
4.	В конструкторе **MainPage** добавьте следующий код после строки **this.Initialize Components();** и строк кода регистрации, добавленных в предыдущем уроке:
...
5.	В конструкторе **MainPage** измените два метода RegisterByteStreamHandler, чтобы добавить следующие параметры:
...
1.	В обозревателе решений щелкните правой кнопкой мыши **MainPage.xaml** и выберите команду **Просмотреть код**.
2.	В классе **MainPage** добавьте следующий элемент данных:
...
4.	В конце конструктора **MainPage** добавьте следующую строку, чтобы подписаться на событие открытия адаптивного источника:
...
3.	В конце конструктора **MainPage** добавьте следующий код, чтобы подписаться на события:
...
3.	В классе **MainPage** добавьте следующие элементы данных:
...
2. Найдите &lt;Grid.RowDefinitions&gt; и измените свойства RowDefinitions следующим образом:
...
3. Между тегами &lt;Grid&gt;&lt;/Grid&gt; добавьте следующий код, чтобы определить элемент управления "список", который позволяет просмотреть список доступных потоков и выбрать нужные:
...
1. В обозревателе решений щелкните правой кнопкой мыши **MainPage.xaml**и выберите команду **"Просмотреть конструктор"**.
2. Найдите тег &lt;Grid&gt; с именем **gridStreamAndBitrateSelection и добавьте в конце этого тега следующий код:
...
3.	В верхней части приложения можно выбрать URL-адрес бесперебойной потоковой передачи по умолчанию или ввести другой. 
4.	Щелкните **Задать источник**. 
5.	По умолчанию выбраны все дорожки потока видеоданных. Чтобы поэкспериментировать со скоростью, выберите минимальную возможную скорость, а затем - максимальную возможную. После каждого изменения необходимо нажать кнопку "Отправить".  Вы увидите, как изменилось качество видеоизображения.
...
- [Технический обзор потоковой передачи Smooth Streaming](http://www.iis.net/learn/media/on-demand-smooth-streaming/smooth-streaming-technical-overview)
