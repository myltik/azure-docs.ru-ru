<properties
   pageTitle="Выражения шаблонов диспетчера ресурсов | Microsoft Azure"
...
# Выражения шаблонов в диспетчере ресурсов Azure

В этом разделе описаны все выражения, которые можно использовать в шаблоне диспетчера ресурсов Azure.

Выражения шаблонов и их параметры зависят от регистра. Например, диспетчер ресурсов одинаково преобразует **variables('var1')** и **VARIABLES('VAR1')**. При вычислении выражение сохранит регистр, если только не изменяет его явным образом (например toUpper или toLower). Требования для некоторых типов ресурсов могут не зависеть от того, как происходит вычисление выражений.

## Числовые выражения

Диспетчер ресурсов предоставляет следующие выражения для работы с целыми числами:
...
Это выражение всегда используется с объектом **copy**. Примеры использования **copyIndex** см. в статье [Создание нескольких экземпляров ресурсов в диспетчере ресурсов Azure](resource-group-create-multiple.md).
...
Диспетчер ресурсов предоставляет следующие выражения для работы со строками:
...
Значение параметра **baseUri** может включать в себя определенный файл, однако при построении URI используется только базовый путь. Например, при передаче ****http://contoso.com/resources/azuredeploy.json** в качестве параметра baseUri получается базовый URI ****http://contoso.com/resources/**.
...
## Выражения значений развертывания

Диспетчер ресурсов предоставляет следующие выражения для получения значений из разделов шаблонов и значений, связанных с развертыванием:

- [deployment](#deployment)
- [parameters](#parameters)
- [variables](#variables)

Сведения о получении значений из ресурсов, групп ресурсов или подписки см. в разделе [Выражения ресурсов](#resource-expressions).
...
Диспетчер ресурсов предоставляет следующие выражения для получения значений ресурсов:
...
Сведения о получении значений из параметров, переменных или текущего развертывания см. в разделе [Выражения значений развертывания](#deployment-value-expressions).
...
Если не требуется непосредственно указывать версию API в шаблоне, можно использовать выражение **providers** и получить одно значение, например последнюю версию, как показано ниже.
