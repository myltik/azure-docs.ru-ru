...
- **Подключение к кластеру с помощью протокола удаленного рабочего стола**. Для получения инструкций см. статью [Управление кластерами Hadoop в HDInsight с использованием портала Azure][hdinsight-manage-portal].
...
Дополнительные сведения см. в разделе [Настройка VPN-подключения «точка-сайт» к виртуальной сети Azure](https://msdn.microsoft.com/library/azure/dn133792.aspx).
...
1. На портале Azure щелкните **ПАНЕЛЬ МОНИТОРИНГА** в верхней части страницы.
...
4. Щелкните **ПАНЕЛЬ МОНИТОРИНГА** в верхней части страницы. Появится визуальная схема виртуальной сети:
	...
	![Схема виртуальной сети Azure{b\> \<b}«точка-сайт»][img-vnet-diagram]
...
3. Следующая команда в приведенном ниже примере создаст и установит корневой сертификат в хранилище личных сертификатов на рабочей станции, а также создает соответствующий CER-файл, который позднее будет отправлен на портал Azure. 
...
1. На портале Azure щелкните **СЕТЬ** в области слева.
