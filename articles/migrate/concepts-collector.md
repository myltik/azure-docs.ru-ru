...
Модуль сборщика — это OVF-файл, который можно скачать из проекта службы "Миграция Azure". Модуль создает экземпляр виртуальной машины VMware с 4 ядрами, 8 ГБ ОЗУ и 1 диском размером 80 ГБ. Для этого модуля используется ОС Windows Server 2012 R2 (64-разрядная).
...
2. Вы можете настроить сборщик для подключения через прокси-сервер.
    ...
    * IP-адрес или полное доменное имя прокси-сервера должны иметь такой формат: http://IPaddress или http://FQDN. Поддерживается только прокси-сервер HTTP.
...
3. Можно выбрать только область, которая содержит *менее 1000 виртуальных машин*. Если выбрать область, которая содержит больше 1000 виртуальных машин, вам придется разделить ее на меньшие области, создав папки. Затем необходимо выполнить отдельные операции по обнаружению небольших папок.
