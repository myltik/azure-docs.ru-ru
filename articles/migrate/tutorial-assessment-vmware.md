...
Для виртуальных машин, которые готовы к миграции, служба "Миграция Azure" рекомендует размер виртуальной машины в Azure. Размер, рекомендуемый службой "Миграция Azure", зависит от условий определения размера, указанных в свойствах оценки. Если указано условие определения размера на основе производительности, для формирования рекомендации учитывается журнал производительности виртуальных машин. Если указано условие определения размера как для локальной виртуальной машины, при формировании рекомендации относительно размера виртуальной машины в Azure учитывается размер локальной виртуальной машины (определение размера "как есть"). При определении размера виртуальной машины данные об использовании ЦП и памяти виртуальной машины не учитываются. Тем не менее в случае определения размера локального компьютера размер дисков определяется на основе данных производительности.  Дополнительные сведения об определении размеров в службе "Миграция Azure" см. [здесь](concepts-assessment-calculation.md).
