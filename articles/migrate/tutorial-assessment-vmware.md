...
2. Поиск **миграции Azure**и выберите службу **Azure миграции (Предварительная версия)** в результатах поиска. Затем щелкните **Создать**.
3. Укажите имя проекта и подписку Azure для него.
4. Создайте новую группу ресурсов.
5. Укажите расположение, в котором следует создать проект, а затем нажмите кнопку **создать**. Проект службы "Миграция Azure" для данной предварительной версии можно создать только в регионе "Центральная часть США". При этом по-прежнему можно планировать перенос в любое целевое расположение Azure. Расположение, указанное для проекта используется только для хранения метаданных, собранные из локальных виртуальных машин. 
...
4. В сборщике миграции Azure откройте **предварительной настройки**.
    ...
    > [!NOTE]
    > Адрес прокси-сервера должен вводиться в форме http://ProxyIPAddress или http://ProxyFQDN. Поддерживается только HTTP-прокси.

    - Сборщик проверяет, что collectorservice запущена. Эта служба устанавливается по умолчанию на виртуальной машине сборщика.
...
6. В **миграции проекта укажите**, укажите идентификатор проекта миграции Azure и ключа, скопированный на портале. Если вы не скопировали их, откройте портал Azure на виртуальной машине сборщика. На странице **Обзор** проекта щелкните **Обнаружение компьютеров** и скопируйте соответствующие значения.  
7. В **следить за ходом выполнения коллекции**, отслеживать обнаружения и убедитесь, что метаданные, полученные от виртуальных машин в области. Сборщик предоставляет приблизительное время завершения обнаружения.
...
> [!NOTE]
> Оценка стоимости, предоставляемая службой "Миграция Azure", приводится для запуска локальных виртуальных машин как виртуальных машин Azure IaaS (инфраструктура как услуга). Azure миграции не учитывает любые платформа как услуга (PaaS) или программное обеспечение как услуга (SaaS) затраты. 
...
- [Дополнительные сведения](how-to-scale-assessment.md) обнаружение и оценку больших среды VMware.
- Сведения о создании групп оценки высокой надежности с помощью [машины сопоставление зависимостей](how-to-create-group-machine-dependencies.md)
