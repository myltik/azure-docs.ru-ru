...
Помимо определения готовности к работе в Azure и нужного размера в службе "Миграция Azure" также предлагаются средства, которые можно использовать для миграции виртуальной машины. Если компьютер подходит для миграции по методике "lift-and-shift", рекомендуется использовать службу [Azure Site Recovery]. Если это компьютер базы данных, рекомендуется использовать Azure Database Migration Service.
...
Каждая оценка в службе "Миграция Azure" связана с оценкой достоверности, которая принимает значения в диапазоне от 1 до 5 звездочек (1 звезда — низкая достоверность, а 5 — высокая). Оценка достоверности назначается на основе доступности точек данных, необходимых для вычисления оценки. Это помогает оценить надежность рекомендаций по выбору размера, предоставленных службой "Миграция Azure". 

Оценка достоверности полезна при *определении размера на основе производительности*, так как не все точки данных могут быть доступны. При *определении размера как для локальной ВМ* оценка достоверности — всегда 5 звезд, так как в службе "Миграция Azure" имеются все данные, необходимые для определения размера виртуальной машины. 

Для определения размера на основе производительности службе "Миграция Azure" требуются данные об использовании ЦП и памяти. Кроме того, для каждого диска, присоединенного к виртуальной машине, необходимы операции записи и чтения на диск в секунду и пропускная способность. Аналогичным образом, для каждого сетевого адаптера, подключенного к виртуальной машине, службе "Миграция Azure" необходимы сведения о сетевом вводе-выводе. Если какой-либо из приведенных выше показателей использования недоступен в vCenter Server, рекомендации по выбору размера, предоставленные службой "Миграция Azure", могут быть ненадежными. В зависимости от процента доступных точек данных оценка достоверности может быть такой:
...
- Для статистики в vCenter Server не задан уровень 3 и для оценки выбрано условие определения размера на основе производительности. Если для статистики в vCenter Server задан уровень меньше чем 3, данные о производительности диска и сети не собираются из vCenter Server. В этом случае рекомендации, предоставленные службой "Миграция Azure" для диска и сети, основаны только на данных выделения ресурсов в локальной среде. Для хранилища служба "Миграция Azure" рекомендует использовать стандартные диски, так как невозможно определить, имеет ли диск высокий уровень операций ввода-вывода в секунду и пропускную способность и необходимо ли использовать диски класса Premium.
- Для статистики в vCenter Server был установлен уровень 3 в течение небольшого периода времени перед выполнением обнаружения. Например, если задать для статистики уровень 3 сегодня и начать выполнение с использованием модуля сборщика завтра (через 24 часа), при создании оценки за один день у вас будут все точки данных. Но если изменить продолжительность производительности в свойствах оценки до одного месяца, оценка достоверности снижается, так как данные о производительности диска и сети за последний месяц недоступны. Если необходимо рассмотреть данные о производительности за последний месяц, рекомендуется задать для статистики vCenter Server уровень 3 на один месяц, прежде чем начать выполнение обнаружения. 
