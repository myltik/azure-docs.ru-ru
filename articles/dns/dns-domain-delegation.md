...
Домен — это уникальное имя в службе доменных имен, например contoso.com. Зона DNS используется для размещения DNS-записей определенного домена. Например, домен contoso.com может содержать несколько записей DNS, включая mail.contoso.com (для почтового сервера) и www.contoso.com (для веб-сайта).
...
> [!NOTE]
> Для создания зоны DNS с доменным именем в Azure DNS необязательно быть его владельцем. Однако вам необходимо быть владельцем домена, чтобы настроить делегирование в Azure DNS у регистратора доменных имен.
...
С помощью Azure PowerShell полномочные записи NS можно получить следующим образом. Имя записи @ используется для указания записей на вершине зоны.
...
Создайте набор записей NS в родительской зоне, чтобы завершить делегирование. Имя набора записей в родительской зоне соответствует имени дочерней зоны (в этом случае partners).
