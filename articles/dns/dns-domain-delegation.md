...
Azure DNS — это служба размещения для доменов DNS. Чтобы запросы DNS для домена достигали Azure DNS, домен должен быть делегирован в Azure DNS из родительского домена. В этой статье объясняется принцип работы делегирования домена и показано, как делегировать домены в Azure DNS.
