...
Служба Azure DNS поддерживает следующие типы записей: A, AAAA, CNAME, MX, NS, SOA, SRV, TXT. Наборы записей типа SOA создаются автоматически вместе с каждой зоной, их невозможно создать отдельно. Обратите внимание, что [тип записи SPF является устаревшим в стандартах DNS по сравнению с созданием записей SPF с помощью типа записи TXT](http://tools.ietf.org/html/rfc7208#section-3.1).
...
Azure DNS поддерживает [записи с подстановочными знаками](https://en.wikipedia.org/wiki/Wildcard_DNS_record). Они возвращаются для любого запроса с совпадающим именем (пока не найдется еще более близкое совпадение из набора записей без подстановочных знаков). Чтобы создать набор записей с подстановочными знаками, используйте знак "*" вместо имени набора записей или вместо первых символов имени, например "*.foo".
...
После создания зоны и записей DNS вы можете [делегировать домен в Azure DNS](dns-domain-delegation.md).<BR> Узнайте, как [управлять записями DNS](dns-operations-dnszones-cli.md) с помощью интерфейса командной строки.<BR> Вы также можете [автоматизировать операции с использованием пакета SDK для .NET](dns-sdk.md), чтобы программировать операции Azure DNS в своем приложении.
