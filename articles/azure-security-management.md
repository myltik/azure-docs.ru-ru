<properties
   pageTitle="Управление безопасностью в Azure | Microsoft Azure"
   description="В этой статье рассматриваются способы улучшения безопасности удаленного управления при администрировании среды Microsoft Azure: облачных служб, виртуальных машин и пользовательских приложений."
...
Хотя все эти возможности доступа и управления и предоставляют множество преимуществ, многообразие вариантов увеличивает риски развертывания облака, усложняя управление, отслеживание и ведение аудита административных действий. Многообразие вариантов также может создавать угрозу безопасности, так как доступ к конечным точкам клиентов, используемым для управления облачными службами, не регулируется. При использовании общедоступных или личных рабочих станций для разработки инфраструктуры и управления ею открываются новые направления непрогнозируемых угроз, например при просмотре веб-страниц (атаки на предприятие) или со стороны электронной почты (социотехнические и фишинговые угрозы).
...
Злоумышленники часто пытаются скомпрометировать учетные данные (например, методом подбора пароля, с помощью фишинга и сбора учетных данных) или обманом вынудить пользователей использовать вредоносный код (например, с помощью скрытых загрузок на вредоносных веб-сайтах или вредоносных вложений в электронной почте), чтобы получить привилегированный доступ. В облачной среде, управляемой удаленно, нарушение безопасности учетной записи создает высокий риск, так как она предоставляет доступ с любых конечных точек и в любое время.
...
Чтобы сделать управление и выполнение операций безопаснее, можно уменьшить количество возможных точек входа, снизив риск атаки клиента. Для этого можно использовать такие принципы безопасности, как разделение обязанностей и разделение сред: изоляция конфиденциальных функций уменьшает вероятность того, что ошибка на одном уровне создаст нарушение на другом. Следовательно, при выполнении задач администрирования следует избегать действий, которые могут дать повод для компрометации (например, если администратор откроет вредоносное вложение в сообщении электронной почты, оно может заразить сервер инфраструктуры). По аналогии и рабочую станцию, которая служит для выполнения операций, связанных с данными высокой конфиденциальности, не следует использовать для просмотра страниц в Интернете.

С установкой каждого приложения или службы на рабочей станции администратора увеличивается угроза безопасности, так как расширяется спектр уязвимостей, которыми можно воспользоваться. Таким образом, если уменьшить количество возможных направлений атак системы, удалив ненужное программное обеспечение из стандартного установочного образа системы, можно улучшить управление клиентом и увеличить его стабильность, а также повысить уровень защиты профиля безопасности ПО. Например, любая стандартная рабочая станция администрирования, поддержки или разработки не требует установки клиента электронной почты или других повышающих эффективность приложений, если основное предназначение устройства — управление облачными службами.

Клиентские системы с административным доступом к компонентам инфраструктуры должны считаться в сети настолько же конфиденциальными, как и компоненты инфраструктуры. Так как компрометация учетных данных администратора или его системы может создать брешь в обслуживании, чтобы уменьшить риск, нужно настроить максимально строгую политику безопасности для клиента. Политики безопасности, обеспечивающие тщательную проверку клиентских устройств с правами администратора, могут предусматривать параметры групповой политики, с помощью которых можно запретить открытый Интернет-доступ с конкретных устройств и настроить ограничения брандмауэра.

Вы также можете предпринять другие меры:

- использовать VPN-подключение по протоколу IPsec, если нужен прямой доступ;
- настроить отдельные домены Active Directory для управления и разработки;
- изолировать сетевой трафик рабочей станции управления и включить его фильтрацию;
- использовать антивредоносное ПО;
- реализовать многофакторную проверку подлинности, чтобы уменьшить риск кражи учетных данных.
...
- проверку подлинности и [контроль доступа на основе ролей](active-directory/role-based-access-control-configure.md);
...
> [AZURE.NOTE] Выполнение некоторых рекомендаций в этой статье может привести к более интенсивному использованию данных, а также сетевых и вычислительных ресурсов, а значит, и к дополнительным затратам на лицензии или подписки.
...
Все это можно реализовать принудительно с помощью [объектов групповой политики](https://www.microsoft.com/download/details.aspx?id=2612) в доменных службах Active Directory (AD DS), применив эти объекты ко всем учетным записям управления в (локальном) домене управления.
...
Чтобы полностью централизовать административный доступ и упростить мониторинг и ведение журналов, вы можете развернуть выделенный сервер [шлюза удаленных рабочих столов](https://technet.microsoft.com/library/dd560672) в локальной сети, подключенной к среде Azure.
...
- Подготовьте [сертификат управления Azure](http://msdn.microsoft.com/library/azure/gg551722.aspx) в шлюзе удаленных рабочих столов, сделав его единственным узлом с доступом к порталу управления Azure.
- Присоедините шлюз удаленных рабочих столов к тому же [домену управления](http://technet.microsoft.com/library/bb727085.aspx), к которому относится рабочая станция администрирования. Это необходимо, если вы используете подключение ExpressRoute или VPN типа "сеть — сеть" с протоколом IPsec в домене, в котором настроено одностороннее отношение доверия к Azure AD, или используете учетные данные федеративно для локального экземпляра AD DS и Azure AD.
- Настройте [политику авторизации клиентских подключений](http://technet.microsoft.com/library/cc753324.aspx), чтобы шлюз удаленных рабочих столов проверял, является ли имя клиентского компьютера допустимым (присоединен ли компьютер к домену) и разрешен ли компьютеру доступ к порталу управления Azure.
- Используйте протокол IPsec для [VPN Azure](https://azure.microsoft.com/documentation/services/vpn-gateway/), чтобы дополнительно защитить трафик управления от перехвата данных и кражи токенов, или изолированное Интернет-подключение через [Azure ExpressRoute](https://azure.microsoft.com/documentation/services/expressroute/).
- Включите многофакторную проверку подлинности (с помощью службы [Azure Multi-Factor Authentication](multi-factor-authentication/multi-factor-authentication.md)) или проверку подлинности с помощью смарт-карт для администраторов, которые входят через шлюз удаленных рабочих столов.
...
Как правило, увеличение безопасности рабочих станций администрирования, предназначенных для использования в облаке, очень похоже на аналогичную процедуру для любой локальной рабочей станции, например все они предусматривают уменьшение сборки и настройку разрешений с ограничениями. У некоторых уникальных аспектов облачного управления больше общего с удаленным управлением или корпоративным управлением по внештатному каналу. К ним относятся использование и аудит учетных данных, удаленный доступ с повышенным уровнем безопасности, обнаружение угроз и реагирование.

### Аутентификация

Вы можете настроить ограничения входа Azure, чтобы ограничить доступ к инструментам администрирования и возможность запрашивать доступ для аудита по исходным IP-адресам. Чтобы Azure было проще определять клиенты управления (рабочие станции и/или приложения), можно настроить API управления службами Azure (с помощью инструментов, разработанных пользователями, например командлетов Windows PowerShell) и портал управления Azure так, чтобы они требовали установку сертификатов управления на стороне клиента в дополнение к сертификатам SSL. Мы также советуем использовать многофакторную проверку подлинности для доступа администраторов.

В одних приложениях или службах, развертываемых в Azure, могут быть свои механизмы проверки подлинности доступа пользователей и администраторов, тогда как другие пользуются лишь механизмом Azure AD. Зависимо от того, используете ли вы учетные данные федеративно через службы федерации Active Directory (AD FS), синхронизируете службы каталогов, обслуживаете учетные записи пользователей только в облаке, использование [Microsoft Identity Manager](https://technet.microsoft.com/library/mt218776.aspx) (который является частью Azure AD Premium) упрощает управление жизненным циклом удостоверений ресурсов.

### Соединение

Есть несколько механизмов, которые позволяют обезопасить клиентские подключения к виртуальным сетям Azure. Два из них, [VPN типа "сеть — сеть"](https://channel9.msdn.com/series/Azure-Site-to-Site-VPN) и [VPN типа "точка — сеть"](vpn-gateway/vpn-gateway-point-to-site-create.md), включают использование стандартных отраслевых протоколов IPsec (VPN типа "сеть — сеть") или [Secure Socket Tunneling Protocol](https://technet.microsoft.com/magazine/2007.06.cableguy.aspx) (SSTP) (VPN типа "точка — сеть") для шифрования и туннелирования. При подключении к общедоступной системе управления службами Azure, такой как портал управления Azure, Azure запрашивает протокол HTTPS.

Изолированная рабочая станция с усиленной защитой, которая подключается к Azure не через шлюз удаленных рабочих столов, должна работать в VPN типа "точка — сеть", использующей протокол SSTP, чтобы создать начальное подключение к виртуальной сети Azure, а затем установить RDP-подключение к отдельным виртуальным машинам из туннеля VPN.
...
В ИТ-средах, где нет локальной инфраструктуры (например, у объектов групповой политики нет доступа к локальному экземпляру AD DS, потому что все серверы находятся в облаке), такая служба, как [Microsoft Intune](https://technet.microsoft.com/library/jj676587.aspx) упрощает развертывание и обслуживание конфигураций рабочих станций.
...
| | Используйте правила привязки сертификатов [Enhanced Mitigation Experience Toolkit 5.5](https://technet.microsoft.com/security/jj653751), чтобы предоставить надлежащие права доступа к сайтам Azure, которые используют сертификат SSL или TLS. |
...
В поддержке среды Azure участвуют эксплуатационные инженеры и технический персонал службы поддержки Майкрософт. У них есть доступ к рабочим системам Azure, которые используют [ПК (рабочие станции с усиленной защитой) с виртуальными машинами](#stand-alone-hardened-workstation-for-management). Эти компьютеры подготовлены для внутреннего доступа к корпоративной сети и использования соответствующих приложений (например, электронной почты, интрасети и т. д.). На всех компьютерах рабочей станции управления есть доверенные платформенные модули, диск загрузки узла зашифрован с помощью BitLocker, и они присоединены к специальному подразделению в основном корпоративном домене Майкрософт.
...
- Усиление безопасности Internet Explorer. Браузер Internet Explorer (или любой веб-браузер) — это основная точка входа вредоносного кода, так как эта программа интенсивно взаимодействует с внешними серверами. Просмотрите политики клиента и принудительно переведите браузер в защищенный режим работы, отключив надстройки и скачивание файлов и включив фильтрацию [Microsoft SmartScreen](https://technet.microsoft.com/library/jj618329.aspx). Убедитесь, что отображаются предупреждения безопасности. Воспользуйтесь преимуществами зон Интернета и создайте список доверенных сайтов, для которых вы настроили достаточный уровень усиления защиты. Заблокируйте все другие сайты и внутренние коды браузера, такие как ActiveX и Java.
...
- Групповая политика. Создайте глобальную политику администрирования для любых рабочих станций управления в домене (и заблокируйте доступ для всех остальных станций), а также для учетных записей пользователей, прошедших проверку подлинности на этих рабочих станциях.
...
- Шифрование. Убедитесь, что на рабочих станциях управления есть доверенные платформенные модули. Это позволит повысить безопасность включения [шифрованной файловой системы](https://technet.microsoft.com/library/cc700811.aspx) (EFS) и BitLocker. Если вы используете Windows To Go, используйте с BitLocker только зашифрованные USB-ключи.
- Регулирование. Используйте объекты групповой политики AD DS, чтобы контролировать все интерфейсы администраторов Windows, например для предоставления доступа к файлам. Включите рабочие станции управления в процессы аудита, мониторинга и ведения журналов. Отслеживайте все действия доступа и использования, которые выполняют администраторы и разработчики.
...
- [Microsoft Trust Center](https://www.microsoft.com/TrustCenter/Security/AzureSecurity) (Центр управления безопасностью Azure). Узнайте о возможностях Azure, которые позволяют защитить структуру и рабочие нагрузки Azure, используемые на соответствующей платформе.
- [Microsoft Security Response Center](http://www.microsoft.com/security/msrc/default.aspx). Это место, куда можно сообщать об уязвимостях, в том числе о проблемах Azure. Это также можно сделать по электронной почте, отправив сообщение по адресу [secure@microsoft.com](mailto:secure@microsoft.com).
