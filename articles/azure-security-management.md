...
Злоумышленники часто пытаются взломать учетные данные (например, методом подбора пароля, с помощью фишинга и сбора учетных данных) или обманом вынудить пользователей запустить вредоносный код (например, с помощью скрытых загрузок с вредоносных веб-сайтов или вредоносных вложений в электронной почте), чтобы получить привилегированный доступ. В облачной среде, управляемой удаленно, нарушение безопасности учетной записи создает высокий риск, так как она предоставляет доступ с любых конечных точек и в любое время.
...
- Присоедините шлюз удаленных рабочих столов к тому же [домену управления](http://technet.microsoft.com/library/bb727085.aspx), к которому относятся рабочие станции администрирования. Это необходимо, если вы используете подключение ExpressRoute или VPN типа "сеть — сеть" с протоколом IPsec в домене, в котором настроено одностороннее отношение доверия к Azure AD, или используете учетные данные федеративно для локального экземпляра AD DS и Azure AD.
...
- [Microsoft Trust Center](https://www.microsoft.com/TrustCenter/Security/AzureSecurity) (Центр управления безопасностью Microsoft). Узнайте о возможностях Azure, которые позволяют защитить структуру и рабочие нагрузки Azure, используемые на соответствующей платформе.
- [Microsoft Security Response Center](http://www.microsoft.com/security/msrc/default.aspx) (Центр реагирования системы безопасности Microsoft). Это место, куда можно сообщать об уязвимостях, в том числе о проблемах Azure. Это также можно сделать по электронной почте, отправив сообщение по адресу [secure@microsoft.com](mailto:secure@microsoft.com).
