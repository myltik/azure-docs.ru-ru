...
Также для отключения реальных измерений пользователя можно удалить ключ. После этого все измерения, отправленные диспетчеру трафика с этим ключом, будут игнорироваться.
...
Нет, вы не обязаны размещать серверные компоненты в Azure, чтобы работала функция реального измерения пользователя. Изображение размером в один пиксель, который скачивает скрипт измерений, и соответствующая служба в разных регионах Azure размещается и управляется Azure. 
...
Эту информацию вы получаете в табличном виде на портале. Также есть возможность скачать необработанные данные.
...
Представление трафика не учитывает сведения о подсети клиента EDNS при создании выходных данных. Оно использует для группировки пользователей только IP-адреса локальных сопоставителей DNS.
...
На период использования предварительной версии представление трафика включается на уровне подписки и доступно для всех профилей диспетчера трафика в этой подписке.

### <a name="how-can-i-turn-off-traffic-view"></a>Как отключить представление трафика?
Если вы хотите отключить представление трафика для подписки в течение периода предварительного просмотра, мы предлагаем вам создать запрос в службу поддержки.
...
В период использования предварительной версии представление трафика предоставляется бесплатно.
