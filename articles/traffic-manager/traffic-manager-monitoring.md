<properties 
   ...
   description="Эта статья содержит информацию о мониторинге и настройке диспетчера трафика."
...
Диспетчер трафика Azure показывает профиль и работоспособность службы конечной точки на портале управления. В столбце состояния для профиля и конечной точки отображается последнее наблюдаемое состояние. Вы можете ориентироваться на него, когда оцениваете работоспособность своих профилей согласно настройкам мониторинга диспетчера трафика. Если профиль работоспособен, запросы DNS будут распределяться между службами на основе параметров балансировки нагрузки для профиля ("Циклический перебор", "Производительность" или "Отработка отказа"). Когда система мониторинга диспетчера трафика обнаруживает, что состояние отслеживания изменилось, она обновляет запись о состоянии на портале управления. Для обновления состояния может потребоваться до 5 минут.
...
|Состояние профиля|Состояние конечной точки|Отслеживаемое состояние конечной точки (API и портал)|Примечания|
|---|---|---|---|
|Отключено|Включено|Неактивно|Отключенные профили не отслеживаются. Однако состоянием конечной точки можно управлять и в выключенных профилях.|
|&lt;любое&gt;|Отключено|Отключено|Отключенные профили не отслеживаются. Однако состоянием конечной точки можно управлять и в выключенных профилях.|
...
|Включено|Включено|Остановлено|Базовая облачная служба или веб-сайт не работают.|
...
|Состояние профиля (согласно настройке)|Отслеживаемое состояние конечной точки|Отслеживаемое состояние профиля(API и портал)|Примечания|
|---|---|---|---|
|Отключено|&lt;любое&gt; или профиль без определенных конечных точек.|Отключено|Конечные точки не отслеживаются.|
|Включено|Состояние по крайней мере одной конечной точки - "Деградация".|Деградация|Это означает, что требуется действие пользователя.|
...
|Включено|Состояние по крайней мере одной конечной точки - "Проверка конечной точки". Нет конечных точек с состоянием "В сети" или "Деградация".|Проверка конечной точки|Переходное состояние. Это обычно происходит, когда профиль только что был включен и проверяется работоспособность конечных точек.|
|Включено|Состояние всех конечных точек, определенных в профиле, либо "Отключено", либо "Остановлено", либо в профиле не определены конечные точки.|Неактивно|Нет активных конечных точек, но профиль все равно включен.|
...
>[AZURE.NOTE] Диспетчер трафика считает, что конечная точка в сети, только если возвращено сообщение 200 ОК. Если он получает ответ, отличный от 200, конечная точка считается недоступной, а проверка - неудачной.
...
В следующей таблице описывается поведение мониторинга дочернего и родительского профилей вложенного профиля и параметра minChildEndpoints. Дополнительные сведения см. в статье [Обзор диспетчера трафика](traffic-manager-overview.md).

|Отслеживаемое состояние дочернего профиля|Отслеживаемое состояние родительской конечной точки|Примечания|
|---|---|---|
|Отключено Это связано с тем, что вы отключили профиль.|Остановлено|Состояние родительской конечной точки - "Остановлено", а не "Отключено". Состояние "Отключено" указывает, что вы отключили конечную точку в родительском профиле.|
|Деградация Как минимум одна дочерняя конечная точка находится в состоянии "Деградация".|Состояние "В сети", если количество конечных точек в сети в дочернем профиле не меньше значения minChildEndpoints. Состояние "Проверка конечной точки", если количество конечных точек с состоянием "В сети" и "Проверка конечной точки" в дочернем профиле не меньше значения minChildEndpoints. В противном случае будет указано состояние "Деградация".|Трафик направляется к конечной точке в состоянии "Проверка конечной точки". Если задано слишком высокое значение minChildEndpoints, родительская конечная точка всегда будет иметь состояние "Деградация".|
|В сети Как минимум одна дочерняя конечная точка находится в состоянии "В сети" и ни одной точки нет в состоянии "Деградация".|То же, что и выше.||
...
|Неактивно Все конечные точки пребывают в состоянии "Отключено" или "Остановлено", или в этом профиле нет конечных точек|Остановлено||
...
3. На портале управления в поле **Относительный путь и имя файла** раздела **Параметры мониторинга** укажите путь и имя файла.
...
[Запись блога "Устранение неполадок с состоянием "Деградация" в диспетчере трафика"](http://go.microsoft.com/fwlink/p/?LinkId=328689) (на английском)
