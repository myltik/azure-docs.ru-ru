...
При настройке метода маршрутизации трафика для отработки отказа важен порядок выбранных конечных точек. На портале Azure на странице конфигурации профиля можно настроить порядок отработки отказа.
...
На данный момент настроить на портале Azure маршрутизацию трафика по приоритетам невозможно. Azure предоставляет программный доступ к этому методу с помощью связанного интерфейса REST API управления службами и командлетов Azure PowerShell.
