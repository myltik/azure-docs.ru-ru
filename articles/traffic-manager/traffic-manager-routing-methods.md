...
В диспетчере трафика доступно три метода маршрутизации трафика. Каждый профиль диспетчера трафика может использовать только один из методов маршрутизации, хотя в любое время можно выбрать другой метод для профиля.

Важно отметить, что все методы маршрутизации трафика используют мониторинг конечных точек. После настройки профиля диспетчера трафика с указанием метода маршрутизации, оптимально подходящего к вашим требованиям, настройте параметры мониторинга. Если мониторинг настроен правильно, диспетчер трафика будет наблюдать за состоянием конечных точек, состоящих из облачных служб и веб-сайтов, и не будет отправлять трафик в конечные точки, которые считает недоступными. Дополнительную информацию о мониторинге диспетчера трафика см. в разделе [О мониторинге диспетчера трафика](traffic-manager-monitoring.md).

Три метода маршрутизации трафика в диспетчере трафика:

- **Отработка отказа**: выберите "Отработка отказа" при наличии конечных точек в одном или разных центрах обработки данных Azure (т. е. регионах на классическом портале Azure), если нужно использовать основную конечную точку для всего трафика с резервированием в случае, если основная или резервная конечная точка будет недоступна. Дополнительные сведения см. в разделе [Метод маршрутизации трафика для отработки отказа](#failover-traffic-routing-method).

- **Циклический перебор**: выберите «Циклический перебор», если необходимо распределять нагрузку между несколькими конечными точками в одном или в разных центрах обработки данных. Дополнительные сведения см. в разделе [Метод маршрутизации трафика с применением циклического перебора](#round-robin-traffic-routing-method).

- **Производительность**: выберите «Производительность» при наличии конечных точек в разных географических расположениях, если нужно, чтобы запрашивающие клиенты использовали «ближайшие» с точки зрения минимальной задержки конечные точки. Дополнительные сведения см. в разделе [Метод маршрутизации трафика для повышения производительности](#performance-traffic-routing-method).

Обратите внимание, что Веб-сайты Azure уже обеспечивают маршрутизацию трафика с отработкой отказа и применением циклического перебора для веб-сайтов в центре обработки данных независимо от режима работы веб-сайта. Диспетчер трафика позволяет использовать отработку отказа и циклический перебор при маршрутизации трафика для веб-сайтов в разных центрах данных.
...
## Метод маршрутизации трафика для отработки отказа
...
При настройке метода маршрутизации трафика для отработки отказа важен порядок выбранных конечных точек. На классическом портале Azure на странице конфигурации профиля можно настроить порядок отработки отказа.

На рис. 1 показан пример метода маршрутизации трафика для отработки отказа для набора конечных точек.

![Диспетчер трафика: метод маршрутизации с отработкой отказа](./media/traffic-manager-routing-methods/IC750592.jpg)
...
## Метод маршрутизации трафика с применением циклического перебора

Общепринятый подход к маршрутизации трафика состоит в предоставлении набора идентичных конечных точек и отправке трафика в каждую из них по принципу циклического перебора. Циклический перебор разделяет трафик между различными конечными точками. Он выбирает работоспособную конечную точку случайным образом и не отправляет трафик недоступным службам. Дополнительную информацию см. в разделе [Мониторинг диспетчера трафика](../traffic-manager-monitoring.md).

На рис. 2 показан пример метода маршрутизации трафика с применением циклического перебора для набора конечных точек.

![Диспетчер трафика: метод маршрутизации с циклическим перебором](./media/traffic-manager-routing-methods/IC750593.jpg)
...
Метод маршрутизации трафика с применением циклического перебора также поддерживает распределение сетевого трафика по приоритетам. На рис. 3 показан пример того, как можно для набора конечных точек использовать метод маршрутизации трафика по приоритетам с применением циклического перебора.

![Метод взвешенной маршрутизации с циклическим перебором](./media/traffic-manager-routing-methods/IC750594.png)

**Рис. 3**

Маршрутизация трафика по приоритетам с применением циклического перебора позволяет распределять нагрузку по различным конечным точкам с учетом назначенного значения приоритета для каждой конечной точки. Чем больше вес, тем чаще будет возвращаться конечная точка. Этот метод будет полезен в указанных ниже сценариях.
...
На данный момент настроить на классическом портале Azure маршрутизацию трафика по приоритетам невозможно. Azure предоставляет программный доступ к этому методу с помощью связанного интерфейса REST API управления службами и командлетов Azure PowerShell.
...
Дополнительную информацию об использовании командлетов PowerShell Azure см. в разделе [Командлеты для диспетчера трафика Azure](http://go.microsoft.com/fwlink/p/?LinkId=400769). Пример конфигурации см. в разделе [Внешние конечные точки диспетчера трафика Azure и взвешенный циклический перебор с помощью PowerShell](https://azure.microsoft.com/blog/2014/06/26/azure-traffic-manager-external-endpoints-and-weighted-round-robin-via-powershell/) в блоге Azure.
...
Для маршрутизации трафика к конечным точкам, которые находятся в разных центрах данных по всему миру, можно направить входящий трафик на ближайшую по минимальной задержке конечную точку. Обычно «ближайшая» конечная точка одновременно является наиболее близкой географически. Метод маршрутизации трафика для повышения производительности позволяет осуществлять распределение с учетом расположения и задержки, но не может учитывать изменения конфигурации или нагрузки сети в режиме реального времени.

Маршрутизация трафика по производительности находит местоположение клиента и связывает его с ближайшей конечной точкой. «Близость» определяется таблицей интернет-задержек, в которой отражается время от отправки запроса до получения ответа между различными IP-адресами и каждым центром данных Azure. Эта таблица обновляется периодически и не отражает производительность работы через Интернет в реальном времени. Она не учитывает нагрузку на данную службу, хотя диспетчер трафика ведет мониторинг конечных точек на основе выбранного метода и не будет включать их в ответы на запросы DNS, если они не работают. Другими словами, маршрутизация трафика для повышения производительности включает также метод маршрутизации трафика для отработки отказа.

На рис. 4 показан пример метода маршрутизации трафика для повышения производительности для набора конечных точек.

![Диспетчер трафика: метод маршрутизации производительности](./media/traffic-manager-routing-methods/IC753237.jpg)
