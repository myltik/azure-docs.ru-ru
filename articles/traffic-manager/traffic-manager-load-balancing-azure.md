...
1. На портале Azure щелкните **Создать** и выполните в Marketplace поиск по запросу "профиль диспетчера трафика".
2. В колонке **Create Traffic Manager profile** (Создание профиля диспетчера трафика) введите основные сведения.
...
1. На портале Azure в области слева щелкните **Создать** > **Сеть** > **Шлюз приложений**.
...
3. В колонке **Добавить внутренний пул** введите имя пула и IP-адреса всех виртуальных машин, включенных в этот пул. В нашем примере мы подключаем два серверных пула виртуальных машин.
...
5. В колонке **Add path-based rule** (Добавление правила на основе пути) настройте правило, указав следующие сведения.
...
3. В колонке **Добавить конечную точку** создайте конечную точку, указав следующие сведения.
...
1. В области слева на портале Azure щелкните **Создать** > **Сеть** > **Подсистема балансировки нагрузки**.
2. В колонке **Create load balancer** (Создание балансировщика нагрузки) выберите имя для своего балансировщика.
...
3. В колонке **Добавить внутренний пул** введите имя серверного пула.
...
2. В колонке **Добавление пробы** введите имя пробы.
...
2. В колонке **Добавить правило балансировки нагрузки** введите **имя** для нового правила.
...
Теперь мы настроим IP-адрес и внешний порт балансировщика нагрузки в приложениях, которые работают на виртуальных машинах веб-уровня и будут подключаться к базам данных. Процесс настройки будет отличаться в зависимости от конкретного используемого приложения. Чтобы настроить IP-адрес назначения и порт, изучите документацию к приложению. Чтобы узнать IP-адрес внешнего интерфейса, на портале Azure перейдите в раздел Frontend IP pool (Пул внешних IP-адресов) в колонке **Параметры** балансировщика нагрузки.
