<properties urlDisplayName="custom dns storage" pageTitle="Настройка имени домена для BLOB-данных в учетной записи хранения | Microsoft Azure" metaKeywords="" description="Узнайте, как настроить пользовательский домен для доступа к BLOB-данным в учетной записи хранения Azure." metaCanonical="" services="storage" documentationCenter="" title="Configure a custom domain name for blob data in a storage account" solutions="" authors="tamram" manager="adinah" />
...
Можно настроить пользовательский домен для доступа к BLOB-данным в учетной записи хранения Azure. Конечной точкой по умолчанию для службы BLOB-объектов является https://<*mystorageaccount*>.blob.core.windows.net. После сопоставления пользовательского домена и поддомена, например **www.contoso.com**, конечной точке больших двоичных объектов в используемой учетной записи хранения пользователи также смогут, используя этот домен, получать доступ к данным больших двоичных объектов в учетной записи хранения. 


> [WACOM.NOTE]	Процедуры в этом задании применяются к учетным записям хранения Azure. Информацию об облачных службах см. в статье <a href = "/ru-ru/develop/net/common-tasks/custom-dns/">Настройка пользовательского доменного имени для облачной службы Azure</a>; о веб-сайтах - в статье <a href="/ru-ru/develop/net/common-tasks/custom-dns-web-site/">Настройка пользовательского имени домена для веб-сайта Azure</a>. 

> [WACOM.NOTE]	Учетную запись хранения Premium нельзя сопоставить с пользовательским именем домена. См. раздел [Служба хранилища Premium. Высокопроизводительная служба хранения для рабочих нагрузок виртуальных машин Azure](http://go.microsoft.com/fwlink/?LinkId=521898) для получения дополнительной информации об учетных записях хранения Premium.
...
В следующей таблице приведены примеры URL-адресов для доступа к данным больших двоичных объектов в учетной записи хранения с именем **mystorageaccount**. Пользовательский домен, зарегистрированный для этой учетной записи хранения: **www.contoso.com**.

<table border="1" cellspacing="0" cellpadding="5" style="border: 1px solid #000000;">
	<tbody>
		<tr>
			<td style="width: 100px;"><strong>Тип ресурса</strong></td>
			<td><strong>Форматы URL</strong></td>
		</tr>
		<tr>
...
- <a href="#register-domain">Регистрация пользовательского домена для используемой учетной записи хранения</a>
...
<h2><a name="register-domain"></a>Регистрация пользовательского домена для используемой учетной записи хранения</h2>
...
4.  В нижней части экрана щелкните **Управление доменом**, чтобы вывести диалоговое окно **Управление пользовательским доменом**. Верхняя часть диалогового окна содержит информацию о том, как создать запись CNAME, используя поддомен **asverify**.
...
10. Для перерегистрации своего пользовательского домена нажмите кнопку **Зарегистрировать**. 

	Если перерегистрация прошла успешно, появится сообщение **Ваш пользовательский домен активен**. 
...
пользовательский поддомен **photos.contoso.com**, сопоставляющий ее большому двоичному объекту в контейнере
**myforms**:
...
-   <a href="http://msdn.microsoft.com/ru-ru/library/windowsazure/gg680307.aspx">Сопоставление содержимого CDN с пользовательским доменом</a>
