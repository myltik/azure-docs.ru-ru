<properties linkid="manage-services-storage-custom-dns-storage" urlDisplayName="пользовательское dns-имя хранилища" pageTitle="Настройка имени домена для BLOB-данных в учетной записи хранения | Microsoft Azure" metaKeywords="" description="Узнайте, как настроить пользовательский домен для доступа к BLOB-данным в учетной записи хранения Azure." metaCanonical="" services="storage" documentationCenter="" title="Настройка пользовательского имени домена для BLOB-данных в учетной записи хранения" authors="tamram" solutions="" manager="mbaldwin" editor="cgronlun" />


# Настройка пользовательского имени домена для BLOB-данных в учетной записи хранения Azure
Можно настроить пользовательский домен для доступа к BLOB-данным в учетной записи хранения Azure. Конечной точкой по умолчанию для службы BLOB-объектов является https://<*mystorageaccount*>. blob.core.windows.net. После сопоставления пользовательского домена и поддомена, например **www.contoso.com**, конечной точке BLOB-объектов в используемой учетной записи хранения пользователи также смогут, используя этот домен, получать доступ к BLOB-данным в учетной записи хранения. 

<div class="dev-callout"> 
<b>Примечание</b> 
	<p>Процедуры в этом задании применяются к учетным записям хранения Azure. Сведения об облачных службах см. в статье <a href = "/ru-ru/develop/net/common-tasks/custom-dns/">Настройка пользовательского имени домена для облачной службы Azure</a>, о веб-сайтах — в статье <a href="/ru-ru/develop/net/common-tasks/custom-dns-web-site/">Настройка пользовательского имени домена для веб-сайта Azure</a>.</p> 
</div>
...
Каждый из регистраторов использует похожий, но немного отличный способ задания записи CNAME, хотя используется одна и та же концепция. Обратите внимание, что многие базовые пакеты регистрации домена не предлагают настройки DNS, поэтому может потребоваться обновить пакет регистрации домена, прежде чем создавать запись CNAME. 
...
9.	Установите флажок с надписью **Дополнительно: использовать поддомен "asverify" для предварительной регистрации моего пользовательского домена**. 
...
пользовательский поддомен **photos.contoso.com**, сопоставляющий ее BLOB-объекту в контейнере
**myforms**:
