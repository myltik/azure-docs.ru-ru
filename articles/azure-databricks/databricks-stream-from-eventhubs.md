...
description: Сведения об использовании Azure Databricks с помощью концентраторов событий для приема потоковой передачи данных из Twitter и чтения данных в режиме реального времени.
...
В этом руководстве вы узнаете, как соединить систему приема данных с Azure Databricks для потоковой передачи данных в кластер Apache Spark в режиме реального времени. Настройте систему приема данных в режиме реального времени с помощью концентраторов событий Azure, а затем подключите ее к Azure Databricks для обработки поступающих сообщений. Для получения доступа к потоку данных в режиме реального времени используйте API-интерфейсы Twitter, чтобы получать твиты в концентраторах событий. При наличии данных в Azure Databricks вы можете выполнять аналитические задания для дальнейшего анализа данных. В рамках этого руководства вы извлечете твиты с термином Azure.

На следующем снимке экрана показана блок-схема приложения.
...
Чтобы получить поток данных твитов в режиме реального времени, создайте приложение в Twitter. Следуйте инструкциям по созданию приложения Twitter и запишите значения, необходимые для выполнения заданий из этого руководства.
...
Чтобы запустить записную книжку, нажмите клавиши **SHIFT+ВВОД**. Вы увидите выходные данные, как в следующем фрагменте кода. Каждое событие в выходных данных — твит в режиме реального времени, полученный в концентраторах событий и содержащий слово Azure. 
...
Вот и все! Вы успешно выполнили потоковую передачу данных в концентраторы событий Azure с помощью Azure Databricks в режиме реального времени. Затем вы применили данные потоковой передачи для Apache Spark, используя соединитель концентраторов событий.
...
>[Руководство по оценке тональности сообщений при потоковой передаче данных с использованием Azure Databricks](databricks-stream-from-eventhubs.md)
