<properties pageTitle="Настройка программного RAID-массива на виртуальной машине под управлением Linux в Azure" description="Узнайте, как использовать mdadm для настройки RAID-массива на платформе Linux в Azure." services="virtual-machines" documentationCenter="" authors="szarkos" writer="szark" manager="timlt" editor=""/>
...
>[AZURE.NOTE] На виртуальных машинах сверхмалого размера не поддерживается присоединение более одного диска данных.  Подробные сведения о размерах виртуальных машин и поддерживаемом количестве дисков данных см. в разделе [Размеры виртуальных машин и облачных служб в Windows Azure](http://msdn.microsoft.com/ru-ru/library/windowsazure/dn197896.aspx).
...
В примере после выполнения этой команды будет создано новое RAID-устройство с именем **/dev/md127**. Заметьте также, что если эти диски данных были ранее частью другого RAID-массива, которого больше не существует, может потребоваться добавить параметр `--force` в команду  `mdadm`.
...
**Внимание!** Некорректное изменение файла /etc/fstab может привести к невозможности загрузить систему. Если вы не знаете, как правильно изменить этот файл, см. документацию по дистрибутиву. Кроме того, прежде чем вносить изменения, мы советуем создать резервную копию файла /etc/fstab.
