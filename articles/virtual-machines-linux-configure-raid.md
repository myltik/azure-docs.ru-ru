<properties linkid="manage-linux-howto-configure-raid" urlDisplayName="Настройка RAID в Linux" pageTitle="Настройка программного RAID-массива на виртуальной машине, работающей под управлением Linux в Azure" metaKeywords="raid в Azure, mdadm Azure, чередование дисков в Azure" description="Узнайте, как использовать mdadm для настройки RAID на основе Linux в Azure." metaCanonical="http://www.windowsazure.com/ru-ru/manage/linux/articles/virtual-machines-linux-configure-raid" services="virtual-machines" documentationCenter="" title="" authors="szark" solutions="" writer="" manager="" editor=""  />
...
Для настройки RAID-устройства обычно требуется не менее двух пустых дисков данных.  В этой статье мы не будем подробно останавливаться на том, как присоединить диски данных к виртуальной машине Linux.  Подробные инструкции по присоединению пустого диска данных к виртуальной машине Linux в Azure см. в статье Windows Azure [Присоединение диска](http://www.windowsazure.com/ru-ru/documentation/articles/storage-windows-attach-disk/#attachempty).
	...
	>[WACOM.NOTE] На виртуальных машинах сверхмалого размера не поддерживается присоединение более одного диска данных.  Подробные сведения о размерах виртуальных машин и поддерживаемом количестве дисков данных см. в разделе [Размеры виртуальных машин и облачных служб в Azure](http://msdn.microsoft.com/ru-ru/library/windowsazure/dn197896.aspx).
...
- Запустите fdisk, чтобы начать создание разделов
		...
		# sudo fdisk /dev/sdc
		Устройство не содержит ни таблицы разделов DOS, ни метки диска Sun, SGI или OSF
		Создается новая метка диска DOS с идентификатором диска 0xa34cb70c.
		Изменения останутся только в памяти, пока вы не решите их записать.
		После этого предыдущее содержимое нельзя будет восстановить.

		ВНИМАНИЕ! Не используйте совместимый с DOS режим. Настоятельно рекомендуется
				 отключить режим (команда "c") и изменить отображаемые единицы
				 на секторы (команда "u").
