...
-   **универсальная запись**: схема JSON задается явно в записи, представленной классом [**AvroRecord**][**AvroRecord**], когда отсутствуют типы .NET для описания схемы, чтобы сериализвать данные.
...
-   [Newtonsoft Json.NET][Newtonsoft Json.NET] (v5.0.5 или выше)
...
Образец с первыми четырьмя примерами можно загрузить с сайта [Примеры кода Azure][Примеры кода Azure].
...
-   [**Сериализация с помощью отражения**][**Сериализация с помощью отражения**]: Схема JSON для типов, которые необходимо сериализовать, создается автоматически с учетом атрибутов контракта данных.
-   [**Сериализация с помощью универсальной записи**][**Сериализация с помощью универсальной записи**]: Схема JSON задается явно в записи, когда типы .NET недоступны для отражения.
-   [**Сериализация с использованием файлов контейнеров объектов с помощью отражения**][**Сериализация с использованием файлов контейнеров объектов с помощью отражения**]: Схема JSON неявно сериализуется вместе с данными и совместно используется через файл контейнера Avro.
-   [**Сериализация с использованием файлов контейнеров объектов с помощью универсальной записи**][**Сериализация с использованием файлов контейнеров объектов с помощью универсальной записи**]: Схема JSON явно сериализуется вместе с данными и совместно используется через файл контейнера Avro.
-   [**Сериализация с использованием файлов контейнеров объектов с помощью настраиваемого кодека сжатия**][**Сериализация с использованием файлов контейнеров объектов с помощью настраиваемого кодека сжатия**]: Схема JSON неявно сериализуется вместе с данными и совместно используется через файл контейнера Avro с помощью настроенной реализации .NET кодека сжатия данных deflate.
...
Набор тестовых данных помещается в список объектов [**AvroRecord**][**AvroRecord**] с использованием явно определенной схемы JSON, а затем сохраняется как файл контейнера объекта, представленный классом [**AvroContainer**][**AvroContainer**]. Этот файл контейнера создает модуль записи, который используется для сериализации несжатых данных в поток в памяти, который затем сохраняется в файле. Это параметр [**Codex.Null**][**Codex.Null**], используемый для создания модуля чтения, который определяет, что эти данные не будут сжиматься.
...
В следующем примере показывается, как использовать настраиваемый кодек сжатия для файлов контейнеров объектов Avro. [Спецификация Avro][Спецификация Avro] позволяет использовать дополнительные кодеки сжатия (помимо **Null** и **Deflate**, которые используются по умолчанию). В примере не реализуется полностью полностью новый кодек Snappy (упоминаемый как поддерживаемый дополнительный кодек вn [Спецификации Аvro][Спецификации Аvro]). Здесь показано, как использовать реализацию .NET Framework 4.5 кодека [**Deflate**][2], которая обеспечивает выполнение лучшего алгоритма сжатия на основе библиотеки сжатия [zlib][zlib]m, чем используемая по умолчанию версия .NET Framework 4.0.
