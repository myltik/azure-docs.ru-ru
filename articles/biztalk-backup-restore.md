...
Службы Azure BizTalk предоставляют функции резервного копирования и восстановления. В этом разделе описывается процесс резервного копирования и восстановления служб BizTalk с помощью портала управления Azure, включая разделы:
...
Выполнять резервное копирование служб BizTalk также можно с помощью [API REST служб BizTalk][API REST служб BizTalk].

## <a name="beforebackup"></a>Перед началом работы

-   Резервное копирование и восстановление доступны не во всех выпусках, см. [Службы BizTalk: диаграмма выпусков][Службы BizTalk: диаграмма выпусков].

    **Примечание.** Вне зависимости от выпуска создавать резервные копии гибридных соединений невозможно.
...
-   Если в пакете имеются активные сообщения, запустите обработку пакета **до** резервного копирования. При создании резервной копии (запланированной или по требованию) сообщения в пакетах не сохраняются.
...
Процесс резервного копирования может выполняться в любое время и полностью управляется вами. В этом разделе приведены инструкции по созданию резервных копий посредством портала управления Azure, включая разделы:

[Резервное копирование по запросу][Резервное копирование по запросу]

[Планирование резервного копирования][Планирование резервного копирования]

#### <a name="backupnow"></a>Резервное копирование по требованию
...
2.  Во вкладке **Панель мониторинга** в верхней части страницы выберите **Резервное копирование**.
3.  Задайте имя резервной копии. Например, введите *моя\_служба\_BizTalk*РК*Дата*.
...
1. На портале управления Azure выберите пункт **Службы BizTalk**, а затем имя службы BizTalk, с помощью которой необходимо запланировать резервное копирование, и перейдите на вкладку **Настройка**.
...
При выполнении задания резервного копирования по расписанию в указанной учетной записи хранения создается контейнер, в котором хранятся данные резервной копии. Имя контейнера формируется следующим образом: *служба BizTalk имя-дата-время*.

Если на панели мониторинга службы BizTalk отображается состояние **Не удалось**:

![Состояние последней запланированной архивации][Состояние последней запланированной архивации]

После перехода по ссылке отображаются журналы операций службы управления для устранения неполадок, см. [Службы BizTalk: устранение неполадок с помощью журналов операций][Службы BizTalk: устранение неполадок с помощью журналов операций].

## <a name="restore"></a>Восстановление

Резервные копии можно создавать с помощью портала управления Azure или [API REST восстановления служб BizTalk][API REST восстановления служб BizTalk]. В этом разделе приведены инструкции по восстановлению с помощью портала управления.
...
-   Перед тем как восстанавливать службу BizTalk под тем же именем, необходимо удалить существующую службу BizTalk. В противном случае восстановление завершается ошибкой.
...
1.  Выберите на портале управления Azure пункт меню **Создать** \> **Службы приложений** \> **Служба BizTalk** \> **Восстановить**.

    ![Восстановление из резервной копии][1]

2.  В поле **URL-адрес резервной копии** выберите значок папки и разверните учетную запись хранилища Azure, в которой сохранена резервная копия конфигурации службы BizTalk. Разверните контейнер и на панели справа выберите TXT-файл, соответствующий резервной копии.
    Нажмите **Открыть**.

3.  На странице **Восстановить службу BizTalk** введите **Имя службы BizTalk** и проверьте поля **URL-адрес домена**, **Выпуск** и **Регион** для восстановленной службы BizTalk. **Создать новый экземпляр базы данных SQL** для базы данных отслеживания.
...
6.  Установите флажок, чтобы запустить восстановление.
...
Восстановленные службы BizTalk всегда имеют статус **Приостановлено**. В этом состоянии можно вносить любые изменения в конфигурацию до того, как новая среда будет переведена в рабочий режим, включая:
...
-   Возможно, вам потребуется восстановить службу BizTalk, чтобы реплицировать существующую рабочую среду службы BizTalk. В таком случае, если у вас настроены соглашения для исходного портала служб BizTalk, использующего в качестве источников папку FTP, вам может потребоваться обновить соглашения в восстановленной среде для работы с другой исходной папкой FTP. В противном случае возможны ситуации, когда одно и то же сообщение будут пытаться извлечь одновременно два соглашения.
...
Для запуска службы BizTalk из портала управления Azure выберите восстановленную службу BizTalk и выберите **Продолжить** на панели задач.
...
<th>
Элементы, включаемые в резервную копию

</th>
</tr>
<tr>
<td colspan="2">
<b>Портал служб Azure BizTalk<b\>
...
<td>
При создании службы BizTalk вводятся сведения о базе данных отслеживания, включая ее имя и сервер базы данных SQL Azure. Автоматическое резервное копирование базы данных отслеживания не выполняется.
<br/><br/>
<strong>Важно!</strong><br/>
Восстановление удаленной базы данных отслеживания возможно только в том случае, если создана ее резервная копия. В противном случае восстановить эти данные не получится. В такой ситуации вам потребуется создать новую базу данных отслеживания с тем же именем. Рекомендуется использовать георепликацию.
...
Сведения о создании служб BizTalk Windows Azure на портале управления Windows Azure см. в разделе [Службы BizTalk: подготовка с использованием портала управления Azure][Службы BizTalk: подготовка с использованием портала управления Azure]. Чтобы приступить к созданию приложений, перейдите по ссылке [Службы BizTalk Azure][Службы BizTalk Azure].

## См. также

-   [Резервное копирование службы BizTalk][API REST служб BizTalk]
-   [Восстановление службы BizTalk из резервной копии][API REST восстановления служб BizTalk]
...
-   [Как приступить к работе с пакетом SDK для служб BizTalk Azure][Как приступить к работе с пакетом SDK для служб BizTalk Azure]
  ...
  [API REST служб BizTalk]: http://go.microsoft.com/fwlink/p/?LinkID=325584
