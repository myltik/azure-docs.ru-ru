...
# Создание нового API SharePoint Server в среде службы приложений организации
...
[Прокси приложение Azure (прокси AAD)](../active-directory-application-proxy-publish.md) позволяет это реализовать. Это общедоступная служба Azure, которая защищает удаленный доступ и единый вход в локальные веб-приложения. Действия по включению прокси AAD детально описываются в библиотеке MSDN. В общем они состоят из следующих этапов.

1. [Включение служб прокси приложений](../active-directory-application-proxy-enable.md):  

	- включение прокси приложения в Azure AD;
	- установка и регистрация соединителя прокси приложения Azure.

2. [Публикация приложений с помощью прокси приложения](../active-directory-application-proxy-publish.md):
...
После создания прокси-приложения нужно создать другое приложение AAD, которое делегирует права прокси-приложению. Это нужно для получения маркера доступа и обновления маркера, которые требуются для потока согласия с условиями использования. Вы можете создать новое приложение AAD, следуя [этим инструкциям](../active-directory-integrating-applications.md).
