...
1. Откройте Visual Studio и создайте новое веб-приложение C\# ASP.NET:
![](./media/powerapps-web-api-tutorial/newwebapp.png "Новое веб-приложение")

2. На следующем экране укажите шаблон веб-API и выберите **Без проверки подлинности**:
![](./media/powerapps-web-api-tutorial/noauth.png "Без проверки подлинности")
...
4. Затем создайте файл Swagger для веб-API. Это легко сделать, открыв __консоль диспетчера пакетов__ и установив __Swashbuckle__:
![](./media/powerapps-web-api-tutorial/swashbuckle-console.png "Консоль Swashbuckle")

5. После установки перейдите к следующим документам Swagger и конечным точкам пользовательского интерфейса соответственно: **\<Корневой\_URL-адрес\>/swagger/docs/v1**

 	**\<Корневой\_URL-адрес\>/swagger**

6. Когда веб-API будет готово, опубликуйте его в Azure. Для публикации в Visual Studio выберите **СБОРКА** и затем **ОПУБЛИКОВАТЬ** .
...
Файл swagger, используемый в этом учебнике, можно скачать [отсюда][6]. Не забудьте заменить или удалить комментарии перед его использованием. Комментарии начинаются с `//`.

## Шаг 2. Настройка проверки подлинности AAD

В этом учебнике предполагается, что вы знаете, как создать приложение AAD в Azure. Дополнительные сведения о том, как создать приложение AAD, приведены в разделе [Учебник по Azure Resource Manager](powerapps-azure-resource-manager-tutorial.md). В этом учебнике нам потребуются два приложения AAD.
...
3. Для **webAPI** используйте следующую конфигурацию:  
  ...
  2. URI идентификатора приложения: ***https://\\<Корневой\_URL-адрес\>*** \(обычно это URL-адрес вашего сайта, развернутого в Azure\)
...
4. Для **webAPI-customAPI** используйте следующую конфигурацию:
  ...
  1. URL-адрес входа: **https://login.windows.net**
  2. URI идентификатора приложения: ***любой уникальный URL-адрес***
...
1. Войдите в [портал Azure](https://portal.azure.com) и перейдите к веб-приложению, развернутому на **шаге 1** \(в этом разделе\).
2. В разделе **Параметры** выберите **Аутентификация или авторизация**.
3. Включите **Проверка подлинности у службы приложений** и выберите **Azure Active Directory**. В следующей колонке выберите **Express**.  
4. Щелкните **Выбрать существующее приложение AD** и выберите первое приложение AAD, созданное на шаге 2. В данном случае это **webAPI**.
...
2. Перейдите на [веб-портал][1] PowerApps и добавьте пользовательский API. Необходимые шаги описаны в разделе [Пользовательские API в логических потоках и PowerApps](powerapps-register-custom-api.md).
...
4. Настройте проверку подлинности AAD для пользовательского API:
  ...
  1. Идентификатор клиента: ***идентификатор клиента для webAPI-CustomAPI*** из пункта 4.5 на **шаге 2** \(в этом разделе\)
  2. Секрет: ***ключ для webAPI-CustomAPI*** из пункта 4.6 на **шаге 2** \(в этом разделе\)
  3. URL-адрес входа: ***https://login.windows.net***
  4. ResourceUri: ***идентификатор клиента для веб-API*** из пункта 3.4 на **шаге 2** \(в этом разделе\)

5. Выберите **Создать** и попробуйте создать подключение в пользовательском API. Если все настроено правильно, то вход должен завершиться успешно.
