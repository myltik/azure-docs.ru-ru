<properties
	pageTitle="Как настроить определение Swagger для PowerApps и логических потоков | Microsoft Azure"
...
Чтобы узнать, как добавить Swagger в свой веб-API, см. раздел [Swashbuckle][1].
...
##### x-ms-summary
Строка, описывающая отображаемые имена для сущностей, у которых нет поля `summary`, определенного в спецификации Swagger. Например, это могут быть **имена параметров**.
...
В SalesForce имеется множество встроенных объектов. `x-ms-dynamic-values` помогает конструктору определить список встроенных объектов Salesforce. Он получает список путем вызова `TableMetadata_ListTables`.
