...
|Модель контроля доступа|Делегирование доступа через маркеры SAS|Управление доступом на основе ролей через ACS|
...
- В службе Microsoft Azure Active Directory Access Control (Access Control Service, ACS), поддерживаемой служебной шиной, предусмотрены три роли, которые в настоящее время не поддерживаются для очередей Azure: **Admin**, **Sender** и **Receiver**.

- Служебная шина поддерживает интеграцию со службой ACS и позволяет создавать федерации с Active Directory (посредством ADFS) и популярными поставщиками веб-удостоверений.
...
|Критерии сравнения|Очереди Azure|Очереди служебной шины|
|---|---|---|
|Стоимость транзакции с очередью|**0,0005 долл. США**<br/><br/>(за 10 000 транзакций)|**Уровень Basic**: **0,05 долл. США**<br/><br/>(за миллион операций)|
...
- Стоимость транзакций ACS при выполнении операций с сообщениями в очередях служебной шины незначительна. Служебная шина получает один маркер ACS на один экземпляр объекта фабрики обмена сообщениями. Этот маркер используется, пока не истечет срок его действия (около 20 минут). Следовательно, объем операций обмена сообщениями в служебной шине не является прямо пропорциональным количеству транзакций ACS, необходимых для выполнения этих операций.
