...
Подобная процедура описана на сайте: [<https://developer.amazon.com/sdk/adm/credentials.html>].
...
Если вы уже используете [клиентскую библиотеку ADM], а также уже [интегрировали ADM], можно переходить прямо к команде android-sdk-adm-receive.
...
-   После добавление тега Amazon может возникнуть ошибка построения, если конечная сборка проекта ниже версии Android 2.1. Необходимо использовать конечное построение выше версии **Android 2.1** (не беспокойтесь, вы сможете получить `minSdkVersion`, равную 4).
...
Для того, чтобы передать идентификатор регистрации устройства службе Push-уведомлений платформы Engagement и получать ее уведомления, необходимо добавить следующий код в файл `AndroidManifest.xml` внутри тега `<application/>` (даже в случае использования ADM без платформы Engagement):
...
[Клиентская библиотека ADM]:https://developer.amazon.com/sdk/adm/setup.html
[Интегрированная служба ADM]:https://developer.amazon.com/sdk/adm/integrating-app.html
[Эта процедура]:https://developer.amazon.com/sdk/adm/integrating-app.html#Asset
