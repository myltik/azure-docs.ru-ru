...
Чтобы задать две или более констант одновременно, заключите имена идентификаторов и значения в скобки и разделите их, используя точки с запятой, как в этом примере:  

    Const { X = 28; Y = 4; }  

Правая сторона каждого выражения назначения может быть целым числом, вещественным числом, логическим значением (True или False) или математическим выражением, как в этом примере:  

	Const { X = 17 * 2; Y = true; }  

##Объявление слоев
Слои нужно обязательно объявить. При этом определяется размер и источник слоя, в том числе его пакеты подключений и атрибуты. Оператор объявления начинается с имени слоя (входной, скрытый или выходной), за которым следуют измерения слоя (кортеж положительных целых чисел). Например:  

	input Data[784];
	hidden Hidden[5,20] from Data all;
	output Result[2] from Hidden all;  

-	Произведение измерений представляет собой количество узлов слоя. В нашем примере указано два измерения [5,20]. Это означает, что в слое содержится  100 узлов.
-	Слои могут объявляться в любом порядке с одним исключением: если определяется более одного входного слоя, порядок, в котором они объявляются, должен соответствовать порядку признаков во входных данных.  

<!-- ЭТО СОДЕРЖИМОЕ УДАЛЕНО И БУДЕТ ВОССТАНОВЛЕНО ПОСЛЕ РЕАЛИЗАЦИИ ПОДДЕРЖКИ ЭТОЙ ФУНКЦИИ В ПРОДУКТЕ
...
-	В случае объявления скрытого слоя, количество узлов определяется числом, заданным значением параметра **Количество скрытых узлов**. 
...
Определение обучаемого слоя (скрытых или выходных слоев) может произвольно содержать выходную функцию (называемую также функцией активации). По умолчанию используется функция **sigmoid**. Поддерживаются следующие выходные функции:  
...
Сразу после определения обучаемого слоя необходимо объявить подключения среди слоев, которые были определены. Пакет подключений начинается с ключевого слова **from**, за которым следует имя слоя источника пакета и указывается вид пакета подключений, который необходимо создать.   
...
-	В предикате для ByRow **s** - параметр, представляющий индекс прямоугольного массива узлов входного слоя Pixels, а **d** - параметр, представляющий индекс массива узлов скрытого слоя ByRow. Тип обоих параметров **s** и **d** представляет собой кортеж целых чисел с длиной два. Концептуально **s** находится в диапазоне всех пар целых чисел при 0 <= s[0] < 10 и 0 <= s[1] < 20, в то время как **d** находится в диапазоне всех пар целых чисел при 0 <= d[0] < 10 и 0 <= d[1] < 12. 
-	На правой стороне выражения предиката находится условие. В данном примере для каждого значения **s** и **d**, условие которого имеет значение True, предусмотрен переход из узла слоя источника в узел слоя назначения. Таким образом, это выражение фильтра указывает, что пакет содержит подключение от узла, определенного параметром **s**, к узлу, определенному параметром **d**, во всех случаях, где s[0] равно d[0].  
...
-	Если ядро содержит нейроны в той же пространственной позиции, что и ***x***, но нейроны находятся в других картах, схема нормализации называется **межкартовой нормализацией**. Этот тип нормализации ответа реализует форму латерального торможения, похожую на форму, найденную у реальных нейронов, создавая конкуренцию за большие уровни активации среди выходов нейронов, вычисленных на различных картах. Чтобы задать межкарточную нормализацию, первая координата должна быть целым числом (больше единицы и не больше количества карт), а остальные координаты должны иметь значения, равные 1.  
...
**Alert** - узлы в слое назначения соответствуют нейронам, которые представляют собой центральные узлы ядер. Например, если KernelShape[d] имеет нечетное значение, то KernelShape[d]/2 соответствует центральному узлу ядра. Если значение KernelShape[d] четное, центральный узел находится в KernelShape[d]/2 - 1. Таким образом, если для атрибута **Padding**[d] задано значение False, первый и последний узлы KernelShape[d]/2 не имеют соответствующих узлов в слое назначения. Чтобы исключить эту ситуацию, задайте атрибут **Padding** как [true, true, ..., true].  

В дополнение к четырем атрибутам, описанным выше, пакеты нормализации ответов также поддерживают следующие атрибуты.  

-	**Alpha** - (обязательный) определяет значение с плавающей запятой, соответствующее ***Î±*** в указанной выше формуле. 
...
-	Значение **KernelShape** указывает, что это тот же слой нормализации карты, в котором окрестность представляет собой прямоугольник размером 3 x 3. 
...
-	Первая строка определяет входной слой (с именем Data), имеющий 100 узлов, каждый из которых представляет собой признак во входных примерах.
...
-	Скрытые слои ByRow и ByCol определяют фильтрованные подключения с использованием выражений предиката. Если говорить точнее, то узел в ByRow с координатами [x, y] подключается к тем узлам в Pixels, у которых первая координата индекса равна первой координате узла (x). Аналогичным образом узел ByCol с координатами [x, y] подключается к тем узлам в Pixels, у которых вторая координата индекса находится в пределах единицы от второй координаты узла (у).  
...
-	Количество весов на ядро равно: 1 + **KernelShape**\[0] * **KernelShape**\[1] * **KernelShape**\[2] = 1 + 1 * 5 * 5 = 26. Или 26 * 50 = 1300.
