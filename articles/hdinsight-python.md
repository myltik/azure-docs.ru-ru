...
Для выполнения этих действий используется Windows Azure PowerShell. Если вы еще не установили и не настроили его на компьютере для разработки, см. статью [Установка и настройка Azure PowerShell](http://azure.microsoft.com/ru-ru/documentation/articles/install-configure-powershell/) перед выполнением следующих действий.
...
2. Используйте   следующий скрипт PowerShell для загрузки файлов **streaming.py** и **jython.py** на сервер. Подставьте имя вашего кластера Azure HDInsight и путь к файлам **streaming.py** и **jython.py** в первые три строки скрипта.
	...
	> [AZURE.NOTE] Другие способы загрузки скриптов можно найти в документе [Отправка данных для заданий Hadoop в HDInsight](/ru-ru/documentation/articles/hdinsight-upload-data/).
