<properties urlDisplayName="How to configure" pageTitle="Как настроить облачную службу - Azure" metaKeywords="Настройка облачных служб" description="Узнайте, как настроить облачные службы в Azure. Как обновить конфигурацию облачной службы и настроить удаленный доступ к экземплярам роли." metaCanonical="" services="cloud-services" documentationCenter="" title="How to Configure Cloud Services" authors="davidmu" solutions="" manager="timlt" editor="" />
...
<h1><a id="configurecloudservice"></a>Как настроить облачную службу</h1>
...
При наличии как минимум двух экземпляров для каждой роли в процессе обновления конфигурации Azure обеспечивается доступность службы в течение 99,95 % времени. Такая конфигурация позволяет обрабатывать запросы клиентов на одной виртуальной машине во время обновления другой. Дополнительные сведения см. в [Соглашениях об уровне обслуживания](https://www.windowsazure.com/ru-ru/support/legal/sla/).
...
1. На [портале управления Azure](http://manage.windowsazure.com/) нажмите **Облачные службы**, выберите имя облачной службы, а затем пункт **Настроить**.
...
2. В разделе **Мониторинг** задайте уровень мониторинга (от минимального до подробного) и настройте диагностические строки подключения, которые необходимы для подробного мониторинга. Инструкции см. в разделе [Как производить мониторинг облачных служб](../how-to-monitor-a-cloud-service/).


3. Для ролей службы, сгруппированных по ролям, можно обновить следующие параметры:

	>- **Настройки**   Здесь можно изменить различные параметры конфигурации, определенные элементами *ConfigurationSettings* в файле конфигурации службы (CSCFG).
...
2. Чтобы применить обновления конфигурации, передайте новый файл в службу:
	...
	Если для каждой роли не определены как минимум два экземпляра, Azure не гарантирует доступность службы в течение 99,95 % времени в процессе обновления конфигурации. Дополнительные сведения см. в [Соглашениях об уровне обслуживания](http://www.windowsazure.com/ru-ru/support/legal/sla/).
...
###Включение или изменение удаленного доступа для экземпляров ролей на портале управления###
...
###Отключение удаленного доступа для экземпляров ролей на портале управления###
