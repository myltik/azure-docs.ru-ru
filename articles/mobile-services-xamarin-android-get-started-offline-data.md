<properties pageTitle="Использование автономных данных в мобильных службах (Xamarin Android) | Центр мобильных разработок" description="Узнайте, как использовать мобильные службы Azure для кэширования и синхронизации автономных данных в приложении на Xamarin для Android" documentationCenter="xamarin" authors="lindydonna" editor="wesmc" manager="dwrede" services=""/>
...
В этом разделе рассматривается код автономной синхронизации в `ToDoActivity.cs`.
...
3. Прежде чем можно будет выполнить операции с таблицами, необходимо инициализировать локальное хранилище. Это можно сделать в методе `InitLocalStoreAsync`:
...
5. В классе `ToDoActivity` метод `SyncAsync()` вызывается после операций, которые изменяют данные - `AddItem()` и `CheckItem()`. Он также вызывается из `OnRefreshItemsSelected()`, поэтому пользователи получают последние данные при каждом нажатии кнопки **Обновить**. Приложение также выполняет синхронизацию при запуске, поскольку `ToDoActivity.OnCreate()` вызывает `OnRefreshItemsSelected()`.
