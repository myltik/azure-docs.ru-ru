...
Размер виртуальной памяти образов VHD в Azure должен быть округлен до 1 МБ. Как правило, размер VHD, созданных с помощью Hyper-V, настроен правильно. Если размер VHD настроен не правильно, при попытке создать  *образ* вашего VHD может возникнуть следующее сообщение об ошибке:
...
Если вы работаете не в среде Windows, мы советуем использовать qemu-img, чтобы (при необходимости) преобразовать и изменить размера VHD:
 ...
 2. Рассчитайте необходимый размер образа диска, чтобы округлить размер виртуальной памяти до 1 МБ. Это можно сделать с помощью следующего сценария оболочки Bash. В сценарии используется `qemu-img info`, который определяет размер виртуальной памяти образа диска, а затем добавляет к нему 1 МБ.
