...
-   [Карты Bing][Карты Bing]
-   [Геокодирование адресов на C# с использованием API потока данных Карт Bing][Геокодирование адресов на C# с использованием API потока данных Карт Bing]
-   [Добавление картографических данных в приложение MVC4 с использованием Поиска Azure и Карт Bing][Добавление картографических данных в приложение MVC4 с использованием Поиска Azure и Карт Bing]
-   [Обзор проекта AdventureWorksWebGeo][Обзор проекта AdventureWorksWebGeo]
...
Чтобы узнать больше, просмотрите это видео с сайта Channel 9 о [Поиске Azure и геопространственных данных][Поиске Azure и геопространственных данных].

![][]

Чтобы создать приложение, мы с помощью картографической службы Bing выполним геокодирование адресов, загруженных из CSV-файла, а затем сохраним полученные данные в индексе поиска.

Этот учебник составлен на основе демонстрации [Поиск Azure: Adventure Works][Поиск Azure: Adventure Works]. Если вы еще не ознакомились с ней, рекомендуем сделать это, чтобы получить общее представление о создании индекса и вызове API Поиска Azure из веб-приложения.
...
-   Служба "Поиск Azure". Вам понадобятся имя службы поиска и ключ администратора. Дополнительные сведения см. в разделе [Настройка поиска на портале предварительной версии Azure][Настройка поиска на портале предварительной версии Azure].
-   Служба "Карты Bing" и ключ для доступа к ней. Инструкции приведены в следующем разделе.
-   [Поиск Azure: пример использования геопространственного поиска на CodePlex][Поиск Azure: пример использования геопространственного поиска на CodePlex]. На вкладке Source Code (Исходный код) нажмите **Download** (Загрузить), чтобы скачать ZIP-архив с решением.
...
-   **AdventureWorksWebGeo** — это приложение на базе MVC4, которое отправляет запросы к индексу Поиска Azure и отображает местоположение магазинов на Картах Bing.

[WACOM.INCLUDE [Для работы с этим учебником необходима учетная запись Azure:][Для работы с этим учебником необходима учетная запись Azure:]]
...
1.  Перейдите на [портал Карт Bing][портал Карт Bing] и создайте учетную запись. Для этого нужно ввести учетные данные.
...
Эти данные поступают из CSV-файла с именем store\_locations.csv, который содержится в загруженном ранее источнике. Если вы откроете этот файл в текстовом редакторе или Excel, то увидите, что он содержит столбцы идентификатора, названия и адреса для каждого магазина.

Давайте подробнее рассмотрим код, чтобы понять, как он работает.

1.  Откройте решение AdventureWorksGeo в Visual Studio, разверните проект **StoreIndexer** в обозревателе решений и откройте файл Program.cs. Мы уже обсуждали создание индекса в описании демонстрации [Поиск Azure: Adventure Works][2], поэтому нет необходимости пояснять, как это реализовано в Program.cs.
...
3.  **ApplyStoreData** загружает данные из CSV-файла с именем store\_locations.csv в таблицу System.Data.DataTable.
...
4.  Перейдите к функции **ExtractAddressInfoToXML()**. Она извлекает данные об адресах из файла store\_locations.csv и загружает их в XML-файл в формате, который поддерживается Картами Bing для геокодирования. Созданный файл будет отправлен на обработку в поток данных Карт Bing с помощью вызова функции **GeoCoding.CreateJob**.

5.  Геокодирование может занять некоторое время, поэтому предусмотрен цикл, каждые десять секунд вызывающий функцию **GeoCoding.CheckStatus**, чтобы проверить, выполнено ли задание. После его завершения результаты загружаются в класс адресов с помощью вызова функции **GeoCoding.DownloadResults**.

6.  Последний шаг — получение геокодированных адресов и их отправка в Поиск Azure. Чтобы посмотреть, как это происходит, откроем функцию **UpdateStoreData**.

   **UpdateStoreData** с помощью действия **@search.action: merge** добавляет в поле местоположения типа Edm.GeographyPoint геокодированные координаты (долготу и широту), которые только что были загружены из Карт Bing. Для этого функция ищет уникальный ключ документа storeId в индексе stores и объединяет эти новые данные с существующим документом.

1.  Перед запуском приложения добавьте свои данные для службы "Поиск Azure" и API Карт Bing. Для этого откройте файл App.config и введите нужные значения SearchServiceName, SearchServiceApiKey и BingMapsAPI. Если ваша служба поиска называется mysearch.search.windows.net, то в качестве имени службы нужно ввести mysearch.

2.  Щелкните проект **StoreIndexer** правой кнопкой мыши и выберите пункты **Отладка** | **Запустить новый экземпляр**, чтобы начать его выполнение.
...
Обратите внимание: теперь магазины отображаются в виде точек на карте. Щелкните одну из них, и вы увидите всплывающее окно с информацией о магазине. Все эти данные загружаются из индекса Поиска Azure с именем stores, который был создан на предыдущих этапах.
...
3.  В этом файле реализована обычная процедура добавления Карт Bing в веб-приложение, однако вам стоит обратить внимание на некоторые моменты, указанные ниже.
   ...
   - ViewBag из контроллера загружает учетные данные для доступа к карте с помощью команды credentials: '@ViewBag.BingAPI'.
...
1.  Откройте файл HomeController.cs в папке **Controllers** (Контроллеры) и взгляните на функцию **Search**. Обратите внимание на вызов \_storeSearch.Search(lat, lon, 10000). Такой запрос возвращает все магазины в пределах 10 тыс. км от точки с указанной широтой (lat) и долготой (lon). Результаты этого запроса обрабатываются, а затем отправляются обратно в представление Index, где они отображаются в виде вешек на Картах Bing.
...
<!--Image references-->
  ...
  [Карты Bing]: #sub-2
  [Геокодирование адресов на C# с использованием API потока данных Карт Bing]: #sub-3
  [Добавление картографических данных в приложение MVC4 с использованием Поиска Azure и Карт Bing]: #sub-4
  [Обзор проекта AdventureWorksWebGeo]: #sub-5
  [Дальнейшие действия]: #next-steps
  [Поиске Azure и геопространственных данных]: http://channel9.msdn.com/Shows/Data-Exposed/Azure-Search-and-Geospatial-Data
  []: ./media/search-create-geospatial/AzureSearch-geo1-App.PNG
  [Поиск Azure: Adventure Works]: http://azuresearchadventureworksdemo.codeplex.com
  [Центра загрузки]: http://www.microsoft.com/en-us/download/details.aspx?id=40747
  [Настройка поиска на портале предварительной версии Azure]: ../search-configure/
  [Поиск Azure: пример использования геопространственного поиска на CodePlex]: https://azuresearchgeospatial.codeplex.com/
  ...
  [портал Карт Bing]: https://www.bingmapsportal.com/
