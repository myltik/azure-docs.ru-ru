...
+ **AdventureWorksWebGeo** - приложение на базе MVC4, которое отправляет запросы к индексу Поиска Azure и отображает местоположение магазинов на Картах Bing.

[WACOM.INCLUDE [Для работы с этим учебником требуется учетная запись Azure.](../includes/free-trial-note.md)]


<h2 id="sub-2">Карты Bing</h2>
...
<h3>Создание учетной записи для Карт Bing</h3>
...
2. Когда учетная запись создана, выберите пункт **Создать или просмотреть ключи** и введите данные для создания ключа.  Для этого демонстрационного проекта можно выбрать пункт **Ключ пробной версии**.

3. Нажмите **Отправить**. После этого вы должны увидеть сведения о ключе для своего приложения на базе Карт Bing. Запишите этот ключ, так как он понадобится позже.

<h2 id="sub-3">Геокодирование адресов на C# с использованием API потока данных Карт Bing</h2>
...
Эти данные поступают из CSV-файла с именем store_locations.csv, который содержится в загруженном ранее источнике. Если открыть этот файл в текстовом редакторе или Excel, то видно, что он содержит столбцы идентификатора, названия и адреса для каждого магазина.

Давайте подробнее рассмотрим код, чтобы понять, как он работает.

1. Откройте решение AdventureWorksGeo в Visual Studio, разверните проект **StoreIndexer** в обозревателе решений и откройте файл Program.cs. Поскольку мы уже рассмотрели создание индекса поиска демонстрации [Azure - Adventure Works](http://azuresearchadventureworksdemo.codeplex.com/), пропустим обсуждение по этому вопросу в файле Program.cs.
...
3. **ApplyStoreData** загружает данные из CSV-файла с именем store_locations.csv в таблицу System.Data.DataTable.  
...
7. Последний шаг - получение геокодированных адресов и их отправка в Поиск Azure. Чтобы посмотреть, как это происходит, откроем функцию **UpdateStoreData**.
  ...
  **UpdateStoreData** использует действие **@search.action: merge**, чтобы добавить в поле местоположения типа Edm.GeographyPoint геокодированные координаты (долготу и широту), только что загруженные из Карт Bing. Для этого функция ищет уникальный ключ документа storeId в индексе stores и объединяет эти новые данные с существующим документом.

8. Перед запуском приложения добавьте свои данные для службы Поиска Azure и API Карт Bing. Для этого откройте файл App.config и введите нужные значения SearchServiceName, SearchServiceApiKey и BingMapsAPI. Если ваша служба поиска называется "mysearch.search.windows.net", то в качестве имени службы нужно ввести "mysearch".
...
<h2 id="sub-4">Добавление картографических данных в приложение MVC4 с использованием Поиска Azure и Карт Bing</h2>
...
3. Откройте файл Web.config в этом проекте и присвойте параметрам SearchServiceName, SearchServiceApiKey и BingMapsAPI свои значения для доступа к службе Поиска Azure и к API Карт Bing. Если ваша служба поиска называется "mysearch.search.windows.net", то в качестве имени службы нужно ввести "mysearch".
...
<h2 id="sub-5">Обзор проекта AdventureWorksWebGeo</h2>
...
1. В обозревателе решений разверните узел **AdventureWorksWebGeo** | **Controller** и откройте файл HomeController.cs. Функция **Index()** вызывается при запуске приложения и загрузке страницы индекса. Эта функция загружает интерфейс API Карт Bing на основе данных из файла Web.config и передает его в представление Index в качестве ViewBag.BingAPI.

2. Откройте файл Index.cshtml в разделе **Представления** | **Главное**.
...
4. Откройте файл HomeController.cs в папке **Controllers** и взгляните на функцию **Search**. Обратите внимание на вызов _storeSearch.Search(lat, lon, 10000). Такой запрос возвращает все магазины в пределах 10 тыс. км от точки с указанной широтой (lat) и долготой (lon). Результаты этого запроса обрабатываются, а затем отправляются обратно в представление Index, где они отображаются в виде вешек на Картах Bing.
...
3. Когда вам будет предложено перезагрузить файл, выберите **Да для всех**.
