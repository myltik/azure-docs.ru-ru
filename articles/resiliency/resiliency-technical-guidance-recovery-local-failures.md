...
Во время создания для каждой базы данных задается предельное значение размера. В настоящее время максимальный размер составляет 150 ГБ. Если база данных достигает максимального размера, последующие команды вставки и обновления отклоняются (запрос и удаление данных по-прежнему возможны).
...
При временном сбое мобильных служб может потребоваться временно использовать альтернативный центр обработки данных Azure. Дополнительные сведения см. в разделе о [мобильных службах в рамках решения по аварийному восстановлению](./resiliency-technical-guidance-recovery-loss-azure-region.md#mobile-services).
...
###База данных SQL
  ...
  1. Просмотрите раздел [База данных](#sql-database) этой статьи.
