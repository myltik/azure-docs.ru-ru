...
Это руководство описывает, как запускать фоновые и интерфейсные компоненты в облачной службе Azure. Альтернативой является запуск интерфейсной части [веб-сайта Azure](/ru-ru/services/web-sites/) и использование функции [WebJobs](http://go.microsoft.com/fwlink/?LinkId=390226) (в настоящее время в предварительной версии) для серверной части. Учебник, где используются веб-задания, см. в разделе [Начало работы с Azure WebJobs SDK](/ru-ru/documentation/articles/websites-dotnet-webjobs-sdk-get-started/). О том, как выбрать службы, наилучшим образом отвечающим требованиям сценария, см. в разделе [Сравнение веб-сайтов Azure, облачных служб и виртуальных машин](http://azure.microsoft.com/ru-ru/documentation/articles/choose-web-site-cloud-service-vm/).
...
Работа с учебником предполагает, что вам знакомы [основные понятия об облачных службах Azure ](http://azure.microsoft.com/ru-ru/documentation/articles/fundamentals-application-models/#CloudServices), в частности термины *веб-роли* и *рабочие роли*.  Кроме этого предполагается, что вы умеете работать с [ASP.NET MVC](http://www.asp.net/mvc/tutorials/mvc-5/introduction/getting-started) и с проектами [Web Forms](http://www.asp.net/web-forms/tutorials/aspnet-45/getting-started-with-aspnet-45-web-forms/introduction-and-overview) в среде Visual Studio. Пример приложения использует MVC, но многое в руководство также применимо к веб-формам. 

Можно запускать приложение локально без подписки Azure, но она понадобится для развертывания приложения в облаке. Если у вас нет учетной записи, то можно [активировать преимущества для подписчиков MSDN](/ru-ru/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A55E3C668) или [подписаться на бесплатную пробную версию](/ru-ru/pricing/free-trial/?WT.mc_id=A55E3C668).
...
- [Создание приложения с нуля](#create-the-application-from-scratch)
- [Устранение неполадок](#troubleshooting) (см. здесь примеры при возникновении проблем с запуском).
...
3. Чтобы построить решение, нажмите CTRL+SHIFT+B.
	...
	По умолчанию Visual Studio автоматически восстанавливает содержимое пакета NuGet, который не был включен в ZIP-файл. Если пакеты не восстановились, установите их вручную, открыв диалоговое окно **Управление пакетами NuGet для решения** и нажав кнопку **Восстановить** в правом верхнем углу. 
...
1. Для запуска приложения нажмите сочетание клавиш CTRL+F5.
	...
	При локальном запуске проекта облачной службы Visual Studio автоматически вызывает *эмулятор вычислений* Azure и *эмулятор хранилища* Azure . Эмулятор хранилища использует ресурсы компьютера для эмуляции сред рабочей и веб-ролей. Эмулятор хранилища использует базу данных [SQL Server Express LocalDB](http://msdn.microsoft.com/ru-ru/library/hh510202.aspx) для моделирования облачного хранилища Azure. 
...
5. Щелкните **Создать учетную запись хранения**. 
	...
	На рисунке учетная запись хранения создается с URL-адресом "contosoads.core.windows.net".
...
>[WACOM.NOTE] В этом и следующем разделах учетные данные будут сохранены в файлах проектов. [Не сохраняйте конфиденциальные данные в репозитории с открытым исходным кодом](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/source-control#secrets).
...
4. В файле преобразования *Web.Release.config* удалите {connectionstring} и вставьте вместо этого строку подключения ADO.NET с портала управления.

5. Замените {your_password_here} в строке подключения, вставленной в файле преобразования *Web.Release.config* пароль, созданный для новой базы данных SQL.
...
7. Выделите текст в параметре "ContosoAdsDbConnectionString", а затем вставьте строку подключения, скопированную из предыдущего раздела этого учебника.
...
1. В диалоговом окне **Создание строки подключения хранилища** нажмите кнопку **Подключиться с использованием подписки**, выберите ранее созданную учетную запись хранилища и нажмите кнопку **ОК**. Если вы еще не вошли, появится запрос на ввод учетных данных Azure.

	![Create Storage Connection String](./media/cloud-services-dotnet-get-started/createstoragecs.png)	

1. Сохраните изменения.

2. Для задания строки подключения "Microsoft.WindowsAzure.Plugins.Diagnostics.ConnectionString" выполните ту же саму процедуру, которая использовалась для строки подключения "StorageConnectionString".
...
И файл *ServiceConfiguration.Cloud.cscfg* содержит значения, введенные для этих параметров в Visual Studio.
...
Параметр <Instances> указывает число виртуальных машин, на которых Azure будет запускать код рабочий роли. Раздел [Дальнейшие действия](#next-steps) содержит ссылки на дополнительные сведения о масштабировании облачной службы
...
3. На шаге мастера **Параметры** нажмите кнопку **Далее**.
	...
	Заданные на вкладке **Дополнительно** значения по умолчанию подходят для работы с этим учебником. Дополнительные сведения о вкладке "Дополнительно" см. в разделе [Мастер публикации приложения Azure](http://msdn.microsoft.com/library/windowsazure/hh535756.aspx).
...
9. Можно теперь протестировать приложение, создавая, просматривая и редактируя некоторые элементы рекламы так же, как это происходило при локальном запуске приложения.

>[WACOM.NOTE] После завершения тестирования удалите или остановите службу. Даже если облачная служба не используется, плата за нее начисляется, поскольку ресурсы виртуальной машины для нее зарезервированы. Если оставить ее работающей, любой кто найдет этот URL-адрес, сможет создать и просмотреть рекламу. На портале управления Azure перейдите на **Панель мониторинга** для облачной службы, а затем нажмите кнопку **Удалить** в нижней части страницы. Если нужно временно запретить другим пользователям доступ к сайту, вместо этого нажмите **Остановить**. В этом случае плата будет взиматься. Можно повторить эту же процедуру для удаления базы данных SQL и учетной записи хранения, если они больше не нужны.
...
2. В левой области диалогового окна **Создание проекта** разверните **Visual C#**, выберите шаблоны **Облачные** и выберите шаблон **Облачная служба Microsoft Azure**.
...
8. В диалоговом окне **Создание проекта ASP.NET** нажмите кнопку **ОК**. 
...
10. Присвойте проекту имя *ContosoAdsCommon* и нажмите **ОК**.
...
13. Найдите пакет *Хранилище Azure* и если он есть в списке, и нажмите кнопку **Обновить**, чтобы получить последнюю версию клиентской библиотеки хранилища.
...
16. Найдите *EntityFramework* пакет NuGet и установите его для всех трех проектов.
...
11. В проекте ContosoAdsWorker задайте ссылку на "System.Drawing".
...
3. В окне свойств **ContosAdsWeb [ роли ]** щелкните **Параметры**, а затем выберите **Добавить параметр**.

	Оставьте поле **Конфигурация службы** в значении **Все конфигурации**.

4. Добавьте новый параметр *StorageConnectionString*. Задайте **Тип** для *ConnectionString*, а также установите **Значение** в *UseDevelopmentStorage = true*.
...
3. В проекте ContosoAdsCommon добавьте следующие файлы из загруженного проекта.
	- *Global.asax.cs*  
	- В папке *Views\Shared*: <em>_Layout.cshtml</em>.
	- В папке *Views\Home *: *Index.cshtml*.
	- В папке *Controllers*: *AdController.cs*.
	- В *папке * Views\Ad* (сначала создайте папку): 5 * файлов cshtml*.
...
Подобный код получает ссылку на очередь *images* и создает новую очередь. В этом случае изменений разрешений не требуется.
...
В файле *Views\Home\Index.cshtml* отображаются ссылки категории на домашнюю страницу. Ссылки передают целое значение перечисления Category в переменной строки запроса страницы индекса рекламы.
...
В файле *AdController.cs* конструктор вызывает метод InitializeStorage для создания объектов клиентской библиотеки хранилища Azure, которые обеспечивают API для работы с большими двоичными объектами и очередями. 

Затем код получает ссылку на контейнер больших двоичных объектов *images*, как это было показано в *Global.asax.cs*. Во время выполнения он устанавливает значение по умолчанию для [политики повтора операций](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/transient-fault-handling), подходящего для веб-приложения. Политика повторения с экспоненциальной задержкой по умолчанию может застопорить веб-приложение более чем на минуту при повторяющихся повторах во время кратковременного сбоя. Политика повторения здесь указывает ожидание в 3 секунды после каждой попытки, всего до 3 повторений.

		var blobClient = storageAccount.CreateCloudBlobClient();
		blobClient.RetryPolicy = new LinearRetry(TimeSpan.FromSeconds(3), 3);
		imagesBlobContainer = blobClient.GetContainerReference("images");

Аналогичный код получает ссылку на очереди *images*.

		CloudQueueClient queueClient = storageAccount.CreateCloudQueueClient();
		queueClient.RetryPolicy = new LinearRetry(TimeSpan.FromSeconds(3), 3);
		imagesQueue = queueClient.GetQueueReference("images");

Большая часть кода контроллера обычна для работы с моделью данных Entity Framework с использованием класса DbContext. Исключением является метод "Create" HttpPost, который загружает файл и сохраняет его в хранилище больших двоичных объектов. Соединитель модели передает объект методу [HttpPostedFileBase](http://msdn.microsoft.com/ru-ru/library/system.web.httppostedfilebase.aspx).
...
Код, выполняющий передачу, находится в методе "UploadAndSaveBlobAsync". Он создает имя GUID для большого двоичного объекта, отправляет и сохраняет файл, а также возвращает ссылку на сохраненный большой двоичный объект.
...
После того, как метод "Create" HttpPost отправил большой двоичный объект и обновил базу данных, он создает сообщение очереди, чтобы проинформировать серверную часть процесса, изображение которой готово к переходу на эскиз.

		string queueMessageString = ad.AdId.ToString();
		var queueMessage = new CloudQueueMessage(queueMessageString);
		await queue.AddMessageAsync(queueMessage);

Код для метода "Edit" HttpPost работает аналогичным образом, за исключением того, что если пользователь выбирает новый файл изображения, должны быть удалены все большие двоичные объекты, которые уже существуют.
...
Файл *Index.cshtml* отображает эскизы с другими данными ad:
...
Файлы *Create.cshtml* и *Edit.cshtml* указывают кодировку формы, которая позволяет контроллеру получить объект "HttpPostedFileBase".

		@using (Html.BeginForm("Create", "Ad", FormMethod.Post, new { enctype = "multipart/form-data" }))

Элемент <input> предписывает браузеру отобразить диалоговое окно выбора файла.

		<input type="file" name="imageFile" accept="image/*" class="form-control fileupload" />

### ContosoAdsWorker - WorkerRole.cs - метод OnStart

В среде Azure рабочая роль вызывает метод OnStart класса WorkerRole, когда рабочая роль начинает использоваться, а затем вызывает метод Run после завершения метода OnStart.

Метод OnStart получает строку подключения базы данных из CSCFG-файла и передает его в класс DbContext платформы Entity Framework. Поставщик SQLClient используется по умолчанию, поэтому поставщик не нужно указывать.

		var dbConnString = CloudConfigurationManager.GetSetting("ContosoAdsDbConnectionString");
		db = new ContosoAdsContext(dbConnString);

Затем метод получит ссылку на учетную запись хранилища и создаст контейнер больших двоичных объектов и очередь, если они не существуют. Этот код похож на тот, который был приведен в методе веб-роли Application_Start.

### ContosoAdsWorker - WorkerRole.cs - метод Run

Метод Run вызывается, когда метод OnStart завершает свою работу по инициализации. Метод выполняет бесконечный цикл, в котором ждет новые сообщения в очереди и обрабатывает их, когда они прибывают.
...
>[WACOM.NOTE] Для простоты код в методе ConvertImageToThumbnailJPG использует классы в пространстве имен System.Drawing. Однако классы в этом пространстве имен были спроектированы для использования с формами Windows. Они не поддерживаются в службе Windows или ASP.NET.
...
Объект RoleEnvironment передается Azure при запуске приложения в Azure или при локальном запуске через эмулятор вычислений Azure.  Если сообщение об этой ошибке появляется при локальном выполнении, убедитесь, что проект ContosoAdsCloudService установлен как начальный. Это настраивает проект для запуска с использованием эмулятора вычислений Azure.
...
Приложение Contoso Ads намеренно сделано простым для руководства по началу работы. Например, он не реализует [внедрения зависимости](http://www.asp.net/mvc/tutorials/hands-on-labs/aspnet-mvc-4-dependency-injection) или [шаблоны репозитория и единицы работы](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/advanced-entity-framework-scenarios-for-an-mvc-web-application#repo), не [использует интерфейс для ведения журнала](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/monitoring-and-telemetry#log) и не использует [Миграции EF Code First](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/migrations-and-deployment-with-the-entity-framework-in-an-asp-net-mvc-application) для управления изменениями модели данных, а также [устойчивость подключения EF](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/connection-resiliency-and-command-interception-with-the-entity-framework-in-an-asp-net-mvc-application) для управления ошибками временного состояния сети и т. д.
...
* [Многоуровневое приложение облачной службы Azure с таблицами, очередями и большими двоичными объектами](http://code.msdn.microsoft.com/windowsazure/Windows-Azure-Multi-Tier-eadceb36). Представляет таблицы хранилищ Azure, а также большие двоичные объекты и очереди, а также включает [серию пошаговых учебников](http://azure.microsoft.com/ru-ru/documentation/articles/cloud-services-dotnet-multi-tier-app-storage-1-overview/).
...
Общие сведения об облачной разработке см. в разделе [Разработка реальных облачных приложений в Microsoft Azure](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/introduction).
