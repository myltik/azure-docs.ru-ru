...
Фактически нужно создать два сертификата: один для сервера (файл с расширением `.cer`) и один для клиента (файл с расширением `.pem`). Чтобы создать файл с расширением `.pem`, выполните следующее:
...
Дополнительную информацию о сертификатах Azure см. в разделе [Обзор сертификатов в Azure](http://msdn.microsoft.com/library/azure/gg981929.aspx). Полное описание параметров OpenSSL см. в документации по адресу [http://www.openssl.org/docs/apps/openssl.html](http://www.openssl.org/docs/apps/openssl.html)
...
После создания этих файлов необходимо передать  `.cer` в Azure с помощью [портала управления][management-portal] и отметить, где сохранен  `.pem`-файл.
...
При создании приложения и запуска его в Windows Azure совокупность кода и конфигурации называется [облачной службой] Azure, в более ранних выпусках Azure это сочетание называлось  *hosted service*. Метод **createHostedServices** позволяет создать новую размещенную службу, предоставляя имя размещенной службы (которое должно быть уникальным в Azure), метку (имя размещенной службы в кодировке base64) и объект **CreateServiceOptions**. Объект [CreateServiceOptions]позволяет задать для службы расположение *or* территориальной группы. 
