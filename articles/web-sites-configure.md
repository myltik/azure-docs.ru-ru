...
В этом разделе объясняется, как настроить веб-приложение с помощью [портала Azure](http://go.microsoft.com/fwlink/?LinkId=529715).
...
**Веб-сокеты**. Установите значение **Включено**, чтобы разрешить использование протокола WebSocket, если ваш веб-сайт использует [ASP.NET SignalR](http://www.asp.net/signalr) или [socket.io](web-sites-nodejs-chat-app-socketio.md).

**Всегда включено**. По умолчанию в случае простоя в течение определенного периода времени веб-приложения будут выгружены. Это позволяет сэкономить системные ресурсы. В режиме "Базовый" и "Стандартный" вы можете включить функцию **Всегда включено**, чтобы приложение постоянно оставалось загруженным. Если приложение выполняет непрерывные веб-задания, следует включить функцию **Всегда включено**, иначе веб-задания не будут выполняться правильно.

**Версия управляемого конвейера**. Задает [режим конвейера](http://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture#Application) IIS. Оставьте здесь значение "Интегрированный" (по умолчанию), кроме тех случаев, когда у вас есть устаревшее веб-приложение, для которого требуется предыдущая версия IIS.
...
**Удаленная отладка**. Включение удаленной отладки. Если этот параметр включен, вы можете использовать удаленный отладчик Visual Studio для прямого подключения к своему веб-приложению. Удаленная отладка останется включенной на 48 часов. 


### Параметры приложения

Этот раздел содержит пары "имя/значение", которые веб-приложение будет загружать при запуске. 

- Для сайтов .NET эти параметры вносятся в конфигурацию `AppSettings` во время выполнения, переопределяя текущие настройки. 
...
Документ по умолчанию - это веб-страница, которая отображается пр открытии корневого URL-адреса веб-сайта.  Используется первый найденный файл в списке. 
...
- **Путь к обработчику сценариев**. Абсолютный путь к обработчику сценариев. Запросы к файлам, совпадающим с расширением, будут обрабатываться обработчиком сценариев. Используйте путь `D:\home\site\wwwroot` для указания корневого каталога вашего веб-приложения.
...
Чтобы настроить виртуальные приложения и каталоги, укажите каждый виртуальный каталог и его соответствующий физический путь относительно корневого каталога веб-сайта. Или установите флажок **Приложение**, чтобы пометить виртуальный каталог как приложение.
...
- **Ведение журналов приложений**. Эта функция записывает журналы приложений в файловую систему. Ведение журнала продолжается в течение 12 часов. 
...
**Ведение журналов веб-серверов**. Журналы сохраняются в расширенном формате журнала W3C. 

**Подробные сведения об ошибках**. Сохраняет HTM-файлы с подробными сведениями об ошибках. Файлы сохраняются в папке /LogFiles/DetailedErrors. 
...
В режимах Basic и Standard SSL-сертификат можно выложить для личного домена. Дополнительные сведения см. в статье [Включение протокола HTTPS для веб-приложения](web-sites-configure-ssl-certificate.md). 
...
- Слоты развертывания. См. статью [Развертывание промежуточных слотов для веб-приложений в службе приложения Azure](web-sites-staged-publishing.md)
...
Дополнительные сведения см. в статье [Практическое руководство. Мониторинг состояния конечной точки веб-сайта](http://go.microsoft.com/fwLink/?LinkID=279906&clcid=0x409).
...
- [Настройка личного доменного имени](web-sites-custom-domain-name.md)
- [Включение протокола HTTPS](web-sites-configure-ssl-certificate.md)
- [Масштабирование веб-приложения в службе приложений Azure](web-sites-scale.md)
- [Мониторинг основ для веб-приложений в службах приложений Azure](web-sites-monitor.md)
