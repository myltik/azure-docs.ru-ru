...
2. Прокрутите до действия `twitterconnector`, найдите значение для запроса и замените его на `@concat('#', parameters('topic'))`. Функция **concat** соединяет вместе несколько строк.
