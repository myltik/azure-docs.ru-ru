...
При работе с этим учебником вы воспользуетесь сайтом [Million Song Dataset][] и загрузите [набор данных][], чтобы создать рекомендации по композициям для пользователей на основе того, что они предпочитали слушать прежде.
...
2.  [Проверка и форматирование данных][]
3.  [Установка Mahout][]
4.  [Выполнение задания Mahout][]

## <a name="setup"></a>Установка и настройка

В этом учебнике предполагается, что вы получили установленный экземпляр с Azure и предварительной версией HDInsight и создали кластер HDInsight, в котором можно запустить пример. Если вы еще не сделали этого, обратитесь к учебнику [Приступая к работе с Azure HDInsight][], который содержит инструкции по обеспечению соответствия предварительным требованиям.
...
-   Откройте портал кластера HDInsight и щелкните значок **Удаленный рабочий стол**.
    ...
    ![Значок управления кластером][]

### Значок удаленного рабочего стола

HDInsight не содержит Mahout по умолчанию. Но поскольку библиотека является частью экосистемы Hadoop, ее можно загрузить с веб-сайта [Mahout][]. Новейшая версия — 0.7, но этот набор инструкций подходит для любой версии: 0.5 и 0.7.

1.  Прежде всего загрузите [Mahout версии 0.7][] на свой локальный компьютер.
...
В статье демонстрируется использование командной строки Hadoop, однако те же задачи можно выполнить с помощью интерактивной консоли HDInsight. Дополнительные сведения см. в разделе [Руководство: Интерактивные консоли JavaScript и Hive в HDInsight][интерактивная-консоль].
  ...
  [Приступая к работе с Azure HDInsight]: /en-us/manage/services/hdinsight/get-started-hdinsight/
  ...
  [репозитории]: https://github.com/wenming/BigDataSamples/tree/master/mahout
  [ZIP-файле]: https://github.com/wenming/BigDataSamples/archive/master.zip
