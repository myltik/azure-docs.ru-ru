<properties linkid="develop-mobile-tutorials-dotnet-backend-validate-modify-and-augment-data-wp8" urlDisplayName="Проверка и изменение данных" pageTitle="Использование серверной части .NET для проверки и изменения данных (Windows Phone 8) | Центр разработчиков для мобильных устройств" metaKeywords="" description="Использование серверных скриптов для проверки, изменения и улучшения данных для вашего приложения Windows Phone с использованием мобильных служб Windows Azure для .NET." metaCanonical="" services="" documentationCenter="Mobile" title="Проверка и изменение данных в мобильных службах с помощью серверной части .NET" authors="wesmc" solutions="" manager="" editor="" />
...
Теперь, когда мобильная служба настроена для проверки данных и отправки сообщений об ошибках при недопустимой длине текста, необходимо обновить приложение для обработки сообщений об ошибках, полученных при выполнении проверки. Ошибка будет определяться как исключение `MobileServiceInvalidOperationException` из вызова метода `IMobileServiceTable<TodoItem].InsertAsync()` в клиентском приложении.

1. В обозревателе решений в Visual Studio откройте проект клиентского приложения и откройте файл MainPage.xaml.cs. Добавьте в файл следующую инструкцию using.

        using Newtonsoft.Json.Linq;

2. В файле MainPage.xaml.cs замените существующий метод **InsertTodoItem** на следующий код:
...
2. В обозревателе решений в Visual Studio в проекте клиентского приложения откройте файл MainPage.xaml.cs, замените обработчик событий `CheckBoxComplete_Checked` на следующий обработчик событий `CheckBoxComplete_Clicked`. Так мы сможем увидеть дату завершения после обработки элемента.
...
3. В файле MainPage.xaml.cs замените существующий класс **TodoItem** на следующее определение, которое включает в себя новое свойство **CompleteDate** как тип, допускающий значение NULL.
    ...
    >[WACOM.NOTE] <code>DataMemberAttribute</code> оповещает клиента для сопоставления нового свойства <code>CompleteDate</code> в приложении со столбцом <code>CompleteDate</code>, определенным в таблице TodoItem. С помощью этого атрибута ваше приложение может получить имена свойств для объектов, которые отличаются от имен столбцов в базе данных SQL.
...
6. В окне обозревателя решений в Visual Studio щелкните правой кнопкой мыши **Решение** и выберите команду **Перестроить решение** для перестройки клиента и серверной службы .NET. Убедитесь, что построение выполнено без ошибок.
