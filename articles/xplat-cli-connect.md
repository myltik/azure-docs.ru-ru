...
> [AZURE.NOTE]При использовании версии Azure CLI ниже 0.9.9 командой `azure login` можно воспользоваться только с удостоверениями рабочей учетной записи; удостоверения учетной записи Microsoft работать не будут. Тем не менее при работе с версиями Azure CLI 0.9.9 и выше для входа в свою учетную запись с помощью интерактивной команды `azure login` можно использовать любое удостоверение.



## Использование метода интерактивного входа

Используйте команду `azure login` (без каких-либо аргументов), чтобы выполнить интерактивную аутентификацию с:
...
> [AZURE.NOTE]В обоих случаях аутентификация и авторизация выполняется с использованием каталога Azure Active. При использовании учетных записей Microsoft для этого осуществляется доступ к домену Azure Active Directory по умолчанию. (Если вы зарегистрировались для получения бесплатной пробной версии, вы можете не знать, что в Azure Active Directory создан домен по умолчанию для вашей учетной записи).
...
Метод неинтерактивного входа работает только с рабочей учетной записью, которая также называется *учетная запись организации*. Это учетная запись, которая управляется организацией и определена в ее каталоге Azure Active Directory. Можно [создать учетную запись организации](#create-an-organizational-account), если таковая отсутствует, или [создать рабочий ИД на основе ИД своей учетной записи Microsoft](./virtual-machines/resource-group-create-work-id-from-personal.md). При этом нужно указать имя пользователя или имя пользователя и пароль в команде `azure login`, например:
