...
* **Вход в Azure с рабочей или учебной учетной записью или удостоверением учетной записи Майкрософт**. Для аутентификации любого типа удостоверений учетной записи с помощью Azure Active Directory используйте команду `azure login`. Этот метод следует использовать большинству клиентов, создающих новые развернутые службы Azure. Если определены учетные записи, то команда `azure login` предписывает выполнять вход интерактивном режиме на веб-портале.

    Для аутентификации субъекта-службы для приложения Azure Active Directory можно также использовать команду `azure login`. Это удобно для выполнения автоматических служб.
    
    Выполнив вход в систему с поддерживаемым удостоверением учетной записи, вы сможете использовать команды режима Azure Resource Manager или интерфейса командной строки режима управления службами Azure.

* **Загрузка и использование файла параметров публикации**. При этом на локальный компьютер устанавливается сертификат, который позволяет выполнять различные задачи управления в течение всего срока действия подписки и сертификата.

    Этот метод позволяет использовать только команды интерфейса командной строки режима управления службами Azure.
...
- удостоверением рабочей или учебной учетной записи (также называемой *учетной записью организации*), требующим многофакторной проверки подлинности, или
...
Скопируйте предложенный код (см. выше) и откройте http://aka.ms/devicelogin в браузере (или другую указанную страницу). Введите код, и вам будет предложено ввести имя пользователя и пароль для удостоверения, которое нужно использовать. По завершении этого процесса командная оболочка завершит процедуру входа в систему. Это будет иметь следующий вид:
...
Если вы создали для приложения Active Directory субъект-службу и предоставили ему разрешения для своей подписки, то для аутентификации этого субъекта-службы можно использовать команду `azure login`. В зависимости от сценария учетные данные субъекта-службы можно передавать как явные параметры команды `azure login`, сценария интерфейса командной строки либо кода приложения. Кроме того, для неинтерактивной проверки подлинности субъекта-службы в сценариях автоматизации можно использовать сертификат. Дополнительные сведения и примеры см. в статье [Проверка подлинности субъекта-службы в диспетчере ресурсов Azure](resource-group-authenticate-service-principal.md).

## Используйте файл параметров публикации

Если вам требуются только команды интерфейса командной строки режима управления службами Azure (например, для развертывания виртуальных машин Azure с помощью классической модели), то для подключения можно использовать файл параметров публикации.

* **Чтобы скачать файл параметров публикации** для своей учетной записи, воспользуйтесь следующей командой (доступна только в режиме управления службами).
...
* Инструкции по работе с командами интерфейса командной строки Azure см. в статьях [Команды Azure CLI в режиме Resource Manager](./virtual-machines/azure-cli-arm-commands.md) и [Команды Azure CLI в режиме управления службами Azure (ASM)](virtual-machines-command-line-tools.md).
...
* Если у вас возникли проблемы с использованием Azure CLI или Azure, посетите [форумы Azure](https://social.msdn.microsoft.com/Forums/ru-RU/home?forum=azurescripting).
