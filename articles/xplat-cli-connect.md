...
Метод неинтерактивного входа работает только с рабочей учетной записью, которая также называется *учетная запись организации*. Это учетная запись, которая управляется организацией и определена в ее каталоге Azure Active Directory. Можно [создать учетную запись организации](#create-an-organizational-account), если таковая отсутствует, или [создать рабочий ИД на основе ИД своей учетной записи Майкрософт](./virtual-machines/virtual-machines-windows-create-aad-work-id.md). При этом нужно указать имя пользователя или имя пользователя и пароль в команде `azure login`, например:
