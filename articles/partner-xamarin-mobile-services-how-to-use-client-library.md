<properties urlDisplayName="Xamarin" pageTitle="Как использовать клиентский компонент Xamarin - руководство по функциям мобильных служб Azure" metaKeywords = "мобильные службы Azure, Xamarin, iOS, Android, клиент .NET" description="Узнайте, как использовать клиент компонента Xamarin для мобильных служб Azure." metaCanonical="" disqusComments="1" umbracoNaviHide="0" title="How to use the Xamarin Component client for Azure Mobile Services" authors="donnam" manager="dwrede" />
...
В этом руководстве показано, как реализовать типичные сценарии с использованием клиентского компонента Xamarin для мобильных служб Azure в приложениях Xamarin для операционных систем iOS и Android. В сценарии входят запрос данных, вставка, обновление и удаление данных, проверка подлинности пользователей и обработка ошибок. Если вы не знакомы с мобильными службами, следует сначала ознакомиться с учебниками "Краткое руководство по мобильным службам" ([Xamarin.iOS][Краткое руководство по Xamarin.iOS]/[Xamarin.Android][Краткое руководство по Xamarin.Android]) и "Приступая к работе с данными в .NET" ([Xamarin.iOS][Учебник по данным Xamarin.iOS]/[Xamarin.Android][Учебник по данным Xamarin.Android]). Для краткого руководства потребуется [Xamarin][Загрузка Xamarin] [пакет SDK для мобильных служб]. Он поможет вам настроить учетную запись и создать свою первую мобильную службу.
...
Предполагается, что мобильная служба и таблица созданы. Дополнительные сведения см. в разделе [Создание таблицы](http://go.microsoft.com/fwlink/?LinkId=298592). В коде, используемом в данном разделе, таблица TodoItem содержит следующие столбцы: id, Text и Complete.
...
Следующий код создает объект MobileServiceClient, который используется для доступа к мобильной службе. 
...
Весь код, который обращается к данным или изменяет данные в таблице мобильных служб, вызывает функции объекта MobileServiceTable. Чтобы получить ссылку на таблицу, вызовите функцию [GetTable](http://msdn.microsoft.com/ru-ru/library/windowsazure/jj554275.aspx) для экземпляра MobileServiceClient. 
...
Следующий код иллюстрирует способ фильтрации данных путем включения предложения Where в запрос. Он возвращает из таблицы todoTable все те элементы, свойство Complete которых равно false. Функция Where применяет к таблице предикат фильтрации строк для запросов. 
...
В следующем коде показано, как сортировать данные, в том числе дополняя запрос функцией OrderBy или OrderByDescending. Он возвращает элементы из таблицы todoTable, упорядочивая их в порядке возрастания по полю Text. По умолчанию сервер возвращает только первые 50 элементов. 
...
Количество возвращаемых элементов можно увеличить путем вызова Take, как описано в следующем разделе.
...
Можно также использовать метод [IncludeTotalCount](http://msdn.microsoft.com/ru-ru/library/windowsazure/jj730933.aspx), чтобы запрос возвращал общее количество <i>всех</i> записей, которые должны быть возвращены без учета указанных предложений разбиения по страницам или ограничения:
...
При попытке обновить элемент, не задавая значения поля "Id", служба не сможет определить, какой экземпляр нужно обновить, поэтому из службы поступает исключение MobileServiceInvalidOperationException. Аналогичным образом, при попытке удалить нетипизированный элемент без заданного поля "Id" служба дает исключение MobileServiceInvalidOperationException. 
...
Возвращаемое MobileServiceClient.GetTable значение и запросы являются интерфейсами. Это означает, что их можно легко имитировать в целях проверки, что дает возможность создать интерфейс MyMockTable: IMobileServiceTable<TodoItem>, реализующий логику тестирования.
...
* Приступая к работе с проверкой подлинности ([Xamarin.iOS][Приступая к работе с проверкой подлинности (iOS)]/[Xamarin.Android][Приступая к работе с проверкой подлинности (Android)])
...
[Загрузка Xamarin]: http://xamarin.com/download/
[Пакет SDK для мобильных служб]: http://go.microsoft.com/fwlink/?LinkId=257545
[Краткое руководство по Xamarin.iOS]: /ru-ru/develop/mobile/tutorials/get-started-xamarin-ios/
[Краткое руководство по Xamarin.Android]: /ru-ru/develop/mobile/tutorials/get-started-xamarin-android/
...
[Проверка подлинности Xamarin.iOS]: /ru-ru/develop/mobile/tutorials/get-started-with-users-xamarin-ios/
[Проверка подлинности Xamarin.Android]: /ru-ru/develop/mobile/tutorials/get-started-with-users-xamarin-android/
[Пакет SDK для мобильных служб]: http://go.microsoft.com/fwlink/?LinkId=257545
...
[Начало работы с проверкой подлинности (iOS)]: /ru-ru/develop/mobile/tutorials/get-started-with-users-xamarin-ios
[Начало работы с проверкой подлинности (Android)]: /ru-ru/develop/mobile/tutorials/get-started-with-users-xamarin-android
[Проверка и изменение данных с помощью скриптов (ios)]: /ru-ru/develop/mobile/tutorials/validate-modify-and-augment-data-xamarin-ios
[Проверка и изменение данных с помощью скриптов (android)]: /ru-ru/develop/mobile/tutorials/validate-modify-and-augment-data-xamarin-android
...
[Авторизация пользователей с помощью скриптов (iOS)]: /ru-ru/develop/mobile/tutorials/authorize-users-in-scripts-xamarin-ios
[Авторизация пользователей с помощью скриптов (Android)]: /ru-ru/develop/mobile/tutorials/authorize-users-in-scripts-xamarin-android
