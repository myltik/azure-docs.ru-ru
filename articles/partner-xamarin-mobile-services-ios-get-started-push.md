...
1. В разделе "Служебные программы" запустите **средство Keychain Access**.
...
3. Введите соответствующие данные в полях **User Email Address** (Адрес электронной почты пользователя) и **Common Name** (Общее имя), убедитесь, что установлен флажок **Saved to disk** (Сохранено на диск), а затем нажмите кнопку **Continue** (Продолжить).
...
2. Введите название приложения в поле **Description** (Описание), введите и запомните уникальный **Bundle Identifier** (Идентификатор пакета), выберите вариант "Push Notifications" (Push-уведомления) в разделе "App Services" (Службы приложений), а затем нажмите кнопку **Continue** (Продолжить). В этом примере используется идентификатор **MobileServices.Quickstart**, но вы не можете повторно использовать тот же идентификатор, поскольку идентификаторы приложений должны быть уникальными для всех пользователей. Поэтому рекомендуется добавить ваше полное имя или инициалы после имени приложения.
...
1. На <a href="http://go.microsoft.com/fwlink/p/?LinkId=272456" target="_blank">портале подготовки iOS</a> выберите пункт **Provisioning Profiles** (Профили подготовки), а затем щелкните **All** (Все) и нажмите кнопку **+**, чтобы создать профиль. Запустится мастер **Add iOS Provisiong Profile** (Добавление профиля подготовки для iOS).
...
2. Прокрутите вниз до пункта **Background Modes** (Фоновые режимы), а затем последовательно установите флажки **Enable Background Modes** (Разрешить фоновые режимы) и **Remote notifications** (Удаленные уведомления).

    ![][122]

3. Дважды щелкните проект на панели решения, чтобы открыть окно **Project Options** (Параметры проекта).

4.  Выберите пункт **iOS Bundle Signing** (Подпись пакета iOS) в разделе **Build** (Сборка), выберите соответствующее **Identity** (Удостоверение) и созданный для данного проекта **Provisioning profile** (Профиль подготовки).
