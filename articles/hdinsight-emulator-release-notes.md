<properties 
	pageTitle="Заметки о выпуске: Эмулятор Microsoft HDInsight для Azure | Azure" 
	description="Получите самую свежую информацию о новейших версиях эмулятора Hadoop в HDInsight." 
	editor="cgronlun" 
	manager="paulettm" 
	services="hdinsight" 
	authors="mumian" 
...
* Эта версия содержит обновление для платформы данных Hortonworks Data Platform версии 1,6.1, которая соответствует службам Hortonworks Data Platform, доступным в кластере Azure HDInsight версии 1. 
...
* Настройка ASV в локальном кластере: 
	...
	3. Обратитесь к любому файлу в этой учетной записи, используя полный URI: asv://{контейнер}@{учетная_запись}/{путь} (или asvs://, если вы хотите использовать HTTPS для доступа к данным). Пример:
