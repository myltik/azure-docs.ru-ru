<properties title="Azure Notification Hubs Secure Push" pageTitle="Безопасная отправка push-уведомления с помощью концентраторов уведомлений Azure" metaKeywords="push-уведомления Azure, концентраторы уведомлений Azure, обмен сообщениями Azure, безопасные push-уведомления" description="Узнайте, как отправлять безопасные push-уведомления в Azure. Примеры кода написаны на C# с использованием API .NET." documentationCenter="Mobile" metaCanonical="" disqusComments="1" umbracoNaviHide="0" authors="sethm" manager="dwrede" />
...
<div class="dev-center-tutorial-selector sublanding"> 
    	<a href="/ru-ru/documentation/articles/notification-hubs-windows-dotnet-secure-push/" title="Windows Universal" class="current">Windows Universal</a><a href="/ru-ru/documentation/articles/notification-hubs-aspnet-backend-ios-secure-push/" title="iOS">iOS</a>
...
1. В проекте **NotifyUserWindowsPhone** добавьте следующий код в файл App.xaml.cs, чтобы зарегистрировать фоновую задачу push-уведомления. В конце метода `OnLaunched()` добавьте следующую строку кода:
...
4. В меню **Файл** Visual Studio выберите пункт **Сохранить все**.
		
## Создайте фоновый компонент push-уведомления

Следующий шаг заключается в создании фонового компонента push-уведомления.

1. В обозревателе решений щелкните правой кнопкой мыши узел верхнего уровня решения (в данном случае **Solution SecurePush**), затем нажмите кнопку **Добавить**, после чего щелкните **Создать проект**.
...
3. В обозревателе решений щелкните правой кнопкой мыши проект **PushBackgroundComponent (Windows Phone 8.1)**, затем нажмите кнопку **Добавить** и выберите **Класс**. Присвойте новому классу имя **PushBackgroundTask.cs**. Нажмите кнопку **Добавить**, чтобы создать класс.
...
8. В списке результатов выберите **Клиентские библиотеки Microsoft HTTP**, после чего выберите **Установить**. Выполните установку.

9. Вернитесь к полю NuGet **Поиск**, введите **Json.net**. Установите пакет **Json.NET**, затем закройте окно диспетчера пакетов NuGet.
...
11. В обозревателе решений в проекте **NotifyUserWindowsPhone (Windows Phone 8.1)** щелкните правой кнопкой мыши **Ссылки**, а затем нажмите **Добавить ссылку...**. В диалоговом окне "Диспетчер ссылок" установите флажок **PushBackgroundComponent** и затем нажмите кнопку **ОК**.
...
13. Находясь в **Package.appxmanifest**, откройте меню **Операторы** вверху. В раскрывающемся списке **Доступные операторы** выберите **Фоновые задачи**, а затем нажмите кнопку **Добавить**.
...
1. В Visual Studio запустите веб-приложение **AppBackend** интерфейса API. Отобразится веб-страница ASP.NET.
