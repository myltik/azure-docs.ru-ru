...
С помощью концентраторов уведомлений Azure вы можете передать ключ объединения через заголовок HTTP, используя универсальный интерфейс API `SendNotification` (например, для пакета SDK для.NET - `SendNotificationAsync`), также принимающий заголовки HTTP, которые передаются как есть в соответствующую службу PNS. 
...
В конце выполнения для параметра `result.State` будет просто установлено значение `Enqueued` без предоставления информации о том, что случилось с вашим push-уведомлением. 
Теперь вы можете использовать логическое свойство `EnableTestSend` во время инициализации `NotificationHubClient`, а также получать подробную информацию об ошибках PNS, возникших при отправке уведомления. Для возврата вызова отправки потребуется дополнительное время, так как он выполняется только после доставки уведомления из концентратора уведомлений в PNS для определения результата. 
...
<!-- LINKS -->
[Общая информация о центрах уведомлений Azure]: http://azure.microsoft.com/documentation/articles/notification-hubs-overview/
...
[Руководство по использованию шаблонов]: https://msdn.microsoft.com/library/dn530748.aspx 
