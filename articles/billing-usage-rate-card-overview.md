<properties
   pageTitle="Получение ценных сведений о потреблении ресурсов Microsoft Azure"
...
- **Azure Role-based Access Control** (Управления доступом на основе ролей Azure) — клиенты и партнеры могут настроить свои политики доступа на [портале предварительной версии Azure](https://portal.azure.com) или с помощью [командлетов Azure PowerShell](https://azure.microsoft.com/ru-ru/documentation/articles/powershell-install-configure/), чтобы указать, какие пользователи или приложения могут получить доступ к данным об использовании в рамках подписки. Вызывающие объекты должны использовать стандартные маркеры Azure Active Directory для проверки подлинности. Вызывающий объект также должен быть добавлен в роль читателя, владельца или участника, чтобы получить доступ к данным об использовании для конкретной подписки Azure.
