...
- После скачивания запустите исполняемый файл PUTTY.EXE. Установите параметры следующим образом.
     ...
     "Имя узла (или IP-адрес)" — это URL-адрес, который задается как "DNS-имя" при создании образа.
     
     Для параметра "Порт" можно выбрать значение 22. Это порт служб SSH по умолчанию.
...
- Затем щелкните «Открыть». Может появиться окно сообщения с информацией о том, что подключаемый компьютер может оказаться не тем компьютером, который требуется подключить. Если вы настроили DNS-имя и номер порта правильно, нажмите кнопку «Да».
