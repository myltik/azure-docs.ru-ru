...
В этом учебнике вы подключите мобильное приложение к корпоративному решению SaaS. Вы обновите приложение из раздела [Проверка подлинности приложения с помощью единого входа библиотеки проверки подлинности Active Directory], чтобы создавать документ Word в SharePoint Online при каждом добавлении нового элемента TodoItem.
...
1. В разделе **Active Directory** на [Портал управления Azure] выберите свой клиент. Перейдите к веб-приложению, созданному для службы приложений.
...
* Введите SP_Authority в качестве конечной точки центра для своего клиента AAD. Это значение должно совпадать со значением центра, используемым для вашего клиентского приложения. Оно будет иметь формат `https://login.windows.net/contoso.onmicrosoft.com`

* Введите SP_ClientSecret в качестве значения секрета клиента, которое вы получили ранее.

* Введите SP_SharePointURL в качестве URL-адреса для своего сайта SharePoint. Значение должно иметь формат `https://contoso-my.sharepoint.com`
...
Для доступа к SharePoint необходимо иметь специальный токен доступа с SharePoint в качестве целевой аудитории. Чтобы получить этот токен, потребуется выполнить обратный вызов AAD с удостоверением службы приложений и токеном, выданным пользователю.
...
2. В проекте кода мобильного приложения создайте новый класс с именем "SharePointUploadContext". Добавьте в него следующую информацию:
