...
Twilio представляет собой платформу API, который облегчает разработчикам сделать и принимать телефонные звонки, отправлять и текстовыми сообщениями и внедрение VoIP вызова браузера и собственных приложений для мобильных устройств.  Сначала вкратце рассмотрим принцип работы.
...
За HTTP-запросов к веб-службы Twilio API, разработчики могут отправлять текстовые сообщения или совершать исходящие вызовы.  Для исходящих вызовов разработчику необходимо также указать адрес URL, который возвращает инструкции TwiML по обработке исходящего вызова после его подключения.
...
Перед использованием службы Twilio необходимо сначала [учетную запись][signup].  Клиенты Microsoft Azure получают специальную скидку - [не забудьте зарегистрироваться здесь][signup]!
...
* Создание файла `server.js` с node.js простого веб-приложения
...
Для выполнения проверенных запросов к серверной части Twilio понадобится SID учетной записи и маркер проверки подлинности, которые будут использоваться как имя пользователя и пароль для учетной записи Twilio. Через системные переменные среды, которые можно установить непосредственно в консоли администрирования Azure - наиболее безопасный способ их настройки для использования с модулем узла в Azure.
...
Этот код объявляет модуль twilio как зависимость, а также популярных [веб-платформу express][express] и подсистему шаблонов EJS.  Итак теперь мы все готово - напишем код!
...
После этого создайте каталог с именем "представления" - в этом каталоге, создайте файл с именем "index.ejs" со следующим содержимым:
...
* [Официальные документы по модулю][документы]
* [Учебник по использованию VoIP с приложениями node.js][voipnode]
* [Votr - в режиме реального времени SMS голосования приложения с node.js и CouchDB (три части)][votr]
* [Парное программирование в браузере с node.js][пары]
