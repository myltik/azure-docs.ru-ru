...
Две библиотеки являются обязательными для расширения NPS. Они устанавливаются одновременно с самим расширением.

-   Распространяемые компоненты Microsoft Visual Studio 2013 C++ (X64).
-   Модуль Microsoft Azure Active Directory для Windows PowerShell, версия 1.1.166.
...
4. Убедитесь, что адрес https://login.windows.new/ доступен с сервера, на котором выполняется расширение.
