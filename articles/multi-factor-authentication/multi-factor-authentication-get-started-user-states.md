...
| Состояние | Описание | Затронутые приложения, не использующие браузер | Затронутые браузерные приложения и современная аутентификация |
|:---:|:---:|:---:|:--:|
| Отключено |Состояние по умолчанию для нового пользователя, не указанного в службе Многофакторной идентификации Azure (MFA). |Нет |Нет |
| Включено |Пользователь указан в Azure MFA, но не зарегистрирован. Ему будет предложено зарегистрироваться при следующем входе в систему. |Нет.  Они будут продолжать работать, пока не завершится регистрация. | Да. После окончания срока действия маркера обновления для сеанса потребуется регистрация в службе MFA.|
| Принудительно |Пользователь указан и зарегистрирован для использования Azure MFA. |Да.  Для приложений нужны пароли приложений. |Да. При входе требуется пройти Многофакторную идентификацию. |
