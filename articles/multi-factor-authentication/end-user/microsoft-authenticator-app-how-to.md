...
## <a name="opt-in-for-two-step-verification"></a>Выбор двухшаговой проверки

Приложение Microsoft Authenticator не работает само по себе. Необходимо настроить свои учетные записи для запроса на указание второго способа проверки после входа с использованием имени пользователя и пароля. 

В случае с рабочей или учебной учетной записью вы не можете выбрать это самостоятельно. Администратор безопасности сделает выбор от вашего имени и затем отправит уведомление о необходимости регистрации способов проверки подлинности для вашей учетной записи. Если этот сценарий применим для вас, см. дополнительные сведения в статье [Что для меня означает Azure Multi-Factor Authentication](multi-factor-authentication-end-user.md).
