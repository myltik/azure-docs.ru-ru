...
Служба Многофакторной идентификации Azure позволяет помечать устройства как доверенные. Дополнительные сведения см. в разделе о [сохранении данных проверки подлинности для устройств, которым доверяют пользователи](multi-factor-authentication-whats-next.md#remember-multi-factor-authentication-for-devices-that-users-trust).
