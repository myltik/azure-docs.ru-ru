...
Сообщения Интернета вещей в Azure форматируются в виде JSON и всегда имеют по крайней мере параметр **body**. Например: 
...
Для такого примера сообщения можно определить несколько условий, например:
* `WHERE $body.machine.status != "running"`
* `WHERE $body.ambient.temperature <= 60 AND $body.machine.temperature >= 60`

Условие также можно использовать для сортировки по типам сообщений, например, в шлюзе для маршрутизации сообщений, поступающих от конечных устройств. Поступающие от модулей сообщения содержат свойство **connectionModuleId**. Так что, если нужно направить сообщения от конечных устройств к Центру Интернета вещей, используйте следующий маршрут, чтобы исключить сообщения от модулей.
