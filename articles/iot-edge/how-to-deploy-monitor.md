...
1. В [портал Azure][lnk-portal], перейдите в центр IoT. 
...
Чтобы добавить модуль из Azure Stream Analytics, выполните следующие действия.
1. Выберите **модуль импорта Azure Stream Analytics IoT Edge**.
...
1. Для **URI изображения** введите образ контейнера Docker для модуля. 
1. Укажите любые **параметры создания контейнера** должен передаваться в контейнер. Дополнительные сведения см. в статье [о создании Docker][lnk-docker-create].
...
1. Используйте раскрывающееся меню, чтобы выбрать **требуемого состояния** для модуля. Выберите один из следующих параметров:
...
1. Выберите **включить** Если вы хотите добавить теги или требуемые свойства двойных модуля. 
1. Выберите **Сохранить**, чтобы добавить модуль в развертывание. 
