...
6. В верхней части пространства имен **FilterModule** добавьте три инструкции `using` для типов, которые будут использоваться позже:
...
5. Войдите в Docker, указав имя пользователя, пароль и данные сервера входа, скопированные из реестра контейнеров Azure при его создании. Во встроенном терминале VS Code введите следующую команду: 
     
   ```csh/sh
   docker login -u <ACR username> -p <ACR password> <ACR login server>
   ```

6. Отправьте образ в реестр контейнеров. Выберите **Вид** > **Палитра команд** и найдите пункт меню **Edge: Push IoT Edge module Docker image** (Edge: отправка образа Docker модуля IoT Edge). В верхней части окна VS Code введите имя образа в текстовом поле всплывающего элемента. Укажите имя образа, выбранное на шаге 4.
7. Чтобы просмотреть образ на портале Azure, перейдите к реестру контейнеров Azure и выберите **Репозитории**. В списке должен отобразиться модуль **filtermodule**.
