...
В этом руководстве приведены примеры кода на C# для запуска процессов при создании или обновлении большого двоичного объекта Azure. В примерах кода используется [Пакет SDK для заданий WebJobs](websites-dotnet-webjobs-sdk.md) версии 1.x.

В этом руководстве предполагается, что вы уже знаете, [как создавать проект WebJob в Visual Studio со строками подключения, указывающими на учетную запись хранения](websites-dotnet-webjobs-sdk-get-started.md).
...
Сведения о сценариях SDK WebJobs, которые не относятся к Service Bus, см. в разделе [Использование хранилища очередей Azure с помощью пакета SDK WebJobs](websites-dotnet-webjobs-sdk-storage-queues-how-to.md). 
