...
**Примечание.** При создании очереди сообщений в приложении, которое не использует пакет SDK для заданий WebJob, не забудьте задать для параметра [BrokeredMessage.ContentType](http://msdn.microsoft.com/ru-ru/library/microsoft.servicebus.messaging.brokeredmessage.contenttype.aspx) значение text/plain.
