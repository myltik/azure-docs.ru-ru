<properties linkid="" urlDisplayName="" pageTitle="Push-уведомления для пользователей (Android)| Центр разработчиков для мобильных устройств" metaKeywords="" description="Узнайте, как использовать мобильные службы для отправки push-уведомлений пользователям вашего приложения Android." metaCanonical="" services="" documentationCenter="Mobile" title="Отправка push-уведомлений пользователям с использованием мобильных служб" authors="ricksal" solutions="" manager="" editor="" />
...
1. В обозревателе пакетов Eclipse щелкните правой кнопкой мыши пакет (под узлом `src`), выберите **Создать**, а затем **Класс**.

2. В поле **Имя** введите `Registration` и нажмите кнопку **Готово**.
...
4. Вставьте вырезанный код в тело класса **Registration**, созданный ранее.
...
7. Найдите свойство `mClient` и замените его на следующий код:
...
8. В файле **MyHandler** добавьте следующие инструкции "import":
...
9. В конце метода `onRegistered` добавьте следующий код:
...
2. В таблице **Registration** щелкните вкладку **Скрипт** и выберите команду **Вставить**.
...
1. В Eclipse в меню **Выполнить** щелкните **Выполнить** для запуска приложения.
...
[Приступая к работе с мобильными службами]: /ru-ru/develop/mobile/how-to-guides/work-with-net-client-library/#create-new-service
