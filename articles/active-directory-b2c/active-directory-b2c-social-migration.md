...
* В зависимости от поставщика удостоверений **идентификатор пользователя социальной сети** является уникальным значением для данного пользователя `per application` или учетной записи разработки. Настройте политику Azure AD B2C с тем же идентификатором приложения, который ранее был назначен поставщиком удостоверений социальных сетей. Или используйте другое приложение `within the same development account`.
