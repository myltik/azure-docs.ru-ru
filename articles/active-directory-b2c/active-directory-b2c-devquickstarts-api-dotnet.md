...
Код для этого учебника размещен на портале [GitHub](https://github.com/AzureADQuickStarts/B2C-WebAPI-DotNet). Чтобы выполнить сборку образца, [загрузите схему проекта в ZIP-архиве](https://github.com/AzureADQuickStarts/B2C-WebAPI-DotNet/archive/skeleton.zip) или клонируйте ее:

```
git clone --branch skeleton https://github.com/AzureADQuickStarts/B2C-WebAPI-DotNet.git
```

Готовое приложение также [доступно в виде ZIP-архива](https://github.com/AzureADQuickStarts/B2C-WebAPI-DotNet/archive/complete.zip) или ветви `complete` того же репозитория.

Загрузив образец кода, откройте файл Visual Studio `.sln`, чтобы начать работу. Вы увидите, что решение содержит два проекта: `TaskWebApp` и `TaskService`. `TaskWebApp` — это веб-приложение MVC, с которым взаимодействует пользователь. `TaskService` — это относящийся к приложению серверный веб-API, в котором хранится список дел каждого пользователя.

## 5\. Настройка веб-приложения для задач

Когда пользователь взаимодействует с проектом `TaskWebApp`, клиент отправляет запросы в Azure AD и получает токены, которые можно использовать для вызова веб-API `TaskService`. Чтобы пользователь смог выполнить вход и получить токены, необходимо предоставить проекту `TaskWebApp` некоторые сведения о приложении. В корне проекта `TaskWebApp` откройте файл `web.config` и замените значения в разделе `<appSettings>`:
...
Дополнительные сведения о том, как Azure AD B2C используется в веб-приложениях, подобных `TaskWebApp`, см. в статье [Приступая к работе: вход в веб-приложения](active-directory-b2c-devquickstarts-web-dotnet.md).
...
Теперь, когда в приложении настроено использование проверки подлинности OAuth 2.0, вам остается только защитить веб-API, добавив тег `[Authorize]` в контроллер задач. В этом контроллере выполняются все операции списка дел, поэтому мы защитим контроллер на уровне класса. Для более точного управления также можно добавить тег `[Authorize]` к отдельным действиям.
...
`TaskController` сохраняет задачи в базе данных, где каждой задаче соответствует пользователь, являющийся ее владельцем. Владелец определяется с помощью **идентификатора объекта** пользователя (поэтому идентификатор объекта необходимо добавить как утверждение приложения во всех политиках):
...
Наконец, выполните сборку и запуск проектов `TaskWebApp` и `TaskService`. Зарегистрируйтесь в приложении с использованием адреса электронной почты или имени пользователя. Создайте несколько задач в списке дел. Обратите внимание, что они сохраняются в API даже после остановки и перезапуска клиента.

## 8\. Изменение политик

Теперь, когда у вас есть API, защищенный с помощью Azure AD B2C, можно поэкспериментировать с политиками приложения и понаблюдать, как это влияет (или не влияет) на API. Вы можете <!--add **identity providers**
to the policies, allowing you users to sign into the Task Client using social accounts.  You can also -->управлять **утверждениями приложения** в политиках и изменять сведения о пользователе, доступные в веб-API. Любые дополнительные утверждения, добавляемые вами, будут доступны для веб-API .NET MVC в объекте `ClaimsPrincipal`, как описано выше.
