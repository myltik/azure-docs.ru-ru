...
> [AZURE.NOTE]Сейчас страницы входа в локальную учетную запись, подтверждающие сообщения электронной почты и страницы самостоятельного сброса пароля можно настроить только с помощью [фирменной символики компании](./active-directory/active-directory-add-company-branding.md). Описанные в этой статье механизмы недоступны для этих целей.
...
Возможность настройки пользовательского интерфейса страницы позволяет изменять внешний вид страниц регистрации, входа и изменения профиля клиента (посредством настройки [политик](active-directory-b2c-references.policies.md)). При переходе между приложением и страницами, за которые отвечает служба Azure AD B2C, ваши пользователи не заметят никакой разницы в работе.
...
## Вспомогательное средство для тестирования возможностей настройки пользовательского интерфейса страницы с помощью предоставленного образца содержимого

Чтобы протестировать возможности настройки пользовательского интерфейса страницы с помощью предоставленного образца содержимого, воспользуйтесь нашим вспомогательным средством (ожидается в ближайшее время).

## Основные элементы пользовательского интерфейса для каждого типа страницы

В этом разделе приведены фрагменты кода HTML5 (для каждого типа страницы), которые служба Azure AD B2C добавляет в элемент `<div id="api"></div>` в вашем содержимом. Настроить эти элементы пользовательского интерфейса можно с помощью таблиц стилей. Эти таблицы могут переопределять таблицы стилей по умолчанию, которые мы добавляем к этим типам страниц в фрагментах `<head>`.

> [AZURE.IMPORTANT]В предварительной версии элементы пользовательского интерфейса будут изменяться по мере получения и обработки ваших отзывов. Проверьте исходный код страниц по умолчанию на наличие всех указанных ниже сведений.
...
- В целях безопасности мы запрещаем добавлять в содержимое какие-либо сценарии JavaScript. Большинство необходимых вам возможностей уже доступны. Если вам не хватает каких-то функций, сообщите нам об этом на [форуме](http://feedback.azure.com/forums/169401-azure-active-directory).
