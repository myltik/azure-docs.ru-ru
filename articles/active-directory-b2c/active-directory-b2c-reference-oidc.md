...
# Azure Active Directory B2C: вход в веб-приложения с помощью OpenID Connect
...
| response\_type | Обязательно | Тип ответа, который должен включать `id_token` для OpenID Connect. Если веб-приложению также потребуются маркеры для вызова веб-API, можете использовать `code+id_token`, как и в наших примерах. |
...
| response\_mode | Рекомендуется | Метод, с помощью которого следует отправлять полученный код авторизации приложению. Возможные методы: query, form\_post, или fragment. Рекомендуется использовать form\_post для максимальной безопасности. |
...
Когда вы проверите подпись маркера id\_token, вам потребуется проверить несколько утверждений.
...
| access\_token | Подписанный маркер JWT, который вы запрашивали. |
...
| expires\_in | Срок действия маркера access\_token (в секундах). |
...
| scope | Рекомендуется | Список областей с разделителями-пробелами. При указании одной области также обозначаются запрашиваемые разрешения. Область `openid` определяет разрешение на вход пользователя и получение данных о пользователе в форме **маркеров идентификации**. Ее можно использовать для получения маркеров для веб-API серверной части приложения, идентификатор приложения которого совпадает с идентификатором приложения клиента. Область `offline_access` означает, что вашему приложению потребуется **маркер обновления** для продолжительного доступа к ресурсам. |
| redirect\_uri | Рекомендуется | URI перенаправления приложения, в котором вы получили код авторизации. |
