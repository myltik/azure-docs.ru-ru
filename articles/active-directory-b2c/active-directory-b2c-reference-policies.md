...
1. [Выполните следующие действия, чтобы перейти к колонке функций B2C на портале Azure](active-directory-b2c-app-registration.md#navigate-to-the-b2c-features-blade).
2. Щелкните **Password reset policies** (Политики сброса паролей).
3. Нажмите **+Добавить** в верхней части колонки.
4. **Имя** определяет имя политики сброса паролей, используемой приложением. Например, введите "SSPR".
5. Щелкните **Поставщики удостоверений** и выберите пункт "Reset password using email address" (Сброс пароля с помощью электронного адреса). Нажмите кнопку **ОК**.
...
7. Щелкните **Создать**. Обратите внимание, что в колонке **Password reset policies** (Политики сброса паролей) только что созданная политика отображается в следующем виде: **B2C\_1\_SSPR** (фрагмент **B2C\_1\_** добавляется автоматически).
