---
title: "Azure Active Directory B2C: вызов веб-API из приложения iOS с использованием сторонней библиотеки | Документация Майкрософт"
...
---
# <a name="azure-ad-b2c-call-a-web-api-from-an-ios-application-using-a-third-party-library"></a>Azure AD B2C: вызов веб-API из приложения iOS с использованием сторонней библиотеки
...
## <a name="download-the-third-party-library-nxoauth2-and-launch-a-workspace"></a>Загрузка сторонней библиотеки NXOAuth2 и запуск рабочей области
...
### <a name="adding-the-library-to-your-project-using-cocoapods"></a>Добавление библиотеки в проект с помощью CocoaPods
...
## <a name="the-structure-of-the-project"></a>Структура проекта
...
## <a name="create-the-settingsplist-file-for-your-application"></a>Создание файла `settings.plist` для приложения
...
Вместо заполнителя `<policy name>` в конце каждого URL-адреса необходимо указать ранее созданную политику. Приложение вызывает эти политики в зависимости от последовательности.

Значение `taskAPI` — это конечная точка REST, которая будет вызываться вместе с токеном B2C для добавления задач или запроса имеющихся задач. Этот параметр настроен специально для данного примера. Не изменяйте его, чтобы этот образец кода работал.
...
## <a name="set-up-a-appdata-class-to-read-our-settings"></a>Настройка класса AppData для считывания параметров
...
## <a name="set-up-the-nxoauth2client-library-in-your-appdelegate"></a>Настройка библиотеки NXOAuth2Client в AppDelegate
...
## <a name="create-a-loginviewcontroller-class-that-we-will-use-to-handle-authentication-requests"></a>Создание класса `LoginViewController` , который будет использоваться для обработки запросов на проверку подлинности
...
Мы создадим тот же метод, что и для `AppDelegate` выше, но на этот раз мы настроим некоторые уведомления `NSNotification`, чтобы знать о том, что происходит в нашей службе. Кроме того, мы настроим наблюдатель, который будет сообщать об изменениях токена. После получения токена пользователь вернется в `masterView`.
...
* Создание класса `GraphAPICaller.m file`
