...
Обратите внимание, что в данной статье не рассматривается использование политик, которые вы только что создали. Дополнительные сведения о работе политик в Azure AD B2C см. в [учебнике по началу работы с веб-приложениями .NET](active-directory-b2c-devquickstarts-web-dotnet.md).

## 4\. Скачивание кода

Код в этом учебнике размещен на портале [GitHub](https://github.com/AzureADQuickStarts/B2C-NativeClient-iOS). Чтобы построить образец, [скачайте схему проекта в ZIP-архиве](https://github.com/AzureADQuickStarts/B2C-NativeClient-iOS/archive/skeleton.zip) или клонируйте ее.

```
git clone --branch skeleton https://github.com/AzureADQuickStarts/B2C-NativeClient-iOS.git
```

> [AZURE.NOTE]**Для изучения этого учебника необходимо загрузить схему.** В связи со сложностью реализации полнофункционального приложения iOS **основа** должна содержать UX-код, который будет выполнен после завершения данного руководства. Это позволит разработчику сэкономить время. UX-код не связан с темой добавления B2C в приложение iOS.

Готовое приложение также [доступно в виде ZIP-архива](https://github.com/AzureADQuickStarts/B2C-NativeClient-iOS/archive/complete.zip) или ветви `complete` того же репозитория.
...
Для того чтобы приложение задач iOS взаимодействовало с Azure AD B2C, необходимо задать несколько общих параметров. В корневой папке `Microsoft Tasks` откройте файл `settings.plist` и замените значения в разделе `<dict>`. Эти значения будут использоваться во всем приложении.
...
В этом разделе показано, как выполнить обмен маркерами OAuth 2.0 в веб-приложении с помощью библиотек и платформ от Майкрософт. Если вы не знакомы с **кодами авторизации** и **маркерами доступа**, см. [справочник по протоколу OAuth 2.0](active-directory-b2c-reference-protocols.md).
...
Создайте файл с именем `samplesWebAPIConnector.h` в папке /Microsoft Task проекта XCode.
...
Вы увидите, что получить маркер в соответствии с желаемой политикой позволяют простые операции CRUD в API, а также метод `doPolicy`.
...
Создайте файл с именем `samplesTaskItem.h` с помощью следующего кода:
...
Также можно создать файл `samplesPolicyData.h` для хранения данных политики.
...
Создайте файл с именем `samplesPolicyData.m` с помощью следующего кода:
...
Быстрый код для хранения объектов нашего пользовательского интерфейса готов. Теперь, используя параметры, определенные в файле `settings.plist`, нужно написать код для доступа в ADAL из iOS, предназначенный для получения маркера доступа и его передачи на сервер задач. Код может быть длинным, поэтому будьте внимательны.

Вся работа будет выполняться в `samplesWebAPIConnector.m`.

Для начала создадим реализацию метода `doPolicy()`, которую мы записали в файл заголовка `samplesWebAPIConnector.h`.
...
3. Используемый метод `convertPolicyToDictionary:policy` принимает нашу политику и форматирует ее как URL-адрес для отправки на сервер. Этот вспомогательный метод мы напишем ниже.
4. И, наконец, мы используем довольно большой метод `getClaimsWithPolicyClearingCache`. Это и есть тот вызов ADAL для iOS, который нам нужно написать. Мы сделаем это позже.


Под кодом, который мы только что составили, запишем метод `convertPolicyToDictionary`.
...
```
Этот довольно простой код просто добавляет к политике букву "p", так что внешний запрос получает вид ?p=<policy>.
...
Мы запишем "получать утверждения и маркер согласно указанной мною политике и не очищать кэш" или `getClaimsWithPolicyClearingCache`. Мы всегда возвращаем маркер из ADAL, поэтому указывать "Утверждения и маркер" в методе необязательно. Однако в случаях, когда требуется только маркер без синтаксического анализа утверждений, в схеме вызывается метод без утверждений, который называется `getTokenWithPolicyClearingCache`.
...
Первая часть должна быть вам знакома. Мы загружаем параметры, указанные в `Settings.plist`, и назначаем их параметру `data`. Затем мы настраиваем метод `ADAuthenticationError`, который принимает любую ошибку, поступающую в iOS из ADAL. Кроме того, мы создаем `authContext`, который настраивает наш вызов в ADAL. Мы передаем ему свое *полномочие*, чтобы запустить процесс. Затем мы передаем `authContext` ссылку на родительский контроллер, чтобы к нему можно было вернуться. А также преобразуем `redirectURI`, который раньше был строкой в `settings.plist`, в тип NSURL, который нужен для ADAL. И, наконец, настроим `correlationId` — это просто UUID, который может проследить вызов от клиента до сервера и обратно. Он пригодится для отладки.
...
**scopes** — это области, передаваемые на сервер, который будет запрашиваться с сервера для входа пользователя. Для предварительной версии B2C мы передаем идентификатор клиента. Однако в будущем вместо это будет использоваться чтение областей. Когда это произойдет, этот документ будет обновлен. **addtionalScopes** — это дополнительные области, которые вы можете использовать в приложении. Этот параметр будет использоваться в будущем. **clientId** — идентификатор приложения, полученный с портала. **redirectURI** — это URL-адрес переадресации для обратной публикации маркера. **identifier** — это способ идентификации пользователя, позволяющий определить наличие пригодного маркера в кэше вместо того, чтобы постоянно запрашивать у сервера новый маркер. Эту задачу выполняет тип `ADUserIdentifier`, и мы можем указать, что будет использоваться в качестве идентификатора. Это должно быть имя пользователя. **promptBehavior** — этот параметр устарел и должен выглядеть как AD\_PROMPT\_ALWAYS. **extraQueryParameters** — любой дополнительный параметр, который нужно передать на сервер в составе URL-адреса. **policy** — это вызываемая политика. Это самая важная часть данного руководства.

В параметре completionBlock мы передаем `ADAuthenticationResult`, который содержит наш маркер и данные профиля (если вызов был выполнен успешно).
...
Обсуждение кода задач выходит за рамки этого руководства, однако есть нечто, на что вам нужно обратить внимание: метод `craftRequest`, принимающий URL-адрес нашей задачи. Мы используем этот метод и полученный маркер доступа для создания запроса к серверу. Давайте это запишем.
...
Как видите, он принимает веб-URI, добавляет к нему маркер с заголовком `Bearer` в HTTP и возвращает его нам. Мы вызываем API `getTokenClearingCache`. Это может показаться странным, но мы используем этот вызов для получения маркера из кэша и проверки его действительности (вызовы метода getToken* делают это за нас, опрашивая ADAL). Этот код применяется при каждом вызове. Теперь вернемся к составлению дополнительных методов задач.
...
Представленный ниже код соответствует тому же шаблону, но вводит еще один (последний!) метод, который нам нужно реализовать: `convertTaskToDictionary`. Он принимает наш массив и превращает его в объект словаря — его проще адаптировать к параметрам запроса, который нам нужно передавать на сервер. Этот код очень простой.
...
Нам осталось только добавить в приложение функцию выхода. Это довольно просто. Добавим в файл `sampleWebApiConnector.m` следующий код.
...
Готовый пример для сравнения в виде ZIP-файла находится [здесь](https://github.com/AzureADQuickStarts/B2C-NativeClient-iOS/archive/complete.zip). Также можно клонировать его с GitHub.
