...
> [AZURE.NOTE]Для предварительной версии B2C используйте одни и те же политики в параметрах клиента и сервера. Если вы уже выполнили действия из пошагового руководства и создали эти политики, то не нужно делать это еще раз. Вы можете повторно использовать политики, ранее созданные вами на портале, если они соответствуют требованиям приложения.
...
3. У нас есть метод `convertPolicyToDictionary:policy`, который принимает нашу политику и форматирует ее в виде URL-адреса для отправки на сервер. Этот вспомогательный метод мы напишем ниже.
...
Под кодом, который мы только что написали, создадим метод `convertPolicyToDictionary`.
