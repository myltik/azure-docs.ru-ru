...
10. Чтобы подготовить приложение Facebook к использованию Azure AD B2C, необходимо сделать его общедоступным. Для этого можно щелкнуть **App Review** (Обзор приложения) в левой области навигации и переключить параметр в верхней части страницы в значение **YES** (ДА). Затем следует щелкнуть **Confirm** (Подтвердить).
