...
- Выберите утверждения **Отображаемое имя** и **Идентификатор объекта** как утверждения приложения в каждой политике. Можно также выбрать другие утверждения.
...
Код в этом учебнике размещен на портале [GitHub](https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet). Чтобы построить образец, [скачайте схему проекта в ZIP-архиве](https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet/archive/skeleton.zip) или клонируйте ее.

```
git clone --branch skeleton https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet.git
```

Готовое приложение также [доступно в виде ZIP-архива](https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet/archive/complete.zip) или ветви `complete` того же репозитория.

Скачав образец кода, откройте файл Visual Studio `.sln`, чтобы начать работу. Вы увидите, что решение содержит два проекта: `TaskClient` и `TaskService`. `TaskClient` — это классическое приложение WPF, с которым взаимодействует пользователь. `TaskService` — это относящийся к приложению серверный веб-API, в котором хранится список дел каждого пользователя. В этом случае приложение и `TaskClient`, и `TaskService` будут представлены одним **идентификатором приложения**, поскольку они оба содержат одно приложение логики.
...
#### Ввод данных B2C
Откройте файл `Globals.cs` и замените все значения свойств на свои собственные. Этот класс используется для ссылки на часто используемые значения во всем проекте `TaskClient`.
...
Основной класс библиотеки ADAL — `AuthenticationContext`. Он представляет соединение вашего приложения с каталогом B2C. Когда приложение запустится, создайте экземпляр класса `AuthenticationContext` в файле `MainWindow.xaml.cs`, который можно использовать для выполнения всех действий в этом окне.
...
Нам нужно, чтобы, когда пользователь нажимал кнопку регистрации, инициировался созданный нами поток регистрации. Для этого в ADAL нужно только вызвать метод `authContext.AcquireTokenAsync(...)`. Параметры, передаваемые в метод `AcquireTokenAsync(...)`, определяют, какой маркер вы получите, какая политика будет использоваться в запросе проверки подлинности и т. д.
...
Если вызов `AcquireTokenAsync(...)` завершается успешно и маркер в кэше присутствует, можно добавить этот маркер в заголовок `Authorization` HTTP-запроса, чтобы `TaskService` мог проверить подлинность запроса на чтение списка дел пользователя.
...
Тот же шаблон можно использовать для проверки кэша на наличие маркеров в любое время даже без предложения пользователю выполнить вход в систему. Например, при запуске приложения нужно проверять `FileCache` на наличие маркеров, чтобы сеанс входа пользователя в систему возобновлялся при каждом запуске приложения. В событии `OnInitialized` класса `MainWindow` вы увидите тот же код, который выполняется при первом включении.
...
Наконец, выполните сборку и запуск проектов `TaskClient` и `TaskService`. Зарегистрируйтесь в приложении с использованием адреса электронной почты или имени пользователя. Выйдите и снова войдите под именем того же пользователя. Измените профиль пользователя. Выйдите и зарегистрируйтесь от имени другого пользователя.
...
Чтобы добавить в приложение поставщиков удостоверений социальных сетей, следуйте подробным инструкциям, приведенным в статьях ниже. Для каждого поставщика удостоверений, для которого нужно добавить поддержку, необходимо зарегистрировать приложение в соответствующей системе и получить идентификатор клиента.
...
Добавляя поставщиков удостоверений в каталог B2C, также необходимо внести изменения во все три политики. Для этого добавьте в них новых поставщиков удостоверений, как описано в [справочной статье о политиках](active-directory-b2c-reference-policies.md). Сохранив политики, снова запустите приложение. Добавленные поставщики удостоверений должны отобразиться в виде вариантов при входе и регистрации.
...
Готовый пример для сравнения в виде ZIP-файла находится [здесь](https://github.com/AzureADQuickStarts/B2C-NativeClient-DotNet/archive/complete.zip). Также можно клонировать его с GitHub.
