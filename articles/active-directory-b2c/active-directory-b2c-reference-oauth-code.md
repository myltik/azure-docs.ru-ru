...
---
# <a name="azure-active-directory-b2c:-oauth-2.0-authorization-code-flow"></a>Azure Active Directory B2C: поток кода авторизации OAuth 2.0
...
В приведенных ниже запросах HTTP будет использован наш пример каталога B2C **fabrikamb2c.onmicrosoft.com**, а также наши примеры приложения и политик. Вы можете пробовать выполнять запросы с этими значениями или заменить их на собственные.
Узнайте, как [получить собственный каталог B2C, приложения и политики](#use-your-own-b2c-directory).
...
| scope |Обязательно |Список областей с разделителями-пробелами. При указании одной области также обозначаются запрашиваемые разрешения. Использование идентификатора клиента в качестве области указывает на то, что приложению нужен **маркер доступа** , который может использоваться в вашей службе или веб-API, представленных таким же идентификатором клиента.  Область `offline_access` означает, что вашему приложению потребуется **маркер обновления** для продолжительного доступа к ресурсам.  Можно также использовать область `openid` для запроса маркера **id_token** из Azure AD B2C. |
| response_mode |Рекомендуется |Метод, с помощью которого следует отправлять полученный код авторизации приложению. Может принимать значение "query", "form_post" или "fragment". |
...
| access_token |Подписанный веб-маркер JSON (JWT), который вы запрашивали. |
...
| client_id |Рекомендуется |Идентификатор приложения, который [портал Azure](https://portal.azure.com) назначил вашему приложению. |
...
## <a name="use-your-own-b2c-directory"></a>Использование собственного каталога B2C
