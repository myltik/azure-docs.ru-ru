...
- Выберите утверждение **Отображаемое имя** как утверждение приложения в каждой политике. Можно также выбрать другие утверждения.
...
Код для этого примера размещен на портале [GitHub](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet). Чтобы выполнить сборку образца, [загрузите схему проекта в ZIP-архиве](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet/archive/skeleton.zip) или клонируйте ее:

```
git clone --branch skeleton https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet.git
```

Готовый образец также [доступен в виде ZIP-архива ](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet/archive/complete.zip) или ветви `complete` того же репозитория.

Загрузив образец кода, откройте файл Visual Studio `.sln`, чтобы начать работу.
...
Когда пользователь нажимает в веб-приложении кнопку «Регистрация», «Вход» или «Изменить профиль», соответствующее действие вызывается в `Controllers\AccountController.cs`. В каждом случае можно использовать встроенные методы OWIN для запуска нужной политики:
...
По умолчанию OWIN не будет отправлять в Azure AD политики, указанные в `AuthenticationProperties`. Однако вы можете изменять запросы, которые создает OWIN в уведомлении `RedirectToIdentityProvider`. Используйте это уведомление в `App_Start\Startup.Auth.cs`, чтобы извлечь из метаданных политики правильную конечную точку для каждой политики. Это обеспечит отправку в Azure AD правильного запроса для каждой политики, выполняемой приложением.
...
Готовый пример (без заданных вами значений конфигурации) для сравнения в виде ZIP-файла находится [здесь](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIdConnect-DotNet/archive/complete.zip). Также можно клонировать его с GitHub:
