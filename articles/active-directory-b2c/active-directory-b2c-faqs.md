...
На этой странице вы найдете ответы на часто задаваемые вопросы о предварительной версии Azure Active Directory (Azure AD) B2C. Следите за обновлениями.
...
Используйте [функцию фирменной символики](../active-directory/active-directory-add-company-branding.md) для настройки содержимого проверочных электронных сообщений. Поле "От:" можно изменить, обратившись в службу поддержки.

### Как перенести в Azure AD B2C существующие имена пользователей, пароли и профили из своей базы данных?

Средство миграции можно создать с помощью API Graph Azure AD. Подробные сведения см. в разделе [Пример API Graph](active-directory-b2c-devquickstarts-graph-dotnet.md). В будущем мы планируем предоставлять различные готовые варианты и средства миграции.
...
Ознакомьтесь с этой статьей о [внешних удостоверениях](../active-directory/active-directory-b2b-compare-external-identities.md), чтобы получить дополнительные сведения о применении соответствующих функций в ваших сценариях, в которых используются внешние удостоверения.
...
В настоящее время нет. Мы планируем добавить эту функцию в будущем. Обратите внимание, что это также нельзя сделать с помощью проверки домена на вкладке **Домены** клиента на классическом портале Azure.

### Как удалить клиент Azure AD B2C?

Для удаления клиента Azure AD B2C выполните следующие действия.

- Войдите на [классический портал Azure](https://manage.windowsazure.com/) под именем администратора подписки. Это та же рабочая или учебная учетная запись либо учетная запись Майкрософт, которая использовалась для регистрации в Azure.
- Перейдите к расширению Active Directory слева и щелкните свой клиент B2C.
- Откройте вкладку **Приложения**.
- В раскрывающемся поле **Показать** выберите **Приложения, которыми владеет моя компания** и щелкните значок флажка.
...
Если в клиенте B2C уже созданы **Пользователи**, **Приложения** или другие объекты, необходимо удалить их, прежде чем выполнять описанные выше действия.
...
См. раздел [Azure Active Directory B2C: регистрация запросов в службу поддержки](active-directory-b2c-support.md).
...
См. также действующие [ограничения предварительной версии](active-directory-b2c-limitations.md).
