...
### Можно ли использовать Azure AD B2C в существующем клиенте Azure AD для работы с сотрудниками?
...
На портале предварительной версии Azure для каждого клиента Azure AD B2C есть своя колонка с функциями B2C. [Здесь](active-directory-b2c-app-registration.md#navigate-to-the-b2c-features-blade) можно узнать, как на портале предварительной версии Azure перейти к колонке функций B2C определенного клиента. В большинстве браузеров после переключения между каталогами Azure AD B2C на портале предварительной версии Azure колонка функций B2C закрывается.
...
В настоящее время нет. Мы планируем добавить эту функцию в будущем. Также обратите внимание, что «проверка» домена на вкладке **Домены** клиента на портале Azure не позволит это сделать.
