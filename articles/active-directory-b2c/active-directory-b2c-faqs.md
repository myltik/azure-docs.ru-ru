...
В настоящее время мы поддерживаем Facebook, Google+, LinkedIn, учетную запись Майкрософт, Amazon, Twitter (предварительную версию), WeChat (предварительную версию), Weibo (предварительную версию) и QQ (предварительную версию).  Мы продолжаем добавлять поддержку других популярных поставщиков удостоверений социальных сетей с учетом запросов клиентов.
...
1. Войдите на [классический портал Azure](https://manage.windowsazure.com/) под именем администратора подписки.
1. Перейдите к клиенту B2C.
1. Выберите вкладку **Настройка** .
1. Измените значение поля **Имя** в разделе **Свойства каталога**.
...
Средство миграции можно создать с помощью API Graph Azure AD. Подробные сведения см. в разделе с [примером API Graph](active-directory-b2c-devquickstarts-graph-dotnet.md).
...
Нет служба Azure AD Connect не предназначена для работы с Azure AD B2C. Вы можете использовать [API Graph](active-directory-b2c-devquickstarts-graph-dotnet.md) для перемещения пользователя.
...
В ближайшее время будет реализована базовая интеграция с порталом Microsoft Dynamics 365.
...
В настоящее время нет. Мы планируем добавить эту функцию в будущем. Проверка домена на вкладке **Домены** на классическом портале Azure не подходит для достижения этой цели.
...
1. Перейдите в колонки **Приложения**, **Поставщики удостоверений** и **Все политики**, а затем удалите из них все записи.
1. Войдите на [классический портал Azure](https://manage.windowsazure.com/) как администратор подписки. (Используйте ту же рабочую или учебную учетную запись либо учетную запись Майкрософт, которая использовалась для регистрации в Azure.)
1. Перейдите к расширению Active Directory слева и щелкните свой клиент B2C.
1. Откройте вкладку **Пользователи** .
1. Затем выберите всех пользователей (кроме пользователя, от имени которого вы вошли, — администратора подписки). В нижней части страницы щелкните **Удалить**, а затем при появлении запроса нажмите кнопку **Да**.
1. Откройте вкладку **Приложения** .
1. В раскрывающемся поле **Показать** выберите **Приложения, которыми владеет моя компания** и щелкните флажок.
1. В открывшемся списке вы увидите приложение с именем **b2c-extensions-app**. В нижней части страницы щелкните **Удалить**, а затем при появлении запроса нажмите кнопку **Да**.
1. Еще раз перейдите к расширению Active Directory и выберите свой клиент B2C.
1. Щелкните **Удалить** в нижней части страницы. Для завершения процесса следуйте инструкциям на экране.
