...
- Скопируйте **имя** каждой политики после ее создания. У имени должен быть префикс `b2c_1_`. Эти имена политик понадобятся вам через некоторое время. 
...
-	Сначала откройте файл `config.js` в корне проекта, а затем укажите параметры конфигурации приложения в разделе `exports.creds`.
...
- Затем откройте в корне проекта файл `app.js` и добавьте вызов стратегии `OIDCStrategy`, входящей в состав `passport-azure-ad`.
...
Готовый пример (без ваших значений конфигурации) можно [скачать в виде ZIP-файла](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIDConnect-NodeJS/archive/complete.zip) или клонировать с GitHub.
