...
В вашем приложении уже настроена связь с конечной точкой версии 2.0 с использованием протокола аутентификации OpenID Connect. Подключаемый модуль `passport-azure-ad` сам создает сообщения проверки, проверяет маркеры из Azure AD и поддерживает сеанс пользователя. Осталось дать пользователям возможность входа, выхода и сбора дополнительной информации о вошедшем в систему пользователе.
...
-	Рассмотрим это подробно.
    -	Маршрут `/` выполнит перенаправление в представление index.ejs, передавая в запросе сведения о пользователе (если они есть).
    ...
    - Маршрут `/login` вызовет приложение для аутентификации azuread-openidconnect из `passport-azuread` и, если она не будет пройдена, перенаправит пользователя назад к /login.
    - `/logout` просто вызовет файл logout.ejs (и маршрут), который очистит файлы cookie и вернет пользователя к index.ejs.
...
Готовый пример (без ваших значений конфигурации) [можно скачать в виде ZIP-файла здесь](https://github.com/AzureADQuickStarts/B2C-WebApp-OpenIDConnect-NodeJS/archive/complete.zip) или клонировать с портала GitHub.
