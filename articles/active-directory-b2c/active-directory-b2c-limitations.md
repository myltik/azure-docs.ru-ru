...
Если вы столкнулись с проблемами при [создании клиента Azure AD B2C](active-directory-b2c-get-started), ознакомьтесь с рекомендациями в [этой статье](active-directory-b2c-support-create-directory.md).
...
Перечисленные ниже типы приложений пока что не поддерживаются в предварительной версии Azure AD B2C. Описание поддерживаемых типов приложений можно найти в [этой статье](active-directory-b2c-apps.md).
...
Если на классическом портале Azure вы сбросите пароль потребителя с локальной учетной записью (команда **Сбросить пароль** на вкладке **Пользователи**), то этот потребитель не сможет изменить свой пароль при следующем входе, при этом для него будет заблокирован доступ к вашим приложениям. Мы работаем над устранением этой проблемы. В качестве обходного пути для сброса пароля потребителя используйте [API Graph Azure AD](active-directory-b2c-devquickstarts-graph-dotnet.md).

## Ограничение на удаление клиентов Azure AD B2C

Вам не удастся удалить клиент Azure AD B2C на классическом портале Azure.

## Проблемы с проверкой домена на классическом портале Azure

В настоящее время успешно проверить домен на [классическом портале Azure](https://manage.windowsazure.com/) невозможно. Мы работаем над тем, чтобы это исправить.

## Предупреждающие сообщения на портале Azure

При доступе к колонке с параметрами B2C на портале Azure в области уведомлений (в правом верхнем углу) вы увидите предупреждение "У вас нет подписок в каталоге <B2CTenantName>. У вас есть другие каталоги, к которым можно перейти". Здесь <B2CTenantName> — имя вашего клиента B2C. Можно спокойно игнорировать это сообщение и продолжить пользоваться компонентами B2C. Совместно с группой портала Azure мы работаем над решением этой проблемы.
