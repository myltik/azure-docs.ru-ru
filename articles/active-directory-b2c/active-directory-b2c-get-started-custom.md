...
Чтобы получить доступ к изменению настраиваемых политик, необходимо связать с клиентом действующую подписку Azure.
...
3. Создайте B2C_1A_TokenSigningKeyContainer, если он не существует:<br>
    ...
    В. Для параметра **Имя** используйте значение `TokenSigningKeyContainer`. <br> 
...
6. Когда создание будет завершено, выберите только что созданное приложение **IdentityExperienceFramework**.<br>
   а. Выберите **Свойства**.<br>
   b. Скопируйте идентификатор приложения и сохраните его для использования в будущем.
...
* [Файлы проверяющей стороны](active-directory-b2c-overview-custom.md#policy-files). Эти файлы запрашивает приложение для выполнения определенных задач.

>[!NOTE]
>Если в редакторе XML поддерживается проверка, можно проверить файлы, используя XML-файл схемы TrustFrameworkPolicy_0.3.0.0.xsd, который расположен в корневом каталоге начального пакета. При проверке по схеме XML определяются ошибки перед отправкой.
...
3. Выберите **Отправить политику**, чтобы отправить файлы политики.
