...
4. Выполните шаги, описанные в [этой статье](http://blogs.msdn.com/b/mast/archive/2013/10/24/windows-azure-technical-support-for-msdn-technet-or-mpn-users-and-partners.aspx), чтобы регистрировать запросы на поддержку для Azure AD B2C. Используйте эти данные для выполнения действий:
	...
	- В колонке **Описание** подробно опишите неполадку. В текстовом поле **Ресурс** введите имя каталога B2C, например contosob2c.onmicrosoft.com.
...
## Известная проблема: регистрация запроса на поддержку в контексте каталога B2C

Если вы пропустили шаг 2, описанный выше, пытаетесь создать запрос на поддержку в контексте своего каталога B2C, появится следующая ошибка.

> [AZURE.IMPORTANT]Не пытайтесь зарегистрироваться для получения новой подписки Azure в каталоге B2C.
