...
> [!WARNING]
> Для работы с этим руководством требуются некоторые важные обновления, которые позволяют обойтись без использования библиотеки ADAL Android для B2C.  На следующей неделе мы опубликуем новые инструкции по использованию Azure AD B2C в приложениях Android. Дождитесь их появления.  Но вы также можете продолжить работу с этим руководством в экспериментальных целях.
>
>
...
## <a name="download-the-android-adal-and-add-it-to-your-android-studio-workspace"></a>Скачивание библиотеки ADAL для Android и добавление ее в рабочую область платформы Android Studio
...
### <a name="option-1-binaries-via-gradle-recommended"></a>Вариант 1. Двоичные файлы через Gradle (рекомендуется)
...
### <a name="option-2-aar-via-maven"></a>Вариант 2. AAR через Maven
Если вы используете подключаемый модуль `m2e` в Eclipse, можно задать зависимость в файле `pom.xml`:
...
### <a name="option-3-source-via-git-last-resort"></a>Вариант 3. Исходные файлы через Git (последнее средство)
Чтобы получить исходный код пакета SDK через Git, введите:

    git clone git@github.com:AzureAD/azure-activedirectory-library-for-android.git
    cd ./azure-activedirectory-library-for-android/src

Используйте **конвергенцию**
...
## <a name="add-references-to-android-adal-to-your-project"></a>Добавление ссылок на библиотеку ADAL для Android в проект
> [!NOTE]
> В ADAL для Android для вызова проверки подлинности используется модель на основе намерений. Намерения работают, "накладываясь" на приложение. Весь этот пример (как и использование ADAL для Android в целом) связан с управлением намерениями и передачей сведений между ними.
>
>
...
## <a name="create-the-sign-in-activity"></a>Создание действия входа
...
## <a name="create-the-settings-activity"></a>Создание действия параметров
...
## <a name="create-the-add-task-activity"></a>Создание действия добавления задачи
...
## <a name="create-the-to-do-list-activity"></a>Создание действия списка дел
...
>
 ...
 Вы можете пропустить пользователя broker с помощью следующей команды:
    ...
    ```
> [!NOTE]
> Чтобы упростить это краткое руководство B2C, пользователь broker пропущен в примере кода.
>
>
...
```
## <a name="create-a-method-to-return-a-user-identifier"></a>Создание метода для возврата идентификатора пользователя
...
### <a name="write-helper-methods"></a>Написание вспомогательных методов
...
`getTasks` возвращает массив, представляющий задачи на сервере.
...
### <a name="create-an-endpoint-url-generator"></a>Создание генератора URL-адресов конечных точек
...
## <a name="write-some-ux-methods"></a>Написание методов UX
...
### <a name="session-cookies-in-web-view"></a>Файлы cookie сеанса в веб-представлении
