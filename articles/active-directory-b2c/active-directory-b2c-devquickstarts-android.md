...
> [AZURE.NOTE]
	Для работы с этим кратким руководством требуется веб-API, защищенный Azure AD с B2C. Мы создали для вас файлы .NET и node.js. В данном пошаговом руководстве предполагается, что образец веб-API в файле node.js уже настроен. См. [руководство по веб-API Azure AD B2C для Node.js](active-directory-b2c-devquickstarts-api-node.md).
...
- Введите `urn:ietf:wg:oauth:2.0:oob` как **URL-адрес ответа**. Это — URL-адрес по умолчанию для этого примера кода.
...
Код для этого учебника размещен на веб-сайте [GitHub](https://github.com/AzureADQuickStarts/B2C-NativeClient-Android). Чтобы построить образец, [скачайте схему проекта в ZIP-архиве](https://github.com/AzureADQuickStarts/B2C-NativeClient-Android/archive/skeleton.zip) или клонируйте ее.
