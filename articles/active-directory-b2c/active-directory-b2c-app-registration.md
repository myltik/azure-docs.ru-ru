...
4. Если вы создаете веб-приложение, переведите переключатель **Include web app / web API** (Включить веб-приложение или веб-API) в положение **Да**. **URL-адреса ответа** — это конечные точки, куда Azure AD B2C будет возвращать все токены, запрашиваемые вашим приложением. Например, введите `https://localhost:44316/`. Кроме того, если веб-приложение будет вызывать некоторые веб-API, защищенные с помощью Azure AD B2C, также потребуется создать **секрет приложения**, щелкнув **Создать ключ**.
