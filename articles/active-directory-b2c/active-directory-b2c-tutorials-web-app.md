---
title: "Руководство по использованию Azure Active Directory B2C для аутентификации пользователей в веб приложении ASP.NET"
description: "Руководство по использованию Azure Active Directory B2C для входа и регистрации пользователей в веб-приложении ASP.NET."
...
2. В разделе параметров B2C щелкните **Приложения**, а затем — **Добавить**.
    ...
    | **Собственный клиент** | Нет  | Так как это веб-приложение, а не собственный клиент, выберите "Нет". |
...
После регистрации веб-приложения и создания политик необходимо настроить приложение для использования клиента Azure AD B2C. В этом руководстве настройте пример веб-приложения. 
...
Пример веб-приложения ASP.NET представляет собой простое приложение для создания и обновления списка задач. Приложение использует [компоненты ПО промежуточного слоя Microsoft OWIN](https://docs.microsoft.com/en-us/aspnet/aspnet/overview/owin-and-katana/), которые позволяют пользователям регистрироваться для использования приложения в клиенте Azure AD B2C. Создав политику Azure AD B2C, пользователи могут использовать учетную запись социальной сети или создать учетную запись для использования в качестве идентификатора для доступа к приложению. 
...
2. В проекте веб-приложения **TaskWebApp** откройте файл **Web.config** и внесите следующие изменения:
...
Пример приложения поддерживает регистрацию пользователей, вход в систему, редактирование профилей и сброс паролей. Ниже описано, как пользователь регистрируется для использования приложения с помощью адреса электронной почты. Вы можете попробовать выполнить другие сценарии самостоятельно.
