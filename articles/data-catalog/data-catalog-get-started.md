...
> [AZURE.NOTE] Дополнительные сведения о подписках Azure и Azure Active Directory см. в статье [Предварительные условия для работы с каталогом данных Azure](data-catalog-prerequisites.md).
...
В этом упражнении мы установили образец базы данных Adventure Works, который будет использоваться далее. В следующем упражнении вы узнаете, как регистрировать источники данных **каталога данных Azure**, используя таблицы из образца базы данных Adventure Works.

## Упражнение 2. Регистрация источников данных

В этом упражнении мы зарегистрируем источники данных Adventure Works в **каталоге данных Azure** с помощью средства регистрации каталога данных Azure. Регистрация — это процесс извлечения ключевых метаданных структуры, таких как имена, типы и расположения, из источника данных и ресурсов, содержащихся в нем, и копирования этих метаданных в каталог. Источники данных и их данные остаются на своем месте, а каталог использует метаданные, чтобы упростить их обнаружение и понимание.
...
7.	Следующая страница — страница регистрации метаданных вашего источника данных. В этом примере мы регистрируем объекты **Production/Product** из пространства имен AdventureWorks Production. Вот как это сделать.
    ...
    а. В дереве **Иерархия серверов** щелкните элемент **Production**.
    ...
    c. Щелкните стрелку перемещения выбранных элементов (**>**). Так вы переместите все выбранные объекты Product в список **Объекты для регистрации**.
    ...
    Е. Щелкните **Зарегистрировать**. **Каталог данных Azure** зарегистрирует все выбранные объекты. В этом упражнении регистрируются выбранные объекты Adventure Works.
...
В **каталоге данных Azure** используется простой, но эффективный синтаксис поиска, который позволяет пользователям легко создавать запросы для получения нужных данных. Дополнительные сведения о поиске в **каталоге данных Azure** см. в статье [Data Catalog Search syntax reference (Справочник по синтаксису поисковых запросов в каталоге данных)](https://msdn.microsoft.com/library/azure/mt267594.aspx).
...
2. В поле **Поиск в каталоге данных** введите следующий **запрос с группированием**: (tags:description OR tags:photo).
...
В этом упражнении мы нашли и просмотрели с помощью портала **каталога данных Azure** зарегистрированные в каталоге ресурсы данных Adventure Works.
...
4. Введите **описание**, которое поможет другим пользователям понять, как и для чего можно использовать этот ресурс данных. Например, введите «Изображения продуктов». Также можно добавить дополнительные теги и просмотреть столбцы.
...
В этом упражнении мы подключимся к источнику данных с помощью портала **каталога данных Azure** и Microsoft Excel.
...
В **каталоге данных Azure** вы можете удалять ресурсы по одному или несколько ресурсов сразу.
...
В этом упражнении с помощью функций управления, которые предлагает **каталог данных Azure**, мы назначим владельца ресурсов данных. Кроме того, мы ограничим круг пользователей, которые могут находить ресурсы данных, а также определим, как эти пользователи смогут управлять этими ресурсами.
...
В этом упражнении мы изучили возможности управления, которые предлагает **каталог данных Azure**, и научились ограничивать видимость отдельных ресурсов данных.
