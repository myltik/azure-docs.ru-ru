...
| :--- | :---------- |
| [Подготовка каталога данных к работе](#provision-data-catalog) | Выполнив эту процедуру, вы подготовите или настроите каталог данных Azure. Этот шаг необходимо выполнить, только если каталог не был настроен ранее. Даже если с учетной записью Azure связано несколько подписок, в рамках одной организации можно настроить только один каталог данных (в домене Microsoft Azure Active Directory). |
| [Регистрация ресурсов данных](#register-data-assets) | С помощью этой процедуры вы зарегистрируете ресурсы данных из образца базы данных AdventureWorks2014 в каталоге данных. Регистрация — это процесс извлечения ключевых метаданных структуры, таких как имена, типы и расположения, из источника данных и копирования этих метаданных в каталог. Источники и ресурсы данных остаются на своем месте, а каталог использует метаданные, чтобы упростить их обнаружение и понимание. |
| [Поиск ресурсов данных](#discover-data-assets) | В рамках этой процедуры вы будете использовать портал каталога данных Azure для поиска ресурсов данных, зарегистрированных на предыдущем этапе. После регистрации источника данных в каталоге данных Azure служба индексирует его метаданные, чтобы пользователи могли легко находить необходимые данные. |
| [Заметки к ресурсам данных](#annotate-data-assets) | В рамках этой процедуры вы добавите к ресурсам данных аннотации (например, описания, теги, документацию, имена экспертов), чтобы дополнить метаданные, полученные из источника данных. Таким образом, источник данных будет понятен большему количеству пользователей. |
| [Подключение к ресурсам данных](#connect-to-data-assets) | В рамках этой процедуры с помощью сведений о подключении вы откроете ресурсы данных в интегрированных клиентских средствах (таких как Excel и SQL Server Data Tools), а также в среде SQL Server Management Studio, которая не является интегрированной. |
...
Подписки Azure помогают организовать доступ к ресурсам облачных служб, таких как каталог данных Azure. Они также позволяют управлять составлением отчетов об использовании ресурса, выставлением счетов за использование и их оплатой. Каждая подписка может иметь различные настройки для выставления счетов и их оплаты, поэтому у вас могут быть разные подписки и тарифные планы для разных отделов, проектов, региональных офисов и т. д. Каждая облачная служба привязана к подписке, поэтому необходимо иметь подписку перед настройкой каталога данных Azure. Дополнительные сведения см. в статье [Связь между подписками Azure и Azure Active Directory](../active-directory/active-directory-how-subscriptions-associated-directory.md).
...
Для настройки каталога данных Azure необходимо выполнить вход с помощью учетной записи пользователя Azure Active Directory. Пользователь должен быть владельцем или совладельцем подписки Azure.

Azure AD предоставляет компаниям простой способ управления удостоверениями и доступом не только в облаке, но и локально. Пользователи могут использовать одну рабочую или учебную учетную запись для входа во все облачные и локальные веб-приложения. Каталог данных Azure использует Azure AD для проверки подлинности входа. Дополнительные сведения см. в статье [Что такое Microsoft Azure Active Directory](../active-directory/active-directory-whatis.md).

### Настройка политики Azure Active Directory

Бывает, что пользователь может войти на портал каталога данных Azure, но при попытке входа в средство регистрации источника данных отображается сообщение об ошибке, которая запрещает вход в систему. Это может произойти, когда вы находитесь в локальной сети компании или пытаетесь подключиться за ее пределами.
...
1. Перейдите по ссылке на [страницу службы каталога данных](https://azure.microsoft.com/services/data-catalog) и нажмите кнопку **Начать**.
...
2. Выполните вход, используя учетную запись владельца или совладельца подписки Azure. После входа вы увидите следующую страницу.
...
1. На отдельной вкладке в браузере или в отдельном окне браузера перейдите по адресу [портала Azure](https://portal.azure.com) и войдите в систему с помощью той же учетной записи, которая использовалась для создания каталога данных на предыдущем этапе.
...
4.  Щелкните созданный вами каталог. Отобразится колонка **каталога данных** на портале.
...
В этом руководстве описывается, как регистрировать ресурсы данных (таблицы) из образца базы данных AdventureWorks2014 для ядра СУБД SQL Server, но вы можете использовать любой другой поддерживаемый источник, если предпочитаете работать с данными, которые вам знакомы и соответствуют вашей роли. Список поддерживаемых источников данных см. в статье [Источники данных, поддерживаемые каталогом данных Azure](data-catalog-dsr.md).
...
2. Чтобы восстановить базу данных на компьютере, следуйте инструкциям, приведенным в статье [Восстановление резервной копии базы данных (среда SQL Server Management Studio)](http://msdn.microsoft.com/library/ms177429.aspx), или выполните следующие инструкции.
	...
	3. В окне **Восстановление базы данных** выберите вариант **Устройство** в качестве **источника** и нажмите кнопку **Обзор**.
	4. В разделе **Выбор устройств резервного копирования** нажмите кнопку **Добавить**.
	...
	6. Нажмите кнопку **ОК**, чтобы закрыть диалоговое окно **выбора устройств резервного копирования**.
...
1.	Перейдите на [домашнюю страницу каталога данных Azure](https://azuredatacatlog.com) и щелкните **Опубликовать данные**.
...
3. На странице **Приветствие** щелкните **Войти** и введите учетные данные.
...
6.	Зарегистрируйте метаданные ресурса данных. В этом примере мы регистрируем объекты **Production/Product** из пространства имен Production базы данных AdventureWorks:
	...
	6. В поле **Добавить теги** введите **adventure works, cycles**. При этом теги поиска будут добавлены в эти ресурсы данных. Теги — отличный способ помочь пользователям найти зарегистрированный источник данных.
...
1. На портале каталога данных Azure нажмите кнопку **Home** (Главная). Если вы закрыли веб-браузер, перейдите на [домашнюю страницу каталога данных Azure ](https://www.azuredatacatalog.com).
...
3. Убедитесь, что в результатах отображаются все четыре таблицы и база данных (AdventureWorks2014). Чтобы переключиться между представлениями **сетки** и **списка**, используйте кнопки, расположенные на панели инструментов, как показано на следующем рисунке. Обратите внимание, что ключевое слово выделено в результатах поиска, так как переключатель **Highlight** (Выделение) находится в положении **ВКЛ.**. Можно также указать количество **результатов поиска на одной странице**.
	...
	Слева отображается панель **Searches** (Поиск), а справа — **Properties** (Свойства). На панели **Searches** (Поиск) можно изменить условия поиска и отфильтровать результаты. На панели **Properties** (Свойства) отображаются свойства выбранного объекта в представлении сетки или списка.

4. В результатах поиска выберите **Product**. Щелкните вкладки **Preview** (Предварительный просмотр), **Columns** (Столбцы), **Профиль данных** и **Documentation** (Документация), чтобы открыть их. Или нажмите стрелку, чтобы развернуть нижнюю панель.

	![Каталог данных Azure — нижняя панель](media/data-catalog-get-started/data-catalog-data-asset-preview.png)

	На вкладке **Preview** (Предварительный просмотр) отобразится представление предварительного просмотра данных в таблице **Product**.
5. Щелкните вкладку **Columns** (Столбцы), чтобы найти сведения о столбцах (например, **имя** и **тип данных**) в ресурсе данных.
...
7. Отфильтруйте результаты с помощью раздела **Filters** (Фильтры), расположенного с левой стороны. Например, если выбрать **Table** (Таблица) в качестве **типа объекта**, отобразятся только четыре таблицы без базы данных.
...
1. Снимите флажок **Table** (Таблица) для параметра **Object Type** (Тип объекта) в разделе **Filters** (Фильтры).
2. В поле поиска введите **tags:cycles** и нажмите клавишу **ВВОД**. Список всех свойств, которые можно использовать для поиска в каталоге данных, см. в [справочнике по синтаксису поиска в каталоге данных](https://msdn.microsoft.com/library/azure/mt267594.aspx).
...
1. На панели **Searches** (Поиск) раздела **Текущий поиск** введите имя для поиска и нажмите кнопку **Сохранить**.

	![Каталог данных Azure — сохранение поиска](media/data-catalog-get-started/data-catalog-save-search.png)
2. Убедитесь, что имя сохраненных условий поиска отображается в разделе **Saved Searches** (Сохраненные условия поиска).
...
2. Снимите флажок **Table** (Таблица) для параметра **Object Type** (Тип объекта).
3. Нажмите клавишу **ВВОД**.
4. Убедитесь, что в результатах поиска отображаются база данных AdventureWorks2014 и таблицы **Product**, **ProductCategory**, **ProductDescription** и **ProductPhoto**, которые вы зарегистрировали.
...
В этом упражнении с помощью портала каталога данных Azure мы создадим аннотации (внесем информацию, такую как описания, теги, имена экспертов) к ресурсам данных, которые уже зарегистрированы в каталоге. Добавленные заметки не только дополнят и расширят структурные метаданные, полученные из источника данных во время регистрации, но и упростят поиск и использование ресурсов данных.

В этом упражнении мы создадим аннотации к набору данных (ProductPhoto). Мы добавим понятное имя и описание для ресурса данных ProductPhoto.

1.  Перейдите на [домашнюю страницу каталога данных Azure](https://www.azuredatacatalog.com) и введите в поле поиска **tags:cycles** для поиска ресурсов данных, которые вы зарегистрировали.
2. Щелкните **ProductPhoto** в результатах поиска.
3. В поле **Friendly Name** (Понятное имя) введите **Product images** (Изображения продуктов), а в поле **Description** (Описание) — **Product photos for marketing materials** (Фотографии продуктов для рекламных материалов).
...
- Добавить экспертов для ресурса данных. Щелкните **Add** (Добавить) в области **Experts** (эксперты).
- Добавить теги на уровне набора данных. Щелкните **Add** (Добавить) в области **Tags** (тэги). Это может быть тег пользователя или тег глоссария. Стандартный выпуск каталога данных включает в себя бизнес-глоссарий, помогающий администраторам каталога определить основную таксономию бизнес-процессов. После этого пользователи каталога смогут добавлять заметки к ресурсам данных с помощью терминов из глоссария. Дополнительные сведения см. в статье [Как настроить бизнес-глоссарий для регулируемого добавления тегов](data-catalog-how-to-business-glossary.md).
- Добавить теги на уровне столбца. Нажмите кнопку **Add** (Добавить) в области **Tags** (Теги) для столбца, для которого необходимо добавить аннотацию.
- Добавить описание на уровне столбца. Введите **описание** для столбца. Можно также просмотреть метаданные описания, извлеченные из источника данных.
...
- Выберите вкладку **Documentation** (Документация) и укажите документацию для ресурса данных. Документация позволяет использовать каталог данных Azure в качестве репозитория содержимого, создавая с его помощью полное описание ресурсов данных.
...
Используя сведения о подключении, в этом упражнении вы откроете ресурсы данных в интегрированном клиентском средстве (Excel) и в среде SQL Server Management Studio, которая не является интегрированной.

> [AZURE.NOTE] Важно помнить, что каталог данных Azure не предоставляет вам доступ к исходному источнику данных. Он только облегчает поиск и понимание информации. При подключении к источнику данных выбранное вами клиентское приложение будет использовать ваши учетные данные Windows или запросит учетные данные при необходимости. Если раньше у вас не было доступа к источнику данных, вам нужно будет получить доступ, прежде чем вы сможете подключиться.

### Подключение к ресурсу данных из Excel

1. В результатах поиска выберите **Product**. На панели инструментов щелкните **Open In** (Открыть в) и выберите **Excel**.

    ![Каталог данных Azure — подключение к ресурсу данных](media/data-catalog-get-started/data-catalog-connect1.png)
2. Во всплывающем окне скачивания щелкните **Open** (Открыть). Это процедура выглядит иначе в разных браузерах.
...
4. В диалоговом окне **Import Data** (Импорт данных) оставьте значения по умолчанию и нажмите кнопку **OK**.
...
В этом упражнении вы подключились к ресурсам данных, найденным с помощью каталога данных Azure. Используя портал каталога данных Azure, вы можете подключаться напрямую к ресурсам с помощью клиентских приложений, интегрированных в меню каталога **Open in** (Открыть в). Кроме того, вы можете подключаться напрямую к ресурсам с помощью любого выбранного приложения, используя сведения о расположении подключения, указанные в метаданных ресурса. Например, вы можете использовать SQL Server Management Studio, чтобы подключиться к базе данных AdventureWorks2014 и получить доступ к данным в ресурсах, зарегистрированных при работе с этим руководством.

1. Откройте среду **SQL Server Management Studio**.
2. В диалоговом окне **Connect to Server** (Подключение к серверу) введите имя сервера, указанное на панели **Properties** (Свойства) на портале каталога данных Azure.
3. Используйте соответствующие учетные данные и проверку подлинности для доступа к ресурсу данных. Если у вас нет доступа, используйте сведения в поле **Request Access** (Запрос на доступ), чтобы получить его.
...
1. Перейдите на [домашнюю страницу каталога данных Azure](https://www.azuredatacatalog.com). В поле **поиска** введите **tags:cycles** и нажмите клавишу **ВВОД**.
2. В списке результатов щелкните элемент и нажмите кнопку **Take Ownership** (Стать владельцем) на панели инструментов.
3. На панели **Properties** (Свойства) в разделе **Management** (Управление) щелкните **Take Ownership** (Стать владельцем).

	![Каталог данных Azure — назначение владельцем](media/data-catalog-get-started/data-catalog-take-ownership.png)
4. Чтобы ограничить видимость, в разделе **Visibility** (Видимость) щелкните **Владельцы и эти пользователи** и нажмите кнопку **Add** (Добавить). Введите адрес электронной почты пользователя в соответствующем текстовом поле и нажмите клавишу **ВВОД**.
...
3. В списке результатов выберите элемент и нажмите кнопку **Delete** (Удалить) на панели инструментов, как показано на следующем рисунке.
