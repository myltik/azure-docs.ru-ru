...
- Возможность просмотра и копирования строк подключения из портала **каталога данных Azure** для источников данных SQL Server (включая базу данных SQL Azure). Пользователи могут перейти по ссылке "Просмотр строк подключения" в сведениях о подключении для таблицы, представления или базы данных SQL Server, чтобы просмотреть строки подключения ADO.NET, ODBC, OLEDB и JDBC, используемые для подключения к источнику данных.
- Поддержка включения профилей данных при регистрации таблиц и представлений Teradata.
- Поддержка команды "Открыть в Power BI Desktop" для источников данных SQL Server (включая базу данных SQL Azure и хранилище данных SQL Azure) и SQL Server Analysis Services.

> [AZURE.NOTE]Для работы функции "Открыть в Power BI Desktop" должна быть установлена текущая версия приложения Power BI Desktop. При возникновении проблем или ошибок при работе с этой функцией убедитесь в наличии последней версии Power BI Desktop, загруженной с сайта [PowerBI.com](https://powerbi.com).
...
- Добавлена новая кнопка "Пользователь" и меню в верхней панели навигации. С помощью этого меню пользователь может отображать сведения об учетной записи, используемой для входа в **каталог данных Azure** и, при необходимости, для выхода из него. В этом меню также отображается имя каталога, которое может пригодиться разработчикам, использующим REST API **каталога данных Azure**.
