<properties title="Learn how to configure an Azure website to use a custom domain name" pageTitle="Настройка пользовательского имени домена для веб-сайта Azure" metaKeywords="Azure, Azure Web Sites, domain name" description="" services="web-sites" documentationCenter="" authors="larryfr, jroth" manager="wpickett" />
...
<div class="dev-center-tutorial-subselector"><a href="/ru-ru/documentation/articles/web-sites-custom-domain-name/" title="Websites" class="current">Веб-сайт</a> | <a href="/ru-ru/documentation/articles/web-sites-traffic-manager-custom-domain-name/" title="Website using Traffic Manager">Веб-сайт с использованием диспетчера трафика</a></div>
...
- Запись **CNAME (каноническое имя)** сопоставляет доменное имя с другим доменным именем. DNS использует второе имя для поиска адреса. Пользователи по-прежнему видят в своем браузере первое доменное имя. Например, можно сопоставить contoso.com с сайтом *&lt;вашсайт&gt;*.azurewebsites.net. 
...
> [WACOM.NOTE] IP-адрес может измениться при удалении и повторном создании веб-сайта или изменения его режима на бесплатный.
...
1.	Найдите страницу управления записями DNS. Найдите ссылки или области сайта, обозначенные как **Имя домена**, **DNS** или **Управление сервером доменных имен**. Часто эту ссылку можно найти, открыв сведения своей учетной записи и найдя такую ссылку, как **Мои домены**.
...
В процессе создания или изменения записи соответствующие поля позволят сопоставить доменное имя с IP-адресом (для А-записей) или с другим доменом (для записей CNAME). Для записи CNAME будете выполняться сопоставление *от* своего пользовательского домена *к* поддомену azurewebsites.net. 
...
- **contoso.com** сопоставлен с 127.0.0.1.
...
- *Если А-запись сопоставлена с корневым доменом или с доменом с подстановочными знаками:* Создайте запись CNAME, сопоставляющую **awverify.&lt;yourdomain&gt;** с **awverify.&lt;yourwebsitename&gt;.azurewebsites.net**.  Например, если А-запись создана для **contoso.com**, то создайте запись CNAME для **awverify.contoso.com**.
- *Если А-запись сопоставлена с конкретным доменом:* Создайте запись CNAME, сопоставляющую **awverify.&lt;subdomain&gt;** с **awverify.&lt;yourwebsitename&gt;.azurewebsites.net**. Например, если А-запись создана для **blogs.contoso.com**, создайте запись CNAME для **awverify.blogs.contoso.com**.
