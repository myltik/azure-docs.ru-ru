...
-   [Регистрация скрипта для операции с таблицей][Регистрация нового скрипта таблицы]
-   [Вывод списка таблиц]
- 	[Удаление существующей таблицы]
-	[Вывод списка мобильных служб]
...
+ Если ее нет, можно создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе <a href="http://www.windowsazure.com/pricing/free-trial/" target="_blank">Бесплатная пробная версия Azure</a>.
...
В этом разделе описывается набор общих задач администрирования, поддерживаемых средствами командной строки Azure. Дополнительные сведения см. в [документации по программам командной строки Azure][reference-docs].

<!--+  Необходимо загрузить и установить программы командной строки Azure на локальный компьютер. Чтобы сделать это, следуйте инструкциям в первой части данного раздела. 

+ (Необязательно) Чтобы выполнять HTTP-запросы непосредственно из командной строки, необходимо использовать cURL или эквивалентное средство. cURL работает на различных платформах. Найдите и установите cURL для своей платформы на <a href=http://go.microsoft.com/fwlink/p/?LinkId=275676 target="_blank">странице загрузки cURL</a>.-->
...
* **Windows**: загрузите [установщик программ командной строки Azure][windows-installer]. Откройте загруженный MSI-файл и выполните установку.
...
После этого откроется браузер по умолчанию и появится запрос на вход в портал управления. После входа загружается файл `.publishsettings`. Запишите расположение сохраненного файла.

Затем импортируйте файл `.publishsettings` с помощью следующей команды, заменив `<path-to-settings-file>` на путь к вашему файлу `.publishsettings`:

		azure account import <path-to-settings-file>

Всю информацию, сохраненную с помощью команды <code>импорт</code> можно удалить с помощью команды <code>account clear</code> команду:
...
После импорта параметров публикации необходимо удалить файл `.publishsettings` из соображений безопасности. Дополнительные сведения см. в разделе [Установка программ командной строки Azure для Mac и Linux][Установка средств командной строки Azure для Mac и Linux]. Теперь вы можете приступить к созданию мобильных служб Azure и начать управлять ими с помощью командной строки или пакетных файлов.  
...
<h2><a name="delete-service"></a>Удаление существующей мобильной службы</h2>
...
> [AZURE.NOTE] Если вы не укажете параметр <code>-q</code> вместе с параметром <code>-a</code> или <code>-d</code>, выполнение будет приостановлено и отобразится запрос для выбора параметров удаления базы данных SQL. Используйте только параметр <code>-a</code> если другие службы не используют базу данных или сервер. В противном случае используйте параметр <code>-d</code> , чтобы удалить только те данные, которые принадлежат удаляемой мобильной службе.

<h2><a name="create-table"></a>Создание таблицы в мобильной службе</h2>
...
В результате этого будет создана новая таблица с разрешениями по умолчанию `application` для операций с таблицей: `insert`, `read`, `update` и `delete`. 

Следующая команда создает новую таблицу с общедоступным разрешением общего доступа `read`, но разрешение `delete` предоставляется только администраторам:

		azure mobile table create <service-name> <table-name> -p read=public,delete=admin

В следующей таблице показано значение разрешения скрипта в сравнении ос значением разрешения на [портале управления Azure][Портал управления Azure].
...
<h2><a name="delete-table"></a>Удаление существующей таблицы из мобильной службы</h2>

Следующая команда удаляет таблицу из мобильной службы, где `<service-name>` - имя мобильной службы, а `<table-name>` - это имя удаляемой таблицы:
...
<h2><a name="register-script"></a>Регистрация скрипта для операции с таблицей</h2>
...
Дополнительные сведения о регистрации скриптов сценариев см. в [справочнике по серверным скриптам мобильных служб][Справочник серверных скриптов мобильных служб].

<!--<h2><a name="test-service"></a>Тестирование новой мобильной службы</h2>
...
-->
<!-- Anchors. -->
[Загрузка и установка программ командной строки]: #install
...
[Регистрация нового скрипта таблицы]: #register-script
...
[Вывод списка мобильных служб]: #list-services
[Вывод списка таблиц]: #list-tables
...
[Установка средств командной строки Azure для Mac и Linux]: http://go.microsoft.com/fwlink/p/?LinkId=275795
