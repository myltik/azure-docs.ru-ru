<properties urlDisplayName="Using Offline Data" pageTitle="Использование автономных данных в мобильных службах (Xamarin iOS) | Центр мобильных разработок" metaKeywords="" description="Узнайте, как использовать мобильные службы Azure для кэширования и синхронизации автономных данных в приложении на Xamarin для iOS" metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Using offline data in Mobile Services" authors="donnam" editor="wesmc" manager="dwrede"/>
...
* [Пакет SDK для мобильных служб Azure версии 1.3.0 (или более поздней версии)][Mobile Services SDK Nuget]
* [Хранилище SQLite для мобильных служб Azure версии 1.0.0 (или более поздней версии)][SQLite store nuget]

>[WACOM.NOTE] Дальнейшие инструкции предполагают, что вы используете Visual Studio 2012 или более поздних версий с расширением Xamarin. При использовании Xamarin Studio OS X воспользуйтесь встроенной поддержкой диспетчера пакетов NuGet.
...
Готовый проект, выполненный по этому учебнику, можно найти [здесь](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.iOS).
...
Измените класс QSTodoService, включив использование автономных функций мобильных служб для локального хранилища SQLite
...
4. В конструкторе QSTodoService добавьте вызов SQLitePCL.CurrentPlatform.Init() второй строкой кода:
...
Внесите изменения в QSTodoListViewController, чтобы новый метод SyncAsync вызывался при использовании жеста обновления.
 
1. Добавьте вызов InitializeStoreAsync в функцию ViewDidLoad() после инициализации todoService:
...
2. Измените метод AddRefreshControl так, чтобы метод SyncAsync вызывался до RefreshAsync:
...
5. Находясь в приложении, сделайте жест обновления, потянув вниз список элементов. После этого приложение вызовет методы MobileServiceClient.SyncContext.PushAsync и IMobileServiceSyncTable.PullAsync(), а затем выполнит код RefreshTodoItems для обновления приложения элементами из локального хранилища. 
...
*  Для отправки изменений на сервер мы вызвали метод IMobileServiceSyncContext.PushAsync(). Этот метод - член объекта IMobileServicesSyncContext, а не таблицы синхронизации, поскольку изменения будут передаваться во все таблицы:
...
Пример проекта, выполненного по этому учебнику, можно скачать из нашего [репозитория примеров на GitHub](https://github.com/Azure/mobile-services-samples/tree/master/TodoOffline/Xamarin.iOS).
