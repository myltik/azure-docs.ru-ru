...
1. В Visual Studio откройте проект, созданный в ходе работы с учебником [Приступая к работе с мобильными службами]. Откройте файл `QSTodoService.cs`.

2. Обратите внимание, что тип элемента `todoTable` — `IMobileServiceSyncTable`. При автономной синхронизации этот интерфейс таблицы синхронизации используется вместо `IMobileServiceTable`. При использовании таблицы синхронизации все операции направляются в локальное хранилище и синхронизируются с удаленной службой только по явным операциям принудительной отправки и принудительного получения.

    Чтобы получить ссылку на таблицу синхронизации, используйте метод `GetSyncTable()`. Чтобы удалить функцию автономной синхронизации, можно воспользоваться `GetTable()`.

3. Прежде чем можно будет выполнить операции с таблицами, необходимо инициализировать локальное хранилище. Это делается с помощью метода `InitializeStoreAsync`.
    ...
    Метод `DefineTable` создает в локальном хранилище таблицу, совпадающую с полями в указанном типе (в этом случае — `ToDoItem`). Тип необязательно должен включать в себя все столбцы, которые находятся в удаленной базе данных, так как можно хранить и подмножество столбцов.
...
4. Метод `SyncAsync` запускает операцию фактической синхронизации:
    ...
    Сначала выполняется вызов `IMobileServiceSyncContext.PushAsync()`. Этот метод входит в объект `IMobileServicesSyncContext`, а не в таблицу синхронизации, так как изменения будут переданы во все таблицы. Только те записи, которые были каким-либо образом локально изменены (с помощью операций CUD), будут отправлены на сервер.
...
5. В классе `QSTodoService` метод `SyncAsync()` вызывается после операций, которые изменяют данные, а именно `InsertTodoItemAsync()` и `CompleteItemAsync`. Он также вызывается из `RefreshDataAsync()`, поэтому пользователь получает последние данные при каждом выполнении жеста обновления. Приложение также выполняет синхронизацию при запуске, так как `QSTodoListViewController.ViewDidLoad()` вызывает `RefreshDataAsync()`.
...
3. Чтобы обеспечить синхронизацию данных при обновлении, измените метод `QSTodoListViewController.RefreshAsync()`. Добавьте вызов в `SyncAsync()` перед вызовом `RefreshDataAsync()`:
