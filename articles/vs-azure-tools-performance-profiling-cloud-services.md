---
title: Тестирование производительности облачной службы | Microsoft Docs
...
3. Чтобы просмотреть отчеты о профилировании экземпляра, выберите роль в службе, откройте контекстное меню конкретного экземпляра и выберите **Просмотреть отчет профилирования**.
    ...
    VSP-файл отчета будет загружен из Azure, а состояние загрузки отобразится в журнале действий Azure. После завершения загрузки отчет о профилировании откроется в редакторе Visual Studio на вкладке с именем <Role name>*<Instance Number>*<identifier>.vsp. Будут показаны сводные данные отчета.
