<properties urlDisplayName="Website with Storage" pageTitle="Веб-сайт на Node.js с табличным хранилищем - Microsoft Azure" metaKeywords="Azure table storage Node.js, Azure Node.js application, Azure Node.js tutorial, Azure Node.js example" description="В этом уроке вы узнаете, как использовать службу таблиц Azure для сохранения данных из приложения Node на веб-сайте Azure." metaCanonical="" services="web-sites,storage" documentationCenter="nodejs" title="Node.js Web Application using the Azure Table Service" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
В этом учебнике показано, как использовать службу таблиц, предоставляемую службой управления данными Azure, для хранения данных и доступа к ним из приложения [node], размещенного в Azure. Этот учебник разработан для читателей, обладающих определенным опытом использования Node и [Git].
...
Файлы проекта для этого учебника будут храниться в каталоге с именем **tasklist**, завершенное приложение будет выглядеть примерно следующим образом:

![A web page displaying an empty tasklist][node-table-finished]

> [WACOM.NOTE] Этот учебник ссылается на папку **tasklist**. Полный путь к этой папке опущен, поскольку семантика путей зависит от операционной системы. Эту папку нужно создать в легкодоступном месте локальной файловой системы, например **~/node/tasklist** или **c:\node\tasklist**.
...
1. Откройте браузер и перейдите на [портал Azure]. При необходимости войдите в систему, используя данные своей подписки Azure.
...
2. Введите следующую команду для установки Express.
    ...
    > [WACOM.NOTE] При использовании параметра -g в некоторых операционных системах может появиться сообщение об ошибке **Error: EPERM, chmod '/usr/local/bin/express'** предложение попробовать использовать учетную запись с правами администратора. В этом случае необходимо с помощью команды **sudo** запустить npm с более высоким уровнем привилегий.
	...
	> [WACOM.NOTE] Параметр -g, используемый при установке модуля express, устанавливает его глобальным образом. Это делается для того, чтобы можно было получить доступ к команде **express** для формирования шаблонов веб-сайта без необходимости вводить дополнительную информацию о путях.**
...
2. В начале файла добавьте следующий код для загрузки модуля azure, задайте имя таблицы, partitionKey, а также учетные данные хранилища, используемые в этом примере:
	...
	> [WACOM.NOTE] nconf загружает значения конфигурации из переменных среды или файла **config.json**, который будет создан позже.
...
3. Прокрутите файл app.js вниз до появления следующей строки:
	...
	Замените вышеприведенные строки на код, приведенный ниже. Это инициализирует экземпляр <strong>Задача</strong> с подключением к своей учетной записи хранения. Это передается <strong>TaskList</strong>, который будет использоваться для взаимодействия со службой таблиц:
...
Файл **layout.jade** в каталоге **views** используется как глобальный шаблон для других файлов **.jade**. На этом шаге мы внесем необходимые изменения для использования [Twitter Bootstrap](https://github.com/twbs/bootstrap)- набор инструментов, с помощью которого можно легко получить красивый веб-сайт.

1. Скачайте и извлеките файлы [Twitter Bootstrap](http://getbootstrap.com/). Скопируйте файл **bootstrap.min.css** из папки **bootstrap\\dist\\css** в каталог **public\\stylesheets** своего приложения tasklist.
...
> [WACOM.NOTE] Эти действия также можно выполнить с помощью портала Azure. Указания по использованию портала Azure для развертывания приложения Node.js см. в статье [Создание и развертывание приложения Node.js на веб-сайте Azure].
...
> [WACOM.NOTE] Дополнительную информацию см. в разделе [Установка и настройка межплатформенного интерфейса командной строки Azure](/ru-ru/documentation/articles/xplat-cli/).
...
3. После завершения загрузки файла используйте для импорта параметров следующую команду:
	...
	Укажите путь и имя файла параметров публикации, загруженного на предыдущем шаге. Результат выполнения команды должен быть похож на следующее:
		...
		информация:   Setting service endpoint to: management.core.windows.net
		информация:   Setting service port to: 443
		информация:   Found subscription: YourSubscription
		информация:   Setting default subscription to: YourSubscription
		предупреждение:   The 'C:\users\username\downloads\YourSubscription-6-7-2012-credentials.publishsettings' file contains sensitive information.
...
2. Используйте следующую команду для создания нового веб-сайта Azure:
	...
	После выполнения этой команды должен появиться результат, похожий на следующий. Обратите внимание, что строка, начинающаяся с **Web site created at** (веб-сайт, созданный по адресу), содержит URL-адрес веб-сайта.
		...
		информация:   Executing command site create
		help:   Need a site name
		Имя: TableTasklist
		информация:   Using location southcentraluswebspace
		информация:   Executing `git init`
		информация:   Creating default .gitignore file
		информация:   Creating a new web site
		информация:   Created web site at  tabletasklist.azurewebsites.net
		информация:   Initializing repository
		информация:   Repository initialized
		информация:   Executing `git remote add azure https://username@tabletasklist.azurewebsites.net/TableTasklist.git`
		информация:   site create command OK

	> [WACOM.NOTE] Если это первый веб-сайт Azure для вашей подписки, необходимо использовать портал для создания веб-сайта. Дополнительную информацию см. в статье [Создание и развертывание приложения Node.js на веб-сайте Azure].
...
3. При принудительной отправке последних изменений репозитория Git на веб-сайт Azure необходимо указать, что целевой ветвью, используемой для содержимого данного веб-сайта, является ветвь **master**.
...
4. После завершения операции принудительной отправки перейдите по URL-адресу веб-сайта, возвращенному ранее командой azure create site, чтобы просмотреть свое приложение.
...
3. В разделе **параметры приложения** введите **STORAGE_NAME** в поле **КЛЮЧ** и имя вашей учетной записи хранения в поле **ЗНАЧЕНИЕ**. Щелкните значок галочки для перехода к следующему полю. Повторите этот процесс для следующих ключей и значений:
...
После развертывания изменений в Azure, веб-приложение должно продолжить работать, поскольку оно теперь считывает строку подключения из записи **параметры приложения**. Для проверки измените значение записи **STORAGE_KEY** в разделе **параметры приложения** на недействительное значение. После сохранения этого значения веб-сайт должен перестать работать из-за неправильного значения ключа доступа к хранилищу.

##Дальнейшие действия

Хотя в действиях этой статьи описывается использование службы таблиц для хранения информации, можно также использовать MongoDB. Дополнительную информацию см. в статье [Веб-приложение Node.js с MongoDB].
...
[Программы командной строки Azure для Mac и Linux]: /ru-ru/documentation/articles/xplat-cli/
