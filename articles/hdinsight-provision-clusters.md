...
Изучите различные способы настраиваемой подготовки кластеров Hadoop в Azure HDInsight с использованием портала Azure, Azure PowerShell, программ командной строки или пакета SDК для HDInsight .NET. Указания по подготовке кластеров HBase и кластеров Storm см. в разделах [Подготовка кластера HBase в HDInsight](hdinsight-hbase-get-started.md) и [Приступая к работе с Storm в HDInsight](hdinsight-storm-getting-started.md). Чтобы понять, почему следует выбрать тот или иной кластер, см. раздел <a href="http://go.microsoft.com/fwlink/?LinkId=510237">В чем разница между Hadoop и HBase?</a>.
...
Кластер HDInsight использует контейнер хранилища больших двоичных объектов Azure в качестве файловой системы по умолчанию. Перед созданием кластера HDInsight необходимо создать учетную запись хранения Azure для того же центра обработки данных. Дополнительные сведения см. в разделе [Использование хранилища BLOB-объектов Azure с HDInsight](hdinsight-use-blob-storage.md). Дополнительную информацию о создании учетной записи хранения Azure см. в разделе [Создание учетной записи хранения](storage/storage-create-storage-account.md).
...
> [AZURE.NOTE]Начиная с 29.8.2014 кроссплатформенный интерфейс командной строки не может использоваться для связывания кластера с виртуальной сетью Azure.

Другим вариантом подготовки кластера HDInsight является использование кроссплатформенного интерфейса командной строки. Средство командной строки реализовано в Node.js. Его можно использовать на любой платформе, которая поддерживает Node.js, включая Windows, Mac и Linux. CLI можно установить из следующих расположений:
...
- **кроссплатформенный интерфейс командной строки** — <a href="https://github.com/Azure/azure-xplat-cli/archive/hdinsight-February-18-2015.tar.gz" target="_blank">https://github.com/Azure/azure-xplat-cli/archive/hdinsight-February-18-2015.tar.gz</a>.  
...
Приведенные ниже инструкции помогут в установке кроссплатформенной командной строки на базе Linux и Windows, а затем использовать командную строку для подготовки кластера.
...
- Установка кроссплатформенной командной строки с помощью диспетчера пакетов Node.js (NPM).
...
Интерфейс командной строки можно установить с помощью NPM или установщика Windows. Корпорация Майкрософт рекомендует установку с использованием только одного из двух параметров.
...
1.	Перейдите в **http://azure.microsoft.com/downloads/**.2.	Прокрутите вниз до раздела **Средства командной строки**, щелкните **Кроссплатформенный интерфейс командной строки** и следуйте инструкциям мастера установщика веб-платформы.
...
Сведения о создании новой учетной записи хранения Azure с помощью портала Azure см. в статье [Создание и удаление учетной записи хранения, а также управление ею](storage/storage-create-storage-account.md).
