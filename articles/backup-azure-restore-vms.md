...
2. Чтобы выбрать более раннюю точку, в раскрывающемся списке выберите параметр **Выбор даты** и, щелкнув значок календаря, выберите дату. В календаре все даты с точками восстановления обозначены светло-серым цветом и доступны для выбора пользователем.
    ...
    После выбора даты в календаре доступные на эту дату точки восстановления отобразятся в таблице ниже. В столбце **Время** указано время, когда был сделан моментальный снимок.  **Тип** столбец отображает [согласованности](https://azure.microsoft.com/documentation/articles/backup-azure-vms/#consistency-of-recovery-points) точки восстановления. В заголовке таблицы в скобках показано количество точек восстановления, доступных в этот день.
...
4. Выберите подсеть. Если виртуальная сеть имеет подсети, по умолчанию будет выбрана первая подсеть. Выберите подсеть по своему усмотрению из раскрывающегося списка. Сведения о подсети, см. в статье расширения сети в [домашней странице портала](https://manage.windowsazure.com/), перейдите к виртуальным сетям и выберите виртуальную сеть и детализацию до настройки, чтобы просмотреть сведения о подсети.
...
После восстановления виртуальной машины может потребоваться повторно установить расширения, существующие в исходной виртуальной Машины и [заново создать конечные точки](virtual-machines-set-up-endpoints) для виртуальной машины на портале Azure.
