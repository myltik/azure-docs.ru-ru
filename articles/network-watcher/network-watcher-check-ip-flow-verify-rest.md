---
title: "Проверка трафика с использованием проверки потока IP-адресов с помощью Наблюдателя за сетями (REST) | Документация Майкрософт"
description: "В этой статье описывается, как проверить состояние передачи входящего и исходящего трафика виртуальной машины (разрешен или запрещен)."
...
> [!NOTE]
> Для проверки IP-потока необходимо, чтобы был выделен ресурс виртуальной машины.
...
Если трафик блокируется, чего не должно быть, прочитайте статью [Управление группами безопасности сети с помощью портала](../virtual-network/virtual-network-manage-nsg-arm-portal.md), чтобы узнать больше о группах безопасности сети.
