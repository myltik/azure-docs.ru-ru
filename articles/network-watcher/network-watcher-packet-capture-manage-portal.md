...
> [!NOTE]
> Для записи пакетов требуется подключение к учетной записи хранения через порт 443.
...
- **Хранилище**. Определяет, нужно ли сохранять запись пакетов в учетной записи хранилища.
...
- **Локальный путь к файлу**. Локальный путь на виртуальной машине для сохранения записи пакетов. (Этот параметр доступен, только если выбран параметр **Файл**.) Необходимо указать допустимый путь
...
- **Максимальное число байтов за сеанс**. Общее число записываемых байтов. Запись пакетов прекращается, когда будет достигнуто указанное значение.
- **Ограничение по времени (в секундах)**. Задает ограничение на время записи пакетов. По умолчанию это 1800 секунд.
...
Когда сеанс записи пакетов завершится, файл записи передается в хранилище BLOB-объектов или сохраняется в локальный файл на виртуальной машине. Расположение для хранения записи пакетов определяется при создании сеанса. Для доступа к файлам записи, сохраненным в учетной записи хранения, удобно использовать обозреватель службы хранилища Microsoft Azure, который можно скачать по адресу http://storageexplorer.com/.
