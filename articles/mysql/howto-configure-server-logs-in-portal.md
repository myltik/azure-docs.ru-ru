---
title: "Настройка журналов сервера в базе данных Azure для MySQL и получение к ним доступа с помощью Azure CLI | Документация Майкрософт"
...
5. Нажмите кнопку развертывания **Показать больше**, чтобы просмотреть развернутый список доступных параметров. 
   ...
   Дополнительные сведения об определениях параметров см. в документации MySQL по [журналам](https://dev.mysql.com/doc/refman/5.7/en/slow-query-log.html).

   ![Нажатие кнопки "Показать больше" для отображения полного списка](./media/howto-configure-server-logs-in-portal/2-show-more.png)
