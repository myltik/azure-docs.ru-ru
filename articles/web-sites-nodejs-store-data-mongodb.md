<properties linkid="develop-node-website-with-mongodb-mac" urlDisplayName="Веб-сайт с MongoDB" pageTitle="Веб-сайт Node.js с MongoDB на виртуальной машине — учебный курс по Azure" metaKeywords="Учебный курс Azure MongoDB, данные хранилища MongoDB, доступ к данным MongoDB Node, приложение Azure Node" description="Учебный курс, показывающий, как использовать MongoDB для хранения данных и доступа к ним из приложения Node, размещенного в Azure." metaCanonical="http://www.windowsazure.com/ru-ru/develop/nodejs/tutorials/website-with-mongodb-mongolab/" services="web-sites,virtual-machines" documentationCenter="Node.js" title="Веб-приложение Node.js с хранилищем в MongoDB (виртуальная машина)" authors=""  solutions="" writer="" manager="" editor=""  />
...
> [WACOM.NOTE] Многие из описанных ниже действий могут быть выполнены из командной строки. Для выполнения этих действий используйте командную строку своей операционной системы, например __Windows PowerShell__ (Windows) или __Bash__ (Unix Shell). На компьютерах с OS X доступ к командной строке можно получить через приложение Terminal.
...
Хотя можно создать новую ВМ и затем установить на нее MongoDB, следуя [руководству по установке MongoDB][installguides], большая часть работы уже была проделана сообществом и доступна в VM Depot. Далее показано, как использовать образ из VM Depot, в котором база данных MongoDB уже установлена и настроена.

1. Войдите на [портал управления Azure][azureportal], выберите __Виртуальные машины__, выберите __Образы__ и __VM Depot__.
...
2. Выберите образ, содержащий MongoDB. В данном случае выбран Ubuntu, чтобы показать только образы, основанные на дистрибутиве Ubuntu Linux. Наконец, я выбираю образ MongoDB v2.2.3 on Hardened Ubuntu.
	...
	> [WACOM.NOTE] Выберите элемент __Дополнительно__ для просмотра всех сведений об образе. Для некоторых образов необходимо задать дополнительные настройки после создания виртуальной машины с помощью образа.
...
3. Выберите регион и учетную запись хранения, которые будут использоваться для хранения VHD для этого образа. Установите флажок для продолжения.
	...
	![снимок экрана выбора учетной записи хранения][selectstorage]
...
4. После изменения состояния образа на __Ожидается регистрация__ выберите элемент __Регистрация__ и введите понятное имя нового образа. Установите флажок для продолжения.

	![снимок экрана регистрации образа][register]

5. После изменения состояния образа на __Доступно__ выберите __+ Новый__, __Виртуальная машина__, __Из коллекции__. В ответ на запрос __выбора образа__ выберите __Мои образы__, а затем выберите образ, созданный на предыдущих шагах. Щелкните стрелку для продолжения.

	![снимок экрана образа][myimage]

6. Введите имя, размер и имя пользователя виртуальной машины. Щелкните стрелку для продолжения.

	![снимок экрана имени ВМ, имени пользователя и т. д.][vmname]
	...
	>
	> Дополнительные сведения об использовании SSH на ВМ с Linux в платформе Azure см. в разделе [Как использовать SSH с Linux на Azure][sshazure].
...
9. После изменения состояния виртуальной машины на __Выполняется__ вы сможете открыть в браузере адрес __http://&lt;DNS-имя_ВМ&gt;.cloudapp.net:28017/__, чтобы проверить работу MongoDB. В нижней части страницы должен отображаться журнал со сведениями о службе, аналогично следующим данным:
...
2. Используйте следующую команду для создания нового веб-сайта Azure. Замените "myuniquesitename" уникальным именем созданного веб-сайта. Это значение используется как часть URL-адреса для созданного веб-сайта.
	...
	После выполнения этой команды должен появиться результат, похожий на следующий. Обратите внимание, что строка, начинающаяся с **Created web site at** (Создан веб-сайт на), содержит URL-адрес веб-сайта.
	...
	> [WACOM.NOTE> Если это первый веб-сайт Azure для вашей подписки, будет рекомендовано использовать портал для создания веб-сайта. Дополнительные сведения см. в статье <a href="/ru-ru/develop/nodejs/tutorials/create-a-website-(mac)/">Создание и развертывание приложения Node.js на веб-сайте Azure</a>.

###Задание переменной среды MONGODB_URL
