<properties
   pageTitle="Настройка подключения VNet-VNet"
   description="Подключения между виртуальными сетями поддерживает подключение виртуальных сетей Azure. Не поддерживается подключение виртуальных машин или облачных служб, размещенных НЕ в виртуальной сети."
...
# Настройка подключения VNet-VNet

Подключение виртуальной сети \(VNet\) Azure к другой виртуальной сети Azure очень похоже на подключение виртуальной сети к расположению локального сайта. Оба типа подключений используют шлюз виртуальной сети для предоставления защищенного туннеля, использующего IPsec/IKE. Подключаемые виртуальные сети могут иметь разные подписки в разных регионах. Можно даже комбинировать подключение виртуальных сетей с многосайтовыми конфигурациями. Это позволяет устанавливать топологии сети, совмещающие распределенные подключения с подключениями между виртуальными сетями, как показано на схеме ниже:

![Схема подключения между виртуальными сетями](./media/virtual-networks-configure-vnet-to-vnet-connection/IC727360.png)

## Что можно делать с подключениями между виртуальными сетями?
...
### Межрегиональная географическая избыточность и географическое присутствие
  ...
  - С помощью подсистемы балансировки нагрузки Azure и технологий кластеризации корпорации Майкрософт или сторонних производителей можно настроить высокодоступную рабочую нагрузку с географической избыточностью в нескольких регионах Azure. Одним из важных примеров является настройка SQL Always On с группами доступности, распределенными по нескольким регионам Azure.
...
### Обмен данными между подписками и организациями в Azure
  - Если у вас несколько подписок Azure, теперь вы можете безопасно связывать рабочие нагрузки из разных подписок между виртуальными сетями.
  - Теперь, благодаря технологии защищенных сетей VPN в Azure, для предприятий или поставщиков услуг теперь возможен обмен данными между организациями.
...
### Требования и рекомендации
  ...
  - Для подключения между виртуальными сетями требуются шлюзы VPN Azure и сети VPN с динамической маршрутизацией — сети VPN Azure со статической маршрутизацией не поддерживаются. Для подключения нескольких виртуальных сетей Azure между собой НЕ требуются локальные шлюзы VPN, если только не требуется распределенное подключение.
  - Подключение к виртуальной сети может использоваться одновременно с многосайтовыми сетями VPN, использующими до 10 туннелей VPN для подключения шлюза VPN виртуальной сети к другим виртуальным сетям или локальным сайтам.
...
В этой процедуре главным образом используется портал управления, тем не менее, для подключения шлюзов VPN необходимо использовать командлеты Microsoft Azure PowerShell.
...
1. [Спланируйте диапазоны IP-адресов.](#plan-your-ip-address-ranges) 
2. [Создайте свои виртуальные сети.](#create-your-virtual-networks) 
3. [Добавьте локальные сети.](#add-local-networks) 
4. [Создайте шлюзы с динамической маршрутизацией для каждой виртуальной сети.](#create-the-dynamic-routing-gateways-for-each-vnet) 
...
5. На странице **Адресное пространство виртуальной сети** укажите диапазон адресов, который вы хотите использовать для виртуальной сети. Это динамические IP-адреса \(DIP\), которые будут назначаться виртуальным машинам и другим экземплярам ролей, развертываемым в этой виртуальной сети. Существует довольно много правил, касающихся адресного пространства виртуальной сети, поэтому вы захотите взглянуть на страницу [Адресное пространство виртуальной сети](https://msdn.microsoft.com/library/azure/09926218-92ab-4f43-aa99-83ab4d355555#BKMK_VNET_ADDRESS) для получения дополнительной информации. Особенно важно выбрать диапазон, который не пересекается с другими диапазонами, используемыми в локальной сети. Необходимо согласование с сетевым администратором, которому может потребоваться выделить диапазон IP-адресов из адресного пространства локальной сети, чтобы использовать его для виртуальной сети.
...
6. Установите флажок в нижнем правом углу страницы, после чего начнется создание виртуальной сети. По завершении на странице **Сети** портала управления в поле **Состояние** появится значение «Создано».

7. Создайте еще одну виртуальную сеть. Для целей этого учебника используйте эти значения: **VNet2**: адресное пространство — 10.2.0.0/16; регион — Япония, восток.

## Добавление локальных сетей

При создании конфигурации подключения между двумя виртуальными сетями необходимо настроить каждую из сетей так, чтобы они идентифицировали друг друга в качестве сайта локальной сети. В этой процедуре вы настроите каждую виртуальную сеть в качестве локальной. Если вы уже настроили виртуальные сети, вот как их можно добавить в качестве локальных сетей на портале управления.
...
5. Теперь вы укажете каждой виртуальной сети друг на друга как на локальную сеть. На портале управления перейдите на страницу **Панель мониторинга** сети VNet1. В разделе **Подключение типа «сеть-сеть»** выберите ** 	Подключиться к локальной сети**, а затем в качестве локальной сети выберите **VNET2**.
...
[Схема настройки виртуальной сети Azure](https://msdn.microsoft.com/library/azure/jj157100.aspx)

[Операции с виртуальными сетями](https://msdn.microsoft.com/library/azure/jj157182.aspx)

[Операции со шлюзами виртуальных сетей](https://msdn.microsoft.com/library/azure/jj154113.aspx)

**Другие ресурсы**

[Запись блога: «Подключения между виртуальными сетями в разных регионах»](http://azure.microsoft.com/blog/2014/06/17/vnet-to-vnet-connecting-virtual-networks-in-azure-across-different-regions/)
