...
- Если необходимо заблокировать доступ к ресурсу, можно воспользоваться блокировкой управления. См. [Блокировка ресурсов с помощью диспетчера ресурсов Azure](resource-group-lock-resources.md)
- Чтобы настроить маршрутизацию и перенаправление IP, см. статью [Создание маршрутов и включение перенаправления IP в Azure](virtual-network/virtual-networks-udr-how-to.md) 
- Обзор управления доступом на основе ролей см. в статье [Управление доступом на основе ролей на портале Microsoft Azure](role-based-access-control-configure.md)
