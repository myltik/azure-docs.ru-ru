...
## Обзор
Сеть доставки содержимого (CDN) Microsoft Azure предлагает разработчикам глобальное решение по доставке содержимого, размещенного в Azure, по сети с высокой пропускной способностью. Используя CDN, вы можете кэшировать общедоступные объекты, загруженные из хранилища BLOB-объектов, например веб-приложение, виртуальную машину или папку приложения. Кэш CDN может храниться в стратегически важных местах для обеспечения максимальной пропускной способности при доставке содержимого пользователям. CDN обычно используется для доставки статического содержимого, например изображений, таблиц стилей, документов, файлов, клиентских скриптов и HTML-страниц.
...
Если Azure CDN не соответствует вашим требованиям, вы сможете использовать другие сети доставки содержимого, которые не реализованы Azure в ваших приложениях. Кроме того, вы сможете использовать Azure CDN для приложений, размещенных у других поставщиков, предоставляя статическое содержимое в службе хранилища Azure или вычислительных экземплярах Azure.
...
|| Время (мс) до получения первого байта (источник) | Время (мс) до получения первого байта (CDN) | Оптимизация CDN (%)| |---|---|---|---| |* Сан-Хосе (Калифорния)| 47,5 | 46,5 | 2 % | |** Даллес (Вашингтон)| 109 | 40,5 | 169 % ||Буэнос-Айрес (Аргентина)| 210 | 151 | 39 % | |* Лондон (СК)| 195 | 44 | 343 % | | Шанхай (КНР)| 242 | 206 | 17 % | |* Сингапур | 214 | 74 | 189 % | |* Токио (Япония) | 163 | 48 | 204 % || Сеул (РК)| 190 | 190 | 0 % |
...
+ **Управление версиями и кэшем**. Вам нужно выбрать способ обновления статического содержимого и развертывания новых версий. Когда новые версии становятся доступными, содержимое CDN можно очистить в диспетчере профилей CDN, расположенном на узле портала Azure. Это задача, аналогичная задаче управления кэшированием на стороне клиента, например в браузере.
...
+ **Устойчивость**. CDN — это потенциальная единая точка отказа для приложения. Она имеет соглашение об уровне обслуживания с более низкой доступностью по сравнению с хранилищем больших двоичных объектов (которое можно использовать для доставки содержимого напрямую), поэтому вам может потребоваться реализовать механизм отработки отказа для критически важного содержимого.
  ...
  С помощью диспетчера профиля CDN, расположенного на узле портала Azure, вы можете отслеживать доступность содержимого CDN, пропускную способность, передаваемые данные, коэффициент попаданий в кэш, а также метрики кэша.
...
Для развертывания содержимого в CDN необходимо просто указать конечную точку HTTP (порт 80), которую служба CDN будет использовать для доступа к содержимому и его кэширования.
...
Сеть Azure CDN позволяет указывать пользовательское доменное имя, а также использовать его для доступа к ресурсам через CDN. Вы можете также настроить пользовательское поддоменное имя с помощью записи *CNAME* в DNS. Этот подход обеспечит дополнительный уровень абстракции и контроля.
...
Выполните анализ трафика CDN для схем использования. Портал Azure предоставляет средства, которые позволяют выполнять мониторинг пропускной способности, переданных данных, попаданий (коды состояния), состояний кэша, коэффициента попаданий в кэш, а также соотношения запросов IPV4 и IPV6.

Дополнительные сведения см. в статье [Анализ схем использования Azure CDN](cdn/cdn-analyze-usage-patterns.md/).

### Потенциальные затраты

С вас взимается плата за передачу исходящих данных из CDN и за транзакции с хранилищем, когда CDN загружает данные из приложения. Следует задать реалистичные сроки действия для кэширования содержимого, но не настолько короткие, чтобы загрузка содержимого из приложения или хранилища больших двоичных объектов в CDN повторялась слишком часто. Однако длительные периоды TTL усложняют удаление элементов из CDN, так как вы должны дождаться истечения их срока действия.
...
+ [Рекомендации по работе с сетями доставки содержимого Microsoft Azure](http://azure.microsoft.com/blog/2011/03/18/best-practices-for-the-windows-azure-content-delivery-network/)
