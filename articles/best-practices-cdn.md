...
|| Время (мс) до получения первого байта (источник) | Время (мс) до получения первого байта (CDN) | Оптимизация CDN (%)|
|-------------|------------------------|--------------------|------------------|
|\* Сан-Хосе (Калифорния)| 47,5 | 46,5 | 2 % |
|\*\* Даллес (Вашингтон)| 109 | 40,5 | 169 % |
|Буэнос-Айрес (Аргентина)| 210 | 151 | 39 % |
|\* Лондон (СК)| 195 | 44 | 343 % |
| Шанхай (КНР)| 242 | 206 | 17 % |
|\* Сингапур | 214 | 74 | 189 % |
...
Выполните анализ трафика CDN для схем использования. Портал Azure предоставляет средства, которые позволяют выполнять мониторинг
+ пропускной способности,
+ переданных данных,
+ попаданий (коды состояния),
+ состояний кэша,
+ коэффициента попаданий в кэш,а также
+ соотношения запросов IPV4 и IPV6.
