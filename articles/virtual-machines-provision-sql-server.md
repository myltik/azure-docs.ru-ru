<properties pageTitle="Подготовка виртуальной машины SQL Server в Azure" description="В этом учебнике показано, как создать и настроить виртуальную машину SQL Server в Azure." services="virtual-machines" documentationCenter="" authors="Selcin" manager="jhubbard" editor="monicar"/>
...
3. На странице **Создание виртуальной машины** выберите образ виртуальной машины, содержащей SQL Server, и щелкните стрелку "Далее" в правом нижнем углу страницы. Самая последняя информация о поддерживаемых образах SQL Server на Azure доступна в разделе [Приступая к работе с SQL Server в среде виртуальной машины Azure](http://go.microsoft.com/fwlink/p/?LinkId=294720) набора документации [SQL Server в виртуальных машинах Azure](http://go.microsoft.com/fwlink/p/?LinkId=294719). 
    ...
    > - вы можете обновить существующий экземпляр пробной версии SQL Server до другой версии SQL Server в рамках соглашения [Перемещение лицензий в рамках программы Software Assurance в Azure](http://www.windowsazure.com/ru-ru/pricing/license-mobility/), следуя процедуре, описанной в разделе [Обновление до другой версии SQL Server 2014](http://go.microsoft.com/fwlink/?LinkId=396915). Информацию о том, как приобрести лицензионную копию SQL Server, см. в статье [Приобретение SQL Server](http://www.microsoft.com/ru-ru/sqlserver/get-sql-server/how-to-buy.aspx).
...
1. Подключитесь к виртуальной машине через удаленный рабочий стол Windows. После входа откройте меню "Пуск", щелкните **Выполнить**, введите **WF.msc** и нажмите кнопку **OK**. В Windows Server 2012 или более поздней версии вы можете также ввести **WF** в меню "Пуск", а затем выбрать **Брандмауэр Windows в режиме повышенной безопасности**.
...
При необходимости откройте дополнительные порты для других компонентов. Дополнительная информация содержится в разделе [Настройка брандмауэра Windows для разрешения доступа к SQL Server](http://msdn.microsoft.com/ru-ru/library/cc646023.aspx).


###<a id="TCP">Настройка SQL Server для прослушивания через протокол TCP</a>

1. Во время подключения к виртуальной машине с помощью удаленного рабочего стола в меню "Пуск" щелкните **Все программы**, **Microsoft SQL Server** *version*, нажмите **Средства настройки**, а затем щелкните **Диспетчер конфигурации SQL Server**.
...
6. В области сведений щелкните правой кнопкой мыши **SSQL Server (_имя экземпляра_)** (экземпляр по умолчанию - **SQL Server (MSSQLSERVER)**) и выберите **Перезагрузить**, чтобы остановить и перезапустить экземпляр SQL Server. 
...
1. Во время подключения к виртуальной машине с помощью удаленного рабочего стола в меню "Пуск" щелкните **Все программы**, **Microsoft SQL Server _версия_**, а затем щелкните **Диспетчер конфигурации SQL Server**. 
...
11. При первом создании учетной записи может потребоваться ее назначение в качестве администратора SQL Server. В таком случае на странице **Роли сервера** выберите **sysadmin**. 
	...
	**Примечание о безопасности.** Представители фиксированной роли сервера sysadmin получают полный контроль над компонентом Database Engine. Вам следует с осторожностью ограничить участие в этой роли.

	![sysadmin][Image25]

12. Щелкните ОК.

Дополнительную информацию об именах для входа SQL Server см. в разделе [Создание учетной записи](http://msdn.microsoft.com/ru-ru/library/aa337562.aspx).
...
Дополнительную информацию см. в статье [Как устранять неполадки при подключении к ядру СУБД SQL Server](http://social.technet.microsoft.com/wiki/contents/articles/how-to-troubleshoot-connecting-to-the-sql-server-database-engine.aspx).
...
[Изображение13]: ./media/virtual-machines-provision-sql-server/13New-FW-Rule.png
