...
1. Используя свою учетную запись, войдите на [портал управления Azure](http://manage.windowsazure.com). Если у вас нет учетной записи Azure, посетите веб-страницу [Бесплатный ознакомительный период в один месяц](http://www.windowsazure.com/pricing/free-trial/).
...
![Выбор образа][Образ34]


Самую последнюю информацию о поддерживаемых в Azure образах SQL Server см. в статье [Приступая к работе с SQL Server в среде виртуальной машины Azure](http://go.microsoft.com/fwlink/p/?LinkId=294720) набора документации [SQL Server в виртуальных машинах Azure](http://go.microsoft.com/fwlink/p/?LinkId=294719). 
...
Вы можете создать виртуальную машину с помощью выпуска SQL Server с поминутной оплатой (из коллекции) и перенести файлы базы данных в эту новую виртуальную машину. Для этого нужно следовать инструкциям, описанным в статье [Миграция файлов баз данных и схемы SQL Server между виртуальными машинами в Azure с помощью дисков с данными](http://go.microsoft.com/fwlink/p/?LinkId=294738). **Или** вы можете обновить существующий экземпляр ознакомительного выпуска SQL Server до другого выпуска согласно соглашению [Перемещение лицензий в рамках программы Software Assurance в Azure](http://www.windowsazure.com/pricing/license-mobility/). Для этого выполните инструкции, описанные в статье [Обновление до другого выпуска SQL Server 2014](http://go.microsoft.com/fwlink/?LinkId=396915). Сведения о том, как приобрести лицензионную копию SQL Server, см. в статье [Как купить SQL Server](http://www.microsoft.com/sqlserver/get-sql-server/how-to-buy.aspx).
...
6. В диалоговом окне **Действие** выберите **Разрешить подключение** и нажмите кнопку **Далее**.
	...
	**Примечание по безопасности.** Если выбрать вариант **Разрешить безопасное подключение**, это может обеспечить дополнительную безопасность. Выберите этот параметр, если вам нужно настроить дополнительные параметры безопасности для своей среды.

	![Allow Connections][Image15]

7. В диалоговом окне **Профиль** выберите **Открытый** и нажмите кнопку **Далее**. 

    **Примечание по безопасности.** Выбрав вариант **Открытый**, вы получите доступ к профилю через Интернет. Всегда, когда возможно, выбирайте профиль с более жесткими ограничениями.
...
При необходимости откройте дополнительные порты для других компонентов. Дополнительные сведения см. статье [Настройка брандмауэра Windows для разрешения доступа к SQL Server](http://msdn.microsoft.com/library/cc646023.aspx).
...
Дополнительные сведения о включении протоколов для компонента SQL Server Database Engine см. в статье [Включение или отключение сетевого протокола сервера](http://msdn.microsoft.com/library/ms191294.aspx).

###<a id="Mixed">Настройка SQL Server для аутентификации в смешанном режиме</a>

Компонент SQL Server Database Engine не может использовать проверку подлинности Windows без среды домена. Чтобы подключиться к компоненту Database Engine с другого компьютера, необходимо настроить для SQL Server смешанный режим проверки подлинности. Смешанный режим позволяет выполнять проверку подлинности SQL Server и проверку подлинности Windows. (Настройка смешанного режима аутентификации может потребоваться, если была настроена виртуальная сеть Azure.) Дополнительную информацию см. в статье [Рекомендации по подключению SQL Server в среде виртуальной машины Azure](http://go.microsoft.com/fwlink/?LinkId=294723) в наборе документации [SQL Server в виртуальных машинах Azure](http://go.microsoft.com/fwlink/?LinkId=294719).
...
2. Management Studio отображает диалоговое окно **Подключение к серверу**. В поле **Имя сервера** введите имя виртуальной машины для подключения к ядру СУБД с помощью обозревателя объектов. (Вместо имени виртуальной машины вы можете использовать также **(local)** или одну точку в качестве **имени сервера**.) Выберите **Проверка подлинности Windows** и оставьте **_your_VM_name_\your_local_administrator** в поле **Имя пользователя**. Щелкните **Подключить**.
...
11. При первом создании учетной записи может потребоваться ее назначение в качестве администратора SQL Server. В таком случае на странице **Роли сервера** выберите **sysadmin**. 
	...
	**Примечание по безопасности.**Участники фиксированной роли сервера sysadmin получают полный контроль над ядром СУБД. Вам следует с осторожностью ограничить участие в этой роли.

	![sysadmin][Image25]

12. Нажмите кнопку "ОК".

Дополнительные сведения об именах входа SQL Server см. в разделе [Создание имени входа](http://msdn.microsoft.com/library/aa337562.aspx).
...
Дополнительные сведения см. в статье [Устранение неполадок при подключении к ядру СУБД SQL Server](http://social.technet.microsoft.com/wiki/contents/articles/how-to-troubleshoot-connecting-to-the-sql-server-database-engine.aspx)(на английском).

##<a id="Optional">Дальнейшие действия</a>
Теперь вы знаете, как создавать и настраивать SQL Server на виртуальной машине Azure с помощью образа платформы. При использовании SQL Server в виртуальных машинах Azure мы советуем соблюдать процедуру, описанную в подробном руководстве [SQL Server в виртуальных машинах Azure](http://go.microsoft.com/fwlink/p/?LinkId=294719), содержащемся в библиотеке документов. Этот набор документации включает в себя ряд статей и учебников, в которых содержатся подробные инструкции. Серия включает в себя следующие разделы:
