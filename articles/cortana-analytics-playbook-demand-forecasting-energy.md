<properties
	pageTitle="Сборник шаблонов решений Cortana Analytics для прогнозирования энергопотребления | Microsoft Azure"
...
>[AZURE.TIP] Чтобы скачать схему, на которой представлен обзор архитектуры этого шаблона, см. раздел [Cortana Analytics Solution Template architecture for demand forecasting of energy](cortana-analytics-architecture-demand-forecasting-energy.md) (Архитектура шаблона решения Cortana Analytics для прогнозирования спроса на энергию).

## Обзор  

В этом документе рассматриваются коммерческие, информационные и технические аспекты использования Cortana Analytics и в особенности системы машинного обучения Azure (AML) для внедрения и развертывания решений по прогнозированию энергопотребления. Документ состоит из трех основных частей.
...
-   Быстрое обучение работе с продуктом Cortana Analytics Suite.
...
### Прогнозирование электрической нагрузки и спроса
...
### Сравнительная характеристика краткосрочного и долгосрочного прогнозирования
...
|---|---|---|
|Период прогноза|От 1 дня до 48 часов|От 1 недели до 6 месяцев или более|
|Степень детализации данных|Ежечасно|Ежечасно или ежедневно|
|Стандартные сценарии использования|<ul><li>Балансировка спроса (предложения)</li><li>Выбор почасового прогнозирования</li><li>Регулирование спроса</li></ul>|<ul><li>Долгосрочное планирование</li><li>Планирование средств сети</li><li>Планирование ресурсов</li></ul>|
|Стандартные прогностические факторы|<ul><li>День или неделя</li><li>Час дня</li><li>Почасовая температура</li></ul>|<ul><li>Месяц года</li><li>День месяца</li><li>Климат и температура на протяжении длительного периода</li></ul>|
|Диапазон исторических данных|Данные за период от 2 до 3 лет|Данные за период от 5 до 10 лет|
|Стандартная точность|MAPE* — 95 % и более|MAPE* — 75 % и более|
|Частота прогнозирования|Создается каждый час или каждые 24 часа|Создается еженедельно, ежемесячно или ежеквартально|
...
### Пример сценария использования 1. Системы eSmart — оптимизация при перегрузке
...
Cortana Analytics и машинное обучение могут быть весьма эффективны в решении данной бизнес-задачи при выполнении указанных ниже условий.
...
-   У клиента есть готовые к использованию **данные достаточного качества** для поддержки сценария использования (подробнее см. в следующем разделе этого сборника, **Качество данных**).
-   Клиент располагает ориентированной на облачные службы архитектурой данных или **средствами машинного обучения на основе облачных служб**, включая машинное обучение Azure и другие компоненты Cortana Analytics Suite.
-   Клиент должен установить **сквозной поток данных**, который обеспечивает средства доставки данных в облако на постоянной основе, и **ввести решение в эксплуатацию**.
...
Наконец, нет необходимости инвестировать средства в разработку будущей инфраструктуры или ИТ-поддержки, так как все это является частью облачных услуг. Пакет Cortana Analytics Suite включает службы высочайшего в своем классе уровня, и их развитие непрерывно продолжается. Постоянно добавляются и разрабатываются новые функции, компоненты и возможности.
...
Поскольку Cortana Analytics Suite предоставляет возможность оплаты по мере использования, нет необходимости вводить в эту формулу компонент фиксированных затрат. Формула может вычисляться ежедневно, ежемесячно или ежегодно.

Текущие тарифные планы Cortana Analytics Suite и машинного обучения Azure можно найти [здесь](http://azure.microsoft.com/pricing/details/machine-learning/).

### Процесс разработки решения
Цикл разработки решения по прогнозированию спроса на электроэнергию обычно включает четыре этапа. На каждом из них будут использоваться облачные технологии и службы Cortana Analytics Suite.
...
### Формат данных
Cortana Analytics Suite поддерживает наиболее распространенные форматы данных, такие как CSV, TSV, JSON *и т. д.* Очень важно, чтобы формат данных был согласованным на протяжении всего жизненного цикла проекта.
...
Чтобы добиться этого, Cortana Analytics Suite предлагает различные способы поддержки процесса приема данных. Этот вопрос будет подробнее рассматриваться в разделе **Развертывание** данного документа.
...
**Зависимая функция**. Это столбец данных, который необходимо использовать в модели для прогнозирования. Прежде всего, при помощи [контролируемого машинного обучения](https://en.wikipedia.org/wiki/Supervised_learning) необходимо обучить модель, используя зависимые функции (которые также называются метками). Это позволяет модели изучить закономерности в данных, связанных с зависимыми функциями. В прогнозе спроса на энергию обычно требуется определить будущий фактический спрос. Поэтому он используется в качестве зависимой функции.
...
### Методы моделирования для прогнозирования спроса
...
#### [**ETS (экспоненциальное сглаживание)**](https://www.otexts.org/fpp/7/5)
Экспоненциальное сглаживание (ETS) — это семейство различных методов, использующих взвешенное среднее последних точек данных для прогнозирования следующей точки. Идея заключается в том, чтобы назначить более высокие весовые коэффициенты новым значениям и постепенно уменьшить этот весовой коэффициент для более ранних измерений. Существует несколько различных методов этого семейства. Некоторые из них включают обработку сезонных колебаний данных, например [сезонный метод Хольта — Уинтерса](https://www.otexts.org/fpp/7/5).

В некоторых из этих методов также учитываются сезонные колебания данных.

#### [**ARIMA (интегрированное скользящее среднее с авторегрессией)**](https://www.otexts.org/fpp/8)
...
![Формула MAPE](media/cortana-analytics-playbook-demand-forecasting-energy/mape-formula.png) *Где A<sub>t</sub> — фактическое значение; F<sub>t</sub> — прогнозируемое значение, а n — период прогноза.*
...
Как можно увидеть, веб-служба развертывается в облаке Cortana Analytics Suite. Ее можно вызывать через предоставляемую в облаке конечную точку REST API. Клиенты разных типов в различных доменах могут одновременно вызывать службу через веб-API. Кроме того, веб-службы можно масштабировать для поддержки тысяч одновременных вызовов.

### Стандартная архитектура решения
...
### Сквозная архитектура развертывания
Чтобы на практике развернуть решение по прогнозированию спроса на энергию в Cortana Analytics, необходимо обеспечить внедрение и надлежащую настройку всех требуемых компонентов.

На схеме ниже представлена стандартная архитектура на основе Cortana Analytics, в которой реализуется цикл потока данных, описанных выше, и выполняется его оркестрация:
