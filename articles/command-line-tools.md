<properties urlDisplayName="Command-Line Tools" pageTitle="Средства командной строки Azure для Mac и Linux" metaKeywords="Azure command-line, Azure tools Mac, Azure tools Linux" description="Learn about using the command-line tool for Mac and Linux in Azure." metaCanonical="" services="web-sites,virtual-machines,mobile-services,cloud-services" documentationCenter="" title="" authors="carolz" solutions="" manager="need to identify contact" editor="" />
...
Эта команда удаляет сохраненные параметры публикации, которые были импортированы. Используйте эту команду, если вы закончили работу с инструментом на этом компьютере и хотите быть уверены, что инструмент нельзя будет использовать с вашей учетной записью в дальнейшем.
...
**create-new** создает диск в хранилище больших двоичных объектов (e:\ на схеме); **attach** подключает к виртуальной машине уже созданный, но еще не подключенный диск.
...
Команда vm create, как и портал Azure, создает виртуальные машины в рабочей среде развертывания. Нет возможности создавать виртуальную машину в промежуточной среде развертывания облачной службы. Если у вашей подписки нет учетной записи хранения Azure, эта команда создаст ее.
...
В этом примере MSFT__Win2K8R2SP1-120514-1520-141205-01-ru-ru-30GB - образ, предоставленный платформой. Дополнительную информацию об образах операционных систем см. в описании команды vm image list.
...
Эта команда удаляет виртуальную машину Azure. По умолчанию эта команда не удаляет большой двоичный объект Azure, из которого созданы диск операционной системы и диск с данными. Для удаления большого двоичного объекта вместе с соответствующей виртуальной машиной используйте параметр -b.
...
Команды для подключения дисков данных (azure vm disk attach и azure vm disk attach-new) назначают логический номер устройства (LUN) подключаемому диску данных в соответствии с требованиями протокола SCSI. Первому подключенному к виртуальной машине диску данных назначается номер LUN 0, следующему - LUN 1 и т. д.
...
Эта команда выводит список дисков Azure или дисков, подключенных к указанной виртуальной машине. Если в команде в качестве параметра указывается имя виртуальной машины, она возвращает все диски, присоединенные к ней. LUN 1 создан вместе с виртуальной машиной, а все остальные перечисленные диски подключены отдельно.
...
Эта команда удаляет диск Azure из личного репозитория. Перед удалением диск необходимо отключить от виртуальной машины.
...
Эта команда передает и регистрирует диск Azure. Необходимо указать --blob-url, --location или --affinity-group. Если в этой команде используется параметр [путь-к-источнику], передается указанный VHD-файл и создается новый образ. Затем этот образ можно подключить к виртуальной машине, используя команду vm disk attach.
...
Эта команда подключает диск данных к виртуальной машине Azure. В этом примере 20 - это размер (в гигабайтах) нового диска, который нужно присоединить. В качестве последнего аргумента при желании можно использовать URL-адрес BLOB-объекта, чтобы явно указать целевой BLOB-объект для создания. Если URL-адрес большого двоичного объекта не указан, он будет создан автоматически.
...
Эта команда отключает диск данных, подключенный к виртуальной машине Windows Azure. Параметр <<lun> указывает диск для отключения. Чтобы перед отсоединением получить список дисков, связанных с данным диском, используйте команду vm disk-list <<имя-виртуальной-машины>.
...
**-q или **--quiet**: не запрашивает подтверждение. Используйте этот параметр в автоматизированных сценариях.
...
**site job upload [[параметры]] <<имя-задания> <<тип-задания> [<имя>]**
...
+ **-s `<идентификатор>`** или **--subscription `<идентификатор>`**: использует конкретную подписку, заданную значением `<идентификатор>`.
...
+ **-d `<sqlDb>`** или **--sqlDb `<sqlDb>`**: использует существующую базу данных SQL, заданную значением `<sqlDb>`.
...
+ **--sqlLocation `<расположение>`**: создает сервер SQL в конкретном `<расположение>`; по умолчанию используется расположение мобильной службы.
...
+ **-t `<тип>`** или **--type `<тип>`**:  отфильтровать возвращаемые журналы по записи `<тип>`, который может иметь значение `information`, `warning` или `error`.
...
Основные типы данных: `главный` и `приложения`.
...
**mobile key set [[параметры]] [<имя-службы>] [<ключ>] [<значение>]**
...
+ **-p `<<разрешения>`** или **--permissions `<<разрешения>`**: Разделенный запятыми список пар `<операция>`=`<разрешения>`, где `<операция>` - это `insert`, `read`, `update` или `delete`, а `<<разрешения>` - это `public`, `application` (по умолчанию), `user` или `admin`.
...
+ **-l** или **--list**: возвращает данные в виде списка.
...
+ **-p `<<разрешения>`** или **--permissions `<<разрешения>`**: Разделенный запятыми список пар `<операция>`=`<разрешения>`, где `<операция>` - это `insert`, `read`, `update` или `delete`, а `<<разрешения>` - это `public`, `application` (по умолчанию), `user` или `admin`.
+ **--deleteColumn `<столбцы>`**: разделенный запятыми список столбцов для удаления, заданных значением `<столбцы>`.
+ **-q** или **--quiet**: удаляет столбцы без запроса подтверждения.
+ **--addIndex `<столбцы>`**: разделенный запятыми список столбцов, включаемых в индекс.
+ **--deleteIndex `<столбцы>`**: разделенный запятыми список столбцов, исключаемых из индекса.
...
Укажите параметр -q, чтобы удалить таблицу без подтверждения. Это необходимо, чтобы не блокировать сценарии автоматизации.
...
Эта команда передает новый сценарий `todoitem.insert.js` из подкаталога `table`.
...
+ **-u `<единица>`** или **--intervalUnit `<единица>`**: единица измерения для _interval_, может принимать следующие значения: 
	+ **minute** (по умолчанию)
	+ **hour**
	+ **day**
	+ **month** 
	+ **none** (задания по запросу)
+ **-t `<время>`** **--startTime `<время>`** Время начала первого выполнения сценария в формате ISO; по умолчанию используется значение `now`.
...
+ **-i `<число>`** или **--numberOfInstances `<число>`**: число экземпляров, используемых при работе в режиме "Reserved".

<div class="dev-callout"><b>Примечание.</b>
   <p>Если для режима среды выполнения задано значение `Reserved`, все ваши мобильные службы в этом регионе будут работать в расширенном режиме.</p>
</div>  
...
**-p** или **--permissions** <<разрешения>:  разделенный запятыми список <<метод>=<<разрешение>.
...
**storage blob show [[параметры]] [<контейнер>] [<большой-двоичный-объект>]**
...
**storage blob delete [[параметры]] [<контейнер>] [<большой-двоичный-объект>]**

Эта команда поддерживает следующие дополнительные параметры:

+ **--container** <<контейнер>: имя контейнера службы хранилища, который необходимо создать.
+ **-b** или **--blob** <<имя-большого-двоичного-объекта>: имя большого двоичного объекта службы хранилища, который необходимо удалить.
...
**storage blob upload [[параметры]] [<файл>] [<контейнер>] [<большой-двоичный-объект>]**
...
+ **--container** <<контейнер>: имя контейнера службы хранилища, который необходимо создать.
+ **-b** или **--blob** <<имя-большого-двоичного-объекта>: имя большого двоичного объекта службы хранилища, который необходимо передать.
+ **-t** or **--blobtype** <<имя-большого-двоичного-объекта>: тип большого двоичного объекта службы хранилища: страничный (Page) или блочный (Block).
...
**storage blob download [[параметры]] [<контейнер>] [<большой-двоичный-объект>] [<назначение>]**
...
+ **--container** <<контейнер>: имя контейнера службы хранилища, который необходимо создать.
+ **-b** или **--blob** <<имя-большого-двоичного-объекта>: имя большого двоичного объекта службы хранилища.
+ **-d** или **--destination** [<назначение>]:  - целевой путь файла или каталога назначения для скачивания.
+ **-m** или **--checkmd5**: проверка контрольной суммы md5sum для скачанного файла.
+ **--concurrenttaskcount** <<число-одновременных-задач>   - максимальное число одновременных запросов на передачу.
...
Для расширенной настройки сети можно экспортировать конфигурацию сети локально. Обратите внимание, что экспортированная конфигурация сети включает параметры DNS-сервера и виртуальной сети, локальные параметры сетевого сайта и другие параметры.
