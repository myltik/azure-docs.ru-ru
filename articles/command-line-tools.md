...
Эта программа предоставляет функциональные возможности для создания, развертывания и использования виртуальных машин, веб-сайтов и мобильных служб Azure с настольных компьютеров Mac и Linux. Эти функциональные возможности аналогичны командлетам Windows PowerShell, которые устанавливаются вместе с пакетами SDK для Azure для .NET, Node.JS и PHP.

Чтобы установить программу в среде Mac, скачайте и запустите [установщик пакета SDK для Azure][установщик пакета SDK для Azure].
...
-   [Управление сведениями об учетной записи и параметрами публикации][Управление сведениями об учетной записи и параметрами публикации]
-   [Команды для управления виртуальными машинами Azure][Команды для управления виртуальными машинами Azure]
-   [Команды для управления конечными точками виртуальных машин Azure][Команды для управления конечными точками виртуальных машин Azure]
-   [Команды для управления образами виртуальных машин Azure][Команды для управления образами виртуальных машин Azure]
-   [Команды для управления дисками данных виртуальных машин Azure][Команды для управления дисками данных виртуальных машин Azure]
-   [Команды для управления облачными службами Azure][Команды для управления облачными службами Azure]
-   [Команды для управления сертификатами Azure][Команды для управления сертификатами Azure]
-   [Команды для управления веб-сайтами][Команды для управления веб-сайтами]
-   [Команды для управления мобильными службами Azure][Команды для управления мобильными службами Azure]
-   [Управление локальными параметрами средства][Управление локальными параметрами средства]
-   [Команды для управления службой Service Bus][Команды для управления службой Service Bus]
-   [Команды для управления объектами службы хранилища][Команды для управления объектами службы хранилища]
-   [Команды для управления базами данных SQL][Команды для управления базами данных SQL]
-   [Команды для управления виртуальными сетями][Команды для управления виртуальными сетями]
...
<div class="dev-callout"><b>Примечание.</b>
   <p>PUBLISHSETTINGS-файл может содержать данные (например, имя и идентификатор подписки) о нескольких подписках. При импорте PUBLISHSETTINGS-файла первая подписка используется в качестве описания по умолчанию. Чтобы использовать другую подписку, выполните следующую команду.</p>
...
![Техническая схема Azure][Техническая схема Azure]
...
**-c, --connect** создает виртуальную машину в уже созданном развертывании в службе размещения. Если с этим параметром не используется параметр -vmname, имя новой виртуальной машины создается автоматически.

**-n, --vm-name** задает имя виртуальной машины. Этот параметр по умолчанию берет имя службы размещения. Если параметр -vmname не указан, для новой виртуальной машины создается имя в следующем формате: \<имя-службы\>\<идентификатор\>, где \<идентификатор\> — количество существующих виртуальных машин в службе плюс 1. Например, если эта команда используется для добавления новой виртуальной машины в службу размещения MyService, в которой имеется одна виртуальная машина, то новая виртуальная машина получает имя MyService2.

**-u, --blob-url** задает URL-адрес хранилища больших двоичных объектов, из которого создается системный диск виртуальной машины.

**-z, --vm-size** задает размер виртуальной машины. Допустимые значения: "очень малый", "малый", "средний", "крупный", "очень крупный". Значение по умолчанию: "малый".

**-r** добавляет RDP-подключение к виртуальной машине Windows.

**-e, --ssh** добавляет SSH-подключение к виртуальной машине Windows.

**-t, --ssh-cert** задает сертификат SSH.

**-s** подписка.

**-o, --community** указанный образ является образом сообщества.

**-w** имя виртуальной сети.

**-l, --location** задает расположение (например, «Северо-центральный регион США»).

**-a, --affinity-group** задает территориальную группу.

**-w, --virtual-network-name** задает виртуальную сеть, в которой нужно добавить новую виртуальную машину. Можно настраивать виртуальные сети и управлять ими на портале Azure.
...
**vm create-from \<dns-имя\> \<файл-роли\>**
...
Эта команда отображает информацию о виртуальной машине Azure. Параметр -json указывает, что результаты возвращаются в необработанном JSON-формате.
...
Образ виртуальной машины можно записать, только когда она находится в состоянии **Остановлено**.
...
**vm endpoint create-multiple [параметры] \<имя-виртуальной-машины\> \<порт-балансировки-нагрузки\>[:\<порт-виртуальной-машины\>[:\<протокол\>[:\<имя-набора-балансировки-нагрузки\>[:\<протокол-зонда\>:\<порт-зонда-балансировки-нагрузки\>[:\<путь-к-зонду\>]]]]] ]{1-\*}**
...
Эта команда возвращает список образов виртуальных машин. Существует три типа образов: образы, созданные корпорацией Майкрософт и имеющие префикс MSFT, образы, созданные сторонними поставщиками и имеющие в качестве префикса имя поставщика, и образы, которые создаете вы. Можно записать образ существующей виртуальной машины или создать образ из пользовательского VHD-файла, переданного в хранилище BLOB-объектов. Дополнительную информацию об использовании пользовательских VHD-файлов см. в описании команды vm image create.
...
Диски данных — это VHD-файлы в хранилище BLOB-объектов, которые может использовать виртуальная машина. Дополнительную информацию о том, как диски данных развертываются в хранилище больших двоичных объектов, см. на приведенной выше технической схеме Azure.
...
Эта команда выводит список дисков Azure или дисков, присоединенных к указанной виртуальной машине. Если в команде в качестве параметра указывается имя виртуальной машины, она возвращает все диски, присоединенные к ней. LUN 1 создан вместе с виртуальной машиной, а все остальные перечисленные диски присоединены отдельно.
...
**vm disk upload [параметры] \<путь-к-источнику\> \<url-адрес-blob-объекта\> \<ключ-учетной-записи-хранения\>**
...
**vm disk attach-new \<имя-виртуальной-машины\> \<размер-в-гб\> [url-адрес-blob-объекта]**
...
Эта команда отсоединяет диск данных, присоединенный к виртуальной машине Azure. \<lun\> указывает диск для отсоединения. Чтобы перед отсоединением получить список дисков, связанных с данным диском, используйте команду vm disk-list \<имя-виртуальной-машины\>.
...
Сертификаты служб Azure — это SSL-сертификаты, подключенные к вашей учетной записи Azure. Дополнительную информацию о сертификатах Azure см. в разделе [Управление сертификатами][Управление сертификатами].
...
Веб-сайт Azure — это веб-конфигурация, доступ к которой осуществляется с помощью универсального кода ресурса (URI). Веб-сайты размещаются на виртуальных машинах, но вам нет необходимости задумываться о деталях создания и развертывания виртуальных машин. Их для вас обработает Azure.
...
**site deployment show [параметры] \<идентификатор-фиксации\> [имя]**

**site deployment redeploy [параметры] \<идентификатор-фиксации\> [имя]**
...
**site job upload [параметры] \<имя-задания\> \<тип-задания\> <jobfile> [имя]**
...
-   [Команды для управления экземплярами мобильных служб][Команды для управления экземплярами мобильных служб]
-   [Команды для управления конфигурацией мобильных служб][Команды для управления конфигурацией мобильных служб]
-   [Команды для управления таблицами мобильных служб][Команды для управления таблицами мобильных служб]
-   [Команды для управления сценариями мобильных служб][Команды для управления сценариями мобильных служб]
-   [Команды для управления запланированными заданиями][Команды для управления запланированными заданиями]
-   [Команды для масштабирования мобильной службы][Команды для масштабирования мобильной службы]
...
-   **-r `<sqlServer>`** или **--sqlServer `<sqlServer>`**: использует существующий сервер базы данных SQL, заданный как `<sqlServer>`.
-   **-d `<sqlDb>`** или **--sqlDb `<sqlDb>`**: использует существующую базу данных SQL, заданную как `<sqlDb>`.
-   **-l `<location>`** или **--location `<location>`**: создает службу в конкретном расположении, заданном как `<location>`. Для получения дополнительных расположений выполните команду azure mobile locations.
-   **--sqlLocation `<location>`**: создает сервер SQL в конкретном `<location>`; по умолчанию используется расположение мобильной службы.
...
Эта команда возвращает журналы мобильной службы, оставляя после фильтрации только журналы типа `error`.
...
-   **-r `<query>`** или **--query `<query>`**: выполняет указанный запрос журнала.
...
-   **-k `<skip>`** или **--skip `<skip>`**: пропускает определенное число строк, заданное значением `<skip>`.
-   **-p `<top>`** или **--top `<top>`**: возвращает конкретное число строк, заданных значением `<top>`.

<div class="dev-callout"><b>Примечание.</b>
   <p>Параметр <b>--query</b> имеет приоритет над параметрами <b>--type</b>, <b>--skip</b> и <b>--top</b>.</p>
</div>
...
-   **-p `&lt;permissions>`** или **--permissions `&lt;permissions>`**: разделенный запятыми список пар `<operation>`=`<permission>`, где `<operation>` принимает значение `insert`, `read`, `update` или `delete`, а `&lt;permissions>` — значение `public`, `application` (используется по умолчанию), `user` или `admin`.
...
-   **-k `<skip>`** или **--skip `<skip>`**: пропускает определенное число строк, заданное значением `<skip>`.
-   **-t `<top>`** или **--top `<top>`**: возвращает конкретное число строк, заданных значением `<top>`.
...
Описанные в этом разделе команды используются для управления серверными скриптами, относящимися к мобильной службе. Дополнительную информацию см. в разделе [Работа с серверными сценариями в мобильных службах][Работа с серверными сценариями в мобильных службах].
...
**mobile script download [параметры] [имя-службы] [имя-сценария]**
...
-   **-p `<path>`** или **--path `<path>`**: расположение в файле, в котором нужно сохранить сценарий; по умолчанию используется текущий рабочий каталог.
-   **-f `<file>`** или **--file `<file>`**: имя файла, в котором нужно сохранить сценарий.
-   **-o** или **--override**: перезаписывает существующий файл.
-   **-c** или **--console**: записывает сценарий в консоль, а не в файл.

**mobile script upload [параметры] [имя-службы] [имя-сценария]**
...
Имя файла должно состоять из имен таблицы и операции, и файл должен находиться во вложенной папке таблиц относительно расположения, где выполняется команда. Можно также с помощью параметра **-f `<file>`** или **--file `<file>`** указать другое имя файла и путь к файлу, содержащему регистрируемый сценарий.

**mobile script delete [параметры] [имя-службы] [имя-сценария]**
...
Описанные в этом разделе команды используются для управления запланированными заданиями, относящимися к мобильной службе. Дополнительные сведения см. в разделе [Расписание заданий][Расписание заданий].
...
-   **-u `<unit>`** или **--intervalUnit `<unit>`**: единица измерения для *interval*, которая может принимать одно из следующих значений:
...
<div class="dev-callout"><b>Примечание.</b>
   <p>Новые задания создаются в отключенном состоянии, так как необходимо еще передать сценарий. Используйте команду <strong>mobile script upload</strong> для передачи сценария и команду <strong>mobile job update</strong> для включения задания.</p>
</div>
...
-   **-u `<unit>`** или **--intervalUnit `<unit>`**: единица измерения для *interval*, которая может принимать одно из следующих значений:
    ...
    -   **none** (задания по запросу)
-   **-t `<time>`** **--startTime `<time>`**: время начала первого выполнения сценария в формате ISO; по умолчанию используется значение `now`.
-   **-a `<status>`** или **--status `<status>`**: состояние задания, которое может принимать значение `enabled` или `disabled`.
...
Описанные в этом разделе команды используются для масштабирования мобильной службы. Дополнительную информацию см. в разделе [Масштабирование мобильной службы][Масштабирование мобильной службы].
...
-   **-c `<mode>`** или **--computeMode `<mode>`**: для режима вычислений необходимо задать значение `Free` или `Reserved`.
...
-   **-a** или **--account-name** \<accountName\>: имя учетной записи хранения.
-   **-k** или **--account-key** \<accountKey\>: ключ учетной записи хранения.
-   **-c** или **--connection-string** \<connectionString\>: строка подключения к службе хранилища.
...
-   **--container** \<контейнер\>: имя контейнера службы хранилища, который необходимо создать.
-   **-b** или **--blob** \<blobName\>: имя большого двоичного объекта службы хранилища, который необходимо удалить.
...
-   **--container** \<контейнер\>: имя контейнера службы хранилища, который необходимо создать.
-   **-b** или **--blob** \<blobName\>: имя большого двоичного объекта службы хранилища, который необходимо передать.
-   **-t** или **--blobtype** \<blobtype\>: тип большого двоичного объекта службы хранилища: страничный или блочный.
...
-   **--container** \<контейнер\>: имя контейнера службы хранилища, который необходимо создать.
-   **-b** или **--blob** \<blobName\>: имя большого двоичного объекта службы хранилища.
...
Удаляет запись DNS-сервера из конфигурации сети.
  ...
  [установщик пакета SDK для Azure]: http://go.microsoft.com/fwlink/?LinkId=252249
  ...
  [Управление сертификатами]: http://msdn.microsoft.com/ru-ru/library/azure/gg981929.aspx
  ...
  [Работа с серверными сценариями в мобильных службах]: http://www.windowsazure.com/ru-ru/develop/mobile/how-to-guides/work-with-server-scripts/
  [Расписание заданий]: http://msdn.microsoft.com/ru-ru/library/windowsazure/jj860528.aspx
  [Масштабирование мобильной службы]: http://msdn.microsoft.com/ru-ru/library/windowsazure/jj193178.aspx
