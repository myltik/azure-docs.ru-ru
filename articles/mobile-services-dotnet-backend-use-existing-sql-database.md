...
Серверная часть мобильных служб .NET упрощает использование существующих ресурсов при создании мобильной службы. Один особенно интересный сценарий - использование существующей базы данных SQL (в локальной среде или в облаке), которая может использоваться другими приложениями, чтобы сделать существующие данные доступными для мобильных клиентов. В этом случае требуется, чтобы модель базы данных (или *schema*) не изменялась, иначе существующие решения перестанут работать.

Это учебное руководство содержит следующие разделы:

1. [Изучение модели существующей базы данных](#ExistingModel)
...
Для этого учебника используется база данных, созданная с помощью вашей мобильной службы, но мы не будем использовать модель по умолчанию. Мы вручную создадим произвольную модель, которая будет представлять существующее приложение. Полные сведения о подключении к локальной базе данных см. в разделе [Подключение к локальному SQL Server из мобильной службы Azure с помощью гибридных подключений](/ru-ru/documentation/articles/mobile-services-dotnet-backend-hybrid-connections-get-started/).

1. Сначала создайте серверный проект мобильных служб в **Visual Studio 2013 с обновлением 2** или с помощью проекта быстрого запуска, который можно загрузить на вкладке "Мобильные службы" вашей службы в [портале управления Azure](http://manage.windowsazure.com). В целях данного руководства мы исходим из того, что ваш серверный проект называется **ShoppingService**.
...
3. Создайте файл **Order.cs** в папке **Models** и используйте следующую реализацию.
    ...
    Можно заметить, что эти два класса имеют *relationship*: каждый **заказ** связан с одним **клиентом**, а одного **клиента** можно связать с несколькими **заказами**. Такие отношения широко распространены в существующих моделях данных.

4. Создайте файл **ExistingContext.cs** в папке **Models** и используйте следующую реализацию:
...
1. Создайте **MobileCustomer.cs** в папке **DataObjects** вашего проекта службы и используйте следующую реализацию:
...
2. Создайте файл **MobileOrder.cs** в папке **DataObjects** вашего проекта службы.
    ...
    Свойство отношения **Customer** было заменено именем **Customer** и свойством **MobileCustomerId**, которое может использоваться для ручного моделирования отношения на стороне клиента. Пока можно не обращать внимания на свойство **CustomerId**, оно будет использовано позже. 
...
Теперь следует реализовать [**MappedEntityDomainManager**](http://msdn.microsoft.com/library/dn643300.aspx), который служит уровнем абстракции между нашим хранилищем данных и контроллером, который будет обслуживать HTTP-трафик наших клиентов. Мы сможем написать контроллер в следующем разделе. При этом добавленные DTO и **MappedEntityDomainManager** осуществляются обработку обмена данными с исходным хранилищем данных, а мы получаем больше возможностей для реализации необходимой логики.
