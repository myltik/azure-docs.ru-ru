...
1.  [Изучение существующей модели базы данных][Изучение существующей модели базы данных]
2.  [Создание объектов передачи данных (DTO) для вашей мобильной службы][Создание объектов передачи данных (DTO) для вашей мобильной службы]
3.  [Установка сопоставления между DTO и моделью][Установка сопоставления между DTO и моделью]
4.  [Реализация логики для домена][Реализация логики для домена]
5.  [Реализация TableController с помощью DTO][Реализация TableController с помощью DTO]
...
1.  Создайте файл **MobileCustomer.cs** в папке **DataObjects** вашего проекта службы и используйте следующую реализацию:
    ...
    Обратите внимание, что этот класс похож на класс **Customer** в модели, за исключением того, что удалено свойство отношения с **Order**. Чтобы объект правильно работал с автономной синхронизацией мобильных служб, ему требуется набор *системных свойств* для оптимистической блокировки. DTO наследует от [**EntityData**][**EntityData**], содержащего эти свойства. Целочисленное свойство **CustomerId** исходной модели заменено строковым свойством **Id** из **EntityData**. Именно это свойство **Id** будет использоваться мобильными службами.
...
4.  Системные свойства только что добавили встроенное поведение (например, автоматическое обновление времени создания и обновления), прозрачное для работы базы данных. Чтобы включить это поведение, нужно внести изменения в **ExistingContext.cs**. В начале файла добавьте следующее:
    ...
    Затем в тексте **ExistingContext** переопределите [**OnModelCreating**][**OnModelCreating**]:
...
5.  Заполним базу данных примерами данных. Откройте файл **WebApiConfig.cs**. Создайте новый [**IDatabaseInitializer**][**IDatabaseInitializer**] и настройте его в методе **Register**, как показано ниже.
...
Теперь у нас есть типы моделей **Customer** и **Order** и DTO **MobileCustomer** и **MobileOrder**, но нужно наладить автоматическое преобразование между ними. Здесь мобильные службы опираются на реляционный модуль сопоставления [**AutoMapper**][**AutoMapper**], на который уже есть ссылка в проекте.
...
В качестве следующего шага можно собрать клиентское приложение для доступа к службе.
  ...
  [Изучение существующей модели базы данных]: #ExistingModel
  [Создание объектов передачи данных (DTO) для вашей мобильной службы]: #DTOs
  [Установка сопоставления между DTO и моделью]: #Mapping
  [Реализация логики для домена]: #DomainManager
  [Реализация TableController с помощью DTO]: #Controller
  [Подключение к локальному SQL Server из мобильной службы Azure с помощью гибридных подключений]: /ru-ru/documentation/articles/mobile-services-dotnet-backend-hybrid-connections-get-started/
