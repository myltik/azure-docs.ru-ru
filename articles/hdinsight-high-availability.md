...
Создание и подготовка кластера с использованием PowerShell см. в разделе [Администрирование HDInsight с помощью PowerShell][]. Настройка очень крупных головных узлов требует добавления параметра `-HeadNodeVMSize ExtraLarge` в командлет `New-AzureHDInsightcluster`, используемый в этом коде.
...
При использовании пакета SDK процедура аналогична. Создание и подготовка кластера с использованием SDK см. в разделе [Использование пакета SDК для HDInsight][]. Настройка очень крупных головных узлов требует добавления параметра `HeadNodeSize = NodeVMSize.ExtraLarge` в метод `ClusterCreateParameters()`, используемый в этом коде.
...
-   [Использование пакета SDK для HDInsight .NET][Использование пакета SDК для HDInsight]
