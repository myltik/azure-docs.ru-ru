...
### <a name="clouddrive-smb-limited-permissions"></a>Ограниченные разрешения Clouddrive SMB
Для некоторых команд в каталоге `clouddrive`, таких как `git clone`, не предусмотрены необходимые разрешения на чтение и запись некоторых файлов. Если вы столкнулись с этой проблемой, повторите попытку из своего каталога `$Home` без ограничений SMB.
