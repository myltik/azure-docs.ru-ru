...
Файлы Azure должны находиться в том же регионе, что и компьютер Cloud Shell, к которому вы их подключаете. Компьютеры Cloud Shell находятся в следующих регионах:
|Область|Регион|
|---|---|
...
Файловый ресурс, подключенный к Cloud Shell, можно отключить в любое время. Но так как Cloud Shell требуется подключенный файловый ресурс, в случае его удаления вам будет предложено создать и подключить новый файловый ресурс в следующем сеансе.
