<properties pageTitle="Как настроить Tomcat7 на виртуальной машине Linux с использованием Microsoft Azure" description="Узнайте, как установить Tomcat7 в виртуальной машине под управлением Linux в Microsoft Azure." services="virtual-machines" documentationCenter="" authors="NingKuang" manager="timlt" editor="tysonn"/>
...
3.	Нажмите кнопку **Создать**, чтобы создать ключи. В процессе можно увеличить степень случайности, переместив указатель мыши на пустую область окна.  
![][1]
4.	После завершения процесса создания полученный ключ отобразится в средстве Puttygen.exe. Например:  
![][2]
5.	Выберите и скопируйте открытый ключ в область **Ключ** и сохраните его в файле с именем publicKey.pem. Не щелкайте **Сохранить открытый ключ**, так как формат файла сохраненного открытого ключа отличается от необходимого открытого ключа.
...
На [портале предварительной версии Azure](https://portal.azure.com/) нажмите кнопку **Создать** на панели задач, чтобы создать образ, выбрав образ Linux, исходя из своих потребностей. В этом примере используется образ Ubuntu 14.04. 
![][3]
...
1.	На портале предварительной версии Azure щелкните **Обзор** -> **Виртуальная машина** и выберите созданную виртуальную машину.  
![][5]
...
3.	Щелкните **Добавить**.  
	1.	Введите имя **конечной точки** в поле "Конечная точка" и значение "80" в поле **Общий порт**.  
		...
		Если задано значение 80, нет необходимости включать номер порта в URL-адрес, по которому вы получаете доступ к Tomcat. Например, http://tomcatdemo.cloudapp.net.    
	...
	2.	Введите 8080 в поле "Частный порт". По умолчанию Tomcat прослушивает TCP-порт 8080. Если изменить значение по умолчанию для прослушивающего порта Tomcat, следует обновить частный порт, чтобы он совпадал с прослушивающим портом Tomcat.  
	![][7]
 
4.	Нажмите кнопку **ОК**, чтобы добавить конечную точку к виртуальной машине.
...
Сначала получите DNS-имя виртуальной машины на портале предварительной версии Azure. **Щелкните **Обзор -> **Виртуальная машина** -> имя виртуальной машины -> **Свойства**, а затем просмотрите значение в поле **Доменное имя** плитки **Свойства**.  
...
В левой области щелкните **Подключение** -> **SSH** -> **Проверка подлинности** и нажмите кнопку **Обзор**, чтобы указать расположение файла **privateKey.ppk**, который содержит закрытый ключ, созданный с помощью программы puttygen на этапе 1. Создайте образ. Пример:  
![][10]
...
Если вы установили OpenJDK, то увидите похожее сообщение:
![][14] 

Если вы установили oracle-jdk, то увидите похожее сообщение:
![][15] 
...
Чтобы убедиться в успешной установке Tomcat7, выберите имя DNS-сервера, на котором установлен Tomcat (http://tomcatexample.cloudapp.net/ is the example URL in this article). Если вы увидите подобную страницу, Tomcat7 установлен правильно:
![][16] 
...
После подключения должен появиться результат, аналогичный приведенному ниже.  
![][18]
...
-	**Симптом**
Служба Tomcat выполняется, но в браузере не отображается страница Tomcat по умолчанию.
-	**Возможная основная причина**
	1.	Прослушивающий порт Tomcat отличается от частного порта конечной точки виртуальной машины для трафика Tomcat.  
	
		Проверьте параметры конечных точек общего и частного портов и убедитесь, что частный порт совпадает с прослушивающим портом Tomcat. Чтобы получить инструкции по настройке конечных точек для своей виртуальной машины, см. этап 1 "Создание образа".  
		...
		Раскомментируйте последнюю строку и замените "no" на "yes".  
...
-	**Решение**
	1. Если прослушивающий порт Tomcat отличается от частного порта конечной точки для трафика в виртуальной машине, необходимо изменить частный порт, чтобы он совпал с прослушивающим портом Tomcat.   
...
-	**Симптом**
Если вы используете любой SFTP-клиент (такой как FileZilla) для подключения к виртуальной машине и переходите в папку /var/lib/tomcat7/webapps/ для публикации своего сайта, отображается сообщение об ошибке, похожее на следующее.  
