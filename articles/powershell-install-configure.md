...
> [AZURE.NOTE]Если вы установили модули Azure PowerShell из коллекции PowerShell, появится запрос на их удаление. Это необходимо для предотвращения путаницы с тем, какие модули установлены и куда. Модули из коллекции PowerShell, как правило, устанавливаются в папку **%ProgramFiles%\\WindowsPowerShell\\Modules**. При использовании установщика веб-платформы модули Azure устанавливаются в папку **%ProgramFiles%\\Microsoft SDKs\\Azure\\PowerShell**. **PowerShellGet** удалит модули, оставив заблокированные библиотеки DLL и соответствующие папки, если при удалении будут загружены зависимости модулей. Если вы удалили модули из коллекции PowerShell, но при установке по-прежнему возникает ошибка, удалите папки Azure* в папке **%ProgramFiles%\\WindowsPowerShell\\Modules**.

Если вы установили Azure PowerShell с помощью коллекции PowerShell, а теперь требуется установка с использованием WebPI, выполните следующие команды перед установкой.
...
> [AZURE.NOTE]При установке с использованием WebPI возникает известная проблема, связанная с PowerShell **$env:PSModulePath**. Если из-за установки обновлений системы или других компонентов потребуется перезагрузка, **$env:PSModulePath** может не включать путь установки Azure PowerShell. Это можно исправить путем перезагрузки или добавления пути Azure PowerShell в **$env: PSModulePath**.
...
## Шаг 2. Запуск
При установке модуля также устанавливается специальная консоль для Azure PowerShell. Командлеты можно выполнять в стандартной консоли Windows PowerShell или в консоли Azure PowerShell. Метод, используемый для открытия консоли, зависит от используемой версии Windows:
...
- На компьютере под управлением Windows более ранней версии, чем Windows 8 или Windows Server 2012, можно использовать меню **Пуск**. В меню **Пуск** щелкните **Все программы**, выберите **Azure**, а затем — **Azure PowerShell**.

Кроме того, можно запустить **интегрированную среду сценариев Windows PowerShell**, где можно использовать пункты меню и сочетания клавиш для выполнения многих задач, которые можно выполнить в консоли Windows PowerShell. Чтобы использовать интегрированную среду сценариев, в консоли Windows PowerShell, запускаемой через cmd.exe, или в поле **запуска** введите **powershell\_ise.exe**.
