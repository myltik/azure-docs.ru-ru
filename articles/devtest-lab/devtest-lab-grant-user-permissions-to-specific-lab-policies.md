...
Как уже обсуждалось в одноименное статье, [контроль доступа на основе ролей (RBAC)](/role-based-access-control-configure.md) обеспечивает точное управление доступом для Azure. С помощью RBAC вы можете распределить обязанности внутри команды разработчиков и предоставить пользователям доступ на том уровне, который им необходим для выполнения поставленных задач.
...
Дополнительные сведения о пользовательских ролях см. в разделе [Пользовательские роли в Azure RBAC](/role-based-access-control-configure.md#custom-roles-in-azure-rbac) статьи [Контроль доступа на основе ролей Azure](/role-based-access-control-configure.md).
