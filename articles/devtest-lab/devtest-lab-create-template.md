...
## Обзор
После [создания лаборатории](devtest-lab-create-lab.md) вы можете [добавить в нее виртуальные машины](devtest-lab-add-vm-with-artifacts.md). При создании виртуальной машины необходимо указать *базовый образ* виртуальной машины, который может представлять собой *пользовательский образ* или *образ Marketplace*. Из этой статьи вы узнаете, как [создать из VHD пользовательский образ](#create-a-custom-image-from-a-vhd), из которого затем можно будет создать виртуальную машину. Кроме того, вы сможете [создать пользовательский образ на основе виртуальной машины](#create-a-custom-image-from-a-vm) для последующего быстрого создания виртуальных машин.

## Создание пользовательского образа из VHD-файла

В этом разделе описано, как создать пользовательский образ из VHD-файла. Обратите внимание, что для этого вам потребуется допустимый VHD-файл.
...
Если у вас есть уже настроенная виртуальная машина, вы можете создать на ее основе пользовательский образ, который можно затем использовать для создания других идентичных виртуальных машин. Ниже описано, как создать пользовательский образ на основе виртуальной машины.
...
1. В колонке **Обзор** выберите виртуальную машину, на основе которой будет создан пользовательский образ.
...
1. В колонке **Create image** (Создание образа) введите имя и описание нового пользовательского образа. Эти данные будут отображаться в списке базовых образов при создании виртуальной машины.
