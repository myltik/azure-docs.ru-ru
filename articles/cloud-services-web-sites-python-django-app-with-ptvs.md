<properties urlDisplayName="Django with Python Tools for Visual Studio 2.0" pageTitle="Создание приложений Django с помощью средств Python для Visual Studio 2,0" metaKeywords="" description="Узнайте, как использовать инструменты Python для Visual Studio, чтобы создать приложение Django, которое хранит данные в экземпляре базы данных SQL или MySQL и может быть развернуто на веб-сайте или в облачной службе." metaCanonical="" services="web-sites,cloud-services" documentationCenter="Python" title="Creating Django applications with Python Tools 2.0 for Visual Studio" authors="huvalo" solutions="" manager="wpickett" editor="" />
...
В этом учебнике рассматриваются инструменты Python для Visual Studio и Microsoft Azure. Дополнительную информацию о Django и приложении для опросов, создаваемом в этом учебнике, см. на веб-сайте [https://www.djangoproject.com/](https://www.djangoproject.com/).
...
Если вы хотите пропустить этот раздел учебника, можете [загрузить исходный код данного проекта](http://download-codeplex.sec.s-msft.com/Download?ProjectName=pytools&DownloadId=783376).
...
В материалы для скачивания НЕ включена виртуальная среда. Ее необходимо создать, выполнив шаги, описанные в разделе [Создание виртуальной среды](#creating-a-virtual-environment). После этого проект будет готов к отладке (см. раздел [Отладка](#debugging)).

## Создание проекта

Инструменты Python для Visual Studio поддерживают виртуальные среды Python.  Мы будем создавать django-проект и используем виртуальную среду для установки зависимостей.  Это рекомендуемый способ настройки проектов, публикуемых на веб-сайтах или в облачных службах Azure.

1. Откройте Visual Studio, выберите "Файл"/"Новый проект", затем выберите приложение Django с именем **tutorial**.
...
Мы используем виртуальную среду для установки зависимостей. Рекомендуется использовать ее для любого приложения Python и публикации в Microsoft Azure.
...
1. Щелкните правой кнопкой мыши **env** и выберите пункт **Установить пакет Python**: **django**

	![Install Django](./media/cloud-services-python-create-deploy-django-app/django-tutorial-003-install-django.png)

1. Django содержит большое количество файлов, поэтому установка займет некоторое время. Ход выполнения можно посмотреть в окне выходных данных.
...
1. Щелкните правой кнопкой мыши **env** и выберите пункт **Установить пакет Python**: **pytz** (необязателен, но рекомендуется, используется в django для поддержки часового пояса)
...
В проекте Django может быть несколько приложений. В этом учебнике наш проект называется "tutorial" и соответствует проекту Visual Studio. Мы добавим приложение с именем **polls** и создадим папку в узле нашего проекта.

1. Выберите **узел проекта**, **Добавить** -> **приложение Django**, с именем **polls**. При этом создается папка для приложения со стандартным кодом для часто используемых файлов приложения.

	![Add Django App](./media/cloud-services-python-create-deploy-django-app/django-tutorial-005-add-django-app.png)

1. В **tutorial/settings.py** добавьте к **INSTALLED\_APPS**:

		'polls',

1. и раскомментируйте из **INSTALLED\_APPS**:
...
1. Переместите файл **polls/templates/index.html** в папку **polls/templates/polls** посредством перетаскивания или вырезания и вставки.
...
1. Замените раздел "БАЗЫ ДАННЫХ" следующим кодом:
...
1. Щелкните правой кнопкой мыши узел проекта и выберите **Django** -> **Django Sync DB**.  Появится окно интерактивного управления Django.  Поскольку базы данных еще не существует, будет предложено создать учетные данные администратора.  Введите имя пользователя и пароль. Адрес электронной почты вводить необязательно.

	![Django Sync DB](./media/cloud-services-python-create-deploy-django-app/django-tutorial-007-sqlite3.png)

1. Запустите веб-сайт используя клавишу F5 или сочетание клавиш CTRL+F5. При этом запустится сервер разработки django, который запустит ваш веб-браузер. В корневом URL-адресе веб-сайта отображается индекс опросов, но база данных пока не содержит ни одного опроса.

	![Web Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-008-dev-server.png)

1. Перейдите по ссылке **http://localhost:{порт}/admin**. Номер порта можно получить из окна консоли сервера разработки. Осуществите вход, используя учетные данные, созданные на предыдущем шаге.

	![Add Poll](./media/cloud-services-python-create-deploy-django-app/django-tutorial-009-admin-login.png)

1. Используйте интерфейс администрирования для добавления одного или двух опросов.  Не стоит тратить время, добавляя опросы в локальную базу данных. В дальнейшем мы переключимся на облачную базу данных, а затем повторно заполним ее.

	![Poll Index](./media/cloud-services-python-create-deploy-django-app/django-tutorial-009-admin-add-poll.png)

1. Перейдите по ссылке **http://localhost:{порт}/admin**.  Вы увидите индекс добавленных опросов.
...
В этом разделе мы обновим внешний вид сайта с использованием таблицы стилей. Статические файлы, такие как таблицы стилей, обрабатываются по-другому, поэтому важно хранить их в правильном месте.

1. В файле **tutorial/settings.py** найдите назначение **STATIC_ROOT** и измените его на:
...
1. Добавьте существующее изображение в папку **polls/static/polls/images** и присвойте ему имя **background.jpg**.
...
1. Перезапустите веб-сайт.  На главной странице и страницах опроса и результатов (index, polls и results) будет использоваться созданная нами таблица стилей с темно-синим текстом и фоновым изображением.
...
1. Откройте файл **polls/templates/polls/index.html** и с помощью клавиши **F9** поместите точку останова в этой строке:
...
1. Можно нажать клавишу **F10**, чтобы выполнить следующий шаг, как в обычном коде Python.  В цикле for можно проверить значение **опроса**:
...
В следующих 2 разделах будет показано, как использовать базы данных SQL и MySQL.  Обе базы данных являются размещенными службами.

Альтернативным вариантом является создание виртуальной машины и установка сервера базы данных.  Инструкции по настройке MySQL на виртуальной машине Linux Azure см. [здесь](http://www.windowsazure.com/ru-ru/manage/linux/common-tasks/mysql-on-a-linux-vm/).
...
1. На портале управления Microsoft Azure выберите **БАЗЫ ДАННЫХ SQL**.

1. Для начала создайте сервер для размещения базы данных.  Выберите **СЕРВЕРЫ**, а затем **ДОБАВИТЬ**.
...
1. Щелкните правой кнопкой мыши **env** и выберите пункт **Установить пакет Python**: **pyodbc** using **easy\_install**.

1. Щелкните правой кнопкой мыши **env** и выберите пункт **Установить пакет Python**: **django-pyodbc-azure** using **pip**.
...
В магазине Azure вы можете добавить различные службы в свою учетную запись, в том числе и базу данных MySQL.  Мы можем создать пробную учетную запись для бесплатного использования и небольшие базы данных, которые будут бесплатными до определенного размера.

1. На портале управления Microsoft Azure выберите **СОЗДАТЬ** -> **МАГАЗИН** -> **СЛУЖБЫ ПРИЛОЖЕНИЙ** -> **База данных MySQL ClearDB**.  Создайте базу данных с использованием бесплатного плана.

1. Далее мы установим пакет, необходимый для доступа к базам данных MySQL из Django в нашей виртуальной среде.  Щелкните **env** правой кнопкой мыши и выберите пункт **Установить пакет Python**: **mysql-python** using **easy\_install**.
...
1. Щелкните правой кнопкой мыши узел проекта и выберите **Django** -> **Open Django Shell**.
...
Инструменты Python для Visual Studio предусматривают возможности публикации на веб-сайтах и в облачных службах Azure.  Они описываются в следующих 2 разделах, при этом вы можете выбрать любой из вариантов или же оба варианта.

В обоих случаях PTVS позаботится о настройке IIS и создаст файл web.config автоматически, если он отсутствует в проекте.  Статические файлы будут собраны автоматически (manage.py collectstatic), если задан параметр STATIC_ROOT в settings.py.

Размещение Django на виртуальной машине не рассматривается в этом учебнике.  Здесь описывается создание виртуальной машины с использованием предпочтительной операционной системы (Windows или Linux), установка Python и развертывание приложения Django вручную. 

### Веб-сайт Azure

1. Для начала необходимо создать веб-сайт.  С помощью портала управления Azure щелкните **СОЗДАТЬ**->**СРЕДА ВЫПОЛНЕНИЯ ПРИЛОЖЕНИЙ**->**ВЕБ-САЙТ**->**БЫСТРОЕ СОЗДАНИЕ**.  Выберите любое имя из доступных имен.

1. По окончании операции создания загрузите профиль публикации для веб-сайта.
...
1. По окончании публикации опубликованный веб-сайт откроется в веб-браузере.
...
1. Щелкните правой кнопкой мыши узел проекта и выберите **Добавить проект облачной службы Azure** или **Преобразовать -> Преобразовать в проект облачной службы Azure** (вы увидите тот или другой элемент в зависимости от версии Visual Studio).  В результате в решение добавится новый проект с суффиксом .Azure. Новый проект будет помечен в решении как запускаемый проект.
...
1. Для запуска эмулятора среды выполнения, необходимо, чтобы приложение **Visual Studio было запущено с правами администратора**.

1. Начните отладку с помощью клавиши **F5**. Приложение запустится и будет развернуто в эмуляторе среды выполнения приложений. Проверьте, работает ли интерфейс администрирования и можете ли вы голосовать в опросах.
...
1. Примите значения по умолчанию и щелкните **Опубликовать**. Это займет больше времени, чем публикация на веб-сайте, поскольку необходимо подготовить виртуальную машину для облачной службы. Ход выполнения можно отслеживать в окне журнала действий Azure:
...
1. По окончании операции щелкните URL-адрес веб-сайта в окне журнала действий Azure, чтобы открыть веб-браузер.

	![Cloud Service Browser](./media/cloud-services-python-create-deploy-django-app/django-tutorial-021-cloudservice.png)

## Заключение

В этом учебнике мы разработали приложение Django с помощью [инструментов Python для Visual Studio](http://pytools.codeplex.com).  Мы использовали 3 разные базы данных: sqlite3, SQL Server и MySQL.  И, наконец, мы опубликовали приложение на веб-сайтах и в облачных службах Azure.
