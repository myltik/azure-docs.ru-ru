...
Обратите внимание, что время первого выполнения будет одинаковым независимо от того, имеет параметр startTime значение 2015-04-05 14:00 или 2015-04-01 14:00\. Время последующих выполнений рассчитывается по расписанию — 2015-04-11 в 14:00, 2015-04-13 в 14:00, 2015-04-15 в 14:00 и т. д.
...
С одной стороны, параметр *schedule* может *ограничивать* число выполнений задания.  Например, если, согласно параметру *schedule*, задание с ежемесячной частотой должно выполняться в 31-й день месяца, оно будет выполняться только в те месяцы, в которых есть 31-е число.

С другой стороны: параметр *schedule* может *увеличивать* число выполнений задания. Например, если, согласно параметру *schedule*, задание с ежемесячной частотой должно выполняться в 1-й и 2-й день месяца, оно будет выполняться 1-го и 2-го числа каждого месяца, а не один раз в месяц.
...
Предполагается, что во всех представленных расписаниях параметр *interval* имеет значение 1\.. Кроме того, в каждом случае следует принимать во внимание частоту, соответствующую параметру *schedule*, например использовать ежедневную частоту и параметр monthDays в одном расписании нельзя. Эти ограничения описаны выше.
