...
Планировщик Azure позволяет описывать действия, которые выполняются в облаке. Затем он планирует и выполняет эти действия автоматически. Планировщик делает это с помощью [портала Azure](scheduler-get-started-portal.md), кода, [API REST](https://msdn.microsoft.com/library/dn528946) или Azure PowerShell.

Планировщик создает, обслуживает и вызывает запланированную работу. Планировщик не размещает какую-либо рабочую нагрузку и не выполняет код. Он только _вызывает_ код, размещенный в другом месте — в Azure, на локальном компьютере или у другого поставщика. Код вызывается через HTTP, HTTPS или очередь хранилища.
