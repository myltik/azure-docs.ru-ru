<properties title="Управление параллелизмом в службе хранилища Microsoft Azure" pageTitle="обязательно" description="Как управлять параллелизмом для служб BLOB-объектов, очередей, таблиц и файлов" metaKeywords="Optional" services="Optional" solutions="Optional" documentationCenter="Optional" authors="Tamra Myers" videoId="Optional" scriptId="Optional" />
...
Следующий фрагмент C\#-кода (с использованием клиентской библиотеки хранилища версии 4.2.0) является простым примером того, как построить **If-Match AccessCondition**, основанное на значении ETag, доступном из свойств извлеченного или вставленного до этого BLOB-объекта. Если затем используется объект **AccessCondition**, когда он обновляет BLOB-объект: объект **AccessCondition** добавляет заголовок **If-Match** к запросу. Если произошло обновление BLOB-объекта другим процессом, служба возвращает сообщение о состоянии HTTP 412 (Необходимое условие не выполнено). Полное описание можно скачать [здесь][здесь].
...
Служба хранилища включает также поддержку дополнительных условных заголовков, таких как **If-Modified-Since**, **If-Unmodified-Since** и **If-None-Match**, а также их комбинаций. Дополнительные сведения см. в разделе [Указание условных заголовков для операций службы BLOB-объектов][Указание условных заголовков для операций службы BLOB-объектов] в MSDN.
...
| Set Container ACL (Задание списка управления доступом для контейнера)   | Да                                   | Да (\*)|                           
                                                                                                                  Удалить контейнер| Нет| Да|         
                                                                                                                  Арендовать контейнер| Да| Да|       
                                                                                                                  Перечислить BLOB-объекты| Нет| Нет  |
...
-   [Указание условных заголовков для операций службы BLOB-объектов][1]
-   [Lease Container (Аренда контейнера)][Lease Container (Аренда контейнера)]
-   [Lease Blob (Аренда BLOB-объекта)][аренду]
...
-   [Операции с сущностями.][Операции с сущностями.]
...
-   [API-интерфейс REST для службы очередей][API-интерфейс REST для службы очередей]
-   [Get Messages (Получение сообщений)][Get Messages (Получение сообщений)]
...
-   [Управление блокировкой файлов][Управление блокировкой файлов]
...
-   [Управление параллелизмом с помощью хранилища Azure — Образец приложения][здесь]
...
-   Знакомство с хранилищем для [BLOB-объектов][BLOB-объектов], [таблиц][таблиц] и [очередей][очередей]
-   Архитектура хранилища — [Хранилище Windows Azure: Высокодоступная служба облачного хранения с развитой системой согласованности][Хранилище Windows Azure: Высокодоступная служба облачного хранения с развитой системой согласованности]

  [здесь]: http://code.msdn.microsoft.com/windowsazure/Managing-Concurrency-using-56018114
  [Указание условных заголовков для операций службы BLOB-объектов]: http://msdn.microsoft.com/ru-ru/library/dd179371.aspx
  [аренду]: http://msdn.microsoft.com/ru-ru/library/azure/ee691972.aspx
  [1]: http://msdn.microsoft.com/ru-ru/library/azure/dd179371.aspx
  [Lease Container (Аренда контейнера)]: http://msdn.microsoft.com/ru-ru/library/azure/jj159103.aspx
  ...
  [Get Messages (Получение сообщений)]: http://msdn.microsoft.com/ru-ru/library/azure/dd179474.aspx
  [Управление блокировкой файлов]: http://msdn.microsoft.com/ru-ru/library/azure/dn194265.aspx
  [Microsoft Azure Storage Home Page]: http://azure.microsoft.com/ru-ru/services/storage/
  [Введение в хранилище Azure]: http://azure.microsoft.com/ru-ru/documentation/articles/storage-introduction/
  [BLOB-объектов]: http://azure.microsoft.com/ru-ru/documentation/articles/storage-dotnet-how-to-use-blobs/
  [таблиц]: http://azure.microsoft.com/ru-ru/documentation/articles/storage-dotnet-how-to-use-tables/
  [очередей]: http://azure.microsoft.com/ru-ru/documentation/articles/storage-dotnet-how-to-use-queues/
  [Хранилище Windows Azure: Высокодоступная служба облачного хранения с развитой системой согласованности]: http://blogs.msdn.com/b/windowsazurestorage/archive/2011/11/20/windows-azure-storage-a-highly-available-cloud-storage-service-with-strong-consistency.aspx
