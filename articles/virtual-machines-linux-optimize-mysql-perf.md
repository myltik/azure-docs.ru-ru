...
#Оптимизация и тестирование производительности MySQL на виртуальных машинах Azure Linux 
...
Хранилище - ключевой фактор, влияющий на производительность базы данных в облачных средах.  По сравнению с одним диском, RAID может обеспечить более быстрый доступ за счет параллелизма.  Дополнительную информацию см. в статье [Стандартные уровни RAID](http://en.wikipedia.org/wiki/Standard_RAID_levels).   

С помощью RAID можно существенно увеличить пропускную способность ввода-вывода диска и улучшить время ответа операций ввода-вывода в Azure. Наши лабораторные тесты показали, что при удвоении количества дисков RAID (с 2 до 4, с 4 до 8 и т. д.) удваивается пропускная способность ввода-вывода дисков, а время ответа операций ввода-вывода уменьшается в среднем в два раза. См. [Приложение А](#AppendixA) для получения дополнительных сведений.  

Помимо дисковых операций ввода-вывода производительность MySQL увеличивается при увеличении уровня RAID.  См. [Приложение Б](#AppendixB) для получения дополнительных сведений.  

Кроме того, вы можете обратить внимание на размер блоков. В целом чем больше размер блока, тем ниже нагрузка, особенно для объемных операций записи. Тем не менее, если размер блока слишком большой, это может привести к дополнительной нагрузке, и вы не сможете воспользоваться преимуществами RAID. Текущий размер блоков по умолчанию - 512 КБ. Он является оптимальным для большинства рабочих сред. См. [Приложение В](#AppendixC) для получения дополнительных сведений.   
...
Ниже объясняется, как создать RAID в Azure с помощью портала управления Microsoft Azure. RAID также можно настроить с помощью сценариев Windows PowerShell. 
...
Для дисков данных для параметра **Настройки кэша узла** должно быть задано значение **Нет**.  
...
[http://Azure.Microsoft.com/ru-ru/Documentation/articles/Virtual-Machines-Linux-Configure-RAID/](http://azure.microsoft.com/documentation/articles/virtual-machines-linux-configure-RAID/)
...
Например, измените файл  vim /etc/fstab, добавив noatime, как показано ниже.  
...
Сравнение уровней производительности после оптимизации приведено в разделе [Приложение Г](AppendixD).
...
Обратите внимание, что по умолчанию он не включен. При включении механизма slow query log могут потребляться некоторые ресурсы ЦП. Поэтому его рекомендуется включать временно для устранения узких мест производительности.

###Шаг 1. Измените файл my.cnf, добавив следующие строки в конец   
...
В этом примере видно, что функция медленных запросов включена. Затем можно использовать инструмент **mysqldumpslow**, чтобы определить узкие места производительности и оптимизировать ее, например, с помощью добавления индексов.
...
<a name="AppendixA"></a>Приложение A:  
...
<a name="AppendixB"></a>Приложение B:  
...
|Параметры	|По умолчанию	|Оптимизация
|-----------|-----------|-----------
...
|Оборудование	|Данные
|-----------|-------
|ЦП	|AMD Opteron(tm) Processor 4171 HE, 4 ядра
|Память	|14 ГБ
|Диск	|10G/диск
