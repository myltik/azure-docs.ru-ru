...
Предлагаемое Azure Service Bus в стандартном и [Premium](service-bus-premium-messaging.md) уровней. Уровень обслуживания можно выбрать для каждого пространства имен службы служебной шины, которое вы создаете, и этот уровень применяется ко всем сущностям, создаваемым в этом пространстве имен.
...
Очереди и разделы/подписки взимается плата за «операция», не отдельно для каждого сообщения. Операция ссылается любой вызов API, совершенные в отношении конечную точку службы очередей или разделов/подписок. Оно включает операции управления, отправки и получения сообщений и операции состояния сеанса.

| Тип операции | ОПИСАНИЕ |
| --- | --- |
| управления |Создание, чтение, обновление и удаление (CRUD) очередей, разделов или подписок. |
| Обмен сообщениями |Отправлять и получать сообщения с использованием очередей или разделов/подписок. |
| Состояние сеанса |Возвращает или задает состояние сеанса в очереди или раздела или подписки. |
...
*Обменов сообщениями через посредников* вместить закономерностей, включающие большое число «постоянно подключенных» отправителями и получателями для очередей, разделов или подписок. Постоянно подключенные отправители/получатели — это те отправители/получатели, которые подключаются по протоколу AMQP или HTTP с ненулевым временем ожидания приема (например, длинный опрос HTTP). Отправители и получатели HTTP с нулевым временем ожидания не создают подключений через посредника.
