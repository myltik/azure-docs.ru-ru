...
На предыдущем рисунке показаны три параллельных получателя сеансов, которым необходимо активно принимать сообщения из очереди для работы. Для предыдущего сеанса с параметром `SessionId` равным 4 нет активного клиента, которому принадлежит этот сеанс. Это означает, что ни один клиент не получит свои сообщения, пока это сообщение не будет принято вновь созданным получателем, владеющим этим сеансом.
