...
Microsoft Azure поддерживает два типа очередей: **очереди службы хранилища** и **очереди служебной шины**.
...
Для удобства сравнения в следующих разделах функции очередей службы хранилища и служебной шины сгруппированы логически и представлены в виде таблиц.
...
* Чтобы найти подозрительные сообщения в очередях службы хранилища, приложение проверяет свойство сообщения **[DequeueCount](https://msdn.microsoft.com/library/azure/dd179474.aspx)** при выведении его из очереди. Если значение **DequeueCount** превышает заданное пороговое значение, сообщение перемещается в определяемую приложением "мертвую очередь".
...
* Имена очередей службы хранилища могут содержать от 3 до 63 знаков, включая строчные буквы, цифры и дефисы (-). Дополнительные сведения см. в статье [Именование очередей и метаданных](https://msdn.microsoft.com/library/azure/dd179349.aspx).
