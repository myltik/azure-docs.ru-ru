---
title: "Обзор подписанных URL-адресов | Документация Майкрософт"
...
Вновь созданной политике назначается *первичный* и *вторичный ключ*. Это криптографически строгие ключи. Их невозможно потерять — они всегда будут доступны на [портале Azure][портал Azure]. Вы можете использовать любой из созданных ключей и создавать их повторно в любое время. Однако в случае повторного создания или изменения первичного ключа в политике любая созданная на его основе подпись общего доступа станет недействительной.
...
## <a name="shared-access-signature-token"></a>Подпись общего доступа (маркер)
...
## <a name="generating-a-signature-from-a-policy"></a>Создание подписи на основе политики
...
## <a name="using-the-shared-access-signature-at-http-level"></a>Использование подписанного URL-адреса (на уровне HTTP)
...
## <a name="using-the-shared-access-signature-at-amqp-level"></a>Использование подписанного URL-адреса (на уровне AMQP)
...
Дополнительные сведения о проверке подлинности с помощью служебной шины см. в [этой статье](service-bus-authentication-and-authorization.md). 
