...
Предварительная выборка ускоряет поток обработки сообщений, подготавливая сообщения для локального получения до того, как приложение их запросит. Это повышение пропускной способности является результатом компромиссного решения, которое разработчик приложения должен принять явным образом.
