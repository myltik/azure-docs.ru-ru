...
Следующий код создает объект **TableService** и использует его для создания новой таблицы. Добавьте в начало файла **server.js** следующий код.

    var tableSvc = azure.createTableService();

Вызов **createTableIfNotExists** создает новую таблицу с определенным именем, если ее еще не существует. В следующем примере создается новая таблица с именем "mytable", если она еще не создана:
...
result будет иметь значение true, если была создана новая страница, и false, если таблица уже существует. response будет содержать информацию о запросе.
...
В пакет SDK Azure для Node.js включены два фильтра, реализующие логику повторных попыток: **ExponentialRetryPolicyFilter** и **LinearRetryPolicyFilter**. Следующий код создает объект **TableService**, который использует **ExponentialRetryPolicyFilter**:
...
> [WACOM.NOTE] Для каждой записи имеется поле **Метка времени**, значение которого задается Azure при вставке или обновлении сущности.
...
Если операция прошла успешно, result будет содержать [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) добавленной записи, а response будет содержать информацию об операции.

> [WACOM.NOTE] По умолчанию метод **insertEntity** не возвращает добавленную сущность как часть информации response. Если вы хотите выполнить другие операции с этой сущностью или кэшировать информацию, необходимо, чтобы она была возвращена как result. Это можно сделать следующим образом, включив **echoContent**:
>
...
> [WACOM.NOTE] По умолчанию обновление сущности не проверяет, были ли обновляемые данные ранее изменены другим процессом. Поддержка одновременных обновлений:
> 
...
Если обновляемая сущность не существует, при использовании методов **updateEntity** и **mergeEntity** операция обновления завершится ошибкой. Поэтому, если нужно сохранить сущность независимо от того, существует она или нет, следует использовать метод **insertOrReplaceEntity** или **insertOrMergeEntity**.
...
Иногда имеет смысл отправлять совместно несколько операций в пакете для атомарной обработки сервером. Чтобы это сделать, используйте класс **TableBatch** для создания пакета, а затем метод **executeBatch** службы **TableService** для выполнения пакетных операций.
...
Для успешно завершенных пакетных операций result содержит сведения о всех операциях в пакете.

###Работа с пакетными операциями

Добавленные в пакет операции можно проверить, просмотрев свойство operations. Для работы с операциями можно также использовать следующие методы.
...
Для выполнения запроса к таблице используйте объект **TableQuery** для создания выражения запроса
с помощью следующих операторов:

* **select** - возвращаемые запросом поля.

* **where** - конструкция where.
...
Так как оператор **select** не используется, возвращаются все поля. Чтобы выполнить запрос в таблице, используйте метод **queryEntities**. В следующем примере используется запрос для возврата сущностей из таблицы 'mytable'.
...
При успешном завершении операции result.entries будет содержать массив сущностей, соответствующих запросу. Если запросу не удалось вернуть все сущности, result.continuationToken можно использовать как третий параметр метода **queryEntities** для получения дополнительных результатов. В начальном запросе второй параметр должен иметь значение NULL.
...
примере объект **task1** содержит значения сущности **RowKey** и
**PartitionKey**, которые будут удалены. Затем объект будет
...
> [WACOM.NOTE] Следует рассмотреть использование ETag при удалении элементов, чтобы гарантировать отсутствие в нем изменений сделанных другим процессом. См. раздел [Практическое руководство. Обновление сущности][], чтобы получить информацию об использовании ETag.
...
Клиентское приложение далее использует подписанный URL-адрес с помощью метода **TableServiceWithSAS** для выполнения операций с очередью. Следующий пример выполняет подключение к таблице и выполняет запрос.
...
В этом примере код получает текущее значение ACL для **hometasks**, а затем добавляет новые политики с помощью **setTableAcl**. Такой подход допускает выполнение:
