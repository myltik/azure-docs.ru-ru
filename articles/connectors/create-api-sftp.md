...
Подключитесь к SFTP-серверу, чтобы управлять своими файлами. На SFTP-сервере можно выполнять различные задачи, такие как отправка файлов, их удаление и многое другое.

API SFTP можно использовать из приложений логики.

>[AZURE.NOTE] Эта версия статьи предназначена для приложений логики со схемой версии 2015-08-01-preview. Чтобы получить версию схемы 2014-12-01-preview, щелкните [соединитель SFTP](../app-service-logic/app-service-logic-connector-sftp.md).
...
### Создание файла
Отправляет файл в SFTP. ```POST: /datasets/default/files```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|folderPath|string|Да|запрос|Нет |Уникальный путь к папке в SFTP|
...
| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|источник|строка|Да|запрос| Нет|Путь к исходному файлу|
|ресурс destination|string|Да|запрос|Нет |Путь к конечному файлу, включая имя файла|
...
### Удаление файла 
Удаляет файл в SFTP. ```DELETE: /datasets/default/files/{id}```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|id|string|Да|path|Нет |Уникальный идентификатор файла в SFTP|
...
### Извлечение папки
Извлекает файл архива в папку с помощью SFTP (например ZIP). ```POST: /datasets/default/extractFolderV2```
...
Получает содержимое файлов из SFTP с помощью идентификатора. ```GET: /datasets/default/files/{id}/content```
...
Получает содержимое файлов из SFTP с помощью пути. ```GET: /datasets/default/GetFileContentByPath```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|path|string|Да|запрос| Нет|Уникальный путь к файлу в SFTP|
...
Запускает поток в случае изменения файла в SFTP. ```GET: /datasets/default/triggers/onupdatedfile```
