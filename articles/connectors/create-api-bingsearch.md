<properties
	pageTitle="Добавление API Поиска Bing в PowerApps или приложения логики | Microsoft Azure"
	description="Обзор API Поиска Bing с параметрами интерфейса API REST"
	services=""
    suite=""
	documentationCenter="" 
	authors="MandiOhlinger"
	manager="erikre"
	editor=""
	tags="connectors"/>

<tags
   ms.service="multiple"
   ms.devlang="na"
   ms.topic="article"
   ms.tgt_pltfrm="na"
   ms.workload="na" 
   ms.date="02/11/2016"
   ms.author="mandia"/>

# Начало работы с API Поиска Bing 
Подключение к Поиску Bing позволяет искать новости, видео и многое другое.

API Поиска Bing можно использовать из PowerApps и приложений логики.

С Поиском Bing можно:

- формировать бизнес-процессы на основе данных, получаемых в результате поиска; 
- использовать действия для поиска изображений, новостей и многого другого. Эти действия получают ответ и делают выходные данные доступными для использования другими действиями. Например, можно найти видеоролик, а затем с помощью Twitter опубликовать его в веб-канале Twitter.
- Добавьте API Поиска Bing в PowerApps Enterprise. После этого пользователи смогут использовать этот API в своих приложениях. 

Сведения о добавлении API в PowerApps Enterprise см. в статье [Регистрация API в PowerApps](../power-apps/powerapps-register-from-available-apis.md).

Сведения о добавлении операции в приложения логики см. в статье [Создание приложения логики](../app-service-logic/app-service-logic-create-a-logic-app.md).

## Триггеры и действия
Поиск Bing включает следующие действия. Триггеры отсутствуют.

триггеры; | Действия
--- | ---
None | <ul><li>Поиск в Интернете</li><li>Поиск видео</li><li>Поиск изображений</li><li>Поиск новостей</li><li>Поиск похожего</li><li>Поиск написания</li><li>Поиск по всем</li></ul>

Все API-интерфейсы поддерживают данные в форматах JSON и XML.

## Добавление дополнительных настроек
При добавлении Поиска Bing в PowerApps Enterprise появляется запрос ключа учетной записи. Если у вас нет ключа Поиска Bing, получите его с помощью бесплатного [предложения для Поиска Bing](https://datamarket.azure.com/dataset/bing/search).


## Справочник по REST API Swagger
Относится к версии 1.0.

### Поиск в Интернете 
Извлекает веб-сайты из Поиска Bing. ```GET: /Web```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|string|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос| Нет|Количество пропускаемых результатов|
|adultContent|строка|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|строка|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос| Нет|Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос| Нет|Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|
|webFileType|string|Нет|запрос|Нет |Тип файла для сужения области поиска (например: DOC)|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск видео 
Извлекает видео из Поиска Bing. ```GET: /Video```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|string|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос| Нет|Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос|Нет |Количество пропускаемых результатов|
|adultContent|строка|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|string|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос|Нет |Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос|Нет |Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|
|videoFilters|строка|Нет|запрос|Нет |Фильтр поиска на основе размера, аспекта, цвета, стиля, распознавания лиц или комбинации этих параметров. Допустимые значения: <ul><li>Длительность:короткое</li><li>Длительность:среднее</li><li>Длительность:долгое</li><li>Формат:стандарт</li><li>Формат:широкоэкранный</li><li>Разрешение:низкое</li><li>Разрешение:среднее</li><li>Разрешение:высокое</li></ul> <p>Например: "Длительность:короткое+разрешение:высокое"</p>|
|videoSortBy|строка|Нет|запрос|Нет |Порядок сортировки результатов. Допустимые значения: <ul><li>Дата</li><li>Релевантность</li></ul> <p>При сортировке по дате используется порядок сортировки по убыванию.</p>|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск изображений    
Извлекает изображения из Поиска Bing. ```GET: /Image```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|строка|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос|Нет |Количество пропускаемых результатов|
|adultContent|string|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|string|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос| Нет|Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос|Нет |Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|
|imageFilters|строка|Нет|запрос|Нет |Фильтр поиска на основе размера, аспекта, цвета, стиля, распознавания лиц или комбинации этих параметров. Допустимые значения: <ul><li>Размер:малый</li><li>Размер:средний</li><li>Размер:большой</li><li>Размер,ширина:[ширина]</li><li>Размер:высота:[высота]</li><li>Формат:квадрат</li><li>Формат: широкое</li><li>Формат:высокое</li><li>Цвета:цветное</li><li>Цвета:монохромное</li><li>Стиль:фото</li><li>Стиль:графика</li><li>Лицо:лицо</li><li>Лицо:портрет</li><li>Лицо:другое</li></ul><p>Например: "Размер:малый+формат:квадрат"</p>|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск новостей    
Извлекает новости из поиска Bing. ```GET: /News```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|string|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос| Нет|Количество пропускаемых результатов|
|adultContent|строка|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|string|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос|Нет |Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос|Нет |Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|
|newsSortBy|строка|Нет|запрос| Нет|Порядок сортировки результатов. Допустимые значения: <ul><li>Дата</li><li>Релевантность</li></ul> <p>При сортировке по дате используется порядок сортировки по убыванию.</p>|
|newsCategory|string|Нет|запрос| |Категории новостей для сужения области поиска (пример: "rt\_Business")|
|newsLocationOverride|string|Нет|запрос|Нет |Переопределение для обнаружения местонахождения Bing. Этот параметр применяется только на рынке ru-RU. Формат ввода — US./<state /> (например: US.WA)|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск написания    
Извлекает предложения по правильному написанию. ```GET: /SpellingSuggestions```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|string|Да|запрос| Нет|Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос| Нет|Количество пропускаемых результатов|
|adultContent|string|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|строка|Нет|запрос| Нет|Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос|Нет |Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос|Нет |Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск похожего    
Извлекает похожие результаты поиска из поиска Bing. ```GET: /RelatedSearch```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|строка|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос| Нет|Количество пропускаемых результатов|
|adultContent|string|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|string|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос|Нет |Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос| Нет|Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


### Поиск по всем    
Извлекает все веб-сайты, видео, изображения и т. д. из Поиска Bing. ```GET: /CompositeSearch```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|запрос|строка|Да|запрос|Нет |Текст для поиска (например: xbox)|
|maxResult|целое число|Нет|запрос|Нет |Максимальное количество отображаемых результатов|
|startOffset|целое число|Нет|запрос|Нет |Количество пропускаемых результатов|
|adultContent|строка|Нет|запрос|Нет |Фильтр содержимого для взрослых Допустимые значения: <ul><li>Отключен</li><li>Умеренный</li><li>Строгий</li></ul>|
|market|строка|Нет|запрос|Нет |Рынок или регион для сужения области поиска (например: ru-RU)|
|longitude|number|Нет|запрос|Нет |Долгота (координаты востока и запада) для сужения области поиска (например: 47.603450)|
|latitude|number|Нет|запрос|Нет |Широта (координаты севера и юга) для сужения области поиска (например: -122.329696)|
|webFileType|строка|Нет|запрос|Нет |Тип файла для сужения области поиска (например: DOC)|
|videoFilters|string|Нет|запрос|Нет |Фильтр поиска на основе размера, аспекта, цвета, стиля, распознавания лиц или комбинации этих параметров. Допустимые значения: <ul><li>Длительность:короткое</li><li>Длительность:среднее</li><li>Длительность:долгое</li><li>Формат:стандарт</li><li>Формат:широкоэкранный</li><li>Разрешение:низкое</li><li>Разрешение:среднее</li><li>Разрешение:высокое</li></ul> <p>Например: "Длительность:короткое+разрешение:высокое"</p>|
|videoSortBy|строка|Нет|запрос|Нет |Порядок сортировки результатов. Допустимые значения: <ul><li>Дата</li><li>Релевантность</li></ul> <p>При сортировке по дате используется порядок сортировки по убыванию.</p>|
|imageFilters|строка|Нет|запрос|Нет |Фильтр поиска на основе размера, аспекта, цвета, стиля, распознавания лиц или комбинации этих параметров. Допустимые значения: <ul><li>Размер:малый</li><li>Размер:средний</li><li>Размер:большой</li><li>Размер,ширина:[ширина]</li><li>Размер:высота:[высота]</li><li>Формат:квадрат</li><li>Формат: широкое</li><li>Формат:высокое</li><li>Цвета:цветное</li><li>Цвета:монохромное</li><li>Стиль:фото</li><li>Стиль:графика</li><li>Лицо:лицо</li><li>Лицо:портрет</li><li>Лицо:другое</li></ul><p>Например: "Размер:малый+формат:квадрат"</p>|
|newsSortBy|string|Нет|запрос|Нет |Порядок сортировки результатов. Допустимые значения: <ul><li>Дата</li><li>Релевантность</li></ul> <p>При сортировке по дате используется порядок сортировки по убыванию.</p>|
|newsCategory|строка|Нет|запрос|Нет |Категории новостей для сужения области поиска (пример: "rt\_Business")|
|newsLocationOverride|строка|Нет|запрос|Нет |Переопределение для обнаружения местонахождения Bing. Этот параметр применяется только на рынке ru-RU. Формат ввода — US./<state /> (например: US.WA)|

#### Ответ
|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|


## Определения объектов

#### WebResultModel: результаты веб-поиска Bing

|Имя свойства | Тип данных | Обязательно |
|---|---|---|
|Название|string|Нет|
|Описание|строка|Нет|
|DisplayUrl|string|Нет|
|Идентификатор|string|Нет|
|FullUrl|string|Нет|

#### VideoResultModel: результаты поиска видео в Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|Название|строка|Нет|
|DisplayUrl|строка|Нет|
|Идентификатор|строка|Нет|
|MediaUrl|string|Нет|
|Среда выполнения|целое число|Нет|
|Эскиз|не определено|Нет|

#### ThumbnailModel: свойства эскиза элемента мультимедиа

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|MediaUrl|string|Нет|
|ContentType|строка|Нет|
|Ширина|целое число|Нет|
|Высота|целое число|Нет|
|FileSize|целое число|Нет|

#### ImageResultModel: результаты поиска изображения в Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|Название|строка|Нет|
|DisplayUrl|string|Нет|
|Идентификатор|строка|Нет|
|MediaUrl|строка|Нет|
|SourceUrl|string|Нет|
|Эскиз|не определено|Нет|

#### NewsResultModel: результаты поиска новостей в Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|Название|строка|Нет|
|Описание|строка|Нет|
|DisplayUrl|string|Нет|
|Идентификатор|строка|Нет|
|Источник|string|Нет|
|Дата|строка|Нет|

#### SpellResultModel: результаты вариантов написания при проверке орфографии в Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|Идентификатор|строка|Нет|
|Значение|строка|Нет|

#### RelatedSearchResultModel: связанные результаты поиска Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|Название|строка|Нет|
|Идентификатор|string|Нет|
|BingUrl|строка|Нет|

#### CompositeSearchResultModel: результаты составного поиска Bing

|Имя свойства | Тип данных |Обязательно |
|---|---|---|
|WebResultsTotal|целое число|Нет|
|ImageResultsTotal|целое число|Нет|
|VideoResultsTotal|целое число|Нет|
|NewsResultsTotal|целое число|
|SpellSuggestionsTotal|целое число|Нет|
|WebResults|array|Нет|
|ImageResults|array|Нет|
|VideoResults|array|Нет|
|NewsResults|array|Нет|
|SpellSuggestionResults|array|Нет|
|RelatedSearchResults|array|Нет|

## Дальнейшие действия

После добавления API Поиска Bing в PowerApps Enterprise [предоставьте пользователям разрешения](../power-apps/powerapps-manage-api-connection-user-access.md) на использование API в своих приложениях.

[Создание приложения логики](../app-service-logic/app-service-logic-create-a-logic-app.md).

<!---HONumber=AcomDC_0224_2016-->