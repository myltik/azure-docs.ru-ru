...
|Отпечаток хост-ключа для сервера SSH | Да | Укажите отпечаток открытого хост-ключа для сервера SSH. <br/><br/>Как правило, этот ключ можно получить у администратора сервера. Для получения отпечатка ключа также используются инструменты ```WinSCP``` или ```ssh-keygen-g3 -F```. | 
