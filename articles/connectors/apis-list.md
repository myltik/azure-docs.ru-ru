...
| [![API Icon][Office-365-Outlookicon]<br/>**Office 365<br/>Outlook**][office365-outlookdoc] | Доступны множество триггеров и действий для использования в рабочих процессах почты и событий Office 365. <br/><br/>Этот соединитель предоставляет действие для *письма с запросом на утверждение*, чтобы утвердить заявления на отпуск, отчеты по расходам и т. д. <br/><br/>С этим соединителем также доступны пользователи Office 365.| [![API Icon][HTTP-Requesticon]<br/>**Запрос и ответ**][HTTP-Requestdoc] | Этот соединитель предоставляет URL-адрес для протокола HTTPS. Приложение логики запускается при получении запроса к этому URL-адресу. |
...
| **Создание таблицы CSV**<br/>**Создание таблицы HTML** | Преобразуйте результирующий набор массива в таблицу CSV или HTML. Например, добавьте действие CRM отображения списка записей и фильтр для записей, которые были добавлены сегодня. Затем отправьте результаты как HTML-таблицу в сообщении электронной почты. |
| **Фильтрация массива** (запрос) | Отфильтруйте результирующий набор по интересующим вас записям.  Например, выполните поиск всех твитов с термином `#Azure`, а затем отфильтруйте возвращенные твиты только для отображения результатов, соответствующих `Tweeted_by_followers > 50`. |
...
| **Выбор** | Выберите определенные свойства массива для дальнейшей обработки.  Если вы выберите отображение списка записей из SQL и при этом вернется 15 столбцов, выберите всего несколько из них для дальнейшей обработки (выходные данные представляют собой массив, содержащий только те свойства, которые вы выбрали). |
