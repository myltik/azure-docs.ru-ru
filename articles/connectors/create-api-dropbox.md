...
Подключение к Dropbox позволяет управлять файлами, например: создавать файлы, получать их и т. д.

API Dropbox можно использовать из PowerApps Enterprise и приложений логики.
...
4. В окне **Создание нового приложения на платформе Dropbox**:  
	...
	1. В поле **Выберите API** выделите **API Dropbox**.
	2. В поле **Выберите необходимый тип доступа** выделите **Весь Dropbox**.  
...
5. На странице настройки приложения:
	...
	2. Щелкните ссылку **Показать**, чтобы раскрыть **Секрет приложения**:  
...
### Создание файла    
Отправляет файл в Dropbox. ```POST: /datasets/default/files```
...
### При создании файла    
Запускает поток при создании файла в папке Dropbox. ```GET: /datasets/default/triggers/onnewfile```

| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|folderId|строка|Да|запрос|Нет |Уникальный идентификатор папки в Dropbox|
...
| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|id|string|Да|path|Нет|Уникальный идентификатор файла, удаляемого из Dropbox|
...
Извлекает файл архива в папку в Dropbox (например: ZIP). **```POST: /datasets/default/extractFolderV2```**
...
| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|id|string|Да|path|Нет |Уникальный идентификатор файла в Dropbox|
...
### При изменении файла    
Запускает поток при изменении файла в папке Dropbox. ```GET: /datasets/default/triggers/onupdatedfile```
