<properties
pageTitle="Добавление API Excel в PowerApps Enterprise | Microsoft Azure"
description="Обзор API Excel с параметрами REST API"
services=""	
documentationCenter="" 	
authors="msftman"	
manager="erikre"	
editor=""
tags="connectors"/>

<tags
ms.service="multiple"
ms.devlang="na"
ms.topic="article"
ms.tgt_pltfrm="na"
ms.workload="na"
ms.date="02/23/2016"
ms.author="deonhe"/>

# Приступая к работе с API FTP

Подключение к Excel для вставки строки, удаления строки и многого другого.

API Excel можно использовать из PowerApps Enterprise.

С помощью Excel можно:

- Добавить API Excel в PowerApps Enterprise. После этого пользователи смогут использовать этот API в своих приложениях. 

Сведения о добавлении API в PowerApps Enterprise см. в статье [Регистрация API в PowerApps](../power-apps/powerapps-register-from-available-apis.md).

Сведения о добавлении операции в приложения логики см. в статье [Создание приложения логики](../app-service-logic/app-service-logic-create-a-logic-app.md).

## Триггеры и действия
Excel содержит следующее действие. Триггеры отсутствуют.

|Триггер|Действия|
|--- | ---|
|None | <ul><li>Получение строк</li><li>Вставка строки</li><li>Удаление строки</li><li>Получение строки</li><li>Получение таблиц</li><li>Обновление строки</li></ul>

Все API поддерживают данные в форматах JSON и XML.

## Справочник по REST API Swagger
#### Эта документация предназначена для версии 1.0.


### Вставка новой строки в таблицу Excel
**```POST: /datasets/{dataset}/tables/{table}/items```**



| Имя| Тип данных|Обязательно|Расположение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|dataset|string|Да|path|Нет|Имя файла Excel|
|таблица|строка|Да|path|Нет|Имя таблицы Excel|
|item| |Да|текст|Нет|Строка, вставляемая в указанную таблицу Excel|


### Ответ

|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|
------



### Извлечение одной строки из таблицы Excel
**```GET: /datasets/{dataset}/tables/{table}/items/{id}```**



| Имя| Тип данных|Обязательно|Расположение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|dataset|строка|Да|path|Нет|Имя файла Excel|
|таблица|строка|Да|path|Нет|Имя таблицы Excel|
|id|строка|Да|path|Нет|Уникальный идентификатор извлекаемой строки|


### Ответ

|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|
------



### Удаление одной строки из таблицы Excel
**```DELETE: /datasets/{dataset}/tables/{table}/items/{id}```**



| Имя| Тип данных|Обязательно|Расположение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|dataset|строка|Да|path|Нет|Имя файла Excel|
|таблица|строка|Да|path|Нет|Имя таблицы Excel|
|id|строка|Да|path|Нет|Уникальный идентификатор удаляемой строки|


### Ответ

|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|
------



### Обновление существующей строки в таблице Excel
**```PATCH: /datasets/{dataset}/tables/{table}/items/{id}```**



| Имя| Тип данных|Обязательно|Расположение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|dataset|string|Да|path|Нет|Имя файла Excel|
|таблица|string|Да|path|Нет|Имя таблицы Excel|
|id|строка|Да|path|Нет|Уникальный идентификатор обновляемой строки|
|item| |Да|текст|Нет|Строка с обновленными значениями|


### Ответ

|Имя|Описание|
|---|---|
|200|ОК|
|по умолчанию|Операция завершилась ошибкой.|
------



## Определение объекта

#### DataSetsMetadata

| Имя | Тип данных | Обязательно|
|---|---|---|
|tabular|Не определен|Нет|
|blob-объект|Не определен|Нет|

#### TabularDataSetsMetadata

| Имя | Тип данных |Обязательно|
|---|---|---|
|источник|строка|Нет|
|displayName|строка|Нет|
|urlEncoding|string|Нет|
|tableDisplayName|string|Нет|
|tablePluralName|string|Нет|

#### BlobDataSetsMetadata

| Имя | Тип данных |Обязательно|
|---|---|---|
|источник|строка|Нет|
|displayName|string|Нет|
|urlEncoding|string|Нет|

#### TableMetadata

| Имя | Тип данных |Обязательно|
|---|---|---|
|name|строка|Нет|
|title|string|Нет|
|x-ms-permission|строка|Нет|
|schema|Не определен|Нет|

#### DataSetsList

| Имя | Тип данных |Обязательно|
|---|---|---|
|value|array|Нет|

#### DataSet

| Имя | Тип данных |Обязательно|
|---|---|---|
|Имя|строка|Нет|
|DisplayName|строка|Нет|

#### Таблица

| Имя | Тип данных |Обязательно|
|---|---|---|
|Имя|строка|Нет|
|DisplayName|строка|Нет|

#### Элемент

| Имя | Тип данных |Обязательно|
|---|---|---|
|ItemInternalId|строка|Нет|

#### TablesList

| Имя | Тип данных |Обязательно|
|---|---|---|
|value|array|Нет|

#### ItemsList

| Имя | Тип данных |Обязательно|
|---|---|---|
|value|array|Нет|


## Дальнейшие действия
[Создание приложения логики](../app-service-logic/app-service-logic-create-a-logic-app.md) [Создание приложения PowerApps](../power-apps/powerapps-get-started-azure-portal.md)

<!---HONumber=AcomDC_0224_2016-->