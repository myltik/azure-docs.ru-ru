<properties
	pageTitle="Добавление API Twitter в службу PowerApps Enterprise и приложения логики | Microsoft Azure"
	description="Обзор API Twitter с параметрами интерфейса API REST"
	services=""
	documentationCenter="" 
...
# Начало работы с API-интерфейсом Twitter
Подключитесь к Twitter, чтобы публиковать твиты, получать временную шкалу пользователя и многое другое.

API Twitter можно использовать из PowerApps Enterprise и приложений логики.

>[AZURE.NOTE] Эта версия статьи предназначена для приложений логики со схемой версии 2015-08-01-preview. Чтобы получить версию схемы 2014-12-01-preview, щелкните [соединитель Twitter](../app-service-logic/app-service-logic-connector-twitter.md).
...
- Добавлять API Twitter в службу PowerApps Enterprise. После этого пользователи смогут использовать этот API в своих приложениях. 
...
Триггер | Действия
--- | ---
<ul><li>При появлении нового твита</li></ul>| <ul><li>Публикация нового твита</li><li>При появлении нового твита</li><li>Получение домашней временной шкалы</li><li>Получение пользователя</li><li>Получение временной шкалы пользователя</li><li>Поиск твита </li><li>Получение подписчиков</li><li>Получение моих подписчиков</li><li>Получение пользователей, на которых подписан пользователь</li><li>Получение пользователей, на которых я подписан</li></ul>
...
При добавлении Twitter в PowerApps Enterprise введите значения **ключа потребителя** и **секрета потребителя** для приложения Twitter. В приложении Twitter также используется **URL-адрес перенаправления**. Если у вас нет приложения Twitter, создайте его, выполнив приведенные ниже действия:

1. Войдите в [Twitter](https://apps.twitter.com).

2. Щелкните **Создать новое приложение**: ![страница приложений Twitter][6]

3. На экране **Создание приложения**:
   
	1. Введите нужные значения в поля **Имя**, **Описание** и **Веб-сайт**.
	2. В поле **URL-адрес обратного вызова** введите значение **URL-адреса перенаправления**, которое отображалось при добавлении API Twitter на портале Azure.
	5. Примите условия соглашения и **создайте приложение Twitter**.  

	![создание приложения Twitter][7]

Скопируйте и вставьте значения **ключа пользователя** и **секрета пользователя** в конфигурацию Twitter на портале Azure.


### Добавление дополнительных настроек в приложения логики
При добавлении API в приложения логики эти приложения необходимо авторизовать для подключения к учетной записи Twitter.
...
После создания подключения укажите свойства Twitter, например текст твита. Эти свойства описаны далее в **справочнике по REST API**.
...
Запускает рабочий процесс, когда опубликован новый твит, который соответствует условиям поискового запроса. ```GET: /onnewtweet```
...
| Имя| Тип данных|Обязательно|Местонахождение|Значение по умолчанию|Описание|
| ---|---|---|---|---|---|
|userName|string|Да|запрос|Нет|Имя пользователя в Twitter|
...
### Поиск твита 
Получает набор связанных твитов, соответствующих указанному запросу. ```GET: /searchtweets```
...
Получает пользователей, подписчиком которых я являюсь. ```GET: /myfriends```
