...
API Facebook можно использовать из приложений логики.
...
| триггеры; | Действия|
| --- | --- |
| <ul><li>При появлении новой публикации в хронике</li></ul> |<ul><li>Получение потока из хроники</li><li>Публикация в хронике</li><li>При появлении новой публикации в хронике</li><li>Получение канала страниц</li><li>Получение публикаций в хронике пользователя</li><li>Публикация на странице</li></ul>
...
Получает поток из хроники вошедшего в систему пользователя. ```GET: /me/feed```
...
|фильтр|строка|Нет|запрос| Нет|Получение только тех публикаций, которые соответствуют определенному фильтру потока.|
...
Публикует сообщение в хронике пользователя, вошедшего в систему. ```POST: /me/feed```
...
Запускает новый поток, когда в хронике пользователя, вошедшего в систему, появляется новая публикация. ```GET: /trigger/me/feed```
...
Получает сообщения из канала указанной страницы. ```GET: /{pageId}/feed```
...
|fields|строка|Нет|запрос| Нет|Укажите поля, которые необходимо вернуть. Пример (идентификатор, имя, фото).|
