...
Подключитесь к Salesforce, чтобы создавать объекты, получать их и выполнять многие другие действия.

API Salesforce можно использовать из PowerApps Enterprise и приложений логики.

>[AZURE.NOTE] Эта версия статьи предназначена для приложений логики со схемой версии 2015-08-01-preview. Чтобы получить версию схемы 2014-12-01-preview, щелкните [API Salesforce](../app-service-logic/app-service-logic-connector-salesforce.md).
...
1. [Войдите на домашнюю страницу разработчика Salesforce][5], выберите свой профиль и **Установка**: ![домашняя страница SalesForce][6]

3. Выберите **Создать** и **Приложения**. На экране **Приложения** в разделе **Подключенные приложения** нажмите кнопку **Создать**: ![создание приложения SalesForce][7]

4. На экране **Новое подключенное приложение**:

	1. Введите значение в поле **Имя подключенного приложения**.  
	2. Введите значение в поле **Имя API**.  
	3. Введите значение в поле **Контактный адрес электронной почты**.  
	4. В разделе _API (включить параметры OAuth)_ установите флажок **Включить параметры OAuth** и в поле **URL-адрес обратного вызова** введите значение, которое отображалось при добавлении нового API Salesforce на портале Azure.  
...
| ---|---|---|---|---|---|
|таблица|string|Да|path|Нет|Тип SObject Salesforce (например "Lead")|
...
| ---|---|---|---|---|---|
|таблица|строка|Да|path|Нет|Тип SObject Salesforce (например "Lead")|
|id|string|Да|path|Нет|Уникальный идентификатор удаляемого объекта Salesforce|
...
| ---|---|---|---|---|---|
|таблица|string|Да|path|Нет|Тип SObject Salesforce (например "Lead")|
|id|string|Да|path|Нет|Уникальный идентификатор обновляемого объекта Salesforce|
...
Запускает поток при создании объекта в Salesforce. ```GET: /datasets/default/tables/{table}/onnewitems```
...
|$filter|строка|Нет|запрос|Нет|Запрос фильтра ODATA для ограничения количества записей|
...
Запускает поток при изменении объекта в Salesforce. ```GET: /datasets/default/tables/{table}/onupdateditems```
