...
Действие копирования фабрики данных Azure обеспечивает первоклассное решение для безопасной, надежной и высокопроизводительной загрузки данных, позволяя каждый день копировать десятки терабайтов информации в самых разнообразных облачных и локальных хранилищах данных. Именно высокоскоростная загрузка данных позволяет сосредоточиться на основных проблемах "больших данных": создании решений расширенной аналитики и получении ценной информации из данных.
...
- Загрузка данных в **хранилище данных Azure SQL** на скорости **1,2 Гбит/с**.
...
- Функции, которые могут значительно повысить пропускную способность копирования в различных сценариях, в том числе [параллельное копирование](#parallel-copy), [облачные единицы перемещения данных](#cloud-data-movement-units) и [промежуточное копирование](#staged-copy).
...
- Контрольные показатели производительности в приведенной выше таблице были измерены с использованием набора данных [TPC-H](http://www.tpc.org/tpch/) для выполнения отдельного действия копирования.
- При копировании данных из одного облачного хранилища данных в другое мы задали для свойства **cloudDataMovementUnits** значение 1 и 4 (или 8) для сравнения. Свойство **parallelCopies** не было указано. Дополнительные сведения об этих функциях см. в разделе [Параллельное копирование](#parallel-copy).
- При использовании хранилищ данных Azure источник и приемник находились в одном регионе Azure.
- Для гибридного перемещения данных (из локальной среды в облако или наоборот) использовался один экземпляр шлюза управления данными на компьютере, на котором не находилось локальное хранилище данных. Конфигурация приведена в следующей таблице. При выполнении одного действия в шлюзе операция копирования потребляла небольшую часть ресурсов ЦП и памяти, а также небольшой объем пропускной способности сети на тестовом компьютере.
	<table>
	<tr>
		<td>ЦП</td>
...
**Допустимые значения** свойства **cloudDataMovementUnits**: 1 (по умолчанию), 2, 4 и 8. **Фактическое число облачных единиц перемещения данных**, используемых во время выполнения операции копирования, меньше или равно заданному значению, в зависимости от шаблона данных.

> [AZURE.NOTE] Если вам требуется больше облачных единиц перемещения данных, чтобы повысить пропускную способность, обратитесь в [службу поддержки Azure](https://azure.microsoft.com/support/). В настоящее время поддерживаются 8 и более облачных единиц перемещения данных только при копировании нескольких файлов, имеющих размер не меньше 16 МБ каждый, из хранилища BLOB-объектов в хранилище BLOB-объектов, Data Lake Store или Базу данных SQL Azure.
