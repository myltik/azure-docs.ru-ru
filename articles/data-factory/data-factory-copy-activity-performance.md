...
- При копировании данных в файловые хранилища из хранилищ, не являющихся файловыми, свойство parallelCopies игнорируется, даже если оно указано. Параллелизм не будет использоваться.
...
В **сценарии облачного копирования**, когда источник и приемник данных размещены в облаке, а шлюз управления данными не используется, операции копирования выполняет **служба фабрики данных Azure**.
...
**Одновременное выполнение действий копирования.** Один экземпляр шлюза управления данными может обработать несколько запусков действия копирования, т. е. шлюз может выполнять несколько заданий копирования одновременно (для вычисления количества одновременно выполняемых заданий используется конфигурация оборудования компьютера шлюза). Дополнительные задания копирования находятся в очереди, пока их не выберет шлюз или пока не истечет время ожидания выполнения задания (в зависимости от того, что произойдет раньше). Чтобы избежать конфликта ресурсов в шлюзе, можно составить график выполнения действий таким образом, чтобы уменьшить количество поставленных в очередь заданий копирования, или разделить нагрузку между несколькими шлюзами.
...
**Сценарий.** Конвейер предназначен для копирования данных с локального сервера SQL Server в большой двоичный объект Azure в формате CSV. Для ускорения копирования необходимо сжать CSV-файлы в формат BZIP2.
...
2.	**Шлюз управления данными.**
	1.	**Локальная сеть.** Шлюз находится далеко от SQL Server с низкой пропускной способностью.
