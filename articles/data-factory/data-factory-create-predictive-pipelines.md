...
Прежде чем приступить к этому примеру, рекомендуется изучить учебник [Построение первого конвейера с помощью фабрики данных Azure][adf-build-1st-pipeline] и с помощью редактора фабрики данных создать артефакты фабрики данных (связанные службы, таблицы, конвейер) для этого примера.
...
Добавьте раздел **typeProperties** в раздел **AzureMLBatchScoringActivty** в конвейере JSON, чтобы указать в этом разделе значения для параметров веб-службы, как показано в следующем примере:
