<properties 
	pageTitle="Фабрика данных. Создание прогнозирующих конвейеров с помощью фабрик данных и машинного обучения | Microsoft Azure" 
...
> [AZURE.NOTE]Сведения о новом действии пакетного выполнения машинного обучения, предоставляющем более гибкие возможности, чем действие пакетной оценки, о которой рассказывается в данной статье, см. в статье [Создание прогнозирующих конвейеров с помощью действия пакетного выполнения машинного обучения Azure](data-factory-azure-ml-batch-execution-activity.md).
...
2. **URI запроса** для API пакетного выполнения. Чтобы найти URI запроса, перейдите по ссылке **ПАКЕТНОЕ ВЫПОЛНЕНИЕ** на странице веб-служб, как показано ниже.
...
Прежде чем приступить к этому примеру, рекомендуется изучить учебник [Создание первого конвейера с помощью фабрики данных][adf-build-1st-pipeline] и с помощью редактора фабрики данных создать артефакты фабрики данных (связанные службы, таблицы и конвейер) для этого примера.
 

1. Создайте **связанную службу** для **службы хранилища Azure**. Если входные и выходные файлы оценки находятся в разных учетных записях хранения, то потребуются две связанные службы. Ниже приведен пример JSON:
...
| ------------ | ------------------ | ------------------- |
| Вывод в SQL Azure | <p>Служба фабрики данных использует сведения строки подключения из связанной службы INPUT для создания параметров веб-службы с именами "Имя сервера базы данных", "Имя базы данных", "Имя учетной записи пользователя сервера", "Пароль учетной записи пользователя сервера". Обратите внимание, что эти имена по умолчанию необходимо использовать для параметров веб-службы в Студии машинного обучения Microsoft Azure.</p><p>Если модуль чтения и модуль записи SQL Azure в модели машинного обучения Azure имеют упомянутые выше одинаковые параметры веб-службы, то все в порядке. Если они не используют одни и те же параметры веб-службы, например, если модуль записи SQL Azure использует имена параметров "Имя сервера базы данных1", "Имя базы данных1", "Имя учетной записи пользователя сервера1", "Пароль учетной записи пользователя сервера1" (с цифрой 1 в конце), то необходимо передать значения для этих параметров веб-службы OUTPUT в раздел webServiceParameters JSON действия. </p><p>Вы можете передавать значения для любых других параметров веб-службы с помощью раздела webServiceParameters в JSON действия.</p> | <p>Служба фабрики данных использует сведения строки подключения из связанной службы OUTPUT для создания параметров веб-службы с именами "Имя сервера базы данных", "Имя базы данных", "Имя учетной записи пользователя сервера", "Пароль учетной записи пользователя сервера". Обратите внимание, что необходимо использовать эти имена по умолчанию для параметров веб-службы в Студии машинного обучения Microsoft Azure. </p><p>Вы можете передавать значения для любых других параметров веб-службы с помощью раздела webServiceParameters в JSON действия. <p>Входной большой двоичный объект будет использоваться в качестве расположения входных данных.</p> |
