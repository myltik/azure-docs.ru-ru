...
**Связанная служба HDFS** В этом примере используется проверка подлинности Windows. Различные типы проверки подлинности, которые можно использовать, см. в разделе [Связанная служба HDFS](#hdfs-linked-service-properties).
...
**Входной набор данных HDFS** Этот набор данных ссылается на папку HDFS DataTransfer/UnitTest. Конвейер копирует все файлы из этой папки в место назначения.
...
Дополнительные сведения о настройке учетных данных для локальной системы HDFS см. в разделе [Настройка учетных данных и безопасность](data-factory-move-data-between-onprem-and-cloud.md#setting-credentials-and-security).
