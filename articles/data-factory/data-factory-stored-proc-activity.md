...
2. В колонке **Создать фабрику данных** введите **SProcDF** в поле "Имя". Имена фабрики данных Azure являются глобально уникальными. Для успешного создания фабрики добавьте свое имя в качестве префикса имени фабрики.
...
3. Замените **servername** именем своего сервера базы данных SQL Azure, **databasename** — именем базы данных, в которой вы создали таблицу и хранимую процедуру, **username@servername** — учетной записью пользователя с доступом к базе данных, а **password** — паролем для учетной записи пользователя.
4. Чтобы развернуть эту службу, нажмите кнопку **Развернуть** на панели команд.

### Создание выходного набора данных
1. Щелкните **Новый набор данных** на панели команд и выберите **Azure SQL**.
...
1. Щелкните **... (многоточие)** на панели команд, затем щелкните **Создать конвейер**.
2. Скопируйте и вставьте следующий фрагмент JSON. Имя хранимой процедуры **StoredProcedureName** было установлено в **sp\_sample**. Имя и регистр параметра **DateTime** должны совпадать с именем и регистром параметра в определении хранимой процедуры.
