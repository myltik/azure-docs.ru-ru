...
### Что такое действие
Действия определяют то, что нужно выполнить с вашими данными. Каждое действие принимает ноль или несколько [наборов данных](data-factory-create-datasets.md) на входе и создает один или несколько наборов данных на выходе. **Действие — это единица управления в фабрике данных Azure.**

Действие копирования, например, может использоваться для управления копированием данных из одного набора данных в другой. Аналогичным образом можно использовать действие Hive, которое выполнит запрос Hive к кластеру Azure HDInsight для преобразования или анализа данных. Фабрика данных Azure предоставляет широкий выбор действий для [преобразования и анализа данных](data-factory-data-transformation-activities.md) и [перемещения данных](data-factory-data-movement-activities.md). Кроме того, вы можете создать действие .NET для выполнения собственного кода.
...
--- | ----------- | --------
name | Имя действия или конвейера. Укажите имя операции, которую должно выполнять действие или конвейер.<br/><ul><li>Максимальное количество символов — 260.</li><li>Имя должно начинаться с цифры, буквы или знака подчеркивания (\_).</li><li>Не разрешается использовать следующие символы: . , + ? / < > * % & : \\</li></ul>| Да
...
inputs | Входные таблицы, используемые действием.<p>// одна входная таблица<br/>"inputs": [ { "name": "inputtable1" } ]</p><p>// две входные таблицы<br/>"inputs": [ { "name": "inputtable1" }, { "name": "inputtable2" } ]</p>| Да
outputs| Выходные таблицы, используемые действием.<p>одна выходная таблица<br/>"outputs": [ { "name": “outputtable1” } ]</p><p>//две выходные таблицы<br/>"outputs": [ { "name": “outputtable1” }, { "name": “outputtable2” } ]</p>| Да
linkedServiceName | Имя связанной службы, используемой действием. <p>В ряде случаев для действий требуется указать связанную службу, через которую осуществляется подключение к вычислительной среде.</p>| Да — для действия HDInsight и действия пакетной оценки показателей машинного обучения Azure<p>Нет — для всех остальных действий</p>
...
start | Дата и время запуска конвейера. Задается в [формате ISO](http://en.wikipedia.org/wiki/ISO_8601). Например, 2014-10-14T16:32:41Z. Свойства start и end определяют период активности конвейера. Выходные срезы данных создаются только в течение этого периода. | Нет<p>Период активности конвейера можно также задать с помощью командлета Set-AzureDataFactoryPipelineActivePeriod</p>
End | Дата и время окончания работы конвейера. Не является обязательным и задается в формате ISO. Например, 2014-10-14T17:32:41Z.<p>Если свойство не задано, дата и время окончания рассчитываются по следующей формуле: значение start + 48 часов. Чтобы конвейер работал в течение неопределенного срока, задайте для свойства end значение 9999-09-09.</p>| Нет
isPaused | Если задано значение true, конвейер работать не будет. Значение по умолчанию — false. Это свойство можно использовать для включения и отключения конвейера. | Нет
...
-------- | ----------- | -------------- | ---------------
concurrency | Целое число<p>Максимальное значение — 10</p> | 1 | Количество одновременных запусков действия.<p>Определяет количество параллельных запусков одного действия для обработки разных срезов. Например, если действию необходимо обработать большой набор доступных данных, то высокое значение этого свойства ускорит обработку данных.</p> 
...
3. Щелкните плитку **Создание и развертывание** в группе связанных элементов **Сводка**. 
...
- Изучите статью [Планирование и выполнение в фабрике данных Azure](data-factory-scheduling-and-execution.md).  
...
## Отправить отзыв
Мы будем очень благодарны за ваши отзывы к этой статье. Пожалуйста, уделите несколько минут тому, чтобы отправить свой отзыв [по электронной почте](mailto:adfdocfeedback@microsoft.com?subject=data-factory-create-pipelines.md).
