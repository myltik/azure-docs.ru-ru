...
Фабрика данных может иметь один или несколько конвейеров. Конвейеры — это логические группы действий, которые вместе отвечают за выполнение задачи. Действия в конвейере определяют операции с данными. Например, действие копирования позволяет скопировать данные из локальной базы данных SQL Server в хранилище BLOB-объектов Azure. Затем используйте действие Hive для запуска сценария Hive в кластере Azure HDInsight, чтобы обработать данные из хранилища BLOB-объектов и преобразовать их в выходные. Наконец, используйте второе действие копирования, чтобы скопировать выходные данные в хранилище данных SQL Azure, на основе которого строятся решения по отчетам бизнес-аналитики. 
...
### <a name="json-example-for-chaining-two-copy-activity-in-a-pipeline"></a>Пример JSON для объединения в цепочку двух действий копирования в конвейере
...
Выходной набор данных создается каждый час в момент начала и момент окончания конвейера. Таким образом, в этом конвейере создаются пять срезов данных — по одному для каждого периода действия (с 12:00 до 01:00, с 01:00 до 02:00, с 02:00 до 03:00, с 03:00 до 04:00, с 04:00 до 05:00). 


В примере действие копирования CopyActivity2 может иметь дополнительные входные данные, например набор данных Dataset3, но нужно указать набор Dataset2 в качестве входных данных, чтобы действие копирования CopyActivity2 не запускалось, пока не завершится действие CopyActivity1. Например:
...
- [Создание конвейера с помощью действия перемещения данных](data-factory-copy-data-from-azure-blob-storage-to-sql-database.md)

После создания и развертывания конвейера вы можете управлять конвейерами и отслеживать их с помощью колонок на портале Azure или приложения по мониторингу и управлению. Пошаговые инструкции представлены в указанных ниже статьях. 

- [Мониторинг конвейеров и управление ими с помощью колонок портала Azure](data-factory-monitor-manage-pipelines.md)
- [Мониторинг конвейеров и управление ими с помощью приложения по мониторингу и управлению](data-factory-monitor-manage-app.md)
