...
scheduler| Свойство scheduler используется, чтобы задавать расписание выполнения действия. Для него предусмотрен тот же набор подсвойств, что и для [свойства availability в наборе данных](data-factory-create-datasets.md#Availability). | Нет | 
...
| expirationTime | Период времени после создания конвейера, в течение которого тот остается действительным и подготовленным. Если на момент завершения этого периода у конвейера не будет активных, невыполненных или ожидающих выполнения прогонов, конвейер будет автоматически удален. | Нет | 
