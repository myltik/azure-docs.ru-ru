...
start | Дата и время запуска конвейера. Задается в [формате ISO](http://en.wikipedia.org/wiki/ISO_8601). Например, 2014-10-14T16:32:41Z. Свойства start и end определяют период активности конвейера. Выходные срезы данных создаются только в течение этого периода. | Нет<p>Если задано значение для свойства end, необходимо задать значение и для свойства start.</p><p>Для создания конвейера время начала и окончания можно оставить пустым, однако, если требуется задать активный период работы конвейера, следует задать оба значения.Если не указать время начала и окончания при создании конвейера, можно установить их позже с помощью командлета Set-AzureDataFactoryPipelineActivePeriod.</p>
End | Дата и время окончания работы конвейера. Не является обязательным и задается в формате ISO. Например, 2014-10-14T17:32:41Z.<p>Если свойство не задано, дата и время окончания рассчитываются по следующей формуле: значение start + 48 часов. Чтобы конвейер работал в течение неопределенного срока, задайте для свойства end значение 9999-09-09.</p>| Нет <p>Если задать значение для свойства start, необходимо задать значение и для свойства end.</p><p>См. примечания для свойства **start**.</p>
