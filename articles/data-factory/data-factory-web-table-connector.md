...
**Входной набор данных WebTable**. Если для параметра **external** задать значение **true** и указать политику **externalData** (необязательно), фабрика данных будет считать эту таблицу внешней и не созданной в результате какого-либо действия в фабрике данных.
...
Сейчас для действия копирования, когда источник относится к типу **WebSource**, дополнительные свойства не поддерживается.
...
4.  В диалоговом окне **Доступ к веб-содержимому** выберите соответствующий **URL-адрес** и **тип проверки подлинности**, а затем нажмите кнопку **Подключить**.
