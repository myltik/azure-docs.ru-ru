...
- **Драйвер Майкрософт для Oracle** включен в комплект шлюза управления данными, начиная с версии 2.7. Таким образом, для подключения к Oracle не нужно устанавливать ничего, кроме шлюза. Поддерживаются базы данных Oracle версии 10g Release 2 или более поздних версий. Чтобы упростить настройку среды, **рекомендуется** использовать этот драйвер.

    > [!NOTE]
    > В настоящее время драйвер Майкрософт для Oracle поддерживает копирование данных из базы данных Oracle, но не поддерживает запись к базу данных Oracle.
    >
