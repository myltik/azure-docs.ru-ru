...
Соединитель Azure Data Lake Store поддерживает аутентификацию на основе **субъекта-службы** и аутентификацию на основе **учетных данных пользователя**. Рекомендуется использовать первый тип, особенно для запланированных заданий копирования данных, чтобы избежать истечения срока действия маркера во втором типе аутентификации. Сведения о настройках см. в разделе [Свойства связанной службы хранилища озера данных Azure](#azure-data-lake-store-linked-service-properties).
...
> [!NOTE]
> Сведения о настройках см. в разделе [Свойства связанной службы хранилища озера данных Azure](#azure-data-lake-store-linked-service-properties).
>
...
В следующей таблице содержится описание элементов JSON, которые относятся к связанной службе Azure Data Lake Store. Можно выбрать тип аутентификации: на основе **субъекта-службы** или на основе **учетных данных пользователя**.

| Свойство | Описание | Обязательно |
|:--- |:--- |:--- |
| type | Для свойства type следует задать значение **AzureDataLakeStore** | Да |
| dataLakeStoreUri | Указывает сведения об учетной записи хранилища озера данных Azure в следующем формате: **https://[имя_учетной_записи].azuredatalakestore.net/webhdfs/v1** или **adl://[имя_учетной_записи].azuredatalakestore.net/**. | Да |
...
При использовании аутентификации на основе субъекта-службы сначала необходимо зарегистрировать сущность приложения в Azure Active Directory (AAD) и предоставить ему доступ в Data Lake Store. Затем можно указать приведенные ниже свойства в фабрике данных Azure, используя соответствующие идентификатор приложения, ключ приложения и сведения о клиенте, чтобы скопировать данные в Data Lake Store или из него. Сведения о настройке хранилища и получении необходимой информации см. в статье [Аутентификация между службами в Data Lake Store с помощью Azure Active Directory](../data-lake-store/data-lake-store-authenticate-using-active-directory.md).

> [!IMPORTANT]
> Если вы используете мастер копирования для создания, для успешного перехода между папками обязательно назначьте субъекту-службе как минимум роль читателя в службе управления доступом (IAM) для учетной записи ADLS И как минимум разрешение на чтение и выполнение для корня ADLS ("/") и его дочерних элементов. Иначе может появиться ошибка "указанные учетные данные недействительны".
>
> Если создаете или обновляете новый субъект-службу из AAD, для его активации может потребоваться несколько минут. Если вы по-прежнему видите сообщение об ошибке "указанные учетные данные недействительны", сначала дважды проверьте конфигурацию субъекта-службы и списка управления доступом ADLS. Затем подождите некоторое время и повторите попытку.
>
...
Также для копирования данных в Data Lake Store или из него можно использовать аутентификацию на основе учетных данных пользователя, указав приведенные ниже свойства.
