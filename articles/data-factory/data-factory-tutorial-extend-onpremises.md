<properties 
	pageTitle="Пошаговое руководство. Копирование выходных данных в базу данных SQL Server (классический портал Azure)" 
	description="Данное пошаговое руководство представляет собой расширенную версию учебника. В нем используется редактор фабрики данных на классическом портале Azure, за счет чего конвейер копирует выходные данные в базу данных SQL Server."
...
# Пошаговое руководство. Копирование выходных данных в локальную базу данных SQL Server (классический портал Azure) 
...
Перед тем как приступить к пошаговому руководству, описанному в этом разделе, **необходимо** выполнить пошаговое руководство [Учебник. Перемещение и обработка файлов журналов с помощью фабрики данных](data-factory-tutorial.md).
...
2. [Создание связанной службы для локального сервера SQL Server](#create-sql-server-linked-service). На этом шаге вы сначала создадите на локальном компьютере SQL Server базу данных и таблицу, а затем создадите связанную службу **OnPremSqlLinkedService**:
...
4. [Мониторинг конвейера и просмотр результата](#monitor-pipeline). На этом шаге вы выполните мониторинг конвейеров, таблиц и срезов данных с помощью портала Azure.
...
1.	Создайте логический шлюз данных. На **портале Azure** щелкните **Связанные службы** в колонке **ФАБРИКА ДАННЫХ** для вашей фабрики.
...
1.	На **портале Azure** щелкните плитку **Автор и развертывание** в колонке **ФАБРИКА ДАННЫХ** для **LogProcessingFactory**.
...
3.	В сценарии JSON выполните следующее: 
	...
	3.	При использовании **проверки подлинности SQL:**
		...
		2.	Удалите последние две строки (свойства **username** и **password** JSON нужны только в том случае, если используется проверка подлинности Windows). 
		3.	Удалите **, (запятую)** в конце строки **gatewayName**.

		**При использовании аутентификации Windows:**
		1. Задайте значение **True** для **встроенной системы безопасности** в **connectionString**. Удалите **User ID=<username>;Password=<password>;** из connectionString. 
		2. В свойстве **username** укажите имя пользователя, который имеет доступ к базе данных. 
		3. В свойстве **password** укажите пароль для учетной записи пользователя.   
...
Теперь можно воспользоваться инструкциями, предоставленными в разделе **Мониторинг конвейеров** [основного учебника](data-factory-tutorial.md#monitor-pipelines), для мониторинга нового конвейера и срезов данных новой локальной таблицы ADF.
