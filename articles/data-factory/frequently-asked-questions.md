...
После выпуска версии 1 мы поняли, что клиенты хотят разрабатывать сложные гибридные сценарии интеграции данных, для которых требуются функции перемещения и обработки данных в облаке, в локальной среде и в облаке виртуальных машин. Эти требования вылились в необходимость передавать и обрабатывать данные в защищенных средах виртуальных сетей, а также развертывать вычислительные мощности по мере потребности.

Когда конвейеры данных стали важнейшим элементом в стратегии бизнес-аналитики, мы обнаружили также необходимость гибкого планирования для этих критичных действий, которое позволит обрабатывать дополнительные объемы данных и выполнять действия по наступлению событий. И наконец, при увеличении сложности оркестрации проявляется потребность поддерживать стандартные алгоритмы рабочих процессов, например ветвление, циклы и условную обработку.

В версии 2 вы сможете выполнить миграцию существующих пакетов SSID SQL в облако, что позволяет перенести готовые решения SSIS в службу Azure, управляемую через ADF с помощью новой функции "среды выполнения интеграции" (IR). Применив среду выполнения интеграции для служб SSIS в версии 2, вы сможете выполнять и создавать пакеты служб SSIS в облаке, управлять ими и отслеживать их.

### <a name="control-flow-and-scale"></a>Поток управления и масштабирование 
Чтобы поддерживать интеграцию разнообразных потоков и шаблонов интеграции в современных хранилищах данных, фабрика данных реализует новую гибкую модель конвейеров данных, которая теперь не привязана к временным рядам данных. В этом выпуске можно моделировать условные выражения и ветвления в потоке управления для конвейера данных, а также явным образом передавать параметры внутри потоков и между ними.

Теперь вы можете свободно создать поток любого стиля, удобного для вашего сценария интеграции данных, и запускать его по запросу или по расписанию. Стали возможными несколько распространенных потоков, которые раньше реализовать не удавалось.   
...
Дополнительные сведения см. в [руководстве по потокам управления](tutorial-control-flow.md).
...
Если вам, как опытному пользователю, нужен программный интерфейс, то в версии 2 вам понравится богатый набор пакетов SDK, которые позволяют создавать конвейеры, управлять ими и отслеживать их из любой удобной интегрированной среды разработки (IDE).
...
Параметры представляют собой пары "ключ — значение" в конфигурации только для чтения.  Параметры определяются в конвейере, а аргументы для них передаются во время выполнения из контекста выполнения, созданного по триггеру, или при ручном выполнении конвейера. Действия в конвейере используют значения параметров.
...
Связанная служба также является строго типизированным параметром, который содержит сведения о подключении к хранилищу данных или вычислительной среде. Эта сущность также доступна для ссылок и (или) повторного использования.

### <a name="control-flow"></a>Поток управления
Оркестрация действий в конвейере, которая включает организацию цепочек действий в последовательности, ветвление и определение параметров на уровне контейнера, аргументы для которых передаются во время вызова конвейера по запросу или из триггера. Также сюда входит передача пользовательского состояния и контейнеров зацикливания (то есть итераторы For-each).
