...
Чтобы подключить шлюз управления данными к базе данных DB2, начиная с версии шлюза 2.1, фабрика данных Azure предоставляет встроенный драйвер с поддержкой DB2 (SQLAM 9, 10 или 11), включая DB2 для LUW (Linux, Unix и Windows), DB2 для z/OS и DB2 для i (AS/400), поэтому вам больше не придется вручную устанавливать драйверы при копировании данных из DB2.
...
В этом примере показано, как скопировать данные из локальной базы данных DB2 в хранилище BLOB-объектов Azure. Однако данные можно **напрямую** копировать в любой указанный [здесь](data-factory-data-movement-activities.md#supported-data-stores) приемник. Это делается с помощью действия копирования в фабрике данных Azure.
