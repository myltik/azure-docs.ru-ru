...
| style | Указывает, в начале или в конце интервала делается срез.<ul><li>StartOfInterval</li><li>EndOfInterval</li></ul><p>Если для frequency задано значение Month, а для style — EndOfInterval, то срез данных будет производиться в последний день месяца. Если для style задано значение StartOfInterval, то срез осуществляется в первый день месяца.</p><p>Если для frequency задано значение Day, а для style — EndOfInterval, то срез создается в последний час дня.</p>Если для frequency задано значение Hour и задан стиль EndOfInterval, то срез создается в конце часа. Например, для периода с 13:00 до 14:00 срез создается в 14:00.</p> | Нет | EndOfInterval |
