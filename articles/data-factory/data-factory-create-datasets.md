...
* параметр linkedServiceName означает связанную службу типа AzureSqlDatabase, определение которой указано в фрагменте кода JSON ниже;
...
**structure** — это **необязательный** раздел, в котором определяется схема набора данных. Схема содержит коллекцию имен и типов данных для столбцов. Используйте раздел structure либо для указания сведений о типе при **преобразовании типов**, либо при **сопоставлении столбцов**. В приведенном ниже примере набор данных содержит три столбца: `slicetimestamp`, `projectname` и `pageviews` с типом String, String и Decimal соответственно.
...
* **Для структурированных источников данных**, в которых наряду с данными хранятся схема данных и сведения о типах (таких источников, как SQL Server, Oracle, таблица Azure и т. п.), раздел structure следует указывать только при сопоставлении исходных столбцов со столбцами в приемнике, если их имена различаются. 
