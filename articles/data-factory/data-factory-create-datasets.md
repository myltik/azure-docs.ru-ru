...
Создавая фабрику данных, вы создаете связанные службы, которые связывают хранилища данных с фабрикой данных. **Связанная служба** определяет информацию, которая нужна фабрике данных для **подключения** к хранилищу данных, например к учетной записи хранения Azure или базе данных SQL Azure. Связанная служба определяет механизм (адрес, протокол, схему проверки подлинности и т. д.) для доступа к хранилищу данных.

**Набор данных** в фабрике данных представляет структуры данных (большие двоичные объекты, таблицы SQL и т. п.) в этом хранилище данных. Набор данных можно указывать в конвейере как входные или выходные данные для действия в контейнере. Созданные наборы данных можно использовать для действий в конвейере. Например, набор данных можно указать как входной или выходной набор данных для действия HDInsightHive или действия копирования.

> [AZURE.NOTE] Если вы не знакомы с фабрикой данных Azure, изучите статью [Введение в фабрику данных Azure](data-factory-introduction.md), в которой содержится обзор службы фабрики данных Azure. Инструкции по созданию фабрики данных см. в статье [Начало работы с фабрикой данных Azure](data-factory-build-your-first-pipeline.md). В этих двух статьях содержатся общие сведения, необходимые для работы с этой статьей.
...
| type | Тип набора данных. Укажите один из типов, которые поддерживаются фабрикой данных Azure (например: AzureBlob, AzureSqlTable). <br/><br/>Подробнее см. в разделе [Тип набора данных](#Type). | Да | — |
| structure | Схема набора данных.<br/><br/>Подробнее см. в разделе [Структура набора данных](#Structure). | Нет | Нет данных |
...
В разделе **availability** в определении набора данных определяется интервал обработки (ежечасно, ежедневно, еженедельно и т. п.) или модель среза для набора данных. Дополнительные сведения о создании срезов данных и модели зависимостей см. в разделе [Планирование и выполнение](data-factory-scheduling-and-execution.md).
...
|minimumRows | Проверяет, содержат ли данные в **базе данных Azure SQL** или **таблице Azure** минимально необходимое количество строк. | <ul><li>База данных SQL Azure</li><li>Таблица Azure</li></ul> | Нет | Нет данных
...
Внешними считаются такие наборы данных, которые не создаются запущенным конвейером в фабрике данных. Если набор данных помечен как внешний (**External**), вы можете изменить доступность соответствующих срезов данных с помощью политики **ExternalData**.
...
> [AZURE.IMPORTANT] Наборы данных с заданной областью поддерживаются только с однократными конвейерами (для **pipelineMode** задано значение **OneTime**). Подробные сведения см. в разделе, посвященном [однократным конвейерам](data-factory-scheduling-and-execution.md#onetime-pipeline).
