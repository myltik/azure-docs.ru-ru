...
3. Select-AzureSubscription <subscription name> — выбор нужной подписки. Используйте ту же подписку, что применялась для создания фабрики данных на портале предварительной версии Azure.
...
## Проблема: не удается запустить диспетчер учетных данных с портала Azure
Чтобы обеспечить безопасность при настройке или обновлении связанной службы SQL Server через портал Azure, запускается диспетчер учетных данных. Для запуска требуется Internet Explorer или другой веб-браузер, совместимый с Microsoft ClickOnce. Если с другими браузерами возникают проблемы, используйте Internet Explorer.
...
2. Попытайтесь подключиться к экземпляру SQL Server с учетными данными, которые вы указали в SQL Server Management Studio (SSMS) на портале Azure.
...
1. Запустите диспетчер конфигурации шлюза управления данными на компьютере, на котором установлен шлюз. Убедитесь, что для параметра **Имя шлюза** задано имя логического шлюза на **портале Azure**, для параметра **Состояние ключа шлюза** — значение **Registered** (зарегистрировано), а для параметра **Состояние службы** — **Started** (работает). 
...
При выполнении настраиваемого действия фабрика данных Azure может записать выходные данные из кластера HDInsight и сохранить их в контейнере хранилища *adfjobs* в вашей учетной записи хранения больших двоичных объектов Azure. При возникновении ошибки ознакомьтесь с текстом в выходном текстовом файле **stderr**. Такие файлы можно прочитать непосредственно на портале Azure, используя веб-браузер. Кроме того, вы можете получить непосредственный доступ к файлам в контейнере хранилища больших двоичных объектов Azure, используя инструменты обозревателя хранилища.
...
1.  На портале Azure щелкните группу **Обзор** и выберите фабрику данных.
...
### Пошаговое руководство. Устранение ошибки обработки Pig и Hive с помощью портала Azure
