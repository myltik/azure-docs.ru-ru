...
3. Select-AzureRmSubscription <subscription name> — выбор нужной подписки. Используйте ту же подписку, что применялась для создания фабрики данных на классическом портале Azure.
...
Запустите **диспетчер конфигурации шлюза управления данными** на компьютере шлюза и используйте вкладку **Устранение неполадок** для проверки подключения к SQL Server с компьютера шлюза. Дополнительные сведения см. в статье [Устранение неполадок шлюза](data-factory-move-data-between-onprem-and-cloud.md#gateway-troubleshooting).
...
Действия по устранению неполадок с копированием в локальное хранилище данных и из него с помощью шлюза управления данными см. в статье [Устранение неполадок шлюза](data-factory-move-data-between-onprem-and-cloud.md#gateway-troubleshooting).
...
Подробные действия см. в статье [Debug a pipeline with custom activity (Отладка конвейера с помощью настраиваемого действия)](data-factory-use-custom-activities.md#debug-the-pipeline).
...
Сведения см. в статье [Monitor Data Factory pipelines using Azure PowerShell (Мониторинг конвейеров фабрики данных с помощью Azure PowerShell)](data-factory-build-your-first-pipeline-using-powershell.md#monitor-pipeline).
