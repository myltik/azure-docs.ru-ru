...
description: "В этой статье сравниваются версии 1 и 2 фабрики данных Azure."
...
---
# <a name="compare-azure-data-factory-versions-1-and-2"></a>Сравнение фабрики данных Azure версий 1 и 2
В этой статье сравниваются версии 2 и 1 фабрики данных Azure. См. дополнительные сведения о фабрике данных [версии 1](v1/data-factory-introduction.md) и [версии 2](introduction.md).
...
| Перенос гибридных данных и диспетчеризация действий | Прежнее название — [шлюз управления данными](v1/data-factory-data-management-gateway.md), который поддерживал перемещение данных между локальной и облачной средами. Теперь этот компонент называется Integration Runtime.| Шлюз управления данными теперь называется Integration Runtime (Self-hosted). Он обеспечивает те же возможности, что и в версии 1. <br/><br/> В версии 2 Integration Runtime служб Azure SSIS также поддерживает развертывание и выполнение пакетов SQL Server Integration Services (SSIS) в облаке. Дополнительные сведения см. в описании [Integration Runtime](concepts-integration-runtime.md).|
| Параметры | Недоступно | Параметры представлены парами "ключ-значение" параметров конфигурации с доступом только на чтение, которые определены в конвейерах. Вы можете передавать аргументы для параметров конвейера при выполнении конвейера вручную. Если вы используете триггер планировщика, он также может передавать значения для параметров. Действия в конвейере используют значения параметров.  |
...
| Запуски конвейера | Недоступно | Экземпляр выполнения конвейера. Например, у вас есть конвейер, который выполняется в 8:00, 9:00 и 10:00. В этом случае выполняются три отдельных запуска конвейера. Каждый запуск конвейера имеет уникальный идентификатор запуска конвейера, однозначно его определяющий. Запуск конвейера обычно создается путем передачи аргументов в параметры, определенные в конвейерах. |
| Выполнения действий | Недоступно | Экземпляр выполнения действий в конвейере. | 
| Выполнения триггеров | Недоступно | Экземпляр выполнения триггера. Дополнительные сведения см. в описании [триггеров](concepts-pipeline-execution-triggers.md). |
...
[Действие ForEach](control-flow-for-each-activity.md) определяет повторяющийся поток управления в конвейере. Это действие используется для выполнения итерации коллекции и выполняет указанные в цикле действия. Реализация цикла этого действия аналогична структуре цикла Foreach на языках программирования. 
...
### <a name="invoke-a-pipeline-from-another-pipeline"></a>Вызов конвейера из другого конвейера
...
[Действие ожидания](control-flow-wait-activity.md) | Конвейер ожидает (приостанавливается) в течение указанного периода времени.

## <a name="deploy-ssis-packages-to-azure"></a>Развертывание пакетов служб SSIS в Azure 
Если вы хотите переместить рабочие нагрузки служб SSIS в облако, создайте фабрику данных версии 2 и подготовьте среду выполнения интеграции Azure SSIS IR. Azure SSIS IR — это полностью управляемый кластер виртуальных машин (узлов) Azure, выделенный для выполнения пакетов служб SSIS в облаке. Подготовив IR Azure SSIS, вы сможете использовать те же средства, с которыми вы работали при развертывании пакетов служб SSIS для служб SSIS в локальной среде. Например, вы можете использовать SQL Server Data Tools (SSDT) ​​или SQL Server Management Studio (SSMS) для развертывания пакетов служб SSIS в этой среде выполнения в Azure. Пошаговые инструкции см. в руководстве [Развертывание пакетов служб интеграции SQL Server (SSIS) в Azure](tutorial-deploy-ssis-packages-azure.md). 

## <a name="flexible-scheduling"></a>Гибкое планирование
В фабрике данных версии 2 вам не нужно определять расписание обеспечения доступности для наборов данных. Вы можете определить ресурс триггера, который будет планировать работу конвейеров, используя парадигму планировщика часов. Вы также можете передавать параметры в конвейеры из триггера для использования гибкой модели планирования и выполнения. В фабрике данных версии 2 для конвейеров не предусмотрено выполнение операций в рамках временных окон. Такие базовые значения фабрики данных версии 1, как startTime, endTime и isPaused, больше не поддерживаются в фабрике данных версии 2. Дополнительные сведения см. в статье [Выполнение конвейера и триггеры в фабрике данных Azure](concepts-pipeline-execution-triggers.md).
...
Версия 2 поддерживает создание кластера HDInsight Spark по запросу. Чтобы создать кластер Spark по запросу, укажите тип кластера как Spark в определении связанной службы HDInsight по запросу. Затем можно настроить действие Spark в конвейере для использования этой связанной службы. При выполнении действия служба фабрики данных автоматически создает кластер Spark. Дополнительные сведения см. в следующих статьях:
...
В версии 1 вы создаете код действия DotNet (настраиваемого) в отдельном проекте библиотеки классов .Net с помощью класса, который реализует метод Execute в интерфейсе IDotNetActivity. По этой причине пользовательский код нужно писать с помощью .Net Framework 4.5.2 и выполнять на узлах пула пакетной службы Azure под управлением Windows. 

В настраиваемых действиях версии 2 не нужно реализовывать интерфейс .Net. Теперь все команды, скрипты и пользовательский код можно компилировать и выполнять в виде исполняемого файла. 
...
- **Командлеты PowerShell** — обновлены для использования с версией 2. Названия командлетов для версии 2 содержат текст **DataFactoryV2**. Например: Get-AzureRmDataFactoryV2. 
- **Пакет SDK для Python** — с предыдущей версией не использовался.
- **REST API** — обновлен для использования с версией 2. 

Пакеты SDK, которые обновлены для использованием с версией 2, не обладают обратной совместимостью с клиентами версии 1. 

## <a name="authoring-experience"></a>Среда разработки
Фабрика данных версии 1 позволяет создавать конвейеры, используя редактор фабрики данных на портале Azure. Сейчас фабрика данных версии 2 поддерживает создание фабрик данных только программными средствами (пакет SDK для .NET, REST API, PowerShell, Python и т. д.). Пользовательский интерфейс для этих целей пока отсутствует.  Фабрика данных версии 1 также поддерживает пакеты SDK, REST и PowerShell для разработки.
