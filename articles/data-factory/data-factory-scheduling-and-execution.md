...
Несколько операций копирования можно выполнить друг за другом последовательно или упорядоченно. Предположим, в конвейере есть два действия копирования, CopyActivity1 и CopyActivity2, со следующими наборами входных и выходных данных.
...
Действие CopyActivity2 будет выполнено только в том случае, если действие CopyActivity1 было выполнено успешно и набор данных Dataset2 доступен.

В приведенном выше примере действие CopyActivity2 может иметь другие входные данные, например набор данных Dataset3, но необходимо также указать набор Dataset2 в качестве входных данных, чтобы действие CopyActivity2 не запускалось, пока не завершится действие копирования CopyActivity1. Например:
...
В разделе [Функции и системные переменные фабрики данных](data-factory-functions-variables.md) приведен список функций и системных переменных, поддерживаемых фабрикой данных Azure.
