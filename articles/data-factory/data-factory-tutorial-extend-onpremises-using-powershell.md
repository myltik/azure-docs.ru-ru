<properties 
	pageTitle="Пошаговое руководство. Копирование выходных данных в базу данных SQL Server (Azure PowerShell)" 
...
# Пошаговое руководство. Копирование выходных данных в локальную базу данных SQL Server (Azure PowerShell)
...
1. [Создание шлюза управления данными](#create-data-management-gateway). Шлюз управления данными — это агент клиента, который предоставляет доступ к локальным источникам данных в вашей организации из облака. Шлюз обеспечивает передачу данных между локальным сервером SQL Server и хранилищами данных Azure.	
...
2. [Создание связанной службы SQL Server](#create-sql-server-linked-service). На этом шаге вы сначала создадите на локальном компьютере SQL Server базу данных и таблицу, а затем создадите связанную службу **OnPremSqlLinkedService**:
3. [Создание набора данных и конвейера](#create-dataset-and-pipeline). На этом шаге вы создадите таблицу **MarketingCampaignEffectivenessOnPremSQLTable** и конвейер **EgressDataToOnPremPipeline**. 

4. [Мониторинг конвейера](#monitor-pipeline). На этом шаге вы выполните мониторинг конвейеров, таблиц и срезов данных с помощью классического портала Azure.


## Создание шлюза управления данными
...
## Создание набора данных и конвейера
...
Теперь можно воспользоваться инструкциями, предоставленными в разделе [Мониторинг конвейеров](data-factory-tutorial-using-powershell.md#monitor-pipelines), для мониторинга нового конвейера и срезов данных новой локальной таблицы ADF.
