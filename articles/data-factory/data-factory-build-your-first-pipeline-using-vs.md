...
2.	В колонке **Создание фабрики данных** введите **DataFactoryMyFirstPipeline** в поле «Имя».
	...
	![Колонка "Создание фабрики данных"](./media/data-factory-build-your-first-pipeline-using-vs/new-data-factory-blade.png)
...
На вашем компьютере должны быть установлены следующие компоненты: Visual Studio 2013. Скачайте пакет SDK Azure для Visual Studio 2013. Перейдите на [страницу загрузки Azure](http://azure.microsoft.com/downloads/) и нажмите кнопку **Установить VS 2013** в разделе **.NET**.
...
4. В обозревателе решений щелкните правой кнопкой мыши **Связанные службы**, выберите команду **Добавить**, а затем — **Новый элемент**.      
5. В диалоговом окне **Добавление нового элемента** выберите в списке **Связанные службы хранилища Azure** и нажмите кнопку **Добавить**. 
...
1. В **обозревателе решений** щелкните правой кнопкой мыши **Связанные службы**, выберите команду **Добавить**, а затем — **Новый элемент**.
...
2. Выберите в списке **Хранилище BLOB-объектов Azure** и нажмите кнопку **Добавить**. 
...
1. В **обозревателе решений** щелкните правой кнопкой мыши **Конвейеры**, выберите команду **Добавить**, а затем — **Новый элемент**. 
2. Выберите в списке **Конвейер преобразования Hive** и нажмите кнопку **Добавить**. 
...
19. Щелкните **раскрывающийся список** на **панели инструментов фабрики данных**, чтобы просмотреть все фабрики данных в подписке Azure. Если вы видите диалоговое окно **Вход в Visual Studio**, сделайте вот что: 
	20. Введите **реквизиты учетной записи** (**адрес электронной почты и пароль**), связанной с подпиской Azure, в которой требуется создать фабрику данных, и нажмите кнопку **Войти**.
...
23. Состояние публикации отображается в окне **Список задач фабрики данных**, представленном на рисунке выше. Убедитесь, что публикация выполнена успешно.


## Использование обозревателя серверов для просмотра сущностей фабрики данных

1. В **Visual Studio** выберите меню **Вид** и нажмите кнопку **Обозреватель серверов**.
2. В обозревателе серверов разверните узлы **Azure** и **Фабрика данных**. Если отображается окно **Вход в Visual Studio**, введите реквизиты **учетной записи**, связанной с подпиской Azure, и нажмите кнопку **Продолжить**. Введите свой **пароль** и щелкните **Войти**. Visual Studio пытается получить сведения обо всех фабриках данных Azure в подписке. Вы увидите состояние операции в окне **Список задач фабрики данных**.

	![Обозреватель серверов](./media/data-factory-build-your-first-pipeline-using-vs/server-explorer.png)
3. Щелкните фабрику данных правой кнопкой мыши и выберите **Экспорт фабрики данных в новый проект**, чтобы создать проект Visual Studio на ее основе.
...
1. Щелкните меню **Сервис** и выберите пункт **Расширения и обновления**.
2. Выберите в панели слева пункт **Обновления**, а затем — **Коллекции Visual Studio**.
