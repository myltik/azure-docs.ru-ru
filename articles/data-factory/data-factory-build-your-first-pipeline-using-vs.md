...
Учитывайте следующее определение JSON для связанной службы хранилища Azure, чтобы указать **connectionString** с разными значениями accountname и accountkey в зависимости от среды (среда разработки, среда тестирования или производственная среда), в которой выполняется развертывание сущностей фабрики данных. Это можно сделать с помощью отдельного файла конфигурации для каждой среды.
...
1. Щелкните правой кнопкой мыши фабрику данных в решении Visual Studio, наведите указатель на пункт **Добавить** и щелкните **Новый элемент**.
2. Выберите пункт **Конфигурация** в списке установленных шаблонов в левой части окна, затем выберите **Файл конфигурации**, введите **имя** файла конфигурации и нажмите кнопку **Добавить**.
...
3. Добавьте параметры конфигурации и их значения в формате, показанном ниже.
	...
	В этом примере настраивается свойство connectionString связанной службы хранилища Azure и связанной службы Azure SQL. Обратите внимание, что для указания имени используется синтаксис [JsonPath](http://goessner.net/articles/JsonPath/).
...
2. Выберите существующую фабрику данных или укажите значения для создания новой фабрики данных на странице **Настройка фабрики данных**, а затем нажмите кнопку **Далее**.   
3. На странице **Публикация элементов** появится раскрывающийся список с доступными конфигурациями для поля **Выбор конфигурации развертывания**.

	![Выбор файла конфигурации](./media/data-factory-build-your-first-pipeline-using-vs/select-config-file.png)

4. Выберите **файл конфигурации**, который вы хотите использовать, и нажмите кнопку **Далее**.
5. Убедитесь, что на странице **Сводка** отображается имя JSON-файла, и нажмите кнопку **Далее**. 
6. По завершении операции развертывания нажмите кнопку **Готово**. 
