...
| password |Пароль для пользователя Пометьте это поле в качестве SecureString. |Да (если используется обычная проверка подлинности) |
...
> [!TIP]
> При указании запроса SQL обратите внимание на формат даты и времени. Например: `$$Text.Format('SELECT * FROM Account WHERE LastModifiedDate >= {{ts\\'{0:yyyy-MM-dd HH:mm:ss}\\'}} AND LastModifiedDate < {{ts\\'{1:yyyy-MM-dd HH:mm:ss}\\'}}', <datetime parameter>, <datetime parameter>)`
