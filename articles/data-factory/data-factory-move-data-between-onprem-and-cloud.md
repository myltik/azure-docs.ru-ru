...
Дополнительную информацию о настройке учетных данных см. в разделе [Настройка учетных данных и безопасность](#setting-credentials-and-security)
...
3. Щелкните стрелку рядом с надписью **Тип** и выберите **Служба хранилища Azure**.
4. В разделе **Тип** появятся новые поля: **Имя учетной записи** и **Ключ учетной записи**. 
3. В поле **Имя учетной записи** введите имя вашей учетной записи хранения Azure.
4. В поле **Ключ учетной записи** введите ключ вашей учетной записи хранения Azure. 
...
В этом шаге вы будете использовать портал Azure для мониторинга фабрики данных Azure. Вы также можете использовать командлеты PowerShell для мониторинга наборов данных и конвейеров. Дополнительную информацию о мониторинге см. в статье [Мониторинг конвейеров и управление ими](monitor-manage-pipelines.md).
...
14. (Необязательное действие) Щелкните **Конвейеры**, а затем — **ADFTutorialOnPremDF** и детализируйте входные таблицы (**Использовано**) или выходные таблицы (**Выполнено**).
...
1.	Разработчик создает шлюз для фабрики данных Azure, используя [портал Azure](http://portal.azure.com) или [командлет PowerShell](https://msdn.microsoft.com/library/dn820234.aspx). 
...
4.	При копировании данных в локальную базу данных SQL Server из базы данных Azure SQL (или наоборот) убедитесь в следующем.	
	- 	Брандмауэр на компьютере шлюза разрешает исходящие подключения TCP по **TCP**-порту **1433**.
...
## Отправить отзыв
Мы будем весьма признательны за ваш отзыв об этой статье. Отправьте его [по электронной почте](mailto:adfdocfeedback@microsoft.com?subject=data-factory-move-data-between-onprem-and-cloud.md).
