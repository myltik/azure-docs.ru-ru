...
Фабрика данных Azure позволяет легко перемещать данные между [поддерживаемыми локальными хранилищами данных и облачными хранилищами данных](data-factory-data-movement-activities.md#supported-data-stores) с помощью **шлюза управления данными**. Шлюз управления данными — это клиентский агент, который устанавливается на локальном компьютере. Он позволяет копировать данные между локальными и облачными хранилищами данных. Шлюз можно установить на том же компьютере, на котором размещается источник данных, или на другом компьютере. Важно, чтобы шлюз имел доступ к источнику данных.

## Поток данных при использовании шлюза управления данными.
Если в конвейере обработки данных с помощью [действия копирования](data-factory-data-movement-activities.md) вы передаете локальные данные в облако для дальнейшей обработки или экспортируете готовые данные из облака обратно в локальное хранилище, такое действие копирования для двусторонней передачи данных использует шлюз управления данными. Ниже показана обобщенная схема потока данных и этапы копирования с использованием шлюза: ![Поток данных при использовании шлюза](./media/data-factory-move-data-between-onprem-and-cloud/data-flow-using-gateway.png)

1.	Разработчик создает новый **шлюз** для фабрики данных Azure, используя [портал Azure](https://portal.azure.com) или [командлет PowerShell](https://msdn.microsoft.com/library/dn820234.aspx). Этот этап включает создание логического шлюза в серверной части, установку физического шлюза на локальном компьютере и связывание этих компонентов с помощью ключа.  
2.	Разработчик определяет новую **связанную службу** для локального хранилища данных и задает имя шлюза, используемого с хранилищем данных. При настройке данных связанной службы разработчик указывает учетные данные для доступа к локальному хранилищу данных.  
3.	Перед сохранением учетных данных в облаке шлюз **шифрует учетные данные**, используя связанный с этим шлюзом сертификат, предоставленный разработчиком.
...
5.	Шлюз **расшифровывает учетные данные** с помощью того же сертификата, после чего подключается с помощью этих учетных данных к локальному хранилищу данных.
6.	Шлюз **копирует данные** из локального хранилища в облачное или наоборот в зависимости от настроек действия копирования в конвейере данных. Примечание: на этом этапе шлюз напрямую взаимодействует с облачной службой хранилища (например, с хранилищем BLOB-объектов Azure, SQL Azure и т. д) через защищенный канал (HTTPS). 

В этой статье содержатся подробные сведения об использовании шлюза управления данными. Сначала давайте рассмотрим простой пример использования шлюза для перемещения данных из локальной базы данных SQL Server в большой двоичный объект Azure.
...
3. В колонке **Настройка** щелкните **Установить непосредственно на этот компьютер**. Это позволит скачать пакет установки для шлюза, а также установить, настроить и зарегистрировать шлюз на компьютере.
	...
	>  
	> То же самое (установку расширения) необходимо сделать и в Firefox. Нажмите кнопку **Открыть меню** на панели инструментов (**три горизонтальные линии** в правом верхнем углу), нажмите кнопку **Надстройки**, введите ClickOnce в строке поиска, а затем выберите и установите одно из расширений ClickOnce.
...
На этом этапе вы создадите две связанные службы: **AzureStorageLinkedService** и **SqlServerLinkedService**. Служба **SqlServerLinkedService** связывает с фабрикой данных локальную базу данных SQL Server, а служба **AzureStorageLinkedService** — хранилище BLOB-объектов Azure. Далее это руководство поможет вам создать конвейер, который позволит копировать данные из локальной базы данных SQL Server в службу хранилища больших двоичных объектов Azure.
...
3.	В **редакторе JSON** выполните следующие действия. 
	...
	2. При использовании проверки подлинности Windows:
		...
		3. Укажите имя пользователя и пароль в свойствах **userName** и **password**.  
            ...
            > [AZURE.NOTE] При использовании проверки подлинности Windows (IntegratedSecurity=true) имя пользователя и пароль можно не указывать. Если эти свойства не указаны, шлюз управления данными использует учетные данные пользователя, который вошел в компьютер шлюза для доступа к базе данных. Если вы хотите, чтобы шлюз обращался к базе данных под другими учетными данными, укажите соответствующее имя пользователя и пароль.
	...
	4. При использовании проверки подлинности SQL:
		...
		Учетные данные будут **зашифрованы** с помощью сертификата, который принадлежит службе фабрики данных. Если вы хотите использовать вместо него сертификат, связанный со шлюзом управления данными, см. раздел [Безопасная установка учетных данных](#set-credentials-and-security).
...
2.	Замените сценарий JSON в области справа на следующий текст: 
	...
	- для параметра **linkedServiceName** установлено значение **AzureStorageLinkedService** (вы создали эту связанную службу на этапе 2);
...
Следующие разделы содержат подробные сведения о шлюзе управления данными.
...
1.	Поддерживаемые **операционные системы**: Windows 7, Windows 8/8.1, Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2. В настоящее время установка шлюза управления данными на контроллере домена не поддерживается.
...
### Установка
В примере выше показано, как загрузить и запустить MSI-файл для шлюза управления данными.

Вы также можете установить шлюз управления данными, скачав пакет MSI в [Центре загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=39717).

Пакет MSI также может использоваться для обновления имеющегося шлюза управления данными до последней версии с сохранением всех параметров.
...
1. Запустите Windows **PowerShell** на компьютере шлюза с правами администратора (**Запуск от имени администратора**). 
...
Описанные здесь рекомендации преимущественно касаются двух брандмауэров: **корпоративного брандмауэра**, работающего на центральном корпоративном маршрутизаторе, и **брандмауэра Windows**, настроенного в качестве основного на локальном компьютере, где установлен шлюз.

![брандмауэры](./media/data-factory-move-data-between-onprem-and-cloud/firewalls.png)

### Подключение шлюза к облачным службам
Чтобы обеспечить подключение шлюза к фабрике данных Azure и другим облачным службам, необходимо убедиться, что настроено правило для исходящего трафика к портам **TCP** **80** и **443**. Дополнительно можно разрешить порты **9350**–**9354** и **5671**. Эти порты служебная шина Microsoft Azure использует для подключения фабрики данных Azure к шлюзу управления данными как средство, обеспечивающее эффективный обмен данными.
...
Входящий порт **8050** используется приложением **Настройка учетных данных** для ретрансляции учетных данных на шлюз при настройке локальной связанной службы на портале Azure (подробности см. ниже в этой статье). По умолчанию при установке шлюза управления данными программа установки открывается на компьютере шлюза.
...
Например, для копирования **из локального хранилища данных в базу данных SQL Azure или хранилище данных SQL Azure** необходимо разрешить исходящий обмен данными через **TCP**-порт **1433** как на брандмауэре Windows, так и на корпоративном брандмауэре. Кроме этого, в настройках брандмауэра на сервере SQL Azure следует добавить IP-адрес компьютера шлюза в список разрешенных IP-адресов.

Обратите внимание, что при загрузке данных в хранилище данных SQL можно использовать функцию [промежуточного копирования](data-factory-copy-activity-performance.md#staged-copy), чтобы избежать открытия дополнительных портов в корпоративном брандмауэре.
...
## Устранение неполадок со шлюзом
...
- Если после **изменения сертификата** шлюз перестанет работать, перезапустите (остановите и снова запустите) **службу шлюза управления данными** с помощью диспетчера конфигурации шлюза управления данными Microsoft или элемента "Службы" на панели управления. Если ошибка сохраняется, может потребоваться предоставить пользователю службы шлюза управления данными явные разрешения для доступа к сертификату в диспетчере сертификатов (certmgr.msc). По умолчанию для службы используется учетная запись пользователя **NT Service\\DIAHostService**. 
- При возникновении ошибок, связанных с подключением к хранилищу данных или с драйвером, запустите **диспетчер конфигурации шлюза управления данными** на компьютере шлюза и перейдите на вкладку **Диагностика**. Там выберите или введите соответствующие значения для полей в разделе **Проверить подключение к локальному источнику данных с помощью этого шлюза** и нажмите кнопку **Проверить подключение**. Так вы сможете проверить, можно ли подключиться к локальному источнику данных с компьютера шлюза с этими параметрами подключения и учетными данными. Если после установки драйвера тестовое подключение по-прежнему не работает, перезапустите шлюз для того, чтобы получить последние изменения.  
...
3. Введите имя сервера в качестве значения для свойства **Data Source** в **connectionString**.
4. Введите имя базы данных в качестве значения для свойства **Initial Catalog** в **connectionString**.    
5. Нажмите кнопку **Зашифровать** на панели команд. Откроется диалоговое окно **Setting Credentials** (Настройка учетных данных). ![Диалоговое окно "Настройка учетных данных"](./media/data-factory-move-data-between-onprem-and-cloud/setting-credentials-dialog.png)
...
5.	Теперь в **connectionString** появится свойство **encryptedCredential**.		
