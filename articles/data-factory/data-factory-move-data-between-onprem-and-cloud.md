...
Шлюз прекращает работать после **изменения сертификата**, перезапустите (остановите и запустите) **службу шлюза управления данными** с помощью диспетчера конфигурации шлюза управления данными Microsoft или приложения "Службы" на панели управления. Если ошибка сохраняется, может потребоваться предоставить пользователю службы шлюза управления данными явные разрешения для доступа к сертификату в диспетчере сертификатов (certmgr.msc). Учетная запись пользователя по умолчанию для службы: **NT Service\\DIAHostService**.
...
3. В колонке **Настройка** щелкните **Установить непосредственно на этот компьютер**. Это позволит скачать пакет установки для шлюза, а также установить, настроить и зарегистрировать шлюз на компьютере.
	...
	>  
	> То же самое (установку расширения) необходимо сделать и в Firefox. Например, можно установить расширение [отсюда](https://addons.mozilla.org/firefox/addon/fxclickonce/).
