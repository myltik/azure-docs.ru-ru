...
## Установка
Шлюз управления данными можно установить, загрузив пакет MSI из центра загрузки Майкрософт. Пакет MSI также может использоваться для обновления имеющегося шлюза управления данными до последней версии с сохранением всех параметров. Ссылку на пакет MSI можно найти на классическом портале Azure, следуя приведенным ниже пошаговым инструкциям.
...
### Общие рекомендации
Описанные здесь рекомендации в первую очередь касаются двух основных брандмауэров: **корпоративного брандмауэра**, работающего на центральном корпоративном маршрутизаторе, и **брандмауэра Windows**, настроенного в качестве основного на локальном компьютере, где установлен шлюз. При использовании стороннего брандмауэра вместо брандмауэра Windows следуйте рекомендациям ниже в качестве справки и настройте порты соответствующим образом. Если в вашей организации используется прокси-сервер, см. сведения в разделе [Рекомендации для прокси-сервера](#proxy-server-considerations). Ниже приведены некоторые общие рекомендации.

**Перед настройкой шлюза**

- Необходимо убедиться, что для **корпоративного брандмауэра и брандмауэра Windows** исходящее правило активировано для **TCP**-портов **80** и **443** (и при необходимости для портов **9350**–**9354**). Служебная шина Microsoft Azure использует эти порты, чтобы установить подключение между фабрикой данных Azure и шлюзом управления данными. Открывать порты 9350–9354 необязательно, однако это может повысить эффективность обмена данными между этими компонентами.

**Во время установки шлюза**

- По умолчанию при установке шлюза управления данными на **локальном брандмауэре Windows** компьютера шлюза открывается входящий порт **8050**. Этот порт будет использовать приложение **Настройка учетных данных** для ретрансляции учетных данных на шлюз в случае настройки локальной связанной службы на портале Azure (подробности ниже в статье). Он не будет доступен из Интернета, поэтому не нужно открывать его в корпоративном брандмауэре.
- Если не требуется, чтобы при установке шлюза в брандмауэре Windows компьютера шлюза открылся порт 8050, можно использовать следующую команду для установки шлюза без настройки брандмауэра.

		msiexec /q /i DataManagementGateway.msi NOFIREWALL=1

Если входящий порт 8050 не открыт на компьютере шлюза, то для настройки учетных данных хранилища данных в рамках настройки локальной связанной службы необходимо использовать другие способы, помимо приложения **Настройка учетных данных**. Например, можно использовать командлет PowerShell [New-AzureRmDataFactoryEncryptValue](https://msdn.microsoft.com/library/mt603802.aspx). Дополнительные сведения о настройке учетных данных хранилища данных см. в разделе [Настройка учетных данных и безопасность](#setting-credentials-and-security).
...
2.	Запустите файл Notepad.exe с правами администратора и откройте текстовый файл с путем C:\\Program Files\\Microsoft Data Management Gateway\\1.0\\Shared\\diahost.exe.config. Там вы найдете тег по умолчанию для system.net:
	...
	Внутри тега прокси-сервера можно указать дополнительные свойства, чтобы задать требуемые параметры, например scriptLocation. Сведения о синтаксисе см. в разделе [Элемент proxy (параметры сети)](https://msdn.microsoft.com/library/sa91de1e.aspx).
...
Кроме того, необходимо также убедиться, что Microsoft Azure входит в белый список в вашей компании. Список действительных IP-адресов Microsoft Azure можно скачать в [Центре загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=41653).
...
На этом шаге вы создадите экземпляр фабрики данных Azure с именем **ADFTutorialOnPremDF**, используя классический портал Azure. Фабрику данных также можно создать с помощью командлетов фабрики данных Azure.
...
10. На классическом портале Azure нажмите кнопку **ОК** в колонке **Настройка**, а затем в колонке **Новый шлюз данных**.
6. В иерархической структуре слева найдите элемент **adftutorialgateway** в узле **Шлюзы данных**. Щелкните его, чтобы увидеть связанные JSON-файлы. 
...
На этом шаге вы будете использовать классический портал Azure для мониторинга фабрики данных Azure. Вы также можете использовать командлеты PowerShell для мониторинга наборов данных и конвейеров. Дополнительные сведения о мониторинге см. в статье [Мониторинг конвейеров и управление ими](data-factory-monitor-manage-pipelines.md).
...
8.	В диалоговом окне **Настройка учетных данных** выполните следующие действия.
	...
	![Диалоговое окно "Настройка учетных данных"](./media/data-factory-move-data-between-onprem-and-cloud/setting-credentials-dialog.png) 1. Выберите тип **проверки подлинности**, который служба фабрики данных будет использовать для подключения к базе данных. 2. В поле **ИМЯ ПОЛЬЗОВАТЕЛЯ** укажите имя пользователя с доступом к базе данных. 3. В поле **ПАРОЛЬ** укажите пароль этого пользователя. 4. Щелкните **ОК**, чтобы закрыть диалоговое окно. 
4. Щелкните **ОК**, чтобы закрыть колонку **Учетные данные**. 
5. Щелкните **ОК** в колонке **Новое хранилище данных**. 	
6. Убедитесь, что в колонке «Связанные службы» состояние службы **SqlServerLinkedService** изменилось на «В сети». ![Состояние связанной службы SQL Server](./media/data-factory-move-data-between-onprem-and-cloud/sql-server-linked-service-status.png)
...
При использовании приложения «Настройка учетных данных» (открывается на классическом портале Azure для настройки доступа к локальному источнику данных) учетные данные шифруются с помощью сертификата, который указан на вкладке «Сертификат» в диспетчере конфигурации шлюза управления данными на компьютере со шлюзом.
...
1.	Разработчик создает новый шлюз для фабрики данных Azure, используя [классический портал Azure](http://portal.azure.com) или [командлет PowerShell](https://msdn.microsoft.com/library/dn820234.aspx). 
