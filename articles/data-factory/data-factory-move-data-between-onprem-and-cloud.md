...
3. В колонке **Новая фабрика данных** введите **ADFTutorialOnPremDF** в поле "Имя".
...
6. В колонке **Создание фабрики данных** нажмите кнопку **Создать**.
   ...
   >
   >
7. После завершения процедуры создания фабрики данных появится колонка **Фабрика данных** , как показано на рисунке ниже.
...
11. Перейдите в браузер и на **портале Azure** нажмите кнопку **ОК** в колонке **Настройка**, а затем — в колонке **Новый шлюз данных**.
...
2. В **редакторе JSON** справа сделайте следующее:
   ...
   2. В **connectionString** сделайте следующее:    
      ...
      3. На панели инструментов нажмите кнопку **Зашифровать**. При этом скачивается и запускается приложение "Диспетчер учетных данных".
...
4. Нажмите кнопку **X** дважды, чтобы закрыть все колонки и вернуться к колонке **Фабрика данных** для **ADFTutorialOnPremDF**.
...
3. Затем в колонке **Наборы данных** щелкните **OutputBlobTable**.

    ![Срезы OputputBlobTable](./media/data-factory-move-data-between-onprem-and-cloud/OutputBlobTableSlicesBlade.png)
4. Щелкните любой срез данных в списке, чтобы отобразить колонку **Срез данных**. Для этого среза отобразится раздел "Запуски операции". Как правило, в этом разделе отображается только одно действие.  
...
6. Закройте все колонки, щелкая значок **X** , пока
7. не вернетесь к начальной колонке **ADFTutorialOnPremDF**.
...
9. Используйте средства, такие как [обозреватель хранилищ Microsoft](http://storageexplorer.com/), чтобы проверить, создается ли файл или большой двоичный объект каждый час.
