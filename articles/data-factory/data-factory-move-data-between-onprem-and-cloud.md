...
Эта статья содержит общие сведения об интеграции данных, хранящихся в локальных и облачных хранилищах данных, а также об обработке данных в облаке с помощью фабрики данных. В статье используются концепции, описанные в статье [Действия по перемещению данных](data-factory-data-movement-activities.md) и других материалах о фабрике данных. Вам следует предварительно ознакомиться с основными понятиями фабрики данных, такими как конвейеры, действия, наборы данных и действие копирования.

## Как фабрика данных Azure работает со шлюзом управления данными 
Хотите ли вы легко и быстро перемещать данные из локальной сети в облако и обратно? Фабрика данных Azure представляет собой локально устанавливаемый агент, который позволяет управлять шлюзом данных. Среди прочего она содержит и приложение для настройки учетных данных шлюза.

## Что делает шлюз данных
...
1.	Поддерживаемые **операционные системы**: Windows 7, Windows 8/8.1, Windows 10, Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2. В настоящее время установка шлюза управления данными на контроллере домена не поддерживается.
...
Входящий порт **8050** используется приложением **настройки учетных данных** для ретрансляции на шлюз учетных данных, которые вы указываете на портале Azure для локальной связанной службы (подробности см. ниже в этой статье). По умолчанию при установке шлюза управления данными программа установки открывается на компьютере шлюза.
...
Например, для копирования данных **из локального хранилища данных в базу данных SQL Azure или хранилище данных SQL Azure** необходимо разрешить исходящий обмен данными через **TCP**-порт **1433** как на брандмауэре Windows, так и на корпоративном брандмауэре. Кроме этого, в настройках брандмауэра на сервере SQL Azure следует добавить IP-адрес компьютера шлюза в список разрешенных IP-адресов.
...
На этом шаге вы с помощью портала Azure создадите экземпляр фабрики данных Azure с именем **ADFTutorialOnPremDF**. Фабрику данных также можно создать с помощью командлетов фабрики данных Azure.
...
На этом шаге вы создадите две связанные службы: **AzureStorageLinkedService** и **SqlServerLinkedService**. Служба **SqlServerLinkedService** связывает с фабрикой данных локальную базу данных SQL Server, а служба **AzureStorageLinkedService** — хранилище BLOB-объектов Azure. Далее это руководство поможет вам создать конвейер, который позволит копировать данные из локальной базы данных SQL Server в службу хранилища больших двоичных объектов Azure.
...
3.	В **редакторе JSON** выполните следующие действия.
	...
	4. При использовании проверки подлинности SQL:
		...
		Учетные данные будут **зашифрованы** с помощью сертификата, который принадлежит службе фабрики данных. Если вы хотите использовать вместо него сертификат, связанный со шлюзом управления данными, см. раздел, посвященный [безопасной настройке учетных данных](#set-credentials-and-security).
...
2.	Замените сценарий JSON в области справа на следующий текст:
	...
	Если не указать **fileName** для **выходной таблицы**, созданные в каталоге **folderPath** файлы получат имена следующего вида: Data.<Guid>.txt (например, Data.0a405f8a-93ff-4c6f-b3be-f69616f1df7a.txt).
...
5. Нажмите кнопку **Шифровать** на панели команд, чтобы запустить приложение ClickOnce **Диспетчер учетных записей**. Откроется диалоговое окно **настройки учетных данных**. ![Диалоговое окно "Настройка учетных данных"](./media/data-factory-move-data-between-onprem-and-cloud/setting-credentials-dialog.png)
...
Ниже показана обобщенная диаграмма потока данных и этапы копирования через шлюз: ![Поток данных при использовании шлюза](./media/data-factory-move-data-between-onprem-and-cloud/data-flow-using-gateway.png)
