...
5. Подождите несколько минут и запустите на компьютере приложение **Диспетчер конфигурации шлюза управления данными**. В окне **Поиск** введите **Шлюз управления данными**, чтобы получить доступ к этой служебной программе. Вы также можете найти исполняемый файл **ConfigManager.exe** в папке по следующему пути: **C:\\Program Files\\Microsoft Data Management Gateway\\1.0\\Shared**.
...
6. Подождите, пока установятся следующие значения:
	1. Для параметра **Состояние** установлено значение **Запущена**.
...
6. В представлении дерева в левой части окна вы увидите **adftutorialgateway** в узле **Шлюзы данных**. Щелкните его, чтобы увидеть связанные JSON-файлы. 
	

### Шаг 2. Создание связанных служб 
На этом шаге вы создадите две связанные службы: **StorageLinkedService** и **SqlServerLinkedService**. Служба **SqlServerLinkedService** связывается с локальной базой данных SQL Server, а связанная служба **StorageLinkedService** связывает службы хранилища больших двоичных объектов Azure с фабрикой данных. Далее это руководство поможет вам создать конвейер, который позволит копировать данные из локальной базы данных SQL Server в службу хранилища больших двоичных объектов Azure.
...
3.	В **редакторе JSON** выполните следующие действия: 
	...
	2. При использовании проверки подлинности Windows:
		...
		3. Укажите имя пользователя и пароль для свойств **userName** и **password**.
	...
	4. При использовании проверки подлинности SQL:
		...
		2. Удалите последние два свойства JSON, **userName** и **password**, из JSON.
		3. Удалите символы **, (запятая)** в конце строки, где указано свойство **gatewayName**. 
...
2. В поле **Имя учетной записи** введите имя вашей учетной записи хранения Azure.
3. В поле **Ключ учетной записи** введите ключ вашей учетной записи хранения Azure.
4. Щелкните **Развернуть**, чтобы развернуть **StorageLinkedService**.
...
1. В **редакторе фабрики данных** щелкните на панели команд **Создать набор данных** и выберите пункт **Таблица SQL Server**. 
...
- для свойства **sqlReaderQuery** типа **SqlSource** задан вид SQL-запроса **select * from emp**.
	...
	Если не указать значение свойства **end**, то оно будет вычислено по формуле «**время начала + 48 ч**». Чтобы запустить конвейер в течение неопределенного срока, укажите значение **9/9/9999** для свойства **end**.
...
3.	На главной странице фабрики данных щелкните элемент **Связанные службы**. 
4.	В колонке **Связанные службы** на командной панели щелкните **Создать хранилище данных**. 
4.	В поле **Имя** введите **AzureSqlLinkedService**. 
