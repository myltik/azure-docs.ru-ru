...
2. Запустите **диспетчер конфигурации SQL Server**. Разверните узел **Сетевая конфигурация SQL Server** для нужного экземпляра и выберите пункт **Протоколы для MSSQLSERVER**. Вы должны увидеть протоколы на панели справа. Включите TCP/TP, щелкнув правой кнопкой мыши имя протокола **TCP/IP** и выбрав значение **Включить**.
...
6. **Проверьте подключение**. Для этого запустите SQL Server Management Studio на другом компьютере и подключитесь к SQL Server, указав полное имя сервера, например <machine>.<domain>.corp.<company>.com,1433.
	...
	> [AZURE.IMPORTANT]
	Дополнительные сведения см. в разделе [Рекомендации по портам и безопасности](data-factory-move-data-between-onprem-and-cloud.md#port-and-security-considerations).
	>   
