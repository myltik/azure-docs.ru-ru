...
В примере предполагается, что вы уже создали таблицу MyTable в SQL Server и она содержит столбец с именем timestampcolumn для данных временного ряда.
...
> [AZURE.NOTE] При использовании **sqlReaderStoredProcedureName** по-прежнему необходимо указать значение для свойства **tableName** в наборе данных JSON. Это ограничение продукта в данный момент. Хотя какие-либо проверки этой таблицы отсутствуют.
...
1. Настройте SQL Server для приема удаленных подключений. Запустите **SQL Server Management Studio**, щелкните свой **сервер** правой кнопкой мыши и выберите пункт **Свойства**. Выберите в списке пункт **Подключения** и установите флажок **Разрешить удаленные подключения к серверу**.
...
2. Запустите **диспетчер конфигурации SQL Server**. Разверните узел **Сетевая конфигурацию SQL Server** для нужного экземпляра и выберите пункт **Протоколы для MSSQLSERVER**. Вы должны увидеть протоколы на панели справа. Включите TCP/TP, щелкнув правой кнопкой мыши имя протокола **TCP/IP** и выбрав значение **Включить**.

	![Включение TCP/IP](.\media\data-factory-sqlserver-connector\EnableTCPProptocol.png)

	Подробные сведения и альтернативные способы включения протокола TCP/IP приведены в статье [Включение или отключение сетевого протокола сервера](https://msdn.microsoft.com/library/ms191294.aspx). 
3. В этом же окне дважды щелкните **TCP/IP**, чтобы открыть окно **Свойства TCP/IP**.
4. Откройте вкладку **IP-адреса**. Прокрутите вниз до раздела **IPAll**. Запишите значение параметра **Порт TCP** (по умолчанию — **1433**).
5. Создайте на компьютере **правило брандмауэра Windows**, чтобы разрешить входящий трафик через этот порт.  
6. **Проверьте подключение**. Для этого запустите SQL Server Management Studio на другом компьютере и подключитесь к SQL Server, указав полное имя сервера, например <machine>.<domain>.corp.<company>.com,1433.

	> [AZURE.IMPORTANT] 
	Подробные сведения см. в разделе [Рекомендации по портам и безопасности](data-factory-move-data-between-onprem-and-cloud.md#port-and-security-considerations).
	>   
	> Советы по устранению неполадок, связанных со шлюзом или подключением, см. в разделе [Устранение неполадок шлюза](data-factory-move-data-between-onprem-and-cloud.md#gateway-troubleshooting).
