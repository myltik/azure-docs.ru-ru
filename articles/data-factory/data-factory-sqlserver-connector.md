...
Полный список разделов и свойств, используемых для определения наборов данных, см. в статье [Создание наборов данных](data-factory-create-datasets.md). Разделы структуры, доступности и политики JSON набора данных одинаковы для всех типов наборов данных (SQL Server, BLOB-объекты Azure, таблицы Azure и т. д.).
...
## Свойства типа "Действие копирования SQL Server"
...
1. Настройте SQL Server для приема удаленных подключений. Запустите **SQL Server Management Studio**, щелкните правой кнопкой мыши по своему **серверу** и выберите **Свойства**. Выберите **Подключения** из списка и установите флажок **Разрешить удаленные подключения к серверу**.
	
	![Включение удаленных подключений](.\media\data-factory-sqlserver-connector\AllowRemoteConnections.png)

	Подробное описание шагов см. в разделе [Настройка параметра конфигурации сервера удаленного доступа](https://msdn.microsoft.com/library/ms191464.aspx). 
2. Запустите **диспетчер конфигурации SQL Server**. Разверните **Сетевую конфигурацию SQL Server** для необходимого экземпляра и выберите **Протоколы для MSSQLSERVER**. Вы должны увидеть протоколы на панели справа. Включите TCP/TP, щелкнув правой кнопкой мыши **TCP/IP** и выбрав **Включить**.

	![Включение TCP/IP](.\media\data-factory-sqlserver-connector\EnableTCPProptocol.png)

	Подробные сведения и альтернативные способы включения протокола TCP/IP см. в разделе [Включение или отключение сетевого протокола сервера](https://msdn.microsoft.com/library/ms191294.aspx). 
