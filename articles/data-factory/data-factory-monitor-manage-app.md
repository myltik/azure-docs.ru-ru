...
В этой статье описываются процедуры мониторинга, управления и отладки конвейеров с помощью **приложения по мониторингу и управлению**. В ней также приводятся инструкции по использованию приложения для создания оповещений и получения предупреждений в случае сбоев.
      
## Запуск приложения по мониторингу и управлению 
Чтобы запустить приложение по мониторингу и управлению, щелкните элемент **Приложение мониторинга** в колонке **ФАБРИКА ДАННЫХ** для фабрики данных.
...
В левой области отображается **представление в виде дерева** обозревателя ресурсов, в верхней части находится **Представление схемы**, в нижней части средней области отображается список **Окна действий**, а в правой области находятся вкладки **Свойства**/ **Обозреватель окон действий**.
...
- связанные окна действий (щелкните [здесь](data-factory-scheduling-and-execution.md) для получения сведений об окнах действий) будут выделены в списке "Окна действий" в нижней части экрана;  
...
При выборе окна действия в списке вы увидите сведения о нем в обозревателе окон действий в окне "Свойства" в правой области.

![Обозреватель окон действий](./media/data-factory-monitor-manage-app/ActivityWindowExplorer-2.png)

### Обновление окон действий  
Данные не обновляются автоматически, поэтому воспользуйтесь кнопкой **Обновить** (вторая кнопка) на панели команд, чтобы обновить список окон действий вручную.
...
При выборе варианта **Последние окна действий** будут отображены все последние окна действий в порядке убывания **времени последней попытки**.
...
1. Укажите **имя** и введите **описание** оповещения, а затем нажмите кнопку **Далее**. Должна открыться страница **Фильтры**.
	...
	Вы также можете **объединить** события оповещений, как показано ниже.

	![Объединение оповещений](./media/data-factory-monitor-manage-app/AggregateAlerts.png)
...
3. Выберите параметр **Администраторы подписки электронной почты** и (или) укажите значение параметра **Адрес электронной почты дополнительного администратора**, а затем нажмите кнопку **Готово**. Оповещение должно появиться в списке. 
...
Выполнение действия завершено | Сбой| Сбой выделения ресурсов<p>Сбой выполнения</p><p>Истекло время ожидания</p><p>Сбой проверки</p><p>Прервано</p>
