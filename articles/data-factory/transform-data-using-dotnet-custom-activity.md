...
## <a name="difference-between-custom-activity-in-azure-data-factory-v2-and-custom-dotnet-activity-in-azure-data-factory-v1"></a>Различия между настраиваемым действием в фабрике данных Azure версии 2 и (настраиваемым) действием DotNet в фабрике данных Azure версии 1 
  ...
  В следующей таблице описаны различия между данными фабрики V2 настраиваемое действие и V1 фабрики данных действия DotNet (пользовательский): 


|Различия      |Пользовательское действие ADFv2      |Действие ADFv1 DotNet (пользовательский)      |
| ---- | ---- | ---- |
|Как определяется пользовательская логика      |Выполнив любой исполняемый файл (существующие или реализации собственных исполняемый файл)      |Реализуя .net DLL      |
|Выполнение организации пользовательская логика      |Windows или Linux      |Windows (.Net Framework 4.5.2)      |
|Выполнение сценариев      |Поддерживает выполнение сценариев напрямую (например «cmd /c echo hello world» на виртуальной Машине Windows)      |Требуется реализация в .net библиотеки DLL      |
|Требуется набор данных      |Необязательно      |Необходимые для передачи информации и цепочки действий      |
|Передать сведения от действия пользовательской логики      |Через ReferenceObjects (LinkedServices и наборы данных) и ExtendedProperties (пользовательские свойства) и      |Через ExtendedProperties (пользовательские свойства), входные и выходные наборы данных      |
|Извлечение сведений в пользовательскую логику      |Синтаксический анализ activity.json, linkedServices.json и datasets.json, хранящихся в той же папке исполняемого файла      |Через .net SDK (кадр .net 4.5.2)      |
|Ведение журналов      |Записывает непосредственно в STDOUT      |Средства ведения журнала Implemeting в .net библиотеки DLL      |
...
Полный пример использования сквозной пример конвейера и библиотеки DLL, описанные в документе V1 фабрики данных [использовать настраиваемые действия в конвейере фабрики данных Azure](https://docs.microsoft.com/en-us/azure/data-factory/v1/data-factory-use-custom-activities) можно переписать в стиль данных фабрики V2 пользовательского действия. Ссылаться на [образец данных фабрики V2 настраиваемое действие](https://github.com/Azure/Azure-DataFactory/tree/master/Samples/ADFv2CustomActivitySample). 
