...
Действие Hive HDInsight в [конвейере](data-factory-create-pipelines.md) фабрики данных выполняет запросы Hive к [собственному](data-factory-compute-linked-services.md#azure-hdinsight-linked-service) кластеру HDInsight или к кластеру HDInsight [по требованию](data-factory-compute-linked-services.md#azure-hdinsight-on-demand-linked-service). Эта статья основана на материалах статьи [Действия преобразования данных](data-factory-data-transformation-activities.md), в которой приведен общий обзор преобразования данных и список поддерживаемых действий преобразования.
...
Рассмотрим пример с аналитикой журналов игры. Предположим, вы хотите определить время, которое пользователи проводят за игрой, выпущенной вашей компанией.
...
**Сценарий Hive** для обработки этих данных выглядит приведенным ниже образом.
...
Чтобы выполнить его в конвейере фабрики данных:
...
2. Создайте [связанную службу](data-factory-azure-storage-connector.md) для настройки подключения к хранилищу BLOB-объектов Azure, в котором хранятся данные. Назовем эту связанную службу StorageLinkedService.
...
4. Скопируйте запрос Hive в файл и сохраните его в хранилище BLOB-объектов Azure, заданном на втором шаге. Если связанная служба, размещающая данные, отличается от службы, размещающей файл запроса, создайте отдельную службу хранилища Azure и добавьте ссылку на нее в конфигурацию действия. Используйте свойство**scriptPath**, чтобы указать путь к файлу запроса Hive, и **scriptLinkedService**, чтобы задать службу Azure, размещающую файл сценария.

	> [AZURE.NOTE]Вы можете добавить сценарий Hive непосредственно в определение действия, используя свойство **script**. Однако мы не рекомендуем этого делать, так как в этом случае потребуется экранировать все специальные символы в сценарии в документе JSON, что может вызвать проблемы при отладке. Рекомендуется следовать инструкциям, описанным в шаге 4.
5.	Создайте следующий конвейер, в котором обработка данных выполняется действием HDInsightHive.
...
## Параметризация сценариев Hive с помощью элемента defines 

Рассмотрим пример, в котором журналы игры ежедневно добавляются в хранилище BLOB-объектов Azure и хранятся в папках, разбитых по дате и времени. Вам необходимо параметризовать сценарий Hive, чтобы адрес входной папки передавался во время выполнения динамически, а выходной набор данных сегментировался по дате и времени.

Чтобы использовать параметризованные сценарии Hive, сделайте вот что:
...
## Отправить отзыв
Мы будем очень благодарны за ваш отзыв об этой статье. Отправьте его [по электронной почте](mailto:adfdocfeedback@microsoft.com?subject=data-factory-hive-activity.md).
