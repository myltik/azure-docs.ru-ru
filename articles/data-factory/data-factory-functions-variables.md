...
Дата | EndOfMonth(X) | X: DateTime | Получает конец месяца, представленного компонентом месяца параметра X. <br/><br/>Пример: EndOfMonth для 15.09.2013 17:10:23 имеет значение 30.09.2013 23:59:59 (дата и время, соответствующие окончанию месяца сентября).
Дата | StartOfDay(X) | X: DateTime | Получает начало суток, представленных компонентом дня параметра X. <br/><br/>Пример: StartOfDay для 15.09.2013 17:10:23 имеет значение 15.09.2013 00:00:00.
...
См. статью [Строки настраиваемых форматов даты и времени](https://msdn.microsoft.com/library/8kb3ddd4.aspx), которая содержит информацию о параметрах форматирования, которые вы можете использовать (например, гг и гггг).
