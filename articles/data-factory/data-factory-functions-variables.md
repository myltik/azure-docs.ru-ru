...
# Фабрика данных Azure — функции и системные переменные
...
------------- | ----------- | ------------ | ------------------------
WindowStart | Начало интервала времени для текущего окна запуска действия. | действие | <ol><li>Укажите запросы на выбор данных. См. статьи о соединителях, на которые ссылается статья [Действия перемещения данных](data-factory-data-movement-activities.md).</li><li>Передайте параметры в скрипт Hive (пример показан выше).</li>
WindowEnd | Конец интервала времени для текущего окна запуска действия. | действие | то же, что и выше
SliceStart | Начало интервала времени для формируемого среза данных. | действие<br/>набор данных | <ol><li>Укажите динамические пути к папкам и имена файлов во время работы с [большим двоичным объектом Azure](data-factory-azure-blob-connector.md) и [наборами данных файловой системы](data-factory-onprem-file-system-connector.md).</li><li>Укажите входные зависимости с помощью функций фабрики данных в коллекции входных данных действия.</li></ol>
SliceEnd | Конец интервала времени для текущего формируемого среза данных. | набор данных <br/>действия | то же, что и выше 
...
Время | AddMinutes(X,Y) | X: DateTime <br/><br/>Y: int | Добавляет Y минут к X. <br/><br/>Пример: 15.09.2013 12:00:00 + 15 минут = 15.09.2013 12:15:00.
...
Дата | AddDays(X,Y) | X: DateTime<br/><br/>Y: int | Добавляет Y дней к X. <br/><br/>Пример: 15.09.2013 12:00:00 + 2 дня = 17.09.2013 12:00:00.
Дата | AddMonths(X,Y) | X: DateTime<br/><br/>Y: int | Добавляет Y месяцев к X. <br/><br/>Пример: 15.09.2013 12:00:00 + 1 месяц = 15.10.2013 12:00:00. 
Дата | AddQuarters(X,Y) | X: DateTime <br/><br/>Y: int | Добавляет Y (одна единица которого равна 3 месяцам) к X. <br/><br/>Пример: 15.09.2013 12:00:00 + 1 квартал = 15.12.2013 12:00:00.
...
Дата | AddYears(X,Y) | X: DateTime<br/><br/>Y: int | Добавляет Y лет к X. <br/><br/>Пример: 15.09.2013 12:00:00 + 1 год = 15.09.2014 12:00:00.
Дата | Day(X) | X: DateTime | Получает компонент дня для X. <br/><br/>Пример: Day для 15.09.2013 12:00:00 имеет значение 15. 
Дата | DayOfWeek(X) | X: DateTime | Получает компонент дня недели, имеющийся в параметре X. <br/><br/>Пример: DayOfWeek для 15.09.2013 12:00:00 имеет значение Sunday (Воскресенье).
Дата | DayOfYear(X) | X: DateTime | Получает день года, представленный компонентом года в параметре X. <br/><br/>Примеры:<br/>01.12.2015: 335 день 2015 года<br/>31.12.2015: 365 день 2015 года<br/>31.12.2016: 366 день 2016 года (високосного).
Дата | DaysInMonth(X) | X: DateTime | Получает количество дней в месяце, представленном компонентом месяца в параметре X. <br/><br/>Пример: DaysInMonth для 15.09.2013 = 30, так как в сентябре 30 дней.
Дата | EndOfDay(X) | X: DateTime | Получает значение даты и времени, которое соответствует окончанию суток (компонента дня) в параметре X. <br/><br/>Пример: EndOfDay для 15.09.2013 17:10:23 имеет значение 15.09.2013 23:59:59.
Дата | EndOfMonth(X) | X: DateTime | Получает конец месяца, представленного компонентом месяца в параметре X. <br/><br/>Пример: EndOfMonth для 15.09.2013 17:10:23 имеет значение 30.09.2013 23:59:59 (дата и время, соответствующие окончанию месяца сентября).
Дата | StartOfDay(X) | X: DateTime | Получает начало суток, представленных компонентом дня в параметре X. <br/><br/>Пример: StartOfDay для 15.09.2013 17:10:23 имеет значение 15.09.2013 00:00:00.
...
См. статью [Строки настраиваемых форматов даты и времени](https://msdn.microsoft.com/library/8kb3ddd4.aspx), содержащую сведения о различных параметрах форматирования, которые вы можете использовать (например, гг и гггг).
