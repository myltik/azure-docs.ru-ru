...
- Поддерживаемые **операционные системы** : Windows 7, Windows 8, Windows 8.1, Windows 10, Windows Server 2008 R2, Windows Server 2012, Windows Server 2012 R2. Установка локальной среды выполнения интеграции на **контроллер домена не поддерживается**.
...
Чтобы связать несколько узлов, достаточно лишь установить программное обеспечение локальной среды выполнения интеграции из [центра загрузки](https://www.microsoft.com/download/details.aspx?id=39717) и зарегистрировать его с помощью любого из ключей аутентификации, полученных командлетом New-AzureRmDataFactoryV2IntegrationRuntimeKey, как описано в [этом руководстве](tutorial-hybrid-copy-powershell.md). 

> [!NOTE]
> Для дополнительных узлов не нужно создавать отдельную локальную среду выполнения интеграции. 
...
### <a name="open-port-8060-for-credential-encryption"></a>Открытие порта 8060 для шифрования учетных данных
Приложение **Настройка учетных данных** использует входящий порт 8060, чтобы передать учетные данные в локальную среду выполнения интеграции, когда вы настраиваете локальную среду выполнения интеграции на портале Azure. Во время установки локальной среды выполнения интеграции программа установки по умолчанию открывает этот порт на компьютере, где будет работать локальная среда выполнения интеграции.

В случае использования стороннего брандмауэра вы сможете открыть порт 8050 вручную. Если во время установки локальной среды выполнения интеграции возникли проблемы с брандмауэром, попробуйте установить локальную среду выполнения интеграции без настройки брандмауэра, используя следующую команду.
