...
> [!NOTE]
> Для дополнительных узлов не нужно создавать отдельную локальную среду выполнения интеграции. Можно установить выполнения резидентной интеграции на другом компьютере и зарегистрировать ее, используя один и тот же ключ проверки подлинности. 

> [!NOTE]
> Прежде чем добавлять другой узел для **высокий уровень доступности и масштабируемости**, убедитесь, **«Удаленный доступ к интрасети»** параметр — **включена** на узле 1-й (Microsoft Интеграция среды выполнения Configuration Manager -> Параметры -> удаленный доступ к интрасети). 
...
- Каждый узел интеграции среды выполнения должны доверять этому сертификату.
...
### <a name="enable-remote-access-from-intranet"></a>Включить удаленный доступ из интрасети  
В случае, если вы используете **PowerShell** или **диспетчер учетных данных приложения** для шифрования учетных данных, с другого компьютера (в сети), отличный от выполнения резидентной интеграции установки, нажмите Вам потребуется **«Удаленный доступ из интрасети»** была включена. При запуске **PowerShell** или **диспетчер учетных данных приложения** для шифрования учетных данных на том же компьютере, выполнения резидентной интеграции установки, затем **"удаленный доступ из интрасети "** может быть выключена.

Удаленный доступ из интрасети, должен быть **включен** перед добавлением другой узел для **высокий уровень доступности и масштабируемости**.  

Во время интеграции резидентной среды выполнения установки (v 3.3.xxxx.x начиная), по умолчанию отключает установки среды выполнения резидентной интеграции **«Удаленный доступ из интрасети»** на компьютере резидентной интеграции среды выполнения.

Если вы используете сторонний брандмауэр, можно вручную открыть порт 8060 (или порт настройкам пользователя). Если во время установки локальной среды выполнения интеграции возникли проблемы с брандмауэром, попробуйте установить локальную среду выполнения интеграции без настройки брандмауэра, используя следующую команду.

```
msiexec /q /i IntegrationRuntime.msi NOFIREWALL=1
```
> [!NOTE]
> **Диспетчер учетных данных приложения** еще не доступен для шифрования учетных данных в ADFv2. Эта поддержка будет добавлен позже.  
