...
| connectionString | Укажите сведения, необходимые для подключения к экземпляру базы данных Oracle. Пометьте это поле в качестве SecureString.<br><br>**Поддерживаемые типы подключения**: вы можете использовать **Oracle SID** или **имя службы Oracle** для идентификации базы данных:<br>-С помощью ИД безопасности:`Host=<host>;Port=<port>;Sid=<sid>;User Id=<username>;Password=<password>;`<br>-С помощью имени службы:`Host=<host>;Port=<port>;ServiceName=<sid>;User Id=<username>;Password=<password>;` | Yes |
