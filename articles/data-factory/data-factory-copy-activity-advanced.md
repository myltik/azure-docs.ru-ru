...
| BlobSource | При переносе из BlobSource в BlobSink не выполняются никакие преобразования типов; типы данных, определенные в разделе Structure определения таблицы, игнорируются. Для назначений, отличных от BlobSink, типы данных, определенные в разделе Structure определения таблицы, будут приниматься. Если в определении таблицы не указан раздел Structure, то обработка типов определяется свойством format таблицы BlobSource. Возможны следующие значения: TextFormat: все типы столбцов обрабатываются как строка, а все имена столбцов задаются как «Prop\_<0-N>». AvroFormat: используются встроенные типы столбцов и имена в файле Avro.
