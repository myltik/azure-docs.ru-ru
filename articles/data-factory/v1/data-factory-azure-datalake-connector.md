...
>- **Для использования хранилища Озера данных в качестве источника**, хотя бы предоставить **чтения + Execute** разрешения для просмотра и скопируйте содержимое папки, доступ к данным или **чтения** разрешение на копирование одного файла. Управление доступом на уровне учетной записи не требуется.
>- **Для использования в качестве приемника хранилища Озера данных**, по крайней мере предоставления **записи + выполнение** разрешение на создание дочерних элементов в папке, доступ к данным. Если для расширения возможностей копирования используется среда IR Azure (источник и приемник находятся в облаке), чтобы фабрика данных смогла определить регион Data Lake Store, назначьте в Системе управления идентификацией и доступом (IAM) роль по крайней мере **читателя**. Чтобы не назначать эту роль IAM, определите [executionLocation](data-factory-data-movement-activities.md#global) с расположением Data Lake Store в действии копирования.
>- Если вы **использовать мастер копирования для создания конвейеров**, хотя бы предоставить **чтения** роли в учетной записи управления доступом (IAM). Кроме того, предоставьте разрешение по крайней мере на **чтение и выполнение** в корневом каталоге Data Lake Store (/) и его дочерних элементах. Иначе может появиться сообщение "Указаны недопустимые учетные данные".
...
> [!IMPORTANT]
> Убедитесь, что вы предоставляете пользователю соответствующие разрешения в хранилище Озера данных Azure:
>- **Для использования хранилища Озера данных в качестве источника**, хотя бы предоставить **чтения + Execute** разрешения для просмотра и скопируйте содержимое папки, доступ к данным или **чтения** разрешение на копирование одного файла. Управление доступом на уровне учетной записи не требуется.
>- **Для использования в качестве приемника хранилища Озера данных**, по крайней мере предоставления **записи + выполнение** разрешение на создание дочерних элементов в папке, доступ к данным. Если для расширения возможностей копирования используется среда IR Azure (источник и приемник находятся в облаке), чтобы фабрика данных смогла определить регион Data Lake Store, назначьте в Системе управления идентификацией и доступом (IAM) роль по крайней мере **читателя**. Чтобы не назначать эту роль IAM, определите [executionLocation](data-factory-data-movement-activities.md#global) с расположением Data Lake Store в действии копирования.
>- Если вы **использовать мастер копирования для создания конвейеров**, хотя бы предоставить **чтения** роли в учетной записи управления доступом (IAM). Кроме того, предоставьте разрешение по крайней мере на **чтение и выполнение** в корневом каталоге Data Lake Store (/) и его дочерних элементах. Иначе может появиться сообщение "Указаны недопустимые учетные данные".
...
**Симптом:** при копировании данных **в** хранилища Озера данных Azure, если действие копирования сбоем со следующей ошибкой:

  ```
  Failed to detect the region for Azure Data Lake account {your account name}. Please make sure that the Resource Group name: {resource group name} and subscription ID: {subscription ID} of this Azure Data Lake Store resource are correct.
  ```

**Основная причина:** 2 возможных причин:

1. `resourceGroupName` И/или `subscriptionId` указан в хранилище Озера данных Azure, связанной службы указано неверно.
2. Пользователь или субъекта-службы не имеет необходимых разрешений.

**Способы устранения:**

1. Убедитесь, что `subscriptionId` и `resourceGroupName` укажите в связанной службе `typeProperties` на самом деле являются те, к которым принадлежит учетной записи Озера данных.

2. Убедитесь, что вы предоставляете по крайней мере "**чтения**«роли для пользователя или участника-службы для учетной записи Озера данных. Вот как можно сделать:

    1. Перейдите на портал Azure -> учетной записи хранилища Озера данных
    2. Нажмите кнопку «доступа элемента управления (IAM)» в колонке хранилища Озера данных
    3. Нажмите кнопку «Добавить» в колонке из «доступа элемента управления (IAM)»
    4. Значение «Роль», как «Чтения» и выберите пользователя или субъекта-службы, используемого для копирования для предоставления доступа

3. Если вы не хотите предоставлять роль «Читатель» для пользователя или участника-службы, — alernative [явно указать место выполнения](data-factory-data-movement-activities.md#global) в копии activitywith расположение хранилища Озера данных. Пример:
