...
>- Если для разработки конвейеров используется мастер копирования данных, назначьте роль по крайней мере **читателя** в Системе управления идентификацией и доступом (IAM). Кроме того, предоставьте разрешение по крайней мере на **чтение и выполнение** в корневом каталоге Data Lake Store (/) и его дочерних элементах. Иначе может появиться сообщение "Указаны недопустимые учетные данные".
>- Чтобы использовать Data Lake Store в качестве источника, предоставьте разрешение на доступ к данным по крайней мере на **чтение и выполнение** для просмотра и копирования содержимого папки, или разрешение на **чтение** для копирования одного файла. Управление доступом на уровне учетной записи не требуется.
>- Чтобы использовать Data Lake Store в качестве приемника, предоставьте разрешение на доступ к данным по крайней мере на **запись и выполнение** для создания дочерних элементов в папке. Если для расширения возможностей копирования используется Azure IR (источник и приемник находятся в облаке), чтобы фабрика данных смогла определить регион Data Lake Store, назначьте в Системе управления идентификацией и доступом (IAM) роль по крайней мере **читателя**. Чтобы не назначать эту роль IAM, определите [executionLocation](data-factory-data-movement-activities.md#global) с расположением Data Lake Store в действии копирования.
