...
## <a name="current-version-2963132"></a>Текущая версия (2.9.6313.2)
...
-    Теперь вы можете скопировать данные в единичный блочный BLOB-объект с размером до 4,75 ТБ (максимальный поддерживаемый размер блочного BLOB-объекта) и из него. (Предыдущий лимит составлял 195 ГБ.)
-    Исправлено. Проблема нехватки памяти при распаковке нескольких небольших файлов во время действия копирования.
-    Исправлено. Проблема, когда индекс выходил за пределы диапазона при копировании из Document DB в локальный SQL с использованием функции идемпотентности.
-    Исправлено. Скрипт очистки SQL не работает с локальным SQL из мастера копирования.
