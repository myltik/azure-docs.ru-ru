...
| password |Пароль для проверки подлинности Windows. Пометьте это поле в качестве SecureString. |Да (для проверки подлинности Windows) |
...
| tempScriptPath | Путь к папке для хранения временного командного скрипта DistCp. Файл скрипта создается фабрикой данных и будут удалены после завершения задания копирования. | Да, если используется DistCp |
