...
- **Передача данных в Azure**. В этом случае данные копируются из локального источника данных (например, SQL Server) в хранилище данных Azure (например, хранилище больших двоичных объектов или таблиц Azure либо база данных SQL Azure). Такие операции возможны в следующих ситуациях.
...
- **Передача данных из Azure**. В этом случае данные копируются из Azure (например, хранилище больших двоичных объектов или таблиц Azure либо база данных SQL Azure) в локальные киоски и хранилища данных (например, SQL Server). Такие операции возможны в следующих ситуациях.
...
- **Копирование внутри Azure**. В этом сценарии данные, распределенные по источникам данных Azure, объединяются в централизованное хранилище данных Azure. Например, из хранилища таблиц Azure в хранилище больших двоичных объектов Azure, из хранилища больших двоичных объектов в хранилище таблиц, из хранилища таблиц в базу данных SQL Azure, из хранилища больших двоичных объектов в базу данных SQL Azure.
...
- Учебник по копированию данных из хранилища больших двоичных объектов Azure в базу данных SQL Azure с помощью действия копирования см. в статье [Начало работы с фабрикой данных Azure][adfgetstarted]. 
- [Использование конвейера для работы с локальными данными][use-onpremises-datasources] — пошаговое руководство по копированию данных из локальной базы данных SQL Server в хранилище больших двоичных объектов Azure с помощью действия копирования.


## Поддерживаемые источники и приемники
Действие копирования можно использовать в следующих случаях:

| *Источник/приемник* | Большой двоичный объект Azure | Таблица Azure | База данных SQL Azure | Azure DocumentDB. | SQL Server на виртуальной машине Azure | Локальный сервер SQL Server |
| ------------- | ---------- | ----------- | ------------------ | ---------------- | ------------------ | ------------------- |
| Большой двоичный объект Azure | X | X | X | X | X | X |
| Таблица Azure | X | X | X | X | X | X |
| База данных SQL Azure | X | X | X | X | X | X |
| Azure DocumentDB. | X | X | X | | | |  
| Локальный сервер SQL Server | X | X | X | | X | X |
| SQL Server на виртуальной машине Azure | X | X | X | | X | X |
| локальная файловая система; | X | X | X | | X | X |
| Локальная база данных Oracle | X | X | X | | X | X |
| Локальная база данных MySQL| X | X | X | | X | X |
| Локальная база данных DB2 | X | X | X | | X | X |
| Локальная база данных Teradata | X | X | X | | X | X |
| Локальная база данных Sybase | X | X | X | | X | X |
| Локальная база данных PostgreSQL | X | X | X | | X | X |
...
|type|Задает тип действия. Для type необходимо задать значение **Copy** |Да|
|inputs|Входные таблицы, используемые действием. Укажите только одну входную таблицу для действия копирования. | Да
|outputs|Выходные таблицы, используемые действием. Укажите только одну выходную таблицу для действия копирования. | Да
|transformation|Свойства преобразования зависят от типа. Для действия копирования необходимо указать разделы source и sink (в разделе transformation). Дополнительная информация приведена далее в этой статье.|Да
|policy| Политики, которые влияют на поведение во время выполнения действия. Если не указано, будут использоваться значения по умолчанию. | Нет
...
Список типов и свойств источников и приемников, которые поддерживаются этими типами, см. в библиотеке MSDN в разделе [Поддерживаемые источники и приемники][msdn-supported-sources-sinks].
...
Следующий скрипт JSON определяет выходную таблицу **MyDemoBlob**, которая ссылается на большой двоичный объект Azure **MyBlob** в папке больших двоичных объектов **MySubFolder** в контейнере больших двоичных объектов **MyContainer**.
...
Для хранилищ данных, которые используют HTTPS-подключение, выбирайте для действия копирования HTTPS-подключение. Это позволит безопасно передавать данные по сети. Например, для **хранилища Azure** используйте в строке подключения **DefaultEndpointsProtocol = https**.
...
Пошаговое руководство по копированию данных из локальной базы данных SQL Server в хранилище больших двоичных объектов Azure с помощью действия копирования см. в статье [Использование конвейера для работы с локальными данными][use-onpremises-datasources].
