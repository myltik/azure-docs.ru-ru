...
1. Откройте Azure PowerShell и выполните следующие команды. Не закрывайте Azure PowerShell, пока выполняются описанные в учебнике инструкции. Если закрыть и снова открыть это окно, то придется вновь выполнять эти команды.
	...
	- Выполните командлет **Select-AzureRmSubscription <имя подписки>**, чтобы выбрать подписку, с которой вы собираетесь работать. Эта подписка должна совпадать с той, которая используется на портале Azure.
...
1. Создайте в папке C:\\ADFGetStarted JSON-файл MyFirstPipelinePSH.json со следующим содержимым.
	...
	> [ACOM.NOTE] Дополнительные сведения о свойствах JSON, используемых в приведенном выше примере, см. в разделе [Анатомия конвейера](data-factory-create-pipelines.md#anatomy-of-a-pipeline).
