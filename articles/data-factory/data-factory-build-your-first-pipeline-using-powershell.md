...
1. Создайте в папке **C:\\ADFGetStarted** JSON-файл с именем **InputTable.json** со следующим содержимым.
	...
	Указанный выше JSON-файл определяет набор данных с именем **AzureBlobInput**; этот набор представляет входные данные для действия в конвейере. Кроме того, файл указывает, что входные данные размещаются в контейнере BLOB-объектов **adfgetstarted** в папке **inputdata**.
...
1. Создайте в папке **C:\\ADFGetStarted** JSON-файл с именем **OutputTable.json** со следующим содержимым.
	...
	Указанный выше JSON-файл определяет набор данных с именем **AzureBlobOutput**; этот набор представляет выходные данные для действия в конвейере. Кроме того, файл указывает, что результаты хранятся в контейнере BLOB-объектов **adfgetstarted** в папке **partitioneddata**. В разделе **availability** указывается частота, с которой будет создаваться выходной набор данных (ежемесячно).
