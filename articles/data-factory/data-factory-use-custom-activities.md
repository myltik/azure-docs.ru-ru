...
12. Создайте ZIP-файл **MyDotNetActivity.zip**, который содержит все двоичные файлы из папки <папка проекта>\\bin\\Debug. Можно также добавить файл **MyDotNetActivity.pdb**, чтобы в случае сбоя получить дополнительные сведения, например номер строки в исходном коде, вызвавшем ошибку. Все файлы в ZIP-файле для настраиваемого действия должны размещаться на **верхнем уровне** без вложенных папок.
...
2. **Связанные службы** для пула виртуальных машин пакетной службы Azure, в котором будет выполняться настраиваемое действие, а также служба хранилища Azure, в которой содержатся входные и выходные большие двоичные объекты.
...
2. В редакторе фабрики данных щелкните на панели команд **Новое вычисление** и выберите в меню **Пакетная служба Azure**.
...
2. В сценарии JSON выполните следующее.
	...
	4. Для свойства **LinkedServiceName** укажите **AzureStorageLinkedService**. Это связанная служба, которая была создана в учебнике по началу работы.
