...
> [AZURE.NOTE] Копирование данных из хранилищ данных в локальной среде или Azure IaaS в Azure DocumentDB и наоборот поддерживается через шлюз управления данными версии 2.1 и более поздних версий.
...
| **Свойство** | **Описание** | **Допустимые значения** | **Обязательный** |
| -------- | ----------- | -------------- | -------- |
| nestingSeparator | Такой специальный символ в имени исходного столбца, который указывает, что нужен вложенный документ. <br/><br/>См. пример выше: элемент Name.First в выходной таблице обуславливает в документе DocumentDB такую структуру JSON:<br/><br/>"Name": {<br/> "First": "John"<br/>}, | Знак, используемый для разделения уровней вложения.<br/><br/>Значение по умолчанию — "." (точка). | Символ, используемый для разделения уровней вложенности. <br/><br/>Значение по умолчанию — "." (точка). | Нет | 
