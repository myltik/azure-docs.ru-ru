...
| ------------ | --------------- | ------------------ | ------------ |
| запрос | Запрос, нужный для чтения данных. | Строка запроса, поддерживаемая базой данных DocumentDB. <br/>Пример. SELECT c.BusinessEntityID, c.PersonType, c.NameStyle, c.Title, c.Name.First AS FirstName, c.Name.Last AS LastName, c.Suffix, c.EmailPromotion FROM c WHERE c.ModifiedDate > "2009-01-01T00:00:00"<br/> | Нет <br/><br/>Если не указано другое, выполняется инструкция SQL: select <columns defined in structure> from mycollection 
