...
-   Метод принимает четыре параметра:
    ...
    4.  **logger** — средство ведения журнала позволяет писать комментарии отладки, которые отобразятся в виде пользовательского журнала для конвейера.
...
6.  В [обозревателе пакетной службы Azure](http://blogs.technet.com/b/windowshpc/archive/2015/01/20/azure-batch-explorer-sample-walkthrough.aspx) просмотрите **задачи**, связанные со **срезами**, а также сведения о том, на какой виртуальной машине выполнялся каждый срез. Задание создается с именем **adf-<имя\_пула>**. Эта задача будет содержать по одному заданию для каждого среза. В этом примере будет использоваться 5 срезов, соответствующих 5 задачам в пакетной службе Azure. Так как для свойства **concurrency** задано значение **5** в JSON-файле конвейера в фабрике данных Azure, а для параметра **Максимальное количество задач на виртуальную машину** — **2** в пуле пакетной службы Azure с **2** виртуальными машинами, задачи выполнялись очень быстро (см. значение времени в столбце **Создано**).
...
4.  Все файлы в ZIP-файле для настраиваемого действия должны находиться на **верхнем уровне** без вложенных папок.
