...
> [AZURE.IMPORTANT] Прежде чем начинать работу с этим руководством, изучите статью [Руководство. Копирование данных из хранилища BLOB-объектов Azure в базу данных SQL Azure](data-factory-get-started.md) и выполните все необходимые действия.
...
1.	После входа на [портал Azure](https://portal.azure.com) щелкните **Создать** в левом нижнем углу, выберите **Анализ данных** в колонке **Создать**, затем щелкните **Фабрика данных** в колонке **Анализ данных**. 
...
## Создание конвейера с помощью мастера копирования
...
2. Вот что нужно сделать на странице **Свойства**:
	...
	2. Введите **описание** (необязательное действие).
...
3. На странице **Хранилище данных источников** щелкните элемент **Хранилище BLOB-объектов Azure**. На этой странице можно указать хранилище данных источников для задачи копирования. Вы можете использовать существующую связанную службу хранилища данных или указать новое хранилище данных. Чтобы использовать существующую связанную службу, щелкните элемент **Из существующих связанных служб** и выберите нужную службу. 
...
5. Вот что нужно сделать на странице **Указание учетной записи хранилища BLOB-объектов Azure**:
	1. В качестве **имени связанной службы** введите **AzureStorageLinkedService**.
	2. Подтвердите, что в качестве **метода выбора учетной записи** нужно использовать метод **Из подписок Azure**. 
	3. Выберите **учетную запись хранилища Azure** из списка учетных записей хранилища Azure, доступных в выбранной вами подписке. Кроме того, настроить параметры учетной записи хранения можно вручную. Для этого в качестве **метода выбора учетной записи** выберите вариант **Ввести вручную** и щелкните **Далее**. 
...
6. Вот что нужно сделать на странице **Выбор входного файла или папки**:
...
7. На странице **Параметры формата файла** выберите значения **по умолчанию** и щелкните **Далее**.
...
8. На странице "Целевое хранилище данных" щелкните элемент **База данных SQL Azure** и нажмите кнопку **Далее**.
9. Вот что нужно сделать на странице **Указание базы данных SQL Azure**:
	1. В поле **Имя связанной службы** введите **AzureSqlLinkedService**. 
	2. Убедитесь, что в качестве **метода выбора базы данных или сервера** выбран вариант **Из подписок Azure**.
	3. Выберите **имя сервера** и **базу данных**.
	4. Введите **имя пользователя** и **пароль**.
	5. Нажмите кнопку **Далее**.  
9. На странице **Сопоставление таблиц** выберите **emp** в поле **Назначение** из раскрывающегося списка, щелкните **стрелку вниз** (необязательно), чтобы отобразить схему и выполнить предварительный просмотр данных.

	![Средство копирования — сопоставление таблиц](./media/data-factory-copy-data-wizard-tutorial/copy-tool-table-mapping-page.png) 
10. На странице **Сопоставление схем** щелкните **Далее**.
...
12. На странице **Развертывание прошло успешно** щелкните **Щелкните здесь, чтобы отслеживать конвейер копирования**.
...
13. Чтобы получить сведения о том, как отслеживать созданный вами конвейер, изучите инструкции в статье [Мониторинг конвейеров фабрики данных Azure и управление ими с помощью нового приложения по мониторингу и управлению](data-factory-monitor-manage-app.md).
