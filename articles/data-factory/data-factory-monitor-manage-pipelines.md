...
4. В колонке «Фабрики данных» выберите фабрику данных, которая вас интересует. Откроется домашняя страница для этой фабрики данных (колонка \*\*Фабрика данных\*\*).
...
1. Щелкните конвейер правой кнопкой мыши и выберите команду **Открыть конвейер**, чтобы отобразить все действия в нем вместе с их входными и выходными наборами данных. Это удобно, когда конвейер состоит из нескольких действий, а вы хотите понять структуру взаимосвязей во всем конвейере.
...
Состояние | Дополнительное состояние | Описание
------ | ---------- | -----------
Waiting | ScheduledTime<br/>DatasetDependencies<br/>ComputeResources<br/>ConcurrencyLimit<br/>ActivityResume<br/>Retry<br/>Validation<br/>ValidationRetry | Ожидание выполнения предварительных условий до выполнения действия. Чтобы узнать, какое именно условие не выполнено, см. список дополнительных состояний.
In-Progress | Starting<br/>Configuring<br/>Allocating Resources<br/>Running<br/>Validating | Действие выполняется: создание и проверка данных для конкретного среза.
Failed | | Сбой при обработке среза. Причину сбоя см. в журнале ошибок.
Ready | | Обработка среза выполнена успешно. Срез готов к использованию.
Skip | | Срез не обработан.

Вы можете просмотреть сведения о срезе, щелкнув запись среза в колонке **Недавно обновленные срезы**.
...
Вы можете просмотреть сведения о выполнении действия, щелкнув запись прогона в списке **Циклы выполнения действия**. Будут отображаться все файлы журналов и сообщения об ошибках, если таковые были. Это позволяет просмотреть и обработать файлы журналов непосредственно из фабрики данных.

![Сведения о цикле выполнения действия](./media/data-factory-monitor-manage-pipelines/activity-run-details.png)

Если состояние среза отличается от **Ready**, в списке **Неготовые вышестоящие срезы** отобразятся вышестоящие срезы, которые не находятся в состоянии Ready и блокируют выполнение текущего среза. Это позволяет просмотреть вышестоящие зависимости, если срез находится в состоянии **Waiting**.
...
Изначально срезы находятся в состоянии **Waiting** (Ожидание). Это означает, что не все предварительные условия выполнены. Затем начинается выполнение действия, и срез переходит в состояние **In-Progress**. В зависимости от того, чем завершится выполнение действия, срез может перейти в состояние **Ready** (Готово) или **Failed** (Сбой).

Пользователь может сбросить срез, находящийся в состоянии **Ready** или **Failed**, в состояние **Waiting**. Он также может пометить срез флагом **Skip** (Пропустить), что приведет к тому, что действие со срезом не будет выполнено и он не будет обработан.
...
Например, на следующем снимке экрана показано, что выявлена проблема с конвейером **PartitionProductsUsagePipeline** в фабрике **productrecgamalbox1dev**, и конвейер нужно приостановить.

![Конвейер будет приостановлен](./media/data-factory-monitor-manage-pipelines/pipeline-to-be-suspended.png)

Выполните следующую команду PowerShell, чтобы приостановить конвейер **PartitionProductsUsagePipeline**.
...
После устранения проблемы с конвейером **PartitionProductsUsagePipeline** можно возобновить его работу, выполнив следующую команду PowerShell.
...
4.	В колонке **СРЕЗ ДАННЫХ** щелкните цикл выполнения действия, который завершился неудачно.
	
	![Срез данных с ошибкой](./media/data-factory-monitor-manage-pipelines/dataslice-with-error.png)
5.	В колонке **СВЕДЕНИЯ О ЦИКЛЕ ВЫПОЛНЕНИЯ ДЕЙСТВИЯ** вы можете скачать файлы, связанные с обработкой HDInsight. Чтобы скачать файл журнала ошибок, содержащий подробные сведения об ошибке, щелкните Скачать для Status/stderr (Состояние/stderr).
...
4. Теперь запустите командлет **Get-AzureDataFactoryRun**, чтобы получить подробные сведения о цикле выполнения действия для среза.
...
После устранения неполадок и отладки ошибок в конвейере можно повторно обработать срез с ошибками. Для этого выберите срез и нажмите кнопку **Запустить** на панели команд.

![Повторная обработка среза](./media/data-factory-monitor-manage-pipelines/rerun-slice.png)

Если срез не прошел проверку из-за ошибки политики (например, данные недоступны), вы можете исправить ошибку и повторно выполнить проверку, нажав кнопку **Проверить** на панели команд. ![Исправление ошибок и проверка](./media/data-factory-monitor-manage-pipelines/fix-error-and-validate.png)
...
**Примечание.** Для UpdateType задано значение UpstreamInPipeline. Это означает, что каждый срез в этой таблице и всех зависимых (вышестоящих) таблицах, используемых в качестве входных таблиц для действия в конвейере, переходит в состояние PendingExecution. Этот параметр может иметь еще одно значение — Individual.
...
Чтобы просмотреть все настроенные оповещения, используя PowerShell, выполните следующую команду. Будут показаны все оповещения, настроенные для событий и для метрик с типом ресурса **microsoft.insights/alertrules**.
...
Введите необходимые сведения для правила, укажите адреса электронной почты для оповещений и нажмите кнопку **ОК**.
...
*metricName* на данный момент поддерживает два значения: - FailedRuns и - SuccessfulRuns.
