...
1. Создайте связанную службу Машинного обучения Azure. Вам потребуется следующее:
	1. **URI запроса** для API выполнения пакета. Чтобы найти URI запроса, перейдите по ссылке **ВЫПОЛНЕНИЕ ПАКЕТА** на странице веб-служб, как показано ниже.
