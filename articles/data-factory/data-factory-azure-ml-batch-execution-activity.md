...
1. В веб-службу оценки добавьте конечную точку, которая не будет являться точкой по умолчанию. Конечная точка веб-службы по умолчанию не допускает изменений, поэтому вам нужно будет создать новую точку с помощью портала Azure. Основные сведения и инструкции см. в статье [Создание конечных точек](../machine-learning/machine-learning-create-endpoint.md).
...
2. Если модель вас устраивает, то опубликуйте веб-службы для **обучающего эксперимента** и оценочного или **прогнозного эксперимента** с помощью Студии машинного обучения Azure.
