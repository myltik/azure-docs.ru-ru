...
1. **Создание обучающего эксперимента**. Это можно сделать с помощью Студии машинного обучения Microsoft Azure, которая представляет собой среду совместной визуальной разработки, используемую для обучения и тестирования модели прогнозной аналитики с использованием данных для обучения, предоставляемых вами.
...
3. **Развертывание эксперимента в виде веб-службы**. Оценивающий эксперимент можно опубликовать в виде веб-службы Azure. С помощью конечной точки этой веб-службы пользователи смогут отправлять данные в вашу модель и получать из нее результаты прогнозов.  
...
1. В веб-службу оценки добавьте конечную точку, которая не будет являться точкой по умолчанию. Конечная точка веб-службы по умолчанию не допускает изменений, поэтому вам нужно будет создать новую точку с помощью классического портала Azure. Основные сведения и инструкции см. в статье [Создание конечных точек](../machine-learning/machine-learning-create-endpoint.md).
...
## Обращение к веб-службе машинного обучения Azure с помощью действия "Выполнение пакета"
...
1. Создайте связанную службу Машинного обучения Azure. Вам потребуется следующее:
	1. **Универсальный код ресурса (URI) запроса** для API пакетного выполнения. Чтобы найти URI запроса, перейдите по ссылке **ВЫПОЛНЕНИЕ ПАКЕТА** на странице веб-служб, как показано ниже.
...
Прежде чем приступить к этому примеру, рекомендуется изучить руководство [Создание первого конвейера с помощью фабрики данных Azure][adf-build-1st-pipeline] и с помощью редактора фабрики данных создать артефакты фабрики данных (связанные службы, наборы данных, конвейер) для этого примера.
...
При использовании модуля чтения в эксперименте Машинного обучения Azure вы можете указать в качестве входных данных большой двоичный объект Azure. Файлы в хранилище больших двоичных объектов Azure могут представлять собой выходные файлы (например, 000000\_0), созданные с помощью сценария Pig и Hive, который выполняется в HDInsight. Модуль чтения позволяет читать файлы (без расширений). Для этого необходимо настроить свойство **Путь к контейнеру, каталогу или большому двоичному объекту** модуля чтения таким образом, чтобы оно указывало на контейнер или папку, содержащие файлы, как показано ниже. Обратите внимание, что звездочка (\*) **указывает на т, что все файлы в контейнере или папке (например, data/aggregateddata/year=2014/month-6/\*)** будут прочитаны в рамках эксперимента.
...
В веб-службе пакетного выполнения Azure ML выходные данные могут быть не настроены. В данном примере не настроены ни входные или выходные параметры, ни глобальные параметры. Обратите внимание на то, что выходные данные в действии настроены, но не назначены в качестве выходных данных в свойстве webServiceOutput.
...
Модули чтения и записи веб-службы Azure ML могут быть настроены на работу с использованием или без использования глобальных параметров. Вызовы службы можно встроить в конвейер пакетной обработки с использованием зависимостей наборов данных для вызова службы только при успешном завершении обработки вышестоящих данных, а затем запустить другое действие после окончания пакетного выполнения. В этом случае зависимости можно представить, используя входные и выходные данные действия, но не называя их входными или выходными данными веб-службы.
...
-   Если в конечной точке эксперимента используются входные данные веб-службы, они представляются набором данных BLOB-объектов и включаются во входные данные действия, а также свойство webServiceInput. В противном случае свойство webServiceInput опускается. 
-   Если в конечной точке эксперимента используются выходные данные веб-службы, они представляются набором данных BLOB-объектов и включаются в выходные данные действия, а также свойство webServiceOutput (сопоставленное с именем входных данных в эксперименте). В противном случае свойство webServiceOutputs опускается.
...
## Обновление моделей машинного обучения Azure с помощью действия "Обновить ресурс"
...
2. Если модель вас устраивает, опубликуйте веб-службы для **обучающего эксперимента** и /**прогнозного эксперимента** оценки с помощью Студии машинного обучения Azure.
...
| :------------------ | :---------- 
| **Веб-служба обучения** | Получает данные обучения и создает обученные модели. Результат повторного обучения — файл формата ILEARNER в хранилище BLOB-объектов Azure. При публикации обучающего эксперимента в виде веб-службы автоматически создается **конечная точка по умолчанию**. Вы можете создать несколько конечных точек, но в этом примере используется только конечная точка по умолчанию. |
| **Веб-служба оценки** | Получает данные без метки и осуществляет прогнозирование. Результаты прогноза могут записываться в различных форматах, например в CSV-файлы или строки в базе данных SQL Azure (в зависимости от конфигурации эксперимента). При публикации прогнозного эксперимента в виде веб-службы автоматически создается конечная точка по умолчанию. Вам потребуется создать вторую **обновляемую конечную точку не по умолчанию** с помощью [классического портала Azure](https://manage.windowsazure.com). Вы можете создать несколько конечных точек, но в этом примере используется только одна обновляемая конечная точка не по умолчанию. Инструкции см. в статье [Создание конечных точек](../machine-learning/machine-learning-create-endpoint.md).       
...
Чтобы обратиться к **веб-службе обучения**, воспользуйтесь **действием "Выполнение пакета" машинного обучения Azure**. Это действие аналогично обращению к веб-службе машинного обучения Azure (веб-службе оценки) для получения данных оценки. В приведенных выше разделах подробно описано, как обращаться к веб-службе машинного обучения Azure из конвейера фабрики данных.
  
Чтобы обновить веб-службу с помощью заново обученной модели, вы можете обратиться к **веб-службе оценки**, воспользовавшись **действием "Обновить ресурс" машинного обучения Azure**. Как уже упоминалось в таблице выше, вам потребуется создать и использовать обновляемую конечную точку не по умолчанию. Также вам потребуется обновить все связанные службы, имеющиеся в фабрике данных, для использования новой конечной точки не по умолчанию. Таким образом, эти службы всегда будут использовать новые, заново обученные модели.

Ниже приведен сценарий, в котором более подробно показано повторное обучение и обновление моделей машинного обучения Azure из конвейера фабрики данных Azure.
...
- Файл iLearner. Это выходные данные из веб-службы машинного обучения Azure. Эти данные также являются входными для действия «Обновить ресурс».  
...
3. Рядом с полем **Ключ API** нажмите кнопку копирования ключа API в буфер обмена. Вставьте ключ в редактор JSON фабрики данных.
4. В **Студии машинного обучения Azure** щелкните **ВЫПОЛНЕНИЕ ПАКЕТА**, скопируйте **URI запроса** из раздела **Запрос** и вставьте его в редактор JSON фабрики данных.   
...
Создав обновляемую конечную точку не по умолчанию, щелкните **ВЫПОЛНЕНИЕ ПАКЕТА**, чтобы получить значение URI свойства JSON **mlEndpoint**. Затем щелкните ссылку **ОБНОВИТЬ РЕСУРС**, чтобы получить значение URI свойства JSON **updateResourceEndpoint**. Ключ API указывается на странице конечной точки (в правом нижнем углу).
...
Действие «Обновить ресурс» машинного обучения Azure не создает никаких выходных данных, однако фабрике данных Azure требуется выходной набор данных. Это необходимо для планирования конвейера, поэтому в этом примере используется фиктивный набор данных (заполнитель набора данных).
...
Добавьте раздел **typeProperties** в раздел **AzureMLBatchScoringActivty** в конвейере JSON, чтобы указать в этом разделе значения для параметров веб-службы, как показано в примере ниже.
