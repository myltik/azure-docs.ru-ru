...
Подробные сведения см. в разделе [Обновление моделей с помощью действия обновления ресурса](#updating-azure-ml-models-using-the-update-resource-activity).
...
1. Создайте связанную службу Машинного обучения Azure. Кроме этого, вам потребуются:
   ...
   2. **Ключ API** для опубликованной веб-службы машинного обучения Azure. Чтобы найти ключ API, щелкните опубликованную веб-службу.
      ...
      1. Используйте действие **AzureMLBatchExecution** .
...
Чтобы обновить веб-службу с помощью заново обученной модели, можно вызвать **веб-службу оценки**, воспользовавшись **действием ресурса обновления машинного обучения Azure**. Как уже упоминалось в таблице выше, вам потребуется создать и использовать обновляемую конечную точку не по умолчанию. Кроме того, вам потребуется обновить все связанные службы, имеющиеся в фабрике данных, для использования новой конечной точки не по умолчанию. Таким образом, эти службы всегда будут использовать новые, заново обученные модели.
