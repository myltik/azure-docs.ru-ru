...
В этой статье рассказывается, как с помощью действия копирования в фабрике данных Azure можно перемещать данные из хранилища данных SQL Azure в другое хранилище данных SQL Azure и обратно.
...
Если источник данных соответствует приведенным ниже критериям, можно выполнять копирование напрямую из исходного хранилища данных в хранилище данных SQL Azure, используя PolyBase. В противном случае можно скопировать данные из исходного хранилища данных в промежуточное хранилище BLOB-объектов Azure, которое соответствуют следующим критериям, а затем воспользоваться PolyBase для загрузки данных в хранилище данных SQL Azure. Дополнительные сведения о промежуточном копировании см. в разделе [Рекомендации по использованию PolyBase](#staged-copy-using-polybase).

Обратите внимание, что фабрика данных Azure проверяет параметры и автоматически возвращается к механизму перемещения данных BULKINSERT, если требования не выполняются.
...
2. **Входной набор данных** имеет тип **большой двоичный объект Azure**; свойства типа набора данных удовлетворяют следующим критериям: 
	1. параметр **Type** имеет значение **TextFormat** или **OrcFormat**; 
...
Для механизма PolyBase исходные данные должны находиться в хранилище BLOB-объектов Azure и иметь один из поддерживаемых форматов (DELIMITEDTEXT с ограничением, RCFILE, ORC, PARQUET). Если исходные данные не соответствуют описанным выше требованиям, можно включить копирование данных в промежуточное хранилище BLOB-объектов Azure. В таком случае в фабрике данных Azure данные будут преобразованы и приведены в соответствие с требованиями к формату PolyBase. Затем воспользуйтесь PolyBase для загрузки данных в хранилище SQL. Подробные сведения о том, как работает копирование данных через промежуточное хранилище BLOB-объектов Azure см. в разделе [Staged Copy](data-factory-copy-activity-performance.md#staged-copy) (Промежуточное копирование).

Чтобы использовать эту функцию, создайте [связанную службу хранилища Azure](data-factory-azure-blob-connector.md#azure-storage-linked-service), относящуюся к учетной записи хранения Azure, которая содержит это промежуточное хранилище BLOB-объектов. Затем укажите свойства **enableStaging** и **stagingSettings** для действия копирования, как показано ниже.
...
Примечание. При копировании данных из локального хранилища данных в хранилище данных SQL Azure с помощью PolyBase и промежуточного хранения необходимо установить JRE (среда выполнения Java) на компьютере шлюза, который будет использоваться для преобразования исходных данных в правильный формат.
