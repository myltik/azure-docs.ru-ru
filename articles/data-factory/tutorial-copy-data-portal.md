...
10. Перейдите на вкладку **Подключение** в окне свойств.   

    ![Вкладка "Подключение"](./media/tutorial-copy-data-portal/source-dataset-connection-tab.png)
11. Щелкните **+Создать** рядом с текстовым полем **Связанная служба**. Связанная служба привязывает хранилище данных или вычислительный ресурс к фабрике данных. В нашем примере вы создаете связанную службу хранилища Azure, чтобы связать учетную запись хранения Azure с хранилищем данных. Связанная служба содержит сведения о подключении, которые служба фабрики данных применяет для подключения к хранилищу BLOB-объектов во время выполнения. Набор данных определяет контейнер, папку и (необязательно) файл, где содержатся исходные данные. 
...
25. В окне **New Linked Service** (Новая связанная служба) выполните следующие действия: 
    ...
    2. Выберите нужный сервер Azure SQL в поле **Имя сервера**.
    4. Выберите базу данных Azure SQL в поле **Имя базы данных**. 
    5. Введите имя пользователя в поле **Имя пользователя**. 
    6. Введите пароль для этого пользователя в поле **Пароль**. 
    7. Нажмите кнопку **Проверить подключение**, чтобы проверить подключение.
...
3. Щелкните **Опубликовать** в области слева. Это действие опубликует созданные сущности (связанные службы, наборы данных и конвейеры) в фабрике данных Azure.
...
1. Щелкните **Триггер** на панели инструментов, а затем **Активировать сейчас**. 
...
7. Щелкните **Опубликовать**, чтобы опубликовать изменения в репозитории. Триггер не будет срабатывать, пока публикация не завершится успешно. 

    ![Публикация триггера](./media/tutorial-copy-data-portal/publish-trigger.png) 
8. Перейдите на вкладку **Мониторинг** слева, чтобы увидеть активированные выполнения конвейера. 
