...
## Поток данных при использовании шлюза
...
- Можно скачать установочный пакет MSI из [Центра загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=39717). Пакет MSI также может использоваться для обновления имеющегося шлюза управления данными до последней версии с сохранением всех параметров.
...
3. Сразу запустите **MSI**-файл или сохраните его на жесткий диск и затем выполните.
...
7. На странице **Все готово для установки** щелкните **Установить**.
...
10. На странице **Регистрация шлюза** **диспетчера конфигурации шлюза управления данными**, выполняемого на вашем компьютере, вставьте ключ в текстовое поле. Чтобы увидеть текст ключа, можно установить флажок **Показать ключ шлюза**. Теперь щелкните **Зарегистрировать**.

### Получение ключа

#### Если логический шлюз на портале еще не создан
Выполните действия из пошагового руководства в статье [Перемещение данных между локальными источниками и облаком с помощью шлюза управления данными](data-factory-move-data-between-onprem-and-cloud.md), чтобы создать новый шлюз на портале, и получите ключ из колонки **Настройка**.

#### Если логический шлюз на портале уже создан
1. На портале Azure перейдите к колонке **Фабрика данных**, затем щелкните элемент **Связанные службы**.

	![Колонка "Фабрика данных"](media/data-factory-data-management-gateway/data-factory-blade.png)
2. В колонке **Связанные службы** выберите логический **шлюз**, который был создан на портале.
...
## Диспетчер конфигурации шлюза управления данными 
...
- просмотреть, изменить и экспортировать **сертификат**, который используется шлюзом;
...
- просмотреть **SSL-сертификат**, используемый для задания учетных данных источников данных.
...
Описанные здесь рекомендации относятся к двум брандмауэрам: к **корпоративному брандмауэру**, работающему на центральном корпоративном маршрутизаторе, и к **брандмауэру Windows** на локальном компьютере, на котором установлен шлюз.

![брандмауэры](./media/data-factory-data-management-gateway/firewalls.png)

## Подключение шлюза к облачным службам
Чтобы обеспечить подключение шлюза к фабрике данных Azure и другим облачным службам, необходимо настроить правило для исходящего трафика **TCP**-портов **80** и **443**. При желании можно разрешить и порты **9350**–**9354**, которые служебная шина Microsoft Azure использует для подключения фабрики данных Azure к шлюзу управления данными. Использование этих портов может повысить эффективность обмена данными между этими компонентами.
...
| *.servicebus.windows.net | 9350-9354 | Дополнительный ретранслятор служебной шины по протоколу TCP | 
...
## Настройка учетных данных шлюза
...
## Устранение неполадок в работе шлюза
...
- При возникновении ошибок, связанных с подключением к хранилищу данных или с драйвером, запустите **диспетчер конфигурации шлюза управления данными** на компьютере шлюза и перейдите на вкладку **Диагностика**. Там выберите или введите соответствующие значения полей в разделе **Проверить подключение к локальному источнику данных с помощью этого шлюза** и нажмите кнопку **Проверить подключение**. Так вы сможете проверить, можно ли подключиться к локальному источнику данных с компьютера шлюза с этими параметрами подключения и учетными данными. Если после установки драйвера тестовое подключение по-прежнему не работает, перезапустите шлюз для того, чтобы получить последние изменения.
...
3. Выбрав все, что нужно передать, щелкните **Отправить журналы**, чтобы действительно отправить журналы за последние 7 дней в корпорацию Майкрософт для диагностики и устранения неполадок. Вы увидите состояние отправки журналов, как показано ниже.
...
Щелкните ссылку **Архивировать журналы шлюза**, чтобы заархивировать и сохранить журналы, а затем предоставить полученный ZIP-файл службе поддержки Майкрософт.
...
1. Запустите веб-браузер на **компьютере шлюза**, на [портале Azure](http://portal.azure.com) найдите свою фабрику данных и откройте ее в колонке **Фабрика данных**, а затем выберите **Создать и развернуть**, чтобы запустить редактор фабрики данных.
1. Щелкните существующую **связанную службу** в иерархической структуре, чтобы просмотреть определение JSON для этой службы, или создайте новую связанную службу, которой требуется шлюз управления данными (например, SQL Server или Oracle).
...
5. Нажмите кнопку **Зашифровать** на панели команд, чтобы запустить приложение ClickOnce **Диспетчер учетных записей**. Откроется диалоговое окно **Setting Credentials** (Настройка учетных данных). ![Диалоговое окно "Настройка учетных данных"](./media/data-factory-data-management-gateway/setting-credentials-dialog.png)
...
Вы можете установить учетные данные для локального источника данных с помощью приложения **настройки учетных данных**, запустив его на портале Azure. При этом портал зашифрует учетные данные с помощью сертификата, который указан на вкладке **Сертификат** в **диспетчере конфигурации шлюза управления данными** на компьютере шлюза.
