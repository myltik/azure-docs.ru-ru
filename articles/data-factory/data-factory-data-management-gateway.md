...
> [AZURE.NOTE] В настоящее время шлюз поддерживает действия копирования и действие хранимой процедуры только в фабрике данных. Невозможно использовать шлюз из настраиваемого действия для доступа к локальным источникам данных. 
...
### <a name="install-gateway-from-download-center"></a>Установка шлюза из центра загрузки
...
#### <a name="if-you-haven't-already-created-a-logical-gateway-in-the-portal"></a>Если логический шлюз на портале еще не создан
...
1. На портале Azure перейдите к колонке **Фабрика данных** и щелкните элемент **Связанные службы**.
...
### <a name="configure-proxy-server-settings-in-diahost.exe.config"></a>Настройка параметров прокси-сервера в файле diahost.exe.config
...
### <a name="to-disable/enable-auto-update-feature"></a>Включение или отключение функции автоматического обновления
...
#### <a name="gateway-cannot-connect-to-cloud-service-through-service-bus"></a>Шлюзу не удается подключиться к облачной службе через служебную шину
...
#### <a name="error:-cloud-service-cannot-connect-to-gateway-through-service-bus."></a>Ошибка: облачной службе не удается подключиться к шлюзу через служебную шину.
Выполните следующие действия, чтобы перевести шлюз в состояние "в сети".
...
## <a name="move-gateway-from-a-machine-to-another"></a>Перемещение шлюза с одного компьютера на другой
Этот раздел содержит процедуру перемещения клиента шлюза с одного компьютера на другой. 

2. На портале перейдите на **главную страницу фабрики данных**, а затем щелкните элемент **Связанные службы**. 
...
4. В колонке **Шлюз данных** щелкните **Скачивание и установка шлюза данных**.
...
8. Вставьте **ключ** в соответствующее текстовое поле на странице **Регистрация шлюза** **диспетчера конфигурации шлюза управления данными** на своем компьютере. (Необязательно) Чтобы увидеть текст ключа, установите флажок **Показать ключ шлюза** . 
...
5. Нажмите кнопку **Зашифровать** на панели команд, чтобы запустить ClickOnce-приложение **Диспетчер учетных данных**. Откроется диалоговое окно **Setting Credentials** (Настройка учетных данных). 
...
5. Вы можете использовать командлет **Get-AzureRmDataFactoryGateway** , чтобы получить список шлюзов своей фабрики данных. Если в поле **Состояние** указано значение **Подключено**, то шлюз готов к использованию.
