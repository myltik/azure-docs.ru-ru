...
1. Выберите контейнер BLOB-объектов в обозревателе сервера. В окне контейнера нажмите кнопку **Передать BLOB-объект**.
...
- Когда контейнер открыт в режиме просмотра, нажмите кнопку **Передать BLOB-объект**.

    Можно выбрать один или несколько файлов для отправки и отправлять файлы любого типа. В **журнале действий Azure** отображается ход отправки. Дополнительные сведения о работе с данными BLOB-объектов см. в статье [Использование службы хранилища BLOB-объектов Azure в .NET](http://go.microsoft.com/fwlink/p/?LinkId=267911).
...
Таблица упорядочена по сущностям (отображаются в строках) и свойствам (отображаются в столбцах). Например, на следующем рисунке показаны сущности, представленные в **конструкторе таблиц**:
...
1. В **конструкторе таблиц** откройте контекстное меню для сущности (одна строка) или свойства (одна ячейка) и выберите пункт **Изменить**.
    ...
    Сущности в одной таблице не обязательно должны иметь одинаковый набор свойств (столбцов). Учитывайте следующие ограничения на просмотр и редактирование данных таблицы. Двоичные данные (типа Byte) нельзя просматривать и редактировать, но их можно сохранять в таблице.
