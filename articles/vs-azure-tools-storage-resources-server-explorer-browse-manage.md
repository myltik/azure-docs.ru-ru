...
Начиная с версии Azure SDK 2.7, для просмотра ресурсов Azure и управления ими можно также использовать новый Cloud Explorer. Дополнительные сведения см. в статье [Управление ресурсами Azure с помощью Cloud Explorer](https://msdn.microsoft.com/library/azure/mt185741.aspx).
...
1. Введите имя нового контейнера в диалоговом окне **Создание контейнера BLOB-объектов** и нажмите кнопку **ОК**. ![Добавление нового контейнера BLOB-объектов](./media/vs-azure-tools-storage-resources-server-explorer-browse-manage/IC744153.bmp)
...
- Откройте контекстное меню для контейнера BLOB-объектов, который нужно удалить, и выберите **Удалить**.

###Отображение списка элементов, содержащихся в контейнере BLOB-объектов

- Откройте контекстное меню для имени контейнера BLOB-объектов в списке и выберите **Просмотреть контейнер BLOB-объектов**.
...
1. В диалоговом окне **Отправка нового файла** нажмите кнопку **Обзор**, чтобы указать файл, который нужно отправить, а затем введите имя папки в поле **Папка (необязательно)**.
...
1. Чтобы просмотреть содержимое папки, дважды щелкните папку или нажмите клавишу ВВОД. Находясь в папке контейнера, можно перейти обратно в корень контейнера, нажав кнопку **Открыть родительский каталог** (стрелка вверх).
...
Например, если ввести префикс `hello` в текст фильтра и нажать кнопку **Выполнить** (**!**), отобразятся только BLOB-объекты, начинающиеся со слова «hello».
...
- В обозревателе сервера откройте контекстное меню для одного или нескольких BLOB-объектов и выберите **Открыть**, или выберите имя BLOB-объекта и нажмите кнопку **Открыть**, или дважды щелкните имя BLOB-объекта.

    В окне **Журнал действий Azure** отображается ход выполнения загрузки.
    ...
    Данные BLOB-объекта кэшируются локально и проверяются на соответствие данным последнего изменения объекта в службе BLOB-объектов. Если BLOB-объект изменился со времени последней загрузки, он будет загружен снова. В противном случае он загрузится с локального диска. По умолчанию BLOB-объект загружается во временный каталог. Чтобы загрузить BLOB-объекты в определенный каталог, откройте контекстное меню для выбранных имен BLOB-объектов и выберите **Сохранить как**. При таком способе сохранения BLOB-объекта файл не открывается, и локальный файл создается с атрибутами чтения и записи.
...
- Чтобы просмотреть представление контейнера BLOB-объектов, при открытом контейнере нажмите кнопку **Отправить BLOB-объект**.
    ...
    Можно выбрать один или несколько файлов для отправки и отправлять файлы любого типа. **Журнал действий Azure** отображает ход отправки. Дополнительную информацию о работе с данными BLOB-объектов см. в статье [Использование службы хранилища BLOB-объектов Azure в .NET](http://go.microsoft.com/fwlink/p/?LinkId=267911).
...
- Откройте контекстное меню BLOB-объекта и выберите **Скопировать URL-адрес**.
...
Дополнительные сведения об очередях службы хранилища см. в статье [Практическое руководство. Использование служб хранилища очередей](http://go.microsoft.com/fwlink/?LinkID=264702). Сведения о веб-службе очередей службы хранилища см. в статье [Основные понятия службы очередей](http://go.microsoft.com/fwlink/?LinkId=264788). Сведения об отправке сообщения в очередь службы хранилища с помощью Visual Studio см. в статье [Отправка сообщений в очередь служб хранилища](https://msdn.microsoft.com/library/azure/jj649344.aspx).
...
1. В обозревателе сервера выберите узел **Таблицы** учетной записи хранения, а затем выберите **Создать таблицу**.
...
1. Чтобы просмотреть список таблиц для учетной записи хранения, откройте требуемый узел учетной записи хранения и выберите узел **Таблицы**.
...
1. В **конструкторе таблиц** откройте контекстное меню для сущности (одна строка) или свойства (одна ячейка) и щелкните **Изменить**.
    ...
    - При вводе значения DateTime следует придерживаться формата, соответствующего языковым и региональным параметрам компьютера (например, ММ/ДД/ГГГГ ЧЧ:ММ:СС [AM|PM] для американского варианта английского языка).
...
[Доступ к службе хранения Azure](https://msdn.microsoft.com/library/azure/ee405490.aspx)
