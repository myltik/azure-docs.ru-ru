...
Чтобы группа управления Operations Manager могла подключиться к службе Operational Insights и зарегистрироваться в ней, ей нужен доступ к номеру порта ваших доменов и URL-адресов. При использовании прокси-сервера для обмена данными между сервером управления Operations Manager и службой Operational Insights необходимо убедиться, что соответствующие ресурсы доступны. Если используется брандмауэр, чтобы ограничить доступ к Интернету, его необходимо настроить таким образом, чтобы разрешить доступ к Operational Insights. В следующих таблицах перечислены порты, связанные с этими задачами.
...
6. В результатах поиска выберите **Серверы управления Operations Manager**. ![изображение поля поиска объектов](./media/operational-insights-proxy-firewall/proxyacct3.png)
7. Щелкните **ОК**, чтобы закрыть окно «Добавить запуск от имени учетной записи». ![изображение мастера запуска от имени профиля](./media/operational-insights-proxy-firewall/proxyacct4.png)
...
### Настройка прокси-сервера на каждом сервере управления для WinHTTP

1. Если для Operations Manager не был установлен накопительный пакет обновления 3 для Operations Manager 2012 R2 или накопительный пакет обновления 7 для Operations Manager 2012 SP1 (либо более поздней версии), откройте окно командной строки от имени администратора на сервере управления Operations Manager. В противном случае эту процедуру использовать не нужно.

2. Введите **netsh winhttp set proxy myproxy:80**.

3. Закройте окно командной строки и перезапустите службу работоспособности управления System Center.

4. Выполните шаг 2 на каждом сервере управления в группе управления.

### Настройка прокси-сервера на каждом сервере управления

1. Откройте консоль Operations Manager и выберите рабочую область **Администрирование**.

2. Щелкните **Управление устройствами**, а затем **Серверы управления**.

3. Щелкните правой кнопкой мыши имя каждого сервера управления, выберите **Свойства**, и затем введите данные на вкладке **Параметры прокси-сервера**. ![Вкладка «Параметры прокси-сервера»](./media/operational-insights-proxy-firewall/proxyms.png)
