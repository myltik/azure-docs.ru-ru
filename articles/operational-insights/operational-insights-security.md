...
**Хранение данных.** Объединенные метрики для каждого решения (прежнее название — пакеты аналитики) хранятся в базе данных SQL, размещенной в Microsoft Azure. Эти данные хранятся 390 дней. Данные индексированного поиска в журналах в среднем хранятся 10 дней, после чего данные очищаются. Если верхний предел в 20 миллионов записей для каждого типа данных достигается раньше, Operational Insights очищает данные раньше, чем через 10 дней. Если лимит данных за 10 дней не достигнут, Operational Insights ожидает, пока он не будет достигнут, чтобы выполнить очистку.
...
**Нормативные требования и сертификация**. Группа разработки и обслуживания Operational Insights активно сотрудничает с юридическим отделом и группой обеспечения соответствия требованиям корпорации Майкрософт, включая стандарт ISO, перед выпуском службы Operational Insights в свободный доступ.
