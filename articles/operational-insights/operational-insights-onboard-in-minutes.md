...
1. Откройте [портал Azure](https://manage.windowsazure.com) и выполните вход, а затем в списке служб выберите **Оперативная аналитика**. ![Портал Azure](./media/operational-insights-onboard-in-minutes/azure-portal-op-insights.png)
...
3. Выберите созданную рабочую область и нажмите **Войти в учетную запись оперативной аналитики**, чтобы открыть веб-сайт Operations Management Suite. ![посещение учетной записи](./media/operational-insights-onboard-in-minutes/visit-account.png)
4. На веб-сайте Operations Management Suite введите свой адрес электронной почты и нажмите **Confirm and continue** (Подтвердить и продолжить). Вам будет отправлено сообщение с подтверждением. Откройте сообщение и нажмите кнопку **Confirm now** (Подтвердить).
5. На сайте Operations Management Suite откроется страница "Overview" (Обзор). Выберите плитку **Get started** (Приступить) для продолжения.
...
Чтобы выбрать решения и подключиться к серверам, которыми нужно управлять, нажмите плитку **Get Started** (Приступить) и следуйте указаниям в этом разделе.

![создание рабочей области и привязка подписки](./media/operational-insights-onboard-in-minutes/get-started.png)

- Выберите нужные вам решения и нажмите кнопку **Add selected Solutions** (Добавить выбранные решения). ![решения](./media/operational-insights-onboard-in-minutes/solutions.png)
...
- Настройте как минимум один журнал для заполнения данных. Вы можете выбрать журналы IIS и/или добавить журналы событий и нажать кнопку **Save** (Сохранить) в нижней части страницы. Для журналов событий можно указать тип сообщений, включая сообщения об ошибках, предупреждения и сведения для отслеживания. ![решения](./media/operational-insights-onboard-in-minutes/logs.png)

## При желании вы можете подключит серверы непосредственно к Operations Management Suite, установив агент.
1. В представлении "Get Started" (Начало работы) выберите команду **Connect a data source** (Подключить источник данных), а затем нажмите кнопку **Download Windows Agent** (Скачать агент Windows). Агент можно установить в Windows Server 2008 с пакетом обновления 1 (SP1) или более поздней версии или Windows 7 с пакетом обновления 1 (SP1) или более поздней версии. Серверы должны иметь архитектуру x64.
...
3. Приняв условия лицензионного соглашения и выбрав папку для установки, установите флажок **Connect the agent to Microsoft Azure Operational Insights** (Подключить агент к службе оперативной аналитике Microsoft Azure). ![установка агента](./media/operational-insights-onboard-in-minutes/agent.png)
...
5. Во время установки можно нажать кнопку **Advanced** (Дополнительно), чтобы настроить прокси-сервер и указать сведения для проверки подлинности. Нажмите кнопку **Next** (Далее), чтобы вернуться на экран сведений о рабочей области.
6. Нажмите кнопку **Next** (Далее), чтобы проверить идентификатор и ключ рабочей области. Если обнаружены ошибки, вы можете нажать кнопку **Back** (Назад) и исправить их. После проверки идентификатора и ключа рабочей области нажмите кнопку **Install** (Установить), чтобы завершить установку агента.
7. Снова войдите на портал Operations Management Suite и нажмите плитку **Settings** (Параметры) на странице "Overview" (Обзор). Когда агенты взаимодействуют со службой Operations Management Suite, отображается зеленый значок галочки. В первый раз это занимает около 5–10 минут.
...
Вы также можете подключить агент к System Center Operations Manager 2012 с пакетом обновления 1 (SP1) или более поздней версии. Для этого можно выбрать элемент **Connect the agent to System Center Operations Manager** (Подключить агент к System Center Operations Manager). При выборе этой команды для отправки данных службе не требуется дополнительное оборудование или нагрузка на группы управления.

Дополнительные сведения о подключении агентов непосредственно к Operations Management Suite см. в статье [Подключение компьютеров непосредственно к службе оперативной аналитики](operational-insights-direct-agent.md).
...
4. Завершив регистрацию, щелкните ссылку **Добавление компьютера/группы**.
5. В диалоговом окне **Поиск компьютера** можно выполнить поиск компьютеров или групп, отслеживаемых с помощью Operations Manager. Выберите компьютеры или группы для подключения к службе оперативной аналитики, нажмите кнопку **Добавить**, а затем — кнопку **ОК**. Вы можете убедиться, что служба оперативной аналитики получает данные, нажав плитку **Usage** (Использование) на портале Operations Management Suite. Данные должны появиться в течение 5–10 минут.

Дополнительные сведения о подключении Operations Manager к Operations Management Suite см. в статье [Подключение к службе оперативной аналитики из System Center Operations Manager](operational-insights-connect-scom.md).
