...
В службах мультимедиа цифровые файлы отправляются в актив. Сущность **Актив** может содержать видео, аудио, изображения, коллекции отпечатков, текстовые каналы и файлы скрытых субтитров(и метаданные этих файлов).  После отправки этих файлов в ресурс содержимое сохраняется в безопасном расположении в облаке для дальнейшей обработки и потоковой передачи. 
...
Каждое задание может состоять из одной или нескольких задач в зависимости от типа обработки, которую необходимо выполнить. REST API позволяет создавать задания и связанные с ними задачи одним из двух способов: встроенными средствами с помощью свойств навигации задач в сущностях Job или с помощью пакетной обработки OData. Пакет SDK служб мультимедиа использует пакетную обработку, однако для удобства чтения примеров кода в этом разделе задачи определены встроенными средствами. Дополнительную информацию о пакетной обработке см. в статье [Пакетная обработка по протоколу OData](http://www.odata.org/documentation/odata-version-3-0/batch-processing/).
...
>[AZURE.NOTE] Так как службы мультимедиа созданы на платформе OData версии 3, ссылки на отдельные ресурсы в коллекциях свойств навигации ресурсов InputMediaAsset и OutputMediaAsset добавляются в виде пары "имя-значение" __metadata: uri. 
...
В следующем разделе мы настроим политику доставки для выходного ресурса задания (закодированного ресурса). В следующем коде показано, как запросить идентификатор выходного ресурса. 
...
### Связывание политики доставки активов с ресурсом

Следующий HTTP-запрос связывает указанную политику доставки с указанным ресурсом.  
...
>[AZURE.NOTE] Необходимо добавить имя файла для файла, который необходимо скачать, в значение **Path** сущности Locator, полученное в предыдущем разделе. Например, https://storagetestaccount001.blob.core.windows.net/asset-e7b02da4-5a69-40e7-a8db-e8f4f697aac0/BigBuckBunny.mp4? . . . 
...
После создания URL-адресов SAS можно будет поэтапно скачивать MP4-файлы, вставив URL-адрес в браузере (например, IE, Chrome, Safari).

Чтобы проверить URL-адрес адаптивной потоковой передачи, используйте следующие проигрыватели:
