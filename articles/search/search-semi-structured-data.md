...
---
# <a name="search-semi-structured-data-in-cloud-storage"></a>Поиск частично структурированных данных в облачном хранилище

В этом руководстве из двух частей показано, как выполнять поиск частично структурированных и неструктурированных данных с помощью службы "Поиск Azure". В этом руководстве показано, как выполнять поиск частично структурированных данных, таких как JSON, хранящихся в больших двоичных объектах Azure. Частично структурированные данные содержат теги или метки, отделяющие содержимое в данных. Они отличаются от структурированных данных тем, что формально не структурированы в соответствии с моделью данных, например, схемой реляционной базы данных.

В этой части будет рассматриваться:

> [!div class="checklist"]
> * создание и заполнение индекса внутри службы "Поиск Azure";
> * использование службы "Поиск Azure" для поиска индекса.

> [!NOTE]
> "Поддержка массива JSON — это предварительная версия возможности в службе "Поиск Azure". В настоящее время она недоступна на портале. Поэтому используется предварительная версия REST API, предоставляющая эту возможность, и инструмент клиента REST для вызова API."

## <a name="prerequisites"></a>Предварительные требования

Для работы с этим руководством:
* Завершите [предыдущее руководство](../storage/blobs/storage-unstructured-search.md).
    * В этом руководстве используется учетная запись хранения и служба поиска, созданные в предыдущем руководстве.
* Установите клиент REST и узнайте, как составлять HTTP-запросы.


## <a name="set-up-the-rest-client"></a>Настройка клиента REST

Для выполнения заданий из данного руководства требуется клиент REST. В целях этого руководства используется [Postman](https://www.getpostman.com/). Вы можете использовать другой клиент REST, если вы уже работали с одним из них.

После установки Postman запустите его.

Если вы впервые вызываете REST на Azure, ниже представлено краткое описание важных компонентов этого руководства. Метод запроса для каждого вызова в этом руководстве — POST. Заголовки ключей — Content-type и api-key. Значения ключей заголовков — application/json и admin key (ключ администратора — это заполнитель для первичного ключа поиска) соответственно. Тело — это расположение фактического содержимого вызова. В зависимости от используемого клиента возможны некоторые различия в способах создания запросов, однако эти являются основными.
