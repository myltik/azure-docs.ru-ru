...
Эта служба предоставляется через простой [REST API](/rest/api/searchservice/) или [пакет SDK для .NET](search-howto-dotnet-sdk.md), которые скрывают внутреннюю сложность технологии поиска. В дополнение к API, портал Azure предоставляет поддержку в администрировании и создании прототипа. Инфраструктурой и доступностью управляет корпорация Майкрософт.
