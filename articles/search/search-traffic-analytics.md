...
Измените состояние на **Включено**, выберите учетную запись хранения Azure, которая будет использоваться, и выберите данные, которые необходимо скопировать: журналы, метрики или и то, и другое. Мы рекомендуем копировать и журналы, и метрику.
Вы можете задать политику хранения данных в диапазоне от 1 до 365 дней. Чтобы хранить данные неопределенное время, задайте значение (количество дней) 0.
...
| operationVersion |string |"2016-09-01" |Используемая версия API |
...
| Запрос |string |"?search=AzureSearch&$count=true&api-version=2016-09-01" |Параметры запроса |
| Документы |int |42 |Количество обработанных документов |
| IndexName |string |"testindex" |Имя индекса, связанного с операцией |

### <a name="metrics"></a>Метрики
Большие двоичные объекты метрики содержат статистические значения для службы поиска.
Каждый файл имеет один корневой объект под названием **records** , который содержит массив объектов метрики. Этот корневой объект содержит метрики для каждой минуты, в течение которой данные были доступны.
...
> [!IMPORTANT]
> Регулирование происходит при отправке слишком большого количества запросов, превышающего возможности подготовленного ресурса службы. В этом случае следует рассмотреть добавление дополнительных реплик в службу.
>
>
...
3. Выберите "Хранилище BLOB-объектов Microsoft Azure", а затем нажмите кнопку "Подключиться".

    ![][6]

4. Введите "Имя" и "Ключ учетной записи" для своей учетной записи хранения.

5. Выберите строки insight-logs-operationlogs и insights-metrics-pt1m, а затем нажмите кнопку "Изменить".
