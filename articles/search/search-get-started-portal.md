...
2. Откройте служебную панель службы поиска Azure. Найти панель мониторинга можно несколькими способами.
	...
	- На панели переходов щелкните **Обзор** и введите в поле поиска слово "search". Отобразится список всех служб поиска, созданных в ваших подписках.
...
- Атрибут **Доступный для получения** означает, что поле будет показано в результатах поиска. Вы можете снять этот флажок напротив отдельных полей, чтобы исключить их из результатов поиска (например, если такие поля используются только в выражениях фильтра). 
...
В мастере **Импорт данных** щелкните **Индексатор** > **Имя**, введите имя индексатора и оставьте значения по умолчанию для всех остальных параметров. Этот объект определяет исполняемый процесс. Создав его, вы можете задать для него повторяющееся расписание. А пока используйте параметр по умолчанию, чтобы однократно и немедленно запустить индексатор после нажатия кнопки **ОК**.
...
Чтобы проверить ход выполнения, на панели мониторинга службы прокрутите вниз и дважды щелкните элемент **Индексаторы**. Откроется список индексаторов. В списке вы увидите только что созданный индексатор, состояние, указывающее на выполнение или завершение, а также количество документов, проиндексированных в службе поиска Azure.
