...
Индекс является документом JSON, содержащим разделы для полей и атрибутов, профилей повышения, средств подбора, профилей повышения по умолчанию и параметров CORS.
...
|Раздел|Описание|
|--------------|-----------|
|Коллекция полей|Поля определяют документ. Набор данных, который вы помещаете в индекс путем передачи или опроса, должен содержать для каждого поля значения (или NULL), совместимые c типом и размером полей, которые определены в схеме. Поля имеют [атрибуты](#index-attributes) — свойства или заметки, определяющие поведение конкретного поля при поиске. Например, вы можете указать, будет ли поле доступным для поиска, извлечения или сортировки данных. Можно также указать, какой анализатор языка имеет приоритет на уровне полей.
...
|[Профили оценки](https://msdn.microsoft.com/library/azure/dn798928.aspx)|Критерии, которые используются для повышения ранжирования найденного совпадения, если оно соответствует большему количеству характеристик, заданных в профиле. Например, в условии поиска задано имя продукта и описание продукта, а вы хотите, чтобы совпадения с названием продукта оценивались выше, чем совпадения с описанием. Можно создать несколько профилей.|
|Профиль повышения по умолчанию|При необходимости можно переопределить встроенную логику, которая вычисляет рейтинг поиска, указав один из профилей повышения как заданный по умолчанию.|
|Параметры CORS|По желанию вы можете разрешить использовать общий доступ к ресурсам независимо от источника, т. е. запросы на ресурс, который используется веб-страницей, будут создаваться за пределами домена. CORS по умолчанию всегда отключен, если вы не включите его для индекса явным образом.|
...
Данные из индекса используются для создания списка результатов поиска, структуры фасетной навигации и страницы сведений для отдельного документа. В службе поиска Azure индекс может также содержать недоступные для поиска данные, которые используются внутри системы (например, в выражениях фильтрации или профилях повышения, предоставляющих критерии для увеличения рейтинга).
...
Очевидно, что для приложений, которые накапливают данные и оперируют ими (например, транзакции продаж в приложении интернет-магазина), индекс службы поиска Azure будет дополнительным источником данных в общем решении. Выделение хранилища данных исключительно для поиска может показаться избыточным, но это дает следующие преимущества: согласованность результатов поиска, минимизация изменчивости данных, снижение количества круговых путей между вашим приложением и службой поиска Azure, а также повышение общей производительности операций поиска благодаря отсутствию состязания за ресурсы и данные.
