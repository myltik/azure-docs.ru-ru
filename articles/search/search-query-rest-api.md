...
Дополнительные сведения о различных параметрах запроса см. в статье [Поиск документов](https://msdn.microsoft.com/library/azure/dn798927.aspx). Кроме того, ниже приведены несколько примеров запросов.
...
Примените фильтр к индексу, чтобы найти гостиницы с номерами дешевле 150 долларов США за ночь, и возвратите значения `hotelId` и `description`:
...
Здесь выполняется поиск по всему индексу, сортировка по конкретному полю (`lastRenovationDate`) в порядке убывания, выбираются два первых результата и отображаются только значения `hotelName` и `lastRenovationDate`:
...
Дополнительные сведения см. в разделе "Ответ" статьи [Поиск документов](https://msdn.microsoft.com/library/azure/dn798927.aspx). Дополнительные сведения о других кодах состояния HTTP, которые могут быть возвращены в случае сбоя, см. в статье [Коды состояния HTTP (Поиск Azure)](https://msdn.microsoft.com/library/azure/dn798925.aspx).
