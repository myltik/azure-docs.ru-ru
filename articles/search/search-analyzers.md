...
*Анализатор* — это компонент [полнотекстового поиска](search-lucene-query-architecture.md), отвечающий за обработку текста в строках запросов и индексированных документах. Во время индексирования анализатор преобразует текст в *лексемы*, которые записываются в индексе как *разбитые на лексемы термины*. Во время поиска анализатор выполняет те же преобразования *терминов запроса*, обеспечивая основу для сопоставления терминов в индексе.
