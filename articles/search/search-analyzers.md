...
*Анализатор* — это компонент [полнотекстового поиска](search-lucene-query-architecture.md), отвечающий за обработку текста в строках запросов и содержимом индексированных документов. Во время индексирования анализатор преобразует текст в токены, которые записываются в индексе как термины. Во время поиска анализатор выполняет те же преобразования в терминах запроса, используемых для извлечения документов с соответствующими терминами из индекса.
...
Служба поиска Azure предоставляет анализатор по умолчанию. С помощью альтернативного анализатора его можно переопределить для каждого поля. Цель этой статьи — описать варианты выбора, а также ознакомить вас с лучшими методами настройки процесса лексического анализа для заданного поля. Здесь также представлен пример конфигураций для ключевых сценариев.
...
3. Анализ происходит во время индексирования. При добавлении `analyzer` в имеющийся индекс учтите следующее:
 
 | Сценарий | Действия |
 |----------|-------|
 | Добавьте новое поле (не индексированное). | Анализ происходит при добавлении или обновлении документов, предоставляющих содержимое для нового поля. Для этой задачи используется команда [Обновить указатель](https://docs.microsoft.com/rest/api/searchservice/update-index) и [mergeOrUpload](https://docs.microsoft.com/rest/api/searchservice/addupdate-or-delete-documents).|
 | Добавьте анализатор в имеющееся индексированное поле. | Инвертированный индекс для этого поля нужно воссоздать с нуля, а содержимое документа для этих полей нужно повторно проиндексировать. <br/> <br/>Для разрабатываемых индексов [удалите](https://docs.microsoft.com/rest/api/searchservice/delete-index) и [создайте](https://docs.microsoft.com/rest/api/searchservice/create-index) индекс, чтобы выбрать новое определение поля. <br/> <br/>Для индексов в производственной среде необходимо создать поле, чтобы указать измененное определение и начать использовать его. Для внедрения нового поля используется команда [Обновить указатель](https://docs.microsoft.com/rest/api/searchservice/update-index) и [mergeOrUpload](https://docs.microsoft.com/rest/api/searchservice/addupdate-or-delete-documents). Позже в рамках планового обслуживания индекса можно очистить его, чтобы удалить устаревшие поля. |
...
Мы рекомендуем использовать [API анализа](https://docs.microsoft.com/rest/api/searchservice/test-analyzer). Ответ содержит токены, которые создаются определенным анализатором для соответствующего текста. 
