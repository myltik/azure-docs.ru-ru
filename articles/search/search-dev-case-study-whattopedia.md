---
title: "Пример разработки решения в службе поиска Azure: как компания WhatToPedia создала информационный портал в службе Microsoft Azure | Документация Майкрософт"
description: "Узнайте, как создать информационный портал и поисковую систему, использующую метаданные, с помощью Поиска Azure, размещенной облачной службы поиска для разработчиков."
...
---
# <a name="azure-search-developer-case-study"></a>Пример разработки решения в службе "Поиск Azure"
## <a name="how-whattopediacomhttpwhattopediacom-built-an-infomedia-portal-on-microsoft-azure"></a>Как администраторы сайта [WhatToPedia.com](http://whattopedia.com/) создали информационный портал в Microsoft Azure
 ![][6]  &nbsp;&nbsp;&nbsp;  <font size="9">Великолепная идея</font> 
...
## <a name="how-we-did-it"></a>Как мы это сделали
...
### <a name="high-level-components"></a>Высокоуровневые компоненты
Мы создали компанию, а не просто сайт. Для этого нам потребовался широкий спектр средств и приложений. Решения Visual Studio и Visual Studio Team Services предназначены для разработки, служба Online Team Foundation Service (TFS) — для управления версиями и применения методики Scrum, Office 365 — для общения и совместной работы, а Microsoft Azure — для всех операций, связанных с сайтом и хранилищем. Интерфейс IDE, доступный в Visual Studio, обеспечил непосредственную подготовку службы Azure к работе, а интеграция с TFS Online позволила дополнительно повысить производительность.

На следующей схеме показаны высокоуровневые компоненты, используемые в инфраструктуре WhatToPedia.

   ![][8]

### <a name="how-we-use-microsoft-azure"></a>Как мы используем службу Microsoft Azure
...
* [веб-сайты Azure, разработанные с использованием платформы MVC 4;](https://azure.microsoft.com/services/websites/)
...
* [почтовая служба SendGrid.](https://azure.microsoft.com/marketplace/partners/sendgrid/sendgrid-azure/)
...
### <a name="presentation-layer"></a>Уровень представления данных
...
### <a name="authentication-permissions-and-sensitive-data"></a>Проверка подлинности, разрешения и конфиденциальные данные
...
Розничные торговцы — совсем другая история. Здесь хранятся общедоступные сведения о профиле и выставлении счетов, а также мультимедийное содержимое, которые торговцы хотят опубликовать на сайте. Каждый розничный торговец, который зарегистрировался на сайте, получает имя для входа, используемое для проверки подлинности пользователя, прежде чем он сможет вносить обновления в профиль подписчика.  Мы безопасно храним все данные подписчиков в базе данных SQL Azure и хранилище BLOB-объектов Azure.
Мы выбрали модель проверки подлинности на основе форм .NET. Мы выбрали этот подход благодаря его простоте; мы не нуждались в ролях, поддержке пользовательского интерфейса и дополнительных функциях, используемых в других моделях. 
...
### <a name="search-engine"></a>Поисковая система
...
В Интернете мы нашли [видео с конференции TechEd Europe](http://channel9.msdn.com/events/TechEd/Europe/2014/DBI-B410) 
...
1. Мы вошли на классический портал Azure и подписались на службу поиска. Мы использовали общедоступную версию (бесплатную с нашей подпиской).
...
2. Мы вызвали интерфейс API службы поиска Azure, чтобы последовательно передать данные о торговцах (см. статью по следующей ссылке: http://msdn.microsoft.com/library/azure/dn798930.aspx).
...
1. Затем мы вызвали службу поиска Azure, чтобы получить все документы, найденные в результатах поиска (см. статью по следующей ссылке: http://msdn.microsoft.com/library/azure/dn798927.aspx).
...
> [!NOTE]
> В службе "Поиск Azure" недавно появился такой компонент, как индексатор прототипа. Он появился слишком поздно, поэтому мы не смогли внедрить его в первой версии решения, но этот компонент решает ту же проблему, что и использованное нами задание индексатора, то есть автоматизирует операции загрузки данных.
> 
> 
...
Наиболее уязвимый ресурс — данные подписчиков в базе данных SQL Azure. Для их резервного копирования мы используем встроенный компонент (см. статью [Обзор обеспечения непрерывности бизнес-процессов с помощью базы данных SQL Azure](http://msdn.microsoft.com/library/azure/jj650016.aspx)). Раз в неделю выполняется резервное копирование всей базы данных, один раз в неделю — разностное резервное копирование базы данных, а каждые 5 минут сохраняются резервные копии журналов транзакций.  Учитывая текущий и прогнозируемый объем наших томов данных, это решение в полной мере справится с поставленными задачами.
...
## <a name="what-we-learned"></a>Чему мы научились
...
### <a name="webjobs-for-scheduling-tasks"></a>Веб-задания для планирования задач
...
### <a name="azure-blob-storage-explorer-for-updating-images"></a>Средство обновления изображений Azure BLOB Storage Explorer
...
## <a name="a-few-final-words"></a>Краткое заключение
...
* [страница документации на сайте Azure.com;](https://azure.microsoft.com/documentation/services/search/)
* [документация по Поиску Azure на сайте MSDN.](http://msdn.microsoft.com/library/azure/dn798933.aspx)

## <a name="appendix-search-indexer-webjob"></a>Приложение. Веб-задание индексатора поиска
...
[Подзаголовок 4]: #subheading-4
[Подзаголовок 5]: #subheading-5
...
<!--Link references-->
[Ссылка 1 на другой раздел документации на сайте azure.microsoft.com]: ../virtual-machines-windows-hero-tutorial.md
[Ссылка 2 на другой раздел документации на сайте azure.microsoft.com]: ../web-sites-custom-domain-name.md
