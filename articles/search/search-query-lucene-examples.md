...
В этой статье можно пошагово выполнить примеры синтаксиса запросов Lucene и просмотреть результаты, которые отображаются напротив них. Примеры выполняются с предварительно загруженным индексом поиска в [JSFiddle](https://jsfiddle.net), интерактивном редакторе кода для тестирования скриптов и HTML. Общие сведения об обработке запросов см. в статье [Как работает полнотекстовый поиск в службе поиска Azure](search-lucene-query-architecture.md).
