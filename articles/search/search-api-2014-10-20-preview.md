...
В этом документе описана версия **2014-10-20-Preview** API REST службы поиска Azure. Эта версия устарела. Если вы используете данный API в рабочем коде, перейдите на общедоступную версию как можно быстрее. Инструкции см. в разделе [Переход с предварительных версий API в Поиске Azure](search-transition-from-preview.md).
...
Существуют различные версии API для работы с Поиском Azure. Если вы проводите оценку поиска Azure, пытаясь понять, насколько целесообразно использовать эту службу в вашей рабочей среде, рекомендуем вам версию `api-version=2014-07-31-Preview`. В настоящий момент это единственная версия, в которую не вносятся изменения. Более подробные сведения см. в статье [Управление версиями службы поиска](http://msdn.microsoft.com/library/azure/dn864560.aspx).
