...
Самый быстрый и простой способ создания индекса — с помощью классического портала Azure. Каждый документ должен, как минимум, иметь уникальный ключ и хотя бы одно поле с данными, по которым можно осуществлять поиск. Инструкции по началу работы см. в разделе [Создание индекса на портале](search-create-index-portal.md).
