<properties
	pageTitle="Типовая процедура разработки службы поиска Azure | Microsoft Azure | Размещенная облачная служба поиска"
...
Самый быстрый и простой способ создания индекса — с помощью портала Azure. Каждый документ должен, как минимум, иметь уникальный ключ и хотя бы одно поле с данными, по которым можно осуществлять поиск. Инструкции по началу работы см. в разделе [Создание индекса на портале](search-create-index-portal.md).
...
Третий вариант подходит для небольших наборов данных и предполагает передачу документов с помощью приложения [Fiddler](search-fiddler.md).
...
Самый быстрый способ получить первые результаты поиска от службы поиска — это использовать приложение [Fiddler](search-fiddler.md) для просмотра ответа, но на практике, скорее всего, вы захотите написать простой код пользовательского интерфейса, позволяющий просмотреть результаты в удобном для чтения формате.
...
- [Пользовательские анализаторы — только предварительная версия](https://msdn.microsoft.com/library/azure/mt605304.aspx)
- [Анализатор запросов Lucene — только предварительная версия](https://msdn.microsoft.com/library/azure/mt589323.aspx)
...
На последнем этапе проанализируйте существующие уровни ресурсов для реплик и разделов и определите, нужны ли корректировки. Управлять мощностью проще всего с помощью [классического портала Azure](https://ms.portal.azure.com/).
