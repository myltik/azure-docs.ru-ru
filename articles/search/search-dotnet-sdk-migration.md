<properties
   pageTitle="Обновление до пакета SDK для .NET для поиска Azure версии 1.0-preview | Microsoft Azure | Размещенная облачная служба поиска"
   description="Обновление до пакета SDK для .NET для поиска Azure версии 1.0-preview"
   services="search"
   documentationCenter=""
...
# Обновление до пакета SDK для .NET для поиска Azure версии 1.0-preview

Если вы используете версию 0.13.0-preview [пакета SDK для .NET для поиска Azure](https://msdn.microsoft.com/library/azure/dn951165.aspx) или более раннюю, эта статья поможет вам обновить приложения для использования последней версии 1.0-preview.
...
Версия 1.0-preview пакета SDK для .NET для поиска Azure содержит несколько критических отличий от предыдущей версии (0.13.0-preview). Эти отличия в основном незначительные, поэтому изменение кода потребует минимальных усилий. В разделе [Действия по обновлению](#UpgradeSteps) вы найдете инструкции о том, как изменить код для использования новой версии пакета SDK.

<a name="WhatsNew"></a>
## Новые возможности 1.0-preview

Версия 1.0-preview использует ту же версию интерфейса API REST, что и предыдущие версии пакета SDK для .NET для поиска Azure (2015-02-28), так что в этом выпуске нет новых компонентов службы. Однако существуют новые возможности сериализации на стороне клиента.
...
Далее необходимо устранить ошибки построения одну за другой. Большинство требует изменения некоторых имен классов и методов, которые были переименованы в пакете SDK. [Список критических изменений в 1.0-preview](#ListOfChanges) содержит список этих изменений имен.

Если вы используете настраиваемые классы для моделирования документов и эти классы имеют свойства примитивных типов, не допускающие нулевого значения (например `int` или `bool` в C#), в версии пакета SDK 1.0-preview существует исправление ошибки, которое следует иметь в виду. Подробнее см. в разделе [Исправления в версии 1.0-preview](#BugFixes).

Наконец, после устранения ошибки сборки при необходимости можно внести изменения в приложение, чтобы воспользоваться преимуществами новых функциональных возможностей. Функция настраиваемой сериализации в новом пакете SDK подробно описана в разделе [Новые возможности версии 1.0-preview](#WhatsNew).

<a name="ListOfChanges"></a>
## Список критических изменений в версии 1.0-preview
...
Каждая операция в пакете SDK для .NET для поиска Azure представляется как набор перегрузок метода для синхронных и асинхронных вызовов. В версии 1.0-preview были изменены подписи и разложение этих перегрузок метода.
...
Подписи метода для той же операции в версии 1.0-preview выглядят следующим образом.
...
Начиная с версии 1.0-preview, пакет SDK для .NET для поиска Azure распределяет методы операции иначе:
...
Эта проблема устранена в пакете SDK версии 1.0-preview. Теперь, если имеется такой класс модели:
