<properties
   pageTitle="Обновление до пакета SDK для .NET для поиска Azure версии 1.0.1-preview | Microsoft Azure | Размещенная облачная служба поиска"
   description="Обновление до пакета SDK для .NET для поиска Azure версии 1.0.1-preview"
   services="search"
   documentationCenter=""
...
# Обновление до пакета SDK для .NET для поиска Azure версии 1.0.1-preview

Если вы используете версию 0.13.0-preview [пакета SDK для .NET для поиска Azure](https://msdn.microsoft.com/library/azure/dn951165.aspx) или более раннюю, эта статья поможет вам обновить приложения для использования последней версии 1.0.1-preview.
...
Версия 1.0.1-preview пакета SDK для .NET для поиска Azure содержит несколько критических отличий от предыдущей версии (0.13.0-preview). Эти отличия в основном незначительные, поэтому изменение кода потребует минимальных усилий. В разделе [Действия по обновлению](#UpgradeSteps) вы найдете инструкции о том, как изменить код для использования новой версии пакета SDK.

<a name="WhatsNew"></a>
## Новые возможности 1.0.1-preview

Версия 1.0.1-preview использует ту же версию интерфейса API REST, что и предыдущие версии пакета SDK для .NET для поиска Azure (2015-02-28), так что в этом выпуске нет новых компонентов службы. Однако существуют новые возможности сериализации на стороне клиента.
...
> [AZURE.NOTE] Убедитесь, что вы видите пакеты предварительного выпуска, выбрав "Включить предварительный выпуск", если вы используете Visual Studio, или с помощью переключателя `-IncludePrerelease` при использовании консоли диспетчера пакетов.

После того как NuGet загрузит новые пакеты и их зависимости, перестройте проект. Вы должны увидеть примерно такие ошибки построения:
...
Далее необходимо устранить ошибки построения одну за другой. Большинство требует изменения некоторых имен классов и методов, которые были переименованы в пакете SDK. [Список критических изменений в 1.0.1-preview](#ListOfChanges) содержит список этих изменений имен.

Если вы используете настраиваемые классы для моделирования документов, и эти классы имеют свойства примитивных типов, не допускающие нулевого значения (например `int` или `bool` в C#), существует исправление ошибки в версии пакета SDK 1.0.1-preview, которое следует иметь в виду. Подробнее см. в разделе [Исправления в версии 1.0.1-preview](#BugFixes).

Наконец, после устранения ошибки сборки при необходимости можно внести изменения в приложение, чтобы воспользоваться преимуществами новых функциональных возможностей. Функция настраиваемой сериализации в новом пакете SDK подробно описана в разделе [Новые возможности версии 1.0.1-preview](#WhatsNew).

<a name="ListOfChanges"></a>
## Список критических изменений в версии 1.0.1-preview
...
Каждая операция в пакете SDK для .NET для поиска Azure представляется как набор перегрузок метода для синхронных и асинхронных вызовов. В версии 1.0.1-preview были изменены подписи и разложение этих перегрузок метода.
...
Подписи метода для той же операции в версии 1.0.1-preview выглядят следующим образом.
...
Начиная с версии 1.0.1-preview, пакет SDK для .NET для поиска Azure распределяет методы операции иначе:
 ...
 - И наоборот, основные интерфейсы теперь демонстрируют методы, которые предоставляют более полный контроль на уровне HTTP при необходимости. Теперь можно передавать настраиваемые заголовки HTTP для включения в запросы, и новый возвращаемый тип `AzureOperationResponse<T>` предоставляет прямой доступ к `HttpRequestMessage` и `HttpResponseMessage` для операции. `AzureOperationResponse` определяется в пространстве имен `Microsoft.Rest.Azure` и заменяет `Hyak.Common.OperationResponse`.
...
Эта проблема устранена в версии пакета SDK 1.0.1-preview. Теперь, если имеется такой класс модели:
