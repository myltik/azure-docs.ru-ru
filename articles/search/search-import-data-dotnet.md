...
`MergeOrUpload` | Эта операция выполняет функцию `Merge`, если в индексе уже существует документ с указанным значением. Если документ не существует, операция выполняется как `Upload` с новым документом. | Поле key, а также другие поля, которые вы хотите определить. |- 
