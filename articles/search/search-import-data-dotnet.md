<properties
    pageTitle="Импорт данных в службу поиска Azure с помощью пакета SDK для .NET | Microsoft Azure | Размещенная облачная служба поиска"
...
# Импорт данных в службу поиска Azure с помощью пакета SDK для .NET
> [AZURE.SELECTOR]
...
Из этой статьи вы узнаете, как импортировать данные в индекс службы поиска Azure с помощью [пакета SDK .NET для службы поиска Azure](https://msdn.microsoft.com/library/azure/dn951165.aspx). Перед выполнением инструкций из этого руководства вам нужно [создать индекс службы поиска Azure](search-create-index-dotnet.md). В статье также предполагается, что вы уже создали объект `SearchServiceClient`, как описано в статье [Создание индекса для службы поиска Azure с помощью .NET](search-create-index-dotnet.md#CreateSearchServiceClient).
...
## 1\. Создание экземпляра класса SearchIndexClient
...
> [AZURE.NOTE] Пакет SDK .NET для службы поиска Azure использует библиотеку [NewtonSoft JSON.NET](http://www.newtonsoft.com/json/help/html/Introduction.htm) для сериализации и десериализации настраиваемых объектов модели в формат JSON и из него. При необходимости эту сериализацию можно настроить. Дополнительные сведения см. в статье [Обновление пакета SDK службы поиска Azure для .NET до версии 1.1](search-dotnet-sdk-migration.md#WhatsNew). Например, в приведенном выше примере кода в свойстве `DescriptionFr` можно указать атрибут `[JsonProperty]`.
...
Возможность использовать собственные классы как документы работает и в обратную сторону. Вы также можете получать результаты поиска и поручить пакету SDK автоматическую десериализацию в выбранный тип, как показано в [следующей статье](search-query-dotnet.md).
...
При разработке собственных классов модели для сопоставления с индексом службы поиска Azure рекомендуется объявлять, что свойства типов значений, такие как `bool` и `int`, допускают значение NULL (например, `bool?` вместо `bool`). Если же вы используете свойство, не допускающее значение NULL, вам необходимо **убедиться**, что ни один документ в индексе не содержит значение NULL для соответствующего поля. Ни пакет SDK, ни служба поиска Azure не помогут вам это обеспечить.

Это не просто гипотетическое соображение. Представьте себе ситуацию, когда вы добавляете новое поле в существующий индекс с типом `DataType.Int32`. После обновления определения индекса все документы будут иметь значение null для этого нового поля (поскольку все типы допускают значение NULL в службе поиска Azure). Если затем для этого поля вы используете класс модели со свойством `int`, не допускающим значение NULL, вы получите такое исключение `JsonSerializationException`:
...
## Далее
Заполнив индекс службы поиска Azure, вы можете приступать к отправке запросов на поиск документов. См. статью [Создание запросов в службе поиска Azure с помощью .NET](search-query-dotnet.md).
