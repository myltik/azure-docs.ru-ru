...
- **База данных SQL Azure** и **SQL Server на виртуальных машинах Azure**. Пошаговое руководство для целевых платформ см. в [этой статье](search-howto-connecting-azure-sql-database-to-azure-search-using-indexers-2015-02-28.md).
...
`api-version` является обязательным элементом. Текущая версия — `2015-02-28`.
...
Для формирования URL-адреса запроса также понадобится имя службы. Имя и `api-key` можно найти на панели мониторинга службы на [портале Azure](https://portal.azure.com/). Справку по навигации по страницам см. в [статье, посвященной созданию службы поиска на портале](search-create-service-portal.md).
...
- `name`: обязательный параметр. Имя источника данных. Имя источника данных должно содержать только строчные буквы, цифры или дефисы, не может начинаться или заканчиваться тире и не должно быть длиннее 128 символов.
...
- `container`, обязательный; указывает данные для индексирования с помощью свойств `name` и `query`.
	- `name`, обязательный.
		...
		- Хранилище таблиц Azure: указывает имя таблицы.
	- `query`, необязательный.
		...
		- Хранилище BLOB-объектов Azure: позволяет указать виртуальную папку в контейнере больших двоичных объектов. Например, в пути к большому двоичному объекту `mycontainer/documents/blob.pdf` в качестве виртуальной папки можно использовать `documents`.
		- Хранилище таблиц Azure: позволяет указать запрос, который фильтрует набор строк для импорта.
...
> [AZURE.NOTE] Некоторые свойства в существующем источнике данных нельзя обновить. Например, невозможно изменить тип существующего источника данных.

> [AZURE.NOTE] Чтобы не изменять строку подключения для существующего источника данных, можно указать литерал `<unchanged>` для строки подключения. Это может быть удобно в ситуациях, когда необходимо обновить источник данных в отсутствие простого доступа к строке подключения (поскольку она относится к конфиденциальным данным).
...
> [AZURE.NOTE] Не устанавливайте для заголовка запроса `Accept` значение `application/json;odata.metadata=none` при вызове данного API, так как это приведет к исключению атрибута `@odata.type` из ответа и вы не сможете различить политики обнаружения изменения и удаления данных разных типов.
...
- `startTime`: обязательный параметр. Дата и время начала работы индексатора в формате UTC.
...
`api-version` является обязательным элементом. Предварительная версия — `2015-02-28-Preview`.
