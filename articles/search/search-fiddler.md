...
3. На вкладке **Composer** (Компоновщик) сформулируйте запрос, который выглядит как на следующем снимке экрана.
...
На вкладке **Composer** (Компоновщик) запрос на добавление документов будет выглядеть примерно так: Тело запроса содержит данные поиска для четырех отелей.
...
После загрузки индекса и документов можно формировать запросы к ним. На вкладке **Composer** (Компоновщик) команда **GET**, которая запрашивает вашу службу, будет выглядеть примерно так, как показано на следующем снимке экрана.
...
Также можно формировать запросы к системе для получения количества документов и занятого объема хранилища. На вкладке **Composer** (Компоновщик) ваш запрос будет выглядеть примерно так, как показано ниже. Ответ вернет количество документов и объем занятого пространства.
