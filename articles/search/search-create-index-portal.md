<properties
	pageTitle="Создание индекса службы поиска Azure на портале Azure | Microsoft Azure | Размещенная облачная служба поиска"
...
5. Чтобы создать индекс на портале, щелкните кнопку **Добавить индекс** и присвойте ему имя, например *hotels*.
...
1. Щелкните **Add/Edit fields** (Добавление или изменение полей), чтобы добавить новые поля. В этом примере мы воссоздадим индекс *hotels*, описанный в статье [Использование приложения Fiddler с Поиском Azure](search-fiddler.md).
...
3. Чтобы удалить ненужные поля, щелкните их правой кнопкой мыши и выберите пункт **Удалить**.

4. Нажмите кнопку **ОК**, чтобы сохранить настроенный индекс, а затем — кнопку **ОК** на странице «Добавление индекса», чтобы создать индекс.


## Дальнейшие действия

Хоть индекс уже определен, его можно использовать только после загрузки документов. Допустим, вы воссоздали индекс hotels, используемый для тестирования. Теперь вы можете легко загрузить небольшое количество документов для этого индекса через приложение [Fiddler](search-fiddler.md), следуя инструкциям из раздела «Загрузка документов» статьи [Использование приложения Fiddler с Поиском Azure](search-fiddler.md). Затем выполните оставшиеся действия, описанные в этой статье, чтобы запустить несколько запросов.
