...
`mergeOrUpload` | Эта операция работает, как `merge`, если документ с указанным значением существует в индексе. Если документ не существует, эта операция выполняет функции `upload` для нового документа. | Поле key, а также другие поля, которые вы хотите определить |- 
`delete` | Удаляет указанный документ из индекса. | Только поле key. | Все указанные поля, кроме поля key, будут игнорироваться. Чтобы удалить из документа определенное поле, воспользуйтесь операцией `merge`, явным образом задав для требуемого поля значение NULL.
...
Дополнительные сведения о действиях с документами и ответах об успешном выполнении и ошибках см. в статье [Добавление, обновление и удаление документов](https://msdn.microsoft.com/library/azure/dn798930.aspx). Дополнительные сведения о других кодах состояния HTTP, которые могут быть возвращены в случае сбоя, см. в статье [Коды состояния HTTP (Поиск Azure)](https://msdn.microsoft.com/library/azure/dn798925.aspx).

## Далее
Заполнив индекс службы поиска Azure, вы можете приступать к отправке запросов на поиск документов. См. статью [Отправка запросов в индекс службы поиска Azure с помощью REST API](search-query-rest-api.md).
