<properties
    pageTitle="Импорт данных в службе поиска Azure с помощью REST API | Microsoft Azure | Размещенная облачная служба поиска"
...
В этой статье объясняется, как использовать [REST API службы поиска Azure](https://msdn.microsoft.com/library/azure/dn798935.aspx) для импорта данных в индекс службы поиска Azure. Перед выполнением инструкций из этого пошагового руководства вам нужно [создать индекс службы поиска Azure](search-create-index-rest-api.md).
...
`mergeOrUpload` | Эта операция работает, как `merge`, если документ с указанным значением существует в индексе. Если документ не существует, эта операция выполняет функции `upload` для нового документа. | Поле key, а также другие поля, которые вы хотите определить |- `delete` | Удаляет указанный документ из индекса. | Только поле key. | Все указанные поля, кроме поля key, будут игнорироваться. Чтобы удалить из документа какое-либо поле, воспользуйтесь операцией *merge*, явно указав для такого поля значение null.
...
В нашем примере мы используем операции `upload`, `mergeOrUpload`, и `delete` в качестве операций поиска.

Предположим, что пример индекса с именем hotels уже заполнен несколькими документами. Обратите внимание, что нам не понадобилось указывать все поля документа при использовании операции `merge`. Мы указали только ключ документа (`hotelId`) при использовании `delete`.
...
#### 429
Код состояния `429` возвращается, если вы превысили количество документов, которые можно включить в индекс.
...
Дополнительные сведения о действиях с документами и ответах на успешное завершение и ошибки см. [на этой странице](https://msdn.microsoft.com/library/azure/dn798925.aspx). Дополнительные сведения о других кодах состояния HTTP, которые могут быть возвращены в случае сбоя, см. в [этой статье](https://msdn.microsoft.com/library/azure/dn798925.aspx).

## Далее
Заполнив индекс службы поиска Azure, вы можете приступать к отправке запросов на поиск документов.
