...
Ниже показаны снимки экрана с колонками службы поиска Azure на классическом портале Azure, позволяющими настраивать определение схемы индекса. В этой колонке пользователи могут создавать любые поля и настраивать свойства анализатора для каждого из них.

> [AZURE.NOTE]Выбрать языковой анализатор можно только в процессе работы с определением поля, например при создании нового индекса с нуля или при добавлении нового поля в существующий индекс. При создании поля задайте все необходимые атрибуты. После того как определение поля установлено, изменить атрибуты или тип анализатора нельзя.

1. Перейдите на [классический портал Azure](https://portal.azure.com) и откройте колонку службы поиска.
2. Щелкните **Добавить индекс** в верхней части панели мониторинга службы, чтобы создать новый индекс, или откройте существующий индекс, чтобы задать анализатор в добавляемых в него полях.
...
7. Сделайте поле доступным для поиска, установив соответствующий значок.
...
Если вы разработчик .NET, обратите внимание на то, что языковые анализаторы можно настраивать с помощью [SDK .NET службы поиска Azure](http://www.nuget.org/packages/Microsoft.Azure.Search/0.13.0-preview). Последний выпуск включает поддержку языковых анализаторов корпорации Майкрософт.
