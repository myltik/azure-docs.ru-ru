---
title: "Использование профилей повышения в службе поиска Azure | Документация Майкрософт"
...
---
# <a name="how-to-use-scoring-profiles-in-azure-search"></a>Использование профилей оценки в службе поиска Azure
...
Для работы с учебником вам потребуется подписка Azure и служба поиска Azure. Справку по настройке службы см. в разделе [Создание службы поиска на портале](search-create-service-portal.md).
 ...
 [Для работы с этим руководством требуется учетная запись Azure.](../../includes/free-trial-note.md)]
...
2. В обозревателе решений дважды щелкните файл **app.config**. Этот файл указывает конечную точку службы и `api-key` , используемый для проверки подлинности запроса. Эти значения можно получить на классическом портале.
3. Войдите на [портал Azure](https://portal.azure.com).
4. Перейдите к панели мониторинга службы поиска Azure.
5. Щелкните плитку **Свойства** , чтобы скопировать URL-адрес службы.
6. Щелкните элемент **Ключи**, чтобы скопировать `api-key`.
...
Изучите дополнительные сведения о синтаксисе и параметрах поисковых запросов. См. статью [Поиск документов (API REST службы поиска Azure)](http://msdn.microsoft.com/library/azure/dn798927.aspx).
