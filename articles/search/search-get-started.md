...
[AZURE.INCLUDE [В этой статье используется портал Azure Preview](../../includes/preview-portal-note.md)]

Служба поиска Microsoft Azure - это новая служба, позволяющая встраивать поисковые функции в настраиваемые приложения. Она предоставляет поисковую систему и хранилище данных, к которым можно обращаться и которыми можно управлять посредством SDK .NET или REST API. Чтобы узнать больше о преимуществах Поиска Azure, см. статью [Сценарии и возможности Поиска Azure](http://azure.microsoft.com/blog/2014/08/28/azure-search-scenarios-and-capabilities/).  
...
4. В коллекции щелкните **Данные + аналитика**.
 
  	![][8]

5. В списке служб данных щелкните **Search**.
 
  	![][10]

7. В нижней части страницы поиска нажмите кнопку **Создать**.

8. Введите имя службы строчными буквами для использования в URL-адресе службы, не используйте пробелы, максимальная длина не должна превышать 15 символов.
...
4. В коллекции щелкните **Дата + аналитика**.
...
Можно просмотреть [Pricing Details](http://go.microsoft.com/fwlink/p/?LinkID=509792) (Сведения о ценообразовании), чтобы узнать о стоимости продуктов. В разделе [Limits and constraints](http://msdn.microsoft.com/library/azure/dn798934.aspx) (Ограничения) можно найти справочные сведения, которые помогут в настройке сочетаний разделов и реплик.
...
3. Введите URL, который задает URL службы (его можно найти на странице "Свойства"), атрибуты запроса и версию API. Не забывайте:
   ...
   + Атрибут запроса: "/indexes/hotels". Он сообщает службе поиска, что нужно создать индекс с именем 'hotels'.
...
4.	При этом откроется панель мониторинга службы. Обратите внимание: команды **Start** (Начать), **Stop** (Остановить) и **Delete** (Удалить) доступны вверху страницы. Панель мониторинга службы включает плитки для просмотра свойств, ключей и быстрого начала работы, где размещены ссылки на информацию и инструкции. Пролистайте вниз, чтобы посмотреть использование служб.

5.	Щелкните **PROPERTIES** (Свойства). Справа откроется страница "Свойства". URL службы отображается в верхней части страницы. Этот URL-адрес потребуется для подключения к службе поиска Azure.

 	![][23]
	
7.	Щелкните **KEYS** (Ключи), чтобы просмотреть ключи API. Для прохождения проверки подлинности в службе потребуется ключ администратора. Можно использовать первичный или вторичный ключ. Дополнительно можно создать ключи запросов для получения доступа только для чтения к службе.
...
- [Создание первого решения поиска Azure](search-create-first-solution.md) 
...
<!--Link references-->
[Управление решением поиска в Microsoft Azure]: search-manage.md
[Рабочий процесс разработки решения поиска Azure]: search-workflow.md
[Создание первого решения поиска Azure]: search-create-first-solution.md
[Создание приложения для геопространственного поиска с помощью службы поиска Azure]: search-create-geospatial.md
