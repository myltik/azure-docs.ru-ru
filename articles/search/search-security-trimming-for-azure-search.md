---
title: "Применение фильтров безопасности с помощью службы поиска Azure"
description: "Реализация фильтров безопасности с помощью службы поиска Azure."
ms.custom: 
ms.date: 08/07/2017
ms.service: search
ms.reviewer: 
...
---
# <a name="security-trimming-with-azure-search"></a>Применение фильтров безопасности с помощью службы поиска Azure

Чтобы ограничить доступ к документам на основе удостоверения пользователя, можно применять фильтры безопасности в результатах поиска. В рамках этого поиска, как правило, требуется сравнить личность инициатора поискового запроса с указанными в поле субъектами, у которых есть разрешения на документ. Если совпадение обнаружено, пользователь или субъект (например, группа или роль) имеет доступ к этому документу.
...
Полные сведения о добавлении или обновлении документов см. в статье [об изменении документов](https://docs.microsoft.com/en-us/rest/api/searchservice/addupdate-or-delete-documents).
...
Дополнительные сведения о поиске документов с помощью службы поиска Azure см. в статье [Search Documents (Azure Search Service REST API)](https://docs.microsoft.com/en-us/rest/api/searchservice/search-documents) (Поиск документов (REST API службы поиска Azure)).
