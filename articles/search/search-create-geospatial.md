...
3. Функция **ApplyStoreData** загружает данные из CSV-файла с именем store\_locations.csv в таблицу System.Data.DataTable.
...
4.	Откройте файл HomeController.cs в папке **Controllers** (Контроллеры) и взгляните на функцию **Search**. Обратите внимание на вызов \_storeSearch.Search(lat, lon, 10000). Такой запрос возвращает все магазины в пределах 10 тыс. км от точки с указанной широтой (lat) и долготой (lon). Результаты этого запроса обрабатываются, а затем отправляются обратно в представление Index, где они отображаются в виде вешек на Картах Bing.
