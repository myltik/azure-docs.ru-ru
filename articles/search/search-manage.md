...
В случае сбоя поиск Azure выполнит отработку отказа в другие кластеры и центры обработки данных, но он не содержит встроенные решения для ручных операций резервного копирования и восстановления, если индекс или служба злонамеренно или случайно удалены. Для пользователей, передающих в службу объекты и данные, исходный код для создания и заполнения индекса де-факто является возможностью восстановления на случай, если индекс удаляется по ошибке. 

Поиск Azure не поддерживает георепликацию индексов в службах. Если ваше решение имеет обширную географию, вы можете обеспечить избыточность с помощью дополнительной службы в различных региональных центрах обработки данных,чтобы все компоненты приложения размещались централизованно. Дополнительные сведения см. в статье [Рекомендации по производительности и оптимизации Поиска Azure](search-performance-optimization.md).
