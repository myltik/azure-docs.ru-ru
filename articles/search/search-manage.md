<properties 
	pageTitle="Администрирование службы поиска Azure на портале Azure" 
...
# Администрирование службы поиска Azure на портале Azure
> [AZURE.SELECTOR]
...
<a id="sys-info"></a>
## Ведение журнала в службе поиска Azure и сведения о системе
...
<a id="rbac"></a>
## Настройка ролей RBAC с правами администратора для службы поиска Azure
...
Роль|Задача
---|---
Владелец|Создание или удаление службы или любого объекта в службе, включая ключи API, индексы, индексаторы, источники данных индексаторов и расписания индексаторов.<p>Просмотр состояния службы, в том числе количества объектов и размера хранилища.<p>Добавление или удаление членства в роли (только владелец может управлять членством в роли).<p>Администраторам подписки и владельцам службы автоматически присваивается роль владельца.
...
1. На панели мониторинга службы щелкните значок "Доступ", чтобы открыть колонку "Пользователи". ![][7]
...
Увеличение количества запросов в секунду (QPS) или достижение высокой производительности осуществляется посредством добавления реплик. Каждая реплика имеет одну копию индекса, так что добавление еще ​​одной реплики означает перенос еще одного индекса, доступного для обработки запросов службы. Для обеспечения высокой доступности требуется не менее 3 реплик (дополнительные сведения см. в разделе [Масштабирование уровней ресурсов для рабочих нагрузок запросов и индексирования в Поиске Azure](search-capacity-planning.md)).
...
## Рекомендации по масштабированию и развертыванию (видео)

В этом 30-минутном видеоролике представлены рекомендации для расширенных сценариев развертывания, включая геораспределение рабочих нагрузок. См. также раздел [Рекомендации по производительности и оптимизации Поиска Azure](search-performance-optimization.md), в котором охватываются те же вопросы.
...
Ознакомьтесь также со статьей [Рекомендации по производительности и оптимизации Поиска Azure](search-performance-optimization.md) (если вы ее еще не прочитали) и, при необходимости, просмотрите видеоролик, указанный в предыдущем разделе. Это позволит вам глубже понять и освоить рекомендуемые методы.
