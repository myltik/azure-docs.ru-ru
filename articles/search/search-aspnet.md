<properties
	pageTitle="Объединение веб-приложений ASP.NET со службой поиска Azure | Microsoft Azure"
...
#Интеграция службы поиска Azure в веб-приложения ASP.NET

ASP.NET является основной платформой веб-приложений в пользовательских решениях, которые интегрируются в службу поиска Azure. В этой статье описано, как включить в приложение ASP.NET службу поиска Azure. Также вы ознакомитесь с шаблонами проектирования стандартных операций и некоторыми методами программирования, которые позволят быстрее освоиться с процессом разработки.
...
В большинстве наших примеров кода индекс создается и загружается в отдельном проекте (в разных примерах он называется DataIndexer, CatalogIndexer или DataCatalog). В то же время код, который обрабатывает поисковые запросы и ответы, помещается в проект приложения MVC ASP.NET. При изучении примеров кода удобно совмещать создание индекса и отправку документа в рамках одного проекта, но в рабочем коде лучше разделять эти операции. После создания индекс редко изменяется (в случае изменения он должен быть перестроен), тогда как документы обычно обновляются регулярно.
...
- ключ API (GUID) для проверки подлинности подключения к службе поиска Azure;
- HTTPClient или SearchServiceClient для формирования запроса на подключение.

####URL-адреса и ключи API

URL-адрес и ключ API вы можете найти на [портале](search-create-service-portal.md) или получить программными средствами с помощью [REST API управления](https://msdn.microsoft.com/library/dn832684.aspx).
...
Вы можете использовать в качестве имени службы поиска короткое имя, указанное во время подготовки, а при подключении добавлять к нему имя домена (search.windows.net). Также вы можете сохранить в файле web.config полное имя (<service-name>.search.windows.net) без префикса HTTPS.
...
Метод .NET для вызова метода **Search** может выглядеть представленным ниже образом. Кроме того, он может включаться в основную программу C#, которая отвечает за операции соединения и поиска.
