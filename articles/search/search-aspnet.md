<properties
	pageTitle="Интеграция службы поиска Azure с веб-приложениями ASP.NET MVC | Microsoft Azure | Размещенная облачная служба поиска"
	description="Подключение веб-приложения ASP.NET к облачной службе поиска Azure. Узнайте, как подключаться, создавать запросы и обрабатывать результаты с помощью библиотеки .NET или REST API."
	services="search"
	documentationCenter=""
	authors="HeidiSteen"
	manager="mblythe"
