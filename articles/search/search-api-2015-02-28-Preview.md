...
API службы поиска Azure поддерживает две синтаксические схемы, которые можно использовать для поиска объектов: [простой](https://msdn.microsoft.com/library/dn798920.aspx) и альтернативный синтаксис OData. Подробные сведения см. в статье [Поддержка OData (поиск в Azure)](http://msdn.microsoft.com/library/azure/dn798932.aspx). В разделах из приведенного ниже списка рассматривается простой синтаксис.
...
**Примечание.** Максимальное разрешенное количество индексов зависит от ценовой категории. В бесплатной версии службы разрешено до трех индексов. В стандартной версии разрешено использовать до 50 индексов на одну службу поиска. Подробные сведения см. в статье [Границы службы](search-limits-quota-capacity.md).
...
Параметр `api-version` является обязательным. Список доступных версий см. в статье [Управление версиями службы поиска Azure](http://msdn.microsoft.com/library/azure/dn864560.aspx).
...
`suggestions`— в предыдущих версиях API использовался атрибут `suggestions`. Это логическое свойство теперь объявлено устаревшим и более не доступно в версиях `2015-02-28` и `2015-02-28-Preview`. Вместо него используйте [API средств подбора](#Suggesters). В версии `2014-07-31` с помощью свойства `suggestions` можно было указать, следует ли использовать данное поле для автозавершения при упреждающем вводе (для полей типа `Edm.String` и `Collection(Edm.String)`). По умолчанию для параметра `suggestions` задано значение `false`, так как оно требует дополнительного места в индексе. Если вы использовали это значение, ознакомьтесь со статьей о [переходе c предварительной версии на окончательную в службе "Поиск Azure"](search-transition-from-preview.md), которая содержит инструкции по переходу на новый API.
...
По сравнению с ним анализатор от Майкрософт вместо парадигматического модуля использует лемматизацию. Это означает, что он может обрабатывать флективные и неправильные словоформы гораздо лучше и позволяет получить более значимые результаты поиска (дополнительные сведения см. в модуле 7 [презентации MVA, посвященной службе "Поиск Azure"](http://www.microsoftvirtualacademy.com/training-courses/adding-microsoft-azure-search-to-your-websites-and-apps)).
...
Код вашего приложения должен создавать документы JSON для отправки в "Поиск Azure". В противном случае (если источником данных является База данных SQL Azure или DocumentDB) для загрузки документов используйте [индексатор](https://msdn.microsoft.com/library/dn946891.aspx). Как правило, индексы заполняются из одного указанного набора данных.
...
При непосредственном вызове REST API для создания запросов GET рекомендуется преобразовывать ряд параметров запроса в [кодировку URL](https://msdn.microsoft.com/library/system.uri.escapedatastring.aspx). Для операций **поиска** используются указанные ниже параметры.
...
`$top=#` (необязательный): количество результатов поиска, которые необходимо получить. В сочетании с параметром `$skip` этот параметр позволяет реализовать постраничное отображение результатов поиска на стороне клиента.

> [AZURE.NOTE]Чтобы запросы не выдавали слишком большое количество документов за один раз, в службе "Поиск Azure" используется ***разбиение на страницы на стороне сервера***. Размер страницы по умолчанию — 50, максимальный размер страницы — 1000. Это значит, что если параметр `$top` не указан, то по умолчанию при **поиске** будут выдаваться не больше 50 результатов. Если результатов больше 50, то отклик будет включать информацию для получения следующей страницы, содержащей не более 50 результатов поиска (см. `@odata.nextLink` и `@search.nextPageParameters` в [примере ниже](#SearchResponse)). Аналогично, если для параметра `$top` указано значение больше 1000, а количество результатов поиска превышает это число, будут показаны только первые 1000 результатов и информация для получения следующей страницы, содержащей не более 1000 результатов поиска.
...
- `count` (максимальное количество условий для аспекта; значение по умолчанию — 10). Максимального предела нет, однако чем больше количество, чем сильнее это влияет на производительность (особенно если поле аспекта содержит большое число уникальных элементов).
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса GET зарезервированные символы в URL-адресе следует заменить сочетанием символов со знаком процента (например, %23 вместо #).
...
Запросы к службе отправляются по протоколу HTTPS. Для отправки запроса **предложений** можно использовать метод GET или POST.
...
При непосредственном вызове REST API для создания запросов GET рекомендуется преобразовывать ряд параметров запроса в [кодировку URL](https://msdn.microsoft.com/library/system.uri.escapedatastring.aspx). Для операций **предложений** это указанные ниже параметры.
...
> [AZURE.NOTE]При вызове **предложений** с помощью запроса GET зарезервированные символы в URL-адресе следует заменять сочетанием символов со знаком процента (например, %23 вместо #).
