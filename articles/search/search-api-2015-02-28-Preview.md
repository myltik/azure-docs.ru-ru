...
- Возможность использовать запрос POST вместо GET в операциях [поиска](#SearchDocs) и [предложений](#Suggestions), что пригодится в случае, если общий размер URL-адреса превышает 8 КБ.
...
Параметр `api-version` является обязательным. Список доступных версий см. в статье [Управление версиями службы поиска](http://msdn.microsoft.com/library/azure/dn864560.aspx).
...
Либо код вашего приложения должен создавать документы JSON для передачи в службу поиска Azure, либо, если источником данных является база данных SQL Azure или DocumentDB, используйте для загрузки документов [индексатор](https://msdn.microsoft.com/library/dn946891.aspx). Как правило, индексы заполняются из одного указанного набора данных.
...
Операция **поиска** выполняется с помощью запроса GET или POST и содержит параметры, которые определяют условия выборки документов.
...
При использовании HTTP-запроса GET для вызова API **поиска** следует помнить, что длина URL-адреса запроса не должна превышать 8 КБ. Для большинства приложений этого достаточно. В то же время некоторые приложения создают очень длинные запросы, в частности выражения фильтров OData. Для таких приложений лучше использовать HTTP-запрос POST. Размер запроса POST ограничен примерно 17 МБ, что позволяет создавать даже самые комплексные запросы.
...
При непосредственном вызове API REST для создания запросов GET рекомендуется преобразовывать определенные параметры запроса в [кодировку URL](https://msdn.microsoft.com/library/system.uri.escapedatastring.aspx). Для операций **поиска** в число таких параметров входят:
...
Кроме того, преобразование в кодировку URL необходимо только при непосредственном обращении к API REST с помощью запроса GET. При вызове **поиска** с помощью запроса POST или при использовании [клиентской библиотеки .NET](https://msdn.microsoft.com/library/dn951165.aspx), которая выполняет кодировку URL-адресов автоматически, кодировка URL не требуется.

**Параметры запроса**

**Поиск** принимает несколько параметров, которые сообщают критерии запроса и определяют способы поиска. Эти параметры указываются в строке URL-запроса (при вызове **поиска** с помощью запроса GET) или в теле запроса в виде свойств JSON (при вызове **поиска** с помощью запроса POST). Синтаксис некоторых параметров зависит от выбора запроса GET и POST. Эти различия описаны ниже.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `skip`, а не `$skip`.

`$top=#` (необязательный): количество результатов поиска, которые нужно вернуть. В сочетании с параметром `$skip` позволяет реализовать постраничное отображение результатов поиска на стороне клиента.

> [AZURE.NOTE]Чтобы запросы не выдавали слишком большое число документов за один раз, в службе поиска Azure используется ***постраничный просмотр на стороне сервера***. Размер страницы по умолчанию — 50, максимальный размер страницы — 1000. Это значит, что если параметр `$top` не указан, то по умолчанию **поиск** выдает не больше 50 результатов. Если результатов больше 50, ответ включает информацию для извлечения следующей страницы, содержащей не более 50 результатов поиска (см. `@odata.nextLink` и `@search.nextPageParameters` в [приведенном ниже примере](#SearchResponse)). Точно так же, если для параметра `$top` указано значение больше 1000, а количество результатов поиска превышает это число, будут показаны только первые 1000 результатов и информация для извлечения следующей страницы, содержащей не больше 1000 результатов поиска.

> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `top`, а не `$top`.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `count`, а не `$count`.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `orderby`, а не `$orderby`.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `select`, а не `$select`.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `facets`, а не `facet`. Кроме того, она задается в виде JSON-массива строк, где каждая строка представляет собой отдельное выражение аспектирования.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `filter`, а не `$filter`.
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса GET зарезервированные символы в URL-адресе следует заменять символом процента (например, %23 вместо #).
...
> [AZURE.NOTE]При вызове **поиска** с помощью запроса POST этот параметр называется `scoringParameters`, а не `scoringParameter`. Кроме того, она задается в виде JSON-массива строк, где каждая строка представляет собой отдельную пару имя:значение.
...
Примечание. Для этой операции `api-version` указывается как параметр запроса в URL-адресе независимо от того, вызывается ли **поиск** с помощью запроса GET или POST.
...
14) Поиск документов в индексе с использованием [простого синтаксиса запросов](https://msdn.microsoft.com/library/dn798920.aspx). Этот запрос возвращает гостиницы, в которых поля, участвующие в поиске, содержат слова comfort и location, но не motel:
...
Операция **предложений** выполняется в виде запроса GET или POST.
...
Запросы к службе отправляются по протоколу HTTPS. Для отправки запроса на получение **предложений** можно использовать метод GET или POST.
...
При непосредственном вызове API REST для создания запросов GET рекомендуется преобразовывать определенные параметры запроса в [кодировку URL](https://msdn.microsoft.com/library/system.uri.escapedatastring.aspx). Для операций **предложений** это следующие параметры.
...
**Предложения** принимают несколько параметров, которые сообщают критерии запроса и определяют способы поиска. Эти параметры указываются в строке URL-запроса (при вызове **предложений** с помощью запроса GET) или в теле запроса в виде свойств JSON (при вызове **предложений** с помощью запроса POST). Синтаксис некоторых параметров зависит от выбора запроса GET и POST. Эти различия описаны ниже.
...
> [AZURE.NOTE]При вызове **предложений** с помощью запроса GET зарезервированные символы в URL-адресе следует заменять символом процента (например, %23 вместо #).
...
> [AZURE.NOTE]При вызове **предложений** с помощью запроса POST этот параметр называется `top`, а не `$top`.

`$filter=[string]` (необязательный): выражение для фильтрации документов, по которым производится поиск предложений.

> [AZURE.NOTE]При вызове **предложений** с помощью запроса POST этот параметр называется `filter`, а не `$filter`.
...
> [AZURE.NOTE]При вызове **предложений** с помощью запроса POST этот параметр называется `orderby`, а не `$orderby`.
...
> [AZURE.NOTE]При вызове **предложений** с помощью запроса POST этот параметр называется `select`, а не `$select`.
...
Примечание. Для этой операции `api-version` указывается как параметр запроса в URL-адресе независимо от того, вызываются ли **предложения** с помощью запроса GET или POST.
