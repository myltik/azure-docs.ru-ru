...
Если вы хотите реализовать эффективный поиск в данных DocumentDB, можно использовать индексатор Поиска Azure для извлечения данных в индекс Поиска Azure. В этой статье показано, как интегрировать Azure DocumentDB со службой поиска Azure без написания кода для поддержки инфраструктуры индексирования.

Для настройки индексатора DocumentDB установите [службу Поиска Azure](search-create-service-portal.md) и создайте индекс, источник данных и, наконец, индексатор. Вы можете создать эти объекты с помощью [портала](search-import-data-portal.md), [пакета SDK для .NET](/dotnet/api/microsoft.azure.search) или [REST API](/rest/api/searchservice/) для всех языков, отличных от .NET. 
...
Вы можете указать запрос DocumentDB для преобразования вложенных свойств или массивов, проецирования свойств JSON, а также для фильтрации данных, подлежащих индексированию. 
...
Политика обнаружения изменения данных предназначена для эффективного определения измененных элементов данных. Сейчас поддерживается только политика `High Water Mark`, использующая свойство отметки времени `_ts`, предоставленное DocumentDB. Эта политика указывается следующим образом.
...
Поздравляем! Вы узнали, как интегрировать Azure DocumentDB со службой поиска Azure с помощью индексатора для DocumentDB.
