<properties
	pageTitle="Профили оценки (API REST службы поиска Azure, версия 2015-02-28-Preview) | Microsoft Azure"
...
<table>
<thead>
<tr><td><b>Элемент</b></td><td><b>Описание</b></td></tr></thead>
  <tbody>
    <tr>
      <td>
        <b>Weights</b>
    ...
    <tr>
      ...
      <td>
        Используются, когда необходимы дополнительные вычисления для определенных контекстов. Допустимые значения: freshness, magnitude, distance и tag. Каждая функция имеет свои уникальные параметры.
        <p>
          — freshness следует использовать, если вам необходимо повысить приоритет нового или старого элемента в зависимости от его «возраста». Эту функцию можно использовать только с полями типа datetime (Edm.DataTimeOffset). Обратите внимание, что атрибут boostingDuration используется только с функцией freshness.
            ...
            Для функции magnitude можно задать обратный диапазон (от высоких оценок до низких), если результаты нужно отобразить в обратном порядке (например, повысить приоритет для элементов с более низкими ценами по сравнению с элементами с более высокими ценами). Если у вас есть диапазон цен от 100 до 1 доллара США, для параметров boostingRangeStart и boostingRangeEnd следует установить значения 100 или 1 соответственно, чтобы повысить приоритет для элементов с более низкими ценами.
          </p><p>
            — distance следует применять, чтобы повысить приоритет по близости или географическому расположению. Эта функция может использоваться только с полями Edm.GeographyPoint.
          </p><p>
            — tag следует применять, чтобы повысить приоритет по тегам, которые одновременно используются в документах и поисковых запросах. Эту функцию можно использовать только с полями Edm.String и (Collection(Edm.String).
          </p>
             <p><b>Правила использования функции</b>
			</p>
            ...
            <br/>
             Функции можно применять только к фильтруемым полям. Подробнее о фильтруемых полях см. в статье [Создание индекса](search-api-2015-02-28/#createindex).
             <br/>
...
<td>freshness</td>	<td>Функция оценки freshness используется для оценки ранжирования элементов на основе значений в полях DateTimeOffset. Например, элемент с более близкой датой может классифицироваться выше, чем старые элементы. В текущем выпуске службы один конец диапазона фиксируется на текущее время. Скорость повышения приоритета от максимального и минимального значений диапазона определяется интерполяцией, которая применяется к профилю оценки (см. рисунок ниже). Чтобы изменить направление коэффициента повышения приоритета, выберите коэффициент повышения приоритета меньше&#160;1.</td>
</tr><tr>
...
<td>distance | referencePointParameter</td>	<td>Параметр, который передается в запросах и используется в качестве эталонного расположения. scoringParameter — это параметр запроса. Описание параметров запроса см. в разделе [Поиск документов](search-api-2015-02-28-preview/#SearchDocs).</td>
</tr><tr>
...
<td>tag | tagsParameter</td>	<td>Параметр, который передается в запросах для указания тегов определенного запроса. scoringParameter — это параметр запроса. Описание параметров запроса см. в разделе [Поиск документов](search-api-2015-02-28-preview/#SearchDocs).</td>
</tr><tr>
