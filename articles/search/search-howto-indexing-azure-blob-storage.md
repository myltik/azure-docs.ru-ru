...
Если вы хотите пропустить извлечение содержимого всех больших двоичных объектов, это можно сделать с помощью объекта конфигурации индексатора вместо того, чтобы добавлять пользовательские метаданные в каждый большой двоичный объект по отдельности. Чтобы сделать это, задайте для свойства конфигурации `SkipContent` в объекте `parameters` значение `true`.
