...
Чтобы настроить индексацию больших двоичных объектов, сделайте следующее.
...
2. Создайте индекс поиска с полем `content` с возможностью поиска. 
3. Создайте индексатор путем подключения источника данных к целевому индексу.
...
Дополнительные сведения об API создания индекса см. в разделе [Создание индекса](https://msdn.microsoft.com/library/dn798941.aspx).
...
Дополнительные сведения об API создания индексатора см. в статье [Создание индексатора](search-api-indexers-2015-02-28-preview.md#create-indexer).
...
- JSON (подробные сведения см. в статье [Индексирование BLOB-объектов JSON](search-howto-index-json-blobs.md))
...
JSON (application/json) | `metadata_content_type`</br>`metadata_content_encoding` | Извлечение текста <br/>Примечание. Сведения о том, как извлечь несколько полей документа из большого двоичного объекта JSON, см. в статье [Индексирование BLOB-объектов JSON](search-howto-index-json-blobs.md).
