...
- JSON (подробные сведения см. в разделе [Indexing JSON blobs (Индексирование больших двоичных объектов JSON)](search-howto-index-json-blobs.md))
...
JSON (application/json) | `metadata_content_type`</br>`metadata_content_encoding` | Извлечение текста <br/>Примечание. Если требуется извлечь несколько полей документа из большого двоичного объекта JSON, пожалуйста, см. раздел [Indexing JSON blobs (Индексирование больших двоичных объектов JSON)](search-howto-index-json-blobs.md).
