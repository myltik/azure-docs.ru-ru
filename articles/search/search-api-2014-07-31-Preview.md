...
В этом документе описывается более ранняя версия API REST службы поиска Azure, **2014-07-31-Preview**, выпущенная для общедоступной предварительной версии службы поиска Azure в августе 2014 г. Документацию к текущей общедоступной версии API REST службы поиска Azure можно найти в MSDN. Дополнительные сведения см. в статье [Интерфейс API REST службы поиска Azure](http://msdn.microsoft.com/library/azure/dn798935.aspx).

Если вы используете в коде приложения предварительную версию API, настоятельно рекомендуем вам перейти на общедоступную версию. Инструкции см. в разделе [Переход с предварительной на общедоступную версию API](search-transition-from-preview.md).
...
Существуют различные версии API для работы с Поиском Azure. Если вы проводите оценку, пытаясь понять, насколько целесообразно использовать эту службу в вашей рабочей среде, рекомендуем вам обратить внимание на сведения о версии, публикуемые для каждого набора интерфейсов API. Выбрать оптимальную версию вам поможет статья [Управление версиями службы поиска](http://msdn.microsoft.com/library/azure/dn864560.aspx).
...
Интерфейс API службы поиска Azure поддерживает две синтаксические схемы, которые можно использовать для запроса объектов. В списке ниже описан как простой синтаксис, так и альтернативный вариант синтаксиса OData.
