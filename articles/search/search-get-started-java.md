<properties 
	pageTitle="Начало работы с Поиском Azure в Java" 
	description="Пошаговое создание пользовательского приложения Поиска Azure с использованием Java в качестве языка программирования." 
	services="search" 
	documentationCenter="" 
	authors="HeidiSteen" 
	manager="mblythe" 
...
Узнайте, как создать собственное приложение поиска на Java, использующее Поиск Azure. В учебнике упоминается создание объектов и операций из этого урока с помощью [API REST службы поиска Azure](https://msdn.microsoft.com/library/dn798935.aspx).
...
Чтобы запустить этот пример, вам также потребуется служба поиска Azure, в которой можно зарегистрироваться на [портале управления Azure](https://portal.azure.com).
...
3. Настройте имя службы, ценовую категорию, группу ресурсов, подписку и местоположение. Эти параметры обязательны. Их нельзя изменить после подготовки службы.
	...
	- **Ценовая категория** определяет доступную емкость и порядок выставления счетов. Оба уровня обеспечивают аналогичные функции на разных уровнях ресурсов.
		...
		- **Бесплатная служба** выполняется на кластерах, используемых совместно с другими подписчиками. Она позволяет проходить учебные курсы и писать экспериментальный код, однако не предназначена для рабочих приложений. Бесплатная служба обычно развертывается в течение всего нескольких минут.
...
4. Используйте **Обозреватель проектов** для просмотра и редактирования файлов. Если обозреватель еще не открыт, выберите **Окно** | **Показать представление** | **Обозреватель проектов** или откройте его с помощью ярлыка.
...
1. В Eclipse в обозревателе проектов щелкните правой кнопкой мыши проект | **Свойства** | **Аспекты проекта**.
...
4. Выберите **Окно** | **Предпочтения** | **Серверы** | **Среды выполнения** | **Добавить**.
...
7. Выберите **Окно** | **Предпочтения** | **Java** | **Установленные JRE** | **Добавить**.
...
12. Перейдите в **Program Files** | **Java** и выберите ранее установленный пакет JDK. Важно выбрать пакет JDK в качестве JRE.
...
1. В обозревателе проектов щелкните правой кнопкой мыши имя проекта и выберите **Запуск от имени** | **Сборка Maven...**, чтобы настроить проект.
...
> [AZURE.TIP]При использовании рабочей области не по умолчанию для хранения проекта вам, вероятно, потребуется изменить **Конфигурацию запуска** и указать расположение проекта, чтобы избежать возникновения ошибки запуска сервера. В обозревателе проектов щелкните правой кнопкой мыши **Search.jsp** | **Запуск от имени** | **Конфигурации запуска**. Выберите сервер Apache Tomcat. Выберите **Аргументы**. Выберите **Рабочая область** или **Файловая система**, чтобы указать папку с проектом.
...
Это первый учебник по Поиску Azure на базе Java и набора данных USGS. Мы будем постепенно расширять этот учебник, чтобы рассказать о дополнительных функциях поиска, которые вы можете использовать в собственных решениях.

Если у вас уже есть опыт работы с Поиском Azure, вы можете использовать этот пример как фундамент для дальнейших исследований, таких как улучшение [страницы поиска](../search-pagination/) или внедрение [фасетной навигации](../search-faceted-navigation/). Вы также можете улучшить страницу результатов поиска путем добавления счетчиков и пакетной обработки документов, чтобы пользователи могли просматривать результаты постранично.
