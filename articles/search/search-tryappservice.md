<properties 
   pageTitle="Бесплатная пробная версия Поиска Azure со службой приложений Azure | Microsoft Azure"
   description="Работа с бесплатной пробной версией службы поиска Azure в течение одного часа с помощью шаблона службы приложений Azure."
   services="search" 
   documentationCenter="" 
   authors="HeidiSteen" 
   manager="mblythe" 
...
# Бесплатная пробная версия Поиска Azure со службой приложений Azure

[Попробуйте службу приложений Azure](https://tryappservice.azure.com/) — это бесплатный часовой сеанс в браузере для ознакомления с службами Azure, включая **поиск Azure**. Для его использования не нужна регистрация подписки. Сайт позволяет выбрать один из нескольких шаблонов. При выборе шаблона ASP.NET, который включает поиск Azure, вы получаете доступ к полнофункциональному веб-сайту, оснащенному выбранными службами.

В **ознакомительную версию службы приложений Azure** входит служба поиска Azure, которая уже создана и готова принимать поисковые запросы. Не удается отправить или использовать собственный индекс или данные — но можно выполнять запросы и изменять пользовательский интерфейс, внося изменения в код.

Данные для поиска взяты из [геологического исследования США (USGS)](http://geonames.usgs.gov/domestic/download_data.htm), включающего примерно 3 млн записей о достопримечательностях, исторических местах, зданиях и других интересных местах США.

## Приступая к работе

Еще 1-часовой сеанс еще не начался, выполните следующие действия, чтобы приступить к работе.

1. Щелкните правой кнопкой мыши ссылку [https://tryappservice.azure.com](https://tryappservice.azure.com/) и откройте этот сайт в новом окне браузера.
2. Прокрутите страницу вниз и выберите тип приложения **Веб-приложение**. 
...
7. Выберите команду **Изменить в Visual Studio Team Services**, чтобы просмотреть решение и сайт.
9. В Visual Studio Team Services разверните узлы сеансов в верхней части страницы и выберите команду **Обзор веб-сайта**.
...
1. Откройте окно браузера, содержащее пример приложения, и выберите пункт **Подключиться к Visual Studio Team Services**.
...
При такой конфигурации результаты поиска включает совпадения для терминов поиска `Yellowstone`, `center` и `building`, а также все документы по условию `NOT North Dakota`. Так как имеет только 13 081 документов с фразой `North Dakota`, то возвращается почти весь набор данных.
...
Сведения о службе поиска Azure см. на [странице документации](https://azure.microsoft.com/documentation/services/search/) сайта [http://azure.microsoft.com](https://azure.microsoft.com/) или в [примерах и видео](search-video-demo-tutorial-list.md), посвященных полному спектру функций поиска Azure.

## Факты о коде и службе поиска Azure

Поиск Azure является полностью управляемым решением типа "платформа как услуга" ([PaaS](https://en.wikipedia.org/wiki/Platform_as_a_service)), что упрощает для разработчиков задачу интеграции удобных интерфейсов поиска в мобильные и веб-приложения. Данные, используемые в операциях поиска, хранятся в службе поиска Azure, где близость данных к операциям обеспечивает низкую задержку и предсказуемое поведение поиска. Немного подробностей:

- Доступные для поиска данные хранятся в индексе, которым управляет служба поиска Azure.
...
- Индекс поиска содержит один или несколько документов (которые аналогичны строкам в таблице), которые доступны для поиска и извлечения.
...
В предложении [службы приложений Azure](https://tryappservice.azure.com/) шаблон "ASP.NET + сайт поиска Azure" предоставляет исходный код для веб-приложения, который можно изменять в Visual Studio Team Services (в рамках сеанса длительностью один час). Для просмотра и изменения кода не требуется отдельных средств разработки.
