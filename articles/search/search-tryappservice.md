...
[Служба приложений Azure](https://tryappservice.azure.com/) — это новый, совершенно бесплатный способ тестирования выбранных служб Azure, включая поиск Azure, продолжительностью до часа без необходимости регистрации подписки на Azure.
...
На сайте [службы приложений Azure](https://tryappservice.azure.com/) поиск Azure входит в состав шаблона "Веб-приложение", предоставляя функциональный полнотекстовый интерфейс поиска с множеством функций поиска, доступных только в этой службе на платформе Azure.
...
В предложение [службы приложений Azure](https://tryappservice.azure.com/) входит служба поиска Azure, которая уже настроена и готова принимать поисковые запросы. Загружать и использовать собственный индекс или данные невозможно. Данные, с которыми вы будете работать, получены из [геологического исследования США (USGS)](http://geonames.usgs.gov/domestic/download_data.htm), состоящего из примерно трех миллионов строк с достопримечательностями, историческими местами, зданиями и другими интересными местами в США.
...
- Доступные для поиска данные хранятся в индексе, управляемом поиском Azure
...
При такой конфигурации результаты поиска включает совпадения для терминов поиска `Yellowstone`, `center` и `building`, а также все документы по условию `NOT North Dakota`. Так как имеется только 13 081 документ с фразой `North Dakota`, то возвращается почти весь набор данных.
...
Для выполнения всех этих задач необходима подписка на Azure, чтобы можно было создавать и заполнять индексы в службе. Дополнительные сведения о регистрации для получения бесплатной пробной версии см. на странице [https://azure.microsoft.com/pricing/free-trial](https://azure.microsoft.com/pricing/free-trial/).

Сведения о службе поиска Azure см. на [странице документации](http://azure.microsoft.com/documentation/services/search/) сайта [http://azure.microsoft.com](http://azure.microsoft.com) или в [примерах и видео](search-video-demo-tutorial-list.md), посвященных полному спектру функций поиска Azure.
