...
В предложение [Попробуйте службу приложений Azure](https://tryappservice.azure.com/) входит служба поиска Azure, которая уже создана и готова принимать поисковые запросы. Не удается отправить или использовать собственный индекс или данные — но можно выполнять запросы и изменять пользовательский интерфейс, внося изменения в код.

Данные поиска получены из [геологического исследования США (USGS)](http://geonames.usgs.gov/domestic/download_data.htm), состоящего из примерно 3 миллионов строк с достопримечательностями, историческими местами, зданиями и другими интересными местами США.
