...
Текущая версия пакета SDK для Поиска Azure в .NET — `0.10.0-preview`. Это предварительная версия пакета SDK. Если у вас есть предложения, как улучшить первую стабильную версию, посетите нашу [страницу отзывов](http://feedback.azure.com/forums/263029-azure-search).

Пакет SDK для .NET поддерживает версию `2015-02-28` интерфейса API REST Поиска Azure, описанную в библиотеке [MSDN](https://msdn.microsoft.com/library/azure/dn798935.aspx). Новые возможности, которые *не* включены в эту версию, например поддержка языковых обработчиков Майкрософт или параметр поиска `moreLikeThis`, находятся в [предварительной версии](../search-api-2015-02-28-preview/) и недоступны в пакете SDK. Об изменениях состояния каждой из этих функций можно узнать на странице [Управление версиями службы поиска](https://msdn.microsoft.com/library/azure/dn864560.aspx) или [Последние обновления службы поиска Azure](../search-latest-updates/).
...
1. Visual Studio 2013 или более поздней версии.

2. Ваша личная служба поиска Azure. Чтобы использовать пакет SDK, необходимо указать имя службы и один или несколько ключей API. Статья [Создание службы на портале](../search-create-service-portal/) поможет вам это сделать.
...
Пакет SDK для Поиска Azure в .NET поддерживает приложения, предназначенные для .NET Framework 4.0 или более поздней версии, а также приложения из Магазина Windows для Windows 8.1 и Windows Phone 8.1. Silverlight не поддерживается.
