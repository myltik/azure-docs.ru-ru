...
> [AZURE.NOTE]При разработке собственных классов модели для сопоставления с индексом поиска Azure обязательно объявляйте типы значений, такие как `bool` и `int`, допускающими нулевое значение (например: `bool?` вместо `bool`). Это необходимо потому, что все примитивные типы поля в поиске Azure допускают нулевое значение. При использовании типов, не допускающих нулевое значение, вы получите непредвиденные результаты при индексировании значения по умолчанию, например `0` и `false`. В частности, такие значения по умолчанию будут преобразованы в NULL во время индексирования. В будущем выпуске пакета SDK использование типов, не допускающих значения NULL, вместо этого будет порождать исключение.
