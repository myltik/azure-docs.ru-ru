...
- `analyzers`, `charFilters`, `tokenizers`, `tokenFilters` используются для настройки разбиения документов и запросов на маркеры для индексации и поиска. Дополнительные сведения см. в разделе [Лингвистический анализ с помощью анализаторов клиента в службе поиска Azure](https://aka.ms//azsanalysis).
...
> [AZURE.NOTE] При вызове **поиска** с помощью запроса POST этот параметр называется `scoringParameters`, а не `scoringParameter`. Кроме того, она задается в виде массива JSON строк, где каждая строка представляет собой отдельную пару `name-values`.
