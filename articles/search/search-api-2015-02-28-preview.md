...
Для формирования URL-адреса запроса также понадобится имя службы. Имя службы и `api-key` можно найти в панели мониторинга службы на классическом портале Azure. Дополнительные сведения о навигации по страницам см. в статье, посвященной [созданию службы поиска на портале](search-create-service-portal.md).
...
Операция **удаления индекса** удаляет из службы поиска Azure индекс и связанные с ним документы. Имя индекса можно найти в панели мониторинга службы на классическом портале Azure или получить с помощью вызова API. Более подробные сведения см. в разделе [Получение списка индексов](#ListIndexes).
...
**Поиск** принимает несколько параметров, которые сообщают критерии запроса и определяют способы поиска. Эти параметры указываются в строке запроса URL-адреса (при вызове **Поиска** с помощью запроса GET) или в теле запроса в виде свойств JSON (при вызове **Поиска** с помощью запроса POST). Синтаксис некоторых параметров зависит от выбора запроса GET и POST. Эти различия описаны ниже.
...
> [AZURE.NOTE]Чтобы запросы не возвращали слишком большое количество документов за один раз, в службе поиска Azure используется ***разбивка на страницы на стороне сервера***. Размер страницы по умолчанию — 50, максимальный размер страницы — 1000. Следовательно, если параметр `$top` не указан, по умолчанию **Поиск** будет возвращать не больше 50 результатов. Если результатов больше 50, ответ будет включать информацию для получения следующей страницы, содержащей не более 50 результатов поиска (см. `@odata.nextLink` и `@search.nextPageParameters` в [примере ниже](#SearchResponse)). Аналогично, если для параметра `$top` указано значение больше 1000, а количество результатов поиска превысит это число, будут показаны только первые 1000 результатов и информация для получения следующей страницы, содержащей не более 1000 результатов поиска.
...
Операция **вызова предложений** принимает несколько параметров, которые сообщают критерии запроса и определяют способы поиска. Эти параметры указываются в строке запроса URL-адреса (при вызове **предложений** с помощью запроса GET) или в теле запроса в виде свойств JSON (при вызове **предложений** с помощью запроса POST). Синтаксис некоторых параметров зависит от выбора запроса GET и POST. Эти различия описаны ниже.
