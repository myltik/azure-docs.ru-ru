...
> [!NOTE]
> При вызове **поиска`facet` с помощью запроса POST этот параметр называется **, а не `facets`. Кроме того, она задается в виде JSON-массива строк, где каждая строка представляет собой отдельное выражение аспектирования.
> 
> 

`$filter=[string]` (необязательный): выражение структурированного поиска в стандартном синтаксисе OData. Дополнительные сведения о подмножестве синтаксических конструкций OData, которые поддерживаются в службе поиска Azure, см. в разделе с описанием [синтаксиса выражений OData](#ODataExpressionSyntax).

> [!NOTE]
> При вызове **поиска`$filter` с помощью запроса POST этот параметр называется **, а не `filter`.
> 
> 
