<properties 
   ...
   description="Действия по развертыванию устройства и службы StorSimple и рекомендуемые методы работы."
   services="storsimple"
...
В этой серии учебных материалов описывается настройка устройств StorSimple. В нее входят: контрольный список установки, предварительные требования к конфигурации и подробное пошаговое руководство настройки.
...
|---| --------------------- | ---------------------- | ------------- |
| 1 | Понятное имя устройства | Имя, описывающее устройство | |
| 2 | Настройки сети <ol><li>IP-адрес устройства</li><li>Сетевые интерфейсы: 4 интерфейса 1 GbE, 2 интерфейса 10 GbE</li><li>Фиксированный IP-адрес контроллера</li><li>Маски подсети</li><li>Шлюз</li></ol> | Всего требуется IP-адресов: 8 <ol><li>Один на устройство</li><li>Один на сетевой интерфейс, всего 6</li><li>Один на контроллер, всего 2</li><li>Один на каждый IP-адрес</li><li>Один на устройство</li></ol> | |
| 3 | Последовательный доступ | Первоначальная конфигурация устройства | Да/нет |
| 4 | IP-адрес DNS-сервера | Необходим для подключения к Microsoft Azure: всего 2, необходимы для обеспечения высокого уровня доступности | |
| 5 | IP-адреса NTP-сервера | Необходим для синхронизации времени с Azure: 1 обязательный, 1 дополнительный | |
| 6 | Прокси-сервер \(необязательно\) | IP-адрес или полное доменное имя прокси-сервера, порт, который будет использоваться | |
| 7 | Учетная запись хранения Azure | Учетные данные доступа, такие как имя учетной записи и ключ доступа, для каждой учетной записи хранения | |
| 8 | Ключ шифрования облачного хранилища \(рекомендуется\) | На контейнер томов | |
| 9 | IQN узла | На каждый узел | |
...
> * Если вы включили автоматическое создание учетной записи хранения, перейдите к шагу 2, "Получение ключа регистрации службы".
