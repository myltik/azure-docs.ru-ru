...
3. В колонке **Диагностика** внесите следующие изменения, чтобы настроить ведение журнала диагностики.
   ...
   * Хранить и обрабатывать данные можно разными способами.
        ...
        * Выберите параметр **Send to Log Analytics** (Отправить в Log Analytics), чтобы проанализировать данные созданного журнала с помощью Log Analytics. При выборе этого параметра необходимо указать сведения для рабочей области Operations Management Suite, которая будет использоваться для анализа журнала. В разделе [представление или анализировать данные, собранные с помощью поиска журналов для аналитики журналов](../log-analytics/log-analytics-tutorial-viewdata.md) подробные сведения об использовании анализа журналов.
...
| correlationId |Строка |Идентификатор журнала, который можно использовать для сгруппировать набор соответствующие записи журнала |
...
| ClientRequestId |Строка |Идентификатор, который однозначно определяет этот запрос |
...
При отправке журналов из хранилища Озера данных Azure в службе анализа журналов Azure (см. [представление или анализировать данные, собранные с помощью поиска журналов для аналитики журналов](../log-analytics/log-analytics-tutorial-viewdata.md) подробные сведения об использовании анализа журналов), следующий запрос возвращает таблицу, содержащую список пользователей Отображает имена, время события и количество событий для время события вместе с visual диаграммы. Его можно легко изменить отображение пользовательского идентификатора GUID или другие атрибуты:
