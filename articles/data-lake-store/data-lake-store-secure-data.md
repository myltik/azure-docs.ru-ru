...
3. В колонке **Доступ** перечислены стандартные и пользовательские варианты доступа, уже назначенные корню. Щелкните значок **Добавить**, чтобы добавить ACL пользовательского уровня.
	...
	* Стандартный доступ — доступ в стиле UNIX, где права на чтение, запись, выполнение (rwx) указываются для трех различных классов пользователей: владелец, группа и прочие.
...
5. Щелкните **Выбрать разрешения**, выберите разрешения, которые вы хотите назначить группе, и нажмите кнопку **ОК**.
	...
	>[AZURE.NOTE]Разрешение на выполнение требуется для перечисления каталогов и часто требуется при предоставлении пользователю или группе доступа к данным только для чтения.
...
6. В колонке **Добавить пользовательский доступ** щелкните **ОК**. Теперь добавленная группа с соответствующими разрешениями отобразится в колонке **Доступ**.
	...
	> [AZURE.IMPORTANT]В текущем выпуске можно установить только 12 записей в разделе **Пользовательский доступ**. Если вы хотите добавить больше 12 пользователей, необходимо создать группы безопасности, добавить пользователей в группы безопасности и предоставить доступ этим группам безопасности к учетной записи хранения озера данных.
...
При удалении групп безопасности из учетной записи хранения озера данных Azure вы только изменяете доступ к операциям управления в учетной записи с помощью портала предварительной версии Azure и API диспетчера ресурсов Azure.
