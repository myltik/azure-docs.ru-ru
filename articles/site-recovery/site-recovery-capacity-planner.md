...
3.	На листе **Планировщик ресурсов** введите необходимую информацию. Необходимо заполнить все поля, помеченные красными кружками на снимке экрана ниже.
	...
	-  В поле **Хранение входных данных** укажите срок хранения реплик. При репликации VMware или физических серверов следует указать значение в днях. При репликации Hyper-V укажите время в часах.
...
2.	После указывания значений для исходной среды отобразятся такие выходные данные:
	...
	- Параметр **Требуемое число серверов конфигурации** показывает, сколько серверов конфигурации необходимо для развертывания (1)
	- Параметр **Требуемое число дополнительных серверов обработки** показывает, необходимы ли дополнительные серверы обработки кроме сервера, настроенного на сервере конфигурации по умолчанию.
	- Параметр **100 % дополнительного хранилища в исходной среде** показывает, требуется ли дополнительное хранилище в исходной среде.
...
3.	На листе **Оценка рабочей нагрузки** введите необходимую информацию. Необходимо заполнить все помеченные поля.
	...
	-  В поле **Сопоставление размера Azure** введите размер виртуальной машины Azure, которую необходимо сопоставить. Если вы не хотите делать это вручную, нажмите кнопку **Вычислить виртуальные машины IaaS**. Обратите внимание: если ввести значение вручную, а затем нажать кнопку "Вычислить виртуальные машины IaaS", введенный вручную параметр можно будет перезаписать, поскольку процесс вычисления определяет оптимальное соответствие для размера виртуальной машины Azure автоматически.
...
- В выходных данных к этому примеру обратите внимание на следующее:
	...
	>[AZURE.NOTE]Количество операций ввода-вывода в секунду для хранилищ уровня Standard и Premium рассчитывается не на уровне дисков, а на уровне виртуальных машин. Стандартная виртуальная машина может обрабатывать до 500 операций ввода-вывода на диск. Если число операций ввода-вывода на диск больше 500, необходимо хранилище уровня Premium. При этом если операций ввода-вывода на диск больше 500, но общее число операций ввода-вывода для всех дисков виртуальной машины соответствует ограничениям для стандартной виртуальной машины Azure (размер виртуальной машины, число дисков, число адаптеров, процессор, память), вместо серии DS или GS планировщик выберет стандартную виртуальную машину. Необходимо вручную обновить ячейку сопоставления размера Azure, указав соответствующую виртуальную машину серии DS или GS.

5. Указав все сведения, нажмите кнопку **Отправить данные в средство планирования**, чтобы открыть **Планировщик ресурсов**. Выделяются рабочие нагрузки, соответствующие или не соответствующие требованиям для защиты.


### Отправьте данные в планировщик ресурсов

1.	Открываемый лист **планировщика ресурсов** заполняется данными в зависимости от заданных вами параметров. В ячейке **Исходная среда входных данных инфраструктуры** появится слово "Рабочая нагрузка", чтобы показать, что источником входных данных является лист **Оценка рабочей нагрузки**. 
2.	Чтобы внести изменения, измените данные на листе **Оценка рабочей нагрузки** и нажмите кнопку "Отправить данные в средство планирования" еще раз.  
