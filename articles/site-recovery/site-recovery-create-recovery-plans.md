---
title: "Создание планов восстановления | Документация Майкрософт"
...
* Планы восстановления можно расширять с помощью сценариев и ручных действий. Обратите внимание на следующее.
  ...
  * Если вы используете VMM в развертывании, обратите внимание на следующее:
    ...
    * Если ваша общая папка библиотеки находится на удаленном сервере (или локальном, но без общего доступа через папку MSCVMMLibrary), настройте общую папку следующим образом (\\\libserver2.contoso.com\share\ используется в качестве примера):
      * Откройте редактор реестра и найдите запись Recovery\Registration HKEY_LOCAL_MACHINE\SOFTWARE\MICROSOFT\Azure.
      * Измените значение ScriptLibraryPath на \\\libserver2.contoso.com\share\.. Укажите полное доменное имя. Предоставьте разрешения для расположения общей папки.
...
* **Репликация виртуальных машин VMware и физических серверов**. Создайте план восстановления и добавьте в него группы репликации, содержащие виртуальные машины и физические серверы.
* **Репликация виртуальных машин Hyper-V (в облаках VMM)**. Создайте план восстановления и добавьте в него защищенные виртуальные машины Hyper-V из облака VMM.
* **Репликация виртуальных машин Hyper-V (не в облаках VMM)**. Создайте план восстановления и добавьте в него защищенные виртуальные машины Hyper-V из группы защиты.
* **Репликация SAN**. Создайте план восстановления и добавьте в него группу репликации, содержащую виртуальные машины. Вы выбираете группу репликации, а не отдельные виртуальные машины, поскольку отработка отказа для всех виртуальных машин в группе репликации должна выполняться одновременно (в первую очередь отработка отказа выполняется на уровне хранилища).

Создайте план восстановления следующим образом:

1. Откройте вкладку **Планы восстановления** и выберите **Создать план восстановления**.
   Укажите имя плана восстановления, а также исходный и целевой серверы. Исходный сервер должен содержать виртуальные машины, активированные для отработки отказа и восстановления.
   
   * При репликации между серверами VMM выберите **Тип источника** > **VMM**, а также укажите исходный и целевой серверы VMM. Щелкните **Hyper-V** , чтобы просмотреть облака, настроенные для использования реплики Hyper-V. 
   * При репликации между серверами VMM с использованием SAN выберите **Тип источника** > **VMM**, а также укажите исходный и целевой серверы VMM. Щелкните **SAN** , чтобы просмотреть облака, настроенные для репликации сети SAN.
   * При репликации между VMM и Azure выберите **Тип источника** > **VMM**.  Выберите исходный сервер VMM, а в качестве целевого сервера — **Azure** .
   * При репликации с сайта Hyper-V выберите **Тип источника** > **Сайт Hyper-V**. Выберите веб-сайт в качестве источника, а в качестве целевого сервера — **Azure **.
...
2. В окне **Выбор виртуальных машин** укажите виртуальные машины (или группу репликации), которые требуется добавить в группу по умолчанию (группа 1) в плане восстановления.

## <a name="customize-recovery-plans"></a>Настройка планов восстановления
...
## <a name="extend-recovery-plans-with-scripts"></a>Добавление сценариев в планы восстановления
...
### <a name="create-a-vmm-script"></a>Создание сценария VMM
...
### <a name="create-an-azure-automation-runbook"></a>Создание модуля Runbook службы автоматизации Azure
...
### <a name="add-custom-settings-to-a-recovery-plan"></a>Добавление пользовательских параметров в план восстановления
...
4. При добавлении сценария VMM выберите **Сценарий отработки отказа в VMM** и в поле **Путь к сценарию** введите относительный путь к общей папке. Например, если путь к общей папке — \\<VMMServerName>\MSSCVMMLibrary\RPScripts, укажите путь \RPScripts\RPScript.PS1.
