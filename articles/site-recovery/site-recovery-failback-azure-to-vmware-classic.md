...
2.	При выборе типа разбиения на разделы следует выбрать **Пользовательское разбиение**. Нажав кнопку **Далее**, выберите **Свободно** и щелкните "Создать". Создайте разделы **/**, **/var/crash** и **/home**, а в поле **Тип файловой системы** укажите **ext4**. Создайте раздел подкачки с типом файловой системы **swap**.
3.	При наличии существующих устройств появится предупреждающее сообщение. Щелкните **Форматировать**, чтобы отформатировать диск с заданными параметрами раздела. Щелкните **Записать изменения на диск**, чтобы применить изменения.
...
1. После установки получите идентификаторы SCSI для каждого жесткого диска SCSI на виртуальной машине. Для этого завершите работу виртуальной машины сервера управления, а в свойствах виртуальной машины VMware щелкните правой кнопкой мыши на виртуальной машине и выберите **Изменить настройки** > **Параметры**.
...
2.	В разделах **Главный целевой сервер** и **Сервер обработки** выберите локальный главный целевой сервер и сервер обработки виртуальной машины Azure.
