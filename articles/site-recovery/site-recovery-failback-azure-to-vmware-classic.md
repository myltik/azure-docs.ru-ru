...
В этой статье описывается, как восстановить размещение виртуальных машин Azure на локальном сайте с переносом из Azure. Следуйте инструкциям в этой статье после подготовки к восстановлению размещения виртуальных машин VMware или физических серверов Windows и Linux, для которых была выполнена отработка отказа с переходом с локального сайта в Azure с помощью этого [руководства](site-recovery-vmware-to-azure-classic.md).
...
- Восстановление размещения на исходной виртуальной машине сопровождается следующими требованиями:
	- Если виртуальная машина управляется сервером vCenter, для виртуальной машины на локальном сайте должно использоваться то же хранилище данных, что и для виртуальной машины, на которой запущен локальный главный целевой сервер. В противном случае ее требуется переместить, чтобы восстановление размещения выполнялось на той же виртуальной машине.
	- Если виртуальная машина находится на узле ESX, но не управляется с помощью vCenter, жесткий диск виртуальной машины и главная целевая виртуальная машина должны находиться в одном хранилище данных.
	- Если виртуальная машина находится на узле ESX и не использует vCenter, прежде чем повторно включить защиту, следует выполнить обнаружение узлов ESX. Это применимо, если выполняется восстановление размещения и для физических серверов.
...
- Для восстановления размещения при первоначальной настройке защиты необходимо создать сеть Azure. Восстановление размещения требует подключение VPN или ExpressRoute к локальному сайту через сеть Azure, в которой расположены виртуальные машины Azure.
...
1.  На странице **Серверы конфигурации** портала Site Recovery выберите команду добавления нового сервера обработки.

	![](./media/site-recovery-failback-azure-to-vmware-classic/ps1.png)

2.  Укажите имя сервера обработки, а также введите имя и пароль, которые вы будете использовать для подключения к виртуальной машине Azure в качестве администратора. В поле **Сервер конфигурации** выберите локальный сервер управления и укажите сеть Azure, в которой следует развернуть сервер обработки. Это должна быть сеть, в которой расположены виртуальные машины Azure. Укажите уникальный IP-адрес из выбранной подсети и начните развертывание.
	...
	>[AZURE.NOTE]Сервер не будет отображаться в разделе **Свойства виртуальной машины**. Он отображается только на вкладке **Серверы** на сервере управления, где он зарегистрирован. Сервер обработки отобразится примерно через 10–15 минут.
...
2.	В ответ на запрос типа установки следует выбрать **Создать пользовательский макет** в качестве раздела. Нажав кнопку **Далее**, выберите **Бесплатно** и щелкните "Создать". Создайте разделы **/**, **/var/crash** и **/home**, а для **Тип FS** укажите **ext4**. Создайте раздел переключения. Для этого укажите для **Тип FS** swap.
3.	При наличии существующих устройств появится предупреждающее сообщение. Щелкните **Форматировать**, чтобы отформатировать диск с использованием параметров раздела. Щелкните **Записать изменения на диск**, чтобы применить изменения разделов.
4.	Последовательно выберите **Установить загрузчик** > **Далее**, чтобы установить загрузчик в корневой раздел.
5.	По завершении установки нажмите кнопку **Перезагрузить**.


#### Получение идентификаторов SCSI

1. После установки получите идентификаторы SCSI для каждого жесткого диска SCSI на виртуальной машине. Для этого завершите работу виртуальной машины сервера управления, а в свойствах виртуальной машины VMware щелкните правой кнопкой мыши запись виртуальной машины и последовательно выберите **Изменить параметры** > **Параметры**.
2. Последовательно выберите **Дополнительно** > **Общий элемент** и щелкните **Параметры конфигурации**. При запуске компьютера этот параметр будет неактивен. Чтобы сделать его активным, необходимо завершить работу компьютера.
...
2.	На страницах **Главный целевой сервер** и **Сервер обработки** выберите локальный главный целевой сервер и сервер обработки виртуальной машины Azure.
...
6.	После нажатия кнопки **ОК** для повторного включения защиты задание начинает реплицировать виртуальную машину с Azure на локальный сайт. Ход выполнения операции можно отслеживать на вкладке **Задания**.
...
1.	На странице **Планы восстановления** выберите план восстановления, содержащий группу восстановления размещения, и последовательно выберите **Отработка отказа** > **Внеплановая отработка отказа**.
2.	На странице **Подтверждение отработки отказа** проверьте направление отработки отказа (в Azure) и выберите точку восстановления, до которой будет выполняться отработка отказа (последняя). Если при настройке свойств репликации вы выбрали **Несколько виртуальных машин**, можно выполнить восстановление до последней точки восстановления приложения или отказоустойчивой точки восстановления. Щелкните значок галочки, чтобы начать отработку отказа.
...
1.	На вкладке **Компьютеры** портала Site Recovery выберите виртуальные машины, для которых выполнена отработка отказа, и щелкните **Защитить повторно**. 
