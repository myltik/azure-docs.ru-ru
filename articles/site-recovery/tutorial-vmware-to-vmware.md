...
## <a name="install-component-updates"></a>Установка обновлений компонентов
...
1. Скачайте ZIP-файл [обновления](https://aka.ms/asr-scout-update5). Файл содержит следующее:
...
2. Извлеките ZIP-файлы.
    - **Сервер RX**: скопируйте файл **RX_8.0.4.0_GA_Update_4_8725872_16Sep16.tar.gz** на сервер RX и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.
    - **Сервер конфигурации и обработки**: скопируйте файл **CX_Windows_8.0.4.0_GA_Update_4_8725865_14Sep16.exe** на сервер конфигурации и сервер обработки. Дважды щелкните файл, чтобы запустить его.<br>
    - **Главный целевой сервер Windows**: чтобы обновить единый агент, скопируйте файл **UA_Windows_8.0.5.0_GA_Update_5_11525802_20Apr17.exe** на сервер. Дважды щелкните файл, чтобы запустить его. Единый агент также можно применять к исходному серверу. Если исходный сервер не был обновлен до обновления 4, необходимо обновить единый агент.
    - **Сервер vContinuum**: скопируйте **vCon_Windows_8.0.5.0_GA_Update_5_11525767_20Apr17.exe** на сервер.  Убедитесь, что вы закрыли мастер vContinuum. Дважды щелкните файл, чтобы запустить его.
    - **Главный целевой сервер Linux**: чтобы обновить единый агент, скопируйте файл **UA_RHEL6-64_8.0.4.0_GA_Update_4_9035261_26Sep16.tar.gz** на главный целевой сервер и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.
    - **Исходный сервер Windows**: на исходном сервере не требуется устанавливать агент обновления 5, если на нем уже выполняется обновление 4. Чтобы обновить единый агент, скопируйте файл **UA_Windows_8.0.5.0_GA_Update_5_11525802_20Apr17.exe** на исходный сервер. Дважды щелкните файл, чтобы запустить его.
    - **Исходный сервер Linux**: чтобы обновить единый агент, скопируйте соответствующую версию файла единого агента на сервер Linux и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.  Пример. Для 64-разрядного сервера RHEL 6.7 скопируйте файл **UA_RHEL6 64_8.0.4.0_GA_Update_4_9035261_26Sep16.tar.gz** на сервер и извлеките его содержимое. В извлеченной папке выполните команду **/Install**.
...
- Если на исходных серверах, главном целевом сервере, сервере конфигурации, сервере обработки и сервере RX уже установлено обновление 5, примените его только на главном целевом сервере. 
