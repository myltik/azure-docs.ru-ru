...
**PowerShell** | Поддерживаются | Поддерживаются
...
--- | --- | ---
Объединение сетевых адаптеров | Yes<br/><br/>Не поддерживается при репликации физических компьютеров| Yes
...
Конечные точки службы виртуальной сети (брандмауэры службы хранилища Azure и виртуальные сети) | Нет  | Нет 
...
Многопутевой (MPIO)<br></br>Протестировано с помощью Microsoft DSM, EMC PowerPath 5.7 SP4 и EMC PowerPath DSM для CLARiiON. | Yes | Yes
...
--- | --- | ---
VMDK | Yes | Недоступно
VHD (VHDX) | Недоступно | Yes
Виртуальная машина 2-го поколения | Недоступно | Yes
UEFI (EFI)| Миграция в Azure для Windows Server 2012 и более поздних версий (только для виртуальных машин VMware). </br></br> ** См. примечание в конце таблицы.  | Yes
Общий диск кластера | Нет  | Нет 
Зашифрованный диск | Нет  | Нет 
NFS | Нет  | Недоступно
SMB 3.0 | Нет  | Нет 
RDM | Yes<br/><br/> Недоступно для физических серверов. | Недоступно
Диск > 1 ТБ | Yes<br/><br/>До 4095 ГБ | Yes<br/><br/>До 4095 ГБ
Диск с физическими и логическими секторами с размером в 4 КБ. | Yes | Не поддерживается для виртуальных машин поколения 1.<br/><br/>Да, поддерживается для виртуальных машин поколения 2
Диск с физическими секторами размером в 512 байтов и логическими секторами с размером в 4 КБ. | Yes |  Yes
...
> - Миграция только в Azure. Восстановление размещения на локальный сайт VMware не поддерживается.
> - На диске операционной системы сервера поддерживается не более четырех разделов.
...
Управляемые диски | Yes | Yes<br/><br/>Восстановление после сбоя в локальную среду для виртуальной машины Azure с управляемыми дисками в настоящее время не поддерживается.
