...
11. После появления сервера в консоли Site Recovery на странице **Источник** > **Подготовка источника** выберите сервер VMM и облако, в котором расположен узел Hyper-V. Нажмите кнопку **ОК**.
...
7. В поле **Сжатие данных при передаче** укажите, требуется ли сжимать передаваемые реплицированные данные.
...
6. В разделе **Целевая сеть** выберите сеть, которую вы хотите использовать для сервера-получателя VMM. Нажмите кнопку **ОК**.
...
2. В окне **Тестовая отработка отказа** щелкните **Нет**. Так вы проверите, должным ли образом работает отработка отказа виртуальной машины, но реплицированная виртуальная машина не будет подключена к какой-либо сети.
...
4. После того, как задание отработки отказа достигнет этапа **Полное тестирование**, сделайте следующее.
