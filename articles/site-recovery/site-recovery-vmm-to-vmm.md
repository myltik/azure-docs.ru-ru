...
3. Установщик выполняет несколько **проверок предварительных требований** и запрашивает разрешение на остановку службы VMM, чтобы начать установку поставщика. Служба VMM будет автоматически перезапущена после завершения установки. При установке в кластере VMM будет выдан запрос на остановку роли Cluster.
...
9. Этот параметр используется только при репликации из VMM в Azure. В случае репликации из VMM в VMM этот экран можно проигнорировать.
...
>[AZURE.NOTE]Также можно установить поставщик Azure Site Recovery с помощью приведенной ниже командной строки. Этот метод можно использовать для установки поставщика на основные серверные компоненты Windows Server 2012 R2.
>
