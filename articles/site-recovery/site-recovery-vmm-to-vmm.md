---
title: Репликация виртуальных машин Hyper-V из облаков VMM на вторичный сайт VMM с помощью портала Azure | Microsoft Docs
...
3. На странице **Цель защиты** выберите **Для сайта восстановления** и **Yes, with Hyper-V** (Да, с помощью Hyper-V).
4. В соответствующем раскрывающемся списке выберите **Да**, чтобы указать, что вы используете VMM для управления узлами Hyper-V, и выберите **Да**, если используется сервер-получатель VMM. Если вы развертываете репликацию между облаками на одном сервере VMM, щелкните **Нет**. Нажмите кнопку **ОК**.
...
3. На странице **Центр обновления Майкрософт** можно включить обновления, чтобы обновления поставщика устанавливались в соответствии с политикой Центра обновления Майкрософт.
...
3. В раскрывающемся списке **Версия узла Hyper-V** выберите, какая операционная система выполняется на узле.
   ...
   > 
   > 
4. Используя раскрывающиеся списки **Тип проверки подлинности** и **Порт для проверки подлинности**, укажите, как аутентифицируется трафик между серверами-источниками узла Hyper-V и серверами узла восстановления Hyper-V. Выберите **Сертификат**, если только не используете рабочую среду Kerberos. Azure Site Recovery автоматически настроит сертификаты для аутентификации HTTPS. Не нужно ничего делать вручную. По умолчанию порт 8083 и 8084 (для сертификатов) будет открыт в брандмауэре Windows на серверах узлов Hyper-V. В случае выбора **Kerberos** для взаимной аутентификации серверов узлов будет использоваться билет Kerberos. Обратите внимание, что этот параметр используется только при запуске серверов узлов Hyper-V в Windows Server 2012 R2.
...
9. Выберите для параметра **Удалить виртуальную машину реплики** значение "Да", чтобы указать, что при отключении защиты для исходной виртуальной машины нужно удалить виртуальную машину реплики. Если включить этот параметр, то при отключении защиты для исходной виртуальной машины она удаляется из консоли Site Recovery, параметры Site Recovery для VMM удаляются из консоли VMM, также удаляется реплика.
10. В случае репликации по сети укажите в разделе **Метод начальной репликации**, следует ли запустить начальную репликацию или запланировать ее. Чтобы сэкономить пропускную способность сети, вы можете запланировать ее вне периодов максимальной нагрузки. Нажмите кнопку **ОК**.
...
3. В колонке **Цель** проверьте сервер-источник VMM и вторичное облако.
...
Ход выполнения действия **включения защиты** можно отслеживать, выбрав "Параметры" > **Задания** > **Site Recovery jobs** (Задания Site Recovery). После запуска задания **Завершение подготовки защиты** виртуальная машина готова к отработке отказа.
...
3. В окне **Тестовая отработка отказа** щелкните **Нет**. Так вы проверите, должным ли образом работает отработка отказа виртуальной машины, но реплицированная виртуальная машина не будет подключена к какой-либо сети.
   
    ![Тестовая отработка отказа](./media/site-recovery-vmm-to-vmm/test-failover1.png)
4. Нажмите кнопку **ОК**, чтобы запустить отработку отказа. За ходом выполнения можно следить в окне свойств, которое можно открыть, щелкнув виртуальную машину, или в задании **тестовой отработки отказа**, выбрав **Параметры** > **Задания** > **Site Recovery jobs** (Задания Site Recovery).
