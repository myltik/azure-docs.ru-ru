...
2. [Подготовить все к сетевому сопоставлению](#prepare-for-network-mapping). Настройте сети, чтобы можно было настроить сетевое сопоставление.
...
2. Последовательно выберите **Создать** > **Управление** > **Службы восстановления**. В качестве альтернативы можно щелкнуть **Обзор** > **Хранилища служб восстановления** > **Добавить**.
...
4. Выберите **Да**, чтобы указать, что вы используете VMM для управления узлами Hyper-V, и выберите **Да**, если используется вторичный сервер VMM. Если вы разворачиваете репликацию между облаками на одном сервере VMM, щелкните **Нет**. Нажмите кнопку **ОК**. 
...
4. На странице **Параметры прокси-сервера** укажите, как запущенный на сервере VMM поставщик будет подключаться к Site Recovery через Интернет.
	...
	- - Для подключения с помощью прокси-сервера, который уже настроен на компьютере, установите переключатель **Connect with existing proxy settings** (Подключение с параметрами существующего прокси-сервера).
...
5. На странице **Регистрация** > **Расположение для сохранения сертификата** щелкните **Далее**. Сертификат для шифрования не важен в данном сценарии. Он используется только при репликации в службу хранилища Azure.
...
2. На странице **Create and associate policy** (Создание и сопоставление политики) укажите имя политики. Исходным и целевым типами должен быть **Hyper-V**.
3. Из раскрывающегося списка **Версия узла Hyper-V** выберите, какая операционная система выполняется на узле.
...
3. Из раскрывающегося списка **Периодичность копирования** выберите периодичность репликации разницы данных после начальной репликации (каждые 30 секунд, 5 или 15 минут).
...
8. Выберите параметр **Удалить виртуальную машину реплики**, чтобы указать, что при отключении защиты для исходной виртуальной машины виртуальную машину реплики нужно удалить. Если включить этот параметр, то при отключении защиты для исходной виртуальной машины она удаляется из консоли Site Recovery, параметры Site Recovery для VMM удаляются из консоли VMM, также удаляется реплика.
...
3. В разделе **Исходная сеть** из списка сетей виртуальных машин, связанных с первичным сервером VMM, выберите нужную сеть. 
...
1.	Нажмите кнопку **Скачать**, чтобы скачать этот инструмент, и запустите его. [Прочитайте статью](site-recovery-capacity-planner.md) о данном инструменте.
...
Ход выполнения задания **включения защиты** можно отслеживать на странице заданий ("Параметры" > **Задания** > **Site Recovery jobs** (Задания Site Recovery)). После запуска задания **завершения подготовки защиты** виртуальная машина будет готова к отработке отказа.
...
1. Чтобы выполнить отработку отказа одной виртуальной машины, в разделе **Параметры** > **Реплицированные элементы** щелкните виртуальную машину и выберите **+Test Failover** (+Тестовая отработка отказа).
...
3. Нажмите кнопку **ОК**, чтобы начать отработку отказа. За ходом выполнения можно следить в окне свойств, которые можно открыть, щелкнув виртуальную машину, или в задании **тестовой отработки отказа**, выбрав **Параметры** > **Задания** > **Site Recovery jobs** (Задания Site Recovery).
4. После того, как задание отработки отказа достигнет фазы **Полное тестирование**, выполните следующее.
...
6. Убедившись, что виртуальные машины успешно запускаются, щелкните **Тестовая отработка отказа выполнена**. На этом этапе любые элементы, автоматически созданные Site Recovery во время тестовой отработки отказа, удаляются.  
