...
В этой статье описывается, как с помощью [Azure Site Recovery](site-recovery-overview.md) на портале Azure выполнить репликацию локальных виртуальных машин Hyper-V, управление которыми осуществляется в облаках System Center Virtual Machine Manager (VMM), на дополнительный сайт. Дополнительные сведения об архитектуре этого сценария см. [здесь](site-recovery-components.md#replicate-hyper-v-vms-to-a-secondary-site).
...
3. Создайте хранилище служб восстановления. Это хранилище содержит параметры конфигурации и управляет репликацией.
...
Действие **включения защиты** можно отслеживать, выбрав **Параметры** > **Задания** > **Задания Site Recovery**. Когда завершится задание **Завершить подготовку защиты	**, виртуальная машина будет готова к отработке отказа.
