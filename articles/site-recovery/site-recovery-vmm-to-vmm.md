...
Инструкции приведены в статье [Создание классификаций хранилищ в VMM](http://go.microsoft.com/fwlink/?LinkId=400937).
...
5. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик подключается к Интернету. Выберите пункт *Use default system proxy settings* (Использовать настройки прокси-сервера по умолчанию), чтобы использовать настройки подключения к Интернету, по умолчанию заданные на сервере.
	...
	![Параметры Интернета](./media/site-recovery-vmm-to-vmm/VMMASRRegisterProxyDetailsScreen.png)
	- Если вы хотите использовать пользовательский прокси-сервер, его следует настроить перед установкой поставщика. При настройке параметров пользовательского прокси-сервера будет выполнена проверка прокси-подключения.
	- Если используется пользовательский прокси-сервер или если прокси-сервер по умолчанию требует проверки подлинности, необходимо ввести данные прокси-сервера, включая адрес и порт.
	- С сервера VMM и узлов Hyper-V должны быть доступны следующие URL-адреса:
		- *.hypervrecoverymanager.windowsazure.com;
		- *.accesscontrol.windows.net;
		- *.backup.windowsazure.com;
		- *.blob.core.windows.net;
		- *.store.core.windows.net.
	- Разрешите доступ с IP-адресов, описанных в статье [Диапазоны IP-адресов центров обработки данных Azure](http://go.microsoft.com/fwlink/?LinkId=511094), и для протокола HTTPS (порт 443). Необходимо разрешить диапазоны IP-адресов региона Azure, который вы планируете использовать, и Запада США.
...
8. В поле синхронизации **Initial cloud metadata** (Первоначальные метаданные в облаке) выберите, нужно ли синхронизировать метаданные для всех облаков сервера VMM в хранилище. На каждом сервере это действие требуется выполнять только один раз. Если не требуется синхронизировать все облака, можно оставить этот флажок снятым и синхронизировать каждое облако по отдельности в свойствах облака в консоли VMM.
	![Регистрация сервера](./media/site-recovery-vmm-to-vmm/VMMASRRegisterFriendlyName.png)
