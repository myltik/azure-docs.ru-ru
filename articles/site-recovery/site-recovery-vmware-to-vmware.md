---
title: "Настройка аварийного восстановления виртуальных машин VMware или физических серверов в дополнительном расположении | Документация Майкрософт"
description: "В этой статье объясняется, как с помощью Azure Site Recovery выполнить репликацию локальных виртуальных машин VMware или физических серверов Windows и Linux в дополнительное расположение."
...
---
# <a name="set-up-disaster-recovery-of-vmware-virtual-machines-or-physical-servers-to-a-secondary-site"></a>Настройка аварийного восстановления виртуальных машин VMware и физических серверов в дополнительном расположении
...
7. Новое хранилище появится в колонке "Панель мониторинга" > "Все ресурсы" и на основной странице "Хранилища служб восстановления".

## <a name="configure-the-vault-and-download-inmage-scout-components"></a>Настройка хранилища и скачивание компонентов InMage Scout
1. На странице "Хранилища служб восстановления" выберите хранилище и щелкните **Параметры**.
...
1. Скачайте ZIP-файл [обновления](https://aka.ms/asr-scout-update5). Этот ZIP-файл содержит следующие файлы:
...
5. **Для главного целевого сервера Windows**: чтобы обновить единый агент, скопируйте файл **UA_Windows_8.0.5.0_GA_Update_5_11525802_20Apr17.exe** на главный целевой сервер. Дважды щелкните файл, чтобы запустить его. Обратите внимание, что единый агент также можно применять к исходному серверу, если исходный сервер не обновлен до обновления 4. Его следует установить на исходном сервере, как описано ниже.<br>
6. **Для сервера vContinuum**: скопируйте файл **vCon_Windows_8.0.5.0_GA_Update_5_11525767_20Apr17.exe** на сервер vContinuum.  Убедитесь, что вы закрыли мастер vContinuum. Дважды щелкните файл, чтобы запустить его.<br>
...
8. **Для исходного сервера Windows**: на исходном сервере не требуется устанавливать агент обновления 5, если к нему уже применено обновление 4. Если версия обновления ниже версии 4, примените агент обновления 5.
Чтобы обновить единый агент, скопируйте файл **UA_Windows_8.0.5.0_GA_Update_5_11525802_20Apr17.exe** на исходный сервер. Дважды щелкните файл, чтобы запустить его. <br>
...
Обновление 5 для Scout является накопительным. Оно содержит все исправления из обновлений 1–4, а также приведенные ниже исправления ошибок и улучшения.
Исправления, которые добавлены из обновления 4 для Site Recovery Scout в обновление 5, относятся к компонентам главного целевого сервера и vContinuum. Если на всех исходных серверах, главном целевом сервере, сервере конфигурации, сервере обработки и RX уже установлено обновление 4 для Site Recovery Scout, обновление 5 следует применять только к главному целевому серверу. 
...
* Повышение надежности поддержки кластера Windows
    * Исправлено. Иногда некоторые диски кластера P2V MSCS переходили в режим RAW после восстановления.
    * Исправлено. Сбой восстановления кластера P2V MSCS из-за несоответствия порядка дисков.
    ...
    * Исправлено. Сбой повторной защиты сервера кластера P2V Windows при наличии дисков целевого кластера. 
    
* Если во время защиты при отработке отказа выбранный MT отсутствует на том же сервере ESXi, что и исходный защищенный компьютер (во время защиты перенаправления), vContinuum выбирает неправильный главный целевой сервер во время восстановления при отработке отказа и происходит сбой последующей операции восстановления.

> [!NOTE]
> 
> * Приведенные выше исправления кластера P2V применяются только к тем физическим кластерам MSCS, которые были недавно защищены с помощью обновления 5 для Site Recovery Scout. Чтобы применить исправления кластера к кластеру P2V MSCS, уже защищенному с помощью более старых обновлений, необходимо выполнить действия по обновлению, описанные в разделе 12 об обновлении защищенных кластеров P2V MSCS с помощью обновления 5 в [заметках о выпуске](https://aka.ms/asr-scout-release-notes).
> 
> * При повторной защите физического кластера MSCS можно повторно использовать существующие целевые диски, только если во время повторной защиты на каждом узле кластера активен тот же набор дисков. В противном случае следует вручную выполнить действия из раздела 12 [заметок о выпуске](https://aka.ms/asr-scout-release-notes) для перемещения целевых дисков по правильному пути к хранилищу данных для их повторного использования при повторной защите. При повторной защите кластера MSCS в режиме P2V без выполнения обновления будет создан новый диск на целевом сервере ESXi. Необходимо вручную удалить старые диски из хранилища данных.
> 
> * При каждой успешной перезагрузке исходного сервера SLES11 или сервера SLES11 с любым пакетом обновления необходимо вручную пометить пары репликации **корневого** диска для повторной синхронизации, так как соответствующее уведомление не выводится в пользовательском интерфейсе CX. Если этого не сделать, могут возникнуть проблемы целостности данных.
> 
