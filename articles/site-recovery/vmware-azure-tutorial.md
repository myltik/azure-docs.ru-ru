...
* [Подготовка Azure](tutorial-prepare-azure.md).
* [Подготовка локальной среды VMware](vmware-azure-tutorial-prepare-on-premises.md).
...
> [!TIP]
> Рекомендуемый метод развертывания сервера конфигурации для защиты виртуальной машины VMware — использование модели развертывания на основе OVF, как предлагается в этой статье. Если в организации имеются ограничения, которые препятствуют развертыванию шаблона OVF, можно использовать [файл установки UnifiedSetup.exe, чтобы установить сервер конфигурации](physical-manage-configuration-server.md).
...
2. В меню **Файл** выберите **Deploy OVF Template** (Развернуть шаблон OVF), чтобы запустить мастер развертывания шаблона OVF. 
...
2. В разделе **Оборудование** выберите **Добавить** > **Ethernet Adapter** (Адаптер Ethernet). Затем нажмите кнопку **Далее**.
...
5. Введите имя, используемое для регистрации сервера конфигурации с помощью Site Recovery. Затем нажмите кнопку **Далее**.
...
3. В разделе **Install third-party software** (Установка стороннего программного обеспечения) примите условия лицензионного соглашения. Выберите **Download and Install** (Скачать и установить), чтобы установить сервер MySQL.
4. Выберите **Install VMware PowerLCI** (Установить VMware PowerCLI). Прежде чем это сделать, закройте все окна браузера. Затем выберите **Продолжить**.
...
7. Введите учетные данные, которые будут использоваться сервером конфигурации для подключения к серверу VMware. Site Recovery использует эти учетные данные для автоматического обнаружения виртуальных машин VMware, доступных для репликации. Выберите **Добавить**, а затем щелкните **Продолжить**.
...
10. По окончании регистрации на портале Azure убедитесь, что сервер конфигурации и VMware указаны на странице **Источник** в хранилище. Нажмите кнопку **OK**, чтобы настроить параметры целевого объекта.
