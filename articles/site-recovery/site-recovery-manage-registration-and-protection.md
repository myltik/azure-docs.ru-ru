...
## <a name="unregister-a-hyper-v-server-in-a-hyper-v-site"></a>Отмена регистрации сервера Hyper-V на узле Hyper-V
...
## <a name="stop-protecting-a-hyper-v-virtual-machine"></a>Отключение защиты виртуальной машины Hyper-V
...
### <a name="remove-protection"></a>Снятие защиты
...
2. На странице **Подтверждение удаления виртуальной машины** доступно два варианта.
   ...
   * **Удалить из хранилища**. После выбора этого варианта виртуальная машина будет удалена только из хранилища Site Recovery. Локальные параметры защиты для виртуальной машины не будут затронуты. Для удаления виртуальной машины из подписки Azure и удаления параметров защиты потребуется вручную выполнить приведенные ниже инструкции.
...
## <a name="stop-protecting-a-vmware-virtual-machine-or-a-physical-server"></a>Остановите защиту виртуальной машины VMware или физического сервера
...
2. В окне **Удаление виртуальной машины** выберите один из вариантов.
   ...
   * **Отключить защиту**. Если включить и сохранить этот параметр, служба Site Recovery больше не будет защищать виртуальную машину. Параметры защиты для виртуальной машины будут автоматически удалены.
   * **Удалить из хранилища**. После выбора этого параметра виртуальная машина будет удалена только из хранилища Site Recovery. Локальные параметры защиты для виртуальной машины не будут затронуты. Для удаления параметров на виртуальной машине и удаления виртуальной машины из подписки Azure потребуется очистить параметры путем удаления службы Mobility Service.
