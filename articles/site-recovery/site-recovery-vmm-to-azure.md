...
6. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик будет подключаться к Site Recovery через Интернет.
	...
	- Если для имеющегося прокси-сервера требуется проверка подлинности или нужно использовать пользовательский прокси-сервер, установите переключатель **Подключиться к Azure Site Recovery с использованием прокси-сервера**.
...
1. Последовательно выберите **Параметры** > **Инфраструктура Site Recovery** > **Сетевые сопоставления** > **Сетевые сопоставления** и щелкните значок **+Network Mapping** (+Сетевое сопоставление).
...
Ход выполнения задания **включения защиты** можно отслеживать на странице заданий (**Параметры** > **Задания** > **Задания Site Recovery**). Когда закончится задание **завершения подготовки к защите**, виртуальная машина будет готова к отработке отказа.
