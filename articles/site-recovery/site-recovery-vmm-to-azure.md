...
5. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик подключается к Интернету. Выберите пункт *Use default system proxy settings* (Использовать настройки прокси-сервера по умолчанию), чтобы использовать настройки подключения к Интернету, по умолчанию заданные на сервере.
	...
	- Если вы хотите использовать настраиваемый прокси-сервер, его следует настроить перед установкой поставщика. При настройке параметров пользовательского прокси-сервера будет выполнена проверка его подключения.
	- Если используется пользовательский прокси-сервер или если ваш прокси-сервер требует аутентификации, необходимо ввести его данные, включая адрес прокси-сервера и порт.
