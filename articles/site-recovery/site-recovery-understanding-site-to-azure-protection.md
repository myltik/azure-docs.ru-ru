...
![Устранение неполадок локальной среды Hyper-V](media/site-recovery-understanding-site-to-azure-protection/image01.png)
...
![Устранение неполадок локальной среды Hyper-V](media/site-recovery-understanding-site-to-azure-protection/image02.png)
...
*Автоматическое выполнение повторной синхронизации* по умолчанию выполняется в нерабочее время. Если вы желаете выполнить повторную синхронизацию виртуальной машины вручную, выберите виртуальную машину на портале и щелкните «ПОВТОРНАЯ СИНХРОНИЗАЦИЯ». ![Устранение неполадок локальной среды Hyper-V](media/site-recovery-understanding-site-to-azure-protection/image04.png)
...
| Устранимая ошибка | Повторные попытки выполняются в рамках каждого интервала репликации с использованием экспоненциального алгоритма отсрочки. При этом каждая следующая попытка выполняется с большей задержкой (1, 2, 4, 8, 10 минут). Если проблема не решается, повторные попытки выполняются каждые 30 минут. Примеры таких ошибок: <ul><li>сетевая ошибка;</li><li>недостаточно места на диске;</li><li>недостаточно памяти.</li></ul>|
