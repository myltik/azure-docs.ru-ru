...
3. В разделе **Подготовка серверов VMM** выберите пункт **Generate registration key** (Создать регистрационный ключ). Файл ключа создается автоматически. Срок действия ключа — пять дней с момента создания. Если вы получаете доступ к порталу Azure не с сервера VMM, вам потребуется скопировать этот файл на сервер.
...
1. Выберите **Быстрый запуск** > **Подготовка серверов VMM** и щелкните **Скачайте поставщик Microsoft Azure Site Recovery для установки на серверы VMM**, чтобы получить последнюю версию установочного файла поставщика.
...
После регистрации метаданные с сервера VMM извлекаются службой Azure Site Recovery. Сервер отображается на вкладке **Серверы VMM** на странице **Серверы** в хранилище.
...
## <a name="step-5-install-the-azure-recovery-services-agent"></a>Шаг 5. Установка агента служб восстановления Azure
Установите агент служб восстановления Azure на всех серверах узлов Hyper-V в облаках VMM.

1. Выберите **Быстрый запуск** > **Download Azure Site Recovery Services Agent and install on hosts** (Скачать агент служб Azure Site Recovery и установить его на узлах), чтобы получить последнюю версию файла установки агента.
...
5. По завершении установки нажмите кнопку **Закрыть** , чтобы завершить работу мастера.
...
После регистрации сервера VMM можно настроить параметры защиты облака. Параметр **Synchronize cloud data with the vault** (Синхронизировать облачные данные с хранилищем) был включен при установке поставщика, поэтому все облака на сервере VMM появятся на вкладке <b>Защищенные элементы</b> в хранилище.
...
5. Задайте для параметра **Шифрование сохраняемых данных** значение **Выкл.**. Этот параметр определяет, что данные должны шифроваться при репликации между локальным сайтом и Azure.
...
1. Чтобы включить защиту, в облаке, на котором расположена виртуальная машина, на вкладке **Виртуальные машины** щелкните **Включить защиту** > **Добавить виртуальные машины**.
2. Из списка виртуальных машин в облаке выберите те, которые хотите защитить.
    ...
    За ходом выполнения действия **Включить защиту**, в том числе и за первоначальной репликацией, вы можете наблюдать на вкладке **Задания**. После запуска задания **Завершение подготовки защиты** виртуальная машина готова к отработке отказа. После включения защиты и репликации виртуальных машин вы сможете просматривать их в Azure.

    ![Задание защиты виртуальной машины](./media/site-recovery-vmm-to-azure-classic/vm-jobs.png)

1. Проверьте свойства виртуальной машины и внесите в них необходимые изменения.

    ![Проверка виртуальных машин](./media/site-recovery-vmm-to-azure-classic/vm-properties.png)
2. В свойствах виртуальной машины на вкладке **Настройка** можно изменить следующие свойства сети:

* **Число сетевых адаптеров на целевой виртуальной машине.** Число сетевых адаптеров зависит от размера, указанного для целевой виртуальной машины. Ознакомьтесь со [спецификациями размеров виртуальных машин](../virtual-machines/virtual-machines-linux-sizes.md#size-tables), чтобы узнать, какое число сетевых адаптеров поддерживается для нужного вам размера виртуальной машины. После изменения размера виртуальной машины и сохранения параметров при следующем открытии страницы **Настройка** количество сетевых адаптеров изменится. Количество сетевых адаптеров целевых виртуальных машин — это минимальное количество сетевых адаптеров исходной виртуальной машины, а также максимальное количество сетевых адаптеров, поддерживаемое размером выбранной виртуальной машины:
...
* **Сеть целевой виртуальной машины.** Сеть, к которой подключается виртуальная машина, определяется при сопоставлении сети исходной виртуальной машины. Если у исходной виртуальной машины есть несколько сетевых адаптеров, а исходные сети сопоставлены с разными сетями на целевом узле, вам нужно выбрать одну из целевых сетей.
* **Подсеть для каждого из сетевых адаптеров.** Для каждого сетевого адаптера вы можете выбрать подсеть, к которой будет подключаться виртуальная машина при отработке отказа.
* **Целевой IP-адрес.** Если сетевой адаптер исходной виртуальной машины использует статический IP-адрес, вы можете указать его. Используйте эту возможность, чтобы сохранить IP-адрес исходной виртуальной машины после отработки отказа. Если IP-адрес не указан, при отработке отказа сетевому адаптеру будет присвоен любой доступный IP-адрес. Если целевой IP-адрес указан, но он уже используется другой виртуальной машиной в Azure, произойдет сбой отработки отказа.  
...
* После отработки отказа для подключения к виртуальной машине в Azure с удаленного рабочего стола воспользуйтесь общедоступным IP-адресом. Если необходимо это сделать, убедитесь в отсутствии политик домена, которые не допустили бы подключение к виртуальной машине с использованием общедоступного адреса.

> [!NOTE]
> Для обеспечения оптимальной производительности при отработке отказа в Azure установите на виртуальной машине агент Azure. Это ускоряет загрузку и помогает устранять неполадки. Скачайте [агент Linux](https://github.com/Azure/WALinuxAgent) или [агент Windows](http://go.microsoft.com/fwlink/?LinkID=394789).
>
>

### <a name="create-a-recovery-plan"></a>Создайте план восстановления
1. На вкладке **Планы восстановления** добавьте новый план. Укажите имя, в поле **Тип источника** выберите **VMM**, а в поле **Источник** — исходный сервер VMM. В качестве целевого расположения выберите Azure.
...
Созданный план восстановления появится на вкладке **Планы восстановления** . Кроме того, чтобы автоматизировать отработку отказа, в план восстановления можно включить [модули Runbook службы автоматизации Azure](site-recovery-runbook-automation.md) .
...
5. Когда операция отработки отказа дойдет до этапа **полного тестирования**, нажмите кнопку **Завершить тест**. Вы можете перейти на вкладку **Задания** , чтобы отслеживать ход выполнения отработки отказа и изменение ее состояния, а также выполнять прочие необходимые действия.
6. После отработки отказа тестовая реплика виртуальной машины будет отображаться на портале Azure. Если настройки предусматривают доступ к виртуальным машинам из локальной сети, можно инициировать подключение протокола удаленного рабочего стола к виртуальной машине. Выполните следующее:
   ...
   3. Если вы используете общедоступный IP-адрес для подключения к виртуальной машине в Azure с помощью удаленного рабочего стола, после отработки отказа убедитесь, что у вас нет политик доменов, которые препятствуют подключению к виртуальной машине с помощью общедоступного адреса.
