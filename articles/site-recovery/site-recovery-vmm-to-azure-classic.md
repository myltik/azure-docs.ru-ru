...
7. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик подключается к Интернету. Щелкните **Connect with existing proxy settings** (Подключиться с существующими настройками прокси-сервера), чтобы использовать параметры подключения к Интернету, настроенные на сервере по умолчанию.
...
8. В поле **Регистрационный ключ** введите ключ, который вы скачали в Azure Site Recovery и скопировали на сервер VMM.
...
1. Последовательно выберите **Быстрый запуск** > **Download Azure Site Recovery Services Agent and install on hosts** (Скачать и установить на узлах агент служб Azure Site Recovery), чтобы получить последнюю версию файла установки агента.
...
2. Из списка виртуальных машин в облаке выберите те, которые хотите защитить.
	...
	За ходом **включения защиты**, в том числе за первоначальной репликацией, можно наблюдать на вкладке **Задания**. После запуска задания **Завершение подготовки защиты** виртуальная машина готова к отработке отказа. После включения защиты и репликации виртуальных машин вы сможете просматривать их в Azure.
...
2. На странице **Выбор виртуальных машин** выберите виртуальные машины, которые нужно добавить в план восстановления. Эти виртуальные машины добавляются в группу плана восстановления, которая используется по умолчанию (группа).
3. 
4. 
5. 1. Проводилось тестирование сценария с максимум 100 виртуальными машинами в одном плане восстановления.
