...
1. Прочитайте о [расширенном развертывании на классическом портале](site-recovery-vmware-to-azure-classic.md). Изучите расширенную [архитектуру](site-recovery-vmware-to-azure-classic.md), а также [предварительные требования](site-recovery-vmware-to-azure-classic.md).
...
3. Скачайте [ключ регистрации хранилища](site-recovery-vmware-to-azure-classic.md) и [запустите единый мастер настройки](site-recovery-vmware-to-azure-classic.md), чтобы установить сервер конфигурации, сервер обработки и компоненты главного целевого сервера. Узнайте больше о [планировании ресурсов](site-recovery-vmware-to-azure-classic.md).
4. [Настройте учетные данные](site-recovery-vmware-to-azure-classic.md) , которые Site Recovery сможет использовать для доступа к серверу VMware для автоматического обнаружения виртуальных машин VMware. Узнайте, [какие требуются разрешения](site-recovery-vmware-to-azure-classic.md).
5. Добавьте [серверы vCenter или узлы vSphere](site-recovery-vmware-to-azure-classic.md). Может пройти более 15 минут, прежде чем серверы появятся на портале Site Recovery.
6. Создайте [группу защиты](site-recovery-vmware-to-azure-classic.md). Порталу может потребоваться до 15 минут для обновления, после чего будут отображаться и обнаруживаться виртуальные машины. Если вы не хотите ждать, можно выделить имя сервера управления (но не щелкать его) и нажать кнопку **Обновить**.
...
| **Служба хранилища Azure** |Для хранения реплицируемых данных вам понадобится учетная запись хранения Azure.<br/><br/> Это должна быть [учетная запись геоизбыточного хранения уровня "Стандартный"](../storage/common/storage-redundancy.md#geo-redundant-storage) или [учетная запись хранения уровня "Премиум"](../storage/common/storage-premium-storage.md).<br/><br/> Она должна находиться в том же регионе, что и служба Azure Site Recovery, и быть связана с той же подпиской. Мы не поддерживаем перемещение учетных записей хранения, созданных с помощью [нового портала Azure](../storage/common/storage-create-storage-account.md) , между группами ресурсов.<br/><br/> Дополнительные сведения см. в статье [Введение в службу хранилища Microsoft Azure](../storage/common/storage-introduction.md). | |
...
> [!NOTE]
> Выберите размер Standard DS4, чтобы настроить защиту для рабочих нагрузок, предусматривающих интенсивные операции ввода-вывода с малыми задержками, и разместить их в [учетной записи хранения уровня "Премиум"](../storage/common/storage-premium-storage.md).
>
>
