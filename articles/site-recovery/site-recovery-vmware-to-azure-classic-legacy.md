<properties pageTitle="Репликация виртуальных машин и физических серверов VMware в Azure с помощью Azure Site Recovery (устарело) | Microsoft Azure" description="Описывает устаревшее развертывание для настройки Azure Site Recovery и оркестрации репликации, отказоустойчивости и восстановления локальных виртуальных машин VMware и физических серверов Windows/Linux в Azure." " services="site-recovery" documentationCenter="" authors="rayne-wiselman" manager="jwhit" editor=""/>
...
- **Репликация физических серверов в Azure** — разверните службу Site Recovery, чтобы координировать репликацию, отработку отказа и восстановление локальных физических серверов Windows и Linux в Azure.

>[AZURE.NOTE]Сценарий, описанный в этой статье, включает **устаревшие инструкции**. Не выполняйте процедуру в этой статье для новых развертываний. Вместо этого используйте инструкции по [расширенному развертыванию](site-recovery-vmware-to-azure-classic) (для классического портала). Если вы уже выполнили развертывание с помощью метода, описанного в этой статье, рекомендуется перейти на новую версию, как описано ниже.
...
- Параметр **Перенос машин** будет доступен только в том случае, если у вас есть существующие группы защиты, созданные во время развертывания устаревших версий.
...
2. Получите [ключ регистрации хранилища](site-recovery-vmware-to-azure-classic.md#step-3:-download-a-vault-registration-key) и [запустите единый мастер настройки](site-recovery-vmware-to-azure-classic.md#step-4:-install-the-management-server) для установки сервера конфигурации, сервера обработки и главного целевого сервера на сервере управления. Дополнительные сведения о [планировании загрузки](site-recovery-vmware-to-azure-classic.md#capacity-planning).
3. Если у вас есть сервер VMware vCenter, [настройте учетные данные](site-recovery-vmware-to-azure-classic.md#step-5:-set-up-credentials-for-the-vcenter-server) для доступа к нему, чтобы компонент Site Recovery мог автоматически обнаруживать виртуальные машины, которыми он управляет. Дополнительные сведения о [необходимых разрешениях](site-recovery-vmware-to-azure-classic.md#vmware-permissions-for-vcenter-access).
4. Добавьте [серверы vCenter или узлы ESXi](site-recovery-vmware-to-azure-classic.md#step-6:-add-vcenter-servers-and-esxi-hosts). Порталу может потребоваться до 15 минут для обновления, после чего будут отображаться учетные данные.
5. Создайте [новую группу защиты](site-recovery-vmware-to-azure-classic.md#step-7:-create-a-protection-group). Порталу может потребоваться до 15 минут для обновления, после чего будут отображаться и обнаруживаться виртуальные машины. Если вы не хотите ждать, можно выделить имя сервера управления (но не щелкая) > **Обновить**.
6. В новой группе защиты щелкните **Перенос машин**.

	![Добавление учетной записи](./media/site-recovery-vmware-to-azure-classic-legacy/legacy-migration1.png)

7. В разделе **Выбор машин** выберите группу безопасности, из которой вы хотите перенести, и машины, которые требуется перенести.

	![Добавление учетной записи](./media/site-recovery-vmware-to-azure-classic-legacy/legacy-migration2.png)

8. В **параметрах настройки целевого объекта** укажите, хотите ли вы использовать те же параметры для всех компьютеров, а затем выберите сервер обработки и учетную запись хранения Azure. Если необходимо настроить один сервер управления, то сервер обработки будет IP-адресом этого сервера управления.

	![Добавление учетной записи](./media/site-recovery-vmware-to-azure-classic-legacy/legacy-migration3.png)

9. В разделе **Указать учетные записи** выберите учетную запись, созданную для автоматической передачи новой версии службы Mobility Service на защищенные машины.

	![Добавление учетной записи](./media/site-recovery-vmware-to-azure-classic-legacy/legacy-migration4.png)

10. Site Recovery перенесет реплицированные данные в учетную запись хранилища Azure, которую вы указали. При необходимости можно повторно использовать учетную запись хранения, использованную в развертывании устаревших версий.
11. После завершения задания виртуальные машины будут автоматически синхронизированы. После завершения синхронизации виртуальные машины можно удалить из группы защиты прежних версий.
12. После миграции всех машин можно удалить группу защиты прежних версий.
13. Не забудьте указать свойства отработки отказа для машин, а также параметры сети Azure после завершения синхронизации.
14. При наличии существующих планов восстановления можно перенести их в расширенное развертывание с помощью переноса плана восстановления. Это следует делать только после миграции всех защищенных машин. 
...
- Производительность и ограничения хранилища Azure
	- Максимальное количество высоко загруженных дисков для виртуальной машины стандартного уровня составляет около 40 (20 000/500 операций ввода-вывода на диск) в одной учетной записи хранения. Дополнительную информацию см. в разделе [Целевые показатели масштабируемости для учетных записей хранения Standard](../storage/storage-scalability-targets.md#scalability-targets-for-standard-storage-accounts). Дополнительные сведения об учетной записи хранения Premium см. также в разделе [Целевые показатели масштабируемости для учетных записей хранения Premium](../storage/storage-scalability-targets.md#scalability-targets-for-premium-storage-accounts).
...
**Служба хранилища Azure** | <p>Для хранения реплицируемых данных потребуется учетная запись хранения Azure</p><p> Учетная запись должна быть либо [учетной записью геоизбыточного хранилища Standard](../storage/storage-redundancy.md#geo-redundant-storage), либо [учетной записью хранения Premium](../storage/storage-premium-storage-preview-portal.md).</p><p>Она должна находиться в том же регионе, что и служба Azure Site Recovery, и быть связанной с той же подпиской.</p><p>Дополнительную информацию см. в разделе [Знакомство со службой Microsoft Azure](../storage/storage-introduction.md)</p>
...
Настроить сетевое подключение между локальным сайтом и виртуальной сетью Azure, в которой развернуты компоненты вашей инфраструктуры (сервер конфигураций и главные целевые серверы), можно одним из двух способов. Способ подключения к сети нужно выбрать до развертывания сервера конфигураций. Способ подключения выбирается в процессе развертывания и не может быть изменен.

**Общий доступ через Интернет**. Передача и репликация данных между локальными серверами (сервером обработки, защищенными серверами) и серверами компонентов инфраструктуры Azure (сервер конфигурации. главный целевой сервер) осуществляются по безопасному SSL/TLS-подключению из локальных в общедоступные конечные точки на сервере конфигурации и главном целевом сервере. (Единственным исключением является соединение между сервером обработки и главным целевым сервером через TCP-порт 9080, которое не шифруется. Это подключение используется только для передачи управляющей информации, связанной с протоколом репликации и используемой для настройки репликации.)

![Схема развертывания: Интернет](./media/site-recovery-vmware-to-azure-classic-legacy/internet-deployment.png)

**VPN**. Передача и репликация данных между локальными серверами (сервером обработки, защищенными серверами) и серверами компонентов инфраструктуры Azure (сервер конфигурации, главный целевой сервер) осуществляются по VPN-подключению между локальной сетью и виртуальной сетью Azure, в которой развернуты сервер конфигурации и главные целевые серверы. Для установки соединения между локальной сетью и виртуальной сетью Azure используйте подключение ExpressRoute или VPN-подключение типа «сеть — сеть».
...
2. Войдите в систему сервера, используя клиент SSH. Примечание. Если подключение к сети Azure выполняется через VPN, используйте внутренний IP-адрес. В противном случае используйте внешний IP-адрес и общедоступную конечную точку SSH.
	3. Извлеките файлы из пакета GZIP установщика, выполнив следующую команду: **tar –xvzf Microsoft-ASR\_UA\_8.4.0.0\_RHEL6-64***.![Главный целевой сервер Linux](./media/site-recovery-vmware-to-azure-classic-legacy/linux-tar.png)
