...
1. Нажмите кнопку **параметры** для просмотра параметров цели и репликации.
2. Чтобы переопределить параметры по умолчанию конечной, установите **Настройка** рядом с **группы ресурсов, сети, хранилища и группы доступности**.
...
- **Целевая группа ресурсов**. Группа ресурсов в целевом регионе, в которую входят виртуальные машины Azure после отработки отказа. По умолчанию Site Recovery создает группу ресурсов в целевом регионе с суффиксом asr.
...
- **Целевые учетные записи хранения**. По умолчанию Site Recovery создает учетную запись хранения в целевом регионе, чтобы отобразить учетную запись хранения исходной виртуальной машины.
...
Чтобы переопределить параметры политики репликации по умолчанию, нажмите кнопку **Настройка** рядом с **политику репликации**.  
...
- **Группа репликации**: Если приложению требуется согласованность нескольких виртуальных Машин в виртуальных машинах, вы можно создать группу репликации для тех виртуальных машин. По умолчанию выбранные виртуальные машины не входят ни в одну группу репликации.

  Нажмите кнопку **Настройка** рядом с **политику репликации** , а затем выберите **Да** для обеспечения согласованности нескольких виртуальных Машин, чтобы виртуальные машины частью группы репликации. Можно создать новую группу репликации или используйте существующую группу репликации. Выберите виртуальные машины, чтобы быть частью группы репликации и нажмите **ОК**.

> [!IMPORTANT]
  Все компьютеры в группу репликации будет доступа к точкам восстановления сбои, согласованное и целостность приложений при сбое. Включение согласованность нескольких виртуальных Машин может повлиять на производительность рабочей нагрузки и должен использоваться только в том случае, если машины работают под управлением той же рабочей нагрузке и требуется согласованность между несколькими компьютерами.

> [!IMPORTANT]
  Если включить согласованность между виртуальными машинами, компьютеры в группе репликации будут обмениваться данными друг с другом через порт 20004. Убедитесь, что нет устройства брандмауэр блокирует внутренний обмен данными между виртуальными машинами через порт 20004. Если вы хотите виртуальных машин Linux, должен входить в группу репликации, убедитесь, что исходящий трафик через порт 20004 вручную открыть согласно указаниям по конкретной версии Linux.
...
В этом учебнике вы настроили аварийного восстановления для виртуальной Машины Azure. Следующим шагом является проверка конфигурации.
