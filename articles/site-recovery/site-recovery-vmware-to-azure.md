...
#### Настройка VPN-подключения (необязательно) 

Если вы хотите использовать подключение VPN или ExpressRoute, выполните следующие действия.

1. Если в данный момент подключение не настроено, см. дополнительные сведения в следующих разделах:

	- [ExpressRoute или VPN. Что выбрать?](http://azure.microsoft.com/blog/2014/06/10/expressroute-or-virtual-network-vpn-whats-right-for-me/)
	- [Настройка подключения «сеть — сеть» к виртуальной машине Azure](../vpn-gateway-site-to-site-create.md)
	- [Настройка подключения ExpressRoute](../expressroute-configuring-exps.md)
2. Щелкните в хранилище **Серверы**, **Серверы конфигурации**, выберите сервер конфигурации и щелкните **Настроить**.
3. В окне **Параметры подключения** убедитесь, что для параметра **Тип подключения** установлено значение **VPN**. Обратите внимание, что, если настроено подключение VPN, а подключение к Интернету с локального сайта недоступно, нужно обязательно выбрать параметр VPN. В противном случаен сервер обработки не будет отправлять данные репликации на главный целевой сервер со своих открытых конечных точек.

	![Включение VPN](./media/site-recovery-vmware-to-azure/ASRVMWare_EnableVPN.png)
