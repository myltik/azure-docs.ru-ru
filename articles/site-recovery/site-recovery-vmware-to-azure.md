...
---
# <a name="replicate-vmware-virtual-machines-to-azure-with-azure-site-recovery"></a>Репликация виртуальных машин VMware в Azure с помощью службы Azure Site Recovery
...
В этой статье описано, как выполнить репликацию локальных виртуальных машин VMware в Azure с помощью службы [Azure Site Recovery](site-recovery-overview.md) на портале Azure.
 ...
 Если вы хотите только перенести виртуальные машины VMware с помощью простого перехода на другой ресурс без полной репликации (репликация, отработка отказа, восстановление размещения), ознакомьтесь с [этой статьей](site-recovery-migrate-to-azure.md).
...
**Локальный сервер конфигурации** | Тип адаптера виртуальной машины VMware должен быть VMXNET3. В обратном случае [установите это обновление](https://kb.vmware.com/selfservice/microsites/search.do?cmd=displayKC&docType=kc&externalId=2110245&sliceId=1&docTypeID=DT_KB_1_1&dialogID=26228401&stateId=1).<br/><br/> Должен быть установлен vSphere PowerCLI 6.0.<br/><br> Компьютер не должен быть контроллером домена или иметь статический IP-адрес.<br/><br/> Имя узла должно содержать 15 знаков или меньше, а операционная система должна быть на английском языке.
...
- **Подготовка учетной записи к автоматическому обнаружению.** Сервер обработки Site Recovery обнаруживает виртуальные машины автоматически. Для этого Site Recovery необходимы учетные данные, которые могут получать доступ к серверам vCenter или узлам vSphere ESXi.
...
4. [Создайте новую группу ресурсов](../azure-resource-manager/resource-group-template-deploy-portal.md) или выберите существующую. Укажите регион Azure. Сведения о поддерживаемых регионах см. в разделе "Географическая доступность" на странице [цен на службу Azure Site Recovery](https://azure.microsoft.com/pricing/details/site-recovery/).
...
3. На странице **Цель защиты** выберите **В Azure** и **Да, с использованием низкоуровневой оболочки VMware vSphere**.
...
1. В программе единой установки на странице **Перед началом работы** выберите **Install the configuration server and process server** (Установить сервер конфигурации и сервер обработки).

   ![Перед началом работы](./media/site-recovery-vmware-to-azure/combined-wiz1.png)
2. В окне **Third-Party Software License** (Лицензия на программное обеспечение стороннего поставщика) установите флажок **I Accept the third party license agreement** (Принимаю условия лицензионного соглашения стороннего поставщика), чтобы скачать и установить MySQL.

    ![Стороннее программное обеспечение](./media/site-recovery-vmware-to-azure/combined-wiz105.PNG)
3. На странице **Регистрация** найдите и выберите регистрационный ключ, загруженный из хранилища.

    ![Регистрация](./media/site-recovery-vmware-to-azure/combined-wiz3.png)
4. В окне **Internet Settings** (Параметры Интернета) укажите параметры для подключения поставщика, который будет выполняться на сервере конфигурации, к Azure Site Recovery через Интернет (443).

   * Чтобы поставщик подключался напрямую, выберите **Подключиться к Azure Site Recovery напрямую без прокси-сервера**.
   * Если вы хотите использовать прокси-сервер, выберите **Подключиться к Azure Site Recovery с использованием прокси-сервера** и укажите параметры.

     ![Брандмауэр](./media/site-recovery-vmware-to-azure/combined-wiz4.png)
5. На странице **Проверка предварительных требований** программа установки проверит настройки.  При появлении предупреждения о настройке времени убедитесь, что время в настройках **даты и времени** совпадает со временем соответствующего часового пояса.
...
9. На странице **Выбор сети** укажите прослушиватель (сетевой адаптер и порт SSL), через который сервер конфигурации будет отправлять и получать данные репликации. Можно изменить порт по умолчанию (9443). Помимо этого порта для управления репликацией используется порт 443. Не используйте порт 443 для трафика репликации.

    ![Выбор сети](./media/site-recovery-vmware-to-azure/combined-wiz9.png)


10. Просмотрите информацию на странице **Сводка** и нажмите кнопку **Установить**. После завершения установки создается парольная фраза. Она потребуется при включении репликации, поэтому скопируйте ее и сохраните в безопасном месте. После завершения регистрации сервер появится в колонке хранилища **Серверы**.
...
### <a name="add-the-account-for-automatic-discovery"></a>Добавление учетной записи, используемой для автоматического обнаружения
 ...
 Добавьте учетную запись, созданную для автоматического обнаружения виртуальных машин VMware.
...
3. В разделе **Сведения об учетной записи** добавьте учетную запись, которая будет использоваться для автоматического обнаружения. Имя учетной записи появится на портале через 15 (или больше) минут. Чтобы выполнить обновление немедленно, выберите **Серверы конфигурации** > имя_сервера > **Сервер обновления**.
...
- При добавлении сервера vCenter или узлов vSphere с помощью учетной записи без прав администратора на сервере ей должны быть назначены следующие привилегии:
    ...
    - для сервера vCenter должны быть активированы привелегии представлений хранилища.
...
2. Выберите **Подготовка инфраструктуры** > **Источник**. В колонке **Подготовка источника** выберите нужный сервер конфигурации. Щелкните **+ vCenter**, чтобы добавить сервер vCenter или узел vSphere.
3. В окне **Добавить сервер vCenter** укажите понятное имя сервера, а также IP-адрес или полное доменное имя. Если серверы VMware настроены на прослушивание запросов через другой порт, оставьте порт 443. Затем выберите учетную запись, созданную для автоматического обнаружения, и нажмите кнопку **ОК**.
...
1. В разделе **Вычисления и сеть** > **Свойства вычислений** можно указать имя и целевой размер виртуальной машины Azure. При необходимости измените имя в соответствии с [требованиями Azure](site-recovery-support-matrix-to-azure.md#failed-over-azure-vm-requirements) .
2. Измените сведения о целевой сети, подсети и целевом IP-адресе, который будет назначен виртуальной машине Azure.
...
1. На странице **Диски** отображаются сведения об операционной системе виртуальной машины и дисках данных, которые будут реплицироваться.

## <a name="run-a-test-failover"></a>Запуск тестовой отработки отказа

Когда настройка будет завершена, [запустите тестовую отработку отказа](site-recovery-test-failover-to-azure.md), чтобы убедиться в надлежащей работе всех компонентов.
...
- См. сведения в [этой статье](site-recovery-failback-azure-to-vmware.md), если вам нужно выполнить восстановление размещения и репликацию виртуальных машин Azure на локальном сайте.
