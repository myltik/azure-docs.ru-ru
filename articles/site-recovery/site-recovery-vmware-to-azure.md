...
Статья содержит общие сведения и предварительные требования к развертыванию, а также указания по настройке. В конце статьи ваши виртуальные машины VMware или физические серверы будут реплицироваться в Azure.
Если у вас возникают проблемы, задайте свои вопросы на [форуме по службам восстановления Azure](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr).
...
4. В окне **Сведения о новом сервере конфигурации** укажите следующее.
	...
	- В раскрывающемся списке типов подключения выберите общий доступ через Интернет или VPN.
	[AZURE.NOTE]Этот параметр выбирается в процессе развертывания и не может быть изменен.  
...
8. На сервере под управлением Linux
	...
	![Главный целевой сервер Linux](./media/site-recovery-vmware-to-azure/ASRVMWare_TSLinuxTar.png)
...
3. В списке **Серверы обработки** щелкните **Изменить сервер обработки** рядом с тем сервером, который требуется изменить.
	![Изменить сервер обработки 1](./media/site-recovery-vmware-to-azure/ASRVMware_ChangePS1.png)
4. В диалоговом окне **Изменение сервера обработки** выберите новый сервер в списке **Целевой сервер обработки**, а затем виртуальные машины, которые требуется реплицировать на новый сервер. Щелкните значок сведений рядом с именем сервера, чтобы получить информацию о нем, включая свободное пространство и объем использованной памяти. Чтобы помочь вам принять решение о загрузке, отображается средний объем свободного места, требуемый для репликации каждой выбранной виртуальной машины на новом сервере обработки.
	![Изменить сервер обработки 2](./media/site-recovery-vmware-to-azure/ASRVMware_ChangePS2.png)
