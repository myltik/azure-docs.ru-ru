---
title: "Проектирование сетевой инфраструктуры для аварийного восстановления с помощью Azure Site Recovery | Документация Майкрософт"
...
В этой статье рассматриваются требования к сети при использовании [Azure Site Recovery](site-recovery-overview.md) для аварийного восстановления из локальной среды в Azure или на дополнительный локальный сайт. Этот раздел посвящен определению диапазонов IP-адресов и подсетей после отработки отказа в дополнительное расположение.

## <a name="overview"></a>Обзор

Site Recovery — это служба Microsoft Azure, отвечающая за оркестрацию защиты и восстановления виртуальных приложений и обеспечивающая аварийное восстановление непрерывности бизнес-процессов (BCDR).

В мире с возможностью круглосуточного подключения важно обеспечивать непрерывную работу корпоративной инфраструктуры и приложений. Целью аварийного восстановления непрерывности бизнес-процессов (BCDR) является восстановление неисправных компонентов, которое позволяет организации быстро возобновить нормальную работу. Разработка стратегии аварийного восстановления, которая будет применяться в случае маловероятных событий, — чрезвычайно сложная задача. Это связано с тем, что очень сложно предсказать будущее, особенно маловероятные события, а также с высокой стоимостью адекватных мер защиты от масштабных катастроф.

Для планирования BCDR очень важно определить целевое время восстановления (RTO) и целевую точку восстановления (RPO) в рамках плана BCDR. Если в центре обработки данных возникнет сбой, можно быстро (низкое значение RTO) подключить к сети реплицированные виртуальные машины, размещенные в дополнительном центре обработки данных или в Microsoft Azure, с минимальными потерями данных (низкое значение RPO).

Отработка отказа становится возможной благодаря службе Site Recovery, которая с самого начала копирует указанные виртуальные машины из основного центра обработки данных в дополнительный центр обработки данных или в Azure (в зависимости от сценария) и периодически обновляет реплики. При планировании инфраструктуры следует рассматривать проект сети как потенциально уязвимое место, которое может помешать достичь целевых значений RTO и RPO, установленных в компании.  

Когда администраторы планируют развертывание решения для аварийного восстановления, они должны в первую очередь решить, как будет осуществляться доступ к виртуальной машине после отработки отказа. Site Recovery позволяет администратору выбрать сеть, к которой будет подключена виртуальная машина после отработки отказа. Если первичный сайт находится в Azure, или это локальный сайт под управлением сервера VMM, используется сетевое сопоставление. Дополнительные сведения см. в статьях [Сетевое сопоставление между двумя регионами Azure](site-recovery-network-mapping-azure-to-azure.md) и [Планирование сопоставления сети для репликации виртуальных машин Hyper-V с помощью Site Recovery](site-recovery-network-mapping.md).
...
При отсутствии "Сети Azure", как показано на рисунке выше, можно сделать следующее. Вы можете создать VPN-подключение типа "сеть — сеть" между основным сайтом и сетью восстановления после отработки отказа.  


#### <a name="fail-over-to-a-secondary-on-premises-site"></a>Выполнение отработки отказа на дополнительный локальный сайт
...
Если дополнительный сайт является локальным и для управления им используется сервер VMM, то при включении защиты для конкретной виртуальной машины Site Recovery выделит сетевые ресурсы в соответствии со следующим рабочим процессом:

* Site Recovery выделяет IP-адреса для каждого сетевого интерфейса на виртуальной машине из пула статических IP-адресов, определенного в соответствующей сети для каждого экземпляра System Center VMM.
* Если администратор определяет для сети на сайте восстановления тот же пул IP-адресов, который используется в сети основного сайта, и при этом выделяет IP-адрес реплике виртуальной машины, Site Recovery выделит тот же IP-адрес, что и у основной виртуальной машины.  IP-адрес резервируется в VMM, но он не задан как IP-адрес для отработки отказа на узле Hyper-V. IP-адрес отработки отказа задается на узле Hyper-V непосредственно перед отработкой отказа.


Если же IP-адрес недоступен, Site Recovery выделяет другие доступные адреса из определенного пула IP-адресов.
...
> [!NOTE]
> В сценарии, в котором виртуальные машины используют DHCP, управление IP-адресами не зависит от Site Recovery. Администратор должен убедиться, что DHCP-сервер, предоставляющий IP-адреса на сайте восстановления, может назначать адреса из того же диапазона, который используется на основном сайте.
>
>
...
* С помощью диспетчера трафика Azure, используемого с Site Recovery для интернет-приложений.
...
### <a name="changing-the-ip-addresses--disaster-recovery-to-azure"></a>Изменение IP-адресов. Аварийное восстановление в Azure
...
Узнайте о [сопоставлении сети](site-recovery-network-mapping.md).
