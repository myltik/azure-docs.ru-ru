...
>[AZURE.NOTE] На данный момент эта функция находится на этапе предварительной версии и доступна, только если управление серверами узла Hyper-V в основном центре обработки данных осуществляется в облаках VMM и настройка VMware управляется [Сервером конфигурации](site-recovery-vmware-to-azure.md#configuration-server-prerequisites). Сейчас эта возможность недоступна на новом портале Azure.
...
2. Выберите элемент **Configure SQL Settings** (Настройка параметров SQL) и в поле **Имя** укажите понятное имя для SQL Server.
3. В поле **SQL Server (FQDN)** укажите полное доменное имя исходного SQL Server, который нужно добавить. Если SQL Server установлен в отказоустойчивом кластере, укажите полное доменное имя кластера, а не какого-либо его узла.
...
Следующим шагом является создание плана восстановления с использованием виртуальных машин и групп доступности. Выберите тот же сервер VMM или сервер конфигурации, который использовали в качестве источника на шаге 1, а в качестве целевого объекта укажите Microsoft Azure.
...
### Защита компьютеров без сервера VMM или сервера конфигурации
