...
**Версия** | **Развертывание** | **Из локальной среды на дополнительный сайт** | **Из локальной среды в Azure** |
--- | --- | --- | --- | --- |
**Экземпляр отказоустойчивого кластера SQL Server Enterprise 2014 и 2012** | Отказоустойчивый кластер | Группы доступности AlwaysOn | Группы доступности AlwaysOn
**Группы доступности AlwaysOn SQL Server 2014 и 2012** | Группы доступности AlwaysOn | AlwaysOn | AlwaysOn
**Экземпляр отказоустойчивого кластера SQL Server Standard 2014 и 2012** | Отказоустойчивый кластер | Репликация Site Recovery с локальным зеркалом | Репликация Site Recovery с локальным зеркалом
**SQL Server Enterprise и Standard 2014 и 2012** | Автономный | Репликация Site Recovery | Репликация Site Recovery
**SQL Server Enterprise и Standard 2008 R2** | Экземпляр отказоустойчивого кластера |Репликация Site Recovery с локальным зеркалом |Репликация Site Recovery с локальным зеркалом |
**SQL Server Enterprise и Standard 2008 R2** | Автономный |Репликация Site Recovery | Репликация Site Recovery
**SQL Server Enterprise и Standard (любая версия)** |Экземпляр отказоустойчивого кластера — приложение DTC | Репликация Site Recovery |Не поддерживается
...
## <a name="integrate-with-sql-server-always-on-for-replication-to-azure-classic-portal-with-a-vmmconfiguration-server"></a>Интеграция с SQL Server AlwaysOn для репликации в Azure (классический портал с сервером VMM и сервером конфигурации)
...
## <a name="integrate-with-sql-server-always-on-for-replication-to-azure-azure-portalclassic-portal-with-no-vmmconfiguration-server"></a>Интеграция с SQL Server Always On для репликации в Azure (портал Azure или классический портал без сервера VMM и сервера конфигурации)
