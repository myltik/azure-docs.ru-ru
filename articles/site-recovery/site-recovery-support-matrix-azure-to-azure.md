...
- 64-разрядная версия Windows Server 2012 R2
- Windows Server 2012
...
Азия | Южная Индия, центральная Индия, Юго-Восточная Азия, Восточная Азия, Восточная Япония, Западная Япония
...
**Конфигурация** | **Поддержка** | **Примечания**
--- | --- | ---
Максимальный размер диска ОС | Максимальный размер диска ОС, поддерживаемый Azure| Дополнительные сведения см.в разделе [Диски, используемые виртуальными машинами](../storage/storage-about-disks-and-vhds-windows.md#disks-used-by-vms).
Максимальный размер данных на диске | Максимальный размер данных на диске, поддерживаемый Azure| Дополнительные сведения см.в разделе [Диски, используемые виртуальными машинами](../storage/storage-about-disks-and-vhds-windows.md#disks-used-by-vms).
...
Временный диск | Всегда исключается из репликации | Временный диск всегда исключается из репликации. Согласно руководству по Azure, постоянные данные не следует помещать на временный диск. Дополнительные сведения см. в разделе [Временный диск](../storage/storage-about-disks-and-vhds-windows.md#temporary-disk).
Частота изменения данных на диске | Максимально 6 Мбит/с на диск | Если средняя частота изменения данных на диске постоянно превышает 6 Мбит/с, репликация может занять некоторое время. Однако, если это случайные пакеты данных и частота изменения данных только на некоторое время превышает 6 Мбит/с, то репликация завершится вовремя. В этом случае вы можете увидеть немного отложенные точки восстановления.
...
Дисковые пространства | Не поддерживается |         
...
>[!IMPORTANT]
> Во избежание проблем с производительностью следуйте [рекомендациям по выбору хранилища](../storage/storage-scalability-targets.md#scalability-targets-for-virtual-machine-disks) для исходных виртуальных машин Azure. Если использовать параметры по умолчанию, Site Recovery создает необходимые учетные записи хранения на основе исходной конфигурации. Если вы настроили собственные параметры и выбрали их, убедитесь, что вы используете целевые показатели масштабируемости для дисков исходных виртуальных машин (../storage/storage-scalability-targets.md#scalability-targets-for-virtual-machine-disks).
