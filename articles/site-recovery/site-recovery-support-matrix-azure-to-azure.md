...
**Перемещение между группами ресурсов вычислений, хранения и сети** | Не поддерживается |При перемещении виртуальной машины (или связанных с ней компонентов, например, хранилища или сети) после включения репликации, необходимо отключить репликацию, а затем включить ее еще раз.
...
(Обновление реплицируемых компьютеров с SLES 11 SP3 до SLES 11 SP4 не поддерживается. Если реплицированной машины была обновлена с SLES 11SP3 для SLES 11 SP4, необходимо отключить репликацию и защитить компьютер снова после обновления.)

>[!NOTE]
>
> Ubuntu серверы с помощью проверки подлинности на основе пароля и имени входа, а с помощью пакета init облака для настройки облака виртуальных машин, могут иметь имя входа на основе пароля отключены после перехода на другой ресурс (в зависимости от конфигурации cloudinit.) Имя входа на основе пароля можно включить на виртуальной машине, сбросив пароль из меню параметров (в разделе поддержки + Устранение неполадок статьи) сбоем по виртуальной машины на портале Azure.
...
* Программное обеспечение многоканального: сопоставления устройства
...
Китай | Восток Китая, China North

>[!NOTE]
>
> Для Южной Бразилии области можно реплицировать и переход на один из центральных штатах юга США, Западная центральной части США, восток США, восток США 2, Запад США, Западная часть США 2 и North Central US областей и только после сбоя.
...
Учетные записи хранения общего назначения V2 (горячие, так и стильной уровень) | Нет  | Увеличение затрат на транзакции значительно по сравнению с общего назначения учетных записей хранения V1
