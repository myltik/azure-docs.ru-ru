...
6. Щелкните элемент **Создать хранилище**. ![Новое хранилище](./media/site-recovery-vmware-to-azure-classic/quick-start-create-vault.png)
...
1. Последовательно выберите вкладки **Серверы** и **Серверы конфигурации**, сервер конфигурации и щелкните **Добавить сервер vCenter**.
...
3. По завершении обнаружения сервер vCenter будет указан на вкладке **Серверы конфигурации**.
...
3. На странице **Укажите параметры репликации** настройте параметры репликации, которые будут использоваться для всех компьютеров в группе.
	...
	- **Пороговое значение RPO**. Задает RPO (целевую точку восстановления). При превышении значением непрерывной репликации защиты данных заданного порогового значения RPO будут создаваться оповещения.
...
- Последнее время обнаружения для виртуальных машин VMware можно узнать на вкладке **Серверы конфигурации** в поле **Последний контакт в** для сервера vCenter или узла ESXi.
...
1. Рекомендуем последовательно выбрать **Защищенные элементы** > **Группа защиты** > **Компьютеры** > Добавить компьютеры. 
...
- **С указанием сети Azure**. При этом типе отработки отказа проверяется правильность работы всей среды репликации и подключения виртуальных машин к указанной сети. 
...
2. На странице **Подтверждение тестовой отработки отказа** выберите **Нет**, чтобы указать, что для тестовой отработки отказа не нужно использовать сеть Azure. Или же выберите сеть, к которой подключатся тестируемые виртуальные машины после отработки отказа. Щелкните значок галочки, чтобы начать отработку отказа.
