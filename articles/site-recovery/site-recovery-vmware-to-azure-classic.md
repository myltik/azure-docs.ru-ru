...
1. На портале Azure в разделе **Создать виртуальную сеть** укажите имя сети. диапазон IP-адресов и имя подсети.
...
>[AZURE.WARNING] Если при проверке предварительных требований в отношении **глобальной синхронизации времени** появится предупреждение, убедитесь, что время на системных часах и время часового пояса совпадают. + + ![TimeSyncIssue](./media/site-recovery-vmware-to-azure-classic/time-sync-issue.png)
...
1. Последовательно щелкните вкладки **Серверы** и **Серверы конфигурации**, выберите сервер конфигурации и щелкните **Добавить сервер vCenter**.
...
5. На странице **Укажите учетные записи** выберите учетную запись, [настроенную](#install-the-mobility-service-with-push-installation) для использования при автоматической установке службы Mobility Service.
...
3. Ниже приведены параметры, которые можно изменить.
	...
	-  **Размер виртуальной машины Azure**. Количество сетевых адаптеров зависит от размера, указанного для целевой виртуальной машины. См. [дополнительные сведения](../virtual-machines/virtual-machines-size-specs.md#size-tables) о размерах и адаптерах. Обратите внимание на следующее:
...
2. На странице **Подтвердить тестовую отработку отказа** выберите **Нет**, чтобы указать, что для тестовой отработки отказа не нужно использовать сеть Azure. Или же выберите сеть, к которой подключатся тестируемые виртуальные машины после отработки отказа. Щелкните значок галочки, чтобы начать отработку отказа.
...
2. На странице **Перед началом работы** установите переключатель **Добавление дополнительных серверов обработки для масштабирования развертывания**.
