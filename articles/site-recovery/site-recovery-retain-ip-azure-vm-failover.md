...
- Нагрузка приложений распределяется по трем подсетям — 10.1.0.0/24, 10.1.1.0/24, 10.1.2.0/24 (**Subnet 1**, **Subnet 2** и **Subnet 3** соответственно).
...
- Узлы реплик, которые необходимы для AlwaysOn, контроллер домена и т. д. помещаются в виртуальную сеть с адресным пространством 20.1.0.0/16 внутри подсети Subnet 4 с адресом 20.1.0.0/24. Виртуальная сеть называется **виртуальной сетью Azure** и располагается в регионе Azure "Юго-Восточная Азия".
...
![Подключения из Azure в Azure перед отработкой отказа](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-before-failover.png)
...
![Подключения из Azure в Azure при отработке отказа всего региона](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-full-region-failover.png)
...
- Виртуальные машины приложения размещаются в регионе Azure "Восточная Азия", используя виртуальную сеть Azure с адресным пространством 10.1.0.0/16 для первого приложения и 15.1.0.0/16 для второго. Виртуальные сети называются **Recovery VNet1** и **Recovery VNet2** для первого и второго приложения соответственно.
...
![Подключение из Azure в Azure изолированного приложения перед отработкой отказа](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-isolated-application-before-failover.png)
...
![Подключение из Azure в Azure изолированного приложения после отработки отказа](./media/site-recovery-retain-ip-azure-vm-failover/azure-to-azure-connectivity-isolated-application-after-failover.png)
...
VPN-шлюзы используют общедоступные IP-адреса и шлюзы для установки соединений. Если вы не хотите использовать общедоступный IP-адрес или хотите избежать дополнительных прыжков, вы можете использовать пиринг глобальной виртуальной сети для пирингового подключения к виртуальным сетям в регионах Azure.

Эта функция в настоящее время находится на стадии общедоступной предварительной версии и будет поддерживаться в большем количестве регионов, что позволит напрямую подключаться между виртуальными машинами, минуя общедоступный Интернет или дополнительные прыжки.

Дополнительные сведения см. в разделе [Регистрация для предварительной версии пиринга между виртуальными сетями в разных регионах](../virtual-network/tutorial-connect-virtual-networks-portal.md#register) и на [странице цен на виртуальные сети](https://azure.microsoft.com/pricing/details/virtual-network/).
...
![Подключения между локальной средой и Azure перед отработкой отказа](./media/site-recovery-retain-ip-azure-vm-failover/on-premises-to-azure-connectivity-before-failover.png)
...
![Подключения между локальной средой и Azure после отработки отказа](./media/site-recovery-retain-ip-azure-vm-failover/on-premises-to-azure-connectivity-after-failover.png)
