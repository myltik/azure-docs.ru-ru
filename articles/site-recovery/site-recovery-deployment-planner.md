...
## <a name="how-to-update-the-deployment-planner"></a>Как обновить планировщик ресурсов?
[Скачайте](site-recovery-deployment-planner.md#download) последнюю версию планировщика ресурсов Azure Site Recovery. Скопируйте ZIP-файл на сервер, на котором вы хотите запустить планировщик. Извлеките ZIP-файл.
Если у вас уже есть предыдущая версия планировщика ресурсов и профилирование выполняется, вам не нужно останавливать его, однако только, если в новой версии не исправлены ошибки профилирования. Если выпуск содержит исправления в компоненте профилирования, то рекомендуется остановить профилирование с помощью старой версии и запустить его снова с помощью новой версии. Обратите внимание, что при запуске профилирования с использованием новой версии необходимо передать тот же путь к выходному каталогу, который будет использоваться средством для добавления данных профиля в существующие файлы. При создании отчетов будет использоваться полный набор данных профилирования. Если передать разные выходные каталоги, то создадутся новые файлы, а старые данные профилирования нельзя будет использовать при создании отчета.<br> Каждое обновление с использованием ZIP-файла является накопительным. Для его использования не нужно копировать файлы новой версии в папку со старой версией. Для этого можно использовать новую папку.


##<a name="version-history"></a>Журнал версий
### <a name="11"></a>1,1
Обновлено: 09.03.2017 <br>

Исправлены следующие ошибки<br>

* Не поддерживается профилирование виртуальных машин, если на сервере vCenter Server находится несколько виртуальных машин с одинаковым именем или IP-адресом, расположенных на разных узлах ESXi.<br>
* Для таблиц совместимых и несовместимых виртуальных машин были отключены функции копирования и поиска.
