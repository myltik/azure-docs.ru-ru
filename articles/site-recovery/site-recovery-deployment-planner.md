...
| -UseManagedDisks | (Необязательно) UseManagedDisks — "Да" или "Нет". Значение по умолчанию — "Да". Число виртуальных машин, которые можно разместить в одной учетной записи хранения, вычисляется в зависимости от того, выбран ли управляемый диск для отработки отказа или тестовой отработки отказа. |

При размещении в одной учетной записи хранения при вычислении учитывается то, где выполняется отработка отказа или тестовая отработка отказа виртуальных машин — на управляемом диске или на неуправляемом диске. |
