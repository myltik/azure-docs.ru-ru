...
3. Для этой политики используются следующие установки значений: пороговое значение RPO = 15 мин, хранение точки восстановления = 24 часа, периодичность моментальных снимков согласования приложений = 60 мин. ![](./media/site-recovery-failback-azure-to-vmware-new/failback-policy.png)
...
1.  Последовательно выберите пункты "Хранилище" > Manage Site Recovery Infrastructure (Управление инфраструктурой Site Recovery) > **Серверы конфигурации** под заголовком For VMware and Physical Machines (Для VMware и физических компьютеров), а затем выберите сервер конфигурации. Щелкните значок "+Сервер обработки".
...
6. Если вы выбрали сеть *Resource Manager* Azure, для развертывания сервера потребуются следующие входные данные:
...
7. В конечном итоге сервер обработки должен появиться на странице серверов конфигурации в разделе соответствующих серверов на вкладке "Серверы обработки". ![](./media/site-recovery-failback-azure-to-vmware-new/pslistingincs.png)
	...
	>[AZURE.NOTE] Сервер не будет отображаться в разделе **свойств виртуальной машины**. Он отображается только на вкладке **Серверы** на сервере управления, на котором он зарегистрирован. Сервер обработки отобразится примерно через 10–15 минут.
...
3. Завершите работу мастера так же, как и при [настройке сервера управления](site-recovery-vmware-to-azure-classic.md#step-5-install-the-management-server). На странице **Configuration Server Details** (Сведения о сервере конфигурации) укажите IP-адрес этого главного целевого сервера и парольную фразу для доступа к виртуальной машине.
...
1.	Установите операционную систему CentOS 6.6 с минимальным набором возможностей на виртуальной машине сервера управления. Оставьте DVD-диск с ISO-образом в дисководе и перезапустите систему. Пропустите тестирование носителя, выберите английский язык (США), щелкните **Basic Storage Devices** (Основные устройства хранилища), убедитесь, что на жестком диске не содержатся важные данные, нажмите кнопку **Да** и удалите все данные. Введите имя узла сервера управления и выберите сетевой адаптер сервера. В диалоговом окне **Editing System** (Изменение системы) выберите **Подключаться автоматически** и добавьте статический IP-адрес, параметры сети и DNS. Укажите часовой пояс и корневой пароль для доступа к серверу управления.
2.	При выборе типа разбиения на секции следует задать значение **Пользовательское разбиение**. Нажав кнопку **Далее**, выберите **Бесплатно** и щелкните "Создать". Создайте секции **/**, **/var/crash** и **/home partitions**, а в поле **FS Type:** (Тип файловой системы:) укажите **ext4**. Создайте секцию подкачки с типом файловой системы **swap**.
3.	При наличии существующих устройств появится предупреждающее сообщение. Щелкните **Форматировать**, чтобы отформатировать диск с заданными параметрами секции. Щелкните **Write change to disk** (Записать изменения на диск), чтобы применить изменения к секции.
4.	Последовательно выберите **Install boot loader** (Установить загрузчик) > **Далее**, чтобы установить загрузчик в корневую секцию.
...
1.	Последовательно щелкните "Хранилище" > "Реплицированные элементы", выберите виртуальную машину, на которой проводилась отработка отказа, щелкните ее правой кнопкой мыши и выберите пункт **Защитить повторно**. Это также можно сделать, щелкнув машину и выбрав повторную защиту с помощью кнопок управления.
2.	В колонке будет уже выбрано направление защиты "Azure в локальную среду".
3.  На страницах **Главный целевой сервер** и **Сервер обработки** выберите локальный главный целевой сервер и сервер обработки виртуальной машины Azure.
...
1.	Последовательно щелкните "Хранилище" > "Настройка" > "Реплицированные элементы", выберите виртуальные машины, для которых выполнено восстановление размещения, и щелкните **Защитить повторно**.
