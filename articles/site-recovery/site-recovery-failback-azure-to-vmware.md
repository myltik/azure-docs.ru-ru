...
1.  Последовательно выберите "Хранилище" > "Site Recovery Infrastructure" (Инфраструктура Site Recovery) (под заголовком "Управление") > **Серверы конфигурации** (под заголовком "For VMware and Physical Machines" (Для VMware и физических компьютеров)), а затем выберите сервер конфигурации. Щелкните элемент "+ Process server" (+ Сервер обработки), выделенный желтым цветом на снимке экрана ниже, приведенном для примера.
...
7. В итоге сервер обработки должен появиться на странице серверов конфигурации в разделе соответствующих серверов на вкладке "Серверы обработки". ![](./media/site-recovery-failback-azure-to-vmware-new/pslistingincs.png)
