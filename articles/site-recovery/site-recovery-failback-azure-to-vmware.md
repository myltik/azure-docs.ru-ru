...
1.  Запустите программу установки vContinuum. На экране приветствия нажмите кнопку «Далее», чтобы указать необходимые параметры ![](./media/site-recovery-failback-azure-to-vmware/image2.png).
2.  Укажите IP-адрес и порт сервера CX. Установите флажок «Использовать HTTPS». ![](./media/site-recovery-failback-azure-to-vmware/image3.png) а. Для обнаружения CX IP-адреса перейдите в развертывание CS в Azure и просмотрите его панель мониторинга. Общедоступный IP-адрес будет отображаться в поле общедоступного виртуального IP-адреса.![](./media/site-recovery-failback-azure-to-vmware/image4.png) б. Для обнаружения открытого порта CX перейдите на вкладку конечных точек на странице виртуальной машины и определите открытый порт конечных точек HTTPs. ![](./media/site-recovery-failback-azure-to-vmware/image5.png)
3.  Укажите парольную фразу сервера конфигурации. Вы должны были записать ее в процессе регистрации сервера конфигурации. Возможно, вы использовали эту парольную фразу во время развертывания главного целевого сервера и сервера обработки. Если вы не помните парольную фразу, перейдите на сервер конфигурации в Azure и найдите ее в папке C:\\Program Files (x86)\\InMage Systems\\private\\connection.passphrase. ![](./media/site-recovery-failback-azure-to-vmware/image6.png)
4.  Укажите расположение для установки сервера vContinuum и начните установку. ![](./media/site-recovery-failback-azure-to-vmware/image7.png)
5.  По завершении установки можно запустить сервер vContinuum, чтобы проверить, работает ли он. ![](./media/site-recovery-failback-azure-to-vmware/image8.png)
...
2.  Чтобы развернуть новый сервер, настройте указанные ниже параметры на сервере обработки. а. Присвойте серверу обработки имя. б. Введите имя пользователя для подключения к виртуальной машине с правами администратора. в. Введите пароль для входа в систему. г. Выберите сервер конфигурации, на котором необходимо зарегистрировать сервер обработки. Убедитесь, что выбран правильный сервер конфигурации. Это тот же сервер, с помощью которого вы защищали виртуальные машины и выполняли отработку отказа для них. д. Укажите сеть Azure, в которой необходимо развернуть сервер обработки. Обязательно выберите ту же сеть, в которой находится сервер конфигурации. е. Укажите уникальный IP-адрес из выбранной подсети. ж. Начните развертывание сервера обработки. ![](./media/site-recovery-failback-azure-to-vmware/image10.png)
3.  Будет запущено задание развертывания сервера обработки. ![](./media/site-recovery-failback-azure-to-vmware/image11.png)
...
6.  Далее укажите локальный главный целевой сервер, на который необходимо перенести виртуальные машины для их защиты.
...
7.  Далее укажите параметры репликации для каждой виртуальной машины.
...
8.  Затем можно настроить свойства, которые получит виртуальная машина после отработки отказа с выполнением переноса на локальный сайт. Вы можете настроить различные свойства, как показано ниже.
...
Свойство|Настройка
---|---
Конфигурация сети|Для каждой обнаруженной сетевой карты настройте IP-адрес восстановления размещения для виртуальной машины. Выберите необходимую сетевую карту и нажмите кнопку **Изменить**, чтобы указать сведения об IP-адресе.
