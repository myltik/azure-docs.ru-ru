...
---
# <a name="replicate-physical-machines-to-azure-with-site-recovery"></a>Репликация физических компьютеров в Azure с помощью Site Recovery
...
Если ваша цель — миграция физических компьютеров в Azure (только отработка отказа), изучите дополнительные сведения из [этой статьи](site-recovery-migrate-to-azure.md).
...
3. Подготовьте локальный компьютер для использования в качестве сервера конфигурации.
...
6. Разверните службу Mobility Service на виртуальных компьютерах, которые нужно реплицировать.
7. Включите репликацию для компьютеров.
...
**Служба Mobility Service** | Она будет установлена на каждом компьютере, который нужно реплицировать.
...
- Просмотрите краткий видеообзор (описывает репликацию виртуальных машин VMware, однако процесс аналогичен и для репликации физических компьютеров)
...
Прежде чем начать, просмотрите краткий видеообзор (описывает репликацию виртуальных машин VMware, однако процесс аналогичен и для репликации физических компьютеров).
...
1. Установить принудительно через сервер обработки. Чтобы использовать этот метод, нужно подготовить компьютеры заранее.
...
9. В поле **Физические компьютеры** щелкните "+", введите **имя** и **IP-адрес**, а затем выберите операционную систему компьютера, который нужно реплицировать.
  ![Включение репликации](./media/site-recovery-physical-to-azure/machineselect.png). Обнаружение компьютеров и отображение их в списке займет несколько минут.
