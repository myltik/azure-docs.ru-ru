...
Для работы с этим руководством необходимы виртуальные машины Azure в регионе Azure, из которого требуется выполнить перенос. Кроме того, перед началом работы следует проверить ряд настроек.
...
3. В разделе **Параметры** щелкните **Аварийное восстановление (предварительная версия)**.
...
>[!NOTE]
  >
  > Сейчас репликация виртуальных машин Azure с управляемыми дисками не поддерживается. 
...
5. В разделе **Реплицированные элементы** щелкните виртуальную машину правой кнопкой мыши и выберите **Завершение миграции**. На этом процесс переноса будет завершен, а репликация виртуальной машины остановлена.
