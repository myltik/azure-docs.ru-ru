...
1.  Включите защиту компьютеров с помощью Azure Site Recovery, выполнив указанные здесь действия.
...
3.  Используйте функцию плана восстановления Azure Site Recovery, чтобы добавить скрипты для [подключения общей папки Azure](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) и получения доступа к этой папке на виртуальной машине.
...
2. [Создание файлового ресурса](https://docs.microsoft.com/azure/storage/files/storage-how-to-create-file-share)
...
6.  В случае аварии в локальной среде выполните отработку отказа, используя план восстановления, и добавьте скрипт, чтобы [подключить общую папку Azure](https://docs.microsoft.com/azure/storage/files/storage-how-to-use-files-windows) и получить доступ к этой папке на виртуальной машине.
...
Указанные ниже действия детально описывают репликацию виртуальной машины VMware. Репликацию виртуальной машины Hyper-V см. здесь.
...
6.  В случае аварии в локальной среде выполните отработку отказа, используя план восстановления, и добавьте скрипт, чтобы подключить общую папку Azure и получить доступ к этой папке на виртуальной машине.
...
Дополнительные сведения о выполнении тестовой отработки отказа см. [здесь](site-recovery-failover.md).
