---
title: Репликация виртуальных машин Hyper-V в облаке VMM в дополнительный ЦОД с помощью службы Azure Site Recovery и сети SAN | Microsoft Docs
...
---
# Репликация виртуальных машин Hyper-V в облаке VMM в дополнительный ЦОД с помощью службы Azure Site Recovery и сети SAN
...
## В чем преимущества репликации с использованием сети SAN?
...
| **VMM** |Вам понадобится по крайней мере один сервер VMM, развернутый как отдельный физический или виртуальный сервер либо как виртуальный кластер. <br/><br/>Сервер VMM должен работать под управлением System Center 2012 R2 с последними накопительными пакетами обновления.<br/><br/>Требуется по крайней мере одно облако на основном сервере VMM, который требуется защитить, и одно облако на дополнительном сервере VMM, который вы хотите использовать для защиты и восстановления.<br/><br/>В исходном облаке, которое вы хотите защитить, должна быть как минимум одна группа узлов VMM.<br/><br/>Для каждого облака VMM необходимо настроить профиль возможностей Hyper-V.<br/><br/>Дополнительные сведения о настройке облаков VMM см. в статьях [Настройка структуры облака VMM](https://msdn.microsoft.com/library/azure/dn469075.aspx#BKMK_Fabric) и [Пошаговое руководство. Создание частных облаков с помощью System Center 2012 SP1 VMM](http://blogs.technet.com/b/keithmayer/archive/2013/04/18/walkthrough-creating-private-clouds-with-system-center-2012-sp1-virtual-machine-manager-build-your-private-cloud-in-a-month.aspx). |
| **Hyper-V** |Вам потребуется один или несколько кластеров Hyper-V в основном и дополнительном ЦОД и одна или несколько виртуальных машин в исходном кластере Hyper-V. В каждой группе узлов VMM в основном и дополнительном ЦОД должен быть как минимум один кластер Hyper-V.<br/><br/>Сервер узла и целевой сервер Hyper-V должны работать под управлением Windows Server 2012 (или более поздней версии этой ОС) с ролью Hyper-V, и на них должны быть установлены последние обновления.<br/><br/>Все серверы Hyper-V, содержащие виртуальные машины, которые необходимо защитить, должны быть расположены в облаке VMM.<br/><br/>Если Hyper-V работает в кластере, учтите, что брокер кластера не создается автоматически, если кластер имеет статический IP-адрес. Вам потребуется настроить брокер кластера вручную. См. [дополнительные сведения](https://www.petri.com/use-hyper-v-replica-broker-prepare-host-clusters) в записи блога Эйдена Финна (Aidan Finn). |
| **Хранилище SAN** |Репликация сети SAN дает возможность реплицировать размещенные в гостевых кластерах виртуальные машины, используя протокол iSCSI или Fibre Channel либо общие виртуальные жесткие диски (VHDX).<br/><br/>Вам потребуется настроить по одному массиву SAN в основном и дополнительном ЦОД.<br/><br/>Между массивами нужно настроить инфраструктуру сети. Выполните настройку пиринга и репликации. Лицензии на репликацию следует настроить в соответствии с требованиями к массиву хранения.<br/><br/>Между серверами узлов Hyper-V и массивом хранения нужно настроить рабочую сеть, которая обеспечит взаимодействие узлов с номерами логических устройств (LUN) хранилища с помощью ISCSI и Fibre Channel.<br/><br/> См. список [поддерживаемых массивов хранения](http://social.technet.microsoft.com/wiki/contents/articles/28317.deploying-azure-site-recovery-with-vmm-and-san-supported-storage-arrays.aspx).<br/><br/>Поставщики SMI-S, предоставленные производителями массивов хранения, должны быть установлены. Массивы SAN должны управляться этими поставщиками. Настройка поставщика выполняется в соответствии с прилагающейся к нему документацией.<br/><br/>Поставщик SMI-S для массива должен размещаться на сервере, к которому сервер VMM может подключиться по IP-адресу или полному доменному имени (FQDN).<br/><br/>Каждый массив SAN должен иметь как минимум один пул носителей, который можно задействовать в этом развертывании.<br/><br/>Сервер VMM в основном ЦОД должен управлять основным массивом, а сервер VMM в дополнительном ЦОД — дополнительным массивом. |
...
## Шаг 6. Включение репликации для групп репликации
