---
title: "Репликация виртуальных машин Hyper-V в облаке VMM в дополнительное местонахождение с помощью службы Azure Site Recovery и сети SAN | Документация Майкрософт"
...
---
# <a name="replicate-hyper-v-vms-in-a-vmm-cloud-to-a-secondary-site-with-azure-site-recovery-using-san"></a>Репликация виртуальных машин Hyper-V в облаке VMM в дополнительный ЦОД с помощью службы Azure Site Recovery и сети SAN
В этой статье вы узнаете, как развернуть Site Recovery для оркестрации и автоматизации репликации SAN и отработки отказа для виртуальных машин Hyper-V, расположенных в облаках System Center VMM, на дополнительный сайт VMM.

После прочтения этой статьи все комментарии или вопросы можно добавить в конце этой статьи или на [форуме по службам восстановления Azure](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr). 
...
Данная статья содержит инструкции по настройке репликации виртуальных машин Hyper-V с одного сайта VMM на другой с помощью репликации SAN. В этой статье можно найти основные сведения об архитектуре, предварительные требования для развертывания, а также указания по настройке. Вы обнаружите и классифицируете системы хранения данных SAN в VMM, проведете подготовку LUN и выделение хранилища для кластеров Hyper-V. В заключение приводится процедура тестирования отработки отказа для подтверждения правильной работы всех компонентов.

## <a name="why-replicate-with-san"></a>В чем преимущества репликации с использованием сети SAN?
Такой подход обеспечивает следующие возможности:

* Получение масштабируемого корпоративного решения для репликации, автоматическую работу которого обеспечивает служба Site Recovery.
* Использование возможностей репликации сети SAN, предоставляемых партнерами корпоративного хранения при помощи протоколов Fibre Сhannel и iSCSI. См. наших [партнеров хранилища SAN](http://social.technet.microsoft.com/wiki/contents/articles/28317.deploying-azure-site-recovery-with-vmm-and-san-supported-storage-arrays.aspx).
* Использование существующей инфраструктуры SAN для защиты критически важных приложений, развернутых в кластерах Hyper-V.
* Поддержка гостевых кластеров.
* Согласованная репликация на различных уровнях приложения. В зависимости от возможностей массива хранения синхронизированная репликация обеспечивает низкие показатели целевого времени восстановления (RTO) и целевого периода для восстановления (RPO), а асинхронная — высокую гибкость.  
* Интеграция с VMM позволяет управлять сетью SAN через консоль VMM и использовать SMI-S для обнаружения существующих хранилищ.  
...
| **VMM** |Вам понадобится по крайней мере один сервер VMM, развернутый как отдельный физический или виртуальный сервер либо как виртуальный кластер. <br/><br/>На сервере VMM должен выполняться System Center 2012 R2 с последними накопительными обновлениями.<br/><br/>Для каждого сервера должно быть настроено как минимум одно облако на основном сервере VMM и одно облако на дополнительном сервере VMM, который предполагается использовать для защиты и восстановления.<br/><br/>Исходное облако, для которого вы настраиваете защиту, должно содержать одну или несколько групп узлов VMM.<br/><br/>Для всех облаков VMM необходимо настроить профиль емкости Hyper-V.<br/><br/>Дополнительные сведения о настройке облаков VMM см. в разделах [Настройка структуры облака VMM](https://msdn.microsoft.com/library/azure/dn469075.aspx#BKMK_Fabric) и [Пошаговое руководство. Создание частных облаков с помощью VMM в System Center 2012 с пакетом обновления 1](http://blogs.technet.com/b/keithmayer/archive/2013/04/18/walkthrough-creating-private-clouds-with-system-center-2012-sp1-virtual-machine-manager-build-your-private-cloud-in-a-month.aspx). |
| **Hyper-V** |Вам потребуется один или несколько кластеров Hyper-V в основном и дополнительном ЦОД и одна или несколько виртуальных машин в исходном кластере Hyper-V. В каждой группе узлов VMM в основном и дополнительном ЦОД должен быть как минимум один кластер Hyper-V.<br/><br/>Сервер узла и целевой сервер Hyper-V должны работать под управлением Windows Server 2012 (или более поздней версии этой ОС) с ролью Hyper-V, и на них должны быть установлены последние обновления.<br/><br/>Все серверы Hyper-V, содержащие виртуальные машины, которые необходимо защитить, должны быть расположены в облаке VMM.<br/><br/>Если Hyper-V выполняется в кластере, учтите, что брокер кластера не создается автоматически при использовании кластера на основе статических IP-адресов. Вам потребуется настроить брокер кластера вручную. См. [дополнительные сведения](https://www.petri.com/use-hyper-v-replica-broker-prepare-host-clusters) в записи блога Эйдана Финна (Aidan Finn). |
| **Хранилище SAN** |Благодаря репликации SAN можно реплицировать размещенные на гостевых кластерах виртуальные машины при помощи протоколов iSCSI или Fibre Channel либо же при помощи общих виртуальных жестких дисков (VHDX).<br/><br/>Установите один массив SAN на первичном узле, а другой — на вторичном.<br/><br/>Настройте инфраструктуру сети между массивами. Выполните настройку пиринга и репликации. Лицензии репликации должны быть настроены в соответствии с требованиями массива хранения данных.<br/><br/>Создайте рабочую сеть между серверами узлов Hyper-V и массивом хранения, чтобы обеспечить взаимодействие узлов с LUN (логическим устройством) хранилища при помощи ISCSI и Fibre Channel.<br/><br/> См. список [поддерживаемых массивов хранения](http://social.technet.microsoft.com/wiki/contents/articles/28317.deploying-azure-site-recovery-with-vmm-and-san-supported-storage-arrays.aspx).<br/><br/>Должны быть установлены поставщики SMI-S, предоставленные производителями массивов хранения, а массивы SAN должны управляться поставщиком. Настройте поставщик в соответствии документацией.<br/><br/>Убедитесь в наличии на сервере поставщика SMI-S для массива данных и наличии сетевого доступа к серверу VMM через IP-адрес или полное доменное имя (FQDN).<br/><br/>Каждый массив SAN должен иметь один или несколько пулов носителей, которые можно задействовать в данном развертывании. Сервер VMM на первичном сайте должен управлять основным массивом, а вторичный сервер – дополнительным массивом.<br/><br/>Сервер VMM на первичном сайте должен управлять основным массивом данных, а вторичный сервер — дополнительным массивом. |
| **Сетевое сопоставление** |Вы можете настроить сетевое сопоставление, чтобы оптимально разместить реплики виртуальных машин на дополнительных серверах узла Hyper-V после отработки отказа и обеспечить их подключение к соответствующим сетям виртуальных машин. Если не настроить сетевое сопоставление, виртуальные машины реплик не будут подключены к каким-либо сетям после отработки отказа.<br/><br/>Для настройки сетевого сопоставления во время развертывания виртуальные машины на сервере исходного узла Hyper-V должны быть подключены к сети виртуальных машин VMM. Эта сеть должна быть подключена к логической сети, которая связана с облаком.<br/<br/>Для целевого облака на дополнительном сервере VMM, используемом для восстановления, необходимо настроить соответствующую сеть виртуальных машин, которая, в свою очередь, должна быть связана с соответствующей логической сетью, сопоставленной с целевым облаком.<br/><br/>[Узнайте больше](site-recovery-network-mapping.md) о сетевом сопоставлении. |
...
### <a name="ensure-vmm-clouds-are-set-up"></a>Проверьте, что облака VMM настроены
Site Recovery организует защиту виртуальных машин, расположенных на серверах узла Hyper-V в облаках VMM. Перед началом развертывания службы Site Recovery убедитесь, что эти облака настроены должным образом. Дополнительные сведения о [создании частных облаков](http://blogs.technet.com/b/keithmayer/archive/2013/04/18/walkthrough-creating-private-clouds-with-system-center-2012-sp1-virtual-machine-manager-build-your-private-cloud-in-a-month.aspx) см. в записи блога Кита Майера (Keith Mayer).

### <a name="integrate-and-classify-san-storage-in-vmm"></a>Интегрируйте и классифицируйте системы хранения данных SAN в VMM
Добавьте и классифицируйте протоколы SAN в консоли VMM:

1. В рабочей области **Структуры** щелкните **Хранилище**. Щелкните **Главная** > **Добавить ресурсы** > **Устройства хранения данных**, чтобы запустить мастер добавления устройств хранения.
2. На странице **Выберите тип поставщика хранилища** выберите **SAN and NAS devices discovered and managed by an SMI-S provider** (SAN и NAS-устройства, обнаруживаемые и управляемые поставщиком SMI-S).
...
1. После интеграции хранилища SAN в VMM создайте (подготовьте) логические устройства (LUN):
   ...
   * [Подготовка логических устройств хранилища в VMM](https://technet.microsoft.com/library/gg696973.aspx)
     ...
     > [!NOTE]
     > После включения репликации для виртуальной машины не следует добавлять виртуальные жесткие диски для него в LUN, которые не находятся в группе репликации Site Recovery. Если это сделать, то они не будут определяться Site Recovery.
     > 
     > 
2. Затем выделите достаточный объем памяти для кластера узлов Hyper-V, чтобы сервер VMM мог развернуть данные виртуальной машины в подготовленное хранилище:
   ...
   * Перед выделением хранилища кластеру необходимо выделить хранилище группе узлов VMM, на которой расположен соответствующий кластер. См. статьи [Выделение логических устройств хранения для группы узлов в VMM](https://technet.microsoft.com/library/gg610686.aspx) и [Выделение пулов носителей для группы узлов в VMM](https://technet.microsoft.com/library/gg610635.aspx).</a>.
   * Затем выделите емкость хранилища для кластера, как описано в статье [Настройка хранилища в кластере узлов Hyper-V в VMM](https://technet.microsoft.com/library/gg610692.aspx).</a>.
...
1. [Прочтите статью](site-recovery-network-mapping.md) о сетевом сопоставлении.
...
1. На странице **Службы восстановления** откройте страницу быстрого запуска. Страницу быстрого запуска можно открыть и в любой другой момент, щелкнув этот значок.
...
3. Извлеките установщик поставщика, выполнив в командной строке приведенные ниже команды. Для их выполнения нужны права **администратора**.
...
После регистрации серверов VMM можно настроить параметры защиты облачных систем. Параметр **Synchronize cloud data with the vault** (Синхронизировать облачные данные с хранилищем) был включен при установке поставщика, поэтому все облака на сервере VMM появятся на вкладке <b>Защищенные элементы</b> в хранилище.
...
4. В поле **Целевое расположение** выберите локальный сервер VMM, управляющий облаком, которое вы хотите использовать для восстановления.
...
Следить за выполнением действия "Включить защиту", в том числе за выполнением первоначальной репликации, можно на вкладке **Задания** . После запуска задачи финализации защиты виртуальная машина готова к отработке отказа.
...
Протестировав отработку отказа и убедившись, что среда работает так, как нужно, [ознакомьтесь](site-recovery-failover.md) с разными типами отработки отказа.
