...
[Служба Azure Site Recovery](site-recovery-overview.md) вносит свой вклад в работу надежного решения по обеспечению непрерывности бизнес-процессов и аварийного восстановления. Это решение защищает ваши локальные физические серверы и виртуальные машины, выполняя оркестрацию, автоматическую репликацию и отработку отказа с переносом в Azure или в дополнительный локальный центр обработки данных. В данной статье содержатся сведения и инструкции по восстановлению (отработке отказа и восстановлению размещения) виртуальных машин и физических серверов, защищенных с помощью Site Recovery.

Если после прочтения статьи у вас возникнут какие-либо вопросы, вы можете задать их на [форуме, посвященном Azure Recovery Services](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr).


## Обзор

После настройки защиты для виртуальных машин и физических серверов они начинают репликацию в дополнительное расположение. Если необходимо, то после репликации можно запустить операции отработки отказа. Site Recovery поддерживает несколько типов отработки отказа.

**Тип отработки отказа** | **Область применения** | **Дополнительные сведения** | **Описание процесса**
---|---|---|---
**Тестовая отработка отказа** | Проверка стратегии репликации или анализа работы системы аварийного восстановления. | <p>Не приводит к потерям данных и простоям.</p><p>Не оказывает влияния на репликацию.</p><p>Не оказывает влияния на рабочую среду.</p> | <p>Запустите операцию отработки отказа.</p><p>Укажите, как тестовые компьютеры должны быть подключены к сетям после отработки отказа.</p><p>Отслеживайте ход выполнения на вкладке **Задания**. Тестовые машины будут создаваться и запускаться в дополнительном расположении.</p><p>Azure: подключение к машине на портале Azure.</p><p>Дополнительный сайт: доступ к машине на одном и том же узле и в одном и том же облаке.</p><p>Полное тестирование и автоматическая очистка параметров тестовой отработки отказа.</p>
**Плановая отработка отказа** | <p>Обеспечение соответствия требованиям.</p><p>Плановое обслуживание.</p><p>Отработка отказа для обеспечения непрерывного выполнения рабочих нагрузок во время прогнозируемых неблагоприятных обстоятельств, например при плановых отключениях питания или экстремальных погодных условиях.</p> <p>Восстановление размещения после отработки отказа с переносом из основного расположения в дополнительное. | <p>Не приводит к потерям данных.</p><p>Время простоя определяется только временем завершения работы виртуальной машины в основном расположении и временем ее запуска в дополнительном расположении.</p><p>Виртуальные машины, рассматриваемые как целевые, после отработки отказа становятся исходными машинами.</p> | <p>Запустите операцию отработки отказа.</p><p>Отслеживайте ход выполнения на вкладке **Задания**. Будет завершена работа исходных машин.</p><p>Реплицированные машины будут запущены в дополнительном расположении.</p><p>Azure: подключение к реплицированной машине на портале Azure.</p><p>Дополнительное расположение: доступ к машине на том же узле и в том же облаке.</p><p>Фиксация отработки отказа.</p>
**Незапланированная отработка отказа** | <p>Такая обработка отказа необходима, когда основной сайт становится недоступным из-за неожиданного инцидента, например из-за сбоя питания или атаки вирусов.</p><p>Вы можете выполнять незапланированную отработку отказа, даже если основной сайт недоступен.<p> | <p>Количество потерянных данных зависит от того, как часто выполняется репликация.</p> <p>Данные будут актуальны на момент последней синхронизации.</p> | <p>Запустите операцию отработки отказа.</p><p>Отслеживайте ход выполнения на вкладке **Задания**. При необходимости можно завершить работу виртуальных машин и синхронизировать последнюю версию данных.</p><p>Реплицированные машины будут запущены в дополнительном расположении.</p><p>Azure: подключение к реплицированной машине на портале Azure.</p><p>Дополнительный сайт получает доступ к машине на том же узле и в том же облаке.</p><p>Фиксация отработки отказа.</p>
...
С сайта VMware в Azure | Не поддерживается |В этом сценарии используется непрерывная репликация, поэтому нет отличий между плановой и незапланированной отработкой отказа. Выберите пункт **Отработка отказа**. | Нет данных
...
---|---|---|---
**Отработка отказа с выполнением переноса в Azure без использования сети** | Не нужно выбирать целевую сеть Azure. | При отработке отказа выполняется проверка, правильно ли тестовая виртуальная машина запускается в Azure. | <p>Все тестовые виртуальные машины, входящие в план восстановления, будут добавлены в одну облачную службу и смогут подключаться друг к другу.</p><p>После отработки отказа машины не будут подключены к сети Azure.</p><p>Пользователи смогут подключаться к тестовым машинам с помощью общедоступного IP-адреса.</p>
**Отработка отказа с выполнением переноса в Azure с использованием сети** | Необходимо выбрать целевую сеть Azure. | При отработке отказа выполняется проверка, подключены ли тестовые виртуальные машины к сети. | <p>Создайте сеть Azure, изолированную от вашей рабочей сети Azure, и настройте инфраструктуру так, чтобы реплицированная виртуальная машина могла правильно работать.</p><p>В основе подсети тестовой виртуальной машины лежит подсеть, к которой виртуальная машина, для которой выполнена отработка отказа, должна подключаться при плановой или незапланированной отработке отказа.</p>
**Отработка отказа с выполнением переноса на вторичный сайт VMM без использования сети** | Не нужно выбирать сеть виртуальных машин. | При отработке отказа выполняется проверка того, созданы ли тестовые машины. <p>Тестовая виртуальная машина будет создана на том же узле, на котором имеется реплицированная виртуальная машина. Она не будет добавлена в облако, в котором расположена реплицированная виртуальная машина.</p></p>| <p>Машина, для которой выполнена отработка отказа, не будет подключена ни к какой сети.</p><p>После создания машины ее можно подключить к сети виртуальных машин.</p>
**Отработка отказа с выполнением переноса на вторичный сайт VMM с использованием сети** | Необходимо выбрать существующую сеть виртуальных машин. | При отработке отказа выполняется проверка того, созданы ли виртуальные машины. | <p>Тестовая виртуальная машина будет создана на том же узле, на котором имеется реплицированная виртуальная машина. Она не будет добавлена в облако, в котором расположена реплицированная виртуальная машина.</p><p>Создайте сеть виртуальных машин, которая будет изолирована от вашей рабочей сети.</p><p>Если вы используете сеть на основе виртуальной локальной сети, то мы рекомендуем вам создать в VMM отдельную логическую сеть (которая не будет использоваться в рабочих целях). Эта логическая сеть используется для создания сетей виртуальных машин, предназначенных для выполнения тестовых отработок отказа.</p><p>Логическая сеть должна быть сопоставлена хотя бы с одним из сетевых адаптеров серверов Hyper-V, в которых размещаются виртуальные машины.</p><p>В логических виртуальных локальных сетях сайты сети, добавляемые к логической сети, должны быть изолированы.</p><p>Если вы используете логическую сеть на основе технологии виртуализации сети Windows, Azure Site Recovery автоматически создаст изолированные сети виртуальных машин.</p>
**Отработка отказа с выполнением переноса на вторичный сайт VMM с созданием сети** | На основе параметра, заданного вами для **логической сети**, и связанных с ним сайтов сети будет автоматически создана временная тестовая сеть. | При отработке отказа выполняется проверка того, созданы ли виртуальные машины. | <p>Используйте этот вариант, если в плане восстановления используется несколько сетей виртуальных машин. При выборе этого варианта для сетей на базе технологии виртуализации сети Windows можно автоматически создавать сети виртуальных машин с теми же параметрами (подсети и пулы IP-адресов), что и параметры сети реплицированной виртуальной машины. Такие сети виртуальных машин автоматически очищаются после завершения тестовой отработки отказа.</p><p>Тестовая виртуальная машина будет создана на том же узле, на котором имеется реплицированная виртуальная машина. Она не будет добавлена в облако, в котором расположена реплицированная виртуальная машина.</p>
...
Запустите пример тестовой отработки отказа указанным ниже образом.

1. Выполните тестовую отработку отказа для виртуальной машины Active Directory и виртуальной машины DNS в той же сети, которую вы будете использовать для тестовой отработки отказа для локальной виртуальной машины.
2. Запишите IP-адреса, выделенные машинам, для которых выполнена отработка отказа.
3. В виртуальной сети Azure, которая будет использоваться для тестовой отработки отказа, укажите эти IP-адреса в качестве адресов серверов DNS и Active Directory.
4. Запустите тестовую отработку отказа для виртуальной машины, указав сеть Azure.
5. После проверки правильности тестовой отработки отказа выполните ее для виртуальных машин, а затем — для виртуальных машин Active Directory и DNS.

### Запуск тестовой отработки отказа для перехода с основного локального сайта на дополнительный

Для запуска тестовой отработки отказа вам потребуется выполнить несколько действий, в том числе создать копию Active Directory, а затем поместить тестовые серверы DHCP и DNS в тестовую среду. Это можно сделать несколькими способами.
...
Чтобы выполнить тестовую отработку отказа для тестирования приложения, потребуется копия рабочей среды Active Directory в тестовой среде. Ниже указано, что необходимо сделать.

1. **Создание копии**. Создайте копию Active Directory, используя один из указанных ниже методов.

	- Репликация Hyper-V. Вы можете запустить репликацию Active Directory, используя репликацию Hyper-V, точно так же, как и для других виртуальных машин. Кроме того, при выполнении тестовой отработки отказа в рамках плана восстановления можно выполнить тестовую отработку отказа для виртуальной машины Active Directory.
	- Репликация Active Directory. Для создания копии установки Active Directory на сайте реплик можно использовать репликацию Active Directory. При выполнении тестовой отработки отказа в рамках плана восстановления можно создать копию виртуальной машины Active Directory, сделав снимок реплицированной установки Active Directory. Эту копию можно использовать для тестовой отработки отказа. После завершения тестовой отработки отказа можно удалить копию Active Directory.

2. **Импорт и экспорт**. Вы можете создать копию виртуальной машины Active Directory, экспортировав, а затем импортировав ее с новым GUID.
3. **Добавление к сети**. Добавьте Active Directory в сеть, созданную при тестовой отработке отказа. Обратите внимание на следующее: 

	- Обязательно убедитесь, что сеть, в которую вы собираетесь добавить Active Directory, полностью изолирована от рабочей сети. При использовании сети типа Windows Network в качестве тестовой система гарантирует изоляцию автоматически создаваемых сетей виртуальных машин при условии, что не вы не добавляли внешний шлюз в сеть. Если вы применяете изоляцию на основе виртуальной локальной сети, обязательно убедитесь, что созданные сети виртуальных машин изолированы от рабочей среды.
	- Последовательность действий, которую, возможно, придется выполнить, будет немного другой. Это зависит от того, работают ли Active Directory и DNS на одной и той же или на разных виртуальных машинах.
		- Одна и та же виртуальная машина. Если Active Directory и DNS работают на одной и той же виртуальной машине, то ее можно использовать в качестве ресурса DNS для тестовой отработки отказа. Можно очистить все записи в DNS и заново создать необходимые зоны в DNS. 
		- Разные виртуальные машины. Если Active Directory и DNS находятся на разных виртуальных машинах, то для тестовой отработки отказа вам потребуется создать ресурс DNS. Можно использовать новый DNS-сервер и создать все необходимые зоны. Например, если используется домен Active Directory contoso.com, можно создать зону с именем contoso.com. 

4. **Обновление Active Directory в DNS**. В обоих случаях в DNS необходимо обновить записи, соответствующие Active Directory. Это можно сделать указанным ниже образом.
	...
	- До включения любой другой виртуальной машины в план восстановления убедитесь, что настроены указанные ниже параметры.
		...
		- Если Active Directory и DNS выполняются на разных виртуальных машинах, сопоставитель виртуальной машины Active Directory должен указывать на IP-адрес DNS-сервера виртуальной машины.
	- В Active Directory выполните следующую команду: nltest /dsregdns.
...
- **DHCP**. Если виртуальные машины используют DHCP, необходимо обновить IP-адрес тестового DNS-сервера на тестовом DHCP-сервере. Если используется сеть с технологией виртуализации сети Windows, VMM-сервер будет выступать в роли DHCP-сервера. Соответственно, необходимо обновить IP-адрес DNS-сервера в пуле статических IP-адресов, используемом для тестовой отработки отказа. В этом случае виртуальные машины будут регистрироваться на соответствующем DNS-сервере.
- **Статический адрес**. Если виртуальные машины используют статические IP-адреса, следует обновить IP-адрес тестового DNS-сервера в пуле статических IP-адресов, используемом для тестовой отработки отказа. Вам потребуется обновить DNS с помощью IP-адреса тестовых виртуальных машин. Для этого можно использовать указанный ниже пример сценария. 
...
- **Добавление зоны**. С помощью указанного ниже сценария добавьте зону на DNS-сервер, разрешите небезопасные обновления и добавьте запись для нее в DNS.
...
Обратите внимание, что если тестовая отработка отказа продолжается более двух недель, она будет завершена принудительно. При этом все элементы или виртуальные машины, автоматически созданные в процессе тестовой отработки отказа, будут удалены.
...
3. На странице **Подтверждение плановой отработки отказа** выберите исходное и целевое расположения. Запомните направление отработки отказа. 
...
3. На странице **Подтверждение незапланированной отработки отказа** выберите исходное и целевое расположения. Запомните направление отработки отказа. 
	- Если предыдущие отработки отказа выполнены правильно и все серверы виртуальных машин размещены в исходном или целевом расположении, то сведения о направлении отработки отказа понадобятся исключительно в информационных целях. 
	- Если виртуальные машины активны как в исходном, так и в целевом расположениях, появится кнопка **Изменить направление**. С помощью этой кнопки можно изменить и указать направление, в котором должна выполняться отработка отказа.
...
2. На странице **Подтверждение плановой отработки отказа** выберите исходное и целевое расположения. Запомните направление отработки отказа. Если отработка отказа с переносом из основного расположения выполнена правильно и все виртуальные машины находятся в дополнительном расположении, то эти данные будут носить исключительно информационный характер. 3. Если вы восстанавливаете размещение, выполняя перенос из Azure, выберите необходимые параметры в разделе **Синхронизация данных**.
