...
С сайта VMware в Azure | Поддерживается (расширенный сценарий) <br/><br/> Не поддерживается (устаревший сценарий) |В этом сценарии используется непрерывная репликация, поэтому нет отличий между плановой и незапланированной отработкой отказа. Выберите пункт **Отработка отказа**. | Нет данных
