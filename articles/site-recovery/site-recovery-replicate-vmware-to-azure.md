...
# <a name="replicate-applications"></a>Репликация приложений
...
1.  [Настроили локальную среду источника](site-recovery-set-up-vmware-to-azure.md).
...
> [!NOTE]
> По умолчанию реплицируются все диски на виртуальной машине. [Диски можно исключить из репликации](site-recovery-exclude-disk.md). Например, репликация может не требоваться для дисков с временными данными или данными, которые обновляются при каждом перезапуске компьютера или приложения (например, pagefile.sys или базы данных tempdb SQL Server).
>
