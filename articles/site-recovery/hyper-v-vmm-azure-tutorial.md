...
description: "Сведения о том, как настраивать аварийное восстановление в Azure для локальных виртуальных машин Hyper-V в облаках System Center VMM с помощью службы Azure Site Recovery."
...
В этом руководстве показано, как настроить аварийное восстановление локальных виртуальных машин Hyper-V в Azure. Оно предназначено для виртуальных машин Hyper-V, управление которыми осуществляется в System Center Virtual Machine Manager (VMM). Из этого руководства вы узнаете, как выполнять такие задачи:
...
1. В разделе **All Services** (Все службы)  > **Хранилища служб восстановления** щелкните имя хранилища, которое мы используем в данных руководствах, — **ContosoVMVault**.
...
5. В разделе **Are your machines virtualized** (Ваши компьютеры виртуализированы?) выберите **Yes, with Hyper-V** (Да, с помощью Hyper-V).
6. В разделе **Are you using System Center VMM** (Используете ли вы System Center VMM?) выберите **Yes** (Да). Нажмите кнопку **ОК**.
...
3. Загрузите программу установки поставщика Microsoft Azure Site Recovery.
...
2. На странице **Installation** (Установка) примите расположение установки по умолчанию для поставщика и нажмите кнопку **Install** (Установить). 
3. После установки в мастере регистрации Microsoft Azure Site Recovery в разделе **Параметры хранилища** щелкните **Browse** (Обзор), а в поле **Key file** (Файл ключа) выберите скачанный файл ключа.
4. Укажите подписку Azure Site Recovery и имя хранилища (**ContosoVMVault**). Укажите понятное имя для сервера VMM, описывающее его в хранилище.
...
7. В поле **Синхронизация метаданных в облаке**выберите **Sync cloud meta data to Site Recovery portal** (Синхронизировать метаданные на портале Site Recovery). На каждом сервере это действие требуется выполнять только один раз. Щелкните **Зарегистрировать**.
...
1. В окне мастера настройки агента служб восстановления Microsoft Azure **Проверка предварительных требований** щелкните **Next** (Далее). Все отсутствующие предварительные требования будут установлены автоматически.
...
3. Оставьте параметры по умолчанию и нажмите кнопку **ОК**.
    ...
    - В поле **Encrypt data stored on Azure** (Шифровать данные, хранящиеся в Azure) настройка по умолчанию **Off** (Отключено) указывает, что статические данные в Azure не шифруются.
...
3. В разделе **Цель** убедитесь, что в качестве целевого объекта выбрано Azure, а также проверьте подписку хранилища и выберите модель **Resource Manager**.
