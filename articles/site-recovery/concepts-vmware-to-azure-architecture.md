...
1.  Следует подготовить ресурсы Azure и локальные компоненты.
2.  В хранилище служб восстановления следует указать параметры источника репликации. В рамках этого процесса настраивается локальный сервер конфигурации. Чтобы развернуть этот сервер в качестве виртуальной машины VMware, скачайте готовый шаблон в формате OVF и импортируйте его в VMware для создания виртуальной машины.
3. Следует указать параметры цели репликации, создать политику репликации и включить репликацию для виртуальных машин VMware.
4.  Компьютеры выполняют репликацию в соответствии с политикой репликации, а репликация начальной копии данных виртуальной машины выполняется в службу хранилища Azure.
