...
В предыдущем примере сбой настройки защиты, вероятно, вызван другой выполняемой операцией. Устраните проблему, как указано в рекомендациях, и нажмите кнопку **RESTART** (Перезагрузить), чтобы снова запустить операцию.
