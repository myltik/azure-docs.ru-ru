---
title: Мониторинг и устранение неполадок защиты виртуальных машин и физических серверов | Microsoft Docs
...
## <a name="understanding-the-components"></a>Общие сведения о компонентах
### <a name="vmware/physical-site-deployment-for-replication-between-on-premises-and-azure."></a>Развертывание сайта Hyper-V или физического сайта для репликации между локальными средами и Azure.
...
### <a name="vmm-site-deployment-for-replication-between-on-premises-site."></a>Развертывание сайта VMM для репликации между локальными средами
...
### <a name="vmm-site-deployment-for-replication-between-on-premises-&-azure."></a>Развертывание сайта VMM для репликации между локальными средами и Azure
В ходе настройки аварийного восстановления между локальными средами и Azure необходимо загрузить поставщика Azure Site Recovery и установить на сервере VMM вместе с агентом служб восстановления Azure, который должен быть установлен на каждом узле Hyper-V. Дополнительные сведения см. в статье [Общие сведения о защите между сайтом и Azure](site-recovery-understanding-site-to-azure-protection.md).
...
### <a name="hyper-v-site-deployment-for-replication-between-on-premises-&-azure"></a>Развертывание сайта Hyper-V для репликации между локальными средами и Azure
Это то же, что и развертывание VMM, только на самом узле Hyper-V устанавливается другой поставщик и агент. Дополнительные сведения см. в статье [Общие сведения о защите между сайтом и Azure](site-recovery-understanding-site-to-azure-protection.md).
...
## <a name="monitor-replication-health-for-virtual-machine"></a>Мониторинг работоспособности репликации для виртуальной машины
...
Если репликация виртуальной машины приостановлена, нажмите правую кнопку мыши и последовательно выберите *Репликация*->*Возобновить репликацию*
![Устранение неполадок локальной среды Hyper-V](media/site-recovery-monitoring-and-troubleshooting/image19.png)
...
## <a name="reaching-out-for-microsoft-support"></a>Служба технической поддержки Майкрософт
### <a name="log-collection"></a>Сбор журналов
Для защиты сайтов VMM: для получения дополнительных сведений см. статью [Сбор журналов ASR с помощью средства Support Diagnostics Platform (SDP)](http://social.technet.microsoft.com/wiki/contents/articles/28198.asr-data-collection-and-analysis-using-the-vmm-support-diagnostics-platform-sdp-tool.aspx). Она содержит сведения для сбора необходимых журналов.

Для защиты сайтов Hyper-V: скачайте [средство](https://dcupload.microsoft.com/tools/win7files/DIAG_ASRHyperV_global.DiagCab) и запустите его на узле Hyper-V для сбора журналов.

Для защиты VMware или физических компьютеров: для получения дополнительных сведений см. статью [Сбор журналов Azure Site Recovery для защиты VMware и физических компьютеров](http://social.technet.microsoft.com/wiki/contents/articles/30677.azure-site-recovery-log-collection-for-vmware-and-physical-site-protection.aspx). Она содержит сведения для сбора необходимых журналов.
...
## <a name="common-asr-errors-and-their-resolutions"></a>Распространенные ошибки ASR и способы их устранения
...
Если кнопка **Подключить** на портале неактивна и вы не используете канал Express Route или VPN-подключение типа "сеть — сеть" для подключения к Azure, прежде всего следует создать общедоступный IP-адрес и назначить его виртуальной машине. Только после этого вы сможете использовать протокол RDP/SSH. Для добавления общедоступного IP-адреса в сетевой интерфейс виртуальной машины выполните следующие действия.  
