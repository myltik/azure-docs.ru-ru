...
6\. После завершения регистрации Azure Site Recovery извлекает метаданные с сервера Hyper-V Server, а сам сервер отображается в разделе **Параметры** > **Site Recovery Infrastructure** (Инфраструктура Site Recovery) > колонка **Hyper-V Hosts** (Узлы Hyper-V).
