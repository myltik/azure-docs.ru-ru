...
7. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик подключается к Интернету. Щелкните **Connect with existing proxy settings** (Подключиться с существующими настройками прокси-сервера), чтобы использовать настройки подключения к Интернету, заданные на сервере по умолчанию.

	![Параметры Интернета](./media/site-recovery-vmm-to-vmm-classic/proxy-details.png)
...
9. В поле **Имя хранилища** проверьте имя хранилища, в котором будет зарегистрирован сервер. Нажмите кнопку *Далее*.
	...
	![Регистрация сервера](./media/site-recovery-vmm-to-vmm-classic/vault-creds.png)
...
10.  Параметр шифрования используется только при репликации виртуальных машин Hyper-V в облаках VMM в Azure. При репликации на дополнительный сайт он не используется.
	...
	![Регистрация сервера](./media/site-recovery-vmm-to-vmm-classic/encrypt.png)
