...
2. Из раскрывающегося списка выберите **Между двумя локальными сайтами VMWare**.
...
7. На странице **Подключение к Интернету** укажите, как запущенный на сервере VMM поставщик подключается к Интернету. Выберите **Connect with existing proxy settings** (Подключиться с существующими настройками прокси-сервера), чтобы использовать настройки подключения к Интернету, заданные на сервере по умолчанию.
