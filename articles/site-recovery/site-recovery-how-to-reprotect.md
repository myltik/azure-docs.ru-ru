...
* Если на локальной виртуальной машине есть моментальные снимки, повторное включение защиты завершится сбоем. Их можно удалить, прежде чем продолжить повторное включение защиты.
...
* Если виртуальная машина не размещена локально на сервере vCenter Server, то во время повторного включения защиты потребуется создать виртуальную машину. Она будет создана на узле ESX, на котором создан главный целевой сервер. Выбирайте узел ESX внимательно, чтобы виртуальная машина для восстановления размещения была создана на нужном узле.
