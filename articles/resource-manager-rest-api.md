...
[REST API диспетчера ресурсов Azure описаны здесь](https://msdn.microsoft.com/library/azure/dn790568.aspx). В этом учебнике не рассматривается использование каждого из них. Мы обратимся только к нескольким API-интерфейсам, чтобы объяснить основные принципы использования API. После этого вы сможете ознакомиться с официальной документацией.
...
... и в результате вы получите список подписок, к которым может получать доступ эта субъект-служба.

(Идентификаторы подписок ниже были сокращены для удобства чтения.)
...
(Замените YOUR\_ACCESS\_TOKEN, SUBSCRIPTION\_ID, RESOURCE\_GROUP\_NAME фактическим маркером доступа, идентификатором подписки и именем группы ресурсов.)
