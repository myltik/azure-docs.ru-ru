<properties
	pageTitle="Учебник Apache Storm: попробуйте примеры Storm Starter | Microsoft Azure"
	description="Начало работы с анализом больших объемов данных с использованием примеров Storm Starter в HDInsight. Информация об использовании Apache Storm для обработки данных в режиме реального времени."
	services="hdinsight"
	documentationCenter=""
	authors="Blackmist"
	manager="paulettm"
...
# Учебник Apache Storm в HDInsight: начало работы с анализом больших объемов данных с использованием примеров Storm Starter 
...
2. Щелкните **СОЗДАТЬ** в левом нижнем углу, выберите **СЛУЖБЫ ДАННЫХ**, **ХРАНИЛИЩЕ**, а затем щелкните **БЫСТРО СОЗДАТЬ**.
	...
	![Портал Azure, на котором можно использовать функцию быстрого создания для настройки новой учетной записи хранения.](./media/hdinsight-storm-getting-started/HDI.StorageAccount.QuickCreate.png)
...
Дополнительную информацию о создании новой учетной записи хранения см. в статье <a href="../storage-create-storage-account/" target="_blank">Создание учетной записи хранения</a>.
...
3. Щелкните во втором столбце значок HDInsight и выберите **STORM**.
	...
	![Быстро создать](./media/hdinsight-storm-getting-started/quickcreate.png)
...
![Портал Azure со ссылкой на панель мониторинга Storm](./media/hdinsight-storm-getting-started/dashboard-link.png)
...
![Панель мониторинга Storm с формой отправки топологии](./media/hdinsight-storm-getting-started/submit.png)
...
1. Выберите **StormStarter - WordCount** (StormStarter — Подсчет слов) из раскрывающегося списка **Jar File** (JAR-файл). После этого действия поля **Class Name** (Имя класса) и **Additional Parameter** (Дополнительные параметры) заполнятся соответствующими параметрами для этого образца.
	...
	![Панель мониторинга Storm с выбранной топологией подсчета слов](./media/hdinsight-storm-getting-started/submit.png)
...
2. Нажмите кнопку **Submit** (Отправить). Через некоторое время в поле **Result** (Результат) отобразится команда, используемая для отправки задания, а также результаты выполнения команды. В поле **Error** (Ошибка) будут отображаться все ошибки, возникающие при отправке топологии.
	...
	![Кнопка отправки и результаты](./media/hdinsight-storm-getting-started/submit-results.png)
...
1. В верхней части панели мониторинга Storm выберите **Storm UI** (Пользовательский интерфейс Storm). Отобразится информация о кластере и обо всех запущенных топологиях.
	...
	![пользовательский интерфейс storm, отображающий сводку топологии](./media/hdinsight-storm-getting-started/stormui.png)
...
4. В разделе **Topology summary** (Сводка топологии) выберите запись **wordcount** (количество слов) в столбце **Name** (Имя). После этого отобразится дополнительная информация о топологии.
	...
	![интерфейс storm](./media/hdinsight-storm-getting-started/topology-summary.png)
	...
	* **Rebalance** (Повторная балансировка) — корректировка параллелизма топологии. После изменения числа узлов в кластере необходимо выполнить повторную балансировку топологий. Это позволяет топологии скорректировать параллелизм для компенсации увеличения или уменьшения количества узлов в кластере. Дополнительные сведения см. в статье <a href="http://storm.apache.org/documentation/Understanding-the-parallelism-of-a-Storm-topology.html" target="_blank">Understanding the parallelism of a Storm topology</a> (Общие сведения о параллелизме топологии Storm).
...
5. На этой странице выберите запись и раздела **Spouts** (Воронки) или **Bolts** (Сита). Отобразится информация о выбранном компоненте.
	...
	![интерфейс storm](./media/hdinsight-storm-getting-started/component-summary.png)
...
* **Средства HDInsight для Visual Studio** —средства HDInsight позволяют использовать Visual Studio для отправки и отслеживания топологии Storm, а также управления ею, точно также как панель мониторинга Storm, которая упоминалась выше. Средства HDInsight также позволяют создавать топологии Storm C# и включает примеры топологий, которые можно развернуть и запустить в кластере.
	...
	Дополнительную информацию см. в статье <a href="../hdinsight-hadoop-visual-studio-tools-get-started/" target="_blank">Начало работы со средствами HDInsight Hadoop для Visual Studio</a>.
