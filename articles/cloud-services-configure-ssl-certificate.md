...
<p>Процедуры в этой задаче применяются для облачных служб Azure; для веб-сайтов см. <a href="../web-sites-configure-ssl-certificate/">Настройка сертификата SSL для веб-сайта Azure</a>.</p>
</div>
...
Теперь, когда файлы определения службы и конфигурации службы обновлены, создайте пакет развертывания для передачи в Azure. Если используется **cspack**, убедитесь, что вы не используете флаг
**/generateConfigurationFile**, поскольку это приведет к перезаписи сведений о сертификате, которые вы только что добавили.

<h2><a name="step3"> </a><span class="short-header">Передача в Azure</span>Шаг 3. Передача пакета развертывания и сертификата</h2>

Ваш пакет развертывания был обновлен для использования сертификата и была добавлена конечная точка HTTPS. Теперь можно передать пакет и сертификат в Azure через портал управления.
...
Теперь, когда развертывание передано и запущено в Azure, вы можете подключиться к нему, используя HTTPS.
