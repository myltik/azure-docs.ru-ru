...
-   [Шаг 1: получение SSL-сертификата][Шаг 1: получение SSL-сертификата]
...
3.  Откройте файл определения службы и добавьте элемент **Binding** в раздел **Sites**. Это действие добавляет привязку HTTPS для сопоставления конечной точки с вашим сайтом:
    ...
    Все необходимые изменения внесены в файл определения службы, но еще нужно добавить информацию о сертификате в файл конфигурации службы.
...
## <a name="step3"> </a><span class="short-header">Передача в Azure</span>Шаг 3: передача пакета развертывания и сертификата
...
## <a name="step4"> </a><span class="short-header">Подключение с использованием HTTPS</span>Шаг 4: подключение к экземпляру роли с использованием HTTPS
