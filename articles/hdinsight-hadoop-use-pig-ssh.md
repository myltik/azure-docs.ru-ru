<properties
   ...
   description="Информация об использовании Pig с Hadoop в HDInsight с применением SSH."
   services="hdinsight"
   documentationCenter=""
   authors="Blackmist"
   manager="paulettm"
...
#Выполнение заданий Pig с помощью команды Pig (SSH)

[AZURE.INCLUDE [pig-selector](../includes/hdinsight-selector-use-pig.md)]

В этом документе приведены пошаговые инструкции по подключению к кластеру HDInsight на платформе Linux с использованием протокола SSH, а также использованию команды Pig для выполнения операторов Pig Latin в интерактивном режиме или в качестве пакетного задания.
...
Дополнительную информацию об использовании Putty см. в подразделе **Использование Putty для подключения к компьютеру под управлением Linux** раздела <a href="http://azure.microsoft.com/ documentation/articles/virtual-machines-linux-use-ssh-key/" target="_blank">"Как использовать SSH с Linux в Azure".</a>.

> [AZURE.NOTE] Если сертификат использовался для аутентификации SSH в кластере HDInsight, вам также потребуется ознакомиться с подразделом **Создание PPK для Putty** раздела <a href="http://azure.microsoft.com/ documentation/articles/virtual-machines-linux-use-ssh-key/" target="_blank">"Как использовать SSH с Linux в Azure".</a>
...
4. Затем преобразуйте данные, применив регулярное выражение для извлечения из каждой записи только информации об уровне ведения журнала, следующим образом.
	...
	Вы можете использовать **DUMP**, чтобы просмотреть данные после преобразования. В этом случае используется `DUMP LEVELS;`.
...
3. После выхода из строки grunt, используйте следующую команду, чтобы связать STDIN с файлом с именем **pigbatch.pig**. Этот файл будет создан в домашнем каталоге учетной записи, которая использовалась для входа в сеанс SSH.
