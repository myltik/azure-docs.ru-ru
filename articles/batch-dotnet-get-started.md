<properties
	...
	description="Дополнительные сведения о пакете Azure и разработке для службы пакета с простой сценарий основные понятия"
	services="batch"
...
-	[Учебное руководство 1: Библиотека пакета Azure для .NET](#tutorial1)
-	[Учебное руководство 2: Библиотека Azure пакета приложений для .NET](#tutorial2)  


Общие сведения и сценарии для пакета Azure см. в разделе [Технический обзор пакета Azure](batch-technical-overview.md).

##<a name="tutorial1"></a>Учебное руководство 1: Библиотека пакета Azure для .NET
...
>
>Необходимо использовать NuGet для получения **Microsoft.Azure.Batch.dll** сборки. После создания проекта в Visual Studio, щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Поиск в Интернете **Azure.Batch** и выберите команду установки для установки пакета Azure пакет и зависимости.
>
...
>
>Кроме того, можно ссылаться на [Azure пакета Образец Hello World](https://code.msdn.microsoft.com/Azure-Batch-Sample-Hello-6573967c) на сайте MSDN образец, код, описанный ниже.
...
-	**Учетную запись** — уникальный идентификатор сущности в службе. Вся обработка данных осуществляется через учетную запись Пакетной службы.
-	**Пула** — коллекцию задач виртуальных машин, на котором выполняются задачи приложения.
-	**Задачи виртуальной машины** -компьютере, который назначен в пул и используется задач, назначенных заданий, запускаемых в пуле.
-	**Задачи пользователя виртуальной машины** — учетную запись пользователя на виртуальной машине задачи.
-	**Рабочий элемент** -указывает, как вычисление выполняется на виртуальных машинах задачи в пуле.
-	**Задания** - выполнение экземпляра рабочего элемента и представляет собой набор задач.
-	**Задачи** — приложение, связанный с заданием и работает на виртуальной машине задачи.
-	**Файл** — содержит сведения, обрабатываемые задачей.  

Давайте начнем с наиболее общих сценариев использования.

###Создание учетной записи Пакетной службы Azure
Для создания учетной записи Пакетной службы можно использовать портал управления. После создания учетной записи вам будет предоставлен ключ. Дополнительные сведения см. в разделе [Технический обзор пакета Azure](batch-technical-overview.md).

###Способ: добавить учетную запись пула
Пул виртуальных машин задач — это первый набор ресурсов, которые необходимо создать для выполнения задач.

1.	Откройте Microsoft Visual Studio 2013 на **файл** меню, щелкните **Создать**, и нажмите кнопку **проекта**.

2.	Из **Windows**, в разделе **Visual C#**, щелкните **Консольное приложение**, назовите проект **GettingStarted**, присвойте решению имя **AzureBatch**, и нажмите кнопку **ОК**.
...
4.	Добавьте в класс Program следующие переменные:
	...
	Замените следующие значения:- **[имя из пула]** - имя, которое вы хотите использовать для пула. - **[имя из пакета промежуточных]** - имя учетной записи пакета. - **[ключ из пакета промежуточных]** -ключ, предоставленная для учетной записи пакета.
...
7.	Добавьте следующий код в метод Main, чтобы создать пул, если он не существует:
		...
		Console.ReadLine();
8.	Сохраните и выполните программу. Находится в состоянии **Active** для пула, который был успешно добавлен.  

###Как: список пулов в учетной записи
...
###Как: список рабочих элементов в учетной записи
...
##Способ: добавить рабочий элемент в учетной записи
...
2.	При создании рабочего элемента также формируется задание. Рабочий элемент можно назначить имя, но всегда присваивается имя задание **задания 0000000001**. Добавьте следующий код в Main (перед кодом списка рабочих элементов), который добавит рабочий элемент:
		...
		Console.ReadLine();
3.	Сохраните и выполните программу. Находится в состоянии **Active** для рабочего элемента, который был успешно добавлен. Вы должны увидеть следующие выходные данные.
...
###Как: Добавление задач к заданию
...
1.	В обозревателе решений, создайте новый проект консольного приложения с именем **ProcessTaskData** в **AzureBatch** решения.
...
Чтобы успешно завершить оставшуюся часть данного учебника, вам потребуется учетная запись службы хранения. Если вы не знаете, как это сделать, см. раздел [Создать учетную запись хранилища Azure](#tutorial1_storage).
...
>[AZURE.NOTE]Группа хранения Azure имеет [в блоге](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) список обозревателей хранилища Azure, которые могут помочь при передаче файлов.
...
1.	Добавьте в класс Program следующие переменные:
	...
	Замените следующие значения:- **[пути к хранилищу]** -путь к BLOB-объектов в хранилище. Например: http://yiding.blob.core.windows.net/gettingstarted/
...
###Как: Удаление пула и рабочих элементов
...
###<a name="tutorial1_storage"></a>ПРИЛОЖЕНИЕ: Создать учетную запись хранилища Azure
...
2.	В нижней части панели навигации, щелкните **Создать**. ![][1]
3.	Щелкните **DATA SERVICES**, затем **ХРАНЕНИЯ**, и нажмите кнопку **БЫСТРОЕ создание**. ![][2]

4.	В **URL-адрес**, введите имя дочернего домена для использования URI для учетной записи хранилища. Записи могут содержать от 3 до 24 строчных букв и цифр. Это значение станет именем узла в URI, который используется для адресации ресурсов большого двоичного объекта, очереди и таблицы в подписке.
5.	Выберите **РАСПОЛОЖЕНИЕ или ТЕРРИТОРИАЛЬНАЯ ГРУППА** в которой требуется расположить хранилище.
6.	При необходимости можно включить георепликацию.
7.	Щелкните **СОЗДАТЬ УЧЕТНУЮ ЗАПИСЬ ХРАНЕНИЯ**.  

Дополнительные сведения о хранилище Azure см. в разделе [Использование службы хранилища больших двоичных объектов Azure в .NET](http://azure.microsoft.com/develop/net/how-to-guides/blob-storage/).


##<a name="tutorial2"></a>Учебное руководство 2: Azure пакета приложений библиотеки для .NET
...
>[AZURE.NOTE]Для работы с этим учебником требуется учетная запись Azure. Вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](http://azure.microsoft.com/pricing/free-trial/). NuGet можно использовать для получения обоих <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps.Cloud/">облачных приложений пакета</a> сборки и <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps/">клиентских приложений пакета</a> сборки. После создания проекта в Visual Studio, щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Можно также загрузить расширение Visual Studio для приложений пакета, который включает шаблон проекта enable облачных приложений, а также возможность развернуть приложение <a href="https://visualstudiogallery.msdn.microsoft.com/8b294850-a0a5-43b0-acde-57a07f17826a">здесь</a> или через поиск **пакета приложения** в Visual Studio через пункт меню расширения и обновления. Можно также найти <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">Образцы начала до конца на сайте MSDN.</a>
>
...
3.	Предоставьте способ отправки заданий приложению в Azure. Это может быть подключаемый модуль пользовательского интерфейса приложения, веб-портал или же автоматическая служба, входящая в ваш конвейер выполнения. В разделе <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">Образцы</a> на сайте MSDN, примеры.
