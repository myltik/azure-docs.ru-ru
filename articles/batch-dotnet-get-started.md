...
>
>Для получения сборки **Microsoft.Azure.Batch.dll** вам необходимо использовать NuGet. После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Выполните поиск **Azure.Batch**, затем нажмите "Установить", чтобы установить пакет Пакетной службы Azure и его зависимости.
>
...
> 
>Кроме того, вы можете обратиться к [примеру Пакетной службы Azure Hello World](https://code.msdn.microsoft.com/Azure-Batch-Sample-Hello-6573967c) на сайте MSDN, который содержит код, по теме этого учебника.
...
-	**Пул** - это коллекция виртуальных машин, на которых выполняются приложения.
-	**Виртуальная машина задач** - это выделенная для пула машина, которая используется для выполнения задач из заданий пула.
...
1.	Откройте Microsoft Visual Studio 2013, в меню **Файл** нажмите **Создать** и затем **Проект**.

2.	В категории **Windows**, **Visual C#** выберите **Консольное приложение**, назовите проект **GettingStarted**, назовите решение **AzureBatch** и нажмите кнопку **ОК**.
...
4.	Добавьте в класс Program следующие переменные:
	...
	-	**[name-of-pool]** - имя, которое вы хотите использовать для пула.
	-	**[name-of-batch-account]** - имя учетной записи Пакетной службы.
	-	**[key-of-batch-account]** - ключ, предоставленный вам для учетной записи Пакетной службы.
...
2.	При создании рабочего элемента также формируется задание. Вы можете определить имя рабочего элемента, но задание всегда будет иметь имя **job-0000000001**. Добавьте следующий код в Main (перед кодом списка рабочих элементов), который добавит рабочий элемент:
...
>[WACOM.NOTE] Команда разработчиков службы хранилища Azure написала [статью в блоге](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) с описанием программ-обозревателей службы хранилища Azure для отправки файлов.
...
4.	В качестве адреса **URL** введите имя поддомена для URI учетной записи хранения. Записи могут содержать от 3 до 24 строчных букв и цифр. Это значение станет именем узла в URI, который используется для адресации ресурсов большого двоичного объекта, очереди и таблицы в подписке.
...
>[WACOM.NOTE] Для работы с этим учебником требуется учетная запись Azure. Вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](http://www.windowsazure.com/ru-ru/pricing/free-trial/). Вы можете использовать NuGet для получения как <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps.Cloud/">сборки облачных технологий приложений Пакетной службы,</a> так и <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps/">сборки клиента приложений Пакетной службы</a> . После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Вы также можете загрузить расширение Visual Studio для приложений Пакетной службы, которое включает в себя шаблон проекта для облачных приложений с возможностью их развертывания, <a href="https://visualstudiogallery.msdn.microsoft.com/8b294850-a0a5-43b0-acde-57a07f17826a">здесь</a> или выполните поиск **приложений Пакетной службы** в Visual Studio через пункт меню "Расширения и обновления".  Вы также можете найти <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">готовые примеры в MSDN.</a>
>
...
3.	Предоставьте способ отправки заданий приложению в Azure. Это может быть подключаемый модуль пользовательского интерфейса приложения, веб-портал или же автоматическая служба, входящая в ваш конвейер выполнения. См. <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">примеры</a> в MSDN.
