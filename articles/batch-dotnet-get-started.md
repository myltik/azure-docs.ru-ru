<properties title="Tutorial - Getting Started with the Azure Batch Library for .NET" pageTitle="Учебник "Приступая к работе с библиотекой Пакетной службы Azure для .NET"" description="обязательно" metaKeywords="" services="batch" solutions="" documentationCenter=".NET" authors="yidingz, karran.batta" videoId="" scriptId="" manager="timlt" />
...
Общую информацию и сценарии для Пакетной службы Azure см. в разделе [Технический обзор Пакетной службы Azure](http://azure.microsoft.com/ru-ru/documentation/articles/batch-technical-overview/).
...
>[WACOM.NOTE] Для работы с этим учебником требуется учетная запись Azure. Вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](http://www.windowsazure.com/ru-ru/pricing/free-trial/). 
>
>Для получения сборки **Microsoft.Azure.Batch.dll** необходимо использовать NuGet. После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Выполните в Интернете поиск **Azure.Batch** и нажмите кнопку "Установить", чтобы установить пакет службы хранилища Azure и его зависимости.
>
>Убедитесь, что используется диспетчер пакетов Nuget версии 2.8 или более поздней версии. Номер версии можно найти в диалоговом окне Visual Studio, выбрав "Справка -> О Microsoft Visual Studio". При наличии более старой версии диспетчера пакетов Nuget необходимо обновить Visual Studio или скачать требуемую версию зависимостей Nuget.
> 
>Дополнительно можно обратиться к примеру [Azure Batch Hello World](https://code.msdn.microsoft.com/Azure-Batch-Sample-Hello-6573967c) на сайте MSDN, который содержит код, подобный коду из этого учебника.
...
-	**Учетная запись** - это уникально идентифицируемая сущность в Пакетной службе. Вся обработка данных осуществляется через учетную запись Пакетной службы.
-	**Пул** - это коллекция виртуальных машин задач, на которых выполняются приложения.
-	**Виртуальная машина задач** - это выделенная для пула машина, которая используется для выполнения задач задания пула.
-	**Пользователь виртуальной машины задач** - это учетная запись пользователя на виртуальной машине задач.
-	**Рабочий элемент** - указывает, как следует выполнять вычисления на виртуальных машинах в пуле.
...
Для создания учетной записи Пакетной службы можно использовать портал управления. После создания учетной записи вам будет предоставлен ключ. Дополнительную информацию см. в разделе [Технический обзор Пакетной службы Azure](http://azure.microsoft.com/ru-ru/documentation/articles/batch-technical-overview/).  
...
1.	Откройте Microsoft Visual Studio 2013, в меню **Файл** выберите **Создать** и щелкните **Проект**.

2.	В подразделе **Windows** раздела **Visual C#** выберите **Консольное приложение**, назовите проект **GettingStarted**, присвойте решению имя **AzureBatch** и нажмите кнопку **ОК**.
...
4.	Добавьте в класс Program следующие переменные:
	...
	-	**[name-of-pool]** - желаемое имя пула.
	-	**[name-of-batch-account]** - имя учетной записи Пакетной службы.
	-	**[key-of-batch-account]** - ключ для учетной записи Пакетной службы.
...
7.	Добавьте следующий код в метод Main, чтобы создать пул, если он не существует:
		...
		Console.ReadLine();
8.	Сохраните и выполните программу. Если пул был добавлен успешно, он будет в состоянии **Активный**.  

###Практическое руководство. Получение списка пулов учетной записи
...
3.	Добавьте в метод Main следующий код, который определит имена и состояния всех пулов учетной записи, а также создаст пул, если он не существует:
...
###Практическое руководство. Получение списка рабочих элементов учетной записи
...
2.	При создании рабочего элемента также создается задание. Вы можете определить имя рабочего элемента, но задание всегда будет иметь имя **job-0000000001**. Добавьте следующий код в метод Main (перед кодом списка рабочих элементов), который добавит рабочий элемент:
		...
		Console.ReadLine();
3.	Сохраните и выполните программу. Если рабочий элемент был добавлен успешно, он будет в состоянии **Активный**. Вы должны увидеть следующие выходные данные.
...
Рабочий элемент без задачи не будет ничего делать. После создания рабочего элемента и задания нужно добавить задачи для задания. Давайте добавим простую задачу в задание.  
...
1.	В обозревателе решений создайте новый проект консольного приложения **ProcessTaskData** в решении **AzureBatch**.
...
1.	Передайте файлы, которые необходимо использовать в сценарии распределенных вычислений, в учетную запись хранения Azure. Чтобы Пакетная служба имела к ним доступ, они должны находиться в учетной записи хранения. Файлы загружаются на виртуальную машину задачи во время выполнения задачи.
...
5.	Добавьте задачи в задание. Каждая задача использует переданную программу для обработки информации из переданного вами файла.
6.	Отслеживайте выходные данные.  

Мы рассмотрели шаги с 3 по 6. Теперь давайте посмотрим, как подготовить службу хранилища Azure для выполнения задачи.
...
1. Создайте в своей учетной записи хранения Azure контейнер с названием "gettingstarted". Это можно сделать на портале Azure. Убедитесь, что для поля "ДОСТУП" выбрано значение "Общедоступный контейнер".
...
>[WACOM.NOTE] В рабочей среде рекомендуется использовать подписанный URL-адрес.


>[WACOM.NOTE] Рабочая группа службы хранилища Azure предоставила [запись блога](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx) с описанием обозревателей службы хранилища Azure, которые могут помочь передавать файлы.
...
1.	Добавьте в класс Program следующие переменные:
	...
	Замените следующие значения:
	-	**[storage-path]** - путь к большому двоичному объекту в хранилище. Например: http://yiding.blob.core.windows.net/gettingstarted/
...
4.	В поле **URL-адрес** введите имя поддомена, который будет использоваться в универсальном коде ресурса (URI) для учетной записи хранения. Записи могут содержать от 3 до 24 строчных букв и цифр. Это значение станет именем узла в универсальном коде ресурса (URI), который используется для обращения к ресурсам больших двоичных объектов, очередей и таблиц для подписки.
5.	В поле **РЕГИОН ИЛИ ТЕРРИТОРИАЛЬНАЯ ГРУППА** выберите, где требуется расположить хранилище.
6.	При необходимости можно включить георепликацию.
7.	Щелкните **СОЗДАТЬ УЧЕТНУЮ ЗАПИСЬ ХРАНЕНИЯ**.  

Дополнительную информацию о службе хранилища Azure см. в разделе [Использование службы хранилища больших двоичных объектов Azure в .NET.](http://www.windowsazure.com/ru-ru/develop/net/how-to-guides/blob-storage/).  
...
Приложения Пакетной службы - одна из функций Пакетной службы Azure, которая дает возможность управлять рабочими нагрузками Пакетной службы и выполнять их на уровне приложения.  С помощью пакета SDK для приложений Пакетной службы можно создать приложения Пакетной службы, а также развернуть их в собственной учетной записи или же сделать их доступными для других пользователей Пакетной службы.  Пакетная служба также дает возможность управлять данными, осуществлять мониторинг заданий, предоставляет встроенные инструменты диагностики и ведения журнала, а также обеспечивает поддержку внутренних зависимостей задач.  Кроме того, она включает в себя портал управления, который позволяет управлять заданиями, просматривать журналы и скачивать выходные данные без необходимости написания собственного клиента.

В сценарии приложений Пакетной службы необходимо написать код с использованием пакета SDK для облачных приложений Пакетной службы, чтобы разделять задания на параллельные задачи, описать зависимости между задачами и указать способ выполнения каждой задачи.  Этот код развертывается в учетной записи Пакетной службы.  После этого клиенты смогут выполнять задания путем указания типа задания и входных файлов для интерфейса REST API.

>[WACOM.NOTE] Для работы с этим учебником требуется учетная запись Azure. Вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Бесплатная пробная версия Azure](http://www.windowsazure.com/ru-ru/pricing/free-trial/). NuGet можно использовать для получения сборок <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps.Cloud/">облачных приложений Пакетной службы</a> и <a href="http://www.nuget.org/packages/Microsoft.Azure.Batch.Apps/">клиента приложений Пакетной службы</a>. После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Также вы можете скачать расширение Visual Studio для приложений Пакетной службы, которое включает в себя шаблон проекта для облака приложений Пакетной службы и возможность развернуть приложение <a href="https://visualstudiogallery.msdn.microsoft.com/8b294850-a0a5-43b0-acde-57a07f17826a">здесь</a> или через поиск **приложений Пакетной службы** в Visual Studio с помощью пункта меню "Расширения и обновления".  Также можно найти <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">полные примеры на сайте MSDN.</a>
>

###Основные положения приложений Пакетной службы Azure 
Пакетная служба предназначена для работы с существующими приложениями, которые разработаны для крупномасштабных вычислений. Она использует существующий код приложения и запускает его в динамической виртуализированной среде общего назначения. Вот что нужно сделать, чтобы приложение стало совместимым с приложениями Пакетной службой:

1.	Подготовьте ZIP-файл с исполняемыми файлами существующего приложения (это те же исполняемые файлы, которые могут выполняться на традиционной ферме серверов или кластерах) и всеми необходимыми файлами поддержки. Затем передайте этот ZIP-файл в вашу учетную запись Пакетной службы с помощью портала управления или REST API.
2.	Создайте ZIP-файл "облачных сборок", которые распределяют рабочую нагрузку для приложения, и передайте его с помощью портала управления или REST API. Облачная сборка состоит из двух компонентов, которые собираются с помощью пакета SDK для облачных приложений:
...
3.	Предоставьте приложению в Microsoft Azure способ отправки заданий. Это может быть подключаемый модуль пользовательского интерфейса приложения, веб-портал или же автоматическая служба, входящая в ваш конвейер выполнения. См. <a href="https://go.microsoft.com/fwLink/?LinkID=512183&clcid=0x409">примеры</a> на сайте MSDN.
...
Задачи, указанные разделителем заданий, представлены в виде объектов TaskSpecifier. TaskSpecifier содержит ряд свойств, которые можно установить перед возвратом задачи.
...
RequiredFiles - это пустая коллекция по умолчанию. В нее можно добавить новые элементы или заменить ее новой коллекцией. Записи из коллекции файлов задания можно копировать с помощью метода RequiringJobFiles или RequiringAllJobFiles.
...
Также вы можете указать, что обработка всей группы задач не должна начинаться до полного завершения задач другой группы. Для этого можно определить свойство TaskSpecifier.Stage. Задачи с определенным значением свойства Stage не будут переданы на обработку, пока не завершены все задачи с меньшими значениями свойства Stage. Например, задачи, у которых свойству Stage присвоено значение 3, не начнут обрабатываться, пока не будут завершены все задачи, у которых свойство Stage имеет значение 0, 1 или 2. Значения свойства Stage должны начинаться с 0 и не иметь разрывов, а метод SplitIntoTasks должен возвращать задачи в порядке следования стадий: Например, возвращение задачи, у которой свойство Stage имеет значение 0, после задачи, у которой свойство Stage имеет значение 1, будет ошибкой.

Каждое задание с параллельной обработкой завершается специальной задачей слияния. Задача слияния объединяет результаты задач параллельной обработки в итоговый результат. Приложения Пакетной службы автоматически создают задачу слияния.  
...
Следующий фрагмент кода демонстрирует вызов программы с именем application.exe. Обратите внимание, что можно использовать вспомогательный метод ExecutablePath для создания абсолютных путей.

Класс ExternalProcess из пакета SDK для облачных приложений предоставляет полезные вспомогательные методы для исполняемых объектов вашего приложения. ExternalProcess может позаботиться об отмене действий, преобразовании кодов выхода в исключения, захвате стандартных потоков вывода и ошибок, а также обеспечить настройку переменных среды. Для непосредственного запуска программ можно дополнительно использовать класс .NET Process.

Метод RunExternalTaskProcess возвращает TaskProcessResult, который включает в себя список выходных файлов. К ним относятся как минимум все файлы, необходимые для слияния. Также можно возвращать файлы журнала, файлы предварительного просмотра и промежуточные файлы (например, для диагностики, если задача завершена с ошибкой).  Обратите внимание, что метод возвращает пути к файлам, а не их содержимое.

Для каждого файла должен быть определен тип выходных данных: выходные данные (то есть часть выходных данных задания), данные предварительного просмотра, данные журнала или промежуточные данные.  Эти значения берутся из перечисления TaskOutputFileKind. Следующий фрагмент возвращает выходные данные одной задачи без данных предварительного просмотра или данных журнала. Метод TaskProcessResult.FromExternalProcessResult упрощает распространенный сценарий записи кода выхода, выходных данных обработчика и выходных файлов программы командной строки:
...
Этот метод вызывается задачей слияния. Он вызывает приложение для объединения выходных данных предыдущих задач, которые соответствуют приложению, и возвращает итоговый результат. 

Реализация RunExternalMergeProcess очень похожа на реализацию RunExternalTaskProcess за небольшим исключением:  

-	RunExternalMergeProcess использует выходные данные предыдущих задач, а не входные файлы пользователя. Следовательно, вам нужно определять имена обрабатываемых файлов на основании идентификатора задачи, а не коллекции Task.RequiredFiles.
...
Задание описывает рабочую нагрузку и должно включать в себя всю необходимую информацию о ней, за исключением содержимого файлов. Например, задание содержит параметры конфигурации, которые передаются от клиента к задаче через разделитель заданий. На сайте MSDN приводятся примеры отправки заданий и работы с несколькими клиентами, включая веб-порталы и клиенты командной строки. 
