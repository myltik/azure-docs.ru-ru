...
Так как таблица DimEmployee в примере базы данных AdventureWorksDW содержит пользователей из домена AdventureWorks и эти имена не существуют в вашей среде, нужно создать в модели таблицу, содержащую небольшую выборку из трех реальных пользователей вашей организации. Затем добавьте этих пользователей в качестве членов в новую роль. Вам не требуется знать пароли для выбранных имен пользователей, но нужно использовать фактические имена пользователей Windows из собственного домена.  
...
3.  Замените имя, фамилию и сочетание домена и имени пользователя на имена и идентификаторы входа трех пользователей из организации. Укажите одного пользователя в первых двух строках с идентификатором EmployeeId 1. Это показывает, что пользователь относится к нескольким территориям сбыта. Не изменяйте поля EmployeeId и SalesTerritoryId.  
...
1.  В таблице **DimGeography** в представлении схемы конструктора моделей нажмите и удерживайте столбец **SalesTerritoryId**, а затем перетащите курсор на столбец **SalesTerritoryId** в таблице **DimSalesTerritory**.  
...
2.  В таблице **FactInternetSales** нажмите и удерживайте столбец **SalesTerritoryId**, а затем перетащите курсор на столбец **SalesTerritoryId** в таблице **DimSalesTerritory**.  
    ...
    Обратите внимание, что свойство "Активный" для этой связи имеет значение False, то есть связь неактивна. Это вызвано тем, что таблица FactInternetSales уже имеет другую активную связь.  
...
8.  Для таблицы **EmployeeSecurity** в столбце **Фильтр DAX** введите следующую формулу:  
    ...
    Эта формула указывает, что все столбцы решаются в логическое условие false. Таким образом, член роли "Торговые специалисты по территориям" не может запросить никакие столбцы для таблицы EmployeeSecurity.  
...
3.  В поле **Введите имя объекта** диалогового окна **Выберите пользователя или группу** введите одно из имен пользователей, включенных в таблицу EmployeeSecurity, после чего щелкните **Проверить имена**.  
...
4.  Нажмите кнопку **ОК**, чтобы закрыть диалоговое окно **Выберите пользователя или группу**, и еще раз нажмите **ОК**, чтобы закрыть диалоговое окно **Анализ в Excel**.  
    ...
    Обратите внимание, что таблица EmployeeSecurity не отображается в списке полей сводной таблицы. Это вызвано тем, что вы скрыли ее от клиентских средств в предыдущей задаче.  
