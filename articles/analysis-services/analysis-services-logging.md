...
При выборе Engine регистрирует все [xEvents](https://docs.microsoft.com/sql/analysis-services/instances/monitor-analysis-services-with-sql-server-extended-events). Нельзя выбрать отдельные события. 

|Категории XEvent |Название мероприятия  |
|---------|---------|
...
### <a name="all-metrics"></a>Все показатели

Категория AllMetrics журналы же [метрики сервера](analysis-services-monitor.md#server-metrics) отображаемых метрик.

## <a name="setup-diagnostics-logging"></a>Настройка ведения журнала диагностики

### <a name="by-using-the-azure-portal"></a>с помощью портала Azure;

1. В [портал Azure](https://portal.azure.com), на сервере служб Analysis Services Azure щелкните **журналы диагностики** в левой области навигации, а затем выберите **включить диагностику**.
...
Ниже приведены основные команды для ознакомления. Пошаговые указания по настройке ведения журнала в учетную запись хранения с помощью PowerShell, в статье [учебника](#tutorial) далее в этой статье.
...
Чтобы просмотреть диагностические данные в службе анализа журналов, откройте страницу поиска журналов из меню слева или область управления страницы, как показано на следующем рисунке:
...
### <a id="identify"></a>Определение учетной записи сервера для журналов
