...
В этом руководстве показано, как программными средствами Python реализовать общие задачи управления службами. Класс **ServiceManagementService** в [пакете SDK для Azure для Python][download-SDK-Python] поддерживает программный доступ к большинству функциональных возможностей, связанных с управлением службами, которые доступны на [портале управления][management-portal] (например, **создание, обновление и удаление облачных служб, развертываний, служб управления данными, виртуальных машин и территориальными группами**). Эта возможность может быть полезна при создании приложений, которым требуется программный доступ к управлению службами. 
...
* [Практическое руководство. Список доступных местоположений][]
...
* [Практическое руководство. Перемещение развертываний из промежуточного хранения в рабочее развертывание][]
* [Практическое руководство. Удаление развертывания][]
* [Практическое руководство. Создание службы хранения][]
* [Практическое руководство. Удаление службы хранения][]
* [Практическое руководство. Создание территориальной группы][]
* [Практическое руководство. Удаление территориальной группы][]
* [Практическое руководство. Список доступных операционных систем][]
* [Практическое руководство. Создание образа операционной системы][]
* [Практическое руководство. Удаление образа операционной системы][]
...
API управления службами обеспечивает программный доступ к большинству функциональных возможностей управления службами, доступных через [портал управления][management-portal]. Пакет Azure SDK для Python позволяет управлять облачными службами, учетными записями хранения и территориальными группами.

Чтобы использовать API-интерфейс управления службами, необходимо [создать учетную запись Azure](http://azure.microsoft.com/pricing/free-trial/). 
...
> [AZURE.NOTE] Начиная с версии Azure SDK для Python v0.8.0, появилась возможность использовать сертификаты, созданные OpenSSL при работе в Windows.  Для этого требуется Python 2.7.4 или более поздняя версия.
...
Команда приведет к созданию файла  `.cer`, после чего файл будет установлен в **Личное** хранилище сертификатов. Дополнительные сведения содержатся в разделе [Создание и передача сертификата управления для Azure](http://msdn.microsoft.com/library/windowsazure/gg551722.aspx)

После создания сертификата необходимо передать  `.cer`-файл в Azure с помощью действия "Загрузить" на вкладке "Параметры" [портала управления][management-portal].
...
Можно также использовать [OpenSSL](http://www.openssl.org/) для создания сертификата управления.  Фактически нужно создать два сертификата: один для сервера (файл с расширением `.cer`) и один для клиента (файл с расширением  `.pem`). Чтобы создать файл с расширением `.pem`, выполните следующее:

	`openssl req -x509 -nodes -days 365 -newkey rsa:1024 -keyout mycert.pem -out mycert.pem`

Чтобы создать сертификат (файл с расширением `.cer`), выполните следующее:
...
После создания этих файлов необходимо передать  `.cer`-файл в Azure с помощью действия "Отправить" на вкладке "Параметры" на [портале управления][management-portal], затем запомнить, куда был сохранен  `.pem`-файл.
...
Обратите внимание: прежде чем удалить службу, необходимо сначала удалить все развернутые приложения этой службы. (См. [Практическое руководство. Удаление развертывания](#DeleteDeployment) .
...
* **конфигурация**: файл конфигурации службы (CSCFG-файл), кодированный в base64.
* **метка**: метка имени размещенной службы (автоматически кодируется в base64).

В следующем примере создается новое развертывание  `v1` for a hosted service called `myhostedservice`:
...
Развертывание может быть обновлено с помощью метода **change\_deployment\_configuration** или метода **update\_deployment\_status** method.
...
Метод **update\_deployment\_status** позволяет настраивать состояние развертывания как ВЫПОЛНЯЕТСЯ или ПРИОСТАНОВЛЕНО. В следующем примере продемонстрировано, как задать состояние ВЫПОЛНЯЕТСЯ для развертывания с именем  `v1` of a hosted service called `myhostedservice`:
...
Служба [хранилища] предоставляет доступ к [большим двоичным объектам][azure-blobs], [таблицам][azure-tables] и [запросам][azure-queues] Azure. Для создания службы хранения требуется имя службы (от 3 до 24 символов нижнего регистра, уникальное имя в Azure), описание, метка (до 100 символов, автоматически кодируется в base64) и расположение или территориальная группа. В следующем примере показано, как создать службу хранилища путем указания расположения. Если вы хотите использовать территориальную группу, вам сначала потребуется ее создать (см. [Практическое руководство. Создание территориальной группы](#CreateAffinityGroup)) и настроить ее с помощью параметра **affinity\_group**.
...
Обратите внимание, что в приведенном выше примере состояние операции **create\_storage\_account** может быть получено путем передачи результата, возвращенного **create\_storage\_account** в метод **get\_operation\_status**.  
...
Чтобы удалить пользовательский образ, используйте метод **delete\_os\_image**:
...
-   См. справочник MSDN: [Облачные службы][]
-   См. справочник MSDN: [Виртуальные машины][]
...
[пакета службы]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
...
[Развертывание службы Azure]: http://msdn.microsoft.com/library/windowsazure/gg433027.aspx
...
[Облачные службы]: http://msdn.microsoft.com/library/windowsazure/jj155995.aspx
[Виртуальные машины]: http://msdn.microsoft.com/library/windowsazure/jj156003.aspx
