<properties
	pageTitle="Пошаговые руководства по процессу обработки и анализа данных в облаке | Azure" 
	description="Пошаговые руководства по процессу обработки и анализа данных в облаке" 
	metaKeywords="" 
...
# Процесс обработки и анализа данных в облаке в Машинном обучении Azure

В этой статье рассматривается схема процесса обработки и анализа данных в облаке, а также несколько примеров источников данных и целевых сценариев. В ней продемонстрированы некоторые варианты и последовательности операций обработки на основе характеристик данных, исходного расположения и целевых репозиториев в Azure. В последнем разделе приводится схема последовательности операций процесса обработки и анализа данных в облаке для примеров сценариев.

[AZURE.INCLUDE [machine-learning-free-trial](../includes/machine-learning-free-trial.md)]

**Для всех сценариев вам понадобится:**
...
**В следующих разделах представлены несколько примеров сценариев. Для каждого сценария перечислены возможные последовательности операций процесса обработки и анализа данных, а также вспомогательные ресурсы Azure.**

- [Сценарий №1. Набор табличных данных небольшого и среднего размера в локальных файлах](#smalllocal)
- [Сценарий №2. Набор данных небольшого и среднего размера в локальных файлах, требующий обработки](#smalllocalprocess)
- [Сценарий №3. Большой набор данных в локальных файлах, загружаемый в большие двоичные объекты Azure](#largelocal)
- [Сценарий №4. Набор данных небольшого и среднего размера в локальных файлах, загружаемый на сервер SQL Server в виртуальной машине Azure](#smalllocaltodb)
- [Сценарий №5. Большой набор данных в локальных файлах, загружаемый на сервер SQL Server в виртуальной машине Azure](#largelocaltodb)
- [Сценарий №6. Большой набор данных в локальной базе данных SQL Server, загружаемый на сервер SQL Server в виртуальной машине Azure](#largedbtodb)
- [Сценарий №7. Данные большего размера в локальных файлах, загружаемые в базу данных Hive в кластере Azure HDInsight Hadoop](#largedbtohive)
...
7.  Считайте данные из больших двоичных объектов Azure с помощью модуля [чтения][reader].
...
9. Считайте данные из больших двоичных объектов Azure с помощью модуля [чтения][reader].
...
6.  Загрузите данные в базу данных SQL Server на виртуальной машине Azure.
    ...
    а. Вариант №1. Использование SQL Server Management Studio.

		i.  Войдите в виртуальную машину SQL Server.
        ii. Запустите среду SQL Server Management Studio.
        iii. Создайте базу данных и целевые таблицы.
        iv. Используйте один из методов массового импорта для загрузки локальных файлов виртуальной машины.

    b. Вариант №2. Использование IPython Notebook (не рекомендуется использовать этот вариант для средних и больших наборов данных).

        i.  Используйте строку подключения ODBC для доступа к SQL Server на виртуальной машине.
        ii. Создайте базу данных и целевые таблицы.
        iii. Используйте один из методов массового импорта для загрузки локальных файлов виртуальной машины.
...
10. Считайте данные непосредственно из SQL Server с помощью модуля [чтения][reader]. При необходимости вставьте необходимый запрос, который извлекает поля, создает компоненты и примеры данных, непосредственно в запрос модуля [чтения][reader].
...
4.  Загрузите данные в базу данных SQL Server на виртуальной машине Azure.
	...
	e. Загрузите данные, используя один из методов массового импорта.

    f. Если требуется объединить таблицы, создайте индексы, чтобы ускорить этот процесс.

 > [AZURE.NOTE]Чтобы ускорить загрузку данных большого размера, мы советуем создать секционированные таблицы и массово импортировать данные в параллельном режиме. Дополнительную информацию см. в разделе [Параллельный импорт данных в секционированные таблицы SQL](machine-learning/machine-learning-data-science-parallel-load-sql-partitioned-tables.md).
...
8. Считайте данные непосредственно из SQL Server с помощью модуля [чтения][reader]. При необходимости вставьте необходимый запрос, который извлекает поля, создает компоненты и примеры данных, непосредственно в запрос модуля [чтения][reader].

9. Создайте простую последовательность операций эксперимента Машинного обучения Azure, начиная с переданных наборов данных.
...
9. Считайте данные непосредственно из `Hive Queries` с помощью модуля [чтения][reader]. При необходимости вставьте необходимый запрос, который извлекает поля, создает компоненты и примеры данных, непосредственно в запрос модуля [чтения][reader].

10. Создайте простую последовательность операций эксперимента Машинного обучения Azure, начиная с переданных наборов данных.

Сводка по примерам сценариев
------------------------

На следующей схеме показаны сценарии, описанные выше, и варианты, выбранные в каждом сценарии процесса обработки и анализа данных в облаке. Обратите внимание, что обработка и просмотр данных, а также разработка компонентов и создание примеров могут применяться в одном или нескольких методах или средах (в исходной, промежуточной или целевой среде) и при необходимости выполняться по несколько раз. Схема иллюстрирует только некоторые из возможных последовательностей операций.
...
Пример комплексного пошагового руководства по обработке и анализу данных в Azure с использованием общедоступного набора данных см. в разделе [Обработка и анализ данных в Azure на практике](machine-learning/machine-learning-data-science-process-sql-walkthrough.md).
