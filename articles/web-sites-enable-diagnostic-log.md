...
Azure предоставляет встроенные средства диагностики, помогающие отлаживать приложения, размещенные на веб-сайтах Windows Azure. В этой статье вы узнаете, как включить ведение журналов диагностики и добавить инструментирование в приложение, а также как получить доступ к данным журналов Azure.

> [WACOM.ПРИМЕЧАНИЕ] В данной статье описывается использование портала управления Azure, Azure PowerShell и межплатформенного интерфейса командной строки Azure для работы с журналами диагностики. Дополнительные сведения о работе с журналами диагностики с использованием Visual Studio см. в разделе [Устранение неполадок, связанных с веб-сайтами Azure, в Visual Studio][Устранение неполадок, связанных с веб-сайтами Azure, в Visual Studio].

## Оглавление

-   [Что такое диагностика веб-сайта?][Что такое диагностика веб-сайта?]
-   [Практическое руководство. Включение диагностики][Практическое руководство. Включение диагностики]
-   [Практическое руководство. Загрузка журналов][Практическое руководство. Загрузка журналов]
-   [Практическое руководство. Журналы потоковой передачи][Практическое руководство. Журналы потоковой передачи]
-   [Практическое руководство. Интерпретация журналов диагностики][Практическое руководство. Интерпретация журналов диагностики]
-   [Дальнейшие действия][Дальнейшие действия]

<a name="whatisdiag"></a>

## Что такое диагностика веб-сайта?
...
-   **Журналы веб-сервера** — выполняет регистрацию всех транзакций HTTP на веб-сайте с использованием [расширенного формата файлов журналов W3C][расширенного формата файлов журналов W3C]. Этот отчет используется при определении общей метрики сайта, например количества обработанных запросов или количества запросов с определенного IP-адреса

### Диагностика приложений

Диагностика приложений позволяет записывать сведения, созданные веб-приложениями. Приложения ASP.NET могут использовать класс [System.Diagnostics.Trace][System.Diagnostics.Trace] для записи информации в журнал диагностики приложений. Например:
...
Дополнительные сведения о работе с диагностикой приложений с использованием Visual Studio см. в разделе [Устранение неполадок, связанных с веб-сайтами Azure, в Visual Studio][1].

> [WACOM.ПРИМЕЧАНИЕ] В отличие от изменения файла web.config, включение диагностики приложений или изменение уровней журнала диагностики не приводит к перезапуску домена приложения, в котором работает данное приложение.
...
## Практическое руководство: включение диагностики
Для включения диагностики посетите страницу **Настройка** веб-сайта Azure на [портале управления Azure][портале управления Azure]. На странице **Настройка** используйте разделы **диагностика приложений** и **диагностика сайта**, чтобы включить ведение журнала.
...
> [WACOM.ПРИМЕЧАНИЕ] Можно одновременно активировать варианты "Файловая система", "Табличное хранилище" или "Хранилище blob-объектов" в любом сочетании, а также настроить индивидуальные конфигурации уровня ведения журнала. Например, запись в хранилище ошибок и предупреждений в хранилище blob-объектов может потребоваться в качестве решения для длительного подробного ведения журнала.

> [WACOM.ПРИМЕЧАНИЕ] Можно также включить диагностику из PowerShell Azure с помощью командлета **Set-AzureWebsite**. Если Azure PowerShell не установлена или не настроена для использования подписки Azure, см. раздел [Использование Azure PowerShell][Использование Azure PowerShell].

<a name="download"></a>

## Практическое руководство: загрузка журналов
...
-   **Журналы веб-сервера** — /LogFiles/http/RawLogs. Эта папка содержит один или несколько текстовых файлов, отформатированных с применением [расширенного формата журнала W3C][расширенного формата файлов журналов W3C].
...
> [WACOM.ПРИМЕЧАНИЕ] Если пункт **Развертывание/FTP-пользователь** не задан или вы забыли пароль для этого пользователя, можно создать нового пользователя и новый пароль с помощью ссылки **Сбросить учетные данные развертывания** в разделе **Сводка** **панели мониторинга**.
...
> [WACOM.ПРИМЕЧАНИЕ] Если Azure PowerShell не установлена или не настроена для использования подписки Azure, см. раздел [Использование Azure PowerShell][Использование Azure PowerShell].
...
Она сохранит журналы для веб-сайта под именем "имя\_веб-сайта" в файл **diagnostics.zip** в текущем каталоге.

> [WACOM.ПРИМЕЧАНИЕ] Если инструменты командной строки Azure PowerShell не установлены или не настроены для использования подписки Azure, см. раздел [Использование инструментов командной строки Azure PowerShell][Использование инструментов командной строки Azure PowerShell].

<a name="streamlogs"></a>

## Практическое руководство: журналы потоковой передачи
...
## Практическое руководство: интерпретация журналов диагностики
...
<td style="border:1px solid black;vertical-align:top">
Дата и время события в формате yyyyMMddHH
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Значение GUID, которое уникальным образом идентифицирует этот объект
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Дата и время, когда произошло событие, в формате отсчетов (более высокая точность)
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Уровень события (например, ошибка, предупреждение, информация)
...
По умолчанию равен 0 (если не указан)
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Экземпляр веб-сайта, на котором произошло событие
...
<td style="border:1px solid black;vertical-align:top;background-color:#8ddaf6">
Идентификатор потока, который вызвал это событие
...
<td style="border:1px solid black;vertical-align:top">
Сообщение с подробными сведениями о событии
...
| ApplicationName | Имя веб-сайта                                                                      |
| InstanceId      | Экземпляр веб-сайта, на котором произошло событие                                  |
...
> [WACOM.ПРИМЕЧАНИЕ] В первой строке журнала содержатся заголовки столбцов, представленных в данном примере.

### Трассировка невыполненных запросов

Трассировка невыполненных запросов хранится в XML-файлах с именами **fr\#\#\#\#\#\#.xml**. Чтобы облегчить просмотр данных журнала, используется таблица стилей XSL с именем **freb.xsl**, расположенная в том же каталоге, что и XML-файлы. При открытии одного из XML-файлов в Internet Explorer будет использоваться таблица стилей XSL для форматированного отображения информации о трассировке. Экран будет выглядеть следующим образом:

![вид трассировки неудачно завершенных запросов в браузере][вид трассировки неудачно завершенных запросов в браузере]
...
Журналы веб-сервера форматируются с помощью [расширенного формата журнала W3C][расширенного формата файлов журналов W3C]. Эти данные могут быть прочитаны с помощью текстового редактора, либо можно провести их синтаксический анализ с помощью служебных программ, таких как [Log Parser][Log Parser]

> [WACOM.ПРИМЕЧАНИЕ] Журналы, созданные веб-сайтами Azure, не поддерживают поля **s-computername**, **s-ip** или **cs-version**.
...
-   [Учебник — устранение неполадок веб-сайтов][Учебник — устранение неполадок веб-сайтов]
...
-   [Анализ журналов веб-сайта в HDInsight][Анализ журналов веб-сайта в HDInsight]
  ...
  [расширенного формата файлов журналов W3C]: http://msdn.microsoft.com/library/windows/desktop/aa814385.aspx
  [System.Diagnostics.Trace]: http://msdn.microsoft.com/ru-ru/library/36hhw2t6.aspx
  [1]: http://www.windowsazure.com/ru-ru/develop/net/tutorials/troubleshoot-web-sites-in-visual-studio/
  [портале управления Azure]: https://manage.microsoft.com
  [пакетов SDK для Azure]: http://www.windowsazure.com/ru-ru/downloads/#
  [Использование Azure PowerShell]: http://www.windowsazure.com/ru-ru/develop/nodejs/how-to-guides/powershell-cmdlets/
  [Использование инструментов командной строки Azure PowerShell]: http://www.windowsazure.com/ru-ru/develop/nodejs/how-to-guides/command-line-tools/
