...
При публикации проекта Service Fabric из Visual Studio в диалоговом окне **Опубликовать приложение Service Fabric** можно выбрать кластер Azure Service Fabric, нажав кнопку **Выбрать** в разделе **Конечная точка подключения**. Вы можете войти в учетную запись Azure и выбрать существующий кластер в своих подписках.
...
![Диалоговое окно **Select Service Fabric Cluster** (Выбор кластера Service Fabric) позволяет настроить имеющееся подключение к кластеру Service Fabric или создать новое.][selectsfcluster]
