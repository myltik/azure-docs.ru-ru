...
* **HTTP**: протокол HTTP — это стандартный вариант для обмена данными между системами на любых языках. Платформа Service Fabric поддерживает любые средства для HTTP и HTTP-серверы, доступные для множества языков. Службы могут использовать любой из существующих стеков HTTP, в том числе [веб-API ASP.NET](service-fabric-reliable-services-communication-webapi.md). Клиенты, написанные на языке C#, могут использовать классы [`ICommunicationClient` и `ServicePartitionClient`](service-fabric-reliable-services-communication.md) для разрешения адресов служб, обработки подключений HTTP и циклов повтора.
* **WCF**: если у вас есть существующий код, который использует WCF как среду обмена данными, то вы можете использовать класс `WcfCommunicationListener` на стороне сервера и классы `WcfCommunicationClient` и `ServicePartitionClient` на стороне клиента. Дополнительные сведения см. в статье [Реализация стека связи на основе WCF](service-fabric-reliable-services-communication-wcf.md).
