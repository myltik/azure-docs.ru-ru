...
### Перемещение служб
В распределенной системе экземпляры службы, которые вы запускаете, могут со временем перемещаться с одного компьютера на другой. Это может происходить по разным причинам, включая балансировку ресурсов при использовании метрик нагрузки для балансировки ресурсов, обновление, отработку отказа, развертывание. Поэтому адреса конечных точек экземпляра службы могут изменяться. Для настройки взаимодействия с экземпляром службы должен быть выполнен следующий цикл. Эти сведения извлекаются у вас при использовании API обмена данными, представляемыми Service Fabric.

* **Разрешение**. Все именованные экземпляры службы в Service Fabric имеют уникальные универсальные коды ресурсов (URI) в качестве имен. Например, `"fabric:/MyApplication/MyService"` остается неизменным для именованной службы. Кроме того, каждый экземпляр именованной службы делает доступными конечные точки, которые могут меняться при перемещении экземпляров именованной службы. Это аналогично веб-сайтам, URL-адреса которых не меняются, даже когда могут меняться их IP-адреса. Подобно службе DNS в Интернете, которая разрешает URL-адреса веб-сайтов в IP-адреса, Service Fabric обладает системной службой, называемой службой именования, которая разрешает универсальные коды ресурса (URI) в конечные точки. Этот шаг подразумевает разрешение URI экземпляра служб до конечной точки.

* **Подключение**. После того как универсальный код ресурса (URI) именованной службы был разрешен в адрес конечной точки, выполняется попытка подключения к этой службе. Такое подключение может дать сбой, если адрес конечной точки изменился из-за перемещения службы, вызванного, например, неисправностью компьютера или балансировкой ресурсов.
...
* **WCF**. Если имеется уже существующий код, который использует WCF как среду обмена данными, то вы можете использовать класс `WcfCommunicationListener` на стороне сервера и классы `WcfCommunicationClient` и `ServicePartitionClient` на стороне клиента. Дополнительные сведения см. в статье [Реализация стека связи на основе WCF](service-fabric-reliable-services-communication-wcf.md).
