...
Вы сможете создавать кластеры на виртуальных машинах или компьютерах под управлением следующих операционных систем:

* Windows Server 2012 R2
* Windows Server 2016
* Linux

Дополнительные сведения о Windows Server см. в статье [Create an Azure Service Fabric cluster on-premises or in the cloud](service-fabric-cluster-creation-for-windows-server.md).

## Создание и настройка кластера
Service Fabric предоставляет установочный пакет, который можно скачать. После его скачивания необходимо внести изменения в файл конфигурации JSON, указав параметры кластера. Изменив параметры, запустите сценарий установки, который создает кластер на компьютерах, указанных в параметрах. Можно также воспользоваться сценарием для удаления кластера с набора компьютеров.

## Сравнение локальных и облачных развертываний
Процесс создания локального кластера Service Fabric аналогичен созданию кластера в любом облаке с набором виртуальных машин. Начальные действия по подготовке виртуальных машин выполняются поставщиком облачных служб или локальной средой, которую вы используете. После создания набора виртуальных машин с сетевым подключением между ними выполните действия по установке пакета Service Fabric, затем измените параметры кластера и запустите сценарии создания кластера и управления им, которые похожи друг на друга. Это гарантирует, что ваши знания и опыт эксплуатации и управления кластерами Service Fabric останутся актуальными при выборе новых сред размещения.
...
## Преимущества кластеров Service Fabric в Azure над автономными кластерами, Service Fabric в локальной среде
Запуск кластеров Service Fabric в Azure предоставляет несколько преимуществ по сравнению с локальной средой, поэтому, если у вас нет конкретных требований относительно расположения кластеров, рекомендуем запускать их в Azure. В Azure мы обеспечиваем интеграцию с другими функциями и службами Azure, которые делают эксплуатацию кластера и управление им проще и надежнее.
...
* **Автомасштабирование:** для кластеров в Azure мы предоставляем встроенную функциональность автомасштабирования, основанную на наборах масштабирования виртуальных машин. В локальной среде и других облачных средах вам потребуется реализовать собственную функцию автомасштабирования или выполнить масштабирование вручную с помощью API-интерфейсов, предоставляемых Service Fabric для масштабирования кластеров.
