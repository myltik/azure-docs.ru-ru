...
В созданном решении будет 2 проекта.
 ...
 + **HelloWorldApplication** Это проект *приложения*, содержащего *службы*. В нем также содержится манифест приложения, описывающий приложение, и ряд скриптов PowerShell для развертывания приложения.
...
`RunAsync()` выполняется в собственной **Задаче**. Обратите внимание, что во фрагменте кода выше мы переходим прямо в цикл **while**, поэтому планировать отдельную задачу для рабочей нагрузки не нужно. Отмена рабочей нагрузки является совместным действием, координированным предоставленным токеном отмены. Для продолжения работы система будет ожидать завершения задачи (успешного завершения, отмены или завершения вследствие ошибки). Когда система запрашивает отмену задачи, **важно** как можно быстрее подтвердить токен отмены, завершить все выполняющиеся операции и выйти из `RunAsync()`.
...
В том же приложении **HelloWorld** добавьте новую службу, щелкнув правой кнопкой мыши проект приложения и выбрав пункт **Новая служба Fabric Service**.
...
Откройте **HelloWorldStateful.cs** в *HelloWorldStateful*, содержащий следующий `RunAsync` метод:
...
В надежных коллекциях есть многие из тех же операций, которые есть в их аналогах `System.Collections.Generic` и `System.Collections.Concurrent`, в том числе LINQ. Однако операции в надежных коллекциях являются асинхронными. Это происходит потому, что операции записи в надежных коллекциях *реплицируются*, то есть отправляются другим репликам службы на разных узлах для обеспечения высокого уровня доступности.

Они также поддерживают *транзакционные* операции, которые позволяют поддерживать несколько надежных коллекций в одинаковом состоянии. Например, вы можете исключить рабочий элемент из надежной очереди, выполнить над ним какую-либо операцию и сохранить результат в надежном словаре, и это все в пределах одной транзакции. Такая операция называется атомарной, она предусматривает, что вся операция завершится успешно, либо совсем не завершится. Если ошибка возникнет после удаления элемента из очереди, но до сохранения результата, вся транзакция будет откатана, и элемент останется в очереди для обработки.
