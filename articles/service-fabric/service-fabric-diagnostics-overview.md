...
---
# <a name="monitoring-and-diagnostics-in-azure-service-fabric"></a>Мониторинг и диагностика в Azure Service Fabric

Мониторинг и диагностика критически важны для разработки, тестирования и успешного развертывания в производственной среде. Решения Service Fabric лучше всего работают при реализации средств мониторинга и диагностики с самого начала. Это обеспечивает бесперебойную работу служб в локальных средах разработки и кластеров в рабочей среде.
...
Средства мониторинга и диагностики — это важный компонент, который гарантирует надлежащую работу всех служб, что позволяет вам непрерывно и своевременно реагировать на рабочие ситуации с минимальными перерывами в работе служб. 

Общий рабочий процесс мониторинга и диагностики состоит из трех этапов. 
1. **Создание событий**. Сюда входят события (журналы, трассировки, пользовательские события) на уровне инфраструктуры (кластер) и приложения или службы. 
2. **Агрегирование событий**. Создаваемые события должны быть собраны и агрегированы, прежде чем они могут быть отображены. Это можно сделать с помощью таблиц хранилища, настроенных с помощью системы диагностики Windows Azure, или при создании конвейера EventFlow.
3. **Анализ**. События должны быть доступными для просмотра и использования в определенном формате, обеспечивающем возможность анализа и отображения. Вы можете использовать такие средства, как ApplicationInsights, Operations Management Suite, Kibana и другие.

Хотя доступны разные продукты с поддержкой трехэтапного процесса, многие клиенты часто предпочитают использовать отдельные инструменты для реализации каждого из этапов. Очень важно, чтобы они работали вместе, обеспечивая комплексное решение для мониторинга приложения.

В зависимости от потребностей вы можете расширить возможности диагностики и мониторинга, чтобы включить функции автоматического уведомления и устранения, которые часто предоставляются вместе с используемыми средствами анализа. 

## <a name="event-generation"></a>Создание событий

События, журналы и трассировки могут создаваться на уровне инфраструктуры (кластера, виртуальной машины или действий Service Fabric) или приложений (инструментарий, добавленный в приложения и службы, развернутые в кластере).

### <a name="infrastructure-monitoring-the-cluster"></a>Инфраструктура: мониторинг кластера

Хотя Service Fabric помогает поддерживать работу приложения во время сбоев инфраструктуры, важно понимать, где произошел сбой: в самом приложении или на уровне базовой инфраструктуры. Мониторинг инфраструктуры требуется еще и для планирования загрузки, позволяя вовремя добавлять или удалять элементы инфраструктуры. Важно осуществлять мониторинг и диагностику как для инфраструктуры, так и для приложения, которые вместе составляют развернутую службу Service Fabric. Инфраструктура может быть источником потенциальных проблем, даже если приложение доступно для клиентов. Чтобы проверить правильность работы кластера, Service Fabric настраивает пять стандартных каналов ведения журналов.

1. Операционный канал: высокоуровневые операции, выполняемые Service Fabric и кластером; содержит ожидаемые события для узла, новое развертываемое приложение или откат обновления SF и т. д. 
2. Канал пользовательских сведений: отчеты о работоспособности и решения о балансировке нагрузки.
3. События Reliable Services: определенные события модели программирования.
4. События надежных субъектов: определенные события модели программирования и счетчики производительности.
5. Журналы поддержки: системные журналы, созданные Service Fabric только для использования нами при оказании поддержки.

Настоятельно рекомендуется, чтобы при создании кластера была включена диагностика. Это можно сделать на портале, как показано ниже, или с использованием шаблона Azure Resource Manager, который включает средство диагностики. 

![Создание кластера на портале Azure: включение диагностики](./media/service-fabric-diagnostics-overview/azure-enable-diagnostics.png)

Как показано выше, есть также дополнительное поле для добавления ключа инструментирования Application Insights (AppInsights). Если вы решили использовать AppInsights для анализа событий (AppInsights — это одно из рекомендуемых решений), включите ключ инструментирования AppInsights (GUID) здесь.
...
### <a name="application-instrumenting-code-for-custom-events"></a>Приложение: инструментирование кода для пользовательских событий
...
## <a name="event-aggregation-and-collection"></a>Агрегирование и сбор событий
...
- События **ETW** передаются как часть операционного канала
...
Последней составляющей мониторинга является визуализация потока событий, создание отчетов о производительности службы и оповещение при обнаружении проблем. Есть много решений, которые можно использовать для реализации этого аспекта мониторинга. AppInsights и Operations Management Suite (OMS) могут создавать оповещения на основе потока событий. Для визуализации данных можно применить Microsoft Power BI или решения сторонних производителей, например [Kibana](https://www.elastic.co/products/kibana) или [Splunk](https://www.splunk.com/).

### <a name="appinsights"></a>AppInsights

AppInsights — это одно из средств, рекомендуемых для наблюдения за приложениями и службами. Обновленный пакет AI.SDK обрабатывает события Service Fabric, предоставляет средства визуализации данных и запросов (используя аналитику AppInsights), а также может создавать точные карты приложений, которые помогают отслеживать зависимости между процессами в приложении или кластере.

Настройте ресурс AppInsights, выполнив поиск по фразе "Application Insights" в Azure Marketplace. Создав ресурс, перейдите к *свойствам*, чтобы найти ключ инструментирования AppInsights (в виде идентификатора GUID). Способы его использования.
* Интеграция AppInsights для получения событий на уровне инфраструктуры из кластера Service Fabric непосредственно с использованием шаблона Azure Resource Manager или портала Azure при создании кластера (при условии включенной системы диагностики).
* Настройка EventFlow (eventFlowConfig.json) для вывода данных в Application Insights, как показано в разделе выше.

### <a name="oms"></a>OMS

OMS — это еще одно средство, рекомендуемое для диагностики и мониторинга кластеров Service Fabric. Решение Service Fabric можно добавить в любую рабочую область. Оно включает панель мониторинга для отображения разных типов событий Service Fabric. Рабочие области OMS также включают мощное средство для создания запросов к журналам в Log Analytics.

Чтобы настроить рабочую область OMS, для кластера необходимо включить диагностику. Добавьте решение "Аналитика Service Fabric" из Azure Marketplace в существующую рабочую область OMS или создайте новую. Настройте источники данных рабочей области, чтобы подключиться к таблицам службы хранилища Azure, в которые кластер записывает события. 

Чтобы обеспечить возможность регистрации пользовательских событий в OMS, добавленный в приложения инструментарий должен поддерживать запись в те же таблицы службы хранилища (или любые другие, настроенные в качестве источников для рабочей области). 

Кроме того, OMS — это рекомендуемый способ анализа и визуализации данных при мониторинге и диагностике контейнеров, так как вы можете добавить решение "Контейнеры" в рабочую область, которая поддерживает оркестрированные контейнеры Service Fabric. Краткое руководство по настройке см. [здесь](service-fabric-diagnostics-containers-windowsserver.md).
