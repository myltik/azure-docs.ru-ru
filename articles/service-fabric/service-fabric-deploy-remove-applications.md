<properties
   pageTitle="Развертывание приложения Service Fabric"
...
После [создания пакета приложения][10] его можно развернуть в кластере Service Fabric. Развертывание включает следующие три шага:
...
Загрузка пакета приложения означает, что он помещается в расположение, доступное внутренним компонентам Service Fabric и к нему можно применить сценарии PowerShell. Перед выполнением команд PowerShell, описанных в этой статье, нужно подключиться к кластеру Service Fabric с помощью команды **Connect-ServiceFabricCluster**.

Предположим, у вас есть папка с именем *MyApplicationType*, содержащая манифест приложения, манифесты службы и пакеты данных, конфигурации и кода. Для загрузки пакета выполните команду **Copy-ServiceFabricApplicationPackage**. Например:
...
Команда **Register-ServiceFabricApplicationType** возвращается только после того, как система успешно завершит копирование пакета приложения. Срок выполнения команды зависит от содержимого пакета приложения. При необходимости можно увеличить время ожидания, используя параметр **-TimeoutSec** (по умолчанию — 60 секунд).
...
Команда **Get-ServiceFabricApplication** выводит список всех созданных экземпляров приложения вместе с их общим состоянием.
...
Экземпляры приложения, которые больше не нужны, можно удалить без возможности восстановления, используя команду **Remove-ServiceFabricApplication**. При этом все службы, относящиеся к этому приложению, и все их состояния также будут удалены. Эта операция необратима, и вы не сможете восстановить состояние приложения.
...
В пакете разработки Service Fabric SDK уже предусмотрены все необходимые значения по умолчанию. Строка ImageStoreConnectionString для всех команд должна совпадать со значением, которое используется кластером Service Fabric. Его можно получить из манифеста кластера с помощью команды **Get-ServiceFabricClusterManifest**:
