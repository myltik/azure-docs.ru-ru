...
```
   > [!NOTE]
   > Режим изоляции Hyper-v доступен для номеров SKU Azure Ev3 и Dv3 с поддержкой вложенной виртуализации. Убедитесь, что на узлах установлена роль Hyper-v. Проверьте это, подключившись к узлам.
   >
...
Пока кластер работает, с вас будет взиматься плата. Рекомендуем [удалить кластер](service-fabric-tutorial-create-vnet-and-windows-cluster.md#clean-up-resources).  [Сторонние кластеры](https://try.servicefabric.azure.com/) удаляются автоматически через несколько часов.
