...
Каждый из этих пакетов можно отдельно обновить, в том числе до определенной версии. Как и к облачным службам, к пакету конфигурации можно получить доступ программным путем через интерфейс API. Как и при работе с облачными службами, для уведомления службы об изменении пакета конфигурации используются события. С помощью файла Settings.xml можно настроить пару "ключ — значение" и программный доступ аналогично. Однако, в отличие от облачных служб, пакет конфигурации Service Fabric может содержать любые файлы конфигурации в любом формате, будь то XML, JSON, YAML или пользовательский двоичный формат.
