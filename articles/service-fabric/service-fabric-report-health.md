...
Буферизация на клиенте выполняется с учетом уникальности отчетов. Например, если какой-либо неисправный информатор отправляет 100 отчетов в секунду для одного свойства одной сущности, будет использоваться только последняя версия отчета. В очереди клиента существует только один такой отчет. Если настроена пакетная обработка, в хранилище данных о работоспособности за один интервал отправки будет отправляться только один отчет. Используется последний добавленный отчет, который отображает актуальное состояние сущности. Все параметры настройки можно указать при создании `FabricClient`. Для этого необходимо передать объект [FabricClientSettings](https://msdn.microsoft.com/library/azure/system.fabric.fabricclientsettings.aspx) с нужными значениями параметров, связанных с работоспособностью.
