...
### Настройка политики запуска от имени с помощью локальной учетной записи

Настроив службу для использования точки SetupEntryPoint, можно изменить разрешения безопасности, с которыми она выполняется в манифесте приложения. В приведенном ниже примере показано, как настроить службу для запуска с правами учетной записи администратора пользователей.
...
###  Настройка политики запуска от имени с помощью учетных записей локальной системы
Как правило, предпочтительным вариантом для выполнения сценариев запуска является учетная запись локальной системы, а не администраторов, как показано выше. Выполнение политики запуска от имени с учетными записями администраторов обычно не рекомендуется, так как по умолчанию включено управление доступом на уровне пользователей. В таких случаях **рекомендуется выполнять SetupEntryPoint с учетной записью локальной системы вместо записи локального пользователя, добавленной в группу администраторов**. В приведенном ниже примере показана настройка SetupEntryPoint для запуска с учетной записью LocalSystem.
...
**Примечание.** По умолчанию при запуске пакетного файла он выполняет поиск файлов в папке приложения с именем **work**. В этом случае нам нужно, чтобы при запуске MySetup.bat пакет нашел файл MySetup.ps1 в той же папке, в которой находится папка **пакета кода** приложения. Чтобы изменить эту папку, задайте рабочую папку, как показано ниже.
...
## С помощью политики перенаправления консоли для локальной отладки точек входа
Иногда бывает удобно просматривать вывод этой консоли, выполнив сценарий для отладки. Для этого можно задать политику перенаправления консоли, которая записывает выходные данные в файл. Выходной файл записывается в папку приложения с именем **log** на узле, где развертывается и выполняется приложение (сведения о том, как найти этот узел, см. в примере выше).

**Примечание. Ни в коем случае** не используйте политику перенаправления консоли в приложении, развернутом в рабочей среде, поскольку это может повлиять на отработку отказа приложения. Используйте этот способ **ТОЛЬКО** для локальной разработки и отладки.
...
Если теперь изменить файл MySetup.ps1 для записи команды **Echo**, запись будет выполняться в выходной файл для отладки.
...
С помощью раздела **DefaultRunAsPolicy** можно указать учетную запись пользователя по умолчанию для всех пакетов кода без конкретного определенного раздела **RunAsPolicy**. Иногда большую часть пакетов кода, указанных в используемых приложением манифестах служб, нужно запускать с применением одной и той же учетной записи пользователя с функцией запуска от имени. Для этого в приложении можно определить стандартную политику запуска от имени с использованием этой учетной записи пользователя, а не указывать **RunAsPolicy** для каждого пакета кода. Следующий пример демонстрирует: если в пакете кода не указано **RunAsPolicy**, такой пакет кода должен запускаться под учетной записью **MyDefaultAccount**, указанной в разделе Principals.
