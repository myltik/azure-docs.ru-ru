<properties
   pageTitle="Управление состоянием субъектов Reliable Actors | Microsoft Azure"
...
Также получить состояние можно, используя метод *TryGet*, который не вызывает исключение, если запись не существует для указанного ключа:
...
Состояние можно удалить без возможности восстановления из диспетчера состояний субъекта, вызвав метод *Remove*. Этот метод вызывает исключение `KeyNotFoundException` при попытке удалить ключ, который не существует:
...
Состояние также можно окончательно удалить с помощью метода *TryRemove*, который не вызывает исключение при попытке удалить несуществующий ключ:
