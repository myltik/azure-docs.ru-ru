...
В Azure Service Fabric можно запустить приложение любого типа, в том числе приложения Node.js или Java, а также собственные приложения. В Service Fabric такие приложения называются гостевыми исполняемыми файлами. Гостевые исполняемые файлы обрабатываются в Service Fabric как службы без отслеживания состояния. Это значит, что они будут размещены на узлах кластера на основе доступности и других метрик. В этой статье объясняется, как упаковать и развернуть гостевой исполняемый файл в кластере Service Fabric.
...
Процесс упаковки гостевого исполняемого файла включает следующие этапы.
