...
В Azure Service Fabric можно запустить существующее приложение любого типа, будь то приложение Node.js, Java или собственное приложение. Service Fabric обрабатывает эти приложения как службы без отслеживания состояния и размещает их на узлах в кластере, исходя из доступности и других метрик узлов. В этой статье описывается упаковка и развертывание существующего приложения в кластере Service Fabric.

## Преимущества выполнения настраиваемого приложения в Service Fabric

Использование приложения в кластере Service Fabric имеет несколько преимуществ.

- Высокая доступность. Приложения, работающие в среде Service Fabric, отличаются высокой доступностью без какой-либо дополнительной настройки. Service Fabric гарантирует постоянную доступность одного экземпляра приложения.
...
Перед изучением подробных сведений о развертывании существующего приложения рекомендуем ознакомиться с моделью развертывания и упаковки Service Fabric. Модель развертывания и упаковки Service Fabric основана преимущественно на двух файлах.


* **Манифест приложения.**

  Этот манифест используется для описания приложения. В файле указаны службы, из которых состоит приложение, а также параметры, которые определяют развертывание служб (например, количество экземпляров).
...
* **Манифест службы.**
  ...
  Мы не будем подробно рассматривать все параметры, которые доступны в манифесте службы. Остановимся только на тех параметрах, которые отвечают за запуск существующего приложения на платформе Service Fabric.
...
В журналах консоли можно отслеживать работоспособность существующих приложений. В частности, из этих журналов можно узнать, не возвращают ли сценарии приложения и конфигурации какие-либо ошибки. Перенаправление консоли можно настроить в файле `ServiceManifest.xml` в элементе `ConsoleRedirection`.
...
## Дальнейшие действия
В этой статье вы ознакомились с основной процедурой упаковки существующего приложения и его развертывания в Service Fabric. Далее можно ознакомиться с дополнительными материалами по этой теме.

- [Пример для упаковки и развертывания настраиваемого приложения на GitHub](https://github.com/Azure-Samples/service-fabric-dotnet-getting-started/tree/master/Custom/SimpleApplication), включая ссылку на предварительную версию упаковочного средства.
