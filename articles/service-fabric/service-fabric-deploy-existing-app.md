...
Следующий этап — редактирование файла манифеста службы с добавлением следующих сведения: * Имя типа службы. Это идентификатор, который Service Fabric использует, чтобы определять службу * Команда, используемая для запуска приложения (ExeHost) * Любой сценарий, который необходимо выполнить для установки или настройки приложения (SetupEntrypoint
...
* `WorkingFolder`: задает рабочий каталог процесса, который будет запущен. Вы можете задать два значения:
	...
	* `CodePackage`: рабочим каталогом будет корневая папка пакета приложения (`MyServicePkg`). Элемент `WorkingDirectory` позволяет задать правильный рабочий каталог, чтобы приложение или сценарии инициализации могли использовать относительные пути. Для элемента `WorkingFolder` (`Work`) можно задать еще одно значение, но оно не очень полезно при упаковывании существующего приложения.
...
* Элемент `ServiceTypeName` используется как идентификатор службы в контексте портирования существующего приложения. `ServiceTypeName` должен содержать уникальные удостоверения службы.
