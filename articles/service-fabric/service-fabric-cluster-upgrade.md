...
Основные или дополнительные сертификаты можно без труда обновить с портала Azure (как показано ниже) или выполнив команду PUT для ресурса servicefabric.cluster.

![Снимок экрана, демонстрирующий отпечатки сертификатов на портале Azure.][CertificateUpgrade]

>[AZURE.NOTE] Перед идентификацией сертификата, который вы хотите использовать для ресурсов кластера, необходимо выполнить описанные ниже действия, иначе новый сертификат не будет использоваться.
1. Отправьте новый сертификат в хранилище ключей Azure. Инструкции см. в разделе [Безопасность Service Fabric](service-fabric-cluster-security.md). Начните с шага 2 в этом документе.
2. Обновите все виртуальные машины в составе кластера, чтобы развернуть на них сертификат. Для этого обратитесь к [блогу группы разработчиков хранилища ключей Azure](http://blogs.technet.com/b/kv/archive/2015/07/14/vm_2d00_certificates.aspx).
...
>[AZURE.NOTE] Дополнительные сведения об использовании ограничений на размещение, свойствах узла и способах их определения см. в разделе "Placement Constraints and Node Properties" в документе по диспетчеру кластерных ресурсов Service Fabric на странице [Describing Your Cluster](service-fabric-cluster-resource-manager-cluster-description.md).

### Метрики емкости

Для каждого типа узла можно добавить настраиваемые метрики емкости, которые будут использоваться в приложениях для передачи данных о нагрузке. Дополнительные сведения об использовании метрик емкости для сообщения о нагрузке см. в документах по диспетчеру кластерных ресурсов Service Fabric на страницах [Describing Your Cluster](service-fabric-cluster-resource-manager-cluster-description.md) и [Metrics and Load](service-fabric-cluster-resource-manager-metrics.md).
