...
- У служб (составляющих приложение) различные требования к инфраструктуре, например больше ОЗУ или тактов центрального процессора? Предположим, что приложение, которое вам необходимо развернуть, содержит интерфейсную и внутреннюю службы. Интерфейсная служба может выполняться на виртуальных машинах меньшего размера (например, D2) с портами, открытыми для доступа через Интернет. Однако внутренняя служба, которая будет выполнять вычисления, должна размещаться на более крупных виртуальных машинах (D4, D6, D15 и т. п.) без доступа через Интернет.
...
### Тип вторичного узла
У кластера с несколькими типами узлов будет один тип первичного узла, а остальные будут типами вторичных узлов. Ниже приведены характеристики типа вторичного узла.
...
- Silver: задания инфраструктуры могут быть приостановлены в течение 30 минут для UD.

- Bronze: привилегии отсутствуют.
...
Обратите внимание, что выбранный уровень надежности определяет минимальное количество узлов для типа первичного узла. Уровень никак не влияет на максимальный размер кластера. Поэтому можно иметь кластер с 20 узлами с уровнем надежности Bronze.
