...
description: "Обзор и руководство по началу работы с Azure Service Fabric. Ознакомьтесь с Service Fabric и узнайте, как создать масштабируемые, надежные и легко управляемые приложения, которые состоят из микрослужб."
...
Это руководство по началу работы содержит краткий обзор Service Fabric, описание основных понятий и терминологии, а также общие сведения о каждой области этой платформы.  Данная статья не содержит полный список связанной документации, но предоставляет ссылки на руководства по началу работы для каждой области Service Fabric. 

## <a name="the-five-minute-overview"></a>Пятиминутный обзор
Azure Service Fabric — это платформа распределенных систем, которая дает возможность не только легко упаковывать и развертывать масштабируемые и надежные микрослужбы, но и управлять ими.  Эта платформа позволяет разрешить значительные трудности, возникающие при разработке облачных приложений и управлении ими. Использование Service Fabric позволяет разработчикам и администраторам избежать многих проблем сложной инфраструктуры.  Вместо этого у вас есть возможность сфокусироваться на реализации критически важных ресурсоемких рабочих нагрузок, учитывая, что они являются масштабируемыми, надежными и легко управляемыми. Service Fabric — это принципиально новая платформа промежуточного слоя, позволяющая создавать облачные приложения корпоративного класса уровня 1 и управлять ими.  
...
Service Fabric предоставляет пакеты SDK для создания служб в среде Linux с помощью .NET Core и Java. Ознакомьтесь со статьями о создании первого приложения Java или C# в Linux и его запуске на компьютере разработчика: [Подготовка среды разработки в Linux](service-fabric-get-started-linux.md), [Создание первого приложения Azure Service Fabric](service-fabric-create-your-first-linux-application-with-java.md) для Java и [Создание первого приложения Azure Service Fabric](service-fabric-create-your-first-linux-application-with-csharp.md) для C#.
...
Приложения Service Fabric можно создавать в MacOS X для запуска на кластерах Linux. В статьях [Настройка среды разработки для Mac OS X](service-fabric-get-started-mac.md) и [Создание первого приложения Azure Service Fabric](service-fabric-create-your-first-linux-application-with-java.md) описывается, как настроить среду Mac для разработки, и приводится процесс создания первого приложения Java в MacOS с последующим запуском на виртуальной машине Ubuntu.
...
Существуют два типа службы: с отслеживанием состояния и без него.  Службы без отслеживания состояния сохраняют устойчивое состояние во внешней службе хранилища, например в службе хранилища Azure, базе данных SQL Azure или Azure DocumentDB. Службу без отслеживания состояния следует использовать, когда у службы нет постоянного хранилища. Служба с отслеживанием состояния используется, когда нужно, чтобы платформа Service Fabric управляла состоянием службы через модели программирования Reliable Collections или Reliable Actors.  
...
Service Fabric предлагает несколько способов записи и управления службами. Службы могут использовать API-интерфейсы платформы Service Fabric, чтобы в полной мере использовать компоненты платформы и платформы приложений.  С другой стороны, службы могут представлять любую скомпилированную исполняемую программу, написанную на любом языке и размещенную в кластере Service Fabric. Дополнительные сведения см. в разделе [Поддерживаемые модели программирования](service-fabric-choose-framework.md).

### <a name="guest-executables"></a>Гостевые исполняемые файлы
[Гостевой исполняемый файл](service-fabric-deploy-existing-app.md) — это произвольный существующий исполняемый файл (написанный на любом языке), который размещен в кластере Service Fabric среди других служб. Однако гостевые исполняемые файлы не интегрируются с интерфейсами API Service Fabric напрямую.  Они не задействуют весь набор предлагаемых платформой функций, таких как настраиваемые отчеты о работоспособности и нагрузке, регистрацию конечных точек служб и вычисления с отслеживанием состояния.
...
## <a name="test-apps-and-services"></a>Тестовые приложения и службы
Чтобы создать службы именно в масштабах облака, очень важно, чтобы приложения и службы были устойчивы к реальным сбоям.  Служба анализа сбоев предназначена для проверки служб, созданных на платформе Service Fabric. [Она](service-fabric-testability-overview.md) позволяет вызывать значимые ошибки и запускать в приложениях тестовые сценарии. Вызываемые ошибки и сценарии позволяют воспроизвести и проверить в контролируемых, безопасных и согласованных условиях разные состояния и переходы, происходящие со службой в течение ее жизненного цикла.
