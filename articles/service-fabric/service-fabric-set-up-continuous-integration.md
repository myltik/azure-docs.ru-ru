...
### Создание сертификата и его передача в новый экземпляр хранилища ключей Azure

>[AZURE.NOTE] В этом примере скрипт создает самозаверяющий сертификат. Так как это небезопасно, это можно делать только для эксперимента. Вместо этого следуйте рекомендациям по получению законного сертификата своей организации.
...
3.	Предоставьте учетной записи локальной службы разрешение на использование сертификата службы автоматизации.
    ...
    г) Нажмите кнопку **Добавить**, а затем введите имя в поле **Локальная служба** и выберите **Проверить имена**.
...
1.	Скачайте архив agent.zip. Для этого:
    ...
    >[AZURE.NOTE] Если вы планируете использовать веб-службы ASP.NET 5, для этой папки лучше выбрать самое короткое имя, чтобы во время развертывания не возникали ошибки **PathTooLongExceptions**.
...
>[AZURE.NOTE] Между этим шагом и предыдущим шагом "Удаление группы ресурсов кластера" существует одного различие: на этом шаге должен быть установлен флажок **Всегда запускать**.
