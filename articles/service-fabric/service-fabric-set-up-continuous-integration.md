...
### Создание определения на основе шаблона сборки приложения Service Fabric
...
4.	В появившемся диалоговом окне в категории шаблонов **Сборка** выберите **Приложение Service Fabric**.
5.	Щелкните **Далее**.
6.	Выберите репозиторий и ветвь, связанные с приложением Service Fabric.
7.	Установите флажок **Непрерывная интеграция**, чтобы эта сборка запускалась при каждом обновлении ветви.
...
11. Ниже приводится описание шагов сборки, созданных шаблоном.

| Шаг сборки | Описание |
| --- | --- |
| Восстановление NuGet | Восстановление пакетов NuGet для решения. |
| Выполнение сборки решения | Выполнение сборки всего решения. |
| Пакет приложения | Создание пакета приложения Service Fabric, который будет использоваться для развертывания приложения. Обратите внимание, что расположение пакета приложения указано в каталоге артефактов сборки. |
...
| Копирование артефактов проекта | Копирование файлов профиля публикации и параметров приложения в артефакты сборки для использования для развертывания. |
...
### Создание определения на основе шаблона выпуска приложения Service Fabric
...
4.	В появившемся диалоговом окне в категории шаблонов **Развертывание** выберите **Приложение Service Fabric**.
5.	Щелкните **Далее**.
...
11.	В поле ввода **Cluster Connection** (Подключение к кластеру) задачи **Deploy Service Fabric Application** (Развертывание приложения Service Fabric) выберите кластер, на котором должно быть развернуто приложение. Подключение к кластеру предоставляет необходимую информацию, позволяющую задаче развертывания подключиться к кластеру. Если у вас еще нет подключения к кластеру, щелкните гиперссылку **Управление** рядом с полем, чтобы добавить подключение. На открывшейся странице выполните следующие действия.
...
1.	Проверьте поля ввода **профиля публикации** для задачи **Deploy Service Fabric Application** (Развертывание приложения Service Fabric). По умолчанию это поле ссылается на профиль публикации Cloud.xml, содержащийся в каталоге артефактов сборки. Если требуется ссылка на другой профиль публикации или если сборка содержит несколько пакетов приложения в каталоге артефактов, то необходимо соответствующим образом обновить путь.
2.	Проверьте поля ввода **пакета приложения** для задачи **Deploy Service Fabric Application** (Развертывание приложения Service Fabric). По умолчанию указывается путь к пакету приложения по умолчанию, используемый в шаблоне определения сборки. Если вы изменили путь к пакету приложения по умолчанию в определении сборки, то необходимо соответствующим образом обновить и указанный здесь путь.
