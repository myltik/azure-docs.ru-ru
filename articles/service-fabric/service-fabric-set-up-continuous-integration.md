...
2.	На вкладке **Переменные** создайте переменные с такими значениями:
    ...
    |BuildPlatform|x64|||
    |ServicePrincipalPassword|Пароль, который использовался при создании субъекта-службы.|X||
    |ServicePrincipalId|Из выходных данных сценария, который вы использовали для создания субъекта-службы.|||
    |ServicePrincipalTenantId|Из выходных данных сценария, который вы использовали для создания субъекта-службы.|||
    |ServicePrincipalSubscriptionId.|Из выходных данных сценария, который вы использовали для создания субъекта-службы.|||
    |ServiceFabricCertificateThumbprint|Из выходных данных CreateAndUpload Certificate.ps1|||
    |ServiceFabricKeyVaultId|Из выходных данных CreateAndUpload Certificate.ps1|||
    |ServiceFabricCertificateSecretId|Из выходных данных CreateAndUpload Certificate.ps1|||
    |ServiceFabricClusterName|Должно соответствовать DNS-имени сертификата.|||
    |ServiceFabricClusterResourceGroupName|Любое имя.|||
    |ServiceFabricClusterLocation|Должно соответствовать расположению хранилища ключей.|||
    |ServiceFabricClusterAdminPassword|Символы 8–123 с по крайней мере 3 из следующих типов символов: прописные буквы, строчные буквы, цифры, специальные символы.|X||
    |ServiceFabricClusterResourceGroupTemplateFilePath|`<path/to/extracted/automation/scripts/ArmTemplate-Full-3xVM-Secure.json>`|||
    |ServiceFabricPublishProfilePath|`<path/to/your/publish/profiles/MyPublishProfile.xml>` Конечная точка подключения в профиле публикации будет игнорироваться. Вместо нее будет использоваться конечная точка подключения временного кластера.|||
    |ServiceFabricDeploymentScriptPath|`<path/to/Deploy-FabricApplication.ps1>`|||
    |ServiceFabricApplicationProjectPath|`<path/to/your/fabric/application/project/folder>` Это должен быть путь к папке, содержащей SFPROJ-файл.||||
...
4.	Нажмите кнопку **…** рядом с полем **Решение**, а затем выберите SLN-файл.
...
4.	Нажмите кнопку **…** рядом с полем **Решение**, а затем выберите SFPROJ-файл проекта приложения.
...
4.	Выберите команду **…** рядом с полем **Имя файла скрипта**. Перейдите в расположение, в которое были извлечены скрипты службы автоматизации, а затем выберите **Remove-ClusterResourceGroup.ps1**.
...
3.	Щелкните значок карандаша возле имени шага сборки, а затем переименуйте шаг в **Подготовка безопасного кластера и развертывание в нем**.

4.	Нажмите кнопку **…** рядом с полем **Имя файла скрипта**. Перейдите в расположение, в которое были извлечены скрипты службы автоматизации, а затем выберите **ProvisionAndDeploy-SecureCluster.ps1**.
...
>[AZURE.NOTE] Между этим шагом и предыдущим шагом "Удаление группы ресурсов кластера" существует одно различие: на этом шаге должен быть установлен флажок **Всегда запускать**.
...
4.	Нажмите кнопку **…** рядом с полем **Имя файла скрипта**. Перейдите в расположение, в которое были извлечены скрипты службы автоматизации, а затем выберите **RemoveClusterResourceGroup.ps1**.
...
1.	Создайте тестовый кластер вручную, используя портал Azure или Azure PowerShell. При этом можно использовать скрипт `ProvisionAndDeploy-SecureCluster.ps1` в качестве примера.
...
3.	Замените шаг **Подготовка безопасного кластера и развертывание в нем** шагом, на котором Deploy-FabricApplication.ps1 вызывается напрямую (и передается в профиль публикации).
