...
<a id="getsupport"></a>
## Получение поддержки для изолированного пакета Service Fabric

- Спросите у сообщества об изолированном пакете Service Fabric для Windows Server на [форуме Azure Service Fabric.](https://social.msdn.microsoft.com/Forums/azure/ru-RU/home?forum=AzureServiceFabric?)

- Откройте запрос на [техническую поддержку для Service Fabric](http://support.microsoft.com/oas/default.aspx?prid=16146). Дополнительные сведения о [технической поддержке корпорации Майкрософт](https://support.microsoft.com/ru-RU/gp/offerprophone?wa=wsignin1.0).
...
|EULA\_ENU.txt|Условия лицензии на использование изолированного пакета Windows Server Microsoft Azure Service Fabric. [Щелкните здесь](http://go.microsoft.com/fwlink/?LinkID=733084), если хотите скачать копию лицензионного соглашения.|
...
|ThirdPartyNotice.rtf |Примечание о включенном в пакет программном обеспечении стороннего производителя.|
...
|CleanFabric.ps1|Сценарий PowerShell для удаления изолированного пакета Service Fabric с текущего компьютера. Предыдущие установки MSI должны быть удалены с помощью собственной связанной программы удаления.|
...
- Рекомендуется наличие не менее 4 ядер ЦП
...
- Пакет Service Fabric нельзя установить на контроллере домена
...
### Шаг 5. Выполнение сценария TestConfiguration

Этот сценарий проверяет инфраструктуру в соответствии с указаниями в cluster.JSON, чтобы проверить наличие необходимых разрешений, подключение компьютеров друг к другу и прочие условия, требуемые для успешного развертывания. По сути, это мини-анализатор соответствия рекомендациям. Со временем мы расширим возможности этого средства дополнительными проверками для повышения надежности.
...
### Шаг 3. Выполнение сценария создания кластера
...
```
### Шаг 5. Вызов Service Fabric Explorer
...
1.            количество служб;
1.            количество типов служб;
1.            количество приложений;
1.            количество обновлений приложений;
1.            количество единиц с отработкой отказа;
1.            количество единиц со встроенной отработкой отказа;
1.            количество единиц с неработоспособной отработкой отказа;
1.            количество реплик;
1.            количество встроенных реплик;
1.            количество реплик в режиме ожидания;
1.            количество автономных реплик;
1.            длина общей очереди;
1.            длина очереди запросов;
1.            длина очереди единиц с отработкой отказа;
1.            длина очереди фиксаций;
1.            количество узлов;
1.            выполнен ли контекст: значение True или False;
1.            идентификатор кластера — это глобальный уникальный идентификатор, который случайным образом формируется для каждого кластера;
1.            версия Service Fabric;
1.             IP-адрес виртуальной машины или компьютера, с которых отправляются данные телеметрии.
