...
* Если из-за ошибок в службе происходит повреждение данных. Это может случится, если при обновлении службы в коллекцию записаны поврежденные данные, в связи с чем требуется вернуть прежнее состояние службы и данных.
...
>[AZURE.NOTE]В любой момент времени в обрабатываемой реплике можно выполнить только один вызов метода **BackupAsync**. Выполнение нескольких вызовов метода **BackupAsync** одновременно приведет к вызову исключения **FabricBackupInProgressException**, которое ограничит создание резервных копий до одной за раз.[AZURE.NOTE]В случае сбоя реплики при резервном копировании операция не будет завершена. Таким образом, после завершения обработки отказа служба должна перезапустить резервное копирование, при необходимости вызвав метод **BackupAsync**.

### Как восстанавливать данные

Сценарии восстановления, в которых запущенная служба должна восстановить данные из резервного хранилища, можно классифицировать следующим образом:

1. Потеря данных в разделе службы. К примеру, диск для двух из трех реплик для раздела (включая первичную реплику) поврежден или очищен. Необходимо восстановить данные для нового источника.
...
Чтобы восстановить данные, создателю службы необходимо настроить следующие шаги: переопределить **IReliableStateManager**, чтобы вернуть новый ReliableStateManager и предоставить функцию обратного вызова на случай потери данных; найти последнюю резервную копию во внешнем расположении, в котором содержатся резервные копии службы; скачать последнюю резервную копию (и распаковать резервную копию в папку резервного копирования, если она была сжата); вызвать метод **IReliableStateManager.RestoreAsync** с путем к папке резервного копирования; вернуть значения true, если восстановление выполнено успешно.
...
Для этого нужно развернуть новый кластер Service Fabric и восстановить резервные копии поврежденных разделов, как описано выше в сценарии с удаленной службой. После обнаружения последней резервной копии, выполненной до повреждения данных, нужно переместить или удалить все остальные резервные копии этого раздела. Теперь при вызове метода **OnDataLossAsync** для раздела в рабочем кластере будет обнаружена необходимая последняя резервная копия во внешнем хранилище.
...
>[AZURE.NOTE]Каждый раз при восстановлении есть вероятность того, что состояние восстанавливаемой резервной копии позже состояния раздела до потери данных. Из-за этого восстановление можно использовать только в качестве последнего средства для восстановления максимального объема данных.

>[AZURE.NOTE]Строка, представляющая собой путь к папке резервного копирования, так же как и пути к файлам в этой папке, может содержать больше 255 символов в зависимости от пути FabricDataRoot и длины имени типа приложения. Из-за этого такие методы .NET, как **Directory.Move**, могут выдать исключение **PathTooLongException**. Пока один метод устраняет проблему, другой может непосредственно вызвать интерфейс API kernel32, такой как **CopyFile**.
...
Диспетчер надежных состояний позволяет выполнять восстановление из резервной копии с помощью интерфейса API IReliableStateManager.RestoreAsync. Метод RestoreAsync можно вызвать только внутри метода OnDataLossAsync. Возвращенное методом OnDataLossAsync логическое значение bool указывает, удалось ли восстановить состояние службы из внешнего источника. Если метод OnDataLossAsync возвращает значение true, Service Fabric перестраивает все реплики из этого источника. Реплики, получившие вызов OnDataLossAsync, сначала получат роль первичных, но им не будет присвоено состояние чтения или записи. Это означает, что для исполнителей StatefulService метод RunAsync будет вызван только после успешного завершения метода OnDataLossAsync. Интерфейс API будет постоянно вызываться, пока служба успешно не завершит его (возвратив значение true или false) и не выполнит соответствующую перенастройку.
