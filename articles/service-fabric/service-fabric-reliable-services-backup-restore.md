<properties
   ...
   description="Основная документация по резервному копированию и восстановлению Reliable Services платформы Service Fabric"
...
* В случае полной утери всего кластера Service Fabric или всех узлов, запущенных в указанном разделе (т. е. всех узлов в кластере). Это может произойти, если георепликация не выполнена (о чем свидетельствует состояние) и весь центр обработки данных, в котором находится кластер, вышел из строя.
...
- Переопределить **CreateReliableStateManager**, чтобы вернуть новый **ReliableStateManager** и предоставить функцию обратного вызова на случай потери данных.

- Найти последнюю резервную копию во внешнем расположении, в котором содержатся резервные копии службы.

- Вернуть значение false, если состояние последней резервной копии отстает от состояния новой первичной реплики. Это гарантирует, что новая первичная реплика не будет перезаписана старыми данными.

- Скачать последнюю резервную копию (и распаковать ее в папку резервного копирования, если она была сжата).

- Вызвать **IReliableStateManager.RestoreAsync** с путем к папке резервного копирования.
...
Чтобы восстановить резервную копию службы до состояния, в котором она находилась до повреждения из-за ошибки в коде, можно использовать действия, описанные в разделе «Удаленная или утерянная служба».
