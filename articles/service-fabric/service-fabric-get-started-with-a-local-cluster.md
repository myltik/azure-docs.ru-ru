---
title: "Начало развертывания и обновления приложений в локальном кластере | Документация Майкрософт"
...
## <a name="1-node-and-5-node-cluster-mode"></a>Режим кластера с 1 и 5 узлами
При работе с локальным кластером для разработки приложений часто бывает необходимо быстро написать, изменить код или выполнить его отладку. Чтобы оптимизировать этот процесс, можно выбрать подходящий режим работы локального кластера: с 1 узлом или с 5 узлами. Каждый из этих режимов имеет свои преимущества.
В режиме кластера с 5 узлами вы можете работать с реальным кластером. Вы можете тестировать сценарии отработки отказа и работать с несколькими экземплярами и репликами служб.
Режим кластера с 1 узлом позволяет быстро развернуть и зарегистрировать службы, чтобы проверить код в среде выполнения Service Fabric.
...
> [!NOTE]
> Эта функция доступна в пакете SDK версии 5.2 и более поздних.
> 
> 
