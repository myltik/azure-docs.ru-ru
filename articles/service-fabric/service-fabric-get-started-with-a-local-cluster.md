...
3. Наконец, просмотрите список разделов для WordCountService:
    ...
    > [AZURE.NOTE] Дополнительные сведения о Service Fabric Explorer см. в статье [Визуализация кластера с помощью обозревателя Service Fabric](service-fabric-visualizing-your-cluster.md).
...
1. [Скачайте пакет WordCount v2](http://aka.ms/servicefabric-wordcountappv2) в то же расположение, куда был скачан пакет v1.
...
## Режим кластера с 1 и 5 узлами

При работе с локальным кластером для разработки приложений часто бывает необходимо быстро написать, изменить код или выполнить его отладку. Чтобы оптимизировать этот процесс, можно выбрать подходящий режим работы локального кластера: с 1 узлом или с 5 узлами. Каждый из этих режимов имеет свои преимущества. В режиме кластера с 5 узлами вы можете работать с реальным кластером. Вы можете тестировать сценарии отработки отказа и работать с несколькими экземплярами и репликами служб. Режим кластера с 1 узлом позволяет быстро развернуть и зарегистрировать службы, чтобы проверить код в среде выполнения Service Fabric.

Режимы кластера с 1 и 5 узлами не являются ни эмуляторами, ни имитаторами. Он выполняет тот же код платформы, что и кластеры, состоящие из нескольких компьютеров.

> [AZURE.NOTE] Эта функция доступна в пакете SDK версии 5.2 и более поздних.

Чтобы переключиться в режим кластера с 1 узлом, используйте диспетчер локального кластера или PowerShell следующим образом:
...
Если используется диспетчер локального кластера Service Fabric:

![Переключение режима кластера][switch-cluster-mode]

> [AZURE.WARNING] При изменении режима текущий кластер удаляется из системы и создается новый. Данные, хранящиеся в кластере, также будут удалены.
