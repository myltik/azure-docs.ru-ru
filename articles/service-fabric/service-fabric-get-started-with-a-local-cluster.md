...
Кластер Service Fabric представляет собой набор аппаратных ресурсов, на которых можно развернуть приложение. Как правило, в состав кластера входит от пяти до нескольких тысяч компьютеров. Но пакет SDK Service Fabric включает в себя конфигурацию кластера, которая может работать на одном компьютере.

> [AZURE.NOTE]Локальный кластер Service Fabric не является ни эмулятором, ни имитатором. Он выполняет тот же код платформы, что и кластеры, состоящие из нескольких компьютеров. Он отличается только тем, что выполняет на одном компьютере процессы платформы, которые обычно распределены между пятью компьютерами.
...
2. Запустите сценарий установки кластера из папки пакета SDK:
  ...
  Установка кластера займет несколько минут. Затем вы увидите результат, который выглядит приблизительно так:
...
Пакет SDK Service Fabric предлагает широкий выбор платформ и средств разработки для создания приложений. Если вы хотите научиться создавать приложения в Visual Studio, см. статью [Создание первого приложения в Visual Studio](service-fabric-create-your-first-application-in-visual-studio.md). В этом руководстве мы будем использовать существующий пример приложения (которое называется WordCount), что позволит нам сосредоточиться на особенностях управления платформой, в том числе на развертывании, мониторинге и обновлении.
...
3. Создайте каталог для хранения приложения, которое вы загрузите и развернете, например c:\\Service Fabric.
...
4. Загрузите приложение WordCount [отсюда](http://aka.ms/servicefabric-wordcountapp) в созданную папку.
...
Новая версия приложения теперь будет подсчитывать слова, начинающиеся с гласных. По мере того как развертывается обновление, мы можем увидеть в поведении приложения два изменения. Во-первых, скорость увеличения счетчика должна уменьшиться, так как подсчитывается меньшее количество слов. Во-вторых, так как первый раздел содержит две гласные буквы (A и E), а остальные разделы только по одной, счетчик первого раздела со временем начнет опережать другие счетчики.

1. Загрузите пакет v2 [отсюда](http://aka.ms/servicefabric-wordcountappv2) в то же расположение, в которое вы загрузили пакет v1.

2. Вернитесь в окно PowerShell и запустите команду обновления пакета SDK, чтобы зарегистрировать новую версию в кластере и начать обновление fabric:/WordCount.
...
3. Если обновление будет продолжаться, вам может быть проще отслеживать его состояние из обозревателя Service Fabric. Откройте окно браузера и перейдите по адресу [http://localhost:19080/Explorer](http://localhost:19080/Explorer). В дереве слева щелкните раздел **Приложения**, затем выберите раздел **Обновления выполняются**.

  ![Ход обновления в обозревателе Service Fabric][sfx-upgradeprogress]

  Обратите внимание, что индикатор «Обновления выполняются» отображает состояние обновления в доменах обновления кластера. По мере того как процесс обновления переходит к следующему домену, выполняются проверки работоспособности и правильного поведения приложения, и только после этого обновление продолжается.
...
4. Если вы повторно запустите предыдущий запрос для набора служб в приложении fabric:/WordCount, вы заметите, что версия WordCountService изменилась, а версия WordCountWebService — нет:
  ...
  Этот результат демонстрирует, как платформа Service Fabric управляет обновлениями: они применяются только к набору служб (либо пакетам кода или конфигурации в этих службах), которые были изменены. Таким образом, процесс обновления становится быстрым и надежным.

5. Теперь вернитесь в браузер, чтобы наблюдать за поведением новой версии приложения. Как и ожидалось, показания счетчика увеличиваются медленнее, а объем первого раздела немного больше, чем объем остальных разделов.

  ![Просмотр новой версии приложения в браузере][deployed-app-UI-v2]

## Дальнейшие действия
- Теперь, когда вы развернули и обновили предварительно собранные приложения, вы можете [собрать свое приложение в Visual Studio](service-fabric-create-your-first-application-in-visual-studio.md).
- Все действия из этой статьи, применяемые к локальному кластеру, можно также выполнять в [кластере Azure](service-fabric-cluster-creation-via-portal.md).
