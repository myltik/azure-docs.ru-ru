<properties
   pageTitle="Общие сведения о конфигурации Reliable Services Azure Service Fabric | Microsoft Azure"
...
Конфигурацию по умолчанию надежной службы с отслеживанием состояния можно изменить с помощью пакета конфигурации (конфигурация) или в реализации службы (код).

+ **Конфигурация**: для настройки с помощью пакета конфигурации необходимо изменить файл Settings.xml, созданный в корне пакета Visual Studio в папке Config каждой службы в приложении.
...
По умолчанию среда выполнения Service Fabric ищет предопределенные имена разделов в файле Settings.xml и использует значения конфигурации для создания базовых компонентов среды выполнения.
...
Значение CheckpointThresholdInMB позволяет управлять объемом места на диске, которое репликатор может использовать для хранения сведений о состоянии в выделенном файле журнала реплики. При добавлении новой реплики в набор замена значения по умолчанию более высоким значением может ускорить процесс перенастройки. Это связано с передачей частичного состояния, которая происходит, так как в журнале появляется больше истории об операциях, что может увеличивать время восстановления реплики после сбоя.
