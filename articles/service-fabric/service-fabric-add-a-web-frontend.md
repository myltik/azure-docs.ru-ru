...
> [!TIP]
> Дополнительные сведения об ASP.NET Core см. в [документации по ASP.NET Core](https://docs.microsoft.com/aspnet/core/).
> 
...
2. После завершения развертывания Visual Studio запустит браузер и откроет в нем корневой каталог службы веб-API ASP.NET, для которого по умолчанию зарезервирован порт 8966. Так как шаблон веб-API ASP.NET Core не предоставляет поведение по умолчанию для корневого каталога, в браузере будет показано сообщение об ошибке.
...
### <a name="create-the-interface"></a>Создание интерфейса
...
4. Найдите пакет **Microsoft.ServiceFabric.Services.Remoting** и установите его.
...
2. Добавьте в проект ASP.NET пакет Microsoft.ServiceFabric.Services.Remoting (так же, как и для проекта библиотеки классов ранее). Будет добавлен класс `ServiceProxy` .
...
## <a name="connecting-to-a-reliable-actors-service"></a>Подключение к службе Reliable Actors
...
Теперь, когда вы настроили веб-интерфейс для приложения с помощью ASP.NET Core, получите дополнительные сведения об интеграции ASP.NET Core с Service Fabric в статье о [ASP.NET Core в Service Fabric Reliable Services](service-fabric-reliable-services-communication-aspnetcore.md).
