...
>[AZURE.WARNING] Из-за изменений, внесенных в ядро ASP.NET версии RC2, эта статья временно содержит неправильную информацию, так как описываемый шаблон проекта был удален из пакета SDK. Данная статья будет исправлена после выпуска ядра ASP.NET версии RC2. До этого времени можно использовать шаблон веб-API без отслеживания состояния, который описан в разделе [Приступая к работе со службами веб-API Service Fabric с саморазмещением OWIN](service-fabric-reliable-services-communication-webapi.md).
...
При использовании подхода `ServiceProxy` (на основе удаленных вызовов процедур (RPC)) определяется интерфейс, который будет выступать в качестве общедоступного контракта службы. Затем этот интерфейс используется для создания класса прокси для взаимодействия со службой.
...
2. В области навигации слева выберите **Visual C#**, а затем — шаблон **Библиотека классов**. Убедитесь, что указана версия платформы .NET **4.5.1**.
