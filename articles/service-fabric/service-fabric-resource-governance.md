...
>
> Прежде чем продолжить работу с этой статьей, ознакомьтесь с [моделью приложения Service Fabric](service-fabric-application-model.md) и [моделью размещения Service Fabric](service-fabric-hosting-model.md).
>
...
Среда выполнения Service Fabric в настоящее время не поддерживает резервирование ресурсов. Если открыт процесс или контейнер, среда выполнения задаст ограничение — ресурсы будут ограничены нагрузками, определенными в момент создания. Кроме того, среда выполнения будет отклонять открытие новых пакетов служб из-за ограничения выше. Чтобы лучше понять сам процесс, рассмотрим пример — узел с 2 ядрами ЦП (механизм управления памятью аналогичен):

1. Сначала на узле размещается контейнер, которому требуется 1 ядро ЦП. Среда выполнения откроет контейнер и задаст для ЦП ограничение — 1 ядро. Таким образом, контейнер не сможет использовать больше 1 ядра.
2. Затем на узле размещается реплика службы и с помощью соответствующего пакета службы задается ограничение ЦП — 1 ядро. Среда выполнения откроет пакет кода и задаст для ЦП ограничение в 1 ядро.

На этом этапе ограничение равно емкости узла, а процесс и контейнер используют по 1 ядру и не влияют на операции друг друга. Service Fabric не сможет разместить большее количество контейнеров или реплик, если для них задано ограничение ЦП. Тем не менее, есть две ситуации, когда несколько процессов конкурируют за использование ЦП, а процесс и контейнер, описанные в примере выше, могут друг другу мешать.

* Сочетание управляемых и не управляемых служб и контейнеров: если пользователь создает службу и не указывает управление ресурсами, среда выполнения рассмотрит это как отсутствие использования ресурсов ЦП и разместит на узле несколько процессов. В таком случае этот новый процесс будет активно использовать некоторые ресурсы ЦП за счет служб, которые уже запущены на узле. Избежать этой проблемы можно, не смешивая управляемые и не управляемые службы в одном кластере или используя [ограничения размещения](service-fabric-cluster-resource-manager-advanced-placement-rules-placement-policies.md) для обеспечения использования разных наборов узлов.
* В случае, когда на узле запущен другой процесс, который не относится к процессам Service Fabric (например, служба ОС), он также будет конкурировать за ресурсы ЦП с имеющимися службами. Избежать этой проблемы можно, правильно настроив емкость узла с учетом дополнительной нагрузки на ОС, как описано в разделе ниже.

## <a name="cluster-set-up-for-enabling-resource-governance"></a>Настройка кластера для включения управления ресурсами

Когда узел запустится и присоединится к кластеру, Service Fabric обнаружит доступный объем памяти и доступное количество ядер и задаст емкости узла для этих двух ресурсов. Чтобы оставить место в буфере для операционной системы и других процессов, которые можно запустить на узле, Service Fabric будет использовать только 80 % от доступных на нем ресурсов. Это процентное значение можно настроить и изменить в манифесте кластера. Ниже приведен пример, как настроить Service Fabric для использования 50 % от доступных ресурсов ЦП и 70 % от доступной памяти: 
...
В случае, если емкости узла нужно настроить только вручную, можно также использовать стандартный механизм описания узлов в кластере. Ниже приведен пример настройки узла с 4 ядрами и 2 ГБ памяти: 
...
Если включено автоматическое обнаружение доступных ресурсов и в манифесте кластера емкости узла определены вручную, Service Fabric проверит, достаточно ли в узле ресурсов для поддержки определенной пользователем емкости:
* Если емкостей узла, определенных в манифесте, недостаточно для доступных на узле ресурсов (или же хватает), Service Fabric будет использовать емкости, указанные в манифесте.
* Если емкости узла, определенные в манифесте, превышают количество доступных ресурсов, Service Fabric будет использовать эти доступные ресурсы как емкости узла.

Автоматическое обнаружение доступных ресурсов можно полностью отключить, если изменение параметра ниже не требует его использования.
...
Кроме ЦП и памяти можно указать другие ограничения ресурсов для контейнеров. Эти ограничения указываются на уровне пакета кода и будут применены при запуске контейнера. В отличие от ограничений ресурсов ЦП и памяти, диспетчер ресурсов кластера не будет учитывать эти ресурсы и не будет выполнять проверки емкости или балансировку нагрузки для них. 
