...
5. Щелкните правой кнопкой мыши на области уведомлений диспетчера локального кластера и выберите пункт «Управление локальным кластером», чтобы запустить обозреватель Service Fabric.
    ...
    Обозреватель Service Fabric отображает кластер, составляющие его физические узлы и развернутые на нем приложения. Дополнительную информацию об обозревателе Service Fabric см. в статье [Визуализация кластера](service-fabric-visualizing-your-cluster).
...
### Очистка.
  ...
  3. Чтобы полностью удалить кластер, в приложении области уведомлений щелкните **Удалить кластер**. Если вы выберете этот вариант, всякий раз, когда вы будете нажимать клавишу F5 в Visual Studio, будет запускаться другое медленное развертывание. Применяйте это действие, только если вы не собираетесь использовать локальный кластер в течение некоторого времени или вам срочно необходимо освободить ресурсы.
