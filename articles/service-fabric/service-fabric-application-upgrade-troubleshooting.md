---
title: Устранение неполадок с обновлением приложений | Microsoft Docs
...
Service Fabric преобразовывает все процентные значения в фактические количества сущностей (например, реплик, секций и служб) для оценки работоспособности и всегда округляет их до числа целых сущностей. Например, если максимальное значение *MaxPercentUnhealthyReplicasPerPartition* равно 21 % и имеется пять реплик, то Service Fabric допустит выделение до двух неработоспособных реплик (т. е. Math.Ceiling (5*0.21)). Поэтому политики работоспособности должны быть настроены с учетом этого.
