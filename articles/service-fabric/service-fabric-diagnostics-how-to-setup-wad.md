...
Также можно скачать пример шаблона диспетчера ресурсов, внести в него изменения и создать кластер на основе измененного шаблона с помощью команды `New-AzureRmResourceGroupDeployment` в окне Azure PowerShell. Ниже приведены параметры, которые необходимо передать в команду. Дополнительные инструкции по развертыванию группы ресурсов с помощью PowerShell см. в статье [Развертывание ресурсов с использованием шаблонов Resource Manager и Azure PowerShell](../resource-group-template-deploy.md).
...
* [Service Fabric Solution in Log Analytics (Решение Service Fabric в Log Analytics)](../log-analytics/log-analytics-service-fabric.md)
