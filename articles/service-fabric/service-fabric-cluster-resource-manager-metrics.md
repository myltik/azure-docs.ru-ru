...
## Дальнейшие действия
- Дополнительные сведения о других вариантах настройки служб см. в разделе о других конфигурациях диспетчера кластерных ресурсов в статье [Configuring cluster resource manager settings for service fabric services](service-fabric-cluster-resource-manager-configure-services.md).
- Определение метрик дефрагментации — один из способов объединения нагрузки на узлах вместо ее рассредоточения. Чтобы узнать, как настроить дефрагментацию, см. [эту статью](service-fabric-cluster-resource-manager-defragmentation-metrics.md).
