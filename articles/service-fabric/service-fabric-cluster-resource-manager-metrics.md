...
Это прекрасно работает, пока вы не начнете задумываться над разными вопросами. Например, какова реальная вероятность того, что все первичные реплики разных служб на самом деле получают одинаковую нагрузку прямо сейчас? И еще один вопрос: какова вероятность того, что данная служба испытывает одинаковую нагрузку все время? Оказывается, что если речь идет о какой-либо серьезной нагрузке, то вероятности эти довольно малы.

На самом деле вы можете спокойно работать с метриками по умолчанию или по крайней мере со статическими пользовательскими метриками. Но обычно это приводит к тому, что кластер используется меньше, чем хотелось бы (так как создание отчетов не является адаптивным). В худшем случае это может привести к неправильному планированию узлов, что, в свою очередь, может вызвать проблемы с производительностью. Лучше использовать пользовательские метрики с отчетами о динамической нагрузке. Теперь перейдем к следующим разделам, посвященным пользовательским метрикам и динамическим нагрузкам.

## Пользовательские метрики
Итак, мы выяснили, что метрики могут быть физическими и логическими и пользователи могут определять свои собственные метрики. Как они это делают? В этом нет ничего сложного. Достаточно при создании службы настроить метрику и используемую по умолчанию начальную нагрузку. Любой набор метрик и значения по умолчанию можно настроить отдельно для каждого экземпляра службы во время создания службы.

Обратите внимание, что если вы начали определять пользовательские метрики, нужно явно добавить их к метрикам по умолчанию, если вы хотите, чтобы они (пользовательские метрики) тоже использовались для балансировки нагрузки в службе. Вот почему нам необходимо иметь четкое представление о связи между метриками по умолчанию и пользовательскими метриками: возможно, вас больше интересует, как потребляется память, чем распределение нагрузки в первичной реплике, или вы хотите смешать несколько реплик с параметрами по умолчанию.
...
## Дальнейшие действия
- [Configuring cluster resource manager settings for service fabric services](service-fabric-cluster-resource-manager-configure-services.md) (Настройка параметров диспетчера Resource Manager кластера, связанных со службами Service Fabric)
- [Дополнительные сведения о метриках дефрагментации](service-fabric-cluster-resource-manager-defragmentation-metrics.md)
- [Дополнительные сведения о балансировке нагрузки в кластере с помощью диспетчера Resource Manager кластера](service-fabric-cluster-resource-manager-balancing.md)
...
- [Дополнительные сведения о стоимости службы перемещения](service-fabric-cluster-resource-manager-movement-cost.md)
