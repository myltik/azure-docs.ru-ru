...
**Служба именования**. У каждого кластера Service Fabric есть служба именования, которая преобразует имена службы в определенное расположение в кластере и позволяет управлять именами и свойствами службы. Это своего рода служба доменных имен (DNS) для кластера. Используя службу именования, клиенты могут безопасно обмениваться данными с любым узлом в кластере для разрешения имени и расположения службы (т. е. для получения IP-адреса и порта фактического компьютера, на котором выполняется служба). С помощью API клиента связи можно разрабатывать службы и клиенты, которые способны разрешать текущее сетевое расположение несмотря на перемещение приложений в кластере, например из-за ошибок, балансировки ресурсов или изменения размера кластера.

Прочтите статью [Взаимодействие со службами](service-fabric-connect-and-communicate-with-services.md), чтобы получить дополнительные сведения об использовании API связи клиента и службы, который работает в сочетании со службой именования.

**Служба хранилища образов**. У каждого кластера Service Fabric есть служба хранилища образов, в которой хранятся развернутые пакеты приложения с контролем версий. Вам необходимо скопировать содержимое пакета приложения в службу хранилища образов и затем зарегистрировать тип приложения, хранящегося в этом пакете. Затем, когда тип приложения будет подготовлен, вы сможете создать именованное приложение данного типа. Удалить регистрацию типа приложения из службы хранилища образов можно только после удаления всех связанных с этим типом именованных приложений.
