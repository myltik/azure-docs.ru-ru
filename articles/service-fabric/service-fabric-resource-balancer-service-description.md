<properties
   ...
   description="Общие сведения о настройке описаний служб, которые будет использовать балансировщик ресурсов | Microsoft Azure"
...
[Ограничения на размещение](service-fabric-placement-constraint.md) службы — это механизм, позволяющий определенному экземпляру службы выбирать необходимые ему свойства узла. Как и свойства узла, эти ограничения представляют собой пары "ключ — значение", описывающие имя свойства и требования службы к значению. Из отдельных инструкций можно сформировать необходимое ограничение, сгруппировав их с помощью простой булевой логики. Обратите внимание, что эти ограничения интерпретирует балансировщик ресурсов Service Fabric.
...
В манифесте приложения указываются определения типов служб (`ServiceTemplates` или `DefaultServices`).
...
## Сходство служб
Будучи особой формой ограничения на размещение службы, определение сходства служб позволяет описывать разные службы, которые по различным причинам имеют локальную зависимость друг от друга. Это означает, что обе службы должны быть установлены на одном узле, иначе они не будут работать. Такая архитектура не должна широко применяться для приложений Service Fabric. Тем не менее, Service Fabric умеет распознавать переходные архитектуры определенных типов приложений прежних версий и позволяет размещать службы на одном узле.
...
- Нельзя сравнивать службы без отслеживания состояния с разным количеством экземпляров. Например, обе службы без отслеживания состояния должны иметь одинаковое свойство InstanceCount при создании.
- Сравнение нельзя использовать для временных или постоянных служб с отслеживанием состояния с различным количеством реплик. Например, обе службы должны иметь одинаковые значения Target и Min ReplicaSetSizes.
...
На диаграмме показан пример стандартной балансировки трех служб с отслеживанием состояния, каждая из которых имеет три реплики:
...
Если стандартных метрик не хватает для определенной службы или для службы выдвигаются конкретные требования относительно определенного ресурса, например памяти или места на диске, следует использовать пользовательские метрики. Пользовательские метрики полезны в случаях, когда в службе необходимо сбалансировать один или несколько ресурсов и предотвратить так называемое состязание между ними. Использование этих показателей может существенно варьироваться в разных репликах в кластере. Например, первичная реплика может использовать 100 % показателей, а другая может использовать только 20 процентов. С помощью пользовательских метрик также можно определять ресурсы, которых очень не хватает на том или ином компьютере (например память, место на диске или подключения). В некоторых случаях можно создать дополнительные пользовательские метрики, отслеживающие или представляющие метрики, у которых на узлах нет ограничений для максимального значения, но которые с точки зрения службы представляют работу и потребление ресурсов (к примеру, метрика "текущие необработанные транзакции"). Даже если у такой метрики на узле и нет ограничений по емкости, она сможет повысить быстродействие, если работа службы будет равномерно распределена по всему кластеру.
...
Дополнительные сведения см. в разделе [Архитектура балансировщика ресурсов](service-fabric-resource-balancer-architecture.md) и [ограничения на размещение](service-fabric-placement-constraint.md).
