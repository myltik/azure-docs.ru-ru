...
Из этого документа вы узнаете, как настроить различные параметры и политику обновления для кластера Service Fabric. Их можно настроить на [портале Azure](https://portal.azure.com) или с помощью шаблона Azure Resource Manager.
...
|NTLMAuthenticationPasswordSecret|SecureString, по умолчанию — Common::SecureString(L"")|Статическое|Содержит зашифрованный хэш, используемый для создания пароля для пользователей NTLM. Если для NTLMAuthenticationEnabled задано значение true, необходимо задать пароль. Проверяется с помощью средства развертывания. |
...
|QueueHealthWarningAtUsagePercent|Целое число без знака, по умолчанию — 80|Статическое|Это значение определяет использование очереди репликации (в процентах), после чего формируется отчет-предупреждение об интенсивном использовании очереди. Отчет формируется по истечении льготного интервала QueueHealthMonitoringInterval, Если показатель использования для очереди становится ниже процентного уровня в этом интервале, предупреждение не создается.|
