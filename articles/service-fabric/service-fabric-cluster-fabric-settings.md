...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
| TargetReplicaSetSize |Целое число, по умолчанию — 0 |Не разрешено|NOT_PLATFORM_UNIX_START. Размер набора целевых реплик для службы анализа сбоев. |
| MinReplicaSetSize |Целое число, по умолчанию — 0 |Не разрешено|Минимальный размер набора целевых реплик для службы анализа сбоев. |
...
| TargetReplicaSetSize | Целое число, по умолчанию — 7 |Не разрешено|Размер набора целевых реплик для службы хранилища образов. |
| MinReplicaSetSize | Целое число, по умолчанию — 3 |Не разрешено|Минимальный размер набора целевых реплик для службы хранилища образов. |
...
| **Параметр** | **Допустимые значения** | **Политика обновления** | **Рекомендация или краткое описание** |
| --- | --- | --- | --- |
| TargetReplicaSetSize |Целое число, по умолчанию — 0 |Не разрешено|Размер набора целевых реплик для службы управления обновлениями. |
| MinReplicaSetSize |Целое число, по умолчанию — 0 |Не разрешено|Минимальный размер набора целевых реплик для службы управления обновлениями.
...
|ResolveSystemService|Строка, по умолчанию — L"Admin\|\|User"|Динамический| Настройка безопасности для разрешения системных служб. |
...
|GracefulReplicaShutdownMaxDuration|Интервал времени, по умолчанию — Common::TimeSpan::FromSeconds(120)|Динамический|Временной диапазон в секундах. Период ожидания, по завершении которого система прекратит действие узлов службы, имеющих реплики, которые не удается закрыть.|
...
|ImageBuilderJobQueueThrottle |Целое число, по умолчанию — 10 |Динамический|Регулирование количества потоков для очереди заданий прокси Image Builder в запросах к приложению. |
|MaxExponentialOperationRetryDelay|Интервал времени, по умолчанию — Common::TimeSpan::FromSeconds(30)|Динамический|Временной диапазон в секундах. Максимальная экспоненциальная задержка для внутренних повторных попыток при повторном возникновении ошибок. |
