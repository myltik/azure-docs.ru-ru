...
После защиты автономного кластера под управлением Windows и настройки аутентифицированных клиентов, которые могут к нему подключаться, перейдите к разделу [Подключение к защищенному кластеру с помощью PowerShell](service-fabric-connect-to-secure-cluster.md#connectsecurecluster).
