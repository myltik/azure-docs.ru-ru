...
Надежные коллекции изначально гарантируют высокую целостность данных, благодаря чему вам будет легче анализировать состояние приложений. Высокая целостность достигается за счет того, что фиксация транзакции завершается только после применения всей транзакции к кворуму реплик (включая первичные реплики). Для достижения более низкой целостности приложения могут отправить подтверждение клиенту (или инициатору запроса) до того, как вернется асинхронная фиксация.
...
- нет выходных параметров (вместо параметров используется **ConditionalResult<T>** для возврата логического типа и значения; **ConditionalResult<T>** напоминает **Nullable<T>**, но может быть структурой без T);
