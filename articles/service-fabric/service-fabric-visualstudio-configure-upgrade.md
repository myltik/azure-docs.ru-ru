...
Средства Service Fabric Visual Studio обеспечивают поддержку обновления для публикации на локальном или удаленном кластерах. У обновления приложения до более новой версии есть два преимущества по сравнению с заменой приложения во время тестирования и отладки: во-первых, данные приложения не будут потеряны во время обновления, а во-вторых, достигается высокий уровень доступности, так как не будет перерывов в доступности службы по время обновления, если по доменам обновления распределено достаточное количество экземпляров службы. Тесты для приложения могут запускаться во время его обновления.
...
Существует три режима обновления: *отслеживаемое*, *неотслеживаемое автоматическое* и *ручное*.
  ...
  - При выполнении *ручного* обновления необходимо вручную обновить каждый домен обновления.
...
2. Выберите режим обновления, которые хотите использовать, и заполните сетку параметров.
