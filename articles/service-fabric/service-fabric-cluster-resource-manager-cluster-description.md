...
Диспетчер Resource Manager кластера Service Fabric предоставляет несколько механизмов для описания кластера. Во время выполнения Resource Manager использует эту информацию, чтобы обеспечить высокую доступность служб, функционирующих в кластере, и в то же время гарантировать, что ресурсы в кластере используются должным образом.
