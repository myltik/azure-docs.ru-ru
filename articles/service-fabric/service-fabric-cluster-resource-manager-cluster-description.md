...
Если бы вы настраивали собственный кластер, вам нужно было бы подумать об этих различных областях сбоя и убедиться, что домены сбоя настроены правильно и Service Fabric знает, где размещение служб будет безопасным. Под "безопасным" размещением мы имеем в виду продуманное размещение, при котором отключение домена сбоя не приводит к выходу служб из строя (например, при неполадках в работе одного из выше перечисленных компонентов).  В среде Azure мы используем данные домена сбоя, предоставляемые средой, для правильной настройки узлов в кластере от вашего имени.
На следующем рисунке (рис. 7) мы для примера выделили цветом все сущности, которые объективно могут привести к появлению домена сбоя, и привели список всех доменов сбоя, которые появились в результате. В этом примере у нас есть центры обработки данных (DC), стойки (R) и колонки (B). Предположительно, если каждая колонка содержит несколько виртуальных машин, в иерархии домена сбоя может быть еще один уровень.
