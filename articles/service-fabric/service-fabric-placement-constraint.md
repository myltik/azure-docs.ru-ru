<properties
   pageTitle="Ограничения на размещение при управлении кластером Service Fabric"
...
Service Fabric позволяет разработчикам использовать ограничения на размещение реплик служб на узлах, не удовлетворяющих определенным условиям. Условия выражаются через логическое выражение, в которое при расчете подставляются конкретные значения в соответствии с контекстом конкретной службы.
