...
**Шаг 2.2**. Убедитесь, что на компьютере установлена платформа Azure PowerShell 1.0 или более поздней версии. Если вы не сделали это ранее, настоятельно рекомендуем вам выполнить инструкции в статье [Установка и настройка Azure PowerShell](../powershell-install-configure.md).

**Шаг 2.3.**. Выполнив описанные выше действия, откройте окно PowerShell и импортируйте файл ServiceFabricRPHelpers.psm (это модуль, который вы загрузили при выполнении шага 2.1).
...
**Шаг 2.4**. При использовании уже полученного сертификата выполните действия ниже. В противном случае перейдите к шагу 2.5. В шаге 2.5 подробно описано, как создать и развернуть самозаверяющий сертификат в хранилище ключей.

Войдите в учетную запись Azure. В случае сбоев в работе PowerShell проверьте, правильно ли установлен пакет Azure PowerShell.
...
Если сценарий успешно выполнен, вы получите следующие выходные данные. Запишите эти данные — они вам потребуются для выполнения шага 3 (настройка безопасности кластера).
...
**Шаг 2.5**. Если у вас *нет* сертификата и вы хотите создать самозаверяющийся сертификат и отправить его в хранилище ключей, выполните описанные ниже действия.

Войдите в свою учетную запись Azure. В случае сбоев в работе PowerShell проверьте, правильно ли установлен пакет Azure PowerShell.
...
Параметр OutputPath, используемый в сценарии, будет содержать новый самозаверяющий сертификат, отправленный этим сценарием в хранилище ключей.
...
**Отпечаток сертификата**: 64881409F4D86498C88EEC3697310C15F8F1540F **SourceVault** /Идентификатор хранилища ключей: /subscriptions/35389201-c0b3-405e-8a23-9f1450994307/resourceGroups/chackdankeyvault4doc/providers/Microsoft.KeyVault/vaults/chackdankeyvault4doc **Certificate URL** /URL-адрес сертификата в хранилище ключей: https://chackdankeyvalut4doc.vault.azure.net:443/secrets/chackdantestcertificate3/fvc8df6300834326a95d05d90e0720ea
...
>[AZURE.NOTE] 
1\. Используемые сертификаты можно выбрать в поле типа узла в разделе конфигураций безопасности. 2. Их нужно указывать для каждого типа узла в кластере. 3. Хотя в этой статье описано, как выбрать сертификат на портале, это также можно сделать с помощью шаблона ARM.
...
**Чтобы добавить сертификат** для используемого кластера, зайдите на портал, выберите ресурс кластера, к которому нужно добавить сертификат, щелкните параметр сертификата, введите отпечаток дополнительного сертификата и щелкните "Сохранить". Запустится развертывание. После его успешного завершения вы сможете использовать оба сертификата, чтобы выполнять операции управления в кластере.

![Снимок экрана: раздел отпечатков сертификатов на портале][SecurityConfigurations_02]

**Чтобы удалить старый сертификат**, который больше не нужно использовать в кластере, зайдите на портал, откройте настройки безопасности и удалите сертификат. После удаления обязательно щелкните «Сохранить», чтобы запустить новое развертывание. После завершения развертывания удаленный сертификат больше не будет использоваться для подключения к кластеру.

Примечание. Для работы с безопасным кластером всегда требуется хотя бы один действительный (не отозванный или просроченный) развернутый сертификат (основной или дополнительный). В противном случае вы не сможете получить доступ к кластеру.


## Сведения о типах сертификатов, используемых Service Fabric.
...
>[AZURE.NOTE]
1\. Сертификаты, которые используются в кластерах, выполняющих рабочие нагрузки, можно создать с помощью правильно настроенной службы сертификации Windows Server или получить через утвержденный [центр сертификации](https://en.wikipedia.org/wiki/Certificate_authority). 2. Никогда не используйте в рабочей среде любые временные или тестовые сертификаты, созданные с помощью таких средств, как MakeCert.exe. 3. Для кластеров, которые используются только для тестирования, можно использовать самозаверяющие сертификаты.
...
- [Безопасность приложений и Runas](service-fabric-application-runas-security.md)
