...
> В таких случаях следует полагаться на замену строк в рабочем процессе развертывания. В Visual Studio Team Services можно воспользоваться таким расширением, как Replace Tokens: https://marketplace.visualstudio.com/items?itemName=qetza.replacetokens, а в Jenkins можно запустить задачу сценария для замены значений.
>
...
- [Как указать переменные среды для служб в Service Fabric](service-fabric-how-to-specify-port-number-using-parameters.md)
- [Как указать номер порта службы с помощью параметров в Service Fabric](service-fabric-how-to-specify-environment-variables.md)
