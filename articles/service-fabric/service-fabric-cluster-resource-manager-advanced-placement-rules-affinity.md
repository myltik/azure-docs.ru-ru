...
Сопоставление — один из тех моментов, которые, по крайней мере, на первый взгляд не имеют большого смысла в среде микрослужб. Сопоставление — это элемент управления, который применяется в основном для облегчения перевода больших, монолитных приложений в мир микро- и облачных служб. С другой стороны, в некоторых случаях этот элемент может использоваться в качестве доступного средства оптимизации производительности служб, хотя и не без возможных последствий.
...
3.	Все работает, но оказывается, что эти два компонента потребляют так много ресурсов, что после их переноса в отдельные службы производительность резко упала и приложением стало невозможно пользоваться.
...
Между сопоставленными и монолитными архитектурами существует несколько различий. Почти все они связаны с тем, что отношение сопоставления не дает никаких гарантий — поскольку службы совершенно различны, они могут отказать независимо друг от друга. В результате каких-либо действий, например из-за ограничений емкости или экземпляров сходных служб, различные реплики службы могут разделиться. В этих случаях даже имеющаяся связь по сходству временно не будет работать из-за других ограничений. Эта проблема будет исправлена автоматически, если впоследствии включить все другие ограничения и сходства.

### Цепочки и звезды
На данный момент моделировать цепочки отношений сопоставления нельзя. Это означает, что служба, которая является дочерней в одном отношении сопоставления, не может быть родительской в другом. Чтобы смоделировать такую связь, это нужно сделать не в виде цепочки, а как звезду, в которой самый нижний дочерний элемент является родительским объектом для родительского объекта среднего дочернего элемента. В зависимости от расположения служб может потребоваться создать службу-заполнитель, которая будет выступать родительским объектом для нескольких дочерних элементов.
