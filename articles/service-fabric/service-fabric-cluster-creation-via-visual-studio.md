...
Шаблон диспетчера ресурсов Service Fabric Visual Studio создает безопасный кластер, защищенный с помощью сертификата. Этот сертификат идентифицируется по трем последним параметрам шаблона (`certificateThumbprint`, `sourceVaultValue` и `certificateUrlValue`) и должен существовать в **хранилище ключей Azure**. Дополнительные сведения о создании сертификата безопасности кластера см. в статье [Сценарии защиты кластера Service Fabric](service-fabric-cluster-security.md#x509-certificates-and-service-fabric).
...
При появлении ошибок перейдите на [портал Azure](https://portal.azure.com/) и откройте группу ресурсов, в которую вы выполнили развертывание. Щелкните **Все параметры**, затем выберите **Развертывания** в колонке "Параметры". В области уведомлений будут находиться подробные диагностические сведения о неудачном развертывании группы ресурсов.
