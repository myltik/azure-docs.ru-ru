...
В этом случае метод `RunAsync()` службы может быть пустым, так как у службы нет необходимости в фоновой обработке задач. Когда создается служба калькулятора, она возвращает интерфейс `ICommunicationListener` (например, [веб-API](service-fabric-reliable-services-communication-webapi.md)), который открывает конечную точку прослушивания на том или ином порту. Эта конечная точка подключается к различным вычислительным методам (например, Add(n1, n2)), которые определяют общедоступный API калькулятора.
