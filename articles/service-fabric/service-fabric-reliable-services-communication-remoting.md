...
Для служб, которые не привязаны к определенному протоколу обмена данными или стеку, например WebAPI, WCF или другим, платформа предоставляет механизм удаленного взаимодействия для быстрой и простой настройки удаленного вызова процедур.
...
Вызов методов в службе с помощью удаленного взаимодействия стека осуществляется с помощью локального прокси к службе через класс `Microsoft.ServiceFabric.Services.Remoting.Client.ServiceProxy`. `ServiceProxy` создает локальный прокси, используя тот же интерфейс, который реализует служба. С помощью этого прокси можно без труда удаленно вызвать методы в интерфейсе.
