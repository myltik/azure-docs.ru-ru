...
В случае сбоя первичной реплики Service Fabric передает функции первичной реплики одной из активных вторичных реплик. Активная вторичная реплика уже имеет обновленную версию состояния (через *репликацию*) и может продолжить дальнейшую обработку операций чтения и записи. Этот процесс называется перенастройкой и описан далее в разделе [Перенастройка](service-fabric-concepts-reconfiguration.md).

Такое представление о том, что реплика может быть первичной или активной вторичной, известно как роль реплики. Это описано далее в разделе [Реплики и экземпляры](service-fabric-concepts-replica-lifecycle.md). 
