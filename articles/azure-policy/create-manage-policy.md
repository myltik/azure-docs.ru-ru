...
8. Выберите **область** — подписку (или группу ресурсов), которую вы ранее зарегистрировали. Она определяет, к каким ресурсам или группе ресурсов принудительно применяется назначение политики. Она может варьировать от подписки до групп ресурсов.
...
После назначения определения политики, требующего SQL Server версии 12.0, попытка создать SQL Server другой версии будет отклонена. В этом разделе мы решим проблему с невозможностью создать сервер SQL Server другой версии. Для этого нам потребуется запросить исключение.
