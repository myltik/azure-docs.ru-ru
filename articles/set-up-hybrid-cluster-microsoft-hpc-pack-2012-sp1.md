...
В этом учебнике предполагается, что у читателя нет опыта работы с вычислительными кластерами или пакетом HPC. Оно предназначено только для того, чтобы помочь вам быстро развернуть кластер вычислений для демонстрационных целей. Рекомендации и действия для развертывания гибридного кластера с пакетом HPC большего масштаба в рабочей среде см. в [подробном руководстве](http://go.microsoft.com/fwlink/p/?LinkID=200493). Если нужно установить кластер пакетов HPC полностью в Azure, см. раздел [Пакет Microsoft HPC в виртуальных машинах Azure](http://go.microsoft.com/fwlink/p/?linkid=330375). 

>[AZURE.NOTE] Azure предлагает [различные размеры](http://go.microsoft.com/fwlink/p/?LinkId=389844) для вычислительных ресурсов, подходящие для разных рабочих нагрузок. Например, экземпляры A8 и A9 обеспечивают высокую производительность и доступ к сети приложений с малой задержкой и высокой пропускной способностью, что необходимо для некоторых приложений HPC. Дополнительные сведения см. в разделе [Сведения о ресурсоемких вычислительных экземплярах A8 и A9](http://go.microsoft.com/fwlink/p/?Linkid=328042). 

<h2 id="BKMK_Prereq">Предварительные требования</h2>

>[AZURE.NOTE]Для работы с этим учебником требуется учетная запись Azure. Если ее нет, можно создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Создание учетной записи Azure](http://www.windowsazure.com/develop/php/tutorials/create-a-windows-azure-account/).
...
* Установочные файлы пакета HPC 2012 R2, доступные бесплатно. Последняя версия пакета - HPC 2012 R2 с пакетом обновлений 1. [Скачайте](http://go.microsoft.com/fwlink/p/?linkid=328024) полный пакет установки и скопируйте файлы на компьютер головного узла или в сетевое расположение. Выберите установочные файлы на языке установленной системы Windows Server.
...
3. На экране **Установка пакета HPC 2012 R2** нажмите кнопку **Новая установка или добавление новых компонентов к существующему экземпляру**.

	![HPC Pack 2012 Setup][install_hpc1]

4. На **странице пользовательского соглашения о программном обеспечении Майкрософт** нажмите кнопку **Далее**.

5. На странице **Выбор типа установки** щелкните **Создание кластера HPC с головным узлом** и нажмите кнопку **Далее**.
...
7. На странице **Настройки базы данных HPC** убедитесь, что **Головной узел** выбран для всех баз данных HPC, и нажмите кнопку **Далее**.
...
- Создание учетной записи хранения Azure
	...
	>[AZURE.NOTE]Также запишите идентификатор подписки Azure, которая понадобится позже. Его можно найти в <a href="[https://account.windowsazure.com/Subscriptions">сведениях об учетной записи Azure</a>.
...
3. На панели команд нажмите кнопку **Отправить**.
...
>[AZURE.NOTE]Для лучшей производительности рекомендуется создавать облачные службы и учетную запись хранения в одном географическом регионе.
...
5. В **списке задач развертывания** щелкните **Ввести учетные данные установки**. 

6. В диалоговом окне **Учетные данные установки** введите учетные данные учетной записи домена, которая использовалась для установки пакета HPC. Затем нажмите кнопку **ОК**.

	![Installation Credentials][config_hpc6]

	>[AZURE.NOTE]Службы пакета HPC используют учетные данные установки только для развертывания локальных вычислительных узлов.
	
7. В **списке задач развертывания**, щелкните **Настроить именование новых узлов**. 

8. В диалоговом окне **Указание серии именования узлов** примите значения по умолчанию и нажмите кнопку **ОК**.

	![Node Naming][config_hpc8]

	>[AZURE.NOTE]Серия именования формирует только имена для вычислительных узлов, присоединенных к домену. Узлы Azure именуются автоматически.
	  
9. В **списке задач развертывания**, щелкните **Создать шаблон узла**. Этот шаблон будет использоваться для добавления узлов Azure в кластер.

10. В окне мастера создания узла шаблона выполните следующие действия.

	a. На странице **Выбор типа шаблона узла** щелкните **шаблон узла Azure**, а затем нажмите кнопку **Далее**.

	![Node Template][config_hpc10]

	b. Щелкните **Далее**, чтобы использовать имя шаблона по умолчанию.

	c. На странице **Укажите сведения о подписке** введите свой идентификатор подписки Azure (он доступен в <a href="[https://account.windowsazure.com/Subscriptions">сведениях об учетной записи Azure</a>). Затем в разделе **Сертификат управления** нажмите кнопку **Обзор** и выберите **Управление HPC Azure по умолчанию.** Нажмите кнопку **Далее**.
	...
	e. Нажмите кнопку **Далее**, чтобы принять значения по умолчанию на оставшихся страницах мастера. Затем на вкладке **Просмотр** нажмите кнопку **Создать**, чтобы создать шаблон узла.

	>[AZURE.NOTE]По умолчанию шаблон узла Azure содержит параметры для запуска (подготовки) и остановки узлов вручную. Вы также можете настроить расписание для автоматического запуска и остановки узлов Azure. 
...
3. На странице **Указание новых узлов** выберите созданный ранее шаблон узла Azure (который по умолчанию называется **шаблоном узла Azure по умолчанию**). Затем укажите узлы **2** **мелких** узла и нажмите кнопку **Далее**.
...
3. Через несколько минут подготовка узлов Azure завершится и они перейдут в состояние **Не в сети**. В этом состоянии экземпляры ролей выполняются, но еще не принимают задания кластера.

4. Чтобы убедиться, что экземпляры ролей выполняются, на [портале управления](https://manage.windowsazure.com) щелкните **Облачные службы**, затем выберите имя вашей облачной службы и выберите **Экземпляры**. 
...
4. Чтобы убедиться, что экземпляры ролей больше не выполняются в Azure, на [портале управления](https://manage.windowsazure.com) щелкните **Облачные службы**, затем выберите имя вашей облачной службы и щелкните **Экземпляры**. В рабочей среде будут отсутствовать развернутые экземпляры.
...
* [Повышение в Azure с помощью пакета Microsoft HPC](http://go.microsoft.com/fwlink/p/?LinkID=200493)
