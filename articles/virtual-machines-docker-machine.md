...
Самый быстрый способ начать работу с **docker-machine** — скачать соответствующий выпуск непосредственно со [страницы общего доступа к выпуску](https://github.com/docker/machine/releases). В этом учебнике упоминался клиентский компьютер с архитектурой x64 под управлением Ubuntu, поэтому использовался образ **docker-machine_linux-amd64**.
