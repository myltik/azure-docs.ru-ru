...
В следующем фрагменте шаблона *templateBaseUrl* указывает корневую папку для шаблона в GitHub. Следующая строка создает новую переменную *sharedTemplateUrl*, которая объединяет значение *templateBaseUrl* с известным именем шаблона общих ресурсов. Ниже для хранения размера используется переменная составного объекта, в котором *templateBaseUrl* объединяется, чтобы указать расположение известного шаблона конфигурации в свойстве *vmTemplate*.
...
С помощью свойства *scriptsToDownload* можно указать сценарии для загрузки в виртуальную машину.
