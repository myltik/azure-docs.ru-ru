...
Теги хранятся непосредственно в ресурсах и группах ресурсов, поэтому чтобы просмотреть, какие теги уже применены, мы можем просто получить ресурс или группу ресурсов с помощью команд **Get-AzureRmResource** или **Get-AzureRmResourceGroup** соответственно. Давайте начнем с группы ресурсов.
...
- Ограничения и соглашения можно применять внутри подписки с помощью настраиваемых политик. Для использования определяемой политики может потребоваться задать конкретный тег для всех ресурсов. Дополнительные сведения см. в статье [Применение политик для управления ресурсами и доступом](resource-manager-policy.md).
