...
Для просмотра таксономии тегов на портале последовательно выберите пункты **Обзор** и **Теги**.

![Поиск тегов с помощью узла "Обзор"](./media/resource-group-using-tags/select-tags.png)

Отобразится сводка тегов в подписке.

![Отображение всех тегов](./media/resource-group-using-tags/show-tag-summary.png)

При выборе любого из этих тегов отображаются ресурсы и группы ресурсов с этим тегом.

![Отображение ресурсов с тегами](./media/resource-group-using-tags/show-tagged-resources.png)

Закрепите наиболее важные теги на панели мониторинга для быстрого доступа к ним.

![Закрепление тегов на начальной панели](./media/resource-group-using-tags/show-pinned-tag.png)
...
Теги находятся непосредственно в ресурсах и группах ресурсов, поэтому, чтобы просмотреть, какие теги уже применены, мы можем просто получить группу ресурсов и соответствующие ресурсы с помощью команды **azure group show**.
...
Теги обновляются полностью, поэтому при добавлении одного тега к ресурсу, уже содержащему теги, вам потребуется извлечь все существующие теги, которые нужно сохранить. Чтобы задать значения тегов для группы ресурсов, используйте команду **azure group set** и укажите все теги для группы ресурсов.
...
Можно вывести список существующих тегов в подписке с помощью команды **azure tag list** и добавить новый тег с помощью команды **azure tag create**. Чтобы удалить тег из таксономии подписки, сначала необходимо удалить этот тег из всех ресурсов, в которых он может применяться, а затем удалить его с помощью команды **azure tag delete**.
