...
1. Запустите протокол удаленного рабочего стола для кластера HDInsight, следуя инструкциям раздела [Соединение с кластерами HDInsight с использованием протокола RDP](http://azure.microsoft.com/documentation/articles/hdinsight-administer-use-management-portal/#rdp). Войдите в узел кластера и щелкните элемент **Командная строка Hadoop** на рабочем столе.
