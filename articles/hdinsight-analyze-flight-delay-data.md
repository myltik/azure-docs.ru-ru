<properties 
	pageTitle="Анализ данных о задержке рейсов с помощью Hadoop в HDInsight | Azure" 
	description="Узнайте, как использовать один сценарий PowerShell, чтобы подготовить кластер HDInsight, выполнить задание Hive, выполнить задание Sqool и удалить кластер." 
	services="hdinsight" 
	documentationCenter="" 
	authors="mumian" 
	manager="paulettm" 
...
* [Необходимые условия](#prerequisite)
* [Подготовка кластера HDInsight к работе и выполнение заданий Hive и Sqoop (M1)](#runjob)
* [Приложение А. Загрузка данных о задержке рейсов в хранилище больших двоичных объектов Azure (A1)](#appendix-a)
* [Приложение Б. Создание и загрузка скрипта HiveQL (A2)](#appendix-b)
...
Кластеры Hadoop в HDInsight используют для хранения данных хранилища BLOB-объектов Azure.  Оно называется *WASB* или *Azure Storage - Blob*. WASB - это реализация *HDFS* на базе хранилища BLOB-объектов Azure от корпорации Майкрософт. Дополнительные сведения см. в разделе [Использование хранилища BLOB-объектов Azure с HDInsight][hdinsight-storage]. 

При подготовке кластера HDInsight контейнер хранилища BLOB-объектов, принадлежащий учетной записи хранения Azure, используется в качестве файловой системы по умолчанию, так же как и HDFS. Эта учетная запись хранения называется *default storage account*, а контейнер BLOB-объектов называется *default Blob container* или *default container*. Учетная запись хранения, используемая по умолчанию, должна находиться в том же центре обработки данных, что и кластер HDInsight. При удалении кластера HDInsight удаления используемого по умолчанию контейнера или используемой по умолчанию учетной записи хранения не происходит.
...
Hadoop MapReduce представляет из себя пакетную обработку данных. Наиболее эффективным способом выполнения заданий Hive является подготовка кластера к конкретному заданию и удаление задания после его выполнения. Приводимый далее скрипт позволяет полностью реализовать этот процесс. Более подробная информация о подготовке кластера HDInsight и выполнении заданий Hive приводится в разделах [Подготовка кластеров Hadoop в HDInsight к работе][hdinsight-provision] и  [Использование Hive с HDInsight][hdinsight-use-hive]. 
...
4. Нажмите клавишу **F5** для запуска сценария. Результат должен быть аналогичен приведенному ниже:
...
- **Укажите учетную запись хранения Azure, которая будет использована в качестве файловой системы по умолчанию для кластера HDInsight.** Поскольку кластер HDInsight будет иметь ключ доступа учетной записи хранения, вам не нужно вносить никаких дополнительных изменений.
- **Укажите другую учетную запись хранения из файловой системы по умолчанию кластера HDInsight.** В этом случае необходимо изменить процедуру подготовки сценария PowerShell в разделе [Подготовка кластера HDInsight к работе и выполнение заданий Hive и Sqoop](#runjob) чтобы добавить учетную запись хранения в качестве дополнительной. Инструкции см. в разделе [Подготовка кластеров Hadoop в HDInsight][hdinsight-provision]. После этого кластеру HDInsight станет известен ключ доступа к учетной записи хранения.

>[AZURE.NOTE] Путь WASB к файлу данных жестко запрограммирован в файле сценария HiveQL. Его следует обновлять соответствующим образом.
...
2. На странице выберите следующие значения:
	...
	<tr><td>Поля:</td><td>*Год*, *FlightDate*, *UniqueCarrier*, *Carrier*, *FlightNum*, *OriginAirportID*, *Origin*, *OriginCityName*, *OriginState*, *DestAirportID*, *Dest*, *DestCityName*, *DestState*, *DepDelayMinutes*, *ArrDelay*, *ArrDelayMinutes*, *CarrierDelay*, *WeatherDelay*, *NASDelay*, *SecurityDelay*, *LateAircraftDelay* (очистите все остальные поля)</td></tr>
	</table>
...
**Передача данных о задержке рейсов в хранилище BLOB-объектов Azure**
...
5. **Вычисляет среднюю задержку из-за погоды и группирует результаты по названию города.** Кроме того, выводит результаты в WASB. Обратите внимание, что запрос удаляет апострофы из данных и исключает строки, где значение *weather_deal*y равно *null*, что является необходимым, так как Sqoop, используемый в этом руководстве, по умолчанию не обеспечивает правильную обработку этих значений.
...
2. Скопируйте следующий скрипт и вставьте его в область скриптов:
	...
	>[AZURE.NOTE] Этот сценарий использует службу REST (http://bot.whatismyipaddress.com) для получения вашего внешнего IP-адреса. Этот IP-адрес используется для создания правила брандмауэра, необходимого для работы сервера базы данных SQL.  
	...
	- **$sqlDatabaseMaxSizeGB**. Этот параметр используется только при создании нового сервера базы данных SQL в Azure. Значение по умолчанию - <u>10 ГБ</u>. Размера 10ГБ достаточно для примеров из данного учебника.
