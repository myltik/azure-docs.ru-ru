...
-   [Приложение A: Отправка данных о задержке рейсов в хранилище BLOB-объектов Azure (A1)][Приложение A: Отправка данных о задержке рейсов в хранилище BLOB-объектов Azure (A1)]
-   [Приложение B: Создание и отправка скрипта HiveQL (A2)][Приложение B: Создание и отправка скрипта HiveQL (A2)]
...
## <span id="runjob"></span></a>Выполнение задания Hive на вновь создаваемом или уже существующем кластере HDInsight
...
## <span id="appendix-a"></span></a>Приложение A: Отправка данных о задержке рейсов в хранилище BLOB-объектов Azure
...
## <span id="appendix-b"></span></a>Приложение B: Создание и загрузка скриптов HiveQL
...
2.  Скопируйте следующий скрипт и вставьте его в область скриптов:
    ...
    > WACOM.NOTE В этом скрипте для получения внешнего IP-адреса используется служба REST: <http://bot.whatismyipaddress.com>. Этот IP-адрес используется для создания правила брандмауэра, необходимого для работы сервера базы данных SQL.
...
*tutorials/flightdelays/data* является виртуальной папкой, созданной вами при отправке файлов. Убедитесь, что присутствует 12 файлов — по одному для каждого месяца.
  ...
  [Приложение A: Отправка данных о задержке рейсов в хранилище BLOB-объектов Azure (A1)]: #appdendix-a
  [Приложение B: Создание и отправка скрипта HiveQL (A2)]: #appendix-b
