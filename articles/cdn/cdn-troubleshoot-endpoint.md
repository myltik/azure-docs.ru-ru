<properties
	pageTitle="CDN. Устранение неполадок конечных точек CDN, возвращающих состояние 404"
	description="Устранение неполадок конечных точек CDN, связанных с кодами ответа 404"
	services="cdn"
...
Давайте вернемся к нашему гипотетическому примеру с `http://www.contoso.com:8080/file.txt`. Как вы помните, Contoso указали `8080` как порт HTTP, однако также предположим, они указали `44300` как порт HTTPS. Если они создали конечную точку с именем `contoso`, именем узла их конечной точки CDN будет `contoso.azureedge.net`. Запрос файла `http://contoso.azureedge.net/file.txt` является HTTP-запросом, поэтому конечная точка будет использовать HTTP по порту 8080, чтобы получить его из источника. Безопасный запрос по протоколу HTTPS, `https://contoso.azureedge.net/file.txt`, заставит конечную точку использовать HTTPS по порту 44300 при получении файла из источника.

#### Заголовок узла источника

**Заголовок узла источника** — это значение заголовка узла, отправляемое в источник с каждым запросом. В большинстве случаев оно должно совпадать со значением **Имя узла источника**, которое мы проверяли ранее. Неверное значение в этом поле, как правило, не вызывает состояния 404, но может вызвать другие состояния 4xx, в зависимости от данных, ожидаемых источником.
