...
> [AZURE.NOTE] В конечных точках **Azure CDN от Akamai** запрещено использовать полный диапазон портов TCP для источников. Список запрещенных портов источников см. в статье [Azure CDN from Akamai Behavior Details](cdn-akamai-behavior-details.md) (Особенности поведения Azure CDN от Akamai).
