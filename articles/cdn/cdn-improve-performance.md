...
### <a name="azure-cdn-from-verizon-profiles-both-standard-and-premium-tiers"></a>Профили Azure CDN от Verizon (уровни "Стандартный" и "Премиум")

Для профилей **Azure CDN от Verizon** будут сжаты только подходящие файлы. Сжатие допускается для следующих файлов:
- более 128 байт;
- менее 1 МБ.
...
Если в запросе ресурса указано сжатие brotli (заголовок HTTP `Accept-Encoding: br`) и запрос приводит к сбросу кэша, Azure CDN выполняет сжатие brotli на исходном сервере. После этого сжатый файл используется напрямую из кэша.

### <a name="azure-cdn-from-akamai-profiles"></a>Профили Azure CDN от Akamai
