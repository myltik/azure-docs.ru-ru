...
[Специальные заголовки прокси-сервера](#proxy-special-headers) | Определяет набор заголовков запросов, относящихся к CDN, которые перенаправляются с пограничного сервера на сервер-источник.
...
Отключено|Приводит к игнорированию параметров регулирования пропускной способности пограничными серверами. Запрашиваемое содержимое предоставляется обычным способом (т. е. без регулирования пропускной способности).
...
- Укажите одно или несколько имен параметров строки запроса. Разделите все имена параметров одним пробелом.
...
Отключено|Предотвращает получение фоновых данных для ресурса на пограничном сервере. В результате при выполнении следующего запроса на этот ресурс из этого региона пограничный сервер запрашивает его с сервера-источника клиента.
