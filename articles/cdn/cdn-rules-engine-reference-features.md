...
**Назначение.** Определяет набор заголовков запросов в CDN, перенаправляемых от точки подключения на сервер-источник.
...
В примере показано перенаправление URL-адреса пограничной записи CNAME на базовый URL-адрес CDN: http://marketing.azureedge.net/brochures

Отвечающие требованиям запросы будут перенаправляться на базовый URL-адрес пограничной записи CNAME: http://cdn.mydomain.com/resources
...
- Все соответствующие запросы будут перенаправляться на URL-адрес граничной записи CNAME, определенный в параметре Destination. 
    ...
    - Пример сценария №2: 
        - Пример запроса (URL-адрес пограничной записи CNAME): http://marketing.mydomain.com/brochures/widgets.pdf 
        - URL-адрес запроса (после перенаправления): http://cdn.mydomain.com/resources/widgets.pdf Пример сценария
    - Пример сценария №3: 
        - Пример запроса (URL-адрес пограничной записи CNAME): http://brochures.mydomain.com/campaignA/final/productC.ppt 
...
Пример перенаправления с URL-адреса пограничной записи CNAME на базовый URL-адрес CDN: http://marketing.azureedge.net/brochures/

Отвечающие требованиям запросы будут перенаправляться на базовый URL-адрес пограничной записи CNAME: http://MyOrigin.azureedge.net/resources/
