...
Имя | Назначение
-----|--------
Параметры пропускной способности | Определяет, будут ли активны параметры регулирования пропускной способности (например, ec_rate и ec_prebuf).
...
Формат файла можно указать с помощью его типа мультимедиа для Интернета (т. е. Content-Type). Тип мультимедиа для Интернета — это метаданные, не зависящие от платформы, по которым наши серверы могут определить формат файла определенного ресурса. Ниже приведен список распространенных типов мультимедиа для Интернета.
...
Удалить|Удаляет указанный заголовок ответа.|**Значение заголовка ответа (клиент):** значение1. <br/> **Конфигурация заголовка ответа Modify Client:** удаление заголовка ответа в вопросе. <br/>**Результат.** Указанный заголовок ответа не будет перенаправляться инициатору запроса.
