...
8. Для параметров **Протокол** и **Порт источника** укажите протоколы и порты, используемые для доступа к ресурсам в источнике. Необходимо указать хотя бы один протокол (HTTP или HTTPS).
	...
	> [AZURE.NOTE] **Порт источника** влияет только на порт, используемый конечной точкой для извлечения данных из источника. Сама конечная точка будет доступна конечным клиентам только на портах HTTP и HTTPS по умолчанию (80 и 443), вне зависимости от **порта источника**.
	>
	> В конечных точках **Azure CDN от Akamai** запрещено использовать полный диапазон портов TCP для источников. Список запрещенных портов источников см. в статье [Azure CDN from Akamai Behavior Details](cdn-akamai-behavior-details.md) (Особенности поведения Azure CDN от Akamai).
	>
