...
8. Для параметров **Протокол** и **Порт источника** укажите протоколы и порты, используемые для доступа к ресурсам в источнике.  Необходимо указать хотя бы один протокол (HTTP или HTTPS).
   ...
   > * Поддержка HTTPS для личных доменов Azure CDN предоставляется только с продуктами **Azure CDN от Verizon** (категории "Стандартный" и "Премиум"). Недоступно в **Azure CDN из Akamai**. См. дополнительные сведения о [включении протокола HTTPS для личного домена Azure CDN](cdn-custom-ssl.md).

Для доступа к HTTPS-содержимому используйте домен, предоставленный сетью CDN (`<endpointname>.azureedge.net`). Поддержка HTTPS для имен личных доменов (CNAME) недоступна, поскольку в данный момент CDN не поддерживает пользовательские сертификаты.
   > 
   > 
