...
С помощью [пакета SDK Azure CDN для Node.js](https://www.npmjs.com/package/azure-arm-cdn) можно автоматизировать создание профилей и конечных точек CDN и управление ими. В этом руководстве описывается создание простого консольного приложения Node.js с примерами некоторых из доступных операций. Это руководство не содержит подробные сведения обо всех свойствах пакета Azure CDN SDK для Node.js.
...
Вам будет предложено ответить на ряд вопросов для инициализации проекта. В качестве **точки входа** в этом учебнике я буду использовать *app.js*. В приведенном ниже примере вы можете увидеть мои ответы на остальные вопросы.
