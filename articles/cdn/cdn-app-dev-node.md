...
# Приступая к работе с пакетом Azure CDN SDK для Node.js

С помощью [пакета Azure CDN SDK для Node.js](https://www.npmjs.com/package/azure-arm-cdn) можно автоматизировать создание профилей и конечных точек CDN и управление ими. В этом руководстве описывается создание простого консольного приложения Node.js с примерами некоторых из доступных операций. Это руководство не содержит подробные сведения обо всех свойствах пакета Azure CDN SDK для Node.js.

Для работы с этим руководством у вас должен быть установлен и настроен [Node.js](http://www.nodejs.org) **4.x.x** или более поздней версии. Для создания приложения Node.js вы можете использовать любой текстовый редактор. Например, в этом руководстве используется [Visual Studio Code](https://code.visualstudio.com).

Полный пример руководства доступен [здесь](https://code.msdn.microsoft.com/Azure-CDN-SDK-for-Nodejs-c712bc74).
...
Вам будет предложено ответить на ряд вопросов для инициализации проекта. В качестве **точки входа** в этом руководстве мы будем использовать *app.js*. В приведенном ниже примере вы можете увидеть мои ответы на остальные вопросы.
...
Давайте откроем файл *app.js* в редакторе и напишем базовую структуру нашей программы.
...
> [AZURE.TIP] Если вы используете в качестве отладчика Visual Studio Code, вам потребуется настроить среду для передачи параметров командной строки. Для этой цели в Visual Studio Code используется файл **lanuch.json**. Найдите в нем свойство с именем **args** и добавьте массив строковых значений со своими параметрами, примерно так: `"args": ["list", "profiles"]`.
...
Справочную информацию о пакете Azure CDN SDK для Node.js вы найдете [здесь](http://azure.github.io/azure-sdk-for-node/azure-arm-cdn/latest/).

Чтобы получить дополнительную документацию о пакете Azure SDK для Node.js, откройте [полный справочник](http://azure.github.io/azure-sdk-for-node/).
