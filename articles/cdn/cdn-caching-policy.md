...
> [!IMPORTANT]
>Службы мультимедиа Azure полностью интегрированы с Azure CDN. Одним щелчком можно интегрировать все доступные поставщики Azure CDN (Akamai и Verizon) в конечную точку потоковой передачи, включая продукты CDN уровней "Стандартный" и "Премиум". Дополнительные сведения см. в [этом объявлении](https://azure.microsoft.com/blog/standardstreamingendpoint/).
> 
...
1. Чтобы настроить заголовки кэша с помощью портала Azure, выполните инструкции в разделе "Настройка конечной точки потоковой передачи" статьи [Управление конечными точками потоковой передачи с помощью портала Azure](../media-services/media-services-portal-manage-streaming-endpoints.md).
