...
При указании **UseMaxAge** в ответ добавляется Cache-Control: max-age=<nnn> на основе значения, указанного в атрибуте **CacheControlMaxAge**. Формат временного диапазона для атрибута **cacheControlMaxAge** имеет вид <дни>.<часы>:<минуты>:<секунды>. Дополнительные сведения об узле **clientCache** см. в разделе [Кэш клиента <clientCache>](http://www.iis.net/ConfigReference/system.webServer/staticContent/clientCache).  
