...
Сеть доставки содержимого (CDN) Azure является основным стандартным блоком для масштабирования любого приложения HTTP/HTTPS в Azure. Она предоставляет клиентам Azure глобальное решение, кэшируя и доставляя содержимое непосредственно конечным пользователям. В результате запросы пользователей, вместо того чтобы каждый раз атаковать источник, будут интеллектуально маршрутизироваться на граничный узел CDN (POP или точка присутствия), обеспечивая лучшее время ответа. Это значительно повышает производительность и улучшает взаимодействие с пользователем. Актуальный список всех расположений узлов CDN см. в разделе [Расположение узлов POP сети доставки содержимого Azure (CDN)](cdn-pop-locations.md).
...
После включения доступа CDN для учетной записи хранения портал управления предоставит вам имя домена CDN в следующем формате: `http://<identifier>.azureedge.net/`. Это доменное имя может использоваться для доступа к BLOB-объектам в общедоступном контейнере. Например, если BLOB-объекты хранятся в общедоступном контейнере, пользователи могут обращаться к нему по двум URL-адресам:

- **URL-адрес службы BLOB-объектов Azure**: `http://<account>.blob.core.windows.net/<container>/`;
- **URL-адрес Azure CDN**: `http://<identifier>.azureedge.net/<container>/`.

> [AZURE.TIP] В примере выше мы указали для конечной точки CDN *всю* учетную запись хранения. Соответственно в URL-адресе CDN нужно указать конкретный контейнер. Чтобы указать в корневом URL-адресе CDN конкретный контейнер, используйте параметр **Исходный путь**.
