...
> [AZURE.NOTE]Возможности расширенных HTTP-отчетов для сетей CDN относятся к функциям уровня «Премиум». Сравнение возможностей CDN уровней «Стандартный» и «Премиум» см. в статье [Обзор Azure CDN](cdn-overview.md).
...
* Если не удалось определить город или регион (штат, провинцию), из которого поступил запрос, то будет указано «Неизвестно». Если не удается определить страну, отобразятся два знака вопроса («??»).
