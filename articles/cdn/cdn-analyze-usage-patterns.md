...
- TCP\_HIT ― это состояние передается тогда, когда запрос напрямую обрабатывается на POP и передается в клиент. Ресурс немедленно отправляется с POP при кэшировании на POP, который расположен наиболее близко к клиенту и имеет допустимое значение TTL. TTL определяется следующими заголовками отклика:
...
- TCP\_EXPIRED \_HIT — это состояние передается тогда, когда запрос к определенному ресурс-контейнеру, срок действия которого истек (превысил TTL), например в случае истечения срока, заданного параметром max-age, был напрямую обработан на POP и передан клиенту.
...
В этом отчете указывается процент кэшированных запросов, обработанных напрямую из кэша. В нем приводятся следующие сведения.
