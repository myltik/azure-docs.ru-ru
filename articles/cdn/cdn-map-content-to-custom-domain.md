...
2. [Создайте записи DNS CNAME](#step-2-create-the-cname-dns-records).
    ...
    - Вариант 2: Сопоставление личного домена с конечной точкой CDN с помощью cdnverify 
...
Если вы используете Azure для размещения [доменов DNS](https://docs.microsoft.com/en-us/azure/dns/dns-overview), вам следует делегировать DNS поставщика домена в Azure DNS. Дополнительные сведения см. в статье [Делегирование домена в Azure DNS](https://docs.microsoft.com/azure/dns/dns-delegate-domain-azure-dns).
...
Этот шаг зависит от шага 2, варианта 2 (сопоставление с поддоменом **cdnverify**). Если вы используете временный поддомен **cdnverify** и проверили, что он работает, вы можете сопоставить свой постоянный личный домен с конечной точкой CDN.
