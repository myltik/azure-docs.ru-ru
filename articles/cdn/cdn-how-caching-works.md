---
title: "Как работает кэширование в сети доставки содержимого Azure | Документация Майкрософт"
...
- Переопределяет заголовок `Expires`, если определен этот заголовок и `Cache-Control`.
- При использовании в заголовке запроса: по умолчанию игнорируется Azure CDN.
- При использовании в заголовке ответа Azure CDN выполняет следующие директивы `Cache-Control`, когда он использует общую веб-доставку, скачивание больших файлов и общие оптимизации или оптимизации потоковой передачи мультимедиа при передаче видео по запросу:  
   ...
   - `private`. Содержимое предназначено только для одного пользователя; содержимое общих кэшей, например CDN, не сохраняется.
   - `no-cache`. Кэширует содержимое, однако она должна проверять содержимое каждый раз перед его доставкой. Эквивалент `Cache-Control: max-age=0`.
...
`Pragma` 
   - По умолчанию не используется Azure CDN.
   - Устаревший заголовок, представленный в HTTP 1.0. Поддерживается для обеспечения обратной совместимости.
...
По умолчанию оптимизация DSA игнорирует эти заголовки. Вы можете настроить, как Azure CDN обрабатывает эти заголовки, с помощью правил кэширования CDN. Дополнительные сведения см. в статье [Управление режимом кэширования Azure CDN с помощью правил кэширования](cdn-caching-rules.md).
...
| Размер файла         | 300 ГБ                 | <ul><li>Общая веб-оптимизация доставки: 1,8 ГБ</li> <li>Оптимизация потоковой передачи мультимедиа: 1,8 ГБ</li> <li>Оптимизация больших файлов: 150 ГБ</li> |
...
|                    | Verizon. Общая веб-доставка | Verizon. Динамическое ускорение сайтов | Akamai. Общая веб-доставка | Akamai. Динамическое ускорение сайтов | Akamai. Скачивание больших файлов | Akamai. Общая потоковая передача или потоковая передача видео по запросу |
|--------------------|--------|------|-----|----|-----|-----|
| **Учет источника**   | Да    | Нет   | Да | Нет | Да | Да |
| **Длительность кэширования CDN** | 7 дней | None | 7 дней | None | 1 день | Год |
