...
После включения CDN для вашего приложения вам, скорее всего, потребуется отслеживать использование CDN, проверять работоспособность службы доставки и устранять возможные неполадки. Azure CDN предоставляет следующие возможности [основной аналитики](cdn-analyze-usage-patterns.md).

Как текущий пользователь Azure CDN с профилем Verizon уровня "Стандартный" или "Премиум" вы уже можете просматривать данные основной аналитики на дополнительном портале, доступ к которому можно получить с помощью команды "Управление" на портале Azure. Благодаря новой функции [журналов диагностики](https://docs.microsoft.com/azure/monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs) теперь можно просматривать данные основной аналитики и сохранять их в нескольких расположениях, включая учетную запись хранения Azure, концентратор событий Azure и (или) [рабочую область Log Analytics (OMS)](https://docs.microsoft.com/azure/log-analytics/log-analytics-get-started). Эта функция доступна для всех конечных точек CDN, относящихся к профилям CDN Verizon (уровни "Стандартный" и "Премиум") и Akamai (уровень "Стандартный").

Журналы диагностики позволяют экспортировать основные метрики использования из конечной точки CDN в различные источники и настраивать их для дальнейшего использования. Например, вы можете выполнять следующие задачи:
...
![Портал — журналы диагностики](./media/cdn-diagnostics-log/OMS-workspace.png)

В приведенных ниже пошаговых инструкциях описана схема данных основной аналитики, действия, необходимые для включения этой функции, доставки данных в различные расположения и их использования в этих расположениях.

## <a name="enable-logging-with-azure-portal"></a>Включение ведения журнала с помощью портала Azure

По умолчанию журналы диагностики **отключены**. Чтобы включить их, выполните следующие действия.


Выполните вход на [портал Azure](http://portal.azure.com). Если в вашем рабочем процессе еще не включена служба CDN, [включите Azure CDN](cdn-create-new-endpoint.md), прежде чем продолжить.
...
2. Выберите профиль CDN, а затем конечную точку CDN, на которой необходимо включить **журналы диагностики**.
    ![Портал — журналы диагностики](./media/cdn-diagnostics-log/Browse-to-Diagnostics-logs.png)
3. Перейдите в колонку **Журналы диагностики** в разделе **Мониторинг** и измените состояние на **Включено**.
    ![Портал — журналы диагностики](./media/cdn-diagnostics-log/Diagnostics-logs-options.png)
4. Выберите и настройте требуемый целевой объект (учетную запись хранения, концентратор событий, Log Analytics) для архивирования данных. 
    
    В этом примере для хранения журналов используется хранилище Azure. Выберите **Архивировать в учетной записи хранения**, выберите количество дней хранения и установите флажок **CoreAnalytics** (Основная аналитика) в разделе **Журнал**. Сейчас мы предлагаем только **основную аналитику**, но в будущем выпустим дополнительные типы журналов. Ниже приведены сведения о схеме, статистической обработке и задержках в средстве основной аналитики. 

    ![Портал — журналы диагностики](./media/cdn-diagnostics-log/Diagnostics-logs-storage.png)
5.  Сохраните новую конфигурацию системы диагностики.
    
    Задержка данных журнала Verizon составляет 1 час. От момента распространения конечной точки до начала отображения данных может пройти до 2 часов.
    Задержка данных журнала Akamai составляет 24 часа. До начала отображения данных, созданных более 24 часов назад, проходит до 2 часов. Если данные созданы недавно, задержка отображения журналов может составить до 25 часов.

## <a name="enable-logging-with-powershell"></a>Включение ведения журнала с помощью PowerShell

Ниже приведены два примера включения и получения журналов диагностики с помощью командлетов Azure PowerShell.
...
## <a name="consuming-logs-from-storage"></a>Использование журналов из хранилища
...
5.    Результаты отобразятся на панели справа, начиная с первого уровня, который обозначен как **resourceId=**. Продолжайте щелкать, пока не отобразится файл **PT1H.json**. См. примечание ниже, в котором описан путь.
...
## <a name="diagnostic-logs-types"></a>Типы журналов диагностики

Сейчас мы предлагаем только журналы основной аналитики, содержащие метрики со статистикой HTTP-ответов и исходящих данных, как видно по точкам присутствия и границам CDN. Со временем мы будем добавлять дополнительные типы журналов.

### <a name="core-analytics-metrics-details"></a>Сведения о метриках основной аналитики
Ниже приведен список метрик, доступных в журналах основной аналитики. Не все метрики доступны у всех поставщиков, хотя различия минимальны. В таблице ниже помимо прочего указано, доступна ли та или иная метрика у определенного поставщика. Обратите внимание, что метрики доступны только для тех конечных точек CDN, на которых есть трафик.
...
Здесь time представляет начало периода, для которого выводятся статистические данные. Обратите внимание: если метрика не поддерживается поставщиком CDN, вместо значения типа double или целого числа используется значение NULL. Это значение указывает на отсутствие метрики и отличается от значения 0. Также обратите внимание, что одному домену, настроенному на конечной точке, соответствует один набор этих метрик.
