<properties linkid="dev-ruby-how-to-blob-storage" urlDisplayName="Служба BLOB-объектов" pageTitle="Использование хранилища BLOB-объектов (Ruby) | Microsoft Azure" metaKeywords="Начало работы с BLOB-объектом Azure, неструктурированные данные Azure, неструктурированное хранилище Azure, BLOB-объект Azure, хранилище BLOB-объектов Azure, BLOB-объект Azure Ruby" description="Узнайте, как использовать службу BLOB-объектов Azure для передачи, загрузки, перечисления и удаления содержания BLOB-объектов. Примеры кода написаны на Ruby." metaCanonical="" services="storage" documentationCenter="Ruby" title="Использование службы BLOB-объектов из Ruby" authors="guayan" solutions="" manager="" editor="" />
...
Дополнительные сведения о BLOB-объектах см. в разделе [Дальнейшие действия](#next-steps).
...
* [Настройка подключения к хранилищу Azure](#SetupStorageConnection)
...
Создайте приложение Ruby. Инструкции см.
в разделе [Создание приложения Ruby в Azure](/ru-ru/develop/ruby/tutorials/web-app-with-linux-vm/).
...
2. Перейдите к учетной записи хранения, которую необходимо использовать.
...
Вы можете просто изменить вызов <strong>create\_container()</strong>, чтобы передать параметр **:public\_access\_level**:
...
* **blob:** полный общий доступ на чтение данных контейнера и BLOB-объекта. Клиенты могут перечислять BLOB-объекты внутри контейнера с помощью анонимного запроса, но не могут перечислять контейнеры в учетной записи хранения.

* **container:** общий доступ на чтение для BLOB-объектов. Данные BLOB-объектов в этом контейнере можно считать с помощью анонимного запроса, но данные контейнера недоступны. Клиенты не могут перечислять BLOB-объекты внутри с помощью анонимного запроса.
