...
мы рекомендуем переход на более новые выпуски продукта: 2015-02-28 — официальная версия API общедоступного выпуска Поиска Azure. Документация по данной версии доступна в [MSDN](https://msdn.microsoft.com/ru-ru/library/azure/dn798933.aspx).

Также мы запускаем следующую предварительную версию [2015-02-28-Предварительная](search-api-2015-02-28-preview.md), представляющую функции, разработка которых еще продолжается. Оставляйте свои отзывы и предложения на [форумах по Поиску Azure](https://social.msdn.microsoft.com/forums/azure/en-US/home?forum=azuresearch) или на нашей [странице отзывов](http://feedback.azure.com/forums/263029-azure-search).
...
В настоящее время такая реализация заменяется новой конструкцией `Suggesters`, определяемой в средстве [индекс](https://msdn.microsoft.com/ru-ru/library/azure/dn798941.aspx) и отслеживающей инфиксные и нечеткие соответствия. Как следует из самих названий, инфиксные и нечеткие соответствия обеспечивают гораздо более широкие возможности сопоставления. Инфиксные соответствия включают в себя и префиксные в том смысле, что по-прежнему отслеживаются соответствия в начале слов, но, кроме этого, отслеживаются и соответствия по всей строке.

Принято решение о прекращении использования предыдущего варианта реализации \(свойство логического типа\). Это означает, что она недоступна ни в одной из версий 2015 г. без обратной совместимости. Это сделано чтобы избежать случайного применения клиентами при построении новых решений. Для того чтобы задействовать функцию опережающего ввода, теперь придется воспользоваться новой конструкцией `Suggesters` при использовании как `2015-02-28`, так и `2015-02-28-Preview`.
...
- [ Новые возможности недавних обновлений Azure Search](../search-latest-updates/)
- [Обзор Поиска Azure](https://msdn.microsoft.com/ru-ru/library/azure/dn798933.aspx)

##Справка##

Версия API `2015-02-28` регулируется Соглашением об уровне обслуживания. Чтобы подать запрос в службу поддержки воспользуйтесь информацией и ссылками, размещенными на [странице](http://azure.microsoft.com/support/options/).
