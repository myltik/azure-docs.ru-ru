<properties 
	pageTitle="Создание веб-сайта на PHP с использованием табличного хранилища Azure - учебник Azure" 
	description="В этом учебнике показывается, как создать веб-сайт на PHP и использовать службу хранилища таблиц Azure в серверном приложении." 
...
* Создайте файл, назвав его **createtable.php**.
...
* Создайте файл, назвав его **additem.php**.
...
У приложения списка задач есть возможность пометить элемент как выполненный, а также снять для него эту пометку. Главная страница передает свойства *RowKey* и *PartitionKey* объекта и его конечное состояние (marked==1, unmarked==0).
...
* Первым действием при передаче объекта является его извлечение из таблицы:
	...
	Как можно видеть, для передаваемого в запрос фильтра используется форма `Key eq 'Value'`. Полное описание синтаксиса запроса доступно [здесь][msdn-table-query-syntax].
