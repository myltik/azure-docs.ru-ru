...
* Затем вызовите метод *createTable*, передав ему имя таблицы. Аналогично другим хранилищам таблиц, не поддерживающим SQL, схема для таблиц Azure не требуется.
	...
	Коды ошибок и соответствующие сообщения см. здесь: [http://msdn.microsoft.com/library/windowsazure/dd179438.aspx][msdn-errors]
...
* Затем вы можете передать объект $entity, который вы только что создали, в метод `insertEntity`:
...
У приложения списка задач есть возможность пометить элемент как выполненный, а также снять для него эту пометку. Главная страница передает свойства *RowKey* и *PartitionKey* объекта и конечное состояние (помеченный==1, непомеченный==0).
...
* Первым действием при передаче объекта является его извлечение из таблицы:
	...
	Как вы видите, для передаваемого в запрос фильтра используется форма "ключ равен значению" (Key eq 'Value'). Полное описание синтаксиса запроса доступно [здесь][msdn-table-query-syntax].
...
2. Щелкните значок **Создать** в нижнем левом углу портала, затем щелкните **Данные + Хранилище** > **Хранилище**. Присвойте учетной записи хранения уникальное имя и создайте новую [группу ресурсов](azure-preview-portal-using-resource-groups.md) для нее.

	![Создание новой учетной записи хранения][storage-quick-create]
	
	После создания новой учетной записи хранения на кнопке **Уведомления** загорится зеленым слово **УСПЕШНО** и откроется колонка учетной записи хранения, в которой будет видно, что учетная запись принадлежит созданной вами группе ресурсов.
...
7. Откройте файл **init.php** и замените параметры [YOUR_STORAGE_ACCOUNT_NAME] и [YOUR_STORAGE_ACCOUNT_KEY] на имя учетной записи и ключ, записанный на последнем шаге Сохраните файл.
...
2. Создайте пустое веб-приложение, следуя инструкциям в статье [Практическое руководство: создание веб-приложения с помощью портала Azure](web-sites-create-deploy.md#createawebsiteportal). Обязательно создайте новый [план службы приложений](azure-web-sites-web-hosting-plans-in-depth-overview) и выберите группу ресурсов, которую вы создали раньше для учетной записи хранения.

	После создания веб-приложения на кнопке **Уведомления** загорится зеленым слово **УСПЕШНО** и откроется колонка веб-приложения, в которой будет видно, что веб-приложение принадлежит созданной вами группе ресурсов.
...
7. Чтобы развернуть локальный репозиторий Git в Azure, сначала настройте учетные данные развертывания. Чтобы настроить учетные данные развертывания, в колонке веб-приложения щелкните **Все настройки** > **Учетные данные развертывания**. Когда завершите настройку, нажмите **Сохранить**.
...
3. Перейдите по адресу **http://[домен веб-приложения]/createtable.php**, чтобы создать таблицу для приложения.
4. Перейдите по адресу **http://[домен веб-приложения]/index.php**, чтобы начать работу с приложением.
...
3. Перейдите по адресу **http://[домен веб-приложения]/index.php**, чтобы просмотреть внесенные изменения. 
