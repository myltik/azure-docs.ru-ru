...
- ExpressRoute
- Хранилище служб восстановления. Не перемещайте ресурсы хранения, а также вычислительные и сетевые ресурсы, связанные с хранилищем служб восстановления. См. раздел [Ограничения служб восстановления](#recovery-services-limitations).
...
## Ограничения служб восстановления

Перемещение не поддерживается для ресурсов хранения, сетевых и вычислительных ресурсов, используемых для настройки аварийного восстановления с помощью Azure Site Recovery.
...
Для перемещения классических ресурсов в новую группу ресурсов **в пределах одной подписки** используйте [портал](#use-portal), [Azure PowerShell](#use-powershell), [интерфейс командной строки Azure](#use-azure-cli) или интерфейс [REST API](#use-rest-api).

Для перемещения **классических ресурсов в новую подписку** необходимо воспользоваться операциями REST, предназначенными для классических ресурсов. Чтобы проверить, может ли подписка участвовать в операции перемещения классических ресурсов между подписками в качестве исходной или целевой подписки, используйте следующее действие:
...
- Сведения о командах Azure CLI для управления подпиской см. в статье [Использование Azure CLI для Mac, Linux и Windows с диспетчером ресурсов Azure](xplat-cli-azure-resource-manager.md).
