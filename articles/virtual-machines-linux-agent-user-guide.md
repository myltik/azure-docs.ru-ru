...
-   **Развернутые приложения SCVMM**
    ...
    -   Обнаружение и загрузка агента VMM для Linux при работе в среде System
...
-   [У различных поставщиков дистрибутивов, работающих с Linux в Azure][У различных поставщиков дистрибутивов, работающих с Linux в Azure]
-   или из [Репозитория Github открытого исходного кода для агента Azure Linux][Репозитория Github открытого исходного кода для агента Azure Linux]
...
-   serialconsole: настройка GRUB для маркировки ttyS0 (первый последовательный порт) в качестве
     консоли загрузки. Это гарантирует, что журналы загрузки ядра отправляются на
     последовательный порт и становятся доступными для отладки.
...
Файл конфигурации (/etc/waagent.conf) управляет действиями waagent.
...
Ниже приводится подробное описание различных параметров конфигурации.
 Существуют параметры конфигурации трех типов: логическое значение, строка или целое число.
...
Введите: Значение целого числа по
 умолчанию: 0
...
### Образец файла топологии роли
  ...
  [Репозитория Github открытого исходного кода для агента Azure Linux]: https://github.com/WindowsAzure/WALinuxAgent
