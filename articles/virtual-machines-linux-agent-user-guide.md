...
Указывает путь, по которому подключен диск ресурсов. Следует отметить, что диск ресурсов является *temporary* и должен быть очищен при отзыве виртуальной машины.
