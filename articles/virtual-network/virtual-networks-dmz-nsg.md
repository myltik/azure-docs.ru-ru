...
Для исходящего трафика стандартное правило разрешает любой трафик в Интернет. В этом примере мы разрешаем исходящий трафик и не меняем никаких исходящих правил. Чтобы применить политику безопасности к трафику в обоих направлениях, требуется определенная пользователем маршрутизация. Этот момент рассмотрен в примере 3 статьи [Облачные службы Microsoft Cloud и сетевая безопасность][HOME].
