...
Вы, наверное, уже знаете, как пользоваться брандмауэрами и списками управления доступом для фильтрации сетевого трафика в сегментах сети, на отдельных компьютерах и сетевых картах на компьютере. Подобным образом можно фильтровать весь сетевой трафик в Azure. Подробнее об этом читайте в этой статье.

- **Списки управления доступом для конечной точки**
	- Предназначены для фильтрации только входящего трафика.
	- Могут использоваться только в конечных точках с доступом в Интернет или через внутренний балансировщик нагрузки.
	- Ограничены 50 правилами списка управления доступом для каждой конечной точки.
	- Виртуальная сеть **НЕ** требуется (классические развертывания).
- **Группы безопасности сети (NSG)**
	- Разрешают или запрещают трафик в зависимости от его направления, протокола, а также адресов и портов источника и назначения.
	- С их помощью можно управлять входящим и исходящим трафиком для ВМ или экземпляров ролей (классические развертывания), сетевых карт (развертывания диспетчера ресурсов) и подсетей (все развертывания). Сюда входят все ресурсы, подключенные к подсети, например облачные службы и среды AppService.
	- Могут применяться только к ресурсам, подключенным к региональной виртуальной сети.
	- **НЕ** требуется управление брандмауэром.
	- Ограничены 100 групп NSG для каждого региона (при этом каждая группа содержит 200 правил).
- **Устройства брандмауэра**
	- Реализованы как виртуальные машины в сети Azure.
	- Разрешают или запрещают трафик в зависимости от его направления, протокола, а также адресов и портов источника и назначения.
	- Предоставляют дополнительные возможности в зависимости от используемого устройства брандмауэра.

Эта статья посвящена группам безопасности сети (NSG). Дополнительные сведения о других вариантах фильтрации трафика можно получить, перейдя по следующим ссылкам.

- [Документация по спискам управления доступом](./virtual-networks-acl.md)
- [Создание сети периметра с помощью групп NSG и устройства брандмауэра](virtual-networks-dmz-nsg-fw-udr-asm.md).

## Как работает группа NSG

Группа NSG содержит правила двух типов: для **входящего** и **исходящего** трафика. Когда трафик направляется на сервер Azure, где размещены виртуальные машины и экземпляры ролей, узел загружает все правила групп NSG для входящего и исходящего трафика в зависимости от его направления. Затем узлы проверяют каждое правило в порядке приоритета. Если правило совпадает с пакетом, который анализирует узел, применяется соответствующее действие правила (разрешить или запретить). Если соответствующие правила для пакета не найдены, он отбрасывается. На следующем рисунке приведена эта схема принятия решений.

![Списки ACL группы NSG](./media/virtual-network-nsg-overview/figure3.png)

>[AZURE.NOTE]Правила, применяемые к данному экземпляру виртуальной машины или роли, могут поступать из нескольких групп NSG. Это случается потому, что группу NSG можно связать с ВМ (классические развертывания), сетевой картой (развертывания диспетчера ресурсов) или подсетью (все развертывания). В разделе [Связывание групп NSG](#Associating-NSGs) рассматриваются способы применения правил из нескольких групп NSG в зависимости от направления трафика.

Группы NSG содержат следующие свойства.

|Свойство|Описание|Ограничения|Рекомендации|
|---|---|---|---|
|Имя|Имя NSG|Должно быть уникальным в пределах региона.<br/>Может содержать буквы, цифры, символы подчеркивания, точки и дефисы.<br/>Должно начинаться с буквы или цифры.<br/>Должно заканчиваться буквами, числами или символами подчеркивания.<br/>Может содержать до 80 символов.|Так как может потребоваться создать несколько групп NSG, убедитесь, что соглашение об именовании позволяет легко определить назначение ваших групп NSG.|
...
|Группа ресурсов|Группа ресурсов, к которой относится группа NSG|Несмотря на то, что группа NSG входит в определенную группу ресурсов, ее можно связать с ресурсами в любой другой группе при условии, что ресурс находится в одном регионе Azure с группой NSG.|Группы ресурсов используются для одновременного управления несколькими ресурсами как единицей развертывания.<br/>Стоит рассмотреть возможность группирования группы NSG со связанными ресурсами.|
|Правила|Правила, которые определяют разрешенный и запрещенный трафик||Дополнительную информацию см. в разделе [Правила группы NSG](#Nsg-rules).| 
...
|Свойство|Описание|Ограничения|Рекомендации|
|---|---|---|---|
|**Имя**|Имя правила|Должно быть уникальным в пределах региона.<br/>Может содержать буквы, цифры, символы подчеркивания, точки и дефисы.<br/>Должно начинаться с буквы или цифры.<br/>Должно заканчиваться буквами, числами или символами подчеркивания.<br/>Может содержать до 80 символов.|Группа NSG может содержать несколько правил. Поэтому обязательно придерживайтесь соглашения об именовании, которое позволяет судить о функции правила.|
...
|**Диапазон портов источника**|Диапазон исходных портов для сопоставления с правилом.|Один номер порта от 1 до 65535, диапазон портов (т. е. от 100 до 2000) или * (все порты).|Старайтесь использовать диапазоны портов, где это возможно, чтобы не задавать несколько правил.|
|**Диапазон портов назначения**|Диапазон портов назначения правила для сопоставления.|Один номер порта от 1 до 65535, диапазон портов (т. е. от 100 до 2000) или * (все порты).|Старайтесь использовать диапазоны портов, где это возможно, чтобы не задавать несколько правил.|
|**Префикс адреса источника**|Префикс адреса источника или тег правила для сопоставления|Один IP-адрес (т. е. 10.10.10.10), IP-подсеть (т. е. 192.168.1.0/24), [тег по умолчанию](#Default-Tags) или * (для всех адресов).|Лучше всего использовать диапазоны, теги и *, чтобы уменьшить количество правил.|
|**Префикс адреса назначения**|Префикс или тег адреса назначения правила для сопоставления|Один IP-адрес (т. е. 10.10.10.10), IP-подсеть (т. е. 192.168.1.0/24), [тег по умолчанию](#Default-Tags) или * (для всех адресов).|Лучше всего использовать диапазоны, теги и *, чтобы уменьшить количество правил.|
...
Теги по умолчанию — это системные идентификаторы для определения категорий IP-адресов. Для свойств **source address prefix** и **destination address prefix** можно использовать теги по умолчанию. Существует три доступных тега по умолчанию.
...
- **AZURE\_LOADBALANCER**. Этот тег обозначает балансировщик нагрузки для инфраструктуры Azure. Он преобразовывается в IP-адрес центра обработки данных Azure, где инициируются проверки работоспособности Azure.

- **INTERNET**. Этот тег обозначает пространство IP-адресов, которые находятся за пределами виртуальной сети и к которым можно получить общий доступ по Интернету. В этот диапазон также входит [пространство общедоступных IP-адресов, принадлежащее Azure](https://www.microsoft.com/download/details.aspx?id=41653).
...
Как показано в таблице правил по умолчанию ниже, входящий и исходящий трафик виртуальной сети разрешен в обоих направлениях. По умолчанию исходящий трафик в Интернет разрешен, входящий трафик из Интернета заблокирован. Есть правило по умолчанию, которое разрешает балансировщику нагрузки Azure инициировать проверку работоспособности виртуальных машин и экземпляров роли. Это правило можно переопределить, если набор балансировки нагрузки не используется.
...
Различные группы NSG можно связать с виртуальной машиной (или сетевой картой в зависимости от модели развертывания) и подсетью, с которой связана сетевая карта или виртуальная машина. В этом случае все правила доступа к сети применяются к трафику в следующем порядке.
...
- **Исходящий трафик**
	...
	3. Группа NSG применяется к подсети.
...
Дополнительные сведения по планированию для обеспечения безопасности сети в Azure см. в статье [Советы по сетевой безопасности облачных служб](best-practices-network-security.md).
...
>[AZURE.IMPORTANT]Перед разработкой решения обязательно ознакомьтесь со всеми [ограничениями, касающимися сетевых служб в Azure](../azure-subscription-service-limits/#networking-limits). Некоторые ограничения можно увеличить, отправив запрос в службу поддержки.
...
- На одном экземпляре виртуальной машины нельзя одновременно использовать списки управления доступом для конечных точек и группы NSG. Если вам нужна группа NSG, но у вас уже есть список ACL для конечных точек, сначала удалите этот список. Сведения о том, как это сделать, см. в статье [Управление списками управления доступом для конечных точек](virtual-networks-acl-powershell.md).
...
|правило;|Access|Приоритет|Диапазон адресов источника|Порт источника|Диапазон адресов назначения|Порт назначения|Протокол|
|---|---|---|---|---|---|---|---|
|Разрешение RDP из подсети переднего плана|Разрешение|100|192\.168.1.0/24|**|*|3389|TCP|
...
|правило;|Access|Приоритет|Диапазон адресов источника|Порт источника|Диапазон адресов назначения|Порт назначения|Протокол|
|---|---|---|---|---|---|---|---|
|Разрешение SQL из подсети переднего плана|Разрешение|100|192\.168.1.0/24|**|*|1433|TCP|
...
- [Создание групп NSG на основе классической модели развертывания](virtual-networks-create-nsg-classic-ps.md).
- [Развертывание группы NSG в диспетчере ресурсов](virtual-networks-create-nsg-arm-pportal.md).
- [Управление журналами групп NSG](virtual-network-nsg-manage-log.md).
