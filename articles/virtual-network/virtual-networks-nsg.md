...
|Регион|Регион Azure, в котором размещается группа NSG|Группы NSG можно применять только к ресурсам в пределах региона, в котором они созданы.|Дополнительную информацию о том, сколько групп безопасности сети может содержать регион, см. в разделе [Ограничения](#Limits) ниже.|
...
|**Диапазон исходных портов**|Диапазон портов источника правила для сопоставления.|Один номер порта от 1 до 65535, диапазон портов (т. е. от 100 до 2000) или * (все порты).|Старайтесь использовать диапазоны портов, где это возможно, чтобы не задавать несколько правил.|
|**Диапазон конечных портов**|Диапазон портов назначения правила для сопоставления.|Один номер порта от 1 до 65535, диапазон портов (т. е. от 100 до 2000) или * (все порты).|Старайтесь использовать диапазоны портов, где это возможно, чтобы не задавать несколько правил.|
|**Префикс исходного адреса**|Префикс исходного адреса или тег правила для сопоставления|Один IP-адрес (т. е. 10.10.10.10), IP-подсеть (т. е. 192.168.1.0/24), [тег по умолчанию](#Default-Tags) или * (для всех адресов).|Лучше всего использовать диапазоны, теги и *, чтобы уменьшить количество правил.|
|**Префикс адреса назначения**|Префикс конечного адреса или тег правила для сопоставления|Один IP-адрес (т. е. 10.10.10.10), IP-подсеть (т. е. 192.168.1.0/24), [тег по умолчанию](#Default-Tags) или * (для всех адресов).|Лучше всего использовать диапазоны, теги и *, чтобы уменьшить количество правил.|
...
Дополнительные сведения о планировании для обеспечения безопасности сети в Azure см. в статье [Облачные службы (Майкрософт) и сетевая безопасность](best-practices-network-security.md).
