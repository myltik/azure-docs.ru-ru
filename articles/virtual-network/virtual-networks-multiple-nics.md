---
title: Создание виртуальной машины с несколькими сетевыми адаптерами
...
* Количество сетевых адаптеров, которые можно создать для виртуальной машины, определяется ее размером. Чтобы определить максимальное число сетевых карт, поддерживаемых каждым из размеров виртуальной машины, см. статьи о размерах виртуальных машин [Windows Server](../virtual-machines/virtual-machines-windows-sizes.md) и [Linux](../virtual-machines/virtual-machines-linux-sizes.md).
...
## Ограничения
При использовании нескольких сетевых карт действуют следующие ограничения.

* Виртуальные машины с несколькими сетевыми адаптерами должны создаваться в виртуальных сетях Azure. Для виртуальных машин без поддержки виртуальной сети нельзя настроить несколько сетевых карт.
* Каждая виртуальная машина в группе доступности должна использовать либо несколько сетевых карт, либо одну сетевую карту. В группе доступности нельзя одновременно использовать виртуальные машины с несколькими сетевыми картами и виртуальные машины с одной сетевой картой. Те же правила действуют для виртуальных машин в облачной службе.
* Чтобы настроить несколько сетевых карт для развернутой виртуальной машины с одной сетевой картой (и наоборот), необходимо удалить, а затем повторно создать эту виртуальную машину.

## Доступ к другим подсетям с использованием дополнительных сетевых карт
