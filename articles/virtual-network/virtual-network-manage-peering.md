...
6. <a name="add-peering"></a>В колонке **Add peering** (Добавить пиринг), введите или выберите значения описанных ниже параметров.
    ...
    - **Разрешить трафик:** установите этот флажок, чтобы разрешить трафик *пересылаемые* из виртуальной сети (который не был получен из виртуальной сети) в поток для этой виртуальной сети через пиринг. Например рассмотрим три виртуальных сетей с именами Spoke1, Spoke2 и концентратора. Пиринг существует между каждой виртуальной сети типа «звезда» и виртуальной сети концентратора, но пиринги не существует между виртуальными сетями типа «звезда». Виртуальное устройство сети развернута в виртуальной сети концентратора и определяемых пользователем маршрутов применяются к каждой виртуальной сети типа «звезда», маршрутизацию трафика между подсетями через сеть виртуальной машины. Если этот флажок не установлен для пиринга между каждой виртуальной сети типа «звезда» и виртуальной сети концентратора, трафик не проходил между виртуальными сетями типа «звезда», так как концентратора fowarding трафика между виртуальными сетями. Если эта возможность активирована, перенаправленный трафик передается с помощью пирингового подключения, но при этом определяемые пользователем маршруты или виртуальные модули сетей не создаются. Определяемые пользователем маршруты или виртуальные модули сетей создаются отдельно. Дополнительные сведения об [определяемых пользователем маршрутах](virtual-networks-udr-overview.md#user-defined).
    - **Разрешить транзит шлюзов.** Установите этот флажок, если к исходной виртуальной сети подключен шлюз и нужно, чтобы трафик из пиринговой виртуальной сети поступал в нее через этот шлюз. Например, исходная виртуальная сеть может быть подключена к локальной сети через шлюз виртуальной сети. Шлюз может быть шлюз ExpressRoute или VPN. Установка этого флажка разрешает трафик с peered виртуальной сети для передачи через шлюз, присоединенного к виртуальной сети к локальной сети. Если этот флажок установлен, в пиринговой виртуальной сети нельзя настроить шлюз. Peered виртуальная сеть должна содержать **использовать удаленный шлюзы** флажок установлен, при настройке пиринг из виртуальной сети в этой виртуальной сети. Если не устанавливать этот флажок (настройка по умолчанию), трафик из пиринговой виртуальной сети по-прежнему поступает в исходную виртуальную сеть, но не проходит через подключенный к ней шлюз. 
    
    Помимо пересылки трафика локальной сети, VPN-шлюза могут пересылать сетевой трафик между виртуальными сетями, которые являются одноранговыми с помощью виртуальной сети, шлюз находится в, без необходимости одноранговыми друг с другом виртуальные сети. Это полезно, если вы хотите использовать VPN-шлюза в концентраторе (см. пример звезда, описанным для **разрешить пересылать трафик**) виртуальной сети для маршрутизации трафика между виртуальными сетями типа «звезда», которые не являются одноранговыми друг с другом. Подробнее о [шлюзах виртуальных сетей](../vpn-gateway/vpn-gateway-about-vpngateways.md?toc=%2fazure%2fvirtual-network%2ftoc.json#s2smulti). Данный сценарий требует реализации определяемых пользователем маршрутов, указывающих шлюз виртуальной сети как тип следующего прыжка. Дополнительные сведения об [определяемых пользователем маршрутах](virtual-networks-udr-overview.md#user-defined). VPN-шлюз можно указать только как тип следующего прыжка маршрута, определяемых пользователем, нельзя указать шлюз ExpressRoute в качестве тип следующего прыжка маршрута, определяемой пользователем.
...
Только один пиринг для этой виртуальной сети может иметь этот параметр включен.
Этот параметр нельзя использовать, если вы уже установили шлюз, настроенный в виртуальной сети.
...
| |Классический|[Участник классической сети](../active-directory/role-based-access-built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#classic-network-contributor)|Н/Д|
