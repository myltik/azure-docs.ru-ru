...
3. В верхней части открывшейся колонки *Создать* в поле **Поиск по Marketplace** введите **виртуальная сеть**.  Когда в результатах поиска появится пункт **Виртуальная сеть**, щелкните его.
4. В открывшейся колонке **Виртуальная сеть** в поле **Выбор модели развертывания** выберите **Классический** и нажмите кнопку **Создать**. 
5. В колонке **Create virtual network (classic)** (Создать виртуальную сеть (классическую)) введите приведенные ниже значения, а затем щелкните **Создать**:
...
5. В открывшейся колонке **Create virtual network (classic)** (Создание виртуальной сети (классическая)) щелкните **Подсети** (в разделе **Параметры**).
6. В открывшейся колонке **myVnet — Подсети** щелкните **+Add** (+Добавить).
7. В колонке **Добавление подсети** для **имени** введите **Частное**. Для **Диапазон адресов** введите **10.0.1.0/24**.  Нажмите кнопку **ОК**.
8. В колонке **myVnet — Подсети** отображаются созданные **общедоступная** и **частная** подсети.
9. **Необязательно.** По завершении работы с этим руководством может потребоваться удалить созданные ресурсы, чтобы за их использование не взималась плата:
    - В колонке **myVnet** щелкните **Обзор**.
    - В колонке **myVnet** щелкните значок **Удаление**.
...
- Создайте виртуальную машину [с Windows](../virtual-machines/windows/classic/createportal.md?toc=%2fazure%2fvirtual-network%2ftoc.json) или [с Linux](../virtual-machines/linux/classic/createportal.md?toc=%2fazure%2fvirtual-network%2ftoc.json), а затем подключите ее к имеющейся виртуальной сети.
