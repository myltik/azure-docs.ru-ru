...
В разделе ссылок приведен сценарий PowerShell, при запуске которого будет построена большая часть описанной выше среды. Сценарий создает также виртуальные машины и сети, но подробного описания этого процесса нет в этом документе.
...
Для исходящего трафика стандартное правило разрешает любой трафик в Интернет. В этом примере мы разрешаем исходящий трафик и не меняем никаких исходящих правил. Для блокировки трафика в обоих направлениях необходима пользовательская маршрутизация. Она рассматривается в другом примере, который можно найти в [основном документе о периметре безопасности][HOME].
...
Сохраните полный сценарий в файл сценария PowerShell. Сохраните конфигурацию сети в файле с именем NetworkConf2.xml. При необходимости измените определенные пользователем переменные. Запустите сценарий, а затем следуйте инструкциям по настройке правил брандмауэра, приведенным выше.
