<properties
   pageTitle="Пример сети периметра. Построение сети периметра для защиты приложений с брандмауэром и группами безопасности сети | Microsoft Azure"
...
В этом примере будет создана сеть периметра с брандмауэром, четырьмя серверами Windows Server и группами безопасности сети. В нем также будут рассмотрены все используемые команды для более глубокого понимания каждого этапа. В разделе "Сценарий прохождения трафика" подробно описано поэтапное прохождение трафика через уровни защиты в сети периметра. Наконец, в разделе ссылок приведены полный код и инструкции по созданию этой среды для тестирования и экспериментов с различными сценариями.
...
- виртуальное сетевое устройство, в этом примере — брандмауэр Barracuda NG Firewall, подключенный к подсети FrontEnd;
- сервер Windows Server, который представляет веб-сервер приложений (IIS01);
- два сервера Windows Server, представляющих внутренние серверы приложения (AppVM01, AppVM02);
- сервер Windows Server, представляющий DNS-сервер (DNS01).

>[AZURE.NOTE]Хотя в этом примере используется брандмауэр Barracuda NG Firewall, вместо него могут быть использовано множество других виртуальных сетевых устройств.

В разделе ссылок приведен сценарий PowerShell, при запуске которого будет построена большая часть описанной выше среды. Создание виртуальных машин и виртуальных сетей, хотя и выполняется с помощью примера сценария, который подробно не описывается в этом документе.
 
Для создания среды

  1.	Сохраните XML-файл конфигурации сети, представленный в разделе ссылок (указав в нем необходимые имена, расположение и IP-адреса в соответствии с данным сценарием)
  2.	Обновите пользовательские переменные в сценарии в соответствии со средой, в которой будет выполняться сценарий (подписки, имена служб и т. д.)
...
2.	В разделе ссылок приведены два сценария для настройки веб-сервера и сервера приложений с простым веб-приложением, с помощью которых можно протестировать эту конфигурацию сети периметра.
...
>[AZURE.TIP]В общем случае, сначала создаются более конкретные правила разрешения и затем более общие правила запрета. Порядок выполнения правил задается назначенным приоритетом. После обнаружения трафика, к которому будет применено конкретное правило, другие правила не выполняются. Правила группы безопасности сети могут применяться во входящем или исходящем направлении (относительно подсети).
...
5.	Весь трафик (для всех портов) из Интернета ко всей виртуальной сети (к обоим подсетям) запрещается
6.	Весь трафик (для всех портов) из подсети FrontEnd в подсеть BackEnd запрещается

С учетом этих правил, связанных с каждой подсетью, если HTTP-запрос поступил из Интернета на веб-сервер, должны быть применены оба правила 3 (Разрешить) и 5 (Запретить), но так как правило 3 имеет более высокий приоритет, то будет применено только оно, и правило 5 не задействуется. Таким образом запрос HTTP сможет достичь брандмауэра. Если тот же трафик пытается достичь сервера DNS01, то сначала будет применено правило 5 (Запретить), и трафику не будет разрешено пройти на сервер. Правило 6 (Запретить) блокирует трафик от подсети FrontEnd к подсети BackEnd (за исключением трафика, разрешенного правилами 1 и 4). Это позволяет защитить сеть Backend при компрометации веб-приложения на сервере FrontEnd злоумышленником. В этом случае злоумышленник получит ограниченный доступ к "защищенной" сети BackEnd (только к ресурсам, предоставляемым на сервере AppVM01).

Исходящего правила по умолчанию, которое разрешает трафик в Интернет, нет. В этом примере мы разрешаем исходящий трафик и не меняем никаких исходящих правил. Для блокировки трафика в обоих направлениях необходима пользовательская маршрутизация. Она рассматривается в другом примере, который можно найти в [основном документе о периметре безопасности][HOME].

Описанные выше правила групп безопасности сети очень похожи на правила в [Примере 1. Построение простой сети периметра с группами безопасности сети][Example1]. Пожалуйста, ознакомьтесь с описанием групп безопасности сети в этом документе, чтобы получить подробное представление о каждом правиле группы безопасности сети и его атрибутах.
...
Инструкции по загрузке клиента и подключению к брандмауэру Barracuda, который используется в этом примере, можно найти здесь: [Администрирование Barracuda NG](https://techlib.barracuda.com/NG61/NGAdmin)

В брандмауэре необходимо создать правила перенаправления. Поскольку в этом примере маршрутизируется только входящий трафик для брандмауэра и затем для веб-сервера, потребуется только одно правило перенаправления NAT. В брандмауэре Barracuda NG Firewall, используемом в этом примере, это будет правило назначения NAT для передачи этого трафика.
...
>[AZURE.IMPORTANT]Важно понимать, что вы не обращаетесь напрямую к веб-серверу. Когда браузер запрашивает страницу HTTP из FrontEnd001.CloudApp.Net, конечная точка HTTP (порт 80) передает этот трафик брандмауэру, а не веб-серверу. Затем брандмауэр в соответствии с правилом, созданным выше, передает этот запрос на веб-сервер.

## Сценарии прохождения трафика

#### (Разрешено) Запрос из Интернета к веб-серверу через брандмауэр
...
3.	Подсеть FrontEnd начинает обработку входящего правила:
...
7.	IIS01 запрашивает сведения у SQL Server на AppVM01
8.	Исходящих правил в подсети FrontEnd нет, трафик разрешается
9.	Подсеть BackEnd начинает обработку входящего правила:
  1.	Правило 1 группы безопасности сети (DNS) не применяется, переход к следующему правилу
  2.	Правило 2 группы безопасности сети (RDP) не применяется, переход к следующему правилу
  3.	Правило 3 группы безопасности сети (Подключение из Интернета к брандмауэру) не применяется, переход к следующему правилу
  4.	Правило 4 группы безопасности сети (Подключение от IIS01 к AppVM01) применяется, трафик разрешается, обработка правил заканчивается
10.	AppVM01 получает SQL-запрос и отправляет ответ
11.	Поскольку исходящих правил для подсети BackEnd нет, отправка ответа разрешается
12.	Подсеть FrontEnd начинает обработку входящего правила:
  1.	Так как к входящему трафику от сети BackEnd в сеть FrontEnd не применяются правила группы безопасности сети, не будет применено ни одно из правил группы безопасности сети
...
#### (Разрешено) Запрос сеанса RDP к подсети BackEnd
...
3.	Подсеть BackEnd начинает обработку входящего правила:
  1.	Правило 1 группы безопасности сети (DNS) не применяется, переход к следующему правилу
  2.	Правило 2 группы безопасности сети (RDP) применяется, трафик разрешается, обработка правил заканчивается
4.	Исходящих правил нет, поэтому применяются правила по умолчанию и возвратный трафик разрешается
5.	Сеанс RDP включается
6.	AppVM01 запрашивает имя пользователя и пароль

#### (Разрешено) DNS-запрос веб-сервера к серверу DNS
1.	Веб-серверу IIS01 необходим поток данных с сайта www.data.gov, но ему необходимо разрешить адрес.
2.	В конфигурации сети для виртуальной сети в качестве основного сервера DNS указан DNS01 (10.0.2.4 в подсети BackEnd). Веб-сервер IIS01 отправляет запрос DNS-серверу DNS01
3.	Исходящих правил в подсети FrontEnd нет, трафик разрешается
4.	Подсеть BackEnd начинает обработку входящего правила:
  1.	 Правило 1 группы безопасности сети (DNS) применяется, трафик разрешается, обработка правил заканчивается
...
7.	Исходящих правил в подсети Backend нет, трафик разрешается
8.	Сервер DNS в Интернете отправляет ответ, так как этот сеанс был инициирован внутри системы, ответ разрешается
9.	DNS-сервер кэширует ответ и отправляет ответ на исходный запрос обратно IIS01
10.	Исходящих правил в подсети Backend нет, трафик разрешается
11.	Подсеть FrontEnd начинает обработку входящего правила:
  1.	Так как к входящему трафику от сети BackEnd в сеть FrontEnd не применяются правила группы безопасности сети, не будет применено ни одно из правил группы безопасности сети
  2.	Системное правило по умолчанию, которое разрешает трафик между подсетями, разрешает прохождение этого трафика, поэтому трафик разрешается.
12.	IIS01 получает ответ от DNS01

#### (Разрешено) Веб-сервер обращается к файлу на AppVM01
1.	IIS01 запрашивает файл на AppVM01
2.	Исходящих правил в подсети FrontEnd нет, трафик разрешается
3.	Подсеть BackEnd начинает обработку входящего правила:
  1.	Правило 1 группы безопасности сети (DNS) не применяется, переход к следующему правилу
  2.	Правило 2 группы безопасности сети (RDP) не применяется, переход к следующему правилу
  3.	Правило 3 группы безопасности сети (Подключение из Интернета к брандмауэру) не применяется, переход к следующему правилу
  4.	Правило 4 группы безопасности сети (Подключение от IIS01 к AppVM01) применяется, трафик разрешается, обработка правил заканчивается
4.	AppVM01 получает запрос и отправляет ответ в виде файла (предполагается, что доступ авторизован)
5.	Поскольку исходящих правил для подсети BackEnd нет, отправка ответа разрешается
6.	Подсеть FrontEnd начинает обработку входящего правила:
  1.	Так как к входящему трафику от сети BackEnd в сеть FrontEnd не применяются правила группы безопасности сети, не будет применено ни одно из правил группы безопасности сети
  2.	Системное правило по умолчанию, которое разрешает трафик между подсетями, разрешает прохождение этого трафика, поэтому трафик разрешается.
7.	Сервер IIS получает запрос

#### (Запрещено) Запрос из Интернета напрямую к веб-серверу
...
#### (Запрещено) Запрос из Интернета к серверу BackEnd
1.	Интернет-пользователь пытается получить доступ к файлу на AppVM01 через службу BackEnd001.CloudApp.Net
2.	Поскольку для общей папки не открыты конечные точки, этот запрос не будет передан облачной службе и не достигнет сервера
...
#### (Запрещено) DNS-запрос из Интернета к серверу DNS
1.	Интернет-пользователь пытается выполнить запрос внутренней записи DNS на DNS01 через службу BackEnd001.CloudApp.Net
2.	Так как для DNS-сервера не открыты конечные точки, то этот запрос не будет передан облачной службе и не достигнет сервера
...
#### (Запрещено) Запрос из Интернета к SQL через брандмауэр
1.	Интернет-пользователь данные SQL из FrontEnd001.CloudApp.Net (облачная служба, открытая для доступа через Интернет)
2.	Поскольку для данных SQL не открыты конечные точки, этот запрос не будет передан облачной службе и не достигнет брандмауэра
3.	Если конечные точки были открыты по какой-либо причине, подсеть FrontEnd начинает обработку входящего правила:
  1.	Правило 1 группы безопасности сети (DNS) не применяется, переход к следующему правилу
  2.	Правило 2 группы безопасности сети (RDP) не применяется, переход к следующему правилу
...
## Ссылки
### Основной сценарий и конфигурация сети
Сохраните полный сценарий в файл сценария PowerShell. Сохраните конфигурацию сети в файле с именем NetworkConf2.xml. При необходимости измените определенные пользователем переменные. Запустите сценарий, а затем выполните настройки правил брандмауэра в соответствии с инструкциями выше.

#### Полный сценарий
Этот сценарий на основе определенных пользователем переменных выполнит следующие действия:

1.	подключение к подписке Azure
2.	Создание новой учетной записи хранения
3.	Создайте новую виртуальную сеть и две подсети, как определено в файле конфигурации сети
4.	Создайте 4 виртуальные машины Windows Server
5.	Настройте группы безопасности сети, выполнив следующие действия:
  -	Создайте группу безопасности сети
  -	Заполните ее правилами
  -	Привяжите группу безопасности сети к соответствующим подсетям

Этот сценарий PowerShell должен запускаться локально на компьютере или сервере, подключенном к Интернету.

>[AZURE.IMPORTANT]При запуске этого сценария могут выдаваться предупреждения и другие информационные сообщения PowerShell. Обращать внимание следует только на сообщения об ошибках, выделенные красным цветом.
...
Сохраните этот файл XML с обновленным расположением и добавьте ссылку на этот файл в переменную $NetworkConfigFile в приведенном выше сценарии.
...
[2]: ./media/virtual-networks-dmz-nsg-fw-asm/dstnaticon.png "Значок правила назначения NAT"
