...
2. Следующая команда возвращает все правила NSG, применяемые к сетевому интерфейсу с именем *VM1-NIC1* в группе ресурсов *RG1*.
   ...
   * Имена правил, которые начинаются с *defaultSecurityRules*: при создании каждой NSG в ней создаются несколько правил безопасности по умолчанию. Правила по умолчанию нельзя удалить, но они могут быть переопределены правилами с более высоким приоритетом. Ознакомьтесь с дополнительными сведениями о [правилах безопасности по умолчанию](security-overview.md#default-security-rules).
