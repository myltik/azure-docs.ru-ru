...
Если вы знакомы с классической моделью развертывания, ознакомьтесь с [различиями в IP-адресации для классической модели развертывания и диспетчера ресурсов](virtual-network-ip-addresses-overview-classic.md#Differences-between-Resource-Manager-and-classic-deployments).
...
- конечным пользователям нужно обновить правила брандмауэра для взаимодействия с ресурсами Azure;
...
### Виртуальные машины
Чтобы связать общедоступный IP-адрес с [виртуальной машиной](virtual-machines-about.md), назначьте этот адрес ее **сетевому адаптеру**. Если в виртуальной машине несколько сетевых адаптеров, IP-адрес достаточно назначить *основному* адаптеру. Виртуальной машине можно назначить динамический или статический общедоступный IP-адрес.

### Балансировщики нагрузки, доступные в Интернете
Чтобы связать общедоступный IP-адрес с [балансировщиком нагрузки Azure](load-balancer-overview.md), назначьте этот адрес для конфигурации **внешнего интерфейса** балансировщика. Этот общедоступный IP-адрес будет выполнять функцию виртуального IP-адреса с балансировкой нагрузки. Конфигурации внешнего интерфейса балансировщика можно назначить динамический или статический общедоступный IP-адрес. Кроме того, подобной конфигурации можно назначить несколько общедоступных IP-адресов. Это позволит вам создавать сценарии с [несколькими виртуальными IP-адресами](load-balancer-multivip.md) (например, среду с несколькими клиентами и веб-сайтами на основе SSL).

### VPN-шлюзы
Подключить виртуальную сеть Azure к другим виртуальным сетям Azure или локальным сетям можно с помощью [VPN-шлюза](vpn-gateway-about-vpngateways.md) Azure. Чтобы включить связь с удаленной сетью, нужно назначить общедоступный IP-адрес **IP-конфигурации** шлюза. В настоящее время VPN-шлюзу можно назначить только динамический общедоступный IP-адрес.

### Шлюзы приложений
Чтобы связать общедоступный IP-адрес со [шлюзом приложений](application-gateway-introduction.md) Azure, назначьте этот адрес конфигурации **внешнего интерфейса** шлюза. Этот общедоступный IP-адрес будет выполнять функцию виртуального IP-адреса с балансировкой нагрузки. Сейчас конфигурации, заданной для внешнего интерфейса шлюза приложений, можно назначить только *динамический* общедоступный IP-адрес. Кроме того, вы можете назначить несколько общедоступных IP-адресов. Это позволяет создавать сценарии с несколькими виртуальными IP-адресами.
...
|---|---|---|---|
|Сетевой адаптер (виртуальной машины)|Да|Да|Нет|
|Интерфейсная часть балансировщика нагрузки|Да|Да|Да|
|VPN-шлюз|Да|Нет|Нет|
|Интерфейсная часть шлюза приложений|Да|Нет|Нет|
...
В модели развертывания диспетчера ресурсов Azure частный IP-адрес связан с разными ресурсами Azure.
...
### Виртуальные машины
Частный IP-адрес назначается **сетевому адаптеру** [виртуальной машины](virtual-machines-about.md). В виртуальной машине с несколькими сетевыми адаптерами IP-адрес назначается каждому сетевому адаптеру. Способ выделения адаптера может быть динамическим или статическим.
...
Когда вы создаете виртуальную машину, в DNS-серверы, находящиеся под управлением Azure, добавляется сопоставление имени узла с частным IP-адресом. Если используется виртуальная машина с несколькими сетевыми адаптерами, имя узла сопоставляется с частным IP-адресом главного сетевого адаптера.
...
|Ресурс|Статическое|Динамический|Несколько IP-адресов|
|---|---|---|---|
...
В следующей таблице показаны ограничения, накладываемые на IP-адресацию в Azure в зависимости от региона и подписки. Для увеличения ограничений по умолчанию до максимальных ограничений в зависимости от потребностей вашего бизнеса вы можете [обратиться в службу поддержки](https://portal.azure.com/#blade/Microsoft_Azure_Support/HelpAndSupportBlade).

||Ограничение по умолчанию|Максимальное ограничение| |---|---|---| |Общедоступные IP-адреса (динамические)|60|связаться со службой поддержки| |Общедоступные IP-адреса (статические)|20|связаться со службой поддержки| |Общедоступный внешний IP-адрес на балансировщик нагрузки|5|связаться со службой поддержки| |Частный внешний IP-адрес на балансировщик нагрузки|1|связаться со службой поддержки|
...
Таким образом, к общедоступным IP-ресурсам применяется следующая структура ценообразования:

- VPN-шлюзы и шлюзы приложений используют только один динамический общедоступный IP-адрес, который предоставляется бесплатно.
- Виртуальные машины используют только один общедоступный IP-адрес, который предоставляется бесплатно, в том случае, если это динамический IP-адрес. Если виртуальная машина использует статический общедоступный IP-адрес, его использование тарифицируется как использование статического (зарезервированного) общедоступного IP-адреса.
- Каждый балансировщик нагрузки может использовать несколько общедоступных IP-адресов. Первый общедоступный IP-адрес предоставляется бесплатно. За дополнительные динамические IP-адреса взимается плата в размере 0,004 доллара в час. Статические общедоступные IP-адреса тарифицируются как статические (зарезервированные) общедоступные IP-адреса.
- Использование зарезервированных общедоступных IP-адресов: 
	- Первые 5 адресов (используемые) предоставляются бесплатно. За дополнительные статические общедоступные IP-адреса взимается плата в размере 0,004 доллара в час. 
	- За статические общедоступные IP-адреса, не привязанные к ресурсам, взимается плата в размере 0,004 доллара в час.
