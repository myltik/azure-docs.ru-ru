---
title: Как задать статический частный IP-адрес в режиме ARM с помощью портала Azure | Microsoft Docs
...
> [!NOTE]
> Если после нажатия кнопки **Сохранить** вы заметите, что назначение по-прежнему остается **динамическим**, это означает, что введенный IP-адрес уже используется. Попробуйте другой IP-адрес.
> 
> 
