...
>[AZURE.NOTE] Можно также использовать `Get-AzureRmNetworkSecurityGroup -ResourceGroupName RG-NSG -Name "NSG-FrontEnd" | Select DefaultSecurityRules -ExpandProperty DefaultSecurityRules` для получения списка правил по умолчанию из группы безопасности сети **NSG-FrontEnd**.
...
4. Чтобы удалить группу безопасности сети, выполните командлет `Remove-AzureRmNetworkSecurityGroup`, как показано ниже.
	...
	>[AZURE.NOTE] Параметр **-Force** гарантирует, что удаление не потребуется подтверждать.
