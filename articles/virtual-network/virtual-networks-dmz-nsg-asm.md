---
title: "Пример сети периметра. Создание простой сети периметра с группами безопасности сети | Документация Майкрософт"
...
---
# <a name="example-1-build-a-simple-dmz-with-nsgs"></a>Пример 1. Построение простой сети периметра с группами безопасности сети
[Вернуться на страницу с советами и рекомендациями по созданию периметра безопасности][Домашняя страница]
...
Ниже приведены пояснения по каждому правилу. (**Примечание.** Все объекты, в начале имени которых стоит знак доллара (например, $NSGName), являются пользовательскими переменными из скрипта, который приведен в разделе справочной информации ниже.)
...
2. Первое правило в этом примере разрешает трафик DNS между всеми внутренними сетями к DNS-серверу во внутренней подсети. У правила есть несколько важных параметров.
   ...
   * Параметр Action определяет, что нужно делать с трафиком, который подпадает под действие правила, — запретить или разрешить.
         ...
         Get-AzureNetworkSecurityGroup -Name $NSGName | `
             ...
             -Protocol *
3. Это правило пропускает трафик RDP из Интернета к порту RDP на любом сервере в подсети или виртуальной сети. В правиле используется два специальных типа адресных префиксов, VIRTUAL_NETWORK и INTERNET, что позволяет работать с большими категориями адресных префиксов.
...
#### <a name="allowed-web-to-web-server"></a>Запрос из Интернета к веб-серверу (*разрешено*)
...
#### <a name="allowed-rdp-to-backend"></a>Запрос через протокол RDP к подсети BackEnd (*разрешено*)
...
#### <a name="denied-web-to-backend-server"></a>Запрос из Интернета к тыловому серверу (*запрещено*)
...
#### <a name="denied-web-dns-lookup-on-dns-server"></a>DNS-запрос из Интернета к серверу DNS (*запрещено*)
...
#### <a name="denied-web-to-sql-access-through-firewall"></a>Запрос данных SQL из Интернета через брандмауэр (*запрещено*)
...
Дополнительные примеры и общие сведения о периметре безопасности сети можно найти [здесь][Домашняя страница].
...
При необходимости измените определенные пользователем переменные. Запустите сценарий, а затем следуйте инструкциям по настройке правил брандмауэра, приведенным в предыдущем разделе с примером 1.
...
<!--Link References-->
[Домашняя страница]: ../best-practices-network-security.md
