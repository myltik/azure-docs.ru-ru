<properties 
   pageTitle="Устранение неполадок маршрутов — PowerShell | Microsoft Azure"
...
# Устранение проблем с маршрутами с помощью Azure PowerShell
...
2.  Следующая команда возвращает все маршруты, применяемые к сетевому интерфейсу с именем *VM1-NIC1* в группе ресурсов *RG1*.
	...
	>[AZURE.TIP] Если вы не знаете имя сетевого интерфейса, введите следующую команду, чтобы извлечь имена всех сетевых интерфейсов в группе ресурсов.*
