---
title: "Создание, изменение или удаление подсетей в виртуальных сетях Azure | Документы Майкрософт"
description: "Сведения о создании, изменении и удалении подсетей виртуальной сети в Azure."
...
---
# <a name="create-change-or-delete-a-virtual-network-subnet"></a>Создание, изменение и удаление подсети в виртуальной сети

Сведения о создании, изменении и удалении подсетей виртуальной сети. 

Если вы не работали с виртуальными сетями, прежде чем создавать, изменять или удалять подсеть, мы рекомендуем прочитать [обзор виртуальных сетей Azure](virtual-networks-overview.md) и статью [Создание, изменение или удаление виртуальных сетей](virtual-network-manage-network.md). Все ресурсы Azure, которые могут подключаться к виртуальной сети, подключаются к подсети в виртуальной сети. Как правило, в виртуальной сети создается несколько подсетей для решения следующих задач.
...
В этой статье описывается, как создавать, изменять и удалять подсети для виртуальных сетей, созданных на основе модели развертывания Azure Resource Manager.
 
## <a name="before"></a>Перед началом работы

Перед началом выполнения процедур, описанных в этой статье, выполните следующие условия.
...
- Чтобы получить сведения об ограничениях для виртуальных сетей, просмотрите [ограничения Azure](../azure-subscription-service-limits.md?toc=%2fazure%2fvirtual-network%2ftoc.json#azure-resource-manager-virtual-networking-limits).
- Войдите на портал Azure, в средство командной строки Azure (Azure CLI) или Azure PowerShell с помощью учетной записи Azure. Если у вас нет учетной записи Azure, зарегистрируйтесь для получения [бесплатной пробной учетной записи](https://azure.microsoft.com/free).
- Если вы планируете использовать команды PowerShell для выполнения задач в этой статье, сначала необходимо [установить и настроить Azure PowerShell](/powershell/azureps-cmdlets-docs?toc=%2fazure%2fvirtual-network%2ftoc.json). Убедитесь, что у вас установлена самая последняя версия командлетов Azure PowerShell. Для получения справки по командам PowerShell в этих примерах введите `get-help <command> -full`.
- Если вы планируете использовать команды Azure CLI для выполнения задач в этой статье, сначала необходимо [установить и настроить Azure CLI](/cli/azure/install-azure-cli?toc=%2fazure%2fvirtual-network%2ftoc.json). Убедитесь, что у вас установлена самая последняя версия Azure CLI. Чтобы получить справку по командам Azure CLI, введите `az <command> --help`.

## <a name="create-subnet"></a>Создание подсети

Создание подсети:

1. Войдите на [портал](https://portal.azure.com) с помощью учетной записи, которой назначены разрешения роли "Участник сети" (как минимум) для подписки. Сведения о назначении ролей и разрешений учетным записям см. в статье [Встроенные роли для управления доступом на основе ролей в Azure](../active-directory/role-based-access-built-in-roles.md?toc=%2fazure%2fvirtual-network%2ftoc.json#network-contributor).
...
Вы можете изменять группы безопасности сети, таблицы маршрутов и права доступа пользователей к подсети, управляя ресурсами, подключенными к подсети. Дополнительные сведения об этих параметрах см. в шаге 6, [Создание подсети](#create-subnet). Если вы хотите изменить адресное пространство подсети, необходимо сначала удалить все ресурсы, которые подключены к этой подсети. Конкретная процедура удаления ресурса зависит от типа ресурса. Чтобы узнать, как удалить ресурсы, подключенные к подсетям, ознакомьтесь с документацией по типам ресурсов, которые требуется удалить. Изменение диапазона адресов для подсети
...
Подсеть можно удалить только в том случае, если к ней не подключены какие-либо ресурсы. В противном случае необходимо удалить ресурсы, подключенные к подсети, и только потом — саму подсеть. Конкретная процедура удаления ресурса зависит от типа ресурса. Чтобы узнать, как удалить ресурсы, подключенные к подсетям, ознакомьтесь с документацией по типам ресурсов, которые требуется удалить. Удаление подсети
...
Сведения о том, как создать виртуальную машину, а затем подключить ее к подсети, см. в статье [Создание виртуальной сети и подключение виртуальных машин](virtual-network-get-started-vnet-subnet.md#a-namecreate-vmsacreate-virtual-machines).
