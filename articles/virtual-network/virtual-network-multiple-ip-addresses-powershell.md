...
# Назначение виртуальным машинам нескольких IP-адресов
...
2. Измените значения следующих переменных со знаком "$" на необходимое имя сетевого интерфейса, [группы ресурсов](../resource-group-overview.md#resource-groups), которой необходимо его назначить, а также имя [расположения](https://azure.microsoft.com/regions) Azure, в котором его необходимо создать.
...
3. <a name="subnet"></a>Сетевой интерфейс необходимо подключить к подсети в рамках имеющейся виртуальной сети Azure (VNet), которая находится в том же расположении и в рамках той же [подписки](../azure-glossary-cloud-terminology.md#subscription), что и сетевой интерфейс. Дополнительные сведения о виртуальных сетях и их создании см. в статьях [Обзор виртуальной сети](virtual-networks-overview.md) и [Создание виртуальной сети с помощью PowerShell](virtual-networks-create-vnet-arm-ps.md). Измените следующие значения переменных со знаком "$" на имя виртуальной сети и подсети, к которым необходимо подключить сетевой интерфейс, и имя группы ресурсов, в которой находится виртуальная сеть.
	...
	Если возвращен пустой список, необходимо создать виртуальную сеть. Чтобы узнать, как это сделать, см. статью [Создание виртуальной сети с помощью PowerShell](virtual-networks-create-vnet-arm-ps.md).
...
5. <a name="ipconfigs"></a>Определите IP-конфигурации, которые необходимо назначить сетевому интерфейсу. В каждой конфигурации может быть один статический или динамический частный IP-адрес и один ресурс общедоступного IP-адреса, связанный со статическим или динамическим адресом.
	...
	В следующих примерах конфигурации будут созданы и назначены сетевому интерфейсу с тремя частными IP-адресами и одним назначенным ему общедоступным IP-адресом:
	...
	**IPConfig-2**
	...
	Измените *IPConfig-2* на имя, которое необходимо присвоить второй IP-конфигурации, и измените *10.0.0.5* на неиспользуемый допустимый IP-адрес для подсети, которой назначается сетевой интерфейс. Введите следующие команды:
...
9. <a name="os"></a>Добавьте вручную все вторичные частные IP-адреса (IP-адреса со значением *False* в столбце **Primary** в выходных данных на предыдущем шаге) для конфигурации TCP/IP в операционной системе. Частный IP-адрес, назначенный *IPConfig-1* на шаге 5, автоматически назначается операционной системе через DHCP Azure, так как это *первичная* конфигурация.
	...
	**Windows**
	...
	2. Откройте окно свойств для сетевого адаптера.
	3. Откройте окно свойств для **IP версии 4**:
		- Щелкните **Использовать следующий IP-адрес** и введите следующие значения:
			...
			- **Шлюз по умолчанию** — первый IP-адрес в подсети. Если подсеть — 10.0.0.0/24m, то IP-адрес шлюза по умолчанию — 10.0.0.1.
		...
		- Нажмите кнопку **ОК**, чтобы закрыть настройки TCP/IP, а затем нажмите кнопку **ОК** еще раз, чтобы закрыть параметры адаптера.
	4. В командной строке введите *ipconfig /all*. Отобразятся все добавленные IP-адреса, а DHCP будет отключен.
...
## <a name="add"></a>Добавление IP-адресов в имеющуюся виртуальную машину
...
3. Создайте переменную и назначьте ее сетевому интерфейсу, введя следующую команду:
