...
Невозможно создать пиринг между двумя виртуальными сетями, созданными с помощью классической модели развертывания. В этом руководстве используются виртуальные сети, существующие в одном и том же регионе. Возможность установления пиринговой связи между виртуальными сетями в разных регионах находится на этапе предварительной версии. Чтобы использовать эту возможность, необходимо [зарегистрироваться](#register). 

При создании пиринга между виртуальными сетями, которые существуют в разных подписках, эти подписки должны быть связаны с одним клиентом Azure Active Directory. Если у вас еще нет клиента Azure Active Directory, можно быстро [создать его](../active-directory/develop/active-directory-howto-tenant.md?toc=%2fazure%2fvirtual-network%2ftoc.json#start-from-scratch). Вы можете соединить виртуальные сети в разных подписках и с разными клиентами Azure Active Directory, используя [VPN-шлюз](../vpn-gateway/vpn-gateway-howto-vnet-vnet-resource-manager-portal.md?toc=%2fazure%2fvirtual-network%2ftoc.json) Azure.
