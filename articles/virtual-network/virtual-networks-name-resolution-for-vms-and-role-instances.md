...
---
# <a name="name-resolution-for-virtual-machines-and-role-instances"></a>Разрешение имен для виртуальных машин и экземпляров ролей
...
| --- | --- | --- |
| Разрешение имен между экземплярами ролей или виртуальными машинами, расположенными в одной облачной службе или виртуальной сети. | [Частные зоны Azure DNS](../dns/private-dns-overview.md) или [разрешение имен Azure](#azure-provided-name-resolution) |Имя узла или полное доменное имя |
| Разрешение имен между экземплярами ролей или виртуальными машинами, расположенными в разных виртуальных сетях. |[Частные зоны Azure DNS](../dns/private-dns-overview.md) или управляемые клиентами DNS-серверы, перенаправляющие запросы между виртуальными сетями для разрешения в Azure (DNS-прокси). См. раздел [Разрешение имен с помощью собственного DNS-сервера](#name-resolution-that-uses-your-own-dns-server). |только полное доменное имя |
...
* Можно использовать разрешение имен между экземплярами ролей или виртуальными машинами, расположенными в одной облачной службе, без необходимости указывать полное доменное имя.
...
Ваши требования к разрешению имен могут выйти за рамки возможностей Azure. Например, это может иметь место при использовании доменов Active Directory или при потребности в разрешении DNS между виртуальными сетями. Для этих сценариев Azure предоставляет возможность использовать ваши собственные DNS-серверы.
...
* Предоставлять корректное разрешение имен узла, например с помощью [DDNS](virtual-networks-name-resolution-ddns.md). Обратите внимание, что при использовании DDNS может потребоваться отключить очистку записей DNS. Время аренды Azure DHCP долгое, и очистка может преждевременно удалить записи DNS. 
...
Предположим, вам необходимо выполнять разрешение имен из веб-приложения, созданного с помощью службы приложений, которое связано с виртуальной сетью, в виртуальные машины в другой виртуальной сети. Для этого потребуются пользовательские DNS-серверы в обеих виртуальных сетях, как описано ниже. 
...
При использовании ваших DNS-серверов Azure предоставляет возможность указать несколько серверов для каждой виртуальной сети. Это также можно сделать для каждого сетевого интерфейса (для Azure Resource Manager) или каждой облачной службы (для классической модели развертывания). DNS-серверы, указанные для облачной службы или сетевого интерфейса, имеют более высокий приоритет по сравнению с DNS-серверами, указанными для виртуальной сети.
...
При использовании модели развертывания с помощью Azure Resource Manager DNS-серверы можно указать на портале Azure. Ознакомьтесь с разделами [Virtual Networks](https://msdn.microsoft.com/library/azure/mt163661.aspx) (Виртуальные сети) и [Network Interfaces](https://msdn.microsoft.com/library/azure/mt163668.aspx) (Сетевые интерфейсы). Кроме того, это можно сделать в PowerShell. Ознакомьтесь с разделами [Virtual Networks](/powershell/module/AzureRM.Network/New-AzureRmVirtualNetwork) (Виртуальные сети) и [Network Interfaces](/powershell/module/azurerm.network/new-azurermnetworkinterface) (Сетевые интерфейсы).
