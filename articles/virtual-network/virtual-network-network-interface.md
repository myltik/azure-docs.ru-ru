...
- [Сетевые интерфейсы.](#nics) Сетевая карта подключена к одной подсети в виртуальной сети Azure. На рисунке к виртуальной машине **VM1** подключены две сетевые карты, а к виртуальной машине **VM2** — одна. Каждая сетевая карта подключена к одной виртуальной сети, но к разным подсетям. В этом разделе содержится процедура вывода списка существующих сетевых карт, а также процедура их создания, изменения и удаления.
...
>[!WARNING]
>Чтобы изменить частный IP-адрес дополнительной IP-конфигурации, связанной с дополнительной сетевой картой, описанные выше действия следует выполнить только после остановки и отмены распределения виртуальной машины.
...
- В прошлом сетевые адаптеры можно было добавлять только к виртуальным машинам, которые поддерживали несколько сетевых адаптеров и были созданы с по крайней мере двумя сетевыми адаптерами. Сетевой адаптер нельзя было добавить к виртуальной машине, которая была создана с одним сетевым адаптером, даже если размер виртуальной машины поддерживал несколько сетевых адаптеров. И наоборот, сетевые адаптеры можно было удалять только из виртуальной машины с по крайней мере тремя подключенными сетевыми адаптерами, так как виртуальные машины, созданные по крайней мере с двумя виртуальными адаптерами, всегда должны были иметь по крайней мере два подключенных сетевых адаптера. Ни одно из этих ограничений больше не действует. Теперь можно создать виртуальную машину с любым количеством сетевых адаптеров (не превышающим поддерживаемое размером виртуальной машины), а затем добавлять и удалять любое количество сетевых адаптеров, но так, чтобы виртуальная машина всегда имела по крайней мере один подключенный сетевой адаптер. 
