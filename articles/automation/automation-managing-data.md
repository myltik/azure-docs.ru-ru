<properties 
   pageTitle="Управление данными службы автоматизации Azure"
...
Служба автоматизации Azure поддерживает георепликацию. Благодаря георепликации служба автоматизации Azure надежно хранит ваши данные в двух регионах. При создании учетной записи службы автоматизации на портале Azure можно выбрать регион, где она будет создана. Это ее основной регион. Регион, куда геореплицируются данные, называется дополнительным регионом. Основной и дополнительный регионы обмениваться данными, чтобы геореплицировать обновления, внесенные в учетной записи службы автоматизации. Так как в дополнительном регионе хранится копия информации, при отработке отказа учетной записи службы автоматизации и переключении с основного региона на дополнительный все данные учетной записи службы автоматизации будет по-прежнему будут доступны в дополнительном регионе.

Георепликация встроена в учетные записи службы автоматизации и не требует дополнительных затрат. Вам не нужно контролировать выбор дополнительного региона, он автоматически определяется по выбранному основному региону.

 
###Расположение геореплик

Сейчас учетные записи службы автоматизации могут быть созданы в приведенных ниже пять регионах. В будущем будет добавлена поддержка дополнительных регионов. В таблице ниже показаны пары основных и дополнительных регионов.
...
###Аварийное восстановление в службе автоматизации Azure

Если серьезная авария влияет на основной регион, сначала команда разработчиков для службы автоматизации пытается восстановить его работу. В некоторых случаях, когда восстановить работу основного региона невозможно, выполняется географическая отработка отказа, и затронутые клиенты получают уведомления об этом через свои подписки.
