<properties
   ...
   description="В этой статье содержатся общие сведения об обеспечении безопасности с использованием разных методов проверки подлинности, доступных для учетных записей службы автоматизации в службе автоматизации Azure."
   services="automation"
   documentationCenter=""
   authors="MGoedtel"
   manager="jwhit"
...
С помощью службы автоматизации Azure можно автоматизировать задачи, выполняемые с ресурсами Azure (локально), а также с ресурсами других поставщиков облачных служб, например Amazon Web Services (AWS). Для выполнения необходимых действий модуль Runbook должен иметь разрешения для доступа к ресурсам, с которыми он безопасно работает, и только минимальные права, необходимые в пределах подписки. В этой статье рассматриваются разные методы обеспечения безопасности, поддерживаемые службой автоматизации Azure, а также объясняется, как приступить к работе в одной или нескольких средах, которыми вы хотите управлять.
...
Метод | Среда | Статья
----------|----------|----------
Учетная запись пользователя Azure AD | Диспетчер ресурсов Azure и управление службами Azure | [Authenticate Runbooks with Azure AD User account](../automation/automation-sec-configure-aduser-account.md) (Проверка подлинности модулей Runbook с помощью учетной записи пользователя Azure AD)
...
Учетные данные AWS | Amazon Web Services | [Authenticate Runbooks with Amazon Web Services (AWS)](../automation/automation-sec-configure-aws-account.md) (Проверка подлинности модулей Runbook с помощью Amazon Web Services)
