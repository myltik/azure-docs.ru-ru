...
Вы можете компилировать конфигурации службы настройки требуемого состояния (DSC) двумя способами: на портале Azure или с помощью PowerShell. Следующая таблица помогает определить, когда нужно использовать какой метод на основе характеристик каждого:
...
4. Если конфигурация не имеет параметров, вам будет предложено подтвердить, нужно ли его компиляция. Если конфигурация имеет параметры, **компиляции конфигурации** открывает колонку, чтобы обеспечить значения параметров. Дополнительные сведения о параметрах см. в разделе [**Базовые параметры**](#basic-parameters) ниже.
...
Вы можете компилировать конфигурацию DSC, показанную выше, с помощью PowerShell. Под PowerShell добавляет две конфигурации узла к опрашивающему серверу DSC службы автоматизации Azure: **CredentialSample.MyVM1** и **CredentialSample.MyVM2**.
...
Можно также импортировать конфигурации узла (MOF-файлы), которые скомпилированы за пределами Azure. Одним из преимуществ является то, что конфигурации узла могут быть заверены.
