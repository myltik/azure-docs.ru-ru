...
Вы можете компилировать конфигурации DSC, использующие базовые параметры, на портале Azure Automation DSC или в Azure PowerShell:
...
Вы можете компилировать приведенную выше конфигурацию DSC с помощью PowerShell. Это добавит две конфигурации узла в опрашивающий сервер Azure Automation DSC: **ConfigurationDataSample.MyVM1** и **ConfigurationDataSample.MyVM3**.
...
Хотя конфигурации DSC в службе автоматизации Azure могут ссылаться на ресурсы-контейнеры учетных данных с помощью конфигурации **Get-AutomationPSCredential**, эти ресурсы-контейнеры можно передавать и в качестве параметров, если нужно. Если конфигурация работает с параметром типа **PSCredential**, в качестве значения этого параметра нужно использовать имя строки актива учетных данных, используемых для службы автоматизации Azure, а не объект PSCredential. Названный так актив учетных данных, используемых для службы автоматизации Azure, будет в фоновом режиме извлечен и передан в конфигурацию.
...
Вы можете компилировать приведенную выше конфигурацию DSC с помощью PowerShell. Это добавит две конфигурации узла в опрашивающий сервер Azure Automation DSC: **CredentialSample.MyVM1** и **CredentialSample.MyVM2**.
