...
Существуют три различные последовательности ссылки, которые проверяются значения были предоставлены два входных параметра runbook представляющий имя виртуальной Машины и имя группы ресурсов, чтобы определить, что является соответствующее действие, выполняемое - запуск одной виртуальной Машины, запуск всех виртуальных машин в ресурсе Группа, или все виртуальные машины в подписке.  Для установления последовательной связи между действиями подключения к Azure и получением одной виртуальной машины используется условная логика:
