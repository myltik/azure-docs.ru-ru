...
1.	Сейчас наш модуль Runbook пуст, поэтому введите *Write-Output "Hello World."*.
...
1.	Щелкните **область тестирования**, чтобы открыть ее.
...
4.	Когда задание модуля Runbook будет выполнено, на экране появится результат. В нашем примере мы увидим текст *Hello World*.
...
1.	Щелкните **Опубликовать**, чтобы опубликовать модуль Runbook, а затем кнопку **Да** в появившемся запросе.
...
4.	Нам просто нужно запустить модуль Runbook, поэтому щелкните **Запустить**, а затем нажмите кнопку **ОК**, когда откроется колонка "Запуск модуля Runbook".
...
9.	Щелкните **Все журналы**, чтобы открыть область "Потоки" для задания Runbook. В потоке выходных данных должен отображаться только текст *Hello World*. Но также могут присутствовать и другие потоки заданий модуля Runbook, например "Подробные сведения" и "Ошибка", если Runbook записывает в них какие-либо данные.
...
1.	Укажите значения для параметров *VMName* и *VMServiceName* в модуле Runbook и используйте эти переменные в командлете **Start-AzureVM**, как показано на рисунке ниже.
...
6.	Щелкните **Пуск**, чтобы запустить модуль Runbook. Укажите значения для параметров **VMName** и **VMServiceName** виртуальной машины, которую нужно запустить.
