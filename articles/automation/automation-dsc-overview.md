...
- При регистрации узла он автоматически передает в Automation DSC Azure сведения о сертификате, используемом для проверки подлинности узла после регистрации. Этот сертификат действует в течение одного года с момента его создания. В настоящее время в протоколе запроса PS DSC нет метода для выпуска нового сертификата, когда срок действия исходного сертификата подходит к концу. По этой причине по истечении года следует повторно регистрировать узлы с помощью Automation DSC Azure до тех пор, пока этот протокол не будет реализован в будущих версиях WMF (надеемся, что это произойдет менее чем через год).
