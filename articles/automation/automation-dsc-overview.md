...
Сейчас DSC службы автоматизации Azure предоставляет в [модуле PowerShell диспетчера ресурсов Azure](https://msdn.microsoft.com/library/mt244122.aspx) следующие командлеты для управления конфигурациями DSC:
...
Сейчас DSC службы автоматизации Azure предоставляет в [модуле PowerShell диспетчера ресурсов Azure](https://msdn.microsoft.com/library/mt244122.aspx) следующие командлеты для управления конфигурациями узлов: `Get-AzureAutomationDscNodeConfiguration`
...
Сейчас DSC службы автоматизации Azure предоставляет в [модуле PowerShell диспетчера ресурсов Azure](https://msdn.microsoft.com/library/mt244122.aspx) следующие командлеты для управления узлами DSC:
...
Командлет `Get-AzureAutomationRegistrationInfo` можно использовать для получения URL-адреса регистрации и ключа, необходимых для адаптации классических виртуальных машин Azure в учетную запись службы автоматизации Azure с помощью расширения для виртуальных машин DSC службы автоматизации Azure на портале Azure или с помощью PowerShell.
...
Сейчас DSC службы автоматизации Azure предоставляет в [модуле PowerShell диспетчера ресурсов Azure](https://msdn.microsoft.com/library/mt244122.aspx) следующие командлеты для управления модулями PowerShell:
...
- Так как DSC службы автоматизации Azure находится на стадии предварительной версии, при первом использовании этого компонента необходимо зарегистрироваться с помощью командлетов Azure PowerShell или портала предварительной версии Azure. Вы можете зарегистрироваться, вызвав два следующих командлета:
...
- Если выполняется перенос виртуальной машины Azure для управления с использованием Azure Automation DSC при помощи `Register-AzureAutomationDscNode`, `Set-AzureVMExtension` или расширения для виртуальных машин Azure Automation DSC на портале предварительной версии Azure, на протяжении часа она отобразится как узел DSC в службе автоматизации Azure. Это связано с тем, что расширение DSC для виртуальных машин Azure устанавливает на виртуальную машину платформу Windows Management Framework 5.0, которая требуется для размещения этой виртуальной машины в DSC службы автоматизации Azure.

- Узлы DSC, размещаемые в DSC службы автоматизации Azure, первоначально демонстрируют состояние "Соответствует", даже если на самом деле они не соответствуют конфигурации узлов DSC, с которыми сопоставляются. Как только узел извлечет первые данные и отправит первый отчет DSC в DSC службы автоматизации Azure, состояние узла станет верным.
