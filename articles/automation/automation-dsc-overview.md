...
- Сейчас DSC службы автоматизации Azure не поддерживает частичные или составные конфигурации DSC. Однако составные ресурсы DSC импортируются и используются так же, как и в локальном PowerShell, позволяя повторно использовать конфигурацию.
...
- Если вы хотите обновить до версии WMF 5 RTM, но виртуальная машина уже зарегистрирована как узел в DSC службы автоматизации Azure, сначала отмените эту регистрацию, а потом зарегистрируйте ее повторно после обновлений. Перед повторной регистрацией удалите файл `$env:windir\system32\configuration\DSCEngineCache.mof`.
...
- Если вы ранее использовали возможности DSC PowerShell в WMF 4 на компьютере, запрос сведений о конфигурации из DSC службы автоматизации Azure для этого компьютера может завершиться ошибкой с исключением "Сбой расшифровки". Чтобы устранить эту проблему, удалите следующие файлы и повторно зарегистрируйте компьютер в DSC службы автоматизации Azure: `$env:windir\system32\configuration\Current.mof`, `$env:windir\system32\configuration\DSCEngineCache.mof`, `$env:windir\system32\configuration\DSCStatusHistory.mof`.
