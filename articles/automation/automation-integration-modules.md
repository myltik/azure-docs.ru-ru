...
Если модули интеграции — это по сути модули PowerShell, это еще не значит, что нет рекомендаций по их созданию. У нас все же есть несколько рекомендаций по созданию модуля PowerShell, благодаря которым вы сможете использовать его в службе автоматизации Azure максимально эффективно. Одни из них предназначены для службы автоматизации Azure, другие же позволяют гарантировать непрерывную работу модулей в рабочем процессе PowerShell, независимо от того, используется ли служба автоматизации. 
...
2. Если модуль работает в удаленной системе, нужно выполнить следующие требования. Модуль должен содержать файл метаданных модуля интеграции, содержащий сведения, необходимые для подключения к этой удаленной системе, то есть тип соединения. b. Каждый командлет в модуле должен принимать объект соединения (экземпляр этого типа соединения) в качестве параметра.  
