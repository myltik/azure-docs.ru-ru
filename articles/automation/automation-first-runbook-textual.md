<properties
	...
	description="Учебник, в котором рассказывается, как создать, протестировать и опубликовать простой текстовый Runbook с помощью рабочего процесса PowerShell. В нем рассматриваются несколько концепций, включая проверку подлинности в ресурсах Azure и входные параметры."
	services="automation"
	documentationCenter=""
	authors="bwren"
	manager="stevenka"
...
9.  Щелкните **Потоки**, чтобы открыть область "Потоки" для задания Runbook. В потоке вывода должен отображаться только текст *Здравствуй, мир!*, но могут присутствовать и другие потоки задания Runbook, например "Подробная информация" и "Ошибка", если Runbook записывает в них какие-либо данные.<br> ![Сводные данные задания](media/automation-first-runbook-textual/job-pane-streams.png) 
...
3. Щелкните **область тестирования**, чтобы протестировать модуль Runbook.
...
9. Сохраните Runbook, а затем щелкните **область тестирования**, чтобы проверить его.
...
12.  Щелкните **Опубликовать**, чтобы опубликовать новую версию Runbook.
13.  Остановите виртуальную машину, запущенную на предыдущем этапе.
13.  Щелкните **Пуск**, чтобы запустить Runbook. Введите значения параметров **VMName** и **VMServiceName** для виртуальной машины, которую необходимо запустить.<br> ![Запуск модуля Runbook](media/automation-first-runbook-textual/start-runbook-input-params.png) 
