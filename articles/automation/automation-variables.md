---
title: Ресурсы-контейнеры переменных в службе автоматизации Azure | Microsoft Docs
...
> [!NOTE]
> Не следует использовать переменные в параметре –Name действия **Get-AutomationVariable** в модуле Runbook или конфигурации DSC, поскольку это усложняет определение зависимостей между модулями Runbook или конфигурацией DSC и переменными автоматизации во время разработки.
> 
> 
...
3. Щелкните **Добавить переменную**.
4. Завершите работу мастера и установите флаг, чтобы сохранить новую переменную.

### Создание новой переменной на портале Azure
1. Из учетной записи автоматизации щелкните **Средства**, чтобы открыть колонку **Средства**.
2. Щелкните **Переменные**, чтобы открыть колонку **Переменные**.
3. Щелкните **Добавить переменную** в верхней части колонки.
4. Заполните форму и нажмите кнопку **Создать** для сохранения новой переменной.
