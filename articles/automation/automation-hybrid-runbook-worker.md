...
>[AZURE.NOTE] В настоящее время компиляция [конфигураций DSC](automation-dsc-overview.md) в DSC службы автоматизации в гибридных рабочих ролях Runbook не поддерживается.
...
>[AZURE.NOTE] Не рекомендуется устанавливать гибридную рабочую роль на контроллер домена в среде.
...
Если у вас еще нет рабочей области Operations Management Suite, создайте ее согласно указаниям, которые можно найти в статье [Set up your workspace](https://technet.microsoft.com/library/mt484119.aspx) (Настройка рабочего пространства). Если у вас уже есть рабочая область, вы можете использовать ее.
...
Выполните инструкции в статье [Добавление решения с использованием коллекции решений](https://technet.microsoft.com/library/mt674635.aspx), чтобы добавить решение **службы автоматизации** в рабочую область Operations Management Suite.
...
Установите агент на локальный компьютер согласно инструкциям в статье [Прямое подключение компьютеров к Operational Insights](https://technet.microsoft.com/library/mt484108.aspx). Выполните эту процедуру на нескольких компьютерах, чтобы добавить в среду несколько компонентов Worker.
...
При запуске модуля Runbook на портале Azure отобразится параметр **Запуск на**, для которого вы можете выбрать значение **Azure** или **Гибридная рабочая роль**. При выборе значения **Гибридный компонент Worker** можно выбрать группу из раскрывающегося списка.
...
Кроме варианта, когда модули Runbook предоставляют собственную аутентификацию для локальных ресурсов, можно также указать учетную запись **запуска от имени** для группы гибридных рабочих ролей. Когда вы указываете [ресурс-контейнер учетных данных](automation-credentials.md) с доступом к локальным ресурсам, все модули Runbook будут выполняться с использованием этих учетных данных в гибридной рабочей роли Runbook в группе.
...
1. Создайте [ресурс-контейнер учетных данных](automation-credentials.md) с доступом к локальным ресурсам.
...
- Дополнительные сведения о различных методах, которые можно использовать для запуска модуля, см. в статье [Запуск модуля Runbook в службе автоматизации Azure](automation-starting-a-runbook.md)
- Описание различных процедур для работы с модулями Runbook PowerShell и рабочих процессов PowerShell в службе автоматизации Azure с помощью текстового редактора см. в статье [Изменение текстовых модулей Runbook в службе автоматизации Azure](automation-edit-textual-runbook.md).
