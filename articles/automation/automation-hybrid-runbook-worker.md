...
Ниже описаны два метода установки и настройки гибридной рабочей роли Runbook.  В первом методе используется сценарий PowerShell, автоматизирующий все действия, которые необходимы для настройки компьютера Windows. Это рекомендуемый метод, так как он упрощает весь процесс развертывания.  Второй метод заключается в пошаговом выполнении инструкций по ручной установке и настройке роли.   
...
1. Скачайте сценарий *New-OnPremiseHybridWorker.ps1* из [коллекции PowerShell](https://www.powershellgallery.com/packages/New-OnPremiseHybridWorker/1.0/DisplayScript) непосредственно на компьютер, на котором выполняется гибридная рабочая роль Runbook, или на другой компьютер в вашей среде, после чего скопируйте этот сценарий в рабочую роль.  
  ...
  * *HybridGroupName* (обязательный) — имя группы гибридных рабочих ролей Runbook, которая будет указана в качестве целевой для модулей Runbook, поддерживающих этот сценарий. 
  *  *SubscriptionID* (обязательный) — идентификатор подписки Azure, которая используется для учетной записи автоматизации.
  *  *WorkspaceName* (необязательный) — имя рабочей области OMS.  Если у вас нет рабочей области OMS, сценарий создаст и настроит ее.

    > [!NOTE]
    > В настоящее время интеграция с OMS поддерживается только для следующих регионов службы автоматизации: **Юго-восточная Австралия**, **Восточная часть США 2**, **Юго-Восточная Азия** и **Западная Европа**.  Если ваша учетная запись службы автоматизации не находится в одном из этих регионов, то сценарий создаст рабочую область OMS, но он сообщит, что связывание невозможно.  
...
4. Будет предложено подтвердить установку пакета **NuGet** и указать учетные данные Azure для аутентификации.<br><br> ![Выполнение сценария New-OnPremiseHybridWorker](media/automation-hybrid-runbook-worker/new-onpremisehybridworker-scriptoutput.png)

5. После завершения сценария в колонке "Hybrid Worker Groups" (Группы гибридных рабочих ролей) отобразится новая группа и количество элементов в ней. Для существующей группы отобразится увеличенное число элементов.  Можно выбрать группу из списка на вкладке **Hybrid Worker Groups** (Группы гибридных рабочих ролей) и щелкнуть элемент **Hybrid Workers** (Гибридные рабочие роли).  На колонке **Hybrid Workers** (Гибридные рабочие роли) вы увидите список элементов группы.  
...
Данные, необходимые для этого командлета, можно найти в колонке **Управление ключами** на портале Azure.  Откройте эту колонку, выбрав параметр **Ключи** в колонке **Параметры** своей учетной записи службы автоматизации.
...
1. На портале Azure перейдите к своей учетной записи службы автоматизации.  
