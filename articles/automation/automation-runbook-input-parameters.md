---
title: "Входные параметры Runbook | Документация Майкрософт"
...
| Имя |обязательный параметр. Имя параметра. Оно должно быть уникальным для Runbook, может содержать только буквы, цифры или символы подчеркивания. Оно должно начинаться с буквы. |
...
Наряду с перечисленными здесь входными параметрами Windows PowerShell поддерживает дополнительные атрибуты входных параметров, например проверки, псевдонимы наборы параметров. Однако в настоящее время служба автоматизации Azure поддерживает только входные параметры, перечисленные выше.
...
> [!NOTE]
> Если не указать атрибут **Mandatory** при определении параметров, то по умолчанию параметр будет необязательным. Кроме того, если в модуле Runbook рабочего процесса PowerShell задать для параметра значение по умолчанию, то он будет необязательным в PowerShell, независимо от значения атрибута **Mandatory**.
> 
> 
...
Получение свойств виртуальной машины с помощью действия [**Get-AzureRmVm**](https://msdn.microsoft.com/library/mt603718.aspx).
...
4. Создайте два параметра со следующими свойствами, которые будут использоваться для действия **Get-AzureRmVm**.
...
Если задание успешно создано, возвращается код состояния "HTTP 201". Дополнительные сведения о заголовках ответа и тексте ответа см. в статье о [создании задания модуля Runbook с помощью REST API](https://msdn.microsoft.com/library/azure/mt163849.aspx).

### <a name="test-a-runbook-and-assign-parameters"></a>Тестирование Runbook и назначение параметров
Во время [тестирования черновой версии модуля Runbook](automation-testing-runbook.md) с использованием параметра тестирования откроется колонка **Тест** , где можно настроить значения недавно созданных параметров.
...
Вы можете [связать расписание](automation-schedules.md) с модулем Runbook, чтобы он запускался в определенное время. При создании расписания вы назначаете входные параметры, значения которых будут использоваться модулем Runbook во время запуска согласно расписанию. Расписание нельзя сохранить, если не указаны значения для всех обязательных параметров.
