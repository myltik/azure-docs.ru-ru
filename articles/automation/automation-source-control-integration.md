...
1. В колонке "Учетная запись автоматизации" на портале предварительной версии Azure щелкните **Настроить систему управления версиями**. 
...
6. После настройки системы управления версиями в вашей учетной записи службы автоматизации будут созданы следующие ресурсы службы автоматизации: два [ресурса-контейнера переменных](automation-variables.md).
    ...
    * Переменная **Microsoft.Azure.Automation.SourceControl.OauthToken** содержит защищенное зашифрованное значение OAuthToken.  
...
6. Когда задание будет завершено, обновите репозиторий GitHub, чтобы увидеть изменения. Репозиторий должен содержать фиксацию со следующим сообщением: **обновленное *имя модуля Runbook* из службы автоматизации Azure**.
...
1. В учетной записи службы автоматизации, где настроена система управления версиями, откройте колонку **Интеграция системы управления версиями/Синхронизация репозитория** и нажмите кнопку **Синхронизация**. Когда появится запрос подтверждения, выберите **Да**, чтобы продолжить.  
