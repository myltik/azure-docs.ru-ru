...
2. Щелкните плитку **Модули Runbook** , чтобы открыть список модулей Runbook.<br> ![Элемент управления "Модули Runbook"](media/automation-first-runbook-graphical/runbooks-control.png)
...
2. Прокрутите список до конца.  Щелкните правой кнопкой мыши **Write-Output** и выберите пункт **Add to canvas** (Добавить на холст) или же щелкните значок многоточия рядом с командлетом и выберите пункт **Add to canvas** (Добавить на холст).
...
3. При этом создается объект [Задание Runbook](automation-runbook-execution.md) , а его состояние отображается на панели.  
   Сначала задание получает состояние *В очереди* , указывающее на то, что задание ожидает, пока рабочая роль Runbook в облаке станет доступной.  Как только рабочая роль затребует задание, оно получит состояние *Запущено*, а с началом фактического выполнения модуля Runbook — состояние *Выполняется*.  
...
2. Так как название **Вывод текста "Привет, мир"** больше не нужно, щелкните его правой кнопкой мыши и выберите **Удалить**.
...
5. Выберите параметр **Имя**.  В списке выберите **Выражение PowerShell** в поле **Выходные данные действия** и введите имя виртуальной машины (в двойных кавычках), которую следует запускать этим модулем Runbook.  Нажмите кнопку **ОК**.<br>![Значение параметра имени для Start-AzureRmVM](media/automation-first-runbook-graphical/runbook-startvm-nameparameter.png)
...
8. В поле **Источник данных** для параметра **Состояние** выберите **Постоянное значение**, а затем щелкните **True**.  Нажмите кнопку **ОК**.  
...
17. В поле **Выражение условия**введите *$ActivityOutput['Get Status'] -eq "Stopped"*.  **Start-AzureRmVM** будет выполняться, только если виртуальная машина остановлена.
