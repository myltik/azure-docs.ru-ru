...
13.  Щелкните **CERTIFICATETHUMBPRINT** и в колонке "Значение параметра" выберите для параметра **Источник данных** значение **Выходные данные действия**. В списке выберите **Get Run As Connection** (Получить подключение запуска от имени), в текстовом поле **Путь поля** введите текст **CertificateThumbrprint**, а затем нажмите кнопку **ОК**. 
...
20. На первом экземпляре **Write-Output** измените значение параметра **Метка** на *Notify VM Started* (Сообщить о запуске ВМ).
...
22. На втором экземпляре **Write-Output** измените значение параметра **Метка** на *Notify VM Start Failed* (Сообщить о сбое при запуске ВМ).
