...
| [Ответ на оповещение Azure](../log-analytics/log-analytics-alerts.md) | <li>Запуск Runbook в ответ на оповещение Azure.<br> <li>Настройка веб-перехватчика для Runbook и ссылки на оповещение.<br> <li>Аутентификация с помощью маркера безопасности в URL-адресе.<br> <li>Сейчас поддерживаются оповещения только для метрик. |
