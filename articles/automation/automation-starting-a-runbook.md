...
Если параметр имеет тип данных [объект], используйте следующий формат JSON, чтобы отправить его в виде списка именованных значений: *{"имя1":значение1,"имя2":значение2, "имя3":значение3}*. Значения должны быть простого типа. Модуль Runbook получает параметр в виде объекта [PSCustomObject](https://msdn.microsoft.com/library/system.management.automation.pscustomobject%28v=vs.85%29.aspx), у которого свойства соответствуют каждому именованному значению.
