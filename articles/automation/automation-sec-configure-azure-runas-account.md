...
В этой статье объясняется, как настроить учетную запись службы автоматизации на портале Azure с помощью учетной записи запуска от имени, чтобы использовать для проверки подлинности ресурсы управления модулями Runbook в Azure Resource Manager или в службе управления службами Azure.
...
13. Закройте колонку **Сводка по заданию** и колонку модуля Runbook **AzureAutomationTutorialScript**.
