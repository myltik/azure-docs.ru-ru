...
6. Убедитесь, что для параметра **Создать учетную запись запуска от имени в Azure** выбрано значение **Да**, и нажмите кнопку **Создать**.  
    ...
    >[AZURE.NOTE] Если после нажатия кнопки **Создать** появляется сообщение "Отказано в разрешении", это происходит потому, что учетная запись не является участником роли "Администраторы подписки".
