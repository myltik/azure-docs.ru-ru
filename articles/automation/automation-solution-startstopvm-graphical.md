...
# Решение службы автоматизации Azure — запуск и остановка виртуальных машин

Это решение автоматизации Azure включает модули Runbook для запуска и остановки классических виртуальных машин. Это решение позволяет:

- Использовать модули Runbook без внесения изменений в вашу рабочую среду. 
- Изменять модули Runbook для реализации настраиваемых функциональных возможностей.  
- Вызывать модули Runbook из другого модуля Runbook в части общего решения. 
...
|:---|:---|:---|
| ServiceName | string | Нет | Если значение указано, будут запущены или остановлены все виртуальные машины с таким именем службы. Если значение не указано, будут запущены или остановлены все классические виртуальные машины в Azure. |
...
При выполнении модулей выводятся [сообщения](automation-runbook-output-and-messages.md) для каждой виртуальной машины. Из них можно узнать о результатах инструкций запуска или остановки. Чтобы определить результат выполнения каждого модуля, вы можете выполнить поиск по определенной строке в выходных данных. В следующей таблице перечислены возможные выходные строки.
