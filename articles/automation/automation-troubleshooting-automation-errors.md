...
3. Локальный сбой проверки подлинности означает, что учетные данные Azure Active Directory были указаны неправильно. Сведения о том, как правильно настроить учетную запись Azure Active Directory, см. в записи блога о [проверке подлинности в Azure с помощью Azure Active Directory](https://azure.microsoft.com/blog/azure-automation-authenticating-to-azure-using-azure-active-directory/).
...
**Причина ошибки:** если в учетной записи Azure активирована многофакторная проверка подлинности, использовать пользователя Azure Active Directory для проверки подлинности в Azure нельзя. Вместо этого при проверке подлинности в Azure используйте сертификат или субъект-службу.
...
**Ошибка:** выполнение модуля Runbook завершается ошибкой "Не удается привязать параметр ``<ParameterName>``. Не удается преобразовать значение ``<ParameterType>`` десериализованного типа ``<ParameterType>`` в тип ``<ParameterType>``".
...
**Ошибка:** не удается импортировать модуль или импорт выполнен успешно, но командлеты не извлекаются.
...
**Советы по устранению неполадок:** убедитесь, что используется имя конфигурации узла, а не имя конфигурации. Вы можете воспользоваться кнопкой "Назначить конфигурацию узла" в колонке узла на портале или командлетом Set-AzureRMAutomationDscNode для сопоставления узла с действительной конфигурацией узла.

### Сценарий: при компиляции конфигурации не образовались файлы конфигурации узла (MOF-файлов).

**Ошибка:** ваше задание компиляции DSC было приостановлено из-за ошибки "Компиляция успешно завершена, но не было сформировано ни одного файла конфигурации MOF".

**Причина ошибки:** если в результате выражения, указанного рядом с "Node" в конфигурации DSC, получается $null, ни одного файла конфигурации узла создано не будет.

**Советы по устранению неполадок:** убедитесь, что выражение рядом с "Node" не вычисляется как $null. При передаче в ConfigurationData убедитесь, что передаются ожидаемые значения из данных конфигурации, необходимые для конфигурации. Например, “$AllNodes. Дополнительные сведения см. в https://azure.microsoft.com/ru-RU/documentation/articles/automation-dsc-compile/#configurationdata.
...
**Советы по устранению неполадок:** следуйте инструкциям в статье https://msdn.microsoft.com/ru-RU/powershell/wmf/limitation_dsc, чтобы устранить проблему.

### Сценарий: не удается использовать учетные данные в конфигурации DSC 

**Ошибка:** ваше задание компиляции DSC было приостановлено из-за ошибки "Ошибка System.InvalidOperationException при обработке свойства 'Credential' ТИПА '<some resource name>': Преобразование и хранение зашифрованного пароля в виде обычного текста разрешено, только если PSDscAllowPlainTextPassword установлено в true".

**Причина ошибки:** вы попытались использовать учетные данные в конфигурации, но не передали соответствующие ConfigurationData для установки PSAllowPlainTextPassword в true для конфигурации каждого узла.

**Советы по устранению неполадок:** передайте правильные ConfigurationData для установки PSAllowPlainTextPassword в true для конфигурации каждого узла. Дополнительные сведения см. в https://azure.microsoft.com/ru-RU/documentation/articles/automation-dsc-compile/#assets.
