...
Если при работе с ресурсами службы автоматизации, такими как модули (включая модули Runbook) и ресурсы-контейнеры, у вас возникает проблема, нужно разобраться в ее причинах. В этой статье описываются некоторые распространенные ошибки, которые могут возникнуть при работе со службой автоматизации Azure, а также предлагаются возможные действия по их исправлению.
...
3. Локальный сбой проверки подлинности означает, что учетные данные Azure Active Directory были указаны неправильно. Сведения о том, как правильно настроить учетную запись Active Directory, см. в записи блога о [проверке подлинности в Azure с помощью Azure Active Directory](https://azure.microsoft.com/blog/azure-automation-authenticating-to-azure-using-azure-active-directory/).
...
**Причина ошибки:** если в учетной записи Azure активирована многофакторная проверка подлинности, использовать пользователя Active Directory для проверки подлинности в Azure нельзя. Вместо этого при проверке подлинности в Azure используйте сертификат или субъект-службу.
...
**Сценарий: не удается привязать параметры при выполнении модуля Runbook**
