...
Объект Webhook может определять значения для параметров модуля Runbook, которые используются при запуске модуля Runbook этим объектом Webhook. Объект Webhook должен содержать значения для всех обязательных параметров модуля Runbook и может также включать необязательные параметры. Значение параметра, который настроен для веб-перехватчик может измениться даже после создания веб-перехватчик. Несколько объектов Webhook, привязанных к одному модулю Runbook, могут использовать разные значения параметров.
...
1. Из **Runbooks страницы** на портале Azure щелкните ту, которая начинается просмотреть страницу сведений о веб-перехватчик.
2. Нажмите кнопку **веб-перехватчика** в верхней части страницы, чтобы открыть **добавить веб-перехватчика** страницы. <br>
   ![Кнопка Webhook](media/automation-webhooks/webhooks-button.png)
3. Нажмите кнопку **создать новый веб-перехватчика** Открытие **страницы веб-перехватчика создания**.
