...
Для совместного использования ресурсов среди всех модулей Runbook в облаке служба автоматизации Azure временно выгружает любое задание после того, как его выполнение продолжалось в течение 3 часов, и запускает его с последней [контрольной точки](http://technet.microsoft.com/library/dn469257.aspx#bk_Checkpoints). В течение этого времени для задания отображается состояние "Выполнение, ожидание ресурсов". Если модуль Runbook не имеет контрольных точек или задание не дошло до первой контрольной точки перед выгрузкой, то оно будет перезапущено с самого начала.
