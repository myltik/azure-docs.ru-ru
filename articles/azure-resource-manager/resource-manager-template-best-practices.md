...
Если указать параметр для имени ресурса, при развертывании необходимо указать уникальное имя. При необходимости вы можете создать переменную, которая использует функцию [uniqueString()](resource-group-template-functions.md#uniquestring) для создания имени. 
...
* Нельзя использовать функцию [reference](resource-group-template-functions.md#reference) в разделе **переменных** шаблона. Функция **reference** получает свое значение из состояния среды выполнения ресурса, а переменные разрешаются при начальной обработке шаблона. Сформируйте значения, которым требуется функция **reference**, непосредственно в разделе **resources** или **outputs** шаблона.
