...
В этом уроке вы узнаете, как вводить сообщения в концентратор событий, используя консольное C# приложение, и параллельно извлекать их, используя библиотеку C# [Event Processor Host].
...
2. Щелкните **Службы приложения**, затем **Service Bus**, после чего **концентратор событий**, затем **Быстрое создание**.
...
6. Нажмите вкладку **Конфигурировать** вверху, добавьте правило с именем **SendRule** с правами *Send*, добавьте еще одно правило с именем **ReceiveRule** с правами *Manage, Send, Listen*, а затем нажмите **Сохранить**.

   	![][5]

7. Нажмите вкладку **Панель инструментов** вверху страницы, а затем нажмите **Информация о подключении**. Запишите две строки подключения.
...
1.	Из Visual Studio запустите проект **Receiver**, затем дождитесь запуска им приемников для всех секций.

   	![][21]

2.	Запустите проект **Sender**, нажмите **Ввод** в окнах консоли и просматривайте события по мере их вывода в окно приемника.
