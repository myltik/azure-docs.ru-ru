...
В этом учебнике показано, как использовать портал управления Azure для создания концентратора событий. В нем также показано, как собирать сообщения в концентратор событий, используя консольное приложение на C#, и как параллельно извлекать их, используя библиотеку C# [Узел обработчика событий].
...
- заверенный [пример приложения, которое использует концентраторы событий];
- пример [Развертывание обработки событий при помощи концентраторов событий];
- [решение для обмена сообщениями в очереди] при помощи служебной шины.
...
[Пример приложения, которое использует концентраторы событий]: https://code.msdn.microsoft.com/windowsazure/Service-Bus-Event-Hub-286fd097
[Развертывание обработки событий при помощи концентраторов событий]: https://code.msdn.microsoft.com/windowsazure/Service-Bus-Event-Hub-45f43fc3
