...
<div class="dev-center-tutorial-subselector">
	<a href="/ru-ru/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/" title=".NET backend" class="current">Серверная часть .NET</a> | <a href="/ru-ru/documentation/articles/mobile-services-schedule-recurring-tasks/"  title="JavaScript backend" >JavaScript backend</a>
</div>
...
+ [Загрузка и установка библиотеки LINQ to Twitter]
...
<ol start="7">
<li><p>В обозревателе решений Visual Studio откройте файл web.config для проекта мобильной службы, найдите параметры <strong>MS_TwitterConsumerKey</strong> и <strong>MS_TwitterConsumerSecret</strong> и замените их значения на ключ клиента и секрет клиента Twitter, заданные на портале.</p></li>
...
2. В левой области выберите категорию **Интернет**, выполните поиск пакета `linq2twitter`, щелкните **Установить** для пакета **linqtotwitter**, затем примите условия лицензионных соглашений. 
...
2. Щелкните правой кнопкой мыши **Ссылки**, выберите пункт **Добавить ссылку...**, выберите **Платформа** в разделе **Сборки**, установите флажок **System.ComponentModel.DataAnnotations** и нажмите кнопку **ОК**.
...
4. Разверните папку Models, откройте файл контекста модели данных (с именем <em>service_name</em>Context.cs) и добавьте следующее свойство, которое возвращает типизированный **DbSet**:
...
1. Разверните папку ScheduledJobs и откройте файл проекта SampleJob.cs.
	...
	Этот класс, который наследует от **ScheduledJob**, представляет собой задание, которое можно запланировать на портале управления Azure для запуска по фиксированному расписанию или по запросу.
...
2. Замените содержимое файла SampleJob.cs на код, приведенный ниже:
	...
	В приведенном выше коде необходимо заменить строки _todolistService_ и_todolistContext_ пространством имен и DbContext загруженного проекта, которые имеют значение <em>mobile&#95;service&#95;name</em>Service и <em>mobile&#95;service&#95;name</em>Context соответственно.  
...
4. Щелкните **Попробовать**, введите `Sample` в качестве значения параметра **{jobName}**, а затем щелкните **Отправить**.
	...
	Так будет отправлен новый запрос POST в конечную точку задания Sample. В локальной службе запускается метод **ExecuteAsync**. Вы можете задать в этом методе точку останова для отладки кода.
...
3. повторно опубликовать проект мобильной службы в Azure.
...
2. Щелкните вкладку **Планировщик**, а затем нажмите кнопку **+Создать**. 
    ...
    >[AZURE.NOTE]При работе в вашей мобильной связи на <em>бесплатном</em> уровне вы можете выполнить одновременно только одно запланированное задание. На оплачиваемых уровнях можно выполнять одновременно до десяти запланированных заданий.

3. В диалоговом окне планировщика введите _Sample_ для поля **Имя задания**, задайте интервал и единицы планирования, затем нажмите кнопку проверки. 
   
   	![][4]

   	В результате создается новое задание **SampleJob**. 

4. Щелкните только что созданное задание, а затем нажмите кнопку **Запустить один раз** для тестирования скрипта. 
...
4. (Необязательно) На [портале управления Azure] щелкните "Управление" для базы данных, сопоставленной с вашей мобильной службой.

    ![][6]

5. На портале управления выполните запрос для просмотра изменений, внесенных приложением. Ваш запрос будет похож на показанный ниже запрос, но вместо  `todolist` укажите в качестве имени схемы имя своей мобильной службы.
