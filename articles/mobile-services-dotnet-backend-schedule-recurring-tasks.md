...
<div class="dev-center-tutorial-subselector">
    <a href="/ru-ru/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/" title="Серверная часть .NET" class="current">Серверная часть .NET</a> | <a href="/ru-ru/documentation/articles/mobile-services-schedule-recurring-tasks/"  title="Серверная часть JavaScript" >Серверная часть JavaScript</a>
</div>
...
-   [Загрузите и установите библиотеку Twitter LINQ][Загрузите и установите библиотеку Twitter LINQ]
...
-   [Локальное тестирование запланированного задания][Локальное тестирование запланированного задания]
-   [Публикация службы и регистрация задания][Публикация службы и регистрация задания]

> [WACOM.NOTE]В этом учебнике используется сторонняя библиотека LINQ to Twitter, чтобы упростить доступ OAuth 2.0 к интерфейсам API Twitter v1.1. Необходимо загрузить и установить пакет NuGet LINQ to Twitter для работы с этим учебником. Дополнительные сведения см. в разделе [Проект LINQ to Twitter CodePlex][Проект LINQ to Twitter CodePlex].
...
2.  В том же разделе добавьте следующие новые параметры приложения, заменив местозаполнители на маркер доступа и секрет маркера доступа Twitter, заданные на портале.
...
1.  В обозревателе решений в Visual Studio щелкните правой кнопкой мыши папку DataObjects, разверните пункт **Добавить**, щелкните **Класс**, введите `Updates` в поле **Имя** и нажмите кнопку **Добавить**.
...
3.  Добавьте следующие инструкции **using** в этот новый класс:

        using Microsoft.WindowsAzure.Mobile.Service;
        using System.ComponentModel.DataAnnotations;

4.  Замените определение класса **Updates** следующим кодом:
...
5.  Разверните папку Models, откройте файл контекста модели данных (с именем *имя\_службы*Context.cs) и добавьте следующее свойство, которое возвращает объект с типом <b>DbSet</b>:
...
2.  Замените содержимое файла SampleJob.cs на код, приведенный ниже:
    ...
    В приведенном выше коде необходимо заменить строки *todolistService* и *todolistContext* на пространство имен и DbContext загруженного проекта, т. е. *mobile\_service\_name*Service и *mobile\_service\_name*Context, соответственно.

    В приведенном выше коде перегрузка метода **ExecuteAsync** вызывает интерфейс API запроса Twitter с помощью сохраненных учетных данных для запроса последних твитов, содержащих хэш-тег `#mobileservices`. Повторяющиеся твиты и ответы исключаются из результатов еще до того, как они сохраняются в таблице.
...
3.  Щелкните **Попробовать**, введите `Sample` в качестве значения параметра **{jobName}**, а затем щелкните **Отправить**.
...
4.  В обозревателе серверов разверните пункты **Подключения к данным**, **MSTableConnectionString** и **таблицы**. Щелкните правой кнопкой мыши **Updates** и выберите пункт **Показать данные таблицы**.
...
1.  Повторно опубликуйте проект мобильной службы в Azure.

2.  На [портале управления Azure][портале управления Azure] щелкните элемент Мобильные службы, а затем выберите свое приложение.

    ![][4]

3.  Щелкните вкладку **Планировщик**, а затем нажмите кнопку **+Создать**.
...
4.  В диалоговом окне планировщика введите *SampleJob* в поле **Имя задания**, укажите интервал и единицы планирования, а затем нажмите кнопку проверки.

    ![][6]

    В результате создается новое задание **SampleJob**.

5.  Щелкните новое, только что созданное задание, затем щелкните **Запустить один раз**, чтобы протестировать сценарий.
    ...
    > [WACOM.NOTE]Запрос POST по-прежнему можно использовать для запуска запланированного задания. Однако по умолчанию при авторизации применяется пользователь, т. е. запрос должен содержать ключ приложения в заголовке.

6.  (Необязательно). На [портале управления Azure][портале управления Azure] щелкните Управление для базы данных, связанной с вашей мобильной службой.

    ![][8]

7.  На портале управления выполните запрос для просмотра изменений, внесенных приложением. Ваш запрос будет похож на показанный ниже запрос, но вместо `todolist` укажите имя вашей мобильной службы в качестве имени схемы.
