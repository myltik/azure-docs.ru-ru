<properties  pageTitle="Планирование серверных задач с помощью планировщика - мобильные службы" metaKeywords="" description="Use the Windows Azure Mobile Services Scheduler to schedule jobs for your mobile app." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Schedule recurring jobs in Mobile Services" authors="glenga"  solutions="mobile" writer="" manager="dwrede" editor=""  />
...
<div class="dev-center-tutorial-subselector">
	<a href="/ru-ru/documentation/articles/mobile-services-dotnet-backend-schedule-recurring-tasks/" title=".NET backend" class="current">Сервер базы данных .NET</a> | <a href="/ru-ru/documentation/articles/mobile-services-schedule-recurring-tasks/"  title="JavaScript backend" >JavaScript backend</a>
</div>
...
2. Щелкните правой кнопкой мыши **Ссылки**, выберите **Добавить ссылку...**, выберите **Платформа** в разделе **Сборки**, установите флажок **System.ComponentModel.DataAnnotations** и щелкните **ОК**.
...
3. Замените определение класса **LocalStorageManager** следующим кодом:
...
2. Замените содержимое файла SampleJob.cs на код, приведенный ниже:
	...
	В приведенном выше коде перегрузка метода **ExecuteAsync** вызывает API запроса Twitter, используя сохраненные учетные данные, для запроса последних твитов, содержащих хэштег `#mobileservices`. Повторяющиеся твиты и ответы удаляются из результатов, прежде чем происходит сохранение в таблице.
...
4. Нажмите кнопку **Попробовать**, введите `Sample` как значение параметра **{jobName}**, затем нажмите кнопку **Отправить**.

	![][9]

	Таким образом, будет отправлен новый запрос POST в конечную точку задания Sample. В локальной службе запускается метод **ExecuteAsync**. Вы можете задать в этом методе точку прерывания для отладки кода.
...
4. Щелкните новое, только что созданное задание, затем нажмите **Запустить один раз** для тестирования скрипта. 
  	...
  	![][5]
   	...
   	Это приведет к выполнению задания, хотя оно отключено в планировщике. На этой странице можно настроить задание и изменить расписание в любое время.
