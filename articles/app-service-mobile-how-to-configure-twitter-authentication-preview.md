...
Чтобы выполнить процедуру, описанную в этом разделе, необходимо иметь учетную запись Twitter с проверенным электронным адресом. Чтобы создать новую учетную запись Twitter, перейдите на сайт <a href="http://go.microsoft.com/fwlink/p/?LinkID=268287" target="_blank">twitter.com</a>.
...
2. Введите значения **Имя**, **Описание** и **Веб-сайт** приложения. Затем в поле **URL-адрес обратного вызова** введите URL-адрес шлюза с добавлением пути _/signin-twitter_. Например, `https://contosogateway.azurewebsites.net/signin-twitter`. Убедитесь, что используете схему HTTPS.
...
4. Откройте вкладку **Параметры**, установите флажок **Разрешить использовать это приложение для входа в Twitter**, затем щелкните **Обновить параметры**.
...
7. В разделе **Параметры** выберите **Удостоверение**, а затем - **Twitter**. Вставьте значения идентификатора и секрета приложения, полученные ранее. Нажмите кнопку **Сохранить**.
