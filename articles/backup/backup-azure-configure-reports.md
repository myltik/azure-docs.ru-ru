...
2. В списке элементов, которые отображаются в хранилище, в разделе "Мониторинг и отчеты" щелкните **Backup Reports** (Отчеты резервного копирования), чтобы настроить учетную запись хранения для отчетов.
      ...
      ![Выбор пункта меню отчетов резервного копирования, шаг 2](./media/backup-azure-configure-reports/backup-reports-settings.PNG)
...
6. Щелкните "Учетная запись хранения", выберите соответствующую подписку и учетную запись хранения для хранения данных отчетов, а затем нажмите кнопку **ОК**.
      ...
      ![Выбор учетной записи хранения, шаг 6](./media/backup-azure-configure-reports/select-subscription-sa.png)
7. В разделе "Журнал" установите флажок **AzureBackupReport** и, перемещая ползунок, выберите период хранения этих данных. Данные отчетов будут храниться в учетной записи хранения в течение периода, выбранного с помощью этого ползунка.
