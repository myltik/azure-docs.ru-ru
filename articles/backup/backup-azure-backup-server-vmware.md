---
title: "Использование сервера Azure Backup Server для защиты рабочей нагрузки сервера VMware | Документация Майкрософт"
description: "Резервное копирование сервера VMware на диск или в Azure с помощью сервера Azure Backup Server. Ознакомьтесь с этой статьей, чтобы защитить рабочие нагрузки VMware."
services: backup
documentationcenter: 
author: markgalioto
manager: carmonm
...
Azure Backup Server может архивировать (защищать) серверы VMware vCenter версий 6.0 и 5.5.
...
10. На странице **Завершение мастера импорта сертификатов** проверьте, находится ли сертификат в нужной папке, а затем щелкните **Готово**, чтобы завершить работу мастера.
...
4. В диалоговом окне **добавления учетных данных** введите имя и описание для новых учетных данных. Затем укажите имя пользователя и пароль. *Contoso Vcenter credential* — имя учетных данных, которое используется для идентификации учетных данных в следующей процедуре. Используйте те же имя пользователя и пароль, которые использовались для сервера vCenter. Если сервер vCenter и сервер Azure Backup Server размещены в разных доменах, укажите домен в **имени пользователя**.
