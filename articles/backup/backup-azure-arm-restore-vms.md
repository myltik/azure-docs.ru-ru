...
> [!NOTE]
> Для восстановления виртуальной машины на портале Azure используется параметр "Быстрое создание". Если нужно настроить конфигурацию восстанавливаемой виртуальной машины, используйте PowerShell или портал, чтобы восстановить резервные копии дисков, и прикрепите их к виртуальной машине с помощью команд PowerShell. Дополнительные сведения см. в разделе [Восстановление виртуальных машин с помощью специальных конфигураций сети](#restoring-vms-with-special-network-configurations).
>
>
...
2. В колонке **Конфигурация восстановления** есть два параметра:
   ...
   * "Restore backed up disks" (Восстановление резервных копий дисков).
   ...
   ### <a name="restore-full-virtual-machine"></a>"Restore full virtual machine" (Полное восстановление виртуальной машины)
   В колонке **Restore configuration** (Конфигурация восстановления) введите или выберите значения для каждого из следующих полей.
   ...
   * **Учетная запись хранения** — в этом меню перечислены учетные записи хранения в том же расположении, где находится хранилище служб восстановления. Следует выбрать учетную запись хранения, которая находится в одном расположении с хранилищем служб восстановления. Учетные записи хранения, избыточные в пределах зоны, не поддерживаются. Если у вас нет учетной записи хранения в одном расположении с хранилищем служб восстановления, ее следует создать до запуска восстановления. Тип репликации учетной записи хранения отображается в скобках.
   ...
   Если вы хотите настроить виртуальную машину, создаваемую из резервных копий дисков, доступных в колонке "Конфигурация восстановления", выберите значение **Restore disks** (Восстановления дисков) для параметра **Тип восстановления**. Необходимо будет указать учетную запись хранения (такие же ограничения действуют для параметра "Restore full virtual machine" (Полное восстановление виртуальной машины)), в которую скопированы резервные копии дисков. Вы можете [подключить восстановленные диски к существующей виртуальной машине](../virtual-machines/virtual-machines-windows-attach-disk-portal.md) или [создать новую виртуальную машину на основе восстановленных дисков](./backup-azure-vms-automation.md#restore-an-azure-vm).

3. Чтобы подтвердить конфигурацию восстановления, в колонке **Конфигурация восстановления** щелкните **ОК**.
...
* Если вы используете дистрибутив Linux на основе cloud-init (например, Ubuntu), то в целях безопасности пароль будет заблокирован после восстановления. Для [сброса пароль](../virtual-machines/virtual-machines-linux-classic-reset-access.md)на восстановленной виртуальной машине рекомендуется использовать расширение VMAccess. Также мы рекомендуем использовать ключи SSH с такими дистрибутивами, чтобы предотвратить сброс пароля после восстановления.
...
* У восстановленной виртуальной машины не будет набора значений доступности. При создании виртуальной машины из восстановленных дисков с помощью PowerShell рекомендуется использовать параметр восстановления дисков и [добавление набора доступности](../virtual-machines/virtual-machines-windows-create-availability-set.md#use-powershell-to-create-an-availability-set). 
