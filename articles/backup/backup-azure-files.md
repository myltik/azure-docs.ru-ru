...
---
# <a name="back-up-azure-file-shares-preview"></a>Резервное копирование файловых ресурсов Azure (предварительная версия)

В этой статье описывается, как с помощью портала Azure создавать резервные копии и восстанавливать [файловые ресурсы Azure](../storage/files/storage-files-introduction.md) в Azure.
...
Прежде чем создавать резервную копию файлового ресурса Azure, убедитесь, что он находится в одном из поддерживаемых [типов учетных записей хранения](troubleshoot-azure-files.md#preview-boundaries). После этого можно защитить файловые ресурсы.

## <a name="limitations-for-azure-file-share-backup-during-preview"></a>Ограничения на резервное копирование файлового ресурса Azure на этапе предварительной версии
Функция резервного копирования файлов Azure доступна в предварительной версии. На этапе предварительной версии необходимо учитывать следующие ограничения:
- Вы не можете применять защиту файловых ресурсов в учетных записях хранения с использованием [хранилища, избыточного в пределах зоны](../storage/common/storage-redundancy.md#zone-redundant-storage) (ZRS) или [геоизбыточного хранилища с доступом на чтение](../storage/common/storage-redundancy.md#read-access-geo-redundant-storage) (RA-GRS).
- Вы не можете применять защиту файловых ресурсов в учетных записях хранения с включенными виртуальными сетями.
...
## <a name="configuring-azure-file-shares-backup"></a>Настройка резервного копирования файловых ресурсов Azure

Все данные резервных копий хранятся в хранилищах служб восстановления. В этом руководстве предполагается, что вы уже установили файловый ресурс Azure. Чтобы создать резервную копию файлового ресурса Azure, выполните следующее:
...
3. Щелкните **Резервное копирование**, чтобы настроить файловый ресурс для хранилища служб восстановления. 

   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/set-backup-goal.png)

    После того как хранилище будет связано с файловым ресурсом Azure, откроется меню резервного копирования, где вам необходимо будет выбрать учетную запись хранения. В меню отображаются все поддерживаемые учетные записи хранения в регионе вашего хранилища, которые еще не связаны с хранилищем служб восстановления.

   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/list-of-storage-accounts.png)
...
4. В списке учетных записей хранения выберите учетную запись и нажмите кнопку **ОК**. Azure ищет учетную запись хранения для файловых ресурсов, которые можно архивировать. Если вы не видите в списке недавно добавленные файловые ресурсы, подождите, пока они появятся.
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/discover-file-shares.png)
...
6. После этого откроется меню **Политика архивации**. В этом меню выберите имеющуюся политику или создайте новую, а затем щелкните **Включить резервное копирование**. 
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/apply-backup-policy.png)
...
1. Откройте хранилище служб восстановления, содержащее точки восстановления файловых ресурсов, и щелкните **Элементы архивации**. Откроется список типов элементов архивации.
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/list-of-backup-items.png)
...
2. В списке выберите **Azure Storage (Azure Files)** (Служба хранилища Azure (файлы Azure)). Откроется список файловых ресурсов Azure.
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/list-of-azure-files-backup-items.png)

3. В списке файловых ресурсов Azure выберите необходимый ресурс. Для него откроется меню элементов архивации.

   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/backup-item-menu.png)
...
4. В меню элементов архивации щелкните **Моментальная архивация**. Так как это задание резервного копирования по запросу, политика хранения не связана с точкой восстановления. Откроется диалоговое окно **Моментальная архивация**. Укажите срок хранения точки восстановления (последний день). 
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/backup-now-menu.png)

## <a name="restore-from-backup-of-azure-file-share"></a>Восстановление из резервной копии файлового ресурса Azure
Если вам нужно восстановить файловый ресурс или отдельные файлы и папки из точки восстановления, перейдите к элементу архивации, как описано в предыдущем разделе. Выберите **Восстановить общую папку**, чтобы восстановить файловый ресурс с состоянием на нужный момент времени. Выберите одну точку восстановления из появившегося списка, чтобы перезаписать текущий файловый ресурс или восстановить альтернативный ресурс в том же регионе.
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/select-restore-location.png)
...
Служба Azure Backup обеспечивает возможность просмотра точки восстановления на портале Azure. Чтобы восстановить файл или папку, щелкните "Восстановление файлов" на странице "Элемент архивации" и выберите в списке нужную точку восстановления. Выберите место назначения восстановления, а затем щелкните **Выбрать файл**, чтобы перейти к точке восстановления. Выберите файл или папку и нажмите кнопку **Восстановить**.
   ...
   ![Нажмите кнопку "Резервное копирование", чтобы связать файловый ресурс Azure с хранилищем](./media/backup-file-shares/restore-individual-files-folders.png)
...
Вы можете создать новую политику для резервного копирования файловых ресурсов Azure в разделе **политик резервного копирования** хранилища служб восстановления. Все политики, созданные при настройке резервного копирования файловых ресурсов, отображаются с типом политики "Общая папка Azure".
...
3. В меню **добавления** выберите **Общая папка Azure**. Откроется меню политики резервного копирования для файлового ресурса Azure. Укажите имя политики, частоту выполнения резервного копирования и период удержания для точек восстановления. Определив политику, нажмите кнопку "ОК".

   ![Выбор задания, данные которого необходимо отследить](./media/backup-file-shares/create-new-policy.png)

### <a name="stop-protecting-an-azure-file-share"></a>Остановка защиты файловых ресурсов Azure

При остановке защиты файловых ресурсов Azure появится запрос на сохранение точек восстановления. Есть два способа остановить защиту файловых ресурсов Azure:
...
Чтобы остановить защиту файловых ресурсов Azure, выполните следующее:
...
4. В элементах файловых ресурсов Azure щелкните меню **Дополнительно** и выберите **Stop Backup** (Остановить архивацию). 
...
### <a name="resume-protection-for-azure-file-share"></a>Возобновление защиты для файлового ресурса Azure
...
Дополнительные сведения о файловых ресурсах Azure см. в статьях:
