...
Да. Начиная с сентября 2016 года можно создать 25 хранилищ служб восстановления для каждой подписки. Вы можете создать не более 25 хранилищ служб восстановления на каждый поддерживаемый регион службы архивации Azure на подписку. Если вам нужно больше хранилищ, создайте дополнительную подписку.
...
### <a name="recovery-services-vaults-are-resource-manager-based-are-backup-vaults-still-supported-br"></a>Хранилища служб восстановления развернуты с помощью Resource Manager. Резервное копирование хранилищ по-прежнему поддерживаются? <br/>
Резервные хранилища были преобразованы в хранилищах службы восстановления. Если в резервное хранилище было не преобразуется в хранилище служб восстановления, затем резервное хранилище был преобразован в хранилище служб восстановления для вас. 
...
Все хранилища резервной копии были преобразованы в хранилищах службы восстановления. Если в резервное хранилище было не преобразуется в хранилище служб восстановления, затем резервное хранилище был преобразован в хранилище служб восстановления для вас.
...
### <a name="why-is-the-size-of-the-data-transferred-to-the-recovery-services-vault-smaller-than-the-data-i-backed-upbr"></a>Почему размер данных, переданных в хранилище служб восстановления, меньше размера резервной копии данных?<br/>
 Все данные, для которых создается резервная копия из агента службы Azure Backup, а также из SCDPM или Azure Backup Server, сжимаются и шифруются перед передачей. После применения сжатия и шифрования данных в хранилище служб восстановления — 30-40% меньшего размера.
