...
**ВОПРОС 2. Откуда можно скачать последнюю версию агента службы архивации Azure?** <br/> ОТВЕТ 2. Последнюю версию агента для резервного копирования с использованием Windows Server, System Center DPM и клиента Windows можно скачать [отсюда](http://aka.ms/azurebackup_agent). Если нужно создать резервную копию виртуальной машины, используйте агент виртуальной машины (он автоматически устанавливает правильное расширение). Как правило, агент виртуальной машины уже есть на виртуальных машинах, созданных с использованием коллекции Azure.
...
****ВОПРОС 4. При настройке агента службы архивации Azure выводится запрос на ввод **учетных данных хранилища**. Ограничен ли срок действия этих учетных данных? ОТВЕТ 4. Да, учетные данные хранилища действительны в течение 48 часов. Если срок действия файла истек, войдите на портал Azure и загрузите новый файл учетных данных из хранилища службы архивации.
...
**ВОПРОС 10. Что произойдет, если переименовать сервер Windows, выполняющий резервное копирование данных в службу Azure?** ОТВЕТ 10. Если переименовать сервер, остановятся все настроенные процессы резервного копирования. Сервер нужно будет снова зарегистрировать в хранилище службы архивации. При регистрации такой сервер будет считаться новым, поэтому во время первого резервного копирования скопируются все данные, а не только добавочные. Если вам нужно восстановить резервные данные из хранилища со старым именем сервера, используйте переключатель [**Другой сервер**](backup-azure-restore-windows-server.md#recover-to-an-alternate-machine) в мастере **восстановления данных**.
...
**ВОПРОС 14. Если у моей организации есть одно резервное хранилище, как изолировать восстановление данных определенного сервера от других серверов?**<br/> ОТВЕТ 14. Все серверы, зарегистрированные в одном и том же хранилище, могут восстанавливать данные резервных копий, созданные другими серверами, для которых *используется та же парольная фраза*. Если вам нужно изолировать восстановление данных резервной копии сервера от других серверов в организации, настройте для него уникальную парольную фразу. Например, серверы отдела по работе с персоналом могут использовать одну зашифрованную парольную фразу, серверы бухгалтерии — другую, а серверы-хранилища — третью.
...
**ВОПРОС 23. Какую длину пути к файлу можно задать в политике резервного копирования Azure с помощью агента службы архивации Azure?** <br/>ОТВЕТ 23. Агент службы архивации Azure использует NTFS. [Длина путей к файлам ограничена согласно параметрам в API Windows](https://msdn.microsoft.com/library/aa365247.aspx#fully_qualified_vs._relative_paths). Если длина путей к файлам, для которых выполняется резервное копирование, больше длины, заданной в Windows API, пользователи могут выполнить резервное копирование родительской папки или диска с этими файлами.

**Вопрос 24. Какие символы можно использовать в путях к файлам в политике резервного копирования Azure при настройке с помощью агента службы архивации Azure?** <br/>ОТВЕТ 24. Агент службы архивации Azure использует NTFS. Поэтому, задавая файлы, можно использовать [символы, которые поддерживает NTFS](https://msdn.microsoft.com/library/aa365247.aspx#naming_conventions).
...
**ВОПРОС 6. Можно ли запланировать резервное копирование на 18:00, а политики хранения настроить на другое время?**<br/> ОТВЕТ 6. Нет. Политики хранения можно применять только в точках резервного копирования. На изображении ниже политика хранения указывается для резервных копий, создаваемых в 24:00 и 18:00. <br/>
...
**ВОПРОС 1. Как изменить расположение кэша, указанное для агента службы архивации Azure?**<br/>

1. Остановите работу модуля архивации, выполнив следующую команду в командной строке с повышенными привилегиями:

  ```PS C:\> Net stop obengine```

2. Скопируйте папку пространства кэша на другой диск с достаточным объемом свободного места. Следует скопировать файлы из папки кэша, а не переместить их. Исходную папку кэша можно удалить, когда убедитесь, что резервные копии используют новый кэш.

3. Обновите следующие записи реестра, указав путь к новой папке кэша: <br/>
