<properties
  pageTitle="Подготовка к резервному копированию рабочих нагрузок с использованием сервера службы архивации Azure | Microsoft Azure"
  description="В этой статье описываются конкретные шаги для подготовки среды к резервному копированию рабочих нагрузок с помощью сервера службы архивации Azure"
  services="backup"
  documentationCenter=""
  authors="SamirMehta"
  manager="shreeshd"
...
| -------- | -------------------- | ----------------------- |
| Таблицы Azure | Виртуальная машина IaaS Azure<br><br>A2 Standard: 2 ядра, 3,5 ГБ ОЗУ | Можно начать с простого образа коллекции центра обработки данных Windows Server 2012 R2. [Защита рабочих нагрузок IaaS с помощью сервера службы архивации Azure (DPM)](https://technet.microsoft.com/library/jj852163.aspx) характеризуется множеством особенностей. Прежде чем развертывать виртуальную машину, полностью ознакомьтесь со статьей. |
| Локальная система | Виртуальная машина Hyper-V,<br> виртуальная машина VMWare<br> или физический узел<br><br>2 ядра и 4 ГБ ОЗУ | С помощью дедупликации Windows Server можно выполнить дедупликацию хранилища DPM. Узнайте больше о взаимодействии [DPM и операции дедупликации](https://technet.microsoft.com/library/dn891438.aspx) при развертывании в виртуальных машинах Hyper-V. |
...
Если вы в дальнейшем планируете присоединить этот сервер к домену, операцию присоединения рекомендуется выполнить до установки сервера службы архивации Azure. Перемещение существующего компьютера сервера службы архивации Azure в новый домен после развертывания *не поддерживается*.
...
6. Отобразится сообщение об успешном создании хранилища, а на странице служб восстановления хранилище будет иметь активное состояние. ![Список хранилищ службы архивации](./media/backup-azure-microsoft-azure-backup/backup_vaultslist.png)
  ...
  >[AZURE.IMPORTANT]Сразу после создания хранилища убедитесь, что выбрано соответствующее значение избыточности хранилища. См. дополнительные сведения о [настройке избыточности хранилища в хранилище службы архивации](backup-configure-vault.md#azure-backup---storage-redundancy-options).
...
3. Выберите все файлы и нажмите кнопку **Далее**. Скачайте все файлы со страницы загрузки сервера службы архивации Microsoft Azure и поместите их в одну папку. ![Центр загрузки 1](./media/backup-azure-microsoft-azure-backup/downloadcenter.png)
...
После загрузки всех файлов щелкните **MicrosoftAzureBackupInstaller.exe**. В результате запустится **мастер установки службы архивации Microsoft Azure**, с помощью которого можно извлечь файлы установки в указанное расположение. Выполните указания мастера и нажмите кнопку **Извлечь**, чтобы начать процесс извлечения.
...
После завершения процесса извлечения установите флажок, чтобы запустить только что извлеченный файл *setup.exe* для начала установки сервера службы архивации Microsoft Azure, и нажмите кнопку **Готово**.
...
2. На экране приветствия нажмите кнопку **Далее**. В результате вы перейдете в раздел *Проверка готовности*. На этом экране нажмите кнопку **Проверить**, чтобы определить, выполняются ли требования к оборудованию и программному обеспечению для сервера службы архивации Azure. Если все обязательные условия выполняются, появится соответствующее сообщение. Нажмите кнопку **Далее**.
...
Состояние подписки Azure можно изменить с *Срок действия истек* или *Отозвана* на *Активная*. Если подписка пребывает в состоянии, отличном от *Активная*, это сказывается на поведении продукта:

- Если состояние подписки *Отозвана*, на период пребывания в этом состоянии теряются функциональные возможности. После изменения состояния на *Активная* функциональные возможности резервного копирования и восстановления продукта восстанавливаются. Кроме того, можно извлечь данные резервного копирования, хранящиеся на локальном диске, если период их удержания достаточно большой. Однако, как только состояние подписки становится *Отозвана*, данные резервного копирования в Azure безвозвратно теряются.
- Если состояние подписки *Срок действия истек*, функциональные возможности теряются только до восстановления состояния *Активная*. Резервные копии, запланированные на период, во время которого подписка пребывала в состоянии *Срок действия истек*, не будут выполнены.
...
Подробные сведения о [подготовке среды для DPM](https://technet.microsoft.com/library/hh758176.aspx) см. на веб-сайте Microsoft TechNet. На этом сайте также содержатся сведения о поддерживаемых конфигурациях, при которых можно развернуть и использовать сервер службы архивации Azure.
