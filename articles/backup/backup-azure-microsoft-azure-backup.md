...
В этой статье рассматривается подготовка среды к резервному копированию рабочих нагрузок с помощью сервера службы архивации Azure.
...
Используя сервер службы архивации Azure, вы можете создавать резервные копии рабочих нагрузок приложения, таких как виртуальные машины Hyper-V, Microsoft SQL Server, SharePoint Server, Microsoft Exchange и клиенты Windows, с помощью единой консоли.
...
5. В открывшемся разделе "Приступая к работе" будет автоматически выбран экран целей архивации. ![Колонка целей архивации, открытая по умолчанию](./media/backup-azure-microsoft-azure-backup/getting-started.png)
    ...
    В разделе **целей архивации** в меню *Where is your workload running?* (Где необходимо запустить рабочую нагрузку?) выберите *Локально*.
...
7. В колонке **Prepare infrastructure** (Подготовка инфраструктуры), которая откроется, щелкните соответствующие кнопки, чтобы **скачать** сервер службы архивации Azure и учетные данные, используемые во время регистрации сервера службы архивации Azure в хранилище служб восстановления. В результате откроется страница Центра загрузки, где можно скачать программный пакет.
