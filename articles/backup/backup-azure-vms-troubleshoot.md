...
| --- | --- |
| Указанная конфигурация диска не поддерживается. Следуйте инструкциям по ссылке http://go.microsoft.com/fwlink/?LinkId=808978, чтобы обработать оповещения, или обратитесь в службу поддержки Майкрософт за помощью.| Сейчас служба Azure Backup не поддерживает размер диска больше 1023 ГБ. Убедитесь, что размер диска не превышает ограничение, разбив диски. Для этого необходимо скопировать данные на дисках размером больше 1023 ГБ на созданные диски размером до 1023 ГБ. |
