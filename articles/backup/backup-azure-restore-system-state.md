...
В этой статье описано, как восстановить резервные копии состояния системы Windows Server из хранилища служб восстановления Azure. Чтобы восстановить состояние системы, необходимо иметь резервную копию состояния системы (созданную с помощью инструкций в разделе [Back up Windows Server System State (Preview)](backup-azure-system-state.md#back-up-windows-server-system-state-preview) (Создание резервной копии состояния системы Windows Server (предварительная версия))) и установленный [агент служб восстановления Microsoft Azure (MARS) последней версии](http://aka.ms/azurebackup_agent). Восстановление данных состояния системы Windows Server из хранилища службы восстановления Azure состоит из двух этапов:
...
2. Выполните следующие действия, чтобы восстановить доменные службы Active Directory с помощью командлетов системы архивации данных Windows Server.
