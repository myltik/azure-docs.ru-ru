...
Обновление резервного хранилища до хранилищ служб восстановления никак не отражается на операциях уровня данных. Все задания архивации продолжают выполняться в обычном режиме, а все активные задания восстановления продолжают работу без перерыва. Во время обновления происходит кратковременный простой операций управления и невозможно защитить новые элементы или создать задания нерегламентированного резервного копирования. Во время обновления не выполняются задания восстановления для виртуальных машин IaaS. <MG>: В чем различие между заданиями восстановления, которые продолжают выполняться в обычном режиме, и заданиями восстановления виртуальных машин IaaS?
Обновление хранилища длится до часа. По завершении хранилище служб восстановления заменяет резервное хранилище.
...
2. Скачайте и [установите Azure PowerShell 3.7.0](https://github.com/Azure/azure-powershell/releases/download/v3.7.0-March2017/azure-powershell.3.7.0.msi).
3. Обновите модули PowerShell.
  - Для Windows Server:
...
4. Скачайте [сценарий PowerShell](http://download.microsoft.com/download/1/c/6/1c6ed72e-38f9-4675-9cf9-9b8a06da7cd3/recoveryservicesvaultupgrade.ps1) для обновления хранилищ.
...
SubscriptionID — идентификатор подписки хранилища, которое обновляется.
VaultName — имя резервного хранилища, которое обновляется.
Location — расположение хранилища, которое обновляется.
ResourceType — укажите BackupVault.
...
Так как все новые возможности планируется выпускать для хранилища служб восстановления и использование классического портала станет нерекомендуемым, корпорация Майкрософт объявит нерекомендуемыми резервные хранилища и модель диспетчера служб. В будущем корпорация Майкрософт инициирует автоматическое обновление всех хранилищ до хранилища служб восстановления. До тех пор клиенты могут сами решать, когда следует обновить свои хранилища.

**Как этот план обновления отразится на моих существующих инструментах?**</br>  
...
**Как долго будет длиться простой?**</br>
...
Чтобы выполнить обновление, вас нужно добавить как соадминистратора подписки на классическом портале Azure. Это необходимо, даже если вы уже добавлены в качестве ее владельца на портале Azure. Попробуйте добавить соадминистратора подписки на классическом портале Azure, чтобы узнать, являетесь ли вы ее соадминистратором. Если вам не удалась добавить соадминистратора, обратитесь к администратору или соадминистратору служб для подписки, чтобы вас добавили.

**Можно обновить резервное хранилище на основе CSP?**</br>
Нет. В настоящее время обновить резервные хранилища на основе CSP невозможно. Мы добавим поддержку этой функции в следующих выпусках.
...
**Мне не удалось обновить хранилище, так как для этого потребовалось обновить агент на компьютере, который больше не существует. Что делать в этом случае?**</br>
Если необходимо длительное хранение резервных копий этого компьютера, то вы не сможете обновить хранилище. Мы добавим поддержку обновления подобных хранилищ в будущих выпусках.
...
Мониторинг локальных резервных копий (агент MARS, DPM и MABS) — это новая функция, которая предоставляется при обновлении резервного хранилища до хранилища служб восстановления. Для синхронизации данных мониторинга со службой может потребоваться до 12 часов.

**Как сообщить о проблеме?**</br>
Если вы столкнулись с какими-либо сбоями во время обновления, запишите идентификатор операции, указанный в сообщении об ошибке. Служба поддержки Майкрософт примет профилактические меры для устранения данной проблемы. Вы можете обратиться в службу поддержки или написать нам на электронный адрес rsvaultupgrade@service.microsoft.com, указав идентификатор подписки, имя хранилища и идентификатор операции. Мы приступим к устранению проблемы как можно скорее. Не повторяйте операцию, если это явно не указано в инструкциях корпорации Майкрософт.
