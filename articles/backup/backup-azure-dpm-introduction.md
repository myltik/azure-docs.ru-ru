...
* **Диспетчер DPM развернут в качестве виртуальной машины Azure**. Начиная с версии System Center 2012 R2 с пакетом обновления 3, диспетчер DPM можно развернуть как виртуальную машину Azure. Если диспетчер DPM развернут как виртуальная машина Azure, резервную копию данных можно создать на дисках Azure, подключенных к этой виртуальной машине, либо можно разгрузить хранилище данных путем его архивации в хранилище служб восстановления.

## <a name="why-backup-from-dpm-to-azure"></a>Для чего нужна архивация из DPM в Azure?
...
* Для экземпляров DPM, развернутых локально, резервную копию в среде Azure можно использовать в качестве альтернативы долгосрочному хранению на магнитной ленте.
* Для экземпляров DPM, развернутых в среде Azure, служба архивации Azure позволяет разгружать хранилище на диске Azure и тем самым увеличивать масштаб развертывания, сохраняя старые данные в хранилище служб восстановления, а новые — на диске.
...
3. **Установка агента службы архивации Azure**. Из службы архивации Azure установите агент на каждом сервере DPM.
...
3. По умолчанию открывается колонка параметров. Если колонка параметров закрыта, щелкните **Параметры** на панели мониторинга хранилища, чтобы открыть ее. В колонке "Параметры" щелкните **Свойства**.
...
2. По умолчанию открывается колонка параметров. Если колонка параметров закрыта, щелкните **Параметры** , чтобы открыть ее. В колонке "Параметры" щелкните **Свойства**.
