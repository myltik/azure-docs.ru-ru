...
# <a name="troubleshoot-azure-backup-failure-vm-agent-unable-to-communicate-with-azure-backup"></a>Устранение сбоя службы Azure Backup: агент виртуальной машины не может связаться со службой Azure Backup
## <a name="summary"></a>Сводка
После регистрации виртуальной машины в службе архивации Azure и добавления ее в расписание служба архивации инициирует задание, взаимодействуя с расширением виртуальной машины, чтобы создать моментальный снимок. Любое из четырех условий может помешать активации создания моментального снимка, что, в свою очередь, может привести к сбою службы архивации. В этой статье описано, как устранять неполадки службы архивации, связанные с ошибками связи с агентом виртуальной машины и расширением.

[!INCLUDE [support-disclaimer](../../includes/support-disclaimer.md)]

## <a name="symptom"></a>Симптом
Происходит сбой службы Azure Backup для виртуальной машины инфраструктуры как услуги (IaaS), при этом на [портале Azure](https://portal.azure.com/) в сведениях об ошибке задания появляется следующее сообщение об ошибке: VM agent is unable to communicate with the Azure Backup Service (Агент не может связаться со службой Azure Backup), "Сбой операции моментального снимка, отсутствует сетевое подключение у виртуальной машины".
...
## <a name="cause-3-the-agent-installed-in-the-vm-but-unresponsive-for-windows-vms"></a>Причина 3. Агент установлен на виртуальной машине, но не отвечает (для виртуальных машин Windows)
...
1. Проверьте, можно ли просматривать службу гостевого агента Windows в службах компьютера (services.msc).
2. Если она не отображается, проверьте в разделе "Программы и компоненты", установлена ли служба гостевого агента Windows.
3. Если вы можете просматривать раздел "Программы и компоненты", удалите гостевой агент Windows.
4. Скачайте и установите [MSI-файл агента](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409). Чтобы выполнить установку, необходимо иметь права администратора.
5. Затем можно будет просмотреть службу гостевого агента Windows в службах.
6. Попробуйте выполнить резервное копирование по запросу или нерегламентированное резервное копирование, щелкнув "Моментальная архивация" на портале.

Проверьте также, **установлен ли пакет NET 4.5 в системе**. Он необходим для взаимодействия агента виртуальной машины со службой.

## <a name="cause-4-the-backup-extension-fails-to-update-or-load"></a>Причина 4. Не удалось обновить или загрузить расширение резервного копирования
...
## <a name="cause-5-the-snapshot-status-cannot-be-retrieved-or-a-snapshot-cannot-be-taken"></a>Причина 5. Не удалось получить состояние моментального снимка или создать моментальный снимок
