...
1. Если хранилище служб восстановления уже открыто, перейдите к следующему шагу. Если хранилище служб восстановления не открыто, на портале Azure в меню **Концентратор** выберите **Обзор**.
   ...
   а. В списке ресурсов введите **Службы восстановления**.
...
7. На этой странице приведены сведения о хранилище ключей, связанном с выбранными зашифрованными виртуальными машинами. Для резервного копирования требуется доступ только для чтения к ключам и секретам в хранилище ключей. Эти разрешения используются, чтобы создать резервную копию ключа и секрета, а также связанных виртуальных машин. *Необходимо предоставить службе резервного копирования разрешения для доступа к хранилищу ключей (для выполнения резервного копирования)*. Эти разрешения можно предоставить, следуя [действиям, описанным в следующем разделе](#provide-permissions-to-azure-backup).
...
| --- | --- | --- |
|Резервное копирование | Служба резервного копирования не имеет достаточных разрешений к хранилищу ключей для резервного копирования зашифрованных виртуальных машин. | Виртуальная машина должна быть зашифрована с помощью и BEK, и КЕК. После этого резервное копирование должно выполняться. Службе необходимо предоставить эти разрешения, следуя [действиям в предыдущем разделе](#provide-permissions-to-azure-backup). Или можно выполнить действия с PowerShell в подразделе о включении защиты документации PowerShell из раздела [Резервное копирование виртуальных машин Azure](backup-azure-vms-automation.md#back-up-azure-vms). |  
| Резервное копирование |Не удалось выполнить проверку, потому что виртуальная машина зашифрована только с помощью BEK. Резервное копирование поддерживается только для виртуальных машин, зашифрованных с использованием и BEK, и KEK. |Виртуальная машина должна быть зашифрована с помощью BEK и КЕК. Сначала расшифруйте виртуальную машину и зашифруйте ее, используя BEK и KEK. Включите резервное копирование после того, как виртуальная машина зашифрована с использованием BEK и KEK. Дополнительные сведения см. в статье о [расшифровке и шифровании виртуальных машин](../security/azure-security-disk-encryption.md).  |
