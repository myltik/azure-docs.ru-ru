...
2. На портале Azure щелкните **Создать** > **Гибридная интеграция** > **Создать резервную копию**. При нажатии кнопки **Создать резервную копию** выполняется автоматический переход на классический портал (см. после примечания).

    ![Портал Ibiza](./media/backup-azure-vms-prepare/Ibiza-portal-backup01.png)

   > [!NOTE]
   > Если в последний раз ваша подписка использовалась на классическом портале, она может открыться там же. В этом случае для создания хранилища архивации щелкните **Создать** > **Службы данных** > **Службы восстановления** > **Хранилище архивации** > **Быстрое создание** (см. изображение ниже).
   >
   >
...
7. Успешное создание хранилища подтверждается сообщением. На странице **служб восстановления** будет указано состояние хранилища **Активно**. Сразу после создания хранилища выберите соответствующее значение избыточности хранилища. Ознакомьтесь с дополнительными сведениями о [настройке избыточности хранилища в хранилище службы архивации](backup-configure-vault.md#step-1-create-a-recovery-services-vault).
