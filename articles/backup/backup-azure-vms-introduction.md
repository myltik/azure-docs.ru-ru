...
Данные резервного копирования, которые копируются из учетной записи хранения, учитываются в метриках числа операций ввода-вывода в секунду и исходящего трафика (пропускной способности хранилища) для этой учетной записи хранения. При этом виртуальные машины продолжают работу, выполняют операции ввода-вывода и используют пропускную способность. Важно, чтобы общий объем трафика (для архивации и работы виртуальной машины) не превышал ограничений для учетной записи хранения.
...
Архивация состоит из двух этапов: создание моментального снимка и перенос этого снимка в хранилище. На втором этапе (перенос данных в хранилище) для оптимизации хранилища, используемого для архивации, переносятся только добавочные изменения из предыдущего моментального снимка. Для этого вычисляется контрольная сумма блоков. Если блок изменился, это обнаруживается, а блок отправляется в хранилище. Здесь мы подробно рассматриваем блоки, чтобы узнать, можно ли свести к минимуму объем передаваемых данных, объединив все измененные блоки для отправки в хранилище. В некоторых старых приложениях операции записи не оптимальны с точки зрения хранилища, так как они малого размера и фрагментированы. И следовательно, необходимо потратить дополнительное время на обработку данных, записанных в этих приложениях. Для приложений, выполняемых на виртуальной машине, мы рекомендуем использовать блоки записи из Azure объемом не менее 8 КБ. Если приложение использует блок меньше 8 КБ, пострадает производительность архивации, так как такой показатель расходится с рекомендациями Azure. Мы рекомендуем ознакомиться со статьей [Хранилище Azure класса "Премиум": разработка, обеспечивающая повышенную производительность](../storage/storage-premium-storage-performance.md), чтобы настроить приложение для оптимальной записи, что позволит повысить производительность архивации. Хотя в статье рассказывается о хранилище класса Premium, она подходит и для дисков, работающих в хранилище уровня "Стандартный".
