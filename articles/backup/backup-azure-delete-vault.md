...
description: "В этой статье объясняется, как удалить хранилище служб восстановления. Статья включает шаги по устранению неполадок при попытаться удалить хранилище, но нельзя."
...
В этой статье описывается удаление хранилища служб восстановления на портале Azure. Если резервное копирование хранилищ, они были преобразованы в хранилищах службы восстановления.   
...
## <a name="removing-items-from-a-vault-protecting-a-vm"></a>Удаление элементов из хранилища, защиты виртуальной Машины
...
2. В представлении хранилища просмотрите панель **Основные компоненты** . Чтобы удалить хранилище, в нем не должно быть защищенных элементов. Если **резервное копирование элементов** или **резервного копирования серверов управления** больше не показывать нуль, необходимо удалить эти элементы. Невозможно удалить хранилище, если он содержит данные.
...
3. Чтобы начать удаление защищенных элементов из хранилища, найдите их в хранилище. В панели мониторинга хранилища щелкните **параметры**, а затем нажмите кнопку **резервное копирование элементов** открыть соответствующее меню.

    ![выберите хранилище в списке](./media/backup-azure-delete-vault/open-settings-and-backup-items.png)

    **Резервное копирование элементов** меню содержит отдельные списки, в зависимости от типа элемента: виртуальные машины Azure или папки (см. рисунок). По умолчанию используется тип элемента "Виртуальные машины Azure". Чтобы просмотреть список элементов типа "Файлы и папки" в хранилище, выберите пункт **Файлы и папки** в раскрывающемся меню.
4. Прежде чем удалить элемент из хранилища, обеспечивающего защиту виртуальной машины, нужно остановить задание архивации элемента и удалить данные точки восстановления. Для каждого элемента в хранилище выполните следующие действия.
    ...
    a. На **архивных элементов** меню, щелкните элемент правой кнопкой мыши и в контекстном меню выберите **остановить архивацию**.

    ![остановка задания архивации](./media/backup-azure-delete-vault/stop-the-backup-process.png)

    Открывает меню остановка резервного копирования.

    Б. На **остановка резервного копирования** меню из **выберите параметр** последовательно выберите пункты **Удаление резервных копий данных** > введите имя элемента > и нажмите кнопку **остановить архивацию**.
    ...
    После завершения задания служба отправляет сообщение: процесс резервного копирования был остановлен, и данные резервной копии был удален.
...
1. На **архивных элементов** меню, щелкните элемент правой кнопкой мыши и в контекстном меню выберите **удаления архивируемых данных**.

    ![Удалить данные архивации](./media/backup-azure-delete-vault/delete-backup-data-menu.png)

    **Удаление данных резервного копирования** открывает меню.
2. На **удаление данных резервного копирования** меню введите имя элемента и нажмите кнопку **удалить**.
...
2. В диалоговом окне **Остановить защиту** установите переключатель **Удалить защищенные данные** и нажмите кнопку **Остановить защиту**.
    ...
    Чтобы удалить хранилище, его необходимо очистить от защищенных данных, то есть удалить их. Если имеется много точек восстановления и данных в группе защиты, он может занять несколько минут для удаления данных. **Остановить защиту** показано диалоговое окно после завершения задания.
...
1. На портале Azure откройте панель мониторинга хранилища и выберите **Параметры** > **Инфраструктура резервного копирования** > **Рабочие серверы**.
    ...
    ![Откройте меню производственных серверах](./media/backup-azure-delete-vault/delete-production-server.png)

    **Рабочих серверов** меню открывается и список всех рабочих серверов в хранилище.

    ![список рабочих серверов](./media/backup-azure-delete-vault/list-of-production-servers.png)
2. На **рабочих серверов** меню правой кнопкой мыши сервер и нажмите кнопку **удалить**.

    ![удаление рабочего сервера ](./media/backup-azure-delete-vault/delete-server-on-production-server-blade.png)

    **Удалить** открывает меню.

    ![удаление рабочего сервера ](./media/backup-azure-delete-vault/delete-blade.png)
3. На **удаление** меню, проверьте имя сервера и нажмите кнопку **удалить**. Чтобы сделать кнопку **Удалить** доступной, нужно правильно ввести имя сервера.
...
2. В **Essentials** области, нажмите кнопку **параметры** открыть соответствующее меню.

    ![Откройте меню "настройки"](./media/backup-azure-delete-vault/backup-vault-click-settings.png)
3. На **параметры** меню, нажмите кнопку **инфраструктуры резервного копирования**.
4. На **инфраструктуры резервного копирования** меню, нажмите кнопку **серверы управления архивацией**. Открывает меню резервного копирования серверов управления.
...
5. Чтобы удалить сервер из списка, щелкните его имя правой кнопкой мыши и щелкните **Удалить**.
    **Удалить** открывает меню.
6. На **удалить** меню, укажите имя сервера. Если имя длинное, его можно скопировать и вставить из списка серверов управления архивацией. Затем щелкните **Удалить**.  
