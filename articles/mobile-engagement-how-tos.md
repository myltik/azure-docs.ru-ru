<properties 
   pageTitle="Руководства с пошаговыми инструкциями по Azure Mobile Engagement" 
   description="Руководства с пошаговыми инструкциями по Azure Mobile Engagement" 
...
# Azure Mobile Engagement - руководства с пошаговыми инструкциями

## Введение

В следующих руководствах с пошаговыми инструкциями по пользовательскому интерфейсу Azure Mobile Engagement предполагается, что вы знакомы с [основными понятиями][Link 6] Azure Mobile Engagement и начнете работу только после интеграции Azure Mobile Engagement [SDK][Link 5] в приложение. В этих пошаговых руководствах в основном показано, как обычные пользователи могут использовать [пользовательский интерфейс],[Link 1] а не то, как пользователи-разработчики могут использовать [API] на основе HTTP REST[Link 4]. Если у вас возникли сложности с любым из этих [пошаговых руководств],[Link 3] ознакомьтесь с [руководствами по устранению неполадок] Azure Mobile Engagement[Link 2].

## <a name="#First">Создание первой кампании push-уведомлений</a>
...
## <a name="#Test">Тестирование кампании push-уведомлений</a>
...
## <a name="#Personalize">Отправка персонализированных push-уведомлений</a>
...
> Предупреждение: Если тег сведений о приложении не определен для пользователя, пользователь не получит push-уведомление.
...
## <a name="#Differentiate">Дифференциация push-уведомления (в приложении или за его пределами)</a>
...
## <a name="#Schedule">Планирование кампании push-уведомлений</a>
 
### Планирование запуска кампании
 
![Shedule1][18]

Сегодня 21 марта, и вы запланировали объявление на 22 марта в полночь. 
Вам не нужно находиться в интерфейсе для отправки push-уведомления! Вы можете заранее запланировать минуту отправки уведомлений.
...
> Примечание. Кампании, созданные без даты окончания, сохраняют push-уведомление локально на устройстве и отображают его при следующем открытии приложения, даже если кампания завершена вручную.

## <a name="#TextView">Усовершенствование push-уведомления с помощью текстового представления</a>
...
## <a name="#WebView">Улучшение push-уведомления с помощью веб-представления</a>
...
- Только уведомление. Это простое стандартное уведомление. Это означает, что, если пользователь щелкнет его, оно откроется без дополнительных представлений, и появится только связанное с ним действие.
- Текстовое объявление. Это уведомление, приглашающее пользователя взглянуть на текстовое представление.
- Веб-объявление Это уведомление, приглашающее пользователя взглянуть на веб-представление.
