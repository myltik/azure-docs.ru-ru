...
Мы реализовали поддержку управления доступом на основе ролей (RBAC) на портале предварительной версии Azure, чтобы упростить для организаций выполнение требований к управлению доступом. В [этой записи блога][этой записи блога] вы найдете краткую вводную информацию, которая поможет вам приступить к использованию этой функции. В данном разделе приведены подробные сведения о ней и описаны дополнительные варианты использования.

<!--Table of contents for topic, the words in brackets must match the heading wording exactly-->

## Оглавление

-   [Управление доступом на основе ролей в Azure][Управление доступом на основе ролей в Azure]
-   [Сосуществование управления доступом на основе ролей и соадминистраторов подписки][Сосуществование управления доступом на основе ролей и соадминистраторов подписки]
-   [Предоставление прав на управление и операции с данными][Предоставление прав на управление и операции с данными]
-   [Предоставление и запрет доступа][Предоставление и запрет доступа]
-   [Известные проблемы при использовании управления доступом на основе ролей][Известные проблемы при использовании управления доступом на основе ролей]
-   [Предоставление отзыва][Предоставление отзыва]
...
-   **Пользователи**: роли можно назначать пользователям организации, включенным в каталог Azure AD, с которым связана подписка Azure. Кроме того, роли можно назначать внешним пользователям с учетными записями Майкрософт (например, [anna@outlook.com][anna@outlook.com]) с помощью действия приглашения на портал предварительной версии Azure. При этом в Azure AD для такого пользователя создается учетная запись гостя. Если эта учетная запись отключена в каталоге, внешний пользователь не сможет получать доступ к ресурсам Azure.
...
-   [Запрет доступа][Запрет доступа]
-   [Предоставление или запрет доступа для внешнего пользователя][Предоставление или запрет доступа для внешнего пользователя]
...
Дополнительные сведения об использовании Windows PowerShell для предоставления и запрета доступа см. в разделе [Управление доступом на основе ролей с помощью Windows PowerShell][Управление доступом на основе ролей с помощью Windows PowerShell].
...
Если при использовании функции управления доступом на основе ролей в предварительной версии у вас возникла проблема, обратитесь к разделу [Устранение неполадок при управлении доступом на основе ролей][Устранение неполадок при управлении доступом на основе ролей], в котором описаны известные причины проблем.

## <span id="feedback"></span></a>Предоставление отзыва

Испытайте функцию управления доступом на основе ролей в Azure и отправьте нам [отзыв][отзыв].
...
-   [Управление доступом на основе ролей с помощью Windows PowerShell][Управление доступом на основе ролей с помощью Windows PowerShell]
...
-   [Azure Active Directory Premium и Basic][Azure Active Directory Premium и Basic]
-   [Как подписки Azure связаны с Azure AD][Как подписки Azure связаны с Azure AD]
-   Базовые сведения о самостоятельном управлении группами безопасности в [блоге группы разработчиков Active Directory][блоге группы разработчиков Active Directory]

<!--Image references-->

  [этой записи блога]: http://go.microsoft.com/fwlink/?LinkId=511576
  ...
  [anna@outlook.com]: mailto:joe@outlook.com
  [Общие сведения об интеграции каталогов]: http://technet.microsoft.com/library/jj573653.aspx
  [функция делегированного управления группами]: http://msdn.microsoft.com/library/azure/dn641267.aspx
  ...
  [отзыв]: http://aka.ms/azurerbacfeedback
  ...
  [блоге группы разработчиков Active Directory]: http://blogs.technet.com/b/ad/archive/2014/02/24/more-preview-enhancements-for-windows-azure-ad-premium.aspx
