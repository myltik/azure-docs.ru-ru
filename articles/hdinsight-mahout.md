...
1.  Загрузите следующие файлы с сайта [][]<http://nsl.cs.unb.ca/NSL-KDD/></a>.

-   [KDDTrain+.ARFF][KDDTrain+.ARFF] — файл для обучения;

-   [KDDTest+.ARFF][KDDTest+.ARFF] — тестовая выборка.

1.  Откройте каждый файл и удалите строки в начале, начинающиеся с <'@'>, затем сохраните файлы. Если этого не сделать, то вы получите сообщения об ошибке при использовании этих данных с Mahout.
...
5.  Создайте лес деревьев принятия решений с помощью следующей команды.
    ...
    Результат ее выполнения сохраняется в каталоге **nsl-forest** , который расположен в хранилище для вашего кластера HDInsight по адресу \_\_wasb://user/\<username\>/nsl-forest/nsl-forest.seq. где \<username\> — это имя пользователя, которое используется для сеанса удаленного рабочего стола. Этот файл имеет машиночитаемый формат.
