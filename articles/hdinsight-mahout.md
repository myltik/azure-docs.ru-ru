<properties title="Создание списка рекомендуемых к просмотру фильмов с использованием Mahout" pageTitle="Создание списка рекомендуемых к просмотру фильмов с использованием Mahout с Microsoft Azure HDInsight (Hadoop)" description="Вы узнаете, как использовать библиотеку для обучения Apache Mahout для создания списка рекомендуемых к просмотру фильмов с помощью HDInsight (Hadoop)" metaKeywords="Azure hdinsight mahout, Azure hdinsight machine learning, azure hadoop mahout, azure hadoop machine learning" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" manager="paulettm" />
...
Вы узнаете, как использовать библиотеку машинного обучения [Apache Mahout][Apache Mahout] для создания списка рекомендуемых к просмотру фильмов с помощью Microsoft Azure HDInsight (Hadoop).

> [WACOM.NOTE] Для использования информации, приведенной в этой статье, вам понадобится кластер HDInsight. Для получения информации по созданию кластера, см. [Начало работы с Hadoop в HDInsight][Начало работы с Hadoop в HDInsight].
>
> Библиотека Mahout предустановлена в кластерах HDInsight 3.1. Если вы используете более раннюю версию HDInsight, перед тем как продолжить, см. раздел [Установка Mahout][Установка Mahout].

## <a name="learn"></a>О чем вы узнаете?

Mahout — это библиотека [машинного обучения][машинного обучения] для Apache Hadoop. Mahout содержит алгоритмы для обработки данных, такие как фильтрация, классификация и кластеризация. В этой статье вы будет использовать подсистему рекомендаций для создания рекомендаций для просмотра фильмов на основе уже просмотренных вашими друзьями. Вы также узнаете о том, как выполнять классификацию с помощью леса деревьев принятия решений. Вы изучите следующее:
...
-   [Создание рекомендаций с использованием PowerShell][Создание рекомендаций с использованием PowerShell]
-   [Классификация данных с использованием командной строки Hadoop][Классификация данных с использованием командной строки Hadoop]
-   [Устранение неполадок][Устранение неполадок]

## <a name="recommendations"></a>Создание рекомендаций с использованием PowerShell

> [WACOM.NOTE] В то время как задание, используемое в этом разделе, работает с PowerShell, многие классы, идущие в составе Mahout, в настоящее время не работают с PowerShell и должны запускаться из командной строки Hadoop. Список классов, которые не работают с PowerShell, приведен в разделе [Устранение неполадок][Устранение неполадок].
>
> Для примера использования командной строки Hadoop для запуска заданий Mahout, см. [Классификация данных с использованием командной строки Hadoop][Классификация данных с использованием командной строки Hadoop].

Одной из функций, предоставляемой Mahout, является подсистема рекомендаций. Оно принимает данные в формате `userID`, `itemId`, `prefValue` (предпочтения пользователей для элемента). Затем Mahout может провести анализ совместной встречаемости для определения,*что пользователи, имеющие предпочтения к одному элементу, также имеют их и к определенным другим*. После этого Mahout определит пользователей со сходными предпочтениями элементов, что можно использовать для создания рекомендаций.
...
Очень кстати, компания GroupLens Research предоставляет [данные о рейтинге фильмов][данные о рейтинге фильмов] в формате, совместимом с Mahout.

1.  Загрузите архив [MovieLens 100k][MovieLens 100k], в котором содержатся 100 000 рейтинговых оценок от 1 000 пользователей по 1 700 фильмам.

2.  Распакуйте архив. В не должен находиться каталог **ml-100k**, содержащий множество файлов с данными, имеющих префикс **u.**. Файл, который будет анализироваться Mahout, называется **u.data**. Структура данных этого файла: `userID`, `movieID`, `userRating` и `timestamp`. Вот пример данных.
...
3.  Загрузите файл **u.data** в каталог **example/data/u.data** на вашем кластере HDInsight. Если у вас имеется [Azure PowerShell][Azure PowerShell], то можно использовать модуль PowerShell [HDInsight-Tools][HDInsight-Tools] для загрузки файла. Другие способы загрузки файлов можно найти в документе [Отправка данных для заданий Hadoop в HDInsight][Отправка данных для заданий Hadoop в HDInsight]. Ниже приведен пример использования `Add-HDInsightFile` для загрузки файла
    ...
    Файл **u.data** будет загружен в каталог **example/data/u.data** в хранилище по умолчанию вашего кластера. Доступ к нему из заданий HDInsight можно получить с помощью идентификатора URI **wasb:///example/data/u.data**.

### Выполнение задания

Используйте следующий скрипт PowerShell для запуска задания с подсистемой рекомендаций Mahout на ранее загруженном файле **u.data**.
...
>
> Для удаления этих файлов можно использовать одну из утилит, упоминавшихся в документе [Отправка данных для заданий Hadoop в HDInsight][Отправка данных для заданий Hadoop в HDInsight]. Либо можно использовать функцию `Remove-HDInsightFile` из скрипта PowerShell [HDInsight-Tools][HDInsight-Tools].
>
> Если не удалить временные файлы или файл с результатами, то вы получите сообщение об ошибке при повторном запуске задания.

Задание Mahout не возвращает результаты в поток STDOUT, а хранит их в указанном каталоге результатов вида **part-r-00000**. Чтобы загрузить и просмотреть файл, используйте функцию `Get-HDInsightFile` модуля PowerShell [HDInsight-Tools][HDInsight-Tools].
...
Для использования этого скрипта необходимо наличие ранее распакованной папки **ml-100k**, а также локальная копия файла результатов **part-r-00000**, созданного заданием Mahout. Ниже приводится пример запуска скрипта.
...
> [WACOM.NOTE] Пример скрипта Python, **show\_recommendations.py**, имеет те же параметры запуска.
...
Одним из доступных в Mahout способов классификации является создание [случайного леса][случайного леса]. Это многоэтапный процесс, использующий данные обучения для создания деревьев принятия решения, которые затем используются для классификации данных. Используется класс **org.apache.mahout.classifier.df.tools.Describe**, входящий в состав Mahout, в настоящее время он должен запускаться из командной строки Hadoop.
...
1.  Загрузите следующие файлы с сайта <http://nsl.cs.unb.ca/NSL-KDD/>.

-   [KDDTrain+.ARFF][KDDTrain+.ARFF] — файл для обучения;

-   [KDDTest+.ARFF][KDDTest+.ARFF] — тестовая выборка.

1.  Откройте каждый файл и удалите все строки сверху, которые начинаются с "@", а затем сохраните файлы. Если этого не сделать, то вы получите сообщения об ошибке при использовании этих данных с Mahout.

2.  Загрузите файл в папку **example/data**. Вы также можете использовать функцию `Add-HDInsightFile` из модуля PowerShell [HDInsight-Tools][HDInsight-Tools].

### Выполнение задания

1.  Так как задание требует использования командной строки Hadoop, вам предварительно необходимо разрешить подключение к удаленному рабочему столу на [портале управления Azure][портале управления Azure]. На портале выберите ваш кластер HDInsight, затем выберите флажок **Включить удаленный рабочий стол** в нижней части страницы **Конфигурация**.
...
3.  После подключения выберите значок **Командная строка Hadoop**, чтобы открыть командную строку Hadoop.

    ![командная строка hadoop][командная строка hadoop]

4.  C помощью следующей команды сформируйте файловый дескриптор (**KDDTrain+.info**,) с использованием Mahout.
...
5.  Создайте лес деревьев принятия решений с помощью следующей команды.

        hadoop jar c:/apps/dist/mahout-0.9.0.2.1.3.0-1887/examples/target/mahout-examples-0.9.0.2.1.3.0-1887-job.jar org.apache.mahout.classifier.df.mapreduce.BuildForest -Dmapred.max.split.size=1874231 -d wasb:///example/data/KDDTrain+.arff -ds wasb:///example/data/KDDTrain+.info -sl 5 -p -t 100 -o nsl-forest

    Выходные данные этой операции сохраняются в каталоге **nsl-forest**, который находится в хранилище вашего кластера HDInsight по адресу \_\_wasb://user/\<имя\_пользователя\>/nsl-forest/nsl-forest.seq. \<имя\_пользователя\> — это имя пользователя, которое использовалось для удаленного сеанса рабочего стола. Этот файл имеет машиночитаемый формат.

6.  Протестируйте лес путем классификации выборки **KDDTest+.arff**, используя следующую команду.
...
1.  Используемая версия Mahout зависит от версии HDInsight вашего кластера. Вы можете узнать версию кластера с помощью следующей команды в [Azure PowerShell][Azure PowerShell]:
...
-   **Для HDInsight версии 2.1** можно загрузить файл jar, содержащий [Mahout версии 0.9][Mahout версии 0.9].

-   **Для HDInsight версии 3.0** вам необходимо [собрать Mahout из исходных кодов][собрать Mahout из исходных кодов] и указать версию Hadoop, предоставленную HDInsight. Установите необходимые компоненты, перечисленные на странице сборки, загрузите исходный код и используйте следующие команды для создания файла jar с Mahout:
...
1.  Загрузите файл jar в каталог **example/jars** в хранилище по умолчанию для вашего кластера. Следующий пример использует скрипт [send-hdinsight] для загрузки файла.
...
Для запуска заданий, использующих указанные классы, подключитесь к кластеру HDInsight и запустите их, используя командную строку Hadoop. Пример см. в разделе [Классификация данных с использованием командной строки Hadoop][Классификация данных с использованием командной строки Hadoop].

  [Apache Mahout]: http://mahout.apache.org
  [Начало работы с Hadoop в HDInsight]: http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-get-started/
  [Установка Mahout]: #install
  [машинного обучения]: http://en.wikipedia.org/wiki/Machine_learning
  ...
  [данные о рейтинге фильмов]: http://grouplens.org/datasets/movielens/
  ...
  [случайного леса]: http://en.wikipedia.org/wiki/Random_forest
  [KDDTrain+.ARFF]: http://nsl.cs.unb.ca/NSL-KDD/KDDTrain+.arff
  [KDDTest+.ARFF]: http://nsl.cs.unb.ca/NSL-KDD/KDDTest+.arff
  [портале управления Azure]: https://manage.windowsazure.com/
  [включить удаленный рабочий стол]: ./media/hdinsight-mahout/enableremote.png
  [подключить]: ./media/hdinsight-mahout/connect.png
  [командная строка hadoop]: ./media/hdinsight-mahout/hadoopcli.png
  [Mahout версии 0.9]: http://repo2.maven.org/maven2/org/apache/mahout/mahout-core/0.9/mahout-core-0.9-job.jar
  [собрать Mahout из исходных кодов]: http://mahout.apache.org/developers/buildingmahout.html
