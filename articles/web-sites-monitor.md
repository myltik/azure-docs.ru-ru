<properties linkid="manage-services-how-to-monitor-websites" urlDisplayName="Мониторинг веб-сайтов" pageTitle="Мониторинг веб-сайтов — управление службами Azure" metaKeywords="мониторинг веб-сайтов Azure, монитор портала управления Azure, мониторинг Azure" description="Узнайте, как следить за веб-сайтами Azure, используя страницу "Монитор" на портале управления." metaCanonical="" services="web-sites" documentationCenter="" title="Мониторинг веб-сайтов" authors="" solutions="" manager="" editor="" />



#<a name="howtomonitor"></a>Мониторинг веб-сайтов

Веб-сайты предоставляют возможности мониторинга с помощью страницы управления "Монитор". Страница управления "Монитор" содержит описанную ниже статистику производительности для веб-сайта.
...
- [Практическое руководство. Настройка диагностики и загрузка журналов для веб-сайта](#howtoconfigdiagnostics)
...
2. Чтобы просмотреть дополнительные показатели для веб-сайта, нажмите кнопку **Добавить метрики** в нижней части страницы, чтобы вывести диалоговое окно **Выберите метрики**. 
...
Чтобы определить степень влияния веб-сайта на квоты использования ресурсов, выполните следующие действия.
...
- **Хранилище файловой системы** - объем хранилища файловой системы, используемого веб-сайтами, работающими в режиме **В общем доступе**.
...
##<a name="howtoconfigdiagnostics"></a>Практическое руководство. Настройка диагностики и загрузка журналов для веб-сайта

Диагностика включается на странице управления **Настройка** для веб-сайта. Существует два типа диагностики: **диагностика приложений** и **диагностика сайтов**.
...
- **Ведение журнала приложения (файловая система)** - включение ведения журнала для информации, создаваемой приложением. Поле **Уровень ведения журнала** определяет уровень, на котором записывается информация: ошибка, предупреждение или сведения. Можно также выбрать вариант "Подробно", чтобы записать в журнал все сведения, созданные приложением.

	Журналы, созданные с помощью этого параметра, хранятся в файловой системе веб-сайта и могут быть загружены с помощью действий, описанных ниже в разделе **Загрузка файлов журнала для веб-сайта**.

- **Ведение журнала приложения (табличное хранилище)** - включение ведения журнала сведений, создаваемых приложением, аналогично параметру "Ведение журнала приложения (файловая система)". Но данные журнала хранятся в таблице учетной записи хранилища Azure.
...
- **Ведение журнала приложения (BLOB-объектов)** - включение ведения журнала сведений, создаваемых приложением, аналогично параметру "Ведение журнала приложения (табличное хранилище)". Но данные журнала хранятся в BLOB-объекте учетной записи хранения Azure.
...
Дополнительные сведения об учетных записях хранения Azure см. в статье [Управление учетными записями хранения](https://www.windowsazure.com/ru-ru/manage/services/storage/how-to-manage-a-storage-account/).
...
<div class="dev-callout"> 
	<b>Примечание</b> 
	<p>Также можно включить диагностику из Azure PowerShell с помощью командлета <b>Set-AzureWebsite</b></p>.<p>Если среда Azure PowerShell не установлена или не настроена для использования подписки Azure, см. раздел <a href="http://www.windowsazure.com/ru-ru/develop/nodejs/how-to-guides/powershell-cmdlets/">Использование Azure PowerShell</a>.</p></div>
...
<li><b>Node.js</b> - <a href="/ru-ru/develop/nodejs/how-to-guides/Debug-Website/">Отладка приложения Node.js для веб-сайтов Azure</a></li>
</ul>
...
<div class="dev-callout"> 
<b>Важно</b> 
<p>Ведение журнала и трассировка предъявляют значительные требования к веб-сайту. После воспроизведения устраняемых проблем рекомендуется отключить функции ведения журнала и трассировки.</p> 
</div>
...
- Расположение, в котором будут сохраняться журналы приложения, относительно корневого каталога документов.
...
1. В меню **Пуск** или на **начальном экране** найдите **Azure PowerShell**. Щелкните правой кнопкой мыши пункт **Azure PowerShell** и выберите **Запуск от имени администратора**.
	...
	<div class="dev-callout"> 
	<b>Примечание</b> 
	<p>Если среда <b>Azure PowerShell</b> не установлена, см. сведения об установке и настройке в статье <a href="http://msdn.microsoft.com/ru-ru/library/windowsazure/jj554332.aspx">Начало работы с командлетами Azure PowerShell</a>.</p> 
	</div>

2. Для загрузки файлов журналов выполните в командной строке Azure PowerShell следующую команду:
...
<div class="dev-callout"> 
<b>Примечание</b> 
<p>Если команда <b>azure</b> не установлена, см. сведения об установке и настройке в статье <a href="http://www.windowsazure.com/ru-ru/develop/nodejs/how-to-guides/command-line-tools/">Использование средств командной строки Azure</a>.</p>
</div>

###Чтение файлов журналов###

Файлы журналов, созданные после включения ведения журнала и / или трассировки для веб-сайта, зависят от уровня ведения журнала / трассировки, заданного на странице управления "Настройка" для веб-сайта. Ниже приведено местоположение файлов журналов и информация о том, как их проанализировать.
...
- **Подробные сведения об ошибке:** содержит сведения об ошибке, такие как <em>Модуль</em>, <em>Обработчик</em>, <em>Код ошибки</em> и <em>Запрошенный URL-адрес</em>.
...
- Расположение: /LogFiles/http/RawLogs. Сведения, хранящиеся в файлах, форматируются с использованием [расширенного формата журналов консорциума W3C](http://go.microsoft.com/fwlink/?LinkID=90561). Поля s-computername, s-ip и cs-version не используются веб-сайтами Azure.
...
1.	Откройте **Веб-сайты**. Щелкните имя настраиваемого веб-сайта.
...
5.	Введите URL-адрес службы, которую требуется отслеживать. Например, [http://contoso.cloudapp.net](http://contoso.cloudapp.net). 
...
Дополнительные сведения об отслеживании конечных точек веб-сайтов см. в следующих видео:
