...
В службе приложений реализована встроенная поддержка технологии общего доступа к ресурсам независимо от источника (Cross Origin Resource Sharing, CORS), что позволяет клиентам JavaScript выполнять междоменные вызовы к API, размещенным в приложениях API, веб-приложениях и мобильных приложениях. Эта функция службы приложений позволяет воспользоваться преимуществами CORS без необходимости писать код в API.

[CORS](https://en.wikipedia.org/wiki/Cross-origin_resource_sharing) — это протокол IP, который позволяет выполнять вызовы API из JavaScript в домен, который отличается от загруженного в браузер из JavaScript. Без CORS с веб-страницы contoso.com можно вызвать конечную точку API contoso.com, но вызвать конечную точку fabrikam.com с той же веб-страницы не удастся.
...
* Начиная с раздела [Продолжение руководства по началу работы с .NET](#tutorialstart) в статье описывается использование CORS. Для работы с этой частью необходимо сначала выполнить инструкции, приведенные в руководстве [по созданию приложений API](app-service-api-dotnet-get-started.md).
...
8. В браузере перейдите на [портал Azure](https://portal.azure.com/).
...
Если вы изучаете серию руководств для приложений API по началу работы с Node.js или Java, на этом этапе данная серия заканчивается. Перейдите к разделу [Дальнейшие действия](#next-steps), в котором предложены дополнительные статьи на тему приложений API.
...
Процедура создания нового веб-приложения и развертывания в нем проекта похожа на ту, что описана в первом руководстве этой серии. Разница только в том, что приложение относится к типу **Веб-приложение**, а не **Приложение API**.
...
3. На вкладке **Размещение** в диалоговом окне **Создание службы приложений** укажите **имя веб-приложения** (оно должно быть уникальным в домене *azurewebsites.net*).
...
3. В разделе **Параметры приложения** добавьте следующий ключ и значение:
	...
	|---|---|---|
	|toDoListAPIURL|https://{your имя\_приложения\_API\_среднего\_уровня}.azurewebsites.net|https://todolistapi0121.azurewebsites.net|
...
3. В окне браузера с пользовательским интерфейсом AngularJS щелкните ссылку **To Do List**.
...
12. Введите в текстовое поле URL-адрес веб-приложения ToDoListAngular (внешнее приложение). Например, если вы развернули проект ToDoListAngular в веб-приложении с именем todolistangular0121, разрешите вызовы с URL-адреса `https://todolistangular0121.azurewebsites.net`.
...
## Использование службы управления API с приложениями API
...
* [API Management cross domain policies](https://msdn.microsoft.com/library/azure/dn894084.aspx#CORS) (Междоменные политики службы управления API).
 
## Устранение неполадок

Если при работе с этим руководством возникли проблемы, убедитесь, что вы используете последнюю версию пакета Azure SDK для .NET. Самый простой способ сделать это — [скачать пакет Azure SDK для Visual Studio 2015](http://go.microsoft.com/fwlink/?linkid=518003). Если у вас установлена последняя версия, установщик веб-платформы сообщит, что установка не требуется.

Если вы продолжаете получать сообщения об ошибках CORS после настройки URL-адреса в колонке CORS на портале, еще раз проверьте правильность внесения изменений. Например:

* Убедитесь, что вы правильно указали протокол (`https`, а не `http`). Также проверьте, используется ли для запуска внешнего веб-приложения протокол `https`.
...
Дополнительные сведения о функциях Visual Studio, которые помогают устранять неполадки, см. в статье [Устранение неполадок веб-приложения в службе приложений Azure с помощью Visual Studio](../app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md).

## Дальнейшие действия 

В этой статье показано, как включить поддержку CORS в службе приложений, чтобы вызывать приложение API, выполняющееся в другом домене, с помощью клиентского кода JavaScript. Дополнительные сведения о приложениях API см. в статьях [Проверка подлинности и авторизация в службе приложений Azure](../app-service/app-service-authentication-overview.md) и [Проверка подлинности пользователя для приложений API в службе приложений Azure](app-service-api-dotnet-user-principal-auth.md).
