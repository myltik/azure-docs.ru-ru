...
В этом руководстве используется пример приложения, который вы загрузили и из которого создали приложение API в [первом руководстве по версии ASP.NET из этой серии](app-service-api-dotnet-get-started.md).

Если вы работаете с руководствами по началу работы с Java или Node.JS, перейдите непосредственно к разделу [Настройка CORS](#corsconfig). Он содержит общие инструкции, которые относятся ко всем приложениям API.
...
## Поддержка CORS в коде веб-API

В проекте веб-API можно установить пакет NuGet [Microsoft.AspNet.WebApi.Cors](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Cors/), который позволяет указать в коде домены, из которых API будет принимать вызовы JavaScript.
...
### CORS службы приложений имеет приоритет над CORS веб-API
