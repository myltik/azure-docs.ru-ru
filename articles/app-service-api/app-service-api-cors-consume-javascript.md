...
* В разделе [Настройка CORS](#corsconfig) содержатся общие инструкции по настройке CORS для любого приложения API. Они одинаково применимы ко всем платформам, которые поддерживаются службой приложений, в том числе .NET, Node.js и Java. 
...
9. Последовательно выберите пункты **Обзор > Приложения API**.
...
13. Щелкните **Сохранить**.
	...
	После того как вы нажмете кнопку **Сохранить**, приложение API будет принимать вызовы JavaScript с указанных URL-адресов.
...
Если вы изучаете серию руководств для приложений API по началу работы с Node.js или Java, перейдите к следующей статье, посвященной [проверке подлинности для приложений API службы приложений](app-service-api-authentication.md).

Оставшаяся часть этой статьи — это продолжение серии руководств по началу работы с .NET. В ней предполагается, что вы успешно прошли [первое руководство](app-service-api-dotnet-get-started.md).

## Развертывание проекта ToDoListAngular в новом веб-приложении

Выполняя инструкции [первого руководства](app-service-api-dotnet-get-started.md), вы создали приложение API среднего уровня и приложение API уровня данных. Изучив это руководство, вы создадите одностраничное веб-приложение (SPA), которое вызывает приложение API среднего уровня. Для работы SPA вам понадобится включить поддержку CORS в приложении API среднего уровня.
...
3. Убедитесь, что в диалоговом окне **Создание службы приложений** на вкладке **Размещение** указан тип **Веб-приложение**.
...
3. В окне браузера с пользовательским интерфейсом AngularJS щелкните ссылку **Список дел**.
...
В проекте веб-API можно установить пакет NuGet [Microsoft.AspNet.WebApi.Cors](https://www.nuget.org/packages/Microsoft.AspNet.WebApi.Cors/), чтобы иметь возможность указывать в коде домены, с которых API будет принимать вызовы JavaScript.
...
1. В контроллере веб-API добавьте атрибут `EnableCors` в класс контроллера или в отдельные методы действий. В следующем примере поддержка CORS действует для всего контроллера.
