...
- Должен подключаться к *имя\_узла*:\*номер\_порта\* локального источника. 
...
В этом разделе показывается, как установить SQL Server Express, включить TCP/IP и создать базу данных. Благодаря этому приложение API сможет работать с [порталом предварительной версии Azure](https://portal.azure.com).

<a name="InstallSQLDB"></a>
### Установка SQL Server Express

1. Чтобы установить SQL Server Express, загрузите один из файлов **SQLEXPRWT\_x64\_ENU.exe** (64-разрядная версия) или **SQLEXPR\_x86\_ENU.exe** (32-разрядная версия) и извлеките его в нужную папку. 

2. После извлечения файлов установки SQL Server Express запустите файл **setup.exe**.

3. Когда отобразится **Центр установки SQL Server**, выберите пункт **Новая установка изолированного экземпляра SQL Server или добавление компонентов к существующей установке**.
...
5. На странице **Конфигурация экземпляров** выберите **Экземпляр по умолчанию**, а затем нажмите кнопку **Далее**.
...
1. В **SQL Server Management Studio** подключитесь к установленному SQL Server. В качестве **типа сервера** выберите **модуль базы данных**. В качестве **имени сервера** можно использовать **localhost** или имя используемого компьютера. Выберите пункт **Проверка подлинности SQL Server**, а затем войдите с именем пользователя `sa` и паролем, созданным ранее.
	...
	Если диалоговое окно **Подключение к серверу** не появляется автоматически, перейдите к **обозревателю объектов** на левой панели, щелкните **Подключение**, а затем щелкните **Ядро СУБД**.
...
8. В диалоговом окне **Добавление элемента формирования шаблонов** выберите параметр **Контроллер веб-API 2 — пустой** и нажмите кнопку **Добавить**.
...
2. Добавьте `/swagger` в конец URL-адреса в адресной строке браузера и нажмите клавишу **&lt;ВВОД>**. Отобразится пользовательский интерфейс Swagger, включенный вами в предыдущем разделе.
...
5. Нажмите кнопку **Попробовать** для просмотра данных, введенных ранее в базу данных.
...
4. В диалоговом окне **Создание приложения API** введите следующее.
	...
	- В разделе **План службы приложений** выберите один из существующих планов службы приложений или щелкните **Создать план службы приложений** и введите имя нового плана. 
	...
	- В разделе **Уровень доступа** выберите **Доступно всем пользователям**. Этот параметр сделает API общедоступным, что подходит для этого учебника. Позднее вы можете ограничить доступ на [портале предварительной версии Azure](https://portal.azure.com).
	- Выберите регион.

	Нажмите кнопку **ОК**, чтобы создать приложение API в своей подписке.
...
6. Когда в ходе процесса подготовки в подписке Azure создается группа ресурсов и приложение API, Visual Studio отображает ход выполнения в окне **Действия службы приложений Azure**.
