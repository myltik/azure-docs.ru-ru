...
Это первая часть в серии руководств. В последующих руководствах используются ресурсы, созданные в этом руководстве. В них вы узнаете следующее:
...
В инструкциях и на снимках экрана предполагается, что используется Visual Studio 2015. Но эти же указания подойдут и для Visual Studio 2013.
...
## Использование Swashbuckle и Swagger при локальном запуске

[Swagger](http://swagger.io/) — формат JSON для описания методов и ответов API. Служба приложений Azure поддерживает Swagger 2.0. Если предоставить метаданные Swagger для API, в Visual Studio можно создать код клиента, который упрощает использование API. Это будет показано позже в этом руководстве.

Чтобы предоставить метаданные Swagger 2.0 для проекта веб-API ASP.NET, можно установить пакет NuGet [Swashbuckle](https://www.nuget.org/packages/Swashbuckle). Swashbuckle использует отражение для динамического создания метаданных. Пакет NuGet уже установлен в скачанном проекте ContactsList.API. Кроме этого, он также уже установлен при создании проекта с помощью шаблона проекта **приложения API Azure**.
...
2. Установите проект ContactsList.API в качестве запускаемого проекта. (В **обозревателе решений** щелкните правой кнопкой мыши проект и выберите пункт контекстного меню **Назначить запускаемым проектом**.)
...
12. В адресной строке браузера добавьте `swagger/docs/v1` в конец строки и нажмите кнопку «Назад». (URL-адрес будет выглядеть следующим образом: `http://localhost:51864/swagger/docs/v1`.)
	...
	При использовании Chrome в окне браузера отобразится JSON-файл.
...
16. Щелкните **Параметры**, а затем в колонке **Параметры** перейдите к разделу API и щелкните **Определение API**.
	...
	При выборе приложения API, для которого нужно создать код, Visual Studio извлекает метаданные из этого URL-адреса.

## <a id="codegen"></a> Использование приложения в .NET с применением созданного кода клиента 
...
Проект ContactsList.MVC уже содержит созданный код клиента, но его нужно удалить и создать снова, чтобы понять, как это можно сделать, и установить URL-адрес своего приложения API в качестве целевого URL-адреса по умолчанию.
...
6. Класс клиента будет создан с другим именем в зависимости от имени приложения API. Измените этот код так, чтобы имя типа совпадало с именем в проекте.
...
2. Закомментируйте строку, которая задает базовый URL-адрес API в качестве URL-адреса localhost, и раскомментируйте строку без параметра конструктора. Теперь код будет выглядеть, как показано ниже, кроме того, что в обоих строках нужно указать имя класса, которое отражает имя приложения API, на основе которого создан код.
...
3.  На вкладке **Профиль** мастера **веб-публикации** щелкните **Служба приложений Microsoft Azure**.
...
В этом руководстве показано, как создать приложение API, развернуть в него код и использовать в клиенте .NET. В следующем руководстве из серии «Приступая к работе» показано, как [использовать приложения API в клиентах JavaScript с помощью CORS](app-service-api-cors-consume-javascript.md).
