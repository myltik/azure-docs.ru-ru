...
5. В диалоговом окне **Настройка плана служб приложений** в соответствующем поле введите ToDoListPlan или другое имя.
...
8. На вкладке **Подключение** мастера **Публикация веб-сайта** нажмите кнопку **Далее**.
	...
	Вы можете сразу нажать кнопку **Опубликовать** и развернуть проект в новом приложении API, но в целях этого руководства мы пройдемся по всем вкладкам диалогового окна и рассмотрим, что на них можно делать.
...
Дополнительные сведения о созданном клиенте см. в репозитории [AutoRest в GitHub](https://github.com/azure/autorest). Чтобы получить информацию по проблемам, связанным с созданным клиентом, откройте [проблему в репозитории AutoRest](https://github.com/azure/autorest/issues).
...
Как мы [уже](#apptype) знаем, приложения API, веб-приложения и мобильные приложения отличаются только тем, как они представлены на портале. Так как все они обладают одинаковыми возможностями, изменять тип приложения нет необходимости.
