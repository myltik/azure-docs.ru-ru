...
1. Задайте все три проекта в качестве автозагружаемых проектов в такой последовательности: сначала ToDoListDataAPI, затем ToDoListAPI и наконец ToDoListAngular. (В **обозревателе решений** щелкните решение правой кнопкой мыши, щелкните **Свойства**, выберите **Несколько запускаемых проектов**, расположите проекты в правильном порядке и для параметра **Действие** каждого проекта выберите значение **Запуск**.)  
...
12. В адресной строке браузера добавьте `swagger/docs/v1` в конец строки и нажмите кнопку «Назад». (URL-адрес — `http://localhost:45914/swagger/docs/v1`.)
	...
	Если вы используете Internet Explorer, в браузере появится запрос на загрузку файла *v1.json*.
...
3. В **обозревателе решений** в проекте ToDoListDataAPI откройте файл *App\_Start\\SwaggerConfig.cs*, найдите в нем следующий код и раскомментируйте его.
	...
	Файл *SwaggerConfig.cs* создается во время установки пакета Swashbuckle в проекте. С его помощью можно настроить Swashbuckle несколькими способами.
...
**Примечание.** В метаданных Swagger есть уникальный идентификатор для каждой операции API. По умолчанию пакет Swashbuckle может создавать дублирующиеся идентификаторы операций Swagger для методов контроллера веб-API. Это происходит, если HTTP-методы контроллера (например, `Get()` и `Get(id)`) перегружены. Сведения о действиях при перегрузке см. в разделе [Настройка определений API, создаваемых Swashbuckle](app-service-api-dotnet-swashbuckle-customize.md). При создании проекта веб-API в Visual Studio с помощью шаблона приложения API Azure к файлу *SwaggerConfig.cs* автоматически добавляется код, который создает уникальные идентификаторы операций.
...
4. В диалоговом окне «Служба приложений» в поле **Подписка** выберите подписку Azure, которую хотите использовать, и нажмите кнопку **Создать**.
	...
	Откроется диалоговое окно **Создание службы приложений** с вкладкой **Размещение**.
...
4. Введите **имя приложения API**, которое будет уникальным в домене *azurewebsites.net*. Например, можно ввести имя ToDoListDataAPI и номер, который сделает имя уникальным.
	...
	Azure использует это имя в качестве префикса для URL-адреса приложения. Полный URL-адрес состоит из этого имени и домена *.azurewebsites.net*. Например, если имя — `ToDoListDataAPI`, то URL-адрес будет такой: `todolistdataapi.azurewebsites.net`.
...
7. В диалоговом окне **Создание службы приложений** нажмите кнопку **Создать**.
	...
	**Примечание.** Создавать приложения API в службе приложений Azure можно и по-другому. Например, в Visual Studio для нового проекта можно создать ресурсы Azure так же, как мы только что создали для существующего проекта. Приложения API можно также создавать с помощью [портала Azure](https://portal.azure.com/), [командлетов Azure для Windows PowerShell](../powershell-install-configure.md) или [кроссплатформенного интерфейса командной строки](../xplat-cli.md).
	...
	Когда приложение API будет создано, Visual Studio создаст профиль публикации со всеми настройками, необходимыми для нового приложения API. Новый профиль публикации используется для развертывания проекта на следующих этапах.

8. На вкладке **Подключение** мастера **Публикация веб-сайта** нажмите кнопку **Далее**.

	Вы можете сразу нажать кнопку **Опубликовать** и развернуть проект в новом приложении API, но в целях этого руководства мы пройдемся по всем вкладкам диалогового окна и выясним, что на них можно делать.
...
1. В **обозревателе решений** продукта Visual Studio в проекте ToDoListAPI удалите папку *ToDoListDataAPI*.
...
8. В диалоговом окне **Служба приложений** разверните группу ресурсов, которую вы используете в этом руководстве, выберите нужное приложение API и нажмите кнопку **ОК**.
...
5. В проекте ToDoListAPI откройте файл *Controllers\\ToDoListController.cs*. Вы увидите код, вызывающий API с помощью созданного клиента.
...
Дополнительные сведения о созданном клиенте см. в [репозитории AutoRest в GitHub](https://github.com/azure/autorest). Чтобы получить помощь в решении проблем, связанных с созданным клиентом, опубликуйте вопрос о [проблеме в репозитории AutoRest](https://github.com/azure/autorest/issues).
...
Как мы [уже](#apptype) знаем, приложения API, веб-приложения и мобильные приложения отличаются друг от друга только тем, как они представлены на портале. Так как все они обладают одинаковыми возможностями, изменять тип приложения нет необходимости.
...
2. На панели навигации слева разверните узел **subscriptions**, а затем разверните подписку, которую вы используете.

4. Разверните узел **resourceGroups**, а затем разверните группу ресурсов, с которой вы работаете.
...
8. Найдите свойство `kind` и замените его значение api значением WebApp.
	...
	![свойство kind для экземпляра службы приложений](./media/app-service-api-dotnet-get-started/resexp.png)
...
В этом руководстве мы видели URL-адрес определения API в программе Visual Studio и на портале Azure. URL-адрес определения API для приложения API можно также настроить с помощью [шаблонов диспетчера Azure Resource Manager](../resource-group-authoring-templates.md), используя такие средства командной строки, как [Azure PowerShell](../powershell-install-configure.md) и [Azure CLI](../xplat-cli-install.md).
