...
2. Создайте решение, чтобы восстановить пакеты NuGet.
	...
	Чтобы увидеть приложение в действии перед развертыванием, можно запустить его локально. Убедитесь, что все три проекта являются запускаемыми. Следует использовать Internet Explorer или Microsoft Edge, так как эти браузеры позволяют вызывать JavaScript к URL-адресам независимо от источника `http://localhost`.
...
4. Нажмите кнопку **Создать** возле раскрывающегося списка **План службы приложений**.
	...
	На снимке экрана показаны примеры значений в полях **API App Name** (Имя приложения API), **Subscription** (Подписка) и **Resource Group ** (Группа ресурсов). Ваши значения будут другими.
