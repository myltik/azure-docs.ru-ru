...
4. Прокрутите колонку **Параметры приложения** вниз до раздела **Параметры приложения** и добавьте следующие ключ и значение.
	...
	|---|---|
	| **Значение** | https://{yourимя\_приложения\_API\_уровня\_данных}.azurewebsites.net |
...
В этом руководстве мы развертываем в службе приложений скачанные проекты веб-API ASP.NET, а не создаем их с нуля. Чтобы создать проект, который затем будет развернут в приложение API, можно создать типичный проект веб-API и установить в него пакет Swashbuckle. Или же можно воспользоваться шаблоном нового проекта **Приложение Azure API**. Чтобы воспользоваться этим шаблоном, щелкните **Файл > Создать > Проект > Веб-приложение ASP.NET > Приложение Azure API**.

![](./media/app-service-api-dotnet-get-started/apiapptemplate.png)

Такой же результат (аналогичный использованию элемента **Приложение Azure API**) можно получить, если выбрать **пустой** шаблон ASP.NET 4.5.2, добавить с помощью флажка поддержку веб-API и установить пакет Swashbuckle. Но в этот шаблон добавлен еще код конфигурации Swashbuckle, который не позволяет создавать повторяющиеся идентификаторы операций Swagger.
...
2. На панели навигации слева разверните узел **Подписки**, а затем — подписку, которая используется в работе.

4. Разверните узел **Группы ресурсов**, а затем — группу ресурсов, которая используется в работе.

5. Разверните узел **Microsoft.Web** > **Сайты**, а затем выберите приложение API, которое требуется изменить.

6. Нажмите кнопку **Изменить**.

8. Найдите свойство `kind` и замените значение "api" значением "WebApp".

	![](./media/app-service-api-dotnet-get-started/resexp.png)

9. Нажмите кнопку **Разместить**.
...
В этом руководстве мы видели URL-адрес определения API в программе Visual Studio и на портале Azure. URL-адрес определения API для приложения API можно также настроить с помощью [шаблонов Azure Resource Manager](../resource-group-authoring-templates.md), используя такие средства командной строки, как [Azure PowerShell](../powershell-install-configure.md) и [Azure CLI](../xplat-cli-install.md).

Чтобы просмотреть пример шаблона Azure Resource Manager, который задает свойство определения API, откройте [файл azuredeploy.json в репозитории с примером приложения для этого руководства](https://github.com/azure-samples/app-service-api-dotnet-todo-list/blob/master/azuredeploy.json). Найдите раздел шаблона, который выглядит так:
