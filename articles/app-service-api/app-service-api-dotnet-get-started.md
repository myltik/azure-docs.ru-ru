...
Это первое из серии руководств, в которых рассматриваются указанные выше возможности. Это руководство посвящено метаданным API. Во втором руководстве основное внимание уделяется CORS, а в третьем и четвертом рассматриваются функции проверки подлинности и авторизации.
...
* Как использовать Azure Active Directory (Azure AD) для защиты API от несанкционированного доступа.
* Как пользователи, вошедшие в Azure AD, могут использовать защищенный API.
...
Код, который будет развернут в приложении API и веб-приложении для этого руководства, находится в репозитории GitHub [Azure-Samples/app-service-api-dotnet-contact-list](https://github.com/Azure-Samples/app-service-api-dotnet-contact-list). Решение ContactsList Visual Studio содержит следующие проекты, используемые в этом руководстве.
...
В следующих руководствах используются другие проекты в рамках того же решения.

* **ContactsList.Angular** — клиент AngularJS для демонстрации поддержки CORS.
* **ContactsList.Angular.AAD** — клиент AngularJS для демонстрации проверки подлинности пользователей.
* **CompanyContacts.API** — проект веб-API ASP.NET для демонстрации проверки подлинности учетной записи службы.  
...
Задайте свойство `apiDefinition` в типе ресурса `Microsoft.Web/sites/config` для ресурса `<site name>/web`. Например, в **обозревателе ресурсов** последовательно выберите пункты **Подписки > {ваша подписка} > Группы ресурсов > {ваша группа ресурсов} > Поставщики > Microsoft.Web > Сайты > {ваш сайт} > Конфигурации > Веб**. Отобразится свойство `apiDefinition`:
...
Проект ContactsList.MVC уже содержит созданный клиентский код. Он будет удален и создан повторно, чтобы задать для приложения API целевой URL-адрес по умолчанию.
...
2. Закомментируйте строку, которая задает базовый URL-адрес API в качестве URL-адреса localhost, и раскомментируйте строку без параметра конструктора. Теперь код будет выглядеть так, как показано ниже, с тем отличием, что имя класса в двух этих строках отражает имя приложения API.
