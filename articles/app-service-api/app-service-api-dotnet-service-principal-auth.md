...
3. В колонке **Аутентификация или авторизация** щелкните **Вкл.**
...
7. В колонке **Аутентификация или авторизация** нажмите кнопку **Сохранить**.
...
10. В колонке **Аутентификация или авторизация** нажмите кнопку **Сохранить**.
...
Для целей этого руководства добавьте код, который проверяет идентификатор приложения и идентификатор субъекта-службы непосредственно в действиях контроллера. В качестве альтернативы используйте пользовательский атрибут `Authorize` или выполните эту проверку в последовательностях запуска (например, промежуточный слой OWIN). Пример такой последовательности см. в [этом примере приложения](https://github.com/mohitsriv/EasyAuthMultiTierSample/blob/master/MyDashDataAPI/Startup.cs).
...
* Убедитесь, что при вводе параметров на портале Azure в колонке **Параметры приложения** было правильно выбрано приложение API или веб-приложение.
...
* [Примеры Azure AD](http://aka.ms/aadsamples). Пример [WebApp-WebAPI-OAuth2-AppIdentity-DotNet](http://github.com/AzureADSamples/WebApp-WebAPI-OAuth2-AppIdentity-DotNet) аналогичен показанному в этом руководстве, но без использования проверки подлинности службы приложений.
