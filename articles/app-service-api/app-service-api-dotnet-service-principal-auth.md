...
* Как использовать защищенное приложение API с помощью учетных данных субъекта-службы (удостоверение приложения).
...
Этот метод защиты приложений API широко используется для [внутренних сценариев](app-service-api-authentication.md#internal), например при обращении одного приложения API к другому.
...
* [Остальная часть статьи](#tutorialstart) содержит руководство по настройке внутреннего доступа для примера приложения .NET, которое работает в службе приложений.
...
4. В раскрывающемся списке **Предпринимаемое действие, если проверка подлинности для запроса не выполнена** выберите **Войти с использованием Azure Active Directory**.
...
6. Настройте колонку **Параметры Azure Active Directory**, чтобы создать новое приложение Azure AD, или выберите существующее приложение Azure AD.
...
Когда вы это сделаете, служба приложений не будет разрешать непроверенным вызовам API достигать приложения API. В защищенном приложении API код проверки подлинности или авторизации не требуется.
...
Токены носителя для пользователей в том же клиенте считаются допустимыми для защищенного приложения API. Если вы хотите, чтобы только субъект-служба мог вызывать защищенное приложение API, добавьте код в защищенное приложение API, чтобы проверить следующие утверждения:

* значение `appid` должно быть таким же, как идентификатор клиента приложения Azure AD, связанного с вызывающей стороной;
* значение `objectidentifier` должно быть идентификатором субъекта-службы вызывающей стороны.
...
1. На [портале Azure](https://portal.azure.com/) перейдите к колонке приложения API, созданного для приложения ToDoListDataAPI (уровень данных), и щелкните **Параметры**.
...
7. В разделе **Режим управления** щелкните **Создать новое приложение AD**.
	...
	По умолчанию приложение Azure AD называется так же, как приложение API. Вы можете ввести другое имя.
...
3. Разверните проект ToDoListAPI. (Щелкните правой кнопкой мыши проект и выберите **Публикация > Опубликовать**.)

4. Закройте окно браузера, которое открылось после успешного развертывания проекта.
...
5. Скопируйте значение **Идентификатор клиента** и сохраните его, чтобы использовать позже.

8. На классическом портале Azure вернитесь в список **Приложения, которыми владеет моя компания** и выберите приложение AAD, которое вы создали для приложения API ToDoListAPI (средний уровень).
...
1. Войдите на [портал Azure](https://portal.azure.com/), откройте колонку **Приложение API** приложения API, в котором размещается проект TodoListAPI (средний уровень).

2. Щелкните элементы **Параметры > Параметры приложения**.

3. В разделе **Параметры приложения** добавьте следующие ключи и значения.

	|Ключ|Значение|Пример
	|---|---|---|
	|ida:Authority|https://login.microsoftonline.com/{your Имя клиента Azure AD}|https://login.microsoftonline.com/contoso.onmicrosoft.com|
	|ida:ClientId|Идентификатор клиента вызывающего приложения (ToDoListAPI)|960adec2-b74a-484a-960adec2-b74a-484a|
	|ida:ClientSecret|Ключ вызывающего приложения (ToDoListAPI)|oCgdj3EYLfnR0p6iR3UvHFAfkn+zQB+0VqZT/6=
	|ida:Resource|Идентификатор клиента вызываемого приложения (ToDoListDataAPI)|e65e8fc9-5f6b-48e8-e65e8fc9-5f6b-48e8|

	Если вы использовали одно приложение Azure AD для вызывающего приложения API и защищенного приложения API, вам нужно использовать одно и то же значение для `ida:ClientId` и `ida:Resource`.
...
Если требуется, чтобы браузер не мог получать доступ без записи кода в защищенное приложение API, вы можете изменить в приложении AAD **URL-адрес ответа** так, чтобы он отличался от базового URL-адреса приложения API.

### Отключение доступа через браузер

1. На классическом портале на вкладке **Настройка** приложения AAD, которое было создано для проекта TodoListService, измените значение в поле **URL-адрес ответа** так, чтобы оно являлось действительным URL-адресом, но не URL-адресом приложения API.
...
Для целей этого руководства добавьте код, который проверяет идентификатор приложения и идентификатор субъекта-службы непосредственно в действиях контроллера. В качестве альтернативы используйте пользовательский атрибут `Authorize` или выполните эту проверку в последовательностях запуска (например, промежуточный слой OWIN).

Внесите следующие изменения в проект TodoListDataAPI.

2. Откройте файл *Controllers/TodoListController.cs*.

3. Раскомментируйте строки, которые указывают значения `trustedCallerClientId` и `trustedCallerServicePrincipalId`.
...
3. В разделе **Параметры приложения** добавьте следующие ключи и значения.
	...
	|Ключ|Значение|Пример
	|---|---|---|
	|todo:TrustedCallerServicePrincipalId|Идентификатор субъекта-службы вызывающего приложения|4f4a94a4-6f0d-4072-4f4a94a4-6f0d-4072|
	|todo:TrustedCallerClientId|Идентификатор клиента вызывающего приложения, скопированный из приложения AAD TodoListAPI|960adec2-b74a-484a-960adec2-b74a-484a|

6. Щелкните **Сохранить**.

	![](./media/app-service-api-dotnet-service-principal-auth/trustedcaller.png)

6. В браузере еще раз откройте URL-адрес веб-приложения и на домашней странице откройте вкладку **To Do List**.
...
* Убедитесь, что при вводе параметров на портале Azure в колонке **Параметры приложения** были правильно выбраны приложение API или веб-приложение.
* Убедитесь, что вы переходите в браузере к URL-адресам HTTPS, а не HTTP.
* Убедитесь, что в приложении API среднего уровня по-прежнему включена поддержка CORS и разрешены вызовы к среднему уровню с внешнего URL-адреса HTTPS. Если вы сомневаетесь, связана ли проблема с CORS, попробуйте ввести "*" как URL-адрес разрешенного источника. **Важно!** Некоторые сообщения об ошибках консоли средств разработчика в браузере могут сообщать об ошибке CORS, хотя на самом деле проблема может заключаться в проверке подлинности на уровне данных. Чтобы выяснить, так ли это, временно отключите проверку подлинности для приложения API ToDoListDataAPI.
...
* Если другие методы не работают, попробуйте использовать [сеанс удаленной отладки](../app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md#remotedebug) и проверить значения переменных, передаваемых в код, который получает токен носителя в ToDoListAPI. Также проверьте код, который проверяет значения Azure AD, поступающие в ToDoListDataAPI. Обратите внимание, что код может выбирать значения конфигурации из множества различных источников, поэтому во время этой проверки возможны сюрпризы. Например, если вы во время настройки параметров службы приложений вместо `ida:ClientId` используете `ida:ClientID`, код может получить значение `ida:ClientId`, которое он ищет в файле Web.config, игнорируя параметр службы приложений. 
