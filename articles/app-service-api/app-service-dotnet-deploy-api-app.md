...
В этом разделе для создания приложения API в Azure используется мастер **публикации веб-проекта** Visual Studio. Когда вам будет предложено ввести имя для приложения API, введите *ContactsList*.
...
4. На открывшейся странице Swagger последовательно выберите **Контакты > Получить > Попробуйте в деле**.

	![Попробуйте сейчас](./media/app-service-dotnet-deploy-api-app/swaggerui.png)

## Просмотр определения API на портале Azure

1. На [портале предварительной версии Azure](https://portal.azure.com) вернитесь к колонке **Приложение API** для развернутого вами нового приложения API.
...
Вы видели, как возможности прямого развертывания в Visual Studio упрощают проверку правильности работы API. В [следующем учебнике](../app-service-dotnet-remotely-debug-api-app.md) будет показано, как отлаживать приложение API во время его выполнения в Azure.

Приложения API — это веб-приложения с дополнительными возможностями для размещения интерфейсов API. Это означает, что можно использовать любой метод развертывания, который подходит для веб-приложений. Дополнительную информацию о вариантах развертывания веб-приложений см. в статье [Развертывание веб-приложения в службе приложений Azure](../app-service-web/web-sites-deploy.md).

Информацию о функциях приложений API см. в статье [Что такое приложения API?](app-service-api-apps-why-best-platform.md).
