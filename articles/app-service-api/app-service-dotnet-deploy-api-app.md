...
В этом разделе вы используете мастер **веб-публикации** Visual Studio, чтобы создать приложение API в Azure.
...
В этом разделе вы используете тот же идентификатор мастера **веб-публикации**, чтобы развернуть свой код.

7. Щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите пункт **Опубликовать**, чтобы повторно открыть диалоговое окно публикации. Профиль публикации, созданный на предыдущем шаге, должен быть уже выбран. Нажмите кнопку **Опубликовать**, чтобы начать процесс развертывания. 
	...
	Во время процесса развертывания Visual Studio автоматически перезапускает *шлюз*. Шлюз — это веб-приложение, которое обрабатывает административные функции для всех приложений API в группе ресурсов, и его нужно перезапустить, чтобы распознать изменения в API в приложении API. Если для развертывания приложения API используется другой метод и если развертывание приводит к изменению API, возможно, потребуется перезапустить шлюз вручную. Информацию о перезагрузке шлюза на портале см. в статье [Управление приложением API](app-service-api-manage-in-portal.md).
