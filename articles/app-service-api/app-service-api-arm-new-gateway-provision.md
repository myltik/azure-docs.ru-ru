...
Значение используется ниже в качестве **variables('packageId')**. Он содержит идентификатор пакета NuGet для приложений API.
...
Обратите внимание, что для параметра **kind** задано значение **gateway**, что служит уведомлением для портала Azure, что это веб-приложение размещает шлюз. Портал не будет отображать веб-приложение в колонке обзора веб-приложений. Между размещением веб-приложения и шлюзом определяется связь. Раздел параметров приложения содержит значения, необходимые для размещения приложения API. **ServerFarmId** содержит имя плана службы приложений, указанное в параметре **hostingPlanName**.
...
Обратите внимание: для параметра **kind** задано значение **apiApp**. Для портала Azure это служит уведомлением о том, что в данном веб-приложении размещается приложение API. Портал не будет отображать веб-приложение в колонке обзора веб-приложений. Приложение содержит расширение для установки пустого пакета приложения API по умолчанию. Между приложением API и размещением веб-приложения определяется связь. Раздел параметров приложения содержит значения, необходимые для размещения приложения API. **ServerFarmId** содержит имя плана службы приложений, указанное в параметре **hostingPlanName**.
