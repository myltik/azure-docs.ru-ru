...
Среда выполнения службы приложений для приложений API предоставляется пакетом NuGet [Microsoft.Azure.AppService.ApiApps.Service](http://www.nuget.org/packages/Microsoft.Azure.AppService.ApiApps.Service/), а динамическая генерация метаданных API [Swagger](http://swagger.io/ "Официальные сведения о Swagger") — пакетом NuGet [Swashbuckle](http://www.nuget.org/packages/Swashbuckle/).
...
2. Когда Azure завершит создание приложения API, установите для этого приложения уровень доступа **Общедоступный (анонимный)**.
