...
1. [Node.js](nodejs.org), выполняемый на компьютере, на котором ведется разработка (в этом примере предполагается, что установлена версия Node.js 4.2.2).
...
1. Замените код в файле **handlers/contacts.js** на код ниже. Этот код использует данные JSON, хранящиеся в файле **lib/contacts.json**, который обрабатывается с помощью **lib/contactRepository.js**. Приведенный ниже новый код contats.js будет отвечать на HTTP-запросы на получение всех контактов с помощью этого кода.
...
1. Перейти на [портал Azure](http://portal.azure.com). 
