...
В этом руководстве описано, как создать простой API [Node.js](http://nodejs.org) и развернуть его в [приложении API](app-service-api-apps-why-best-platform.md) с помощью [службы приложений Azure](../app-service/app-service-value-prop-what-is.md). Для этой процедуры можно использовать любую операционную систему, поддерживающую Node.js, а на протяжении всей работы нужно использовать такие программы командной строки, как cmd.exe или Bash.

## Предварительные требования

1. Установленная платформа [Node.js](http://nodejs.org) (в этом примере предполагается использование Node.js версии 4.2.2).
...
1. Перейдите к папке, которую можно использовать в качестве локального репозитория Git, и клонируйте [репозиторий GitHub, содержащий пример кода](https://github.com/Azure-Samples/app-service-api-node-contact-list).
	...
	Пример API предоставляет две конечные точки: запрос Get к `/contacts` возвращает список имен и адресов электронной почты в формате JSON, а `/contacts/{id}` возвращает только выбранный контакт.

## Формирование шаблона кода Node.js на основе метаданных Swagger

[Swagger](http://swagger.io/) — это формат файла JSON для метаданных, описывающих API RESTful. В службу приложений Azure [встроена поддержка метаданных Swagger](app-service-api-metadata.md). Работая с этой частью руководства, вы сформируйте шаблон серверного кода для примера API на основе файла метаданных Swagger.

>[AZURE.NOTE] Если вас не интересует формирование шаблона файла Swagger, можно выполнить шаги руководства только по развертыванию примера кода для нового приложения API. Перейти сразу к разделу [Создание приложения API в Azure](#createapiapp).
...
1. Выполните следующие команды, чтобы установить модули npm **yo** и **generator-swaggerize** на глобальном уровне.
	...
	Swaggerize — это средство, которое позволяет создать серверный код для API, описываемого с помощью файла метаданных Swagger. Файл Swagger, который нужно будет использовать, называется *api.json*. Он расположен в клонированном репозитории в папке *start*.
...
2. Перейдите к папке *start*, а затем выполните команду `yo swaggerize`. Swaggerize выдаст запрос на ряд параметров. Для параметра **what to call this project** введите contactlist, для **path to swagger document** — api.json, а для **Express, Hapi, or Restify** — express.
	...
	**Примечание**. На этом шаге у вас может возникнуть ошибка. Способ ее устранения описан на следующем шаге.
	...
	Swaggerize создает папку приложения, формирует шаблоны обработчиков и файлов конфигурации, а также создает файл **package.json**. Чтобы создать страницу справки по Swagger, необходимо использовать обработчик представлений express.

3. Если выполнение команды `swaggerize` завершится ошибкой "непредвиденный маркер" или "недопустимая escape-последовательность", чтобы устранить причину ошибки, измените созданный файл *package.json*. В строке `regenerate` раздела `scripts` замените обратную косую черту перед *api.json* на косую черту, чтобы строка выглядела так:

 		"regenerate": "yo swaggerize --only=handlers,models,tests --framework express --apiPath config/api.json"

1. Перейдите к папке, в которой содержится сформированный шаблон кода (в данном случае в подпапку *ContactList*).
...
1. Замените код в файле **handlers/contacts.js** на код ниже.

	Этот код использует данные JSON, хранящиеся в файле **lib/contacts.json**, который обрабатывается с помощью **lib/contactRepository.js**. Показанный ниже новый код contats.js будет отвечать на HTTP-запросы, чтобы получать все контакты и возвращать их в виде полезных данных JSON.
...
1. Замените код в файле **handlers/contacts/{идентификатор}.js** на код ниже.
...
1. Замените код в файле **server.js** на код ниже.
...
В этом разделе рассматривается процесс создания пустого приложения API в Azure. Затем здесь описывается подключение приложения к репозиторию Git. Таким образом можно обеспечить непрерывную доставку изменений кода.
...
6. В раскрывающемся меню **Группа ресурсов** щелкните **Создать**, а затем в поле **Новое имя группы ресурсов** введите имя NodejsAPIAppGroup или любое другое.
	...
	[Группа ресурсов](../azure-portal/resource-group-portal.md) — это коллекция ресурсов Azure, таких как приложения API, базы данных, виртуальные машины и т. д. Мы рекомендуем создать новую группу ресурсов, так как это позволит быстро удалить все ресурсы Azure, созданные для работы с этим руководством.
...
5. В колонке **План службы приложений** введите NodejsAPIAppPlan или другое нужное имя.
...
Этот раздел руководства предусматривает создание учетных данных, которые вы будете использовать для развертывания, а также репозитория для приложения API с использованием службы приложений Azure. Вы развернете код в приложении API, отправив фиксации в этот репозиторий в службе приложений Azure.

1. После создания приложения API на домашней странице портала последовательно выберите **Службы приложений > {свое приложение API}**. 
...
1. После создания репозитория Git вид колонки изменится и в ней будут отображаться активные развертывания. Так как репозиторий новый, в списке не будет активных развертываний.

    ![Нет активных развертываний](media/app-service-api-nodejs-api-app/no-active-deployments.png)

1. Скопируйте URL-адрес репозитория Git. Чтобы сделать это, перейдите к колонке нового приложения API и просмотрите раздел **Основные компоненты**. Вы увидите **URL-адрес клона Git** в разделе **Основное**. Если навести указатель мыши на этот URL-адрес, справа вы увидите значок, который позволяет скопировать URL-адрес в буфер обмена. Щелкните этот значок, чтобы скопировать URL-адрес.
...
1. Если вы выполнили первую часть руководства, скопируйте папку `start\ContactList`, созданную с использованием компонента формирования шаблонов swaggerize, в другую папку. Или же можно скопировать папку `end\ContactList` в другую папку.
...
1. Выполните следующую команду, чтобы добавить удаленный репозиторий Git для своего репозитория приложения API.
    ...
    **Примечание**. Замените строку YOUR\_GIT\_CLONE\_URL\_HERE на URL-адрес клона Git, который скопировали ранее.

1. Выполните две команды ниже, чтобы создать фиксацию, которая будет содержать весь код.
...
1. В браузере вернитесь к колонке **Развертывания** своего приложения API, и вы увидите, что выполняется развертывание.
	...
	После завершения развертывания в колонке **Развертывания** отобразится сообщение об успешном развертывании изменений кода в приложении API.
