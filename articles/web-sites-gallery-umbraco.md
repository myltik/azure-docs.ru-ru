...
1. Выполните вход на [Портал управления Microsoft Azure](https://portal.azure.com/).

2. Щелкните значок **Коллекции Azure**.
...
7. Теперь после настройки веб-сайта и базы данных можно начать развертывание приложения, нажав кнопку **Создать** внизу первого раздела **Umbraco CMS** на предыдущем рисунке.
	
	![Click Create][06UmbracoCMSGroup]
	
После завершения развертывания начальная панель на портале показывает, что группа ресурсов для Umbraco CMS (в данном случае **UmbracoCMSgroup**) создана. В разделе **Сводка** щелкните имя веб-сайта (в данном случае **umbracocmsgroup**), чтобы увидеть его свойства. Кроме того, в разделе **Сводка** можно выбрать ресурс базы данных, чтобы увидеть свойства соответствующей базы данных.
	
![][07UmbracoCMSGroupBlade]

## Запуск и настройка веб-сайта Umbraco CMS ##

1. В разделе сведений для веб-сайта нажмите **Обзор**, чтобы выбрать ваш сайт (в данном случае это umbracocmsgroup.azurewebsites.net).
...
3. Укажите данные для подключения и проверки подлинности для базы данных, которая будет использоваться Umbraco. В качестве типа базы данных выберите **Microsoft SQL Azure**.  Строку подключения для базы данных можно получить в разделе **Параметры сайта** вашего веб-сайта.
...
6. Теперь будет создаваться образец текстовой страницы для публикации. Выберите **Содержимое**, затем **Переполнение**и щелкните **TextPage**.
