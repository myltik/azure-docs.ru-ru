...
Прежде чем начать, потребуется подключить диск данных к виртуальной машине, чтобы хранить файл подкачки Windows (pagefile.sys) во время этой процедуры. Если у вас его нет, ознакомьтесь со статьей [Как присоединить диск данных к виртуальной машине]. Указания, помогающие определить, какие диски подключены, см. в разделе «Управление дисками» статьи [О дисках виртуальных машин в Azure].

Если вы хотите использовать существующий диск данных на диске D, убедитесь, что вы также передали соответствующий VHD в учетную запись хранения. Инструкции см. в шагах 3 и 4 в разделе [Создание и отправка VHD Windows Server в Azure].
...
[О дисках виртуальных машин в Azure]: http.md://msdn.microsoft.com/library/azure/dn790303.aspx
[Создание и отправка VHD Windows Server в Azure]: virtual-machines-create-upload-vhd-windows-server.md
[Как войти в виртуальную машину под управлением Windows Server]: virtual-machines-log-on-windows-server.md
[Отключение диска данных от виртуальной машины]: storage-windows-detach-disk.md
[Об учетных записях хранения Azure]: storage-whatis-account.md
