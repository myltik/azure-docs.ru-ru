<properties
   pageTitle="Анализ тональностей и популярности в Twitter в реальном времени | Microsoft Azure"
   description="Узнайте, как использовать потоковую аналитику для анализа тональностей и популярности в Twitter в режиме реального времени. В этом учебнике описываются шаги от создания событий до отображения данных на живой панели мониторинга."
...
# Аналитика социальных сетей: анализ тональности в Twitter в режиме реального времени

В этом учебнике рассказывается о том, как построить решение — перенести события Twitter в концентраторы событий, составить запросы Stream Analytics для анализа данных, а затем сохранить результаты или использовать панель мониторинга для получения ценной аналитической информации в режиме реального времени.

## Сценарий

Новостной веб-сайт хочет получить конкурентное преимущество, мгновенно предлагая читателям содержимое, которое будет им интересно. Его создатели используют аналитические обзоры социальных сетей по темам, которые интересуют их читателей, анализируя данные из Twitter в режиме реального времени. В частности, чтобы определить наиболее популярные темы, им нужно в режиме реального времени анализировать количество твитов и тональность по ключевым темам.

## Предварительные требования
1.	Для выполнения инструкций в этом учебнике необходима учетная запись Twitter.  
2.	В этом пошаговом руководстве используется генератор событий, размещенный на GitHub. Скачайте его [здесь](https://github.com/streamanalytics/samples/tree/master/TwitterClient) и выполните следующие действия, чтобы настроить решение.
...
1.	На портале Azure нажмите кнопку **СОЗДАТЬ** > **СЛУЖБЫ ПРИЛОЖЕНИЙ** > **SERVICE BUS** > **КОНЦЕНТРАТОР СОБЫТИЙ** > **БЫСТРО СОЗДАТЬ** и укажите имя, регион и новое или существующее пространство имен для создания нового концентратора событий.  
...
Мы предоставили клиентское приложение, которое будет использовать данные Twitter с помощью [интерфейсов API REST Twitter](https://dev.twitter.com/rest/public) для сбора событий-твитов о параметризованном наборе тем. Открытый инструмент стороннего разработчика [Sentiment140](http://help.sentiment140.com/) используется для присвоения значения тональности каждому твиту (0: отрицательный; 2: нейтральный; 4: положительный) и последующей передачи событий-твитов в концентратор событий.
...
2.	Откройте файл App.config и замените маркеры Twitter oauth_consumer_key, oauth_consumer_secret, oauth_token, oauth_token_secret требуемыми значениями.  
...
3.	Замените значения EventHubConnectionString и EventHubName в файле App.config строкой подключения и именем концентратора событий.
4.	*Необязательно:* измените ключевые слова для поиска. По умолчанию это приложение выполняет поиск по ключевым словам «Azure,Skype,XBox,Microsoft,Seattle». При необходимости можно настроить значения для twitter_keywords в файле App.config.
5.	Выполните сборку решения.
6.	Запустите приложение. Вы увидите события-твиты, значения CreatedAt, Topic и SentimentScore которых будут отправляться концентратору событий:

	![Значения SentimentScore передаются в концентратор событий.](./media/stream-analytics-twitter-sentiment-analysis-trends/stream-analytics-twitter-sentiment-output-to-event-hub.png)

## Создание задания Stream Analytics

Теперь, когда у нас есть поток событий-твитов, можно настроить задание Stream Analytics, чтобы анализировать эти события в режиме реального времени.

### Подготовка задания Stream Analytics

1.	На [портале Azure](https://manage.windowsazure.com/) щелкните **СОЗДАТЬ** > **СЛУЖБЫ ДАННЫХ** > **АНАЛИТИКА ПОТОКА** > **БЫСТРО СОЗДАТЬ**.
...
4.	На третьей странице введите или выберите следующие значения:
	...
	* **ПСЕВДОНИМ ВХОДНЫХ ДАННЫХ** — введите понятное имя для входных данных этого задания, например TwitterStream. Впоследствии это имя будет использоваться в запросе. **КОНЦЕНТРАТОР СОБЫТИЙ** — если созданный концентратор событий находится в той же подписке, что и задание аналитики потока, выберите пространство имен, к которому принадлежит концентратор.
...
Для определения популярных тем мы будем искать темы, упоминания по которым превышают пороговое значение за определенный промежуток времени. Здесь мы проверим с помощью [SlidingWindow](https://msdn.microsoft.com/library/azure/dn835051.aspx) темы, которые упоминаются более 20 раз в течение 5 секунд.
...
Теперь, когда мы определили поток событий, входные данные концентратора событий для приема событий и запрос для выполнения преобразования потока, остался последний этап — определить приемник выходных данных задания. Мы запишем агрегированные события-твиты из нашего запроса задания в большой двоичный объект Azure. Вы также можете передать результаты в базы данных SQL, табличное хранилище или концентратор событий в зависимости от потребностей приложения.
...
2.	Выберите **ХРАНИЛИЩЕ БОЛЬШИХ ДВОИЧНЫХ ОБЪЕКТОВ**, а затем щелкните правой кнопкой мыши.
3.	На третьей странице введите или выберите следующие значения:
	...
	* **ПОДПИСКА** — если хранилище больших двоичных объектов создано в той же подписке, что и задание Stream Analytics, выберите **Использовать учетную запись хранения из текущей подписки**. Если хранилище связано с другой подпиской, выберите **Использовать учетную запись хранения из другой подписки** и вручную заполните поля **УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ**, **КЛЮЧ УЧЕТНОЙ ЗАПИСИ ХРАНЕНИЯ** и **КОНТЕЙНЕР**.
...
5.	Укажите следующие значения:
	* **ФОРМАТ СЕРИАЛИЗАТОРА СОБЫТИЙ** — JSON.
	* **КОДИРОВКА** — UTF8.
6.	Нажмите кнопку «Проверить», чтобы добавить этот источник и убедиться, что Stream Analytics может успешно подключаться к учетной записи хранения.

## Запуск задания

Входные данные, запрос и выходные данные заданы, так что теперь можно приступать к запуску задания потоковой аналитики.
...
чтобы просматривать выходные данные задания в реальном времени, воспользуйтесь такими инструментами, как [Обозреватель хранилищ Azure](https://azurestorageexplorer.codeplex.com/) или [Обозреватель Azure](http://www.cerebrata.com/products/azure-explorer/introduction). Здесь вы можете включить в приложение настраиваемую панель мониторинга выходных данных, как показано на рисунке ниже, используя [Power BI](https://powerbi.com/).

![Выходные данные Stream Analytics на панели мониторинга Power BI.](./media/stream-analytics-twitter-sentiment-analysis-trends/stream-analytics-output-power-bi.png)
