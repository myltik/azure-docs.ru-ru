...
- Кластер HDInsight. Дополнительную информацию о различных способах создания таких кластеров см. в разделе [Подготовка кластеров HDInsight][hdinsight-provision].

- Рабочая станция, на которой установлена и настроена среда Azure PowerShell. Указания см. в разделе [Установка и настройка Azure PowerShell][powershell-install-configure].

##В этом учебнике рассматриваются следующие темы:
1. [Ознакомление со сценарием](#scenario)
2. [Выполнение примера с использованием Azure PowerShell](#run-sample)	
3. [Код Java для программы счетчика слов MapReduce](#java-code)
...
* Процедура map берет каждую строку из входного текста в качестве входных данных и разбивает ее на слова. Она генерирует пару "ключ-значение" каждый раз, когда встречается слово, за которым следует 1. Перед отправкой в процедуру reduce выходные данные сортируются. 
...
* Программа MapReduce. В этом учебнике вы воспользуетесь примером счетчика слов, поставляемым с кластером HDInsight, так что вам не потребуется создавать собственную программу. Этот пример находится в файле /example/jars/hadoop-examples.jar. В кластерах HDInsight версии 3.0 файл называется hadoop-mapreduce-examples.jar. Указания по написанию собственного задания MapReduce см. в разделе [Разработка программ MapReduce на Java для HDInsight][hdinsight-develop-MapReduce-jobs].
* Входной файл. В качестве входного файла будет использоваться /example/data/gutenberg/davinci.txt. Дополнительную информацию об отправке файлов см. в разделе [Отправка данных в HDInsight][hdinsight-upload-data].
...
##<a id="run-sample"></a>Выполнение примера с использованием Azure PowerShell
...
4. Выполните следующую команду и укажите данные учетной записи Azure:
...
2. Выполните следующую команду, чтобы изменить каталог на c:\ root:
	...
	Каталогом по-умолчанию для Azure Powershell является *C:\Windows\System32\WindowsPowerShell\v1.0*. По умолчанию у вас нет разрешения на запись для этой папки. Необходимо изменить каталог на C:\ root или на папку, в которой у вас имеется разрешение на запись.
...
3. Выполните следующие команды для создания объекта контекста хранилища Azure:
	...
	Пример: *Командлет Select-AzureSubscription* используется для задания текущей подписки в случае, если имеется несколько подписок и должна использоваться подписка, отличная от подписки по умолчанию. 
...
##<a id="java-code"></a>Код Java для программы счетчика слов MapReduce
...
* [Разработка программ MapReduce на Java для HDInsight][hdinsight-develop-MapReduce-jobs]
* [Разработка программ MapReduce потоковой передачи Hadoop на C# для HDInsight][hdinsight-develop-streaming]
* [Использование Hive с HDInsight][hdinsight-use-hive]
* [Использование Pig с HDInsight][hdinsight-use-pig] 
* [Выполнение примеров HDInsight][hdinsight-samples]
