...
description: "Здесь объясняется, как можно использовать страницу \"Настройка\" в StorSimple Manager для добавления, изменения и удаления или смены ключей безопасности для учетной записи хранения."
services: storsimple
...
---
# <a name="use-the-storsimple-manager-service-to-manage-your-storage-account"></a>Использование службы StorSimple Manager для управления учетной записью хранения
> [!NOTE]
> Классический портал StorSimple устарел. Диспетчеры устройств StorSimple автоматически перейдут на новый портал Azure в соответствии с графиком устаревания. Вы получите сообщение электронной почты и уведомление с портала, касающиеся этого перехода. Этот документ скоро также перестанет использоваться. Чтобы просмотреть версию этой статьи для нового портала Azure, перейдите к статье [Использование службы диспетчера устройств StorSimple для управления учетными данными учетной записи хранения](storsimple-8000-manage-storage-accounts.md). Сведения, связанные с переходом, см. в [ответах на вопросы о перемещении на портал Azure](storsimple-8000-move-azure-portal-faq.md).

## <a name="overview"></a>Обзор
На странице **Настройка** представлены все параметры глобальной службы, создаваемые в службе StorSimple Manager. Эти параметры могут применяться для всех устройств, подключенных к службе, включая:
...
> [!NOTE]
> Процедура добавления учетной записи хранения зависит от используемой версии программного обеспечения StorSimple. Убедитесь, что процедуры для вашей версии StorSimple выполняются правильно.
> 
> 
...
1. На главной странице службы StorSimple Manager выберите службу, дважды щелкните ее имя и щелкните **Настроить**.
...
> [!NOTE]
> Информация о смене ключей и процедура замены применима только к учетным записям хранения Microsoft Azure. Если вы используете другого поставщика облачных служб, управлять ключами учетной записи хранения можно через панель мониторинга этого поставщика.
> 
> 
...
Ключи учетной записи хранения всегда хранятся в службе в зашифрованном виде. Их можно сбросить с помощью службы StorSimple Manager. Служба может получить первичный и вторичный ключи для всех учетных записей хранения в одной подписке, включая учетные записи, созданные в службе хранилища, а также учетные записи хранения по умолчанию, созданные при первом создании службы StorSimple Manager. Служба StorSimple Manager будет всегда получать эти ключи с классического портала Azure и хранить их в зашифрованном виде.

## <a name="rotation-workflow"></a>Рабочий процесс смены ключей
Администратор Microsoft Azure может повторно создавать и изменять первичный и вторичный ключи путем прямого доступа к учетной записи хранения (с помощью службы хранилища Microsoft Azure). Служба StorSimple Manager не выявляет эти изменения автоматически.

Чтобы сообщить службе StorSimple Manager об изменениях, необходимо получить доступ к службе StorSimple Manager, затем перейти к учетной записи хранения, после чего синхронизировать основной или дополнительный ключ (в зависимости от того, какой из них был изменен). Затем служба возвращает последний ключ, шифрует ключи и передает зашифрованный ключ на устройство.

#### <a name="to-synchronize-keys-for-storage-accounts-in-the-same-subscription-as-the-service-azure-only"></a>Синхронизация ключей для учетных записей хранения в той же подписке, что и служба (только Azure)
...
3. В диалоговом окне сделайте следующее.
   ...
   2. В службе StorSimple Manager необходимо обновить ключ, который ранее был изменен в службе хранилища Microsoft Azure. Если был изменен (создан повторно) основной ключ доступа, щелкните **Синхронизировать основной ключ**. Если был изменен дополнительный ключ, щелкните **Синхронизировать дополнительный ключ**.
...
3. В диалоговом окне сделайте следующее.
   ...
   2. Обновлять ключ доступа к хранилищу необходимо в службе StorSimple Manager. В этом случае его можно увидеть. Введите новый ключ в поле **Ключ доступа к учетной записи хранения**. 
