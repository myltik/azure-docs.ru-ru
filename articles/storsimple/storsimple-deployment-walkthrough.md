...
- Устройство должно быть полностью распаковано, установлено в стойку. Кроме того, к нему должны быть подключены все кабели питания, сети и последовательного доступа, как описано в разделах:
	...
	-  [Распаковка устройства 8100](storsimple-8100-hardware-installation.md)
	-  [Распаковка устройства 8600](storsimple-8600-hardware-installation.md)
...
- Порты в брандмауэре центра данных открыты для трафика iSCSI и облака, как описано в разделе [Требования к сети для устройства StorSimple]().
...
| | Проверьте, открыт ли порт 53. Это применимо только в том случае, если для устройства используется внешний DNS-сервер. Внутренний DNS должен автоматически разрешать внешние URL-адреса. | `Test-Port -comp dc1 -port 53 -udp -UDPtimeout 10000` </br>[Дополнительные сведения об этом командлете]()|
| | | |
| **NTP** | После ввода NTP-сервера сразу же запускается синхронизация времени. При вводе `time.windows.com` или общедоступных серверов времени проверьте, открыт ли UDP-порт 123. | [Загрузите и используйте этот сценарий](). |
| | | |
| **Прокси (необязательно)** | Это допустимые URI и порт прокси-сервера? </br> Правильно ли установлен режим проверки подлинности? | `wget http://bing.com % {$_.StatusCode}`</br>Эту команду следует выполнить сразу после настройки веб-прокси. Если возвращается ответ 200, это значит, что соединение установлено успешно. |
...
| | | |
| **Регистрация** | Проверьте, открыты ли исходящие порты TCP 443, 80, 9354. | `Test-NetConnection -Port   443 -InformationLevel Detailed`</br>[Дополнительные сведения для командлета Test-NetConnection]() |
...
Выполните следующие шаги на портале Azure.
...
Выполните следующие действия на портале Azure, чтобы завершить минимальную настройку устройства.
...
Чтобы создать контейнер томов, выполните следующие действия на портале Azure.
...
Чтобы создать запланированную резервную копию, выполните следующие шаги на портале Azure.
...
Выполните следующие действия на странице **Служба StorSimple Manager** на портале Azure.
...
На портале Azure выполните следующие шаги, чтобы получить возможность по запросу создавать резервные копии отдельного тома на своем устройстве StorSimple.
