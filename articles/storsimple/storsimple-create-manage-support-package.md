...
## Создание пакета поддержки на портале управления
Для решения любых проблем, которые могут возникнуть в службе диспетчера StorSimple, можно создать и загрузить пакет поддержки на сайт службы поддержки Майкрософт на странице **Обслуживание** службы на портале управления. Для загрузки будет необходимо указать секретный ключ поддержки. Секретный ключ поддержки будет предоставлен специалистом технической поддержки по электронной почте. Пакет поддержки создается в незашифрованном и сжатом виде (CAB-файл). Затем специалист технической поддержки может загрузить этот пакет с сайта технической поддержки, указав секретный ключ поддержки.

Для создания пакета поддержки выполните следующие действия на портале управления.

#### Создание пакета поддержки на портале управления

1. Перейдите в раздел **Устройства > Обслуживание**.

1. В разделе **Пакет поддержки** щелкните **Создание и загрузка пакета поддержки**.

	

1. В окне **Создание и загрузка пакета поддержки** выполните следующие действия.
...
1. В сеансе Windows PowerShell подключитесь к пространству консоли SSAdmin устройства:
...
1. В открывшемся сеансе введите соответствующую команду.
	...
	- Для открытых общих сетевых папок (тех, которые не защищены паролем), указывать учетные данные не требуется. Введите следующую команду:

		`Export-HcsSupportPackage –PackageTag "MySupportPackage" -Force`

		Пакет поддержки будет создан для обоих контроллеров в указанной общей сетевой папке. Это зашифрованный сжатый файл, который можно отправить в службу поддержки Майкрософт для устранения неполадок. Подробная информация приведена в статье [Обращение в службу поддержки Майкрософт](https://msdn.microsoft.com/library/dn757750.aspx).
...
| 6 | Область | Необязательно | Задает **кластер** (по умолчанию) для создания пакета поддержки для обоих контроллеров. Если требуется создать пакет только для текущего контроллера, укажите **контроллер**. |
...
> [AZURE.IMPORTANT]Изменить можно только пакет поддержки, который был создан через Windows PowerShell для StorSimple. Пакет, который был создан на портале управления в службе диспетчера StorSimple, изменить нельзя.
...
1. Создайте пакет поддержки с помощью Windows PowerShell для StorSimple в соответствии с инструкциями в статье [Создание пакета поддержки](#Create-a-Support-package-in-Windows-PowerShell-for-StorSimple).

1. [Загрузите скрипт](http://gallery.technet.microsoft.com/scriptcenter/Script-to-decrypt-a-a8d1ed65) в локальную систему на клиентском компьютере.


1. Импортируйте модуль Windows PowerShell. Потребуется указать путь к локальному каталогу, в который вы загрузили скрипт. Для импорта модуля введите команду:
 
	`Import-module <Path to the folder that contains the Windows PowerShell script>`

1. Откройте папку с пакетом поддержки. Обратите внимание, что все файлы имеют расширение *.aes*, т. е. они сжаты и зашифрованы. Откройте файлы. Чтобы открыть файлы, введите команду:
...
1. При появлении запроса введите парольную фразу, которая использовалась при создании пакета поддержки.
...
1. Перейдите в папку, содержащую файлы журналов. Так как файлы журналов теперь распакованы и расшифрованы, у них будут исходные расширения файлов. Измените эти файлы, удалив специфичную информацию о пользователе, такую как имена томов и IP-адреса устройств, и сохраните файлы.

1. Закройте файлы. При закрытии файлов они будут сжаты Gzip и затем зашифрованы с AES-256. Это выполняется по соображениям безопасности и для сокращения времени пересылки пакета поддержки по сети. Чтобы закрыть файлы, введите:

	`Close-HcsSupportPackage <Path to the folder that contains support package files>`

	![Изменение пакета поддержки 2](./media/storsimple-create-manage-support-package/IC750707.png)

1. По запросу введите парольную фразу для шифрования для измененного пакета поддержки.

	    cmdlet Close-HcsSupportPackage at command pipeline position 1
    	Supply values for the following parameters:EncryptionPassphrase: ****

1. Запишите новую парольную фразу, чтобы сообщить ее специалистам службы технической поддержки Майкрософт по их запросу.
