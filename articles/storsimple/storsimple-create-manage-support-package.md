...
## Создание и отправка пакета поддержки на классическом портале Azure
...
> [AZURE.NOTE] Для отправки требуется ключ доступа к поддержке. Он должен быть предоставлен вам сотрудником службы поддержки по электронной почте.
...
## Создание пакета поддержки вручную
...
### Параметры командлета Export-HcsSupportPackage
...
> [AZURE.IMPORTANT] Изменить можно только пакет поддержки, который был создан через Windows PowerShell для StorSimple. Пакет, созданный на классическом портале Azure в службе диспетчера StorSimple, изменить нельзя.
