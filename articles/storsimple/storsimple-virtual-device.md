...
- Открытый ключ сертификата, используемый для шифрования данных, которые хранятся в Azure StorSimple, безопасно используется классическим порталом Azure, а закрытый ключ хранится на устройстве StorSimple. На виртуальном устройстве StorSimple и открытый, и закрытый ключи хранятся в Azure.
...
- У вас есть учетная запись на классическом портале Azure и учетные данные для доступа к нему.
