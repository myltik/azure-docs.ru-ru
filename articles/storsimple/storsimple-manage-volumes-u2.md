...
# Использование службы диспетчера StorSimple для управления томами
...
Многоуровневый том можно назначить архивным, установив флажок **Используйте этот том для архивных данных, доступ к которым осуществляется нечасто**. В архивных томах для блока дедупликации используется размер 512 КБ, а в неархивных — 64 КБ. Увеличенный размер блока дедупликации позволяет устройству передавать большие блоки данных в облако.
...
3. В нижней части страницы нажмите кнопку **Добавить**. Откроется мастер добавления тома.
     ...
     ![Основные параметры мастера добавления тома](./media/storsimple-manage-volumes-u2/AddVolume1.png)

4. В окне мастера добавления тома в разделе **Основные параметры** сделайте следующее.

  1. Укажите имя тома в поле **Имя**.
  2. В раскрывающемся списке выберите пункт **Тип использования**. Для рабочих нагрузок, требующих, чтобы данные были в любое время доступны на локальном устройстве, выберите **Локально закрепленный**. Для любых других типов данных выберите **Многоуровневый**. (**Многоуровневый** — значение по умолчанию.)
  ...
  4. Введите значение в поле **Подготовленная емкость** для тома в ГБ или ТБ. В разделе [Подготовленная емкость](#provisioned-capacity) приведены максимальные размеры каждого устройства и типа тома. Параметр **Доступная емкость** указывает, какой объем памяти фактически доступен на устройстве.
    ...
    При настройке локально закрепленного тома появится следующее сообщение.

    ![Сообщение об изменении типа тома](./media/storsimple-manage-volumes-u2/LocalVolMessage.png)
...
5. Щелкните значок с изображением стрелки ![Значок стрелки](./media/storsimple-manage-volumes-u2/HCS_ArrowIcon.png), чтобы перейти на страницу **Дополнительные параметры**.

6. В разделе **Дополнительные параметры** добавьте новую запись контроля доступа:
  
  1. Выберите запись контроля доступа (ACR) в раскрывающемся списке. Кроме того, можно добавить новую запись ACR. Записи ACR определяют, какие узлы могут получать доступ к томам, сопоставляя IQN узлов с томами, перечисленными в записи. Если не указать ACR, появится следующее сообщение.
...
6. Сохраните изменения, щелкнув значок с изображением флажка ![значок с изображением флажка](./media/storsimple-manage-volumes-u2/HCS_CheckIcon.png). На портале Azure отобразится сообщение об обновлении тома. При успешном обновлении тома будет отображаться соответствующее сообщение.
...
Чтобы просмотреть видео о расширении тома, щелкните [здесь](http://azure.microsoft.com/documentation/videos/expand-a-storsimple-volume).

## Изменение типа тома

Тип тома можно изменить с многоуровневого на локально закрепленный или наоборот. Как правило, изменение выполняется для небольших томов, к которым часто осуществляется доступ. Однако такое преобразование не следует выполнять часто. Вот некоторые причины для преобразования тома из многоуровневого в локально закрепленный:
...
При создании локально закрепленный том полностью подготавливается. Перед началом преобразования многоуровневого тома в локально закрепленный StorSimple проверяет наличие достаточного пространства на устройстве. Если места недостаточно, появится сообщение об ошибке и операция будет отменена.
...
4. На странице **Основные параметры** измените тип использования, выбрав новый тип из раскрывающегося списка **Тип использования**.
    ...
    ![Флажок архивации](./media/storsimple-manage-volumes-u2/ModifyVolume1.png)

5. Щелкните значок с изображением стрелки ![Значок стрелки](./media/storsimple-manage-volumes-u2/HCS_ArrowIcon.png), чтобы перейти на страницу **Дополнительные параметры**.

    Появится указанное ниже сообщение.

    ![Сообщение об изменении типа тома](./media/storsimple-manage-volumes-u2/TypeChangeMessage.png)
...
5. В мастере «Изменение тома» в разделе **Основные параметры** откройте раскрывающийся список **Мониторинг** и выберите **Включить** или **Отключить**.
