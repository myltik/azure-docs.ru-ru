...
2. Укажите один из IP-адресов интерфейса в качестве целевого портала или IP-адреса обнаружения и подключитесь к устройству. После подключения устройства система Windows сможет получить доступ к томам. Дополнительные сведения об использовании инициатора Microsoft iSCSI см. в статье [Установка и настройка инициатора iSCSI][1] в разделе «Подключение к целевому iSCSI-оборудованию».
...
5. Инициализируйте тома, выполнив следующие действия:
   ...
   3. В диалоговом окне **Инициализация дисков** выберите диски, которые нужно инициализировать, а затем нажмите кнопку **ОК**.
...
7. Отформатируйте тома с несколькими разделами. Инструкции см. в статье [Реализация управления дисками](https://msdn.microsoft.com/library/dd163556.aspx) в разделе «Разделы и тома».
...
2. На панели **Область** щелкните узел **Тома**. На панели **Результаты** появится список локальных и подключенных томов, включая все тома Azure StorSimple. Столбцы на панели **Результаты** можно настраивать. (Щелкните правой кнопкой мыши узел **Тома**, а затем последовательно выберите пункты **Вид** и **Добавить или удалить столбцы**.)
    ...
    :--------------|:-------------
    Имя | В столбце **Имя** указывается буква диска, назначенная обнаруженному тому.
    Устройство | Столбец **Устройство** содержит IP-адрес устройства, подключенного к главному компьютеру.
    Имя тома устройства | В столбце **Имя тома устройства** указывается имя тома устройства, к которому относится выбранный том. Это имя тома, определенное на портале управления Azure для конкретного тома.
...
5. Откроется диалоговое окно **Удаление тома**. Введите **Confirm** (Подтверждаю) в текстовое поле и нажмите кнопку **ОК**.
...
8. Щелкните правой кнопкой мыши имя группы томов.
    ...
    - Чтобы запустить интерактивное задание резервного копирования (по запросу), щелкните **Сделать архив**. 
...
7. Вы можете отслеживать ход выполнения задания архивации. На панели **Область** разверните узел **Задания**, а затем щелкните **Выполняются**. Информация о задании отобразится на панели **Результаты**. После завершения задания резервного копирования информация о нем вносится в список заданий **За последние 24 часа**.

## Дальнейшие действия

- Узнайте, как [администрировать решение StorSimple с помощью диспетчера моментальных снимков StorSimple](storsimple-snapshot-manager-admin.md).
