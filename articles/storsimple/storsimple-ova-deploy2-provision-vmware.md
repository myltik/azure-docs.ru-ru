...
-   Вы изучили сетевые требования к развертыванию виртуального устройства StorSimple и настроили сеть центра обработки данных в соответствии с требованиями. Чтобы получить дополнительные сведения, см. руководство по системным требованиям к виртуальному массиву Microsoft Azure StorSimple.
...
1.  Скопируйте образ виртуального устройства в систему. Это образ, скачанный с классического портала Azure. Запишите расположение, в которое был скопирован образ. Эти сведения потребуются вам позднее.
...
1.  Сначала необходимо отправить файл VMDK на сервер ESXi. Перейдите на вкладку **Configuration** (Конфигурация) на панели справа. В списке **Hardware** (Оборудование) выберите пункт **Storage** (Хранилище).
...
1.  Теперь необходимо найти скачанный файл VMDK и указать его.

	![](./media/storsimple-ova-deploy2-provision-vmware/image12.png)

1.  Щелкните **Открыть**. Начнется отправка файла VMDK в указанное хранилище данных.

	![](./media/storsimple-ova-deploy2-provision-vmware/image13.png)

1.  Отправка файла может занять несколько минут. По завершении отправки файл появится в созданной папке в хранилище данных.
...
1.  На странице **Virtual Machine Version** (Версия виртуальной машины) выберите параметр **Virtual Machine Version: 8** (Версия виртуальной машины: 8). Обратите внимание, что это единственный вариант, который поддерживается для данного выпуска.
...
1.  На странице **Select Existing Disk** (Выбор существующего диска) в разделе **Disk File Path** (Путь к файлу диска) нажмите кнопку **Browse** (Обзор). Откроется диалоговое окно **Browse Datastores** (Обзор хранилищ данных). Перейдите к расположению, в которое был передан файл VMDK. Выберите файл и нажмите кнопку **ОК**. Нажмите кнопку **Далее**.
