<properties 
   pageTitle="Использование службы диспетчера StorSimple для изменения конфигурации устройства StorSimple | Microsoft Azure" 
...
Устройству StorSimple, которое подключено к службе диспетчера StorSimple, присваивается имя по умолчанию. Оно обычно содержит серийный номер устройства. Например, имя устройства по умолчанию из 15 символов, такое 8600-SHX0991003G44HT, указывает на следующее:
...
- **Скорость** — этот параметр не настраивается пользователем. DATA 0, DATA 1, DATA 4 и DATA 5 всегда поддерживают 1 GbE, а DATA 2 и DATA 3 — 10 GbE.
     ...
     >[AZURE.NOTE]>>- Скорость и дуплекс всегда согласуются автоматически. >- Кадры крупных размеров не поддерживаются.
...
- **IP-адрес** — это может быть IPv4-адрес, IPv6-адрес или и то, и другое. Для сетевых интерфейсов устройства поддерживаются семейства адресов IPv4 и IPv6. При использовании IPv4-адреса укажите 32-разрядный IP-адрес (*xxx.xxx.xxx.xxx*) в точечно-десятичной нотации. При использовании IPv6--адрес просто введите префикс из 4 цифр, а 128-разрядный адрес для сетевого интерфейса будет создан автоматически на основе этого префикса.

- **Подсеть** — это маска подсети, которая настраивается с помощью интерфейса Windows PowerShell.

- **Шлюз** — это шлюз по умолчанию, который должен использовать интерфейс при попытке связаться с узлами, которые находятся не в том же адресном пространстве IP (подсети). Шлюз по умолчанию должен быть в том же адресном пространстве (подсети), что и IP-адрес интерфейс (определяется маской подсети).
