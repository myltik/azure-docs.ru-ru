...
|:---|:-------------------|:-------------------|
|1|Один контроллер находится в состоянии сбоя, другой контроллер исправен и работает.|[Замена одного контроллера](#replace-a-single-controller)|
|2|Оба контроллера неисправны и требуют замены. Корпус, диски и корпуса дисков исправны.|[Замена двух контроллеров](#replace-both-controllers)|
...
|4\.|Один контроллер отсутствует, а во втором произошел сбой.|[Замена двух контроллеров](#replace-both-controllers)|
...
|6|Контроллеры имеют различную версию сборки, что может возникать по следующим причинам:<ul><li>Контроллеры имеют различные версии ПО.</li><li>Контроллеры имеют различные версии встроенного ПО.</li></ul>|Если контроллеры имеют различные версии ПО, логика замены обнаруживает это и обновляет версию ПО на новом контроллере.<br><br>Если контроллеры имеют различные версии встроенного ПО и старая версия встроенного ПО **не** может быть обновлена автоматически, в портале управления появится предупреждающее сообщение. Следует проверить наличие обновлений и установить обновления встроенного ПО.</br></br>Если контроллеры имеют различные версии встроенного ПО и старая версия встроенного ПО может быть обновлена автоматически, логика замены контроллеров обнаружит это, и после запуска контроллера встроенное ПО будет обновлено автоматически.|

Если в модуле контроллера произошел сбой, его необходимо удалить. Сбой может произойти в одном или обоих модулях контроллеров, что приведет к замене одного контроллера или замене двух контроллеров. Процедуры замены описаны в следующих разделах:
...
>[AZURE.IMPORTANT]Перед снятием и заменой контроллера ознакомьтесь со сведениями о безопасности в разделе [Замена компонентов оборудования StorSimple](storsimple-hardware-component-replacement.md).
...
При сбое одного из контроллеров устройства Microsoft Azure StorSimple выполните следующие действия. (Другой контроллер должен быть включен и запущен. Если оба контроллера находятся в состоянии сбоя или не работают, перейдите к [Действиям по замене двух контроллеров](#dual-controller-replacement-steps).)
...
1. На портале управления в службе диспетчера StorSimple щелкните вкладку **Устройства**, а затем щелкните имя устройства, которое требуется отслеживать.

2. Щелкните вкладку **Обслуживание**, затем выберите **Состояние оборудования**. Состояние контроллера 0 или 1 должно отображаться красным цветом, который указывает на сбой.
...
5. Для снятия неисправного контроллера следуйте указаниям из раздела [Снятие контроллера](#remove-a-controller).
...
8. После перезагрузки контроллера проверьте **Состояние контроллера** и **Состояние кластера** в портале управления и убедитесь, что контроллер вернулся в работоспособное состояние и находится в режиме ожидания.

>[AZURE.NOTE]При отслеживании устройства через последовательную консоль можно заметить несколько перезапусков при восстановлении контроллера после процедуры замены. При появлении меню последовательной консоли процедура замены считается завершенной. Если меню не отображается в течение двух часов после начала замены контроллера, пожалуйста, [обратитесь в службу поддержки Microsoft](storsimple-contact-microsoft-support.md).
...
6. Включите устройство StorSimple. Для модели 8600:
    ...
    >[AZURE.NOTE]При отслеживании устройства через последовательную консоль можно заметить несколько перезапусков при восстановлении контроллера после процедуры замены. При появлении меню последовательной консоли процедура замены считается завершенной. Если меню не отображается в течение 2,5 часа после начала замены контроллера, пожалуйста, [обратитесь в службу поддержки Microsoft](storsimple-contact-microsoft-support.md).
...
5. Чтобы убедиться, что замена выполнена успешно, на портале управления выберите **Устройства** > **Обслуживание** > **Состояние оборудования** и убедитесь, что оба контроллера 0 и 1 работают (их индикаторы зеленые).
...
- [Определение активного контроллера с помощью портала управления](#use-the-management-portal-to-identify-the-active-controller)
...
### Определение активного контроллера с помощью портала управления

В портале управления выберите **Устройства** > **Обслуживание** и перейдите к разделу **Контроллеры**. Здесь можно проверить, какой контроллер активен.

![Определение активного контроллера с помощью портала управления](./media/storsimple-controller-replacement/IC752072.png)

**Рис. 6.** Портал управления с активным контроллером
...
![Заглавное сообщение последовательной консоли](./media/storsimple-controller-replacement/IC741098.png)
