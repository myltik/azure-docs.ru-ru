...
| Размер общей папки или тома |Многоуровневые — до 20 ТБ, локально закрепленные — до 2 ТБ |
...
|---------|---------|
| Прозрачная интеграция | Виртуальный массив поддерживает протокол iSCSI или SMB. Это гарантирует, что данные, сохраненные в облаке, в центре обработки данных или на удаленных серверах, будут сохранены в одном местоположении.|
| Снижение затрат на хранилище | StorSimple позволяет подготовить локальное хранилище достаточного объема в соответствии с текущими требованиями. По мере роста потребностей хранения StorSimple распределяет данные по уровням экономичного облачного хранилища. В облаке это решение использует дедупликацию и сжатие для дальнейшего уменьшения требований к хранилищу.|
| Упрощенное управление хранилищем | StorSimple обеспечивает централизованное управление в облаке. Для настройки данных, хранящихся в виртуальном массиве, на удаленном сервере и в облаке, и управления ими можно использовать веб-инструменты.| 
| Усовершенствованное аварийное восстановление и соответствие | StorSimple не требует дополнительного времени восстановления. Вместо этого метаданные восстанавливаются немедленно, а хранимые данные — по мере необходимости. Это означает, что нормальная работа может быть продолжена с минимальными простоями.|
...
- **Восстановление тома или общей папки**. Используйте восстановление в качестве нового рабочего процесса для восстановления тома или общей папки. Этот подход можно использовать для восстановления всего тома или общей папки.
- **Восстановление на уровне элементов**. Общие папки упрощают доступ к последним резервным копиям. Можно легко восстановить отдельный файл из специальной папки .backup, доступной в облаке. Этой возможностью восстановления управляет пользователь, поэтому вмешательство администратора не требуется.
- **Аварийное восстановление**. Используйте возможность отработки отказа для восстановления всех общих папок или томов на новом виртуальном массиве. Нужно создать виртуальный массив и зарегистрировать его в службе диспетчера StorSimple, а затем выполнить отработку отказа для исходного виртуального массива. Предполагается, что новый виртуальный массив использует подготовленные ресурсы. 

## Компоненты виртуального массива

Виртуальный массив содержит следующие компоненты.

- [Виртуальный массив](#virtual-array) — гибридное облачное хранилище на основе виртуальной машины, подготовленной в виртуализированной среде или низкоуровневой оболочке.  
