...
# Использование пользовательского веб-интерфейса для администрирования виртуального массива StorSimple (предварительная версия)

![последовательность операций процесса настройки](./media/storsimple-ova-web-ui-admin/manage4.png)

## Обзор

Руководства, приведенные в этой статье, относятся только к виртуальному массиву Microsoft Azure StorSimple (также известному как локальное виртуальное устройство StorSimple и виртуальное устройство StorSimple) версии 1.1.1.0 (предварительной версии). Здесь описываются некоторые сложные рабочие процессы и задачи управления, которые можно выполнять с виртуальным массивом StorSimple. Для управления виртуальным массивом StorSimple можно использовать два пользовательских интерфейса — интерфейс службы диспетчера StorSimple (это пользовательский интерфейс портала) или локальный пользовательский веб-интерфейс для устройства. В этой статье речь идет о задачах, выполняемых с использованием пользовательского веб-интерфейса.
...
2. В нижней части страницы щелкните **Запустить диагностические тесты**. Это действие запустит тестирование для выявления возможных проблем с настройками сети, устройства, веб-прокси, времени или облака. Вы получите уведомление о том, что устройство тестируется.
...
1. В локальном пользовательском веб-интерфейсе последовательно выберите **Устранение неполадок** > **Журналы системы**.
...
Чтобы завершить работу виртуального устройства или перезапустить его, можно использовать локальный пользовательский веб-интерфейс.

#### Завершение работы виртуального устройства

1. В локальном пользовательском веб-интерфейсе последовательно выберите **Обслуживание** > **Параметры питания**.
...
![video\_icon](./media/storsimple-ova-web-ui-admin/video_icon.png) **Доступно видео**
...
Узнайте, как [использовать службу диспетчера StorSimple для управления устройством](storsimple-manager-service-administration.md).
