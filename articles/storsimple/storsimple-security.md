...
> [AZURE.NOTE]Ключ регистрации службы — это длинный ключ, который содержит больше 100 символов. Вы можете скопировать ключ и сохранить его в текстовом файле в безопасном расположении, чтобы вы могли при необходимости использовать его для авторизации дополнительных устройств.
> 
...
> * Мы рекомендуем повторно создавать ключ регистрации службы после каждого использования.
...
Чтобы получить дополнительные сведения и настроить CHAP, перейдите к статье [Настройка CHAP для устройства StorSimple](storsimple-configure-chap.md).
...
- Используйте надежные пароли. Дополнительные сведения см. в статье [Создание более надежных паролей и их защита](http://blogs.microsoft.com/cybertrust/2014/08/25/create-stronger-passwords-and-protect-them/).
...
> * Если ключ шифрования данных службы потерян, сотрудник службы поддержки Майкрософт может помочь вам извлечь его при условии, что хотя бы одно устройство подключено к Интернету. После извлечения ключа шифрования данных рекомендуется изменить его. Для получения инструкций перейдите в разделу [Изменение ключа шифрования данных службы](storsimple-service-dashboard.md#change-the-service-data-encryption-key).
> * Ключ шифрования данных службы и сертификат шифрования данных являются бессрочными. Но для обеспечения защиты ключа шифрования мы рекомендуем менять его ежегодно. </li></ul>
...
- Корпорация Майкрософт рекомендует изменять ключ шифрования облачного хранилища раз в квартал. Это не принудительная процедура.
...
**Ответ.** Этот ключ был создан при создании службы StorSimple Manager. При использовании службы диспетчера StorSimple для подключения к устройству вы можете использовать страницу быстрого запуска службы, чтобы просмотреть или повторно создать ключ регистрации службы. Создание нового ключа регистрации службы не влияет на существующие зарегистрированные устройства. Подробные инструкции описаны в разделе
...
**Вопрос.** Устройству было разрешено изменить ключ шифрования данных службы, но этот процесс так и не начался. Что делать?
...
**Вопрос.** Я хочу отправить пароль StorSimple Snapshot Manager на узел, который подключается к устройству StorSimple, но пароль недоступен. Что можно сделать?
...
**Ответ.** Вы можете скачать новый сертификат из службы StorSimple Manager, а затем предоставить его для установки в хранилище сертификатов на клиентах удаленного доступа. Подробные инструкции описаны в разделе

- [Командлет Import-Certificate](https://technet.microsoft.com/library/hh848630.aspx)

**Вопрос.** Остаются ли мои данные защищенными, если служба StorSimple Manager взломана?
