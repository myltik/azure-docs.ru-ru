...
3. В мастере **Добавление ролей и компонентов** выполните следующие шаги.
   ...
   3. На странице **Выбор целевого сервера** выберите **Select a server from the server pool** (Выбор сервера из пула серверов). Сервер должен быть найден автоматически. Щелкните **Далее**.
