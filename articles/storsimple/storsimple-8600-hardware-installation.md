...
1. См. следующую схему сетевого подключения. (Минимальная конфигурация отображается сплошными линиями синего цвета. Дополнительные подключения для обеспечения высокого уровня доступности и производительности отображаются в виде пунктирных линий.)
...
- При возможности подключите интерфейсы каждого контроллера по крайней мере к двум различным коммутаторам для обеспечения доступности при сбоях коммутатора. На рисунке показаны два сетевых интерфейса 10 Гбит/с, DATA 2 и DATA 3, соединяющие каждый контроллер с двумя различными коммутаторами. Дополнительные сведения см. в разделе **Сетевые интерфейсы** статьи [Требования к высокой доступности для устройства StorSimple](storsimple-system-requirements/#high-availability-requirements-for-storsimple.md).
