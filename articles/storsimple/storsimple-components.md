...
Дополнительные сведения см. в разделе [Использование службы диспетчера StorSimple для администрирования устройства StorSimple](storsimple-manager-service-administration.md).
...
| размер кластера (AUS) | Минимальный объем места на диске, может быть выделен для хранения файла в файловых системах Windows. Если размер файла не кратен размеру кластера, приходится использовать дополнительное пространство для хранения файла (до следующего размера, кратного размеру кластера), что приводит к потере пространства и фрагментации жесткого диска. <br>Рекомендуемый AUS для томов Azure StorSimple составляет 64 КБ, поскольку с таким размером кластера хорошо работают алгоритмы дедупликации.|
