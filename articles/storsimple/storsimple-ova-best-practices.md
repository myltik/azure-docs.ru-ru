...
| **Тип диска данных** | При подготовке настройте диск как **динамически расширяемый**.<br></br> Если выбрать параметр **Фиксированный размер**, процедура займет много времени. <br></br> Не используйте параметр **Разностный**. | Используйте параметр **Тонкая подготовка**. |
| **Изменение диска данных** | Расширение и сжатие не допускаются. Попытка сделать это приведет к потере локальных данных на устройстве. | Расширение и сжатие не допускаются. Попытка сделать это приведет к потере локальных данных на устройстве. |

### Определение размера

При планировании размера виртуального массива StorSimple необходимо учитывать следующие моменты:

- Зарезервированное пространство для общих папок или томов на локальном уровне. Приблизительно 12 % пространства резервируется на локальном уровне для каждого подготовленного многоуровневого тома или общей папки. Примерно 10 % пространства также резервируется для локально закрепленного тома в файловой системе.
...
- Необходимость в восстановлении. При определении размера следует учитывать пространство, необходимое для восстановления, выполняемого в качестве новой операции. Обратите внимание, что после восстановления общая папка или том будет иметь такой же или больший размер.
...
В-третьих, на данном этапе нам понадобится 15 % от общего локального пространства для хранения локальных моментальных снимков, так что доступными останутся только 85 %. В этом примере это составит около 990 ГБ — 0,85* ТБ на подготовленном диске данных. Таким образом, объем подготовленного диска данных будет равен 1164 ГБ (990 * *(1 / 0,85)), т. е. примерно 1,16–1,25 ТБ (округление к наиболее близкому значению квартиля).
...
> [AZURE.NOTE] Кроме того, рекомендуется применять к локальному диску метод тонкой подготовки. Это связано с тем, что место для восстановления требуется только в случае, если необходимо восстановить данные, хранящиеся дольше 5 дней. Восстановление на уровне элементов позволяет восстановить данные за последние 5 дней, не требуя дополнительного места.
...
Так как 12 % локального пространства зарезервировано для многоуровневых томов и общих папок, а 10 % — для локально закрепленных томов и общих ресурсов, нам понадобится:

- 240 ГБ локально зарезервированного пространства (для многоуровневого тома и общей папки объемом 2 ТБ);
- 330 ГБ для локально закрепленного тома или общей папки.

Общее пространство, требуемое на локальном уровне, составляет 570 ГБ (240 + 330).

Минимальное локальное пространство, необходимое для восстановления, составит 330 ГБ.

15 % от общего места на диске будет использоваться для хранения моментальных снимков, поэтому только 0,85 объема будет доступно. Таким образом, размер диска будет составлять примерно 1,06–1,25 ТБ (900*(1/0.85)) с округлением к наиболее близкому значению квартиля.
...
-   Если планируется развернуть устройство в качестве сервера iSCSI:
	...
	-   Настройте статические IP-адреса. Необходимо также настроить основной и дополнительный DNS-серверы.

	-   При определении нескольких сетевых интерфейсов на виртуальном массиве учтите, что только первый сетевой интерфейс (по умолчанию — **Ethernet**) имеет доступ к облаку. Чтобы управлять типом трафика, на виртуальном массиве (настроенном в качестве сервера iSCSI) необходимо создать несколько виртуальных сетевых интерфейсов и подключить их к различным подсетям.
...
После создания томов StorSimple на сервере iSCSI их необходимо инициализировать, подключить и форматировать. Эта операция выполняется на узле, подключенном к устройству StorSimple. При подключении и форматировании томов на узле StorSimple следует учитывать приведенные ниже советы и рекомендации.
...
При настройке томов iSCSI на виртуальном массиве StorSimple необходимо позаботиться об управлении доступом. Создайте и свяжите записи контроля доступа с томами StorSimple, чтобы определить, какие серверы узлов могут получать доступ к томам.
...
-   Несколько записей контроля доступа нужно определять только в кластерной среде.
...
-   Планируйте архивацию на часы наименьшей нагрузки. Время начала архивации не должно совпадать с периодом интенсивных рабочих нагрузок ввода-вывода узла.
...
Выполнить восстановление из набора архивации можно двумя способами: восстановить в другой том или общую папку или выполнить восстановление на уровне элементов (если виртуальный массив настроен в качестве файлового сервера). Восстановление на уровне элементов позволяет выполнять фрагментарное восстановление файлов и папок из облачной резервной копии всех общих папок на устройстве StorSimple. Дополнительные сведения см. в статье [Восстановление из резервной копии виртуального массива StorSimple](storsimple-ova-restore.md).
...
-   Прежде чем отключать виртуальное устройство, сделайте облачный моментальный снимок всех данных. При отключении виртуального массива все локальные данные устройства будут потеряны. С помощью моментального снимка облака вы сможете восстановить все данные в дальнейшем.
...
-   Не используйте индексатор Windows для многоуровневых томов. Если индекс нужно часто перестраивать, из облака будут перемещены большие объемы данных (ТБ). При перестроении индекса файлы всех типов извлекаются для индексирования их содержимого.
...
Приложения могут блокировать заданный диапазон байтов в файлах. Если блокировка диапазона байтов включена в приложениях, которые осуществляют запись в StorSimple, распределение по уровням на виртуальном массиве работать не будет. Для распределения по уровням все области файлов, к которым осуществляется доступ, должны быть разблокированы. Блокировка диапазона байтов не поддерживается для многоуровневых томов на виртуальном массиве.

Ниже приведены рекомендации, позволяющие обойти эту проблему.
