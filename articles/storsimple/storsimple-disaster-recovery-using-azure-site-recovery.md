...
2. Создайте учетную запись автоматизации. Сохраните ее в том же геообъекте или регионе, в котором создано облачное устройство StorSimple и учетные записи хранения.
...
6. В учетной записи автоматизации перейдите на вкладку **Активы**, последовательно выберите **Учетные данные** &gt; **Добавить учетные данные** и добавьте учетные данные Azure. Присвойте ресурсу имя AzureCredential.
...
7. В учетной записи службы автоматизации выберите вкладку **Активы**, последовательно выберите **Переменные** &gt; **Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления (который вы создадите на следующем шаге) — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
  ...
  Например, если имя плана восстановления — fileServerpredayRP, после добавления всех ресурсов вкладка **Учетные данные** & **Переменные** будет выглядеть следующим образом:
