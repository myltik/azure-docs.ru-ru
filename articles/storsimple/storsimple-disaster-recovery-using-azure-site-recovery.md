...
1. Отключите целевые объекты iSCSI для локальных виртуальных машин, которые необходимо защитить, с помощью Azure Site Recovery:
   ...
   >
   >
2. [Включите защиту виртуальной машины](../site-recovery/site-recovery-hyper-v-site-to-azure.md#step-6-enable-replication) файлового сервера на портале Azure Site Recovery.
...
#### <a name="to-create-the-account"></a>Создание учетной записи
1. Перейдите на классический портал Azure к разделу **Автоматизация** .
2. Создайте учетную запись автоматизации. Сохраните ее в том же геообъекте или регионе, в котором создано устройство StorSimple Cloud Appliance и учетные записи хранения.
3. Последовательно выберите **Создать** &gt; **Службы приложений** &gt; **Автоматизация** &gt; **Runbook** &gt; **Из коллекции**, чтобы импортировать все необходимые модули Runbook в учетную запись автоматизации.

   ![](./media/storsimple-dr-using-asr/image3.png)
4. Добавьте следующие модули Runbook из области **Аварийное восстановление** в коллекции:
...
5. Опубликуйте все сценарии, выбрав модуль Runbook в учетной записи автоматизации и перейдя на вкладку **Автор** . После выполнения этого шага вкладка **Модули Runbook** будет выглядеть следующим образом:

    ![](./media/storsimple-dr-using-asr/image5.png)
6. В учетной записи автоматизации перейдите на вкладку **Ресурсы-контейнеры**, последовательно выберите **Добавить настройку** &gt; **Добавить учетные данные** и добавьте учетные данные Azure. Присвойте ресурсу имя AzureCredential.

   Используйте учетные данные Windows PowerShell, которые должны содержать имя пользователя и пароль идентификатора организации с доступом к этой подписке Azure (Многофакторная идентификация должна быть отключена). Это необходимо для прохождения проверки подлинности от имени пользователя при отработке отказа и для открытия томов файлового сервера на сайте аварийного восстановления.
7. В учетной записи автоматизации выберите вкладку **Ресурсы-контейнеры**, последовательно выберите **Добавить настройку** &gt; **Добавить переменную** и добавьте приведенные ниже переменные. Вы также можете зашифровать эти ресурсы. Переменные зависят от плана восстановления. Если имя плана восстановления (который вы создадите на следующем шаге) — TestPlan, то у переменных будут имена TestPlan-StorSimRegKey, TestPlan-AzureSubscriptionName и т. п.
...
9. Выберите вкладку **Планы восстановления** и создайте план восстановления следующим образом:
   ...
   d.  Щелкните **Завершение работы всех групп**, **Сценарий**, а затем — **Add a primary side script before all Group shutdown** (Добавить первичный сценарий перед завершением работы всех групп).
...
10. Добавьте ручное действие после четырех сценариев, указанных выше, в том же разделе **Группа 1: последующие действия** . Это действие позволяет проверить, все ли работает правильно. Его необходимо добавить только как часть тестовой отработки отказа (поэтому установите только флажок **Тестовая отработка отказа** ).
11. После ручного действия добавьте сценарий очистки, выполнив ту же процедуру, что и для других модулей Runbook. Сохраните план восстановления.
...
6. По завершении проверки щелкните **Validations Complete**(Проверка завершена). Среда тестовой отработки отказа удалится, и операция тестовой отработки отказа будет завершена.

## <a name="perform-an-unplanned-failover"></a>Внеплановая отработка отказа
...
#### <a name="to-perform-an-unplanned-failover"></a>Выполнение внеплановой отработки отказа
1. На классическом портале Azure выберите свое хранилище Site Recovery.
2. Щелкните план восстановления, созданный для виртуальной машины файлового сервера.
3. Щелкните **Отработка отказа**, а затем выберите **Внеплановая отработка отказа**.

   ![](./media/storsimple-dr-using-asr/image9.png)
4. Выберите целевую сеть, а затем щелкните значок флажка ✓, чтобы запустить отработку отказа.
...
1. На классическом портале Azure выберите свое хранилище Site Recovery.
2. Щелкните план восстановления, созданный для виртуальной машины файлового сервера.
3. Щелкните **Отработка отказа**, а затем выберите **Плановая отработка отказа**.
4. Выберите целевую сеть, а затем щелкните значок флажка ✓, чтобы запустить отработку отказа.
...
1. На классическом портале Azure выберите свое хранилище Site Recovery.
2. Щелкните план восстановления, созданный для виртуальной машины файлового сервера.
3. Щелкните **Отработка отказа**, а затем выберите **Плановая отработка отказа** или **Внеплановая отработка отказа**.
4. Щелкните **Изменить направление**.
5. Настройте соответствующие параметры синхронизации данных и создания виртуальной машины.
6. Чтобы запустить восстановление размещения, щелкните значок флажка ✓.
