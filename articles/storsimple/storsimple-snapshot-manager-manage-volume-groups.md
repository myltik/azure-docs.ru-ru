...
>[AZURE.IMPORTANT]Настраивая группы томов, не размещайте в одной группе томов общие тома кластера (CSV) и тома других типов. Диспетчер моментальных снимков StorSimple не поддерживает совместное использование общих томов кластера и томов других типов в одном моментальном снимке.
...
3.  Введите следующие сведения:
    ...
    2. В поле **Приложения** выберите приложения, связанные с томами, которые будут добавлены в группу томов.
        ...
        При выборе приложения будут автоматически выбраны все тома, связанные с ним. Наоборот, если выбрать тома, связанные с определенным приложением, приложение будет автоматически выбрано в поле **Приложения**.
...
3. Откроется диалоговое окно **Создание группы томов**. Вы можете изменить значения полей **Имя**, **Приложения** и **Тома**. 
...
3. Откроется диалоговое окно **Удаление группы томов**. Введите **Confirm** (Подтверждаю) в текстовом поле и нажмите кнопку **ОК**.

    Удаленная группа томов исчезнет из списка на панели **Результаты**, а все резервные копии, связанные с этой группой, будут удалены из каталога резервных копий.
