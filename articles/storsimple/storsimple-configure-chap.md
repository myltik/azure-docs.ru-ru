<properties 
   ...
   description="Описываются способы настройки протокола CHAP на устройстве StorSimple."
   services="storsimple"
   documentationCenter=""
   authors="alkohli"
   manager="carolz"
...
В этом учебнике описывается настройка CHAP для устройства StorSimple.

CHAP — сокращение от Challenge Handshake Authentication Protocol (протокол проверки подлинности на основе вызова). Это схема проверки подлинности, используемая серверами для проверки удаленных клиентов. Проверка выполняется на основе общего пароля или секрета.
...
В каждом из этих случаев необходимо настроить портал управления устройствами и программное обеспечение инициатора на сервере iSCSI. Подробное описание действий для этой конфигурации приведено в следующем учебнике.
...
1. На портале управления на странице **Устройства** щелкните вкладку **Настройка**.
...
4. В диалоговом окне **Дополнительные параметры** выполните следующие действия.
	...
	2. В поле **Имя** введите имя пользователя, указанное для инициатора CHAP на портале управления.

	3. В поле **Секрет целевого узла** введите пароль, указанный для инициатора CHAP на портале управления.

	4. Нажмите кнопку **ОК**.

		![Расширенные параметры: общие](./media/storsimple-configure-chap/IC740946.png)

5. На вкладке **Целевые устройства ** окна **Свойства инициатора iSCSI** состояние устройства должно измениться на **Подключено**.
...
4. В диалоговом окне **Секрет взаимного CHAP инициатора iSCSI**
	...
	1. Введите **Пароль обратного CHAP**, указанный при настройке портала управления.
...
8. В диалоговом окне **Дополнительные параметры** выполните следующие действия.
	...
	5. Щелкните **ОК**, чтобы завершить настройку CHAP.
...
4. В диалоговом окне **Сведения об избранном целевом объекте** запомните содержимое поля **Аутентификация**. Если настройки верны, в нем должно быть указано **CHAP**.
