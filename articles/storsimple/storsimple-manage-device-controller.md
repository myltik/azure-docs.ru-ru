<properties 
   pageTitle="Управление контроллерами устройств StorSimple | Microsoft Azure"
   description="Узнайте, как остановить, перезапустить, выключить контроллеры устройства StorSimple или восстановить их заводские настройки."
   services="storsimple"
   documentationCenter=""
   authors="alkohli"
   manager="carolz"
...
Рекомендуется, чтобы пользователь управлял контроллерами устройств через службу диспетчера StorSimple. Если действие можно выполнить только с помощью Windows PowerShell для StorSimple, в учебном курсе будет соответствующая пометка.
...
1. В нижней части страницы **Обслуживание** щелкните **Управление контроллерами**.
	...
	> [AZURE.NOTE]Если вы не видите кнопку «Управление контроллерами», необходимо установить обновления. Дополнительные сведения см. в разделе [Обновление устройства](storsimple-update-device.md).
...
1. При появлении запроса введите следующую команду:
	...
	> - Дополнительные сведения об использовании этого командлета см. в [справочнике по командлетам Windows PowerShell для StorSimple](https://technet.microsoft.com/ru-ru/library/dn688168.aspx).
...
- **Завершить работу активного контроллера** — вы получите уведомление о том, что завершение работы активного контроллера вызовет простой. Также необходимо будет нажать кнопку питания на устройстве, чтобы включить контроллер.
...
**Вопрос.** Как вернуть контроллер в службу после завершения работы и удаления из службы?

**Ответ.** Чтобы вернуть контроллер в службу, нужно вставить его в корпус согласно инструкциям [руководства по замене компонента оборудования](https://msdn.microsoft.com/library/azure/dn757736.aspx).

## Дальнейшие действия

Если возникли затруднения с контроллерами устройства StorSimple, которые не удается разрешить с помощью процедур, описанных в этом учебнике, [обратитесь в службу поддержки Microsoft](https://msdn.microsoft.com/library/azure/dn757750.aspx).
