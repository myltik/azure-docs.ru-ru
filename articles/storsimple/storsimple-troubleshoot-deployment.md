...
- Проверьте, соблюдены ли предварительные требования, связанные с развертыванием. Убедитесь, что у вас есть вся информация, перечисленная в [контрольном списке развертывания](storsimple-deployment-walkthrough.md#pre-installation-checklist).
...
- Get-HcsRoutingTable: этот командлет отображает локальную таблицу IP-маршрутизации.
...
**Образец вывода – успешно зарегистрированное устройство под управлением выпуска StorSimple Update 1 (май 2015 г.)**
...
Этот командлет выполняет трассировку маршрута. Он отправляет пакеты на все маршрутизаторы, расположенные на пути к конечной точке, а затем вычисляет результат, используя полученные из каждой точки перехода пакеты. Поскольку этот конандлет не исключает потерю некоторого количества пакетов для каждого маршрутизатора или линии связи, можно установить, какие именно маршрутизаторы или линии связи могут вызывать проблемы с сетью.
...
Если при запуске командлета Get-HcsRoutingTable не задать параметры (как показано в следующем примере), командлет будет выводить таблицы маршрутизации IPv4 и IPv6. Также для получения соответствующей таблицы маршрутизации можно указать `Get-HcsRoutingTable -IPv4` или `Get-HcsRoutingTable -IPv6`.
