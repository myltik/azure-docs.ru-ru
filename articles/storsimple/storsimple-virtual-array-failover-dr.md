---
title: "Аварийное восстановление и отработка отказа для устройств в виртуальном массиве Microsoft Azure StorSimple | Документация Майкрософт"
...
* На портале Azure для целевого устройства должно отображаться состояние **Готово к настройке**. Подготовьте целевой виртуальный массив такой же или большей емкости. Используйте локальный веб-интерфейс для настройки и регистрации виртуального массива.
  ...
  > 
  > 
* Целевое и исходное устройства должны иметь разные имена. Целевое устройство можно будет переименовать позже, после завершения отработки отказа.
...
14. Теперь можно переименовать устройство (присвоить ему то же имя, которое было у старого устройства), чтобы серверы приложений подключались к нему напрямую. Если вы не хотите переименовывать устройство, потребуется [создать DNS-псевдоним](https://support.microsoft.com/kb/168322), чтобы все приложения при попытке подключения перенаправлялись на новое устройство.
