...
<!-- Legacy MSDN URL = https://msdn.microsoft.com/ru-RU/library/azure/hh690951.aspx -->
...
## Включение удаленного доступа перед развертыванием в Azure ##

>[AZURE.NOTE]Чтобы включить удаленный доступ перед развертыванием приложения в Azure, необходимо использовать Eclipse в Windows.
...
## Включение удаленного доступа после развертывания в Azure ##
...
1. В списке **Облачные службы** выберите развернутую облачную службу.
1. На веб-странице облачной службы щелкните ссылку **Настроить**.
1. В нижней части страницы конфигурации щелкните ссылку **Удаленный**.
1. При отображении всплывающего диалогового окна:
    1. Укажите роль, для которой требуется разрешить удаленный доступ.
    1. Установите флажок **Включить удаленный рабочий стол**.
    1. Укажите имя пользователя и пароль, которые вы хотите использовать для удаленного доступа.
    1. Выберите используемый сертификат.
1. Нажмите кнопку **ОК**. 
...
## Включение удаленного доступа в пакете ##

* В области обозревателя проектов Eclipse щелкните правой кнопкой мыши свой проект Azure и выберите пункт **Свойства**.
* В диалоговом окне **Свойства** разверните узел **Azure** на левой панели и щелкните **Удаленный доступ**.
* В диалоговом окне **Удаленный доступ** убедитесь, что установлен флажок **Enable all roles to accept Remote Desktop Connections with these login credentials** (Разрешить всем ролям принимать подключения удаленного рабочего стола с этими учетными данными входа).
* Укажите имя пользователя для подключения удаленного рабочего стола.
* Укажите и подтвердите пароль для пользователя. Значения имени пользователя и пароля в этом диалоговом окне будут использоваться при установке подключения удаленного рабочего стола. (Обратите внимание, что этот пароль отличается от пароля PFX.)
* Укажите дату истечения срока действия для учетной записи пользователя.
* Щелкните элемент **Создать** для создания нового самозаверяющего сертификата. (Кроме того, вы можете выбрать сертификат из рабочей области или файловой системы с помощью кнопки **Рабочая область** или **Файловая система** соответственно, но в данном руководстве мы создадим новый сертификат.)
...
* Укажите путь и имя файла для сохранения нового сертификата в виде CER-файла. На этом и следующем шаге вы можете использовать папку **cert** проекта Azure, однако вы можете выбрать и другое расположение. В рамках данного руководства мы будем использовать **c:\\mycert\\mycert.cer**. (Создайте папку **c:\\mycert** перед продолжением или используйте существующую папку.)
* Укажите путь и имя файла для сохранения нового сертификата и его закрытого ключа в виде PFX-файла. В рамках данного руководства мы будем использовать **c:\\mycert\\mycert.pfx**. Диалоговое окно **Новый сертификат** должно выглядеть следующим образом (обновите пути к папкам, если вы не использовали **c:\\mycert**): ![][ic712275]
* Нажмите кнопку **ОК**, чтобы закрыть диалоговое окно **Новый сертификат**.
* Ваше диалоговое окно **Удаленный доступ** должно выглядеть следующим образом:</p> ![][ic719495]
...
* Другой способ удаленного вход заключается в использовании <a href="http://go.microsoft.com/fwlink/?LinkID=512959">портала управления Azure</a>:
    * В представлении **Облачные службы** на портале управления Azure выберите облачную службу, щелкните элемент **Экземпляры**, выберите определенный экземпляр и нажмите кнопку **Подключить**. Кнопка **Подключить** в панели команд выглядит следующим образом: ![][ic659273]
    >[AZURE.NOTE]Если вы работаете с операционной системой, отличной от Windows, необходимо использовать клиент удаленного рабочего стола, совместимый с вашей операционной системой, настроив его с помощью параметров из скачанного RDP-файла в соответствии с приведенными ниже инструкциями.
