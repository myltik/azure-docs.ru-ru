...
Во время работы с учебником, будет подготовлено две службы: Поиск Azure и Azure Active Directory (AD). Кроме того вы создадите AD приложение, которое установит доверительное отношение между клиентским приложением и конечной точкой диспетчера ресурсов в Azure.

Для завершения заданий в учебнике потребуется учетная запись Azure.


<h2 id="download">Загрузка примера приложения</h2>
...
Клиентское приложение можно найти на сайте Codeplex в разделе [Azure Search Management API Demo](https://azuresearchmgmtapi.codeplex.com/).


<h2 id="config">Настройка приложения</h2>

Перед запуском примера приложения, необходимо включить проверку подлинности, чтобы запросы, отправленные из клиентского приложения к конечной точке диспетчера ресурсов, могли быть приняты. Требование проведения проверки подлинности исходит от [диспетчера ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790568.aspx), который служит основой для всех операций на портале, запрашиваемых через API-интерфейс, в том числе относящиеся к управлению службой поиска. API управления службами для поиска Azure это просто расширение диспетчера ресурсов Azure и таким образом наследует все его зависимости.  

Диспетчеру ресурсов Azure требуется служба Azure Active Directory в качестве поставщика удостоверений. 

Чтобы получить маркер доступа, который позволит запросам дойти до диспетчера ресурсов, клиентское приложение имеет сегмент кода, который вызывает Active Directory. Этот сегмент кода, а также все необходимое для его понимания и написания, был позаимствован из статьи: [Запросы проверки подлинности для диспетчера ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790557.aspx).
...
1. Создание приложения AD
1. Настройка приложения AD путем регистрации сведений о примере загруженного клиентского приложения 
1. Загрузить в пример клиентского приложения значения, которые будут использоваться для авторизации его запросов

> [WACOM.NOTE]Это ссылки на материалы с описанием использования Azure Active Directory для проверки подлинности клиентских запросов к диспетчеру ресурсов: [Диспетчер ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790568.aspx), [Проверка подлинности запросов к диспетчеру ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790557.aspx) и [Azure Active Directory](http://msdn.microsoft.com/library/azure/jj673460.aspx).

<h3>Создание службы Active Directory</h3>
...
4. Нажмите кнопку **СОЗДАТЬ**, чтобы открыть **Службы приложений** | **Active Directory**. На этом шаге вы создаете новую службу Active Directory. В этой службе будет размещаться приложение AD, которое мы определим через несколько шагов. Создание новой службы помогает изолировать учебное приложение от других приложений, которые могли быть размещены в Azure ранее.

5. Щелкните **Каталог** | **Настраиваемое создание**.

6. Введите имя службы, домена и географическое расположение. Имя домена должно быть уникальным. Чтобы создать службу, установите флажок.

     ![][5]

<h3>Создание нового приложения AD для этой службы</h3>
...
6. Выберите тип приложения **собственное клиентское приложение**. Это верно для примера приложения; это клиентское приложение Windows (консольное), а не веб-приложение.
...
7. Установите флажок, чтобы создать приложение Active Directory. Вы увидите Azure-Search-Manager-App в левой области навигации.

<h3>Настройка приложения AD</h3>
 
9. Щелкните приложение AD Azure-Search-Manager-App, созданное вами. Оно должно быть приведено в левой области навигации.

10. В верхнем меню выберите пункт **Настроить**.
 
11. Прокрутите вниз до разрешений и выберите **API управления Windows Azure**. На этом шаге укажите API (в данном случае API диспетчера ресурсов Azure), к которому требуется доступ клиентского приложения, а также уровень нужного доступа.

12. В разделе "Делегированные разрешения" щелкните раскрывающийся список и выберите **Доступ к управлению службами Azure (Предварительная версия**).
...
<h3>Загрузите программу примера приложения со значениями регистрации и подписки</h3>
...
Если вы еще не [загрузили пример приложения с сайта Codeplex] (linkTBD), то следует это сделать перед выполнением данного шага учебника.
...
5. Укажите TenantID. 
	...
	- В нижней части страницы щелкните **Просмотреть конечные точки**. 
...
6. Укажите SubscriptionID.
	...
	- Щелкните **Параметры** в нижней части области навигации.
...
<h3>Устранение ошибок построения</h3>

Проблемы со ссылками на сборки могут прервать процесс сборки решения. В этом разделе описаны способы устранения некоторых из этих проблем.

- Должна быть установлена сборка Microsoft.IdentityModel.Clients.ActiveDirectory (см. в [Проверка подлинности запросов к диспетчеру ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790557.aspx) подробные сведения о том, как установить [библиотеку проверки подлинности Active Directory](https://www.nuget.org/packages/Microsoft.IdentityModel.Clients.ActiveDirectory/)).
- Повторная установка сборки также может разрешить проблему. Перейдите в **Сервис** | **Диспетчер пакетов NuGet** | **Консоль диспетчера пакетов**. В командной строке PM> введите *Update-package -reinstall Microsoft.IdentityModel.Clients.ActiveDirectory*.
- В этом решении также используется Newtonsoft.Json. Если возникает ошибка по этой сборке, удалите ее из проекта, затем добавьте снова. Щелкните правой кнопкой мыши **Ссылки** | **Управление пакетами NuGet** | **Установленные пакеты**. Выберите **Json.Net** и удалите ее. Разверните **В сети**, выберите **Json.Net** | **Установить**..


<h2 id="explore">Обзор приложения</h2>

Добавьте точку останова в месте вызова первого метода, чтобы провести пошаговое выполнение программы. Нажмите клавишу **F5** для запуска приложения и затем нажмите клавишу **F11**, чтобы пройти по коду.

Пример приложения создает бесплатную службу поиска Azure для существующей подписки Azure. Если бесплатная служба в подписке уже есть, то пример приложения работать не будет. Допускается наличие только одной бесплатной службы на подписку.

1. Откройте файл Program.cs в обозревателе решений и перейти к функции Main (строка []void). 
 
3. Обратите внимание, что метод **ExecuteArmRequest** используется для выполнения запросов к конечной точке диспетчера ресурсов Azure https://management.azure.com/subscriptions для указанного subscriptionID. Этот метод используется в программе для выполнения операций с использованием API-интерфейса диспетчера ресурсов Azure или API-Интерфейс управления поиском.

3. Запросы к диспетчеру ресурсов Azure должны проходить проверку подлинности и авторизацию. Это выполняется с помощью метода **GetAuthorizationHeader**, который вызывается методом **ExecuteArmRequest**, как показано в статье [Проверка подлинности запросов к диспетчеру ресурсов Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn790557.aspx). Обратите внимание, что **GetAuthorizationHeader** вызывает https://management.core.windows.net для получения токена доступа.

4. Будет предложено выполнить вход с именем пользователя и паролем, допустимым для вашей подписки.

5. Затем новая служба поиска Azure будет зарегистрирована в поставщике диспетчера ресурсов Azure. Опять же это выполняется с помощью метода **ExecuteArmRequest**, который создает службу поиска в Azure для вашей подписки обращением к адресу providers/Microsoft.Search/register. 

6. Остальная часть программа использует [Azure Search Management REST API](http://msdn.microsoft.com/ru-ru/library/dn832684.aspx). Обратите внимание, что api-version данного API-интерфейса отличается от api-version диспетчера ресурсов Azure. Например, команда "/listAdminKeys?api-version=2014-07-31-Preview" показывает api-version для Azure Search Management REST API.
...
- Дополнительные сведения об интеграции клиентского приложения с Active Directory. См. [Интеграция приложений в Azure Active Directory](http://msdn.microsoft.com/ru-ru/library/azure/dn151122.aspx).
- Дополнительные сведения о других операциях управления службами в Azure. См. [Управление службами](http://msdn.microsoft.com/ru-ru/library/azure/dn578292.aspx).

<!--Anchors-->
[Загрузка примера приложения]: #Download
[Настройка приложения]: #config
[Обзор приложения]: #explore
...
[Создание приложения для геопространственного поиска с использованием службы "Поиск Azure"]: ../search-create-geospatial/
