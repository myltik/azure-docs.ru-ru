...
Ниже приведен снимок экрана с текущего портала Azure для BLOB-объекта, зашифрованного с помощью шифрования на стороне клиента с ключом, хранящимся в хранилище ключей. Свойство **KeyId** представляет собой универсальный код ресурса (URI) ключа в хранилище ключей, которое выступает в качестве ключа шифрования ключа (KEK). Свойство **EncryptedKey** содержит зашифрованную версию ключа шифрования содержимого (CEK).
