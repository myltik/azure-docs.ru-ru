...
# Создание гибридной коллекции для RemoteApp
...
Этот учебник представляет из себя пошаговое руководство по гибридному развертыванию. Для этого необходимо выполнить семь шагов:

1.	Создание [для RemoteApp пользовательского образа](remoteapp-imageoptions.md) или выбор одного из образов, включенных в подписку.
...
- Собрать сведения о локальной сети, а именно получить информацию об IP-адресе и VPN-устройстве.
- установить модуль [Azure PowerShell](install-configure-powershell.md).
- Соберите сведения о пользователях, которым нужно предоставить доступ. Это могут быть либо данные учетной записи Майкрософт, либо данные рабочей учетной записи Active Directory.
- Создание образа шаблона Образ шаблона RemoteApp содержит приложения программы, которыми вы хотите поделиться с пользователями. Дополнительные сведения приведены в [статье о создании образа RemoteApp](remoteapp-imageoptions.md). 
