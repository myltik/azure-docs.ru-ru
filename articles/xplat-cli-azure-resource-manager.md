<properties linkid="script-xplat-intro" urlDisplayName="Межплатформенный интерфейс командной строки Microsoft Azure" pageTitle="Использование межплатформенного интерфейса командной строки Microsoft Azure совместно с диспетчером ресурсов" title="Использование межплатформенного интерфейса командной строки Microsoft Azure совместно с диспетчером ресурсов" metaKeywords="диспетчер ресурсов межплатформенного интерфейса командной строки windows azure, диспетчер ресурсов командной строки windows azure, диспетчер ресурсов командной строки azure, диспетчер ресурсов интерфейса командной строки azure" description="Использование межплатформенного интерфейса командной строки Microsoft Azure совместно с диспетчером ресурсов" metaCanonical="http://www.windowsazure.com/ru-ru/script/xplat-cli-intro" umbracoNaviHide="0" disqusComments="1" editor="mollybos" manager="paulettm" documentationCenter="" solutions="" authors="larryfr" services="" />
...
Когда шаблон используется для изменения или создания группы, создается _развертывание_, которое затем применяется к этой группе.
...
1. После сохранения файла **params.json** используйте следующую команду, чтобы создать новую группу ресурсов на основе шаблона. Параметр `-e` указывает файл **params.json**, созданный на предыдущем шаге.
...
3. Для просмотра группы используйте следующую команду.
	...
	Эта команда возвращает сведения о ресурсах в группе. Если имеется несколько групп, можно использовать команду `azure group list`, чтобы получить список имен групп, а затем использовать `azure group show` для просмотра сведений о конкретной группе.
