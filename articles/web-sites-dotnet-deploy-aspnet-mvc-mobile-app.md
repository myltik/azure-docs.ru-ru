...
Этот учебник поможет вам освоить основы создания адаптированного для мобильных устройств веб-приложения ASP.NET MVC 5, а также его развертывания в службе приложений Azure. Для этого учебника вам требуется 
...
-	Как использовать Visual Studio 2013 для публикации своего веб-приложения напрямую в веб-службу Azure.
...
-   Эмулятор браузера в [инструментах разработчика F12 Internet Explorer 11 ][EmulatorIE11] (используется в снимках экрана всех браузеров мобильных устройств). Он содержит предустановки строк агента пользователя для Windows Phone 8, Windows Phone 7 и Apple iPad.
...
2. 	Затем в проводнике щелкните правой кнопкой мыши файл Mvc5Mobile.zip и выберите *Properties*.

3. 	В диалоговом окне **Свойства Mvc5Mobile.zip** нажмите кнопку **Разблокировать**. (Разблокировка устраняет предупреждение системы безопасности, выводимое при попытке использовать ZIP-файл, загруженный из Интернета).
...
7.	В окне веб-публикации щелкните **веб-приложения Microsoft Azure**.
...
9.	Введите имя учетной записи Azure и нажмите кнопку **Продолжить**.
...
11. Когда вы войдете, отобразится диалоговое окно "Выбор существующего веб-приложения". Щелкните **Создать**.

	<!--todo: ![][DeployNewWebsite] -->  

12. В поле **Имя сайта** задайте уникальный префикс имени сайта. Полностью определенным именем вашего сайта будет *&lt;префикс>*.azurewebsites.net. Также выберите регион в поле **Регион**. Затем нажмите **Создать**.

	<!--todo: ![][DeploySiteSettings] -->

13.	Диалоговое окно веб-публикации будет заполнено параметрами нового веб-приложения. Щелкните **Опубликовать**.
...
14.	Запустите эмулятор браузера мобильного устройства, скопируйте в эмулятор URL-адрес приложения конференции (*<префикс>*.azurewebsites.net), затем нажмите кнопку справа вверху и выберите **Поиск по тегу**. Если вы используете Internet
Explorer 11 в качестве браузера по умолчанию, просто введите `F12`
`Ctrl+8` и измените профиль браузера на **Windows Phone**. На следующем рисунке показано представление *AllTags* в портретной ориентации (которое открывается, если выбрать
**Поиск по тегу**).

	![][AllTags]

>[AZURE.NOTE] Выполняя отладку своего приложения MVC 5 с помощью Visual Studio, вы можете повторно опубликовать веб-приложение в Azure, чтобы проверить действующее приложение непосредственно из браузера мобильного устройства или эмулятора браузера.

Это представление очень удобно для чтения на мобильном устройстве. Кроме того, вы уже можете просмотреть некоторые визуальные эффекты, применяемые платформой Bootstrap CSS.
...
Обратите внимание, что сворачиваемая панель навигации теперь стала обычным неупорядоченным списком. Еще раз нажмите **Поиск по тегу**, затем щелкните **ASP.NET**.
...
Представление *Index*, скопируйте в  *Views\\Home\\Index.cshtml*
*Views\\Home\\Index.Mobile.cshtml*.

В этом разделе для мобильных устройств будет создан специальный файл макета.

Чтобы начать, скопируйте *Views\\Shared\\\_Layout.cshtml* в
Views\\Shared\\\_Layout.Mobile.cshtml. Index *\_Layout.Mobile.cshtml* и измените заголовок с **Приложение MVC5** на **Приложение MVC5 (мобильное)**.

В каждом вызове `Html.ActionLink` панели навигации удалите "Поиск по" в каждой ссылке
*ActionLink*. В следующем примере кода показан завершенный тег `<ul class="nav navbar-nav">` файла макета для мобильных устройств.
...
Перейдите на страницу тегов, используя классический браузер и эмулятор браузера для мобильных устройств. Эмулятор браузера для мобильных устройств показывает два внесенных вами изменения (заголовки из *\_Layout.Mobile.cshtml* и
...
iPhone), ASP.NET MVC будет искать все представления, в имени которых есть суффикс
iPhone.
...
В коде щелкните правой кнопкой мыши `DefaultDisplayMode`, выберите **Разрешить**, а затем выберите `using System.Web.WebPages;`. This adds a reference to the
...
Файл *Views\\Shared\\\_Layout.Mobile.cshtml* в
*Views\\Shared\\\_Layout.iPhone.cshtml*. Откройте новый файл и измените заголовок с `MVC5 Application (Mobile)` на
`Приложение MVC5 (iPhone)`.

Скопируйте файл *Views\\Home\\AllTags.Mobile.cshtml* в
*Views\\Home\\AllTags.iPhone.cshtml*. В новом файле измените элемент `<h2>` "Tags (M)" на "Tags (iPhone)".
...
В браузере мобильного устройства выберите ссылку **Speakers**. Так как представление для мобильных устройств (AllSpeakers.Mobile.cshtml) отсутствует, докладчики по умолчанию (AllSpeakers.cshtml) отображаются, используя представление с макетом для мобильных устройств (_Layout.Mobile.cshtml). Как показано ниже, заголовок **Приложение MVC5 Application (мобильное)** определяется в *\_Layout.Mobile.cshtml*.
...
Если значение `RequireConsistentDisplayMode` равно `true`, макет для мобильных устройств (*\_Layout.Mobile.cshtml*) используется только для мобильных представлений (т. е. для файла представления используется форма ***ViewName**.Mobile.cshtml*). Возможно, вы захотите установить для `RequireConsistentDisplayMode` значение `true`, если макет для мобильных устройств не работает с
представлениями, не предназначенными для мобильных устройств. На следующем снимке экрана показано, как страница  *Speakers* отображается при значении  `RequireConsistentDisplayMode`, равном  `true` (без строки "(мобильное)" на панели навигации вверху страницы).

![][AllSpeakers_LayoutMobileOverridden]

Согласованный режим отображения можно отключить, задав для параметра
`RequireConsistentDisplayMode` значение `false` в файле представления. Следующая разметка в файле *Views\\Home\\AllSpeakers.cshtml* устанавливает для
...
*Views\\Home\\AllSpeakers.cshtml* добавьте тег \<form\> сразу после тега \<h2\>, как показано ниже:
...
Также включите filter.js в свои зарегистрированные наборы. Откройте
*App\_Start\\BundleConfig.cs* и измените первые наборы Измените первый оператор `bundles.Add` (для набора **jquery**), чтобы включить
...
Как и представление  *Speakers*, представление *Tags* удобно для чтения, но ссылки малы, и касаться их на мобильном устройстве трудно. Можно исправить представление *Tags* так же, как это было сделано с представлением  *Speakers*. Для этого потребуются изменения кода, описанные выше, и следующий синтаксис метода `Html.ActionLink` в *Views\\Home\\AllTags.cshtml*:
...
Можно улучшить представление *Dates* так же, как и представления *Speakers* и
*Tags*. Для этого потребуются изменения кода, описанные выше, и следующий синтаксис `Html.ActionLink` в *Views\\Home\\AllDates.cshtml*:
...
Этот код создает отдельный тег `<div class="panel panel-primary>` для каждой отличной даты в списке и, как и ранее, использует [настраиваемую группу связанного списка][] для соответствующих ссылок. Вот как выглядит браузер мобильного устройства при выполнении этого кода:
...
Как вы видите, отображение имеет формат таблицы, которая сейчас предназначена для просмотра в классическом браузере. При этом чтение в браузере мобильного устройства немного затруднено. Чтобы исправить эту проблему откройте
...
-   использует [настраиваемую группу связанного списка][] Bootstrap
    для форматирования данных сеанса по вертикали, чтобы все эти данные можно было читать в браузере мобильного устройства (используя классы, такие как list-group-item-text);
...
Можно перейти к нужному сеансу нажатием на ссылку в заголовке. На следующем рисунке показаны изменения кода.
