...
11. После входа отобразится диалоговое окно "Выбор существующего веб-приложения". Нажмите кнопку **Создать**.

	![][DeployNewWebsite]

12. В поле **Имя веб-приложения** задайте уникальный префикс имени приложения. Полностью определенным именем вашего веб-приложения будет *&lt;префикс>*.azurewebsites.net. Также укажите значения в полях **План службы приложений**, **Группа ресурсов** и **Регион**. Затем щелкните **Создать**.
...
Чтобы увидеть, как бы выглядело веб-приложение без поддержки начальной загрузки, откройте *App_Start\\BundleConfig.cs* и закомментируйте строки, содержащие *bootstrap.js* и *bootstrap.css*. Следующий код показывает последние два оператора метода `RegisterBundles` после изменения:
...
Также включите filter.js в свои зарегистрированные наборы. Откройте *App_Start\\BundleConfig.cs* и измените первые наборы. Измените первую инструкцию `bundles.Add` (для набора **jquery**), чтобы включить *Scripts\\filter.js* следующим образом.
