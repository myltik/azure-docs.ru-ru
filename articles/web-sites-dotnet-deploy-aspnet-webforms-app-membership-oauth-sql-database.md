...
> [WACOM.NOTE]
> Версию MVC этого учебника см. в [Развертывание безопасного приложения ASP.NET MVC 5 с членством, OAuth и базой данных SQL на веб-сайте Azure][Развертывание безопасного приложения ASP.NET MVC 5 с членством, OAuth и базой данных SQL на веб-сайте Azure].
...
![Страница Контакты — изменить][Страница Контакты — изменить]

> [WACOM.NOTE]
> Для работы с этим учебником требуется учетная запись Azure. Если у вас нет учетной записи, можно [активировать преимущества для подписчиков MSDN][активировать преимущества для подписчиков MSDN] или [подписаться на бесплатную пробную версию][подписаться на бесплатную пробную версию].

В этом учебнике содержатся следующие разделы:

-   [Настройка среды разработки][Настройка среды разработки]
-   [Настройка среды Azure][Настройка среды Azure]
-   [Создание приложения веб-форм ASP.NET][Создание приложения веб-форм ASP.NET]
-   [Добавление базы данных в приложение][Добавление базы данных в приложение]
-   [Включение SSL для проекта][Включение SSL для проекта]
-   [Добавление поставщика OAuth 2.0][Добавление поставщика OAuth 2.0]
-   [Использование API членства для ограничения доступа][Использование API членства для ограничения доступа]
-   [Развертывание приложения с базой данных в Azure][Развертывание приложения с базой данных в Azure]
...
1.  Установите [Visual Studio 2013][Visual Studio 2013], если вы еще этого не сделали.
2.  Установите [Azure SDK для Visual Studio 2013][Azure SDK для Visual Studio 2013]. Для этого учебника требуется Visual Studio 2013 перед установкой Azure SDK для Visual Studio 2013.
...
4.  В окне **Установщика веб-платформы** щелкните **Установить** и продолжите установку.
    ![Установщик веб-платформы][Установщик веб-платформы]
...
5.  Если не установлено **Visual Studio с обновлением 2**, загрузите и установите **[Visual Studio 2013 с обновлением 2][Visual Studio 2013 с обновлением 2]** или выше.
...
2.  Щелкните **Веб-сайты**, а затем щелкните **Настраиваемое создание** .
    ![Настраиваемое создание ][Настраиваемое создание ]
...
3.  На шаге **Создать веб-сайт** в мастере введите в поле **URL-адрес** строку, которая будет использоваться как уникальный URL-адрес для приложения. Полный URL-адрес будет состоять из введенной здесь строки и суффикса, который отображается рядом с текстовым полем. На иллюстрации показан URL-адрес, который вероятно уже используется. Поэтому **необходимо выбрать другой URL-адрес** .
    ![Контакты — Создать новый веб-сайт][Контакты — Создать новый веб-сайт]
...
1.  Выберите **Новый проект** в меню **Файл** в Visual Studio.
    ![Меню Файл — Новый проект][Меню Файл — Новый проект]
...
2.  На вкладке **Профиль** диалогового окна **Публикация веб-сайта** щелкните **Веб-сайты Azure**.
    ![ Диалоговое окно Публикация веб-сайтов][ Диалоговое окно Публикация веб-сайтов]
3.  Если вы еще не выполнили вход, нажмите кнопку **Вход** в диалоговом окне **Выбрать существующие веб-сайты**. После выполнения входа выберите веб-сайт, который был создан в первой части учебника. Нажмите **OK**, чтобы продолжить.
    ![Диалоговое окно Выбрать существующий веб-сайт][Диалоговое окно Выбрать существующий веб-сайт]
     Visual Studio загрузит ваши параметры публикации.
4.  В диалоговом окне **Опубликовать веб-сайт** нажмите **Опубликовать**.
    ![Диалоговое окно Опубликовать веб-сайт][Диалоговое окно Опубликовать веб-сайт]
     Вы сможете просмотреть общее состояние публикации и окне **Действие веб-публикации** в Visual Studio:
    ![ Действие веб-публикации][ Действие веб-публикации]
...
Созданное вами приложение теперь выполняется в облаке. При следующем развертывании приложения из Visual Studio будут развернуты только измененные (или новые) файлы.
 ![Приложение в браузере][Приложение в браузере]
...
1.  В **Обозревателе решений** щелкните правой кнопкой мыши папку *Models* и выберите **Добавить** -\> **Класс**.
    ![ Выбрать класс][ Выбрать класс]
     Отображается диалоговое окно **Добавить новый элемент**.

2.  Присвойте этому новому классу имя *Contacts.cs*.
    ![ Диалоговое окно Добавить новый элемент][ Диалоговое окно Добавить новый элемент]
...
4.  Если компонент формирования шаблонов веб-форм еще не установлен, выберите **Загрузить**, чтобы загрузить и установить его. При необходимости перезапустите Visual Studio. Не забудьте сохранить изменения в проекте при получении запроса.
    ![ Диалоговое окно Расширения и обновления][ Диалоговое окно Расширения и обновления]
...
7.  Выберите **Веб-формы** из левой панели и выберите **Страницы веб-форм с использованием Entity Framework** из центральной панели. Далее щелкните **Добавить**.
    ![ Диалоговое окно Добавить формирование шаблонов][ Диалоговое окно Добавить формирование шаблонов]
...
8.  В диалоговом окне **Добавление страниц веб-форм** установите для поля **Класс модели** значение `Contact (ContactManager.Models)`. В поле **Класс контекста данных** выберите `ApplicationDbContext (ContactManager.Models)`. Далее щелкните **Добавить** .
    ![ Диалоговое окно Добавить страницы веб-форм][ Диалоговое окно Добавить страницы веб-форм]
...
6.  Замените метод `Seed` следующим кодом:
    ...
    Этот код инициализирует базу данных с контактной информацией. Дополнительные сведения об инициализации базы данных см. в разделе [Заполнение и отладка баз данных Entity Framework (EF)][Заполнение и отладка баз данных Entity Framework (EF)].
...
3.  Выберите ссылку **Contact Demo** в верхней части страницы, чтобы отобразить страницу *Contact List*.
    ![ Страница Список контактов][ Страница Список контактов]
...
6.  Измените **Url проекта** на **URL-адрес SSL**, который вы сохранили ранее.
    ...
    ![Веб-свойства проекта][Веб-свойства проекта]
...
9.  Нажмите **Yes** для подтверждения доверия сертификату IIS Express SSL и продолжения.
    ![Сведения о сертификате IIS Express SSL][Сведения о сертификате IIS Express SSL]
     Отображается предупреждение системы безопасности.

10. Нажмите **Yes**, чтобы установить сертификат в localhost.
    ![ Диалоговое окно Предупреждение системы безопасности][ Диалоговое окно Предупреждение системы безопасности]
...
3.  Перейдите к [Консоли разработчиков Google][Консоли разработчиков Google]. Вам также потребуется выполнить вход со своей учетной записью электронной почты разработчика Google (gmail.com). Если у вас нет учетной записи Google, перейдите по ссылке **Создать учетную запись**.
     Далее вы увидите **Консоль разработчиков Google**.
    ![ Консоль разработчиков Google][ Консоль разработчиков Google]
...
4.  Нажмите кнопку **Создать проект** и введите имя и идентификатор проекта (можно использовать значения по умолчанию). Далее установите флажок **agreement checkbox** и нажмите кнопку **Создать** .
    ![ Google — Новый проект][ Google — Новый проект]
...
6.  Щелкните **Создать идентификатор нового клиента** в разделе **OAuth**.
    ...
    ![Google — Создать идентификатор клиента][Google — Создать идентификатор клиента]
...
16. Нажмите кнопку **Принимаю**. Теперь произойдет перенаправление снова на страницу **Регистрация** приложения **ContactManager**, на которой можно зарегистрировать учетную запись Google.
    ![Регистрация с учетной записью Google][Регистрация с учетной записью Google]
...
5.  После сохранения всех изменений в **Консоли диспетчера пакетов** выполните следующие команды:
    ...
    Этот код создает новую роль с именем `canEdit` и создает нового пользователя с адресом электронной почты <canEditUser@wideworldimporters.com>. Далее код добавляет <canEditUser@wideworldimporters.com> в роль `canEdit`. Дополнительные сведения см. на [странице ресурсов ASP.NET Identity][странице ресурсов ASP.NET Identity].
...
2.  Щелкните правой кнопкой мыши проект в **Обозревателе решений** и выберите **Опубликовать**.
    ...
    ![ Параметр меню Публикация][ Параметр меню Публикация]
     Отображается диалоговое окно **Публикация веб-сайта**.

    ![ Диалоговое окно Публикация веб-сайта][ Диалоговое окно Публикация веб-сайта]
...
5.  Выберите существующий веб-сайт, созданный ранее в этом учебнике, из раскрывающегося списка **Существующие веб-сайты** и нажмите кнопку **OK**.
    ...
    ![ Диалоговое окно Выбор существующего веб-сайта][ Диалоговое окно Выбор существующего веб-сайта]
...
1.  В браузере перейдите по ссылке **Contact Demo**.
    ...
    ![Список контактов в браузере][Список контактов в браузере]
...
4.  Введите новые данные для каждого поля и нажмите кнопку **Вставить**.
    ![ Страница добавления нового контакта][ Страница добавления нового контакта]
...
3.  Щелкните правой кнопкой мыши каждый экземпляр веб-сайта и выберите **Остановить веб-сайт**.
    ![ Элемент меню Остановить веб-сайт][ Элемент меню Остановить веб-сайт]
...
2.  Щелкните **ContactDB** правой кнопкой мыши и выберите **Открыть в обозревателе объектов SQL Server**.
    ![ Элемент меню Открыть в обозревателе объектов SQL Server][ Элемент меню Открыть в обозревателе объектов SQL Server]
...
6.  Щелкните правой кнопкой мыши таблицу **dbo.AspNetUsers** и выберите **Просмотр данных**.
    ...
    ![ Элемент меню Просмотр данных][ Элемент меню Просмотр данных]

     Теперь вы можете просмотреть данные, связанные с пользователем <canEditUser@contoso.com>.

    ![Окно ContactManager][Окно ContactManager]
...
1.  В **обозревателе объектов SQL Server** щелкните правой кнопкой мыши элемент **dbo.AspNetUserRoles** и выберите **Просмотр данных**.
    ...
    ![ Данные AspNetUserRoles][ Данные AspNetUserRoles]
...
Дополнительную информацию о веб-формах ASP.NET см. в разделе [Знакомьтесь с веб-формами ASP.NET][Знакомьтесь с веб-формами ASP.NET] на веб-сайте ASP.NET и в [Microsoft Azure: учебники и руководства][Microsoft Azure: учебники и руководства].

Этот учебник построен на основе учебника MVC [Развертывание безопасного приложения ASP.NET MVC 5 с членством, OAuth и базой данных SQL на веб-сайте Azure][Развертывание безопасного приложения ASP.NET MVC 5 с членством, OAuth и базой данных SQL на веб-сайте Azure], написанным Риком Андерсоном (Rick Anderson) (Twitter [@RickAndMSFT][@RickAndMSFT]) при помощи Тома Дейкстры (Tom Dykstra) и Бэрри Дорранса (Barry Dorrans) (Twitter [@blowdart][@blowdart]).
