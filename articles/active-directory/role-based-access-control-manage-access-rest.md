...
Для вывода списка назначений ролей требуется доступ к операции `Microsoft.Authorization/roleAssignments/read` *в определенной области*. Доступ к этой операции предоставляется всем встроенным ролям. Дополнительные сведения о назначениях ролей и контроле доступа к ресурсам Azure см. в статье [Контроль доступа на основе ролей Azure](role-based-access-control-configure.md).

### Запрос

| **Метод** | **URI запроса** |
|--------|-------------|
| ПОЛУЧЕНИЕ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleAssignments?api-version={api-version}&filter={filter}` |

Замените текст *{api-version}* на 2015-07-01.
...
| | Фильтр |
|-----------------------------------------------------------------------------------------------------------------|--------|
| Вывод списка назначений ролей только для определенной области без учета внутренних областей | `atScope()` <br/> Например, <br/> `&filter=atScope()` |
| Вывод списка назначений ролей только для определенного пользователя, группы или приложения | `principalId%20eq%20'{objectId}'` <br/> Замените текст *{objectId}* ObjectId Azure AD пользователя, группы или субъекта-службы. <br/> Например, <br/> `&filter=principalId%20eq%20'3a477f6a-6739-4b93-84aa-3be3f8c8e7c2'` |
| Вывод списка назначений ролей только для определенного пользователя, в том числе тех, которые назначены группам, в которые он входит | `assignedTo('{objectId}')` <br/> Замените текст *{objectId}* ObjectId Azure AD пользователя. <br/> Например, <br/> `&filter=assignedTo('3a477f6a-6739-4b93-84aa-3be3f8c8e7c2')` |
...
Чтобы получить сведения о назначении роли, требуется доступ к операции `Microsoft.Authorization/roleAssignments/read`. Доступ к этой операции предоставляется всем встроенным ролям. Дополнительные сведения о назначениях ролей и контроле доступа к ресурсам Azure см. в статье [Контроль доступа на основе ролей Azure](role-based-access-control-configure.md).

### Запрос

| **Метод** | **URI запроса** |
|--------|-------------|
| ПОЛУЧЕНИЕ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleAssignments/{role-assignment-id}?api-version={api-version}` |

Замените текст *{api-version}* на 2015-07-01.

Замените текст *{role-assignment-id}* идентификатором GUID назначения роли.
...
Чтобы создать назначение роли, требуется доступ к операции `Microsoft.Authorization/roleAssignments/write`. Из встроенных ролей эту операцию могут выполнять только *владелец и администратор доступа пользователей*. Дополнительные сведения о назначениях ролей и контроле доступа к ресурсам Azure см. в статье [Контроль доступа на основе ролей Azure](role-based-access-control-configure.md).

### Запрос

| **Метод** | **URI запроса** |
|--------|-------------|
| ОТПРАВКА | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleAssignments/{role-assignment-id}?api-version={api-version}` |

Замените текст *{api-version}* на 2015-07-01.

Замените текст *{role-assignment-id}* новым GUID. Он будет использоваться в качестве идентификатора GUID нового назначения роли.
...
При создании назначения роли в родительской области все дочерние области также его наследуют.
...
|------------------|----------|--------|-------------------------------------------------------------------------------------------------------------|
| roleDefinitionId | Да | Строка | Идентификатор роли, которую необходимо назначить. Он указывается в следующем формате: <br/> `{scope}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id-guid}` |
...
| **Метод** | **URI запроса** |
|--------|-------------|
| УДАЛИТЬ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleAssignments/{role-assignment-id}?api-version={api-version}` |
...
Для вывода списка ролей требуется доступ к операции `Microsoft.Authorization/roleDefinitions/read` в определенной области. Доступ к этой операции предоставляется всем встроенным ролям. Дополнительные сведения о назначениях ролей и контроле доступа к ресурсам Azure см. в статье [Контроль доступа на основе ролей Azure](role-based-access-control-configure.md).

### Запрос

| **Метод** | **URI запроса** |
|--------|-------------|
| ПОЛУЧЕНИЕ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleDefinitions?api-version={api-version}&filter={filter}` |
...
| | Фильтр |
|--------------------------------------------------------------------------------------------|--------|
| Вывод списка ролей, доступных для назначения в указанной области и любой из ее дочерних областей | `atScopeAndBelow()` <br/> Например, <br/> `&filter=atScopeAndBelow()` |
| Поиск с помощью точного отображаемого имени роли | `roleName%20eq%20'{role-display-name}'` <br/> Замените *{role-display-name}* закодированным URL-адресом точного отображаемого имени роли. <br/> Например, <br/> `$filter=roleName%20eq%20'Virtual%20Machine%20Contributor'` |
...
Чтобы получить сведения о роли, требуется доступ к операции `Microsoft.Authorization/roleDefinitions/read`. Доступ к этой операции предоставляется всем встроенным ролям. Дополнительные сведения о назначениях ролей и контроле доступа к ресурсам Azure см. в статье [Контроль доступа на основе ролей Azure](role-based-access-control-configure.md).

### Запрос

| **Метод** | **URI запроса** |
|--------|-------------|
| ПОЛУЧЕНИЕ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}?api-version={api-version}` |

Замените текст *{api-version}* на 2015-07-01.

Замените текст *{role-definition-id}* идентификатором GUID определения роли.
...
| **Метод** | **URI запроса** |
|--------|-------------|
| ОТПРАВКА | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}?api-version={api-version}` |

Замените текст *{api-version}* на 2015-07-01.

Замените текст *{role-definition-id}* новым GUID. Он будет использоваться в качестве идентификатора GUID новой настраиваемой роли.

Замените *{scope}* первой областью *AssignableScope* настраиваемой роли. В следующих примерах показано, как указать область для различных уровней.
...
Замените текст *{role-definition-id}* идентификатором GUID настраиваемой роли, которую требуется обновить.
...
| **Метод** | **URI запроса** |
|--------|-------------|
| УДАЛИТЬ | `https://management.azure.com/{scope}/providers/Microsoft.Authorization/roleDefinitions/{role-definition-id}?api-version={api-version}` |

Замените текст *{api-version}* на 2015-07-01.

Замените текст *{role-definition-id}* идентификатором GUID определения настраиваемой роли, которую требуется удалить.
