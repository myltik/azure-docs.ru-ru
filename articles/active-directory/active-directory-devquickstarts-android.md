...
11. **Broker**. Компонент broker предоставляется приложением корпоративного портала Microsoft Intune. Библиотека ADAL будет использовать учетную запись broker, если существует одноименная пользовательская учетная запись, созданная в этой структуре проверки подлинности, и разработчик решил не игнорировать ее. Разработчик может игнорировать (пропустить) пользователя broker с помощью следующего вызова:
