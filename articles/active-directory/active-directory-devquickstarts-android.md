...
Библиотека ADAL предоставляет кэш по умолчанию в SharedPrefrecens и несколько простых функций по работе с кэшем. Для получения текущего содержимого кэша из AuthenticationContext используйте следующий метод:
```Java
 ITokenCacheStore cache = mContext.getCache();
```
При необходимости вы можете реализовать собственный кэш.
```Java
