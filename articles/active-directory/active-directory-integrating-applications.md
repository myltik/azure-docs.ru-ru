...
1. В верхнем меню щелкните **Приложения**. Если в каталоге нет добавленных приложений, на этой странице отображается только ссылка "Добавить приложение". Щелкните ссылку или нажмите кнопку **Добавить** на панели команд.
...
1. На странице "Свойства приложения" укажите URL-адрес входа и URI кода приложения для регистрируемого веб-приложения \(или просто URI перенаправления для собственного клиентского приложения\), а затем установите флажок в правом нижнем углу страницы.
...
Дополнительные сведения об инфраструктуре согласия см. в статьях [OAuth 2.0 in Azure AD](https://msdn.microsoft.com/library/azure/dn645545.aspx) \(OAuth 2.0 в Azure AD\), [Сценарии аутентификации в Azure AD](active-directory-authentication-scenarios.md), а также в разделе об Office 365 в статье [Understanding authentication with Office 365 APIs](https://msdn.microsoft.com/office/office365/howto/common-app-authentication-tasks) \(Проверка подлинности с помощью API-интерфейсов Office 365\).
...
1. Разверните веб-API доступа в области "Другие приложения" страницы быстрого запуска и перейдите по ссылке **Настроить сейчас** в разделе "Выберите разрешения". Отображается страница свойств приложения.

1. Выполните прокрутку вниз до раздела "Разрешения для других приложений". В первом столбце представлен список доступных приложений ресурсов в каталоге, предоставляющих доступ к веб-API. Выбрав нужное приложение, можно выбрать разрешения приложения и делегирования, предоставляемые веб-API.

1. Выбрав нужные элементы, нажмите кнопку **Сохранить** на панели команд.
...
В можете разработать веб-API и сделать его доступным для клиентских приложений, предоставив области разрешений. Доступ к правильно настроенному веб-API предоставляется по аналогии с другими веб-API корпорации Майкрософт, включая API Graph и API Office 365. Области разрешений предоставляются с помощью манифеста приложения, JSON-файла, который представляет собой конфигурацию удостоверения приложения. Чтобы предоставить области разрешений, перейдите в свое приложение на классическом портале управления и нажмите кнопку "Манифест приложения" на панели команд.

#### Добавление области разрешений для приложения ресурсов

1. Войдите на [классический портал Azure](https://manage.windowsazure.com).

1. Щелкните значок Active Directory в левом меню, а затем выберите нужный каталог.

1. В верхнем меню щелкните элемент **Приложения** и выберите приложение ресурсов, которое требуется настроить. Отображается страница быстрого запуска с единым входом и другими сведениями о конфигурации.

1. Нажмите кнопку **Управление манифестом** в панели команд и выберите **Загрузить манифест**.

1. Откройте JSON-файл манифеста приложения и замените узел oauth2Permissions следующим фрагментом JSON-кода. В этом фрагменте приведен пример предоставления области разрешений, которая называется олицетворением пользователя. Не забудьте изменить текст и значения для своего приложения:
	...
	>[AZURE.NOTE] Позднее вы можете по мере необходимости предоставлять дополнительные области разрешений. Помните о том, что веб-API может предоставлять несколько разрешений, связанных с множеством разных функций. Теперь вы можете управлять доступом к веб-API с помощью утверждения области \(scp\) в полученном токене JWT OAuth 2.0.

1. Сохраните обновленный JSON-файл и отправьте его. Для этого нажмите кнопку **Управление манифестом** в панели команд, выберите команду **Отправить манифест**, перейдите к обновленному файлу манифеста и выберите его. После отправки веб-API настроен для использования другими приложениями в каталоге.
...
1. Выполните прокрутку вниз до раздела "Разрешения для других приложений". Щелкните раскрывающееся меню "Выбор приложения", чтобы выбрать веб-API, для которого только что было предоставлено разрешение. В раскрывающемся меню "Делегированные разрешения" выберите новое разрешение.
...
На самом деле манифест приложения является механизмом обновления сущности "Приложение", который определяет все атрибуты конфигурации удостоверения приложения Azure AD, в том числе области разрешений API, рассмотренные выше. Дополнительную информацию о сущности "Приложение" см. в разделе Application entity \(Сущность "Приложение"\) в статье [Graph API Application entity documentation](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#EntityreferenceApplicationEntity) \(Документация по сущности "Приложение API Graph"\). В этом разделе вы найдете полную информацию об элементах сущности "Приложение", которые используются при указании разрешений для API:

- элемент appRoles — коллекция сущностей [AppRole](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#AppRoleType), которые можно использовать при определении **разрешений приложения** для веб-API;  
- элемент oauth2Permissions — коллекция сущностей [OAuth2Permission](https://msdn.microsoft.com/Library/Azure/Ad/Graph/api/entity-and-complex-type-reference#OAuth2PermissionType), которые можно использовать при определении **делегированных разрешений** для веб-API.
...
Полную информацию об областях разрешений можно найти на следующих ресурсах.

- Описание областей разрешений, предоставляемых Azure AD API Graph, см. в статье [Permission scopes \| Graph API concepts](https://msdn.microsoft.com/Library/Azure/Ad/Graph/howto/azure-ad-graph-api-permission-scopes) \(Области разрешений \| Понятия API Graph\).
...
1. В верхнем меню нажмите **Приложения** и выберите приложение, которое требуется настроить. Отобразится страница быстрого запуска с параметрами конфигурации.

1. На странице быстрого запуска разверните раздел **Настройка мультитенантного приложения**, а затем в разделе "Включение доступа" щелкните ссылку **Настроить сейчас**. Отображается страница свойств приложения.
...
1. В верхнем меню нажмите **Приложения** и выберите приложение, которое требуется настроить. Отображается страница быстрого запуска с единым входом и другими сведениями о конфигурации.
1. Нажмите кнопку **Управление манифестом** в панели команд и выберите **Загрузить манифест**. Откройте файл манифеста приложения JSON и установите значение параметра oauth2AllowImplicitFlow в true. По умолчанию значение этого параметра равно false.
...
1. По завершении обновления рабочей среды можно вернуться на классический портал Azure и удалить старый ключ.
...
1. Нажмите кнопку **Да** в сообщении с подтверждением.
...
1. В разделе "Приложение является мультитенантным" нажмите кнопку **Нет**. Приложение будет преобразовано в однотенантное, но оно по-прежнему будет относиться к организации, которая предоставила свое согласие.
