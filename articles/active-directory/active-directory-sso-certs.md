...
![единого входа Azure AD](./media/active-directory-sso-certs/fed-sso.PNG)

## <a name="how-to-customize-the-expiration-date-for-your-federation-certificate"></a>Настройка даты окончания срока действия сертификата федерации
...
2. Выберите **Единый вход Azure AD**, а затем нажмите кнопку **Далее**.
3. Введите **URL-адрес входа** своего приложения и установите флажок **Настроить сертификат, используемый для федеративного единого входа**. Нажмите кнопку **Далее**.
   
    ![единого входа Azure AD](./media/active-directory-sso-certs/new-app-config-sso.PNG)
...
3. На второй странице установите флажок **Настроить сертификат, используемый для федеративного единого входа**. Нажмите кнопку **Далее**.
    ...
    ![единого входа Azure AD](./media/active-directory-sso-certs/renew-config-sso.PNG)
4. На следующей странице выберите пункт **Создание нового сертификата**и укажите срок действия нового сертификата. Нажмите кнопку **Далее**.
    ...
    ![Создание нового сертификата](./media/active-directory-sso-certs/new-app-config-cert.PNG)
5. Нажмите кнопку **Загрузить сертификат**. Чтобы обновить сертификат, сделайте следующее:
   ...
   * В нижней части диалогового окна службы Azure AD установите флажок подтверждения, чтобы разрешить использование нового сертификата. Затем нажмите кнопку **Далее** для отправки.
     ...
     > [!IMPORTANT]
     > Единый вход в приложение будет отключен, как только будет выполнено одно из указанных действий. После выполнения второго действия эта функция будет включена. Таким образом, чтобы максимально сократить время простоя, приготовьтесь выполнить оба действия с небольшим интервалом.
     > 
     > 
