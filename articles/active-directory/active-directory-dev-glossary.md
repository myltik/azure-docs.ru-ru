---
title: "Глоссарий по Azure Active Directory для разработчика | Документация Майкрософт"
...
При регистрации или обновлении приложения [на классическом портале Azure][AZURE-classic-portal] создается или обновляется [объект приложения](#service-principal-object) и соответствующий объект субъекта-службы для этого клиента. Объект приложения глобально *определяет* конфигурацию удостоверения для приложения (для всех клиентов, где у него есть доступ) и предоставляет шаблон, *на основе* которого создаются соответствующие объекты субъектов-служб для локального использования во время выполнения (в конкретном клиенте).

Дополнительные сведения см. в статье [Объекты приложений и субъекта-службы в Azure Active Directory][AAD-App-SP-Objects].
...
## <a name="authentication"></a>метод проверки подлинности
...
При интеграции приложений Azure AD эта служба реализует роль сервера авторизации для этих приложений и API службы Майкрософт, например [Microsoft API Graph][Microsoft-Graph].
...
Как определено в [OAuth2 Authorization Framework][OAuth2-Role-Def], это сущность, которая может предоставить доступ к защищенному ресурсу. Если владелец ресурса — человек, он называется пользователем. Например, если [клиентскому приложению](#client-application) требуется получить доступ к почтовому ящику пользователя через [Microsoft API Graph][Microsoft-Graph], ему необходимо получить разрешение у владельца ресурса почтового ящика.
...
Рекомендуется использовать такой формат именования: resource.operation.constraint. Подробное описание областей, предоставляемых API Graph Azure AD, см. в статье [Области разрешений | Основные понятия API Graph][AAD-Graph-Perm-Scopes]. Сведения об областях, предоставляемых службами Office 365, см. в [справочнике по разрешениям API Office 365][O365-Perm-Ref].
...
[Руководство разработчика по Azure AD][AAD-Dev-Guide] охватывает все темы, связанные с разработкой Azure AD, в том числе обзор [интеграции приложений][AAD-How-To-Integrate], а также основы [проверки подлинности Azure AD и поддерживаемые сценарии проверки подлинности][AAD-Auth-Scenarios].
