...
5. Следуйте инструкциям на экране, а затем создайте новое **Собственное клиентское приложение**. **Имя** приложения является его описанием для конечных пользователей. **URI перенаправления** представляет собой сочетание схемы и строки, используемое Azure AD для возвращения любых маркеров, запрошенных приложением.  Укажите значение, специфичное для вашего приложения, например `http://DirectorySearcher`. Щелкните **Создать**, чтобы создать приложение.
...
8. Настройте разрешения для приложения. В меню "Параметры" выберите раздел "Необходимые разрешения", щелкните **Добавить**, а затем — **Выбор API** и выберите Windows Azure Active Directory (это API AADGraph). Затем щелкните **Выбор разрешений** и выберите разрешение "Чтение данных каталога". Это позволит приложению запрашивать интерфейс Graph API для пользователей.
