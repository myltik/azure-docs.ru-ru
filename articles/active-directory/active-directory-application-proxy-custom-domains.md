...
## <a name="select-your-custom-domain"></a>Выбор личного домена
...
**Вопрос. Можно ли выбрать уже переданный сертификат без его повторной передачи?**  
...
**Вопрос. Как добавить сертификат и в каком формате необходимо передавать экспортированный сертификат?**  
...
Ответ. В разделе "Сертификаты" на странице настройки приложения появляется предупреждение. При обращении пользователя к приложению появляется предупреждение о безопасности.  
...
Ответ. При передаче нового сертификата старый сертификат автоматически удаляется, если он не используется другим приложением.  
...
Ответ. Да, самозаверяющие сертификаты использовать разрешено. При использовании частного центра сертификации CDP (точка распространения точки отзыва сертификата) для сертификата должна быть общедоступной.  
