...
- При использовании отдельного SQL Server действуют следующие требования:
    ...
    - Необходимо использовать параметры сортировки SQL без учета регистра. Их имена содержат "\_CI\_". Параметры сортировки с учетом регистра, имена которых содержат "\_CS\_", **не поддерживаются**.
    - На один экземпляр базы данных может приходиться только один модуль синхронизации. Совместное использование экземпляра базы данных с FIM/MIM Sync, DirSync или Azure AD Sync **не поддерживается**.
...
- По умолчанию Azure AD Connect использует для взаимодействия с Azure AD протокол TLS 1.0. Его можно изменить на TLS 1.2, выполнив инструкции раздела [Включение протокола TLS 1.2 для Azure AD Connect](#enable-tls-12-for-azure-ad-connect).
