...
На прокси-сервере должен быть также открыт необходимый URL-адрес. Официальный список см. в разделе [URL-адреса и диапазоны IP-адресов Office 365](https://support.office.com/article/Office-365-URLs-and-IP-address-ranges-8548a211-3fe7-47cb-abb1-355ea5aa88a2).
...
Эта ошибка отобразится в том случае, если не удается связаться с конечной точкой **https://secure.aadcdn.microsoftonline-p.com** и глобальный администратор включил Многофакторную идентификацию (MFA).  
...
Если мастер установки успешно подключается к Azure AD, но проверить пароль невозможно, то отображается следующее: ![badpassword](./media/active-directory-aadconnect-troubleshoot-connectivity/badpassword.png).
...
### <a name="the-signin-assistant-has-not-been-correctly-configured"></a>Неправильно настроен помощник по входу
