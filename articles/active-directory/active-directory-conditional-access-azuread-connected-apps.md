...
# Основные сведения об условном доступе к Azure AD  

Условный доступ к Azure Active Directory для приложений SaaS и приложений, подключенных к Azure AD, доступен в виде общедоступной предварительной версии. В предварительной версии условный доступ можно настроить по группам, расположениям и уровням конфиденциальности приложений.

Правила многофакторной проверки подлинности можно применять ко всем пользователям, которые имеют доступ к приложению, или только к пользователям, которые входят в определенные группы безопасности. Используя условный доступ по уровням конфиденциальности приложений, правила многофакторной проверки подлинности можно настроить для каждого приложения отдельно. Это дает возможность блокировать пользователей, которые не находятся в доверенной сети. Пользователи могут быть исключены из требования многофакторной проверки подлинности, если они получают доступ к приложению с IP-адреса в сети организации.
...
Эта предварительная версия применяется только к предварительно интегрированным федеративным приложениям SaaS, приложениям, в которых используется единый вход с паролем, зарегистрированным разработанным бизнес-приложениям и прокси приложений Azure AD. Еще добавляются дополнительные приложения.
...
* **Требовать многофакторную проверку подлинности**. При использовании этого варианта пользователям, к которым применяются правила доступа, потребуется пройти многофакторную проверку. Только так они смогут получить доступ к приложению, к которому применяется политика.

* **Требовать многофакторную проверку подлинности, если не на работе**. Если выбран этот вариант, пользователю с доверенным IP-адресом не нужно проходить многофакторную проверку подлинности. Диапазоны надежных IP-адресов можно задать на странице параметров многофакторной проверки подлинности.

* **Блокировать доступ, если не на работе**. Если используется этот вариант, пользователь с недоверенным IP-адресом будет заблокирован. Диапазоны надежных IP-адресов можно задать на странице параметров многофакторной проверки подлинности.
...
По умолчанию многофакторная проверка подлинности осуществляется на странице, размещенной в Azure Active Directory. Чтобы настроить локальную многофакторную проверку подлинности, воспользуйтесь модулем Azure AD для Windows PowerShell и в Azure Active Directory задайте для параметра -SupportsMFA значение **True**.
