...
- оно не должно содержать специальные символы: &#92; ! # $ % & * + / = ? ^ &#96; { } | ~ < > ( ) ' ; : , [ ] " @;
...
В случае перемещения пользователей между лесами или доменами вам потребуется найти такой атрибут, который никогда не изменяется. Зачастую можно использовать атрибут **employeeID**. Если значение вашего атрибута будет содержать буквы, например **sAMAccountName**, вам потребуется исключить вероятность изменения регистра (на верхний или нижний) в значении атрибута. Не следует использовать атрибуты, содержащие имя пользователя. В случае женитьбы или развода имя может измениться, что недопустимо для этого атрибута. Это одна из причин, почему мастер установки Azure AD Connect не позволяет выбирать такие атрибуты, как **userPrincipalName**, **mail** и **targetAddress**. Эти атрибуты также содержат символ @, что недопустимо для sourceAnchor.

Если вы не можете подобрать подходящий атрибут, потребуется ввести искусственное значение. Для этого подойдет атрибут, который будет выглядеть, как идентификатор GUID. При создании объекта пользователю присваивается новый идентификатор GUID. При перемещении объекта убедитесь, что это значение также копируется.
