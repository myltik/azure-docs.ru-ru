...
| completed-*-errors | Выполнение завершено, но с ошибками (меньше 5000), которые должны быть рассмотрены. | | completed-*-warnings | Запуск завершен, но некоторые данные не находятся в ожидаемом состоянии. Если обнаружены ошибки, обычно это лишь симптом. Пока вы не исправили ошибки, не следует рассматривать предупреждения. |
...
Начните с щелчка строки ошибки (в приведенном выше рисунке **sync-rule-error-function-triggered**). Сначала отобразится обзор объекта. Чтобы просмотреть фактическую ошибку, нажмите кнопку **Stack Trace** (Трассировка стека). Отобразится информация уровня отладки об ошибке.

**Совет.** Вы можете щелкнуть правой кнопкой поле **call stack information** (Сведения о стеке вызовов), выбрать пункт **select all** (Выбрать все) и щелкнуть **copy** (Копировать). Затем можно скопировать стек и просмотреть ошибку в своем любимом редакторе, например Блокноте.
