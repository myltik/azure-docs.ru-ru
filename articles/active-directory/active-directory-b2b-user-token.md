...
Если вам необходимо знать, как выглядит токен для пользователя службы совместной работы B2B, то ниже приведены сведения о токене носителя и содержимое токена для гостевого пользователя Azure Active Directory (Azure AD) и гостевого пользователя учетной записи Майкрософт в клиенте ресурса (для tenantid:04dcc6ab-388a-4559-b527-fbec656300ea). Чтобы просмотреть содержимое токена JSON Web Token (JWT), воспользуйтесь сайтом [https://jwt.io](https://jwt.io/) или [http://calebb.net](http://calebb.net/).
