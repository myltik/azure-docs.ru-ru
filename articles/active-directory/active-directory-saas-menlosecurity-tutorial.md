...
- С помощью Azure AD вы можете контролировать доступ к Menlo Security.
...
Чтобы настроить интеграцию Menlo Security с Azure AD, необходимо добавить Menlo Security из коллекции в список управляемых приложений SaaS.

**Чтобы добавить Menlo Security из коллекции, сделайте следующее:**
...
Чтобы единый вход работал, Azure AD необходимо знать, какой пользователь в Menlo Security соответствует пользователю в Azure AD. Иными словами, необходимо установить связь между пользователем Azure AD и соответствующим пользователем в Menlo Security.
...
Чтобы настроить и проверить единый вход Azure AD в Menlo Security, вам потребуется выполнить действия в следующих стандартных блоках:
...
В этом разделе описано, как включить единый вход Azure AD на портале Azure и настроить его в приложении Menlo Security.

**Чтобы настроить единый вход Azure AD в Menlo Security, сделайте следующее:**
...
8. В разделе **Параметры** выберите **Проверка подлинности** и выполните следующие действия:
    ...
    г) **SAML 2.0 Endpoint** (Конечная точка SAML 2.0). Вставьте **URL-адрес службы единого входа SAML**, скопированный на портале Azure.
    ...
    g. Нажмите кнопку **Сохранить** , чтобы сохранить параметры.
...
В этом разделе описано, как предоставить пользователю Britta Simon доступ к Menlo Security, чтобы он мог использовать единый вход Azure.

![Назначение пользователя][200] 

**Чтобы назначить пользователя Britta Simon в Menlo Security, сделайте следующее:**
...
Войдите в приложение по любому URL-адресу с префиксом (например, `https://safe.menlosecurity.com/http://www.cnn.com/`) и введите учетные данные Azure AD. Затем после успешной проверки подлинности вы будете перенаправлены на соответствующий веб-сайт.
