...
- Вы можете использовать Microsoft Identity Manager (MIM) 2016 SP1 и агент управления MIM для Microsoft Graph. (Для этого требуется подписка Azure AD Premium 1.)
- Вы можете использовать скрипт PowerShell. (Для этого решения не требуются MIM или AD Premium.)
...
Теперь мы предоставили возможность синхронизировать локальные учетные записи с помощью AAD Connect с облаком, где они будут считаться "гостевыми пользователями" и использоваться точно так же, как обычные пользователи AAD B2B. Такое решение работает даже в сочетании с локальной системой управления удостоверениями, которая позволяет партнерам использовать внешние адреса электронной почты в качестве имен для входа.
