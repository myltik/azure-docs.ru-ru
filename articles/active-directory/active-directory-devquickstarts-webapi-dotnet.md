...
В веб-приложениях ASP.NET это можно сделать с помощью реализации Майкрософт промежуточного слоя OWIN (открытого интерфейса для .NET, поддерживаемого сообществом разработчиков), включенного в .NET Framework 4.5. Здесь мы используем OWIN для создания веб-интерфейса API приложения To Do List (список задач), который: определяет, какие интерфейсы следует защищать, и проверяет, чтобы вызовы веб-API содержали допустимый маркер доступа.
...
-	После завершения регистрации перейдите на вкладку **Настройка** и найдите поле **URI кода приложения**. Укажите для этого параметра идентификатор, специфичный для клиента, например `https://contoso.onmicrosoft.com/TodoListService`
- Также на вкладке **Настройка** найдите раздел "Разрешения для других приложений". Щелкните "Добавить приложение". Из раскрывающегося списка "Показать" выберите "Другое" и щелкните флажок вверху. Найдите и щелкните службу To Do List, затем щелкните флажок внизу, чтобы добавить приложение. Выберите "Доступ к ’Служба To Do List’" из выпадающего меню "Делегированные разрешения" и сохраните конфигурацию.
