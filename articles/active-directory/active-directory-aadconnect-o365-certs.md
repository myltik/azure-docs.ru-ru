...
Замените `<your_FS_name> ` именем узла службы федерации, который используется вашей организацией (например, fs.contoso.com). Если проверка двух этих параметров оказалась успешной, вам больше не нужно ничего делать.
...
- Чтобы создать сертификат, выполните следующую команду в командной строке PowerShell: `PS C:\>Update-ADFSCertificate –CertificateType token-signing`.
...
4.	Если эти команды выполняются на компьютере, который не является сервером основной федерации AD FS, выполните команду Set-MSOLAdfscontext -Computer <AD FS primary server>, где <AD FS primary server> является внутренним полным доменным именем основного сервера AD FS. Этот командлет создает контекст, подключающий вас к AD FS. 
5.	Запустите Update-MSOLFederatedDomain –DomainName <domain>. Этот командлет обновляет параметры AD FS в облачной службе и настраивает отношения доверия между ними.

>[AZURE.NOTE]Если требуется поддержка нескольких доменов верхнего уровня, например contoso.com и fabrikam.com, необходимо использовать параметр SupportMultipleDomain с любыми командлетами. Дополнительные сведения см. в статье, описывающей поддержку нескольких доменов верхнего уровня. Наконец, убедитесь, что все прокси-серверы веб-приложения обновлены с помощью пакета [Windows Server, май 2014 г.](http://support.microsoft.com/kb/2955164), иначе случае прокси могут не обновиться, что приведет к прерыванию работы.
