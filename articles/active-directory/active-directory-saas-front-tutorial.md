...
3. Если требуется настроить приложение в режиме, инициируемом **IDP**, то в разделе **Домены и URL-адреса приложения Front** сделайте следующее:
...
4. Установите флажок **Показать дополнительные параметры URL-адресов**, если хотите настроить приложение для работы в режиме, инициируемом **поставщиком услуг**:
    ...
    В текстовом поле **URL-адрес для входа** введите URL-адрес в следующем формате: `https://<companyname>.frontapp.com`
     
    > [!NOTE] 
    > Эти значения приведены в качестве примера. Вместо этих значений укажите фактические идентификатор, URL-адрес ответа и URL-адрес входа, к которым мы вернемся позже в этом руководстве, или обратитесь в [службу поддержки клиентов Front](mailto:support@frontapp.com), чтобы получить эти значения. 
...
7. В разделе **Конфигурация Front** щелкните **Настроить Front**, чтобы открыть окно **Настройка единого входа**. Скопируйте **URL-адрес выхода, идентификатор сущности SAML и URL-адрес службы единого входа SAML**  из раздела **Краткий справочник**.

    ![Настройка единого входа](./media/active-directory-saas-front-tutorial/tutorial_front_configure.png) 

8. Войдите в клиент Front с правами администратора.

9. Последовательно выберите пункты **Settings (Параметры) (значок шестеренки в нижней части левой боковой панели) > Preferences (Предпочтения)**.
...
11. В раскрывающемся списке **Single Sign On** (Единый вход) выберите **SAML**.
   
    ![Настройка единого входа на стороне приложения](./media/active-directory-saas-front-tutorial/tutorial_front_002.png)

12. В текстовое поле **Entry Point** (Точка входа) введите значение **URL-адрес службы единого входа** из мастера настройки приложения Azure AD.
    
    ![Настройка единого входа на стороне приложения](./media/active-directory-saas-front-tutorial/tutorial_front_003.png)

13. Откройте скачанный файл **сертификата в кодировке Base64** в Блокноте, скопируйте его содержимое в буфер обмена, а затем вставьте его в текстовое поле **Signing certificate** (Сертификат для подписи).
    
    ![Настройка единого входа на стороне приложения](./media/active-directory-saas-front-tutorial/tutorial_front_004.png)

14. В разделе **Service provider settings** (Параметры поставщика услуг) сделайте следующее:
    ...
    b. Скопируйте значение **ACS URL** (URL-адрес ACS) и вставьте его в текстовое поле **URL-адрес входа** в разделе **Домены и URL-адреса приложения Front** на портале Azure.
