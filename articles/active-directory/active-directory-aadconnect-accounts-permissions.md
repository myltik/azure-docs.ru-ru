...
Локальная учетная запись службы создается мастером установки (если только вы не укажете нужную учетную запись в пользовательских параметрах). Эта учетная запись имеет префикс **AAD\_** и используется для запуска фактической службы синхронизации. Если Azure AD Connect устанавливается на контроллере домена, учетная запись создается в этом домене. Если SQL Server используется на удаленном сервере, учетная запись службы **AAD\_** должна находиться в домене.
...
Эта учетная запись будет использоваться системой Windows для хранения ключей шифрования, поэтому не следует сбрасывать или изменять пароль для этой учетной записи.
