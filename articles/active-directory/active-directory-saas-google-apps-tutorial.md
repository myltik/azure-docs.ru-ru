<properties
   pageTitle="Учебник. Интеграция Google Apps с Azure Active Directory | Microsoft Azure"
...
2. В списке **Каталог** выберите каталог, в который хотите добавить Google Apps.
...
6. В **поле поиска** введите **Google Apps**. Выберите **Google Apps** в списке результатов и нажмите кнопку **Завершить**, чтобы добавить приложение.
...
2. Откроется диалоговое окно, и вы увидите экран с вопросом «Как пользователи должны входить в Google Apps?». Выберите пункт **Единый вход Azure AD**, а затем нажмите кнопку **Далее**.
...
4. На странице **Настройка единого входа в Google Apps** нажмите кнопку **Загрузить сертификат**, а затем сохраните файл сертификата локально на компьютере.

	![Загрузка сертификата][9]

5. Откройте в браузере новую вкладку и войдите в [консоль администратора Google Apps](http://admin.google.com/) с использованием учетной записи администратора.

6. Щелкните **Security (Безопасность)**. Если эта ссылка не отображается, она может быть скрыта в разделе **More Controls (Дополнительно)** в нижней части экрана.

	![Выбор элемента «Безопасность»][10]

7. На странице **Security (Безопасность)** выберите команду **Set up single sign-on (SSO) (Настроить единый вход)**.
...
8. Выполните следующие настройки.
	...
	- Скопируйте **URL-адрес службы единого входа** в Azure AD и вставьте его в поле **Sign-in page URL (URL-адрес страницы входа)** в Google Apps.

	- Скопируйте **URL-адрес службы единого выхода** в Azure AD и вставьте его в поле **Sign-out page URL (URL-адрес страницы выхода)** в Google Apps.

	- Скопируйте **URL-адрес изменения пароля** в Azure AD и вставьте его в поле **Change password URL (URL-адрес изменения пароля)** в Google Apps.

	- В поле **Verification certificate (Сертификат проверки)** в Google Apps добавьте сертификат, загруженный на этапе 4.

	- Нажмите кнопку **Сохранить изменения**.

9. В Azure AD установите флажок подтверждения настройки единого входа, чтобы активировать сертификат, который вы отправили в Google Apps. Нажмите кнопку **Далее**.
...
11. Нажмите кнопку **Завершить**, чтобы закрыть диалоговое окно. Сведения о проверке конфигурации см. в разделе [Назначение пользователей Google Apps](#step-4-assign-users-to-google-apps).

##Шаг 3. Включение автоматической подготовки пользователей

> [AZURE.NOTE]Одним из способов автоматизации подготовки пользователей для Google Apps является использование средства синхронизации каталогов [Google Apps Directory Sync (GADS)](https://support.google.com/a/answer/106368?hl=en), которое позволяет локально подготовить удостоверения Active Directory для Google Apps. В этом же учебнике мы рассмотрим подготовку к использованию Google Apps пользователей Azure Active Directory (облако) и групп, поддерживающих почту.

1. Войдите в [консоль администратора Google Apps](http://admin.google.com/) с использованием учетной записи администратора и щелкните **Security (Безопасность)**. Если эта ссылка не отображается, она может быть скрыта в разделе **More Controls (Дополнительно)** в нижней части экрана.
...
4. Если вы еще не добавили имя личного домена в Azure Active Directory, выполните следующие действия.
	...
	- Щелкните **Домены** в меню верхнего уровня, а затем нажмите кнопку **Добавить личный домен**.

		![Добавление пользовательского домена][17]
	...
	- Нажмите кнопку **Далее**, чтобы перейти на страницу проверки. Для подтверждения того, что домен принадлежит вам, необходимо изменить его записи DNS в соответствии со значениями, приведенными на этой странице. Вы можете выполнить проверку с помощью параметра **Записи MX** или **Записи TXT**. Для этого выберите соответствующее значение в поле **Тип записи**. Более полные инструкции по подтверждению доменного имени в Azure AD см. в разделе [Добавление доменного имени в Azure AD](https://go.microsoft.com/fwLink/?LinkID=278919&clcid=0x409).
...
5. После подтверждения всех доменов в Azure AD необходимо подтвердить их в Google Apps. Для каждого домена, который еще не зарегистрирован в Google Apps, выполните следующие действия.
	...
	- Щелкните **Continue and verify domain ownership (Перейти к проверке владельца домена)**. Затем выполните действия, подтверждающие, что вы владеете доменным именем. Подробные инструкции по проверке домена в Google Apps см. на странице [Verify your site ownership with Google Apps (Подтверждение права собственности на сайт в Google Apps)](https://support.google.com/webmasters/answer/35179).

	- Повторите описанные выше шаги для других доменов, которые нужно добавить в Google Apps.

	> [AZURE.WARNING]Если вы измените основной домен клиента Google Apps после того, как настроите единый вход в Azure AD, вам необходимо будет повторить шаг 3, описанный в разделе [Шаг 2. Настройка единого входа](#step-two-enable-single-sign-on).
...
7. Определите, какую учетную запись администратора вы хотите использовать для управления подготовкой пользователей. Чтобы назначить **административную роль** этой учетной записи, измените свойство **Privileges (Привилегии)** для этой роли. Убедитесь, что для учетной записи выбраны все параметры в разделе **Admin API Privileges (Административные права доступа к API)**. Это позволит использовать ее для подготовки.

	![Выбор Google Apps][27]

	> [AZURE.NOTE]При настройке рабочей среды рекомендуется создать новую учетную запись администратора Google Apps специально для этого шага. Этой учетной записи должна быть назначена административная роль с необходимыми правами доступа к API.

8. В Azure Active Directory выберите **Приложения** в верхнем меню, а затем нажмите **Google Apps**.

	![Выбор Google Apps][23]

9. На странице быстрого запуска Google Apps нажмите кнопку **Настроить подготовку пользователей**.

	![Настройка подготовки пользователей][24]
...
##Шаг 4. Назначение пользователей Google Apps
