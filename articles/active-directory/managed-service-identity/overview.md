...
![Пример MSI виртуальной машины](../media/msi-vm-imds-example.png)

1. Azure Resource Manager получает сообщение, предписывающее включить управляемое удостоверение службы (MSI) на виртуальной машине.
...
3. Azure Resource Manager настраивает сведения субъекта-службы для виртуальной машины в службе метаданных экземпляров Azure на виртуальной машине. Этот этап включает в себя настройку идентификатора и сертификата клиента, которые используются для получения маркеров доступа из Azure AD. *Примечание. Конечная точка MSI IMDS заменяет текущую конечную точку расширения MSI для виртуальной машины. Дополнительные сведения об этом изменении см. на странице часто задаваемых вопросов и известных проблем.*
...
5. Код, запущенный на виртуальной машине, запрашивает маркер из конечной точки MSI службы метаданных экземпляров (IMDS) Azure, который доступен только внутри виртуальной машины: http://169.254.169.254/metadata/identity/oauth2/token. Параметр ресурса указывает службу, в которую отправляется маркер. Например, если нужно выполнить аутентификацию кода в Azure Resource Manager, используйте параметр resource=https://management.azure.com/.
6. Служба метаданных экземпляров Azure запрашивает маркера доступа из Azure AD, используя идентификатор и сертификат клиента для виртуальной машины. Azure AD возвращает маркер доступа JSON Web Token (JWT).
