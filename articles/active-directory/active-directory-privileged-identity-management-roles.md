...
> [!IMPORTANT]
> Корпорация Майкрософт рекомендует управлять с помощью Azure AD [Центр администрирования Azure AD](https://aad.portal.azure.com) на портале Azure.

Глобальный администратор может обновлять которой пользователи являются **окончательно** назначены роли в Azure AD, с помощью командлетов PowerShell, таких как `Add-MsolRoleMember` и `Remove-MsolRoleMember`, или на портале, как описано в [назначение роли администратора в Azure Active Directory](active-directory-assign-admin-roles-azure-portal.md).
...
Доступ к порталу Azure требуется пользователь быть администратором служб или соадминистратора в подписку Azure, даже если пользователь не требуется для управления подписками Azure.  Например для управления параметрами конфигурации для Azure AD, пользователь должен быть глобальным администратором в Azure AD и соадминистратор подписки, на подписку Azure.  Сведения о том, как добавлять пользователей в подписки Azure, см. в статье [Добавление или изменение ролей администратора Azure](../billing/billing-add-change-azure-subscription-administrator.md).
...
1. Войдите в [портал Azure](http://portal.azure.com) с учетной записью глобального администратора или учетной записи соадминистратора.
3. Выберите Azure AD и каталога, к которому требуется работа с и что есть лицензии, связанные с ним.
4. Выберите **лицензий** в левой части экрана. Отобразится список доступных лицензий.
