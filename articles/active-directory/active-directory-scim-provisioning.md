---
title: Автоматическая подготовка пользователей и групп из Azure Active Directory в приложениях с использованием SCIM | Microsoft Docs
...
![][1] *Рисунок. Перенос информации из Azure Active Directory в хранилище удостоверений через веб-службу*
...
## Подготовка пользователей и групп для приложений, поддерживающих SCIM
...
5. Если попытка соединения с приложением будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершение**, чтобы выйти из диалогового окна.
...
9. В разделе **Инструменты** щелкните **Перезапустить подготовку учетных записей**, чтобы запустить процесс подготовки.
...
## Создание собственного решения подготовки для любого приложения
...
1. На появившемся экране нажмите вторую кнопку **Настроить подготовку учетных записей**.
2. В диалоговом окне введите доступные в Интернете URL-адрес и порт конечной точки SCIM. Это будет строка типа http://testmachine.contoso.com:9000 или http://<ip-address>: 9000 /, где <ip-address> является IP-адресом, доступным в Интернете.
...
4. Если попытка соединения с веб-службой будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершение**, чтобы выйти из диалогового окна.
...
8. В разделе **Инструменты** щелкните **Перезапустить подготовку учетных записей**, чтобы запустить процесс подготовки.
...
**2.** Добавьте в этот метод приведенный ниже код. Для всех запросов в любую из конечных точек службы этот метод будет проверять, является ли отправителем запроса служба Azure Active Directory, выпустившая токен от имени клиента Azure Active Directory для доступа к веб-службе Azure AD Graph.
