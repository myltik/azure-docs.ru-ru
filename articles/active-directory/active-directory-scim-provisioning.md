...
3. Щелкните **Далее** и нажмите кнопку **Начать тест**. Azure Active Directory попробует подключиться к конечной точке SCIM. В случае ошибки вы увидите сведения диагностики.  
4. Если попытка соединения с веб-службой будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершить**, чтобы закрыть диалоговое окно.
5. На появившемся экране нажмите третью кнопку **Назначить учетные записи**. Вы увидите раздел пользователей и групп. Выберите здесь пользователей или группы, которых хотите назначить для приложения.
6. После назначения пользователей и групп откройте вкладку **Настройка** в верхней части экрана.
7. В разделе **Подготовка учетных записей**убедитесь, что параметр "Состояние" включен. 
8. В разделе **Инструменты** щелкните **Restart account provisioning** (Перезапустить подготовку учетных записей), чтобы запустить процесс подготовки.
