---
title: "Автоматизированная подготовка пользователей и групп из Azure Active Directory в приложениях с использованием SCIM | Документация Майкрософт"
...
---
# <a name="using-scim-to-enable-automatic-provisioning-of-users-and-groups-from-azure-active-directory-to-applications"></a>Автоматическая подготовка пользователей и групп из Azure Active Directory в приложениях с использованием SCIM
## <a name="overview"></a>Обзор
Azure Active Directory может выполнять автоматическую подготовку пользователей и групп для любого приложения или хранилища удостоверений, представляющего собой веб-службу с интерфейсом, определенным в [спецификации протокола SCIM 2.0](https://tools.ietf.org/html/draft-ietf-scim-api-19). Azure Active Directory будет отправлять запросы на создание, изменение и удаление пользователей и групп, назначенных для веб-службы. Эта служба на основании полученных запросов будет выполнять операции в целевом хранилище удостоверений. 

![][1]
*Рисунок. Перенос информации из Azure Active Directory в хранилище удостоверений через веб-службу*

Эта функция в сочетании с возможностью использовать сторонние приложения ([BYOA](http://blogs.technet.com/b/ad/archive/2015/06/17/bring-your-own-app-with-azure-ad-self-service-saml-configuration-gt-now-in-preview.aspx)) в Azure AD позволяет организовать режим единого входа и автоматической подготовки пользователей для приложений, которые предоставляют веб-службу SCIM или работают с ней.

Существует два варианта использования SCIM в Azure Active Directory.

* **Подготовка пользователей и групп в приложениях, поддерживающих SCIM**. Приложения, которые поддерживают SCIM 2.0 и используют токены носителя OAuth для аутентификации, будут работать с Azure AD без дополнительной настройки.
* **Сборка собственного решения подготовки для приложений, поддерживающих другую подготовку на основе API**. Для приложений, не поддерживающих SCIM, можно создать конечную точку SCIM для преобразования данных между конечной точкой SCIM Azure AD и любым API, поддерживаемым приложением для подготовки пользователей.  Чтобы помочь вам в разработке конечной точки SCIM, мы предоставляем библиотеки CLI, а также примеры кода, демонстрирующие подготовку конечной точки SCIM и преобразование сообщений SCIM.  
...
В Azure Active Directory можно настроить автоматическую подготовку назначенных пользователей и групп для приложений, реализующих веб-службу [SCIM 2](https://tools.ietf.org/html/draft-ietf-scim-api-19) и принимающих токены носителя OAuth для проверки подлинности. В спецификации SCIM 2.0 приложения должны удовлетворять следующим требованиям.
...
Описанные выше приложения, поддерживающие профиль SCIM, можно подключать к Azure Active Directory с помощью функции "пользовательского" приложения в коллекции приложений Azure AD. После подключения Azure AD запускает процесс синхронизации каждые 5 минут, запрашивая конечную точку SCIM приложения для получения назначенных пользователей и групп, и создает или изменяет их согласно сведениям о назначении.

**Подключение приложения, поддерживающего SCIM:**

1. В веб-браузере откройте классический портал управления Azure: https://manage.windowsazure.com.
...
3. Слева выберите вкладку **Пользовательское**, введите произвольное имя приложения и щелкните значок галочки. Будет создан объект приложения.

![][2]

1. На появившемся экране нажмите вторую кнопку **Настроить подготовку учетных записей** .
2. В поле **URL-адрес конечной точки подготовки** введите URL-адрес конечной точки приложения SCIM.
3. Если конечной точке SCIM требуется токен носителя OAuth от издателя, отличного от Azure AD, скопируйте необходимый токен носителя OAuth в поле **Токен проверки подлинности (необязательно)** . Если оставить это поле пустым, то Azure AD будет включать токен носителя OAuth, выданный Azure AD, в каждый запрос. Приложения, использующие Azure AD в качестве поставщика удостоверений, могут проверить этот выданный Azure AD токен.
4. Щелкните **Далее** и нажмите кнопку **Начать тест**. Azure Active Directory попробует подключиться к конечной точке SCIM. В случае ошибки вы увидите сведения диагностики.  
5. Если попытка соединения с приложением будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершить**, чтобы закрыть диалоговое окно.
6. На появившемся экране нажмите третью кнопку **Назначить учетные записи**. Вы увидите раздел пользователей и групп. Выберите здесь пользователей или группы, которых хотите назначить для приложения.
7. После назначения пользователей и групп откройте вкладку **Настройка** в верхней части экрана.
8. В разделе **Подготовка учетных записей**убедитесь, что параметр "Состояние" включен. 
9. В разделе **Инструменты** щелкните **Restart account provisioning** (Перезапустить подготовку учетных записей), чтобы запустить процесс подготовки.
...
4. Последовательно щелкните пункты **Инструменты > Диспетчер пакетов библиотеки > Консоль диспетчера пакетов** и выполните приведенные ниже команды, чтобы разрешить ссылки в проекте FileProvisioningAgent.
...
2. В диалоговом окне введите доступные в Интернете URL-адрес и порт конечной точки SCIM. Это будет строка типа http://testmachine.contoso.com:9000 или http://<IP-адрес>:9000 /, где <IP-адрес> является IP-адресом, доступным в Интернете.  
5. Щелкните **Далее** и нажмите кнопку **Начать тест**. Azure Active Directory попробует подключиться к конечной точке SCIM. В случае ошибки вы увидите сведения диагностики.  
6. Если попытка соединения с веб-службой будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершить**, чтобы закрыть диалоговое окно.
7. На появившемся экране нажмите третью кнопку **Назначить учетные записи**. Вы увидите раздел пользователей и групп. Выберите здесь пользователей или группы, которых хотите назначить для приложения.
8. После назначения пользователей и групп откройте вкладку **Настройка** в верхней части экрана.
9. В разделе **Подготовка учетных записей**убедитесь, что параметр "Состояние" включен. 
10. В разделе **Инструменты** щелкните **Restart account provisioning** (Перезапустить подготовку учетных записей), чтобы запустить процесс подготовки.
...
**2.** [Обработчики Express Route](http://expressjs.com/guide/routing.html) используются для синтаксического анализа объектов запроса Node.js, которые представляют собой вызовы (как определено в спецификации SCIM), направленные к веб-службе Node.js.   
...
### <a name="handling-endpoint-authentication"></a>Обработка проверки подлинности на конечной точке
Запросы от Azure Active Directory содержат токен носителя OAuth 2.0.   Любая веб-служба, которая получит запрос, должна убедиться, что токен выдала служба Azure Active Directory от имени клиента Azure Active Directory для доступа к веб-службе Azure AD Graph.  В токене издатель обозначается утверждением iss, которое имеет следующий вид: iss:https://sts.windows.net/cbb1a5ac-f33b-45fa-9bf5-f37db0fed422/.  В этом примере базовый адрес утверждения со значением https://sts.windows.net подтверждает, что издателем является Azure Active Directory. Относительный адрес сегмента (cbb1a5ac-f33b-45fa-9bf5-f37db0fed422) — это уникальный идентификатор клиента Azure Active Directory, от имени которого выдан маркер.  Если токен выпущен для доступа к веб-службе Azure Active Directory Graph, утверждение adu такого токена должно иметь значение 00000002-0000-0000-c000-000000000000, то есть значение идентификатора этой веб-службы.  
...
**2.** Добавьте в этот метод приведенный ниже код. Для всех запросов к любой из конечных точек службы этот метод будет проверять, является ли отправителем запроса служба Azure Active Directory, выпустившая маркер от имени клиента Azure Active Directory для доступа к веб-службе Azure AD Graph. 
...
![][4]
*Рисунок. Последовательность действий при подготовке и отзыве пользователя*

**1.** Azure Active Directory запросит службу того пользователя, у которого значение атрибутов externalId совпадает со значениями атрибутов mailNickname пользователя Azure Active Directory.  Запрос будет выражен в формате HTTP-запроса, пример которого приведен ниже. Значение jyoung в этом запросе обозначает идентификатор mailNickname пользователя Azure Active Directory. 
...
**3.** Для обновления пользователя, уже существующего в хранилище удостоверений с интерфейсом службы SCIM, Azure Active Directory запросит у службы текущее состояние этого пользователя, сформировав запрос следующего вида. 
...
**6.** Для отзыва пользователя из хранилища удостоверений, интерфейсом для которого является служба SCIM, Azure Active Directory направит запрос следующего вида. 
...
* Схема ресурса группы будет идентифицирована как http://schemas.microsoft.com/2006/11/ResourceManagement/ADSCIM/Group.  
...
![][5]
*Рисунок. Последовательность действий при подготовке и отзыве группы*
