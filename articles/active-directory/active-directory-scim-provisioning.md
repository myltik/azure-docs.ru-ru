...
5.	В поле **URL-адрес конечной точки подготовки** введите URL-адрес конечной точки приложения SCIM.
6.	Если конечной точке SCIM требуется токен носителя OAuth от издателя, отличного от Azure AD, скопируйте необходимый токен носителя OAuth в поле **Токен проверки подлинности (необязательно)**. Если оставить это поле пустым, то Azure AD будет включать токен носителя OAuth, выданный Azure AD, в каждый запрос. Приложения, использующие Azure AD в качестве поставщика удостоверений, могут проверить этот выданный Azure AD токен.
7.	Щелкните **Далее** и нажмите кнопку **Начать тест**. Azure Active Directory попробует подключиться к конечной точке SCIM. В случае ошибки вы увидите сведения диагностики.  
8.	Если попытка соединения с приложением будет успешной, нажмите кнопку **Далее** на каждом из следующих экранов, а затем — кнопку **Завершение**, чтобы выйти из диалогового окна.
9.	На появившемся экране нажмите третью кнопку **Назначить учетные записи**. Вы увидите раздел пользователей и групп. Выберите здесь пользователей или группы, которых хотите назначить для приложения.
10.	После назначения пользователей и групп откройте вкладку **Настройка** в верхней части экрана.
11.	В разделе **Подготовка учетных записей** убедитесь, что параметр "Состояние" включен. 
12.	В разделе **Инструменты** щелкните **Перезапустить подготовку учетных записей**, чтобы запустить процесс подготовки.
...
11.	В разделе **Инструменты** щелкните **Перезапустить подготовку учетных записей**, чтобы запустить процесс подготовки.
