...
Что нужно для установки Azure AD Connect вместо DirSync:
...
Для обновления на месте выполните указанные ниже действия высокого уровня.
...
3. По завершении анализа мы предоставим рекомендации по дальнейшим действиям. В этом сценарии (меньше 50 000 объектов, использующих SQL Express) отобразится указанный ниже экран.
...
6. Теперь все готово для настройки. Нажмите кнопку "Далее", чтобы удалить DirSync и начать настройку и синхронизацию Azure AD Connect.  
...
В этом случае рекомендуется рассмотреть возможность параллельного обновления на отдельном сервере. Это связано с тем, что в зависимости от размера организации обновление на месте может повлиять на соглашения об уровне обслуживания вашей компании, касающиеся скорости отражения изменений локальной службы Active Directory в Azure AD и Office 365. Мы пытаемся оценить, сколько времени займет первая синхронизация с помощью Azure AD Connect. Как упоминалось выше, скорее всего, ваш собственный опыт исходной установки или обновления DirSync служит лучшим показателем.
...
При наличии более 50 000 объектов на шаге 3 программа установки Azure AD Connect порекомендует параллельное развертывание. Сведения о выборе развертывания на месте или параллельного развертывания см. в разделе "Обновление на месте — более 50 000 объектов" выше. Появится экран, показанный ниже.
...
2. При отображении экрана "Вас приветствует Azure AD Connect" выйдите из мастера, нажав кнопку c крестиком в правом верхнем углу окна.
...
Azure AD Connect выполнит подключение и отобразит указанный ниже пользовательский интерфейс.
...
[AZURE.NOTE]Мы снимаем флажок запуска синхронизации, чтобы служба DirSync (которая по-прежнему установлена и работает) и Azure AD Connect не пытались записывать данные в Azure AD одновременно.

### Убедитесь, что Azure AD Connect готова к синхронизации.

Чтобы определить, готова ли служба Azure AD Connect наследовать настройки DirSync, необходимо открыть диспетчер службы синхронизации Azure AD Connect. Чтобы найти это приложение, в меню "Пуск" выполните поиск по слову "синхронизация".
...
При повторном открытии Azure AD Connect после установки будет запущен мастер конфигурации. Откройте Azure AD Connect.
...
Поздравляем! Вы успешно выполнили миграцию на Azure AD Connect с помощью параллельного развертывания.

## Вспомогательные компоненты Azure AD Connect

Ниже перечислены необходимые и вспомогательные компоненты, которые Azure AD Connect установит на сервере, на котором вы установили эту службу. Этот список предназначен для базовой установки Express. Если на странице "Установить службы синхронизации" вы выбрали другой выпуск SQL Server, то перечисленные ниже компоненты SQL Server 2012 не будут установлены.
