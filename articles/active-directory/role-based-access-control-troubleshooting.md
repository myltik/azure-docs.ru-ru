...
[Управление доступом на основе ролей](../role-based-access-control-configure.md) — эффективная функция, позволяющая делегировать доступ к ресурсам Azure на детальном уровне. Это означает, что можно предоставить конкретному человеку только те права, которые ему действительно нужны. Однако в некоторых случаях модель ресурсов Azure может быть весьма сложной, так что становится нелегко понять, к чему именно вы предоставляете доступ.

Из этого документа вы узнаете, к чему нужно быть готовым при использовании некоторых ролей на классическом портале Azure. Существуют три основных роли, которые охватывают все типы ресурсов: роль владельца, роль участника и роль читателя

Владельцы и участники обладают полным доступом к действиям по управлению. Различие между этими ролями в том, что участник не может предоставлять доступ другим пользователям или группам. С ролью читателя не все так просто, поэтому мы уделим ей больше внимания. Подробные сведения о предоставлении доступа см. в [этой статье](../role-based-access-control-configure.md).

## Рабочие нагрузки службы приложений

### Доступ только для чтения

Если вы предоставили пользователю права на чтение одного веб-приложения или сами имеете только эти права, то вам могут быть недоступны некоторые возможности. Перечисленные ниже возможности управления требуют доступа к веб-приложению с правом **записи** (роль участника или владельца), поэтому они будут недоступны при наличии прав только на чтение.
...
Если у вас нет доступа ни к одной из этих плиток, вам потребуется доступ к веб-приложению с правами участника.
...
С виртуальными машинами связаны следующие ресурсы: доменные имена, виртуальные сети, учетные записи хранилища и правила генерации оповещений.
