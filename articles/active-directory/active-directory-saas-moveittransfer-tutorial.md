...
12. На странице **Edit Federated Identity Provider User Settings** (Изменение параметров федеративного поставщика удостоверений) выполните следующие действия и нажмите кнопку **Save** (Сохранить).
    ...
    b. Выберите **Other** (Другое) для параметра **Full name** (Полное имя). А в текстовом поле **Attribute name** (Имя атрибута) введите значение http://schemas.microsoft.com/identity/claims/displayname.
    
    c. Выберите **Other** (Другое) для параметра **Email** (Адрес электронной почты). А в текстовом поле **Attribute name** (Имя атрибута) введите значение http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress.
...
### <a name="creating-a-moveit-transfer-test-user"></a>Создание тестового пользователя MOVEit Transfer
