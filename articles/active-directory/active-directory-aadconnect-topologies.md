...
![Один лес, один клиент](./media/active-directory-aadconnect-topologies/SingleForestSingleDirectory.png)

Стандартная топология — один локальный лес с одним или несколькими доменами и один клиент Azure AD. Для проверки подлинности Azure AD используется синхронизация паролей. Экспресс-установка Azure AD Connect поддерживает только эту топологию.
...
4. Если у пользователя есть связанный почтовый ящик, то есть и другая учетная запись в другом лесу, которая используется для входа.
...
К одному клиенту Azure AD нельзя подключить более одного сервера синхронизации Azure AD Connect. В порядке исключения можно использовать [промежуточный сервер](#staging-server).
...
На этом рисунке каждый объект в каждом лесу представлен в метавселенной в единственном экземпляре с объединением в целевом клиенте Azure AD.
...
В топологии леса ресурсов учетных записей есть один или несколько лесов учетных записей с активными учетными записями пользователей. Может также существовать один (или несколько) лес ресурсов с отключенными учетными записями.
...
Корпорация Майкрософт рекомендует использовать один клиент в Azure AD для всей организации.
Если вы намерены создать несколько клиентов Azure AD, сначала прочитайте следующие статьи. В них рассматриваются типичные сценарии, которые позволят использовать один клиент.

| Раздел |
| --- | --- |
| Делегирование при помощи административных единиц |

![Несколько лесов, несколько клиентов](./media/active-directory-aadconnect-topologies/MultiForestMultiDirectory.png)

Между сервером синхронизации Azure AD Connect и клиентом Azure AD устанавливается отношение 1:1. Для каждого клиента Azure AD потребуется установить отдельный сервер синхронизации Azure AD Connect. Экземпляры клиента Azure AD изначально изолированы, то есть пользователи одного клиента не будут видеть пользователей другого клиента. Если так и должно быть, то такая конфигурация поддерживается. В противном случае следует использовать модель с одним клиентом Azure AD.
...
* Гибридное развертывание Exchange с локальным каталогом Active Directory можно включить только для одного клиента Azure AD.
...
![Один лес, несколько клиентов, не поддерживается](./media/active-directory-aadconnect-topologies/SingleForestMultiDirectoryUnsupported.png) ![Один лес, несколько соединителей, не поддерживается](./media/active-directory-aadconnect-topologies/SingleForestMultiConnectorsUnsupported.png)

* Синхронизация одного пользователя с несколькими клиентами Azure AD не поддерживается.
* Нельзя изменить конфигурацию таким образом, чтобы пользователи одного клиента Azure AD отображались в другом клиенте Azure AD как контакты.
* Нельзя изменить настройки службы синхронизации Azure AD Connect для подключения к нескольким клиентам Azure AD.
...
Клиенты Azure AD являются изолированными по своей природе.

* Нельзя настроить службу синхронизации Azure AD Connect для чтения данных из другого клиента Azure AD.
...
Для синхронизации пользователей между двумя организациями Exchange с помощью GALsync можно локально использовать FIM2010 или MIM2016. Пользователи из одной организации будут отображаться в другой организации как внешние пользователи или контакты. Затем эти локальные каталоги AD можно синхронизировать с их собственными клиентами Azure AD.

## <a name="next-steps"></a>Дальнейшие действия
Больше об установке Azure AD Connect для этих сценариев можно узнать в статье [Выборочная установка Azure AD Connect](connect/active-directory-aadconnect-get-started-custom.md).
