...
- Политики сложности паролей, настроенные в локальном каталоге Active Directory, переопределяют политики сложности, настроенные в облаке для синхронизированных пользователей. 

- Все допустимые пароли из локального Active Directory можно использовать для доступа к службам Azure AD.
...
Если пользователь входит в область синхронизации паролей:

- Для пароля облачной учетной записи устанавливается значение "*Никогда не истекает*". 

- Синхронизированный пароль, срок действия которого в локальной среде истек, можно использовать для входа в облачные службы.
...
Если выбран параметр **Федерация с AD FS**, можно включить синхронизацию паролей на случай сбоя в инфраструктуре AD FS. Его также можно включить, если вы собираетесь использовать доменные службы Azure AD.

### Синхронизация паролей и FIPS

Если сервер заблокирован в соответствии с FIPS (федеральный стандарт обработки информации), схема MD5 отключена. Чтобы включить MD5 для синхронизации паролей, добавьте ключ enforceFIPSPolicy в файл конфигурации miiserver.exe.config, который хранится в папке в C:\\Program Files\\Azure AD Sync\\bin.
...
7. Также следует [отследить пользователя](active-directory-aadconnectsync-service-manager-ui-connectors.md#follow-an-object-and-its-data-through-the-system) до пространства соединителя Azure AD через метавселенную. Объект пространства соединителя должен содержать правило исходящих подключений, в котором параметр **Синхронизация паролей** имеет значение **True**. В конфигурации по умолчанию это будет правило синхронизации с именем **Out to AAD - User Join**.
...
Как правило, нет необходимости выполнять полную синхронизацию всех паролей. Однако если потребуется, ее можно активировать с помощью следующего сценария:
