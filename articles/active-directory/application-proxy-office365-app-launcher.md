...
В этой статье описывается настройка приложений для направления пользователей на пользовательскую домашнюю страницу. При публикации приложения с помощью прокси приложения задается внутренний URL-адрес, но иногда он не является той страницей, на которую пользователи должны попадать в первую очередь. Настройте пользовательскую домашнюю страницу, на которую будут попадать пользователи при открытии приложений из панели доступа Azure Active Directory или из средства запуска приложений Office 365.
...
Например:
- В корпоративной сети пользователи переходят на страницу *https://ExpenseApp/login/login.aspx*, чтобы выполнить вход и открыть приложение.
- Так как у вас есть другие ресурсы, например изображения, которые требуются прокси приложения для доступа на верхнем уровне структуры папок, при публикации приложения в качестве внутреннего URL-адреса используется *https://ExpenseApp*.
- Внешний URL-адрес по умолчанию — *https://ExpenseApp-contoso.msappproxy.net*, по которому пользователи не переходят на страницу входа.  
- Задайте адрес *https://ExpenseApp-contoso.msappproxy.net/login/login.aspx* в качестве URL-адреса домашней страницы, чтобы пользователи могли бесперебойно работать с приложениями. 
...
1. Убедитесь, что вы нашли правильное приложение, затем замените значение *8af89bfa-eac6-40b0-8a13-c2c4e3ee22a4* значением ObjectID, которое вы скопировали на предыдущем шаге.
...
>[!NOTE]
>Все изменения, которые вы внесли в приложение, могут сбросить URL-адрес домашней страницы. Если URL-адрес домашней страницы сбрасывается, то повторите шаг 2.
