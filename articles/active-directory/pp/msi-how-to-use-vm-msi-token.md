...
| 401 — недостаточно прав | unknown_source | Неизвестный *\<URI\>* источника | Убедитесь, что универсальный код ресурса (URI) запроса HTTP GET имеет правильный формат. Часть `scheme:host/resource-path` должна быть указана как `http://localhost:50342/oauth2/token`. См. пример запроса в предыдущем разделе [Получение маркера с использованием HTTP](#get-a-token-using-http).|
