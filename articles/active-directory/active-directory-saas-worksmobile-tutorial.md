...
3. В разделе **Домены и URL-адреса приложения WORKS MOBILE** сделайте следующее:
    ...
    > [!NOTE] 
    > Это значение приведено для справки. Вместо него необходимо указать фактический URL-адрес входа. Чтобы получить это значение, обратитесь в [службу поддержки клиентов WORKS MOBILE](mailto:dl_ssoinfo@worksmobile.com). 
