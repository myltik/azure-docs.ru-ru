...
> [AZURE.SELECTOR]
- [Портал Azure](active-directory-domains-add-qzure-portal.md)
- [Классический портал Azure](active-directory-add-domain.md)
...
> [AZURE.NOTE] Если вы собираетесь настроить имя личного домена для использования в службах федерации Active Directory (AD FS) или другой службе маркеров безопасности в корпоративной сети, следуйте инструкциям в статье [Add and configure a domain for federation with Azure Active Directory](active-directory-add-domain-federated.md) (Добавление и настройка домена федерации с использованием Azure Active Directory). Это целесообразно, если вы планируете синхронизировать учетные записи пользователей из каталога организации с Azure AD, а [синхронизация хэша паролей](active-directory-aadconnectsync-implement-password-synchronization.md) не соответствует вашим требованиям.
...
3. На панели команд нажмите **Добавить**. Введите имя пользовательского домена, например contoso.com. Обязательно включите расширение COM, NET или другое расширение верхнего уровня и оставьте флажок единого входа (федеративного) не установленным.
