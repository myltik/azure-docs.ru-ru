...
## <a name="set-up-the-rest-api"></a>Настройка REST API
Пока мы все настраиваем, давайте обеспечим работу серверной части REST API.  В корневой папке проекта откройте `web.config` и замените значение `audience`.  Интерфейс API REST будет использовать это значение для проверки токенов, получаемых от приложения Angular по запросам AJAX.
