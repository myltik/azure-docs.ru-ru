...
| `groups`                   | Группы, участником которых является пользователь.                                                                                                                                                               | JWT, SAML  |           | Дополнительные свойства: <br> Sam_account_name<br> Dns_domain_and_sam_account_name<br> Netbios_domain_and_sam_account<br> Max_size_limit<br> Emit_as_roles<br>                                                                                                                            |
...
Некоторые необязательные утверждения можно настроить так, чтобы изменить способ возврата утверждения.  Эти дополнительные свойства варьируются от изменений форматирования (например, `include_externally_authenticated_upn_without_hash`) до изменения набора возвращаемых данных (`Dns_domain_and_sam_account_name`).
...
| | `max_size_limit`                                    | Повышает количество возвращаемых групп до максимального размера группы (1000).                                                            |             
| | `emit_as_roles`                                     | Выводит утверждение "роли" вместо утверждения "группы" с теми же значениями.  Предназначено для миграции приложений из локальной среды, в которой RBAC традиционно контролируется через членство в группах.   |             
...
>
>Дополнительные свойства `account_name` для групп несовместимы, поэтому упорядочение дополнительных свойств имеет значение. Будет использоваться только первое дополнительное свойство имени учетной записи. 
...
Этот объект OptionalClaims вернет то же утверждение `groups`, как если бы `sam_account_name` не было включено. Так как он находится `netbios_domain_and_sam_account_name`, оно не обрабатывается. 
