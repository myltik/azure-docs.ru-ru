---
title: "Смене ключей подписывания в Azure AD"
...
Если проверка подлинности добавлена в решение вручную, в приложении может отсутствовать необходимая логика смены ключей. Необходимо будет написать самостоятельно или следовать инструкциям в [веб-приложений и API-интерфейсы используются любые другие библиотеки или вручную реализации любого из поддерживаемых протоколов](#other).
...
3. Выполните сборку приложения и запустите его. Если у вас получилось войти, приложение успешно обновляет ключ, скачивая необходимые данные из документа метаданных федерации каталога. Если возникли проблемы со входом, убедитесь, изменения в приложении верны, считывая [добавления единого входа для вашей веб с помощью приложения Azure AD](https://github.com/Azure-Samples/active-directory-dotnet-webapp-openidconnect) статьи, а также скачивание и проверка в следующем образце кода: [ Многопользовательского облачного приложения для Azure Active Directory](https://code.msdn.microsoft.com/multi-tenant-cloud-8015b84b).
...
Если ключ хранятся в другом или жестко задано в приложении, можно вручную извлечь ключ и обновите его соответствующим образом путем выполнения вручную продолжения согласно инструкциям, приведенным в конце этого документа рекомендации. **Настоятельно рекомендуется усилить приложению поддерживать автоматическую смену** одним из подходов структуры в этой статье во избежание будущих нарушений в работе и служебных данных, если Azure AD увеличивает его периодичности переключения или имеет возникло экстренной ситуации. продолжения по каналу.
...
## <a name="how-to-perform-a-manual-rollover-if-your-application-does-not-support-automatic-rollover"></a>Как выполнять вручную продолжения, если приложение не поддерживает автоматическую смену
