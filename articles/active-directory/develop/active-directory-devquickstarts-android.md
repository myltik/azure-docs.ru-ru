...
> [!TIP]
> Воспользуйтесь предварительной версией нашего нового [портала разработчиков](https://identity.microsoft.com/Docs/Android) , который поможет вам приступить к работе с Azure Active Directory через несколько минут.  Портал разработчиков поможет зарегистрировать приложение и интегрировать Azure AD в коде.  Завершив работу, вы получите простое приложение, с помощью которого выполняется аутентификация пользователей в клиенте и на сервере, принимающем маркеры и проводящем проверку. 
> 
> 

## <a name="step-1-download-and-run-the-nodejs-rest-api-todo-sample-server"></a>Этап 1. Загрузка и запуск образца службы, реализующей интерфейс REST API приложения To Do List для Node.js
...
## <a name="step-2-register-your-web-api-with-your-microsoft-azure-ad-tenant"></a>Этап 2. Регистрация веб-API с помощью клиента Microsoft Azure AD
**Что я делаю?**
...
6. В качестве URL-адреса единого входа введите базовый URL-адрес образца, который по умолчанию имеет значение `https://localhost:8080`.
7. В качестве идентификатора URI приложения введите `https://<your_tenant_name>/TodoListService`, заменив `<your_tenant_name>` на имя вашего клиента Azure AD.  Для завершения регистрации нажмите кнопку "OK".
8. На портале Azure перейдите на страницу приложения, найдите значение идентификатора приложения и скопируйте его. Оно потребуется позднее при настройке приложения.
...
7. На странице приложения найдите значение идентификатора приложения и скопируйте его. Оно потребуется позднее при настройке приложения.
...
### <a name="step-4-download-the-android-adal-and-add-it-to-your-eclipse-workspace"></a>Этап 4. Загрузка библиотеки ADAL для Android и добавление ее в рабочую область платформы Eclipse
...
#### <a name="option-5-jar-package-inside-libs-folder"></a>Способ 5: JAR-пакет в папке libs
...
1. Добавьте в проект ссылку на библиотеку для Android. Если вы затрудняетесь это сделать, [щелкните здесь для получения дополнительных сведений](http://developer.android.com/tools/projects/projects-eclipse.html)
...
4. Создайте экземпляр класса AuthenticationContext с вашим MainActivity. Описание этого вызова выходят за рамки данной статьи, но вы можете получить дополнительные сведения в разделе [Образец клиентского нативного приложения для Android](https://github.com/AzureADSamples/NativeClient-Android). Ниже приведен пример, где SharedPreferences используется как кэш по умолчанию и права представлены в виде `https://login.windows.net/yourtenant.onmicrosoft.com`:
...
1. **Broker**. Компонент broker предоставляется приложением корпоративного портала Microsoft Intune. Библиотека ADAL будет использовать учетную запись broker, если существует одноименная пользовательская учетная запись, созданная в этой структуре проверки подлинности, и разработчик решил не игнорировать ее. Разработчик может игнорировать (пропустить) пользователя broker с помощью следующего вызова:
...
### <a name="authority-url-and-adfs"></a>URL-адрес центра и служба федерации Active Directory
...
### <a name="ntlm-dialog"></a>Диалоговое окно NTLM
...
См. раздел [Как включить единый вход для нескольких приложений Android с помощью ADAL](active-directory-sso-android.md).  
