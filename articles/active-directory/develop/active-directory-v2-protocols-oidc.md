...
В настоящее время конечная точка версии&2;.0 не поддерживает OpenID Connect `end_session_endpoint`. Это значит, что приложение не может отправлять запросы к конечной точке версии&2;.0 для завершения сеанса пользователя и очистки файлов cookie, установленных конечной точкой версии&2;.0.
Для выполнения выхода пользователя приложение может просто завершить свой сеанс пользователя, не затрагивая сеанс пользователя для конечной точки версии&2;.0. При следующей попытке входа пользователь увидит страницу выбора учетной записи с перечнем учетных записей, с помощью был выполнен вход. На этой странице он может выйти из любой учетной записи, тем самым завершая сеанс с конечной точкой версии&2;.0.
...
Конечная точка версии&2;.0 использует файлы cookie для идентификации сеанса пользователя. При первом входе пользователя в приложение конечная точка версии&2;.0 настраивает файл cookie в браузере пользователя. При последующем входе пользователя в другое приложение Azure AD сначала проверяет файл cookie, чтобы определить, открыт ли у пользователя действительный сеанс входа на конечную точку Azure AD версии&2;.0, вместо того, чтобы повторно аутентифицировать этого пользователя.

Точно так же при первом выходе пользователя из приложения конечная точка версии&2;.0 удаляет файл cookie из браузера. Тем не менее пользователь может оставаться вошедшим в другие приложения, использующие конечную точку Azure AD версии&2;.0 для аутентификации. Чтобы обеспечить выход пользователя из всех приложений, конечная точка версии&2;.0 отправляет HTTP-запрос GET к `LogoutUrl` для всех приложений, в которые в настоящее время вошел пользователь. Приложения должны ответить на него, удалив все файлы cookie, которые идентифицируют сеанс пользователя. `LogoutUrl` можно настроить на портале Azure.
