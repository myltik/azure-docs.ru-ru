...
> [!TIP]
> Чтобы выполнить этот запрос, щелкните приведенную ниже ссылку. После входа в систему в браузере выполняется переход по адресу https://localhost/myapp/ с указанием маркера идентификации и кода в адресной строке. Обратите внимание, что в этом запросе используется `response_mode=query` (исключительно в демонстрационных целях). Мы рекомендуем использовать `response_mode=form_post`.
