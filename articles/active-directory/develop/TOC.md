# Обзор
## [Интеграция с Azure AD](active-directory-how-to-integrate.md)
## [Сценарии проверки подлинности](active-directory-authentication-scenarios.md)
## [Graph API](active-directory-graph-api.md)
## [Библиотеки аутентификации](active-directory-authentication-libraries.md)
## [Протоколы аутентификации](active-directory-authentication-protocols.md)
## [Объекты приложений и субъектов-служб](active-directory-application-objects.md)
## [Руководство разработчика](active-directory-developers-guide.md)
## [Неявное разрешение OAuth2](active-directory-dev-understanding-oauth2-implicit-grant.md)
## [Приложения в Azure AD](active-directory-how-applications-are-added.md)

# Приступая к работе
## Приложения для мобильных устройств и ПК
### [iOS](active-directory-devquickstarts-ios.md)
### [Android](active-directory-devquickstarts-android.md)
### [Windows Phone](active-directory-devquickstarts-windowsphone.md)
### [.Net](active-directory-devquickstarts-dotnet.md)
### [Магазин Windows](active-directory-devquickstarts-windowsstore.md)
### [Xamarin](active-directory-devquickstarts-xamarin.md)
### [Cordova](active-directory-devquickstarts-cordova.md)
### [OAuth](active-directory-protocols-oauth-code.md)
## веб-приложений:
### [.NET](active-directory-devquickstarts-webapp-dotnet.md)
### [Java](active-directory-devquickstarts-webapp-java.md)
### [AngularJS](active-directory-devquickstarts-angular.md)
### [NodeJS](active-directory-devquickstarts-openidconnect-nodejs.md)
### [OpenID Connect](active-directory-protocols-openid-connect-code.md)
### [Командная строка Java](active-directory-devquickstarts-headless-java.md)
## Веб-интерфейс API
### [Веб-API и .NET](active-directory-devquickstarts-webapi-dotnet.md)
### [NodeJS](active-directory-devquickstarts-webapi-nodejs.md)
## [Graph API](active-directory-graph-api-quickstart.md)

# Практическое руководство
## [Получение клиента](active-directory-howto-tenant.md)
## [Вход любого пользователя Azure AD](active-directory-devhowto-multi-tenant-overview.md)
## [Интеграция приложений](active-directory-integrating-applications.md)
## Включение единого входа
### [Android](active-directory-sso-android.md)
### [iOS](active-directory-sso-ios.md)
## [Получение сертификата AppSource](active-directory-devhowto-appsource-certified.md)
## [Добавление приложения в коллекцию](active-directory-app-gallery-listing.md)
## [Интерпретация манифеста приложения](active-directory-application-manifest.md)
## [Аутентификация веб-пользователей](active-directory-java-authenticate-users-access-control-eclipse.md)
## Работа с SAML
### [Просмотр данных SAML, возвращенных службой контроля доступа](active-directory-java-view-saml-returned-by-access-control.md)
### [Azure AD и SAML](active-directory-saml-protocol-reference.md)
### [Настройка утверждений](active-directory-saml-claims-customization.md)
### [Отладка единого входа на основе SAML](active-directory-saml-debugging.md)
## [Использование смены ключей подписывания](active-directory-signing-key-rollover.md)


## Использование конечной точки версии 2
### [Обзор](active-directory-appmodel-v2-overview.md)
### [В чем отличия версии 2?](active-directory-v2-compare.md)
### [Библиотеки аутентификации](active-directory-v2-libraries.md)
### [Типы приложений](active-directory-v2-flows.md)
### [Ограничения](active-directory-v2-limitations.md)
### [Область](active-directory-v2-scopes.md)
### [Портал регистрации](active-directory-v2-registration-portal.md)
### [Справочник по маркерам](active-directory-v2-tokens.md)
### Протоколы
#### [Учетные данные клиента OAuth 2.0](active-directory-v2-protocols-oauth-client-creds.md)
#### [OAuth 2.0 и OpenID](active-directory-v2-protocols.md)
#### [Последние обновления](active-directory-v2-preview-oidc-changes.md)
### Мобильное приложение
#### [iOS](active-directory-v2-devquickstarts-ios.md)
#### [Windows Phone](active-directory-v2-devquickstarts-wpf.md)
#### [Android](active-directory-v2-devquickstarts-android.md)
### Веб-приложения и API
#### [Использование AngularJS с NodeJS](active-directory-v2-devquickstarts-angular-node.md)
#### [Использование AngularJS с .NET](active-directory-v2-devquickstarts-angular-dotnet.md)
#### [Веб-приложение .NET](active-directory-v2-devquickstarts-dotnet-web.md)
#### [Веб-приложение NodeJS](active-directory-v2-devquickstarts-node-web.md)
#### [API .NET](active-directory-v2-devquickstarts-dotnet-api.md)
#### [API NodeJS](active-directory-v2-devquickstarts-node-api.md)
#### [Вызов веб-API](active-directory-v2-devquickstarts-webapp-webapi-dotnet.md)
### Прямая интеграция с протоколами
#### [Регистрация приложения](active-directory-v2-app-registration.md)
#### [OpenID Connect](active-directory-v2-protocols-oidc.md)
#### [Неявный поток](active-directory-v2-protocols-implicit.md)
#### [Код авторизации OAuth 2.0](active-directory-v2-protocols-oauth-code.md)
#### [Учетные данные клиента OAuth 2.0](active-directory-v2-protocols-oauth-client-creds.md)
#### [Вызовы между службами](active-directory-protocols-oauth-service-to-service.md)

## Работа с Visual Studio
### [Приступая к работе с .NET](vs-active-directory-dotnet-getting-started.md)
### [Что произошло с моим проектом MVC?](vs-active-directory-dotnet-what-happened.md)
### [Ошибка при обнаружении аутентификации](vs-active-directory-error.md)
### [Приступая к работе с WebApi](vs-active-directory-webapi-getting-started.md)
### [Что произошло с моим проектом WebApi?](vs-active-directory-webapi-what-happened.md)


# Справочные материалы
## [Глоссарий](active-directory-dev-glossary.md)
## [Рекомендации по фирменной символике](active-directory-branding-guidelines.md)
## [Примеры кода](active-directory-code-samples.md)
## [Метаданные федерации](active-directory-federation-metadata.md)
## [Единый вход](active-directory-single-sign-on-protocol-reference.md)
## [Единый выход](active-directory-single-sign-out-protocol-reference.md)
## [Маркеры и утверждения](active-directory-token-and-claims.md)


<!--HONumber=Jan17_HO3-->


