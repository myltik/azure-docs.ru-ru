...
Политики условного доступа могут применяться к приложению, а также к веб-API, к которому получает доступ ваше приложение. Дополнительные сведения о том, как настроить политику условного доступа, см. в разделе [Настройка правил доступа для каждого приложения](../active-directory-conditional-access-azuread-connected-apps.md#configure-per-application-access-rules).
...
* Создается мультитенатное веб-приложение, которое использует Microsoft Graph для доступа к Exchange.  Корпоративный клиент, использующий это приложение, устанавливает политику для SharePoint Online.  Когда веб-приложение запрашивает маркер для Microsoft Graph, применяется политика для любой службы Майкрософт (в частности для служб, к которым можно получить доступ через граф).  Пользователю предлагается пройти многофакторную проверку подлинности. Если пользователь войдет с помощью действительных маркеров, в веб-приложение возвращается запрос утверждений.  
* Создается собственное приложение, в котором используется служба среднего уровня для доступа к Microsoft Graph.  Корпоративный клиент в организации, использующей это приложение, применяет политику к Exchange Online.  Когда пользователь входит в систему, собственное приложение запрашивает доступ к среднему уровню и отправляет маркер.  Средний уровень выполняет поток On-Behalf-Of для запроса доступа к MS Graph.  На этом этапе запрос утверждений передается на средний уровень. Средний уровень отправляет запрос обратно к собственному приложению, которое должно соответствовать политике условного доступа.
...
### <a name="scenario-app-accessing-the-microsoft-graph"></a>Сценарий: доступ приложения к Microsoft Graph
...
Примеры кода, демонстрирующие способ обработки запросов утверждений для ADAL .NET, см. на странице [примера кода классического приложения .NET](https://github.com/Azure-Samples/active-directory-dotnet-native-desktop) или [примера кода On-Behalf-Of](https://github.com/Azure-Samples/active-directory-dotnet-webapi-onbehalfof-ca).
