...
| response_mode |рекомендуется |Указывает метод, с помощью которого следует отправлять полученный код авторизации приложению.  Поддерживаются значения `form_post` для *формы HTTP POST* или `fragment` для *фрагмента URL-адреса*.  Для веб-приложений рекомендуется использовать `response_mode=form_post`, чтобы обеспечить наиболее безопасную передачу маркеров в приложение. |
...
Azure AD использует файлы cookie для идентификации сеанса пользователя. Веб-приложение также может устанавливать файлы cookie для управления сеансами в приложении. При первом входе пользователя в приложение Azure AD настраивает файл cookie в браузере пользователя. При последующем входе пользователя в другое приложение Azure AD сначала проверяет файл cookie, чтобы определить, открыт ли у пользователя действительный сеанс входа на конечную точку Azure AD, вместо того, чтобы повторно аутентифицировать этого пользователя.

Точно так же при первом выходе пользователя из приложения Azure AD удаляет файл cookie из браузера. Тем не менее пользователь может оставаться вошедшим в другие приложения, использующие Azure AD для аутентификации. Чтобы обеспечить выход пользователя из всех приложений, Azure AD отправляет HTTP-запрос GET к `LogoutUrl` для всех приложений, в которые в настоящее время вошел пользователь. Приложения должны ответить на него, удалив все файлы cookie, которые идентифицируют сеанс пользователя. `LogoutUrl` можно настроить на портале Azure.
...
4. Щелкните **Свойства** и найдите текстовое поле **URL-адрес выхода**. 
...
После получения авторизации `code` и `id_token` вы можете разрешить вход пользователя и получить токены доступа от его имени.  Для входа пользователя необходимо проверить маркер `id_token` , как описано выше. Чтобы получить маркеры доступа, необходимо выполнить действия, описанные в разделе "Использование кода авторизации для запроса маркера доступа" [документации по протоколу OAuth](active-directory-protocols-oauth-code.md).
