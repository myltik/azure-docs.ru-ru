...
| --- | --- | --- |
| tenant |Требуется |Клиент каталога, из которого необходимо запросить разрешение. Его можно указать в виде GUID или понятного имени. Если неизвестно, какому клиенту относится пользователь, и нужно обеспечить его вход с помощью любого клиента, используйте `common`. |
...
| client_secret |Требуется |Секрет приложения, созданный для приложения на портале регистрации приложений. |
| grant_type |Требуется |Этот параметр должен содержать значение `client_credentials`. |
