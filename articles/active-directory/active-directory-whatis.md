...
`SAML`, `WS-Federation`, интерактивная с использованием поддерживаемых учетных данных, OAuth 2.0, OpenID Connect 

#### <a name="on-premises-ad-ds"></a>Локальные службы AD DS
...
#### <a name="azure-ad"></a>Azure AD
Доступ через Azure AD Graph и Microsoft Graph. 
...
#### <a name="azure-ad"></a>Azure AD
Интерфейсы REST API Azure AD Graph или REST API Майкрософт. 
...
#### <a name="azure-ad"></a>Azure AD
Области на основе ресурсов и управление доступом на основе ролей, определяемые клиентом делегируемые разрешения и разрешения приложения, инфраструктура предоставления согласия (корректно обеспечивает согласие пользователя или администратора на предоставление разрешений в соответствии с определением либо запросом ресурса или клиента) 

Посредством роли приложения разрешения могут применяться к отдельным пользователям или к группам. Поддерживается: доступ под управлением администратора, самостоятельный доступ к приложениям, предоставление согласия пользователем.

#### <a name="on-premises-ad-ds"></a>Локальные службы AD DS
Посредством списков контроля доступа разрешения могут применяться к отдельным пользователям или группам. Поддерживается: доступ под управлением администратора. 
...
#### <a name="azure-ad"></a>Azure AD
`Admin managed`, управление на основе правил или динамическое управление, самостоятельное управление группами 

#### <a name="on-premises-ad-ds"></a>Локальные службы AD DS
`Admin managed`, для управления на основе правил или динамического управления требуется внешняя система (FIM или другая) |
