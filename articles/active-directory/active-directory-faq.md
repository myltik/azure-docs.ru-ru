...
****Вопрос. Почему появляется сообщение "Подписка не найдена" при попытке получить доступ к Azure AD на классическом портале Azure (https://manage.windowsazure.com)?**
...
- [Связь между подписками Azure и Azure Active Directory](active-directory-how-subscriptions-associated-directory.md)
...
Дополнительную информацию см. в статье [Интеграция локальных удостоверений с Azure Active Directory](active-directory-aadconnect.md).
...
**Ответ**. Вам нужно настроить только единый вход между локальным каталогом и Azure AD. Если вы получаете доступ к облачным приложениям через Azure AD, служба автоматически подсказывает пользователям, как правильно пройти проверку подлинности с помощью учетных данных своей локальной учетной записи.
...
**Ответ.** Azure Active Directory предоставляет несколько способов просмотра приложений и доступа к ним, например:
...
**Ответ.** Azure Active Directory поддерживает множество стандартных протоколов для проверки подлинности и авторизации, например SAML 2.0, OpenID Connect, OAuth 2.0 и WS-Federation. Azure AD также поддерживает хранилища паролей и возможности автоматического входа для приложений, которые поддерживают только проверку подлинности на основе форм.
...
Дополнительную информацию см. в статье [Как обеспечить безопасный удаленный доступ к локальным приложениям](active-directory-application-proxy-get-started.md).
...
Дополнительную информацию см. в статье [Защита доступа к Office 365 и другим приложениям, подключенным к Azure Active Directory](active-directory-conditional-access.md).
