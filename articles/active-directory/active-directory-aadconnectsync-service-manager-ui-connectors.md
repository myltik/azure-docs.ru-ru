...
Если, к примеру, выполняется поиск в поддереве, вы получите все объекты в одном подразделении.
![Synchronization Service Manager](./media/active-directory-aadconnectsync-service-manager-ui/cssearchsubtree.png)
Здесь можно выбрать объект, **свойства** и [отслеживание](#follow-an-object-and-its-data-through-the-system) из исходного пространства соединителя через метавселенную до целевого пространства соединителя.
...
**Импорт**.  
При открытии объекта пространства соединителя в верхней части будут находиться несколько вкладок. На вкладке **Импорт** отображаются данные, добавленные после импорта.
![Synchronization Service Manager](./media/active-directory-aadconnectsync-service-manager-ui/csimport.png)
В столбце **Старое значение** отображаются объекты, в настоящее время хранящиеся в системе. В столбце **Новое значение** отображаются объекты, полученные из исходной системы и еще не примененные. Поскольку в этом случае существует ошибка синхронизации, изменение применить нельзя.

**Ошибка**.  
Страница ошибки отображается только при наличии проблемы с объектом. Подробные сведения об [устранении неполадок синхронизации](active-directory-aadconnectsync-service-manager-ui-operations.md#troubleshoot-errors-in-operations-tab) см. на странице операций.

**Журнал обращений и преобразований**.  
На вкладке "Журнал обращений и преобразований" отображается связь объекта пространства соединителя с объектом метавселенной. Можно увидеть, когда последний раз импортировалось изменение из подключенной системы и какие правила были применены для внесения данных в метавселенную.
![Synchronization Service Manager](./media/active-directory-aadconnectsync-service-manager-ui/cslineage.png)
В столбце **Действия** отображается одно правило синхронизации **Входящее** с действием **Подготовка**. Это значит, что объект метавселенной будет храниться, пока существует этот объект пространства соединителя. Если в списке правил синхронизации отображается правило синхронизации с направлением **Исходящее** и действием **Подготовка**, это значит, что объект будет удален вместе с объектом метавселенной.
...
**Журнал**.  
Страница "Журнал" используется для проверки состояния синхронизации пароля и просмотра исторических данных. Дополнительные сведения см. в статье [Устранение неполадок синхронизации паролей](active-directory-aadconnectsync-implement-password-synchronization.md#troubleshoot-password-synchronization).

### Свойства объекта метавселенной
**Атрибуты**.  
На вкладке "Атрибуты" можно просмотреть значения и узнать, какой соединитель их передал.
![Synchronization Service Manager](./media/active-directory-aadconnectsync-service-manager-ui/mvattributes.png)
**Соединители**.  
На вкладке "Соединители" отображаются все пространства соединителя, имеющие представление объекта.
