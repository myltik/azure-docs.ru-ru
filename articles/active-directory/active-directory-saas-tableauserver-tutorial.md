...
2. В диалоговом окне **Атрибуты токена SAML** сделайте следующее.
   ...
   2. В текстовом поле **Имя атрибута** введите значение **имени пользователя**.
   3. В списке **Значение атрибута** выберите **user.displayname**.
   4. Нажмите **Завершено**.    
...
4. Щелкните **Настройка единого входа**, чтобы открыть диалоговое окно **Настройка единого входа**.
   
    ![Настройка единого входа][6] 
5. На странице **Как пользователи должны входить в Tableau Server** выберите **Единый вход Azure AD** и нажмите кнопку **Далее**.
   
    ![Настройка единого входа](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_03.png) 
6. На странице диалогового окна **Настроить параметры приложения** выполните следующие действия, а затем нажмите кнопку **Далее**.
...
8. Чтобы единый вход был настроен для вашего приложения, войдите в клиент Tableau Server с правами администратора.
   ...
   1. В окне конфигурации Tableau Server откройте вкладку **SAML** .
  
    ![Настройка единого входа](./media/active-directory-saas-tableauserver-tutorial/tutorial_tableauserver_001.png) 
   2. Установите флажок **Использовать SAML для единого входа**.
   3. Найдите файл метаданных федерации, скачанный с классического портала Azure, а затем передайте его в **файл метаданных поставщика удостоверений SAML**.
   4. URL-адрес возврата Tableau Server — URL-адрес для доступа пользователей Tableau Server, например http://tableau_server. Мы не рекомендуем использовать http://localhost. URL-адреса с конечной косой чертой (например http://tableau_server/) не поддерживаются. Скопируйте **URL-адрес возврата Tableau Server** и вставьте его в текстовое поле **URL-адрес входа Azure AD**, как показано в шаге 3.
   5. Идентификатор сущности SAML — идентификатор сущности однозначно определяет установку Tableau Server для поставщика удостоверений. Если нужно, здесь можно еще раз ввести URL-адрес Tableau Server, но это необязательно должен быть URL-адрес Tableau Server. Скопируйте **идентификатор сущности SAML** и вставьте его в текстовое поле **идентификатор** Azure AD, как показано в шаге 3.
   6. Щелкните **Экспорт файла метаданных** и откройте его в текстовом редакторе. Найдите URL-адрес службы обработчика утверждений с Http Post и индексом 0 и скопируйте URL-адрес. Вставьте его в текстовое поле **URL-адрес ответа** Azure AD, как показано в шаге 3. 
   7. Нажмите кнопку **ОК** на странице конфигурации Tableau Server.
