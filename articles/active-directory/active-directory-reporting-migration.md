...
Отчеты **Azure AD об аномальных действиях** с классического портала Azure были объединены в централизованное представление всех событий риска, связанных с безопасностью, которые Azure Active Directory может обнаружить и указать в отчетах. 

В следующей таблице перечислены различные отчеты **Azure AD об аномальных действиях** и соответствующие типы событий риска на портале Azure. 
...
Следующие отчеты Azure AD об аномальных действиях не включены как события риска в Azure AD. Эти отчеты по-прежнему доступны на классическом портале Azure. Но в будущем они будут считаться устаревшими.

* "Операции входа после нескольких неудачных попыток";
* "Операции входа из нескольких географических регионов".

Полный обзор представлен в статье [События риска Azure Active Directory](active-directory-identity-protection-risk-events.md).  
