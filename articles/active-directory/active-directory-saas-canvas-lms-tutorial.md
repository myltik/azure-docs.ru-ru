<properties pageTitle="Учебник. Интеграция Azure Active Directory с Canvas LMS | Microsoft Azure" description="Узнайте, как использовать Canvas LMS вместе с Azure Active Directory для реализации единого входа, автоматической подготовки пользователей и выполнения других задач." services="active-directory" authors="MarkusVi"  documentationCenter="na" manager="stevenpo"/>
<tags ms.service="active-directory" ms.devlang="na" ms.topic="article" ms.tgt_pltfrm="na" ms.workload="identity" ms.date="08/01/2015" ms.author="markvi" />
#Руководство. Интеграция Azure Active Directory с Canvas LMS
>[AZURE.TIP]Чтобы оставить отзыв, нажмите [здесь](http://go.microsoft.com/fwlink/?LinkId=524182).
...
1.  На странице интеграции с приложением **Canvas** портала Azure AD нажмите **Настройка единого входа**, чтобы открыть диалоговое окно **Настройка единого входа**.
...
3.  На странице **Настройка URL-адреса приложения** в текстовом поле **URL-адрес для входа в Canvas** введите свой URL-адрес, используя следующий шаблон `https://<tenant-name>.instructure.com`, — **Далее**.
...
4.  На странице **Настройка единого входа в Canvas** нажмите кнопку **Загрузить сертификат**, а затем сохраните файл сертификата локально на компьютере.
...
6.  Последовательно выберите **Курсы > Управляемые учетные записи > Microsoft**.

    ![Холст](./media/active-directory-saas-canvas-lms-tutorial/IC775990.png "Холст")

7.  На панели навигации слева выберите раздел **Проверка подлинности** и нажмите кнопку **Добавить новую конфигурацию SAML**.
...
8.  На странице "Текущая интеграция" выполните следующие действия.
    ...
    1.  На странице **Настройка единого входа в Canvas** на портале Azure скопируйте значение поля **Идентификатор сущности** и вставьте его в текстовое поле **Идентификатор сущности поставщика удостоверений**.
    ...
    3.  На странице **Настройка единого входа в Canvas** портала Azure скопируйте значение поля **URL-адрес удаленного выхода** и вставьте его в текстовое поле **URL-адрес выхода**.
    4.  На странице **Настройка единого входа в Canvas** портала Azure скопируйте значение поля **URL-адрес изменения пароля** и вставьте его в текстовое поле **Ссылка на изменение пароля**.
...
2.  Последовательно выберите **Курсы > Управляемые учетные записи > Microsoft**.
...
5.  На странице диалогового окна "Добавление пользователя" выполните следующие действия.
    ...
    2.  В тестовом поле **Адрес электронной почты** введите адрес электронной почты пользователя.
    3.  В текстовом поле **Имя входа** введите адрес электронной почты пользователя в Azure AD.
    4.  Установить флажок **Сообщить пользователю о создании этой учетной записи по электронной почте**.
...
2.  На странице интеграции с приложением **Canvas** нажмите **Назначить пользователей**.
