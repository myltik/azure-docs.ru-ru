...
> [AZURE.NOTE]
<li> Если учетная запись, которая использовалась для установки Azure AD Connect, не имеет прав локального администратора на серверах AD FS, вам будет предложено ввести учетные данные для учетной записи с соответствующими разрешениями.</li> <li> Перед этим этапом настройки убедитесь в возможности HTTP/HTTPS-подключения между сервером Azure AD Connect и прокси-сервером веб-приложения.</li> <li> Также убедитесь в возможности HTTP/HTTPS-подключения между сервером веб-приложений и сервером AD FS, чтобы разрешить сквозные запросы на проверку подлинности.</li>
...
- Проверьте вход через браузер Internet Explorer с подключенного к домену компьютера интрасети. Для этого подключитесь к https://myapps.microsoft.com и войдите, используя учетные данные для входа в учетную запись. **Примечание.** Встроенная учетная запись администратора в AD DS не синхронизируется и не может использоваться для проверки.
- Проверьте вход через браузер с любого устройства из экстрасети. Для этого подключитесь к https://myapps.microsoft.com с домашнего компьютера или мобильного устройства, а затем укажите идентификатор для входа и пароль.
- Проверьте вход клиента с расширенными возможностями. Для этого подключитесь к https://testconnectivity.microsoft.com, откройте вкладку **Office 365** и выберите **Тест единого входа Office 365**.
