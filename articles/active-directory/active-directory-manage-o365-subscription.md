<properties
   pageTitle="Управление каталогом для подписки Office 365 в Azure"
   description="Управление каталогом учетной записи подписки Office 365 с помощью Azure Active Directory и портала управления"
...
В этой статье описывается управление каталогом, созданным для подписки Office 365, на портале управления Azure. Выполнение представленных ниже действий зависит от того, имеется ли уже подписка на Azure. Для входа на портал управления вы должны быть администратором служб или соадминистратором подписки Azure.
...
По заверении оформления подписки Azure можно войти на портал управления Azure и приступить к работе со службами Azure. Щелкните расширение Active Directory для управления тем же каталогом, который проверяет подлинность ваших пользователей Office 365.
...
1.	Войдите на портал управления Azure в качестве пользователя msmith@hotmail.com.
...
4.	Войдите на портал управления в качестве глобального администратора Contoso.onmicrosoft.com (например,msmith@contoso.com)).
5.	При появлении запроса **Использовать каталог Contoso с Azure?** нажмите **Продолжить**.
6.	Щелкните **Выйти сейчас**.
7.	Войдите на портал управления в качестве пользователя msmith@hotmail.com. Каталог Contoso и каталог по умолчанию отобразятся в расширении Active Directory.
...
Теперь предположим, что пользователю John Doe необходимо войти на портал управления Azure и приступить к администрированию веб-сайтов и ресурсов базы данных, которые связаны с подпиской Azure пользователя msmith@hotmail.com. Для этого пользователю Michael Smith необходимо дополнительно выполнить указанные ниже действия.

1.	Войдите на портал управления, используя учетную запись администратора служб подписки Azure (в данном примере msmith@hotmail.com)).
