...
- Использование AADSync для синхронизации свойств пользователей из локального домена Active Directory в службу Azure AD.
...
- Обязательная регистрация пользовательских данных для сброса пароля при входе в панель доступа по адресу [http://myapps.microsoft.com](http://myapps.microsoft.com) (для параметра конфигурации **Требовать у пользователей регистрировать SSPR** установите значение **Да**).

Чтобы система работала, пользователям необязательно регистрировать данные для сброса пароля. Например, если в локальном каталоге уже имеются телефонные номера, их можно синхронизировать в службу Azure AD, после чего они будут автоматически использоваться для сброса пароля.
