...
Управление привилегированными пользователями (PIM) Azure создает предупреждения при возникновении подозрительных или небезопасных действий в среде. В случае активации оповещения оно отображается на панели мониторинга PIM.
...
| **Роли активируются слишком часто** | Слишком много повторных активаций, выполненных для одной и той же роли в рамках разрешенного в параметрами времени. | Свяжитесь с пользователем, чтобы выяснить причины столь частой активации роли. Возможно, ограничения по времени не позволяют ему выполнить свои задачи. Или, возможно, он использует сценарии для обхода процесса. |
...
| **Администраторы не используют свои привилегированные роли** | Имеются временные администраторы, которые в последнее время не выполняли активацию своих ролей. | Запустите проверку доступа для определения пользователей, которым больше не требуется доступ. |
...
- **Number of activation renewals** (Количество обновлений активации): в пределах выбранного временного интервала укажите количество активаций, (от 2 до 100), которое вы считаете подозрительным или, по крайней мере, заслуживающим внимания. Чтобы задать значение, переместите ползунок или введите число в текстовое поле.


### Оповещение "Слишком много глобальных администраторов"

PIM инициирует это предупреждение, если выполняются два условия, каждое из которых можно настроить. Во-первых, количество глобальных администраторов должно достичь определенного порогового значения. Во-вторых, в общем числе назначенных ролей должен быть определенный процент глобальных администраторов. Если выполняется только одно из условий, то оповещение не отображается.
