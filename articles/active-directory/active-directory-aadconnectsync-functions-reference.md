...
**Пример:**<br> `ConvertFromUTF8Hex("48656C6C6F20776F726C6421")`<br> Возвращает «\*Hello world!\*».
...
**Описание.**<br> Функция Count возвращает количество элементов в атрибут с несколькими значениями.
...
**Описание.** <br> Возвращает значение Date, содержащее дату, к которой добавлен конкретный временной интервал.
...
**Пример:** <br> `If dn is “cn=Joe,ou=Atlanta,ou=GA,ou=US, dc=contoso,dc=com” then DNComponentRev([dn],3)` <br> `DNComponentRev([dn],1,”DC”)` <br> Оба возвращают US.
...
**Примечания.** <br> Возможные значения для формата можно найти здесь: [User-Defined Date/Time Formats (Format Function)](http://msdn2.microsoft.com/library/73ctwf33(VS.90).aspx).
 
**Пример:** <br>
 
`FormatDateTime(CDate(“12/25/2007”),”yyyy-mm-dd”)` <br> Результаты: «2007-12-25».
...
**Примечания.** <br> Возвращает позицию, в которой найдена подстрока, или 0, если подстрока не найдена.
...
**Пример:** <br> `InStrRev("abbcdbbbef","bb")` <br> Возвращает 7.
...
**Пример:** <br> `IsBitSet(&HF,4)` <br> Возвращает значение True, поскольку для бита «4» задано шестнадцатеричное значение «F».
...
**Пример:** <br> `LCase(“TeSt”)` <br> Возвращает «test».
...
**Пример:** <br> `Left(“John Doe”, 3)` <br> Возвращает «Joh».
...
**Пример:** <br> `Len(“John Doe”)` <br> Возвращает 8.
...
**Пример:** <br> `LTrim(“ Test ”)` <br> Возвращает «Test ».
...
**Описание.** <br> Функция Mid возвращает указанное количество символов из конкретной позиции строки.
...
**Примечания.** <br> Вернуть numChars символов, начиная с позиции start в строке.<br> Строка, содержащая numChars символов начиная с позиции start в строке:
...
`Mid(“John Doe”, 3, 5)` <br> Возвращает «hn Do».

`Mid(“John Doe”, 6, 999)` <br> Возвращает «Doe».
...
**Пример:** <br> `NumFromDate(CDate("2012-01-01 23:00:00"))` <br> Возвращает 129699324000000000.
...
**Пример:** <br> `PadRight(“User”, 10, “0”)` <br> Возвращает «User000000».
...
**Пример:** <br> `PCase(“TEsT”)` <br> Возвращает «Test».
...
- start: число, определяющее нижний предел случайного значения для создания. <br>
- end: число, определяющее верхний предел случайного значения для создания.
 
**Пример:** <br> `Random(100,999)` <br> Возвращает 734.
...
`ReplaceChars(”Räksmörgås”,%ReplaceString%)` <br> Возвращает Raksmorgas

`ReplaceChars(“O’Neil”,%ReplaceString%)` <br> Возвращает «ONeil», апостроф определен как символ для удаления.
...
**Пример:** <br> `Right(“John Doe”, 3)` <br> Возвращает «Doe».
...
**Пример:** <br> `RTrim(“ Test ”)` <br> Возвращает « Test».
...
**Пример:** <br> `Trim(“ Test ”)` <br> Возвращает «Test».
...
**Пример:** <br> `UCase(“TeSt”)` <br> Возвращает «TEST».
...
**Пример:** <br> `Word(“The quick brown fox”,3,” “)` <br> Возвращает «brown».
