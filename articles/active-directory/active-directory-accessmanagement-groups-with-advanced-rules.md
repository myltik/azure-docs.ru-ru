...
Использование нескольких значений атрибутов, синхронизированных из локального экземпляра Active Directory, сейчас не поддерживается.
...
>
> Рекомендуется заранее проверить новое правило членства, чтобы убедиться, оно соответствующим образом добавляет элементы в группу.

1. Откройте группу на [классическом портале Azure](https://manage.windowsazure.com).
2. Выберите вкладку **Настройка**, чтобы просмотреть текущее состояние динамического членства.
3. Чтобы сделать группу статической, переключите параметр **Включить динамическое членство** в положение **Нет**. Нажмите кнопку **Сохранить** на панели инструментов, чтобы подтвердить действие. Существующие элементы останутся в группе, и с настоящего момента правило членства не будет обрабатываться.
4. Чтобы сделать группу динамической, переключите этот параметр в положение **Да**, укажите нужное правило членства и нажмите кнопку **Сохранить**. Существующие элементы будут удалены, после чего начнется обработка нового правила для добавления новых элементов.
