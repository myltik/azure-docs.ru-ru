...
Добавьте в каталог клиента учетную запись каждого пользователя, которому предоставлен доступ к облачной службе Майкрософт. По умолчанию добавленные пользователи не имеют прав администратора, но вы можете назначать им роли в любое время.
...
4. На странице **Тип учетной записи пользователя** в раскрывающемся меню **Тип пользователя** выберите один из следующих пунктов.
	...
	- **Пользователь из другого каталога Azure AD** — если требуется добавить в ваш каталог учетную запись пользователя из другого каталога Azure AD. Чтобы выбрать пользователя в другом каталоге, необходимо быть членом этого каталога.
...
5. В зависимости от **типа пользователя** введите имя пользователя, адрес электронной почты или передайте CSV-файл с адресами электронной почты.
...
>
> - Можно добавить учетные записи пользователей с одним и тем же именем участника-пользователя (UPN) в разных доменах. Для этого сначала следует добавить, например, geoffgrisso@contoso.onmicrosoft.com, а затем geoffgrisso@contoso.com.
> - **Не** добавляйте geoffgrisso@contoso.com перед добавлением geoffgrisso@contoso.onmicrosoft.com. Порядок добавления важен: если вы допустите ошибку сейчас, потом ее может быть сложно исправить.
...
1. Откройте свой каталог.
2. Перейдите на вкладку **Пользователи**, а затем выберите отображаемое имя пользователя, которое будет изменено.
3. В области управления выберите **Сбросить пароль**.
...
- **Dynamics AX** позволяет внешнему пользователю обращаться к авторизованным ресурсам Dynamics AX, если у него есть лицензия PowerShell. На внешних пользователей в Dynamics AX распространяются ограничения для [внешних пользователей Azure AD](#known-limitations-of-azure-ad-external-users) и [пользователей-гостей](#guest-user-management-and-limitations).
...
На вкладке **Configure** (Настройка) каталога содержатся параметры для управления доступом внешних пользователей. Эти параметры может изменить только глобальный администратор каталога на классическом портале Azure. В данный момент это нельзя сделать с помощью PowerShell или API.
