<properties
	...
	description="Объясняется, как создавать или изменять учетные записи пользователей в Azure Active Directory."
...
Чтобы создать внешнего пользователя, создайте пользователя на классическом портале Azure, а затем в меню**Тип пользователя** выберите пункт **Пользователь из другого каталога Azure AD**.
...
Пользователь, являющийся администратором нескольких каталогов, может управлять ими на классическом портале Azure. Однако другие приложения, такие как Office 365, в текущий момент не позволяют назначать и использовать службы от лица внешнего пользователя в другом каталоге. Забегая вперед — мы предоставим разработчикам инструкции для того, чтобы их приложения могли работать с пользователями, входящими в несколько каталогов.
...
На вкладке **Configure** (Настройка) каталога содержатся параметры для управления доступом внешних пользователей. Эти параметры могут быть изменены глобальным администратором каталога только в пользовательском интерфейсе (Windows PowerShell или API для этого не подходят) на классическом портале полной версии Azure. Чтобы открыть вкладку **Конфигурация** на классическом портале Azure, щелкните **Active Directory**, а затем щелкните имя каталога.
