...
В этой статье описывается процедура включения прокси приложения Microsoft Azure AD для облачного каталога в Azure AD. Процедура включает следующие этапы.

- Установка соединителя прокси приложения в частной сети, которая поддерживает подключение к прокси-службе из сети.
- Регистрация соединителя в подписке клиента Microsoft Azure AD.
...
- Сервер под управлением Windows Server 2012 R2 или Windows 8.1 или более поздней версии, на котором можно установить соединитель прокси приложения. Сервер отправляет HTTPS-запросы в службы прокси приложения в облаке; у него должно быть подключение HTTPS к приложениям, которые вы публикуете.
...
5. Выберите **Скачать сейчас**. Откроется окно **скачивания соединителя прокси приложения Azure AD**. Прочтите и примите условия лицензионного соглашения, затем щелкните **Загрузить**, чтобы сохранить файл установщика Windows (EXE-файл) для соединителя прокси приложения.
...
3. Во время установки вам будет предложено зарегистрировать соединитель прокси приложения клиента Azure AD.
  ...
  - Если не удается зарегистрировать соединитель, см. раздел [Устранение неполадок прокси-сервера приложений](active-directory-application-proxy-troubleshoot.md).  
