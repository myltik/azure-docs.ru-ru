...
1. Откройте следующие порты для **исходящего** трафика:
   ...
   | --- | --- |
   | 80 | Скачивание списков отзыва сертификатов при проверке сертификата SSL |
   ...
   > [!IMPORTANT]
   > В таблице представлены требования к портам соединителей версии 1.5.132.0 и более поздней версии. Если используется более старая версия соединителя, необходимо также открыть следующие порты: 5671, 8080, 9090, 9091, 9350, 9352 и 10100–10120.
   >
...
2. Если брандмауэр или прокси-сервер позволяет использовать "белый список" DNS, можете добавить подключения к msappproxy.net и servicebus.windows.net в список разрешенных. Если нет, необходимо разрешить доступ к [диапазонам IP-адресов центра обработки данных Azure](https://www.microsoft.com/download/details.aspx?id=41653). Список диапазонов IP-адресов обновляется еженедельно.
