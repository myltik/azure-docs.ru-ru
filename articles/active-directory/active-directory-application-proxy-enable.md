...
<p> Укажите учетные данные глобального администратора Azure AD. <p> Убедитесь, что администратор, который регистрирует соединитель, находится в том каталоге, где включены службы прокси приложения. Например, если домен клиента — contoso.com, у администратора должен быть псевдоним admin@contoso.com или другой псевдоним в этом домене. Также убедитесь, что вы являетесь глобальным администратором клиента Azure AD. Ваш клиент глобального администратора может отличаться от учетных данных Microsoft Azure. <p> Если на сервере, на который устанавливается соединитель Azure AD, конфигурация усиленной безопасности Internet Explorer **включена**, экран регистрации может быть заблокирован. В этом случае следуйте указаниям в сообщении об ошибке, чтобы разрешить доступ. Убедитесь, что усиленная безопасность Internet Explorer отключена. <p> Если зарегистрировать соединитель не удается, см. раздел [Устранение неполадок прокси-приложения](active-directory-application-proxy-troubleshoot.md).
