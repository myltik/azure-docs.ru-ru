...
> [!IMPORTANT]
> Если вы используете **пробную** учетную запись Salesforce.com, то не сможете настроить автоматическую подготовку пользователей. У пробных учетных записей нет необходимых прав доступа к API.
> 
...
8. На странице **Single Sign-On Settings** (Параметры единого входа) нажмите кнопку **Edit** (Изменить).
   ...
   > [!NOTE]
   > Если вам не удается включить параметры единого входа для своей учетной записи Salesforce, возможно, вам придется обратиться за помощью в службу поддержки Salesforce.
   > 
   > 
...
2. В диалоговом окне **Настройка подготовки пользователей** введите имя пользователя и пароль администратора Salesforce.
   ...
   > [!NOTE]
   > При настройке рабочей среды рекомендуется создать новую учетную запись администратора Salesforce специально для этого шага. Данной учетной записи в Salesforce должен быть назначен профиль **Системный администратор** .
   > 
   > 
