<properties 
	pageTitle="Обзор Microsoft Passport и сведения об этой новой аутентификации на основе сертификации | Microsoft Azure" 
...
Использования текущих способы проверки подлинности лишь с помощью паролей недостаточно для защиты пользователей. Пользователи имеют тенденцию менять пароли и забывать их. Пароли можно раскрыть, украсть, взломать и подобрать методом перебора. Они также трудны для запоминания и уязвимы для атак типа «передача хэша». «Передача хэша» — это методика взлома, которая позволяет злоумышленнику пройти аутентификацию на удаленном сервере или в службе, используя базовый хэш NTLM и (или) LanMan пароля пользователя вместо обязательного связанного пароля в виде открытого текста, как в обычном случае. Дополнительные сведения о передаче хэша см. в разделе [Передача хэша](https://technet.microsoft.com/ru-RU/dn785092.aspx).
...
3. Для разблокировки устройства будет использоваться единый жест, который позволит получить доступ к разным ресурсам, если устройство присоединено к домену или Azure AD.

## Жизненный цикл Microsoft Passport
Жизненный цикл аутентификации Microsoft Passport![](./media/active-directory-azureadjoin/active-directory-azureadjoin-microsoft-passport.png). На схеме выше показаны пара закрытого и открытого ключей и процедура проверки поставщиком удостоверений. Каждое из этих действий подробно рассмотрено ниже.
...
* [Управление аутентификацией с помощью Microsoft Passport] (https://technet.microsoft.com/library/mt219735(v=vs.85).aspx)
