...
6. Чтобы настроить единый вход на стороне **Degreed**, отправьте в [службу поддержки Degreed](mailTo:admin@degreed.com) скачанный **XML-файл метаданных**. Специалисты службы поддержки настроят подключение единого входа SAML на обеих сторонах подключения.
