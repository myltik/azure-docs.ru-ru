...
> 
> На данный момент не удается создать группу устройств, на основе атрибутов пользователя-владельца. Правила членства для устройств могут ссылаться только на непосредственные атрибуты объектов устройств в каталоге.
...
Выполняется обновление портала Azure для поддержки этой функции. В то же время можно использовать командлеты PowerShell, как показано ниже.
