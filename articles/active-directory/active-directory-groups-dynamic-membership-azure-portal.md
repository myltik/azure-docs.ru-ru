...
> 
> Рабочая область Microsoft Teams пока не поддерживает членство в динамических группах. Вы можете проверить ошибку в журналах, содержащих ошибку Cannot migrate Dynamic membership group (Не удается переместить членство в динамических группах).
...
Выполняется обновление портала Azure для поддержки этой функции. Пока можно использовать командлеты PowerShell, как показано ниже.
...
> [!NOTE]
> Для изменения свойств динамической группы потребуется использовать командлеты **предварительной версии** [PowerShell версии 2 для Azure AD](https://docs.microsoft.com/powershell/azure/active-directory/install-adv2?view=azureadps-2.0). Вы можете скачать предварительную версию [здесь](https://www.powershellgallery.com/packages/AzureADPreview).
