...
Если у вас есть подписка на Azure или Office 365 и вы используете портал Azure или приложение Office 365, вам будет предоставлен список приложений, не требующих повторного входа. Если вы не прошли проверку подлинности вам будет предложено выполнить вход, используя имя пользователя и пароль для учетной записи в Azure AD. Если в организации настроена федерация, достаточно ввести имя пользователя.
...
Расширение панели доступа остается доступной для:
...
## <a name="my-apps-secure-sign-in-extension"></a>Расширение защищенного входа в мои приложения
Расширение является обязательным для пользователей для входа на основе пароля единого входа. После установленного пользователи могут также включить дополнительные функции при входе в расширении, щелкнув **вход приступить к работе**. 

- Пользователи могут входить в приложения напрямую, перейдя по адресу приложения **URL-адрес входа**. При переходе приложения URL-адрес входа расширение обнаружит это и предоставляют возможности для пользователя для входа в него из модуля.
- Пользователи также могут запускать свои приложения с помощью панели доступа **быстрого поиска** возможность расширения. 
- Расширения также будет показано пользователям последние три приложения, они запускаются под **недавно использованные** раздела.
> [!NOTE]
> Дополнительные возможности доступны только в Edge, Chrome, Firefox.
