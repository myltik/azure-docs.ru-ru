<properties pageTitle="Руководство. Интеграция Azure Active Directory с Work.com | Microsoft Azure" description="Узнайте, как использовать Work.com вместе с Azure Active Directory для реализации единого входа, автоматической подготовки пользователей и выполнения других задач." services="active-directory" authors="MarkusVi"  documentationCenter="na" manager="stevenpo"/>
<tags ms.service="active-directory" ms.devlang="na" ms.topic="article" ms.tgt_pltfrm="na" ms.workload="identity" ms.date="08/01/2015" ms.author="markvi" />
#Руководство. Интеграция Azure Active Directory с Work.com
  
>[AZURE.TIP]Чтобы оставить отзыв, нажмите [здесь](http://go.microsoft.com/fwlink/?LinkId=529836).
  
Цель данного руководства — продемонстрировать интеграцию Azure и Work.com. Сценарий, описанный в этом учебнике, предполагает, что у вас уже имеется:

-   Действующая подписка на Azure
-   Подписка с поддержкой единого входа Work.com
  
После завершения этого учебника пользователи AAD, которым вы назначите доступ к Work.com, смогут пользоваться единым входом в приложение на сайте Work.com для вашей организации (вход, инициированный поставщиком услуг) или с помощью [панели доступа](https://msdn.microsoft.com/library/dn308586).
...
В этом разделе показано, как разрешить пользователям проходить проверку подлинности в Work.com со своей учетной записью Azure AD, используя федерацию на основе протокола SAML. В рамках этой процедуры потребуется отправить сертификат на сайт Work.com.
...
3.  В разделе **Администрирование** левой области навигации щелкните **Управление доменами**, чтобы развернуть соответствующий раздел, и щелкните **Мой домен**, чтобы открыть страницу **Мой домен**.
...
8.  На странице **Настроить URL-адрес приложения** в текстовом поле **URL-адрес входа в Work.com** введите URL-адрес, используемый вашими пользователями для входа в приложение Work.com (например, "**http://company.my.salesforce.com*”)) и нажмите кнопку **Далее**.
...
13. На странице **Параметры единого входа** выполните следующие действия.
...
14. В разделе **Параметры единого входа SAML** выполните следующие действия.
    ...
    2.  На странице **Настройка единого входа в Work.com** портала Azure скопируйте значение поля **URL-адрес издателя** и вставьте его в текстовое поле **Издатель**.
    3.  Чтобы отправить скачанный, нажмите кнопку **Обзор**.
...
5.  В поле **Тип удостоверения SAML** выберите значение **Проверочное утверждение содержит идентификатор федерации из объекта User**.
    ...
    7.  На странице **Настройка единого входа в Work.com** портала Azure скопируйте значение поля **URL-адрес удаленного входа** и вставьте его в текстовое поле **URL-адрес входа поставщика удостоверений**.
    8.  На странице **Настройка единого входа в Work.com** портала Azure скопируйте значение поля **URL-адрес удаленного выхода** и вставьте его в текстовое поле **URL-адрес выхода поставщика удостоверений**.
...
5.  В разделе "Изменить пользователя" выполните следующие действия.
    ...
    1.  Заполните поля **Фамилия**, **Псевдоним**, **Электронный адрес**, **Имя пользователя** и **Прозвище** данными действующей учетной записи Azure Active Directory, которую нужно подготовить.
...
2.  На странице интеграции с приложением Work.com щелкните **Назначить пользователей**.
