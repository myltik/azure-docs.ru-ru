...
1.  На странице интеграции с приложением **песочницы Salesforce** портала Azure AD нажмите кнопку **Настроить единый вход**, чтобы открыть диалоговое окно **Настройка единого входа**.

    ![Настройка единого входа](./media/active-directory-saas-salesforce-sandbox-tutorial/IC749323.png "Настройка единого входа")

2.  На странице **Как пользователям следует входить в песочницу Salesforce?** выберите **Единый вход Microsoft Azure AD** и нажмите кнопку **Далее**.

    ![Salesforce Sandbox](./media/active-directory-saas-salesforce-sandbox-tutorial/IC746479.png "Salesforce Sandbox")

3.  На странице **Настройка URL-адреса приложения** в текстовом поле **URL-адрес для входа в песочницу Salesforce** введите свой URL-адрес, используя шаблон http://company.my.salesforce.com, а затем нажмите кнопку **Далее**.
...
4.  На странице **Настройка единого входа в Salesforce** нажмите кнопку **Загрузить сертификат**, а затем сохраните файл сертификата на компьютере.
...
9.  В разделе «Параметры единого входа SAML» выполните следующие действия:
    ...
    2.  На странице **Настройка единого входа в песочницу Salesforce** портала Azure скопируйте значение поля **URL-адрес издателя** и вставьте его в текстовое поле **Издатель**.
    3.  В текстовом поле **Идентификатор сущности** введите ****https://test.salesforce.com**
4.  Чтобы отправить скачанный сертификат, нажмите кнопку **Обзор**.
    5.  В поле **Тип удостоверения SAML** выберите значение **Утверждение содержит идентификатор федерации из объекта пользователя**.
    ...
    7.  На странице **Настройка единого входа в песочницу Salesforce** портала Azure скопируйте значение поля **URL-адрес удаленного входа** и вставьте его в текстовое поле **URL-адрес входа поставщика удостоверений**.
    8.  На странице **Настройка единого входа в песочницу Salesforce** портала Azure скопируйте значение поля **URL-адрес удаленного выхода** и вставьте его в текстовое поле **URL-адрес выхода поставщика удостоверений**.
    9.  В поле **Привязка запроса, инициированного поставщиком услуг** выберите значение **HTTP POST**.
...
1.  На портале Salesforce в верхней области навигации выберите имя пользователя, чтобы открыть меню пользователя:
...
4.  На странице **Сброс маркера безопасности** щелкните **Сбросить маркер безопасности**, чтобы запросить электронное сообщение, содержащее ваш маркер безопасности Salesforce.com.
...
7.  На странице интеграции приложений **песочницы Sandbox** портала управления Azure щелкните **Настроить подготовку учетных записей пользователей**, чтобы открыть диалоговое окно **Настроить подготовку учетных записей пользователей**.
...
8.  На странице **Введите учетные данные песочницы Salesforce, чтобы включить автоматическую подготовку учетных записей пользователей** укажите следующие параметры конфигурации:
    ...
    1.  В текстовом поле **Имя администратора песочницы Salesforce** введите имя учетной записи песочницы Salesforce с **профилем системного администратора** в Salesforce.com.

    2.  В текстовом поле **Пароль администратора песочницы Salesforce** введите пароль для этой учетной записи.

    3.  В текстовом поле **Маркер безопасности пользователя** вставьте значение маркера безопасности.
...
2.  На странице интеграции с приложением **песочницы Salesforce** щелкните **Назначить пользователей**.
