...
* [Веб-приложения и интерфейсы API, использующие ПО промежуточного слоя для аутентификации Microsoft Azure Active Directory, .NET OWIN OpenID Connect или WS-Fed](#owin)
* [Веб-приложения и интерфейсы API, использующие ПО промежуточного слоя для аутентификации JWT или .NET Core OpenID Connect](#owincore)
* [Веб-приложения и интерфейсы API, использующие модуль Node.js passport-azure-ad](#passport)
* [Веб-приложения и интерфейсы API, созданные с помощью Visual Studio 2015](#vs2015)
* [Веб-приложения, созданные в Visual Studio 2013](#vs2013)
* [Веб-интерфейсы, созданные с помощью Visual Studio 2013](#vs2013_webapi)
* [Веб-приложения, созданные в Visual Studio 2012](#vs2012)
* [Веб-приложения, созданные с помощью Visual Studio 2010, 2008 или Windows Identity Foundation](#vs2010)
* [Веб-приложения и интерфейсы API, использующие другие библиотеки или ручное внедрение поддерживаемых протоколов](#other)

### <a name="owin"></a> Веб-приложения и интерфейсы API, использующие ПО промежуточного слоя для аутентификации Microsoft Azure Active Directory, .NET OWIN OpenID Connect или WS-Fed
...
### <a name="owincore"></a> Веб-приложения и интерфейсы API, использующие ПО промежуточного слоя для аутентификации JWT или .NET Core OWIN OpenID Connect
...
### <a name="passport"></a> Веб-приложения и интерфейсы API, использующие модуль Node.js passport-ad
...
### <a name="vs2015"></a> Веб-приложения и интерфейсы API, созданные с помощью Visual Studio 2015
...
### <a name="vs2013"></a> Веб-приложения, созданные с помощью Visual Studio 2013
...
### <a name="vs2013"></a> Веб-интерфейсы, созданные с помощью Visual Studio 2013
...
В следующем фрагменте кода показано, как получить последние ключи из документа метаданных федерации, а затем использовать [обработчик токенов JWT](https://msdn.microsoft.com/library/dn205065.aspx) для проверки токена. Во фрагменте кода предполагается, что для сохранения ключа, с помощью которого будет выполняться последующая проверка токенов из Azure AD, будет использоваться собственный механизм кэширования, несмотря на место его хранения (база данных, файл конфигурации или другое расположение).
...
### <a name="vs2012"></a> Веб-приложения, созданные с помощью Visual Studio 2012
...
3. Выполните сборку приложения и запустите его. Если у вас получилось войти, приложение успешно обновляет ключ, скачивая необходимые данные из документа метаданных федерации каталога. Если при входе возникают проблемы, убедитесь, что в приложение внесены правильные изменения. Для этого см. раздел [Добавление единого входа в веб-приложение с помощью Azure AD](https://github.com/Azure-Samples/active-directory-dotnet-webapp-openidconnect) или скачайте и проверьте пример кода в разделе [Multi-Tenant Cloud Application for Azure Active Directory](https://code.msdn.microsoft.com/multi-tenant-cloud-8015b84b) (Мультитенантное облачное приложение для Azure Active Directory).


### <a name="vs2010"></a> Веб-приложения, созданные с помощью Visual Studio 2008 или 2010 и Windows Identity Foundation v1.0 для .NET 3.5
...
### <a name="other"></a> Веб-приложения и интерфейсы API, использующие другие библиотеки или ручное внедрение поддерживаемых протоколов
...
Если ключ хранится в приложении или он жестко запрограммирован, его можно вручную извлечь и должным образом обновить. Чтобы избежать нарушений в работе приложения или расходов при увеличении частоты или внештатной смены ключей в Azure AD, **настоятельно рекомендуется настроить поддержку автоматической смены ключей в приложении**, используя методы, описанные в этой статье.

Чтобы вручную извлечь последний ключ из документа обнаружения OpenID, сделайте следующее:

1. В веб-браузере перейдите по адресу `https://login.microsoftonline.com/your_directory_name/.well-known/openid-configuration`. Отобразится содержимое документа обнаружения OpenID Connect. Дополнительные сведения об этом документе см. в статье [OpenID Discovery Document specification](http://openid.net/specs/openid-connect-discovery-1_0.html) (Спецификация документа обнаружения OpenID).
2. Скопируйте ссылку в значение jwks\_uri.
3. Откройте новую вкладку в браузере и перейдите по скопированному URL-адресу. Отобразится содержимое документа JSON с набором веб-ключей.
4. Чтобы обновить приложение для использования нового ключа, найдите все элементы **x5c**, а затем скопируйте значение в каждом из них. Например:
```
keys: [
	{
		kty: "RSA",
...
```
5. Скопировав значение элемента **<X509Certificate>**, откройте обычный текстовый редактор и вставьте значение. Удалите все пробелы в конце, а затем сохраните файл с расширением **CER**.

Чтобы вручную извлечь последний ключ из документа метаданных федерации, сделайте следующее:

1. В веб-браузере перейдите по адресу `https://login.microsoftonline.com/your_directory_name/federationmetadata/2007-06/federationmetadata.xml`. Отобразится содержимое XML-документа метаданных федерации. Дополнительные сведения об этом документе см. в статье [Метаданные федерации](active-directory-federation-metadata.md).
2. Чтобы обновить приложение для использования нового ключа, найдите все блоки **<RoleDescriptor>**, а затем скопируйте значение элемента **<X509Certificate>** в каждом из них. Например:
```
...
```
3. Скопировав значение элемента **<X509Certificate>**, откройте обычный текстовый редактор и вставьте значение. Удалите все пробелы в конце, а затем сохраните файл с расширением **CER**.
