...
В этой статье описывается, как настроить соединитель прокси приложения Azure Active Directory для работы с исходящими прокси-серверами. Она предназначена для клиентов, использующих сетевые среды с имеющимися прокси.
...
Если в вашей среде имеется исходящий прокси-сервер, для установки следует использовать учетную запись, у которой есть полномочия использовать этот исходящий прокси. Поскольку программа установки выполняется в контексте пользователя, который выполняет установку, настройки можно проверить с помощью Microsoft Edge или любого другого браузера.

Настройка параметров прокси-сервера с помощью Microsoft Edge:
...
Базовые компоненты ОС, которые соединитель использует для исходящих запросов, по умолчанию автоматически обнаруживают прокси-серверы в сети. Для этого применяется протокол автоматической настройки прокси, если он поддерживается в сети.
...
В следующем разделе мы рассмотрим, что нужно сделать, чтобы настроить исходящий прокси-сервер для передачи трафика. Но сначала давайте рассмотрим настройки соединителя, при которых он игнорирует локальный прокси-сервер и подключается к службам Azure напрямую. Мы рекомендуем использовать именно этот способ (если допускает политика сети), так как он позволяет сократить количество конфигураций.
...
Как упоминалось выше, в некоторых средах весь исходящий трафик без исключений должен проходить через исходящий прокси-сервер. В результате этого невозможно будет выполнить обход прокси-сервера.
...
Поскольку соединитель использует только исходящий трафик, нет необходимости настраивать балансировку нагрузки или пропускать входящий трафик через брандмауэры.

Но вам нужно выполнить по меньшей мере следующее.
1. Настроить использование исходящего прокси-сервера для соединителя и службы средства обновления.
2. Настроить параметры прокси-сервера, чтобы он пропускал подключения к службе прокси приложения Azure AD.
...
Для этого измените файл C:\Program Files\Microsoft AAD App Proxy Connector\ApplicationProxyConnectorService.exe.config и добавьте раздел [system.net], поместив в него следующий образец кода:
...
>[!NOTE]
>Вместо _proxyserver:8080_ укажите имя или IP-адрес локального прокси-сервера и номер порта для доступа к нему.
>
...
Кроме того, для базовых каналов управления служебной шины, которые используются службой соединителя, нужен доступ к конкретным IP-адресам. Пока служебная шина не переведена полностью на использование доменных имен, есть два варианта действий:

* разрешите соединителю исходящий доступ к любым адресам назначения;
* разрешите соединителю исходящий доступ к [IP-адресам центров обработки данных Azure](https://www.microsoft.com/en-gb/download/details.aspx?id=41653).

>[!NOTE]
>Если вы предпочитаете использовать диапазоны IP-адресов центров обработки данных Azure, вас ждет одна сложность — эти списки обновляются еженедельно. Необходимо организовать процесс соответствующего обновления правил доступа.
>
...
Соединитель устанавливает исходящие SSL-подключения, используя метод CONNECT. По сути он создает туннель через исходящий прокси-сервер. По умолчанию некоторые прокси-серверы разрешают исходящее туннелирование только к стандартным портам SSL, например 443. Если это относится к вашему прокси-серверу, на нем необходимо разрешить туннелирование к дополнительным портам.

В настройках прокси-сервера разрешите туннелирование к нестандартным портам SSL: 8080, 9090, 9091 и 10100–10120.
...
Не используйте проверку SSL для трафика соединителя, так как она создает проблемы.
...
Если вы введете этот фильтр в окно **Фильтр отображения** и щелкните **Применить**, собранный трафик отфильтруется по этому фильтру.
