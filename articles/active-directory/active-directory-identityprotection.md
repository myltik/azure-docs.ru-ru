...
| Читатель безопасности              | Доступ только для чтения к защите идентификации | Встроенная защита идентификации, исправление пользователей, настройщик политик, сброс паролей |
...
Aure Active Directory обнаруживает [типы событий риска](active-directory-reporting-risk-events.md#risk-event-types) в режиме реального времени. Все события риска, обнаруженные в режиме реального времени при входе пользователя в систему, формируют логическое понятие, которое называется *Вход, представляющий риск*. Вход, представляющий риск, означает, что в систему пытался войти пользователь, который не является законным владельцем учетной записи. Жизненный цикл входа, представляющего риск, завершается, когда пользователь выходит из системы.
...
Все [события риска](active-directory-identity-protection-risk-events.md), обнаруженные Azure Active Directory для отдельных пользователей, формируют логическое понятие, которое называется *Пользователи, находящиеся в группе риска*. *Пользователь, находящийся в группе риска* (или *пользователь с риском*) означает, что безопасность учетной записи пользователя, возможно, была нарушена.   
