...
| Версия | `ver` | `2.0` | Версия маркера "id\_token", как указано в Azure AD. Для конечной точки версии 2.0 значение будет равно `2.0`. |
...
Утверждение `alg` определяет алгоритм, использованный для подписания маркера, а утверждение `kid` — конкретный открытый ключ, использованный для подписания маркера.
