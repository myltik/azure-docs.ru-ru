<properties 
	...
	description="Статья для администраторов, описывающая настройку групповых политик, которые позволяют присоединять устройства к домену в корпоративной сети." 
...
1. Выполните развертывание Azure Active Directory Connect: компонент Azure AD Connect позволяет выполнять подготовку локальных компьютеров как объектов устройств в облаке. Сведения о развертывании Azure AD Connect см. в статье "Обеспечение гибридного управления для каталога с помощью Azure AD Connect".
...
## Настройка автоматической регистрации устройств с помощью групповой политики в Active Directory
