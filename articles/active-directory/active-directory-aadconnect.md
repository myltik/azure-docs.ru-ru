...
| Решение | Сценарий |
| ----- | ----- |
| Перед началом работы | <li>[Azure AD Connect: оборудование и необходимые компоненты](active-directory-aadconnect-prerequisites.md)</li> |
...
| Обновление из Azure AD Sync | <li>Это бесперебойное обновление на месте.</li>
...
Также следует подготовиться к [рабочим](active-directory-aadconnectsync-operations.md) задачам. Может потребоваться наличие резервного сервера, на который можно будет легко перейти в случае [аварии](active-directory-aadconnectsync-operations.md#disaster-recovery). Если планируются частые изменения конфигурации, следует предусмотреть сервер для [резервного режима](active-directory-aadconnectsync-operations.md#staging-mode).
