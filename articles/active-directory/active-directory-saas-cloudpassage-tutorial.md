...
Цель этого руководства — показать, как интегрировать Azure Active Directory (Azure AD) с приложением CloudPassage. Интеграция приложения CloudPassage с Azure AD обеспечивает следующие преимущества.
...
Для работы единого входа в Azure AD необходимо знать, какой пользователь в CloudPassage соответствует пользователю в Azure AD. Иными словами, необходимо установить связь между пользователем Azure AD и соответствующим пользователем в CloudPassage. Чтобы установить эту связь, следует назначить **имя пользователя** в Azure AD в качестве значения **имени пользователя** в CloudPassage.
...
Цель этого раздела — показать, как включить единый вход Azure AD на классическом портале Azure AD и настроить единый вход в приложение CloudPassage. Приложение CloudPassage ожидает проверочные утверждения SAML в определенном формате, который требует добавить настраиваемые сопоставления атрибутов в вашу конфигурацию атрибутов токена SAML. На следующем снимке экрана приведен пример.

![Настройка единого входа][21]

**Чтобы настроить единый вход Azure AD в CloudPassage, выполните следующие действия:**

1. На странице интеграции с приложением **CloudPassage** классического портала Azure AD щелкните **Настроить единый вход**, чтобы открыть диалоговое окно **Настройка единого входа**.

	![Настройка единого входа][7]

2. На странице **How would you like users to sign on to CloudPassage** (Как пользователи должны входить в CloudPassage?) выберите **Единый вход Azure AD** и нажмите кнопку **Далее**.
...
3. В диалоговом окне на странице **Настройка параметров приложения** выполните следующие действия.
    ...
    b. В текстовом поле **URL-адрес ответа** введите URL-адрес службы AssertionConsumerService (например, *https://portal.cloudpassage.com/saml/consume/accountid*). Значение этого атрибута можно получить, щелкнув **SSO Setup documentation** (Документация по настройке единого входа) в разделе **Параметры единого входа** на портале CloudPassage.![Настройка единого входа][10]

    C. Нажмите кнопку **Далее**.



4. На странице **Configure single sign-on at CloudPassage** (Настройка единого входа в CloudPassage) щелкните **Скачать сертификат**, а затем сохраните файл сертификата на локальном компьютере.
...
8. В разделе **Параметры единого входа** сделайте следующее:
    ...
    а. На диалоговой странице **Configure single sign-on at CloudPassage** (Настройка единого входа в CloudPassage) классического портала Azure скопируйте значение поля **URL-адрес издателя** и вставьте его в текстовое поле **SAML issuer URL** (URL-адрес издателя SAML).

    b. На диалоговой странице **Configure single sign-on at CloudPassage** (Настройка единого входа в CloudPassage) классического портала Azure скопируйте значение поля **Конечная точка, инициированная поставщиком услуг** и вставьте его в текстовое поле **SAML endpoint URL** (URL-адрес конечной точки SAML).

    c. На диалоговой странице **Configure single sign-on at CloudPassage** (Настройка единого входа в CloudPassage) классического портала Azure скопируйте значение поля **URL-адрес выхода** и вставьте его в текстовое поле **Logout landing page** (Целевая страница выхода).
...
11. В меню вверху щелкните **Атрибуты**, чтобы открыть диалоговое окно **Атрибуты токена SAML**.

	![Настройка единого входа][17]

12. Чтобы добавить необходимые атрибуты пользователя, для каждой строки в таблице ниже выполните следующие действия:
...
13. На панели инструментов внизу щелкните **Применить изменения**.
...
2.	В верхнем меню щелкните **Параметры**, а затем выберите **Администрирование сайта**.
...
4.	В разделе **Добавить нового пользователя** выполните следующие действия.
    ...
    c. В текстовых полях **Пользователь**, **Электронная почта** и **Введите адрес еще раз** введите имя пользователя Britta в Azure AD.

    г) Для параметра **Тип доступа** выберите **Включить доступ к порталу Halo**.
...
Цель этого раздела — проверить конфигурацию единого входа Azure AD с помощью панели доступа. Щелкнув элемент CloudPassage на панели доступа, вы автоматически войдете в приложение CloudPassage.
