...
> [AZURE.TIP] Чтобы выполнить этот запрос, щелкните ссылку ниже! После входа в браузере должно быть выполнено перенаправление на адрес `https://localhost/myapp/` с `id_token` в адресной строке. Обратите внимание на то, что в этом запросе используется `response_mode=query` (в учебных целях). Рекомендуется применять `response_mode=form_post`. <a href="https://login.microsoftonline.com/common/oauth2/v2.0/authorize?client_id=6731de76-14a6-49ae-97bc-6eba6914391e&response_type=id_token&redirect_uri=http%3A%2F%2Flocalhost%2Fmyapp%2F&scope=openid&response_mode=query&state=12345&nonce=678910" target="_blank">https://login.microsoftonline.com/common/oauth2/v2.0/authorize...</a>
...
| response\_mode | рекомендуется | Указывает метод, с помощью которого следует отправлять полученный код авторизации приложению. Может принимать значение "query", "form\_post" или "fragment". Для веб-приложений рекомендуется использовать `response_mode=form_post`, чтобы обеспечить наиболее безопасную передачу токенов в приложение.  
...
Дополнительные сведения об утверждениях в параметре id\_token см. в [справочнике по токенам конечной точки версии 2.0](active-directory-v2-tokens.md).
...
Полный процесс входа OpenID Connect и получения маркера выглядит примерно так (каждый этап подробно описан ниже).
