...
Сегодня пользователи рассчитывают на возможность единого входа, позволяющую вводить учетные данные только один раз с последующим автоматическим входом в разные приложения. Неудобство, связанное с вводом имени пользователя и пароля на маленьком экране (часто с дополнительной проверкой подлинности, например в виде телефонного звонка или текстового сообщения с кодом), причиняет дискомфорт пользователям, которым нужно выполнять эту процедуру больше одного раза.
...
*Обратите внимание, что формат этих кодов URI перенаправления описан ниже. Вы можете использовать любой код URI перенаправления, если не хотите использовать брокер. В таком случае он будет выглядеть как показано выше.*


#### Шаг 2. Настройка общего хранилища в Android

Настройка идентификатора `SharedUserID` выходит за рамки этого документа, но вы можете узнать больше о нем из документации по Google Android в разделе [manifest](http://developer.android.com/guide/topics/manifest/manifest-element.html). Важно, что именно вы решаете, как следует назвать идентификатор sharedUserID, и используете его во всех ваших приложениях.

Добавив идентификатор `SharedUserID` во все свои приложения, вы будете готовы использовать единый вход.
...
Этот код URI перенаправления нужно указать во время регистрации приложения на [классическом портале Azure](https://manage.windowsazure.com/). Дополнительные сведения о регистрации приложения Azure AD см. в статье [Интеграция с Azure Active Directory](active-directory-how-to-integrate.md).


#### Шаг 3. Настройка требуемых разрешений в приложении

В нашем приложении брокера в Android используется функция диспетчера учетных записей ОС Android для управления учетными данными в разных приложениях. Чтобы использовать брокер в приложении Android, ваш манифест приложения должен содержать разрешения на использование учетных записей AccountManager. Это подробно рассматривается [здесь, в документации Google по использованию диспетчера учетных записей](http://developer.android.com/reference/android/accounts/AccountManager.html).
