...
*Обратите внимание, что формат URI перенаправления описан ниже. Вы можете использовать любой код URI перенаправления, если не хотите использовать брокер. В таком случае он будет выглядеть как показано выше.*


#### Шаг 2. Настройка общего хранилища в Android

Настройка идентификатора `SharedUserID` выходит за рамки этого документа, но вы можете узнать больше о нем из документации по Google Android в разделе [Манифест](http://developer.android.com/guide/topics/manifest/manifest-element.html). Важно, что именно вы решаете, как следует назвать идентификатор sharedUserID, и используете его во всех ваших приложениях.
...
Пример: **msauth://com.example.userapp/IcB5PxIyvbLkbFVtBI%2FitkW%2Fejk%3D*.

Этот URI перенаправления должен быть указан при регистрации приложения на [классическом портале Azure](https://manage.windowsazure.com/). Дополнительные сведения о регистрации приложения Azure AD см. в разделе [Интеграция приложений с Azure Active Directory](active-directory-how-to-integrate.md).
