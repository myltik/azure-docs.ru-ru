...
6.	После ввода имени пользователя в Azure AD выполняется поиск соответствующего клиента. Если вы находитесь в федеративном домене, то будете перенаправлены на локальный сервер службы маркеров безопасности, например в службы федерации Active Directory (AD FS).
