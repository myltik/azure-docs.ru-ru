<properties
	pageTitle="Управление службами AD FS и их настройка с помощью Azure AD Connect | Microsoft Azure"
...
# Управление службами AD FS и их настройка с помощью Azure AD Connect

В этой статье подробно описаны различные задачи, связанные с AD FS, которые можно выполнить с помощью Azure AD Connect, а также другие стандартные задачи AD FS, выполнение которых может потребоваться для полной настройки фермы AD FS.
...
Выберите **Восстановить доверие Azure AD и AD FS** в списке доступных задач.
...
### Добавление нового WAP-сервера AD FS

> [AZURE.NOTE] Для добавления WAP-сервера Azure AD Connect требует файл сертификата PFX. Таким образом, эту операцию можно выполнить, только если ферма AD FS настроена с помощью Azure AD Connect.
...
На следующей странице добавьте сервер, который будет добавлен в качестве WAP-сервера. Поскольку WAP-сервер может быть как присоединен, так и не присоединен к домену, мастер запросит учетные данные администратора для добавляемого сервера.
...
**Правило 3. Выдача ms-ds-consistencyguid в качестве неизменяемого идентификатора при его наличии** c:[Type == "http://contoso.com/ws/2016/02/identity/claims/msdsconcistencyguid"] => issue(Type = "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier", Value = c.Value);
