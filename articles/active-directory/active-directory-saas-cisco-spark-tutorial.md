...
2. В диалоговом окне **Атрибуты токена SAML** сделайте следующее.
    ...
    c. Из списка **Значение атрибута** выберите **user.userprincipal**.
...
6. В диалоговом окне на странице **Настройка параметров приложения** выполните следующие действия.
    ...
    а. В текстовом поле URL-адрес для входа введите URL-адрес в следующем формате: `https://web.ciscospark.com/#/signin`.

    b. Нажмите кнопку **Далее**.


1. На странице **Настройка единого входа в Cisco Spark** нажмите кнопку **Скачать метаданные**, а затем сохраните файл метаданных на свой компьютер.
   
    ![Настройка единого входа](./media/active-directory-saas-cisco-spark-tutorial/tutorial_cisco_spark_09.png)
2. Войдите в [службу Cisco для управления совместной работой в облаке](https://admin.ciscospark.com/), указав учетные данные администратора с полными правами.
3. Выберите **Settings** (Параметры) и в разделе **Authentication** (Аутентификация) щелкните **Modify** (Изменить).
   
    ![Настройка единого входа](./media/active-directory-saas-cisco-spark-tutorial/tutorial_cisco_spark_10.png)
4. Выберите **Integrate a 3rd-party identity provider. (Advanced)** (Интеграция стороннего поставщика удостоверений. (Дополнительно)) и перейдите к следующему экрану.
5. Щелкните **Download Metadata File** (Скачать файл метаданных) и сохраните файл на свой компьютер.
...
4. Выберите **Names and Email address** (Имя, фамилия и электронный адрес). Затем заполните текстовые поля следующим образом.
    ...
    b. В текстовом поле **Last Name** (Фамилия) введите **Simon**.
