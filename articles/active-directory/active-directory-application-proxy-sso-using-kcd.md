...
# <a name="provide-single-sign-on-to-your-apps-with-application-proxy"></a>Реализация единого входа в приложения с помощью прокси приложения
...
2. Все проверки безопасности происходят в облаке (предварительная аутентификация).  
3. Если запрос отправляется в приложение в локальной среде, соединитель прокси-службы приложения олицетворяет пользователя. Серверная часть приложения полагает, что запрос поступил от обычного пользователя с устройства, присоединенного к домену.
...
* Сервер, на котором работает соединитель, должен иметь доступ на чтение TokenGroupsGlobalAndUniversal для пользователей. Эта стандартная настройка может измениться в случае усиления защиты среды. Дополнительную информацию об этом параметре см. в статье базы знаний [KB2009157](https://support.microsoft.com/en-us/kb/2009157).
...
4. Выберите **Доверять компьютеру делегирование указанных служб**. В разделе **Службы, с которыми эта учетная запись может использовать делегированные учетные данные** добавьте значение для идентификации имени субъекта-службы сервера приложений.
...
4. Введите **Внутреннее имя субъекта-службы приложения** сервера приложений. В этом примере таким именем для опубликованного приложения будет http/www.contoso.com.  
...
Кроме того, организации также могут работать с приложениями, которые не принимают адреса в формате адреса электронной почты. Это распространенный сценарий для внутренних серверов не на базе Windows.
...
2. В параметрах конфигурации приложения для приложения, которое вы хотите изменить, выберите нужное значение параметра **Делегированное удостоверение для входа** :
   ...
   * Имя участника-пользователя: joe@contoso.com  
   * Дополнительное имя участника-пользователя: joed@contoso.local  
   * Компонент, содержащий имя пользователя, в имени участника-пользователя: joe  
   * Компонент, содержащий имя пользователя, в дополнительном имени участника-пользователя: joed  
