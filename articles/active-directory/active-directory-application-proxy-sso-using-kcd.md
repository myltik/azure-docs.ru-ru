...
Прокси приложения предполагает, что пользователи используют одно и то же удостоверение как в облаке, так и в локальной среде. Если это не так, по-прежнему можно использовать для проверки подлинности Kerberos для единого входа. Настройте параметр **Делегированная идентификация для входа** для каждого приложения. Он обозначает, какой идентификатор следует использовать при выполнении единого входа.  
