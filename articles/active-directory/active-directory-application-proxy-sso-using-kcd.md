<properties
	pageTitle="Единый вход с помощью прокси приложения"
...
1. Настройте параметры Azure AD Connect так, чтобы основным удостоверением был адрес электронной почты (почта). Это осуществляется во время процесса настройки путем изменения поля "Имя субъекта-пользователя" в параметрах синхронизации.

![](./media/active-directory-application-proxy-sso-using-kcd/app_proxy_sso_diff_id_connect_settings.png) Примечание. Эти параметры также определяют, как именно пользователи входят в Office 365, на устройства Windows 10 устройств и в другие приложения, использующие Azure AD в качестве хранилища их удостоверений. 2. В параметрах конфигурации приложения для приложения, которое вы хотите изменить, выберите нужное значение **Делегированная идентификация для входа**:
