...
В таблице ниже описано, какие сценарии поддерживаются для каких версий наших возможностей синхронизации.  Как правило, настоятельно рекомендуется установить новейшую версию [Azure AD Connect](active-directory-aadconnect.md#install-azure-ad-connect) , если требуется использовать обратную запись паролей.
...
> [!NOTE]
> Рабочий телефон не отображается на портале регистрации, поскольку пользователи в настоящее время не могут изменять это свойство в каталоге.
>
>
...
#### <a name="data-accessible-with-azure-ad-powershell"></a>Данные, доступные при использовании Azure AD PowerShell
...
##### <a name="alternate-email"></a>Запасной адрес электронной почты
```
Connect-MsolService
...
##### <a name="authentication-phone"></a>Телефон для проверки подлинности
```
Connect-MsolService
Get-MsolUser -UserPrincipalName user@domain.com | select -Expand StrongAuthenticationUserDetails | select PhoneNumber
```

##### <a name="authentication-email"></a>Адрес электронной почты для проверки подлинности
```
Connect-MsolService
