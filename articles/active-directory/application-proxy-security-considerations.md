...
Так как он является частью Azure Active Directory, прокси приложения может использовать [защиту идентификации Azure AD](active-directory-identityprotection.md) с аналитикой на базе машинного обучения и данными из Microsoft Security Response Center и Digital Crimes Unit. Вместе мы в упреждающем режиме определяем скомпрометированные учетные записи и обеспечиваем защиту в реальном времени от входов, представляющих высокий уровень риска. Мы учитываем множество факторов, таких как доступ с зараженных устройств, из нетипичных и сомнительных расположений, а также через анонимизирующие сети.
...
2. Служба прокси приложения оценивает этот маркер. Это гарантирует, что пользователь является администратором компании в клиенте, для которого был выдан маркер. Если пользователь не является администратором, процесс завершается.
