...
В этой статье описываются различные варианты обновления установки Azure AD Connect до последней версии. Мы рекомендуем устанавливать все новые выпуски Azure AD Connect. Действия, описанные в разделе [Обновление со сменой сервера](#swing-migration), можно также использовать при значительных изменениях конфигурации.

Инструкции по обновлению DirSync см. в статье [Azure AD Connect: обновление DirSync](connect/active-directory-aadconnect-dirsync-upgrade-get-started.md).
...
Разрешения, необходимые для обновления, см. [здесь](connect/active-directory-aadconnect-accounts-permissions.md#upgrade).
...
При наличии сложного развертывания или большого числа объектов обновление системы "на месте" будет нецелесообразно. У некоторых клиентов оно может занять несколько дней, в течение которых никакие изменения синхронизироваться не будут. Этот метод также используется, если планируется значительно изменить конфигурацию и нужно проверить ее перед отправлением в облако.

В подобных случаях мы рекомендуем обновление со сменой сервера. Вам потребуется по крайней мере два сервера — активный и промежуточный. Активный сервер (сплошные синие линии на приведенной ниже схеме) отвечает за активные рабочие нагрузки. Промежуточный сервер (фиолетовые пунктирные линии на приведенной ниже схеме) используется для установки нового выпуска или настройки, по завершении чего он станет активным. Сервер, который раньше был активным и на котором теперь установлена старая версия или конфигурация, становится промежуточным и обновляется.

На двух серверах могут использоваться разные версии. Например, активный сервер, который вы планируете списать, может использовать Azure AD Sync, а новый промежуточный сервер — Azure AD Connect. Если для развертывания новой конфигурации используется обновление со сменой сервера, мы рекомендуем устанавливать одинаковые версии на двух серверах.  
![промежуточного сервера](./media/active-directory-aadconnect-upgrade-previous-version/stagingserver1.png)

Примечание. Отмечено, что некоторые клиенты предпочитают использовать для этого сценария три или четыре сервера. Когда промежуточный сервер обновляется, он не может служить резервным сервером для [аварийного восстановления](active-directory-aadconnectsync-operations.md#disaster-recovery). При наличии трех или четырех серверов можно подготовить новую пару основного и резервного серверов, в случае чего промежуточный сервер всегда будет доступен.

Эти действия подходят также для обновления Azure AD Sync или решения с FIM и соединителем Azure AD. Они не подходят для обновления DirSync. Метод обновления со сменой сервера (также называемый параллельным развертыванием) для DirSync см. в статье [Azure AD Connect: обновление DirSync](connect/active-directory-aadconnect-dirsync-upgrade-get-started.md).

### <a name="swing-migration-steps"></a>Порядок обновления со сменой сервера
1. Если вы используете Azure AD Connect на обоих серверах и хотите только изменить конфигурацию, убедитесь, что на активном и промежуточном серверах установлена одна и та же версия. Это упростит сравнение различий позже. При обновлении Azure AD Sync эти серверы имеют разные версии. При обновлении более ранней версии Azure AD Connect мы советуем использовать одинаковую версию на обоих серверах, но это необязательно.
...
### <a name="move-custom-configuration-from-active-to-staging-server"></a>Перенос пользовательской конфигурации с активного на промежуточный сервер
...
**Перенос правил синхронизации**  
Для переноса пользовательских правил синхронизации выполните следующие действия:
