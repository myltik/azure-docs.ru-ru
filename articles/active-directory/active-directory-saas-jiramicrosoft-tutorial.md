...
6. Обратитесь к [корпорации Майкрософт](mailto:waadpartners@microsoft.com), предоставив следующие данные подключаемого модуля JIRA:
    ...
    *   Azure AD Premium: да или нет (подключаемый модуль будет доступен для клиентов с номерами SKU Free, Basic и Premium);
...
9. На вкладке "Add-ons" (Надстройки) щелкните **Manage add-ons** (Управление надстройками).

    ![Настройка единого входа](./media/active-directory-saas-jiramicrosoft-tutorial/addon7.png)

10. Вручную передайте подключаемый модуль, предоставленный корпорацией Майкрософт. После установки подключаемый модуль появится в подразделе надстроек **User Installed** (Установленные пользователем) в разделе **Manage Add-on** (Управление надстройками).
...
12. Выполните следующие действия на странице настройки.
    ...
    а. В поле **Metadata URL** (URL-адрес метаданных) вставьте **URL-адрес метаданных**, созданный в Azure AD, и нажмите кнопку **Resolve** (Разрешить). Будет считан URL-адрес метаданных IdP и заполнены все поля сведений.
    ...
    c. В поле **Login Button Name** (Имя кнопки входа) введите имя кнопки, которую должны видеть на экране входа пользователи вашей организации.
    ...
    д. Если выбран параметр **User ID is in an Attribute element** (Идентификатор пользователя указан в элементе Attribute), то в текстовом поле **Attribute name** (Имя атрибута) введите имя атрибута, в котором ожидается идентификатор пользователя. 

    f. При использовании федеративного домена (например, AD FS и т. д.) для Azure AD выберите параметр **Enable Home Realm Discovery** (Включить обнаружение домашней области) и настройте **доменное имя**.
    
    g. В поле **Domain Name** (Доменное имя) введите доменное имя, если вы используете вход на основе AD FS.
