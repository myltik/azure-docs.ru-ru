...
|Имя| Обязательно/повторяется | Тип | Примечания |
|--- | ---                 | ---  | ---   |
| ****source1 ... sourceN ** | Обязательно; количество повторов может меняться | Строка | **Исходные** значения, доступные для выбора |
...
**Функция:**<br> MatchRegex(source, find, group)

**Описание:**<br> Возвращает подстроку в рамках исходного значения, которая соответствует шаблону регулярного выражения, указанному в параметре find. Если указана группа, возвращает значение только этой группы регулярных выражений.
...
| **поиск** | Обязательно | Строка | Регулярное выражение для сопоставления в рамках значения **source**. |
...
**Функция:**<br> Replace(source, find, replace)

**Описание:**<br> Заменяет все вхождения значения **find** в строке **source** значением параметра **replace**.
...
**Функция:**<br> ReplaceRegex(source, find, replace, group)

**Описание:**<br> В строке **source** заменяет все подстроки, соответствующие регулярному выражению **find**, значением **replace**. Если указан параметр **group**, будет заменено только значение этой группы регулярного выражения.
...
| **key** | Обязательно | Строка | **Параметр**, с которым сравнивается значение **source**. |
| **значение** | Обязательно | Строка | Значение, которым заменяется **исходное** значение, соответствующее параметру. |
