...
| **длина** | Обязательно | целое число | Длина подстроки. Если длина превышает размер строки **source**, функция возвращает подстроку из индекса **start** и до конца строки **source**. |
