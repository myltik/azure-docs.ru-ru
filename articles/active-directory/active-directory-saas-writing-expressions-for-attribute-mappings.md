...
* Все выражение должно определяться функциями, состоящими из имени и следующих за ним в скобках аргументов:  <br>
  *FunctionName (<< аргумент 1 >>, <<argument N>>)*
...
[Добавление](#append) &nbsp; &nbsp; &nbsp; &nbsp; [FormatDateTime](#formatdatetime) &nbsp; &nbsp; &nbsp; &nbsp; [Join](#join) &nbsp; &nbsp; &nbsp; &nbsp; [Mid](#mid) &nbsp; &nbsp; &nbsp; &nbsp; [не](#not) &nbsp; &nbsp; &nbsp; &nbsp; [Заменить](#replace) &nbsp; &nbsp; &nbsp; &nbsp; [SingleAppRoleAssignment](#singleapproleassignment) &nbsp; &nbsp; &nbsp; &nbsp; [StripSpaces](#stripspaces) &nbsp; &nbsp; &nbsp; &nbsp; [коммутатора](#switch)
...
| --- | --- | --- | --- |
| **источник** |Требуется |Строка |Как правило, имя атрибута из исходного объекта. |
| **суффикс** |Требуется |Строка |Строка, которую необходимо присоединить к концу исходного значения. |
...
| --- | --- | --- | --- |
| **источник** |Требуется |Строка |Как правило, имя атрибута из исходного объекта. |
| **входной_формат** |Требуется |Строка |Предполагаемый формат исходного значения. Список поддерживаемых форматов см. на странице [http://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx](http://msdn.microsoft.com/library/8kb3ddd4%28v=vs.110%29.aspx). |
| **выходной_формат** |Требуется |Строка |Формат вывода даты. |
...
| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **разделитель** |Требуется |Строка |Строка, используемая для разделения исходных значений, когда они объединяются в одну строку. Если разделитель не требуется, может не иметь значения (""). |
...
| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **источник** |Требуется |Строка |Как правило, имя атрибута. |
| **start** |Требуется |целое число |Индекс положения в строке **source**, откуда должна начинаться подстрока. Первый символ в строке будет иметь индекс 1, второй символ — индекс 2 и т. д. |
| **длина** |Требуется |целое число |Длина подстроки. Если длина превышает размер **исходной** строки, функция возвращает подстроку из **начала** индекса и до конца **исходной** строки. |
...
| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **источник** |Требуется |Логическая строка |Предполагаемые значения **source** : True или False. |
...
**Описание.**<br> Возвращает один appRoleAssignment из списка всех appRoleAssignments присваивается пользователю для данного приложения. Эта функция необходима для преобразования объекта appRoleAssignments в строку имени отдельной роли. Обратите внимание, что рекомендуется обеспечить только один appRoleAssignment назначается один пользователь одновременно, а не можно прогнозируемого роли строку, возвращаемую при присвоении нескольких ролей.

**Параметры:**<br> 

| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **[appRoleAssignments]** |Требуется |Строка |**[appRoleAssignments]**  объекта. |
...
| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **источник** |Требуется |Строка |**источник** , которое необходимо обновить. |
...
| ИМЯ | Обязательно/повторяется | type | Заметки |
| --- | --- | --- | --- |
| **источник** |Требуется |Строка |**Source** , которое необходимо обновить. |
...
| **key** |Требуется |Строка |Параметр **key**, с которым сравнивается значение **source**. |
| **значение** |Требуется |Строка |Значение, которым заменяется значение **source** , соответствующее ключу. |
