...
- [**По доменам**](#configure-domain-based filtering). Этот тип фильтрации можно использовать для управления свойствами соединителя SourceAD в службах синхронизации Azure AD Connect. Этот тип позволяет выбирать домены, которые можно синхронизировать в Azure AD.

- [**Настройка фильтрации по подразделениям**](#configure-organizational-unit–based filtering). Этот тип фильтрации можно использовать для управления свойствами соединителя SourceAD в службах синхронизации Azure AD Connect. Этот тип позволяет выбирать подразделения, которые можно синхронизировать в Azure AD.

- [**По атрибутам**](#configure-attribute-based-filtering). Этот метод фильтрации можно использовать для определения фильтров на основе атрибутов. Это дает возможность определять, какие объекты будут синхронизироваться в облако.

> [AZURE.NOTE]При внесении изменений в конфигурацию фильтра, если используется синхронизация паролей, необходимо запустить полную синхронизацию всех паролей после завершения настройки. Описание шагов по запуску полной синхронизации паролей см. в разделе [Запуск полной синхронизации всех паролей](active-directory-aadconnectsync-implement-password-synchronization.md#trigger-a-full-sync-of-all-passwords)

## Настройка фильтрации по доменам

В этом разделе описана процедура настройки фильтра доменов.

Если вы установили Azure AD Connect, а затем добавили или удалили в лесу домены, вам также необходимо обновить конфигурацию фильтрации.


### Изменение фильтра для существующих доменов
...
1. Войдите на компьютер, на котором запущены службы синхронизации Azure AD Connect, используя учетную запись участника группы безопасности **ADSyncAdmins**.

2. На **начальном экране** щелкните значок **Служба синхронизации**. Откроется **диспетчер службы синхронизации**.
...
7. В списке **Выберите разделы каталога** выберите только те разделы, которые должны синхронизироваться. <br> Чтобы исключить домен из процесса синхронизации, снимите флажок напротив имени соответствующего домена.
...
### Изменение фильтра для новых или удаленных доменов
Если вы добавили в лес новый домен или удалили домен из леса, конфигурацию фильтрации доменов необходимо обновить.

**Действия при добавлении домена**
...
7. Нажмите кнопку **Обновить** в верхней части страницы.

8. В списке **Выберите разделы каталога** выберите новый домен.

9. Чтобы закрыть диалоговое окно **Свойства**, нажмите кнопку «ОК».

Убедитесь, что все еще выбран тот же соединитель. Теперь необходимо настроить профили выполнения для соединителя.
...
10. Чтобы открыть диалоговое окно **Настройка профилей выполнения**, в меню **Действия** щелкните **Настроить профили выполнения**.

11. В списке **Профили выполнения соединителя** выберите профиль выполнения, который нужно настроить.

12. Чтобы открыть диалоговое окно **Настройка профиля выполнения**, нажмите кнопку **Новый шаг**.

13. На странице **Настройка шага** в списке типов шагов выберите нужный тип и нажмите кнопку **Далее**.

14. На странице **Настройка соединителя** в списке **Раздел** выберите имя раздела, добавленного в фильтр доменов.

15. Чтобы закрыть диалоговое окно **Настройка профиля выполнения**, нажмите кнопку **Готово**.

16. Чтобы закрыть диалоговое окно **Настройка профилей выполнения**, нажмите кнопку **ОК**.

17. Повторите эти действия для всех профилей выполнения.

**Действия при удалении домена**
...
8. Чтобы закрыть диалоговое окно **Свойства**, нажмите кнопку «ОК». Появится всплывающее сообщение о том, что домен был удален и конфигурация будет очищена.
...
7. Щелкните **Добавить преобразование**, для параметра **Тип потока** выберите значение **Постоянный**, для параметра «Целевой атрибут» — значение cloudFiltered, а затем в текстовом поле «Источник» введите значение True. Нажмите кнопку «Добавить», чтобы сохранить правило.
...
6. Щелкните **Добавить преобразование**, для параметра **Тип потока** выберите значение **Выражение**, а для параметра **Целевой атрибут** — значение **sourceObjectType**. В поле **Источник** введите следующее выражение:<br>`IIF(IsNullOrEmpty([department]),NULL,IIF([department]<>”Sales”,”DoNotSync”,NULL))`
