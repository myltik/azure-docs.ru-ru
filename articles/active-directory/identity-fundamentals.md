...
Так как все большее количество корпоративных цифровых ресурсов используется за пределами корпоративной сети, в облаке и на устройствах, эффективное облачное решение по управлению удостоверениями и доступом — лучший способ контроля над методом и временем получения доступа к корпоративным приложениям и данным, а также лучший способ получения сведений о подобных операциях.
...
Корпорация Майкрософт предоставляет не только удостоверение, которое можно использовать на любых ресурсах, но и набор средств для автоматизации и защиты ИТ-инфраструктуры в организации, а также для управления ею. Даже после появления облачных вычислений управление ИТ-задачами, например звонки в службу поддержки для сброса паролей пользователей, управление группами пользователей и запросы приложений, все еще пользуется спросом. Сейчас все стало еще сложнее. Теперь сотрудники приносят личные устройства для работы и используют готовые приложения SaaS. Таким образом, контроль над приложениями пользователей в корпоративных центрах обработки данных и на общедоступных облачных платформах представляет собой нетривиальную задачу.

> [!Note]
> Чтобы воспользоваться возможностями, описанными в этой статье, нужно приобрести подписку P1 или P2 Azure Active Directory отдельно или в составе [подписки E3 или E5 Enterprise Mobility + Security](https://docs.microsoft.com/enterprise-mobility-security/solutions/learn-about-ems).
...
[Azure AD Connect](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect) — это единственное средство, необходимое для интеграции. Azure AD Connect предоставляет новые возможности в соответствии с вашими требованиями к синхронизации удостоверений и заменяет старые версии средств для интеграции удостоверений, таких как DirSync и Azure AD Sync. В Azure AD Connect управление удостоверениями и синхронизация локальной среды и Azure AD обеспечивается за счет следующих операций и компонентов:
...
-   Создание единого удостоверения для каждого пользователя в пределах гибридной системы, управление этими удостоверениями, а также синхронизация пользователей, групп и устройств с помощью [Azure Active Directory Connect](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect).
