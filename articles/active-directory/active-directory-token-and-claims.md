 <properties
   pageTitle="Поддерживаемые токены и типы утверждений | Microsoft Azure"
...
В токене JWT имя отображается в утверждении given\_name.
...
В токене JWT фамилия отображается в утверждении family\_name.
...
В токене JWT имя отображается в утверждении unique\_name.
...
Утверждение Scope токена обозначает предоставленные клиентскому приложению разрешения на олицетворение. По умолчанию используется разрешение user\_impersonation. Владелец защищаемого ресурса может регистрировать дополнительные значения в Azure AD.
