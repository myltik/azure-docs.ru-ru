...
## <a name="id_tokens"></a>Маркеры id_token
...
#### <a name="sample-id_token"></a>Пример маркера id_token
...
> [AZURE.TIP] В целях обучения попробуйте изучить утверждения в примере маркера идентификации, вставив его в [calebb.net](http://jwt.calebb.net).

#### <a name="claims-in-id_tokens"></a>Утверждения в маркерах id_token
...
Маркеры обновления используются для нескольких ресурсов, то есть маркеры обновления, полученные при запросе маркера для одного ресурса, можно обменять на маркеры доступа к совершенно другому ресурсу. Чтобы указать возможность использования для нескольких ресурсов, в запросе к целевому источнику необходимо задать параметр `resource`.
...
В настоящее время клиентским приложениям нужно проверять только токены id_token.  Для проверки маркера "id_token" приложение должно проверить подпись маркера и содержащиеся в нем утверждения.

Мы предоставляем библиотеки и примеры кода, демонстрирующие удобный способ проверки маркеров, если вам интересно понять суть происходящего.  Для проверки маркеров JWT доступны также несколько сторонних библиотек с открытым кодом. Среди них вы найдете как минимум один вариант для каждой платформы и языка. Дополнительные сведения о библиотеках аутентификации Azure AD и примеры кода см. в статье [Библиотеки проверки подлинности Azure Active Directory](active-directory-authentication-libraries.md).

#### <a name="validating-the-signature"></a>Проверка подписи

Маркер JWT содержит три сегмента, разделенных символом `.` .  Первый сегмент называется **заголовком**, второй — **телом**, а третий — **подписью**.  Сегмент подписи можно использовать для проверки подлинности маркера "id_token", чтобы приложение доверяло ему.

Маркеры "id_token" подписываются при помощи стандартных отраслевых алгоритмов асимметричного шифрования, таких как RSA 256. Заголовок маркера "id_token" содержит сведения о ключе и методе шифрования, используемых для подписания маркера:
...
Когда приложение получает маркер "id_token" после входа пользователя, ему также требуется выполнить несколько проверок утверждений в этом маркере.  Помимо прочего, к ним относятся:

  - Утверждение **Аудитория**. Необходимо для подтверждения того, что маркер id_token действительно должен был быть выдан вашему приложению.
  - Утверждения **Не до** и **Время окончания срока действия**. Необходимы для подтверждения того, что срок действия маркера идентификации не истек.
...
Подробные сведения об ожидаемых значениях для этих утверждений можно найти выше в разделе о [маркерах id_token](#id-tokens).
