...
- При запуске клиентов Office, особенно на общих компьютерах, выдаются дополнительные приглашения на вход в систему для пользователей. Потребуется часто вводить имя пользователя, но не пароль.
- После успешного единого входа пользователю не предоставляется возможность выбрать вариант "Оставаться в системе". Из-за этого поведения сценарии сопоставления SharePoint и OneDrive не поддерживаются.
