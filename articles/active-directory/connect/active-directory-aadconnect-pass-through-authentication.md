...
- Простота в использовании
  ...
  - У локального соединителя есть возможность автоматического обновления, что позволяет автоматически получать обновления функций и исправления ошибок.
...
>[!IMPORTANT]
>В качестве временного решения для сценариев, не поддерживаемых сейчас для функции сквозной проверки подлинности (клиентских приложений Office прежних версий, Exchange ActiveSync и присоединения устройств Windows 10 к Azure AD), по умолчанию при включении сквозной проверки подлинности также включается синхронизация паролей. Синхронизация паролей выступает в качестве запасного варианта только для этих конкретных сценариев. Если она вам не требуется, вы можете ее отключить на странице [дополнительных возможностей](active-directory-aadconnect-get-started-custom.md#optional-features) в мастере Azure AD Connect.
...
>[!NOTE]
>Мы рекомендуем использовать только облачную учетную запись глобального администратора, позволяющую управлять конфигурацией клиента в случае сбоя локальных служб или отсутствия к ним доступа. Можно добавить облачную учетную запись глобального администратора, как показано [здесь](../active-directory-users-create-azure-portal.md).

- Версию 1.1.486.0 для Azure AD Connect или более позднюю. Рекомендуется использовать [последнюю версию Azure AD Connect](https://www.microsoft.com/download/details.aspx?id=47594).
- Сервер под управлением Windows Server 2012 R2 или более поздней версии для запуска Azure AD Connect.
  ...
  - Обратите внимание, что соединитель сквозной проверки подлинности устанавливается на том же сервере, что и Azure AD Connect. У соединителя должна быть версия 1.5.58.0 или более поздняя.
...
- Если требуется обеспечить высокий уровень доступности, потребуются дополнительные серверы под управлением Windows Server 2012 R2 или более поздней версии, чтобы установить изолированные соединители (для этого необходима версия 1.5.58.0 или более поздняя).
...
1.    [Скачайте](https://go.microsoft.com/fwlink/?linkid=837580) последнюю версию соединителя. У соединителя должна быть версия 1.5.58.0 или более поздняя.
...
У вас должен быть хотя бы один активный соединитель, чтобы включить сквозную проверку подлинности в клиенте. Вы можете установить соединитель с помощью установки Azure AD Connect или изолированного соединителя.
...
### <a name="collecting-pass-through-authentication-connector-logs"></a>Сбор журналов соединителя сквозной проверки подлинности
