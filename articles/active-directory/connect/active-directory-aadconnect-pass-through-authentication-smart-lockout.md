...
Azure Active Directory (Azure AD) обеспечивает защиту от атак методом подбора пароля и предотвращает потерю доступа разрешенных пользователей к приложениям Office 365 и SaaS. Эта функция, которая называется *смарт-блокировкой*, поддерживается при входе с помощью сквозной проверки подлинности. Смарт-блокировка по умолчанию включена для всех клиентов. Она обеспечивает непрерывную защиту учетных записей пользователей.
...
>[!NOTE]
>Функция смарт-блокировки предоставляется бесплатно. Она _включена_ по умолчанию для всех пользователей. Но если нужно изменить пороговое значение блокировки Azure AD и продолжительность блокировки с помощью API Graph, вашему клиенту понадобится как минимум одна лицензия Azure AD Premium P2. Чтобы получить возможность смарт-блокировки с помощью сквозной аутентификации, не требуется лицензия Azure AD Premium P2 для _каждого пользователя_.
