...
* Чтобы синхронизировать группу Active Directory в Azure AD как группу с поддержкой электронной почты, выполните следующие действия:
    ...
    * Если атрибут группы *proxyAddress* не пуст, он должен содержать также значение адреса основного прокси-сервера SMTP (обозначенное префиксом **SMTP** в верхнем регистре). Вот несколько примеров:
    
      * Группа Active Directory, атрибут proxyAddress которой имеет значение *{"X500:/0=contoso.com/ou=users/cn=testgroup"}*, не будет поддерживать электронную почту в Azure AD. У нее нет основного SMTP-адреса.
      
      * Группа Active Directory, атрибут proxyAddress которой имеет значения *{"X500:/0=contoso.com/ou=users/cn=testgroup", "smtp:johndoe@contoso.com"}*, не будет поддерживать электронную почту в Azure AD. У нее есть SMTP-адрес, но он не является основным.
