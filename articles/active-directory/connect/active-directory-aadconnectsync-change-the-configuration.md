...
По умолчанию ресурсы Office 365 для пользователей находятся в одном регионе с клиентом Azure AD. Например если клиент находится в Северной Америке почтовые ящики Exchange пользователей также расположен в Северной Америке. Для глобальной экономике организации это может быть неоптимальным решением. Установив атрибут preferredDataLocation области пользователя может быть определена.

Области в Office 365 — это:
...
| JPN | Япония |
| СИДНЕЙСКОЕ | Австралия |
| МОЖНО | Канада |
| GBR | Великобритания |
| LAM | Латинская Америка |

Не все рабочие нагрузки Office 365 поддерживает использование параметра области пользователя.
...
* Схема типа объекта **пользователя** в соединителя Azure AD будет расширено и включит PreferredDataLocation атрибута, который является однозначным строкового типа.
...
> [!IMPORTANT]
> С момента 1 сентября 2017 г., Azure AD не допускает атрибут PreferredDataLocation на **синхронизировать объекты пользователя** напрямую можно задать с помощью Azure AD PowerShell. Чтобы настроить атрибут PreferredLocation синхронизированных объектов-пользователей, необходимо использовать Azure AD Connect.

Перед включением синхронизации атрибута PreferredDataLocation необходимо выполнить следующее.

 * Сначала определите, какой атрибут локального каталога Active Directory следует использовать в качестве исходного атрибута. Оно должно иметь тип **однозначный строка**. В шагах ниже одного из extensionAttributes используется.
...
2. Отключить синхронизацию по расписанию с помощью командлета: `Set-ADSyncScheduler -SyncCycleEnabled $false`.
3. Запуск **Synchronization Service Manager** последовательно выбрав пункты **запустить** > **служба синхронизации**.
4. Последовательно выберите пункты **операции** вкладку и подтвердите нет операции с состоянием *выполняется*.
...
Не все атрибуты AD импортируются в пространство локального соединителя AD. Если вы решили использовать атрибут, не синхронизирована по умолчанию, необходимо импортировать его. Чтобы добавить исходный атрибут в список импортируемых атрибутов, сделайте следующее.

1. Откройте вкладку **Connectors** (Соединители) в Synchronization Service Manager.
2. Щелкните правой кнопкой мыши **локального соединителя AD** и выберите **свойства**.
3. Во всплывающем диалоговом окне перейдите на вкладку **Select Attributes** (Выбор атрибутов).
4. Убедитесь, что установлен флажок исходный атрибут, выбранный для использования в списке атрибутов.
...
По умолчанию атрибут PreferredDataLocation не импортирован в пространство соединителя Azure AD. Вот как можно добавить атрибут PreferredDataLocation в список импортируемых атрибутов.

1. Откройте вкладку **Connectors** (Соединители) в Synchronization Service Manager.
2. Щелкните правой кнопкой мыши **соединителя Azure AD** и выберите **свойства**.
3. Во всплывающем диалоговом окне перейдите на вкладку **Select Attributes** (Выбор атрибутов).
...
1. Запуск **редактор правил синхронизации** последовательно выбрав пункты **запустить** > **редактор правил синхронизации**.
...
4. На вкладке **Описание** укажите следующую конфигурацию.
    ...
    | ОПИСАНИЕ | *Введите описание, пользовательские* |  |
...
5. Сохранить **фильтр области** пустыми, чтобы включить все объекты. Возможно, вам необходимо настроить фильтр области в соответствии с развертыванием Azure AD Connect.
...
1. Выполните шаг **полного импорта** для **локального соединителя AD**.
   ...
   2. Щелкните правой кнопкой мыши **локального соединителя AD** и выберите **запуска...** .
...
Исходный атрибут из локального Active Directory и PreferredDataLocation из Azure AD, были импортированы в пространстве соответствующего соединителя. Перед выполнением шага полную синхронизацию, рекомендуется выполнить **предварительного просмотра** на существующего пользователя объекта в локальных пространство соединителя AD. У выбранного объекта должен быть заполнен исходный атрибут. Если при **предварительном просмотре** атрибут PreferredDataLocation успешно заполнен, то это хороший показатель того, что вы верно настроили правила синхронизацию. Сведения о том, как выполнить **предварительный просмотр**, см. в разделе [Проверка изменений](#verify-the-change).

4. Выполните шаг **полной синхронизации** для **локального соединителя AD**.

   1. Щелкните правой кнопкой мыши **локального соединителя AD** и выберите **запуска...** .
...
5. Проверьте **ожидающие операции экспорта** в Azure AD.
   ...
   1. Щелкните правой кнопкой мыши **соединителя Azure AD** и выберите **поиска пространства соединителя**.
...
6. Выполните шаг **экспорта** для **соединителя Azure AD**.
   ...
   1. Щелкните правой кнопкой мыши **соединителя Azure AD** и выберите **запуска...** .
...
> [!NOTE]
> Вы можете заметить, что шаги не содержат действие полной синхронизации для соединителя Azure AD и экспорт в соединителе AD. Они и не требуются, так как значения атрибутов передаются только из локального каталога Active Directory в Azure AD.
...
## <a name="enable-synchronization-of-usertype"></a>Включение синхронизации UserType
Azure AD Connect поддерживает синхронизацию **UserType** для атрибута **пользователя** объектов в версии 1.1.524.0 и после. В частности, были введены следующие изменения.

- Схема типа объекта **пользователя** в соединителя Azure AD будет расширено и включит пользовательского атрибута, который имеет строковый тип и одно значение.
- Схема типа объекта **лица** в метавселенной будет расширено и включит пользовательского атрибута, который имеет строковый тип и одно значение.

По умолчанию пользовательского атрибута не включен для синхронизации, так как отсутствует соответствующий атрибут UserType в локальной Active Directory. Необходимо вручную включить синхронизацию. Перед включением синхронизации пользовательского атрибута, необходимо будет выполнить запишите следующее поведение принудительно с помощью Azure AD.

- Azure AD принимает только два значения для атрибута UserType – **член** и **гостевой**.
- Если атрибут UserType для синхронизации в Azure AD Connect не включена, пользователи Azure AD, созданные с помощью синхронизации службы каталогов будет иметь UserType набор атрибутов для **член**.
- Azure AD не разрешает атрибут UserType на существующих пользователей Azure AD для изменения Azure AD Connect. Его можно задать только во время создания пользователи Azure AD.

Перед включением синхронизации атрибута UserType, сначала необходимо решить, каким образом UserType, атрибут будет получен из локальной AD. Два типичные подходы включают:

- Назначить неиспользуемым в локальной среде AD атрибут (например, extensionAttribute1) для использования в качестве исходного атрибута. Указанной в локальной среде AD атрибут должен иметь тип **строка**, является однозначным и содержит значение **член** или **гостевой**. Если выбран этот подход, необходимо убедиться, что указанный атрибут заполняется правильное значение для всех существующих объектов пользователей в локальной Active Directory, синхронизируемые с Azure AD перед включением синхронизации пользовательского атрибута .
- Кроме того можно получить значение для атрибута UserType от других свойств. Например, вы хотите синхронизировать всех пользователей как у гостя при свои локальные атрибут AD UserPrincipalName заканчивается доменная часть "@partners.fabrikam123.org«. Как упоминалось ранее, Azure AD Connect не допускает атрибут UserType на существующих пользователей Azure AD для изменения Azure AD Connect. Таким образом Убедитесь, что логику, было принято согласуется с как атрибут UserType уже настроена для всех существующих пользователей Azure AD в клиенте.

Действия, чтобы включить синхронизацию пользовательского атрибута можно вкратце описать следующим образом:

>[!NOTE]
> В оставшейся части этого раздела описываются следующие действия. Они описаны в контексте развертывания Azure AD в топологии с отдельным лесом и без пользовательских правил синхронизации. Если используется топология с несколькими лесами, настроенными пользовательскими правилами синхронизации или промежуточным сервером, то необходимо соответствующим образом изменить действия.

1.  Отключить **синхронизации планировщика** и убедитесь в наличии синхронизация не выполняется
2.  Добавить **исходный атрибут** для локального соединителя AD схемы
3.  Добавить **UserType** схему соединителя Azure AD
4.  Создайте правило входящей синхронизации для передачи значения атрибута из локального каталога Active Directory.
5.  Создайте правило исходящей синхронизации для передачи значения атрибута в Azure AD.
6.  Запустите **полная синхронизация** цикла
7.  Включить **синхронизации планировщика**
...
Не все атрибуты AD импортируются в пространство локального соединителя AD. Чтобы добавить исходный атрибут в список импортируемых атрибутов, сделайте следующее.
 ...
 5. Нажмите кнопку **OK**, чтобы сохранить изменения.
![Добавьте исходный атрибут для локального соединителя AD схемы](./media/active-directory-aadconnectsync-change-the-configuration/usertype1.png)

### <a name="step-3-add-usertype-to-the-azure-ad-connector-schema"></a>Шаг 3: Добавление UserType схемы соединителя Azure AD
По умолчанию атрибут UserType не импортируется в Azure AD подключения пространство. Чтобы добавить атрибут UserType список импортированных атрибутов:
...
4. На вкладке **Описание** укажите следующую конфигурацию.
    ...
    | Атрибут | Значение | Сведения |
    | --- | --- | --- |
    | ИМЯ | *Укажите имя* | Например *» в from AD – UserType пользователя»* |
    | ОПИСАНИЕ | *Укажите описание* |  |
    | Подключенная система | *Выберите локальный соединитель AD*. |  |
    | Тип объекта подключенной системы | **User** |  |
    | Тип объекта метавселенной | **Person** |  |
...
5. Перейдите на вкладку **Scoping filter** (Фильтр области) и добавьте **отдельную группу фильтра области со следующим предложением**.
    ...
    Фильтр области определяет локальные объекты AD, к которым применяется данное правило входящей синхронизации. В этом примере мы используем же фильтр области используется в качестве» в from AD – общий пользователя» правила синхронизации OOB, запрещающую правила синхронизации применяются пользовательские объекты, созданные с помощью функции обратной записи в Azure AD пользователя. Возможно, вам необходимо настроить фильтр области в соответствии с развертыванием Azure AD Connect.

6. Последовательно выберите пункты **вкладка преобразование** и реализовать правило нужное преобразование. Например, назначенный неиспользуемым в локальной среде AD атрибут (например, extensionAttribute1) как атрибут источника для UserType, можно реализовать поток прямой атрибутов:

    | Тип потока | Целевой атрибут | Источник | Применить однократно | "Merge Type" (Тип объединения) |
    | --- | --- | --- | --- | --- |
    | Напрямую | UserType | extensionAttribute1 | Флажок снят. | Блокировка изменений |

    Другой пример — требуется получить значение для атрибута UserType от других свойств. Например, вы хотите синхронизировать всех пользователей как у гостя при свои локальные атрибут AD UserPrincipalName заканчивается доменная часть "@partners.fabrikam123.org«. Можно реализовать выражение.

    | Тип потока | Целевой атрибут | Источник | Применить однократно | "Merge Type" (Тип объединения) |
    | --- | --- | --- | --- | --- |
    | Напрямую | UserType | IIf(IsPresent([userPrincipalName]),IIf(CBool(InStr(LCase([userPrincipalName])»@partners.fabrikam123.org»)=0), «Элемент», «Гость»), ошибка («UserPrincipalName не присутствует, чтобы определить UserType»)) | Флажок снят. | Блокировка изменений |
...
4. На вкладке **Описание** укажите следующую конфигурацию.
    ...
    | Атрибут | Значение | Сведения |
    | ----- | ------ | --- |
    | ИМЯ | *Укажите имя* | Например «Out для AAD — пользователь UserType» |
    | ОПИСАНИЕ | *Укажите описание* ||
    | Подключенная система | *Выберите соединитель AAD* ||
    | Тип объекта подключенной системы | Пользователь ||
    | Тип объекта метавселенной | **Person** ||
...
1. Выполните шаг **полного импорта** для **локального соединителя AD**.
   ...
   4. Дождитесь завершения операции.
    ...
    > [!NOTE]
    > Вы можете пропустить полный импорт на локального соединителя AD, если уже имеется исходный атрибут в списке импортированы атрибуты. Другими словами, можно было не вносить изменения во время шага 2 [Добавление исходного атрибута в схему локального соединителя AD](#step-2-add-the-source-attribute-to-the-on-premises-ad-connector-schema).
...
3. Проверьте изменения правила синхронизации на существующем объекте User.
    ...
    Исходный атрибут из локального Active Directory и UserType из Azure AD, были импортированы в соответствующие места соединитель. Перед выполнением шага полной синхронизации рекомендуется выполнить **предварительный просмотр** существующего объекта User в пространстве локального соединителя AD. У выбранного объекта должен быть заполнен исходный атрибут. Успешное выполнение **предварительного просмотра** с UserType в метавселенной заполняется является хорошим показателем, что вы настроили синхронизацию правила правильно. Сведения о том, как выполнить **предварительный просмотр**, см. в разделе [Проверка изменений](#verify-the-change).
...
6. Выполните шаг **экспорта** для **соединителя Azure AD**.
   ...
   1. Щелкните правой кнопкой мыши **соединитель Azure AD** и выберите команду **Run** (Выполнить).
   2. Во всплывающем диалоговом окне "Run Connector" (Запуск соединителя) выберите **Экспорт** и нажмите кнопку **ОК**.
   3. Дождитесь экспорта в Azure AD для выполнения.
