...
* Lotus Notes:
  ...
  * Импорт класса ImportOperations исправлена определения операций, которые могут выполняться в режиме «Представления» и в режиме «Поиск».
...
* Универсальный соединитель SQL:</br>
  **Сценарий симптомы:** это хорошо известное ограничение с соединителем SQL здесь мы только разрешить ссылку на один тип объектов и требуют перекрестная ссылка с участниками. </br>
  **Описание решения:** на этапе обработки для ссылок были «*» выбрано значение механизму синхронизации будут возвращены все комбинации типов объектов.
 ...
 **Сценарий:** при выборе только несколько контейнеров в определенной секции, затем поиск по-прежнему будет выполняться в весь раздел. Этот определенный раздел будет фильтроваться по службе синхронизации, а не по агенту управления, что может привести к снижению производительности. </br>
...
* Lotus Domino.
  ...
  **Решение:** поддержки удаления можно настроить почту для удаления пользователя во время экспорта.
