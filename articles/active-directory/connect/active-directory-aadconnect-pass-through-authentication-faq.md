...
Нет, эта конфигурация _не_ поддерживается.
...
Но если обратная запись паролей не настроена на клиенте или пользователю не назначена действительная лицензия Azure AD, пользователь не сможет обновить свой пароль в облаке. Он не сможет обносить свой пароль, даже если истек срок его действия. Пользователь увидит следующее сообщение: "Ваша организация запрещает обновлять пароль на этом сайте. Обновите пароль рекомендованным организацией способом или обратитесь за помощью к администратору". Пользователь или администратор должен сбросить пароль в локальной службе Active Directory.
...
- Агенты проверки подлинности выполняют HTTPS-запросы через порт 443 для всех операций с функцией, например включение функции, обработка всех запросов на вход пользователей и т. д.
- Агенты проверки подлинности выполняют HTTP-запросы через порт 80 для скачивания списков отзыва SSL-сертификатов.

     >[!NOTE]
     >В рамках последних обновлений мы сократили количество портов, необходимых агентам проверки подлинности для взаимодействия с Azure AD. Если вы запускаете предыдущие версии Azure AD Connect и (или) изолированных агентов проверки подлинности, следует держать открытыми следующие дополнительные порты: 5671, 8080, 9090, 9091, 9350, 9352, 10100–10120.
...
- Общее количество установленных агентов проверки подлинности не должно превышать два или три экземпляра. Этого будет достаточно для большинства клиентов.
...
Обратите внимание на системное ограничение — 12 агентов проверки подлинности для каждого клиента.
