...
![Страница "Фильтрация домена и подразделения"](./media/active-directory-aadconnect-get-started-custom/domainoufiltering.png) На этой странице мастера настраивается фильтрация по доменам. Дополнительные сведения см. в разделе [Фильтрация по доменам](../active-directory-aadconnectsync-configure-filtering.md#domain-based-filtering).
...
#### <a name="creating-the-computer-account-in-active-directory"></a>Создание учетной записи компьютера в Active Directory
Для каждого леса, добавленного с помощью средства AAD Connect, необходимо указать учетные данные администратора домена. Таким образом в каждом из них будет создана учетная запись компьютера.  Учетные данные используются только для создания учетной записи и не хранятся, а также не используются для выполнения любой другой операции.  Просто добавьте учетные данные на странице "Включить единый вход" мастера AAD Connect, как показано ниже:
...
Чтобы убедиться, что клиент автоматически подключается к зоне интрасети, проверьте, входят ли в нее URL-адреса.  Таким образом при подключении к корпоративной сети присоединенный к домену настольный компьютер будет автоматически отправлять билет Kerberos.
...
3.  Перейдите в раздел "Текущий пользователь\Административные шаблоны\Компоненты Windows\Internet Explorer\Панель управления браузером\Вкладка безопасности" и выберите "Список назначений зоны для веб-сайтов", как на рисунке ниже.
...
5.  Это должно выглядеть следующим образом: ![Зоны интрасети](./media/active-directory-aadconnect-get-started-custom/sitezone.png)
