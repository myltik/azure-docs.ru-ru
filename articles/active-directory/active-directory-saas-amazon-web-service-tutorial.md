...
17. В диалоговом окне **Establish Trust** (Установка доверия) выполните следующие действия.  
    ...
    a. В качестве поставщика SAML выберите поставщик SAML, созданный ранее (например, *WAAD*). 
...
22. Щелкните **Services** -> **Security, Identity & Compliance** -> **IAM** (Службы -> Безопасность, удостоверения и соответствие требованиям -> IAM).

    ![Получение ролей из учетной записи AWS](./media/active-directory-saas-amazon-web-service-tutorial/fetchingrole1.png)

23. Выберите вкладку **Policies** (Политики) в разделе IAM.

    ![Получение ролей из учетной записи AWS](./media/active-directory-saas-amazon-web-service-tutorial/fetchingrole2.png)

24. Создайте новую политику, щелкнув **Create policy** (Создать политику).
...
28. Теперь создайте новую политику для этого пользователя, выполнив следующие действия.
...
30. Скачайте учетные данные пользователя, выполнив следующие действия.
...
31. Перейдите к разделу **User Provisioning** (Подготовка пользователя) приложения Amazon Web Services на портале управления Azure AD.
...
33. Теперь **включите** параметр "Состояние подготовки" в разделе "Параметры" и нажмите кнопку **Сохранить** вверху.
