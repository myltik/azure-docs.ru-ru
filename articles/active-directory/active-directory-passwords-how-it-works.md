...
#### <a name="changing-authentication-methods"></a>Изменение методов проверки подлинности
...
Пример: 
1. В исходной политике настроено 2 обязательных метода проверки подлинности, где используются только рабочий телефон и контрольные вопросы. 
2. Администратор изменяет политику таким образом, чтобы больше не использовались контрольные вопросы, но использовались мобильный телефон и запасной адрес электронной почты.
...
>
> Если они уже выполнили вход, то это не нарушит подключение пользователей.
...
Установив, настроив и включив Azure AD Connect, вы получите приведенные ниже дополнительные параметры интеграции с локальной средой. Если эти параметры выделены серым цветом, то это значит, что неправильно настроена обратная запись паролей. Ознакомьтесь с дополнительными сведениями в разделе [Настройка компонента обратной записи паролей](active-directory-passwords-writeback.md#configuring-password-writeback).
...
> [!NOTE]
> Учетные записи Майкрософт, которым был предоставлен гостевой доступ к клиенту Azure AD, например адреса из Hotmail.com, Outlook.com или другие личные адреса электронной почты, не смогут использовать Azure AD SSPR. Им потребуется сбросить свой пароль, как описано в статье [Не удается войти в учетную запись Майкрософт](https://support.microsoft.com/help/12429/microsoft-account-sign-in-cant).
