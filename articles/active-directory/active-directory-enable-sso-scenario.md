<properties
   pageTitle="Управление приложениями с помощью Azure Active Directory | Microsoft Azure"
...
- *Кто* — это *служба идентификации*, хранилище данных, состоящее из пользователей и групп.

- *Что* — это *управление доступом* к защищенным ресурсам.

Оба компонента вместе называются *Identity and Access Management (IAM)*. Это система, для которой компания [Gartner](http://www.gartner.com/it-glossary/identity-and-access-management-iam) разработала следующее определение: «*дисциплина безопасности, предоставляющая нужным пользователям доступ к нужным ресурсам в нужное время по нужным причинам*».
...
- Пользователи должны запоминать несколько наборов учетных данных для доступа к приложениям, необходимым для работы, поскольку они могут не использовать один и тот же пароль для всех приложений в целях безопасности. В результате они записывают свои пароли или используют иные решения для управления паролями, связанные с другими угрозами безопасности данных.
...
## Интеграция Azure Active Directory с приложениями

Azure Active Directory (AD) — это комплексное решение Майкрософт типа «идентификация как услуга» (IDaaS), позволяющее использовать IAM как облачную службу и включающее встроенное управление доступом, единый вход (SSO) и создание отчетов. Это решение [предварительно интегрировано с тысячами приложений](https://azure.microsoft.com/marketplace/active-directory/), включая Salesforce, Google Apps, Box, Concur и многие другие. Благодаря Azure AD приложения, которые вы публикуете для партнеров и клиентов (деловых партнеров или потребителей), имеют одинаковые возможности управления удостоверениями и доступом. Поэтому вы можете сосредоточиться на своих основных делах.

Ценность Azure AD не ограничивается облачными приложениями. Эту службу также можно использовать с локальными приложениями, предоставляя безопасный удаленный доступ к ним без необходимости использовать VPN или другую традиционную систему управления удаленным доступом.
...
Обеспечивая централизованное управление доступом и единый вход (SSO) для всех приложений, Azure AD позволяет решить проблемы производительности и безопасности данных.
...
## Преимущества интегрированных приложений
...
С помощью Azure AD можно защитить приложения, используя настраиваемые политики доступа и предварительно интегрированную службу MFA. Дополнительную информацию об Azure MFA см. на странице [Azure MFA](https://azure.microsoft.com/services/multi-factor-authentication/).
