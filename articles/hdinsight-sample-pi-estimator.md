...
Другие доступные примеры, которые помогут вам увеличить скорость при использовании HDInsight для выполнения заданий MapReduce, а также ссылки на инструкции по их выполнению см. в разделе [Выполнение примеров HDInsight][].
...
2.  [Код Java для программы MapReduce по оценке числа пи][]
3.  [Сводка][]
...
3.  Выполните следующую команду, чтобы создать определение MapReduce:
    ...
    Первый аргумент указывает, сколько задач map нужно создать (по умолчанию 16). Второй аргумент указывает, сколько образцов создается для каждой задачи map (по умолчанию 10 миллионов). То есть в этой программе используется 16\*10 миллионов = 160 миллионов произвольных точек для оценки числа пи.
...
Здесь приводится полный скрипт, используемый для выполнения примера в кластере HDInsight 2.1 по умолчанию. (Чтобы выполнить пример в кластере HDInsight 3.0 ,необходимо только изменить имя jar-файла: hadoop-examples.jar becomes hadoop-mapreduce-examples.jar.)
