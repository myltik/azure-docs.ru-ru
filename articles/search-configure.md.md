...
# Настройка Поиска в портале предварительной версии Azure

[WACOM.INCLUDE [В этой статье используется портал предварительной версии Azure][]]

В новом портале предварительной версии Azure стал доступен Поиск Azure (общедоступная предварительная версия) Как администратор, вы можете бесплатно добавить службу Поиска в существующую подписку при выборе общей службы либо по сниженной цене при выборе службы для выделенных ресурсов. Данная статья состоит из следующих разделов:
...
-   [Обзор страниц конфигурации службы Поиска][]
-   [Попробуйте сейчас][]
...
1.  Войдите на [портал предварительной версии Azure][] с помощью существующей учетной записи подписки. Обратите внимание, что по ссылке производится переход на портал предварительной версии.
...
Перейдите к разделу [Операции тестирования службы][], чтобы получить инструкции по подключению к службе с использованием этих значений.
...
1.  Войдите на [портал предварительной версии Azure][] с помощью существующей учетной записи подписки.
...
Подтверждение того, что ваша служба находится в рабочем состоянии и доступна из клиентского приложения, является заключительным этапом в настройке Поиска. В данной процедуре для формирования HTTP-запросов и просмотра ответов используется отладчик Fiddler, его можно [бесплатно загрузить с Telerik][]. Используя Fiddler, можно тестировать API непосредственно, без необходимости написания кода.
...
1.  Укажите заголовок запроса, заменив адрес узла и ключ API (строчные буквы) значениями, соответствующими вашей службе.
...
Если вы получите код HTTP 504, проверьте, что в URL указан протокол HTTPS. Если вы получите код HTTP 404, еще раз проверьте синтаксис. Код HTTP 400 означает, что возникла ошибка с ключом API (либо он недействительный, либо ошибка синтаксиса при указании ключа).
...
Ниже приведен пример запроса из [Работа с поисковым индексом (API Поиска Azure)][] на MSDN. Многие примеры запросов в данном разделе включают пробелы, что является недопустимым в Fiddler. Замените все пробелы символом + перед вставкой в строку запроса и его выполнением в Fiddler:
...
[Создание первого решения для поиска Azure][]

[Управление решением Поиска в Microsoft Azure][]

[Технические сведения о Поиске Azure][]

[API REST Поиска Azure][]
...
<!--Anchors-->  
  ...
  [Обзор страниц конфигурации службы Поиска]: #sub-4
...
<!--Image references-->
  ...
  [Работа с поисковым индексом (API Поиска Azure)]: http://msdn.microsoft.com/en-us/library/dn798927.aspx
...
<!--Link references-->
  ...
  [Технические сведения о Поиске Azure]: http://msdn.microsoft.com/en-us/library/dn798933.aspx
  [API REST Поиска Azure]: http://msdn.microsoft.com/en-us/library/dn798935.aspx
