<properties 
	pageTitle="Как использовать AMQP 1.0 с API .NET служебной шины Azure" 
	description="Узнайте, как использовать протокол Advanced Message Queuing Protocol(AMQP) 1.0 c API .NET служебной шины Azure." 
...
В этом руководстве предполагается, что вы уже создали пространство имен служебной шины, содержащее очередь с именем queue1. Если это не так, можно создать пространство имен и очередь, используя [портал управления Azure](http://manage.windowsazure.com). Для получения дополнительной информации о создании пространства имен и очередей служебной шины см. практическое руководство под названием [Как использовать очереди служебной шины](https://azure.microsoft.com/develop/net/how-to-guides/service-bus-queues/).
...
где [пространство имен] и [ключ SAS] принимаются с портала управления Azure. Для получения подробной информации см. [Использование очередей служебной шины][].
...
Дополнительные сведения см. в статье [Протокол AMQP 1.0 для Service Bus. Руководство разработчика](http://msdn.microsoft.com/library/jj841071.aspx). В данном разделе приводится подробный список неподдерживаемых интерфейсов API.
...
Протокол AMQP 1.0 служебной шины можно также использовать из других языков, в числе которых Java, C, Python и PHP. Компоненты, созданные с помощью этих языков, могут надежно и точно обмениваться сообщениями, используя AMQP 1.0 в служебной шине. Дополнительные сведения см. в статье [Протокол AMQP 1.0 для Service Bus. Руководство разработчика](http://msdn.microsoft.com/library/jj841071.aspx).
