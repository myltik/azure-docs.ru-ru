...
## <a name="enable"></a>Включение прокси Функций Azure
...
### <a name="modify-backend-request"></a>Изменение запросов внутреннего сервера
...
Сейчас на портале не предусмотрена возможность изменения запросов внутреннего сервера. Ознакомьтесь с разделом [Определение объекта requestOverrides], чтобы узнать, как использовать эту возможность в файле proxies.json.
...
Сейчас на портале не предусмотрена возможность изменения запросов. Ознакомьтесь с разделом [Определение объекта responseOverrides], чтобы узнать, как использовать эту возможность в файле proxies.json.
...
#### <a name="additonal-request-parameters"></a>Дополнительные параметры запроса
...
* **{backend.response.statusReason}.** Описание HTTP, возвращаемое для ответа внутреннего сервера.
...
* **backend.request.method.** Метод HTTP, который будет использоваться для вызова внутреннего сервера.
* **backend.request.querystring.&lt;имя_параметра&gt;.** Параметр строки запроса, который можно задать для вызова внутреннего сервера. Замените &lt;имя_параметра&gt; именем параметра, который вы собираетесь задать. Если указана пустая строка, параметр не будет включен в запрос внутреннего сервера.
* **backend.request.headers.&lt;имя_заголовка&gt;.** Заголовок, который можно задать для вызова внутреннего сервера. Замените &lt;имя_заголовка&gt; именем заголовка, который вы собираетесь задать. Если указана пустая строка, заголовок не будет включен в запрос внутреннего сервера.
...
* **response.headers.&lt;имя_заголовка&gt;.** Заголовок, который можно задать для ответа клиенту. Замените &lt;имя_заголовка&gt; именем заголовка, который вы собираетесь задать. Если указана пустая строка, заголовок не будет включен в ответ.
...
```
> [!Note] 
> В этом примере текст задается напрямую, поэтому задавать свойство `backendUri` не требуется. Это пример того, как можно использовать прокси-серверы Функций Azure для имитации API.
