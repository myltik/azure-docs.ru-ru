...
>[!NOTE]  
> [Размещение Linux](functions-create-first-azure-function-azure-cli-linux.md) в данный момент доступна только на план служб приложений.
...
> [!NOTE]
> Время ожидания по умолчанию для функций по плану потребления равно 5 минутам. Значение можно увеличить до 10 минут для функции приложения, изменив свойство `functionTimeout` в [host.json](functions-host-json.md#functiontimeout) файл проекта.
...
> - Разместите приложение-функцию в плане службы приложений с включенной функцией AlwaysOn.
> - Используйте другой механизм для активации обработки большого двоичного объекта, например подписку сетки событий или сообщение очереди, содержащее имя большого двоичного объекта. Пример см. в [скрипте C# и примерах JavaScript для привязок ввода и вывода большого двоичного объекта](functions-bindings-storage-blob.md#input--output---example).
...
Есть многие аспекты приложения функции, влияющие на насколько хорошо он будет выполнено масштабирование, включая конфигурации узла, используемый объем среды выполнения и эффективность работы с ресурсами.  Дополнительные сведения см. в [разделе о масштабируемости статьи о вопросах производительности](functions-best-practices.md#scalability-best-practices).
