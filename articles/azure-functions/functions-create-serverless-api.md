...
1. Выполните функцию и убедитесь, что она работает. Возможно, потребуется указать параметр имени в качестве строки запроса, чтобы использовать код быстрого запуска.
...
1. Выберите элемент **Параметры**. Включите параметр **Использовать прокси-серверы для API-интерфейсов**.
1. Выберите **Platform Settings** (Параметры платформы), а затем — **Параметры приложения**.
