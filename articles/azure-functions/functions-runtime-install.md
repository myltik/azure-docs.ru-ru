...
1. Установите предварительную версию среды выполнения службы "Функции Azure".
1. Завершите настройку предварительной версии среды выполнения службы "Функции Azure".
...
1. Щелкните вкладку **База данных**, введите сведения о подключении для экземпляра SQL Server и выберите **Применить**.  Это необходимо, чтобы создать базу данных для поддержки среды выполнения в среде выполнения службы "Функции Azure".
    
    ![Настройка базы данных предварительной версии среды выполнения Функций Azure][7]

1. Щелкните вкладку **Учетные данные**.  На этом экране необходимо создать две пары учетных данных для использования с общей папкой для размещения всех Функций Azure.  Укажите **имя пользователя** и **пароль** для **владельца общей папки** и **пользователя общей папки**, а затем выберите **Применить**.
...
1. Щелкните вкладку **IIS**.  На ней отображаются сведения о веб-сайтах в службах IIS, которые будут созданы с помощью инструмента настройки среды выполнения службы "Функции Azure".  Нажмите кнопку **Применить** для завершения.

    ![Предварительная версия среды выполнения Функций Azure — IIS][10]

1. Щелкните вкладку **Службы**.  На этой вкладке отображается состояние служб в инструменте настройки среды выполнения службы "Функции Azure".  Если после первоначальной настройки **служба активации узла службы "Функции Azure"** не запущена, выберите **Запустить службу**.

    ![Предварительная версия среды выполнения Функций Azure — завершение настройки][11]

1. Наконец, перейдите на **портал среды выполнения службы "Функции Azure"** как `https://<machinename>/`.
