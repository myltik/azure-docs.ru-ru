...
> [!NOTE]
> [Учетная запись хранения только для больших двоичных объектов](../storage/storage-create-storage-account.md#blob-storage-accounts) не поддерживается. Триггерам и привязкам хранилища BLOB-объектов требуется учетная запись хранения общего назначения. 
> 
...
> - Используйте другой механизм для активации обработки BLOB-объектов, например сообщение очереди, содержащее имя BLOB-объекта. Пример см. в разделе [Триггер очереди с привязкой BLOB-объекта для ввода](#input-sample).
