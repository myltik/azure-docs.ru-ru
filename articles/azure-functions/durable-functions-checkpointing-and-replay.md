...
| PartitionKey (InstanceId)                     | EventType             | Timestamp               | Входные данные | Имя             | Результат                                                    | Состояние | 
|----------------------------------|-----------------------|----------|--------------------------|-------|------------------|-----------------------------------------------------------|---------------------| 
...
* **EventType.** Предоставляет тип события. Принимается один из следующих типов:
    ...
    * **TimerFired.** Срок действия устойчивого таймера истек.
...
* **Output.** Выходные данные функции (то есть ее возвращаемое значение).
...
* Код оркестратора не должен иметь **блокировки**. Например, не должно быть `Thread.Sleep` или эквивалентного API.
