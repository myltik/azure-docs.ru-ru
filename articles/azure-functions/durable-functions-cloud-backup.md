...
Обратите внимание на строку `await Task.WhenAll(tasks);`. Все вызовы функции `E2_CopyFileToBlob` *не* ожидались. Это сделано намеренно, чтобы они могли выполняться параллельно. При передаче массива задач в `Task.WhenAll` мы получаем задачу, которая не выполнится *до завершения всех операций копирования*. Если вы знакомы с библиотекой параллельных задач (TPL) в .NET, то вы уже знаете об этом. Разница заключается в том, что задачи могут выполняться на нескольких виртуальных машинах одновременно, а расширение гарантирует, что комплексное выполнение устойчиво к перезапуску процессов.
...
Реализация также не вызывает затруднений. Иногда используются некоторые дополнительные функции привязок Функций Azure (то есть параметр `Binder`). Но не нужно беспокоиться об этом в рамках нашего пошагового руководства.

[!code-csharp[Main](~/samples-durable-functions/samples/csx/E2_CopyFileToBlob/run.csx)]

Реализация загружает файл с диска и асинхронно выполняет потоковую передачу содержимого в одноименный большой двоичный объект. В результате возвращается количество байтов, скопированных в хранилище, которое затем используется функцией оркестратора для вычисления общей суммы.
