...
Чтобы скачать файлы из приложения-функции с помощью FTP или локального репозитория Git, на портале необходимо настроить учетные данные для доступа к сайту. Учетные данные задаются на уровне приложения-функции.
...
1. В приложении-функции на [портале функций Azure](https://functions.azure.com/signin) щелкните **Параметры приложения-функции** > **Перейти к параметрам службы приложений** > **Свойства** и скопируйте значения параметров **Пользователь FTP или развертывания**, **Имя узла FTP** и **Имя узла FTPS**.  
    ...
    Параметр **Пользователь FTP или развертывания** должен быть введен так, как отображается на портале, включая имя приложения, чтобы обеспечить правильный контекст для FTP-сервера.
