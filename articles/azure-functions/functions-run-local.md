---
title: Разработка и выполнение Функций Azure локально | Microsoft Docs
...
3. Создайте переменную среды с именем AzureWebJobsServiceBus и присвойте ей значение, соответствующее строке подключения к служебной шине.
...
4. Убедитесь, что заданы все нужные переменные среды. (См. предыдущий раздел [Условные предварительные требования](#conditional-prerequisites).)
5. Запустите Visual Studio и откройте решение WebJobs.Script.
...
7. Если вы выбрали запускаемый проект WebJobs.Script.Host:
...
8. Если вы выбрали запускаемый проект WebJobs.Script.WebHost:
...
9. Откройте файл *sample\\host.json* и добавьте свойство `functions`, чтобы указать нужные функции для выполнения.
...
10. Выполните сборку и запуск решения.
