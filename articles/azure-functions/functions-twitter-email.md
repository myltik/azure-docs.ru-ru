...
3. Нажмите **AI + Analytics** (Искусственный интеллект и аналитика) > **API анализа текста**. Затем используйте параметры, указанные в таблице, примите условия и установите флажок **Закрепить на панели мониторинга**.
    ...
    | **Расположение** | Запад США | Используйте ближайшее к вам местоположение. |
...
1. Нажмите кнопку **Создать** и выберите **Среда выполнения приложений** > **Приложение-функция**. Затем используйте настройки, указанные в таблице, находящейся ниже. Примите условия, а затем выберите **Закрепить на панели мониторинга**.

    ![Создание приложения-функции Azure](media/functions-twitter-email/create_fun.png)

    | Настройка      |  Рекомендуемое значение   | Описание       |
    | --- | --- | --- |
    | **Имя** | MyFunctionApp | Выберите уникальное имя учетной записи. |
    | **Группа ресурсов** | myResourceGroup | Используйте одну группу ресурсов для всех служб в этом руководстве.|
    | **План размещения** | План потребления | Этот параметр определяет лимиты стоимости и использования.
    | **Расположение** | Запад США | Используйте ближайшее к вам местоположение. |
    | **Хранилище** | Создать | Автоматически создает учетную запись хранения.|
    | **Ценовая категория** | F0 | Всегда начинайте с самого нижнего уровня. Если вы превысили количество вызовов, перейдите на более высокий уровень.|

2. Выберите приложение-функцию на панели мониторинга, разверните функцию, нажмите кнопку **+** рядом с пунктом **Функции**, а затем щелкните **Веб-перехватчик + API**, **CSharp** и **Создать эту функцию**. С помощью этих действий и шаблона C# HTTPTrigger будет создана функция. Код появится в новом окне как файл `run.csx`
