...
1. На [портале Azure](https://portal.azure.com) перейдите к приложению Функций Azure или службы приложений.
...
Теперь, когда определение API экспортировано, импортируйте его, чтобы создать настраиваемый соединитель в PowerApps и Microsoft Flow. В следующем примере используется PowerApps, однако настраиваемые соединители совместно используются двумя службами, поэтому импортировать определение необходимо только один раз.

Чтобы импортировать определение API в PowerApps и Microsoft Flow, выполните такие действия:

1. Войдите в [web.powerapps.com](https://web.powerapps.com) или [flow.microsoft.com](https://flow.microsoft.com/). 
...
2. Нажмите кнопку **Параметры** (значок шестеренки), расположенную в правом верхнем углу страницы, и выберите **Настраиваемые соединители**.
    ...
    ![Настраиваемые соединители](media/app-service-export-api-to-powerapps-and-flow/custom-connectors.png)

3. Щелкните **Создать настраиваемый соединитель**.

4. На вкладке **Общие** укажите имя API, а затем передайте определение OpenAPI или вставьте URL-адрес метаданных. Щелкните **Отправить**, а затем — **Продолжить**.

    ![Вкладка «Общие»](media/app-service-export-api-to-powerapps-and-flow/tab-general.png)
