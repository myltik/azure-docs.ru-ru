...
[Устойчивые функции](durable-functions-overview.md) и [Azure Logic Apps](../logic-apps/logic-apps-what-are-logic-apps.md) используются для управления переходами состояний и обмена данными между несколькими функциями.
...
Размер отдельных сообщений в очереди хранилища ограничен до 64 КБ. Если нужно передать сообщения большего размера между функциями, Azure Service Bus, очередь может использоваться для поддержки сообщение размером до 256 КБ на стандартном уровне и на уровне Premium до 1 МБ.
