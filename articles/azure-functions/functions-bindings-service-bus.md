...
Эта статья объясняет, как настроить и запрограммировать привязки служебной шины в Функциях Azure. Функции Azure поддерживают привязки триггера и выходные привязки для очередей и разделов Центров уведомлений.
...
Триггеры очереди и раздела Центров уведомлений для функции используют следующие объекты JSON в массиве `bindings` файла function.json:
...
* Для `connection` [создайте параметр приложения в приложении-функции](), содержащем строку подключения к пространству имен центра обслуживания, а затем укажите имя параметра приложения в свойстве `connection` в триггере. Чтобы получить строку подключения, следуйте инструкциям, указанным в разделе [Получение учетных данных управления](../service-bus-messaging/service-bus-dotnet-get-started-with-queues.md#obtain-the-management-credentials).
...
В этом разделе показано, как использовать триггер центра обслуживания в коде функции. 
...
В Node.js сообщение триггера служебной шины передается в функцию в качестве строки, или, в случае сообщения JSON, объекта JavaScript.
...
Выходные данные очереди и раздела Центров уведомлений для функции используют следующие объекты JSON в массиве `bindings` файла function.json:
...
* Для `connection` [создайте параметр приложения в приложении-функции](), содержащем строку подключения к пространству имен центра обслуживания, а затем укажите имя параметра приложения в свойстве `connection` в выходной привязке. Чтобы получить строку подключения, следуйте инструкциям, указанным в разделе [Получение учетных данных управления](../service-bus-messaging/service-bus-dotnet-get-started-with-queues.md#obtain-the-management-credentials).
  Строка подключения указывается для пространства имен служебной шины, и она не должна ограничиваться определенной очередью или разделом.
