...
При создании службы управления API она содержит только одну [единицу][] и располагается в одном регионе Azure, который считается первичным. Однако через портал Azure можно легко добавить дополнительные регионы. В каждом регионе развертывается прокси-сервер управления API, и весь трафик вызовов направляется на ближайший из таких серверов. Если регион переходит в автономный режим, трафик автоматически перенаправляется к другому ближайшему прокси-серверу.
...
Перейдите на вкладку **Масштаб** на портале Azure для вашего экземпляра службы управления API.
...
Чтобы удалить экземпляр службы управления API из отдельного региона, перейдите на вкладку **Масштаб** портала Azure для вашего экземпляра службы управления API.
