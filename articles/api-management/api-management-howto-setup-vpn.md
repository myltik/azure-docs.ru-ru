...
Поддержка VPN службой управления API позволяет подключить ваш прокси службы управления API к виртуальной сети Azure. Это позволяет клиентам службы управления API безопасно подключаться к локальным серверным веб-службам, в противном случае они были бы недоступны для доступа через Интернет.

## <a name="enable-vpn"> </a>Включение VPN-подключений

>VPN-подключения доступны только на уровне **Premium**. Чтобы перейти к нему, откройте службу управления API на [портале управления][] и перейдите на вкладку **Масштаб**. В разделе **Общие** выберите уровень Premium и нажмите кнопку "Сохранить".

Чтобы включить VPN-подключения, откройте службу управления API на [портале управления][] и перейдите на вкладку **Настройка**.
...
В нижней части экрана нажмите кнопку **Сохранить**. Во время обновления на портале управления Azure невозможно выполнять другие операции в службе управления API. Прокси службы будет оставаться доступным и вызовы времени выполнения не будут затронуты.

Обратите внимание, что виртуальный IP-адрес прокси будет меняться при каждом включении или отключении VPN.
