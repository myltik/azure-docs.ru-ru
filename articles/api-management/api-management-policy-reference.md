...
Этот раздел содержит индекс для политик в разделе [Справка по политике управления интерфейсами API][]. Дополнительные сведения о добавлении и настройке политик см. в разделе [Политики в API Management][].

Выражения политики можно использовать в качестве значений атрибутов или текстовых значений в любой политике управления API, если в ней не указано иное. Некоторые политики, такие как [Управление потоками][] и [Задание переменной][], основаны на выражениях политики. Дополнительную информацию см. в разделе [Расширенные политики][] и [Выражения политики][], а также в следующем видео.

> [AZURE.VIDEO policy-expressions-in-azure-api-management]

## Индекс справки по политикам

-	[Политики ограничения доступа][]
	-	[Проверка заголовка HTTP][] — обеспечивает принудительный ввод заголовка HTTP и/или его значения.
	-	[Ограничение количества вызовов][] — предотвращает пики использования API путем ограничения числа вызовов и/или скорости потребления пропускной способности.
	...
	-	[Задание квоты использования][] — позволяет принудительно устанавливать возобновляемую или действующую в течение срока службы квоту на число вызовов и/или квоту пропускной способности.
...
-	[Политики аутентификации][]
	...
	-	[Аутентификация с помощью сертификата клиента][] — аутентификация внутренней службы с помощью сертификатов клиента.
...
-	[Междоменные политики][] 
	...
	-	[JSONP][] - добавляет поддержку JSON с заполнением (JSONP) для операции или API, чтобы разрешить кросс-доменные вызовы из клиентов на основе браузера JavaScript.
-	[Политики преобразования][] 
	...
	-	[Маскирование URL-адресов в содержимом][] — перезаписывает (маскирует) ссылки в тексте ответа и в заголовке папки так, чтобы они указывали на эквивалентную ссылку через прокси-сервер.
	...
	-	[Перезапись URI-адреса][] — преобразует URL-адрес запроса из его общедоступной формы в форму, ожидаемую веб-службой.
...
[Ограничение количества вызовов]: https://msdn.microsoft.com/library/azure/034febe3-465f-4840-9fc6-c448ef520b0f#LimitCallRate
...
[Задание квоты использования]: https://msdn.microsoft.com/library/azure/034febe3-465f-4840-9fc6-c448ef520b0f#SetUsageQuota
...
[Междоменные политики]: https://msdn.microsoft.com/library/azure/dn894084.aspx
...
[Маскирование URL-адресов в содержимом]: https://msdn.microsoft.com/library/azure/7406a8ce-5f9c-4fae-9b0f-e574befb2ee9#MaskURLSContent
...
[Перезапись URI-адреса]: https://msdn.microsoft.com/library/azure/7406a8ce-5f9c-4fae-9b0f-e574befb2ee9#RewriteURL



[Политики в API Management]: api-management-howto-policies.md
