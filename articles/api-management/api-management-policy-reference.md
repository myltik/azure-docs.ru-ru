...
В этом разделе содержится ссылка на политики [Справка по политике управления интерфейсами API][Справка по политике управления интерфейсами API]. Дополнительные сведения о добавлении и настройке политик см. в разделе [Политики службы управления API][Политики службы управления API].

Выражения политики можно использовать в качестве значений атрибутов или текстовых значений в любой политике управления API, если в ней не указано иное. Некоторые политики, например [Управление потоками][Управление потоками] и [Задание переменной][Задание переменной], основаны на выражениях политики. Дополнительную информацию см. в разделах [Расширенные политики][Расширенные политики] и [Выражения политики][Выражения политики].

## Справочные указатели по политикам
* [Политики ограничения доступа][Политики ограничения доступа]
  * [Проверка заголовка HTTP][Проверка заголовка HTTP] – обеспечивает принудительный ввод заголовка HTTP и/или его значения.
  ...
  * [Задание квоты использования по подписке][Задание квоты использования по подписке] — позволяет принудительно устанавливать возобновляемую или действующую в течение срока службы квоту на число вызовов и (или) квоту пропускной способности для каждой подписки.
...
* [Расширенные политики][Расширенные политики]
  ...
  * [Перенаправляющий запрос][Перенаправляющий запрос] — перенаправляет запрос в серверную службу.
  * [Регистрировать в концентраторе событий][Регистрировать в концентраторе событий] — отправляет сообщения в определенном формате объекту, который указан [средством ведения журнала](https://msdn.microsoft.com/library/azure/mt592020.aspx#Logger).
  ...
  * [Установка переменной][Установка переменной] – сохраняет значение в именованной переменной [контекста][контекста] для последующего использования.
  * [Трассировка](https://msdn.microsoft.com/ru-RU/library/dn894085.aspx#Trace) — добавляет строку в выходные данные [инспектора API](api-management-howto-api-inspector.md).
...
* [Политики аутентификации][Политики аутентификации]
  * [Обычная проверка подлинности][Обычная проверка подлинности] – обычная проверка подлинности внутренней службы.
...
* [Кроссдоменные политики][Кроссдоменные политики]
  ...
  * [JSONP][JSONP] – добавляет поддержку JSON с заполнением (JSONP) для операции или API, чтобы разрешить кросс-доменные вызовы из браузерных клиентов JavaScript.
* [Политики преобразования][Политики преобразования]
  * [Преобразование JSON в XML][Преобразование JSON в XML] – преобразует текст запроса или ответа в формате JSON в формат XML.
  * [Преобразование XML в JSON][Преобразование XML в JSON] – преобразует текст запроса или ответа в формате XML в формат JSON.
  * [Поиск и замена строки в тексте][Поиск и замена строки в тексте] – отыскивает подстроку запроса или ответа и заменяет ее на другую подстроку.
  ...
  * [Перезапись URL-адреса][Перезапись URL-адреса] – преобразует URL-адрес запроса из его общедоступной формы в форму, ожидаемую веб-службой.
