...
Аналогично политике **Set usage quota per subscription** (Ограничение скорости вызова по подписке), политика **Limit call rate per subscription** (Установка квоты использования по подписке) позволяет настраивать ограничения для операций и API продукта. В этом руководстве мы не будем использовать эту возможность, поэтому удалите элементы **api** и **operation** из элемента **quota**, как показано в следующем примере.
