---
title: Защита API с помощью управления API Azure| Microsoft Docs
...
> [!NOTE]
> Если вы уже настроили продукт и хотите использовать его в этом учебнике, можно сразу перейти к разделу [Настройка политик ограничения скорости вызова и политик квот][Настройка политик ограничения скорости вызова и политик квот] и выполнить указания учебника с этого места, используя вместо бесплатной пробной версии продукта свой продукт.
> 
> 
...
> Каждый экземпляр службы API Management поставляется предварительно настроенным с Echo API, с которым можно экспериментировать при изучении API Management. Дополнительную информацию см. в статье [Начало работы со службой управления Azure API][Начало работы со службой управления Azure API].
> 
> 
...
В этом руководстве добавляются две политики: [Ограничение скорости вызова по подписке](https://msdn.microsoft.com/library/azure/dn894078.aspx#LimitCallRate) и [Установка квоты использования по подписке](https://msdn.microsoft.com/library/azure/dn894078.aspx#SetUsageQuota).

![Правила политики][api-management-limit-policies]

Поместив курсор в элемент политики **inbound**, щелкните стрелку рядом с политикой **Ограничение скорости вызова по подписке**, чтобы вставить шаблон политики.
...
Чтобы настроить политику **Установка квоты использования по подписке**, поместите курсор под только что добавленным элементом **rate-limit** в элементе **inbound** и щелкните стрелку слева от политики **Установка квоты использования по подписке**.
