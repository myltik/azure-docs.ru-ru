...
Чтобы начать, выберите пункт **Управление** для службы управления API на портале Azure. Будет открыт портал издателя службы управления API.
...
Перейдите на вкладку **Операции** и выберите операцию **GET Resource (cached)** в списке **Операции**.
...
Выберите требуемый ключ из раскрывающегося списка **subscription-key**. Если ваша учетная запись содержит только одну подписку, то он уже будет выбран.

Введите **sampleheader:value1** в текстовом поле **Заголовки запроса**.

Щелкните **HTTP Get** и запишите значение заголовков ответа.

Введите **sampleheader:value2** в текстовом поле **Заголовки запроса** и нажмите **HTTP Get**.
...
Введите **25** в поле **param2** и нажмите **HTTP Get**.
