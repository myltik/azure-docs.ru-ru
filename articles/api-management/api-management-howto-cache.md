...
При использовании конфигурации кэширования в этом примере на первый запрос операции **GET Resource (cached)** возвращает ответ из серверной службы. Этот ответ будет кэшироваться, а также отмечаться (ключом) указанных заголовков и параметров строки запроса. Для последующих вызовов операции с совпадающими параметрами будет возвращаться кэшированный ответ до тех пор, пока не истечет срок действия кэша.
...
Щелкните **Политики** в меню **Управление API** слева и выберите в раскрывающемся списке **Операция** пункт **Echo API/GET Resource (cached)**.
