...
> [!NOTE]
> Для работы с этим учебником требуется учетная запись Azure. Если ее нет, можно создать бесплатную учетную запись всего за несколько минут. Дополнительные сведения см. [здесь][Бесплатная пробная версия Azure].
> 
> 

Создание экземпляра службы — это первый шаг в работе с API Management. Войдите на [портал Azure][портал Azure] и щелкните **Создать**, **Интернет+мобильные устройства**, **Управление API**.
...
> [!NOTE]
> Этот адрес электронный почты используется для получения уведомлений от системы API Management. Дополнительные сведения см. в статье [Как настраивать уведомления и почтовые шаблоны в службе управления Azure API][Как настраивать уведомления и почтовые шаблоны в службе управления Azure API].
> 
> 
...
[Как управлять учетными записями разработчика в службе управления Azure API]: api-management-howto-create-or-invite-developers.md
[Настройка параметров API]: api-management-howto-create-apis.md#configure-api-settings
[Как настраивать уведомления и почтовые шаблоны в службе управления Azure API]: api-management-howto-configure-notifications.md
[Ответы]: api-management-howto-add-operations.md#responses
[Как создать и опубликовать продукт]: api-management-howto-add-products.md
[Расценки на службу управления API]: http://azure.microsoft.com/pricing/details/api-management/
