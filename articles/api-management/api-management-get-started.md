<properties
	pageTitle="Управление API в управлении API Azure"
...
# Управление API в управлении API Azure

## <a name="overview"> </a>Обзор

Это руководство покажет как быстро начать работу с API Management и сделать первый вызов API.

## <a name="concepts"></a>Что такое управление API Azure?

Управление API Azure позволяет запустить полноценную программу API на основе любого внутреннего сервера.

Ниже приведены распространенные сценарии.

* **Защита мобильной инфраструктуры** путем ограничения доступа с помощью ключей API, а также предотвращения DOS-атак за счет регулирования или использования расширенных политик безопасности, таких как проверка токена JWT.
* **Включение партнерской экосистемы независимого поставщика программного обеспечения** путем предложения быстрой партнерской адаптации на портале разработчика и создания интерфейсной части API, чтобы отделиться от внутренних реализаций, а не ожидать использования партнером.
...
* **Шлюз API** — конечная точка, которая:
  ...
  * проверяет ключи API, токены JWT, сертификаты и другие учетные данные;
...
* **Портал издателя** — административный интерфейс, предназначенный для настройки программы API, который позволяет:
	...
	* настраивать политики, такие как квоты или преобразования, в интерфейсах API;
	* получать дополнительную информацию на основе аналитики;
	* управлять пользователями

* **Портал разработчика**, выступающий в качестве основного веб-присутствия для разработчиков, которое позволяет:
	* читать документацию по API;
	* использовать API через интерактивную консоль;
	* создать учетную запись и подписаться, чтобы получить ключи API;
...
Создание экземпляра службы — это первый шаг в работе с API Management. Войдите на портал [Портал управления][] и щелкните **Создать**, **Службы приложений**, **Управление API**, **Создать**.

![Новый экземпляр API Management][api-management-create-instance-menu]
...
![Новая служба API Management][api-management-create-instance-step1]
...
![Новая служба API Management][api-management-create-instance-step2]
...
Можно создавать интерфейсы API и добавлять операции вручную, а также импортировать и то, и другое. В этом учебнике мы импортируем API для примера веб-службы калькулятора, предоставляемой корпорацией Майкрософт и размещенной в Azure.

>Указания о создании API и добавлении операций вручную см. в статье [Как создать интерфейсы API](api-management-howto-create-apis.md) и [Как добавить операции к API](api-management-howto-add-operations.md).

Интерфейсы API настраиваются на портале издателя API, который доступен на портале управления Azure. Чтобы перейти на портал издателя, щелкните **Управление** на портале Azure для службы управления API.

![Портал издателя][api-management-management-console]

Чтобы импортировать API калькулятора, щелкните **API** в расположенном слева меню **Управление API**, а затем **Импортировать API**.
...
Чтобы делать вызовы к API, разработчикам сначала необходимо подписаться на продукт, который дает доступ к ним. Разработчики могут подписываться на продукты на портале разработчика. За них это также могут сделать администраторы. Вы администратор, так как создали экземпляр API управления на предыдущих шагах руководства, поэтому вы подписаны на каждый продукт по умолчанию.

## <a name="call-operation"></a>Вызов операции с портала разработчика

Операции могут быть вызваны из портала разработчика, где предоставляется удобный способ просмотра и проверки операций API. На этом шаге учебника будет вызвана операция **Добавление двух целых** **базового API калькулятора**. Выберите **Портал разработчика** в правом верхнем меню на портале издателя.
...
Обратите внимание, что показаны описание и параметры примера, импортированные с API и операциями, что предоставляет документацию разработчикам, которые будут использовать эту операцию. Эти описания можно также добавить при добавлении операций вручную.
...
Можно ввести некоторые значения для параметров или оставить значения по умолчанию и щелкнуть **Отправить**.

![HTTP Get][api-management-invoke-get]

После вызова операции на портале разработчика отображается **состояние ответа**, **заголовки ответа** и **содержимое ответа**.

![Ответ][api-management-invoke-get-response]

## <a name="view-analytics"></a>Просмотр аналитики

Чтобы просмотреть аналитику для **Базового калькулятора**, перейдите снова на портал разработчика, выбрав **Управление** в верхнем правом меню пользователя на портале разработчика.
...
[Управление API Цены]: http://azure.microsoft.com/pricing/details/api-management/
