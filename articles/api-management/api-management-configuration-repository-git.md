...
Чтобы настроить доступ к Git, щелкните меню **Безопасность** и перейдите к вкладке **Репозиторий конфигурации**.

![Включение доступа к GIT][api-management-enable-git]

Чтобы включить доступ к Git, установите флажок **Включить доступ к Git**.

Через некоторое время изменение будет сохранено, и появится сообщение с подтверждением.
...
В следующих примерах используется средство Git Bash из [Git для Windows](http://www.git-scm.com/downloads), но можно использовать любое знакомое вам средство Git.
...
| DelegationValidationKey | Текстовое поле **Ключ проверки делегирования** |
