...
В этом руководстве показано, как создать новый API и импортировать его операции за один шаг. Дополнительные сведения о создании API и добавлении операций вручную см. в разделе [Как создать интерфейсы API][Как создать интерфейсы API] и [Как добавить операции к API][Как добавить операции к API].

## <a name="import-api"> </a>Импорт API
API создаются и настраиваются на портале издателя. Чтобы перейти на портал издателя, на классическом портале Azure для службы управления API щелкните **Управление**. Если экземпляр службы управления API еще не создан, см. раздел [Создание экземпляра службы управления API][Создание экземпляра службы управления API] в руководстве [Начинаем работу со службой управления API][Начинаем работу со службой управления API].
...
> [!NOTE]
> Инструкции по импорту базового API калькулятора в формате Swagger см. в статье [Управление API в службе управления API Azure](api-management-get-started.md).
> 
> 
