<properties urlDisplayName="Analyze realt-time Twitter sentiment with Hbase in HDInsight" pageTitle="Анализ мнений пользователей Twitter в режиме реального времени с использованием HBase в HDInsight на платформе Azure" metaKeywords="" description="Узнайте, как проводить анализ мнений в режиме реального времени на основе данных большого размера с помощью HBase в кластере HDInsight (Hadoop)." metaCanonical="" services="hdinsight" documentationCenter="" title="Анализ мнений пользователей Twitter в режиме реального времени с использованием HBase в HDInsight" authors="jgao" solutions="big-data" manager="paulettm" editor="cgronlun" />
...
-   **Кластер HBase в HDInsight**. Инструкции по подготовке кластера см. в разделе [Приступая к использованию HBase с Hadoop в HDInsight][Приступая к использованию HBase с Hadoop в HDInsight]. Для выполнения учебника необходимы следующие данные:
    ...
    | Свойство кластера            | Описание                                                                                  |
    |------------------------------|-------------------------------------------------------------------------------------------|
    | Имя кластера HBase           | Это имя вашего кластера HBase в HDInsight. Например: https://myhbase.azurehdinsight.net/  |
    | Имя пользователя кластера    | Имя учетной записи пользователя Hadoop. Имя пользователя Hadoop по умолчанию — **admin**. |
    | Пароль пользователя кластера | Пароль пользователя кластера Hadoop.                                                      |
...
## <span id="twitter"></span></a>Создание идентификатора и секретов приложения Twitter
...
3.  Введите **Имя**, **Описание**, **Веб-сайт**. Поле "Веб-сайт" на самом деле не используется. В нем не обязательно указывать действительный URL-адрес. В следующей таблице приведены некоторые примеры значений:
    ...
    | Поле     | Значение                           |
    |----------|------------------------------------|
    | Имя      | MyHDInsightHBaseApp                |
    | Описание | MyHDInsightHBaseApp                |
    | Веб-сайт | http://www.myhdinsighthbaseapp.com |
...
## <span id="streaming"></span></a>Создание простой службы потоковой передачи данных Twitter
...
## <span id="web"></span></a>Создание веб-сайта Azure для визуализации мнений пользователей Twitter
...
3.  Добавьте в функцию **Application\_Start()** следующие строки:
