...
Имеется второй документ с небольшим отличием - используются "givenName" и "familyName" и вместо "firstName" и "lastName".
...
Давайте попробуем сформировать несколько запросов на этом массиве данных, чтобы понять некоторые основные принципы SQL для DocumentDB. Например, следующий запрос возвращает документы с одинаковыми идентификаторами полей "AndersenFamily". Поскольку это является производной функции "SELECT *", результатом запроса является весь документ JSON:
...
Следующий запрос возвращает все заданные имена детей, идентификатор которого соответствует "WakefieldFamily".
...
-	В DocumentDB язык SQL работает со значениями JSON, т. е. имеет дело с сущностями в виде деревьев, а не со строками и столбцами. Таким образом, язык позволяет обращаться к узлам дерева любой произвольной вложенности, как "Node1.Node2.Node3...Nodem ", что похоже на реляционный SQL со ссылкой, состоящей из двух частей<table>"<столбец>".   
...
См. в разделе [Примеры DocumentDB](http://code.msdn.microsoft.com/Azure-DocumentDB-NET-Code-6b3da8af#content) о том, как настроить политику индексации для коллекции. Давайте теперь перейдем к деталям языка SQL DocumentDB .
...
Запрос, типа "ВЫБЕРИТЕ* ИЗ семей" указывает, что вся коллекция Семьи является источником для перечисления. Специальный идентификатор ROOT может использоваться для представления коллекции вместо использования имени коллекции. 
Следующий список содержит правила, которые применяются для запроса:

- Запрос может быть сделан различными способами, например "SELECT f.id FROM Families AS f" или просто "SELECT f.id FROM Families f". Здесь "f" является эквивалентом "Семейства". "AS" является необязательным ключевым словом для определения идентификатора.

-	Обратит внимание, что после создания псевдонима оригинальный источник связывать нельзя. Например, "SELECT Familes.id FROM Families f" является синтаксически неправильным, так как идентификатор "Families" уже не может быть извлечен.

-	Все свойства, на которые необходимо ссылаться, должны иметь полное название. В отсутствие строгого следования схеме это реализуется для того, чтобы избежать любых неоднозначных привязок. Таким образом, выражение "SELECT id FROM Families f" является синтаксически недопустимым, так как не привязано свойство "id".
...
Кроме бинарных и унарных операторов, также разрешены ссылки на свойства. Например "SELECT * FROM Families f WHERE f.isRegistered" возвращает JSON-документ, содержащий свойство "isRegistered", где значение свойства равно значению "true" для JSON. Любые другие значения (false, null, Undefined, "<число>", "<строка>", "<объект>", "<массив>", т. д.) приводят к тому, что исходный документ будет исключен из результата. 
...
#Выражение SELECT
Выражение ВЫБЕРИТЕ (**"SELECT < select_list >"**) является обязательным и указывает значения, которые будут получены из запроса аналогично ANSI SQL. Подмножество, которое было отфильтрован на верхней части исходных документов, передаются на фазе проекции, где указанные значения JSON извлекаются и строится новый объект JSON, на него осуществляется передача для каждого входа. 
...
Рассмотрим роль "$1" ниже. Выражение SELECT необходимо для создания объекта JSON, и так как ни один из ключей не предоставлен, мы используем имена неявных переменных аргументов, начиная с "$1". Например, этот запрос возвращает два переменных неявных аргумента с меткой "$1" и "$2".
...
##Оператор *
Специальный оператор (*) выводит проект документа в формате "как есть". При его использовании должно быть единственное отображаемое поле. В то время как запрос "SELECT * FROM Families f" является допустимым, такие запросы, как "SELECT VALUE * FROM Families f" и "SELECT *, f.id FROM Families f" не являются допустимыми.
...
Прежде всего, следует отметить это "from_source" из выражения **JOIN** является итератором. В этом случае поток выглядит следующим образом.  
...
У "AndersenFamily" имеется один ребенок, у которого имеется один питомец. Таким образом, перекрестное произведение создает одну строку (1 * 1 * 1) из этого семейства. Однако семейство Wakefield включает двух детей, но только у одного ребенка "Джесси" есть питомцы. У нее 2 питомца. Векторное произведение представляет собой две строки 1 * 1 * 2 = 2 в случае с этим семейством.

В следующем примере введен дополнительный фильтр по "pet" (домашнее животное). Будут исключены все кортежи, у которых имя питомца не равно "Shadow". Обратите внимание, что мы можем построить кортежи из массивов, установить фильтр на любой из элементов набора и проецировать любую комбинацию из элементов. 
...
Приведенный выше пример создает UDF с именем "SQRT". Она принимает одно значение JSON "number" и вычисляет квадратный корень числа с помощью математической библиотеки.
...
SQL в DocumentDB не выполняет неявные преобразования, в отличие от JavaScript. Например, запрос "SELECT * FROM Person p WHERE p.Age = 21" возвращает документы со свойством "возраст", равным 21. Любой документ, свойство "возраст" в котором равно строке "21" или другим вариациям, таким как "021", "21.0", "0021", "00021" и т. д. не будут возвращены. 
...
###Оператор Select
Синтаксис "input.Select(x => f(x))", где "f" является скалярным выражением.
...
###Оператор SelectMany
Синтаксис "input.SelectMany(x => f(x))", где "f" представляет скалярное выражение, которое возвращает тип коллекции.
...
###Оператор Where
Синтаксис "input.Where(x => f(x))", где "f" представляет скалярное выражение, возвращающее логическое значение.
...
Клиент .NET автоматически перебирает все страницы результатов запроса в блоках FOREACH, как показано выше. Параметры запроса, введенные в разделе API REST, также доступны в .NET SDK с помощью классов "FeedOptions" и "FeedResponse" в методе CreateDocumentQuery. Количеством страниц можно управлять с помощью параметра "MaxItemCount". 

Разработчики также могут выполнять разбиение по страницам путем создания IDocumentQueryable с помощью объекта "IQueryable", а затем, считывая значения "ResponseContinuationToken" и возвращая в виде "RequestContinuationToken" в "FeedOptions". "EnableScanInQuery" можно задать для включения проверки, если запрос не поддерживаются настроенной политикой индексирования.
