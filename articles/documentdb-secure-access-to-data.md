...
##<a id="Sub1"></a>Понятия управления доступом DocumentDB##
...
- Маркеры ресурсов. Маркер ресурса связан с разрешением DocumentDB и отражает отношение между пользователем базы данных и разрешением этого пользователя к конкретному ресурсу приложения DocumentDB (например, коллекции, документу).
...
Маркер ресурса DocumentDB является безопасным альтернативным решением, которое позволяет клиентам читать, записывать и удалять ресурсы в учетной записи DocumentDB согласно предоставленным разрешениям и без необходимости использования главного ключа или ключа только для чтения.
...
- Полный. Пользователь имеет полный набор разрешений для ресурса.
- Чтение. Пользователь может только считывать содержимое ресурса, но не может выполнять с ресурсом операции записи, обновления или удаления.
