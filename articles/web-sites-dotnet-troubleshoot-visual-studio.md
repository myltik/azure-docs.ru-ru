...
3. Щелкните правой кнопкой мыши узел веб-сайта, созданный в разделе [Начало работы с Azure и ASP.NET][GetStarted], и щелкните **Параметры просмотра**.
	...
	Если вам необходимо выполнить задачу по управлению сайтом, которую невозможно выполнить в этом окне, можете щелкнуть пункт **Полные параметры веб-сайта**, чтобы открыть в окне браузера портал управления. Дополнительные сведения см. в разделе [Настройка веб-сайтов](/ru-ru/manage/services/web-sites/how-to-configure-websites/#howtochangeconfig).
...
4. Обновите окно браузера, в котором отображается неинформативное сообщение. Вы увидите подробную информацию об ошибке, как в следующем примере:
	...
	(Для создания отображаемой ошибки в файл *Views\Home\Index.cshtml* была добавлена выделенная красным строка.)
...
В этом разделе демонстрируется удаленная отладка с использованием проекта и веб-сайта, созданного в разделе [Начало работы с пакетом SDK веб-заданий Azure](../websites-dotnet-webjobs-sdk). Функции, приведенные в этом разделе, доступны только в Visual Studio 2013 с обновлением 4.
...
2. В раскрывающемся списке **Профиль** выберите тот же профиль, который вы использовали в разделе [Начало работы с пакетом SDK веб-заданий Azure](../websites-dotnet-webjobs-sdk).
...
Если функция [выполняет запись сообщений в журнал](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs), можно щелкнуть **ToggleOutput** для их просмотра.
...
Сведения о создании журналов приложения в веб-заданиях см. в разделе [Использование пакета SDK веб-заданий для работы с хранилищем очередей Azure - запись журналов](../websites-dotnet-webjobs-sdk-storage-queues-how-to/#logs). Следующие инструкции для просмотра журналов и управления их хранением в Azure также применимы к журналам приложения, созданным веб-заданиями. 

### Добавление инструкций трассировки в приложение

1. Откройте *Controllers\HomeController.cs* и замените содержимое файла на следующий код, чтобы добавить операторы `Trace` и `using` в 'System.Diagnostics':
...
3. Добавьте <a href="http://msdn.microsoft.com/ru-ru/library/vstudio/6915t83k(v=vs.100).aspx">элемент трассировки</a> под `<system.web>` в файле Web.config, как в следующем примере:

		<trace enabled="true" writeToDiagnosticsTrace="true" mostRecent="true" pageOutput="false" />

3. Для запуска приложения нажмите клавиши CTRL+F5.
...
По умолчанию при первом включении журналов веб-сервера с помощью Visual Studio Azure записывает журналы в файловой системе. В качестве альтернативы можно использовать портал управления для указания, что журналы веб-сервера должны записываться в контейнер blob, содержащийся в учетной записи хранения. Дополнительные сведения см. в разделе **Диагностика сайта** статьи [Настройка веб-сайтов](/ru-ru/manage/services/web-sites/how-to-configure-websites/#howtochangeconfig). 
...
<h2><a name="downloadlogs"></a>Загрузка журналов файловой системы</h2>
...
На данный момент вы настроили подробное ведение журналов файловой системы. Далее вы настроите журналы информационного уровня, которые будут передаваться в таблицы учетной записи хранения. Информационный уровень означает, что все журналы, созданные вызовом `Trace.TraceInformation`, `Trace.TraceWarning` и `Trace.TraceError`, будут отображаться, однако не будут отображаться журналы, созданные при вызове `Trace.WriteLine`.
...
* [How to Monitor Web Sites](/ru-ru/manage/services/web-sites/how-to-monitor-websites/)
...
* [Использование функций диагностики и ведения журнала для веб-сайта Azure с Дэвидом Эббо (David Ebbo)](http://www.windowsazure.com/ru-ru/documentation/videos/azure-web-site-logging-and-diagnostics/) и [Потоковая передача журналов с веб-сайтов Azure с Дэвидом Эббо (David Ebbo)](http://www.windowsazure.com/ru-ru/documentation/videos/log-streaming-with-azure-web-sites/)<br>
