<properties 
	...
	description="Узнайте, как устранять неполадки в работе веб-приложения Azure с помощью удаленной отладки, трассировки и средств ведения журналов, которые встроены Visual Studio 2013." 
...
В этой статье показано, как использовать инструменты Visual Studio, которые позволяют отлаживать работу веб-приложения во время его работы в [службе приложений](http://go.microsoft.com/fwlink/?LinkId=529714), путем запуска веб-приложения удаленно в [режиме отладки](http://www.visualstudio.com/ru-ru/get-started/debug-your-app-vs.aspx) или путем просмотра журналов приложения и веб-сервера.
...
<h2><a name="sitemanagement"></a>Настройка веб-приложения и управление им</h2>

Visual Studio обеспечивает доступ к ряду функций управления веб-приложением и параметров конфигурации, доступных на [портале Azure](http://go.microsoft.com/fwlink/?LinkId=529715). В этом разделе вы узнаете, какие функции доступны.
...
2. В **обозревателе сервера** разверните **Azure**, а затем разверните **Веб-приложения**.

3. Щелкните правой кнопкой мыши узел веб-приложения, созданного на этапе [Начало работы с Azure и ASP.NET][GetStarted], затем щелкните **Параметры просмотра**.
	...
	Информацию о полях "Параметры приложения" и "Строки подключения" см. в разделе [Веб-приложения Azure: как работают параметры приложения и строки подключения](http://blogs.msdn.com/b/windowsazure/archive/2013/07/17/windows-azure-web-sites-how-application-strings-and-connection-strings-work.aspx).
...
### Удаленная отладка веб-приложений
...
5. Для Visual Studio 2013: В **обозревателе сервера** разверните узел **Azure**, затем разверните **Веб-приложения**, щелкните правой кнопкой мыши свое приложение и нажмите **Подключить отладчик**. 
...
### <a name="remotedebugwj"></a> Веб-задания по удаленной отладке

В этом разделе демонстрируется удаленная отладка с использованием проекта и веб-приложения, созданного на этапе [Начало работы с пакетом SDK веб-заданий Azure](websites-dotnet-webjobs-sdk.md). Функции, приведенные в этом разделе, доступны только в Visual Studio 2013 с обновлением 4.
...
2. В раскрывающемся списке **Профиль** выберите тот же профиль, который вы использовали в разделе[Get Started with the Azure WebJobs SDK](websites-dotnet-webjobs-sdk.md).
...
Если функция [записала журналы](websites-dotnet-webjobs-sdk-storage-queues-how-to.md#logs), щелкните **ToggleOutput**, чтобы просмотреть их.
...
* Во время отладки сервер отправляет данные в среду Visual Studio, что может повлиять на расходы, связанные с пропускной способностью. Информацию о тарифах на пропускную способность см. в разделе [Цены Azure](/pricing/calculator/).
...
* **Журналы веб-сервера**<br/>
  Веб-сервер создает запись журнала для каждого HTTP-запроса, адресованного веб-приложению.
...
Информацию о создании журналов приложения в веб-заданиях см. в разделе [Использование пакета SDK веб-заданий для работы с хранилищем очередей Azure - запись журналов](websites-dotnet-webjobs-sdk-storage-queues-how-to.md#logs). Следующие инструкции для просмотра журналов и управления их хранением в Azure также применимы к журналам приложения, которые созданы веб-заданиями. 
...
3. Добавьте <a href="http://msdn.microsoft.com/library/vstudio/6915t83k(v=vs.100).aspx">элемент трассировки</a> под строкой <system.web> в файле Web.config, например следующим образом.

		<trace enabled="true" writeToDiagnosticsTrace="true" mostRecent="true" pageOutput="false" />

3. Для запуска приложения нажмите сочетание клавиш CTRL+F5.

4. В адресной строке окна браузера добавьте *trace.axd* к URL-адресу и нажмите клавишу ВВОД (URL-адрес будет выглядеть, как этот: http://localhost:53370/trace.axd).
...
5. На странице **Трассировка приложения** щелкните **Просмотр сведений** в первой строке (не в строке BrowserLink).
	...
	По умолчанию `trace.axd` доступен только локально. Если он должен быть доступен также на удаленном сайте, можно добавить строку `localOnly="false"` to the `trace` в элемент файла *Web.config*, как показано в следующем примере:

		<trace enabled="true" writeToDiagnosticsTrace="true" localOnly="false" mostRecent="true" pageOutput="false" />

	Но включение `trace.axd` в рабочем приложении обычно не рекомендуется по соображениям безопасности. В следующих разделах вы ознакомитесь с более простым способом чтения журналов трассировки веб-приложения Azure.
...
1. На вкладке **Конфигурация веб-приложения Azure**, открытой в окне **Обозреватель сервера**, измените значение параметра "Ведение журналов веб-сервера" на **Включено** и нажмите **Сохранить**.
...
4. В окне браузера, в котором отображается веб-приложение, щелкните **Главная**, затем - **О программе** и, наконец, **Связь**.
...
1. На вкладке **Конфигурация веб-приложения Azure**, открытой в окне **Обозреватель сервера**, измените значение параметра **Подробные сообщения об ошибках** на **Включено**, затем нажмите **Сохранить**.
...
7. В окне браузера, в котором отображается веб-приложение приложения, щелкните **Главная**, затем - **О программе** и, наконец, **Связь**.
...
1. На вкладке **Конфигурация** окна **Веб-приложение Azure**, открытого в окне **Обозреватель сервера** измените значение параметра **Трассировка неудачно завершенных запросов** на **Включено**, затем нажмите **Сохранить**.
...
2. На вкладке Visual Studio **Конфигурация** окна **Веб-приложение Azure** щелкните **Открыть на портале управления**.
...
Дополнительную информацию об использовании режима отладки в Visual Studio см. в разделе MSDN [Отладка в Visual Studio](http://msdn.microsoft.com/library/vstudio/sc65sadd.aspx) и [Рекомендации по отладке в Visual Studio 2010](http://weblogs.asp.net/scottgu/archive/2010/08/18/debugging-tips-with-visual-studio-2010.aspx).
...
* [Потоковая передача диагностических журналов трассировки из командной строки Azure (включая Glimpse!)](http://www.hanselman.com/blog/StreamingDiagnosticsTraceLoggingFromTheAzureCommandLinePlusGlimpse.aspx)<br/>
  Как использовать командную строку, чтобы сделать все действия этого руководства, выполнение которые показано на примере Visual Studio. [Glimpse](http://www.hanselman.com/blog/IfYoureNotUsingGlimpseWithASPNETForDebuggingAndProfilingYoureMissingOut.aspx) - это инструмент для отладки приложений ASP.NET. 
