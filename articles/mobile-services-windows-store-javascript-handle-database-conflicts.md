...
-   Этот учебник создан на основе краткого руководства по мобильным службам. Перед работой с этим учебником необходимо сначала пройти учебник [Приступая к работе с мобильными службами][Приступая к работе с мобильными службами] и загрузить версию проекта для JavaScript.
...
1.  Откройте в Visual Studio версию проекта TodoList для JavaScript, скачанного во время работы с учебником [Приступая к работе с мобильными службами][Приступая к работе с мобильными службами].
...
Мобильные службы Azure поддерживают управление оптимистичным параллелизмом за счет отслеживания изменений каждого элемента с помощью столбца системного свойства `__version`, который добавляется к каждой таблице. В этом разделе описано, как сделать так, чтобы приложение определяло эти конфликты записи с помощью системного свойства `__version`. После включения этого системного свойства для todoTable приложение во время попытки обновления будет получать уведомления об исключении `MobileServicePreconditionFailedException`, если с момента последнего запроса запись была изменена. Затем оно сможет выбрать, зафиксировать свои изменения в базе данных или оставить без изменений последнее изменение в базе данных. Дополнительные сведения о системных свойствах мобильных служб см. в [Системные свойства][Системные свойства]

1.  В файле default.js в объявлении переменной `todoTable` добавьте следующий код, чтобы включить в него системное свойство \*\*\_\_version\*\*. Это обеспечит поддержку обнаружения конфликтов записи.
...
7.  В экземпляре 1 приложения измените текст последнего элемента на **Тест записи 1**, а затем нажмите клавишу **ВВОД**, чтобы обновить базу данных. На следующем снимке экрана показан пример.
...
8.  На этом этапе последний элемент в экземпляре 2 приложения содержит старую версию элемента. В этом экземпляре приложения введите **Тест записи 2** для свойства `text` последнего элемента и нажмите клавишу **ВВОД**, чтобы обновить базу данных с использованием старого свойства `_version`.
...
9.  Так как значение `__version`, использованное при попытке обновления, не соответствует значению `__version` сервера, пакет SDK для мобильных служб высылает уведомление `MobileServicePreconditionFailedException` в качестве ошибки в функции `updateTodoItem`, позволяющее приложению разрешить этот конфликт. Чтобы разрешить конфликт, нажмите **Да** для сохранения значений из экземпляра. Или же нажмите **Нет**, чтобы отклонить значения в экземпляре 2 и использовать значения из экземпляра 1 приложения.
...
5.  Запустите приложение **todolist** на обоих компьютерах. Измените поле `text` таблицы TodoItem для последнего элемента в экземпляре 2 и нажмите клавишу **ВВОД**, чтобы приложение обновило базу данных.
...
6.  В экземпляре 1 приложения введите отличающееся значение для последнего текстового свойства и нажмите клавишу **ВВОД**. Приложение попытается обновить базу данных с неправильным свойством `__version`.
...
9.  В экземпляре 2 попытайтесь обновить текст последнего элемента TodoItem и нажмите клавишу **ВВОД**. Это приведет к конфликту, так как элемент был обновлен, а в поле complete задано значение true. В ответ на конфликт сценарий разрешил его, отказавшись от обновления, потому что элемент уже завершен. Скрипт предоставляет сообщение в ответе.
