...
В Azure для обработки ресурсоемких задач можно использовать виртуальную машину. Например, виртуальная машина может обрабатывать задачи и предоставлять результаты на клиентские компьютеры или в мобильные приложения. После прохождения этого руководства вы будете иметь представление о создании виртуальной машины с выполняемым ресурсоемким приложением Java, за работой которого может следить другое приложение Java.

В этом учебнике предполагается, что вы знаете, как создавать консольные приложения Java, импортировать библиотеки в свое приложение Java и создать архив Java (JAR). Знания о платформе Azure не требуются.

Вы узнаете:

-   Как создать виртуальную машину с установленным JDK.
...
6.  В следующем диалоговом окне **Конфигурация виртуальной машины**:
    ...
    2.  Значение поля **DNS-имя облачной службы** должно быть уникальным для cloudapp.net. При необходимости измените это значение, чтобы система Azure показывала, что оно уникально.
...
7.  В последнем диалоговом окне **Конфигурация виртуальной машины**:
...
Обратите внимание, что функции Azure Service Bus требуют, чтобы в хранилище **cacerts** JRE был установлен сертификат Baltimore CyberTrust Root. Этот сертификат автоматически включается в состав среды JRE, используемой в этом учебнике. Если у вас нет этого сертификата в хранилище **cacerts** JRE, сведения о его добавлении (а также сведения о просмотре сертификатов в хранилище cacerts) см. в разделе [Добавление сертификата в хранилище сертификатов ЦС Java][Добавление сертификата в хранилище сертификатов ЦС Java].
...
1.  Выполните вход на [портал управления Azure][портал управления Azure].
2.  В нижней левой области навигации портала управления щелкните **Service Bus, Access Control и Caching**.
3.  В верхней левой области портала управления выберите узел **Service
    Bus**, а затем нажмите кнопку **Создать**.
    ![Снимок экрана узла Service Bus][Снимок экрана узла Service Bus]
4.  В диалоговом окне **Создание нового пространства имен службы** введите
    **Пространство имен** и убедитесь, что данное значение уникально, нажав кнопку
    **Проверить доступность**.
    ![Снимок экрана создания пространства имен][Снимок экрана создания пространства имен]
5.  Убедившись в доступности имени пространства имен, выберите
    страну или регион, где должно размещаться это пространство имен, и нажмите кнопку **Создать пространство имен**.

    Созданное пространство имен появится на портале управления,
    его активация займет некоторое время. Прежде чем продолжить, дождитесь отображения состояния **Активно**.
...
3.  Правая область **Свойства** будет содержать свойства для
    нового пространства имен:
    ![Снимок экрана панели свойств][Снимок экрана панели свойств]
4.  Параметр **Ключ по умолчанию** скрыт. Нажмите кнопку **Просмотр**, чтобы отобразить учетные
    данные безопасности:
    ![Снимок экрана ключа по умолчанию][Снимок экрана ключа по умолчанию]
5.  Запишите значения **Издатель по умолчанию** и **Ключ по умолчанию**, так как эти данные
    будут использованы ниже для выполнения операций с
    пространством имен.

## Как создать приложение Java, выполняющее ресурсоемкую задачу

1.  На компьютере для разработки (который не обязательно должен быть созданной вами виртуальной машиной) скачайте [пакет Azure SDK для Java][пакет Azure SDK для Java].
...
7.  Вам необходимо создать очередь Service Bus перед запуском перестановок средства поиска решения для задачи коммивояжера. Выполните следующую команду, чтобы создать очередь Service Bus:
...
Как для средства поиска решения, так и для клиентского приложения можно нажать клавиши **CTRL+C** для выхода из программы, если потребовалось прервать работу до ее нормального завершения.
  ...
  [Средство поиска решения для задачи коммивояжера]: ./media/virtual-machines-java-run-compute-intensive-task/WA_JavaTSPSolver.png
  [Клиент задачи коммивояжера]: ./media/virtual-machines-java-run-compute-intensive-task/WA_JavaTSPClient.png
  [портал управления Azure]: https://manage.windowsazure.com
  [Добавление сертификата в хранилище сертификатов ЦС Java]: ../java-add-certificate-ca-store
  [Снимок экрана узла Service Bus]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_02_SvcBusNode.jpg
  [Снимок экрана создания пространства имен]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_03_CreateNewSvcNamespace.jpg
  [Снимок экрана доступных пространств имен]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_04_SvcBusNode_AvailNamespaces.jpg
  [Снимок экрана списка пространств имен]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_05_NamespaceList.jpg
  [Снимок экрана панели свойств]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_06_PropertiesPane.jpg
  [Снимок экрана ключа по умолчанию]: ./media/virtual-machines-java-run-compute-intensive-task/SvcBusQueues_07_DefaultKey.jpg
