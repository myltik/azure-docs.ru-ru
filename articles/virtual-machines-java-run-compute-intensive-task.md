<properties linkid="dev-java-compute-load" urlDisplayName="Задача коммивояжера на виртуальной машине" pageTitle="Ресурсоемкое приложение Java на виртуальной машине — Azure" metaKeywords="виртуальная машина Java в Azure, приложение Java в Azure, приложение Java в Azure" description="Узнайте, как создать виртуальную машину Azure для выполнения ресурсоемкого приложения Java, которое может контролироваться другим приложением Java." metaCanonical="" services="virtual-machines" documentationCenter="Java" title="Выполнение ресурсоемкой задачи в Java-коде на виртуальной машине" authors="waltpo" videoId="" scriptId="" solutions="" manager="bjsmith" editor="mollybos" />
...
2. Щелкните **Создать**, **Среда выполнения приложений**, **Виртуальная машина**, а затем **Из галереи**.
...
4. В диалоговом окне **Конфигурация виртуальной машины**:
    ...
    5. Щелкните **Далее**.
5. В следующем диалоговом окне **Конфигурация виртуальной машины**:
    1. Используйте для параметра **Облачная служба** значение по умолчанию **Создать новую облачную службу**.
    ...
    4. Щелкните **Далее**.
5. В последнем диалоговом окне **Конфигурация виртуальной машины**:
    1. Примите стандартные параметры для конечной точки.
    2. Щелкните **Выполнено**.
...
1.  Выполните вход на [портал управления Azure](https://manage.windowsazure.com).
2.  В нижней левой области навигации портала управления щелкните элемент **Service Bus, Access Control и Caching**.
...
Для выполнения операций управления, таких как создание очереди, для
нового пространства имен необходимо получить учетные данные управления для
пространства имен.

1.  В левой навигационной области щелкните узел **Service Bus**, чтобы отобразить список доступных пространств имен:   
...
5.  Запишите значения **Издатель по умолчанию** и **Ключ по умолчанию**, так как эти сведения
    будут использованы ниже для выполнения операций с
    пространством имен. 

## Как создать приложение Java, выполняющее ресурсоемкую задачу

1. На компьютере для разработки (который не обязательно должен быть созданной вами виртуальной машиной) загрузите [Пакет Azure SDK для Java](http://www.windowsazure.com/ru-ru/develop/java/).
...
### Как запускать ресурсоемкое приложение
...
4. Создайте файл с именем **c:\TSP\cities.txt** и следующим содержимым:
...
7. Необходимо создать очередь Service Bus перед запуском перестановок средства поиска решения для задачи коммивояжера. Выполните следующую команду, чтобы создать очередь Service Bus:
...
1. Войдите на компьютер, где выполняется клиентское приложение. Это не обязательно должен быть именно компьютер с приложением **TSPSolver**.
2. Создайте папку, где будет выполняться приложение. Например, **c:\TSP**.
