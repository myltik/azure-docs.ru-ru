...
Следующие элементы обеспечить безопасный доступ к базе данных:
...
Параметры сервера PostgreSQL определяют конфигурацию сервера. В базе данных Azure для PostgreSQL список параметров, можно просматривать и изменять с помощью портала Azure или Azure CLI. 
