...
Объем памяти на единицу вычислений оптимизируется для ценовых категорий "Базовый" и "Стандартный". Повысить уровень производительности, удвоив число единиц вычислений, означает удвоить количество ресурсов, доступных этой базе данных Azure для PostgreSQL.
...
Если необходимо перенести существующий локальный сервер PostgreSQL или сервер PostgreSQL, работающий в виртуальной машине, то для определения числа единиц вычислений можно оценить, сколько ядер потребуется для обеспечения пропускной способности, необходимой для данной рабочей нагрузки. 

Если сейчас ваш сервер, работающий локально или в виртуальной машине, использует 4 ядра (не считая технологии Hyper-Threading в ЦП), то для базы данных Azure для сервера PostgreSQL можно сначала настроить 400 единиц вычислений. Число единиц вычислений может масштабироваться динамически в зависимости от потребностей рабочих нагрузок. Простои приложения при этом практически отсутствуют. 
