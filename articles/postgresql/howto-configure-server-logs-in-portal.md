---
title: "Настройка журналов сервера для PostgreSQL и получение к ним доступа на портале Azure | Документация Майкрософт"
...
5. Нажмите кнопку развертывания **Показать больше**, чтобы просмотреть развернутый список доступных параметров. 

   Дополнительные сведения об определениях параметров см. в документации PostgreSQL по [отчетам об ошибках и ведению журнала](https://www.postgresql.org/docs/current/static/runtime-config-logging.html).

   ![Короткий список параметров журнала. Нажатие кнопки "Показать больше" для отображения полного списка](./media/howto-configure-server-logs-in-portal/2-show-more.png)

6. Измените параметры, которые необходимо настроить. Все изменения, вносимые в этом сеансе, выделены фиолетовым цветом.
...
7. Вернитесь к списку журналов, нажав **кнопку закрытия** (значок "X") на странице **Параметры сервера**.
...
- Дополнительные сведения о PostgreSQL ведения журнала и определения параметров см. в документации PostgreSQL на [отчеты об ошибках и ведение журнала](https://www.postgresql.org/docs/current/static/runtime-config-logging.html).
