...
База данных Azure для сервера PostgreSQL создается с определенным набором [вычислительных ресурсов и ресурсов хранения](./concepts-compute-unit-and-storage.md). Сервер создается в пределах [группы ресурсов Azure](../azure-resource-manager/resource-group-overview.md).
...
3.    Заполните форму сведений о новом сервере, указав следующую информацию, как показано на предыдущем рисунке.
    - Имя сервера — **mypgserver-20170401** (имя сервера сопоставляется с DNS-именем, поэтому оно должно быть глобально уникальным). 
    - Подписка. Если вы используете несколько подписок, выберите соответствующую подписку, в которой находится ресурс, либо за которую взимается плата.
    - Группа ресурсов — **myresourcegroup**.
    - Учетные данные администратора сервера для входа и выбранный пароль.
    - Расположение
    - Версия PostgreSQL.
...
7.    На панели инструментов щелкните **Уведомления**, чтобы отслеживать процесс развертывания.
 ![База данных Azure для PostgreSQL. Просмотр уведомлений](./media/quickstart-create-database-portal/3-notifications.png)
  ...
  По умолчанию база данных **postgres** создается на сервере. База данных [postgres](https://www.postgresql.org/docs/9.6/static/app-initdb.html) — это база данных по умолчанию, предназначенная для использования пользователями, служебными программами и сторонними приложениями. 
...
1.    По завершении развертывания щелкните **Все ресурсы** в меню слева и введите имя **mypgserver-20170401**, чтобы найти созданный сервер. Щелкните имя сервера, указанное в результатах поиска. После этого откроется страница **обзора** сервера с параметрами для дальнейшей конфигурации.
...
3. Выберите страницу **обзора** сервера. Запишите значения **имени сервера** и **имени для входа администратора сервера**.
 ...
 ![База данных Azure для PostgreSQL. Учетные данные администратора сервера для входа](./media/quickstart-create-database-portal/6-server-name.png)
...
Теперь подключимся к базе данных Azure для сервера PostgreSQL с помощью служебной программы командной строки psql. 
...
3. Подключитесь к базе данных Azure для сервера PostgreSQL, выполнив команду psql в командной строке Cloud Shell. Используйте следующий формат, чтобы подключиться к базе данных Azure для сервера PostgreSQL с помощью служебной программы [psql](https://www.postgresql.org/docs/9.6/static/app-psql.html):
   ```bash
...
Чтобы подключиться к серверу PostgreSQL Azure с помощью средства графического интерфейса пользователя _pgAdmin_, сделайте следующее:
...
9.    Выберите имя базы данных **mypgsqldb** и ее владельца для учетных данных администратора сервера для входа **mylogin**.
...
- Чтобы создать базу данных Azure для сервера PostgreSQL с помощью Azure CLI, ознакомьтесь с [этой статьей](./quickstart-create-server-database-azure-cli.md).
- Технический обзор см. в статье [What is Azure Database for PostgreSQL?](./overview.md) (Что такое база данных Azure для PostgreSQL?)
