<properties
	pageTitle="Использование C# с Hive и Pig в Azure HDInsight"
...
#Использование C# с Hive и Pig в HDInsight
...
* Visual Studio следующих версий:
	...
	* Visual Studio 2012 Professional, Premium или Ultimate с <a href="http://www.microsoft.com/download/details.aspx?id=39305" target="_blank">обновлением 4</a>;

	* Visual Studio 2013 Community, Professional, Premium или Ultimate с <a href="https://www.microsoft.com/download/details.aspx?id=44921" target="_blank">обновлением 4</a>;
...
* Инструменты Hadoop для Visual Studio. Пошаговые указания по установке и настройке инструментов см. в разделе <a href="../hdinsight-hadoop-visual-studio-tools-get-started" target="_blank">Начало работы со средствами HDInsight Hadoop для Visual Studio</a>.
...
6. В качестве запроса Hive используйте следующее:
    ...
    Этот запрос предусматривает выбор полей `clientid`, `devicemake` и `devicemodel` в `hivesampletable` и их передачу в приложение HiveCSharp.exe. В результате запроса приложение должно возвратить три поля, которые хранятся под именами `clientid`, `phoneLabel` и `phoneHash`. Кроме этого, в корневом контейнере хранилища по умолчанию должен быть найден файл HiveCSharp.exe (`add file wasb:///HiveCSharp.exe`).
...
1. Для потоковой передачи Pig требуется локальное приложение в файловой системе кластера. Включите удаленный рабочий стол для кластера HDInsight, а затем выполните подключение, следуя указаниям в разделе <a href="http://azure.microsoft.com/documentation/articles/hdinsight-administer-use-management-portal/#rdp" target="_blank">Подключение к кластерам HDInsight с использованием протокола RDP</a>.
