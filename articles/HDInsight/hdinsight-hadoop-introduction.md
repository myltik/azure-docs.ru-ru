...
* Azure HDInsight позволяет развертывать и подготавливать кластеры Hadoop в облаке, используя **Linux** или **Windows** в качестве базовой операционной системы.
	...
	* **HDInsight на платформе Windows** — это кластер Hadoop под управлением Windows Server. Используйте его, если вы знакомы со средой Windows и вам необходимо выполнить миграцию с существующего решения Hadoop под управлением Windows или интеграцию с .NET или другими решениями для Windows.
	...
	В следующей таблице представлено сравнение этих двух кластеров.

	Категория | HDInsight на платформе Linux | HDInsight на платформе Windows 
	---------| -------------------| --------------------
	**ОС кластера** | Ubuntu 12.04 Long Term Support (LTS) | Windows Server 2012 R2
	**Тип кластера** | Hadoop | Hadoop, Hbase, Storm
	**Развертывание** | Портал управления Azure, кроссплатформенная командная строка, Azure PowerShell | Портал управления Azure, кроссплатформенная командная строка, Azure PowerShell, пакет SDK для HDInsight .NET
...
* HDInsight использует распределение Hadoop **Hortonworks Data Platform (HDP)**.

* Apache Hadoop представляет собой программную платформу для управления данными большого размера и их анализа. HDInsight предоставляет несколько конфигураций для определенных рабочих нагрузок. Вы также можете <a href="http://azure.microsoft.com/documentation/articles/hdinsight-hadoop-customize-cluster/" target="_blank">настроить кластеры, используя действия сценария</a>.
	...
	* **<a target="_blank" href="http://hbase.apache.org/">HBase</a>** — это база данных NoSQL, которая создана на основе Hadoop и обеспечивает прямой доступ и строгую согласованность для больших объемов неструктурированных и частично структурированных данных (с потенциальным размером таблиц в миллиарды строк и миллионы столбцов). См. раздел [Общие характеристики HBase в HDInsight](hdinsight-hbase-overview.md).
	
	* **<a  target="_blank" href="https://storm.incubator.apache.org/">Apache Storm</a>** — это распределенная система для вычислений в реальном времени, которая позволяет быстро обрабатывать потоки данных большого размера. Storm предлагается в качестве управляемого кластера в HDInsight. См. раздел [Анализ данных, передаваемых датчиками в реальном времени, с помощью Storm и Hadoop](hdinsight-storm-sensor-data-analysis.md).
...
* **[Hive](#hive)** — обеспечивает создание запросов на языке аналогичном SQL.
...
* Новейшие компоненты Hadoop. Дополнительную информацию см. в статье [
* Новые возможности версий кластеров Hadoop, предоставляемых HDInsight][component-versioning].
...
* Эффективное и вместительное хранилище данных BLOB-объектов Azure, версия с поддержкой Hadoop. Более подробно см. раздел [Использование хранилища BLOB-объектов Azure с HDInsight](../hdinsight-use-blob-storage.md).

* Интеграция с другими сервисами Azure, включая [Веб-сайты](../documentation/services/websites/) и [Базу данных SQL](../documentation/services/sql-database/).
...
* [Приступая к работе с HDInsight в Linux](../hdinsight-hadoop-linux-get-started.md) — это краткий учебник по подготовке кластеров HDInsight Hadoop в Linux и по выполнению примеров запросов Hive.

* [Подготовка HDInsight в Linux с использованием настраиваемых параметров](hdinsight-hadoop-provision-linux-clusters.md) — узнайте, как подготовить кластер HDInsight Hadoop в Linux с использованием настраиваемых параметров на портале управления Azure, с помощью кроссплатформенной командной строки Azure или Azure PowerShell.

* [Работа с HDInsight в Linux](hdinsight-hadoop-linux-information.md) — получите краткие советы по работе с кластерами Hadoop под управлением Linux, подготовленными в Azure.

* [Управление кластерами HDInsight с помощью Ambari](hdinsight-hadoop-manage-ambari.md) — узнайте, как выполнять мониторинг работы кластера Hadoop в HDInsight под управлением Linux и управлять им, используя веб-интерфейс API Ambari или REST API Ambari.
...
* [Приступая к работе с Azure HDInsight](../hdinsight-get-started.md) — краткий учебник по использованию Hadoop в HDInsight.
...
* <a target="_blank" href="http://hadoop.apache.org/">Apache Hadoop</a> — узнайте о библиотеке программного обеспечения Apache Hadoop, платформе, которая позволяет распределенно обрабатывать большие наборы данных по компьютерным кластерам.	
...
* [Портал управления для базы данных SQL](https://msdn.microsoft.com/library/azure/dn771027.aspx) — легкий и простой в использовании инструмент управления базами данных для управления базой данных SQL в облаке.

* [Adventure Works для базы данных SQL](http://msftdbprodsamples.codeplex.com/releases/view/37304) — страница скачивания примера базы данных SQL.
...
* <a target="_blank" https://msdn.microsoft.com/library/hh231701.aspx">Дополнительная информация о службах анализа SQL Server</a>.
