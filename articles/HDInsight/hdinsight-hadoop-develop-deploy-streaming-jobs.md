...
1. На рабочем столе щелкните **Состояние Hadoop YARN** или перейдите к файлу **http://localhost:50030/jobtracker.jsp**.2. Найдите задание, используя идентификатор задания в категории **ЗАПУЩЕННЫЕ** или **ЗАВЕРШЕННЫЕ**. 
...
Служба Azure HDInsight использует хранилище BLOB-объектов Azure в качестве файловой системы по умолчанию. Можно настроить кластер HDInsight на использование дополнительного хранилища BLOB-объектов для хранения файлов данных. В этом разделе мы создадим учетную запись хранения Azure и передадим файлы данных в хранилище больших двоичных объектов. В качестве файлов данных используются TXT-файлы в каталоге %hadoop_home%\\share\\doc\\hadoop\\common.
...
**Запуск сценария Azure PowerShell**
...
3. Задайте первые четыре переменных скрипта: Переменная **$stringPrefix** используется для добавления префикса к имени кластера HDInsight, имени учетной записи хранения и имени контейнера хранилища больших двоичных объектов. Так как количество символов в имени должно быть больше 3 и меньше 24, следите за тем, чтобы общее количество символов в строке и именах, используемых сценарием, не превышало максимального количества. Для **$stringPrefix** необходимо использовать буквы только в нижнем регистре.

	Переменные **$storageAccountName_Data** и **$containerName_Data** — это учетная запись хранения и контейнер, которые вы создали ранее. поэтому им необходимо присвоить имена. Они предназначены для хранения файлов данных и приложений. Переменная **$location** должна соответствовать расположению учетной записи хранения данных.
