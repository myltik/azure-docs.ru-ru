<properties 
   pageTitle="Настройка DNS между двумя виртуальными сетями Azure | Azure" 
...
- **Две виртуальные сети Azure с VPN-подключением**. Инструкции см. в разделе [Настройка VPN-подключения между двумя виртуальными сетями Azure][hdinsight-hbase-replication-vnet].
...
4.	Введите:
	...
	- **УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ**: Использовать автоматически созданную учетную запись хранения
		...
		Имя облачной службы будет совпадать с именем виртуальной машины. В нашем случае это Contoso-DNS-EU. Для последующих виртуальных машин можно использовать ту же службу. Все виртуальные машины в одной и той же облачной службе совместно используют одну виртуальную сеть и одинаковый суффикс домена.
...
DNS-серверам требуются статические IP-адреса. Это действие нельзя выполнить на портале Azure. Вы будете использовать Azure PowerShell.
...
1.	На портале Azure щелкните **Виртуальные машины** слева. 
...
1.	На портале Azure нажмите **Создать**, **СЕТЕВЫЕ СЛУЖБЫ**, **ВИРТУАЛЬНАЯ СЕТЬ**, **ЗАРЕГИСТРИРОВАТЬ DNS-СЕРВЕР**.
2.	Введите:
	- **ИМЯ**: Contoso-DNS-EU
...
1.	Щелкните **Сети** в левой области на портале управления.
