<properties linkid="manage-scenarios-how-to-scale-websites" urlDisplayName="Масштабирование" pageTitle="Масштабирование веб-сайтов - управление службами Microsoft Azure" metaKeywords="Масштабирование веб-сайтов Azure" description="Узнайте, как масштабировать веб-сайты в Azure для использования планов размещения веб-сайтов "Бесплатный", "В общем доступе" и "Стандартный"." metaCanonical="" services="web-sites" documentationCenter="" title="Масштабирование веб-сайтов" authors="timamm"  solutions="" writer="timamm" manager="paulettm" editor="mollybos"  />

# Масштабирование веб-сайтов #

Для повышения производительности и пропускной способности своих веб-сайтов Microsoft Azure можно использовать портал управления Azure, выполняя масштабирование плана размещения веб-сайтов с "Бесплатный" до "В общем доступе", "Базовый" или "Стандартный". 

Масштабирование веб-узлов Azure включает два связанных действия: первым является перевод плана размещения веб-сайтов на более высокий уровень обслуживания, а вторым — настройка некоторых параметров после перехода на более высокий уровень обслуживания. В данной статье рассматриваются оба этих действия. Более высокие уровни обслуживания, такие как режим "Стандартный", обеспечивают повышение надежности и гибкости при определении способа использования ресурсов в Azure.
...
Сведения о планах размещения веб-сайтов см. в статье [Планы размещения веб-сайтов Azure](http://go.microsoft.com/fwlink/?LinkId=9845584). Сведения о ценах и возможностях отдельных планов размещения веб-сайтов см. в статье [Сведения о ценах для веб-сайтов](http://www.windowsazure.com/ru-ru/pricing/details/web-sites/).  

> [WACOM.NOTE] Прежде чем перевести веб-сайт с плана размещения веб-сайтов **Бесплатный** на план **Базовый** или **Стандартный**, необходимо сначала снять существующие ограничения расходов, установленные для вашей подписки на веб-сайт. Сведения о просмотре или изменении параметров подписки для веб-сайтов Microsoft Azure см. в разделе [Подписки Microsoft Azure][azuresubscriptions].
...
2. На вкладке **Веб-сайты** выберите свой веб-сайт.
	...
	![Выбор веб-сайта][SelectWebsite]
	
3. Выберите вкладку **Масштаб**.
	
	![Вкладка "Масштаб"][SelectScaleTab]
...
4. В разделе **Режим плана размещения веб-сайтов** выберите либо **В ОБЩЕМ ДОСТУПЕ**, либо **БАЗОВЫЙ**. В приведенном примере изображения выбирается план "Базовый".
	...
	![Выбор плана размещения веб-сайтов][ChooseWHP]
...
5. В разделе **Емкость** выберите **Размер экземпляра**. Доступны следующие варианты: **Малый**, **Средний** или **Крупный**. Параметр размера экземпляра недоступен в режиме "В общем доступе". Дополнительные сведения об использовании размеров экземпляров см. в разделе [Размеры виртуальных машин и облачных служб в Microsoft Azure][vmsizes].
	
	![Размер экземпляра для режима "Базовый"][ChooseBasicInstanceSize]
	
6. Используйте ползунок, чтобы выбрать нужное **Число экземпляров**.
	
	![Число экземпляров для режима "Базовый"][ChooseBasicInstanceCount]
	
7. В панели команд выберите **Сохранить**. 
	
	![Кнопка сохранения][SaveButton]
 	
	> [WACOM.NOTE] При необходимости значения параметров **План размещения веб-сайтов**, **Размер экземпляра** и **Число экземпляров** можно настроить и сохранить параметры по отдельности.
...
8. Сообщение подтверждения напоминает, что режим для веб-сайтов в этом же плане размещения веб-сайтов также изменится на новый. Выберите **Да** для выполнения изменений. 
	...
	![Выполнение изменения плана][BasicComplete]
...
1. Для масштабирования с переключением на план "Стандартный" выполните те же начальные действия, что и при масштабировании с переключением на план "В общем доступе" или "Базовый", а затем выберите значение **Стандартный** для параметра **Режим плана размещения веб-сайтов**. 
	...
	![Выбор плана "Стандартный"][ChooseStandard]
...
2. Выбор значения **Стандартный** расширяет раздел **Емкость**, чтобы показать значения **Размер экземпляра** и **Число экземпляров**, также доступные в режиме "Базовый". Параметры **Изменить параметры масштабирования по расписанию** и **Масштабирование по метрике** доступны только в режиме "Стандартный".
	...
	![Раздел "Емкость" для режима "Стандартный"][CapacitySectionStandard]
	
3. Настройте **Размер экземпляра**. Доступны следующие варианты: **Малый**, **Средний** или **Крупный**
	
	![Выбор размера экземпляра][ChooseInstanceSize]
	
	Дополнительные сведения об использовании размеров экземпляров см. в разделе [Размеры виртуальных машин и облачных служб в Microsoft Azure][vmsizes].
...
4. Если нужно автоматически выполнять масштабирование (Автомасштабирование) ресурсов в зависимости от времени суток и/или конкретных дат и конкретных времен, выберите значение **Настройка планируемого времени** для параметра **Изменить параметры масштабирования по расписанию**.
	...
	![Настройка планируемого времени][SetUpScheduleTimesButton]
...
6. В разделе **Повторяющиеся расписания** выберите **Различное масштабирование днем и ночью** и **Различное масштабирование в рабочий день и выходные** для масштабирования ресурсов на основе отдельных различных расписаний для дня и ночи и/или отдельных расписаний для будней и выходных.
	...
	> [WACOM.NOTE] Для этого компонента все выходные начинаются в конце дня пятницы (20:00 по умолчанию) и заканчиваются в начале дня в понедельник (8:00 по умолчанию). Для профиля выходных дней используются те же времена начала и окончания дня, которые определены для параметра **Время**.
...
10.   Теперь поле **Изменить параметры масштабирования по расписанию** содержит настраиваемые расписания или события на основе сделанных изменений. Выберите для настройки одно из повторяющихся расписаний или конкретные даты. 
	...
	![Изменение параметров масштабирования по расписанию][EditScaleSettingsForSchedule]
...
11.  Для динамического изменения числа экземпляров, используемых веб-сайтом при его загрузке, включите **Масштабирование по метрике**, выбрав **ЦП**.
	...
	![Масштабирование по метрике][ScaleByMetric]
...
12. **Масштабирование по метрике** изменяет **Число экземпляров**, чтобы можно было установить минимальное и максимальное число виртуальных машин, которое будет использоваться для автоматического масштабирования. В Azure установленные ограничения никогда не будут нарушены.
	...
	![Число экземпляров][InstanceCount]
	
13. **Масштабирование по метрике** также активирует параметр **Целевой ЦП**, позволяя задать целевой диапазон загрузки ЦП. Этот диапазон представляет среднюю загрузку ЦП для веб-сайта. Windows Azure будет добавлять или удалять стандартные экземпляры, чтобы удержать веб-сайт в этом диапазоне.
	
	![Целевой ЦП][TargetCPU]
	
	**Примечание**. Если функция **Масштабирование по метрике** включена, Microsoft Azure каждые пять минут проверяет ЦП веб-сайта и добавляет экземпляры по мере необходимости. В случае низкой загрузки ЦП Microsoft Azure будет удалять экземпляры раз в два часа, чтобы гарантировать эффективность работы веб-сайта. Обычно правильным является установка минимального значения счетчика экземпляров равным 1. Но если на веб-сайте возможны внезапные всплески использования ЦП, необходимо убедиться в наличии достаточного минимального количества экземпляров для надлежащей обработки этой нагрузки. Например, при появлении внезапного пика трафика в течение 5-минутного интервала перед проверкой загрузки ЦП в Microsoft Azure веб-сайт может перестать реагировать на запросы в это время. Если ожидаются неожиданные большие объемы трафика, увеличьте минимальное значение счетчика экземпляров, чтобы предвидеть эти всплески. 
...
1. Для масштабирования одной из баз данных щелкните в разделе **Связанные ресурсы** ссылку **Управление масштабированием для этой базы данных** рядом с именем базы данных.
	...
	![Связанная база данных][LinkedResources]
...
2. Эта ссылка открывает вкладку "SQL Server" портала управления Azure, на которой можно настроить параметры **Выпуск** и **Максимальный размер** для базы данных:
	...
	![Масштабирование сервера базы данных SQL.][ScaleDatabase]
...
- Режимы планов "Бесплатный" и "В общем доступе" поддерживают только 32-разрядные приложения.

**Поддержка отладчика**

- Поддержка отладчика доступна в режиме плана размещения веб-сайтов "Бесплатный", "В общем доступе" и "Базовый" для 1 одновременного подключения на каждое приложение.
...
- Мониторинг конечных веб-точек доступен в режиме плана размещения веб-сайтов "Базовый" и "Стандартный". Дополнительные сведения о мониторинге веб-точек см. в статье [Мониторинг веб-сайтов](http://www.windowsazure.com/ru-ru/documentation/articles/web-sites-monitor/).

- Подробные сведения обо всех остальных возможностях планов размещения веб-сайтов, включая цены и возможности, интересные для всех пользователей (включая разработчиков), см. в статье [Сведения о ценах для веб-сайтов](http://www.windowsazure.com/ru-ru/pricing/details/web-sites/).

<a name="Next Steps"></a>	
## Дальнейшие действия
- Рекомендации для веб-сайтов Azure, включая рекомендации по созданию масштабируемой и гибкой архитектуры, см. в статье [Рекомендации: веб-сайты Windows Azure (WAWS)](http://blogs.msdn.com/b/windowsazure/archive/2014/02/10/best-practices-windows-azure-websites-waws.aspx).
...
- Сведения о ценах, поддержке и соглашениях об уровне обслуживания см. по следующим ссылкам.
	...
	[Сведения о ценах — веб-сайты](http://www.windowsazure.com/ru-ru/pricing/details/web-sites/)
...
- Ниже приведены ссылки на видеоролики о масштабировании веб-сайтов Azure:
	...
	[Как масштабировать веб-сайты Azure - автор: Стефан Щаков (Stefan Schackow)](http://www.windowsazure.com/ru-ru/documentation/videos/how-azure-web-sites-scale/)
