<properties
   pageTitle="Настройка диагностики Azure для отправки данных в Application Insights | Microsoft Azure"
...
Для элемента **Sink** атрибут *name* задает строковое значение, которое будет использоваться для уникальной ссылки на приемник. Элемент **ApplicationInsights** указывает ключ инструментирования ресурса Application Insights, в который будут отправляться данные диагностики Azure. Если ресурс Application Insights еще не существует, см. раздел [Создание нового ресурса Application Insights](app-insights-create-new-resource.md), где содержатся дополнительные сведения о создании ресурса и получении ключа инструментирования.

При разработке проекта облачной службы с помощью пакета Azure SDK 2.8 этот ключ инструментирования заполняется автоматически в открытой конфигурации на основе параметра конфигурации службы **APPINSIGHTS\_INSTRUMENTATIONKEY** при создании пакета проекта облачной службы. См. раздел [Использование Application Insights с диагностикой Azure для устранения неполадок облачной службы](cloud-services-dotnet-diagnostics-applicationinsights.md).

Элемент **Channels** позволяет определить один или несколько элементов **Channel** для данных, которые будут отправлены в приемник. Канал действует как фильтр и позволяет выбирать конкретные уровни журнала, которые требуется отправить в приемник. Например, можно собирать подробные журналы и отправлять их в хранилище или определить канал с уровнем журнала Error (Ошибка), при отправке журналов по этому каналу в приемник будут отправляться только ошибки. Для элемента **Channel** атрибут *name* используется для однозначной ссылки на этот канал. Атрибут *Loglevel* позволяет указать уровень журнала для канала. Предусмотрены следующие уровни журнала в порядке от минимального объема информации: Verbose (Подробный), Information (Информация), Warning (Предупреждение), Error (Ошибка), Critical (Критический).

## Отправка данных в приемник Application Insights
...
- Используйте [PowerShell](cloud-services-diagnostics-powershell.md), чтобы включить расширение диагностики Azure для вашего приложения. 
