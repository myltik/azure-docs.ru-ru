<properties urlDisplayName="Tomcat on Virtual Machine" pageTitle="Tomcat на виртуальной машине &mdash; учебник по Azure" metaKeywords="Azure vm, creating vm Tomcat, configuring vm Tomcat" description="Узнайте, как создать виртуальную машину под управлением Windows и настроить ее для работы сервера приложений Apache Tomcat." metaCanonical="" services="virtual-machines" documentationCenter="Java" title="Использование сервера приложений Java на виртуальной машине" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
1.  Войдя в систему на виртуальной машине, откройте сеанс браузера <http://tomcat.apache.org/download-70.cgi>.
...
7.  В диалоговом окне **Сведения о новой конечной точке**:
...
![Новое правило для входящего подключения][Новое правило для входящего подключения]

1.  При создании нового правила выберите **Порт** для **Типа правила** и нажмите кнопку **Далее**.

![Порт нового правила для входящего подключения][Порт нового правила для входящего подключения]

1.  Выберите **TCP** в качестве протокола и укажите **8080** в качестве порта, затем нажмите **Далее**.

![Новое правило для входящего подключения][1]

1.  Выберите **Разрешить подключение**, а затем щелкните **Далее**.

![Действие нового правила для входящего подключения][Действие нового правила для входящего подключения]

1.  Убедитесь, что в профиле отмечены пункты **Домен**, **Частный** и **Общий**, а затем щелкните **Далее**.

![Профиль нового правила для входящего подключения][Профиль нового правила для входящего подключения]

1.  Укажите имя правила, например **HttpIn** (при этом имя правила не обязательно должно соответствовать имени конечной точки), и нажмите кнопку **Готово**.

![Имя нового правила для входящего подключения][Имя нового правила для входящего подключения]

С этого момента ваш веб-сайт Tomcat должен быть доступен для просмотра во внешнем браузере с использованием URL-адреса формата **[http://\*your\\_DNS\\_name][http://\*your\\_DNS\\_name]*.cloudapp.net**, где*** your\_DNS\_name**\* соответствует DNS-имени, указанному при создании виртуальной машины.

## Вопросы, связанные с жизненным циклом приложения

-   Вы можете создать собственный веб-архив приложения (WAR) и добавить его в папку **webapps**. Например, создайте динамический веб-проект базовой страницы службы Java и экспортируйте его как файл формата WAR, скопируйте WAR-файл в папку Apache Tomcat **webapps** на виртуальной машине и запустите его в браузере.
-   По умолчанию при установке службы Tomcat выбирается ручной запуск. Для переключения на автоматический запуск используйте оснастку служб. Запустите оснастку служб, нажав кнопку **«Пуск» в Windows**, **Администрирование**, а затем **Службы**. Чтобы настроить Tomcat на автоматический запуск, дважды щелкните службу **Apache Tomcat** в оснастке служб и для параметра **Тип запуска** выберите значение **Автоматически**, как показано далее.
    ...
    ![Настройка службы для автоматического запуска][Настройка службы для автоматического запуска]
...
-   Узнайте о других службах, таких как служба хранилища Azure, Service Bus, база данных SQL и т. д., которые можно включить в приложения Java, изучив информацию в <http://www.windowsazure.com/ru-ru/develop/java/>.

  [Виртуальная машина с Apache Tomcat]: ./media/virtual-machines-java-run-tomcat-application-server/WA_VirtualMachineRunningApacheTomcat.png
  [портал управления Azure]: https://manage.windowsazure.com
  [Новое правило для входящего подключения]: ./media/virtual-machines-java-run-tomcat-application-server/NewInboundRule.png
  [Порт нового правила для входящего подключения]: ./media/virtual-machines-java-run-tomcat-application-server/NewRulePort.png
