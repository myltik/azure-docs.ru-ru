...
1.  [Загрузка проекта приложения Windows Phone][Загрузка проекта приложения Windows Phone]
...
1.  [Тестирование приложения с мобильной службой][Тестирование приложения с мобильной службой]
2.  [Ручное обновление данных в серверной части для создания конфликта][Ручное обновление данных в серверной части для создания конфликта]

Для работы с этим учебником необходимы Visual Studio 2012 и [пакет Windows Phone 8 SDK][пакет Windows Phone 8 SDK].

## <a name="download-app"></a>Загрузка примера проекта

Этот учебник построен на [примере кода обработки конфликтов][примере кода обработки конфликтов], который является проектом приложения Windows Phone 8 в Visual Studio 2012.
Пользовательский интерфейс этого приложения аналогичен приложению в учебнике [Приступая к работе с автономными данными][Приступая к работе с автономными данными], за исключением того, что для каждого элемента TodoItem существует новый столбец даты.

![][0]

1.  Загрузите версию Windows Phone [примера кода обработки конфликтов][примере кода обработки конфликтов].

2.  Установите [решение SQLite для Windows Phone 8][решение SQLite для Windows Phone 8], если оно не установлено.
