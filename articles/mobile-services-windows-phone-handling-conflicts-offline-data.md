<properties pageTitle="Обработка конфликтов с автономными данными в мобильных службах (Windows Phone) | Центр мобильных разработок" description="Узнайте, как использовать возможности мобильных служб Azure для обработки конфликтов при синхронизации автономных данных в приложении Windows Phone." documentationCenter="windows" authors="wesmc7777" manager="dwrede" editor="" services=""/>
...
3. [Тестирование приложения с помощью мобильной службы]
4. [Ручное обновление данных в серверной части для создания конфликта]

Для работы с этим учебником необходимы Visual Studio 2012 и [пакет SDK для Windows Phone 8].
...
Пользовательский интерфейс для этого приложения аналогичен приложению в учебнике [Приступая к работе с автономными данными], за исключением того, что в нем имеется новый столбец даты для каждого элемента TodoItem.

![][0]


1. Загрузите версию [примера кода обработки конфликтов] для Windows Phone. 
...
В этом разделе вы обновите базу данных мобильной службы, чтобы добавить таблицу TodoItem со столбцом даты выполнения. Приложение позволяет изменить дату выполнения для элемента во время выполнения, чтобы создавать конфликты синхронизации в следующем разделе данного учебника. 
...
3. В обозревателе решений Visual Studio разверните папку **App_Start**, а затем откройте файл WebApiConfig.cs. 
...
4. В обозревателе решений Visual Studio разверните папку **Контроллеры** и откройте файл ToDoItemController.cs. Переименуйте класс `TodoItemController` в `TodoWithDateController`. Это приведет к изменению конечной точки REST для табличных операций. 

        public class TodoWithDateController : TableController<TodoItem>
    

5. В обозревателе решений Visual Studio щелкните правой кнопкой мыши проект мобильной службы для серверной части .NET и выберите пункт **Опубликовать** для публикации изменений.
...
9. Оставьте эмулятор **Emulator WVGA 512MB** запущенным для следующего раздела, в котором приложение будет запускаться в двух эмуляторах для создания конфликта.

## <a name="handle-conflict"></a>Обновление данных в серверной части для создания конфликта

В реальной ситуации конфликт синхронизации происходит в том случае, когда одно приложение отправляет обновления записи в базе данных, а затем другое приложение пытается отправить изменение в ту же запись, используя устаревшую версию поля в этой записи. Если экземпляр приложения пытается обновить ту же запись без извлечения обновленной записи, возникает конфликт, который перехватывается в приложении как исключение `MobileServicePreconditionFailedException`.  

В этом разделе будут одновременно запускаться два экземпляра приложения для создания конфликта. 


1. Если эмулятор **Emulator WVGA 512MB** еще не запущен, нажмите сочетание клавиш **CTRL+F5**, чтобы запустить его.

2. В Visual Studio измените устройство вывода на эмулятор **Emulator WVGA** и запустите второй экземпляр приложения в новом эмуляторе, нажав клавишу **F5**.
 
    ![][5]
 
   
3. Во втором экземпляре приложения нажмите **Получить**, чтобы синхронизировать локальное хранилище с базой данных мобильной службы. Оба экземпляра приложения должны иметь одни и те же данные.
 
    ![][6]

4. Во втором экземпляре приложения щелкните флажок, чтобы заполнить один из элементов, затем нажмите **Отправить**, чтобы отправить изменение в удаленную базу данных. На следующем снимке экрана раздел **Pick up James** заполнен, что указывает, что James уже был выбран. Первый экземпляр приложения теперь имеет устаревшую запись.
...
Для обработки конфликтов автономной синхронизации в коде создайте класс, реализующий интерфейс `IMobileServiceSyncHandler`. Передайте объект данного типа в вызов метода `InitializeAsync`:

     await App.MobileService.SyncContext.InitializeAsync(store, new SyncHandler(App.MobileService));

Класс `SyncHandler` в **MainPage.xaml.cs** реализует `IMobileServiceSyncHandler`. Метод `ExecuteTableOperationAsync` вызывается при каждой передаче операции принудительной отправки на сервер. Если возникает исключение типа `MobileServicePreconditionFailedException`, это означает, что возник конфликт между локальной и удаленной версиями элемента.
