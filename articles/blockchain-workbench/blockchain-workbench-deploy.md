...
### <a name="create-an-azure-ad-key-vault-application"></a>Создание приложения Key Vault Azure AD

Для развертывания Blockchain Workbench необходима регистрация приложения Key Vault Azure AD.

1. На портале Azure в области навигации слева выберите службу **Azure Active Directory**. Выберите **Регистрация приложений** > **Регистрация нового приложения**.
2. Укажите **Имя** и **URL-адрес входа** для приложения. Так как значения заполнителей могут быть измены во время развертывания, их можно использовать.

    ![Создание регистрации приложения Key Vault](media/blockchain-workbench-deploy/key-vault-app-create.png)

    | Параметр  | Значение  |
    |---------|---------|
    | ИМЯ | `Blockchain Key Vault app` |
    | Тип приложения | Веб-приложение или API |
    | URL-адрес входа | https://keyvaultclient |

5. Чтобы зарегистрировать приложение Key Vault Azure AD, выберите **Создать**.
...
11. Завершение настроек **Azure Monitor**.
    ...
    | Параметр | ОПИСАНИЕ  |
    |---------|--------------|
    | Мониторинг | Выберите, должен ли Azure Monitor применяться для мониторинга сети блокчейнов. |
    | Подключение к существующему экземпляру OMS | Вы можете выбрать, следует ли использовать существующую Operations Management Suite или создать новую. 
...
Теперь, когда вы развернули Azure Blockchain Workbench, необходимо ознакомиться со статьей [Manage Users in Azure Blockchain Workbench](blockchain-workbench-manage-users.md) (Управление пользователями в Azure Blockchain Workbench).