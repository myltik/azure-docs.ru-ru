...
В настоящее время Azure Key Vault не поддерживает blockchain-транзакции в Key Vault. Вместо этого Blockchain Workbench извлекает соответствующий закрытый ключ из Key Vault и подписывает транзакцию за пределами Key Vault. После подписания транзакция отправляется к маршрутизаторам транзакций и реестрам.
