---
title: "Новая версия схемы 2015-08-01-preview"
...
---
# <a name="new-schema-version-2015-08-01-preview"></a>Новая версия схемы 2015-08-01-preview
Новая версия схемы и API для Azure Logic Apps содержит ряд улучшений, которые повышают надежность и простоту использования приложений логики. Четыре основных отличия новой версии:
...
## <a name="1-moving-to-api-connections"></a>1. Переход к API-подключениям
...
Например, при использовании Office 365 необходимо создать подключение, которое содержит маркер входа в Office 365. Этот маркер безопасно хранится и обновляется, поэтому ваше приложение логики всегда может вызвать интерфейс API Office 365. С другой стороны, для подключения к серверу FTP или SQL Server необходимо создать подключение, которое содержит строку подключения. 
...
При использовании интерфейса API в нем могут быть определены или не определены **параметры подключения**. Если они не определены, то **подключение** не требуется. Если они определены, то необходимо создать подключение. При создании подключения это подключение имеет выбранное вами имя. Затем необходимо сослаться на это имя в объекте `connection` внутри объекта `host`. Чтобы создать подключение в группе ресурсов, выполните следующий вызов:
...
### <a name="deploying-managed-apis-in-an-azure-resource-manager-template"></a>Развертывание управляемых интерфейсов API в шаблоне Azure Resource Manager
...
Если вы используете собственные интерфейсы API (в частности, не управляемые корпорацией Майкрософт), то для их вызова следует использовать встроенное действие **HTTP**. Чтобы обеспечить идеальную работу, для интерфейса API следует предоставлять конечную точку Swagger. Эта конечная точка позволяет конструктору приложений логики обрабатывать входные и выходные данные для API. Без конечной точки Swagger конструктор может отображать входные и выходные данные только как непрозрачные объекты JSON.
...
Этот подход должен работать для всех действий приложений API. Однако следует помнить, что предыдущие приложения API больше не поддерживаются, поэтому следует выбрать один из двух других вариантов, указанных выше (управляемый интерфейс API или размещение пользовательского веб-API).

## <a name="2-repeat-renamed-to-foreach"></a>2) Переименование действия Repeat в Foreach

За время использования предыдущей версии схемы мы получили множество отзывов клиентов о том, что название **Repeat** приводит к путанице и не отображает того, что действие **Repeat** применяется к каждому циклу. В результате мы изменили название этого действия с **Repeat** на **Foreach**. Например, ранее вы бы написали так:
...
### <a name="referencing-the-outputs-of-the-foreach"></a>Ссылка на выходные данные действия Foreach
Для дальнейшего упрощения выходные данные действий **Foreach** не помещаются в объект **repeatItems**. Ранее выходные данные вышеупомянутого действия Repeat выглядели так:
...
Чтобы, ссылаясь на эти выходные данные, получить текст действия, необходимо сделать следующее:
...
## <a name="3-native-http-listener"></a>3. Встроенный прослушиватель HTTP
Возможности прослушивателя HTTP теперь являются встроенными, поэтому больше не нужно развертывать приложение API прослушивателя HTTP. Ознакомьтесь с [подробными инструкциями о том, как сделать конечную точку приложения логики вызываемой](../logic-apps/logic-apps-http-endpoint.md). 

После внесения этих изменений функция `@accessKeys()` была удалена и заменена функцией `@listCallbackURL()` для получения конечной точки (при необходимости). Кроме того, теперь в приложении логики необходимо определить хотя бы один триггер. Если вы хотите запустить (`/run`) рабочий процесс, то необходим один из этих триггеров: `manual`, `apiConnectionWebhook` или `httpWebhook`.

## <a name="4-calling-child-workflows"></a>4. Вызов дочерних рабочих процессов
Ранее для вызова дочерних рабочих процессов следовало перейти к нужному процессу, получить маркер доступа и вставить его в определение приложения логики, которое должно вызвать этот дочерний рабочий процесс. В новой версии схемы ядро Logic Apps автоматически создает SAS в среде выполнения для дочернего рабочего процесса. Это означает, что вам не нужно вставлять в определение никакие секреты.  Пример:
...
### <a name="parse-function-renamed"></a>Переименование функции parse()
...
Мы пока не располагаем управляемыми версиями интерфейсов API интеграции Enterprise (например, AS2). Скоро эти API станут доступными, как описано в [стратегическом плане](http://www.zdnet.com/article/microsoft-outlines-its-cloud-and-server-integration-roadmap-for-2016/). А пока вы можете использовать существующие развернутые API BizTalk с помощью действия HTTP, как описано выше в разделе "Использование уже развернутых приложений API".
