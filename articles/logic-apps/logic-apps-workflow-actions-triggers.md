...
Для эффективной работы с приложением логики триггеру HTTP требуется, чтобы HTTP API соответствовал определенному шаблону. Распознаются следующие свойства:  
...
|-----------------|----------------|------------|  
|200|\(Нет\)|Выполните рабочий процесс и после определенного количества повторений снова проверьте, не добавилось ли содержимое.|  
|200|10|Рабочий процесс запускается, и через 10 секунд проверяется наличие дополнительного содержимого.|  
|202|60|Рабочий процесс не запускается. Следующая попытка происходит через одну минуту с учетом определенного повторения. Если определенное повторение составляет менее минуты, приоритет имеет заголовок retry-after. В противном случае соблюдается определенное повторение.|  
|400|\(Нет\)|Недопустимый запрос, рабочий процесс не запускается. Если **политика повтора** не задана, используется политика по умолчанию. После достижения количества попыток триггер снова проверяет содержимое после определенного числа повторений.|  
|500|\(Нет\)|Ошибка сервера. Рабочий процесс не запускается.  Если **политика повтора** не задана, используется политика по умолчанию. После достижения количества попыток триггер снова проверяет содержимое после определенного числа повторений.|  
