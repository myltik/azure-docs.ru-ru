...
Первый шаг — это добавить триггер в определение приложения логики, способное принимать входящие запросы.  Можно выполнить поиск "HTTP Request" в конструкторе, чтобы добавить карту триггера. Вы можете определить схему JSON текста запроса, а конструктор создаст маркеры для анализа и передачи данных из триггера manual посредством рабочего процесса. Если у вас нет готовой схемы, выберите `Use sample payload to generate schema`, чтобы создать схему JSON на основе примера полезных данных.
