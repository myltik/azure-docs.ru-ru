...
2. Затем нужно добавить нашего приложения базы данных SQL Azure Cosmos API **InsertLogEntry** операции, как показано здесь в конструктор логику приложения.
...
Наши решения новые возможности в с [Azure Cosmos DB](https://azure.microsoft.com/services/cosmos-db).
...
* **ErrorController** вставляет записи об ошибке (документ) в коллекции Azure Cosmos DB.
* **LogController** вставляет записи в журнал (документ) в коллекции Azure Cosmos DB.

> [!TIP]
> Использовать оба контроллера `async Task<dynamic>` операций, что позволяет выполнять операции разрешения во время выполнения, поэтому схема базы данных Cosmos Azure можно создать в тексте операции. 
> 

Каждый документ в базе данных Azure Cosmos должен иметь уникальный идентификатор. Мы используем идентификатор `PatientId` , к которому добавляем метку времени, преобразованную в значение в формате Unix (значение с двойной точностью). Чтобы удалить дробное значение, мы обрежем метку времени.
...
* Azure Cosmos DB можно использовать как репозиторий для записей журнала и ошибок (документов).
