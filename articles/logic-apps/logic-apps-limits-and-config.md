...
В этом разделе описаны текущие ограничения и сведения о конфигурации приложений Azure Logic Apps.
...
Если необходимо превысить ограничения длительности выполнения или хранения при обычной последовательности обработки, то [свяжитесь с нами](mailto://logicappsemail@microsoft.com). Мы поможем удовлетворить ваши требования.
...
Эти ограничения относятся к одному экземпляру приложения логики.

| ИМЯ | Ограничение | Заметки | 
| ----- | ----- | ----- | 
| Выполняемые действия за 5 минут | 100 000 |<p>Ограничение можно увеличить до 300 000, запустив приложение логики в режиме `High Througput`. Для этого нужно изменить свойство `operationOptions` ресурса рабочего процесса `runtimeConfiguration` на `OptimizedForHighThroughput`. <p>Обратите внимание на то, что режим высокой пропускной способности доступен в предварительной версии. Кроме того, при необходимости рабочую нагрузку можно распределить по нескольким приложениям. | 
...
Чтобы превысить эти ограничения при обычной обработке или запустить нагрузочное тестирование, при котором возможно превышение этих ограничений, [свяжитесь с нами](mailto://logicappsemail@microsoft.com). Мы поможем удовлетворить ваши требования.
