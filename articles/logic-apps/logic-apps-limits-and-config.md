...
Эти ограничения относятся к одному HTTP-запросу или вызову соединителя.

#### <a name="timeout"></a>Время ожидания

| ИМЯ | Ограничение | Заметки | 
| ---- | ----- | ----- | 
| Время ожидания запроса | 120 секунд | При необходимости возможна компенсация с помощью [асинхронной схемы](../logic-apps/logic-apps-create-api-app.md) или [цикла Until](logic-apps-loops-and-scopes.md). |
|||| 
...
Эти ограничения относятся к одному выполнению приложения логики.

| ИМЯ | значение по умолчанию | Ограничение |
| ---- | ------- | ----- |
| Длительность выполнения   | 90 дней | от 7 до 90 дней |
| Хранение периода удержания | 90 дней со времени начала выполнения |  от 7 до 90 дней со времени начала выполнения |
||||

Если нужно увеличить пороговое значение для длительности выполнения или хранения при обычной последовательности обработки, [обратитесь в группу разработки продукта](mailto://logicappsemail@microsoft.com), чтобы обеспечить соответствие со своими требованиями.


### <a name="recurrence-interval"></a>Интервал повторений
...
Эти ограничения относятся к одному ресурсу приложения логики.

| ИМЯ | Ограничение | Заметки | 
| ----- | ----- | ----- | 
| Выполняемые действия за 5 минут | 100 000 |<p>Пороговое значение можно увеличить до 300 000. Для этого запустите приложение логики в режиме `High Througput`. Чтобы настроить режим высокой пропускной способности, установите для свойства `operationOptions` в разделе `runtimeConfiguration` ресурса рабочего процесса значение `OptimizedForHighThroughput`. <p>Обратите внимание, что режим высокой пропускной способности доступен в предварительной версии. Кроме того, при необходимости рабочую нагрузку можно распределить по нескольким приложениям. | 
...
Если нужно увеличить эти пороговые значения при обычной обработке или запустить нагрузочное тестирование, при котором возможно превышение этих ограничений, [обратитесь в группу разработки продуктов](mailto://logicappsemail@microsoft.com), чтобы обеспечить соответствие своим требованиями.

### <a name="logic-app-definition-limits"></a>Ограничения определений приложений логики

Эти ограничения относятся к одному определению приложения логики.
...
Эти ограничения относятся к артефактам, которые можно добавить в учетную запись интеграции.
...
Эти ограничения относятся к протоколам B2B.
