...
> [!IMPORTANT] 
> Убедитесь, что ресурс шлюза создается в том же регионе Azure, что и приложение логики. Если развернуть его в другом регионе, то он будет недоступен в приложении логики. 
> 
...
## <a name="data-gateway-connection-modifications"></a>Изменение подключения к шлюзу данных
После добавления подключения к шлюзу данных в приложение логики может потребоваться изменить его, чтобы более точно настроить параметры этого подключения. Подключение можно найти в одном из двух мест:
* В основной колонке приложения логики в разделе "Средства разработки" отображается панель подключений API. Если выбрать ее, отобразятся все подключения API, связанные с приложением логики. Одним из них будет ваше подключение к шлюзу данных. Выбрав его, можно просмотреть и изменить параметры, связанные с подключением.
* Если выбрать основную колонку "Подключения API", отобразятся все подключения API в подписке. В этом списке будет и подключение к шлюзу данных. Выбрав его, можно просмотреть и изменить параметры, связанные с подключением.
