...
* Требуется учетная запись Azure, у которой есть рабочий или учебный адрес электронной почты, используемый для [установки локального шлюза данных](logic-apps-gateway-install.md#requirements).

* Не должно быть заявок на доступ к этой установке шлюза от другого ресурса шлюза Azure. Вы можете связать установку шлюза только с одним ресурсом шлюза. Заявка осуществляется при создании ресурса шлюза, поэтому установка недоступна для других ресурсов.
