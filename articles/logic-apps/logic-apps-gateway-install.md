...
* Можно выполнить вход только с учетной записью Azure с рабочим или учебным адресом электронной почты, управляемым Azure Active Directory (Azure AD). Эта учетная запись необходима, чтобы связать локальный шлюз данных с подпиской Azure для учетной записи Azure AD.

  > [!TIP] 
  > Если вы используете учетную запись Майкрософт, например @outlook.com, вы можете использовать учетную запись Azure для [создания рабочего или учебного адреса электронной почты](../virtual-machines/windows/create-aad-work-id.md#locate-your-default-directory-in-the-azure-classic-portal). Если вы зарегистрировались для использования предложения Office 365 и не предоставили действительный рабочий электронный адрес, то ваш адрес для входа может выглядеть так: jeff@contoso.onmicrosoft.com. 
...
5. Теперь зарегистрируйте установленный шлюз в [облачной службе шлюза](#gateway-cloud-service). 

     Облачная служба шлюза шифрует и хранит учетные данные источников данных и сведения о шлюзах. 
     Эта служба также передает запросы и их результаты между пользователями в облаке, например приложениями логики, локальным шлюзом данных и локальными источниками данных.

     1. Укажите имя установленного шлюза и создайте ключ восстановления. 
     Подтвердите ключ восстановления.

        > [!IMPORTANT] 
        > Ключ восстановления должен содержать по крайней мере 8 знаков. Сохраните его в надежном месте. Этот ключ потребуется также, когда возникнет необходимость перенести, восстановить или перехватить существующий шлюз.

     2. Чтобы изменить регион по умолчанию для облачной службы шлюза и служебной шины Azure, используемый для установки шлюза, выберите **Изменить регион**.
     ...
     3. Когда все будет готово, щелкните **Настроить**.

6. Теперь выполните приведенные ниже инструкции на портале Azure, чтобы [создать ресурс Azure для шлюза](../logic-apps/logic-apps-gateway-connection.md). 
...
2. После запуска установщика укажите ключ восстановления для шлюза, который вы хотите перенести, восстановить или перехватить.
...
Шлюз данных обеспечивает быстрый и безопасный обмен данными между пользователем в облаке, например приложением логики, облачной службой шлюза и локальным источником данных. 
...
**Ответ**. Локальный шлюз данных можно связать только с рабочей или учебной учетной записью Azure. Учетная запись для входа хранится в клиенте, которым управляет Azure Active Directory (Azure AD). Обычно имя участника-пользователя учетной записи Azure AD совпадает с адресом электронной почты.
