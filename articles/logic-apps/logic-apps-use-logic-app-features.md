...
В [предыдущем разделе](../logic-apps/logic-apps-create-a-logic-app.md) вы создали свое первое приложение логики. Чтобы управлять рабочим процессом приложения логики, необходимо указать различные пути выполнения для него и способы обработки данных в массивах, коллекциях и пакетах. Эти элементы можно включить в рабочий процесс приложения логики:
...
2. Выберите действие `When_a_feed-item_is_published`, найдите раздел `queries` и замените значение запроса значением: `"feedUrl": "#@{parameters('currentFeedUrl')}"`. 
    ...
    Например `"@concat('#',parameters('currentFeedUrl'))"` работает так же, как описано выше.
