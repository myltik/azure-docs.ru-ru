<properties linkid="manage-linux-howto-service-management-api" urlDisplayName="API управления службой" pageTitle="Использование API-интерфейса управления службой для виртуальных машин — Azure" metaKeywords="" description="Узнайте, как использовать API-интерфейс управления службой Azure для виртуальной машины Linux." metaCanonical="" services="virtual-machines" documentationCenter="" title="Использование API-интерфейса управления службой" authors="" solutions="" manager="" editor="" />




#Использование API управления службой

##Инициализация##

Для вызова API-интерфейса управления службой Azure IaaS из NodeJS используется модуль `azure`.
...
- Auth — необязательный объект, который задает закрытый ключ и открытый сертификат для использования с этой учетной записью.	
	...
	- certvalue — фактическое значение открытого сертификата, хранящееся в PEM-файле.
	...
	- Если не удается загрузить закрытый ключ и открытый сертификат, выводится сообщение об ошибке.
...
- Options — необязательный объект, который может использоваться для управления свойствами, используемыми клиентом.	
	...
	- apiversion — строка версии для использования в HTTP-заголовке. Если не указана, используется версия по умолчанию.
...
- headers — фактические HTTP-заголовки ответа
...
- imageOptions — необязательный объект. Он может содержать следующие свойства:
	- Category
	- Label - по умолчанию принимает значение imageName, если не задана.
	- Location
	- RoleSize
...
- serviceOptions — необязательный объект. Он может содержать следующие свойства:
	...
	- Label — по умолчанию принимает значение serviceName, если не задана.
	- Location — по умолчанию принимает значение "Azure Preview", которое изменяет -TODO после выпуска.
-	callback — обязательный параметр.
...
- serviceName — обязательное строковое имя размещенной службы.
- callback — обязательный параметр.
- В случае успеха объект ответа будет содержать ключи доступа хранилища.
...
- deployOptions — необязательный объект. Он может содержать следующие свойства:
	...
	- Label — по умолчанию принимает значение deploymentName, если не задана.
...
- deploymentName — обязательное строковое имя развертывания.
- roleInstance — обязательное строковое имя экземпляра роли.
- captOptions — обязательный объект, определяющий действия записи
	- PostCaptureActions
...
- RoleSize — строка. Необязательна для создания.
