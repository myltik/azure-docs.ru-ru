...
[AZURE.INCLUDE [learn-about-deployment-models](../includes/learn-about-deployment-models-classic-include.md)] Вы также можете ознакомиться с [командами в рамках модели Resource Manager](virtual-machines/azure-cli-arm-commands.md).
...
Чтобы просмотреть текущий синтаксис и параметры команд в командной строке в режиме ASM, введите `azure help`. Чтобы получить справку по определенной команде, введите `azure help [command]`. Вы также найдете примеры для CLI в документации по созданию отдельных служб Azure и управлению ими.
...
Для одного из способов подключения CLI к учетной записи используются данные подписки Azure. (Другие способы см. в статье [Подключение к среде Azure с использованием интерфейса командной строки Azure (Azure CLI)](xplat-cli-connect.md)). Эту информацию вы можете получить через классический портал Azure в файле параметров публикации, как описано здесь. Вы можете импортировать файл параметров публикации как постоянный локальный параметр конфигурации, который CLI будет использовать для последующих операций. Вам необходимо импортировать параметры публикации только один раз.
...
Команда vm create, как и классический портал Azure, создает виртуальные машины в рабочей среде развертывания. Нет возможности создавать виртуальную машину в промежуточной среде развертывания облачной службы. Если для вашей подписки нет существующей учетной записи хранения Azure, эта команда создаст ее.
...
На следующей схеме показана архитектура типичного развертывания нескольких экземпляров классической виртуальной машины. Обратите внимание, что в этом примере на каждой виртуальной машине открыт порт 3389 (для доступа по RDP), а также имеется внутренний IP-адрес (например, 168.55.11.1), который используется средством балансировки нагрузки для маршрутизации трафика на виртуальную машину. Этот внутренний IP-адрес также может использоваться для обмена данными между виртуальными машинами.
