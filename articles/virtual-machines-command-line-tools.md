<properties
	pageTitle="Команды Azure CLI в режиме управления службами | Microsoft Azure"
...
Чтобы приступить к работе, [установите Azure CLI](xplat-cli-install.md) и [подключитесь к своей подписке Azure](xplat-cli-connect.md).

Чтобы просмотреть текущий синтаксис и параметры команд в командной строке, введите `azure help`. Чтобы получить справку по определенной команде, введите `azure help [command]`. Вы также найдете примеры для CLI в документации по созданию отдельных служб Azure и управлению ими.
...
## Установка режима ASM

В настоящее время режим ASM включается по умолчанию при установке CLI. При необходимости используйте следующую команду, чтобы установить режим ASM.

	azure config mode asm

>[AZURE.NOTE] Режим Azure Resource Manager и ASM для интерфейса командной строки являются взаимоисключающими. То есть ресурсами, созданными в одном из режимов, нельзя управлять из другого режима.

## Управление информацией об учетной записи и параметрами публикации
Для одного из способов подключения интерфейса командной строки к учетной записи используются данные подписки Azure. (Другие способы см. в статье [Подключение к среде Azure с использованием интерфейса командной строки Azure (Azure CLI)](xplat-cli-connect.md)). Эту информацию вы можете получить через классический портал Azure в файле параметров публикации, как описано здесь. Вы можете импортировать файл параметров публикации как постоянный локальный параметр конфигурации, который интерфейс командной строки будет использовать для последующих операций. Вам необходимо импортировать параметры публикации только один раз.
...
## Команды для управления классическими виртуальными машинами
...
**-c, --connect** создает виртуальную машину в уже созданном развертывании в службе размещения. Если с этим параметром не используется параметр -vmname, то имя новой виртуальной машины создается автоматически.<br /> **-n, --vm-name** задает имя виртуальной машины. Этот параметр по умолчанию берет имя службы размещения. Если параметр -vmname не указан, то имя для новой виртуальной машины будет создано в следующем формате: &lt;имя\_службы>&lt;идентификатор>, где &lt;идентификатор> — это число, равное увеличенному на 1 количеству имеющихся в службе виртуальных машин. Например, если использовать эту команду для добавления новой виртуальной машины в службу размещения MyService, в которой имеется одна виртуальная машина, то новая виртуальная машина получит имя MyService2.<br /> **-u, --blob-url** задает URL-адрес целевого хранилищах BLOB-объектов, в котором необходимо создать системный диск виртуальной машины. <br /> **-z, --vm-size** задает размер виртуальной машины. Допустимые значения: ExtraSmall, Small, Medium, Large, ExtraLarge, A5, A6, A7, A8, A9, A10, A11, Basic\_A0, Basic\_A1, Basic\_A2, Basic\_A3, Basic\_A4, Standard\_D1, Standard\_D2, Standard\_D3, Standard\_D4, Standard\_D11, Standard\_D12, Standard\_D13, Standard\_D14, Standard\_DS1, Standard\_DS2, Standard\_DS3, Standard\_DS4, Standard\_DS11, Standard\_DS12, Standard\_DS13, Standard\_DS14, Standard\_G1, Standard\_G2, Standard\_G3, Standard\_G4, Standard\_G55. По умолчанию используется значение Small. <br /> **-r** добавляет возможности подключения к виртуальной машине Windows с помощью протокола удаленного рабочего стола. <br /> **-e, --ssh** добавляет возможность подключения к виртуальной машине Windows с помощью протокола SSH. <br /> **-t, --ssh-cert** задает сертификат SSH. <br /> **-s** подписка. <br /> **-o, --community** указывает, что образ представляет собой образ сообщества. <br /> **-w** имя виртуальной сети. <br/> **-l, --location** задает расположение (например, северо-центральный регион США). <br /> **-a, --affinity-group** задает территориальную группу.<br /> **-w, --virtual-network-name** задает виртуальную сеть, в которую необходимо добавить виртуальную машину. Настраивать виртуальные сети и управлять ими можно на классическом портале Azure.<br /> **-b, --subnet-names** Задание имен подсетей для назначения виртуальной машине.
...
**vm endpoint create-multiple [параметры] &lt;имя\_виртуальной\_машины> &lt;порт\_балансировщика\_нагрузки>[:&lt;порт\_виртуальной\_машины>[:&lt;протокол>[:&lt;enable-direct-server-return>[:&lt;имя\_набора\_балансировщика\_нагрузки>[:&lt;протокол\_пробы>[:&lt;порт\_пробы>[:&lt;путь\_к\_пробе>[:&lt;имя\_внутреннего\_балансировщика\_нагрузки>]]]]]]]] {1-*}**
...
>[AZURE.NOTE]В настоящее время эта команда поддерживает только передачу фиксированных VHD-файлов. Чтобы передать динамический VHD-файл, используйте [служебные программы Azure VHD для GO](https://github.com/Microsoft/azure-vhd-utils-for-go).
...
Эта команда отправляет и регистрирует диск Azure. Необходимо указать параметр --blob-url, --location или --affinity-group. Если в этой команде используется параметр [путь\_к\_источнику], то передается указанный VHD-файл и создается образ. Затем можно присоединить этот образ к виртуальной машине, используя команду vm disk attach.
...
**-q** или --quiet**: отключает запрос подтверждения. Используйте этот параметр в автоматизированных скриптах.
...
**site job upload [параметры] &lt;имя\_задания> &lt;тип\_задания> <файл\_задания> [имя]**
...
> [AZURE.NOTE] Если для режима среды выполнения задано значение `Reserved`, то все ваши мобильные службы в одном и том же регионе будут работать в режиме "Премиум".
