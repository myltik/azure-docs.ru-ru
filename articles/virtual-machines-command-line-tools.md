...
>[AZURE.NOTE]В настоящее время эта команда поддерживает только передачу фиксированных VHD-файлов. Чтобы отправить динамический VHD-файл, используйте [служебные программы Azure VHD для GO](https://github.com/Microsoft/azure-vhd-utils-for-go).
...
+ **-u `<unit>`** или **--intervalUnit `<unit>`**: единица измерения для параметра _interval_. Может принимать следующие значения:
    + **minute** (по умолчанию)
    + **hour**
    + **day**
    + **month**
...
> [AZURE.NOTE] Новые задания создаются в отключенном состоянии, так как необходимо еще передать сценарий. Используйте команду **mobile script upload** для отправки сценария и команду **mobile job update** для включения задания.
