...
Если в кластере Docker Swarm запущено несколько контейнеров, с помощью команды `docker ps` можно увидеть, на каком узле работает каждый из контейнеров. В этом примере мы видим три контейнера, которые равномерно распределены между тремя агентами Swarm:
