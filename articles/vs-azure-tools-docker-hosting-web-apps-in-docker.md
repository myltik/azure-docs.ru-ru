...
Перед развертыванием приложения из Visual Studio в Azure добавьте конечную точку 80 на виртуальную машину узла Docker, чтобы впоследствии можно было просматривать приложение в браузере. Это можно сделать на классическом портале Azure или с помощью Windows PowerShell.

- **Использование классического портала Azure для настройки конечной точки узла Docker в Azure**

    1.  Перейдите на [классический портал Azure](https://manage.windowsazure.com/). 
    
    1.  Коснитесь пункта **Виртуальные машины**.
    
    1.  Выберите свою виртуальную машину узла Docker.
    
    1.  Перейдите на вкладку **Конечные точки**.
    
    1.  Коснитесь кнопки **Добавить** (внизу страницы).
...
При развертывании на удаленных узлах функция сопоставления тома, используемая для разработки редактирования и обновления, работать не будет. Таким образом, при выполнении сборки приложения вам придется использовать *конфигурацию выпуска*, чтобы избежать настройки сопоставления тома. Выполните следующие действия, чтобы запустить свое приложение.

1.  В панели инструментов Visual Studio выберите конфигурацию **выпуска**.

1.  Измените целевой объект для запуска на **Docker**.

1.  Коснитесь значка **Docker**, чтобы выполнить сборку и запустить приложение.
