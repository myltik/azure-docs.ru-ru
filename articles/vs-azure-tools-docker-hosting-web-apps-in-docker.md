...
В этих процедурах вы создадите новый проект веб-приложения ASP.NET 5, затем создадите узел контейнера и выполните сборку и запуск проекта веб-приложения в контейнере Docker. Для начала загрузите и установите [Visual Studio 2015 Tools for Docker](aka.ms/DockerToolsForVS).
...
1. Нажмите кнопку **Создать**, чтобы создать новую виртуальную машину Docker в Azure, затем нажмите кнопку **ОК**.
    ...
    В качестве альтернативы можно опубликовать приложение в пользовательский узел Docker. Дополнительные сведения см. в главе [Создание пользовательского узла Docker](#BKMK_CustomHost) далее в этом разделе.
...
1. Введите следующую информацию в диалоговом окне **Создание виртуальной машины в Microsoft Azure**. Когда все будет готово, нажмите кнопку **ОК**. После этого будет создана виртуальная машина под управлением Linux с настроенным расширением Docker.
    ...
    |Каталог сертификатов |Указывает каталог, в котором хранятся сертификаты Docker. Хотя вы можете создать новый каталог или выбрать существующий, рекомендуется использовать каталог для сертификатов по умолчанию (C:\\Users\\ [*имя пользователя*] \\.docker). В противном случае параметры проверки подлинности не смогут быть извлечены автоматически при повторном использовании одного и того же узла в другом проекте или системе.|

1. Нажмите кнопку с многоточием (...) рядом с разделом **Каталог сертификатов** и затем создайте новый каталог для сертификатов Docker или перейдите к существующему каталогу сертификатов.
...
1. После завершения подготовки узла Docker в Azure вы можете проверить состояние своей учетной записи на портале Azure. В категории **Виртуальные машины** на портале Azure должна появиться новая виртуальная машина.
...
1. На вкладке **Подключения** в диалоговом окне **Публикация веб-проекта** установите флажок **Проверить подключение**, чтобы проверить готовность узла Docker. Если подключение хорошее, нажмите кнопку **Опубликовать**, чтобы опубликовать веб-приложение.

    При первой публикации приложения на узле Docker может потребоваться некоторое время на загрузку всех базовых образов, которые указаны в файле Docker (в формате **FROM** *название образа*).
...
1. В диалоговом окне **Публикация веб-проекта** укажите значения следующих параметров в разделе **Пользовательский узел Docker**: URL-адрес сервера, название образа, расположение файла Docker и номера портов узла и контейнера.
...
1. Если подключение работает правильно, вы можете нажать кнопку **Далее**, чтобы просмотреть список компонентов, которые будут опубликованы, или нажать кнопку **Опубликовать**, чтобы немедленно опубликовать проект.
...
1. Назначьте узел Docker и проверьте переменные среды. Для этого введите следующие команды в командной строке. (Замените *NameofAzureVM* на имя своего узла Docker.)
    ...
    Выполнение этих команд позволяет избежать добавления `–H (Host) tcp://<NameofAzureVM>.cloudapp.net:2376` и `--TLSVERIFY` ко всем выполняемым командам.
...
1. Теперь вы можете выполнить приведенные ниже команды, чтобы проверить, что узел Docker развернут и работает.
    ...
    Для получения полного списка команд Docker просто введите команду `docker` в командной строке. Дополнительные сведения см. в разделе [Командная строка Docker](https://docs.docker.com/reference/commandline/cli/).
...
Проблемы при использовании Docker в Visual Studio описаны в статье [Устранение ошибок Docker](vs-docker-troubleshooting-docker-errors.md).
