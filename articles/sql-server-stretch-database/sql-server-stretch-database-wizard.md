...
|**Миграция**|Можно перенести всю таблицу (**Вся таблица**) или указать фильтр в колонке таблицы. Если вы хотите использовать другую функцию фильтра для выбора строк при миграции, после выхода из мастера выполните оператор ALTER TABLE и укажите нужную функцию. Дополнительные сведения о функции фильтра см. в статье о [выборе строк для переноса с помощью функции фильтра](sql-server-stretch-database-predicate-function.md). Дополнительные сведения об использовании функции см. в статье [Включение базы данных Stretch для таблицы](sql-server-stretch-database-enable-table.md) и в описании [ALTER TABLE (Transact-SQL)](https://msdn.microsoft.com/library/ms190273.aspx).|
...
5. Щелкните **Проверка**, чтобы протестировать функцию. Тест будет считаться пройденным **успешно**, если заданная функция вернет из таблицы результаты, то есть существуют строки, которые соответствуют указанному условию.
...
-   Закройте мастер и выполните оператор ALTER TABLE, чтобы применить режим Stretch для таблицы и указать функцию фильтра. Дополнительные сведения см. в статье [Enable Stretch Database for a table](sql-server-stretch-database-enable-table.md) (Включение базы данных Stretch для таблицы).
