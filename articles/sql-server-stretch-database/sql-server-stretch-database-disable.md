...
2.  Щелкните по этой таблице правой кнопкой мыши и выберите **Растяжение**, а затем выберите один из следующих вариантов.
    ...
    -   **Отключение | Сохранение данные в Azure**. Отключите растяжение базы данных для таблицы. Оставьте удаленные данные для таблицы в Azure.
        ...
        Если удаленные данные остаются в Azure, то при отключении растяжения они не удаляются. Если вы хотите удалить эти данные, необходимо удалить соответствующую таблицу с помощью портала управления Azure.
...
2.  Щелкните по этой базе данных правой кнопкой мыши и выберите **Задачи**, затем **Растяжение** и **Отключить**.
...
## Удаление базы данных, для которой включено растяжение
При удалении базы данных, для которой было включено растяжение, удаляется локальная база данных, но удаленные данные сохраняются. Если вы хотите удалить эти данные, необходимо удалить соответствующую базу данных с помощью портала управления Azure.

## См. также
[Параметры инструкции ALTER DATABASE SET (Transact-SQL)](https://msdn.microsoft.com/library/bb522682.aspx) [Приостановка и возобновление растяжения базы данных](sql-server-stretch-database-pause.md)
