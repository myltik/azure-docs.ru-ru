...
### Восстановление базы данных Azure в другом регионе Azure  
Служба базы данных SQL Server Stretch в Azure асинхронно копирует моментальные снимки в другой географический регион Azure, чтобы обеспечить дополнительные возможности восстановления в случае регионального сбоя. Если нет доступа к базе данных из-за сбоя в регионе Azure, вы можете восстановить базу данных в один из географически избыточных моментальных снимков.

>   [AZURE.NOTE] Для восстановления базы данных Azure в другом регионе Azure требуется изменить строку подключения в клиентских приложениях после восстановления, при этом возможна безвозвратная потеря данных. Применяйте этот тип восстановления только в случае, если ожидается длительный простой из-за сбоя.

Чтобы восстановить работающую базу данных Azure до более ранней точки во времени в другом регионе Azure с помощью портала Azure, выполните следующие действия.
...
После восстановления базы данных Azure в другом регионе необходимо выполнить хранимые процедуры **sys.sp\_rda\_deauthorize\_db** и **sys.sp\_rda\_reauthorize\_db**, чтобы повторно установить подключение между базой данных SQL Server с поддержкой переноса и удаленной базой данных Azure. Дополнительные сведения см. в разделе [Восстановление подключения между базой данных SQL Server и удаленной базы данных Azure](#Restore-the-connection-between-the-SQL-Server-database-and-the-remote-Azure-database).
