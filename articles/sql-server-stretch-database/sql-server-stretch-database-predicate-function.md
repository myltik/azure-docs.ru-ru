<properties
	...
	description="Узнайте, как выбрать строки для переноса с помощью предиката фильтра."
...
При запуске мастера включения переноса для базы данных можно перенести всю таблицу или указать простой предикат фильтра. Если вы хотите использовать другой тип предиката фильтра, чтобы выбрать строки для переноса, выполните одну из следующих процедур.
...
Синтаксис инструкции ALTER TABLE для добавления предиката описывается далее в этом разделе.

## Основные требования к предикату фильтра
Встроенная функция табличных значений, необходимая для предиката фильтра растяжения базы данных, выглядит следующим образом.
...
>   [AZURE.NOTE] Чтобы повысить производительность функции фильтрации, создайте индекс столбцов, используемых этой функцией.

### Передача имен столбцов в предикат фильтра
При назначении таблице функции фильтрации укажите имена столбцов, передаваемых в функцию фильтрации, состоящие из одной части. Если при передаче указать имена столбцов, состоящие из трех частей, то это приведет к сбою последующих запросов к таблице с поддержкой переноса.
...
## <a name="addafterwiz"></a>Добавление предиката фильтра после запуска мастера  

Если вы хотите использовать предикат, который нельзя создать в мастере **включения переноса для базы данных**, то после выхода из мастера можете выполнить инструкцию ALTER TABLE, чтобы указать предикат. Тем не менее перед применением предиката необходимо остановить текущий перенос данных и вернуть перенесенные данные. (Дополнительные сведения о том, почему это необходимо см. в разделе [Замена существующего предиката фильтра](#replacePredicate).)

1. Измените направление переноса и верните уже перенесенные данные. После запуска эту операцию нельзя отменить. Кроме того, за исходящую передачу данных (исходящий трафик) в Azure взимается плата. Дополнительные сведения см. в разделе [Стоимость услуг Azure](https://azure.microsoft.com/pricing/details/data-transfers/).  
...
2. Дождитесь завершения переноса. Вы можете проверить состояние **монитора базы данных Stretch** в SQL Server Management Studio или запросить представление **sys.dm\_db\_rda\_migration\_status**. Дополнительные сведения см. в разделе [Мониторинг и устранение неполадок переноса данных (база данных Stretch)](sql-server-stretch-database-monitor.md) или [sys.dm\_db\_rda\_migration\_status](https://msdn.microsoft.com/library/dn935017.aspx).

3. Создайте предикат фильтра, который требуется применить к таблице.

4. Добавьте предикат в таблицу и перезапустите перенос данных в Azure.
...
Начните с предиката фильтра, аналогичного приведенному в примере, переносящего строки, в которых столбец **systemEndTime** содержит значение, предшествующее 1 января 2016 года.
...
Примените предикат фильтра к таблице.
...
2.  Замените предыдущий предикат фильтра новым, вызвав ALTER TABLE, как показано в примере ниже.
...
## Дополнительные примеры допустимых предикатов фильтров
...
## Примеры недопустимых предикатов фильтров
...
## <a name="replacePredicate"></a>Замена существующего предиката фильтра
...
```
После удаления предиката фильтра все строки в таблице пригодны для миграции. В результате, если вы сначала не вернете все удаленные данные для таблицы из Azure, то не сможете позднее указать предикат фильтра для той же таблицы. Это ограничение позволяет избежать ситуации, в которой при предоставлении нового предиката фильтра неподходящие для миграции строки уже были перенесены в Azure.

## Проверка применения предиката фильтра к таблице
Чтобы проверить предикат фильтра, который применен к таблице, откройте представление каталога **sys.remote\_data\_archive\_tables** и проверьте значение столбца **filter\_predicate**. Если значение равно null, то может архивироваться вся таблица. Дополнительные сведения см. в разделе [sys.remote\_data\_archive\_tables (Transact-SQL)](https://msdn.microsoft.com/library/dn935003.aspx).

## Заметки о безопасности для предикатов фильтра  
Скомпрометированная учетная запись с привилегиями db\_owner может выполнять следующие действия.
