...
В CTP-версии от 3.1 до RC2 включительно параметр для указания предиката фильтра отсутствует в мастере включения базы данных для растяжения. В этом случае необходимо использовать инструкцию ALTER TABLE для настройки растяжения базы данных. Дополнительные сведения см. в статьях [Включение растяжения базы данных для таблицы](sql-server-stretch-database-enable-table.md) и [Инструкция ALTER TABLE (Transact-SQL)](https://msdn.microsoft.com/library/ms190273.aspx).
...
-   Сравнение параметра функции с константным выражением. Пример: `@column1 < 1000`.
    ...
    Ниже приведен пример, который проверяет, меньше ли значение столбца *date*, чем 1/1/2016.
...
Ниже приведен пример, который переносит строки, где столбец **date** содержит значение, более раннее, чем 1 января 2016 г.
...
Ниже приведен пример, который переносит строки, где столбец **status** содержит одно из указанных значений.
...
Начните с предиката фильтра, аналогичного приведенному в примере, который переносит строки, где столбец **systemEndTime** содержит значение, более раннее, чем 1 января 2016 г.
...
1.  Создайте функцию, которая задает новое скользящее окно. В примере ниже вместо 1 января 2016 г. выбираются даты, более ранние, чем 2 января 2106 г.
