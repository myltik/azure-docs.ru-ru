...
## <a name="why-dont-i-see-optimizer-recommendations"></a>Почему я не вижу оптимизатор рекомендации

Рекомендация информация доступна только для учетных записей, которые были активированы. Не увидят все рекомендации сведения из **оптимизатор** отчетов категории для учетных записей, которые являются *через который Неактивированные*, в том числе:

- Диспетчер оптимизации
- Оптимизация размера
- Неэффективное

Если не удается просмотреть все данные оптимизатора рекомендация, скорее всего, вы имеете учетных записей, через который Неактивированные. Чтобы активировать учетную запись, необходимо зарегистрировать его в учетные данные Azure.
...
2.  На вкладке учетные записи Microsoft Azure найдите учетными записями, имеющими **через который Неактивированные** подписки.
3.  Учетной записи, через который Неактивированные, правее **изменить** символ, который напоминает карандаша.
4.  Клиент Идентификаторы и частота определяется автоматически. Нажмите кнопку **Далее**.
5.  Будет перенаправлен на портале Azure. Войдите портал и авторизуйте сборщика Cloudyn доступ к данным Azure.
6.  Далее будет перенаправлен на страницу управления Cloudyn учетные записи и обновления подписки с **active** состояние учетной записи. Он показывает зеленый символ.
7.  Вы не видите символ Зеленый флажок для одного или нескольких подписок, означает, что у вас разрешения на создание приложения для чтения (CloudynCollector) для подписки. Пользователь с более высокие разрешения для подписки необходимо повторить шаги 3 и 4.  

После выполнения предыдущих шагов можно просматривать рекомендации оптимизатор внутри одной или двух дней. Тем не менее может потребоваться до пяти дней до полной оптимизации данные недоступны.
...
Если у вас несколько подписок с использованием различных валют, валюта родительской и соответствующих дочерних сущностей отображаются как символ **$**. Рекомендуем избегать использования различных валют в одной иерархии сущностей. Другими словами, для всех подписок в структуре сущностей следует использовать одинаковую валюту.

В Cloudyn автоматически обнаруживается и представляется в отчетах валюта подписки Enterprise Agreement.  Но в Cloudyn символ **$** отображается только для учетных записей Azure для поставщика служб шифрования и WebDirect.
