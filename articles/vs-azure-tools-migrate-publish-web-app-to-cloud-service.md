...
>[AZURE.NOTE]Эта статья посвящена развертыванию в облачных службах, а не на веб-сайтах. Сведения о развертывании на веб-сайтах см. в статье "Развертывание веб-сайта Azure".
...
1. Чтобы включить веб-приложение для развертывания в Azure, откройте контекстное меню веб-проекта в решении и выберите пункт "Добавить проект развертывания Azure". После этого будут выполнены следующие действия.
...
1. Для создания экземпляра базы данных SQL Azure в консоли управления выполните действия, описанные в статье [Создание сервера базы данных SQL](http://go.microsoft.com/fwlink/?LinkId=225109).
...
### Использование экземпляра базы данных SQL с помощью портала управления
...
1. Для подготовки служб приложения к публикации в Azure требуется учетная запись Майкрософт и подписка на Azure. Настройте службы согласно инструкциям в разделе [Настройка служб, необходимых для публикации облачной службы из Visual Studio](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md).

1. Чтобы опубликовать веб-приложение в Azure, откройте контекстное меню веб-проекта и выберите пункт **Опубликовать в Azure**.

    Появится диалоговое окно **Публикация приложения Azure**, а Visual Studio запустит процесс развертывания. Дополнительные сведения о публикаций приложение см. в разделе **Публикация приложения Azure из Visual Studio** статьи [Публикация облачной службы с помощью средств Azure](vs-azure-tools-publishing-a-cloud-service-using-the-azure-tools.md).
...
1. (Необязательно) После запуска экземпляров ролей Visual Studio автоматически отображает среду развертывания в узле **Вычисление Azure** в **обозревателе серверов**. Здесь можно просматривать состояние отдельных экземпляров ролей. Дополнительные сведения об узле вычисления Azure в обозревателе серверов см. в статье [Просмотр состояния облачной службы с помощью обозревателя серверов](vs-azure-tools-viewing-the-state-of-a-cloud-service-using-server-explorer.md).

    На следующей иллюстрации показаны экземпляры роли в состоянии "Инициализация".
...
1. Чтобы создать базу данных SQL Azure, которую можно будет использовать в веб-приложении динамических сущностей ASP.NET, выполните процедуру **Использование базы данных SQL Azure для приложения**, описанную выше в этой статье.
...
## Дальнейшие действия
Дополнительные сведения о публикации см. в статье [Настройка служб, необходимых для публикации облачной службы из Visual Studio](vs-azure-tools-setting-up-services-required-to-publish-a-cloud-service-from-visual-studio.md). Также см. статью [Настройка именованных учетных данных для проверки подлинности](vs-azure-tools-setting-up-named-authentication-credentials.md).
