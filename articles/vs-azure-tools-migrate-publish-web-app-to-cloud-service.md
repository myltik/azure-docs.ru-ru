---
title: Инструкции. Миграция и публикация веб-приложения в облачную службу Azure из среды Visual Studio | Microsoft Docs
...
1. Чтобы включить веб-приложение для развертывания в Azure, откройте контекстное меню веб-проекта в решении и выберите пункт «Добавить проект развертывания Azure».
   ...
   * Свойству **Копировать локально** будет присвоено значение true для всех сборок, необходимых для MVC 2, MVC 3, MVC 4 и бизнес-приложений Silverlight. В результате эти сборки будут добавлены в пакет служб, используемый для развертывания.
   ...
   > 
   > [!NOTE]
   > Если веб-роль для конкретного веб-проекта уже существует в проекте Azure соответствующего решения, команды **Преобразовать**, **Преобразовать в проект облачной службы Azure** в контекстном меню этого веб-проекта отображаться не будут.
   > 
   > 
...
5. (Необязательно) Чтобы отменить процесс развертывания, откройте контекстное меню строки в журнале действий и выберите пункт **Отменить и удалить**. Это позволит остановить процесс развертывания и удалить среду развертывания из Azure.
   ...
   > 
   > 
6. (Необязательно) После запуска экземпляров ролей Visual Studio автоматически отображает среду развертывания в узле **Вычисление Azure** в **обозревателе облака** или в **обозревателе серверов**. Здесь можно просматривать состояние отдельных экземпляров ролей.
...
7. Для доступа к приложению после развертывания выберите стрелку рядом с развертыванием, когда в **журнале действий Azure** появится статус **Завершено**. Это действие позволит отобразить URL-адрес веб-приложения в Azure. Сведения о запуске веб-приложения определенного типа из Azure см. в следующей таблице.
...
3. Строка подключения для приложений этого типа имеет в файле web.config следующий формат:
    ...
    ```
4. Чтобы сохранить файл web.config с внесенными в строку подключения изменениями, последовательно выберите пункты **Файл**, **Сохранить web.config** в строке меню.
