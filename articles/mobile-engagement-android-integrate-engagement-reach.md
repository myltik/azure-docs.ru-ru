<properties 
	pageTitle="Интеграция SDK Azure Mobile Engagement в Android" 
	description="Последние обновления и процедуры для Android SDK для Azure Mobile Engagement"
...
Для Reach SDK требуется **библиотека поддержки Android (v4)**.

Самый быстрый способ добавить библиотеку в проект в **Eclipse** - `Right click on your project -> Android Tools -> Add Support Library...`.
...
-   Укажите значок для уведомлений (и в приложении, и системных уведомлений), скопировав и отредактировав следующий раздел (между тегами `<application>` и `</application>`):

			<meta-data android:name="engagement:reach:notification:icon" android:value="<name_of_icon_WITHOUT_file_extension_and_WITHOUT_'@drawable/'>" />

> [AZURE.IMPORTANT] Этот раздел **обязателен**, если вы планируете использовать системные уведомления при создании кампаний Reach. В Android системные уведомления без значков не отображаются. Поэтому, если не добавить этот раздел, пользователи не смогут получать уведомления.
...
##Обработка данных push-уведомлений
...
Параметр категории не является обязательным при создании кампании с push-обновлениями данных. Этот параметр позволяет отфильтровывать push-обновления . Это полезно при наличии нескольких широковещательных получателей, обрабатывающих разные типы push-обновлений, или если нужно распространять разные виды данных Base64 и определять их тип перед разбором.

### Параметр возврата обратных вызовов
...
По умолчанию уведомление приложения - это представление, которое динамически добавляется в пользовательский интерфейс текущего действия с помощью метода Android  `addContentView()`. Это называется наложением уведомлений. Наложения уведомлений удобный для быстрой интеграции, поскольку для них не требуется изменять разметку приложений.

Чтобы изменить внешний вид наложения уведомления, можно просто изменить файл  `engagement_notification_area.xml` нужным образом.

> [AZURE.NOTE] Файл  `engagement_notification_overlay.xml` используется для создания наложения уведомления, он включает файл  `engagement_notification_area.xml`. Его также можно настроить в соответствии с потребностями (например, для расположения область уведомлений в области наложения).

##### Включите разметку уведомления в состав разметки действия
...
Благодаря инструкции Android **include** можно определить, как следует включить разметку в существующую разметку. Ниже приведен пример измененной разметки  `ListActivity`, содержащей только  `ListView`.

**До интеграции Engagement:**
...
В этом примере мы добавили родительский контейнер, поскольку исходная разметка использовала представление списка в качестве элемента верхнего уровня. Мы также добавили `android:layout_weight="1"` , чтобы иметь возможность просматривать список, настроеный с помощью `android:layout_height="fill_parent"`.
...
При изменении предоставленных разметок изменяется внешний вид всех уведомлений. С помощью категорий можно определить различный внешний вид (и поведение) для уведомлений. Можно указать категорию при создании кампании Reach. Помните, что с помощью категорий также можно настраивать объявления и опросы, как описано ниже в этом документе.

Чтобы зарегистрировать обработчик категорий для уведомлений, нужно добавить вызов при инициализации приложения.

> [AZURE.IMPORTANT] Перед продолжением прочтите предупреждение об атрибуте android:process \<android-sdk-engagement-process\> в разделе "Интеграция Engagement в Android".
...
Объект  `MyNotifier` является реализацией обработчика категорий обновлений. Это либо реализация интерфейса  `EngagementNotifier`, либо подкласс реализации по умолчанию:  `EngagementDefaultNotifier`.
...
Можно изменить большую часть процесса создания уведомления путем переопределения методов в  `EngagementDefaultNotifier`. Для более широких возможностей настройки см. техническую документацию и исходный код.
...
Если нужно это изменить, придется заново определить метод  `EngagementDefaultNotifier.prepareInAppArea`. Рекомендуем ознакомиться с технической документацией и с исходным кодом в  `EngagementNotifier` и  `EngagementDefaultNotifier`, если требуется расширенная настройка.
...
Если нужно создать объект  `Notification` с нуля, можно вернуть  `false` в метод и вызвать  `notify` для  `NotificationManager`. В этом случае важно сохранить  `contentIntent`,  `deleteIntent` и идентификатор уведомления, используемый в  `EngagementReachReceiver`.
...
##### Объявления только для уведомлений
...
Также можно реализовать  `EngagementNotifier.executeNotifAnnouncementAction` с нуля.

##### Жизненный цикл уведомления

При использовании категории по умолчанию некоторые методы жизненного цикла вызываются для объекта  `EngagementReachInteractiveContent`, чтобы передать статистику и обновить состояние кампании:
...
-   Если уведомление закрывается, вызывается метод  `exitNotification`, передается статистика, можно переходить к следующим кампаниям.
-   При щелчке уведомления вызывается  `actionNotification`, передается статистика и запускается связанное намерение.

Если в вашей реализации  `EngagementNotifier` поведение по умолчанию не используется, потребуется вызвать эти методы жизненного цикла. В следующих примерах показаны случаи, когда поведение по умолчанию не используется:

-   Не выполняется расширение  `EngagementDefaultNotifier`, т. е. обработка категорий реализована с нуля.
-   Для системных уведомлений вы заменили  `onNotificationPrepared` и изменили  `contentIntent` или  `deleteIntent` в объекте  `Notification`.
-   В уведомлениях приложения вы заменили  `prepareInAppArea`. Не забудьте сопоставить  `actionNotification` с одним из элементов управления пользовательского интерфейса.

> [AZURE.NOTE] Если  `handleNotification` выдает исключение, то содержимое удаляется и вызывается  `dropContent`. Это отражается в статистике, после этого можно обрабатывать последующие кампании.
...
Как и уведомления, можно использовать категорию компании для дополнительных разметок для объявлений и опросов.

Например, чтобы создать категорию для текстового объявления, можно расширить  `EngagementTextAnnouncementActivity` и добавить ссылку на этот элемент в файле  `AndroidManifest.xml`:
...
Чтобы заменить категорию текстовых объявлений по умолчанию, что замените  `android:name="com.microsoft.azure.engagement.reach.activity.EngagementTextAnnouncementActivity"` в вашей реализации.

Аналогичным образом можно настраивать веб-объявления и опросы.

Для веб-объявлений можно расширить  `EngagementWebAnnouncementActivity` и объявить ваше действие в  `AndroidManifest.xml`, как в следующем примере:
...
Для опросов можно расширить  `EngagementPollActivity` и объявлять в  `AndroidManifest.xml`, как в следующем примере:
...
Можно реализовать категории для действий объявлений (и опросов), не расширяя классы  `Engagement*Activity`, входящие в состав Reach SDK. Это полезно в случае, если вам нужно определить разметку, не использующую такие же представления, как в стандартных разметках.

Как и при расширенной настройке уведомлений, рекомендуется ознакомиться с исходным кодом стандартных реализаций.
...
Если не вызвать  `actionContent` или  `exitContent`, то статистика не будет отправлена (т. е. кампания без аналитики), а также, что важнее, не будет уведомлений о последующих кампаниях до перезапуска процесса приложения.

При изменении ориентации и других изменениях конфигурации становится непросто определить, переключается ли действие в фоновый режим. При стандартной реализации фиксируется выход из содержимого, если пользователь выходит из действия (путем нажатия  `HOME` или  `BACK`), но не при изменении ориентации.
