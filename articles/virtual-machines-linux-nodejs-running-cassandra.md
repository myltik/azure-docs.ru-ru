...
**Высокая доступность.** Узлы Cassandra, изображенные на рисунке 1, развертываются в двух группах доступности для распределения узлов между несколькими доменами сбоя с целью обеспечения высокой доступности. Виртуальные машины с заметками о каждой группе доступности сопоставляются с 2 доменами сбоев. В службе Microsoft Azure для управления незапланированными простоями (например, вследствие ошибок аппаратного или программного обеспечения) используется домен сбоя, а домен обновления (например, исправления либо обновления узла или гостевой ОС, обновления приложений) применяется для управления плановыми простоями. Дополнительные сведения о роли доменов сбоя и обновления для обеспечения высокого уровня доступности см. в статье [Аварийное восстановление и высокий уровень доступности для приложений Azure](http://msdn.microsoft.com/ru-ru/library/dn251004.aspx).
...
<tr><td>Стратегия репликации </td><td>	NetworkTopologyStrategy[Дополнительные сведения см. в статье [Репликация данных](http://www.datastax.com/documentation/cassandra/2.0/cassandra/architecture/architectureDataDistributeReplication_c.html) в документации к Cassandra.] </td><td>	Понимает топологию развертывания и помещает реплики на узлы таким образом, чтобы все реплики не попадут в одну стойку.  </td></tr>
<tr><td>Информатор</td><td> GossipingPropertyFileSnitch. [Дополнительные сведения см. в статье [Информаторы](http://www.datastax.com/documentation/cassandra/2.0/cassandra/architecture/architectureSnitchesAbout_c.html) в документации к Cassandra.] </td><td>NetworkTopologyStrategy использует концепцию информатора для получения данных о топологии. GossipingPropertyFileSnitch обеспечивает лучший контроль при сопоставлении каждого узла с центром обработки данных и стойкой. Затем кластер использует полученные данные для распространения этих сведений. Процесс выполняется гораздо проще в параметре динамического IP-адреса относительно PropertyFileSnitch </td></tr> 
</table> 
...
<tr><td>Ubuntu	</td><td>[Портал Mcrosoft Azure](http://azure.microsoft.com) </td><td>14.04 LTS</td></tr>
</table>
...
<tr><td>УРОВЕНЬ	                 </td><td> СТАНДАРТ	                       </td><td> Оставьте значение по умолчанию.              </td><tr>
...
#####1. Отзыв#####
Выполните команду "sudo waagent -deprovision + user", чтобы удалить сведения о конкретных экземплярах виртуальной машины. Дополнительные сведения о процессе записи образа см. в статье [Как записать образ виртуальной машины Linux для использования в качестве шаблона](http://azure.microsoft.com/ru-ru/documentation/articles/virtual-machines-linux-capture-image/). 
...
###Шаг 6. Установка соединения между виртуальными сетями###
...
###Шаг 8. Настройка Cassandra на виртуальных машинах###
Войдите на виртуальную машину и выполните следующие действия: 
