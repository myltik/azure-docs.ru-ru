...
Конечным результатом моделирования является невозможность разработки обоснованной политики только путем устранения известных причин оттока. Фактически, **необходимо** придерживаться непрерывной стратегии моделирования, включая классические модели количественной оценки категориальных переменных (например, дерево принятия решения).
...
-	Получение показателей для четырех основных категорий:
	...
	-	**Активность**. Получение всей возможной информации об использовании, касающейся объекта, например количества входов.
...
Использование четырех приведенных выше категорий создает впечатление, что для определения клиентов, подверженных риску оттока, будет достаточно простого *deterministic* подхода на основе индексов, сформированных на базе разумных факторов в каждой категории. К сожалению, несмотря на кажущуюся правдоподобность, это ложное понимание. Дело в том, что отток является временным эффектом, а факторам, способствующим оттоку, обычно присущи переходные состояния. Причины, по которым происходит отток клиентов сегодня, могут отличаться от завтрашних и, конечно, будут совершенно иными через шесть месяцев. Поэтому следует признать необходимость модели *probabilistic*.  
...
 
##Ссылки
[1.] Прогнозная аналитика: за пределами предсказаний, В. Макнайт (W. McKnight), управление информацией, июль-август 2011 г., стр.18-20  

[2.] [Статья о точности и прецизионности](http://en.wikipedia.org/wiki/Accuracy_and_precision) на Википедии 
