...
Убедитесь, что инструменты Thales находятся в пути (**%nfast_home%\bin** и **%nfast_home%\python\bin**). Например, введите следующую команду:
...
Эта команда создает файл ключа с токеном в папке %NFAST_KMDATA%\local с именем, начинающимся с **key_simple_**, за которым следует значение параметра ident, указанное в команде. Например: **key_simple_contosokey**. Этот файл содержит зашифрованный ключ.
...
- Замените *SubscriptionID* на идентификатор подписки Azure, содержащей ваше хранилище ключей. Это значение было получено ранее в подразделе **Шаг 1.2. Получение идентификатора подписки Azure** раздела [Подготовка рабочей станции, подключенной к Интернету](#step-1-prepare-your-internet-connected-workstation).
