...
|---|---|
|Подписка на Azure|Для создания хранилища ключей Azure требуется подписка Azure: [зарегистрируйтесь для получения бесплатной пробной версии](http://azure.microsoft.com/pricing/free-trial/).|
|Хранилище ключей Azure, поддерживающее аппаратные модули безопасности|Дополнительные сведения об уровнях обслуживания и возможностях хранилища ключей Azure см. на веб-сайте [Цены на хранилище ключей Azure](http://azure.microsoft.com/pricing/details/key-vault/).|
...
Перейдите в центр загрузки Майкрософт и [скачайте набор инструментов BYOK для хранилища ключей Azure](http://www.microsoft.com/download/details.aspx?id=45345) для своего региона.

|Регион|Имя пакета|Хэш пакета SHA-256|
|---|---|---|
|Северная Америка|KeyVault-BYOK-Tools-UnitedStates.zip|D9FDA9F5A34E1388CD6C9138E5B75B7051FB7D6B11F087AFE0553DC85CCF0E36|
...
|Австралия|KeyVault-BYOK-Tools-Australia.zip|8EBC69E58E809A67C036B50BB4F1130411AD87A7464E0D61A9E993C797915967|
...
1.	Запустите сценарий verifykeypackage.py, указав одно из приведенных ниже значений в зависимости от региона:
...
2.	При успешном выполнении проверки должно отобразиться сообщение **Результат: УСПЕШНО**.
...
Чтобы ограничить разрешения в ключе, выполните через командную строку одну из следующих команд в зависимости от вашего региона.
...
После завершения работы команды отобразится сообщение **Результат: УСПЕШНО**, а копия ключа с ограниченными разрешениями появится в файле с именем key\_xferacId\_<contosokey>.
...
Выполните одну из следующих команд в зависимости от вашего региона.
...
- Замените *SubscriptionID* идентификатором подписки Azure, содержащей ваше хранилище ключей. Это значение было получено ранее на этапе **Шаг 1.2. Получение идентификатора подписки Azure** процедуры [Подготовка рабочей станции, подключенной к Интернету](#step-1-prepare-your-internet-connected-workstation).
...
Теперь ключ, защищенный с помощью аппаратного модуля безопасности, можно использовать в хранилище ключей. Дополнительные сведения см. в разделе **Использование аппаратного модуля безопасности** руководства [Начало работы с хранилищем ключей Azure](key-vault-get-started.md).
