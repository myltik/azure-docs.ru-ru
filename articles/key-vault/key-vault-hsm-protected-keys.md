...
Убедитесь, что инструменты Thales находятся в пути (**%nfast\_home%\\bin** и **%nfast\_home%\\python\\bin**). Например, введите следующую команду:
...
Этот сценарий проверяет цепочку до корневого ключа Thales подписавшего пользователя. Хэш этого корневого ключа встроен в сценарий и должен иметь значение **59178a47 de508c3f 291277ee 184f46c4 f1d9c639**. Вы также можете проверить это значение отдельно на [веб-сайте Thales](http://www.thalesesec.com/).

Теперь можно создать новый ключ.

###Шаг 3.3. Создание нового ключа

Создайте ключ с помощью программы Thales **generatekey**.
...
Эта команда создает файл ключа с токеном в папке %NFAST\_KMDATA%\\local с именем, начинающимся с **key\_simple\_**, за которым следует значение параметра ident, указанное в команде. Например: **key\_simple\_contosokey**. Этот файл содержит зашифрованный ключ.
...
При выполнении этой команды замените *contosokey* тем же значением, которое вы указали при выполнении этапа **Шаг 3.3. Создание нового ключа** процедуры [Создание ключа](#step-3-generate-your-key).
...
- Замените *contosokey* идентификатором, который использовался для создания ключа в подразделе **Шаг 3.3. Создание системы безопасности** раздела [Создание ключа](#step-3-generate-your-key).
...
- Замените *ContosoFirstHSMKey* меткой, которая будет использоваться для имени выходного файла.

После выполнения команды появится текст **Результат: УСПЕШНО**, а в текущей папке появится новый файл с именем TransferPackage-*ContosoFirstHSMkey*.byok.
