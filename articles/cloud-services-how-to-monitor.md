<properties urlDisplayName="How to monitor" pageTitle="Мониторинг облачной службы - Azure" metaKeywords="Azure monitoring cloud services, Azure Management Portal cloud services" description="Узнайте, как осуществлять мониторинг облачных служб с помощью портала управления Azure." metaCanonical="" services="cloud-services" documentationCenter="" title="How to Monitor Cloud Services" authors="raynew" solutions="" manager="johndaw" editor="" />
...
> [WACOM.NOTE]
> Для подробного мониторинга вы можете добавить дополнительные счетчики производительности при запуске экземпляра роли с помощью файла конфигурации диагностики. Чтобы отслеживать эти метрики на портале управления, перед настройкой подробного мониторинга необходимо добавить соответствующие счетчики производительности. Дополнительную информацию см. в разделе <a href="http://azure.microsoft.com/ru-ru/documentation/articles/cloud-services-dotnet-diagnostics/">Включение диагностики в облачных службах и виртуальных машинах Azure</a>.
...
- Создайте учетную запись хранения, в которой будут размещаться данные мониторинга. Для каждой роли можно использовать собственную учетную запись хранения. Дополнительную информацию см. в справке для компонента **Учетные записи хранения** или в разделе [Создание учетной записи хранения](/ru-ru/manage/services/storage/how-to-create-a-storage-account/).


- Включите систему диагностики Azure для ролей облачной службы. <br /><br />Дополнительную информацию см. в разделе [Включение системы диагностики для облачных служб и виртуальных машин Azure](/ru-ru/documentation/articles/cloud-services-dotnet-diagnostics/).
...
1. Скопируйте ключ доступа к хранилищу для учетной записи, в которой будут размещаться данные подробного мониторинга. На [портале управления Azure](https://manage.windowsazure.com/) вы можете нажать **Управление ключами** на странице **Учетные записи хранения**. Дополнительную информацию см. в разделе [Управление облачными службами](/ru-ru/documentation/articles/cloud-services-how-to-manage/) или в справке для страницы**Учетные записи хранения**. 

2. Откройте **Облачные службы**. Щелкните имя настраиваемой облачной службы, чтобы открыть панель мониторинга.
...
4. Щелкните **Настройка**.
	...
	Вы можете изменить параметры **мониторинга** в верхней части страницы **Настройка** (см. ниже). Если для облачной службы не включена система диагностики Azure, параметр **Уровень** будет недоступен. Изменить политику хранения данных нельзя. Данные подробного мониторинга для облачной службы хранятся в течение 10 дней.
...
1. На [портале управления](https://manage.windowsazure.com/) откройте страницу **Настройка** для развертывания облачной службы.
...
<h2><a id="receivealerts"></a>Практическое руководство: Получение оповещений для метрик облачной службы</h2>

Вы можете настроить получение оповещений на основе метрик мониторинга облачной службы. На странице **Службы управления** портала управления Azure можно создать правило, которое инициирует оповещение при достижении заданного значения выбранной метрики. Кроме того, вы можете настроить отправку сообщения электронной почты с оповещением. Дополнительную информацию см. в разделе [Практическое руководство. Получение уведомлений об оповещениях и управление правилами оповещения в Azure](http://go.microsoft.com/fwlink/?LinkId=309356).

<h2><a id="addmetrics"></a>Практическое руководство. Добавление метрик в таблицу метрик</h2>

1. На [портале управления](http://manage.windowsazure.com/) откройте страницу **Мониторинг** для облачной службы.

	По умолчанию в таблице отображается лишь часть доступных метрик. На этом рисунке показаны метрики подробного мониторинга, устанавливаемые по умолчанию для облачной службы. К ним относятся сводные данные сведения о свободной памяти и доступном хранилище (в мегабайтах) на уровне роли. Чтобы выбрать дополнительные сводные метрики и метрики на уровне роли для мониторинга на портале управления, используйте пункт **Добавить метрики**.
...
2. Добавление метрик в таблицу метрик:
	...
	a. Щелкните **Добавить метрики**. Откроется представленное ниже окно **Выберите метрики**.
