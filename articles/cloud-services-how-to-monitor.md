<properties linkid="manage-services-how-to-monitor-a-cloud-service" urlDisplayName="Способы мониторинга" pageTitle="Мониторинг облачной службы — Windows Azure" metaKeywords="Azure, мониторинг облачной службы, портал управления Azure, портал управления" description="Узнайте, как осуществлять мониторинг облачных служб с помощью портала управления Windows Azure." metaCanonical="" services="cloud-services" documentationCenter="" title="Мониторинг облачных служб" authors=""  solutions="" writer="ryanwi" manager="" editor=""  />
...
Вы можете отслеживать ключевые метрики производительности облачных служб на портале управления Windows Azure. Для каждой роли службы можно задать минимальный или подробный уровень, а также настроить отображение данных мониторинга. Подробные данные мониторинга размещаются в учетной записи хранения, которая доступна вне портала. 
...
> [WACOM.NOTE]
> Для подробного мониторинга вы можете добавить дополнительные счетчики производительности при запуске экземпляра роли с помощью файла конфигурации диагностики, а также удаленно через API диагностики Windows Azure. Чтобы отслеживать эти метрики на портале управления, перед настройкой подробного мониторинга необходимо добавить соответствующие счетчики производительности. Дополнительные сведения см. в разделе <a href="http://msdn.microsoft.com/ru-ru/library/gg433048.aspx">Сбор данных журнала с помощью диагностики Windows Azure</a> и <a href="http://msdn.microsoft.com/ru-ru/library/hh411542.aspx">Создание и использование счетчиков производительности в приложении Windows Azure</a>.
...
<h2><a id="verbose"></a>Практическое руководство. Настройка мониторинга для облачных служб</h2>

Чтобы настроить минимальный или подробный уровень мониторинга на портале управления, выполните следующие действия. Прежде чем настраивать подробный мониторинг, необходимо включить диагностику Windows Azure и настроить строки подключения диагностики, чтобы обеспечить доступ к учетным записям хранения, где будут размещаться данные мониторинга.

###Перед началом работы###

- Создайте учетную запись хранения, в которой будут размещаться данные мониторинга. Для каждой роли можно использовать собственную учетную запись хранения. Дополнительные сведения см. в справке для компонента **Учетные записи хранения** или в разделе [Создание учетной записи хранения](/ru-ru/manage/services/storage/how-to-create-a-storage-account/).


- Включите диагностику Windows Azure для ролей облачной службы. <br /><br />При этом необходимо обновить файлы определения (CSDEF) и конфигурации (CSCFG) службы. Дополнительные сведения см. в разделе [Настройка диагностики в Windows Azure](http://msdn.microsoft.com/ru-ru/library/windowsazure/dn186185.aspx).

На портале управления вы можете добавить или изменить строки подключения диагностики, которые используются Windows Azure для доступа к учетной записи хранения, где размещаются данные подробного мониторинга, а также выбрать необходимый уровень мониторинга. Поскольку данные подробного мониторинга размещаются в учетной записи хранения, перед настройкой этого уровня необходимо определить строки подключения диагностики.

###Настройка строк подключения диагностики для подробного мониторинга###

1. Скопируйте ключ доступа к хранилищу для учетной записи, в которой будут размещаться данные подробного мониторинга. Для этого войдите на [Портал управления Windows Azure](https://manage.windowsazure.com/) и выберите **Управление ключами** на странице **Учетные записи хранения**. Дополнительные сведения см. в разделе [Управление облачными службами](http://www.windowsazure.com/ru-ru/manage/services/cloud-services/how-to-manage-a-cloud-service/) или в справке для страницы **Учетные записи хранения**. 
...
4. Нажмите **Настроить**.
	...
	Параметры **мониторинга** представлены в верхней части страницы **Настройка** (см. ниже). Если для облачной службы не включена система диагностики Windows Azure, параметр **Уровень** будет недоступен. Изменить политику хранения данных нельзя. Данные подробного мониторинга для облачной службы хранятся в течение 10 дней.
...
<h2><a id="receivealerts"></a>Практическое руководство. Получение оповещений для метрик облачных служб</h2>

Вы можете настроить получение оповещений на основе метрик мониторинга облачной службы. На странице **Службы управления** портала управления Windows Azure можно создать правило, которое вызывает оповещение при достижении заданного значения выбранной метрики. Кроме того, вы можете настроить отправку сообщения электронной почты с оповещением. Дополнительные сведения см. в разделе [Практическое руководство. Получение уведомлений об оповещениях и управление правилами оповещения в Windows Azure](http://go.microsoft.com/fwlink/?LinkId=309356).

<h2><a id="addmetrics"></a>Практическое руководство. Добавление метрик в таблицу метрик</h2>
...
<h2><a id="customizechart"></a>Практическое руководство. Настройка диаграммы метрик</h2>
...
<h2><a id="accessverbose"></a>Практическое руководство. Доступ к данным подробного мониторинга вне портала управления</h2>
...
- *aggregation_interval* = 5M, 1H или 12H
...
Например, в следующих таблицах размещаются сводные данные подробного мониторинга, собираемые с интервалом в 1 час:
	...
	WAD8b7c4233802442b494d0cc9eb9d8dd9fPT1HRTable (почасовые сводные данные для роли)

	WAD8b7c4233802442b494d0cc9eb9d8dd9fPT1HRITable (почасовые сводные данные для экземпляров роли)
