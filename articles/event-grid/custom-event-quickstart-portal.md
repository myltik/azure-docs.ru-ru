...
1. Укажите имя раздела. Имя раздела должно быть уникальным, так как оно представлено записью службы доменных имен (DNS). Предварительная версия службы "Сетка событий Azure" поддерживает расположения **westus2** и **westcentralus**. Выберите группу ресурсов, созданную ранее. Нажмите кнопку **Создать**.
...
1. Укажите уникальное имя подписки на события. В качестве типа темы выберите **Event Grid Topics** (Разделы сетки событий). В качестве экземпляра выберите созданный пользовательский раздел. Укажите URL-адрес RequestBin как конечную точку для уведомления о событиях. По завершении ввода значений выберите **Создать**.
