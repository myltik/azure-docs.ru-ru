...
4. Нажмите кнопку **Создать**, чтобы добавить подписку на событие. Создается подписка на событие, которая вызывает функцию `imageresizerfunc` при добавлении большого двоичного объекта в контейнер *images*. Эта функция изменяет размер образов и добавляет их в контейнер *thumbs*.
...
Обратите внимание, что после того, как переданное изображение исчезнет, его копия появится в карусели **Generated thumbnails** (Созданные эскизы). Функция изменила размер этого образа, после чего он был добавлен в контейнер *thumbs* и скачан веб-клиентом.
