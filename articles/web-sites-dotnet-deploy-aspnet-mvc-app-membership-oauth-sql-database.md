...
-   Как использовать [OAuth][OAuth], [OpenID][OpenID] и базу данных членства ASP.NET для защиты приложения.
...
-   [Использование API членства][Использование API членства]
...
1.  В меню **Файл** выберите **Новый проект**.
  ...
  !["Новый проект" в меню "Файл"]["Новый проект" в меню "Файл"]
...
1.  В диалоговом окне **Публикации веб-сайта** щелкните **Опубликовать**.
    ...
    ![Запуск в облаке][Запуск в облаке]
...
3.  В диалоговом окне **Добавление Scaffold** выберите **Контроллер MVC 5 с представлениями, использующий EF** и щелкните **Добавить**.
    ...
    ![Диалоговое окно "Добавление Scaffold"][Диалоговое окно "Добавление Scaffold"]
...
6.  В текстовом поле **Имя контроллера** введите имя контроллера «CmController».
    ...
    ![Диалоговое окно "Новый контекст данных"][Диалоговое окно "Новый контекст данных"]
...
1.  В меню **Средства** выберите **Диспетчер пакетов NuGet**, а затем **Консоль диспетчера пакетов** .
    ![Консоль диспетчера пакетов в меню Средства][Консоль диспетчера пакетов в меню Средства]
...
6.  Замените метод *Seed* следующим кодом:
    ...
    Этот код инициализирует (заполняет) базу данных с контактной информацией. Дополнительные сведения об инициализации базы данных см. в разделе [Инициализация и отладка баз данных Entity Framework (EF)][Инициализация и отладка баз данных Entity Framework (EF)].
...
[OAuth][OAuth] — это открытый протокол, обеспечивающий безопасную авторизацию простым и стандартным способом из веб-приложений, мобильных и настольных приложений. В интернет-шаблоне ASP.NET MVC используются OAuth и [OpenID][OpenID] для предоставления Facebook, Twitter, Google и Microsoft в качестве поставщиков проверки подлинности. В этом учебнике в качестве поставщика проверки подлинности используется только Google, но вы легко можете изменить код, чтобы использовать любой другой поставщик. Шаги по реализации других поставщиков очень похожи на шаги, представленные в этом учебнике. Сведения об использовании Facebook в качестве поставщика проверки подлинности см. в учебнике [Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google][Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google]
...
1.  Выполните инструкции в учебнике [Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google][Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google] в **Создание приложения Google для OAuth 2, чтобы настроить приложение Google для OAuth2**.
...
4.  Сервер проверки подлинности Google запросит разрешение для приложения на просмотр адреса электронной почты и базовых сведений о вашей учетной записи. Нажмите **Принять** .
  ...
  ![Запрос разрешения GOOG][Запрос разрешения GOOG]
...
5.  Убедитесь, что обозреватель Visual Studio открывает браузер Internet Explorer, как показано на рисунке ниже:
    ...
    ![браузер по умолчанию][браузер по умолчанию]
...
1.  Для запуска приложения нажмите сочетание клавиш CTRL+F5. Следуйте инструкциям, чтобы подтвердить доверие к самозаверяющему сертификату, созданному IIS Express.
    ...
    ![инструкции для подтверждения доверия к самозаверяющему сертификату, созданному IIS Express][инструкции для подтверждения доверия к самозаверяющему сертификату, созданному IIS Express]
...
2.  Прочитайте текст в диалоговом окне **Предупреждение системы безопасности** и нажмите **Да**, если необходимо установить сертификат, представляющий **localhost**.
    ...
    ![Предупреждение о сертификате localhost в IIS Express][Предупреждение о сертификате localhost в IIS Express]

1.  В IE откроется *Главная* страница без предупреждений SSL.

    ![IE с SSL-сертификатом localhost и без предупреждений][IE с SSL-сертификатом localhost и без предупреждений]
    ...
    ![Предупреждение сертификата FireFox][Предупреждение сертификата FireFox]
...
2.  Щелкните правой кнопкой мыши каждый экземпляр веб-сайта и выберите **Остановить веб-сайт**.
    ...
    ![Остановка веб-сайта][Остановка веб-сайта]
...
2.  Щелкните правой кнопкой мыши **ContactDB** и выберите **Открыть в обозревателе объектов SQL Server**.
    ...
    ![Открытие в SSOX][Открытие в SSOX]
...
2.  Перейдите на вкладку **Серверы** и выберите сервер, который требуется настроить.
    ...
    ![Вкладка "Серверы" в Azure][Вкладка "Серверы" в Azure]

3.  Выберите вкладку **Настройка**.

4.  Добавьте имя правила, начальный и конечный IP-адреса.

    ![Диапазон IP-адресов][Диапазон IP-адресов]
...
7.  Вставьте значения **Сервер** и **ИД пользователя** в диалоговое окно **Подключение к серверу** в Visual Studio. Значение **ИД пользователя** переходит в поле **Вход**. Введите пароль, который использовался для создания базы данных SQL.
    ...
    ![Диалоговое окно "Подключиться к серверу"][Диалоговое окно "Подключиться к серверу"]
...
При текущей регистрации членства ASP.NET в проекте не поддерживается сброс паролей и не проверяется, человек ли регистрируется (например, с помощью запроса [CAPTCHA][CAPTCHA]). Пользователь, прошедший проверку подлинности с использованием одного из сторонних поставщиков, может зарегистрироваться. Если вы решите отключить локальную регистрацию, выполните следующие шаги:
...
Инструкции по добавлению данных профиля в базу данных регистрации пользователей и подробные инструкции по использованию Facebook в качестве поставщика проверки подлинности приведены в моем учебнике [Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google][Создание приложения ASP.NET MVC 5 с использованием OAuth2 и OpenID для входа с Facebook и Google].

Чтобы включить кнопки входа с помощью социальных сетей, расположенные в верхней части этого учебника, изучите раздел [Удобные кнопки входа социальных сетей для ASP.NET MVC 5][Удобные кнопки входа социальных сетей для ASP.NET MVC 5].
