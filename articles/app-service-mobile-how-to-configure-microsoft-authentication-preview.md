...
3. Щелкните **Параметры API**. Выберите **Да** для пункта **Мобильное или классическое клиентское приложение**. В поле **URL-адрес перенаправления** введите URL-адрес шлюза с добавлением пути _/signin-microsoft_. Например, `https://contosogateway.azurewebsites.net/signin-microsoft`. Убедитесь, что используете схему HTTPS. После ввода URL-адреса перенаправления щелкните **Сохранить**.
