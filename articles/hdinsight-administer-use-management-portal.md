<properties urlDisplayName="Administration" pageTitle="Управление кластерами Hadoop в HDInsight с использованием портала Azure | Azure" metaKeywords="" description="Learn how to administer HDInsight Service. Create an HDInsight cluster, open the interactive JavaScript console, and open the Hadoop command console." metaCanonical="" services="hdinsight" documentationCenter="" title="Manage Hadoop clusters in HDInsight using the Azure Management Portal" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />
...
- Дополнительную информацию об администрировании HDInsight с помощью Azure PowerShell см. в статье [Администрирование HDInsight с использованием PowerShell][hdinsight-admin-powershell].

- Дополнительную информацию об администрировании HDInsight с помощью кроссплатформенных программ командной строки см. в разделе [Администрирование HDInsight с использованием кроссплатформенного интерфейса командной строки][hdinsight-admin-cross-platform]. 
...
* [Подготовка кластеров HDInsight](#create)
...
Существует несколько методов для создания кластеров HDInsight, но в этой статье описывается только использование параметра "Быстрое создание" на портале управления Azure. Другие методы см. в разделе [Подготовка кластеров HDInsight][hdinsight-provision].

Кластер HDInsight использует контейнер хранилища BLOB-объектов Azure в качестве файловой системы по умолчанию. Дополнительную информацию о том, каким образом хранилище больших двоичных объектов Azure обеспечивает удобную работу с кластерами HDInsight, см. в статье [Использование хранилища BLOB-объектов Azure с HDInsight][hdinsight-storage].
...
Подробную информацию о создании учетной записи хранения Azure см. в статье [Создание учетной записи хранения][azure-create-storageaccount].


**Подготовка кластера HDInsight**

1. Войдите на [портал управления Azure][azure-management-portal].
2. В нижней части страницы щелкните **СОЗДАТЬ**, а затем последовательно выберите **СЛУЖБЫ ДАННЫХ**, **HDINSIGHT** и **БЫСТРОЕ СОЗДАНИЕ**.

3. Введите значения в полях **Имя кластера**, **Размер кластера**, **Пароль администратора кластера** и **Учетная запись хранения** Azure, а затем щелкните **Создать кластер HDInsight**. После создания и запуска кластера отображается состояние "Работает".
	...
	> [WACOM.NOTE] Если для кластера HDInsight выбрана учетная запись хранения Azure, единственный способ изменить ее - удалить кластер и создать новый кластер с нужной учетной записью хранения.
...
- Используйте параметры настройки кластера в Azure PowerShell или пакете SDK для HDInsight .NET во время подготовки кластера. Такие изменения конфигурации сохраняются на протяжении всего срока существования кластера и не попадают под воздействие операций повторного создания образа узла, которые периодически выполняет платформа Azure в рамках обслуживания. Дополнительную информацию об использовании параметров настройки кластера см. в разделе [Подготовка кластеров HDInsight][hdinsight-provision].
- Некоторые собственные компоненты Java, такие как Mahout, Cascading, могут выполняться в кластере как JAR-файлы. Эти JAR-файлы можно распространить в хранилище BLOB-объектов Azure (WASB) и отправить в кластеры HDInsight с помощью механизмов отправки заданий Hadoop. Дополнительную информацию см. в разделе [Отправка заданий Hadoop программными средствами][hdinsight-submit-jobs]. 


	>[WACOM.NOTE] Если возникают проблемы при развертывании или вызове JAR-файлов в кластерах HDInsight, обратитесь в [службу поддержки Майкрософт][hdinsight-support].
...
8. Введите значения в полях **ИМЯ ПОЛЬЗОВАТЕЛЯ** и **НОВЫЙ ПАРОЛЬ**.  Это будут новые имя пользователя и пароль для кластера.
...
6. В мастере **Настройка удаленного рабочего стола** введите имя пользователя и пароль для удаленного рабочего стола. Обратите внимание, что имя пользователя должно отличаться от имени пользователя, которое использовалось при создании кластера (при использовании параметра "Быстрое создание" имя пользователя по умолчанию - admin). Введите дату окончания срока действия в поле **ИСТЕКАЕТ**. Обратите внимание, что дата окончания срока действия должна быть в будущем и не позднее чем через неделю от сегодняшней даты. В качестве времени окончания срока действия по умолчанию предполагается полночь указанной даты. Затем щелкните значок флажка.
...
> [WACOM.NOTE] После включения RDP для кластера прежде чем подключаться к кластеру, вам необходимо обновить страницу.
...
3. Щелкните кластер HDInsight, к которому хотите подключиться.
4. В верхней части страницы щелкните **КОНФИГУРАЦИЯ**.
5. Щелкните **ПОДКЛЮЧИТЬСЯ**, а затем следуйте указаниям.
...
1. Создайте самозаверяющий сертификат, который используется для проверки подлинности запросов. Вы можете создать его с помощью IIS или [makecert][makecert-info].
...
6. Введите значения в полях **ИМЯ ПОЛЬЗОВАТЕЛЯ** и **НОВЫЙ ПАРОЛЬ**.  Это будут новые имя пользователя и пароль для кластера.
...
См. статью [Администрирование HDInsight с использованием PowerShell][hdinsight-admin-powershell].
...
7. Щелкните **Да**.
8. На рабочем столе дважды щелкните **Командная строка Hadoop**.
		
	![HDI.HadoopCommandLine][image-hadoopcommandline]


	Дополнительную информацию о командах Hadoop см. в [справочнике по командам Hadoop][hadoop-command-reference].
...
* [Администрирование HDInsight с использованием кроссплатформенного интерфейса командной строки][hdinsight-admin-cross-platform]
