<properties linkid="manage-services-hdinsight-howto-administer-hdinsight" urlDisplayName="Администрирование" pageTitle="Администрирование кластеров HDInsight с портала управления | Azure" metaKeywords="" description="Узнайте, как осуществляется администрирование службы HDInsight. Создайте кластер HDInsight, откройте интерактивную консоль JavaScript и командную консоль Hadoop." metaCanonical="" services="hdinsight" documentationCenter="" title="Администрирование кластеров HDInsight с использованием портала управления" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />



# Администрирование кластеров HDInsight с использованием портала управления

На портале управления Azure можно подготовить кластеры HDInsight, изменить пароль пользователя Hadoop и активировать RDP, чтобы получить доступ к командной консоли Hadoop в кластере. Помимо портала управления доступны и другие средства для администрирования HDInsight. 
...
- Дополнительные сведения об администрировании HDInsight с помощью межплатформенных средств командной строки см. в разделе [Администрирование HDInsight с использованием межплатформенного интерфейса командной строки][hdinsight-admin-cross-platform]. 
...
3. Укажите значения **Имя кластера**, **Размер кластера**, **Пароль администратора кластера**и **Учетная запись хранилища** Azure, затем щелкните **Создать кластер HDInsight**. После создания и запуска кластера отображается статус *Выполняется*.

	![HDI.QuickCreate][image-cluster-quickcreate]

	Если для создания кластера используется параметр "Быстрое создание", то для учетной записи администратора по умолчанию используется имя пользователя *admin*. Чтобы присвоить учетной записи другое имя пользователя, используйте параметр "Настраиваемое создание" вместо параметра "Быстрое создание". Имя учетной записи можно также изменить после завершения подготовки.
...
HDInsight работает со множеством компонентов Hadoop. Список проверенных и поддерживаемых компонентов см. в разделе [Какая версия Hadoop включена в Azure HDInsight?][hdinsight-version]. Настройка HDInsight может выполняться с использованием одного из следующих вариантов:
...
- Некоторые собственные компоненты Java, такие как Mahout, Cascading, могут выполняться в кластере как JAR-файлы. Эти JAR-файлы можно распространить в хранилище BLOB-объектов Azure (WASB) и отправить в кластеры HDInsight с помощью механизмов отправки заданий Hadoop. Дополнительные сведения см. в разделе [Отправка заданий Hadoop программными средствами][hdinsight-submit-jobs]. 
	...
	>[WACOM.NOTE] Если возникают проблемы при развертывании JAR-файлов в кластерах HDInsight или при вызове JAR-файлов в кластерах HDInsight, обратитесь в [службу поддержки Майкрософт][hdinsight-support].
	
	> Компоненты Mahout и Cascading не поддерживаются службой HDInsight, и на них не распространяется техническая поддержка Майкрософт. Списки поддерживаемых компонентов см. в разделе [Новые возможности версий кластеров, предоставляемых HDInsight][hdinsight-version].
...
Кластер HDInsight может иметь две учетные записи пользователя.  Учетная запись пользователя кластера HDInsight создается в процессе подготовки.  Можно также создать учетную запись пользователя RDP для доступа к кластеру через RDP. См. [Включение удаленного рабочего стола](#enablerdp).
...
* [Какая версия Hadoop включена в Azure HDInsight?][hdinsight-version]

[hdinsight-admin-cross-platform]: /ru-ru/manage/services/hdinsight/administer-hdinsight-using-command-line-interface/
...
[hdinsight-support]: http://www.windowsazure.com/ru-ru/support/options/
