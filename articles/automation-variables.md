...
Средства переменных — это значения, которые доступны для всех модулей Runbook в учетной записи автоматизации. Средства можно создавать, изменять или получать с помощью портала Azure, Windows PowerShell и из модуля Runbook. Переменные автоматизации полезны в следующих случаях:
...
- Управление значением из портала или из командной строки Windows PowerShell, используемой модулем Runbook.

Переменные службы автоматизации сохраняются для того, чтобы они были доступны после сбоя модуля Runbook. Это также позволяет модулю Runbook задать значение, которое может быть использовано другим модулем или этим же модулем при следующем запуске.

При создании переменной можно указать, что она должна храниться в зашифрованном виде. Если переменная зашифрована, она безопасно хранится в службе автоматизации Azure и ее значение не может быть получено с помощью командлета [Get-AzureAutomationVariable](http://msdn.microsoft.com/library/dn913772.aspx), который входит в модуль Azure PowerShell. Единственный способ получить зашифрованное значение — использовать процесс **Get-AutomationVariable** в модуле Runbook.

>[AZURE.NOTE]Безопасные средства в службе автоматизации Azure включают учетные данные, сертификаты, подключения и зашифрованные переменные. Эти средства шифруются и хранятся в службе автоматизации Azure, используя уникальный ключ, который создается для каждой учетной записи в службе автоматизации. Ключ шифруется главным сертификатом и хранится в службе автоматизации Azure. Прежде чем сохранять безопасное средство, ключ учетной записи в службе автоматизации дешифруется с помощью главного сертификата и используется для шифрования средства.
...
Командлеты, представленные в следующей таблице, используются для создания переменных и управления ими с помощью Windows PowerShell в службе автоматизации Azure. Они входят в состав [модуля Azure PowerShell](powershell-install-configure.md), доступного в модулях Runbook в службе автоматизации.
...
Действия рабочего процесса в следующей таблице используются для доступа к переменным автоматизации в модуле Runbook. Они доступны для использования в модуле Runbook и не поставляются вместе с модулем Azure PowerShell.
...
>[AZURE.NOTE]Не следует использовать переменные в параметре Name в командлете **Get-AutomationVariable** в модуле Runbook, поскольку это усложняет определение зависимостей между модулями Runbook и переменными автоматизации во время разработки.
...
### Создание новой переменной на портале предварительной версии Azure
...
Используйте команду **Set-AutomationVariable**, чтобы задать значение переменной автоматизации в модуле Runbook, и команду **Get-AutomationVariable**, чтобы получить его. Не следует использовать командлеты **Set-AzureAutomationVariable** и **Get-AzureAutomationVariable** в модуле Runbook, поскольку они менее эффективны, чем действия рабочего процесса. Также с помощью **Get-AzureAutomationVariable** нельзя получить значение безопасных переменных. Единственный способ создать новую переменную из модуля Runbook — это использовать командлет [New-AzureAutomationVariable](http://msdn.microsoft.com/library/dn913771.aspx).
