<properties title="How to use the queue service (PHP) - Azure feature guide" pageTitle="Как использовать службу очередей (PHP) - Microsoft Azure" metaKeywords="Azure Queue Service messaging PHP" description="Learn how to use the Azure Queue service to create and delete queues, and insert, get, and delete messages. Samples written in PHP." documentationCenter="PHP" services="storage" authors="tamram" manager="adinah" />
...
* [Создание приложения на PHP](#create-app)
...
<h2><a id="create-app"></a>Создание приложения на PHP</h2>
...
> [WACOM.NOTE]
> При работе с этим примером (и другими примерами в этой статье) предполагается, что вы установили клиентские библиотеки PHP для Azure с помощью Composer. При установке библиотек вручную или в качестве пакета PEAR необходимо сослаться на файл автозагрузчика WindowsAzure.php.
...
Объект **QueueRestProxy** позволяет создать контейнер больших двоичных объектов с помощью метода **createQueue**. При создании очереди можно задать ее параметры, однако это не является обязательным. (В приведенном ниже примере показано, как задать метаданные в очереди.)
...
Вы можете просмотреть сообщения в начале очереди, не удаляя их из нее, вызвав метод **QueueRestProxy->peekMessages**. По умолчанию метод **peekMessage** возвращает одно сообщение, но вы можете изменить это значение с помощью метода **PeekMessagesOptions->setNumberOfMessages**.
