...
>
>Операцию доступа к данным также можно выполнить с помощью встроенных вспомогательных функций API, доступ к которым можно получить по ссылке **Попробуйте** на начальной странице. Дополнительные сведения об использовании страниц API для тестирования мобильной службы см. в подразделе "Локальное тестирование проекта мобильной службы" раздела [Добавление мобильных служб к существующему приложению](/ru-ru/documentation/articles/mobile-services-dotnet-backend-windows-universal-dotnet-get-started-data/#test-the-service-locally).  

Оба базовых класса инициализатора удаляют из базы данных все таблицы, представления, функции и процедуры в схеме, используемой мобильной службой. 

+ **ClearDatabaseSchemaIfModelChanges** <br/> Объекты схемы удаляются, только если Code First обнаружит изменения в модели данных. Инициализатор по умолчанию в проекте серверной части .NET, загруженном с [портала управления Azure], наследует от этого базового класса.
...
1. В обозревателе решений Visual Studio щелкните правой кнопкой мыши проект мобильной службы и выберите пункт**Назначить запускаемым проектом**.
...
4. В консоли выполните следующую команду:
	...
	При этом создается новая миграция с именем *Initial*. Код миграции хранится в папке Migrations проекта.
...
8.   Теперь внесите изменение в модель данных, например добавьте новое свойство UserId в тип TodoItem, повторите сборку проекта и в диспетчере пакетов выполните следующую команду:
	...
	При этом создается новая миграция с именем *NewUserId*. В папку Migrations добавляется новый файл кода, который реализует данное изменение.  
...
14. (Необязательно) Нажмите кнопку **Управление**, войдите на сервер базы данных SQL, а затем нажмите кнопку **Конструктор** и убедитесь, что изменения схемы внесены в Azure. 
