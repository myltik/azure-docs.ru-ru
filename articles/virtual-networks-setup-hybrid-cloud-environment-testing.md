<properties 
	pageTitle="Настройка гибридной облачной среды для тестирования" 
...
#Настройка гибридной облачной среды для тестирования
...
Используйте инструкции в разделе "Действия для настройки подсети Corpnet" [Руководства по лаборатории тестирования: базовая конфигурация Windows Server 2012 R2](http://www.microsoft.com/download/details.aspx?id=39638) для настройки компьютеров DC1, APP1 и CLIENT1 в подсети с именем Corpnet. **Эта подсеть должна быть изолирована от сети организации, поскольку она будет подключена прямо к Интернету через компьютер RRAS1.** 
...
Используйте следующие команды в командной строке Windows PowerShell с правами администратора на RRAS1. Перед выполнение этих команд заполните значения переменных и удалите символы < and >. Текущие имена сетевых адаптеров можно получить из отображения команды **Get-NetAdapter**.
...
Во-первых, войдите в [Портал управления Azure](https://manage.windowsazure.com/microsoft.onmicrosoft.com#Workspaces/All/dashboard) с учетными данными своей подписки Azure и создайте виртуальную сеть с именем TestVNET.
...
3.	В разделе **Расположение** выберите подходящий центр обработки данных для вашего расположения.
4.	Нажмите стрелку Далее.
5.	На странице "DNS-серверы и подключение VPN" в разделе **DNS-серверы** введите **DC1** в поле **Выбрать или ввести имя** введите **10.0.0.1** в поле **IP-адрес**, а затем выберите **Настроить VPN тип "сайт-сайт"**.
...
8.	На странице "Подключение сайт-сайт":
	...
	- В поле **Адресное пространство** в столбце **Начальный IP-адрес** введите **10.0.0.0**. В **CIDR (Количество адресов)** введите **/8**, а затем щелкните **Добавить адресное пространство**.
...
10.	На странице " Адресные пространства виртуальной сети":
	...
	- В столбце **CIDR (Количество адресов)** для TestSubnet щелкните **/24 (256)**.
	- Щелкните **Добавить подсеть шлюза**.
11.	Щелкните значок Завершить. Дождитесь создания виртуальной сети прежде, чем продолжить.

Далее следуйте инструкциям раздела [Установка и настройка Azure PowerShell](../install-configure-powershell/) для установки Azure PowerShell на локальном компьютере.

После этого создайте новую облачную службу для виртуальной сети TestVNET. Необходимо выбрать уникальное имя. Например, можно ее назвать TestVNET-*UniqueSequence*, где *UniqueSequence* - это сокращенное название вашей организации. К примеру, если ваша организация называется Tailspin Toys можно назвать облачную службу, как TestVNET-Tailspin.
...
#Этап 4. Создание VPN-подключения сайт-сайт

Сначала необходимо создать шлюз виртуальной сети.

1.	В портале управления Azure на локальном компьютере щелкните **Сети** в левой панели, а затем убедитесь, что столбец **Состояние** для TestVNET имеет значение **Создана**.
2.	Щелкните **TestVNET**. На странице Панель мониторинга вы должны увидеть состояние **Шлюз не был создан**.
3.	На панели задач щелкните **Создать шлюз**, а затем щелкните **Динамическая маршрутизация**. Щелкните **Да** при появлении запроса. Дождитесь завершения создания шлюза и изменения его состояния на **Подключение**. Это может занять несколько минут.
4.	На странице Панель мониторинга обратите внимание на **IP-адрес шлюза**. Это общий IP-адрес шлюза Azure VPN для виртуальной сети TestVNET. Данный IP-адрес понадобится для настройки RRAS1.
...
Затем перейдите на портал управления Azure на локальном компьютере и дождитесь, пока виртуальная сеть TestVNET не покажет состояние **Подключена**.
...
5.	В дереве консоли щелкните правой кнопкой мыши **NAT**, нажмите кнопку **Новый интерфейс**, щелкните **Corpnet**, а затем нажмите кнопку **OK** два раза.
6.	Щелкните правой кнопкой мыши **NAT**, нажмите кнопку **Новый интерфейс**, щелкните **И Интернет**, а затем нажмите кнопку **OK**.
7.	На вкладке **NAT** нажмите кнопку **Общий интерфейс подключен к Интернету**, выберите **Включить NAT на данном интерфейсе**, а затем нажмите кнопку **OK**.
...
3.	При появление запроса на открытие DC2.rdp щелкните **Открыть**.
...
9.	На странице "Подтверждение выбранных параметров" щелкните **Далее**.
...
4.	В разделе **DNS-серверы** добавьте запись с **DC2** в качестве имени и **192.168.0.4** в качестве IP-адреса. 
...
[Настройка фермы SharePoint в интрасети в гибридном облаке для тестирования](./virtual-networks-setup-sharepoint-hybrid-cloud-testing/)
...
1.	В портале управления Azure на локальном компьютере щелкните **Сети** в левой панели, а затем щелкните **TestVNET**. На странице Панель мониторинга вы должны увидеть состояние **Шлюз не был создан**.
...
3.	На странице Панель мониторинга обратите внимание на **IP-адрес шлюза**. Это новый общий IP-адрес шлюза Azure VPN для виртуальной сети TestVNET. Данный IP-адрес понадобится для перенастройки RRAS1.
