...
Подробные сведения об этом узле см. [здесь][MSDNFunctionImportLink].
...
**ReturnType** *(необязательно)* — задает тип элементов, возвращаемых URI. Не используйте этот атрибут, если функция не возвращает значение. Ниже приведены поддерживающиеся типы.
 ...
 - **Collection (<имя типа сущности>)** — определяет коллекцию определенных типов сущностей. Имя указано в атрибуте Name узла EntityType. Например, Collection(WXC.HourlyResult).
 - **Raw (<тип MIME>)** — определяет необработанный документ или большой двоичный объект, возвращаемый пользователю. Например, Raw(image/jpeg). Другие примеры:
...
Сведения об этом узле см. [здесь](http://msdn.microsoft.com/library/bb399206.aspx). (Используйте раскрывающийся список **Другие версии**, чтобы выбрать для просмотра документации другую версию.)
...
| AttributeName | Обязательное | Значение |
|----|----|----|
| Имя | Да | Имя свойства. |
