...
**Допустимые HTTP-команды** GET — возвращает значения из базы данных (возвращает коллекцию); POST — используется для передачи данных и необязательных значений, возвращаемых из базы данных (создать новую запись в коллекции, вернуть идентификатор или URI); DELETE — удаляет данные из базы данных (удаляет коллекцию); PUT — обновляет данные в БД (заменяет коллекцию или создает новую)
...
**Условия.** Сопоставление условий CSDL и условий [портала публикации](https://publish.windowsazure.com) пользовательского интерфейса (PPUI). — Предложение "Title" в PPUI соответствует предложению MyWebOffer. — MyCompany в PPUI соответствует **отображаемому имени издателя** в пользовательском интерфейсе [центра разработчиков Майкрософт](http://dev.windows.com/registration?accountprogram=azure). — Ваш API соответствует веб-службе или службе данных (план в PPUI).
