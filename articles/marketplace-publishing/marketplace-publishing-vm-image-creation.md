...
В следующем разделе приводятся рекомендации по созданию образа виртуальной машины Linux для Azure Marketplace. Пошаговые инструкции см. в статье [Создание и передача виртуального жесткого диска с операционной системой Linux](../virtual-machines/virtual-machines-linux-classic-create-upload-vhd.md?toc=%2fazure%2fvirtual-machines%2flinux%2fclassic%2ftoc.json).
...
Чтобы начать, создайте виртуальную машину из одного из следующих образов, находящихся на [портале Microsoft Azure][link-azure-portal]:
...
Чтобы скачать файл удаленного рабочего стола на локальный компьютер, используйте [командлет Get-AzureRemoteDesktopFile cmdlet][link-technet-2]. Чтобы использовать этот командлет, необходимо знать имя службы и имя виртуальной машины. Если вы создали виртуальную машину на [портале Microsoft Azure][link-azure-portal], эти сведения можно найти в свойствах виртуальной машины.

1. На портале Microsoft Azure выберите **Обзор** > **Виртуальные машины**.
...
* Вы можете запустить следующую команду из каталога %windir%\System32\Sysprep.
  ...
  Рекомендации по подготовке операционной системы к использованию команды sysprep приведены на шаге 1 статьи MSDN [Создание и передача виртуального жесткого диска Windows Server в Azure](../virtual-machines/virtual-machines-windows-classic-createupload-vhd.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fclassic%2ftoc.json).
...
* [Приступая к работе с командлетами Azure PowerShell](../powershell-install-configure.md)
...
* [интерфейс командной строки Azure](../virtual-machines/virtual-machines-linux-capture-image.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
Инструкции по созданию URI подписанного URL-адреса см. в статье [Использование подписанных URL-адресов (SAS)][link-azure-1] и [Подписанные URL-адреса. Часть 2: создание и использование подписанного URL-адреса в службе BLOB-объектов][link-azure-2].
...
8. В диалоговом окне **Blob Container Security** (Безопасность контейнера больших двоичных объектов) на вкладке **Access Level** (Уровень доступа) оставьте значения по умолчанию и перейдите на вкладку **Shared Access Signatures** (Подписанные URL-адреса).
...
9. Выполните следующие действия, чтобы создать URI подписанного URL-адреса для VHD-образа.
    ...
    b.    **Access permitted to** (Доступ разрешен до). Выберите дату, которая наступит через 7–8 дней после даты, указанной для параметра **Access permitted from** (Доступ разрешен с).
   
    В.    **Actions permitted** (Разрешенные действия). Выберите разрешения на **создание списков** и **чтение**.
...
1. Перейдите на [портал публикации][link-pubportal] и выполните вход, используя свою учетную запись продавца.
