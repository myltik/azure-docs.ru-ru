...
* [Приступая к работе с командлетами Azure PowerShell](/powershell/azureps-cmdlets-docs)
...
10.    Выполните следующие действия, чтобы создать URI подписанного URL-адреса для VHD-образа.
    ...
    а. **Access permitted from** (Доступ разрешен с). Чтобы сохранить время в формате UTC, выберите дату начала за день до текущей даты. Например, если сегодня 6 октября 2014 г., выберите 05.10.2014.
...
1.    Скачайте обозреватель хранилищ Microsoft Azure Storage Explorer на сайте [http://storageexplorer.com/](http://storageexplorer.com/). Найдите [обозреватель хранилищ Microsoft Azure](http://storageexplorer.com/releasenotes.html) и нажмите кнопку **скачивания для Windows**.
...
7.    Обновите время начала, время окончания срока действия и разрешения, как показано ниже.
    ...
    а.    **Start Time** (Время начала). Чтобы сохранить время в формате UTC, выберите дату начала за день до текущей даты. Например, если сегодня 6 октября 2014 г., выберите 05.10.2014.
...
3.    Создайте файл PowerShell, используя следующий код, а затем сохраните его в локальной переменной.
    ...
    c. **`<Permission Start Date>`**: чтобы сохранить время в формате UTC, выберите дату начала за день до текущей даты. Например, если сегодня 26 октября 2016 г., выберите значение 25.10.2016.

    d. **`<Permission End Date>`**: выберите дату, которая наступит через 3 недели после даты **Start Time** (Время начала). Значение должно быть **02.11.2016**.
