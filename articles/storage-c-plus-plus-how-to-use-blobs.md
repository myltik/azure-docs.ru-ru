...
Для перечисления BLOB-объектов в контейнере сначала необходимо получить ссылку на контейнер. Затем можно использовать метод контейнера **list\_blobs\_segmented** для извлечения BLOB-объектов или находящихся в нем каталогов. Для доступа к широкому набору свойств и методов возвращаемого **blob\_result\_segment** необходимо вызвать свойство **blob\_result\_segment.blobs** для получения объекта **cloud\_blob** или свойство **blob\_result\_segment.directories** для получения объекта cloud\_blob\_directory. В следующем коде показано, как получить и вывести URI каждого элемента в контейнере **my-sample-container**.
