<properties urlDisplayName="Upload a SUSE Linux VHD" pageTitle="Создание и передача виртуального жесткого диска с операционной системой SUSE Linux в Azure" metaKeywords="Azure VHD, uploading Linux VHD, SUSE, SLES, openSUSE" description="Узнайте, как создать и передать виртуальный жесткий диск (VHD-файл) Azure, содержащий операционную систему SUSE Linux." metaCanonical="" services="virtual-machines" documentationCenter="" title="Создание и передача виртуального жесткого диска с операционной системой SUSE Linux" authors="kathydav" solutions="" manager="timlt" editor="tysonn" />
...
-   [Подготовка виртуальной машины SLES 11 SP3 для Azure][Подготовка виртуальной машины SLES 11 SP3 для Azure]
-   [Подготовка виртуальной машины openSUSE 13.1+ для Azure][Подготовка виртуальной машины openSUSE 13.1+ для Azure]
...
## <span id="sles11"></span> </a>Подготовка SUSE Linux Enterprise Server 11 SP3
...
## <span id="osuse"></span> </a>Подготовка openSUSE 13.1+
...
3.  В консоли оболочки выполните команду `zypper lr`. Если эта команда возвращает результат аналогичный приведенному ниже (обратите внимание, что номера версий могут отличаться):
    ...
    Чтобы убедиться в том, что репозитории были добавлены, можно еще раз выполнить команду "`zypper lr`". Если один из соответствующих репозиториев обновлений не включен, включите его, выполнив следующую команду:
