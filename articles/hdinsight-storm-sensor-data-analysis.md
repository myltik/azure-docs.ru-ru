...
> [AZURE.NOTE] Полная версия проекта доступна на <https://github.com/Blackmist/hdinsight-eventhub-example>.

## Предварительные требования

-   Подписка Azure

-   Visual Studio с [Microsoft Azure SDK для .NET][Microsoft Azure SDK для .NET]
...
13. Выполните команду **Сохранить все** для этого проекта.
...
8.  Выполните команду **Сохранить все**, когда запускаете приложение для заполнения Event Hub сообщениями.
...
2.  В каталоге **helpers** создайте два новых файла:
    ...
    -   **EventHubMessage.java** — определяет формат сообщений Еvent Нub

    -   **SignalRMessage.java** — определяет формат сообщений, отправляемых в SignalR
...
2.  В каталоге **bolts** создайте два новых файла:
    ...
    -   **ParserBolt.java** — выполняет синтаксический разбор входящих сообщений от Event Hub в отдельные поля, затем выдает два потока
    -   **DashboardBolt.java** — регистрирует информацию в панели мониторинга в Интернете через SignalR
...
-   Подробнее об Apache Storm см. по адресу <https://storm.incubator.apache.org/>

-   Подробнее о HBase с HDInsight см. в статье [Обзор HBase с HDInsight][Обзор HBase с HDInsight]

-   Подробнее о SignalR см. в статье [ASP.NET SignalR][ASP.NET SignalR]

-   Подробнее о D3.js см. в статье [D3.js — документы, управляемые данными][D3.js — документы, управляемые данными]

-   Подробнее о создании топологий в .NET см. в статье [Разработка приложений для обработки потоковых данных с помощью SCP.NET и C\# на платформе Storm в HDInsight][Разработка приложений для обработки потоковых данных с помощью SCP.NET и C\# на платформе Storm в HDInsight]

  [Microsoft Azure SDK для .NET]: http://azure.microsoft.com/ru-ru/downloads/archive-net-downloads/
  ...
  [Разработка приложений для обработки потоковых данных с помощью SCP.NET и C\# на платформе Storm в HDInsight]: /ru-ru/documentation/articles/hdinsight-hadoop-storm-scpdotnet-csharp-develop-streaming-data-processing-application/
