...
Панель мониторинга используется для отображения информации датчика в режиме, близком к реальному времени. В данном случае панель мониторинга является приложением ASP.NET, расположенным на веб-сайте Azure. Основная цель приложения - работать в качестве концентратора [SignalR](http://www.asp.net/signalr/overview/getting-started/introduction-to-signalr) , который получает информацию из топологии Storm и обрабатывает ее сообщения.
...
3. В диалоговом окне **Настройка сайта Microsoft Azure** укажите **Имя сайта** и **Регион** для вашего веб-сайта, а затем нажмите кнопку **ОК**. Так вы создадите веб-сайт Azure, содержащий панель мониторинга.

3. В **обозревателе решений** щелкните правой кнопкой мыши проект, затем выберите **Добавить | Класс концентратора SignalR (v2)**. Присвойте новому классу имя **DashHub.cs** и добавьте его в проект. Он будет содержать концентратор SignalR, используемый для взаимодействия между HDInsight и веб-страницей панели мониторинга.

	> [AZURE.NOTE] Если вы используете Visual Studio 2012, шаблон **Класс концентратора SignalR (v2)** не будет доступен. Вместо этого вы можете добавить простой **Класс** с именем DashHub. Вам может также потребоваться вручную установить пакет SignalR, открыв **Инструменты | Диспетчер пакета библиотек | Консоль диспетчера пакетов** и выполнив следующую команду:
	>
	> `install-package Microsoft.AspNet.SignalR`

4. Замените код в **DashHub.cs** на приведенный ниже:
...
5. В **обозревателе решений** щелкните правой кнопкой мыши проект, затем выберите **Добавить | Класс запуска OWIN**. Назовите новый класс **Startup.cs**.

	> [AZURE.NOTE] Если вы используете Visual Studio 2012, шаблон **Класс запуска OWIN** не будет доступен. Вместо этого вы можете добавить простой **Класс** с именем Startup.

6. Замените содержимое **Startup.cs** следующим кодом.
...
7. В **обозревателе решений** щелкните правой кнопкой мыши проект, затем выберите **Добавить | Страница HTML**. Присвойте новой странице имя **index.html**. Страница будет содержать панель мониторинга в режиме реального времени для этого проекта. Она будет получать информацию от DashHub и будет отображать диаграмму, используя D3.js.

8. В **Обозревателе решений** щелкните правой кнопкой мыши **index.html** и выберите **Задать как начальную страницу**.
...
11. Замените существующий код в файле **test.html** следующим кодом.
...
11. Выполните команду **Сохранить все** для этого проекта.

12. В **Обозревателе решений** щелкните правой кнопкой мыши проект **Панель мониторинга** и нажмите **Опубликовать. Выберите веб-сайт, созданный для проекта, затем нажмите **Опубликовать**.

13. После того как вы опубликовали сайт, должна открыться веб-страница, отображающая движущуюся временную шкалу.

###Тестирование панели мониторинга

14. Чтобы проверить, работает ли SignalR и отображаются ли линии графика данных, отправленных в SignalR, откройте новое окно браузера на странице **test.html этого веб-сайта. Например: **http://mydashboard.azurewebsites.net/test.html**.

15. Панель мониторинга ожидает передачи форматированных данных JSON с **кодом устройства** и  **температурой**. Например **{"device":0, "temperature":80}**. Введите тестовые значения на странице **test.html**, используя коды устройств от 0 до 9, пока панель мониторинга открыта на другой странице. Обратите внимание, что линии для каждого отдельного устройства рисуются разными цветами.
...
2. В диалоговом окне **Настройка концентратора событий** введите **Число разделов** и **Хранение сообщений**. В этом примере числу разделов присвойте значение 10, а хранению сообщений - 1.
...
5. После сохранения политик используйте клавишу **Генератор ключей совместного доступа** в нижней части страницы для получения ключа для обеих политик: **устройства** и **storm**. Сохраните для дальнейшего использования.
...
2. В **Обозреватель решений** щелкните правой кнопкой мыши элемент **SendEvents** и выберите команду **Управление пакетами NuGet**.
...
4. В файле **Program.cs** установите значение следующих переменных в начале файла в соответствии со значениями, полученными из вашего концентратора событий на портале управления Azure.
...
4. В **Обозревателе решений** щелкните правой кнопкой мыши **SendEvents** и **Добавить | Класс**. Присвойте новому классу имя **Event.cs**. Это послужит описанием для сообщения, передаваемого Event Hub.

5. Замените содержимое **Event.cs** следующим.
...
6. Выполните команду **Сохранить все**, затем запустите приложение для заполнения концентратора событий сообщениями.
...
1. Осуществите вход на [портал управления Azure][azure-portal].
...
1. Осуществите вход на [Портал управления Azure][azureportal]
...
6. Для установки пакета в локальный репозиторий Maven выполните следующую команду. Это позволит нам в дальнейшем легко добавлять его в качестве ссылки в проект Storm.

		mvn install:install-file -Dfile=target\eventhubs-storm-spout-0.9-jar-with-dependencies.jar -DgroupId=com.microsoft.eventhubs -DartifactId=eventhubs-storm-spout -Dversion=0.9 -Dpackaging=jar

####Загрузка и установка клиента SignalR

Чтобы отправить сообщения в панель мониторинга ASP.NET, используйте [пакет SDK клиента SignalR для Java](https://github.com/SignalR/java-client).
...
4. CИзмените каталог на **java-client\signalr-client-sdk** и скомпилируйте проект в файл JAR, выполнив следующие команды.
...
1. В каталоге **Temperature** создайте новый файл с именем **conf**.
...
3. Измените содержимое файла **Config.properties** следующим образом.
	...
	Замените **password** ключом для политики **storm**, созданным ранее на концентраторе событий. Замените **namespace** пространством имен вашего концентратора событий.

3. Измените содержимое файла **hbase-site.xml** следующим образом.
...
3. В файле **hbase-site.xml** замените значение суффикса (**suffix**) данных zookeeper DNS-суффиксом, полученным вами ранее для HBase. Например, **zookeeper0.mycluster.b4.internal.cloudapp.net, zookeeper1.mycluster.b4.internal.cloudapp.net, zookeeper2.mycluster.b4.internal.cloudapp.net**.
...
2. В каталоге **helpers** создайте два новых файла:
	...
	* **EventHubMessage.java** - определяет формат сообщений концентратора событий
...
2. В каталоге **bolts** создайте два новых файла:
	...
	* **ParserBolt.java** - выполняет синтаксический разбор входящих сообщений от концентратора событий в отдельные поля, затем выдает два потока
	* **DashboardBolt.java** - регистрирует информацию в панели мониторинга в Интернете через SignalR

2. Вставьте в файл **ParserBolt.java** следующее содержимое.
...
3. Вставьте в файл **DashboardBolt.java** следующее содержимое.
...
1. Запустите приложение **SendEvent** .NET, чтобы начать посылать события и дать нам возможность прочитать какие-либо данные с концентратора событий.
...
3. Когда вы убедитесь, что топология функционирует, остановите ее, нажав Ctrl-C. Для остановки приложения SendEvent выберите окно и нажмите любую клавишу.

###Включение HBaseBolt и подготовка HBase

1. Откройте файл **Temperature.java** и удалите комментирование (//) в следующих строках:
...
1. Выполните следующую команду, чтобы создать файл JAR из вашего проекта.
	...
	Это создаст файл под именем **TemperatureMonitor-1.0-SNAPSHOT.jar** в папке **target** вашего проекта.
...
1. Подключитесь к вашему кластеру HDInsight Storm с помощью удаленного рабочего стола и скопируйте файл **TemperatureMonitor-1.0-SNAPSHOT.jar** в каталог **c:\apps\dist\storm&lt;version number>**.

2. Используйте значок **Коммандная строка HDInsight** на рабочем столе, чтобы открыть новую командную строку, а также запустите топологию с помощью следующих команд.

		cd %storm_home%
		bin\storm jar TemperatureMonitor-1.0-SNAPSHOT.jar com.microsoft.examples.Temperature Temperature

3. После запуска топологии перед появлением данных на веб-панели мониторинга пройдет несколько секунд.

3. После появления на панели мониторинга данных переключитесь на сеанс удаленного рабочего стола в кластере HBase.

4. Из оболочки HBase введите следующую команду.

		scan 'SensorData'

	Обратите внимание, что появятся строки с данными, написанными топологией Storm.

5. Для остановки топологии перейдите к сеансу удаленного рабочего стола с кластером Storm и введите в командную строку HDInsight следующее.
...
* Дополнительную информацию о топологиях в .NET см. в статье [Разработка приложений для обработки потоковых данных с помощью SCP.NET и C# на платформе Storm в HDInsight](/ru-ru/documentation/articles/hdinsight-hadoop-storm-scpdotnet-csharp-develop-streaming-data-processing-application/)
