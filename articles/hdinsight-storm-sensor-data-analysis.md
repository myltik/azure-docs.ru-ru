<properties title="Анализ данных датчиков с помощью Storm и HDInsight" pageTitle="Анализ данных датчиков с помощью Apache Storm и Microsoft Azure HDInsight (Hadoop)" description="Узнайте, как использовать Apache Storm для обработки данных датчиков в режиме реального времени в HDInsight (Hadoop)" metaKeywords="Azure hdinsight storm, Azure hdinsight realtime, azure hadoop storm, azure hadoop realtime, azure hadoop real-time, azure hdinsight real-time" services="hdinsight" solutions="" documentationCenter="big-data" authors="larryfr" videoId="" scriptId="" />

<tags ms.service="hdinsight" ms.workload="big-data" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="09/30/2014" ms.author="larryfr" />

# Анализ данных датчиков с помощью Storm и HBase в HDInsight (Hadoop)

Узнайте, как построить решение на базе кластера HDInsight Storm для обработки данных датчиков, полученных из Azure Event Hub. Во время обработки топология Storm сохранит входящие данные в кластере HBase. Топология будет использовать также библиотеку SignalR для предоставления информации в режиме, близком к реальному времени, с помощью информационной панели, расположенной на веб-сайте Azure.

> [AZURE.NOTE] Полная версия проекта доступна на [][]<https://github.com/Blackmist/hdinsight-eventhub-example></a>.

## Предварительные требования

-   Подписка Azure

-   Visual Studio с [Microsoft Azure SDK for .NET][Microsoft Azure SDK for .NET]

-   [Java и JDK][Java и JDK]

-   [Maven][Maven]

-   [Git][Git]

> [AZURE.NOTE] Java, /JDK, Maven и Git доступны также через диспетчер пакетов [Chocolatey NuGet][Chocolatey NuGet].

## Создание панели мониторинга

Панель мониторинга используется для отображения информации датчика в режиме, близком к реальному времени. В данном случае панель мониторинга является приложением ASP.NET, расположенным на веб-сайте Azure. Основной целью приложения является служить концентратором [SignalR][SignalR], получающим информацию от топологии Storm по мере обработки сообщений.
...
> [WACOM.NOTE] Хотя вместо SignalR вы можете использовать необработанный WebSockets, WebSockets не имеет встроенного механизма масштабирования, который потребуется при необходимости масштабировать веб-страницу. SignalR можно масштабировать с помощью Azure Service Bus ([][1]<http://www.asp.net/signalr/overview/performance/scaleout-with-windows-azure-service-bus></a>).
>
> Пример использования топологии Storm для взаимодействия с веб-сайтом Python с помощью WebSockets см. в проекте [Storm Tweet Sentiment D3 Visualization][Storm Tweet Sentiment D3 Visualization].

1.  Создайте новое приложение на C# в Visual Studio с использованием шаблона **веб-приложения ASP.NET**. Назовите новое приложение **Панель мониторинга**.

2.  В окне **Новый проект ASP.NET** выберите шаблон приложения **Новый**. В разделе **Windows Azure** выберите **Разместить в облаке** и **Веб-сайт**. Чтобы завершить, нажмите кнопку **ОК**.

    > [AZURE.NOTE] Если появится соответствующая подсказка, выполните вход в подписку Azure.

3.  В диалоговом окне **Настройка сайта Windows Azure Site** введите **Имя веб-сайта** и **Регион** вашего веб-сайта, затем выберите **ОК**. Так вы создадите веб-сайт Azure, содержащий панель мониторинга.

4.  В **Обозревателе решений** правой кнопкой мыши выберите проект, затем **Добавить | Класс концентратора SignalR (v2)**. Присвойте новому классу имя **DashHub.cs** и добавьте его в проект. Он будет содержать концентратор SignalR, используемый для взаимодействия между HDInsight и веб-страницей панели мониторинга.

    > [AZURE.NOTE] Если вы используете Visual Studio 2012, шаблон **Класс концентратора SignalR (v2)** не будет доступен. Вместо этого вы можете добавить простой **Класс** DashHub. Вам может также потребоваться вручную установить пакет SignalR, открыв **Инструменты | Диспетчер пакета библиотек | Консоль диспетчера пакетов** и выполнив следующую команду:
    >
...
6.  В **Обозревателе решений** правой кнопкой мыши выделите проект, затем выберите **Добавить | Класс запуска OWIN**. Назовите новый класс **Startup.cs**.

    > [AZURE.NOTE] Если вы используете Visual Studio 2012, шаблон **Класс запуска OWIN** не будет доступен. Вместо этого вы можете создать **Класс**, назвав его Startup.

7.  Замените содержимое **Startup.cs** следующим.
...
10. Замените существующий код в файле **index.html** следующим кодом.
    ...
    > [AZURE.NOTE] Диспетчер пакетов может установить новую версию скриптов SignalR. Проверьте, совпадают ли ссылки скриптов, приведенных ниже, с версиями файлов скриптов в проекте (если вместо концентратора с помощью NuGet была добавлена библиотека SignalR, они будут различаться.)

11. В **Обозревателе решений** щелкните правой кнопкой мыши проект, затем щелкните **Добавить | Страница HTML**. Присвойте новой странице имя **test.html**. Эту страницу можно использовать для проверки DashHub и панели мониторинга, отправляя и получая сообщения.
...
13. **Сохранить все**(сохранение проекта).

14. В **Обозревателе решений** щелкните правой кнопкой мыши проект **Панель мониторинга** и нажмите **Опубликовать**. Выберите веб-сайт, созданный для проекта, затем нажмите **Опубликовать**.
...
1.  Чтобы проверить, работает ли SignalR и отображаются ли линии графика данных, отправленных в SignalR, откройте новое окно браузера на странице **test.html** этого веб-сайта. Например, **<http://mydashboard.azurewebsites.net/test.html>**.

2.  Панель мониторинга ожидает передачи форматированных данных JSON с **кодом устройства** и **температурой**. Например **{"устройство":0, "температура":80}**. Введите тестовые значения на странице **test.html**, используя коды устройств от 0 до 9, пока панель управления открыта на другой странице. Обратите внимание, что линии для каждого отдельного устройства рисуются разными цветами.
...
1.  На портале [Портал Azure][Портал Azure] выберите **СОЗДАТЬ | Служебная шина | Концентратор событий | Настраиваемое создание**.

2.  В диалоговом окне **Добавить новый концентратор Event Hub**введите **Имя концентратора Event Hub**, выберите **Регион** для создания в нем концентратора, либо создайте новое пространство имен или выберите существующее. После этого щелкните клавишу **Стрелка**.

3.  В диалоговое окно **Настройка концентраторов Event Hub** введите значения параметров **Число разделов** и **Хранение сообщений**. В этом примере числу разделов присвойте значение 10, а хранению сообщений — 1.

4.  После создания концентратора событий выберите пространство имен, затем выберите **концентраторы Event Hubs**. После этого выберите раннее созданный вами концентратор событий.
...
1.  В Visual Studio создайте новый проект **Рабочий стол Windows** и выберите шаблон проекта **Консольное приложение**. Присвойте проекту имя **SendEvents** и нажмите кнопку **ОК**.
...
5.  В файле **Program.cs** установите значение следующих переменных в начале файла в соответствии со значениями, полученными из вашего концентратора Event Hub на портале управления Azure.

    | Задать в качестве значения... | Следующее...                                                              |
    |-------------------------------|---------------------------------------------------------------------------|
    | eventHubName                  | Имя вашего концентратора событий. Например, **temperature**.              |
    | eventHubNamespace             | Пространство имен вашего концентратора событий. Например, **sensors-ns**. |
    | sharedAccessPolicyName        | Созданная вами политика доступа к отправке. Например, **устройства**.     |
    | sharedAccessPolicyKey         | Ключ для политики доступа к отправке.                                     |
...
8.  **Выберите Сохранить все**, затем запустите приложение для заполнения концентратора Event Hub сообщениями.
...
2.  В нижней части страницы щелкните **+НОВЫЙ**, щелкните **Сетевые службы**, щелкните **Виртуальная сеть**, **Быстрое создание**.
...
3.  Введите или выберите следующие значения:
    ...
    -   **Сервер DNS**: В этой статье используется внутренний сервер DNS, предоставленный Azure, поэтому вы можете выбрать **Нет**. Также поддерживаются более продвинутые сетевые конфигурации с другими серверами DNS. Подробное руководство см. по адресу [][2]<http://msdn.microsoft.com/library/azure/jj156088.aspx></a>.
...
9.  В нижней части страницы указано имя подсети по умолчанию **Subnet-1**. Используйте клавишу**добавить подсеть**, чтобы добавить **Subnet-2** (Подсеть 2). Эти подсети будут содержать кластеры Storm и HBase

    > [WACOM.NOTE] В этой статье используются кластеры с одним узлом. Если вы создаете кластеры с несколькими узлами, вы должны проверить **CIDR(ADDRESS COUNT)** для подсети, которая будет использоваться с кластером. Количество адресов должен быть больше, чем количество рабочих узлов плюс семь (шлюзов: 2, головной узел: 2, узел Zookeeper: 3. Например, если вам нужен кластер HBase на 10 узлов, количество адресов для подсети должно быть больше 17 (10 + 7). В противном случае подготовка завершится с ошибкой.
    >
    > Настоятельно рекомендуется назначать одну подсеть для одного кластера.

10. В нижней части страницы щелкните **Сохранить**.

## Создание кластера HDInsight Storm

1.  Войдите в портал управления [портал управления Azure][azureportal]

2.  Щелкните **HDInsight** слева, затем **+NEW** в левом нижнем углу страницы.
...
5.  В качестве значения количества **Узлов данных**, используемых в этом кластере, введите 1. В качестве параметра **Регион/Виртуальная сеть** выберите созданную ранее виртуальную сеть Azure. В качестве параметра**Подсеть виртуальной сети** выберите **Subnet-2**.

    > [WACOM.NOTE] Для уменьшения стоимости кластера уменьшите **Размер кластера** до 1 и удалите кластер после использования.

6.  Введите **Имя пользователя** администратора и **Пароль**, затем выберите стрелку, чтобы продолжить.

7.  Для **Учетной записи хранения** выберите **Создать новое хранилище** или выберите существующую учетную запись хранения. Выберите или введите **Имя учетной записи** и **Контейнер по умолчанию**. Щелкните значок галочки слева внизу для создания кластера Storm.
...
5.  В качестве значения количества **Узлов данных**, используемых в этом кластере, введите 1. В качестве параметра **Регион/Виртуальная сеть** выберите созданную ранее виртуальную сеть Azure. В качестве параметра **Подсеть виртуальной сети** выберите **Subnet-1**.
...
7.  Для **Учетной записи хранения** выберите **Создать новое хранилище** или выберите существующую учетную запись хранения. Выберите или введите **Имя учетной записи** и **Контейнер по умолчанию**. Щелкните значок галочки слева внизу для создания кластера Storm.
    ...
    > [WACOM.NOTE] Вам следует использовать контейнер, который не занят под кластер Storm.
...
## Разработка топол огии Storm

> [WACOM.NOTE] действия, указанные в этом разделе, должны выполняться в вашей локальной среде разработки.
...
-   "Воронка" — компонент, читающий сообщения концентратора Event Hub.

-   SDK Java-клиента SignalR

#### Загрузка и сборка "воронки" (spout) концентратора Event Hub

Для получения данных с концентратора Event Hub мы воспользуемся **eventhubs-storm-spout**.

1.  Используйте удаленный рабочий стол для подключения к вашему кластеру Storm, затем скопируйте файл **%STORM\_HOME%\\examples\\eventhubspout\\eventhubs-storm-spout-0.9-jar-with-dependencies.jar** в вашу локальную среду разработки. Он содержит пакет **events-storm-spout**.
...
Для отправки сообщений в панель мониторинга ASP.NET Dashboard воспользуйтесь [SDK клиента SignalR для Java][SDK клиента SignalR для Java].
...
4.  Измените каталог на **java-client\\signalr-client-sdk** и скомпилируйте проект в файл JAR, выполнив следующие команды.
    ...
    > [WACOM.NOTE] Если вы получаете сообщение о том, что зависимость **gson** не может быть загружена, удалите следующие строки из файла **java-client\\signalr-client-sdk\\pom.xml**.
    >
    > ``` <repositories>
    > <repository>
    > <id>central</id>
...
3.  Используйте следующие команды Maven для создания основных шаблонов для ваших приложений.
    ...
    -   Создаст новый каталог, используя указанный *artifactId*. В этом случае **Температура**.
    -   Создаст файл **pom.xml**, который содержит информацию Maven для проекта
    -   Создаст структуру каталога **src**, которая содержит некоторые основные коды и тесты.
...
1.  С помощью текстового редактора откройте файл **pom.xml** и добавьте следующее в раздел **\<dependencies\>**. Вы можете добавить их в конце раздела — после зависимости для junit.
    ...
    -   eventhubs-storm-spout — spout-компонент концентратора Event Hub
    ...
    > [WACOM.NOTE] Заметьте, что некоторые зависимости отмечены областью **предоставлены** для указания на то, что их следует загрузить с репозитория Maven и использовать для построения и тестирования вашего приложения на локальном ресурсе, но что они будут также доступны в вашей среде выполнения, их не нужно компилировать и включать в JAR, созданный этим проектом.
...
2.  В конце файла **pom.xml** непосредственно перед записью **\</project\>** добавьте следующее.
    ...
    -   Включить файл ресурсов **/conf/Config.properties**. Этот файл будет создан позже, но будет содержать данные о настройках для подключения к концентратору Event Hub в Azure.
    -   Включить файл ресурсов **/conf/hbase-site.xml**. Этот файл будет создан позже, но будет содержать информацию о том, как подключиться к HBase.
    -   Использовать **maven-compiler-plugin** для компилирования приложения.
    -   Использовать **maven-shade-plugin** для создания uberjar или fat jar, который содержит проект и все необходимые зависимости.
    -   Использовать **exec-maven-plugin**, что позволяет запускать приложение локально без кластера Hadoop.

### Добавление файлов конфигурации

**eventhubs-storm-spout** считывает информацию из файла **Config.properties**. В нем содержатся сведения о том, к чему подключать концентратор Event Hub.

1.  В каталоге **Температура** создайте новый каталог с именем **conf**.
...
3.  Измените содержимое файла **Config.properties** следующим образом.
    ...
    Замените **пароль** ключом для политики **storm**, созданным ранее на концентраторе Event Hub. Замените **пространство имен** на пространство имен вашего концентратора Event Hub.
...
5.  В файле **hbase-site.xml** замените значение **суффикса** данных zookeeper DNS-суффиксом, полученным вами ранее для HBase. Например, **zookeeper0.mycluster.b4.internal.cloudapp.net, zookeeper1.mycluster.b4.internal.cloudapp.net, zookeeper2.mycluster.b4.internal.cloudapp.net**.
...
1.  В каталоге **\\temperaturemonitor\\src\\main\\java\\com\\microsoft\\examples** создайте новый каталог под именем **helpers**.
...
1.  В каталоге **\\temperaturemonitor\\src\\main\\java\\com\\microsoft\\examples** создайте новый каталог под именем **bolts**.

2.  В каталоге **bolts** создайте два новых файла:

    -   **ParserBolt.java** — разбивает входящее сообщение концентратора Event Hub на отдельные поля, затем формирует два потока
...
4.  Вставьте в файл **DashboardBolt.java**следующее содержимое.
    ...
    Замените `http://yourwebsiteaddress` адресом веб-сайта Azure, опубликованном вами в панели мониторинга ранее. Например, <http://mydashboard.azurewebsites.net>.
...
1.  В каталоге **\\temperaturemonitor\\src\\main\\java\\com\\microsoft\\examples** создайте новый каталог под именем **Temperature.java**.
...
Чтобы запустить топологию температуры в вашем кластере HDInsight Storm, в вашей среде разработки выполните следующие действия.
...
2.  На вашем локальном компьютере запустите приложение **SendEvents** .NET, чтобы дать нам возможность прочитать какие-либо события.

3.  Подключитесь к вашему кластеру HDInsight Storm с помощью удаленного рабочего стола и скопируйте файл **TemperatureMonitor-1.0-SNAPSHOT.jar** в каталог **c:\\apps\\dist\\storm\<version number\>**.

4.  Используйте значок **HDInsight Command Line** на рабочем столе, чтобы открыть новую командную строку, а также запустите топологию с помощью следующих команд.
...
-   Дополнительные сведения по Apache Storm см. по адресу [][3]<https://storm.incubator.apache.org/></a>

-   Дополнительные сведения об HBase и HDInsight см. в разделе [Обзор HBase с HDInsight][Обзор HBase с HDInsight]

-   Дополнительные сведения о SignalR см. в разделе [ASP.NET SignalR][ASP.NET SignalR]

-   Дополнительные сведения о D3.js см. в разделе [D3.js — документы, управляемые данными][D3.js — документы, управляемые данными]

-   Информацию о создании топологии в .NET см. разделе [Разработка приложений для обработки потоковой передачи данных на SCP.NET и C# в Storm и HDInsight][Разработка приложений для обработки потоковой передачи данных на SCP.NET и C# в Storm и HDInsight]

  []: https://github.com/Blackmist/hdinsight-eventhub-example
  [Microsoft Azure SDK for .NET]: http://azure.microsoft.com/ru-ru/downloads/archive-net-downloads/
  ...
  [SDK клиента SignalR для Java]: https://github.com/SignalR/java-client
  [3]: https://storm.incubator.apache.org/
  [Обзор HBase с HDInsight]: http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-hbase-overview/
  [ASP.NET SignalR]: http://signalr.net/
  [D3.js — документы, управляемые данными]: http://d3js.org/
  [Разработка приложений для обработки потоковой передачи данных на SCP.NET и C# в Storm и HDInsight]: /ru-ru/documentation/articles/hdinsight-hadoop-storm-scpdotnet-csharp-develop-streaming-data-processing-application/
