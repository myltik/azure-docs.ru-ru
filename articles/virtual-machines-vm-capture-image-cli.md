<properties
	pageTitle="Запись образа виртуальной машины под управлением Linux с помощью CLI"
...
Azure воспринимает этот шаблон как образ и отображает его в списке образов. В этой папке также хранятся все переданные вами образы. Дополнительную информацию об образах см. в разделе [Образы виртуальных машин в Azure][].
...
3. Чтобы записать образ виртуальной машины, выполните следующую команду:
    ...
    Для записи образа виртуальную машину нужно удалить. Используйте для этого параметр --deleted или -t.
...
4. Чтобы убедиться, что образ записан, проверьте список образов виртуальных машин:
    ...
    Фрагмент `| grep <target-image-name>` является необязательным, но он упрощает поиск нужного образа в списке образов.
...
Дополнительную информацию и дополнительные команды см. на [Страница документации по интерфейсу командной строки Azure][].


[Страница документации по интерфейсу командной строки Azure]: ../virtual-machines-command-line-tools/
