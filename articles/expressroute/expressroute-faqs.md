...
* С помощью [фильтров маршрутов](#route-filters-for-microsoft-peering) можно получить доступ к тем же общедоступным службам с использованием пиринга Майкрософт:
  ...
  * Большинство служб Azure, с несколькими исключениями:
    * CDN
    * Нагрузочное тестирование Visual Studio Team Services
    * Многофакторная идентификация
    * Диспетчер трафика
...
Высокого уровня доступности можно достичь, подключив к виртуальной сети несколько каналов ExpressRoute в разных местах пиринга. Например, если один сайт ExpressRoute выйдет из строя, подключение перейдет на другой сайт ExpressRoute. По умолчанию трафик, исходящий из виртуальной сети, маршрутизируется по принципу выбор маршрута в зависимости от стоимости (ECMP). Для предпочтения одного подключения перед другим можно использовать вес подключения. Дополнительные сведения о весе подключения см. в статье [Оптимизация маршрутизации ExpressRoute](expressroute-optimize-routing.md).
...
Да. ExpressRoute авторизации может занимать подписки, клиента и регистрация границы без дополнительной настройки. 
