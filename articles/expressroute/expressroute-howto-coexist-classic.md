...
- **Требуется статическая маршрутизация**. Если локальная сеть подключена и к сети ExpressRoute, и к VPN "сеть — сеть", необходимо использовать статический маршрут, настроенный в локальной сети для маршрутизации VPN-подключения типа "сеть — сеть" к Интернету.
- **Шлюз ExpressRoute необходимо настроить в первую очередь.** Шлюз ExpressRoute необходимо создать перед добавлением VPN-шлюза "сеть — сеть".
...
6. На этом этапе вы будете иметь виртуальную сеть без шлюзов. Чтобы создать новые шлюзы и завершить подключение, перейдите к [Шагу 4. Создание шлюза ExpressRoute](#gw) из предыдущего раздела.
