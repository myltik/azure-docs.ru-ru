...
6. Далее создайте VPN-шлюз к сети типа "сеть-сеть". Дополнительные сведения о настройке VPN-шлюза см. в статье [Настройка подключения между виртуальными сетями с помощью Azure Resource Manager и PowerShell](../vpn-gateway/vpn-gateway-vnet-vnet-rm-ps.md). Параметр GatewaySKU может иметь значение *Standard* или *HighPerformance*. Параметр VpnType должен иметь значение *RouteBased*.
