...
Майкрософт будет использовать AS 12076 для общедоступного и частного пиринга Azure, а также пиринга Майкрософт. Номера AS с 65515 по 65520 зарезервированы для внутреннего использования. Поддерживаются 16- и 32-битные номера AS. На стороне пиринга (клиента или поставщика) номер AS может быть открытым ASN, если возможно подтвердить, что он принадлежит вам, или закрытым номером ASN для частного пиринга. Для общедоступного пиринга и пиринга Майкрософт требуется открытый ASN.

Передача данных на основном и дополнительном пути данного канала не обязательно должна быть симметричной. Прямые и обратные пути могут проходить по разным парам маршрутов. Со стороны основного и дополнительного пути любых принадлежащих вам пар каналов должны быть объявлены идентичные маршруты. Метрики маршрута могут не совпадать.
...
Если количество префиксов превысит лимит, сеанс BGP будет сброшен. Маршруты по умолчанию принимаются только по связи частного пиринга. Клиент или заказчик должен отфильтровывать стандартный маршрут и частные IP-адреса (RFC 1918) от объявлений путей BGP в общедоступный пиринг Azure и пиринг Майкрософт.
...
## Поддержка сообществ BGP (предварительная версия)
...
>[AZURE.NOTE] Майкрософт не учитывает установленные вами значения сообществ BGP в маршрутах, объявленных для Майкрософт.
