...
Описание сеансов маршрутизации, которые необходимо настроить для установки подключения, см. в статье о [каналах и доменах маршрутизации](expressroute-circuit-peerings.md).
...
Для настройки пиринга можно использовать либо частные, либо общедоступные IP-адреса. Диапазон адресов, используемых для настройки маршрутов, не должен пересекаться с диапазонами адресов, используемых для создания виртуальных сетей в Azure.
 ...
 - [Соглашение об уровне обслуживания и доступности](http://azure.microsoft.com/support/legal/sla/) будет действовать, если настроены оба сеанса BGP.  
...
Допустим, вы выбрали для настройки частного пиринга подсеть 192.168.100.128/29. Подсеть 192.168.100.128/29 включает адреса с 192.168.100.128 по 192.168.100.133 причем:
...
Обмен маршрутами осуществляется по протоколу eBGP. Сеансы EBGP устанавливаются между MSEEs и вашими маршрутизаторами. Проверка подлинности сеансов BGP не требуется. При необходимости можно настроить MD5-хэш. Сведения о настройке сеансов BGP см. в рабочем процессе настройки маршрутизации.
...
ExpressRoute нельзя настроить как транзитный маршрутизатор. За услугами транзитной передачи данных обращайтесь к поставщику услуг подключения.
...
|---|---|---|
| **США** |	Восток США | 12076:3004 |
| | Восток США 2 | 12076:3005 |
| | Запад США | 12076:3006 |
| | Центральный регион США | 12076:3009 |
| | Северо-центральный регион США | 12076:3007 |
| | Южно-центральный регион США | 12076:3008 |
| **Северная Америка** | Южная часть Бразилии | 12076:3014 |
| **Европа** | Северная Европа | 12076:3003 |
| | Западная Европа | 12076:3002 |
| **Азиатско-Тихоокеанский регион** | Восточная Азия | 12076:3010 |
| | Юго-Восточная Азия | 12076:3011 |
| **Япония** | Восточная часть Японии | 12076:3012 |
| | Западная часть Японии | 12076:3013 |
| **Австралия** | Восточная часть Австралии | 12076:3015 |
| | Юго-Восточная часть Австралии | 12076:3016 |
| **Индия** | Южная Индия | 12076:3019 |
| | Западная Индия | 12076:3018 |
| | Центральная Индия | 12076:3017 |
