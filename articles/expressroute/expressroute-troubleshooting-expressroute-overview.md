---
title: "Руководство по устранению неполадок Azure ExpressRoute. Проверка подключения | Документация Майкрософт"
...
Подключение ExpressRoute, расширяющее локальную сеть в Microsoft Cloud посредством выделенного частного подключения, которое обеспечивает поставщик услуг подключения, включает в себя следующие три различные сетевые зоны:
...
3.  Пограничные маршрутизаторы (коммутаторы) поставщика услуг связи, подключенные к клиентским пограничным маршрутизаторам (PE, подключенные к CE).
4.  Пограничные маршрутизаторы (коммутаторы) поставщика услуг связи, подключенные к MSEE.
...
>[!NOTE]
>Для обеспечения высокой доступности ExpressRoute корпорации Майкрософт необходима избыточная пара сеансов BGP между маршрутизаторами MSEE (5) и маршрутизаторами поставщика услуг связи, подключенными к MSEE (MSEE-PR) (4). Также рекомендуется избыточная пара сетевых путей между клиентской сетью и маршрутизаторами MSEE-PR. Однако в модели подключения типа "любой к любому" (IPVPN) одно устройство CE (2) может быть подключено к одному или нескольким маршрутизаторам PE (3).
>
>
...
<p/>
>[!NOTE]Если поставщик услуг связи предлагает услуги уровня 3 и сеансы пиринга на портале пусты, можно использовать PowerShell для просмотра параметров настройки поставщика услуг..
>
>
...
>[!NOTE]
>Если пиринг не включен, проверьте, соответствуют ли назначенные первичные и вторичные подсети конфигурации маршрутизаторов MSEE-PR. Если это не так, см. сведения об изменении настроек маршрутизаторов MSEE в статье [Создание и изменение маршрутизации для канала ExpressRoute][CreatePeering]
>
>
...
Пример ответа в случае успешной настройки частного пиринга:
 ...
 Если контекст пиринга включен, в списке отобразятся первичные и вторичные префиксы адресов. Подсети /30 используются для IP-адреса интерфейса маршрутизаторов MSEE и MSEE-PR.
...
<p/>
>[!NOTE]Если пиринг не включен, проверьте, соответствуют ли назначенные первичные и вторичные подсети конфигурации связанных маршрутизаторов MSEE-PR. Также проверьте, используются ли в маршрутизаторах MSEE правильные значения t*VlandId*, *AzureASN*,d *PeerASN* и соответствуют ли они тем, которые используются в связанных маршрутизаторах MSEE-PR. Если вы выбрали хэширование MD5, общий ключ должен быть одинаковым для пары маршрутизаторов MSEE и MSEE-PR. Дополнительные сведения об изменении настроек маршрутизаторов MSEE см. в статье "Создание и изменение маршрутизации для канала ExpressRoute".  
>
>
...
Если контекст пиринга включен, в списке отобразятся первичные и вторичные подсети. Подсети /30 используются для IP-адреса интерфейса маршрутизаторов MSEE и MSEE-PR.
...
<p/>
>[!NOTE]Если пиринг не включен, проверьте, соответствуют ли назначенные первичные и вторичные подсети конфигурации связанных маршрутизаторов MSEE-PR. Также проверьте, используются ли в маршрутизаторах MSEE правильные значения t*VlandId*, *AzureASn*,d *PeerASn* и соответствуют ли они тем, которые используются в связанных маршрутизаторах MSEE-PR. Дополнительные сведения об изменении настроек маршрутизаторов MSEE см. в статье "Создание и изменение маршрутизации для канала ExpressRoute".
>
>
...
>[!NOTE]
>Если состояние имеет значение Active (Активно) или Idle (Простаивает), проверьте, соответствуют ли назначенные первичные и вторичные подсети конфигурации связанных маршрутизаторов MSEE-PR. Также проверьте, используются ли в маршрутизаторах MSEE правильные значения **VlandI*, *AzureAsn*, *PeerAsn* и соответствуют ли они тем, которые используются в связанных маршрутизаторах MSEE-PR. Если вы выбрали хэширование MD5, общий ключ должен быть одинаковым для пары маршрутизаторов MSEE и MSEE-PR. Дополнительные сведения об изменении настроек маршрутизаторов MSEE см. в статье [Создание и изменение маршрутизации для канала ExpressRoute][CreatePeering].
>
>

<p/>
>[!NOTE]Если определенные получатели недоступны через определенный пиринг, проверьте таблицу маршрутов для маршрутизаторов MSEE, принадлежащих данному контексту пиринга. Если соответствующий префикс (может быть преобразованный через NAT IP-адрес) присутствует в таблице маршрутизации, проверьте, разрешают ли трафик существующие на пути брандмауэры, ACL или NSG.
>
>
