...
<p/>
>[!NOTE] Если поставщик услуг связи предлагает услуги уровня 3 и сеансы пиринга на портале пусты, то можно использовать PowerShell для просмотра параметров настройки поставщика услуг.
>>
>
...
<p/>
>[!NOTE] Если пиринг не включен, то проверьте, соответствуют ли назначенные первичные и вторичные подсети конфигурации связанных маршрутизаторов MSEE-PR. Также проверьте, используются ли в маршрутизаторах MSEE правильные значения *VlandId*, *AzureASN* и *PeerASN* и соответствуют ли они тем, которые используются в связанных маршрутизаторах MSEE-PR. Если вы выбрали хэширование MD5, общий ключ должен быть одинаковым для пары маршрутизаторов MSEE и MSEE-PR. Дополнительные сведения об изменении настроек маршрутизаторов MSEE см. в статье [Создание и изменение маршрутизации для канала ExpressRoute][CreatePeering].  
>>
>
...
<p/>
>[!NOTE] Если определенные получатели недоступны через определенный пиринг, то проверьте таблицу маршрутов для маршрутизаторов MSEE, принадлежащих данному контексту пиринга. Если соответствующий префикс (может быть преобразованный через NAT IP-адрес) присутствует в таблице маршрутизации, то проверьте, разрешают ли трафик существующие на пути брандмауэры, группы безопасности сети и списки управления доступом.
>>
>
