...
Сети VPN типа ExpressRoute и "сеть-сеть" можно подключать к одной и той же виртуальной сети. Существует два различных сценария, в которых может потребоваться использование сосуществующих подключений. В сценарии 1 ExpressRoute используется в качестве активного канала, а подключение типа "сеть-сеть" — как резервное подключение. В сценарии 2 оба подключения активны.

Процедуры в этой статье относятся к виртуальным сетям, использующим классическую модель развертывания. Такой тип сосуществования подключений еще недоступен для виртуальных сетей, использующих модель развертывания диспетчера ресурсов.



### Сценарий 1

В этом сценарии имеется одна локальная сеть. ExpressRoute является активным каналом, а VPN типа "сеть-сеть" — резервным. В случае сбоя подключения ExpressRoute активным становится VPN-подключение типа "сеть-сеть". Этот сценарий лучше всего подходит для случаев, когда нужно обеспечить высокий уровень доступности.

![Существуют одновременно](media/expressroute-coexist/scenario1.jpg)



### Сценарий 2

В этом сценарии имеется две локальных сети. Одна из них может подключаться к Azure с помощью ExpressRoute, а другая — через VPN типа "сеть-сеть". В этом случае оба соединения активны одновременно.

![Существуют одновременно](media/expressroute-coexist/scenario2.jpg)


## Выбор процедуры настройки
...
- **Создайте новую виртуальную сеть и подключения, которые сосуществуют:**
	...
	Если у вас еще нет виртуальной сети, то, выполнив эту процедуру, вы сможете создать новую виртуальную сеть и новые подключения ExpressRoute и VPN типа "сеть-сеть". Для настройки выполните действия, необходимые для [создания новой виртуальной сети с сосуществованием подключений](#create-a-new-vnet-with-coexisting-connections).

- **Настройка существующей виртуальной сети для сосуществующих подключений:**

	Возможно, у вас уже имеется виртуальная сеть, а также подключения к VPN типа "сеть-сеть" или к ExpressRoute. Выполнив процедуру [настройки сосуществующих подключений для существующей виртуальной сети](#configure-coexisting-connections-for-an-existing-vnet), вы сможете удалить шлюз, а затем создать новое подключение ExpressRoute и VPN типа "сеть-сеть". Обратите внимание, что при создании новых подключений, необходимо выполнять действия в строго определенном порядке. При создании шлюзов и подключений не пользуйтесь инструкциями, взятыми из других статей.
...
В этой процедуре подробно рассматривается создание виртуальной сети, а также создание сосуществующих подключений ExpressRoute и VPN типа "сеть-сеть".
...
2. Создайте схему для виртуальной сети. Дополнительные сведения о работе с файлом конфигурации сети см. в разделе [Создание виртуальной сети с помощью файла конфигурации сети](../virtual-network/virtual-networks-create-vnet-classic-portal.md#how-to-create-a-vnet-using-a-network-config-file-in-the-azure-portal). Дополнительные сведения о схеме конфигурации см. в разделе [Схема конфигурации виртуальной сети Azure](https://msdn.microsoft.com/library/azure/jj157100.aspx).

	При создании схемы, убедитесь, что используются следующие значения.

	- Длина префикса сети подсети шлюза для виртуальной сети не должна быть больше 27 (/26, /25 и т. д.).
