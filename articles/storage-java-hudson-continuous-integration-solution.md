...
-   [Использование службы больших двоичных объектов с решением непрерывной интеграции Hudson][]
-   [Установка подключаемого модуля службы хранилища Azure][]
-   [Настройка подключаемого модуля службы хранилища Azure для использования своей учетной записи хранения][]
...
## <a id="benefits"></a>Преимущества использования службы больших двоичных объектов
...
- Решение непрерывной интеграции Hudson.
    ...
    3. Откройте в браузере сайт http://localhost:8080/. При этом открывается панель мониторинга Hudson.

    4. При первом использовании Hudson выполните начальную настройку по адресу http://localhost:8080/. 

    5. После завершения процесса начальной настройки отмените запущенный экземпляр Hudson WAR, снова запустите WAR-файл Hudson, после чего откройте панель мониторинга Hudson по адресу http://localhost:8080/, которая будет использоваться для установки и настройки подключаемого модуля службы хранилища Azure.
...
## <a id="howtouse"></a>Использование службы больших двоичных объектов с решением непрерывной интеграции Hudson
...
3. В разделе **Настройка учетной записи хранения Microsoft Azure**:
    ...
    5. [Необязательно] Если у вас есть дополнительные учетные записи хранения, которые требуется сделать доступными для решения Hudson CI, щелкните **Добавить дополнительные учетные записи хранения**.
...
2. Назовите задание **MyJob**, щелкните **Build a free-style software job** (Сборка программного задания в свободном стиле) и нажмите кнопку **OK**..
...
5. В разделе конфигурации задания **Post-build Actions** (Действия после сборки) щелкните **Upload artifacts to Microsoft Azure Blob storage** (Передать артефакты в хранилище больших двоичных объектов Azure).
...
## <a id="components"></a>Компоненты, используемые службой больших двоичных объектов
...
- **Форматы URL-адреса**: Для адресации BLOB-объектов используется следующий формат URL-адресов:
  ...
  [Использование службы больших двоичных объектов с решением непрерывной интеграции Hudson]: #howtouse
