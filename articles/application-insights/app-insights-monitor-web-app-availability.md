...
Развернув веб-приложение на любом узле, вы можете настроить веб-тесты для наблюдения за его доступностью и скоростью реагирования. [Visual Studio Application Insights](app-insights-overview.md) отправляет веб-запросы через одинаковые промежутки времени из разных точек по всему миру, предупреждая вас, если приложение реагирует медленно или не реагирует вообще.
...
- **Запросы, зависимые от синтаксического анализа**: изображения, сценарии, файлы стилей и другие ресурсы страницы запрашиваются как часть теста. Тест завершается ошибкой, если эти ресурсы не удается загрузить в течение времени ожидания, актуального для всего теста.
...
- **Критерии успешного завершения**:
    ...
    **Совпадение содержимого**: строка, например «Добро пожаловать!». Проверим, содержится ли она в каждом ответе. Это должна быть строка обычного текста без подстановочных знаков. Не забывайте, что если контент страницы изменяется, необходимо обновить эту строку.
...
- **Оповещения** по умолчанию отправляются в случае неудачных попыток в трех расположениях на протяжении 5 минут. Сбой в одном расположении, вероятно, будет вызван проблемой с сетью, а не с сайтом. Но для настройки чувствительности это пороговое значение можно изменить; можно также изменить адресатов, которые получат сообщения по электронной почте.
    ...
    Можно настроить [веб-перехватчик](../azure-portal/insights-webhooks-alerts.md), вызываемый при возникновении предупреждения. (Обратите внимание, что в настоящее время параметры запроса не передаются как свойства.)
...
Во время тестирования веб-страницы запрашиваются изображения, таблицы стилей и сценарии, а также другие статические компоненты.
...
*Кажется, что все работает правильно, но выдается отчет об ошибке?* Проверьте все изображения, сценарии, таблицы стилей и любые другие файлы, загружаемые на страницу. Если не удается загрузить какой-либо компонент, отчет о тесте выдаст ошибку, даже если главная HTML-страница загружается правильно.
...
* Проверка подлинности SAML. Используйте подключаемый модуль SAML, доступный для веб-тестов.
* Секрет клиента. Если в приложении предусмотрен маршрут входа с использованием секрета клиента, используйте этот маршрут. В Azure Active Directory доступен вход в систему с использованием секрета клиента.
...
В колонке "Обзор" выберите элементы **Параметры**, а затем — **Тесты производительности**. При создании теста вам будет предложено подключиться к учетной записи служб Visual Studio Team Services или создать ее.
...
* Настройте [веб-перехватчик](../azure-portal/insights-webhooks-alerts.md), вызываемый при возникновении оповещения.
...
* *Мне нужно использовать тесты доступности на нашем внутреннем сервере, который работает за брандмауэром.*
    ...
    Разрешите в брандмауэре запросы с [IP-адресов агентов веб-тестирования](app-insights-ip-addresses.md#availability).
