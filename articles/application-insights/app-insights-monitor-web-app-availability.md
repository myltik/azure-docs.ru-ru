...
## Вход с помощью OAuth

Если пользователи входят в приложение с помощью пароля OAuth (например, Microsoft, Google или Facebook), вы можете имитировать вход в контексте многоэтапного веб-теста с помощью подключаемого модуля SAML.

![Пример веб-теста для OAuth](./media/app-insights-monitor-web-app-availability/81.png)

Пример теста выполняет следующие действия.

1. Запрашивает у тестируемого веб-приложения адрес конечной точки OAuth.
2. Выполняет вход с помощью подключаемого модуля SAML.
3. Завершает тест в состоянии выполненного входа.

Подключаемый модуль SAML задает переменную `Assert`, которая используется на шаге 2.
