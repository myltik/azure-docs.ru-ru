...
В колонке обзора выберите **Параметры**, а затем — **Тесты производительности**. При создании теста вам будет предложено подключиться к учетной записи служб Visual Studio Team Services или создать ее.

По завершении теста на экране отобразится время отклика и показатели успешных попыток.
...
* Настройте объект [webhook](../azure-portal/insights-webhooks-alerts.md), вызываемый при возникновении оповещения.
...
* *Мне нужно использовать тесты доступности на нашем внутреннем сервере, который работает за брандмауэром.*
    ...
    Разрешите в брандмауэре запросы с [IP-адресов веб-агентов тестирования](app-insights-ip-addresses.md#availability).
