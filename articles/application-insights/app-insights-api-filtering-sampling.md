...
* Также доступна функция [выборки с фиксированной частотой](app-insights-sampling.md). В этом случае вы сами можете задать процент выборки. Модуль доступен для кода веб-приложения ASP.NET и веб-страниц JavaScript. Благодаря тому, что клиент и сервер будут синхронизировать свои выборки, при поиске вы сможете перемещаться между связанными представлениями страниц и запросами.
* Выборка приема действует по мере поступления телеметрии на портал Application Insights, поэтому ее можно использовать независимо от того, какой пакет SDK вы применяете. При этом сетевой трафик телеметрии не уменьшается, однако уменьшается объем данных, обрабатываемых и сохраняемых в Application Insights. В месячной квоте учитываются только сохраненные данные телеметрии. 

### Включение выборки приема

На панели "Параметры" откройте колонку "Quotas and Pricing" (Квоты и цены). Щелкните "Выборка" и выберите коэффициент выборки.

### Включение адаптивной выборки

**Обновите пакеты NuGet** до последней *предварительной* версии Application Insights. Для этого в обозревателе решений щелкните ваш проект правой кнопкой мыши, выберите "Управление пакетами NuGet", установите флажок **Включить предварительный выпуск** и выполните поиск Microsoft.ApplicationInsights.Web.

В файле [ApplicationInsights.config](app-insights-configuration-with-applicationinsights-config.md) можно настроить максимальную частоту телеметрии, к которой будет стремиться адаптивный алгоритм:
