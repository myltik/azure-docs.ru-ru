...
* *Aggregation* — вызов [статистической функции](app-analytics-aggregations.md), например `count()` или `avg()`, с именами столбцов в качестве аргументов. См. [список статистических функций](app-analytics-aggregations.md).
...
* **Используйте простые сравнения** между именами столбцов и константами. (Так как речь идет о константах, содержащихся в таблице, можно использовать функции `now()` и `ago()`, а также скалярные значения, назначенные с помощью [инструкции `let`](app-analytics-syntax.md#let-statements).)
