...
`Count` является одним из многих [операторов запросов](app-analytics-queries.md), с помощью которых можно организовать фильтрацию, изменение формы и объединение данных в канале в несколько этапов.
	
## Take: показать n строк
...
Воспользуйтесь `project`, чтобы выбрать только нужные столбцы:
...
## Расширение: вычисление столбцов

Для добавления новых столбцов к существующим используйте `extend`:
...
Если вы хотите сохранить все существующие столбцы, можете воспользоваться `extend`. Это менее подробный вариант, чем `project`.
...
Например, давайте создадим список HTTP-запросов, для которых возникают эти исключения. Еще раз взглянув на выборку из таблицы исключений, вы заметите, что пути запросов HTTP записываются в столбец с именем `operation_Name`.
...
У нас есть доступ к трем таблицам: метрики, исключения и события. `event` содержит отчеты о запросах, просмотры страниц, пользовательские события и т. д.

Чтобы найти исключения, связанные с запросом, который завершился неудачно, можно объединить таблицы на `session_Id`:
