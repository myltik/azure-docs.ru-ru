...
## 5\. Добавление отслеживания зависимостей и счетчиков производительности
...
Этот шаг также включает [отчеты о счетчиках производительности](app-insights-web-monitor-performance.md#system-performance-counters), например для ЦП, памяти и сетевой загрузки.
...
(Монитор состояний можно также использовать для [инструментирования уже запущенного приложения](app-insights-monitor-performance-live-website-now.md), даже если оно было создано без использования пакета SDK.)
...
(Это расширение подходит только для приложения, созданного с использованием пакета SDK и опубликованного в Azure. В отличие от монитора состояния, оно не позволяет оснастить инструментальными средствами уже существующее приложение.)

#### Если это проект облачной службы Azure

[Добавление скриптов в веб-роли и рабочие роли](app-insights-cloudservices.md)

## 6\. Добавление наблюдения на стороне клиента

Вы установили пакет SDK, который отправляет данные телеметрии с серверной стороны вашего приложения. Теперь можно добавить мониторинг на стороне клиента. Он обеспечит вас данными по пользователям, сеансам, просмотрам страниц, а также исключениям или сбоям, возникшим на стороне клиента.
...
Вам потребуется [Visual Studio 2013](http://go.microsoft.com/fwlink/?linkid=397827&clcid=0x409) (с обновлением 3 или более поздняя версия) и учетная запись в [Microsoft Azure](http://azure.com).

#### Добавление в новый проект

При создании нового проекта в Visual Studio установите флажок **Добавление Application Insights**.
...
В обозревателе решений щелкните проект правой кнопкой мыши и выберите **Добавление Application Insights**.
