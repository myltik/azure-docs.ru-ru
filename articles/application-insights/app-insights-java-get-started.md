...
* Ключ Application Insight можно динамически передать с портала Azure в качестве свойства системы (-DAPPLICATION\_INSIGHTS\_IKEY=your\_ikey). Если свойство не указано, оно проверяет Azure Appsetting на наличие переменной среды (APPLICATION\_INSIGHTS\_IKEY). Если оба свойства не определены, по умолчанию используется InstrumentationKey из ApplicationInsights.xml. Это позволяет динамически управлять разными ключами InstrumentationKey для разных сред.
