...
* Используйте [выборку](app-insights-sampling.md). Эта технология уменьшает скорость передачи данных, не искажая значения метрик и не влияя на возможность перехода между связанными элементами в поиске. Начиная с версии 2.0.0-beta3 пакета SDK для ASP.NET адаптивная выборка включена по умолчанию.
