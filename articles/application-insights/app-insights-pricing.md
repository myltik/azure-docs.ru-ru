...
* В рамках плана "Базовый" доступен [непрерывный экспорт](app-insights-export-telemetry.md). Эта услуга повышает стоимость каждого гигабайта данных, но до начала марта 2017 года она предоставляется бесплатно.
...
* **Ежедневное ограничение.** Максимальное ограничение — 500 ГБ в день. Ограничение по умолчанию при создании ресурса Application Insights из Visual Studio невелико (всего 32,3 МБ в день). При создании ресурса Application Insights на портале Azure устанавливается максимальное значение ограничения. Соблюдайте осторожность при изменении ограничения, так как достижение его максимального значения приведет к потере данных за остаток дня. Чтобы изменить его, используйте колонку "Ограничение ежедневного объема", доступную в колонке "Управление данными".
...
* Наконец, можно снизить ограничение ежедневного объема. Это ограничит объем собираемых данных, но приведет к потере данных за остаток дня. Чтобы изменить этот параметр, откройте **Компоненты и расценки** > **Управление данными**.
