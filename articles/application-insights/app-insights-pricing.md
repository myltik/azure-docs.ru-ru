...
* Квота измеряется в *точках данных.* Одна точка данных представляет собой вызов одного из методов отслеживания явно в коде или в одном из модулей стандартной телеметрии. 
...
* Скорость передачи данных (по умолчанию) снижается [адаптивной выборкой](app-insights-sampling). Это означает, что по мере более активного использования приложения скорость телеметрии не увеличится настолько, насколько этого можно ожидать.
...
Диаграмма показывает объем данных, поступающих в службу Application Insights после [выборки](app-insights-sampling).
...
*Как сократить объем данных, отправляемых моим приложением?*

* Используйте [выборку](app-insights-sampling.md). Эта технология уменьшает скорость передачи данных, не искажая значения метрик и не влияя на возможность перехода между связанными элементами в поиске. Адаптивная выборка включена по умолчанию начиная с версии 2.0.0-beta3 пакета SDK для Application Insights для ASP.NET.
* [Отключите сборщики телеметрии](app-insights-configuration-with-applicationinsights-config.md), которыми не пользуетесь.
