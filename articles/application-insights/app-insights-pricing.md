...
Если объем данных достигает месячной квоты, на диаграмме отображается аннотация.
...
[Выборка](app-insights-sampling.md) позволяет уменьшить скорость отправки данных телеметрии в ваше приложение. Она оставляет возможность поиска связанных событий во время поиска по журналу диагностики и сохраняет правильные значения числа событий.
...
Чтобы настроить выборку приема, задайте управление в колонке "Quotas+Pricing" (Квоты и цены).
...
> [AZURE.WARNING] На плитке "Сохраненные примеры" отображается только то значение, которое вы задали для выборки приема. Это значение не обозначает частоту выборки, которая действует в пакете SDK в вашем приложении.
> 
...
1.	Максимальное количество уникальных имен метрик — 200, а уникальных имен свойств — 200. Метрики включают данные, отправленные в TrackMetric, а также измерения по другим типам данных, таким как события. [Имена метрик и свойств][api] являются глобальными для каждого ключа инструментирования.
2.	[Свойства][apiproperties] можно использовать для фильтрации и группирования, только если на каждое свойство приходится менее 100 уникальных значений. Если количество уникальных значений превышает 100, свойство по-прежнему можно искать, но его больше нельзя использовать для фильтрации или группирования.
...
Если приложение превышает эти пределы, рекомендуем разделить данные между разными ключами инструментирования, т. е. [создать ресурсы Application Insights](app-insights-create-new-resource.md) и отправить часть данных в новые ключи инструментирования. В этом случае результаты, возможно, будут более структурированными. Разные метрики на одном экране можно просматривать с помощью [панелей мониторинга](app-insights-dashboards.md#dashboards), поэтому такой подход не ограничивает возможности сравнения нескольких метрик.
