...
Помимо месячной квоты имеются ограничения по скорости передачи данных. Для [уровня][pricing] Free это ограничение составляет среднее значение в 200 точек данных в секунду за пятиминутный период, а для платных уровней оно составляет 500 точек данных в секунду с вычислением среднего значения поминутно.
...
* [Исключения](app-insights-api-custom-events-metrics.md#track-exception) ограничены значением "50 точек в секунду".
* Все другие данные телеметрии (просмотры страниц, сеансы, запросы, зависимости, метрики, пользовательские события).

Если ваше приложение отправляет больше данных, чем это предусмотрено ограничением, некоторый объем данных пропускается. Вы увидите предупреждение о том, что это произошло.
...
* Агрегируйте метрики заранее. Если в ваше приложение включены вызовы в адрес TrackMetric, вы можете снизить объем трафика путем перегрузки, которая будет получать ваши вычисления среднего значения и стандартного отклонения замеров пакетов. Либо вы можете использовать [пакет предварительной статистической обработки](https://www.myget.org/gallery/applicationinsights-sdk-labs). 
