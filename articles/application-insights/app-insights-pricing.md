...
* Скорость передачи данных может (по умолчанию) снижаться [адаптивной выборкой](app-insights-sampling). Это означает, что по мере более активного использования приложения скорость телеметрии не увеличится настолько, насколько этого можно ожидать.
