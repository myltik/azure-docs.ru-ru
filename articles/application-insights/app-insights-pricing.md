...
Application Insights позволяет легко понять ценообразования доступные планы и какие затраты, скорее всего, будет основываться на последние шаблоны использования. Откройте колонку **Компоненты и расценки** в ресурсе Application Insights на портале Azure:
...
Можно использовать ежедневный лимит в том, чтобы ограничить объем собираемых данных, но если выполнено конец, это приведет к потере все данные телеметрии, отправленные из приложения в течение дня. Мы **не рекомендуем**, чтобы приложение достигало ограничения ежедневного объема, так как после этого вы не сможете отслеживать работоспособность и производительность приложения. 

Вместо этого используйте [выборки](app-insights-sampling.md) для настройки тома данных на уровень будет, например и использовать ежедневный лимит только в крайнем «» в случае, если отправка много больших объемах данные телеметрии неожиданно запуска приложения. 

Чтобы изменить ежедневный лимит, в разделе Настройка ресурса Application Insights, нажмите кнопку **управление томами данных** затем **ежедневный лимит**.
