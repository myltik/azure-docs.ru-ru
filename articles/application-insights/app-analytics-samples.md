<properties 
	pageTitle="Примеры запросов в аналитике Application Insights" 
	description="Примеры запросов в аналитике Application Insights, мощном средстве поиска для Application Insights." 
...
# Выборки в аналитике Application Insights

[Аналитика Application Insights](app-analytics.md) — это мощная поисковая система для вашей телеметрии [Application Insights](app-insights-overview.md). На этих страницах описан AIQL, язык запросов аналитики Application Insights. Также существует [обзор языка](app-analytics-tour.md), с которого удобно начать работу.
...
Затем выполним группировку по времени начала и IP-адресу, чтобы получить группу для каждого сеанса. Мы должны указать функцию `bin` для параметра StartTime. Если этого не сделать, аналитика AI автоматически использует группы длительностью 1 час, что приведет к сопоставлению некоторых событий начала с неправильными событиями окончания.
