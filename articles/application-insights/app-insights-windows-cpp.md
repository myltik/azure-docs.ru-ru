<properties pageTitle="Application Insights для приложений C++" description="Анализ данных использования и производительности приложения C++ с помощью Application Insights." services="application-insights" documentationCenter="cpp" authors="crystk" manager="douge""/>
...
* Универсальное приложение для Windows 10
...
3. В параметрах проектов для выпуска и отладки:
  ...
  - добавьте $(SolutionDir)packages\\ApplicationInsights.1.0.0-Beta\\lib\\native<PLATFORM TYPE>\\release\\AppInsights\_Win10-UAP в свойства проекта, выбрав элементы «Каталоги VC++» -> «Каталоги библиотек».
...
5. Добавьте файл AppInsights\_Win10-UAP.dll из $(SolutionDir)packages\\ApplicationInsights.1.0.0-Beta\\lib\\native<PLATFORM TYPE>\\release\\AppInsights\_Win10-UAP. Перейдите к свойствам и установите значение «ДА» для содержимого. Таким образом, DLL-файл будет скопирован в каталог сборки.


#### Обновление до будущих версий пакета SDK

После выпуска нового [пакета SDK](https://github.com/Microsoft/ApplicationInsights-CPP/releases):
...
2. В App.xaml.cpp:
  ...
  - После создания корневого кадра (обычно в конце App::OnLaunched) инициализируйте m\_session:
