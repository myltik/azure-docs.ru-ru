<properties 
	pageTitle="Устранение неполадок, а также вопросы и ответы по Application Insights" 
...
# Устранение неполадок, а также вопросы и ответы по Application Insights для ASP.NET
...
+ Убедитесь в том, что установлена версия [Visual Studio 2013 с обновлением 3 или более поздняя версия](http://go.microsoft.com/fwlink/?LinkId=397827). В состав этой версии входят предварительно установленные Средства Application Insights.
+ Несмотря на то, что эти средства не поддерживают все типы приложений, вы, возможно, сможете добавить пакет SDK для Application Insights в проект вручную. Используйте [эту процедуру][windows]. 
...
+ (Только для новых проектов. Если [добавить Application Insights в существующий проект][start], то придется это делать вручную.) Вставляет фрагменты кода в код клиента и сервера для их инициализации с идентификатором ресурса Application Insights. Например, в приложении MVC код вставляется в файл главной страницы Views/Shared/_Layout.cshtml.
...
При появлении ошибки выше после обновления пакетов NuGet до версии 0.17 или более поздней необходимо изменить PROJ-файл и удалить неиспользуемые целевые объекты BCL.
...
+ Нажмите кнопку «Обновить». В данной версии содержимое модулей автоматически не обновляется.
...
Если же вы хотите запускать веб-тесты для вашей службы, она должна быть доступна из Интернета на порту 80.
...
* Проверьте [блог о состояниях](http://blogs.msdn.com/b/applicationinsights-status/).
* Возможно, вы достигли месячной квоты точек данных. Чтобы выяснить это, последовательно выберите «Параметры», «Квота и расценки». Если вы достигли квоты, вы можете изменить свой тарифный план или заплатить за дополнительную емкость. См. [схему ценообразования](http://azure.microsoft.com/pricing/details/application-insights/).
...
#### <a name="q06"></a>На начальном экране предварительной версии Microsoft Azure состояние моего приложения показано на этой карте?
...
<table border="1">
<tr><th>Что вы должны увидеть</th><th>Как получить необходимое</th><th>Для чего это необходимо</th></tr>
<tr><td>Диаграммы доступности</td><td><a href="../app-insights-monitor-web-app-availability/">Веб-тесты</a></td><td>Осведомленность о рабочем состоянии веб-приложения</td></tr>
<tr><td>Производительность сервера приложений: время ответа и т.&#160;д.
</td><td><a href="../app-insights-start-monitoring-app-health-usage/">Добавьте Application Insights в ваш проект</a><br/>или <br/><a href="../app-insights-monitor-performance-live-website-now/">Установите монитор состояний Application Insights на сервере</a></td><td>Выявление проблем производительности</td></tr>
<tr><td>Телеметрия зависимостей</td><td><a href="../app-insights-monitor-performance-live-website-now/">Установите монитор состояний Application Insights на сервере</a></td><td>Диагностика проблем с базами данных или другими внешними компонентами</td></tr>
<tr><td>Получение данных трассировки стека из исключений</td><td><a href="../app-insights-search-diagnostic-logs/#exceptions">Вставьте вызовы TrackException в код</a> (некоторые выводятся автоматически)</td><td>Обнаружение и диагностика исключений</td></tr>
<tr><td>Поиск по трассировкам журнала</td><td><a href="../app-insights-search-diagnostic-logs/">Добавьте адаптер ведения журнала</a></td><td>Диагностика исключений, проблем производительности</td></tr>
...
<tr><td>Пользовательские метрики клиента</td><td><a href="../app-insights-web-track-usage-custom-events-metrics/">Отслеживание вызовов на веб-страницах</a></td><td>Расширение возможностей для пользователя</td></tr>
<tr><td>Пользовательские метрики сервера</td><td><a href="../app-insights-web-track-usage-custom-events-metrics/">Отслеживание вызовов в коде сервера</a></td><td>бизнес-аналитика</td></tr>
</table>
