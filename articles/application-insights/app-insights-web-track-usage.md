...
* **Данные о пользователях и сеансах** — предоставляются по умолчанию.  
...
Данные об использовании веб-приложения поступают из клиентского браузера.

#### Настройка ресурса Application Insights 

Ресурс Application Insights — это место в Microsoft Azure, где анализируются и отображаются данные телеметрии приложения. Возможно, он уже настроен для отображения данных со стороны сервера вашего приложения в [ASP.NET][greenbrown] или [J2EE][java]. В противном случае, может потребоваться сделать это сейчас.

Обычно лучше отображать данные об использовании из веб-клиента в том же ресурсе, что и данные сервера. Таким образом можно легко сопоставлять данные диагностики и метрики из обоих источников. Итак, при наличии ресурса перейдите к следующему шагу.

Если же вы хотите использовать отдельный ресурс для данных об использовании, просто войдите в [портал Azure][portal] и создайте его:

![](./media/app-insights-web-track-usage/01-create.png)

#### Вставка кода в веб-страницы

В своем ресурсе в [портале Azure][portal] , откройте вкладку «Быстрый запуск» (Quick Start), чтобы получить фрагмент кода для отслеживания веб-страниц.

![](./media/app-insights-web-track-usage/02-monitor-web-page.png)

Поместите код на главной странице, например в _Layout.cshtml (в .NET), или во включаемом файле, чтобы убедиться, что он включен во все страницы.

Фрагмент кода включает ключ инструментирования (iKey), который определяет требуемый ресурс. Для отправки данных на другой ресурс — например, во время тестирования — достаточно заменить iKey.

Опубликуйте веб-страницы или запустите их в режиме отладки, чтобы сформировать данные телеметрии.
...
### Какие страницы больше всего читают?

Щелкните диаграмму просмотра страниц, чтобы увидеть более подробные данные.
...
![В колонке обзора щелкните диаграмму "Сеансы"](./media/app-insights-web-track-usage/02-sessions.png)
 
(Этот пример взят с веб-сайта, но диаграмма выглядит схожим образом для приложений, работающих на устройствах.)

### Это аналогично прошлой неделе?

Сравните с данными за предыдущую неделю, чтобы узнать, что изменилось:

![Выберите диаграмму, показывающую одну метрику, и переключитесь на предыдущую неделю](./media/app-insights-web-track-usage/021-prior.png)


### Какая часть моих пользователей является новыми пользователями?
...
По умолчанию пользователь идентифицируется посредством размещения файла cookie. В таком случае пользователь, использующий несколько браузеров или устройств, будет учтен более одного раза.
...
Метрика**новые пользователи** показывает количество пользователей, которые начали свой первый сеанс работы в приложении в данный интервал времени. При использовании метода учета пользователей с помощью файлов cookie (метод по умолчанию) будут также учитываться пользователи, удалившие файлы cookie или использующие новое устройство или браузер для доступа к приложению.
...
Используйте настраиваемые события. Их можно отправлять из приложений устройств, веб-страниц или веб-сервера:
...
![В колонке обзора прокрутите вниз и щелкните "Пользовательские события".](./media/app-insights-web-track-usage/04-events.png)
