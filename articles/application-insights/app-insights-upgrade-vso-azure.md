...
6. Веб-страницы. Если проект содержит веб-страницы, замените сценарии в <head> разделах. Обычно существует только одна копия на главной странице, например Views\\Shared\_Layout.cshtml. [Получите новый скрипт из колонки "Быстрый запуск" в ресурсе Application Insights в Azure][usage]. Если веб-страницы содержат вызовы телеметрии, например методы logEvent или logPage, [обновите их для использования нового API][track].
...
10. Счетчики производительности. При использовании счетчиков производительности можно написать собственный код для периодической выборки данных счетчиков и передачи их с помощью [TrackMetric()][track].
...
3. На портале Microsoft Azure [добавьте новый ресурс Application Insights][java]. На компьютере разработки добавьте [пакет Java SDK][java] для веб-проекта. Теперь можно [передавать настраиваемые данные телеметрии][track] из серверного кода.
