...
* Ссылки, которые помогут при диагностике проблемы:
  ...
  * Результаты поиска этих вызовов для просмотра свойств вызовов.
...
Application Insights определяет проблемы производительности, которые влияют только на часть пользователей или влияют на пользователей лишь в некоторых случаях. Например, уведомления о загрузке страниц медленнее на один тип браузера, чем для других типов браузеров, или если запросы обслуживаются медленнее из конкретного сервера. Можно также выявлять проблемы, связанные с сочетаниями свойств. Например, можно обнаружить медленную загрузку страниц в одном географическом регионе для клиентов в определенной операционной системе.  
