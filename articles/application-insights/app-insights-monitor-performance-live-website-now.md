...
* **Во время выполнения.** Используйте монитор состояний для инструментирования веб-приложений на сервере.
...
* **Во время выполнения и во время сборки.** Выполните компиляцию пакета SDK в коде вашего веб-приложения и запустите монитор состояния на веб-сервере. Вы получаете следующие преимущества обоих способов.
...
4. В мастере установки выполните вход в Microsoft Azure.
    ...
    ![Войдите в Azure с учетными данными учетной записи Майкрософт.](./media/app-insights-monitor-performance-live-website-now/appinsights-035-signin.png)

5. Выберите установленное веб-приложение или веб-сайт для наблюдения и настройте ресурс, где вы будете просматривать результаты на портале Application Insights.

    ![Выберите приложение и ресурс.](./media/app-insights-monitor-performance-live-website-now/appinsights-036-configAIC.png)

    Как правило, следует настроить новый ресурс и [группу ресурсов][roles].
...
6. Обратите внимание, что в веб-сайты, за которыми планируется наблюдать, вставляется файл ApplicationInsights.config.
    ...
    ![Найдите файл .config наряду с файлами кода веб-приложения.](./media/app-insights-monitor-performance-live-website-now/appinsights-034-aiconfig.png)
...
Можно подробно рассмотреть конкретные исключения (за последние семь дней) и получить данные трассировки стека и контекста.


### Отсутствие данных телеметрии

  * С помощью своего сайта создайте данные.
  * Подождите несколько минут, пока поступят данные, а затем нажмите кнопку «Обновить».
  * Откройте поиск по журналу диагностики, щелкнув плитку «Поиск», чтобы просмотреть отдельные события. События часто отображаются в колонке поиска по журналу диагностики до появления объединенных данных на диаграмме.
  * Откройте монитор состояний и выберите приложение в области слева. Просмотрите в разделе «Уведомления о конфигурации», нет ли сообщений об ошибках для этого приложения:
  
  ![](./media/app-insights-monitor-performance-live-website-now/appinsights-status-monitor-diagnostics-message.png)

  * Убедитесь, что брандмауэр сервера обеспечивает исходящий трафик на порту 443 по адресу dc.services.visualstudio.com. 
  ...
  * На панели управления компьютера добавьте это значение для группы пользователей монитора производительности.
...
Поддерживаются такие версии IIS: 7, 7.5, 8, 8.5 (обязательный компонент).
...
* [Добавьте пакет SDK для Application Insights в код веб-службы][greenbrown], чтобы иметь возможность вставлять вызовы трассировки и журналов в серверном коде.
