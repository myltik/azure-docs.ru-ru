...
* Все другие данные телеметрии (просмотры страниц, запросы, зависимости, метрики, пользовательские события).
...
*Точка данных* — это элемент телеметрии, например:

* вызовы API `Track...`, например `TrackEvent` или `trackPageView`;
* элементы телеметрии, отправленные модулями SDK, например для отчета о запросе или аварийном завершении;
* данные счетчиков производительности — одна точка для каждого измерения.
...
Доступность | Код ответа веб-теста, длительность каждого шага теста
