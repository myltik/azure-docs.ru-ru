...
* *Column:* необязательное имя для столбца результатов. По умолчанию это имя, получаемое из выражения.
* *Aggregation:* вызов статистической функции, такой как `count()` или `avg()`, с именами столбцов в качестве аргументов. Список статистических функций см. ниже.
* *GroupExpression:* выражение для столбцов, предоставляющее набор уникальных значений. Обычно это либо имя столбца, которое уже предоставляет ограниченный набор значений, или `bin()` с числовым или временным столбцом в качестве аргумента. 

Если указать числовое или временное выражение без `bin()`, аналитика AI автоматически применяет его с интервалом `1h` для временных значений или `1.0` для числовых значений.

Если не указать *GroupExpression*, то вся таблица будет суммирована в одну результирующую строку.
...
*Accuracy*, если параметр указан, он управляет балансом между скоростью и точностью.
...
Противоположную функцию см. в [операторе `mvexpand`](app-analytics-queries.md#mvexpand-operator).
