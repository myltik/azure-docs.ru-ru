...
В этой статье показано, как переместить данные телеметрии из [Visual Studio Application Insights][start] в базу данных SQL Azure, используя [непрерывный экспорт][export] и небольшой код.
...
В Azure получите строку подключения из своей учетной записи хранения:
...
Замените существующий метод выполнения и выберите нужный интервал. Интервал должен быть длительностью по крайней мере в один час, так как функция экспорта экспортирует один объект JSON за час.
...
> [AZURE.NOTE]Не нужно выполнять этот сценарий. Атрибуты в JSON-документе определяют столбцы в таблице.
