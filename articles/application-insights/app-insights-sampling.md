...
* **Адаптивная выборка** автоматически корректирует процент выборки для достижения определенного объема запросов. Она включена по умолчанию в пакете SDK Application Insights для ASP.NET 2.0.0-beta3 или более поздней версии. 
* В пакетах SDK 2 и более поздних версий также доступна функция **выборки с фиксированной частотой**. В этом случае вы сами можете задать процент выборки. Эта возможность доступна как на клиенте JavaScript, так и на сервере ASP.NET. Благодаря тому, что клиент и сервер будут синхронизировать свои выборки, при поиске вы сможете перемещаться между связанными представлениями страниц и запросами.
* **Выборка приема**. Выполняется в службе Application Insights. Пакет SDK отправляет все данные телеметрии из браузера и сервера, однако мы сохраняем только процентные значения. Эта выборка адаптивна и работает только в том случае, если приложение отправляет больше минимального объема данных телеметрии и пакет SDK не выполняет один из других типов выборки. Она также зависит от используемой ценовой категории. Несмотря на то, что эта выборка не уменьшает трафик, она помогает оставаться в пределах месячной квоты.
...
Адаптивная выборка включается автоматически, если используется пакет SDK для ASP.NET 2.0.0-beta3 или более поздней версии. Независимо от того, какая версия SDK используется, выборка приема (на нашем сервере) может работать, даже если выборка не выполняется в пакете SDK.
...
Все виды выборки работают только при превышении определенного порога трафика. Например, выборка не влияет на большинство сеансов отладки или редко используемые сайты.
...
В противном случае мы рекомендуем использовать адаптивную выборку. Она включена по умолчанию в SDK сервера ASP.NET.

Если вы не используете выборку в пакете SDK (например, если вы используете Java или отключили выборку в файле ApplicationInsights.config), выборка приема может работать по мере поступления данных в службу Application Insights. Это вариант адаптивной выборки.
 ...
 * Да. Настройте адаптивную выборку в коде, и вы сможете использовать обратный вызов, который возвращает частоту выборки. При использовании непрерывного экспорта частота выборки отображается в экспортированных точках данных.
