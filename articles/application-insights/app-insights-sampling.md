---
title: Выборка данных телеметрии в Application Insights | Microsoft Docs
...
Выборка — это функция [Visual Studio Application Insights](app-insights-overview.md), которая рекомендуется для сокращения трафика телеметрии и хранения. При этом она поддерживает статистически правильный анализ данных приложений. Фильтр выбирает связанные элементы, чтобы вы могли переходить между ними при диагностике. Отображаемые на портале счетчики метрик перенормированы с учетом выборки, чтобы их влияние на статистику было минимальным.
