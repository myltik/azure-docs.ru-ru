...
При [настройке веб-страниц для Application Insights](app-insights-javascript.md) измените фрагмент, загруженный с портала Application Insights. (В приложениях ASP.NET фрагменты обычно содержатся в \_Layout.cshtml.) Вставьте строку, похожую на `samplingPercentage: 10,`, перед ключом инструментирования:
...
([Сведения о процессорах телеметрии](app-insights-api-filtering-sampling/#filtering).)
...
* Вам нужна синхронизированная выборка между клиентом и сервером, которая позволяет вам перемещаться между связанными событиями на стороне клиента и сервера (такими, как просмотры страниц и HTTP-запросы) при использовании [службы поиска](app-insights-diagnostic-search.md) в ходе анализа событий.
 ...
 * В текущей версии это сделать нельзя.
 ...
 * Создайте отдельный экземпляр TelemetryClient с отдельным свойством TelemetryConfiguration. Используйте его для отправки редких событий.
