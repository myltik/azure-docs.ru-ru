---
title: "Обнаружение, рассмотрение, диагностика"
...
---
# <a name="detect-triage-and-diagnose-with-application-insights"></a>Обнаружение, рассмотрение и диагностика с помощью Application Insights


[Application Insights](app-insights-overview.md) — это расширяемая служба управления производительностью приложений (APM) для веб-разработчиков. Она помогает получить сведения о производительности и использовании активного приложения. А в случае возникновения проблемы эта служба уведомляет вас о ней, помогает оценить влияние и определить причину.
...
Давайте посмотрим, как обычная команда веб-разработчиков использует Application Insights для управления производительностью. Мы будем наблюдать за действиями команды, разрабатывающей систему интернет-банкинга в банке Fabrikam.

![Типовой веб-сайт банка](./media/app-insights-detect-triage-diagnose/03-bank.png)

Цикл DevOps команды примерно следующий.

![Цикл разработки и операций](./media/app-insights-detect-triage-diagnose/00-devcycle.png)

Требования поступают в невыполненную работу по разработке (список задач). Команда работает в коротких спринтах, результатом которых, как правило, является работающее программное обеспечение — обычно в виде улучшений и расширений для существующего приложения. В динамическое приложение часто добавляются новые функции. Поскольку приложение динамическое, команда отслеживает его производительность и использование с помощью Application Insights. Данные APM поступают в невыполненную работу по разработке.
...
## <a name="monitor-performance-metrics"></a>Мониторинг метрик производительности
...
Поэтому она задает [оповещения](app-insights-metrics-explorer.md) о времени отклика, превышающем обычное пороговое значение. Это гарантирует, что она узнает, если ответы будут выполняться медленно.
...
## <a name="smart-detection-alerts"></a>Оповещения интеллектуального обнаружения
На следующий день по электронной почте поступает оповещение из Application Insights. Но когда она его открывает, то находит не оповещение о времени ответа, которое настроила. Вместо этого ей сообщается о неожиданном увеличении числа невыполненных запросов — то есть запросов, которые вернули код ошибки 500 или более.

Невыполненные запросы — это запросы, после выполнения которых пользователи видят сообщение об ошибке, обычно сопровождаемое исключением, порожденным кодом. Возможно, пользователи увидят сообщение "К сожалению, сейчас невозможно обновить ваши данные" или, в худшем случае, по вине веб-сервера на экране отобразится дамп стека.
...
Она связывается с руководителем команды разработчиков для баз данных. Да, они выпустили исправление полчаса назад. И, возможно, немного изменили схему…
...
## <a name="detecting-exceptions"></a>Определение исключений
...
## <a name="monitoring-user-activity"></a>Отслеживание активности пользователей
...
## <a name="dealing-with-it"></a>Устранение проблемы
