<properties 
	pageTitle="Операторы и запросы аналитики в Application Insights" 
	description="Справочник операторов, используемых при создании запросов в аналитике, мощном инструменте поиска Application Insights." 
...
Существует неявное ограничение на количество строк, возвращаемых клиенту, даже если вы не используете оператор `take`. Чтобы снять это ограничение, используйте параметр `notruncation` клиентского запроса.
...
Тип развернутого столбца всегда динамический. Используйте такое приведение, как `todatetime()` или `toint()`, чтобы выполнить обычное или статистическое вычисление значений.
...
Аргументы должны иметь числовые значения, значения даты или временного диапазона. Они не могут ссылаться на столбцы таблицы. (Если вы хотите вычислить диапазон на основе входной таблицы, используйте [*функцию* range](app-insights-analytics-scalars.md#range), возможно, с [оператором mvexpand](#mvexpand-operator).)
...
Пример того, как оператор `range` может использоваться для создания небольшой ad-hoc-таблицы измерений, которая затем применяется для замены нулями значений, отсутствующих в исходных данных.
...
[Другие примеры](app-insights-analytics-aggregations.md).
...
* *Aggregation* — вызов [статистической функции](app-insights-analytics-aggregations.md), например `count()` или `avg()`, с именами столбцов в качестве аргументов. См. [список статистических функций](app-insights-analytics-aggregations.md).
...
Выражения `top 5 by name` внешне эквивалентны `sort by name | take 5`. Однако первое работает быстрее и в отличие от `take` всегда возвращает отсортированные результаты.
...
* *Table1*, *Table2*…
 *  Имя таблицы, например `events`, или
...
* *Predicate* — `boolean` [выражение](app-insights-analytics-scalars.md#boolean) для столбцов *T*. Вычисляется для каждой строки в *T*.

**Результаты**

Строки в *T*, для которых *Predicate* имеет значение `true`.
