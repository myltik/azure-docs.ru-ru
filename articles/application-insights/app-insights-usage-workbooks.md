...
* Сколько пользователей просматривали конкретные наборы страниц на сайте? Это может быть полезно для определения кластера пользователей, использующих различные подмножества функциональных возможностей веб-сайта (используйте оператор `join` с модификатором `kind=leftanti` на языке запросов аналитики).

Дополнительные сведения о создании запросов см. в статье [Справочные материалы по аналитике](app-insights-analytics-reference.md).
