...
4. В мастере установки выполните вход в Microsoft Azure.
    ...
    *Ошибки подключения? См. раздел [Устранение неполадок](#troubleshooting).*
...
5. Выберите установленное веб-приложение или веб-сайт для мониторинга и настройте ресурс, где вы будете просматривать результаты на портале Application Insights.
    ...
    В противном случае используйте существующий ресурс, если вы уже настроили [веб-тесты][availability] для своего сайта, или используйте [мониторинг веб-клиента][client].
...
## <a name="diagnosis"></a> Диагностика проблем производительности зависимостей
...
Можно написать код, который отправляет информацию о зависимостях, используя тот же [TrackDependency API](app-insights-api-custom-events-metrics.md#track-dependency), который используются в стандартных модулях.

Например, формируя код на основе готовой сборки, можно назначить время для всех вызовов этого кода и таким образом выяснить, как он влияет на время отклика вашей системы. Чтобы эти данные отображались в диаграммах зависимостей в Application Insights, отправляйте их с помощью `TrackDependency`.
...
Если вы хотите отключить стандартный модуль отслеживания зависимостей, удалите ссылку на DependencyTrackingTelemetryModule из [ApplicationInsights.config](app-insights-configuration-with-applicationinsights-config.md).
