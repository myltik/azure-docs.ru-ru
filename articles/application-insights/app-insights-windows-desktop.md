...
Стандартный канал в памяти теряет все данные телеметрии, которые не были отправлены до закрытия приложения. Вы можете попытаться отправить остающиеся в буфере данные с помощью `Flush()`, однако в случае отсутствия подключения к Интернету время ожидания истечет и при завершении работы приложения возникнет задержка.
