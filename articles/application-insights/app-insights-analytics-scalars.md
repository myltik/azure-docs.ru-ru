...
[Аналитика](app-insights-analytics.md) — это эффективный инструмент поиска
[Application Insights](app-insights-overview.md). 
На этих страницах описан язык запросов аналитики приложений.
...
Скалярные выражения отличаются от [запросов](app-insights-analytics-queries.md), результаты которых представляют собой таблицы.
...
Функция `iff()` вычисляет первый аргумент (предикат) и возвращает
значение второго или третьего аргументов, в зависимости от того, является
ли предикат `true` или `false`. Второй и третий аргументы должны быть одного типа.
...
Эта функция возвращает значение *ifTrue*, если *predicate* равен `true`,
или значение *ifFalse* в противном случае.
...
Округляет значения до целого, кратного указанному размеру группы. Часто используется в запросе [`summarize by`](app-insights-analytics-queries.md#summarize-operator). При наличии разрозненного набора значений они будут сгруппированы в меньший набор определенных значений.
...
Вычитает заданный интервал времени из текущего времени 
UTC. Как и `now()`, эту функцию можно использовать в выражении несколько раз, и 
значение времени UTC будет одинаковым 
для всех случаев.
...
* *a_timespan* — интервал для вычитания из текущего времени UTC
(`now()`).
...
Для образования скрываемого строкового литерала добавьте в начало символ `h` или "H". Например:
...
* *text* — строка.
* *search* — строка обычного текста или [регулярное выражение](app-insights-analytics-reference.md#regular-expressions), которое проверяется на вхождение в *text*.
...
Определяет совпадение для [регулярного выражения](app-insights-analytics-reference.md#regular-expressions) из текстовой строки. При необходимости извлеченная подстрока преобразуется к указанному типу.
...
* *regex* — [регулярное выражение](app-insights-analytics-reference.md#regular-expressions).
...
В примере строки `Trace` производится поиск определения `Duration`.
Совпадение преобразуется в `real`, а затем умножается на константу времени (`1s`), так чтобы `Duration` получил тип `timespan`. В этом примере он равен 123,45 секунды:
...
**Литералы**. Чтобы создать явный массив или объект контейнера свойств, запишите его в виде строки JSON и приведите:
...
|[`mvexpand` listColumn](app-insights-analytics-queries.md#mvexpand-operator) | Реплицирует строку для каждого значения в списке в указанной ячейке.
|[`summarize buildschema(`column`)`](app-insights-analytics-queries.md#summarize-operator) |Определяет схему типа по содержимому столбца.
|[`summarize makelist(`column`)` ](app-insights-analytics-queries.md#summarize-operator)| Выполняет сведение групп строк и помещает значения столбца в массив.
|[`summarize makeset(`column`)`](app-insights-analytics-queries.md#summarize-operator) | Выполняет сведение групп строк и помещает значения столбца в массив без дублирования значений.

### Динамические объекты в предложениях let


В [предложениях let](app-insights-analytics-queries.md#let-clause) динамические значения хранятся в виде строк, поэтому эти два предложения эквивалентны. Оба предложения требуют `parsejson` (или `todynamic`) перед использованием:
...
* По возможности используйте сопоставление с регулярным выражением с [extract](#extract). Он выполняется гораздо быстрее и эффективнее, если JSON создается из шаблона.
...
следующий фрагмент получает значение слота `duration` в объекте и из него получает два слота `duration.value` и `duration.min` (`118.0` и `110.0`, соответственно).
...
Когда [выборка](app-insights-sampling.md) выполняется в операции, лишь часть данных, создаваемых пакетом SDK, принимается в портал Application Insights. Выборку можно применить в пакете SDK или при приеме в конечную точку портала.

`itemCount` указывает, сколько событий было создано в пакете SDK для каждого события, отображаемого в потоке. Например, если текущая частота выборки составляет 25 %, то itemCount == 4.

Если необходимо подсчитать события, используйте `sum(itemCount)` вместо `count()` для точной оценки фактических значений. Например:

* Количество исключений, возникших за последние два часа:
