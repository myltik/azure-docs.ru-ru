...
*Зависимость* является службой, которую вызывает приложение, — обычно через подключение к API REST или базы данных. [Монитор состояния Application Insights][redfield] автоматически отслеживает различные типы вызова зависимостей, измеряя продолжительность вызова и регистрируя его успешное выполнение или сбой.

Для получения данных о зависимости необходимо [установить монитор состояния][redfield] на сервере IIS или, если приложение является веб-приложением Azure, используйте [расширение Application Insights][azure]. Можно воспользоваться любым из этих способов.
...
2. Замените только ExceptionHandler пользовательской реализацией IExceptionHandler. Он вызывается, только когда платформа по-прежнему может выбирать ответное сообщение для отправки (но не после прерывания подключения для экземпляра) 
