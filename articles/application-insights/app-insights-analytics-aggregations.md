...
Найти число запросов каждого имени за последний час:

    requests | where timestamp > ago(1h)
    | summarize req_count = sum(itemCount) by name

> [AZURE.NOTE] Вместо простого подсчета точек данных запроса используйте свойство itemCount. Это восполняет любую [выборку](app-insights-sampling.md), которая может быть в работе. Например, если выборка устанавливается при 33 %, itemCount будет равно 3 в каждой точке данных.

#### Функции Min и Max

Найти минимальную и максимальную метки времени всех записей в потоке запросов. Предложение group by отсутствует, поэтому в выходных данных всего одна строка:
...
Операция `extend` добавляет столбец "Продолжительность", используя функцию `bin` (также называемую `floor`) для округления продолжительности до ближайшей секунды.
...
Мы добавили столбец `client_City` в предложение `by`, чтобы он проходил через первую операцию суммирования. Если все события из одного клиентского сеанса произошли в одном и том же городе, то количество выходных строк в результате статистической обработки не увеличится.
...
Находит строку в группе, которая минимизирует или максимизирует *ExprToMaximize* и возвращает значение *ExprToReturn* (или `*` для возврата всей строки).

**Совет**. Прошедшие столбцы переименовываются автоматически. Чтобы убедиться, что вы используете правильные имена, проверьте результаты с помощью `take 5`, прежде чем передавать результаты в другой оператор.
...
Вычисляет среднее значение *Expression* в группе.
...
В качестве параметра типа столбца должно быть указано `dynamic` — массив или контейнер свойств.
...
Возвращает число строк, для которых предикат *Predicate* равен `true`. Если ни один *Predicate* не указан, возвращается общее число записей в группе.

**Совет по улучшению производительности**: используйте `summarize count(filter)` вместо `where filter | summarize count()`.

> [AZURE.NOTE] Избегайте использования count() для поиска числа запросов, исключений или других произошедших событий. Когда [выборка](app-insights-sampling.md) обрабатывается, число точек данных будет меньше, чем количество фактических событий. Вместо этого используйте `summarize sum(itemCount)...`. Свойство itemCount отражает число исходных событий, представленных каждой сохраненной точкой данных.
...
Возвращает оценку числа уникальных значений *Expr* в группе. (Чтобы получить список уникальных значений, используйте [`makeset`](#makeset).)

Параметр *Accuracy* (если указан) управляет балансом между скоростью и точностью.
...
Возвращает массив `dynamic` (JSON) всех значений *Expr* в группе.
...
Возвращает массив `dynamic` (JSON) набора различных значений, которые *Expr* принимает в группе. (Совет. Чтобы только подсчитать уникальные значения, используйте [`dcount`](#dcount).)
...
Противоположную функцию см. в статье [Оператор `mvexpand`](app-insights-analytics-queries.md#mvexpand-operator).
...
Вычисляет максимальное значение *Expr*.
    
    min(Expr)

Вычисляет минимальное значение *Expr*.

**Совет**. Эта функция дает само по себе минимальное или максимальное значение — например, наибольшую или наименьшую цену. Но если вы хотите получить в строке и другие столбцы, например имя поставщика с наименьшей ценой, используйте [argmin или argmax](#argmin-argmax).
...
Похожа на `percentile()`, но вычисляет несколько процентилей (это быстрее, чем вычислять каждый процентиль по отдельности).

**Примеры**


Значение `duration`, которое больше 95 % значений выборки и меньше 5 % значений выборки, вычисленное для каждого имени запроса:
...
Возвращает среднеквадратичное отклонение *Expr* для группы.

## variance

    variance(Expr)

Возвращает величину отклонения *Expr* для группы.

## sum

    sum(Expr)

Возвращает сумму *Expr* для группы.
