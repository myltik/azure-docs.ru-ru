...
2. Эта возможность доступна в предварительной версии, но вы можете запросить доступ к ней.

 * В ресурсе Application Insights на [портале Azure](https://portal.azure.com) откройте инструмент аналитики. 
 * В нижней части панели схемы рядом с разделом **Другие источники данных** щелкните ссылку "Обратная связь". 
 * Если кнопка "Добавить источник данных" отображается, у вас уже есть доступ.
...
1. Запустите мастер источников данных.
...
4. Выберите метку времени. Все данные в инструменте аналитики должны включать поле метки времени. Это должно быть поле даты и времени (`datetime`), но его не обязательно называть timestamp. Если данные содержат столбец, содержащий дату и время в формате ISO, выберите его в качестве столбца метки времени. Или выберите "по мере поступления данных". В этом случае процесс импорта добавит полу метки времени.
    ...
    ![Проверка схемы](./media/app-insights-analytics-import/data-source-review-schema.png)
...
1. Отправьте данные в [хранилище BLOB-объектов Azure](../storage/storage-dotnet-how-to-use-blobs.md). 
