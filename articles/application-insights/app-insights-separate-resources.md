...
В самом простом случае при создании кода для нового приложения можно также создать новый ресурс в Application Insights. В Visual Studio диалоговое окно *Новый проект* сделает это автоматически.
...
* Добавьте свойство для всей телеметрии, что позволит сегментировать и фильтровать идентификатор компонента. С экземплярами роли сервера это происходит автоматически, но в других случаях для добавления свойства можно использовать [инициализатор телеметрии](app-insights-api-filtering-sampling.md#add-properties).
