...
Visual Studio Application Insights — это расширяемая служба аналитики, которая отслеживает работающие веб-приложения. С ее помощью можно обнаруживать и диагностировать проблемы производительности, а также понять, что пользователи действительно делают с вашим приложением. Данная служба предназначена для разработчиков и помогает постоянно улучшать производительность и удобство использования. Application Insights работает с приложениями на самых разнообразных платформах, включая .NET, Node.js и J2EE, с локальным или облачным размещением.
...
![Инструментирование Application Insights в приложении отправляет данные телеметрии в ресурс Application Insights на портале Azure.](./media/app-service-app-insights-get-started/01-scheme.png)
...
![Колонка "Обзор"](./media/app-service-app-insights-get-started/01-find.png)
...
![Колонка "Серверы"](./media/app-service-app-insights-get-started/04.png)
...
![Изменение диаграммы](./media/app-service-app-insights-get-started/05.png)
...
![Поиск экземпляров](./media/app-service-app-insights-get-started/06.png)

Используйте фильтры, чтобы сосредоточиться на определенных типах событий и выбранных значениях свойств.

![Фильтрация по свойствам](./media/app-service-app-insights-get-started/07.png)
...
![Связанные элементы и сведения о свойствах](./media/app-service-app-insights-get-started/08.png)
...
![Создание рабочего элемента](./media/app-service-app-insights-get-started/09.png)
...
![Аналитика](./media/app-service-app-insights-get-started/10.png)
...
Запросы обычно начинаются с имени потока данных телеметрии, например запросов, исключений или зависимостей. Откройте панель схемы слева, чтобы просмотреть список доступных потоков телеметрии. Запрос — это конвейер [операций запроса](app-insights-analytics-reference.md#queries-and-operators). Например, `where` — логический фильтр, а `project` вычисляет новые свойства. `summarize` [агрегирует экземпляры](app-insights-analytics-tour.md#aggregate-groups-of-rows), группируя их по определяемым вами функциям, а затем применяет статистические функции к сгруппированным данным.
...
![Аналитика](./media/app-service-app-insights-get-started/11.png)
...
Чтобы отслеживать всю систему, вы можете выбрать интересующие вас диаграммы из разных приложений и закрепить их на [панели мониторинга](app-insights-dashboards.md) Azure. Это позволит непрерывно следить за всей системой.

![Панели мониторинга](./media/app-service-app-insights-get-started/12.png)
...
* Функция [аналитики](app-insights-analytics.md) предоставляет мощный язык запросов для телеметрии с возможностью экспорта результатов.
* Для поиска и извлечения данных может использоваться REST API доступа к данным. Он позволяет выполнять запросы аналитики.
* Если вы собираетесь [исследовать данные в Power BI](http://blogs.msdn.com/b/powerbi/archive/2015/11/04/explore-your-application-insights-data-with-power-bi.aspx), это можно сделать, не прибегая к непрерывному экспорту.
...
[![Анимационное ознакомительное видео](./media/app-service-app-insights-get-started/video-front-1.png)](https://www.youtube.com/watch?v=fX2NtGrh-Y0)
