...
В течение первых нескольких минут после развертывания можно определить, правильно ли работает приложение. Например, при замене старой версии вам нужно узнать, улучшилась ли производительность. Если возникнет проблема, вам, возможно, потребуется вернуться к старой версии.

Поток в реальном времени дает возможность сразу увидеть набор ключевых метрик производительности. Их можно отслеживать во время повторного развертывания или изменения конфигурации.

![В колонке обзора щелкните "Поток в реальном времени"](./media/app-insights-asp-net/45.png)

В отличие от других диаграмм с метриками поток в реальном времени показывает сведения только за последние несколько минут и не хранит данные. Накапливаемый объем данных минимален, и представление обновляется каждую секунду.

Для работы потока в реальном времени требуется пакет SDK начиная с версии 2.1.0-beta1.

*Поток в реальном времени "застыл" на странице конфигурации? Обновите страницу в браузере (F5).*
