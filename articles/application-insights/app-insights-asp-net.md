...
#### Как работает команда «Добавить Application Insights»?

Команда выполняет два действия (которые при желании можно произвести вручную).
...
* Добавляет в проект пакет SDK NuGet Application Insights и добавляет ключ в `ApplicationInsights.config`.
...
Если ваше приложение обслуживает таких клиентов, как телефоны и другие устройства, добавьте в приложение для устройств [соответствующий пакет SDK](app-insights-platforms.md).
...
[Запись трассировки журналов][netlogs] из предпочтительной платформы ведения журналов помогает диагностировать неисправности. Записи журнала будут отображаться в результатах [диагностического поиска][diagnostic] вместе с событиями телеметрии Application Insights.
...
Для основного приложения рекомендуется отправлять данные телеметрии из отладки, тестирования и эксплуатации в [отдельные ресурсы](app-insights-separate-resources.md).



## Добавление отслеживания зависимостей
...
(Монитор состояний можно также использовать для [оснащения уже запущенного приложения](app-insights-monitor-performance-live-website-now.md), даже если оно было создано без использования пакета SDK.)
