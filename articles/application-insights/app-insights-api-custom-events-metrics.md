...
Когда веб-приложение получает HTTP-запрос, модуль отслеживания запросов Application Insights назначает этому запросу идентификатор и задает значение, равное идентификатору текущей операции. Идентификатор операции очищается при отправке ответа на запрос. Любым вызовам отслеживания, получаемым во время операции, присваивается один и тот же идентификатор операции (если они используют TelemetryContext по умолчанию). Это позволяет сопоставлять события, связанные с конкретным запросом, когда вы проверяете их на портале.

![Связанные элементы](./media/app-insights-api-custom-events-metrics/21.png)

Если вы наблюдаете за событиями, не связанными с HTTP-запросом, или не используете модуль отслеживания запросов, например, при наблюдении за внутренним процессом, то можно задать собственный контекст операции, используя приведенный шаблон.
