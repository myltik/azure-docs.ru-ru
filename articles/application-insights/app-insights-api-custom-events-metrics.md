...
Обратите внимание, что эта функция является асинхронной для каналов в памяти и синхронной при использовании [постоянного канала](app-insights-windows-desktop.md#persistence-channel).
...
**Чтобы добавить свойства для всей телеметрии**, включая данные из модулей стандартной коллекции, [создайте инициализатор телеметрии](app-insights-api-filtering-sampling.md#add-properties).
...
* [Добавьте свойства](app-insights-api-filtering-sampling.md#add-properties) в телеметрию, например номера версий или значения, вычисленные на основе других свойств.
* [Выборка](app-insights-api-filtering-sampling.md#sampling) уменьшает объем данных, отправляемых из приложения на портал, не влияя на отображаемые метрики и возможность диагностировать проблемы путем перехода между связанными элементами, например исключениями, запросами и просмотрами страниц.
* [Фильтрация](app-insights-api-filtering-sampling.md#filtering) также уменьшает объем. Она позволяет управлять тем, какие данные отправляются, а какие отклоняются. Однако при этом необходимо учитывать влияние на метрики. В зависимости от способа отклонения элементов можно потерять возможность перехода между связанными элементами.
...
1. Максимальная скорость в секунду, которая применяется для каждого ключа инструментирования в отдельности. При превышении этого предела некоторые данные будут потеряны.
 * До 500 точек данных в секунду для вызовов TrackTrace и собранных данных журнала. (100 в секунду для бесплатной ценовой категории.)
 * До 50 данных точек в секунду для исключений, собранных нашими модулями или вызовами TrackException. 
 * До 500 точек данных в секунду для всех остальных данных, включая стандартную телеметрию, отправляемую модулями пакета SDK, а также пользовательские события, метрики и другие данные телеметрии, отправленные в коде. (100 в секунду для бесплатной ценовой категории.)
1. Общий объем данных за месяц в зависимости от вашей [ценовой категории](app-insights-pricing.md).
...
2.	Свойства можно использовать для фильтрации и группирования, только если на каждое свойство приходится менее 100 уникальных значений. После превышения максимального количества уникальных значений (100 уникальных значений) свойство можно по-прежнему использовать для поиска, но не для фильтрации.
...
* Установите последний пакет SDK для использования [выборки](app-insights-sampling.md).
...
* *Существует ли REST API для получения данных из портала?*
    ...
    Да, его выпуск ожидается вскоре. Тем временем рекомендуется использовать [непрерывный экспорт](app-insights-export-telemetry.md).
