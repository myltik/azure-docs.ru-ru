...
#### <a name="evaluate-autocluster-deprecated"></a>evaluate autocluster (не рекомендуется)
     T | evaluate autocluster()

С помощью функции autocluster можно быстро находить естественные группирования в набор данных. Например, из объема данных запросов может быстро определить, что 80 % ошибок с кодом 404 были запросами к одному определенному URL-адресу, отправленными клиентом в определенном городе.
...
**Эта версия `autocluster` не рекомендуется. Используйте [autocluster_v2](#evaluate-autocluster_v2).**
...
AutoCluster находит распространенные шаблоны дискретных атрибутов (измерения) в данных и сокращает результаты исходного запроса (будь то 100 или 100 тысяч строк) до небольшого числа шаблонов. Эта функция предназначена для анализа сбоев (например, исключений, поломок), но потенциально может работать с любым отфильтрованным набором данных. Алгоритм AutoCluster разработан исследовательской группой по разработке аналитических средств для разработчиков (KustoML@microsoft.com).

Этот подключаемый модуль заменяет устаревший синтаксис подключаемого модуля функции autocluster.     

**Синтаксис**
`T | evaluate autocluster_v2( arguments )`

**Результаты** AutoCluster возвращает набор шаблонов (как правило, небольшого размера), которые содержат фрагменты данных с одинаковыми значениями для нескольких дискретных атрибутов. В результатах каждый шаблон соответствует строке. Первый столбец — это идентификатор сегмента. Следующие два столбца — это число и процент строк из исходного запроса, которые охватывает шаблон. Остальные столбцы берутся из исходного запроса. Их значения соответствуют конкретным значениям в столбцах или подстановочным значениям (которыми по умолчанию являются значения NULL), представляющим значения переменных. Обратите внимание, что шаблоны не являются уникальными. Они могут перекрываться и обычно не охватывают все исходные строки. Некоторые строки могут не охватываться ни одним из шаблонов.

**Советы** Используйте аргументы `where` и `project` во входном канале, чтобы получить только требуемые данные.
Если вы хотите узнать дополнительные сведения о конкретной строке, добавьте ее значения в фильтр `where` .

**Аргументы (все необязательные)** `T | evaluate autocluster_V2([*SizeWight*,*WeightColumn*,*NumSeeds*,*CustomWildcard*,...])
...
- SizeWeight - 0<*double* <1 [default 0.5] позволяет управлять балансом между универсальностью (большой охват) и информативностью (много общих значений). При увеличении значения количество шаблонов, как правило, уменьшается, а каждый шаблон охватывает больший процент данных. Если уменьшить значение, обычно получаются более конкретные шаблоны с большим количеством общих значений и меньшим процентом охвата. Поэтому для аргумента используется скрытая формула, представляющая собой взвешенное среднее геометрическое значение между нормализованной универсальной оценкой и информативной оценкой, для которых в качестве весовых коэффициентов применяются *SizeWeight* и *1-SizeWeight*. 
...
Число начальных значений определяет число начальных локальных точек поиска для алгоритма. В некоторых случаях в зависимости от структуры данных при увеличении числа начальных значений увеличивается число (или повышается качество) результатов. Это достигается за счет увеличения области поиска при меньшей скорости выполнения запросов. Это значение сокращает количество результатов в двух направлениях. Поэтому если указать значение меньше 5, повышение производительности будет незначительным, а если указать значение больше 50, вы вряд ли получите дополнительные шаблоны.

**Пример**
`T | evaluate autocluster_v2('~','~',15)`

- CustomWildcard — *любое_значение_для_каждого_типа*

Задает подстановочный знак для определенного типа в таблице результатов, который укажет, что текущий шаблон не имеет ограничения по этому столбцу. По умолчанию имеет значение NULL, для строки по умолчанию указывается пустая строка. Если значение по умолчанию является приемлемым значением в данных, следует использовать другое подстановочное значение (например, *).
...
-  Функция AutoCluster преимущественно основана на алгоритме Seed-Expand, с которым вы можете ознакомиться в документе, посвященном [алгоритмам для интеллектуального анализа данных телеметрии с помощью дискретных атрибутов](http://www.scitepress.org/DigitalLibrary/PublicationsDetail.aspx?ID=d5kcrO+cpEU=&t=1). Полностью с темой вы можете ознакомиться здесь: [PDF-файл](https://kusto.azurewebsites.net/docs/queryLanguage/images/queries/ICPRAM17telemetry.pdf). 


#### <a name="evaluate-basket-deprecated"></a>evaluate basket (не рекомендуется)

     T | evaluate basket()

**Эта версия `basket` не рекомендуется. Следует использовать [basket_v2](#evaluate-basketv2).**
...
Заменяет нерекомендуемый синтаксис `evaluate basket`.
...
Пример: `T | evaluate basket_v2([Threshold, WeightColumn, MaxDimensions, CustomWildcard, CustomWildcard, ...])`

Все аргументы необязательны, но должны указываться в приведенном порядке. Чтобы указать, что следует использовать значение по умолчанию, добавьте знак тильды "~" (см. примеры ниже).

* Пороговое значение: 0,015 < *double* < 1 (по умолчанию: 0,05) 
  
    Задает минимальный коэффициент строк, определяющий распространенность шаблонов (шаблоны с меньшим коэффициентом не будут возвращаться).
  
    Пример: `T | evaluate basket_v2(0.02)`
* Весовой столбец: *-имя_столбца*
  
    Рассматривает каждую строку входных данных в соответствии с указанным весовым коэффициентом (по умолчанию весовой коэффициент каждой строки — 1). Аргумент должен быть именем числового столбца (например, int, long, real). Как правило, при использовании столбца с весовым коэффициентом следует учитывать выборку, группирование или агрегирование данных, внедренных в каждой строке.
  
    Пример: `T | evaluate basket_v2('~', sample_Count)`
* Максимальное число измерений: 1 < *int* (по умолчанию: 5)
  
    Задает максимальное количество некоррелированных измерений на корзину. По умолчанию они ограничены, чтобы обеспечить сокращение времени выполнения запроса.

    Пример: `T | evaluate basket_v2('~', '~', 3)`
* Пользовательские типы подстановочных знаков: *любое значение для каждого типа*
  
    Задает подстановочный знак для определенного типа в таблице результатов, который укажет, что текущий шаблон не имеет ограничения по этому столбцу. По умолчанию имеет значение NULL, для строки по умолчанию указывается пустая строка. Если значение по умолчанию является приемлемым значением в данных, следует использовать другое подстановочное значение (например, *).

    Пример: `T | evaluate basket_v2('~', '~', '~', '*', int(-1), double(-1), long(0), datetime(1900-1-1))`
...
#### <a name="evaluate-diffpatterns-deprecated"></a>evaluate diffpatterns (не рекомендуется)
**Эта версия подключаемого модуля для функции diffpatterns устарела. Используйте новый синтаксис** подключаемого модуля для функции [diffpatterns](#evaluate-diffpatterns_v2).
запросы | evaluate diffpatterns("split=success")

Функция diffpatterns определяет различия между двумя наборами данных одинаковой структуры — например, между журналом запросов во время инцидента и обычным журналом запросов. Функция Diffpatterns предназначена для анализа сбоев (например, путем сравнения сбоев и успешных результатов за заданный период времени), но потенциально может найти различия между двумя наборами данных с одинаковой структурой. 
...
Diffpatterns возвращает набор шаблонов (как правило, небольшого размера), который содержит разные фрагменты данных в двух наборах (т. е. шаблон, охватывающий большое количество строк в первом наборе данных и малое количество строк во втором наборе). В результатах каждый шаблон соответствует строке.
Первый столбец — это идентификатор сегмента. Следующие четыре столбца соответствуют количеству и проценту строк из исходного запроса, которые охватывает шаблон в каждом наборе. В шестом столбце представлена разница (в абсолютных процентах) между двумя наборами. Остальные столбцы берутся из исходного запроса.
Для каждого шаблона столбцы, которые не были заданы в шаблоне (т. е. без ограничения для определенного значения), будут содержать подстановочные значения, которыми по умолчанию являются значения NULL (в разделе "Аргументы" ниже описывается, как можно изменить их вручную).
Обратите внимание, что шаблоны не являются уникальными. Они могут перекрываться и обычно не охватывают все исходные строки. Некоторые строки могут не охватываться ни одним из шаблонов.
...
Строковое представление одного из значений в указанном аргументе SplitColumn. Все строки, которые содержат это значение в своих аргументах SplitColumn, рассматриваются, как набор данных B.
...
Задает подстановочный знак для определенного типа в таблице результатов, который укажет, что текущий шаблон не имеет ограничения по этому столбцу. По умолчанию имеет значение NULL, для строки по умолчанию указывается пустая строка. Если значение по умолчанию является приемлемым значением в данных, следует использовать другое подстановочное значение (например, *). См. следующий пример.
...
|Идентификатор сегмента|Число A|Число B|Процент A|Процент B|DiffAB|Состояние|EventType|Источник|DamageCrops
----------|------|------|--------|--------|------|-----|---------|------|-----------
...
## <a name="numeric-series"></a>Числовые ряды
...
Функция series_fir() применяет [КИХ](https://wikipedia.org/wiki/Finite_impulse_response)-фильтр к ряду (представленному динамическим столбцом, содержащим числовой массив).

Указав коэффициенты фильтра, его можно использовать для вычисления скользящего среднего, сглаживания, обнаружения изменений и многих других задач.

Функция принимает входной столбец, содержащий динамический массив, и статический массив коэффициентов фильтра и применяет фильтр к столбцу. Она выводит новый столбец динамического массива, содержащий отфильтрованные результаты. 
...
* *x:* ячейка динамического массива, которая представляет собой массив числовых значений (обычно полученный результат выполнения оператора [make-series](#make-series-operator) или [makelist](#makelist-operator)).
* *filter:* необязательное логическое значение, указывающее, должны ли быть нормализованы коэффициенты фильтра (т. е. разделены на их сумму). По умолчанию аргумент нормализации имеет значение true. Если фильтр содержит отрицательные значения, автоматическая нормализация невозможна, и для аргумента нормализации необходимо явно задать значение false.
* *normalize:* необязательное логическое значение, указывающее, следует ли нормализовать фильтр. По умолчанию аргумент нормализации имеет значение true. Если фильтр содержит отрицательные значения, то для аргумента нормализации необходимо задать значение false. 
* *center:* необязательное логическое значение, указывающее, как применяется фильтр к временному окну: симметрично до и после текущей точки или в обратном направлении, начиная с текущего момента. По умолчанию center имеет значение false, которое подходит для потоковой передачи данных, где фильтр можно применить только к текущим и более ранним точкам. Тем не менее для ad-hoc-обработки можно задать значение true, обеспечив синхронизацию с временными рядами (см. примеры ниже). Технически этот параметр определяет задержку группы фильтра.

**Примеры**

Вычисление скользящего среднего для 5 точек может выполняться с помощью параметров filter=[1,1,1,1,1] и normalize=true (по умолчанию). Обратите внимание на результат center=false (по умолчанию) по сравнению с center=true.
...
* *5h_MovingAvg:* фильтр скользящего среднего для 5 точек. Пик сглажен и его вершина смещена на (5 – 1)/2 = 2 ч.
* *5h_MovingAvg_centered:* тот же фильтр, но с параметром center=true, оставляет вершину в исходном положении.

![Результаты запроса](./media/app-insights-analytics-reference/series-fir-1.png) (Чтобы увидеть несколько строк, снимите флажок "Разделить" в элементе управления диаграммой.)

Вычисление разности между точкой и предшествующей ей точкой можно выполнять с помощью параметра filter=[1,-1].
...
![Результаты запроса](./media/app-insights-analytics-reference/series-fir-2.png)


### <a name="seriesfitline"></a>series\_fit\_line
Функция series_fit_line() принимает столбец, содержащий динамический числовой массив, в качестве входных данных и выполняет линейную регрессию для поиска наиболее соответствующей ему линии. Эту функцию следует использовать для массивов временных рядов, чтобы подбирать выходные данные оператора make-series. Она создает динамический столбец, содержащий следующие поля.

* *slope:* наклон аппроксимируемой линии (это коэффициент `a` в `y=ax+b`).
* *interception:* сдвиг аппроксимируемой линии (это коэффициент `b` в `y=ax+b`).
* *rsquare:* R-квадрат представляет собой стандартную меру качества соответствия. Это число в диапазоне [0–1], где 1 означает максимальное соответствие, а 0 означает, что данные абсолютно не упорядочены и не соответствуют какой-либо линии.
* *variance:* дисперсия входных данных.
* *rvariance:* остаточная дисперсия, представляющая собой расхождение между значениями входных данных и аппроксимированными значениями.
* *line_fit:* числовой массив, содержащий ряд значений линии с наилучшим соответствием. Длина ряда равна длине входного массива. Он главным образом используется для построения диаграмм.

Эту функцию удобнее всего применять к результатам оператора make-series.
...
* `x:` динамический массив числовых значений. Обратите внимание, что функция ожидает, что все строки содержат одинаковое число элементов массива. В противном случае возвращаются пустые результаты. 
...
![Результаты запроса](./media/app-insights-analytics-reference/series-fit-line.png)
...
Функция series_fit_2lines() применяет двухсегментную линейную регрессию к ряду (временному) для определения и количественного выражения изменения тенденции в ряду. Функция выполняет итерацию индексов ряда и в каждой итерации разделяет ряд на две части, подбирает отдельную линию (с помощью series_fit_line()) для каждой из них и вычисляет общий R-квадрат. Наилучшее разделение характеризуется максимальным значением R-квадрата. Функция возвращает следующие параметры.

* *rsquare:* R-квадрат представляет собой стандартную меру качества соответствия. Это число в диапазоне [0–1], где 1 означает максимальное соответствие, а 0 означает, что данные абсолютно не упорядочены и не соответствуют какой-либо линии.
* *split_idx:* индекс точки разделения на 2 сегмента (от нуля).
* *variance:* дисперсия входных данных.
* *rvariance:* остаточная дисперсия, представляющая собой расхождение между значениями входных данных и аппроксимированными значениями (для двух сегментов линии).
* *line_fit:* числовой массив, содержащий ряд значений линии с наилучшим соответствием. Длина ряда равна длине входного массива. Он главным образом используется для построения диаграмм.
* *right_rsquare:* R-квадрат линии справа от точки разделения (ознакомьтесь с функцией series_fit_line()).
* *right_slope:* наклон правой аппроксимируемой линии (это коэффициент a в y = ax + b).
* *right_interception:* сдвиг правой аппроксимируемой линии (это коэффициент b в y = ax + b).
* *right_variance:* дисперсия входных данных справа от точки разделения.
* *right_rvariance:* остаточная дисперсия входных данных справа от точки разделения.
* *left_rsquare:* R-квадрат линии слева от точки разделения (ознакомьтесь с функцией series_fit_line()).
* *left_slope:* наклон левой аппроксимируемой линии (это коэффициент a в y = ax + b).
* *left_interception:* сдвиг левой аппроксимируемой линии (это коэффициент b в y = ax + b).
* *left_variance:* дисперсия входных данных слева от точки разделения.
* *left_rvariance:* остаточная дисперсия входных данных слева от точки разделения.

Обратите внимание, что эта функция возвращает несколько столбцов, поэтому ее нельзя использовать в качестве аргумента другой функции.

**Синтаксис**

    project series_fit_2lines(x)

Возвращает все упомянутые выше столбцы со следующими именами: `series_fit_2lines_x_rsquare, series_fit_2lines_x_split_idx and etc.`

    project (rs, si, v)=series_fit_2lines(x)

Возвращает столбцы `rs (r-square), si (split index), v (variance)`, а также выходные данные вида `series_fit_2lines_x_rvariance, series_fit_2lines_x_line_fit` и т. д.
...
![Результаты запроса](./media/app-insights-analytics-reference/series-fit-2lines.png)

### <a name="seriesiir"></a>series_iir

Функция series_iir применяет БИХ-фильтр к ряду (представленному динамическим столбцом, содержащим числовой массив).

Указав коэффициенты фильтра, его можно использовать, например, для вычисления общей суммы ряда, применения операций сглаживания, а также в качестве различных фильтров высоких частот, полосовых фильтров и фильтров низких частот.

Функция принимает входной столбец, содержащий динамический массив, и два статических массива коэффициентов a и b фильтра и применяет фильтр к столбцу. Она выводит новый столбец динамического массива, содержащий отфильтрованные результаты. 
...
* *x:* ячейка динамического массива, которая представляет собой массив числовых значений (обычно полученный результат выполнения оператора make-series или makelist).
* *b:* константное выражение, содержащее коэффициенты числителя фильтра (хранятся в виде динамического массива числовых значений).
* *a:* константное выражение, как и b. Содержит коэффициенты знаменателя фильтра.

    Первый элемент a (т. е. a[0]) не должен быть равен нулю (чтобы избежать деления на 0; см. формулу ниже).

**Дополнительные сведения о рекурсивной формуле фильтра**

При заданном входном массиве X и массивах коэффициентов a и b длиной `n_a` и `n_b` соответственно передаточная функция фильтра, создающая выходной массив Y, определяется следующим образом (дополнительные сведения см. также в Википедии). 
...
Вычислить общую сумму можно с помощью фильтра iir с коэффициентами a = [1, -1] и b = [1]. 
...
Функция series_outliers() принимает столбец, содержащий динамический массив в качестве входных данных и создает динамический числовой массив такой же длины, как и входные данные. Каждое значение массива содержит показатель, указывающий на возможную аномалию по тесту Тьюки. Значение больше 1,5 или меньше –1,5 означает повышение или снижение аномалии (соответственно) в одном элементе входных данных.  
...
```
**Аргументы** 
* *x:* ячейка динамического массива, которая представляет собой массив числовых значений. Предполагается, что значения являются равноудаленными. В противном случае это может вызвать непредвиденные результаты.  
* *Тип:* алгоритм обнаружения аномалий. В настоящее время поддерживает аргументы tukey и ctukey. По умолчанию поддерживается аргумент ctukey.  
* *ignore_val:* числовое значение, указывающее отсутствующие значения в сериях. По умолчанию используется значение double(null).
* *min_percentile:* для расчета обычного межквартильного диапазона. По умолчанию используется значение 10 (только для ctukey).
* *max_percentile:* для расчета обычного межквартильного диапазона. По умолчанию используется значение 90 (только для ctukey).

В таблице ниже описаны различия между аргументами tukey и ctukey.
...
**Важное примечание.** Эту функцию удобнее всего применять к результатам оператора `make-series`.
...
то есть, по сравнению с остальными сериями, 5 — аномалия снижения, а 80 — аномалия увеличения. 

### <a name="seriesperiods"></a>series_periods

Функция series_periods() находит наиболее значимые периоды во временных рядах.

Например, очень часто метрика измерения трафика приложения характеризуется двумя значимыми циклами: недельным и дневным. Для таких временных рядов функция series_periods() должна обнаруживать эти 2 основных периода.

Эта функция принимает в качестве входных данных столбец, содержащий динамический массив данных временных рядов (обычно это выходные данные оператора make-series), два вещественных числа, определяющие минимальный и максимальный размер периода (т. е. количество ячеек, например, для ячейки 1h размер дневного периода будет равен 24) для поиска, и длинное число, определяющее общее количество периодов для поиска. Результатом является динамический массив, содержащий размеры обнаруженных периодов, упорядоченные по продолжительности периодов в данных.
...
* *x:* ячейка динамического массива, которая представляет собой массив числовых значений (обычно полученный результат выполнения оператора make-series или makelist).
* *min_period:* вещественное число, задающее минимальный период для поиска.
* *max_period:* вещественное число, задающее максимальный период для поиска.
* *num_periods:* длинное число, определяющее максимальное необходимое количество периодов. Это будет длина выходного динамического массива.

**Важные примечания**

* Алгоритм функции series\_periods() требует наличия по крайней мере 4 точек в периоде, чтобы он мог быть обнаружен. Поэтому минимальное значение min_period равно 4. Если для min_period задать меньшее значение, функция заменит его на 4.
* Максимальное значение max_period равно половине длины входных рядов. Если для max_period задать большее значение, функция сократит его до нужной величины.
* Как упоминалось выше, функция выдает периоды в ячейках. Например, если в первоначальном ряду использовался дневной период и были вычислены данные по часовым ячейкам, то дневной период в выходных данных будет равен 24. Если данные вычисляются по минутам, то результатом будет 60 * 24 = 1440.
* Следует задать значение min\_period немного меньше, а max\_period — немного больше, чем у ожидаемых периодов в числовом ряду. Например, если имеется часовой сигнал и нужно найти дневные и недельные периоды (которые будут равны 24 и 168 соответственно), то можно задать min\_period = 0,824 и *max\_period = 1,2*  168.
* Длина выходного динамического массива равна num\_of\_periods. Если функция нашла меньше num\_of\_periods значимых периодов, то остальная часть записей массива будет равна 0.
* Входной временной ряд должен быть регулярным, т. е. объединенным в константные ячейки (что происходит всегда, если ряд был создан с помощью оператора make-series). В противном случае выходные данные не имеют смысла.

**Пример**

Например, приведенный ниже запрос выдает моментальный снимок месяца трафика приложения, вычисляемого два раза в день (т. е. каждые 12 часов).
...
![Результаты запроса](./media/app-insights-analytics-reference/series-periods1.png)

Если с этим рядом выполнить функцию series_periods(), будет получен недельный период (из 14 точек).
...
Функция series_stats() принимает столбец, содержащий динамический числовой массив, в качестве входных данных и вычисляет следующие столбцы.

* *min:* минимальное значение во входном массиве.
* *min_idx:* максимальное значение во входном массиве.
* *max:* максимальное значение во входном массиве.
* *max_idx:* максимальное значение во входном массиве.
* *average:* среднее значение входного массива.
* *variance:* дисперсия выборки входного массива.
...
Возвращает все упомянутые выше столбцы со следующими именами: serie\_stats\_x\_min, series\_stats\_x\_min\_idx и т. д.

    project (m, mi)=series_stats(x)

Возвращает столбцы `m (min), mi (min_idx)`, а также выходные данные вида `series_stats_x_max, series_stats_x_line_max_idx` и т. д.

    extend (m, mi)=series_stats(x)

Возвращает только: m (min) и mi (min_idx).

**Аргументы**

* *x:* ячейка динамического массива, которая представляет собой массив числовых значений. 
...
Функция series_stats() возвращает:
...
> [!NOTE]
> Метки и строковые значения в JSON необходимо заключать в двойные кавычки (`"`). Поэтому обычно проще заключать закодированные JSON литералы в одинарные кавычки (`'`).
> 
...
> [!NOTE]
> Метки и строковые значения в JSON необходимо заключать в двойные кавычки. 
>
