...
Для некоторых типов диаграмм `kind` предоставляет дополнительные параметры. Например, линейчатая диаграмма `stacked` сегментирует каждую линию по выбранному измерению, показывая вклад различных значений измерения в общий показатель. На диаграмме `stacked100` все строки имеют одинаковую высоту (100 %), чтобы можно было сравнить относительный вклад.
...
Интервал времени представляет временной период, например 3 часа или год.

Дата и время представляет конкретную дату и время в формате UTC по календарю или по часам.

Отдельный тип date отсутствует. Чтобы удалить время из значения даты и времени, используйте выражение, например `bin(timestamp, 1d)`.
...
| Expression | Результат |Результат|
| --- | --- |---|
| `datetime("2015-01-02") - datetime("2015-01-01")` |`1d` | Разница во времени|
| `datetime("2015-01-01") + 1d` |`datetime("2015-01-02")` | Добавление дней |
| `datetime("2015-01-01") - 1d` |`datetime("2014-12-31")` | Вычитание дней|
| `2h * 24` |`2d` |Умножение интервала времени|
| `2d` / `2h` |`24` |Деление интервала времени|
| `datetime("2015-04-15T22:33") % 1d` |`timespan("22:33")` |Время в значении даты и времени|
| `bin(datetime("2015-04-15T22:33"), 1d)` |`datetime("2015-04-15T00:00")` |Дата в значении даты и времени|
|  | ||
| `<` ||Меньше |
| `<=` ||Меньше или равно |
| `>` ||Больше |
