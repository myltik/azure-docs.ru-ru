...
В этом примере оператор `range` создает небольшую ad-hoc таблицу измерения, на основе которой заполняет нулями все поля, для которых нет значений в исходных данных.
...
* `asc` или `desc` (по умолчанию) могут использоваться как указатели направления выбора элементов в диапазоне: снизу вверх или сверху вниз.


**Советы**

Выражение `top 5 by name` внешне эквивалентно `sort by name | take 5`. Но первое работает быстрее и в отличие от `take` всегда возвращает отсортированные результаты.
 ...
 *  Набор таблиц, указанных с помощью подстановочного знака. Например, `e*` формирует объединение всех таблиц, определенных в предыдущих предложениях let, имена которых начинаются на e, вместе с таблицей exceptions.
* `kind`: 
 * `inner` — результат содержит подмножество столбцов, которые являются общими для всех входных таблиц.
 * `outer` — результат содержит все столбцы, которые присутствуют в любых входных таблицах. Для ячеек, которые не были определены входной строкой, будет установлено значение `null`.
...
* *Predicate* — [выражение](#boolean) `boolean` для столбцов таблицы *T*. Вычисляется для каждой строки в *T*.
...
* **Вводите сначала более простые элементы**. Если у вас есть несколько предложений, объединенных с помощью `and`, сначала введите предложения, которые включают только один столбец. Это значит, что порядок `Timestamp > ago(1d) and OpId == EventId` является предпочтительным.
...
Вычисляет среднее значение выражения *Expression* в группе.
...
**Совет по улучшению производительности**: используйте `summarize countif(filter)` вместо `where filter | summarize count()`.
...
Возвращает массив `dynamic` (JSON) со всеми значениями выражения *Expr* в группе.
...
Целое число дней с предыдущего воскресенья в формате `timespan`.
...
Обратная косая черта (``) используется для экранирования знаков, таких как `\t` (табуляция), `\n` (новая строка) и кавычки.
...
Для образования скрываемого строкового литерала укажите перед строкой знак `h` или H. Например:
...
При проверке на наличие полного лексического слова (т. е. знака или буквенно-цифрового слова, окруженного не буквенно-цифровыми знаками, не началом и не концом поля) используйте `has` или `in`. `has` выполняется быстрее, чем `contains` или `startswith`. Первый из этих запросов выполняется быстрее:
...
* *text* — строка.
* *search* — текстовая строка или регулярное выражение, которое проверяется на соответствие в *text*.
...
Если *regex* находит соответствие в *text*, возвращается подстрока, которая сравнивалась с указанной в *captureGroup* группой захвата, при необходимости преобразованная в тип *typeLiteral*.

Если соответствия нет или не удается выполнить преобразование типа, возвращается `null`.

**Примеры**

В примере строки `Trace` производится поиск определения `Duration`. Соответствие преобразуется в `real`, а затем умножается на константу времени (`1s`), так чтобы `Duration` получился тип `timespan`. В этом примере он равен 123,45 секунды:
...
True, если аргумент является пустой строкой или имеет значение null. См. также сведения о функции [isnull](#isnull).
...
* *rewrite* — регулярное выражение, результат которого заменит все найденные соответствия *matchingRegex*. Используйте `\0` для указания полного соответствия, `\1` для первой группы захвата, `\2` и более — для следующих групп захвата.
* *text* — строка.
...
**Индексирование** — массивы и объекты, которые можно индексировать так же, как в JavaScript.
...
* По возможности заменяйте его на сопоставленные с регулярным выражением и [extract](#extract). Он выполняется гораздо быстрее и эффективнее, если JSON создается из шаблона.
...
Имя может содержать до 1024 знаков. Оно зависит от регистра и может содержать буквы, цифры и знаки подчеркивания (`_`).
