...
`now()`|Текущее время.
`now(`-*временной диапазон*`)`|`now()-`*timespan*
`ago(`*временной диапазон*`)`|`now()-`*временной диапазон*
**интервал времени**|
`2d`|2 дня
`1.5h`|1,5 часа 
`30m`|30 минут
...
`!hasprefix`|Правая часть не представляет собой префикс слова в левой части|Нет|`"North America" !hasprefix "mer"`
...
`!startswith`|Правая часть не является начальным вхождением в левую часть|Нет|`"Fabrikam" !startswith "abr"`
`endswith`|Правая часть является конечным вхождением в левую часть|Нет|`"Fabrikam" endswith "kam"`
`!endswith`|Правая часть не является конечным вхождением в левую часть|Нет|`"Fabrikam" !endswith "ka"`
...
При проверке на наличие полной лексической единицы (т. е. знака или буквенно-цифрового слова, окруженного не буквенно-цифровыми знаками, в начале или конце поля) используйте `has` или `in`. `has` выполняется быстрее, чем `contains`, `startswith` или `endswith`. Первый из этих запросов выполняется быстрее:
