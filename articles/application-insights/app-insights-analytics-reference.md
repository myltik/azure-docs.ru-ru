...
Все шаблоны, которые содержатся в большей части событий, чем указано (по умолчанию — 0,05). Для каждого шаблона столбцы, которые не были заданы в шаблоне (т. е. без restricition для определенного значения), будут содержать подстановочные значения, которыми по умолчанию являются значения NULL (в разделе "Аргументы" ниже описывается, как можно изменить их вручную).
...
* Весовой столбец *itemCount*
  
    Позволяет учитывать выборки и предварительную статистическую обработку метрик. Каждая строка имеет атрибут веса, указанный в этом столбце. По умолчанию вес каждой строки равен "1". Учитывается группирование или агрегирование данных, внедренных в каждый столбец.
  
    Пример: `T | evaluate basket('~', itemCount)`
