...
Можно указать одно или несколько [предложений let](#let-clause) перед запросом, чтобы определить используемые в нем скалярные значения, таблицы или функции.
...
Тип развернутого столбца всегда динамический. Чтобы использовать это значение в обычных или статистических вычислениях, используйте приведение, например `todatetime()` или `toint()`.
...
Для статистических выражений и выражений группирования допускаются произвольные выражения, но более эффективно использовать простые имена столбцов или функцию `bin()` для числовых столбцов.
...
Настраиваемый параметр *Accuracy* определяет баланс между скоростью и точностью.
...
При проверке на наличие полной лексической единицы (т. е. знака или буквенно-цифрового слова, окруженного не буквенно-цифровыми знаками, в начале или конце поля) используйте `has` или `in`. `has` выполняется быстрее, чем `contains` или `startswith`. Первый из этих запросов выполняется быстрее:
