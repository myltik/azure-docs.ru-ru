...
| | | | | 
|---|---|---|---|---
|[ago](#ago)|[dayofweek](#dayofweek)|[Предложение let](#let-clause)|[range](#range)|[Оператор summarize](#summarize-operator)
|[любой](#any)|[dcount](#dcount)|[Оператор limit](#limit-operator)|[Оператор range](#range-operator)|[Оператор take](#take-operator)
|[argmax](#argmax)|[Динамические объекты в предложениях let](#dynamic-objects-in-let-clauses)|[makelist](#makelist)|[Оператор reduce](#reduce-operator)|[todatetime](#todatetime)
|[argmin](#argmin)|[Оператор extend](#extend-operator)|[makeset](#makeset)|[Директива render](#render-directive)|[todouble](#todouble)
|[Арифметические операторы](#arithmetic-operators)|[extract](#extract)|[max](#max)|[replace](#replace)|[todynamic](#todynamic)
|[Литералы массива и объекта](#array-and-object-literals)|[extractjson](#extractjson)|[min](#min)|[Скалярные сравнения](#scalar-comparisons)|[toint](#toint)
|[arraylength](#arraylength)|[floor](#floor)|[Оператор mvexpand](#mvexpand-operator)|[Оператор sort](#sort-operator)|[tolong](#tolong)
|[avg](#avg)|[getmonth](#getmonth)|[notempty](#notempty)|[split](#split)|[tolower](#tolower)
|[bin](#bin)|[gettype](#gettype)|[notnull](#notnull)|[sqrt](#sqrt)|[Оператор top](#top-operator)
|[Логические литералы](#boolean-literals)|[getyear](#getyear)|[now](#now)|[startofmonth](#startofmonth)|[totimespan](#totimespan)
|[Логические операторы](#boolean-operators)|[hash](#hash)|[Числовые литералы](#numeric-literals)|[startofyear](#startofyear)|[toupper](#toupper)
|[buildschema](#buildschema)|[iff](#iff)|[Скрываемые строковые литералы](#obfuscated-string-literals)|[stdev](#stdev)|[treepath](#treepath)
|[Приведение типов](#casts)|[isempty](#isempty)|[Оператор parse](#parse-operator)|[strcat](#strcat)|[Оператор union](#union-operator)
|[count](#count)|[isnotempty](#isnotempty)|[parsejson](#parsejson)|[Сравнение строк](#string-comparisons)|[variance](#variance)
|[Оператор count](#count-operator)|[isnotnull](#isnotnull)|[percentile](#percentile)|[Строковые литералы](#string-literals)|[Оператор where](#where-operator)
|[countof](#countof)|[isnull](#isnull)|[percentiles](#percentiles)|[strlen](#strlen)
|[Выражения для даты и времени](#date-and-time-expressions)|[Оператор join](#join-operator)|[Оператор project](#project-operator)|[substring](#substring)
|[Литералы даты и времени](#date-and-time-literals)|[Выражения пути JSON](#json-path-expressions)|[rand](#rand)|[sum](#sum)
...
Перед запросом можно указать одно или несколько [предложений let](#let-clause), чтобы определить скалярные значения, таблицы или функции, которые вы используете в запросе.
...
* Не используйте оператор `extend`, если просто хотите использовать в длинном выражении более короткое имя. `...| extend x = anonymous_user_id_from_client | ... func(x) ...` 
...
* *Table1* — "левая часть" для операции соединения.
* *Table2* — "правая часть" для операции соединения. Может быть выражением в виде вложенного запроса для вывода таблицы.
...
* До оператора `join` используйте `where` и `project`, чтобы сократить количество строк и столбцов во входных таблицах. 
...
`Take` — простой и эффективный способ просмотреть пример результатов во время работе в интерактивном режиме. Учтите, что этот оператор не используется для создания конкретных строк либо их создания в указанном порядке.
...
* *Column* — указывает новый столбец, в который будет сохранено соответствие.
...
* *T* — входная таблица.
* *ColumnName* — имя столбца, которое будет отображено в выходных данных. Если аргумент *Expression* отсутствует, столбец с таким именем должен присутствовать во входных данных. 
...
Для статистических выражений и для выражений группирования допускаются произвольные выражения, но для эффективности рекомендуем использовать простые имена столбцов или функцию `bin()` для числовых столбцов.
...
Приоритет между скоростью и точностью контролирует необязательный параметр *Accuracy* (точность).
...
Значение `duration`, ниже которого находится 95 % значений, а выше — 5 % значений для входной выборки, вычисленное для каждого имени запроса:
...
Эта статистическая функция оценивает значения с помощью алгоритма [T-Digest](https://github.com/tdunning/t-digest/blob/master/docs/t-digest-paper/histo.pdf).
...
* *roundTo:* "размер корзины". Число, дата или интервал времени, разбивающий *value* на сегменты. 
...
Интервал `timespan`, прошедший с момента полуночи в начале предыдущего воскресенья, с округлением вниз до целого числа дней.
...
Определяет совпадение [регулярного выражения](#regular-expressions) с подстрокой текстовой строки. При необходимости извлеченная подстрока преобразуется к указанному типу.
...
Если *regex* находит совпадение в *text*, то возвращается подстрока, которая сравнивалась с указанной в *captureGroup* группой захвата, при необходимости преобразованная в тип *typeLiteral*.
...
В [предложениях let](app-analytics-queries.md#let-clause) динамические значения хранятся в виде строк, поэтому оба предложения эквивалентны и должны использоваться с функцией `parsejson` (или `todynamic`).
...
Число элементов в массиве *array* или `null`, если *array* не является массивом.
...
* Используйте `parsejson()`, если хотите извлечь из JSON несколько значений.
...
Анализирует `string` как [значение JSON](http://json.org/) и возвращает значение типа `dynamic`. Этот метод предпочтительнее, чем `extractjson()`, если необходимо извлечь более одного элемента из составного объекта JSON.
...
* *stop:* значение последнего элемента итогового массива или наименьшее значение, которое больше последнего элемента итогового массива и находится в целом количестве шагов *step* от первого элемента *start*.
