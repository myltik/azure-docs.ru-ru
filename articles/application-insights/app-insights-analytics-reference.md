---
title: Справочные материалы по аналитике в Application Insights | Microsoft Docs
...
* *Predicate* — `boolean`выражение[](#boolean) для столбцов таблицы *T*. Вычисляется отдельно для каждой строки в *T*.
...
* *kind`normal` — *. Значение по умолчанию — `"normal"|"regex"`. 
