...
|Имя | Импликация, если задано значение true
|---|---
...
В качестве типа столбца параметра должно быть указано `dynamic` — массив или контейнер свойств.
...
* *rewrite* — регулярное выражение, результат которого заменит все найденные соответствия *matchingRegex*. Используйте `\0` для указания полного соответствия, `\1` для первой группы захвата, `\2` и так далее — для следующих групп захвата.
* *text* — строка.
...
следующий фрагмент извлечет значение слота `duration` из объекта, а затем из него извлечет два слота `duration.value` и `duration.min` (`118.0` и `110.0` соответственно).
