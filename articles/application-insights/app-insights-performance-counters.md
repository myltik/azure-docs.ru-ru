...
Если на серверах еще не установлен монитор состояний Application Insights, его нужно установить для просмотра счетчиков производительности.

Скачайте [установщик монитора состояний](http://go.microsoft.com/fwlink/?LinkId=506648) и запустите его на каждом экземпляре сервера. Если он уже установлен, повторная установка не требуется.

* *Пакет SDK для Application Insights [установлен в приложении](app-insights-asp-net.md) еще на этапе разработки. Нужен ли мне еще и монитор состояний?*
  
    Да, монитор состояний обязательно нужен для сбора данных о счетчиках производительности от веб-приложений ASP.NET. Как вы, возможно, уже знаете, монитор состояний можно использовать для [мониторинга запущенных веб-приложений](app-insights-monitor-performance-live-website-now.md) без установки пакета SDK на этапе разработки.
