...
Сейчас профилировщик работает для веб-приложений ASP.NET и ASP.NET Core, запущенных в службе приложений Azure. Он требуется, начиная от уровня службы **Базовый**.

## <a id="installation"></a> Включение профилировщика для веб-приложения служб приложений
Если вы уже опубликовали приложение в службах приложений, но ничего не сделали в исходном коде для использования Application Insights, перейдите к панели служб приложений на портале Azure. Затем выберите **Мониторинг > Application Insights** и следуйте инструкциям на панели, чтобы создать ресурс Application Insights или выбрать имеющийся для отслеживания веб-приложения.
...
Когда приложение получит часть трафика, откройте колонку **Производительность** и перейдите к разделу **Take Actions** (Предпринять действия), чтобы просмотреть трассировку профилировщика. Нажмите кнопку **Трассировки Profiler**.
...
За использование службы профилировщика плата не взимается. Для использования службы профилировщика веб-приложение должно быть размещено по крайней мере на уровне "Базовый" службы приложений Azure.
...
1.  Перейдите на сайт Kudu. На вкладке средств разработки выберите вкладку Advanced Tools (Дополнительные инструменты) с логотипом Kudu. Щелкните Go (Перейти). Вы перейдете на новый сайт (вход будет выполнен автоматически).
...
3.  Щелкните "Сайт > wwwroot > App_Data > Задания > Непрерывно". Должна отобразиться папка ApplicationInsightsProfiler2. Щелкните значок загрузки слева от папки. Будет скачан файл ApplicationInsightsProfiler2.zip.
4.  Это скачивает все необходимые файлы. Прежде чем продолжить, рекомендуется создать чистый каталог, чтобы поместить в него ZIP-архив.

### <a name="setting-up-the-web-job-archive"></a>Настройка архива веб-задания
При добавлении нового веб-задания на веб-сайт Azure, по сути, вы создаете ZIP-архив с файлом run.cmd внутри. run.cmd сообщает системе веб-заданий, что нужно делать при запуске веб-задания.
...
3.  Создайте файл run.cmd. (Для удобства можно открыть рабочую папку в VS Code.)
...
Далее мы добавим новое веб-задание на нашем сайте. В этом примере показано, как добавить веб-задание, которое запускается вручную. Этот процесс для запланированного веб-задания практически ничем не отличается.

1.  Перейдите на панель мониторинга веб-заданий.
2.  На панели инструментов щелкните команду "Добавить".
3.  Присвойте веб-заданию имя. Для ясности вы можете дать ему имя архива и открыть его, чтобы иметь разные версии run.cmd.
...
1. По умолчанию на компьютере в любой момент времени может выполняться только один процесс ApplicationInsightsProfiler.exe. Прежде чем приступить к работе, отключите непрерывное веб-задание на панели мониторинга. Щелкните строку и выберите "Остановить". Затем выберите "Обновить" на панели инструментов и убедитесь, что в состоянии указано, что задание остановлено.
...
- Мы не можем обновить двоичные файлы агента для веб-задания с помощью нашей службы. Сейчас у нас нет стабильной страницы скачивания для двоичных файлов. Чтобы получить их последнюю версию, обновите расширение и извлеките их из папки непрерывного веб-задания, как это делалось на предыдущих шагах.

- Так как используются аргументы командной строки, которые изначально разработаны для использования разработчиком, а не пользователем, они могут изменяться в будущем, поэтому учитывайте это при обновлении. Это не должно вызвать больших проблем, так как можно добавить веб-задание, а затем запустить его и проверить, что оно работает. В конечном итоге будет создан пользовательский интерфейс для выполнения этих действий автоматически.

- Функция веб-заданий для служб приложений уникальна тем, что при запуске веб-задания процесс гарантированно будет иметь те же переменные среды и параметры приложения, что и веб-сайт. Это значит, что в профилировщик не нужно передавать ключ инструментирования через командную строку. Он должен получить его из среды. Однако если вы хотите запустить профилировщик в среде разработки или на компьютере за пределами служб приложений, необходимо предоставить ключ инструментирования. Это можно сделать путем передачи аргумента `--ikey <instrumentation-key>`. Это значение должно соответствовать ключу инструментирования, используемому вашим приложением. Выходные данные журнала профилировщика содержат сведения о ключе инструментирования, используемом профилировщиком, а также сведения о том, были ли во время профилирования обнаружены действия от ключа инструментирования.

- Фактически веб-задания, запускаемые вручную, можно запустить с помощью веб-перехватчика. Чтобы получить этот URL-адрес, щелкните правой кнопкой мыши веб-задание на панели мониторинга и просмотрите свойства. Вы также можете выбрать свойства на панели инструментов, выбрав веб-задание в таблице. Это открывает неограниченные возможности, такие как запуск профилировщика из конвейера CI/CD (как в VSTS) или функции, как у Microsoft Flow (https://flow.microsoft.com/en-us/). В конечном счете, это зависит от того, насколько сложным вы хотите сделать свой run.cmd (который также может быть run.ps1), при этом сохраняя гибкость.
