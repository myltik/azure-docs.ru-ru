<properties 
   ...
   description="Обзор раздела "Настройки" пользовательского интерфейса Azure Mobile Engagement" 
...
Параметры в меню "Параметры", доступные для приложения, отличаются в зависимости от платформы приложения и разрешений, которые вы ему предоставили. Вот что включает в себя меню "Параметры": "Подробности", "Проекты", "Системные push-уведомления", "Скорость отправки push-уведомлений", "Пакет SDK", "Отслеживание", "Информация о приложении", "Коммерческое давление" и "Разрешения". Только пункт меню "Информация о приложении" в разделе "Параметры" пользовательского интерфейса содержит элементы, которыми можно управлять с помощью функции "Тег" в API устройства. "Глоссарий" в разделе "Основные понятия" содержит такие, например, определения терминов и сокращений: APNS, GCM, IDFA, API, пакет SDK, ключ API, ключ SDK, идентификатор приложения, идентификатор AppStore, план добавления тегов, идентификатор пользователя, идентификатор устройства, делегат приложения, трассировка стека и создание глубинных ссылок.

**См. также** 

- [Документация по API - API устройств][Link 4], [Основные понятия - глоссарий][Link 6], [Руководство по устранению неполадок - Служба][Link 2]
 
  ![settings1][46]

## Подробности:
Позволяет менять имя и описание вашего приложения, 
просматривать его владельца и разрешения роли. 
Область "Конфигурация аналитики" - позволяет просматривать и менять день, с которого начинается неделя, и время удержания в днях.
...
Позволяет удалить существующий сертификат или зарегистрировать новый сертификат, который необходимо использовать при передаче системных push-уведомлений. 
Системные push-уведомления позволяют Azure Mobile Engagement отправлять push-уведомления в ваше приложение в любое время, даже если оно не работает. 
...
- [Документация по API - Reach API] [Link 4],[Документация по API - Push API] [Link 4], [Документация по пользовательскому интерфейсу - Reach - новая кампания push-уведомлений][Link 17]
...
- [Документация по пакету SDK - iOS - Как подготовить приложение для работы с push-уведомлениями Apple][Link 5]
 
![settings4][49]
 
**Служба push-уведомлений Windows (WPNS)**

Чтобы включить системные push-уведомления, используя службу уведомлений Windows, необходимо указать учетные данные приложения. 
Вам понадобятся ваши идентификатор безопасности пакета (SID) и секретный ключ.
 
![settings5][50]
 
**Включение системных push-уведомлений Android с помощью GCM или ADM**

**Google Cloud Messaging (GCM) для Android**

Чтобы включить системные push-уведомления с помощью GCM, необходимо следовать указаниям, приведенным по адресу http://developer.android.com/guide/google/gcm/gs.html. 
Затем необходимо вставить простой ключ API сервера, настроенный без ограничений IP-адресов. 
Требуется интеграция пакета SDK для Android версии v1.12.0 или более поздней.

**См. также**

- [Документация по пакету SDK - Android - как интегрировать GCM][Link 5]
 
**Amazon Device Messaging (ADM) для Android**

Чтобы включить системные push-уведомления, используя ADM, необходимо указать <учетные данные OAuth>, которые состоят из идентификатора клиента и секрета клиента (требуется интеграция пакета SDK для Android версии v2.1.0 или более поздней).

**См. также**

- [Документация по пакету SDK - Android - Как интегрировать ADM][Link 5]
...
-    использование сторонней службы атрибуции (сейчас Azure Mobile Engagement поддерживает [Mobile App Tracking],( http://www.mobileapptracking.com/) помимо чего планируется поддержка [Trademob](http://www.trademob.com/) в ближайшем будущем);
-    использование стороннего источника ссылки на установку (сейчас Azure Mobile Engagement поддерживает [Google Play Install Referrer](https://developers.google.com/app-conversion-tracking/docs/third-party-trackers/), но только для Android);
...
- [Документация по пользовательскому интерфейсу - новая кампания push-уведомлений в разделе рекламных кампаний.][Link 17] 
...
- [Документация по пользовательскому интерфейсу - Рекламные кампании - Содержимое push-уведомления][Link 17]
...
- [Документация по пользовательскому интерфейсу - новая кампания push-уведомлений в разделе рекламных кампаний.][Link 17], [Документация по пользовательскому интерфейсу - Сегменты].[Link 18]
