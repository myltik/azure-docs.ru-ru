...
Используется для просмотра шаблон набора Минимальный масштаб работало, GitHub [azuredeploy.json](https://raw.githubusercontent.com/gatneil/mvss/minimum-viable-scale-set/azuredeploy.json).

В этом учебнике, давайте рассмотрим diff (`git diff master minimum-viable-scale-set`) для создания допустимого Минимальный масштаб шаблона набора по частям.

## <a name="define-schema-and-contentversion"></a>Определение элементов $schema и contentVersion
Во-первых, определите `$schema` и `contentVersion` в шаблоне. Элемент `$schema` определяет версию языка шаблона. Он используется для разметки синтаксиса в редакторе Visual Studio и других функций проверки кода. Элемент `contentVersion` не используется в Azure, но позволяет отслеживать версию шаблона.
...
Затем определите два параметра `adminUsername` и `adminPassword`. Параметры — это значения, которые пользователь задает во время развертывания. `adminUsername` Параметр является просто `string` типа, но поскольку `adminPassword` секрете, присвойте его тип является `securestring`. Позднее эти параметры будут переданы в конфигурацию масштабируемого набора.
...
Шаблоны Resource Manager позволяют определять переменные и затем использовать их в шаблоне. Пример не использует переменные, поэтому объект JSON будет пустым.
...
Чтобы указать расположение для виртуальной сети, используйте [функции шаблона диспетчера ресурсов](../azure-resource-manager/resource-group-template-functions.md). Каждая функция заключается в кавычки и квадратные скобки, примерно так: `"[<template-function>]"`. В этом случае использовать `resourceGroup` функции. Она не принимает никаких аргументов и возвращает объект JSON с метаданными о группе ресурсов, в которой выполняется развертывание. Группу ресурсов устанавливает пользователь во время развертывания. Это значение затем индексируется в этот объект JSON с `.location` для получения сведений о расположении из объекта JSON.
...
Каждый ресурс Resource Manager имеет собственный раздел `properties` для связанных с ним конфигураций. В этом случае указать, что виртуальные сети должны иметь одну подсеть частных IP-адресов с помощью `10.0.0.0/16`. Масштабируемый набор всегда содержится в одной подсети. Он не может охватывать подсети.
...
В нашем примере этот список содержит только один элемент — это упомянутая выше виртуальная сеть. Укажите эту зависимость, так как сеть должно существовать до создания все виртуальные машины при этом требуется задать масштаб. Тогда масштабируемый набор сможет назначить этим виртуальным машинам частные IP-адреса из диапазона IP-адресов, указанного ранее в свойствах сети. Каждая строка в списке dependsOn имеет формат `<type>/<name>`. Используйте такие же значения `type` и `name`, как и в определении ресурса виртуальной сети.
...
Для масштабируемого набора нужно указать операционную систему, которую следует установить на виртуальных машинах. Здесь можно Создайте виртуальные машины с полностью обновленного образа 16.04 LTS Ubuntu.
...
В масштабируемом наборе развертываются несколько виртуальных машин. Вместо имени каждой виртуальной Машины, указать `computerNamePrefix`. Масштабируемый набор добавляет индекс к этому префиксу для каждой виртуальной машины, поэтому имена виртуальных машин выглядят следующим образом: `<computerNamePrefix>_<auto-generated-index>`.

В следующем фрагменте используйте параметры из перед задайте имя пользователя администратора и пароль для всех виртуальных машин в наборе масштабирования. В этом процессе используется `parameters` функции шаблона. Эта функция принимает строку, обозначающую параметр для ссылки, и возвращает значение для этого параметра.
...
Наконец укажите конфигурации сети для виртуальных машин в наборе масштабирования. В этом случае необходимо только указать идентификатор подсети, созданного ранее. Это означает, что масштабируемый набор будет размещать в этой подсети сетевые интерфейсы.
