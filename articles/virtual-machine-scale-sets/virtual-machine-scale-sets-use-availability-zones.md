...
Процесс создания масштабируемого набора, использующего зону доступности, описан в статье [Создание масштабируемого набора виртуальных машин с помощью Azure PowerShell](virtual-machine-scale-sets-create-cli.md). Чтобы использовать зоны доступности, необходимо создать масштабируемый набор в поддерживаемом регионе Azure и [зарегистрироваться в предварительной версии зон доступности](http://aka.ms/azenroll).
...
Процесс создания масштабируемого набора, использующего зону доступности, описан в статье [Создание масштабируемого набора виртуальных машин с помощью Azure PowerShell](virtual-machine-scale-sets-create-powershell.md). Чтобы использовать зоны доступности, необходимо создать масштабируемый набор в поддерживаемом регионе Azure и [зарегистрироваться в предварительной версии зон доступности](http://aka.ms/azenroll). Добавьте параметр `-Zone` в команду [New-AzureRmVmssConfig](/powershell/module/azurerm.compute/new-azurermvmssconfig) и укажите требуемую зону (например, *1*, *2* или *3*). 
...
Создание масштабируемого набора, использующего зону доступности, описано в статьях о начале работы для [Linux](virtual-machine-scale-sets-create-template-linux.md) или [Windows](virtual-machine-scale-sets-create-template-windows.md). Чтобы использовать зоны доступности, необходимо создать масштабируемый набор в поддерживаемом регионе Azure и [зарегистрироваться в предварительной версии зон доступности](http://aka.ms/azenroll). Добавьте свойство `zones` в тип ресурса *Microsoft.Compute/virtualMachineScaleSets* в шаблон и укажите требуемую зону (например, *1*, *2* или *3*).
