...
Тесты выполняются с помощью JMeter. Главный сервер JMeter загружает план тестирования и передает его в набор подчиненных серверов JMeter, которые фактически выполняют тесты. Главный сервер JMeter координирует подчиненные JMeter серверы и собирает их результаты.
...
Для создания и отправки тестовых данных в планах тестирования применяется дискретизатор запросов JUnit. План тестирования JMeter создает и запускает этот дискретизатор, а также отслеживает данные производительности каждого узла Elasticsearch. Дополнительные сведения смотри в приложениях к документам [Настройка производительности приема данных для Elasticsearch в Azure] и [Настройка объединения данных и производительности запросов для Elasticsearch в Azure].
