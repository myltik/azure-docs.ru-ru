...
В этом учебном курсе показано, как использовать службу Azure DocumentDB для хранения данных и обеспечения доступа к ним из приложения Node.js Express, размещенного на веб-сайтах Azure.
...
Затем вернитесь к этой статье, где вы узнаете ответы на следующие вопросы:
...
> [AZURE.TIP]Этот учебный курс разработан для читателей, обладающих определенным опытом использования Node.js и веб-сайтов Azure.
...
Теперь давайте создадим базовый проект Node.js «Hello World» с помощью платформы [Express](http://expressjs.com/).
...
5. Новое приложение можно просмотреть, перейдя в браузере по адресу [http://localhost: 3000](http://localhost:3000).
	...
	![Снимок экрана приложения Hello World в окне браузера](./media/documentdb-nodejs-application/image12.png)
...
2. Добавьте в файл **config.js** следующий код. Этот код определяет параметры конфигурации и значения, необходимые для нашего приложения.
...
3. В файле **config.js** обновите значения HOST и AUTH_KEY значениями, найденными в выноске Keys учетной записи DocumentDB на [портале Microsoft Azure](http://portal.azure.com):
...
2. Откройте файл **layout.jade**, расположенный в папке **views**, и замените его содержимое следующим:
	...
	тот код указывает обработчику **Jade** создать HTML для нашего приложения, а также **блок** под названием **контент** в котором можно поместить макет наших страниц.
...
1. Если это еще не сделано, включите репозиторий git для веб-сайта Azure. Указания о том, как это сделать, см. [здесь](app-service-web/web-sites-publish-source-control-git.md#step4).
