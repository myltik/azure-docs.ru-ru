<properties urlDisplayName="Dashboard, Monitor, Scale, Configure and Hybrid Connections tabs" pageTitle="Панель мониторинга, монитор и масштаб в службах BizTalk | Azure" metaKeywords="BizTalk Services, Azure, dashboard, monitor, scale, wabs, mabs" description="Узнайте об элементах управления на вкладках портала управления для служб BizTalk: «Панель мониторинга», «Мониторинг» и «Масштаб»." metaCanonical="" services="biztalk-services" documentationCenter="" title=" Monitor and Scale tabs" authors="mandia" solutions="" manager="dwrede" editor="cgronlun" />
...
- ![Quick Start][QuickStart]  [Быстрый запуск](#QuickStart)
...
##<a name="QuickStart"></a>Быстрый запуск (![Quick Start][QuickStart])
...
<tr>
        ...
        <td>Для получения дополнительных сведений о службах BizTalk в Azure перейдите в <a HREF="http://azure.microsoft.com/ru-ru/documentation/services/biztalk-services/">центр обучения</a>.</td>
</tr>
</table>
...
<br/><br/>
Также вы можете открыть портал управления для службы контроля доступа. Работа с этим порталом аналогична выбору параметра <strong>Active Directory</strong> в области навигации слева.
<br/><br/>
...
<td>При создании учетной записи хранения автоматически создаются первичный ключ и вторичный ключ. Эти ключи шифрования позволяют управлять доступом к учетной записи хранения. Служба BizTalk автоматически использует первичный ключ. <strong>Синхронизировать ключи</strong> - эта команда позволяет пользователям переключаться между первичным и вторичным ключами без прерывания работы службы BizTalk.
<br/><br/>
 Например, пусть вам требуется, чтобы служба BizTalk использовала новый первичный ключ для учетной записи хранения. Для этого потребуется следующее.
<br/><br/>
<ol>
<li>Выберите свою службу BizTalk, а затем <strong>Синхронизировать ключи</strong>. Выберите вторичный ключ. После этого в службе BizTalk будет использоваться этот вторичный ключ.</li>
<li>На портале управления Azure выберите свою учетную запись хранения и повторно создайте первичный ключ. Помните, что в службе BizTalk используется вторичный ключ.</li>
<li>Выберите свою службу BizTalk, а затем <strong>Синхронизировать ключи</strong>. Теперь выберите первичный ключ. Это новый первичный ключ, который был создан повторно.</li>
<li>На портале управления Azure выберите свою учетную запись хранения и заново создайте вторичный ключ.</li>
</ol>
<br/>
Этот процесс называется "переключением ключей". Его цель - позволить пользователям переключаться между первичным и вторичным ключами без прерывания работы службы BizTalk.</td>
</tr>
...
Если вы выберете имя своей службы BizTalk, появится вкладка "Панель мониторинга". На панели мониторинга отображаются следующие сведения.
...
Эти метрики содержат обновляемые в реальном времени значения, отражающие работоспособность службы BizTalk. Также можно указать **относительные** или **абсолютные** значения и **период времени** отображаемых на диаграмме метрик. 
...
<td>Может изменить службу BizTalk так, чтобы в ней использовался другой сертификат SSL. При <a HREF="http://go.microsoft.com/fwlink/p/?LinkID=302280">создании службы BizTalk</a> будет создан самозаверяющий сертификат SSL.</td>
</tr>
<tr>
...
<td>Отображение текущего состояния службы BizTalk. См. <a HREF="http://go.microsoft.com/fwlink/p/?LinkID=329870">Службы BizTalk: диаграмма состояния службы</a>. </td>
</tr>
<tr>
...
<td>IP-адрес, присвоенный вашей службе BizTalk. Он используется для всех конечных точек ввода и является адресом источника для исходящего трафика. Указанный IP-адрес принадлежит данной службе BizTalk, пока она предоставляется. При удалении службы BizTalk данный IP-адрес присваивается другой службе BizTalk.</td>
</tr>
<tr>
...
<td>Выводит подписку, на которой размещена ваша служба BizTalk. Эта подписка обеспечивает доступ к порталу управления Azure.</td>
</tr>
<tr>
...
[Службы BizTalk: подготовка с использованием портала управления Azure](http://go.microsoft.com/fwlink/p/?LinkID=302280) выводит список этапов создания службы BizTalk.
...
Также можно указать **относительные** или **абсолютные** значения и **период времени** отображаемых метрик. 

##### Отображение метрик на диаграмме и их удаление.
1. Выберите вкладку **Монитор**.
2. Выберите **Добавить метрики** в панели задач.
<br/>
![Select Add Metrics][AddMetrics]
3. Проверьте метрики производительности, которые должны отображаться.
4. Установите этот флажок, чтобы вернуться на вкладку **Монитор**.
...
Некоторые выпуски BizTalk не поддерживают параметр масштаба. В этом случае разрешен только один модуль. Для определения количества единиц, до которого может масштабироваться данный выпуск, см. раздел [Службы BizTalk: диаграмма выпусков](http://go.microsoft.com/fwlink/p/?LinkID=302279).
...
[Службы BizTalk: диаграмма выпусков](http://go.microsoft.com/fwlink/p/?LinkID=302279) определяет "единицу".


##<a name="Configure"></a>Настройка
Неприменимо для гибридных подключений.

Устанавливает для состояния резервного копирования значение "Нет" или "Автоматически". При установке в значение "Нет" не происходит автоматического создания резервных копий. При значении "Автоматически" вы выполняете настройку местоположения резервной копии, частоты резервного копирования и срока хранения файлов резервных копий. 

[Службы BizTalk: резервное копирование и восстановление](http://go.microsoft.com/fwlink/p/?LinkID=329873) - здесь описаны подробные сведения. 
...
Чтобы создать гибридные подключения на веб-сайтах Azure, см. раздел [Гибридное подключение: подключение веб-сайта Azure к локальным ресурсам](http://go.microsoft.com/fwlink/p/?LinkId=397538).

Чтобы использовать гибридные подключения в мобильных службах Azure, см. раздел [Мобильные службы Azure и гибридные подключения](http://azure.microsoft.com/ru-ru/documentation/articles/mobile-services-dotnet-backend-hybrid-connections-get-started).

Чтобы создать гибридные подключения или управлять ими в службах Azure BizTalk, см. раздел [Гибридные подключения](http://go.microsoft.com/fwlink/p/?LinkID=397274).
...
- [Службы BizTalk: состояние службы BizTalk](http://go.microsoft.com/fwlink/p/?LinkID=329870)<br/>
