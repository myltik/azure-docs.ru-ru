...
В этом учебнике вы создадите проект веб-API ASP.NET 2 с помощью Visual Studio 2015 и настроите его для развертывания в облаке в виде [приложения API](app-service-api-apps-why-best-platform.md) в [службе приложений Azure](app-service-value-prop-what-is.md). Вы также выполните развертывание этого проекта в Azure. В результате прохождения этого учебника у вас будет готовое приложение API, запущенное в облаке Azure.
...
По умолчанию в проектах приложений API включена функция автоматического создания метаданных [Swagger](http://swagger.io/ "Официальные сведения о Swagger"), которую обеспечивает пакет NuGet Swashbuckle. При установке пакета по умолчанию также включена тестовая страница API.
...
1. В папке проекта создайте папку *Models*.

2. Добавьте файл класса *Contact.cs* и замените его содержимое следующим кодом.
...
1. Создайте приложение API на [портале предварительной версии Azure](https://portal.azure.com). 
	...
	* Щелкните **Ценовая категория**, чтобы получить список вариантов, нажмите кнопку **Просмотреть все** и выберите ценовую категорию **Free**.
...
2. Когда Azure завершит создание приложения API (см. раздел **Уведомления** в левой части страницы), установите для уровня доступа приложения API значение **Общедоступный (анонимный)**.

	* Щелкните **Обзор > Группы ресурсов > [группа ресурсов, которую вы создали] > [приложение API, которое вы создали]**.

	* Щелкните **Параметры > Параметры приложения**.
