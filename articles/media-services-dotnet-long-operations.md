...
#Потоковая трансляция с помощью служб Azure Media Services
...
Неопросные методы имеют следующий шаблон именования: Методы Send*OperationName*Operation (например, SendCreateOperation). Методы Send*OperationName*Operation возвращают объект **IOperation**. Полученный объект содержит сведения, которые можно использовать для отслеживания операции. Методы Send*OperationName*OperationAsync возвращают **Task<IOperation>**.

В настоящее время неопросные методы поддерживаются следующими классами:  **Channel**, **StreamingEndpoint** и **Program**.

Чтобы опросить состояние операции, используйте метод **GetOperation** в классе **OperationBaseCollection**. Для проверки состояния операции используйте следующие интервалы: для операций **Channel** и **StreamingEndpoint** используйте 30 секунд, для операций **Program** используйте 10 секунд.
