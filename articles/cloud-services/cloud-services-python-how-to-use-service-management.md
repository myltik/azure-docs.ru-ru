---
title: "Использование API для управления службами (Python) — руководство по функциям"
...
API управления службами обеспечивает программный доступ к большинству функций управления службами, доступных на [портале Azure][management-portal]. Пакет Azure SDK для Python позволяет управлять облачными службами и учетными записями хранения.
...
Пакет SDK Azure для Python служит оболочкой для [API управления службами Azure][svc-mgmt-rest-api], который является интерфейсом REST API. Все операции API выполняются по SSL и проходят взаимную проверку подлинности с использованием сертификатов X.509 v3. Доступ к службе управления можно получить из службы, работающей в Azure или непосредственно через Интернет из любого приложения, которое может отправить HTTPS-запрос и получить HTTPS-ответ.

## <a name="Installation"> </a>Установка
Все функции, описанные в этой статье, доступны в пакете `azure-servicemanagement-legacy` , который можно установить с помощью pip. Дополнительные сведения об установке (например, если вы не работали с Python) можно найти в этой статье: [Установка Python и пакета SDK](../python-how-to-install.md).
...
Дополнительные сведения о сертификатах Azure см. в статье [Общие сведения о сертификатах для облачных служб Azure](cloud-services-certs-create.md). Полное описание параметров OpenSSL см. в документации по адресу [http://www.openssl.org/docs/apps/openssl.html](http://www.openssl.org/docs/apps/openssl.html).

Создав эти файлы, необходимо передать `.cer`-файл в Azure с помощью действия "Отправить" (доступно на вкладке "Параметры" [портала Azure][management-portal]) и обязательно записать, куда был сохранен этот `.pem`-файл.
...
Эта команда создает `.cer`-файл и устанавливает его в **личное** хранилище сертификатов. Дополнительные сведения см. в статье [Общие сведения о сертификатах для облачных служб Azure](cloud-services-certs-create.md).

Создав сертификат, необходимо передать `.cer`-файл в Azure с помощью действия "Отправить" (доступно на вкладке "Параметры" [портала Azure][management-portal]).
...
Чтобы получить сведения о конкретной размещенной службе, можно передать имя размещенной службы в метод **get\_hosted\_service\_properties**:
...
Обратите внимание, что в описанном выше примере состояние операции **create\_storage\_account** можно извлекать путем передачи результатов, возвращенных **create\_storage\_account**, в метод **get\_operation\_status**.  
...
Дополнительные сведения о записи образа виртуальной машины Linux см. в статье [Запись классической виртуальной машины Linux в виде образа](../virtual-machines/linux/classic/capture-image.md?toc=%2fazure%2fvirtual-machines%2flinux%2fclassic%2ftoc.json).

Дополнительные сведения о записи образа виртуальной машины Windows см. в статье [Запись образа виртуальной машины Azure Windows, созданной с использованием классической модели развертывания](../virtual-machines/windows/classic/capture-image.md?toc=%2fazure%2fvirtual-machines%2fwindows%2fclassic%2ftoc.json).
