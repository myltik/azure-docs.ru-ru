...
При развертывании пакета приложения облачной службы в Azure вы можете получить сведения о развертывании на панели **Свойства** на классическом портале Azure. Вы можете использовать сведения, отображающиеся на этой панели, чтобы устранять неполадки в работе облачной службы. Кроме того, эту информацию вы можете передать в службу поддержки Azure, когда открываете новый запрос о поддержке.
...
1. На классическом портале Azure щелкните развертывание облачной службы.
2. На панели **Свойства** классического портала Azure проверьте порты для экземпляров ролей (под входными конечными точками).
...
1. На классическом портале Azure щелкните развертывание облачной службы.
2. На панели **Свойства** классического портала Azure ознакомьтесь с информацией и определите, восстанавливалась ли служба тогда, когда перезапускались роли.
...
1. На классическом портале Azure щелкните развертывание облачной службы.
2. На панели **Свойства** найдите значение параметра **Состояние**. Если указано значение **Готово**, проверьте **последнюю операцию**: возможно, недавно выполнялась операция, которая могла помешать переключению виртуального IP-адреса.
...
Возможна проблема с кодом, пакетом или файлом конфигурации приложения. Если эта так, состояние будет изменяться каждые несколько минут, а на классическом портале Azure могут отображаться примерно такие состояния: **Перезапуск**, **Занято** или **Инициализация**. Это означает, что что-то не так с приложением и что эта проблема не дает работать экземпляру роли.
...
1. На классическом портале Azure щелкните экземпляр роли.
2. В области **Свойства** классического портала Azure ознакомьтесь с такими условиями, связанными с устранением проблемы.
