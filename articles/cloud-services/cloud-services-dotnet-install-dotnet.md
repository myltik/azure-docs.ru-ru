---
title: Установка .NET для роли облачной службы | Microsoft Docs
...
В этой статье описывается, как установить платформу .NET Framework для веб-роли и рабочей роли облачной службы. Вы можете использовать эти шаги для установки .NET 4.6.1 в семействе версий 4 гостевой ОС Azure. Последние сведения о выпусках гостевой ОС см. в статье [Таблица совместимости версий гостевых ОС и Azure SDK](cloud-services-guestos-update-matrix.md).
...
С помощью начальных задач вы можете выполнять различные операции еще до запуска роли. Установка .NET Framework как начальной задачи позволит завершить установку платформы до запуска всех приложений. Дополнительные сведения о начальных задачах см. в статье [Выполнение задач запуска в Azure](cloud-services-startup-tasks.md).
...
1. В файл *ServiceDefinition.csdef* в узле **WebRole** или **WorkerRole** добавьте для всех ролей следующий код:
    ...
    Он выполняет консольную команду *install.cmd* с правами администратора, что позволит установить платформу .NET Framework. Конфигурация также создает LocalStorage с именем *NETFXInstall*. Сценарий запуска в качестве временной папки задаст этот локальный ресурс хранилища, чтобы установщик .NET Framework был скачан и установлен из данного ресурса. Важно задать размер этого ресурса не менее 1024 МБ, чтобы обеспечить правильную установку платформы. Дополнительные сведения о задачах запуска см. в разделе [Общие задачи запуска облачной службы](cloud-services-startup-tasks-common.md).
...
## Настройка системы диагностики для передачи журналов задач запуска в хранилище BLOB-объектов
