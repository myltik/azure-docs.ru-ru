---
title: "Настройка гибридного кластера HPC с помощью пакета Microsoft HPC | Документация Майкрософт"
...
* Создание учетной записи хранения Azure
  ...
  > [!NOTE]
  > Также запишите идентификатор подписки Azure, которая понадобится позже. Его можно найти на классическом портале, щелкнув **Параметры** > **Подписки**.
  > 
  > 
...
2. Щелкните **Параметры** > **Сертификаты управления**.
...
2. Щелкните **Вычисления** > **Облачная служба** > **Быстрое создание**.
...
1. На панели команд классического портала нажмите кнопку **Создать**.
2. Щелкните **Службы данных** > **Хранилище** > **Быстрое создание**.
...
3. В мастере настройки сети выберите параметр **Все узлы только в корпоративной сети** (топология 5).
   ...
   > [!NOTE]
   > Это простейшая конфигурация для демонстрационных целей, поскольку головному узлу для подключения к Active Directory и Интернету требуется только один сетевой адаптер. В этом учебнике не рассматриваются сценарии, в которых кластеру требуются дополнительные сети.
   > 
   > 
...
6. В диалоговом окне **Учетные данные установки** введите учетные данные доменной учетной записи, которая использовалась для установки пакета HPC. Нажмите кнопку **ОК**.
   ...
   > [!NOTE]
   > Службы пакета HPC используют учетные данные установки только для развертывания присоединенных к домену вычислительных узлов. Узлы Azure, добавляемые в этом учебнике, не присоединены к домену.
   > 
   > 
7. В **список дел развертывания**щелкните **Настроить именование новых узлов**.
8. В диалоговом окне **Указать серию именования узлов** примите значения по умолчанию и нажмите кнопку **ОК**.
   
    ![Именование узлов][config_hpc8]
   
   > [!NOTE]
   > Серия именования формирует только имена для вычислительных узлов, присоединенных к домену. Имена узлов Azure задаются автоматически.
   > 
   > 
9. В **список дел развертывания**щелкните **Создать шаблон узла**. Этот шаблон будет использоваться для добавления узлов Azure в кластер.
...
3. На странице **Укажите новые узлы** выберите созданный ранее шаблон узла Azure (который по умолчанию называется **шаблоном узла Azure по умолчанию**). Затем укажите **2** **мелких** узла и нажмите кнопку **Далее**.
    ...
    Сведения о доступных размерах см. в статье [Размеры для облачных служб](cloud-services-sizes-specs.md).
...
4. Чтобы убедиться, что экземпляры ролей выполняются, на [классическом портале](https://manage.windowsazure.com) щелкните **Облачные службы** > , а затем выберите *имя вашей облачной службы* >  и щелкните **Экземпляры**.
...
4. Чтобы убедиться, что экземпляры ролей больше не выполняются в Azure, на [классическом портале](https://manage.windowsazure.com) щелкните **Облачные службы** > , а затем выберите *имя вашей облачной службы* >  и щелкните **Экземпляры**. В рабочей среде будут отсутствовать развернутые экземпляры.
