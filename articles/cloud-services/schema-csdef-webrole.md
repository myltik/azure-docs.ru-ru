---
title: "Определение облачных службы Azure. Схема WebRole | Документация Майкрософт"
ms.custom: 
ms.date: 04/14/2015
ms.prod: azure
ms.reviewer: 
...
|vmsize|string|необязательный параметр. Это значение используется для изменения размера виртуальной машины, выделяемой для роли. По умолчанию используется значение `Small`. Дополнительные сведения см. в статье [Размеры для облачных служб](cloud-services-sizes-specs.md).|  
