...
1. На [портале управления Azure](http://manage.windowsazure.com) выберите **Создать** — **Службы данных** — **Базы данных SQL** — **Быстрое создание**.
...
1. Выберите тот же **Регион**, что и для облачной службы.
...
1. На [портале управления Azure](http://manage.windowsazure.com) щелкните кнопку **Создать** — **Службы данных** — **Хранилище** — **Быстрое создание**.
...
2. На [портале управления Azure](http://manage.windowsazure.com) щелкните **Базы данных SQL**, в области слева выберите базу данных, созданную в этом учебнике, затем **Панель мониторинга**, а затем пункт **Показать строки подключения**.
...
5. В разделе **Роли** **обозревателя решений** щелкните правой кнопкой мыши **ContosoAdsWorker** и выберите пункт **Свойства**.
...
1. В диалоговом окне **Создать строку подключения хранилища** щелкните переключатель **Подключиться, используя подписку**, выберите учетную запись, созданную ранее, и нажмите кнопку **ОК**. Если вы еще не вошли, появится запрос на ввод учетных данных учетной записи Azure.
...
Параметр `<Instances>` указывает число виртуальных машин, на которых Azure будет запускать код рабочий роли. В раздел [Дальнейшие действия](#next-steps) включены ссылки на дополнительную информацию по масштабированию вширь облачной службы.
...
2. В левой области диалогового окна **Создание проекта** разверните **Visual C#**, выберите шаблоны **Облачные** и шаблон **Облачная служба Azure**.
...
7. В диалоговом окне "Изменить проверку подлинности" щелкните **Без проверки** и нажмите кнопку **ОК**.
...
11. В диалоговом окне **Добавление нового проекта** выберите **Настольная ОС Windows** в разделе **Visual C#** в левой панели, а затем щелкните шаблон **Библиотека классов**.
...
13. Найдите пакет *Хранилище Azure*, а если его нет в списке, щелкните **Обновить** для получения последней версии клиентской библиотеки хранилища.
...
10. Задайте в проекте ContosoAdsWeb ссылку на проект ContosoAdsCommon. Щелкните правой кнопкой мыши проект ContosoAdsWeb и выберите пункты **Ссылки** — **Добавить ссылку**. В диалоговом окне **Диспетчер ссылок** выберите пункты **Решение — Проекты** в левой панели, выберите ContosoAdsCommon и затем нажмите **ОК**.
