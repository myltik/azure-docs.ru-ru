<properties 
	...
	description="Настройка SSL для рабочей роли облачной службы Node.js в Azure" 
...
SSL-шифрование – это наиболее распространенный метод обеспечения безопасности данных, отправленных через Интернет. В этой теме, посвященной общим задачам, обсуждается, как определить конечную точку HTTPS для приложения Node.js, размещенного в рабочей роли в облачной службе Azure.
...
1.  В каталоге службы откройте файл определения службы (**ServiceDefinition.csdef**) и измените элемент http **InputEndpoint** в разделе **Endpoints** таким образом, чтобы обеспечить обмен данными через порт 443.
...
4.  Обновите измененную конфигурацию в облаке, выполнив повторную публикацию службы. В командной строке Azure PowerShell перейдите в каталог службы и введите **Publish-AzureServiceProject**.
...
[Настройка SSL-сертификата в конечной точке HTTPS]
  ...
  [Настройка SSL для приложения Node.js в веб-роли Azure]: /develop/nodejs/common-tasks/enable-ssl/
