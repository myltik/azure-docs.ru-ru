...
> [AZURE.NOTE]Процедуры в этом задании применяются для облачных служб Azure. Информацию о веб-сайтах см. в статье [Настройка пользовательского домена для веб-приложения службы приложений Azure](../app-service-web/web-sites-custom-domain-name.md). Для учетных записей хранения см. [Настройка пользовательского доменного имени для учетной записи хранения Azure](../storage/storage-custom-domain-name.md).

<p/>

> [AZURE.TIP]Приступите к работе быстрее — воспользуйтесь НОВЫМ [пошаговым руководством](http://support.microsoft.com/kb/2990804) Azure! Оно поможет легко и быстро связать пользовательское доменное имя с облачными службами Azure или веб-сайтами Azure И защитить обмен данными (SSL) с ними.
...
1. Чтобы найти доменное имя **.cloudapp.net**, назначенное для вашей облачной службы, используйте один из этих методов.
    ...
    * Войдите в [портал предварительной версии Azure], выберите облачную службу и найдите **URL-адрес сайта** в разделе **Общее**.
...
> [AZURE.NOTE]Посетитель сайта **www.contoso.com** никогда не увидит настоящий узел (contoso.cloudapp.net), поэтому процесс перенаправления невидим для конечного пользователя.
...
1. Чтобы получить IP-адрес для своей облачной службы, используйте один из следующих способов.
    ...
    * Войдите в [портал предварительной версии Azure], выберите облачную службу и найдите **Общедоступные IP-адреса** в разделе **Общее**.
