...
В целях тестирования можно [создать](cloud-services-certs-create.md) самозаверяющий сертификат и использовать его. Самозаверяющий сертификат не прошел аутентификацию через ЦС, и можно использовать домен cloudapp.net в качестве URL-адреса веб-сайта. Например, в нижеприведенной задаче используется самозаверяющий сертификат, в котором находится общее имя (CN) в сертификате **sslexample.cloudapp.net**. Дополнительные сведения о создании самозаверяющего сертификата с помощью диспетчера IIS см. в статье [Создание сертификата для роли][].
...
2. На панели навигации слева щелкните **Облачные службы**.
...
6. Укажите **файл** и **пароль**, а затем установите флажок **Готово**.
