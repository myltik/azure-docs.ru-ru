<properties 
	...
	metakeywords="" 
	footerexpose="" 
	description="Узнайте, как тестировать производительность облачной службы в локальном эмуляторе вычислений Azure с помощью профилировщика Visual Studio" 
	umbraconavihide="0" 
	disquscomments="1" 
	authors="kempb" 
	manager="douge" 
...
При публикации облачной службы в Azure можно собрать в Visual Studio данные профилирования,
а затем проанализировать их локально, как описано в разделе [Профилирование приложения Azure][1].
Кроме того, с помощью средств диагностики можно отслеживать различные счетчики производительности,
как описано в разделе [Создание и использование счетчиков производительности в приложении Azure][2].
...
В качестве примера добавьте в свой проект фрагмент кода, который
долго выполняется и демонстрирует явную проблему с производительностью. Например, добавьте в проект роли рабочего процесса следующий код:
...
Если папка проекта находится на сетевом диске, профилировщик попросит указать другое расположение для сохранения отчетов профилирования.
 ...
 При наличии в приложении нескольких процессов роли рабочего процесса они идентифицируются по значению processID. Запрос processID можно выполнить в программе путем обращения к объекту Process. Например, если добавить этот код в метод Run класса, производного от RoleEntryPoint в роли, можно просмотреть
журнал в пользовательском интерфейсе эмулятора вычислений, чтобы узнать, к какому процессу подключиться.
...
На этом этапе профилировщик останавливает работу, сохраняет данные в VSP-файле и выводит отчет,
показывающий анализ этих данных.
...
Вы увидите, что метод Concatenate и String.Concat занимают большую часть
времени выполнения.
...
- Если вы использовали команду профилирования из
командной строки, особенно глобальные параметры, убедитесь, что вызван VSPerfClrEnv/globaloff и завершена работа VsPerfMon.exe.
