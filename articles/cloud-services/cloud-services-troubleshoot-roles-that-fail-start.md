...
**Симптом:** симптомы отсутствия сборок и библиотек DLL могут быть такими:
...
Решение: эти проблемы можно решить тремя способами.

## Диагностика проблем отсутствующих библиотек DLL в веб-роли

Когда вы переходите на веб-сайт, развернутый в веб-роли, в браузере отображается похожее сообщение об ошибке сервера:
...
Чтобы получить удаленный доступ к роли и просмотреть ошибки полностью, вы можете использовать удаленный рабочий стол. Чтобы просмотреть ошибки с помощью удаленного рабочего стола, выполните следующие действия.

1. Убедитесь, что установлен пакет Azure SDK 1.3 или более поздней версии.

2. Во время развертывания решения с помощью Visual Studio щелкните элемент «Настроить подключения к удаленному рабочему столу...». Дополнительные сведения о настройке подключения к удаленному рабочего столу см. в статье [Использование удаленного рабочего стола с ролями Azure](https://msdn.microsoft.com/library/gg443832.aspx).

3. Когда отображается статус экземпляра **Готово**, на классическом портале Microsoft Azure щелкните один из экземпляров роли.
...
Переход на веб-сайт приведет к отображению более точных сообщений об ошибках.

* Server Error in '/' Application («Ошибка сервера в приложении "/"»).
...
## Диагностика проблем с помощью эмулятора вычислений

Вы можете использовать эмулятор вычислений Microsoft Azure, чтобы диагностировать и устранять неполадки, связанные с отсутствующими зависимостями, и ошибки web.config.
...
4. Скопируйте папку CSX и CSCFG-файл на компьютер, который вы используете для отладки проблем.

5. На чистой машине откройте командную строку пакета SDK Azure и введите `csrun.exe /devstore:start`.
...
## Диагностика проблем с помощью IntelliTrace

Для рабочих ролей и веб-ролей, которые используют платформу .NET Framework 4, вы можете использовать функцию [IntelliTrace](https://msdn.microsoft.com/library/dd264915.aspx), доступную в [Microsoft Visual Studio Ultimate](https://www.visualstudio.com/products/visual-studio-ultimate-with-MSDN-vs).
...
## Устранение отсутствующих библиотек DLL и сборок
...
4. На панели **Свойства** найдите свойство «Копировать локально» и задайте значение **True**.

5. Разверните размещенную службу повторно.

После подтверждения исправления всех ошибок службу можно развернуть, не устанавливая флажок **Включить IntelliTrace для ролей .NET 4**.
