...
> [AZURE.NOTE] Чтобы пользоваться этим учебником, не нужен опыт работы с виртуальными машинами Azure, но нужна учетная запись Azure. Вы можете создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. в разделе [Создание учетной записи Azure](http://azure.microsoft.com/develop/php/tutorials/create-a-windows-azure-account/). 
