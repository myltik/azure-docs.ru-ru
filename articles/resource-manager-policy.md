...
Определение политики может быть задано как в показанном выше примере. В качестве версии API (api-version) используйте значение *2015-10-01-preview*. Примеры и дополнительные сведения см. в статье [API REST для определений политик](https://msdn.microsoft.com/library/azure/mt588471.aspx).
...
{policy-assignment} — это имя назначения политики. В качестве версии API (api-version) используйте значение *2015-10-01-preview*.
