<properties 
	pageTitle="Средство управления производительностью приложений New Relic для Azure" 
	description="Сведения об использовании мониторинга производительности New Relic в Azure." 
...
#Средство управления производительностью New Relic для веб-приложений Azure

В этом руководстве показано, как добавить соответствующее мировым стандартам средство мониторинга производительности New Relic
на веб-сайт Azure. Вы узнаете, как быстро и просто добавить
его в свое приложение, а также найдете описание некоторых
...
New Relic - это ориентированное на разработчиков средство, осуществляющее мониторинг рабочих приложений
и предоставляющее подробные сведения об их производительности и надежности. Оно
позволяет экономить время при выявлении и диагностике проблем с производительностью и
предоставляет в ваше распоряжение сведения, необходимые для разрешения этих проблем.

Средство New Relic отслеживает время загрузки и пропускную способность веб-транзакций
как через сервер, так и через браузеры пользователей. Оно показывает, сколько времени тратится
в базе данных, анализирует медленные запросы и веб-запросы, обеспечивает мониторинг работоспособности и
оповещения, отслеживает исключения приложений и т. д.
...
7. Введите имя New Relic, которое будет отображаться в параметрах Azure,
   или используйте значение по умолчанию - **NewRelic**. Это имя должно быть уникальным в
   в списке элементов Магазина Azure, на которые у вас имеются подписки.
8. Выберите значение для региона, например **Запад США**.
9. Нажмите кнопку **Далее**.
10. В диалоговом окне **Просмотр приобретения** просмотрите сведения о плане и ценах,
...
2. Откройте консоль диспетчера пакетов, последовательно выбрав пункты **Инструменты > Диспетчер пакетов библиотеки > 
   Консоль диспетчера пакетов**. Установите свой проект как проект по умолчанию
   в верхней части окна консоли диспетчера пакетов.
...
<!--5. Необязательно: В запросе имени приложения введите имя своего приложения,
   которое будет отображаться в панели мониторинга New Relic. Или используйте имя решения по умолчанию.
