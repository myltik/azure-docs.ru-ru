...
2. На [портале предварительной версии Azure](https://portal.azure.com) откройте колонку своего веб-приложения.
3. Щелкните **Мониторинг приложений** > **New Relic**. Выберите учетную запись, созданную на предыдущем шаге, и нажмите кнопку **ОК**. 

	![](./media/store-new-relic-web-sites-dotnet-application-performce-management/configure-new-relic-integration.png)

	Когда операция сохранения завершится, нажмите кнопку **Все параметры** в колонке веб-приложения, а затем щелкните **Параметры приложения**. В раздел колонки **Параметры приложения** должен добавиться параметр **NEWRELIC_LICENSEKEY** для поддержки New Relic.
