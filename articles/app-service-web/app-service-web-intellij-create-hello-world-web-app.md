---
title: "Создание веб-приложения Hello World для Azure в IntelliJ | Документация Майкрософт"
...
---
# <a name="create-a-hello-world-web-app-for-azure-in-intellij"></a>Создание веб-приложения Hello World для Azure в IntelliJ
...
1. Запустите IntelliJ и последовательно выберите **File** (Файл), **New** (Создать) и **Project** (Проект).
   
    ![Файл > Создать > Проект][02]
2. В диалоговом окне New Project (Создание проекта) выберите **Java**, а затем установите флажок **Web Application** (Веб-приложение) и нажмите кнопку **Далее**.
    ...
    ![SDK не указан][03b]
3. Укажите имя проекта **Java-Web-App-On-Azure** и нажмите кнопку **Готово**.
   
    ![Диалоговое окно "Новый проект"][04]
4. В представлении обозревателя проектов IntelliJ разверните **Java-Web-App-On-Azure**, а затем разверните **web** и дважды щелкните **index.jsp**.
   
    ![Открытие страницы индексации][05]
5. При открытии в IntelliJ файла index.jsp добавьте код для динамического отображения фразы **Hello World!** в существующий элемент `<body>`. Обновленное содержимое элемента `<body>` должно выглядеть так:
   
    `<body><b><% out.println("Hello World!"); %></b></body>` 
6. Сохраните index.jsp.
...
2. Если вы еще не вошли в Azure из IntelliJ, появится запрос на вход в учетную запись Azure. При наличии нескольких учетных записей Azure некоторые запросы во время входа в систему могут отображаться несколько раз, даже если они выглядят одинаковыми. Если это происходит, выполните вход.
...
## <a name="starting-or-stopping-an-existing-web-app"></a>Запуск или остановка существующего веб-приложения
...
2. Разверните узел **Web Apps** (Веб-приложения). 
3. Щелкните правой кнопкой мыши необходимое веб-приложение.
4. В открывшемся контекстном меню выберите пункт **Start** (Запустить) или **Stop** (Остановить). Обратите внимание, что доступные меню контекстно зависимы, поэтому можно только остановить веб-приложение или запустить веб-приложение, которое в данный момент не запущено.
