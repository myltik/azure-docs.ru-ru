...
В этом учебнике показано, как создать веб-приложение ASP.NET 5, используя [Visual Studio Code (VSCode)](http://code.visualstudio.com//Docs/whyvscode), и развернуть его в [службе приложений Azure](../app-service/app-service-value-prop-what-is.md). ASP.NET 5 — это существенно переработанная версия ASP.NET. Это новая кроссплатформенная среда с открытым кодом, предназначенная для создания современных облачных веб-приложений с использованием .NET. Дополнительную информацию см. в статье [Введение в ASP.NET 5](http://docs.asp.net/en/latest/conceptual-overview/aspnet.html). Дополнительные сведения о веб-приложениях службы приложений Azure см. в статье [Обзор веб-приложений](app-service-web-overview.md).

[AZURE.INCLUDE [app-service-web-try-app-service.md](../../includes/app-service-web-try-app-service.md)]

## Предварительные требования  

* Установите [VSCode](http://code.visualstudio.com/Docs/setup).
* Установите [Node.js](http://nodejs.org/download/). [Node](http://nodejs.org/) — это платформа для создания быстрых масштабируемых серверных приложений с помощью JavaScript. Node — это среда выполнения (Node), а [npm](http://www.npmjs.com/) — диспетчер пакетов для модулей Node. Мы будем использовать npm в этом учебнике для формирования шаблонов веб-приложения ASP.NET 5.
* Установите Git. Можно установить систему с любого из этих сайтов: [Chocolatey](https://chocolatey.org/packages/git) или [git-scm.com](http://git-scm.com/downloads). Если вы раньше не работали с Git, скачайте его с [git-scm.com](http://git-scm.com/downloads) и при установке выберите параметр, позволяющий работать с Git используя GitBash или командную строку Windows. После установки Git задайте имя и адрес электронной почты пользователя Git, так как это потребуется далее в этом учебнике (при выполнении фиксации из VSCode).  

## Установка ASP.NET 5 и DNX
ASP.NET 5 и DNX представляют собой простой стек .NET для разработки современных облачных и веб-приложений на базе OS X, Linux и Windows. Он был создан с нуля и теперь представляет собой оптимизированную платформу для разработки и развертывания приложений в облачной или локальной среде. Платформа состоит из модульных компонентов с минимальными служебными данными, что обеспечивает гибкость при построении решений.

> [AZURE.NOTE]ASP.NET 5 и DNX (среда выполнения .NET) в OS X и Linux находятся на стадии предварительной или ранней бета-версии.

Этот учебник позволяет приступить к созданию приложений с помощью последних версий ASP.NET 5 и DNX для разработчиков. Приведенные ниже указания относятся только к Windows. Более подробные инструкции по установке для OS X, Linux и Windows см. в разделе [Установка ASP.NET 5 и DNX](https://code.visualstudio.com/Docs/ASPnet5#_installing-aspnet-5-and-dnx).
...
1. Чтобы установить диспетчер версий .NET (DNVM) в Windows, выполните в командной строке следующую команду:
	...
	Она скачает сценарий DNVM и поместит его в ваш каталог профиля пользователя.
...
4. После установки DNVM необходимо с его помощью скачать DNX для запуска приложений. Выполните в командной строке эту команду:
...
6. Если указано несколько сред выполнения DNX, то нужно выбрать активную среду выполнения DNX той версии, которую использует генератор ASP.NET 5 при создании веб-приложения далее в этом учебнике. Для этого введите в командной строке следующее.

		dnvm use 1.0.0-beta4 –p

> [AZURE.NOTE]Более подробные инструкции по установке для OS X, Linux и Windows см. в разделе [Установка ASP.NET 5 и DNX](https://code.visualstudio.com/Docs/ASPnet5#_installing-aspnet-5-and-dnx).
...
1. Откройте командную строку с правами администратора и перейдите к папке, в которой нужно создать проект ASP.NET.
...
6. Откройте VSCode, выполнив в командной строке следующую команду.
...
7. В VSCode щелкните **Файл > Открыть папку** и выберите папку, содержащую веб-приложение ASP.NET.
	...
	VSCode загрузит проект и отобразит файлы проекта в окне **Просмотр**.
...
9. В **Палитре команд** введите следующие команды:
	...
	Когда вы начнете вводить текст, вы увидите список подходящих команд с полным текстом.

	![Команда Restore](./media/web-sites-create-web-app-using-vscode/04-dnu-restore.png)

	Команда Restore устанавливает пакеты NuGet, необходимые для запуска приложения. По завершении в командной строке отобразится сообщение **Восстановление завершено**.
...
1. Чтобы запустить приложение локально, введите в **Палитре команд** следующую команду:

		dnx: kestrel - (SampleWebApp, Microsoft.AspNet.Hosting --server Kestrel --server.urls http://localhost:5001

	В командном окне отобразится состояние *Запущено*. Если в командном окне состояние *Запущено* не отображается, убедитесь, что в проекте нет ошибок (в левом углу VSCode).
...
2. Щелкните команду **СОЗДАТЬ** в нижнем левом углу.
...
5. Выберите существующий **план службы приложений** или создайте новый. При создании нового плана выберите ценовую категорию, расположение и другие параметры. Дополнительные сведения о планах службы приложений см. в статье [Подробный обзор планов службы приложений Azure](../app-service/azure-web-sites-web-hosting-plans-in-depth-overview.md).
...
Git — это распределенная система управления версиями, которую можно использовать для развертывания веб-приложений службы приложений Azure. Код, созданный для веб-приложения, будет храниться в локальном репозитории Git, а для развертывания кода в Azure он будет передаваться в удаленный репозиторий.

1. Войдите на [портал предварительной версии Azure](https://portal.azure.com).

2. Щелкните **Просмотреть все**.
...
8. Если ранее вы не настроили учетные данные развертывания для публикации веб-приложения или другого приложения службы приложений, сделайте это сейчас, как показано ниже.
	...
	* Щелкните **FTP**, чтобы настроить учетные данные развертывания.

	* Укажите имя пользователя и пароль. Этот пароль потребуется позднее при настройке Git.

	* Щелкните **Сохранить**.

	![Учетные данные развертывания Azure](./media/web-sites-create-web-app-using-vscode/16-azure-credentials.png)

9. В колонке веб-приложения щелкните **Параметры > Свойства**. В разделе **URL-АДРЕС** отображается URL-адрес удаленного репозитория Git, в который выполняется развертывание.
10.  
...
2. Выберите **Инициализировать репозиторий Git**, чтобы задействовать управление версиями Git в рабочей области.

	![Инициализация Git](./media/web-sites-create-web-app-using-vscode/19-initgit.png)

3. Добавьте сообщение о фиксации и щелкните значок флажка **Фиксировать все**.

	![«Фиксировать все» в Git](./media/web-sites-create-web-app-using-vscode/20-git-commit.png)

4. После завершения обработки Git вы увидите, что в окне Git в разделе **Изменения** отсутствуют файлы.

	![Git без изменений](./media/web-sites-create-web-app-using-vscode/no-changes.png)

5. Откройте командную строку и перейдите в каталог, в котором размещено веб-приложение.

6. Создайте внешнюю ссылку для отправки обновлений в веб-приложение, используя URL-адрес Git (заканчивающийся на «.git»), скопированный ранее.

		git remote add azure [URL for remote repository]

7. Отправьте обновления в Azure с помощью следующей команды:

		git push azure master

	Появится запрос на ввод ранее заданного пароля. **Примечание. Ваш пароль не будет отображаться.**

	В результате выполнения этой команды выводится сообщение об успешном развертывании:

		remote: Deployment successful.
		To https://user@testsite.scm.azurewebsites.net/testsite.git
		[new branch]      master -> master

> [AZURE.NOTE]При внесении изменений в приложение можно выполнять повторную публикацию. Для этого нужно щелкнуть галочку **Фиксировать все** в VSCode, а затем ввести команду **git push azure master** в командной строке.
...
* На портале предварительной версии Azure перейдите к колонке веб-приложения и щелкните **Обзор** для просмотра приложения в браузере по умолчанию.

![Веб-приложение Azure](./media/web-sites-create-web-app-using-vscode/21-azurewebapp.png)

## Сводка
Из этого учебника вы узнали, как создавать веб-приложения в VSCode и развертывать их в Azure. Дополнительную информацию о VSCode см. в статье [Почему Visual Studio Code?](https://code.visualstudio.com/Docs/). Дополнительную информацию о веб-приложениях службы приложений см. в статье [Обзор веб-приложений](app-service-web-overview.md).
