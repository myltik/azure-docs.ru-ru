...
В этом учебнике показано, как создать веб-приложение ASP.NET 5, используя [Visual Studio Code (VSCode)](http://code.visualstudio.com//Docs/whyvscode), и развернуть его в [службе приложений Azure](../app-service/app-service-value-prop-what-is.md). ASP.NET 5 является существенно переработанной версией ASP.NET. ASP.NET 5 — это новая кроссплатформенная платформа с открытым кодом, предназначенная для создания современных облачных веб-приложений с использованием .NET. Дополнительную информацию см. в статье [Введение в ASP.NET 5](http://docs.asp.net/en/latest/conceptual-overview/aspnet.html). Дополнительную информацию о веб-приложениях службы приложений Azure см. в статье [Обзор веб-приложений](app-service-web-overview.md).
...
* Установка [Node.js](http://nodejs.org/download/). [Node](http://nodejs.org/) — это платформа для создания быстрых масштабируемых серверных приложений с помощью JavaScript. Node — это среда выполнения (Node), а [npm](http://www.npmjs.com/) — диспетчер пакетов для модулей Node. Для формирования шаблона веб-приложения ASP.NET 5 в этом учебнике будет использоваться npm.
* Установка Git. Вы можете установить эту систему из любого из таких расположений: [Chocolatey](https://chocolatey.org/packages/git) или [git-scm.com](http://git-scm.com/downloads). Если вы не знакомы с Git, выберите [git-scm.com](http://git-scm.com/downloads) и параметр, чтобы использовать Git с GitBash из командной строки Windows. После установки Git также необходимо задать имя пользователя Git и его адрес электронной почты, так как это потребуется далее в этом учебнике (при выполнении фиксации из VSCode).  
...
1. Чтобы установить диспетчер версий .NET (DNVM) в Windows, выполните следующую команду в командной строке:
...
3. Чтобы проверить расположение DNVM, откройте командную строку и введите следующее.

		where dnvm

	В командной строке будет отображаться путь, аналогичный следующему:

	![dnvm location](./media/web-sites-create-web-app-using-vscode/00-where-dnvm.png)

4. Теперь, установив DNVM, необходимо с его помощью скачать DNX для запуска приложений. В командной строке выполните следующее.
...
6. Если указано несколько сред выполнения DNX, чтобы задать активную среду выполнения DNX той версии, которую использует генератор ASP.NET 5 при создании веб-приложения далее в этом учебнике, введите следующее в командной строке.
...
1. Откройте командную строку, используя права администратора, и перейдите к расположению, в котором нужно создать проект ASP.NET.
...
5. Задайте имя **SampleWebApp** для нового веб-приложения ASP.NET. Это имя используется в учебнике, и поэтому, если выбрать другое имя, нужно будет заменить его для каждого вхождения **SampleWebApp**. При нажатии клавиши &lt;ВВОД> Yeoman создаст новую папку с именем **SampleWebApp** и файлы, необходимые для нового приложения.
...
Теперь, когда веб-приложение создано и для приложения получены все пакеты NuGet, веб-приложение можно запустить локально.
...
5. Откройте браузер и перейдите по следующему URL-адресу:
	...
	**http://localhost:5001**
	...
	Страница веб-приложения по умолчанию будет выглядеть следующим образом.
...
Следующие шаги помогут вам создать веб-приложение на портале предварительной версии Azure.
...
4. Введите значение в поле **Имя**, например **SampleWebAppDemo**. Обратите внимание, что это имя должно быть уникальным и портал затребует это, когда вы попытаетесь ввести имя. Поэтому, если вы введете другое значение, вам необходимо будет заменить это значение для каждого вхождения **SampleWebAppDemo**, приведенного в этом учебнике.
...
5. В колонке веб-приложения прокрутите содержимое вниз, чтобы перейти к разделу **Развертывание**, и щелкните **Настройка непрерывного развертывания**.
...
9. В колонке веб-приложения щелкните **Параметры > Свойства**. URL-адрес удаленного репозитория Git, в который выполняется развертывание, отображается в разделе **
10.  URL-адрес**.

10. Копируйте значение в поле **URL-АДРЕС GIT** для последующего использования в этом учебнике.
...
В этом разделе вы создадите локальный репозиторий Git и выполните принудительную отправку содержимого из этого репозитория в Azure, чтобы развернуть веб-приложение в Azure.
...
5. Откройте командную строку и измените каталоги на каталог, в котором содержится веб-приложение.
...
> [AZURE.NOTE]При внесении изменений в приложение можно выполнять повторную публикацию. Для этого нужно щелкнуть значок флажка **Фиксировать все** в VSCode, а затем ввести команду **git push azure master** в командной строке.

## Запуск приложения в Azure
Теперь, когда веб-приложение развернуто, давайте запустим приложение, размещенное в Azure.

Это можно сделать двумя способами:

* Откройте браузер и введите имя веб-приложения следующим образом.   
...
## Сводка
В этом учебнике вы узнали, как создавать веб-приложение в VSCode и развертывать его в Azure. Дополнительную информацию о VSCode см. в разделе [Почему Visual Studio Code?](https://code.visualstudio.com/Docs/). Дополнительную информацию о веб-приложениях службы приложений Azure см. в статье [Обзор веб-приложений](app-service-web-overview.md).
