...
</pre>Это значение представляет собой URI, который ASP.NET OAuth использует для обмена данными с сервером Google OAuth. Не забудьте URL-адрес SSL, который вы использовали выше (**https://localhost:44300/**, если только не создавались другие проекты SSL).  
...
namespace ContactManager
{
    public partial class Startup {
        ...
        {
            // Настройка контекста БД и диспетчера пользователей для использования одного экземпляра на запрос
            ...
            // и использовать файл cookie для временного хранения информации о пользователе, вошедшем в систему с помощью стороннего поставщика входа
            // Настройка входа в файле cookie
            ...
            // Раскомментируйте следующие строки, чтобы включить ведение журнала с помощью сторонних поставщиков входа
...
5. После сохранения всех изменений в **консоли диспетчера пакетов** выполните следующую команду:  
	<pre class="prettyprint">
Update-Database
</pre>Этот код создает новую роль с именем `canEdit`, а также создает нового локального пользователя с адресом электронной почты canEditUser@wideworldimporters.com. Затем код добавляет canEditUser@wideworldimporters.com для роли `canEdit`. Дополнительные сведения см. на [странице ресурсов ASP.NET Identity](http://www.asp.net/identity).
...
3. Обновите ссылки в *Contacts/Default.aspx* путем добавления "Admin/" перед ссылками на страницы, которые направляют на *Insert.aspx*, *Edit.aspx* и *Delete.aspx*:  
	<pre class="prettyprint">
...
7. Замените существующее содержимое XML в файле *Web.config* на следующее:
	<pre class="prettyprint">
&lt;?xml version="1.0"?>
&lt;configuration>
  &lt;system.web>
