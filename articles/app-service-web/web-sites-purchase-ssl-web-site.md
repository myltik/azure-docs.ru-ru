...
По умолчанию в **[службе приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714)** уже включен HTTPS для вашего веб-приложения с групповым сертификатом для домена *.azurewebsites.net. Если вы не планируете настраивать пользовательский домен, можно воспользоваться сертификатом HTTPS по умолчанию. Однако, как и все *[групповые домены](https://casecurity.org/2014/02/26/pros-and-cons-of-single-domain-multi-domain-and-wildcard-certificates), этот домен не так безопасен, как пользовательский домен с собственным сертификатом. Служба приложений Azure теперь позволяет легко приобрести сертификат SSL и управлять им прямо на портале Azure. В этой статье объясняется, как приобрести и настроить сертификат SSL для вашей **[службы приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714)** всего за 3 простых шага.
...
##<a> Приобретение, хранение и назначение сертификата SSL для пользовательского домена </a>
Чтобы включить протокол HTTPS для пользовательского домена, например contoso.com, необходимо сначала **[настроить пользовательское имя домена в службе приложений Azure](web-sites-custom-domain-name.md)**.
...
##<a name="bkmk_purchasecert"></a>Шаг 0. Размещение заказа на сертификат SSL
...
##<a name="bkmk_StoreKeyVault"></a>Шаг 1. Размещение сертификата в хранилище ключей Azure

На этом шаге вы узнаете, как разместить приобретенный сертификат SSL в выбранном хранилище ключей Azure.

1.	После завершения покупки сертификата SSL необходимо вручную открыть колонку ресурса **Сертификаты службы приложений**, перейдя на нее снова (см. шаг 1 выше).   
    ...
    Вы увидите, что состояние сертификата — **"Ожидание выдачи"**. Это вызвано тем, что нужно выполнить еще некоторые действия, прежде чем вы сможете воспользоваться этим сертификатом.
 
2. Щелкните **"Конфигурация сертификата"** в колонке "Свойства сертификата" и затем **"Шаг 1. Размещение"** для размещения этого сертификата в хранилище ключей Azure.

3.	В колонке **"Состояние хранилища ключей"** щелкните **"Репозиторий хранилища ключей"**, чтобы выбрать существующее хранилище ключей для размещения этого сертификата, или **"Создать новое хранилище ключей"**, чтобы создать новое хранилище ключей в той же подписке и группе ресурсов.
 
    ![вставить изображение для создания нового хранилища ключей](./media/app-service-web-purchase-ssl-web-site/NewKV.jpg)
 
    > [AZURE.NOTE]
    Расходы для хранения этого сертификата в хранилище ключей будут минимальными. Стоимость составит около 0,03 долларов США. Дополнительные сведения см. в разделе **[Сведения о ценах для хранилища ключей Azure](https://azure.microsoft.com/pricing/details/key-vault/)**.

4. После выбора репозитория хранилища ключей для размещения сертификата разместите сертификат, нажав кнопку **"Разместить"** в верхней части колонки **"Состояние хранилища ключей"**.
...
1.	Щелкните **"Шаг 2. Проверка"** в колонке **"Конфигурация сертификата"**. Существует 4 типа проверки домена, которые поддерживаются сертификатами службы приложений.
    ...
    * **Проверка службы приложений** 
        ...
        1. Этот способ наиболее удобен, если у вас уже есть **пользовательский домен, назначенный приложениям службы приложений**. При использовании этого метода будет выведен список всех приложений службы приложений, соответствующих этому критерию. Например, в данном случае **contosocertdemo.com** — пользовательский домен, назначенный приложению службы приложений под названием **"ContosoCertDemo"**. Поэтому это единственное приложение службы приложений, которое есть в списке. Если бы развертывание выполнялось в нескольких регионах, то был бы выведен список для всех регионов.
            > [AZURE.NOTE]
        ...
        2. Нажмите кнопку **"Проверить"**, чтобы завершить этот шаг.
        3. Нажмите кнопку **"Обновить"**, чтобы обновить состояние сертификата после завершения проверки. Проверка может занять несколько минут.
        
            ![вставить изображение для проверки службы приложений](./media/app-service-web-purchase-ssl-web-site/AppVerify.jpg)

    * **Проверка домена**

        1. Этот метод наиболее удобен **ТОЛЬКО В ТОМ СЛУЧАЕ**, если вы приобрели **[пользовательский домен в службе приложений Azure.](custom-dns-web-site-buydomains-web-app.md)**
    ...
    * **Проверка почты**
        ...
        1. Проверочное сообщение уже было отправлено по адресам электронной почты, связанным с этим пользовательским доменом.
         
        2. Откройте его и щелкните ссылку для проверки, чтобы завершить шаг проверки электронной почты.
        
        3. Если необходимо повторно отправить проверочное сообщение, нажмите кнопку **"Отправить сообщение повторно"**.
...
5.	В разделе **Сертификаты** щелкните **Импортировать сертификат** и выберите сертификат, который вы приобрели.
    ...
    ![вставить изображение для импорта сертификата](./media/app-service-web-purchase-ssl-web-site/ImportCertificate.jpg)
...
7. Чтобы сохранить изменения и активировать SSL, нажмите кнопку *Сохранить*.
