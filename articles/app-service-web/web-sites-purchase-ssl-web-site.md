...
1.	Щелкните **"Шаг 2. Проверка"** в колонке **"Конфигурация сертификата"**. Существует 4 типа проверки домена, которые поддерживаются сертификатами службы приложений.
    ...
    * **Проверка службы приложений** 
        ...
        * Этот способ наиболее удобен, если у вас уже есть **пользовательский домен, назначенный приложениям службы приложений**. При использовании этого метода будет выведен список всех приложений службы приложений, соответствующих этому критерию. Например, в данном случае **contosocertdemo.com** — пользовательский домен, назначенный приложению службы приложений под названием **"ContosoCertDemo"**. Поэтому это единственное приложение службы приложений, которое есть в списке. Если бы развертывание выполнялось в нескольких регионах, то был бы выведен список для всех регионов.
...
##<a name="bkmk_Rekey"></a>Повторное создание ключей и синхронизация сертификата

1. Если вам когда-либо придется повторно создать ключи сертификата по соображениям безопасности, просто выберите **Rekey and Sync** \(Повторно создать ключи и синхронизировать\) в колонке **Свойства сертификата**. 

2. Для запуска процесса нажмите кнопку **Rekey** \(Повторно создать ключи\). Этот процесс может занять от 1 до 10 минут.
