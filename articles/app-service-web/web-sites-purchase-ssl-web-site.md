...
На этом шаге вы узнаете, как разместить приобретенный SSL-сертификат в выбранном Azure Key Vault.
...
2. Щелкните **Конфигурация сертификата** в колонке "Свойства сертификата" и затем **Шаг 1. Хранилище**, чтобы сохранить этот сертификат в Azure Key Vault.
3. В колонке **Состояние Key Vault** щелкните **Репозиторий Key Vault**, чтобы выбрать существующее Key Vault для сохранения этого сертификата, или щелкните **Создать новое Key Vault**, чтобы создать Key Vault в той же подписке и группе ресурсов.
...
1. Щелкните **Шаг 2. Проверка** в колонке **Конфигурация сертификата**. Есть 3 типа проверки домена, которые поддерживаются сертификатами службы приложений.
   ...
   * **Проверка вручную**    
      ...
      **Проверка веб-страницы HTML (работает только с номером SKU сертификата уровня "Стандартный")**
        ...
        * Создайте HTML-файл **starfield.html**.
        * Содержимое этого файла должно полностью совпадать с маркером проверки домена. (Маркер можно скопировать из колонки состояния проверки домена.)
        * Передайте этот файл в корневую папку веб-сервера, связанного с вашим доменом: **/.well-known/pki-validation/starfield.html**.
        * Нажмите кнопку **"Обновить"** , чтобы обновить состояние сертификата после завершения проверки. Проверка может занять несколько минут.
      ...
      **Проверка записей типа TXT DNS**
        ...
        * С помощью диспетчера DNS создайте запись типа TXT в поддомене **‘@’** со значением, идентичным **маркеру проверки домена**.
        * Нажмите кнопку **"Обновить"** , чтобы обновить состояние сертификата после завершения проверки. Проверка может занять несколько минут.
          
          Например, для проверки группового сертификата с именем узла **\*.contosocertdemo.com** или **\*.subdomain.contosocertdemo.com** и маркером проверки домена **tgjgthq8d11ttaeah97s3fr2sh** нужно создать запись типа TXT на **contosocertdemo.com** со значением **tgjgthq8d11ttaeah97s3fr2sh**.     
...
Можно создать локальную копию (PFX-файл) сертификата службы приложений, чтобы использовать ее с другими службами Azure. Дополнительные сведения см. в **[нашей записи блога](https://blogs.msdn.microsoft.com/appserviceteam/2017/02/24/creating-a-local-pfx-copy-of-app-service-certificate/)**.

## <a name="bkmk_Renew"></a>Автообновление сертификата службы приложений
Чтобы включить параметры автообновления для сертификата или обновить сертификат вручную, просто выберите параметр **Параметры автовозобновления** в колонке **Свойства сертификата**. 


  ![вставить изображение для создания с помощью кнопки "Обзор"](./media/app-service-web-purchase-ssl-web-site/autorenew.png)

Включите параметр **Автовозобновление**, если вы хотите, чтобы сертификат автоматически обновлялся до истечения его срока действия. Это параметр по умолчанию. Если его включить, мы попытаемся обновить ваш сертификат за 90 дней до истечения срока действия. Если вы создали привязки SSL к приложениям службы приложений с помощью портала Azure, то они также будет обновлены с учетом нового сертификата, когда он будет готов (как и при повторном создании ключей или синхронизации). С другой стороны, если вы хотите самостоятельно заниматься обновлением, то следует отключить этот параметр. Сертификат службы приложений можно обновить вручную только в том случае, если его срок действия не превышает 90 дней.
