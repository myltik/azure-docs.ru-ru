...
Основная цель заключается в том, чтобы сделать невозможным общий доступ к внутреннему приложению API из Интернета, разрешив вызывать API только вышестоящими веб-приложениями.  В подсетях, содержащих среды службы приложений, можно использовать [группы безопасности сети (NSG)][NetworkSecurityGroups], чтобы ограничить общий доступ к приложениям API.
...
Сначала для инфраструктуры управления Azure добавляется одно явно разрешающее правило, как описано в статье, посвященной [входящему трафику][InboundTraffic] в среде службы приложений.
...
Основные сведения о средах службы приложений и исходящих IP-адресах см. [здесь][NetworkArchitecture].

Дополнительные сведения о сетевых портах, используемых в средах службы приложений, см. [здесь][InboundTraffic].
