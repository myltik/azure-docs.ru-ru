...
> [!NOTE]
> Процедуры в этой задаче применяются к [веб-приложениям службы приложений]. Информацию об облачных службах см. в статье [Создание приложения чата на Node.js с использованием Socket.IO в облачной службе Azure].
> 
> 
...
2. Извлеките архив и скопируйте каталог **examples\\chat** в новое место. Например, **\\node\\chat**.
...
## Масштабирование
Приложения Socket.IO могут быть масштабированы при использовании **адаптера** для распределения сообщений и событий между несколькими экземплярами приложения. Хотя доступно несколько адаптеров, адаптер [socket.io-redis] можно легко использовать с функцией кэша Azure Redis Cache.
...
2. Отредактируйте файл **app.js**, добавив следующий текст сразу после `var io = require('socket.io')(server);`
   ...
   > [!NOTE]
   > Хотя адаптер **socket.io-redis** может взаимодействовать непосредственно с Redis, текущая версия не поддерживает проверку подлинности, требуемую кэшем Redis для Azure. Поэтому первоначальное соединение создается с помощью модуля **redis**, а затем клиент передается адаптеру **socket.io-redis**.
   > 
