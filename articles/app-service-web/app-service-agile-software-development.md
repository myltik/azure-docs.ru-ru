...
Рабочая и промежуточная среды определяются шаблоном в файле [*&lt;repository\_root>*/ARMTemplates/ProdandStage.json](https://github.com/azure-appservice-samples/ToDoApp/blob/master/ARMTemplates/ProdAndStage.json).

Среды разработки и тестирования определяются шаблоном в файле [*&lt;repository\_root>*/ARMTemplates/Dev.json](https://github.com/azure-appservice-samples/ToDoApp/blob/master/ARMTemplates/Dev.json).
...
> [AZURE.NOTE]Для прохождения этого учебника необходима учетная запись Azure. Вы можете [открыть учетную запись Azure бесплатно](/pricing/free-trial/?WT.mc_id=A261C142F) — вы получаете кредиты, которые можно использовать, чтобы попробовать платные службы Azure, и даже после израсходования кредитов вы сохраняете учетную запись и возможность использовать бесплатные службы Azure, такие как веб-приложения. Вы можете [активировать преимущества подписчика Visual Studio](/pricing/member-offers/msdn-benefits-details/?WT.mc_id=A261C142F) — ваша подписка Visual Studio каждый месяц приносит вам баллы, которые можно использовать для оплаты служб Azure.
>
...
4.	После создания локального клона перейдите в каталог *&lt;repository\_root>*\\ARMTemplates и запустите сценарий deploy.ps1 следующим образом:
...
4.	При появлении запроса введите желаемое имя пользователя и пароль для доступа к базе данных.
	...
	>[AZURE.TIP]Узнайте, как сценарий *&lt;repository\_root>*\\ARMTemplates\\Deploy.ps1 создает ресурсы с уникальными идентификаторами. Тот же подход можно использовать для создания клонов одного и того же развертывания, не беспокоясь о конфликтующих именах ресурсов.
...
7.	После выполнения сценария, вернитесь назад и перейдите по адресу внешнего интерфейса (http://ToDoApp*&lt;unique_string>*master.azurewebsites.net/), чтобы посмотреть на работу приложения в рабочей среде.
 
5.	Перейдите на [портал Azure](https://portal.azure.com) и посмотрите на результаты.

	Вы увидите два веб-приложения в одной группе ресурсов, имя одного из них будет иметь суффикс `Api`. Если вы посмотрите на представление группы ресурсов, вы также увидите базу данных SQL и сервер, план службы приложений и промежуточные слоты для веб-приложений. Просмотрите различные ресурсы и сравните их с файлом *&lt;repository\_root>*\\ARMTemplates\\ProdAndStage.json, чтобы узнать, как они настроены в шаблоне.
...
>
>1.	Создайте собственное разветвление репозитория в GitHub (см. статью [Разветвление репозитория](https://help.github.com/articles/fork-a-repo/)).
...
2.	Внесите простое изменение на уровне пользовательского интерфейса приложения, прописав в коде использование списков [начальной загрузки](http://getbootstrap.com/components/). Откройте файл *&lt;repository\_root>*\\src\\MultiChannelToDo.Web\\index.cshtml и внесите изменения, выделенные желтым цветом ниже:
...
Перейдите в колонку веб-приложения для тестовой среды, чтобы увидеть новую фиксацию (объединенную с ветвью NewUpdate), отправленную в тестовую среду. Затем нажмите кнопку **Обзор**, чтобы увидеть изменения стиля в динамическом приложении Azure.
