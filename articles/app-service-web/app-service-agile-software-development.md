...
Рабочая и промежуточная среды определяются шаблоном в файле [*& lt; repository\_root >*/ARMTemplates/ProdandStage.json](https://github.com/azure-appservice-samples/ToDoApp/blob/master/ARMTemplates/ProdAndStage.json).
...
4.	После создания локального клона перейдите в каталог *& lt; repository\_root >*\\ARMTemplates и запустите сценарий deploy.ps1 следующим образом:
...
4.	При появлении запроса введите желаемое имя пользователя и пароль для доступа к базе данных.
	...
	>[AZURE.TIP]Посмотрите на сценарий *& lt; repository\_root >*\\ARMTemplates\\Deploy.ps1, чтобы увидеть, как он создает ресурсы с уникальными идентификаторами. Тот же подход можно использовать для создания клонов одного и того же развертывания, не беспокоясь о конфликтующих именах ресурсов.
...
2.	Внесите простое изменение на уровне пользовательского интерфейса приложения путем изменения кода для использования списков [начальной загрузки](http://getbootstrap.com/components/). Откройте *&lt;repository\_root>*\\src\\MultiChannelToDo.Web\\app\\index.cshtml и внесите изменения, выделенные желтым цветом ниже:
