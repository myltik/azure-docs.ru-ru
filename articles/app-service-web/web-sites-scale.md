<properties 
	pageTitle="Увеличение масштаба приложения в Azure" 
...
В этой статье показано, как масштабировать приложение в службе приложений Azure. Существует два рабочих процесса масштабирования, и в этой статье рассматривается первый из них (увеличение масштаба).

- [Увеличение масштаба](https://en.wikipedia.org/wiki/Scalability#Horizontal_and_vertical_scaling) — это получение дополнительных ресурсов, в том числе ЦП, памяти и дискового пространства, а также дополнительных возможностей, таких как выделенные виртуальные машины, личные домены и сертификаты, промежуточные слоты, автомасштабирование и т. д. Вы увеличиваете масштаб приложения путем изменения ценовой категории плана службы приложений, к которой принадлежит ваше приложение.
- [Развертывание](https://en.wikipedia.org/wiki/Scalability#Horizontal_and_vertical_scaling) — это увеличение количества экземпляров виртуальных машин, на которых работает приложение. В зависимости от ценовой категории вы можете развернуть приложение на виртуальных машинах в количестве до 20 экземпляров. Использование [сред службы приложений](../app-service/app-service-app-service-environments-readme.md) на уровне **Премиум** позволит увеличить масштаб, развертывая до 50 экземпляров. Дополнительные сведения см. в статье [Масштабирование числа экземпляров вручную или автоматически](../azure-portal/insights-how-to-scale.md). В ней вы узнаете, как использовать автомасштабирование, которое позволяет масштабировать число экземпляров автоматически на основе предварительно определенных правил и расписаний.
...
4. Прокрутите и выберите нужную категорию, а затем нажмите кнопку **Выбрать**.
...
2. Затем в области **Сводка** колонки группы ресурсов щелкните один из ресурсов, который требуется масштабировать. На приведенном ниже снимке экрана показаны ресурс базы данных SQL и ресурс службы хранилища Azure.
...
3. Для ресурса базы данных SQL щелкните **Параметры** > **Ценовая категория**, чтобы выбрать масштаб ценовой категории.
	...
	Можно также включить [георепликацию](../sql-database/sql-database-geo-replication-overview.md) для базы данных SQL.
...
- Видео о масштабировании приложений службы приложений:
