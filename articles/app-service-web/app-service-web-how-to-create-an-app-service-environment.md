...
О каждом из этих элементов важно помнить следующее. 
1) Имя ASE будет использоваться в дочернем домене для любых приложений, созданных в этой среде ASE. 
2) Все приложения, созданные в среде ASE, будут включены в ту же подписку, что и сама среда ASE. 
3) При отсутствии доступа к подписке, с помощью которой была создана среда ASE, вы не сможете использовать ASE для создания приложений. 
4) Виртуальные сети, используемые для размещения ASE, должны быть классическими региональными виртуальными сетями v1.
5) Подсеть, используемая для размещения ASE, не должна содержать никакие другие вычислительные ресурсы, кроме одной среды ASE.
