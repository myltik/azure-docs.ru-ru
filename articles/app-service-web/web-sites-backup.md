...
## <a name="backup-just-part-of-your-app"></a>Создание резервной копии части приложения
...
![Папка изображений][ImagesFolder]
...
3. Отправьте этот файл в каталог `D:\home\site\wwwroot\` своего сайта с помощью протокола [ftp](web-sites-deploy.md#ftp) или любым другим способом. При желании можно создать файл непосредственно в `http://{yourapp}.scm.azurewebsites.net/DebugConsole` и вставить в него содержимое.
4. Выполните архивацию привычным для вас способом — [вручную](#create-a-manual-backup) или [автоматически](#configure-automated-backups).
...
Резервная копия базы данных приложения хранится в корне ZIP-файла. Для базы данных SQL это файл BACPAC (без расширения), и его можно импортировать. Инструкции по созданию новой базы данных SQL на основе экспорта BACPAC-файла см. в статье [Импорт файла BACPAC для создания новой пользовательской базы данных](http://technet.microsoft.com/library/hh710052.aspx).
