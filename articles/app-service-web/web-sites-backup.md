...
Иногда резервное копирование всего содержимого сайта не требуется, особенно при регулярном выполнении резервного копирования сайта или если размер содержимого сайта превышает 10 ГБ (максимальный объем данных, которые можно заархивировать в ходе одной процедуры резервного копирования).

Например, вам, возможно, не нужно создавать резервную копию файлов журнала. Или же вы [настроили еженедельное резервное копирование](https://azure.microsoft.com/ru-ru/documentation/articles/web-sites-backup/#configure-automated-backups) и вам ни к чему заполнять свою учетную запись хранения неизменяемым статическим содержимым, включающим старые записи в блоге или изображения.

Частичное резервное копирование позволяет выбрать только те файлы, для которых нужно создать резервную копию.

###Выбор файлов, для которых не нужно создавать резервную копию
Вы можете создать список файлов и папок, которые будут исключены из резервной копии.
...
Приведенные ниже указания описывают процесс создания файла _backup.filter с помощью консоли Kudu. Вы также можете воспользоваться привычным вам методом развертывания, чтобы сохранить файл в нужное расположение.

###Что следует делать
Имеется сайт, содержащий неизменяемые файлы журнала и статические изображения, собранные за последние несколько лет.

Полная резервная копия сайта, включающая старые изображения, уже есть. Выполнять резервное копирование сайта нужно каждый день, но зачем платить за хранение неизменяемых файлов журнала или файлов статических изображений?

![Папка журналов][LogsFolder] ![Папка с изображениями][ImagesFolder]
	
Ниже описано, как исключить эти файлы из резервной копии.

####Определение файлов и папок, для которых не нужно создавать резервную копию
Это очень просто. Если вы не хотите включать в резервную копию все файлы журналов, исключите `D:\home\site\wwwroot\Logs`.

Для всех веб-приложений Azure есть еще одна папка файла журнала, расположенная в каталоге `D:\home\LogFiles`. Ее также нужно исключить.

Кроме того, не нужно повторно создавать резервную копию изображений, добавленных в предыдущие годы. Следовательно, в список также следует включить `D:\home\site\wwwroot\Images\2013` и `D:\home\site\wwwroot\Images\2014`.

Наконец, вы можете исключить из резервной копии файл brand.png (находится в папке с изображениями), — просто в качестве примера того, как исключать отдельные файлы. Файл расположен в каталоге `D:\home\site\wwwroot\Images\brand.png`.

Итак, вы выбрали следующие папки, которые будут исключены из резервной копии:
...
Сохраните список исключаемых из резервной копии файлов и папок в специальный файл с именем _backup.filter. Создайте файл и поместите его в расположение `D:\home\site\wwwroot_backup.filter`.

Включите все файлы и папки, исключаемые из резервной копий, в файл _backup.filter. Можно добавить относительный путь к исключаемым из резервной копии папке или файлу из каталога D:\home, указав каждый путь в новой строке.
...
Все попавшие под фильтр файлы и папки (включенные в файл _backup.filter) будут исключены из резервной копии. Это означает, что файлы журналов и файлы изображений за 2013 и 2014 гг.. больше не будут архивироваться.

###Восстановление резервной копии сайта
Восстановление частичных резервных копий сайта происходит так же, как и [восстановление обычных резервных копий](https://azure.microsoft.com/ru-ru/documentation/articles/web-sites-restore/). Алгоритм один и тот же.
...
![Вы можете создать копию базы данных на основе содержимого или данных, но не тогда, когда запущена платформа Ghost. Остановите ее сначала.][GhostUpgradeWarning]
...
Если бы я сперва попытался выполнить восстановление в тестовом слоте, я бы увидел эту проблему и не утратил бы все свои записи.
