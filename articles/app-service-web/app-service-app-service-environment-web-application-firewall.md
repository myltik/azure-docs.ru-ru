...
Инструкции по настройке среды службы приложений см. в соответствующей [документации](app-service-web-how-to-create-an-app-service-environment.md). После создания среды службы приложений вы можете создавать в ней [веб-приложения](app-service-web-overview.md), [приложения API](app-service-api-apps-why-best-platform.md) и [мобильные приложения](app-service-mobile-value-prop-preview.md), защищенные брандмауэром WAF, который мы настроим в следующем разделе.
...
После создания в облачной службе двух и более экземпляров виртуальных машин с WAF вы можете использовать [портал управления Azure](https://portal.azure.com), чтобы добавить конечные точки HTTP и HTTPS, которые используются вашим приложением, как показано на рисунке ниже.
...
Если приложение требует проверки подлинности, убедитесь, что у вас есть ресурсы для проверки доступности приложения, которые не требуют проверки подлинности через диспетчер трафика. URL-адрес можно указать в разделе "Настройка" [портала управления Azure](https://manage.azure.com), как показано ниже.
...
Подробные сведения об ограничении трафика в среду службы приложений только от WAF с помощью виртуального IP-адреса облачной службы см. в [документации по управлению входящим трафиком](app-service-app-service-environment-control-inbound-traffic.md). Ниже приведен пример команды Powershell для выполнения этой задачи для TCP-порта 80.
