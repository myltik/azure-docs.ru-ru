---
title: "Создание веб-приложения PHP-SQL и развертывание в службе приложений App с помощью Git"
...
7. Определив параметры веб-приложения, нажмите кнопку **Создать**.
    ...
    После создания веб-приложения на кнопке **Уведомления** появится зеленая надпись **Выполнено** и откроется колонка группы ресурсов, в которой будет видно, что и веб-приложение, и база данных SQL находятся в группе.
8. Щелкните значок веб-приложения в колонке группы ресурсов, чтобы открыть колонку веб-приложения.
...
10. В колонке **Настройка** выберите **Свойства**, чтобы увидеть удаленный URL-адрес Git, который понадобится для развертывания приложения PHP позднее.
...
2. В колонке Базы данных SQL выберите **Настройка** > **Свойства**, а затем щелкните **Показать строки подключения к базам данных**. 
...
3. Откройте файл `createtable.php` в текстовом редакторе или интегрированной среде разработки и добавьте приведенный ниже код. Этот код будет использоваться для создания таблицы `registration_tbl` в базе данных `registration`.
    ...
    Обратите внимание, что потребуется обновить значения <code>$user</code> и <code>$pwd</code> с учетом имени пользователя и пароля локального сервера SQL Server.
4. В терминале в корневом каталоге типа приложения введите следующую команду:
   
        php -S localhost:8000
5. Откройте веб-браузер и перейдите по адресу **http://localhost:8000/createtable.php**. В результате в базе данных будет создана таблица `registration_tbl` .
6. Откройте файл **index.php** в текстовом редакторе или в интегрированной среде разработки и добавьте базовый HTML-код и CSS для страницы (PHP-код будет добавлен позднее).
...
7. Внутри тегов PHP добавьте PHP-код для подключения к базе данных.
...
8. После кода подключения к базе данных добавьте код для вставки регистрационных данных в базу данных.
...
9. Наконец, после вышеприведенного кода добавьте код для извлечения данных из базы данных.
...
Теперь можно перейти на узел **http://localhost:8000/index.php** для тестирования приложения.

## <a name="publish-your-application"></a>Публикация приложения
После локального тестирования приложения его можно опубликовать в веб-приложениях службы приложений с использованием Git. Тем не менее, необходимо сначала обновить в приложении сведения подключения к базе данных. С помощью полученной ранее информации о подключении к базе данных (ознакомьтесь с разделом **Получение информации о подключении к базе данных SQL**) обновите приведенные ниже данные в **обоих** файлах (`createdatabase.php` и `index.php`), используя соответствующие значения.
...
> [!NOTE]
> В <code>$host</code> значение Server должно быть должно быть указано с префиксом <code>tcp:</code>.
> 
> 
...
2. Перейдите по адресу **http://[имя_веб-приложения].azurewebsites.net/createtable.php**, чтобы создать базу данных SQL для приложения.
3. Перейдите по адресу **http://[имя_веб-приложения].azurewebsites.net/index.php**, чтобы приступить к работе с приложением.
...
2. Откройте GitBash (или терминал, если Git включен в переменную `PATH`), переключитесь на корневой каталог приложения и выполните следующие команды:
    ...
    Появится запрос на ввод ранее заданного пароля.
3. Перейдите по адресу **http://[имя_веб-приложения].azurewebsites.net/index.php**, чтобы просмотреть внесенные изменения.
