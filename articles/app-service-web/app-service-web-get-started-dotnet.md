...
В диалоговом окне **Создать службу приложений** выберите команду **Добавить новую учетную запись**, а затем выполните вход в подписку Azure. Если вы уже вошли в систему, проверьте, содержит ли ваша учетная запись подписку Azure. Можно добавить правильную учетную запись к текущей.
...
Как только вы войдете в систему, вы сможете создать все ресурсы, необходимые для веб-приложения Azure в этом диалоговом окне.
...
В поле **Имя веб-приложения** введите уникальное имя (допустимые символы: `a-z`, `0-9` и `-`). URL-адрес веб-приложения: `http://<app_name>.azurewebsites.net`, где `<app_name>` — имя веб-приложения. 

Можно принять имя, созданное автоматически, если оно является уникальным.
