<properties
	...
	description="Убедитесь, как просто запускать веб-приложения в службе приложений, развернув пример приложения с помощью всего нескольких действий. Начните разрабатывать настоящее приложение за пять минут — и сразу же получите результаты."
...
1. Перейдите в рабочий каталог (`CD`) и клонируйте пример приложения следующим образом:
...
4. Создайте ресурс приложения службы приложений Azure с уникальным именем приложения, используя приведенную ниже команду. При появлении запроса укажите номер нужного региона.
    ...
    >[AZURE.NOTE] Если вы еще не настроили учетные данные развертывания для своей подписки Azure, вам будет предложено создать их. Служба приложений использует именно эти учетные данные (а не данные учетной записи Azure) только для развертывания GIT и входа по протоколу FTP.
...
4. Разверните пример кода в новом приложении службы приложений так, как вы отправляете любой код с помощью GIT:
...
Теперь с помощью Git можно в любой момент передать на рабочий сайт изменения из корневого каталога проекта (репозитория). Это можно сделать так же, как при первом развертывании приложения в Azure. Например, каждый раз, когда вам нужно отправить новое изменение, протестированное локально, выполните следующие команды из корневого каталога проекта (репозитория):
...
1. Войдите на [портал Azure](https://portal.azure.com) с помощью учетной записи Майкрософт в рамках своей подписки Azure.

2. На панели слева щелкните **Службы приложений**.

3. Выберите только что созданное приложение, чтобы открыть соответствующую страницу ([колонку](../azure-portal-overview.md)) на портале. Для удобства по умолчанию также откроется колонка **Параметры**.

    ![Просмотр первого веб-приложения в Azure на портале](./media/app-service-web-get-started/portal-view.png)

Колонка портала для приложения службы приложений содержит широкий набор параметров и средств для настройки, мониторинга, защиты приложения и устранения неполадок в нем. Уделите минуту, чтобы ознакомиться с этим интерфейсом, выполнив несколько простых задач. (Номер задачи соответствует номеру на снимке экрана).
...
- Помимо GIT и интерфейса командной строки Azure, существуют другие способы развертывания веб-приложений в Azure. См. статью [Развертывание приложения в службе приложений Azure](../app-service-web/web-sites-deploy.md). Выберите платформу в начале статьи, чтобы открыть шаги по разработке и развертыванию, соответствующие вашей языковой платформе.
