...
Если появится сообщение об ошибке `Site <app_name> does not exist or has no hostnames`, повторите команду через несколько секунд. Для развертывания некоторых приложений, например приложений Java, требуется больше времени.
...
3. Выберите только что созданное приложение службы приложений, чтобы открыть соответствующую колонку на портале. Вы увидите, что колонка **Параметры** также открывается по умолчанию для вашего удобства.
...
- Щелкните ссылку **Группа ресурсов**, чтобы просмотреть все ресурсы, развернутые в группе ресурсов.
- Щелкните **Параметры** > **Свойства**, чтобы просмотреть дополнительные сведения о приложении.
- Щелкните **Инструменты**, чтобы получить доступ к полезным средствам для мониторинга и устранения неполадок.  
