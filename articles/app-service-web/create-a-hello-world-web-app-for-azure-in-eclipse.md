...
1. В контекстном меню выберите **Azure**, затем щелкните **Publish as Azure Web App...** (Опубликовать как веб-приложение Azure...).
   ...
   Кроме того, при выборе проекта веб-приложения в обозревателе проектов можно нажать кнопку с раскрывающимся списком **Publish** (Опубликовать) на панели инструментов и выбрать в списке **Publish as Azure Web App** (Опубликовать как веб-приложение Azure):
...
1. Если вы не создавали контейнер веб-приложения Azure или хотите опубликовать приложение в новый контейнер, выполните следующие действия. В противном случае выберите существующий контейнер веб-приложения и перейдите к шагу 7 ниже.
  ...
  1. Введите **DNS Label** (метку DNS) для вашего контейнера веб-приложения. Она образует метку листа DNS для URL-адреса узла веб-приложения в Azure. Примечание. Имя должно быть доступным и соответствовать требованиям к именованию веб-приложений Azure.
...
1. После этого вы должны увидеть представление **Azure Activity Log** (Журнал действий Azure), в котором будет отображаться состояние развертывания веб-приложения.

    ![][12]

    Процесс развертывания веб-приложения в Azure занимает всего несколько секунд. Когда процесс будет завершен, вы увидите ссылку **Published** (Опубликовано) в столбце **Status** (Состояние). Если щелкнуть по ней, откроется домашняя страница развернутого веб-приложения.
...
2. В контекстном меню выберите **Azure**, затем щелкните **Publish as Azure Web App...** (Опубликовать как веб-приложение Azure...).
...
Через несколько секунд в представлении **Azure Activity Log** (Журнал действий Azure) состояние обновленного развертывания изменится на **Published** (Опубликовано), и вы сможете проверить свое обновленное приложение в веб-браузере.
...
Если представление **Azure Explorer** еще не открыто, его можно открыть, щелкнув меню **Window** (Окно) в Eclipse и последовательно выбрав **Show View** (Показать представление), **Other...** (Другие...), **Azure**, **Azure Explorer**. Если ранее вы не вошли в систему, вам будет предложено это сделать.

Когда представление **Azure Explorer** откроется, выполните следующие действия для остановки веб-приложения.
...
5. В открывшемся контекстном меню выберите пункт **Stop** (Остановить). 
    ![][13]
