...
В этом пошаговом руководстве показано, как создать пакет SDK для Azure для приложения Java, которое создает веб-приложение в [службе приложений Azure][], а затем развернуть приложение в это веб-приложение. Руководство состоит из двух частей:
...
Если ваша подписка Azure не включает в себя Active Directory, войдите на [классический портал Azure][], используя свою учетную запись Майкрософт. Если у вас несколько подписок, щелкните **Подписки** и выберите каталог по умолчанию для подписки, которую необходимо использовать для этого проекта, а затем щелкните **Применить** для перехода в представление подписки.
...
Дополнительную информацию об Active Directory см. в разделе [Что такое каталог Azure AD?][].
...
Для прохождения аутентификации с помощью Azure код в этой процедуре использует самозаверяющий сертификат. Чтобы выполнить эту процедуру, необходимо создать сертификат и заранее загрузить его на [классический портал Azure][]. Для этого необходимо выполнить следующие шаги.
...
По завершении этой процедуры CER-сертификат будет находиться в подписке Azure, а JKS-сертификат — на локальном диске. Дополнительную информацию о сертификатах управления см. в разделе [Создание и передача сертификата управления для Azure][].
...
> **Примечание.** На компьютере, где вы будете выполнять эту команду, должен быть установлен пакет JDK. Кроме того, путь к keytool зависит от расположения, в котором установлен этот пакет. Дополнительную информацию см. в разделе онлайн-документации Java [Инструмент управления сертификатами и ключом (keytool)][].
...
Дополнительную информацию см. в разделе [Создание и передача сертификата управления для Azure][].
...
- для `webSpaceName` следует указать одно из значений, определенных в классе [WebSpaceNames][];
...
Чтобы получить информацию о подключении по FTP из колонки веб-приложения на [портале Azure][]\:
