...
# Создание веб-приложения в службе приложений Azure с использованием пакета SDK для Azure для Java

<!-- Azure Active Directory workflow is not yet available on the Azure Portal -->

## Обзор

В этом пошаговом руководстве показано, как создать пакет SDK для Azure для приложения Java, которое создает веб-приложение в [службе приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714), а затем развернуть приложение в это веб-приложение. Руководство состоит из двух частей:
...
Код приложения AzureWebDemo в этой статье написан с использованием пакета SDK 0.7.0 для Azure для Java, который можно установить с помощью [установщика веб-платформы (WebPI)](http://go.microsoft.com/fwlink/?LinkID=252838). Вам также следует убедиться, что вы используете последнюю версию [набора средств Azure для Eclipse](https://msdn.microsoft.com/library/azure/hh690946.aspx). После установки пакета SDK обновите зависимости в проекте Eclipse, запустив команду **Обновление индекса** в представлении **Репозитории Maven**, а затем снова добавьте последнюю версию каждого пакета в окне **Зависимости**. Вы можете проверить версию установленного программного обеспечения в Eclipse, щелкнув **Справка > Подробная информация об установке**. Вы увидите по крайней мере следующие версии:
...
### Создание и настройка облачных ресурсов в Azure
...
### Создание Active Directory в Azure

Если ваша подписка Azure не включает в себя Active Directory, войдите на [классический портал Azure](https://manage.windowsazure.com), используя свою учетную запись Майкрософт. Если у вас несколько подписок, щелкните **Подписки** и выберите каталог по умолчанию для подписки, которую необходимо использовать для этого проекта, а затем щелкните **Применить** для перехода в представление подписки.

1. В меню слева выберите **Active Directory**. Щелкните **Создать > Каталог > Настраиваемое создание**.

2. В разделе **Добавить каталог** выберите **Создать новый каталог**.
...
Дополнительную информацию об Active Directory см. в разделе [Что такое каталог Azure AD?](http://technet.microsoft.com/library/jj573650.aspx).


### Создание сертификата управления для Azure
...
Для прохождения аутентификации с помощью Azure код в этой процедуре использует самозаверяющий сертификат. Чтобы выполнить эту процедуру, необходимо создать сертификат и заранее загрузить его на [классический портал Azure](https://manage.windowsazure.com). Для этого необходимо выполнить следующие шаги.
...
По завершении этой процедуры CER-сертификат будет находиться в подписке Azure, а JKS-сертификат — на локальном диске. Дополнительную информацию о сертификатах управления см. в разделе [Создание и передача сертификата управления для Azure](http://msdn.microsoft.com/library/azure/gg551722.aspx).
...
Дополнительную информацию см. в разделе [Создание и передача сертификата управления для Azure](http://msdn.microsoft.com/library/azure/gg551722.aspx).


#### Загрузка сертификата

Чтобы загрузить самозаверяющий сертификат в Azure, перейдите на страницу **Параметры** на классическом портале, а затем щелкните вкладку **Сертификаты управления**. Щелкните **Загрузить** в нижней части страницы и перейдите к расположению CER-файла, который вы создали.


#### Преобразование PFX-файла в JKS-файл
...
## Сборка приложения для создания веб-приложения

### Создание рабочей области Eclipse и проекта Maven

В этом разделе вы создадите рабочую область и проект Maven для приложения, создающего веб-приложение AzureWebDemo.

1. Создайте проект Maven. Щелкните **Файл > Создать > Проект Maven**. На странице **Новый проект Maven** выберите **Создание простого проекта** и **Использовать расположение рабочей области по умолчанию**.
...
4. Откройте представление «Репозитории Maven». Щелкните **Окно > Показать представление > Другие > Maven > Репозитории Maven** и нажмите кнопку **ОК**. Представление **Репозитории Maven** будет отображаться в нижней части интегрированной среды разработки.

5. Откройте **Глобальные репозитории**, щелкните правой кнопкой мыши **центральный** репозиторий, а затем выберите **Перестройка индекса**.
...
6. На вкладке **Зависимости** щелкните **Добавить**. В поле **Введите идентификатор группы...** введите `azure-management`. Выберите пакеты для базового управления и управления веб-приложениями службы приложений:

        com.microsoft.azure  azure-management
        com.microsoft.azure  azure-management-websites

    > **Примечание.** При обновлении зависимостей после выпуска новой версии в этот список необходимо повторно добавить все зависимости. После того, как вы щелкните **Добавить** и выберите каждую зависимость, они будут отображаться с номером новой версии в списке **Зависимости**.

Нажмите кнопку **ОК**. После этого в списке **Зависимости** будут отображаться пакеты Azure.


### Написание кода Java для создания веб-приложения путем вызова пакета SDK для Azure
...
2. В окне **Новый класс Java** укажите имя класса `WebCreator` и установите флажок **public static void main**. Значения выбранных параметров должны выглядеть так:
...
### Вызов API Azure для создания веб-приложения службы приложений
...
#### Определение класса главной точки входа
...
- для `webSpaceName` следует указать одно из значений, определенных в классе [WebSpaceNames](http://dl.windowsazure.com/javadoc/com/microsoft/windowsazure/management/websites/models/WebSpaceNames.html);
- `appServicePlanName` следует указать, как показано выше.

> **Примечание.** При каждом запуске этого приложения необходимо менять значения `webAppName` и `appServicePlanName` (или удалять веб-приложение на портале Azure) перед повторным запуском приложения. В противном случае произойдет сбой выполнения, так как в Azure уже есть такой же ресурс.


#### Определение метода создания веб-приложения

Далее следует определить метод создания веб-приложения. Метод `createWebApp` указывает параметры веб-приложения и веб-пространства. Он также создает и настраивает клиент управления веб-приложениями службы приложений, определенный объектом [WebSiteManagementClient](http://dl.windowsazure.com/javadoc/com/microsoft/windowsazure/management/websites/WebSiteManagementClient.html). Клиент управления — это ключ к созданию веб-приложений. Он позволяет использовать веб-службы RESTful, которые, в свою очередь, позволяют приложениям управлять веб-приложениями (операции создания, обновления и удаления), вызывая API управления службами.
...
#### Запуск приложения и проверка созданного веб-приложения
...
## Развертывание приложения в веб-приложение

После того, как вы запустили AzureWebDemo и создали веб-приложение, войдите на классический портал, щелкните **Веб-приложения** и выберите **WebDemoWebApp** в списке **Веб-приложения**. На странице панели мониторинга веб-приложения щелкните **Обзор** (или URL-адрес `webdemowebapp.azurewebsites.net`), чтобы перейти к нему. Вы увидите пустую страницу заполнителя, так как в веб-приложении еще не опубликовали никакого содержимого.

Далее мы создадим приложение Hello World и развернем его в веб-приложение.


### Создание приложения JSP Hello World
...
1. Щелкните **Файл > Создать > Динамический веб-проект**. Назовите его `JSPHello`. В этом диалоговом окне не нужно менять никакие другие настройки. Нажмите кнопку **Готово**

    ![][3]

2. В обозревателе проектов разверните проект **JSPHello**, щелкните правой кнопкой мыши **WebContent**, а затем щелкните **Создать > JSP-файл**. В диалоговом окне «Создание JSP-файла» укажите имя для нового файла `index.jsp`. Нажмите кнопку **Далее**.

3. В диалоговом окне **Выберите шаблон JSP** выберите **Новый JSP-файл (html)** и нажмите кнопку **Готово**.
...
#### Запуск приложения Hello World на localhost

Перед запуском этого приложения необходимо настроить некоторые свойства.

1. Щелкните правой кнопкой мыши проект **JSPHello** и выберите **Свойства**.

2. В диалогом окне **Свойства** выберите **Путь построения Java**, щелкните вкладку **Порядок и экспорт** и установите флажок **Системная библиотека JRE**, а затем щелкните **Вверх** для перемещения вверх по списку.

    ![][4]

3. В диалогом окне **Свойства** выберите **Целевые среды выполнения** и щелкните **Создать**.

4. В диалогом окне **Создание среды выполнения сервера** выберите сервер, например **Apache Tomcat v7.0**, и щелкните **Далее**. В диалогом окне **Сервер Tomcat** в поле **Имя** задайте `Apache Tomcat v7.0`, а в поле **Каталог установки Tomcat** — каталог с установленной версией сервера Tomcat, которую необходимо использовать.

    ![][5]

    Нажмите кнопку **Готово**

5. Затем вернитесь на страницу **Целевые среды выполнения** диалогового окна **Свойства**. Выберите **Apache Tomcat v7.0** и нажмите кнопку **ОК**.

    ![][6]

6. В Eclipse в меню **Запуск** щелкните **Запустить**. В диалогом окне **Запустить как** выберите **Запустить на сервере**. В диалогом окне **Запустить на сервере** выберите **Сервер Tomcat v7.0**:

    ![][7]

    Нажмите кнопку **Готово**

7. Когда приложение запустится, вы увидите страницу **JSPHello** в окне localhost в Eclipse (`http://localhost:8080/JSPHello/`), где будет отображаться следующее сообщение:

    `Hello World, the time is Tue Mar 24 23:21:10 GMT 2015`


#### Экспорт приложения в виде WAR-файла
...
2. В диалогом окне **Экспорт выбранного содержимого** щелкните **Веб > WAR-файл**, а затем щелкните **Далее**.
...
Дополнительную информацию о развертывании WAR-файлов см. в разделе [Добавление приложения Java в веб-приложения службы приложений Azure](web-sites-java-add-app.md).


### Развертывание приложения Hello World с использованием FTP
...
#### "Настройка учетных данных для развертывания"
...
1. Войдите на классический портал Azure и щелкните **Веб-приложения**. Убедитесь, что **WebDemoWebApp** отображается в списке веб-приложений и что приложение работает. Щелкните **WebDemoWebApp**, чтобы открыть его страницу **Панель мониторинга**.

2. На странице **Панель мониторинга** в разделе **Сводка** щелкните **Задайте учетные данные развертывания** (если у вас уже есть учетные данные развертывания, этот пункт будет называться **Сброс учетных данных развертывания**).
...
#### Получение информации о подключении по FTP
...
Чтобы получить информацию о подключении по FTP из колонки веб-приложения на [портале Azure](https://portal.azure.com):
...
- `publishMethod="FTP"` означает, что для параметра **Протокол** вы задали значение **FTP — протокол передачи файлов**, а для **Шифрование** — **Использовать простой FTP**.
- `userName` и `userPWD` — это ключи для фактических значений имени пользователя и пароля, которые вы указали при сбросе учетных данных развертывания. Значение `userName` аналогично значению **Пользователь FTP или развертывания**. Они соответствуют параметрам **Пользователь** и **Пароль** в FileZilla.
- `ftpPassiveMode="True"` означает, что FTP-сайт использует пассивный режим передачи FTP. Выберите **Пассивный** на вкладке **Параметры передачи**.


#### Настройка размещения приложения Java в веб-приложении
...
2. Значение по умолчанию для параметра **Версия Java** — **Отключено**. Выберите целевую версию Java для своего приложения, например 1.7.0\_51. После этого убедитесь, что для **Веб-контейнер** установлена версия «Сервер Tomcat».

3. В **Документы по умолчанию** добавьте index.jsp и переместите этот документ в верхнюю часть списка (файл по умолчанию для веб-приложений— hostingstart.html).

4. Щелкните **Сохранить**.


#### Публикация приложения с помощью Kudu
...
2. В верхнем меню выберите **Консоль отладки > Командная строка**.
...
#### Публикация приложения с помощью FileZilla (необязательно)

Для публикации своего приложения вы также можете использовать другой инструмент — FileZilla, популярный сторонний FTP-клиент с удобным графическим пользовательским интерфейсом. Если вы еще не установили FileZilla, вы можете сделать это, скачав его с веб-сайта [http://filezilla-project.org/](http://filezilla-project.org/). Дополнительную информацию об использовании этого клиента см. в разделе [Документация по FileZilla](https://wiki.filezilla-project.org/Documentation), а также в записи блога [FTP-клиенты. Часть 4. FileZilla](http://blogs.msdn.com/b/robert_mcmurray/archive/2008/12/17/ftp-clients-part-4-filezilla.aspx).
...
4. На панели сайта **Локальные** выберите исходный каталог, в котором находится файл JSPHello.war. Путь будет выглядеть аналогично следующему:

    `<project-path>/JSPHello/src/`

5. На панели сайта **Удаленные** выберите папку назначения. Разверните WAR-файл в каталоге `webapps` в корне веб-приложения. Перейдите к `/site/wwwroot`, щелкните правой кнопкой мыши `wwwroot` и выберите **Создать каталог**. Укажите для каталога имя `webapps` и войдите в этот каталог.

6. Передайте JSPHello.war в `/site/wwwroot/webapps`. Выберите JSPHello.war в списке файлов **Локальные**, щелкните его правой кнопкой мыши и выберите **Загрузить**. Вы должны увидеть его в `/site/wwwroot/webapps`.
...
#### Запуск приложения Hello World в веб-приложении
...
1. Загрузив WAR-файл и убедившись, что сервер Tomcat создал распакованный каталог `JSPHello`, перейдите к `http://webdemowebapp.azurewebsites.net/JSPHello`, чтобы запустить приложение.
    ...
    > **Примечание.** Если вы щелкните **Обзор** на классическом портале, может отобразиться веб-страница по умолчанию с сообщением «Это веб-приложение на основе Java успешно создано». Возможно, понадобится обновить веб-страницу, чтобы просмотреть выходные данные приложения вместо веб-страницы по умолчанию.
