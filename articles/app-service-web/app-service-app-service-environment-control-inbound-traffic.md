...
Обратите внимание, что для правильной работы среды службы приложений также требуется исходящий доступ к службе хранилища Azure по всему миру и к базе данных SQL в том же регионе Azure. Если исходящий доступ к Интернету заблокирован в виртуальной сети, среды службы приложений не смогут получить доступ к этим конечным точкам Azure.
...
**Примечание.** Группы безопасности сети можно настроить только с помощью командлетов Powershell, описанных ниже. Группы безопасности сети нельзя настроить с помощью графического интерфейса пользователя на новом портале (portal.azure.com), потому что новый портал позволяет так настроить только группы безопасности сети, связанные с виртуальными сетями версии 2. Тем не менее, сейчас среды службы приложений работают только с классическими виртуальными сетями версии 1. Поэтому для настройки групп безопасности сети, связанных с виртуальными сетями версии 1, можно использовать только командлеты Powershell.
