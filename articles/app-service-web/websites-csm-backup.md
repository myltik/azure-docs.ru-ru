<properties
	pageTitle="Использование REST для резервного копирования и восстановления веб-приложений Azure"
	description="Узнайте, как использовать вызовы REST API для запуска резервного копирования и восстановления веб-приложения в службе приложений Azure."
...
# Использование REST для резервного копирования и восстановления веб-приложений Azure
Для [веб-приложений Azure](https://azure.microsoft.com/services/app-service/web/) можно создавать резервную копию в виде больших двоичных объектов в хранилище Azure. Резервная копия может включать также базы данных приложения. Если приложение будет случайно удалено или потребуется восстановить его предыдущую версию, можно восстановить любую из резервных копий. Резервное копирование может выполняться по запросу в любое время или по расписанию с нужными интервалами.

В этой статье объясняется, как выполнять резервное копирование и восстановление веб-приложений Azure с помощью запросов REST API. Если вы хотите создавать резервные копии приложения и управлять ими через графический интерфейс на портале Azure, ознакомьтесь со статьей [Резервное копирование веб-приложений в службе приложений Azure](web-sites-backup.md).

<a name="gettingstarted"></a>
## Приступая к работе
Для отправки запросов REST необходимо знать **имя** веб-приложения, его **группу ресурсов** и **идентификатор подписки**. Эти сведения можно найти, щелкнув веб-приложение в колонке **веб-приложений** на [портале предварительной версии Azure](https://portal.azure.com). В примерах в этой статье мы будем настраивать веб-сайт `backuprestoreapiexamples.azurewebsites.net`. Это приложение хранится в группе ресурсов с именем Default-Web-WestUS и выполняется в подписке с идентификатором 00001111-2222-3333-4444-555566667777.

![Пример информации о веб-сайте][SampleWebsiteInformation]

<a name="backup-restore-rest-api"></a>
## REST API архивации и восстановления
Теперь мы в нескольких примерах рассмотрим, как использовать REST API для резервного копирования и восстановления веб-приложения Azure. Каждый пример содержит URL-адрес и текст HTTP-запроса. URL-адреса в примерах будут содержать заполнители, заключенные в фигурные скобки, например {subscriptionId}. Вам нужно заменить их соответствующими сведениями о веб-приложении. Для справки приводится описание всех заполнителей, которые встречаются в URL-адресах наших примеров:

* subscriptionId — идентификатор подписки Azure, которая содержит веб-приложение;
* resourceGroupName — имя группы ресурсов, которая содержит веб-приложение;
* sitemane — имя веб-приложения Azure;
* backupId — идентификатор резервной копии веб-приложения.
...
## Резервное копирование веб-приложения по требованию
Чтобы немедленно создать резервную копию веб-приложения, отправьте запрос **POST** на адрес `https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/sites/{sitename}/backup/`.
...
Резервное копирование веб-приложения можно запускать не только по требованию, но и автоматически по расписанию.
...
Чтобы получить актуальные настройки резервного копирования веб-приложения, отправьте запрос **POST** на URL-адрес ` https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/sites/{name}/config/backup/list`.
...
В зависимости от размера веб-приложения резервное копирование может занять некоторое время. Более того, процесс копирования может завершиться ошибкой, прекратиться по истечении времени ожидания или оказаться частично выполненным. Чтобы увидеть состояние всех резервных копий веб-приложения, отправьте запрос **GET** на URL-адрес `https://management.azure.com/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/sites/{name}/backups`.
...
### Восстановление в виде нового веб-приложения
Иногда при восстановлении резервной копии может потребоваться создать новое веб-приложение, а не перезаписывать уже существующее. Чтобы сделать это, измените URL-адрес запроса так, чтобы он указывал на новое веб-приложение, а также установите в объекте JSON для свойства **overwrite** значение **false**.

<a name="delete-app-backup"></a>
## Удаление резервной копии веб-приложения
