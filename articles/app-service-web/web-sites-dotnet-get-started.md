...
В этом учебнике показано, как создать веб-приложение ASP.NET и развернуть его [в веб-приложениях службы приложений](app-service-web-overview.md) с помощью Visual Studio 2015 или Visual Studio 2013. Этот учебник разработан для читателей, не имеющих опыта использования Azure или ASP.NET. По завершении изучения этого учебника вы получите простое веб-приложение, работающее в облаке.
...
* Как использовать [портал предварительной версии Azure](/overview/preview-portal/) для мониторинга веб-приложения и управления им.

##<a name="video"></a>Регистрация в Microsoft Azure

Для работы с этим учебником требуется учетная запись Azure.

* Вы можете [создать учетную запись Azure бесплатно](/pricing/free-trial/?WT.mc_id=A261C142F). В этом случае вы получите кредиты, которые можно использовать для ознакомления с платными службами Azure. Когда кредиты закончатся, вы сможете сохранить учетную запись и пользоваться бесплатными службами Azure, например веб-приложениями службы приложений.
...
На следующей схеме показаны действия по созданию и развертыванию веб-приложения.
...
4.  Для мониторинга доступности, производительности и использования веб-приложения используется служба [Application Insights](app-insights-overview.md). Если вы не хотите ее использовать, снимите флажок **Добавить Application Insights в проект**.
...
5. В диалоговом окне **Новый проект ASP.NET** выберите шаблон **MVC**.
	...
	[MVC](http://www.asp.net/mvc) — это платформы ASP.NET для разработки веб-приложений.
...
5. Введите *MyExampleGroup* или другое имя группы ресурсов на ваш выбор.
...
5. Оставьте поле базы данных без изменений.
	...
	В этом учебнике база данных не используется. В разделе [Дальнейшие действия](#next-steps) приводится ссылка на руководство, в котором показано, как использовать базу данных.
...
11. На вкладке **Предварительный просмотр** нажмите кнопку **Опубликовать**.
	...
	Если вы хотите узнать, какие файлы будут скопированы в Azure, сначала нажмите кнопку **Начало предварительного просмотра**, а потом — **Опубликовать**.
...
Этот раздел руководства является необязательным. В нем вы измените заголовок **первого уровня** на домашней странице, запустите проект локально на компьютере разработчика, чтобы проверить изменения, а затем развернете их в Azure.
...
1. Нажмите клавиши CTRL+F5, чтобы увидеть обновленный заголовок, запустив веб-приложение на локальном компьютере.
	...
	URL-адрес `localhost` указывает, что веб-приложение выполняется на локальном компьютере. По умолчанию он работает в IIS Express — упрощенной версии IIS, используемой на этапе разработки веб-приложений.
...
1. В **Обозревателе решений** щелкните правой кнопкой проект и выберите пункт **Опубликовать**.
	...
	Откроется вкладка **Предварительный просмотр** мастера **Публикация веб-сайта**. Если бы вам требовалось изменить какие-либо параметры публикации, вы бы выбрали другую вкладку, но нам нужно просто выполнить повторное развертывание с теми же параметрами.
...
[Портал управления Azure](/services/management-portal/) — это веб-интерфейс, который позволяет осуществлять мониторинг таких служб Azure, как только что созданное веб-приложение, и управлять ими. В этом разделе мы познакомимся с тем, что можно сделать в портале.

1. В браузере перейдите по адресу [https://portal.azure.com]() и войдите со своими учетными данными Azure.

2. Щелкните **Просмотреть все > Веб-приложения**, а затем выберите имя вашего веб-приложения.
...
* Как избежать затрат времени на ожидание пробуждения после времени простоя
	...
	По умолчанию в случае простоя в течение определенного периода времени веб-приложения будут выгружены. Первый отправленный после этого запрос будет ожидать, пока веб-приложение загрузится снова. Чтобы избежать времени ожидания, можно включить функцию AlwaysOn. Дополнительные сведения см. в разделе о параметрах конфигурации в статье [Настройка веб-приложений](web-sites-configure.md).
