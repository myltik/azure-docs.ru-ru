<properties
	pageTitle="Развертывание приложения ASP.NET в службе приложений Azure с помощью Visual Studio | Microsoft Azure"
...
В этом руководстве по началу работы продемонстрировано развертывание простого веб-проекта, в котором не используется база данных и не выполняется аутентификация или авторизация. Ссылки на другие разделы, посвященные развертыванию, см. в статье [Развертывание приложения в службе приложений Azure](web-sites-deploy.md).
...
* В этом руководстве предполагается, что у вас есть опыт работы с ASP.NET MVC и Visual Studio. Общие сведения об этих компонентах см. в разделе [Getting Started with ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started) (Приступая к работе с Azure и ASP.NET MVC 5).
...
5. В диалоговом окне **Новый проект ASP.NET** выберите шаблон **MVC**, а затем щелкните **Изменить способ проверки подлинности**.
...
5. В диалоговом окне **Новый проект ASP.NET** в разделе **Microsoft Azure** установите флажок **Разместить в облаке**, а в раскрывающемся списке ниже выберите пункт **Служба приложений**.
...
## Настройка ресурсов Azure для нового веб-приложения

Теперь в Visual Studio можно указать ресурсы Azure, которые требуется создать.

5. В диалоговом окне **Создать службу приложений** щелкните **Добавить учетную запись** и войдите в Azure с помощью идентификатора и пароля, которые используются для управления подпиской Azure.
...
3. В поле **Имя веб-приложения** введите имя, которое является уникальным в домене *azurewebsites.net*. Вы можете назвать приложение MyExample и добавить уникальный номер справа (например, MyExample810) или использовать уникальное имя, которое создается автоматически.
	...
	Для веб-приложения Azure можно также использовать личный домен. Дополнительные сведения см. в статье [Настройка личного доменного имени для службы приложений Azure](web-sites-custom-domain-name.md).

6. Нажмите кнопку **Создать** рядом с полем **Группа ресурсов** и введите имя MyExample (или любое другое имя).
...
## Создание проекта и веб-приложения в Visual Studio
...
8. На вкладке **Подключение** **мастера веб-публикации** нажмите кнопку **Далее**.
	...
	После этого отобразится вкладка **Параметры**. Здесь можно изменить конфигурацию сборки, чтобы развернуть отладочную сборку для [удаленной отладки](../app-service-web/web-sites-dotnet-troubleshoot-visual-studio.md#remotedebug). На этой вкладке также представлено несколько [параметров публикации файлов](https://msdn.microsoft.com/library/dd465337.aspx#Anchor_2).
...
*	Дополнительные примеры создания веб-приложения ASP.NET в службе приложений см. в статьях [Create and deploy an ASP.NET web app in Azure App Service](https://github.com/Microsoft/HealthClinic.biz/wiki/Create-and-deploy-an-ASP.NET-web-app-in-Azure-App-Service) (Создание и развертывание веб-приложения ASP.NET в службе приложений Azure) и [Create and deploy a mobile app in Azure App Service](https://github.com/Microsoft/HealthClinic.biz/wiki/Create-and-deploy-a-mobile-app-in-Azure-App-Service) (Создание и развертывание мобильного приложения в службе приложений Azure), описывающих [демонстрационный проект](https://blogs.msdn.microsoft.com/visualstudio/2015/12/08/connectdemos-2015-healthclinic-biz/) [HealthClinic.biz](https://github.com/Microsoft/HealthClinic.biz) 2015 Connect. Дополнительные инструкции по быстрому началу работы с помощью средств разработчика Azure из демонстрационного проекта HealthClinic.biz см. на [этой странице](https://github.com/Microsoft/HealthClinic.biz/wiki/Azure-Developer-Tools-Quickstarts).
