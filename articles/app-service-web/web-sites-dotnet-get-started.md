...
В этом учебнике описано, как развернуть веб-приложение ASP.NET [в веб-приложении службы приложений Azure](app-service-web-overview.md) с помощью Visual Studio 2015 или Visual Studio 2013. Этот учебник создан для разработчиков ASP.NET без опыта работы с Azure. По завершении изучения этого учебника вы получите простое веб-приложение, работающее в облаке.
...
* Как настроить Visual Studio для создания веб-приложения службы приложений в процессе создания веб-проекта ASP.NET MVC 5.
* Как развернуть веб-проект для веб-приложения службы приложений с помощью Visual Studio.
* Как использовать **обозреватель сервера** Visual Studio для открытия удаленных файлов и запуска сеанса удаленной отладки. 
* Как использовать [портал Azure](/overview/preview-portal/) для отслеживания веб-приложения и управления им.

> [AZURE.NOTE]Этот учебник описывает использование ASP.NET со службой приложений Azure. В нем не объясняется, как разработать веб-приложение ASP.NET. Вводные сведения об ASP.NET MVC 5 см. в статье [Приступая к работе с ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started) на сайте [ASP.NET](http://asp.net/). Ссылки на другие статьи с подробной информацией об использовании службы приложений Azure см. в разделе [Дальнейшие действия](#next-steps).
> 
> Помогите нам оптимизировать содержание этого учебника. Если вы хотите, чтобы кроме вводных инструкций здесь описывались и другие темы, оставьте отзыв в [комментарии Disqus](#comments) в конце учебника.
...
В этом видео Скотт Хансельман (Scott Hanselman) демонстрирует, как легко зарегистрироваться для получения бесплатной пробной версии Microsoft Azure. (Длительность: 1:58)
...
3. Убедитесь, что в качестве целевой платформы выбрана версия **.NET Framework 4.5.2**.

4.  Для мониторинга доступности, производительности и использования веб-приложения можно использовать службу [Azure Application Insights](app-insights-overview.md). Если вы не хотите ее использовать, снимите флажок **Добавить Application Insights в проект**.
...
6. В диалоговом окне **Изменить аутентификацию** щелкните **Без аутентификации** и нажмите кнопку **ОК**.
	...
	В примере приложения, которое вы создаете, не будет функции входа пользователей в систему. В разделе [Дальнейшие действия](#next-steps) приводятся ссылки на учебник, в котором описывается реализация функций проверки подлинности и авторизации.

5. В диалоговом окне **Создание проекта ASP.NET** оставьте параметры в разделе **Microsoft Azure** без изменений и нажмите кнопку **ОК**.
...
3. В диалоговом окне **Настройка параметров веб-приложения Microsoft Azure** укажите **имя веб-приложения** (оно должно быть уникальным в домене *azurewebsites.net*). Вы можете назвать приложение MyExample и добавить уникальный номер справа (например, MyExample810) или использовать уникальное имя, которое создается автоматически.
...
4. В раскрывающемся списке **План службы приложений** выберите пункт **Создать новый план службы приложений**.
	...
	В разделе [Дальнейшие действия](#next-steps) содержатся ссылки на ресурсы, в которых описано использование планов службы приложений.
...
5. В раскрывающемся списке **Регион** выберите ближайшее к вам расположение.
	...
	Этот параметр указывает, в каком центре обработки данных Azure будет выполняться ваше веб-приложение. При работе с учебником вы можете выбрать любой регион, и это не будет оказывать существенного влияния. Тем не менее для рабочего веб-приложения необходимо, чтобы веб-сервер располагался максимально близко к браузерам, которые осуществляют доступ к вашему сайту. Это позволит минимизировать [задержки](http://www.bing.com/search?q=web%20latency%20introduction&qs=n&form=QBRE&pq=web%20latency%20introduction&sc=1-24&sp=-1&sk=&cvid=eefff99dfc864d25a75a83740f1e0090).

5. Оставьте поле базы данных без изменений.

	В этом учебнике база данных не используется. В разделе [Дальнейшие действия](#next-steps) содержатся ссылки на учебник, в котором описано использование базы данных.
...
10. На вкладке **Параметры** нажмите кнопку **Далее**.
	...
	Вы можете выполнить удаленную отладку, развернув отладочную сборку с помощью раскрывающегося списка **Конфигурация**. В разделе [Дальнейшие действия](#next-steps) содержатся ссылки на учебник, в котором описан удаленный запуск Visual Studio в режиме отладки.
...
11. На вкладке **Предварительный просмотр** нажмите кнопку **Опубликовать**.
	...
	Если вы хотите увидеть, какие файлы будут скопированы в Azure, сначала щелкните **Начать предварительный просмотр**, а потом — **Опубликовать**.

	![](./media/web-sites-dotnet-get-started/GS13previewoutput.png)

	Как только вы нажмете кнопку **Опубликовать**, Visual Studio начнет копирование файлов на сервер Azure.

	В окнах **Вывод** и **Действия службы приложений Azure** отображаются сведения о действиях, выполняемых при развертывании, а также отчет об успешном завершении развертывания.
...
13. Не закрывайте это окно браузера. Оно вам понадобиться, когда вы дойдете до следующего раздела.

**Совет.** Вы можете выполнять быстрое развертывание, включив панель инструментов **Веб-публикация одним щелчком**. Щелкните **Вид > Панели инструментов**, а затем выберите элемент **Веб-публикация одним щелчком**. С помощью панели инструментов можно выбрать профиль, нажать кнопку публикации или открыть **мастер веб-публикации**.

![Панель инструментов веб-публикации одним щелчком](./media/web-sites-dotnet-get-started/weboneclickpublish.png)

## Открытие удаленных файлов в обозревателе сервера

Во время тестирования и отладки веб-приложения можно вносить быстрые временные изменения на удаленном сайте, открывая и изменяя файлы в **обозревателе сервера**.

1.  В **обозревателе сервера** щелкните **Azure > Служба приложений > MyExampleGroup** и разверните узел для веб-приложения.

2. Разверните элементы **Файлы > Представления > Главная**, а затем дважды щелкните файл *Index.cshtml*.

	![](./media/web-sites-dotnet-get-started/indexfileinse.png)

3. Измените `<h1>ASP.NET</h1>` на `<h1>Azure App Service</h1>`.

4. Сохраните файл.

5. Обновите окно браузера с сайтом, запущенным в Azure.

	![](./media/web-sites-dotnet-get-started/afterindexedit.png)

Это изменение внесено в развернутый сайт, но в локальном проекте его нет. Если вы повторно развернете проект, сайт вернется к состоянию до внесения этого изменения.

Эта функция полезна для [временного отключения в файле Web.config конфигурации customErrors, что позволяет получить подробное сообщение об ошибке](web-sites-dotnet-troubleshoot-visual-studio.md).

В **обозревателе сервера** щелкните правой кнопкой мыши узел веб-приложения, чтобы получить доступ к настройкам веб-приложения в окне Visual Studio, запустить удаленный сеанс отладки или просмотреть журналы приложений в режиме реального времени, по мере того как приложение записывает их.

![](./media/web-sites-dotnet-get-started/sewebappmenu.png)

Дополнительную информацию см. в разделе [Устранение неполадок веб-приложений Azure в Visual Studio](web-sites-dotnet-troubleshoot-visual-studio.md).
...
2. Щелкните **Веб-приложения** и выберите имя своего веб-приложения.
...
3. Щелкните **Параметры**, чтобы отобразить дополнительные параметры конфигурации веб-приложения.
...
4. Щелкните **Параметры приложения**, чтобы отобразить примеры разных параметров, которые можно настроить на портале.
...
* Как добавить функции доступа к базе данных и авторизации
	...
	Учебник по доступу к базе данных и ограничению некоторых функций приложения для использования авторизованными пользователями см. в разделе [Развертывание безопасного приложения ASP.NET MVC с членством, OAuth и базой данных SQL в веб-приложении Azure](/develop/net/tutorials/web-site-with-sql-database/). Для прохождения этого курса вы должны иметь некоторое представление о MVC 5. Если у вас нет опыта работы с MVC 5, см. раздел [Приступая к работе с ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started).
...
* Руководство по переходу от портала Azure к порталу предварительной версии см. в разделе [Справочник по навигации на портале Azure](http://go.microsoft.com/fwlink/?LinkId=529715).
