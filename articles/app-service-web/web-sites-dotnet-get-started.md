...
Это краткий и простой учебник, в котором не описывается настройка создаваемого веб-проекта. Вводные сведения о разработке веб-приложений ASP.NET MVC 5 см. в разделе [Приступая к работе с ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started) на сайте [ASP.NET](http://asp.net/). Ссылки на другие статьи с более глубоким описанием веб-приложений в службе приложений Azure см. в разделе [Дальнейшие действия](#next-steps).
...
3. Убедитесь, что в качестве целевой выбрана платформа **.NET Framework 4.5.2**.

4.  Для мониторинга доступности, производительности и интенсивности использования веб-приложения можно применять службу [Azure Application Insights](app-insights-overview.md). Если вы не хотите ее использовать, снимите флажок **Добавить Application Insights в проект**.
...
4. В раскрывающемся списке **План службы приложений** выберите пункт **Создать новый план службы приложений**.
	...
	В разделе [Дальнейшие действия](#next-steps) содержатся ссылки на ресурсы, посвященные планам службы приложений.
...
1. В **Обозревателе решений** щелкните правой кнопкой проект и выберите пункт **Опубликовать**.
	...
	Через несколько секунд появится **мастер веб-публикации**. В мастере откроется *профиль публикации*, содержащий параметры для развертывания веб-проекта в новом веб-приложении. Если вы хотите выполнить развертывание в другое веб-приложение, откройте вкладку **Профиль** и создайте другой профиль. В этом руководстве вам нужно будет принять параметры, разворачивающие веб-приложение, которое было создано ранее.
...
10. На вкладке **Параметры** нажмите кнопку **Далее**.
	...
	В раскрывающемся списке **Конфигурация** можно развернуть отладочную сборку для удаленной отладки. В разделе [Дальнейшие действия](#next-steps) содержатся ссылки на учебник, в котором описан удаленный запуск Visual Studio в режиме отладки.
...
11. На вкладке **Предварительный просмотр** нажмите кнопку **Опубликовать**.
	...
	Если вы хотите увидеть, какие файлы будут скопированы в Azure, сначала выберите элемент **Начать предварительный просмотр**, а потом — **Опубликовать**.
...
**Совет:** для быстрого развертывания можно включить панель инструментов **Веб-публикация одним щелчком**. Щелкните **Вид > Панели инструментов**, а затем выберите элемент **Веб-публикация одним щелчком**. Используя панель инструментов, можно выбрать профиль, нажать кнопку публикации или открыть **мастер веб-публикации**.
...
[Портал Azure](/services/management-portal/) — это веб-интерфейс, позволяющий отслеживать работу служб Azure, например только что созданное веб-приложение, а также управлять ими. В этом разделе вы узнаете, что можно делать на портале.
...
* Как добавить функции доступа к базе данных и авторизации
	...
	Учебник по доступу к базе данных и ограничению некоторых функций приложения для использования авторизованными пользователями см. в разделе [Развертывание безопасного приложения ASP.NET MVC с членством, OAuth и базой данных SQL в веб-приложении Azure](/develop/net/tutorials/web-site-with-sql-database/). Этот учебник предполагает некоторый опыт работы с MVC 5. Если вы не знакомы с MVC 5, см. раздел [Приступая к работе с ASP.NET MVC 5](http://www.asp.net/mvc/overview/getting-started/introduction/getting-started).
...
* Как выбрать между службой приложений, облачными службами Azure и виртуальными машинами Azure для веб-приложений
	...
	Веб-приложения в Azure можно выполнять в веб-приложениях службы приложений Azure, как показано в этом учебнике, а также в облачных службах и на виртуальных машинах. Дополнительные сведения см. в статье [Сравнение веб-приложений, облачных служб и виртуальных машин Azure](/manage/services/web-sites/choose-web-app-service/).
