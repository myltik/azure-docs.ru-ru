...
4.	Щелкните нужное приложение и нажмите кнопку **Личные домены**.

6.  Запишите IP-адрес, указанный над разделом имен узлов.
...
Для дополнительной записи типа TXT применяется соглашение, в соответствии с которым &lt;*поддомен*>.&lt;*корневой\_домен*> сопоставляется с адресом &lt;*имя\_приложения*>.azurewebsites.net. Настройте запись TXT следующим образом.
...
3.	Щелкните нужное приложение и затем нажмите **Личные домены** > **Добавить имя узла**.
...
> [AZURE.NOTE] Если запись DNS уже используется (в случаях, когда активный домен передает трафик), а вам необходимо предварительно привязать к ней веб-приложение для проверки домена, просто создайте записи типа TXT, как показано в примерах из следующей таблицы. Для дополнительной записи типа TXT применяется соглашение, в соответствии с которым &lt;*поддомен*>.&lt;*корневой\_домен*> сопоставляется с адресом &lt;*имя\_приложения*>.azurewebsites.net.
> <table cellspacing="0" border="1">
>   <tr>
>     <th>Пример полного доменного имени</th>
>     <th>Узел, для которого задается TXT</th>
...
>   </tr>
>     <tr>
>     <td>*.contoso.com (поддомен)</td>
>     <td>awverify.*.contoso.com</td>
>     <td>&lt;<i>имя_приложения</i>>.azurewebsites.net</td>
>   </tr>
> </table>
После создания записи DNS вернитесь на портал Azure и добавьте пользовательское имя домена в свое веб-приложение.
