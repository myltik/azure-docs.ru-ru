...
[!INCLUDE [нижний колонтитул](../../includes/custom-dns-web-site-intro-notes.md)]
...
> [!NOTE]
> Если запись DNS уже используется (в случаях, когда активный домен передает трафик), а вам необходимо предварительно привязать к ней веб-приложение для проверки домена, просто создайте записи типа TXT, как показано в примерах из следующей таблицы. Для дополнительной записи типа TXT применяется соглашение, в соответствии с которым &lt;*поддомен*>.&lt;*корневой\_домен*> сопоставляется с адресом &lt;*имя\_приложения*>.azurewebsites.net.
> 
> <table cellspacing="0" border="1">
> 
> <tr>
