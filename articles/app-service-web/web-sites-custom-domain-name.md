...
2.	Поищите ссылку, которая позволяет добавлять или изменять записи DNS. Она может иметь название **Файл зоны**, **Записи DNS** или **Дополнительно**.
...
2.	Добавьте в список полное доменное имя личного домена (например, **www.contoso.com**).
    ...
    >[AZURE.NOTE] Azure проверит указанное здесь имя домена. Поэтому убедитесь, что оно соответствует имени домена, для которого создана запись DNS на [шаге 2](#createdns). Если вы уверены, что
