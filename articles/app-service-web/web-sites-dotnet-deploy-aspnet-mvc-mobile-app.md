...
Стиль [группы связанного списка][группы связанного списка] начальной загрузки делает поле каждой ссылки полностью интерактивным, что значительно удобней для пользователя. Переключитесь в представление для настольного компьютера, чтобы проверить единый стиль отображения и поведения.
...
Вы также можете улучшить представление *Dates*, упорядочив отображение значений даты и времени по дате. Вы можете это сделать с помощью стиля [панелей][панелей] начальной загрузки. Замените содержимое файла *Views\\Home\\AllDates.cshtml* следующим кодом:
...
Этот код создает отдельный тег `<div class="panel panel-primary">` для каждой отличной даты в списке и, как и ранее, использует [группу связанного списка][группу связанного списка] для соответствующих ссылок. Вот как выглядит браузер мобильного устройства при выполнении этого кода:
