...
В этой статье вы ознакомитесь с методом присоединения веб-приложения WordPress в [службе приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714) к [кэшу Redis для Azure][12] с помощью протокола [Memcache][13]. Существующее веб-приложение, которое использует сервер Memcached для кэширования в память, можно перенести в службу приложений Azure и использовать для кэширования основное решение в службе Microsoft Azure с минимальными изменениями кода приложения или без изменений вообще. Кроме того, можно использовать существующие навыки работы с Memcache для создания в службе приложений Azure распределенных приложений высокой степени масштабируемости с кэшем Redis для Azure для кэширования в память, используя при этом известные программные платформы, такие как .NET, PHP, Node.js, Java и Python.  
...
Перейдите к [PECL][6]. В категории кэширования щелкните [memcache][7]. В столбце загрузок щелкните ссылку DLL.
...
> [!NOTE]
> Чтобы установить Visual Studio Team Services, следуйте инструкциям в записи блога о том, [как включить расширение сайта в веб-приложениях][8].
> 
> 
...
### <a name="enable-the-nonssl-port-support-in-azure-redis-cache"></a>Включение поддержки порта подключения без SSL в кэше Redis для Azure
> [!NOTE]
...
### <a name="connect-to-azure-redis-cache-from-rediscli"></a>Подключение к кэшу Redis для Azure через Redis CLI
> [!NOTE]
> При выполнении этого действия предполагается, что программное обеспечение Redis установлено локально на компьютере, на котором проводится разработка. [Установите Redis локально, следуя этим инструкциям][9].
> 
> 
