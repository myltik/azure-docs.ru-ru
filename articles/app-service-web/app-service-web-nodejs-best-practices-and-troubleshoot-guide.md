<properties
	pageTitle="Рекомендации и руководство по устранению неполадок приложений Node в веб-приложениях Azure"
...
# Рекомендации и руководство по устранению неполадок приложений Node в веб-приложениях Azure
...
>[AZURE.WARNING] Будьте осторожны при выполнении действий по устранению неполадок на рабочем сайте. Действия по устранению неполадок приложения рекомендуется выполнять на нерабочем сайте. Например, настройте там промежуточный слот, а когда проблема будет решена, замените промежуточный слот на рабочий.
...
* namedPipeConnectionRetryDelay
    ...
    Этот параметр определяет интервал (в мс), с которым iisnode будет отправлять повторные запросы в файл node.exe через именованный канал. Значение по умолчанию — 250 мс. Общее время ожидания в секундах равно произведению maxNamedPipeConnectionRetry на namedPipeConnectionRetryDelay, разделенному на 1000.
...
>[AZURE.WARNING] Соблюдайте осторожность при включении следующих параметров конфигурации в рабочих приложениях. Эти параметры не рекомендуется включать в активных рабочих приложениях.
...
### В этом сценарии приложение Node отравляет слишком много вызовов.
...
Перейдите на сайт SCM https://yoursite.scm.azurewebsites.net/DebugConsole.
...
В каталоге node\_modules установится средство v8-profiler и все его зависимости. Далее, чтобы выполнить профилирование приложения, измените файл server.js.
...
#### Обнаружение утечки и определение различий кучи для node.js 

Чтобы обнаружить утечки памяти, можно использовать [node-memwatch](https://github.com/lloyd/node-memwatch). Установите memwatch (аналогично средству V8-Profiler) и измените код, чтобы извлечь кучи и выявить в них различия, что позволяет определить утечки памяти в приложении.
...
4.  Холодный запуск. Приложение запускается слишком долго. Если для запуска приложения требуется больше времени, чем задано для общего времени ожидания в секундах (произведение maxNamedPipeConnectionRetry на namedPipeConnectionRetryDelay, разделенное на 1000), iisnode вернет ошибку с кодом 500. Чтобы предотвратить превышение времени ожидания iisnode и возвращение ошибки с кодом 500, увеличьте значения этих параметров для соответствия времени запуска приложения.
...
Вероятнее всего, это происходит потому, что в каталоге node\_modules содержится много файлов и приложение пытается загрузить большинство этих файлов при запуске. Так как по умолчанию эти файлы содержатся в сетевой папке в веб-приложениях Azure, загрузка такого количества файлов занимает больше времени. Чтобы ускорить этот процесс, сделайте следующее:
