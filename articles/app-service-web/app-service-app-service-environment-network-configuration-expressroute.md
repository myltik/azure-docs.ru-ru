...
-  Исходящее сетевое подключение к *ocsp.msocsp.com*. Это необходимо для поддержки функций протокола SSL.
...
1. Установите последнюю версию Azure Powershell со [страницы загрузок Azure][AzureDownloads] (за июнь 2015 г. или позже). В разделе "Средства командной строки" выберите ссылку "Установка" в разделе Windows Powershell, чтобы установить последние командлеты Powershell.
...
- Исходящий трафик на конечные точки Azure и другие, указанные выше в этой статье, **не** проходит по каналу ExpressRoute. Очень важно убедиться, что это так, поскольку, если исходящий трафик из подсети по-прежнему принудительно туннелируется в локальную среду, все попытки создать среду службы приложений завершатся неудачей. 
