...
1. Среда службы приложений (ASE), которая развертывается в виртуальной сети клиента и предоставляет более широкие возможности.  

В этой статье не описывается размещение ASE в виртуальной сети V2. Эта возможность все еще не поддерживаются и не имеет отношения к этой статье. Здесь содержатся сведения о настройке приложений для использования ресурсов в виртуальных сетях V1 или V2.
...
- поддерживается виртуальной сетью V1 (классическая) или V2 (диспетчер ресурсов); 
...
В этом документе основное внимание уделяется интеграции с виртуальной сетью с помощью портала Azure. Чтобы включить интеграцию приложения с виртуальной сетью с помощью PowerShell, следуйте указаниям в статье [Подключение приложения к виртуальной сети с помощью PowerShell][IntPowershell].
...
Пользовательский интерфейс интеграции с виртуальной сетью позволяет делать выбор в списке виртуальных сетей. Тип виртуальных сетей V1 определяется словом "Classic" (классическая) в скобках рядом с именем виртуальной сети. Список отсортирован так, что сначала указаны виртуальные сети V2. На рисунке ниже показано, что можно выбрать только одну виртуальную сеть. Виртуальные сети могут быть неактивными в этом списке по разным причинам, например:
...
##### Включение подключения "точка-сеть" в виртуальной сети V1 #####
Если для виртуальной сети не настроен шлюз или не активировано подключение типа «точка-сеть», это нужно сделать до интеграции. Чтобы выполнить эту задачу для виртуальной сети V1, перейдите на [портал Azure][AzurePortal] и откройте список "Виртуальные сети (классика)". Выберите сети для интеграции, а затем в разделе «Основные компоненты» щелкните поле «VPN-подключения». Здесь вы можете создать VPN-подключение типа «точка-сеть» и настроить шлюз. Активация созданного подключения типа «точка-сеть» и шлюза занимает около 30 минут.

![][8]

##### Включение подключения "точка-сеть" в виртуальной сети V2 #####

Чтобы настроить виртуальную сеть V2 с шлюзом и подключением "точка-сеть", необходимо использовать PowerShell, как описано в статье [Настройка подключения типа "точка-сеть" к виртуальной сети с помощью PowerShell][V2VNETP2S]. Пользовательский интерфейс для выполнения этой процедуры пока недоступен.

### Создание предварительно настроенной виртуальной сети ###
Если вы хотите создать виртуальную сеть, для которой настроен шлюз и подключение "точка-сеть", можно воспользоваться сетевым пользовательским интерфейсом службы приложений. Однако помните, что сейчас вы можете создать только виртуальную сеть V2. Если вы хотите создать виртуальную сеть V1 с шлюзом и подключением "точка-сеть", вам придется сделать это вручную с помощью сетевого пользовательского интерфейса.

Чтобы создать виртуальную сеть V2 с помощью интерфейса интеграции с виртуальной сетью, выберите пункт **Создать виртуальную сеть** и укажите следующие сведения:
...
>[AZURE.NOTE] Создание виртуальной сети с шлюзом V2 занимает около 30 минут. В настоящее время интеграция виртуальной сети с приложением не поддерживается. После создания виртуальной сети с шлюзом необходимо вернуться в пользовательский интерфейс интеграции с виртуальной сетью и выбрать новую виртуальную сеть.
...
>[AZURE.NOTE] Интеграция с виртуальной сетью обеспечивает доступ к локальным ресурсам с помощью VPN-подключения типа «сеть-сеть»; использование ExpressRoute VPN сейчас не поддерживается. Это относится к интеграции с виртуальной сетью V1 или V2. Если требуется получить доступ к ресурсам через VPN ExpressRoute, можно использовать среду ASE, поддерживающую выполнение в виртуальной сети.
