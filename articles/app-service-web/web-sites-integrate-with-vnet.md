<properties 
	pageTitle="Интеграция приложения с виртуальной сетью Azure" 
...
В этом документе описана функция службы приложений Azure — интеграция с виртуальной сетью. Также приведены инструкции по ее настройке для приложений в [службе приложений Azure](http://go.microsoft.com/fwlink/?LinkId=529714). Если вы не знакомы с виртуальными сетями Azure, вот краткое описание. Эта функция позволяет размещать множество ресурсов Azure в сети, недоступной из Интернета, а также самостоятельно управлять доступом к ним. Эти сети можно подключать к локальным сетям с помощью разных технологий VPN. Начать изучение виртуальных сетей Azure лучше всего со статьи [Обзор виртуальной сети][VNETOverview].

Служба приложений Azure реализована в виде двух моделей. Наиболее распространенный вариант — это мультитенантные системы с поддержкой всей линейки тарифных планов. Вторая модель представлена средой службы приложений (ASE), которая развертывается в виртуальной сети клиента и предоставляет более широкие возможности. В среде службы приложений интеграция с виртуальной сетью обычно не нужна, так как эта система уже находится в вашей виртуальной сети и имеет доступ ко всем ее ресурсам. Интеграция с виртуальной сетью в рамках среды ASE может быть полезна, если вы хотите получить доступ к ресурсам в другой виртуальной сети, которая не подключена к виртуальной сети, в которой размещена ASE.

Интеграция с виртуальной сетью предоставляет веб-приложению доступ к ресурсам виртуальной сети, при этом само приложение остается для этой сети недоступным. Обычно эта функция используется для предоставления веб-приложению доступа к базе данных или веб-службе, которые выполняются на виртуальной машине в виртуальной сети Azure. Благодаря интеграции с виртуальной сетью вам не нужно предоставлять общедоступную конечную точку приложениям, запущенным на виртуальной машине. Вместо этого будут использоваться частные адреса, недоступные из Интернета.

Интеграция с виртуальной сетью связана со следующими особенностями:

- требуется тарифный план «Стандартный» или «Премиум»; 
- поддерживается только классическими виртуальными сетями и сетями V1; 
...
Вы можете подключиться к существующей виртуальной сети или создать новую. Во время создания новой сети будет автоматически настроен шлюз с динамической маршрутизацией и активировано VPN-подключение типа «точка-сеть».

>[AZURE.NOTE]Настройка интеграции с новой виртуальной сетью может занять несколько минут.
...
###Включение интеграции с существующей виртуальной сетью###
Пользовательский интерфейс интеграции с виртуальной сетью включает список виртуальных сетей V1. В примере, который представлен на рисунке ниже, мы можем выбрать только одну виртуальную сеть. Виртуальные сети могут быть неактивными в этом списке по разным причинам, например:

- виртуальная сеть относится к другой подписке, к которой ваша учетная запись имеет доступ;
...
Также стоит отметить, что в список не включены виртуальные сети V2, так как интеграция с ними пока не поддерживается.
...
Если для виртуальной сети не настроен шлюз или не активировано подключение типа «точка-сеть», это нужно сделать до интеграции. Для этого на портале Azure откройте список «Виртуальные сети (классические)». Выберите сети для интеграции, а затем в разделе «Основные компоненты» щелкните поле «VPN-подключения». Здесь вы можете создать VPN-подключение типа «точка-сеть» и настроить шлюз. Активация созданного подключения типа «точка-сеть» и шлюза занимает около 30 минут.

![][8]

### Создание виртуальной сети и интеграция с ней ###
Если вы хотите создать новую виртуальную сеть, помните, что сейчас вы можете создать только сеть V1 или классическую виртуальную сеть. Чтобы создать виртуальную сеть V1 с помощью интерфейса интеграции с виртуальной сетью, выберите пункт «Создать виртуальную сеть», а затем укажите для новой виртуальной сети имя и адресное пространство.
...
>[AZURE.NOTE]Создание новой виртуальной сети с работающими шлюзами может занять до 30 минут. После завершения этого процесса интерфейс обновится.
...
Виртуальная сеть V1, созданная через интерфейс интеграции с виртуальной сетью, будет включена в ту же группу ресурсов, что и ваше приложение.
...
Теперь в пользовательском интерфейсе интеграции с виртуальной сетью на уровне приложения отображаются те же сведения, что и на уровне плана ASP. Они представлены ниже.
...
**Сертификаты.** Состояние сертификатов отображает результаты проверки, которую выполняет служба приложений. Цель этой проверки — оценка пригодности сертификатов, используемых для VPN-подключения. При первой интеграции любого приложения в рамках ASP с виртуальной сетью происходит обязательный обмен сертификатами для обеспечения безопасности подключения. Одновременно с сертификатами мы получаем конфигурацию DNS, маршруты и другую полезную информацию о сети. Если сертификаты или сведения о сети изменились, щелкните элемент «Синхронизация с сетью». **Примечание**. Выполнение синхронизации с сетью сопровождается кратковременной потерей подключения между приложением и виртуальной сетью. Хотя приложение не будет перезапущено, потеря подключения может привести к нарушению его работоспособности.
...
>[AZURE.NOTE]Интеграция с виртуальной сетью обеспечивает доступ к локальным ресурсам с помощью VPN-подключения типа «сеть-сеть»; использование ExpressRoute VPN сейчас не поддерживается.
...
Использовать эту функцию могут только те приложения, которые входят в план службы приложений класса «Стандартный» или «Премиум». Дополнительные сведения о ценах на планы см. на странице [Стоимость службы приложений][ASPricing].

Из-за особенностей VPN-подключения типа «точка – сеть» трафик всегда тарифицируется как исходящий при подключении к виртуальной сети, даже если виртуальная сеть находится в том же центре обработки данных. Узнать соответствующие расценки можно на странице [Сведения о ценах за передачу данных][DataPricing].
...
####Доступ к ресурсам виртуальной сети для отладки####

У приложения могут возникнуть проблемы с доступом к определенной комбинации «узел:порт». Это происходит по многим причинам. Чтобы устранить проблему, начните с проверки самых простых вещей.

- Отображается ли на странице портала шлюз в рабочем состоянии?
...
- На нужном узле запущен брандмауэр, который блокирует доступ к порту приложения для диапазона IP-адресов «точка-сеть».
...
Если виртуальная машина, входящая в виртуальную сеть, может подключиться к ресурсу локальной системы, а приложению это не удается, скорее всего, это вызвано следующими причинами. 1) На локальном шлюзе не настроены маршруты для диапазона IP-адресов «точка-сеть». 2) Группы безопасности сети блокируют доступ для диапазона IP-адресов «точка-сеть». 3) Локальные брандмауэры блокируют трафик из диапазона IP-адресов «точка-сеть». 4) В виртуальной сети определен пользовательский маршрут, который не позволяет трафику в рамках подключения типа «точка-сеть» попасть в локальную сеть.

## Гибридные подключения и среда службы приложений##
