...
>[AZURE.NOTE]Это обширное руководство. Если вас интересует краткое введение в службу приложений Azure и веб-проекты Visual Studio, см. статью [Создание веб-приложения ASP.NET в службе приложений Azure](web-sites-dotnet-get-started.md).
>
>Если же вам нужно приступить к работе со службой приложений Azure, прежде чем регистрировать учетную запись Azure, посетите страницу [Попробовать службу приложений](http://go.microsoft.com/fwlink/?LinkId=523751), где вы сразу можете создать недолговечное базовое веб-приложение в службе приложений. Никаких кредитных карт и обязательств.
...
Чтобы настроить среду разработки, необходимо установить [Visual Studio 2013 с обновлением 4](http://go.microsoft.com/fwlink/?LinkId=390521) или более позднюю версию и последнюю версию пакета [Azure SDK для .NET](http://go.microsoft.com/fwlink/?linkid=324322&clcid=0x409). Эта статья была написана для версии Visual Studio с обновлением 4 и пакета SDK 2.7.1. Аналогичные инструкции можно использовать и для Visual Studio 2015 с последней версией пакета [Azure SDK для .NET](http://go.microsoft.com/fwlink/?linkid=518003&clcid=0x409), но некоторые экраны будут отличаться от приведенных здесь иллюстраций.
...
1. В диалоговом окне **Новый проект ASP.NET** выберите шаблон **MVC**. Убедитесь, что параметр **Проверка подлинности** имеет значение **Учетные записи индивидуальных пользователей**, флажок **Размещать в облаке** установлен, а параметр **Веб-приложение** выбран.
...
2. Выберите **Создать новый сервер**, введите имя сервера, имя пользователя и пароль.
...
1. В **обозревателе решений** щелкните проект **ContactManager**, а затем нажмите F4, чтобы открыть диалоговое окно **Свойства**.
...
2. В окне **Консоль диспетчера пакетов** введите следующую команду:
	...
	Команда **add-migration Initial** создает файл с именем **&lt;date\_stamp&gt;Initial** в папке *Миграции*. Код в этом файле создает таблицы базы данных. Первый параметр (**Initial**) используется для создания имени файла. Новые файлы классов можно просмотреть в **обозревателе решений**.
...
Помимо проверки подлинности, в этом учебнике используются роли для реализации авторизации. Только пользователи, добавленные к роли *canEdit*, смогут изменять данные (то есть создавать, править и удалять контакты).

1. Выполните инструкции из учебника [Приложение MVC 5 со входом через Facebook, Twitter, LinkedIn и Google OAuth2](http://www.asp.net/mvc/tutorials/mvc-5/create-an-aspnet-mvc-5-app-with-facebook-and-google-oauth2-and-openid-sign-on#goog) (раздел **Создание приложения Google для OAuth2 для настройки приложения Google для OAuth2**).
...
В этом разделе будет временно изменен метод **ExternalLoginConfirmation** в контроллере Account для добавления новых пользователей, регистрирующихся с помощью поставщика OAuth, к роли *canEdit*. Надеемся в будущем предоставить средство, аналогичное [WSAT](http://msdn.microsoft.com/library/ms228053.aspx), которое позволит вам создавать и изменять учетные записи пользователей и роли. А пока то же самое можно делать с помощью временного кода.
...
Команда **Update-Database** запускает метод **Seed**, а затем метод **AddUserAndRole**, добавленный ранее. Метод **AddUserAndRole** создает пользователя **user1@contoso.com* и добавляет его к роли *canEdit*.

## Защита приложения с помощью SSL и атрибута Authorize ##

В этом разделе применяется атрибут [Authorize](http://msdn.microsoft.com/library/system.web.mvc.authorizeattribute.aspx) для ограничения доступа к методам действий. Анонимные пользователи смогут просматривать только метод действия **Index** контроллера Home. Зарегистрированные пользователи смогут просматривать контактные данные (страницы **индекса** и **сведений** контроллера Cm), страницу сведений о программе и страницу с контактной информацией. Только пользователи с назначенной ролью *canEdit* получат доступ к методам действий, изменяющих данные.
...
1. Щелкнув ссылку «Изменить» на странице, вы перейдете на страницу входа (так как новый локальный пользователь не добавляется к роли *canEdit*).
...
1. Запишите идентификатор учетной записи Google, с которой вы зарегистрировались для добавления в роль **canEdit**, а также идентификатор пользователя **user1@contoso.com*. Только эти пользователи должны входить в роль **canEdit**. (Вы проверите это на следующем шаге.)
...
3. Убедитесь, что идентификаторы **UserId** принадлежат пользователю **user1@contoso.com* и зарегистрированной вами учетной записи Google.
...
Расширенные инструкции по использованию Entity Framework см. в учебнике [Начало работы с EF и MVC](http://www.asp.net/mvc/tutorials/getting-started-with-ef-using-mvc/creating-an-entity-framework-data-model-for-an-asp-net-mvc-application).

Этот учебник написан [Риком Андерсоном](http://blogs.msdn.com/b/rickandy/) (Rick Anderson) (Twitter [@RickAndMSFT](https://twitter.com/RickAndMSFT)) в соавторстве с Томом Дейкстрой (Tom Dykstra) и Бэрри Доррансом (Barry Dorrans)(Twitter [@blowdart](https://twitter.com/blowdart)).
