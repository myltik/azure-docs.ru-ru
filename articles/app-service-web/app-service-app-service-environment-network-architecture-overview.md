...
Когда среда службы приложений (ASE) использует общедоступный виртуальный IP-адрес для приложений, на него поступает весь входящий трафик.  Он включает в себя трафик HTTP и HTTPS приложений, а также прочий трафик FTP, удаленной отладки и операций управления Azure.  Полный список всех портов (обязательных и необязательных) с общедоступным виртуальным IP-адресом см. в статье об [управлении входящим трафиком][controllinginboundtraffic] в среде службы приложений. 
...
Поскольку среда службы приложений развертывается в подсети, группы безопасности сети можно использовать для управления входящим трафиком подсети.  Дополнительные сведения о том, как управлять входящим трафиком в среде службы приложений, см. в следующей [статье][controllinginboundtraffic].

Дополнительную информацию о том, как разрешить исходящие подключения к Интернету в среде службы приложений, см. в статье о работе с [Express Route][ExpressRoute].  Метод, описанный в статье, применяется также при работе с подключением «сеть — сеть» и при использовании принудительного туннелирования.
...
Дополнительные сведения о входящих портах, используемых средами службы приложений, и о регулировании входящего трафика с использованием сетевых групп безопасности см. [здесь][controllinginboundtraffic].

Дополнительные сведения о том, как использовать определенные пользователями маршруты для предоставления средам службы приложений исходящего доступа к Интернету, см. в этой [статье][ExpressRoute]. 
