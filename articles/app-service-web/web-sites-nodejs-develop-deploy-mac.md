...
1. В командной строке измените каталоги на **helloworld** и введите следующие команды для инициализации локального репозитория Git. 
	...
	> [AZURE.NOTE]**Команда Git недоступна?** [Git](http://git-scm.com/%20target="_blank) является распределенной системой управления версиями, которую можно использовать для развертывания веб-сайта Azure. Указания по установке для своей платформы см. на [странице скачивания Git](http://git-scm.com/download%20target="_blank").
...
Хотя приведенные в этой статье действия позволяют использовать портал Azure для создания веб-приложения, можно также использовать [средства командой строки Azure для Mac и Linux](../xplat-cli.md), которые позволяют выполнить те же самые операции.
...
* [Средства командной строки Azure для Mac и Linux](../xplat-cli.md)
