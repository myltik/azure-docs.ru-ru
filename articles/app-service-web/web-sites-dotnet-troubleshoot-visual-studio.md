...
## Обзор
В этом учебнике показано, как использовать средства Visual Studio, которые позволяют отлаживать работу веб-приложения в [службе приложений](http://go.microsoft.com/fwlink/?LinkId=529714) путем запуска веб-приложения удаленно в [режиме отладки](http://www.visualstudio.com/get-started/debug-your-app-vs.aspx) или путем просмотра журналов приложения и веб-сервера.
...
1. Если вы еще не выполнили вход в Azure в Visual Studio, нажмите кнопку **Подключиться к Azure** в **Обозревателе сервера**.
    ...
    Также вы можете установить сертификат управления, обеспечивающий доступ к вашей учетной записи. Чтобы установить сертификат, щелкните правой кнопкой мыши узел **Azure** в **Обозревателе серверов** и выберите пункт **Управление подписками и их фильтрация** в контекстном меню. В диалоговом окне **Manage Azure Subscriptions** (Управление подписками Azure) откройте вкладку **Сертификаты**, а затем щелкните **Импорт**. Следуя указаниям, загрузите и импортируйте файл подписки (файл *.publishsettings*) для своей учетной записи Azure.
...
6. В раскрывающемся списке **Профиль** выберите тот же профиль, который вы использовали в разделе [Начало работы с Azure и ASP.NET][GetStarted].
...
9. В **Обозревателе серверов** щелкните правой кнопкой мыши веб-приложение и выберите пункт **Подключить отладчик**.
    ...
    **Примечание.** При возникновении сложностей с запуском отладчика попробуйте сделать это с помощью **Обозревателя облака** вместо **Обозревателя серверов**.
...
4. В **обозревателе решений** щелкните правой кнопкой мыши проект (не проект веб-задания) и выберите **Опубликовать**.
...
6. Щелкните вкладку **Параметры** и измените значение **Конфигурация** на **Отладка**, затем щелкните пункт **Опубликовать**.
...
7. В **обозревателе решений** разверните узлы **Azure > Служба приложений > группа ресурсов > веб-приложение > WebJobs > Continuous**, а затем щелкните правой кнопкой мыши элемент **ContosoAdsWebJob**.
...
14. В **обозревателе сервера** щелкните правой кнопкой мыши узел ContosoAdsWebJob и нажмите кнопку **Представление панели мониторинга**.
...
1. Откройте файл *Controllers\\HomeController.cs* и замените методы `Index`, `About` и `Contact` на следующий код, чтобы добавить инструкции `Trace` и инструкцию `using` для `System.Diagnostics`:
...
6. На странице **Трассировка приложения** щелкните **Просмотр сведений** в первой строке (не строке BrowserLink).
...
3. В окне браузера, в котором теперь отображается страница **Связь**, нажмите кнопку **Главная**, затем **О программе** и щелкните **Контакт**.
...
5. В окне Visual Studio **Веб-приложение Azure** откройте вкладку **Журналы**, а затем щелкните **Настроить ведение журналов на портале управления**.
...
11. В окне браузера, в котором отображается веб-приложение приложения, щелкните **Главная**, а затем **О программе** и **Связь**.
    
     Ведение журнала сведений, полученных при просмотре веб-страниц, будут записаны в учетную запись хранения.
12. На вкладке **Журналы** окна **Веб-приложение Azure** в Visual Studio нажмите **Обновить** в разделе **Сводка по диагностике**.
...
4. В колонке [Портал Azure](https://portal.azure.com) **Параметры** вашего веб-приложения щелкните **Учетные данные развертывания**, а затем введите новые имя пользователя и пароль.
