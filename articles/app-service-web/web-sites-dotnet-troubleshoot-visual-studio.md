...
3. Разверните группу ресурсов, которая включает веб-приложение, созданное в руководстве [Начало работы с Azure и ASP.NET][GetStarted], а затем щелкните узел веб-приложения правой кнопкой мыши и выберите команду **Просмотреть параметры**.
	...
	Если необходимо выполнить задачу по управлению веб-приложением, которую невозможно выполнить в этом окне, щелкните **Открыть на портале управления**, чтобы открыть портал предварительной версии Azure в браузере. Дополнительную информацию см. в разделе [Настройка веб-приложений](/ru-RU/manage/services/web-sites/how-to-configure-websites/#howtochangeconfig).
...
1. В окне Visual Studio **Веб-приложение Azure** откройте вкладку **Журналы**, а затем щелкните **Настроить ведение журналов на портале управления**.
	...
	После этого на портале Azure откроется вкладка **Настройка** для вашего веб-приложения.

2. На вкладке портала **Настройка** прокрутите вниз до раздела диагностики приложения и установите переключатель **Ведение журнала приложения (табличное хранилище)** в положение **Включено**.
...
6. На вкладке **Настройка** портала нажмите **Сохранить**.
...
3. В колонке вашего веб-приложения на портале предварительной версии Azure выберите **Настройки > Учетные данные развертывания**, а затем введите новые имя пользователя и пароль.
...
5. В новом окне браузера перейдите по URL-адресу, показанному в поле **Имя узла FTP** или **Имя узла FTPS** в колонке **Веб-приложение** на портале.
...
* [LogParser](http://www.microsoft.com/download/details.aspx?id=24659)<br/> Средство для просмотра данных в журналах веб-сервера (файлы *.log*).
