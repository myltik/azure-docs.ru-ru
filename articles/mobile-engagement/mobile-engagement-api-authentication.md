...
Это вспомогательное руководство по миграции с Capptain, где используются API, на платформу Azure Mobile Engagement для клиентов. Здесь содержатся более подробные сведения о том, как запустить механизм проверки подлинности для новых API.
...
После миграции с API Capptain в новые API Azure Mobile Engagement основные различия будут заключаться в процессе проверки подлинности. Ранее API поддерживали обычную проверку подлинности. Теперь же для проверки подлинности необходимо использовать токен OAuth Microsoft Azure Active Directory.
