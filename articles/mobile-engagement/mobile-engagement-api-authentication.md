...
2. Назначьте роль субъекта-службы, например Reader или Owner, с помощью [интерфейса командной строки Azure](../xplat-cli-install.md).
	...
	Если вы используете Windows, измените переменную среды `PATH`, добавив `C:\Program Files (x86)\Microsoft SDKs\Azure\CLI\bin`, чтобы активировать возможность использования команд Azure.
...
1. Установите последнюю версию Azure PowerShell. Инструкции по скачиванию см. [здесь](../powershell-install-configure.md). 

2. Откройте Windows PowerShell в режиме администратора и убедитесь, что вы установили [командлеты Azure Resource Manager](https://msdn.microsoft.com/library/mt125356.aspx).
...
7. Скопируйте текст сценария [New-AzureRmServicePrincipalOwner.ps1](https://raw.githubusercontent.com/matt-gibbs/azbits/master/src/New-AzureRmServicePrincipalOwner.ps1) на локальный компьютер и запустите сценарий.
	...
	По завершении сценария отобразятся четыре значения, которые нужны для программной аутентификации с использованием AD: **TenantId**, **SubscriptionId**, **ApplicationId** и **Secret**.

	Скопируйте эти значения для справки. Чтобы получить маркер доступа сейчас, в качестве значения TenantId следует использовать `{TENANT_ID}`, в качестве значения ApplicationId — `{CLIENT_ID}`, а в качестве значения Secret — `{CLIENT_SECRET}`.
