...
> [!TIP]
> Атрибут `android:label` позволяет выбрать имя службы Engagement в том виде, в каком оно будет выводиться для конечных пользователей на экране "Работающие службы" их телефонов. Рекомендуется задать для этого атрибута значение `"<Your application name>Service"` (например, `"AcmeFunGameService"`).
> 
> 
