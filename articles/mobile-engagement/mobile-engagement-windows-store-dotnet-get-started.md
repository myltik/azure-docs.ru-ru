...
1. Войдите на портал Azure, а затем щелкните элемент **+СОЗДАТЬ** в нижней части экрана.

2. Последовательно выберите элементы **Службы приложений**, **Mobile Engagement** и **Создать**.
...
3. Во всплывающем окне введите следующие сведения:
    ...
    ![][8]
	 ...
	 - **Платформа**. Выберите целевую платформу для этого приложения (**Универсальная платформа для Windows**) (если приложение предназначено для нескольких платформ, повторите операции из этого учебника для каждой из них).
	 ...
	 - **Расположение**. Выберите центр обработки данных, в котором будет размещено это приложение и его коллекция.
	 - **Коллекция**. Если вы уже создали приложение, выберите созданную ранее коллекцию, а если нет, щелкните элемент **Новая коллекция**.
...
5. Щелкните элемент **Сведения о подключении**, чтобы отобразить параметры раздела **Подключение**, которые следует использовать при интеграции пакета SDK в вашем мобильном приложении.

    ![][10]

6. Скопируйте **строку подключения**. Это необходимо, чтобы обозначить данное приложение в коде приложения и подключиться к Mobile Engagement из вашего универсального приложения.

    ![][11]

##<a id="connecting-app"></a>Подключение приложения к серверной части Mobile Engagement

В этом учебнике описаны действия по базовой интеграции, т. е. минимум, требуемый для сбора данных и отправки push-уведомлений. Информацию о полной интеграции см. в документации к пакету SDK для Mobile Engagement для универсальных приложений для Windows.
...
2. Во всплывающем окне последовательно выберите пункты **Приложения Магазина** -> **Универсальные приложения** -> **Пустое приложение (универсальные приложения)**. Введите значения в поля **Имя** и **Имя решения**, а затем нажмите кнопку **ОК**.
...
1. Установите пакет Nuget [Mobile Engagement Windows Universal SDK] в проект. Если приложение предназначено как для платформы Windows, так и для Windows Phone, это понадобится сделать для обоих проектов. Один и тот же пакет Nuget добавляет нужные двоичные файлы для определенной платформы в каждый проект.

2. Откройте файл **Package.appxmanifest** и добавьте следующий код, если он не был добавлен автоматически:

		Internet (Client)

	![][20]

3. Теперь скопируйте строку подключения, которую вы скопировали ранее для приложения Mobile Engagement, и вставьте ее в файл `Resources\EngagementConfiguration.xml` между тегами `<connectionString>` и `</connectionString>`:
...
Чтобы начать отправку данных и убедиться, что пользователи активны, отправьте по крайней мере один экран (действие) в серверную часть Mobile Engagement. Для этого нужно создать подкласс **MainPage** с помощью **EngagementPage** в пакете SDK для Mobile Engagement.

1. 	Добавьте оператор `using`:

		   using Microsoft.Azure.Engagement;

2. Замените на **EngagementPage** суперкласс **MainPage**, который расположен перед **Page**:
...
2. Вы перейдете на страницу **Параметры** на портале Mobile Engagement для вашего приложения. На этой странице щелкните вкладку **Монитор**, как показано ниже. ![][30]
...
1. В файле `Package.appxmanifest`, на вкладке **Приложение**, в разделе **Уведомления**, выберите пункт **Да** для параметра **Всплывающие уведомления:**
...
1. Необходимо связать свое приложение с приложением Магазина Windows, чтобы получить **идентификатор безопасности пакета (SID)** и **секретный ключ** (секрет клиента). Вы можете создать приложение в [Центре разработки для Магазина Windows], а затем **связать приложение с Магазином** в Visual Studio.
...
3. Выполните следующие шаги, чтобы настроить первое поле кампании: ![][38]

	а. Присвойте имя кампании.

	b. Выберите для параметра **Время доставки** значение *Любое время*, чтобы приложение могло принимать уведомления независимо от того, запущено оно или нет.
...
4. Прокрутите окно вниз и в разделе **Содержимое** выберите **Только уведомления**. ![][39]
