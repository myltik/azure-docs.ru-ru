...
## Настройка Mobile Engagement для вашего приложения Android
...
4. Выберите **Blank Activity** (Пустое действие) — это единственный экран этого приложения — и нажмите кнопку **Next** (Далее).
...
Если хотите сохранить `ActionBar` в приложении, см. статью [Параметры отчетов для службы Engagement на устройствах Android](mobile-engagement-android-advanced-reporting.md#modifying-your-codeactivitycode-classes).
...
Во время кампаний Mobile Engagement позволяет взаимодействовать с пользователями и ОХВАТЫВАТЬ АУДИТОРИЮ с помощью push-уведомлений и сообщений в приложении. На портале Mobile Engagement этот модуль называется МОДУЛЕМ ОБРАБОТКИ РЕКЛАМНЫХ КАМПАНИЙ. В следующем разделе показано, как настроить приложение для приема уведомлений и сообщений.
