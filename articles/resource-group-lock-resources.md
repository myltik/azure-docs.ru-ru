...
- **ReadOnly** означает, что авторизованные пользователи смогут только читать ресурс, но не смогут не только удалить его, но и производить с ним какие-либо действия. Разрешение для ресурса будет ограничено до роли **Читатель**. Применение уровня блокировки **ReadOnly** может привести к непредвиденным результатам, так как некоторые операции, кажущиеся операциями чтения, на самом деле требуют дополнительных действий. Например, установка уровня блокировки **ReadOnly** для учетной записи хранения не позволит всем пользователям получить список ключей. Операция отображения списка ключей обрабатывается с помощью запроса POST, поскольку возвращаемые ключи могут быть доступны для операций записи. Еще один пример — установка уровня блокировки **ReadOnly** для ресурса службы приложений не позволит обозревателю сервера Visual Studio отображать файлы для ресурса, поскольку для их взаимодействия требуется доступ на запись.
...
При применении блокировки к родительской области все дочерние ресурсы наследуют ту же блокировку. Приоритет имеет наиболее строгая блокировка.
...
2. Чтобы добавить блокировку, выберите **Добавить**. Если вы хотите создать блокировку на родительском уровне, чтобы она наследовалась выбранным ресурсом, выберите родительский ресурс (например подписку, как показано ниже).

      ![добавление блокировки](./media/resource-group-lock-resources/add-lock.png)

3. Присвойте блокировке имя и уровень блокировки. При необходимости можно добавить примечания, описывающие причину блокировки.
