...
Если к ресурсу применяется более одной блокировки, приоритет имеет наиболее строгая блокировка. Например, если применить **ReadOnly** на уровне родителя (такого как группа ресурсов) и **CanNotDelete** к ресурсу в пределах этой группы, то приоритет будет иметь более строгая блокировка (ReadOnly) родительского элемента.
...
Для уровня блокировки (lock-level) укажите **CanNotDelete** или **ReadOnly**.
