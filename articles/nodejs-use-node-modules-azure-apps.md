...
При установке модулей они сохраняются в каталоге **node\_modules**, находящемся в корне структуры каталогов приложения. Каждый модуль в каталоге **node\_modules** использует свой собственный каталог **node\_modules**, в котором содержатся все модули, от которых он зависит, и это снова повторяется для каждого модуля до самого конца цепочки зависимостей. Это позволяет каждому установленному модулю иметь свои требования к версии модулей, от которых он зависит, но в то же время может привести к большому разрастанию структуры каталогов.
...
* Выполните **npm install** на компьютере под управлением Windows, на котором установлены все необходимые компоненты собственного модуля. Затем развернуть созданную папку **node\_modules** как часть приложения на веб-сайтах Azure.
