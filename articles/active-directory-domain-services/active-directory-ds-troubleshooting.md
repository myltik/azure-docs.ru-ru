...
Учетная запись пользователя остается в управляемом домене в отключенном состоянии, даже если в каталоге Azure AD повторно создать учетную запись с тем же именем участника-пользователя. Чтобы удалить учетную запись пользователя из управляемого домена, принудительно удалите ее из своего клиента Azure AD.
