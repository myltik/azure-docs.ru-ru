...
1. Перейдите к **приложений** своего каталога Azure AD в [портал Azure](https://portal.azure.com/#blade/Microsoft_AAD_IAM/StartboardApplicationsMenuBlade/AllApps/menuId/).
2. Выберите **все приложения** в **Показать** раскрывающегося списка. Выберите **любой** в **состояние приложения** раскрывающегося списка. Выберите **любой** в **видимость приложения** раскрывающегося списка.
3. Тип **домен Azure AD Sync Services** в поле поиска. Если оно существует, щелкните его и нажмите кнопку **удалить** на панели инструментов, чтобы удалить его.
...
Чтобы полностью удалить учетную запись пользователя из управляемого домена, навсегда удалите пользователя из своего клиента Azure AD. Используйте `Remove-MsolUser` командлета PowerShell с `-RemoveFromRecycleBin` параметра, как описано в этом [статьи MSDN](https://msdn.microsoft.com/library/azure/dn194132.aspx).
