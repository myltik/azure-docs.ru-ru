...
В этой статье приводятся советы по решению проблем, с которыми вы можете столкнуться при настройке или администрировании доменных служб Azure AD.
...
- **Синхронизированные учетные записи:** если учетные записи пользователей синхронизируются из локального каталога, убедитесь в следующем.
    - Вы развернули общедоступную версию Azure AD Connect или обновили свою версию до нее. В более ранних версиях не выполняется синхронизация хэшей учетных данных, необходимых для проверки подлинности NTLM/Kerberos.
...
- в [пользовательском канале Azure Active Directory](http://feedback.azure.com/forums/169401-azure-active-directory). Чтобы мы получили ваш вопрос, он должен начинаться со слова **AADDS**.
