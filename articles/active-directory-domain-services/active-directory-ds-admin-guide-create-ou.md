...
> [AZURE.NOTE] Только члены группы "Администраторы контроллера домена AAD" имеют необходимые разрешения для создания пользовательского подразделения. Выполните следующие действия от имени пользователя, принадлежащего к этой группе.
...
6. В диалоговом окне **Создание подразделения** укажите **имя** нового подразделения. Добавьте краткое описание подразделения. Кроме того, можно задать значение в поле **Управляется** для подразделения. Нажмите кнопку **ОК**, чтобы создать пользовательское подразделение.
