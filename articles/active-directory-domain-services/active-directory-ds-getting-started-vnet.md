...
- Убедитесь, что регион, в котором находится выбранная виртуальная сеть, поддерживается доменными службами Azure AD. Текущий список поддерживаемых регионов Azure можно найти на странице [регионов](https://azure.microsoft.com/regions/#services).
...
- Доменные службы Azure AD не поддерживаются виртуальными сетями, созданными с помощью диспетчера ресурсов Azure.
...
> [AZURE.NOTE]Убедитесь, что выбранная или создаваемая виртуальная сеть для использования с доменными службами Azure AD принадлежит к региону Azure, поддерживаемому доменными службами Azure AD. Список регионов Azure, в которых доступны доменные службы Azure AD, приведен на странице [регионов](active-directory-ds-regions.md).
...
7. Убедитесь, что вы выбрали поддерживаемый регион Azure в раскрывающемся списке **Расположение**. Список регионов Azure, в которых доступны доменные службы Azure AD, приведен на странице [регионов](active-directory-ds-regions.md). Этот шаг очень важен. Если вы выберете виртуальную сеть в регионе Azure, который не поддерживается доменными службами Azure AD, вы не сможете включить службу в виртуальной сети.
