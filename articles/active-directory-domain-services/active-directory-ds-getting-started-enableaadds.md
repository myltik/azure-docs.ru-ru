...
7. Укажите **Имя домена DNS для доменных служб**.
   ...
   - Кроме того, в этот список можно добавить пользовательское имя домена, введя его.
     ...
     > [AZURE.WARNING] Убедитесь, что длина указываемого префикса доменного имени (например, contoso для доменного имени contoso.com) не превышает 15 символов. Если префикс доменного имени превышает 15 символов, создать домен доменных служб Azure AD нельзя.
