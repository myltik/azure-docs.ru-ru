...
Если вы еще не знакомы с прокси приложения Azure AD, узнайте, [как обеспечить безопасный удаленный доступ к локальным приложениям](../active-directory/active-directory-application-proxy-get-started.md).
...
Ранее была выполнена подготовка виртуальной машины Windows Server и ее присоединение к управляемому домену. В этой задаче будет установлен соединитель прокси приложения Azure AD в этой виртуальной машине.
...
* **Публикация приложений с помощью прокси приложения Azure AD.** Публикация приложений, работающих в виртуальных машинах Azure, с использованием прокси приложения Azure AD. Дополнительные сведения см. в статье [Публикация приложений с помощью прокси приложения Azure AD](../active-directory/application-proxy-publish-azure-portal.md).
...
Для приложений с использованием интегрированной проверки подлинности Windows активируйте единый вход (SSO), разрешив соединителям прокси приложения олицетворять пользователей, а также отправлять и получать токены от их имени. Настройка ограниченного делегирования Kerberos для коннектора, чтобы предоставлять необходимые разрешения для доступа к ресурсам управляемого домена. Используйте механизм ограниченного делегирования Kerberos на основе ресурсов в управляемых доменах для повышения уровня безопасности.
