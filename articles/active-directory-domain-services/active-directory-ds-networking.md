...
* Включить доменные службы Azure AD можно в классической виртуальной сети Azure. Однако поддержка классических виртуальных сетей скоро будет прекращена. Мы рекомендуем использовать виртуальные сети Resource Manager для созданных управляемых доменов.
...
* **Близкое расположение к рабочим нагрузкам Azure.**Выберите виртуальную сеть, где размещаются или будут размещены виртуальные машины, которым необходим доступ к доменным службам Azure AD. Вы также можете подключиться к виртуальным сетям, если рабочая нагрузка развертывается в виртуальной сети, отличной от виртуальной сети управляемого домена.
...
Порт 5986 используется для выполнения задач управления с помощью удаленного взаимодействия PowerShell, выполняемого для управляемого домена. Контроллеры домена для управляемого домена обычно не прослушивают этот порт. Служба открывает этот порт на контроллерах управляемого домена, только если для управляемого домена необходимо выполнить операции управления или обслуживания. Сразу после завершения операции служба закрывает этот порт на контроллерах управляемого домена.

Порт 3389 используется для подключений удаленного рабочего стола к управляемому домену. Этот порт остается отключенным для управляемого домена. Служба включает этот порт, только если требуется подключение к управляемому домену для устранения неполадок, как правило, в ответ на инициированный запрос к службе. Этот механизм не используется регулярно, так как задачи управления и мониторинга выполняются с помощью удаленного взаимодействия PowerShell. Этот порт используется только в тех редких случаях, когда нам требуется удаленное подключение к управляемому домену для расширенного устранения неполадок. Порт будет закрыт сразу после устранения неполадок.
