...
Порт 3389 используется для подключений удаленного рабочего стола к управляемому домену. Этот порт остается отключенным для управляемого домена. Служба включает этот порт, только если требуется подключение к управляемому домену для устранения неполадок в ответ на инициированный запрос к службе. Этот механизм не используется регулярно, так как задачи управления и мониторинга выполняются с помощью удаленного взаимодействия PowerShell. Этот порт используется только в тех редких случаях, когда нам требуется удаленное подключение к управляемому домену для расширенного устранения неполадок. Порт будет закрыт сразу после устранения неполадок.
