...
## Azure RemoteApp
С помощью Azure RemoteApp администратор Contoso может создавать коллекции, присоединенные к домену. Благодаря этой функции удаленные приложения, обслуживаемые Azure RemoteApp, могут запускаться на компьютерах, присоединенных к домену, а также получать доступ к другим ресурсам с помощью встроенной проверки подлинности Windows. Contoso может использовать доменные службы Azure AD для предоставления управляемого домена, который используется коллекциями Azure RemoteApp, присоединенными к домену.
