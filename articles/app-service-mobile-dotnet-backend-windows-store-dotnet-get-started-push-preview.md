...
1. [Регистрация приложений для получения push-уведомлений](#register)
...
##<a id="register"></a>Регистрация приложений для получения push-уведомлений
...
1. Если вы еще не зарегистрировали приложение, перейдите на страницу <a href="http://go.microsoft.com/fwlink/p/?LinkID=266582" target="_blank">Отправка приложения</a> в центре разработки для приложений Магазина Windows, войдите в учетную запись Майкрософт и щелкните **Имя приложения**.
...
9. На вкладке **Параметры приложения** запишите все значения полей **Секрет клиента** и **Идентификатор безопасности пакета (SID)**. 
...
1. Войдите на [портал предварительной версии Azure], щелкните **Обзор**, **Мобильное приложение**, а затем выберите свое приложение и откройте службы push-уведомлений.

2. В службе уведомлений Windows введите **Секрет клиента**, **Идентификатор безопасности пакета (SID)** и сохраните файл.
...
2. Выполните поиск пакета SDK для клиента мобильного приложения службы приложений для управляемых приложений и щелкните **Установить**, выберите все проекты в решении и примите условия использования. 

    При этом во всех проектах скачивается, устанавливается и добавляется ссылка на библиотеку мобильных push-уведомлений Azure для Windows. 
...
5. В верхней части обработчика событий **OnLaunched** в **App.xaml.cs** добавьте следующий вызов в новый метод **InitNotificationsAsync**:

        InitNotificationsAsync();

    Это обеспечит регистрацию кратковременного ChannelURI при каждом запуске приложения.

6. В обозревателе решений дважды щелкните **Package.appxmanifest** приложения Магазина Windows, в меню **Уведомления** задайте для пункта **Всплывающие уведомления** значение **Да**.
