...
Нет, DocumentDB хранит данные в коллекциях документов JSON.  Для получения информации о ресурсах DocumentDB см. раздел [Модель ресурсов и основные принципы DocumentDB](../documentdb-resources/) . 
...
## <a id="setup"></a> Установка Microsoft Azure DocumentDB

###Как зарегистрироваться для Microsoft Azure DocumentDB?
Служба Microsoft Azure DocumentDB (предварительная версия) доступна на новом [портале управления Azure (предварительная версия)][preview-portal].  Сначала необходимо зарегистрироваться для получения подписки Microsoft Azure.  После регистрации подписки Microsoft Azure в нее можно добавить учетную запись DocumentDB через Marketplace.   

###Что такое главный ключ?
Главный ключ - это токен безопасности для доступа ко всем ресурсам учетной записи. Пользователи с этим ключом имеют доступ на чтение и запись во всех ресурсах в учетной записи базы данных. Поэтому при распространении главных ключей необходимо соблюдать осторожность. Первичный главный ключ и вторничный главный ключ доступны во вкладке "Ключи" на [портале управления Azure (предварительная версия) ][preview-portal].
...
Каждая приобретенная единица мощности предоставляется с выделенным хранилищем базы данных и подготовленной пропускной способностью. Кроме того, предусмотрены квоты для каждого ресурса, управляемого этой службой. См. раздел "Ограничения предварительной версии DocumentDB" в [Подписка Azure и ограничения служб, квоты и ограничения](../azure-subscription-service-limits/) для получения дополнительных сведений.  

###Как настраивать пользователей и разрешения?
Пользователей и разрешения можно создавать с помощью одного из пакетов SDK DocumentDB или с помощью интерфейсов API REST. См. раздел "Разработка" на [странице документации DocumentDB](http://go.microsoft.com/fwlink/p/?LinkID=402319) для получения информации о разработке приложений.  
