...
# Настройка экспортированного шаблона Resource Manager

В этой статье показано, как изменить экспортированный шаблон, чтобы обеспечить передачу дополнительных значений в качестве параметров. Это руководство основывается на шагах, приведенных в статье по [экспорту шаблона Resource Manager](resource-manager-export-template.md), но выполнять их до выполнения шагов этой статьи необязательно. Здесь вы можете найти необходимые шаблоны и скрипты.

## Просмотр экспортированного шаблона

Если вы выполнили шаги руководства по [экспорту шаблона Resource Manager](resource-manager-export-template.md), откройте скачанный шаблон. Он называется **template.json**. Если вы используете Visual Studio или Visual Code, с помощью них вы можете изменить шаблон. Можно также использовать любой редактор JSON или текстовый редактор.

Если вы не выполнили предыдущее пошаговое руководство, можно использовать экспортированный шаблон ниже. Создайте файл с именем **template.json** и добавьте в него содержимое ниже.
...
Шаблон выше отлично подходит, если нужно создавать учетные записи хранения одинакового типа в одном регионе и виртуальной сети, используя один и тот же префикс адреса и префикс подсети при каждом развертывании. Тем не менее Resource Manager обеспечивает более гибкое развертывание, чем этот шаблон. Например, во время развертывания может понадобиться указать тип создаваемой учетной записи хранения или значения, которые следует использовать в качестве префикса адреса и префикса подсети виртуальной сети.

## Настройка шаблона

В этом разделе вам предстоит добавить параметры в экспортированный шаблон, чтобы его можно было использовать снова при развертывании тех же ресурсов в других средах. Кроме того, мы добавим в шаблон некоторые функции, чтобы уменьшить вероятность возникновения ошибки при его развертывании. Для учетной записи хранения больше не нужно подбирать уникальное имя. Шаблон сделает это за вас. Вы ограничите значения, которые можно указывать для типа учетной записи хранения, только до допустимых вариантов.

1. Чтобы можно было передавать значения, которые может понадобиться указать во время развертывания, введите в разделе **parameters** указанные ниже определения. Обратите внимание на разрешенные значения для **storageAccount\_accountType**. Если случайно указать недопустимое значение, ошибка будет распознана до начала развертывания. Кроме того, обратите внимание, что для имени учетной записи указывается только префикс, длина которого ограничена 11 знаками. Такое ограничение префикса гарантирует, что полное имя не будет превышать максимальное количество знаков для имени учетной записи хранения. Префикс позволяет применять соглашение об именовании к учетным записям хранения. Сведения о создании уникального имени описаны на следующем шаге.
...
2. Раздел **variables** шаблона сейчас пуст. Введите в нем код, указанный ниже. В разделе **variables** вы как создатель шаблона можете создавать значения, чтобы упростить синтаксис остальной части. Переменная **storageAccount\_name** объединяет префикс параметра с уникальной строкой, созданной на основе идентификатора группы ресурсов.

        "variables": {
          "storageAccount_name": "[concat(parameters('storageAccount_prefix'), uniqueString(resourceGroup().id))]"
        },

3. Чтобы использовать раздел параметров и переменную в определениях ресурсов, укажите в разделе **resources** приведенные ниже определения. Обратите внимание, что в действительности, кроме значений, назначенных свойствам, в определениях ресурсов мало что изменилось. Эти свойства точно соответствуют свойствам экспортированного шаблона. Вы просто назначаете свойства значениям параметров, вместо того чтобы жестко задавать значения. С помощью выражения **resourceGroup().location** для расположения ресурсов установлено использование того же расположения, что и для группы ресурсов. Ссылка на переменную, созданную для имени учетной записи хранения, указывается с помощью выражения **variables**.
...
Файл параметров выше указывает значения только для тех параметров, для которых не заданы значения по умолчанию. Вы можете указать значения для других параметров, если для них нужно использовать не значения по умолчанию.

## Развертывание шаблона

Чтобы развернуть настроенный шаблон и файл параметров, можно использовать Azure PowerShell или интерфейс командной строки Azure. При необходимости установите [Azure PowerShell](powershell-install-configure.md) или [интерфейс командной строки Azure](xplat-cli-install.md). Вы можете использовать скрипты, скачанные с шаблоном во время экспорта исходного шаблона, или написать свой скрипт для развертывания шаблона. В этом разделе описаны оба метода.

2. Чтобы использовать для развертывания свой скрипт, примените один из следующих методов.
...
- [Пошаговое руководство по созданию шаблона Resource Manager](resource-manager-template-walkthrough.md) основано на действиях, выполненных в этом разделе, и предусматривает создание шаблона для более сложного решения. Оно позволяет узнать больше о доступных ресурсах и о том, как определить, какие значения следует указывать.
