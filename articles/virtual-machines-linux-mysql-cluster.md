...
Наконец, в основной виртуальной машине (`hadb01`) установим принудительное владение этим ресурсом DRBD:

    sudo drbdadm primary --force r0

Если проверить содержимое /proc/drbd (`sudo cat /proc/drbd` в обеих виртуальных машинах, там должно быть указано `Primary/Secondary` в `hadb01` and `Secondary/Primary` в `hadb02` в соответствии с решением на этот момент. Диск в 5 ГБ будет синхронизирован в сети 10.10.10.0/24 бесплатно.
...
Второй вариант заключается в том, чтобы выполнить отработку отказа на `hadb02`, а затем установить там mysql-server (сценарии установки заметят существующую установку и не будут ее затрагивать).
...
Если в текущий момент переход на другой ресурс не планируется, то первый вариант более простой, хотя, возможно, менее красивый. После выполнения этой установки можно начинать работу с базой данных MySQL. На `hadb02` (либо том сервере, который активен по данным DRBD):
...
Вернемся на портал Azure и перейдем к виртуальной машине `hadb01`, а затем в раздел конечных точек. Создадим новую конечную точку, выберем в раскрывающемся меню пункт MySQL (TCP 3306) и установим флажок *Create new load balanced set*. Назовем нашу конечную точку с балансировкой нагрузки `lb-mysql`. Не будем трогать большинство параметров, уменьшим только время до минимального значения 5 (секунд).
