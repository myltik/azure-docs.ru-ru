<properties 
	...
	description="Описано расширение Docker и контейнеры, расширения виртуальных машин Azure, а также приводятся ссылки на дополнительные ресурсы по созданию контейнеров Docker в xplat-cli и на портале." 
...
+ [Как использовать расширение Docker для виртуальных машин из кроссплатформенного интерфейса Azure (xplat-cli)]
...
## <a id='Docker and Linux Containers'>Расширение Docker и контейнеры Linux</a>
...
Docker и контейнеры Linux не являются [низкоуровневыми оболочками](http://en.wikipedia.org/wiki/Hypervisor), такими как Windows Hyper-V и [KVM](http://www.linux-kvm.org/page/Main_Page) в Linux (существует множество других примеров). Гипервизоры виртуализуют базовую операционную систему для того, чтобы полноценные операционные системы могли выполняться внутри гипервизора так, как будто они являются приложением. 
...
|   Функция      | Низкоуровневые оболочки  | Контейнеры  |
| :------------- |-------------| ----------- |
|Изоляция процессов | Более-менее полная | В случае получения прав root целостность узла контейнера может быть нарушена|
| Память на диске | Вся ОС и приложения | Только требования к приложениям |
...
| Автоматизация контейнеров | Сильно зависит от ОС и приложений | [Коллекция образов Docker](https://registry.hub.docker.com/), другое 
...
## <a id='Virtual Machine Extensions for Linux and Windows'>Расширения виртуальных машин для Linux и Windows</a>
Расширение виртуальных машин Docker для Azure - это лишь одно из нескольких расширений виртуальных машин, которые обеспечивают особое поведение, а еще больше таких расширений находятся на стадии разработки. Например, некоторые возможности [расширения агента ВМ Linux](http://azure.microsoft.com/documentation/articles/virtual-machines-linux-agent-user-guide/) позволяют изменять виртуальную машину и управлять ей, включая компоненты обеспечения безопасности, ядро, сетевые компоненты и т. д. Например, расширение VMAccess позволяет сбросить пароль администратора или ключ SSH.

Полный список приведен в разделе [Расширения виртуальных машин Azure](http://msdn.microsoft.com/library/azure/dn606311.aspx).

<!--Anchors-->
[Как использовать расширение Docker для виртуальных машин из кроссплатформенного интерфейса Azure (xplat-cli)]: http://azure.microsoft.com/documentation/articles/virtual-machines-docker-with-xplat-cli/
...
[Как быстро приступить к работе с Docker в Azure Marketplace]: http://azure.microsoft.com/documentation/articles/virtual-machines-docker-ubuntu-quickstart/
