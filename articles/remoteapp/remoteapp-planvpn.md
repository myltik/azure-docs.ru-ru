...
В этом документе описывается настройка виртуальной сети (VNET) и подсети Azure для Azure RemoteApp. Виртуальные сети Azure позволяют виртуализовать сетевую инфраструктуру в облаке и создавать гибридные решения, используя Azure и локальные ресурсы. Подробнее об этом можно прочитать [здесь](virtual-networks-overview.md).

Если вы хотите определить политики безопасности для трафика (как входящего, так и исходящего) в виртуальной сети, в которой выполняется развертывание Azure RemoteApp, настоятельно рекомендуем создать для Azure RemoteApp отдельную подсеть. Дополнительные сведения об определении политик безопасности для подсети виртуальной сети Azure см. в статье [Группа безопасности сети](virtual-networks-nsg.md).
