<properties 
	pageTitle="Создание облачной коллекции Azure RemoteApp"
...
- Облачные: располагаются полностью в Azure. Можно сохранить все данные в облаке (для коллекции только для облака) или подключить коллекцию к виртуальной сети и сохранить данные в ней.   
...
2.	При необходимости настройка синхронизации каталогов. Если вы используете Azure AD и Active Directory, придется синхронизировать пользователей, контакты и пароли между локальной Active Directory и клиентом Azure AD.
5.	Публикация приложений RemoteApp.
6.	Настройка доступа пользователей.

**Примечание** *Данный раздел находится в процессе переработки. Я обновил шаги для пользовательского интерфейса, но еще не смог повторно опубликовать весь раздел. Я работаю над несколькими новыми статьями, которые позволят гораздо проще определить доступные вам варианты проверки подлинности и варианты коллекций. Таким образом, если вы запутались, помните, что я об этом знаю и работаю над тем, чтобы предоставить подробные сведения в кратчайшие сроки. Спасибо.*
...
- Хотите использовать образ Office 365 профессиональный плюс? Ознакомьтесь со сведениями, представленными [здесь](remoteapp-officesubscription.md).
...
- Определите, нужно ли вам подключаться к виртуальной сети. Если вы будете подключаться к виртуальной сети, убедитесь, что она удовлетворяет требованиям к размеру и может подключиться к RemoteApp.
...
1. На портале управления перейдите на страницу RemoteApp.
2. Нажмите **Создать** > **Создать с виртуальной сетью**.
...
Если планируется использовать Active Directory, то RemoteApp требуется синхронизация каталогов между Azure Active Directory и локальными службами Active Directory для синхронизации пользователей, контактов и паролей с клиентом Azure Active Directory. Сведения о планировании см. в [статье о настройке Active Directory для Azure RemoteApp](remoteapp-ad.md). Для получения информации также можно перейти напрямую к [AD Connect](http://blogs.technet.com/b/ad/archive/2014/08/04/connecting-ad-and-azure-ad-only-4-clicks-with-azure-ad-connect.aspx).
