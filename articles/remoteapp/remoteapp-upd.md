...
>[AZURE.NOTE] Отключение UPD Узнайте, как это сделать, из сообщения [Отключение дисков профилей пользователей (UPD) в Azure RemoteApp](https://blogs.technet.microsoft.com/enterprisemobility/2015/11/11/disable-user-profile-disks-upds-in-azure-remoteapp/) в блоге Pavithra.
...
- Вам не требуется локальное хранилище данных либо все ваши данные хранятся в облаке (например, в OneDrive для бизнеса) или в общей папке, и вы хотели бы локально управлять сохранением данных с помощью Azure RemoteApp.

Дополнительные сведения см. в разделе [Отключение дисков профилей пользователей (UPD) в Azure RemoteApp](https://blogs.technet.microsoft.com/enterprisemobility/2015/11/11/disable-user-profile-disks-upds-in-azure-remoteapp/).
...
## Можно ли хранить данные на виртуальной машине локально?

Нет, данные, хранящиеся на виртуальной машине в любом месте, отличном от UPD, будут потеряны. Существует большая вероятность, что при следующем входе в Azure RemoteApp пользователь не получит ту же самую виртуальную машину. Мы не поддерживаем сохранение виртуальной машины пользователя, поэтому пользователь не будут входить в ту же виртуальную машину, и данные будут потеряны. Кроме того, при обновлении коллекции существующие виртуальные машины заменяются новыми; это означает, что все данные, хранящиеся на самой виртуальной машине, теряются. Рекомендуется хранить данные на диске UPD, в общем хранилище, таком как служба файлов Azure, на файловом сервере в виртуальной сети или в облаке, используя OneDrive для бизнеса или другую поддерживаемую облачную систему хранения, такую как DropBox.

## Как подключить общую папку Azure на виртуальной машине с помощью PowerShell?
