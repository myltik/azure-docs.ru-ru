...
<div class="dev-callout"><strong>Примечание.</strong><p>Доступна <a href="/en-us/develop/net/tutorials/web-site-with-sql-database/">новая версия этого учебника</a>. Этой версией можно продолжать пользоваться, если планируется использовать Visual Studio 2012, но работать с новой версией намного проще.</p></div>
...
1.  Чтобы установить пакет Azure SDK для .NET, щелкните приведенную ниже ссылку. Если среда Visual Studio 2012 еще не установлена, ее можно установить по ссылке. Для этого учебника требуется Visual Studio 2012.
    [Пакет Azure SDK для Visual Studio 2012][]
...
![Установщик веб-платформы — Azure SDK для .NET][]
...
1.  В браузере откройте [управления Azure][].

2.  На вкладке **Веб-сайты** щелкните имя ранее созданного сайта.

    ![Приложение диспетчера контактов на вкладке веб-сайтов портала управления][]

3.  В правой части окна щелкните **Загрузить профиль публикации**.

    ![Вкладка быстрого запуска и кнопка "Загрузить профиль публикации"][]
...
8.  В диалоговом окне **Импорт профиля публикации** выберите **Импорт с веб-сайта Windows Azure**, выберите свой веб-сайт в раскрывающемся списке, затем нажмите кнопку **ОК**.
    ...
    ![Главная страница списка дел на ресурсе Azure][]
...
8.  В диалоговом окне переопределения **Добавить контроллер** убедитесь, что все параметры выбраны и нажмите кнопку **ОК**.
    ...
    ![Окно сообщения «Добавить контроллер» ][]
...
1.  В меню **Сервис** выберите **Диспетчер пакетов библиотеки**, а затем **Консоль диспетчера пакетов**.
    ...
    !["Консоль диспетчера пакетов" в меню "Сервис"][]
...
2.  В окне **Консоль диспетчера пакетов** введите следующую команду:
    ...
    ![enable-migrations][]
    Необходимо задать имя типа контекста (**ContactManagerContext**), так как проект содержит два производных класса [DbContext][]: один, который только что добавлен **ContactManagerContext**, и **UsersContext**, который используется для базы данных членства. Класс **ContactManagerContext** был добавлен в мастер формирования шаблонов Visual Studio.
...
6.  Замените метод *Seed* следующим кодом:
    ...
    Приведенный выше код инициализирует базу данных с контактной информацией. Дополнительные сведения об инициализации базы данных см. в разделе [Инициализация и отладка баз данных Entity Framework (EF)][].
...
Этот учебник и пример приложения были написаны [Риком Андерсоном (Rick Anderson)][] (Твиттер [@RickAndMSFT][]) в соавторстве с Томом Дейкстрой (Tom Dykstra), Томом ФитцМакеном (FitzMacken) и Бэрри Доррансом (Barry Dorrans) (Твиттер [@blowdart][]).
...
Чтобы создать цветные кнопки входа в Facebook, Google и Yahoo, воспользуйтесь блогом [Настройка кнопок внешнего входа в ASP.NET MVC 4][]. Дополнительные сведения об использовании проверки подлинности Windows см. в следующих статьях:
...
-   [Многоуровневое приложение .NET, использующее таблицы, очереди и BLOB-объекты хранилища][].
-   [Введение в ASP.NET MVC 4][]
-   [Общие сведения о работе с Entity Framework с использованием MVC][]
-   [OAuth 2.0 и вход][]

Теперь вы знаете, как развертывать веб-приложение на веб-сайте Azure. Дополнительные сведения о настройке, масштабировании веб-сайтов Windows Azure и управлении ими см. в разделах практических руководств на странице [Типичные задачи][].

Сведения об отладке веб-сайтов Azure см. в разделе [Устранение неполадок веб-сайтов Azure в Visual Studio][].

Сведения о развертывании приложения в облачной службе Azure см. в [версии этого учебника для облачной службы][] и в статье [Разработка веб-приложений с использованием Azure][]. Выбор для запуска веб-приложения ASP.NET облачной службы Azure вместо веб-сайта Azure может объясняться следующими причинами:
...
Дополнительные сведения о базе данных SQL и хранилище Windows Azure см. в статье [Предлагаемые варианты хранения данных для платформы Azure][].

Дополнительные сведения об использовании базы данных SQL см. в статье [Работа с базой данных SQL Azure в карте содержимого доступа к данным ASP.NET][].
...
<!-- images-->
  ...
  [новая версия этого учебника]: /en-us/develop/net/tutorials/web-site-with-sql-database/
  ...
  [Центра загрузки Microsoft SQL Server 2012 Express]: http://www.microsoft.com/en-us/download/details.aspx?id=29062
  ...
  [Создание сайта интрасети с помощью ASP.NET MVC]: http://msdn.microsoft.com/en-us/library/gg703322(v=vs.98).aspx
  [Многоуровневое приложение .NET, использующее таблицы, очереди и BLOB-объекты хранилища]: http://www.windowsazure.com/en-us/develop/net/tutorials/multi-tier-web-site/1-overview/
  ...
  [Типичные задачи]: http://www.windowsazure.com/en-us/develop/net/common-tasks/
  ...
  [версии этого учебника для облачной службы]: http://www.windowsazure.com/en-us/develop/net/tutorials/cloud-service-with-sql-database/
  [Разработка веб-приложений с использованием Azure]: http://msdn.microsoft.com/en-us/library/Hh674484
  ...
  [11]: http://msdn.microsoft.com/en-us/library/hh770484
