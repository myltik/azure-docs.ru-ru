<properties 
	pageTitle="Как использовать диагностику (.NET) - руководство по компонентам Azure" 
...
<tbody>
	...
	<tr>
		...
		<td>События, созданные вашим кодом с использованием класса <a href="http://msdn.microsoft.com/library/system.diagnostics.tracing.eventsource(v=vs.110).aspx">EventSource .NET</a>.</td>            
	</tr>
	<tr>
...
3.	Выберите пункт **Worker Role**.
4.	В **обозревателе решений** дважды щелкните файл свойств **WorkerRole1**.
5.	На вкладке **Настройка** снимите флажок **Включить диагностику**, чтобы отключить систему диагностики 1.0 (для пакета SDK для Azure версии 2.4 и более поздней).
...
В **обозревателе сервера** Visual Studio перейдите к учетной записи хранения wadexample. После того, как облачная служба проработаустановить и настроить Azure PowerShell версии 0.8.7 или более позднейла около пяти минут, следует посмотреть таблицы **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** и **WADSetOtherTable**. Дважды щелкните одну из таблиц, чтобы просмотреть собранную телеметрию.
...
2.	В **обозревателе сервера** разверните **Azure**, щелкните правой кнопкой мыши **Виртуальные машины** и выберите **Создать виртуальную машину**.
3.	Выберите подписку Azure в диалоговом окне **Выберите подписку** и щелкните **Далее**.
4.	Выберите **Windows Server 2012 R2 Datacenter, November 2014** в диалоговом окне **Выбор образа виртуальной машины** и щелкните **Далее**.
...
6.	В диалоговом окне **Параметры облачной службы** создайте новую облачную службу с именем "wadexample". Создайте новую учетную запись хранения с именем "wadexample" и щелкните **Далее**.
7.	Щелкните **Создать**.
...
1.	Щелкните правой кнопкой мыши **WadExampleVM** в **обозревателе решений** и выберите **Открыть папку**.
...
<h3>Шаг 6. Просмотр данных телеметрии</h3>
В **обозревателе сервера** Visual Studio перейдите к учетной записи хранения wadexample. После того, как виртуальная машина проработала около пяти минут, следует посмотреть таблицы **WADEnumsTable**, **WADHighFreqTable**, **WADMessageTable**, **WADPerformanceCountersTable** и **WADSetOtherTable**. Дважды щелкните одну из таблиц, чтобы просмотреть собранную телеметрию.
...
Файл конфигурации системы диагностики определяет значения, которые используются для инициализации параметров конфигурации диагностики, когда запускается монитор диагностики. Пример файла конфигурации и подробная документация его схемы расположена здесь: [Схема конфигурации системы диагностики Azure 1.2][].
...
**В.** Если у меня уже установлено расширение системы диагностики 1.1 в моей роли или виртуальной машине, как мне обновиться до системы диагностики 1.2 или 1.3?
...
<h2><a name="comparing"></a>Сравнение версий систем диагностики Azure</h2>
...
<tbody>
	<tr>
			...
			<td><strong>Система диагностики 1.1, 1.2 или 1.3</strong></td>
	</tr>
...
[Настройка диагностики для облачных служб и виртуальных машин Azure]: http://msdn.microsoft.com/library/windowsazure/dn186185.aspx
