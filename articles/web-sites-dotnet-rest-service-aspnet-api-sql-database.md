...
***Авторы: [Рик Андерсон (Rick Anderson)](https://twitter.com/RickAndMSFT) и Том Дикстра (Tom Dykstra). Последнее обновление: март 2014 г.***

В данном учебнике описывается развертывание веб-приложения ASP.NET на веб-сайте Azure с использованием мастера веб-публикации из Visual Studio 2013 или Visual Studio 2013 для Web Express. (Если вы предпочитаете работать в Visual Studio 2012, см. [предыдущую версию этого учебника](/ru-ru/develop/net/tutorials/get-started-vs2012/).)
...
В данном учебнике предполагается, что у читателя нет опыта использования платформы Azure. По завершении этого учебника будет сделано простое веб-приложение, работающее в облаке.
...
Вы создадите простое веб-приложение со списком контактов на платформе ASP.NET MVC 5 и использующее ADO.NET Entity Framework для доступа к базе данных. На иллюстрации ниже показано готовое приложение:
...
* [Добавление контроллера и представления данных][addcontroller]
* [Добавление Web API Restful интерфейса][addwebapi]
...
Ваш веб-сайт Azure работает в среде общего хостинга, что означает, выполнение на виртуальных машинах, которые также используют другие клиенты Azure.  Общая среда внешнего размещения - это недорогой способ начать работу в облачной среде. Если позднее ваш веб-трафик увеличится, приложение можно масштабировать по необходимости, запуская его на выделенных виртуальных машинах. Если требуется более сложная архитектура, можно перейти в облачную службу Azure. Облачные службы работают на выделенных виртуальных машинах, которые можно настроить в соответствии с конкретными потребностями.

SQL Database - это служба реляционных баз данных, размещенная в облаке и созданная на базе технологий SQL Server. Средства и приложения, работающие с SQL Server, работают также и с базой данных SQL.

1. На [Портале управления Azure](https://manage.windowsazure.com), щелкните **Веб-сайты** в левой вкладке, затем щелкните **Создать**.

2. Щелкните элемент **НАСТРАИВАЕМОЕ СОЗДАНИЕ**.
...
3. В окне мастера **Новый веб-сайт** введите строку в поле **URL**, который будет использоваться в качестве уникального URL-адреса для вашего приложения. Полный URL-адрес будет состоять из введенной здесь строки и суффикса, который отображается под текстовым полем. На данной иллюстрации это "contactmgr11", но этот URL, возможно, уже занят, так что вам придется выбрать другой.

1. В раскрывающемся списке **Регион** выберите ближайший к вам регион.

1. В раскрывающемся списке **База данных** выберите **Создать бесплатную базу данных SQL емкостью 20 Мб**.
...
6. Щелкните стрелку, указывающую вправо, в нижней части окна.
	...
	Мастер перейдет к шагу **Настройка базы данных**.

7. В поле **Имя** введите *ContactDB*.

8. В поле **Сервер** выберите **Создать сервер базы данных SQL**. Если вы уже создали ранее базу данных SQL Server, можно выбрать этот сервер в раскрывающемся списке.

9. Щелкните стрелку, указывающую вправо, в нижней части окна.

10. Введите **ИМЯ ДЛЯ ВХОДА В СИСТЕМУ** и **ПАРОЛЬ** администратора. Если выбрано действие **Создать сервер базы данных SQL**, не используйте здесь существующие имя и пароль, введите новые, которые в дальнейшем будут использоваться при обращении к базе данных. Если выбран созданный ранее SQL Server, появится запрос на ввод пароля для ранее созданной учетной записи SQL Server. В этом учебнике поле **Дополнительно** не отмечается. В поле **Дополнительно ** можно задать размер базы данных (по умолчанию он равен 1 Гб, но может быть увеличен до 150 Гб) и сортировку.
...
11. Щелкните флажок снизу от поля, чтобы указать завершение работы.
	...
	Портал управления вернется к странице "Веб-сайты", и в столбце **Состояние** будет показано, что идет создание сайта. Через какое-то время (обычно меньше минуты) в столбце **Состояние** указывается, что сайт успешно создан. Слева на панели навигации рядом со значком **Веб-сайты** появилось количество сайтов, имеющихся в вашей учетной записи, а рядом со значком **Базы данных SQL** - количество баз данных.
...
Вы создали веб-сайт Azure, но в нем еще нет контента. Следующий шаг - создание проекта веб-приложения в Visual Studio, который будет опубликован на Azure.

### Создание проекта

1. Откройте Visual Studio 2013.
1. В меню **Файл** выберите **Новый проект**.
3. В диалоговом окне **Новый проект** разверните пункт **Visual C#** и выберите **Веб**, а затем - **Веб-приложение ASP.NET MVC 5**. Оставьте значение по умолчанию **.NET Framework 4.5**. Присвойте приложению имя **ContactManager** и нажмите кнопку **ОК**.
	![New Project dialog box](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr4.PNG)]
1. В диалоговом окне **Новый проект ASP.NET** выберите шаблон **MVC**, поставьте флажок для **Веб-API** и щелкните **Изменение проверки подлинности**.

	![New ASP.NET Project dialog box](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rt3.PNG)

1. В диалоговом окне **Изменение проверки подлинности** щелкните **Без проверки**, а затем нажмите кнопку **ОК**.

	![No Authentication](./media/web-sites-dotnet-get-started-vs2013/GS13noauth.png)

	Приложение, которое вы создаете, не имеет функций, требующих от пользователей введения учетных данных. Для получения сведений об использовании проверки подлинности и функциях авторизации см. раздел [Дальнейшие шаги](#nextsteps) в конце этого учебника. 
...
Разметка, приведенная выше, изменит имя приложения с "My ASP.NET App" на "Contact Manager" и удалит ссылки на **Домашнюю страницу**, **О приложении** и **Контакты**.
...
1. В Visual Studio щелкните правой кнопкой мыши проект в **Обозревателе решений** и из контекстного меню выберите **Опубликовать**.
	...
	Откроется мастер **Веб-публикация**.

2. Во вкладке **Профиль** мастера **Веб-публикация** щелкните **Импортировать**.

	![Import publish settings][ImportPublishSettings]

	Откроется диалоговое окно **Импорт профиля публикации**.

 3.	Выберите импорт из веб-сайта Azure. Если вы еще не выполнили вход в систему, сделайте это. Щелкните **Войти**. Введите имя пользователя, ассоциированного с этой подпиской, и следуйте дальнейшим инструкциям, чтобы выполнить вход.

	![sign in](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr7.png)

	В раскрывающемся списке выберите ваш веб-сайт и нажмите кнопку **ОК**.

	![Import Publish Profile](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rr8.png)

8. На вкладке **Подключение** щелкните **Проверка подключения**, чтобы убедиться в правильности настроек.

	![Validate connection][ValidateConnection]

9. Когда подключение будет проверено, рядом с кнопкой **Проверка подключения** появится зеленая отметка.
...
1. Нажмите кнопку **Далее**.
	...
	На этой вкладке вы можете принять настройки по умолчанию.  Вы развертываете конфигурацию сборки для выпуска, и вам не требуется удалять файлы на целевом сервере, выполнять предварительную компиляцию приложения или исключать файлы из папки App_Data. Чтобы выполнить отладку активного сайта Azure, вам необходимо развернуть конфигурацию отладки (не выпуск). См. раздел [Дальнейшие шаги](#nextsteps) в конце этого учебника.

12. На вкладке **Предварительный просмотр** нажмите кнопку **Начать предварительный просмотр**.
...
12. Щелкните **Опубликовать**.
	...
	Visual Studio начнет процесс копирования файлов в Azure. В окне **Результаты** показаны выполненные действия по развертыванию и отчет об успешном завершении.
...
1. В **вбозревателе решений** щелкните правой кнопкой мыши папку Models, выберите **Добавить**, а затем щелкните **Класс**.
...
Класс **Contacts** определяет данные, которые будут храниться для каждого контакта, а также первичный ключ ContactID, который необходим для базы данных. Для получения дополнительной информации о моделях данных, см. раздел [Дальнейшие шаги](#nextsteps) в конце этого учебника.
...
1. В **Обозревателе решений** разверните папку "Controllers".

3. Постройте проект **(Ctrl+Shift+B)**. Проект необходимо создать перед использованием механизма формирования шаблонов. 

4. Щелкните правой кнопкой мыши по папке "Controllers", затем нажмите **Добавить** и **Контроллер**.

	![Add Controller in Controllers folder context menu][addcode001]

1. В диалоговом окне **Добавить элемент скаффолдинга** выберите **Контроллер MVC с представлениями, Entity Framework** и щелкните **Добавить**.
...
6. Укажите имя контроллера **HomeController**. Выберите класс модели **Contact**. Нажмите кнопку **Новый контекст данных** и оставьте значение "ContactManager.Models.ContactManagerContext", предлагаемое по умолчанию в разделе **Новый тип контекста данных**. Щелкните **Добавить**.
	...
	Появится диалоговое окно со следующим текстом: "Файл с именем HomeController уже существует. Вы хотите заменить его?". Щелкните **Да**. Контроллер Home, созданный для нового проекта, будет перезаписан. Новый контроллер будет использоваться для списка контактов.

	Visual Studio создаст методы контроллера и представления для CRUD-операций базы данных объектов класса **Contact**.
...
1. В меню **Средства** выберите **Диспетчер пакетов библиотек**, а затем **Консоль диспетчера пакетов**.
...
2. В окне **Консоль диспетчера пакетов** введите следующую команду:
	...
	Команда **enable-migrations** создает папку *Migrations* и размещает в ней файл *Configuration.cs*, который можно редактировать для настройки миграции. 

2. В окне **Консоль диспетчера пакетов** введите следующую команду:

		add-migration Initial

	Команда **add-migration Initial** формирует класс под названием **&lt;date_stamp&gt;Initial**, который создает базу данных. Первый параметр (*Initial*) является необязательным и используется для указания имени файла. Новые файлы классов можно просмотреть в **Обозревателе решений**.

	В классе **Initial** метод **Up** создает таблицу контактов, а метод **Down** (используется, когда нужно вернуться к предыдущему состоянию) удаляет ее.
...
5. Замените метод *Seed* следующим кодом.
	...
	Код, приведенный выше, инициализирует базу данных контактной информацией. Для получения дополнительной информации о заполнении базы данных, см. [Отладка баз данных на платформе Entity Framework (EF)](http://blogs.msdn.com/b/rickandy/archive/2013/02/12/seeding-and-debugging-entity-framework-ef-dbs.aspx).
...
1. Откройте файл *Views\Home\Index.cshtml*. На следующем шаге мы заменим созданную разметку кодом, в котором используются [jQuery](http://jquery.com/) и [Knockout.js](http://knockoutjs.com/). Новый код извлекает список контактов с помощью веб-API и JSON, а затем привязывает данные контактов к пользовательскому интерфейсу с использованием knockout.js. Дополнительную информацию см. в разделе [Дальнейшие действия](#nextsteps) в конце этого учебника. 
...
3. Щелкните правой кнопкой мыши по папке "Content", затем выберите команду **Добавить** и **Новый элемент...**.

	![Add style sheet in Content folder context menu][addcode005]

4. В диалоговом окне **Добавить новый элемент** введите **Стиль** в верхнем правом поле поиска, затем выберите **Список стилей**.
	![Add New Item dialog box][rxStyle]

5. Назовите файл *Contacts.css* и щелкните **Добавить**. Замените содержимое файла на код, приведенный ниже.
...
8. Измените содержимое или запись css, чтобы зарегистрировать список стилей *contacts.css*. Измените следующую строку:
...
1. В диалоговом окне **Добавить элемент скаффолдинга** выберите **Контроллер Web API 2 с действиями, Entity Framework** и щелкните **Добавить**.
...
3. В браузере добавьте **/api/contacts** к URL-адресу.
...
1. В **Обозревателе решений**, щелкните правой кнопкой мыши проект **ContactManager** и нажмите **Добавить**, а затем **Класс**.

2. Откройте файл *ValidateHttpAntiForgeryTokenAttribute.cs* и добавьте следующий код:
...
1. Добавьте атрибут **[ValidateHttpAntiForgeryToken]** в методы Post объекта **ContactsController**, чтобы защитить его от с подделкой межсайтовых запросов (XSRF). Вы добавите его к методам PutContact, PostContact и **DeleteContact**.

	[ValidateHttpAntiForgeryToken]
        public IHttpActionResult PutContact(int id, Contact contact)
        {

1. Обновите раздел *Scripts* файла *Views\Home\Index.cshtml*, чтобы включить код для получения токенов XSRF.
...
1. В **Обозревателе решений** щелкните правой кнопкой мыши проект и выберите команду **Опубликовать**.
...
1. В **ContactsManagerContext(ContactsManagerContext)**, щелкните значок **v**, чтобы изменить элемент *Строка удаленного подключения* на строку для базы данных контактов. Щелкните **ContactDB**.

	![Settings](./media/web-sites-dotnet-rest-service-aspnet-api-sql-database/rt5.png)

7. Поставьте флажок **Выполнить миграции Code First (при запуске приложения)**.

1. Щелкните **Далее**, затем **Предварительный просмотр**. В Visual Studio отобразится список файлов, которые будут добавлены или обновлены.
...
После завершения развертывания в браузере открывается главная страница приложения.
	...
	Процесс публикации Visual Studio автоматически настраивает в развернутом файле *Web.config* строку подключения на базу данных SQL. Он также настраивает миграции Code First, чтобы автоматически обновлять базу данных до последней версии, как только приложение получит доступ к базе данных после развертывания.

	В результате такой настройки Сode First создает базу данных, запустив код класса **Initial**, который вы создали ранее. Это происходит, как только после развертывания приложение в первый раз пытается получить доступ к базе данных.
...
Для настоящего приложения потребуется проверка подлинности и авторизация, а также использование членства базы данных. Учебник [Развертывание безопасного приложения ASP.NET MVC с членством, OAuth и базой данных SQL](http://www.windowsazure.com/ru-ru/develop/net/tutorials/web-site-with-sql-database/) основан на данном учебнике и демонстрирует развертывание веб-приложения в базе данных с членством.

Еще один способ хранения данных в приложении Azure - хранилище Azure, обеспечивающее нереляционное хранение данных в форме Blob-объектов и таблиц. Дополнительная информация о Web API, ASP.NET MVC и Window Azure содержится по ссылкам ниже.
