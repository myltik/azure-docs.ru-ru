...
-   [Создание веб-приложения ASP.NET в Visual Studio][]
...
![Схема демонстрирует шаги по созданию проекта и его развертыванию][]
...
6.  В диалоговом окне **Новый проект ASP.NET** выберите шаблон **MVC**. Если вы предпочитаете работать с веб-формами ASP.NET, можно выбрать шаблон **Веб-формы**.
    ...
    [MVC и Web Forms][] — это платформы ASP.NET для создания веб-сайтов. В этом учебнике вы можете выбрать любую из них. Однако при выборе веб-форм далее придется изменить *Default.aspx*, когда в учебнике даются указания на изменение *Index.cshtml*.
...
8.  В диалоговом окне **Изменить аутентификацию** щелкните **Без аутентификации** и нажмите кнопку **ОК**.
    ...
    Создаваемое примерное приложение не дает пользователям возможности выполнить вход. В разделе [Дальнейшие действия][] представлены ссылки на учебник, в котором реализованы проверка подлинности и авторизация.
...
13. В раскрывающемся списке **Регион** выберите ближайшее к вам расположение.
    ...
    Этот параметр определяет, в каком центре обработки данных Azure будет работать ваш веб-сайт. При работе с учебником вы можете выбрать любой регион, и это не будет оказывать существенного влияния. Однако для производственного сайта необходимо, чтобы веб-сервер располагался максимально близко к браузерам, которые выполняют доступ к вашему сайту, чтобы минимизировать [задержку][].

14. Оставьте поле базы данных без изменений.

    В этом учебнике база данных не используется. В разделе [Дальнейшие действия][] в конце данного учебника приводится ссылка на учебник, в котором показано, как использовать базу данных.
...
4.  На вкладке **Параметры** нажмите кнопку **Далее**.
    ...
    Раскрывающийся список **Конфигурация** позволяет развертывать отладочное построение для удаленной отладки. В разделе [Дальнейшие действия][] приводятся ссылки на учебник, в котором описывается удаленный запуск Visual Studio в режиме отладки.
...
1.  Откройте файл *Views/Home/Index.cshtml* или *.vbhtml* в **Обозревателе решений**, измените заголовок **h1** с ASP.NET на ASP.NET and Azure и сохраните файл.
    ...
    ![index.cshtml для MVC][]

    ![Изменение h1 для MVC][]
...
![Панель инструментов веб-публикации одним щелчком][]

## Мониторинг веб-сайта и его управление с помощью портала управления

[Портал управления Azure][] — это веб-интерфейс, который позволяет отслеживать службы Azure, например только что созданный вами веб-сайт, и управлять ими. В этом разделе мы познакомимся с тем, что можно сделать в портале.
...
2.  Щелкните имя веб-сайта.
    ...
    ![Вызывается главная страница портала с новым веб-сайтом][]
...
3.  Щелкните вкладку **Панель мониторинга**.
    ...
    ![Вкладка панели мониторинга веб-сайта портала][]
...
4.  Выберите вкладку **Настройка**.
    ...
    Вкладка [Настройка][] позволяет управлять версией .NET, используемой для сайта, включать такие функции, как [WebSockets][] и [ведение журнала диагностики][], устанавливать [значения строки подключения][] и не только.

    ![Вкладка настройки веб-сайта портала][]

5.  Щелкните вкладку **Масштаб**.

    На оплачиваемых уровнях службы веб-сайта вкладка [Масштаб][] позволяет управлять размером и количеством машин, которые обслуживают ваше веб-приложение для того, чтобы обрабатывать изменения в трафике.

    Масштабировать можно вручную, а также настраивать критерии или расписания для автоматического масштабирования.

    ![Вкладка Масштаб веб-сайта портала][]
...
**Подсказка.** в настоящее время существует предварительная версия портала управления, которая заменит используемую вами сейчас версию. Для получения дополнительной информации см. раздел [Предварительная версия портала Azure][].
...
-   Другие способы развертывания веб-проекта
    ...
    В этом учебнике был показан самый быстрый способ создания сайта и его развертывания в одной операции. Обзор других способов развертывания с помощью Visual Studio или [автоматизации развертывания][] из [системы управления версиями][] см. в разделе [Развертывание веб-сайта Azure][].

    Visual Studio может также создавать скрипты Windows PowerShell, которые позволяют автоматизировать развертывание. Дополнительные сведения см. в разделе [Полная автоматизация (создание реальных облачных приложений в Azure)][].

-   Управление веб-сайтом в Visual Studio

    Дополнительные сведения о функциях управления сайтом, которые можно использовать в **Обозревателе сервера**, см. в разделе [Устранение неполадок веб-сайтов Azure в Visual Studio][].

-   Как устранить неполадки веб-сайта

    Visual Studio предоставляет функции, упрощающие просмотр журналов Azure по мере их создания в режиме реального времени. Можно также работать в Azure удаленно в режиме отладки. Дополнительные сведения см. в разделе [Устранение неполадок веб-сайтов Azure в Visual Studio][].

-   Как добавить функции доступа к базе данных и авторизации

    Учебник по доступу к базе данных и ограничению некоторых функций сайта для использования авторизованными пользователями см. в статье [Развертывание безопасного приложения ASP.NET MVC с членством, OAuth и базой данных SQL на веб-сайте Azure][].
...
-   Как избежать затрат времени на ожидание пробуждения после времени простоя
    ...
    По умолчанию в случае простоя в течение определенного периода времени веб-сайты будут выгружены. Первый запрос после этого вынужден ожидать повторной загрузки сайта. Чтобы избежать времени ожидания, можно включить функцию AlwaysOn. Дополнительные сведения см. в информации о параметрах конфигурации в статье [Настройка веб-сайтов][].

-   Как добавить работающие в режиме реального времени функции, например, чат

    Если ваш веб-сайт будет содержать функции, работающие в режиме реального времени (например, служба чата, игра, тикер акций и т. д.), наилучшей производительности можно добиться при использовании [ASP.NET SignalR][] с методом транспортировки [WebSockets][]. Дополнительные сведения см. в разделе [Использование SignalR с веб-сайтами Windows Azure][].

-   Как выбрать между веб-сайтами Azure, облачными службами и виртуальными машинами для веб-приложений

    Веб-приложения в Azure можно выполнять на веб-сайтах, как показано в этом учебнике, а также в облачных службах и на виртуальных машинах. Дополнительные сведения см. в статьях [Модели выполнения Azure][] и [Веб-сайты Azure, облачные службы и виртуальные машины: когда они используются?][]
