...
-   Создание модели
    ...
    -   [Шаг 1: Получение данных][Шаг 1: Получение данных]
    -   [Шаг 2: Предварительная подготовка данных][Шаг 2: Предварительная подготовка данных]
    -   [Шаг 3: Define features][Шаг 3: Define features]
-   Обучение модели

    -   [Шаг 4: Выбор и применение алгоритма обучения][Шаг 4: Выбор и применение алгоритма обучения]
-   Оценка и тестирование модели

    -   [Шаг 5: Прогнозирование по новым данным][Шаг 5: Прогнозирование по новым данным]
...
2.  В левой части области эксперимента расположена выборка данных и модулей. Введите «автомобили» в поле поиска в верхней части палитры и найдите выборку данных под названием **Данные о ценах на автомобили (необработанные)**.
    ...
    ![Поиск в палитре][Поиск в палитре]
...
![Визуализация выборки данных][Визуализация выборки данных]
...
2.  Выберите модуль **Столбцы проекта** и щелкните **Запустить средство выбора столбцов** в области свойств.
    ...
    ![Свойства столбцов проекта][Свойства столбцов проекта]
...
3.  Перетащите модуль **Очистка отсутствующих значений** в область эксперимента и подключите его к модулю **Столбцы проекта**. В области свойств выберите **Удалять строку полностью** в **Для отсутствующих значений**, чтобы очистить данные путем удаления строк, имеющих отсутствующие значения. Дважды щелкните модуль и введите комментарий «Удаление строк с отсутствующими значениями».
    ...
    ![Свойства модуля очистки значений][Свойства модуля очистки значений]
...
![Первый запуск эксперимента][Первый запуск эксперимента]
...
4.  Найдите и переместите модуль **Обучение модели** в эксперимент. Щелкните **Запустить средство выбора столбцов** и выберите столбец *цена*. Это значение, которое наша модель собирается спрогнозировать.
    ...
    ![Выберите столбец «Цена»][Выберите столбец «Цена»]
...
![Применение обучающего алгоритма][Применение обучающего алгоритма]
...
1.  Найдите и перетащите модуль **Оценка модели** в поле эксперимента и подключите левый входной порт к выходу модуля **Обучение модели** и правый входной порт с выходом тестовых данных (правый порт) модуля **Разделение**.
    ...
    ![Модуль «Оценка модели»][Модуль «Оценка модели»]
...
![Завершенный эксперимент][Завершенный эксперимент]
...
Для более обширного и подробного пошагового руководства по созданию, подготовке, оценке и публикации прогнозной модели см. [Пошаговое руководство: Разработка прогнозного решения с помощью машинного обучения Azure][Пошаговое руководство: Разработка прогнозного решения с помощью машинного обучения Azure].
