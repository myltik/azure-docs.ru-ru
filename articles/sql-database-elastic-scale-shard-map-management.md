<properties title="Shard Map Management" pageTitle="Управление картой сегментов" description="Как использовать ShardMapManager, API эластичного масштабирования для .NET" metaKeywords="sharding scaling, Azure SQL Database sharding, elastic scale, shardmapmanager" services="sql-database" documentationCenter="" manager="jhubbard" authors="sidneyh@microsoft.com"/>

<tags ms.service="sql-database" ms.workload="sql-database" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="10/02/2014" ms.author="sidneyh" />

#Управление картой сегментов 
**Карта сегментов** в среде сегментированной базы данных осуществляет информационную поддержку и позволяет приложениям подключаться к нужной базе данных с учетом **ключа сегментирования**. Для управления сегментами в предварительной версии эластичного масштабирования важно понимать принцип создания этих карт сопоставлений.
...
Карты сегментов создаются с использованием **списков отдельных ключей сегментирования** и **диапазонов значений ключей сегментирования**. 

###Список сопоставления шардлетов и сегментов
**Сегменты** содержат **шардлеты**, а сопоставление шардлетов с сегментами осуществляется с помощью карты сопоставления сегментов. **Список сопоставления шардлетов и сегментов** устанавливает соответствие между отдельными ключами, которые идентифицируют шардлеты, и базами данных, которые используются как сегменты.  **Списки сопоставлений** задаются в явном виде (например, ключ 1 соответствует базе данных А), причём разные ключи могут быть сопоставлены с одной базой данных (например, оба ключа 3 и 6 сопоставляются базе данных Б).
<table>
   <tr>
    <td>Ключ</td>
...
###Диапазоны сопоставления сегментов 
В **карте сопоставления сегментов** диапазон ключей определяется парой **[нижнее значение, верхнее значение**, где *нижнее значение* - это наименьший ключ диапазона, а *верхнее значение* - это значение ключа, следующее за наибольшим значением диапазона (например, на единицу больше наибольшего значения, если ключ является целым числом). 

Например, диапазон **[0, 100)** охватывает все целые числа, которые больше или равны 0 и меньше 100. Учитывайте, что несколько диапазонов могут указывать на одну базу данных. Кроме того, поддерживаются не пересекающиеся диапазоны (например, диапазоны [100,200) и [400,600) указывают на одну и ту же БазуДанных_В, как показано в примере ниже).
<table>
   <tr>
...
Каждая из приведенных выше таблиц - это концептуальный пример объекта **ShardMap**.  Каждая строка - упрощенный пример отдельного объекта **PointMapping** (для списка сопоставления шардлетов и сегментов) или **RangeMapping** (для диапазона сопоставления сегментов).
...
1. **Глобальное сопоставление сегментов(GSM)**. При создании **ShardMapManager** указывается база данных для хранения всех карт сегментов и сопоставлений. Для управления этой информацией автоматически создаются специальные таблицы и хранимые процедуры. Обычно это небольшая и база данных с простым доступом, но её не рекомендуется использовать для других прикладных задач. Таблицы находятся в особой схеме **__ShardManagement**. 
...
Объект **ShardMapManager** следует объявлять в коде инициализации приложения и не более одного раза для каждого домена приложения. Объект **ShardMapManager** может содержать неограниченное число карт сегментов. Многим приложениям достаточно одной карты сегментов, но в некоторых случаях применяются разные наборы баз данных, которые используются в разных схемах или имеют уникальное назначение. В таком случае желательно использовать несколько карт сегментов. 
...
Для приложений администрирования карт сегментов (добавление и изменение сегментов, карт сегментов, сопоставлений сегментов и т.д.)необходимо объявлять **ShardMapManager**, пользуясь **учетными данными с правами чтения и записи как для базы данных GSM, так и для базы данных, выступающей в роли сегмента**. Учетные данные должны давать право записи в таблицы в GSM и LSM при вводе или изменения информации карт сегментов, а также право создавать таблицы LSM в новых сегментах.  

### Изменяются только метаданные 

Для заполнения и изменения данных объекта **ShardMapManager** используются методы, не изменяющие пользовательские данные, хранящиеся в самих сегментах. Например, такие методы как **CreateShard**, **DeleteShard**, **UpdateMapping** и др. изменяют только метаданные карты сегментов, но не удаляют, не добавляют и не изменяют пользовательские данные в этих сегментах. На самом деле, эти методы предназначены для использования с отдельными операциями, с помощью которых вы создаете и удаляете фактические базы данных, либо перемещаете строки между сегментами для выравнивания сегментированной среды.  (Служба **разбиения/объединения**, включенная в приложение эластичного масштабирования, применяет эти интерфейсы API наряду с оркестрацией перемещения данных между сегментами.) 

## Заполнение карты сегментов. Пример 
Ниже приведен пример последовательности операций для заполнения конкретной карты сегментов. Этот код выполняет следующие действия. 

1. При помощи диспетчера карт сегментов создается новая карта сегментов. 
2. В карту сегментов добавляются метаданные двух разных сегментов. 
3. Добавляется ряд сопоставлений диапазонов ключей, отображается всё содержимое карты сегментов. 

Код программы написан таким образом, что весь метод можно безопасно повторить при возникновении неизвестной ошибки: перед попыткой создания сегмента или сопоставления при каждом запросе проверяется наличие такого сегмента или сопоставления. Приведенный ниже код написан с допущением, что базы данных **sample_shard_0**, **sample_shard_1** и **sample_shard_2** уже существуют на сервере **shardServer**. 
...
После заполнения карт сегментов можно создавать приложения для доступа к данным, либо приспособить для работы с этими картами существующие приложения. Дальнейшего заполнения или обработки карт не требуется до момента внесения изменений в **структуру карты**.  
...
Учитывайте, что эти приложения (использующие **ShardMapManager** с правом доступа только для чтения) не смогут внести изменения в ни карты, ни в сопоставления сегментов.  Для этого создайте специальные административные приложения или сценарии PowerShell, которые будут передавать учетные данные с более высокими правами доступа, как об этом говорилось ранее.   

Дополнительные сведения см. в разделе [Маршрутизация, зависящая от данных](./sql-database-elastic-scale-data-dependent-routing.md). 

##Изменение карты сегментов 

Карту сегментов можно изменять различными способами. Любым из приведенных способов можно изменить метаданные сегментов и сопоставления сегментов, но нельзя изменить данные самих сегментов, создать или удалить базу данных.  Некоторые из описанных ниже операций над картами сегментов необходимо выполнять согласованно с административными действиями для физического перемещения данных, добавления и удаления баз данных, выступающих в качестве сегментов.
...
* Чтобы добавлять и удалять сегменты, используйте **CreateShard** и **DeleteShard**. 
    ...
    Для успешного выполнения этих операций необходимо указать существующие сервер и базу данных, которая играет роль целевого сегмента. Эти методы не изменяют сами базы данных, они изменяют только метаданные в карте сегментов.
...
* Чтобы разбить существующий диапазон на две части, объединить соседние диапазоны в один, используйте **SplitMapping** и **MergeMappings**.  
    ...
    Учтите, что операции разбиения и объединения **не изменяют привязку сегментов и ключей**. Функция разбиения разделяет существующий диапазон на две части, но оставляет их обе привязанными к одному сегменту. Функция объединения соединяет два соседних диапазона, привязанных к одному сегменту, в один целый диапазон.  Перемещение точек или диапазонов между сегментами при помощи метода **UpdateMapping** необходимо выполнять совместно с перемещением данных.  При необходимости перемещения данных, для координации изменений карты сегментов и перемещения данных используйте службу **разбиения/объединения**, входящую в состав приложения эластичного масштабирования. 

* Повторное сопоставление (или перемещение) отдельных точек или диапазонов с другими сегментами: **UpdateMapping**.  
...
* Сделать сопоставления активными и неактивными: методы **MarkMappingOffline** и **MarkMappingOnline** используются для контроля состояния сопоставления. 
