...
-   NUMA не поддерживается для виртуальных машин большего размера вследствие неисправности в версиях ядра Linux ниже 2.6.37. Установка waagent вручную приведет к автоматическому отключению NUMA в конфигурации GRUB для ядра Linux. Эта проблема влияет в основном на дистрибутивы, в которых используется исходное ядро Red Hat 2.6.32.

-   Мы рекомендуем не создавать раздел подкачки SWAP во время установки. Вы можете настроить пространство SWAP с помощью агента Linux для Azure. Также мы не рекомендуем использовать широко распространенное ядро Linux на виртуальной машине Azure без исправления, доступного на [веб-сайте Майкрософт][веб-сайте Майкрософт] (многие текущие дистрибутивы/ядра уже могут содержать это исправление).
...
Обязательно выполните эти действия с конфигурацией операционной системы, чтобы виртуальная машина могла использоваться как образ.
...
10. Замените файл /etc/yum.repos.d/CentOS-Base.repo следующим текстом
...
6.  Измените строку загрузки ядра в конфигурации Grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл /etc/default/grub в текстовом редакторе, найдите переменную GRUB\_CMDLINE\_LINUX\_DEFAULT (или добавьте ее, если это необходимо) и измените эту переменную, включив в нее следующие параметры:
...
**ПРИМЕЧАНИЕ.** [SUSE Studio][SUSE Studio] можно без труда использовать для создания образов SLES/opeSUSE для Azure и Hyper-V. Кроме того, в коллекции SUSE Studio доступны для скачивания приведенные ниже официальные образы, которые можно затем клонировать в свою учетную запись SUSE Studio для последующей настройки — [SLES 11 SP3 для Azure в коллекции SUSE Studio][SLES 11 SP3 для Azure в коллекции SUSE Studio].
...
3.  Измените строку загрузки ядра в конфигурации grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл /boot/grub/menu.lst в текстовом редакторе и убедитесь, что ядро по умолчанию включает следующие параметры:
