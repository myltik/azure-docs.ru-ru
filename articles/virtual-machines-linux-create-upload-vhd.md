<properties linkid="manage-linux-common-task-upload-vhd" urlDisplayName="Передача VHD" pageTitle="Создайте и передайте VHD Linux в Azure" metaKeywords="Azure VHD, передача VHD Linux" description="Узнать, как создать и передать виртуальный жесткий диск (VHD) в Azure с операционной системой Linux." metaCanonical="" services="virtual-machines" documentationCenter="" title="Создание и загрузка виртуального жесткого диска, содержащего операционную систему Linux" authors="kathydav" solutions="" manager="jeffreyg" editor="tysonn" />
...
- **Сертификат управления** -  Создан (и экспортирован в файл .cer) сертификат управления для подписки, для которой требуется загрузить VHD. Дополнительные сведения о создании сертификатов см. в разделе [Создание сертификата управления в Azure](http://msdn.microsoft.com/library/windowsazure/gg551722.aspx). 

- **Операционная система Linux установлена в VHD-файле.**  - Установлена поддерживаемая операционная система Linux на виртуальном жестком диске. Существует несколько средств для создания VHD-файлов. Для создания файла VHD и установки операционной системы можно использовать решения для виртуализации, например Hyper-V. Инструкции см. в разделе [Установка роли Hyper-V и настройка виртуальной машины](http://technet.microsoft.com/library/hh846766.aspx). 
...
- **Командлет Add-AzureVHD**, который является частью модуля Azure PowerShell. Чтобы загрузить модуль, см. страницу [Загрузки Azure](/ru-ru/develop/downloads/). Справочные сведения см. в разделе [Add-AzureVhd](http://msdn.microsoft.com/library/windowsazure/dn495173.aspx).
...
- Агент Linux для Azure (Waagent) несовместим с NetworkManager. Конфигурация сети должна быть управляема с помощью сценариев ifup/ifdown.  Большинство пакетов RPM/Deb, предоставленных вместе с дистрибутивами, настраивают NetworkManager с использованием параметров, конфликтующих с пакетом waagent, поэтому это приведет к удалению NetworkManager при установке пакета агента Linux.  Агент Linux для Azure также нуждается в установке пакета python-pyasn1.

- NUMA не поддерживается для ВМ большего размера вследствие неисправности в версиях ядра Linux ниже 2.6.37. Установка waagent вручную приведет к автоматическому отключению NUMA в конфигурации GRUB для ядра Linux. Эта проблема влияет в основном на дистрибутивы, в которых используется исходное ядро Red Hat 2.6.32.
...
6.	Переместите или удалите правила udev во избежание создания статических правил для интерфейса Ethernet.  Эти правила приводят к появлению проблем при клонировании виртуальной машины в Windows Azure или Hyper-V:
...
8.	**Только CentOS 6.2 или 6.3**. Установите драйверы для служб интеграции Linux
	...
	**Примечание.** Это действие действительно только для CentOS 6.2 и 6.3.  В CentOS 6.4 + службы интеграции Linux уже доступны в ядре.
	...
	![Откройте параметры Hyper-V](./media/virtual-machines-linux-create-upload-vhd/settings.png)

	с) На панели **Оборудование** щелкните пункт **Контроллер IDE 1**.

	![Добавьте DVD-привод с установочным носителем](./media/virtual-machines-linux-create-upload-vhd/installiso.png)
...
9. Установите python-pyasn1, выполнив следующую команду:

		# sudo yum install python-pyasn1

10. Замените файл /etc/yum.repos.d/CentOS-Base.repo на следующий текст
...
11.	Добавьте следующую строку в файл /etc/yum.conf:
	...
	В CentOS 6.2 и 6.3 добавьте только следующую строку:
...
14. В CentOS 6.2 и 6.3 обновите работающее ядро виртуальной машины, выполнив следующие команды
...
15.	Измените строку загрузку ядра в конфигурации grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл /boot/grub/menu.lst в текстовом редакторе и убедитесь, что ядро по умолчанию включает следующие параметры:

		console=ttyS0 earlyprintk=ttyS0 rootdelay=300 numa=off

	Это также гарантирует отправку всех сообщений консоли на первый последовательный порт, что может помочь технической поддержке Azure в плане отладки. Кроме того, это приведет к отключению NUMA в связи с неисправностью версии ядра, используемого CentOS 6.
...
9.	Измените строку загрузку ядра в конфигурации grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл /boot/grub/menu.lst в текстовом редакторе и убедитесь, что ядро по умолчанию включает следующие параметры:
...
15. Щелкните в диспетчере Hyper-V пункт **Завершение работы**.


### Подготовка SUSE Linux Enterprise Server 11 SP2 и SP3 ###
...
3. Добавьте репозиторий, содержащий последнюю версию ядра и агента Linux для Azure. Выполните команду: `zypper lr`. Например, в SLES 11 SP3 выход будет выглядеть следующим образом:
	...
	возможно, необходимо повторно включить репозитории или зарегистрировать систему.  Это можно сделать с помощью служебной программы suse_register.  Дополнительные сведения см. в [документации по SLES](https://www.suse.com/documentation/sles11/).
...
7.	Рекомендуется изменить в файле /etc/sysconfig/network/dhcp или его эквиваленте значение с DHCLIENT_SET_HOSTNAME=&quot;yes&quot; на DHCLIENT_SET_HOSTNAME=&quot;no&quot;
...
4.  В консоли оболочки выполните команду &apos;`zypper lr`&apos;. Если эта команда возвращает результат аналогичный приведенному ниже (обратите внимание, что номера версий могут отличаться):
   ...
   Если команда возвращает &quot;Репозитории не определены. Воспользуйтесь командой `zypper addrepo` для добавления одного 
   или нескольких репозиториев&quot;, тогда репозитории нужно включить повторно:
...
7.	Измените строку загрузки ядра в конфигурации grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл /boot/grub/menu.lst в текстовом редакторе и убедитесь, что ядро по умолчанию включает следующие параметры:
...
9.	Рекомендуется изменить в файле /etc/sysconfig/network/dhcp или его эквиваленте значение с DHCLIENT_SET_HOSTNAME=&quot;yes&quot; на DHCLIENT_SET_HOSTNAME=&quot;no&quot;
...
5.	Рекомендуется изменить в файле /etc/sysconfig/network/dhcp или его эквиваленте значение с DHCLIENT_SET_HOSTNAME=&quot;yes&quot; на DHCLIENT_SET_HOSTNAME=&quot;no&quot;
