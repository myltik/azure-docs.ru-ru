...
![В диаграмме сбоев щелкните сначала "Правила оповещения", а затем "Добавить оповещение"](./media/appinsights/appinsights-d023-alert.png)
...
При сборке в режиме отладки события передаются по мере их создания. При потере доступа в Интернет и выходе из приложения до восстановления доступа данные автономной телеметрии удаляются.

#### Выпуск

Если сборка выполняется в конфигурации выпуска, события хранятся на устройстве и передаются, когда приложение возобновляет работу. Данные также передаются при первом использовании приложения. Если подключение к Интернету отсутствует при запуске, данные предыдущей телеметрии, а также данные телеметрии для текущего жизненного цикла хранятся и отправляются при следующем возобновлении работы.
