...
Ниже приведен пример, иллюстрирующий преимущества параллельного выполнения задач. Предположим, что приложение задачи имеет такие требования к ЦП и памяти, что их можно удовлетворить, используя узел размером [Standard\_D1](../cloud-services/cloud-services-sizes-specs.md#general-purpose-d). Однако для выполнения задачи в заданное время необходимы 1000 таких узлов.
...
Ознакомьтесь с проектом [ParallelNodeTasks][parallel_tasks_sample] на GitHub. Это рабочий пример кода, демонстрирующий использование [CloudPool.MaxTasksPerComputeNode][maxtasks_net].
