...
> [AZURE.TIP]В этом учебнике используются некоторые основные понятия, связанные с работой пакетной службы, которые рассматриваются в статье [Обзор функций пакетной службы Azure](batch-api-basics.md). Если у вас нет опыта работы с пакетной службой, мы настоятельно рекомендуем прочесть эту статью.
...
2. Добавьте следующий код в Main, который настраивает учетные данные для вызова пакетной службы Azure.
	...
	Замените значения в квадратных скобках значениями, связанными с вашей учетной записью пакетной службы. Все эти значения можно найти на [портале Azure](https://portal.azure.com). Чтобы найти эти значения, войдите на [портал Azure](https://portal.azure.com) и сделайте следующее:
