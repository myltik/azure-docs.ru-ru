...
- Учетные записи.
	...
	- **Учетная запись пакетной службы** — см. раздел **Учетная запись пакетной службы** в статье [Технический обзор пакетной службы Azure](batch-technical-overview.md).

	- **Учетная запись хранения** — см. раздел **Создание учетной записи хранения** в статье [Учетные записи хранения Azure](../storage-create-storage-account.md). Следуя указаниям этого учебника, вы создадите контейнер в этой учетной записи с именем **testcon1**.
...
- Сборки NuGet.
	...
	1. После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Выполните поиск в Интернете, введя **Azure.Batch**, затем щелкните **Установить**, чтобы установить пакет пакетной службы Microsoft Azure и зависимые компоненты.

	2. Выполните поиск в Интернете, введя **WindowsAzure.Storage**, а затем щелкните **Установить**, чтобы установить пакет службы хранилища Azure и зависимые компоненты.

> [AZURE.TIP]В этом учебнике используются некоторые основные понятия пакетной службы, рассмотренные в статье [Основные сведения об API Пакетной службы Azure](batch-api-basics.md). Поэтому настоятельно рекомендуется прочитать эту статью тем, кто раньше не сталкивался с пакетной службой.
...
1. Откройте файл App.config проекта GettingStarted, а затем добавьте элемент *&lt;appSettings&gt;* в *&lt;configuration&gt;*.
	...
	- **[account-key]** — первичный ключ учетной записи хранения. Первичный ключ находится на странице «Хранилище» на портале управления Azure.

2. Сохраните файл App.config.

Чтобы больше узнать о строках подключения службы хранилища Azure, см. раздел [Настройка строк подключения службы хранилища Azure](../storage/storage-configure-connection-string.md).
...
2. Добавьте *System.Configuration* в раздел **Ссылки** в **обозревателе решений** для проекта GettingStarted.
...
Чтобы больше узнать о хранилище больших двоичных объектов, см. в раздел [Использование хранилища BLOB-объектов из .NET](../storage/storage-dotnet-how-to-use-blobs.md).
...
2. После создания проекта в Visual Studio щелкните правой кнопкой мыши проект в **обозревателе решений** и выберите **Управление пакетами NuGet**. Выполните поиск в Интернете, введя **WindowsAzure.Storage**, а затем щелкните **Установить**, чтобы установить пакет службы хранилища Azure и зависимые компоненты.
...
2. Добавьте следующий код в Main, который настраивает учетные данные для вызова пакетной службы Azure.
	...
	Замените значения в квадратных скобках значениями, связанными с вашей учетной записью пакетной службы. Их можно найти на [портале предварительной версии Azure](https://portal.azure.com). Чтобы найти эти значения, войдите на [портал предварительной версии Azure](https://portal.azure.com) и:
...
1. Добавьте следующий метод в класс Program, который добавляет три задачи в задание.
	...
	Значение **[account-name]** необходимо заменить именем ранее созданной учетной записи хранения. В предыдущем примере замените все четыре вхождения **[account-name]**.
