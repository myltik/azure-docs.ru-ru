...
> [AZURE.IMPORTANT] В настоящее время пакетная служба поддерживает *только* учетные записи хранения **общего назначения**, как описано в шаге 5 раздела [Создайте учетную запись хранения](../storage/storage-create-storage-account.md#create-a-storage-account) в статье [Об учетных записях хранения Azure](../storage/storage-create-storage-account.md).
...
[**Шаг 1.**](#step-1-create-storage-containers) Создайте **контейнеры** в хранилище BLOB-объектов Azure.<br/> [**Шаг 2.**](#step-2-upload-task-application-and-data-files) Отправьте файлы приложения задач и входные файлы в контейнеры.<br/> [**Шаг 3.**](#step-3-create-batch-pool) Создайте **пул** пакетной службы.<br/> &nbsp;&nbsp;&nbsp;&nbsp;**3а.** Пул **StartTask** загружает двоичные файлы задач (TaskApplication) на узлы во время их присоединения к пулу.<br/> [**Шаг 4.**](#step-4-create-batch-job) Создайте **задание** пакетной службы.<br/> [**Шаг 5.**](#step-5-add-tasks-to-job) Добавьте **задачи** в задание.<br/> &nbsp;&nbsp;&nbsp;&nbsp;**5а.** Планирование задач выполняется на узлах.<br/> &nbsp;&nbsp;&nbsp;&nbsp;**5б.** Каждая задача загружает свои входные данные из службы хранилища Azure, а затем начинает выполняться.<br/> [**Шаг 6.**](#step-6-monitor-tasks) Выполните мониторинг задач.<br/> &nbsp;&nbsp;&nbsp;&nbsp;**6а.** После выполнения задач их выходные данные отправляются в службу хранилища Azure.<br/> [**Шаг 7.**](#step-7-download-task-output) Скачайте выходные данные задачи из службы хранилища.
...
> [AZURE.IMPORTANT] Как упоминалось выше, вы должны указать учетные данные для учетной записи хранения **общего назначения** в службе хранилища Azure. Приложения пакетной службы будут использовать хранилище BLOB-объектов в пределах учетной записи хранения **общего назначения**. Не указывайте учетные данные для учетной записи хранения, во время создания которой в качестве типа учетной записи выбрано значение *Хранилище BLOB-объектов*.
...
> [AZURE.TIP] Если пакеты NuGet не восстановились автоматически или во время их восстановления возникли ошибки, убедитесь, что вы установили [диспетчер пакетов NuGet][nuget_packagemgr]. Затем включите загрузку недостающих пакетов. Чтобы включить загрузку пакетов, см. инструкции в разделе [Enabling Package Restore During Build][nuget_restore] (Включение восстановления пакетов во время сборки).
...
Перейдите к началу метода `MainAsync` в файле `Program.cs` проекта *DotNetTutorial*, чтобы начать с шага 1. Каждый из шагов ниже в целом соответствует последовательности вызовов методов в `MainAsync`.
...
- **input** — задачи будут загружать файлы данных, которые они должны обрабатывать, из контейнера *input*.
...
Мы используем ссылку `blobClient` везде в приложении и передаем ее в качестве параметра нескольким методам. Пример этого можно увидеть в блоке кода, который находится сразу за методом blobClient. С его помощью мы вызываем `CreateContainerIfNotExistAsync`, чтобы фактически создать контейнеры.
...
Во время отправки файлов приложение *DotNetTutorial* сначала определяет коллекции путей к файлам **приложения** и **входным файлам** на локальном компьютере. Затем оно отправляет эти файлы в контейнеры, созданные в рамках предыдущего шага.
...
Объект [ResourceFile][net_resourcefile] передает задачи в пакетную службу с URL-адресом файла в службе хранилища Azure, который будет загружен на вычислительный узел перед выполнением этой задачи. Свойство [ResourceFile.BlobSource][net_resourcefile_blobsource] указывает полный URL-адрес файла, по которому его можно найти в службе хранилища Azure. URL-адрес может также включать подписанный URL-адрес (SAS), который обеспечивает безопасный доступ к файлу. Большинство типов задач в пакетной службе .NET, в том числе перечисленные ниже, содержат свойство *ResourceFiles*.
...
- **Подписанные URL-адреса больших двоичных объектов** — задача StartTask пула в DotNetTutorial использует подписанные URL-адреса больших двоичных объектов во время загрузки двоичных файлов приложения и файлов входных данных из службы хранилища (см. шаг 3 ниже). Метод `UploadFileToContainerAsync` в файле `Program.cs` приложения DotNetTutorial содержит код, который получает подписанный URL-адрес каждого большого двоичного объекта. Для этого выполняется вызов [CloudBlob.GetSharedAccessSignature][net_sas_blob].
...
После отправки файлов приложения и файлов данных в учетную запись хранения приложение *DotNetTutorial* начинает взаимодействие с пакетной службой, используя библиотеку .NET пакетной службы. Для этого сначала создается [BatchClient][net_batchclient]\:
...
В этом примере приложения задача StartTask копирует файлы, которые она загружает из рабочего каталога StartTask службы хранилища (эти файлы указаны в свойстве [StartTask][net_starttask].[ResourceFiles][net_starttask_resourcefiles]) в общий каталог, к которому есть доступ у *всех* задач, выполняемых на узле. По сути это обеспечивает копирование `TaskApplication.exe` и его зависимостей в общий каталог на каждом узле, когда узел присоединяется к пулу, чтобы к нему был доступ у всех задач, запускаемых на узле.
...
![Добавление задач в задание][5]<br/> *(1) Задачи добавляются в задание, (2) планируется запуск задач на узлах, (3) задачи загружают файлы данных для обработки*
...
> [AZURE.NOTE] При вызове `DownloadBlobsFromContainerAsync` в приложении *DotNetTutorial* указывается, что файлы необходимо загрузить в папку `%TEMP%`. Вы можете изменить это расположение выходных данных.
