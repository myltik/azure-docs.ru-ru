<properties
	...
	description="Узнайте, как уменьшить количество элементов пакетной службы Azure, возвращаемых в списке, а также количество сведений, возвращаемых для каждого элемента."
	services="batch"
	documentationCenter=""
	authors="davidmu1"
	manager="timlt"
...
Мониторинг используется в стандартных сценариях; определение объема и состояния пула требует, например, чтобы все вычислительные узлы (виртуальные машины) в пуле опрашивались. Другим примером служит запрос задач задания для выявления задач в очереди. В некоторых случаях необходим широкий набор данных, а в других — только общее количество элементов или объектов в определенном состоянии.

Важно понимать, что как количество элементов, которое может быть возвращено, так и объем данных, необходимый для представления списка элементов, могут быть очень велики. Простое опрашивание большого количества элементов, возвращающее множество результатов, может привести к ряду проблем.

- Время ответа API пакетной службы может стать слишком большим. Чем большее количество элементов, тем больше пакетной службе нужно времени на обработку запроса. Большое количество элементов необходимо разбить на отрезки, поэтому клиентской библиотеке, возможно, потребуется создать несколько вызовов API к службе, чтобы получить все элементы для этого одного списка.
...
- Параметр DetailLevel с предложениями Select и Expand может также передаваться в соответствующие методы Get, например PoolOperations.GetPool().
...
Предложение [ExpandClause](https://msdn.microsoft.com/library/azure/microsoft.azure.batch.odatadetaillevel.expandclause.aspx) похоже на предложение Select, поскольку оно управляет возвратом определенных данных в результатах. Предложение Expand поддерживается только для списка задач, списка пулов и списка заданий. В настоящее время оно поддерживает только статистические данные. Если необходимы все свойства и предложение выбора не указано, для получения статистических сведений следует использовать предложение развертывания. Если предложение выбора используется для получения подмножества свойств, то в предложении выбора можно указать статистику, а для предложения развертывания можно оставить значение null.
