...
> [AZURE.IMPORTANT]Для успешного запуска примера приложения вам потребуется зарегистрировать его в Azure AD с помощью портала Azure. Обратитесь к разделу "Добавление приложения" в статье [Интеграция приложений с Azure Active Directory][aad_integrate]. Затем следуйте инструкциям в этой статье для регистрации примера приложения в вашей учетной записи.
...
2. Создание [SubscriptionClient][resman_subclient] для получения у Azure списка подписок учетной записи с помощью маркера безопасности, полученного из Azure AD. Если найдено несколько подписок, пользователь сможет выбрать одну.
