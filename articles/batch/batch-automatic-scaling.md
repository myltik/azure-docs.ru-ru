  ...
  <tr>
    ...
    <td><p>Возвращает вектор образцов данных. Например:</p>
        <ul>
          ...
          <p>Метод $CPUPercent.GetSample(TimeInterval_Minute*10) должен вернуть 200&#160;выборок, если все они присутствовали в журнале CPUPercent в течение последних 10&#160;минут. Если последняя минута журнала по-прежнему отсутствует, возвращаются только 180&#160;выборок.</p>
					<p>Метод $CPUPercent.GetSample(TimeInterval_Minute*10, 80) выполняется успешно, а метод $CPUPercent.GetSample(TimeInterval_Minute*10,95) не выполняется.</p></li>
