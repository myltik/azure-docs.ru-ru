...
При включенном автоматическом масштабировании количество доступных вычислительных узлов корректируется каждые 15 минут на основе формулы. Формула опирается на выборки, которые собираются периодически. Между получением выборки и ее доступностью для формулы происходит задержка. Это необходимо учитывать при использовании метода GetSample, описанного ниже.
