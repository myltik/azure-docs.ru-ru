...
В следующем фрагменте кода создается пул с поддержкой автомасштабирования с помощью библиотеки [.NET пакетной службы][net_api]. Формула автомасштабирования пула устанавливает целевое количество узлов в 5 по понедельникам и в 1 в остальные дни недели. [Интервал автомасштабирования](#automatic-scaling-interval) составляет 30 минут. В этом и других фрагментах кода C#, приведенных в этой статье, myBatchClient представляет собой полностью инициализированный экземпляр [BatchClient][net_batchclient].
...
> [!IMPORTANT]
> При создании пула с поддержкой автомасштабирования **не** нужно указывать параметр `targetDedicated`. Кроме того, если необходимо вручную изменить размер пула с поддержкой автомасштабирования (например, с помощью [BatchClient.PoolOperations.ResizePool][net_poolops_resizepool]), требуется сначала **отключить** автомасштабирование пула, а затем изменить его размер.
> 
> 
...
* [включение автомасштабирования в пуле][rest_enableautoscale] (REST API).
