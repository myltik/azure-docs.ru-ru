...
> [AZURE.TIP] Дополнительные сведения о возможностях, которые предоставляют интерфейсы API пакетной службы, см. в статье [Обзор функций пакетной службы Azure](batch-api-basics.md).
...
- Клиентская библиотека [.NET для пакетной службы][api_net]. Эта библиотека используется для разработки большинства решений на основе пакетной службы. Она [доступна на сайте NuGet][api_net_nuget].

- Клиентская библиотека [.NET для управления пакетной службой][api_net_mgmt]. Эта библиотека используется для программного управления учетными записями пакетной службы в клиентских приложениях или службах. Она тоже [доступна на сайте NuGet][api_net_mgmt_nuget].
...
- [Обозреватель службы хранилища Microsoft Azure][storage_explorer]. Хотя этот обозреватель не является собственно средством пакетной службы Azure, это полезный инструмент для разработки и отладки решений, использующих пакетную службу.
...
- О способах использования многих функций пакетной службы с помощью библиотеки .NET для пакетной службы вы можете узнать не только из обозревателя пакетной службы, но и из других [примеров кодов на сайте GitHub][github_samples].

- Список ресурсов по работе с пакетной службой см. на схеме [Пакетная служба][learning_path].
