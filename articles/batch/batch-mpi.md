...
С помощью задач с несколькими экземплярами можно выполнять задание пакетной службы Azure на нескольких узлах одновременно, чтобы запускать высокопроизводительные вычислительные сценарии, такие как приложения с интерфейсом передачи сообщений (MPI). В этой статье вы узнаете, как выполнять задачи с несколькими экземплярами с помощью библиотеки [.NET для пакетной службы][api_net].

> [AZURE.IMPORTANT] Задачи с несколькими экземплярами в настоящий момент поддерживаются только пулами, созданными на основе образов **CloudServiceConfiguration**. Использовать такие задачи в пулах, созданных на основе образов VirtualMachineConfiguration, нельзя. Дополнительные сведения об этих двух конфигурациях см. в разделе [Конфигурация виртуальных машин](batch-linux-nodes.md#virtual-machine-configuration) в статье [Подготовка вычислительных узлов Linux в пулах пакетной службы Azure](batch-linux-nodes.md).
...
2. При выполнении основной задачи и подзадач скачиваются любые **общие файлы ресурсов**, указанные в параметрах нескольких экземпляров.
3. После скачивания общих файлов ресурсов основная задача и подзадачи выполняют **команду координации**, указанную в параметрах нескольких экземпляров. Эта команда координации обычно используется для запуска фоновой службы (такой как [Microsoft MPI][msmpi_msdn] `smpd.exe`) и проверки готовности узлов к обработке сообщений между узлами.
4. После того, как основная задача и все подзадачи успешно выполнят команду координации, **командную строку** задачи с несколькими экземплярами ("команду приложения") выполняет *только* **основная задача**. Например, вы можете запустить выполнение приложения с поддержкой MPI в решении на базе [MS-MPI][msmpi_msdn] с помощью команды `mpiexec.exe`.
...
При выполнении задачи с несколькими экземплярами требуется, чтобы в пуле был **включен обмен данными между узлами** и **отключено выполнение параллельных задач**. Если запустить задачу с несколькими экземплярами в пуле, где отключен обмен данными между узлами или значение параметра *maxTasksPerNode* для которого больше 1, задача не будет запланирована. Вместо этого она будет постоянно находиться в состоянии "Активно". В этом фрагменте кода показано создание такого пула с помощью библиотеки .NET для пакетной службы.
...
Кроме того, задачи с несколькими экземплярами будут выполняться *только* на узлах в **пулах, созданных после 14 декабря 2015 года**.
...
## Команды координации и приложения

**Команду координации** выполняют основная задача и подзадачи. После того как основная задача и все подзадачи выполнят команду координации, командную строку задачи с несколькими экземплярами выполняет *только* основная задача. Назовем эту командную строку **командой приложения**, чтобы отличать ее от команды координации.
...
Обратите внимание, что в этой команде координации используется `start`. Это необходимо, так как приложение `smpd.exe` не возвращается сразу после выполнения. Если не использовать команду [start][cmd_start], команда координации не будет возвращена, а выполнение команды приложения будет заблокировано.

**Команду приложения**, командную строку, указанную для задачи с несколькими экземплярами, выполняет *только* основная задача. Для приложений MS-MPI это подразумевает выполнение приложения с поддержкой MPI с использованием команды `mpiexec.exe`. В качестве примера ниже приведена команда приложения для решения с использованием MS-MPI версии 7:
...
Для задач с несколькими экземплярами есть два набора файлов ресурсов: **общие файлы ресурсов**, которые скачивают *все* задачи (основная задача и подзадачи), и **файлы ресурсов**, указанные непосредственно для задачи с несколькими экземплярами, которые скачивает *только основная* задача.

В параметрах нескольких экземпляров для задачи можно указать один или несколько **общих файлов ресурсов**. Основная задача и все подзадачи скачивают эти общие файлы ресурсов из [службы хранилища Azure](./../storage/storage-introduction.md) в общий каталог задачи каждого узла. Доступ к общему каталогу задачи можно получить из приложения и командной строки координации с помощью переменной среды `AZ_BATCH_TASK_SHARED_DIR`.

Файлы ресурсов, указанные для самой задачи с несколькими экземплярами, скачиваются в рабочий каталог задачи, `AZ_BATCH_TASK_WORKING_DIR`, *только* основной задачей, так как подзадачи не скачивают эти файлы ресурсов.
...
Обратите внимание, что в примерах кода в этой статье мы указываем файлы ресурсов не для самой задачи с несколькими экземплярами, а только для задачи StartTask пула и [CommonResourceFiles][net_multiinsance_commonresfiles] параметров нескольких экземпляров.

> [AZURE.IMPORTANT] Для ссылки на эти каталоги в командной строке всегда используйте переменные среды `AZ_BATCH_TASK_SHARED_DIR` и `AZ_BATCH_TASK_WORKING_DIR`. Не пытайтесь создать пути вручную.
...
Свойства [TaskConstraints][net_taskconstraints] для задачи с несколькими экземплярами, такие как [MaxTaskRetryCount][net_taskconstraint_maxretry], [MaxWallClockTime][net_taskconstraint_maxwallclock] и [RetentionTime][net_taskconstraint_retention], учитываются как для стандартной задачи и применяются к основной задаче и всем подзадачам. Однако если изменить свойство [RetentionTime][net_taskconstraint_retention] после добавления задачи с несколькими экземплярами в задание, это изменение будет применено только к основной задаче. Для всех подзадач и дальше будет использоваться исходное значение свойства [RetentionTime][net_taskconstraint_retention].

Если последняя задача является частью задачи с несколькими экземплярами, в списке последних задач вычислительного узла будет отображаться идентификатор подзадачи.
...
- Вам может потребоваться создать простое приложение MS-MPI, которое можно использовать при тестировании задач с несколькими экземплярами в пакетной службе. В статье блога рабочей группы пакетной службы Azure и Microsoft HPC [How to compile and run a simple MS-MPI program][msmpi_howto] \(Как скомпилировать и запустить простую программу MS-MPI) содержатся пошаговые инструкции по созданию простого приложения MPI с помощью MS-MPI.
