...
Задача подготовки задания выполняется на всех вычислительных узлах, на которых запланировано выполнение задач, до выполнения задач задания. Когда задание завершается, на каждом узле в пуле, на котором была выполнена хотя бы одна задача, выполняется задача снятия задания. Для задач подготовки и снятия задания можно указать командную строку, вызываемую при выполнении задачи. Такие особые задачи предоставляют уже знакомые вам возможности, такие как загрузка файла, выполнение с повышенными правами, пользовательские переменные среды, максимальная продолжительность выполнения, число повторных попыток и период удержания файла.
...
> [AZURE.TIP] Задачи подготовки и снятия задания особенно полезны в средах с "общим пулом", то есть таких средах, в которых пул вычислительных узлов сохраняется между запусками заданий и может использоваться совместно с различными заданиями.
