...
Для заданий пакетной службы Azure часто требуется определенная наладка перед выполнением, а также определенные действия после завершения задач задания. Пакетная служба предоставляет механизмы для такой подготовки и обслуживания в виде необязательных задач *подготовки задания* и *снятия задания*.

Задача подготовки задания выполняется на всех вычислительных узлах, на которых запланировано выполнение задач, до выполнения задач задания. Когда задание завершается, на каждом узле в пуле, на котором была выполнена хотя бы одна задача, выполняется задача снятия задания. Задачи подготовки и снятия задания позволяют указать командную строку, которая будет выполняться при вызове задачи, а также предлагают такие возможности, как загрузка файла, выполнение с повышенными правами, использование пользовательских переменных среды, установка максимальной длительности выполнения, числа повторных попыток и времени хранения файла.

В следующих разделах вы узнаете, как использовать эти задачи с помощью [JobPreparationTask][net_job_prep] и [JobReleaseTask][net_job_release] в интерфейсе API [пакетной службы .NET][api_net].
...
- **Хранение журнала**. Может возникнуть необходимость хранить файлы журнала, создаваемые задачами, или, например, файлы аварийных дампов, создаваемые приложениями, в которых возникает сбой. В таких случаях с помощью *задачи снятия задания* можно сжать и передать эти данные в учетную запись [хранения Azure][azure_storage].

## Задача подготовки задания

Перед выполнением задач задания на каждом вычислительном узле, на котором запланировано выполнение задач, выполняется задача подготовки задания. По умолчанию пакетная служба ожидает завершения задачи подготовки задания и только после этого запускает запланированные задачи на узле, однако в настройках службы это ожидание можно отключить. Задача подготовки задания запускается на вычислительном узле снова при перезапуске узла, но это поведение также можно отключить.

Задача подготовки задания выполняется только на узлах, на которых запланировано выполнение задач. Благодаря этому на узлах, которым не назначены задачи, ненужная задача подготовки выполняться не будет, что позволяет, например, снизить затраты на передачу данных. Такая ситуация возникает, когда число задач в задании меньше количества узлов в пуле или когда включено [параллельное выполнение задач](batch-parallel-node-tasks.md), в результате чего некоторые узлы бездействуют, если число задач оказывается меньше, чем общее число возможных параллельных задач.

> [AZURE.NOTE]Задача [JobPreparationTask][net_job_prep_cloudjob] отличается от [CloudPool.StartTask][pool_starttask] тем, что выполняется в начале каждого задания, тогда как StartTask выполняется только при первом присоединении вычислительного узла к пулу или при его перезапуске.
...
## Задачи подготовки и снятия задания в пакетной службе .NET

Чтобы указать задачу подготовки задания, создайте и настройте задачу [JobPreparationTask][net_job_prep] и укажите ее в качестве значения свойства [CloudJob.JobPreparationTask][net_job_prep_cloudjob] своего задания. Аналогичным образом инициализируйте задачу [JobReleaseTask][net_job_release] и назначьте ее свойству [CloudJob.JobReleaseTask][net_job_prep_cloudjob] задания, чтобы настроить задачу снятия задания.
...
Как упоминалось выше, задача снятия выполняется, когда задание прекращается или удаляется. Чтобы прекратить задание с помощью интерфейса API пакетной службы .NET, нужно вызвать [PoolOperations.TerminateJobAsync][net_job_terminate], а чтобы удалить задание, нужно вызвать [PoolOperations.DeleteJobAsync][net_job_delete]. Оба этих действия обычно выполняются, когда задачи задания завершились или было достигнуто заданное время ожидания.
...
3. Запускает задачу подготовки задания, которая сначала записывает идентификатор узла в текстовый файл в каталоге shared узла.
