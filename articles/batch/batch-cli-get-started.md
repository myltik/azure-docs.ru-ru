...
В этой статье рассматривается интерфейс командной строки Azure версии 0.10.3.
...
Эта команда указывает пакет при создании пула и развертывает его при добавлении каждого узла к новому пулу:

    azure batch pool create --id "pool001" --target-dedicated 1 --vm-size "small" --os-family "4" --app-package-ref "MyTaskApplication"

В настоящее время нельзя указать версию развертываемого пакета с помощью параметров командной строки. Следует задать версию приложения, используемую по умолчанию, с помощью портала Azure, прежде чем назначать приложение пулу. Узнайте, как задать версию приложения по умолчанию, из статьи [Развертывание приложения с помощью пакетов приложений пакетной службы Azure](batch-application-packages.md). Вы также можете задать версию приложения по умолчанию при создании пула, если вместо параметров командной строки используется [JSON-файл](#json-files).
