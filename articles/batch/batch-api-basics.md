...
Пакет поддерживает две конфигурации учетной записи на основе свойства *режим распределения пула*. Две конфигурации предоставляют разные параметры для аутентификации с помощью пакетной службы и для подготовки [пулов](#pool) пакетной службы и управления или (см. далее в этой статье). 


* **Пакетная служба** (по умолчанию) — вы можете получить доступ к API пакетной службы, выполнив аутентификацию с использованием общего ключа или [аутентификацию Azure Active Directory](batch-aad-auth.md). Вычислительные ресурсы пакетной службы выделяются в фоновом режиме в учетной записи под управлением Azure.   
* **Пользовательская подписка** — вы можете получить доступ к API пакетной службы, только выполнив [аутентификацию Azure Active Directory](batch-aad-auth.md). Вычислительные ресурсы пакетной службы выделяются непосредственно в подписке Azure. Этот режим обеспечивает большую гибкость для настройки вычислительных узлов и интеграции с другими службами. Если вы используете этот режим, необходимо настроить дополнительное хранилище ключей Azure для учетной записи пакетной службы.
