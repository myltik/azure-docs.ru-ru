...
Пакет поддерживает две конфигурации учетной записи на основе свойства *режим распределения пула*. Эти конфигурации позволяют получить доступ к различным возможностям [пулов](#pool) пакетной службы (см. далее в этой статье).


* **Пакетная служба** — это компонент по умолчанию, в котором виртуальные машины пулов выделяются в фоновом режиме в подписке Azure. Используйте такую конфигурацию учетной записи, если требуются пулы облачных служб. Ее нельзя применять для пулов виртуальных машин, созданных на основе пользовательских образов виртуальных машин или использующих виртуальную сеть. Получить доступ к API пакетной службы можно путем проверки подлинности на основе общего ключа или ознакомившись со статьей [Проверка подлинности в решениях пакетной службы с помощью Active Directory](batch-aad-auth.md). Вы можете использовать выделенные или низкоприоритетные вычислительные узлы в пулах конфигурации учетной записи пакетной службы.

* **Пользовательская подписка.** Используйте такую конфигурацию учетной записи, если требуются пулы виртуальных машин, созданные на основе пользовательских образов виртуальных машин или использующие виртуальную сеть. Получить доступ к API пакетной службы можно только путем [проверки подлинности Active Directory](batch-aad-auth.md). Пулы облачных служб не поддерживаются. Вычислительные виртуальные машины пакетной службы выделяются непосредственно в подписке Azure. Если вы используете этот режим, необходимо настроить Azure Key Vault для учетной записи пакетной службы. Вы можете использовать только выделенные вычислительные узлы в пулах конфигурации учетной записи подписки пользователя. 
...
* **Операционная система** и **версия** вычислительного узла.
    ...
    > [!NOTE]
    > В режиме выделения пула пакетной службы при выборе операционной системы для вычислительных узлов в вашем пуле доступны два варианта: **конфигурация виртуальной машины** и **конфигурация облачных служб**. В режиме пользовательской подписки можно использовать только конфигурацию виртуальной машины.
    >

    Параметр **Конфигурация виртуальной машины** предоставляет образы Linux и Windows для вычислительных узлов из [магазина виртуальных машин Azure Marketplace][vm_marketplace]. В режиме пользовательской подписки можно использовать настраиваемые образы виртуальных машин.

    Создавая пул, содержащий узлы конфигурации виртуальных машин, необходимо указать не только размер узлов, но и **ссылку на образ виртуальной машины** и **номер SKU агента узла** пакетной службы для установки на узлах. Дополнительные сведения об указании этих свойств пула см. в статье [Подготовка вычислительных узлов Linux в пулах пакетной службы Azure](batch-linux-nodes.md).

    **Cloud Services Configuration** (Конфигурация облачных служб) предоставляет *только*вычислительные узлы Windows. Доступные операционные системы пулов с конфигурацией облачных служб перечислены в статье [Таблица совместимости выпусков гостевых ОС Azure и пакетов SDK](../cloud-services/cloud-services-guestos-update-matrix.md). При создании пула, содержащего узлы облачных служб, необходимо указать только размер узла и соответствующее *семейство ОС*. При создании пулов вычислительных узлов Windows чаще всего используются облачные службы.
...
* **Конфигурация сети**
    ...
    Можно указать идентификатор [виртуальной сети Azure](../virtual-network/virtual-networks-overview.md), в которой необходимо создать вычислительные узлы для пула. Более подробную информацию см. в разделе [Конфигурация сети пула](#pool-network-configuration).
