...
При использовании пакетной службы вы будете применять многие из указанных ниже ресурсов. Некоторые из них, например учетные записи, вычислительные узлы, пулы, задания и задачи, используются во всех решениях пакетной службы. Другие, например расписания заданий и пакеты приложений, являются полезными дополнительными функциями.
...
- [Задача.](#task)
    ...
    - [Зависимости задачи](#taskdep)
- [Расписания заданий](#jobschedule)
- [Пакеты приложений](#appkg)
...
- **Файлы ресурсов**, которые содержат данные для обработки. Эти файлы автоматически копируются на узел из хранилища BLOB-объектов в учетной записи хранения Azure **общего назначения**. Дополнительную информацию см. в разделах *Задача запуска* и [Файлы и каталоги](#files) ниже.
...
Подробные сведения о выполнении заданий MPI в пакетной службе с использованием библиотеки .NET для пакетной службы см. в статье [Использование задач с несколькими экземплярами для запуска приложений с интерфейсом передачи сообщений в пакетной службе Azure](batch-mpi.md).

#### <a name="taskdep"></a>Зависимости задач
...
* *Задача B* зависит от *задачи A* (выполнение *задачи B* не начнется, пока не завершится выполнение *задачи A*).
* *Задача C* зависит от *задачи A* и *задачи B*.
* *Задача D* зависит от ряда задач — от *задачи 1* до *задачи 10*.

Ознакомьтесь с примером кода [TaskDependencies][github_sample_taskdeps] в репозитории [azure-batch-samples][github_samples] на портале GitHub. В нем показано, как настроить задачи, зависящие от других задач, с помощью библиотеки [.NET пакетной службы][batch_net_api].
...
### <a name="appkg"></a>Пакеты приложений
...
Дополнительные сведения о пакетах приложений см. в статье, посвященной [Развертывание приложения с помощью пакетов приложений пакетной службы Azure](batch-application-packages.md).
...
Автоматическое масштабирование выполняется, когда вы указываете для пула **формулу автоматического масштабирования**. Пакетная служба использует эту формулу для определения целевого количества узлов в пуле для следующего интервала масштабирования (интервал, который вы можете указать).
...
- **Перезапустите узел** ([REST][rest_reboot] | [.NET][net_reboot]).
...
- **Пересоздайте образ узла** ([REST][rest_reimage] | [.NET][net_reimage]).
