...
> [!NOTE]
> При создании учетной записи пакетной службы необходимо выбрать режим по умолчанию — **Пакетная служба**. В нем пулы автоматически выделяются в подписках, управляемых Azure. В альтернативном режиме **Пользовательская подписка**, который больше не рекомендуется, виртуальные машины пакетной службы и другие ресурсы создаются непосредственно в подписке пользователя при создании пула.
>
...
При подготовке пула вычислительных узлов в пакетной службе можно связать его с подсетью [виртуальной сети Azure](../virtual-network/virtual-networks-overview.md). Дополнительные сведения о создании виртуальной сети с подсетями см. в статье [Создание виртуальной сети с несколькими подсетями](../virtual-network/virtual-networks-create-vnet-arm-pportal.md). 

Требования к виртуальной сети:

* Виртуальная сеть должна быть в том же **регионе** и иметь такую же **подписку**, что и учетная запись пакетной службы Azure.

* Для пулов, созданных с помощью конфигурации виртуальной машины, поддерживаются только виртуальные машины на основе Azure Resource Manager (ARM). Для пулов, созданных с конфигурацией облачных служб, поддерживаются и классические виртуальные сети, и виртуальные сети ARM. 

* Для использования сети на основе ARM API клиента пакетной службы должен использовать [проверку подлинности Azure Active Directory](batch-aad-auth.md). Для использования классической виртуальной сети субъекту-службе "MicrosoftAzureBatch" должна быть назначена роль "участника классической виртуальной машины" для указанной виртуальной сети в системе управления доступом на основе ролей (RBAC). 
