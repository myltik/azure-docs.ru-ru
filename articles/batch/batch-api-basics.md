...
При создании пула вычислительных узлов в пакетной службе Azure вы можете с помощью API указать идентификатор [виртуальной сети Azure](../virtual-network/virtual-networks-overview.md), в которой нужно создать вычислительные узлы для пула.
...
* Виртуальная сеть должна иметь достаточно свободных **IP-адресов**, чтобы соответствовать значению свойства `targetDedicated` для пула. Если в подсети недостаточно свободных IP-адресов, пакетная служба частично выделяет вычислительные узлы пула и возвращает ошибку изменения размера.
...
* Если в указанной виртуальной сети есть связанные группы безопасности сети, необходимо включить входящий трафик. Для пулов Linux и Windows необходимо включить порты 29876 и 29877. Можно дополнительно включить (или выборочно фильтровать) порты 22 или 3389 для SSH в пулах Linux или для протокола удаленного рабочего стола в пулах Windows соответственно.
