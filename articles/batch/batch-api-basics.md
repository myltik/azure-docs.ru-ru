...
* **Пакетная служба** — это компонент по умолчанию, в котором виртуальные машины пулов выделяются в фоновом режиме в подписке Azure. Используйте такую конфигурацию учетной записи, если требуются пулы облачных служб. Ее нельзя применять для пулов виртуальных машин, созданных на основе пользовательских образов виртуальных машин или использующих виртуальную сеть. Получить доступ к API пакетной службы можно путем проверки подлинности на основе общего ключа или ознакомившись со статьей [Проверка подлинности в решениях пакетной службы с помощью Active Directory](batch-aad-auth.md). 

* **Пользовательская подписка.** Используйте такую конфигурацию учетной записи, если требуются пулы виртуальных машин, созданные на основе пользовательских образов виртуальных машин или использующие виртуальную сеть. Получить доступ к API пакетной службы можно только путем [проверки подлинности Active Directory](batch-aad-auth.md). Пулы облачных служб не поддерживаются. Вычислительные виртуальные машины пакетной службы выделяются непосредственно в подписке Azure. Если вы используете этот режим, необходимо настроить Azure Key Vault для учетной записи пакетной службы.
