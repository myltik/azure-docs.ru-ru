...
Учетная запись Пакетной службы — это уникально идентифицируемая сущность в Пакетной службе. Вся обработка данных привязана к учетной записи пакетной службы. Для выполнения операций пакетной службы нужно имя и ключ учетной записи. Чтобы создать учетную запись пакетной службы, см. статью [Создание учетной записи пакетной службы на портале предварительной версии Azure и управление ею](batch-account-create-portal.md).
...
Функция [автоматического масштабирования](batch-automatic-scaling.md) позволяет приложению легко увеличивать или уменьшать вычислительную мощность по мере изменения потребностей. Вы можете динамически менять количество узлов в пуле в зависимости от текущей рабочей нагрузки и статистики использования ресурсов. Это снижает суммарную стоимость работы приложения, поскольку выделяется только необходимое количество ресурсов. Параметры масштабирования можно указать при создании пула, а впоследствии изменить в любое время.

Следует уделить особое внимание выполнению текущих задач при автоматическом уменьшении количества узлов. Вы можете определить политику отмены выделения: запущенные задачи могут быть остановлены для немедленного удаления узла или оставлены для планового завершения работы с последующим удалением узлов. Чтобы обеспечить максимальную эффективность использования ресурсов, укажите нулевое целевое количество узлов на момент завершения задания, но позвольте текущим задачам завершиться нормально.

Автоматическое масштабирование приложения выполняется по набору формул масштабирования. Эти формулы определяют целевое количество узлов в пуле на следующий интервал масштабирования. Рассмотрим для примера задание, которое требует планирования большого количества задач. Вы можете назначить для пула формулу масштабирования, которая изменяет размер пула (количество узлов пула) в зависимости от текущего числа задач в очереди и скорости выполнения этих задач. Пакетная служба периодически вычисляет и изменяет размер пула по этой формуле в зависимости от объема рабочей нагрузки.
...
> [AZURE.TIP]Такая возможность нужна нечасто, но вы можете даже указать конкретные узлы для удаления из пула. Например, можно удалить из пула узел, надежность которого вызывает подозрения, чтобы не назначать ему дополнительные задачи.
...
Каждому узлу в пуле присваивается уникальный идентификатор, а узел, на котором выполняется задача, включается в метаданные задачи. Клиентское приложение пакетной службы может отследить ситуацию, когда несколько задач на определенном узле завершаются ошибкой, отметить такой узел как подозрительный и удалить его из пула. Если при удалении узла на нем еще выполняются какие-либо задачи, они будут автоматически помещены в очередь для повторного выполнения на других узлах.
