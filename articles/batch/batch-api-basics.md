...
Зависимости задач, как можно понять из названия, позволяют настроить выполнение задачи в зависимости от предварительного завершения одной или нескольких других задач. "Подчиненная" задача может использовать выходные данные "вышестоящей" задачи; также она может зависеть от определенной инициализации, выполняемой вышестоящей задачей. В этом случае можно указать, что задание использует зависимости задач. Затем для каждой задачи, зависящей от другой (или многих других), следует указать задачи, от которых она зависит.
...
Использование [пакетов приложений](batch-application-packages.md) обеспечивает простое управление и развертывание приложений на вычислительных узлах в пулах. С помощью этой функции вы можете легко отправлять несколько версий приложений, выполняемых задачами, включая двоичные файлы и файлы поддержки (а также управлять ими), а затем автоматически развертывать одно или несколько таких приложений на вычислительных узлах в пуле.
...
Дополнительные сведения о пакетах приложений см. в статье, посвященной [развертыванию приложений с помощью пакетов приложений пакетной службы Azure](batch-application-packages.md).
