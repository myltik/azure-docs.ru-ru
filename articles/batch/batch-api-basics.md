...
Пакетная служба поддерживает две конфигурации учетной записи, и вам необходимо будет выбрать соответствующую конфигурацию при создании учетной записи пакетной службы. Разница между двумя конфигурациями учетных записей пакетной службы в том, как выделяются [пулы](#pool) пакетной службы для учетной записи. Вы можете выделить пулы вычислительных узлов в подписке, управляемой Azure, или же в собственной подписке. Свойство *Режим распределения пула* учетной записи определяет, какую конфигурацию она использует. 

Чтобы решить, какую конфигурацию учетной записи следует использовать, определите, какая из них лучше всего соответствует вашему сценарию:

* **Пакетная служба.** Пакетная служба — это конфигурация по умолчанию учетной записи. Для учетной записи, созданной в этой конфигурации, пулы пакетной службы выделяются в фоновом режиме в подписках, управляемых в Azure. Необходимо учитывать следующие основные моменты в отношении конфигурации учетной записи пакетной службы:

    - Конфигурация учетной записи пакетной службы поддерживает как пулы облачной службы, так и пулы виртуальных машин.
    - Конфигурация учетной записи пакетной службы поддерживает доступ к API пакетной службы с помощью проверки подлинности на основе общего ключа или [проверки подлинности Azure Active Directory](batch-aad-auth.md). 
    - Вы можете использовать выделенные или низкоприоритетные вычислительные узлы в пулах конфигурации учетной записи пакетной службы.
    - Если вы планируете создавать пулы виртуальных машин Azure из пользовательских образов виртуальных машин или использовать виртуальную сеть, не используйте конфигурацию учетной записи пакетной службы. Вместо этого создайте свою учетную запись с конфигурацией подписки пользователя.
    - Пулы виртуальных машин, подготовленные в учетной записи с конфигурацией подписки пакетной службы, должны быть созданы из образов [магазина виртуальных машин Azure][vm_marketplace].

* **Подписка пользователя.** При использовании конфигурации учетной записи подписки пользователя пулы пакетной службы выделяются в подписке Azure, где создана учетная запись. Необходимо учитывать следующие основные моменты в отношении конфигурации учетной записи подписки пользователя:
     
    - Конфигурация учетной записи подписки пользователя поддерживает только пулы виртуальных машин. Она не поддерживает пулы облачных служб.
    - Чтобы создать пулы виртуальных машин из пользовательских образов виртуальных машин или использовать с ними виртуальную сеть, следует использовать конфигурацию подписки пользователя.  
    - Необходимо проверять подлинность запросов к пакетной службе с помощью [проверки подлинности Azure Active Directory](batch-aad-auth.md). 
    - Если вы используете конфигурацию учетной записи подписки пользователя, необходимо настроить хранилище ключей Azure для вашей учетной записи пакетной службы. 
    - Вы можете использовать только выделенные вычислительные узлы в пулах в учетных записях с конфигурацией подписки пользователя. Низкоприоритетные узлы не поддерживаются.
    - Пулы виртуальной машины, подготовленные в учетной записи с конфигурацией подписки пользователя, могут быть созданы из пользовательских образов, предоставленных вами, или из образов [магазина виртуальных машин Azure][vm_marketplace].

> [!IMPORTANT]
> Сейчас пакетная служба поддерживает только учетные записи хранения общего назначения, как описано в шаге 5 раздела о [создании учетной записи хранения](../storage/storage-create-storage-account.md#create-a-storage-account) в статье [Об учетных записях хранения Azure](../storage/storage-create-storage-account.md). В задачах пакетной службы (включая стандартные задачи, задачи запуска, задачи подготовки и прекращения заданий) необходимо указывать файлы ресурсов, которые находятся в учетных записях хранения общего назначения.
>
>
...
Чтобы использовать пользовательские образы для пулов виртуальных машин, создайте учетную запись пакетной службы с конфигурацией подписки пользователя. В такой конфигурации пулы пакетной службы выделяются в подписку, в которой находится учетная запись. Дополнительные сведения о настройке режима выделения пула при создании учетной записи пакетной службы см. в разделе [Учетная запись](#account).
...
- В настоящее время поддерживаются только стандартные учетные записи хранения. В дальнейшем будет поддерживаться хранилище Azure класса Premium.
...
>
> 1. Можно применить пакеты для распределения приложений или данных в каждом узле пула пакетной службы. Дополнительные сведения о приложениях и пакетах приложений см. в статье [Развертывание приложений на вычислительных узлах с помощью пакетов приложений пакетной службы](batch-application-packages.md).
...
## Конфигурация сети пула

При создании пула вычислительных узлов в пакетной службе Azure вы можете указать идентификатор подсети [виртуальной сети Azure](../virtual-network/virtual-networks-overview.md), в которой нужно создать вычислительные узлы для пула.

* Виртуальная сеть должна:
...
* Тип поддерживаемой виртуальной сети зависит от способа распределения пулов в учетной записи пакетной службы.
    - Если при создании учетной записи пакетной службы для свойства **poolAllocationMode** задано значение BatchService, следует указать классическую виртуальную сеть.
    - Если при создании учетной записи пакетной службы для свойства **poolAllocationMode** задано значение UserSubscription, можно указать классическую виртуальную сеть или виртуальную сеть Azure Resource Manager. Чтобы пулы могли использовать виртуальную сеть, они должны создаваться с помощью конфигурации виртуальной машины. Пулы, созданные с помощью конфигурации облачной службы, не поддерживаются.

* Если при создании учетной записи пакетной службы для свойства **poolAllocationMode** указано значение BatchService, субъекту-службе пакетной службы необходимо предоставить разрешения для доступа к виртуальной сети. Субъекту-службе пакетной службы, которому присвоено имя Microsoft Azure Batch или MicrosoftAzureBatch, должна быть назначена роль [участника классической виртуальной машины](https://azure.microsoft.com/documentation/articles/role-based-access-built-in-roles/#classic-virtual-machine-contributor) для указанной виртуальной сети в системе управления доступом на основе ролей (RBAC). Если такая роль RBAC не назначена, пакетная служба возвращает ошибку 400 (неправильный запрос).
...
* Если у виртуальной сети есть связанные группы безопасности сети (NSG), необходимо включить несколько зарезервированных системных портов для входящего трафика. Для пулов, созданных с помощью конфигурации виртуальной машины, включите порты 29876 и 29877, а также порт 22 для Linux и порт 3389 для Windows. Для пулов, созданных с помощью конфигурации облачной службы, включите порты 10100, 20100 и 30100. Кроме того, включите исходящие подключения к службе хранилища Azure через порт 443.
...
Дополнительные параметры виртуальной сети зависят от режима выделения пула в учетной записи пакетной службы.

<a id="vnets-for-pools-provisioned-in-the-batch-service" class="xliff"></a>

### Виртуальные сети для пулов, подготовленных в пакетной службе

В режиме выделения пакетной службы виртуальной сети можно назначать только пулы с **конфигурацией облачных служб**. Кроме того, указанная виртуальная сеть должна быть создана с помощью **классической** модели развертывания. Виртуальные сети, созданные по модели развертывания с помощью Azure Resource Manager, не поддерживаются.



* Субъекту-службе *MicrosoftAzureBatch* должна быть назначена роль [участника классической виртуальной машины](../active-directory/role-based-access-built-in-roles.md#classic-virtual-machine-contributor) для указанной виртуальной сети в системе управления доступом на основе ролей (RBAC). На портале Azure выполните следующие действия.

  * Выберите **виртуальную сеть**, затем последовательно щелкните **Управление доступом (IAM)** > **Роли** > **Участник классических виртуальных машин** > **Добавить**.
  * Введите "MicrosoftAzureBatch" в поле **поиска**.
  * Установите флажок **MicrosoftAzureBatch**.
...
### Виртуальные сети для пулов, подготовленных в пользовательской подписке

В режиме пользовательской подписки поддерживаются только пулы с **конфигурацией виртуальной машины**, и только такие пулы можно назначать виртуальной сети. Кроме того, указанная виртуальная сеть должна быть развернуты с помощью модели **Resource Manager**. Виртуальные сети, созданные с помощью классической модели развертывания, не поддерживаются.
...
### Устранение неполадок проблемных вычислительных узлов
