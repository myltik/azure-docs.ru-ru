...
> [AZURE.TIP]
 Такая возможность нужна нечасто, но вы можете даже указать конкретные узлы для удаления из пула. Например, можно удалить из пула узел, надежность которого вызывает подозрения, чтобы не назначать ему дополнительные задачи.
