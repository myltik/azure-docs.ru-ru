---
title: "Выполнение рабочих нагрузок пакетной службы Azure на экономичных низкоприоритетных виртуальных машинах (предварительная версия) | Документация Майкрософт"
...
Более подробное обсуждение низкоприоритетных виртуальных машин см. в записи блога [Batch computing at a fraction of the price](https://azure.microsoft.com/blog/announcing-public-preview-of-azure-batch-low-priority-vms/) (Пакетная обработка данных при относительно небольших затратах).
...
-   Иногда виртуальные машины могут быть недоступны или замещаться, что приведет к сокращению емкости для заданий и может привести к прерыванию и повторному запуску задач. Таким образом, во время выполнения задания должны быть гибкими.

-   Прерывание заданий с более продолжительными задачами может привести к более серьезным последствиям. Если реализовать контрольные точки в длительных задачах для сохранения при выполнении, то последствия прерывания будут менее серьезными. Менее продолжительные задачи, как правило, лучше всего выполняются на виртуальных машинах с низким приоритетом, так как результаты их прерывания не столь серьезные.

-   Длительные задания MPI, использующие несколько виртуальных машин, невыгодно выполнять на низкоприоритетных виртуальных машинах, так как из-за замещения одной виртуальной машины, скорее всего, придется запускать задание заново.
...
-   **Дополнение емкости по запросу.** Виртуальные машины с низким приоритетом можно использовать для дополнения обычных выделенных виртуальных машин. Если они доступны, задания можно масштабировать, ускорив их и сократив расходы, если нет — доступны базовые выделенные виртуальные машины.

-   **Гибкое время выполнения задания.** Если время выполнения задания можно скорректировать, допускается возможное сокращение емкости. Тем не менее при добавлении виртуальных машин с низким приоритетом задания будут выполняться быстрее за более низкую цену.
...
-   Виртуальные машины с низким приоритетом можно использовать только в пуле. При этом пакетная служба будет просто восстанавливать замещенные ресурсы, когда они станут доступными. Это самый экономичный способ выполнения заданий, так как используются только низкоприоритетные виртуальные машины.
...
-   Пулы пакетной службы могут содержать выделенные и низкоприоритетные виртуальные машины. Число виртуальных машин любого типа можно указать при создании пула или изменить в любое время для имеющегося пула, используя операцию явного изменения размера или автоматическое масштабирование. Отправка заданий и задач не изменяется. На нее не влияет тип виртуальной машины в пуле. Вы можете сделать так, чтобы пул в основном использовал виртуальные машины с низким приоритетом для выполнения заданий как можно дешевле и развертывал выделенные виртуальные машины, если значение емкости падает ниже заданного порогового значения, чтобы выполнение заданий продолжалось.

-   Пулы пакетной службы автоматически переходят на целевое количество виртуальных машин с низким приоритетом. Если виртуальные машины замещаются, пакетная служба попытается заменить потерянную емкость и вернуться к целевому показателю.

-   При прерывании задачи пакетная служба обнаруживает и автоматически повторно ставит в очередь задачи для повторного выполнения.

-   Низкоприоритетные виртуальные машины имеют квоту ядер, которая отличается от квоты выделенных виртуальных машин. 
    Квота для низкоприоритетных виртуальных машин выше, чем для выделенных виртуальных машин, так как они меньше стоят. Дополнительные сведения см. в статье [Квоты и ограничения пакетной службы](batch-quota-limit.md#resource-quotas).    

> [!NOTE]
> Сейчас виртуальные машины с низким приоритетом не поддерживаются для учетных записей пакетной службы, где в качестве режима распределения пула используется [подписка пользователя](batch-account-create-portal.md#user-subscription-mode).
>
>
...
После замещения одного или нескольких узлов в пуле операция выведения списка узлов в пуле будет по-прежнему возвращать эти узлы, текущее число узлов с низким приоритетом не изменится, но их состояние изменится на **Отложено**. Пакетная служба попытается найти виртуальные машины на замену. Если она найдет их, состояние узлов сменится на **Создание** и **Запуск** перед тем, как они станут доступными для выполнения задачи, как и новые узлы.

## <a name="scale-a-pool-containing-low-priority-vms"></a>Масштабирование пула низкоприоритетных виртуальных машин

Как и пулы, состоящие исключительно из выделенных виртуальных машинах, можно масштабировать пулы низкоприоритетных виртуальных машин, вызвав метод Resize или использовав автомасштабирование.
...
Заданиям и задачам почти не требуется поддержка узлов с низким приоритетом. Они поддерживают следующее:

-   Свойство JobManagerTask задания имеет новое свойство **AllowLowPriorityNode**. 
    Если это свойство имеет значение true, задачу диспетчера заданий можно запланировать на выделенном или низкоприоритетном узле. Если это свойство имеет значение false, задача диспетчера заданий будет запланирована только на выделенном узле.
