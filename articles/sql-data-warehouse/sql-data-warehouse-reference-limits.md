<properties
   pageTitle="Возможности хранилища данных SQL | Microsoft Azure"
   description="Минимальные и максимальные возможности хранилища данных SQL."
...
# Возможности хранилища данных SQL

Для хранилища данных SQL существуют ограничения на обработку запросов и системные представления, аналогичные ограничениям для SQL Server. Но есть и некоторые отличия, которые обусловлены особенностями используемой масштабируемой параллельной архитектуры. Ниже приведено краткое описание ограничений для хранилища данных SQL.
...
| Количество входов в систему | Количество входов на сервер | 500 000 |
...
| Таблица | Количество байт в строке (размер внутреннего буфера для перемещаемых данных) | 32 768<br/><br/>Хранилище данных SQL использует внутренний буфер для перемещения строк в распределенной системе хранилища данных SQL. Служба, которая перемещает строки, называется службой перемещения данных (DMS). Она хранит строки в формате, отличном от формата, который используется в SQL Server.<br/><br/>Если строка не помещается во внутренний буфер, возникнет ошибка компиляции запроса или ошибка перемещения внутренних данных. Решение этой проблемы см. в статье [Details about the DMS buffer size](#details-about-the-dms-buffer-size) (Сведения о размере буфера DMS).|
