...
Поэтому, начиная разработку хранилища данных SQL, удобно будет следовать такой схеме:
- создание статистики по отдельным столбцам для каждого столбца в каждой таблице;
- создание статистики по нескольким столбцам для столбцов, используемых в запросах при соединении, и группирование по предложениям.
...
На этот вопрос нельзя ответить, исходя только из возраста данных. Актуальный объект статистики может быть очень старым, если исходные данные существенно не менялись. Когда значительно изменяется количество строк или распределение значений для заданного столбца, вот *тогда* необходимо обновить статистику.

Для справки **SQL Server** (не хранилище данных SQL) автоматически обновляет статистику в следующих случаях:

- Если в таблице есть нулевые строки, при добавлении новой строки (или строк) статистика обновится автоматически.
