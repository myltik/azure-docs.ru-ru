...
Изменение размера секций важен для хранилища данных SQL. Обычно операции управления данными и процедуры загрузки данных нацелены на отдельные секции, а не обрабатывают сразу всю таблицу. Это особенно важно для кластеризованных индексов columnstore (CCI). CCI могут потреблять значительные объемы памяти. Поэтому, даже если нам необходимо исправить степень детализации плана секционирования, мы не хотим изменить секции до размера, приводящего к нехватке памяти при попытке выполнить задачи управления.

При выборе степени детализации секций важно помнить, что хранилище данных SQL автоматически распределяет данные. Следовательно, данные, которые обычно находились бы в одной таблице и одной секции в базе данных SQL Server, теперь существует в одной секции во многих таблицах в базе данных хранилища данных SQL. Для поддержания разумного количества строк в каждой секции обычно изменяется размер границы секции. Например, если вы использовали для хранилища данных секционирование за день, может понадобиться рассмотреть что-нибудь менее детальное, например месяц или квартал.
