...
Настроив все компоненты, мы можем начать настройку конвейера. Для этого нужно перейти к вашему экземпляру фабрики данных Azure на портале Azure. Чтобы сделать это, щелкните элемент [Классический портал Azure](portal.azure.com) и выберите фабрику данных в меню слева.
