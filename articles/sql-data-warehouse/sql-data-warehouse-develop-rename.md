...
Важно помнить, что используемую таблицу переименовать нельзя. Для переименования таблицы требуется ее монопольная блокировка. Если таблица используется, сеанс с использованием этой таблицы необходимо завершить. Для завершения сеанса используйте команду [KILL][]. Применяйте команду `KILL` с осторожностью, так как после завершения целевого сеанса все несохраненные изменения будут отменены. К сеансам в хранилище данных SQL добавляет префикс SID. При вызове команды KILL укажите этот префикс и номер сессии. Например, `KILL 'SID1234'`. Ознакомьтесь со статьей о подключениях, чтобы больше узнать о [сеансах].
