...
CREATE TABLE AS SELECT или `CTAS` — одна из самых важных функций T-SQL. Это полностью распараллеленная операция, которая создает новую таблицу на основе выходных данных оператора SELECT. `CTAS` — это самый простой и быстрый способ создания копии таблицы. Если хотите, считайте ее расширенной версией оператора `SELECT..INTO`. Этот документ содержит примеры и рекомендации для `CTAS`.

## Копирование таблицы с помощью CTAS
...
Предположим, что вы создали эту таблицу с использованием типа распределения по умолчанию `ROUND_ROBIN`, поскольку в операторе `CREATE TABLE` не был указан столбец распространения.
...
> [AZURE.NOTE] Хранилище данных SQL Azure пока не поддерживает автоматическое создание или автоматическое обновление статистики. Чтобы добиться максимально высокой производительности запросов, крайне важно сформировать статистические данные для всех столбцов всех таблиц после первой загрузки или после любых значительных изменений в данных. Подробные сведения о работе со статистикой см. в разделе [Статистика][] из группы разделов по разработке.

## Обход неподдерживаемых функций с помощью CTAS
...
> [AZURE.NOTE] Старайтесь всегда начинать с функции CTAS. Если вы считаете, что решить проблему с помощью `CTAS` можно, то, скорее всего, этот вариант окажется лучшим, даже если в результате вам придется записывать больше данных.
>

## SELECT..INTO
Оператор `SELECT..INTO` может встречаться в разных местах вашего решения.

Вот пример оператора `SELECT..INTO`.
...
Его довольно просто преобразовать в `CTAS`:
...
> [AZURE.NOTE] В настоящее время CTAS требует указания столбца распределения. Если вы не пытаетесь намеренно изменить столбец распределения, `CTAS` будет выполняться максимально быстро при выборе столбца распределения, который совпадает с базовой таблицей, так как такая стратегия позволяет избежать перемещения данных. Если вы создаете небольшую таблицу, для которой производительность не так важна, можно указать `ROUND_ROBIN`, чтобы выбирать столбец распределения не требовалось.
...
Поскольку хранилище данных SQL не поддерживает соединения ANSI в предложении `FROM` оператора `UPDATE`, вы не сможете скопировать этот код, не подвергнув его небольшой корректировке.
...
> [AZURE.NOTE] Для правильного задания допустимости значений NULL важно использовать `ISNULL` и не использовать `COALESCE`. `COALESCE` не является детерминированной функцией, и поэтому результат выражения всегда будет допускать значения NULL. `ISNULL` работает по-другому. Он детерминирован, поэтому если вторая часть функции `ISNULL` является константой или литералом, итоговым значением будет NOT NULL.
