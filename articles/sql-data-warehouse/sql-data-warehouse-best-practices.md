---
title: Рекомендации по использованию хранилища данных SQL Azure | Microsoft Docs
...
Дополнительные сведения см. в разделах о [приостановке][приостановке] и [возобновлении][возобновлении] использования вычислительных ресурсов, а также об их [масштабировании][масштабировании].
...
Дополнительные сведения см. в статьях [Транзакции в хранилище данных SQL][Транзакции в хранилище данных SQL] и [Оптимизация транзакций для хранилища данных SQL][Оптимизация транзакций для хранилища данных SQL].
...
Дополнительные сведения см. в статьях [Manage table statistics][Manage table statistics] \(Управление статистикой таблиц), [CREATE STATISTICS (Transact-SQL)][CREATE STATISTICS (Transact-SQL)] и [UPDATE STATISTICS (Transact-SQL)][UPDATE STATISTICS (Transact-SQL)].
...
Дополнительные сведения см. в статье [Инструкция INSERT (Transact-SQL)][Инструкция INSERT (Transact-SQL)].
...
Дополнительные сведения см. в статьях [Загрузка данных в хранилище данных SQL][Загрузка данных в хранилище данных SQL], [Руководство по использованию PolyBase в хранилище данных SQL][Руководство по использованию PolyBase в хранилище данных SQL], [Azure SQL Data Warehouse loading patterns and strategies][Azure SQL Data Warehouse loading patterns and strategies] \(Стратегии и шаблоны загрузки данных в хранилище данных SQL Azure), [Загрузка данных с помощью фабрики данных Azure][Загрузка данных с помощью фабрики данных Azure], [Перемещение данных в хранилище данных SQL Azure и из него с помощью фабрики данных Azure][Перемещение данных в хранилище данных SQL Azure и из него с помощью фабрики данных Azure], [CREATE EXTERNAL FILE FORMAT (Transact-SQL)][CREATE EXTERNAL FILE FORMAT (Transact-SQL)], [Функция Create Table As Select (CTAS) в хранилище данных SQL][Функция Create Table As Select (CTAS) в хранилище данных SQL].
...
Ознакомьтесь также с [руководством по использованию PolyBase][руководством по использованию PolyBase].

## Хэш-распределение больших таблиц
По умолчанию таблицы распределяются по методу циклического перебора. Это позволяет упростить процесс создания таблиц, так как пользователям не нужно принимать решение о типе распределения. Таблицы с распределением по методу циклического перебора могут вполне годиться для некоторых рабочих нагрузок, но в большинстве случаев намного эффективнее использовать столбец распределения. Наглядно это превосходство можно увидеть при объединении больших таблиц фактов. Например, во время выполнения запроса на объединение таблицы Orders, распределенной по идентификатору order\_id, с таблицей Transactions, распределенной по тому же идентификатору, этот запрос превращается в запрос к серверу, что исключает выполнение операций перемещения данных. Чем меньше в запросе действий, тем быстрее он выполняется. Скорость выполнения запроса также зависит от объема перемещаемых данных. В этом разделе представлены только общие сведения. При загрузке распределенной таблицы входящие данные не должны быть отсортированы по ключу распределения, так как это замедлит процесс загрузки. Ниже приведены ссылки на статьи, содержащие сведения о том, как с помощью столбца распределения можно улучшить производительность и как определить распределенную таблицу в предложение WITH инструкции CREATE TABLES.

Дополнительные сведения см. в статьях [Общие сведения о таблицах в хранилище данных SQL][Общие сведения о таблицах в хранилище данных SQL] и [Распределение таблиц в хранилище данных SQL][Распределение таблиц в хранилище данных SQL], а также в статьях о [выборе распределения таблицы][выборе распределения таблицы], инструкциях [CREATE TABLE][CREATE TABLE] и [CREATE TABLE AS SELECT][CREATE TABLE AS SELECT].
...
Дополнительные сведения см. в статье о [секционировании таблиц][секционировании таблиц].
...
Дополнительные сведения см. в статьях [Транзакции в хранилище данных SQL][Транзакции в хранилище данных SQL] и [Оптимизация транзакций для хранилища данных SQL][Оптимизация транзакций для хранилища данных SQL], а также в статьях о [секционировании таблиц][секционировании таблиц], инструкции [TRUNCATE TABLE][TRUNCATE TABLE], [ALTER TABLE][ALTER TABLE] и [функции Create Table As Select (CTAS)][функции Create Table As Select (CTAS)].
...
Дополнительные сведения см. в статьях, посвященных [общим сведениям о таблицах][общим сведениям о таблицах], [типам данных таблиц][типам данных таблиц] и инструкции [CREATE TABLE][CREATE TABLE].
...
Ознакомьтесь также с [индексированием таблиц][индексированием таблиц], [руководством по индексам columnstore][руководством по индексам columnstore] и [перестроением индексов columnstore][перестроением индексов columnstore].
...
Дополнительные сведения см. в статьях [Управление параллелизмом и рабочей нагрузкой в хранилище данных SQL][Управление параллелизмом и рабочей нагрузкой в хранилище данных SQL] и [sys.dm\_pdw\_waits (Transact-SQL)][sys.dm\_pdw\_waits (Transact-SQL)].
...
[Общие сведения о таблицах в хранилище данных SQL]: ./sql-data-warehouse-tables-overview.md
...
[Распределение таблиц в хранилище данных SQL]: ./sql-data-warehouse-tables-distribute.md
[Индексирование таблиц в хранилище данных SQL]: ./sql-data-warehouse-tables-index.md
...
[выборе распределения таблицы]: https://blogs.msdn.microsoft.com/sqlcat/2015/08/11/choosing-hash-distributed-table-vs-round-robin-distributed-table-in-azure-sql-dw-service/
