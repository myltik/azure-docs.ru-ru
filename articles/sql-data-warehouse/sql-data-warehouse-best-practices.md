...
# Рекомендации по использованию хранилища данных SQL
...
Дополнительные сведения см. в статьях [Управление статистикой в хранилище данных SQL][], [CREATE STATISTICS (Transact-SQL)][] \(Инструкция CREATE STATISTICS (Transact-SQL)), [UPDATE STATISTICS (Transact-SQL)][] \(Инструкция UPDATE STATISTICS (Transact-SQL)).
...
Дополнительные сведения см. в статье [Insert (Transact-SQL)][] \(Инструкция INSERT (Transact-SQL)).
...
Дополнительные сведения см. в статьях [Хэш-распределение и его влияние на скорость выполнения запросов в хранилище данных SQL][], [Choosing hash distributed table vs. round-robin distributed table][] \(Выбор хэш-распределенных таблиц и таблиц, распределенных по методу циклического перебора), [CREATE TABLE (Azure SQL Data Warehouse, Parallel Data Warehouse)][] \(Инструкция CREATE TABLE (хранилище данных SQL Azure, Parallel Data Warehouse)), [CREATE TABLE AS SELECT (Azure SQL Data Warehouse)][] \(Инструкция CREATE TABLE AS SELECT (хранилище данных SQL Azure)).

## Избыточное секционирование
...
Дополнительные сведения см. в статье [Секции таблиц в хранилище данных SQL][].
...
Дополнительные сведения см. в статьях [Транзакции в хранилище данных SQL][], [Оптимизация транзакций для хранилища данных SQL][], [Секции таблиц в хранилище данных SQL][], [TRUNCATE TABLE (Transact-SQL)][] \(Инструкция TRUNCATE TABLE (Transact-SQL)), [ALTER TABLE (Transact-SQL)][] \(Инструкция ALTER TABLE (Transact-SQL)), [Функция Create Table As Select (CTAS) в хранилище данных SQL][].
...
Дополнительные сведения см. в статье [CREATE TABLE (Azure SQL Data Warehouse, Parallel Data Warehouse)][] \(Инструкция CREATE TABLE (хранилище данных SQL Azure, Parallel Data Warehouse)).
...
Дополнительные сведения см. в статьях [Устранение неполадок][], [Управление индексами columnstore в хранилище данных SQL Azure][] и [Columnstore Indexes Guide][] \(Руководство по индексам Columnstore).
...
Дополнительные сведения см. в статьях [Управление параллелизмом и рабочей нагрузкой в хранилище данных SQL][], [sys.dm\_pdw\_waits (Transact-SQL)][].
...
## Другие ресурсы
Существует множество ресурсов, где можно просмотреть сведения об использовании хранилища данных SQL Azure. Эта статья является частью документации по Azure, и в ней содержатся ссылки на другие статьи Azure и MSDN. Мы просматриваем все отзывы и периодически обновляем сведения в статьях. Если статья была полезной, ответьте на вопрос "Страница была полезной?" После нажатия кнопки "Да" или "Нет" можно оставить комментарии. Если статья была полезной и вы хотите оставить отзыв, нажмите кнопку "Да" и оставьте ваши комментарии. Это поможет нам усовершенствовать статью. Если вопрос не появился, его можно найти в конце каждой статье Azure. Кроме того, можно также перейти по ссылке "Есть предложения?" в правом верхнем углу каждой страницы MSDN. Ваши отзывы очень важны для нас, и мы всегда их учитываем.

Если у вас есть **предложения по улучшению функций** хранилища данных SQL Azure, опубликуйте их на странице [отзывов о хранилище данных SQL Azure][]. Ваши отзывы и голоса за отзывы, оставленные другими пользователями, помогут нам определить, какие улучшения функций наиболее приоритетные.

На [форуме хранилища данных SQL Azure на портале MSDN][] можно задать вопросы другим пользователям и разработчикам хранилища данных SQL. Мы регулярно просматриваем этот форум и следим за тем, чтобы другие пользователи или наши специалисты ответили на интересующий вас вопрос. Вопросы также можно задавать на [форуме хранилища данных SQL Azure на сайте Stack Overflow][].
...
[Хэш-распределение и его влияние на скорость выполнения запросов в хранилище данных SQL]: sql-data-warehouse-develop-hash-distribution-key.md
...
[масштабировании]: sql-data-warehouse-overview-scalability.md#scale-performance-bk
[Table design in SQL Data Warehouse]: sql-data-warehouse-develop-table-design.md
[Секции таблиц в хранилище данных SQL]: sql-data-warehouse-develop-table-partitions.md
[Временные таблицы в хранилище данных SQL]: sql-data-warehouse-develop-temporary-tables.md
