...
Запросы в состоянии **Приостановлено** поставлены в очередь из-за ограничений параллелизма. Эти запросы также отображаются в результатах запроса Waits к sys.dm_pdw_waits с типом UserConcurrencyResourceType. Дополнительные сведения об ограничениях параллелизма см. в статье [Управление параллелизмом и рабочей нагрузкой][Управление параллелизмом и рабочей нагрузкой]. Запросы также могут быть отложены по другим причинам, в том числе из-за блокировки объектов.  Если запрос ожидает ресурс, ознакомьтесь с разделом [Исследование запросов, ожидающих ресурсы][Исследование запросов, ожидающих ресурсы] далее в этой статье.
...
Если запрос выполняется, то можно использовать [DBCC PDW_SHOWEXECUTIONPLAN][DBCC PDW_SHOWEXECUTIONPLAN], чтобы получить из кэша планов SQL Server предполагаемый план для выполняемого шага SQL в рамках определенного распределения.
...
Дополнительные сведения о динамических административных представлениях см. в статье [Системные представления][Системные представления].
