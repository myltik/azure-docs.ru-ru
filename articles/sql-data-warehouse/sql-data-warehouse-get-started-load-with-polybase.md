...
- как создавать объекты базы данных для определения внешних данных;
...
1. Выполните следующую команду, чтобы передать файл: Укажите URL-адрес конечной точки службы BLOB-объектов для <blob service endpoint URL> и ключ учетной записи хранилища Azure для <azure_storage_account_key>.
...
PolyBase использует внешние таблицы для доступа к данным в хранилище BLOB-объектов или в Hadoop. Поскольку данные хранятся не в хранилище данных SQL, PolyBase выполняет проверку подлинности для доступа к внешним данным с помощью учетных данных, заданных для базы данных.
...
Этот пример создает статистику по отдельным столбцам для новой таблицы DimDate2External.
