...
Во время выполнения инструкций TSQL запишите параметры MAXSIZE и SERVICE_OBJECTIVE. Они определяют размер исходного хранилища и вычисляют экземпляр, предназначенный для вашего хранилища данных. Параметр MAXSIZE принимает следующие значения (рекомендуется выбрать большой размер, чтобы оставить место для роста базы данных): 250 ГБ, 500 ГБ, 750 ГБ, 1024 ГБ, 5120 ГБ, 10 240 ГБ, 20 480 ГБ, 30 720 ГБ, 40 960 ГБ, 51 200 ГБ.

Значение SERVICE_OBJECTIVE определяет количество DWU, с которым начнет работать экземпляр, и принимает следующие значения: DW100, DW200, DW300, DW400, DW500, DW600, DW1000, DW1200, DW1500, DW2000. Сведения о влиянии этих параметров на стоимость см. на нашей [странице цен][].
