...
1. Представляет собой статическое значение, так как обновить столбец хэша невозможно.
2. используется в запросах в предложениях `JOIN`, `GROUP BY`, `DISTINCT` или `HAVING`;
