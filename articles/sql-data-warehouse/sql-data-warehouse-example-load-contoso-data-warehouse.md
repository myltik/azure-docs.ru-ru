<properties
   pageTitle="Загрузка данных из хранилища данных розничной торговли Contoso в хранилище данных SQL | Microsoft Azure"
   description="Узнайте, как загрузить данные из хранилища данных розничной торговли Contoso в хранилище данных SQL с помощью PolyBase для обеспечения максимальной производительности"
...
# Загрузка хранилища данных розничной торговли Contoso

Из этой статьи вы узнаете, как быстро загрузить данные в схему хранилища данных розничной торговли Contoso.

Этот набор данных предоставляет альтернативный источник данных, которым можно пользоваться для изучения хранилища данных SQL Azure.

Размер набора данных Contoso составляет около 1 ГБ. Необработанные данные размещаются в хранилище BLOB-объектов Azure. Для загрузки данных в хранилище данных SQL мы будем обращаться напрямую к хранилищу BLOB-объектов Azure.

Изучив данный учебник, вы научитесь:

1. Настраивать PolyBase для загрузки данных из общедоступного контейнера хранилища BLOB-объектов.
2. Импортировать данные с помощью [CTAS][].
3. Перестраивать ([REBUILD][]) таблицы для оптимизации индексов columnstore.
4. Создавать статистику по ключевым столбцам.

## Настройка PolyBase

Для загрузки данных сначала необходимо настроить скрипты PolyBase.

### Создание внешнего источника данных

Так как данные хранятся в общедоступном контейнере, сначала необходимо создать внешний источник данных:
...
Для этого не требуется создавать главный ключ или учетные данные на уровне базы данных, поскольку контейнер хранилища BLOB-объектов является общедоступным и любой пользователь может считывать и циклически обрабатывать объекты в этом контейнере.

> [AZURE.IMPORTANT] Если вы решили сделать контейнеры хранилища BLOB-объектов Azure общедоступными, не забывайте, что за передачу данных из центра обработки данных будет взиматься плата.

### Создание формата внешнего файла

Формат файла предоставляет сведения, необходимые для анализа текстовых файлов, размещенных в хранилище BLOB-объектов.
...
В нашем случае используются данные без сжатия с разделением вертикальной чертой.

## Создание внешних таблиц

Теперь после настройки подключения к хранилищу BLOB-объектов Azure можно создать внешние таблицы.

Давайте будем действовать по порядку и сначала создадим схему:
...
Теперь создадим внешние таблицы. Помните, что здесь мы просто создаем сопоставление для типов данных. Привязка этого определения к метаданным позволяет обнаружить файлы.

Будьте внимательны — это длинный сценарий!
...
## Загрузка данных с помощью CTAS

Для загрузки данных лучше использовать строго типизированные внешние таблицы, которые мы только что создали. Поэтому простых инструкций [CTAS][] будет достаточно.

Однако сначала давайте создадим новую схему, чтобы загрузить наши таблицы.
...
Для загрузки данных мы используем инструкцию [CTAS][]. Обратите внимание, что для каждой таблицы используется отдельная инструкция [CTAS][].
...
Ход выполнения загрузки можно отслеживать с помощью динамического административного представления (DMV) `[sys].[dm_pdw_exec_requests]`. Поскольку приведенные выше инструкции CTAS имеют `label`, ход выполнения также можно отслеживать с помощью `label`. Дополнительные сведения см. в статье [label][].

## Перестроение индексов

И, наконец, чтобы таблицы гарантированно имели сжатый формат columnstore, можно выполнить операцию REBUILD для этих таблиц. Следующая команда преобразует все открытые или закрытые группы строк в сжатый формат columnstore.
...
Дополнительные сведения о том, как проверить состояние кластеризованного индекса columnstore, см. в статье [manage columnstore indexes][].

## Создание некоторых объектов статистики

Для создания статистики по всем столбцам можно использовать код хранимой процедуры `prc_sqldw_create_stats`, который описан в статье [statistics][].

Ниже приводится часть выходного результата, охватывающего измерения в полном объеме и все соединяемые столбцы в хранилище данных розничной торговли Contoso. Это хорошая отправная точка.

Позднее вы можете самостоятельно добавить дополнительную статистику для других столбцов с данными.

Этот скрипт тоже довольно длинный.
...
Теперь у вас есть все данные, загруженные в хранилище данных SQL Azure. Отличная работа!

Теперь вы можете получать данные из таблиц с помощью таких запросов:
...
## Дальнейшие действия
Общие сведения о загрузке см. в разделе [Загрузка данных в хранилище данных SQL][]. Сведения об управлении индексами columnstore см. в разделе [manage columnstore indexes][].
...
<!--MSDN references-->
[sys.dm_pdw_exec_requests]: https://msdn.microsoft.com/ru-RU/library/mt203887.aspx
[REBUILD]: https://msdn.microsoft.com/ru-RU/library/ms188388.aspx
