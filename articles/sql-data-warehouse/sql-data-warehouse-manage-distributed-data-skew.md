...
Для определения смещения данных можно воспользоваться [DBCC PDW\_SHOWSPACEUSED()][]
...
Это позволяет легко и быстро увидеть, сколько строк таблицы хранится в каждом из 60 распределений базы данных. Помните, что для максимально сбалансированной производительности строки в распределенной таблице должны размещаться равномерно по всем распределениям.

Более глубокий анализ можно выполнить с помощью динамических административных представлений (DMV) хранилища данных SQL Azure. В оставшейся части этой статьи показано, как это сделать.
...
Стандартный способ устранения смещения данных — повторно создать таблицу с другим столбцом распределения. Рекомендации по выбору столбца хэш-распределения см. в статье [Хэш-распределение][]. В примере ниже для повторного создания таблицы с другим столбцом распределения используется оператор [CTAS][].
...
<!--MSDN references-->
[DBCC PDW\_SHOWSPACEUSED()]: https://msdn.microsoft.com/ru-RU/library/mt204028.aspx
