...
В этом пошаговом руководстве показано, как всего за несколько минут подключиться к базе данных в хранилище данных SQL Azure с помощью средств SQL Server Data Tools в Visual Studio. После подключения вы сможете выполнить пробный запрос.
...
> [AZURE.NOTE] Для правильной работы дерева объектов важно указать базу данных хранилища данных SQL в поле **Имя базы данных** при подключении (шаг 4). Если этот параметр оставить пустым и подключиться к базе данных по умолчанию (**master**), дерево может не работать, если некоторые базы данных логического сервера приостановлены. Это ограничение будет разрешено в конечном счете, но важно помнить, что подключение к базе данных хранилища данных SQL обеспечит надлежащее функционирование дерева обозревателя объектов, как показано выше.
