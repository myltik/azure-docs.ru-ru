...
Хранилище данных SQL поддерживает до 1024 одновременных подключений. Все эти подключения могут одновременно отправлять запросы. Тем не менее, чтобы оптимизировать пропускную способность, хранилище данных SQL может помещать некоторые запросы в очередь. Это гарантирует, что на выполнение каждого запроса выделяется минимальный объем памяти. Во время выполнения запросов очередь возникает в случае превышения пределов параллелизма. При достижении пределов параллелизма хранилище данных SQL может повысить общую пропускную способность, обеспечивая активным запросам доступ к необходимым ресурсам памяти.

Предельные границы параллелизма в хранилище данных SQL зависят от двух значений: **количества одновременных запросов** и **количества слотов выдачи**. Запрос выполняется, если не превышено пороговое значение параллелизма и достаточно выделенного количества слотов выдачи.
...
- **Количество слотов выдачи** — это более динамическое понятие. Каждый выполняемый одновременно запрос задействует один или несколько слотов выдачи. Точное количество слотов, используемых при выполнении запроса, зависит от размера хранилища данных SQL и используемого [класса ресурсов](#resource-classes).
...
> [AZURE.NOTE]  Запросы SELECT, выполняемые исключительно в динамических административных представлениях и представлениях каталогов, **не** управляются при помощи классов ресурсов. По этой причине пользователи могут выполнять мониторинг системы, даже если используются все слоты выдачи.

## Классы ресурсов

Классы ресурсов составляют неотъемлемую часть управления рабочей нагрузкой в хранилище данных SQL, так как они управляют памятью и циклами ЦП, выделяемыми для запроса. Существует четыре разных класса ресурсов, каждый из которых представлен в виде **роли базы данных**: **smallrc, mediumrc, largerc и xlargerc**. Для пользователей, которые относятся к классу ресурсов smallrc, выделяется меньший объем памяти, что, в свою очередь, приводит к более высокому параллелизму. В то же время для пользователей, которым назначен класс ресурсов xlargerc, выделяется больший объем памяти, и, следовательно, меньше запросов могут выполняться параллельно.

Существует несколько типов запросов, для выполнения которых не требуется большой объем памяти. Их необходимо выполнять в малых классах ресурсов, не учитывая выделенные ресурсы в соответствующем классе. Таким образом, эти запросы смогут выполняться при нехватке слотов выдачи и не будут потреблять больше слотов, чем необходимо. [Исключения классов ресурсов](#resource-class-exceptions) рассматриваются ниже в этой статье.
...
Повышение класса ресурсов имеет свои преимущества и недостатки. Если назначить для пользователя более высокий класс ресурсов, для запросов будет выделяться большой объем памяти, а значит, они будут выполняться быстрее, но количество одновременно выполняемых запросов снизится. Компромисс заключается в том, что при выделении большого объема памяти для одного запроса снижается количество одновременных запросов, для которых также требуется определенный объем памяти. Если для выполнения запроса конкретного пользователя выделяется большой объем памяти, для запросов других пользователей может не хватить свободной памяти.

В следующей таблице объем памяти, выделенный для каждого распределения, сопоставлен по количеству DWU и классу ресурсов. В хранилище данных SQL данные в одной базе данных разделяются по 60 распределениям. Таким образом, в каждой из 60 баз данных для запроса, который выполняется при DW2000 с классом ресурсов xlarge, выделяется 6400 МБ.
...
Исходя из вышеупомянутого примера, для системного запроса, который выполняется при DW2000 с классом ресурсов xlarge, выделяется 375 ГБ памяти (6400 МБ * 60 распределений / 1024 (чтобы преобразовать в ГБ)).
...
Как уже говорилось, чем выше назначенный пользователю класс ресурсов, тем больше ему выделяется памяти. Так как память является фиксированным ресурсом, то чем больше памяти выделяется на выполнение одного запроса, тем ниже параллелизм. В следующей таблице приведено количество слотов выдачи, доступное для определенного значения DWU, а также показатель их использования по классам ресурсов.
...
| **Выделение** | | | | | | | | | | | | |
| Макс. число одновременных запросов | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 | 32 |
| Макс. число слотов выдачи | 4\. | 8 | 12 | 16 | 20 | 24 | 40 | 48 | 60 | 80 | 80 | 80 |
...
Как видно из этой таблицы, при DW100 в хранилище данных SQL можно выполнять либо 4 одновременных запроса smallrc, либо 2 одновременных запроса largerc.
