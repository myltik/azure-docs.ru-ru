...
Так, например, если DW500 является текущим параметром DWU для хранилища данных SQL, то активные группы рабочей нагрузки сопоставляются с классами ресурсов следующим образом:
...
После этого пользователю потребуется предоставить разрешения. В примере ниже предоставляется разрешение `CONTROL` для базы данных хранилища данных SQL. Разрешение `CONTROL` на уровне базы данных равносильно db_owner в SQL Server.
