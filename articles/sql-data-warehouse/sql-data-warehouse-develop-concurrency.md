...
При достижении одного из этих пороговых значений последующие запросы помещаются в очередь. Хранилище данных SQL выполняет запросы в очереди по принципу очередности по мере выполнения других запросов и нормализации числа запросов и слотов выдачи.

> [AZURE.NOTE]  Запросы SELECT, выполняемые исключительно с динамическими административными представлениями или представлениями каталогов, *не* регулируются пороговыми значениями параллелизма. Пользователи могут следить за системой независимо от того, какое количество запросов в ней выполняется.
...
В предыдущем примере для запроса, который выполняется при DW2000 с классом ресурсов xlarge, всего выделяется 375 ГБ памяти (6400 МБ * 60 распределений / 1024 (чтобы преобразовать в ГБ)) во всем хранилище данных SQL.
...
Хранилище данных SQL предоставляет больше памяти запросам, выполняемым при более высоких классах ресурсов. Так как память является фиксированным ресурсом, то чем больше памяти выделяется на выполнение одного запроса, тем ниже параллелизм. В следующей таблице все вышесказанное сведено в одно представление. В нем приведено количество слотов выдачи, доступное для определенного значения DWU, а также показатель их использования по классам ресурсов.
...
1. **Создание имени для входа**. Откройте подключение к базе данных **master** для хранилища данных SQL и выполните следующие команды.
...
5. **Понижение класса ресурсов**. Чтобы удалить пользователя из роли управления рабочей нагрузкой, используйте следующий запрос.
	...
	> [AZURE.NOTE] Невозможно удалить пользователя из smallrc.
...
.<!--MSDN references-->
[Управление базами данных и учетными записями в базе данных SQL Azure]: https://msdn.microsoft.com/library/azure/ee336235.aspx
