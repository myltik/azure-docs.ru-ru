...
По умолчанию каждый сервер (например, myserver.database.windows.net) имеет квоту, которая ограничивает размер и масштаб баз данных на этом экземпляре. На сервере могут размещаться базы данных SQL и хранилища данных, которые должны использоваться согласно установленной квоте. Эта квота измеряется в единицах транзакций базы данных (DTU) и по умолчанию имеет значение 54 000, рассчитанной на 6000 единиц cDWU. Эта квота является просто ограничением для безопасности. Вы можете увеличить квоту, отправив запрос в службу поддержки и указав "Квота" в качестве типа запроса. 

Чтобы рассчитать требуемое количество единиц DTU, примените множители ниже при расчете:

| Уровень производительности | Единица измерения | Множитель DTU | Пример                   |
|:----------------:|----------------:|---------------:|--------------------------:|
| Эластичность       |  DWU            | 7.5            | DW6000 x 7,5 = 45 000 DTU |
| Среда выполнения приложений          | cDWU            | 9              | DW6000 x 7,5 = 54 000 DTU |

На портале в свойствах сервера SQL вы можете просмотреть использование единиц DTU по состоянию на текущий момент.
