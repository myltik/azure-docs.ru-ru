...
Чтобы повысить вычислительную мощность, можно добавить службе дополнительные DWU, используя ползунок масштабирования на классическом портале Azure. Кроме того, добавить DWU можно с помощью T-SQL, интерфейсов REST API или командлетов Azure PowerShell. Изменение уровня производительности (масштабирование как в сторону увеличения, так и в сторону понижения) приводит к отмене всех запущенных и поставленных в очередь действий. Но уже через несколько секунд вы сможете возобновить работу вычислительных ресурсов с уже повышенной или пониженной мощностью.
...
Того же результата можно добиться с помощью Azure PowerShell, импортировав модуль AzureRM.Sql и используя приведенный ниже код.
...
Приостановить и возобновить вычисления можно на [классическом портале Azure][], с помощью Azure PowerShell или интерфейсов REST API. Приостановка работы вычислительных ресурсов отменяет все запущенные и поставленные в очередь действия. Вернувшись, вы можете возобновить их работу в считанные секунды.

Чтобы приостановить и возобновить работу службы с помощью Azure PowerShell, сначала необходимо импортировать модуль AzureRM.Sql следующим образом.

```Powershell
Import-Module AzureRM.Sql
```

Приведенный ниже код демонстрирует, как выполнять приостановку с помощью Azure PowerShell.

```Powershell
Suspend-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

Возобновить вычисления с помощью Azure PowerShell тоже очень просто.

```Powershell
Resume-AzureRmSqlDatabase –ResourceGroupName "ResourceGroup11" –ServerName "Server01" –DatabaseName "Database02"
```

Подробнее о том, как использовать Azure PowerShell, см. в статье [Использование командлетов PowerShell и интерфейсов REST API при работе с хранилищем данных SQL][].
