...
При экспорте данных в файл формата ORC число текстовых столбцов большого объема может быть ограничено до 50 из-за ошибок нехватки памяти в Java. Чтобы обойти это ограничение, экспортируйте только подмножество столбцов.
...
Одноразовую загрузку в небольшую таблицу или даже периодическую перезагрузку результатов поиска рекомендуется выполнять, используя такой [синтаксис инструкции INSERT](/sql/t-sql/statements/insert-transact-sql.md): `INSERT INTO MyLookup VALUES (1, 'Type 1')`.  Тем не менее единоразовые множественные вставки не так эффективны, как массовая загрузка. 
