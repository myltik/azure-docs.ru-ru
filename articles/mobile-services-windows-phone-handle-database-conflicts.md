<properties linkid="develop-mobile-tutorials-optimistic-concurrent-data-wp8" urlDisplayName="Оптимистичный параллелизм" pageTitle="Обработка конфликта записи базы данных с помощью оптимистичного параллелизма (Магазин Windows) | Центр разработчиков для мобильных устройств" metaKeywords="" description="Узнайте, как обрабатывать конфликты записи базы данных как на сервере, так и в приложении Магазина Windows." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Обработка конфликтов записи базы данных" authors="" />
...
2. В обозревателе решений Visual Studio откройте MainPage.xaml и замените определение `phone:LongListSelector` на ListBox, показанный ниже, и сохраните изменения.
...
3. В обозревателе решений Visual Studio откройте MainPage.xaml.cs. Добавьте обработчик событий MainPage для текстового поля события `LostFocus`, как показано ниже.
...
1. В MainPage.xaml.cs обновите определение класса **TodoItem** с помощью следующего кода, чтобы включить в него системное свойство **__version**. Это включит поддержку обнаружения конфликтов записи:
...
1. В Visual Studio убедитесь, что **эмулятор WVGA 512 МБ** выбирается в раскрывающемся списке в качестве целевого объекта развертывания, как показано на следующем снимке экрана.
...
6. В правом эмуляторе введите другое значение для последнего текстового свойства последнего элемента TodoItem. Затем выберите другое текстовое поле таким образом, чтобы обработчик событий `LostFocus` в правом эмуляторе попытался обновить базу данных со старой версией
...
8. В приложение в левом эмуляторе установите флажок для завершения последнего элемента TodoItem.

	![][11]

9. В приложение в правом эмуляторе попытайтесь обновить тот же текст TodoItem и вызвать событие `LostFocus`. В ответ на конфликт сценарий разрешил его, отказавшись от обновления, потому что элемент уже завершен. 
