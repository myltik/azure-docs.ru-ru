<properties urlDisplayName="Upload custom Java website" pageTitle="Отправка настраиваемого веб-сайта Java в Azure" metaKeywords="" description="В этом учебнике показано, как передать настраиваемый веб-сайт на Java в Azure." metaCanonical="" services="web-sites" documentationCenter="Java" title="Upload a custom Java website to Azure" videoId="" scriptId="" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" />
...
В этом разделе объясняется, как отправить нестандартный веб-сайт Java в среду Azure. Информация, приведенная здесь, применима к любому веб-сайту Java, также даны несколько примеров для конкретных приложений.

Обратите внимание, что Azure предоставляет средства для создания веб-сайтов Java с помощью коллекции приложений Azure и пользовательского интерфейса настройки портала Azure, как описано в разделе [Начало работы с веб-сайтами Azure и Java
](../web-sites-java-get-started). Этот учебник предназначен для сценариев, в которых не требуется использовать пользовательский интерфейс настройки Azure или коллекцию приложений Azure.  
...
- Приложения, которым требуются каталоги с разрешением на запись, должны находиться в каталоге контента веб-сайта Azure, т. е. в каталоге **D:\home**.
...
> [WACOM.NOTE] %HTTP_PLATFORM_PORT% - специальный заполнитель, который должен быть указан как часть элемента **arguments** или как часть списка **httpPlatform** **environmentVariables**. **HttpPlatformHandler** заменит это значение на сформированный системой порт, чтобы указанный в **processPath** процесс мог прослушивать этот порт.
...
Необходимо изменить конфигурацию Jetty в файле start.ini, чтобы задать значение "java.net.preferIPv4Stack=true".
...
3. Создайте файл web.config в каталоге **d:\home\site\wwwroot** и вставьте в него следующее содержимое:
		...
		значение="d:\home\site\wwwroot\hudson_home" />
		  <имя environmentVariable ="JAVA_OPTS" 
		значение="-Djava.net.preferIPv4Stack=true -Duser.home=d:/home/site/wwwroot/user_home -Dhudson.DNSMultiCast.disabled=true" />
		</environmentVariables>            
		    </httpPlatform>
		  </system.webServer>
		</configuration>
...
На том же уровне структуры каталогов, где находится папка tomcat-7.0.40, создайте файл с именем **web.config** и следующим содержанием:
