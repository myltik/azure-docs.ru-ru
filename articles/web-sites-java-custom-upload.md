...
Обратите внимание, что Azure предоставляет средства для создания веб-сайтов Java с помощью коллекции приложений Azure и пользовательского интерфейса настройки портала Azure, как описано в разделе [Приступая к работе с веб-сайтами Azure и Java][Приступая к работе с веб-сайтами Azure и Java]. Этот учебник предназначен для сценариев, в которых не требуется пользовательский интерфейс настройки Azure или коллекция приложений Azure.
...
> [WACOM.ПРИМЕЧАНИЕ] `%HTTP_PLATFORM_PORT%` — специальный заполнитель, который должен быть указан как часть элемента **arguments** или как часть списка **httpPlatform** **environmentVariables**. **HttpPlatformHandler** заменит это значение на сформированный системой порт, чтобы указанный в **processPath** процесс мог прослушивать этот порт.
...
3.  Создайте файл web.config в каталоге **d:\\home\\site\\wwwroot** и вставьте в него следующее содержимое:
    ...
    На данном этапе можно перезапустить веб-сайт для вступления изменений в силу. Подключитесь к узлу [http://ваш\_сайт/hudson][http://ваш\_сайт/hudson] для запуска Hudson.
...
6.  Настройте JDK, как показано ниже.
    ...
    ![Конфигурация Hudson][Конфигурация Hudson]

7.  Настройте Maven, как показано ниже.

    ![Конфигурация Maven][Конфигурация Maven]

8.  Сохраните параметры. Теперь продукт Hudson должен быть настроен и готов к использованию.

Дополнительные сведения о Hudson см. по адресу [][]<http://hudson-ci.org></a>.
...
После внесения этих изменений перезапустите веб-сайт с Liferay, а затем откройте страницу [http://ваш\_сайт][http://ваш\_сайт]. Портал Liferay доступен из корневого каталога веб-сайта.
