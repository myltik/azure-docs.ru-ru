...
#Выполнение заданий Pig с помощью пакета SDK для .NET для Hadoop
...
Указания о том, как это сделать см. в статье <a href="http://go.microsoft.com/fwlink/?LinkId=511138" target="_blank">Создание самозаверяющего сертификата</a>.
...
##<a id="subscriptionid"></a>Найдите идентификатор подписки
...
1. Зайдите в <a href="https://manage.windowsazure.com/" target="_blank">консоль управления Azure</a>.

2. На панели в левой части портала выберите **Параметры**.

3. Среди информации в правой части страницы, найдите подписку, которую необходимо использовать и обратите внимание на значение в столбце **Код подписки**.
...
7. Двойным щелчком откройте **Program.cs** в обозревателе решений. Замените имеющийся код следующим.
...
Используйте **F5** для запуска приложения. При появлении запроса введите **код подписки**, **понятное имя сертификата** и **имя кластера HDInsight**. Приложение создаст несколько строк данных во время выполнения, добавив в конце нечто похожее на следующее.
