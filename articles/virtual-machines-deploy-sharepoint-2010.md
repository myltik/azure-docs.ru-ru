...
1.  Войдите на [портал управления Azure][портал управления Azure] под своей учетной записью.
...
2.  Создайте виртуальную машину с базовой операционной системой: на портале управления Azure щелкните **+СОЗДАТЬ**, затем — **ВИРТУАЛЬНАЯ МАШИНА** и **ИЗ КОЛЛЕКЦИИ**. ![azure-sharepoint-wp-4](./media/virtual-machines-deploy-sharepoint-2010/azure-sharepoint-wp-4.png)
3.  Откроется диалоговое окно **Выбор ОС виртуальной машины**. Нажмите кнопку **Образы платформы**и выберите образ платформы **Windows Server 2008 R2 с пакетом обновления 1**. ![azure-sharepoint-wp-5](./media/virtual-machines-deploy-sharepoint-2010/azure-sharepoint-wp-5.png)
  
4.  Откроется диалоговое окно ***Конфигурация виртуальной машины***. Введите следующие сведения:
    -   Введите **ИМЯ ВИРТУАЛЬНОЙ МАШИНЫ**.
        -   Это имя машины должно быть глобально уникальным.
    -   Оставьте в поле **НОВОЕ ИМЯ ПОЛЬЗОВАТЕЛЯ** значение «Администратор».
    ...
    -   Выберите подходящее значение в поле **РАЗМЕР**.
        -   Для рабочей среды (сервер приложений SharePoint и база данных) мы рекомендуем выбрать размер «Большой» *(4 ядра, 7 ГБ памяти)*.

    ![azure-sharepoint-wp-6](./media/virtual-machines-deploy-sharepoint-2010/azure-sharepoint-wp-6.png)
5.  Откроется диалоговое окно ***Режим виртуальной машины***. Введите следующие сведения:
    -   Выберите режим "Автономная виртуальная машина".
    -   В поле **DNS-ИМЯ** введите первую часть DNS-имени по своему усмотрению.
        -   Эта часть завершит имя в формате MyService1.cloudapp.net.
    -   В поле **УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ** выполните одно из следующего:
        -   Выберите учетную запись хранения, под которой хранится VHD-файл.
        -   Выберите автоматическое создание учетной записи хранения.
...
6.  Откроется диалоговое окно ***Параметры виртуальной машины***. Введите следующие сведения:
    -   В поле **ГРУППА ДОСТУПНОСТИ** выберите «(нет)».
    -   Прочтите и примите условия.
    -   Установите флажок, чтобы создать виртуальную машину.

    ![azure-sharepoint-wp-8](./media/virtual-machines-deploy-sharepoint-2010/azure-sharepoint-wp-8.png)
7.  Откроется страница ***Экземпляры виртуальной машины***. Убедитесь, что виртуальная машина успешно создана. ![azure-sharepoint-wp-9](./media/virtual-machines-deploy-sharepoint-2010/azure-sharepoint-wp-9.png)
8.  Завершение установки виртуальной машины:
...
9.  Создайте виртуальную машину сервера SQL Server с помощью любого из следующих параметров:
...
10. Создайте ферму SharePoint, выполнив следующие дополнительные действия:
...
11. После завершения выполнения скрипта подключитесь к виртуальной машине с помощью панели мониторинга виртуальной машины.
12. Проверьте конфигурацию SharePoint: войдите на сервер SharePoint, а затем перейдите в центр администрирования, чтобы проверить конфигурацию.
