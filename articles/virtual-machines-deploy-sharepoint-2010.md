...
В этом подразделе обсуждаются основные возможности Azure. Доступная в Azure <strong>библиотека образов</strong> предоставляет список доступных предварительно настроенных виртуальных машин. Пользователи могут публиковать в этой библиотеке образов SharePoint Server, SQL Server, Windows Server и другие ISO-образы либо виртуальные жесткие диски. Для упрощения создания виртуальных машин в библиотеке опубликованы базовые образы. Имеющие нужные полномочия пользователи могут использовать эти образы для создания нужной виртуальной машины. Дополнительную информацию см. в статье <a href="/manage/windows/tutorials/virtual-machine-from-gallery/">Создание виртуальной машины под управлением Windows Server 2008 R2</a> на сайте Azure. На рисунке 2 показаны основные шаги для создания виртуальной машины с помощью портала управления Azure:
...
Кроме того, пользователи могут передать образ "sysprepped" на портал управления Azure. Дополнительную информацию см. в статье <a href="/manage/windows/common-tasks/upload-a-vhd/">Создание и загрузка виртуального жесткого диска</a>. На рисунке 3 показаны основные шаги выгрузки образа для создания виртуальной машины:
...
<li>Если у вас нет учетной записи Azure, <a href="http://www.windowsazure.com/pricing/free-trial/">зарегистрируйтесь для получения бесплатной пробной версии Azure</a>.</li>
</ul>
</li>
...
<li>Дополнительную информацию см. в разделе <a href="/manage/windows/common-tasks/upload-a-vhd/#createstorage">Создание учетной записи хранения в Azure</a>.</li>
</ul>
</li>
<li>В поле <strong>ГРУППА ДОСТУПНОСТИ</strong> выберите значение <STRONG>(нет)</STRONG>, а затем нажмите кнопку со стрелкой вправо.</li>
</ul>
</li>
</ul>
</li>
...
<li>Создайте учетную запись хранения в Azure: учетная запись хранения в Azure требуется для передачи VHD-файла, который может использоваться для создания виртуальной машины. Эту учетную запись можно создать с использованием портала управления Azure. Дополнительную информацию см. в разделе <a href="/manage/windows/common-tasks/upload-a-vhd/">Создание учетной записи хранения в Azure</a>.</li>
...
<li>Передайте образ в Azure: чтобы передать образ из файла виртуального жесткого диска, вам необходимо создать и установить сертификат управления. Получите отпечаток сертификата и идентификатор подписки. Установите подключение и загрузите данный VHD-файл с помощью средства командной строки CSUpload. Дополнительную информацию см. в статье <a href="/manage/windows/common-tasks/upload-a-vhd/">Передача образа в Azure</a>.</li>
</ol>
...
<ol>
<li><em>Подготовка</em>: в первую очередь подготовьте VPN-подключение между локальной средой и Azure с помощью виртуальной сети Azure. (Поскольку здесь не используется Active Directory, требуется VPN-туннель). Дополнительную информацию см. в статье <a href="http://msdn.microsoft.com/library/windowsazure/jj156007.aspx">Виртуальная сеть Azure (вопросы проектирования и сценарии безопасного подключения)</a>. Затем с помощью портала управления требуется подготовить новую виртуальную машину с использованием готового образа из библиотеки образов.
<ul>
...
<li>Вместо образа Windows Server 2008 R2 с пакетом обновления 1 можно воспользоваться образом SQL Server 2012. Дополнительную информацию см. в разделе <a href="/manage/windows/common-tasks/install-sql-server/">Подготовка виртуальной машины Server SQL в Azure</a>.</li>
</ul>
</li>
...
<ul>
<li>для создания компьютера разработчика SharePoint воспользуйтесь сценарием простой настройки SharePoint 2010. Дополнительную информацию см. в статье <a href="http://www.microsoft.com/download/details.aspx?id=23415">Сценарий простой настройки SharePoint 2010</a>.</li>
...
<li>Дополнительную информацию о создании нового леса Active Directory на виртуальной машине в виртуальной сети Azure см. в разделе <a href="/manage/services/networking/active-directory-forest/">Установка нового леса Active Directory в Azure</a>.</li>
</ul>
</li>
</ul>
</li>
...
Перенос существующих приложений бизнес-аналитики в Azure обеспечивает более широкие возможности масштабирования. С помощью SharePoint Server, SSAS и SSRS организации могут создавать мощные приложения в области бизнес-аналитики и отчетности, а также панели мониторинга с масштабированием вверх или вниз. Эти приложения и панели мониторинга также поддерживают более безопасную интеграцию с локальными данными и приложениями. Azure обеспечивает соответствие центра обработки данных стандартам ISO 27001. Дополнительную информацию см. на странице <a href="/support/trust-center/compliance/">Центр управления безопасностью Azure</a>.
...
<ul>
<li>Создайте виртуальную сеть. Дополнительную информацию см. в статье <a href="http://msdn.microsoft.com/library/windowsazure/jj156007.aspx">Виртуальная сеть Azure (вопросы проектирования и сценарии безопасного подключения)</a>.
<ul>
