<properties 
	...
	description="Информация об использовании хранилища больших двоичных объектов Azure с пакетом SDK для WebJob Вызов процесса при появлении нового большого двоичного объект в контейнере и обработка  'poison blobs'." 
...
В этом руководстве приведены примеры кода на C# для запуска процессов при создании или обновлении большого двоичного объекта Azure. В примерах кода используется [пакет SDK для заданий WebJob](websites-dotnet-webjobs-sdk.md) версии 1.x.

Примеры кода для создания больших двоичных объектов см. в разделе [Использование пакета SDK веб-заданий для работы с хранилищем очередей Azure](websites-dotnet-webjobs-sdk-storage-queues-how-to.md). 
...
-   [Вызов функции при создании или обновлении большого двоичного объекта](#trigger)
...
-   [Типы, с которыми работает атрибут BlobTrigger](#types)
-   [Получение содержимого текстового большого двоичного объекта путем привязки к строке](#string)
-   [Получение содержимого сериализованного большого двоичного объекта с помощью ICloudBlobStreamBinder](#icbsb)
-   [Способ обработки подозрительных больших двоичных объектов](#poison)
-   [Алгоритм опроса больших двоичных объектов](#polling)
-   [Уведомления о получении большого двоичного объекта](#receipts)
-   [Связанные разделы, которые описаны в статье об очередях](#queues)
-   [Дальнейшие действия](#nextsteps)

## <a id="trigger"></a>Вызов функции при создании или обновлении большого двоичного объекта
...
Конструктор атрибута принимает параметр строки, который указывает имя контейнера и заполнитель для имени большого двоичного объекта. В этом примере если большой двоичный объект с именем *Blob1.txt* создается в контейнере *input*, функция создает большой двоичный объект с именем *Blob1.txt* в контейнере *output*. 
...
Следующий пример кода изменяет расширение файла во время копирования больших двоичных объектов, которые появляются в контейнере *input*, в контейнер *output*. Код заносит расширение большого двоичного объекта *input* в журнал и задает для большого двоичного объекта *output* расширение*.txt*.
...
## <a id="string"></a> Получение содержимого текстового большого двоичного объекта путем привязки к строке

Если будут использоваться текстовые большие двоичные объекты, можно применить `BlobTrigger` к параметру `string`. Следующий пример кода привязывает текстовый большой двоичный объект к параметру `string` с именем  `logMessage`. Функция использует этот параметр для записи содержимого большого двоичного объекта на панель мониторинга пакета SDK для заданий WebJob. 
...
В этом руководстве предоставлены примеры кода обработки обычных сценариев для работы с большими двоичными объектами Azure. Дополнительные сведения об использовании веб-заданий Azure и пакета SDK веб-заданий см. в разделе [Веб-задания Azure - рекомендуемые ресурсы](http://go.microsoft.com/fwlink/?linkid=390226).
