...
- Маршрутизация данных в нудное приложение или систему.
...
2. Настройте схему XSD для проверки входящих сообщений XML. Выберите действие _Проверить_ и выберите _triggers('httplistener').outputs.Content_ в качестве значения параметра _inputXml_.
...
3. Наконец, преобразование выполняется только в том случае, если проверка завершается успешно. Чтобы настроить это условие, нажмите значок шестеренки в правом верхнем углу и выберите "Добавить условие для выполнения". Установите в качестве условия `equals(actions('xmlvalidator').status,'Succeeded')`.
