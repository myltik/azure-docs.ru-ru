...
Чтобы начать процесс импорта или экспорта из хранилища больших двоичных объектов, необходимо сначала создать задание ( *job*). Это может быть задание импорта ( *import job*) или задание экспорта ( *export job*):
...
Чтобы подготовить диск к отправке данных для задания импорта, необходимо запустить **средство импорта/экспорта Microsoft Azure**, которое облегчает копирование данных на диск, шифрование данных на диске с использованием BitLocker и создание файлов журналов для дисков, что рассматривается ниже.
...
При создании задания импорта или экспорта также понадобится идентификатор диска ( *drive ID*), который является серийным номером, предоставленным производителем для определенного жесткого диска. Идентификатор отображается на внешней стороне диска. 

<h3>Требования и область применения</h3>

1.	**Подписка и учетные записи хранения.** У вас должна быть существующая подписка Azure и одна или несколько учетных записей хранения для использования службы импорта и экспорта. С помощью каждого задания можно переносить данные в или из только одной учетной записи хранения. Иными словами, задание не может охватывать несколько учетных записей хранения. Информацию о создании новой учетной записи хранения см в разделе [Как создать учетную запись хранения](../storage-create-storage-account/).
2.	**Диски.** Служба импорта и экспорта поддерживает только 3,5-дюймовые диски SATA II/III. Диски объемом свыше 4 ТБ не поддерживаются. Для заданий импорта будет обрабатываться только первый том данных на диске. Он должен быть отформатирован в файловой системе NTFS. Ве можете подключить внешний диск SATA II/III к большинству компьютеров, воспользовавшись адаптером USB для SATA II/III.
...
4.	**Целевые большие двоичные объекты.** Данные могут быть переданы в блочные и страничные большие двоичные объекты или переданы из них.
5.	**Количество заданий.** У клиента может быть до 20 заданий на одну учетную запись хранения.
6.	**Максимальный размер задания.** Размер задания определяется емкостью используемых дисков и максимальным объемом данных, который может быть сохранен в четной записи хранения. Каждое задание может содержать не больше 10 дисков.
...
Перед созданием задания импорта подготовьте свои диски с помощью средства импорта/экспорта Microsoft Azure. Чтобы получить дополнительную информацию о средстве импорта/экспорта Microsoft Azure, см. [справочник по средству импорта/экспорта Microsoft Azure](http://go.microsoft.com/fwlink/?LinkId=329032). [Средство импорта/экспорта Microsoft Azure](http://go.microsoft.com/fwlink/?LinkID=301900&clcid=0x409) можно скачать как автономный пакет.
...
Средство импорта/экспорта Microsoft Azure создает файл журнала диска ( *drive journal*) для каждого диска после его подготовки. Файл журнала диска хранится на локальном компьютере, а не на самом диске. Файл журнала передается при создании задания импорта. Файл журнала диска включает идентификатор диска и ключ BitLocker, а также другие сведения о диске.  

<h3>Создание задания импорта</h3>

1.	После подготовки диска перейдите к своей учетной записи хранения на портале управления и откройте панель мониторинга. В разделе <strong>Сводка</strong> щелкните <strong>Создать задание импорта</strong>. 
 
2.	На шаге 1 мастера укажите, что вы подготовили диск и уже имеется доступный файл журнала
 
3.	На шаге 2 предоставьте контактные сведения лица, ответственного за это задание импорта. Если вы хотите сохранить подробные данные журнала задания импорта, установите флажок <strong>Сохранить подробный журнал в контейнере BLOB-объекта  'waimportexport'</strong>.
...
6. 	На шаге 5 выберите транспортного агента из списка и введите его номер учетной записи. Майкрософт будет использовать эту учетную запись для отправки дисков вам обратно после завершения задания импорта.
	...
	Если у вас пока нет номера отслеживания, выберите **Я предоставлю сведения о доставке для этого задания импорта после отправки пакета**, затем завершите процесс импорта.

7. Чтобы ввести номер для отслеживания после отправки пакета, вернитесь на страницу **Импорт или экспорт** своей учетной записи хранения на портале управления, выберите задание из списка и щелкните **Информация об отправке**. Просмотрите мастер и введите номер для отслеживания на шаге 2.
...
1. 	Чтобы создать задание экспорта, перейдите к учетной записи хранения на портале управления и просмотрите панель мониторинга. 	В разделе <strong>Сводка</strong> щелкните <strong>Создать задание экспорта</strong> и выполните указания мастера.

2. 	На шаге 2 предоставьте контактные сведения лица, ответственного за это задание экспорта. Если необходимо сохранить подробные данные журнала задания экспорта, установите флажок <strong>Сохранить подробный журнал в контейнере BLOB-объекта  'waimportexport'</strong>.

3.	На шаге 3 укажите, какие данные больших двоичных объектов необходимо экспортировать из своей учетной записи хранения на пустой диск или диски. Можно экспортировать все данные blob-объектов в учетной записи хранения или выбрать blob-объекты, или их наборы.
	...
	<table border="1" cellspacing="0" cellpadding="5" style="border: 1px solid #000000;">
		<tbody>
			<tr>
				...
				<td><strong>Путь к большому двоичному объекту</strong></td>
			...
			<tr>
				<td>Равно</td>
				<td>$root/logo.bmp</td>
				<td>Экспортирует большой двоичный объект <strong>logo.bmp</strong> в корневом контейнере</td>
			</tr>
			<tr>
				<td>Равно</td>
				<td>videos/story.mp4</td>
				<td>Экспортирует большой двоичный объект <strong>story.mp4</strong> в контейнере <strong>videos</strong></td>
			</tr>
		</tbody>
	</table>
...
5. 	На шаге 5 выберите транспортного агента из списка и введите его номер учетной записи. Майкрософт будет использовать эту учетную запись для отправки дисков вам обратно после завершения задания экспорта.
	...
	Если у вас пока нет номера отслеживания, выберите <strong>Я предоставлю сведения о доставке для этого задания экспорта после отправки пакета</strong>, затем завершите процесс экспорта.

6. Чтобы ввести номер для отслеживания после отправки пакета, вернитесь на страницу **Импорт или экспорт** своей учетной записи хранения на портале управления, выберите задание из списка и щелкните **Информация об отправке**. Просмотрите мастер и введите номер для отслеживания на шаге 2.
...
- Информацию о ценах см. на [странице цен](http://go.microsoft.com/fwlink/?LinkId=329033).
...
> [AZURE.NOTE] Если вы используете конвертер, не указанный в списке выше, попробуйте проверить работу своего конвертера при работе со средством импорта и экспорта Microsoft Azure для подготовки диска. Если все нормально, то приобретать поддерживаемый конвертер не нужно.
...
**Как долго можно видеть состояние выполненных заданий на портале управления?**
...
- Для регионов в Азии поддерживается только [DHL](http://www.dhl-welcome.com/Tutorial/). Все посылки будут возвращаться через DHL Express Worldwide.
...
- Майкрософт будет использовать учетную запись транспортного агента, которую вы указали во время создания задания для отправки дисков на ваш обратный адрес из центра обработки данных. Не забудьте указать учетную запись транспортного агента для региона ЦОД, который его поддерживает Вы можете создать учетную запись транспортного агента для [FedEx](http://www.fedex.com/us/oadr/) (США и Европа) или [DHL](http://www.dhl-welcome.com/Tutorial/) (Азия), если ее у вас еще нет.
...
- При отправке посылок должны соблюдаться условия раздела [Условия обслуживания Мicrosoft Azure](http://azure.microsoft.com/ru-ru/support/legal/services-terms/). 
