...
3. Для среды выполнения SQLite требуется указать один из этих типов процессорной архитектуры проекта: **x86**, **x64** или **ARM**. **Любой процессор** - параметр не поддерживается. Измените архитектуру процессора на один из поддерживаемых параметров, которые требуется протестировать.
...
6. В файле Mainpage.xaml.cs замените объявление `todoTable` на объявление типа `IMobileServicesSyncTable`, которое инициализируется при вызове `MobileServicesClient.GetSyncTable()`.

        //private IMobileServiceTable<TodoItem> todoTable = App.MobileService.GetTable<TodoItem>();
        private IMobileServiceSyncTable<TodoItem> todoTable = App.MobileService.GetSyncTable<TodoItem>();

7. В файле MainPage.xaml.cs обновите класс `TodoItem`, чтобы он включал системное свойство **Version**, следующим образом.
...
9. В обозревателе решений Visual Studio откройте файл MainPage.xaml. Найдите определение для кнопки **Обновить**. Замените его следующим определением панели стека. 
    ...
    Этот код добавляет две кнопки с обработчиком события нажатия для операций **Push** (Отправить) и **Pull** (Получить). Кнопки располагаются горизонтально, там, где находится кнопка обновления. Сохраните файл.
