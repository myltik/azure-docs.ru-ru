...
Сведения о восстановлении веб-приложения Azure из резервной копии см. в разделе [Восстановление веб-приложения](web-sites-restore.md).
...
* Для компонента резервного копирования и восстановления сайт должен пребывать в режиме Standard. Дополнительные сведения о масштабировании веб-приложения для использования режима Standard см. в статье [Масштабирование веб-приложения в службе приложения Azure](web-sites-scale.md). Обратите внимание, что режим Premium позволяет ежедневно выполнять большее количество резервных копий, чем режим Standard.
...
7. На панели команд щелкните команду **Создать резервную копию**.
...
3. В поле **Частота** укажите, как часто требуется осуществлять автоматическое резервное копирование. 
...
7. В панели команд нажмите кнопку **Сохранить**, чтобы сохранить изменения конфигурации (или выберите **Отменить**, чтобы отказаться от сохранения).
...
Файл резервной копии базы данных сохраняется в корне ZIP-файла. Для базы данных SQL это файл BACPAC (без расширения), и его можно импортировать. Чтобы создать новую базу данных SQL на основе экспорта BACPAC, выполните шаги, описанные в разделе [Импорт файла BACPAC для создания новой базы данных пользователя](http://technet.microsoft.com/library/hh710052.aspx).
...
Чтобы начать работу со службой Azure, посетите веб-страницу [Бесплатный ознакомительный период в один месяц](/pricing/free-trial/).
