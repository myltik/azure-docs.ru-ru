...
2. Добавьте следующий скрипт в верхнюю часть файла, где определяется функция `getAADToken`. Если заданы значения *tenant_domain*, встроенное приложение *client id* и приложение *key*, эта функция предоставляет токен доступа Graph, использованный для чтения сведений в каталоге.

    >[AZURE.NOTE]Следует кэшировать маркер вместо того, чтобы создавать новый при каждой проверке доступа. Затем обновите кэш, если попытки использовать токен приведут к ответу 401 Authentication_ExpiredToken, как показано в [справочнике по ошибкам API Graph]. Для простоты это не показано в коде ниже, но количество дополнительного трафика в Active Directory будет уменьшено.
