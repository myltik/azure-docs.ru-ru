...
![С помощью общего кэша\_](media/best-practices-caching/Figure2.png)
...
Также возможно переполнение кэша, если допустить нахождение данных в кэше в течение длительного времени. В этом случае запросы для добавления новых элементов в кэш могут привести к тому, что некоторые элементы будут принудительно удалены в ходе процесса, называемого вытеснение. Службы кэша обычно исключают данные на основе принципа наиболее давно использовавшихся (LRU) элементов, но обычно можно переопределить эту политику, чтобы предотвратить удаление элементов. Однако, если будет применен такой подход, есть риск того, что размер кэша превысит объем доступной памяти и приложение, которое попытается добавить элемент в кэш, завершится ошибкой с исключением.

Некоторые реализации кэширования могут предоставлять дополнительные политики вытеснения. К таким относится политика наиболее недавно использовавшегося элемента (в предположении, что данные не потребуются снова), политика первым прибыл, первым обслужен (сначала извлекаются более старые данные) или явное удаление на основе триггера события (например, по факту изменения данных).
...
![Использование локального частного кэша с общим кэшем\_](media/best-practices-caching/Caching3.png) _Рисунок 3. Использование локального частного кэша с общим кэшем_
...
Служба Redis — это больше, чем простой сервер кэширования. Он предоставляет распределенную базу данных в памяти с набором расширенных команд, которая поддерживает многие распространенные сценарии, как описано в разделе Варианты использования кэширования Redis далее в этом документе. В этом разделе перечислены некоторые ключевые функции, которые предоставляет Redis.
...
> [AZURE.NOTE]Кэш Redis для Azure в настоящее время не поддерживает кластеризацию. Если вы хотите создать кластер Redis, вы можете создать собственный пользовательский сервер Redis. Для получения дополнительных сведений обратитесь к разделу Построение пользовательского кэша Redis далее в этом документе.
