...
- Значение элемента *Name* (Имя) в параметре *FrontendIPConfiguration* (IP-конфигурация внешнего интерфейса) используется в элементе *FrontendIP* (IP-адрес внешнего интерфейса) параметра HTTPListener для ссылки на параметр FrontendIPConfiguration (IP-конфигурация внешнего интерфейса).
