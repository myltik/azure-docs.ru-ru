...
С помощью портала Azure можно создать [шлюз приложений](application-gateway-introduction.md) с сертификатом для завершения SSL-запросов, в котором используются виртуальные машины для внутренних серверов.
...
5. Щелкните **Выбор виртуальной сети**, выберите **Создать**, а затем введите следующие значения для виртуальной сети:

    - *myVNet* — для имени виртуальной сети;
    - *10.0.0.0/16* — для диапазона адресов виртуальной сети;
    - *myAGSubnet* — для имени подсети;
    - *10.0.0.0/24* — для диапазона адресов подсети.

    ![Создание виртуальной сети](./media/application-gateway-ssl-portal/application-gateway-vnet.png)

6. Нажмите кнопку **ОК**, чтобы создать виртуальную сеть и подсеть.
7. Щелкните **Выберите общедоступный IP-адрес**, выберите **Создать**, а затем введите имя общедоступного IP-адреса. В этом примере общедоступный IP-адрес — *myAGPublicIPAddress*. Оставьте значения по умолчанию для остальных параметров и нажмите кнопку **ОК**.
...
10. Введите *mycert1* для имени сертификата и *Azure123456!* — для пароля, а затем нажмите кнопку **OK**.
    ...
    ![Создание шлюза приложений](./media/application-gateway-ssl-portal/application-gateway-create.png)
...
2. Щелкните **Подсети**, а затем — **Подсеть**.
    ...
    ![Создание подсети](./media/application-gateway-ssl-portal/application-gateway-subnet.png)
...
В этом примере вы создадите две виртуальные машины, которые будут использоваться как внутренние серверы для шлюза приложений. Также установите службы IIS на виртуальных машинах, чтобы убедиться, что шлюз приложений успешно создан.
...
1. Откройте интерактивную оболочку и убедитесь, что для нее задано значение **PowerShell**.
    ...
    ![Установка пользовательского расширения](./media/application-gateway-ssl-portal/application-gateway-extension.png)
...
5. Щелкните **Добавить целевой объект**, чтобы добавить в серверный пул каждую созданную виртуальную машину.

    ![Добавление внутренних серверов](./media/application-gateway-ssl-portal/application-gateway-backend.png)
...
1. Выберите **Все ресурсы**, а затем щелкните **myAGPublicIPAddress**.
    ...
    ![Запись общедоступного IP-адреса шлюза приложений](./media/application-gateway-ssl-portal/application-gateway-ag-address.png)
...
2. Скопируйте общедоступный IP-адрес и вставьте его в адресную строку браузера. Чтобы принять предупреждение системы безопасности, если используется самозаверяющий сертификат безопасности, выберите "Сведения", а затем — "Перейти на веб-страницу":
    ...
    ![Предупреждение системы безопасности](./media/application-gateway-ssl-portal/application-gateway-secure.png)

    На экране отобразится защищенный веб-сайт IIS, как в показано следующем примере:

    ![Тестирование базового URL-адреса в шлюзе приложений](./media/application-gateway-ssl-portal/application-gateway-iistest.png)
