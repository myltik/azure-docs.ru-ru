...
Размещение нескольких сайтов позволяет развернуть в одном шлюзе приложений не одно, а несколько веб-приложений. Чтобы определить, какой прослушиватель должен принимать трафик, шлюз приложений проверяет наличие заголовка узла во входящем HTTP-запросе. Затем прослушиватель направляет трафик в соответствующий серверный пул согласно определениям правил шлюза. В случае веб-приложений с поддержкой протокола SSL шлюз приложений использует расширение SNI (указание имени сервера) для выбора соответствующего прослушивателя для веб-трафика.

Как правило, размещение нескольких сайтов используется для балансировки нагрузки запросов к разным веб-доменам между различными внутренними пулами серверов. Точно так же в одном шлюзе приложений можно разместить нескольких поддоменов одного корневого домена.


## Сценарий
В следующем примере шлюз приложений обслуживает трафик сайтов contoso.com и fabrikam.com с помощью двух внутренних пулов серверов: contoso и fabrikam. Аналогичную конфигурацию можно использовать для размещения поддоменов, например app.contoso.com и blog.contoso.com.
...
- **Прослушиватель**. У прослушивателя есть внешний порт, протокол (Http или Https — с учетом регистра) и имя SSL-сертификата (при настройке разгрузки SSL). Для шлюзов приложений с поддержкой нескольких сайтов также добавляются имя узла и индикаторы SNI.
...
В этом примере будут использоваться два внутренних пула серверов для маршрутизации сетевого трафика на основе запрошенного сайта. Один пул принимает трафик от сайта contoso.com, а другой — от сайта fabrikam.com. Замените приведенные выше IP-адреса и добавьте конечные точки IP-адресов своего приложения. Обратите внимание, что вместо внутренних IP-адресов для серверных экземпляров можно также использовать общедоступные IP-адреса, полное доменное имя или сетевую карту виртуальной машины. Используйте параметр -BackendFQDNs в PowerShell, чтобы указать полные доменные имена, а не IP-адреса.
...
### Шаг 6
Настройте два SSL-сертификата для обоих веб-сайтов, которые будут обслуживаться в этом примере. Один сертификат предназначен для трафика contoso.com, а другой — для трафика fabrikam.com. Это должны быть сертификаты, выданные для веб-сайтов центром сертификации. Самозаверяющие сертификаты поддерживаются, но их не рекомендуется использовать трафика в рабочих средах.
