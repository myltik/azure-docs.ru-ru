...
- **Параметры пула тыловых серверов**. У каждого пула есть такие параметры, как порт, протокол и сходство на основе файлов cookie. Эти параметры привязываются к пулу и применяются ко всем серверам в этом пуле.
...
>[AZURE.NOTE] Если вам нужно настроить пользовательскую проверку для шлюза приложений, см. статью [Создание пользовательской проверки для шлюза приложений с помощью PowerShell для диспетчера ресурсов Azure](application-gateway-create-probe-ps.md). Дополнительные сведения см. в статье [Обзор мониторинга работоспособности шлюза приложений](application-gateway-probe-overview.md).
...
1. Чтобы остановить работу шлюза, используйте командлет **Stop-AzureRmApplicationGateway**.
...
Чтобы остановить работу шлюза приложений, используйте командлет **Stop-AzureRmApplicationGateway**.

	Stop-AzureRmApplicationGateway -ApplicationGateway $getgw  


Когда работа шлюза приложений будет остановлена, удалите службу с помощью командлета **Remove-AzureRmApplicationGateway**.
...
Дополнительные сведения о настройке разгрузки SSL см. в статье о [настройке шлюза приложений для разгрузки SSL](application-gateway-ssl.md).
