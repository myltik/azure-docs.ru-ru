...
* **Уровень**. Этот параметр является уровнем шлюза приложений. Доступны два уровня: **WAF** и **Стандартный**. WAF включает функцию брандмауэра веб-приложения.
* **Размер SKU**. Этот параметр указывает размер шлюза приложений. Доступные значения: **Мелкий**, **Средний** и **Крупный**. Мелкий размер недоступен при выборе уровня WAF.
...
Последнее, что осталось настроить для создания шлюза приложений, это конфигурация прослушивателя.  Если используется протокол **HTTP**, то настраивать больше ничего не нужно и можно нажать кнопку **ОК**. Чтобы использовать протокол **HTTPS** , нужно настроить дополнительные параметры.
...
После создания шлюза приложений в него нужно добавить системы, в которых размещается приложение, нагрузка которого будет балансироваться. IP-адреса или полные доменные имена этих серверов добавляются во внутренние пулы адресов.

### <a name="ip-address-or-fqdn"></a>IP-адрес или полное доменное имя

#### <a name="step-1"></a>Шаг 1

Щелкните созданный шлюз приложений, щелкните **Серверные пулы** и выберите текущий внутренний пул.

![Внутренние пулы шлюза приложений][11]

#### <a name="step-2"></a>Шаг 2

Щелкните **Добавить цель**, чтобы добавить IP-адреса для значений полных доменных имен.

![Внутренние пулы шлюза приложений][11-1]

#### <a name="step-3"></a>Шаг 3.

После ввода всех значений серверной части щелкните **Сохранить**.

![Добавление значений для внутренних пулов шлюза приложений][12]

Это действие сохраняет введенные значения во внутреннем пуле. После обновления шлюза приложений трафик, который поступает в него, направляется на адреса внутреннего пула, добавленные на этом шаге.
...
Щелкните **Добавить цель**, чтобы добавить новый элемент внутреннего пула. Выберите виртуальную машину и сетевую карту из раскрывающихся списков.

![Добавление сетевых карт во внутренние пулы шлюза приложений][13]

#### <a name="step-3"></a>Шаг 3.

По завершении щелкните **Сохранить**, чтобы сохранить сетевые карты в качестве элементов внутреннего пула.

![Сохранение сетевых карт во внутренних пулах шлюза приложений][14]
