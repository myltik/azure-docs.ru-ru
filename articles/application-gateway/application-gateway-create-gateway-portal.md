...
>[AZURE.IMPORTANT] Дополнительная настройка шлюза приложений, включая пользовательские пробы работоспособности, серверный пул адресов и дополнительные правила, осуществляется после настройки шлюза приложений, а не во время первоначального развертывания.
...
В результате выполнения этих действий создается базовый шлюз приложений с параметрами по умолчанию для прослушивателя, серверного пула, протокола HTTP серверной части и правил. Вы сможете изменить эти параметры в соответствии с развертыванием после успешного завершения подготовки. Как уже упоминалось, этот процесс создает шлюз приложений по умолчанию.  Следующие действия необходимы, чтобы настроить шлюз приложений, добавив элементы пула, изменив параметры и настроив правила в шлюзе для его правильной работы.
