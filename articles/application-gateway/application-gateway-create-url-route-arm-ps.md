...
В следующем примере создаются два правила: одно для пути маршрутизации трафика /image в пул серверной части pool1, а другое для пути маршрутизации трафика /video в пул серверной части pool2. Эти правила гарантируют, что трафик для каждого набора URL-адресов направляется к серверной части. Например, файл http://contoso.com/image/figure1.jpg будет отправлен в pool1, а http://contoso.com/video/example.mp4 — в pool2.
...
Создайте параметр правила. Этот шаг позволяет настроить шлюз приложений для маршрутизации на основе URL-путей. Определенная ранее переменная `$urlPathMap` теперь используется для создания правила на основе пути. На этом шаге мы связываем правило с прослушивателем и сопоставлением пути URL-адреса, созданным ранее.
