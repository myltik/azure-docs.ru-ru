...
**В. Поддерживает ли шлюз приложений также мультитенантные серверные части?**

Нет, в настоящее время шлюз приложений сохраняет входящий заголовок узла и отправляет этот же заголовок в серверную часть. Если серверной части требуется другой заголовок, произойдет сбой. Аналогично, если серверная часть является мультитенантной и включено сквозное шифрование SSL, то серверная часть будет ожидать имя сервера в расширении SNI. В настоящее время шлюз приложений не отправляет заголовок SNI в запросах к серверной части в сценариях сквозного шифрования SSL, что может привести к проблемам с пробами и путями к данным. 
