...
>[AZURE.NOTE]Показатели производительности также зависят от HTTP-ответа веб-приложения обратно шлюзу приложений.


## Мониторинг
 
Шлюз приложений отслеживает состояние работоспособности серверных экземпляров с помощью портов проб, периодически тестируя HTTP-ответы из разделов HttpSettings шлюза. Проба требует успешного HTTP- ответа в диапазоне кодов ответов от 200 до 390 и тестирует серверные IP-адреса каждые 30 секунд для проверки HTTP-ответа.

При получении успешного HTTP-ответа IP-адрес помечается как работоспособный. В случае сбоя пробы IP-адрес удаляется из серверного пула работоспособных адресов и трафик перестает поступать на этот сервер. Проба работоспособности выполняется каждые 30 секунд на нерабочем веб-экземпляре до тех пор, пока он снова не перейдет в сетевой режим. Веб-экземпляр, успешно прошедший проверку работоспособности, добавляется в серверный пул работоспособных экземпляров, и в этот экземпляр возобновляется передача трафика.
