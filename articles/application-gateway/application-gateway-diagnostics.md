...
2. Запишите или запомните ИД ресурса шлюза приложений, для которого нужно включить ведение журнала, в формате: /subscriptions/\<идентификатор_подписки\>/resourceGroups/\<имя_группы_ресурсов\>/providers/Microsoft.Network/applicationGateways/\<имя_шлюза_приложений\>. Получить эти сведения можно на портале.
...
Метрики — это функция определенных ресурсов Azure, позволяющая просматривать данные счетчиков производительности на портале. Сейчас для шлюза приложений доступна одна метрика. Эта метрика измеряет пропускную способность, и ее можно просмотреть на портале. Перейдите к шлюзу приложений и щелкните **Метрики**. Выберите "Throughput" (Пропускная способность) в разделе **Available metrics** (Доступные метрики), чтобы просмотреть значения. На следующем рисунке приведен пример с фильтрами, которые можно использовать для отображения данных за различные периоды времени.
