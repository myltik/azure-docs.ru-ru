---
title: "Настройка имеющегося шлюза приложения для размещения нескольких узлов на портале Azure | Документация Майкрософт"
...
2. Создание нового прослушивателя для каждого сайта, который будет поддерживать шлюз приложений.
...
Потребуется отдельный пул серверной части для каждого сайта, который будет поддерживать шлюз приложений. В данном случае создаются 2 пула: один для contoso11.com и второй для fabrikam11.com.
...
### <a name="step-2"></a>Шаг 2
Заполните сведения о прослушивателе. В этом примере мы настраиваем оконечную точку SSL, поэтому создайте новый внешний порт. Отправьте PFX-файл сертификата, который будет использоваться для оконечной точки SSL. Эта колонка отличается от колонки стандартного базового прослушивателя только наличием имени узла.
...
### <a name="step-3"></a>Шаг 3.
Щелкните **Базовое правило**, чтобы создать второе правило. Заполните форму для второго прослушивателя и второго пула серверной части, затем щелкните **ОК** для сохранения.
