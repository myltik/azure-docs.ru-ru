...
- **Правило.** Правило связывает прослушиватель и пул внутренних серверов, а также определяет, в какой пул внутренних серверов следует направлять трафик, поступающий на определенный прослушиватель.
...
>[AZURE.NOTE] Если вам нужно настроить пользовательскую проверку для шлюза приложений, см. статью [Создание пользовательской проверки для шлюза приложений (классического) Azure с помощью PowerShell](application-gateway-create-probe-classic-ps.md). Дополнительные сведения см. в статье [Обзор мониторинга работоспособности шлюза приложений](application-gateway-probe-overview.md).
...
Теперь необходимо настроить шлюз приложений. Воспользуйтесь командлетом **Set-AzureApplicationGatewayConfig** и XML-файлом конфигурации.
...
Чтобы проверить состояние шлюза, используйте командлет **Get-AzureApplicationGateway**. Если командлет **Start-AzureApplicationGateway** на предыдущем этапе выполнен успешно, параметр *State* будет иметь значение Running, а параметры *Vip* и *DnsName* — допустимые значения.
...
Сведения о том, как настроить шлюз приложений для использования с внутренним балансировщиком нагрузки, см. в статье [Создание шлюза приложений с внутренней подсистемой балансировщика нагрузки (ILB)](application-gateway-ilb.md).
