...
>[AZURE.NOTE] Если необходимо настроить пользовательскую пробу для шлюза приложений, см. статью [Создание пользовательской проверки для шлюза приложений (классического) Azure с помощью PowerShell](application-gateway-create-probe-classic-ps.md). Дополнительные сведения см. в статье о [пользовательских пробах и мониторинге состояния работоспособности](application-gateway-probe-overview.md).
...
Чтобы проверить созданный шлюз, используйте командлет **Get-AzureApplicationGateway**.
...
> [AZURE.NOTE] Выполнение командлета **Start-AzureApplicationGateway** может занять около 15–20 минут.
...
Чтобы проверить состояние шлюза, используйте командлет **Get-AzureApplicationGateway**. Если командлет **Start-AzureApplicationGateway** на предыдущем этапе выполнен успешно, у параметра *State* должно быть значение Running, а у параметров *Vip* и *DnsName* — допустимые значения.
...
Когда работа шлюза будет остановлена, удалите службу с помощью командлета **Remove-AzureApplicationGateway**.
