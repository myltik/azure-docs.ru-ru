...
В этом примере создается базовый шлюз приложений с параметрами по умолчанию для прослушивателя, серверного пула, протокола HTTP серверной части и правил. Он также настраивает разгрузку SSL. Вы сможете изменить эти параметры в соответствии с развертыванием после успешного завершения подготовки. Если веб-приложение уже задано, а на предыдущих этапах были заданы IP-адреса для серверного пула, сразу после подготовки и запуска шлюза приложений начинается балансировка нагрузки.
