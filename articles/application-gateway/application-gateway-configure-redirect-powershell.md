---
title: "Настройка перенаправления для шлюза приложений Azure с помощью PowerShell | Документация Майкрософт"
description: "Эта страница содержит сценарии для настройки перенаправления для шлюза приложений с помощью PowerShell."
...
## <a name="http-to-https-redirect-on-an-existing-application-gateway"></a>Перенаправление трафика HTTP в HTTPS на имеющемся шлюзе приложений

В следующем примере создается прослушиватель HTTP на порте 80 для перенаправления запросов в имеющейся прослушиватель на порте 443.
...
После обновления шлюза приложений, проверьте нового прослушивателя на порте 80.  В следующем примере показано использование `curl` для проверки перенаправления.
...
В следующем примере создается прослушиватель и правило на основе пути в имеющемся шлюзе приложений, который перенаправляет только трафик определенного пути URL-адреса.
...
В следующем примере создается шлюз приложений с 2 многосайтовыми прослушивателями на порте 80. Прослушиватели предназначены для сайтов adatum.com и adatum.org. Правило перенаправлениия создано для перенаправления трафика из сайта adatum.org на сайт adatum.com. Чтобы настроить псевдонимы CNAME в общедоступном IP-адресе шлюза приложения, необходимо выполнить дополнительные настройки. Дополнительные сведения о делегировании домена в Azure DNS и создании записей CNAME для вашего домена см. в [этой статье](../dns/dns-delegate-domain-azure-dns.md).
...
## <a name="redirect-to-an-external-site"></a>Перенаправление на внешний сайт

В следующем примере трафик, который поступает в шлюз приложений, перенаправляется на внешний веб-сайт (bing.com). При использовании `TargetUrl` переключать коммутаторы `IncludePath` и `IncludeQuery` запрещено.
...
Ознакомьтесь со статьей [Настройка политики SSL и сквозного режима связи SSL для шлюза приложений с помощью PowerShell](application-gateway-end-to-end-ssl-powershell.md)
