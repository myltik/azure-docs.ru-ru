---
title: Поддерживаемый платформой перенос ресурсов IaaS из классической модели в модель Azure Resource Manager | Microsoft Docs
...
| Сеть |Виртуальные сети с использованием пиринга. (Перенос виртуальной сети в ARM и последующий пиринг) Дополнительные сведения о [пиринге виртуальных сетей](../virtual-network/virtual-network-peering-overview.md). |
...
| Среда выполнения приложений |Расширения центра безопасности Azure с виртуальной сетью, использующей VPN-шлюз или ER-шлюз с локальным DNS-сервером |Центр безопасности Azure автоматически устанавливает расширения на виртуальные машины, чтобы наблюдать за их безопасности и создавать оповещения. Обычно эти расширения устанавливается автоматически, если для подписки включена политика центра безопасности Azure. В настоящее время перенос шлюза не поддерживается, и его необходимо удалить перед фиксацией миграции. После удаления шлюза доступ к Интернету учетной записи хранения виртуальной машины утрачивается. После этого миграция не продолжится, так как будет невозможно заполнить большой двоичный объект состояния гостевого агента. Рекомендуется отключить политику центра безопасности Azure в подписке за 3 часа до выполнения миграции. |
...
Чтобы перенести виртуальную машину, для которой включена архивация, ознакомьтесь с разделом [Резервные копии классических виртуальных машин созданы в хранилище службы архивации. Теперь эти виртуальные машины нужно перевести из классического режима в режим Resource Manager. Как выполнить их резервное копирование в хранилище служб восстановления?](../backup/backup-azure-backup-ibiza-faq.md#i-have-backed-up-my-classic-vms-in-backup-vault-now-i-want-to-migrate-my-vms-from-classic-mode-to-resource-manager-mode-how-can-i-backup-them-in-recovery-services-vault)
