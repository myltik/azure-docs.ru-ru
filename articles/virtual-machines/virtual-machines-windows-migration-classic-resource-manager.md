...
>[AZURE.NOTE] С этим объявлением мы запускаем общедоступную предварительную версию службы миграции. В течение периода действия общедоступной предварительной версии рекомендуется переносить в подписку Azure только непроизводственные рабочие нагрузки.
...
Ниже приведены конфигурации, которые в настоящее время не поддерживаются. При добавлении поддержки для них в будущем может быть затронута работа некоторых виртуальных машин в этой конфигурации (остановка (освобождение) и перезапуск):
...
В настоящее время миграция учетных записей хранения не поддерживается для этой общедоступной предварительной версии. Тем не менее в скором времени планируется предоставить такую поддержку. При миграции и соответствующих операциях с учетными записями хранения следует учесть два важных аспекта.
...
- Рекомендации по миграции учетных записей хранения.
	- При поддержке миграции учетных записей хранения платформа будет использовать вычислительные и сетевые ресурсы отдельно от ресурсов учетных записей хранения, чтобы обеспечить удобство работы.
...
3. Мы рекомендуем не выполнять миграцию производственных ресурсов на этом этапе.
...
1.	Подготовка.
	* Это первый шаг в процессе миграции. На этом этапе имитируется преобразование классических ресурсов IaaS в ресурсы Resource Manager. Эти процессы представляются параллельно для возможности визуализации. Подробная последовательность действий описана ниже.
  ...
  *	После завершения подготовки можно визуализировать ресурсы в классической модели и в модели Resource Manager. Для каждой облачной службы в классической модели развертывания мы создадим имя группы ресурсов по шаблону `cloud-service-name>-migrated`.
...
2.	Проверка по сценарию или вручную
  ...
  * Миграцию не нужно зафиксировать в течение строго определенного срока. В этом состоянии время не ограничено. Однако имейте в виду, что плоскость управления для этих ресурсов будет заблокирована, пока не будет выполнено прерывание или фиксация.
...
Нет. Виртуальные машины (классическая модель) являются полностью поддерживаемыми общедоступными службами. Поэтому можно продолжать использовать эти ресурсы, чтобы расширить инфраструктуру в облаке Microsoft Azure.
...
- [Use CLI to migrate IaaS resources from Classic to Azure Resource Manager (Перенос ресурсов IaaS из классической модели в модель Azure Resource Manager с помощью интерфейса командной строки Azure)](virtual-machines-linux-cli-migration-classic-resource-manager.md)
