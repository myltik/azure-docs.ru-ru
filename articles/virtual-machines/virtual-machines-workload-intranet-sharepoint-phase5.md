...
1.	Войдите на компьютер с сервером-получателем SQL Server с учетной записью **[имя\_домена]\\sp\_farm\_db**.
...
7.	В поле **Местоположение резервных копий** введите **\[имя\_компьютера]\\backup**, нажмите клавишу ВВОД, введите **[имя\_базы\_данных].bak** и дважды нажмите кнопку **ОК**. В разделе **Восстанавливаемые резервные наборы данных** должны появиться полная резервная копия и резервная копия журнала.
...
После создания прослушивателя вам потребуется указать его на всех виртуальных машинах SharePoint вместо имени первого компьютера SQL Server в кластере. Вместо имен настройте виртуальные машины SharePoint для использования псевдонима SQL. Дополнительную информацию и указания см. в статье [Псевдоним SQL для SharePoint](http://blogs.msdn.com/b/priyo/archive/2013/09/13/sql-alias-for-sharepoint.aspx).
