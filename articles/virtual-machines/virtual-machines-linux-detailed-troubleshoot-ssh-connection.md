<properties
	pageTitle="Подробное описание устранения неполадок SSH для виртуальной машины Azure | Microsoft Azure"
...
Клиенту SSH не всегда удается получить доступ к SSH-службе на виртуальной машине по многим причинам. Если вы использовали более [общие действия по устранению неполадок SSH](virtual-machines-linux-troubleshoot-ssh-connection.md), необходимы будут дополнительные действия для решения проблемы подключения. В этой статье подробно изложены пошаговые указания, направленные на то, чтобы определить, где происходит сбой SSH-подключения и как его устранить.
