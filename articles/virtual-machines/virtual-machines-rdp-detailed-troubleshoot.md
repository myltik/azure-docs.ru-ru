...
В [статье о базовой диагностике удаленного рабочего стола](virtual-machines-troubleshoot-remote-desktop-connections.md) описывается, как использовать [пакет диагностики Azure IaaS (Windows)](https://home.diagnostics.support.microsoft.com/SelfHelp?knowledgebaseArticleFilter=2976864). Если данный пакет диагностики не устранил проблему **Подключение к порту удаленного рабочего стола в виртуальной машине Azure (требуется перезагрузка)**, следуйте указаниям в [этой статье](virtual-machines-windows-reset-password.md), чтобы выполнить сброс службы удаленного рабочего стола на виртуальной машине. В результате:
...
После этого введите имя своей подписки Azure, имя облачной службы и имя виртуальной машины (удаляйте из них знаки < and >), а затем выполните следующие команды.
...
Уточнить имя подписки можно в свойстве _SubscriptionName_, которое возвращает команда **Get-AzureSubscription**. Уточнить имя облачной службы своей виртуальной машины можно в столбце _ServiceName_ в данных, которые возвращает команда **Get-AzureVM**.
