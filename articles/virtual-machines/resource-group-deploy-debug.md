...
Чтобы проверить квоты ядер своей собственной подписки, следует использовать `azure vm list-usage` в Azure CLI и командлет `Get-AzureVMUsage` в PowerShell. Ниже приведена команда в Azure CLI и показано, что квота ядер для бесплатно пробной учетной записи равна 4:
