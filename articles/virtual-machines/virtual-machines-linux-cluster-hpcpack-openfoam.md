...
3.	Создайте файл с именем C:\\cred.xml и скопируйте в него данные ключей RSA. Пример этого файла можно найти в приложении в конце этой статьи.
...
2.  Для автоматической установки библиотеки Intel MPI используйте файл silent.cfg. Пример этого файла можно найти в приложении в конце этой статьи. Поместите этот файл в общую папку /openfoam. Дополнительные сведения о файле silent.cfg см. в статье [Руководство по установке библиотеки Intel MPI для Linux. Автоматическая установка](http://scc.ustc.edu.cn/zlsc/tc4600/intel/impi/INSTALL.html#silentinstall).
...
Для тестирования вам следует добавить следующие строки в файл /etc/security/limits.conf на узлах Linux:
...
Вы можете создать файл limits.conf в папке C:\\OpenFoam (сохраните текстовый файл с нотацией Linux для завершения строк) и выполнить следующую команду, чтобы скопировать его на все узлы Linux:

```
clusrun /nodegroup:LinuxNodes cp /openfoam/limits.conf /etc/security
```

После обновления файла limits.conf перезапустите узлы Linux. После перезапуска убедитесь, что общая папка доступна по адресу /openfoam.
...
2.  Чтобы скомпилировать OpenFOAM с помощью библиотеки Intel MPI, сначала потребуется настроить некоторые переменные среды для Intel MPI и OpenFOAM. Для этого используйте сценарий bash с именем settings.sh. Пример этого файла можно найти в приложении в конце этой статьи. Поместите этот файл (сохраненный с нотацией Linux для завершения строк) в общую папку /openfoam. Этот файл также содержит параметры для сред выполнения MPI и OpenFOAM, которые впоследствии будут использованы для выполнения задания OpenFOAM.

3. Установите зависимые пакеты, необходимые для компиляции OpenFOAM. В зависимости от дистрибутива Linux, вам может потребоваться сначала добавить несколько репозиториев. Репозитории и пакеты перечислены в приложении в конце этой статьи. Для надлежащего выполнения команд мы рекомендуем подключаться к каждому узлу Linux по SSH.
...
Теперь попробуем запустить задание MPI с названием sloshingTank3D (стандартный пример из пакета OpenFoam) на двух узлах Linux. В этом примере /opt/openfoam231 является путем установки OpenFOAM на узлах Linux.

### Настройка среды выполнения

На головном узле откройте Windows PowerShell и запустите следующую команду, чтобы настроить среду выполнения MPI и OpenFOAM на всех узлах Linux.
...
6.	Добавьте 4 задачи к заданию, используя следующие команды и параметры.
    ...
    *   **Задача 2**. Запуск **interDyMFoam** в параллельном режиме для выполнения расчета на основании примера данных.
        ...
        *   **Рабочий каталог** — /openfoam/sloshingTank3D.
        ...
        ![Сведения о задаче 2][task_details2]
    ...
    *   **Задача 3**. Запуск **reconstructPar** для слияния наборов каталогов времени из каждого processor\_N\_ directory в общий набор каталогов времени.
        ...
        *   **Рабочий каталог** — /openfoam/sloshingTank3D.
        ...
        ![Сведения о задаче 3][task_details3]
    ...
    *   **Задача 4**. Запуск **foamToEnsight** в параллельном режиме для преобразования выходных файлов OpenFOAM в формат EnSight. Преобразованные файлы помещаются в папку Ensight учебного каталога.
        ...
        *   **Рабочий каталог** — /openfoam/sloshingTank3D.
        ...
        ![Сведения о задаче 4][task_details4]
...
5.  Создайте объект **Iso-volume** из коллекции **стен**. Для этого на панели **Parts (Части)** выберите **Walls (Стены)** и нажмите кнопку **Isosurfaces**.

6.	В диалоговом окне в поле **Type (Тип)** выберите **Isovolume** и для диапазона **Isovolume range** задайте минимальное значение 0,5. Щелкните **Create with selected parts (Создать с помощью выделенных частей)**, чтобы создать объект Isovolume.
...
### Примеры команд для добавления репозиториев и зависимых пакетов на узлах Linux
