...
В версии 2.0 данные включают в себя:
...
Расширение можно включить с помощью [портала Azure](https://portal.azure.com/#), Azure PowerShell или сценариев интерфейса командной строки Azure (Azure CLI).
...
- **Агент Linux для Azure 2.0.6 или более поздней версии**. Обратите внимание на то, что большинство образов в коллекции виртуальных машин Azure на базе Linux включает версию 2.0.6 или более позднюю. Узнать, какая версия установлена на виртуальной машине, можно с помощью команды **WAAgent -version**. Если на виртуальной машине используется версия до 2.0.6, обновите ее, следуя этим [инструкциям на сайте GitHub](https://github.com/Azure/WALinuxAgent "инструкции").

- **Azure CLI**. Чтобы настроить среду Azure CLI на компьютере, воспользуйтесь [этим руководством по установке интерфейса командной строки](../xplat-cli-install.md). После установки Azure CLI для доступа к его командам вы можете использовать команду **azure** в интерфейсе командной строки (Bash, терминал, командная строка). Например:
	- Выполните команду **azure vm extension set --help**, чтобы получить более подробную справочную информацию.
	- Выполните команду **azure login** для входа Azure.
	- Выполните команду **azure vm list**, чтобы вывести список виртуальных машин, которые имеются в Azure.
...
- базовый набор основных данных о системе. Обратите внимание, что полный набор данных описан на [сайте кроссплатформенных решений System Center](https://scx.codeplex.com/wikipage?title=xplatproviders). Если вы хотите включить в набор дополнительные данные, выполните действия, описанные в сценариях 2 и 3.
...
## Просмотр данных
Данные о состоянии и производительности хранятся в таблице хранилища Azure. Ознакомьтесь с разделом [Использование табличного хранилища Azure в Ruby](../storage/storage-ruby-how-to-use-table-storage.md), чтобы узнать о способах доступа к данным в таблице хранилища с помощью сценариев Azure CLI.
...
## Известные проблемы
- В версии 2.0 диагностического расширения Linux доступ к данным Rsyslog и файлам журнала, указанным клиентом, можно получить только с помощью сценариев.
