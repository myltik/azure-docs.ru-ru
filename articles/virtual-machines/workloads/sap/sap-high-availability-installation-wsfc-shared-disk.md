---
title: "Установка SAP NetWeaver высокого уровня доступности в Azure с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS | Документация Майкрософт"
description: "Установка SAP NetWeaver высокого уровня доступности с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS"
...
---
# <a name="sap-netweaver-ha-installation-on-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance-on-azure"></a>Установка SAP NetWeaver высокого уровня доступности в Azure с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS
...
В этом документе описаны шаги по установке и настройке в Azure высокодоступной системы SAP с использованием **отказоустойчивого кластера Windows (WSFC)** и **общего диска кластера** для кластеризации экземпляра SAP (A)SCS.
...
2.  Чтобы сохранить порты по умолчанию для внутреннего балансировщика нагрузки Azure, выберите:
  ...
  * для **системы ABAP с Java** — экземпляр **ASCS** с номером **00** и экземпляр **SCS** с номером **01**.
  ...
  Чтобы использовать номера экземпляров, отличные от 00 для ABAP ASCS и от 01 для Java SCS, сначала необходимо изменить [стандартные правила балансировки нагрузки ASCS/SCS для внутренней подсистемы балансировки нагрузки Azure][sap-ha-guide-8.9].
...
1.  Проверьте текущее значение параметра **ProbePort**, выполнив следующую команду PowerShell:
  ...
  ```
   ...
   Выполните ее на одной из виртуальных машин в конфигурации кластера.
...
2.  Укажите порт пробы. По умолчанию для проб задан порт **0**. В нашем примере мы используем порт пробы **62000**.
  ...
  Чтобы задать новое значение ProbePort для кластерного ресурса **SAP <*SID*> IP**, выполните следующий сценарий PowerShell, который изменяет значение переменных среды PowerShell:
  ...
  ```
  После выполнения сценария вам будет предложено перезапустить кластерную группу SAP, чтобы активировать изменения.
