...
---
# <a name="high-availability-architecture-and-scenarios-for-sap-netweaver"></a>Архитектура высокого уровня доступности и сценарии для SAP NetWeaver
...
* **Высокий уровень доступности приложений SAP.**
  ...
  * избыточные **серверы приложений SAP** и
  * уникальные компоненты (**единая точка отказа (SPOF)**), такие как
    * **экземпляр SAP (A)SCS** и
    *  **СУБД**.
...
Конфигурации высокой доступности SAP, интегрированной с SAPinst, для Linux (такой же, как для Windows) не существует. Сведения об обеспечении высокой доступности SAP в локальной среде Linux см. в статье с [информацией о партнерских решениях для обеспечения высокой доступности][sap-ha-partner-information].

## <a name="azure-infrastructure-high-availability"></a>Высокая доступность инфраструктуры Azure

### <a name="single-instance-virtual-machine-sla"></a>Соглашение об уровне обслуживания для одного экземпляра виртуальной машины

В настоящее время предоставляется соглашение SLA для одной виртуальной машины с временем бесперебойной работы на уровне 99,9 % и хранилищем класса Premium. Чтобы понять, как может выглядеть доступность одной виртуальной машины, можно выполнить сборку продукта, регулируемого другими доступными [соглашениями об уровне обслуживания Azure][azure-sla].

Для расчета используется период 30 дней в месяц или 43 200 минут. Таким образом, время простоя 0,05 % соответствует 21,6 мин. Обычно доступность различных служб вычисляется следующим образом:
...
Для виртуальных машин, два и более экземпляра которых развернуты в одной **группе доступности**, мы гарантируем доступность подключения хотя бы к одному экземпляру на уровне не менее 99,95 %.
...
**Домены сбоя** гарантируют, что виртуальные машины развернуты на разном оборудовании, которое не имеет общего источника питания и сетевого коммутатора. Незапланированный простой серверов, отключение сетевого коммутатора или источника питания влияют только на одну виртуальную машину.

**Домены обновления** гарантируют, что разные виртуальные машины не будут перезагружены одновременно во время планового обслуживания инфраструктуры Azure (одновременно перезагружается только одна виртуальная машина).
...
Группа доступности используется для достижения высокого уровня доступности для:

* избыточных серверов приложений SAP;  

* кластеров с двумя или несколькими узлами (например, виртуальные машины), защищающих единые точки отказа, такие как экземпляр SAP (A)SCS и СУБД.

### <a name="virtual-machine-vm-planned-and-unplanned-maintenance"></a>Запланированное и незапланированное обслуживание виртуальной машины
...
## <a name="utilizing-azure-infrastructure-ha-to-achieve-sap-application-higher-availability"></a>Использование высокой доступности инфраструктуры Azure для повышения доступности приложений SAP

Если вы решили не использовать такие функциональные возможности, как отказоустойчивая кластеризация Windows Server (WSFC) или Pacemaker для Linux (поддерживается только для SLES 12 или более поздней версии), для защиты системы SAP от запланированных и незапланированных простоев инфраструктуры физического сервера и всей платформы Azure можно использовать перезапуск виртуальной машины Azure.

Подробное описание этого подхода см. в документе [Использование перезапуска виртуальной машины в инфраструктуре Azure для повышения доступности системы SAP][sap-higher-availability].
...
Ниже подробно описано, как добиться высокой доступности для всех трех важных компонентов системы SAP.

### <a name="high-availability-for-sap-application-servers"></a>Высокая доступность серверов приложений SAP

> Этот раздел применим для:
>
...
![Рис. 2. Высокая доступность серверов приложений SAP в группе доступности Azure][planning-guide-figure-3000]
_**. Рис. 2. Высокая доступность серверов приложений SAP в группе доступности Azure**_. Дополнительные сведения см. в статье [Управление доступностью виртуальных машин Windows в Azure][virtual-machines-manage-availability].


Группы доступности Azure описаны в разделе [Группы доступности Azure][planning-guide-3.2.3] статьи о планировании и внедрении SAP NetWeaver на виртуальных машинах Azure.
...
> [!IMPORTANT]
>
> Для установки SAP с высокой доступностью рекомендуется использовать управляемые диски Azure, так как они автоматически соответствуют группам доступности виртуальной машины, к которой они подключены, и таким образом повышают ее доступность и доступность служб, выполняющихся на ней.  
>


### <a name="high-availability-architecture-for-the-sap-ascs-instance"></a>Архитектура высокой доступности экземпляра SAP (A)SCS

### <a name="high-availability-architecture-for-the-sap-ascs-instance-on-windows"></a>Архитектура высокой доступности для экземпляра SAP (A)SCS на Windows


> ![Windows][Logo_Windows] Windows
>

Вы можете использовать **отказоустойчивый кластер Windows Server** (**WSFC**) для защиты экземпляра SAP (A)SCS. Есть два возможных решения:

* Кластеризация экземпляра SAP (A)SCS, используя **кластеризованные общие диски**.

   Дополнительные сведения об архитектуре высокой доступности с кластеризованными общими дисками см. в статье [Кластеризация экземпляра SAP (A)SCS на отказоустойчивом кластере Windows с помощью общего диска кластера в Azure][sap-high-availability-guide-wsfc-shared-disk].   

* Кластеризация экземпляра SAP (A)SCS, используя **файловый ресурс**.

  Дополнительные сведения об архитектуре высокой доступности см. в статье [Кластеризация экземпляра SAP (A)SCS в отказоустойчивом кластере Windows с помощью файлового ресурса в Azure][sap-high-availability-guide-wsfc-file-share].
...
Дополнительные сведения о кластеризации экземпляра SAP (A)SCS с помощью платформы кластера SUSE Linux Enterprise Server см. в статье [Руководство по обеспечению высокого уровня доступности виртуальных машин Azure для SAP NetWeaver на SUSE Linux Enterprise Server для приложений SAP][sap-suse-ascs-ha].

### <a name="sap-netweaver-multi-sid-configuration-for-clustered-sap-ascs-instance"></a>Конфигурации SAP NetWeaver с несколькими ИД безопасности для кластеризованного экземпляра SAP (A)SCS

> ![Windows][Logo_Windows] Windows
>
>В настоящее время несколько ИД безопасности поддерживается только для **отказоустойчивого кластера Windows Server (WSFC)**. Несколько ИД безопасности поддерживается при использовании **файлового ресурса** и **общего диска**.
>

Дополнительные сведения об архитектуре высокой доступности с несколькими ИД безопасности см. в этих статьях:
...
### <a name="high-availability-dbms-instance"></a>Высокий уровень доступности экземпляра СУБД
...
Дополнительные сведения о кластеризации **СУБД SAP HANA** в Azure посредством модели развертывания с помощью Azure Resource Manager можно найти в следующей статье:

* [Высокий уровень доступности SAP HANA на виртуальных машинах Azure][sap-hana-ha]
