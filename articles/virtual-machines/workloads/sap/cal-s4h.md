...
> [!NOTE]
Прежде чем продолжить, проверьте, есть ли у вас квоты ядер Azure для виртуальных машин Azure серии H. Сейчас SAP CAL использует виртуальные машины Azure серии H для развертывания некоторых решений на основе SAP HANA. Ваша подписка может не иметь квот ядер для серии Н. В этом случае обратитесь в службу поддержки Azure, чтобы получить квоту на не менее 16 ядер серии H.
