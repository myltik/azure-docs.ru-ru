---
title: "Кластеризация экземпляра SAP (A)SCS в отказоустойчивом кластере Windows с использованием общего диска кластера в Azure | Документация Майкрософт"
description: "Кластеризация экземпляра SAP (A)SCS в отказоустойчивом кластере Windows с помощью общего диска кластера."
...
# <a name="clustering-sap-ascs-instance-on-windows-failover-cluster-using-cluster-shared-disk-on-azure"></a>Кластеризация экземпляра SAP (A)SCS в отказоустойчивом кластере Windows с помощью общего диска кластера в Azure
...
### <a name="sap-ascs-ha-with-cluster-shared-disks"></a>Высокодоступная система SAP (A)SCS с общими дисками кластера
В **Windows** экземпляр **SAP (A)SCS** содержит не только **центральные службы SAP**, **сервер сообщений SAP** и **серверные процессы постановки в очередь**, но и файлы **глобального узла SAP**, используемые для хранения центральных файлов для всей системы SAP.

Таким образом имеются следующие два компонента экземпляра SAP (A)SCS:

* **центральные службы SAP** с:
    * двумя **процессами**, сервером сообщений и сервером постановки в очередь, и **<имя_виртуального_узла_(A)SCS>**, используемое для доступа к этим двум процессам;
    * **структура файлов**: S:\usr\sap\\&lt;SID&gt;\(A)SCS<InstanceNumber>;


* файлы **глобального узла SAP** с **файловым ресурсом sapmnt**:
    * **структура файлов**: S:\usr\sap\\&lt;SID&gt;\SYS\.;
    * **файловый ресурс sapmnt** обеспечивает доступ к этим глобальным файлам S:\usr\sap\\&lt;SID&gt;\SYS\. с помощью UNC-пути:

     \\\\<имя_виртуального_узла_(A)SCS>\sapmnt\\&lt;SID&gt;\SYS\..


![Рисунок 2. Процессы, структура файлов и файловый ресурс sapmnt глобального узла экземпляра SAP (A)SCS][sap-ha-guide-figure-8001]

_**Рисунок 2.** Процессы, структура файлов и файловый ресурс sapmnt глобального узла экземпляра SAP (A)SCS_

В параметрах высокого уровня доступности мы кластеризуем экземпляры SAP (A)SCS. Мы используем **кластерные общие диски** (диск S:\ в нашем примере) для размещения файлов SAP (A)SCS и файлов глобального узла SAP.

![Рисунок 3. Архитектура высокодоступной системы SAP (A)SCS с общим диском][sap-ha-guide-figure-8002]

_**Рисунок 3.** Архитектура высокодоступной системы SAP (A)SCS с общим диском_

> [!IMPORTANT]
>Так как эти два компонента выполняются в одном экземпляре SAP (A)SCS:
>* Для доступа к процессам сервера сообщений и сервера постановки в очередь SAP, а также к файлам глобального узла SAP через файловый ресурс sapmnt используется одинаковое значение **<имя_виртуального_узла_(A)SCS>**.
>* **Общий диск кластера S:\** используется ими совместно.
>


![Рисунок 4. Архитектура высокодоступной системы SAP (A)SCS с общим диском][sap-ha-guide-figure-8003]

_**Рисунок 4.** Архитектура высокодоступной системы SAP (A)SCS с общим диском_

### <a name="shared-disk-in-azure-with-sios-datakeeper"></a>Создание общего диска в Azure с помощью SIOS DataKeeper
...
Можно создать ресурс общего диска для кластера, следуя приведенным ниже инструкциям.
...
_**Рисунок 5.** Конфигурация отказоустойчивой кластеризации Windows Server в Azure с использованием SIOS DataKeeper_
...
* [Установка SAP NetWeaver высокого уровня доступности с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS][sap-high-availability-installation-wsfc-shared-disk]
