---
title: "Подготовка инфраструктуры Azure для SAP высокого уровня доступности с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS | Документация Майкрософт"
description: "Сведения о подготовке инфраструктуры Azure для SAP высокого уровня доступности с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS"
...
---
# <a name="azure-infrastructure-preparation-for-sap-ha-using-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance"></a>Подготовка инфраструктуры Azure для SAP высокого уровня доступности с использованием отказоустойчивого кластера Windows и общего диска для экземпляра SAP (A)SCS
...
В этом документе описываются шаги подготовки инфраструктуры Azure, необходимые для установки и настройки высокодоступной системы SAP в **отказоустойчивом кластере Windows (WSFC)** с использованием **общего диска кластера** в качестве варианта для кластеризации экземпляра SAP (A)SCS.
...
3.  Чтобы добавить объект имени кластера в список, нажмите кнопку **Добавить**. Измените фильтр для добавления дополнительных объектов-компьютеров, как показано на рис. выше.

  ![Рис. 21. Изменение типа объекта для добавления компьютеров][sap-ha-guide-figure-3020]

  _**Рис. 21.** Изменение типа объекта для добавления компьютеров_

  ![Рис 22. Установка флажка "Компьютеры"][sap-ha-guide-figure-3021]

  _**Рис 22.** Установка флажка **Компьютеры**_

4.  Введите объект имени кластера, как показано на **рис. Изменение типа объекта для добавления компьютеров**. Так как запись уже создана, можно изменить разрешения, как показано на **рис. Назначение разрешений для общего ресурса для объекта имени кластера**.
...
5.  Выберите нужные изменения и нажмите кнопку **Далее**. Конфигурацию кластера необходимо перенастроить, как показано ниже.  
...
- Первый способ — с помощью мастера добавления ролей и компонентов в Windows, как показано ниже.
...
1.  Запустите инструмент управления и настройки DataKeeper, а затем щелкните **Подключение к серверу**. (Этот параметр обведен красным.)
...
5.  Определите имя, TCP/IP-адрес и дисковый том целевого узла.
  ...
  ![Рис. 41. Определение базовых данных для узла, который должен быть текущим целевым узлом][sap-ha-guide-figure-3041]

  _**Рис. 41.** Определение базовых данных для узла, который должен быть текущим целевым узлом_
...
8.  Определите, следует ли том, реплицируемый с помощью задания репликации, представить в конфигурации кластера WSFC как общий диск. Для конфигурации SAP ASCS/SCS выберите **Да**, чтобы кластер Windows воспринимал реплицируемый том как общий диск, который можно использовать в качестве тома кластера.
  ...
  Теперь диск отображается в диспетчере отказоустойчивости кластеров как диск DataKeeper, как показано ниже.
