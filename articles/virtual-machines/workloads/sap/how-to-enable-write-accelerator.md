...
>[!NOTE]
> На этом этапе ускоритель записи Azure предоставляется в общедоступной предварительной версии, и чтобы его использовать, необходимо включить идентификатор подписки Azure в список разрешенных.

Ускоритель записи Azure доступен в общедоступной предварительной версии в следующих регионах:
...
- Для кэширования диска класса Premium необходимо установить значение "None". Все другие режимы кэширования не поддерживаются.
...
Далее нужно обновить JSON-файл и включить ускоритель записи на диске с именем "log1". Это можно сделать, добавив этот атрибут в JSON-файл после записи в кэш диска. 
