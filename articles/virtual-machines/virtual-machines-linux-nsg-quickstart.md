...
Чтобы открыть порт или создать конечную точку в Azure, создайте фильтр сети, который разрешает трафик для выбранного порта в подсети или сетевого интерфейса виртуальной машины. Эти фильтры, контролирующие входящий и исходящий трафик, добавляются в группу безопасности сети и присоединяются к ресурсу, который будет получать трафик. Давайте используем распространенный пример веб-трафика через порт 80.
