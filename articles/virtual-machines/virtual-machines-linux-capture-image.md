...
4. Введите **Exit**, чтобы закрыть SSH-клиент.
	...
	>[AZURE.NOTE]Далее предполагается, что вы уже [установили Azure CLI](../xplat-cli-install.md) на клиентском компьютере. Также все действия можно выполнить на [портале Azure][].
...
9.	Новый образ станет доступным в списке образов, которые можно использовать для настройки новых виртуальных машин. Его можно просмотреть с помощью следующей команды:
	...
	На [портале Azure][] он будет показан в списке **ОБРАЗЫ**.

	![Успешная запись образа](./media/virtual-machines-linux-capture-image/VMCapturedImageAvailable.png)


## Дальнейшие действия
Образ готов к использованию для создания виртуальных машин. Вы можете использовать команду Azure CLI `azure vm create` и указать имя образа, который только что создали. Сведения о команде см. в статье [Управление службами Azure с помощью интерфейса командной строки Azure на компьютерах Mac, Linux и Windows](virtual-machines-command-line-tools.md). Кроме того, с помощью [портала Azure][] можно создать настраиваемую виртуальную машину, используя метод **Из коллекции** и выбрав образ, который вы только что создали. Подробнее см. в разделе [Как создать настраиваемую виртуальную машину][].
