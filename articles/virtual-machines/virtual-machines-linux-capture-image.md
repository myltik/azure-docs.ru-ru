...
Обычно шаблон диспетчера ресурсов используют для создания виртуальной машины из образа. Однако виртуальную машину можно создать и _принудительно_ — с помощью команды **azure vm create** с параметром **-Q** (**--image-urn**). В случае использования данного метода также передается параметр **-d** (**--os-disk-vhd**), чтобы указать расположение VHD-файла операционной системы для новой виртуальной машины. Он должен находиться в контейнере виртуальных жестких дисков учетной записи хранения, где хранится VHD-файл образа. Эта команда автоматически копирует VHD для новой виртуальной машины в контейнер виртуальных жестких дисков.
