...
Использование интерфейса командной строки Azure (Azure CLI) для записи образа виртуальной машины под управлением Linux, чтобы в дальнейшем использовать его как шаблон Azure Resource Manager для создания других виртуальных машин. В шаблоне указаны диск операционной системы и диски с данными, присоединенные к виртуальной машине. В нем не указаны ресурсы виртуальной сети, которые необходимы для создания виртуальной машины менеджера ресурсов Azure, поэтому в большинстве случаев их нужно настраивать отдельно перед созданием другой виртуальной машины, которая использует шаблон.
...
>[AZURE.TIP] Чтобы найти расположение образа, откройте шаблон JSON-файла. В разделе **storageProfile** в контейнере **system** найдите **URI** **образа**. Например, URI для образа диска ОС может выглядеть так: `https://xxxxxxxxxxxxxx.blob.core.windows.net/system/Microsoft.Compute/Images/vhds/<your-image-prefix>-osDisk.xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.vhd`.
