<properties 
 ...
 description="Статья описывает добавление, поиск, обновление и удаление расширений." 
...
Расширения ВМ Azure реализуют реакции на события или функции, которые помогают другим программам работать на виртуальных машинах Azure (например, расширение **WebDeployForVSDevTest** позволяет Visual Studio использовать веб-развертывание решений на ВМ Azure) или предоставляют возможность взаимодействия с виртуальной машиной для поддержки другой реакции на события (например, вы можете использовать расширения доступа к виртуальной машине через клиенты Powershell, xplat-cli или REST для сброса или изменения значений удаленного доступа на ВМ Azure).
...
Расширения ВМ Azure (полный список расширений с поддерживаемыми ими функциям см. в статье [Расширения и компоненты виртуальных машин Azure](https://msdn.microsoft.com/library/dn606311.aspx)). Вы можете найти расширения и дополнительную информацию, используя:

-   PowerShell
-   Межплатформенный интерфейс Azure (xplat-cli)
...
Некоторые модули имеют собственные команды xplat-cli (например, расширение ВМ Docker), что упрощает их настройку. Следующие команды будут работать для всех расширений ВМ.
...
Например, в следующем примере кода показано, как с помощью xplat-cli команды **azure vm extension list** отобразить сведения о расширении **IaaSDiagnostics**, и приводится пример использования параметра **—-json** для получения полной информации.
...
-   Для экземпляров виртуальных машин можно использовать командлет **Get AzureServiceAvailableExtension**.
