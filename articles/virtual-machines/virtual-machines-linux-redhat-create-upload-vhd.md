...
- При установке операционной системы Linux рекомендуется использовать стандартные разделы, а не диспетчер логических разделов (LVM) (который часто используется по умолчанию при установке). Это поможет избежать конфликтов имен LVM при клонировании виртуальных машин, особенно когда диск с ОС должен быть подключен к другой виртуальной машине для устранения неполадок. При желании можно использовать LVM или RAID на дисках данных.
...
### <a id="rhel7xhyperv"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 в диспетчере Hyper-V###
...
8.	Внеся изменения в файл `/etc/default/grub`, выполните следующую команду, чтобы повторно создать конфигурацию grub:
...
10. Добавьте модули Hyper-V в initramfs:
    ...
    Измените `/etc/dracut.conf` и добавьте содержимое: add\_drivers+="hv\_vmbus hv\_netvsc hv\_storvsc"
...
### <a id="rhel7xkvm"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 в KVM###
...
9.	Внеся изменения в файл `/etc/default/grub`, выполните следующую команду, чтобы повторно создать конфигурацию grub:
...
### Предварительные требования
В этом разделе предполагается, что вы уже установили виртуальную машину RHEL в VMWare. Дополнительные сведения об установке операционной системы в VMWare см. в [руководстве по установке гостевой операционной системы на виртуальной машине VMWare](http://partnerweb.vmware.com/GOSIG/home.html).
...
### <a id="rhel7xvmware"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 в VMWare###
...
6.	Внеся изменения в файл `/etc/default/grub`, выполните следующую команду, чтобы повторно создать конфигурацию grub:
...
### <a id="rhel7xkickstart"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 в файле kickstart###
...
3.	В диспетчере Hyper-V создайте новую виртуальную машину. На странице **Подключение виртуального жесткого диска** выберите параметр **Подключить виртуальный жесткий диск позже** и выполните инструкции в мастере создания виртуальной машины.

4.	Откройте параметры виртуальной машины:

    а. Загрузите новый виртуальный жесткий диск на виртуальную машину. Не забудьте выбрать параметры **Формат VHD** и **Фиксированный размер**.
...
## Дальнейшие действия
Теперь виртуальный жесткий диск Red Hat Enterprise Linux можно использовать для создания новых виртуальных машин Azure. Если вы загружаете файл VHD в Azure впервые, см. шаги 2 и 3 в статье [Создание и загрузка виртуального жесткого диска, содержащего операционную систему Linux](virtual-machines-linux-classic-create-upload-vhd.md).
