...
Самый быстрый способ приступить к работе с [Docker] — перейти в Azure Marketplace и создать виртуальную машину с помощью шаблона образа **Docker on Ubuntu Server**, созданного [Canonical] при сотрудничестве с [MSOpenTech]. При этом создается виртуальная машина Ubuntu Server и автоматически устанавливается [расширение виртуальных машин Docker](virtual-machines-linux-dockerextension.md) вместе с **последней** версией ядра Docker, которая предварительно установлена и запущена в Azure.
...
> [AZURE.NOTE]Виртуальная машина, созданная с помощью шаблона Azure Marketplace, не имеет удаленного API Docker для управления удаленным клиентом Docker. Чтобы включить удаленное управление узлом Docker в этой виртуальной машине, см. [статью о запуске Docker с использованием HTTPS](https://docs.docker.com/articles/https/) или выполните действия, описанные в статье [Использование расширения виртуальных машин Docker на классическом портале Azure](virtual-machines-linux-classic-portal-use-docker.md) или [Использование расширения виртуальных машин Docker в CLI Azure](virtual-machines-linux-classic-cli-use-docker.md).
<!-- -->
Если нужно автоматизировать виртуальную машину Docker Azure из Windows, можно [установить панель элементов Docker Toolbox](https://docs.docker.com/installation/windows/) или получить Docker.exe [из Chocolatey](https://chocolatey.org/packages/docker).

## Вход на портал

Эта часть не представляет никаких сложностей, если у вас есть учетная запись Azure. [Кроме того, вы можете легко получить такую учетную запись совершенно бесплатно](https://azure.microsoft.com/pricing/free-trial/)!
...
> ![Выбор образа Ubuntu Docker на баннере](./media/virtual-machines-linux-classic-docker-quickstart/CreateNewDockerBanner.png)

2. Если этого не происходит, найдите его в коллекции образов:

> ![Поиск изображения в коллекции образов](./media/virtual-machines-linux-classic-docker-quickstart/DockerOnUbuntuServerMSOpenTech.png)
...
> ![Настройка экземпляра виртуальной машины](./media/virtual-machines-linux-classic-docker-quickstart/CreateVMDockerUbuntuPwd.png)

4. Дождитесь завершения выполнения, это не должно занять много времени.

> ![Запуск образа Docker на портале](./media/virtual-machines-linux-classic-docker-quickstart/DockerUbuntuRunning.png)
...
> ![Подключение по протоколу SSH](./media/virtual-machines-linux-classic-docker-quickstart/SSHToDockerUbuntu.png)
...
> ![Извлечение образов](./media/virtual-machines-linux-classic-docker-quickstart/DockerPullSmallImages.png)
