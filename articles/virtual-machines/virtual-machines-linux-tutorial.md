<properties
	...
	description="Узнайте, как с помощью образа из Azure и интерфейса командной строки Azure создать виртуальную машину Azure под управлением Linux или управлением Ubuntu."
	keywords="виртуальная машина linux, Azure Linux, azure ubuntu" 
...
Вы можете легко создать виртуальную машину (VM) Azure, работающую под управлением ОС Linux, с помощью командной строки или из портала. В этом руководстве показано, как с помощью интерфейса командной строки (CLI) Azure для Mac, Linux и Windows можно быстро создать в Azure виртуальную машину Ubuntu Server, подключиться к ней с помощью **SSH**, а затем создать и подключить новый диск. В этом разделе описывается виртуальная машина Ubuntu Server, но вы можете также создавать виртуальные машины Linux, используя [собственные образы в качестве шаблонов](virtual-machines-linux-create-upload-vhd.md).
...
Отлично. Теперь [войдите в систему с помощью рабочего или учебного идентификатора](../xplat-cli-connect.md#use-the-log-in-method). Для этого введите `azure login` и следуйте подсказкам по интерактивному входу в учетную запись Azure.

> [AZURE.NOTE]Если вы знаете, что двухфакторная проверка подлинности не включена, но имеется рабочий или учебный идентификатор, вы можете войти альтернативным способом, используя рабочий или учебный идентификатор в комбинации с `azure login -u`. Если у вас нет такого идентификатора, его можно [создать в личной учетной записи Майкрософт](resource-group-create-work-id-from-personal.md).
...
Теперь создайте свою виртуальную машину, введя `azure vm quick-create`, и следуйте появляющимся подсказкам для ввода остальных параметров. Используйте имя только что созданной группы ресурсов, а в качестве значения **ImageURN** используйте `canonical:ubuntuserver:14.04.2-LTS:latest`. Вы должны получить примерно следующее: Обратите внимание, что команда `azure vm quick-create` запрашивает основные сведения, необходимые для создания и размещения виртуальной машины Linux и подключения к ней, включая следующее:
...
Подключение к виртуальным машинам Linux обычно выполняется по протоколу **SSH**.

> [AZURE.NOTE]В этом разделе для подключения к виртуальной машине используются имена пользователей и пароли; чтобы использовать пары открытых и закрытых ключей для взаимодействия со своей виртуальной машиной, см. [Использование SSH с Linux в Azure](virtual-machines-linux-use-ssh-key.md). Чтобы изменить параметры **SSH**-подключения к виртуальным машинам, созданным с помощью команды `azure vm quick-create`, воспользуйтесь командой `azure vm reset-access`. Она позволяет полностью сбросить настройки **SSH**-доступа, добавить или удалить пользователей, а также добавить файлы открытого ключа для обеспечения безопасного доступа. В этой статье для краткости используется доступ по протоколу **SSH** с помощью имени пользователя и пароля.
...
Помните, для того чтобы после перезапуска виртуальная машина получила доступ к вашему новому диску, информацию о нем необходимо прописать в файле [fstab](http://en.wikipedia.org/wiki/Fstab). При необходимости вы можете добавить еще несколько дисков и [настроить RAID](virtual-machines-linux-configure-raid.md).
