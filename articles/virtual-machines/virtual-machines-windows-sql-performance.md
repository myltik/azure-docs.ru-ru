...
При создании образов SQL Server [рассмотрите возможность подготовки виртуальных машин на портале Azure](virtual-machines-windows-portal-sql-server-provision.md), чтобы воспользоваться такими преимуществами, как применение хранилища класса Premium по умолчанию, и другими возможностями, такими как автоматическое исправление, автоматическая архивация и конфигурации AlwaysOn.
...
|[Хранилище](#storage-guidance)|Используйте [хранилище класса Premium](../storage/storage-premium-storage.md).<br/><br/>Храните [учетную запись хранения](../storage/storage-create-storage-account.md) и виртуальную машину SQL Server в одном и том же регионе.<br/><br/>Отключите [геоизбыточное хранилище](../storage/storage-redundancy.md) \(георепликацию\) Azure в учетной записи хранения.|
|[диски;](#disks-guidance)|Используйте как минимум 2 [диска P30](../storage/storage-premium-storage.md#scalability-and-performance-targets-when-using-premium-storage) \(1 для файлов журнала, 1 для файлов данных и TempDB\).<br/><br/>Избегайте использования дисков операционной системы или временных дисков для хранения базы данных или журналов.<br/><br/>Включите кэширование операций чтения на дисках, где размещаются файлы данных и базы данных TempDB.<br/><br/>Не включайте кэширование на дисках, где находится файл журнала.<br/><br/>Обеспечьте чередование нескольких дисков данных Azure для увеличения пропускной способности ввода-вывода.<br/><br/>Выполняйте форматирование с использованием задокументированных размеров кластеров.|
...
- **Диск ОС** — при создании виртуальной машины Azure платформа подключает к ней по меньшей мере один диск \(обозначенный буквой **C**\) для использования в качестве диска операционной системы. Этот диск представляет собой VHD-файл, сохраненный как страничный BLOB-объект в хранилище.
...
- **Диски данных** — к виртуальной машине можно подключить дополнительные диски в качестве дисков данных, которые будут сохранены в хранилище как страничные BLOB-объекты.
...
Для виртуальных машин серии D, Dv2 и G храните базу данных TempDB и \(или\) расширения буферного пула на диске **D**. Временный диск на этих виртуальных машинах является SSD-диском. Это позволяет повысить производительность рабочих нагрузок, которые интенсивно используют временные объекты или имеют не помещающиеся в памяти рабочие наборы.

Для виртуальных машин \(серии D, Dv2 и G\), которые поддерживают хранилище класса Premium, рекомендуется хранить базу данных TempDB и \(или\) расширения буферного пула на диске, поддерживающем хранилище класса Premium, с включенным кэшированием чтения. Существует одно исключение: если работа базы данных TempDB связана с активным использованием функции записи, то для достижения более высокой производительности можно сохранить базу данных TempDB на локальном диске **D**.

### Диски данных

- **Использование дисков данных для файлов данных и журналов**: используйте не менее 2 [дисков P30](../storage/storage-premium-storage.md#scalability-and-performance-targets-when-using-premium-storage) хранилища класса Premium, где один диск содержит файлы журнала, а другой — файлы данных и базу данных TempDB.
...
- **Чередование дисков**: для повышения пропускной способности можно добавить дополнительные диски данных и использовать чередование дисков. Чтобы определить количество дисков данных, необходимые проанализировать число доступных операций ввода-вывода для ваших дисков данных и журналов. Эта информация содержится в таблицах с числом операций ввода-вывода в секунду для [размера виртуальной машины](virtual-machines-windows-sizes.md) и размера диска в следующем разделе: [Использование хранилища класса Premium для дисков](../storage/storage-premium-storage.md). Придерживайтесь приведенных далее рекомендаций.
	...
	- Если хранилище класса Premium не применяется \(сценарии разработки и тестирования\), рекомендуется добавить максимальное количество дисков данных, поддерживаемое для вашего [размера виртуальной машины](virtual-machines-windows-sizes.md), и использовать чередование дисков.
...
Просмотрите другие разделы, посвященные виртуальным машинам SQL Server, в разделе [Общие сведения об SQL Server на виртуальных машинах Azure](virtual-machines-windows-sql-server-iaas-overview.md).
