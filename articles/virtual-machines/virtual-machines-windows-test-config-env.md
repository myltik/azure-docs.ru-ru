...
В этой статье содержатся пошаговые указания по созданию тестовой среды с базовой конфигурацией в виртуальной сети Microsoft Azure с использованием виртуальных машин, созданных в Resource Manager.
...
Если у вас еще нет учетной записи Azure, вы можете зарегистрироваться на странице [Пробная версия Azure](https://azure.microsoft.com/pricing/free-trial/), чтобы получить бесплатную пробную версию. Если у вас есть подписка MSDN или Visual Studio, ознакомьтесь с разделом [Ежемесячная сумма денег на счете в Azure для подписчиков Visual Studio](https://azure.microsoft.com/pricing/member-offers/msdn-benefits-details/).
...
Настройте свою подписку Azure. Замените все содержимое внутри кавычек, включая знаки < и >, правильными именами.
...
Создайте группу ресурсов, выполнив следующие команды. Замените все содержимое внутри кавычек, включая знаки < и >, правильными именами.
...
Затем необходимо создать виртуальную сеть TestLab, в которой будет размещена подсеть базовой конфигурации Corpnet, и защитить ее с помощью группы безопасности сети.
...
Обратите внимание, что эта команда требует ввести пароль учетной записи User1. *Выберите надежный пароль*, так как эта учетная запись будет использоваться для подключения к удаленному рабочему столу на всех компьютерах в домене CORP. Чтобы проверить его надежность, см. раздел [Проверка надежности пароля. Использование надежных паролей](https://www.microsoft.com/security/pc-security/password-checker.aspx). Запишите пароль учетной записи User1 и сохраните его в надежном месте.
...
- Создайте [имитацию гибридной облачной среды](virtual-machines-setup-simulated-hybrid-cloud-environment-testing.md).
