<properties
	...
	description="Содержит простые примеры использования fleet и Docker в кластере CoreOS виртуальных машин Linux, созданного с помощью классической модели развертывания в Azure."
...
Вот два кратких примера использования [fleet](https://github.com/coreos/fleet) и [Docker](https://www.docker.com/) для запуска приложений в кластере виртуальных машин[CoreOS].
...
## Пример 1. Hello World с Docker</a>
...
## Пример 2. Высокодоступный сервер nginx</a>
...
>[AZURE.IMPORTANT] Чтобы запустить высокодоступный сервер nginx, необходимо настроить конечную точку HTTP с балансировкой нагрузки на виртуальных машинах (общий порт 80, частный порт 80). Это можно сделать после создания кластера CoreOS с помощью классического портала Azure или команды **azure vm endpoint**. Пошаговые инструкции см. в статье о [настройке набора с балансировкой нагрузки].
...
* Попробуйте выполнить больше функций с трехузловым кластером CoreOS в Azure. Узнайте, как создавать более сложные кластеры и использовать Docker для создания более интересных приложений. Для этого прочитайте [учебник Тима Парка (Tim Park) по CoreOS], [учебник Патрика Чейнзона (Patrick Chanezon) по CoreOS], документацию по [Docker] и [обзор CoreOS].

* Чтобы приступить к работе с fleet и CoreOS в диспетчере ресурсов Azure, воспользуйтесь этим [шаблоном быстрого запуска](https://github.com/Azure/azure-quickstart-templates/tree/master/coreos-with-fleet-multivm).
...
[настройке набора с балансировкой нагрузки]: ../load-balancer/load-balancer-get-started-internet-classic-cli.md
