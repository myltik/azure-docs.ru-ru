...
> [!IMPORTANT]
> После выполнения следующей процедуры создается один большой двоичный объект, размещенный в контейнере по умолчанию /vhds. Этот объект хранится в файле с именем VMName.xxxxxxx.status. Этот файл создается в Azure, только если на виртуальной машине установлены [расширения виртуальных машин](windows/classic/agents-and-extensions.md). Не удаляйте этот оставшийся объект состояния. В дальнейшем эта проблема будет решена.
