...
Более подробные сведения о конфигурации обеих ферм см. в статье [Подробные сведения о конфигурации фермы серверов SharePoint](virtual-machines-sharepoint-farm-config-azure-preview.md).

## Пошаговая настройка
 
Чтобы создать ферму SharePoint с помощью шаблона фермы серверов SharePoint, выполните следующее:

1. На [портале предварительной версии Microsoft Azure](https://portal.azure.com/) выберите **New** (Создать) > **Compute** (Вычисления) > **SharePoint Server Farm** (Ферма серверов SharePoint). Если пункта **SharePoint Server Farm** нет, щелкните **New** (Создать) > **Compute** (Вычисления) > **Azure Marketplace**, введите **SharePoint** в поле **Search Everything** (Поиск везде) и выберите **SharePoint Server Farm** (Ферма серверов SharePoint). 
...
Ферма серверов SharePoint использует диспетчер ресурсов Azure и скрипты для автоматического создания инфраструктуры и конфигураций серверов для этих ферм SharePoint. Дополнительную информацию см. в разделе [Использование Windows PowerShell с диспетчером ресурсов](../powershell-azure-resource-manager.md).
