...
Шаблон быстрого запуска Azure позволяет быстро и без проблем развернуть кластер пакета HPC на портале предварительной версии Azure. Открыв шаблон на портале предварительной версии, вы увидите простой пользовательский интерфейс, где вводятся параметры кластера. Для этого выполните следующие действия.
...
Настройка привязки NetTcp аналогична подключению к локальному кластеру. На виртуальной машине головного узла потребуется открыть несколько конечных точек. На портале Azure выполните следующие действия.
