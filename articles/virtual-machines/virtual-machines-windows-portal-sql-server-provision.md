---
title: Подготовка виртуальной машины SQL Server | Microsoft Docs
...
1. Войдите на [портал Azure](https://portal.azure.com), используя свою учетную запись.
   ...
   > 
   > 
2. На портале Azure нажмите кнопку **Создать**. На портале откроется колонка **Создать**. Ресурсы виртуальной машины SQL Server находятся в магазине Marketplace в группе **Виртуальные машины**.
3. В колонке **Создать** щелкните **Виртуальные машины**.
4. Чтобы просмотреть все доступные образы, в колонке **Виртуальные машины** щелкните **Просмотреть все**.
   
    ![Колонка "Виртуальные машины Azure"](./media/virtual-machines-windows-portal-sql-server-provision/azure-compute-blade.png)
5. В разделе **Серверы баз данных** щелкните **SQL Server**. Возможно, потребуется прокрутить страницу вниз, чтобы найти элемент **Серверы баз данных**. Просмотрите доступные шаблоны SQL Server.
...
3. Браузер загрузит RDP-файл для ВМ. Откройте RDP-файл. ![Удаленное подключение к виртуальной машине SQL](./media/virtual-machines-windows-portal-sql-server-provision/azure-sql-vm-remote-desktop.png)
...
5. В диалоговом окне **Безопасность Windows** выберите **Другая учетная запись**.
6. В поле **Имя пользователя** введите **<имя пользователя>**, которое вы указали во время настройки виртуальной машины. Перед именем необходимо добавить обратную косую черту.
7. Введите **пароль**, ранее настроенный для этой виртуальной машины, и нажмите кнопку **ОК**, чтобы подключиться.
8. Если появится еще одно диалоговое окно **Remote Desktop Connection** (Подключение к удаленному рабочему столу) с запросом на подключение, нажмите кнопку **Да**.
...
> [!INCLUDE [Подключение к SQL Server в диспетчере ресурсов VM](../../includes/virtual-machines-sql-server-connection-steps-resource-manager.md)]
> 
> 
