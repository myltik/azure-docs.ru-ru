...
## <a id="Provision">Подготовка образа VM SQL из коллекции с помощью модели развертывания Resource Manager

1. Войдите на [портал Azure](https://portal.azure.com), используя свою учетную запись.
1. На портале Azure щелкните **+Создать**. Откроется колонка **Создание**. Шаблоны виртуальных VM SQL Server находятся на Marketplace в группе **Вычисления**.
...
1.	Колонка со сведениями содержит описание образа виртуальной машины. В этой колонке можно выбрать модель развертывания. В разделе **Выбор модели развертывания** выберите элемент **Resource Manager** и нажмите кнопку **Создать**. <br/> ![Колонка «Вычисления» в Azure](./media/virtual-machines-windows-portal-sql-server-provision/azure-compute-sql-deployment-model.png) <br/>
...
## 1\. Настройка основных параметров
В колонке **Создание виртуальной машины** в разделе **Основные** укажите следующую информацию.
...
* В поле **Группа ресурсов** введите имя для группы ресурсов. Чтобы использовать существующую группу ресурсов, нажмите кнопку **Выбрать существующую**. Группа ресурсов — это коллекция связанных служб в Azure. Дополнительные сведения о группах ресурсов см. в статье [Общие сведения об Azure Resource Manager](../resource-group-overview.md). Убедитесь, что выбрано **Расположение**, соответствующее вашим требованиям.
...
В колонке **Создание виртуальной машины** в разделе **Размер** выберите размер виртуальной машины. На портале Azure отобразятся рекомендуемые размеры. Дополнительные сведения о размерах виртуальных машин см. в разделе [Размеры виртуальных машин](virtual-machines-linux-sizes.md). Размеры зависят от выбранного шаблона. Размер определяет ежемесячные затраты на работу виртуальной машины. Выберите размер VM для сервера. Описание размеров VM SQL Server см. в разделе [Рекомендации по оптимизации производительности SQL Server в виртуальных машинах Azure](virtual-machines-windows-classic-sql-perf.md).
...
В колонке **Создание виртуальной машины** в разделе **Настройка SQL Server** настройте конкретные параметры и оптимизации SQL Server. Для SQL Server можно настроить следующие параметры:
...
### Соединение
В разделе **Соединение SQL** выберите вариант **Общий (Интернет)** для подключения к SQL Server с машин и служб в Интернете. Если этот параметр выбран, Azure автоматически настроит брандмауэр и группу безопасности сети, чтобы разрешить трафик через порт 1433. <br/>![Подключение к SQL ARM](./media/virtual-machines-windows-portal-sql-server-provision/azure-sql-arm-connectivity-alt.png) <br/>

Чтобы подключиться к SQL Server через Интернет, также необходимо включить проверку подлинности SQL Server.

>[AZURE.NOTE]В целях безопасности ограничьте исходный порт с помощью группы безопасности сети. Дополнительные сведения см. в разделе [Что такое группа безопасности сети?](../virtual-network/virtual-networks-nsg.md)
...
- **Закрытый (в виртуальной сети)** для подключения к SQL Server с машин или служб в той же виртуальной сети.


Значение в поле **Порт** по умолчанию — 1433. Вы можете указать другой номер порта. Дополнительные сведения см. в разделе [Подключение к виртуальной машине SQL Server (Resource Manager) | Microsoft Azure](virtual-machines-windows-sql-connect.md).



### Аутентификация
Если требуется проверка подлинности SQL Server, выберите вариант **Включить** возле параметра **Проверка подлинности SQL**.
...
Если проверка подлинности SQL Server включена, укажите **Имя для входа** и **Пароль**. Это имя пользователя будет именем для входа во время проверки подлинности SQL Server и членом фиксированной серверной роли sysadmin. Дополнительные сведения о режимах проверки подлинности см. в разделе [Выбор режима проверки подлинности](http://msdn.microsoft.com/library/ms144284.aspx). По умолчанию SQL Server не поддерживает проверку подлинности SQL Server. В этом случае локальные администраторы на виртуальной машине могут подключаться к экземпляру SQL Server.
...
- необходимость шифрования резервной копии. Для шифрования архивной копии выберите пункт **Включить**. Если автоматические резервные копии шифруются, укажите пароль. Azure создает сертификат для шифрования резервных копий и использует указанный пароль для защиты этого сертификата.
...
<br/>![Резервное копирование SQL ARM](./media/virtual-machines-windows-portal-sql-server-provision/azure-sql-arm-autobackup.png) <br/>
 ...
 Дополнительную информацию см. в статье [Автоматическое резервное копирование для SQL Server на виртуальных машинах Azure](virtual-machines-windows-classic-ps-sql-backup.md).
...
Дополнительные сведения см. в разделе [Настройка интеграции с хранилищем ключей Azure для SQL Server на виртуальных машинах Azure](virtual-machines-windows-classic-ps-sql-keyvault.md).

## 5\. Просмотр сводки
Просмотрите сводные данные и нажмите кнопку **ОК**, чтобы создать SQL Server, группу ресурсов и ресурсы, указанные для этой виртуальной машины. Развертывание можно отслеживать на портале Azure. Если нажать кнопку **Уведомления** в верхней части окна, будут показаны основные данные о состоянии развертывания.
...
1.	В диалоговом окне **Безопасность Windows** выберите пункт **Другая учетная запись**. В поле **Имя пользователя** введите <machine name><имя\_пользователя>, которое вы указали во время настройки виртуальной машины.
