...
В результате выполнения команды ping должны быть получены четыре успешных ответа с IP-адреса 10.0.0.1. Если используется *имитация гибридной облачной конфигурации*, вы увидите четыре успешных ответа, полученных с IP-адреса 10.0.0.4. Это проверка трафика через VPN-подключение "сеть — сеть" или "виртуальная сеть — виртуальная сеть".
