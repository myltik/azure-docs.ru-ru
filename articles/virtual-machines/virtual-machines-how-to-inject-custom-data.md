...
> [AZURE.NOTE]В этой статье рассказывается, как вставить пользовательские данные с помощью виртуальной машины, созданной с помощью стека вычислений для управления службами Azure. Сведения о том, как использовать стек вычислений в режиме управления ресурсами Azure, см. в [этом примере шаблона](https://github.com/Azure/azure-quickstart-templates/tree/master/101-vm-customdata).
...
В образах Ubuntu Cloud для настройки диска ресурсов (который также называется временным) и раздела подкачки необходимо использовать пакет cloud-init. Дополнительные сведения см. на следующей странице на вики-сайте по Ubuntu:
