...
Чтобы создать виртуальную машину RedHat Enteprise Linux 7.2, мы используем ту же команду, что и выше, но с псевдонимом RHEL. Ниже показана каждая ожидаемая командная строка с выходными данными.
...
Чтобы развернуть виртуальную машину RedHat Enterprise Linux 7.2, мы используем `RHEL` для параметра **ImageURN** (`-Q`). (Эти семь псевдонимов представляют малую долю ОС, доступных в Azure. Дополнительные образы можно найти в Marketplace, используя [поиск образа](virtual-machines-linux-cli-ps-findimage.md). Можно также [загрузить собственный настраиваемый образ](virtual-machines-linux-create-upload-generic.md).)

В пошаговом руководстве ниже замените командные строки значениями собственной среды. Здесь используются примеры значений.
...
Теперь вы можете зайти через SSH на виртуальную машину, используя стандартный порт SSH 22 и полное доменное имя (FQDN), указанное в выходных данных выше. (Можно также использовать указанный IP-адрес.)
