---
title: "SAP NetWeaver на виртуальных машинах Linux. Руководство по развертыванию СУБД | Документация Майкрософт"
...
---
# <a name="sap-netweaver-on-azure-virtual-machines-vms--dbms-deployment-guide"></a>SAP NetWeaver на виртуальных машинах Azure. Руководство по развертыванию СУБД
...
В зависимости от того, какой класс хранилища Azure используется — Standard или Premium, (например, при использовании виртуальных машин серии DS или серии GS), в Azure предусмотрены другие квоты, описанные [здесь][virtual-machines-sizes]. При планировании виртуальных жестких дисков Azure вам нужно найти оптимальный баланс между следующими квотами:
...
* **Производительность SQL.** Мы уверены, что размещенные в Microsoft Azure виртуальные машины будут работать очень хорошо по сравнению с другими общедоступными облачными предложениями виртуализации, но отдельные результаты могут отличаться. См. [эту][virtual-machines-sql-server-performance-best-practices] статью.
...
### <a name="ibm-db2-for-linux-unix-and-windows-configuration-guidelines-for-sap-installations-in-azure-vms"></a>Рекомендации по конфигурации IBM DB2 для Linux, UNIX и Windows для установки SAP на виртуальные машины Azure
#### <a name="storage-configuration"></a>Конфигурация хранилища
Все файлы базы данных должны храниться на виртуальных жестких дисках (VHD) с файловой системой. Эти виртуальные жестки диски должны быть подключены к виртуальной машине Azure и созданы на основе хранилища страничных BLOB-объектов Azure (<https://msdn.microsoft.com/library/azure/ee691964.aspx>).
...
Кроме того, можно использовать LVM (диспетчер логических томов) или MDADM, как описано в разделе [Программный RAID-массив][dbms-guide-2.2] этого документа, чтобы создать одно большое логическое устройство из нескольких подключенных дисков VHD.
