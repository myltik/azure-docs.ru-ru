...
Чтобы развернуть шаблон с помощью портала Azure, нужно просто отправить в него URL-адрес. Для развертывания шаблона необходимо указать имя файла, в котором он хранится. Имя можно найти на страницах в коллекции шаблонов либо в репозитории Github. Замените {имя шаблона} в URL-адресе на имя шаблона, который требуется развернуть, и вставьте результат в адресную строку браузера.
...
В колонке "Настраиваемое развертывание" выполните описанные ниже действия.
...
4.	Щелкните **Группа ресурсов** и выберите существующую группу ресурсов. Либо нажмите **Или создать новую**, чтобы создать группу для этого развертывания.
5.	При необходимости щелкните элемент **Расположение** и укажите правильное расположение в Azure.
...
[Установите Azure CLI](../xplat-cli-install.md), выполните вход и убедитесь, что команды диспетчера ресурсов включены. Инструкции см. в статье [Использование интерфейса командной строки Azure на компьютерах Mac, Linux и Windows с диспетчером ресурсов Azure](../xplat-cli-azure-resource-manager.md).
...
При выполнении команды **azure group deployment create** появится запрос на ввод значений для параметров шаблона. В зависимости от шаблона для развертывания в Azure может потребоваться некоторое время.
