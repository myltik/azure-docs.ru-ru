...
Можно присоединять пустые диски и диски с данными. На самом деле диски — это VHD-файлы, которые размещаются в вашей учетной записи хранения Azure. После подключения диска необходимо его инициализировать, чтобы подготовить к использованию.
...
Одни и те же инструкции можно использовать для инициализации нескольких дисков данных, указывая соответствующий идентификатор устройства, как показано ниже.
...
2. Далее вам необходимо найти идентификатор устройства для инициализации диска данных. Это можно осуществить двумя способами.
	...
	а) В окне SSH введите следующую команду.
	...
	![Получение сообщений диска](./media/virtual-machines-linux-classic-attach-disk/DiskMessages.png)
...
3. Чтобы создать новое устройство, введите в окне SSH следующую команду.
...
4. При появлении запроса введите **n** для создания нового раздела.
	...
	![Создание нового устройства](./media/virtual-machines-linux-classic-attach-disk/DiskPartition.png)

5. При появлении запроса введите **p**, чтобы сделать раздел основным. Введите **1**, чтобы сделать его первым разделом, а затем нажмите клавишу ВВОД, чтобы принять значение по умолчанию для цилиндров. В некоторых системах в первом и последнем секторах вместо цилиндра могут отображаться значения по умолчанию. При желании вы можете их принять.


	![Создание раздела](./media/virtual-machines-linux-classic-attach-disk/DiskCylinder.png)
...
6. Введите **p**, чтобы просмотреть сведения о диске, на котором создаются разделы.
	...
	![Просмотр сведений о диске](./media/virtual-machines-linux-classic-attach-disk/DiskInfo.png)
...
7. Введите **w**, чтобы записать параметры данного диска.
	...
	![Запись изменений на диске](./media/virtual-machines-linux-classic-attach-disk/DiskWrite.png)

8. Создайте файловую систему на новом разделе. Добавьте номер секции (1) к идентификатору устройства. Например, для создания раздела ext4 в /dev/sdc1 введите следующее.

		# sudo mkfs -t ext4 /dev/sdc1

	![Создание файловой системы](./media/virtual-machines-linux-classic-attach-disk/DiskFileSystem.png)
...
9. Создайте каталог для подключения новой файловой системы. Например, введите следующую команду.
