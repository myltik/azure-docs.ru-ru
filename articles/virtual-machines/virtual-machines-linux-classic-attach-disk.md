---
title: Подключение диска к виртуальной машине Linux | Microsoft Docs
...
См. статью о том, как [присоединить диск данных, используя модель развертывания с помощью Resource Manager](virtual-machines-linux-add-disk.md).
...
2. Далее вам необходимо найти идентификатор устройства для инициализации диска данных. Это можно осуществить двумя способами.
    ...
    Последний номер в кортеже в каждой строке и есть логический номер устройства (*LUN*). См. `man lsscsi` для получения подробной информации.
...
1. Теперь можно создать файловую систему в новом разделе. Добавьте номер раздела в код устройства (в примере ниже это `/dev/sdc1`). В следующем примере создается раздел ext4 в устройстве /dev/sdc1:
   ...
   > 
   > 
2. Создайте каталог для подключения новой файловой системы с помощью следующей команды:
   
        # sudo mkdir /datadrive
3. Теперь вы можете подключить диск с помощью следующей команды:
...
4. Добавьте новый диск в /etc/fstab:
