<properties 
	pageTitle="Развертывание сервера WordPress на Ubuntu с помощью шаблона диспетчера ресурсов Azure" 
...
# Развертывание сервера WordPress на Ubuntu с помощью шаблона диспетчера ресурсов Azure

Используйте указания в этой статье для развертывания сервера WordPress на базе Ubuntu с помощью шаблона диспетчера ресурсов. Этот шаблон позволяет создать одну виртуальную машину в новой виртуальной сети.
...
В зависимости от шаблона для создания рабочей нагрузки в Azure может потребоваться некоторое время. По завершении вы получите новый сервер WordPress на базе Ubuntu в существующей или новой группе ресурсов.

## Azure PowerShell

Прежде чем начать, убедитесь, что у вас установлена подходящая версия Azure PowerShell, выполнен вход и включен новый режим диспетчера ресурсов. Дополнительную информацию см. [здесь](virtual-machines-deploy-rmtemplates-powershell.md#setting-up-powershell-for-resource-manager-templates).

Введите имя развертывания Azure, новое имя группы ресурсов и расположение центра обработки данных Azure в следующем наборе команд. Удалите все содержимое внутри кавычек, включая знаки < and >.
...
При выполнении команды **New-AzureResourceGroupDeployment** отобразится запрос на ввод значений для ряда параметров. После указания всех значений параметров **New-AzureResourceGroupDeployment** создает и настраивает виртуальные машины.

По завершении выполнения шаблона вы получите сервер WordPress на базе Ubuntu в новой группе ресурсов.

## Инфраструктура CLI Azure

Прежде чем начать, убедитесь, что у вас установлена подходящая версия интерфейса командной строки Azure, выполнен вход и включен новый режим диспетчера ресурсов. Дополнительную информацию см. [здесь](virtual-machines-deploy-rmtemplates-azure-cli.md#getting-ready).
...
При выполнении команды **azure group deployment create** отобразится запрос на ввод значений для ряда параметров. После указания всех значений параметров Azure создает и настраивает виртуальные машины.
