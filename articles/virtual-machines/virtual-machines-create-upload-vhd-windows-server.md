...
1. В окне Azure PowerShell, использованном при выполнении предыдущего шага, введите команду такого типа:
	...
	Где **BlobStorageURL** — URL-адрес учетной записи хранения; **YourImagesFolder** — контейнер в хранилище больших двоичных объектов, в котором следует сохранить образы; **VHDName** — имя для идентификации виртуального жесткого диска, которое будет отображаться на портале Azure; **PathToVHDFile** — полный путь и имя VHD-файла.
...
> [AZURE.TIP]Чтобы использовать для добавления образа модуль Azure PowerShell, а не портал Azure, воспользуйтесь командлетом **Add-AzureVMImage**. Например:

>	`Add-AzureVMImage -ImageName <ImageName> -MediaLocation <VHDLocation> -OS <OSType>`

1. На портале Azure в разделе **Все элементы** щелкните **Виртуальные машины**.
