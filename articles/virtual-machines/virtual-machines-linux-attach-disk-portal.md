...
2. Щелкните раскрывающееся меню **Имя** и выберите пункт **Создать диск**:
    ...
    ![Создание управляемого диска Azure](./media/virtual-machines-linux-attach-disk-portal/create-new-md.png)
...
3. Введите имя управляемого диска. Просмотрите параметры по умолчанию, при необходимости измените их, а затем нажмите кнопку **Создать**.
   ...
   ![Просмотр параметров диска](./media/virtual-machines-linux-attach-disk-portal/create-new-md-settings.png)

4. Щелкните **Сохранить**, чтобы создать управляемый диск и обновить конфигурацию виртуальной машины:

   ![Сохранение нового управляемого диска Azure](./media/virtual-machines-linux-attach-disk-portal/confirm-create-new-md.png)
...
5. После того как Azure создаст диск и подключит его к виртуальной машине, он появится в параметрах дисков виртуальной машины в разделе **Диски данных**. Так как управляемые диски являются ресурсами верхнего уровня, диск отображается в корне группы ресурсов:
   ...
   ![Управляемый диск Azure в группе ресурсов](./media/virtual-machines-linux-attach-disk-portal/view-md-resource-group.png)
...
2. Щелкните раскрывающееся меню **Имя**, чтобы просмотреть список имеющихся управляемых дисков, доступных для подписки Azure. Выберите управляемый диск, который следует подключить:
   ...
   ![Подключение имеющегося управляемого диска Azure](./media/virtual-machines-linux-attach-disk-portal/select-existing-md.png)

3. Щелкните **Сохранить**, чтобы подключить имеющийся управляемый диск и обновить конфигурацию виртуальной машины:
   
   ![Сохранение обновлений управляемого диска Azure](./media/virtual-machines-linux-attach-disk-portal/confirm-attach-existing-md.png)
...
После добавления диска его необходимо подготовить для использования. Дополнительные сведения см. в разделе [Инициализация нового диска данных в Linux](linux/classic/attach-disk.md#initialize-a-new-data-disk-in-linux).
