<properties 
	pageTitle="Создание и передача виртуального жесткого диска с операционной системой RedHat Linux в Azure" 
	description="Узнайте, как создать и передать виртуальный жесткий диск (VHD-файл) Azure, содержащий операционную систему RedHat Linux." 
...
В этой статье вы узнаете, как подготовить виртуальную машину Red Hat Enterprise Linux (RHEL) для использования в Azure. Статья охватывает RHEL версий 6.6, 6.7, 7.0 и 7.1, а также низкоуровневые оболочки для подготовки виртуальных машин Hyper-V, KVM и VMWare.
...
В этом разделе предполагается, что вы уже установили образ RHEL из ISO-файла, сохраненного с веб-сайта RedHats на виртуальный жесткий диск (VHD). Дополнительные сведения об установке образа операционной системы с помощью диспетчера Hyper-V см. в статье [Установка роли Hyper-V и настройка виртуальной машины](http://technet.microsoft.com/library/hh846766.aspx).

**Замечания по установке RHEL**

- Более новый формат VHDX не поддерживается в Azure. Можно преобразовать диск в формат VHD с помощью диспетчера Hyper-V или командлета PowerShell convert-vhd.
...
8.	Зарегистрируйте подписку Red Hat, чтобы активировать установку пакетов из репозитория RHEL:

        # sudo subscription-manager register --auto-attach --username=XXX --password=XXX

9.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий Fedora EPEL 6.

        # wget http://download.fedoraproject.org/pub/epel/6/x86_64/epel-release-6-8.noarch.rpm
        # rpm -ivh epel-release-6-8.noarch.rpm

10.	Измените строку загрузки ядра в конфигурации grub, чтобы включить дополнительные параметры ядра для Azure. Для этого откройте файл `/boot/grub/menu.lst` в текстовом редакторе и убедитесь, что ядро по умолчанию включает следующие параметры:
...
6.	Зарегистрируйте подписку Red Hat, чтобы активировать установку пакетов из репозитория RHEL:
...
9.	Убедитесь, что SSH-сервер установлен и настроен для включения во время загрузки. Обычно это сделано по умолчанию. Измените файл `/etc/ssh/sshd_config`, включив в него следующую строку:

        ClientAliveInterval 180

10.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий Fedora EPEL 7.
...
12.	Не создавайте пространство подкачки на диске с ОС. Агент Linux для Azure может автоматически настраивать пространство подкачки с использованием диска на локальном ресурсе, подключенном к виртуальной машине после подготовки для работы в среде Azure. Следует отметить, что локальный диск ресурсов является временным диском и должен быть очищен при отмене подготовки виртуальной машины. После установки агента Linux для Azure (см. предыдущий шаг) измените следующие параметры в файле `/etc/waagent.conf` следующим образом:
...
1.	Загрузите KVM-образ RHEL 6.6/6.7 с веб-сайта Red Hat.
...
3.	Создайте виртуальную машину в KVM из образа qcow2, выберите тип диска **qcow2** и модель устройства интерфейса виртуальной машины **virtio**. Затем запустите виртуальную машину и войдите в систему как привилегированный пользователь.
...
12.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий **Fedora EPEL 6**.
...
1.	Загрузите KVM-образ RHEL 7.0 с веб-сайта Red Hat.
...
7.	Зарегистрируйте подписку Red Hat, чтобы активировать установку пакетов из репозитория RHEL:
...
12.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий **Fedora EPEL 7**.
...
14.	Не создавайте пространство подкачки на диске с ОС. Агент Linux для Azure может автоматически настраивать пространство подкачки с использованием диска на локальном ресурсе, подключенном к виртуальной машине после подготовки для работы в среде Azure. Следует отметить, что локальный диск ресурсов является временным диском и должен быть очищен при отмене подготовки виртуальной машины. После установки агента Linux для Azure (см. предыдущий шаг) измените следующие параметры в файле `/etc/waagent.conf` следующим образом:
...
###Предварительные требования
В этом разделе предполагается, что вы уже установили виртуальную машину RHEL в VMWare. Дополнительные сведения об установке операционной системы в VMWare см. в [руководстве по установке гостевой операционной системы VMWare](http://partnerweb.vmware.com/GOSIG/home.html).
...
7.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий Fedora EPEL 6.
...
11.	Не создавайте пространство подкачки на диске с ОС:
    ...
    Агент Linux для Azure может автоматически настраивать пространство подкачки с использованием диска на локальном ресурсе, подключенном к виртуальной машине после подготовки для работы в среде Azure. Следует отметить, что локальный диск ресурсов является временным диском и должен быть очищен при отмене подготовки виртуальной машины. После установки агента Linux для Azure (см. предыдущий шаг) измените следующие параметры в файле `/etc/waagent.conf` следующим образом:
...
4.	Зарегистрируйте подписку Red Hat, чтобы активировать установку пакетов из репозитория RHEL:
...
7.	Добавьте модули Hyper-V в initramfs:
    ...
    Измените файл `/etc/dracut.conf`, добавив в него следующую строку:
...
8.	Убедитесь, что SSH-сервер установлен и настроен для включения во время загрузки. Обычно это сделано по умолчанию. Измените файл `/etc/ssh/sshd_config`, включив в него следующую строку:

        ClientAliveInterval 180

9.	Включите репозиторий epel, поскольку пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий Fedora EPEL 7.
...
11.	Не создавайте пространство подкачки на диске с ОС. Агент Linux для Azure может автоматически настраивать пространство подкачки с использованием диска на локальном ресурсе, подключенном к виртуальной машине после подготовки для работы в среде Azure. Следует отметить, что локальный диск ресурсов является временным диском и должен быть очищен при отмене подготовки виртуальной машины. После установки агента Linux для Azure (см. предыдущий шаг) измените следующие параметры в файле `/etc/waagent.conf` следующим образом:
...
1.	Создайте kickstart-файл, содержащий приведенный ниже код, и сохраните файл. Дополнительные сведения об установке kickstart-файла см. в соответствующем [руководстве](https://access.redhat.com/documentation/ru-RU/Red_Hat_Enterprise_Linux/7/html/Installation_Guide/chap-kickstart-installations.html).
...
3.	В диспетчере Hyper-V создайте новую виртуальную машину. На странице **Подключение виртуального жесткого диска** выберите параметр **Подключить виртуальный жесткий диск позднее** и выполните инструкции мастера новой виртуальной машины.
...
5.	Запустить виртуальную машину. Когда появится руководство по установке, нажмите клавишу **TAB**, чтобы настроить параметры загрузки.
...
##Известные проблемы:
Существуют две известные проблемы, которые могут возникнуть при использовании RHEL 6.6, 7.0 и 7.1 в Hyper-V и Azure.

###Проблема 1. Таймаут подготовки
Эта проблема может возникать при загрузке с использованием RHEL в Hyper-V и Azure. Чаще всего она связана с RHEL 6.6.

Скорость воспроизведения:

Проблема возникает периодически. Чаще встречается в небольших виртуальных машинах с одним виртуальным процессором и на загруженных серверах.


###Проблема 2. Зависание дискового ввода-вывода 

Проблема может возникать при частых операциях ввода-вывода, связанных с диском хранилища RHEL 6.6, 7.0 и 7.1 в Hyper-V и Azure.
...
[AZURE.NOTE]Обе распространенные проблемы уже решены Red Hat. Чтобы установить соответствующие исправления, выполните следующую команду:
