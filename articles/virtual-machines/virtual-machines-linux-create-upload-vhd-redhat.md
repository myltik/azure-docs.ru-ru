...
[Подготовка виртуальной машины RHEL 6.7 (VMWare)](#rhel67vmware)

[Подготовка виртуальной машины RHEL 7.1/7.2 (VMWare)](#rhel7xvmware)

[Подготовка виртуальной машины RHEL 7.1/7.2 (kickstart-файл)](#rhel7xkickstart)
...
13.	Пакет WALinuxAgent `WALinuxAgent-<version>` был принудительно отправлен в репозиторий Fedora EPEL 6. Включите репозиторий epel, выполнив следующую команду:
...
15.	Не создавайте пространство подкачки на диске с ОС. Агент Linux для Azure может автоматически настраивать пространство подкачки с использованием диска на локальном ресурсе, подключенном к виртуальной машине после подготовки для работы в среде Azure. Следует отметить, что локальный диск ресурсов является временным диском и должен быть очищен при отмене подготовки виртуальной машины. После установки агента Linux для Azure (см. предыдущий шаг) соответствующим образом измените следующие параметры в файле `/etc/waagent.conf`.
...
### <a id="rhel67vmware"> </a>Подготовка виртуальной машины RHEL 6.7 (VMWare)###
...
### <a id="rhel7xvmware"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 (VMWare)###
...
### <a id="rhel7xkickstart"> </a>Подготовка виртуальной машины RHEL 7.1/7.2 (kickstart-файл)###
...
3.	В диспетчере Hyper-V создайте новую виртуальную машину. На странице **Подключение виртуального жесткого диска** выберите параметр **Подключить виртуальный жесткий диск позже** и выполните инструкции мастера создания виртуальной машины.
...
## Дальнейшие действия
Теперь виртуальный жесткий диск Red Hat Enterprise Linux можно использовать для создания новых виртуальных машин Azure. Если вы используете Azure и отправляете VHD-файл в Azure в первый раз, следуйте инструкциям, описанным в шагах 2 и 3 [этого руководства](virtual-machines-linux-create-upload-vhd.md).
