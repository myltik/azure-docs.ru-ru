<properties
	pageTitle="Устранение неполадок доступа к приложению, выполняющемуся в виртуальной машине Azure"
...
> [AZURE.NOTE]Чтобы получить помощь по вопросам подключения к виртуальной машине Azure, см. статью [Устранение неполадок с подключением к удаленному рабочему столу виртуальной машины Windows в службе Azure](virtual-machines-troubleshoot-remote-desktop-connections.md) или [Устранение неполадок с подключением Secure Shell (SSH) к виртуальной машине Azure под управлением Linux](virtual-machines-troubleshoot-ssh-connections.md).
...
3.	Конечные точки Azure для облачной службы, содержащей виртуальную машину (для виртуальных машин, созданных в управлении службами), правила преобразования сетевых адресов для входящих подключений (для виртуальных машин, созданных в диспетчере ресурсов) и группы безопасности сети.
...
Если у вас есть доступ к приложению, перейдите к [шагу 2](#step2).
...
## <a id="step3"></a>Шаг 3. Есть ли у вас доступ к приложению с компьютера, который находится за пределами виртуальной сети, но не подключен к сети, в которой находится ваш компьютер?
...
- Для виртуальных машин, созданных в управлении службами, конфигурация конечной точки для виртуальной машины разрешает входящий трафик, в частности для протокола (TCP или UDP) и публичных и частных номеров портов. Дополнительные сведения см. в разделе [Настройка конечных точек виртуальной машины](virtual-machines-set-up-endpoints.md).
- Для виртуальных машин, созданных в управлении службами, списки управления доступом (ACL) для конечной точки не блокируют входящий трафик из Интернета. Дополнительные сведения см. в разделе [Настройка конечных точек виртуальной машины](virtual-machines-set-up-endpoints.md).
...
- Группы безопасности сети разрешают трафик входящих запросов и исходящих ответов. Дополнительную информацию см. в статье [Что такое группа безопасности сети?](virtual-networks-nsg.md).
...
- Если протокол зонда и порт отличаются от протокола и порта для комплекта балансировки нагрузки, сделайте следующее.
	- Убедитесь, что приложение прослушивает протокол зонда (TCP или UDP) и номер порта (используйте команду **netstat –a** на целевой виртуальной машине).
...
- Отправьте запрос в службу поддержки Azure Перейдите на [веб-сайт поддержки для Azure](http://azure.microsoft.com/support/options/) и перейдите по ссылке **Получить поддержку** в разделе **Поддержка по выставлению счетов и техническая поддержка**.
