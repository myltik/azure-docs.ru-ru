...
- Настройка мониторинга виртуальной машины с помощью расширения ВМ Microsoft Monitoring Agent. Дополнительные сведения см. в статье [Сбор данных о виртуальных машинах Azure](../../log-analytics/log-analytics-azure-vm-extension.md).
...
С помощью команды `Set-AzureRmVMExtension` можно запустить любое расширение виртуальной машины. Дополнительные сведения см. в разделе справки [о команде AzureRmVMExtension](https://msdn.microsoft.com/en-us/library/mt603745.aspx).
...
В некоторых случаях может потребоваться перезапуск расширения ВМ. Можно сначала удалить расширение, затем повторно запустить его любым удобным способом. Чтобы удалить расширение, выполните командлет [Remove-AzureRmVMExtension](/powershell/module/AzureRM.Compute/Remove-AzureRmVMExtension) следующим образом:
