...
Этот этап необходимо выполнить, прежде чем переходить к [этапу 3](virtual-machines-workload-high-availability-LOB-application-phase3.md). Все этапы перечислены в статье [Развертывание высокодоступных бизнес-приложений в Azure](virtual-machines-workload-high-availability-LOB-application-overview.md).
...
--- | --- | --- | --- 
1\. | \_\_\_\_\_\_\_\_\_\_\_\_\_\_ (первый контроллер домена, например DC1) | Центр обработки данных Windows Server 2012 R2 | Standard\_D1
...
С помощью приведенного ниже набора команд Azure PowerShell создайте виртуальные машины для двух контроллеров домена. Укажите значения переменных, удалив знаки < and >. Обратите внимание на то, что в наборе команд PowerShell используются значения из перечисленных ниже таблиц.
...
Значения в таблицах V, S, ST и A были заданы на [этапе 1 (настройка Azure)](virtual-machines-workload-high-availability-LOB-application-phase1.md).
...
1.	На [портале предварительной версии Azure](https://portal.azure.com/) в области слева щелкните **Просмотреть все > Виртуальные сети** и выберите имя своей виртуальной машины (в таблице V в столбце значений см. позицию 1).
2.	На панели виртуальной сети выберите **Все параметры**.
3.	На панели **Параметры** щелкните **DNS-серверы**.
4.	На панели **DNS-серверы** введите следующую команду.
	- Для **основного DNS-сервера**: таблица V — позиция 6 — столбец значений.
	- Для **дополнительного DNS-сервера**: таблица V — позиция 7 — столбец значений.
5.	В левой области портала предварительной версии Azure последовательно щелкните **Просмотреть все > Виртуальные машины**.
6.	На панели **Виртуальные машины** щелкните имя первого контроллера домена (в таблице M в столбце имен виртуальных машин см. позицию 1).
7.	На панели виртуальной машины щелкните **Перезапустить**.
8.	После запуска первого контроллера домена на панели **Виртуальные машины** щелкните имя второго контроллера домена (в таблице M в столбце имен виртуальных машин см. позицию 2).
9.	На панели виртуальной машины щелкните **Перезапустить**. Дождитесь запуска второго контроллера домена.
