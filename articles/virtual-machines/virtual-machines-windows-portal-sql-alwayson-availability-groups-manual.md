...
> [AZURE.SELECTOR]
- [Портал — Resource Manager — шаблон](virtual-machines-windows-portal-sql-alwayson-availability-groups.md)
- [Портал — Resource Manager — руководство](virtual-machines-windows-portal-sql-alwayson-availability-groups-manual.md)
- [Портал — классическая модель — руководство](virtual-machines-windows-classic-portal-sql-alwayson-availability-groups.md)
...
## Создание группы ресурсов, сетей и контроллеров домена
...
1. Создайте вторую подсеть. Щелкните **+ Subnet** (+ Подсеть). 
 ...
 В колонке **Добавить подсеть** настройте подсеть, введя **subnet-2** в поле **Имя**. Azure автоматически укажет допустимое значение в поле **Диапазон адресов**. Убедитесь, что этот диапазон адресов содержит по крайней мере 10 адресов. Для рабочей среды может потребоваться больше адресов.
...
1. Введите **Центр обработки данных Windows Server 2012 R2**.

1. Щелкните **Центр обработки данных Windows Server 2012 R2**. В колонке **Windows Server 2012 R2 Datacenter** убедитесь, что в качестве модели развертывания установлен **Диспетчер ресурсов** и нажмите кнопку **Создать**. В Azure откроется колонка **Создание виртуальной машины**.
...
1. Выберите роли **Доменные службы Active Directory** и **DNS-сервер**. При появлении запроса добавьте требуемые для этих ролей дополнительные компоненты.
	...
	>[AZURE.NOTE] Вы получите предупреждение проверки об отсутствии статического IP-адреса. Если вы тестируете конфигурацию, нажмите кнопку "Продолжить". Для рабочих сценариев на портале Azure установите статический IP-адрес или [используйте PowerShell для установки статического IP-адреса компьютера контроллера домена](./virtual-network/virtual-networks-reserved-private-ip.md).
...
1. Войдите в компьютер **ad-primary-dc** снова. 
...
1. Запустите RDP-файл для основного контроллера домена (**ad-primary-dc**) и войдите в виртуальную машину с использованием настроенной учетной записи администратора (**BUILTIN\\DomainAdmin**) и пароля (**Contoso!000**).

1. Из основного контроллера домена запустите удаленный рабочий стол для **ad-secondary-dc**, используя IP-адрес. Используйте ту же учетную запись и пароль.
...
1. Повторно выполните те же действия, что при создании первого контроллера домена. Однако в **мастере настройки доменных служб Active Directory** используйте следующие значения:
...
Далее создайте три виртуальные машины, включая две виртуальные машины SQL Server, и узел кластера WSFC. Чтобы создать каждую виртуальную машину, вернитесь к группе ресурсов **HA-AG-RG**, нажмите кнопку **Добавить**, найдите соответствующий элемент коллекции, щелкните **Виртуальная машина**, а затем — **Из коллекции**. Затем используйте шаблоны из следующей таблицы, чтобы упростить создание виртуальных машин.
...
|Конфигурация виртуальной машины **Параметры**|**Хранилище** = Premium (SSD)<br/>**ПОДСЕТИ В СЕТИ** = autoHAVNET<br/>**УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ** = "Использовать автоматически созданную учетную запись хранения"<br/>**Подсеть** = subnet-2(10.1.1.0/24)<br/>**Общедоступный IP-адрес** = нет<br/>**Группа безопасности сети** = нет<br/>**Мониторинг диагностики** = включено<br/>**Учетная запись хранения для диагностики** = "Использовать автоматически созданную учетную запись хранения"<br/>**ГРУППА ДОСТУПНОСТИ** = sqlAvailabilitySet<br/>|**Хранилище** = Premium (SSD)<br/>**ПОДСЕТИ В СЕТИ** = autoHAVNET<br/>**УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ** = "Использовать автоматически созданную учетную запись хранения"<br/>**Подсеть** = subnet-2(10.1.1.0/24)<br/>**Общедоступный IP-адрес** = нет<br/>**Группа безопасности сети** = нет<br/>**Мониторинг диагностики** = включено<br/>**Учетная запись хранения для диагностики** = "Использовать автоматически созданную учетную запись хранения"<br/>**ГРУППА ДОСТУПНОСТИ** = sqlAvailabilitySet<br/>|**Хранилище** = Premium (SSD)<br/>**ПОДСЕТИ В СЕТИ** = autoHAVNET<br/>**УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ** = "Использовать автоматически созданную учетную запись хранения"<br/>**Подсеть** = subnet-2(10.1.1.0/24)<br/>**Общедоступный IP-адрес** = нет<br/>**Группа безопасности сети** = нет<br/>**Мониторинг диагностики** = включено<br/>**Учетная запись хранения для диагностики** = "Использовать автоматически созданную учетную запись хранения"<br/>**ГРУППА ДОСТУПНОСТИ** = sqlAvailabilitySet<br/>
|Конфигурация виртуальной машины **Параметры SQL Server**|Не применяется|**Подключение к SQL** = частное (в виртуальной сети)<br/>**Порт** = 1433<br/>**Аутентификация SQL** = отключено<br/>**Конфигурация хранилища** = общая<br/>**Автоматическое обновление** = воскресенье в 2:00<br/>**Автоматическое резервное копирование** = отключено</br>**Интеграция хранилища ключей Azure** = отключено|**Подключение к SQL** = частное (в виртуальной сети)<br/>**Порт** = 1433<br/>**Аутентификация SQL** = отключено<br/>**Конфигурация хранилища** = общая<br/>**Автоматическое обновление** = воскресенье в 2:00<br/>**Автоматическое резервное копирование** = отключено</br>**Интеграция хранилища ключей Azure** = отключено|
...
1. Из основного контроллера домена запустите удаленный рабочий стол для **sqlserver-0**, используя IP-адрес. Используйте ту же учетную запись и пароль.
...
1. Дождитесь перезапуска виртуальной машины, а затем запустите RDP-файл еще раз из основного контроллера домена, чтобы войти в **sqlserver-0**, используя учетную запись **BUILTIN\\DomainAdmin**.
...
1. Создайте кластер с одним узлом на одном из узлов (**sqlserver-0**).
...
1. На странице **Указание реплик** сервер **sqlserver-1** должен отобразиться в списке **Доступные реплики**. Настройте реплики, как показано ниже. После завершения нажмите кнопку **Далее**.
...
Чтобы подключиться к группе доступности напрямую, необходимо настроить внутренний балансировщик нагрузки в Azure, а затем создать прослушиватель в кластере. В этом разделе представлен общий обзор этих действий. Подробные инструкции см. в статье [Configure an internal load balancer for an AlwaysOn availability group in Azure](virtual-machines-windows-portal-sql-alwayson-int-listener.md) (Настройка внутреннего балансировщика нагрузки для группы доступности AlwaysOn в Azure).
...
1. В поле **Имя** введите **aglistener**. Нажмите кнопку **Далее** дважды, а затем нажмите **Готово**. Не подключайте прослушивателя или ресурс на этом этапе.
