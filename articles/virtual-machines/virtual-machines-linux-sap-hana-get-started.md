...
* Разверните две виртуальные машины Azure с операционной системой SLES 12 с пакетом обновлений 1 (SP1) в соответствии с моделью Azure Resource Manager.
...
В шаблоне 101-vm-simple-linux показан базовый шаблон, который содержит раздел хранилища с дополнительным диском данных емкостью 100 ГБ.
...
Дополнительные сведения о кэше страниц, связанные с запуском SAP HANA в SLES, см. в разделе 6.1 Kernel: Page-Cache Limit (6.1. Ядро: предельный размер кэша страницы) [руководства по SUSE Linux Enterprise Server для приложений SAP 12 с пакетом обновлений 1 (SP1)](https://www.suse.com/documentation/sles_for_sap/singlehtml/sles_for_sap_guide/sles_for_sap_guide.html#sec.s4s.configure.page-cache).

В другом [примечании SAP № 1557506](https://launchpad.support.sap.com/#/notes/1557506) описывается предельный размер кэша страниц.

Вместо устаревшей программы sapconf в SLES 12 представлено новое средство tuned-adm с особенным профилем SAP HANA. Дополнительные сведения об этом средстве см. в следующих ресурсах:

* [Документация SLES по профилю sap-hana в tuned-adm](https://www.suse.com/documentation/sles-for-sap-12/book_s4s/data/sec_s4s_configure_sapconf.html)
* [Документация SLES по профилю tuned-adm: глава 6.2 со сведениями о настройке систем для рабочих нагрузок SAP с помощью tuned-adm](https://www.suse.com/documentation/sles-for-sap-12/pdfdoc/book_s4s/book_s4s.pdf)
...
Чтобы сделать параметры ядра SAP HANA постоянными, используйте в SLES 12 загрузчик GRUB2. Дополнительные сведения о загрузчике GRUB2 см. в [разделе о структуре файла конфигурации документации по SUSE](https://www.suse.com/documentation/sled-12/book_sle_admin/data/sec_grub2_file_structure.html).
...
При установке SAP NetWeaver в стандартной среде SLES 12 из образа коллекции Azure отображается сообщение об отсутствии области буфера. Чтобы закрыть это сообщение, можно вручную добавить файл подкачки с помощью dd, mkswap и swapon. Сведения о том, как это сделать, см. в подразделе Adding a Swap File Manually (Добавление файла подкачки вручную) [раздела Using the YaST Partitioner (Использование модуля секционирования YaST) документации по SUSE](https://www.suse.com/documentation/sled-12/book_sle_deployment/data/sec_yast2_i_y2_part_expert.html).
...
### <a name="installing-gnome-desktop-and-xrdp-on-sles-12"></a>Установка графического рабочего стола Gnome и xrdp в SLES 12
При наличии операционной системы Microsoft Windows для запуска Firefox, Sapinst, графического пользовательского интерфейса SAP, консоли управления SAP или HANA Studio и подключения к виртуальной машине через удаленный рабочий стол с компьютера Windows можно легко использовать графический рабочий стол непосредственно в виртуальных машинах SAP под управлением Linux. Хотя это нецелесообразно выполнять для рабочего сервера базы данных, это нормально для демонстрационной среды или среды-прототипа. Чтобы установить рабочий стол Gnome на виртуальной машине Azure SLES 12, сделайте следующее.
...
После установки рабочего стола Gnome при запуске графической консоли управления SAP на основе Java из Firefox в виртуальной машине Azure SLES 12 может отображаться ошибка из-за отсутствия в браузере подключаемого модуля Java.
...
Еще одна проблема, которая может возникнуть, — это сообщение об отсутствующем файле javafx.properties. Скорее всего, эта ошибка связана с установкой Java 1.8, необходимой для графического пользовательского интерфейса SAP 7.4. В версии подключаемого модуля Java от IBM, представленного в Yast, этот файл отсутствует. Чтобы решить проблему, необходимо скачать Java из Oracle. Дополнительные сведения об этой проблеме см. в записи [SAPGui 7.4 Java for openSUSE 42.1 Leap](https://scn.sap.com/thread/3908306) (SAPGui 7.4 Java для openSUSE 42.1 Leap).
