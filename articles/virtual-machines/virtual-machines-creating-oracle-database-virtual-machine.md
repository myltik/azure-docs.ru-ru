...
6.	Создайте переменную среды **ORACLE_HOSTNAME** со значением имени компьютера виртуальной машины. Создать переменную среды можно, выполнив следующие действия:
	...
	4.	В диалоговом окне **Новая системная переменная** введите имя переменной **ORACLE_HOSTNAME**. В качестве значения введите имя компьютера виртуальной машины. Чтобы определить имя компьютера, откройте командную строку и выполните **SET COMPUTERNAME** (выходные данные этой команды будут содержать имя компьютера).
...
Если вы хотите разрешить удаленный доступ к Oracle Database Enterprise Manager, откройте в брандмауэре порт 5500 и создайте конечную точку виртуальной машины для порта 5500 на портале Azure (следуя описанным выше указаниям по открытию порта 1521 и созданию конечной точки для него). Затем, чтобы запустить Oracle Enterprise Manager с удаленного компьютера, откройте в браузере URL-адрес вида `http://<<unique_domain_name>>:5500/em`. (Значение *<< unique_domain_name >>* можно определить на [портале Azure](https://ms.portal.azure.com/), щелкнув **Виртуальные машины** и выбрав виртуальную машину, которая используется для запуска базы данных Oracle.)
