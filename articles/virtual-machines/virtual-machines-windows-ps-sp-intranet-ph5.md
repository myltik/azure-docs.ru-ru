<properties
	pageTitle="Ферма SharePoint Server 2013, этап 5 | Microsoft Azure"
	description="На пятом этапе развертывания фермы SharePoint Server 2013 в Azure создайте группу доступности и добавьте в нее базы данных SharePoint."
	documentationCenter=""
...
Описания всех этапов см. в разделе [Развертывание среды SharePoint с группами доступности AlwaysOn для SQL Server на платформе Azure](virtual-machines-windows-sp-intranet-overview.md).
...
Для выполнения операций резервного копирования и восстановления файлы резервных копий (BAK-файлы) должны быть доступны с дополнительной виртуальной машины SQL Server. Выполните перечисленные ниже действия.

1.	Войдите на узел сервера-источника SQL Server с учетной записью **[домен]\\sp\_farm\_db**.
2.	Перейдите на диск F:\\.
3.	Щелкните правой кнопкой мыши папку **Backup**, выберите команду **Предоставить доступ** и щелкните **отдельным людям**.
4.	В диалоговом окне **Совместный доступ к файлам** введите **[домен]\\sqlservice** и нажмите кнопку **Добавить**.
...
6.	Щелкните **Добавить**. В поле **Имя файла** введите **\\[имя\_компьютера]\\backup[имя\_базы\_данных].bak**, где имя\_компьютера — имя компьютера с сервером-источником SQL Server, а имя\_базы\_данных — имя базы данных. Нажмите кнопку **ОК**, а затем щелкните **ОК** еще раз в окне с сообщением об успешном создании резервной копии.
7.	В левой области щелкните правой кнопкой мыши **[имя\_базы\_данных]**, наведите указатель на элемент **Задачи** и выберите **Создать резервную копию**.
...
1.	Войдите на компьютер с сервером-получателем SQL Server, используя учетную запись **[имя\_домена]\\sp\_farm\_db**.
...
После создания прослушивателя вам потребуется указать его на всех виртуальных машинах SharePoint вместо имени первого компьютера SQL Server в кластере. Вместо имен настройте виртуальные машины SharePoint для использования псевдонима SQL. Дополнительные сведения и инструкции см. в статье [Псевдоним SQL для SharePoint](http://blogs.msdn.com/b/priyo/archive/2013/09/13/sql-alias-for-sharepoint.aspx).
