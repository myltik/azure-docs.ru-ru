...
При использовании [классического портала](https://manage.windowsazure.com/) Azure вам понадобится PEM-файл сертификата X.509 для ключей SSH.  Другие типы открытых ключей SSH не используются. *Требуются* только сертификаты X.509.
