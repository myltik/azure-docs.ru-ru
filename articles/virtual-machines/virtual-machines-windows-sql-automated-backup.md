...
При создании новой виртуальной машины SQL Server 2014 с моделью развертывания с помощью Resource Manager можно настроить автоматическую архивацию, используя портал Azure.

В колонке **Параметры SQL Server** выберите **Автоматическая архивация**. Колонка **Автоматическая архивация SQL** показана на следующем снимке экрана портала Azure.
...
Для существующих виртуальных машин SQL Server необходимо использовать PowerShell для настройки параметров автоматической архивации.
...
Чтобы отключить автоматическую архивацию, выполните тот же сценарий без параметра **-Enable** команды **AzureRM.Compute\\New-AzureVMSqlServerAutoBackupConfig**. Отсутствие параметра **-Enable** означает, что функцию нужно отключить. Как и установка, отключение автоматической архивации занимает несколько минут.
