...
В этой статье показано, как настроить кластер Linux RDMA в Azure с виртуальными машинами A8 и A9 для параллельного выполнения приложений MPI. Когда на виртуальных машинах A8 и A9 выполняется поддерживаемый дистрибутив Linux HPC и поддерживаемая реализация MPI, приложения MPI эффективно взаимодействуют по сети с низкой задержкой и высокой пропускной способностью, которая основана на технологии удаленного доступа к памяти (RDMA).

>[AZURE.NOTE] В настоящее время для поддержки Azure Linux RDMA необходимо установить библиотеку Intel MPI версии 5 на виртуальные машины, созданные на основе образов SUSE Linux Enterprise Server 12 HPC, CentOS 6.5 HPC или CentOS 7.1 HPC из Azure Marketplace. Дополнительные сведения и рекомендации см. в разделе [Сведения об экземплярах A8, A9, A10 и A11 с большим объемом вычислений](virtual-machines-linux-a8-a9-a10-a11-specs.md).
...
* **Сценарии интерфейса командной строки Azure** (Azure CLI). Как показано далее в этой статье, используйте [интерфейс командной строки Azure](../xplat-cli-install.md) для создания сценария развертывания кластера виртуальных машин Linux размера A8 или A9. В классической модели развертывания интерфейс командной строки в режиме управления службами создает узлы кластера последовательно, поэтому развертывание нескольких вычислительных узлов может потребовать несколько минут. В классической модели развертывания виртуальные машины A8 или A9 должны быть развернуты в одной облачной службе для подключения через сеть RDMA.

* **Шаблоны Azure Resource Manager**. Используя модель развертывания с помощью Resource Manager, можно развернуть кластер виртуальных машин Linux размера A8 и A9, который подключается к сети RDMA для выполнения рабочих нагрузок MPI. Для развертывания своего решения вы можете [создать собственный шаблон](../resource-group-authoring-templates.md) или изучить [страницу шаблонов быстрого запуска Azure](https://azure.microsoft.com/documentation/templates/), которая содержит шаблоны, созданные корпорацией Майкрософт и сообществом пользователей. Шаблоны диспетчера ресурсов — самый быстрый и надежный способ развертывания кластера Linux. В модели развертывания с помощью Resource Manager виртуальные машины A8 или A9 должны быть развернуты в одной группе доступности для подключения через сеть RDMA.

## Пример развертывания в классической модели

Далее показано, как использовать Azure CLI для развертывания виртуальной машины SUSE Linux Enterprise Server (SLES) 12 HPC из Azure Marketplace, установки библиотеки Intel MPI и создания настраиваемого образа виртуальной машины. После этого образ используется в сценарии развертывания кластера виртуальных машин A8 или A9.
...
*   **Intel MPI**. В рамках настройки образа виртуальной машины SLES 12 HPC для своего кластера необходимо скачать и установить текущую среду выполнения библиотеки Intel MPI 5 с [сайта Intel.com](https://software.intel.com/ru-RU/intel-mpi-library/). Более подробно это описано далее в этой статье. Чтобы подготовится к установке, после регистрации на сайте Intel скопируйте ссылку для скачивания TGZ-файла соответствующей версии Intel MPI. В этой статье используется Intel MPI версии 5.0.3.048.

    >[AZURE.NOTE] При использовании образа CentOS 6.5 HPC или CentOS 7.1 HPC из Azure Marketplace для создания узлов кластера автоматически предварительно устанавливается Intel MPI версии 5.1.3.181.
...
Теперь подготовьте к работе виртуальную машину A9 с помощью образа SLES 12 HPC, выполнив команду следующего вида.
...
* Размером (в данном примере — A9) может быть A8 или A9.

* Номер внешнего порта SSH (в данном примере — 22, номер порта SSH по умолчанию) — любой допустимый номер порта. Для внутреннего порта SSH будет задан номер 22.

* Новая облачная служба создается в регионе Azure, указанном согласно расположению. Укажите расположение, в котором доступны экземпляры A8 и A9.

* В качестве имени образа SLES 12 сейчас можно выбрать `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-v20150708` или `b4590d9e3ed742e4a1d46e5424aa335e__suse-sles-12-hpc-priority-v20150708`, чтобы обеспечить приоритетную поддержку SUSE (за дополнительную плату).
...
* **Обновления**. Установите обновления с помощью **zypper**. Также можно установить служебные программы NFS.
    ...
    >[AZURE.IMPORTANT]Если развернута виртуальная машина SLES 12 HPC, рекомендуется не применять обновления ядра. Они могут вызвать проблемы с драйверами Linux RDMA.
    >
    >В образах CentOS HPC, доступных в Marketplace, обновления ядра отключены в файле конфигурации **yum**. Так как драйверы Linux RDMA распространяются в виде пакета RPM, обновления драйверов могут не работать в случае обновления ядра.

* **Обновления драйверов Linux RDMA**. Если развернута виртуальная машина SLES 12 HPC, нужно обновить драйверы RDMA. Дополнительные сведения см. в статье [об использовании экземпляров A8, A9, A10 и A11 для ресурсоемких вычислений](virtual-machines-linux-a8-a9-a10-a11-specs.md#rdma-driver-updates-for-sles-12).
...
* **Ключи SSH для виртуальных машин SLES 12**. Создайте ключи SSH, чтобы установить доверие для учетной записи пользователя среди всех вычислительных узлов в кластере SLES 12 HPC при выполнении заданий MPI. (Если развернута виртуальная машина HPC на основе CentOS, не выполняйте этот шаг. Инструкции по установке доверия SSH без пароля для узлов кластера после записи образа и развертывания кластера см. далее в статье.)
