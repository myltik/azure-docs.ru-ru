...
* **Ключи SSH для виртуальных машин SLES 12**. Создайте ключи SSH для установления доверия для учетной записи пользователя среди всех вычислительных узлов в кластере SLES 12 HPC при выполнении заданий MPI. (Если развернута виртуальная машина HPC на основе CentOS, не выполняйте этот шаг. Инструкции по установке доверия SSH без пароля для узлов кластера после записи образа и развертывания кластера см. далее в статье.)
...
* Чтобы создать кластер Intel Lustre с помощью образа CentOS HPC, воспользуйтесь [шаблоном быстрого запуска](https://github.com/Azure/azure-quickstart-templates/tree/master/intel-lustre-clients-on-centos).
