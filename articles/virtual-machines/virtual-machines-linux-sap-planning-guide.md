...
# <a name="sap-netweaver-on-azure-virtual-machines-vms-planning-and-implementation-guide"></a>SAP NetWeaver на виртуальных машинах Azure. Руководство по планированию и реализации
...
Дополнительные сведения о создании образов виртуальных машин см. в [этой статье][virtual-machines-linux-capture-image-resource-manager].
...
* В конфигурации новой виртуальной машины укажите этот виртуальный жесткий диск в качестве диска операционной системы с помощью командлета _Set-AzureRmVMOSDisk_. Дополнительные сведения см. здесь: <https://msdn.microsoft.com/library/mt603746.aspx>
...
* Добавьте диск данных к новой виртуальной машине с помощью командлета _Add-AzureRmVMDataDisk_. Дополнительные сведения см. здесь: <https://msdn.microsoft.com/library/mt603673.aspx>
...
* Используйте команду _sysprep_ в Windows или _waagent -deprovision_ в Linux, чтобы обобщить виртуальную машину. Процесс для Windows подробно описан в [техническом руководстве по Sysprep](https://technet.microsoft.com/library/cc766049.aspx), а для Linux — в разделе [Как записать образ виртуальной машины Linux для его использования в качестве шаблона Resource Manager][virtual-machines-linux-capture-image-resource-manager-capture].
* Войдите в свою подписку с помощью командлета _Login-AzureRmAccount_
* Укажите подписку контекста с помощью командлета _Set-AzureRmContext_ с параметром SubscriptionId или SubscriptionName. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt619263.aspx>
* Загрузите виртуальный жесткий диск с помощью командлета _Add-AzureRmVhd_ в учетную запись хранения. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603554.aspx>
* В конфигурации новой виртуальной машины укажите этот виртуальный жесткий диск в качестве диска операционной системы с помощью командлета _Set-AzureRmVMOSDisk -SourceImageUri -CreateOption fromImage_. Дополнительные сведения см. здесь: <https://msdn.microsoft.com/library/mt603746.aspx>
* Создайте новую виртуальную машину из конфигурации виртуальной машины с помощью командлета _New-AzureRmVM_. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603754.aspx>
...
Дополнительные сведения о командлете Save-AzureRmVhd см. здесь: <https://msdn.microsoft.com/library/mt622705.aspx>. 
...
* Создайте контекст учетной записи хранения для исходной учетной записи хранения с помощью командлета _New-AzureStorageContext_. Дополнительные сведения см. здесь: <https://msdn.microsoft.com/library/dn806380.aspx>.
* Создайте контекст учетной записи хранения для целевой учетной записи хранения с помощью командлета _New-AzureStorageContext_. Дополнительные сведения см. здесь: <https://msdn.microsoft.com/library/dn806380.aspx>.
...
**Примечание**. Виртуальные жесткие диски в конкретный момент времени могут быть подключены только к одной виртуальной машине.
...
Допустим, вы уже создали образ виртуальной машины, как описано в некоторых частях раздела [Подготовка виртуальных машин с SAP для Azure][planning-guide-5.2] в настоящем документе.
...
Дополнительные сведения о том, какие порты нужны для различных продуктов и служб SAP, сгруппированные по продуктам SAP, см. здесь: <http://scn.sap.com/docs/DOC-17124>. С помощью этого документа вы сможете открыть на VPN-устройстве конкретные порты, необходимые для определенных продуктов и сценариев SAP.
...
Функции высокой доступности и аварийного восстановления для СУБД в целом и для конкретных СУБД рассматриваются в [Руководстве по развертыванию СУБД][dbms-guide].
