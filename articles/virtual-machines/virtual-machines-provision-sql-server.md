...
>[AZURE.NOTE]Виртуальную машину, созданную с помощью ознакомительной версии образа платформы SQL Server, нельзя обновить в коллекции до образа с поминутной оплатой. Вы можете создать виртуальную машину с помощью выпуска SQL Server с поминутной оплатой (из коллекции) и перенести файлы базы данных в эту новую виртуальную машину. Для этого нужно следовать инструкциям, описанным в статье [Миграция файлов баз данных и схемы SQL Server между виртуальными машинами в Azure с помощью дисков с данными](http://go.microsoft.com/fwlink/p/?LinkId=294738). **Или** вы можете обновить существующий экземпляр ознакомительного выпуска SQL Server до другого выпуска согласно соглашению [Перемещение лицензий в рамках программы Software Assurance в Azure](http://azure.microsoft.com/pricing/license-mobility/). Для этого выполните инструкции, описанные в статье [Обновление до другого выпуска SQL Server 2014](http://go.microsoft.com/fwlink/?LinkId=396915). Сведения о покупке лицензионной копии SQL Server см. в разделе [Как приобрести SQL Server](http://www.microsoft.com/sqlserver/get-sql-server/how-to-buy.aspx).
...
4. На первой странице **Конфигурация виртуальной машины** укажите следующие сведения:
	...
	![Конфигурация виртуальной машины][Image4]
...
>[AZURE.NOTE]При подключении в пределах той же облачной службы или виртуальной сети не нужно создавать общедоступную конечную точку. В этом случае можно перейти к следующему шагу. Дополнительную информацию см. в разделе [Рекомендации по подключению SQL Server в среде виртуальной машины Azure](https://msdn.microsoft.com/library/azure/dn133152.aspx).
...
7. В диалоговом окне **Профиль** выберите **Public**, **Private** и **Domain**. Нажмите кнопку **Далее**.
...
5. В области консоли выберите **Службы SQL Server**. В области сведений щелкните правой кнопкой мыши сервер **SSQL Server (\_имя экземпляра\_)** (экземпляр по умолчанию — **SQL Server (MSSQLSERVER)**) и выберите команду **Перезагрузить**, чтобы остановить и перезапустить экземпляр SQL Server.
...
[Хранение данных SQL Server и рабочих нагрузок операций в виртуальных машинах Azure](http://msdn.microsoft.com/library/windowsazure/dn387396.aspx)
