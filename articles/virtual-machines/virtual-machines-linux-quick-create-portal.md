...
2. Щелкните в **Marketplace** элемент **Виртуальные машины**, а затем выберите в списке образов **Рекомендуемые приложения** пункт **Ubuntu Server 14.04 LTS**. Внизу страницы проверьте модель развертывания (должна быть `Resource Manager`) и нажмите кнопку **Создать**.
...
3. На странице **Основные сведения** укажите следующие данные:
    ...
    - открытый ключ SSH в виде строки (по умолчанию — из каталога `~/.ssh/`);
    - имя группы ресурсов (для создания группы развертывания) или имя существующей группы.

    Нажмите кнопку **ОК**, чтобы продолжить и выбрать размер виртуальной машины. Вы должны увидеть приблизительно следующее:

    ![экран\_3](../media/virtual-machines-linux-quick-create-portal/screen3.png)

4. Выберите размер **DS1**, чтобы установить Ubuntu на твердотельном накопителе (Premium SSD), а затем нажмите кнопку **Выбрать** для настройки параметров.

    ![экран\_4](../media/virtual-machines-linux-quick-create-portal/screen4.png)

5. В разделе **Параметры** оставьте настройки по умолчанию для хранилища и сети, а затем нажмите кнопку **ОК**, чтобы просмотреть сводку. Обратите внимание, что при выборе DS1 устанавливается тип диска Premium SSD; буква **S** обозначает именно SSD.
...
7. Откройте панель мониторинга портала и в разделе **Сетевые интерфейсы** выберите нужный сетевой адаптер.
