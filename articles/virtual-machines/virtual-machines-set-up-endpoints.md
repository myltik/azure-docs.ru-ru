...
[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-classic-include.md)]Модель диспетчера ресурсов. Для развертывания диспетчера ресурсов обратитесь к разделам [Приступая к настройке балансировщика нагрузки, доступного через Интернет, с помощью диспетчера ресурсов Azure](../load-balancer/load-balancer-arm-powershell.md) и [Сведения о группах безопасности сети](virtual-networks-nsg.md).

Все виртуальные машины, созданные в Azure с помощью классической модели развертывания, могут автоматически взаимодействовать с другими виртуальными машинами в той же облачной службе или виртуальной сети, используя канал частной сети. Однако компьютерам в Интернете или другим виртуальным сетям требуются конечные точки, чтобы направить входящий трафик к виртуальной машине.

При создании виртуальной машины на классическом портале Azure в зависимости от выбранной операционной системы обычно автоматически создаются общие конечные точки, такие как конечные точки для удаленного рабочего стола, удаленного взаимодействия Windows PowerShell и Secure Shell (SSH). При необходимости при создании виртуальной машины или впоследствии можно настроить дополнительные конечные точки.

У каждой конечной точки есть *общедоступный порт* и *частный порт*.
...
Чтобы использовать командлет Azure PowerShell для настройки конечных точек, см. раздел [Add-AzureEndpoint](https://msdn.microsoft.com/library/azure/dn495300.aspx). При использовании интерфейса командной строки Azure в режиме управления службами пользуйтесь командой **azure vm endpoint create**.
...
Правила оцениваются по порядку, начиная с первого и заканчивая последним правилом. Это означает, что правила должны быть упорядочены, начиная от наименее строгого к наиболее строгому. Примеры и дополнительную информацию см. в разделе [Сетевой список контроля доступа](../virtual-network/virtual-networks-acl/).

Дополнительную информацию о настройке с помощью командлета Azure PowerShell см. в разделе [Управление списками управления доступом для конечных точек с помощью PowerShell](../virtual-network/virtual-networks-acl-powershell.md).
