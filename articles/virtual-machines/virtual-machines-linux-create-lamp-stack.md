...
Если у вас уже есть виртуальная машина и вы просто хотите найти общую информацию об установке стека LAMP на различных дистрибутивах Linux, в дополнение к указанному разделу см. статью [Установка стека LAMP на виртуальную машину Linux в Azure](virtual-machines-linux-install-lamp-stack.md).
...
-	Скачайте и установите Puttygen из следующего расположения: [http://www.chiark.greenend.org.uk/\~sgtatham/putty/download.html](http://www.chiark.greenend.org.uk/~sgtatham/)
...
В поле **Ключ проверки подлинности SSH** скопируйте значение ключа из файла **publicKey.pem**, который содержит открытый ключ, созданный Puttygen.
...
Сначала получите DNS-имя виртуальной машины на портале Azure. Щелкните **Обзор -> Виртуальные машины ->** имя своей виртуальной машины **-> Свойства**, а затем просмотрите значение в поле **Доменное имя** плитки **Свойства**.
...
В левой области щелкните **Подключение -> SSH -> Проверка подлинности** и нажмите кнопку **Обзор**, чтобы указать расположение файла **privateKey.ppk**, который содержит закрытый ключ, созданный с помощью Puttygen на этапе 1 «Создание образа». Пример:
