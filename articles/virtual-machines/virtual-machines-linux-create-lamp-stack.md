...
В поле **Ключ проверки подлинности SSH** скопируйте значение ключа из файла **publicKey.pem**, который содержит открытый ключ, созданный с помощью служебной программы Puttygen.
...
В левой области щелкните **Подключение -> SSH -> Проверка подлинности** и нажмите кнопку **Обзор**, чтобы указать расположение файла **privateKey.ppk**, который содержит закрытый ключ, созданный с помощью служебной программы Puttygen на этапе 1 «Создание образа». Пример:
...
####Тестирование Apache
Чтобы проверить, успешно ли установлено приложение Apache, перейдите к DNS-имени сервера Apache (например, по используемому в этой статье URL-адресу http://lampdemo.cloudapp.net/)). На странице должно отобразиться слово «Сработало!».![][14]
...
Затем выполните указания мастера и выберите свой **пароль пользователя root в MySQL**.
...
-	**Симптом** Если, когда вы используете любой SFTP-клиент (такой как FileZilla) для подключения к виртуальной машине и переходите в папку /var/www/html для публикации своего сайта, отображается сообщение об ошибке, похожее на следующее:  
