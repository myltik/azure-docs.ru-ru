...
В этом пошаговом руководстве мы создадим две виртуальные машины Azure под управлением Ubuntu 14.04. Назовем их и *script-vm* и *lamp-vm*. Когда вы будете самостоятельно выполнять описанные действия, используйте уникальные имена. Одну из этих виртуальных машин мы будем использовать для выполнения команд CLI, а другую — для развертывания приложения LAMP.

Также нам потребуется учетная запись службы хранилища Azure и ключ доступа к ней (все это можно получить на портале Azure).
...
Теперь расширение CustomScript для Linux можно развернуть на удаленной виртуальной машине с помощью интерфейса CLI Azure:
...
Дополнительную информацию об интерфейсе CLI Azure, Linux и расширении CustomScript см. в следующих статьях.
