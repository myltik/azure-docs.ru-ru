...
Если не удается подключиться к виртуальной машине Windows, можно сбросить пароль локального администратора или конфигурацию службы удаленного рабочего стола. Для сброса пароля можно использовать портал Azure или расширение VMAccess в Azure PowerShell. Если вы используете PowerShell, то убедитесь, что у вас установлен [и настроен последний модуль PowerShell](/powershell/azureps-cmdlets-docs) и вы вошли в подписку Azure. Вы также можете [выполнить эти действия для виртуальных машин, созданных с использованием классической модели](windows/classic/reset-rdp.md).
...
> [!NOTE] 
> - После изменения пароля и завершения операции на портале может пройти 3–5 минут, прежде чем это изменение вступит в силу на виртуальной машине. Если же изменение все равно не вступило в силу, перезапустите виртуальную машину.
> - Расширение VMAccess работает только для встроенной учетной записи локального администратора и не будет ничего делать для любых других локальных или доменных идентификаторов.
> - Если целевой компьютер является контроллером домена, то расширение сбрасывает или переименовывает учетную запись администратора домена.


### <a name="reset-the-remote-desktop-service-configuration"></a>**Сброс конфигурации службы удаленных рабочих столов**

Выберите виртуальную машину Windows, а затем щелкните **Поддержка и устранение неполадок** > **Сбросить пароль**. Отобразится колонка сброса пароля, как показано ниже.

![Сброс конфигурации удаленного рабочего стола](./media/virtual-machines-windows-reset-rdp/Portal-RM-RDP-Reset.png)
...
Убедитесь, что у вас [установлен и настроен последний модуль PowerShell](/powershell/azureps-cmdlets-docs) и вы вошли в подписку Azure, выполнив командлет `Login-AzureRmAccount`.

### <a name="reset-the-local-administrator-account-password"></a>**Сброс пароля учетной записи локального администратора**
Сбросьте пароль или имя пользователя администратора, выполнив командлет PowerShell [Set-AzureRmVMAccessExtension](https://msdn.microsoft.com/library/mt619447.aspx). Создайте учетные данные следующим образом.
...
### <a name="reset-the-remote-desktop-service-configuration"></a>**Сброс конфигурации службы удаленных рабочих столов**
Сбросьте параметры удаленного доступа к виртуальной машине с помощью командлета PowerShell [Set-AzureRmVMAccessExtension](https://msdn.microsoft.com/library/mt619447.aspx). Следующий пример сбрасывает параметры расширения доступа `myVMAccess` на виртуальной машине `myVM` в группе ресурсов `myResourceGroup`.
