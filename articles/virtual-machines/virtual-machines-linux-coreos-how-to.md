<properties
	...
	description="Описывает CoreOS, создание кластера виртуальных машин CoreOS в Azure и его основы его использования."
...
>[AZURE.NOTE]В этой статье показано, как использовать интерфейс командной строки Azure для выполнения команд управления службой, чтобы создавать виртуальные машины CoreOS. Чтобы приступить к работе с CoreOS в диспетчере ресурсов Azure, попробуйте этот [шаблон быстрого запуска](https://azure.microsoft.com/documentation/templates/coreos-with-fleet-multivm/).
...
3. Создайте облачную службу для основного кластера, введя `azure service create <cloud-service-name>`, где <*cloud-service-name*> — это имя для облачной службы CoreOS. В этом примере используется имя **`coreos-cluster`**. Необходимо будет повторно использовать имя, которое вы выбрали, для создания экземпляров виртуальной машины CoreOS в облачной службе.

	Одно примечание: если до этого вы наблюдали за своей работой в [предварительной версии портала](https://portal.azure.com), то обнаружите, имя вашей облачной службы также является именем группы ресурсов и домена, как показано на следующем рисунке:
