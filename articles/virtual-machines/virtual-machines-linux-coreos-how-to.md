...
[AZURE.INCLUDE [learn-about-deployment-models](../../includes/learn-about-deployment-models-include.md)]Эта статья описывает использование Azure CLI для создания виртуальных машин CoreOS с помощью классической модели развертывания. Вы также можете создать виртуальные машины CoreOS с помощью шаблонов в [модели развертывания диспетчера ресурсов](https://azure.microsoft.com/documentation/templates/coreos-with-fleet-multivm/).
...
## <a id='usingcoreos'>Практическое руководство. Использование CoreOS в Azure</a>
...
> [AZURE.NOTE]В этом разделе предполагается, что у вас нет этих ключей и вы должны создать файлы `myPrivateKey.pem` и `myCert.pem`. Если у вас уже есть открытый и закрытый ключи, сохраненные в `~/.ssh/id_rsa`, можно просто ввести `openssl req -x509 -key ~/.ssh/id_rsa -nodes -days 365 -newkey rsa:2048 -out myCert.pem`, чтобы получить PEM-файл, который необходимо передать в Azure.
...
> [AZURE.NOTE]Не забудьте ввести `cat etcdid` для получения идентификатора обнаружения etcd из файла `etcdid`, созданного ранее, и заменить `<token>` в следующем файле `cloud-config.yaml` сгенерированным числом из вашего файла `etcdid`. Если в конце вы не сможете проверить работу вашего кластера, возможно, именно этот шаг вы упустили!
...
3. Создайте облачную службу для базового кластера, введя `azure service create <cloud-service-name>`, где <*cloud-service-name*> — это имя для облачной службы CoreOS. В этом примере используется имя **`coreos-cluster`**. Необходимо будет повторно использовать имя, которое вы выбрали, для создания экземпляров виртуальной машины CoreOS в облачной службе.
	...
	Одно примечание: если до этого вы наблюдали за своей работой на [портале предварительной версии](https://portal.azure.com), то обнаружите, что имя вашей облачной службы также является именем группы ресурсов и домена, как показано на следующем рисунке.

	![][CloudServiceInNewPortal]

4. Подключитесь к своей облачной службе и создайте в ней новую виртуальную машину CoreOS с помощью команды **azure vm create**. Вы передадите расположение сертификата X.509 в параметре **--ssh-cert**. Создайте первый образ виртуальной машины. Для этого введите следующее и замените **coreos-cluster** именем созданной облачной службы:
...
Наконец, давайте протестируем кластер CoreOS из локального клиента Linux. Вы можете установить **fleetctl** с помощью **npm** или установить **fleetctl** и выполнить сборку **fleetctl** в локальном клиенте самостоятельно. Для **fleetctl** требуется **golang**,поэтому этот компонент необходимо установить первым, введя:
