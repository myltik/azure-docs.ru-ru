...
3. Создайте облачную службу для основного кластера, введя `azure service create <cloud-service-name>`, где <*cloud-service-name*> — это имя для облачной службы CoreOS. В этом примере используется имя **`coreos-cluster`**. Необходимо будет повторно использовать имя, которое вы выбрали, для создания экземпляров виртуальной машины CoreOS в облачной службе.
	...
	Одно примечание: если до этого вы наблюдали за своей работой в [предварительной версии портала](https://portal.azure.com), то обнаружите, что имя вашей облачной службы также является именем группы ресурсов и домена, как показано на следующем рисунке:
