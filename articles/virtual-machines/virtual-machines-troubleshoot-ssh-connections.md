...
## Устранение распространенных ошибок SSH в классической модели развертывания
...
1. **Выполните сброс удаленного доступа** на [портале Azure](https://portal.azure.com). Click **Просмотреть все** > **Виртуальные машины (классические)**, затем выберите виртуальные машины, которые нужно сбросить, и щелкните **Сброс удаленного доступа**.

	![Снимок экрана, на котором показан сброс конфигурации SSH](./media/virtual-machines-troubleshoot-ssh-connections/Portal-SSH-Reset-Windows.png)

2. **Перезапустите** виртуальную машину. На [портале Azure](https://portal.azure.com) щелкните **Просмотреть все** > **Виртуальные машины (классические)**, затем выберите виртуальные машины, которые нужно сбросить, и щелкните **Перезапуск**. На [классическом портале Azure](https://manage.windowsazure.com) откройте **Виртуальные машины** > **Экземпляры** и выберите **Перезагрузить**.
...
## Устранение распространенных ошибок SSH в модели развертывания диспетчера ресурсов
...
1. **Сбросьте SSH-подключение** к виртуальной машине Linux в командной строке с помощью Azure CLI или Azure PowerShell. Убедитесь, что у вас установлен [агент Linux для Microsoft Azure](virtual-machines-linux-agent-user-guide.md) версии 2.0.5 или более поздней.
	...
	**Использование Azure PowerShell**
	...
	c. Запустите расширение `VMAccessForLinux` для сброса SSH-подключения, как в следующем примере. (При использовании Azure PowerShell 1.0 следующим командлетом является командлет `Set-AzureRMVMExtension`.)
...
2. **Перезапустите** виртуальную машину Linux через портал. На [портале Azure](https://portal.azure.com) последовательно выберите **Просмотреть все** > **Виртуальные машины** > имя вашей виртуальной машины Windows > **Перезагрузить**.

	![Снимок экрана, на котором показан перезапуск виртуальной машины версии 2](./media/virtual-machines-troubleshoot-ssh-connections/Portal-SSH-Restart-V2-Windows.png)
