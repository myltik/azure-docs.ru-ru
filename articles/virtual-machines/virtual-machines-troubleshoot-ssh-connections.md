<properties 
	...
	description="Пошаговый процесс обнаружения источника проблемы и устранение проблемы с созданием SSH-подключения к виртуальной машине Linux, работающей в среде Microsoft Azure."
...
Данная статья описывает методический подход к устранению неполадок подключения Secure Shell (SSH) к виртуальной машине Azure под управлением Linux.

## Шаг 1. Сброс конфигурации, пароля или ключа SSH

Следуйте инструкциям в статье [Сброс пароля или ключа SSH в виртуальных машинах Linux](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md). Следуя этим инструкциям, вы сможете:
...
На портале управления Azure сделайте вот что.
...
1. Щелкните **Обзор** > **Виртуальные машины** > *Имя виртуальной машины*. Для виртуальной машины на основе диспетчера ресурсов щелкните **Обзор** > **Виртуальные машины (v2)** > *Имя виртуальной машины*. Панель состояния виртуальной машины должна отображать состояние **Работает**. Прокрутите страницу вниз для просмотра последних действий в ресурсах вычислений, хранения и сетевых ресурсах.
...
- Chmod 700 ~/.ssh;
- Chmod 644 ~/.ssh/*.pub;
- Chmod 600 ~/.ssh/id_rsa (или другие файлы, в которых могут храниться ваши закрытые ключи);
- Chmod 644 ~/.ssh/known_hosts (содержит узлы, с которыми вы установили подключение по SSH-протоколу).
...
Если у вас нет такого компьютера, вы можете легко создать вместо него новую виртуальную машину Azure, размещенную в собственной облачной службе. Дополнительные сведения см. в статье [Создание виртуальной машины Linux в среде Azure](virtual-machines-linux-tutorial.md). Завершив проверку, удалите виртуальную машину и облачную службу.
...
Чтобы устранить конечную точку облачной службы и список управления доступом из числа возможных источников проблем и ошибок в настройках, убедитесь, что с другой виртуальной машины, расположенной в той же виртуальной сети, можно устанавливать SSH-подключения к вашей виртуальной машине Azure.
...
- Конфигурация конечной точки для SSH-трафика на целевой виртуальной машине. Частный TCP-порт на конечной точке должен совпадать с TCP-портом, который прослушивает SSH-служба на виртуальной машине (по умолчанию это порт 22). Для виртуальных машин на основе диспетчера ресурсов, созданных с помощью шаблонов, убедитесь, проверьте номер TCP-порта на портале Azure предварительной версии. Откройте **Обзор** > **Виртуальные машины (v2)** > *Имя виртуальной машины* > **Параметры** > **Конечные точки**.
...
Если вы еще не сбросили пароль, следуйте инструкциям в статье [Сброс пароля или ключа SSH в виртуальных машинах Linux](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md).
...
Если вы выполнили действия с первого по четвертое из этой статьи и опубликовали свой вопрос на форумах поддержки Azure, но это вам не помогло установить SSH-подключение, возможно, следует удалить и повторно создать виртуальную машину.
...
[Сброс пароля или ключа SSH в виртуальных машинах Linux](virtual-machines-linux-use-vmaccess-reset-password-or-ssh.md)
