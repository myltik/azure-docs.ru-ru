...
Azure хранит образ в папке **Images** вместе со всеми передаваемыми пользователем образами. Дополнительную информацию об образах см. в статье [Об образах виртуальной машины в Azure][].

## Перед началом работы

В шагах этой статьи предполагается, что вы уже создали виртуальную машину Azure с использованием классической модели развертывания и настроили операционную систему, а также присоединили диски данных. Если необходимо создать виртуальную машину, прочтите статью [Создание виртуальной машины Linux][].
...
4. Введите **Exit**, чтобы закрыть SSH-клиент.
	...
	>[AZURE.NOTE] Далее предполагается, что вы уже [установили интерфейс командной строки Azure CLI](../xplat-cli-install.md) (Azure CLI) на клиентском компьютере. Все следующие действия можно также выполнить на [классическом портале Azure][].
...
7. Завершите работу виртуальной машины, которая была отозвана на предыдущих шагах, с помощью следующей команды.
	...
	>[AZURE.NOTE] Вы можете получить список всех виртуальных машин, созданных в подписке, с помощью команды `azure vm list`.
