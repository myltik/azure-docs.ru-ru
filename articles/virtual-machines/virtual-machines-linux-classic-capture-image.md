...
4. Введите **Exit** , чтобы закрыть SSH-клиент.
   ...
   > [!NOTE]
   > Далее предполагается, что вы уже [установили интерфейс командной строки Azure CLI](../xplat-cli-install.md) (Azure CLI) на клиентском компьютере. Все следующие действия можно также выполнить на [классическом портале Azure][Azure classic portal].
...
9. Новый образ станет доступным в списке образов, которые можно использовать для настройки любой новой виртуальной машины. Его можно просмотреть с помощью следующей команды:
   ...
   На [классическом портале Azure][Azure classic portal] он отображается в списке **ОБРАЗЫ**.
