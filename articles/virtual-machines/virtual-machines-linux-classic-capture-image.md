---
title: Запись образа виртуальной машины Linux | Microsoft Docs
...
Узнайте, как [выполнить эти действия с помощью модели Resource Manager](virtual-machines-linux-capture-image.md).
...
Azure хранит образ в папке **Images** вместе со всеми передаваемыми пользователем образами. Дополнительную информацию об образах см. в статье [Об образах виртуальной машины в Azure][Об образах виртуальной машины в Azure].

## Перед началом работы
В шагах этой статьи предполагается, что вы уже создали виртуальную машину Azure с использованием классической модели развертывания и настроили операционную систему, а также присоединили диски данных. Если необходимо создать виртуальную машину, прочтите статью [Создание виртуальной машины Linux][Создание виртуальной машины Linux].

## Запись виртуальной машины
1. [Подключитесь к виртуальной машине](virtual-machines-linux-mac-create-ssh-keys.md) с помощью выбранного клиента SSH.
...
4. Введите **Exit**, чтобы закрыть SSH-клиент.
   ...
   > [!NOTE]
   > Далее предполагается, что вы уже [установили интерфейс командной строки Azure CLI](../xplat-cli-install.md) (Azure CLI) на клиентском компьютере. Все следующие действия можно также выполнить на [классическом портале Azure][классическом портале Azure].
   > 
   > 
5. На клиентском компьютере откройте интерфейс командной строки Azure и войдите в подписку Azure. Дополнительные сведения см. в статье [Подключение к среде Azure с использованием интерфейса командной строки Azure](../xplat-cli-connect.md).
