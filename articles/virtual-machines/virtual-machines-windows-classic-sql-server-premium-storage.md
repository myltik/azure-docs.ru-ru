...
1. Получите список присоединенных к виртуальной машине дисков с помощью команды **Get-AzureVM**:
    ...
    Get-AzureVM -ServiceName <имя\_службы> -Name <имя\_ВМ> | Get-AzureDataDisk
...
Вы можете использовать существующий образ. Вы также можете [взять образ существующей машины](virtual-machines-windows-classic-capture-image.md). Обратите внимание, виртуальная машина, образ которой вы создаете, не обязательно должна быть серии DS*. Далее показано, как скопировать созданный образ в учетную запись хранения уровня "Премиум" с помощью командлета PowerShell **Start-AzureStorageBlobCopy**.
...
### Миграция развернутых служб AlwaysOn, допускающая некоторое время простоя
...
1. **Миграция в новый кластер AlwaysOn.**
...
### Миграция развернутых служб AlwaysOn для минимального времени простоя
...
## Приложение. Миграция многосайтового кластера AlwaysOn в хранилище класса Premium

В оставшейся части настоящего раздела представлен подробный пример преобразования кластера AlwaysOn с несколькими сайтами в хранилище класса Premium. В примере также показан перевод прослушивателя с использования внешней подсистемы балансировки нагрузки (ELB) на внутреннюю подсистему балансировки нагрузки (ILB).
...
#### Шаг 2. Увеличение допустимого числа сбоев для ресурсов (необязательно)
...
#### Шаг 3. Добавление ресурса IP-адреса для группы кластера (необязательно)
...
Плавность перехода зависит от того, как используется и обновляется DNS. При установке AlwaysOn создается группа ресурсов кластера Windows. Если открыть диспетчер отказоустойчивости кластеров, вы увидите, что он будет включать по меньшей мере три ресурса. Данный документ касается следующих двух ресурсов.
