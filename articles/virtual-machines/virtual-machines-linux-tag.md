...
Теги также поддерживаются для ресурсов, которые уже созданы с помощью интерфейса командной строки Azure. Для начала настройте [среду интерфейса командной строки Azure][]. Войдите в свою подписку с помощью Azure CLI и переключитесь в режим ARM (`azure config mode arm`).
