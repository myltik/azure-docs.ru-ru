<properties 
	pageTitle="Бизнес-аналитика SQL Server на виртуальных машинах Azure"
	description="В этом разделе описаны функции бизнес-аналитики, доступные для SQL Server, запущенного на виртуальных машинах Azure."
...
1. Оплата виртуальных машин Azure с установленным SQL Server по часовой ставке. См. подраздел "SQL Server" раздела [Виртуальные машины: цены](http://azure.microsoft.com/pricing/details/virtual-machines/#Sql).

Дополнительные сведения о лицензировании и текущих тарифах см. в разделе [Вопросы и ответы по лицензированию виртуальных машин](http://azure.microsoft.com/pricing/licensing-faq/%20).
...
![Образ SQL в коллекции виртуальных машин Azure](./media/virtual-machines-sql-server-business-intelligence/IC741367.png)
...
<sup>1</sup> Дополнительные сведения о SharePoint и виртуальных машинах Azure см. в разделах [Архитектура Microsoft Azure для SharePoint 2013](https://technet.microsoft.com/library/dn635309.aspx) и [Развертывание SharePoint на виртуальных машинах Microsoft Azure](https://www.microsoft.com/download/details.aspx?id=34598).

![PowerShell](./media/virtual-machines-sql-server-business-intelligence/IC660119.gif) Выполните следующую команду PowerShell, чтобы получить список установленных служб, содержащих в имени службы значение SQL.

	get-service | Where-Object{ $_.DisplayName -like '*SQL*' } | Select DisplayName, status, servicetype, dependentservices | format-Table -AutoSize

## Общие рекомендации

- При использовании SQL Server Enterprise Edition минимальный размер, рекомендуемый для виртуальной машины, — **А3**. Размер виртуальной машины **A4** рекомендуется использовать для развертывания бизнес-аналитики SQL Server в службах Analysis Services и Reporting Services.
...
- В рамках управления дисками рекомендуется хранить данные, журналы и файлы резервных копий на дисках, отличных от **C:** и **D:**. Например, создайте диски данных **E:** и **F:**.
	...
	- **D:** — это временный диск, который используется главным образом для файла подкачки. Диск **D:** не является сохраняемым и не сохраняется в хранилище BLOB-объектов. Задачи управления, такие как изменение размера виртуальной машины, приводят к сбросу диска **D:**. **НЕ** рекомендуется использовать диск **D:** для файлов базы данных, включая tempdb.

	Дополнительные сведения о создании и подключении дисков см. в разделе [Подключение диска с данными к виртуальной машине](http://www.windowsazure.com/manage/windows/how-to-guides/attach-a-disk/).
...
>[AZURE.NOTE]Аналогичные материалы, где для настройки сервера отчетов используются сценарии Windows PowerShell, см. в разделе [Использование PowerShell для создания виртуальной машины Azure с помощью сервера отчетов, работающего в собственном режиме](virtual-machines-sql-server-create-native-mode-report-server-powershell.md).
...
- Для подключения выберите имя виртуальной машины и нажмите кнопку **Подключение**. Открывает подключение к удаленному рабочему столу, и автоматически заполняется имя компьютера.
...
1. На **начальном** экране введите **Reporting Services** для просмотра списка приложений.
...
1. Убедитесь, что для параметра **Состояние сервера отчетов** указано значение **Работает**. Если вы хотите проверить службу в Windows Server Manager, она является службой Windows **SQL Server Reporting Services**.

1. Щелкните **Учетная запись службы** и при необходимости измените учетную запись. При использовании виртуальной машины в среде без присоединения к домену встроенной учетной записи **ReportServer** достаточно. Дополнительные сведения об учетной записи службы см. в разделе [Учетная запись службы](https://msdn.microsoft.com/library/ms189964.aspx).

1. Щелкните **URL-адрес веб-службы** на левой панели.

1. Нажмите кнопку **Применить** для настройки значений по умолчанию.
...
1. Проверьте значение **Имя сервера** и нажмите кнопку **Проверка соединения**.

1. Если получен результат **Проверка соединения завершена успешно**, нажмите кнопку **ОК** и кнопку **Далее**.

1. Обратите внимание, что в качестве имени базы данных используется **ReportServer**, а для параметра **Режим сервера отчетов** указано значение **Собственный**, а затем нажмите кнопку **Далее**.
...
Сведения о разрешениях сервера отчетов см. в разделе [Предоставление разрешений для сервера отчетов в собственном режиме](https://msdn.microsoft.com/library/ms156014.aspx).
...
1. Перейдите к http://localhost/reports на виртуальной машине.
...
- Создайте жесткий диск VHD, содержащий отчеты, а затем отправьте и подключите этот диск.
	...
	1. Отправьте VHD-файл в Azure с помощью командлета Add-AzureVHD, руководствуясь указаниями из раздела [Создание и передача виртуального жесткого диска Windows Server в Azure](virtual-machines-create-upload-vhd-windows-server.md).
...
1. Щелкните **Microsoft SQL Server 2014** или **Microsoft SQL Server 2012** и затем **Средства настройки**.
...
1. В левой области мастера установки SQL Server щелкните элемент **Установка** и затем элемент **Новая установка изолированного экземпляра SQL Server или добавление компонентов к существующей установке**.

	- Если вы видите элемент **Обзор папок**, перейдите к c:\\SQLServer\_12.0\_full или c:\\SQLServer\_11.0\_full и нажмите кнопку **ОК**.
...
**Экземпляр по умолчанию** служб Analysis Services прослушивает TCP-порт **2383**. Откройте этот порт в брандмауэре виртуальных машин. Кластеризованный именованный экземпляр служб Analysis Services также прослушивает порт **2383**.
...
В брандмауэре виртуальных машин откройте порт **2382** и статический порт именованного экземпляра служб Analysis Services.
...
1. Используйте SQL Server Management Studio для создания статического порта именованного экземпляра служб Analysis Services, обновив значение "Порт" в общих свойствах служб Analysis Services в табличном режиме. Дополнительные сведения см. в подразделе "Использование фиксированного порта для экземпляра по умолчанию или именованного экземпляра" раздела [Настройка брандмауэра Windows для разрешения доступа к службам Analysis Services](https://msdn.microsoft.com/library/ms174937.aspx#bkmk_fixed).

1. Перезагрузите табличный экземпляр служб Analysis Services.

Дополнительные сведения см. в подразделе **Конечные точки виртуальной машины и порты брандмауэра** этого документа.
...
- SQL Server: см. подраздел "Выполнение шагов по подключению к виртуальной машине с помощью SQL Server Management Studio" раздела [Подготовка виртуальной машины SQL Server в Azure](virtual-machines-provision-sql-server.md).
