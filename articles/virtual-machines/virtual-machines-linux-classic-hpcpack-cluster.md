...
* **Шаблон Azure Resource Manager**. Используйте шаблон из Azure Marketplace или шаблон быстрого запуска от сообщества для автоматизации создания кластера в рамках модели развертывания с помощью Resource Manager. Например, шаблон [кластера пакета HPC для рабочих нагрузок Linux](https://azure.microsoft.com/marketplace/partners/microsofthpc/newclusterlinuxcn/) из Azure Marketplace создает полную инфраструктуру кластера пакета HPC для рабочих нагрузок высокопроизводительных вычислительных систем Linux.

* **Сценарий PowerShell**. Используйте [сценарий развертывания IaaS пакета Microsoft HPC](virtual-machines-windows-classic-hpcpack-cluster-powershell-script.md) (**New-HpcIaaSCluster.ps1**) для автоматизации полного развертывания кластера в рамках классической модели развертывания. Этот сценарий Azure PowerShell использует образ виртуальной машины пакета HPC в Azure Marketplace для быстрого развертывания и предоставляет исчерпывающий набор параметров конфигурации для развертывания вычислительных узлов Linux.

Общие сведения о вариантах развертывания кластера пакета HPC см. в разделе [Варианты создания кластера высокопроизводительных вычислений (HPC) в Azure и управления им с помощью пакета Microsoft HPC](virtual-machines-linux-hpcpack-cluster-options.md).

### Предварительные требования

* **Подписка на Azure**. Вы можете использовать подписку на службу Azure Global или Azure China. Если ее нет, можно создать [бесплатную учетную запись](https://azure.microsoft.com/pricing/free-trial/) всего за несколько минут.
...
* **Дистрибутивы Linux**. В настоящее время пакет HPC поддерживает следующие дистрибутивы Linux для вычислительных узлов. Можно использовать версии этих дистрибутивов, доступные в Marketplace, или создать собственные.
    ...
    * SUSE Linux Enterprise Server: SLES 12, SLES 12 (Premium), SLES 12 для HPC, SLES 12 для HPC (Premium).
    * Ubuntu Server: 14.04 LTS, 16.04 LTS.



    >[AZURE.TIP]Чтобы использовать сеть Azure RDMA с виртуальными машинами, оснащенными вычислительными узлами размера A8 и A9, выберите в Marketplace один из образов SUSE Linux Enterprise Server 12 HPC или один образов на основе CentOS. Дополнительные сведения см. в разделе [Сведения о ресурсоемких вычислительных экземплярах A8, A9, A10 и A11](virtual-machines-linux-a8-a9-a10-a11-specs.md).

Ниже перечислены дополнительные компоненты, необходимые для развертывания кластера с помощью сценария развертывания IaaS пакета HPC.
...
* **Сценарий развертывания IaaS из пакета HPC**. Скачайте и распакуйте последнюю версию сценария из [Центра загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=44949). Вы можете проверить версию скрипта, выполнив команду `.\New-HPCIaaSCluster.ps1 –Version`. В этой статье используется сценарий версии 4.4.1 или более поздней версии.
...
Сценарий развертывания IaaS из пакета HPC использует входной XML-файл конфигурации, описывающий инфраструктуру кластера HPC. В следующем примере файл конфигурации разворачивает небольшой кластер, состоящий из головного узла пакета HPC и двух вычислительных узлов CentOS 7.0 Linux размера A7. Измените этот файл в соответствии с параметрами своей среды и необходимой конфигурацией кластера. Сохраните его, например, как HPCDemoConfig.xml. Например, может понадобиться указать имя подписки и уникальные имя учетной записи хранения и имя облачной службы, а также выбрать другой образ Linux, поддерживаемый для вычислительных узлов. Дополнительные сведения об элементах файла конфигурации см. в файле Manual.rtf в папке скрипта или в статье, посвященной [созданию кластера HPC с помощью скрипта развертывания IaaS из пакета HPC](virtual-machines-windows-classic-hpcpack-cluster-powershell-script.md).
...
3. Выполните следующую команду, чтобы развернуть кластер пакета HPC. В этом примере предполагается, что путь к файлу конфигурации — E:\\HPCDemoConfig.xml.
    ...
    b. Затем сценарий начинает проверку файла конфигурации. Это занимает до нескольких минут, в зависимости от сетевого подключения.
...
В этом примере allvhdsje — это имя учетной записи хранения, storageaccountkey — ключ этой учетной записи, а rdma — имя файлового ресурса папки Azure. Общая папка Azure будет смонтирована на диск Z: головного узла.
