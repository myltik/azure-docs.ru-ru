<properties
    pageTitle="Создание виртуальной машины SQL Server в Azure PowerShell (Resource Manager) | Microsoft Azure"
...
- [Azure PowerShell](../powershell-install-configure.md) версии 1.0.0 или более поздней (в этом руководстве использовалась версия 1.0.4).
...
Используйте следующие переменные, чтобы определить образ, который будет применен для виртуальной машины. В этом примере используется образ пробной версии SQL Server 2016 RC3.
...
Кроме того, выполнив команду Get-AzureRmVMImageSku, можно получить сведения о единицах хранения, доступных для предложения. При выполнении следующей команды отображаются все единицы хранения для предложения **SQL2014SP1-WS2012R2**.

    Get-AzureRmVMImageSku -Location 'East US' -Publisher 'MicrosoftSQLServer' -Offer 'SQL2014SP1-WS2012R2' | Select Skus

## Создание группы ресурсов

Если используется модель развертывания с помощью Resource Manager, первый создаваемый объект — группа ресурсов. Мы используем командлет [New-AzureRmResourceGroup](https://msdn.microsoft.com/library/mt678985.aspx), чтобы создать группу ресурсов Azure и соответствующие ресурсы. При этом имя и расположение группы ресурсов определяются в переменных, инициализированных ранее.
...
Виртуальной машине требуются ресурсы хранения для диска операционной системы, а также файлов данных и журналов SQL Server. Для упрощения мы создадим один диск для всех ресурсов. Позже можно подключить дополнительные диски, выполнив командлет [Add-Azure Disk](https://msdn.microsoft.com/library/azure/dn495252.aspx), чтобы поместить файлы данных и журналов SQL Server на выделенные диски. Мы выполним командлет [New-AzureRmStorageAccount](https://msdn.microsoft.com/library/mt607148.aspx), чтобы создать учетную запись хранения в новой группе ресурсов. При этом имя учетной записи хранения, имя хранилища и расположение определяются в переменных, инициализированных ранее.
...
Сначала мы создадим конфигурацию подсети для нашей виртуальной сети. Для целей руководства мы создадим подсеть по умолчанию, используя командлет [New-AzureRmVirtualNetworkSubnetConfig](https://msdn.microsoft.com/library/mt619412.aspx). Имя подсети и префикс адреса для нашей конфигурации подсети в виртуальной сети определяются в переменных, инициализированных ранее.

>[AZURE.NOTE] Выполнив этот командлет, можно определить дополнительные свойства конфигурации подсети виртуальной сети. Однако это выходит за рамки данного руководства.
...
>[AZURE.NOTE] Выполнив этот командлет, можно определить дополнительные свойства общедоступного IP-адреса. Однако это выходит за рамки данного руководства. Кроме того, можно создать частный или статический адрес, но это также выходит за рамки данного руководства.
...
Теперь мы можем задать свойства операционной системы виртуальной машины. Мы используем командлет [Set-AzureRmVMOperatingSystem](https://msdn.microsoft.com/library/mt603843.aspx), чтобы задать тип операционной системы Windows. Для этого нужно установить [агент виртуальной машины](virtual-machines-windows-classic-agents-and-extensions.md). Укажите, что командлет включает автоматическое обновление, и задайте имя виртуальной машины, имя компьютера и учетные данные в переменных, инициализированных ранее.
...
Теперь этот компьютер можно просмотреть на портале Azure, чтобы увидеть [его общедоступный IP-адрес и полное доменное имя](virtual-machines-windows-portal-sql-server-provision.md#Connect).

## Пример сценария

Файл ниже содержит полный сценарий PowerShell для этого руководства. В нем предполагается, что подписка Azure уже настроена соответствующим образом для использования команд **Add-AzureRmAccount** и **Select-AzureRmSubscription**.
