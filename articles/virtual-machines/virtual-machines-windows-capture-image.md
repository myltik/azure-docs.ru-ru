...
5. Запишите образ виртуальной машины в целевой контейнер хранилища, используя следующую команду:
	...
	>[AZURE.NOTE] Чтобы найти расположение образа, откройте локальный шаблон JSON-файла. Последовательно выберите **resources** > **storageProfile** > **osDisk** > **image** > **uri**, чтобы получить полный путь к образу. Сейчас простой способ проверки этих образов на портале отсутствует, так как *системный* контейнер в учетной записи хранения скрыт. Поэтому несмотря на то, что переменная `-Path` не является обязательной, вы наверняка захотите использовать ее для локального сохранения шаблона и определения URL-адреса образа. Кроме того, его можно узнать с помощью **обозревателя хранилищ Azure**, работа которого объясняется в следующем разделе.
...
8. Чтобы получить VHD и шаблон нового образа, скачайте и установите средство Azure для управления ресурсами хранилища — [обозреватель хранилищ Azure](http://storageexplorer.com/). Обозреватель хранилищ Azure будет установлен локально на компьютере.
