...
4. В диалоговом окне **Программа подготовки системы** выполните указанные далее действия.
...
В этой статье предполагается, что у вас установлен Azure PowerShell версии 1.0.x. Рекомендуется использовать эту версию, так как новые возможности диспетчера ресурсов не будут добавлены в более старые версии PowerShell. Ознакомьтесь с [Azure PowerShell 1.0](https://azure.microsoft.com/blog/azps-1-0/), чтобы получить дополнительные сведения о различиях версий.
...
4. Теперь необходимо задать состояние виртуальной машины **Обобщена**. Обратите внимание, что это необходимо сделать потому, что Azure не поддерживает ранее приведенный способ (с использованием `sysprep`).
...
5. Запишите образ виртуальной машины в целевой контейнер хранилища, используя следующую команду:
	...
	Переменная `-Path` не является обязательной. Ее можно использовать для сохранения шаблона JSON локально. Переменная `-DestinationContainerName` представляет имя контейнера для хранения образов. URL-адрес сохраненного образа будет выглядеть так: `https://YourStorageAccountName.blob.core.windows.net/system/Microsoft.Compute/Images/YourImagesContainer/YourTemplatePrefix-osDisk.xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx.vhd`. Он будет создан в той же учетной записи, что и исходная виртуальная машина.

	>[AZURE.NOTE] Чтобы найти расположение образа, откройте локальный шаблон JSON-файла. Последовательно выберите **Ресурсы** > **Профиль хранилища** > **Диск ОС** > **Образ** > **URI**, чтобы получить полный путь к образу. Сейчас простой способ проверки этих образов на портале отсутствует, так как *системный* контейнер в учетной записи хранения скрыт. Поэтому несмотря на то, что переменная `-Path` является необязательной, вы наверняка захотите использовать ее для локального сохранения шаблона и определения URL-адреса образа. Кроме того, его можно узнать с помощью **обозревателя службы хранилища Azure**, работа которого объясняется в следующем разделе.


### Использование обозревателя ресурсов Azure (предварительная версия)

[Обозреватель ресурсов Azure (предварительная версия)](https://azure.microsoft.com/blog/azure-resource-explorer-a-new-tool-to-discover-the-azure-api/) — это новое средство, предназначенное для управления ресурсами Azure, созданными при применении модели развертывания с использованием диспетчера ресурсов. С помощью этого средства можно легко выполнять следующие задачи:
...
Чтобы узнать, что еще можно сделать с помощью этого мощного средства, просмотрите видеоролик [об обозревателе Azure Resource Manager с Дэвидом Эббо (David Ebbo)](https://channel9.msdn.com/Shows/Azure-Friday/Azure-Resource-Manager-Explorer-with-David-Ebbo).
...
7. Под кнопкой действия **Запись** можно задать значения для записи образа. Внесенные значения могут выглядеть следующим образом.
	...
	Нажмите кнопку действия **Запись**, чтобы записать образ виртуальной машины. Будет создан новый VHD для образа, а также файла шаблона JSON. Сейчас этот файл недоступен ни через обозреватель ресурсов, ни через [портал Azure](https://portal.azure.com).
...
8. Чтобы получить VHD и шаблон нового образа, скачайте и установите средство Azure для управления ресурсами хранилища — [обозреватель службы хранилища Azure](http://storageexplorer.com/). Обозреватель хранилищ Azure будет установлен локально на компьютере.
	...
	- В левой части будет расположена учетная запись хранения виртуальной машины, образ которой был записан при выполнении описанных выше действий. Дважды щелкните меню **Система** под ней. В правой части вы увидите содержимое папки **system**.
