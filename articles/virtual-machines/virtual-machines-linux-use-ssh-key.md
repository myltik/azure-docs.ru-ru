...
В качестве параметров команды **ssh** указываются имя пользователя для входа в систему, сетевой адрес компьютера и номер порта для подключения, а также множество других специальных параметров. (Дополнительные сведения о команде **ssh** можно получить [здесь](https://en.wikipedia.org/wiki/Secure_Shell).)
