...
В этом разделе описывается использование служебных программ **ssh-keygen** и **openssl** в Linux и Mac для создания и использования файлов формата **ssh-rsa** и **pem** для обеспечения безопасности при обмене данными с виртуальными машинами Azure под управлением Linux. В новых развертываниях рекомендуется создавать виртуальные машины Azure под управлением Linux с помощью модели развертывания диспетчера ресурсов. В этом случае нужно указать файл открытого ключа или строку типа *ssh-rsa* (в зависимости от клиента развертывания). [Портал Azure](https://portal.azure.com) в настоящий момент принимает только строки формата **ssh-rsa** как для классического развертывания, так и для развертывания через диспетчер ресурсов.
