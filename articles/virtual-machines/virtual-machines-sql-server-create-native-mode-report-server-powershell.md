...
1. Выберите **SQL Server 2014 RTM Standard — Windows Server 2012 R2**, а затем щелкните стрелку, чтобы продолжить.
...
1. На следующей странице измените следующие поля:
	...
	- **DNS-имя облачной службы**. Это общедоступное DNS-имя облачной службы, связанной с виртуальной машиной. По умолчанию используется имя, введенное в качестве имени виртуальной машины. Если ниже в данном разделе вы создадите доверенный SSL-сертификат, то DNS-имя используется в качестве значения "**Кому выдан**" этого сертификата.
	...
	- **Конечные точки**. Сохраните конечные точки **Удаленный рабочий стол** и **PowerShell**, а затем добавьте конечную точку HTTP или HTTPS в зависимости от вашей среды.
		...
		- **HTTPS**. Общий и частный порты по умолчанию имеют номер **443**. Из соображений безопасности рекомендуется изменить частный порт и настроить брандмауэр и сервер отчетов на использование этого частного порта. Дополнительную информацию о конечных точках см. в статье [Настройка связи с виртуальной машиной](virtual-machines-set-up-endpoints.md). Обратите внимание, что при использовании порта, отличного от 443, следует изменить параметр **HTTPsport = 443** в сценарии HTTPS.
	
	- Нажмите кнопку «Далее». ![далее](./media/virtual-machines-sql-server-create-native-mode-report-server-powershell/IC692021.gif)

1. На последней странице мастера оставьте параметр по умолчанию **Установить агент ВМ**. В действиях, описанных в этом разделе, данный агент виртуальной машины не используется, но если планируется сохранить эту виртуальную машину, агент и расширения позволят расширить ее возможности. Дополнительные сведения об агенте ВМ см. в разделе [Агент виртуальной машины и расширения. Часть 1](http://azure.microsoft.com/blog/2014/04/11/vm-agent-and-extensions-part-1/). Одним из установленных и запущенных расширений по умолчанию является BGINFO, которое отображает на рабочем столе виртуальной машины системные данные, такие как внутренний IP-адрес и свободное место на диске.

1. Щелкните «Готово». ![ОК](./media/virtual-machines-sql-server-create-native-mode-report-server-powershell/IC660122.gif)

1. Параметр **Состояние** виртуальной машины получает значение **Запуск (Подготовка)** во время процесса подготовки и значение **Выполняется**, когда виртуальная машина подготовлена и готова к использованию.
...
1. Чтобы обеспечить доверие к корневому ЦС сертификата на локальной виртуальной машине, добавьте этот сертификат в список **Доверенные корневые центры сертификации**. Ниже приведены сводные сведения о необходимых действиях. Подробные инструкции по обеспечению доверия к ЦС см. в разделе [Установка сертификата сервера](https://technet.microsoft.com/library/cc740068).

	1. На классическом портале Azure выберите виртуальную машину и щелкните команду подключения. В зависимости от конфигурации браузера вам может быть предложено сохранить RDP-файл для подключения к виртуальной машине.
	...
	1. В меню **Файл** консольного приложения добавьте оснастку **Сертификаты**, при отображении запроса выберите **Учетная запись компьютера** и нажмите кнопку **Далее**.
	
	1. Выберите нужный **локальный компьютер** и нажмите кнопку **Готово**.
	...
	1. Для проверки дважды щелкните имя сертификата в области **Доверенные корневые центры сертификации** и убедитесь, что ошибки отсутствуют, а сам сертификат отображается. Если вы хотите использовать приведенный в этом разделе сценарий HTTPS для настройки сервера отчетов, в качестве параметра сценария требуется указать значение сертификата **Отпечаток**. **Для получения значения отпечатка** воспользуйтесь приведенной ниже процедурой. В разделе [Использование сценария для настройки сервера отчетов и HTTPS](#use-script-to-configure-the-report-server-and-HTTPS) также приведен пример сценария PowerShell для извлечения отпечатка.
		
		1. Дважды щелкните имя сертификата, например ssrsnativecloud.cloudapp.net.
		
		1. Откройте вкладку **Сведения**.
		
		1. Щелкните **Отпечаток**. Значение отпечатка появится в поле сведений, например ‎a6 08 3c df f9 0b f7 e3 7c 25 ed a4 ed 7e ac 91 9c 2c fb 2f.
...
1. В интегрированной среде сценариев Windows PowerShell откройте меню **Вид** и выберите **Показать область сценариев**.
...
1. На классическом портале Azure выберите виртуальную машину и щелкните команду подключения. Используйте имя пользователя и пароль, которые были настроены при создании виртуальной машины.
...
1. Запустите Центр обновления Windows и установите обновления для виртуальной машины. Если требуется перезагрузка виртуальной машины, перезапустите ее и повторно подключитесь к ней с классического портала Azure.
...
1. Щелкните элемент **База данных** на левой панели.
	...
	1. Оставьте значение по умолчанию для параметра **Имя сервера**, в качестве которого используется имя виртуальной машины, а также оставьте для параметра **Тип проверки подлинности** значение по умолчанию **Текущий пользователь** — **Встроенная система безопасности**. Нажмите кнопку **Далее**.
	
	1. Оставьте для параметра **Имя базы данных** значение по умолчанию **ReportServer** и нажмите кнопку **Далее**.
	
	1. Оставьте для параметра **Тип проверки подлинности** значение по умолчанию **Учетные данные службы** и нажмите кнопку **Далее**.
	
	1. На странице **Сводка** нажмите кнопку **Далее**.
	
	1. Завершив настройку, нажмите кнопку **Готово**.

1. Щелкните элемент **URL-адрес диспетчера отчетов** на левой панели. Оставьте для параметра **Виртуальный каталог** значение по умолчанию **Отчеты** и нажмите кнопку **Применить**.

1. Щелкните **Выйти**, чтобы закрыть диспетчер конфигурации служб Reporting Services.
...
1. Когда команда будет выполнена, в окне командной строки отобразится слово **ОК**.
...
- **Сценарий RS.exe**. Используйте сценарий RS.exe, чтобы скопировать элементы отчета с существующего сервера отчетов на виртуальную машину Microsoft Azure. Дополнительные сведения см. в разделе «С сервера отчетов в собственном режиме на сервер отчетов в собственном режиме — виртуальная машина Microsoft Azure» статьи [Образец скрипта программы rs.exe служб Reporting Services для переноса содержимого между серверами отчетов](https://msdn.microsoft.com/library/dn531017.aspx).
...
- **Использование сценария**. Используйте сценарий для копирования содержимого сервера отчетов. Дополнительные сведения см. в статье [Образец скрипта программы rs.exe служб Reporting Services для переноса содержимого между серверами отчетов](https://msdn.microsoft.com/library/dn531017.aspx).

## Минимизация затрат на неиспользуемую виртуальную машину

>[AZURE.NOTE]Чтобы свести к минимуму расходы на неиспользуемые виртуальные машины Azure, завершите работу виртуальной машины на классическом портале Azure. Если использовать для завершения работы виртуальной машины параметры электропитания Windows внутри нее, плата за эту виртуальную машину будет взиматься в полном объеме. Для сокращения расходов необходимо завершить работу виртуальной машины на классическом портале Azure. Если виртуальная машина больше не нужна, не забудьте удалить ее и связанные с ней VHD-файлы во избежание расходов на хранение. Дополнительные сведения см. на странице [Цены на виртуальные машины](http://azure.microsoft.com/pricing/details/virtual-machines).

## Дополнительные сведения

### Ресурсы

- Похожие материалы о развертывании SQL Server Business Intelligence и SharePoint 2013 с одним сервером см. в разделе [Использование Windows PowerShell для создания VM Azure с помощью SQL Server BI и SharePoint 2013](https://msdn.microsoft.com/library/azure/dn385843.aspx).

- Похожие материалы о развертывании SQL Server Business Intelligence и SharePoint 2013 с несколькими серверами см. в разделе [Развертывание SQL Server Business Intelligence на виртуальных машинах Azure](https://msdn.microsoft.com/library/dn321998.aspx).
...
- Пошаговые инструкции по созданию сервера отчетов служб Reporting Services в основном режиме без использования сценария см. в разделе [Размещение служб SQL Reporting Services на виртуальной машине Azure](http://adititechnologiesblog.blogspot.in/2012/07/hosting-sql-reporting-service-on-azure.html).
