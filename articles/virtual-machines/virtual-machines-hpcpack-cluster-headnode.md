...
>[AZURE.TIP]Для развертывания полного пакета кластера HPC в рабочей среде Azure мы рекомендуем использовать автоматизированный метод, такой как [сценарий развертывания IaaS пакета HPC](virtual-machines-hpcpack-cluster-powershell-script.md), или шаблон диспетчера ресурсов, такой как шаблон [пакета кластера HPC для рабочих нагрузок Windows](https://azure.microsoft.com/marketplace/partners/microsofthpc/newclusterwindowscn/). В разделе [Параметры кластера пакета HPC в Azure](virtual-machines-hpcpack-cluster-options.md) можно найти дополнительные шаблоны.
...
4. Используйте портал для указания настроек и создания виртуальной машины. Подробное описание шагов см. в учебнике [Создание виртуальной машины Windows на портале Azure](virtual-machines-windows-tutorial.md). Для первого развертывания можно оставить большинство параметров без изменения.
...
4. После создания и запуска виртуальной машины [подключитесь к виртуальной машине](virtual-machines-log-on-windows-server-preview.md). 
...
* Добавьте [узлы повышения Azure](virtual-machines-hpcpack-cluster-node-burst.md) в облачную службу для увеличения вычислительной емкости кластера по запросу. 
