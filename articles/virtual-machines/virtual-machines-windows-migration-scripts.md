...
## Проверка подлинности для доступа к подпискам классической модели развертывания и модели Azure Resource Manager
...
Если флаг -Deploy установлен, после создания файлов командлет развертывает шаблон <ServiceName>-<VMName>-setup.json, копирует большие двоичные объекты с диска исходной виртуальной машины, если для параметра DiskAction задано значение CopyDisks, а затем развертывает шаблон <ServiceName>-<VMName>-deploy.json, используя файл параметров <ServiceName>-<VMName>-parameters.json. После развертывания виртуальной машины выполняется императивный сценарий (для расширений агента виртуальной машины) или сценарий для копирования дисков (если они имеются).
