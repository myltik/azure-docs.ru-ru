...
В этой статье рассматривается подключение к виртуальной машине SQL Server с помощью модели на основе диспетчера ресурсов. Полное пошаговое руководство по подготовке и подключению см. в статье [Подготовка виртуальной машины SQL Server в Azure](virtual-machines-windows-portal-sql-server-provision.md).
...
Если это не было сделано во время подготовки, можно настроить SQL Server и виртуальные машины вручную, выполнив [действия по настройке подключения вручную, описанные в этой статье](#steps-for-manually-configuring-sql-server-connectivity-in-an-azure-vm).
