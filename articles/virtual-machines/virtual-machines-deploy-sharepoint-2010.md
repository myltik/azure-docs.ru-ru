...
1. Подготовка к работе. В первую очередь подготовьте VPN-подключение между локальной средой и Azure с помощью виртуальной сети Azure. (Поскольку здесь не используется Active Directory, требуется VPN-туннель). Дополнительные сведения см. в статье [Общие сведения о виртуальных сетях](../virtual-network/virtual-networks-overview.md). Затем с помощью портала Azure требуется подготовить новую виртуальную машину с использованием готового образа из библиотеки образов.
...
[SharePoint Server на виртуальных машинах Azure](http://msdn.microsoft.com/library/dn275955.aspx)
