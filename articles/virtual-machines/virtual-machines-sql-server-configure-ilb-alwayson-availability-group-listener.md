<properties 
	pageTitle="Настройка прослушивателя внутренней подсистемы балансировки нагрузки для группы доступности AlwaysOn в Azure"
	description="В этом учебнике показаны этапы создания прослушивателя группы доступности AlwaysOn в Azure с использованием внутренней подсистемы балансировки нагрузки (ILB)."
...
В группе доступности могут быть реплики, доступные только локально или только в Azure. В гибридных конфигурациях возможны оба способа доступа одновременно. Реплики в Azure могут находиться в одном или нескольких регионах (при использовании нескольких виртуальных сетей). Описанные ниже действия предполагают, что вы уже [настроили группу доступности](https://msdn.microsoft.com/library/azure/dn249504.aspx), но не настроили прослушиватель.

Обратите внимание, что к прослушивателю группы доступности в Azure с подсистемой внутренней балансировки нагрузки применяются следующие ограничения.
...
- Одна облачная служба поддерживает только один прослушиватель группы доступности, так как прослушиватель может быть настроен на использование либо виртуального IP-адреса облачной службы, либо виртуального IP-адреса внутренней подсистемы балансировки нагрузки. Обратите внимание, что это ограничение по-прежнему в силе, несмотря на то, что Azure теперь поддерживает создание нескольких виртуальных IP-адресов в одной облачной службе.

>[AZURE.NOTE]В этом учебнике рассматривается использование PowerShell при создании прослушивателя для группы доступности, включающей реплики в Azure. Дополнительные сведения о настройке прослушивателей с помощью SSMS или Transact-SQL см. в статье [Создание или настройка прослушивателя группы доступности](https://msdn.microsoft.com/library/hh213080.aspx).

## Определение доступности прослушивателя

[AZURE.INCLUDE [ag-listener-accessibility](../../includes/virtual-machines-ag-listener-determine-accessibility.md)]

В этой статье рассматривается создание прослушивателя, использующего **внутреннюю подсистему балансировки нагрузки (ILB)**. Если вам требуется публичный/внешний прослушиватель, см. другую версию этой статьи, где приведены пошаговые инструкции по настройке [внешнего прослушивателя](virtual-machines-sql-server-configure-public-alwayson-availability-group-listener.md).
...
1. Для **подсистемы балансировки нагрузки** вам потребуется назначить статический IP-адрес. Во-первых, изучите текущую конфигурацию виртуальной сети, выполнив следующую команду:

		(Get-AzureVNetConfig).XMLConfiguration

1. Обратите внимание на имя **Subnet** для подсети, содержащей виртуальные машины с репликами. Его значение будет присвоено параметру **$SubnetName** в приведенном ниже сценарии.

1. Также обратите внимание на имя **VirtualNetworkSite** и значение **AddressPrefix**, которое указывает начальный префикс подсети, содержащей виртуальные машины с репликами. Определите доступные IP-адреса. Для этого передайте оба значения в команду **Test-AzureStaticVNetIP** и просмотрите содержимое **AvailableAddresses**. Например, если виртуальная сеть называется *MyVNet*, а диапазон адресов подсети начинается с *172.16.0.128*, следующая команда выведет список доступных адресов:
...
>[AZURE.NOTE]В данный момент портал управления Azure не поддерживает внутреннюю балансировку нагрузки, поэтому система внутренней балансировки нагрузки и конечные точки не будут видны пользователю портала. Тем не менее команда **Get-AzureEndpoint** вернет внутренний IP-адрес, если на портале запущена балансировка нагрузки. В противном случае будет возвращено значение null.
