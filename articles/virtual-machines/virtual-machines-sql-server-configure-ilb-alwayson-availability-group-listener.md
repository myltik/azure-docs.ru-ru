...
>[AZURE.NOTE]В данный момент портал Azure не поддерживает внутреннюю балансировку нагрузки, поэтому система внутренней балансировки нагрузки и конечные точки не будут видны пользователю портала Azure. Но команда **Get-AzureEndpoint** вернет внутренний IP-адрес, если на портале запущен балансировщик нагрузки. В противном случае будет возвращено значение null.
