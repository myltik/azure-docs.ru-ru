...
Дополнительные ресурсы перечислены в разделе [Материалы][planning-guide-1.2] этого документа.
...
### <a name="a-namee55d1e22c2c8460b989764622a34fdffaresources"></a><a name="e55d1e22-c2c8-460b-9897-64622a34fdff"></a>Ресурсы
...
> [AZURE.IMPORTANT] Когда возможно, используется ссылка на руководство по установке SAP (Reference InstGuide-01, см. <http://service.sap.com/instguides>). Что касается необходимых компонентов и процесса установки, внимательно изучите руководства по установке SAP NetWeaver. В документе, который вы сейчас читаете, рассматриваются только отдельные задачи для систем SAP NetWeaver, установленных на виртуальной машине Microsoft Azure.
...
### <a name="a-name1625df664cc64d609202de8a0b77f803acloudonly-virtual-machine-deployments-into-azure-without-dependencies-on-the-onpremises-customer-network"></a><a name="1625df66-4cc6-4d60-9202-de8a0b77f803"></a>Только облако. Развертывание виртуальных машин в Azure без зависимостей в локальной сети клиента
...
### <a name="a-namef5b3b18c302c4bd89ab2c388f1ab3d10acrosspremise-deployment-of-single-or-multiple-sap-vms-into-azure-with-the-requirement-of-being-fully-integrated-into-the-onpremises-network"></a><a name="f5b3b18c-302c-4bd8-9ab2-c388f1ab3d10"></a>Распределенная сеть. Развертывание одной или нескольких виртуальных машин SAP в Azure с необходимостью полной интеграции с локальной сетью
...
> 
> * Выполнение различных уровней приложений SAP с помощью различных методов развертывания. Например:  выполнение уровня СУБД локально, а уровня приложений SAP в виртуальных машинах, развернутых в Azure, и наоборот.
...
* Типы виртуальных машин серии DS: используют те же узлы, что и виртуальные машины серии D, но могут подключаться к службе хранилища Azure класса Premium (см. раздел [Служба хранилища Azure класса Premium] [planning-guide-3.3.2] в этом документе). Также не все типы сертифицированы для использования с SAP.
* Типы виртуальных машин серии G: это типы виртуальных машин с большим объемом памяти. 
* Типы виртуальных машин серии GS: аналогичны серии G, но могут подключаться к службе хранилища Azure класса Premium (см. раздел [Служба хранилища Azure класса Premium] [planning-guide-3.3.2] в этом документе). Если виртуальные машины серии GS используются в качестве серверов баз данных, хранилище класса Premium является обязательным для хранения данных и файлов журналов транзакций баз данных.
...
### <a name="a-namebe80d1b9a4634845bd35f4cebdb5424aaazure-regions"></a><a name="be80d1b9-a463-4845-bd35-f4cebdb5424a"></a>Регионы Azure
...
### <a name="a-name8d8ad4b860934b91ac36ea56d80dbf77athe-microsoft-azure-virtual-machine-concept"></a><a name="8d8ad4b8-6093-4b91-ac36-ea56d80dbf77"></a>Концепция виртуальных машин Microsoft Azure
...
#### <a name="a-namedf49dc09141b4f34a4a2990913b30358afault-domains"></a><a name="df49dc09-141b-4f34-a4a2-990913b30358"></a>Домены сбоя
...
#### <a name="a-name18810088f9be4c97958a27996255c665aazure-availability-sets"></a><a name="18810088-f9be-4c97-958a-27996255c665"></a>Группы доступности Azure
...
### <a name="a-namea72afa264bf44a258cf7855d6032157fastorage-microsoft-azure-storage-and-data-disks"></a><a name="a72afa26-4bf4-4a25-8cf7-855d6032157f"></a>Хранилище. Служба хранилища Microsoft Azure и диски данных
...
#### <a name="a-nameff5ad0f9f7f440229102af07aef3bc92aazure-premium-storage"></a><a name="ff5ad0f9-f7f4-4022-9102-af07aef3bc92"></a>Хранилище Azure уровня "Премиум"
...
В отличие от хранилища класса Standard, возможности которого не зависят от размера диска (или виртуального жесткого диска), хранилище класса Premium в настоящее время включает в себя три категории дисков. Их список приводится в конце данной статьи (перед разделом вопросов и ответов): <https://azure.microsoft.com/pricing/details/storage/>.
...
### <a name="a-name616783878868435d9f8c450b2424f5bdamicrosoft-azure-networking"></a><a name="61678387-8868-435d-9f8c-450b2424f5bd"></a>Сеть Microsoft Azure
...
Дополнительные сведения см. по адресу: <https://azure.microsoft.com/documentation/services/virtual-network/>.
...
Так как сеть и разрешение имен являются важной частью развертывания базы данных для системы SAP, эта концепция описывается подробно в [Руководстве по развертыванию СУБД][dbms-guide].
...
* [Развертывание виртуальных машин с несколькими сетевыми адаптерами с помощью PowerShell][virtual-network-deploy-multinic-arm-ps]
...
#### <a name="sitetosite-connectivity"></a>Подключение типа "сеть — сеть"
...
#### <a name="pointtosite-vpn"></a>VPN-подключение типа "точка — сеть"
...
#### <a name="multisite-vpn"></a>Многосайтовая VPN
...
#### <a name="forced-tunneling-in-case-of-crosspremise"></a>Принудительное туннелирование для распределенного развертывания
...
Процессы подготовки локального компьютера или ноутбука для использования командлетов Azure PowerShell и настройки их использования с подпиской Azure описаны в [этой статье][powershell-install-configure]. 
...
См. пример здесь: <http://blogs.technet.com/b/keithmayer/archive/2015/07/07/18-steps-for-end-to-end-iaas-provisioning-in-the-cloud-with-azure-resource-manager-arm-powershell-and-desired-state-configuration-dsc.aspx>.
...
Общий список команд PowerShell, связанных с Azure, см. по адресу: <https://msdn.microsoft.com/library/azure/dn708514.aspx>. 
...
#### <a name="a-name4d175f1b735341379d2f817683c26e53amoving-a-vm-from-onpremises-to-azure-with-a-nongeneralized-disk"></a><a name="4d175f1b-7353-4137-9d2f-817683c26e53"></a>Перемещение виртуальной машины из локальной среды в Azure с помощью специализированного диска
...
#### <a name="a-namee18f7839c0e24385b1e64538453a285cadeploying-a-vm-with-a-customer-specific-image"></a><a name="e18f7839-c0e2-4385-b1e6-4538453a285c"></a>Развертывание виртуальной машины с помощью пользовательского образа
...
### <a name="a-name6ffb9f41a29240bf9e708204448559e7apreparing-vms-with-sap-for-azure"></a><a name="6ffb9f41-a292-40bf-9e70-8204448559e7"></a>Подготовка виртуальных машин с SAP для Azure
...
#### <a name="a-name1b287330944b495d9ea794b83aff73efapreparation-for-moving-a-vm-from-onpremises-to-azure-with-a-nongeneralized-disk"></a><a name="1b287330-944b-495d-9ea7-94b83aff73ef"></a>Подготовка виртуальной машины к перемещению из локальной среды в Azure с помощью специализированного диска
...
#### <a name="a-name57f32b1c0cba4e57ab6ec39fe22b6ec3apreparation-for-deploying-a-vm-with-a-customer-specific-image-for-sap"></a><a name="57f32b1c-0cba-4e57-ab6e-c39fe22b6ec3"></a>Подготовка виртуальной машины к развертыванию с помощью пользовательского образа для SAP
Файлы на виртуальном жестком диске, содержащие обобщенную ОС, также хранятся в контейнерах учетных записей хранения Azure. Вы можете развернуть из такого виртуального жесткого диска с образом новую виртуальную машину. Для этого укажите виртуальный жесткий диск как исходный диск в файлах шаблона развертывания. Этот процесс описан в разделе [Сценарий 2. Развертывание виртуальной машины для SAP с помощью пользовательского образа][planning-guide-3.3] [Руководства по развертыванию][planning-guide]. 
...
> ![ Linux][Logo_Linux]  Linux
>
> [Как записать образ виртуальной машины Linux для использования в качестве шаблона Resource Manager][virtual-machines-linux-capture-image-resource-manager]

___

### <a name="transferring-vms-and-vhds-between-onpremises-to-azure"></a>Передача виртуальных машин и виртуальных жестких дисков из локальной среды в Azure
...
Есть и третий вариант — вы можете использовать различные инструменты сторонних производителей с графическим интерфейсом. Но перед этим убедитесь, что эти средства поддерживают страничные BLOB-объекты Azure. Для наших целей необходимо использовать хранилище BLOB-объектов Azure (различия описаны здесь: <https://msdn.microsoft.com/library/windowsazure/ee691964.aspx>). Средства, которые предоставляет Azure, очень эффективно выполняют сжатие отправляемых виртуальных машин и виртуальных жестких дисков. Это важно, поскольку эффективное сжатие сокращает время отправки (которое зависит также от скорости передачи интернет-подключения локальной инфраструктуры и целевого региона развертывания Azure). Можно уверенно предположить, что отправка виртуальной машины или виртуального жесткого диска из любой точки Европы в центр обработки данных Azure, расположенный в США, займет больше времени, чем отправка той же виртуальной машины или виртуального жесткого диска в европейский центр обработки данных Azure. 

#### <a name="a-namea43e40e61acc463398168f095d5a7b6aauploading-a-vhd-from-onpremises-to-azure"></a><a name="a43e40e6-1acc-4633-9816-8f095d5a7b6a"></a>Передача VHD из локальной среды в Azure
Чтобы передать существующую виртуальную машину или виртуальный жесткий диск из локальной сети, они должны отвечать требованиям, перечисленным в разделе [Подготовка виртуальной машины к перемещению из локальной среды в Azure с помощью специализированного диска][planning-guide-5.2.1] данного документа.
...
* Укажите подписку контекста с помощью командлета _Set-AzureRmContext_ с параметром SubscriptionId или SubscriptionName. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt619263.aspx>
* Загрузите виртуальный жесткий диск с помощью командлета _Add-AzureRmVhd_ в учетную запись хранения. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603554.aspx>
* В конфигурации новой виртуальной машины укажите этот виртуальный жесткий диск в качестве диска операционной системы с помощью командлета _Set-AzureRmVMOSDisk_. Дополнительные сведения см. по адресу <https://msdn.microsoft.com/library/mt603746.aspx>.
* Создайте новую виртуальную машину из конфигурации виртуальной машины с помощью командлета _New-AzureRmVM_. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603754.aspx>
* Добавьте диск данных к новой виртуальной машине с помощью командлета _Add-AzureRmVMDataDisk_. Дополнительные сведения см. по адресу <https://msdn.microsoft.com/library/mt603673.aspx>.
...
* Войдите в свою подписку с помощью командлета _azure login_
* Выберите подписку с помощью команды _azure account set `<subscription name or id`>_
* Передайте виртуальный жесткий диск с помощью команды _azure storage blob upload_. Дополнительные сведения см. в статье [Использование интерфейса командной строки (CLI) Azure со службой хранилища Azure][storage-azure-cli]
...
* Укажите подписку контекста с помощью командлета _Set-AzureRmContext_ с параметром SubscriptionId или SubscriptionName. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt619263.aspx>
* Загрузите виртуальный жесткий диск с помощью командлета _Add-AzureRmVhd_ в учетную запись хранения. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603554.aspx>
* В конфигурации новой виртуальной машины укажите этот виртуальный жесткий диск в качестве диска операционной системы с помощью командлета _Set-AzureRmVMOSDisk -SourceImageUri -CreateOption fromImage_. Дополнительные сведения см. по адресу <https://msdn.microsoft.com/library/mt603746.aspx>.
* Создайте новую виртуальную машину из конфигурации виртуальной машины с помощью командлета _New-AzureRmVM_. Дополнительные сведения см. по адресу: <https://msdn.microsoft.com/library/mt603754.aspx>
...
#### <a name="downloading-vhds-to-onpremises"></a>Загрузка виртуального жесткого диска в локальную среду
...
Дополнительные сведения о командлете Save-AzureRmVhd см. по адресу <https://msdn.microsoft.com/library/mt622705.aspx>. 
...
#### <a name="a-name9789b07620114afab2feb07a8aba58a1acopying-disks-between-azure-storage-accounts"></a><a name="9789b076-2011-4afa-b2fe-b07a8aba58a1"></a>Копирование дисков между учетными записями хранения Azure
...
* Создайте контекст учетной записи хранения для исходной учетной записи хранения с помощью командлета _New-AzureStorageContext_. Дополнительный сведения см. по адресу: <https://msdn.microsoft.com/library/dn806380.aspx>.
* Создайте контекст учетной записи хранения для целевой учетной записи хранения с помощью командлета _New-AzureStorageContext_. Дополнительный сведения см. по адресу: <https://msdn.microsoft.com/library/dn806380.aspx>.
...
#### <a name="a-name4efec40191e040c08e64f2dceadff646avmvhd-structure-for-sap-deployments"></a><a name="4efec401-91e0-40c0-8e64-f2dceadff646"></a>Структура виртуальной машины и VHD для развертываний SAP
...
Дополнительные рекомендации и сведения, в частности относящиеся к виртуальным машинам для СУБД, приводятся в [Руководстве по развертыванию СУБД][dbms-guide].
...
#### <a name="a-name17e0d5437e8c4160a7dadd7117a1ad9dasetting-automount-for-attached-disks"></a><a name="17e0d543-7e8c-4160-a7da-dd7117a1ad9d"></a>Настройка автоподключения для подключенных дисков
...
#### <a name="configuration-of-the-sap-system-and-sap-gui-connectivity-for-cloudonly-scenario"></a>Настройка подключения к системе SAP и графическому пользовательскому интерфейсу SAP в сценарии полностью облачного развертывания.
...
## <a name="a-name96a77628a05e475d9df3fb82217e8f14aconcepts-of-cloudonly-deployment-of-sap-instances"></a><a name="96a77628-a05e-475d-9df3-fb82217e8f14"></a>Концепция полностью облачного развертывания экземпляров SAP

### <a name="a-name3e9c3690da67421abc3f12c520d99a30asingle-vm-with-sap-netweaver-demotraining-scenario"></a><a name="3e9c3690-da67-421a-bc3f-12c520d99a30"></a>Обучающий сценарий с демонстрацией одной виртуальной машины с SAP NetWeaver
...
#### <a name="setup-network-for-communication-between-the-different-vms"></a>Настройка сети для взаимодействия между разными виртуальными машинами
...
## <a name="deploying-sap-vms-with-corporate-network-connectivity-crosspremises"></a>Развертывание виртуальных машин SAP с подключением к корпоративной сети (распределенное развертывание)
...
Дополнительные сведения о том, какие порты нужны для различных продуктов и служб SAP, сгруппированные по продуктам SAP, см. по адресу <http://scn.sap.com/docs/DOC-17124>. С помощью этого документа вы сможете открыть на VPN-устройстве конкретные порты, необходимые для определенных продуктов и сценариев SAP.
...
##### <a name="printing-over-tcpip-in-crosspremises-scenario"></a>Печать по протоколу TCP/IP в распределенном сценарии
...
##### <a name="hostbased-printer-over-smb-shared-printer-in-crosspremises-scenario"></a>Доступ по протоколу SMB к принтеру на основе узла (общему принтеру) в распределенном сценарии
...
> ![ Windows][Logo_Windows] Windows
>
> Дополнительные сведения о печати в Windows можно найти по адресу: <http://technet.microsoft.com/library/jj590748.aspx>.

___

 
#### <a name="integration-of-sap-azure-systems-into-correction-and-transport-system-tms-in-crosspremises"></a>Интеграция систем SAP Azure в систему исправлений и транспортировки (TMS) при распределенном развертывании
...
#### <a name="rfc-traffic-between-sap-instances-located-in-azure-and-onpremises-crosspremises"></a>Трафик RFC между экземплярами SAP в Azure и в локальной среде (распределенное развертывание)
...
### <a name="a-name6f0a47f3a2894090a0532521618a28c3aazure-monitoring-solution-for-sap"></a><a name="6f0a47f3-a289-4090-a053-2521618a28c3"></a>Решение мониторинга Azure для SAP
...
Исходный универсальный код ресурса (URI) портала выглядит так: http(s):`<Portalserver`>: 5XX00/irj, где номер порта вычисляется как 50000 + (номер системы × 100). По умолчанию для системы SAP с номером 00 универсальный код ресурса (URI) будет выглядеть так: `<dns name`>.`<azure region`>.Cloudapp.azure.com:PublicPort/irj. Дополнительные сведения см. по адресу <http://help.sap.com/saphelp_nw70ehp1/helpdata/de/a2/f9d7fed2adc340ab462ae159d19509/frameset.htm>. 
...
Сведения о том, как установить отказоустойчивый кластер Windows с SIOS DataKeeper и SAP, см. в техническом документе [Clustering SAP ASCS/SCS Instance using Windows Server Failover Cluster on Azure with SIOS DataKeeper] (Кластеризация экземпляра SAP ASCS или SCS с использованием отказоустойчивого кластера Windows Server в Azure с помощью SIOS DataKeeper)[ha-guide-classic]. 
...
#### <a name="endtoend-high-availability-for-the-complete-sap-system"></a>Комплексная высокая доступность полной системы SAP
...
### <a name="a-name4e165b5874ca474fa7f45e695a93204fausing-autostart-for-sap-instances"></a><a name="4e165b58-74ca-474f-a7f4-5e695a93204f"></a>Использование автозапуска для экземпляров SAP
...
### <a name="larger-3tier-sap-systems"></a>Более крупные трехуровневые системы SAP
...
#### <a name="location-of-3tier-sap-configurations"></a>Расположение трехуровневых конфигураций SAP
...
Восстановление предыдущего состояния заключается в удалении базовой виртуальной машины, ее исходных дисков и подключенных VHD-дисков с последующим копированием сохраненных VHD-дисков в исходную учетную запись хранения и повторным развертыванием системы.
...
>
> Ознакомиться с резервным копированием виртуальных машин Azure можно здесь: <https://azure.microsoft.com/documentation/articles/backup-azure-vms/>.
>
> Кроме того, для резервного копирования и восстановления баз данных вы можете использовать комбинацию Microsoft Data Protection Manager на виртуальной машине Azure и службы архивации Azure. Дополнительные сведения см. по адресу: <https://azure.microsoft.com/documentation/articles/backup-azure-dpm-introduction/>.  
