...
1. **Создание головного узла кластера и виртуальных машин вычислительных узлов путем выполнения скрипта развертывания IaaS с пакетом HPC на клиентском компьютере.**
    ...
    * **Операционная система Windows Server**. Для поддержки соединения RDMA укажите операционную систему Windows Server 2012 R2 или Windows Server 2012 для виртуальных машин размера A8 или A9.
    ...
    * **Размер головного узла** — для этого сценария в качестве головного узла рекомендуется по крайней мере размер A4 (очень крупный).
...
* В качестве альтернативы пакету HPC для выполнения приложений MPI в управляемых пулах вычислительных узлов в Azure можно использовать пакетную службу Azure. См. раздел [Использование задач с несколькими экземплярами для запуска приложений с интерфейсом передачи сообщений (MPI) в пакетной службе Azure](../batch/batch-mpi.md).
