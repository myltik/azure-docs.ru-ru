...
Настройте кластер Windows RDMA в Azure с помощью [пакета Microsoft HPC](https://technet.microsoft.com/library/cc514029) и [экземпляров для ресурсоемких вычислений размеров A8 и A9](virtual-machines-windows-a8-a9-a10-a11-specs.md) для запуска параллельных приложений MPI. Если настроить экземпляры размеров A8 и A9 под управлением Windows Server для выполнения в кластере пакета HPC, приложения MPI будут эффективно взаимодействовать по сети с низкой задержкой и высокой пропускной способностью в Azure на основе технологии удаленного доступа к памяти (RDMA).
...
Пакет Microsoft HPC — это бесплатное средство для создания кластеров HPC на базе Windows Server в Azure. В пакет HPC входит среда выполнения для реализации интерфейса передачи сообщений для Windows (MS-MPI). При использовании с экземплярами A8 и A9 пакет HPC обеспечивает эффективный способ запуска приложений MPI на базе Windows, которые получают доступ к сети RDMA в Azure.

В этой статье представлены два сценария и ссылки на подробные руководства по развертыванию кластеризованных экземпляров A8 и A9 с помощью пакета Microsoft HPC.
...
4. **Развертывание и настройка головного узла HPC Pack 2012 R2.**
    ...
    Загрузите новейшую версию пакета установки HPC Pack из [Центра загрузки Майкрософт](https://www.microsoft.com/download/details.aspx?id=49922). Требования и инструкции для подготовки к развертыванию ускорения в Azure см. в разделах [Getting Started Guide for Microsoft HPC Pack 2012 R2 and HPC Pack 2012](https://technet.microsoft.com/library/jj884144.aspx) (Руководство по началу работы с пакетами Microsoft HPC 2012 R2 и HPC 2012) и [Burst to Azure with Microsoft HPC Pack](https://technet.microsoft.com/library/gg481749.aspx) (Ускорение в Azure с помощью пакета Microsoft HPC).
...
1. **Создание головного узла кластера и виртуальных машин вычислительных узлов путем выполнения скрипта развертывания IaaS с пакетом HPC на клиентском компьютере.**
    ...
    При развертывании вычислительных узлов размера A8 и A9 обратите внимание на следующие дополнительные рекомендации.
    
    * **Виртуальная сеть** — укажите новую виртуальную сеть в регионе, в котором доступны экземпляры A8 и A9.

    * **Операционная система Windows Server**. Чтобы обеспечить поддержку подключений RDMA, укажите операционную систему Windows Server 2012 R2 или Windows Server 2012 для виртуальных машин размера A8 или A9.
    ...
    * **Размер головного узла** — для этого сценария для головного узла рекомендуется по крайней мере размер A4 (очень крупный).
...
* Чтобы запустить приложение MPI в экземплярах Azure, следует зарегистрировать приложение MPI в брандмауэре Windows в экземплярах, запустив команду **hpcfwutil**. Это позволит осуществлять обмен данными MPI через порт, который динамически назначается брандмауэром.
    ...
    >[AZURE.NOTE] Для развертываний ускорения в Azure можно также настроить команду исключения брандмауэра с целью автоматического запуска на всех новых узлах Azure, которые добавляются к кластеру. Запустив команду **hpcfwutil** и убедившись в том, что ваше приложение работает, добавьте команду в скрипт запуска для узлов Azure. Дополнительные сведения см. в статье [Use a Startup Script for Azure Nodes](https://technet.microsoft.com/library/jj899632.aspx) (Использование сценария запуска для узлов Azure).
