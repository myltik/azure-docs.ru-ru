...
На этом этапе развертывания высокодоступного бизнес-приложения в интрасети на базе служб инфраструктуры Azure настраиваются два компьютера: SQL Server и узел большинства в кластере. Затем эти компьютеры объединяются в кластер Windows Server.

Этот этап необходимо выполнить, прежде чем переходить к [этапу 4](virtual-machines-workload-high-availability-LOB-application-phase4.md). Все этапы перечислены в статье [Развертывание высокодоступных бизнес-приложений в Azure](virtual-machines-workload-high-availability-LOB-application-overview.md).

> [AZURE.NOTE]В этой статье используется образ SQL Server из коллекции образов Azure. За использование лицензии SQL Server взимаются соответствующие сборы. Также на виртуальные машины, установленные в Azure, можно установить свои лицензии SQL Server, но описание этой процедуры в этой статье не приводится.
...
Используя предпочитаемый клиент удаленного рабочего стола, создайте подключение к удаленному рабочему столу виртуальной машины, относящейся к первому контроллеру домена. Выполните это действие на каждой виртуальной машине с сервером SQL Server. Для подключения используйте DNS интрасети контроллера домена или имя компьютера, а также учетные данные локальной учетной записи администратора.
...
9.	В поле **Имя входа** введите *домен*\\sqladmin, заменив *домен* именем домена, в котором на [этапе 2](virtual-machines-workload-high-availability-LOB-application-phase2.md) была создана учетная запись sqladmin. 
...
1.	Войдите на основную виртуальную машину с сервером SQL Server, используя учетную запись sqladmin, созданную на [этапе 2](virtual-machines-workload-high-availability-LOB-application-phase2.md).
...
5.	На странице **Выбор серверов** введите имя основной виртуальной машины с сервером SQL Server, нажмите кнопку **Добавить**, а затем — **Далее**.
6.	На странице **Предупреждение проверки** выберите вариант **Нет. Для этого кластера не требуется поддержка корпорации Майкрософт, поэтому нет необходимости выполнять проверочные тесты. После нажатия кнопки «Далее» продолжить создание кластера**, а затем нажмите кнопку **Далее**.
...
17.	Чтобы добавить в кластер остальные узлы, в области слева щелкните правой кнопкой мыши имя кластера и выберите команду **Добавить узел**.
18.	На странице **Перед началом работы** нажмите кнопку **Далее**. 
19.	На странице **Выбор серверов** добавьте в кластер дополнительный SQL Server и узел большинства кластера (для каждого из них введите имя и нажмите кнопку **Добавить**). Добавив два компьютера, нажмите кнопку **Далее**. Если добавить машину не удается и появляется сообщение об ошибке «Служба удаленного реестра не запущена», выполните указанные ниже действия. Войдите на машину, откройте оснастку «Службы» (файл services.msc) и включите удаленный реестр. Дополнительные сведения см. в статье [Не удается подключиться к службе удаленного реестра](http://technet.microsoft.com/library/bb266998.aspx). 
20.	На странице **Предупреждение проверки** выберите вариант **Нет. Для этого кластера не требуется поддержка корпорации Майкрософт, поэтому нет необходимости выполнять проверочные тесты. После нажатия кнопки «Далее» продолжить создание кластера**, а затем нажмите кнопку **Далее**. 
...
Дальнейшие действия по настройке рабочей нагрузки см. в статье [Этап 4. Настройка веб-серверов](virtual-machines-workload-high-availability-LOB-application-phase4.md).
