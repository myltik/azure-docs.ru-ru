...
8. **Только CentOS 6.3**. Установите драйверы для служб интеграции Linux.
	...
	**Важно. Это действие действительно только для CentOS версии 6.3 и более ранних версий.** В CentOS 6.4 + службы интеграции Linux *уже доступны в ядре*.
	...
	a) Загрузите ISO-файл, содержащий драйверы для служб интеграции Linux, из [Центра загрузки Майкрософт](http://www.microsoft.com/download/details.aspx?id=41554).

	b) На панели **Действия** диспетчера Hyper-V щелкните **Параметры**.

	![Откройте параметры Hyper-V](./media/virtual-machines-linux-create-upload-vhd-centos/settings.png)

	c) На панели **Оборудование** щелкните пункт **Контроллер IDE 1**.

	![Добавьте DVD-привод с установочным носителем](./media/virtual-machines-linux-create-upload-vhd-centos/installiso.png)
