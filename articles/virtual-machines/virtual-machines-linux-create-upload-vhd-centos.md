...
3. Удалите NetworkManager, выполнив следующую команду:
	...
	**Примечание.** Если пакет еще не установлен, эта команда завершится с сообщением об ошибке. Это ожидаемый результат.
...
8. **Только для CentOS 6.3**: Установите драйверы для служб интеграции Linux
	...
	**Важно! Это действие действительно только для CentOS 6.3 и более ранних версий.**  В CentOS 6.4+ службы интеграции Linux *already available in the kernel*.
	...
	c) На панели **Оборудование** щелкните **Контроллер IDE 1**.

	![Add DVD drive with install media](./media/virtual-machines-linux-create-upload-vhd-centos/installiso.png)

	d) В поле **Контроллер IDE** щелкните **DVD-привод**, а затем выберите **Добавить**.

	e) Выберите **Файл образа**, перейдите к **Linux IC v3.2.iso** и щелкните **Открыть**.
...
10. Если вы хотите использовать зеркала OpenLogic, размещенные в центрах обработки данных Azure, замените файл /etc/yum.repos.d/CentOS-Base.repo следующими репозиториями.  При этом также будет добавлен репозиторий **[openlogic]**, включающий в себя пакеты для агента Linux для Azure:
...
8. Если вы хотите использовать зеркала OpenLogic, размещенные в центрах обработки данных Azure, замените файл /etc/yum.repos.d/CentOS-Base.repo следующими репозиториями.  При этом также будет добавлен репозиторий **[openlogic]**, включающий в себя пакеты для агента Linux для Azure:
