...
>[AZURE.NOTE]Сейчас Azure Windows RDMA поддерживается приложениями MPI, использующими интерфейс Microsoft Network Direct для взаимодействия между экземплярами A8 и A9.
>
> Кроме того, Azure дает возможность пользоваться экземплярами для ресурсоемких вычислений A10 и A11 с идентичными экземплярам A8 и A9 возможностями обработки, но без подключения к серверной сети RDMA. Для запуска рабочих нагрузок MPI в Azure наилучшей производительности, как правило, можно добиться с экземплярами A8 и A9.

Если требуется применить рабочие нагрузки MPI на виртуальных машинах под управлением Linux, получающих доступ к сети RDMA в Azure, см. раздел [Настройка кластера Linux RDMA для запуска приложений MPI](virtual-machines-linux-cluster-rdma.md).

## Варианты развертывания кластера Windows HPC
Пакет Microsoft HPC является рекомендованным инструментом для создания кластеров HPC на базе Windows Server в Azure. При использовании с экземплярами A8 и A9 пакет HPC обеспечивает эффективный способ запуска приложений MPI на базе Windows, которые получают доступ к сети RDMA в Azure. В пакет HPC входит среда времени выполнения для реализации Майкрософт интерфейса MPI для Windows.
...
* **Ознакомьтесь[ с основной информацией и рекомендациями](virtual-machines-a8-a9-a10-a11-specs.md)** по экземплярам для ресурсоемких вычислений.
...
Ниже описаны шаги для ускорения до экземпляров Azure A8 или A9 из существующего (обычно локального) кластера. Аналогичные процедуры позволяют добавлять экземпляры рабочих ролей в головной узел HPC Pack, который развертывается на виртуальной машине Azure.
...
5. **Настройка сертификата управления в подписке Azure**.
    ...
    Настройте сертификат для обеспечения безопасного соединения между головным узлом и Azure. Параметры и процедуры см. в разделе [Сценарии настройки сертификата управления Azure для пакета HPC](http://technet.microsoft.com/library/gg481759.aspx).

6. **Создание новой облачной службы и учетной записи хранения**.

    С помощью портала управления Azure создайте облачную службу и учетную запись хранения для развертывания в регионе, в котором доступны экземпляры с ресурсоемкими вычислениями. (Не связывайте облачную службу и учетную запись хранения с существующей группой сходства, используемой для других развертываний).
...
* **Прокси-узлы** — при каждом развертывании ускорения в Azure для экземпляров с ресурсоемкими вычислениями пакет HPC Pack автоматически разворачивает не менее 2 дополнительных экземпляров размера A8 в качестве прокси-узлов (в дополнение к указанным вами экземплярам рабочих ролей). Дополнительные сведения см. в разделе [Указание количества прокси-узлов Azure](https://technet.microsoft.com/library/jj899633.aspx). Прокси-узлы используют ядра, выделенные для подписки, и за них взимается плата наряду с экземплярами рабочих ролей Azure.

* **Виртуальная сеть** — пакет HPC Pack в настоящее время не поддерживает конфигурацию VPN «точка — сеть» для развертываний PaaS.


## Сценарий 2. Развертывание вычислительных узлов на виртуальных машинах с ресурсоемкими вычислениями (IaaS)

В этом сценарии развертываются головной узел HPC Pack и вычислительные узлы кластера на виртуальных машинах, присоединенных к домену Active Directory в виртуальной сети Azure. [Скрипт развертывания IaaS пакета HPC Pack](virtual-machines-hpcpack-cluster-powershell-script.md) автоматизирует большую часть этого процесса и обеспечивает гибкость параметров развертывания, включая возможность указать размер ВМ A8 или A9 для узлов кластера. Далее приведены сведения о том, как использовать этот метод автоматического развертывания. Другой вариант — развертывание кластера с помощью модели развертывания диспетчера ресурсов с использованием шаблона быстрого запуска Azure. Кроме того, для тестовых развертываний вы можете вручную развернуть домен Active Directory, ВМ головного узла, ВМ вычислительных узлов и другие части инфраструктуры кластера HPC Pack в Azure. См. статью [Параметры кластера HPC с пакетом Microsoft HPC в Azure](virtual-machines-hpcpack-cluster-options.md).
...
3. **Отправка заданий в кластер**.
    ...
    Подключитесь к головному узлу для отправки заданий или настройте для этого локальный компьютер. Сведения см. в разделе [Отправка заданий в кластер HPC в Azure](virtual-machines-hpcpack-cluster-submit-jobs.md).
...
### Дополнительные рекомендации по выполнению скрипта развертывания кластера
...
* **Расширение HpcVmDrivers** — скрипт развертывания автоматически устанавливает агент ВМ Azure и расширение HpcVmDrivers, когда вы развертываете вычислительные узлы размера A8 или A9, в которых используется ОС Windows Server. HpcVmDrivers устанавливает драйверы на виртуальные машины вычислительного узла, чтобы они могли подключаться к сети RDMA. См. [Расширения и компоненты виртуальных машин Azure](virtual-machines-extensions-features.md).
...
1. На головном узле или на соответствующим образом настроенном клиентском компьютере откройте окно команд.
...
* Чтобы запустить приложение MPI в экземплярах Azure, следует зарегистрировать приложение MPI в брандмауэре Windows в экземплярах, запустив команду **hpcfwutil**. Это позволит осуществлять обмен данными MPI через порт, который динамически назначается брандмауэром.
    ...
    >[AZURE.NOTE]Для развертываний ускорения в Azure можно также настроить команду исключения брандмауэра с целью автоматического запуска на всех новых узлах Azure, которые добавляются к кластеру. Запустив команду **hpcfwutil** и убедившись в том, что ваше приложение работает, добавьте команду в скрипт запуска для узлов Azure. Дополнительные сведения см. в разделе [Использование скрипта запуска для узлов Azure](https://technet.microsoft.com/library/jj899632(v=ws.10).aspx).
...
* Если требуется запускать приложения MPI для Linux, получающие доступ к сети RDMA в Azure, см. раздел [Настройка кластера Linux RDMA для запуска приложений MPI](virtual-machines-linux-cluster-rdma.md).
