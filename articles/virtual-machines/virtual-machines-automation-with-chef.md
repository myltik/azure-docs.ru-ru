...
**Сервер Chef Server** — это наша точка управления. Он доступен в двух вариантах: в виде размещенного и локального решений. Мы будем использовать размещенное решение.

**Клиент Chef (узел)** — это агент, который находится на управляемых серверах.

**Рабочая станция Chef** — это рабочая станция администрирования, на которой мы создаем политики и выполняем команды управления. Чтобы управлять нашей инфраструктурой, мы выполняем команду **knife** на рабочей станции Chef.
...
Прежде всего создайте каталог **C:\\chef**.
...
Загрузите параметры публикации отсюда: <a href="https://manage.windowsazure.com/publishsettings/" target="_blank">https://manage.windowsazure.com/publishsettings/</a>.

Сохраните файл параметров публикации в каталоге **C:\\chef**.

##Создание управляемой учетной записи Chef

Зарегистрируйте учетную запись размещенного Chef по адресу <a href="https://manage.chef.io/signup" target="_blank">https://manage.chef.io/signup</a>.
...
**Примечание.** Если появится предупреждение о том, что ваши ключи будут сброшены, вы можете спокойно продолжить процедуру, так как у вас пока нет никакой настроенной инфраструктуры.
...
PEM-файлы содержат закрытые ключи администратора и организации для обеспечения связи, а файл **knife.rb** — конфигурацию knife. Нам потребуется изменить файл **knife.rb**.

Откройте файл в предпочитаемом вами редакторе и измените cookbook\_path, удалив "/.../" из пути, чтобы он имел следующий вид:
...
**Примечание.** Благодаря аргументу –pre вы получите последнюю версию-кандидат подключаемого модуля knife для Azure, предоставляющего доступ к актуальному набору API.
...
**Примечание.** Мы автоматизируем работу правил сетевых фильтров конечной точки, используя параметр –tcp-endpoints в командной строке. Здесь открыты порты 80 и 3389, чтобы обеспечить доступ к веб-странице и сеансу протокола удаленного рабочего стола.
