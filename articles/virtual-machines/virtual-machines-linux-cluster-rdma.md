...
>[AZURE.NOTE] В настоящее время Azure Linux RDMA поддерживается библиотекой Intel MPI версии 5 для образа SUSE Linux Enterprise Server 12 (SLES 12) из Azure Marketplace. В этой статье используется Intel MPI версии 5.0.3.048.
>
...
* **Пакет HPC** — вы можете создать кластер пакета Microsoft HPC в Azure и добавить вычислительные узлы под управлением поддерживаемых дистрибутивов Linux. Некоторые узлы Linux можно настроить для доступа к сети RDMA. См. [Начало работы с вычислительными узлами Linux в кластере пакета HPC в Azure](virtual-machines-linux-cluster.md).
...
* **Шаблоны диспетчера ресурсов Azure** — вы можете развернуть несколько виртуальных машин A8 и A9 под управлением Linux, а также определить параметры виртуальных сетей, DNS, статические IP-адреса и другие ресурсы, чтобы создать вычислительный кластер, который может использовать все преимущества сети RDMA для выполнения рабочих нагрузок MPI, используя модель развертывания диспетчера ресурсов. Для развертывания вашего решения вы можете [создать собственный шаблон](../resource-group-authoring-templates.md) или изучить [страницу краткого руководства по шаблонам Azure](https://azure.microsoft.com/documentation/templates/), которая содержит шаблоны, созданные корпорацией Майкрософт и сообществом пользователей. Шаблоны диспетчера ресурсов — самый быстрый и надежный способ развертывания кластера Linux.

## Развертывание в режиме управления службами Azure с помощью скриптов Azure CLI

Далее показано, как использовать Azure CLI для развертывания виртуальных машин SUSE Linux Enterprise Server 12, установки библиотеки Intel MPI и других настроек, создания образа виртуальной машины и скрипта для развертывания кластера виртуальных машин A8 или A9.
...
Затем на клиентском компьютере выполните следующие команды Azure CLI для записи образа. Дополнительные сведения см. в статье [Запись классической виртуальной машины Linux в виде образа](virtual-machines-linux-capture-image.md).
...
## Обновление драйверов Linux RDMA для SLES 12

После создания кластера Linux RDMA на основе образа SLES 12 HPC может возникнуть необходимость в обновлении драйверов RDMA на виртуальных машинах для подключения к сети RDMA.

>[AZURE.IMPORTANT]В настоящее время этот шаг является **обязательным** для развертывания кластеров Linux RDMA в большинстве регионов Azure. **Обновления не требуют только виртуальные машины SLES 12, созданные в следующих регионах Azure: западная часть США, Западная Европа и восточная Япония**.

Прежде чем обновлять драйверы, остановите все процессы **zypper** или процессы, блокирующие базы данных репозиториев SUSE на виртуальной машине. В противном случае драйверы могут не обновиться должным образом.


Обновите драйверы Linux RDMA на каждой виртуальной Машине, выполнив на клиентском компьютере один из следующих наборов команд Azure CLI.

**Для виртуальной машины, подготовленной в управлении службами Azure**
...
**Для виртуальной машины, подготовленной в диспетчере ресурсов Azure**
...
Обновление драйверов на всех узлах можно прописать в сценарии. Например, следующий сценарий обновляет драйверы в кластере с 8 узлами, созданном с помощью сценария, который был описан в предыдущем шаге.
