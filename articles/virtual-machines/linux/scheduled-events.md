...
Запланированные события — это подслужба в службе метаданных Azure, которая дает время вашему приложению для подготовки к обслуживанию виртуальной машины. Эта служба предоставляет сведения о предстоящем событии обслуживания (например, о перезагрузке), чтобы приложение могло подготовиться к ним и ограничить перебои в работе. Она доступна для всех типов виртуальных машин Azure, включая PaaS и IaaS (как для Windows, так и для Linux). 
...
Для виртуальной машины с поддержкой виртуальной сети полная конечная точка для последней версии запланированных событий выглядит так: 
...
При первом создании запроса запланированных событий Azure неявно включает эту функцию на виртуальной машине. Как результат, при первом вызове ожидайте задержки с ответом длительностью до двух минут.

> [!NOTE]
> Запланированные события автоматически отключаются для службы, которая не вызывает конечную точку в течение одного дня. После того, как в службах будут отключены запланированные события, для обслуживания, инициированного пользователем, события создаваться не будут.
