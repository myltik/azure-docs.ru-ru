...
- [Обзор и архитектура](sap-hana-overview-architecture.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Инфраструктура и подключение](sap-hana-overview-infrastructure-connectivity.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Установка SAP HANA](sap-hana-overview-sap-hana-installation.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Высокий уровень доступности и аварийное восстановление](sap-hana-overview-high-availability-disaster-recovery.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
- [Устранение неполадок и мониторинг](sap-hana-overview-troubleshooting-monitoring.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
- [Использование решений SAP на виртуальных машинах Microsoft Azure](../virtual-machines-linux-sap-get-started.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json)
...
![Основные сведения об архитектуре решения "SAP HANA в Azure (крупные экземпляры)"](./media/sap-hana-overview-architecture/image1-architecture.png)
...
Оборудование SAP HANA в Azure (крупные экземпляры) могут использовать несколько клиентов, и каждый из них будет изолирован друг от друга, так как они выполняют развертывание в собственных виртуальных локальных сетях. Для подключения крупных экземпляров HANA к виртуальной сети Azure сетевые компоненты на месте выполняют преобразование сетевых адресов (NAT) между диапазоном IP-адресов виртуальной сети Azure и адресным пространством виртуальной локальной сети в рамках аппаратной инфраструктуры.
...
![Обязанности, связанные с SAP HANA в Azure (крупные экземпляры)](./media/sap-hana-overview-architecture/image2-responsibilities.png)
...
- SAP Note #1793345 - Sizing for SAP Suite on HANA (Примечание SAP № 1793345. Определение размера для SAP Suite в HANA);
- SAP Note #1872170 - Suite on HANA and S/4 HANA sizing report (Примечание SAP № 1872170. Отчет для определения размера Suite в HANA и S/4 HANA);
- SAP Note #2121330 - FAQ: SAP BW on HANA Sizing Report (Примечание SAP № 2121330. Часто задаваемые вопросы: отчет для определения размера SAP BW в HANA);
- SAP Note #1736976 - Sizing Report for BW on HANA (Примечание SAP № 1736976. Отчет для определения размера BW в HANA);
- SAP Note #2296290 - New Sizing Report for BW on HANA (Примечание SAP № 2296290. Новый отчет для определения размера BW в HANA).
...
- Поддержка Microsoft Premier для критически важных рабочих нагрузок. Сведения о работе с решением SAP в Azure см. в примечании [SAP Support Note #2015553 – SAP on Microsoft Azure: Support Prerequisites](https://launchpad.support.sap.com/#/notes/2015553) (Примечание SAP № 2015553. SAP в Microsoft Azure: требования для поддержки).
...
- Контракт на поддержку и обслуживание с поставщиком Linux, который косвенным образом включается в подписку на конкретную версию Linux или в другой контракт на поддержку и обслуживание, применяется только к определенной используемой версии Linux, которая удовлетворяет критериям SAP.
...
- Службы Active Directory и DNS, размещенные локально, можно использовать в Azure через ExpressRoute.

**Рекомендация.** Развертывайте весь ландшафт SAP в рамках одной подписки Azure. Для многих операций в пределах ландшафта SAP требуется прозрачное и, возможно, более медленное сетевое подключение между экземплярами разработки, тестирования и рабочими экземплярами SAP. В архитектуре SAP NetWeaver реализовано множество механизмов автоматизации, которые зависят от прозрачного сетевого подключения между этими экземплярами. Поэтому настоятельно рекомендуется развернуть весь ландшафт SAP в одной подписке Azure, даже если он развернут в нескольких регионах Azure.

Архитектура и процессы, связанные с SAP HANA в Azure (крупные экземпляры), основаны на этой рекомендации.
...
Развертывание SAP HANA в Azure (крупные экземпляры) в двух разных регионах Azure приведет к тому, что другой клиент будет развернут в стеке крупных экземпляров. Тем не менее оба клиента могут относиться к одной подписке Azure, если эти экземпляры являются частью одного ландшафта SAP.
...
![Виртуальная сеть Azure подключена к SAP HANA в Azure (крупные экземпляры) и к локальной сети](./media/sap-hana-overview-architecture/image3-on-premises-infrastructure.png)
...
![Развертывание уровня приложений SAP в нескольких виртуальных сетях Azure](./media/sap-hana-overview-architecture/image4-networking-architecture.png)
...
### <a name="minimal-deployment"></a>Минимальное развертывание

Для небольшой системы SAP (минимальное развертывание) на виртуальных машинах Azure размещается уровень приложений SAP в собственной среде Azure (в одной виртуальной сети) и осуществляется подключение к стеку крупных экземпляров через ExpressRoute. Чтобы подготовить SAP HANA в Azure (крупные экземпляры) к использованию, выполните следующие действия.

- Соберите конкретные сведения, связанные с четырьмя разными диапазонами IP-адресов.

  1. Диапазон адресов /29 для подключений P2P, которые будут использоваться для каналов ExpressRoute.
  
  2. Уникальный блок CIDR с диапазоном /24 (рекомендуется) для назначения определенных IP-адресов, необходимых для SAP HANA в Azure (крупные экземпляры).
  3. Один или несколько блоков CIDR с диапазоном /24 (рекомендуется) для подсетей клиентов виртуальной сети Azure. Это подсети в клиентской подписке Azure, где будут размещены виртуальные машины Azure, связанные с SAP. Для этих адресов разрешен доступ к SAP HANA в Azure (крупные экземпляры). Для каждой подсети требуется один блок клиентских адресов, а блоки могут быть объединены, если они являются смежными и расположены в одной виртуальной сети.
  4. Один диапазон адресов /28 подсетей шлюза виртуальной сети (если требуется подключение типа "точка — сеть", необходимо использовать диапазон /27).

  - Необходимы первые два диапазона (по одному на подписку и регион Azure). Диапазоны IP-адресов, указанные в пунктах 3 и 4, соответствуют минимальным требованиям для каждой виртуальной сети Azure. Если в виртуальной сети требуется несколько подсетей или клиентов, для пункта 3 нужно указать несколько диапазонов.
![Диапазоны IP-адресов, необходимые для минимального развертывания SAP HANA в Azure (крупные экземпляры)](./media/sap-hana-overview-architecture/image5-ip-address-range-a.png)

  -  При настройке нескольких подсетей клиентов в виртуальной сети Azure: ![Диапазоны IP-адресов с непрерывным адресным пространством для виртуальной сети Azure](./media/sap-hana-overview-architecture/image6-ip-address-range-b.png)

> [!IMPORTANT] 
> Каждый диапазон IP-адресов, указанный выше, не должен перекрываться другим диапазоном. Кроме того, они должны быть дискретными и не соответствовать подсети с другим диапазоном адресов. Для виртуальных сетей Azure следует применять только адреса, указанные в пунктах 3 и 4. Все остальные используются для подключения и маршрутизации крупных экземпляров. Также рекомендуется, чтобы диапазоны адресов адресного пространства соответствовали диапазонам подсети и не содержали пустое или неназначенное пространство. Если между диапазонами, определенными в пунктах 1 и 2, а также 3 и 4, возникают перекрытия, виртуальная сеть Azure не будет подключена к каналу ExpressRoute.

- Корпорация Майкрософт создает канал ExpressRoute между подпиской Azure и стеком крупных экземпляров.
- Создайте клиент сети в стеке крупных экземпляров.
- Настройте сеть таким образом, чтобы инфраструктура SAP HANA в Azure (крупные экземпляры) принимала IP-адреса из диапазона, указанного в виртуальной сети Azure, которая будет взаимодействовать с крупными экземплярами HANA.
- Настройте преобразование сетевых адресов (NAT) в клиенте стека крупных экземпляров (чтобы внутренний IP-адрес клиента был сопоставлен с IP-адресом, определенным клиентом для Azure).
- В зависимости от конкретного приобретенного SKU SAP HANA в Azure (крупные экземпляры) назначьте единицу вычисления в сети клиента, выделите и подключите хранилище и установите операционную систему (SUSE или RedHat Linux).

Минимальное развертывание сетевой архитектуры SAP HANA в Azure (крупные экземпляры):

![Минимальное развертывание с диапазонами IP-адресов](./media/sap-hana-overview-architecture/image7-minimal-deployment.png)
