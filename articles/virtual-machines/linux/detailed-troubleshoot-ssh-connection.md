...
2. Выберите **Параметры**, чтобы проверить конечные точки, IP-адреса, группы безопасности и другие параметры.
   ...
   Виртуальная машина должна иметь конечную точку, определенную для SSH-трафика, которую можно просмотреть в разделе **Конечные точки** или **[Группа безопасности сети](../../virtual-network/virtual-networks-nsg.md)**. Конечные точки в виртуальных машинах, созданных с помощью Resource Manager, сохраняются в группе безопасности сети. Убедитесь, что правила были применены к группе безопасности сети и на которые имеются ссылки в подсети.

Чтобы проверить подключение к сети, проверьте настроенные конечные точки и можно ли подключиться к виртуальной машине через другой протокол, например HTTP или другой службы.
...
Клиент SSH на компьютере не удастся подключиться к службе SSH в виртуальной Машине Azure, из-за проблем или неверных настроек в следующих областях:
...
Во избежание организации периферийным устройством как источника сбоя, убедитесь, что компьютер подключен непосредственно к Интернету можно сделать соединений по протоколу SSH для виртуальной Машины Azure. Если вы получаете доступ к виртуальной машине через VPN-подключение типа "сеть — сеть" или подключение Azure ExpressRoute, перейдите к подразделу [Источник 4: группы безопасности сети](#nsg).

![Схема, на которой выделено пограничное устройство организации](./media/detailed-troubleshoot-ssh-connection/ssh-tshoot3.png)

Если у вас нет компьютера, находящегося непосредственно подключен к Интернету, создание новой виртуальной Машины Azure в собственной группе ресурсов или облачной службы и использования этой новой виртуальной Машины. Дополнительные сведения см. в статье [Создание виртуальной машины Linux с помощью интерфейса командной строки Azure 2.0](quick-create-cli.md). Завершив проверку, удалите группу ресурсов или виртуальную машину и облачную службу.
...
Чтобы исключить источник сбоя конечная точка облачной службы и список управления Доступом, убедитесь, что другой ВМ в той же виртуальной сети могут подключаться с помощью SSH.
