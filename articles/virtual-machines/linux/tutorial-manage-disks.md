...
В этом руководстве рассматриваются различные типы дисков виртуальной машины и описывается, как выбрать конфигурацию дисков, а затем создать и подключить диски к виртуальным машинам Azure. Кроме того, в этом руководстве рассматривается создание моментальных снимков дисков. 
...
При создании виртуальной машины Azure в ней автоматически подключаются два диска. 
...
| Тип | Размер виртуальной машины | Максимальный размер временного диска |
|----|----|----|
...
Чтобы обеспечить повторное подключение диска после перезагрузки, его необходимо добавить в файл `/stc/fstab`. Для этого получите UUID диска с помощью служебной программы `blkid`.
...
Добавьте в файл `/etc/fstab` строку следующего вида. Также обратите внимание, что запрет ограничение можно отключить с помощью `barrier=0`. Это может повысить производительность диска. 
...
Сначала найдите имя диска данных, выполнив команду [az disk list](https://docs.microsoft.com/cli/azure/disk#list). В этом примере имя диска помещается в переменную `datadisk`, которая будет использоваться в следующем шаге.
...
В этом руководстве вы изучили диски виртуальных машин. Перейдите к следующему руководству, чтобы узнать об автоматической настройке виртуальных машин.

[Как настроить виртуальную машину Linux при первой загрузке](./tutorial-automate-vm-deployment.md)
