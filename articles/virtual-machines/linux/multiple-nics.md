...
Установите последнюю версию [Azure CLI 2.0](/cli/azure/install-az-cli2) и войдите в систему с учетной записью Azure, выполнив команду [az login](/cli/azure/#az_login).
...
Чтобы изменения вступили в силу и применялись во время активации сетевого стека, измените файл */etc/sysconfig/network-scipts/ifcfg-eth0* и */etc/sysconfig/network-scipts/ifcfg-eth1*. Измените строку *NM_CONTROLLED=yes* на *NM_CONTROLLED=no*. Если не выполнить этот шаг, дополнительные правила и маршруты не применяются автоматически.
