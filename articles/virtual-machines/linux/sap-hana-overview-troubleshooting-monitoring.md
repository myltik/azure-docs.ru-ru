...
---
# <a name="troubleshooting-and-monitoring-of-sap-hana-on-azure-large-instances"></a>Устранение неполадок и мониторинг архитектуры решения "SAP HANA в Azure (крупные экземпляры)".
...
![В SAP HANA Studio выберите Administration Console: Alerts: Show: all alerts (Консоль администрирования: Оповещения: Показать: Все оповещения).](./media/sap-hana-overview-troubleshooting-monitoring/image1-show-alerts.png)
...
![Восстановление значения по умолчанию или задание приемлемого порогового значения](./media/sap-hana-overview-troubleshooting-monitoring/image2-cpu-utilization.png)
...
![Отображаемое потребление ресурсов ЦП на экране обзора](./media/sap-hana-overview-troubleshooting-monitoring/image3-cpu-usage.png)
...
![На графике Load (Нагрузка) может быть показан высокий уровень потребления ресурсов ЦП или высокий уровень потребления в прошлом](./media/sap-hana-overview-troubleshooting-monitoring/image4-load-graph.png)
...
![На вкладке Volumes (Тома) в SAP HANA Studio отображаются подключенные тома, а также то, какие тома использует каждая служба](./media/sap-hana-overview-troubleshooting-monitoring/image5-volumes-tab-a.png)
...
![В подключенных томах в нижней части экрана можно просматривать сведения о томах, например о файлах и статистике операций ввода-вывода](./media/sap-hana-overview-troubleshooting-monitoring/image6-volumes-tab-b.png)
...
![В SAP HANA Studio на вкладке System Information (Сведения о системе) щелкните правой кнопкой мыши столбец Name (Имя) и выберите Import SQL Statements (Импорт инструкций SQL)](./media/sap-hana-overview-troubleshooting-monitoring/image7-import-statements-a.png)
...
![Откроется полная инструкция SQL, что позволит изменить входные параметры (раздел изменения), а затем выполнить их](./media/sap-hana-overview-troubleshooting-monitoring/image8-import-statements-b.png)
...
![Другой пример — щелкнуть правой кнопкой мыши инструкции, выбрав Replication: Overview (Репликация: Обзор). Выбор Execute (Выполнить) в контекстном меню](./media/sap-hana-overview-troubleshooting-monitoring/image9-import-statements-c.png)

После этого отобразятся сведения, которые помогут в устранении неполадок.

![После этого отобразятся сведения, которые помогут в устранении неполадок](./media/sap-hana-overview-troubleshooting-monitoring/image10-import-statements-d.png)
...
![HANA\_Configuration\_MiniChecks\_Rev102.01 +&1; для общих проверок SAP HANA](./media/sap-hana-overview-troubleshooting-monitoring/image11-configuration-minichecks.png)

**HANA\_Services\_Overview** для получения общих сведений о том, какие службы SAP HANA запущены в данный момент.

![HANA\_Services\_Overview для получения общих сведений о том, какие службы SAP HANA запущены в данный момент](./media/sap-hana-overview-troubleshooting-monitoring/image12-services-overview.png)

**HANA\_Services\_Statistics** для получения сведений о службе SAP HANA (ЦП, память и т. д.).

![HANA\_Services\_Statistics для получения сведений о службе SAP HANA ](./media/sap-hana-overview-troubleshooting-monitoring/image13-services-statistics.png)

**HANA\_Configuration\_Overview\_Rev110 +** для получения общих сведений об экземпляре SAP HANA.

![HANA\_Configuration\_Overview\_Rev110 + для получения общих сведений об экземпляре SAP HANA](./media/sap-hana-overview-troubleshooting-monitoring/image14-configuration-overview.png)

**HANA\_Configuration\_Parameters\_Rev70 +** для проверки параметров SAP HANA.

![HANA\_Configuration\_Parameters\_Rev70 + для проверки параметров SAP HANA](./media/sap-hana-overview-troubleshooting-monitoring/image15-configuration-parameters.png)
