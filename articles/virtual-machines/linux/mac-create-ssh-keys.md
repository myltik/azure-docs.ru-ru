...
Для создания файлов открытого и закрытого ключа SSH используйте команду `ssh-keygen`. По умолчанию эти файлы будут созданы в каталоге `~/.ssh`. При появлении запроса можно указать другие каталоги и дополнительные парольные фразы (пароль для доступа к файлу закрытого ключа). Если в текущем каталоге существует пара ключей SSH, они будут перезаписаны.
