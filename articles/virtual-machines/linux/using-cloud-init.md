...
Если не включать Azure CLI `--custom-data` ключ во время WALA подготовки имеет минимальной подготовки параметров, необходимых для подготовки виртуальной Машины и выполнить развертывание с параметрами по умолчанию виртуальной Машины.  При ссылке облака init `--custom-data` переключения, все, что содержится в пользовательских данных (отдельные параметры или полный скрипт) переопределяет значения по умолчанию WALA. 

WALA конфигураций виртуальных машин, ограниченное время для работы в максимальное время подготовки виртуальной Машины.  Конфигурация cloud-init применяется к виртуальным машинам без временных ограничений, которые не приведут к сбою развертывания по времени. 

## <a name="deploying-a-cloud-init-enabled-virtual-machine"></a>Развертывание виртуальной машины с поддержкой cloud-init
Развертывать виртуальную машину с включенным cloud-init так же просто, как и ссылаться на распределение с включенным cloud-init во время развертывания.  Распространители дистрибутивов Linux должны включать и интегрировать cloud-init в свои базовые опубликование образы Azure. Когда вы убедились, что изображение, которое вы хотите развернуть облачные init включена, Azure CLI можно использовать для развертывания образа. 

Первым шагом при развертывании образа является создание группы ресурсов с использованием команды [az group create](/cli/azure/group#create). Группа ресурсов Azure является логическим контейнером, в котором происходит развертывание ресурсов Azure и управление ими. 
...
После подготовки виртуальной Машины, облачных init будет выполняться через все модули и определенных сценариев в `--custom-data` для настройки виртуальной Машины.  Если вам необходимо устранить ошибки или пропуски в конфигурации, найдите имя модуля (`disk_setup` или `runcmd`, к примеру) в журнале cloud-init, который находится в файле **/var/log/cloud-init.log**.
