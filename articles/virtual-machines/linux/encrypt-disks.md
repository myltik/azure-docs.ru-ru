...
Для расширенного виртуальной машины (VM) безопасности и соответствия требованиям виртуальные диски и самой ВМ могут быть зашифрованы. Виртуальные машины, шифруются с помощью криптографических ключей, которые защищены в хранилище ключей Azure. Вы будете управлять этими криптографическими ключами и проводить аудит их использования. В этой статье описывается шифрование дисков на виртуальной машине Linux с помощью Azure CLI 2.0. Эти действия можно также выполнить с помощью [Azure CLI 1.0](encrypt-disks-nodejs.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json).
...
Создайте субъект-службу с помощью Azure Active Directory, выполнив команду [az ad sp create-for-rbac](/cli/azure/ad/sp#create-for-rbac). Субъект-служба осуществляет проверку подлинности и обмен криптографическими ключами из Key Vault. В следующем примере считывается в значениях идентификатор участника службы и пароль для использования в командах более поздней версии:
...
Создайте виртуальною машину, выполнив команду [az vm create](/cli/azure/vm#create), и подключите диск данных емкостью 5 ГБ. Только некоторые образы Marketplace поддерживают шифрование диска. В следующем примере создается виртуальная машина с именем *myVM* с использованием образа *CentOS 7.2n*:
...
SSH для виртуальной Машины с помощью *publicIpAddress* в выходных данных предыдущей команды. Создайте раздел и файловую систему, а затем подключите диск данных. Дополнительные сведения см. в разделе [Подключение к виртуальной машине Linux для подключения нового диска](add-disk.md?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json#connect-to-the-linux-vm-to-mount-the-new-disk). Закройте сеанс SSH.

Зашифруйте виртуальную машину, выполнив команду [az vm encryption enable](/cli/azure/vm/encryption#enable). В следующем примере используется *$sp_id* и *$sp_password* переменные из указанных выше `ad sp create-for-rbac` команды:
...
Процесс шифрования диска завершается во время загрузки, поэтому подождите 5 минут, прежде чем повторно проверять состояние шифрования, и выполните команду [az vm encryption show](/cli/azure/vm/encryption#show):
...
* Стандартный объект, доменных служб Active Directory, G, D GS, т. д., виртуальные машины серии.
...
Дополнительные сведения о поддерживаемых сценариях и ограничениях см. в разделе [шифрование диска Azure ВМ IaaS](../../security/azure-security-disk-encryption.md)
...
Хранить криптографические ключи можно с помощью программного обеспечения или защиты HSM. Для использования HSM требуется хранилище ключей уровня "Премиум". Создание хранилища ключей уровня "Премиум" осуществляется за дополнительную плату в отличие от хранилища ключей уровня "Стандартный", в котором хранятся ключи, защищенные программным обеспечением. Чтобы создать хранилище ключей уровня "Премиум", на предыдущем шаге добавьте `--sku Premium` к команде. В следующем примере используется ключей, защищенных с помощью программного обеспечения с момента создания стандартных хранилища ключей.
...
Зашифруйте виртуальную машину, выполнив команду [az vm encryption enable](/cli/azure/vm/encryption#enable). В следующем примере используется *$sp_id* и *$sp_password* переменные из указанных выше [az ad sp создать для rbac](/cli/azure/ad/sp#create-for-rbac) команды:
...
Повторно выполните команду для шифрования виртуальные диски следующим образом:
