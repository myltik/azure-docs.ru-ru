...
- Введите имя пользователя, **администратора**, затем укажите *initialAdminPassword* получен из виртуальной Машины в предыдущем шаге.
- Выберите **управления Jenkins**, затем **управление подключаемыми модулями**.
- Выберите **доступно**, выполните поиск *GitHub* в текстовом поле в верхней части. Установите флажок для *подключаемый модуль GitHub*, а затем выберите **загрузить и установить после перезапуска**.
- Установите флажок для **Jenkins перезапустить после завершения установки и не выполняются задания**, а затем подождите, пока подключаемый модуль процесс установки завершается.
...
- В разделе **Build** (Сборка) щелкните **Add build step** (Добавить шаг сборки). Выберите **выполнение оболочки**, затем введите `echo "Testing"` в окне команд.
...
В Jenkins запускается новая сборка в разделе **Build history** (Журнал сборок) в левом нижнем углу страницы задания. Выберите ссылку номера сборки и выберите **вывод на консоль** с левой стороны. Вы можете просмотреть действия, выполняемые Jenkins по мере получения кода из GitHub и вывода действием сборки сообщения `Testing` на консоли. Каждый раз, когда в GitHub, выполняется фиксация веб-перехватчик достигает Jenkins и запускает новую сборку таким образом.
...
Измените путь SSH-подключения к виртуальной машине, задав каталог рабочей области Jenkins, имя которого соответствует заданию, созданному на предыдущем шаге. В этом примере, называется *HelloWorld*.

```bash
cd /var/lib/jenkins/workspace/HelloWorld
```

Создать файл в этом каталоге рабочей области с `sudo sensible-editor Dockerfile` и вставьте следующее содержимое. Убедитесь, что весь файл Dockerfile скопирован правильно, особенно первая строка:
