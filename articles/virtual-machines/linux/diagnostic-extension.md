...
Это расширение можно включить с помощью командлетов Azure PowerShell, скриптов Azure CLI или шаблонов развертывания Azure. Дополнительные сведения см. в статье [Обзор расширений](./extensions-features.md).
...
Класс метрик "Процессор" предоставляет сведения об использовании процессоров в виртуальной машине. В результате статистической обработки процентных значений получается среднее значение по всем ЦП. Если в двухъядерной виртуальной машине одно ядро было загружено на 100 %, а другое бездействовало 100 % времени, счетчик PercentIdleTime будет иметь значение 50. Если в течение этого же периода каждое ядро было загружено на 50 %, значением счетчика будет 50. Если в четырехъядерной виртуальной машине одно ядро было загружено на 100 %, а другие бездействовали, счетчик PercentIdleTime будет иметь значение 75.
