...
Со временем вам понадобится обновить драйверы InfiniBand вычислительных узлов Linux. Дополнительные сведения см. в статье [Обновление драйверов Linux RDMA для SLES 12](virtual-machines-linux-classic-rdma-cluster.md/#update-the-linux-rdma-drivers-for-sles-12).
...
Скачайте пакет Linux64 STAR-CCM+ на [портале CD-adapco](https://steve.cd-adapco.com). В нашем случае мы использовали версию 11.02.010 в режиме смешанной точности.

В головном узле в общей папке Azure **/hpcdata** создайте сценарий оболочки **setupstarccm.sh** со следующим содержимым. Этот сценарий будет выполняться на каждом вычислительном узле для локальной настройки STAR-CCM+.
...
Узлы выделяются исключительно для конкретного задания, и их не могут совместно использовать другие задания. Задание не запускается как задание MPI напрямую. Сценарий оболочки **runstarccm.sh** запускает средство запуска MPI.
