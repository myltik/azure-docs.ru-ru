...
После успешного входа на экране будут отображаться некоторые сведения, в том числе имя и идентификатор подписки по умолчанию. Это подписка, в которой будут созданы ресурсы для этого руководства (если вы не перейдете к другой подписке). При наличии нескольких подписок, чтобы получить их список, выполните следующий командлет:

```PowerShell
Get-AzureRmSubscription
```
Чтобы использовать другую подписку, выполните команду ниже. Укажите имя своей подписки.
...
Используйте следующие переменные, чтобы определить образ, который будет применен для виртуальной машины. В этом примере используется образ SQL Server 2016 Developer Edition. Лицензию Developer Edition для тестирования и разработки можно получить бесплатно. Вы оплачиваете только стоимость выполнения виртуальной машины.
...
Кроме того, выполнив команду Get-AzureRmVMImageSku, можно получить сведения о единицах хранения, доступных для предложения. При выполнении команды ниже отображаются все SKU предложения **SQL2016SP1-WS2016**.
