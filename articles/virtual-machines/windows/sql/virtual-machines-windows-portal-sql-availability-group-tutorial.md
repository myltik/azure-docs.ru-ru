...
# <a name="create-always-on-availability-group-to-improve-availability-and-disaster-recovery"></a>Создание группы доступности AlwaysOn для улучшения доступности и аварийного восстановления
...
## <a name="a-namecreateclusteracreate-the-cluster"></a><a name="CreateCluster"></a>Создание кластера
...
### <a name="a-nameaddnodeaadd-the-other-sql-server-to-cluster"></a><a name="addNode"></a>Добавление в кластер другого сервера SQL Server
...
### <a name="a-namebackupsharea-create-a-backup-share"></a><a name="backupshare"></a>Создание общего ресурса для резервных копий
...
1. Выберите подсистему балансировки нагрузки, щелкните **Серверные пулы** и нажмите кнопку **+ Добавить**. Задайте следующие параметры внутреннего пула.

   | Настройка | Описание | Пример
   | --- | --- |---
   | **Имя** | Введите текстовое имя. | SQLLBBE
   | **группа доступности;** | Используйте имя группы доступности, в которой находятся виртуальные машины SQL Server. | sqlAvailabilitySet |
   | **Виртуальные машины** |Имена двух виртуальных машин SQL Server в Azure. | sqlserver-0, sqlserver-1

1. Введите имя для внутреннего пула.

1. Щелкните **+ Добавить виртуальную машину**.

1. Выберите группу доступности, содержащую серверы SQL Server.

1. В качестве виртуальных машин выберите оба сервера SQL Server. Не следует добавлять сервер файлового ресурса-свидетеля. Выбранные параметры должны иметь вид, как на следующем рисунке.

   ![Поиск подсистемы балансировки нагрузки в группе ресурсов](./media/virtual-machines-windows-portal-sql-availability-group-tutorial/88-configurebepool.png)
...
## <a name="a-nameconfigure-listenera-configure-the-listener"></a><a name="configure-listener"></a>Настройка прослушивателя
