...
С последней облако api выпуска Chef обеспечивает прозрачную интеграцию с Azure, позволяя вам подготовить и развернуть состояния конфигурации одной командой.

В этой статье Настройка среды Chef для подготовки виртуальных машин Azure и выполнить пошаговые инструкции по созданию политики или «Практическое руководство» и затем развернуть это руководство к виртуальной машине Azure.

Итак, начнем!

## <a name="chef-basics"></a>Основы использования Chef
Прежде чем начать, [ознакомьтесь с основными понятиями Chef](http://www.chef.io/chef). 
...
Сервер Chef является точкой управления и есть два варианта для сервера: ведущее решение или локальным решением. Мы будем использовать размещенное решение.

Клиент Chef (узел) — это агент, который находится на управляемых серверах.

Рабочая станция Chef является администраторские рабочие станции, где Создание политик и выполнять команды управления. Мы запуска **knife** команду на рабочей станции Chef для управления инфраструктурой.

Кроме того, существует понятие "детальных инструкций" и "рецептов". Это эффективно политики мы определить и применить к серверам.

## <a name="preparing-the-workstation"></a>Подготовка рабочей станции
Сначала давайте подготовим рабочую станцию. В данном случае используется стандартная рабочая станция Windows. Необходимо создать каталог для хранения файлов конфигурации и справочники.

Сначала создайте каталог с именем C:\chef.

Затем создайте каталог C:\chef\cookbooks.

Теперь нам нужно загрузить файл параметров в Azure, чтобы Chef можно взаимодействовать с подпиской Azure.
...
Подробная инструкция Chef используется для определения набора команд, которые необходимо выполнить на управляемом клиенте. Создание кулинарной книги довольно просто, и мы используем **chef создания рецепты** команду, чтобы создать шаблон Практическое руководство. Мы назовем свою подробную инструкцию именем webserver, так как нам нужна политика, которая автоматически развертывает службы IIS.

В каталоге C:\Chef выполните следующую команду:

    chef generate cookbook webserver

Будет создан набор файлов в каталоге C:\Chef\cookbooks\webserver. Теперь нам нужно определить набор команд, мы хотели бы клиент Chef для выполнения на управляемую виртуальную машину.
...
На этом шаге мы создавая копию рецепты, созданную на локальном компьютере и передачи их в размещенного сервера Chef. После передачи подробная инструкция появится на вкладке **Политика** .
...
Мы развертывание виртуальной машины Azure и применить рецепты «Веб-сервер», который будет установлен IIS web service и по умолчанию веб-странице.
