...
При запуске пользовательского анализа медленной виртуальной машины одновременно выполняются все выбранные трассировки (счетчик производительности, xperf, сеть, storport). После завершения трассировки средство выполняет тест производительности diskspd, если он выбран. 
...
1.  В сценариях "Тест производительности" или "Пользовательский анализ медленной виртуальной машины", для которых настроено использование XPerf или DiskSpd, скрипт может неблагоприятно повлиять на производительность виртуальной машины. Мы не рекомендуем выполнять эти скрипты в рабочей среде без контроля со стороны инженера CSS.

2.  При использовании сценариев "Тест производительности" или "Пользовательский анализ медленной виртуальной машины", в которых настроено использование DiskSpd, убедитесь, что фоновые действия не мешают рабочей нагрузке ввода-вывода на тестируемых дисках.
...
8.  Также вы можете запустить PerfInsights без пользовательского интерфейса.
    ...
    Следующая команда выполняет сценарий устранения неполадок "Анализ медленной виртуальной машины" в течение 30 секунд, не создавая запросы пользовательского интерфейса и не собирая данные. Вам будет предложено принять отказ от ответственности и лицензионное соглашение, указанные на шаге 4.
...
### <a name="how-do-i-run-the-script-in-custom-slow-vm-analysis-mode"></a>Как запустить скрипт в режиме пользовательского анализа медленной виртуальной машины?

Если вы выберете **Пользовательский анализ медленной виртуальной машины**, можно запустить несколько трассировок в параллельном режиме (используйте клавишу SHIFT для выбора нескольких вариантов).

![выбор сценариев](media/how-to-use-perfInsights/select-scenario.png)

Если вы выберете сценарий трассировки счетчиков производительности, трассировки XPerf, сетевой трассировки или трассировки Storport, выполните все инструкции в диалоговых окнах и попробуйте воспроизвести проблему со снижением производительности, когда начнется трассировка.
...
![Вкладка поиска](media/how-to-use-perfInsights/findingtab.png)
![findings](media/how-to-use-perfInsights/findings.PNG)

**Примечания**

-   В категорию критически важных результатов попадают известные проблемы, которые могут вызвать проблемы с производительностью.

-   В категорию важных попадают результаты, указывающие на неоптимальную конфигурацию, которая может не приводить к проблемам с производительностью.

-   Результаты из категории информационных содержат сообщения, которые полезно принять к сведению.

Обязательно изучите рекомендации и ссылки, описывающие взаимодействие со всеми критическими и важными результатами. Вы получите подробные сведения о найденных проблемах и их влиянии на производительность, а также рекомендации по оптимизации конфигурации для повышения производительности.
...
Вкладка диагностики содержит сведения об основных потребителях ЦП, дисков и памяти за период, в течение которого на компьютере выполнялся скрипт PerfInsights. Здесь есть и другие полезные сведения, например отсутствующие критически важные исправления, список задач и важные системные события. 
