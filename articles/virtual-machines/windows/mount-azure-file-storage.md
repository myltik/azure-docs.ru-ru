...
Вы можете использовать общие файловые ресурсы Azure для хранения файлов с вашей виртуальной машины и для доступа к ним. Например, можно сохранить скрипт или файл конфигурации приложения, который требуется использовать на всех виртуальных машинах. В этой статье мы показано, как создать и подключить Azure файловый ресурс и как отправка и загрузка файлов.

## <a name="connect-to-a-file-share-from-a-vm"></a>Подключение к общему файловому ресурсу с виртуальной машины

В этом разделе предполагается, что у вас уже есть общий файловый ресурс, к которому нужно подключиться. Если необходимо создать его, см. раздел [создайте общую папку](#create-a-file-share) далее в этой статье.
...
5. Выберите общую папку или нажмите кнопку **+ общий файловый ресурс** для создания общей папки для использования.
...
7. В **букву диска**, выберите это буква, которую вы хотите использовать для идентификации диска.
8. Выберите, какой синтаксис и выберите "Копировать" справа, чтобы скопировать его в буфер обмена. Вставьте некоторые месте, где его можно легко получить. 
...
3. Выберите учетную запись хранения.
4. На странице **Обзор** в разделе **Служба** выберите **Файлы**.
5. На странице службы файла щелкните **+ общий файловый ресурс**.
...
8. Нажмите кнопку **ОК** для создания общей папки.
