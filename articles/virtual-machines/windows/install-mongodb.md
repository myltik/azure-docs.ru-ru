...
[MongoDB](http://www.mongodb.org) — это популярная высокопроизводительная база данных NoSQL с открытым кодом. В этой статье руководство по установке и настройке MongoDB для виртуальной машины (VM) Windows Server 2016 в Azure. [MongoDB можно также установить на виртуальной машине Linux в Azure](../linux/install-mongodb.md).
...
1. После подключения к виртуальной Машине с помощью удаленного рабочего стола, откройте Internet Explorer из панели задач.
...
3. Конфигурация усиленной безопасности Internet Explorer включена по умолчанию. Добавьте веб-сайт MongoDB в список разрешенных сайтов.
   ...
   * Нажмите кнопку **Сайты**. Добавить *https://\*. mongodb.com* в список надежных узлов, а затем закройте диалоговое окно.
     
     ![Настройка параметров безопасности Internet Explorer](./media/install-mongodb/configure-internet-explorer-security.png)
4. Перейдите к [загружает MongoDB -](http://www.mongodb.com/downloads) страницы (http://www.mongodb.com/downloads).
5. При необходимости выберите **сервер сообщества** выпуск и установите флажок, последняя стабильная текущего выпуска для*Windows Server 2008 R2 64-разрядная и более поздние версии*. Чтобы скачать установщик, щелкните **DOWNLOAD (msi)** (Скачать (MSI-файл)).
...
7. При желании можно также установить компас графический интерфейс для MongoDB.
