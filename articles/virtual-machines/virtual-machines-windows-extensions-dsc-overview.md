...
**Локальный компьютер**. Для взаимодействия с расширением виртуальной машины Azure нужен портал Azure или пакет SDK для Azure PowerShell.
...
Когда расширение вызывается впервые, оно запускает процесс установки. Этот процесс устанавливает определенную версию Windows Management Framework (WMF), как указано ниже.
...
`Set-AzureVMDscExtension` внедряет параметры, необходимые расширению DSC PowerShell, в объект конфигурации виртуальной машины, который затем можно применить к виртуальной машине Azure с помощью командлета `Update-AzureVM`.
