...
В первом разделе «Основные шаги» перечислены шаги по решению распространенных проблем, а во втором — шаги по устранению ошибок на основе соответствующих сообщений.
...
- Измените размер виртуальной машины.<br> Последовательно выберите «Просмотреть все» > «Виртуальные машины (классические)» > имя своей виртуальной машины > «Настройки» > **Размер**. Подробные указания см. в разделе [Изменение размера виртуальной машины](https://msdn.microsoft.com/library/dn168976.aspx).
...
## Устранение ошибки выделения ресурсов в модели развертывания диспетчера ресурсов Azure
В этом разделе описаны распространенные сценарии выделения ресурсов, которые приводят к прикреплению запросов на выделение. Далее в этой статье будут описаны отдельные сценарии: изменение размера виртуальной машины либо добавление дополнительных виртуальных машин или экземпляров ролей в существующую облачную службу; перезагрузка остановленных (освобожденных) виртуальных машин, **частичное** освобождение; перезагрузка остановленных (освобожденных) виртуальных машин, **полное** освобождение.
...
## Устранение ошибки выделения ресурсов в классической модели развертывания
В этом разделе описаны распространенные сценарии выделения ресурсов, которые приводят к прикреплению запросов на выделение. Далее в этой статье будут описаны отдельные сценарии: изменение размера виртуальной машины либо добавление дополнительных виртуальных машин или экземпляров ролей в существующую облачную службу; перезагрузка остановленных (освобожденных) виртуальных машин, частичное освобождение; перезагрузка остановленных (освобожденных) виртуальных машин, полное освобождение; промежуточное и рабочее развертывание (только для платформы как услуги); группа сходства; взаимодействие с виртуальной машиной или службой; виртуальная сеть на основе группы сходства.
...
Если эта статья не помогла решить проблему с Azure, см. форумы Azure на сайтах [MSDN и Stack Overflow](http://azure.microsoft.com/support/forums/). Кроме того, можно зарегистрировать обращение с проблемой в службу поддержки Azure. Перейдите на [веб-сайт службы поддержки Azure](http://azure.microsoft.com/support/options/) и щелкните «Получить поддержку». Дополнительную информацию об использовании службы поддержки Azure см. в статье [Часто задаваемые вопросы о поддержке Microsoft Azure](http://azure.microsoft.com/support/faq/).
