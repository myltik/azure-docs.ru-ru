<properties
	...
	description="Поиск и устранение распространенных проблем с подключением к виртуальной машине Windows через RDP. Порядок действий в зависимости от полученного сообщения об ошибке и подробные сведения об устранении неполадок, связанных с сетью."
...
Сбой подключения виртуальной машины Azure под управлением Windows к удаленному рабочему столу может быть вызван различными причинами. Проблемы могут быть связаны с программным обеспечением RDP на виртуальной машине, главным компьютером или сетевым подключением либо возникнуть при подключении на стороне клиента. Эта статья поможет определить и устранить причины сбоя.

Статья относится только к виртуальным машинам Azure под управлением Windows. Инструкции по устранению неполадок подключения на *виртуальных машинах Azure под управлением Linux* см. в [этой статье](virtual-machines-troubleshoot-ssh-connections.md).
...
- Перезапустите виртуальную машину для решения других проблем с запуском.<br> Последовательно выберите "Просмотреть все" > "Виртуальные машины" > имя вашей виртуальной машины Windows > **Перезапустить**.

- Для решения любой проблемы размещения измените размер виртуальной машины.<br> Последовательно выберите "Просмотреть все" > "Виртуальные машины" > имя вашей виртуальной машины Windows > "Параметры" > **Размер**.
...
## Устранение распространенных ошибок RDP
...
Убедитесь в том, что у учетной записи, которую вы используете для подключения, есть права для входа на удаленный рабочий стол. В качестве обходного решения вы можете воспользоваться учетной записью администратора домена или локального администратора, чтобы создать подключение к удаленному рабочему столу и добавить требуемую учетную запись в локальную группу пользователей удаленного рабочего стола в оснастке "Управление компьютером" (**Служебные программы > Локальные пользователи и группы > Группы > Пользователи удаленного рабочего стола**).
