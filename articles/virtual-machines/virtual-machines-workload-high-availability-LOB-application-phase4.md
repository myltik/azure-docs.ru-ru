...
Если в поле **IsAvailable** в данных, возвращаемых командой Test-AzureStaticVNetIP, присутствует значение **True**, IP-адрес можно использовать.
...
4. На странице выбора конечного сервера нажмите кнопку **Далее**.
5. На странице ролей сервера в списке **Роли** щелкните **Веб-сервер (IIS)**.
6. При появлении запроса щелкните **Добавить компоненты**, а затем нажмите кнопку **Далее**.
7. На странице выбора компонентов нажмите кнопку **Далее**.
...
Дальнейшие действия по настройке этой рабочей нагрузки см. в статье [Пятый этап: создание группы доступности и добавление баз данных приложений](virtual-machines-workload-high-availability-LOB-application-phase5.md).
