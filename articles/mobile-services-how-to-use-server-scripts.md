<properties linkid="register-for-facebook-auth" urlDisplayName="Мобильные службы" pageTitle="Работа с серверными скриптами в мобильных службах" metaKeywords="серверные скрипты, мобильные устройства, Azure, планировщик" description="Примеры того, как определить, зарегистрировать и использовать серверные скрипты в мобильных службах Azure." metaCanonical="" services="" documentationCenter="Mobile" title="Работа с серверными скриптами в мобильных службах" authors="ricksal" solutions="" manager="" editor="" />


# Работа с серверными скриптами в мобильных службах
 
Эта статья содержит подробные сведения о серверных скриптах в мобильных службах Azure и примеры работы с ними. Этот раздел содержит следующие параграфы:
...
+ [Операции с таблицами]
	...
	+ [Практическое руководство. Работа с пользователями таблицы][Практическое руководство. Работа с пользователями]
+ [Настраиваемая служба API][Привязка настраиваемого интерфейса API]
...
+ [Планировщик заданий]
	+ [Практическое руководство. Определение скриптов запланированных заданий]
...
В мобильных службах можно определить настраиваемую бизнес-логику как код JavaScript, который хранится и выполняется на сервере. Этот серверный код скрипта назначается одной из следующих функций сервера:
...
+ [HTTP-методы, определенные в настраиваемом API-интерфейсе][Привязка настраиваемого интерфейса API]. 
...
Скрипты операций с таблицами пишутся при необходимости применять настраиваемую бизнес-логику при выполнении операции. Например, приведенный ниже скрипт отклоняет операции вставки, если длина строки поля `text` больше десяти символов. 
...
- Третьим аргументом всегда является [объект запроса][объект запроса], с помощью которого можно контролировать выполнение запрошенной операции и ответ, отправляемый клиенту.

Ниже приведены канонические подписи функций main для операций с таблицей. 

+ [Вставить][функция insert]: `function insert (item, user, request) { ... }`
+ [Обновить][функции update]: `function update (item, user, request) { ... }`
+ [Удалить][функция delete]: `function del (id, user, request) { ... }`
+ [Читать][функция read] `function read (query, user, request) { ... }`

>[WACOM.NOTE]Поскольку delete является зарезервированным ключевым словом в JavaScript, функция, зарегистрированная для операции удаления, должна иметь имя _del_. 
...
Скрипт операции с таблицей должен вызывать по крайней мере одну из перечисленных ниже функций [объекта запроса][объект запроса], чтобы гарантировать получение ответа клиентом. 
...
При вызове функции **execute** значение `item`, [query][объект query] или `id` передается в качестве первого аргумента в функцию скрипта, используемую для выполнения операции. Для операции вставки, обновления или запроса можно изменить значение item или query перед вызовом функции **execute**. 
...
Передавая обработчик **success** при вызове функции "execute", можно изменить результаты запроса до их записи в ответ. В следующем примере вызывается функция `execute({ success: function(results) { ... })` для выполнения дополнительной работы после чтения данных из базы данных, но перед записью ответа.
...
>[WACOM.NOTE]Функцию **respond** без аргументов для вызова ответа по умолчанию можно вызвать только в том случае, если вначале вызвать функцию **execute**.
...
Обратите внимание, что в **insertItemIfNotComplete** функция **execute** [объекта запроса][объект запроса] вызывается для вставки элемента при отсутствии повторяющегося текста; в противном случае вызывается функция **respond**, которая уведомляет клиента о повторе. 
...
+ На [портале управления Azure][Management Portal]. Скрипты настраиваемого интерфейса API создаются и изменяются во вкладке **API**. Код серверного скрипта находится во вкладке **Скрипты** данного настраиваемого интерфейса API. Ниже приведен скрипт, который вызывается по запросу POST к конечной точке `CompleteAll` настраиваемого API-интерфейса. 
...
Эта функция настраиваемого API вызывается с помощью запроса HTTP POST к следующей конечной точке:
		...
		https://<служба>.azure-mobile.net/api/orderpizza

Доступ к конкретному заголовку HTTP можно также получить из [объекта запроса][объект запроса], как показано в следующем коде:
...
+ `https://<служба>.azure-mobile.net/api/calculator/add`
+ `https://<служба>.azure-mobile.net/api/calculator/sub`
...
+ `https://<служба>.azure-mobile.net/api/calculator/add?a=1&b=2`

		{"result":3}

+ `https://<служба>.azure-mobile.net/api/calculator/sub?a=3&b=5`
...
+ На [портале управления Azure][Management Portal] на вкладке **Скрипт** в планировщике:
...
>[WACOM.NOTE]Если система управления версиями включена, можно редактировать файлы скриптов запланированных заданий непосредственно во вложенной папке .\service\scheduler в репозитории git. Дополнительные сведения содержатся в разделе [Практическое руководство. Общий код с использованием системы управления версиями][Практическое руководство. Совместное использование кода с помощью системы управления версиями].
...
+ **crypto**: обеспечивает функциональные возможности шифрования OpenSSL. Дополнительные сведения см. в [Документации по Node.js][API: шифрование].
+ **path**: содержит служебные программы для работы с путями файлов. Дополнительные сведения см. в [Документации по Node.js][API: путь].
+ **querystring**: содержит служебные программы для работы со строками запросов. Дополнительные сведения см. в [Документации по Node.js][API: строка запроса].
...
+ **url**: содержит служебные программы для анализа и разрешения URL-адресов. Дополнительные сведения см. в [Документации по Node.js.][API: URL-адрес]
...
+ Для закрытых или настраиваемых модулей можно использовать npm, чтобы вручную установить модуль в каталог `.\service\node_modules` системы управления версиями. Пример того, как вручную загрузить модуль, см. в разделе [Использование общего кода и модулей Node.js в серверных скриптах].
	...
	>[WACOM.NOTE]Если `node_modules` уже существует в иерархии каталогов, NPM создает там подкаталог `\node-uuid`, вместо того чтобы создать новый каталог `node_modules` в репозитории. В данном случае просто удалите существующий каталог `node_modules`.
...
Наиболее простой способ доступа к таблицам из скрипта — использовать [объект таблиц]. Функция **GetTable** возвращает экземпляр [объекта таблицы][объект таблицы], который является прокси для доступа к запрашиваемой таблице. Затем можно вызвать функции прокси для доступа к данным и их изменения. 

Скрипты, зарегистрированные для операций с таблицей и запланированных заданий, могут получить доступ к [объекту таблиц][объект таблиц] как глобальному объекту. Эта строка кода возвращает прокси для таблицы *TodoItems* из глобального [объекта таблиц][объект таблиц]: 

		var todoItemsTable = tables.getTable('TodoItems');

Скрипты настраиваемого API могут получить доступ к [объекту таблиц][объект таблиц] из свойства <strong>службы</strong> предоставляемого [объекта запроса][объект запроса]. Эта строка кода получает [объект таблиц] из запроса:
...
Последний пример — в образце кода, который можно посмотреть здесь: [Практическое руководство. Доступ к настраиваемым параметрам][Практическое руководство. Добавление пользовательских параметров].
...
Полный пример кода и сопутствующее обсуждение можно найти в этой [записи блога](http://blogs.msdn.com/b/jpsanders/archive/2013/03/20/server-script-to-insert-table-items-in-windows-azure-mobile-services.aspx). Если вы используете этот код, можно адаптировать его к конкретной ситуации и тщательно протестировать.
...
- Типы клиентов в зависимости от конкретного языка упорядочиваются в JSON.
- Прежде чем появиться в серверном скрипте, представление JSON преобразуется в JavaScript.
...
При написании серверных скриптов, использующих функции [insert][функция insert], [update][функции update], [read][функция read] или [delete][функция delete], можно получить доступ к представлению данных в JavaScript. Мобильные службы используют функцию десериализации Node.js ([JSON.parse](http://es5.github.io/#x15.12)) для преобразования объектов JSON при передаче в объекты JavaScript. Однако мобильные службы выполняют преобразование для извлечения объектов **Дата** из строк ISO 8601.

Если вы используете [объект таблиц] или [объект mssql][объекта mssql] или просто позволяете выполниться скриптам таблицы, десериализованные объекты JavaScript вставляются в базу данных SQL. В этом процессе свойства объекта сопоставляются с типами T-SQL:
...
Самый простой способ получить данные таблицы из серверных скриптов — использовать прокси [объект таблицы]. Тем не менее существуют более расширенные скрипты, которые не поддерживаются [объектом таблицы][объект таблицы], такие как запросы соединения и другие сложные запросы и вызовы хранимых процедур. В этих случаях необходимо выполнить инструкции Transact-SQL непосредственно в отношении реляционной таблицы, пользуясь [объектом mssql][объекта mssql]. Данный объект предоставляет следующие функции:
...
Успешное выполнение **open** вызывает подключение к базе данных, которое передается в функцию **success** в качестве параметра. Можно вызвать любую из следующих функций на объекте **подключения**: *close*, *queryRaw*, *query*, *beginTransaction*, *commit* и *rollback*.
...
[Практическое руководство. Переопределение удачного завершения и ошибки]: #override-success-error
...
[Практическое руководство. Определение скриптов запланированных заданий]: #scheduler-scripts
...
[Практическое руководство. Доступ к заголовкам запроса настраиваемого интерфейса API]: #get-api-headers
...
[объекта ответа]: http://msdn.microsoft.com/ru-ru/library/windowsazure/dn303373.aspx
...
[объекта таблиц]: http://msdn.microsoft.com/ru-ru/library/windowsazure/jj614364.aspx
