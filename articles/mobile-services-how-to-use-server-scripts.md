...
+ [Настраиваемый интерфейс API][Привязка настраиваемого интерфейса API]
...
+ [Контроль версий, общий код и вспомогательные функции]
...
+ [Insert][функция insert]: `function insert (item, user, request) { ... }`
+ [Update][функция update]: `function update (item, user, request) { ... }`
+ [Delete][функция delete]: `function del (id, user, request) { ... }`
+ [Read][функция read]: `function read (query, user, request) { ... }`

>[AZURE.NOTE]Поскольку удаление (delete) является зарезервированным ключевым словом JavaScript, функция, зарегистрированная для операции удаления, должна иметь имя _del_. 
...
Скрипт операции с таблицей должен вызывать по крайней мере одну из перечисленных ниже функций [Объект request], чтобы гарантировать получение ответа клиентом. 
...
> [AZURE.IMPORTANT] Когда скрипт содержит путь кода, в котором не вызывается ни функция **execute**, ни функция **respond**, операция может перестать отвечать.
...
>[AZURE.NOTE]В скрипте удаления изменение значения предоставляемой переменной userId не влияет на то, какая именно запись удаляется.
...
Обратите внимание, что в **insertItemIfNotComplete** функция **execute** [Объект request] вызывается для вставки элемента при отсутствии повторяющегося текста; в противном случае вызывается функция **respond**, которая уведомляет клиента о повторе. 
...
Следующая функция настраиваемого интерфейса API **OrderPizza** возвращает простой XML-документ как полезные данные ответа:
...
Доступ к конкретному заголовку HTTP можно также получить из [Объект request], как показано в следующем коде:
...
>[AZURE.NOTE]Если система управления версиями включена, можно редактировать файлы скриптов запланированных заданий непосредственно во вложенной папке .\service\scheduler в репозитории git. Дополнительные сведения см. в разделе [Практическое руководство. Совместное использование кода с помощью системы управления версиями]

##<a name="shared-code"></a>Контроль версий, общий код и вспомогательные функции
...
+ **azure**: предоставляет функциональные возможности пакета SDK для Azure для Node.js. Дополнительные сведения см. в разделе [Azure SDK для Node.js]. 
+ **crypto**: обеспечивает функциональные возможности шифрования OpenSSL. Дополнительные сведения см. в разделе [Документация по Node.js][crypto API].
+ **path**: содержит служебные программы для работы с путями файлов. Дополнительные сведения см. в разделе [Документация по Node.js][path API].
+ **querystring**: содержит служебные программы для работы со строками запросов. Дополнительные сведения см. в разделе [Документация по Node.js][querystring API].
...
+ Для модулей, которые опубликованы и установлены с помощью npm, используйте файл package.json, чтобы объявить, какие пакеты должна устанавливать ваша мобильная служба. Таким образом служба всегда имеет доступ к последней версии необходимых пакетов. Файл package.json находится в каталоге `.\service`. Дополнительные сведения см. в разделе [Поддержка package.json в мобильных службах Azure].
...
>[AZURE.NOTE] Модули, которые вы указали в package.json или отправили в свою мобильную службу, используются только в коде серверного скрипта. Эти модули не используются средой выполнения мобильных служб.
...
Наиболее простой способ доступа к таблицам из скрипта - использовать [объект таблиц]. Функция **getTable** возвращает экземпляр [объекта table], который является прокси для доступа к запрашиваемой таблице. Затем можно вызвать функции прокси для доступа к данным и их изменения. 
...
Скрипты настраиваемого API могут получить доступ к [объекту tables] из свойства <strong>service</strong> предоставляемого [Объект request]. Эта строка кода получает [объект таблиц] из запроса:
...
Еще один, последний, пример кода см. в разделе [Практическое руководство. Доступ к пользовательским параметрам][Практическое руководство. Добавление пользовательских параметров].
...
При написании серверных скриптов, использующих функции [insert], [update], [read] или [delete], можно получить доступ к представлению данных в JavaScript. Мобильные службы используют функцию десериализации Node.js ([JSON.parse](http://es5.github.io/#x15.12)) для преобразования объектов JSON в объекты JavaScript во время передачи. Однако мобильные службы выполняют преобразование для извлечения объектов **Date** из строк ISO 8601.
...
Можно объединить две таблицы, используя метод **query** [объекта mssql] для передачи кода TSQL, который реализует соединение. Предположим, у нас есть некоторые элементы в таблице **ToDoItem**, а каждый элемент в таблице имеет свойство **priority**, которое соответствует столбцу в таблице. Элемент может выглядеть следующим образом:
...
[Вызов настраиваемого API из клиента]: #define-custom-api
...
[Принудительная отправка для Windows Phone]: /ru-ru/develop/mobile/tutorials/get-started-with-push-wp8/
