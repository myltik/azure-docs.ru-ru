...
##### Создание контейнера
Большие двоичные объекты хранилища располагаются в контейнерах аналогично тому, как файлы располагаются в папках. Можно добавить ссылку на существующий контейнер с помощью объекта **CloudBlobClient** или создать новый, вызвав метод CreateCloudBlobClient().

В следующем примере кода показано, как создать контейнер хранилища больших двоичных объектов. Код сначала создает объект **BlobClient**, открывающий доступ к функциям объекта, например созданию контейнера хранилища. Затем код ссылается на контейнер с именем mycontainer. Если контейнер с таким именем отсутствует, он будет создан.

**Примечание** Интерфейсы API, выполняющие обращения к хранилищу Azure в ASP.NET 5 являются асинхронными. Для получения дополнительных сведений см. раздел [Асинхронное программирование с помощью Async и Await](http://msdn.microsoft.com/library/hh191443.aspx). В следующем примере кода предполагается, что используются асинхронные методы программирования.
...
По умолчанию новый контейнер закрытый, и вам необходимо указать ключ доступа к хранилищу, чтобы загрузить большие двоичные объекты из этого контейнера. Вы можете сделать файлы в контейнере доступными для всех пользователей, открыв контейнер с помощью следующего кода:
...
Для отправки файла большого двоичного объекта в контейнер получите ссылку на контейнер и используйте ее для получения ссылки на большой двоичный объект. Получив ссылку на большой двоичный объект, вы можете отправить в него любой поток данных с помощью вызова метода **UploadFromStreamAsync()**. Эта операция создает большой двоичный объект, если он не существует, или заменяет его, если он существует. В следующем примере показано, как отправить BLOB-объект в контейнер. Предполагается, что контейнер уже был создан.
...
Есть другие способы вывести список содержимого контейнера больших двоичных объектов. Подробнее см. в статье [Использование хранилища больших двоичных объектов из .NET](http://azure.microsoft.com/ru-ru/documentation/articles/storage-dotnet-how-to-use-blobs/#list-blob).
...
Есть другие способы сохранения больших двоичных объектов в виде файлов. Подробнее см. в статье [Использование хранилища больших двоичных объектов из .NET](http://azure.microsoft.com/ru-ru/documentation/articles/storage-dotnet-how-to-use-blobs/#download-blobs).
...
См. также [Обзор ресурсов хранилища в обозревателе серверов](http://msdn.microsoft.com/ru-ru/library/azure/ff683677.aspx) и [ASP.NET 5](http://www.asp.net/vnext).<!--HONumber=42-->
