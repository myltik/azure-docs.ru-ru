...
2.  Логика основного приложения находится в файле **SamplePlayerViewController.m** в папке `Shared`. В этом файле находится большинство фрагментов кода, описанных ниже.
...
-   Свойство **clipEndMediaTime** представляет время окончания воспроизведения видео. Если **clipEndMediaTime**=100, воспроизведение видео будет завершено на 100-й секунде видеоклипа.
...
-   Для первого клипа **appendTo** равно -1. Когда мы вызываем `[framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex]`, `adIndex` получает уникальное значение, указывающее конец первого ролика в рекламном блоке. Затем для второго ролика в рекламном блоке следует сопоставить начало второго рекламного объявления с концом первого, установив для свойства **appendTo** значение `adpodInfo2.appendTo = adIndex;`, что указывает конечную позицию первого ролика как момент начала воспроизведения второго ролика.
...
### Дополнительные сведения см. в [вики по платформе Azure Media Player][].
  ...
  [вики по платформе Azure Media Player]: https://github.com/WindowsAzure/azure-media-player-framework/wiki
