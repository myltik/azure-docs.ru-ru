...
**Примечание. В поле URL-адреса следует обязательно указать значение.**
...
*Затем платформе отправляется запрос **appendContentClip** для планирования **MediaTime**. В предыдущем примере URL-адрес задается в параметре `[NSURL URLWithString:url]`, а планирование для этого мультимедиа задается с помощью **withMedia**:
 `[framework appendContentClip:[NSURL URLWithString:url] withMediaTime:mediaTime andGetClipId:&clipId])`.

**Примечание.** Основное содержимое всегда планируется до планирования рекламных объявлений (включая рекламу до показа). 
...
Единственное отличие от планирования объявлений до показа состоит в строке `postAdInfo.type = AdTypePostroll;`. В приведенном выше коде воспроизведение 5-секундного рекламного объявления запланировано после показа контента. 
...
* Для первого ролика значение параметра **appendTo** равно -1. При вызове `[framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex]`, `adIndex` получает уникальное значение, указывающее на конец первого клипа в рекламном блоке. Затем для второго ролика в рекламном блоке следует сопоставить начало второго рекламного объявления с концом первого, установив для свойства **appendTo** значение `adpodInfo2.appendTo = adIndex;`, что указывает конечную позицию первого ролика как момент начала воспроизведения второго ролика. 
...
### Дополнительную информацию см. в [вики по платформе Azure Media Player](https://github.com/WindowsAzure/azure-media-player-framework/wiki).
