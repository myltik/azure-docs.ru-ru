...
- Планирование рекламы или вставок до показа.
- Планирование рекламы или вставок после показа.
- Планирование рекламы или вставок во время показа и создание рекламных блоков.
- Управление воспроизведением рекламы или вставок во время показа - при каждой перемотке шкалы воспроизведения содержимого или однократное воспроизведение с удалением со шкалы времени.
- Динамическая вставка содержимого непосредственно в шкалу времени в результате любого события - для этого пользователь может нажать кнопку или получить уведомление от службы через приложение. Например, программа показа новостного содержимого может отправлять уведомления с экстренными новостями, в ответ на что приложение будет приостанавливать воспроизведение основного содержимого для динамической загрузки потока экстренных новостей. 
...
4. В папке iPad имеется два файла .xib: **SeekbarViewController** и **SamplePlayerViewController**. Они создают макет пользовательского интерфейса приложения iPad. Аналогичным образом, в папке iPhone есть два XLB-файла, которые определяют поле поиска и контроллер. 
...
**Примечание. Поле URL-адреса нельзя оставлять незаполненным.**

* Кнопка **Выбор URL-адреса** позволит вам выбрать альтернативные URL-адреса в списке URL-адресов мультимедиа.
...
* Элемент управления **Время проигрывателя** позволяет отображать два времени (Label:playerTime), например 00:23/02:10. В этом случае 00:23 - это текущее время воспроизведения, а 02:10 - общая продолжительность мультимедиа. 

* Сейчас действие **кнопок SkipFroward и SkipBackward** отличается от ожидаемого. В ближайшее время будет выпущена обновленная версия.

* При нажатии кнопки **Запланировать сейчас** во время воспроизведения основного содержимого вы вставите рекламное объявление (URL-адрес объявления определяется в базовом коде). Примечание. В текущей версии невозможно запланировать рекламное объявление во время воспроизведения другого объявления. 

### Планирование основного контента
Запланирован клип с содержимым длительностью от 0 до 80 секунд:
...
* Объект **MediaTime** управляет видеоклипом, который необходимо запланировать в качестве основного содержимого. В предыдущем примере для видеоролика будет запланирована длительность 80 секунд (от 0 секунд до 80 секунд);
* Свойством **clipBeginMediaTime** обозначается время начала воспроизведения видео. Например, если свойство **clipBeginMediaTime**=5, воспроизведение видеоклипа начнется с 5 секунды.
* Свойством **clipEndMediaTime** обозначается время окончания воспроизведения видео. Если свойство **clipEndMediaTime**=100, воспроизведение видео будет завершено на 100-й секунде видеоклипа.
*Затем мы планируем объект **MediaTime**, создав запрос **appendContentClip** к платформе. В предыдущем примере основной URL-адрес задается в параметре [NSURL URLWithString:url], а планирование для этого мультимедиа задается с помощью **withMedia**:
 [framework appendContentClip:[NSURL URLWithString:url] withMediaTime:mediaTime andGetClipId:&clipId]).

**Примечание.** Основное содержимое всегда планируется до рекламных объявлений (в том числе рекламы до показа). 
...
Если сделать это после предшествующего кода, будет запланировано два потока контента на основной временной шкале контента. Первое содержимое планируется на основе значения URLWithString:url, а второе - на основе URLWithString:secondContent. Второй контент начинается с 30 секунд потока мультимедиа и заканчивается на 80 секунде после начала. 

## Планирование рекламных объявлений 
В текущем выпуске поддерживаются только рекламные объявления **pauseTimeline=false**. Это означает, что после окончания рекламы проигрыватель продолжит воспроизведение с момента приостановки основного содержимого. 

Ниже приведены некоторые ключевые моменты:
<ul><li> При планировании рекламного объявления у всех параметров **LinearTime.duration** должно быть значение 0.</li>
<li> Если значение **clipEndMediaTime** превышает длительность рекламного объявления, эта реклама завершится, когда достигнет конца, не породив исключение. Рекомендуется проверить, попадает ли исходная длительность рекламы в рамки времени воспроизведения (**clipEndMediaTime**), чтобы не потерять возможность показа рекламы.</li> 
<li> Поддерживаются рекламные объявления до показа, во время показа или после показа. Объявления до показа можно планировать только на самое начало воспроизведения всего контента. Например, нельзя запланировать объявление до показа для второго контента в сценарии с предварительным монтажом (RCE). </li>
<li> Поддерживаются непрерывные и разовые рекламные объявления. Их можно сочетать с рекламой до показа, во время показа или после показа.</li>
<li> Допускается формат рекламного объявления MP4 или HLS.</li>
</ul>
...
Объект **AdInfo** указывает всю информацию о вашем рекламном клипе:
* Поле **ClipURL** указывает URL-адрес источника клипа.
* Свойство **mediaTime** указывает, как долго будет воспроизводиться реклама. (**clipBeginMediaTime** указывает момент начала рекламного объявления, а значение **clipEndMediaTime** определяет его конец). В предыдущем примере кода запланирована реклама в течение 5 секунд, от 0 до 5-й секунды длительности объявления.
* Объект **Policy** сейчас в платформе не используется.
* Вам необходимо установить для параметра **appendTo** значение -1, если вы планируете не рекламный блок. 
* Значение **type** может указывать объявление до показа, во время показа, после показа или рекламный блок. Тип требуется указать для объявлений до или после показа, так как у них отсутствуют свойства времени. 

####Планирование рекламных объявлений во время показа

Если добавить в предыдущий пример кода строку adLinearTime.startTime=23;, объявление начнет воспроизводиться с 23 секунды по временной шкале основного содержимого.
...
Единственное отличие от планирования объявлений до показа заключается в строке postAdInfo.type = AdType_Postroll;. В приведенном выше коде воспроизведение 5-секундного рекламного объявления запланировано после показа контента. 
...
* Для первого ролика значение параметра **appendTo** равно -1. Когда мы вызываем команду [framework scheduleClip:adpodInfo1 atTime:adLinearTime forType:PlaylistEntryType_Media andGetClipId:&adIndex], adIndex получает уникальное значение, указывающее на конец первого клипа в рекламном блоке. Затем для второго клипа в рекламном блоке следует сопоставить начало второго рекламного объявления с концом первого, установив для свойства **appendTo** значение adpodInfo2.appendTo = adIndex;, что указывает конечную позицию первого ролика как секунду, с которой начнется воспроизведение второго ролика. 
...
Как показано в предыдущем примере, если установить для свойства **deleteAfterPlay** значение **YES**, реклама воспроизведется только один раз. Если же установить для свойства **deleteAfterPlay** значение **NO**, реклама будет воспроизводиться непрерывно (из чего и следует название таких объявлений).
### Дополнительную информацию см. на [странице о платформе Azure Media Player в Википедии](https://github.com/WindowsAzure/azure-media-player-framework/wiki) .
