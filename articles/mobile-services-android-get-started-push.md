...
4. На следующих двух шагах замените _`**my_app_package**`_ в коде именем пакета приложения для вашего проекта, которое является значением атрибута `package` тега `manifest`. 

5. Добавьте после существующего элемента `uses-permission` следующие новые разрешения:
...
6. Добавьте после открывающего тега `application` следующий код: 
...
9.  Откройте файл *ToDoItemActivity.java* и добавьте следующую инструкцию import:
...
13. В обозревателе пакетов щелкните правой кнопкой мыши пакет (в узле `src`), щелкните **Создать** и **Класс**.

14. В поле **Имя** введите `MyHandler`, в поле **Суперкласс** введите `com.microsoft.windowsazure.notifications.NotificationsHandler`, затем нажмите кнопку **Готово**

	![][6]

	При этом создается новый класс MyHandler.

15. Добавьте следующие инструкции import:
...
<!-- URLs. -->
[Консоль Google Cloud]: https://cloud.google.com/console
