...
> Термин «эксперимент» используется для описания интерактивного рабочего процесса, который может включать в себя ввод и манипулирование данными, тренеров и счетчики очков в виде направленного ациклического графа (DAG). Как только рабочий процесс будет опубликован как веб-служба Azure, он больше не является интерактивным. Это означает, что для его изменения модель необходимо обновить, а затем переиздать, чтобы обновить веб-службу и ее поведение.
...
На рис. ниже показаны высокоуровневые шаги, выполняемые на двух этапах: сначала формируется модель, а затем выполняется ее публикация в виде веб-службы. В этом документе подробно рассматривается правая часть рис. 1 (публикация веб-службы оценки) и объясняются концепции, задействуемые в данном процессе.

![][]

Рис. 1. Провизионирование, формирование и публикация веб-службы оценки
...
-   Стадии разработки: Являясь частью рабочего процесса Azure ML, служба RRS сначала формируется в промежуточной среде и может быть там протестирована. Как только она считается полностью сформированной и готовой к производству, данная служба развертывается в производственной среде.
-   Развернуто в Azure: Результатом развертывания службы RRS является конечная точка веб-службы Azure.
-   Параметры интерфейса: Запрос в службу RRS представляет собой данные, которые необходимо оценить с помощью конкретного эксперимента в Studio. Ответ представляет собой результатом прогнозирования модели.
...
-   Стадии разработки: Являясь частью рабочего процесса для создания BES, данная служба сначала формируется в промежуточной среде и может быть там протестирована. Как только она будет полностью сформированной и готовой к производству, данная служба развертывается в производственной среде.
-   Развернуто в Azure: Результатом развертывания службы BES является конечная точка веб-службы Azure.
...
Azure ML Studio предоставляет приложение на основе на браузера, позволяющее легко создавать и выполнять эксперименты машинного обучения в графическом интерфейсе пользователя с помощью различных источников данных, модулей манипулирования данными и проверки достоверности, а также алгоритмов машинного обучения. Эксперимент в ML Studio создается в виде направленного ациклического графа (DAG) модулей обработки данных.

Как только эксперимент будет настроен и успешно выполнен для обучения на основе данных, его можно сохранить в виде обученной модели и использовать для оценки. Затем обученная модель применяется в эксперименте оценки или в рабочем процессе, и публикуется в виде веб-службы Azure.

## Эксперимент обучения

Эксперимент может содержать различные модули для загрузки и манипулирования данными, применения алгоритмов машинного обучения и анализа результатов. Модель обучения использует набор обучающих данных и алгоритм обучения для прогнозирования ответа.
...
После настройки портов и выполнения эксперимента модель можно опубликовать в виде веб-службы. Сначала служба публикуется в промежуточной среде, а затем там тестируется, чтобы убедиться, что она возвращает ожидаемые результаты. После этого, служба считается готовой к производственному развертыванию.
...
Публикация веб-службы в производственной среде позволяет сделать ее доступной для других приложений, которые смогут ее применять для прогнозирования и оценки. После успешного развертывания в промежуточной среде и тестирования веб-служба маркируется для развертывания в производственной среде.
...
После обновления эксперимента, а также создания и оценки новой обученной модели, щелкните «Публикация веб-службы» для публикации службы. Только что созданная служба перезапишет текущую службу.
