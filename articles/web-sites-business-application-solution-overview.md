<properties urlDisplayName="Create a Line-of-Business Application on Azure Websites" pageTitle="Создание бизнес-приложения на веб-сайтах Azure" metaKeywords="Web Sites" description="This guide provides a technical overview of how to use Azure Websites to create intranet, line-of-business applications. This includes authentication strategies, service bus relay, and monitoring." umbracoNaviHide="0" disqusComments="1" editor="mollybos" manager="wpickett" title="Create a Line-of-Business Application on Azure Websites" authors="jroth" />
...
Данное руководство содержит технический обзор использования веб-сайтов Azure для создания бизнес-приложений. Для целей настоящего документа предполагается, что эти приложения являются приложениями интрасети, которые должны быть защищены для внутреннего использования. Существуют две типичных характеристики бизнес-приложений. Эти приложения требуют проверки подлинности, обычно с использованием каталога организации. И для них обычно требуется определенный уровень доступа и интеграции с локальными службами и данными. Это руководство предназначено для создания бизнес-приложений на [веб-сайтах Azure][websitesoverview]. Но существуют ситуации, когда лучше подходят [облачные службы Azure][csoverview] или [виртуальные машины Azure][vmoverview]. Важно разобраться в различиях между этими вариантами, описанными в статье [Веб-сайты Windows Azure, облачные службы и виртуальные машины: Когда же использовать каждую службу?][chooseservice]. 
...
В случае бизнес-приложения выбранная стратегия проверки подлинности является одним из наиболее важных решений. Существует несколько вариантов:

- Использование [службы Azure Active Directory][adoverview]. Эту службу можно использовать в качестве отдельного каталога либо синхронизировать ее с локальной службой Active Directory. В этом случае приложения для проверки подлинности пользователей взаимодействуют со службой Azure Active Directory. Общие сведения об этом подходе см. в статье [Использование Azure Active Directory][adusing].
- Использование для установки Active Directory виртуальных машин и виртуальной сети Azure. Это позволяет распространить в облако локальную установку Active Directory. Можно также использовать службы федерации Active Directory (ADFS) для федеративного направления запросов удостоверений обратно в локальную службу AD. Затем проверка подлинности для приложения Azure выполняется с помощью ADFS в локальной службе Active Directory. Дополнительные сведения об этом подходе см. в статьях [Запуск Windows Server Active Directory на виртуальных машинах][adwithvm] и [Руководства по развертыванию Windows Server Active Directory на виртуальных машинах Azure][addeployguidelines]. 
- Использование промежуточной службы, такой как [служба Azure Access Control Service][acs2] (ACS), чтобы применять несколько служб удостоверений для проверки подлинности пользователей. Это позволяет абстрагировать проверку подлинности либо с помощью Active Directory, либо с помощью другого поставщика удостоверений. Дополнительные сведения см. в статье [Использование Azure Active Directory Access Control][acsusing].

В данном случае бизнес-приложения первый вариант, использующий службу Azure Active Directory, является самым быстрым способом реализовать стратегию проверки подлинности для приложения. Далее в этом руководстве основное внимание уделяется именно Azure Active Directory. Тем не менее, в зависимости от бизнес-требований одно из двух других решений может подойти лучше. Например, если синхронизировать данные удостоверения с облаком нельзя, лучшим вариантом может оказаться решение ADFS. Или же, если необходимо поддерживать других поставщиков удостоверений, например Facebook, лучше подойдет решение ACS.

Перед началом установки новой службы Azure Active Directory следует отметить, что существующие службы, такие как Office 365 и Windows Intune, уже используют Azure Active Directory. В этом случае существующая подписка будет связана с подпиской Azure. Дополнительные сведения см. в статье [Что такое клиент Azure AD?][adtenant].

Если в настоящее время нет службы, уже использующей Azure Active Directory, то можно создать новый каталог на портале управления. Для создания нового каталога пользуйтесь разделом **Active Directory** портала управления.
...
Полное пошаговое руководство для этих действий см. в статье [Добавление единого входа в веб-приложение с помощью Azure AD][adsso]. При использовании этого нового каталога в качестве автономного ресурса следующим шагом является разработка приложений, интегрированных с каталогом. Но при наличии удостоверения локальной службы Active Directory обычно нужно синхронизировать ее с новым каталогом Active Directory Azure. Дополнительные сведения о выполнении соответствующих действий см. в статье [Интеграция каталогов][dirintegration].
...
В сценарии глобального веб-присутствия мы уже обсуждали различные варианты создания и развертывания нового веб-сайта. Если вы впервые работаете с веб-сайтами Azure, будет полезным ознакомиться с [этой информацией][scenarioglobalweb]. Приложение ASP.NET в Visual Studio является типичным вариантом веб-приложения интрасети, использующим проверку подлинности Windows. Одной из причин такого выбора является тесная интеграция и поддержка этого сценария, обеспечиваемая в ASP.NET и Visual Studio.

Например, при создании проекта ASP.NET MVC 4 в Visual Studio можно выбрать вариант **Приложение интрасети** в диалоговом окне создания проекта.

![BusinessApplicationsVSIntranetApp][BusinessApplicationsVSIntranetApp]

В этом случае в настройки проекта будут внесены изменения для поддержки проверки подлинности Windows. В частности, файл web.config содержит для элемента **authentication** атрибут **mode**, равный **Windows**. При создании другого типа проекта ASP.NET, например проекта веб-форм, или при работе с существующим проектом, это изменение необходимо внести вручную.

Для проекта MVC необходимо также изменить два значения в окне свойств проекта. Установить параметр **Проверка подлинности Windows** в значение **Включено**, а параметр **Анонимная проверка подлинности** - в значение **Отключено**.
...
Один из способов - использование [средства идентификации и доступа][identityandaccess] (которое можно загрузить и установить). Это средство интегрируется с Visual Studio в контекстном меню проекта. Следующие инструкции и снимки экрана получены с помощью Visual Studio 2012. Щелкните проект правой кнопкой мыши и выберите **Идентификация и доступ**. Необходимо настроить три момента. На вкладке **Поставщики** необходимо ввести **путь к документу метаданных STS** и **Идентификатор URI приложения** (о том, как получить эти значения, см. в разделе [Регистрация приложений в Azure Active Directory](#registerwaadapp)).

![BusinessApplicationsVSIdentityAndAccess][BusinessApplicationsVSIdentityAndAccess]

Последнее изменение конфигурации, которое необходимо выполнить, вносится на вкладке **Конфигурация** диалогового окна **Идентификация и доступ**. Необходимо установить флажок **Включить файлы cookie веб-фермы**. Подробное руководство об этих действиях см. в статье [Добавление единого входа в веб-приложение с помощью Azure AD][adsso].

####<a name="registerwaadapp"></a>Регистрация приложений в Azure Active Directory:
Чтобы заполнить вкладку **Поставщики**, нужно зарегистрировать приложение с помощью Azure Active Directory. На портале управления Azure выберите в разделе **Active Directory** каталог, а затем перейдите на вкладку **Приложения**. Она позволяет добавить веб-сайт Azure по URL-адресу. Обратите внимание, что при выполнении этих действий для локальной отладки в Visual Studio в качестве URL-адреса сначала указывается адрес localhost. Позже, когда выполняется развертывание, можно заменить этот адрес фактическим URL-адресом веб-сайта.
...
Альтернативный способ выполнить вышеописанные действия - использование [средств Microsoft ASP.NET для Azure Active Directory][aspnettools]. Для этого выберите пункт **Включить проверку подлинности Azure** в меню **Проект** в Visual Studio. Появляется простое диалоговое окно с запросом адреса домена Azure Active Directory (а не URL-адреса приложения).

![BusinessApplicationsVSEnableAuth][BusinessApplicationsVSEnableAuth]

Если вы являетесь администратором этого домена Active Directory, установите флажок **Подготовить это приложение в Azure AD**. Приложение будет зарегистрировано в Active Directory. Если вы не являетесь администратором, снимите этот флажок и сообщите появившиеся сведения администратору. Этот администратор может создать интегрированное приложение с помощью портала управления, используя приведенную выше последовательность действий для средства идентификации и доступа. Инструкции по использованию средств ASP.NET для Azure Active Directory см. в статье [Проверка подлинности Azure][azureauthtutorial].
...
Затем служба Service Bus соединяет облачное приложение с локальной службой WCF. Это обеспечивает базовую архитектуру для создания гибридных приложений, использующих службы и ресурсы как в Azure, так и локальные. Дополнительные сведения см. в статье [Использование службы ретрансляции Service Bus][sbrelayhowto] и [Учебник по ретранслируемому обмену сообщениями Service Bus][sbrelaytutorial]. Пример, демонстрирующий этот способ, см. в статье [Корпоративная пицца - подключение веб-сайтов к локальным ресурсам с помощью Service Bus][enterprisepizza].

##<a name="monitor"></a>Мониторинг приложения
Бизнес-приложения получают преимущества стандартных функциональных возможностей веб-сайтов, таких как масштабирование и отслеживание. Для бизнес-приложения, нагрузка на которое зависит от конкретных дней или часов, может быть полезной функция автомасштабирования (предварительная версия), обеспечивающая масштабирование сайта в соответствии с нагрузкой для эффективного использования ресурсов. В случае мониторинга возможен мониторинг конечных точек и мониторинг квот. Все эти сценарии более подробно рассматриваются в сценариях [Глобальное веб-присутствие][scenarioglobalweb] и [Цифровая маркетинговая кампания][scenariodigitalmarketing].

Требования к мониторингу могут различаться для разных бизнес-приложений, обладающих разной значимостью для бизнеса. Для более важных приложений рассмотрите возможность инвестиций в решение для мониторинга, созданное независимым производителем, такое как [New Relic][newrelic].
...
Различные журналы приложений и сайта помогут в устранении неполадок веб-сайта. Обратите внимание, что для некоторых вариантов выбрано **Файловая система**, что помещает файлы журнала в файловую систему сайта. Для доступа к ним можно использовать FTP, Azure PowerShell и средства командной строки Azure. Для других вариантов выбрано **Хранилище**. В этом случае данные отправляются в указанную учетную запись хранилища Azure. Для параметра **Журналы веб-сервера** также можно указать политику хранения при использовании хранилища или дисковую квоту для файловой системы. Это позволит избежать неограниченного увеличения объема хранимых данных журнала.

![BusinessApplicationsDiagRetention][BusinessApplicationsDiagRetention]

Дополнительные сведения о ведении журнала по этим параметрам см. в разделе [Практическое руководство: Настройка диагностики и загрузка журналов для веб-сайта][configurediagnostics].

Помимо просмотра исходных журналов с помощью FTP или служебных программ хранилища, таких как обозреватель хранилищ Azure, данные журнала также можно просматривать в Visual Studio. Дополнительные сведения об использовании этих журналов в сценариях устранения неполадок см. в статье [Устранение неполадок, связанных с веб-сайтами Azure, в Visual Studio][troubleshootwebsites].
