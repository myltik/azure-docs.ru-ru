...
-   [Практическое руководство. Обновление роли облачной службы или развертывания][Практическое руководство. Обновление роли облачной службы или развертывания]
-   [Практическое руководство. Переключение из промежуточного в рабочее развертывание][Практическое руководство. Переключение из промежуточного в рабочее развертывание]
-   [Практическое руководство. Привязка ресурсов к облачной службе][Практическое руководство. Привязка ресурсов к облачной службе]
-   [Практическое руководство. Удаление развертываний и облачной службы][Практическое руководство. Удаление развертываний и облачной службы]

## <span id="updaterole"></span></a>Практическое руководство. Обновление роли облачной службы или развертывания
...
1.  Войдите на [портал управления Azure][портал управления Azure], откройте панель мониторинга, перейдите на страницу **Облачные службы** или **Экземпляры** и щелкните **Обновить**.

    Откроется окно **Обновление развертывания**.

    ![Обновление развертывания][Обновление развертывания]

2.  В поле **Метка развернутого приложения** введите имя развертывания, например mycloudservicev2. Заданное имя появится в разделе **Быстрый запуск** панели мониторинга.

3.  В поле **Файл пакета** нажмите кнопку **Обзор** и отправьте файл пакета службы (CSPKG).

4.  В поле **Файл конфигурации** нажмите кнопку **Обзор** и отправьте файл конфигурации службы (CSCFG).

5.  В поле **Роль** выберите **Все**, если требуется обновить все роли облачной службы. Чтобы обновить только конкретную роль, выберите ее. Даже если выбрана конкретная роль, обновления в файле конфигурации службы применяются ко всем ролям.

6.  Если в процессе обновления изменяется число или размер ролей, установите флажок **Разрешить обновление при изменении размера или числа ролей**, чтобы обеспечить обновление.
...
## <span id="swap"></span></a>Практическое руководство. Переключение из промежуточного в рабочее развертывание
...
1.  Войдите на [Портал управления][портал управления Azure] и щелкните элемент **Облачные службы**.
...
3.  Щелкните **Переключить**.
    ...
    ![Переключение облачных служб][Переключение облачных служб]
...
## <span id="linkresources"></span></a>Практическое руководство. Привязка ресурсов к облачной службе

Чтобы показать зависимости облачной службы от других ресурсов, вы можете привязать экземпляр базы данных SQL или учетную запись хранения Azure к облачной службе. Для установления и отмены связей с ресурсами используйте страницу **Связанные ресурсы**. Контроль за использованием ресурсов осуществляется на панели мониторинга облачной службы. Если для связанной учетной записи хранения включен мониторинг, вы можете отслеживать общее число запросов на панели мониторинга облачной службы.

Чтобы связать новые или существующие экземпляры базы данных SQL и учетные записи хранения с облачной службой, используйте команду **Связать**. После этого вы сможете масштабировать базу данных со связанной ролью облачной службы на странице **Масштабирование**. В этом случае учетные записи хранения масштабируются автоматически по мере увеличения объемов использования. Дополнительные сведения см. в разделе [Масштабирование облачной службы и связанных ресурсов][Масштабирование облачной службы и связанных ресурсов].

Вы также можете отслеживать, масштабировать базу данных и управлять ею в области **Базы данных** портала управления.
...
2.  Щелкните **Связанные ресурсы**.
    ...
    ![Страница «Связанные ресурсы»][Страница «Связанные ресурсы»]

3.  Щелкните **Привязать ресурс** или **Связать**.

    Откроется мастер **привязки ресурсов**.

    ![Мастер привязки ресурсов, страница 1][Мастер привязки ресурсов, страница 1]

4.  Щелкните **Создать новый ресурс** или **Привязать существующий ресурс**.

5.  Выберите тип связываемого ресурса. Войдите на [портал управления][Портал управления] и щелкните элемент **База данных SQL**. В предварительной версии портала управления не поддерживается привязка учетной записи хранения к облачной службе.
...
6.  Выполните настройку базы данных в соответствии с инструкциями в справке для области **Базы данных SQL** портала управления.
    ...
    ![Ход выполнения связывания][Ход выполнения связывания]

    По завершении этого процесса вы можете проконтролировать состояние связанных ресурсов на панели мониторинга облачной службы. Дополнительные сведения о масштабировании связанной базы данных SQL см. в разделе [Масштабирование облачной службы и связанных ресурсов][Масштабирование облачной службы и связанных ресурсов].
...
## <span id="deletecloudservice"></span></a>Практическое руководство. Удаление развертываний и облачной службы
...
1.  Войдите на [Портал управления][Портал управления] и щелкните элемент **Облачные службы**.
...
> Если для удаляемой облачной службы был настроен подробный мониторинг, платформа Azure не удаляет данные мониторинга из учетной записи хранения. В этом случае требуется удалить такие данные вручную. Дополнительные сведения о поиске таблиц метрик см. в подразделе «Практическое руководство. Доступ к данным подробного мониторинга вне портала управления» раздела [Мониторинг облачных служб][Мониторинг облачных служб].
  ...
  [Масштабирование облачной службы и связанных ресурсов]: ../cloud-services-how-to-scale/
