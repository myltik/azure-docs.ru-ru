<properties linkid="develop-python-queue-service" urlDisplayName="Служба очередей" pageTitle="Использование службы очередей (Python) | Microsoft Azure" metaKeywords="Обмен службы очередей Azure сообщениями с Python" description="Узнайте, как использовать службу очередей Azure для создания и удаления очередей, а также для вставки, получения и удаления сообщений. Примеры написаны на языке Python." metaCanonical="" services="storage" documentationCenter="Python" title="Использование службы хранилища очередей из Python" authors="" solutions="" manager="" editor="" />



# Использование службы хранилища очередей из Python
В этом руководстве показано, как реализовать типичные сценарии с использованием службы
хранилища очередей Azure. Эти примеры написаны с помощью API
Python. Здесь описаны такие сценарии, как **вставка**, **просмотр**,
**получение** и удаление **deleting** сообщений очереди, а также **создание и
...
возвращаемые методом **get\_messages**, становятся невидимыми для другого
...
необходимо вызвать метод **delete\_message**. Этот двухэтапный процесс удаления
...
сообщение и повторить попытку. Код вызывает метод **delete\_message** сразу
...
## <a name="advanced-get"> </a>Практическое руководство. Дополнительные варианты для удаления сообщений из очереди
...
об очереди, а **x-ms-approximate-messages-count** ответа следует использовать в качестве индекса в возвращенном словаре для определения количества.
Результат указывается лишь приблизительно, так как сообщения могут добавляться или
