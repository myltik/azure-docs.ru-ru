<properties urlDisplayName="Queue Service" pageTitle="Использование службы очередей (Node.js) &mdash;  Microsoft Azure" metaKeywords="Azure Queue Service get messages Node.js" description="Вы узнаете, как использовать службы очередей Azure для создания и удаления очередей, вставки, получения и удаления сообщений. Примеры кода написаны на Node.js." metaCanonical="" services="storage" documentationCenter="nodejs" title="Использование службы очередей в Node.js" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
В этом руководстве показано, как реализовать типичные сценарии с использованием службы очередей Windows
Azure. Примеры написаны с помощью
Node.jsAPI. Здесь описаны такие сценарии, как **вставка**, **просмотр**,
...
Следующий пример кода создает объект **QueueService**, который
позволяет работать с очередями.
...
очереди, вызвав метод **peekMessages**. По умолчанию
