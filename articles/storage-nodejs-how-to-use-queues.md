<properties urlDisplayName="Queue Service" pageTitle="Использование службы очередей (Node.js) -  Microsoft Azure" metaKeywords="Azure Queue Service get messages Node.js" description="Learn how to use the Azure Queue service to create and delete queues, and insert, get, and delete messages. Samples written in Node.js." metaCanonical="" services="storage" documentationCenter="nodejs" title="How to Use the Queue Service from Node.js" authors="larryfr" solutions="" manager="wpickett" editor="" />
...
. Здесь описаны такие сценарии, как **вставка**, **просмотр**,
...
именем, если она отсутствует.
	...
	queueSvc.createQueueIfNotExists('myqueue', function(error, result, response){
      if(!error){
        // Очередь создана или существует
	  }
	});

Если очередь создана, `результат` истинный. Если очередь уже существует, `результат` ложный.
...
В Azure SDK для Node.js включены два фильтра, реализующие логику повторных попыток: **ExponentialRetryPolicyFilter** и **LinearRetryPolicyFilter**. Следующий код создает объект **QueueService**, который использует  **ExponentialRetryPolicyFilter**:
...
`результат` содержит сообщение.

> [WACOM.NOTE] Использование **peekMessages** когда в очереди нет сообщений, сообщение об ошибке не отображается, однако сообщения не возвращаются.
...
> [WACOM.NOTE]
> При использовании <b>getMessages</b>, когда в очереди нет сообщений, сообщение об ошибке не отображается, однако сообщения не возвращаются.
    ...
    queueSvc.getMessages('myqueue', {numOfMessages: 15, visibilityTimeout: 5 * 60}, function(error, result, response){
	  if(!error){
		...
		for(var index in result){
		  // текст доступен в[указателе результатов].messageText
...
Метод **getQueueMetadata** возвращает метаданные очереди, в том числе приблизительное количество сообщений, ожидающих в очереди.
    ...
    queueSvc.getQueueMetadata('myqueue', function(error, result, response){
	  if(!error){
		// Длина очереди доступна в result.approximatemessagecount
	  }
	});
...
Надежное приложение, например, облачная служба, создает подпись SAS с помощью **generateSharedAccessSignature** of the **QueueService** и предоставляет ее ненадежному или полунадежному приложению. Например, мобильному приложению. Подпись SAS создается с использованием политики, которая описывает даты начала и окончания срока действия SAS, а также уровень доступа, который предоставляется держателю подписи SAS.
...
В следующем примере создается новая общая политика, которая позволяет держателю подписи SAS добавлять сообщения в очередь в течение 100 минут с момента своего создания.
	...
	var sharedAccessPolicy = {
	  AccessPolicy: {
	    Разрешения: azure.QueueUtilities.SharedAccessPermissions.ADD,
	    Начало: startDate,
...
ACL реализуется с помощью массива политик доступа, каждая из которых связана со своим идентификатором. В  следующем примере определяются две политики - для пользователей user1 и user2:
	...
	var sharedAccessPolicy = [
	  {
	    AccessPolicy: {
	      Разрешения: azure.QueueUtilities.SharedAccessPermissions.PROCESS,
	      Начало: startDate,
	      Срок действия: expiryDate
	    },
	    ИД: 'user1'
