...
API documentation" pageTitle="Рекомендации в машинном обучении Azure - 
документация по API" description="Рекомендации обучения Azure машины - документация по API" metaKeywords="" services="machine-learning" solutions="" documentationCenter="" authors="AharonGumnik" manager="paulettm" editor="cgronlun" videoId="" scriptId="" />
...
#Рекомендации в машинном обучении Azure - <br>документация по API

В этом документе описываются интерфейсы API механизма рекомендаций в машинном обучении Azure.

##Содержание

- [1. Общие сведения](#1-general-overview)
- [2. Ограничения](#2-limitations)
- [3. Интерфейсы API: общие сведения](#3-apis--general-information)
	- [3.1. Проверка подлинности](#31-authentication)
	- [3.2. URI служб](#32-service-uri)
	- [3.3. Версия API](#33-api-version)
- [4. Базовая обработка модели](#4-model-basic)
	- [4.1. Создание модели](#41-create-a-model)
	- [4.2. Получение модели](#42-get-model)
	- [4.3. Получение всех моделей](#43-get-all-models)
	- [4.4. Обновление модели](#44-update-model)
	- [4.5. Удаление модели](#45-delete-model)
- [5. Расширенная обработка модели](#5-model-advanced)
	- [5.1. Подробные сведения о данных модели](#51-model-data-insight)
	- [5.2. Подробные сведения о модели](#52-model-insight)
	- [5.3. Получение образца модели](#53-get-model-sample)
- [6. Бизнес-правила модели](#6-model-business-rules)
	- [6.1. Получение правил для модели](#61-get-model-rules)
	- [6.2. Добавление правила](#62-add-rule)
	- [6.3. Удаление правила](#63-delete-rule)
	- [6.4. Удаление всех правил](#64-delete-all-rules)
- [7. Каталог](#7-catalog)
	- [7.1. Импорт данных каталога](#71-import-catalog-data)
	- [7.2. Получение каталога](#72-get-catalog)
	- [7.3. Получение элементов каталога по маркеру](#73-get-catalog-items-by-token)
- [8. Данные об использовании](#8-usage-data)
	- [8.1. Импорт данных об использовании](#81-import-usage-data)
		- [8.1.1. Передача файла](#811-uploading-file)
		- [8.1.2. Использование системы сбора данных](#812-using-data-acquisition)
	- [8.2. Вывод списка файлов использования модели](#82-list-model-usage-files)
	- [8.3. Получение статистики использования](#83-get-usage-statistics)
	- [8.4. Получение образца файла использования](#84-get-usage-file-sample)
	- [8.5. Получение файла использования модели](#85-get-model-usage-file)
	- [8.6. Удаление файла использования](#86-delete-usage-file)
	- [8.7. Удаление всех файлов использования](#87-delete-all-usage-files)
- [9. Сборка](#9-build)
	- [9.1. Сборка модели](#91-build-a-model)
	- [9.2. Получение статусов сборок модели](#92-get-builds-status-of-a-model)
	- [9.3. Получение статусов сборок пользователя](#93-get-builds-status-of-a-user)
	- [9.4. Удаление сборки](#94-delete-build)
	- [9.5. Отмена сборки](#95-cancel-build)
	- [9.6. Получение параметров сборки](#96-get-build-parameters)
- [10. Рекомендации](#10-recommendation)
	- [10.1. Получение рекомендаций](#101-get-recommendations)
- [11. Уведомления](#11-notifications)
	- [11.1. Получение уведомлений](#111-get-notifications)
	- [11.2. Удаление уведомлений модели](#112-delete-model-notifications)
	- [11.3. Удаление уведомлений пользователя](#113-delete-user-notifications)
- [12. Юридическая информация](#12-legal)
...
1.	<ins>Базовая обработка модели</ins> - API-интерфейсы, позволяющие выполнять основные операции с моделью, например создание, обновление или удаление модели.
2.	<ins>Расширенная обработка модели</ins> - API-интерфейсы, позволяющие получать подробные сведения о данных модели.
3.	<ins>Бизнес-правила модели</ins> - API-интерфейсы, позволяющие управлять бизнес-правилами в отношении результатов рекомендаций модели.
4.	<ins>Каталог</ins> - API-интерфейсы, позволяющие выполнять базовые операции с каталогом модели. Каталог содержит метаданные по элементам данных об использовании.
5.	<ins>Данные об использовании</ins> - API-интерфейсы, позволяющие выполнять основные операции с данными об использовании модели. Данные об использовании в основном виде состоят из строк, содержащих пары <userId>,<itemId>.
...
* Максимальное количество моделей для каждой подписки: 10
...
* Максимальный объем данных, которые можно передать в POST (например, при импорте данных каталога или импорте данных об использовании): 200 МБ.
* Скорость транзакций для сборки модели рекомендаций, которая неактивна, составляет около 2 транзакций в секунду. Только активные сборки модели рекомендаций могут поддерживать до 20 транзакций в секунду.
...
###3.2. URI служб
Корневые URI служб для каждого API рекомендаций машинного обучения Azure можно найти [здесь.](https://api.datamarket.azure.com/amla/recommendations/v1/)

Полный URI-код службы выражается с помощью элементов спецификации OData.  

###3.3. Версия API
В конце запроса каждого вызова API содержится параметр с именем apiVersion, для которого должно быть установлено значение 1.0

##4. Базовая обработка модели

###4.1. Создание модели
Запрос создания модели:

| Метод HTTP | URI |
|:--------|:--------|
|POST     |`<rootURI>/CreateModel?modelName=%27<model_name>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/CreateModel?modelName=%27MyFirstModel%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetModel?id=%27<model_id>%27&apiVersion=%271.0%27`<br>Пример:<br>`<rootURI>/GetModel?id=%271cac7b76-def4-41f1-bc81-29b806adb1de%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|DELETE     |`<rootURI>/GetAllModels?apiVersion=%271.0%27`<br>Пример:<br>`<rootURI>/GetAllModels?apiVersion=%271.0%27`|
...
Можно обновить описание модели или активный идентификатор сборки.<br>
<ins>Активный идентификатор сборки</ins> У каждой сборки каждой модели есть идентификатор сборки. Активный идентификатор сборки - это первая успешная сборка каждой новой модели. Если у вас уже есть активный идентификатор сборки, но вы делаете дополнительные сборки для той же модели, при желании его можно задать как идентификатор сборки по умолчанию. Если при получении рекомендаций не указать, какой идентификатор сборки нужно использовать, автоматически будет использоваться идентификатор по умолчанию.<br>
Этот механизм позволяет, имея рабочую модель рекомендаций, собирать новые модели и тестировать их перед использованием.


| Метод HTTP | URI |
|:--------|:--------|
|PUT     |`<rootURI>/UpdateModel?id=%27<modelId>%27&apiVersion=%271.0%27`<br>Пример:<br>`<rootURI>/UpdateModel?id=%279559872f-7a53-4076-a3c7-19d9385c1265%27&apiVersion=%271.0%27`|
...
|	apiVersion		| 1.0 |
|||
| Текст запроса | `<ModelUpdateParams>`<br>`<Description>Новое описание</Description>`<br>`<ActiveBuildId>-1</ActiveBuildId>`<br>` </ModelUpdateParams>`<br><br>Обратите внимание: XML-теги Description и ActiveBuildId необязательны. Если вы не хотите задавать эти параметры, удалите весь тег.|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetDataInsight?modelId=%27<model_id>%27&apiVersion=%271.0%27`<br>Пример:<br>`<rootURI>/GetDataInsight?modelId=%271cac7b76-def4-41f1-bc81-29b806adb1de%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetModelSample?modelId=%27<model_id>%27&apiVersion=%271.0%27`<br>Пример:<br>`<rootURI>/GetModelSample?modelId=%271cac7b76-def4-41f1-bc81-29b806adb1de%27&apiVersion=%271.0%27`|
...
|	apiVersion		| 1.0 |
|||
| Текст запроса | <ins>Для добавления правила списка блокировок (BlockList):</ins><br>`<ApiFilter><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>BlockList</Type><Value>{"ItemsToExclude":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`<br><br><ins>Для добавления правила повышения продаж (Upsale):</ins>[US1]<br>`<ApiFilter><ModelId>24024f7e-b45c-419e-bfa2-dfd947e0d253</ModelId><Type>Upsale</Type><Value>{"ItemsToUpsale":["2406E770-769C-4189-89DE-1C9283F93A96"]}</Value></ApiFilter>`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/DeleteRule?modelId=%27<model_id>%27&filterId=%27<filter_Id>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`DeleteRule?modelId=%2724024f7e-b45c-419e-bfa2-dfd947e0d253%27&filterId=%271000011%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetCatalog?modelId=%27<model_id>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`GetCatalog?modelId=%2724024f7e-b45c-419e-bfa2-dfd947e0d253%27&apiVersion=%271.0%27`|
...
###7.3.	Получение элементов каталога по маркеру

| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetCatalogItemsByToken?modelId=%27<modelId>%27&token=%27<token>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`GetCatalogItemsByToken?modelId=%270dbb55fa-7f11-418d-8537-8ff2d9d1d9c6%27&token=%27Cla%27&apiVersion=%271.0%27`|

|	Имя параметра 	|	Допустимые значения 	|
|:--------			|:--------								|
|	modelId	|	Уникальный идентификатор модели. |
|	token	|	Маркер имени элемента каталога. Должен содержать не менее трех символов. |
|	apiVersion		| 1.0 |
|||
| Текст запроса | НЕТ |
...
####8.1.1. Передача файла
В этом разделе показано, как передавать данные об использовании с помощью файла. Этот API можно вызывать несколько раз с данными об использовании. Для всех вызовов сохранятся все данные об использовании.

| Метод HTTP | URI |
|:--------|:--------|
|POST     |`<rootURI>/ImportUsageFile?modelId=%27<modelId>%27&filename=%27<fileName>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/ImportUsageFile?modelId=%27a658c626-2baa-43a7-ac98-f6ee26120a12%27&filename=%27ImplicitMatrix10_Guid_small.txt%27&apiVersion=%271.0%27`|
...
####8.1.2. Использование системы сбора данных
В этом разделе показано, как отправлять события в модуль рекомендаций машинного обучения Microsoft Azure в режиме реального времени (как правило, со своего веб-сайта).
...
Тело запроса
	...
	Ввод данных события для каждого события, которое нужно отправить. Для одного и того же пользователя или сеанса браузера необходимо всегда отправлять один и тот же идентификатор в поле SessionId.
    ...
    <EventData>
      <Name>Click</Name>
      <ItemId>21BF8088-B6C0-4509-870C-E1C7AC78304A</ItemId>
      <ItemName>название_товара</ItemName>
      <ItemDescription>описание_товара</ItemDescription>
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/ListModelUsageFiles?forModelId=%27<model_id>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/ListModelUsageFiles?forModelId=%270dbb55fa-7f11-418d-8537-8ff2d9d1d9c6%27&apiVersion=%271.0%27`|
...
| eventTypes |	Строка разделенных запятыми типов событий или нуль для получения всех событий.  |
| apiVersion | 1.0 |
|||
| Текст запроса | НЕТ |
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetUsageFileSample?modelId=%27<modelId>%27& fileId=%27<fileId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/GetUsageFileSample?modelId=%271c1110f8-7d9f-4c64-a807-4c9c5329993a%27&fileId=%274c067b42-e975-4cb2-8c98-a6ab80ed6d63%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetModelUsageFile?mid=%27<modelId>%27& fid=%27<fileId>%27&download=%27<download_value>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/GetModelUsageFile?mid=%271c1110f8-7d9f-4c64-a807-4c9c5329993a%27&fid=%273126d816-4e80-4248-8339-1ebbdb9d544d%27&download=%271%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|DELETE     |`<rootURI>/DeleteUsageFile?modelId=%27<modelId>%27&fileId=%27<fileId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/DeleteUsageFile?modelId=%270f86d698-d0f4-4406-a684-d13d22c47a73%27&fileId=%27f2e0b09d-be5c-46b2-9ac2-c7f622e5e1a5%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|DELETE     |`<rootURI>/DeleteAllUsageFiles?modelId=%27<modelId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/DeleteAllUsageFiles?modelId=%271c1110f8-7d9f-4c64-a807-4c9c5329993a%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|POST     |`<rootURI>/BuildModel?modelId=%27<modelId>%27&userDescription=%27<description>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/BuildModel?modelId=%27a658c626-2baa-43a7-ac98-f6ee26120a12%27&userDescription=%27First%20build%27&apiVersion=%271.0%27`|

|	Имя параметра 	|	Допустимые значения 	|
|:--------			|:--------								|
| modelId |	Уникальный идентификатор модели.  |
| userDescription | Текстовый идентификатор каталога. Обратите внимание: при использовании пробелов их необходимо заменить комбинацией символов %20 (см. примеры выше).<br>Максимальная длина: 50 |
| apiVersion | 1.0 |
|||
| Текст запроса | Если оставить это поле пустым, сборка будет выполняться с параметрами по умолчанию<br><br>Если требуется задать параметры сборки, отправьте следующий XML-код в тексте запроса (значение параметров см. в разделе "Получение параметров сборки"):<br>`<BuildParametersList><NumberOfModelIterations>40</NumberOfModelIterations><NumberOfModelDimensions>20</NumberOfModelDimensions><UseFeaturesInModel>false</UseFeaturesInModel><AllowColdItemPlacement>false</AllowColdItemPlacement><MinItemAppearance>5</MinItemAppearance><MinUserAppearance>5</MinUserAppearance><EnableFeatureCorrelation>false</EnableFeatureCorrelation><RefreshFeatureScoreOnBuild>false</RefreshFeatureScoreOnBuild><ComputeUpd>false</ComputeUpd><EnableModelingInsights>true</EnableModelingInsights><ModelingFeatureList /><ReasoningFeatureList /></BuildParametersList>`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetModelBuildsStatus?modelId=%27<modelId>%27&onlyLastBuild=<bool>&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/GetModelBuildsStatus?modelId=%279559872f-7a53-4076-a3c7-19d9385c1265%27&onlyLastBuild=true&apiVersion=%271.0%27`|


|	Имя параметра 	|	Допустимые значения 	|
|:--------			|:--------								|
|	modelId			|	Уникальный идентификатор модели.	|
|	onlyLastBuild	|	Указывает, возвращать ли весь журнал сборок модели или только статус последней сборки.	|
|	apiVersion		|	1.0									|
...
OData XML
	...
	Ответ включает отдельную запись для каждой сборки, и каждая запись содержит такие данные:
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetUserBuildsStatus?onlyLastBuilds=<bool>&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/GetUserBuildsStatus?onlyLastBuilds=true&apiVersion=%271.0%27`|
...
ПРИМЕЧАНИЕ. Активные сборки не удаляются. Перед удалением активной сборки необходимо обновить модель до другой активной сборки.

| Метод HTTP | URI |
|:--------|:--------|
|DELETE     |`<rootURI>/DeleteBuild?buildId=%27<buildId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/DeleteBuild?buildId=%271500068%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|PUT     |`<rootURI>/CancelBuild?buildId=%27<buildId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/CancelBuild?buildId=%271500076%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/GetBuildParameters?buildId=%27<buildId>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/GetBuildParameters?buildId=%271500068%27&apiVersion=%271.0%27`|
...
| Метод HTTP | URI |
|:--------|:--------|
|GET     |`<rootURI>/ItemRecommend?modelId=%27<modelId>%27&itemIds=%27<itemId>%27&numberOfResults=<int>&includeMetadata=<bool>&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/ItemRecommend?modelId=%272779c063-48fb-46c1-bae3-74acddc8c1d1%27&itemIds=%271003%27&numberOfResults=10&includeMetadata=false&apiVersion=%271.0%27`|

|	Имя параметра 	|	Допустимые значения 	|
|:--------			|:--------								|
| modelId | Уникальный идентификатор модели. |
| itemIds | Разделенный запятыми список элементов, для которых предоставляются рекомендации.<br>Максимальная длина: 200 |
| numberOfResults | Количество необходимых результатов. |
| includeMetatadata | Для будущего устанавливается значение false. |
| apiVersion | 1.0 |
...
| Метод HTTP | URI |
|:--------|:--------|
|DELETE     |`<rootURI>/DeleteModelNotifications?modelId=%<model_id>%27&apiVersion=%271.0%27`<br><br>Пример:<br>`<rootURI>/DeleteModelNotifications?modelId=%27967136e8-f868-4258-9331-10d567f87fae%27&apiVersion=%271.0%27`|
...
Данный документ предоставляется "как есть". Сведения и мнения, представленные в данном документе, включая URL-адреса и ссылки на другие веб-сайты, могут быть изменены без предварительного уведомления.<br><br>
...
Настоящий документ не предоставляет юридических прав на интеллектуальную собственность в отношении продуктов корпорации Майкрософт. Разрешается копирование и использование настоящего документа в справочных целях.<br><br>
(c) Корпорация Майкрософт 2014 г. Все права защищены.

[US1]There's a mistake in the source segment. It duplicates the previous one, while the code string that illustrates the segment is about Upsale rule. 
