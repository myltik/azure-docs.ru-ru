...
-    Все интерфейсы REST API теперь вместо **бета-**-версии доступны в версии **/v1.0**.
-    Пакеты SDK для .NET и JavaScript обновлены до версии **v1.0**.
-    Проверить подлинность вызовов интерфейсов API Power BI теперь можно напрямую с помощью ключей API. Маркеры приложений необходимы только для внедрения. Маркеры подготовки и разработки теперь не рекомендуется использовать в интерфейсах API 1.0, но они будут и дальше работать в бета-версии до 30.12.2016. Дополнительные сведения см. в статье [Сведения о потоке маркеров приложений в Power BI Embedded](power-bi-embedded-app-token-flow.md).
-    Поддержка безопасности на уровне строк для маркеров приложений и внедренных отчетов. Дополнительные сведения см. в статье [Row level security with Power BI Embedded](power-bi-embedded-rls.md) (Безопасность на уровне строк в Power BI Embedded).
