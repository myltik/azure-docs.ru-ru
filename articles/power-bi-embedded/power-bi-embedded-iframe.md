...
> [AZURE.NOTE] Также можно подготовить коллекцию рабочих областей и получить ключ доступа через REST API. Дополнительные сведения см. в статье [Power BI Resource Provider APIs](https://msdn.microsoft.com/library/azure/mt712306.aspx) (Интерфейсы Power BI API для поставщика ресурсов).
...
> [AZURE.NOTE] Если мы хотим использовать безопасность на уровне строк (RLS) со службой Power BI Embedded, нам необходимо также указать в утверждениях **имя пользователя** и **роли**.
