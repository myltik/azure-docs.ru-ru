...
Управление доступом в Azure начинается с оплаты. Владелец учетной записи Azure, доступ к которой осуществляется через [центр учетных записей Azure](https://account.windowsazure.com/subscriptions), является администратором учетной записи (АУ). Подписки — это контейнеры для выставления счетов, которые в то же время обеспечивают защиту: у каждой подписки есть администратор службы, который может добавлять, удалять и изменять ресурсы Azure в этой подписке на портале Azure. По умолчанию АУ новой подписки становится и ее АС, однако АУ может изменить АС в центре учетных записей Azure.
