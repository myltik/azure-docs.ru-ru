...
- Большие приложения, такие как SAP, MongoDB или Apache Cassandra, установленные и выполненные в ОС перед шифрованием. Расширению не удается правильно завершить работу этих приложений. Если в приложениях хранятся открытые дескрипторы файлов на диске операционной системы, то диск невозможно отключить, что приводит к сбою.
...
По умолчанию компонент **bdehdcfg** недоступен в основных серверных компонентах Windows Server 2016. Этот компонент требуется для шифрования дисков Azure. Добавить компонент **bdehdcfg** можно так:

   1. Скопируйте следующие 4 файла из виртуальной машины центра обработки данных Windows Server 2016 в папку **c:\windows\system32** образа основных серверных компонентов:
