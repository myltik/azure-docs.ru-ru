...
- Когда большие приложения, такие как SAP, MongoDB или Apache Cassandra, устанавливаются или выполняются в ОС перед шифрованием.  Расширению не удается правильно завершить их работу, и если они обслуживают открытые дескрипторы файлов на диске операционной системы, то диск нельзя отключить, что приведет к отказу.
- Когда пользовательские скрипты выполняются практически одновременно с включением шифрования или при внесении каких-либо других изменений на виртуальной машине во время процесса шифрования.   Это может произойти, если шаблон Resource Manager определяет несколько расширений для одновременного выполнения или когда пользовательское расширение скрипта или другое действие выполняется одновременно с шифрованием диска.   Сериализация и изоляция таких шагов может решить проблему.
...
В некоторых случаях шифрование диска зависает на этапе "OS disk encryption started" (Начато шифрование диска ОС) и SSH отключается. Процесс может занять от 3 до 16 часов и может требовать еще больше времени на выполнение.  В рамках шифрования диска ОС Linux диск временно отключается и выполняется поблочное шифрование всего диска перед переподключением его в зашифрованном состоянии.   В отличие от шифрования дисков Azure в Windows во время шифрования диска в Linux нельзя использовать виртуальную машину.  Характеристики производительности виртуальной машины, включая размер диска и хранилище, связанное с учетной записью хранения (класса Standard или Premium (SSD)), могут оказать значительное влияние на время, необходимое для выполнения шифрования.
