...
| Azure DocumentDB | <ul><li>[Для подключения к Document DB по возможности используйте маркеры ресурсов](#resource-docdb)</li></ul> |
...
| Устройства Интернета вещей | <ul><li>[Выполняйте проверки авторизации на устройстве, если оно поддерживает действия, для которых требуются разрешения различных уровней](#device-permission)</li></ul> |
...
## <a name="a-idacl-restricted-accessaensure-that-proper-acls-are-configured-to-restrict-unauthorized-access-to-data-on-the-device"></a><a id="acl-restricted-access"></a>Соответствующим образом настройте списки ACL для предотвращения несанкционированного доступа к данным на устройстве
...
## <a name="a-idsensitive-directoryaensure-that-sensitive-user-specific-application-content-is-stored-in-user-profile-directory"></a><a id="sensitive-directory"></a>Конфиденциальное пользовательское содержимое приложения должно храниться в каталоге профиля пользователя
...
## <a name="a-iddeployed-privilegesaensure-that-the-deployed-applications-are-run-with-least-privileges"></a><a id="deployed-privileges"></a>Развернутые приложения следует запускать с минимальным числом привилегий
...
## <a name="a-idsequential-logicaenforce-sequential-step-order-when-processing-business-logic-flows"></a><a id="sequential-logic"></a>Обеспечьте последовательный порядок обработки потоков бизнес-логики
...
## <a name="a-idrate-enumerationaimplement-rate-limiting-mechanism-to-prevent-enumeration"></a><a id="rate-enumeration"></a>Реализуйте ограничение частоты транзакций, чтобы предотвратить перечисление
...
## <a name="a-idprinciple-least-privilegeaensure-that-proper-authorization-is-in-place-and-principle-of-least-privileges-is-followed"></a><a id="principle-least-privilege"></a>Обеспечьте применение соответствующего типа авторизации и соблюдение принципа наименьших необходимых привилегий
...
## <a name="a-idenumerable-browsingaensure-that-content-and-resources-are-not-enumerable-or-accessible-via-forceful-browsing"></a><a id="enumerable-browsing"></a>Убедитесь, что содержимое и ресурсы не могут быть перечислены или доступны путем несанкционированного просмотра
...
## <a name="a-idprivileged-serveraensure-that-least-privileged-accounts-are-used-to-connect-to-database-server"></a><a id="privileged-server"></a>Убедитесь, что для подключения к серверу базы данных используются учетные записи с минимальными привилегиями доступа
...
## <a name="a-idrls-tenantsaimplement-row-level-security-rls-to-prevent-tenants-from-accessing-each-others-data"></a><a id="rls-tenants"></a>Реализуйте безопасность на уровне строк (RLS), чтобы запретить клиентам доступ к данным друг друга
...
## <a name="a-idsysadmin-usersasysadmin-role-should-only-have-valid-necessary-users"></a><a id="sysadmin-users"></a>Роль системного администратора должна быть только у необходимых действительных пользователей
...
## <a name="a-idcloud-least-privilegedaconnect-to-cloud-gateway-using-least-privileged-tokens"></a><a id="cloud-least-privileged"></a>Подключайтесь к облачному шлюзу с помощью маркеров с минимальными привилегиями доступа
...
| Ссылки              | [Руководство разработчика по Центру Интернета вещей Azure](https://azure.microsoft.com/documentation/articles/iot-hub-devguide/#Security) |
...
## <a name="a-idsendonly-sasause-a-send-only-permissions-sas-key-for-generating-device-tokens"></a><a id="sendonly-sas"></a>Для создания маркеров устройства используйте ключ SAS с разрешениями только для отправки
...
## <a name="a-idaccess-tokens-hubado-not-use-access-tokens-that-provide-direct-access-to-the-event-hub"></a><a id="access-tokens-hub"></a>Не используйте маркеры доступа, которые предоставляют прямой доступ к концентратору событий
...
## <a name="a-idsas-minimum-permissionsaconnect-to-event-hub-using-sas-keys-that-have-the-minimum-permissions-required"></a><a id="sas-minimum-permissions"></a>Подключайтесь к концентратору событий, используя ключи SAS с минимальными необходимыми разрешениями
...
## <a name="a-idresource-docdbause-resource-tokens-to-connect-to-documentdb-whenever-possible"></a><a id="resource-docdb"></a>Для подключения к Document DB по возможности используйте маркеры ресурсов
...
| Атрибуты              | Недоступно  |
| Ссылки              | Недоступно  |
| Действия | Маркер ресурса связан с разрешением DocumentDB и отражает отношение между пользователем базы данных и разрешением этого пользователя к конкретному ресурсу приложения DocumentDB (например, коллекции, документу). Всегда используйте маркер ресурса для доступа к DocumentDB, если клиенту (приложению пользователя, например клиенту мобильного или настольного приложения) нельзя доверить обработку главных ключей или ключей только для чтения. Используйте главный ключ или ключи только для чтения серверных приложений, которые могут хранить их безопасно.|

## <a name="a-idgrained-rbacaenable-fine-grained-access-management-to-azure-subscription-using-rbac"></a><a id="grained-rbac"></a>Обеспечьте детальное управление доступом к подписке Azure с помощью RBAC
...
## <a name="a-idcluster-rbacarestrict-clients-access-to-cluster-operations-using-rbac"></a><a id="cluster-rbac"></a>С помощью RBAC ограничьте доступ клиента к операциям кластера
...
## <a name="a-idmodeling-fieldaperform-security-modeling-and-use-field-level-security-where-required"></a><a id="modeling-field"></a>Выполняйте моделирование безопасности, при необходимости применяя безопасность на уровне полей
...
## <a name="a-idpermission-entitiesagrant-fine-grained-permission-on-a-range-of-entities-in-azure-table-storage"></a><a id="permission-entities"></a>Предоставьте детальное разрешение диапазону сущностей в хранилище таблиц Azure
...
## <a name="a-idrbac-azure-manageraenable-role-based-access-control-rbac-to-azure-storage-account-using-azure-resource-manager"></a><a id="rbac-azure-manager"></a>С помощью Azure Resource Manager включите для учетной записи хранения Azure управление доступом на основе ролей (RBAC)
...
## <a name="a-idrooting-detectionaimplement-implicit-jailbreak-or-rooting-detection"></a><a id="rooting-detection"></a>Реализуйте неявное обнаружение снятия защиты или получение административного доступа
...
## <a name="a-idweak-class-wcfaweak-class-reference-in-wcf"></a><a id="weak-class-wcf"></a>Слабый механизм ссылки на класс в WCF
...
## <a name="a-idwcf-authzawcf-implement-authorization-control"></a><a id="wcf-authz"></a>Реализуйте контроль авторизации WCF
...
## <a name="a-idauthz-aspnetaimplement-proper-authorization-mechanism-in-aspnet-web-api"></a><a id="authz-aspnet"></a>Реализуйте соответствующий механизм авторизации в веб-API ASP.NET
...
| Ссылки              | [Проверка подлинности и авторизация в веб-API ASP.NET](http://www.asp.net/web-api/overview/security/authentication-and-authorization-in-aspnet-web-api) |
...
## <a name="a-iddevice-permissionaperform-authorization-checks-in-the-device-if-it-supports-various-actions-that-require-different-permission-levels"></a><a id="device-permission"></a>Выполняйте проверки авторизации на устройстве, если оно поддерживает действия, для которых требуются разрешения различных уровней
...
## <a name="a-idfield-permissionaperform-authorization-checks-in-the-field-gateway-if-it-supports-various-actions-that-require-different-permission-levels"></a><a id="field-permission"></a>Выполняйте проверки авторизации в полевом шлюзе, если он поддерживает действия, для которых требуются разрешения различных уровней
