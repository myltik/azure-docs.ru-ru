...
| Azure Cosmos DB | <ul><li>[Использование маркеров ресурса (по возможности) для подключения к Cosmos DB](#resource-docdb)</li></ul> |
...
| Атрибуты              | Недоступно  |
| Ссылки              | Недоступно  |
| Действия | Маркер ресурса связан с разрешением Cosmos DB и отражает отношение между пользователем базы данных и разрешением этого пользователя к конкретному ресурсу приложения Cosmos DB (например, коллекции, документу). Всегда используйте маркер ресурса для доступа к Cosmos DB, если клиенту (приложению пользователя, например клиенту мобильного или настольного приложения) нельзя доверить обработку главных ключей или ключей только для чтения. Используйте главный ключ или ключи только для чтения серверных приложений, которые могут хранить их безопасно.|
