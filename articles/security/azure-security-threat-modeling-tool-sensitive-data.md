...
| --------------- | ------- |
| Отношения доверия между компьютерами | <ul><li>[Двоичные файлы, содержащие конфиденциальные данные, должны быть замаскированы](#binaries-info)</li><li>[Используйте шифрование на уровне файловой системы (EFS) для защиты конфиденциальных данных пользователя](#efs-user)</li><li>[Настройте шифрование конфиденциальных данных, сохраняемых приложением в файловой системе](#filesystem)</li></ul> | 
...
| Границы доверия виртуальной машины IaaS Azure | <ul><li>[Шифруйте диски, используемые виртуальными машинами, с помощью шифрования дисков Azure](#disk-vm)</li></ul> | 
...
## <a name="a-idbinaries-infoaensure-that-binaries-are-obfuscated-if-they-contain-sensitive-information"></a><a id="binaries-info"></a>Двоичные файлы, содержащие конфиденциальные данные, должны быть замаскированы
...
## <a name="a-idefs-useraconsider-using-encrypted-file-system-efs-is-used-to-protect-confidential-user-specific-data"></a><a id="efs-user"></a>Используйте шифрование на уровне файловой системы (EFS) для защиты конфиденциальных данных пользователя
...
## <a name="a-idfilesystemaensure-that-sensitive-data-stored-by-the-application-on-the-file-system-is-encrypted"></a><a id="filesystem"></a>Настройте шифрование конфиденциальных данных, сохраняемых приложением в файловой системе
...
## <a name="a-idcache-browseraensure-that-sensitive-content-is-not-cached-on-the-browser"></a><a id="cache-browser"></a>Конфиденциальное содержимое не должно кэшироваться в браузере
...
## <a name="a-idencrypt-dataaencrypt-sections-of-web-apps-configuration-files-that-contain-sensitive-data"></a><a id="encrypt-data"></a>Зашифруйте разделы файлов конфигурации веб-приложения, содержащие конфиденциальные данные
...
## <a name="a-idautocomplete-inputaexplicitly-disable-the-autocomplete-html-attribute-in-sensitive-forms-and-inputs"></a><a id="autocomplete-input"></a>Явным образом отключите автозаполнение атрибута HTML в конфиденциальных формах и входных данных
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [Сведения об атрибуте autocomplete на сайте MSDN](http://msdn.microsoft.com/library/ms533486(VS.85).aspx), [использование функции автозаполнение в формах HTML](http://msdn.microsoft.com/library/ms533032.aspx), [Бюллетень по безопасности (Майкрософт) MS10-071 — критическое](http://technet.microsoft.com/security/bulletin/MS10-071), [запись блога о функции автозаполнения](http://blog.mindedsecurity.com/2011/10/autocompleteagain.html) |
...
## <a name="a-iddata-maskaensure-that-sensitive-data-displayed-on-the-user-screen-is-masked"></a><a id="data-mask"></a>Замаскируйте конфиденциальные данные, отображаемые на экране пользователя
...
## <a name="a-iddynamic-usersaimplement-dynamic-data-masking-to-limit-sensitive-data-exposure-non-privileged-users"></a><a id="dynamic-users"></a>Реализуйте динамическую маскировку данных, чтобы уменьшить шансы раскрытия конфиденциальных данных для непривилегированных пользователей
...
## <a name="a-idsalted-hashaensure-that-passwords-are-stored-in-salted-hash-format"></a><a id="salted-hash"></a>Обеспечьте хранение паролей в формате хэша с использованием случайной строки данных
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [Хеширование паролей с использованием API .NET для шифрования](http://docs.asp.net/en/latest/security/data-protection/consumer-apis/password-hashing.html) |
...
## <a name="a-iddb-encryptedaensure-that-sensitive-data-in-database-columns-is-encrypted"></a><a id="db-encrypted"></a>Шифруйте конфиденциальные данные в столбцах базы данных
...
## <a name="a-idtde-enabledaensure-that-database-level-encryption-tde-is-enabled"></a><a id="tde-enabled"></a>Включите шифрование уровня базы данных (TDE)
...
## <a name="a-idbackupaensure-that-database-backups-are-encrypted"></a><a id="backup"></a>Зашифруйте резервные копии базы данных
...
## <a name="a-idapi-browseraensure-that-sensitive-data-relevant-to-web-api-is-not-stored-in-browsers-storage"></a><a id="api-browser"></a>В хранилище браузера не должны храниться конфиденциальные данные, относящиеся к веб-API
...
## <a name="a-idencrypt-docdbaencrypt-sensitive-data-stored-in-documentdb"></a><a id="encrypt-docdb"></a>Шифруйте конфиденциальные данные, хранящиеся в DocumentDB
...
## <a name="a-iddisk-vmause-azure-disk-encryption-to-encrypt-disks-used-by-virtual-machines"></a><a id="disk-vm"></a>Шифруйте диски, используемые виртуальными машинами, с помощью шифрования дисков Azure
...
## <a name="a-idfabric-appsaencrypt-secrets-in-service-fabric-applications"></a><a id="fabric-apps"></a>Шифруйте секреты в приложениях Service Fabric
...
## <a name="a-idmodeling-teamsaperform-security-modeling-and-use-business-unitsteams-where-required"></a><a id="modeling-teams"></a>Выполняйте моделирование безопасности, при необходимости привлекая рабочие подразделения и группы
...
## <a name="a-identitiesaminimize-access-to-share-feature-on-critical-entities"></a><a id="entities"></a>Ограничьте доступ к общим функциям критически важных сущностей
...
## <a name="a-idgood-practicesatrain-users-on-the-risks-associated-with-the-dynamics-crm-share-feature-and-good-security-practices"></a><a id="good-practices"></a>Информируйте пользователей о рисках, связанных с использованием функции "Поделиться" Dynamics CRM, и предоставьте им рекомендации безопасности
...
## <a name="a-idexception-mgmtainclude-a-development-standards-rule-proscribing-showing-config-details-in-exception-management"></a><a id="exception-mgmt"></a>Включите стандартные правила разработки, запрещающие показ данных о конфигурации в управлении исключениями
...
## <a name="a-idsse-previewause-azure-storage-service-encryption-sse-for-data-at-rest-preview"></a><a id="sse-preview"></a>Используйте шифрование службы хранилища Azure (SSE) для неактивных данных (предварительная версия)
...
## <a name="a-idclient-storageause-client-side-encryption-to-store-sensitive-data-in-azure-storage"></a><a id="client-storage"></a>Используйте шифрование на стороне клиента для хранения конфиденциальных данных в службе хранилища Azure
...
## <a name="a-idpii-phonesaencrypt-sensitive-or-pii-data-written-to-phones-local-storage"></a><a id="pii-phones"></a>Шифруйте конфиденциальные данные или личные данные, записываемые в локальное хранилище телефонов
...
| Атрибуты              | Недоступно  |
| Ссылки              | [Создание политики конфигурации](https://docs.microsoft.com/intune/deploy-use/manage-settings-and-features-on-your-devices-with-microsoft-intune-policies#create-a-configuration-policy), [сведения об обслуживании цепочки ключей](https://components.xamarin.com/view/square.valet) |
...
## <a name="a-idbinaries-endaobfuscate-generated-binaries-before-distributing-to-end-users"></a><a id="binaries-end"></a>Маскируйте созданные двоичные файлы перед их распространением конечным пользователям
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [Crypto Obfuscator для .Net](http://www.ssware.com/cryptoobfuscator/obfuscator-net.htm) |
...
## <a name="a-idcertaset-clientcredentialtype-to-certificate-or-windows"></a><a id="cert"></a>Установите для параметра clientCredentialType значение Certificate или Windows
...
| Атрибуты              | Недоступно  |
| Ссылки              | [Fortify](https://vulncat.fortify.com/en/vulncat/index.html) |
...
## <a name="a-idsecurityawcf-security-mode-is-not-enabled"></a><a id="security"></a>Не включен режим безопасности WCF
...
| Ссылки              | [MSDN](https://msdn.microsoft.com/library/ff648500.aspx), [Fortify Kingdom](https://vulncat.fortify.com/en/vulncat/index.html), [основы безопасности WCF](http://www.codemag.com/article/0611051) |
