...
Служба интеграции журналов данных Azure (AzLog) предоставляет клиентам альтернативу, если у их поставщика SIEM еще нет соединителя [Azure Monitor](../monitoring-and-diagnostics/monitoring-get-started.md). Она делает журналы Azure доступными для SIEM и позволяет создать единую панель мониторинга безопасности для всех ресурсов.

>[!NOTE]
>Дополнительные сведения см. в статье [Приступая к работе с Azure Monitor](../monitoring-and-diagnostics/monitoring-get-started.md). За дополнительными сведениями о состоянии соединителя Azure Monitor обращайтесь к своему поставщику SIEM.
...
Эта статья поможет вам приступить к работе с интеграцией журналов данных Azure. Она посвящена установке службы AzLog и ее интеграции с системой диагностики Azure. После этого служба интеграции журналов Azure сможет собирать данные журнала событий Windows из канала событий безопасности Windows, поступающие из виртуальных машин, развернутых в IaaS Azure. Это похоже на пересылку событий, которую вы, возможно, использовали в локальной среде.

>[!NOTE]
>SIEM обеспечивает возможность передачи выходных данных службы интеграции журналов данных Azure в SIEM. Дополнительные сведения см. в статье [Azure Log Integration SIEM configuration steps](https://blogs.msdn.microsoft.com/azuresecurity/2016/08/23/azure-log-siem-configuration-steps/) (Настройка службы интеграции журналов данных Azure в SIEM).

Следует уточнить: служба интеграции журналов данных Azure выполняется на физическом компьютере или виртуальной машине под управлением операционной системы Windows Server 2008 R2 или более поздней версии (Windows Server 2012 R2 или Windows Server 2016 являются предпочтительными).

Физический компьютер может работать в локальной среде (или на сайте для размещения). Если вы решили выполнять службу интеграции журналов Azure на виртуальной машине, эта виртуальная машина может быть расположена в локальной среде или общедоступном облаке, например Microsoft Azure.
...
* **Две системы**: компьютер, на котором будет выполняться служба интеграции журналов Azure, и компьютер, который будет отслеживаться и предоставлять данные журнала для передачи на компьютер службы AzLog.
   ...
   * Компьютер, на котором выполняется служба интеграции журналов Azure. Этот компьютер собирает все данные журнала, которые позже будут импортироваться в SIEM.
    ...
    * На нем должен использоваться 64-разрядный выпуск Windows Server 2008 2008 R2 с пакетом обновления 1 (SP1) или более поздней версии и установлена платформа .NET Framework 4.5.1. Установленную версию .NET можно определить, следуя указаниям в статье [Практическое руководство. Определение установленных версий платформы .NET Framework](https://msdn.microsoft.com/library/hh925568).  
    Он должен иметь возможность подключения к учетной записи хранения Azure, используемой для ведения журналов диагностики Azure. В этой статье даны указания по проверке этого подключения.

* **Учетная запись хранения**, которую можно использовать для ведения журнала диагностики Windows Azure. Вы можете использовать имеющуюся учетную запись хранения или создать новую. Вы настроите учетную запись хранения позже.
  >[!NOTE]
  В зависимости от ситуации учетная запись хранения может не потребоваться. Однако она требуется для сценария диагностики Azure, описанном в этой статье.
* **Две системы**: компьютер, на котором выполняется служба интеграции журналов Azure, и компьютер, который отслеживается и предоставляет данные журнала для передачи на компьютер службы Azlog.
   * Компьютер, который требуется отслеживать, — это виртуальная машина [Azure](../virtual-machines/virtual-machines-windows-overview.md).
   * Компьютер, на котором выполняется служба интеграции журналов Azure. Этот компьютер собирает все данные журнала, которые позже будут импортироваться в SIEM.
    * Эта система может размещаться в локальной среде или в облаке Microsoft Azure.  
    * На нем должен использоваться 64-разрядный выпуск Windows Server 2008 2008 R2 с пакетом обновления 1 (SP1) или более поздней версии и установлена платформа .NET Framework 4.5.1. Установленную версию .NET можно определить, следуя указаниям в статье [Практическое руководство. Определение установленных версий платформы .NET Framework](https://msdn.microsoft.com/library/hh925568).  
    Он должен иметь возможность подключения к учетной записи хранения Azure, используемой для ведения журналов диагностики Azure. Способ проверки подключения описан в этой статье далее.
...
Вы можете запустить несколько экземпляров службы интеграции журналов данных Azure. Запускать можно по одному экземпляру на физический компьютер или виртуальную машину. Кроме того, можно балансировать нагрузку учетных записей хранения системы диагностики Azure для Windows (WAD) и число подписок, предоставляемых для экземпляров, в соответствии с нагрузкой.
...
В случае большого объема событий можно запустить несколько экземпляров службы интеграции журналов Azure (по одному на физический компьютер или виртуальную машину). Кроме того, вы можете распределять нагрузку учетных записей хранения системы диагностики Azure для Windows (WAD).
...
2. Первое, что необходимо сделать, — импортировать командлеты AzLog. Для этого можно выполнить сценарий **LoadAzlogModule.ps1** (обратите внимание на знак ".\" в приведенной ниже команде). Введите **.\LoadAzlogModule.ps1** и нажмите клавишу **ВВОД**. На рисунке ниже показан пример того, что вы должны увидеть на экране. </br></br>
...
4. Прежде чем начать мониторинг системы, необходимо получить имя учетной записи хранения, используемой для системы диагностики Azure.  На портале Azure перейдите в колонку **Виртуальные машины** и найдите виртуальную машину, которую вы отслеживаете. В разделе **Свойства** щелкните **Параметры диагностики**.  Щелкните **Агент** и запишите указанное имя учетной записи хранения. Оно потребуется вам в дальнейшем.
...
5. Теперь мы перейдем к компьютеру интеграции журналов Azure. Нужно проверить возможность подключения к учетной записи хранения из системы, в которой установлена служба интеграции журналов Azure. Компьютеру, на котором работает служба интеграции журналов Azure, требуется доступ к учетной записи хранения для получения сведений, регистрируемых системой диагностики Azure, в соответствии с настройками в каждой из отслеживаемых систем.  
    a. Скачать Azure Storage Explorer можно [отсюда](http://storageexplorer.com/).
   Б. Выполните процедуру установки. В. После завершения установки нажмите кнопку **Далее** и не устанавливайте флажок **Launch Microsoft Azure Storage Explorer** (Запустить Microsoft Azure Storage Explorer).  
    d. Войдите в Azure.
   д. Убедитесь, что на портале отображается учетная запись хранения, настроенная для системы диагностики Azure. 
...
6. Обратите внимание, что в учетной записи хранения отображается несколько элементов. Один из них — **Таблицы**. В элементе **Таблицы** отображается таблица **WADWindowsEventLogsTable**. </br></br>
 ...
 Если мониторинг не был включен во время создания виртуальной машины, вам предоставляется возможность включить его, как показано выше.
7. Теперь мы перейдем к компьютеру интеграции журналов Azure. Нужно проверить возможность подключения к учетной записи хранения из системы, в которой установлена служба интеграции журналов Azure. Физическому компьютеру или виртуальной машине, на которой работает служба интеграции журналов Azure, требуется доступ к учетной записи хранения для получения сведений, регистрируемых системой диагностики Azure, в соответствии с настройками в каждой из отслеживаемых систем.  
  ...
  6. Обратите внимание, что в учетной записи хранения отображается несколько элементов. Один из них — **Таблицы**. В элементе **Таблицы** отображается таблица **WADWindowsEventLogsTable**. </br></br>
   ![Учетные записи хранения](./media/security-azure-log-integration-get-started/storage-explorer.png) 
...
Для выполнения этого шага необходимо заранее подготовить следующее.  
...
Выполните следующее, чтобы получить ключ к хранилищу данных.
 ...
 2. В области навигации консоли Azure щелкните **Все службы.**
  3. В текстовом поле **Фильтр** введите **Учетные записи хранения**. Щелкните элемент **Учетные записи хранения**.

       ![Снимок экрана, на котором показаны учетные записи хранения в колонке "Все службы"](./media/security-azure-log-integration-get-started/filter.png)
