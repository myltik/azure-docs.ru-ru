...
## <a name="a-iddisable-xsltadisable-xslt-scripting-for-all-transforms-using-untrusted-style-sheets"></a><a id="disable-xslt"></a>Отключайте скрипты XSLT для всех операций преобразования, использующих ненадежные таблицы стилей
...
| Атрибуты              | Недоступно  |
| Ссылки              | [Безопасность XSLT](https://msdn.microsoft.com/library/ms763800(v=vs.85).aspx), [XsltSettings.EnableScript Property](http://msdn.microsoft.com/library/system.xml.xsl.xsltsettings.enablescript.aspx) (Свойство XsltSettings.EnableScript) |
...
## <a name="a-idout-sniffingaensure-that-each-page-that-could-contain-user-controllable-content-opts-out-of-automatic-mime-sniffing"></a><a id="out-sniffing"></a>Убедитесь, что каждая страница, на которой может быть содержимое, управляемое пользователем, не использует автоматическое обнаружение типов MIME
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [IE8 Security Part V — Comprehensive Protection](http://blogs.msdn.com/ie/archive/2008/07/02/ie8-security-part-v-comprehensive-protection.aspx) (Безопасность IE8, часть 5: комплексная защита)  |
...
## <a name="a-idxml-resolutionaharden-or-disable-xml-entity-resolution"></a><a id="xml-resolution"></a>Усильте защиту для разрешения сущностей XML или отключите его
...
## <a name="a-idapp-verificationaapplications-utilizing-httpsys-perform-url-canonicalization-verification"></a><a id="app-verification"></a>Приложения, использующие драйвер http.sys, выполняют проверку нормализации URL-адресов
...
## <a name="a-idcontrols-usersaensure-appropriate-controls-are-in-place-when-accepting-files-from-users"></a><a id="controls-users"></a>Убедитесь, что при приеме файлов от пользователей обеспечиваются соответствующие меры безопасности
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [Unrestricted File Upload](https://www.owasp.org/index.php/Unrestricted_File_Upload) (Неограниченная передача файлов), [File Signature Table](http://www.garykessler.net/library/file_sigs.html) (Таблица подписей файлов) |
...
## <a name="a-idtypesafeaensure-that-type-safe-parameters-are-used-in-web-application-for-data-access"></a><a id="typesafe"></a>Убедитесь, что в веб-приложении для доступа к данным используются параметры типобезопасности
...
## <a name="a-idbinding-mvcause-separate-model-binding-classes-or-binding-filter-lists-to-prevent-mvc-mass-assignment-vulnerability"></a><a id="binding-mvc"></a>Используйте отдельные классы привязки моделей или списки привязки фильтров для устранения уязвимости массового назначения MVC
...
## <a name="a-idtypemodelaperform-input-validation-and-filtering-on-all-string-type-model-properties"></a><a id="typemodel"></a>Проверяйте и фильтруйте входные данные по всем свойствам модели строкового типа
...
| Атрибуты              | Недоступно  |
| Ссылки              | [Adding Validation](http://www.asp.net/mvc/overview/getting-started/introduction/adding-validation) (Добавление проверки), [Validating Model Data in an MVC Application](http://msdn.microsoft.com/library/dd410404(v=vs.90).aspx) (Проверка данных модели в приложении MVC), [Основные принципы ваших приложений ASP.NET MVC](http://msdn.microsoft.com/magazine/dd942822.aspx) |
...
## <a name="a-idrichtextasanitization-should-be-applied-on-form-fields-that-accept-all-characters-eg-rich-text-editor"></a><a id="richtext"></a>Очищайте поля форм, принимающие все знаки, например редактор форматированного текста
...
## <a name="a-idinbuilt-encodeado-not-assign-dom-elements-to-sinks-that-do-not-have-inbuilt-encoding"></a><a id="inbuilt-encode"></a>Не назначайте элементы модели DOM приемникам без встроенных возможностей кодирования
...
## <a name="a-idredirect-safeavalidate-all-redirects-within-the-application-are-closed-or-done-safely"></a><a id="redirect-safe"></a>Убедитесь, что операции перенаправления в приложении закрыты или выполняются безопасно
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [The OAuth 2.0 Authorization Framework — Open Redirectors](http://tools.ietf.org/html/rfc6749#section-10.15) (OAuth 2.0 Authorization Framework: открытые средства перенаправления) |
...
## <a name="a-idstring-methodaimplement-input-validation-on-all-string-type-parameters-accepted-by-controller-methods"></a><a id="string-method"></a>Внедрите проверку входных данных для всех параметров строкового типа, принимаемых методами контроллера
...
| Применимые технологии | Универсальные, MVC 5, MVC 6 |
| Атрибуты              | Недоступно  |
| Ссылки              | [Validating Model Data in an MVC Application](http://msdn.microsoft.com/library/dd410404(v=vs.90).aspx) (Проверка данных модели в приложении MVC), [Основные принципы ваших приложений ASP.NET MVC](http://msdn.microsoft.com/magazine/dd942822.aspx) |
...
| Применимые технологии | Универсальные, веб-формы, MVC 5, MVC 6  |
| Атрибуты              | Недоступно  |
| Ссылки              | [Свойство DefaultRegexMatchTimeout](https://msdn.microsoft.com/library/system.web.configuration.httpruntimesection.defaultregexmatchtimeout.aspx) |
...
## <a name="a-idhtml-razoraavoid-using-htmlraw-in-razor-views"></a><a id="html-razor"></a>Не используйте метод Html.Raw в представлениях Razor
...
| Атрибуты              | Недоступно  |
| Ссылки              | Недоступно  |
| Шаг | Веб-страницы ASP.NET (Razor) выполняют автоматическое кодирование HTML. Все строки, которые выводятся внедренными фрагментами кода (@-блоки), автоматически кодируются в формате HTML. Однако при вызове метод `HtmlHelper.Raw` возвращает разметку без кодировки HTML. Если используется вспомогательный метод `Html.Raw()`, он обходит автоматическую защиту кодирования, обеспечиваемую Razor.|
...
## <a name="a-idstored-procado-not-use-dynamic-queries-in-stored-procedures"></a><a id="stored-proc"></a>Не используйте динамические запросы в хранимых процедурах
...
## <a name="a-idvalidation-apiaensure-that-model-validation-is-done-on-web-api-methods"></a><a id="validation-api"></a>Убедитесь, что в методах веб-API выполнена проверка моделей
...
| Применимые технологии | MVC 5, MVC 6 |
| Атрибуты              | Недоступно  |
| Ссылки              | [Model Validation in ASP.NET Web API ](http://www.asp.net/web-api/overview/formats-and-model-binding/model-validation-in-aspnet-web-api) (Проверка модели в веб-API ASP.NET) |
...
## <a name="a-idstring-apiaimplement-input-validation-on-all-string-type-parameters-accepted-by-web-api-methods"></a><a id="string-api"></a>Внедрите проверку входных данных для всех параметров строкового типа, принимаемых методами веб-API
...
## <a name="a-idtypesafe-apiaensure-that-type-safe-parameters-are-used-in-web-api-for-data-access"></a><a id="typesafe-api"></a>Убедитесь, что в веб-API для доступа к данным используются параметры типобезопасности
...
## <a name="a-idsql-docdbause-parametrized-sql-queries-for-documentdb"></a><a id="sql-docdb"></a>Используйте параметризованные SQL-запросы для DocumentDB
...
| Применимые технологии | Универсальный |
| Атрибуты              | Недоступно  |
| Ссылки              | [Announcing SQL Parameterization in DocumentDB](https://azure.microsoft.com/blog/announcing-sql-parameterization-in-documentdb/) (Объявление о параметризации SQL в DocumentDB) |
...
## <a name="a-idschema-bindingawcf-input-validation-through-schema-binding"></a><a id="schema-binding"></a>Проверка входных данных WCF путем привязки к схеме
...
## <a name="a-idparametersawcf--input-validation-through-parameter-inspectors"></a><a id="parameters"></a>Проверка входных данных WCF с помощью инспекторов параметров
