...
Например, в виртуальной сети Azure можно использовать виртуальное устройство безопасности сети. Необходимо убедиться, что весь трафик в виртуальную сеть Azure и из нее проходит через это виртуальное устройство безопасности. Это можно сделать, настроив [пользовательские маршруты](../virtual-network/virtual-networks-udr-overview.md) в Azure.

[Принудительное туннелирование](https://www.petri.com/azure-forced-tunneling) — это механизм, который позволяет запретить службам инициировать подключения к устройствам в Интернете. Обратите внимание, что это отличается от приема входящих подключений и последующего ответа на них. Интерфейсные веб-серверы должны отвечать на запрос от интернет-узлов, поэтому трафику из Интернета разрешено поступать на такие веб-серверы, а им в свою очередь разрешено отвечать.
...
Дополнительные сведения о **пользовательских маршрутах** см. в статье [Что такое определяемые пользователем маршруты и IP-пересылка?](../virtual-network/virtual-networks-udr-overview.md).

### Виртуальные устройства сетевой безопасности
Хотя группы безопасности сети, пользовательские маршруты и принудительное туннелирование предоставляют уровень безопасности на уровнях сети и транспорта [модели OSI](https://en.wikipedia.org/wiki/OSI_model), возможны случаи, когда необходимо включить безопасности на уровнях выше сети.
...
Доступ к этим улучшенным функциям сетевой безопасности осуществляется с помощью решения партнера Azure. Самые новые решения в области безопасности из партнерской сети Azure можно найти, посетив [Azure Marketplace](https://azure.microsoft.com/marketplace/) и выполнив поиск строк "безопасность" и "сетевая безопасность".

## Безопасный удаленный доступ и межорганизационное взаимодействие
Установка, настройка ресурсов Azure и управление ими должны выполняться удаленно. Кроме того, может потребоваться развернуть [гибридные](http://social.technet.microsoft.com/wiki/contents/articles/18120.hybrid-cloud-infrastructure-design-considerations.aspx) ИТ-решения, компоненты которых находятся как в локальной сети, так и в общедоступном облаке Azure. В этих сценариях требуется безопасный удаленный доступ.
...
Дополнительные сведения о **выделенных каналах глобальной сети** между локальным расположением и виртуальными сетями Azure см. в статье [Технический обзор ExpressRoute](../expressroute/expressroute-introduction.md).
...
Один из вариантов подключения служб одной виртуальной сети Azure к службам другой заключается в "замыкании" через Интернет. Такое подключение запускается в одной виртуальной сети Azure, проходит через Интернет и затем возвращается в целевую виртуальную сеть Azure. Этот вариант подвержен проблемам безопасности, связанным с взаимодействием через Интернет.

Более привлекательным вариантом может оказаться создание VPN-подключения "сеть-сеть" между виртуальными сетями Azure. Такое VPN-подключения использует тот же протокол [режима туннелирования IPsec](https://technet.microsoft.com/library/cc786385.aspx), что и упомянутое выше межорганизационное VPN-подключения "сеть-сеть".
...
Дополнительные сведения о VPN типа "сеть-сеть" между виртуальными сетями Azure см. в статье [Настройка подключения между виртуальными сетями с помощью Azure Resource Manager и PowerShell](../vpn-gateway/vpn-gateway-vnet-vnet-rm-ps.md).
...
Дополнительные сведения о шлюзе приложения Azure см. в статье [Обзор шлюза приложений](../application-gateway/application-gateway-introduction.md).
...
Дополнительные сведения о внешнем Azure Load Balancer см .в статье [Доступная в Интернете подсистема балансировки нагрузки, используемая для нескольких виртуальных машин или служб](../load-balancer/load-balancer-internet-overview.md).

Дополнительные сведения о внутреннем Azure Load Balancer см .в статье [Обзор внутренней подсистемы балансировки нагрузки](../load-balancer/load-balancer-internal-overview.md).
...
Дополнительные сведения о глобальной балансировке нагрузки с помощью диспетчера трафика Azure см. в статье [Что такое диспетчер трафика](../traffic-manager/traffic-manager-overview.md).
...
Дополнительные сведения о ведении журнала групп безопасности сети см. в статье [Аналитика журналов для групп безопасности сети](../virtual-network/virtual-network-nsg-manage-log.md).
...
- Внешнее разрешение имен — оно используется, пользователями и устройствами за пределами ваших локальных сетей и виртуальных сетей Azure. Это имена, которые доступны через Интернет и используются для прямого подключения к облачным службам.
...
Дополнительные сведения о **DNS-сервере виртуальной сети Azure** см. в статье [Обзор виртуальной сети](../virtual-network/virtual-networks-overview.md).

Дополнительные сведения об использовании **собственных DNS-серверов** в виртуальной сети Azure см. в статье [Управление DNS-серверами, используемыми виртуальной сетью](../virtual-network/virtual-networks-manage-dns-in-vnet.md).
...
Дополнительные сведения о Azure DNS см. в статье [Обзор Azure DNS](../dns/dns-overview.md).
...
Дополнительные сведения о вариантах демилитаризованной зоны в Azure см. в статье [Облачные службы Microsoft Cloud и сетевая безопасность](../best-practices-network-security.md).
...
Дополнительные сведения о центре безопасности Azure см. в статье [Введение в Центр безопасности Azure](../security-center/security-center-intro.md).
