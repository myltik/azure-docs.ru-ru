...
-   Назначения ролей по умолчанию: [владелец](https://docs.microsoft.com/azure/active-directory/role-based-access-built-in-roles#owner).
...
-   Назначения встроенных ролей: [участник базы данных SQL](https://docs.microsoft.com/azure/active-directory/role-based-access-built-in-roles#sql-db-contributor), [участник SQL Server](https://docs.microsoft.com/azure/active-directory/role-based-access-built-in-roles#sql-server-contributor).
...
-   Назначения встроенных ролей: [участник учетной записи хранения](https://docs.microsoft.com/azure/active-directory/role-based-access-built-in-roles#storage-account-contributor).
...
-   Назначения встроенных ролей: [читатель](https://docs.microsoft.com/azure/active-directory/role-based-access-built-in-roles#reader).
...
-   [Контроль доступа на основе ролей (RBAC) Azure](/azure/active-directory/role-based-access-control-configure) обеспечивает точное управление доступом для Azure. Доступ к подписке ограничен администратором подписки, а доступ к Azure Key Vault разрешен только администратору сайта. Требуются надежные пароли (не менее 12 символов с хотя бы одной буквой в верхнем или нижнем регистре, цифрой и специальным символом).
...
-   В этом решении для управления доступом к ресурсам используются [встроенные роли](/azure/active-directory/role-based-access-built-in-roles).
...
- Использование рабочей среды [машинного обучения](/azure/machine-learning/preview/experimentation-service-configuration) требует разработки экспериментов, которые предоставляют возможность предсказать набор решений. [Интеграция рабочей среды](/azure/machine-learning/preview/using-git-ml-project) может упростить управление экспериментами.
