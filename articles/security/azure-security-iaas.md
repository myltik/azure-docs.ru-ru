...
Проверка электронной почты и просмотр страниц в Интернете кажутся достаточно безобидными занятиями. Однако при этом учетные записи с повышенными правами подвергаются риску атаки злоумышленника, который может использовать действия просмотра, специально созданные сообщения и другие методы для получения доступа к вашей организации. Поэтому настоятельно рекомендуется выполнять все задачи администрирования Azure на защищенных рабочих станциях, так как таким образом снижается вероятность случайного нарушения безопасности.
...
Подход с использованием рабочих станций с привилегированным доступом — это уже известный, но более широкий подход, который заключается в том, чтобы использовать отдельную учетную запись администратора, т. е. запись, отличную от учетной записи обычного пользователя. При этом подходе для работы с конфиденциальными учетными записями предоставляется надежная рабочая станция.
