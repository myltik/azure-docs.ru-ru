...
Microsoft Azure очень серьезно относится к обеспечению конфиденциальности и независимости ваших данных. Вы можете управлять размещенными в Azure данными с помощью ряда передовых технологий, обеспечивающих шифрование, контроль, управление ключами шифрования, а также управление доступом к данным и его аудит. Благодаря этому обеспечивается гибкость в выборе решения, которое лучше всего соответствует потребностям организации.

В этой статье рассматривается шифрование дисков Azure для виртуальных машин IaaS под управлением Windows и Linux. Это новая технология, которая помогает обеспечить защиту данных. Она разработана, чтобы помочь организациям выполнять обязательства в отношении безопасности и соответствия требованиям. В статье содержатся подробные инструкции по использованию функций шифрования дисков Azure, включая поддерживаемые сценарии и взаимодействие с пользователем.
...
* включение шифрования для новых виртуальных машин IaaS, созданных с помощью предварительно зашифрованных виртуальных жестких дисков и ключей шифрования;
* включение шифрования для новых виртуальных машин IaaS, созданных с помощью образов Azure Marketplace;
* включение шифрования для имеющихся виртуальных машин IaaS, запущенных в Azure;
* отключение шифрования для виртуальных машин IaaS под управлением Windows;
...
> [!NOTE]
> Архивация и восстановление зашифрованных виртуальных машин поддерживается только для виртуальных машин, зашифрованных с помощью конфигурации KEK (ключ шифрования ключей). Виртуальные машины, зашифрованные без ключа шифрования ключей, не поддерживаются. KEK — это необязательный параметр для включения шифрования виртуальных машин.
...
>
> Шифрование дисков Azure доступно для следующих версий сервера Linux: Ubuntu, CentOS, SUSE, SUSE Linux Enterprise Server (SLES) и Red Hat Enterprise Linux.

> [!NOTE]
> Сейчас шифрование диска ОС Linux поддерживается для следующих дистрибутивов Linux: RHEL 7.2, RHEL 7.3, CentOS 7.2n и Ubuntu 16.04.
>
> Все ресурсы (хранилище ключей, учетная запись хранения, виртуальная машина и т. д.) должны относиться к одному региону и одной подписке Azure.

> [!NOTE]
> Для шифрования дисков Azure требуется, чтобы хранилище ключей и виртуальные машины находились в одном регионе Azure. Их настройка в отдельных регионах приведет к сбою при включении функции шифрования дисков Azure.

* Чтобы установить и настроить хранилище ключей для шифрования дисков Azure, ознакомьтесь с разделом "Установка и настройка хранилища ключей для шифрования дисков Azure" в приложении.
* Сведения об установке и настройке приложения Azure AD в Azure Active Directory для шифрования дисков Azure см. в подразделе "Настройка приложения Azure AD в Azure Active Directory" раздела "Предварительные требования".
* Сведения об установке и настройке политики доступа к хранилищу ключей для приложения Azure AD см. в подразделе "Настройка политики доступа к хранилищу ключей для приложения Azure AD" раздела "Предварительные требования".
...
* Чтобы использовать функцию шифрования дисков Azure, конфигурация конечной точки виртуальной машины IaaS должна соответствовать приведенным ниже требованиям.
  ...
  * Виртуальная машина IaaS должна иметь возможность подключиться к конечной точке Azure Active Directory \[Login.windows.net\], чтобы получить маркер для подключения к хранилищу ключей.
  ...
  * Виртуальная машина IaaS должна иметь возможность подключиться к конечной точке службы хранилища Azure, в которой размещен репозиторий расширений Azure, и к учетной записи хранения Azure, в которой размещены VHD-файлы.
  ...
  >
  > Для настройки шифрования дисков Azure скачайте последнюю версию [Azure PowerShell](https://github.com/Azure/azure-powershell/releases).
...
* Чтобы использовать службу архивации Azure для архивации и восстановления зашифрованных виртуальных машин, использующих шифрование дисков Azure, необходимо шифровать виртуальные машины с помощью конфигурации с ключом шифрования для шифрования дисков Azure. Служба архивации поддерживает только виртуальные машины, зашифрованные с помощью конфигурации с ключом шифрования ключей (KEK). Эта служба поддерживает только виртуальные машины, зашифрованные с помощью ключа шифрования ключей. Ниже приведены сценарии развертывания шифрования дисков, обеспечивающие шифрование виртуальных машин с помощью конфигурации KEK.
...
##### <a name="setting-up-the-azure-ad-client-id-and-secret-from-the-azure-classic-portal"></a>Настройка идентификатора и секрета клиента Azure AD на классическом портале Azure
Можно также настроить идентификатор и секрет клиента Azure AD с помощью [классического портала Azure]( https://manage.windowsazure.com). Для этого выполните следующее.
...
5. Сохраните секрет клиента Azure AD, щелкнув кнопку **Сохранить**. Запишите секрет клиента Azure AD, указанный в текстовом поле ключей. Храните его с соблюдением мер предосторожности.
 ...
 > [!NOTE]
 > Предшествующая последовательность действий не поддерживается на классическом портале Azure.
...
Включить шифрование дисков на существующих или работающих виртуальных машинах IaaS под управлением Linux в Azure можно с помощью [шаблона Resource Manager](https://github.com/Azure/azure-quick-start-templates/tree/master/201-encrypt-running-linux-vm).
...
| keyVaultName | Имя хранилища ключей, в которое будет передан ключ BitLocker. Его можно получить с помощью командлета `(Get-AzureRmKeyVault -ResourceGroupName <yourResourceGroupName>). Vaultname`. |
|  keyEncryptionKeyURL | URL-адрес ключа шифрования ключей, который используется для шифрования созданного ключа BitLocker. Это необязательный параметр, если выбрать **nokek** из раскрывающегося списка UseExistingKek. Если из раскрывающегося списка UseExistingKek выбрано значение **kek**, то потребуется ввести значение _keyEncryptionKeyURL_. |
| volumeType | Тип тома, для которого будет выполняться шифрование. Поддерживаемые допустимые значения: _OS_ или _All_ (для RHEL 7.2, CentOS 7.2 и Ubuntu 16.04) и _Data_ (для всех остальных дистрибутивов). |
| sequenceVersion | Версия последовательности операций с BitLocker. Этот номер версии увеличивается каждый раз, когда шифрование диска выполняется на той же виртуальной машине. |
| vmName | Имя виртуальной машины, для которой будет выполняться шифрование. |
...
1. Войдите на [классический портал Azure](https://portal.azure.com/) и щелкните **Виртуальные машины** в левой области, чтобы просмотреть представление сводки по виртуальным машинам в своей подписке. Вы можете отфильтровать представление виртуальных машин, выбрав имя подписки из раскрывающегося списка **Подписка**.
...
Прежде чем продолжить, просмотрите раздел "Предварительные требования" в этой статье. Убедившись, что выполнены все предварительные требования, подключитесь к своей подписке, выполнив следующее.
...
6. Как только командлет Get-AzureRmVmDiskEncryptionStatus вернет значение VMRestartPending, перезапустите виртуальную машину, войдя в нее, либо с помощью портала, PowerShell или интерфейса командной строки.
    ...
    ```
Перед перезагрузкой рекомендуется сохранить [диагностические данные загрузки](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/) виртуальной машины.
...
* Просмотрите [диагностические данные загрузки](https://azure.microsoft.com/en-us/blog/boot-diagnostics-for-virtual-machines-v2/). Сообщения расширения шифрования дисков Azure должны начинаться с `[AzureDiskEncryption]`.
...
6. Подготовьте виртуальную машину к передаче в Azure, следуя [этим инструкциям](https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-linux-create-upload-ubuntu/). Пока не выполняйте последний шаг (отзыв виртуальной машины).
