...
4. Azure обновляет модель службы виртуальной машины, и виртуальная машина IaaS помечается как расшифрованная. Содержимое виртуальной машины больше не шифруется при хранении.
5. При отключении операции шифрования хранилище ключей клиента и материал ключа шифрования (ключи шифрования BitLocker для Windows или парольная фраза для Linux) не удаляются.
...
> [AZURE.NOTE] В настоящее время проверка подлинности на основе сертификата AAD на виртуальных машинах Linux не поддерживается.
...
> [AZURE.NOTE] Все пользовательские процессы, не запущенные как службы `systemd`, необходимо завершить с помощью `SIGKILL`. После этого виртуальная машина должна быть перезагружена. При включении шифрования диска операционной системы на работающей виртуальной машине необходимо запланировать время простоя.
...
*Прежде* чем перезагружать виртуальную машину, советуем сохранить [данные диагностики загрузки](https://azure.microsoft.com/ru-RU/blog/boot-diagnostics-for-virtual-machines-v2/).
...
2\. Просмотрите [данные диагностики загрузки](https://azure.microsoft.com/ru-RU/blog/boot-diagnostics-for-virtual-machines-v2/). Сообщения от расширения шифрования дисков Azure должны начинаться с `[AzureDiskEncryption]`.
...
6\. Подготовьте виртуальную машину к отправке в Azure, используя [приведенные здесь инструкции](https://azure.microsoft.com/ru-RU/documentation/articles/virtual-machines-linux-create-upload-ubuntu/). Пока не выполняйте последний шаг (отзыв виртуальной машины).
...
3\. Подготовьте виртуальную машину к отправке в Azure, используя [приведенные здесь инструкции](https://azure.microsoft.com/ru-RU/documentation/articles/virtual-machines-linux-suse-create-upload-vhd/#prepare-opensuse-131). Пока не выполняйте последний шаг (отзыв виртуальной машины).
...
5\. Подготовьте виртуальную машину к отправке в Azure, используя [приведенные здесь инструкции](https://azure.microsoft.com/ru-RU/documentation/articles/virtual-machines-linux-create-upload-centos/#centos-70). Пока не выполняйте последний шаг (отзыв виртуальной машины).
