...
* включение шифрования для виртуальных машин IaaS с ОС Windows и Linux и виртуальных машин с управляемыми дисками;
...
> [!NOTE]
> Архивация и восстановление зашифрованных виртуальных машин поддерживается только для виртуальных машин, зашифрованных с помощью конфигурации KEK (ключ шифрования ключей). Виртуальные машины, зашифрованные без ключа шифрования ключей, не поддерживаются. KEK — это необязательный параметр для включения шифрования виртуальных машин. Поддержка ожидается в ближайшем будущем.
...
* Чтобы использовать функцию шифрования дисков Azure, конфигурация конечной точки виртуальной машины IaaS должна соответствовать приведенным ниже требованиям.
  ...
  * Виртуальная машина IaaS должна иметь возможность подключиться к конечной точке службы хранилища Azure, в которой размещен репозиторий расширений Azure, и к учетной записи хранения Azure, в которой размещены VHD-файлы.
  ...
  >
  >Сведения о настройке хранилища ключей Azure и доступе к нему из-за брандмауэра: https://docs.microsoft.com/en-us/azure/key-vault/key-vault-access-behind-firewall
...
* При использовании командлета PowerShell шифрования дисков Azure Set-AzureRmVMDiskEncryptionExtension или команды интерфейса командной строки для включения шифрования в виртуальной машине Azure с управляемыми дисками следует использовать параметр -skipVmBackup.
> [!NOTE]
 > Если не указать параметр -skipVmBackup, включение шифрования завершится сбоем.
...
* Чтобы использовать службу архивации Azure для архивации и восстановления зашифрованных виртуальных машин, использующих шифрование дисков Azure, необходимо шифровать виртуальные машины с помощью конфигурации с ключом шифрования для шифрования дисков Azure. Служба архивации поддерживает только виртуальные машины, зашифрованные с помощью конфигурации с ключом шифрования ключей (KEK). См. раздел [Архивация и восстановление зашифрованных виртуальных машин с помощью службы архивации Azure](https://docs.microsoft.com/en-us/azure/backup/backup-azure-vms-encryption).

> [!NOTE]
> Архивация и восстановление зашифрованных виртуальных машин поддерживается только для виртуальных машин, зашифрованных с помощью конфигурации KEK (ключ шифрования ключей). Виртуальные машины, зашифрованные без ключа шифрования ключей, не поддерживаются. KEK — это необязательный параметр для включения виртуальных машин.
...
### <a name="enable-encryption-on-a-new-windows-iaas-vm-with-azure-managed-disk"></a>Включение шифрования для новой виртуальной машины IaaS с ОС Windows и управляемым диском Azure
 ...
 [Создание зашифрованной виртуальной машины IaaS с ОС Windows и управляемым диском на основе образа из коллекции] (https://github.com/Azure/azure-quickstart-templates/tree/master/201-encrypt-create-new-vm-gallery-image-managed-disks)
  ...
  >При использовании командлета PowerShell шифрования дисков Azure Set-AzureRmVMDiskEncryptionExtension или команды интерфейса командной строки для включения шифрования в виртуальной машине Azure с управляемыми дисками следует использовать параметр -skipVmBackup.
  >
  >Перед включением шифрования с помощью командлета PowerShell Set-AzureRmVMDiskEncryptionExtension для виртуальной машины Linux с управляемым диском рекомендуется создать резервную копию запущенного экземпляра виртуальной машины.
