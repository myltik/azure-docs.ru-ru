...
2. [Включение в приложении функции обнаружения конфликтов]
...
2. В обозревателе решений Visual Studio откройте файл MainPage.xaml, замените определение ListView на приведенный ниже элемент и сохраните изменения.
...
3. В MainPage.xaml.cs добавьте такую директиву using в верхней части страницы:
...
<h2><a name="enableOC"></a>Включение в приложении функции обнаружения конфликтов</h2>
...
6. В экземпляре 1 приложения измените значение свойства текста последнего элемента на **Тест записи 1**, а затем щелкните другое текстовое поле, чтобы обработчик событий LostFocus обновил базу данных. На следующем снимке экрана показан пример.
...
+  если в поле завершения таблицы TodoItem указано значение true, таблица считается завершенной и свойство текста больше не меняется;
+  если в поле завершения таблицы TodoItem указано значение false, будут продолжаться попытки обновить это свойство.
...
<!-- Anchors. -->
[Обновление приложения для предоставления разрешения на выполнение обновлений]: #uiupdate
...
<!-- URLs. -->
[Управление оптимистическим параллелизмом]: http://go.microsoft.com/fwlink/?LinkId=330935
