...
##Обновление приложения для разрешения обновлений
...
##Тестирование конфликтов записи базы данных в приложении
...
7. На этом этапе последний элемент в экземпляре 2 приложения содержит старую версию элемента. В этом экземпляре приложения введите **Тест записи 2** для свойства `text`. Затем выберите другое текстовое поле, чтобы обработчик событий `LostFocus` попытался обновить базу данных со старым свойством `_version`.
...
+  Если поле `Завершено` таблицы TodoItem по-прежнему имеет значение false, попытки обновить свойство  `text` будут продолжаться.
...
5. Запустите приложение **todolist** на обеих машинах. Измените свойство `text` TodoItem для последнего элемента в экземпляре 2. Затем выберите другое текстовое поле, чтобы обработчик событий `LostFocus` обновил базу данных.
...
6. В экземпляре 1 приложения введите отличающееся значение для последнего текстового свойства. После этого выберите другое текстовое поле, чтобы обработчик событий `LostFocus` попытался обновить базу данных с неправильным свойством __version.
...
В этом учебнике показано, как сделать так, чтобы приложение для Магазина Windows обрабатывало конфликты записи при работе с данными в мобильных службах. Затем рекомендуется ознакомиться с одним из следующих учебников по Магазину Windows.
