<properties linkid="develop-mobile-tutorials-optimistic-concurrent-data-dotnet" urlDisplayName="Оптимистичный параллелизм" pageTitle="Обрабатывайте конфликт записи базы данных с помощью оптимистичного параллелизма (Магазин Windows) | Центр разработчиков для мобильных устройств" metaKeywords="" description="Узнайте, как обрабатывать конфликты записи базы данных как на сервере, так и в приложении Магазина Windows." metaCanonical="" disqusComments="1" umbracoNaviHide="1" documentationCenter="Mobile" title="Обработка конфликтов записи базы данных" authors="" />

# Обработка конфликтов записи базы данных

<div class="dev-center-tutorial-selector sublanding">
<a href="/ru-ru/develop/mobile/tutorials/handle-database-write-conflicts-dotnet/" title="Магазин Windows C#" class="current">Магазин Windows C#</a>
<a href="/ru-ru/develop/mobile/tutorials/handle-database-write-conflicts-javascript/" title="Магазин Windows JavaScript">Магазин Windows JavaScript</a>
...
+ Microsoft Visual Studio 2012 Express для Windows или более поздняя версия.
+ Этот учебник основан на кратком руководстве по использованию мобильных служб. Перед началом работы с учебником необходимо пройти задания учебника [Приступая к работе с мобильными службами]. 
...
2. В обозревателе решений Visual Studio откройте MainPage.xaml и замените определение `ListView` показанным ниже `ListView` и сохраните изменения.
...
3. В MainPage.xaml.cs добавьте следующую директиву `с помощью` в верхней части страницы.

		using System.Threading.Tasks;


4. В обозревателе решений Visual Studio откройте MainPage.xaml.cs. Добавьте обработчик событий MainPage для текстового поля события `LostFocus`, как показано ниже.
...
1. В MainPage.xaml.cs обновите определение класса **TodoItem** с помощью следующего кода, чтобы в него системное свойство **__version**. Это включит поддержку обнаружения конфликтов записи.
...
2. После добавления свойства `Версия` к классу `TodoItem` приложение во время обновления будет получать уведомления об исключении `MobileServicePreconditionFailedException`, если запись была изменена с момента последнего запроса. Это исключение включает последнюю версию элемента с сервера. В файле проекта MainPage.xaml.cs добавьте следующий код, чтобы обрабатывать исключение в методе `UpdateToDoItem()`.
...
6. В экземпляре 1 приложения обновите текст последнего элемента до **Тест записи 1**, затем выберите другое текстовое поле таким образом, чтобы обработчик событий `LostFocus` обновил базу данных. На следующем снимке экрана показан пример.
...
7. На этом этапе соответствующий элемент в экземпляр 2 приложения содержит старую версию элемента. В этом экземпляре приложения введите **Тест записи 2** для свойства `текст`. Затем выберите другое текстовое поле, чтобы обработчик событий `LostFocus` попытался обновить базу данных со старым свойством `_version`.
...
8. Так как значение `__version`, использованное в попытке обновления, не соответствует значению `__version` сервера, пакет SDK для мобильных служб высылает уведомление `MobileServicePreconditionFailedException`, которое позволит приложению разрешить этот конфликт. Для разрешения конфликта нажмите **Принять локальный текст** для сохранения значений из экземпляра 2. Или же щелкните **Оставить текст сервера** для отклонения значений в экземпляре 2, оставляя зафиксированными значения из экземпляра 1 приложения. 
...
+ Если поле `Завершено` таблицы TodoItem имеет значение "истина", таблица считается завершенной и `текст` больше не может быть изменен.
+ Если поле ` Завершено` таблицы TodoItem по-прежнему имеет значение "ложь", попытки обновить `текст` будут продолжаться.

Следующие шаги описывают добавление серверного скрипта обновления и его тестирование.
...
5. Запустите приложение **todolist** на обеих машинах. Измените поле `текст` таблицы TodoItem для последнего элемента в экземпляре 2. Затем выберите другое текстовое поле таким образом, чтобы обработчик событий `LostFocus` обновил базу данных.
...
6. В экземпляре 1 приложения введите отличающееся значение для последнего текстового свойства. Затем выберите другое текстовое поле, чтобы обработчик событий `LostFocus` попытался обновить базу данных с неправильным свойством `_version`.
...
* [Приступая к работе с проверкой подлинности] 
  <br/>Дополнительные сведения об аутентификации учетных данных пользователей приложения.
