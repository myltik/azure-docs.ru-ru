...
Кодировщики сжимают мультимедиа с помощью кодеков. Обычно кодировщики имеют различные параметры, которые позволяют указать свойства создаваемых данных мультимедиа, например, используемые кодеки, формат файла, разрешение и скоростью. Форматы файлов являются контейнерами, в которых хранится сжатое видео, а также сведения о кодеках, использованных для сжатия этого видео. 

Кодеки состоят из двух компонентов: один служит для сжатия цифровых файлов мультимедиа при передаче, а другой - для распаковки цифровых файлов при воспроизведении. Существуют аудиокодеки, которые сжимают и распаковывают аудиоданные, и видеокодеки, которые сжимают и распаковывают видеоданные. Кодеки могут использовать сжатие без потерь или сжатие с потерей данных. Кодеки со сжатием без потерь сохраняют все сведения во время сжатия. При распаковке файла получается файл, идентичный входному файлу мультимедиа, поэтому кодеки без потерь хорошо подходят для архивирования и хранения. Кодеки с потерями теряют часть сведений во время кодирования и создают файлы меньшего размера (по сравнению с исходным), жертвуя качеством видео, и хорошо подходят для потоковой передачи через Интернет. Кодировщик служб мультимедиа Azure использует два основных кодека - H.264 и VC-1. В нашей партнерской экосистеме кодировщиков могут быть доступны и другие кодеки.
