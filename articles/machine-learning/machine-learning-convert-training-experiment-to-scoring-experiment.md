...
1. Сохраните обученную модель машинного обучения, а затем замените алгоритм машинного обучения и модули [Обучение модели][train-model] с сохраненной обученной моделью.
...
1. При этом обученная модель сохраняется в качестве модуля в разделе **Trained Models** (Обученные модели) палитры модулей (слева от холста эксперимента), а затем алгоритм машинного обучения и модули [Обучение модели][train-model] заменяются сохраненной обученной моделью.
2. Модули, которые явно не нужны, удаляются. В нашем примере это модуль [Split Data][split] (Разделение данных), второй модуль [Score Model][score-model] (Оценка модели) и модуль [Evaluate Model][evaluate-model] (Анализ модели).
...
* **Оценка.** Чтобы разделить поток данных на набор проверочных данных и набор данных для обучения, в этом примере используется модуль "Разделение". В прогностическом эксперименте он не требуется, и его можно удалить. Аналогично второй модуль [Score Model][score-model] (Оценка модели) и модуль [Evaluate Model][evaluate-model] (Анализ модели) используются для сравнения результатов проверочных данных, поэтому эти модули также не нужны в прогностическом эксперименте. Но оставшийся модуль [Score Model][score-model] (Оценка модели) необходим для возврата результатов оценки через веб-службу.
...
Аналогичным образом при нажатии кнопки **Настройка веб-службы** модуль выходных данных веб-службы по умолчанию помещается в нижнюю часть потока данных. В этом примере веб-служба будет возвращать пользователю выходные данные модуля [Score Model][score-model] (Оценка модели), который включает полный вектор входных данных и результаты оценки.

Но если вам необходимо, чтобы возвращались другие данные, например только результаты оценки, а не весь вектор входных данных, можно вставить модуль [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных), чтобы исключить все столбцы, кроме результатов оценки. Затем переместите модуль **Web service output** (Выходные данные веб-службы) в выходные данные модуля [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).
...
Дополнительные сведения о параметрах веб-службы см. в статье [Параметры веб-службы машинного обучения Azure][webserviceparameters].
