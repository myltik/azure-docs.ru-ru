...
Это второй шаг из пошагового руководства [Разработка прогнозирующего решения с помощью Машинного обучения Azure](machine-learning-walkthrough-develop-predictive-solution.md).
...
Эти данные будут использоваться для обучения модели прогнозной аналитики. По завершении наша модель должна уметь принимать сведения о новых претендентах и прогнозировать для них степень кредитного риска.
...
1.	Войдите в Студию машинного обучения ([https://studio.azureml.net](https://studio.azureml.net)), используя учетную запись Майкрософт, которая была указана в качестве владельца рабочей области, и щелкните вкладку **Студия** вверху.
2.	В нижней части окна щелкните **+СОЗДАТЬ**.
3.	Выберите **НАБОР ДАННЫХ**.
4.	Выберите **ИЗ ЛОКАЛЬНОГО ФАЙЛА**.
5.	В диалоговом окне **Отправка нового набора данных** нажмите кнопку **Обзор** и найдите созданный ранее файл **german.csv**.
6.	Введите имя для набора данных. В этом примере мы назовем его "UCI German Credit Card Data".
7.	Для типа данных выберите **Универсальный CSV-файл без заголовка (.nh.csv)**.
8.	При желании добавьте описание.
9.	Нажмите кнопку **ОК**.  
