<properties
   pageTitle="Прогнозирование ответа с помощью простой модели | Microsoft Azure"
   description="Узнайте, как создать простую модель для прогнозирования стоимости бриллиантов. В этом вам поможет видео 4 из цикла ";Обработка и анализ данных для начинающих";. Содержит базовую линейную регрессию с целевыми данными."                                  
...
# Обработка и анализ данных для начинающих (видео 4): прогнозирование ответа с помощью простой модели
...
* Данные являются **соответствующими**: масса определенно связана с ценой.
* Они **точные**: мы перепроверили цены, которые записали.
* Они **подключены**: все столбцы и строки заполнены.
...
Теперь мы возьмем эти данные и превратим их в *точечную диаграмму*. Это отличный способ визуализации числовых наборов данных.
...
Тот факт, что линия не проходит через все точки, является нормальным. Специалисты по обработке данных объясняют это следующим образом: существует модель (в нашем случае — линия), и каждая точка в модели подвержена воздействию некоторого *шума* или *отклонения*, связанного с ней. Есть базовая функциональная связь, а есть переменчивый реальный мир, добавляющий шум и неопределенность.
...
Было бы логичным проверить, насколько точен этот прогноз. Полезно знать, будет ли бриллиант массой 1,35 карата стоить ровно 10 000 долларов США, а может быть гораздо больше или меньше. Чтобы это определить, давайте нарисуем вокруг линии регрессии так называемый конверт, который будет включать в себя большинство точек. Этот конверт является нашим *доверительным интервалом*: мы можем быть уверены, что цены будут попадать в указанный диапазон, так как это происходило с большинством цен в прошлом. Можно провести еще две горизонтальные линии от точек пересечения линии, проведенной от отметки 1,35 карата, с верхней и нижней границами данного конверта.

![Доверительный интервал](./media/machine-learning-data-science-for-beginners-predict-an-answer-with-a-simple-model/confidence-interval.png)
