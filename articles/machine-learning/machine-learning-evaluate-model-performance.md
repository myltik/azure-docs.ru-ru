...
В этом эксперименте мы будем использовать популярный набор данных [Iris](http://archive.ics.uci.edu/ml/datasets/Iris "Iris"), который содержит экземпляры трех различных типов (классов) растения ирис. Для каждого экземпляра существует 4 значения признаков: длина и ширина чашелистика и длина и ширина лепестка. В предыдущих экспериментах мы обучали и тестировали модели, используя те же наборы данных. Здесь мы будем использовать модуль [разделения][split], чтобы создать два подмножества данных: первый набор данных будем обучать, а второй — подсчитывать и оценивать. Набор данных Iris находится в открытом доступе в [Репозитории машинного обучения UCI](http://archive.ics.uci.edu/ml/index.html). Его можно скачать с помощью модуля [Читатель][reader].
...
Щелкните модуль [Читатель][reader] и установите для свойства *Источник данных* значение *URL-адрес веб-сайта через HTTP*, а для свойства *URL-адрес* — значение http://archive.ics.uci.edu/ml/machine-learning-databases/iris/iris.data.

Укажите дробное число экземпляров, которые будут использоваться для обучения модуля [Разделение][split] (например, 0,7).
