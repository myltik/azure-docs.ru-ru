...
Виртуальная машина Linux для обработки и анализа данных — это виртуальная машина Azure, на которой предварительно установлен ряд инструментов, обычно необходимых для анализа данных и машинного обучения. К основным установленным приложениям относятся:
...
- Средства машинного обучения
    ...
    - [Rattle](http://rattle.togaware.com/) (аналитический инструмент R для легкого обучения): инструмент, который облегчает начало работы с анализом данных и машинным обучением в R с помощью графического интерфейса, в котором осуществляется исследование данных и моделирование с автоматическим созданием кода R.
...
3.	 В следующих разделах приведены **входные данные** для всех **5 шагов** мастера (номера см. справа на рисунке выше) по созданию виртуальной машины Майкрософт для обработки и анализа данных. Ниже приведено описание входных данных, необходимых для настройки на каждом из шагов.
  ...
  **а. Основные сведения**.
   ...
   - **Подписка** — при наличии нескольких подписок выберите ту, в которой будет создана виртуальная машина и для которой будет взиматься плата. Примечание. Вам необходимо иметь права на создание ресурсов для этой подписки.
...
При желании для установки [20 самых популярных пакетов R](http://www.kdnuggets.com/2015/06/top-20-r-packages.html) можно воспользоваться R-скриптом. Этот скрипт можно запустить в интерактивном интерфейсе R, для подключения к которому, как уже упоминалось, необходимо ввести *R* в оболочке.
...
В пакете драйвера ODBC для Microsoft SQL Server также содержатся две программы командной строки:
...
>[AZURE.NOTE] Эта служебная программа немного отличается для платформ Windows и Linux. Дополнительные сведения см. на страницах документации.
...
- **Microsoft Azure Storage Explorer**. Это графический инструмент, который позволяет просматривать объекты, сохраненные в учетной записи хранения Azure, а также отправлять и скачивать данные больших двоичных объектов Azure. Для доступа к обозревателю хранилища воспользуйтесь значком на рабочем столе. Его можно открыть из командной строки, введя ***StorageExplorer***. Для этого необходимо выполнить вход в клиенте X2Go или настроить перенаправление X11.
- **Библиотеки Azure**. Ниже перечислены некоторые библиотеки, которые уже установлены и готовы к использованию.
...
* **R** — для R существует обширный набор функций машинного обучения. В число предустановленных входят библиотеки lm, glm, randomForest и rpart. Другие библиотеки можно установить, выполнив команду
...
Vowpal Wabbit — система машинного обучения, которая раздвигает границы машинного обучения с помощью таких методов, как сетевой метод, хэширование, общее сокращение, сокращение, обучение для поиска, активное и интерактивное обучение.
...
Эта библиотека, так же как и библиотека R, предоставляется в виде программы командной строки.
...
#### Rattle
Rattle (**R** **A**nalytical **T**ool **T**o **L**earn **E**asily) — аналитический инструмент R для легкого обучения, который облегчает интеллектуальный анализ в R с помощью графического интерфейса для исследования и моделирования. Он предоставляет статистические и визуальные сводные данные, преобразует данные для удобного моделирования, создает контролируемые и неконтролируемые модели на основе данных, показывает производительность моделей в графическом виде и оценивает новые наборы данных. Он также формирует код R, соответствующий операциям в пользовательском интерфейсе. Этот код можно запустить непосредственно в R или использовать в качестве отправной точки для дальнейшего анализа.
...
Откроется графический интерфейс с набором вкладок. Вот список действий в Rattle, которые позволят построить модель на основе примера данных о погоде. В некоторых действиях будет предложено автоматически скачать и установить необходимые пакеты R, которые еще не установлены в системе.
...
2. Откроется диалоговое окно с вопросом, хотите ли вы использовать пример данных о погоде. Нажмите кнопку **Yes** (Да), чтобы загрузить пример.
3. Перейдите на вкладку **Model** (Модель).
4. Нажмите кнопку **Execute** (Выполнить) для построения дерева принятия решений.
5. Нажмите кнопку **Draw** (Рисование) для отображения дерева принятия решений.
...
9. Перейдите на вкладку **Log** (Журнал), чтобы создать код R для указанных выше операций (примечание: в текущем выпуске Rattle есть ошибка — вставьте знак *#* перед строкой *Export this log ...* в журнале).
10. Нажмите кнопку **Export** (Экспортировать), чтобы сохранить R-скрипт в файл с именем *weather\_script.R* в домашней папке.
