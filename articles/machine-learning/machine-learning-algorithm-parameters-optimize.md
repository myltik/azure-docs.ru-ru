...
Тот же модуль [Гиперпараметры модели настройки][tune-model-hyperparameters] обучает все модели, которые соответствуют набору параметров, оценивает различные метрики, а затем выводит оптимально обученную модель на основе выбранной пользователем метрики. Для этого модуля необходимо обязательно предоставить следующее:
