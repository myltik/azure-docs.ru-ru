...
>[!TIP]
> Размеры виртуальных машин, которые начинаются с NC, оснащены графическим процессором.
...
Виртуальные машины Linux в Azure обычно оснащаются диском операционной системы на 30 ГБ. При использовании в качестве целевого объекта вычислений для Машинного обучения Azure он может быстро расходоваться модулем Docker, который извлекает образы Docker и создает на его основе слои Conda. Пока не завершено выполнение, рекомендуется расширить диск ОС до большего размера (например, до 200 ГБ) во избежание ошибки "Диск заполнен". В статье [Расширение виртуальных жестких дисков на виртуальной машине Linux с помощью Azure CLI](../../virtual-machines/linux/expand-disks.md), показано, как это легко сделать с помощью azure-cli. 
