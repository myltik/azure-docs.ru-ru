...
* Установленная копия [Azure Machine Learning Workbench](./overview-what-is-azure-ml.md). Чтобы установить эту программу и создать рабочую область, выполните инструкции из [краткого руководства по установке](./quickstart-installation.md). Если у вас несколько подписок, вы можете [задайте нужный подписка должна текущей активной подписки](https://docs.microsoft.com/cli/azure/account?view=azure-cli-latest#az_account_set).
* Windows 10 (если вы используете macOS, большинство инструкций будут аналогичными).
* Данные обработки и анализа виртуальной машины (DSVM) для Linux (Ubuntu), предпочтительно в регионе Восток США, где находит данные. Вы можете подготовить виртуальную машину для обработки и анализа данных Ubuntu, выполнив эти [инструкции](https://docs.microsoft.com/azure/machine-learning/data-science-virtual-machine/dsvm-ubuntu-intro). Просмотрите также это [краткое руководство](https://ms.portal.azure.com/#create/microsoft-ads.linux-data-science-vm-ubuntulinuxdsvmubuntu). Рекомендуем использовать виртуальную машину с как минимум 8 ядрами и 32 ГБ памяти. 

Выполните [инструкции](https://docs.microsoft.com/en-us/azure/machine-learning/preview/known-issues-and-troubleshooting-guide#remove-vm-execution-error-no-tty-present) для включения доступа sudoer без пароля на виртуальной Машине для AML рабочей среды.  Вы можете использовать [на основе ключей проверки подлинности SSH для создания и использования виртуальной Машины в среде AML](https://docs.microsoft.com/en-us/azure/machine-learning/preview/experimentation-service-configuration#using-ssh-key-based-authentication-for-creating-and-using-compute-targets). В этом примере мы используем пароль для доступа к виртуальной Машине.  Сохраните таблицу ниже с данными виртуальной машины для обработки и анализа данных для выполнения дальнейших действий:
...
Данные, которые используются в этом примере, — это синтезированные данные рабочей нагрузки сервера, Она размещается в учетную запись хранилища больших двоичных объектов Azure, публично доступны в регионе Восток США. Сведения учетной записи системы хранения можно найти в `dataFile` поле [ `Config/storageconfig.json` ](https://github.com/Azure/MachineLearningSamples-BigData/blob/master/Config/fulldata_storageconfig.json) в формате «wasb: / /<BlobStorageContainerName>@<StorageAccountName>.blob.core.windows.net/<path>». Можно использовать данные непосредственно из хранилища BLOB-объектов. Если хранилище используется несколькими пользователями одновременно, можно использовать [azcopy](https://docs.microsoft.com/azure/storage/common/storage-use-azcopy-linux) для загрузки данных в хранилище для повышения удобства экспериментов. 
...
-  myhdi.COMPUTE: этот файл содержит сведения о соединении и конфигурации для удаленного выполнения целевого объекта.
