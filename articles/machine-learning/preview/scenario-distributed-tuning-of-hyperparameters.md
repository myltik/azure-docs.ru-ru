...
Популярный метод настройки гиперпараметров — *поиск в сетке* в сочетании с *перекрестной проверкой*. Перекрестная проверка — это метод, с помощью которого можно оценить, насколько хорошо модель, обученная на учебном наборе, выполняет прогнозирование по тестовому набору. При использовании этого метода мы изначально разделили набор данных на K сверток, а затем обучили алгоритм K раз путем циклического перебора со всеми свертками, кроме одной, которая называется контрольной сверткой. Мы вычислили среднее значение метрик K моделей и K контрольных сверток. Это среднее значение, называемое *оценкой эффективности путем перекрестной проверки*, зависит от значений гиперпараметров, используемых при создании K моделей. При настройке гиперпараметров мы выполняем поиск в пространстве потенциальных значений гиперпараметров, чтобы найти те из них, которые оптимизируют оценку эффективности путем перекрестной проверки. Поиск в сетке — это распространенный способ поиска, при котором пространство потенциальных значений нескольких гиперпараметров является векторным произведением наборов потенциальных значений отдельных гиперпараметров. 

Поиск в сетке с помощью перекрестной проверки может занимать много времени. Если алгоритм включает 5 гиперпараметров, каждый из которых имеет 5 потенциальных вариантов значений, и мы используем пять сверток (K = 5), то для поиска в сетке нам необходимо обучить 5<sup>6</sup> = 15 625 моделей. К счастью, поиск в сетке с помощью перекрестной проверки является процедурой с массовым параллелизмом, поэтому все модели можно обучить в параллельном режиме.
...
* Чтобы выполнить этот сценарий в кластере Spark, подготовьте кластер HDInsight, следуя этим [инструкциям](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-hadoop-provision-linux-clusters). Рекомендуем использовать кластер с как минимум 6 рабочими узлами, 8 ядрами и 28 ГБ памяти как на головном узле, так и на рабочих узлах. Такими характеристиками обладают экземпляры виртуальных машин D4. Чтобы добиться максимальной производительности кластера, рекомендуем изменить параметры spark.executor.instances, spark.executor.cores и spark.executor.memory, а также изменить определения в разделе Custom spark-defaults (Настраиваемые значения Spark по умолчанию), следуя этим [инструкциям](https://docs.microsoft.com/en-us/azure/hdinsight/hdinsight-apache-spark-resource-manager).
...
На следующих шагах мы подключим среду выполнения к учетной записи Azure. Откройте окно командной строки (CLI), щелкнув меню File (Файл) в левом верхнем углу AML Workbench и выбрав пункт Open Command Prompt (Открыть командную строку). Затем выполните в CLI следующую команду.
...
и найдите идентификатор подписки Azure, включающей учетную запись рабочей области AML Workbench. Наконец, выполните в CLI следующую команду:
...
После этого выберите контейнер набора данных из списка и нажмите кнопку "Отправить". Через портал Azure можно одновременно отправить несколько файлов. В разделе "Отправить BLOB-объект" щелкните значок папки, выберите все файлы из набора данных, нажмите кнопку "Открыть", а затем — кнопку "Отправить". Эти действия показаны на следующем снимке экрана:
...
Обратите внимание, что файл load_data.py не нужно запускать вручную. Позже он будет вызываться из других файлов.
...
* признаки-индикаторы для категорий приложений, с помощью которых пользователь создавал события (функция one\_hot_category).
...
* colsample\_by_level;
* subsample.
* Описание цели этих гиперпараметров см. [здесь](http://xgboost.readthedocs.io/en/latest/python/python_api.html#module-xgboost.sklearn) и [здесь](https://github.com/dmlc/xgboost/blob/master/doc/parameter.md). Сначала мы используем удаленную виртуальную машину для обработки и анализа данных и настроим гиперпараметры из небольшой сетки потенциальных значений:
...
По умолчанию в окне журнала выполнения отображаются значения и графики для первых 1–2 зарегистрированных значений. Чтобы просмотреть полный список выбранных значений гиперпараметров, щелкните значок настроек, обведенный красным на предыдущем снимке экрана, и выберите гиперпараметры, которые следует отобразить в таблице. Чтобы выбрать графики, которые будут отображаться в верхней части окна журнала выполнения, щелкните значок настроек, обведенный синим, и выберите графики из списка. 
...
В правом верхнем углу окна Run Properties (Свойства запуска) есть раздел Output Files (Выходные файлы) со списком всех файлов, которые были созданы в папке .\output в среде выполнения. Чтобы скачать файл sweeping\_results.txt из этой папки, выберите его и нажмите кнопку скачивания. Файл sweeping_results.txt должен содержать следующие выходные данные:
...
Выполнение этой команды в кластере Spark с 6 рабочими узлами и 28 ГБ памяти занимает 1 час 6 минут. Доступ к результатам настройки гиперпараметров в кластере Spark (журналам, оптимальным значениям гиперпараметров и файлу sweeping_results.txt) можно получить в Azure Machine Learning Workbench, так же как и при настройке гиперпараметров на удаленной виртуальной машине для обработки и анализа данных. 

### <a name="architecture-diagram"></a>Схема архитектуры

На следующей схеме показан рабочий процесс от начала до конца. ![Архитектура](media/scenario-distributed-tuning-of-hyperparameters/architecture.png) 

## <a name="conclusion"></a>Заключение 

В этом сценарии мы показали, как с помощью Azure Machine Learning Workbench выполнить настройку гиперпараметров на удаленной виртуальной машине и в удаленном кластере Spark. Мы узнали, что Azure Machine Learning Workbench предоставляет средства для простой настройки сред выполнения и переключения между ними. 
