...
4. Демонстрация следующих возможностей Azure Machine Learning Workbench:
    ...
    * выполнение кода в записных книжках Jupyter, а также скриптов Python;
    * отслеживание журналов выполнения для файлов Python;
    ...
    * простая реализация моделей глубинного обучения в виде веб-служб в службах контейнеров Azure.
...
* извлечение данных о заболеваниях и симптомах из электронных медицинских или санитарных карт;
...
* обучение нейронной модели векторного представления слов (см. дополнительные сведения о начале работы по [ссылке на код на сайте GitHub](https://github.com/Azure/MachineLearningSamples-BiomedicalEntityExtraction/blob/master/Code/01_DataPreparation/ReadMe.md)).
...
См. раздел о [создании признаков](https://github.com/Azure/MachineLearningSamples-BiomedicalEntityExtraction/tree/master/Code/02_Modeling/01_FeatureEngineering).

Word2Vec — это алгоритм обучения векторных представлений слов, с помощью которого обучаются модель нейронной сети в совокупности учебных данных без меток. В результате создается непрерывный вектор для каждого слова из совокупности, представляющий семантические данные о слове. Эти модели представляют собой простые нейронные сети с одним скрытым слоем. Векторные представления слов обучаются по методам обратного распространения ошибки и вероятностного градиентного спуска. Существует два типа моделей Word2Vec: Skip-Gram и CBOW (Continuous Bag of Words). Дополнительные сведения см. [здесь](https://arxiv.org/pdf/1301.3781.pdf). Так как мы используем алгоритм Word2Vec, реализованный MLlib, который поддерживает модель Skip-Gram, мы приведем краткое описание модели ниже (изображение взято по [этой](https://ahmedhanibrahim.wordpress.com/2017/04/25/thesis-tutorials-i-understanding-word2vec-for-word-embedding-i/) ссылке). 
...
См. раздел об [обучении нейронного средства для извлечения сущностей](https://github.com/Azure/MachineLearningSamples-BiomedicalEntityExtraction/tree/master/Code/02_Modeling/02_ModelCreation/ReadMe.md).
