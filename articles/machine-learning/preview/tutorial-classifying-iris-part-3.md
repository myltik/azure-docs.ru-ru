...
2. Создайте среду. Этот шаг однократно выполняется для каждой среды. Например, он один раз выполняется для среды разработки и один раз — для производства. Для первой среды используйте _локальный режим_. Позже вы можете подставить в эту команду параметры `-c` или `--cluster`, чтобы настроить среду в _режиме кластера_.
   ...
   Чтобы убедиться, что это окружение настроено, выполните следующую команду для проверки состояния:

   ```azurecli
   az ml env show -n <deployment environment name> -g <existing resource group name>
   ```

   У параметра "Состояние подготовки" должно быть значение "Успешно" (как показано ниже). Проверьте это, прежде чем продолжать настройку окружения на шаге 5.
