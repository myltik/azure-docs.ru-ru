...
С помощью [веб-службы Машинного обучения Azure](https://docs.microsoft.com/azure/machine-learning/preview/model-management-service-deploy) внешнее приложение синхронно обменивается данными с прогнозной моделью с помощью вызова HTTP POST к URL-адресу службы. Чтобы вызвать веб-службу, клиентскому приложению необходимо указать ключ API, который создается при развертывании прогноза, и поместить данные запроса в тело запроса POST.
