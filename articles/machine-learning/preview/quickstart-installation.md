...
1. В области **Экспериментирование в Машинном обучении** настройте учетную запись службы "Экспериментирование в Машинном обучении". 
   ...
   Учетная запись хранения | _Уникальное имя_ | Выберите **Создать** и укажите имя, чтобы создать [учетную запись хранения Azure](https://docs.microsoft.com/en-us/azure/storage/common/storage-quickstart-create-account?tabs=portal). Вы также можете щелкнуть **Использовать существующий** и выбрать имеющуюся учетную запись хранения из раскрывающегося списка. Учетная запись хранения является обязательным ресурсом. В ней хранятся артефакты проекта и данные журнала выполнения. 
   Рабочая область для учетной записи Экспериментирования | IrisGarden | Введите имя для рабочей области в этой учетной записи. Имя должно содержать от 2 до 32 алфавитно-цифровых символов и дефис (-). Такая рабочая область содержит инструменты, необходимые для создания, публикации экспериментов и управления ими.
...
5. Выберите **Создать**, чтобы начать процесс создания учетной записи службы "Экспериментирование" и учетной записи службы "Управление моделями".
   ...
   Создание учетной записи может занять несколько минут. Вы можете проверить состояние процесса развертывания, щелкнув колокольчик на панели инструментов Azure.
...
1. Следуйте инструкциям на экране, чтобы выполнить установку. 
   ...
   **Установка может занять до 30 минут.**  
   ...
   Программа установки скачивает и настраивает все необходимые зависимости, например Python, Miniconda и другие связанные библиотеки.    Эта установка также включает программу командной строки Azure (Azure CLI), которая работает на всех платформах.

  
1. Запустите Workbench, нажав кнопку **Launch Workbench** (Запуск Workbench) на последней странице программы установки. 

   При закрытии установщик это не проблема. 
...
1. На первом экране выберите **Войти** для проверки подлинности с помощью Azure Machine Learning Workbench. Используйте те же учетные данные, использованные на портале Azure, для создания учетных записей службы "Экспериментирование" и службы "Управление моделями". 
...
В службе "Машинное обучение Azure" проект представляет собой логический контейнер для всех действий, выполняемых для решения проблемы. Он сопоставляется с отдельной папкой с файлами на локальном диске, в которую можно добавить любые файлы и вложенные папки. 
...
>[!TIP]
>Вы можете настроить Workbench для работы с интегрированной средой разработки Python, чтобы обеспечить плавную обработку и анализ данных. Затем можно взаимодействовать с проектом в интегрированной среде разработки. [Подробнее](how-to-configure-your-IDE.md). 

## <a name="run-a-python-script"></a>Выполнение скрипта Python

Теперь можно запустить скрипт **iris_sklearn.py** на локальном компьютере. Этот скрипт включен по умолчанию с шаблоном проекта **Classifying Iris** (Классификация цветков ириса). Этот скрипт создает модели с алгоритмом [логистической регрессии](https://en.wikipedia.org/wiki/logistic_regression) из популярной библиотеки Python [scikit-learn](http://scikit-learn.org/stable/index.html).
...
1. В текстовом поле **Аргументы** введите **0,01**. Это число используется в коде скрипта для установки скорости регуляризации. Это значение используется для настройки способа обучения модели линейной регрессии. 

1. Выберите **Запустить** для начала выполнения сценария на компьютере. Задание **Iris_sklearn** сразу же отображается на панели **Задания** справа, где можно отслеживать выполнение скрипта.

   Поздравляем! Вы успешно выполнили скрипт Python в Azure Machine Learning Workbench.

1. Повторите шаги 2–3 несколько раз, используя другой аргумент значения в диапазоне от **0,001** до **10**. Каждое задание выполнения отображается в области **Задания**.

1. Проверьте журнал выполнения, выбрав представление **Выполнения**, а затем **iris_sklearn.py** в списке выполнений, чтобы показать журнал выполнения для этого сценария. 

   ![Панель мониторинга журнала выполнения](media/quickstart-installation/run_view.png)

   Здесь вы увидите все запуски, выполненные для **iris_sklearn.py**. Также панель мониторинга с журналом выполнения содержит самые лучшие метрики, набор диаграмм по умолчанию и список метрик для каждого запуска. 

1. Это представление можно настроить путем сортировки, фильтрации и настройки конфигураций, используя значок шестеренки или фильтра.
...
Кроме того, устанавливается CLI службы "Машинное обучение Azure". Интерфейс CLI дает возможность доступа к службам "Машинное обучение Azure" и взаимодействия с ними с помощью команд `az` для выполнения всех задач, необходимых для обработки и анализа данных от начала и до конца рабочего процесса. [Подробнее.](tutorial-iris-azure-cli.md)

Интерфейс командной строки azure-cli-ml можно запустить с панели инструментов Workbench с помощью **File --> Open Command Prompt** (Файл --> Открыть командную строку).

Кроме того, можно получить справку о командах в CLI azure-cli-ml с помощью аргумента --help.
...
Вы создали необходимые учетные записи службы "Машинное обучение Azure" и установили приложение Azure Machine Learning Workbench. В приложении, где создан проект, вы выполнили скрипт и изучили журнал выполнения скрипта.
...
> [Часть 1. Классификация цветков ириса: подготовка данных](tutorial-classifying-iris-part-1.md)

>[!NOTE]
> Несмотря на наличие учетной записи управления моделью, среда еще не настроена для развертывания веб-служб.  Узнайте, как настроить [среду развертывания](deployment-setup-configuration.md).