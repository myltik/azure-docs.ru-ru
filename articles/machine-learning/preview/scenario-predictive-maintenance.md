...
Этот сценарий более подробно рассматривается относительно [крупномасштабных имитацию набора данных](https://github.com/Microsoft/SQL-Server-R-Services-Samples/tree/master/PredictiveMaintanenceModelingGuide/Data) пошагового превентивного обслуживания проекта обработки и анализа данных из приема данных, возможностей разработки, построения модели и модели ввода в эксплуатацию и развертывание. Код для всего процесса записывается в записные книжки Jupyter, с помощью PySpark Azure ML рабочей среде. Окончательной модели развертывается с помощью службы управления Azure машины обучения модели для прогнозирования сбой оборудования в реальном времени.   
...
Этот сценарий принимает идеи [репертуара превентивного обслуживания](https://docs.microsoft.com/en-us/azure/machine-learning/team-data-science-process/cortana-analytics-playbook-predictive-maintenance) для демонстрации создания прогнозной модели для имитации набора данных. Пример данных является производным от наблюдается во многих случаях диагностическое обслуживание используйте Общие элементы.
...
* Установленная копия [Azure Machine Learning Workbench](./overview-what-is-azure-ml.md) следующие [руководство по установке краткое руководство](./quickstart-installation.md) установить данную программу и создать рабочую область.
...
Чтобы запустить проект на локальном компьютере, в меню `File` AML Workbench выберите пункт `Open Command Prompt` или `Open PowerShell CLI`. Предоставляет доступ к службам Azure с помощью интерфейса CLI `az` команд. Во-первых, выполните вход в учетную запись Azure с помощью команды:

```
az login
``` 

Эта команда предоставляет ключ проверки подлинности для использования с `https:\\aka.ms\devicelogin` URL-адрес. CLI ожидает, пока возвращает операция входа в устройство и некоторые сведения о соединении. Затем, если у вас локальный [docker](https://www.docker.com/get-docker) установки, подготовьте среду локальных вычислений с помощью следующих команд:

```
az ml experiment prepare --target docker --run-configuration docker
```

Рекомендуется выполнять на [виртуальная машина анализа данных для Linux (Ubuntu)](https://azuremarketplace.microsoft.com/en-us/marketplace/apps/microsoft-ads.linux-data-science-vm-ubuntu) требования к памяти и диска. После настройки DSVM подготовьте среду удаленных docker с помощью следующих двух команд:

```
az ml computetarget attach remotedocker --name [Connection_Name] --address [VM_IP_Address] --username [VM_Username] --password [VM_UserPassword]
```

После подключения к контейнеру удаленного docker Подготовка DSVM docker среды вычислений с использованием: 

```
az ml experiment prepare --target [Connection_Name] --run-configuration [Connection_Name]
```

С помощью docker вычислений среда подготовлены, откройте блокнота jupyter, либо на вкладке записных книжек AML Workbench или запустить сервер на основе браузера с: 
```
az ml notebook start
```

Пример ноутбуков, хранятся в `Code` каталога. Портативные компьютеры настроены на выполняются последовательно, начиная с первого (`Code\1_data_ingestion.ipynb`) ноутбука. При открытии каждой ноутбук, вам будет предложено выбрать ядра вычисления. Выберите `[Project_Name]_Template [Connection_Name]` ядра для выполнения на ранее настроенном DSVM.
...
* [Данные телеметрии](https://pdmmodelingguide.blob.core.windows.net/pdmdata/telemetry.csv): данные телеметрии, данные состоят из временных рядов меры из нескольких датчиков внутри каждого компьютера. Усреднение значений датчика за каждый час интервал записываются данные.
...
[Readme](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/README.md) файл описывает рабочий процесс подготовки данных, построение модели и затем развернуть это решение для производства. Каждый шаг рабочего процесса, содержащийся в записной книжке Jupyter в [кода](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/tree/master/Code) папки в репозитории.   

[`Code\1_data_ingestion.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/1_data_ingestion.ipynb) загружает пять входных CSV-файлов, выполняет предварительную очистку данных и визуализацию. Записной книжки преобразует каждый набор данных в формате PySpark и сохраняет его в контейнере больших двоичных объектов для использования в записной книжке разработки компонентов.

[`Code\2_feature_engineering.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/2_feature_engineering.ipynb): С помощью необработанного набора данных из больших двоичных объектов Azure, функции формируются с помощью ряда подход время (зима) для данных телеметрии, ошибки и обслуживания модели. Сбой связан компонент замены используются для создания модели метки, описывающее, какой компонент не удалось. Данные с меткой компонентов сохраняется в большом двоичном объекте Azure построения записной книжки модели.
...
[`Code\4_operationalization.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/4_operationalization.ipynb): С помощью последней модели сохранен на локальном диске (kernel записной книжки Jupyter), записной книжки создает компоненты для развертывания модели в Azure веб-службы. Весь набор ресурсов для ввода в эксплуатацию сжимается в файл `o16n.zip`, который сохраняется в другом контейнере больших двоичных объектов Azure. Сжатый ZIP-файл содержит следующие данные.
...
Этот сценарий предоставляет пользователю общие сведения о создании комплексного решения для прогнозного обслуживания с помощью PySpark в среде записной книжки Jupyter в Azure ML Workbench. Этот пример также подробные сведения о модели развертывания с помощью среды Azure машины обучения модели управления делать прогнозы сбой оборудования в реальном времени.
