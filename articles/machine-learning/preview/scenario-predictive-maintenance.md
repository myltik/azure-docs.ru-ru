...
* Для практического применения машинного обучения Azure требуется локальная среда развертывания и [учетная запись службы "Управление моделями"](https://docs.microsoft.com/en-us/azure/machine-learning/preview/model-management-overview).
...
Чтобы запустить проект на локальном компьютере, в меню `File` AML Workbench выберите пункт `Open Command Prompt` или `Open PowerShell CLI`. В окне CLI выполните следующие команды:

`az ml experiment prepare --target docker --run-configuration docker`

Мы рекомендуем использовать виртуальную машину Linux (Ubuntu) для обработки и анализа данных. Когда виртуальная машина для обработки и анализа данных будет настроена, выполите две следующие команды.
...
Подготовив образы Docker, откройте сервер записных книжек Jupyter, используя вкладку записных книжек в AML Workbench, или запустите веб-сервер с помощью команды `az ml notebook start`.

Записные книжки хранятся в каталоге `Code` в среде Jupyter. Мы будем запускать записные книжки последовательно, начиная с первой (`Code\1_data_ingestion.ipynb`). При открытии каждой записной книжки вам предлагается выбор ядра для вычислений. Выберите элемент "[имя_проекта]_шаблон [желаемое_имя_подключения]" и щелкните Set Kernel (Задать ядро).
...
Далее мы пошагово рассмотрим рабочий процесс этого сценария. Полный сценарий написан с помощью PySpark и разделен на следующие четыре записные книжки.

[`Code\1_data_ingestion.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/1_data_ingestion.ipynb) загружает пять входных CSV-файлов, выполняет предварительную очистку данных и визуализацию. Эта записная книжка преобразует данные в формат PySpark и сохраняет результаты в контейнере больших двоичных объектов Azure, чтобы их можно было использовать в задаче разработки компонента.

[`Code\2_feature_engineering.ipynb`](https://github.com/Azure/MachineLearningSamples-PredictiveMaintenance/blob/master/Code/2_feature_engineering.ipynb) использует набор данных, очищенный на предыдущем шаге, чтобы подготовить задержки и статистические данные по датчикам телеметрии, а также включить в данные телеметрии сведения о количестве ошибок, заменах компонентов и параметрах компьютеров. На основе сведений о заменах компонентов, вызванных сбоями, создаются метки с описанием неисправных компонентов. Данные о компонентах с метками сохраняются в большом двоичном объекте Azure для задачи построения модели.
