...
-	В предикате для ByRow **s** — параметр, представляющий индекс прямоугольного массива узлов входного слоя Pixels, а **d** — параметр, представляющий индекс массива узлов скрытого слоя ByRow. Тип обоих **s** и **d** представляет собой кортеж целых чисел с длиной два. Концептуально, **s** находится в диапазоне всех пар целых чисел с 0 <= s[0] < 10 и 0 <= s[1] < 20, в то время как **d** находится в диапазоне всех пар целых чисел с 0 <= d[0] < 10 и 0 <= d[1] < 12. 
-	На правой стороне выражения предиката находится условие. В этом примере для каждого значения **s** и **d**, условие которого имеет значение True, предусмотрен переход из узла слоя источника в узел слоя назначения. Это выражение фильтра указывает, что пакет содержит подключение от узла, определенного параметром **s**, к узлу, определенному параметром **d**, во всех случаях, где s[0] равно d[0].  
...
-	Скрытые слои ByRow и ByCol определяют фильтрованные подключения с использованием выражений предиката. Если говорить точнее, то узел в ByRow с координатами [x, y]подключается к тем узлам в Pixels, у которых первая координата индекса равна первой координате узла (x). Аналогично узел ByCol с координатами [x, y] подключается к тем узлам в Pixels, у которых вторая координата индекса находится в пределах единицы от второй координаты узла (у).  
