...
# Как использовать веб-службу Машинного обучения Azure, опубликованную из эксперимента Машинного обучения
...
Информацию о том, как создать и опубликовать веб-службу машинного обучения Azure, см. в разделе [Публикация веб-службы машинного обучения Azure][publish]. Пошаговое руководство по созданию эксперимента машинного обучения и его публикации см. в разделе [Разработка прогнозирующего решения с помощью машинного обучения Azure][walkthrough].
...
### Пример BES
На странице справки по API кроме универсального кода ресурса вы найдете информацию о нескольких доступных вызовах. В отличие от службы RRS служба BES является асинхронной. Это означает, что BES API просто ставит в очередь задание, а вызывающий объект запрашивает состояние задания, чтобы увидеть, когда оно будет завершено. Ниже перечислены операции, которые в настоящее время поддерживаются для пакетных заданий:
...
* **Входные данные**: представляет собой ссылку большого двоичного объекта на место хранения входных данных пакетного задания.
...
* **Выходные данные**: если служба определила один или несколько массивов выходных данных, вызывающий объект может перенаправлять любой из них по месту расположения большого двоичного объекта Azure по своему выбору. Это позволит сохранить выходные данные службы в желаемом месте и с предсказуемым именем, тогда как в ином случае имя выходного большого двоичного объекта выбирается случайным образом. **ОБРАТИТЕ ВНИМАНИЕ**, что служба ожидает выходное содержимое исходя из его типа с сохранением в поддерживаемых форматах:
  ...
  Переопределенные места расположения выходных данных указываются как коллекция *<output name  blob reference>* пар, где *имя выходных данных* определяется пользователем по конкретным узлам выходных данных (см. также страницу справки по API службы), а *ссылка на большой двоичный объект* представляет собой ссылку на место расположения большого двоичного объекта Azure, в которое будут перенаправляться выходные данные.
  
Все эти параметры создания задания могут быть необязательным в зависимости от службы. Например, службы, у которых входной узел не определен, не требуют переноса в параметр *Входные данные*, и возможность переопределения расположения выходных данных является необязательной, поскольку выходные данные будут храниться в учетной записи хранения по умолчанию, которая была создана для рабочей области машинного обучения Azure. Ниже показан пример запроса полезных данных, перенесенных в REST API, для службы, в которую были перенесены только входные данные:
...
#### Использование [BES SDK](machine-learning-consume-web-services.md#batch-execution-service-sdk)

[Пакет BES SDK Nugget](http://www.nuget.org/packages/Microsoft.Azure.MachineLearning/) предоставляет функции, упрощающие вызов BES для оценки в пакетном режиме. Чтобы установить пакет Nuget в Visual Studio, перейдите в меню инструментов, выберите диспетчер пакетов Nuget и щелкните консоль диспетчера пакетов.

Эксперименты AzureML, публикуемые в виде веб-служб, могут включать в себя входные модули веб-служб. Это означает, что ожидается предоставление входных данных через вызов веб-службы в форме ссылки на расположение большого двоичного объекта. Кроме того, имеется возможность не использовать входной модуль веб-службы, а воспользоваться модулем чтения. В этом случае средство чтения, как правило, считывает данные из БД SQL посредством запроса в процессе выполнения для получения данных. Параметры веб-службы можно использовать для динамического указания других серверов или таблиц и т. д. SDK поддерживает оба этих варианта.
