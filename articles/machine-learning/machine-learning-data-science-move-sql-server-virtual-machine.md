<properties 
	pageTitle="Перемещение данных в SQL Server на виртуальной машине | Azure"
...
В этом документе описаны варианты перемещения данных из неструктурированных файлов (форматов CSV или TSV) или из локального SQL Server в SQL Server на виртуальной машине Azure. Эти задачи перемещения данных в облако являются частью процесса обработки аналитических данных и технологии расширенного анализа (ADAPT) в рамках машинного обучения Azure.
...
В следующей таблице перечислены варианты перемещения данных в SQL Server на виртуальной машине Azure. <table>

<tr>
<td><b>ИСТОЧНИК</b></td>
<td colspan="2" align="center"><b>МЕСТО НАЗНАЧЕНИЯ: SQL Server на виртуальной машине Azure</b></td>
</tr>
...
<tr>
  ...
  <td>
    1. <a href="#deploy-a-sql-server-database-to-a-microsoft-azure-vm-wizard">Мастер развертывания Базы данных SQL Server на виртуальной машине Microsoft Azure</a><br>
    2. <a href="#export-flat-file">Экспорт в неструктурированный файл</a><br>
    3. <a href="#sql-migration">Мастер миграции баз данных SQL</a> <br>    
    4. <a href="#sql-backup">Восстановление и резервное копирование базы данных</a> <br>
  </td>
</tr>
</table>
...
* Подготовленный **SQL Server на виртуальной машине Azure**. Инструкции см. в статье [Настройка виртуальной машины SQL Server как сервера IPython Notebook для расширенной аналитики](machine-learning-data-science-setup-sql-server-virtual-machine.md).
...
## <a name="filesource_to_sqlonazurevm"></a> Перемещение данных из неструктурированного файла в SQL Server на виртуальной машине Azure
...
## <a name="sqlonprem_to_sqlonazurevm"></a>Перемещение данных из локального SQL Server в SQL Server на виртуальной машине Azure
...
4. [Восстановление и резервное копирование базы данных](#sql-backup)
...
**Мастер развертывания Базы данных SQL Server на виртуальной машине Microsoft Azure** представляет собой простой и рекомендуемый способ перемещения данных из локального экземпляра SQL Server в SQL Server на виртуальной машине Azure. Подробные инструкции, а также описание других вариантов, см. в разделе [Миграция базы данных в SQL Server на виртуальной машине Azure](../virtual-machines/virtual-machines-migrate-onpremises-database.md).

### <a name="export-flat-file"></a>Экспорт в неструктурированный файл

Для массового экспорта данных из локального SQL Server можно использовать различные способы, которые описаны [здесь](https://msdn.microsoft.com/library/ms175937.aspx). В этом документе в качестве примера описано использование программы массового копирования (BCP). После экспорта данных в неструктурированный файл этот файл можно импортировать в другой SQL Server с помощью операции массового импорта.
