...
Заголовки столбцов можно добавить с помощью модуля [Редактор метаданных][metadata-editor]. Модуль [Редактор метаданных][metadata-editor] используется для изменения метаданных, связанных с набором данных. В этом случае он позволяет предоставить более понятные имена заголовков столбцов.

Для использования [редактора метаданных][metadata-editor] сначала необходимо указать, какие столбцы следует изменить (в данном случае — все), а затем указать действие, выполняемое с этими столбцами (в данном случае — изменение заголовков столбцов).

1.	В палитре модуля введите "metadata" в поле **Поиск**. [Редактор метаданных][metadata-editor] появится в списке модулей.
2.	Щелкните и перетащите модуль [Редактор метаданных][metadata-editor] на холст и вставьте его под набором данных, добавленным ранее.
3.	Подключите набор данных к [редактору метаданных][metadata-editor]\: щелкните порт вывода набора данных (маленький кружок в нижней части набора данных), перетащите во входной порт [редактора метаданных][metadata-editor] (маленький кружок в верхней части модуля) и отпустите кнопку мыши. Набор данных и модуль останутся подключенными даже при их перемещении по полотну.
    ...
    > [AZURE.TIP] Дважды щелкните модуль и введите текст, чтобы добавить комментарий. Это поможет вам увидеть описание модуля и его действие в рамках эксперимента. В этом случае дважды щелкните модуль [Редактор метаданных][metadata-editor] и введите комментарий "Добавить заголовки столбцов". Щелкните в любом месте холста, чтобы закрыть текстовое поле. Щелкните стрелку вниз в модуле, чтобы отобразить комментарий.

4.	Выберите [Редактор метаданных][metadata-editor], затем на панели **Свойства** справа от холста щелкните **Запустить средство выбора столбцов**.
...
1.	Найдите модуль [Разделение данных][split], перетащите его на холст и подключите к последнему модулю [Редактор метаданных][metadata-editor].
