...
3.	Подключите набор данных к модулю [Изменение метаданных][edit-metadata]\: щелкните порт вывода набора данных (маленький кружок в нижней части набора данных), перетащите в порт ввода модуля [Изменение метаданных][edit-metadata] (маленький кружок в верхней части модуля) и отпустите кнопку мыши. Набор данных и модуль останутся подключенными даже при их перемещении по полотну.
...
6.	Строка под полем **Начинаются с** позволяет включить или исключить определенные столбцы из области изменения модуля [Изменение метаданных][edit-metadata]. Поскольку нам требуется изменить *все* столбцы, удалите эту строку, нажав кнопку "минус" ("–") в правой части строки. Диалоговое окно должно выглядеть следующим образом: ![Селектор столбцов, где выделены все столбцы][4]
...
> [AZURE.TIP] Если вы хотите проверить заголовки столбцов, запустите эксперимент (щелкните **ВЫПОЛНИТЬ** под холстом эксперимента). После его завершения (в модуле [Изменение метаданных][edit-metadata] появится зеленая галочка) щелкните порт вывода модуля [Изменение метаданных][edit-metadata], а затем выберите **Визуализировать**. Аналогичным образом можно просматривать выход любого модуля, чтобы следить за ходом использования данных в эксперименте.
...
Для этого используется модуль [Разделение данных][split].

1.	Найдите модуль [Разделение данных][split], перетащите его на холст и подключите к последнему модулю [Изменение метаданных][edit-metadata].
2.	По умолчанию коэффициент разделения равен 0,5 и задан параметр **Randomized split** (Случайное разделение). Это означает, что случайная половина данных будет выходить через один порт модуля [Разделение данных][split], а вторая половина — через другой. Это можно отрегулировать, а также использовать параметр **Псевдослучайные числа**, чтобы изменить соотношение между данными для обучения и тестирования. Для этого примера оставим их как есть.
	> [AZURE.TIP] Свойство **Доля строк в первом выходном наборе данных** определяет, какой объем данных будет выходить через левый порт вывода. Например, если коэффициент установлен на 0,7, то 70% данных выходит через левый порт, а 30% — через правый.  
3. Дважды щелкните модуль [Разделение данных][split] и введите комментарий "Соотношение данных обучения и тестирования составляет 50 %". 

Выходные данные модуля [Разделение данных][split] можно использовать по своему усмотрению. Однако предлагаем использовать левый выход для обучения данных, а правый — для их оценки.
...
1.	Найдите и перетащите модуль [Выполнение скрипта R][execute-r-script] на холст эксперимента и подключите левый порт вывода модуля [Разделение данных][split] к первому порту ввода (Dataset1) модуля [Выполнение скрипта R][execute-r-script].
2. Дважды щелкните модуль [Выполнение скрипта R][execute-r-script] и введите комментарий "Задать корректировку стоимости".
...
Необходимо проделать ту же операцию репликации для каждого выхода модуля [Разделение данных][split], чтобы данные для обучения и тестирования получили одинаковую корректировку стоимости.
...
3.	Подключите первый порт ввода данного модуля [Выполнение скрипта R][execute-r-script] к правому порту вывода модуля [Разделение данных][split].  
