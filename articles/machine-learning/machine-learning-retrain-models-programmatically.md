...
6. *Оценка результатов переобучения*: используя сочетание значений параметров BaseLocation, RelativeLocaiton и SasBlobToken из приведенных выше результатов вывода (Output2), мы увидим результаты работы переобученной модели, вставив полный URL-адрес в адресную строку браузера.

	Эти данные покажут, достаточно ли хорошо работает только что обученная модель, чтобы заменить существующую.

7. *Обновление обученной модели добавленной конечной точки*: чтобы завершить процесс, потребуется обновить обученную модель прогнозирующей конечной точки (оценки), которую мы создали на шаге 4 выше.

	(Во время добавления новой конечной точки с помощью портала Azure щелкните имя новой конечной точки, затем ссылку "Обновить ресурс", чтобы получить URL-адрес, необходимый для обновления модели конечной точки. При добавлении конечной точки с помощью кода выходные данные этого вызова будут содержать URL-адрес конечной точки.)
