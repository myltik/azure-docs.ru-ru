...
В рамках процесса операционализации моделей машинного обучения в машинном обучении Azure модель необходимо обучить и сохранить, после чего ее следует использовать при создании веб-службы прогнозирования. Созданная веб-служба может впоследствии использоваться на веб-сайтах, панелях мониторинга и в мобильных приложениях.
...
Чтобы начать, необходимы следующие компоненты: обучающий эксперимент и эксперимент прогнозирования, опубликованные в виде веб-службы. Чтобы включить переобучение обученной модели, обучающий эксперимент необходимо также опубликовать в виде веб-службы, выход которой будет представлять собой обученную модель. Это позволит использовать API-доступ для переобучения модели. Процесс настройки повторного обучения включает следующие шаги.
...
2. *Создание оценочного эксперимента и опубликование его в виде веб-службы*
	...
	После того как выполнение эксперимента будет завершено, мы используем функцию "Создать эксперимент прогнозирования". При этом создается эксперимент прогнозирования, модель сохраняется в виде обученной модели и добавляются модули ввода и вывода данных веб-службы, как показано ниже. Затем щелкнем "Выполнить".

	После завершения выполнения эксперимента и нажатия кнопки "Опубликовать веб-службу" эксперимент прогнозирования будет опубликован в виде веб-службы, при этом создастся конечная точка по умолчанию. Обученная модель в этой веб-службе может обновляться, как показано ниже. Данные для этой конечной точки затем будут отображаться на экране.
3. *Публикация обучающего эксперимента в виде веб-службы*: чтобы переобучить обученную модель, нам потребуется опубликовать обучающий эксперимент, созданный на шаге 1 выше, в качестве веб-службы. Этой веб-службе для создания новых обученных моделей потребуется выходной модуль, подключенный к модулю [Обучение модели][train-model]. Щелкните значок "Эксперименты" на левой панели, затем выберите эксперимент "Модель ценза", чтобы вернуться в обучающий эксперимент.
	...
	Далее нажмем кнопку "Развернуть веб-службу", а затем "Да". При этом обучающий эксперимент будет развернут в качестве веб-службы, которая создаст обученную модель и результаты их оценки. Панели мониторинга веб-службы будут отображаться с ключом API и со страницей справки API для пакетного выполнения. Обратите внимание, что метод пакетного выполнения может использоваться для создания обученных моделей.
4. *Добавление новой конечной точки*: веб-служба прогнозирования, которую мы опубликовали на шаге 2 выше, представляет собой конечную точку оценки по умолчанию. Конечные точки по умолчанию синхронизируются с исходными экспериментами по обучению и оценке, поэтому обученную модель конечной точки по умолчанию нельзя заменить. Чтобы создать конечную точку оценки с моделью обновления, откройте классический портал Azure и щелкните "Добавить конечную точку" (более подробно см. [здесь](machine-learning-create-endpoint.md)). Конечные точки оценки также можно добавить с помощью примера кода, приведенного [здесь](https://github.com/raymondlaghaeian/AML_EndpointMgmt/blob/master/Program.cs).

5. *Переобучение модели с помощью новых данных и BES.* Для переобучения модели необходимо вызвать функцию BES веб-службы, которая создана на шаге 3, описанном выше.

	Чтобы вызывать интерфейсы API переобучения, создадим новое приложение для консоли C# в Visual Studio (Создать->Проект->Рабочий стол Windows->Консольное приложение).
	...
	1. Предоставление сведений хранилища Azure Образец кода для BES выгрузит файл с локального диска (например, C:\\temp\\CensusIpnput.csv) в хранилище Azure, обработает его и снова запишет результаты в хранилище Azure.
		...
		Для этого потребуется на классическом портале Azure для вашей учетной записи хранения получить имя учетной записи хранения, ключ и сведения о контейнере, а затем обновить код здесь. Также нужно обеспечить доступность файла вводных данных в местоположении, указанном в коде.
...
6. *Оценка результатов переобучения.* Из выходных данных описанного выше вызова можно получить URL-адрес и маркер SAS для доступа к результатам оценки.

	Используя сочетание значений параметров BaseLocation, RelativeLocaiton и SasBlobToken из приведенных выше результатов вывода (Output2), мы увидим результаты работы переобученной модели, вставив полный URL-адрес в адресную строку браузера.

	Эти данные покажут, достаточно ли хорошо работает только что обученная модель, чтобы заменить существующую.

7. *Обновление обученной модели добавленной конечной точки.* Чтобы завершить процесс, потребуется обновить обученную модель конечной точки, которую мы добавили на шаге 4 выше.

	- Во время добавления новой конечной точки с помощью портала Azure щелкните имя новой конечной точки на портале Azure, а затем — ссылку "Обновить ресурс", чтобы получить URL-адрес, необходимый для обновления модели конечной точки.
	- При добавлении конечной точки с помощью образца кода выходные данные этого вызова будут содержать URL-адрес HelpLocationURL. Скопируйте и вставьте этот URL-адрес в адресную строку браузера. Затем щелкните ссылку обновления ресурса. Скопируйте URL-адрес POST запроса PATCH (например, для конечной точки "newendpoint2 URL-адрес PATCH будет следующим: https://management.azureml.net/workspaces/00bf70534500b34rebfa1843d6/webservices/af3er32ad393852f9b30ac9a35b/endpoints/newendpoint2)

	В выходных данных функции BES (шаг 5.a выше) отображаются результаты переобучения в output1, в которых указано расположение переобученной модели. Теперь нужно взять эту обученную модель и обновить конечную точку оценки (созданную на шаге 4 выше). Пример кода выглядит следующим образом:
	...
	Параметры apiKey и endpointUrl для этого вызова отображаются на панели мониторинга конечной точки. Параметр "Имя" в ресурсах должен соответствовать имени сохраненной обученной модели в эксперименте прогнозирования.
	
	Обратите внимание, что маркер SAS истекает через 1 час (55 минут). Для получения нового маркера необходимо выполнить запрос GET с идентификатором задания.

	Если этот вызов выполнен успешно, новая конечная точка начнет использовать переобученную модель примерно через 15–30 секунд.
