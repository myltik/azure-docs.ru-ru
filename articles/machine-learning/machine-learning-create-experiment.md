...
1. Запустите новый эксперимент, щелкнув **+СОЗДАТЬ** в нижней части окна Студии машинного обучения, выберите **ЭКСПЕРИМЕНТ**, а затем — **Пустой эксперимент**. В верхней части холста выберите имя эксперимента по умолчанию и присвойте ему понятное имя, например **Прогнозирование цен на автомобили**.
...
1. Введите **столбцы проекта** в поле поиска в верхней части палитры модулей, чтобы найти модуль [Столбцы проекта][project-columns], а затем перетащите его на холст эксперимента и подключите к порту вывода набора данных **Данные о ценах на автомобили (необработанные)**. Этот модуль позволяет выбрать, какие столбцы данных нужно включить в модель или исключить из нее.

2. Выберите модуль [Столбцы проекта][project-columns] и щелкните **Запустить средство выбора столбцов** в области **Свойства**.
...
2. Запустите эксперимент. При этом модули [Столбцы проекта][project-columns] и [Разделение][split] передают определения столбцов в модули, которые будут добавлены на следующем этапе.
...
4. Найдите и переместите модуль [Обучение модели][train-model] на холст эксперимента. Подключите левый порт ввода к порту вывода модуля [Линейная регрессия][linear-regression]. Подключите правый порт ввода к порту вывода данных для обучения (левому порту) модуля [Разделение][split].

5. Выберите модуль [Обучение модели][train-model], щелкните **Запустить средство выбора столбцов** в области **Свойства** и выберите столбец **цена**. Это значение, которое наша модель собирается спрогнозировать.
...
1. Найдите и перетащите модуль [Оценка модели][score-model] на холст эксперимента и подключите левый порт ввода к порту вывода модуля [Обучение модели][train-model]. Подключите правый порт ввода к выводу тестовых данных (правому порту) модуля [Разделение][split].  
...
Чтобы проверить выходные данные модуля [Анализ модели][evaluate-model], щелкните порт вывода и выберите **Визуализировать**. Для нашей модели будет выведена следующая статистика.
...
Если вы удовлетворены результатами своей модели, вы можете опубликовать ее в качестве веб-службы, чтобы ее можно было использовать для прогнозирования цен на автомобили, используя новые данные. Дополнительную информацию см. в статье [Публикация веб-службы машинного обучения Azure][publish].

[publish]: machine-learning-publish-a-machine-learning-web-service.md

Более обширное и подробное пошаговое руководство по приемам прогнозного моделирования для создания, обучения, оценки и публикации модели см. в статье [Разработка прогнозирующего решения с помощью машинного обучения Azure][walkthrough].
