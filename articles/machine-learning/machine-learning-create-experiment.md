...
2. Эксперименту будет присвоено имя по умолчанию, которое отображается в верхней части холста. Щелкните этот текст и измените его на любое удобное для вас имя, например **Прогнозирование цен на автомобили**.
...
Соединяя эти порты, вы создаете в эксперименте поток данных.
...
3. Выберите модуль [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных), затем в области **Свойства** щелкните **Launch column selector** (Запустить средство выбора столбцов).

    - Слева щелкните **With rules**
    - В разделе **Begin With** (Начинаются с) нажмите кнопку **All columns** (Все столбцы). Теперь модуль [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных) будет пропускать все столбцы (кроме тех, которые мы собираемся исключить).
...
1. Выберите модуль [Split Data][split] (Разбиение данных) и перетащите его на холст эксперимента, затем соедините с последним из модулей [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных).

2. Щелчком выберите модуль [Split Data][split] (Разбиение данных). В панели **Properties** (Свойства) справа от холста найдите параметр **Fraction of rows in the first output dataset** (Доля строк в первом выходном наборе данных) и установите для него значение 0,75. Это означает, что 75 % данных мы будем использовать для обучения модели, а остальные 25 % сохраним для тестирования (в следующий раз вы можете использовать и другие значения).
...
2. Запустите эксперимент. После выполнения эксперимента модули [Select Columns in Dataset][select-columns] (Выбор столбцов в наборе данных) и [Split Data][split] (Разбиение данных) смогут передать определения столбцов следующим модулям, которые мы добавим позже.  
...
4. Найдите и переместите модуль [обучения модели][train-model] на холст эксперимента. Соедините выход модуля [Linear Regression][linear-regression] (Линейная регрессия) с левым входом модуля [Train Model][train-model] (Обучение модели), затем соедините выход (левый порт) модуля [Split Data][split] (Разбиение данных) с правым входом модуля [Train Model][train-model] (Обучение модели).
...
5. Выберите модуль [Train Model][train-model] (Обучение модели), щелкните **Launch column selector** (Запустить средство выбора столбцов) в панели **Properties** (Свойства), затем выберите столбец **price** (Цена). Это значение, которое наша модель собирается спрогнозировать.
...
1. Найдите модуль [Score Model][score-model] (Оценка модели) и перетащите его на холст эксперимента. Соедините выход модуля [Train Model][train-model] (Обучение модели) с левым входным портом модуля [Score Model][score-model] (Оценка модели). Подключите тестовый выход данных (правый порт) модуля [Split Data][split] (Разбиение данных) к правому входному порту модуля [Score Model][score-model] (Оценка модели).
...
2. Запустите эксперимент и проверьте выходные данные модуля [Score Model][score-model] (Оценка модели). Для проверки данных щелкните выходной порт модуля [Score Model][score-model] (Оценка модели) и выберите **Visualize** (Визуализировать). На порту вывода будут показаны прогнозируемые значения цены вместе с известными значениями проверочных данных.  
...
3. Теперь мы готовы проверить качество результатов. Выберите модуль [Evaluate Model][evaluate-model] (Анализ модели) и перетащите его на холст эксперимента, а затем соедините выход модуля [Score Model][score-model] (Оценка модели) с левым входом модуля [Evaluate Model][evaluate-model] (Анализ модели).
    ...
    > [!TIP]
    > Так как модуль [анализа модели][evaluate-model] можно использовать для сравнения двух моделей, у него есть два входных порта. Вы можете добавить к эксперименту другой алгоритм и использовать модуль [Evaluate Model][evaluate-model] (Анализ модели) для оценки, какой из них дает лучшие результаты.

4. Запустите эксперимент.

Чтобы проверить выходные данные модуля [анализа модели][evaluate-model], щелкните порт вывода и выберите элемент **Визуализировать**.
