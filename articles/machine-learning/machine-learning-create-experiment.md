---
title: Простой эксперимент в Студии машинного обучения | Microsoft Docs
...
4. В разделе **Начинаются с** выберите **Нет столбцов**, а затем выберите **Включить** и **Имена столбцов** в строке фильтра. Введите наш список столбцов. В этом случае модуль будет передавать на вывод только указанные нами столбцы.
   ...
   > [!TIP]
   > Запустив этот эксперимент, мы убедились, что определения столбцов для наших данных передаются из исходного набора данных через модуль [очистки отсутствующих данных][clean-missing-data]. Это означает, что другие модули, которые вы подключите, также будут содержать информацию из набора данных.
   > 
   > 
