...
* загрузили данные в таблицы Hive в кластере Hadoop под управлением службы Azure HDInsight Если данные не загружены, необходимо предварительно загрузить их в таблицы Hive, воспользовавшись инструкциями из раздела [Создание данных и их загрузка в таблицы Hive](machine-learning-data-science-hive-tables.md).
...
- [Отправка запросов Hive](#submit)
- [Исследование данных и проектирование функций](#explore)
- [Дополнительные разделы: настройка параметров Hive для ускорения обработки запросов](#tuning)
...
Также можно воспользоваться консолью запросов (редактор Hive), введя в браузере указанный URL-адрес `https://<Hadoop cluster name>.azurehdinsight.net/Home/HiveEditor` (при этом вам будет предложено ввести учетные данные для входа в кластер Hadoop), или воспользоваться инструкциями из раздела [Отправка заданий Hive с помощью PowerShell](../hdinsight/hdinsight-submit-hadoop-jobs-programmatically.md#hive-powershell).
...
3. [Дополнительный раздел: настройка параметров Hive для ускорения обработки запросов](#tune-parameters)
...
В этом примере для сглаживания значений рисков, вычисленных на основе данных, задаются переменные `smooth_param1` and `smooth_param2`. Значения рисков находятся в диапазоне от минус бесконечности до плюс бесконечности. Если риск больше 0, вероятность того, что целевое значение равно 1, больше 0,5.
...
Поля, используемые в этом запросе - GPS-координаты мест высадки и посадки: pickup_longitude, pickup_latitude, dropoff_longitude, и dropoff_latitude. Вот как выглядят запросы для расчета прямого расстояния между координатами посадки и высадки:
