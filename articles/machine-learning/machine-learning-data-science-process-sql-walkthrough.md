...
2. Сохранить выбранные и спроектированные данные, которые планируется использовать для построения модели в новой таблице базы данных, и использовать новую таблицу в модуле [Импорт данных][import-data] в Машинном обучении Azure.
...
1. Сохранить окончательный запрос SQL для извлечения и выборки данных, скопировать и вставить этот запрос непосредственно в модуль [Импорт данных][import-data] в Машинном обучении Azure. Этот метод демонстрируется в разделе [Построение моделей в машинном обучении Azure](#mlmodel).    
...
При подготовке данных к построению модели в [Студии машинного обучения Azure](https://studio.azureml.net) вы можете определить **SQL-запрос для использования непосредственно в модуле "Импорт данных"** или сохранить спроектированные и выбранные данные в новой таблице, которую можно использовать в модуле [Импорт данных][import-data], используя простой запрос **SELECT * FROM <имя\_новой\_таблицы>**.
...
>
> Чтобы исключить ненужные столбцы и (или) целевые утечки, можно воспользоваться модулем [Выбор столбцов в наборе данных][select-columns] или [Изменение метаданных][edit-metadata]. Дополнительные сведения см. на страницах справки [Выбор столбцов в наборе данных][select-columns] и [Изменение метаданных][edit-metadata].
