...
## <a name="a-namedatasetanyc-taxi-trips-dataset-description"></a><a name="dataset"></a>Описание набора данных "Поездки такси Нью-Йорка"
...
## <a name="a-namesetupasetting-up-the-azure-data-science-environment-for-advanced-analytics"></a><a name="setup"></a>Настройка среды обработки данных Azure для расширенной аналитики
...
## <a name="a-namegetdataaget-the-data-from-public-source"></a><a name="getdata"></a>Получение данных из общедоступного источника
...
## <a name="a-namedbloadabulk-import-data-into-sql-server-database"></a><a name="dbload"></a>Выполните массовый импорт данных в базу данных SQL Server
...
## <a name="a-namedbexploreadata-exploration-and-feature-engineering-in-sql-server"></a><a name="dbexplore"></a>Просмотр данных и проектирование характеристик в SQL Server
...
1. Сохранить окончательный запрос SQL для извлечения и выборки данных, скопировать и вставить этот запрос непосредственно в модуль [импорта данных][import-data] в машинном обучении Azure.
2. Сохранить выбранные и спроектированные данные, которые вы планируете использовать для создания модели в новой таблице базы данных, и использовать новую таблицу в модуле [импорта данных][import-data] в машинном обучении Azure.
...
1. Сохранить окончательный запрос SQL для извлечения и выборки данных, скопировать и вставить этот запрос непосредственно в модуль [импорта данных][import-data] в машинном обучении Azure. Этот метод демонстрируется в разделе [Построение моделей в машинном обучении Azure](#mlmodel) .    
2. Сохранить выбранные и спроектированные данные, которые вы планируете использовать для создания модели, в новой таблице базы данных, затем использовать эту новую таблицу в модуле [импорта данных][import-data].
...
Подготавливая данные к созданию модели в [студии машинного обучения Azure](https://studio.azureml.net), вы можете определить **SQL-запрос для использования непосредственно в модуле импорта данных**. Или вы можете или сохранить спроектированные и выбранные данные в новой таблице, которую можно использовать в модуле [импорта данных][import-data] с помощью простого запроса **SELECT * FROM <имя\_вашей\_новой\_таблицы>**.
...
## <a name="a-namemlmodelabuilding-models-in-azure-machine-learning"></a><a name="mlmodel"></a>Построение моделей в машинном обучении Azure
...
1. Скачайте данные в машинное обучение Azure, используя модуль [импорта данных][import-data], доступный в разделе **Data Input and Output** (Ввод и вывод данных). Дополнительные сведения см. на странице со справочной информацией о модуле [импорта данных][import-data].
...
> 
> Чтобы исключить все ненужные столбцы или целевые утечки, можно воспользоваться модулем [выбора столбцов в наборе данных][select-columns] или модулем [изменения метаданных][edit-metadata]. Дополнительные сведения см. на страницах со справочной информацией о модулях [выбора столбцов в наборе данных][select-columns] и [изменения метаданных][edit-metadata].
> 
> 

## <a name="a-namemldeployadeploying-models-in-azure-machine-learning"></a><a name="mldeploy"></a>Развертывание моделей в машинном обучении Azure
