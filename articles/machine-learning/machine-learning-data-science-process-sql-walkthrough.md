...
В этом учебнике вы проследите использование расширенного процесса аналитики и технологий (ADAPT) Azure от начала до конца, чтобы построить и развернуть модель с использованием общедоступного набора данных — [Поездок такси Нью-Йорка](http://www.andresmh.com/nyctaxitrips/).
...
Мы сформулируем три перечисленные ниже задачи на прогнозирование на основе *tip_amount*.

1. Двоичная классификация: Спрогнозировать, были ли выплачены чаевые за поездку, т. е. значение *tip_amount* больше 0 $ является позитивным примером, а значение *tip_amount* 0 $ является негативным примером.

2. Мультиклассовая классификация: Спрогнозировать диапазон суммы чаевых за поездку. Мы разделяем *tip_amount* на пять ячеек или классов:
...
6. Затем создайте две таблицы секционирования: одну для trip_data, а другую для trip_fare. Откройте образец сценария **create_partitioned_table.sql**, который выполнит следующие действия.
	...
	- Создайте две таблицы секционирования для сопоставления со схемой секционирования: **nyctaxi_trip** будет содержать trip_data, а **nyctaxi_fare** будет содержать trip_fare.
...
#### Просмотр: распределение поездок по параметрам medallion и hack_license
...
#### Просмотр: Выплаченные чаевые против Распределение поездок с чаевыми и без чаевых
...
#### Просмотр: распределение классов/диапазонов чаевых
...
#### Просмотр: Вычисление и сравнение расстояния поездок
...
1. Сохранить окончательный запрос SQL для извлечения и выборки данных, скопировать и вставить этот запрос непосредственно в модуль [Reader][reader] в системе машинного обучения Azure. Этот метод демонстрируется в разделе [Построение моделей в машинном обучении Azure](#mlmodel).    
2. Сохранить выбранные и спроектированные данные, которые планируется использовать для построения модели, в новой таблице базы данных, затем использовать эту новую таблицу в модуле [Reader][reader].
...
Теперь все готово для просмотра выборки данных. Начнем с рассмотрения описательной статистики для** trip_distance** (или любых других полей):
...
Мы покажем точечную диаграмму между параметрами **trip_time_in_secs** и **trip_distance**, чтобы просмотреть, есть ли в этом случае корреляция
...
#### Создание метки: Создать метки классов
...
#### Проектирование характеристик: Ячейка характеристик для числовых столбцов
...
3. На главной странице Студии содержится множество информации, видеороликов, учебников, ссылок на справочник модулей и другие ресурсы. Дополнительную информацию о машинном обучении Azure см. на сайте [Центр машинного обучения Azure](http://azure.microsoft.com/documentation/services/machine-learning/).
...
1. Получите данные в Azure ML с помощью модуля [Читатель][reader], доступного в разделе **Ввод и вывод данных**. Для получения дополнительной информации см. справочную страницу модуля [Reader][reader].
...
>
> Чтобы исключить ненужные столбцы и/или целевые утечки, можно воспользоваться модулем [Столбцы проекта][project-columns] или [Редактором метаданных][metadata-editor]. Для получения дополнительной информации см. справочные страницы [Столбцы проекта][project-columns] и [Редактор метаданных][metadata-editor].
