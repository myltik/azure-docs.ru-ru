---
title: "Жизненный цикл процесса обработки и анализа данных группы Azure | Документация Майкрософт"
...
* Определяются релевантные **источники данных**— те, к которым уже есть или требуется доступ.
...
1. Основная задача этого этапа — определить ключевые **бизнес-переменные**, которые должен прогнозировать ваш анализ. Эти переменные называются **целевыми показателями модели**, а связанные с ними метрики позволяют определить успешность проекта. Два примера таких целевых показателей представлены прогнозом объемов продаж или вероятностью того, что заказ окажется мошенническим.

2. Определите **целевые показатели модели**, создавая и уточняя острые вопросы: релевантные, конкретные и однозначные. Обработка и анализ данных — это работа с именами и числами для получения ответов на такие вопросы. Дополнительные сведения о постановке точных вопросов см. в блоге [How to do Data Science](https://blogs.technet.microsoft.com/machinelearning/2016/03/28/how-to-do-data-science/) (Обработка и анализ данных). Обработка и анализ данных наряду с машинным обучением обычно применяются, чтобы отвечать на вопросы следующих пяти типов.
...
* [**Источники данных**](https://github.com/Azure/Azure-TDSP-ProjectTemplate/blob/master/Docs/DataReport/Data%20Defintion.md#raw-data-sources). Это раздел **Источники необработанных данных** в отчете **Определения данных**, который находится в папке **Data Report** в проекте TDSP. В нем указаны исходное и целевое расположения для необработанных данных. На более поздних этапах вы добавите новые сведения, например скрипты для переноса данных в аналитическую среду.  
* [**Словари данных**](https://github.com/Azure/Azure-TDSP-ProjectTemplate/tree/master/Docs/DataDictionaries). В этом документе содержатся описания данных, предоставленных клиентом. Эти описания содержат сведения о схеме (типы данных, сведения о правилах проверки, если таковые имеются) и диаграммы отношений сущностей, если таковые имеются.


## <a name="2-data-acquisition-and-understanding"></a>2) Получение и изучение данных

### <a name="goals"></a>Цели
* Чистый набор данных высокого качества с понятными связями с целевыми переменными, размещенный в соответствующей среде аналитики и готовый к моделированию.
...
Процесс TDSP включает автоматизированное средство [IDEAR](https://github.com/Azure/Azure-TDSP-Utilities/blob/master/DataScienceUtilities/DataReport-Utils), которое позволяет визуализировать данные и подготавливать сводные отчеты. Мы рекомендуем приступить к интерактивному изучению исходных данных с помощью IDEAR до написания кода. И только потом создавать пользовательский код для просмотра и визуализации данных. Рекомендации по очистке данных см. в статье [Задачи по подготовке данных для расширенного машинного обучения](machine-learning-data-science-prepare-data.md).  
...
В зависимости от типа вопроса, на который вы ищете ответ, можно использовать разные алгоритмы моделирования. Инструкции по выбору алгоритма см. в статье [Выбор алгоритмов машинного обучения Microsoft Azure](machine-learning-algorithm-choice.md). Хотя эта статья посвящена службе машинного обучения Azure, содержащиеся рекомендации полезны и для любых других проектов машинного обучения. 
...
Создав набор эффективно работающих моделей, можно ввести их в эксплуатацию для взаимодействия с другими приложениями. В зависимости от бизнес-требований прогнозы выполняются в режиме реального времени или в пакетном режиме. Модель готова к вводу в эксплуатацию, если она оснащена открытым API-интерфейсом, с которым без проблем могут взаимодействовать самые разные приложения, включая веб-сайты, электронные таблицы, панели мониторинга, бизнес-приложения и серверные приложения. Примеры ввода в эксплуатацию веб-службы машинного обучения Azure см. в статье [Развертывание веб-службы машинного обучения Azure](machine-learning-publish-a-machine-learning-web-service.md). Также рекомендуется встроить в производственную модель и развернутый конвейер средства телеметрии и мониторинга. Это позволит вам получать отчеты о состоянии последующей системы и устранять неполадки.  
