...
1. В Студии машинного обучения откройте вкладку **Веб-службы**, а затем откройте веб-службу, к которой необходим доступ. Скопируйте ключ, указанный в разделе **Ключ API**, и сохраните его.
...
3. Перейдите по ссылке [Веб-приложение службы запрос-ответ Azure ML](https://azure.microsoft.com/marketplace/partners/microsoft/azuremlaspnettemplateforrrs/) и нажмите кнопку **Создать веб-приложение**. Шаблон перенаправит вас на портал Microsoft Azure, где создается новое веб-приложение.
...
4. После завершения развертывания веб-приложения службой Azure щелкните **URL-адрес** на странице параметров веб-приложения в Azure или введите URL-адрес в веб-браузере. Например, `http://carprediction.azurewebsites.net.`

5. При первом запуске веб-приложение запросит **URL-адрес API Post** и **Ключ API**. Введите значения, которые были сохранены ранее:
	- **URI запроса** со страницы справки API для **URL-адреса API Post**;
	- **ключ API** с панели мониторинга веб-службы для **Ключа API**.

	Нажмите кнопку **Submit** (Отправить).

	![Введите Post URI и ключ API][image6]

6. Веб-приложение отображает страницу **Конфигурация веб-приложения** с текущими параметрами веб-службы. Здесь можно вносить изменения в параметры, используемые веб-приложением.

	> [AZURE.NOTE]Изменение этих параметров влияет только на параметры этого веб-приложения. Это не изменяет параметры веб-службы по умолчанию. Например, если здесь изменить параметр **Описание**, то описание, отображаемое на панели мониторинга веб-службы в Студии машинного обучения, не изменится.

	Когда все будет готово, нажмите кнопку **Сохранить изменения**, а затем выберите команду **Перейти на домашнюю страницу**.

7. На домашней странице можно ввести значения для отправки в веб-службу, нажать кнопку **Отправить**, в результате чего вернется результат.

Если необходимо вернуться на страницу **Конфигурация**, перейдите на страницу веб-приложения `setting.aspx` . Например, на `http://carprediction.azurewebsites.net/setting.aspx.` Вам будет предложено ввести ключ API снова. Это нужно, чтобы получить доступ к странице и обновить параметры.
...
1. Чтобы создать веб-приложение BES, выполните ту же процедуру, что и для шаблона RRS, кроме таких действий:
	...
	- перейти по ссылке [Шаблон веб-приложения службы пакетного выполнения Azure ML](https://azure.microsoft.com/marketplace/partners/microsoft/azuremlbeswebapptemplate/), чтобы открыть шаблон BES в Azure Marketplace, и нажать кнопку **Создать веб-приложение**.
...
- Другие способы доступа к веб-службе — см. раздел [Как использовать веб-службу машинного обучения Azure](machine-learning-consume-web-services.md).
