...
> [AZURE.NOTE]В этой статье используется [портал предварительной версии Azure](https://ms.portal.azure.com/).
...
Инструкции по созданию новой фабрики данных Azure и группы ресурсов в [портале предварительной версии Azure](https://ms.portal.azure.com/) представлены [здесь](data-factory-build-your-first-pipeline-using-editor.md#step-1-creating-the-data-factory). Задайте имя *adfdsp* для нового экземпляра ADF и имя *adfdsprg* для созданной группы ресурсов.
...
Для создания связанной службы для локального SQL Server нажмите **Хранилище данных** на стартовой странице ADF портала Azure, выберите *SQL* и введите *имя пользователя* и *пароль* для локального SQL Server. Имя сервера необходимо указать в следующем формате: **полное имя сервера, обратная косая черта, имя экземпляра (имя\_сервера\\имя\_экземпляра)**. Укажите имя *adfonpremsql* для связанной службы.

###<a name="adf-linked-service-blob-store"></a>Связанная служба для больших двоичных объектов
Для создания связанной службы для хранилища больших двоичных объектов Azure нажмите **Хранилище данных** на стартовой странице ADF портала Azure, выберите *учетную запись хранилища Azure* и введите ключ учетной записи хранилища больших двоичных объектов Azure и имя контейнера. Укажите имя *adfds* для связанной службы.

###<a name="adf-linked-service-azure-sql"></a>Связанная служба для базы данных SQL Azure
Для создания связанной службы для базы данных SQL Azure нажмите **Хранилище данных** на стартовой странице ADF портала Azure, выберите *Azure SQL* и введите *имя пользователя* и *пароль* для локального SQL Server. *Имя пользователя* необходимо указать как **user@servername*.
...
> [AZURE.NOTE]Перед выполнением командлета [New-AzureDataFactoryTable](https://msdn.microsoft.com/library/azure/dn835096.aspx) следует выполнить командлеты `Switch-AzureMode -Name AzureResourceManager` и `Add-AzureAccount`, чтобы гарантировать доступность командлетов Azure PowerShell и то, что для выполнения команды выбрана правильная подписка Azure. Документацию по этим командлетам см. в [Switch-AzureMode](https://msdn.microsoft.com/library/dn722470.aspx) и [Add-AzureAccoun](https://msdn.microsoft.com/library/azure/dn790372.aspx).
...
> [AZURE.NOTE]В следующих процедурах для определения и создания действий ADF используется Azure PowerShell. Однако эти задачи также можно выполнить на портале предварительной версии Azure. Подробности см. в статье [Создание входных и выходных наборов данных](data-factory-use-onpremises-datasources.md#step-3-create-input-and-output-datasets).

###<a name="adf-table-onprem-sql"></a>Таблица для локального SQL Server
...
> [AZURE.NOTE]В следующих процедурах для определения и создания конвейера ADF используется Azure PowerShell. Однако эту задачу также можно выполнить на портале предварительной версии Azure. Дополнительные сведения см. в разделе [Создание и запуск конвейера](data-factory-use-onpremises-datasources.md#step-4-create-and-run-a-pipeline).
...
Убедитесь, что конвейер в ADF отображается в портале Azure следующим образом (при щелчке на схеме).
