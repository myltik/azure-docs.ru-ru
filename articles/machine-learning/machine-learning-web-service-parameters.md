...
В качестве примера предположим, что у нас имеется эксперимент с модулем [Экспорт данных][writer], который отправляет сведения в хранилище BLOB-объектов Azure. Мы определим параметр веб-службы с именем "Путь к BLOB-объекту", который позволяет пользователю веб-службы изменять путь к хранилищу BLOB-объектов при доступе к этой службе.

1. В Студии машинного обучения щелкните модуль [Экспорт данных][writer], чтобы выбрать его. Его свойства отображаются на панели "Свойства" справа от холста эксперимента.
...
3. Щелкните значок справа от элемента **Путь к BLOB-объекту, начиная с параметра контейнера**. Это выглядит следующим образом.
   ...
   В раздел **Параметры веб-службы** внизу на странице "Свойства" добавляется запись с именем "Путь к BLOB-объекту, начиная с контейнера". Этот параметр веб-службы теперь связан с параметром модуля [Экспорт данных][writer].
...
Теперь при доступе к веб-службе пользователь может указать новое расположение для модуля [Экспорт данных][writer].
