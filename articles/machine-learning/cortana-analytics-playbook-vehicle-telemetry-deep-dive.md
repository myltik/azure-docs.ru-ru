...
В это решение входит симулятор телематики автомобиля. Он выдает диагностическую информацию и сигналы, соответствующие состоянию автомобиля и шаблону вождения в определенный момент времени. Щелкните [здесь](http://go.microsoft.com/fwlink/?LinkId=717075), чтобы скачать **решение Visual Studio симулятора телематики автомобиля** для настройки в соответствии с требованиями. Каталог автомобилей содержит справочный набор данных для сопоставления идентификационного номера (VIN) с моделью автомобиля.
...
После того, как конвейер успешно выполнит это действие и набор данных RawCarEventsTable будет помечен как Ready, создаются имитированные сигналы автомобиля и диагностические данные за год. Затем отобразятся папка и файл, как показано ниже, созданные в учетной записи хранения в контейнере connectedcar.
...
На этапе подготовки данных необработанные полуструктурированные сигналы автомобиля и набор диагностических данных секционируются по годам и месяцам для эффективного выполнения запросов и долговременного масштабируемого хранения (другими словами это позволяет отрабатывать сбои из учетной записи одного большого двоичного объекта в следующий после заполнения первого). Выходные данные (с меткой *PartitionedCarEventsTable*) в течение длительного периода времени хранятся в виде фундаментальных данных или данных rawest в озере данных заказчика. Входные данные для этого конвейера обычно отклоняются, так как выходные данные полностью с ними совпадают — они лучше хранятся (секционируются) для последующего использования.

![](./media/cortana-analytics-playbook-vehicle-telemetry-deep-dive/fig11-vehicle-telematics-partition-car-events-workflow.png)

*Рис. 11. Рабочий процесс секционирования событий автомобиля*

Необработанные данные секционируются с помощью действия Hive HDInsight в PartitionCarEventsPipeline. Годовые образцы данных, созданные на первом этапе, секционируются по годам и месяцам для создания секций сигналов автомобиля и диагностических данных, соответствующих каждому месяцу года (всего 12 секций).
...
Дополнительные сведения о всех возможностях окон в Azure Stream Analytics см. [здесь](https://msdn.microsoft.com/library/azure/dn835019.aspx).
...
Сценарий Hive aggresivedriving.hql, который используется для анализа шаблона агрессивного вождения, находится в скачанном файле ZIP в папке \\demo\\src\\connectedcar\\scripts.
...
Здесь используются включенная передача коробки передач, положение педали тормоза и скорость автомобиля, чтобы определить неосторожное или агрессивное поведение во время вождения на основе шаблона торможения при высокой скорости.
...
Сценарий Hive fuelefficientdriving.hql, который используется для анализа шаблона агрессивного вождения, находится в скачанном файле ZIP в папке \\demo\\src\\connectedcar\\scripts.
...
Здесь используются включенная передача коробки передач, положение педали тормоза, педали газа и скорость автомобиля, чтобы определить манеру вождения, позволяющую экономить топливо, на основе шаблонов ускорения, торможения и скорости.
