---
title: Настройка виртуальной машины как сервера IPython Notebook | Microsoft Docs
...
Появится предупреждение с текстом *Возникла проблема с сертификатом безопасности этого веб-сайта* (Internet Explorer) или *Ваше подключение не защищено* (Chrome), как показано на рисунках ниже. Чтобы продолжить, щелкните **Продолжить открытие этого веб-сайта (не рекомендуется)** (Internet Explorer) или **Дополнительно**, а затем — **Перейти к &#60;**DNS-имя**> (небезопасно)** (Chrome). После этого введите пароль, указанный ранее для доступа к IPython Notebook.
