...
6.	Подключите левый порт вывода ("Преобразованный набор данных") модуля [Нормализация данных][normalize-data] к правому порту ввода ("Набор данных") модуля [Обучение модели][train-model].
...
9.	Щелкните знак "плюс" (+) в правой части этой строки — будет создана строка раскрывающегося списка. Выберите **Исключить** в первом раскрывающемся списке, затем **имена столбцов** во втором раскрывающемся списке и введите "Кредитный риск" в текстовом поле (или выберите **индексы столбцов** и введите "21"). Это указывает, что столбец "Кредитный риск" должен игнорироваться (это необходимо сделать, так как он является числовым и в противном случае будет преобразован).
...
Мы будем использовать данные тестирования, которые были отделены модулем [Разделение данных][split] для оценки обученных моделей. Затем можно сравнить результаты двух моделей, чтобы увидеть, которая модель создает лучший результат.
...
3.	Соедините правый порт ввода порт модуля [Оценка модели][score-model] с левым портом вывода правого модуля [Выполнение скрипта R][execute-r-script].

    Теперь модуль [Оценка модели][score-model] может получить сведения о кредите из тестовых данных, запустить их с помощью модели и сравнить созданные моделью прогнозы с фактическими данными столбца кредитного риска в тестовых данных.
...
6.	Для модели SVM необходимо выполнить то же самое преобразование тестовых данных, которое делалось для обучающих данных. Поэтому скопируйте и вставьте модуль [Нормализация данных][normalize-data], чтобы создать вторую копию, и соедините ее с левым выходным портом правого модуля [Выполнение скрипта R][execute-r-script].
7.	Соедините правый входной порт модуля [Оценка модели][score-model] с левым выходным портом модуля [Нормализация данных][normalize-data].  
...
Чтобы проверить результаты, щелкните правой кнопкой мыши порт вывода модуля [Анализ модели][evaluate-model] и выберите **Визуализировать**.
