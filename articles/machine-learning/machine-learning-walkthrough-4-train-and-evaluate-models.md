<properties 
	pageTitle="Шаг 4. Обучение и анализ моделей прогнозирующей аналитики" 
...
3.	Соедините левую точку вывода модуля [Выполнение скрипта R][execute-r-script] с правым портом ввода ("Набор данных") модуля [Обучение модели][train-model].
...
8.	Щелкните **Запустить средство выбора столбцов**, в первом раскрывающемся списке выберите **Включить**, во втором раскрывающемся списке выберите **Тип столбца** и в третьем раскрывающемся списке выберите **Числовой**. Это указывает, что будут преобразованы все числовые столбцы (и только числовые).
...
3.	Соедините правую точку ввода модуля [Оценка модели][score-model] с выводом правого модуля [Выполнение скрипта R][execute-r-script]. Обратите внимание, что вывод модуля может передаваться в несколько мест.
...
Если эта галочка появилась во всех модулях, значит эксперимент завершил выполнение. Чтобы проверить результаты, щелкните правой кнопкой мыши порт вывода модуля [Анализ модели][evaluate-model] и выберите **Визуализировать**.
...
**Далее: [публикация веб-службы](machine-learning-walkthrough-5-publish-web-service.md)**
