...
* Рабочая область, служба и ключ API для эксперимента AzureML, опубликованного в качестве веб-службы. Сведения о том, как создать эксперимент AzureML, см. [здесь](machine-learning/machine-learning-create-experiment.md). Сведения о том, как эксперимент AzureML опубликовать в качестве веб-службы, см. [здесь](machine-learning/machine-learning-publish-a-machine-learning-web-service.md). Инструкции по созданию и тестированию простого эксперимента AzureML и публикации его в качестве веб-службы содержатся также в дополнении А.
...
Введите текст **AzureML Demo API** («Демонстрационный API AzureML») в поле **Имя веб-API**. Введите **https://ussouthcentral.services.azureml.net** в поле **URL-адрес веб-службы**. Введите **azureml-demo** в поле **Суффикс URL-адреса веб-API**. Установите флажок **HTTPS** для параметра **URL-адрес веб-API**. Выберите **Начальный комплект** в поле **Продукты**. Чтобы после этого создать интерфейс API, нажмите кнопку **Сохранить**.
...
Сначала создайте операцию для службы AzureML RRS. Выберите значение **POST** в поле **HTTP-команда**. В поле **Шаблон URL-адреса** введите текст **/workspaces/{workspace}/services/{service}/execute?api-version={apiversion}&details={details}**. Введите текст **RRS Execute** (Выполнение RRS) в поле **Отображаемое имя**.
...
В разделе «Заголовки запросов» щелкните **Добавить заголовок** и введите текст **Content-Type** и **application/json**. Затем щелкните **Добавить заголовок** и введите текст **Authorization** и **Bearer<YOUR AZUREML SERVICE API-KEY>**. Название вашего **ключа API** отображается на панели мониторинга веб-службы AzureML (см. раздел **Тестирование веб-службы** в дополнении A).
...
Ниже описаны действия, которые нужно выполнить, чтобы создать простой эксперимент AzureML и опубликовать его в качестве веб-службы. Веб-служба принимает столбец случайного текста и возвращает набор функций в виде целых чисел. Например:
