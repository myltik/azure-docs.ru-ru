---
title: Расширенное исследование и моделирование данных с помощью Spark | Microsoft Docs
...
* [Случайные леса](http://spark.apache.org/docs/latest/mllib-ensembles.html#Random-Forests) — это совокупности деревьев принятия решений.  Они объединяют множество деревьев принятия решений, чтобы уменьшить риск лжевзаимосвязи. Случайные леса используются для регрессии и классификации, могут обрабатывать категориальные функции и могут расширяться до многоклассовой настройки классификации. С их помощью можно определять нелинейные зависимости и взаимодействия признаков. Этот метод не требует масштабирования признаков. Случайные леса — одна из самых эффективных моделей машинного обучения для задач классификации и регрессии.
...
* **Волшебная команда SQL (`%%sql`)**. Ядро HDInsight PySpark позволяет легко выполнять встроенные запросы HiveQL к sqlContext. Аргумент (-o VARIABLE_NAME) сохраняет выходные данные запроса SQL в формате Pandas DataFrame на сервере Jupyter. Это означает, что они доступны в локальном режиме.
...
2. С использованием **функции конвейера pySpark CrossValidator**. Обратите внимание, что, несмотря на подтвержденное нашим опытом удобство, функция CrossValidator имеет несколько ограничений для Spark 1.5.0: 
...
### <a name="use-mllib's-crossvalidator-pipeline-function-with-logistic-regression-(elastic-regression)-model"></a>Использование функции конвейера MLlib CrossValidator с моделью LogisticRegression (эластичная регрессия)
