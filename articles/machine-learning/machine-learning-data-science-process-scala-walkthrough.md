...
> [AZURE.NOTE] О том, как использовать Python вместо Scala для выполнения задач по полной обработке и анализу данных, см. в статье [Общие сведения об обработке и анализе данных с помощью платформы Spark в Azure HDInsight](machine-learning-data-science-spark-overview.md).
...
### Импорт данных, создание RDD и определение кадра данных согласно схеме
...
> [AZURE.TIP] В этой статье магическая команда local используется несколько раз. Если используется большой набор данных, сделайте выборку, чтобы создать кадр данных, который можно сохранить в локальной памяти.
...
## Модель двоичной классификации: прогнозирование вероятности выплаты чаевых за поездку
...
- **модель регрессии по методу деревьев с градиентным повышением**, использующую функцию машинного обучения Spark `GBTRegressor()`.
...
>[AZURE.NOTE] В этой статье данная магическая команда Spark используется несколько раз. Если объем данных большой, сделайте выборку, чтобы создать кадр данных, который можно разместить в локальной памяти.
