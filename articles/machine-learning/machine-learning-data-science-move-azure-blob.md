...
Обозреватель хранилищ Azure — это бесплатный инструмент для проверки и изменения данных в учетной записи хранения Azure на платформе Windows. Его можно скачать в [обозревателе хранилищ Azure](http://azurestorageexplorer.codeplex.com/). Ниже рассказывается о том, как отправить или скачать данные с помощью обозревателя хранилищ Azure.
...
**Предупреждение**. Если вы используете другой компьютер вместо виртуальной машины, установленной ранее в процессе расширенного анализа, установите AzCopy, следуя указаниям по установке из раздела [Скачивание и установка AzCopy](../storage-use-azcopy.md#install).
...
> [AZURE.NOTE]Если вы используете другой компьютер вместо виртуальной машины, установленной ранее в процессе расширенного анализа, то прежде чем использовать пример кода ниже, необходимо установить [пакет SDK для Azure на Python](../python-how-to-install.md).
...
Чтобы загрузить данные из большого двоичного объекта, используйте следующие методы: 1. get_blob_to_path; 2. get_blob_to_file; 3. get_blob_to_bytes; 4. get_blob_to_text.
