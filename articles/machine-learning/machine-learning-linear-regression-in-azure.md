...
3. Использование модуля [Split Data][split] (Разделение данных) (в режиме *Relative Expression* (Относительное выражение)) для разделения данных на точно такие же наборы данных для обучения, как в Excel.  
...
Мы также поняли, что в будущем для ускорения получения результатов по проектам прогнозирования целесообразно использовать модуль [Filter-Based Feature Selection][filter-based-feature-selection] (Выбор признаков на основе фильтра).  Применив выбор признаков к данным, можно создать улучшенную модель Машинного обучения Azure с более высокой общей производительностью. 
