...
Параметры:
- event (строковый , обязательный) — "щелчок";
- item (строковый, обязательный) — уникальный идентификатор элемента;
- itemName (строковый, необязательный) — имя элемента;
- itemDescription (строковый, необязательный) — описание элемента;
- itemCategory (строковый, необязательный) — категория элемента.
...
Параметры:
- event (строковый, обязательный) — recommendationclick;
- item (строковый, обязательный) — уникальный идентификатор элемента;
- itemName (строковый, обязательный) — имя элемента;
- itemDescription (строковый, обязательный) — описание элемента; - itemCategory (строковый, необязательный) — категория элемента;
- seeds (набор строк, необязательный) — начальные значения, которые создали запрос рекомендации;
- recoList (набор строк, необязательный) — результат запроса рекомендации, который создал элемент, который щелкнул пользователь.
...
* event (строковый, обязательный) — addshopcart;
* item (строковый, обязательный) — уникальный идентификатор элемента;
* itemName (строковый, необязательный) — имя элемента;
* itemDescription (строковый, необязательный) — описание элемента;
* itemCategory (строковый, необязательный) — категория элемента.
...
Параметры:
* event (строковый, обязательный) — removeshopcart;
* item (строковый, обязательный) — уникальный идентификатор элемента;
* itemName (строковый, необязательный) — имя элемента;
* itemDescription (строковый, необязательный) — описание элемента;
* itemCategory (строковый, необязательный) — категория элемента.
...
Параметры:
* event (строковый) — purchase;
* items (приобретено) — массив, содержащий запись по каждому приобретенному элементу.<br><br>
Формат приобретения:
	* item (строковый) — уникальный идентификатор элемента;
...
Параметры:
* event (строковый) — userlogin;
* user (строковый)— уникальный идентификатор пользователя.
		<script>
			if (typeof AzureMLRecommendationsEvent=="undefined") { AzureMLRecommendationsEvent = []; }
...
Параметры:
* items (массив строк) — один или несколько элементов, по которым будут получены рекомендации. При использовании сборки Fbt здесь можно указать только один элемент.
* numberOfResults (int) — количество требуемых результатов.
* includeMetadata (логический, необязательный) — если установлено значение true, то это значит, что поле метаданных результата должно быть заполнено.
* Функция обработки — функция, при помощи которой будут обработаны возвращенные рекомендации. Данные возвращаются в виде следующего массива:
	* item — уникальный идентификатор элемента; * name — имя элемента (если есть в каталоге);
	* rating — рейтинг рекомендации;
	* metadata — строка, представляющая собой метаданные элемента.
