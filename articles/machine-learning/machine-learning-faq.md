...
**Где находится список классических веб-служб и новых веб-служб на основе ARM?**

Список классических веб-служб можно найти в [Студии машинного обучения](http://studio.azureml.net) на вкладке веб-служб. Список новых веб-служб на основе ARM можно найти на портале [веб-служб машинного обучения Microsoft Azure](https://services.azureml.net/). Перекрестный список недоступен.

## Вопросы по веб-службам машинного обучения Microsoft Azure

**Что представляют собой веб-службы машинного обучения Azure?**

С помощью веб-службы машинного обучения Azure внешнее приложение взаимодействует с рабочим процессом машинного обучения, оценивая модель в режиме реального времени. Вызов веб-службы машинного обучения возвращает результаты прогноза внешнему приложению. Чтобы вызвать веб-службу машинного обучения, следует передать ключ API, созданный при развертывании этой службы. Веб-служба машинного обучения работает на основе архитектуры REST, используемой в основном в проектах с веб-программированием.
...
**Можно ли развернуть прогнозной эксперимент как новую веб-службу (на основе ARM), если он уже развернут в виде классической веб-службы?**

Нет. Эксперимент, который ранее был развернут как классическая веб-служба, развернуть нельзя. Необходимо создать другой прогнозной эксперимент и развернуть его.
...
В настоящее время Студия машинного обучения поддерживает более 400 пакетов CRAN R. Актуальный список всех включенных пакетов можно просмотреть [здесь](http://az754797.vo.msecnd.net/docs/RPackages.xlsx). Чтобы самостоятельно получить список, см. инструкции в статье [Расширение возможностей эксперимента с помощью R](machine-learning-extend-your-experiment-with-r.md). Если нужный вам пакет отсутствует в этом списке, укажите его название на [форуме обратной связи](http://go.microsoft.com/fwlink/?LinkId=404231).
...
В рамках уровня "Стандартный" счета за использование Студии машинного обучения выставляются помесячно за одно место. При запуске эксперимента в Студии счета выставляются за используемые вычислительные ресурсы. При развертывании классической веб-службы счета за транзакции и часы вычислений выставляются по принципу "оплата по мере использования".

В рамках новых веб-служб машинного обучения представлены планы выставления счетов, которые предусматривают более предсказуемые затраты. Структурированный тарифный план предназначен для клиентов, которым нужен большой объем ресурсов по ставке со скидкой.

При настройке плана вы принимаете фиксированную стоимость включенного количества часов вычислений и транзакций API. Если вам требуется большее количество, можно добавить дополнительные экземпляры в план. Если же вам необходимо гораздо большее количество ресурсов, можно выбрать план более высокого уровня, который предусматривает значительно больше ресурсов по лучшей цене с учетом скидки.

После израсходования включенного количества ресурсов в имеющихся экземплярах плата за последующее использование будет взиматься по ставке за превышение, предусмотренной для конкретного уровня плана выставления счетов.

Примечание. Включенный объем повторно выделяется каждые 30 дней, а неиспользованный объем не переносится на следующий период.
...
Новые веб-службы машинного обучения Azure предоставляют несколько уровней, которые можно использовать для подготовки плана выставления счетов. Уровень разработки и тестирования предоставляет ограниченный включенный объем ресурсов для тестирования эксперимента в качестве новой веб-службы без дополнительных затрат. Таким образом, у вас есть возможность испробовать веб-службы.
...
Служба машинного обучения является многопользовательской, и фактические вычислительные ресурсы, используемые на сервере, различаются. Кроме того, они оптимизируются для повышения производительности и качества прогнозирования.
...
Экземпляр план — это единица включенного объема, которую можно добавить в план выставления счетов. Изначально в уровень выставления счетов, который выбирается для плана, включен один экземпляр. Если вам требуется больший объем, можно добавить дополнительные экземпляры выбранного уровня выставления счетов в план.
...
Единицы объема добавляются на пропорциональной основе. Они становятся действительными примерно через 24 часа.
...
Вам может потребоваться определить нужный план выставления счетов перед развертыванием службы, например если необходимо развернуть службу не в регионе по умолчанию.

В этом случае нужно войти на портал веб-служб машинного обучения Azure и перейти на страницу планов. Там можно добавлять, удалять и изменять планы.
...
Да. Цены планов зависят от региона. При развертывании веб-службы в другом регионе необходимо назначить ей план, который поддерживается в этом регионе.

### Превышение включенного объема в рамках новых веб-служб

**Как проверить, превышен ли включенный объем в веб-службе?**

Показатель использования по всем планам можно просмотреть на странице планов на портале веб-служб машинного обучения Azure. Войдите на портал и выберите пункт меню "Планы".
...
Для рабочей нагрузки RRS с вас будет взиматься плата за каждый вызов API-транзакции, а также за вычислительное время, связанное с этими запросами. Стоимость вызовов производственного API с моделью RRS будет вычисляться как сумма всех вызовов API, умноженная на цену за 1000 транзакций (пропорционально распределенную по каждой транзакции). Стоимость часа вычислений производственного API в RRS будет вычисляться как время, необходимое для выполнения каждого вызова API, умноженное на полное число API-транзакций, умноженное на стоимость каждого часа вычислений производственного API. Например, при избыточном использовании на уровне "Стандартный" S1 1 000 000 API-транзакций, каждая из которых выполняется в течение 0,72 секунды, приведет к стоимости в 500 долл. США (1 000 000 * 0,50 долл. США/1000 API-транзакций) в терминах транзакций производственного API и 400 долл. США (1 000 000 * 0,72 с * 2 долл. США/час) в терминах часов вычислений производственного API. Таким образом, полная стоимость составит 900 долл. США.

Для рабочей нагрузки BES с вас будет взиматься плата точно таким же образом, однако стоимость API-транзакций вычисляется исходя из количества пакетных заданий, переданных в систему, а стоимость вычисления представляет собой вычислительное время, связанное с этими пакетными заданиями. Стоимость транзакций производственного API для BES будет вычисляться как общее число отправленных заданий, умноженное на цену 1000 транзакций (пропорционально распределенную по каждой транзакции). Стоимость часа вычислений производственного API для BES будет вычисляться исходя из времени, затрачиваемого на каждую строку задания, умноженного на полное количество заданий и на цену часа вычислений в производственного API. При использовании калькулятора машинного обучения счетчик транзакций показывает количество заданий, которые вы планируете отправить, при этом временные затраты на одно поле транзакции выражают совокупное время, необходимое для обработки строк в каждом задании. Например, если вы при избыточном использовании на уровне "Стандартный" S1 отправите 100 заданий каждый день, каждое из которых будет состоять из 500 строк, на разрешение каждой из которых требуется 0,72 секунды, то ваши ежемесячные расходы на избыточное использование составят 1,55 долл. США (100 заданий в день = 3100 заданий/месяц * 0,50 долл. США/1000 API-транзакций) в терминах расходов на транзакции производственного API и 620 долл. США (500 строк * 0,72 с * 3100 заданий * 2 долл. США/час) в терминах часов вычислений в производственном API, что вместе составит 621,55 долл. США.
...
Уровень "Бесплатный" Машинного обучения Azure позволит подробно ознакомиться со Студией машинного обучения Azure. Для регистрации требуется только учетная запись Майкрософт. Уровень "Бесплатный" предусматривает свободный доступ к одному рабочему пространству Студии машинного обучения Azure для одной [учетной записи Майкрософт](https://www.microsoft.com/account/default.aspx). Он обеспечивает возможность использовать до 10 ГБ хранилища и эксплуатировать модели как промежуточные API. На рабочие нагрузки, выполняемые на уровне "Бесплатный", не распространяется действие соглашения об уровне обслуживания. Они предназначены только для разработчиков и личного использования. Рабочие нагрузки уровня "Бесплатный" не имеют доступа к данным на локальном сервере SQL. В приведенной выше таблице описаны всевозможные различия между уровнями "Бесплатный" и "Стандартный", при этом могут существовать и другие различия, а функции, которые доступны на уровне "Бесплатный", могут быть изменены в любое время.

**Что входит в уровень "Стандартный" Машинного обучения Azure?**

Уровень "Стандартный" Машинного обучения Azure — это оплачиваемая рабочая версия Студии машинного обучения Azure. Ежемесячная плата за использование Студии машинного обучения Azure взимается по принципу "за одно место за месяц" и вычисляется пропорционально, если служба использовалась только часть месяца. Плата за время, затраченное на эксперименты в Студии машинного обучения Microsoft Azure, взимается за час вычислений для активного эксперимента. Плата вычисляется пропорционально за неполные часы.
...
Все указанные ниже начисления сводятся для каждой рабочей области вашей подписки.

* Подписка на место Студии машинного обучения. Эта подписка оплачивается ежемесячно и предоставляет доступ к рабочему пространству Студии машинного обучения Microsoft Azure. Она необходима для проведения экспериментов как с использованием Студии, так и производственных API-интерфейсов.
...
* Избыточные часы вычислений, API "Стандартный" S1, S2 и S3 — плата за вычислительные ресурсы, использованные для новых веб-служб в рабочей среде после того, как включенный объем в имеющихся экземплярах был истрачен. Плата за избыточное использование начисляется по ставке за превышение определенного уровня плана (S1, S2 и S3).
* "Стандартный" S1, S2 и S3, избыточные транзакции API (в тысячах) — плата за вызов в новой рабочей веб-службе после того, как включенный объем в имеющихся экземплярах был истрачен. Плата за избыточное использование начисляется по ставке за превышение определенного уровня плана (S1, S2 и S3).
...
Да, вы можете задействовать модели машинного обучения для промежуточных служб API в рамках бесплатного уровня. Чтобы внедрить промежуточную службу API в рабочую среду и получить рабочую конечную точку для эксплуатируемой службы, необходимо использовать уровень "Стандартный".

**В чем разница между бесплатной пробной версией Azure и уровнем "Бесплатный" Машинного обучения Azure?**

В рамках [бесплатной пробной версии Microsoft Azure](https://azure.microsoft.com/free/) предлагаются кредиты, которые можно применять для любой службы Azure в течение одного месяца. Уровень "Бесплатный" Машинного обучения Azure обеспечивает непрерывный доступ, в частности, к службе машинного обучения Azure для непроизводственных рабочих нагрузок.

**Как переместить эксперимент с уровня "Бесплатный" на уровень "Стандартный"?**

Чтобы скопировать свои эксперименты с уровня "Бесплатный" на "Стандартный", выполните следующие шаги.
...
4.	Выберите рабочее пространство уровня "Стандартный" во всплывающем диалоговом окне и нажмите кнопку "Копировать".
5.	Обратите внимание, что все связанные наборы данных, обученная модель и другое будут скопированы в рабочее пространство уровня "Стандартный" вместе с экспериментом.
...
**Что такое подписка на место службы машинного обучения и когда она нужна?**

Место службы машинного обучения представляет собой рабочее пространство. Всем пользователям, выполняющим эксперименты в Студии машинного обучения Microsoft Azure или службы производственного API-интерфейса, рекомендуется приобретать подписку на место службы машинного обучения.

**За различные рабочие пространства будут выставляться разные счета?**

Плата за рабочее пространство будет разделяться для каждого применимого показателя в одном счете.
...
Гостевой доступ в настоящее время ограничен и доступен в пробном режиме. Он позволяет создавать и запускать эксперименты в Студии машинного обучения Azure бесплатно и без проверки подлинности. Гостевые сеансы временные (не могут быть сохранены) и ограничены 8 часами. Другие ограничения включают отсутствие поддержки R и Python, отсутствие промежуточных API и ограниченный размер набора данных и объем ресурсов хранения. В качестве сравнения: пользователи, которые выполняют вход с учетной записью Майкрософт, имеют полный доступ к бесплатному уровню Студии машинного обучения, описанному выше, который содержит поддерживаемое рабочее пространство и более целостные возможности. Выберите наиболее подходящее решение машинного обучения, щелкнув кнопку "Начать" на сайте [https://studio.azureml.net](https://studio.azureml.net) и выбрав гостевой доступ, или выполните вход при помощи учетной записи Майкрософт.
