<properties
pageTitle="Создание множества моделей из одного эксперимента | Microsoft Azure"
...
# Создание множества моделей машинного обучения и конечных точек веб-службы из одного эксперимента с помощью PowerShell
...
Мы будем использовать уже готовый пример [обучающего эксперимента](https://gallery.cortanaintelligence.com/Experiment/Bike-Rental-Training-Experiment-1) из [коллекции аналитики Кортаны](http://gallery.cortanaintelligence.com). Откройте этот эксперимент в рабочей области [Студии машинного обучения Azure](https://studio.azureml.net).

>[AZURE.NOTE] Для работы с этим примером рекомендуется использовать стандартную рабочую область, а не бесплатную. Мы создадим одну конечную точку для каждого заказчика общим числом 10, и для этого потребуется стандартная рабочая область, так как бесплатная рабочая область ограничена 3 конечными точками. Если у вас имеется только бесплатная рабочая область, просто измените приведенные ниже сценарии, чтобы разрешить только 3 расположения.

Эксперимент использует модуль **чтения**, чтобы импортировать набор данных для обучения *customer001.csv* из учетной записи службы хранилища Azure. Предположим, что мы собрали наборы данных для обучения из всех пунктов аренды велосипедов и сохранили их в одном хранилище BLOB-объектов с именами от *rentalloc001.csv* до *rentalloc10.csv*.

![изображение](./media/machine-learning-create-models-and-endpoints-with-powershell/reader-module.png)

Обратите внимание, что в модуль **обучения модели** был добавлен модуль **выходных данных веб-службы** . При развертывании эксперимента в виде веб-службы конечная точка, связанная с этими выходными данными, будет возвращать обученную модель в формате ILEARNER-файла.

Обратите внимание и на то, что мы устанавливаем для параметра веб-службы URL-адрес, используемый модулем **чтения**. Это позволяет использовать такой параметр, чтобы указать отдельные наборы данных в целях обучения модели для каждого расположения. Существуют и другие способы для выполнения этой задачи, например с помощью SQL-запроса с параметром веб-службы для получения данных из Базы данных SQL Azure или простое использование модуля **входных данных веб-службы** для передачи набора данных в веб-службу.

![изображение](./media/machine-learning-create-models-and-endpoints-with-powershell/web-service-output.png)

Теперь давайте запустим этот обучающий эксперимент, используя значение по умолчанию *rental001.csv* в качестве набора данных для обучения. Если просмотреть выходные данные модуля **оценки** (щелкните выходные данные и выберите **Визуализировать**), вы увидите, мы получаем неплохую производительность с *AUC* = 0,91. На этом этапе мы готовы развернуть веб-службу из этого обучающего эксперимента.

## Развертывание обучающей и оценивающей веб-служб
...
Теперь нам требуется развернуть оценивающую веб-службу. Для этого можно нажать кнопку **Set Up Web Service** (Настройка веб-службы) ниже холста и выбрать **Predictive Web Service** (Прогнозная веб-служба). При этом создается оценивающий эксперимент. Необходимо внести несколько небольших корректировок, чтобы он заработал как веб-служба, например удалить столбец метки cnt из входных данных и ограничить выходные данные лишь идентификатором экземпляра и соответствующим прогнозируемым значением.
...
## Создание 10 идентичных конечных точек веб-службы с помощью PowerShell
...
## Обновление конечных точек для использования отдельных наборов данных для обучения с помощью PowerShell
