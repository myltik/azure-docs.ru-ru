...
### Таблицы Azure 
На виртуальной машине установлено несколько средств Azure. 1) На рабочем столе есть ярлык для доступа к документации по пакету SDK Azure. 2) **AzCopy** используется для перемещения данных из учетной записи хранения Microsoft Azure и обратно. 3) **Обозреватель хранилищ Azure** позволяет просматривать объекты, которые хранятся в учетной записи хранения Azure. 4) **Microsoft Azure Powershell** — средство, используемое для администрирования ресурсов Azure на языке сценариев PowerShell, который также установлен на вашей виртуальной машине.
