...
Приведенный выше сценарий создаст хэш пароля и сохранит его в файле конфигурации Jupyter **C:\ProgramData\jupyter\jupyter_notebook_config.py** в параметре ***c.NotebookApp.password***.

Этот сценарий также включит и запустит сервер Jupyter в фоновом режиме. Сервер Jupyter создается как задача Windows **Start_IPython_Notebook** в планировщике заданий WIndows.  Может потребоваться подождать несколько секунд после задания пароля, прежде чем Notebook откроется в браузере. Сведения о том, как получить доступ к серверу Jupyter Notebook, приведены в разделе **Jupyter Notebook** ниже. 
