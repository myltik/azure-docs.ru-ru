...
**Установите подключение к своему хранилищу данных SQL Azure с помощью Visual Studio.** Инструкции приведены в шагах 1 и 2 статьи [Подключение к хранилищу данных SQL с помощью Visual Studio](../sql-data-warehouse/sql-data-warehouse-connect-overview.md).
...
**Создайте рабочую область машинного обучения Azure, используя подписку Azure.** Инструкции см. в статье [Создание рабочей области машинного обучения Azure](machine-learning-create-workspace.md).

## <a name="getdata"></a>Загрузка данных в хранилище данных SQL

Откройте командную консоль Windows PowerShell. Выполните следующие команды PowerShell, чтобы скачать файлы примеров сценариев SQL, доступные на сайте Github, в локальный каталог, указанный с помощью параметра *-DestDir*. Значение параметра *-DestDir* можно изменить на любой локальный каталог. Если параметр *-DestDir* не существует, сценарий PowerShell его создаст.

>[AZURE.NOTE] Если для создания папки *DestDir* или записи данных в эту папку требуются права администратора, при выполнении представленного ниже сценария PowerShell может потребоваться **Запуск от имени администратора**.
...
Этот **сценарий PowerShell** выполняет следующие задачи:

- **Загружает и устанавливает утилиту AzCopy**, если она еще не установлена.
...
>
> Чтобы исключить все ненужные столбцы и (или) целевые утечки, можно воспользоваться модулем [Выбор столбцов в наборе данных][select-columns] или [Изменение метаданных][edit-metadata]. Дополнительные сведения см. на страницах справки [Выбор столбцов в наборе данных][select-columns] и [Изменение метаданных][edit-metadata].
