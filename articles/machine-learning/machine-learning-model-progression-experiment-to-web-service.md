...
После завершения эксперимента модель можно развернуть как ***веб-службу Azure***, чтобы пользователи могли отправлять в нее новые данные и получать результаты.
...
>[AZURE.NOTE]Существуют и другие способы разработки и развертывания моделей машинного обучения, но в этой статье рассматривается использование студии машинного обучения. Описание способов создания прогнозной веб-службы при помощи R см. в записи блога [Создание и развертывание прогнозных веб-приложений с помощью RStudio и Azure ML](http://blogs.technet.com/b/machinelearning/archive/2015/09/25/build-and-deploy-a-predictive-web-app-using-rstudio-and-azure-ml.aspx).
...
>[AZURE.NOTE]После того как вы нажмете кнопку **Настроить веб-службу**, начинается преобразование обучающего эксперимента в прогнозный эксперимент. В большинстве случаев этого достаточно. Но если вы работаете над сложным обучающим экспериментом (например, имеется несколько объединяющихся путей для обучения), возможно, стоит преобразовать его вручную. Дополнительную информацию о выполнении этого преобразования см. в статье [Преобразование обучающего эксперимента машинного обучения в прогнозный эксперимент](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-convert-training-experiment-to-scoring-experiment/).

### Веб-служба

Убедившись в готовности прогнозного эксперимента, нажмите кнопку **Развернуть веб-службу**, чтобы развернуть модель как ***Веб-службу Azure*** и ввести ее в действие. Пользователи теперь могут отправлять данные в вашу модель с помощью REST API веб-службы и получать результаты. Дополнительные сведения о том, как это сделать, см. в статье[Использование веб-службы машинного обучения Azure, развернутой из эксперимента машинного обучения](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-consume-web-services/).
...
|конфигурация веб-службы в студии|**Управление конечными точками...**|управление конечными точками на портале Azure|
|управление конечными точками на портале Azure|**Редактировать в студии**|холст эксперимента в студии|
...
2.  Щелкните команду **Выполнить**.

3.  Нажмите кнопку **Развернуть веб-службу**.
...
|&nbsp;|- **Переобучение веб-службы**|Преобразует обучающий эксперимент в переобучающий эксперимент (см. раздел «Обновление» ниже)|
|&nbsp;|-*или*- **Развертывание веб-службы**|Если вы настроили для развертывания переобучающий эксперимент, то эта команда разворачивает его как веб-службу|
|Прогнозный эксперимент|**Развернуть веб-службу**|Развертывание прогнозного эксперимента в качестве веб-службы|
...
|Готовый к развертыванию эксперимент|**Развернуть веб-службу**|Развертывает эксперимент как веб-службу, открывает страницу настройки веб-службы|
...
Если изменяется не модель, а только способ обработки данных веб-службой, можно изменить прогнозный эксперимент и затем еще раз нажать кнопку **Развернуть веб-службу**. Веб-служба будет остановлена, обновленный прогнозный эксперимент будет развернут, а затем снова будет запущена веб-служба.

Рассмотрим такой пример: предположим, что прогнозный эксперимент возвращает всю строку входных данных с прогнозируемым результатом. Вы можете решить, что веб-служба должна возвращать только результат. Для этого в прогнозном эксперименте непосредственно перед портом вывода можно добавить модуль **Столбцы проекта**, чтобы исключить все столбцы, кроме результата. При повторном нажатии кнопки **Развернуть веб-службу** будет выполнено обновление веб-службы.
...
-   Преобразование эксперимента: [Преобразование обучающего эксперимента машинного обучения в прогнозный эксперимент](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-convert-training-experiment-to-scoring-experiment/).

-   Развертывание веб-службы: [Развертывание веб-службы машинного обучения Azure](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-publish-a-machine-learning-web-service/).

-   Переобучение модели: [Программное переобучение моделей машинного обучения](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-retrain-models-programmatically/).

Примеры всего процесса см. в этих статьях.

-   [Инструкции по созданию первого эксперимента в Студии машинного обучения Azure](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-create-experiment/)

-   [Пошаговое руководство по разработке решения для прогнозной аналитики в службе машинного обучения Azure для оценки кредитных рисков](https://azure.microsoft.com/ru-RU/documentation/articles/machine-learning-walkthrough-develop-predictive-solution/)
