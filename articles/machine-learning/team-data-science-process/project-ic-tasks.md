...
>[AZURE.NOTE] Далее приведены инструкции по настройке среды TDSP для работы в команде с помощью Visual Studio Team Services (VSTS). Мы объясняем, как выполнить эти задачи с помощью VSTS, так как именно с помощью этой платформы мы реализуем TDSP в Майкрософт. Если в вашей группе для размещения кода используется другая платформа, задачи руководителя команды остаются прежними. Но способ их выполнения будет другим.
...
- На вашем компьютере должна быть установлена система Git. Если вы используете виртуальную машину для обработки и анализа данных (DSVM), система Git уже установлена и вы можете приступать к работе. Если система не установлена, см. [описание платформ и средств](platforms-and-tools.md#appendix).  
- Если вы используете **DSVM Windows**, установите на компьютере [диспетчер учетных данных Git (GCM)](https://github.com/Microsoft/Git-Credential-Manager-for-Windows). Откройте файл README.md, прокрутите его до раздела **Download and Install** (Загрузка и установка) и выберите *latest installer* (последний установщик). Откроется страница последнего установщика. Загрузите EXE-файл установщика и запустите его. 
- Если вы используете **DSVM Linux**, создайте открытый ключ SSH для DSVM и добавьте его на сервер VSTS группы. Дополнительные сведения об SSH см. в разделе о **создании открытого ключа SSH** в [описании платформ и средств](platforms-and-tools.md#appendix). 
...
Ниже приведены ссылки на подробные описания ролей и задач, определенных процессом обработки и анализа данных команды:

- [Group Manager tasks](group-manager-tasks.md) (Задачи менеджера группы)
- [Team Lead tasks](team-lead-tasks.md) (Задачи руководителя команды)
- [Project Lead tasks](project-lead-tasks.md) (Задачи руководителя проекта)
- [Individual Contributor tasks](project-ic-tasks.md) (Задачи отдельных участников)
