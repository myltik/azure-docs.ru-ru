...
На странице **ПАНЕЛЬ МОНИТОРИНГА** нажмите кнопку **Тестирование** в разделе **Конечная точка по умолчанию**.

![Тестирование веб-службы][7]

Появится диалоговое окно, где предлагается ввести входные данные для службы. Это те же столбцы, которые были в изначальном наборе данных по кредитным рискам для немецкого языка.
...
После развертывания веб-службой можно управлять с помощью [классического портала Azure](https://manage.windowsazure.com).
