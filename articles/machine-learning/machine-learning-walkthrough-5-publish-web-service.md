...
С этим экспериментом необходимо выполнить одно дополнительное действие. Мы добавили два модуля [Выполнение скрипта R][execute-r-script], чтобы реализовать функцию взвешивания данных для обучения и тестирования. В конечной модели этого делать не нужно. Студия машинного обучения удалила один модуль [Выполнение сценария R][execute-r-script] при удалении модуля [Разбиение][split], поэтому теперь мы можем удалить другой модуль и подключить модуль [Изменение метаданных][edit-metadata] непосредственно к модулю [Оценка модели][score-model].
...
Данные в веб-службе поступают через модуль **Входные данные веб-службы**, проходят через модуль [Изменение метаданных][edit-metadata] и поступают в модуль [Оценка модели][score-model], где они оцениваются. Затем результаты выводятся из веб-службы через модуль **Выходные данные веб-службы**.

> [AZURE.TIP] С нашими настройками прогнозного эксперимента возвращаются все результаты из модуля [Оценка модели][score-model]. Они включают все входные данные, а также значение кредитного риска и оценку вероятности. Если вы хотите возвращать другие результаты, например только значение кредитного риска, то можно вставить модуль [Выбор столбцов в наборе данных][select-columns] между модулями [Оценка модели][score-model] и **Выходные данные веб-службы**, чтобы исключить столбцы, которые не должны возвращаться веб-службой.
...
[select-columns]: https://msdn.microsoft.com/ru-RU/library/azure/1ec722fa-b623-4e26-a44e-a50c6d726223/
