...
Данные, использующиеся в этой статье, а также сценарии на языке R можно [скачать здесь][download]. Эти данные основаны на информации, собранной университетом Висконсина, которая доступна по адресу http://future.aae.wisc.edu/tab/production.html.
...
Для разработки, тестирования и отладки небольших фрагментов кода на языке R мы будем использовать RStudio. Впоследствии мы вставим эти готовые фрагменты в модуль [Выполнение сценария R][execute-r-script] в Студии машинного обучения.

###Модуль «Выполнение сценария R»

В Студии машинного обучения сценарии на языке R выполняются в модуле [Выполнение сценария R][execute-r-script]. Пример модуля [Выполнение сценария R][execute-r-script] в Студии машинного обучения показан на рис. 1.
...
Используя рис. 1, рассмотрим основные элементы среды Студии машинного обучения для работы с модулем [Выполнение сценария R][execute-r-script].
...
- В нижней части правой панели отображаются свойства модуля [Выполнение сценария R][execute-r-script]. Журнал ошибок и журнал выходных данных можно просмотреть, щелкнув соответствующие элементы панели.

Разумеется, в оставшейся части этого документа мы рассмотрим модуль [Выполнение сценария R][execute-r-script] подробнее.

При работе с более сложными функциями R для изменения, тестирования и отладки кода я рекомендую использовать RStudio. Как и в случае разработки любого программного обеспечения, создавайте код пошагово, отдельными фрагментами, и тестируйте его на небольших, простых проверочных примерах. Затем вырезайте и вставляйте готовые функции в окно сценария модуля [Выполнение сценария R][execute-r-script]. Такой подход позволяет в полной мере использовать как интегрированную среду разработки (IDE) RStudio, так и возможности Машинного обучения Azure.
...
- В поле **Поиск элементов эксперимента** в верхней части левой панели введите текст [Выполнение сценария R][execute-r-script]. Этот модуль появится в списке результатов поиска.

- Перетащите модуль [Выполнение сценария R][execute-r-script] на рабочую область.

- Соедините порт ввода **набора данных csdairydata.csv** с крайним левым портом вывода (**Набор данных 1**) модуля [Выполнение сценария R][execute-r-script].
...
4.	Перетащите ZIP-файл из раздела **Наборы данных** на **холст Студии машинного обучения**.
...
> [AZURE.NOTE]Эти же результаты можно получить с помощью порта вывода «Устройство R» модуля [Выполнение сценария R][execute-r-script]. Порты вывода модуля [Выполнение сценария R][execute-r-script] мы рассмотрим подробнее в следующем разделе.
...
Выполнив эксперимент, щелкните порт вывода «Итоговый набор данных 1», а затем — **Визуализировать**. Вы увидите нечто вроде этого (рис. 6):
...
Порт вывода «Устройство R» модуля [Выполнение сценария R][execute-r-script] предназначен для вывода сообщений и графики. Как стандартный вывод данных, так и сообщения об ошибках на R направляются в порт вывода "Устройство R".
...
Выполним этот код и посмотрим на результат. Вывод данных, полученных после нажатия кнопки **Визуализировать** в меню порта «Устройство R», приведен на рис. 9.

![Сводка таблицы данных с фактором в качестве переменной][10]

*Рис. 9. Сводка таблицы данных с фактором в качестве переменной*

Тип столбца Month изменился на **Factor w/ 14 levels**. Такой вариант нас не устраивает, поскольку в году всего 12 месяцев. Также можно убедиться, что тип выходных данных — **Категориальный**, выбрав **Визуализировать** в меню порта «Итоговый набор данных».
...
Выполним эксперимент повторно и **визуализируем** вывод данных порта «Устройство R». Результат представлен на рис. 10.

![Сводка таблицы данных с правильным количеством уровней фактора][11]
...
> [AZURE.NOTE]С этого момента я буду показывать только дополнительный код, который добавляется в модуль [Выполнение сценария R][execute-r-script]. Я буду добавлять новые строки **перед** функцией `str()`. Эта функция нужна для проверки результатов в Студии машинного обучения Azure.

Добавим следующую строку в код на R в модуле [Выполнение сценария R][execute-r-script].

	# Remove two columns we do not need
	cadairydata <- cadairydata[, c(-1, -2)]

Выполните этот код в своем эксперименте и проверьте результат с помощью команды **Визуализировать** в меню порта «Устройство R». Результат приведен на рис. 11.

![Сводка таблицы данных после удаления двух столбцов][12]
...
Приступим к разбору примера. Перетащим **новый** модуль [Выполнение сценария R][execute-r-script] в свой эксперимент. Соединим порт вывода «Итоговый набор данных 1» существующего модуля [Выполнение сценария R][execute-r-script] с портом ввода «Набор данных 1» нового модуля [Выполнение сценария R][execute-r-script].
...
![Сводка таблицы данных в модуле «Выполнение сценария R»][15]
...
####Создание объекта временных рядов
...
![Сводка таблицы данных с объектом временных рядов][16]
...
![Список CCF-объектов после попарного корреляционного анализа][19]
...
Обратите внимание, что имена строк находятся в столбце таблицы данных. Благодаря этому имена строк сохраняются при выводе из модуля [Выполнение сценария R][execute-r-script].

Результат выполнения кода после **Визуализации** вывода из порта «Итоговый набор данных» приведен на рис. 19. Имена строк находятся в первом столбце, как и предполагалось.
...
В первую очередь добавьте в эксперимент **новый** модуль [Выполнение сценария R][execute-r-script]. Соедините порт вывода **Итоговый набор данных** существующего модуля [Выполнение сценария R][execute-r-script] с портом ввода **Набор данных 1** нового модуля. Результат изображен на рисунке 20.
