...
Данные, которые используются в этой статье, а также сценарии на языке R можно [скачать здесь][download]. Эти данные основаны на информации, собранной университетом Висконсина и доступной по адресу http://future.aae.wisc.edu/tab/production.html.
...
##<a name="<a-id="mlstudio"></a>interact-with-r-language-in-machine-learning-studio"></a><a id="mlstudio"></a>Работа с языком R в Студии машинного обучения
...
Для разработки, тестирования и отладки небольших фрагментов кода на языке R мы будем использовать RStudio. Впоследствии мы вставим эти готовые фрагменты в модуль [Execute R Script][execute-r-script] (Выполнение сценария R) в студии машинного обучения.  

###<a name="the-execute-r-script-module"></a>Модуль «Выполнение сценария R»

В студии машинного обучения сценарии на языке R выполняются в модуле [Execute R Script][execute-r-script]. Пример модуля [Execute R Script][execute-r-script] в студии машинного обучения показан на рис. 1.
...
Используя рис. 1, рассмотрим основные элементы среды студии машинного обучения для работы с модулем [Execute R Script][execute-r-script].
...
- В нижней части правой панели отображаются свойства модуля [Execute R Script][execute-r-script]. Журнал ошибок и журнал выходных данных можно просмотреть, щелкнув соответствующие элементы панели.

В оставшейся части этого документа мы рассмотрим модуль [Execute R Script][execute-r-script] подробнее.

При работе с более сложными функциями R для изменения, тестирования и отладки кода я рекомендую использовать RStudio. Как и в случае разработки любого программного обеспечения, создавайте код пошагово, отдельными фрагментами, и тестируйте его на небольших, простых проверочных примерах. Затем вырезайте и вставляйте готовые функции в окно сценария R модуля [Execute R Script][execute-r-script]. Такой подход позволяет в полной мере использовать как интегрированную среду разработки (IDE) RStudio, так и возможности Машинного обучения Azure.  

####<a name="execute-r-code"></a>Выполнение кода R

Любой код R в модуле [Execute R Script][execute-r-script] выполняется при запуске эксперимента нажатием кнопки **Run** (Запуск). После выполнения кода на значке модуля [Execute R Script][execute-r-script] появляется флажок.
...
Повторюсь: я рекомендую использовать RStudio для отладки и тестирования небольших фрагментов кода на R. Однако бывают случаи, когда необходимо отслеживать проблемы кода R непосредственно в модуле [Execute R Script][execute-r-script]. Кроме того, это неплохая привычка — проверять результаты работы в Студии машинного обучения.
...
Похоже, нужно просмотреть файл output.log, чтобы прочитать сообщение об ошибке в коде на R. Щелкните [Execute R Script][execute-r-script], а затем — **View output.log** (Просмотреть журнал выходных данных) на **панели свойств** справа. Откроется новое окно браузера, где мы увидим следующее:
...
Служба машинного обучения Azure содержит более 350 предустановленных пакетов на языке R. Чтобы получить список предустановленных пакетов, используйте приведенный ниже код в модуле [Execute R Script][execute-r-script].
...
RStudio — широко распространенная среда IDE для языка R. Я буду использовать RStudio для изменения, тестирования и отладки фрагментов кода на R, используемых в этом руководстве. Когда код на R будет готов и проверен, его можно просто вырезать из редактора RStudio и вставить в модуль [Execute R Script][execute-r-script] в студии машинного обучения.  
...
Подробные сведения об использовании RStudio можно найти в [приложении А][appendixa].  

##<a name="<a-id="scriptmodule"></a>get-data-in-and-out-of-the-execute-r-script-module"></a><a id="scriptmodule"></a>Добавление данных в модуль «Выполнение сценария R» и их извлечение оттуда

В этом разделе мы рассмотрим ввод и вывод данных в модуле [Execute R Script][execute-r-script]. Мы ознакомимся с принципами обработки различных типов данных, передаваемых в модуль [Execute R Script][execute-r-script] и получаемых из него.
...
####<a name="<a-id="loading"></a>load-the-dataset"></a><a id="loading"></a>Загрузка набора данных
...
- Щелкните __+ NEW__ (+ Создать) в нижнем левом углу экрана и выберите **Dataset** (Набор данных).
...
- Щелкните __+ NEW__ (+ Создать) в левом нижнем углу и выберите **Experiment** (Эксперимент), а затем — **Blank Experiment** (Пустой эксперимент).
...
- В поле **Search experiment items** (Поиск элементов эксперимента) в верхней части левой панели введите текст [Execute R Script][execute-r-script]. Этот модуль появится в списке результатов поиска.

- Перетащите модуль [Execute R Script][execute-r-script] на палитру.  

- Соедините порт вывода **набора данных csdairydata.csv** с крайним левым портом ввода (**Dataset1** (Набор данных 1)) модуля [Execute R Script][execute-r-script].
...
Рассмотрим порты ввода модуля [Execute R Script][execute-r-script]. В этом примере мы будем считывать данные по молочной продукции в Калифорнии в модуль [Execute R Script][execute-r-script].  

У модуля [Execute R Script][execute-r-script] может быть три порта ввода. В зависимости от приложения можно использовать любой из них или все. Кроме того, иногда целесообразно использовать сценарий R вообще без портов ввода.  
...
Порт ввода Script Bundle (Пакет сценариев) позволяет передать содержимое ZIP-файла в модуль [Execute R Script][execute-r-script]. Для считывания содержимого ZIP-файла в код на R можно использовать одну из этих команд.

    source("src/yourfile.R") # Reads a zipped R script
    load("src/yourData.rdata") # Reads a zipped R data file

> [AZURE.NOTE] Машинное обучение Azure обрабатывает файлы ZIP-архива, как если бы они находились в каталоге src/, поэтому необходимо добавлять имя этого каталога в начало имен файлов. Например, если ZIP-файл содержит файлы `yourfile.R` и `yourData.rdata` в корне, вы будете рассматривать их как `src/yourfile.R` и `src/yourData.rdata` при использовании `source` и `load`.
...
2.  Создайте ZIP-файл и скопируйте в него свой скрипт. В Windows щелкните файл правой кнопкой мыши и выберите __Отправить__, а затем — __Сжатая ZIP-папка__. Будет создан ZIP-файл, содержащий файл "simpleplot.R".
...
5.  Соедините значок порта вывода **ZIP-данных** с портом ввода **Script Bundle** (Пакет сценариев) модуля [Execute R Script][execute-r-script].

6.  В окне кода модуля [Execute R Script][execute-r-script] введите функцию `source()` с именем вашего ZIP-файла. В данном случае введено `source("src/simpleplot.R")`.  

7.  Обязательно щелкните **Сохранить**.

После выполнения этих шагов модуль [Execute R Script][execute-r-script] выполнит сценарий R в ZIP-файле при запуске эксперимента. На этом этапе ваш эксперимент должен выглядеть, как изображено на рисунке 5.
...
Выполните эксперимент, нажав кнопку **Запуск** . По завершении выполнения щелкните модуль [Execute R Script][execute-r-script], а затем — **View output log** (Просмотреть журнал выходных данных) на панели свойств. В браузере откроется новая страница с содержимым файла output.log. Прокрутив ее вниз, вы увидите следующее:
...
> [AZURE.NOTE] Эти же результаты можно получить с помощью порта вывода R Device (Устройство R) модуля [Execute R Script][execute-r-script]. Порты вывода модуля [Execute R Script][execute-r-script] мы рассмотрим подробнее в следующем разделе.  
...
Порт вывода R Device (Устройство R) модуля [Execute R Script][execute-r-script] предназначен для вывода сообщений и графики. Как стандартный вывод данных, так и сообщения об ошибках на R направляются в порт вывода "Устройство R".  
...
##<a name="<a-id="filtering"></a>data-filtering-and-transformation"></a><a id="filtering"></a>Фильтрация и преобразование данных
...
Мы уже можем считать данные по молочной продукции Калифорнии в код R модуля [Execute R Script][execute-r-script], но нужно убедиться, что в столбцах содержатся данные нужного типа и формата.  
...
Я удалил строку, создававшую матрицу точечной диаграммы, и добавил строку, преобразующую тип столбца Month в фактор. В нашем эксперименте мы просто вырежем код R и вставим его в окно кода модуля [Execute R Script][execute-r-script]. Также можно обновить ZIP-файл и передать его в Студию машинного обучения Azure, но это будет дольше.  
...
> [AZURE.NOTE] С этого момента в этом разделе будет показан только дополнительный код, который мы добавляем в модуль [Execute R Script][execute-r-script]. Новые строки будут добавляться **перед** функцией `str()`. Эта функция нужна для проверки результатов в Студии машинного обучения Azure.

Добавим следующую строку в код R в модуле [Execute R Script][execute-r-script].
...
Теперь все наши данные в порядке и мы можем приступать к моделированию. Просмотрев визуализацию вывода данных через порт Result Dataset (Итоговый набор данных) модуля [Execute R Script][execute-r-script], вы увидите, что тип столбца Month (Месяц) теперь Categorical (Категориальный) с 12 уникальными значениями, что нам и требовалось.

##<a name="<a-id="timeseries"></a>time-series-objects-and-correlation-analysis"></a><a id="timeseries"></a>Объекты временного ряда и корреляционный анализ
...
Приступим к разбору примера. Перетащим **новый** модуль [Execute R Script][execute-r-script] в свой эксперимент. Подключим порт вывода Result Dataset1 (Итоговый набор данных 1) существующего модуля [Execute R Script][execute-r-script] к порту ввода Dataset1 (Набор данных 1) нового модуля [Execute R Script][execute-r-script].
...
Обратите внимание, что имена строк находятся в столбце таблицы данных. Благодаря этому имена строк сохраняются при выводе из модуля [Execute R Script][execute-r-script].
...
##<a name="<a-id="seasonalforecasting"></a>time-series-example:-seasonal-forecasting"></a><a id="seasonalforecasting"></a>Пример временных рядов: сезонное прогнозирование
...
В первую очередь добавьте в эксперимент **новый** модуль [Execute R Script][execute-r-script]. Подключите порт вывода **Result Dataset** (Итоговый набор данных) существующего модуля [Execute R Script][execute-r-script] к порту ввода **Dataset1** (Набор данных 1) нового модуля. Результат изображен на рисунке 20.
...
Для проверки корректности создадим диаграмму временных рядов данных молочного производства Калифорнии с использованием полученной линии тренда. Для создания модели и построения диаграммы приведенный ниже код добавлен в модуль [Execute R Script][execute-r-script] в службе машинного обучения Azure (а не в RStudio). Результат показан на рис. 23.
...
Построим еще одну диаграмму временного ряда данных по молочному производству Калифорнии, чтобы проверить работу модели. Следующий код был добавлен в модуль [Execute R Script][execute-r-script] в службе машинного обучения Azure, чтобы создать модель и построить диаграмму.
...
##<a name="<a-id="appendixa"></a>appendix-a:-guide-to-rstudio"></a><a id="appendixa"></a>ПРИЛОЖЕНИЕ А. Руководство по RStudio
...
##<a name="<a-id="appendixb"></a>appendix-b:-further-reading"></a><a id="appendixb"></a>ПРИЛОЖЕНИЕ Б. Дополнительные материалы
