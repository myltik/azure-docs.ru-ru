---
title: Краткое руководство по языку программирования R для службы машинного обучения | Microsoft Docs
...
> [!NOTE]
> Эти же результаты можно получить с помощью порта вывода R Device (Устройство R) модуля [Execute R Script][execute-r-script]. Порты вывода модуля [Execute R Script][execute-r-script] мы рассмотрим подробнее в следующем разделе.  
> 
> 
...
> [!NOTE]
> С этого момента в этом разделе будет показан только дополнительный код, который мы добавляем в модуль [Execute R Script][execute-r-script]. Новые строки будут добавляться **перед** функцией `str()`. Эта функция нужна для проверки результатов в Студии машинного обучения Azure.
> 
> 
