...
После развертывания веб-службы для нее создается конечная точка по умолчанию. Эту конечную точку можно вызвать, используя ключ API. Дополнительные конечные точки с собственными ключами можно добавить на классическом портале Azure или программным путем с использованием интерфейсов API управления веб-службами. Для вызова веб-службы требуются ключи доступа. Дополнительные сведения см. в статье [Как использовать веб-службу машинного обучения Azure, развернутую из эксперимента машинного обучения](consume-web-services.md).
