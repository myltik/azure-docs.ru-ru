<properties 
	...
	description="Узнайте, как определить конечную точку HTTPS для веб-роли Node.js и как передать SSL-сертификат, чтобы обеспечить безопасность приложения." 
...
определить конечную точку HTTPS для приложения Node.js, размещенного в облачной службе Azure, расположенной в веб-роли, а также как передать
сертификат SSL, чтобы обеспечить защиту своего приложения.

> [AZURE.NOTE] Действия, описанные в данной статье, применимы только к приложениям узла, размещенным в качестве облачной службы Azure в веб-роли; информацию, применимую к веб-сайтам, см. в разделе [Настройка SSL-сертификата для веб-сайта Azure](web-sites-configure-ssl-certificate.md).

Эта задача включает следующие шаги:

-   [Шаг 1. Создание службы Node.js и ее публикация в облаке]
-   [Шаг 2. Получение сертификата SSL]
-   [Шаг 3. Импортирование SSL-сертификата]
...
сертификатом сервера и SSL-подключением управляют
службы IIS, поэтому служба Node.js может быть записана
в качестве HTTP-службы. Вы можете создать простую службу Node.js 'hello
...
Теперь необходимо изменить определение службы для ссылки на сертификат
установили.

## <a name="step4"> </a>Шаг 4. Изменение файлов определения и конфигурации службы

Ваше приложение должно ссылаться на сертификат, также точка HTTPS
необходимо добавить конечную точку HTTPS. Для этого следует обновить файлы определения и
и конфигурации службы.

1.  В каталоге службы откройте файл определения службы
    (ServiceDefinition.csdef), добавьте раздел **Certificates** в раздел **WebRole** и добавьте в него следующие данные о
    сертификате:
    ...
    его расположение и имя хранилища, в котором он находится. Поскольку мы установили сертификат в хранилище сертификатов пользователя, используется значение "My". Можно использовать другие хранилища сертификатов. См. [Практическое руководство.
...
3.  В файлах конфигурации службы
    ...
    **ServiceConfiguration.Local.cscfg**) добавьте сертификат в 
    пустой раздел **Certificates** в разделе **Role**,
    заменив приведенный ниже пример значения отпечатка значением своего
    сертификата:
...
4.  Обновите конфигурацию службы в облаке, выполнив публикацию
...
[Как настроить SSL-сертификат на конечной точке HTTPS]
  ...
  [Практическое руководство. Как связать сертификат со службой]: http://msdn.microsoft.com/library/windowsazure/gg465718.aspx
