<properties linkid="dev-nodejs-enablessl" urlDisplayName="Включение SSL" pageTitle="Настройка SSL для облачной службы (Node.js) - Azure" metaKeywords="Node.js Azure SSL, Node.js Azure HTTPS" description="Сведения об указании конечной точки HTTPS для веб-роли Node.js и передаче сертификата SSL для обеспечения безопасности приложения." metaCanonical="" services="cloud-services" documentationCenter="Node.js" title="Настройка SSL для приложения Node.js в веб-роли Azure" authors="" solutions="" manager="" editor="" />
...
защиты данных, передаваемых через Интернет. В этой теме, посвященной общим задачам, обсуждается, как определить конечную точку HTTPS для приложения Node.js, размещенного в облачной службе Azure, расположенной в веб-роли, а также как передать SSL-сертификат, чтобы обеспечить безопасность приложения.

<div class="dev-callout">Примечание.
<p>Действия, описанные в данной статье, применимы только к приложениям узла, размещенным в качестве облачной службы Azure в веб-роли; сведения, применимые к веб-сайтам, см. в разделе <a href="../web-sites-configure-ssl-certificate/">Настройка SSL-сертификата для веб-сайта Azure</a>.</p>
</div>
...
При развертывании приложения Node.js в веб-роли Azure,
сертификат сервера и SSL-подключение управляются IIS,
поэтому служба Node.js может быть записана, как если бы
являлась службой http. Чтобы создать простую службу Node.js "hello
...
<div class="dev-callout">
<strong>Примечание.</strong>
<p>Действия, используемые в этом разделе, основаны на версии Windows 8 мастера импорта сертификатов. При использовании предыдущей версии Windows приведенные здесь действия могут не совпадать с порядком, отображаемым в мастере. В этом случае полностью прочитайте данный раздел, прежде чем использовать мастер импорта сертификатов, чтобы понимать, какие действия нужно выполнять в целом.</p>
</div>

Чтобы импортировать SSL-сертификат, выполните следующие действия.

1.   С помощью проводника Windows перейдите в каталог, где находится **PFX-файл**, содержащий сертификат, а затем дважды щелкните сертификат. Откроется мастер импорта сертификатов.
...
Теперь необходимо изменить определение службы для ссылки на
установленный сертификат.

## <a name="step4"> </a>Шаг 4. Изменение файлов определения и конфигурации службы

Ваше приложение должно ссылаться на сертификат, также должна быть добавлена
конечная точка HTTPS. Для этого следует обновить файлы определения
службы и конфигурации службы.

1. В каталоге службы откройте файл определения службы (ServiceDefinition.csdef),
     добавьте раздел **Certificates** внутри раздела **WebRole** и включите следующие сведения о сертификате:
    ...
    В разделе **Certificates** определяется имя сертификата,
     его расположение и имя хранилища, в котором он находится. Поскольку мы установили сертификат в хранилище сертификатов пользователя, используется значение "My". Можно использовать другие хранилища сертификатов. Дополнительные сведения см. в разделе [Связь
...
3.  В файлах конфигурации службы
    (**ServiceConfiguration.Cloud.cscfg** и
    **ServiceConfiguration.Local.cscfg**) добавьте сертификат в пустой раздел
    **Certificates** раздела **Role**,
    заменяя приведенный ниже пример значения отпечатка на значение отпечатка
    вашего сертификата:
...
4.  Обновите конфигурацию службы в облаке, выполнив
...
3.  Откроется новый браузер, в котором будет отображен ваш веб-сайт.
