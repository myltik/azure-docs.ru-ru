<properties title="How to Configure PHP in Azure Websites" pageTitle="Как настроить PHP на веб-сайтах Microsoft Azure" metaKeywords="Azure, Azure Web Sites, configuration, PHP" description="Learn how to configure the default PHP installation or add a custom PHP installation in Azure Websites." services="Web Sites" documentationCenter="PHP" authors="tomfitz" manager="wpickett" />
...
1. На портале Microsoft Azure выберите панель мониторинга веб-сайта и щелкните **Настройка.**.
...
1. Добавьте параметры конфигурации в файл .user.ini, используя тот же синтаксис, что и для файла php.ini. Например, если вы хотите включить параметр display_errors и установить для параметра upload_max_filesize значение 10 МБ, в файле .user.ini будет указан следующий текст:
...
1. Разверните приложение.
1. На портале Microsoft Azure выберите панель мониторинга веб-сайта и щелкните **Настроить**.

	![Configure tab on Web Sites dashboard][configure]

1. В разделе **app settings** создайте ключ **PHP_EXTENSIONS** и присвойте ему значение **bin\your-ext-file**. Чтобы включить несколько расширений, используйте разделенный запятыми список DLL-файлов.
...
1. Измените файл php.ini для среды выполнения. Обратите внимание, что все параметры конфигурации, являющиеся директивами только системного уровня, будут игнорироваться веб-сайтами Azure. (Дополнительную информацию о директивах, действующих только на уровне системы, см. в разделе [Список директив php.ini].)
...
1. В разделе **сопоставления обработчика**, добавьте к РАСШИРЕНИЮ "*.php" и добавьте путь к исполняемому файлу php-cgi.exe. Если поместить среду выполнения PHP в каталог bin корневого каталога вашего приложения, путь будет выглядеть следующим образом: D:\home\site\wwwroot\bin\php\php-cgi.exe.
...
- [Скачивание пакета SDK для Azure для PHP]


[бесплатная пробная версия]: https://www.windowsazure.com/ru-ru/pricing/free-trial/
