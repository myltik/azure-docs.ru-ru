...
Следуйте приведенным [здесь][здесь] инструкциям, чтобы создать виртуальную машину дистрибутива *OpenSUSE* или *Ubuntu*. В этом учебнике используются OpenSUSE 12.3 и Ubuntu Server 13.04. Предположим, что имя пользователя по умолчанию — *azureuser*.

### Виртуальная машина Windows

Следуйте приведенным [здесь][1] инструкциям, чтобы создать виртуальную машину с дистрибутивом *Windows Server 2012 Datacenter*. В этом учебном курсе предполагается, что имя пользователя — *azureuser*.
...
конечную точку», чтобы создать новую конечную точку (в этом примере носит название `ipython_nb`). Выберите TCP для протокола, 443 для общего порта и 9999 для частного порта:
...
Область, отмеченная запросом `In []:`, является областью ввода: здесь можно ввести любой допустимый код Python и он будет исполнен, если нажать клавиши `Shift-Enter` или щелкнуть значок «Воспроизведение» (направленный вправо треугольник на панели инструментов).
...
Сама работа с записной книжкой должна быть интуитивно понятна для любого, кто использовал Python и редактор текстов, так как это является в некотором смысле сочетанием обоих вариантов: здесь можно выполнять блоки кода Python, но можно также сохранять заметки и другой текст, изменив стиль ячейки с «Код» на «Разметка» в раскрывающемся меню на панели инструментов:

![Снимок экрана][8]

Но это гораздо больше, чем редактор текстов, так как записная книжка IPython позволяет совмещать вычисления и широкие возможности мультимедиа (текст, графика, видео и практически любые данные, которые способен отображать современный браузер). Например, можно сочетать пояснительные видео с расчетами для использования в образовательных целях:
