<properties linkid="develop-python-ipython-notebook" urlDisplayName="IPython Notebook" pageTitle="IPython Notebook — учебный курс Azure" metaKeywords="" description="В этом учебном курсе описывается развертывание IPython Notebook в Azure с использованием виртуальных машин под управлением Linux или Windows." metaCanonical="" services="virtual-machines" documentationCenter="Python" title="IPython Notebook в Azure" authors=""  solutions="" writer="" manager="" editor=""  />
...
В этом документе показано, как развернуть IPython Notebook в Azure с использованием виртуальных машин Linux или Windows.  С помощью IPython Notebook в Azure можно с легкостью предоставить веб-интерфейс для масштабируемых вычислительных ресурсов в сочетании со всеми возможностями Python и его многочисленных библиотек.  Так как вся установка ведется в облаке, пользователи могут получать доступ к этим ресурсам без необходимости вести настройку локально вне современного веб-обозревателя.

[WACOM.INCLUDE [create-account-and-vms-note](../includes/create-account-and-vms-note.md)]

## Создание и настройка виртуальной машины в Azure

Первым шагом является создание виртуальной машины (ВМ) в Azure.
Эта виртуальная машина является полноценной операционной системой в облаке и будет использоваться для запуска IPython Notebook. Azure способна размещать виртуальные машины Linux и Windows. Мы рассмотрим установку IPython на обоих типах виртуальных машин.

### Виртуальная машина Linux

Следуйте приведенным [здесь][portal-vm-linux] инструкциям, чтобы создать виртуальную машину дистрибутива *OpenSUSE* или *Ubuntu*. В этом учебном курсе используются OpenSUSE 12.3 и Ubuntu Server 13.04. Предположим, что имя пользователя по умолчанию — *azureuser*.
...
Чтобы создать конечную точку, перейдите на панель мониторинга виртуальной машины и нажмите кнопку "Конечные точки", а затем — "Добавить конечную точку", чтобы создать новую конечную точку (в этом примере носит название `ipython_nb`). Выберите TCP для протокола, 443 для общего порта и 9999 для частного порта:
...
1.  Установите Python 2.7.5 (32-разрядный) с сайта [python.org](http://www.python.org/download). 
    Также потребуется добавить `C:\Python27` и `C:\Python27\Scripts` в переменную среды `PATH`.

1.  Установите дистрибутив, загрузив файл **distribute_setup.py**
    с сайта [python-distribute.org](http://python-distribute.org/) и выполнив следующую команду:
...
1.  Загрузите и установите [NumPy][numpy] с помощью установщика бинарных файлов
    `.exe`, доступного на этом веб-сайте.  На момент написания этой статьи последняя версия — **numpy-1.7.1-win32-superpack-python2.7.exe**.

1.  Загрузите и установите [Matplotlib][matplotlib] с помощью установщика бинарных файлов
    `.exe`, доступного на этом веб-сайте.  На момент написания этой статьи последняя версия — **matplotlib-1.2.1.win32-py2.7.exe**.

1.  Загрузите и установите OpenSSL. Потребуется установить как 
    **Win32 OpenSSL v1.0.1e Light**, так и **Visual C++ 2008 Redistributable** со страницы
    [http://slproweb.com/products/Win32OpenSSL.html](http://slproweb.com/products/Win32OpenSSL.html). Также потребуется добавить
    `C:\OpenSSL-Win32\bin` в переменную среды `PATH` .
...
Область, отмеченная запросом `In []:`, является областью ввода: здесь можно ввести любой допустимый код Python и он будет исполнен, если нажать клавиши `Shift + ВВОД` или щелкнуть значок "Воспроизведение" (направленные вправо треугольник на панели инструментов).
...
Достаточно загрузить файлы `.ipynb` с сайта на панель мониторинга виртуальной машины Azure для записной книжки (или загрузить их непосредственно в виртуальную машину).
...
[Средствах Python для Visual Studio](http://pytools.codeplex.com) (PTVS). PTVS является бесплатным подключаемым модулем с открытым исходным кодом от корпорации Майкрософт, который превращает Visual Studio в среду разработки Python с расширенными возможностями, включающую редактор с интеграцией IntelliSense, средства отладки, профилирования и параллельных вычислений.
