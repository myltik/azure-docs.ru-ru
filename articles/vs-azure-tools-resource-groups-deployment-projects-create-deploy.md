...
В этой статье используется Visual Studio 2015 с обновлением 2 и пакет Microsoft Azure SDK для .NET 2.9. При использовании Visual Studio 2013 с пакетом Azure SDK 2.9 инструкции будут в целом такими же. Можно использовать версии пакета Azure SDK 2.6 и выше, но в таком случае описанный в статье рабочий процесс может отличаться. Настоятельно рекомендуется установить последнюю версию пакета [Azure SDK](https://azure.microsoft.com/downloads/) перед началом работы.
...
1. Вы можете изменить параметры развертывания, нажав кнопку **Изменить параметры**. Укажите значения параметров и нажмите кнопку **Сохранить**.
...
1. По окончании развертывания в окне **Вывод** появится сообщение следующего содержания:
...
1. Откройте в браузере [портал Azure](https://portal.azure.com/) и войдите в свою учетную запись. Чтобы просмотреть группу ресурсов, щелкните **Группы ресурсов** и укажите группу ресурсов с выполненным развертыванием.
...
1. После создания веб-приложения добавьте ссылку на проект веб-приложения в проект группы ресурсов.
    ...
    - Свойство **Дополнительные свойства** содержит данные о промежуточном расположении пакета веб-развертывания, который будет отправлен в хранилище Azure. 
    - Свойство **Включать путь к файлу** содержит путь к расположению, в котором будет создан пакет. Свойство **Включать целевые объекты** содержит команду, которая будет выполнена при развертывании. 
    - Значение по умолчанию **Build;Package** отвечает за сборку и создание пакета веб-развертывания (package.zip) при развертывании.  
