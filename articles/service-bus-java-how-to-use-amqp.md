<properties urldisplayname="Service Bus AMQP" headerexpose="" pageTitle="Использование протокола AMQP 1.0 с API для Service Bus для Java: Azure " metakeywords="протокол AMQP для сообщений Java, протокол AMQP для Service Bus, закачка библиотеки JMS для протокола" footerexpose="" description="Узнайте, как использовать службу сообщений Java (JMS) с Azure Service Bus и протоколом AMQP 1.0." umbraconavihide="0" disquscomments="1" metaCanonical="" title="How to use the Java Message Service (JMS) API with Service Bus & AMQP 1.0" authors="sethm"  solutions="" documentationCenter="Java" writer="sethm" manager="timlt" editor="mattshel" />
...
В этом руководстве предполагается, что уже создано пространство имен Service Bus, содержащее очередь с именем "queue1". Если это не так, можно создать пространство имен и очередь, используя [портал управления Azure](http://manage.windowsazure.com). Дополнительную информацию о создании пространства имен и очередей Service Bus см. в практическом руководстве [Использование очередей Service Bus](https://www.windowsazure.com/ru-ru/develop/net/how-to-guides/service-bus-queues/).
...
**Примечание**. При получении данных из подписки раздела Service Bus физическое имя, указанное в JNDI, должно быть именем раздела. Имя подписки предоставляется при создании устойчивой подписки в коде приложения JMS. В документе [Протокол AMQP 1.0 для Service Bus. Руководство разработчика](http://msdn.microsoft.com/ru-ru/library/windowsazure/jj841071.aspx) содержится более подробная информация о работе с подписками разделов Service Bus из JMS.
...
Используя приведенный выше пример приложения JMS и аналогичное приложение .NET, взятое из связанного руководства [Использование протокола AMQP 1.0 с API Service Bus .NET](http://aka.ms/lym3vk), можно организовать обмен сообщениями между .NET и Java. 

Дополнительную информацию о межплатформенном обмене сообщениями с помощью Service Bus и AMQP 1.0 см. в статье [Протокол AMQP 1.0 для Service Bus. Руководство разработчика](http://msdn.microsoft.com/ru-ru/library/windowsazure/jj841071.aspx).
...
* Временные назначения, т. е. **TemporaryQueue**, **TemporaryTopic** в данный момент не поддерживаются, как и API-интерфейсы **QueueRequestor** и **TopicRequestor**, которые их используют.
...
Протокол AMQP 1.0 Service Bus можно также использовать из других языков, в числе которых .NET, C, Python и PHP. Компоненты, созданные с помощью этих различных языков, могут надежно и точно обмениваться сообщениями, используя поддержку AMQP 1.0 в Service Bus. Дополнительную информацию см. в статье [Протокол AMQP 1.0 для Service Bus. Руководство разработчика](http://msdn.microsoft.com/ru-ru/library/windowsazure/jj841071.aspx).

##Дополнительные сведения

* [Поддержка AMQP 1.0 в служебной шине](http://aka.ms/pgr3dp)
