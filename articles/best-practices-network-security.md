---
title: Облачные службы Microsoft Cloud и сетевая безопасность | Microsoft Docs
...
* Резервное правило (для трафика, который не соответствует указанным выше правилам).
  1. Правило запрета любого трафика. Это правило всегда должно быть последним (по приоритету). Если трафик не соответствует ни одному из предыдущих правил, он будет отклонен этим правилом. Это правило по умолчанию, и обычно оно активно. В него обычно не требуется вносить никаких изменений.
...
> [!NOTE]
> Из-за сложности динамической маршрутизации, используемой на виртуальном шлюзе Azure, существуют некие ограничения по применению определяемых пользователем маршрутов с ExpressRoute. Вот они:
> 
