...
Дополнительные сведения можно найти по адресу [http://azure.microsoft.com/support/trust-center/compliance/](https://azure.microsoft.com/support/trust-center/compliance/)
...
>[AZURE.NOTE] Из-за сложности динамической маршрутизации, используемой на виртуальном шлюзе Azure, существуют определенные ограничения по применению определяемых пользователем маршрутов и ExpressRoute. Они перечислены ниже.
>
> 1. Не следует применять определяемые пользователем маршруты для подсети шлюза, к которой подключен виртуальный шлюз Azure, связанный с ExpressRoute.
> 2. Кроме того, виртуальный шлюз Azure, связанный с ExpressRoute, не может быть указан в качестве устройства следующего прыжка в определяемых пользователем маршрутах для других подсетей.
>
>Возможность полной интеграции определяемых пользователем маршрутов и ExpressRoute будет доступна в будущих выпусках Azure. Примеры использования сети периметра с ExpressRoute или соединениями типа "сеть-сеть" описаны ниже (примеры 3 и 4).
...
> 2. Кроме того, виртуальный шлюз Azure, связанный с ExpressRoute, не может быть указан в качестве устройства следующего прыжка в определяемых пользователем маршрутах для других подсетей.
>
>В следующем выпуске Azure будет включена возможность полностью интегрировать определяемые пользователем маршруты с ExpressRoute.
