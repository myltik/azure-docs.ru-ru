---
title: "Облачные службы Microsoft Cloud и сетевая безопасность | Документация Майкрософт"
...
![Блок-схема вариантов системы безопасности][0]

[Пример 1. Создание сети периметра (также называемой DMZ, демилитаризованной зоной и промежуточной подсетью) для защиты приложений с помощью групп безопасности сети (NSG).](#example-1-build-a-perimeter-network-to-help-protect-applications-with-nsgs)</br>
...
[Пример 3. Создание сети периметра для защиты сетей с помощью брандмауэра, определяемого пользователем маршрута (UDR) и группы безопасности сети (NSG).](#example-3-build-a-dmz-to-protect-networks-with-a-firewall-and-udr-and-nsg)</br>
[Пример 4. Добавление гибридного подключения типа "сеть — сеть" с виртуальным устройством и виртуальной частной сетью (VPN).](#example-4-add-a-hybrid-connection-with-a-site-to-site-virtual-appliance-virtual-private-network)</br>
[Пример 5. Добавление гибридного подключения типа "сеть — сеть" с VPN-шлюзом Azure.](#example-5-add-a-hybrid-connection-with-a-site-to-site-azure-gateway-vpn)</br>
...
Дополнительные сведения см. в разделе о соответствии нормативным требованиям в [центре управления безопасностью Azure](https://azure.microsoft.com/support/trust-center/compliance/).
...
* **Распределенное подключение.** Клиенты могут установить подключение между виртуальной сетью и несколькими локальными сайтами или другими виртуальными сетями в Azure. Для этого они могут воспользоваться VPN-шлюзами Azure или виртуальными сетевыми устройствами сторонних производителей. Azure поддерживает VPN типа "сеть — сеть" со стандартными протоколами IPsec/IKE, а также службу частного соединения ExpressRoute.
...
Это относительно простой и эффективный способ изоляции внутренней подсети от входящего трафика. Дополнительные сведения см. в [подробных инструкциях по сборке][Example1]. Эти инструкции включают:
...
#### <a name="conclusion"></a>Заключение
Это относительно простой способ защиты приложения с помощью брандмауэра и изоляции внутренней подсети от входящего трафика. Дополнительные сведения см. в [подробных инструкциях по сборке][Example2]. Эти инструкции включают:
...
### <a name="example-4-add-a-hybrid-connection-with-a-site-to-site-virtual-appliance-virtual-private-network"></a>Пример 4. Добавление гибридного подключения типа "сеть — сеть" с виртуальным устройством и виртуальной частной сетью (VPN)
...
### <a name="example-5-add-a-hybrid-connection-with-a-site-to-site-azure-gateway-vpn"></a>Пример 5. Добавление гибридного подключения типа "сеть — сеть" с VPN-шлюзом Azure
[Вернуться к выбору решения](#fast-start) | Подробные инструкции по сборке ожидаются в ближайшее время
...
* Доступ к Azure с помощью PowerShell: [https://azure.microsoft.com/documentation/articles/powershell-install-configure/](/powershell/azureps-cmdlets-docs)
...
[1]: ./media/best-practices-network-security/compliancebadges.png "Индикаторы соответствия требованиям Azure"
