---
title: "Управление виртуальными машинами с помощью Azure Explorer для IntelliJ | Документы Майкрософт"
...
# <a name="managing-virtual-machines-using-the-azure-explorer-for-intellij"></a>Управление виртуальными машинами с помощью Azure Explorer для IntelliJ
...
## <a name="creating-a-virtual-machine-in-intellij"></a>Создание виртуальной машины в IntelliJ

Следующие инструкции помогут вам поэтапно создать виртуальную машину с использованием Azure Explorer.
...
1. В окне средства **Azure Explorer** разверните узел **Azure**, щелкните правой кнопкой мыши элемент **Виртуальные машины** и выберите **Создать виртуальную машину**.
...
1. При отображении мастера **Создание виртуальной машины** выберите свою подписку и нажмите кнопку **Далее**.
   ![Мастер по созданию виртуальной машины][CR02]

1. На следующем экране мастера укажите приведенные ниже параметры и нажмите кнопку **Далее**: ![Мастер по созданию виртуальной машины][CR03]

   а. **Расположение**: указывает расположение для создания виртуальной машины, например "westus".

   b. **Recommended Image** (Рекомендуемый образ): указывает, что вы выберете образ из сокращенного списка часто используемых образов.

   c. **Пользовательский образ**: указывает, что вы выберете пользовательский образ, для которого потребуется задать следующие параметры:

      * **Издатель**: указывает издателя, создавшего образ, который вы используете для создания виртуальной машины, например "Microsoft".

      * **Предложение**: определяет, какое предложение по виртуальной машине выбранного издателя нужно использовать, например "JDK".

      * **SKU**: указывает нужный *номер SKU* из выбранного предложения, например "JDK_8".

      * **Версия #**: указывает, какую версию нужно использовать из выбранного номера SKU.

1. На следующем экране мастера укажите приведенные ниже параметры и нажмите кнопку **Далее**: ![Мастер по созданию виртуальной машины][CR04]

   а. **Имя виртуальной машины**: указывает имя для новой виртуальной машины, которое должно начинаться с буквы и содержать только буквы, цифры и дефисы.

   b. **Размер**: указывает количество ядер и памяти, выделенных для виртуальной машины.

   c. **Имя пользователя**: указывает учетную запись администратора, создаваемую для управления виртуальной машиной.

   d. **Пароль** и **Подтверждение**: указывают пароль для учетной записи администратора.

1. На последнем экране мастера укажите приведенные ниже параметры и нажмите кнопку **Готово**: ![Мастер по созданию виртуальной машины][CR07]
   ...
   c. **Виртуальная сеть** и **Подсеть**: указывают виртуальную сеть и подсеть, к которым будет подключаться виртуальная машина. Можно выбрать существующую сеть и подсеть или создать новые. При выборе элемента **&lt;&lt;Создать&gt;&gt;** отображается следующее диалоговое окно:

      ![Диалоговое окно "Создание виртуальной сети"][CR06]

   г) **Общедоступный IP-адрес**: указывает внешний IP-адрес для виртуальной машины. Можно создать IP-адрес или выбрать значение **(Нет)**, если у виртуальной машины не будет общедоступного IP-адреса.
   ...
   f. **Группа доступности**: указывает необязательную группу доступности, к которой может принадлежать виртуальная машина. Можно выбрать существующую группу доступности, создать новую или задать значение **(Нет)**, если виртуальная машина не будет принадлежать к группе доступности.

1. После выполнения описанных выше действий новая виртуальная машина отображается в окне средства Azure Explorer.
   ![Новая виртуальная машина][CR08]

## <a name="restarting-a-virtual-machine-in-intellij"></a>Перезапуск виртуальной машины в IntelliJ

Чтобы перезапустить виртуальную машину с помощью Azure Explorer в IntelliJ, выполните следующие действия:

1. В окне средства **Azure Explorer** щелкните правой кнопкой мыши виртуальную машину и выберите пункт **Перезапустить**.
   ![Перезапуск виртуальной машины][RE01]

1. При появлении запроса выберите **Да** для перезапуска виртуальной машины.
   ![Перезапуск виртуальной машины][RE02]

## <a name="shutting-down-a-virtual-machine-in-intellij"></a>Завершение работы виртуальной машины в IntelliJ

Чтобы завершить работу виртуальной машины с помощью Azure Explorer в IntelliJ, выполните следующие действия:

1. В окне средства **Azure Explorer** щелкните правой кнопкой мыши виртуальную машину и выберите пункт **Завершить работу**.
   ![Завершение работы виртуальной машины][SH01]

1. При появлении запроса выберите **Да** для завершения работы виртуальной машины.
   ![Завершение работы виртуальной машины][SH02]

## <a name="deleting-a-virtual-machine-in-intellij"></a>Удаление виртуальной машины в IntelliJ

Чтобы удалить виртуальную машину с помощью Azure Explorer в IntelliJ, выполните следующие действия:

1. В окне средства **Azure Explorer** щелкните правой кнопкой мыши виртуальную машину и выберите пункт **Удалить**.
   ![Удаление виртуальной машины][DE01]

1. При появлении запроса выберите **Да** для удаления виртуальной машины.
   ![Удаление виртуальной машины][DE02]

## <a name="see-also"></a>См. также
Дополнительные сведения о размерах виртуальных машин Azure и ценах на них см. по следующим ссылкам:
