<properties linkid="develop-media-services-how-to-guides-deliver-streaming-content" urlDisplayName="Доставка потокового контента из служб мультимедиа" pageTitle="Доставка потокового контента из служб мультимедиа Azure" metaKeywords="" description="Узнайте, как предоставлять потоковый контент из служб мультимедиа с помощью прямого URL-адреса. Примеры кода написаны на C#, в них используется пакет SDK служб носителей для .NET." metaCanonical="" disqusComments="1" umbracoNaviHide="0" writer="migree" title="Доставка потокового контента" />


#Доставка потокового контента

Данная статья является частью серии вводных статей о программировании служб мультимедиа Azure. Предыдущий раздел: [Доставка актива путем загрузки](http://go.microsoft.com/fwlink/?LinkID=301734&clcid=0x409)
...
На данный момент мы рассмотрели доставку мультимедиа путем загрузки из хранилища Azure и с использованием Smooth Streaming. В следующем разделе, [Доставка контента HLS](http://go.microsoft.com/fwlink/?LinkId=301817), описывается доставка потокового контента с помощью службы Apple HTTP Live Streaming (HLS).
