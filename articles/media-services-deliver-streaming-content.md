<properties pageTitle="Доставка содержимого потоковой передачи из служб мультимедиа - Azure" description="Узнайте, как передавать потоковый контент из служб мультимедиа, используя прямой URL-адрес. Примеры кода написаны на языке C# и используют пакет SDK служб мультимедиа для .NET." authors="juliako" manager="dwrede" editor="" services="media-services" documentationCenter=""/>
...
Эта статья является частью серии вводных статей о программировании служб мультимедиа в Azure. Предыдущий раздел [Практическое руководство: Доставка актива путем загрузки](../ media services доставки активов загрузки /).
...
На данный момент мы рассмотрели доставку мультимедиа путем загрузки из хранилища Azure и с использованием Smooth Streaming. В следующем разделе, [Доставка контента HLS](../media-services-deliver-http-live-streaming-content/), описывается доставка потокового контента с помощью службы Apple HTTP Live Streaming (HLS).
