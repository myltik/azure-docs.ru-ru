<properties urlDisplayName="Deliver Streaming Content from Media Services" pageTitle="Доставка содержимого потоковой передачи из служб мультимедиа - Azure" metaKeywords="" description="Узнайте, как передавать потоковый контент из служб мультимедиа, используя прямой URL-адрес. Примеры кода написаны на языке C# и используют пакет SDK служб мультимедиа для .NET." metaCanonical="" disqusComments="1" umbracoNaviHide="0" title="How to: Deliver streaming content" authors="juliako" manager="dwrede" />
