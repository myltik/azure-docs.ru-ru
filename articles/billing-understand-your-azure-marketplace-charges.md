...
2. Выберите метод оплаты, к которому нужно перейти. Параметр **Pay with** (Оплата с помощью) позволяет выбрать кредитную карту. Параметр **Add a new way to pay** (Добавить новый метод оплаты) позволяет добавить новую кредитную карту.

    ![Изменить метод оплаты](./media/billing-understand-your-azure-marketplace-charges/change-payment-method.jpg)

## Управление заказами
На странице сводного обзора содержится ссылка **Manage Order** (Управление заказами). Если щелкнуть ее, откроется портал, на котором отображаются все заказы Marketplace. Кроме того, здесь можно просмотреть состояние служб и подать запрос в службу поддержки.

![Microsoft Azure](./media/billing-understand-your-azure-marketplace-charges/portal.jpg)
