...
Но решение, которое мы обсудим здесь, еще лучше. В нем используется Azure Service Bus - модель "раздел-подписка", которая позволяет снизить сложность, обеспечив при этом масштабируемое решение. 
...
1. **EnterprisePushBackendSystem**
	...
	c. `CreateTopic` используется для создания раздела Service Bus, в который будут отправляться сообщения. 
	...
	d. `SendMessage` используется для отправки сообщений в этот раздел Service Bus. В данном примере мы будем периодически отправлять набор случайных сообщений в этот раздел. Как правило эту функцию будет выполнять серверная система, которая будет отправлять сообщения при наступлении события. 
...
2. **ReceiveAndSendNotification**
	...
	e. Чтобы опубликовать этот процесс в виде задания **WebJob**, щелкните правой кнопкой мыши решение в Visual Studio и выберите **Опубликовать как WebJob**

	![][2]

	f. Выберите профиль публикации и создайте новый веб-сайт Azure для размещения задания WebJob (если он еще не существует), а когда сайт будет создан, щелкните **Опубликовать**. 
...
3. **EnterprisePushMobileApp**
	...
	a. Это приложение Магазина Windows, которое будет получать всплывающие уведомления из задания WebJob, выполняющегося в составе серверной части мобильной службы, и отображать это уведомление. Следующий пример основан на материале статьи [Центры уведомлений - универсальный учебник для Windows].  
...
3. Запустите консольное приложение **EnterprisePushBackendSystem**, которое будет имитировать серверную бизнес-систему и начнет отправлять сообщения. Вы должны увидеть примерно такие всплывающие уведомления: 
