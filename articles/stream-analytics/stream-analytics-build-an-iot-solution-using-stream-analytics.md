...
На портале Azure внизу в области управления слева щелкните **Больше служб**. В текстовом поле введите **Концентраторы событий** и щелкните **Концентраторы событий**. Откроется новое окно браузера с областью **Служебная шина** на **классическом портале**. Здесь вы увидите концентратор событий, созданный с помощью скрипта Setup.ps1.

![Служебная шина](media/stream-analytics-build-an-iot-solution-using-stream-analytics/image8.png)

Щелкните концентратор событий, имя которого начинается с *tolldata*. Перейдите на вкладку **Концентраторы событий** . В этом пространстве имен вы увидите два созданных концентратора событий с именами *entry* и *exit*.

![Вкладка "Концентраторы событий" на классическом портале](media/stream-analytics-build-an-iot-solution-using-stream-analytics/image9.png)

### <a name="azure-storage-container"></a>Контейнер хранилища Azure
1. Вернитесь на вкладку, где открыт портал Azure. Чтобы увидеть контейнер хранилища Azure, используемый в руководстве, щелкните **Хранилище** в левой части портала Azure.
   
    ![Пункт меню хранилища](media/stream-analytics-build-an-iot-solution-using-stream-analytics/image11.png)
2. Щелкните концентратор событий, имя которого начинается с *tolldata*. Откройте вкладку **Контейнеры** , чтобы увидеть созданный контейнер.
   
    ![Вкладка "Контейнеры" на портале Azure](media/stream-analytics-build-an-iot-solution-using-stream-analytics/image10.png)
3. Щелкните контейнер **tolldata** , чтобы увидеть отправленный JSON-файл с данными регистрации транспортных средств.
   
    ![Снимок экрана файла registration.json в контейнере](media/stream-analytics-build-an-iot-solution-using-stream-analytics/image12.png)
