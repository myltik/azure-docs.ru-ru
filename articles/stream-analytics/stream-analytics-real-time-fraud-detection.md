...
2. В колонке **Создать пространство имен** укажите имя пространства имен, например `<yourname>-eh-ns-demo`. Для пространства имен можно использовать любое уникальное в Azure имя с допустимым URL-адресом. 
...
Прежде чем запускать приложение TelcoGenerator, необходимо его настроить, чтобы оно могло отправлять записи вызовов в только что созданный концентратор событий.
...
3.  Затем в элементе `<appSettings>` сделайте следующее:
...
1. На панели мониторинга или в колонке **Все ресурсы** найдите и выберите задание Stream Analytics `sa_frauddetection_job_demo`. 
2. В разделе **Топология задания** колонки задания Stream Analytics щелкните поле **Входные данные**.

    ![Поле входных данных в разделе топологии задания в колонке задания Streaming Analytics](./media/stream-analytics-real-time-fraud-detection/stream-analytics-sa-job-input-box-new-portal.png)
 
3. Щелкните **+&nbsp;Добавить**, а затем заполните колонку следующими значениями:
...
Очень простой запрос должен только считывать все входящие данные. Тем не менее часто создаются запросы, которые выполняют поиск конкретных данных или связей в данных. В этом разделе руководства вы создадите и проверите несколько запросов, чтобы ознакомиться с несколькими способами преобразования входного потока для анализа. 
...
2. Вернитесь в колонку задания Streaming Analytics на портале. (Если вы закрыли колонку, найдите `sa_frauddetection_job_demo` в колонке **Все ресурсы**.)
...
4. В колонке **Запрос** щелкните точки рядом с входными данными `CallStream`, а затем выберите **Образец данных со входа**.
...
3. Щелкните **Сохранить**. После этого запрос на самосоединение сохранится как часть задания Streaming Analytics. (Образец данных не сохраняется.)
...
1. Вернитесь в колонку задания Streaming Analytics на портале Azure. (Если вы закрыли колонку, найдите `sa_frauddetection_job_demo` в колонке **Все ресурсы**.)
2. В разделе **Топологии задания** щелкните поле **Выходные данные**. 
3. В колонке **Выходные данные** щелкните **+&nbsp;Добавить**, а затем заполните колонку следующими значениями:
...
3. В колонке **Запуск задания** для параметра "Время запуска создания выходных данных задания" выберите **Сейчас**. 

4. Нажмите **Запуск**. 

    ![Колонка "Запуск задания" для задания Stream Analytics](./media/stream-analytics-real-time-fraud-detection/stream-analytics-sa-job-start-job-blade.png)
...
1. Завершите задание Streaming Analytics. В колонке **Задания** в верхней области щелкните **Остановить**.
...
Вы можете ознакомиться со следующими статьями, связанными с этим руководством:
...
* [Как сохранять данные из Azure Stream Analytics в кэш Redis для Azure с помощью функций Azure](stream-analytics-functions-redis.md). В этой статье описывается, как использовать функции Azure для записи мошеннических вызовов в кэш Redis Azure через очередь служебной шины Azure.
