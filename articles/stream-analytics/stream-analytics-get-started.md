...
У поставщика телекоммуникационных услуг есть большой объем данных для входящих вызовов. Потребности компании относительно данных: * сокращение объема данных для упрощения управления ими, а также анализ информации по использованию клиентом услуг по времени и географическим регионам; * выявление мошенничества в отношении SIM-карт (когда один пользователь одновременно выполняет несколько вызовов из разных географических расположений) в режиме реального времени, чтобы с легкостью реагировать на угрозы путем уведомления клиентов или завершения работы службы.
...
2.	Укажите следующие значения и щелкните **Создать задание Stream Analytics**:
	...
	* **Учетная запись хранения**: выберите учетную запись хранения Azure, которую вы хотите использовать для отслеживания данных всех заданий Stream Analytics, запущенных в этом регионе. Можно создать учетную запись хранения или выбрать уже существующую.
...
4.	На третьей странице введите или выберите следующие значения:
	...
	* **Группа пользователей концентратора событий**: введите группу потребителей, созданную ранее в этом руководстве.
...
3.	На третьей странице введите или выберите следующие значения:
	...
	* **ПОДПИСКА**: если хранилище BLOB-объектов создано в той же подписке, что и задание Stream Analytics, выберите **Использовать учетную запись хранения из текущей подписки**. Если хранилище связано с другой подпиской, выберите **Использовать учетную запись хранения из другой подписки** и вручную заполните поля **УЧЕТНАЯ ЗАПИСЬ ХРАНЕНИЯ**, **КЛЮЧ УЧЕТНОЙ ЗАПИСИ ХРАНЕНИЯ** и **КОНТЕЙНЕР**.
...
2.	В открывшемся диалоговом окне выберите **ВРЕМЯ НАЧАЛА ЗАДАНИЯ**, а затем нажмите кнопку с галочкой в нижней части диалогового окна. Состояние задания изменится на **Запуск**, а затем — на **Выполняется**.
