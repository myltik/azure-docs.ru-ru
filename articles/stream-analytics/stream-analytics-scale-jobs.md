...
Этот запрос содержит ключ группирования, а значит, этот ключ должен обрабатываться тем же экземпляром запроса. Мы используем ту же стратегию, что и для предыдущего запроса. Запрос состоит из нескольких этапов. Указан ли на каждом этапе параметр **Partition By** со значением ** PartitionId**? Да, все в порядке. Для выходных данных необходимо задать для параметра **PartitionKey** значение **PartitionId**, как описано выше. Выходные данные содержат столько же разделов, сколько и входные. Данная топология является топологией с усложненным параллелизмом.
...
- Источник входных данных должен быть секционирован. Дополнительные сведения см. в статье [Руководство по программированию концентраторов событий](../event-hubs/event-hubs-programming-guide.md).
