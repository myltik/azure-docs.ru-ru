...
**Объяснение**: этот запрос будет генерировать события каждые 5 секунд и выводить последнее событие, которое было получено ранее. Длительность ["прыгающего" окна](https://msdn.microsoft.com/library/dn835041.aspx ""Прыгающее" окно — Azure Stream Analytics") определяет, насколько далеко запрос пойдет назад в поисках последнего события (300 секунд в этом примере).
