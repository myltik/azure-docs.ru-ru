...
Azure Stream Analytics (ASA) в IoT Edge помогает разработчикам развертывать решения аналитики ближе к устройствам Интернета вещей практически в реальном времени, что позволяет максимально эффективно использовать данные, полученные от устройств. Предприятия, предназначенные для клиентов, которым требуется минимальная задержка, устойчивость, эффективное использование пропускной способности и соответствие требованиям, теперь могут развертывать логику управления на уровне, близком к уровню промышленных операций, и дополнять аналитику больших данных, выполняемую в облаке.  
Azure Stream Analytics в Azure IoT Edge работает на платформе [Azure IoT Edge](https://azure.microsoft.com/campaigns/iot-edge/), а развертывание заданий ASA и управление ими может выполняться с помощью Центра Интернета вещей после создания задания в ASA.
...
![Схема высокого уровня](media/stream-analytics-edge/ASAedge_highlevel.png) Вот несколько типичных сценариев, в которых применяется выполнение ASA в Edge:
* **Инструменты контроля и управления с низкой задержкой.** Например, производственные системы безопасности должны реагировать на операционные данные со сверхнизкой задержкой. Используя ASA в IoT Edge, можно проанализировать данные датчиков практически в режиме реального времени и выполнять команды при обнаружении аномалий, чтобы остановить работу компьютера или активировать предупреждения.
...
* **Соответствие.** Для обеспечения соответствия нормативным требованиям может потребоваться локально анонимизировать или статистически обработать некоторые данные перед их отправкой в облако. С ASA вы можете выполнять приведенные ниже задания. 
...
Вы можете выполнить инструкции из [этого пошагового руководства](https://docs.microsoft.com/azure/iot-edge/tutorial-deploy-stream-analytics), чтобы развернуть свое первое задание ASA в IoT Edge.
