...
![stream analytics машинное обучение руководство рисунок 1](./media/stream-analytics-machine-learning-integration-tutorial/stream-analytics-machine-learning-integration-tutorial-figure-1.png)

Рис. 2.

![stream analytics машинное обучение руководство рисунок 2](./media/stream-analytics-machine-learning-integration-tutorial/stream-analytics-machine-learning-integration-tutorial-figure-2.png)
...
2.	CSV-файл с данными. Файл, приведенный на рисунке 2, можно скачать здесь. Либо вы можете создать свой. В этом руководстве предполагается, что будет использоваться скачанный файл.

В общих чертах, будут выполнены следующие действия.

1.	Отправка входного CSV-файла в хранилище BLOB-объектов.
2.	Добавление модели Sentiment Analytics из коллекции Cortana Analytics в рабочую область машинного обучения.
...
Для этого шага можно использовать любой CSV-файл, в том числе указанный в начале этой статьи. Отправить файл можно с помощью [обозревателя хранилища Azure](http://storageexplorer.com/) или Visual Studio, а также пользовательского кода. В этом руководстве приведены примеры для Visual Studio.

1.	Разверните Azure и щелкните правой кнопкой мыши **Хранилище**. Выберите **Подключить внешнее хранилище** и укажите **имя учетной записи** и **ключ учетной записи**.  
...
1.	Скачайте [модель прогнозной аналитики](https://gallery.cortanaanalytics.com/Experiment/Predictive-Mini-Twitter-sentiment-analysis-Experiment-1) из коллекции Cortana Analytics.  
...
6.	Перейдите в **Выходные данные** и нажмите кнопку **Добавить выходные данные**.  
...
7.	Выберите **Хранилище BLOB-объектов** и введите те же параметры, за исключением контейнера. **Входные данные** настроены для чтения из контейнера с именем test, в который отправлен **CSV**-файл. Для **Выходных данных** укажите testoutput. Имена контейнеров не должны повторяться. Кроме этого, убедитесь в наличии контейнеров.
...
11.	Перейдите на вкладку **Запрос** и измените запрос, как показано ниже.
    ...
    ![stream analytics машинное обучение руководство запрос машинного обучения](./media/stream-analytics-machine-learning-integration-tutorial/stream-analytics-machine-learning-integration-tutorial-ml-query.png)
...
С помощью этого руководства мы создали задание Stream Analytics, которое читает потоковые текстовые данные и применяет к ним анализ мнений в реальном времени. При этом нам не требовалось вникать в особенности построения модели Sentiment Analytics. Это одно из преимуществ набора Cortana Analytics.
