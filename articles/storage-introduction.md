<properties urlDisplayName="Introduction to Azure Storage" pageTitle="Вводная информация о хранилище - Microsoft Azure" metaKeywords="Get started  Azure storage introduction  Azure storage overview  Azure blob   Azure unstructured data   Azure unstructured storage   Azure blob   Azure blob storage  Azure queue   Azure asynchronous processing   Azure queue   Azure queue storage Azure table   Azure nosql   Azure large structured data store   Azure table   Azure table storage  Azure file storage  Azure file  Azure file share  Azure " description="Общая информация по хранилищам Microsoft Azure" metaCanonical="" disqusComments="1" umbracoNaviHide="1" services="storage" documentationCenter="" title="Introduction to Microsoft Azure Storage" authors="tamram" manager="adinah" />
...
Служба хранилища Azure Premium теперь доступна в предварительной версии. Служба хранилища Azure Premium предоставляет высокую производительность с низкой задержкой для дисковых операций высоконагруженных рабочих нагрузок, выполняемых на виртуальных машинах Azure. С помощью службы хранилища Azure Premium вы можете подключить к виртуальной машине множество постоянных дисков с данными и настроить их должным образом для удовлетворения ваших потребностей в производительности. За каждым диском с данными стоит SSD-диск службы хранения Azure Premium для достижения максимальной производительности для операций ввода-вывода. См. раздел [Служба хранилища Premium. Высокопроизводительная служба хранения для рабочих нагрузок виртуальных машин Azure](http://go.microsoft.com/fwlink/?LinkId=521898) для получения дополнительной информации. 
...
- Учетная запись хранения Premium в настоящее время поддерживает только диски виртуальных машин Azure. Служба хранилища Azure Premium доступна по запросу на странице [предварительной версии Azure](/ru-ru/services/preview/).

Прежде чем создавать учетную запись хранения, необходимо оформить подписку Azure, которая представляет собой план, дающий доступ к различным службам Azure. В рамках одной подписки можно создать до 100 учетных записей хранения с уникальными именами. См. раздел [Информация о ценах - стоимость хранения],(http://www.windowsazure.com/ru-ru/pricing/details/storage/) чтобы получить дополнительную информацию о стоимости хранения.

Приступая к работе с Azure вы можете использовать [бесплатную пробную версию](/ru-ru/pricing/free-trial/). Как только вы решите приобрести план, можете выбрать из множества [вариантов приобретения](/ru-ru/pricing/purchase-options/). Если вы уже являетесь [подписчиком MSDN](/ru-ru/pricing/member-offers/msdn-benefits-details/), вы можете получать бесплатно ежемесячные кредиты, которые можно использовать для служб Azure, включая службу хранилища Azure.
...
- **Хранилище файлов (предварительная версия)** предлагает совместно используемое хранилище для старых приложений, использующих стандартный протокол SMB 2.1. Виртуальные машины и облачные службы Azure могут использовать файловые данные компонентов приложений через подключенные ресурсы, а локальные приложения получают доступ к этим данным совместно с помощью службы файлов REST API. Файловое хранилище доступно по запросу со страницы [предварительной версии Azure](/ru-ru/services/preview/). 

Каждая стандартная учетная запись хранения может содержать до 500 ТБ объединенных данных больших двоичных объектов, таблиц, очередей и файлов. См. раздел [Производительность и масштабируемость ость хранилища Azure],(http://msdn.microsoft.com/library/windowsazure/dn249410.aspx) чтобы получить дополнительную информацию о возможностях хранения стандартной учетной записи хранения.
...
Когда вы будете готовы создать стандартную учетную запись хранения, см. раздел [Создание, управление и удаление учетной записи хранения](../storage-create-storage-account/) для получения дополнительной информации.

### Учетные записи хранения Premium (предварительная версия)

Учетная запись хранения Premium в настоящее время поддерживает только диски виртуальных машин Azure. Служба хранилища Azure Premium доступна по запросу на странице [предварительной версии Azure](/ru-ru/services/preview/). Для получения более подробной информации о службе хранилища Azure Premium, см. раздел [Служба хранилища Premium. Высокопроизводительная служба хранения для рабочих нагрузок виртуальных машин Azure](http://go.microsoft.com/fwlink/?LinkId=521898).
...
При использовании очень больших наборов данных, когда сетевые ограничения не позволяют скачать или загрузить данные в хранилище больших двоичных объектов, можно передать жесткий диск в корпорацию Майкрософт для импорта или экспорта данных непосредственно в центре обработки данных с помощью [службы импорта и экспорта Azure](http://azure.microsoft.com/ru-ru/documentation/articles/storage-import-export-service/). Можно также копировать данные BLOB-объекта внутри вашей учетной записи хранения или между разными учетными записями хранения. 
...
## Доступ к ресурсам больших двоичных объектов, таблиц, очередей и файлов
...
Если требуется предоставить пользователям управляемый доступ к ресурсам хранилища, можно создать [подписанный URL-адрес].(../storage-dotnet-shared-access-signature-part-1/). Подписанный URL-адрес - это маркер, который можно добавить к URL-адресу, что позволяет делегировать доступ к контейнеру, большому двоичному объекту, таблице или очереди. Любой пользователь, обладающий маркером, может получить доступ к ресурсу, на который указывает маркер, с определяемыми им разрешениями на период времени, в течение которого он действителен. Обратите внимание, что хранилище файлов Azure в настоящее время не поддерживает подписи общего доступа.
...
## Репликация для долговечности и высокого уровня доступности
...
[Информация о ценах - стоимость хранения](/ru-ru/pricing/details/storage/) странице содержится информация о ценах на объем хранилища, репликацию и транзакции. [Информация о ценах - передача данных](/ru-ru/pricing/details/data-transfers/) содержит детализированную информацию об исходящих данных. Вы можете воспользоваться [калькулятором цен службы хранилища Azure](/ru-ru/pricing/calculator/?scenario=data-management) to help estimate your costs.

## Разработка для хранилища

Хранилище Azure предоставляет ресурсы хранилища через интерфейс [REST API] (http://msdn.microsoft.com/library/windowsazure/dd179355.aspx), к которому можно обратиться с помощью любого языка, позволяющего отправлять запросы HTTP или HTTPS. Кроме того, хранилище Azure предлагает библиотеки программирования для нескольких распространенных языков. Эти библиотеки упрощают многие аспекты работы с хранилищем Azure, обрабатывая такие процессы, как синхронное и асинхронное инициирование, пакетная обработка операций, управление исключениями, автоматические повторы, рабочие характеристики и т. д. В настоящее время доступны библиотеки для следующих языков и платформ (другие находятся на стадии подготовки):

- [.NET](http://go.microsoft.com/fwlink/?LinkID=390731)
- [Машинный код](http://msdn.microsoft.com/library/dn495438.aspx)
...
- [Документация хранилища Azure](/ru-ru/documentation/services/storage/)
- [Справочник по REST API служб хранения Microsoft Azure](http://msdn.microsoft.com/ru-ru/library/dd179355.aspx)
...
- [Командлеты хранилища Azure](http://msdn.microsoft.com/ru-ru/library/azure/dn806401.aspx)
...
- [Как использовать хранилище больших двоичных объектов из .NET](../storage-dotnet-how-to-use-blobs/)
- [Как использовать табличное хранилище из .NET](../storage-dotnet-how-to-use-tables/)
...
- [Как использовать хранилище больших двоичных объектов из PHP](../storage-php-how-to-use-blobs/)
- [Как использовать табличное хранилище из PHP](../storage-php-how-to-use-table-storage/)
- [Как использовать хранилище очередей из PHP](../storage-php-how-to-use-queues/)

<h3>Для разработчиков Ruby</h3>

- [Как использовать хранилище больших двоичных объектов из Ruby](../storage-ruby-how-to-use-blob-storage/)
- [Как использовать табличное хранилище из Ruby](../storage-ruby-how-to-use-table-storage/)
- [Как использовать хранилище очередей из Ruby](../storage-ruby-how-to-use-queue-storage/)

<h3>Для разработчиков Python</h3>

- [Как использовать хранилище больших двоичных объектов из Python](../storage-python-how-to-use-blob-storage/)
