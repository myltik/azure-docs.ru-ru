...
- Они создаются с помощью одного из следующих методов:
  ...
  - с использованием [портала предварительной версии](https://portal.azure.com/);
  ...
  - с помощью выполнения команд PowerShell в режиме **AzureResourceManager**;
...
- Тип ресурса не включает **(классический)** в названии. На следующем рисунке тип отображается как **Учетная запись хранения**.
...
- Они создаются с помощью одного из следующих методов:
  ...
  - с использованием команды PowerShell, выполняемых в режиме **AzureServiceManagement** (который является режимом по умолчанию, поэтому если вы специально не переключились на AzureResourceManager, то работаете в режиме AzureServiceManagement);
  ...
  - с использованием команд интерфейса командной строки Azure, запускаемых в режиме **arm** или в режиме по умолчанию.
- Тип ресурса включает **(классический)** в названии. На следующем рисунке тип отображается как **Учетная запись хранения (классический)**.

    ![классический тип](./media/resource-manager-deployment-model/classic-type.png)

Для управления ресурсами, созданными с помощью классического развертывания, по-прежнему можно использовать портал предварительной версии.
...
До появления диспетчера ресурсов каждый ресурс, который создавался с помощью классического развертывания, не являлся членом группы ресурсов. При появлении диспетчера ресурсов все ресурсы были добавлены в группы ресурсов по умолчанию. Теперь при создании ресурса с помощью классического развертывания он будет автоматически помещен в пустую группу ресурсов, даже несмотря на то, что при развертывании эта группа ресурсов не была указана. Однако только нахождение в группе ресурсов не означает, что ресурс был преобразован в модель диспетчера ресурсов. Если ресурс был создан при помощи классического развертывания, необходимо продолжать работать с ним с помощью классических операций.
...
Список эквивалентных команд интерфейса командной строки Azure при переходе от классического развертывания к диспетчеру ресурсов см. в разделе [Эквивалентные команды диспетчера ресурсов и управления службами для операций с виртуальными машинами](./virtual-machines/xplat-cli-azure-manage-vm-asm-arm.md).
