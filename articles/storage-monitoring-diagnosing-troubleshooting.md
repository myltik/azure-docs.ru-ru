...
| Microsoft.WindowsAzure.Storage | Ошибка                  | 1                       | 85d077ab -…                       | Retry policy did not allow for a retry. Сбой, при котором удаленный сервер вернул ошибку: (403) — Запрещено.                                                                            |
...
| e2d06d78-…                  | Метод **CreateIfNotExists** для создания контейнера BLOB-объектов. Обратите внимание: эта операция включает в себя запрос **HEAD** для проверки существования контейнера. Пример: **HEAD** возвращает сообщение 404, но работа не прерывается. |
| de8b1c3c-...                | Метод **UploadFromStream** для создания BLOB-объекта. Пример: **PUT** завершается сбоем с сообщением 404.                                                                                                                                      |
