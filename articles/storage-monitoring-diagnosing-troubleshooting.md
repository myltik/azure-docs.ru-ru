...
-   [Введение][Введение]
    ...
    -   [Организационная структура руководства][Организационная структура руководства]
-   [Мониторинг службы хранения][Мониторинг службы хранения]

    -   [Мониторинг работоспособности службы][Мониторинг работоспособности службы]
    -   [Мониторинг мощностей][Мониторинг мощностей]
    -   [Мониторинг доступности][Мониторинг доступности]
    -   [Мониторинг производительности][Мониторинг производительности]
-   [Диагностика неполадок хранилища][Диагностика неполадок хранилища]

    -   [Неполадки работоспособности служб][Неполадки работоспособности служб]
    -   [Неполадки производительности][Неполадки производительности]
    -   [Ошибки диагностики][Ошибки диагностики]
    -   [Неполадки эмулятора хранилища][Неполадки эмулятора хранилища]
    -   [Средства ведения журналов хранилища][Средства ведения журналов хранилища]
    -   [Использование средств ведения сетевых журналов][Использование средств ведения сетевых журналов]
-   [Сквозная трассировка][Сквозная трассировка]

    -   [Связь данных журнала][Связь данных журнала]
    -   [Идентификатор запроса клиента][Идентификатор запроса клиента]
    -   [Идентификатор запроса сервера][Идентификатор запроса сервера]
    -   [Метки времени][Метки времени]
-   [Рекомендации по устранению неполадок][Рекомендации по устранению неполадок]

    -   [Метрики содержат высокие значения AverageE2ELatency и низкие значения AverageServerLatency][Метрики содержат высокие значения AverageE2ELatency и низкие значения AverageServerLatency]
    -   [Метрики содержат низкие значения AverageE2ELatency и низкие значения AverageServerLatency, но клиент сталкивается с большой задержкой][Метрики содержат низкие значения AverageE2ELatency и низкие значения AverageServerLatency, но клиент сталкивается с большой задержкой]
    -   [Метрики содержат высокие значения AverageServerLatency][Метрики содержат высокие значения AverageServerLatency]
    -   [При доставке сообщений в очереди возникают неожиданные задержки][При доставке сообщений в очереди возникают неожиданные задержки]
    -   [Метрики указывают на увеличение значения PercentThrottlingError][Метрики указывают на увеличение значения PercentThrottlingError]
    -   [Метрики указывают на увеличение значения PercentTimeoutError][Метрики указывают на увеличение значения PercentTimeoutError]
    -   [Метрики указывают на увеличение значения PercentNetworkError][Метрики указывают на увеличение значения PercentNetworkError]
    -   [Клиент получает сообщения HTTP 403 (запрещено)][Клиент получает сообщения HTTP 403 (запрещено)]
    -   [Клиент получает сообщения HTTP 404 (не найдено)][Клиент получает сообщения HTTP 404 (не найдено)]
    -   [Клиент получает сообщения HTTP 409 (конфликт)][Клиент получает сообщения HTTP 409 (конфликт)]
    -   [Метрики содержат низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors][Метрики содержат низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors]
    -   [Метрики емкости указывают на неожиданное увеличение использования емкости хранилища][Метрики емкости указывают на неожиданное увеличение использования емкости хранилища]
    -   [Происходит неожиданный перезапуск виртуальных машин, имеющих большое количество виртуальных жестких дисков][Происходит неожиданный перезапуск виртуальных машин, имеющих большое количество виртуальных жестких дисков]
    -   [Проблема связана с использованием эмулятора хранения на этапе разработки или тестирования][Проблема связана с использованием эмулятора хранения на этапе разработки или тестирования]
    -   [Возникают проблемы при установке пакета Microsoft Azure SDK для .NET][Возникают проблемы при установке пакета Microsoft Azure SDK для .NET]
    -   [Возникла другая проблема со службой хранилища][Возникла другая проблема со службой хранилища]
-   [Приложения][Приложения]

    -   [Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler][Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler]
    -   [Приложение 2. Отслеживание сетевого трафика с помощью Wireshark][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]
    -   [Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]
    -   [Приложение 4. Просмотр метрик и данных журналов с помощью Excel][Приложение 4. Просмотр метрик и данных журналов с помощью Excel]
    -   [Приложение 5. Мониторинг с использованием Application Insights для Visual Studio Online][Приложение 5. Мониторинг с использованием Application Insights для Visual Studio Online]
...
В разделе [Мониторинг службы хранилища][Мониторинг службы хранения] описывается, как с помощью метрик аналитики хранилища Azure (метрик хранилища) отслеживать работоспособность и производительность ваших служб хранилища Azure.

Раздел [Диагностика неполадок с хранилищем][Диагностика неполадок хранилища] посвящен диагностике проблем с использованием аналитических журналов хранилища Azure (журналов хранилища). Кроме того, в нем описывается, как включить ведение журналов на стороне клиента с использованием средств из одной или нескольких клиентских библиотек, таких как клиентская библиотека хранилища для .NET или Azure SDK для Java.

В разделе [Комплексная трассировка][Сквозная трассировка] вы найдете сведения о корреляции данных из различных журналов и метрик.

В разделе [Рекомендации по устранению неполадок][Рекомендации по устранению неполадок] представлены инструкции по устранению самых распространенных неполадок хранилища, с которыми вы можете столкнуться.

Раздел [Приложения][Приложения] включает сведения об использовании других средств, например Wireshark и Netmon для анализа обмена сетевыми пакетами, Fiddler — для анализа сообщений HTTP и HTTPS, а также Microsoft Message Analyzer — для корреляции данных журналов.

## <a name="monitoring-your-storage-service"></a>Мониторинг службы хранилища

Если вы знакомы с системным монитором Windows, то можете считать, что метрики хранилища — эквивалент счетчиков этой службы для хранилища Azure. Они предоставляют полный набор метрик (счетчиков в терминологии системного монитора Windows), позволяющих оценить, например, доступность службы, общее количество запросов к службе, а также процент успешно обработанных запросов (полный список доступных метрик см. в статье [Схема таблицы метрик аналитики хранилища][Схема таблицы метрик аналитики хранилища] на MSDN). Вы можете настроить службу хранилища так, чтобы она собирала и агрегировала метрики каждый час или каждую минуту. Дополнительные сведения о том, как включить метрики и отслеживать учетные записи хранилища, см. в статье [Включение метрик хранилища][Включение метрик хранилища] на MSDN.

Вы можете выбрать, какие ежечасные метрики должны отображаться на портале Azure, и настроить правила для уведомления администраторов по электронной почте обо всех случаях, когда эти метрики превышают пороговые уровни (дополнительные сведения см. в разделе [Практическое руководство. Получение уведомлений об оповещениях и управление правилами оповещения в Azure][Практическое руководство. Получение уведомлений об оповещениях и управление правилами оповещения в Azure]). Метрики позволяют получить довольно полную картину о состоянии службы хранилища, но иногда регистрируют не все операции.

На рисунке 2 показана страница "Монитор" на портале, где вы можете просматривать такие метрики, как доступность, общее количество запросов и средние показатели задержки для учетной записи хранилища. Кроме того, на этой странице настроено правило для оповещения администратора о спаде доступности ниже определенного уровня. Помимо этих данных, на возможные проблемы также может указывать то, что уровень успешной обработки службы таблиц составляет менее 100 % (дополнительные сведения см. в разделе [Метрики содержат низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors][Метрики содержат низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors]).
...
-   Изучать потенциальные проблемы с помощью диагностических инструментов, описанных в разделе [Диагностика неполадок с хранилищем][Диагностика неполадок хранилища].
...
Чтобы узнать, как оценить размер различных объектов для хранения данных, например BLOB, прочтите публикацию в блоге [Как выставляются счета за хранилище Microsoft Azure: пропускная способность, транзакции и емкость][Как выставляются счета за хранилище Microsoft Azure: пропускная способность, транзакции и емкость].
...
В разделе [Рекомендации по устранению неполадок][Рекомендации по устранению неполадок] этого руководства описаны некоторые распространенные проблемы со службой хранилища, связанные с доступностью.
...
В разделе [Рекомендации по устранению неполадок][Рекомендации по устранению неполадок] этого руководства описаны некоторые распространенные проблемы со службой хранилища, связанные с производительностью.
...
-   существенные отклонения от базовых значений метрик, мониторинг которых выполняется в соответствии с описанием в предыдущем разделе [Мониторинг службы хранилища][Мониторинг службы хранения];
...
В разделе [Рекомендации по устранению неполадок][Рекомендации по устранению неполадок] ниже в этом руководстве содержатся дополнительные сведения о некоторых распространенных неполадках.
...
-   [Общие коды ошибок API-интерфейса REST][Общие коды ошибок API-интерфейса REST]
-   [Коды ошибок службы BLOB-объектов][Коды ошибок службы BLOB-объектов]
-   [Коды ошибок службы очередей][Коды ошибок службы очередей]
-   [Коды ошибок для службы таблиц][Коды ошибок для службы таблиц]
...
-   Fiddler (<http://www.telerik.com/fiddler>) — бесплатный веб-прокси для отладки, с помощью которого можно проверять заголовки и полезные данные HTTP- и HTTPS-запросов, а также ответных сообщений. Дополнительные сведения см. в разделе [Appendix 1: Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler][Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler]
-   Microsoft Network Monitor (Netmon) (<http://www.microsoft.com/ru-ru/download/details.aspx?id=4865>) и Wireshark (<http://www.wireshark.org/>) — бесплатные средства для анализа сетевых протоколов, с помощью которых можно просматривать подробную информацию о пакетах для широкого спектра протоколов. Дополнительные сведения о Wireshark см. в разделе "[Приложение 2: Отслеживание сетевого трафика с помощью Wireshark][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]".
-   Microsoft Message Analyzer — это средство, предлагаемое корпорацией Майкрософт в качестве замены Netmon. Оно позволяет не только собирать данные о сетевых пакетах, но и просматривать, а также анализировать информацию из журналов, созданных с помощью других инструментов. Дополнительные сведения см. в разделе [Appendix 3: Отслеживание сетевого трафика с помощью Microsoft Message Analyzer][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]".
-   Если вы хотите выполнить базовый тест подключения, чтобы проверить, может ли клиентская машина подключиться к службе хранилища Azure по сети, этого не удастся сделать с помощью стандартного инструмента **ping** на клиенте. Однако вы можете воспользоваться для проверки подключения инструментом **tcping**. **Tcping** можно загрузить на странице <http://www.elifulkerson.com/projects/tcping.php>.
...
Вы должны просмотреть журналы на стороне клиента, чтобы понять, сколько запросов отправляет приложение, а также проверить общие узкие места, связанные с производительностью .NET, в приложении. К ним относятся использование процессора, сети и памяти, а также сборка мусора .NET. В качестве отправной точки для диагностики и устранения неполадок в клиентских приложениях .NET используйте статью [Отладка, трассировка и профилирование][Отладка, трассировка и профилирование] с сайта MSDN.
...
Дополнительные сведения об устранении неполадок в сети с помощью Wireshark см. в разделе "[Приложение 2: отслеживание сетевого трафика с помощью Wireshark][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]".

Дополнительные сведения об устранении неполадок в сети с помощью Microsoft Message Analyzer см. в разделе "[Приложение 3: отслеживание сетевого трафика с помощью Microsoft Message Analyzer][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]".
...
-   [Временное увеличение метрики PercentThrottlingError][Временное увеличение метрики PercentThrottlingError]
-   [Постоянное увеличение метрики PercentThrottlingError][Постоянное увеличение метрики PercentThrottlingError]
...
Если в значении метрики **PercentThrottlingError** наблюдаются пиковые скачки, совпадающие с периодами высокой активности приложения, следует реализовать стратегию повторения с экспоненциальной, а не линейной задержкой в клиенте. Это позволит снизить мгновенную нагрузку на раздел и сгладить пиковые скачки трафика. Подробнее о том, как реализовать политики повтора с помощью клиентской библиотеки хранилища, см. в статье [Пространство имен Microsoft.WindowsAzure.Storage.RetryPolicies][Пространство имен Microsoft.WindowsAzure.Storage.RetryPolicies] в библиотеке MSDN.
...
Наиболее распространенной причиной этой ошибки является отключение клиента, прежде чем в службе хранилища истекает время ожидания. Изучите код клиента, чтобы понять, когда и почему клиент отключается от службы хранилища. Вы также можете использовать такие средства, как Wireshark, анализатор сообщений Майкрософт или Tcping, для анализа проблем с сетевым подключением клиента. Эти средства описываются в [приложениях][Приложения].
...
| Microsoft.WindowsAzure.Storage | Информация              | 3                       | 85d077ab -…                       | Начинается выполнение синхронного запроса к https://domemaildist.blob.core.windows.netazure                                                                                             
...
| Microsoft.WindowsAzure.Storage | Предупреждение          | 2                       | 85d077ab -…                       | При ожидании ответа вызвано исключение: удаленный сервер вернул ошибку (403) "Запрещено"...                                                                                             |
...
| Microsoft.WindowsAzure.Storage | Предупреждение          | 2                       | 85d077ab -…                       | Во время операции вызвано исключение: удаленный сервер вернул ошибку (403) "Запрещено"...                                                                                               |
...
| Microsoft.WindowsAzure.Storage | Ошибка                  | 1                       | 85d077ab -…                       | Политика повтора не позволила выполнить повтор. Сбой, при котором удаленный сервер вернул ошибку: (403) — Запрещено.                                                                    |
...
-   Проверьте, соответствует ли параметр версии в ключе SAS (например, **sv=2012-02-12**) версии клиентской библиотеки хранилища, которую вы используете. Всегда используйте последнюю версию клиентской библиотеки хранилища. Дополнительные сведения об управлении версиями и зависимостях для маркера SAS см. в статье [][4].
...
Если вы генерируете маркеры SAS с помощью клиентской библиотеки хранилища, то можете легко создать действительный маркер. Однако если вы используете REST API хранилища и создаете маркеры SAS вручную, рекомендуем внимательно изучить статью [Делегирование доступа с помощью подписанного URL-адреса][Делегирование доступа с помощью подписанного URL-адреса] на сайте MSDN.
...
-   [Объект удален ранее клиентом или другим процессом][Объект удален ранее клиентом или другим процессом]
-   [Проблема с авторизацией подписанного URL-адреса (SAS)][Проблема с авторизацией подписанного URL-адреса (SAS)]
-   [У кода JavaScript на стороне клиента нет разрешения на доступ к объекту][У кода JavaScript на стороне клиента нет разрешения на доступ к объекту]
-   [Сбой сети][Сбой сети]
...
| 07b26a5d-...                | Метод **DeleteIfExists** для удаления контейнера BLOB-объектов. Обратите внимание: эта операция включает в себя запрос **HEAD** для проверки существования контейнера.                                                                       |
| e2d06d78-…                  | Метод **CreateIfNotExists** для создания контейнера BLOB-объектов. Обратите внимание: эта операция включает в себя запрос **HEAD** для проверки существования контейнера. Класс **HEAD** возвращает сообщение 404, но работа не прерывается. |
| de8b1c3c-...                | Метод **UploadFromStream** для создания BLOB-объекта. Класс **PUT** завершается сбоем с сообщением 404.                                                                                                                                      |
...
| 07b26a5d-...                |  Response received. Status code = 200, Request ID = eeead849-...Content-MD5 = , ETag = "0x8D14D2DC63D059B". (Ответ получен. Код состояния = 200, ИД запроса = eeead849-...Content-MD5 = , ETag = "0x8D14D2DC63D059B".)                    |
...
| e2d06d78-...                | При ожидании ответа вызвано исключение: удаленный сервер вернул ошибку (404) — не найдено.                                                                                                                                                |
...
| e2d06d78-...                | При ожидании ответа вызвано исключение: удаленный сервер вернул ошибку (409) — Конфликт                                                                                                                                                   |
...
| de8b1c3c-...                | При ожидании ответа вызвано исключение: удаленный сервер вернул ошибку (404) — не найдено.                                                                                                                                                |
...
| de8b1c3c-...                | Во время операции вызвано исключение: удаленный сервер вернул ошибку (404) — не найдено.                                                                                                                                                  |
| de8b1c3c-...                | Retry policy did not allow for a retry. Сбой, при котором удаленный сервер вернул ошибку: (404) — не найдено.                                                                                                                             |
| e2d06d78-...                | Retry policy did not allow for a retry. Сбой, при котором удаленный сервер вернул ошибку: (409) — Конфликт                                                                                                                                |
...
| Request URL (URL-адрес запроса)                      | https://domemaildist.blob.core.windows.net/azureimblobcontainer/blobCreatedViaSAS.txt?sv=2014-02-14&sr=c&si=mypolicy&sig=XXXXX&api-version=2014-02-14& |
...
Эти ошибки появляются из-за того, что веб-браузер применяет ограничение безопасности по [принципу одинакового источника][принципу одинакового источника], в связи с чем веб-страница не может вызвать API ни из каких других доменов, кроме собственного.

Чтобы решить эту проблему JavaScript обходным путем, вы можете настроить для службы хранилища, к которой обращается клиент, общий доступ к ресурсам независимо от источника (CORS). Дополнительные сведения см. в статье [Поддержка общего доступа к ресурсам независимо от источника (CORS) для служб хранилища Azure][Поддержка общего доступа к ресурсам независимо от источника (CORS) для служб хранилища Azure] на сайте MSDN.
...
Полный список кодов ошибок API REST, возвращаемых службами хранилища. см. на странице [Общие коды ошибок API-интерфейса REST][Общие коды ошибок API-интерфейса REST].

### <a name="capacity-metrics-show-an-unexpected-increase"></a>Метрики емкости указывают на неожиданное увеличение использования емкости хранилища
...
-   [Функция "X" не работает в эмуляторе хранения][Функция "X" не работает в эмуляторе хранения]
-   [При использовании эмулятора хранения происходит ошибка "Значение одного из заголовков HTTP имеет неправильный формат"][При использовании эмулятора хранения происходит ошибка "Значение одного из заголовков HTTP имеет неправильный формат"]
-   [Для запуска эмулятора хранения требуются права администратора][Для запуска эмулятора хранения требуются права администратора]

#### <a name="feature-X-is-not-working"></a>Функция "X" не работает в эмуляторе хранения

Эмулятор хранения поддерживает не все функции служб хранилища Azure, например файловой службы. Подробнее см. в статье [Различия между эмулятором хранилища и службами хранилища Azure][Различия между эмулятором хранилища и службами хранилища Azure] в библиотеке MSDN.
...
Подробнее см. в статье [Инициализация эмулятора хранилища с помощью командной строки][Инициализация эмулятора хранилища с помощью командной строки] в библиотеке MSDN (эмулятор хранения можно инициализировать и в Visual Studio, но для этого также потребуются права администратора).
...
Дополнительные сведения о Fiddler, см. в разделе "[Приложение 1: отслеживание HTTP- и HTTPS-трафика с помощью Fiddler][Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler]."

Дополнительные сведения об использовании Wireshark, см. в разделе "[Приложение 2: отслеживание сетевого трафика с помощью Wireshark][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]".

Дополнительные сведения об использовании Microsoft Message Analyzer, см. в разделе "[Приложение 3: отслеживание сетевого трафика с помощью Microsoft Message Analyzer][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]".
...
Fiddler — это полезное средство для анализа трафика HTTP и HTTPS между клиентским приложением и используемой службой хранилища Azure. Загрузить Fiddler можно на странице <http://www.telerik.com/fiddler>.

> [WACOM.NOTE] Средство Fiddler может декодировать трафик HTTPS. Внимательно ознакомьтесь с документацией Fiddler, чтобы понять, как это осуществляется и какие последствия может иметь для безопасности.
...
-   Остановка и запуск захвата трафика. В главном меню выберите пункт **File** (Файл), а затем щелкните **Capture Traffic** (Захват трафика), чтобы включить или отключить захват.
-   Сохранение захваченных данных трафика. В главном меню последовательно выберите пункты **File**, **Save** (Сохранить) и **All Sessions** (Все сеансы). Это позволяет сохранить трафик в файле архива сеанса. Позже вы можете загрузить архив сеанса для анализа или отправить его в службу поддержки Майкрософт по запросу.
...
Wireshark — это анализатор сетевых протоколов, который позволяет просматривать подробную информацию о пакетах для широкого спектра сетевых протоколов. Загрузить Wireshark можно на веб-сайте <http://www.wireshark.org/>.
...
4.  В текстовом поле **Capture Filter** (Фильтр захвата) добавьте фильтр. Например, при использовании фильтра **host contosoemaildist.table.core.windows.net** Wireshark будет захватывать только пакеты, отправляемые в конечную точку службы таблиц в учетной записи хранения **contosoemaildist** или из нее. Полный список фильтров захвата см. на странице <http://wiki.wireshark.org/CaptureFilters>.

    ![][7]

5.  Нажмите **Start** (Запуск). Средство Wireshark будет захватывать все пакеты, отправляемые в конечную точку службы таблиц или из нее в процессе работы клиентского приложения на локальном компьютере.
6.  По завершении в главном меню выберите пункт **Capture** (Захват), а затем пункт **Stop** (Остановить).
7.  Чтобы сохранить полученные данные в файл захвата Wireshark, в главном меню выберите пункт **File** (Файл), а затем пункт **Save** (Сохранить).

Обнаруженные ошибки будут выделены в окне со **списком пакетов** WireShark. Вы также можете просмотреть сводку ошибок и предупреждений в окне **Expert Info** (Расширенная информация) (в меню **Analyze** (Анализ) выберите пункт **Expert Info**).

![][8]

Кроме того, вы можете просмотреть данные TCP так, как они видны прикладному уровню. Для этого щелкните данные TCP правой кнопкой мыши и выберите пункт **Follow TCP Stream** (Отслеживать поток TCP). Это особенно полезно в том случае, если дамп был получен без использования фильтра захвата. Подробнее см. [здесь][здесь].

![][9]

> [WACOM.NOTE] Дополнительные сведения об использовании Wireshark см. в [руководстве пользователя Wireshark][руководстве пользователя Wireshark].
...
#### Настройка сеанса трассировки веб-трафика с помощью анализатора сообщений Майкрософт

Чтобы настроить сеанс трассировки веб-трафика HTTP и HTTPS с помощью анализатора сообщений Майкрософт, запустите анализатор, а затем в меню **File** (Файл) выберите пункт **Capture/Trace** (Захват и трассировка). В списке доступных сценариев трассировки выберите пункт **Web Proxy** (Веб-прокси). Затем на панели **Trace Scenario Configuration** (Конфигурация сценария трассировки) в текстовом поле **HostnameFilter** (Фильтр имен узлов) добавьте имена конечных точек хранилища (их можно узнать на портале Azure). Например, если ваша учетная запись хранения Azure имеет имя **contosodata**, добавьте в поле **HostnameFilter** следующий текст:

    contosodata.blob.core.windows.net contosodata.table.core.windows.net contosodata.queue.core.windows.net

> [WACOM.NOTE] Имена узлов разделяются пробелом.

Когда вы будете готовы начать сбор данных трассировки, нажмите кнопку **Start With** (Начать с).

Подробнее о трассировке **Web Proxy** в анализаторе сообщений Майкрософт см. в статье [Поставщик PEF-WebProxy][Поставщик PEF-WebProxy] на сайте TechNet.

Встроенная трассировка **Web Proxy** в анализаторе сообщений Майкрософт основана на Fiddler. С ее помощью можно захватывать трафик HTTPS на стороне клиента и просматривать незашифрованные сообщения HTTPS. Для выполнения трассировки **Web Proxy** настраивается локальный прокси-сервер для всего трафика HTTP и HTTPS, который обеспечивает доступ к незашифрованным сообщениям.

#### Диагностика неполадок сети с помощью анализатора сообщений Майкрософт

Помимо использования трассировки **Web Proxy** в анализаторе сообщений Майкрософт для захвата данных трафика HTTP и HTTPS между клиентским приложением и службой хранилища, вы также можете использовать встроенную трассировку **Local Link Layer** (Локальный канальный уровень) для захвата информации о сетевых пакетах. Это позволяет получать те же данные, что и с помощью Wireshark, и диагностировать неполадки сети, такие как потеря пакетов.
...
-   На вкладке **Данные** нажмите кнопку **Из текста**.
-   Выберите файл журнала, который нужно просмотреть, и нажмите кнопку **Импорт**.
-   В первом шаге **мастера импорта текста** выберите параметр **С разделителями**.
...
На момент написания этой статьи служба Application Insights была доступна в предварительной версии. Дополнительные сведения можно найти в статье [Application Insights для Visual Studio Online][Application Insights для Visual Studio Online] в библиотеке MSDN.

<!--Anchors--> <!--Image references-->

  [Введение]: #introduction
  [Организационная структура руководства]: #how-this-guide-is-organized
  [Мониторинг службы хранения]: #monitoring-your-storage-service
  ...
  [Неполадки работоспособности служб]: #service-health-issues
  [Неполадки производительности]: #performance-issues
  [Ошибки диагностики]: #diagnosing-errors
  [Неполадки эмулятора хранилища]: #storage-emulator-issues
  [Средства ведения журналов хранилища]: #storage-logging-tools
  [Использование средств ведения сетевых журналов]: #using-network-logging-tools
  [Сквозная трассировка]: #end-to-end-tracing
  [Связь данных журнала]: #correlating-log-data
  ...
  [При доставке сообщений в очереди возникают неожиданные задержки]: #you-are-experiencing-unexpected-delays-in-message-delivery
  ...
  [Метрики указывают на увеличение значения PercentNetworkError]: #metrics-show-an-increase-in-PercentNetworkError
  ...
  [Метрики емкости указывают на неожиданное увеличение использования емкости хранилища]: #capacity-metrics-show-an-unexpected-increase
  ...
  [Возникают проблемы при установке пакета Microsoft Azure SDK для .NET]: #you-are-encountering-problems-installing-the-Windows-Azure-SDK
  ...
  [Схема таблицы метрик аналитики хранилища]: http://msdn.microsoft.com/ru-ru/library/azure/hh343264.aspx
  [Включение метрик хранилища]: http://go.microsoft.com/fwlink/?LinkId=510865
  [Практическое руководство. Получение уведомлений об оповещениях и управление правилами оповещения в Azure]: http://msdn.microsoft.com/library/azure/dn306638.aspx
  ...
  [Как выставляются счета за хранилище Microsoft Azure: пропускная способность, транзакции и емкость]: http://blogs.msdn.com/b/windowsazurestorage/archive/2010/07/09/understanding-windows-azure-storage-billing-bandwidth-transactions-and-capacity.aspx
  [0]: http://msdn.microsoft.com/ru-ru/library/azure/hh343260.aspx
  [Параметры избыточности в службе хранилища Microsoft Azure и геоизбыточное хранилище с доступом на чтение]: http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/04/introducing-read-access-geo-replicated-storage-ra-grs-for-windows-azure-storage.aspx
  [Общие коды ошибок API-интерфейса REST]: http://msdn.microsoft.com/ru-ru/library/azure/dd179357.aspx
  [Коды ошибок службы BLOB-объектов]: http://msdn.microsoft.com/ru-ru/library/azure/dd179439.aspx
  [Коды ошибок службы очередей]: http://msdn.microsoft.com/ru-ru/library/azure/dd179446.aspx
  [Коды ошибок для службы таблиц]: http://msdn.microsoft.com/ru-ru/library/azure/dd179438.aspx
  [Ведение журнала на стороне сервера]: http://go.microsoft.com/fwlink/?LinkId=510867
  [Ведение журнала на стороне клиента с помощью клиентской библиотеки хранилища]: http://go.microsoft.com/fwlink/?LinkId=510868
  [3]: ./media/storage-monitoring-diagnosing-troubleshooting/high-e2e-latency.png
  [Алгоритм Нейгла не ориентирован на мелкие запросы]: http://blogs.msdn.com/b/windowsazurestorage/archive/2010/06/25/nagle-s-algorithm-is-not-friendly-towards-small-requests.aspx
  [Отладка, трассировка и профилирование]: http://msdn.microsoft.com/ru-ru/library/7fe0dd2y(v=vs.110).aspx
  [Целевые показатели по производительности и масштабируемости для хранилища Azure]: http://msdn.microsoft.com/library/azure/dn249410.aspx
  ...
  [Пространство имен Microsoft.WindowsAzure.Storage.RetryPolicies]: http://msdn.microsoft.com/ru-ru/library/microsoft.windowsazure.storage.retrypolicies.aspx
  ...
  [Делегирование доступа с помощью подписанного URL-адреса]: http://msdn.microsoft.com/ru-ru/library/azure/ee395415.aspx
  ...
  [принципу одинакового источника]: http://www.w3.org/Security/wiki/Same_Origin_Policy
  [Поддержка общего доступа к ресурсам независимо от источника (CORS) для служб хранилища Azure]: http://msdn.microsoft.com/ru-ru/library/windowsazure/dn535601.aspx
  [5]: http://msdn.microsoft.com/ru-ru/library/azure/dn249410.aspx
  [Функция "X" не работает в эмуляторе хранения]: #feature-X-is-not-working
  [При использовании эмулятора хранения происходит ошибка "Значение одного из заголовков HTTP имеет неправильный формат"]: #error-HTTP-header-not-correct-format
  [Для запуска эмулятора хранения требуются права администратора]: #storage-emulator-requires-administrative-privileges
  [Различия между эмулятором хранилища и службами хранилища Azure]: http://msdn.microsoft.com/ru-ru/library/gg433135.aspx
  [Инициализация эмулятора хранилища с помощью командной строки]: http://msdn.microsoft.com/ru-ru/library/gg433132.aspx
  ...
  [здесь]: http://www.wireshark.org/docs/wsug_html_chunked/ChAdvFollowTCPSection.html
  [9]: ./media/storage-monitoring-diagnosing-troubleshooting/wireshark-screenshot-3.png
  [руководстве пользователя Wireshark]: http://www.wireshark.org/docs/wsug_html_chunked/
  [Поставщик PEF-WebProxy]: http://technet.microsoft.com/ru-ru/library/jj674814.aspx
  ...
  [Поставщик PEF-NDIS-PacketCapture]: http://technet.microsoft.com/ru-ru/library/jj659264.aspx
  [Формат журналов аналитик хранилища]: http://msdn.microsoft.com/ru-ru/library/hh343259.aspx
  [12]: http://msdn.microsoft.com/ru-ru/library/hh343264.aspx
  [Application Insights для Visual Studio Online]: http://msdn.microsoft.com/ru-ru/library/dn481095.aspx
