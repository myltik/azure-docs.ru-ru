...
-   [Введение][Введение]
    ...
    -   [Организация этого руководства][Организация этого руководства]
-   [Мониторинг службы хранилища][Мониторинг службы хранилища]

    -   [Мониторинг работоспособности службы][Мониторинг работоспособности службы]
    -   [Мониторинг емкости][Мониторинг емкости]
    -   [Мониторинг доступности][Мониторинг доступности]
    -   [Мониторинг производительности][Мониторинг производительности]
-   [Диагностика неполадок с хранилищем][Диагностика неполадок с хранилищем]

    -   [Проблемы с работоспособностью службы][Проблемы с работоспособностью службы]
    -   [Проблемы с производительностью][Проблемы с производительностью]
    -   [Диагностика ошибок][Диагностика ошибок]
    -   [Проблемы с эмулятором хранения][Проблемы с эмулятором хранения]
    -   [Инструменты для ведения журналов хранилища][Инструменты для ведения журналов хранилища]
    -   [Использование инструментов для ведения сетевого журнала][Использование инструментов для ведения сетевого журнала]
-   [Комплексная трассировка][Комплексная трассировка]

    -   [Сопоставление данных журналов][Сопоставление данных журналов]
...
В разделе [Мониторинг службы хранилища][Мониторинг службы хранилища] описывается, как с помощью метрик аналитики хранилища Azure (метрик хранилища) отслеживать работоспособность и производительность ваших служб хранилища Azure.

Раздел [Диагностика неполадок с хранилищем][Диагностика неполадок с хранилищем] посвящен диагностике проблем с использованием аналитических журналов хранилища Azure (журналов хранилища). Кроме того, в нем описывается, как включить ведение журналов на стороне клиента с использованием средств из одной или нескольких клиентских библиотек, таких как клиентская библиотека хранилища для .NET или Azure SDK для Java.

В разделе [Комплексная трассировка][Комплексная трассировка] вы найдете сведения о корреляции данных из различных журналов и метрик.
...
-   Изучать потенциальные проблемы с помощью диагностических инструментов, описанных в разделе [Диагностика неполадок с хранилищем][Диагностика неполадок с хранилищем].
...
-   существенные отклонения от базовых значений метрик, мониторинг которых выполняется в соответствии с описанием в предыдущем разделе [Мониторинг службы хранилища][Мониторинг службы хранилища];
...
-   Microsoft Network Monitor (Netmon) (<http://www.microsoft.com/en-us/download/details.aspx?id=4865>) и Wireshark (<http://www.wireshark.org/>) — бесплатные средства для анализа сетевых протоколов, с помощью которых можно просматривать подробную информацию о пакетах для широкого спектра протоколов. Дополнительные сведения о Wireshark см. в [приложении 2 "Отслеживание сетевого трафика с помощью Wireshark".][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]".
...
Дополнительные сведения об устранении неполадок в сети с помощью Wireshark см. в [приложении 2 "Отслеживание сетевого трафика с помощью Wireshark".][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]."

Дополнительные сведения об устранении неполадок в сети с помощью Microsoft Message Analyzer см. в [приложении 3 "Отслеживание сетевого трафика с помощью Microsoft Message Analyzer".][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]."
...
<table>
  ...
  <tr>
    <td>de8b1c3c-...</td>
    <td>Во время операции вызвано исключение: удаленный сервер вернул ошибку (404)&nbsp;&mdash; не найдено.</td>
  </tr>
  <tr>
    <td>de8b1c3c-...</td>
...
<table>
  ...
  <tr>
    <td>Request id header (Заголовок идентификатора запроса)</td>
...
Дополнительные сведения об использовании Fiddler см. в [приложении 1 "Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler".][Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler]."

Дополнительные сведения об использовании Wireshark см. в [приложении 2 "Отслеживание сетевого трафика с помощью Wireshark".][Приложение 2. Отслеживание сетевого трафика с помощью Wireshark]."

Дополнительные сведения об использовании Microsoft Message Analyzer см. в [приложении 3 "Отслеживание сетевого трафика с помощью Microsoft Message Analyzer".][Приложение 3. Отслеживание сетевого трафика с помощью Microsoft Message Analyzer]."
...
<!--Image references-->
  ...
  [Организация этого руководства]: #how-this-guide-is-organized
  ...
  [Схема таблицы метрик аналитики хранилища]: http://msdn.microsoft.com/en-us/library/azure/hh343264.aspx
  ...
  [Общие коды ошибок API-интерфейса REST]: http://msdn.microsoft.com/en-us/library/azure/dd179357.aspx
  [Коды ошибок службы BLOB-объектов]: http://msdn.microsoft.com/en-us/library/azure/dd179439.aspx
  [Коды ошибок службы очередей]: http://msdn.microsoft.com/en-us/library/azure/dd179446.aspx
  [Коды ошибок для службы таблиц]: http://msdn.microsoft.com/en-us/library/azure/dd179438.aspx
  ...
  [Отладка, трассировка и профилирование]: http://msdn.microsoft.com/en-us/library/7fe0dd2y(v=vs.110).aspx
  ...
  [Пространство имен Microsoft.WindowsAzure.Storage.RetryPolicies]: http://msdn.microsoft.com/en-us/library/microsoft.windowsazure.storage.retrypolicies.aspx
  ...
  [Делегирование доступа с помощью подписанного URL-адреса]: http://msdn.microsoft.com/en-us/library/azure/ee395415.aspx
  ...
  [Поддержка общего доступа к ресурсам независимо от источника (CORS) для служб хранилища Azure]: http://msdn.microsoft.com/en-us/library/windowsazure/dn535601.aspx
  ...
  [Различия между эмулятором хранилища и службами хранилища Azure]: http://msdn.microsoft.com/en-us/library/gg433135.aspx
  [Инициализация эмулятора хранилища с помощью командной строки]: http://msdn.microsoft.com/en-us/library/gg433132.aspx
  ...
  [Поставщик PEF-WebProxy]: http://technet.microsoft.com/en-us/library/jj674814.aspx
  ...
  [Поставщик PEF-NDIS-PacketCapture]: http://technet.microsoft.com/en-us/library/jj659264.aspx
  [Формат журналов аналитик хранилища]: http://msdn.microsoft.com/en-us/library/hh343259.aspx
  [12]: http://msdn.microsoft.com/en-us/library/hh343264.aspx
  [Application Insights для Visual Studio Online]: http://msdn.microsoft.com/en-us/library/dn481095.aspx
