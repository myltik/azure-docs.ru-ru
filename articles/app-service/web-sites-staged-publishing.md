...
Каждый режим плана службы приложений поддерживает разное количество слотов развертывания. Чтобы узнать, сколько слотов поддерживает режим вашего приложения, изучите статью [Цены на службу приложений](https://azure.microsoft.com/pricing/details/app-service/).

* Если приложение имеет несколько слотов, режим изменить нельзя.
...
Чтобы включить несколько слотов развертывания, приложение должно работать в режиме **Стандартный** или **Премиум**.
...
2. Выберите параметр **Слоты развертывания** и щелкните **Добавить слот**.
   ...
   > [!NOTE]
   > Если приложение не работает в режиме **Стандартный** или **Премиум**, появится сообщение со списком поддерживаемых режимов, которые позволяют использовать промежуточную публикацию. На этом этапе вы можете выбрать команду **Обновить** и перейти на вкладку **Масштаб** приложения, а затем продолжить процесс.
   > 
   > 
...
Чтобы привязать параметры приложения или строку подключения к слоту (запретить их перенос в другой слот), откройте для нужного слота колонку **Параметры приложения** и установите флажок **Параметр слота** для всех элементов конфигурации, которые не нужно переносить в другой слот. Обратите внимание, что установка этого флажка для определенного элемента конфигурации приводит к тому, что этот элемент нельзя будет переключить в другие слоты развертывания, связанные с веб-приложением.
...
- Сохраняет в неизменном виде целевой слот, чтобы не влиять на текущую рабочую нагрузку в этом слоте (т. е. в рабочем слоте).
...
Вы сможете увидеть, как приложение будет работать в конфигурации целевого слота. После завершения проверки выполните отдельное действие подтверждения переноса. Этот шаг дает еще одно преимущество: целевой слот будет заранее подготовлен в требуемой конфигурации, и клиенты не столкнутся с простоем приложения.  
...
- - -
### <a name="initiate-a-swap-with-review-multi-phase-swap-and-apply-destination-slot-configuration-to-source-slot"></a>Запуск переключения с предварительным просмотром (многофазное переключение) и применение конфигурации целевого слота к исходному слоту
```
...
> [!NOTE] 
> Чтобы ознакомиться с командами [Azure CLI 2.0](https://github.com/Azure/azure-cli) для слотов развертывания, выполните команду [az webapp deployment slot](/cli/azure/webapp/deployment/slot).

- - -
### <a name="azure-site-list"></a>azure site list
Чтобы получить информацию о приложениях в текущей подписке, вызовите команду **azure site list**, как показано в следующем примере.

`azure site list webappslotstest`

- - -
### <a name="azure-site-create"></a>azure site create
Чтобы создать слот развертывания, вызовите команду **azure site create** и укажите имя существующего приложения и имя создаваемого слота, как показано в следующем примере.
...
[Azure Web Sites – block web access to non-production deployment slots](http://ruslany.net/2014/04/azure-web-sites-block-web-access-to-non-production-deployment-slots/) (Веб-сайты Azure блокируют доступ к непроизводственным слотам развертывания) 
