...
Ограничения IP-адресов позволяют определить список IP-адресов, для которых заблокирован доступ к вашему приложению. Список блокировок может включать отдельные IP-адреса или их диапазон, который определяется маской подсети.

Когда клиентом создается запрос к приложению, IP-адрес сравнивается с адресами из списка блокировок. Если обнаружено соответствие, приложение выводит код состояния [HTTP 403](https://en.wikipedia.org/wiki/HTTP_403).
