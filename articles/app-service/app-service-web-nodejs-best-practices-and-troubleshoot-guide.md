...
3. В веб-приложениях Azure используется функция, которая называется "локальный кэш". С помощью этой функции можно скопировать содержимое сетевой папки на локальный диск виртуальной машины. Так как файлы расположены локально, загрузка node\_modules происходит значительно быстрее.
