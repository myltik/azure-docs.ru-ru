...
При создании нового приложения следует рассмотреть возможность создания новой группы ресурсов. С одной стороны, если создаваемое приложение будет компонентом более крупного приложения, то это приложение необходимо создать в группе ресурсов, которая выделена более крупному приложению.

Независимо от того, является ли новое приложение независимым проектом или частью боле крупного приложения, вы можете использовать существующий план службы приложений для размещения или создания такого приложения. Это, скорее, вопрос емкости и ожидаемой нагрузки.

Если это новое приложение будет потреблять много ресурсов, а действующие на него факторы масштабирования будут отличаться от аналогичных факторов других приложений, размещенных в существующем плане, рекомендуется изолировать такое приложение в отдельном плане.

Создание нового плана позволяет выделить новый набор ресурсов для приложения и предоставляет дополнительные возможности для контроля выделенных ресурсов, так как каждый план получает свой собственный набор экземпляров.
...
Можно создать пустой **план службы приложений** с помощью средства выбора **плана службы приложений** или при создании приложения.

На [портале Azure](https://portal.azure.com) щелкните **СОЗДАТЬ**, затем выберите **Интернет+мобильные устройства** и щелкните **Веб-приложения**, **Мобильные приложения**, **Приложения API** или **Function Apps** (Приложения-функции). ![][createWebApp]

Затем можно выбрать или создать план службы приложений для нового приложения.
  
 .![][createASP]

Чтобы создать новый план службы приложений, щелкните **+ Создать**, введите имя в поле **План службы приложений** и выберите соответствующее **расположение**. Щелкните **Ценовая категория** и выберите соответствующую категорию для службы. Щелкните **Просмотреть все**, чтобы просмотреть другие варианты тарификации, например **Бесплатный** и **Общий**. После выбора ценовой категории нажмите кнопку **Выбрать**.
 
## Перемещение приложения в другой план службы приложений

Приложения можно переместить в другой план службы приложений на [портале Azure](https://portal.azure.com). Приложения можно перемещать между планами, если планы находятся в одной группе ресурсов и географическом регионе.
...
Откроется селектор "План служб приложений". На этом шаге можно выбрать существующий план или создать новый. Отображаются только допустимые планы (в одной группе ресурсов и географическом расположении).

.![][change]

Обратите внимание, что каждый план имеет свою ценовую категорию. Например, при перемещении сайта с уровня **Бесплатный** на уровень **Стандартный** ваше приложение сможет использовать все функции и ресурсы уровня **Стандартный**.

## Клонирование приложения в другой план служб приложений
Если вы хотите переместить приложение в другой регион, один из вариантов — клонировать его. При клонировании создается копия приложения в новом или существующем плане служб приложений либо среде службы приложений в любом регионе.

 .![][appclone]
 
Функцию **клонирования приложения** можно найти в меню **Средства**.

С клонированием связаны определенные ограничения. Дополнительные сведения об этом см. [здесь](../app-service-web/app-service-web-app-cloning-portal.md).
...
- Измените **число экземпляров** плана. Например, план уровня **Стандартный**, развернутый до 3 экземпляров, можно масштабировать до 10 экземпляров, а план уровня **Премиум** можно масштабировать до 20 экземпляров (зависит от доступности). Все приложения, связанные с этим планом, смогут использовать дополнительные ресурсы ЦП и памяти, предоставляемые увеличенным числом экземпляров.

Можно изменить ценовую категорию и размер экземпляров, щелкнув элемент **Увеличить масштаб** в разделе параметров приложения или плана служб приложений. Изменения будут применены к **плану службы приложений** и повлияют на все приложения, размещенные в нем.
