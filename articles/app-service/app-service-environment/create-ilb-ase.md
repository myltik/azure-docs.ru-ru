...
При использовании Функций Azure в среде ASE с ILB может возникнуть сообщение об ошибке "Сейчас не удается извлечь ваши функции. Повторите попытку позже". Это обусловлено тем, что пользовательский интерфейс функций Azure использует сайт SCM по протоколу HTTPS. Так происходит, когда сертификат HTTP используется для среды ASE, у которой нет корневого сертификата в браузере. При этом в браузерах Internet Explorer и Edge параметр *accept-invalid-cert* нельзя применить одновременно на нескольких вкладках. Таким образом, необходимо выполнить одно из следующих действий:

- Добавьте сертификат в хранилище доверенных сертификатов. 
- Используйте Chrome. Однако сначала перейдите на сайт SCM и примите недоверенный сертификат. Вернитесь на портал.
