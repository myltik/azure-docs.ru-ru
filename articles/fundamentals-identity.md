<properties urlDisplayName="Identity" pageTitle="Удостоверение Azure" metaKeywords="Azure identity, Azure Active Directory, Azure AD, cloud identity ad, cloud active directory" description="Узнайте, как можно использовать службу каталогов Active Directory в Azure." metaCanonical="" services="active-directory" documentationCenter=".NET" title="" authors="robb" solutions="" manager="johndaw" editor="" />

<tags ms.service="multiple" ms.workload="multiple" ms.tgt_pltfrm="na" ms.devlang="na" ms.topic="article" ms.date="01/01/1900" ms.author="robb" />

[WACOM.INCLUDE [identity](../includes/identity.md)]

