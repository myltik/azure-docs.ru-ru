...
* Развернутое приложение SaaS Wingtip Tickets с отдельной базой данных для каждого клиента. Вы можете развернуть его менее чем за пять минут, следуя инструкциям из статьи о [развертывании и изучении приложения Wingtip Tickets SaaS Database Per Tenant](saas-dbpertenant-get-started-deploy.md).
...
Скрипты и исходный код приложения SaaS Wingtip Tickets с отдельной базой данных для каждого клиента доступны в репозитории github [WingtipTicketsSaaS-DbPerTenant](https://github.com/Microsoft/WingtipTicketsSaaS-DbPerTenant). Файлы скриптов находятся в папке [Learning Modules](https://github.com/Microsoft/WingtipTicketsSaaS-DbPerTenant/tree/master/Learning%20Modules). Скачайте папку **Learning Modules** на локальный компьютер, сохраняя ее структуру.
