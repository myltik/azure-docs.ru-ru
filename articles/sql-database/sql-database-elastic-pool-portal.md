...
Базы данных, которые нужно добавить в пул, можно выбрать в любой момент. (Чтобы создать новую базу данных в пуле, см. раздел [Добавление и удаление баз данных](sql-database-elastic-pool-portal.md#add-and-remove-databases-from-the-pool) ниже.)

При создании нового пула служба Azure рекомендует базы данных, которые можно включить в пул, и автоматически отмечает их для добавления. В пул можно добавить все базы данных на сервере или же добавить только часть из исходного списка.
...
### Добавление существующей базы данных в пул
...
Кроме того, после создания пула можно использовать Transact-SQL для создания новых эластичных баз данных в пуле и перемещения баз данных в пул и из него. Дополнительную информацию об этом см. в статье [Справочник по пулу эластичных баз данных для базы данных SQL](sql-database-elastic-pool-reference.md#Transact-SQL), раздел Transact-SQL.*


### Добавление новой базы данных в пул
...
В базе данных SQL уже настроен нужный сервер и пул. Вам нужно только ввести имя и выбрать нужные параметры базы данных, а затем нажать кнопку **ОК**, чтобы создать новую базу данных.
   ...
   ![создание эластичной базы данных][13]
...
- **Настроить пул**. Позволяет изменять настройки eDTU для пула и отдельных баз данных.
