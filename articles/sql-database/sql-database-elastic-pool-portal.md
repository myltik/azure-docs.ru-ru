...
# Создание масштабируемого пула эластичных баз данных SQL с использованием портала предварительной версии Azure
...
В этой статье описано создание масштабируемого [пула эластичных баз данных ](sql-database-elastic-pool.md)с помощью портала предварительной версии Azure. Конфигурация базы данных SQL с пулами эластичных баз данных упрощает администрирование и распределение ресурсов между несколькими базами данных.
...
Если появится сообщение о том, что для выбранного сервера есть рекомендуемый пул, щелкните его, чтобы с легкостью просмотреть и создать пул, оптимизированный для баз данных вашего сервера. Дополнительные сведения см. в разделе [Рекомендуемые пулы эластичных баз данных](sql-database-elastic-pool-portal.md#recommended-elastic-database-pools).


![Создание эластичного пула][1]


Колонка **Пул эластичных баз данных** предлагает возможности выбора ценовой категории, добавления баз данных и настройки производительности пула.

> [AZURE.NOTE]При первом выборе команды **Добавить пул** необходимо принять условия предоставления предварительной версии, выбрав пункт **Условия предварительной версии** и заполнив колонку **Условия предварительной версии**. Это нужно сделать только один раз для каждой подписки.
...
Исходя из рабочей нагрузки на базы данных служба отметит звездочкой (![звезда][10]) одну или несколько ценовых категорий.
...
Производительность пула определяется тремя настраиваемыми параметрами: гарантированным количеством eDTU для пула и eDTU MIN и eDTU MAX для эластичных баз данных в пуле. В таблице ниже приведено описание каждого из этих параметров, а также рекомендации по их настройке. Значения доступных параметров описаны в [справочнике по пулу эластичных баз данных](sql-database-elastic-pool-reference.md).
...
- **Настройка пула** позволяет изменять настройки eDTU для пула и для отдельных баз данных.
