...
Вам следует попытаться восстановить соединение SQL или установить его повторно в зависимости от следующих условий: * **Временная ошибка возникает при попытке подключения**: подключение следует повторить после задержки в несколько секунд. * **Временная ошибка возникает, когда выполняется команда SQL-запроса**: выполнение команды не следует повторять сразу. Лучше заново установить подключение после некоторой задержки. Затем выполнение команды можно повторить.
...
Если вы используете пакет ADO.NET 4.0 или более ранней версии, мы рекомендуем установить последнюю версию. Начиная с ноября 2015 г. вы можете [скачать пакет ADO.NET 4.6.1](http://blogs.msdn.com/b/dotnet/archive/2015/11/30/net-framework-4-6-1-is-now-available.aspx).
...
Логика повторных попыток при временных ошибках — возможности, которые предоставляет EntLib60 по этому аспекту, описаны в статье [4 - Perseverance, Secret of All Triumphs: Using the Transient Fault Handling Application Block](http://msdn.microsoft.com/library/dn440719%28v=pandp.60%29.aspx) (4. Настойчивость, секрет всех побед: использование решения Transient Fault Handling Application Block).
