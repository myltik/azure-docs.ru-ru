...
Дополнительные сведения см. в статье [Руководство по настройке параметров брандмауэра для базы данных SQL](sql-database-configure-firewall-settings.md).
...
Если вы используете пакет ADO.NET 4.0 или более ранней версии, мы рекомендуем обновить его до последней версии. С июля 2015 г. вы можете [загрузить пакет ADO.NET 4.6](http://blogs.msdn.com/b/dotnet/archive/2015/07/20/announcing-net-framework-4-6.aspx).
...
В Linux могут быть полезны следующие служебные программы: `netstat -nap` и `nmap -sS -O 127.0.0.1` (вместо значения, приведенного в примере, укажите свой IP-адрес).
...
Enterprise Library 6 (EntLib60) — это платформа классов .NET, которая помогает реализовывать надежные клиенты облачных служб, одной из которых является служба базы данных SQL Azure. Некоторую информацию обо всех полезных возможностях библиотеки EntLib60 вы найдете в статье [Enterprise Library 6 (апрель 2013 г.)](http://msdn.microsoft.com/library/dn169621%28v=pandp.60%29.aspx).


Логика повторных попыток при временных сбоях — возможности, которые предоставляет EntLib60 по этому аспекту, описаны в статье [4\. Настойчивость, секрет всех побед: использование решения Transient Fault Handling Application Block](http://msdn.microsoft.com/library/dn440719%28v=pandp.60%29.aspx).


Небольшой пример кода на языке C#, в котором используется логика повторных попыток на основе библиотеки EntLib60, см. в статье [Образец кода: логика повторных попыток из библиотеки Enterprise Library 6 в процедуре C# для подключения к базе данных SQL](sql-database-develop-entlib-csharp-retry-windows.md).
