...
Временные ошибки (или временные сбои) возникают из-за причин, которые вскоре устраняются автоматически. Иногда временная ошибка возникает потому, что система Azure быстро сменяет аппаратные ресурсы, чтобы лучше распределить нагрузку, связанную с разными рабочими нагрузками. Когда выполняется эта перенастройка, могут возникнуть ошибки подключения к базе данных SQL Azure.

Если клиентская программа использует пакет ADO.NET, ваша программа получает исключение **SqlException** и таким образом узнает о временной ошибке. Значение свойства **Number** можно сравнить со списком временных ошибок в начале статьи 
[Сообщения об ошибках для клиентских программ базы данных SQL](sql-database-develop-error-messages.md).

### Подключения и команды

Вам следует попытаться восстановить соединение SQL или установить его повторно в зависимости от следующих условий: 
* **Временная ошибка возникает при попытке подключения**: подключение следует повторить после задержки в несколько секунд. 
* **Временная ошибка возникает, когда выполняется команда SQL-запроса**: выполнение команды не следует повторять сразу. Лучше заново установить подключение после некоторой задержки. Затем выполнение команды можно повторить.
...
Если клиентская программа подключается к базе данных SQL Azure с помощью класса .NET Framework **System.Data.SqlClient.SqlConnection**, следует использовать .NET 4.6.1 или более поздней версии. В этих версиях реализована поддержка повторных попыток подключения. Дополнительную информацию об этой функции см. [здесь](http://go.microsoft.com/fwlink/?linkid=393996).
...
ADO.NET 4.6.1: добавлена поддержка протокола TDS 7.4. В эту поддержку входит улучшенное по сравнению с версией 4.0 подключение. 2. Поддержка пула подключений. В нее входит также эффективная проверка функционирования объекта соединения, который видит ваша программа.
...
Если вы используете пакет ADO.NET 4.0 или более ранней версии, мы рекомендуем установить последнюю версию. 
...
Логика повторных попыток при временных ошибках — возможности, которые предоставляет EntLib60 по этому аспекту, описаны в статье 
