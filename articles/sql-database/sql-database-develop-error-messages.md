...
> [AZURE.TIP]Обратите особое внимание на следующий раздел о [*временных сбоях*](#bkmk_connection_errors).
...
### Наиболее распространенные временные сбои
...
|49918|16|Не удается обработать запрос. Недостаточно ресурсов для обработки запроса.<br/><br/>Служба в настоящий момент занята. Повторите запрос позже. |
...
**Примечание.** Попробуйте включить в логику повторных попыток ошибки федераций 10053 и 10054.
...
В приведенной ниже таблице представлены ошибки, которые могут возникнуть при работе с федерациями.
...
> 
> Дополнительные сведения об эластичном масштабировании можно найти в [статьях об эластичном масштабировании Базы данных SQL Azure](sql-database-elastic-scale-documentation-map.md). Внедрение настраиваемых решений сегментирования позволяет повысить масштабируемость, гибкость и производительность. Дополнительные сведения о настраиваемом сегментировании можно найти в статье [Общие сведения о возможностях эластичных баз данных](sql-database-elastic-scale-introduction.md).


|Номер ошибки|Уровень серьезности|Описание|Устранение|
|---:|---:|:---|:---|
|266|16|Оператор <statement> не поддерживается в транзакции с использованием нескольких операторов.|Перед использованием данного оператора убедитесь, что `@@trancount` имеет значение 0 при подключении.|
...
|40604|16|Нельзя выполнить `CREATE DATABASE`, так как это приведет к превышению квоты сервера.|Увеличьте допустимое количество баз данных на сервере.|
