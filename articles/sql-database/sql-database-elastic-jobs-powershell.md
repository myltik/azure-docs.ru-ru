...
Для выбора подписки вам понадобится идентификатор (**-SubscriptionId**) или имя подписки (**-SubscriptionName**). Если у вас несколько подписок, вы можете запустить командлет **Get-AzureSubscription** и скопировать необходимые сведения о подписке из полученных данных. Получив сведения о подписке, выполните следующий командлет, чтобы сделать ее подпиской по умолчанию, в частности целью для создания заданий и управления ими:
...
1. Скомпилируйте и запустите приложение, описанное в качестве примера в статье **Приступая к работе с инструментами эластичных баз данных**. Следуйте инструкциям из раздела [Загрузка и запуск примера приложения](sql-database-elastic-scale-get-started.md#Getting-started-with-elastic-database-tools) вплоть до шага 7. В конце шага 7 появится следующее окно командной строки:

	![окно командной строки.][1]

2.  В командном окне введите 1 и нажмите клавишу **ВВОД**. Это позволит создать диспетчер сопоставления сегментов и добавить два сегмента на сервер. Затем введите 3 и нажмите клавишу **ВВОД**. Повторите это действие четыре раза. Это позволит вставить строки демонстрационных данных в свои сегменты.
  
3.  На [портале предварительной версии Azure](https://portal.azure.com) на сервере v12 должны появиться три новые базы данных.
