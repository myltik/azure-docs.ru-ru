<properties
	pageTitle="Развертывание службы разбиения и объединения | Microsoft Azure"
...
Средство разбиения и объединения перемещает данные между сегментированными базами данных. См. [Перемещение данных между масштабируемыми облачными базами данных](sql-database-elastic-scale-overview-split-and-merge.md).
...
1. Создайте базу данных SQL Azure, которая будет использоваться в качестве базы данных состояния разбиения и объединения. Перейдите на [портал Azure](https://ms.portal.azure.com). Создайте новую **Базу данных SQL**. Присвойте базе данных имя и создайте администратор и пароль. Обязательно запишите имя и пароль для последующего использования.
...
3. Создайте учетную запись хранения Azure, которая будет использоваться для вывода диагностических данных. Перейдите на портал Azure. На левой панели выберите **Создать**, затем **Данные и хранилище** и **Хранилище**.

4. Создайте облачную службу Azure для размещения службы разбиения и объединения. Перейдите на портал Azure. На левой панели выберите **Создать**, затем **Вычисления**, **Облачная служба** и **Создать**.
...
1. В папке, куда вы скачали сборку службы разбиения и объединения, создайте копию файла **ServiceConfiguration.Template.cscfg**, который поставляется вместе с **SplitMergeService.cspkg**, и переименуйте его в **ServiceConfiguration.cscfg**.
...
Определите конечную веб-точку службы разбиения и объединения. Ее можно найти на классическом портале Azure, открыв **панель мониторинга** облачной службы и посмотрев справа в разделе **URL-адрес узла**. Замените **http://** на **https://**, так как параметры безопасности по умолчанию отключают конечную точку HTTP. Откройте в браузере этот URL-адрес.
