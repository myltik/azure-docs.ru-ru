...
1. Создайте базу данных SQL Azure DB, которая будет использоваться в качестве базы данных состояния разбиения и объединения. Войдите на [портал предварительной версии Azure](https://ms.portal.azure.com). Создайте новую **Базу данных SQL**. Введите имя базы данных и создайте нового пользователя и пароль. Обязательно запишите имя и пароль для последующего использования.

2. Убедитесь, что сервер баз данных SQL Azure позволяет службам Azure подключиться к нему. На [портале предварительной версии](https://ms.portal.azure.com) в колонке **Параметры брандмауэра** задайте для параметра **Разрешить доступ к службам Azure** значение **Вкл.**. Щелкните значок «Сохранить».

    ![Допустимые службы][1]

3. Создайте учетную запись хранения Azure, которая будет использоваться для вывода диагностических данных. Перейдите на [портал управления Azure](https://manage.windowsazure.com). В нижнем левом углу выберите **Создать**, щелкните **Службы данных**, **Хранилище**, а затем **Быстрое создание**.

4. Создайте облачную службу Azure для размещения службы разбиения и объединения. Перейдите на [портал управления Azure](https://manage.windowsazure.com). В нижнем левом углу выберите **Создать**, а затем щелкните **Вычисления**, **Облачная служба** и **Быстрое создание**.
...
Подробные указания по настройке безопасности службы см. в разделе [Настройка безопасности службы разбиения и объединения](../sql-database-elastic-scale-configure-security.md).
