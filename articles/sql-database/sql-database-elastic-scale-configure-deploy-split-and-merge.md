<properties 
	...
	pageTitle="Учебник по службе разбиения и объединения SQL Azure" 
	description="Разбиение и объединение с технологией эластичного масштабирования" 
...
#Учебник по эластичному масштабированию службы разбиения и объединения
...
2. Убедитесь, что сервер баз данных SQL Azure позволяет службам Microsoft Azure подключиться к нему. Перейдите на [портал управления Azure](https://manage.windowsazure.com) и на левой панели щелкните **Базы данных SQL**. На ленте в верхней части экрана щелкните **Серверы** и выберите сервер. Затем щелкните сверху **Настройка** и убедитесь, что для параметра **Службы Windows Azure** задано значение **Да**.
...
4. Создайте облачную службу Azure для размещения службы разбиения и объединения.  Перейдите на [портал управления Azure](https://manage.windowsazure.com). В нижнем левом углу выберите **Создать**, затем **Вычисления**, **Облачная служба** и **Быстрое создание**. 
...
5.    Настройка целевого объекта для журналов диагностики требует учетной записи хранения в Azure. Настройка для разбиения и объединения требует наличия строки подключения к вашей учетной записи хранения Azure. Строка подключения должна выглядеть следующим образом:
...
Подробные указания по настройке безопасности службы см. в разделе [Настройки безопасности эластичного масштабирования](../sql-database-elastic-scale-configure-security.md).

В рамках  простого теста развертывания, подходящего для работы с этим учебником, для запуска службы необходимо выполнить минимальный набор шагов настройки. Выполнение следующих действий позволяет взаимодействовать со службой только одной машине или учетной записи.

### Создание самозаверяющего сертификата 

Создайте новый каталог и из этого каталога и выполните следующие команды в окне [командной строки разработчика для Visual Studio](http://msdn.microsoft.com/library/ms229859.aspx):
...
Пожалуйста, обратите внимание, что в рабочих развертываниях необходимо использовать отдельные сертификаты для ЦС, шифрования, сервера и клиента. Подробные указания см. в разделе [Конфигурация безопасности](../sql-database-elastic-scale-configure-security.md).
...
5. В диалоговом окне введите метку развертывания. Для 'Package' и 'Configuration' выберите 'From Local', а затем - файл **SplitMergeService.cspkg** и настроенный ранее CSCFG-файл.
...
* Убедитесь, что сервер баз данных SQL Azure позволяет службам Microsoft Azure подключиться к нему. Для этого откройте https://manage.windowsazure.com, нажмите кнопку слева "База данных SQL", вверху щелкните "Серверы" и выберите сервер. Щелкните сверху **Настройка** и убедитесь, что для параметра **Службы Windows Azure** задано значение "Да". (См. раздел "Предварительные требования" в верхней части этой статьи.)

* Просмотрите журналы диагностики для экземпляра службы Split/Merge. Откройте экземпляр Visual Studio и в строке меню щелкните **Вид**, затем **Обозреватель серверов**. Щелкните значок **Windows Azure** для подключения к подписке Azure. Перейдите в Windows Azure -> Хранилище -> <ваша учетная запись хранения> -> Таблицы -> WADLogsTable. Дополнительную информацию см. в статье [Обзор ресурсов хранилища с помощью обозревателя серверов](http://msdn.microsoft.com/library/azure/ff683677.aspx). 
...
Эта ошибка означает, что SSL-сертификат настроен неправильно. Следуйте указаниям в разделе  'Connecting with a web browser'.
