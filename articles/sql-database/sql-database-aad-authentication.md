...
- [ODBC версии 13.1](https://www.microsoft.com/download/details.aspx?id=53339) поддерживает проверку подлинности Azure Active Directory, однако `sqlcmd.exe` и `bcp.exe` не могут подключаться с использованием проверки подлинности Azure Active Directory, так как они используют устаревший поставщик ODBC.
...
- Многофакторная проверка подлинности (MFA/2FA) и другие виды интерактивной проверки подлинности не поддерживаются.
...
> [AZURE.NOTE] При настройке администратора Azure AD новое имя администратора (пользователя или группы) не может уже присутствовать в виртуальной базе данных master как имя для входа с проверкой подлинности SQL Server. Если оно присутствует, настройка администратора Azure AD завершится ошибкой. Будет выполнен откат его создания и появится сообщение, что такой администратор (имя) уже существует. Такое имя для входа с проверкой подлинности SQL Server не входит в Azure AD, поэтому любая попытка подключиться к серверу с помощью проверки подлинности Azure AD завершится сбоем.
