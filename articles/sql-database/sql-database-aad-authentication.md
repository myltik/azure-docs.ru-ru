...
1. Запустите Management Studio, и в диалоговом окне **Подключение к серверу** \(или **Подключение к ядру СУБД**\) в поле **Проверка подлинности** выберите пункт **Встроенная проверка подлинности Active Directory**. Пароль не требуется \(или его нельзя ввести\), так как для подключения используются существующие учетные данные.
...
1. Запустите Management Studio и в диалоговом окне **Подключение к серверу** \(или **Подключиться к компоненту Database Engine**\) в поле **Проверка подлинности** выберите пункт **Проверка пароля Active Directory**.
