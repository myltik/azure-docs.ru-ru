...
- Изменение гарантированного количества eDTU на каждую базу данных \(databaseDtuMin\) или максимального количества eDTU на каждую базу данных \(databaseDtuMax\) обычно требует не более 5 минут.
- Изменение eDTU или размера хранилища \(Dtu\) пула зависит от общей емкости, используемой всеми базами данных в пуле. Изменение занимает порядка 90 минут или меньше на каждые 100 ГБ. Например, если общее пространство, используемое всеми базами данных в пуле, равно 200 ГБ, то ожидаемая задержка при изменении eDTU пула или размера хранилища составит 3 часа или менее.
