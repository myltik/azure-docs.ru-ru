<properties 
	pageTitle="Маршрутизация, зависящая от данных" 
	description="Как использовать ShardMapManager для маршрутизации, зависящей от данных — функции эластичных баз данных для БД Azure SQL" 
...
В данном примере инициализируются как диспетчер **ShardMapManager**, так и конкретная карта **ShardMap**, которую он содержит.

Для приложений, не использующих карту сегментов, учетные данные, применяемые фабричным методом, для обращения к диспетчеру **ShardMapManager** (в примере выше, *smmConnectionString*), должны заменяться учетными данными, доступными только для чтения, в базе данных **Глобальная карта сегментов**, ссылка на которую содержится в строке подключения. Обычно эти учетные данные отличаются от учетных данных, используемых в открытых подключениях к диспетчеру карты сегментов. См. также раздел [Использование учетных данных в клиентских библиотеках эластичной базы данных](sql-database-elastic-scale-manage-credentials.md).
...
* Строка **connectionString** используется для передачи только учетных данных пользователя для необходимого подключения. Имя базы данных и имя сервера не включаются в эту строку *connectionString*, так как метод будет определять базу данных и сервер с помощью карты **ShardMap**.
...
Это пример кода, использующего диспетчер сопоставления сегментов для осуществления маршрутизации, зависящей от данных, на основе значения целочисленного ключа **CustomerID**, использующего объект **ShardMap** с именем **customerShardMap**.
...
Метод **OpenConnectionForKeyAsync** также доступен, если в приложении используется асинхронное программирование с помощью ADO.Net. Его поведение представляет собой маршрутизацию, зависящую от данных, эквивалентную методу **Connection.OpenAsync** в ADO.NET.
...
Обработка временных сбоев может естественным образом сосуществовать с шаблоном маршрутизации, управляемой данными. Основным требованием является повтор всего запроса доступа к данным, включая блок **using**, с которым осуществляется подключение с помощью маршрутизацией, зависящей от данных. Приведенный выше пример можно переписать следующим образом (обратите внимание на выделенные изменения).
