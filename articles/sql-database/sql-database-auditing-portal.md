...
2. **База данных-получатель.** Аудит больших двоичных объектов можно включить и выключить только в параметрах аудита базы данных-источника.
   ...
   * После включения аудита больших двоичных объектов в базе данных-источнике он будет включен и в базе данных-получателе.

    > [!IMPORTANT]
    > По умолчанию настройки хранилища для базы данных-получателя идентичны настройкам для базы данных-источника, что приводит к появлению трафика между регионами. Чтобы избежать этого, включите аудит больших двоичных объектов на сервере-получателе и настройте локальное хранилище в параметрах хранилища сервера-получателя. Это переопределит расположение хранения для базы данных-получателя, и в результате каждая база данных будет хранить журналы аудита в своем локальном хранилище.  
