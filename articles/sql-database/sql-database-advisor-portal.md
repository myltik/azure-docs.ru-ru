---
title: Использование помощника по базам данных SQL Azure на портале Azure | Microsoft Docs
...
3. Выберите **Все параметры** > **Рекомендации**, чтобы просмотреть доступные **рекомендации** для выбранной базы данных.

> [!NOTE]
> Чтобы рекомендации сформировались, база данных должна использоваться примерно одну неделю и в течение этой недели должна быть активность. Кроме того, активность должна быть однообразной. Помощник по работе с базами данных SQL обеспечивает более эффективную оптимизацию при стабильном потоке запросов, чем на основе случайных всплесков активности. Если доступных рекомендаций нет, на странице **Рекомендации** будет указана причина.
> 
> 
...
3. При необходимости щелкните **Отменить отклонение**, чтобы вернуть индекс обратно в основной список **рекомендаций**.
