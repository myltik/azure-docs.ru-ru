...
Сегментированная среда включает ряд баз данных, которые могут размещаться на разных логических серверах. Каждый сервер содержит определенное количество баз данных — по одной на пользователя в однотенантном решении. Каждая база данных должна быть с чем-то сопоставлена в карте сегментов, чтобы вызовы могли направляться к правильному серверу и базе данных. Базы данных отслеживаются по ключу сегментирования, а каждому серверу назначается диапазон значений ключа. Например, ключ сегментирования может представлять имена клиентов от «В» до «Е». Сопоставление всех сегментов (т. е. баз данных) и их диапазонов сопоставления содержится в глобальной карте сегментирования. Каждая база данных также содержит карту содержащихся в сегментах диапазонов. Она называется "локальной картой сегментирования". LSM используется для проверки кэшированных данных. (Когда приложение подключается к сегменту, сопоставление кэшируется с приложением для быстрого извлечения данных. LSM проверяет сопоставление.)

GSM и LSM могут рассинхронизироваться по следующим причинам:

1. Несогласованность, вызванная удалением сегмента, диапазон которого считается более неиспользуемым, или переименованием сегмента. Удаление сегмента приводит к **потере сопоставления сегментов**. Переименование базы данных точно так же может привести к потере сопоставления сегментов. Возможно, сегмент придется удалить или просто изменить его расположение. 
