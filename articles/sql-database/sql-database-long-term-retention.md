...
Если изменить указанную выше политику и задать значение W = 0 (отмена еженедельных резервных копий), периодичность резервного копирования изменится согласно порядку, приведенному в таблице выше, на указанные сроки. Размеры хранилища, необходимые для хранения этих резервных копий, сократятся соответственно. Примечание. Копии LTR создаются службой хранилища Azure, и поэтому процесс копирования не влияет на производительность имеющейся базы данных.
Восстановление базы данных из хранилища долгосрочного хранения (LTR) можно выполнить, выбрав необходимую резервную копию по метке времени.   Базу данных можно восстановить на любой доступный сервер с той же подпиской, что и у исходной базы данных. 
