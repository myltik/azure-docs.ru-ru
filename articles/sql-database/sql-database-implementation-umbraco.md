<properties
   pageTitle="База данных SQL Azure: пример использования Azure — Umbraco | Microsoft Azure"
...
# Umbraco использует Базу данных SQL Azure, чтобы быстро подготавливать и масштабировать службы для тысяч клиентов в облаке

![Логотип Umbraco](./media/sql-database-case-study-umbraco/umbracologo.png)
...
![На рисунке 1](./media/sql-database-case-study-umbraco/figure1.png)

Рисунок 1. Жизненный цикл подготовки для UaaS
 
##Пулы эластичных БД Azure и автоматизация упрощают развертывание
...
2.	Использование
    ...
    ![На рис. 2](./media/sql-database-case-study-umbraco/figure2.png)

    ![Рис. 3](./media/sql-database-case-study-umbraco/figure3.png)
...
##Пулы эластичных БД обеспечивают удобное масштабирование UaaS
...
![Рис. 4](./media/sql-database-case-study-umbraco/figure4.png)

Рис. 3. Архитектура развертывания UaaS в Microsoft Azure

##Путь от центра обработки данных к облаку
...
> — Мортен Кристенсен, технический руководитель в Umbraco.
...
Кроме того, команда разработчиков Umbraco хотела найти решение, которое бы позволило по-максимуму повторно использовать существующий код. Разработчик Umbraco Миккель Хансен (Mikkel Hansen) рассказывает: "Мы были довольны уже привычными средствами разработки Майкрософт, такими как Microsoft SQL Server, База данных SQL Microsoft Azure, ASP.Net и IIS. Прежде чем инвестировать в облачное решение IaaS или PaaS, мы хотели убедиться в том, что оно будет поддерживать инструменты и платформы Майкрософт, чтобы нам не пришлось кардинально менять базовый код".
...
##Почему компания Umbraco выбрала Azure для UaaS
