...
В этом руководстве рассматриваются основы защиты базы данных SQL. Выполнив всего несколько простых действий, можно взять любую базу данных и значительно повысить ее уровень защиты от пользователей-злоумышленников или несанкционированного доступа.
...
1. Подключитесь к базе данных, например с помощью [SSMS](./sql-database-connect-query-ssms.md).
...
1. Подключитесь к базе данных, например с помощью [SSMS](./sql-database-connect-query-ssms.md), с учетными данными администратора сервера.
...
* Обзор всех функций защиты в базе данных SQL см. в статье [Защита Базы данных SQL](sql-database-security-overview.md).
* Для дополнительного шифрования конфиденциальных столбцов базы данных рекомендуется использовать шифрование на стороне клиента с помощью функции [Always Encrypted](https://docs.microsoft.com/sql/relational-databases/security/encryption/always-encrypted-database-engine).
* Если требуются дополнительные возможности управления доступом, можно применить [безопасность на уровне строк](https://docs.microsoft.com/sql/relational-databases/security/row-level-security). Она позволяет ограничить доступ к строкам в базе данных на основе членства пользователя в группе или контекста выполнения. Кроме того, можно использовать [динамическое маскирование данных](https://docs.microsoft.com/azure/sql-database/sql-database-dynamic-data-masking-get-started), которое ограничивает раскрытие конфиденциальных данных, маскируя их для непривилегированных пользователей на уровне приложения. 
