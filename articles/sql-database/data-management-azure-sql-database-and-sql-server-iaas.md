<properties
	pageTitle="Сравнение базы данных SQL (PaaS) с SQL Server в облаке на виртуальных машинах Azure (IaaS) | Microsoft Azure"
...
# Выбор варианта SQL Server в Azure: база данных SQL Azure (PaaS) или SQL Server на виртуальных машинах Azure (IaaS)

В Azure доступны два варианта размещения SQL Server в облаке.
...
**База данных SQL Azure** — это реляционная база данных, которая предоставляется как услуга (DBaaS), размещается в облаке Azure и относится к отраслевым категориям *программное обеспечение как услуга (SaaS)* и *платформа как услуга (PaaS)*. База данных SQL создана на основе стандартного аппаратного и программного обеспечения, которое принадлежит корпорации Майкрософт, размещается и обслуживается ею. Встроенные функции и функциональные возможности базы данных SQL позволяют разрабатывать приложения непосредственно в службе. При использовании базы данных SQL вы платите по мере использования и имеете возможность увеличивать масштаб базы данных или развертывать ее для повышения производительности без прерывания работы.
...
<tr>
   ...
   <th align="left" valign="middle">База данных SQL</th>
...
<tr>
   ...
   <td valign="middle">
          ...
          <li type=round>Приложения, которым требуются такие встроенные возможности, как высокая доступность, аварийное восстановление и обновление.
          <li type=round>Разработчики, которые не хотят тратить силы на настройку конфигурации и базовой операционной системы.
         <li type=round>Приложения, использующие шаблоны горизонтального масштабирования.
         <li type=round>Базы данных размером до 1&#160;ТБ.
         <li type=round>Создание приложений по модели «программное обеспечение как услуга» (SaaS).
  </ul>
</td>
   <td valign="middle">
      <ul>
      <li type=round>Существующие приложения, которым требуется быстрая миграция в облако с минимальными изменениями.
      <li type=round>Приложения SQL Server, которым нужен доступ к локальным ресурсам (например, Active Directory) из Azure по безопасному туннелю.
      <li type=round>Если вам требуется настраиваемая ИТ-среда со всеми правами администратора.
      <li type=round>Сценарии быстрой разработки и тестирования без покупки оборудования для дополнительного локального сервера SQL Server.
      <li type=round>Аварийное восстановление для локальных приложений SQL Server с помощью [резервного копирования в хранилище Azure] (http://msdn.microsoft.com/library/jj919148.aspx) или [реплик AlwaysOn с виртуальными машинами Azure] (../virtual-machines/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions.md).
...
<tr>
   ...
   <td valign="middle"><ul>
      <li type=round>SQL Server на виртуальных машинах Azure позволяет создавать приложения, которые частично работают в облаке и частично&#160;— на локальных ресурсах. Например, вы можете расширить локальную сеть и домен Active Directory в облако с помощью [виртуальной сети Azure] (../virtual-network/virtual-networks-overview.md). Кроме того, вы можете хранить локальные файлы данных в службе хранилища Azure с помощью функции [Файлы данных SQL Server в Azure] (http://msdn.microsoft.com/library/dn385720.aspx). Дополнительные сведения см. в статье [Общие сведения о гибридном облаке SQL Server 2014] (http://msdn.microsoft.com/library/dn606154.aspx).
      <li type=round>Поддерживает аварийное восстановление для локальных приложений SQL Server посредством [резервного копирования и восстановления SQL Server с помощью хранилища BLOB-объектов Azure] (http://msdn.microsoft.com/library/jj919148.aspx) или [реплик AlwaysOn на виртуальных машинах Azure] (../virtual-machines/virtual-machines-sql-server-high-availability-and-disaster-recovery-solutions.md).
      </ul></td>
...
**База данных SQL** продается клиентам в качестве услуги без лицензии, тогда как для SQL Server на виртуальных машинах Azure нужна обычная лицензия SQL Server.

**База данных SQL** имеет несколько уровней службы. Для каждого уровня счета выставляются за каждый час использования по фиксированной ставке, которая зависит от выбранного уровня службы и уровня производительности. Уровни служб Basic, Standard и Premium созданы, чтобы обеспечить прогнозируемую производительность нескольких уровней для удовлетворения максимальных требований приложения. Вы можете менять уровни служб и уровни производительности в соответствии с различными требованиями приложения к производительности. Последние сведения о доступных в настоящее время уровнях служб см. в статье [Уровни служб базы данных SQL Azure](sql-database-service-tiers.md).

При использовании **базы данных SQL** корпорация Майкрософт автоматически настраивает, исправляет и обновляет программное обеспечение базы данных, что позволит вам сократить затраты на администрирование. Кроме того, [встроенные функции резервного копирования](sql-database-business-continuity.md) помогают значительно экономить средства, особенно при наличии большого количества баз данных. При использовании базы данных SQL оплата не зависит от количества запросов, выполняемых в базе данных SQL Azure, и от входящего трафика. Счет выставляется только за [исходящий интернет-трафик](http://azure.microsoft.com/pricing/details/data-transfers/). Если в вашей базе данных осуществляется большое количество транзакций и нужно поддерживать много пользователей одновременно, мы рекомендуем использовать уровень службы «Премиум».

Для **SQL Server на виртуальных машинах Azure** вам потребуется традиционная лицензия SQL Server. Вы можете использовать образ SQL Server, предоставляемый платформой (он уже включает лицензию), или собственную лицензию SQL Server. Стоимость использования образов, которые предоставляет Azure, зависит от размера виртуальной машины, а также от выбранного выпуска SQL Server. Независимо от размера виртуальной машины или выпуска SQL Server вы оплачиваете лицензии SQL Server и Windows Server, а также стоимость службы хранилища Azure для дисков виртуальной машины по поминутному тарифу. Поминутное выставление счетов позволяет использовать SQL Server ровно столько, сколько нужно, без приобретения дополнительных лицензий SQL Server. Если вы передаете в Azure собственную лицензию SQL Server, плата взимается только за Windows Server и хранение данных. Дополнительные сведения о возможности использовать собственную лицензию см. в статье [Перемещение лицензий в рамках программы Software Assurance в Azure](http://azure.microsoft.com/pricing/license-mobility/).
...
Рекомендуем начать с руководства [Создание первой базы данных SQL Azure](sql-database-get-started.md).
...
Для начала рекомендуем ознакомиться со статьей [Подготовка виртуальной машины с SQL Server в Azure](virtual-machines-provision-sql-server.md).
