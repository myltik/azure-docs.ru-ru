...
- В следующем видеоролике на примере рабочей нагрузки показано, как достигается значительное улучшение показателей потребления ресурсов: [In-Memory OLTP in Azure SQL DB](https://channel9.msdn.com/Shows/Data-Exposed/In-Memory-OTLP-in-Azure-SQL-DB) (Выполняющаяся в памяти OLTP в базе данных SQL Azure). Дополнительные сведения см. в записи блога [In-Memory OLTP in Azure SQL Database](https://azure.microsoft.com/blog/in-memory-oltp-in-azure-sql-database/) (Выполняющаяся в памяти OLTP в базе данных SQL Azure).
...
- Выполняющуюся в памяти OLTP и индексы columnstore можно использовать вместе. Таким образом, вы можете создать оптимизированную для обработки в памяти таблицу с индексом columnstore. Это позволит ускорить как обработку транзакций, так и выполнение аналитических запросов к тем же данным.
...
В этой статье описываются особенности выполняющейся в памяти OLTP и индексов сolumnstore, которые применимы только к базе данных SQL Azure. Описание дополняется примерами. Сначала мы рассмотрим, как эти технологии влияют на использование хранилища и ограничения размера данных. Затем вы узнаете, как правильно изменять ценовую категорию для баз данных, в которых используются эти технологии. И наконец, вы ознакомитесь с двумя примерами по использованию выполняющейся в памяти OLTP и индексов сolumnstore в базе данных SQL Azure.
...
При расчете емкости хранилища выполняющейся в памяти OLTP используются следующие параметры.
...
Когда вы достигнете установленного ограничения, появится сообщение о том, что квота израсходована, и вы не сможете больше выполнять операции вставки и обновления данных. Чтобы избежать этого, необходимо удалить часть данных или перейти на более высокую ценовую категорию базы данных или пула.
...
- Установите для баз данных параметр Max-eDTU ниже, чем значение eDTU для пула в целом. Это ограничит использование хранилища выполняющейся в памяти OLTP для любой отдельной базы данных пула в соответствии с указанным значением eDTU.
- Установите для баз данных параметр Min-eDTU больше нуля. Это гарантирует, что каждая база данных в пуле получит некоторый объем хранилища выполняющейся в памяти OLTP в соответствии с указанным значением Min-eDTU.
...
Чтобы перейти на более высокую ценовую категорию базы данных, в которой используются технологии обработки в оперативной памяти, особые рекомендации не требуются, так как чем выше категория, тем больше функциональных возможностей и дополнительных ресурсов в нее включено. Понижение ценовой категории может повлиять на базу данных, особенно при переходе с уровня "Премиум" на уровень "Стандартный" или "Базовый", или если уровень снижается для базы данных категории "Премиум" с поддержкой выполняющейся в памяти OLTP. Эти же рекомендации будут применимы при изменении на более низкую ценовую категорию для пула эластичных баз данных либо при перемещении баз данных с поддержкой технологий обработки в оперативной памяти в пул эластичных баз данных уровня "Стандартный" или "Базовый".
...
*Понижение до более низкого уровня "Премиум"*: данные в оптимизированных для памяти таблицах должны умещаться в хранилище выполняющейся в памяти OLTP, которое связано с ценовой категорией базы данных или доступно для использования в пуле эластичных баз данных. Если вы пытаетесь понизить ценовую категорию базы данных или переместить базу данных в пул, в котором недостаточно места для хранилища выполняющейся в памяти OLTP, операция завершится ошибкой.

### <a name="columnstore-indexes"></a>Индексы columnstore

*Переход на уровни "Базовый" и "Стандартный"*: индексы columnstore не поддерживаются базами данных уровней "Стандартный" или "Базовый". Когда база данных понижается до уровней "Стандартный" или "Базовый", индексы сolumnstore становятся недоступными. Если вы используете кластеризованный индекс сolumnstore, вся таблица становится недоступной.

Прежде чем понизить базу данных до уровня "Стандартный" или "Базовый", удалите все кластеризованные индексы columnstore.

*Понижение до более низкого уровня "Премиум"*: этот сценарий будет успешным, только если общий размер базы данных не превышает максимальный размер, установленный для баз данных в новой ценовой категории, или он соответствует доступному объему хранилища в пуле эластичных баз данных. Индексы columnstore особого влияния не оказывают.
