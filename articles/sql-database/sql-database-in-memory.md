...
- Предварительную версию компонента In-Memory OLTP нельзя использовать с базами данных, входящими в пул эластичных баз данных.
 - Чтобы переместить базу данных, которая содержит или содержала объекты In-Memory OLTP, выполните описанные ниже действия.
  ...
  - 2. Изменить уровень обслуживания базы данных на стандартный (*в настоящее время существует проблема, препятствующая перемещению баз данных Premium, которые в прошлом содержали объекты In-Memory OLTP, в пул эластичных БД; команда Azure по базам данных активно работает над решением этой проблемы).
