...
# Настройка правил брандмауэра для базы данных SQL Azure: обзор
...
- На [классическом портале](http://go.microsoft.com/fwlink/p/?LinkID=161793) на вкладке **Настройка** сервера в разделе **Разрешенные службы** выберите значение **Да** для пункта **Службы Microsoft Azure**.

## Создание первого правила брандмауэра уровня сервера

Первый параметр брандмауэра уровня сервера можно создать на [портале Azure](https://portal.azure.com/) или программным путем с помощью REST API либо Azure PowerShell. Последующие правила брандмауэра уровня сервера можно создавать и контролировать с помощью этих методов, а также посредством Transact-SQL. Для повышения производительности правила брандмауэра на уровне сервера временно кэшируются на уровне базы данных. Сведения об обновлении кэша см. в статье [DBCC FLUSHAUTHCACHE](https://msdn.microsoft.com/library/mt627793.aspx). Дополнительные сведения о правилах брандмауэра уровня сервера см. в статье [Практическое руководство. Настройка брандмауэра базы данных SQL Azure с помощью портала Azure](sql-database-configure-firewall-settings.md).
