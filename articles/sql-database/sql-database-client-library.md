<properties 
   pageTitle="Создание базы данных SQL и управление ею с помощью библиотеки базы данных SQL Azure для .NET" 
   description="В этой статье показано, как создавать базы данных SQL Azure и управлять ими с помощью библиотеки базы данных SQL Azure для .NET." 
...
# Создание базы данных SQL и управление ею с помощью библиотеки базы данных SQL Azure для .NET
...
Эта статья содержит команды для выполнения многих задач управления базой данных SQL Azure с помощью C#. Код разбит на отдельные фрагменты для ясности, и пример консольного приложения объединяет все команды в нижней части этой статьи.

Библиотека базы данных SQL Azure для .NET предоставляет API на основе [диспетчера ресурсов Azure](resource-group-overview.md), который создает оболочку для [API REST базы данных SQL на основе диспетчера ресурсов](https://msdn.microsoft.com/library/azure/mt163571.aspx). Эта клиентская библиотека следует общему шаблону для клиентских библиотек на основе диспетчера ресурсов.


Диспетчер ресурсов требует групп ресурсов и проверки подлинности с помощью [Azure Active Directory](https://msdn.microsoft.com/library/azure/mt168838.aspx)(AAD).
...
Если у вас еще нет подписки Azure, щелкните **БЕСПЛАТНАЯ ПРОБНАЯ ВЕРСИЯ** в верхней части этой страницы. После оформления подписки вернитесь к этой статье. Бесплатный экземпляр Visual Studio см. на странице [Загружаемые файлы Visual Studio](https://www.visualstudio.com/downloads/download-visual-studio-vs).
...
5. Предоставьте **ИМЯ** приложения и выберите **СОБСТВЕННОЕ КЛИЕНТСКОЕ ПРИЛОЖЕНИЕ**.

    ![Добавление приложения][7]

6. Предоставьте **URI ПЕРЕНАПРАВЛЕНИЯ**. Он не обязательно должен быть фактической конечной точкой, а просто допустимым URI.
...
1. Выберите **Приложения Microsoft**.
