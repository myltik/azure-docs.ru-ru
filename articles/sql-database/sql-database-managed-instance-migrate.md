...
## <a name="choose-app-connectivity-option"></a>Выбор варианта подключения к приложению

Управляемый экземпляр полностью находится в виртуальной сети, поэтому он обеспечивает максимальный уровень изоляции и безопасности ваших данных. На следующей схеме показано несколько вариантов развертывания различных топологий приложений в Azure или в гибридной среде, независимо от того, что вы выбираете: полностью управляемую службу или модель размещения для своих интерфейсных приложений.

![топологии развертывания приложения](./media/sql-database-managed-instance-migration/application-deployment-topologies.png)

Любой из выбранных вариантов позволяет подключаться к конечной точке SQL только через частные IP-адреса. Это гарантирует оптимальный уровень изоляции ваших данных. <!--- For more information, see How to connect your application to Managed Instance.--->
...
Сведения о создании инфраструктуры виртуальной сети и управляемого экземпляра, а также о восстановлении базы данных из файла резервной копии см. в статье [Создание управляемого экземпляра базы данных SQL Azure на портале Azure](sql-database-managed-instance-tutorial-portal.md).
...
|Восстановление из службы хранилища Azure в управляемый экземпляр|[Восстановление на основе URL-адреса с помощью учетных данных SAS](sql-database-managed-instance-tutorial-portal.md#restore-the-wide-world-importers-database-from-a-backup-file)|
...
Полное руководство со сведениями о восстановлении резервной копии базы данных в управляемый экземпляр с использованием учетных данных SAS см. в статье [Create an Azure SQL Database Managed Instance in the Azure portal](sql-database-managed-instance-tutorial-portal.md) (Создание Управляемого экземпляра Базы данных SQL Azure на портале Azure).
...
- Руководство со сведениями о восстановлении из резервной копии см. в статье [Создание управляемого экземпляра базы данных SQL Azure на портале Azure](sql-database-managed-instance-tutorial-portal.md).
