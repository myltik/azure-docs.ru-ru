...
Пулы эластичных баз данных позволяют разработчикам покупать ресурсы для пула, который совместно используется несколькими базами данных, чтобы обеспечить работу отдельных баз данных в непрогнозируемые периоды использования. Вы можете настроить ресурсы для пула с использованием [модели приобретения на основе единиц DTU (предварительная версия) ](sql-database-service-tiers.md#dtu-based-purchasing-model) или [модели приобретения на основе виртуальных ядер (предварительная версия)](sql-database-service-tiers.md#vcore-based-purchasing-model-preview). Требование к ресурсу для пула определяется совокупным использованием его баз данных. Количество ресурсов, доступных для пула, определяется бюджетом разработчика. Разработчик просто добавляет базы данных в пул, задает минимальное и максимальное количества ресурсов для баз данных (минимальное и максимальное число единиц DTU или виртуальных ядер в зависимости от выбора модели ресурсов), а затем задает ресурсы для пула на основе своего бюджета. С помощью пулов разработчик может легко перевести качество своих услуг с уровня небольшой новой компании на уровень состоявшегося бизнеса с возможностями роста.
...
Если количество ресурсов для одной базы данных превышает 1,5 ресурсов, необходимых для пула, использовать эластичный пул будет более экономично.
...
Доступные уровни служб для каждой модели ресурсов см. в разделе [DTU-based purchasing model](sql-database-service-tiers.md#dtu-based-purchasing-model) (Модель приобретения на основе единиц DTU) или [vCore-based purchasing model (preview)](sql-database-service-tiers.md#vcore-based-purchasing-model-preview) (Модель приобретения на основе виртуальных ядер (предварительная версия)).
