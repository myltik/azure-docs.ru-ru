<properties 
    ...
    description="Объясняет, как управлять сегментами и перемещать данные с помощью резидентной службы с применением интерфейсов API эластичного масштабирования." 
...
# Масштабирование с применением средства разбиения и объединения гибкой базы данных

База данных SQL Azure предоставляет разнообразные средства масштабирования уровня данных приложения соответственно потребностям бизнеса. Примерами могут служить поддержка скрытых приложений или обслуживание отдельных клиентов, расширяющих базу данных до предельного размера. Возможности масштабирования позволяют:

* Пулы эластичных баз данных позволяют базам данных в пуле по запросу индивидуально увеличивать и уменьшать потребление ресурсов, сохраняя прогнозируемую общую стоимость пула в целом. Базы данных можно также добавлять или удалять из пула для поддержки новых или выбывающих клиентов. Дополнительные сведения см. в разделе [Пул эластичных баз данных Azure SQL(предварительная версия)](sql-database-elastic-pool.md). 

* Явное увеличение или уменьшение ресурсов базы данных с помощью изменения версии или уровней производительности, вручную или на основе политик (см. [Эластичность сегментов](sql-database-elastic-scale-elasticity.md))

* Изменение распределения данных между сегментами — часто в сочетании с увеличением или уменьшением общего числа баз данных для набора сегментов. Это называется разбиение и объединение и часто применяется при управлении несколькими конечными пользователями (клиентами) в пределах одного сегмента.

Последний сценарий называется **средство разбиения и объединения эластичных баз данных** и используется, когда недостаточно простых возможностей вертикального масштабирования или добавления новой базы данных для нового клиента. Средство разбиения и объединения контролирует степень уменьшения или увеличения; возможно добавление или удаление баз данных из вашего набора сегментов, а также использование средства разбиения и объединения для балансирования распределения шардлетов между ними. (Определения терминов см. в [Глоссарии эластичного масштабирования](sql-database-elastic-scale-glossary.md)).

При текущем выборе уровней баз данных SQL Azure управление емкостью может также осуществляться путем масштабирования в сторону увеличения или уменьшения емкости одной базы данных SQL Azure. Аспект масштабирования эластичной емкости в сторону увеличения или уменьшения не определяется службой разбиения и объединения — см. описание в разделе [Эластичность сегментов](sql-database-elastic-scale-elasticity.md).

 
## Новые возможности разбиения и объединения

В последних версиях средства разбиения и объединения представлены следующие усовершенствования.
...
1. Загрузите последнюю версию пакета разбиения и объединения из NuGet, как описано в разделе [Загрузка пакетов разбиения-объединения](sql-database-elastic-scale-configure-deploy-split-and-merge.md#download-the-Split-Merge-packages).
...
-    Убедитесь, что схема допускает удаления. Службе разбиения и объединения требуется возможность удаления данных из исходного сегмента после успешного копирования данных в целевой сегмент. Например, **удаление триггеров** может препятствовать запуску службы удаления в источнике данных и может привести к сбою.
