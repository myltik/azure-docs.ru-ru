...
В этой статье приведен пример кода C#, который можно использовать для запуска инструкции [CREATE ASSEMBLY](http://msdn.microsoft.com/library/ms189524.aspx) в базе данных Azure SQL. В базе данных SQL предложение FROM не принимает простой формат пути к расположению на локальном компьютере, где размещена база данных. Чтобы обойти это ограничение, закодируйте двоичные разряды DLL-файла сборки в длинную строку, содержащую шестнадцатеричное число, и укажите ее как значение предложения FROM.
...
В списке выше не упоминается:<br/>
**execute sp_configure 'clr enabled', 1,**<br/>
Этот код не нужен для базы данных Azure SQL. Но для сервера Microsoft SQL Server он нужен.
...
## Б. DLL-файл простой сборки, на который будет ссылаться функция T-SQL
...
## В. Образец кода C&#x23; для EXE-файла, который запускает инструкцию CREATE ASSEMBLY


При запуске EXE-файла, созданного из этого образца кода C#, выполняется такая последовательность действий:


1. Выполнение в командной строке EXE-файла вызывает метод **Main**.
...
Для компиляция и тестирования EXE-файла, в котором использовался образец кода, мы использовали:
...
## Г. Выполнение инструкции CREATE FUNCTION
