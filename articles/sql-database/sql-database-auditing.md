...
При использовании геореплицированных баз данных можно настроить аудит в базе данных-получателе либо путем включения аудита на **сервере-получателе**, либо включив аудит в базе данных-источнике (в этом случае у базы данных-получателя будет такая же политика аудита, как и у базы данных-источника).
...
* Аудит на уровне базы данных. Аудит на уровне базы данных для баз данных-получателей можно настроить только с помощью параметров аудита в базе данных-источнике.
   ...
   * После включения аудита больших двоичных объектов в базе данных-источнике он станет доступным в базе данных-получателе.
     ...
     >[!IMPORTANT]
     >При использовании аудита на уровне базы данных настройки хранилища для базы данных-получателя будут совпадать с настройками для базы данных-источника, что приведет к появлению трафика между регионами. Если нет необходимости использовать аудит на уровне базы данных, мы рекомендуем включить аудит только на уровне сервера на сервере-источнике и сервере-получателе и не включать аудит на уровне базы данных для всех баз данных.
<br>
