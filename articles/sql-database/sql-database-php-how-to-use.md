...
Как уже упоминалось, использование базы данных SQL очень похоже на использование сервера SQL. Как только вы установили подключение к базе данных SQL (как показано выше), вы можете использовать API **SQLSRV** или **PDO_SQLSRV** для ввода, извлечения, обновления и удаления данных.  Но существует ряд различий между базой данных SQL и SQL Server, которые могут повлиять на работу приложения. Дополнительную информацию см. в разделе [Рекомендации и ограничения (база данных SQL)][limitations].
