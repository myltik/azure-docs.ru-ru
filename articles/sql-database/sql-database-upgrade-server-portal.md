...
Если появится сообщение **Щелкните здесь, чтобы просмотреть рекомендуемые пулы эластичных баз данных для этого сервера**, щелкните его, чтобы с легкостью создать пул, оптимизированный для баз данных вашего сервера. Подробные сведения см. в статье [Вопросы цены и производительности для пула эластичных баз данных](sql-database-elastic-pool-guidance.md).
...
>[AZURE.IMPORTANT] Установите последнюю версию SQL Server Management Studio (SSMS), чтобы воспользоваться всеми преимуществами новой версии 12. [Скачайте среду SQL Server Management Studio](https://msdn.microsoft.com/library/mt238290.aspx).
...
В дополнение к наблюдению за отдельными базами данных можно отслеживать пулы эластичных баз данных, как описано в статье [Мониторинг, управление и выбор размера пула эластичных баз данных на портале Azure](sql-database-elastic-pool-manage-portal.md), или с помощью [PowerShell](sql-database-elastic-pool-powershell.md#monitoring-elastic-databases-and-elastic-database-pools).
