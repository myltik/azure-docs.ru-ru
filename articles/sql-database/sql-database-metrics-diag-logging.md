...
- **[QueryStoreRuntimeStatistics](sql-database-metrics-diag-logging.md#query-store-runtime-statistics)** содержит сведения о статистике выполнения запросов, такие как данные об использовании ЦП, длительность запросов, и т. д.
- **[QueryStoreWaitStatistics](sql-database-metrics-diag-logging.md#query-store-wait-statistics)** содержит сведения о статистике времени ожидания запросов, с помощью которых можно узнать, что ожидали запросы, например ЦП, журнал, блокировку и т. п.
...
- **[Timeouts](sql-database-metrics-diag-logging.md#timeouts-dataset)** содержит сведения о значениях времени ожидания различных типов для базы данных.
...
- **[SQLInsights](sql-database-metrics-diag-logging.md#intelligent-insights-dataset)** содержит ценную аналитику. [Дополнительные сведения об Intelligent Insights](sql-database-intelligent-insights.md)
...
Создайте новые или измените существующие параметры диагностики, выбрав целевой объект и данные телеметрии.

   ![Настройка параметров диагностики](./media/sql-database-metrics-diag-logging/diagnostics-portal.png)
...
- Для настройки нескольких подписок Azure используйте следующие команды:
...
Решение "Аналитика SQL Azure" — это иерархическая панель мониторинга, которая позволяет перемещаться по иерархии ресурсов базы данных SQL Azure. [Щелкните здесь, чтобы узнать, как использовать решение "Аналитика SQL Azure"](../log-analytics/log-analytics-azure-sql.md).
...
|ResourceGroup|Имя группы ресурсов, которой принадлежит база данных.|
|LogicalServerName_s|Имя сервера, на котором размещена база данных.|
|ElasticPoolName_s|Имя эластичного пула, которому принадлежит база данных (если она входит в пул).|
...
[Щелкните здесь, чтобы узнать больше о статистических данных времени ожидания хранилища запросов.](https://docs.microsoft.com/en-us/sql/relational-databases/system-catalog-views/sys-query-store-wait-stats-transact-sql)
...
[Сообщения об ошибках SQL Server](https://msdn.microsoft.com/en-us/library/cc645603.aspx)

### <a name="database-waits-dataset"></a>Набор данных времени ожидания базы данных
...
[Щелкните здесь, чтобы узнать больше о статистике времени ожидания базы данных.](https://docs.microsoft.com/en-us/sql/relational-databases/system-dynamic-management-views/sys-dm-os-wait-stats-transact-sql)

### <a name="timeouts-dataset"></a>Набор данных времени ожидания
...
|duration_d|Длительность блокировки в миллисекундах.|

### <a name="intelligent-insights-dataset"></a>Набор данных Intelligent Insights
[Щелкните здесь, чтобы узнать больше о формате журнала Intelligent Insights](sql-database-intelligent-insights-use-diagnostics-log.md).
