...
<p> Аудит базы данных Azure отслеживает события базы данных и записывает их в журнал аудита, расположенный в учетной записи хранения Azure. Функция аудита доступна для уровней Basic, Standard и Premium.
...
2. Перейдите к выноске настройки базы данных для аудита. Перейдите к разделу **Операции**, а затем щелкните **Аудит**, чтобы включить функцию аудита и открыть колонку настройки аудита.
...
5. Выберите **Сохранить эти параметры в качестве параметров сервера по умолчанию**, если вы хотите применить эти настройки ко всем создаваемым в дальнейшем на данном сервере базам данных и тем базам данных, у которых еще нет настроек аудита. В дальнейшем эти настройки можно переопределить, выполнив аналогичные действия. 
...
В рабочей среде вам, скорее всего, понадобится, чтобы весь трафик между базой данных и всеми приложениями и инструментами проверялся. Для этого необходимо изменить значение параметра **Безопасный доступ** с *Optional* на *Required* и сохранить политику. Если установлено значение  *Required*, к базе данных невозможно подключиться с помощью исходной строки подключения и необходимо использовать безопасную строку подключения.
...
1. В колонке настройки аудита (описанной в разделе настройки аудита выше) измените значение параметра **Ключ доступа к хранилищу** с *Primary* на *Secondary* и нажмите кнопку **СОХРАНИТЬ**.
![][10]
2. Перейдите к колонке настройки хранилища и **заново создайте**  *Primary Access Key*.

3. В колонке настройки аудита измените значение параметра **Ключ доступа к хранилищу** с *Secondary* на *Primary* и нажмите кнопку **СОХРАНИТЬ**.

4. В пользовательском интерфейсе хранилища **повторно создайте**  *Secondary Access Key* (для подготовки к следующему циклу обновления ключей).
...
> [AZURE.NOTE] Модуль AzureResourceManager находится в режиме предварительного просмотра. Он может не обеспечивать те же возможности управления, что и модуль Azure.

 Режим [Диспетчер ресурсов Azure](https://msdn.microsoft.com/library/dn654592.aspx) включается посредством выполнения командлета Switch-AzureMode (`Switch-AzureMode AzureResourceManager`). При работе в режиме диспетчера ресурсов Azure выполните команду `Get-Command *AzureSql*`, чтобы получить список доступных командлетов.
