...
- [Сообщения об ошибках (база данных SQL Azure)](http://msdn.microsoft.com/library/azure/ff394106.aspx). Здесь в разделе **Ошибки, связанные с потерей подключения** приводится список временных ошибок, после которых должна автоматически выполняться повторная попытка подключения.
...
- [Устранение неполадок подключения к базе данных Azure SQL](http://support.microsoft.com/kb/2980233/)
...
Примеры кода приведены для клиентов, работающих в среде Windows и Linux.


**Общие примеры:** есть несколько примеров кода для различных языков программирования, включая PHP, Python, Node.js и .NET CSharp. Кроме того, приведены примеры для клиентов, работающих в среде Windows или Linux.
...
**Логика повторных попыток:** имеются примеры сложного кода, необходимые для обработки временных ошибок с помощью логики автоматического повтора.
