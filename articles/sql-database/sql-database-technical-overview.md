...
> ВАЖНО! В настоящее время Управляемый экземпляр базы данных SQL находится на стадии предварительной версии и доступен на одном уровне обслуживания. Дополнительные сведения см. в статье [What is a Managed Instance (preview)?](sql-database-managed-instance.md) (Сведения об Управляемом экземпляре (предварительная версия)).
>
