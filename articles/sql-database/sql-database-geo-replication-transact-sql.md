...
> [!NOTE]
> Если вы используете активную георепликацию (считываемые вторичные базы данных) для аварийного восстановления, вам необходимо настроить группу отработки отказа для всех баз данных в приложении, чтобы обеспечить автоматическую и прозрачную отработку отказа. Эта функция предоставляется в предварительной версии. Дополнительные сведения см. в статье [Обзор: активная георепликация для базы данных SQL](sql-database-geo-replication-overview.md).
> 
> 
