...
description: "Группы автоматической отработки отказа и активная георепликация позволяют настроить четыре реплики базы данных в любом центре обработки данных Azure, а также автоматическую отработку отказа в случае сбоя."
...
Обратите внимание на то, что отработка отказа включает в себя обновление DNS-записи, поэтому клиентские подключения будут перенаправляться на новый сервер-источник только после обновления кэша DNS на стороне клиента.
