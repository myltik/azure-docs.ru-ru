...
###Портал Azure
Портал Azure позволяет прекратить связь непрерывной копии с базой данных-получателем, восстановленной с помощью георепликации.
...
> [AZURE.NOTE] При восстановлении базы данных создается новая база данных. Важно убедиться, что на сервере, куда выполняется восстановление, имеется достаточный объем DTU для новой базы данных. Можно [обратиться в службу поддержки](https://azure.microsoft.com/blog/azure-limits-quotas-increase-requests/) с просьбой увеличить квоту.

###Портал Azure (восстановление в автономную базу данных)
Чтобы восстановить базу данных SQL с помощью геовосстановления на портале Azure, выполните указанные ниже действия.
...
###Портал Azure (восстановление в пул эластичных баз данных)
Чтобы восстановить базу данных SQL с использованием географического восстановления в пул эластичных баз данных с помощью портала, следуйте приведенным ниже инструкциям.

1. Войдите на [портал Azure](https://portal.Azure.com).
2. В левой части экрана выберите **Обзор**, а затем — **Пулы эластичных БД SQL**.
3. Выберите пул для географического восстановления базы данных.
4. В верхней части колонки эластичного пула выберите **Создать базу данных**.
5. Выберите **РЕЗЕРВНАЯ КОПИЯ** в качестве источника, затем выберите избыточную резервную копию геообъекта, из которой вы хотите восстановить базу данных.
6. Укажите остальные свойства базы данных и нажмите кнопку **Создать**.
7. Начнется процесс восстановления базы данных, который можно отслеживать с помощью **УВЕДОМЛЕНИЙ** в левой части экрана.

###PowerShell 
> [AZURE.NOTE] В настоящее время географическое восстановление с помощью PowerShell поддерживает только восстановление в автономные базы данных. Для географического восстановления в пул эластичных баз данных, пожалуйста, пользуйтесь [порталом Azure](https://portal.Azure.com).
