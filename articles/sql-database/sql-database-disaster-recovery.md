...
Убедитесь, что все учетные данные, используемые приложением, существуют на сервере с восстановленной базой данных. Дополнительные сведения см. в статье [Настройки безопасности для стандартной и активной георепликации](sql-database-geo-replication-security-config.md).

>[AZURE.NOTE] Необходимо настроить и протестировать правила брандмауэра и имена для входа (и их разрешения) на сервере во время отработки аварийного восстановления. Эти объекты уровня сервера и их конфигурации могут быть недоступны во время сбоя. Дополнительные сведения об отработке аварийного восстановления см. [здесь](sql-database-disaster-recovery-drills.md).
