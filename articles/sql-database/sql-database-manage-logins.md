...
Для работы с базой данных SQL Майкрософт рекомендует использовать пользователей автономной базы данных. Дополнительные сведения см. в статье [Пользователи автономной базы данных — создание переносимой базы данных](https://msdn.microsoft.com/library/ff929188.aspx).
...
Чтобы подключиться к базе данных SQL Microsoft Azure с помощью созданных учетных данных, вам сначала нужно предоставить всем учетным данным разрешения уровня базы данных. Для этого воспользуйтесь командой ``CREATE USER``. Дополнительные сведения см. в статье [Предоставление учетным данным разрешений уровня базы данных](https://msdn.microsoft.com/library/ee336235.aspx#DatabasePerms).

Некоторые средства реализуют поток табличных данных (TDS) по-разному. Поэтому вам, возможно, нужно будет добавить имя сервера базы данных SQL Azure к учетным данным в строке подключения. Для этого используйте нотацию ``<login>@<server>``. Кроме того, нужно разделить учетные данные и имя сервера базы данных SQL Azure символом ``@``. Например, если ваши учетные данные — **login1**, а полное имя сервера базы данных SQL Azure — **servername.database.windows.net**, параметр username для строки подключения будет выглядеть так: ****login1@servername**. Это правило накладывает ограничения на текст, который можно выбрать для имени входа. Дополнительные сведения см. в статье [CREATE LOGIN (Transact-SQL)](https://msdn.microsoft.com/library/ms189751.aspx).

## Предоставление разрешений уровня сервера для имени входа

Для управления защитой на уровне сервера могут использоваться учетные данные, отличные от объекта серверного уровня. Для таких учетных данных база данных SQL Microsoft Azure предлагает две роли безопасности: **loginmanager** (для создания учетных данных) и **dbmanager** (для создания баз данных). Эти роли базы данных можно назначать только пользователям из базы данных **master**.
...
Как и предопределенная роль сервера **securityadmin** для локального экземпляра SQL Server, роль базы данных **loginmanager** в базе данных SQL Azure имеет разрешение на создание учетных данных входа. Новые учетные данные могут создаваться только из учетных данных объекта серверного уровня (создается в процессе подготовки) или членами роли базы данных **loginmanager**.

### Роль dbmanager

Роль базы данных **dbmanager** в базе данных SQL Azure похожа на предопределенную роль сервера **dbcreator** для локального экземпляра SQL Server. Новые базы данных могут создаваться только из учетных данных объекта серверного уровня (создается в процессе подготовки) или членами роли базы данных **dbmanager**. Если пользователь является членом роли базы данных **dbmanager**, он может создать базу данных с помощью команды ``CREATE DATABASE`` базы данных SQL Azure. Эту команду необходимо выполнить в базе данных master. Дополнительные сведения см. в статье [CREATE DATABASE (Transact-SQL)](https://msdn.microsoft.com/library/dn268335.aspx).

### Назначение ролей уровня сервера Базы данных SQL

Чтобы создать учетные данные и связанного пользователя, который может создавать базы данных или другие учетные данные, выполните следующие действия. 1. Подключитесь к базе данных **master** с помощью учетных данных объекта серверного уровня (создается в процессе подготовки) или учетных данных существующего члена роли базы данных **loginmanager**. 2. Создайте учетные данные, используя команду ``CREATE LOGIN``. Дополнительные сведения см. в статье [CREATE LOGIN (Transact-SQL)](https://msdn.microsoft.com/library/ms189751.aspx). 3. С помощью команды ``CREATE USER`` создайте нового пользователя для этих учетных данных в базе данных master. Дополнительные сведения см. в статье [CREATE USER (Transact-SQL)](https://msdn.microsoft.com/library/ms173463.aspx). 4. Используйте хранимую процедуру ``sp_addrolememeber``, чтобы добавить нового пользователя в роль базы данных **dbmanager**, роль базы данных loginmanager или в обе эти роли.
...
Все учетные данные должны быть созданы в базе данных **master**. После создания имени входа для него можно создать учетную запись пользователя в другой базе данных. Кроме того, база данных SQL Azure поддерживает роли базы данных так же, как и локальный экземпляр SQL Server.
...
2. Создайте учетные данные с помощью команды ``CREATE LOGIN``. Дополнительные сведения см. в статье [CREATE LOGIN (Transact-SQL)](https://msdn.microsoft.com/library/ms189751.aspx). Проверка подлинности Windows не поддерживается.
3. Создайте базу данных с помощью команды ``CREATE DATABASE``. Дополнительные сведения см. в статье [CREATE DATABASE (Transact-SQL)](https://msdn.microsoft.com/library/dn268335.aspx).
4. Установите подключение к новой базе данных (с помощью имени входа, создавшего эту базу данных).
5. Создайте пользователя в новой базе данных, используя команду ``CREATE USER``. Дополнительные сведения см. в статье [CREATE USER (Transact-SQL)](https://msdn.microsoft.com/library/ms173463.aspx).

В следующем примере кода показано, как создать учетные данные с именем **login1** и базу данных с именем **database1**:
...
Просмотреть учетные данные и базы данных на сервере базы данных SQL Azure можно с помощью соответствующих представлений ``sys.sql_logins`` и ``sys.databases`` базы данных master. В следующем примере показано, как отобразить список всех имен входа и баз данных на сервере Базы данных SQL Azure.
