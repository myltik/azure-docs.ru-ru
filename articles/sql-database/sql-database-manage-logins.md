...
Одной учетной записи Azure Active Directory также можно предоставить права администратора. Эта учетная запись может быть отдельным пользователем Azure AD или группой Azure AD, в которую входит несколько пользователей Azure AD. Администратора Azure AD необходимо настраивать, только если вы хотите использовать проверку подлинности Windows для учетных записей Azure AD при подключении к базе данных SQL. Дополнительные сведения о настройке доступа Azure Active Directory см. в статье [Подключение к базе данных SQL или хранилищу данных SQL c использованием проверки подлинности Azure Active Directory](sql-database-aad-authentication.md).

### Настройка брандмауэра
Если настроить брандмауэр на уровне сервера, учетная запись подписчика базы данных SQL Azure и учетная запись Azure Active Directory смогут подключаться к виртуальной базе данных master и всем пользовательским базам данных. Брандмауэр серверного уровня можно настроить на портале. После подключения также можно настроить дополнительные правила брандмауэра уровня сервера с помощью инструкции Transact-SQL [sp\_set\_firewall\_rule](https://msdn.microsoft.com/library/dn270017.aspx). Дополнительные сведения о настройке брандмауэра см. в статье [Практическое руководство. Настройка брандмауэра базы данных SQL Azure с помощью портала Azure](sql-database-configure-firewall-settings.md).
...
Учетные записи администраторов могут создавать новые базы данных. Чтобы создать дополнительную учетную запись, которая может создавать базы данных, необходимо создать пользователя в базе данных master и добавить его в специальную роль базы данных **dbmanager**. Пользователь может быть пользователем автономной базы данных или пользователем с именем входа SQL Server в виртуальную базу данных master.

1.	С помощью учетной записи администратора подключитесь к виртуальной базе данных master.
2.	Необязательный шаг. Создайте имя входа для проверки подлинности SQL Server с помощью инструкции [CREATE LOGIN](https://msdn.microsoft.com/library/ms189751.aspx). Пример инструкции:

     ```
     CREATE LOGIN Mary WITH PASSWORD = '<strong_password>';
     ```

     > [AZURE.NOTE] Создавая имя входа автономной базы данных, используйте надежный пароль. Дополнительные сведения см. в разделе [Надежные пароли](https://msdn.microsoft.com/library/ms161962.aspx).
...
При желании можно выполнить те же действия (создание имени входа и пользователя и назначение пользователю роли **loginmanager**), чтобы разрешить пользователю создавать имена для входа в виртуальную базу данных master. В большинстве случаев это не обязательно, так как корпорация Microsoft рекомендует использовать пользователей автономной базы данных, подлинность которых проверяется на уровне базы данных, а не пользователей с именами для входа. Дополнительные сведения см. в статье [Пользователи автономной базы данных — создание переносимой базы данных](https://msdn.microsoft.com/library/ff929188.aspx).
...
Изначально создавать пользователей может только один администратор или владелец базы данных. Чтобы разрешить другим пользователям создавать новых пользователей, предоставьте выбранному пользователю разрешение `ALTER ANY USER` с помощью такой инструкции:
...
## Группы и роли
При эффективном управлении доступом используются разрешения, назначенные группам и ролям, а не отдельным пользователям. Например, при использовании проверки подлинности Azure Active Directory следуйте такой процедуре.
...
Роли базы данных могут быть встроенными ролями, например **db\_owner**, **db\_ddladmin**, **db\_datawriter**, **db\_datareader**, **db\_denydatawriter** и **db\_denydatareader**. Роль **db\_owner** обычно используется для предоставления полных прав ограниченному числу пользователей. Другие фиксированные роли можно использовать для быстрого получения простых баз данных в разработке, но не рекомендуется использовать их для большинства рабочих баз данных. Например, фиксированная роль базы данных **db\_datareader** предоставляет доступ на чтение для каждой таблицы в базе данных, который обычно является обязательным. Гораздо лучше использовать инструкцию [CREATE ROLE](https://msdn.microsoft.com/library/ms187936.aspx), чтобы создать пользовательские роли базы данных, и осторожно предоставлять каждой роли наименьший набор разрешений, необходимых для работы. Если пользователь является участником нескольких ролей, ему будут предоставлены разрешения всех этих ролей.
