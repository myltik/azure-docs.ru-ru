...
Пользователи Azure работают с базой данных SQL с использованием контроля доступа на основе ролей Azure, а API REST диспетчера ресурсов Azure получают разрешения из соответствующих ролей Azure. Действия, соответствующие членам роли Azure, выполняются для них ядром СУБД. Они не затрагиваются моделью разрешений ядра СУБД и не рассматриваются в этом разделе. Дополнительные сведения см. в статье [RBAC: встроенные роли]( https://azure.microsoft.com/documentation/articles/role-based-access-built-in-roles/#sql-db-contributor).
