...
Так как [базы данных SQL Azure уровня Web и Business устарели](sql-database-web-business-sunset-faq.md), самое время начать планировать их переход на [уровни служб «Базовый», «Стандартный» или «Премиум»](sql-database-service-tiers.md).


> [AZURE.IMPORTANT] Процесс перехода базы данных версии Web или Business на новый уровень служб не требует перевода базы данных в автономный режим. В процессе обновления база данных не прекращает работу и остается доступной.
...
Во время изменения уровня служб баз данных Web или Business либо при их обновлении до Базы данных SQL версии 12 для каждой из этих баз данных предлагается рекомендуемая ценовая категория.

В зависимости от конкретной используемой среды служба может порекомендовать обновить некоторые или все базы данных до [пула эластичных баз данных](sql-database-elastic-pool.md).

Чтобы просмотреть рекомендуемые уровни служб для устаревших баз данных, можно воспользоваться [порталом Azure](https://portal.azure.com) или PowerShell. Пошаговые инструкции см. в следующих статьях:

- [Обновление до версии 12 Базы данных SQL (портал Azure)](sql-database-v12-upgrade.md)
- [Обновление до версии 12 Базы данных SQL (PowerShell)](sql-database-upgrade-server.md)


Следует отметить, что База данных SQL не привязана к конкретному уровню служб или уровню производительности. По мере изменения требований к базе данных вы можете с легкостью переходить с одних доступных уровней служб и уровней производительности на другие. Для Баз данных SQL уровней «Базовый», «Стандартный» и «Премиум» выставляются счета за почасовое использование. Каждую базу данных можно масштабировать вверх или вниз 4 раза в течение 24 часов. Это значит, что вы можете отрегулировать уровень служб и уровень производительности таким образом, чтобы обеспечить необходимую производительность, набор возможностей и стоимость базы данных в зависимости от требований приложения и рабочей нагрузки. Также это означает, что операция оценки и изменения уровня служб и уровня производительности базы данных (увеличение и уменьшение) выполняется периодически, и поэтому она должна быть частью планового обслуживания и процедур настройки производительности.
 
Для получения дополнительных сведений о различиях между Web/Business и новыми уровнями служб и о миграции скачайте [Подробную инструкцию по миграции баз данных уровня Web и Business](http://download.microsoft.com/download/3/C/9/3C9FF3D8-E702-4F5D-A38C-5EFA1DBA73E6/Azure_SQL_Database_Migration_Cookbook.pdf).
...
Например, подумайте, как долго должны храниться резервные копии, нужны ли вам функции [стандартной или активной георепликации](sql-database-business-continuity.md), какой общий максимальный размер базы данных вам необходим и т. д. Эти требования позволяют определить оптимальный для вас минимальный уровень служб.
...
| Статья | Описание |
|:--|:--|
|[Уровни служб и уровни производительности в базе данных SQL Azure](sql-database-service-tiers.md)| Общие сведения, метрики и возможности для каждого уровня служб (а также информация о том, как отслеживать использование баз данных с помощью классического портала и динамических административных представлений). |
|[Обеспечение непрерывности работы базы данных SQL Azure](sql-database-business-continuity.md)|Информация о функциях обеспечения непрерывности бизнес-процессов и аварийного восстановления (восстановление на момент времени, геовосстановление, георепликация), доступных для разных уровней служб.|
|[Цены на базы данных SQL](https://azure.microsoft.com/pricing/details/sql-database/)|Подробная информация о стоимости разных уровней служб и уровней производительности.|
...
Служба базы данных SQL Azure предоставляет информацию о сравнительно новом уровне служб и уровне производительности для вашей базы данных серии Web или Business на классическом портале и в системных представлениях.
...
Просмотрите общий обзор процентных показателей использования единиц передачи данных (DTU) на портале Azure, а затем перейдите к системным представлениям, чтобы просмотреть подробную информацию.

Кроме того, на портале Azure можно просмотреть уровень служб для базы данных Web или Business, рекомендуемый при обновлении сервера до версии 12 Базы данных SQL Azure.

### Как просмотреть рекомендуемый уровень служб на портале Azure
При обновлении сервера до версии 12 Базы данных SQL на портале предварительной версии Azure предлагается соответствующий уровень служб для базы данных уровня Web или Business. Рекомендация основана на результате анализа исторических данных потребления ресурсов базы данных.

**Новый портал управления**

1. Войдите на [портал Azure](https://portal.azure.com) и перейдите к серверу, содержащему базу данных уровня Web или Business.
...
1. Войдите на [классический портал](https://manage.windowsazure.com) и перейдите к существующей базе данных уровня Web или Business.
...
| :---| :---|
| [Классический портал Azure](https://manage.windowsazure.com) | Щелкните вкладку **МАСШТАБ** на странице панели мониторинга вашей базы данных. |
...
## 6. Мониторинг обновления до нового уровня служб и производительности
База данных SQL Azure содержит информацию о ходе операций управления (например, CREATE, ALTER, DROP), выполняемых в базе данных в динамическом административном представлении sys.dm\_operation\_status в главной базе данных логического сервера, на котором размещена ваша текущая база данных [см. документацию по sys.dm _operation_status.](http://msdn.microsoft.com/library/azure/dn270022.aspx). Воспользуйтесь динамическим административным представлением состояния операции, чтобы узнать о ходе выполнения обновления базы данных. Приведенный ниже образец запроса содержит все операции управления, выполняемые для базы данных.
...
Если обновление выполнялось с помощью классического портала, отображается оповещение портала о выполняемой операции.
...
- **Оповещения:** настройте оповещения на классическом портале Azure, чтобы получать уведомления о приближении показателя потребления DTU для обновленной базы данных к заданному максимальному уровню. На классическом портале Azure можно настроить оповещения базы данных для различных метрик производительности, например DTU, ЦП, ввода-вывода и журнала. 
