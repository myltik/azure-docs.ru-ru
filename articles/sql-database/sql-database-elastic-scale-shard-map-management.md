...
Используйте [клиентскую библиотеку эластичных баз данных](sql-database-elastic-database-client-library.md) для управления сегментированными приложениями. В среде сегментированных баз данных [**карта сегментов**](sql-database-elastic-scale-glossary.md) хранит информацию, позволяющую приложению связываться с корректной базой данных согласно значению **ключа сегментирования**. Для управления картами сегментов важно понимать их структуру.
...
**Сегменты** содержат **шардлеты**, и сопоставление шардлетов сегментам производится с помощью карты сегментов. **Карта сегментов по списку** представляет сопоставление между отдельными значениями ключа, определяющими шардлеты, и базами данных, обслуживающими сегменты. **Сопоставления списков** являются явными (например, база данных A, раздел 1 соответствует), и различные значения ключа могут быть сопоставлены с одной базой данных (значения ключа 3 и 6 ссылки базы данных B).
...
Каждая из приведенных выше таблиц является концептуальным примером объекта **ShardMap**. Каждая строка представляет упрощенный пример отдельного **PointMapping** (для карты сегментов по списку) или **RangeMapping** (для карты сегментов по диапазонам) объекта.
