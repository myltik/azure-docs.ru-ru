...
Исходный код в этом разделе является сокращенной версией более длинного файла `Program.cs`, который приводится выше. Вся логика повторных попыток и обработка `Exception` в нем удалена.


Сокращенная версия позволяет упростить отображение вызовов ADO.NET, зная, что обычно они работают. Обычно временные сбои не возникают и `Exception` не выдается. Точно так же парашютисту обычно не требуется запасной парашют.
