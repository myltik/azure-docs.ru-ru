<properties
	pageTitle="Руководство по базам данных SQL: защита конфиденциальных данных с помощью функции постоянного шифрования | Microsoft Azure"
	description="Сведения о настройке защиты конфиденциальных данных в базе данных SQL всего за несколько минут."
	keywords="руководство по базам данных SQL, шифрование данных в базе данных SQL"	
...
# Руководство по базам данных SQL: защита конфиденциальных данных с помощью функции постоянного шифрования (в хранилище сертификатов Windows)


Постоянное шифрование — это новая технология шифрования, применяемая в базе данных SQL Azure и SQL Server. Она защищает конфиденциальные данные, которые хранятся на сервере в неактивном состоянии, перемещаются между клиентом и сервером и просто используются, предотвращая их доступность в виде открытого текста внутри системы базы данных. К зашифрованным данным можно получить доступ только через клиентские приложения и серверы приложений, у которых есть доступ к ключам. Дополнительные сведения см. в статье [Always Encrypted (Database Engine)](https://msdn.microsoft.com/library/mt163865.aspx) (Постоянное шифрование (ядро СУБД)).

В этом руководстве показано, как защитить конфиденциальные данные, настроив функцию постоянного шифрования в базе данных SQL всего за несколько минут с помощью SQL Server Management Studio.
...
- Учетная запись Azure и подписка (еще до начала). Если у вас ее нет, зарегистрируйтесь, чтобы [воспользоваться бесплатной пробной версией](http://azure.microsoft.com/pricing/free-trial/).
- [SQL Server Management Studio](https://msdn.microsoft.com/library/mt238290.aspx) версии 13.0.700.242 или более поздней.
- [NET Framework](https://msdn.microsoft.com/library/w0x726c2.aspx) версии 4.6 или более поздней (на клиентском компьютере).
...
3. Создайте **пустую** базу данных с именем **Clinic** на новом или существующем сервере. Подробные инструкции по созданию базы данных на портале Azure см. в разделе [Create a SQL database in minutes](sql-database-getting-started.md) (Создание базы данных SQL за несколько минут).
...
4. Выполните действия на странице **Настройка главного ключа**.
...
Чтобы активировать функцию постоянного шифрования, необходимо добавить ключевое слово **Column Encryption Setting** и установить для него значение **Enabled**.
...
- [Always Encrypted (client Development)](https://msdn.microsoft.com/library/mt147923.aspx) (Постоянное шифрование (разработка клиентских приложений))
- [Transparent Data Encryption](https://msdn.microsoft.com/library/bb934049.aspx) (Прозрачное шифрование данных)
- [SQL Server Encryption](https://msdn.microsoft.com/library/bb510663.aspx) (Шифрование SQL Server)
- [Always Encrypted Wizard](https://msdn.microsoft.com/library/mt459280.aspx) (Мастер настройки постоянного шифрования)
- [Блог по функции постоянного шифрования](http://blogs.msdn.com/b/sqlsecurity/archive/tags/always%20encrypted/)
