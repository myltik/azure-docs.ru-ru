...
Ниже представлен общий рабочий процесс переноса базы данных SQL Server в отдельную базу данных или базу данных в составе пула с помощью этого метода. Перенос в управляемый экземпляр описывается в разделе [Перенос экземпляра SQL Server в Управляемый экземпляр Базы данных SQL Azure](sql-database-cloud-migrate.md#migration-to-azure-sql-database-managed-instance).
