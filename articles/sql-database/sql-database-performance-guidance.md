...
База данных SQL Microsoft Azure поддерживает три [уровня служб](sql-database-service-tiers.md): Basic, Standard и Premium. Все они строго изолируют ресурс для Базы данных SQL Azure и гарантируют прогнозируемую производительность. Пропускная способность, гарантируемая для базы данных, возрастает от уровня Базовый до уровней Стандартный и Премиум.

>[AZURE.NOTE] Поддержка уровней служб "Бизнес" и "Интернет" была прекращена в сентябре 2015 г. Дополнительные сведения см. в статье [Вопросы и ответы по остановке Web Edition и Business Edition](https://msdn.microsoft.com/library/azure/dn741330.aspx). Подробные сведения об обновлении существующих баз данных Web и Business до новых уровней служб см. в разделе [Обновление баз данных SQL уровня Web или Business до новых уровней служб](sql-database-upgrade-server-portal.md).
...
Кроме того, корпорация Майкрософт добавила несколько автоматических функций управления Базой данных SQL Azure, например автоматический режим высокой доступности и встроенное управление.
...
Есть три уровня служб: Базовый, Стандартный и Премиум. У каждого уровня служб есть один или несколько уровней производительности, обеспечивающих предсказуемую работу баз данных. Их мощность исчисляется в [единицах пропускной способности базы данных (DTU)](sql-database-technical-overview.md#understand-dtus).
...
- **Базы данных с большим количеством параллельных запросов**. Приложениям, обслуживающим множество пользователей одновременно, таким как веб-сайты с умеренным объемом трафика или приложения для различных подразделений, требующие большого объема ресурсов, подойдет уровень служб Стандартный.
...
Следующие разделы предоставляют дополнительные сведения о каждой области в предыдущей таблице.

### Максимальный размер базы данных

**Максимальный размер базы данных** — это просто ограничение размера базы данных в гигабайтах.

### DTU

**DTU** — это единицы транзакций базы данных. Это единица измерения в Базе данных SQL, представляющая относительную производительность баз данных в зависимости от фактического показателя — количества транзакций базы данных. Единица измерения включает набор операций, характерных для запроса по обработке транзакций в режиме реального времени (OLTP), и измеряет количество транзакций, которые могут быть совершены в течение одной секунды в условиях полной загрузки. Чтобы получить дополнительные сведения о DTU, см. раздел [Общие сведения о единицах транзакций базы данных](sql-database-technical-overview.md#understand-dtus). Дополнительные сведения об измерении DTU см. в разделе [Обзор производительности](sql-database-benchmark-overview.md).

### Восстановление состояния системы в определенной временной точке

**Восстановление состояния системы в определенной временной точке** — возможность восстановления состояния базы данных на более ранний момент времени. Уровень службы определяет, состояние какой давности вы можете использовать. Дополнительные сведения см. в разделе [Восстановление до точки во времени](sql-database-recovery-using-backups.md#point-in-time-restore).

### Аварийное восстановление

**Аварийное восстановление** — это возможность восстановления после сбоя основной Базы данных SQL.

*Геовосстановление* доступно для всех уровней служб и не требует дополнительной оплаты. В случае сбоя для восстановления базы данных в любом регионе Azure можно использовать самую последнюю географически избыточную резервную копию.

[Активная георепликация](sql-database-geo-replication-overview.md) предоставляет аналогичные функции аварийного восстановления, но с более низкой целевой точкой восстановления (RPO). Например, с геовосстановлением целевая точка восстановления — менее одного часа (другими словами, резервная копия сделана не позже часа назад). Но в случае активной георепликации целевая точка восстановления — менее 5 секунд.

Дополнительные сведения см. в разделе [Обзор обеспечения непрерывности работы](sql-database-business-continuity.md).

### Максимальное хранилище OLTP в памяти
**Максимальное хранилище In-Memory OLTP** — это максимальный объем дискового пространства, доступный в [предварительной версии In-Memory OLTP](sql-database-in-memory.md) для баз данных уровня Премиум. Его также иногда называют *хранилищем XTP In-Memory*. Можно использовать классический портал Azure или представление **sys.dm\_db\_resource\_stats**, чтобы отслеживать использование хранилища In-Memory. Дополнительные сведения об отслеживании см. в разделе [Мониторинг хранилища In-Memory OLTP](sql-database-in-memory-oltp-monitoring.md).
...
**Максимальное число параллельных запросов** — это максимальное количество запросов пользователей или приложений, одновременно выполняющихся в базе данных. Чтобы просмотреть число одновременных запросов, выполните следующий запрос Transact-SQL в Базе данных SQL:
...
Обратите внимание, что это только моментальный снимок в один момент времени. Для лучшего понимания рабочей нагрузки потребуется собрать большое количество примеров за период временем, чтобы понять требования к параллельным запросам.

### Максимальное количество одновременных входов

**Максимальное количество одновременных входов** — это ограничение на количество пользователей или приложений, которые одновременно пытаются войти в базу данных. Обратите внимание, что, даже если эти клиенты используют ту же строку подключения, служба проверяет подлинность каждого входа. Поэтому если десять пользователей одновременно подключаются к базе данных с тем же именем пользователя и паролем, это будет десять одновременных процессов входа. Это ограничение применяется только на время входа и проверки подлинности. Поэтому если те же десять пользователей последовательно подключатся к базе данных, количество одновременных входов никогда не будет больше одного.
...
**Максимальное число сеансов** — это максимальное число одновременных подключений к базе данных. При входе пользователя сеанс устанавливается и остается активным, пока пользователь не выйдет или не истечет время сеанса. Чтобы просмотреть текущее число активных сеансов, выполните следующий запрос Transact-SQL в Базе данных SQL:

	SELECT COUNT(*) AS [Sessions]
	FROM sys.dm_exec_connections

При анализе рабочей нагрузки локального SQL Server измените запрос, чтобы сосредоточиться на определенной базе данных. Это поможет определить возможные потребности в сеансах для этой базы данных, если вы собирались переместить ее в Базу данных SQL Azure.
...
>[AZURE.NOTE] Можно также использовать классический портал Azure для просмотра объема потребления ресурсов. Например, см. раздел [Уровни служб. Мониторинг производительности](sql-database-service-tiers.md#monitoring-performance).
...
>[AZURE.NOTE] Некоторые столбцы **sys.resource\_stats** были изменены в текущей версии 12 баз данных, поэтому образцы запросов в следующих примерах могут вызывать ошибки. Будущие обновления этого раздела предоставят новые версии запросов, которые устраняют эту проблему.
...
>[AZURE.NOTE] Перед использованием динамических административных представлений для поиска отсутствующих индексов сначала ознакомьтесь с разделом об [анализе производительности запросов и помощнике по базам данных SQL](#query-performance-insight-and-index-advisor).
...
Например, если база данных содержит сведения о заказчике, заказе и цене (как можно увидеть в стандартном примере базы данных Northwind, который используется для SQL Server), эти данные можно разбить на несколько баз, группируя все сведения и обеспечивая целостность данных. Приложение должно разбивать заказчиков по разным базам данных и эффективно распределять нагрузку. В результате можно не только не превышать существующее ограничение максимального размера базы данных, но и обрабатывать в базе данных SQL Azure рабочие нагрузки, которые существенно превышают ограничения различных уровней производительности, если каждая отдельная база данных будет работать в соответствии с ограничениями DTU.
