<properties
	...
	description="Эта статья содержит инструкции, которые помогут определить, подойдет ли тот или иной уровень служб для вашего приложения, а также рекомендации по настройке приложения для эффективного использования возможностей базы данных SQL Azure."
...
База данных SQL Microsoft Azure поддерживает три [уровня служб](sql-database-service-tiers.md): Basic, Standard и Premium. Все они строго изолируют ресурс для Базы данных SQL Azure и гарантируют прогнозируемую производительность. Производительность измеряется в DTU. Общие сведения о DTU см. [здесь](sql-database-what-is-a-dtu.md). Эта статья содержит инструкции, которые помогут вам выбрать уровень служб для вашего приложения, а также рекомендации по настройке приложения для эффективного использования возможностей базы данных SQL Azure.

>[AZURE.NOTE] В ней приводятся рекомендации по производительности отдельной базы данных в базе данных SQL. Сведения о производительности пулов эластичных БД см. в статье [Когда следует использовать пул эластичных баз данных?](sql-database-elastic-pool-guidance.md). Обратите внимание, что многие рекомендации и связанные с ними преимущества, описанные в этой статье, могут быть применимы к пулам эластичных баз данных.

- **Basic**. Уровень служб Basic предназначен для обеспечения предсказуемой почасовой производительности для каждой базы данных. Значение DTU базы данных Basic позволяет предоставлять достаточные ресурсы небольшим базам данных без большого количества параллельных запросов.
...
- **Premium**. Уровень служб Premium обеспечивает предсказуемую посекундную производительность для каждой базы данных Premium. Уровень служб Premium позволяет клиентам определять размер приложения на основе пиковой загрузки базы данных и устраняет отклонения производительности, из-за которых небольшие запросы могут выполняться дольше, чем требуется, в операциях, чувствительных к задержке. Эта модель может значительно упростить разработку и проверку приложений, для которых большое значение имеют пиковая нагрузка, изменяемая производительность и задержка выполнения запросов.

Задав настройки производительности для любого уровня служб, вы можете платить только за потребляемые ресурсы с [учетом](sql-database-scale-up.md) изменений рабочей нагрузки. Например, если рабочая нагрузка базы данных повышается в начале учебного года, то можно повысить уровень производительности на этот период, а затем снизить его после снижения пиковой нагрузки. Вы можете минимизировать издержки, оптимизировав облачную среду в соответствии с сезонными потребностями бизнеса. Кроме того, такая модель подходит для поддержки жизненного цикла программных продуктов. Команда тестирования может увеличивать мощность на время тестирования и снижать ее после завершения тестирования. В результате этого будет оплачиваться только используемая мощность, а не выделенные редко используемые ресурсы.

## Преимущества уровней служб

Хотя рабочие нагрузки могут отличаться, цель уровней служб — обеспечить предсказуемость производительности на разных уровнях. Они помогают клиентам с высокими требованиями к ресурсам работать в более изолированной вычислительной среде.
...
- **Минимальная задержка**. Некоторым приложениям требуется минимальное время отклика базы данных. Если определенная хранимая процедура вызывается в рамках более широкого взаимодействия с клиентом, то в 99 % случаев отклик на такой запрос должен выполняться не более чем за 20 миллисекунд. Такое приложение эффективно работает с уровнем служб Premium, который обеспечивает необходимую вычислительную мощность.

Уровень, который требуется приложению, зависит от пиковой нагрузки для каждого измерения ресурсов. Некоторые приложения могут использовать небольшой объем одного ресурса и значительный — других.
...
- Если размер пула эластичных БД изменен на новое значение eDTU, то оплата пула согласно этому новому значению eDTU начисляется только после завершения операции по изменению размера. Используется та же схема выставления счетов, что и при изменении уровня производительности автономных баз данных.
...
## Возможности и ограничения уровней служб
Каждый уровень службы и уровень производительности связан с различными ограничениями и характеристиками производительности. Следующая таблица описывает эти характеристики отдельной базы данных.

[AZURE.INCLUDE [Таблица уровней служб базы данных SQL](../../includes/sql-database-service-tiers-table.md)]

В следующих разделах приведены дополнительные сведения о просмотре использования, связанного с этими ограничениями.

### Максимальное хранилище OLTP в памяти

Можно использовать представление **sys.dm\_db\_resource\_stats**, чтобы отслеживать использование хранилища в памяти (In-Memory). Дополнительные сведения об отслеживании см. в разделе [Мониторинг хранилища In-Memory OLTP](sql-database-in-memory-oltp-monitoring.md).

>[AZURE.NOTE] Предварительная версия In-Memory OLTP в данный момент поддерживается только для отдельных баз данных, но не для баз данных в пулах эластичных баз данных.

### Максимальное число параллельных запросов

Чтобы просмотреть число одновременных запросов, выполните следующий запрос Transact-SQL в Базе данных SQL:

	SELECT COUNT(*) AS [Concurrent_Requests]
	FROM sys.dm_exec_requests R

При анализе рабочей нагрузки локальной Базы данных SQL Server следует изменить запрос для фильтрации конкретной базы данных, которая анализируется. Например, если у вас есть локальная база данных с именем MyDatabase, следующий запрос Transact-SQL возвратит число одновременных запросов в этой базе данных.
...
Это только моментальный снимок в один момент времени. Для лучшего понимания рабочей нагрузки потребуется собрать большое количество примеров за период временем, чтобы понять требования к параллельным запросам.

### Максимальное количество одновременных входов

Нет запроса или динамического административного представления, которые могли бы показать количество одновременных входов или журнал. Чтобы получить представление о частоте входа, можно проанализировать шаблоны работы пользователей и приложений. Можно также запустить реальные нагрузки в тестовой среде, чтобы убедиться в том, что вы не приближаетесь к этим или другим ограничениям, описанным в данном разделе.

Если несколько клиентов используют ту же строку подключения, служба проверяет подлинность каждого входа. Если десять пользователей одновременно подключаются к базе данных с тем же именем пользователя и паролем, это будет десять одновременных процессов входа. Это ограничение применяется только на время входа и проверки подлинности. Если те же десять пользователей последовательно подключатся к базе данных, количество одновременных входов никогда не будет больше одного.

>[AZURE.NOTE] В настоящее время это ограничение не применимо к базам данных в пулах эластичных баз данных.

### Максимальное число сеансов

Чтобы просмотреть текущее число активных сеансов, выполните в базе данных SQL следующий запрос Transact-SQL:
...
Опять же, эти запросы возвращают значение счетчика на момент времени, поэтому сбор нескольких образцов в течение времени обеспечивает лучшее понимание использования сеансов.

Для анализа Базы данных SQL можно также выполнить запрос **sys.resource\_stats** и получить журнал статистики использования сеансов с помощью столбца **active\_session\_count**. Предоставляется информация об использовании этого представления в следующем разделе мониторинга.

## Мониторинг использования ресурсов
Существуют два представления, которые позволяют отслеживать использование ресурсов для Базы данных SQL относительно ее уровня служб.
...
### Использование sys.dm\_db\_resource\_stats
Представление [Sys.dm\_db\_resource\_stats](https://msdn.microsoft.com/library/dn800981.aspx) существует в каждой Базе данных SQL и предоставляет последние данные об использовании ресурсов относительно уровня службы. Средний процент для ЦП, ввод-вывод данных, записи в журнал и данные о памяти записываются каждые 15 секунд и хранятся один час.

Так как это представление содержит более подробные сведения об использовании ресурсов, следует сначала использовать **sys.dm\_db\_resource\_stats** для любого анализа текущего состояния и устранения неполадок. Например, следующий запрос показывает среднее и максимальное использование ресурсов для текущей базы данных за последний час.
...
Примеры других запросов см. в [sys.dm\_db\_resource\_stats](https://msdn.microsoft.com/library/dn800981.aspx).

### Использование sys.resource\_stats

Представление [Sys.resource\_stats](https://msdn.microsoft.com/library/dn269979.aspx) в базе данных **master** предоставляет дополнительные сведения для мониторинга использования производительности Базы данных SQL в рамках конкретного уровня службы и производительности. Данные собираются каждые пять минут и хранятся приблизительно 35 дней. Это представление более полезно для анализа использования ресурсов Базы данных SQL за более долгий период.

На следующей диаграмме показано почасовое использование ресурсов процессора для базы данных Premium с уровнем производительности P2 в течение недели. Данные на диаграмме представлены начиная с понедельника. Она охватывает пять рабочих дней и выходные, когда нагрузка заметно ниже.

![Использование ресурсов базы данных SQL](./media/sql-database-performance-guidance/sql_db_resource_utilization.png)

Судя по этим данным, пиковая нагрузка на процессор составляет чуть более 50 % от максимальной нагрузки на уровне производительности P2 (полдень вторника). Если мощность процессора была главным фактором ресурсного профиля приложения, то клиент может решить, что P2 — оптимальный уровень производительности, который гарантирует стабильную работу с учетом средней нагрузки. Если ожидается рост нагрузки с течением времени, то целесообразно немного увеличить запас ресурсов, чтобы приложение не смогло достигнуть предела производительности. Увеличение уровня производительности позволит избежать заметных пользователю ошибок на стороне базы данных в результате нехватки мощности для эффективной обработки запросов, особенно в средах с небольшими задержками (например, для базы данных приложения, создающего веб-страницы на основе запросов к базе данных).
...
Следующий пример демонстрирует различные способы восприятия сведений об использовании ресурсов базы данных SQL с помощью представления каталога **sys.resource\_stats**.

1. Например, с учетом использования ресурсов базы данных userdb1 за неделю можно выполнить следующий запрос:
...
3. С помощью средних и максимальных значений по каждому ресурсу можно оценить, насколько выбранный уровень производительности подходит для вашей рабочей нагрузки. Как правило, средние значения из sys.resource\_stats дают хорошую основу для определения целевого резервирования. Эти сведения следует использовать в качестве отправной точки анализа. Например, если вы используете уровень служб Standard с уровнем производительности S2, средняя нагрузка на процессор (число операций чтения и записи ввода-вывода) меньше 40 %, среднее число рабочих потоков меньше 50, а среднее количество сеансов меньше 200, для этой рабочей нагрузки может подойти уровень S1. Вы легко можете определить, отвечает ли уровень базы данных ограничениям рабочих потоков и сеансов. Чтобы узнать, можно ли для базы данных использовать более низкий уровень производительности с учетом нагрузки на процессор (количества операций чтения и записи), разделите число DTU более низкого уровня на DTU текущего уровня производительности и умножьте результат на 100:
...
### Анализ производительности запросов и помощник по работе с базами данных SQL
База данных SQL предоставляет на портале Azure два средства для анализа и устранения проблем производительности базы данных.
...
См. предыдущие ссылки, чтобы получить дополнительные сведения о каждом средстве и способах их использования. Два следующих раздела об отсутствующих индексах и настройке запросов предоставляют другие способы вручную найти и устранить аналогичные проблемы производительности. Рекомендуется сначала попробовать средства на портале для более эффективной диагностики и устранения неполадок. Используйте настройку вручную для особых случаев.
...
Эффект можно увидеть в таблице **sys.resource\_stats** (между временем выполнения теста и появлением данных в таблице есть задержка). В этом примере первая часть выполнялась в рамках временного окна 22:25:00, а вторая — в 22:35:00. Обратите внимание, что в первой части было использовано больше ресурсов, чем во второй (в силу повышения эффективности).
...
Рекомендуется просмотреть таблицу **sys.resource\_stats**, чтобы определить, использует ли один тест больше ресурсов, чем другой. При сравнении данных нужно закладывать между тестами достаточно времени, чтобы они не группировались в одном 5-минутном окне в представлении **sys.resource\_stats**. Цель расчетов — минимизировать общее использование ресурсов, а не пиковую нагрузку. Обычно оптимизация части кода с целью уменьшения задержки выполнения также приводит к сокращению использования ресурсов. Убедитесь, что вносимые изменения нужны и не сказываются на работе приложения отрицательным образом при использовании указаний запроса.

Если рабочая нагрузка содержит ряд повторяющихся запросов, то часто бывает целесообразно отследить и проверить оптимальность планов, так как это позволит уменьшить минимальный размер ресурсов, необходимых базе данных. После проверки выборочный пересмотр планов позволит убедиться, что их производительность со временем не ухудшилась. Дополнительные сведения об указаниях запросов см. в разделе [Указания запросов (Transact-SQL)](https://msdn.microsoft.com/library/ms181714.aspx).

### Сегментирование баз данных
Так как база данных SQL Azure работает на стандартном оборудовании, ограничения для отдельной базы данных будут ниже, чем в традиционной локальной среде SQL Server. В силу этого несколько пользователей используют сегментирование баз данных, если размеры отдельной базы данных в базе данных SQL Azure недостаточны. Большинство клиентов, реализующих сегментирование в базе данных SQL Azure на сегодняшний день, разбивают данные одного измерения на несколько баз данных. Этот подход предполагает, что часто приложения OLTP выполняют транзакции, которые применимы к одной строке или небольшой группе строк внутри одной схемы.

>[AZURE.NOTE] База данных SQL теперь предоставляет библиотеку для упрощения сегментирования. Дополнительные сведения см. в статье [Обзор клиентской библиотеки для эластичных баз данных](sql-database-elastic-database-client-library.md).

Например, если база данных содержит сведения о заказчике, заказе и цене (как можно увидеть в стандартном примере базы данных Northwind, который используется для SQL Server), эти данные можно разбить на несколько баз, группируя все сведения и обеспечивая целостность данных. Приложение должно разбивать заказчиков по разным базам данных и эффективно распределять нагрузку. В результате можно не только не превышать имеющееся ограничение максимального размера базы данных, но и обрабатывать в базе данных SQL Azure рабочие нагрузки, которые существенно превышают ограничения различных уровней производительности, если каждая отдельная база данных будет работать в соответствии с ограничениями DTU.
