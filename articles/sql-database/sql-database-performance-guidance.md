<properties
	pageTitle="База данных SQL Azure и производительность отдельных баз данных | Microsoft Azure"
...
В следующих разделах содержатся дополнительные сведения о просмотре данных об использовании в связи с этими ограничениями.

### Максимальный объем хранилища OLTP в памяти

Вы можете использовать представление **sys.dm\_db\_resource\_stats**, чтобы отслеживать использование хранилища Azure в памяти. Дополнительные сведения об отслеживании см. в разделе [Мониторинг хранилища OLTP в памяти](sql-database-in-memory-oltp-monitoring.md).

>[AZURE.NOTE] Сейчас предварительная версия оперативной обработки транзакций Azure в памяти поддерживается только для отдельных баз данных. Ее нельзя использовать для баз данных в пулах эластичных баз данных.
...
>[AZURE.NOTE] Сейчас это ограничение не применимо к базам данных в пулах эластичных баз данных.
...
Для анализа Базы данных SQL можно получить журнал статистики использования сеансов. Для этого выполните запрос **sys.resource\_stats** и воспользуйтесь столбцом **active\_session\_count**. Дополнительные сведения об использовании этого представления см. в следующем разделе.

## Отслеживание использования ресурсов
Два представления позволяют отслеживать использование ресурсов для Базы данных SQL относительно ее уровня служб:
...
Представление [sys.dm\_db\_resource\_stats](https://msdn.microsoft.com/library/dn800981.aspx) можно использовать в каждой Базе данных SQL. В представлении **sys.dm\_db\_resource\_stats** отображаются последние данные использования ресурсов в соответствии с уровнем служб. Средний процент нагрузки ЦП, показатели операций ввода-вывода данных, записи в журнал и данные о памяти записываются каждые 15 секунд и хранятся 1 час.

Так как это представление содержит более подробные сведения об использовании ресурсов, сначала используйте представление **sys.dm\_db\_resource\_stats** для любого анализа текущего состояния и устранения неполадок. Например, этот запрос показывает среднее и максимальное использование ресурсов для текущей базы данных за последний час:
...
Представление [sys.resource\_stats](https://msdn.microsoft.com/library/dn269979.aspx) в базе данных **master** предоставляет дополнительные сведения, с помощью которых можно отслеживать использование производительности Базы данных SQL в рамках конкретного уровня службы и производительности. Данные собираются каждые 5 минут и хранятся приблизительно 35 дней. Это представление полезно для анализа использования ресурсов Базы данных SQL за более долгий период.
...
![Представление каталога sys.resource\_stats](./media/sql-database-performance-guidance/sys_resource_stats.png)

Ниже показаны различные способы использования представления каталога **sys.resource\_stats**, чтобы получить сведения об использовании ресурсов в Базе данных SQL.
...
На портале Azure есть два средства, с помощью которых можно проанализировать и устранить проблемы производительности в Базе данных SQL:
...
Просмотреть результат можно в таблице **sys.resource\_stats** (между временем выполнения теста и появлением данных в таблице есть задержка). В этом примере первая часть выполнялась в рамках временного окна 22:25:00, а вторая часть — в 22:35:00. Обратите внимание, что в первой части использовано больше ресурсов, чем во второй (в силу повышения эффективности).
...
>[AZURE.NOTE] В этом примере преднамеренно использовались небольшие значения, однако последствия неоптимальных параметров могут быть существенными, особенно для больших баз данных. Разница для быстрых и медленных запросов в некоторых случаях может достигать от нескольких секунд до нескольких часов.

Вы можете просмотреть таблицу **sys.resource\_stats**, чтобы определить, использует ли один тест больше ресурсов, чем другой. При сравнении данных между тестами необходимо установить интервалы, чтобы они не группировались в одном 5-минутном окне в представлении **sys.resource\_stats**. Цель расчетов — минимизировать общее использование ресурсов, а не пиковую нагрузку. Обычно оптимизация части кода с целью уменьшения задержки выполнения также приводит к сокращению использования ресурсов. Убедитесь, что изменения, внесенные в приложение, необходимы и что они не оказывают негативное влияние на работу приложения при использовании указаний запроса.
...
- Сведения о производительности и пулах эластичных баз данных см. в статье [Когда следует использовать пул эластичных баз данных?](sql-database-elastic-pool-guidance.md)
