...
Узнайте, как использовать [георепликацию](sql-database-geo-replication-overview.md) в базе данных SQL для установки последовательных обновлений облачного приложения. Так как обновление представляет собой нарушающую работу операцию, оно должно учитываться при планировании и проектировании непрерывности бизнес-процессов. В этой статье мы рассмотрим два разных метода для оркестрации процесса обновления, а также преимущества и недостатки каждой из них. В рамках этой статьи мы используем простое приложение, состоящее из веб-сайта, подключенного к изолированной базе данных в качестве уровня данных. Наша цель заключается в обновлении версии 1 приложения до версии 2 без значительного влияния на работу конечных пользователей. 
...
## <a name="additionale-resources"></a>Дополнительные ресурсы
