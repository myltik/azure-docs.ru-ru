...
При использовании решения [In-Memory](sql-database-in-memory.md) данные в оптимизированных для памяти таблицах и табличных переменных находятся в хранилище XTP In-Memory. Каждому уровню служб Премиум выделяется максимальный объем хранилища в памяти; подробнее об этом см. в статье [Уровни служб базы данных SQL](sql-database-service-tiers.md#service-tiers-for-single-databases). При превышении этого ограничения операции вставки и обновления могут завершаться сбоем (ошибка 41805). В таком случае вам придется либо удалить данные, чтобы освободить память, либо повысить уровень производительности базы данных.
...
На [портале](http://portal.azure.com/) Azure вы можете отслеживать использование хранилища в памяти, выраженное в процентах от [емкости хранилища для своего уровня производительности](sql-database-service-tiers.md#service-tiers-for-single-databases).
...
## Дальнейшие действия
Узнайте подробнее о [мониторинге базы данных SQL Azure с помощью динамических административных представлений](sql-database-monitoring-with-dmvs.md).
