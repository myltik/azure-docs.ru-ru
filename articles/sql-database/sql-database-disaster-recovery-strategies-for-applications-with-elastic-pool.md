<properties 
   pageTitle="Разработка облачных решений для аварийного восстановления с применением георепликации базы данных SQL | Microsoft Azure"
...
Чтобы обеспечить упрощение операций, необходимо развернуть все базы данных клиента в одном пуле эластичных БД в любом регионе Azure и развернуть базы данных управления как геореплицированные автономные базы данных. Для аварийного восстановления баз данных клиента следует использовать геовосстановление. Эта возможность не требует дополнительных затрат. Чтобы обеспечить доступность баз данных управления, их необходимо геореплицировать в другой регион (шаг 1). Текущие затраты на настройку аварийного восстановления в этом сценарии равны совокупной стоимости баз данных-получателей. Процесс настройки показан на схеме ниже.
...
- Незамедлительная отработка отказа баз данных управления (2) в регионе аварийного восстановления. 
- Изменение строки подключения приложения для указания региона аварийного восстановления. В регионе аварийного восстановления будут созданы новые учетные записи и базы данных клиента. Для существующих клиентов данные будут временно недоступными.
- Создание пула эластичных БД с такими же настройками, как и в исходном пуле БД (3). 
- Использование геовосстановления для создания копий баз данных клиента (4). При необходимости можно запустить отдельные процессы восстановления на основе подключений пользователей или использовать другие схемы расстановки приоритетов приложения.
...
Если сбой временный, не исключено, что Azure восстановит работоспособность основного региона до завершения всех операций восстановления в регионе аварийного восстановления. В этом случае необходимо проследить за процессом перемещения приложения обратно в основной регион. Этапы этого процесса показаны на схеме ниже.
 
- Отмена всех ожидающих запросов на геовосстановление.   
- Отработка отказа баз данных управления в основном регионе (5). Примечание. После восстановления работоспособности региона старые базы данных-источники автоматически станут базами данных-получателями. Теперь они снова поменяются ролями. 
- Изменение строки подключения приложения для указания основного региона. В основном регионе будут созданы новые учетные записи и базы данных клиента. Для некоторых клиентов данные будут временно недоступными.   
...
Для выполнения этого сценария необходимо отделить пользователей пробной версии от пользователей платной версии и поместить их в разные пулы эластичных БД. Пользователям пробной версии предоставляется небольшое количество eDTU на клиента и соглашение об уровне обслуживания с более низким уровнем услуг, а процесс восстановления занимает больше времени. Пользователи платной версии помещаются в пул с большим количеством eDTU на клиента и соглашением об уровне обслуживания с более высоким уровнем услуг. Чтобы максимально сократить время восстановления, базы данных клиентов платной версии необходимо геореплицировать. Процесс настройки показан на схеме ниже.

![Рис. 4](./media/sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool/diagram-4.png)

Как и в предыдущем сценарии, базы данных управления следует использовать как автономные геореплицированные базы данных (1). Это позволит обеспечить предсказуемую производительность новых клиентских подписок, операций обновления профиля и других операций управления. Регион, в котором содержатся базы данных-источники баз данных управления, будет использоваться в качестве основного региона, а регион, в котором находятся базы данных-получатели, — в качестве региона аварийного восстановления.

Для баз данных клиентов платной версии выделяются активные базы данных в платном пуле основного региона. В регионе аварийного восстановления необходимо подготовить вторичный пул с тем же именем. Каждого клиента необходимо геореплицировать во вторичный пул (2). Это позволит ускорить восстановление всех баз данных клиента с применением отработки отказа.
...
- Изменение строки подключения приложения для указания региона аварийного восстановления. В регионе аварийного восстановления будут созданы новые учетные записи и базы данных клиента. Для существующих пользователей пробной версии данные будут временно недоступными.
...
- Если количество eDTU вторичного пула было ниже, чем у основного, так как для вторичных баз данных eDTU требуются только для обработки журналов изменений при пребывании в роли вторичных, необходимо увеличить объем пула с учетом рабочей нагрузки всех клиентов (5). 
...
На этом этапе работа приложения в регионе аварийного восстановления будет восстановлена. Пользователи платной версии будут иметь бесперебойный доступ к данным, а пользователи бесплатной пробной версии будут получать доступ с задержками.

Если Azure восстановит основный регион *после* восстановления приложения в регионе аварийного восстановления, приложение можно использовать в том же регионе или восстановить размещение в основном регионе. Если работоспособность основного региона восстановлена *до* завершения процесса отработки отказа, необходимо выполнить восстановление размещения. На схеме ниже показаны этапы процесса восстановления размещения.
...
- Отработка отказа для баз данных платных клиентов (9). Аналогичным образом после восстановления работоспособности региона старые базы данных-источники автоматически стали базами данных-получателями. Теперь они снова станут базами данных-источниками. 
...
Для выполнения этого сценария у вас должно быть три отдельных пула эластичных БД. В двух разных регионах необходимо подготовить два пула одинакового размера с большим количеством eDTU на базу данных, в которых будут содержаться базы данных платных пользователей. Третий пул, в котором содержатся клиенты платных пользователей, должен иметь более низкое количество eDTU на базу данных, и его необходимо подготовить в одном из двух регионов.

Чтобы максимально сократить время восстановления после сбоев, базы данных платных пользователей необходимо геореплицировать с половиной баз данных-источников, размещенных в каждом из двух регионов. Точно так же в каждом регионе будет содержаться 50 % вторичных баз данных. Таким образом, если регион вне сети, только половина баз данных платных пользователей будет затронута, и для них будет выполняться отработка отказа. На производительность остальных баз данных это не повлияет. Процесс настройки показан на схеме ниже.

![Рис. 4](./media/sql-database-disaster-recovery-strategies-for-applications-with-elastic-pool/diagram-7.png)

Как и в предыдущих сценариях, базы данных управления следует настроить как автономные геореплицированные базы данных (1). Это позволит обеспечить предсказуемую производительность новых клиентских подписок, операций обновления профиля и других операций управления. Регион А будет использоваться в качестве основного региона для баз данных управления, а регион Б — в качестве региона для их восстановления.

Базы данных платных пользователей также будут геореплицированы, но с разделением баз данных-источников и баз данных-получателей между регионами А и Б (2). Таким образом, в случае сбоя для затронутых баз данных-источников можно выполнить отработку отказа в другой регион и обеспечить их доступность. На производительность другой половины баз данных сбой не повлияет.
...
- Изменение строки подключения приложения для указания баз данных управления в регионе Б. Измените базы данных управления, чтобы обеспечить перемещение существующих баз данных в регион Б и создание там же новых учетных записей и баз данных клиентов. Для существующих пользователей пробной версии данные будут временно недоступными.
...
- Так как теперь в пуле 2 содержатся только базы данных-источники, рабочая нагрузка в нем увеличится, поэтому необходимо незамедлительно увеличить количество eDTU (5). 
...
На этом этапе работа приложения в регионе Б будет восстановлена. Пользователи платной версии приложения будут иметь доступ к их данным, а пользователи бесплатной пробной версии будут получать доступ с задержками.

После восстановления региона А необходимо принять решение: использовать регион Б для пользователей пробной версии или выполнить восстановление размещения в пул этих пользователей в регионе А. Основным показателем может быть процент баз данных пользователей пробной версии, измененных в процессе восстановления. Независимо от решения, необходимо перераспределить платных клиентов между двумя пулами. На схеме ниже показан процесс восстановления размещения баз данных пользователей пробной версии в регион А.
...
- Выбор баз данных пользователей платной версии, для которых необходимо выполнить восстановление размещения в пул 1, и запуск отработки отказа для их баз данных-получателей (9). После восстановления работоспособности региона все базы данных в пуле 1 автоматически стали базами данных-получателями. Теперь половина баз данных снова станут базами данных-источниками. 
- Уменьшение размера пула 2 до исходного количества eDTU (10).
...
- Позволяет более эффективно использовать емкость пула, так как в 50 % баз данных-получателей в пуле 1 и 2 снижена активность по сравнению с базами данных-источников.

**Недостатки** этой стратегии:

- Операции CRUD в базах данных управления будут иметь меньшую задержку для пользователей в регионе А, чем для пользователей в регионе Б, так как они выполняются с базами данных-источниками баз данных управления.
- Она требует более сложной структуры базы данных управления. Например, каждая запись клиента обязательно должна включать в себя тег расположения, который необходимо изменить во время отработки отказа и восстановления размещения.  
...
Эта статья посвящена стратегиям аварийного восстановления баз данных для мультитенантного приложения независимых поставщиков программного обеспечения SaaS. Выбор стратегии зависит от потребностей приложения, включая бизнес-модель и соглашение об уровне обслуживания, которые вы планируете предоставлять своим пользователям, бюджета и т. д. Для каждой рассмотренной стратегии приведены ее основные преимущества и недостатки. Это позволит вам принять обоснованное решение. Вероятно, ваше приложение будет включать другие компоненты Azure. Изучите руководство по обеспечению непрерывности бизнес-процессов для этих компонентов и выполните восстановление на уровне базы данных. Дополнительные сведения об управлении восстановлением приложений базы данных в Azure см. в статье о [разработке приложения для аварийного восстановления облака](./sql-database-designing-cloud-solutions-for-disaster-recovery.md).

Отдельные шаги, необходимые для каждого сценария, включают операции с большим количеством баз данных. Мы рекомендуем использовать задания обработки эластичных БД SQL для управления этими операциями в масштабе. Дополнительные сведения см. в статье [Обзор заданий обработки эластичных баз данных](./sql-database-elastic-jobs-overview.md). Ниже приведены ссылки на ресурсы с дополнительными сведениями, необходимыми для реализации описанных здесь сценариев.
