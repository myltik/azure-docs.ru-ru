<properties
	...
	description="Создайте первую базу данных SQL за несколько минут на портале Azure с помощью демонстрационных данных и системы управления реляционными базами данных корпорации Майкрософт (RDBMS)."
...
В этом руководстве показано, как всего за несколько минут можно создать первую базу данных SQL с помощью демонстрационных данных и портала предварительной версии Azure. Вы узнаете, как:
...
4. Введите легко запоминающееся **имя администратора сервера** (мы используем **AdventureAdmin**). Затем введите надежный **пароль** и повторите его в поле **Подтверждение пароля**.

	![Новые параметры сервера базы данных](./media/sql-database-get-started/get-started-serversettings.png)

	 Чтобы получить доступ к новейшим возможностям, сохраните значение **Да** для параметра **Создать сервер V12 (последнее обновление)**. **Расположение** определяет регион центра обработки данных, где создается сервер.
...
- Если вы знакомы с Excel, узнайте, как [подключиться к базе данных SQL с помощью Excel](sql-database-connect-excel.md).
