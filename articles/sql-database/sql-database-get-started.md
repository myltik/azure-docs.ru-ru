...
В этой статье описано, как создать демонстрационную базу данных SQL Azure за несколько минут с помощью портала предварительной версии Azure Вы узнаете, как:
...
> [AZURE.NOTE]В этом практическом руководстве описывается настройка базы данных с помощью службы реляционной базы данных в облаке (база данных SQL Azure). Второй вариант — запуск SQL Server на виртуальной машине Azure. См. статью [Основные сведения о базе данных SQL Azure и SQL Server в виртуальных машинах Azure](data-management-azure-sql-database-and-sql-server-iaas.md), чтобы сравнить оба варианта, или см. статью [Подготовка виртуальной машины сервера SQL](virtual-machines-provision-sql-server.md), чтобы начать работу с виртуальной машиной.
...
2. В разделе **Сервер** щелкните **Настроить необходимые параметры**, а затем выберите **Создать сервер**.
...
4. Введите **учетные данные администратора сервера**, которые легко запомнить (мы используем **AdventureAdmin**). Затем введите надежный **пароль** и введите его еще раз в поле **Подтверждение пароля**.
...
4. Нажмите **Добавить IP-адрес клиента**, чтобы система Azure создала правило для этого IP-адреса, а затем нажмите кнопку **Сохранить**.
	...
	>[AZURE.IMPORTANT]Ваш IP-адрес клиента, скорее всего, будет периодически меняться, и в таких случаях вы не сможете получить доступ к серверу, пока не создадите новое правило брандмауэра. Вы можете проверить IP-адрес с помощью службы [Bing](http://www.bing.com/search?q=my%20ip%20address), а затем добавить один IP-адрес или диапазон IP-адресов. Подробные сведения см. в статье [Настройка параметров брандмауэра](sql-database-configure-firewall-settings.md).

## Дальнейшие действия
Теперь, когда у вас есть база данных с образцами данных, ее можно исследовать с помощью предпочитаемых вами средств.

- Если вы знакомы с языком Transact-SQL и средой SQL Server Management Studio, узнайте о [подключении к базе данных SQL и выполнении запросов с помощью SSMS](sql-database-connect-query-ssms.md).

- Если вы знаете Excel, узнайте, как [подключиться к базе данных SQL с помощью Excel](sql-database-connect-excel.md).

- Если вы готовы писать код, см. статьи [Подключение к базе данных SQL и выполнение запросов с помощью языка C#](sql-database-connect-query.md) и [Использование базы данных SQL из .NET (C#)](sql-database-develop-dotnet-simple.md). Кроме примеров C#, в статье [Простые примеры кода клиентских приложений для работы с базой данных SQL](sql-database-develop-quick-start-client-code-samples.md) содержатся образцы кода на языках Node.js, Python, Ruby, Java, PHP и C++, а также практические руководства.

- Если вам требуется переместить локальные базы данных SQL Server в Azure, подробные сведения об этом см. в статье [Перенос базы данных в базу данных SQL Azure](sql-database-cloud-migrate.md).
