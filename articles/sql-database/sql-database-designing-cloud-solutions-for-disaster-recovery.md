<properties 
   pageTitle="Разработка облачных решений для аварийного восстановления с помощью георепликации базы данных SQL."
...
# Разработка облачных приложений для непрерывности бизнес-процессов с использованием георепликации

Существует несколько способов использования георепликации в базе данных SQL при разработке приложений для непрерывности бизнес-процессов. Факторы, влияющие на разработку, включают топологию развертывания приложения, планируемое соглашение об уровне обслуживания, задержку трафика и расходы. В этой статье мы рассмотрим типичные модели создания приложений, а также преимущества и недостатки каждой из них.

## Модель разработки 1: активное — пассивное развертывание для аварийного восстановления с совмещенной базой данных
...
> [AZURE.NOTE]При использовании [активной георепликации](https://msdn.microsoft.com/library/azure/dn741339.aspx) можно оставить активными оба приложения мониторинга и проверять как базу данных-источник, так и базу данных-получатель. Последнюю можно использовать для обнаружения сбоя в дополнительном регионе и предупреждения об отсутствии защиты приложения.

На следующей схеме показана эта конфигурация до сбоя.

![На рисунке 1](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern1-1.png)
...
2. Вызов базы данных-получателя для [запуска отработки отказа базы данных](https://msdn.microsoft.com/library/azure/dn509573.aspx). 
...
![На рис. 2](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern1-2.png)
...
![Рис. 3](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern1-3.png)
...
+ На работу приложения не влияет отработка отказа, т. к. приложение и база данных всегда находятся в одном расположении. 
...
> [AZURE.NOTE]Хотя эта модель использует несколько баз данных-получателей, только одна из них будет использоваться для отработки отказа по причинам, описанным ранее. Так как для этой модели требуется доступ к базе данных-получателю только для чтения, для нее необходима [активная георепликация](https://msdn.microsoft.com/library/azure/dn741339.aspx).

В диспетчере трафика должна быть настроена маршрутизация производительности, чтобы направлять подключения пользователей к экземпляру приложения, расположенному ближе к географическому положению пользователя. На следующей схеме показана эта конфигурация до сбоя. ![Рис. 4](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern2-1.png)
...
2. Вызов соответствующей базы данных-получателя для [запуска отработки отказа базы данных](https://msdn.microsoft.com/library/azure/dn509573.aspx). 

На следующей схеме показана новая конфигурация после отработки отказа. ![Рис. 5](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern2-2.png)
...
![Рис. 6](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern2-3.png)

Основным **преимуществом** этой модели разработки является возможность масштабирования нагрузки приложения на нескольких базах данных-получателях для оптимальной работы конечных пользователей. **Недостатки** этого варианта:
...
![Рис. 8](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern3-2.png)
...
> [AZURE.NOTE]Так как для этой модели требуется доступ к базе данных-получателю только для чтения, для нее необходима [активная георепликация](https://msdn.microsoft.com/library/azure/dn741339.aspx).
...
![Рис. 8](./media/sql-database-designing-cloud-solutions-for-disaster-recovery/pattern3-3.png)
...
Конкретная стратегия аварийного восстановления может объединить или расширить эти модели для большего соответствия требованиям приложения. Как уже упоминалось, выбор стратегии зависит от соглашения об уровне обслуживания, предлагаемого клиентам, и топологии развертывания приложения. Чтобы помочь вам принять решение, в следующей таблице сравниваются варианты с учетом предполагаемой потери данных или целевой точки восстановления (RPO) и оценки времени восстановления (ERT).
