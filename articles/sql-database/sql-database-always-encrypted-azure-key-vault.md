...
- Azure PowerShell [не ниже версии 1.0](../powershell-install-configure.md).
    - Чтобы определить свою версию PowerShell, введите команду **(Get-Module azure -ListAvailable).Version**.
...
2. Выберите **Active Directory** в меню слева и щелкните каталог Active Directory, который будет использовать ваше приложение.
...
5. В полях "URL-АДРЕС ВХОДА" и "URI КОДА ПРИЛОЖЕНИЯ" можно просто ввести допустимый URL-адрес (например: *http://myClientApp*).
6. Нажмите **НАСТРОИТЬ**.
7. Скопируйте **ИДЕНТИФИКАТОР КЛИЕНТА** (он потребуется в коде позже).
8. В разделе "Ключи" выберите **1 год** из раскрывающегося списка **Выбрать длительность** (мы скопируем ключ после сохранения ниже).
11. Прокрутите вниз и щелкните **Добавить приложение**.
12. Оставьте для параметра **ПОКАЗАТЬ** значение **Приложения Майкрософт**, затем найдите и выберите **Windows Azure Service Management** (Управление службами Microsoft Azure) и щелкните галочку, чтобы продолжить.
13. В строке **Windows Azure Service Mana…** (Управление службами Microsoft Azure) щелкните раскрывающийся список **Делегированные разрешения** и выберите **Access Azure Service Management** (Доступ к управлению службами Azure).
14. Щелкните **СОХРАНИТЬ** (внизу).
15. После завершения сохранения найдите и скопируйте значение ключа в разделе **ключей** (это значение потребуется в коде позже).
...
4. **Настройка главного ключа**
    ...
    На странице **Настройка главного ключа** можно настроить главный ключ столбца и выбрать поставщик хранилища ключей, в котором будет находиться этот ключ. В настоящее время главные ключи столбцов можно хранить в хранилище сертификатов Windows, хранилище ключей Azure или аппаратном модуле безопасности.
...
Чтобы убедиться, что ключи в SQL Server Management Studio созданы, последовательно разверните узлы **Clinic** > **Безопасность** > **Ключи постоянного шифрования**. Таким образом можно увидеть новые ключи, созданные с помощью мастера.
