...
> [!IMPORTANT]
>Во время каждого сеанса передачи данных в Базу данных SQL Azure и из нее все подключения к базе должны быть зашифрованы (SSL/TLS). В строке подключения приложения, необходимо задать параметры для шифрования соединения и *не* доверие сертификату сервера (это выполняется автоматически при копировании строки подключения из портала Azure), в противном случае подключение не будет проверять подлинность сервера и будут подвержены атакам «злоумышленник среднего». Например, для драйвера ADO.NET это параметры **Encrypt=True** и **TrustServerCertificate=False**. 
...
> [!IMPORTANT]
> Управление базами данных и логическими серверами в Azure осуществляется путем назначения ролей учетной записи пользователя портала. Дополнительные сведения по этой теме см. в разделе [управления доступом на основе ролей на портале Azure](../active-directory/role-based-access-control-what-is.md).
>
