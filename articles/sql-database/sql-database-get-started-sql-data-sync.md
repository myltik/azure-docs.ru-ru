...
> [!NOTE]
> Полная техническая документация по синхронизации данных SQL Azure, которая ранее располагалась на сайте MSDN, доступна в виде PDF-файла. Его можно скачать [здесь](http://download.microsoft.com/download/4/E/3/4E394315-A4CB-4C59-9696-B25215A19CEF/SQL_Data_Sync_Preview.pdf).
>
>

## <a name="step-1-connect-to-the-azure-sql-database"></a>Шаг 1. Подключение к Базе данных SQL Azure
1. Перейдите на [классический портал Azure](http://manage.windowsazure.com).
2. Щелкните **БАЗЫ ДАННЫХ SQL** на левой панели.
3. Щелкните **СИНХРОНИЗИРОВАТЬ** в нижней части страницы. Щелкнув "Синхронизация", вы откроете список элементов, которые можно добавить: **Новая группа синхронизации** и **Новый агент синхронизации**.
...
## <a name="step-2-add-a-client-agent"></a>Шаг 2. Добавьте агента клиента
Этот шаг необходим только в том случае, если база данных SQL Server, включенная в группу синхронизации, будет размещена на локальных ресурсах.
Если в вашей группе синхронизации заданы только экземпляры Базы данных SQL, переходите к этапу 4.

<a id="InstallRequiredSoftware"></a>

### <a name="step-2a-install-the-required-software"></a>Шаг 2а. Установите требуемое программное обеспечение
...
* **Системные типы CLR Microsoft SQL Server 2008 R2 SP1 (x86)**
  ...
  Системные типы Microsoft SQL Server 2008 R2 SP1 System CLR Types (x86) можно установить [здесь](http://www.microsoft.com/download/en/details.aspx?id=26728).
* **Общие объекты управления Microsoft SQL Server 2008 R2 SP1 (x86)**

  Общие объекты управления Microsoft SQL Server 2008 R2 SP1 (x86) можно установить [здесь](http://www.microsoft.com/download/en/details.aspx?id=26728)

<a id="InstallClient"></a>

### <a name="step-2b-install-a-new-client-agent"></a>Шаг 2b. Установка нового агента клиента
...
### <a name="step-2c-finish-the-new-sql-data-sync-agent-wizard"></a>Шаг 2c. Завершение работы мастера нового агента синхронизации данных SQL
...
## <a name="step-3-register-a-sql-server-database-with-the-client-agent"></a>Шаг 3. Зарегистрируйте базу данных SQL Server в агенте клиента
...
Чтобы зарегистрировать базу данных в агенте, выполните указания, приведенные в разделе [Регистрация базы данных SQL Server в клиентском агенте](http://download.microsoft.com/download/4/E/3/4E394315-A4CB-4C59-9696-B25215A19CEF/SQL_Data_Sync_Preview.pdf).

## <a name="step-4-create-a-sync-group"></a>Шаг 4. Создайте группу синхронизации
<a id="StartNewSGWizard"></a>

### <a name="step-4a-start-the-new-sync-group-wizard"></a>Шаг 4а. Запустите мастер новой группы синхронизации
...
### <a name="step-4b-enter-the-basic-settings"></a>Шаг 4b. Укажите базовые настройки
...
### <a name="step-4c-define-the-sync-hub"></a>Шаг 4c. Определите концентратор синхронизации
1. В раскрывающемся списке выберите экземпляр базы данных SQL, который будет служить концентратором группы синхронизации.
2. Введите учетные данные для этого экземпляра базы данных SQL: **имя пользователя концентратора** и **пароль**.
...
### <a name="step-4d-add-a-reference-database"></a>Шаг 4d. Добавьте справочную базу данных
...
## <a name="step-5-define-the-data-to-sync"></a>Шаг 5. Определите данные для синхронизации
...
## <a name="step-6-configure-your-sync-group"></a>Шаг 6. Настройка группы синхронизации
...
Чтобы выполнять синхронизацию по расписанию, необходимо настроить группу синхронизации.
