<properties
    pageTitle="Развертывание с помощью Базы данных SQL Azure | Microsoft Azure"
...
Можно легко развертывать базы данных SQL Azure с помощью инструментов **эластичной базы данных**. Эти инструменты и компоненты позволяют использовать практически неограниченные ресурсы **Базы данных SQL Azure** для создания решений для транзакционных рабочих нагрузок и, в частности, приложений SaaS (программное обеспечение как услуга). Возможности эластичных баз данных включают следующее.
...
* [Инструмент разбиения и объединения эластичной базы данных](sql-database-elastic-scale-overview-split-and-merge.md) перемещает данные между сегментированными базами данных. Это удобно для перемещения данных из мультитенантной базы данных в базу данных с одним клиентом (или наоборот). См. [учебник по инструменту разбиения и объединения эластичной базы данных](sql-database-elastic-scale-configure-deploy-split-and-merge.md).
...
3. Подмножество баз данных помещается в **пул эластичных баз данных**. (См. раздел [Что такое пул?](sql-database-elastic-pool.md).)
...
Пример приложения, демонстрирующий клиентскую библиотеку, приведен в разделе [Приступая к работе с инструментами эластичных баз данных](sql-database-elastic-scale-get-started.md).

Сведения о том, как преобразовать существующие базы данных, чтобы использовать эти инструменты, см. в разделе [Перенос существующих баз данных для масштабирования](sql-database-elastic-convert-to-use-elastic-tools.md).

Сведения об особенностях пула эластичных баз данных см. в статье [Вопросы цены и производительности для пула эластичных баз данных](sql-database-elastic-pool-guidance.md) или воспользуйтесь [руководством](sql-database-elastic-pool-create-portal.md) по созданию нового пула.
