...
Файлы резервных копий хранятся в учетной записи геоизбыточного хранилища, для которой установлен доступ только для чтения (RA-GRS), чтобы обеспечить их доступность для аварийного восстановления. Ниже показана георепликация еженедельных и ежедневных резервных копий, хранящихся в учетной записи геоизбыточного хранилища с доступом только для чтения (RA-GRS), чтобы обеспечить их доступность для аварийного восстановления.
...
Вы можете [восстановить базу данных из резервных копий, инициированных службой](sql-database-recovery-using-backups.md), в пределах их [срока хранения](sql-database-service-tiers.md) в:
