<properties 
    pageTitle="Конфигурации безопасности гибкого масштабирования | Microsoft Azure" 
...
Введите пароль и затем экспортируйте сертификат со следующими параметрами: * Да, экспортировать закрытый ключ * Экспортировать все расширенные свойства
...
Каждый пользователь, для которого создан сертификат клиента, должен выполнить эти действия, чтобы получить отпечаток сертификата, добавляемый в CSCFG-файл. * Запустите certmgr.exe. * Перейдите на вкладку "Личные данные". * Дважды щелкните сертификат клиента, который будет использоваться для проверки подлинности. * В открывшемся диалоговом окне "Сертификат" перейдите на вкладку "Сведения". * Убедитесь, что параметр "Показать" имеет значение "Все". * Выберите поле "Отпечаток" в списке. * Скопируйте значение отпечатка. ** Удалите невидимые символы Юникода перед первым числом. ** Удалите все пробелы.
...
Введите пароль и экспортируйте сертификат со следующими параметрами. * Да, экспортировать закрытый ключ. * Экспортировать все расширенные свойства. * Во время загрузки сертификата в облачную службу потребуется ввести пароль.
...
8.     Нажмите кнопку **Готово**
	...
	* Если было выбрано хранилище доверенного корневого центра сертификации, нажмите кнопку **Да**.
9.     Нажмите кнопку **ОК** во всех диалоговых окнах.

## Передача сертификата

Перейдите на [классический портал Azure](http://portal.azure.com/).
