...
У любой рекомендации по **созданию индекса** есть политика отхода, которая не позволит применить рекомендацию при высоком уровне использования базы данных или пула. Эта политика учитывает такие метрики, как загрузка ЦП, операции ввода-вывода данных и журнала, а также доступный объем хранилища. Если эти метрики (загрузка ЦП, операции ввода-вывода данных и журнала) превышали 80 % в течение последних 30 минут, создание индекса откладывается. Если после создания индекса доступный объем хранилища будет ниже 10 %, рекомендация перейдет в состояние ошибки. Если по прошествии нескольких дней служба автонастройки снова определит, что создание индекса полезно, процесс запустится повторно. Этот процесс будет повторяться, пока объем хранилища не будет достаточным для создания индекса или индекс не будет считаться полезным.
...
> [!IMPORTANT]
> Корпорация Майкрософт скоро перестанет поддерживать рекомендации по устранению ошибок схемы. Начните использовать средство [Intelligent Insights](sql-database-intelligent-insights.md), чтобы автоматически отслеживать проблемы с производительностью базы данных, включая ошибки схемы, которые ранее учитывались в рекомендациях по устранению ошибок схемы.
> 
