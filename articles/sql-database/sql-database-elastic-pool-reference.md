...
| storageMB | 200 ГБ* | 200 ГБ, 400 ГБ, 800 ГБ, 1200 ГБ |

*единицами измерения в API являются МБ, а не ГБ

Если размер хранилища пула превышен, все базы данных в пуле становятся доступными только для чтения. В этом случае попробуйте увеличить количество DTU или размер самого пула либо исключите из пула одну или несколько баз данных. Для выявления и предотвращения превышения можно создать оповещение на портале, которое будет отображаться, когда использование хранилища пула превышает заданное значение.
