...
- Импорт базы данных непосредственно в пул не поддерживается. Вы можете выполнить импорт в отдельную базу данных, а затем переместить эту базу данных в пул. *Поддерживается* экспорт базы данных из пула.
...
- **management-endpoint** для API REST вызывает (диспетчер ресурсов): https://management.azure.com
