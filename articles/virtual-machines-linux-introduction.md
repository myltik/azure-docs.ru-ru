...
При создании виртуальной машины Linux с помощью портала управления Azure вам будет предложено указать имя пользователя, пароль и (при необходимости) открытый SSH-ключ. К имени пользователя при развертывании виртуальной машины Linux на платформе Azure применяется следующее ограничение: запрещается использовать имена учетных записей (идентификатор пользователя <100), которые уже используются в виртуальной машине (например, 'root').
 ...
 - См. статью [Использование SSH с Linux в Azure](linux-use-ssh-key.md)
...
6. При необходимости вы можете скопировать `myPrivateKey.key` в `~/.ssh/id_rsa`, чтобы клиент openssh мог автоматически получать к нему доступ без использования параметра -i.
   В качестве альтернативы можно изменить `~/.ssh/config`, включив в него раздел для вашей виртуальной машины:
...
- См. статью [Использование прав корневой учетной записи на виртуальных машинах Linux в Azure].(virtual-machines-linux-use-root-privileges.md)
...
Azure предоставляет фильтр входящих пакетов, который ограничивает возможности подключения к портам, указанным на портале управления. По умолчанию разрешен только порт SSH. Можно открыть доступ к дополнительным портам на виртуальной машине Linux, настроив конечные точки на портале управления:
 ...
 - См. [Настройка конечных точек виртуальной машины](virtual-machines-set-up-endpoints.md)
...
В Linux диск данных можно назвать по ядру, например, `/dev/sdc`, и пользователям придется выполнить разбиение на разделы, форматирование и подключение этого ресурса. Это подробно рассматривается в учебнике [Как подключить диск данных к виртуальной машине](virtual-machines-linux-how-to-attach-disk.md)

 - См. также: [Настройка программного RAID-массива в Linux](virtual-machines-linux-configure-raid.md)
