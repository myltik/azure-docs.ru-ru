...
2. Щелкните **Создать** на панели задач в нижней части страницы. Последовательно выберите пункты **Службы данных**, **Хранилище** и **Быстрое создание**.
...
4. С помощью поля **Расположение или территориальная группа** выберите расположение своей учетной записи хранения, наиболее близкое к вам или вашим клиентам. Если необходимо, чтобы данные из вашей учетной записи хранения были доступны для другой службы Azure (например, виртуальной машины или облачной службы Azure), рекомендуется выбирать территориальную группу, которая совпадает с территориальной группой вашей учетной записи хранения, чтобы данные и службы Azure, которые их обрабатывают, хранились в одном центре обработки данных, так как это позволит повысить производительность и снизить расходы.
	...
	Обратите внимание, что территориальная группа выбирается во время создания учетной записи хранения. Переместить существующую учетную запись в другую территориальную группу невозможно. Дополнительные сведения о территориальных группах см. в разделе [Размещение служб в территориальные группы](#service-co-location-with-an-affinity-group) ниже.

	>[AZURE.IMPORTANT]Чтобы узнать, какие расположения доступны для вашей подписки, вызовите операцию [List Providers](https://msdn.microsoft.com/library/azure/dn790524.aspx). Чтобы просмотреть список поставщиков из PowerShell, вызовите операцию [Get-AzureLocation](https://msdn.microsoft.com/library/azure/dn757693.aspx). В .NET используйте метод [List](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.provideroperationsextensions.list.aspx) класса ProviderOperationsExtensions.
	>
	>Кроме того, в разделе [Регионы Azure](https://azure.microsoft.com/ru-RU/regions/#services) представлены дополнительные сведения о доступности служб в разных регионах.
...
Каждый объект, который хранится в службе хранилища Azure, имеет уникальный URL-адрес. Имя учетной записи хранения образует поддомен этого адреса. Имя поддомена и доменное имя, уникальное для каждой службы, вместе образуют *конечную точку* вашей учетной записи хранения.
...
**Службы мультимедиа**. Если у вас есть службы мультимедиа, зависящие от учетной записи хранения, повторно созданные ключи доступа необходимо еще раз синхронизировать со службой мультимедиа.

**Приложения**. Если у вас есть веб-приложения или облачные службы, использующие учетную запись хранения, повторно созданные ключи необходимо обновить, иначе подключения будут разорваны. Процесс приведен ниже.
