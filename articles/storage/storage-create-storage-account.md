<properties
	...
	description="Узнайте, как на портале Azure создать или удалить учетную запись хранения, а также управлять ею."
	services="storage"
	documentationCenter=""
	authors="tamram"
	manager="adinah"
...
- Учетная запись хранения класса Premium в настоящее время поддерживает только диски виртуальных машин Azure. Подробный обзор хранилища класса Premium см. в статье [Хранилище Premium: высокопроизводительное хранилище для рабочих нагрузок виртуальных машин Azure](storage-premium-storage-preview-portal.md).
...
На странице [Цены Azure](http://azure.microsoft.com/pricing/details/#storage) предоставлена подробная информация о ценах на объемы хранилища, репликацию и транзакции. Подробные сведения о стоимости исходящих данных см. на странице [Сведения о ценах на передачу данных](http://azure.microsoft.com/pricing/details/data-transfers/).

Дополнительные сведения об объеме и целевых показателях производительности, предусмотренных для учетной записи хранения, см. в статье [Целевые показатели по производительности и масштабируемости для хранилища Azure](http://msdn.microsoft.com/library/azure/dn249410.aspx).
...
3. В поле **URL-адрес** введите имя своей учетной записи хранения.
	...
	Сведения об использовании имени учетной записи хранения для доступа к объектам в службе хранилища Azure можно найти в разделе [Конечные точки учетной записи хранения](#storage-account-endpoints) ниже.

4. С помощью поля **Расположение или территориальная группа** выберите расположение своей учетной записи хранения, наиболее близкое к вам или вашим клиентам. Если данные из вашей учетной записи хранения будут использоваться другими службами Azure (например, виртуальными машинами или облачной службой Azure), рекомендуется выбирать территориальную группу, которая совпадает с территориальной группой вашей учетной записи хранения. Таким образом данные и используемые службы Azure будут храниться в одном центре обработки данных, что позволит повысить производительность и снизить расходы.

	Обратите внимание, что территориальная группа выбирается во время создания учетной записи хранения. Переместить существующую учетную запись в другую территориальную группу невозможно. Дополнительные сведения о территориальных группах см. в разделе [Группирование служб в территориальные группы](#service-co-location-with-an-affinity-group) ниже.

	>[AZURE.IMPORTANT]Чтобы просмотреть список поставщиков из PowerShell, вызовите операцию [List Providers](https://msdn.microsoft.com/library/azure/dn790524.aspx). Чтобы определить, какие расположения доступны для подписки, вы можете вызвать операцию [Get-AzureLocation](https://msdn.microsoft.com/library/azure/dn757693.aspx). В .NET используйте метод [List](https://msdn.microsoft.com/library/azure/microsoft.azure.management.resources.provideroperationsextensions.list.aspx) класса ProviderOperationsExtensions.
	>
	>Кроме того, в разделе [Регионы Azure](https://azure.microsoft.com/ru-RU/regions/#services) представлены дополнительные сведения о том, какие службы доступны в разных регионах.
...
6. В поле **Репликация** выберите желаемый уровень репликации для своей учетной записи хранения. Рекомендуется использовать геоизбыточную репликацию, так как она обеспечивает наибольшую сохранность ваших данных. Дополнительные сведения о параметрах репликации службы хранилища Azure см. в статье [Репликация службы хранилища Azure](storage-redundancy.md).
...
Каждый объект, который хранится в службе хранилища Azure, имеет уникальный URL-адрес. Имя учетной записи хранения образует поддомен этого адреса. Сочетание имен поддомена и домена, уникальное для каждой службы, формирует *конечную точку* вашей учетной записи хранения.
...
Конечные точки вашей учетной записи хранения можно увидеть на панели мониторинга портала Azure сразу после создания учетной записи.
...
Войдите на [портал Azure](http://manage.windowsazure.com) и на панели мониторинга или странице **Хранилище** щелкните **Управление ключами**. Это даст вам возможность просмотреть, скопировать или повторно создать ключи доступа к хранилищу, которые используются для получения доступа к службам BLOB-объектов, таблиц и очередей.

### Копирование ключа доступа к хранилищу  

Можно также использовать функцию **Управление ключами** для копирования ключа доступа к хранилищу с целью использования этой функции в строке подключения. Для строки подключения требуется имя учетной записи хранения и ключ, которые задействуются в проверке подлинности. Сведения о настройке строк подключения к службам хранилища Azure см. в статье [Настройка строк подключения для Azure](http://msdn.microsoft.com/library/azure/ee758697.aspx).

1. На [портале Azure](http://manage.windowsazure.com) нажмите **Хранилище**, а затем щелкните имя учетной записи хранения, чтобы открыть панель мониторинга.
...
Чтобы обеспечить постоянную безопасность подключения к хранилищу, рекомендуем периодически изменять ключи доступа к учетной записи хранения. Назначаются два ключа доступа, которые позволяют обслуживать подключения к учетной записи хранения с помощью одного ключа доступа во время переназначения другого.
...
**Службы мультимедиа**. Если у вас есть службы мультимедиа, зависящие от учетной записи хранения, необходимо повторно синхронизировать ключи доступа со службой мультимедиа после повторного создания ключей.

**Приложения**. Если у вас есть веб-приложения или облачные службы, которые используют учетную запись хранения, после повторного создания ключей подключения будут утрачены, если только ключи не будут переключены. Процесс приведен ниже.

1. Обновите строки подключения в коде приложения, чтобы ссылаться на дополнительный ключ доступа учетной записи хранения.

2. Повторное создание основного ключа доступа для вашей учетной записи хранения. На [портале Azure](http://manage.windowsazure.com) перейдите на панель мониторинга или страницу **Настройка** и нажмите **Управление ключами**. В разделе основного ключа доступа щелкните **Создать повторно**, а затем нажмите кнопку **Да**, чтобы подтвердить создание нового ключа.
...
1. На [портале Azure](http://manage.windowsazure.com) нажмите **Хранилище**.
...
- Дополнительные сведения о службе хранилища Azure см. в соответствующей документации на сайтах [Azure.com](http://azure.microsoft.com/documentation/services/storage/) и [MSDN](http://msdn.microsoft.com/library/azure/gg433040.aspx).
