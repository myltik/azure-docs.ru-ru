...
На странице [Цены Azure](http://azure.microsoft.com/pricing/details/#storage) содержится информация о ценах на объемы хранилища, репликацию и транзакции. Подробные сведения о стоимости исходящих данных см. на странице [Сведения о ценах на передачу данных](http://azure.microsoft.com/pricing/details/data-transfers/).

Дополнительную информацию о емкости и пропускной способности учетной записи хранения см. в статье [Целевые показатели по производительности и масштабируемости для хранилища Azure](http://msdn.microsoft.com/library/azure/dn249410.aspx).
...
4. С помощью поля **Расположение или территориальная группа** выберите расположение своей учетной записи хранения, наиболее близкое к вам или вашим клиентам. Если данные из вашей учетной записи хранения будут использоваться другими службами Azure (например, виртуальными машинами или облачной службой Azure), рекомендуется выбирать территориальную группу, которая совпадает с территориальной группой вашей учетной записи хранения. Таким образом данные и используемые службы Azure будут храниться в одном центре обработки данных, что позволит повысить производительность и снизить расходы.
	...
	Обратите внимание, что территориальная группа выбирается во время создания учетной записи хранения. Переместить существующую учетную запись в другую территориальную группу невозможно. Дополнительную информацию о территориальных группах см. в разделе [Группирование служб в территориальные группы](#service-co-location-with-an-affinity-group) ниже.
...
6. В поле **Репликация** выберите желаемый уровень репликации для своей учетной записи хранения. Рекомендуется использовать геоизбыточную репликацию, так как она обеспечивает наибольшую сохранность ваших данных. Дополнительную информацию о параметрах репликации службы хранилища Azure см. в статье [Репликация службы хранилища Azure](storage-redundancy.md).
...
Для своей учетной записи хранения вы можете настроить имя домена. Дополнительную информацию см. в статье [Настройка имени домена для BLOB-данных в учетной записи хранения Azure](storage-custom-domain-name.md).
...
> [AZURE.NOTE]Чтобы создать территориальную группу, откройте раздел <b>Параметры</b> на портале Azure, перейдите на страницу <b>Территориальные группы</b> и затем щелкните <b>Добавить территориальную группу</b> или <b>Добавить</b>. Вы также можете создавать территориальные группы и управлять ими с помощью API управления службами Azure. Дополнительную информацию см. в статье <a href="http://msdn.microsoft.com/library/azure/ee460798.aspx">Операции с территориальными группами</a>.
...
На [портале Azure](http://manage.windowsazure.com) на панели мониторинга или странице **Хранилище** щелкните **Управление ключами**. Это даст вам возможность просмотреть, скопировать или повторно создать ключи доступа к хранилищу, которые используются для получения доступа к службам BLOB-объектов, таблиц и очередей.
...
1. На [портале Azure](http://manage.windowsazure.com) щелкните **Хранилище**, а затем щелкните имя учетной записи хранения, чтобы открыть панель мониторинга.
...
1. На [портале Azure](http://manage.windowsazure.com) щелкните **Хранилище**.
