...
Запрос к службе таблиц может возвращать максимум 1000 сущностей одновременно, а время его выполнения не превышает 5 секунд. Если результирующий набор содержит более 1000 сущностей, если запрос выполняется более 5 секунд или выходит за пределы раздела, служба таблиц возвращает маркер продолжения, чтобы клиентское приложение смогло выполнить запрос к следующему набору сущностей. Дополнительную информацию о принципе действия маркеров продолжения см. в статье [Query Timeout and Pagination](http://msdn.microsoft.com/library/azure/dd135718.aspx) (Время ожидания запроса и разбивка на страницы).
