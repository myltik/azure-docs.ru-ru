...
Хранилище файлов Azure предлагает доступ к общим папкам в облаке с использованием стандартного [протокола SMB](https://msdn.microsoft.com/library/windows/desktop/aa365233.aspx). Это хранилище файлов теперь стало общедоступным и поддерживает протоколы SMB 2.1 и SMB 3.0.

Вы можете создавать общие папки Azure с помощью [портала Azure](portal.azure.com), командлетов PowerShell службы хранилища Azure, клиентских библиотек службы хранилища Azure и интерфейса REST API службы хранилища Azure. Кроме того, так как для этих общих папок используется протокол SMB, вы можете получать к ним доступ с помощью стандартных и знакомых API-интерфейсов файловой системы.

Приложения, работающие в Azure, могут легко подключать общие папки из виртуальных машин Azure. Кроме того, с помощью общедоступной версии хранилища файлов можно подключить общую папку на локальном клиенте, поддерживающем протокол SMB 3.0 (например, на локальном ноутбуке, настольном компьютере и сервере).

Хранилище файлов Azure основано на той же технологии, что и хранилища BLOB-объектов, таблиц и очередей, поэтому оно может использовать все средства обеспечения доступности, надежности, масштабируемости и геоизбыточности, встроенные в платформу хранилища Azure.
...
Дополнительные сведения о целевых показателях масштабируемости и производительности для хранилища файлов см. в статье [Цели масштабируемости хранилища и производительности Azure](storage-scalability-targets.md#scalability-targets-for-standard-storage-accounts).
...
На [портале Azure](portal.azure.com) общими папками можно управлять с помощью пользовательского интерфейса. На портале вы можете выполнять следующие действия:
...
2. В меню навигации щелкните пункт **Учетные записи хранения** или **Учетные записи хранения (классические)**.
...
3. Щелкните файл правой кнопкой мыши и выберите команду **Сказать**, чтобы скачать его в локальное хранилище.
...
1. Нажмите кнопку **Квота** для изменения размера общей папки (до 5120 ГБ).

	![Снимок экрана, показывающий, как настроить квоту общей папки](./media/storage-dotnet-how-to-use-files/files-manage-1.png)

2. Нажмите кнопку **Подключение**, чтобы вызвать командную строку для подключения общей папки из Windows.
	...
	>[AZURE.TIP]Чтобы найти ключ доступа к учетной записи хранения для подключения, нажмите кнопку **Параметры** в своей учетной записи хранения и выберите пункт **Ключи доступа**.
