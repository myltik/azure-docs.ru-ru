...
Подробные сведения об использовании хранилища файлов Azure в ОС Linux см. в статье [Использование хранилища файлов Azure в Linux](storage-how-to-use-files-linux.md).

Дополнительную информацию о целевых показателях масштабируемости для хранилища файлов см. в статье [Целевые показатели масштабируемости и производительности хранилища Azure](storage-scalability-targets.md#scalability-targets-for-standard-storage-accounts).
...
Хранение файлов теперь поддерживается для всех учетных записей хранения. Вы можете использовать существующую учетную запись хранения или создать новую. Дополнительные сведения о создании новой учетной записи хранения см. в статье [Об учетных записях хранения Azure](storage-create-storage-account.md#create-a-storage-account).

## Управление общей папкой с помощью портала предварительной версии Azure

[Портал предварительной версии Azure](https://ms.portal.azure.com/) предоставляет пользовательский интерфейс, с помощью которого клиенты могут управлять хранилищем файлов. На портале предварительной версии вы можете выполнять следующие действия:
...
- получать команду `net use`, которая позволит подключить общую папку с клиентского компьютера Windows. 
...
> [AZURE.IMPORTANT]Имя общей папки должно состоять из символов в нижнем регистре. Полную информацию о присвоении имен общим папкам и файлам см. в статье [Именование общих ресурсов, каталогов, файлов и метаданных и ссылка на них](https://msdn.microsoft.com/library/azure/dn167011.aspx).
...
Когда клиент обращается к хранилищу файлов, используется та версия SMB, которая поддерживается операционной системой. В таблице ниже перечислены версии, поддерживаемые клиентами Windows. Дополнительные сведения см. в блоге << Which version of the SMB protocol blog post>>.
...
1. Прежде всего создайте виртуальную машину Azure, следуя инструкциям, приведенным в статье [Создание виртуальной машины под управлением Windows на портале предварительной версии Azure](../virtual-machines-windows-tutorial.md).
2. Затем установите удаленное подключение к виртуальной машине, следуя инструкциям в статье [Вход в виртуальную машину под управлением Windows Server](../virtual-machines-log-on-windows-server.md).
...
Windows автоматически восстановит подключение к общему ресурсу файлов при перезагрузке виртуальной машины. Вы можете проверить, восстанавливается ли подключение к общей папке, выполнив команду `net use` в окне PowerShell.
...
Установив удаленное подключение к виртуальной машине, вы можете подключить общую папку с помощью команды `net use`, используя синтаксис из примера ниже. Замените `<storage-account-name>` именем учетной записи хранения, а `<share-name>` именем вашего ресурса хранилища файлов.
...
Для программной работы с хранилищем файлов можно использовать клиентские библиотеки хранилища для .NET и Java или REST API службы хранилища Azure. В этом разделе приведен пример работы с общей папкой из простого консольного приложения, которое выполняется на настольном компьютере. В этом примере используется [клиентская библиотека хранилища Azure .NET](http://go.microsoft.com/fwlink/?LinkID=390731&clcid=0x409).
...
1. В Visual Studio последовательно выберите элементы **Файл -> Создать проект**, а затем в списке шаблонов Visual C# выберите пункт **Windows -> Консольное приложение**.
...
Затем сохраните данные учетной записи хранения в файле app.config вашего проекта. Измените содержимое файла app.config, как показано в следующем примере: вместо `myaccount` следует указать имя учетной записи хранения, а вместо `mykey` — ключ учетной записи хранения.
...
Дополнительные сведения о создании и использовании подписанных URL-адресов см. в статьях [Подписи общего доступа. Общие сведения о модели SAS](storage-dotnet-shared-access-signature-part-1.md) и [Подписи общего доступа. Создание и использование подписи общего доступа с помощью службы BLOB-объектов](storage-dotnet-shared-access-signature-part-2.md).
...
Метрики для хранилища файлов вы можете включить на портале Azure. Кроме того, вы можете включить метрики программным путем. Для этого используйте операцию Set File Service Properties через интерфейс REST API или любой ее аналог из имеющихся в клиентской библиотеке хранилища.
...
12. **Скорость работы при попытке распаковать файлы в хранилище файлов оказалась низкой. Что делать?**
