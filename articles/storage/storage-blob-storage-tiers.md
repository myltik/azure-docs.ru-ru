<properties
    ...
    description="Уровни хранилища BLOB-объектов обеспечивают экономичное хранение данных объектов в зависимости от шаблона доступа. ";Холодный"; уровень хранилища Azure оптимизирован для данных, доступ к которым осуществляют нечасто."
    services="storage"
    documentationCenter=""
...
Учетные записи хранения BLOB-объектов используют атрибут **уровня доступа**, который позволяет задавать **горячий** или **холодный** уровень хранилища в зависимости от данных, хранящихся в учетной записи. Если нужно изменить шаблон использования данных, вы в любое время можете изменить уровень доступа.

> [AZURE.NOTE] За изменение уровня доступа может взиматься дополнительная плата. Дополнительные сведения см. в разделе [Цены и выставление счетов](storage-blob-storage-tiers.md#pricing-and-billing).

Вот некоторые примеры сценариев использования, при которых данные нужно хранить на "горячем" уровне доступа к хранилищу:
...
Вот некоторые примеры сценариев использования, при которых данные нужно хранить на "холодном" уровне доступа к хранилищу:
...
<tr>
    ...
    <td><strong><center>"Горячий" уровень доступа к хранилищу</center></strong></td>
    <td><strong><center>"Холодный" уровень доступа к хранилищу</center></strong>&lt;/td
</tr>
<tr>
...
- **Расходы на хранение**. Плата за хранение данных зависит не только от их объема, но и от уровня доступа. Для "холодного" уровня доступа к хранилищу цена за гигабайт ниже, чем для "горячего" уровня доступа.
- **Расходы на доступ к данным**. На "холодном" уровне доступа к хранилищу взимается плата за гигабайт данных для операций чтения и записи.
- **Расходы на транзакции**. На обоих уровнях взимается плата за каждую транзакцию. Тем не менее стоимость транзакции для "холодного" уровня доступа к хранилищу больше, чем для "горячего".
...
7. Выберите уровень доступа: **горячий** или **холодный**. По умолчанию используется значение **Горячий**.
...
#### Изменение уровня доступа в учетной записи хранилища BLOB-объектов с использованием портала Azure
...
4. Выберите необходимый уровень доступа: **горячий** или **холодный**.
...
Этот раздел создан, чтобы помочь пользователям плавно перейти на учетную запись хранения BLOB-объектов. Учетная запись хранения BLOB-объектов предназначена для хранения только блочных и добавочных BLOB-объектов. Существующие учетные записи хранения общего назначения, которые также позволяют хранить таблицы, очереди, файлы, диски, а также большие двоичные объекты, нельзя преобразовать в учетные записи хранения BLOB-объектов. Чтобы использовать уровни хранилища, вам необходимо будет создать учетные записи хранения BLOB-объектов и перенести существующие данные в новые учетные записи.

### Планирование миграции существующих данных

При перемещении данных в учетную запись хранения BLOB-объектов, вероятно, вы захотите использовать преимущества "холодного" уровня доступа к хранилищу, чтобы сэкономить на хранении редко используемых данных. Планируя перенос данных в учетную запись хранения BLOB-объектов на "холодный" уровень доступа к хранилищу, в первую очередь нужно определить текущий шаблон использования, чтобы узнать, получите ли вы преимущества, перейдя на такую учетную запись. В общем, необходимо выяснить следующее:
...
Дополнительные сведения см. в разделе [Azure Storage Data Movement Library for .NET](https://github.com/Azure/azure-storage-net-data-movement) (Библиотека перемещения данных службы хранилища Azure для .NET).
...
4. **Можно ли хранить объекты на обоих уровнях доступа в одной учетной записи?**
    ...
    На уровне учетной записи устанавливается атрибут уровня доступа, который применяется ко всем ее объектам. Уровень доступа нельзя задать на уровне объектов.
...
7. **Будет ли поведение больших двоичных объектов на "холодном" уровне доступа к хранилищу отличаться от поведения объектов на "горячем" уровне?**

    Для больших двоичных объектов на "горячем" уровне доступа к хранилищу характерна такая же задержка, как для больших двоичных объектов в учетных записях хранения общего назначения. Для больших двоичных объектов "холодного" уровня доступа к хранилищу характерна такая же задержка (в миллисекундах), как для больших двоичных объектов в учетных записях хранения общего назначения.

    Доступность больших двоичных объектов на "холодном" уровне доступа более низкая (согласно соглашению об уровне обслуживания), чем у больших двоичных объектов, хранящихся на "горячем" уровне доступа к хранилищу. Дополнительные сведения см. в статье [Соглашение об уровне обслуживания для хранилища](https://azure.microsoft.com/support/legal/sla/storage).
...
10. **Будет ли взиматься плата за работу пользователей?**
    ...
    Учетные записи хранения BLOB-объектов аналогичны учетным записям хранения общего назначения, что касается хранения блочных и добавочных BLOB-объектов, и поддерживают все ключевые функции службы хранилища Azure, в том числе обеспечение высокой надежности и доступности, а также масштабируемости, производительности и безопасности. Кроме функций, определенных ограничений для учетных записей хранения BLOB-объектов, а также уровней доступа, описанных выше, они ничем не отличаются.
