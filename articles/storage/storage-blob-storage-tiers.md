...
Учетные записи хранения больших двоичных объектов используют атрибут **уровня доступа**, который позволяет задавать **горячий** или **холодный** уровень хранилища в зависимости от данных, хранящихся в учетной записи. Если схема использования данных изменилась, вы в любое время можете изменить уровень хранилища.
...
> [!NOTE]
> Сейчас учетные записи хранения BLOB-объектов поддерживаются в большинстве регионов Azure, и планируется расширить поддержку. Обновленный список регионов доступности см. на странице [Доступность продуктов по регионам](https://azure.microsoft.com/regions/#services).
> 
> 
...
<tr>
    <td><strong><center>Целевые показатели масштабируемости и производительности хранилища<center></strong></td>
...
Если эти метрики включены, данные о емкости ежедневно записываются для службы учетной записи хранения больших двоичных объектов. Эти данные регистрируются как запись, которая записывается в таблицу *$MetricsCapacityBlob* в той же учетной записи хранения.
...
Чтобы правильно определить приблизительное потребление данных и схему доступа, рекомендуем выбрать период хранения для метрик, представляющих регулярное использование, и экстраполировать эти данные.
Один из вариантов — хранить данные метрик в течение 7 дней и собирать данные каждую неделю для анализа в конце месяца.
Альтернативный вариант — хранить данные метрик за последние 30 дней и собирать и анализировать данные в конце 30-дневного периода.
