...
Сценарий автоматизации предназначен для развертывания Azure Resource Manager вместо операций с содержимым хранилища. Дополнительные сведения см. в статье [Развертывание ресурсов с использованием шаблонов Resource Manager и Azure PowerShell](../../azure-resource-manager/resource-group-template-deploy.md).

**Существует ли какая-либо плата за копирование данных между двумя разными файловыми ресурсами в одной учетной записи хранения в одном регионе?**
...
3.   Затем выполните следующую команду, чтобы переместить контейнер. Замените текст фактическим значением.   
     
            AzCopy /Source:https://sourceaccount.blob.core.windows.net/mycontainer1
            /Dest:https://destaccount.blob.core.windows.net/mycontainer2
            /SourceKey:key1 /DestKey:key2 /S

        - /source: укажите URI исходной учетной записи хранения (до контейнера)  
        - /dest: укажите URI целевой учетной записи хранения (до контейнера)  
        - /sourcekey: предоставьте первичный ключ исходной учетной записи хранения, который можно скопировать на портале, выбрав учетную запись хранения.  
        - /destkey: предоставьте первичный ключ целевой учетной записи хранения, который можно скопировать на портале, выбрав учетную запись хранения.

После выполнения этой команды файлы контейнера перемещаются в целевую учетную запись хранения.

**CLI AzCopy не работает вместе с переключателем Pattern, когда вы выполняете копирование из одного большого двоичного объекта Azure в другой.**

Можно непосредственно скопировать и изменить командную строку AzCopy и перекрестно проверить, что шаблон соответствует источнику. Также убедитесь, что подстановочные знаки **/S** работают. Дополнительные сведения см. в статье [Перенос данных с помощью AzCopy для Windows](storage-use-azcopy.md).
...
2.  Используйте [Azcopy](https://azure.microsoft.com/en-us/blog/azcopy-5-1-release/), чтобы скопировать содержимое из исходного контейнера больших двоичных объектов в другой контейнер больших двоичных объектов.

**Как создать сценарий PowerShell для перемещения данных из общего файлового ресурса Azure в другое хранилище Azure?**

Используйте AzCopy для перемещения данных из общего файлового ресурса Azure в другое хранилище Azure. Дополнительные сведения см. в статьях [Перенос данных с помощью AzCopy для Windows](storage-use-azcopy.md) и [Перенос данных с помощью AzCopy для Linux](storage-use-azcopy-linux.md).
...
**Как перенести данные из локальной среды в службу файлов Azure**?

Используйте AzCopy для перемещения данных. Дополнительные сведения см. в статьях [Перенос данных с помощью AzCopy для Windows](storage-use-azcopy.md) и [Перенос данных с помощью AzCopy для Linux](storage-use-azcopy-linux.md).

**Как сопоставить папку контейнера на виртуальной машине?**

Используйте общий файловый ресурс Azure.

**Как создать резервную копию файлового хранилища Azure?**

Решение для резервного копирования отсутствует. Однако служба файлов Azure также поддерживает асинхронное копирование. Таким образом, вы можете копировать файлы из общего ресурса в другой общий ресурс (в учетной записи хранения или в другую учетную запись) или в контейнер BLOB-объектов (в учетной записи хранения или в другую учетную запись).
...
3.  Создайте управляемый диск, используя файл VHD в другом регионе, в который вы скопировали VHD. Это можно сделать с помощью следующего сценария Azure PowerShell.  
...
При создании учетной записи хранения вы выбираете первичный регион для учетной записи. Выбор вторичного региона определяется на основе первичного региона, его невозможно изменить. Ознакомьтесь со статьей [Репликация службы хранилища Azure](storage-redundancy.md).
...
После включения шифрования в учетной записи хранения имеющиеся данные не шифруются. Чтобы выполнить шифрование имеющихся данных, необходимо снова загрузить данные в учетную запись хранения.  Для этого выполните следующие действия.

Используйте AZcopy, чтобы скопировать данные в другую учетную запись и затем вернуться к первой учетной записи. Можно также использовать [шифрование неактивных данных](storage-service-encryption.md).
...
**Существуют ли какие-либо предварительные требования для изменения репликации учетной записи хранения с GRS на LRS?**

Нет. 

**Как получить доступ к резервному хранилищу службы файлов Azure?**

Для доступа к избыточному хранилищу требуется геоизбыточное хранилище с доступом для чтения (RA-GRS). Тем не менее служба файлов Azure поддерживает только LRS и стандартное GRS без доступа только для чтения. 

**Как перейти из хранилища класса Premium в хранилище класса "Стандарт"?**

Для этого выполните следующие действия.

1.  Создайте учетную запись хранилища класса "Стандарт" (или вы можете использовать имеющуюся учетную запись в своей подписке).
...
2.  Скачайте AzCopy. Выполните следующие команды AzCopy:
    ...
    Чтобы скопировать только один диск, введите имя диска в шаблон.
...
Чтобы убедиться, что передача завершена успешно, изучите контейнер учетной записи целевого хранилища на портале Azure. После копирования дисков в стандартную учетную запись хранения можно подключить их к виртуальной машине в качестве имеющегося диска. Дополнительные сведения см. в статье [Как подключить управляемый диск данных к виртуальной машине Windows на портале Azure](../../virtual-machines/windows/attach-managed-disk-portal.md).  
  
**Как преобразовать общий файловый ресурс в хранилище класса Premium?**

Хранилище класса Premium не предоставляется для общей папки Azure.

**Как обновить учетную запись хранения уровня "Стандарт" до учетной записи уровня "Премиум"? Как перейти с использования учетной записи хранения уровня "Премиум" на учетную запись уровня "Стандарт"?**

- Необходимо создать целевую учетную запись хранения, скопировать данные из исходной учетной записи в целевую учетную запись и удалить исходную учетную запись.

- Для копирования данных можно использовать такой инструмент, как AzCopy.
...
-   Если вы установили 32-разрядную версию, она будет расположена здесь: **%ProgramFiles(x86)%\\Microsoft SDKs\\Azure\\AzCopy**.

-   Если вы установили 64-разрядную версию, она будет расположена здесь: **%ProgramFiles%\\Microsoft SDKs\\Azure\\AzCopy**.

**Для реплицированной учетной записи хранения (например, ZRS, GRS или RA-GRS) как получить доступ к данным, которые хранятся во вторичном регионе?**

-   Если вы используете хранилище, избыточное в пределах зоны (ZRS) или геоизбыточное хранилище (GRS), вы не можете получить доступ к данным из вторичной области, если не произойдет отработка отказа. Дополнительные сведения о процессе отработки отказа см. в разделе [Что произойдет в случае отработки отказа хранилища](storage-disaster-recovery-guidance.md#what-to-expect-if-a-storage-failover-occurs).

-   При использовании геоизбыточного хранилища с доступом на чтение (**RA-GRS**) данные доступны из вторичного региона в любое время. Для этого используйте один из следующих методов.  
      
    AzCopy: добавьте -secondary к имени учетной записи хранения в URL-адресе для доступа к вторичной конечной точке. Например:  
...
SSL в настоящее время не поддерживается в учетных записях хранения с настраиваемыми доменами.
...
Если вы хотите только загружать данные без использования проводника хранилища или аналогичного приложения, они могут использовать токен SAS. Дополнительные сведения см. в статье [Использование подписанных URL-адресов (SAS)](storage-dotnet-shared-access-signature-part-1.md).
