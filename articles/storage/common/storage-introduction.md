...
Дополнительные сведения о защите учетной записи хранения и шифровании см. в [руководстве по безопасности службы хранилища Azure](storage-security-guide.md).

## <a name="replication"></a>Репликация

Чтобы обеспечить сохранность данных, служба хранилища Azure будет хранить несколько копий данных (и управлять ими). Этот процесс называется репликацией (иногда избыточностью). При настройке учетной записи хранения вы выбираете тип репликации. В большинстве случаев этот параметр можно изменить и после того, как учетная запись хранения будет настроена.

**Локально избыточное хранилище**

Локально избыточное хранилище (LRS) обеспечивает устойчивость объектов как минимум на уровне 99,999999999 % (11 девяток) в течение определенного года. Это означает, что служба хранилища Azure управляет несколькими копиями данных в центре обработки данных, указанном при настройке учетной записи хранения. Когда фиксируются изменения, перед сообщением об успешном выполнении обновляются все копии. Это означает, что реплики постоянно синхронизируются. Кроме того, копии находятся в отдельных доменах сбоя и обновления. Поэтому данные доступны, даже если узел хранилища, в котором они содержатся, выходит из строя или переводится в автономный режим для обновления.

**Хранилище, избыточное в пределах зоны (предварительная версия)**

Хранилище, избыточное в пределах зоны, (ZRS) упрощает развертывание приложений с высоким уровнем доступности. ZRS обеспечивает устойчивость объектов хранилища как минимум на уровне 99,9999999999 % (12 девяток) в течение определенного года. ZRS позволяет синхронно реплицировать данные в нескольких зонах доступности. Используйте ZRS для сценариев с транзакционными приложениями, когда простои недопустимы. ZRS позволяет клиентам считывать и записывать данные, даже если одна зона недоступна или ее невозможно восстановить. Вставка и обновление данных выполняются синхронно и строго согласованно.    

Предыдущее предложение ZRS теперь называется классическим хранилищем ZRS. Классические учетные записи ZRS доступны только для блочных BLOB-объектов в учетных записях хранения общего назначения версии 1. В классическом хранилище ZRS данные асинхронно реплицируются в центрах обработки данных в пределах одного или двух регионов. Реплика будет недоступна, если корпорация Майкрософт не инициирует отработку отказа в дополнительном регионе. Классическую учетную запись ZRS невозможно преобразовать в учетную запись хранилища LRS или GRS и наоборот. Кроме того, она не поддерживает использование метрик или ведение журналов.

**Геоизбыточное хранилище (GRS)**

Геоизбыточное хранилище (GRS) обеспечивает устойчивость объектов на уровне 99,99999999999999 % (16 девяток) в течение определенного года и поддерживает локальные копии данных в основном регионе, а также другой набор данных в дополнительном регионе, который на сотни километров удален от основного. В случае сбоя в основном регионе служба хранилища Azure выполнит отработку отказа в дополнительном регионе.
...
> [!IMPORTANT]
> Вы можете изменить способ репликации данных после создания учетной записи хранения. Но если вы переключаетесь с LRS или ZRS на GRS или RA-GRS, то это может привести к дополнительной одноразовой оплате за передачу данных.
>

Дополнительные сведения о вариантах репликации см. в статье [Репликация службы хранилища Azure](storage-redundancy.md).
