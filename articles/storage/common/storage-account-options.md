...
Служба хранилища Azure предоставляет три варианта учетной записи с разными ценами и поддерживаемыми возможностями. Очень важно, чтобы пользователи учитывали эти различия для определения подходящего варианта для своих приложений.  Ниже приведены три различных варианта.

* Учетные записи **общего назначения версии 2 (GPv2)** предоставляют все новейшие функции и поддерживают большие двоичные объекты, файлы, очереди и таблицы. В настоящее время к новым возможностям относятся распределение BLOB-объектов по уровням, архивное хранилище, повышенные пределы масштабирования учетной записи и события хранилища. Ценообразование было сформировано для обеспечения самых низких цен на гигабайт и конкурентоспособных цен на транзакции в отрасли.

* Учетные записи **хранилища BLOB-объектов** предоставляют новейшие возможности для блочных BLOB-объектов, однако поддерживают только их.  Цены в целом похожи на цену учетных записей общего назначения версии 2. Мы рекомендуем пользователям использовать учетные записи общего назначения версии 2 вместо хранилища BLOB-объектов.

* Учетные записи **общего назначения версии 1 (GPv1)** обеспечивают использование всех служб хранилища Azure, однако не предоставляют новейших возможностей или низких цен за гигабайт. Например, "холодный" и архивный уровни хранилища не поддерживаются в GPv1.  Цены для транзакций ниже, поэтому этот тип учетной записи выгоден для рабочих нагрузок с высокой скоростью изменения данных или высокой скоростью чтения.

### <a name="changing-account-kind"></a>Изменение типа учетной записи
Пользователи могут в любое время обновить учетную запись GPv1 до учетной записи GPv2 с помощью портала, CLI или PowerShell. Это изменение невозможно отменить, а другие изменения не разрешены. Возможность обновления учетных записей хранилища BLOB-объектов до учетных записей GPv2 ожидается в ближайшее время.

## <a name="general-purpose-v2"></a>Учетная запись общего назначения версии 2
Учетные записи **общего назначения v2 (GPv2)** являются учетными записями хранения, которые поддерживают все функции для всех служб хранилища, включая большие двоичные объекты, файлы, очереди и таблицы. Для блочных BLOB-объектов вы можете выбрать между горячим и холодным уровнями хранилища для учетной записи или между горячим, холодным и архивным уровнями для BLOB-объекта. Чтобы оптимизировать затраты, храните данные, к которым часто, иногда и редко обращаются, на горячем, холодном и активном уровнях хранилища соответственно. Важно то, что любую учетную запись GPv1 можно обновить до учетной записи GPv2 на портале, с помощью CLI или PowerShell. Учетные записи GPv2 поддерживают все API и функции, поддерживаемые учетными записями хранилища BLOB-объектов и GPv1, а также, подобно этим типам учетных записей, обеспечивают возможности устойчивости, доступности, масштабируемости и производительности.
...
**Учетные записи хранилища BLOB-объектов** поддерживают те же функции блочных BLOB-объектов, что и учетные записи GPv2. Однако эти учетные записи поддерживают лишь блочные BLOB-объекты. Клиентам лучше обратить внимание на различия в ценах между учетными записями хранилища BLOB-объектов и GPv2, а также рассмотреть возможность перехода на GPv2. Обратите внимание, что это обновление окончательное.

> [!NOTE]
> Учетные записи хранения BLOB-объектов поддерживают только блочные и добавочные BLOB-объекты и не поддерживают страничные.

## <a name="general-purpose-v1"></a>Учетная запись общего назначения версии 1
Учетные записи **общего назначения версии 1 (GPv1)** являются старой версией учетной записи хранения, и это единственный тип, который может использоваться в классической модели развертывания. Такие функции, как "холодный" и архивный уровни хранилища, недоступны для GPv1. GPv1 имеет более высокие цены на хранение за гигабайт, но меньшие цены на транзакции, чем в учетных записях хранения GPv2 или хранилища BLOB-данных.
...
* Преобразование учетной записи GPv1 или хранилища BLOB-объектов в учетную запись хранилища GPv2.
...
3. В колонке "Параметры" щелкните **Конфигурация**.

4. В разделе "Тип учетной записи" щелкните **Обновить**.

5. Справа появится новая колонка для подтверждения. В разделе "Подтвердить обновление" введите имя учетной записи. 
...
> [!NOTE]
> Учетные записи хранилища BLOB-объектов предоставляют конечную точку службы таблиц только для хранения данных метрик для этой учетной записи и доступа к этим данным. Учетные записи хранения GPv1 ZRS не поддерживают данные метрики.
...
Учетную запись GPv1 можно легко обновить до GPv2 без простоя или изменений API и без необходимости перемещения данных. Это одно из основных преимуществ GPv2 в сравнении с учетными записями хранилища BLOB-объектов.

Однако, если нужно выполнить миграцию в учетную запись хранилища BLOB-объектов, вы можете использовать приведенные ниже инструкции.
