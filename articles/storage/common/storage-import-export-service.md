...
В этой статье предоставляются пошаговые инструкции по использованию службы "Импорт и экспорт Azure" для безопасного переноса больших объемов данных в хранилище BLOB-объектов и файлов Azure при отправке дисков в центр обработки данных Azure. Кроме того, эту службу можно использовать, чтобы переносить данные из хранилища BLOB-объектов Azure на жесткие диски и передавать на локальный сайт. Данные с одного внутреннего диска SATA можно импортировать в хранилище BLOB-объектов Azure или хранилище файлов Azure. 

> [!IMPORTANT] 
> Эта служба принимает только внутренние жесткие диски SATA или SSD. Другие варианты устройств не поддерживаются. Не отправляйте внешние жесткие диски или запоминающие устройства, подключаемые к сети, так как они будут возвращены, если это возможно, или отменены.
>
>

Выполните следующие действия, если данные на диске необходимо импортировать в хранилище BLOB-объектов Azure.
...
1.  Определите данные, которые нужно импортировать в хранилище BLOB-объектов Azure. Это могут быть папки или отдельные файлы на локальном сервере или в общей сетевой папке.
...
4.  Создайте один том NTFS на каждом жестком диске и присвойте ему букву диска. Точки подключения не используются.
5.  Включите шифрование BitLocker на томе NTFS. Воспользуйтесь инструкциями на странице https://technet.microsoft.com/en-us/library/cc731549(v=ws.10).aspx, чтобы включить шифрование на компьютере с Windows.
6.  Полностью скопируйте данные на эти зашифрованные отдельные тома NTFS на дисках, используя копирование и вставку, перетаскивание, Robocopy или любое другое подобное средство.
7.  Скачайте WAImportExport версии 1 по адресу https://www.microsoft.com/en-us/download/details.aspx?id=42659.
...
10. Скопируйте приведенную ниже командную строку в блокнот и отредактируйте ее, чтобы создать командную строку.
  ./WAImportExport.exe PrepImport /j:JournalTest.jrn /id:session#1 /sk:***== /t:D /bk:*** /srcdir:D:\ /dstdir:ContainerName/ /skipwrite
    
    "/j:" — имя файла, называемого файлом журнала, в формате JRN. Файл журнала создается для каждого диска, поэтому рекомендуется использовать серийный номер диска в качестве имени файла журнала.
    "/sk:" — ключ учетной записи хранения Azure. "/t:" — буква отправляемого диска. Например, "D /bk:" — это ключ диска BitLocker, "/srcdir:" — буква отправляемого диска, за которой следует ":\". Например, "D:\".
    "/dstdir:" — имя контейнера службы хранилища Azure, в который будут импортироваться данные.
    "/skipwrite" — пропуск записи. 
    
11. Повторите шаг 10 для каждого диска, который необходимо отправить.
12. Файл журнала с именем, указанным с параметром /j:, создается при каждом запуске командной строки.

### <a name="step-2-create-an-import-job-on-azure-portal"></a>Шаг 2. Создание задания импорта на портале Azure

1. Войдите на портал по адресу https://portal.azure.com/ и выберите "Больше служб -> Хранилище -> Import/export jobs (Задания импорта и экспорта)", а затем щелкните **Create Import/export Job** (Создать задание импорта или экспорта).
...
Для отправки пакета в центр обработки данных Azure можно использовать службы FedEx, UPS или DHL.
