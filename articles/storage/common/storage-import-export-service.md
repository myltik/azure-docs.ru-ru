...
* Создайте задание экспорта, если необходимо передать данные, которые в настоящее время хранятся в качестве больших двоичных объектов вашей учетной записи хранения, на отправленных нам жестких дисках. При создании задания вы уведомляете службу импорта и экспорта о том, что отправите один или несколько жестких дисков в центр обработки данных Azure.
...
FedEx, UPS или DHL можно использовать для отправки дисков по адресу доставки.
...
1. После подготовки диска на портале Azure перейдите в раздел "Дополнительные службы -> Хранилище -> Import/export jobs" (Задания импорта и экспорта). Нажмите кнопку **Create Import/export Job** (Создать задание импорта или экспорта).

2. На шаге 1 "Основные сведения" выберите Import into Azure (Импорт в Azure), введите строку для имени задания, выберите подписку, введите или выберите группу ресурсов. Введите описательное имя для задания импорта. Обратите внимание,что вводимое имя может содержать только строчные буквы, цифры, дефисы и подчеркивания, кроме того, оно должно начинаться с буквы и не содержать пробелов. Выбранное имя будет использоваться для отслеживания выполняемых заданий, а также после их завершения.

3. На шаге 2 "Основные сведения о задании" передайте файлы журналов диска, полученные на этапе подготовки диска. Если использовалась версия 1 waimportexport.exe, необходимо передать один файл для каждого подготовленного диска. Выберите учетную запись хранилища, в которую будут импортированы данные, в разделе учетной записи хранения Import destination (Место назначение импорта). Расположение места назначения будет автоматически заполнено с учетом выбранной области учетной записи хранения.
   
   ![Создание задание импорта – шаг 3](./media/storage-import-export-service/import-job-03.png)
4. На шаге 3 "Информация об обратной доставке" выберите перевозчик из раскрывающегося списка и введите его доступный номер учетной записи, созданной с помощью этого оператора. Майкрософт будет использовать эту учетную запись для отправки дисков обратно после завершения задания импорта. Укажите полное и допустимое имя контактного лица, телефон, адрес электронной почты, адрес, город, почтовый индекс, страну (провинцию) и страну (регион).
