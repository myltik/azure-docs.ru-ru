...
---
# <a name="use-the-microsoft-azure-importexport-service-to-transfer-data-to-azure-storage"></a>Использование службы "Импорт и экспорт Microsoft Azure" для передачи данных в хранилище Azure
Служба "Импорт и экспорт Azure" позволяет безопасно переносить большие объемы данных в хранилище Azure, отправляя жесткие диски в центр обработки данных Azure. Кроме того, с ее помощью можно переносить данные из хранилища Azure на жесткие диски и передавать на локальный сайт. Служба пригодится в тех случаях, когда необходимо передать несколько терабайт (ТБ) данных в Azure или обратно, но использовать для этого сеть невозможно в связи с ограниченной пропускной способностью или высокой стоимостью трафика.
...
Службу "Импорт и экспорт Azure" можно использовать для копирования данных в **блочные** или **страничные** BLOB-объекты или **файлы**. Кроме того, с ее помощью можно экспортировать из службы хранилища Azure только **блочные**, **страничные** или **добавочные** BLOB-объекты. Эта служба не поддерживает экспорт файлов Azure. С ее помощью можно только импортировать файлы в хранилище Azure.
...
5.  Создайте CSV-файл для набора данных и набора дисков.
  ...
  Ниже приведен пример CSV-файла с набором данных для импорта данных в виде больших двоичных объектов Azure.
  ...
  Ниже приведен пример CSV-файла с набором данных для импорта данных в виде файлов Azure.
    ...
    ```
   В этом примере видно, что файл 100M_1.csv.txt будет скопирован в корень контейнера под именем "containername" или "fileshare". Если контейнеры с такими именами отсутствуют, они будут автоматически созданы. Все файлы и папки, расположенные в каталоге 50M_original, будут рекурсивно скопированы в контейнер под именем "containername" или "fileshare". Структура папок сохраняется.
