<properties 
	...
	description="Узнайте, как настроить строку подключения к учетной записи хранения Azure. Строка подключения содержит сведения, необходимые для проверки подлинности при доступе к ресурсам в учетной записи хранения программными средствами. Строка подключения может инкапсулировать ключ доступа учетной записи, которой вы владеете, или он может включать подпись общего доступа для доступа к ресурсам в контексте учетной записи без ключа доступа." 
	services="storage" 
	documentationCenter="" 
	authors="tamram" 
	manager="adinah" 
...
Конечная точка может быть конечной точкой службы по умолчанию или пользовательской конечной точкой. `base64Signature` соответствует части подписи в подписи общего доступа. Подпись представляет собой код HMAC, рассчитанный по алгоритму SHA256 с использованием ключа и преобразования строки в символы. Полученное значение преобразуется в кодировку Base64.
