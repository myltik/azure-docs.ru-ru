...
Чтобы написать функцию, которую пакет SDK для веб-заданий будет вызывать при получении сообщения очереди, используйте атрибут **QueueTrigger**. Конструктор атрибута принимает строковый параметр, который указывает имя очереди для опроса. Также можно [задать имя очереди динамически](how-to-set-configuration-options).
...
Для объектов POCO, сохраненных в формате JSON в сообщении очереди, можно использовать заполнители, которые называют свойства объектов в параметре **blobPath** атрибута **Queue**. Можно также использовать [имена свойства метаданных очереди](#get-queue-or-queue-message-metadata) в качестве заполнителей.
...
Если необходимо выполнить некоторую работу в функции перед привязкой большого двоичного объекта к объекту, можно использовать атрибут в основном тексте функции, [как показано выше для атрибута Queue](#use-webjobs-sdk-attributes-in-the-body-of-a-function).
...
Пакет SDK вызывает функцию обработки сообщения очереди до 5 раз. В случае сбоя во время пятой попытки сообщение перемещается в очередь подозрительных сообщений. [Можно настроить максимальное количество попыток](#how-to-set-configuration-options).
...
Можно отключить ведение журнала, [присвоив строке подключения к панели мониторинга значение null](#how-to-set-configuration-options).
