---
title: "Использование управляемых дисков в шаблонах Azure Resource Manager | Документация Майкрософт"
description: "Сведения о том, как использовать управляемые диски в шаблонах Azure Resource Manager."
services: storage
documentationcenter: 
author: jboeshart
manager: 
...
Затем можно указать ссылку на него в объекте виртуальной машины, чтобы подключить этот объект диска. Указав идентификатор ресурса созданного управляемого диска в свойстве `managedDisk`, можно подключить этот диск при создании виртуальной машины. Для использования этой функции необходимо, чтобы свойство `apiVersion` ресурса виртуальной машины имело значение `2016-04-30-preview`. Кроме того, обратите внимание, что мы создали зависимость от дискового ресурса, чтобы обеспечить его успешное создание перед созданием виртуальной машины. 
...
Чтобы создать управляемые группы доступности с виртуальными машинами, использующими управляемые диски, добавьте объект `sku` в ресурс группы доступности и задайте для свойства `name` значение `Aligned`. Это гарантирует, что диски каждой виртуальной машины будут достаточно изолированы друг от друга, чтобы избежать образования единых точек отказа. Свойство `apiVersion` ресурса группы доступности также должно иметь значение `2016-04-30-preview`, чтобы можно было использовать данную функциональную возможность.
...
* Ознакомьтесь с документом [Обзор управляемых дисков Azure](storage-managed-disks-overview.md), чтобы узнать больше об управляемых дисках.
* Ознакомьтесь с информацией по шаблонам для ресурсов виртуальных машин, приведенной в [справочнике по шаблону Microsoft.Compute/virtualMachines](/templates/microsoft.compute/virtualmachines).
* Ознакомьтесь с информацией по шаблонам дисковых ресурсов, приведенной в [справочнике по шаблону Microsoft.Compute/disks](/templates/microsoft.compute/disks).
