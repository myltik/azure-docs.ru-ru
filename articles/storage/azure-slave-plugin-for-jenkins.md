<properties
    pageTitle="Использование подключаемого модуля Azure на сервере непрерывной интеграции Jenkins"
    description="Описано использование подчиненного подключаемого модуля Azure на сервере непрерывной интеграции Jenkins"
	services="storage" 
...
#Использование подключаемого модуля Azure на сервере непрерывной интеграции Jenkins

Подчиненный подключаемый модуль Azure для Jenkins позволяет подготовить подчиненные узлы в Azure при выполнении распределенных сборок.
...
4. В поле фильтра над списком доступных подключаемых модулей введите **Azure**, чтобы ограничить список соответствующими подключаемыми модулями.
	...
	Также вы можете выполнить поиск в прокручиваемом списке доступных подключаемых модулей. Подчиненный подключаемый модуль Azure находится в разделе **Cluster Management and Distributed Build** (Управление кластером и распределенная сборка).
...
Эти сведения можно найти в профиле подписки. Если у вас нет копии вашего профиля подписки, вы можете загрузить его [здесь](https://manage.windowsazure.com/publishsettings/Index?SchemaVersion=2.0). Ниже приведен пример профиля подписки.
...
4. Щелкните **Add new cloud > Microsoft Azure** (Добавить новое облако > Microsoft Azure).
	...
	![раздел «Облако»](./media/azure-slave-plugin-for-jenkins/jenkins-cloud-section.png)

	Появятся поля, в которые необходимо ввести сведения о подписке.

	![конфигурация подписки](./media/azure-slave-plugin-for-jenkins/jenkins-account-configuration-fields.png)
...
4. В разделе «Cloud» найдите элемент «Add Azure Virtual Machine Template» (Добавить шаблон виртуальной машины Azure) и нажмите кнопку «Add» (Добавить).
	...
	![добавление шаблона виртуальной машины](./media/azure-slave-plugin-for-jenkins/jenkins-add-vm-template.png)

	Появятся поля, в которые необходимо ввести сведения о создаваемом шаблоне.

	![форма для ввода общих настроек](./media/azure-slave-plugin-for-jenkins/jenkins-slave-template-general-configuration-blank.png)

5. В поле **Name** (Имя) введите имя облачной службы Azure. Если введенное имя ссылается на уже существующую облачную службу, виртуальная машина будет подготовлена в этой службе. В противном случае Azure создаст новую службу.

6. В поле **Description** (Описание) введите описание создаваемого вами шаблона. Это описание будет использоваться только в информационных целях и при подготовке виртуальной машины учитываться не будет.
...
11. Время хранения — это время в минутах, которое должно пройти до удаления неактивного подчиненного модуля из Jenkins. Оставьте значение по умолчанию 60. Также вы можете не удалять неактивный подчиненный модуль, а просто отключить его. Для этого установите флажок **Shutdown Only (Do Not Delete) After Retention Time** (Отключить (не удалять) по истечении времени хранения).
12. В поле **Usage** (Использование) выберите условие для использования этого подчиненного узла. На этом этапе выберите вариант **Utilize this node as much as possible** (Использовать этот узел постоянно).
	...
	![конфигурация общего шаблона контрольной точки](./media/azure-slave-plugin-for-jenkins/jenkins-slave-template-general-configuration.png)
...
13. В поле **Image Family or Id** (Семейство или идентификатор образов) укажите, какой образ системы будет установлен на вашей виртуальной машине. Выберите образ из списка семейств или укажите пользовательский вариант.
	...
	![образец списка образов ОС](./media/azure-slave-plugin-for-jenkins/jenkins-os-family-list-sample.png)
...
В этом разделе вы узнаете о том, как создать задачу Jenkins, которая должна выполняться в подчиненном узле в Azure. Для большей наглядности создайте собственный проект на github.
...
6. В поле **Label Expression** (Описание метки) введите **linux**. В предыдущем разделе мы создали шаблон подчиненного модуля, который мы назвали **linux**. Сейчас мы указываем именно его.
...
10. На панели мониторинга Jenkins наведите указатель мыши на созданную задачу и щелкните значок стрелки вниз, чтобы отобразить параметры задачи.
