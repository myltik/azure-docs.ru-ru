...
> [!IMPORTANT]
> Служба хранилища Azure пока не поддерживает протокол HTTPS для личных доменов. Хотя у нас еще нет определенного графика, которым мы можем поделиться, нам известно о заинтересованности клиентов в этой функции.
>
...
1. В колонке **Личный домен** выберите *Сохранить*, чтобы зарегистрировать личный домен. Если регистрация прошла успешно, появится сообщение о том, что учетная запись хранения успешно обновлена.
...
Воспользуйтесь командлетом PowerShell [Set-AzureRmStorageAccount](https://docs.microsoft.com/powershell/module/azurerm.storage/Set-AzureRmStorageAccount) и укажите пустую строку (`""`) для значения аргумента `-CustomDomainName`, чтобы отменить регистрацию личного домена.
...
Сейчас невозможно удалить регистрацию личного домена с помощью портала Azure. Это известная проблема. На этом этапе мы не можем решить эту проблему. Как только решение появится, мы обновим ​​эту статью. А сейчас используйте Azure CLI 2.0 или Azure PowerShell для удаления настройки личного домена.
