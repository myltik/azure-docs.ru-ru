...
Функция SSE шифрует данные при их записи в хранилище Azure. Ее можно использовать для хранилища BLOB-объектов Azure и для хранилища файлов (предварительная версия). Вот что поддерживает служба:
...
* хранилище BLOB-объектов во всех регионах. Сведения о хранилище файлов см. в разделе, посвященном доступности.

Шифрование службы хранения для файлов (предварительная версия). Теперь можно использовать SSE для шифрования данных в хранилище файлов. Сейчас этот компонент доступен в предварительной версии. Ниже приведен список регионов, в которых доступна функция SSE для хранилища файлов.

Чтобы использовать предварительную версию SSE для файлов, свяжитесь с нами по адресу ssediscussions@microsoft.com.

Дополнительные сведения см. в разделе вопросов и ответов.

### <a name="availability-for-file-storage"></a>Доступность хранилища файлов
Шифрование службы хранилища для хранилища файлов в настоящее время доступно в следующих регионах: восточная часть США, восточная часть США&2;, центральная часть США, центральная часть США, юго-центральный регион США, Западно-центральная часть США, западная часть США, западная часть США&2;, Восточная Азия, Северная Европа, Западная Европа, восточная Австралия и юго-восточная Австралия.
...
<br/>*Рис. 1.1. Включение SSE для службы BLOB-объектов (шаг 2)*

![Снимок экрана портала со свойствами шифрования](./media/storage-service-encryption/image4.png)
<br/>*Рис. 2.1. Включение SSE для службы файлов (шаг 2)*
...
* Шифрование для хранилища BLOB-объектов и хранилища файлов.
* Шифрование для классических учетных записей хранения, перенесенных в учетные записи хранения Resource Manager, поддерживается только для службы BLOB-объектов, но не для службы файлов.
* Шифрование для хранилища файлов поддерживается только для новых учетных записей хранения.
...
В процессе переноса классической учетной записи хранения в учетную запись хранения Resource Manager передаваемые данные не шифруются. Но если вы включите шифрование после переноса учетной записи хранения, будут шифроваться все новые данные, записываемые в эту учетную запись хранения. Дополнительные сведения см. в статье [Поддерживаемый платформой перенос ресурсов IaaS из классической модели в модель Azure Resource Manager](https://azure.microsoft.com/blog/iaas-migration-classic-resource-manager/). Обратите внимание на то, что эта функция поддерживается только для хранилища BLOB-объектов. Для предварительной версии хранилища файлов пользователям необходимо создавать новые учетные записи хранения Resource Manager.

**Вопрос. У меня есть учетная запись хранения Resource Manager. Можно включить в ней SSE?**

Ответ. Да, но шифрование будет применяться только к тем данным, которые будут записываться в большие двоичные объекты после включения SSE. Данные, сохраненные до этого, шифроваться не будут. Эта функция пока не поддерживается в предварительной версии хранилища файлов.

**Вопрос. Мне бы хотелось зашифровать текущие данные в существующей учетной записи хранения Resource Manager.**

Ответ. Вы можете включить SSE в учетной записи Resource Manager в любое время. Однако большие двоичные объекты, которые существовали до этого, не будут зашифрованы. Чтобы зашифровать эти большие двоичные объекты, их можно скопировать в другой контейнер или под другим именем и удалить их незашифрованные версии. Эта функция пока не поддерживается для предварительной версии хранилища файлов.

**Вопрос. Я использую хранилище класса "Премиум"; доступна ли мне функция SSE?**

Ответ. Да, функция SSE поддерживается для стандартного хранилища и хранилища класса Premium. Она пока не поддерживается для предварительной версии хранилища файлов.
