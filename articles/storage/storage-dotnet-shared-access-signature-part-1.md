...
- **Версия службы.** Обязательный параметр, который указывает версию службы хранилища для проверки подлинности запроса.
...
- **IP-адрес.** Необязательный параметр, который указывает IP-адрес или диапазон IP-адресов за пределами Azure (см. в разделе [Состояние конфигурации сеанса маршрутизации](../expressroute/expressroute-workflows.md#routing-session-configuration-state) для ExpressRoute), с которых следует принимать запросы. 
...
- **Типы ресурсов хранилища.** SAS учетной записи применяется к одному или нескольким классам ресурсов хранилища, а не к конкретному ресурсу. С помощью SAS учетной записи можно делегировать доступ к следующим типам ресурсов:
	- API-интерфейсы уровня службы, которые вызываются для ресурсов учетной записи хранения. Вот некоторые примеры: **Получение/задание свойств службы**, **Получение статистики службы** и **Получение списка контейнеров/очередей/таблиц/общих папок**.
	- API-интерфейсы уровня контейнеров, которые вызываются для объектов-контейнеров каждой службы: BLOB-контейнеров, очередей, таблиц и общих папок. Вот некоторые примеры: **Создание/удаление контейнера**, **Создание/удаление очереди**, **Создание/удаление таблицы**, **Создание/удаление общей папки** и **Получение списка больших двоичных объектов/файлов и каталогов**.
	- API-интерфейсы уровня объектов, которые вызываются для больших двоичных объектов, сообщений в очереди, сущностей таблицы и отдельных файлов. Например: **Вставка BLOB-объекта**, **Сущность запроса**, **Получение сообщений** и **Создание файла**.
