...
- **IP-адрес.** Необязательный параметр, который указывает IP-адрес или диапазон IP-адресов, с которых будут приниматься запросы. 
...
Поскольку разрешения ограничены уровнем службы, этот подписанный URL-адрес позволяет выполнять такие операции: **Получение свойств службы BLOB-объектов** (чтение) и **Задание свойств службы BLOB-объектов** (запись). Этот же маркер SAS с другим универсальным кодом ресурса (URI) ресурса будет предоставлять доступ к операции **Получение статистики службы BLOB-объектов** (чтение).
