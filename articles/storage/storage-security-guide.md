...
-   [Поставщики вычислительных и сетевых ресурсов, а также ресурсов службы хранилища Azure в диспетчере ресурсов Azure](../virtual-machines/virtual-machines-windows-compare-deployment-models.md)
    ...
    В этой статье описывается, как работают поставщики вычислительных и сетевых ресурсов, а также ресурсов службы хранилища Azure в рамках модели Resource Manager.
...
Под безопасностью плоскости данных понимаются методы защиты объектов данных в службе хранилища Azure Storage: BLOB-объектов, очередей, таблиц и файлов. Мы рассмотрели методы шифрования и защиты данных в процессе их передачи. Теперь давайте поговорим о предоставлении доступа к объектам.
...
###Использование шифрования при обмене данными с общими папками Azure
...
>[AZURE.NOTE] Для виртуальных машин Linux, уже работающих в Azure, или новых виртуальных машин Linux, созданных из образов в Azure Marketplace, шифрования диска операционной системы не поддерживается. Шифрование тома операционной системы для виртуальных машин Linux поддерживается только для тех из них, которые были зашифрованы локально и переданы в Azure. Это ограничение действует только для диска операционной системы. Шифрование томов данных для виртуальной машины Linux поддерживается без ограничений.
...
-   [Шифрование дисков Azure для виртуальных машин IaaS с ОС Windows и Linux](https://gallery.technet.microsoft.com/Azure-Disk-Encryption-for-a0018eb0)
    ...
    В этой статье описывается предварительный выпуск шифрования дисков Azure и приводится ссылка для скачивания технического документа.

###Сравнение шифрования дисков Azure, шифрования службы хранилища и шифрования на стороне клиента
...
Управление шифрованием службы хранилища осуществляется службой хранилища Azure и не вызывает сложностей. Шифрование службы хранилища не обеспечивает защиту данных при передаче, но данные шифруются при записи в службу хранилища Azure. Использование этой функции не влияет на производительность.

С помощью шифрования службы хранилища можно шифровать только блочные, добавочные и страничные BLOB-объекты. Если вам необходимо шифровать данные таблиц или очередей, рекомендуем воспользоваться шифрованием на стороне клиента.
...
2.  BLOB-объект является частным и использовался с подписанным URL-адресом. В этом случае поле request-status имеет значение "SASSuccess", а поле authorization-type — "sas".
...
1.  **Как можно проверять целостность BLOB-объектов, передаваемых в службу хранилища Azure или из нее, если протокол HTTPS использовать невозможно?**
	...
	Дополнительные сведения см. в записи блога [Azure Blob MD5 Overview](http://blogs.msdn.com/b/windowsazurestorage/archive/2011/02/18/windows-azure-blob-md5-overview.aspx) (Общие сведения о проверке MD5 BLOB-объектов Azure).
