...
`result` будет содержать сообщение.

> [AZURE.NOTE]Если использовать **peekMessages**, когда в очереди нет сообщений, оповещение об ошибке не отображается, но сообщения не возвращаются.
...
Чтобы удалить сообщение из очереди, используйте **getMessage**. В результате сообщения становятся невидимыми в очереди, и другие клиенты не могут их обрабатывать. После того как приложение обработало сообщение, вызовите **deleteMessage**, чтобы удалить сообщение из очереди. Следующий пример получает сообщение, а затем удаляет его:
...
> [AZURE.NOTE]По умолчанию сообщение остается скрытым в течение 30 секунд, а потом снова становится видимым для других клиентов. Можно задать другое значение, используя с методом **getMessages** параметр `options.visibilityTimeout`.

> [AZURE.NOTE]Если использовать метод **getMessages**, когда в очереди нет сообщений, оповещение об ошибке не отображается, но сообщения не возвращаются.
...
Чтобы удалить очередь и все сообщения в ней, вызовите метод **deleteQueue** в объекте очереди.
