...
* Шифрование неактивных данных ([шифрование службы хранилища Azure](../common/storage-service-encryption.md?toc=%2fazure%2fstorage%2ffiles%2ftoc.json)). Мы работаем над шифрованием службы хранилища (SSE) на базовой платформе службы хранилища Azure. Это означает, что шифрование будет включено по умолчанию для всех учетных записей хранения. При создании учетной записи хранения в регионе, в котором шифрование неактивных данных включено по умолчанию, никаких действий для включения выполнять не нужно. Неактивные данные шифруются с помощью полностью управляемых ключей. Шифрование неактивных данных не увеличивает затраты на хранение и не снижает производительность. 
* Необязательно требование к зашифрованным данным во время передачи: если выбран этот вариант, служба файлов Azure запретит доступ к данным по незашифрованным каналам. В частности, будут разрешены только HTTPS и SMB 3.0 с шифрованием подключений. 
...
Служба файлов Azure поддерживает два варианта избыточности данных: локально избыточное хранилище (LRS) и геоизбыточное хранилище (GRS). В следующих разделах описываются различия между этими типами хранилища.
