...
<a id="agent-installation-websitename-failure"></a>**Установка агента завершится неудачно со следующей ошибкой: «Причиной преждевременного завершения мастера агент синхронизации хранилища»**  
Эта проблема может возникать, если изменить имя по умолчанию веб-сайт IIS. Чтобы обойти эту проблему, переименуйте на веб-сайт IIS по умолчанию как «Default Web Site» и повторить попытку установки. Проблема будет устранена в будущем обновлении агента. 
...
Эта проблема возникает, если путь к конечной точке сервера на системном томе и облако уровнями включен. Но для системного тома не поддерживается распределение по уровням облака. Чтобы создать конечную точку сервера на системном томе, отключите облака уровнями при создании сервера конечной точки.

<a id="server-endpoint-deletejobexpired"></a>**Удаление конечной точки сервера завершается сбоем, со следующей ошибкой: «MgmtServerJobExpired»**                
Эта проблема возникает, если сервер находится в автономном режиме, или отсутствует подключение к сети. Если сервер больше не доступен, отмените регистрацию сервера в портал, который приведет к удалению конечных точек сервера. Чтобы удалить конечные точки сервера, выполните шаги, описанные в [Отмена регистрации сервера с помощью Azure файл Sync](storage-sync-files-server-registration.md#unregister-the-server-with-storage-sync-service).
