...
Конечная точка сервера представляет определенное расположение на зарегистрированного сервера, например в папке на том сервере. Если их пространства имен не пересекаются на том же томе может существовать несколько конечных точек сервера (например, `F:\sync1` и `F:\sync2`). Политики распределения по уровням облака можно настроить отдельно для каждой конечной точки сервера. В настоящее время не удается создать конечную точку сервера для корневого каталога тома (например `F:\` или `C:\myvolume`, если подключается как точки подключения тома).

> [!Note]  
> Конечная точка сервера может находиться на системном томе Windows. Но для системного тома не поддерживается распределение по уровням облака.
...
- службы управления правами Azure (Azure RMS) и устаревшие службы Active Directory RMS.
