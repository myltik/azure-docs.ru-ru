...
Конечная точка сервера представляет собой определенное расположение на зарегистрированном сервере, например папку в томе сервера. В томе может располагаться несколько конечных точек сервера, если их пространства имен не перекрываются (например, `F:\sync1` и `F:\sync2`). Политики распределения по уровням облака можно настроить отдельно для каждой конечной точки сервера. В настоящее время невозможно создать конечную точку сервера для корня тома (например, `F:\` или `C:\myvolume`, если том подключается как точка подключения).
