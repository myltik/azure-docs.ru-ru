<properties
	pageTitle="Использование хранилища BLOB-объектов из Node.js | Microsoft Azure"
...
В этой статье показано, как реализовать типичные сценарии с использованием службы BLOB-объектов Azure. Примеры написаны с использованием интерфейса API Node.js. Здесь описаны такие сценарии, как **отправка**, **перечисление**,**загрузка** и **удаление** BLOB-объектов.
...
Если контейнер только что создан, `result` имеет значение true. Если контейнер уже существует, значение `result` будет ложным. `response` будет содержать сведения об операции, включая сведения [ETag](http://en.wikipedia.org/wiki/HTTP_ETag) для контейнера.
...
Используя **BlobService**, к выполняемым операциям можно применить дополнительные операции фильтрации. К операциям фильтрации могут относиться ведение журнала, автоматический повтор и т. д. Фильтры являются объектами, реализующими метод со следующей сигнатурой:
...
В следующем примере кода содержимое файла **test.txt** передается в BLOB-объект **myblob**.
...
`result`, возвращаемый этими методами, содержит информацию об операции, такую как **ETag** BLOB-объекта.
...
В следующем примере кода содержимое файла **test.txt** передается в BLOB-объект **mypageblob**.
...
В следующем примере кода показано, как использовать метод **getBlobToStream**, чтобы загрузить содержимое BLOB-объекта **myblob** и сохранить его в файл **output.txt** с помощью потока:
...
`result` содержит данные о большом двоичном объекте, в том числе **ETag**.

## Удаление большого двоичного объекта

Наконец, чтобы удалить BLOB-объект, вызовите **deleteBlob**. В следующем примере кода удаляется BLOB-объект с именем **myblob**.
...
Условия для ETag можно задать с помощью дополнительного параметра `options.accessConditions`. В следующем примере кода файл **test.txt** будет отправляться, только если большой двоичный объект уже существует, а значение ETag задается параметром `etagToMatch`.
...
Новая аренда (lease) может быть получена с помощью метода **acquireLease**, который определяет блоб-объект или контейнер для аренды. Например, код в следующем примере получает аренду **myblob**.
...
В следующем примере кода создается новая общая политика, которая позволяет держателю подписи SAS выполнять операции чтения blob-объекта **myblob** в течение 100 минут с момента своего создания.
...
ACL реализуется с помощью массива политик доступа, каждая из которых связана со своим идентификатором. В следующем примере кода определяются две политики: для пользователей 'user1' и 'user2'.
...
После задания ACL можно создать подпись SAS на основе идентификатора политики. В следующем примере кода создается новая подпись SAS для пользователя 'user2':
