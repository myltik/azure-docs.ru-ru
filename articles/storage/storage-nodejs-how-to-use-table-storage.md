<properties 
	pageTitle="Использование табличного хранилища из Node.js | Microsoft Azure" 
...
- [Создание и развертывание веб-сайта Node.js в Azure][Create and deploy a Node.js application to an Azure Web Site]
- [Создание и развертывание веб-сайта Node.js в Azure с использованием WebMatrix][Web Site with WebMatrix]
- [Создание и развертывание приложения Node.js в облачной службе Azure][Node.js Cloud Service] (с помощью Windows PowerShell)
...
### Как выполнить запрос подмножества свойств сущности
...
Сущность можно удалить с помощью ее ключей раздела и строки. В этом примере объект **task1** содержит значения **RowKey** и **PartitionKey** удаляемой сущности. Затем этот объект передается методу **deleteEntity**.
...
> [AZURE.NOTE]Следует рассмотреть использование ETag при удалении элементов, чтобы гарантировать отсутствие в нем изменений сделанных другим процессом. Сведения об использовании ETag см. в разделе [Практическое руководство. Обновление сущности].
...
## Практическое руководство. Использование маркеров продолжения
...
Если обратиться к объекту `continuationToken`, то вы обнаружите такие его свойства, как `nextPartitionKey`, `nextRowKey` и `targetLocation`, которые можно использовать для итерации по всем результатам.

Кроме того, на сайте GitHub, в репозитории Node.js для службы хранилища Azure, есть пример использования маркеров продолжения (см. раздел azure-storage-node/`examples/samples/continuationsample.js`).
