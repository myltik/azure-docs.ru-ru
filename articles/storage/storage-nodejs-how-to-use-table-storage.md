<properties
	...
	description="Сведения об использовании табличного хранилища Azure. Примеры написаны с помощью Node.js API."
	services="storage"
...
- [Сборка и развертывание веб-сайта Node.js в Azure](Создание и развертывание приложения Node.js на веб-сайте Azure)
- [Сборка и развертывание веб-сайта Node.js в Azure с использованием WebMatrix](Создание и развертывание приложения Node.js с использованием WebMatrix)
- [Сборка и развертывание приложения Node.js в облачной службе Azure](облачная служба Node.js) (с использованием Windows PowerShell)
...
**PartitionKey** и **RowKey** должны быть строковыми значениями. Дополнительную информацию см. в статье [Основные сведения о модели данных службы таблиц](http://msdn.microsoft.com/library/azure/dd179338.aspx).
...
Если обратиться к объекту `continuationToken`, то вы обнаружите, что они имеет такие свойства, как `nextPartitionKey`, `nextRowKey` и `targetLocation`, которые можно использовать для итерации по всем результатам.
...
Доверенное приложение, например облачная служба, создает подпись SAS с помощью **generateSharedAccessSignature** службы **TableService** и предоставляет ее недоверенному или полудоверенному приложению, например предназначенному для мобильных устройств. Подпись SAS создается с использованием политики, которая описывает даты начала и окончания срока действия SAS, а также уровень доступа, который предоставляется держателю подписи SAS.
...
-   Справочник MSDN о [хранении данных и доступе к ним в Azure][].
...
-   [Центр разработчиков Node.js](/develop/nodejs/).
  ...
  [хранении данных и доступе к ним в Azure]: http://msdn.microsoft.com/library/azure/gg433040.aspx
