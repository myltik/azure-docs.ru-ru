...
Затем получите ссылку на класс **cloud\_table\_client**, так как это позволяет получить исходные объекты для таблиц и сущностей, хранящихся в службе хранилища таблиц. Следующий код создает объект **cloud\_table\_client** с помощью объекта учетной записи хранения, полученного выше.
...
Сущность можно легко удалить после ее получения. После получения сущности вызовите **table\_operation::delete\_entity** с подлежащей удалению сущности. Затем выполните метод **cloud\_table.execute**. Следующий код извлекает и удаляет сущность с ключом раздела Smith и ключом строки Jeff.
