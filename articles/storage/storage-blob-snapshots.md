...
Снимок BLOB-объекта имеет то же имя, что и базовый BLOB-объект, моментальный снимок которого создается, но к концу имени добавляется значение **DateTime**, указывающее на время, в которое был сделан снимок. Например, если универсальный код ресурса (URI) BLOB-объекта -  `http://storagesample.core.blob.windows.net/mydrives/myvhd`, то универсальный код ресурса (URI) моментального снимка будет примерно таким:  `http://storagesample.core.blob.windows.net/mydrives/myvhd?snapshot=2011-03-09T01:42:34.9360000Z`. Все моментальные снимки BLOB-объекта используют общий универсальный код ресурса (URI) и распределяются только по добавленному значению **DateTime**.
...
- Учетная запись хранения Premium поддерживает до 100 моментальных снимков на каждый страничный BLOB-объект. При превышении этого количества операцией создания моментального снимка BLOB-объекта возвращается ошибка с кодом 409 (**SnapshotCountExceeded**).
...
- При использовании учетной записи хранения Premium чтение моментальных снимков страничного BLOB-объекта с помощью операции Get Blob не поддерживается. При попытке вызова этой операции возвращается ошибка с кодом 400 (**InvalidOperation**). Однако для моментальных снимков поддерживаются операции Get Blob Properties и Get Blob Metadata.
...
В этом примере кода C# составляется абсолютный универсальный код ресурса (URI) моментального снимка от базового BLOB-объекта.
...
![Ресурсы службы хранилища Azure](./media/storage-blob-snapshots/storage-blob-snapshots-billing-scenario-1.png)
...
![Ресурсы службы хранилища Azure](./media/storage-blob-snapshots/storage-blob-snapshots-billing-scenario-2.png)
...
![Ресурсы службы хранилища Azure](./media/storage-blob-snapshots/storage-blob-snapshots-billing-scenario-3.png)
