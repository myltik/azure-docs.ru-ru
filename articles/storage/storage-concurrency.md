...
Следующий фрагмент C#-кода является примером получения на 30 секунд монопольной аренды на BLOB-объект, обновления содержимого BLOB-объекта и освобождения от аренды. Если на BLOB-объекте уже есть действительная аренда, а вы пытаетесь получить новую аренду, служба BLOB-объектов возвращает статус "HTTP (409) Conflict" (HTTP (409) — конфликт) Фрагмент, приведенный ниже, использует объект **AccessCondition** для инкапсуляции данных аренды при направлении запроса на обновление BLOB-объектов в службе хранилища. Полный пример можно скачать на странице [Managing Concurrency using Azure Storage - Sample Application](http://code.msdn.microsoft.com/Managing-Concurrency-using-56018114) (Управление параллелизмом с помощью службы хранилища Azure. Пример приложения).
