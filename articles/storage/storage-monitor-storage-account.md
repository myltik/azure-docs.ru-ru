...
Вы можете отслеживать свою учетную запись на портале Azure. При настройке учетной записи хранения для отслеживания с помощью портала хранилище Azure использует функцию [Аналитика хранилища](http://msdn.microsoft.com/library/azure/hh343270.aspx) для мониторинга метрик вашей учетной записи и данных журнала.

> [AZURE.NOTE]За ознакомление с данными мониторинга на портале Azure взимается дополнительная плата. Дополнительные сведения см. в разделе <a href="http://msdn.microsoft.com/library/azure/hh360997.aspx">Аналитика и выставление счетов для хранилища</a>. <br />
...
1. На [портале Azure](https://manage.windowsazure.com/) выберите **Хранилище**, а затем щелкните имя учетной записи хранения, чтобы открыть панель мониторинга.

2. Щелкните **Настроить** и прокрутите вниз до параметров **мониторинга** служб BLOB-объектов, таблиц и очередей.
...
3. В разделе **мониторинга** задайте уровень мониторинга и политику хранения данных для каждой службы:
	...
	-  Чтобы задать уровень мониторинга, выберите один из следующих вариантов:
      ...
      **Минимальный** — сбор таких метрик, как исходящие и входящие данные, доступность, задержка и процент успешных операций, которые вычисляются для служб BLOB-объектов, таблиц и очередей.
...
После установки уровней мониторинга и политик хранения можно выбрать, какие из доступных метрик будут отслеживаться на портале Azure и какие будут выводиться на диаграммах метрик. На каждом уровне мониторинга отображается набор метрик по умолчанию. Для добавления метрики или удаления ее из списка метрик можно использовать кнопку **Добавить метрики**.
...
1. На [портале Azure](https://manage.windowsazure.com/) щелкните **Хранилище**, а затем щелкните имя учетной записи хранения, чтобы открыть панель мониторинга.
...
5. Щелкните стрелку вниз рядом с метрикой, чтобы развернуть список операций, которые можно включить для метрики. Выберите каждую операцию, которую следует просмотреть в таблице метрик на портале Azure.
