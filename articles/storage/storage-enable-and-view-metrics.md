...
По умолчанию для служб хранилища метрики хранилища не включены. Вы можете включить наблюдение с помощью портала управления Azure, Windows PowerShell или программно с помощью интерфейса API хранилища.
...
Обратите внимание, что в настоящее время портал управления не позволяет настраивать минутные метрики в учетной записи хранения. Их необходимо включить с помощью PowerShell или программно.
...
- Возможными значениями MetricsLevel являются None (эквивалентно значению "Выключено" на портале управления), Service (эквивалентно значению "Минимальный" на портале управления) и ServiceAndApi (эквивалентно значению "Подробный" на портале управления).
...
Дополнительную информацию о настройке командлетов Azure PowerShell для работы с подпиской Azure и для выбора учетной записи хранения по умолчанию см. в следующем разделе: [Установка и настройка Azure PowerShell](http://azure.microsoft.com/documentation/articles/install-configure-powershell/).

## Как программно включить метрики хранилища

Помимо управления метриками хранилища с помощью портала управления Azure и командлетов Azure PowerShell, также можно использовать один из интерфейсов API службы хранилища Azure. Например, если используется язык .NET, можно использовать клиентскую библиотеку хранилища.
...
После настройки метрик хранилища для мониторинга учетной записи хранения метрики записываются в набор известных таблиц в учетной записи хранения. Просматривать часовые метрики по мере их появления на диаграмме можно на странице "Монитор" учетной записи хранения на портале управления. На этой странице портала управления можно:
...
Чтобы скачивать метрики для длительного хранения или анализа в локальной среде, необходимо использовать соответствующий инструмент или написать код для чтения таблиц. Необходимо скачать минутные метрики для анализа. Таблицы не отображаются, если в учетной записи хранения выведены все таблицы. Однако к ним можно обращаться напрямую по имени. Многие сторонние инструменты обзора хранилищ поддерживают эти таблицы и позволяют просматривать их напрямую (см. список доступных инструментов в записи блога [Обозреватели службы хранилища Microsoft Azure](http://blogs.msdn.com/b/windowsazurestorage/archive/2014/03/11/windows-azure-storage-explorers-2014.aspx)).
