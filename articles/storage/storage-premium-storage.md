...
**Диски хранилища класса Premium**: хранилище Azure класса Premium поддерживает диски виртуальных машин, которые можно подключить к виртуальным машинам Azure серии DS или GS. При использовании хранилища класса Premium доступен выбор из трех размеров диска — P10, (128 ГиБ), P20 (512 ГиБ) и P30 (1024 ГиБ), каждый из которых имеет свои собственные характеристики производительности. В зависимости от потребностей приложения можно подключить один или несколько этих дисков к вашей виртуальной машине серии DS или GS. В следующем разделе на [Цели по масштабируемости и производительности хранилища класса Premium](#premium-storage-scalability-and-performance-targets) мы опишем характеристики более подробно.
...
**Учетная запись Premium\_LRS**: чтобы начать использование хранилища класса Premium, необходимо создать соответствующую учетную запись. Создайте учетную запись хранения типа "Premium\_LRS" с помощью [портала Azure](https://portal.azure.com) или следующих библиотек пакетов SDK: [REST API службы хранилища](http://msdn.microsoft.com//library/azure/dd179355.aspx) версии 2014-02-14 или более поздней, [REST API управления службами](http://msdn.microsoft.com/library/azure/ee460799.aspx) версии 2014-10-01 или более поздней (классические развертывания), [справочник по REST API поставщика ресурсов службы хранилища Azure](http://msdn.microsoft.com/library/azure/mt163683.aspx) (развертывания ARM) и [Azure PowerShell](../powershell-install-configure.md) версии 0.8.10 или более поздней. Дополнительные сведения об ограничениях учетных записей хранения класса Premium см. в следующем разделе [Цели по масштабируемости и производительности хранилища класса Premium](#premium-storage-scalability-and-performance-targets).

**Локально избыточное хранилище класса Premium**: учетная запись хранения класса Premium является локально избыточной (LRS), то есть в ней сохраняется три копии данных в пределах одной области. Вопросы, касающиеся географической репликации при использовании хранилища класса Premium, см. в разделе [Моментальные снимки и копирование BLOB-объектов](#snapshots-and-copy-blob) этой статьи.

Служба Azure использует учетную запись хранилища как контейнер для операционной системы (ОС) и дисков данных. Когда вы создаете виртуальную машину Azure серии DS или GS и выбираете учетную запись хранения Azure класса Premium, ваша операционная система и диски данных хранятся в этой учетной записи.

Хранилище Premium можно использовать для дисков одним из двух способов.
- Сначала создайте учетную запись хранения класса Premium. Потом при создании новой виртуальной машины серии DS или GS выберите учетную запись хранения класса Premium в параметрах конфигурации хранилища. ИЛИ
- При создании виртуальной машины серии DS или GS создайте учетную запись хранения класса Premium в параметрах конфигурации хранилища или позвольте порталу Azure создать учетную запись хранения класса Premium по умолчанию.
...
Хранилище класса Premium поддерживает виртуальные машины серии DS и GS. С виртуальными машинами серии DS или GS можно использовать диски хранилища Standard и Premium. Однако диски хранилища Premium невозможно использовать с виртуальными машинами других серий. Сведения о доступных типах и размерах виртуальных машин Azure см. в статье [Размеры виртуальных машин](../virtual-machines/virtual-machines-size-specs.md). Ниже перечислены некоторые функции виртуальных машин серии DS и GS:
...
**Диск операционной системы**: виртуальные машины Azure серии DS и GS можно настроить для использования дисков с операционной системой (ОС) как в учетной записи хранения Standard, так и в учетной записи хранения класса Premium. Если диск c ОС применяется только для загрузки, рекомендуем использовать такой диск с хранилищем Standard. Это позволяет уменьшить траты и получить после загрузки производительность, аналогичную производительности хранилища класса Premium. Если кроме загрузки диск с ОС выполняет еще какие-либо операции, используйте хранилище Premium, чтобы повысить их производительность. Например, если ваше приложение выполняет чтение и запись данных на диске с ОС, хранилище Premium обеспечит более высокую производительность вашей виртуальной машины.

**Диски данных**: на одной виртуальной машине серии DS или GS можно одновременно использовать диски хранилищ Premium и Standard. С помощью хранилища класса Premium можно подготовить виртуальную машину серии DS или GS и подключить несколько дисков с постоянными данными к виртуальной машине. Если необходимо увеличить емкость и производительность тома, вы можете создать на дисках чередующийся том.
...
**Производительность и пределы масштабирования виртуальной машины**: каждая виртуальная машина серии DS и серии GS имеет пределы масштабирования по размеру, а также характеристики для числа операций ввода-вывода в секунду, пропускной способности и количества подключаемых дисков. При использовании дисков хранилища класса Premium для виртуальных машин серии DS или GS убедитесь, что в машина обладает достаточными показателями числа операций ввода-вывода в секунду и пропускной способности для передачи трафика диска. Например, для виртуальной машины STANDARD\_DS1 выделенный канал для обмена данными с дисками хранилища Premium составляет 32 МБ в секунду. Диск P10 хранилища класса Premium может обеспечивать пропускную способность 100 МБ в секунду. Скорость передачи данных для диска P10 хранилища класса Premium, подключенного к такой виртуальной машине, составляет не более 32 МБ в секунду и не может достигать 100 МБ в секунду, которые способен обеспечить диск P10.
...
Актуальные сведения о максимальных значениях числа операций ввода-вывода в секунду и пропускной способности для виртуальных машин серии DS и GS см. в статье [Размеры виртуальных машин](../virtual-machines/virtual-machines-size-specs.md). Сведения о дисках, используемых в хранилищах класса Premium, числе операций ввода-вывода в секунду и ограничениях пропускной способности см. в таблице из раздела [Целевые показатели по масштабируемости и производительности хранилища класса Premium](#premium-storage-scalability-and-performance-targets) данной статьи.
...
> [AZURE.NOTE] Убедитесь, что виртуальная машина обладает достаточной пропускной способностью для обслуживания трафика диска (см. раздел [Виртуальные машины серии DS и GS](#ds-and-gs-series-vms) выше в этой статье). В противном случае пропускная способность и скорость ввода-вывода при обмене данными с диском будет ограничена более низкими значениями для виртуальной машины вместо значений, указанных в предыдущей таблице.
...
Дополнительные сведения о достижении высокой производительности при использовании хранилища класса Premium см. в статье [Проектирование для повышения производительности с хранилищем класса Premium](storage-premium-storage-performance.md).
...
- Чтобы сохранить метки томов после перезапуска виртуальной машины, необходимо обновить каталог /etc/fstab, указав ссылки UUID на диски. Кроме того, см. статью [Подключение диска данных к виртуальной машине Linux](../virtual-machines/virtual-machines-linux-how-to-attach-disk.md).
...
3.	Перейдите в раздел **Новый**. Выберите **Данные + хранилище**. Щелкните **Учетная запись хранения**. Выберите модель развертывания. Используйте Resource Manager для новых развертываний. Щелкните **Создать**.

4.	На выноске учетной записи хранилища введите имя своей учетной записи хранилища. Выберите нужное расположение для развертывания. Убедитесь, что хранилище класса Premium доступно в выбранном расположении (см. статью [Службы Azure по регионам](https://azure.microsoft.com/regions/#services)).

5.	Щелкните **Тип**. В колонке **Выберите тип учетной записи хранения** выберите вариант **Локально избыточный Premium**. Нажмите кнопку **Выбрать**. После нажатия кнопки **Выбрать** параметру **Тип** присваивается значение **Premium-LRS**.

6.	В колонке **Учетная запись хранения** выберите существующую или создайте новую **группу ресурсов**. Щелкните Создать.

	![Ценовая категория][Image1]

#### 2\. Создание виртуальной машины Azure с помощью портала Azure

Для использования хранилища класса Premium необходимо создать виртуальную машину серии DS или GS. Следуйте указаниям в статье [Создание виртуальной машины Windows на портале Azure](../virtual-machines/virtual-machines-windows-tutorial.md) для создания новой виртуальной машины серии DS или GS.

#### 3\. Подключение диска данных хранилища класса Premium с помощью портала Azure

1. На портале Azure найдите или создайте виртуальную машину серии DS или GS.
2. В разделе **Все параметры** для этой машины последовательно выберите элементы **Диски** и **Подключить новый**.
3. Введите имя диска данных и выберите значение **Premium** в поле **Тип**. Выберите нужный **Размер**, а также задайте параметр **Кэширование узлов**.

	![Диск класса Premium][Image2]

Более подробные инструкции см. в статье [Подключение диска данных на портале Azure](../virtual-machines/virtual-machines-attach-disk-preview/).
...
Если для виртуальной машины требуется больше места на диске, подключите новый диск данных к существующей виртуальной машине серии DS или GS после ее создания. Для этого выполните следующие командлеты PowerShell в окне консоли:
...
1. **Можно ли подключить к виртуальной машине серии DS или GS диски данных Standard и Premium?**

	Да. К виртуальной машине серии DS или GS можно подключить диски данных как Standard, так и Premium.

2. **Можно ли подключить к виртуальной машине серии D или G диски данных Standard и Premium?**

	Нет. Диск данных Standard можно подключить ко всем виртуальным машинам, которые не относятся к серии D или G.
...
4. **Имеются ли затраты на транзакции при использовании хранилища класса Premium?**
	...
	Существует фиксированная стоимость для каждого размера диска, который подготавливается с определенным числом операций ввода-вывода в секунду и определенной пропускной способностью. Дополнительные затраты включают в себя только пропускную способность для исходящих данных и емкость моментальных снимков (если используется). Дополнительные сведения см. в статье [Цены на хранилище Azure](https://azure.microsoft.com/pricing/details/storage/).

5. **Где можно хранить диагностику загрузки для моей виртуальной машины серии DS или GS?**

	Создайте для этого стандартную учетную запись хранения.
