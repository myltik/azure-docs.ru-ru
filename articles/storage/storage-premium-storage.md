...
- При создании виртуальной машины серии DS, DSv2 или GS создайте новую учетную запись хранения уровня "Премиум" с помощью параметров конфигурации службы хранилища или позвольте порталу Azure создать учетную запись хранения уровня "Премиум" по умолчанию.
...
Хранилище класса "Премиум" поддерживает виртуальные машины серий DS, DSv2 и GS. С виртуальными машинами серии DS, DSv2 или GS можно использовать диски хранилища классов "Стандартный" и "Премиум". Однако диски хранилища Premium невозможно использовать с виртуальными машинами других серий.
...
Ниже перечислены некоторые функции виртуальных машин серий DS, DSv2 и GS:
...
**Диск операционной системы**. Виртуальные машины Azure серий DS, DSv2 и GS можно настроить для использования дисков с операционной системой (ОС), размещенных в учетной записи хранения класса Standard или Premium. Для наилучших результатов рекомендуем использовать диск ОС с хранилищем Premium.

**Диски данных**. На одной виртуальной машине серии DS, DSv2 или GS можно одновременно использовать диски хранилищ классов Premium и Standard. С помощью хранилища класса "Премиум" можно подготовить виртуальную машину серий DS, DSv2 или GS и подключить несколько дисков с постоянными данными к виртуальной машине. Если необходимо увеличить емкость и производительность тома, вы можете создать на дисках чередующийся том.
...
Актуальные сведения о максимальном количестве операций ввода-вывода в секунду и пропускной способности для виртуальных машин серий DS, DSv2 и GS описаны в статьях о размерах виртуальных машин [для Windows](../virtual-machines/virtual-machines-windows-sizes.md) и [для Linux](../virtual-machines/virtual-machines-linux-sizes.md).
...
4. Укажите модель развертывания: **Resource Manager** или **классическую**. Мы рекомендуем использовать **диспетчер ресурсов**. Подробнее об этих моделях можно узнать в статье [Общие сведения о развертывании диспетчера ресурсов и классическом развертывании](../resource-manager-deployment-model.md).
...
Для использования хранилища класса "Премиум" необходимо создать виртуальную машину серии DS, DSv2 или GS. Следуйте указаниям в статье [Создание виртуальной машины Windows на портале Azure](../virtual-machines/virtual-machines-windows-hero-tutorial.md) для создания виртуальной машины серии DS, DSv2 или GS.

#### 3\. Подключение диска данных хранилища уровня "Премиум" с помощью портала Azure

1. На портале Azure найдите или создайте виртуальную машину серии DS, DSv2 или GS.
...
2. Запустите консоль PowerShell, подключитесь к своей подписке и запустите следующий командлет PowerShell в окне консоли. Как видно в инструкции PowerShell, при создании учетной записи хранения уровня "Премиум" для параметра **Тип** необходимо задать значение **Premium\_LRS**.
...
9. **Как преобразовать виртуальную машину серии D в серию DS?**
	...
	Обратитесь к руководству [Перенос в хранилище Azure Premium](storage-migration-to-premium-storage.md), чтобы переместить свою рабочую нагрузку из виртуальной машины серии D с учетной записью хранения уровня "Стандартный" в виртуальную машину серии DS с учетной записью хранения уровня "Премиум".
