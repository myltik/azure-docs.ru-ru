...
**Размер диска хранилища класса Premium/BLOB-объекта**. Сумма оплаты за диск хранилища класса Premium или BLOB-объект зависит от размера подготовленного диска или BLOB-объекта. Azure сопоставляет подготовленный (округленный) размер с ближайшим вариантом диска хранилища Premium согласно таблице в разделе [Целевые показатели по масштабируемости и производительности при использовании хранилища Premium](#scalability-and-performance-targets-whru-RUing-premium-storage). Все объекты, размещаемые в учетной записи хранения, сопоставляются с одним из поддерживаемых подготовленных размеров и оплачиваются соответственно. В связи с этим не следует использовать учетную запись хранения класса Premium для хранения мелких BLOB-объектов. Счета за подготовленные диски или BLOB-объекты выставляются пропорционально с учетом часов по ежемесячной стоимости хранилища Premium. Например, если вы подготовили диск P10 и удалили его через 20 часов, взимается пропорциональная плата за использование диска P10 в течение 20 часов. Это происходит независимо от фактического объема данных, записанных на диск, количества операций ввода-вывода в секунду или использованной пропускной способности.

**Моментальные снимки хранилища класса Premium**. Счета за моментальные снимки в хранилище класса Premium выставляются с учетом дополнительной емкости, используемой такими снимками. Сведения о моментальных снимках см. в статье [Создание моментального снимка большого двоичного объекта](http://msdn.microsoft.com/library/azure/hh488361.aspx).
...
5. Укажите уровень производительности **Premium** для учетной записи хранилища.

6. Единственный доступный вариант репликации для хранилища класса Premium — **локально избыточное хранилище (LRS)**. Дополнительные сведения о репликации службы хранилища Azure см. в статье [Репликация службы хранилища Azure](storage-redundancy.md).
...
9. Выберите географический регион для учетной записи хранения. Убедитесь, что хранилище класса Premium доступно в выбранном расположении, как описано в статье [Службы Azure по региону](https://azure.microsoft.com/regions/#services).
...
3. Введите имя диска данных и в поле **Тип** выберите значение **Premium**. Выберите нужное значение в поле **Размер**, а также задайте параметр **Кэширование узлов**.

	![Диск класса Premium][Image1]

Подробные инструкции см. в статье [Подключение диска данных на портале Azure](../virtual-machines/virtual-machines-windows-attach-disk-portal.md).
...
9. **Как преобразовать виртуальную машину серии D в серию DS?**
	...
	Обратитесь к руководству [Перенос в хранилище Azure Premium](storage-migration-to-premium-storage.md), чтобы переместить свою рабочую нагрузку из виртуальной машины серии D с учетной записью хранения Standard в виртуальную машину серии DS с учетной записью хранения Premium.
