...
* [Отображается сообщение об ошибке "Узел не работает" в имеющихся общих папках или оболочка перестает отвечать при выполнении списка команд в точке подключения](#errorhold)
...
**Invalid handle value GetLastError: 53** (Недопустимое значение дескриптора GetLastError: 53)

Работая в Linux, вы получаете сообщения об ошибках, которые выглядят примерно следующим образом:

**<filename> [отказ в разрешении]**
...
## <a name="error-53-when-you-try-to-mount-or-unmount-an-azure-file-share"></a>Ошибка 53 при попытке подключения общей папки службы файлов Azure
...
Системная ошибка 53 может также произойти, если в клиенте включена проверка подлинности NTLMv1. Протокол проверки подлинности NTLMv1 делает клиент менее безопасным, в результате чего связь для службы файлов Azure блокируется. Чтобы проверить, заключается ли причина ошибки в этом, убедитесь, что для следующего подраздела реестра задано значение 3:
