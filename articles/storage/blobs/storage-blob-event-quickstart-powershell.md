...
> [!NOTE]
> Средство **Hookbin** не предназначено для использования с высокой пропускной способностью. Это средство используется исключительно для демонстрации. Если вы одновременно отправляете несколько событий, в средстве могут отобразиться не все события. Кроме того, имейте в виду, что служба "Сетка событий Azure" взаимодействует со средством **Hookbin** [особым образом](https://docs.microsoft.com/en-us/azure/azure-functions/functions-bindings-event-grid#create-a-requestbin-endpoint). Чтобы упростить тестирование, Сетка событий отправляет события, не требуя правильного ответа на запросы подтверждения подписки (что [так или иначе](https://docs.microsoft.com/en-us/azure/event-grid/security-authentication#validation-details) произойдет).
