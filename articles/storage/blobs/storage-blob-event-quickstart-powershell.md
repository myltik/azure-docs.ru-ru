...
Как правило, события можно отправлять в конечную точку, реагирующую на событие, такую как веб-перехватчик или функция Azure. Для упрощения примера, показанного в этой статье, события отправляются на URL-адрес, предназначенный только для сбора сообщений. Этот URL-адрес можно создать с помощью сторонних средств от [RequestBin](https://requestb.in/) или [Hookbin](https://hookbin.com/).
...
Подпишитесь на тему, чтобы определить в сетке событий Azure, какие события вам необходимо отслеживать. В следующем примере создается подписка на созданную учетную запись хранения и передается URL-адрес из RequestBin или Hookbin в качестве конечной точки для уведомления о событии. 
