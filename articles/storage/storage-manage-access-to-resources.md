...
Подписанный URL-адрес — это универсальный код ресурса (URI), обеспечивающий ограниченные права доступа к ресурсу хранилища в течение заданного интервала времени. Такие URL-адреса можно создавать для следующих ресурсов хранилища:
...
С помощью этого URL-адреса вы можете разрешить клиенту доступ к ресурсам в вашей учетной записи хранения, не предоставляя ключ своей записи.
...
Параметры запроса URI подписанного URL-адреса включают всю информацию, необходимую для предоставления контролируемого доступа к ресурсу хранилища. Параметры запроса URI подписанного URL-адреса включают в себя интервал времени, в течение которого подписанный URL-адрес является действительным, разрешения, которые он предоставляет, ресурс, который должен быть доступным по нему, версию, используемую для выполнения запроса, и подпись, по которой службы хранилища будут аутентифицировать запрос.
...
Хранимая политика доступа предоставляет больший контроль над выданными подписанными URL-адресами. Вместо указания времени существования и разрешений для подписанного URL-адреса можно указать эти параметры в политике доступа, хранимой в контейнере, общей папке, очереди или таблице, в которых содержатся общие ресурсы. Чтобы изменить эти параметры для одной или нескольких подписей, следует изменить хранимую политику доступа, при этом не нужно перевыпускать подписи. Можно также быстро отозвать подпись, изменив хранимую политику доступа.
...
Имя хранимой политики доступа может состоять из максимум 64 символов и является уникальным в пределах контейнера, общей папки, очереди и таблицы. Чтобы сопоставить подписанный URL-адрес с хранимой политикой доступа, следует указать этот идентификатор при создании подписанного URL-адреса. Поле *signedidentifier* в URI подписанного URL-адреса задает идентификатор для хранимой политики доступа.

Контейнер, общая папка, очередь и таблица могут включать до 5 хранимых политик доступа. Каждая политика может использоваться любым количеством подписанных URL-адресов.

>[AZURE.NOTE]Для ввода в действие хранимой политики доступа в контейнере, общей папке, очереди или таблице после настройки может потребоваться до 30 секунд. В этот промежуток времени попытка применения подписанного URL-адреса, связанного с хранимой политикой доступа, будет завершаться ошибкой с кодом состояния 403 (запрещено), пока политика доступа не станет активной.
