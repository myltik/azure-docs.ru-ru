...
> [AZURE.NOTE] В настоящее время учетные записи хранения с типом репликации "Хранилище, избыточное в пределах зоны (ZRS)" не имеют действующих функций метрик или ведения журнала. Кроме того, служба файлов Azure сейчас не поддерживает ведение журнала.
...
Если вы знакомы с системным монитором Windows, то можете считать, что метрики хранилища — эквивалент счетчиков службы хранилища Azure. Они предоставляют полный набор метрик (счетчиков в терминологии системного монитора Windows), позволяющих оценить, например, доступность службы, общее количество запросов к службе, а также процент успешно обработанных запросов. Полный список доступных показателей см. в разделе [Схема таблицы показателей аналитики хранения](http://msdn.microsoft.com/library/azure/hh343264.aspx). Вы можете настроить службу хранилища так, чтобы она собирала и агрегировала метрики каждый час или каждую минуту. Дополнительные сведения о том, как включить метрики и отслеживать учетные записи хранилища, см. в статье [Включение метрик хранилища Azure и просмотр данных метрик](http://go.microsoft.com/fwlink/?LinkId=510865).

Вы можете выбрать, какие ежечасные метрики должны отображаться на [портале Azure](https://portal.azure.com), и настроить правила для уведомления администраторов по электронной почте обо всех случаях, когда эти метрики превышают пороговые уровни. Дополнительные сведения см. в статье [Получение уведомлений об оповещениях](../azure-portal/insights-receive-alert-notifications.md).
...
С помощью клиентской библиотеки хранилища для .NET вы можете собирать на стороне клиента данные журнала, связанные с операциями хранилища, которые выполняются приложением. Дополнительные сведения см. в разделе [Вход в клиентскую библиотеку хранилища .NET на стороне клиента](http://go.microsoft.com/fwlink/?LinkId=510868).
...
- [Fiddler](http://www.telerik.com/fiddler) — это бесплатный прокси-сервер отладки веб-приложений, с помощью которого можно проверять заголовки и полезные данные HTTP- и HTTPS-запросов, а также ответных сообщений. Дополнительные сведения см. в разделе [Приложение 1. Отслеживание HTTP- и HTTPS-трафика с помощью Fiddler](#appendix-1).
- [Microsoft Network Monitor (Netmon)](http://www.microsoft.com/download/details.aspx?id=4865) и [Wireshark](http://www.wireshark.org/) — бесплатные средства для анализа сетевых протоколов, с помощью которых можно просматривать подробную информацию о пакетах для широкого спектра сетевых протоколов. Дополнительные сведения о Wireshark см. в [приложении 2 "Отслеживание сетевого трафика с помощью Wireshark"](#appendix-2).
...
На приведенной ниже иллюстрации из средства мониторинга [портала Azure](https://portal.azure.com) показана ситуация, когда значение метрики **AverageE2ELatency** значительно выше значения метрики **AverageServerLatency**.
...
Вы должны просмотреть журналы на стороне клиента, чтобы понять, сколько запросов отправляет приложение, а также проверить общие узкие места, связанные с производительностью .NET, в приложении. К ним относятся использование процессора, сети и памяти, а также сборка мусора .NET. В качестве отправной точки для диагностики и устранения неполадок в клиентских приложениях .NET используйте статью [Debugging, Tracing, and Profiling](http://msdn.microsoft.com/library/7fe0dd2y) (Отладка, трассировка и профилирование).
...
### <a name="metrics-show-low-AverageE2ELatency-and-low-AverageServerLatency"></a>Метрики содержат низкие значения AverageE2ELatency и AverageServerLatency, но клиент сталкивается с большой задержкой
...
> [AZURE.NOTE] Подробный контрольный список обеспечения производительности можно найти в статье [Производительность хранилища Microsoft Azure и контрольный список масштабируемости](storage-performance-checklist.md).
...
#### <a name="transient-increase-in-PercentThrottlingError"></a>Временное увеличение значения PercentThrottlingError
...
> [AZURE.NOTE] В значении метрики **PercentThrottlingError** также могут наблюдаться пиковые скачки, не совпадающие с периодами высокой активности приложения. Наиболее вероятная причина в этом случае — перемещение секций службой хранилища для балансировки нагрузки.
...
Microsoft.WindowsAzure.Storage|Информация|3|85d077ab -…|Запуск синхронного запроса по адресу https://domemaildist.blob.core.windows.netazureimblobcontainer/blobCreatedViaSAS.txt?sv=2014-02-14&amp;sr=c&amp;si=mypolicy&amp;sig=OFnd4Rd7z01fIvh%2BmcR6zbudIH2F5Ikm%2FyhNYZEmJNQ%3D&amp;api-version=2014-02-14.
...
Если вы генерируете маркеры SAS с помощью клиентской библиотеки хранилища, то можете легко создать действительный маркер. Но если вы используете REST API службы хранилища и создаете токены SAS вручную, рекомендуем внимательно изучить статью [Делегирование доступа с помощью подписанного URL-адреса](http://msdn.microsoft.com/library/azure/ee395415.aspx).
...
#### <a name="client-previously-deleted-the-object"></a>Объект ранее удален клиентом или другим процессом
...
---|---
07b26a5d-...|Метод **DeleteIfExists** для удаления контейнера BLOB-объектов. Обратите внимание, что эта операция включает в себя запрос **HEAD**, который проверяет существование контейнера.
...
---|---
07b26a5d-...|Запуск синхронного запроса по адресу https://domemaildist.blob.core.windows.net/azuremmblobcontainer.
...
07b26a5d-... | Downloading response body (Загружается тело ответа).
07b26a5d-... | Operation completed successfully (Операция выполнена успешно).
e2d06d78-... | Запуск асинхронного запроса по адресу https://domemaildist.blob.core.windows.net/azuremmblobcontainer.</td>.
e2d06d78-... | StringToSign = HEAD............x-ms-client-request-id:e2d06d78-....x-ms-date:Tue, 03 Jun 2014 10:33:12 GMT.x-ms-version:2014-02-14./domemaildist/azuremmblobcontainer.restype:container.
e2d06d78-...| Waiting for response (Ожидание ответа).
de8b1c3c-... | Запуск синхронного запроса по адресу https://domemaildist.blob.core.windows.net/azuremmblobcontainer/blobCreated.txt.
...
e2d06d78-... | Запуск асинхронного запроса по адресу https://domemaildist.blob.core.windows.net/azuremmblobcontainer.
...
### <a name="metrics-show-low-percent-success"></a>Метрики показывают низкие значения PercentSuccess, или в записях журналов аналитики присутствуют операции с состоянием транзакции ClientOtherErrors
...
Полный список кодов ошибок API REST, возвращаемых службами хранилища. см. на странице [Общие коды ошибок API-интерфейса REST](http://msdn.microsoft.com/library/azure/dd179357.aspx).
...
#### <a name="error-HTTP-header-not-correct-format"></a>При использовании эмулятора хранения возникает ошибка "Неправильный формат значения одного из заголовков HTTP"
...
Кроме того, вы можете просмотреть данные TCP так, как они видны прикладному уровню. Для этого щелкните данные TCP правой кнопкой мыши и выберите пункт **Follow TCP Stream** (Отслеживать поток TCP). Это особенно полезно в том случае, если дамп был получен без использования фильтра захвата. Дополнительные сведения см. в разделе [Following TCP Streams](http://www.wireshark.org/docs/wsug_html_chunked/ChAdvFollowTCPSection.html) (Отслеживание потоков TCP).

![][8]

> [AZURE.NOTE] Дополнительные сведения об использовании Wireshark см. в [руководстве пользователя Wireshark](http://www.wireshark.org/docs/wsug_html_chunked).
...
Дополнительные сведения о трассировке **веб-прокси** в Microsoft Message Analyzer см. в разделе [Microsoft-PEF-WebProxy Provider](http://technet.microsoft.com/library/jj674814.aspx) (Поставщик Microsoft-PEF-WebProxy).
