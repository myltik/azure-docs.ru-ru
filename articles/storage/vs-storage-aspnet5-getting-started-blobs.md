<properties
	...
	description="Как приступить к работе с хранилищем больших двоичных объектов Azure в проекте ASP.NET 5 в Visual Studio"
	services="storage"
	documentationCenter=""
	authors="patshea123"
	manager="douge"
...
Вы также можете использовать `CloudBlobClient` для создания контейнера в своей учетной записи хранения. Вам нужно всего лишь добавить вызов `CreateIfNotExistsAsync`, как в следующем коде:
...
Чтобы скачать большой двоичный объект, сначала получите ссылку на него, а затем вызовите метод `DownloadToStreamAsync`. В следующем примере метод `DownloadToStreamAsync` используется для переноса содержимого большого двоичного объекта в объект stream, который затем можно сохранить в локальном файле.
