<properties
	pageTitle="Производительность хранилища Microsoft Azure и контрольный список масштабируемости"
...
|----|------|-----------|-----------
||Все службы| Целевые показатели масштабируемости|[Ваше приложение создано таким образом, чтобы избегать приближения к целевым показателям масштабируемости?](#subheading1) ||Все службы| Сетевые подключения| [Устройства на стороне клиента имеют достаточно высокую пропускную способность и низкую задержку, чтобы достигнуть необходимой производительности?](#subheading2) ||Все службы| Сетевые подключения| [Устройства на стороне клиента имеют связь достаточно высокого качества?](#subheading3) ||Все службы| Сетевые подключения| [Клиентское приложение расположено "рядом" с учетной записью хранения?](#subheading4) ||Все службы| Распространение содержимого| [Вы используете CDN для распространения содержимого?](#subheading5) ||Все службы| Прямой клиентский доступ| [Вы используете SAS и CORS, чтобы обеспечить прямой доступ к хранилищу вместо прокси-сервера?](#subheading6) ||Все службы| Кэширование| [Ваше приложение кэширует данные, которые часто используются, но редко изменяются?](#subheading7) ||Все службы| Кэширование| [Ваше приложение проводит пакетную обработку обновлений (кэшируя их на стороне клиента и отправляя более крупными пакетами)?](#subheading8) ||Все службы| Конфигурация .NET | [Вы настроили свой клиент для использования достаточного количества одновременных подключений?](#subheading9) ||Все службы| Конфигурация .NET| [Вы настроили .NET на использование достаточного количества потоков?](#subheading10) ||Все службы| Конфигурация .NET| [Вы используете .NET 4.5 или более поздней версии, где улучшена функция сборки мусора?](#subheading11) ||Все службы| Параллелизм| [Вы обеспечили надлежащую связь параллелизма, чтобы не перегружать ни возможности клиента, ни целевые показатели масштабирования?](#subheading12) ||Все службы| Средства| [Вы используете последние версии клиентских библиотек и средств, предоставленных Майкрософт?](#subheading13) ||Все службы| Повторные попытки| [Вы используете политику повторных попыток с экспоненциальным переключением для регулирования ошибок и тайм-аутов?](#subheading14) ||Все службы| Повторные попытки| [Ваше приложение избегает повторных попыток при возникновении ошибок без повторов?](#subheading15) ||BLOB-объекты| Целевые показатели масштабируемости| [Ваше приложение остается в пределах пропускной способности или операционных показателей масштабируемости для одного BLOB-объекта?](#subheading16) ||BLOB-объекты| Копирование BLOB-объектов| [Вы эффективно копируете BLOB-объекты?](#subheading17) ||BLOB-объекты| Копирование BLOB-объектов| [Вы используете AzCopy для массового копирования BLOB-объектов?](#subheading18) ||BLOB-объекты| Копирование BLOB-объектов| [Вы используете службу импорта и экспорта Azure для передачи больших объемов данных?](#subheading19) ||BLOB-объекты| Использование метаданных| [Вы храните часто используемые метаданные о BLOB-объектах в их метаданных?](#subheading20) ||BLOB-объекты| Быстрая отправка| [Пытаясь быстро отправить BLOB-объект, отправляете ли вы блоки параллельно?](#subheading21) ||BLOB-объекты| Быстрая отправка| [Пытаясь быстро отправить несколько BLOB-объектов, отправляете ли вы их параллельно?](#subheading22) ||BLOB-объекты| Правильный тип BLOB-объекта| [Используете ли вы блочные и страничные BLOB-объекты, когда следует?](#subheading23) ||Таблицы| Целевые показатели масштабируемости| [Приближаетесь ли вы к целевым показателям масштабируемости для количества сущностей в секунду?](#subheading24) ||Таблицы| Конфигурация| [Вы используете JSON для запросов таблиц?](#subheading25) ||Таблицы| Конфигурация| [Вы задействовали отключение алгоритма Nagle, чтобы улучшить производительность небольших запросов?](#subheading26) ||Таблицы| Таблицы и секции| [Вы секционировали данные надлежащим образом?](#subheading27) ||Таблицы| Горячие разделы| [Вы избегаете использования инкрементируемых и декрементируемых шаблонов?](#subheading28) ||Таблицы| Горячие разделы| [Ваши добавления и обновления распространяются на несколько секций?](#subheading29) ||Таблицы| Объем запроса| [Вы создали такую схему, при которой запросы точки используются в большинстве случаев, а запросы таблицы время от времени?](#subheading30) ||Таблицы| Плотность запроса| [Ваши запросы обычно проверяют и возвращают только те строки, которые будет использовать приложение?](#subheading31) ||Таблицы| Ограничение возвращаемых данных| [Вы используете фильтрацию, чтобы ограничить возвращение ненужных сущностей?](#subheading32) ||Таблицы| Ограничение возвращаемых данных| [Вы используете проекцию, чтобы избежать возвращения ненужных свойств?](#subheading33) ||Таблицы| Денормализация| [Вы денормализовали свои данные, чтобы избежать неэффективных запросов или множественных запросов на чтение при попытке получить данные?](#subheading34) ||Таблицы| Вставка/Обновление/Удаление| [Вы проводите пакетную обработку запросов, которые должны быть транзакционными или могут выполняться одновременно, чтобы сократить количество круговых путей?](#subheading35) ||Таблицы| Вставка/Обновление/Удаление| [Вы избегаете получения сущности только для того, чтобы узнать, следует ли вызывать вставку или обновление?](#subheading36) ||Таблицы| Вставка/Обновление/Удаление| [Вы обдумали возможность хранения набора данных, которые будут часто вызываться вместе, в виде свойств в одной сущности, а не в виде нескольких сущностей?](#subheading37) ||Таблицы| Вставка/Обновление/Удаление| [Вы подумали об использовании BLOB-объектов вместо таблиц для сущностей, которые всегда будут вызываться вместе и могут быть записаны в виде пакетов (например, данных временной последовательности)?](#subheading38) ||Очереди| Целевые показатели масштабируемости| [Вы приближаетесь к целевым показателям масштабируемости для количества сообщений в секунду?](#subheading39) ||Очереди| Конфигурация| [Вы задействовали отключение алгоритма Nagle, чтобы улучшить производительность небольших запросов?](#subheading40) ||Очереди| Размер сообщения| [Ваши сообщения достаточно компактны, чтобы улучшить производительность очереди?](#subheading41) ||Очереди| Пакетное извлечение| [Вы получаете несколько сообщений с помощью одной операции Get?](#subheading41) ||Очереди| Частота опроса| [Вы проводите опрос достаточно часто, чтобы сократить воспринимаемую задержку приложения?](#subheading42) ||Очереди| Обновление сообщения| [Вы используете UpdateMessage, чтобы сохранять прогресс обработки сообщений, избегая необходимости повторно обрабатывать все сообщения при возникновении ошибки?](#subheading43) ||Очереди| Архитектура| [Вы используете очереди, чтобы сделать все приложение более масштабируемым за счет устранения длительных рабочих нагрузок с критически важного пути и их независимого масштабирования?](#subheading44)
...
####Полезные ресурсы
Следующие ссылки предоставляют дополнительные сведения о целевых показателях масштабирования. Текущие целевые показатели масштабируемости можно посмотреть на странице [Целевые показатели масштабируемости и производительности хранилища Azure в MSDN](http://msdn.microsoft.com/library/azure/dn249410.aspx). Дополнительные сведения о вариантах избыточности хранилища в записи блога [Варианты избыточности хранилища Azure и геоизбыточное хранилище с доступом для чтения](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/11/introducing-read-access-geo-replicated-storage-ra-grs-for-windows-azure-storage.aspx) — подробнее см. в следующем разделе. Текущие сведения о ценах на службы Azure см. в разделе [Обзор цен на Azure](http://azure.microsoft.com/pricing/overview/).
...
#####Полезные ресурсы
Для получения дополнительной информации о размерах виртуальной машины и выделенной полосы пропускания см. раздел [Рекомендации по проектированию крупномасштабных служб на основе облачных служб Azure](http://msdn.microsoft.com/library/azure/dn197896.aspx) в сети MSDN.
...
Для получения дополнительной информации о сети Azure CDN см. раздел [Сеть Azure CDN](http://azure.microsoft.com/services/cdn/).
...
Для получения дополнительной информации о технологии CORS см. раздел [Поддержка общего доступа к ресурсам независимо от источника (CORS) для службы хранилища Azure в сети MSDN](http://msdn.microsoft.com/library/azure/dn535601.aspx).
...
Пример того, как получить свойства BLOB-объекта, необходимые для выяснения даты последнего изменения с помощью .NET, см. в разделе [Задание и получение свойств и метаданных](http://msdn.microsoft.com/library/azure/hh225342.aspx) в MSDN. Для получения дополнительной информации об условных загрузках см. раздел [Условное обновление локальной копии BLOB-объекта](http://msdn.microsoft.com/library/azure/dd179371.aspx) в MSDN.
...
Дополнительные сведения см. в записи [Веб-службы: одновременные подключения](http://blogs.msdn.com/b/darrenj/archive/2005/03/07/386655.aspx) на сайте MSDN.
...
Дополнительные сведения см. в разделе [Метод ThreadPool.SetMinThreads] (http://msdn.microsoft.com/library/system.threading.threadpool.setminthreads(v=vs.110).aspx) на сайте MSDN.
...
Для получения дополнительной информации см. статью [Обзор новых способов повышения производительности в .NET 4.5](http://msdn.microsoft.com/magazine/hh882452.aspx) в MSDN.
...
Для получения дополнительной информации о целевой пропускной способности для BLOB-объекта см. раздел [Целевые показатели по масштабируемости и производительности хранилища Azure](http://msdn.microsoft.com/library/azure/dn249410.aspx) в MSDN.
...
Пример того, как получить метаданные BLOB-объекта с помощью .NET, см. в разделе [Задание и получение свойств и метаданных](http://msdn.microsoft.com/library/azure/hh225342.aspx) в MSDN.
...
Для получения дополнительной информации см. раздел [Что такое блочные и страничные BLOB-объекты](http://msdn.microsoft.com/library/azure/ee691964.aspx) в MSDN.
...
Дополнительные сведения см. в записях [Таблицы Microsoft Azure: введение в JSON](http://blogs.msdn.com/b/windowsazurestorage/archive/2013/12/05/windows-azure-tables-introducing-json.aspx) и [Формат полезных данных для операций службы таблиц](http://msdn.microsoft.com/library/azure/dn535600.aspx) на сайте MSDN.
...
Текущие целевые показатели масштабирования см. на странице [Целевые показатели производительности и масштабируемости хранилища Azure](http://msdn.microsoft.com/library/azure/dn249410.aspx) на сайте MSDN.
...
Актуальные сведения о стоимости см. в разделе [Сведения о ценах на хранилища](http://azure.microsoft.com/pricing/details/storage/).
...
Дополнительные сведения см. в статье [Практическое руководство: изменение содержимого сообщения в очереди](../storage-dotnet-how-to-use-queues/#change-contents).
