<properties
   pageTitle="Приступая к работе с внутренним балансировщиком нагрузки | Microsoft Azure"
   description="Настройка внутреннего балансировщика нагрузки и способы его реализации для виртуальных машин и облачных служб."
...
Внутренняя балансировка нагрузки Azure обеспечивает балансировку нагрузки между виртуальными машинами, которые размещены в облачной службе или региональной виртуальной сети. Информацию об использовании и настройке региональных виртуальных сетей см. в статье [Региональные виртуальные сети](virtual-networks-migrate-to-regional-vnet.md). В имеющихся виртуальных сетях, настроенных для территориальной группы, невозможно использовать внутреннюю балансировку нагрузки.
...
Для существующих виртуальных машин можно добавить конечные точки в экземпляр внутренней балансировки нагрузки с помощью следующих команд:
...
- Существующая облачная служба с именем Contoso-Legal, в которой размещены виртуальные машины.
...
- Три существующих сервера бизнес-приложений с именами LEGAL-1, LEGAL-2 и LEGAL-3.
...
Чтобы добавить виртуальную машину в экземпляр внутренней балансировки нагрузки при его создании, воспользуйтесь командлетами New-AzureInternalLoadBalancerConfig и New-AzureVMConfig.
...
## Внутренняя балансировка нагрузки для облачных служб


Внутренняя балансировка нагрузки поддерживается как для виртуальных машин, так и для облачных служб. Доступ к конечной точке внутренней балансировки нагрузки, созданной в облачной службе за пределами региональной виртуальной сети, можно будет получить только из облачной службы.

Конфигурацию внутренней балансировки нагрузки необходимо настраивать во время создания первого развертывания в облачной службе, как показано в приведенном ниже примере командлета.
...
Дополнительные сведения о схеме подсистемы балансировки нагрузки см. в статье [Добавление подсистемы балансировки нагрузки](https://msdn.microsoft.com/library/azure/dn722411.aspx).
...
## Дополнительные сведения о командлетах для внутренней балансировки нагрузки
