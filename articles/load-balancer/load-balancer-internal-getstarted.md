...
>[AZURE.IMPORTANT]Для выполнения указанных ниже действия необходимо, чтобы виртуальная сеть была уже создана и готова для облачного развертывания. Для создания внутренней балансировки нагрузки потребуется имя виртуальной сети и имя подсети.

### Шаг 1.

Откройте файл конфигурации службы (.cscfg) для облачного развертывания в Visual Studio и добавьте под последним элементом «</Role>» в конфигурации сети показанный ниже раздел, чтобы создать внутреннюю балансировку нагрузки.
...
Добавим значения в файл конфигурации сети, чтобы показать, как он будет выглядеть. Для примера предположим, что вы создали подсеть с именем test_vnet и подсетью 10.0.0.0/24, имеющей имя test_subnet и статический IP-адрес 10.0.0.4. Подсистема балансировки нагрузки будет называться testLB.
...
Дополнительные сведения о схеме балансировки нагрузки см. в статье [Добавление подсистемы балансировки нагрузки](https://msdn.microsoft.com/library/azure/dn722411.aspx)

### Шаг 2.


Измените файл определения службы (.csdef), добавив конечные точки для подсистемы внутренней балансировки нагрузки. При создании экземпляра роли файл определения службы добавляет его в подсистему внутренней балансировки нагрузки.
...
Используя те же значения, что и в приведенном выше примере, добавим значения в файл определения службы.
...
Сетевой трафик будет балансироваться с использованием подсистемы балансировки нагрузки testLB через порт 80 для входящих запросов и отправлять в экземпляры рабочих ролей также через порт 80.
