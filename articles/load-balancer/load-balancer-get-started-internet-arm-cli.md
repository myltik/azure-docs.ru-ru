...
Создайте виртуальную сеть *NRPVnet* в регионе "Восток США" (East US) с помощью группы ресурсов *NRPRG*.
...
- **-n** — имя ресурса (это может быть правило NAT, правило пробы или правило балансировки нагрузки);
- **-p** — протокол (TCP или UDP);  
- **-f** — порт клиентской части, который следует использовать (команда пробы использует параметр -f для определения пути пробы);
...
Создайте сетевую карту *lb-nic1-be* и сопоставьте ее с правилом NAT *rdp1*, а также с пулом адресов серверной части *NRPbackendpool*.
...
- **-e** — идентификатор правила NAT, который будет связан с ресурсом сетевой карты (начинается с /subscriptions/####################################/resourceGroups/<resourcegroup-name>/providers/Microsoft.Network/loadBalancers/<load-balancer-name>/inboundNatRules/<nat-rule-name>).
...
Создайте сетевую карту *lb-nic2-be* и сопоставьте ее с правилом NAT *rdp2*, а также с пулом адресов серверной части *NRPbackendpool*.

 	azure network nic create -g nrprg -n lb-nic2-be --subnet-name nrpvnetsubnet --subnet-vnet-name nrpvnet -d "/subscriptions/####################################/resourceGroups/nrprg/providers/Microsoft.Network/loadBalancers/nrplb/backendAddressPools/NRPbackendpool" -e "/subscriptions/####################################/resourceGroups/nrprg/providers/Microsoft.Network/loadBalancers/nrplb/inboundNatRules/rdp2" eastus

### Шаг 3. 

Создайте виртуальную машину *web1* и сопоставьте ее с сетевой картой *lb-nic1-be*. Учетная запись хранения *web1nrp* была создана перед выполнением следующей команды.
...
Так как сетевая карта *lb-nic1-be* сопоставлена с правилом NAT *rdp1*, вы можете подключиться к виртуальной машине *web1* с помощью протокола удаленного рабочего стола через порт 3441 в балансировщике нагрузки.

### Шаг 4.

Создайте виртуальную машину *web2* и сопоставьте ее с сетевой картой *lb-nic2-be*. Учетная запись хранения *web1nrp* была создана перед выполнением следующей команды.
