...
>[!NOTE]
> Azure предоставляет набор полностью управляемых решений балансировки нагрузки для сценариев.  Если вам необходима обработка подключений TLS для последующей передачи (разгрузка SSL) или обработка прикладного уровня запросов по HTTP/HTTPS, ознакомьтесь со статьей [Обзор шлюза приложений](../application-gateway/application-gateway-introduction.md).  Если вам нужна глобальная балансировка нагрузки DNS, ознакомьтесь со статьей о [диспетчере трафика](../traffic-manager/traffic-manager-overview.md).  Комплексные сценарии могут получить преимущества от объединения этих решений при необходимости.
...
| Зоны доступности | Интерфейсы, избыточные в пределах зоны, и зональные интерфейсы для входящего и исходящего трафика, устойчивость сопоставлений исходящих потоков к сбоям зоны, межзональная балансировка нагрузки. | / |
| Диагностика | Azure Monitor, многомерные метрики (счетчики пакетов и байтов), состояние проверки работоспособности, попытки подключения (TCP SYN), работоспособность исходящего подключения (успешные и завершившиеся со сбоем потоки SNAT), активные измерения на плоскости данных. | Azure Log Analytics только для общедоступных Load Balancer, оповещение о нехватке SNAT, счетчик работоспособности серверного пула. |
| Порты HA | Внутренний Load Balancer | / |
| Обеспечение безопасности по умолчанию | По умолчанию закрыто для общедоступных IP-адресов и конечных точек Load Balancer, а группа безопасности сети используется, чтобы явно указать в списке разрешений прохождение трафика. | Открыто по умолчанию, группа безопасности сети необязательна. |
| Исходящие подключения | Несколько внешних интерфейсов с отказом по правилу. Чтобы виртуальная машина могла использовать исходящее подключение, _необходимо_ явно создать сценарий исходящего подключения.  К [конечным точкам службы виртуальной сети](../virtual-network/virtual-network-service-endpoints-overview.md) можно получить доступ без исходящего подключения. Такие подключения не учитываются в обработанных данных.  Доступ к любым общедоступным IP-адресам, включая службы Azure PaaS, недоступные в качестве конечных точек службы виртуальной сети, следует получать через исходящие подключения. Это учитывается при обработке данных. Когда только внутренний Load Balancer обслуживает виртуальную машину, исходящие подключения через SNAT по умолчанию недоступны. При программировании исходящих SNAT-подключений учитывается транспортный протокол на основе протокола правила балансировки нагрузки входящего трафика. | При наличии нескольких внешних интерфейсов один интерфейс выбирается случайно.  Когда только внутренний Load Balancer обслуживает виртуальную машину, используется SNAT по умолчанию. |
| Несколько внешних интерфейсов | Исходящий и входящий | Только входящие данные |
| Операции управления | Большинство операций < 30 секунд | Обычно 60–90+ секунд |
...
| Цены | Плата основывается на числе правил, обработанных входящих или исходящих данных, связанными с ресурсом.  | Бесплатно |
...
**Рисунок 1. Балансировка нагрузки веб-трафика с помощью общедоступного Load Balancer**
...
**Рисунок 2. Балансировка нагрузки многоуровневых приложений с помощью общедоступных и внутренних подсистем балансировки нагрузки**

## <a name="pricing"></a>Цены
Load Balancer уровня "Стандартный" является продуктом, за который взимается плата, основанная на количестве настроенных правил балансировки нагрузки и всех обработанных входящих и исходящих данных. Сведения о ценах на Load Balancer уровня "Стандартный" см. на [странице цен](https://azure.microsoft.com/pricing/details/load-balancer/).
