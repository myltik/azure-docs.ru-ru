...
## Основные сведения о балансировщике нагрузки Azure в классической модели Azure и диспетчере ресурсов Azure (ARM)

Чтобы ресурсы облака были доступны через Интернет, всем им необходимы общедоступные IP-адреса. В облачной инфраструктуре Microsoft Azure для ресурсов используются немаршрутизируемые IP-адреса, и для взаимодействия с Интернетом выполняется преобразование сетевых адресов (NAT) в общедоступные IP-адреса.

В Microsoft Azure существует две модели развертывания и 2 соответствующие реализации балансировщика нагрузки:

 
### Классическая модель Azure

Классическая модель Azure — первая модель развертывания, реализованная в Microsoft Azure. В этой модели облачной службе назначаются общедоступный IP-адрес и полное доменное имя, и виртуальные машины, развернутые в пределах облачной службы, можно группировать для использования балансировщика нагрузки. Балансировщик нагрузки будет выполнять преобразование портов и балансировать нагрузку сетевого трафика, используя общедоступный IP-адрес для облачной службы.

В классической модели развертывания преобразование портов выполняется с помощью конечных точек, которые связывают общедоступный порт общедоступного IP-адреса и локальный порт, назначенный для отправки трафика на конкретную виртуальную машину.
...
Далее приведено графическое представление балансировщика нагрузки в классической модели развертывания: ![подсистема балансировки нагрузки на основе хэша](./media/load-balancer-overview/asm-lb.png)

### Диспетчер ресурсов Azure
 
Концепция балансировки нагрузки для диспетчера ресурсов Azure (ARM) изменяется, поскольку в этом случае облачная служба для создания балансировщика нагрузки не нужна.

В диспетчере ресурсов Azure общедоступный IP-адрес — это собственный ресурс диспетчера, и его можно связать с меткой домена или DNS-именем. Общедоступный IP-адрес в этом случае связывается с ресурсом балансировщика нагрузки, поэтому правила балансировки нагрузки и правила для входящих подключений NAT будут использовать общедоступный IP-адрес в качестве конечной точки Интернета для ресурсов, получающих сбалансированный сетевой трафик.
...
Группа доступности — это метод группировки, используемый для добавления виртуальных машин к балансировщику нагрузки. Группа доступности гарантирует, что виртуальные машины не находятся на одном и том же физическом оборудовании и что в случае сбоя физической облачной инфраструктуры балансировщик нагрузки всегда будет иметь виртуальную машину, получающую сбалансированный сетевой трафик.

Далее приведено графическое представление балансировщика нагрузки в диспетчере ресурсов Azure (ARM):

![подсистема балансировки нагрузки на основе хэша](./media/load-balancer-overview/arm-lb.png)
