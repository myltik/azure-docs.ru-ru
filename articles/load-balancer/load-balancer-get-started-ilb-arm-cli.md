---
title: Создание внутреннего балансировщика нагрузки в Resource Manager с помощью Azure CLI | Microsoft Docs
...
[Классическая модель развертывания](load-balancer-get-started-ilb-classic-cli.md).
...
1. Если вы никогда не использовали Azure CLI, то см. статью [Установка Azure CLI](../xplat-cli-install.md). Следуйте инструкциям до того момента, где выбираются учетная запись и подписка Azure.
...
1. Создайте сетевую карту *lb-nic2-be*, а затем свяжите ее с правилом NAT *rdp2* и внутренним пулом адресов *beilb*.
   
        azure network nic create -g nrprg -n lb-nic2-be --subnet-name nrpvnetsubnet --subnet-vnet-name nrpvnet -d "/subscriptions/####################################/resourceGroups/nrprg/providers/Microsoft.Network/loadBalancers/nrplb/backendAddressPools/beilb" -e "/subscriptions/####################################/resourceGroups/nrprg/providers/Microsoft.Network/loadBalancers/nrplb/inboundNatRules/rdp2" eastus
2. Создайте виртуальную машину *DB1*, а затем свяжите ее с сетевой картой *lb-nic1-be*. Учетная запись хранения *web1nrp* создается перед выполнением следующей команды:
   ...
   > 
   > 
3. Создайте виртуальную машину *DB2*, а затем свяжите ее с сетевой картой *lb-nic2-be*. Учетная запись хранения *web1nrp* была создана перед выполнением следующей команды:
