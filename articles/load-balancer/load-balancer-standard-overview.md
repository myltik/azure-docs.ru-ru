...
Общедоступный IP-адрес, созданный в определенной зоне, всегда существует только в этой зоне.  Невозможно изменить зону общедоступного IP-адреса.  Если вам нужен общедоступный IP-адрес, который можно назначать ресурсам в нескольких зонах, создайте избыточный в пределах зоны общедоступный IP-адрес.
...
При использовании в регионе, который также предлагает зоны доступности, общедоступный IP-адрес уровня "Стандартный" автоматически становится отказоустойчивым в зоне, если он не был объявлен зональным.  Невозможно изменить зоны зональных общедоступных IP-адресов.
...
>[!NOTE]
>Если вы хотите использовать Load Balancer уровня "Стандартный" с функциями [Зоны доступности](https://aka.ms/availabilityzones) и [Порты HA](https://aka.ms/haports), для этих предварительных версий требуется отдельная регистрация.  Выполните соответствующие инструкции.
