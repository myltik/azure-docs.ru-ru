...
| Размер пула (экземпляры виртуальных машин) | Выделенные порты SNAT |
| --- | --- |
| 1–50 | 1024 |
| 51–100 | 512 |
| 101–200 | 256 |
...
| Группа безопасности сети | Необязательно для сетевого адаптера и подсети | Требуется |
...
| Группа безопасности сети | Необязательно для сетевого адаптера | Требуется |
...
>[!NOTE]
>Регистрация компонента Load Balancer уровня "Стандартный" вступит в силу повсеместно где-то в течение часа. Если вы хотите использовать Load Balancer уровня "Стандартный" с функциями [Зоны доступности](https://aka.ms/availabilityzones) и [Порты HA](https://aka.ms/haports), для этих предварительных версий требуется отдельная регистрация. Следуйте инструкциям по регистрации для использования этих функций.
...
2. Выполнение операции может занять до 10 минут. Состояние операции можно проверить, выполнив следующую команду:
    ...
    После завершения регистрации функции перейдите к следующему шагу:
