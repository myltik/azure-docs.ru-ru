...
4. В разделе **Интервал** укажите частоту выполнения проб приложения.
5. В разделе **Unhealthy treshold** (Порог состояния неработоспособности) укажите число неудачных попыток, по достижении которого серверная виртуальная машина будет помечена как неработоспособная.
5. Нажмите кнопку **ОК**, чтобы создать пробу.
