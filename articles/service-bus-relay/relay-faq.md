...
Ранее именованные службы ретрансляции шины обслуживания теперь называется [ретрансляции WCF](relay-wcf-dotnet-get-started.md). Вы можете использовать эту службу как обычно. Функция гибридных подключений представляет собой обновленную версию службы, перенесенной из служб BizTalk Azure. Ретранслятор WCF и гибридные подключения будут и дальше поддерживаться.
