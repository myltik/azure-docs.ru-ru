...
Чтобы показать эти возможности, работающие на базе телеметрии устройства, собранной через службы пакета IoT, и предоставленные пользователям в веб-приложении, это решение использует существующую модель машинного обучения Azure, доступную в качестве шаблона. Корпорация Майкрософт создала модель регрессии и опубликовала полный шаблон данных<sup>[1]</sup> и пошаговые руководства [здесь][lnk_regression_model].

Предварительно настроенное решение прогнозируемого обслуживания Azure IoT использует модель регрессии, созданную на основе этого шаблона. Оно развертывается в подписке Azure и предоставляется через опубликованный API. Решение включает набор проверочных данных, представляющий 4 (из 100) двигателя и 4 (из 21) потока данных датчиков, который обеспечивает точный результат из обученной модели.
