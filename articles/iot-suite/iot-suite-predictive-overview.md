...
Чтобы показать эти возможности, работающие на базе телеметрии устройства, собранной через службы пакета IoT, и предоставленные пользователям в веб-приложении, это решение использует существующую модель машинного обучения Azure, доступную в качестве шаблона. Корпорация Майкрософт создала [модель регрессии][lnk_regression_model] и опубликовала полный шаблон, данные<sup>[1]</sup> и пошаговые руководства.
