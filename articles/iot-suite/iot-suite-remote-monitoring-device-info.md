...
В [предварительной версии управления устройствами с помощью центра IoT][lnk-dm-preview] в центр IoT добавляются функции, похожие на функции управления сведениями об устройствах предварительно настроенного решения для удаленного мониторинга, описанные в этой статье. Однако сейчас решение для удаленного мониторинга использует только общедоступные функции в центре IoT.
...
- **HubEnabledState** — состояние устройства в центре IoT. До первого подключения устройства для этого параметра установлено значение **null**. На портале решения это представляется так: "Зарегистрировано, но отсутствует".
...
- **IsSimulatedDevice** — флаг, определяющий, что устройство является виртуальным.
...
При создании устройства на портале впервые решение создает запись в реестре устройств, как показано выше. Изначально для большей части сведений создаются заглушки, а для параметра **HubEnabledState** задается значение **null**. На этом этапе решение также формирует запись для устройства в реестре удостоверений устройств в центре IoT, которая создает ключи. Устройство использует их для проверки подлинности в центре IoT.
...
В списке устройств на портале решения отображаются следующие свойства устройства в виде столбцов: **состояние**, **идентификатор устройства**, **производитель**, **номер модели**, **серийный номер**, **встроенное ПО**, **платформа**, **процессор** и **установленное ОЗУ**. Такие свойства устройства, как **широта** и **долгота**, управляют расположением на карте Bing на панели мониторинга.

![Список устройств][img-device-list]

Если щелкнуть **Изменить** в области **Сведения об устройстве** на портале решения, можно изменить все перечисленные выше свойства. При изменении этих свойств обновляется запись для устройства в базе данных DocumentDB. Тем не менее если устройство отправляет сообщение с обновленными сведениями об устройстве, любые изменения, внесенные на портале решения, перезаписываются. На портале решения нельзя изменить свойства **DeviceId**, **Hostname**, **HubEnabledState**, **CreatedTime**, **DeviceState** и **UpdatedTime**, так как только устройство может изменять их.
...
Подробнее об отправке сообщений со сведениями об устройстве с пользовательского устройства см. в статье [Подключение устройства к предварительно настроенному решению для удаленного мониторинга][lnk-connect-custom].

Дополнительные сведения о настройке предварительно настроенных решений см. в статье [Настройка предварительно настроенного решения][lnk-customize].
