...
Microsoft Azure IoT Suite использует преимущества средств обеспечения безопасности и конфиденциальности, встроенных в платформу Azure и предоставляемых в рамках процессов SDL и OSA для безопасной разработки и эксплуатации программного обеспечения Майкрософт. Эти процедуры обеспечивают защиту инфраструктуры и сети, а также регулируют процедуры идентификации и управления, столь важные для безопасности любого решения. 
            ...
            [Центр Интернета вещей Azure](../iot-hub/iot-hub-what-is-iot-hub.md) в рамках [IoT Suite](iot-suite-what-is-azure-iot.md) — это полностью управляемая служба, которая обеспечивает надежный и безопасный двусторонний обмен данными между устройствами Интернета вещей и службами Azure, включая [машинное обучение Azure](../machine-learning/studio/what-is-machine-learning.md) и [Azure Stream Analytics](../stream-analytics/stream-analytics-introduction.md), с помощью учетных данных для безопасного доступа к каждому устройству и управления доступом.
...
Надежный обмен данными — это важная характеристика любого решения IoT. Обеспечение своевременной и точной доставки команд устройствам с обратным получением данных осложняется тем, что устройства IoT подключены через Интернет или другие аналогичные сети, которые могут быть ненадежными. Центр Интернета вещей Azure обеспечивает надежный обмен данными между облаком и устройствами с помощью системы подтверждений в ответ на сообщения. Дополнительный уровень надежности при обмене данными достигается за счет кэширования сообщений в Центре Интернета вещей: на протяжении семи дней для данных телеметрии и двух дней для команд.
...
Все ключи, используемые в инфраструктуре IoT, хранятся в защищенном облачном хранилище с возможностью продления, в случае если ключи должны быть повторно подготовлены. Данные могут храниться в [Azure Cosmos DB](../cosmos-db/introduction.md) или [базах данных SQL](../sql-database/sql-database-faq.md), что позволяет определять требуемый уровень безопасности. Кроме того, в Azure предусмотрены функции мониторинга и аудита всех операций доступа к данным, которые оповещают о попытках вторжения и несанкционированного доступа.
...
* 
            [
            **Центр Интернета вещей Azure**](https://azure.microsoft.com/services/iot-hub/) — шлюз, который обеспечивает подключение между облаком и "вещами". Вы можете изменять количество подключений к центру до миллионов, по-прежнему обрабатывая большие объемы данных с помощью функции проверки подлинности для каждого устройства, помогающей защитить решение.
