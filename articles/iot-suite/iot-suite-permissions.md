...
* Глобальный администратор может подготовить базовые и стандартные предварительно настроенные решения.

### <a name="domain-user"></a>Пользователь домена

На каждый клиент AAD может приходиться много пользователей домена.

* Пользователь домена может подготовить базовое предварительно настроенное решение на сайте [azureiotsuite.com][lnk-azureiotsuite].
* Пользователь домена может создать базовое предварительно настроенное решение с помощью интерфейса командной строки.
