...
Набор [предварительно настроенных решений][lnk-preconfigured-solutions] Azure IoT включает несколько служб Azure IoT, которые используются при создании комплексных решений для реализации распространенных бизнес-сценариев IoT. Предварительно настроенное решение для *удаленного мониторинга* подключается к устройствам и отслеживает их работу. Благодаря мониторингу вы можете анализировать поток данных, поступающих с устройств, и улучшать результаты работы компании, организовав автоматическое реагирование процессов на этот поток данных.

В этом руководстве показано, как подготовить предварительно настроенное решение для удаленного мониторинга. Кроме того, описываются основные функции этого решения. Многие функции доступны на панели мониторинга решения, которая развертывается вместе с предварительно настроенным решением:
...
Когда устройство впервые подключается к решению, оно отправляет свои метаданные решению. Если вы измените метаданные устройства через панель мониторинга решения, решение отправит новые значения метаданных на устройство, а также сохранит их в своей базе данных DocumentDB. Дополнительные сведения см. в разделе [Реестр удостоверений устройств и DocumentDB][lnk-devicemetadata].
...
Завершив работу, можно удалить предварительно настроенное решение из подписки Azure на сайте [azureiotsuite.com][lnk-azureiotsuite]. На этом сайте можно легко удалить все ресурсы, которые были подготовлены при создании предварительно настроенного решения.
