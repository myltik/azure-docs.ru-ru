...
> [!NOTE]
> Можно также выполнить этот шаг после установки Docker из диалогового окна **Параметры**. Щелкните правой кнопкой мыши значок программы **Docker** на панели задач Windows и выберите **Параметры**.
...
- Используемая `<SharedFolder>` и ее синтаксис описаны в разделе [Установка и настройка Docker](#install-and-configure-docker). Издателю OPC `<SharedFolder>` нужна для чтения собственного файла конфигурации, записи файла журнала, а также обеспечения доступности этих файлов вне контейнера.

- Издатель OPC считывает свою конфигурацию из файла **publishednodes.json**, который необходимо поместить в папку `<SharedFolder>/docker`. В этом файле конфигурации определено, на какие данные узла OPC UA на данном сервере OPC UA издатель OPC должен подписаться.
...
- Полный синтаксис файла **publishednodes.json** описан на странице [Издатель OPC](https://github.com/Azure/iot-edge-opc-publisher) в GitHub.

    В следующем фрагменте кода показан простой пример файла **publishednodes.json**. В этом примере показано, как опубликовать значение **CurrentTime** из сервера OPC UA с именем узла **win10pc**.
    ...
    В файле **publishednodes.json** сервер OPC UA задается с помощью URL-адреса конечной точки. Если вы укажете имя узла, используя метку имени узла (например, **win10pc**), как в предыдущем примере, вместо IP-адреса функция разрешения сетевого адреса в контейнере должна разрешить эту метку имени узла в IP-адрес.
...
1. Перейдите на страницу **Connect your own OPC UA server** (Подключение собственного сервера OPC UA) на портале решения подключенной фабрики. Выполните те же шаги, что и в предыдущем разделе, чтобы установить отношения доверия между порталом подключенной фабрики и сервером OPC UA.

    ![Портал решения](./media/iot-suite-connected-factory-gateway-deployment/image4.png)

1. Просмотрите дерево узлов OPC UA сервера OPC UA, щелкните правой кнопкой мыши узлы OPC UA, которые нужно передать подключенной фабрике, и выберите **Опубликовать**.
