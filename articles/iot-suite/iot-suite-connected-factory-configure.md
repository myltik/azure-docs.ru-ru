---
title: "Настройка топологии подключенной фабрики | Документация Майкрософт"
description: "Настройка топологии предварительно настроенного решения подключенной фабрики."
services: 
suite: iot-suite
...
---
# <a name="configure-the-connected-factory-preconfigured-solution"></a>Настройка предварительно настроенного решения подключенной фабрики

Предварительно настроенное решение подключенной фабрики показывает смоделированную панель мониторинга для вымышленной компании Contoso. Компания располагает фабриками в различных точках земного шара.

В этой статье в качестве примера используется Contoso для описания способа настройки топологии решения подключенной фабрики.
...
На серверах OPC UA расположены узлы OPC UA, а [издатель OPC](https://github.com/Azure/iot-edge-opc-publisher) отправляет значения этих узлов на подключенную фабрику. А именно:
...
Панель мониторинга можно использовать для подробного анализа топологии фабрики Contoso с глобального представления до представления уровня станции. Панель мониторинга подключенной фабрики позволяет выполнить следующее:
...
## <a name="connected-factory-topology"></a>Топология подключенной фабрики
...
Чтобы настроить свойства, перечисленные в предыдущем разделе, решение подключенной фабрики использует файл конфигурации с именем [ContosoTopologyDescription.json](https://github.com/Azure/azure-iot-connected-factory/blob/master/WebApp/Contoso/Topology/ContosoTopologyDescription.json).
...
`<opc_node_description>` содержит сведения об узлах OPC UA станции (серверы OPC UA). Узлы, которые не представляют существующие узлы OPC UA, но используются в качестве хранилища в логике вычислений подключенной фабрики, также допустимы. Имеет следующие свойства.
...
На уровне станции вы также можете видеть **смоделированные** объекты. Эти объекты используются только для настройки моделирования подключенной фабрики и не должны использоваться для настройки реальной топологии.
...
Свойства этой категории определяют внешний вид панели мониторинга подключенной фабрики. Примеры приведены ниже.
...
Показатель OEE и ключевой показатель эффективности для моделирования подключенной фабрики параметризуются в зависимости от:
...
Подключенная фабрика использует формулы OEE, опубликованные тут: http://oeeindustrystandard.oeefoundation.org.
...
Подключенная фабрика поддерживает простой механизм создания оповещений на основе минимальных и максимальных пороговых значений. Существует ряд предопределенных действий, которые вы можете настроить в качестве ответных на эти оповещения. Этот механизм управляется следующими свойствами:
...
Для определенных операций, таких как визуализация последнего значения или создание запросов службы "Аналитика временных рядов", WebApp нуждается в схеме адресации для полученных данных телеметрии. Телеметрия, отправленная в подключенную фабрику, также должна храниться во внутренних структурах данных. Два свойства, разрешающие эти операции, находятся на станции (сервер OPC UA) и на уровне узла OPC UA:
...
На [этой странице](https://github.com/Azure/iot-edge-opc-publisher) GitHub содержатся дополнительные сведения о том, как данные телеметрии поступают на подключенную фабрику с помощью издателя OPC.
