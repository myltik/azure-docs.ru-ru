...
1. На портале Azure нажмите кнопку **Cloud Shell**.
    ...
    ![Значок запуска на портале](media/iot-suite-integrate-data-lake/portal-launch-icon.png)
...
1. Нажмите кнопку **Создать ресурс**, выберите "Интернет вещей" в Marketplace и щелкните **Задание Stream Analytics**.
    ...
    ![Новое задание Stream Analytics](media/iot-suite-integrate-data-lake/new-stream-analytics-job.png)
...
1. Нажмите кнопку **Создать**.
    ...
    ![Создание задания Stream Analytics](media/iot-suite-integrate-data-lake/create-stream-analytics-job.png)
...
1. На странице обзора щелкните **Входные данные**.
    ...
    ![Страница обзора](media/iot-suite-integrate-data-lake/stream-analytics-overview.png)

1. Щелкните **Добавить потоковый вход** и выберите **Центр Интернета вещей** из раскрывающегося списка.

    ![Добавление входных данных](media/iot-suite-integrate-data-lake/stream-analytics-add-input.png)
...
1. В раскрывающемся списке "Группа потребителей" выберите ранее созданную группу потребителей. Здесь мы используем **streamanalyticsjob**.
    ...
    ![Выбор входных данных](media/iot-suite-integrate-data-lake/stream-analytics-new-input.png)

1. Выберите команду **Сохранить**.

1. На странице обзора щелкните **Выходные данные**.

    ![Добавление Data Lake Store](media/iot-suite-integrate-data-lake/stream-analytics-overview-2.png)

1. Щелкните **Добавить** и выберите **Data Lake Store** из раскрывающегося списка.

    ![Добавление выходных данных](media/iot-suite-integrate-data-lake/stream-analytics-output.png)
...
1. В поле формата даты введите **/streaming/{дата}/{время}**. Оставьте стандартные значения формата даты (ГГГГ/ММ/ДД) и формата времени (ЧЧ).
    ...
    ![Указание структуры папок](media/iot-suite-integrate-data-lake/stream-analytics-new-output.png)
...
1. Щелкните **Авторизовать**.
    ...
    ![Авторизация Stream Analytics в Data Lake Store](media/iot-suite-integrate-data-lake/stream-analytics-out-authorize.png)
...
1. На вкладке "Обзор" щелкните **Изменить запрос**.
    ...
    ![Изменить запрос](media/iot-suite-integrate-data-lake/stream-analytics-edit-query.png)
...
1. В редакторе запросов замените заполнители [YourOutputAlias] и [YourInputAlias] на псевдонимы выходных и входных данных, которые вы определили ранее.
    ...
    ![Запрос Stream Analytics](media/iot-suite-integrate-data-lake/stream-analytics-query.png)
...
1. На вкладке "Обзор" нажмите кнопку **Запуск**.
    ...
    ![Запускает задание Stream Analytics.](media/iot-suite-integrate-data-lake/stream-analytics-start.png)
...
1. Нажмите **Запуск**.
    ...
    ![Выбор настраиваемой даты](media/iot-suite-integrate-data-lake/stream-analytics-start-custom.png)
    ...
    ![Выполнение задания](media/iot-suite-integrate-data-lake/stream-analytics-running.png)
...
1. В обозревателе данных откройте папку **/streaming**. Вы увидите папки, созданные в формате "ГГГГ/ММ/ДД/ЧЧ".
    ...
    ![Анализ данных потоковой передачи](media/iot-suite-integrate-data-lake/data-lake-store-data-explorer.png)

    Вы увидите JSON-файлы, один файл за каждый час.

    ![Анализ данных потоковой передачи](media/iot-suite-integrate-data-lake/data-lake-store-file-preview.png)
