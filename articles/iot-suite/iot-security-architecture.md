...
Корпорация Майкрософт уже давно использует модели рисков и сделала процесс моделирования угроз доступным для всех. Опыт корпорации показывает, что кроме быстрого выяснения того, какие угрозы самые серьезные, моделирование предоставляет неожиданные преимущества. Например, таким образом можно обсудить угрозы с пользователями не из команды разработки, чтобы почерпнуть новые идеи и улучшить продукт.
...
**Незаконное изменение (Tampering, далее "T")**. Злоумышленник может частично или полностью заменить программное обеспечение устройства, чтобы другое ПО могло использовать удостоверение подлинности устройства, если материал ключа или средства шифрования, в которых есть материалы ключей, доступны несанкционированной программе. Например, злоумышленник может использовать полученный материал ключа, чтобы перехватить или изъять данные с устройства во время передачи и заменить их на ложные данные, а значит аутентифицироваться, используя украденный материал ключа.
...
**Незаконное изменение**. Для устройства могут настроить другое неизвестное системе управления состояние (параметры, которые не принадлежат к эталонным), поэтому устройство будет передавать данные, которые можно неправильно интерпретировать. Несанкционированное повышение привилегий. Устройство, которое выполняет определенную функцию, используют для принудительного выполнения других действий. Например, клапан, запрограммированный открываться наполовину, могут настроить открываться полностью.

**Отказ в обслуживании**. Устройство могут перевести в состояние, при котором невозможно поддерживать связь.

**Незаконное изменение**. Для устройства могут настроить другое неизвестное системе управления состояние (параметры, которые не принадлежат к эталонным), поэтому устройство будет передавать данные, которые можно неправильно интерпретировать.
...
**Спуфинг, раскрытие информации**. Устройства с ограниченными функциями и специализированные устройства часто используют общие данные защиты, такие как пароль или ПИН-код, или используют только отношение доверия к сети. Это значит, что они будут предоставлять доступ к сведениям устройствам в той же сети, а сети зачастую защищены только общим ключом. Следовательно, при раскрытии общего секрета устройства или сети можно будет управлять этим устройством или просматривать данные, которые оно отправляет.
...
**Незаконное изменение**. Злоумышленник может перехватить сигнал вещания, частично переопределить его данные или отправить ложные сведения. Раскрытие информации. Злоумышленник может перехватывать сигнал вещания и получить сведения, не авторизовавшись. Отказ в обслуживании. Злоумышленник может перехватить сигнал вещания и отказать в распространении информации
...
- [SDL Threat Modeling Tool](https://www.microsoft.com/sdl/adopt/threatmodeling.aspx) (Средство моделирования рисков SDL)

- [Microsoft Azure IoT reference architecture](https://azure.microsoft.com/updates/microsoft-azure-iot-reference-architecture-available/) (Эталонная архитектура Microsoft Azure IoT)
