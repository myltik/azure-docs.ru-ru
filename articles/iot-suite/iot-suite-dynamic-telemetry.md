...
## Добавление нового типа данных телеметрии
...
2. В файле remote\_monitoring.js можно увидеть базовые значения для существующих данных телеметрии температуры, влажности и внешней температуры. Добавьте новое базовое значение данных для **rpm** следующим образом.
...
3. Имитация устройства Node.js генерирует данные телеметрии, добавляя случайное приращение базовых значений данных с помощью функции **generateRandomIncrement**, определенной в файле remote\_monitoring.js. Рандомизируйте значение **rpm**, добавив строку кода после существующего кода рандомизации следующим образом.
...
Добавление определения **Telemetry**, показанное в примере выше, не изменяет поведение панели мониторинга. Тем не менее метаданные также могут содержать атрибут **DisplayName** для настройки отображения на панели мониторинга. Измените определение метаданных **Telemetry**, как показано ниже.
...
Обратите внимание, что это влияет только на внешний вид диаграммы. Значения данных **ExternalTemperature** по-прежнему сохраняются и остаются доступными для внутренней обработки.
