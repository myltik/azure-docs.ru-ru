...
* Активная подписка Azure. Если ее нет, можно создать бесплатную пробную учетную запись всего за несколько минут. Дополнительные сведения см. на странице [бесплатной пробной версии Azure][lnk_free_trial].
...
1. На панели удаленного мониторинга щелкните **+ Add a device** (+ Добавить устройство), затем добавьте пользовательское устройство. Запишите имя узла центра IoT, идентификатор устройства и ключ устройства. Они потребуются позже, при подготовке клиентского приложения устройства remote_monitoring.js.
2. Убедитесь, что на компьютере разработки установлен компонент Node.js 0.12.x или более поздняя версия. Выполните команду `node --version` в командной строке или оболочке, чтобы проверить версию. Сведения об использовании диспетчера пакетов для установки Node.js в Linux см. в статье [Installing Node.js via package manager][node-linux] (Установка Node.js с помощью диспетчера пакетов).
