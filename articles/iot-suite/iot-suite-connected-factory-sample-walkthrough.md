...
В этой статье рассматриваются некоторые основные компоненты решения подключенной фабрики, которые помогут вам представить, как оно работает. В статье также описывается порядок обмена данными в решении. Эти знания помогут вам:
...
11. Веб-браузер подключается к веб-приложению подключенной фабрики.
    ...
    - Для доступа к приложению подключенной фабрики требуется проверка подлинности пользователя через Azure Active Directory.
...
5. Транспорт прокси-сервера OPC получает из стека OPC UA запрос на подключение сокета TCP к серверу OPC UA.
    ...
    - Этот шаг является внутренним по отношению к веб-приложению подключенной фабрики.
...
> [!NOTE]
> Для сервера OPC UA и стека OPC UA в веб-приложении подключенной фабрики обмен данными прокси-сервера OPC прозрачный и применяются все возможности обеспечения безопасности для проверки подлинности и шифрования в OPC UA.
