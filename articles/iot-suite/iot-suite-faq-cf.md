...
### <a name="how-do-i-enable-an-interactive-map-in-my-connected-factory-solution"></a>Как включить интерактивную карту в решении подключенной фабрики?

Чтобы включить интерактивную карту в решении подключенной фабрики, требуется API для Карт Bing для организаций.

При развертывании с [www.azureiotsuite.com](http://www.azureiotsuite.com) проверяется, чтобы в вашей подписке был включен API для Карт Bing для организаций, и автоматически развертывается интерактивная карта в подключенную фабрику. Если это не так, можно включить интерактивную карту в развертывании следующим образом:

Если при развертывании с помощью сценария `build.ps1` в репозитории GitHub подключенной фабрики у вас есть API для Карт Bing для организаций, задайте переменную среды `$env:MapApiQueryKey` в окне создания для запроса ключа плана. Интерактивная карта включается автоматически.

При отсутствии API для Карт Bing для организаций разверните решение подключенной фабрики с адреса [www.azureiotsuite.com](http://www.azureiotsuite.com) или с помощью сценария `build.ps1`. Затем добавьте API для Карт Bing для организаций в подписку, как описано в разделе [Как создать API для Карт Bing для учетной записи для организаций](#how-do-i-create-a-bing-maps-api-for-enterprise-account). Просмотрите ключ запроса этой учетной записи, как описано в разделе [Как получить QueryKey для API для Карт Bing для организаций](#how-to-obtain-your-bing-maps-api-for-enterprise-querykey) и сохраните его. Перейдите на портал Azure и получите доступ к ресурсу службы приложений в развертывании подключенной фабрики. Перейдите к **параметрам приложений**, где можно найти раздел **Параметры приложения**. Задайте **MapApiQueryKey** для полученного ключа запроса. Сохраните параметры, а затем перейдите к области **Обзор** и перезапустите службу приложений.
...
После создания плана API карт Bing для организаций добавьте ресурс Карт Bing для организаций в группу ресурсов решения подключенной фабрики на портале Azure.
...
### <a name="how-do-i-use-non-opc-ua-devices-with-connected-factory"></a>Как использовать отличные от OPC UA устройства с подключенной фабрикой

Чтобы отправить данные телеметрии из отличных от OPC UA устройств в подключенную фабрику:

1. [Настройте новую станцию в топологии подключенной фабрики](iot-suite-connected-factory-configure.md) в файле `ContosoTopologyDescription.json`.

1. Примите данные телеметрии в подключенной фабрике, совместимые с форматом JSON:
...
1. Перезапустите службу приложений подключенной фабрики.
