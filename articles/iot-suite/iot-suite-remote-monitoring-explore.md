...
| Тип устройства        | Телеметрия                                  | Свойства                                  | Теги                    | Методы                                                                                      |
| ------------------ | ------------------------------------------ | ------------------------------------------- | ----------------------- | -------------------------------------------------------------------------------------------- |
...
| Имя правила | Описание | Пороговое значение | Severity | Затрагиваемые устройства |
| --------- | ----------- | --------- | -------- | ---------------- |
...
1. Чтобы перейти на страницу **Maintenance** (Обслуживание), щелкните **...** в столбце **Explore Alarm** (Изучить оповещение) рядом с оповещением на сетке оповещений.
...
1. Можно просмотреть, сколько раз было активировано оповещение, а также подтвержденные, открытые и закрытые оповещения:
...
1. Чтобы просмотреть состояние задания, вернитесь на страницу **Maintenance** (Обслуживание) и просмотрите список заданий в представлении **System Status** (Состояние системы). Вы можете увидеть, что задание запустилось, чтобы выпустить давление в клапане холодильника:

    ![Состояние заданий в представлении System Status (Состояние системы)](media/iot-suite-remote-monitoring-explore/maintenancerunningjob.png)
...
1. Чтобы просмотреть список задач, перейдите на страницу **Maintenance** (Обслуживание) и щелкните **System status** (Состояние системы).

1. Найдите событие, связанное с созданным заданием. Убедитесь, что процесс обновления встроенного ПО запущен правильно.

Можно создать фильтр для проверки правильности обновления версии встроенного ПО.

1. Чтобы создать фильтр, перейдите на страницу **Devices** (Устройства) и выберите **Manage** (Управление):
...
1. Выберите **Tag** (Пометить), а затем создайте новый тег строки под названием **FieldService** со значением **ConnectedVehicle**. Выберите имя для задания. Затем нажмите кнопку **Apply** (Применить):
...
1. Выберите **Tag** (Пометить), а затем создайте новый тег под названием **FieldService** со значением **SmartBuilding**. Выберите имя для задания. Нажмите кнопку **Save** (Сохранить):
