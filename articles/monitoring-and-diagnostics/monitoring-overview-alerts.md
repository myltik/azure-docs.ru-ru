...
* **Действие**. Определенный вызов, отправляемый получателю уведомления (например, отправка сообщения на электронный адрес или передача данных на URL-адрес веб-перехватчика). Обычно уведомления могут активировать несколько действий.
    ...
    > [!NOTE]
    > В ходе развития оповещений в Azure стала доступна новая унифицированная служба в предварительной версии. Новая служба "Оповещения" (предварительная версия) использует другую классификацию. Узнайте больше о службе [Оповещения (предварительная версия)](monitoring-overview-unified-alerts.md). 
    >
...
* Настройте [оповещения в службе "Оповещения" (предварительная версия)](monitor-alerts-unified-usage.md).
