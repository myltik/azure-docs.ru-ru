...
2. Щелкните **Мониторинг**, чтобы открыть страницу **Мониторинг**. На этой странице отображено единое представление всех параметров мониторинга и данных. Сначала открывается раздел **Журнал действий** .
...
12. Перейдите к разделу **Оповещения** страницы "Мониторинг".
...
13. Щелкните **Add metric alert** (Добавить оповещение метрики), чтобы создать оповещение.
    ...
    В Azure Monitor теперь также есть [**оповещения на основе метрик практически в реальном времени**](https://aka.ms/azuremonitor/near-real-time-alerts) (предварительная версия), которые могут измеряться поминутно.
