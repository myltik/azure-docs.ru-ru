...
| Настраиваемые полезные данные JSON |Настраиваемые полезные данные для отправки с помощью веб-перехватчика, если этот параметр выбран при создании оповещения. Подробные сведения см. в статье [Создание и просмотр оповещений, а также и управление ими с помощью Azure Monitor ("Оповещения (предварительная версия)")](monitor-alerts-unified-usage.md). |
