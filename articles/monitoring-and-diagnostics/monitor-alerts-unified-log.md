...
Из этой статьи вы узнаете, как в предварительной версии интерфейса оповещений Azure работают правила генерации оповещений в запросах Analytics. Также здесь описаны различия между типами правил генерации оповещений журнала.
...
> Запросы между рабочими областями или запросы между несколькими приложениями в сейчас не поддерживаются.

## <a name="log-alert-rules"></a>Правила генерации оповещений журнала

Оповещения, создаваемые в предварительной версии интерфейса оповещений Azure, автоматически выполняют регулярные запросы к журналу.  Если результаты запроса к журналу отвечают определенным условиям, создается запись оповещения. После этого правило может автоматически запустить одно или несколько действий, чтобы заранее уведомить вас об оповещении либо вызвать другой процесс, например модуль runbook, используя [группы действий](monitoring-action-groups.md).  В каждом типе правил генерации оповещений используется определенная логика выполнения этого анализа.
...
**Порог.** Порог для правила генерации оповещений **Число результатов** больше или меньше определенного значения.  Если число записей, возвращенных в результатах поиска по журналам, соответствует этому критерию, создается оповещение.
...
Для оповещения об одном событии задайте для числа результатов значение 0, а для частоты и временного окна — значение 5 минут.  При этом запросы выполняются каждые 5 минут. Они проверяют наличие одного события, созданного с момента последнего выполнения запроса.  Уменьшение частоты может привести к увеличению интервала между сбором данных о событии и созданием оповещения.
