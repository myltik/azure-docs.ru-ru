...
> [!NOTE]
> Начиная с 14 мая 2018 года корпорация Майкрософт начнет процесс автоматического расширения оповещений в Azure. Не все рабочие области и оповещения будут расширены. Вместо этого в предстоящие недели корпорация Майкрософт начнет поэтапно автоматически расширять оповещения. Следовательно, ваши оповещения на портале OMS не будут автоматически распространяться в Azure сразу 14 мая 2018 года, и пользователь может по-прежнему вручную расширять свои предупреждения, используя приведенные ниже параметры.
...
В процессе перемещения оповещений из OMS в Azure могут возникать проблемы, не позволяющие системе создать необходимые [группы действий](monitoring-action-groups.md). В таких случаях сообщение об ошибке будет отображаться на портале OMS в виде баннера в разделе оповещений и в вызовах GET к интерфейсу API.
...
3. **Ошибка. Политика присутствует на уровне группы подписки или группы ресурсов**: ![страница параметров оповещения портала OMS с сообщением об ошибке политики](./media/monitor-alerts-extend/ErrorPolicy.png)

    a. Когда применяется [политика Azure](../azure-policy/azure-policy-introduction.md), ограничивается любой новый ресурс в подписке или группе ресурсов, содержащей рабочую область Log Analytics (OMS). Система не может распространять (копировать) оповещения в Azure и создавать необходимые группы действий.
    
    Б. Для устранения отредактируйте политику, вызвав ошибку *[RequestDisallowedByPolicy](../azure-resource-manager/resource-manager-policy-requestdisallowedbypolicy-error.md)*, которая препятствует созданию новых ресурсов в вашей подписке или группе ресурсов, содержащих рабочую область. Используя портал Azure, Powershell, Azure CLI или API, вы можете проводить аудит действий, чтобы найти соответствующую политику, приводящую к ошибке. Чтобы узнать больше, просмотрите статью в [журналах действий для аудита](../azure-resource-manager/resource-group-audit.md). 
