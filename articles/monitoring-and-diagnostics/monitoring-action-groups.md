...
* **Тип действия**: Отправка SMS, отправить сообщение электронной почты, вызывать веб-перехватчика, отправлять данные в средство ITSM, вызова приложения Azure или запуска модуля runbook службы автоматизации.
...
7. Определите список действий, указав для каждого из них следующие данные:
    ...
    Б. **Тип действия**: выберите SMS, электронной почты, веб-перехватчика, приложение Azure, ITSM или Runbook службы автоматизации.

    c. **Сведения о**: в зависимости от типа действия, введите номер телефона, адрес электронной почты, веб-перехватчика URI, приложение Azure, ITSM соединения или runbook службы автоматизации. Для действия ITSM дополнительно укажите **рабочий элемент** и заполните другие поля, требуемые инструментом ITSM. 

   > [!NOTE]
   > Действие ITSM требует подключения ITSM. Дополнительные сведения о создании подключения ITSM см. в статье [Централизованное управление рабочими элементами ITSM с помощью соединителя управления ИТ-службами (предварительная версия)](../log-analytics/log-analytics-itsmc-overview.md). Сейчас действие ITSM работает только для оповещений журнала действий. Для других типов оповещений это действие не поддерживается.
