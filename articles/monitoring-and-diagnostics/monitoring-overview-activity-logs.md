...
> [!WARNING]
> Журнал действий Azure используется главным образом для действий, которые происходят в Azure Resource Manager. Ресурсы, использующие классическую модель или модель RDFE, не отслеживаются. Для некоторых классических типов ресурсов в Azure Resource Manager имеется прокси-поставщик ресурсов (например, Microsoft.ClassicCompute). Если вы взаимодействуете с ресурсом классического типа через Azure Resource Manager с помощью этих прокси-поставщиков ресурсов, то такие операции отображаются в журнале действий. Если взаимодействовать с типом ресурса классического за пределами прокси диспетчера ресурсов Azure, ваши действия только записываются в журнал операций. Журнал операций можно просмотреть в отдельном разделе портала.
>
>
...
На портале Azure журнал действий можно просмотреть в нескольких местах:
* В колонке **Журнал действий**, к которой можно перейти, выполнив поиска журнала действий в разделе "Больше служб" в области навигации слева.
* В колонке **Монитор**, которая по умолчанию отображается в области навигации слева. Журнал действий занимает один раздел этой колонки Azure Monitor.
* В колонке любого **ресурса**, например в колонке конфигурации виртуальной машины. Журнал действий занимает один из разделов в большинстве этих колонок ресурсов, и если щелкнуть его, события будут автоматически отфильтрованы в соответствии с данным ресурсом.
