<properties urlDisplayName="Store JavaScript project code in source control" pageTitle="Хранение кода проекта в системе управления версиями: мобильные службы Azure" metaKeywords="" description="Узнайте, как сохранить файлы серверных сценариев и модулей в локальном репозитории Git на компьютере." metaCanonical="" services="mobile-services" documentationCenter="Mobile" title="Store project code in source control" authors="glenga" solutions="" manager="dwrede" editor="" />

<tags ms.service="mobile-services" ms.workload="mobile" ms.tgt_pltfrm="mobile-multiple" ms.devlang="multiple" ms.topic="article" ms.date="11/21/2014" ms.author="glenga" />

<div class="dev-center-tutorial-subselector">
	<a href="/ru-ru/documentation/articles/mobile-services-dotnet-backend-store-code-source-control/" title=".NET backend">Серверная служба .NET</a> | <a href="/ru-ru/documentation/articles/mobile-services-store-scripts-source-control/"  title="JavaScript backend" class="current">Серверная служба JavaScript</a>
</div>
...
3. [Развертывание обновленных файлов сценариев в вашей мобильной службе].
4. [Использование общего кода и модулей Node.js в серверных сценариях].

Чтобы пройти этот учебник, необходимо создать мобильную службу, выполнив задания учебника [Приступая к работе с мобильными службами] или [Добавление мобильных служб в существующее приложение].
...
1. Установите Git на локальном компьютере. 
	...
	Действия, необходимые для установки Git, отличаются в разных операционных системах. Информацию о дистрибутивах для разных операционных систем и руководство по установке см. в разделе [Установка Git].
...
2. Откройте окно командной строки, например **GitBash** (Windows) или **Bash** (оболочка Unix). На компьютерах с OS X доступ к командной строке можно получить с помощью приложения **Terminal**.
...
7. Откройте вложенную папку .\service\table и обратите внимание на то, что она содержит файл TodoItem.json, который является представлением JSON разрешений операции в таблице TodoItem. 
	...
	Если для этой таблицы указаны серверные сценарии, также будет отображаться один или несколько файлов с именем <code>TodoItem._&lt;operation&gt;_.js</code> и сценариями для заданной операции с таблицей. Планировщик и скрипты настраиваемого интерфейса API сохраняются в отдельных папках с соответствующими именами. Дополнительную информацию см. в разделе [Управление версиями].
...
6. На портале управления перейдите на вкладку **Данные**, а затем щелкните таблицу **TodoItem**.
...
3. Найдите  
	...
	Диспетчер пакетов NPM создаст каталог "node_modules" в текущем расположении и установит модуль [node-uuid] в подкаталоге "\node-uuid". 

	<div class="dev-callout">
	<strong>Примечание.</strong>
	<p>Если <code>node_modules</code> уже существует в иерархии каталогов, NPM создаст подкаталог <code>\node-uuid</code> вместо создания нового <code>node_modules</code> в репозитории. В этом случае удалите существующий каталог <code>node_modules</code> .</p>
	</div>
...
+ [Вызов настраиваемого API из клиента] 
	<br/> В этой статье показано, как создавать настраиваемые интерфейсы API, которые можно вызвать из клиента.
...
[Развертывание обновленных файлов сценариев в вашей мобильной службе]: #deploy-scripts
[Использование общего кода и модулей Node.js в серверных сценариях]: #use-npm
...
[Добавление мобильных служб в существующее приложение]: /ru-ru/documentation/articles/mobile-services-ios-get-started-data/
[Работа с серверными сценариями в мобильных службах]: /ru-ru/documentation/articles/mobile-services-how-to-use-server-scripts/
[Портал управления Azure]: https://manage.windowsazure.com/
[Вызов настраиваемого API из клиента]: /ru-ru/documentation/articles/mobile-services-ios-call-custom-api/
