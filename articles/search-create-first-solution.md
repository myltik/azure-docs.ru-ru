...
5.	Перейдите к функции CreateCatalogIndex и обратите внимание, как создается индекс с использованием схемы, сопоставляющей столбцы с таблицы Products в SQL Server. Для каждого поля предусмотрено свойство Type (т. е. Edm.String или Edm.Double), а также атрибуты, определяющие назначение этих полей. Подробнее об этих атрибутах можно узнать в статье [Документация по интерфейсу API REST Поиска Azure](http://msdn.microsoft.com/library/azure/dn798935.aspx).
...
+	Ознакомьтесь с [интерфейсом API Lookup](http://msdn.microsoft.com/library/azure/dn798929.aspx), чтобы узнать, как создавать запросы Поиска Azure для извлечения определенного документа (например, можно передать значения идентификатора продукта).
...
Кроме того, можно просмотреть статью [Интерфейс API REST для Поиска Azure](http://msdn.microsoft.com/library/azure/dn798935.aspx) в библиотеке MSDN.
