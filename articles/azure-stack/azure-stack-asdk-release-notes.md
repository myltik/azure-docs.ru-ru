...
- Возможность [открыть новый запрос на поддержку из раскрывающегося списка](azure-stack-manage-portals.md#quick-access-to-help-and-support) на портале администрирования недоступна. Вместо этого перейдите по следующей ссылке:     
    - Чтобы узнать больше о пакете средств разработки Azure Stack, см. https://aka.ms/azurestackforum.    
...
- Azure Stack поддерживает только основный тип VHD. Некоторые образы, предлагаемые в Marketplace Azure Stack, используют динамические VHD, но они были удалены. Изменение размера виртуальной машины с помощью подключенного к ней динамического диска приводит к сбою работы виртуальной машины.
...
- При создании виртуальных машин на пользовательском портале Azure Stack на портале отображается неверное число дисков данных, которые можно вложить в виртуальную машину серии DS. В виртуальные машины серии DS можно вложить такое же количество дисков данных, как и в конфигурацию Azure.

- При сбое создания образа виртуальной машины элемент, который вы не смогли удалить, может быть добавлен в колонку вычислений образов виртуальной машины.

  В качестве обходного пути создайте новый образ виртуальной машины с заглушкой VHD. Его можно создать с помощью Hyper-V (New-VHD -Path C:\dummy.vhd -Fixed -SizeBytes 1 GB). Этот процесс должен устранить проблему, которая не дает удалить неисправный элемент. Затем через 15 минут после создания образа-заглушки вы сможете его удалить.

  После этого вы можете попытаться повторно загрузить образ виртуальной машины, который ранее загрузить не удалось.

-  Если предоставление расширения для развертывания виртуальной машины занимает слишком много времени, пользователи должны немного подождать, а не пытаться остановить процесс освобождения или удаления виртуальной машины.  
...
- После создания виртуальной машины со связанным общедоступным IP-адресом вы не можете отменить сопоставление виртуальной машины с этим IP-адресом. Будет выглядеть так, будто вам удалось его отменить, но ранее присвоенный общедоступный IP-адрес останется связанным с исходной виртуальной машиной.

  В настоящее время для создания виртуальной машины необходимо использовать только новые общедоступные IP-адреса.
...
-   Функция IP-пересылки видна на портале, однако включение IP-пересылки ни на что не влияет. Пока такая возможность не поддерживается.
...
- В виртуальной сети, созданной с параметром DNS-сервера *Автоматический*, изменение на пользовательский DNS-сервер завершается с ошибкой. Обновленные параметры не переносятся на виртуальные машины в этой виртуальной сети.
 
- Azure Stack не поддерживает добавление дополнительных сетевых интерфейсов к экземпляру виртуальной машины после развертывания виртуальной машины. Если ей необходимо более одного сетевого интерфейса, их нужно определить во время развертывания.

-   <!-- 2096388 --> You cannot use the admin portal to update rules for a network security group. 

    Обходной путь для службы приложений Azure. Если вам нужен удаленный рабочий стол для экземпляров Controller, измените правила безопасности в группах сетевой безопасности с помощью PowerShell.  Ниже приведены примеры того, как *разрешить*, а затем восстановить конфигурацию до состояния *запрета*: 
...
- Чтобы масштабировать инфраструктуру (рабочие роли, управление, внешние роли), используйте PowerShell, как описано в заметках о выпуске служб вычислений.
...
#### <a name="downloading-azure-stack-tools-from-github"></a>Загрузка средств Azure Stack из GitHub
- При использовании командлета PowerShell *invoke-webrequest* для загрузки средств Azure Stack из Github вы получите следующее сообщение об ошибке:     
  ...
  Эта ошибка возникает из-за недавней поддержки GitHub устаревших стандартов шифрования Tlsv1 и Tlsv1.1 (по умолчанию для PowerShell). Дополнительные сведения см. в статье [Weak cryptographic standards removal notice](https://githubengineering.com/crypto-removal-notice/) (Уведомление об удалении слабых стандартов шифрования).
