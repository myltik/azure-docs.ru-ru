...
Номер сборки обновления 1711 Azure Stack — **20171122.1**.
...
<br><br>
3. **Симптом.** Операторы Azure Stack могут увидеть следующую ошибку во время процесса обновления: *"Тип 'CheckHealth' роли 'VirtualMachines' вызвал исключение: \n\nПри проверке работоспособности виртуальной машины <machineName>-ACS01 появились следующие ошибки.\nПри получении сведений о виртуальной машине из узлов возникла ошибка. Сведения об исключении: \nGet-VM: Не удалось выполнить операцию на компьютере 'Node03': служба WS-Management не может обработать запрос. Служба \nWMI или поставщик WMI вернул неизвестную ошибку: HRESULT 0x8004106c".*
...
В этом разделе содержатся известные проблемы со сборкой **20171122.1** после установки.
