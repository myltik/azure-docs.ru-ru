...
Чтобы приступить к оценке Azure Stack, необходимо подготовить главный серверный компьютер пакета разработки, а затем [установить ASDK](asdk-deploy.md). После этого вы сможете войти на портал администратора и портал пользователя, чтобы начать использовать Azure Stack.