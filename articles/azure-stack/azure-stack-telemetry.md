...
Редактор реестра Windows используется, чтобы вручную установить уровень телеметрии на физическом компьютере узла перед развертыванием Azure Stack. Если политика управления уже существует (например, групповая политика), она перезапишет этот параметр реестра. 
...
Обратите внимание, что при отключении телеметрии Windows и Azure Stack также отключается телеметрия SQL. Дополнительные сведения о последствиях настроек телеметрии Windows Server см. в [техническом документе о телеметрии Windows](https://aka.ms/winservtelemetry). 
