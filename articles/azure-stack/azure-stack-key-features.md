...
Существует четыре основных поставщика ресурсов: сети, хранилища, вычисления и хранилище ключей. Каждый из этих поставщиков ресурсов поможет вам настроить соответствующие ресурсы и управлять ими. Кроме того, администраторы служб могут добавлять новые поставщики ресурсов.
...
Чтобы добиться высокого уровня доступности рабочих систем с несколькими виртуальными машинами в Azure, виртуальные машины размещаются в группе доступности, которая распространяет их по нескольким доменах сбоя и обновления. Таким образом [виртуальные машины, развернутые в группах доступности](https://docs.microsoft.com/azure/virtual-machines/windows/tutorial-availability-sets), физически изолированы друг от друга на отдельных серверных стойках, что позволяет обеспечить отказоустойчивость, как показано на следующей диаграмме:

  ![Высокий уровень доступности Azure Stack](media/azure-stack-key-features/high-availability.png)

### <a name="availablity-sets-in-azure-stack"></a>Группы доступности в Azure Stack
...
- **Домены сбоя**. Виртуальные машины, размещенные в группе доступности, физически изолированы друг от друга и как можно более равномерно распределяются по нескольким доменам сбоя (узлы Azure Stack). В случае сбоя оборудования виртуальные машины из поврежденного домена сбоя перезагружаются в других доменах сбоя, но, если это возможно, хранятся в доменах, отдельных от других виртуальных машин в той же группе доступности. После восстановления работоспособности оборудования виртуальные машины перераспределяются, что обеспечивает высокий уровень доступности. 
...
На виртуальных машинах в группах доступности, созданных до выпуска обновления 1802 Azure Stack, количество доменов сбоя и обновления установлено по умолчанию (1 и 1 соответственно). Чтобы достичь высокого уровня доступности виртуальных машин в этих группах доступности, необходимо сначала удалить имеющиеся виртуальные машины, а затем повторно развернуть их в новой группе доступности с правильным количеством доменов сбоя и обновления, как описано в статье [Изменение группы доступности для виртуальной машины Windows](https://docs.microsoft.com/azure/virtual-machines/windows/change-availability-set). 

Масштабируемые наборы виртуальных машин имеют внутреннюю группу с указанным по умолчанию количеством доменов сбоя и обновления (3 и 5 соответственно). Все масштабируемые наборы виртуальных машин, созданные до обновления 1802, будут помещены в группу доступности с количеством доменов сбоя и обновления по умолчанию (1 и 1 соответственно). Чтобы обновить эти экземпляры масштабируемых наборов виртуальных машин и повторно распределить, масштабируйте их по количеству экземпляров, которые присутствовали до обновления 1802, а затем удалите старые экземпляры. 
