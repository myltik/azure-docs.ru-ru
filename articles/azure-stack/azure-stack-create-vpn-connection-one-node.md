---
title: "Создание VPN-подключения типа \"сеть — сеть\" между двумя виртуальными сетями в разных средах комплекта разработки Azure Stack | Документация Майкрософт"
description: "Описание пошаговой процедуры, с помощью которой администратор облака может создать VPN-подключение типа \"сеть — сеть\" между двумя средами комплекта разработки Azure Stack с одним узлом."
...
---
# <a name="create-a-site-to-site-vpn-connection-between-two-virtual-networks-in-different-azure-stack-development-kit-environments"></a>Создание VPN-подключения типа "сеть — сеть" между двумя виртуальными сетями в разных средах комплекта разработки Azure Stack
## <a name="overview"></a>Обзор
В этой статье объясняется, как создать VPN-подключение типа "сеть — сеть" между двумя виртуальными сетями в двух отдельных средах комплекта разработки Azure Stack. Во время настройки подключений вы узнаете, как работают VPN-шлюзы Azure Stack.
...
* Два сервера, которые соответствуют требованиям к оборудованию комплекта разработки Azure Stack, определенным в [предварительных требованиях к развертыванию Azure Stack](azure-stack-deploy.md). Убедитесь, что также выполнены другие требования, приведенные в [этой статье](azure-stack-deploy.md).
* Пакет развертывания [комплекта разработки Azure Stack](https://azure.microsoft.com/en-us/overview/azure-stack/try/).

## <a name="deploy-the-azure-stack-development-kit-environments"></a>Развертывание сред комплекта разработки Azure Stack
Чтобы выполнить конфигурацию подключения, необходимо развернуть две среды комплекта разработки Azure Stack.
> [!NOTE] 
> Это можно сделать, следуя этим [инструкциям по развертыванию](azure-stack-run-powershell-script.md). В этой статье среды комплекта разработки Azure Stack называются *POC1* и *POC2*.
...
В таблице ниже перечислены параметры сети обеих сред комплекта разработки Azure Stack. Выполните процедуру, приведенную после таблицы, чтобы добавить адрес EBGP, соответствующий вашей сети.
...
> [!NOTE]
> В примере среды используются следующие IP-адреса EBGP: 10.16.167.195 (POC1) и 10.16.169.131 (POC2). Выполните процедуру ниже, чтобы определить IP-адреса EBGP узлов комплекта разработки Azure Stack, а затем добавьте их в приведенную выше таблицу конфигурации сети.
...
Проще говоря, ресурс шлюза локальной сети предназначен для указания удаленного шлюза на другом конце подключения. Архитектура комплекта разработки Azure Stack предусматривает ввод IP-адреса внешнего сетевого адаптера на виртуальной машине преобразования сетевых адресов (NAT) другого комплекта разработки Azure Stack в качестве общедоступного IP-адреса шлюза локальной сети. Затем на виртуальной машине NAT необходимо создать сопоставления NAT, чтобы убедиться в том, что оба конца подключены правильно.
...
Чтобы проверять данные, проходящие через VPN-подключение, потребуются виртуальные машины, которые отправляют данные в комплект разработки Azure Stack и получают их оттуда. Создайте виртуальную машину в POC1, а затем поместите ее в подсеть виртуальной машины в своей виртуальной сети.
...
## <a name="configure-the-nat-virtual-machine-on-each-azure-stack-development-kit-for-gateway-traversal"></a>Настройка виртуальной машины NAT в каждом комплекте разработки Azure Stack для обхода шлюза
Комплект разработки Azure Stack работает автономно и изолирован от сети, в которой развернут физический узел, поэтому *внешняя* сеть VIP, к которой подключены шлюзы, на самом деле не внешняя. Вместо этого сеть VIP скрыта за маршрутизатором, выполняющим преобразование сетевых адресов. 
...
> [!NOTE]
> Эта конфигурация требуется только в средах комплекта разработки Azure Stack.
> 
> 

### <a name="configure-the-nat"></a>Настройка NAT
> [!IMPORTANT]
> Эту процедуру следует выполнить в *обеих* средах.
...
3. Вставьте и измените приведенный ниже сценарий PowerShell. Чтобы настроить NAT в каждом комплекте разработки Azure Stack, выполните приведенный ниже сценарий в интегрированной среде сценариев Windows PowerShell с повышенными привилегиями. В этом сценарии измените значение заполнителей *External BGPNAT address* (Адрес EBGP) и *Internal IP address* (Внутренний IP-адрес):
...
Теперь, когда подключение типа "сеть —сеть" установлено, необходимо проверить, может ли трафик проходить через него. Чтобы проверить, войдите на одну из виртуальных машин, созданных в среде комплекта разработки Azure Stack. Затем проверьте связь виртуальной машины, созданной в другой среде. 
