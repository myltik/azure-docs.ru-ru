...
6. На следующей странице:
    1. Нажмите кнопку **Подключить** рядом с полем **Azure Stack Subscriptions** (Подписки Azure Stack).
        ...
        - Если вы используете службы федерации Active Directory (AD FS), предоставьте соответствующую учетную запись администратора. Например, azurestackadmin@azurestack.local. Введите пароль и нажмите кнопку **Войти**.
...
7. Введите данные для общего файлового ресурса и нажмите кнопку **Далее**.
...
11. Просмотрите параметры номера SKU и экземпляра роли. По умолчанию указываются минимальные рекомендуемые SKU экземпляров для каждой роли. Ниже приведена сводка требований к ядрам и памяти, чтобы помочь вам спланировать развертывание. Выбрав необходимые значения, нажмите кнопку **Далее**.
...
2. В общих сведениях в разделе **Состояние** должно отображаться **Все роли готовы**.
    ...
    ![Установщик службы приложений](media/azure-stack-app-service-deploy/image12.png)    
