...
В этой статье рассматриваются рекомендации по защите пользовательских виртуальных машин, развернутых в Azure Stack.

Чтобы избежать потери данных и незапланированных простоев, необходимо реализовать план резервного копирования и восстановления или план аварийного восстановления для пользовательских приложений и данных. Этот план уникален для каждого приложения, но должен соответствовать схеме, установленной стратегией обеспечения непрерывности бизнес-процессов и аварийного восстановления вашей организации (BC/DR). Сведения об общих шаблонах и рекомендациях по доступности и устойчивости приложений см. в статье [Проектирование устойчивых приложений для Azure](https://docs.microsoft.com/azure/architecture/resiliency) в Центре архитектуры Azure.

## <a name="azure-stack-infrastructure-recovery"></a>Восстановление инфраструктуры Azure Stack

Пользователям следует защищать свои виртуальные машины отдельно от служб инфраструктуры Azure Stack.

Если облако Azure Stack находится в автономном режиме в течение длительного времени или его невозможно восстановить, необходимо иметь план, чтобы приложение продолжало обслуживать запросы пользователей с минимальным временем простоя. Оператор облака Azure Stack отвечает за подготовку плана восстановления базовой инфраструктуры и служб Azure Stack. Дополнительные сведения см. в статье [Восстановление после катастрофической потери данных](https://docs.microsoft.com/azure/azure-stack/azure-stack-backup-recover-data). 
...
Необходимо определить, какую длительность простоя и объем потерянных данных организация может допустить для каждого приложения, чтобы выбрать наилучший подход к защите приложений в Azure Stack. Эти значения нужно определить, чтобы разработать и реализовать план восстановления, который минимизирует влияние на вашу организацию. Для каждого приложения рассмотрите следующее:  
...
RTO и RPO являются бизнес-требованиями. Проведите оценку риска, чтобы определить RTO и RPO для приложения. Другой распространенной метрикой является **среднее время восстановления**  (MTTR). Это среднее время, которое требуется для восстановления приложения после сбоя. MTTR является эмпирическим фактом о системе. Если MTTR превышает RTO, то сбой в системе приведет к недопустимому сбою бизнес-процессов, поскольку восстановить систему в пределах определенного RTO невозможно.
...
Планирование стратегии резервного копирования и определение требований к масштабу начинается с определения количества экземпляров виртуальной машины, которые необходимо защитить. Резервное копирование всех виртуальных машин на всех серверах в среде — это очень распространенная стратегия. Однако в Azure Stack есть некоторые виртуальные машины, не требующие резервного копирования. Например, виртуальные машины в масштабируемом наборе считаются временными ресурсами, которые появляются и исчезают, иногда без предупреждения. Любые надежные данные, которые необходимо защитить, хранятся в отдельном репозитории, таком как база данных или хранилище объектов. Ниже приведены важные рекомендации по резервному копированию виртуальных машин в Azure Stack.

 - **Классификация**
    - Рассмотрите модель, в которой пользователи явно соглашаются на резервное копирование виртуальной машины.
    - Определите соглашение об уровне обслуживания для восстановления на основе приоритетов приложений или влияния на бизнес.
 - **Масштабирование**
    ...
    - Оцените решения резервного копирования, которые эффективно хранят данные архивации, используя добавочные или разностные резервные копии, чтобы свести к минимуму необходимость извлекать полные резервные копии во всех виртуальных машинах среды.
 - **Restore**
...
Альтернативный подход к поддержанию высокой доступности заключается в репликации виртуальных машин приложения в другое облако и использовании запускаемой вручную отработке отказа. Репликацию операционной системы, двоичных файлов и данных приложений можно выполнить на уровне виртуальной машины или на уровне гостевой ОС. Управляйте отработкой отказа с помощью дополнительного программного обеспечения, отдельного от приложения.
 
В этом случае приложение развертывается только в одно облако. Затем виртуальная машина реплицируется в другое облако в одном из центров обработки данных или в общедоступное облако. Если активируется отработка отказа, во втором облаке потребуется включить дополнительные виртуальные машины. В некоторых случаях отработка отказа создает виртуальные машины и присоединяет их к дискам. Выполнение этого процесса может занять продолжительное время, особенно при использовании многоуровнего приложения с определенной последовательностью запуска. Чтобы приложение могло начать обрабатывать запросы, может потребоваться выполнить дополнительные шаги.
...
![Репликация и переход на другой ресурс вручную](media\azure-stack-manage-vm-backup\vm_backupdataflow_02.png)
 ...
 - RPO: разные показатели потери данных.
...
Физическая избыточность Azure Stack и доступность инфраструктуры обеспечивают защиту только от сбоев аппаратного уровня, таких как неисправный диск, неисправный источник питания, неисправный сетевой порт или неисправный узел. Однако, если ваше приложение должно быть доступно всегда и не может терять данные, в нем необходимо реализовать встроенную отказоустойчивость или обеспечить отказоустойчивость с помощью дополнительного программного обеспечения. 
 
Сначала нужно убедиться, что виртуальные машины с приложением развернуты с использованием масштабируемых наборов для защиты от сбоев на уровне узла. Чтобы обеспечить защиту от перехода облака в автономный режим, то же приложение должно быть развернуто в другом облаке, чтобы оно могло продолжать непрерывное обслуживание запросов. Обычно это называется развертыванием в режиме "активный — активный".
...
В этой статье рассматривались рекомендации по защите пользовательских виртуальных машин, развернутых в Azure Stack. Дополнительные сведения о защите виртуальных машин с помощью служб Azure:
 - [Документация по Azure Backup](https://docs.microsoft.com/en-us/azure/backup/ ) 
 - [Документация по Site Recovery](https://docs.microsoft.com/en-us/azure/site-recovery/)  
