...
## <a name="automatic-deployment-to-web-app-for-containers"></a>Автоматическое развертывание в веб-приложение для контейнеров
...
Выберите **Репозитории** > **acr-helloworld**, щелкните правой кнопкой мыши тег **v1** в разделе **Теги** и выберите **Развертывание в службу приложений**.

![Развертывание в службу приложений на портале Azure][deploy-app-portal-01]

В показанном разделе **Web app on Linux (preview)** (Веб-приложения в системе Linux (предварительная версия)) укажите следующие значения для каждого параметра:
...
Используйте процедуру, описанную в предыдущем разделе для развертывания второго веб-приложения в регионе *восточная часть США*. В разделе **Web app on Linux (preview)** (Веб-приложения в системе Linux (предварительная версия)), укажите следующие значения:
