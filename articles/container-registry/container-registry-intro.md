...
* [Анонс предварительной версии службы реестра контейнеров Azure](https://azure.microsoft.com/blog/azure-container-registry-preview/) 
...
* **Репозиторий.** Каждый реестр содержит по крайней мере один репозиторий, представляющий собой группу образов контейнеров. Реестр контейнеров Azure поддерживает многоуровневые пространства имен для репозитория. Благодаря этому можно группировать коллекции образов, связанные с определенным приложением, или коллекции приложений, связанные с определенным развертыванием или рабочими группами. Например:
  ...
  * `myregistry-contoso.azurecr.io/aspnetcore:1.0.1` представляет образ, используемый в организации;
  * `myregistry-contoso.azurecr.io/warrantydept/dotnet-build` представляет образ, используемый для создания приложений .NET, доступ к которым предоставляется отделу гарантий;
  * `myregistry-contoso.azrecr.io/warrantydept/customersubmissions/web` представляет веб-образ, сгруппированный в приложении constomersubmissions, которое принадлежит отделу гарантий.
...
* **Службы Azure**, поддерживающие создание и выполнение масштабированных приложений, в том числе [служба контейнеров](../container-service/index.md), [служба приложений](/app-service/index.md), [пакетная служба](../batch/index.md) и [Service Fabric](../service-fabric/index.md). 
