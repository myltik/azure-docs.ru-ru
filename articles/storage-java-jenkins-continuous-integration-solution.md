<properties urlDisplayName="Jenkins Continuous Integration" pageTitle="Использование хранилища Azure с решением непрерывной интеграции Jenkins - Microsoft Azure" metaKeywords="" description="В этом уроке вы научитесь использовать службы BLOB-объектов Azure в качестве репозитория для артефактов построения, созданных решением непрерывной интеграции Jenkins." metaCanonical="" services="storage" documentationCenter="Java" title="Using Azure Storage with a Jenkins Continuous Integration solution" authors="robmcm" solutions="" manager="wpickett" editor="mollybos" scriptId="" videoId="" />
...
-   [Преимущества использования службы больших двоичных объектов][]
-   [Предварительные условия][]
-   [Использование службы больших двоичных объектов с решением непрерывной интеграции Jenkins][]
-   [Установка подключаемого модуля службы хранилища Azure][]
...
-   [Создание шага сборки для скачивания больших двоичных объектов из хранилища Azure][]
-   [Компоненты, используемые службой больших двоичных объектов][]

## <a name="overview"></a>Общая информация о Jenkins
...
Дополнительную информацию о Jenkins см. в разделе [Знакомство с Jenkins][].

## <a name="benefits"></a>Преимущества использования службы больших двоичных объектов
...
- Решение непрерывной интеграции Jenkins.
    ...
    3. Откройте в браузере сайт http://localhost:8080/. Открывается панель мониторинга Jenkins, которую вы будете использовать для установки и настройки подключаемого модуля хранилища Azure.
...
## <a name="howtouse"></a>Как использовать службу больших двоичных объектов с решением непрерывной интеграции Jenkins
...
## <a name="howtoinstall"></a>Как установить подключаемый модуль службы хранилища Azure

1. На панели мониторинга Jenkins щелкните элемент **Управление Jenkins**.
2. На странице **Управление Jenkins** щелкните **Управление подключаемыми модулями**.
3. Откройте вкладку **Доступно**.
4. В разделе **Средства отправки артефактов** установите флажок **Подключаемый модуль службы хранилища Microsoft Azure**.
5. Щелкните **Установить без перезагрузки** или **Загрузить сейчас и установить позднее**.
6. Перезапустите Jenkins.

## <a name="howtoconfigure"></a>Как настроить подключаемый модуль службы хранилища Azure для использования своей учетной записи хранения

1. На панели мониторинга Jenkins щелкните элемент **Управление Jenkins**.
2. На странице **Управление Jenkins** щелкните **Настройка системы**.
3. В разделе **Настройка учетной записи хранения Microsoft Azure**:
    ...
    5. [Необязательно] Если у вас есть дополнительные учетные записи хранения, которые требуется сделать доступными для решения Jenkins, щелкните **Добавить дополнительные учетные записи хранения**.
    6. Нажмите кнопку **Сохранить**, чтобы сохранить изменения.

## <a name="howtocreatepostbuild"></a>Как создать действие, выполняемое после сборки и отправляющее артефакты сборки в вашу учетную запись хранения
...
2. Назовите задание **MyJob**, щелкните **Build a free-style software project** (Сборка программного проекта в свободном стиле) и нажмите кнопку **OK**..
...
5. В разделе конфигурации задания **Post-build Actions** (Действия после сборки) щелкните **Add post-build action** (Добавить действие после сборки) и выберите **Upload artifacts to Azure Blob storage** (Передать артефакты в хранилище больших двоичных объектов Azure).
6. В поле **Имя учетной записи хранения** выберите используемую учетную запись хранения.
...
9. [Необязательно] Щелкните **Очистить контейнер перед отправкой**, если хотите, чтобы контейнер был перед отправкой артефактов сборки (не устанавливайте этот флажок, если не хотите очищать содержимое контейнера).
10. В поле **Список отправляемых артефактов** введите значение **text/*.txt**.
11. Введите в поле **Общий виртуальный путь для загруженных артефактов** значение **${BUILD\_ID}/${BUILD\_NUMBER}**.
...
14. После успешного завершения задания можно просмотреть артефакты построения, открыв общедоступный BLOB-объект.
    1. Войдите на портал управления Azure по адресу https://manage.windowsazure.com.
    ...
    5. Выберите контейнер с именем **myjob**, который является копией имени задания Jenkins и записан в нижнем регистре. Имена контейнеров и BLOB-объектов в хранилище Azure отображаются строчными буквами (и с учетом регистра). В списке больших двоичных объектов для контейнера с именем **myjob** вы увидите **hello.txt** и **date.txt**. Скопируйте URL-адрес любого из этих элементов и откройте его в браузере. Вы увидите текстовый файл, который был загружен в качестве артефакта построения.

На каждое задание может быть создано только одно действие после построения, которое отправляет артефакты в хранилище blob-объектов Azure. Обратите внимание, что одно действие после сборки по отправке артефактов в хранилище больших двоичных объектов Azure может определять различные файлы (в том числе подстановочные знаки) и пути для файлов в **списке отправляемых артефактов** с использованием точки с запятой в качестве разделителя. Например, если операция сборки Jenkins создает JAR-файлы и TXT-файлы в папке **build** рабочей области, при этом их нужно отправить хранилище больших двоичных объектов Azure, укажите в поле **Список отправляемых артефактов** следующее значение: **build/\*.jar;build/\*.txt**. Чтобы задать путь для использования в имени blob-объекта, можно также использовать синтаксис с двойным двоеточием. Например, если необходимо отправить JAR-файлы с использованием **двоичных файлов** в пути большого двоичного объекта, а файлы TXT - с использованием **уведомлений** в пути большого двоичного объекта, используйте для параметра **Список отправляемых артефактов** следующее значение: **build/\*.jar::binaries;build/\*.txt::notices**.

## <a name="howtocreatebuildstep"></a>Создание шага сборки для скачивания больших двоичных объектов из хранилища Azure
...
2. В поле **Имя учетной записи хранения** выберите используемую учетную запись хранения.
3. Для поля **Имя контейнера** задайте имя контейнера с большими двоичными объектами, которое нужно скачать. Можно использовать переменные среды.
...
5. [Необязательно] В поле **Папка для загрузки** задайте путь к компьютеру с Jenkins, на который нужно скачать файлы из хранилища больших двоичных объектов Azure. Переменные среды также могут использоваться. (Если не задать значения для поля **Путь для загрузки**, файлы из хранилища больших двоичных объектов Azure будут скачиваться в рабочую область задания.)
...
## <a name="components"></a>Компоненты, используемые службой больших двоичных объектов
