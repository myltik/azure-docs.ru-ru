...
службы хранилища BLOB-объектов Azure. Эти примеры написаны с помощью
Python API. Здесь описаны такие сценарии, как **отправка**, **перечисление**,
**скачивание** и **удаление** BLOB-объектов. Дополнительные сведения о BLOB-объектах
...
[Что такое хранилище BLOB-объектов?][Что такое хранилище BLOB-объектов?]   
 ...
 [Практическое руководство. Скачивание BLOB-объектов][Практическое руководство. Скачивание BLOB-объектов]   
 [Практическое руководство. Удаление BLOB-объекта][Практическое руководство. Удаление BLOB-объекта]   
 [Практическое руководство. Отправка и скачивание больших BLOB-объектов][Практическое руководство. Отправка и скачивание больших BLOB-объектов]   
...
код создает объект **BlobService**. Добавьте следующий код
в начало любого файла Python, из которого планируется получать доступ к хранилищу Azure программным способом.

	from azure.storage import BlobService

Следующий код создает объект **BlobService**, используя имя и ключ доступа учетной записи хранения.  Замените "myaccount" и "mykey" на фактические значения учетной записи и ключа.
...
После этого изменения любой пользователь Интернета сможет видеть BLOB-объекты в общем
контейнере, но право на их изменение или удаление будет только у вас.
...
В следующем примере содержимое файла **task1.txt** передается в BLOB-объект **myblob**.
...
## <a name="download-blobs"> </a>Практическое руководство. Скачивание BLOB-объектов

Чтобы скачать данные из BLOB-объекта, используйте **get\_blob\_to\_path**, **get\_blob\_to\_file**, **get\_blob\_to\_bytes** или **get\_blob\_to\_text**. Это высокоуровневые методы, которые выполняют необходимое фрагментирование данных, если их размер превышает 64 МБ.

В следующем пример показано использование **get\_blob\_to\_path** для скачивания содержимого BLOB-объекта **myblob** и его сохранения в файле **out-task1.txt**.
...
Вы изучили основные сведения о хранилище BLOB-объектов. Дополнительные сведения
...
-   Посетите блог [команды разработчиков хранилища Azure][команды разработчиков хранилища Azure]
  ...
  [Практическое руководство. Скачивание BLOB-объектов]: #download-blobs
  [Практическое руководство. Удаление BLOB-объекта]: #delete-blobs
  [Практическое руководство. Отправка и скачивание больших BLOB-объектов]: #large-blobs
