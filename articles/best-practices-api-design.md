...
| /customers/1/orders | Создание нового заказа для клиента 1 | Получение всех заказов для клиента 1 | Массовое обновление заказов для клиента 1 (_если реализовано_) | Массовое обновление всех заказов для клиента 1 (_если реализовано_) |
...
Возможное решение — предоставить схему URI, такую как _/orders/ordervalue_greater_than_n_, где _n_ — стоимость заказа. Но такой подход непрактичен для большинства уровней стоимости, за исключением лишь некоторых. Кроме того, если вам требуется отправлять запросы по заказам на основе других критериев, в конце концов вам, вероятно, придется предоставлять длинный список универсальных кодов ресурсов (URI) со сложными именами.
