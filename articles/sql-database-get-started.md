<properties 
	pageTitle="Приступая к работе с базами данных SQL - Azure" 
	description="Начните создавать базы данных SQL и управлять ими в Azure." 
...
Чтобы начать работу с бесплатной учетной записью, щелкните "Бесплатная пробная версия" в верхнем правом углу и следуйте дальнейшим указаниям.
...
**Примечание.** Схема и значения данных взяты из этой [статьи MSDN](http://msdn.microsoft.com/library/windowsazure/ee621790.aspx "MSDN article") и изменены для работы с базой данных SQL.
...
На этом шаге с помощью следующего сценария будет создана схема. Сценарий сначала проверяет, нет ли уже таблицы с тем же именем, чтобы убедиться в отсутствии конфликта имен, и создает таблицу с помощью оператора [CREATE TABLE](http://msdn.microsoft.com/library/windowsazure/ee336258.aspx). Далее этот сценарий использует оператор [ALTER TABLE](http://msdn.microsoft.com/library/windowsazure/ee336286.aspx), чтобы определить первичный ключ и связи таблицы.
...
Открыть новое окно запроса и вставьте в него следующий скрипт. Выполните этот скрипт, чтобы вставить данные. Этот сценарий использует оператор [INSERT](http://msdn.microsoft.com/library/windowsazure/ee336284.aspx), чтобы добавить значения в каждый столбец.
...
Чтобы активно управлять новой базой данных, рассмотрите возможность установки и использования среды SQL Server Management Studio. Management Studio - это главное средство администрирования для управления базами данных SQL Server, включая выполняемые в Azure. В среде Management Studio можно сохранять запросы для дальнейшего использования, добавлять новые таблицы и хранимые процедуры, а также отточить свое мастерство Transact-SQL в среде использования скриптов с широкими возможностями, включая проверку синтаксиса, Intellisense и шаблоны. Чтобы начать работу, следуйте указаниям раздела [Управление базами данных SQL с помощью среды SQL Server Management Studio](http://www.azure.microsoft.com/documentation/articles/sql-database-manage-azure-ssms/).

Хорошее владение запросами Transact-SQL и языком определения данных крайне важно для администраторов баз данных. Если вы новичок в языке Transact-SQL, рекомендуем для начала использовать [Учебник. Написание инструкций Transact-SQL](http://msdn.microsoft.com/library/ms365303.aspx), чтобы познакомиться с некоторыми основными навыками.
...
* [Перенос баз данных в базу данных SQL](http://msdn.microsoft.com/library/windowsazure/ee730904.aspx)
* [Копирование баз данных в базу данных SQL](http://msdn.microsoft.com/library/windowsazure/ff951624.aspx)
* [Развертывание базы данных SQL Server на виртуальной машине Azure](http://msdn.microsoft.com/library/dn195938)
