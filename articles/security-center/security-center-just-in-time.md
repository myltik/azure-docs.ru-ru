...
Когда пользователь запрашивает доступ к виртуальной машине, центр безопасности проверяет, что у пользователя есть разрешения [управления доступом на основе ролей (RBAC)](../active-directory/role-based-access-control-configure.md), которые предоставляют доступ на запись для ресурса Azure. Если у пользователя есть разрешение на запись, доступ разрешается, и центр безопасности автоматически настраивает группу безопасности сети, которая разрешает входящий трафик к портам управления на указанный период времени. После истечения этого времени центр безопасности восстанавливает прежнее состояние групп безопасности сети.
...
В элементе **JIT-доступ к виртуальной машине** в колонке **Центр безопасности** отображается количество виртуальных машин, для которых настроен JIT-доступ, и количество одобренных запросов за последнюю неделю.

Выберите элемент **JIT-доступ к виртуальной машине**, и откроется колонка **JIT-доступ к виртуальной машине**.
...
3. В колонке **Настройка JIT-доступа к виртуальной машине** можно изменить существующие параметры уже защищенного порта, щелкнув по этому порту, или добавить новый порт, щелкнув **Добавить**. Откроется колонка **Добавление конфигурации порта**.
