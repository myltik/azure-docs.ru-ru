<properties
   pageTitle="Шифрование виртуальной машины Azure | Microsoft Azure"
...
Этот документ поможет вам зашифровать виртуальные машины, даже если у вас недостаточно или совсем нет опыта работы с Azure PowerShell. В этом документе предполагается, что в качестве клиентского компьютера, на котором будет настраиваться шифрование дисков Azure, используется компьютер под управлением ОС Windows 10.
...
6.	Сохраните файл Блокнота: щелкните **Файл**, а затем — **Сохранить как**. В поле "Имя файла" введите **"ADEPrereqScript.ps1"** и нажмите кнопку **Сохранить** (имя нужно заключить в кавычки, иначе файл будет сохранен с расширением TXT). 
...
3.	В окне **Администратор: интегрированная среда сценариев Windows PowerShell** щелкните **Представление**, а затем — **Показать область сценариев**. 
...
4.	Войдите в свою учетную запись Azure. В окне консоли введите **Login-AzureRmAccount** и нажмите клавишу **ВВОД**. Появится диалоговое окно, в котором можно ввести учетные данные. (Убедитесь, что у вас есть права на изменение виртуальных машин, иначе вы не сможете зашифровать виртуальные машины. Если вы не уверены, уточните эти сведения у владельца подписки или администратора.) Должны отобразиться следующие сведения: **Environment**, **Account**, **TenantId**, **SubscriptionId** и **CurrentStorageAccount**. Скопируйте значение **SubscriptionId** в Блокнот. Оно понадобится вам на этапе 6.
...
9.	Скрипт запросит значение **keyVaultName:** введите имя *хранилища ключей*, которое вы хотите использовать, а затем нажмите клавишу ВВОД. Если у вас нет такого хранилища ключей, введите имя, которое будет использовано для создания нового хранилища. Если у вас уже есть *хранилище ключей*, которое следует использовать, введите его имя.
...
11.	Сценарий запросит значение **aadAppName:** введите имя приложения *Azure Active Directory*, которое вы хотите использовать, а затем нажмите клавишу **ВВОД**. Если у вас нет такого приложения, введите имя, которое будет использовано для создания нового приложения. Если *приложение Azure Active Directory*, которое вы хотите использовать, уже существует, введите его имя.
...
**$resourceGroupName = <’группа ресурсов виртуальной машины’>**

Замените **< Virtual_Machine_RG >** именем группы ресурсов, в которой находятся ваши виртуальные машины, взяв его в одинарные кавычки. Нажмите клавишу **ВВОД**. Чтобы убедиться, что указано правильное имя группы ресурсов, введите следующую команду в консоли PowerShell ISE:
...
Замените **<’имя вашей виртуальной машины’>** именем виртуальной машины (убедитесь, что имя заключено в одинарные кавычки) и нажмите клавишу **ВВОД**.
