...
Агент мониторинга безопасности Azure имеет следующие базовые показатели использования ресурсов:

* Агент в основном потребляет 3 МБ памяти, но во время пиковых нагрузок этот показатель достигает 10 МБ (если сценарий запускается каждые 12 часов).  
* Незначительное использование ресурсов ЦП при выполнении постоянных процессов и операций сканирования. 
* Незначительное использование диска.

Агент мониторинга безопасности имеет цепочку процессов, которые вместе потребляют всего около 30 МБ памяти.  Каждый экземпляр агента мониторинга может использовать до 3 ГБ дискового пространства. Каждый экземпляр не может использовать больше 20 % ресурсов ЦП (на практике используется намного меньше). 
...
> [!NOTE]
> Сведения по устранению неполадок, связанных с агентом мониторинга центра безопасности Azure, см. в [этом руководстве](security-center-troubleshooting-guide.md).
> 
> 
...
Оплата использования этого хранилища не входит в цену центра безопасности и взимается отдельно по обычным [тарифам для службы хранилища Azure](https://azure.microsoft.com/pricing/details/storage/). С точки зрения планирования следует учесть, что центр безопасности добавляет данные в хранилище Azure, и это может повысить стоимость на 1–3 доллара в год.
...
Чтобы войти в центр безопасности через портал Azure, щелкните **Обзор** и в поле **Фильтр** введите **Центр безопасности**. Информация будет отображаться в соответствии с примененными фильтрами.
...
При первом включении сбора данных в центре безопасности для текущей среды Azure обязательно просмотрите все рекомендации. Для этого щелкните колонку **Рекомендации** или просмотрите их для каждого ресурса (выберите **Виртуальная машина**, **Сети**, **SQL** и **Приложение**).
