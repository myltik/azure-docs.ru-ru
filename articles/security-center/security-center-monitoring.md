...
Если щелкнуть **Данные** на плитке **Resources security health** (Работоспособности безопасности ресурсов), откроется колонка **Data Resources** (Ресурсы данных) с рекомендациями по SQL и хранилищу. Она также содержит [рекомендации](security-center-sql-service-recommendations.md), касающиеся общего состояния работоспособности базы данных. Дополнительные сведения о шифровании хранилищ см. в статье Enable encryption for Azure storage account in Azure Security Center (Включение шифрования для учетной записи хранения Azure в центре безопасности Azure).
