...
На плитке **Resources security health** (Работоспособность системы безопасности ресурсов) можно отследить состояние защиты ресурсов. Ниже показан пример, когда выявлено несколько проблем с высоким и средним уровнем серьезности, на которые следует обратить внимание. Включенные политики безопасности будут влиять на типы элементов управления, которые отслеживаются.
...
Если на плитке **Resources security health** (Работоспособность системы безопасности ресурсов) щелкнуть **Виртуальные машины**, откроется колонка **Виртуальные машины**. На ней представлены подробные сведения о подключении и защите, а также перечислены все виртуальные машины, отслеживаемые центром безопасности:
...
####Процесс внедрения
В этом разделе указывается общее количество виртуальных машин, на которых инициализирован сбор данных, а также их текущее состояние. После инициализации сбора данных на всех виртуальных машинах можно настроить политики безопасности Центра безопасности. Щелкнув эту запись, вы откроете колонку **Initializing data collection** (Инициализация сбора данных), в которой перечислены имена виртуальных машин. В столбце **Состояние инициализации** отображено текущее состояние сбора данных:
...
Чтобы просмотреть более подробные указания, касающиеся рекомендации, щелкните **Update OS version** (Обновить версию ОС) в столбце **Описание**. Откроется колонка **Update OS version (Preview)** (Обновление версии ОС (предварительная версия)) с более подробными сведениями.
...
Если на плитке **Resources security health** (Работоспособность системы безопасности ресурсов) щелкнуть **Сеть**, откроется колонка **Сеть** с дополнительными сведениями, как показано ниже.
...
В колонке **Выбор группы безопасности сети** выберите самую подходящую группу безопасности сети для подсети или создайте новую.
...
В разделе **Networking topology** (Топология сети) отображается иерархическое представление ресурсов, как показано ниже.
...
Если вы щелкнете базу данных, чтобы выполнить рекомендацию, откроется колонка **Аудит и обнаружение и угроз**, как показано ниже.

![Работоспособность ресурса SQL](./media/security-center-monitoring/security-center-monitoring-fig17-new.png)

Чтобы включить аудит, просто переведите переключатель для параметра **Аудит** в положение **Вкл.** и нажмите кнопку **Сохранить**.
