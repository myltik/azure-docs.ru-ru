...
* **Файл журнала был очищен**: это предупреждение означает, что файл журнала истории команд был удален, что мог сделать злоумышленник для скрытия следов.
...
* **Подозрительное действие входа**: это предупреждение свидетельствует о необычном действии входа, которое может свидетельствовать об атаке методом подбора, направленную на блок сообщений сервера (SMB). Если затронутый ресурс действует как сервер IIS, это предупреждение может быть связано с определенной конфигурацией аутентификации IIS, которая является допустимой.
...
* **Брандмауэр Windows был отключен**: это предупреждение означает, что брандмауэр Windows отключен.
...
* **Новый пользователь был добавлен в группу администраторов**: это предупреждение означает, что новый пользователь был добавлен в группу локальных администраторов.
* **Создана новая служба**: это предупреждение означает, что была создана новая служба.
...
* **Подготовка к извлечению документов через черный вход IIS**: это предупреждение означает, что документы собираются и готовятся к извлечению.
...
* **Загружен модуль ядра**: это предупреждение означает, что был загружен модуль ядра.
* **Удален модуль ядра**: это предупреждение означает, что был удален модуль ядра.
* **Аномальные попытки входа на компьютер**: это предупреждение означает, что пользователь вошел в систему с необычного IP-адреса.
* **Файл загружен и выполнен**: это предупреждение означает, что файл был загружен на компьютер, получил права выполнения и был выполнен.
...
* **Незаконное изменение настроек SENSE**: это предупреждение указывает на попытку отключить SENSE, службу мониторинга безопасности ATP защитника Windows.
...
* **Возможный подозрительный доступ к задачам планировщика**: это предупреждение означает, что была выполнена задача cron, которую злоумышленники могут использовать для выполнения вредоносных программ по расписанию.
* **Возможный подозрительный доступ к файлу истории команд**: это предупреждение указывает на аномальный доступ к файлу истории команд.
* **Создание учетной записи**: это предупреждение свидетельствует о создании новой учетной записи на компьютере.
* **Изменение параметра bash**: это предупреждение указывает на доступ к файлу профиля Bash. Это может свидетельствовать о том, что злоумышленник пытается выполнять вредоносные программы по расписанию.
* **Подозрительная последовательность неудачных попыток выполнения sudo**: это предупреждение указывает на последовательность неуспешных команд sudo, которые часто наблюдаются при атаках методом подбора, направленных на эскалацию привилегий со стороны неавторизованных пользователей.
* **Подозрительные удачные попытки выполнения sudo**: это предупреждение указывает на последовательность неуспешных попыток выполнения sudo, за которым последовала успешная, что часто наблюдаются при атаках методом подбора, направленных на эскалацию привилегий со стороны неавторизованных пользователей. 
* **Новый пользователь был добавлен в группу sudoers**: это предупреждение означает, что пользователь был добавлен в группу sudoers, участники которой могут запускать команды с высокими привилегиями.
* **Вход в сеть с учетными данными в виде обычного текста**: это предупреждение означает, что был зарегистрирован сетевой вход, при котором пароль был отправлен по сети в обычном текстовом формате. Это обычное явление для входа в систему с помощью скрипта ASP с использованием ADVAPI или при входе пользователя в IIS с использованием режима обычной проверки подлинности IIS. Обычная проверка подлинности не является рекомендуемым методом, если она не выполняется на уровне шифрования, например SSL (т. е. только с использованием соединений HTTPS).
...
* **Уязвимость к атакам путем внедрения кода SQL**: это предупреждение активируется при обнаружении ошибки приложения в базе данных. Оно может указывать на возможную уязвимость к атакам путем внедрения кода SQL.
* **Необычный вход из неизвестного расположения**: это предупреждение активируется, когда на сервере обнаруживается попытка доступа с незнакомого IP-адреса, который не использовался за последний период.
