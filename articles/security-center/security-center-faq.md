...
Уровень "Стандартный" предоставляет все возможности уровня "Бесплатный", а также расширенные возможности обнаружения: аналитику угроз, анализ поведения, анализа сбоев и обнаружение аномалий. Доступна бесплатная 90-дневная версия для уровня "Стандартный". Чтобы выполнить обновление, выберите ценовую категорию в [политике безопасности](security-center-policies.md#set-security-policies-for-subscriptions). Дополнительные сведения см. на странице с [ценами на использование центра безопасности Azure](security-center-pricing.md).

## <a name="data-collection"></a>Сбор данных
Центр безопасности собирает с виртуальных машин данные, оценивает состояние их безопасности на основе этих данных, предоставляет рекомендации по обеспечению безопасности и предупреждает об угрозах. Когда вы впервые обращаетесь к Центру безопасности, сбор данных включается на всех виртуальных машинах в рамках подписки. Сбор данных является рекомендуемым параметром, но вы можете [отключить его](#how-do-i-disable-data-collection) в политике центра безопасности.

### <a name="how-do-i-disable-data-collection"></a>Как отключить сбор данных?
Отключить **сбор данных** для подписки можно в политике безопасности в любой момент. ([Войдите на портал Azure](https://portal.azure.com), выберите **Обзор**, а затем — **Центр безопасности** и **Политика**.)  При выборе подписки открывается новая колонка, в которой вы можете отключить **сбор данных**. В верхней ленте выберите параметр **Удалить агенты**, чтобы удалить агенты с существующих виртуальных машин.
...
### <a name="how-are-permissions-handled-in-azure-security-center"></a>Как обрабатываются разрешения в Центре безопасности Azure?
Центр безопасности Azure поддерживает доступ на основе ролей. Дополнительные сведения об управлении доступом на основе ролей (RBAC) в Azure см. в статье [Использование назначений ролей для управления доступом к ресурсам в подписке Azure](../active-directory/role-based-access-control-configure.md).

Когда пользователь открывает центр безопасности, он видит только те рекомендации и оповещения, которые относятся к доступным ему ресурсам. Это означает, что пользователь будет видеть элементы, относящиеся только к тем ресурсам, где ему назначена роль владельца, участника или читателя подписки или группы ресурсов, к которым относится ресурс.

Если вам нужно:

* **изменить политику безопасности**, то вы должны быть владельцем или участником подписки;
* **применить рекомендацию**, то вы должны быть владельцем или участником подписки.
* **видеть состояние безопасности во всех подписках**, то вы должны быть владельцем, участником или читателем (администратором ИТ в группе безопасности) для каждой подписки;
* **видеть состояние безопасности своих ресурсов**, то вы должны быть владельцем, участником или читателем (DevOps) группы ресурсов.
...
### <a name="what-types-of-virtual-machines-will-be-supported"></a>Какие типы виртуальных машин будут поддерживаться?
...
После включения сбора данных в центре безопасности Azure может потребоваться некоторое время (обычно меньше часа), чтобы получить данные сканирования. Сканирование виртуальных машин в остановленном состоянии не позволяет получать данные.
