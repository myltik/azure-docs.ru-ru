...
- Настройка [групп безопасности сети](virtual-networks-nsg.md) и правил управления трафиком для виртуальных машин
...
Центр обеспечения безопасности Azure поддерживает доступ на основе ролей. Дополнительные сведения об управлении доступом на основе ролей (RBAC) в Azure см. в статье [Контроль доступа на основе ролей в Azure Active Directory](role-based-access-control-configure.md).
...
В разделе "Контроль доступа" отображаются рекомендации, которые относятся к классическим виртуальным машинам. Группы сетевой безопасности и рекомендации по установке брандмауэра веб-приложения в настоящее время применяются только к виртуальным машинам диспетчеров ресурсов.

### Поддерживаются ли виртуальные машины Linux?
Центр безопасности Azure предлагает базовые функции мониторинга для виртуальных машин Linux (только Ubuntu). Впоследствии будут добавлены функции мониторинга работоспособности и сбора или анализа данных, а также поддержка других дистрибутивов Linux.
