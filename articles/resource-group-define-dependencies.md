...
В шаблоне элемент dependsOn позволяет определить один ресурс как зависимый от одного или нескольких ресурсов. В качестве значения свойства может выступать разделенный запятыми список имен ресурсов.

В следующем примере показан масштабируемый набор виртуальных машин, который зависит от балансировщика нагрузки, виртуальная сеть и цикл, который создает несколько учетных записей хранения. Остальные ресурсы не показаны ниже, но они должны существовать в другом месте в шаблоне.
...
Если необходимо определить зависимость между ресурсом и ресурсами, которые создаются с помощью цикла копирования (как указано выше), можно указать в качестве значения элемента dependsOn имя цикла. Пример см. в статье [Создание нескольких экземпляров ресурсов в диспетчере ресурсов Azure](resource-group-create-multiple.md).
