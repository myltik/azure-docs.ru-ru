...
4. В раскрывающемся меню последовательно выберите пункты **Продукты и услуги** и **Диспетчер API**, а затем в разделе **API социальных сетей** щелкните **Google+ API** > **Включить API**.

5. Щелкните **Учетные данные** > **Экран согласия OAuth**, а затем выберите **Адрес электронной почты**, введите **имя продукта** и нажмите кнопку **Сохранить**.
...
7. Введите URL-адрес мобильной службы в поле **Авторизованные источники JavaScript**, замените созданный URL-адрес в поле **Авторизованный универсальный код ресурса (URI) перенаправления** на URL-адрес в одном из следующих форматов, а затем щелкните **Создать**:
