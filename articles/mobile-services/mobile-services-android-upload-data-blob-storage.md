...
Прежде чем приступить к этому руководству, необходимо прочитать статью о начале работы с мобильными службами: [Начало работы с мобильными службами].
...
- Новый скрипт SQL **insert** мобильных служб запрашивает у службы хранилища Azure подписанный URL-адрес (SAS).
...
2. В файл **activity\_to\_do.xml**, находящийся в папке **res => layout**, добавьте следующий код перед уже имеющимся для кнопки **Добавить**:
...
3. Щелкните **Take Photo** (Сделать снимок). После запуска приложения камеры сделайте снимок. Щелкните флажок, чтобы принять фотографию.

4. Нажмите **Upload** (Передать). Обратите внимание, что элемент ToDoItem будет добавлен в список, как обычно.
...
6. Отобразится список переданных файлов BLOB-объектов. Выберите один и нажмите кнопку **Загрузить**.
...
[Начало работы с мобильными службами]: mobile-services-javascript-backend-windows-store-dotnet-get-started.md
