---
title: Обработка конфликтов записи базы данных с поддержкой оптимистичного параллелизма (Магазин Windows) | Microsoft Docs
...
3. В обозревателе решений Visual Studio откройте файл MainPage.cs в общем проекте. Добавьте обработчик событий в MainPage для события текстового поля `LostFocus`, как показано ниже.
...
1. В файле MainPage.xaml.cs для общего проекта добавьте определение для метода MainPage `UpdateToDoItem()`, на который ссылается обработчик событий, как показано ниже.
...
1. Откройте файл TodoItem.cs в общем проекте и обновите определение класса `TodoItem` с помощью приведенного ниже кода, чтобы добавить системное свойство `__version`. Благодаря этому будет включена поддержка обнаружения конфликтов записи.
   ...
   > [!NOTE]
   > При использовании нетипизированных таблиц включите оптимистичный параллелизм, добавив флаг "Версия" в системные свойства таблицы.
   > 
   > `````
...
7. В экземпляре 1 приложения измените значение последнего элемента на **Тест записи 1**, а затем выберите второе текстовое поле, чтобы обработчик событий `LostFocus` обновил базу данных. На следующем снимке экрана показан пример.
