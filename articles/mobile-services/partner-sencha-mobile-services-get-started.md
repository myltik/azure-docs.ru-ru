...
3. Чтобы создать таблицу для хранения данных приложения, нажмите кнопку **Создание таблицы TodoItems**.
	...
	> [AZURE.NOTE] НЕ загружайте HTML-приложение с классического портала Azure. Вместо этого мы вручную создадим приложение Sencha Touch в следующем разделе.


1. Запишите значения **appKey** и **appUrl** на классическом портале Azure. Они нам понадобятся в других разделах данного учебника.

    ![ключ приложения](./media/partner-sencha-mobile-services-get-started/mobile-app-key-portal.png)

1. На вкладке **Настройка** убедитесь, что элемент `localhost` уже указан в списке **Разрешить запросы от имен узлов** в разделе **Общий доступ к ресурсам независимо от источника (CORS)**. Если этот элемент не указан, введите `localhost` в поле **Имя узла** и нажмите кнопку **Сохранить**.
...
> [AZURE.NOTE] Этот метод можно использовать только в случае, если приложение было создано с помощью команды <code>sencha generate app</code>.
