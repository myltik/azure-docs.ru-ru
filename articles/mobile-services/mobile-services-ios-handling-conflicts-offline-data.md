...
В конце данного учебника запустите это приложение на двух телефонах, локально измените один и тот же элемент на обоих телефонах и принудительно отправьте изменения обратно на сервер. Благодаря этому вы разрешите пользователю на каждом телефоне выбрать версию, которую необходимо оставить: * версию клиента (которая переопределяет версию на сервере), * версию сервера (которая обновляет локальную таблицу клиента) или * не оставлять ни одну версию (в этом случае будут отменены push-уведомления, а операция будет оставлена в состоянии ожидания).
...
[Начало работы с автономными данными]: mobile-services-ios-get-started-offline-data.md
