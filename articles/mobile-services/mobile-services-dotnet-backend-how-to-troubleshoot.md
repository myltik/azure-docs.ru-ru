---
title: Устранение неполадок серверной части .NET мобильных служб | Microsoft Docs
...
3. Выберите узел **Символы** слева и добавьте ссылку на сервер [SymbolSource] с помощью универсального кода ресурса (URI) `http://srv.symbolsource.org/pdb/Public`. Символы для серверной части .NET мобильных служб становятся доступными здесь с каждым новым выпуском.
...
> [!NOTE]
> На данный момент мобильные службы совместимы только с ASP.NET 5.1; ASP.NET 5.2 в настоящее время не поддерживается. Обновление пакетов ASP.NET NuGet до 5.2.* может привести к ошибке после развертывания.
> 
> 
