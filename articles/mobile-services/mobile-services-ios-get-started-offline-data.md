---
title: Приступая к работе с синхронизацией автономных данных в мобильных службах (iOS) | Microsoft Docs
...
> [!NOTE]
> Чтобы удалить записи из локального хранилища устройства, если они удалены из базы данных мобильной службы, включите [обратимое удаление]. В противном случае приложение должно периодически вызывать `MSSyncTable.purgeWithQuery` для очистки локального хранилища.
> 
> 
...
> [!NOTE]
> Пакет SDK для мобильных служб резервирует имена столбцов, начинающиеся с "**`ms_`**". Не используйте этот префикс где-либо, кроме системных столбцов. В противном случае при использовании удаленной службы имена таких столбцов будут изменены.
> 
> 
...
3. Просмотрите содержимое удаленной таблицы TodoItem. Убедитесь, что новые элементы *не* синхронизированы с сервером.
