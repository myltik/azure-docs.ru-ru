---
title: Управление доступом на основе ролей в мобильных службах с помощью .NET и Azure Active Directory (Магазин Windows) | Microsoft Docs
...
11. В файле AuthorizeAadRole.cs обновите метод `OnAuthorization` в классе `AuthorizeAadRole` с помощью следующего кода: Этот код предполагает, что пользователь, вызывающий мобильную службу, прошел проверку подлинности в AAD. Он возвращает идентификатор объекта пользователя AAD и проверяет членство в группе Active Directory, которая соответствует роли.
    ...
    > [!NOTE]
    > Поиск группы Active Directory осуществляется по имени. Тем не менее, во многих случаях лучше сохранить идентификатор группы как параметр приложения мобильной службы. Это связано с тем, что имя группы может измениться, но идентификатор останется прежним.
    > 
    > 
