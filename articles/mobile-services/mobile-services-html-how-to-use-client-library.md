---
title: Использование HTML-клиента с мобильными службами Azure | Microsoft Docs
...
Замените заполнитель `AppUrl` на URL-адрес приложения мобильной службы, а заполнитель `AppKey` — на ключ приложения, который вы найдете на [классическом портале Azure](http://manage.windowsazure.com/).
...
Вызывая `where` объекта Query и передавая объект в качестве параметра, мы указываем мобильным службам вернуть только строки, в которых столбец `complete` содержит значение `false`. Кроме того, посмотрите на приведенный ниже URI запроса и обратите внимание на изменение самой строки запроса:
...
> [!NOTE]
> Чтобы предотвратить возврат всех элементов, по умолчанию используется размер страницы, управляемый сервером. Это предотвращает негативное воздействие больших наборов данных на функционирование службы. Можно увеличить количество возвращаемых элементов путем вызова `take`, как описано в следующем разделе. `todoItemTable` — это ссылка на созданную ранее таблицу мобильной службы.
> 
> 
...
> [!NOTE]
> Если в приложении PhoneGap или Cordova используется проверка подлинности, также необходимо добавить в проект следующие подключаемые модули:
> 
...
Поддерживается два потока проверки подлинности: *серверный* и *клиентский*. Серверный поток обеспечивает самый простой способ проверки подлинности, так как он использует веб-интерфейс проверки подлинности. Клиентский поток обеспечивает более тесную интеграцию с возможностями устройства, такими как единый вход, так как использует пакеты SDK конкретного поставщика для конкретного устройства.
...
В следующем примере показано использование пакета Live SDK с интерфейсами WinJS API для обеспечения расширенного интерфейса единого входа:
