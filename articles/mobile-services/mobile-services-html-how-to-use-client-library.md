...
В этом примере предполагается, что маркер, предоставленный соответствующим поставщиком SDK, сохраняется в переменной `token`. В настоящее время Twitter не может использоваться для проверки подлинности клиента. На данный момент Microsoft Azure Active Directory не может использоваться с серверами JavaScript для проверки подлинности клиента.
