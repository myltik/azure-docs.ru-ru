---
title: Работа с серверной мобильной службой JavaScript
...
> [!NOTE]
> Так как delete является зарезервированным ключевым словом в JavaScript, функция, зарегистрированная для операции удаления, должна включать имя *del*.
> 
> 
...
> [!IMPORTANT]
> Когда сценарий содержит путь кода, в котором не вызывается ни функция **execute**, ни функция **respond**, операция может перестать отвечать.
> 
> 
...
> [!NOTE]
> В скрипте удаления изменение значения предоставляемой переменной userId не влияет на то, какая запись удаляется.
> 
> 
...
> [!NOTE]
> Функцию **respond** без аргументов для вызова ответа по умолчанию можно вызвать, только после вызова функции **execute**.
> 
> 
...
> [!NOTE]
> Если система управления версиями включена, можно редактировать файлы скриптов запланированных заданий непосредственно во вложенной папке .\\service\\scheduler в репозитории git. Дополнительные сведения содержатся в разделе [Практическое руководство. Совместное использование кода с помощью системы управления версиями].
> 
> 
...
> [!NOTE]
> Модули, которые вы указали в package.json или загрузили в свою мобильную службу, используются только в коде серверного скрипта. Эти модули не используются средой выполнения мобильных служб.
> 
> 
...
> [!NOTE]
> Общие функции не могут получить доступ к объекту **tables** напрямую. В случае общей функции необходимо передать этот объект функции.
> 
> 
...
> [!NOTE]
> Для просмотра журналов мобильной службы войдите на [классический портал Azure](https://manage.windowsazure.com/), выберите мобильную службу и перейдите на вкладку **Журналы**.
> 
> 
