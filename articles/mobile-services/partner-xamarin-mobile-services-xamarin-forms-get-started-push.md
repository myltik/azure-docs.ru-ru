---
title: Добавление push-уведомлений в приложение мобильных служб (Xamarin.Forms) — мобильные службы
...
3. [Добавление push-уведомлений в приложение Xamarin.Forms.iOS](#iOS)
4. [Добавление push-уведомлений в приложение Xamarin.Forms.Android](#Android)
5. [Добавление push-уведомлений в приложение Xamarin.Forms.Windows](#Windows)
6. [Обновление скрипта вставки таблицы Azure для отправки push-уведомлений всем приложениям](#all-apps)
...
5. Перейдите на вкладку **Столбцы** и убедитесь, что имеется только один столбец **ID**, который должен быть создан автоматически.
   ...
   > [!NOTE]
   > Если в вашей мобильной службе появится динамическая схема, новые столбцы будут создаваться автоматически при отправлении объектов JSON в мобильную службу при операции вставки или обновления.
   > 
   > 
...
> [!IMPORTANT]
> В соответствии с требованиями службы push-уведомлений Apple (APNS) необходимо развернуть и протестировать push-уведомления на устройстве с iOS (iPhone или iPad), а не в эмуляторе.
> 
> 
...
1. В **AppDelegate** переопределите событие **RegisteredForRemoteNotifications**:
...
2. В **AppDelegate** переопределите событие **ReceivedRemoteNotification**:
...
3. Замените функцию вставки следующим кодом и нажмите кнопку **Сохранить**:
   ...
   > [!NOTE]
   > Этот скрипт задерживает отправку уведомления, чтобы вы успели закрыть приложение для получения всплывающего уведомления.
   > 
   > 
...
> [!NOTE]
> Ниже показано, как на классическом портале Azure обновить сценарий, зарегистрированный для операции вставки в таблице TodoItem. Вы также можете вызвать и изменить этот скрипт мобильной службы непосредственно в Visual Studio на узле Azure в обозревателе сервера.
> 
> 
...
1. В **todoitem** перейдите на вкладку **Скрипт** и выберите **Вставка**.
   
   ![][22]

При этом отображается функция, вызываемая при выполнении вставки в таблице **TodoItem**.

1. Замените функцию вставки следующим кодом и нажмите кнопку **Сохранить**:
...
3. Откройте файл проекта MainActivity.cs и добавьте в класс приведенный ниже оператор Using.
...
1. В проекте ToDoAzure.Droid создайте новый класс в проекте с именем `GcmService`.
2. Добавьте следующие операторы using в класс **GcmService**.
...
4. В файл проекта **GcmService.cs** добавьте приведенный ниже класс.
...
5. В файл проекта GcmService.cs добавьте приведенный ниже код, который является определяющим для класса **GcmService**.
...
1. Добавьте в класс **GcmService** приведенный ниже код, который переопределяет обработчик событий **OnRegistered**.
...
2. Переопределите метод **OnMessage** в **GcmService** с использованием приведенного ниже кода.
...
3. Добавьте следующие переопределения метода для **OnUnRegistered()** и **OnError()**, необходимые для компиляции проекта.
...
> [!IMPORTANT]
> Чтобы получать push-уведомления, необходимо настроить учетную запись Google на виртуальном устройстве Google Android (в эмуляторе выберите **Параметры** и **Добавить учетную запись**). Кроме того, убедитесь, что эмулятор подключен к Интернету.
> 
> 
...
4. Добавьте новую задачу в приложении.
5. Проведите пальцем вниз с верхней части экрана, чтобы открыть центр уведомлений для просмотра уведомления.
...
2. Добавьте в App.xaml.cs следующий код:
   ...
   > [!NOTE]
   > В этом учебнике мобильная служба отправляет на устройство всплывающее уведомление. Когда вы отправляете всплывающее уведомление, необходимо вместо этого вызвать метод **BindToShellTile** в канале.
   > 
   > 
...
5. В обозревателе решений разверните узел **Свойства**, откройте файл WMAppManifest.xml, щелкните вкладку **Возможности** и убедитесь, что установлен флажок для возможности **ID\_CAP\_PUSH\_NOTIFICATION**.
...
1. Откройте вкладку **Push-уведомления**, установите флажок **Включить push-уведомления без проверки подлинности**, затем щелкните **Сохранить**.
...
1. В Visual Studio нажмите клавишу F5, чтобы запустить приложение.
   ...
   > [!NOTE]
   > При проверке в эмуляторе Windows Phone может возникнуть исключение "401 Не санкционировано" RegistrationAuthorizationException. Это может произойти при выполнении вызова `RegisterNativeAsync()` из-за способа синхронизации часов в эмуляторе Windows Phone с главным компьютером. В результате маркер безопасности может быть не принят. Чтобы устранить эту проблему, вручную переведите часы в эмуляторе перед тестированием.
   > 
   > 
2. В приложении создайте новую задачу с названием **Hello push**, а затем сразу же нажмите кнопку «Запустить» или кнопку «Назад», чтобы выйти из приложения.
