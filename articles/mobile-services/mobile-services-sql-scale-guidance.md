...
Важно понимать различные уровни базы данных, имеющиеся в вашем распоряжении. Это позволит обеспечивать выбор нужного уровня с учетом потребностей вашего приложения. База данных Azure SQL предлагает два варианта базы данных на трех уровнях:

- выпуски Web и Business (скоро станут недоступны),
- уровни обслуживания Basic, Standard и Premium.

Несмотря на то, что выпуски Web и Business полностью поддерживаются, с 12 сентября 2015 г. они будут недоступны (см. раздел [Вопросы и ответы по остановке выпусков Web и Business](http://msdn.microsoft.com/library/azure/dn741330.aspx)). В связи с этим мы рекомендуем новым клиентам сразу использовать уровни обслуживания Basic, Standard и Premium. Они предоставляют различные функции мониторинга, упрощающие анализ производительности базы данных и устранение связанных с ней проблем. Все новые мобильные службы создаются с использованием новых уровней обслуживания.

Чтобы преобразовать мобильную службу, использующую выпуск Web или Business, в службу на основе уровня обслуживания Basic, Standard или Premium, выполните следующие действия.
...
3. Введите имя базы данных, а затем выберите **Новый сервер базы данных SQL** в поле **Сервер**. Будет создан сервер на основе уровня обслуживания Basic, Standard или Premium. 
...
> [AZURE.NOTE]Выполните этот запрос в базе данных **master** на своем сервере. В этой базе данных присутствует только представление **sys.resource\_stats**.
...
> [AZURE.NOTE]Выполните этот запрос в базе данных **master** на своем сервере. В этой базе данных присутствует только представление **sys.event\_log**.
