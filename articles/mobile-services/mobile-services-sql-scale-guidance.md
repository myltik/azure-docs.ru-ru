...
3. Выберите метрику, для которой необходимо задать предупреждение, а затем выберите команду **Добавить правило**. ![Классический портал Azure — предупреждение SQL][PortalSqlAddAlert]
4. Укажите имя и описание предупреждения. ![Классический портал Azure — имя и описание предупреждения SQL][PortalSqlAddAlert2]
5. Укажите значение, используемое в качестве порога предупреждения. Выберите значение **80%**, чтобы предусмотреть некоторое время на реагирование. Кроме этого, обязательно укажите адрес электронной почты, который вы активно отслеживаете. ![Классический портал Azure — порог и электронный адрес предупреждения SQL][PortalSqlAddAlert3]
...
В случае использования уровней «Базовый», «Стандартный» и «Премиум» классический портал Azure предоставляет несколько готовых метрик. Однако при использовании уровней Web и Business на портале доступна только метрика хранилища. Но, независимо от применяемого уровня, эти и другие метрики можно очень просто получить с помощью административного представления **[sys.resource\_stats](http://msdn.microsoft.com/library/dn269979.aspx)**. Рассмотрим следующий запрос:
