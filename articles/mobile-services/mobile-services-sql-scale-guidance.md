---
title: Масштабирование мобильных служб, работающих с Базой данных Azure SQL | Microsoft Docs
...
Дополнительные сведения об индексах см. в разделе [Заметки об индексе на платформе Entity Framework][Заметки об индексе на платформе Entity Framework]. Дополнительные рекомендации по оптимизации индексов см. в разделе [Расширенная индексация](#AdvancedIndexing) ниже в этом документе.
...
SQL Server Management Studio представляет собой бесплатное приложение Windows, которое предоставляет наиболее широкие возможности. Если у вас нет доступа к компьютеру Windows (например, в случае использования ПК Mac), провизионируйте виртуальную машину в Azure, как показано в разделе [Создание виртуальной машины, на которой выполняется Windows Server](../virtual-machines/virtual-machines-windows-hero-tutorial.md), а затем дистанционно подключитесь к ней. Если вы планируете использовать ВМ, прежде всего, для выполнения пакета SQL Server Management Studio, то будет достаточно применить экземпляр **Basic A0** (ранее назывался «Очень мелкий»).
...
## См. также
* [Документация по базе данных SQL Azure][Документация по базе данных SQL Azure]
* [Производительность и масштабирование базы данных SQL Azure][Производительность и масштабирование базы данных SQL Azure]
* [Устранение неполадок, связанных с базами данных SQL Azure][Устранение неполадок, связанных с базами данных SQL Azure]

### Индексация
* [Общие сведения об индексировании][Общие сведения об индексировании]
...
* [Ограничения для первичного и внешнего ключей][Ограничения для первичного и внешнего ключей]
...
* [Заметки к данным Code First][Заметки к данным Code First]
