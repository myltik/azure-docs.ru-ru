---
title: Изменение модели данных в серверной мобильной службе для .NET
...
В этом разделе показано, как использовать Code First Migrations в Entity Framework для изменения модели данных существующей Базы данных SQL Azure, чтобы избежать потери существующих данных. В этой процедуре предполагается, что вы уже опубликовали проект серверной части .NET в Azure, в базе данных есть данные и удаленная и локальная модели данных по-прежнему синхронизированы. В этом разделе также описываются первые инициализаторы Code First, внедренные мобильными службами Azure, которые используются при разработке. Эти инициализаторы позволяют легко выполнять изменения схемы без использования Code First Migrations, когда не нужно поддерживать существующие данные.

> [!NOTE]
> Имя схемы, которое используется как префикс для имен таблиц в Базе данных SQL, определяется параметром приложения MS\_MobileServiceName в файле web.config. После загрузки начального проекта с портала этому значению уже задано имя мобильной службы. Когда имя схемы соответствует мобильной службе, несколько мобильных служб могут безопасно использовать один и тот же экземпляр базы данных.
> 
> 

Обратите внимание, что в проекте серверной части .NET не поддерживаются автоматические миграции.
...
* **ClearDatabaseSchemaIfModelChanges** <br/> Объекты схемы удаляются, только если Code First обнаружит изменения в модели данных. Инициализатор по умолчанию в проекте серверной части .NET, скачанном с [классического портала Azure], наследует характеристики от этого базового класса.
...
При работе на локальном компьютере можно использовать другие инициализаторы модели данных Code First. Однако в Azure инициализаторы, которые пытаются удалить базу данных, породят сбой, так как у пользователя нет разрешений удалять базу данных, и это хорошо.
...
> [!IMPORTANT]
> Во время разработки и тестирования проекта мобильной службы с действующими службами Azure следует всегда использовать экземпляр мобильной службы, предназначенный для тестирования. Никогда не используйте для разработки и тестирования мобильную службу, которая работает в производственной среде или используется клиентскими приложениями.
> 
> 

В скачанном проекте быстрого запуска инициализатор Code First определен в файле WebApiConfig.cs. Переопределите метод **Seed**, чтобы добавить исходные строки данных в новые таблицы. Примеры заполнения данных см. в разделе [Заполнение данных при миграции].
...
5. Разверните папку App\_Start, откройте файл проекта WebApiConfig.cs и добавьте следующие инструкции **using**:
    ...
    В приведенном выше коде необходимо заменить строку *todolistService* пространством имен вашего проекта. Для загруженного примера проекта это <em>mobile&95;service&95;name</em>Service.
...
11. (Необязательно). На [классическом портале Azure] выберите свою мобильную службу, откройте вкладку **Настройка** и щелкните **База данных SQL**. При этом откроется страница базы данных SQL для базы данных вашей мобильной службы.
...
## Использование Code First Migrations без инициализатора
Прежде чем использовать Code First Migrations в своем проекте серверной части .NET, следует запустить инициализатор модели данных. Если инициализатор НЕ используется, при попытке применить миграции могут возникнуть ошибки. Если вы решили не использовать ни один из предварительно определенных инициализаторов модели данных, убедитесь, что для миграций настроено использование EntityTableSqlGenerator в качестве SqlGenerator в файле Migrations\\Configuration.cs, как показано в примере ниже.
