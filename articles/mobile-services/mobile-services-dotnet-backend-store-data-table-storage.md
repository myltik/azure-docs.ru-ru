---
title: Построение серверной мобильной службы .NET, использующей табличное хранилище | Microsoft Docs
...
1. Измените тип данных **TodoItem**, чтобы он наследовался от **StorageData**, а не от **EntityData**, как показано ниже.
   ...
   > [!NOTE]
   > Тип **StorageData** имеет свойство Id (идентификатор), для которого требуется составной ключ (строка в формате *partitionId*,*rowValue*).
   > 
   > 
...
4. Замените метод **GetAllTodoItems** следующим кодом.
...
2. Запустите учебное клиентское приложение, подключенное к вашей мобильной службе.
...
3. Добавьте новые элементы, чтобы сгенерировать изменения в базе данных.
