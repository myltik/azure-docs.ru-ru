...
Сертификаты используются в Azure для проверки подлинности VPN-клиентов в VPN-подключениях типа "точка — сеть".
...
## <a name="upload"></a>4. Передача CER-файла корневого сертификата

Передайте CER-файл (который содержит сведения об открытом ключе) доверенного корневого сертификата в Azure. Вы можете отправить файлы для 20 корневых сертификатов. Не отправляйте закрытый ключ для корневого сертификата в Azure. После отправки CER-файла он используется в Azure для проверки подлинности клиентов, подключающихся к виртуальной сети. При необходимости позже вы можете передать дополнительные открытые ключи корневого сертификата.
...
Настройте и создайте шлюз для своей виртуальной сети. У параметра *-GatewayType* должно быть значение **Vpn**, а у параметра *-VpnType* — **RouteBased**. В этом примере открытый ключ корневого сертификата привязывается к VPN-шлюзу. Создание VPN-шлюза может занять до 45 минут.
...
### <a name="to-add-a-root-certificate"></a>Добавление корневого сертификата
