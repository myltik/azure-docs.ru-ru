...
Для подключения типа "точка–сеть" не требуется VPN-устройство или общедоступный IP-адрес. Дополнительные сведения о подключениях типа "точка — сеть" см. в статьях [VPN-шлюз: вопросы и ответы](vpn-gateway-vpn-faq.md#point-to-site-connections) и [О безопасных распределенных подключениях для виртуальных сетей](vpn-gateway-cross-premises-options.md).

Это статья о подключениях типа "точка — сеть" через VPN-шлюз к виртуальным сетям, созданным с помощью **модели развертывания диспетчера ресурсов**.
...
- Имя — **TestVNet**, используемые адресные пространства — **192.168.0.0/16** и **10.254.0.0/16**. Обратите внимание, что для виртуальной сети можно использовать более одного адресного пространства.
...
- Вам потребуется установить командлеты PowerShell диспетчера ресурсов Azure (1.0.2 или более поздней версии). Дополнительные сведения об установке командлетов PowerShell см. в статье [Установка и настройка Azure PowerShell](../powershell-install-configure.md).
...
Сертификаты используются для проверки подлинности VPN-клиентов в VPN-подключениях типа "точка — сеть". Ниже приведены пошаговые указания, которые позволят добавить и удалить корневые сертификаты. При добавлении CER-файла Base64-encoded X.509 в Azure вы указываете Azure доверять корневому сертификату, который представляет собой файл.

### Добавление доверенного корневого сертификата

В Azure можно добавить до 20 доверенных корневых сертификатов. Чтобы добавить корневой сертификат, сделайте следующее:
...
2. Добавьте новый корневой сертификат. Обратите внимание, что можно добавлять только один сертификат за раз.
...
Можно удалить доверенный корневой сертификат из Azure. При удалении доверенного сертификата сертификаты клиента, созданные на его основе, не смогут подключаться к Azure с использованием подключения типа "точка — сеть", пока не будет установлен сертификат клиента, созданный на основе доверенного сертификата в Azure.

1. Чтобы удалить доверенный корневой сертификат, измените образец кода ниже.
