...
Сведения в этой статье применимы к виртуальным сетям и VPN-шлюзам, созданным с помощью модели развертывания **Azure Resource Manager**. Если нужно настроить подключение типа "точка-сеть" для виртуальной сети, созданной с помощью управления службами (другое название — классическая модель развертывания), см. сведения в статье [Настройка VPN-подключения типа "точка-сеть" к виртуальной сети](vpn-gateway-point-to-site-create.md).
