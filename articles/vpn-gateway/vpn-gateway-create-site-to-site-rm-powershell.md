...
В этой статье мы покажем вам, как создать виртуальную сеть, соединенную с локальной сетью через VPN-подключение типа "сеть — сеть", с использованием модели развертывания **Azure Resource Manager**. Подключения типа "сеть — сеть" можно использовать для распределенных и гибридных конфигураций.
...
На примерах ниже показана подсеть шлюза размера /28. Хотя можно создать подсеть шлюза размера /29, мы не рекомендуем это делать. Рекомендуется создать подсеть шлюза размера /27 или больше (/26, и /25 и т. д.), чтобы удовлетворить требования к дополнительным компонентам. Если у вас уже есть виртуальная сеть с подсетью шлюза размера /29 или больше, сразу переходите к разделу [Шаг 3. Добавление шлюза локальной сети](#localnet).
...
## 3\. <a name="localnet"></a>Добавление шлюза локальной сети
...
Иногда префиксы адресов шлюза локальной сети могут изменяться. Действия по изменению префиксов IP-адресов будут разными в зависимости от того, создано ли подключение через VPN-шлюз. См. раздел [Изменение префиксов IP-адресов для шлюза локальной сети](#modify) этой статьи.
...
- Значение параметра **-GatewayType** для конфигурации "сеть — сеть" — **Vpn**. Тип шлюза всегда зависит от реализуемой конфигурации. Например, для других конфигураций шлюза может потребоваться тип шлюза ExpressRoute. 
...
Далее вам нужно создать VPN-подключение типа "сеть — сеть" между шлюзом виртуальной сети и VPN-устройством. Обязательно подставьте собственные значения. Общий ключ должен соответствовать значению, использованному в конфигурации VPN-устройства. Обратите внимание, что для подключения типа "сайт — сайт" параметр `-ConnectionType` имеет значение **IPsec**.
...
Если вам нужно изменить префиксы для шлюза локальной сети, выполните следующие действия. Мы приводим два набора инструкций. Выбор между ними зависит от того, создано ли у вас подключение через VPN-шлюз.
