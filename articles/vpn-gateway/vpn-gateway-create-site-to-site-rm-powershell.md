...
На примерах ниже показана подсеть шлюза размера /28. Хотя можно создать подсеть шлюза размера /29, мы не рекомендуем это делать. Рекомендуется создать подсеть шлюза размера /27 или больше (/26, и /25 и т. д.), чтобы удовлетворить требования к дополнительным компонентам. Если у вас уже есть виртуальная сеть с подсетью шлюза c длиной префикса 29 бит (/29) или больше, сразу переходите к разделу [Добавление шлюза локальной сети](#localnet).
...
Иногда префиксы адресов шлюза локальной сети могут изменяться. Действия по изменению префиксов IP-адресов будут разными в зависимости от того, создано ли подключение через VPN-шлюз. См. раздел [Изменение префиксов IP-адресов для шлюза локальной сети](#modify) в этой статье.
