...
- Последняя версия командлетов Azure PowerShell. Вы можете загрузить ее в разделе Windows PowerShell на [странице загрузки](http://azure.microsoft.com/downloads/), а затем установить. Данная статья написана для Azure PowerShell *0.9.8*.

>[AZURE.NOTE]Если у вас запущены критически важные приложения, продолжайте использовать Azure PowerShell 0.9.8. В большинстве случаев единственное различие между двумя версиями заключается в том, что имя командлета предварительной версии 1.0 имеет формат «{глагол}-AzureRm{существительное}», а имя командлета версии 0.9.8 не содержит буквы Rm. Например, New-AzureRmResourceGroup вместо New-AzureResourceGroup. Сведения о Azure PowerShell предварительной версии 1.0 см. в этой [записи блога](https://azure.microsoft.com/blog/azps-1-0-pre/). Дополнительную информацию о командлетах Azure PowerShell предварительной версии 1.0 см. в разделе [Командлеты диспетчера ресурсов Azure](https://msdn.microsoft.com/library/mt125356.aspx).


## 1\. Подключение к подписке 


Откройте консоль PowerShell и подключитесь к своей учетной записи.

**Примечание**. Приведенные ниже инструкции основаны на версии командлетов Azure PowerShell 0.9.8.
...
- Тип шлюза — *Vpn*.
- Параметр VpnType может иметь значение RouteBased* (в некоторых документах такой шлюз называется шлюзом с динамической маршрутизацией) или *Policy Based* (или шлюз со статической маршрутизацией). Дополнительные сведения о типах VPN-шлюзов см. в статье [Шлюзы VPN](vpn-gateway-about-vpngateways.md). 	
...
На этом этапе вам понадобится общедоступный IP-адрес шлюза виртуальной сети для настройки локального VPN-устройства. Обратитесь к производителю устройства, чтобы получить конкретные сведения о конфигурации. Дополнительные сведения также см. в статье [Сведения о VPN-устройствах](http://go.microsoft.com/fwlink/p/?linkid=615099).
...
- Чтобы **добавить** дополнительные префиксы для созданного локального сайта без подключения через VPN-шлюз, используйте следующий пример.
