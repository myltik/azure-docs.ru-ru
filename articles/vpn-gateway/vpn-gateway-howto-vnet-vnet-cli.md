...
description: "В этой статье описывается, как настроить подключение между виртуальными сетями, используя подключение \"виртуальная сеть — виртуальная сеть\" и Azure CLI."
...
В этой статье объясняется, как подключить одну виртуальную сеть к другой, используя подключение "виртуальная сеть — виртуальная сеть". Виртуальные сети могут относиться к одному или разным регионам и к одной или разным подпискам. При подключении виртуальных сетей из разных подписок подписки не обязательно должны быть связаны с одним и тем же клиентом Active Directory.
...
Подключение "виртуальная сеть — виртуальная сеть" (VNet2VNet) между виртуальными сетями похоже на создание подключения IPsec к локальному сайту. В обоих типах подключений используется VPN-шлюз для создания защищенного туннеля, использующего IPsec/IKE. При обмене данными оба типа подключений работают одинаково. Различие между этими типами заключается в способе настройки шлюза локальной сети. При создании подключения "виртуальная сеть — виртуальная сеть" диапазон адресов для шлюза локальной сети не отображается. Он создается и заполняется автоматически. Если вы обновите диапазон адресов для одной виртуальной сети, другая виртуальная сеть автоматически определит маршрут к обновленному диапазону адресов.

При работе со сложными конфигурациями можно использовать подключение IPsec, а не "виртуальная сеть — виртуальная сеть". Это позволит вам указать дополнительный диапазон адресов для шлюза локальной сети, чтобы маршрутизировать трафик. Если между виртуальными сетями установить подключение IPsec, вам потребуется вручную создать и настроить шлюз локальной сети. Дополнительные сведения см. в статье о [конфигурациях "сеть — сеть"](vpn-gateway-howto-site-to-site-resource-manager-cli.md).

Кроме того, если виртуальные сети находятся в одном регионе, их можно соединить между собой с помощью пиринга. При пиринговой связи между виртуальными сетями VPN-шлюз не используется. Цены и возможности также отличаются. Дополнительную информацию см. в статье [Пиринговая связь между виртуальными сетями](../virtual-network/virtual-network-peering-overview.md).
...
Из этой статьи вы узнаете, как подключить виртуальные сети, используя подключение "виртуальная сеть — виртуальная сеть". В этой статье приведены два разных набора действий. Один предназначен для [виртуальных сетей в одной подписке](#samesub), а другой — для [виртуальных сетей в разных подписках](#difsub). 
