...
- Последняя версия командлетов Azure PowerShell. Скачать и установить последнюю версию вы можете в разделе Windows PowerShell на [странице скачивания](http://azure.microsoft.com/downloads/).
...
1. **Создайте VPN типа "сеть — сеть" со шлюзом с динамической маршрутизацией (маршрутизацией на основе маршрутов).** Если это уже сделано, отлично! Вы можете перейти к [экспорту параметров конфигурации виртуальной сети](#export). Если нет, выполните следующее:
