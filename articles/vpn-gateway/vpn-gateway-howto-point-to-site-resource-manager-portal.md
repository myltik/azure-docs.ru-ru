...
2. На странице **Point-to-Site-Configuration** (Конфигурация "точка — сеть") можно удалить автоматически заполненный диапазон, а затем добавить диапазон частных IP-адресов, который вы хотите использовать. VPN-клиенты динамически получают IP-адрес из указанного вами диапазона. Щелкните **Сохранить**, чтобы проверить и сохранить настройки.
