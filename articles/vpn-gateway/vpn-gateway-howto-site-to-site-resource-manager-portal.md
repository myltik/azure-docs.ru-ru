...
В этой статье мы покажем вам, как создать виртуальную сеть, соединенную с локальной сетью через VPN-подключение типа "сеть — сеть", с использованием модели развертывания Azure Resource Manager и портала Azure. С помощью указанных ниже шагов вы создадите виртуальную сеть, добавите подсеть шлюза, шлюз и локальный сайт, а также настроите подключение. Кроме того, вам потребуется настроить VPN-устройство.
...
Если вы уже создали виртуальную сеть, убедитесь, что настройки совместимы со схемой шлюза VPN; обратите особое внимание на подсети, которые могут пересекаться с другими сетями. Если у вас есть пересекающиеся подсети, подключение будет работать неправильно. Убедившись, что виртуальная сеть настроена правильно, переходите к действиям из раздела [Выбор DNS-сервера](#dns).
...
Если вы создаете эту конфигурацию в качестве упражнения, при выборе DNS-сервера можно использовать [вот эти значения](#values).
...
Если вы создаете эту конфигурацию в качестве упражнения, при создании шлюза можно использовать [вот эти значения](#values).
...
Термин *локальный сетевой шлюз* указывает на локальное расположение. Шлюзу локальной сети необходимо присвоить имя, по которому на него сможет ссылаться служба Azure.

Если вы создаете эту конфигурацию в качестве упражнения, при добавлении локального сайта можно использовать [вот эти значения](#values).
...
Прежде чем начинать работу с этим разделом, убедитесь, что создание шлюза виртуальной сети и шлюзов локальных сетей завершено. Если вы создаете эту конфигурацию в качестве упражнения, при создании подключения можно использовать [вот эти значения](#values).
