...
Данная статья поможет вам создать и подключить виртуальные сети, объединенные с помощью классической модели развертывания (которую также называют управлением службами). При этом будут использоваться портал Azure (не портал предварительной версии) и командлеты PowerShell. Чтобы подключить виртуальные сети, созданные с помощью модели развертывания диспетчера ресурсов Azure, обратитесь к предыдущей вкладке. Порядок действий зависит от модели.

Виртуальную сеть, созданную по классической модели развертывания, можно также подключить к виртуальной сети, созданной с помощью модели диспетчера ресурсов. См. статью [Подключение классических виртуальных сетей к новым виртуальным сетям](../virtual-network/virtual-networks-arm-asm-s2s.md).
...
5. Теперь вы укажете каждой виртуальной сети друг на друга как на локальную сеть. На портале Azure перейдите на страницу **Панель мониторинга** сети VNet1. В разделе **Подключение типа "сеть-сеть"** выберите **Подключиться к локальной сети**, а затем в качестве локальной сети выберите **VNET2**.
...
Если вы хотите добавить виртуальные машины в свою виртуальную сеть, см. раздел [Создание виртуальной машины](../virtual-machines/virtual-machines-windows-tutorial-classic-portal.md).
...
Сведения об API REST см. в статье [Операции со шлюзами виртуальных сетей](https://msdn.microsoft.com/library/azure/jj154113.aspx).

Дополнительные сведения о VPN-шлюзах см. в статье [VPN-шлюз: вопросы и ответы](vpn-gateway-vpn-faq.md).

Дополнительные сведения о виртуальных сетях см. в статьях [Обзор виртуальных сетей Azure](../virtual-network/virtual-networks-overview.md) и [Часто задаваемые вопросы о виртуальных сетях](../virtual-network/virtual-networks-faq.md).
