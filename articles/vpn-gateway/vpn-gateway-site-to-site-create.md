...
Далее вы настроите шлюз виртуальной сети, чтобы создать безопасное подключение типа "сеть-сеть". См. статью [Настройка VPN-шлюза для классической модели развертывания](vpn-gateway-configure-vpn-gateway-mp.md).
