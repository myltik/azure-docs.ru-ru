...
Для работы VPN-шлюза требуется -GatewayType *Vpn*, как показано в следующем примере PowerShell для модели развертывания с помощью Resource Manager. При создании шлюза виртуальной сети необходимо убедиться, что в конфигурации указан правильный тип шлюза.
...
Для каждой конфигурации необходим определенный тип подключения шлюза виртуальной сети. Для `-ConnectionType` в PowerShell можно указать такие значения (развертывание Resource Manager):
...
В приведенном ниже примере PowerShell для модели развертывания с помощью Resource Manager для `-VpnType` задано значение *RouteBased*. При создании шлюза необходимо убедиться, что в конфигурации указано правильное значение -VpnType.
...
Чтобы настроить шлюз виртуальной сети, сначала нужно создать подсеть шлюза для виртуальной сети. Чтобы подсети шлюзов работали правильно, каждую подсеть нужно назвать *GatewaySubnet*. Это имя указывает Azure, что данную подсеть следует использовать для шлюза. При использовании классического портала подсети шлюза автоматически присваивается *Gateway* в интерфейсе портала. Это примечание касается только просмотра подсети шлюзов на классическом портале. Подсеть фактически создается с именем *GatewaySubnet*, под которым она отображается на портале Azure и в PowerShell.
...
При создании решения на основе VPN-шлюза нередко шлюз локальной сети представляет ваше локальное расположение. В классической модели развертывания шлюз локальной сети называется "локальным сайтом".
...
Следующий пример PowerShell для Resource Manager создает шлюз локальной сети.
