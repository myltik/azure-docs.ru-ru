...
> [!NOTE]
> Используйте инструкции этой статьи, чтобы создать сертификаты только при отсутствии доступа к компьютеру под управлением Windows 10. В противном случае мы рекомендуем использовать статью [Создание и экспорт сертификатов для подключений типа "точка — сеть" с помощью PowerShell](vpn-gateway-certificates-point-to-site.md).
>
...
* MakeCert не может создавать сертификаты SHA-2, а только SHA-1. Сертификаты SHA-1 все еще действительны для подключения типа "точка — сеть". SHA-1 использует хэш шифрования, который менее надежный чем SHA-2.
