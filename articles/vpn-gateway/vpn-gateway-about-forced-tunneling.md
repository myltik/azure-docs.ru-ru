<properties pageTitle="Configure Forced Tunneling for Microsoft Azure VPN Gateways | Microsoft Azure" description="Если у вас есть виртуальная сеть с распределенным шлюзом VPN, вы можете перенаправить или «принудительно направить» весь интернет-трафик обратно в локальное расположение. " services="vpn-gateway" documentationCenter="na" authors="cherylmc" manager="carolz" editor="" tags="azure-service-management"/>
<tags  
...
Сведения в этой статье применимы к виртуальным сетям и VPN-шлюзам, созданным с использованием классической модели развертывания. При наличии инструкции по настройке принудительного туннелирования для виртуальных сетей и VPN-шлюзов, созданных с использованием модели диспетчера ресурсов, мы добавим соответствующую ссылку в верхней части этой страницы.
...
- Обратите внимание, что с помощью этого механизма невозможно настроить принудительное туннелирование ExpressRoute. Такое туннелирование включается, когда предлагается маршрут по умолчанию с помощью сеансов пиринга BGP ExpressRoute. Дополнительные сведения см. в [документации ExpressRoute](https://azure.microsoft.com/documentation/services/expressroute/).
...
В данном примере у виртуальной сети MultiTier-VNet имеются 3 подсети (*Frontend*, *Midtier* и *Backend*) с 4 межорганизационными подключениями *DefaultSiteHQ* и 3 филиалами *Branch*. Выполнив указанные ниже действия, можно настроить подключение *DefaultSiteHQ* в качестве подключения к сайту по умолчанию для принудительного туннелирования и настроить принудительное туннелирование для подсетей *Midtier* и *Backend*.
...
- Последняя версия командлетов Azure PowerShell, использующая установщик веб-платформы. Загрузить и установить последнюю версию можно в разделе **Windows PowerShell** на [странице загрузки](http://azure.microsoft.com/downloads/).
...
Сведения о пользовательских маршрутах см. в статье [Пользовательские маршруты и перенаправление IP-адресов](../virtual-network/virtual-networks-udr-overview.md).

Сведения о защите сетевого трафика см. в статье [Группа безопасности сети](../virtual-network/virtual-networks-nsg.md). Обратите внимание на то, что группу безопасности сети ни в коем случае нельзя применять к подсети шлюза виртуальной сети Azure.
