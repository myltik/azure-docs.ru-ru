...
Подключение типа "виртуальная сеть — виртуальная сеть" похоже на подключение виртуальной сети к локальному сайту. В обоих типах подключений используется VPN-шлюз для создания защищенного туннеля, использующего IPsec/IKE. Между виртуальными сетями вместо подключений "виртуальная сеть — виртуальная сеть" можно создавать подключения "сеть — сеть" (IPsec). Оба типа подключений работают одинаково при обмене данными. Но если вы создадите подключение "виртуальная сеть — виртуальная сеть" и обновите диапазон адресов для одной виртуальной сети, другая виртуальная сеть автоматически определит маршрут к обновленному диапазону адресов. Если же вы создадите подключение "сеть — сеть" (IPsec), вам придется вручную настраивать диапазон адресов для шлюза виртуальной сети. При работе со сложными конфигурациями вы можете создавать подключения IPsec, а не "виртуальная сеть — виртуальная сеть". Это позволит вам вручную указать дополнительный диапазон адресов для шлюза локальной сети.
