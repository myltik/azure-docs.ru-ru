...
Данная статья относится к классической модели развертывания и порталу Azure (но не к порталу предварительной версии). На данный момент настройку VPN-шлюза для классической модели развертывания с использованием портала следует выполнять на портале Azure.
...
Описанная ниже процедура позволит вам выполнить настройку VPN-шлюза на портале Azure. Указанные действия относятся к шлюзам для виртуальных сетей, которые были созданы с помощью режима управления службами и отображаются на портале Azure. Они не подходят для работы с порталом предварительной версии или виртуальными сетями, настроенными с помощью режима диспетчера ресурсов. Сведения о создании шлюзов для виртуальных сетей, созданных с помощью режима диспетчера ресурсов, см. в статье [Создание виртуальной сети с подключением типа «сеть — сеть» с помощью диспетчера ресурсов Azure и PowerShell](vpn-gateway-create-site-to-site-rm-powershell.md).
...
1. На странице **Сети** портала Azure убедитесь в том, что в столбце состояния виртуальной сети отображается значение **Создана**.
...
Прежде чем менять тип шлюза убедитесь, что ваше VPN-устройство поддерживает выбранный вами тип маршрутизации. В разделе [VPN-устройства для виртуальной сети](vpn-gateway-about-vpn-devices.md) можно загрузить.новые примеры настроек маршрутизации и просмотреть требования к VPN-устройствам.
