...
1. Войдите на **классический портал Azure** (не просто портал Azure).
...
1. При создании корневого сертификата (на предыдущем этапе) также создается *CER*-файл. Теперь его необходимо передать на классический портал Azure. Обратите внимание на то, что CER-файл не содержит закрытый ключ корневого сертификата. Вы можете отправить до 20 корневых сертификатов.
...
1. На классическом портале Azure на странице **Панель мониторинга** вашей виртуальной сети перейдите в меню сводки в правом углу и щелкните пакет VPN для клиента, который нужно подключить к виртуальной сети.
2. 
