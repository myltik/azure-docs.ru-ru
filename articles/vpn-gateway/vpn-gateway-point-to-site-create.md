...
Это статья о подключениях типа "точка — сеть" для виртуальных сетей, созданных с помощью классической модели развертывания. В этой статье описывается использование классического портала. В настоящее время создание этой конфигурации на портале Azure не поддерживается.
...
1. На классическом портале Azure на странице **Панель мониторинга** для вашей виртуальной сети перейдите к меню быстрого обзора в правом углу. Список поддерживаемых клиентских операционных систем см. в разделе [Подключения "точка — сеть"](vpn-gateway-vpn-faq.md#point-to-site-connections) статьи "VPN-шлюз: вопросы и ответы".<br><br>Выберите пакет загрузки для операционной системы, в которой требуется установить клиент.
