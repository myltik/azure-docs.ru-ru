<properties
   ...
   description="Безопасно подключайтесь к своей виртуальной сети Azure, создав VPN-подключение ";точка — сеть";. Инструкции для виртуальных сетей, созданных с помощью модели развертывания службы управления (классическая модель)."
...
# Настройка VPN-подключения типа "точка — сеть" к виртуальной сети с помощью классического портала
...
Это статья о подключениях типа "точка — сеть" через VPN-шлюз к виртуальным сетям, созданным с помощью **классической модели развертывания** (Service Management) и классического портала. Когда мы разработаем инструкцию для портала Azure, мы добавим на эту страницу ссылку на соответствующую статью.
...
Настройка подключения типа "точка — сеть" состоит из четырех этапов. Порядок настройки этих компонентов играет важную роль, поэтому не пропускайте эти этапы и не меняйте их последовательность.


- **Раздел 1** посвящен созданию виртуальной сети и VPN-шлюза.
- **Раздел 2** поможет создать и отправить сертификаты, используемые для проверки подлинности.
- **Раздел 3** содержит инструкции по экспорту и установке сертификатов клиента.
- **Раздел 4** поможет настроить VPN-клиент.
...
1. Перейдите на [классический портал Azure](https://manage.windowsazure.com/). Обратите внимание, что в этих инструкциях используется классический портал, а не портал Azure.
...
6. Укажите следующие сведения и нажмите кнопку "Далее".
 ...
 - **Добавить адресное пространство**: добавляйте адресное пространство, только если это предусмотрено конфигурацией сети.
...
2. В нижней части страницы **Панель мониторинга** нажмите кнопку **Создать шлюз**. Появится сообщение **Создать шлюз для виртуальной сети «ваша сеть»?** Нажмите кнопку **Да**, чтобы начать создание шлюза. Создание шлюза может занять около 15 минут.
...
Сведения о том, как использовать самозаверяющий сертификат, приведены ниже. Инструкции по использованию сертификата, созданного с помощью корпоративного решения, будут отличаться, но вам в любом случае нужно будет сделать следующее.

### Часть 1. Определение или создание корневого сертификата

Если вы не планируете использовать сертификат, созданный с помощью корпоративного решения, вам нужно создать самозаверяющий корневой сертификат. Описанные в этом разделе действия выполняются в ОС Windows 8. Действия для ОС Windows 10 см. в статье [Работа с самозаверяющими корневыми сертификатами для конфигураций типа "точка — сеть"](vpn-gateway-certificates-point-to-site.md).
...
1. Откройте папку "Инструменты Visual Studio" и запустите окно командной строки от имени администратора.

2. В следующем примере команда создаст и установит корневой сертификат в хранилище личных сертификатов на компьютере, а также создаст соответствующий *CER-*файл, который вы затем отправите на классический портал Azure.

3. Перейдите в каталог, в котором должен располагаться CER-файл, и выполните указанную ниже команду, где *RootCertificateName* — имя сертификата. Если выполнить команду без изменений, будет создан корневой сертификат и соответствующий файл *RootCertificateName.cer*.
...
2. На классическом портале Azure на странице **Сертификаты** виртуальной сети щелкните **Загрузить корневой сертификат**.

3. На странице **Загрузка сертификата** найдите CER-файл корневого сертификата и установите флажок.

### Часть 3. Создание сертификата клиента

Описанные ниже шаги помогут вам создать сертификат клиента из корневого самозаверяющего сертификата. Описанные в этом разделе действия выполняются в ОС Windows 8. Действия для ОС Windows 10 см. в статье [Работа с самозаверяющими корневыми сертификатами для конфигураций типа "точка — сеть"](vpn-gateway-certificates-point-to-site.md). Если вы используете сертификат, созданный с помощью корпоративного решения, придерживайтесь следующих правил.
...
Для подключения к виртуальной сети также потребуется настроить VPN-клиент. Для подключения ему необходим как сертификат клиента, так и правильные настройки. Чтобы настроить VPN-клиент, выполните по порядку следующие действия.

### Часть 1. Создание пакета конфигурации VPN-клиента

1. На классическом портале Azure на странице **Панель мониторинга** виртуальной сети перейдите в меню сводки в правом углу и щелкните пакет VPN, относящийся к клиенту, который нужно подключить к виртуальной сети.
...
3. Выберите пакет загрузки для операционной системы, в которой требуется установить клиент.
...
4. Создание пакета занимает несколько минут. После завершения этой процедуры вы сможете скачать файл. Загруженный *EXE*-файл можно сохранить на локальном компьютере.
...
### Часть 2. Установка пакета конфигурации VPN на клиенте и запуск подключения
...
### Часть 3. Проверка VPN-подключения
