...
В этой статье мы покажем вам, как создать виртуальную сеть, соединенную с локальной сетью через подключение VPN типа "сеть — сеть" с помощью шлюза, используя классическую модель развертывания и портал Azure. 
...
9. Нажав кнопку "Создать", вы увидите плитку на панели мониторинга, на которой отображается ход создания виртуальной сети. Когда виртуальная сеть будет создана, плитка изменится.
...
4. В верхней части страницы щелкните **Сохранить**, чтобы сохранить параметры.
...
Необходимо создать подсеть шлюза для VPN-шлюза. Подсеть шлюза содержит IP-адреса, которые будут использовать службы VPN-шлюза.
...
Обратитесь к производителю устройства, чтобы получить конкретные сведения о конфигурации и настроить свое устройство. Дополнительные сведения о VPN-устройствах, которые работают с Azure, см. в статье о [VPN-устройствах](vpn-gateway-about-vpn-devices.md). Кроме того, проверьте VPN-устройство, которое вы хотите использовать, на наличие [известных проблем совместимости устройств](vpn-gateway-about-vpn-devices.md#known). 

При настройке VPN-устройства вам потребуется IP-адрес созданного VPN-шлюза. Его можно узнать, перейдя к колонке **Обзор** виртуальной сети.
