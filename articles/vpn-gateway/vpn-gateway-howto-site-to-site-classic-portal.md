...
В этой статье мы покажем вам, как создать виртуальную сеть, соединенную с локальной сетью через подключение VPN типа "сеть — сеть" с помощью шлюза, используя классическую модель развертывания и портал Azure. Эту конфигурацию также можно создать для модели развертывания с помощью Resource Manager, выбрав вариант из следующего списка:
...
* **DNS-сервер:** 8.8.8.8 (необязательно для этого упражнения).
...
2. Нажмите кнопку **Создать**. В поле **Поиск по Marketplace** введите "Виртуальная сеть". Найдите **виртуальную сеть** в результатах поиска и щелкните ее, чтобы открыть колонку **Виртуальная сеть**.
    ...
    ![Поиск колонки виртуальной сети](./media/vpn-gateway-howto-site-to-site-classic-portal/newvnetportal700.png)
...
4. В колонке **Создание виртуальной сети** настройте параметры виртуальной сети. В этой колонке добавьте первое адресное пространство и единый диапазон адресов подсети. После создания виртуальной сети вернитесь назад и добавьте дополнительные подсети и адресные пространства.
    ...
    ![Колонка "Создать виртуальную сеть"](./media/vpn-gateway-howto-site-to-site-classic-portal/createvnet.png "Колонка "Создать виртуальную сеть"")
5. Убедитесь, что вы используете правильную **подписку** . Подписки можно менять с помощью раскрывающегося списка.
6. Щелкните **Группа ресурсов** и либо выберите существующую, либо создайте новую группу ресурсов. Чтобы создать группу, введите ее имя. Дополнительные сведения о группах ресурсов см. в разделе "Группы ресурсов" [обзора Azure Resource Manager](../azure-resource-manager/resource-group-overview.md#resource-groups).
...
2. В колонке виртуальной сети в разделе **Параметры** щелкните **Адресное пространство**.
3. В колонке "Адресное пространство" щелкните **+ Добавить** и введите дополнительное адресное пространство.
 
## <a name="dns"></a>3. Выбор DNS-сервера
Необязательно настраивать параметры DNS при настройке соединения типа "сеть — сеть". Однако DNS необходима для разрешения имен.

После создания виртуальной сети можно добавить IP-адрес DNS-сервера для обработки разрешения имен. Откройте параметры виртуальной сети, щелкните "DNS-серверы" и добавьте IP-адрес DNS-сервера, который нужно использовать для разрешения имен. Этот параметр не приводит к созданию DNS-сервера. В примере параметров мы используем общедоступный DNS-сервер. Обычно используется частный DNS-сервер. Нужно добавить DNS-сервер, с которым могут взаимодействовать ваши ресурсы.
...
2. В колонке виртуальной сети в колонке **Обзор** в разделе "VPN-подключения" щелкните **Шлюз**, чтобы открыть колонку **Новое VPN-подключение**.
...
3. В колонке **Новое VPN-подключение** выберите **Сеть — сеть**.

    ![Щелкните "Сеть — сеть"](./media/vpn-gateway-howto-site-to-site-classic-portal/site-to-site.png "Сеть — сеть")
4. Щелкните **Локальный сайт > Настройка обязательных параметров**, чтобы открыть колонку **Локальный сайт**. Настройте параметры и нажмите кнопку **ОК**, чтобы сохранить их.
    ...
    - **IP-адрес VPN-шлюза.** Это общедоступный IP-адрес VPN-устройства для локальной сети. Для VPN-устройства требуется общедоступный IP-адрес IPv4. Укажите допустимый общедоступный IP-адрес VPN-устройства, к которому необходимо подключиться. Он не может быть скрыт за NAT и должен быть доступен для Azure.
...
1. В колонке **Новое VPN-подключение** установите флажок **Немедленно создать шлюз**. Появится колонка "Дополнительная настройка шлюза". Если не установить флажок, колонка настройки подсети шлюза не появится.
...
2. Щелкните **Дополнительная настройка шлюза > Subnet, size, and routing type** (Подсеть, размер и тип маршрутизации), чтобы открыть колонку **Настройка шлюза**.
3. В колонке **Настройка шлюза** щелкните **Подсеть > Настройка обязательных параметров**, чтобы открыть колонку **Добавить подсеть**.
...
4. В колонке **Добавить подсеть** добавьте подсеть шлюза. Мы рекомендуем по возможности при добавлении подсети шлюза создать подсеть шлюза с использованием блока CIDR размера /28 или /27. Таким образом вы получаете достаточно IP-адресов для удовлетворения будущих дополнительных требований к конфигурации.  Нажмите кнопку **ОК** , чтобы сохранить настройки.
...
1. Выберите **размер** шлюза. Это SKU шлюза, который будет использоваться для создания шлюза виртуальной сети. На портале SKU по умолчанию — **Базовый**. Дополнительные сведения о номерах SKU шлюзов см. в статье [Сведения о параметрах VPN-шлюза](vpn-gateway-about-vpn-gateway-settings.md#gwsku).
...
4. В нижней части колонки **Новое VPN-подключение** нажмите кнопку **ОК**, чтобы приступить к созданию шлюза виртуальной сети. Это может занять до 45 минут.
...
1. Создайте каталог на компьютере, а затем экспортируйте в него файл конфигурации сети. В этом примере файл конфигурации сети экспортируется в каталог C:\AzureNet.
  ...
  ```
2. Откройте файл конфигурации сети с помощью редактора XML и проверьте значения имен LocalNetworkSite и VirtualNetworkSite. Измените пример с учетом значений. Если указывается имя с пробелами, заключите его в одиночные кавычки.
