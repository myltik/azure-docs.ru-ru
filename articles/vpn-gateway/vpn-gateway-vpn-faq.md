...
- [Несколько сайтов](vpn-gateway-multi-site.md). Это вариация конфигурации "сеть — сеть", которая позволяет подключить несколько локальных сайтов к виртуальной сети.
...
В настоящее время подключения "точка — сеть" доступны только для классической модели развертывания (которую также называют управлением службами). Мы работаем над решением "точка — сеть" для модели развертывания диспетчера ресурсов Azure. Как только оно появится, эта страница будет обновлена.
...
Нет. Поддержка предоставляется только для версий операционной системы Windows, перечисленных выше. Клиент Windows 10 в настоящее время проверяется.
...
Если вы пользуетесь порталом Azure, на странице "Сети" в разделе локальных сетей добавьте все диапазоны, которые нужно отправить через шлюз виртуальной сети.
...
Дополнительную информацию см. в статье [Сведения о VPN-шлюзах](vpn-gateway-about-vpngateways.md).
...
Да, это поддерживается. Дополнительные сведения см. в разделе [Настройка параллельного соединения ExpressRoute и соединения VPN типа "сеть-сеть"](../expressroute/expressroute-coexist.md).
...
Дополнительные сведения о виртуальных сетях см. в статье [часто задаваемые вопросы о виртуальных сетях](../virtual-network/virtual-networks-faq.md).
