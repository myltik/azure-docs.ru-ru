...
>[!NOTE]
>Протокол IKEv2 для подключений типа "точка — сеть в настоящее время находится на этапе предварительной версии и доступен для модели развертывания с помощью Resource Manager.
>
...
>[!NOTE]
>Аутентификация RADIUS при подключениях типа "точка — сеть" сейчас доступна в предварительной версии.
>
...
### <a name="which-gateway-skus-support-p2s-vpn"></a>Какие номера SKU поддерживают VPN-подключение "точка — сеть"?
...
[Настройка подключения типа "точка — сеть" к виртуальной сети с помощью собственной аутентификации Azure на основе сертификата и PowerShell](vpn-gateway-howto-point-to-site-rm-ps.md)
