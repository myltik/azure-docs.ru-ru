...
- Вам потребуется установить командлеты PowerShell диспетчера ресурсов Azure. Дополнительные сведения об установке командлетов PowerShell см. в статье [Установка и настройка Azure PowerShell](../powershell-install-configure.md).
...
## <a name="Verify"></a>Проверка подключения между виртуальными сетями
...
Чтобы проверить VPN-подключение, на портале Azure выберите **Шлюзы виртуальной сети** -> **имя шлюза** -> **Параметры** -> **Подключения**. Выбрав имя подключения, можно просмотреть дополнительные сведения в колонке **Подключение**.


### Проверка подключения с помощью PowerShell

Подключение также можно проверить с помощью командлета *Get-AzureRmVirtualNetworkGatewayConnection -Debug*. Вы можете использовать следующий пример, заменив значения своими. При появлении запроса выберите "A", чтобы запустить команду All (Все).
...
Приведенные ниже инструкции продолжают действия, описанные выше. Чтобы создать и настроить сеть TestVNet1 и VPN-шлюз для нее, необходимо выполнить [шаг 1](#Step1) и [шаг 2](#Step2). Если вы планируете подключать только виртуальные сети в рамках разных подписок, шаги 3 и 4 из предыдущего упражнения можно пропустить и перейти сразу к шагу 5.
...
4. **[Подписка 5]**. Создание подключения между TestVNet1 и TestVNet5
...
5. Проверка подключения
	...
	Выполнив эти действия, вы сможете проверить подключение с помощью методов, описанных в разделе [Проверка подключения между виртуальными сетями](#Verify).
