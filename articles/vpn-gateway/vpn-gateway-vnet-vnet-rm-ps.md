...
description: "В этой статье описывается, как настроить связь между виртуальными сетями, используя подключение \"виртуальная сеть — виртуальная сеть\" и PowerShell."
...
При работе со сложными конфигурациями можно использовать подключение IPsec, а не "виртуальная сеть — виртуальная сеть". Это позволит вам указать дополнительный диапазон адресов для шлюза локальной сети, чтобы маршрутизировать трафик. Если между виртуальными сетями установить подключение IPsec, вам потребуется вручную создать и настроить шлюз локальной сети. Дополнительные сведения см. в статье о [конфигурациях "сеть — сеть"](vpn-gateway-create-site-to-site-rm-powershell.md).
