...
В этой статье описывается, как выполнить сброс настроек VPN-шлюза Azure с помощью командлетов PowerShell. Эти указания применимы к классической модели развертывания. В настоящее время шлюзы виртуальных сетей, которые создаются в модели развертывания Resource Manager, нельзя сбросить.
