...
1. При создании файлов конфигурации VPN-клиента установите для параметра AuthenticationMethod значение EapTls. Создайте конфигурацию VPN-клиента с помощью следующей команды:
  ...
  ```
2. Предыдущая команда возвращает ссылку для скачивания файлов конфигурации клиента. Скопируйте и вставьте ссылку в веб-браузер, чтобы скачать ZIP-файл с именем VpnClientConfiguration.zip. Распакуйте файл. Отобразятся следующие папки:
  ...
  * **Generic**. Эта папка содержит общие сведения для создания собственной конфигурации VPN-клиента. Не используйте эту папку. Эта папка доступна, только если для шлюза настроены протоколы IKEv2 и SSTP+IKEv2. Если настроен только протокол SSTP, эта папка отсутствует.

### <a name="to-retrieve-client-configuration-files"></a>Получение файлов конфигурации клиента

Если вы уже создали файлы конфигурации клиента и вам нужно просто получить их, воспользуйтесь командлетом Get-AzureRmVpnClientConfiguration. Командлет Get-AzureRmVpnClientConfiguration возвращает URL-адрес (ссылку), по которому вы можете скачать файл VpnClientConfiguration.zip. Если изменить конфигурацию VPN-подключения типа "точка — сеть", например изменить тип VPN-протокола или аутентификации, конфигурация не обновится автоматически. Необходимо выполнить командлет New-AzureRmVpnClientConfiguration для повторного создания конфигурации.

Чтобы получить созданные файлы конфигурации клиента, используйте следующую команду:
