...
Для настройки VPN-подключения типа «сеть-сеть» требуется VPN-устройство. Подключения типа «сеть-сеть» можно использовать для создания решения гибридного облака или в случае, когда требуется безопасное подключение между локальной сетью и виртуальной сетью. В этой статье рассматриваются совместимые VPN-устройства и параметры конфигурации.
