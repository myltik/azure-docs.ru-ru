...
* Как развернуть решение SCP в кластере HDInsight Storm
...
SCP предоставляет библиотеки, которые дают возможность с легкостью создавать решения Storm, используя .NET. Кластеры Azure HDInsight Storm включают в себя необходимые серверные компоненты для выполнения созданных вами решений SCP.

Дополнительную информацию о HDInsight Storm см. в разделе [Обзор HDInsight Storm](http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-storm-overview/).
...
Пакет SDK для SCP поставляется с кластером HDInsight Storm. После [создания кластера HDInsight Storm](http://azure.microsoft.com/ru-ru/documentation/articles/hdinsight-storm-getting-started/) выполните следующие действия, чтобы скачать пакет SDK в локальную среду разработки.
...
6. В мастере "Настройка удаленного рабочего стола" введите **имя пользователя** и **пароль** для удаленного рабочего стола. Введите дату окончания срока действия в поле **ИСТЕКАЕТ**, затем щелкните значок флажка.
...
9. Щелкните правой кнопкой мыши папку**SDK** и выберите **Копировать**.

10. В своей локальной среде разработки откройте **обозреватель файлов** и перейдите в расположение, в которое хотите сохранить пакет SDK. Щелкните правовой кнопкой мыши и выберите **Вставить**.

##Создание решения SCP

1. В Visual Studio создайте новый проект для **консольного приложения**. Назовите это решение **WordCount**.

2. В **обозревателе решений** щелкните правой кнопкой мыши **Ссылки**, а затем выберите **Добавить ссылку**.

3. Нажмите кнопку **Обзор** внизу **диспетчера ссылок**, а затем перейдите к папке, в которую вы ранее скачали пакет SDK. Выберите **Microsoft.SCP.dll** и **Microsoft.SCPLogger.dll**, а затем щелкните **Добавить**. Чтобы закрыть окно диспетчера ссылок, нажмите кнопку**ОК**.

###Создание "воронки"

1. В **обозревателе решений** щелкните правой кнопкой мыши **WordCount**, а затем выберите **Добавить > Новый элемент**. Выберите **Класс** и введите имя **WordSpout.cs**. Наконец, нажмите кнопку **Добавить**.
...
1. В **обозревателе решений** щелкните правой кнопкой мыши **WordCount**, а затем выберите **Добавить > Новый элемент**. Выберите **Класс** и введите имя **LocalTest.cs**. Наконец, нажмите кнопку **Добавить**.
...
3. Запустите приложение. Когда оно закончит работу, посмотрите в каталог **WordCount\bin\debug** в своем проекте Visual Studio. Там должен быть файл с именем **spout.txt**, который содержит данные, сформированные во время этого теста "воронкой". Его содержимое должно выглядеть подобно этому тексту:
    ...
    > [AZURE.NOTE] В вышеприведенных строках элемент Data является строкой, которую сформировала "воронка" и которая сохранена как массив байтов. Например, [[97,110,32,97,112,112,108,101,32,97,32,100,97,121,32,107,101,101,112,115,32,116,104,101,32,100,111,99,116,111,114,32,97,119,97,121]] соответствует выражению "an apple a day keeps the doctor away".

###Создание "сит"

1. В **обозревателе решений** щелкните правой кнопкой мыши **WordCount**, а затем выберите **Добавить > Новый элемент**. Выберите **Класс** и введите имя **SplitterBolt.cs**. Наконец, нажмите кнопку **Добавить**. Повторите этот шаг, чтобы добавить класс с именем **CounterBolt.cs**.
...
1. Откройте файл **Program.cs** и замените строку "//Code to run on HDInsight cluster will go here" следующим текстом, а затем выполните повторную сборку проекта.
...
4. Подключитесь к своему кластеру HDInsight Storm с помощью удаленного рабочего стола и скопируйте папку **bin\debug** для локального проекта WordCount в кластер HDInsight Storm. Например, скопируйте его в папку **%storm_home%\examples** и переименуйте в **WordCount**.

3. Также скопируйте файл **WordCount.spec** на сервер HDInsight. Поместите его в каталог **%storm_home%\examples**.

4. В кластере HDInsight Storm используйте значок **командной строки Storm** на рабочем столе, чтобы открыть командную строку, а затем выполните следующую команду, чтобы запустить топологию WordCount.
...
-	**ISCPBolt** - используйте при реализациии нетранзакционных "сит".
-	**ISCPBatchBolt** - используйте при реализациии транзакционных "сит".
...
Когда вы вызываете команду NextTuple(), код может отправить один или несколько кортежей данных. Если данных для отправки не существует, то возвращение метода должно произойти без отправки данных.

Команды Ack() и Fail() будут вызываться, только если в файле спецификации включено подтверждение. Уникальный идентификатор (seqId) используется для определения подтверждения или отклонения принятия кортежа. Если подтверждение включено в нетранзакционной топологии, в "воронке" должны использоваться следующие функции отправки:  
...
> [AZURE.NOTE] NextTuple(), Ack() и Fail() - все вызываются в высоконагруженном цикле в одном потоке. Если больше нет кортежей для передачи, используйте ключевое слово sleep с небольшим значением (например, 10 миллисекунд) для экономии ресурсов ЦП.
...
> [AZURE.NOTE] NextTx(), Ack() и Fail() - все вызываются в высоконагруженном цикле в одном потоке. Если больше нет кортежей для передачи, используйте ключевое слово sleep с небольшим значением (например, 10 миллисекунд) для экономии ресурсов ЦП.
...
При наличии нового кортежа для его обработки будет вызвана функция Execute(). По завершении этой транзакции будет вызвана функция FinishBatch().

Параметр parms для этих функций является пустым объектом Dictionary и зарезервирован для использования в дальнейшем.

> [AZURE.NOTE] "Сита", которые реализуют ISCPBatchBolt, могут получать StormTxAttempt от parms. StormTxAttempt может использоваться для определения, является ли кортеж оригинальным, или это попытка повторной передачи. Этот процесс обычно происходит в подтверждающем "сите". См. пример с **HelloWorldTx**.  
...
* public static ILogger Logger = null; - предназначен для выполнения входа.  

* public static SCPPluginType pluginType; - получает тип подключаемого модуля процесса C#. Если процесс C# запущен в локальном тестовом режиме (без Java), в качестве типа подключаемого модуля устанавливается SCP_NET_LOCAL.
...
* public static Config Config { get; set; } - получает параметры конфигурации из JVM. Параметры передаются из JVM при инициализации дополнительного модуля. Config содержит два словаря.

    * public Dictionary<string, Object> stormConf { get; set; } - содержит параметры, определяемые Storm.

    * public Dictionary<string, Object> pluginConf { get; set; } - содержит параметры, определяемые SCP.

* public static TopologyContext TopologyContext { get; set; } - получает контекст топологии. Он наиболее полезен для компонентов с множественным параллелизмом. В следующем примере показано, как получить доступ к контексту топологии.
...
* public void DeclareComponentSchema(ComponentStreamSchema schema); - объявляет схемы ввода и вывода для потоков. Следующий пример объявляет схему ввода, которая состоит из одного кортежа строк, и схему вывода, которая состоит из кортежа стро и кортежа целых чисел.
...
* public abstract void Emit(List<object> values); - формирует один или несколько кортежей в поток по умолчанию. Следующий пример формирует два кортежа в поток по умолчанию.

        this.ctx.Emit(new Values(word, count));

* public abstract void Emit(string streamId, List<object> values); - формирует один или несколько кортежей в указанный поток. Следующий пример формирует два кортежа в поток с именем mystream.
...
* public abstract void Emit(string streamId, List<object> values, long seqId); - формирует один или несколько кортежей и идентификатор последовательности **из "воронки"** в указанный поток. Следующий пример формирует кортеж и идентификатор последовательности в поток по умолчанию.

        this.ctx.Emit(Constants.DEFAULT_STREAM_ID, new Values(word), lastSeqId);

* public abstract void Emit(string streamId, IEnumerable<SCPTuple> anchors, List<object> values); - формирует один или несколько кортежей и идентификатор последовательности **из "сита"** в указанный поток. Сформированные кортежи привязываются к входящим кортежам, указанным как anchors. Это позволяет выполнять подтверждения в обратном порядке вверх по конвейеру, следуя цепочке входящих/исходящих кортежей. Следующий пример передает кортеж и идентификатор последовательности в поток по умолчанию и привязывает порожденные кортежи к входящим кортежам, которые содержатся в tuple.
...
* public abstract void Fail(SCPTuple tuple); - завершает кортеж ошибкой. Вызывает функцию ISCPSpout.Fail "воронки", которая изначально произвела кортеж.
...
* public static StateStore Get(string storePath, string connStr); - получает StateStore для заданного пути и строки подключения.

* public State Create(); - создает новый объект State в этом экземпляре хранилища состояний.

* public IEnumerable<State> GetUnCommitted(); - получает все объекты State, которые не подтверждены, исключая прерванные состояния.

* public IEnumerable<State> States(); - получает все объекты State в StateStore.


* public T Get<T>(string info = null); - получает объект State или объект Registry.

    * info - получение имени Registry. Используется только при получении объекта Registry.

    * T - тип State или Registry.

* public IEnumerable<Registry> Commited(); - получает объекты Registry, которые содержат подтвержденные State.

* public IEnumerable<Registry> Aborted(); - получает объекты Registry, которые содержат прерванные State.

* public State GetState(long stateId) - получает объект State для указанного идентификатора состояния.

Объект **State** предоставляет следующие методы.

* public void Commit(bool simpleMode = true); - задает состояние объекта State как подтвержденное.
...
* public void Abort(); - задает состояние объекта State как прерванное.
...
* public T GetAttribute<T>(string key); - получает значение атрибута для заданного ключа.
...
* List<SCPTuple> RecvFromMsgQueue(); - получает кортежи из очереди.
...
|**scp-tx-spout**|	exec-name<br> args<br> fields|	Определение транзакционной "воронки". Будет запущено приложение с именем **exec-name** и аргументами **args**.<br><br>**поля** - это поля вывода для "воронки".
|**scp-tx-batch-bolt**|	exec-name<br> args<br> fields| 	Определение транзакционного пакетного "сита". Будет запущено приложение с именем **exec-name** и аргументами **args**.<br><br>Поля - это поля вывода для "сита".
|**scp-tx-commit-bolt**|	exec-name<br>args<br>fields| 	Определение транзакционного подтверждающего "сита". Будет запущено приложение с именем **exec-name** и аргументами args.<br><br>**поля** - это поля вывода для "сита".
...
|**scp-spout**|	exec-name<br>args<br>поля<br>parameters|	Определение нетранзакционной "воронки". Будет запущено приложение с именем **exec-name** и аргументами **args**.<br><br>**поля** - это поля вывода для "воронки".<br><br>**параметры** не являются обязательными и используются для указания некоторых параметров, например - nontransactional.ack.enabled.
|**scp-bolt**|	exec-name<br>args<br>поля<br>parameters|	Определение нетранзакционного "сита". Будет запущено приложение с именем **exec-name** и аргументами **args**.<br><br>**поля** - это поля вывода для "сита".<br><br>**параметры** не являются обязательными и используются для указания некоторых параметров, например - nontransactional.ack.enabled.
...
|Параметр|	Описание
|---------|------------
|**plugin.name**|	Имя EXE-файла для подключаемого модуля на C#.
...
Спецификацию топологии можно отправлять напрямую в кластер Storm, чтобы выполнять с помощью команды **runspec**, которая находится в каталоге **%storm_home%\bin** в кластерах HDInsight Storm.  
...
При вызове Emit() платформа сериализует кортеж в массив байтов и передает его на JVM. Затем Storm передаст этот кортеж целевым объектам. Затем "сита" получат кортеж. Для "сит" C# кортеж получается от JVM и преобразовывается обратно в исходный тип.
...
Объект Context предоставляет метод DeclareComponentSchema, который может использоваться для объявления схемы для сериализации или десериализации.
...
* scp-field-group - использовать настраиваемое объединение.  
* :tx или :non-tx - указывает тип (транзакционный или нетранзакционный).
* [0,1] - использовать набор хэширования идентификаторов полей, начиная с 0.  
...
* **Сериализация и десериализация между Java и C#** - чтобы сериализировать объекты между Java и C#, используйте CustomizedInteropJSONSerializer
    ...
    > [WACOM.NOTE] В настоящее время CustomizedInteropJSONSerializer поддерживает только **"воронки" Java** и **"сита" C#**.
...
* **HelloWorld** - это очень простой пример SCP.Net; он подобен примеру подсчета слов, использованному ранее в этой статье. Он использует нетранзакционную топологию, в которой "воронка" называется **generator**, а два "сита" - **splitter** и **counter**. "Воронка" **generator** случайным образом создает предложения и отправляет их в **splitter**. "Сито" **splitter** разбивает эти предложения на слова и отправляет их на "сито" **counter**. "Сито" **counter** использует словарь для записи частоты употребления каждого слова.

    Для этого примера есть два файла спецификации - **HelloWorld.spec** и **HelloWorld_EnableAck.spec**. При использовании **HelloWorld_EnableAck.spec** пример подтверждает кортежи, которые проходят через "сита", однако**Разделитель** разработан таким образом, чтобы некоторые "сита" случайным образом выдавали ошибку для демонстрации ошибки при обработке нетранзакционных топологий.

* **HelloWorldTx** - пример реализации транзакционной топологии. Он содержит транзакционную "воронку" с именем **Создатель**, пакетное "сито" с именем **partial-count** и подтверждающее "сито" с именем **count-sum**. Также есть три предварительно созданных TXT-файла для использования с этой топологией: **DataSource0.txt**, **DataSource1.txt** и **DataSource2.txt**.

    При каждой транзакции "воронка" в случайном порядке выбирает два файла и отправляет два имени файла в "сито" **partial-count**. "Сито" получает имя файла от пришедшего кортежа данных, затем открывает файл и считывает количество слов в нем. И, наконец, оно отправляет данные с количеством слов на "сито" **count-sum**. "Сито" **count-sum** выводит конечный результат подсчета.  
    ...
    Когда создается экземпляр "сита", он получает попытку транзакции (txAttempt) от входящих параметров:  
    ...
    После того, как вызвана FinishBatch(), к транзакции будет добавлен параметр lastCommittedTxId, если это не была перезапущенная транзакция.  
