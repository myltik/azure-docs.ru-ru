...
> [AZURE.NOTE] При описаний действий в данной статье предполагается, что вы используете клиент OS X, Linux или Unix. Хотя то же самое можно сделать и на клиентах Windows после установки пакета, который предоставляет `ssh` и `ssh-keygen` (например, Git для Windows), на таких клиентах рекомендуется выполнить действия, описанные в статье [Использование SSH с HDInsight (Hadoop) на платформе Linux в ОС Windows](/ru-ru/documentation/articles/hdinsight-hadoop-linux-use-ssh-windows/).
...
* **Кроссплатформенный интерфейс командной строки Azure (xplat-cli)** - для создания кластера используются команды командной строки.

Для каждого из этих способов потребуется **пароль** или **открытый ключ**. Полную информацию о создании кластера HDInsight на основе Linux см. в разделе <a href="/ru-ru/documentation/articles/hdinsight-hadoop-provision-linux-clusters/" target="_blank">"Подготовка кластеров HDInsight на основе Linux"</a>.
...
Вы можете использовать <a href="../xplat-cli/" target="_brad">кроссплатформенный интерфейс командной строки Azure</a>, чтобы создать новый кластер с помощью команды `azure hdinsight cluster create`.

Дополнительную информацию об использовании этой команды см. в разделе <a href="../hdinsight-hadoop-provision-linux-clusters/" target="_blank">"Подготовка кластеров Hadoop на основе Linux в HDInsight с помощью настраиваемых параметров".</a>
...
1. Следующая команда используется для создания туннеля SSH на головном узле кластера.
	...
	* **f** - выполнение в фоновом режиме.
	...
	При настройке кластера с **ключом SSH** можно использовать параметр `-i` и указать путь к закрытому ключу SSH.
...
2. Настройте клиентскую программу, например Firefox, чтобы использовать **localhost:9876** в качестве прокси-сервера **SOCKS v5**. Вот как выглядят параметры Firefox.
	...
	Вы можете проверить, направляется ли трафик через туннель, например, посетив сайт <a href="http://www.whatismyip.com/" target="_blank">http://www.whatismyip.com/</a> с включенными и отключенными параметрами прокси-сервера в Firefox. Если он включен, IP-адрес будет относиться к компьютеру в центре обработки данных Microsoft Azure.

###Расширения браузера

Обычно при настройке браузера для использования туннеля вы не хотите направлять через туннель весь трафик. Расширения браузера, такие как <a href="http://getfoxyproxy.org/" target="_blank">FoxyProxy,</a>  поддерживают сопоставление шаблонов для запросов URL-адресов (только FoxyProxy Standard или FoxyProxy Plus), что позволяет отправлять через туннель только запросы определенных URL-адресов.
...
4. Выберите вкладку **Шаблоны URL**, а затем нажмите кнопку **Добавить**. Используйте следующие параметры, чтобы определить шаблон, а затем нажмите кнопку **ОК**.
	...
	* **Шаблон URL** - **\*headnode\***, определяет шаблон, которому соответствует любой URL-адрес со словом **headnode** в нем.
