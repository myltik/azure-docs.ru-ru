...
### Закрытый сертификат
Закрытые сертификаты также можно передать в учетную запись интеграции. Для этого потребуются выполнить следующее.
1. [Передайте закрытый ключ в хранилище ключей](../key-vault/key-vault-get-started.md "Сведения о хранилище ключей").
2. Создайте закрытый сертификат. Передайте его в учетную запись интеграции.
...
4.  Выберите элемент **Сертификаты**. ![](./media/app-service-logic-enterprise-integration-certificates/certificate-1.png)
5. В открывшейся колонке "Сертификаты" нажмите кнопку **Добавить**. ![](./media/app-service-logic-enterprise-integration-certificates/certificate-2.png)
6. Введите **имя** сертификата, выберите тип сертификата (в этом примере использован закрытый сертификат), затем щелкните значок папки в правой части текстового поля **Сертификат**. Откроется средство выбора файлов, позволяющее найти и выбрать файл сертификата, который следует передать в учетную запись интеграции. Выбрав сертификат, нажмите кнопку **ОК** в средстве выбора файлов. Сертификат будет проверен и передан в учетную запись интеграции. Наконец, в колонке **Добавление сертификата** нажмите кнопку **ОК**. ![](./media/app-service-logic-enterprise-integration-certificates/privatecertificate-1.png)
7. В течение одной минуты появится уведомление о том, что передача сертификата завершена.
8. Выберите элемент **Сертификаты**. Страница будет обновлена, и вы увидите только что добавленный сертификат. ![](./media/app-service-logic-enterprise-integration-certificates/privatecertificate-2.png)
...
- [Создание приложения логики, использующего возможности "бизнес-бизнес"](./app-service-logic-enterprise-integration-b2b.md)
