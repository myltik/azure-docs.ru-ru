...
3.	Настройте соединитель SMTP следующим образом.
	...
	- **Имя** - введите имя соединителя SMTP.
	- **Параметры пакета**.
		...
		- **Включить SSL** — укажите значение true для использования SMTP через защищенный канал SSL/TLS.
4.	Нажмите кнопку "Создать". Будет создан новый соединитель SMTP.
5.	После создания экземпляра приложения API можно будет создать приложение логики в той же группе ресурсов для использования соединителя SMTP. 

## Использование соединителя SMTP в приложении логики ##
После создания приложения API можно будет использовать соединитель SMTP как действие для приложения логики. Для этого необходимо выполнить следующие действия.
