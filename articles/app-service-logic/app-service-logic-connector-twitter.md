...
> [AZURE.IMPORTANT]В настоящее время для создания соединителя Twitter приложение API необходимо регистрировать в Twitter. Бесплатно зарегистрировать приложение можно по адресу [http://apps.twitter.com](http://apps.twitter.com). При создании соединителя потребуется ключ и секрет API Twitter.

Соединитель можно создать в приложении логики или непосредственно в Azure Marketplace. Создание соединителя в Marketplace

1. Создайте бесплатное приложение для Twitter на сайте [http://apps.twitter.com](http://apps.twitter.com)
    * При регистрации приложения можно указать для веб-сайта любой URL-адрес, а поле "URL-адрес обратного вызова" оставить пустым.
2. На начальной панели Azure выберите **Marketplace**.
2. Выполните поиск по запросу «Соединитель Twitter».
3. Выберите "Соединитель Twitter" и нажмите "Создать". Откроется колонка "Создать".
3. Щелкните "Параметры пакета" и вставьте ключ потребителя из приложения Twitter в поле clientId. Вставьте секрет клиента из приложения Twitter в поле clientSecret. ![][10]
4. Введите другие необходимые параметры, такие как имя, служба приложений и группы ресурсов.
5.	Щелкните **Создать**.

> [AZURE.NOTE]Если вы хотите усилить защиту API Twitter, используя URL-адрес перенаправления, изучите инструкции, которые приводятся [здесь](app-service-logic-oauth-security.md).
