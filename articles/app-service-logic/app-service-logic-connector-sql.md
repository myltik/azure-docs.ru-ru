...
3. Триггер будет показан в приложении логики как настроенный. Будут отображены результаты триггера, которые можно использовать в качестве входных данных в последующих действиях: <br/> ![][7]
...
1. После триггера (или выбора параметра "Запустить эту логику вручную") добавьте соединитель SQL, созданный из коллекции. Выберите одно из действий вставки, например *Вставить в TempEmployeeDetails (JSON)*: <br/> ![][8]
...
--- | --- | ---
Выражение WHERE | <ul><li>Операторы: AND, OR, =, <>, <, <=, >, >= и LIKE</li><li>Можно объединять несколько условий с помощью скобок «(» и «)»</li><li>Строковые литералы, значения даты и времени (заключаются в одинарные кавычки), числа (должны содержать только цифры)</li><li>Необходимо строго придерживаться двоичного формата выражения, в виде ((операнд оператор операнд) AND/OR (операнд оператор операнд))*</li></ul> | <ul><li>Операторы: Between, IN</li><li>Все встроенные функции, такие как ADD(), MAX(), NOW(), POWER() и т. д.</li><li>Математические операторы, такие как *, -, + и т. д.</li><li>Объединение строк с помощью +.</li><li>Все соединения</li><li>IS NULL и IS NOT Null</li><li>Любые числа с нецифровыми символами, например шестнадцатеричные числа</li></ul> Поля (в запросе Select) |<ul><li>Допустимые имена столбцов, разделяемые запятыми. Префиксы имен таблиц не допускаются (этот соединитель работает над одной таблицей одновременно).</li><li>Имена можно экранировать при помощи квадратных скобок «[» и «]»</li></ul> | <ul><li>Ключевые слова, например TOP, DISTINCT и т. д.</li><li>Псевдонимы, например Street + City + Zip AS Address</li><li>Все встроенные функции, такие как ADD(), MAX(), NOW(), POWER() и т. д.</li><li>Математические операторы, такие как *, -, + и т. д.</li><li>Объединение строк с помощью +</li></ul>
