...
В этой статье описан общий шаблон интеграции: шаблона: «одностороннее преобразование сообщений» или VETR. Шаблон VETR преобразует данные между исходной сущностью и целевой сущностью. Обычно источник и назначение представляют собой источники данных.
...
2. Выберите действие **Отправить сообщение** и установите в поле **Сообщение** для действия значение _actions('transformservice').outputs.OutputXml_.
...
2. В поле **Содержимое ответа** задайте значение *Конвейерная обработка завершена*; в поле **Код состояния ответа** задайте значение *200*, чтобы указать код HTTP 200 ОК, и в качестве **условия** задайте выражение ```@equals(actions('servicebusconnector').status,'Succeeded')```.
