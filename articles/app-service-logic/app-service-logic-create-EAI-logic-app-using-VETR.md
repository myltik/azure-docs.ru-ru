...
2. Оставьте в параметре **Отправлять ответ автоматически?** значение false. Настройте действие триггера, задав для параметра _Метод HTTP_ значение _POST_, а для параметра _Относительный URL-адрес_ — значение _/OneWayPipeline_.  
	...
	![Триггер HTTP][2]
3. Щелкните значок с зеленой галочкой.
...
3. Введите в поле **Тип содержимого** значение application/XML.
...
3. Укажите в поле **Код состояния ответа** значение *200*, чтобы задать для HTTP значение 200, и нажмите кнопку «OK».
4. Щелкните справа вверху раскрывающееся меню и выберите пункт **Добавить условие для выполнения**. Укажите в качестве условия следующее выражение: ```@equals(actions('azureservicebusconnector').status,'Succeeded')``` <br/>
