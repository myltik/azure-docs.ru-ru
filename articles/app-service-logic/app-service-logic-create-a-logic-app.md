...
В этой статье показывается, как всего за несколько минут создать [приложение логики службы приложений](app-service-logic-what-are-logic-apps.md). Мы рассмотрим рабочий процесс, который позволяет получать набор интересующих твитов в папку Dropbox.
...
Сначала необходимо создать два соединителя: [**соединитель Dropbox**](app-service-logic-connector-dropbox.md) и [**соединитель Twitter**](app-service-logic-connector-twitter.md). Чтобы создать их, выполните следующие действия.

0. Войдите на портал Azure.

1. Щелкните [**Marketplace**](https://portal.azure.com/#blade/HubsExtension/GalleryFeaturedMenuItemBlade/selectedMenuItemId/apiapps) на начальном экране и найдите **Twitter** (или [щелкните здесь](https://portal.azure.com/#create/microsoft_com.TwitterConnector.0.2.2)).

2. Выберите соединитель Twitter и нажмите кнопку "Создать". Откроется представление, в котором можно задать все параметры. Вы можете оставить имя **Соединитель Twitter**.

3. В поле **Создать новый план обслуживания приложения** введите имя плана.

	>[AZURE.NOTE]В этом разделе предполагается, что вы создаете новый план обслуживания приложения. Если вы используете существующий план обслуживания приложения, нажмите **Выбрать существующий**, выберите существующий план и перейдите к последнему этапу в этом разделе. Вам потребуется план для размещения всех своих приложений.
...
7. Выберите **расположение** для запуска приложения логики.
	...
	![Представление создания приложения API](./media/app-service-logic-create-a-logic-app/gallery.png)

8. Щелкните **Создать**. Этап подготовки может занять одну-две минуты.

9. Теперь повторите эту процедуру для [Dropbox](https://portal.azure.com/#create/microsoft_com.DropboxConnector.0.2.2).
...
1. Нажмите кнопку **+ Создать** в нижнем левом углу экрана, разверните узел **Мобильные и веб-приложения**, а затем щелкните элемент **Приложение логики**.
 	...
 	Откроется представление «Создание приложения логики», где можно указать некоторые основные параметры для начала работы.

	![Представление создания приложения логики](./media/app-service-logic-create-a-logic-app/createlogicapp.png)
...
Триггеры — это то, что делает ваше приложение логики. Затем вы добавите триггер повторения, который будет запускать ваш рабочий процесс по заданному расписанию.

1. В представлении **Создание приложения логики** щелкните пункт **Триггеры и действия**.
...
2. В верхнем разделе щелкните **Повторение**.
...
Менее чем за 5 минут вы смогли создать простое приложение логики, работающее в облаке. Дополнительные сведения об использовании функций приложений логики см. в статье [Использование функций приложения логики]. Дополнительные сведения об определениях приложений логики см. в статье [Создание определений приложений логики](app-service-logic-author-definitions.md).
