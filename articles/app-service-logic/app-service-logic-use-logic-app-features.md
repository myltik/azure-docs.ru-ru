...
2. Прокрутите до действия `twitterconnector`, найдите значение для запроса и замените его на `#@{parameters('topic')}`. Также можно воспользоваться функцией **concat** для соединения двух и более строк. Например, результат использования функции `@concat('#',parameters('topic'))` идентичен описанным выше действиям.
...
Дополнительные сведения об определениях приложений логики см. в статье [Создание определений приложений логики](app-service-logic-author-definitions.md).
