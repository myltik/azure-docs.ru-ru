...
3.	Настройте соединитель SFTP следующим образом.
	...
	- **Имя** — задайте имя для соединителя SFTP. 
	- **Параметры пакета**. 
		- **Адрес сервера** — укажите имя или IP-адрес сервера. 
		- **Принимать любой ключ узла сервера по SSH** — указывает, следует ли принимать любой открытый ключ узла по SSH от сервера. Если задано значение FALSE, ключ узла будет сопоставлен с ключом, указанным в свойстве «Признак ключа узла SSH». 
		- **Ключ узла сервера SSH** — укажите признак открытого ключа узла для сервера SSH (*необязательно*). **Корневая папка** — укажите путь к корневой папке. Если значение пустое, то значением по умолчанию является корневой каталог. 
		- **Ключ шифрования** — укажите ключ шифрования (*необязательно*). 
