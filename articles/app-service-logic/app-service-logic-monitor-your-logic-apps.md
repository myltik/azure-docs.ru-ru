...
## Включение диагностики Azure

Вы можете включить сведения диагностики, чтобы получать и сохранять данные выполнения рабочих процессов. В колонке "Приложения логики" прокрутите окно вниз до диаграммы **Мониторинг** и выберите **Щелкнуть здесь, чтобы включить диагностику**. Затем настройте диагностику для учетной записи, которая хранится в том же регионе, что и приложение логики, и подпишитесь из приложения логики на **Журналы** или **Метрики**. **Журналы** будут отправлять событие при каждом запуске и действии либо во время начала или завершения события. **Метрики** отображают объединенные данные о количестве запусков за определенный период времени.
