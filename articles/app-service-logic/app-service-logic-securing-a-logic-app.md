---
title: "Обеспечение безопасности приложения логики Azure | Документация Майкрософт"
description: "Обеспечение безопасности входных данных приложения логики Azure, доступа к нему и функций управления"
services: logic-apps
...
# <a name="securing-a-logic-app"></a>Обеспечение безопасности приложения логики
...
Как часть URL-адреса каждой конечной точки запроса для приложения логики используется [подписанный URL-адрес](../storage/storage-dotnet-shared-access-signature-part-1.md) (SAS).  Каждый URL-адрес содержит параметр запроса `sp`, `sv` и `sig`.  Разрешения указываются с помощью параметра `sp` и соответствуют разрешенным методам HTTP. `sv` — это версия, используемая для создания, а `sig` используется для проверки подлинности доступа к активации.  URL-адрес создается с помощью алгоритма SHA256 с секретным ключом для всех URL-путей и свойств.  Секретный ключ хранится как часть приложения логики в зашифрованном виде, никогда не раскрывается и не публикуется.  Приложение логики авторизует только те триггеры, которые содержат действительную подпись, созданную с помощью секретного ключа.

#### <a name="regenerate-access-keys"></a>Повторное создание ключей доступа

Вы можете в любое время повторно создать новый ключ безопасности с помощью REST API или портала Azure.  Все текущие URL-адреса, созданные ранее с помощью старого ключа, станут недействительны, и их нельзя будет использовать для запуска приложения логики.
...
> [!NOTE]
> Приложение логики с триггером запросов может по-прежнему выполняться с помощью REST API или управления API `/triggers/{triggerName}/run` независимо от IP-адреса.  Для этого потребуется проверка подлинности с помощью Azure REST API, а все события отобразятся в журнале аудита Azure.  Настройте политики контроля доступа соответствующим образом.

#### <a name="setting-ip-ranges-on-the-resource-definition"></a>Задание диапазонов IP-адресов в определении ресурсов

При использовании [шаблона развертывания](app-service-logic-create-deploy-template.md) для автоматизации развертывания можно настроить параметры диапазона IP-адресов в шаблоне ресурсов.  
...
## <a name="secure-access-to-manage-or-edit-a-logic-app"></a>Защита доступа к управлению приложением логики или изменению его
...
Кроме того, можно также использовать [блокировку ресурсов Azure](../azure-resource-manager/resource-group-lock-resources.md) для предотвращения изменения или удаления приложения логики.  Это позволяет предотвратить непреднамеренное изменение или удаление рабочих ресурсов.
...
> Заголовки авторизации никогда не видны во входных и выходных данных. При использовании секрета их нельзя будет извлечь.

#### <a name="resource-deployment-template-with-secrets"></a>Шаблон развертывания ресурсов с использованием секретов

Ниже приведен пример развертывания, который во время выполнения ссылается на безопасный параметр `secret`.  В отдельном файле параметров можно указать значения среды для `secret` или использовать [хранилище ключей Azure Resource Managere](../azure-resource-manager/resource-manager-keyvault-parameter.md) для получения секретов во время развертывания.
...
При работе с HTTP, HTTP + Swagger (открытый API) или с действием webhook можно добавить проверку подлинности отправляемого запроса.  Это может быть обычная проверка подлинности, проверка подлинности сертификата или проверка подлинности Azure Active Directory.  Дополнительные сведения о настройке проверки подлинности см. [в этой статье](../connectors/connectors-native-http.md#authentication).

### <a name="restricting-access-to-logic-app-ip-addresses"></a>Разрешение доступа для определенных IP-адресов приложения логики

Все вызовы из приложений логики поступают из определенного набора IP-адресов для каждого региона.  Можно добавить дополнительные фильтры, чтобы принимать запросы только из указанных IP-адресов.  Список этих IP-адресов можно найти [в этой статье](app-service-logic-limits-and-config.md#configuration).
...
[Обработка исключений](app-service-logic-exception-handling.md)  
[См. статью Мониторинг приложений логики.](app-service-logic-monitor-your-logic-apps.md)  
[Диагностика сбоев приложений логики](app-service-logic-diagnosing-failures.md)  
