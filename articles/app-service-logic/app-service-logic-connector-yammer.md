...
## Использование соединителя Yammer для отправки сообщения

6.	Соединитель Yammer также можно использовать в качестве действия в приложении логики. Сначала необходимо задать для приложения триггер или установить флажок «Запускать эту логику вручную», как показано ниже. Добавьте соединитель Yammer, выполните авторизацию и выберите действие «Отправить сообщение». Настройте входные свойства для действия "Отправить сообщение" следующим образом.

	- **Текст сообщения** — текстовое содержимое отправляемого сообщения.
	- **Идентификатор группы** — укажите идентификатор группы, в которую следует отправить новое сообщение. Если не указан идентификатор группы, сообщение будет отправлено в канал всей компании. Идентификатор группы может быть извлечен из URL-адреса группы в Yammer. Например, идентификатор группы в следующем URL-адресе — 5453203: https://www.yammer.com/microsoft.com/#/threads/inGroup?type=in_group&feedId=5453203
	- 	**Отметить пользователей** — массив, содержащий имена пользователей сети, которые должны быть отмечены в сообщении. 
