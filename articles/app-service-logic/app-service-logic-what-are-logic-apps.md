...
Приложения логики позволяют разработчикам проектировать рабочие процессы, которые запускаются триггером, а затем выполняют ряд действий. Каждый шаг вызывает приложение API службы приложений, при этом выполняется проверка подлинности и рекомендации, такие как создание контрольных точек и устойчивое выполнение.
...
Более того, с нашими [приложениями API BizTalk][biztalk] вы можете выполнять масштабирование до сложных сценариев интеграции, используя возможности [обработчика правил][rules], [управления торговыми партнерами][tpm] и многое другое.
...
- **Интегрированная расширяемость** — не удается найти нужный соединитель? Приложения логики являются частью пакета службы приложений и предназначены для работы с приложениями API; вы можете легко создать свое собственное приложение API для использования в качестве соединителя. Постройте новое приложение только для себя или предложите его на рынке и получайте прибыль.
