...
Первый шаг — это добавить триггер в определение приложения логики, способное принимать входящие запросы. Принимать запросы могут 3 типа триггеров — * manual * apiConnectionWebhook * httpWebhook
...
Для получения этой конечной точки в пользовательском интерфейсе используется следующий вызов:
...
Это — небольшое отличие от версии *2014-12-01-preview*, где бы доступ к телу прослушивателя HTTP осуществлялся с помощью функции вида `@triggerOutputs().body.Content`.
...
Эти функции доступны в **управлении API**: * [Изменение метода запроса](https://msdn.microsoft.com/library/azure/dn894085.aspx#SetRequestMethod) * [Изменение сегментов URL-адреса запроса](https://msdn.microsoft.com/library/azure/7406a8ce-5f9c-4fae-9b0f-e574befb2ee9#RewriteURL) * Настройка доменов управления API на вкладке **Настройка** классического портала Azure * Настройка политики для контроля базовой проверки подлинности (**требуется ссылка**)

## Отличия от версии 2014-12-01-preview

| 2014-12-01-preview | 2015-08-01-preview |
|---------------------|--------------------|
| Щелкните приложение API **Прослушиватель HTTP** | Щелкните **Вызов вручную** (приложение API не требуется) |
| Параметр прослушивателя HTTP "*Отправляет ответ автоматически*" | Включенное или не включение действие **response** в определении рабочего процесса |
