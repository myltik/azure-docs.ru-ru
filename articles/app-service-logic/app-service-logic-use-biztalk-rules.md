...
1. Запустите приложение API "Правила BizTalk" с портала, последовательно щелкнув "Обзор -> Приложения API -> <Правила приложения API>". Откроется панель мониторинга приложения API «Правила», аналогичная представленной ниже.
...
7. 	is\_greater\_than (больше чем)
8. 	is\_greater\_than\_equal\_to (больше или равно)
9. 	is\_in (в диапазоне)
10. is\_less\_than (меньше чем)
11. is\_less\_than\_equal\_to (меньше или равно)
12. is\_not\_in (не в диапазоне)
...
Правила BizTalk поддерживают явное прямое построение цепочки, поэтому, если пользователям требуется повторно оценить правила в ответ на определенные действия, они могут активировать это с помощью определенных ключевых слов. Поддерживаются следующие ключевые слова.
   1.	Update <словарное определение> — это ключевое слово выполняет переоценку всех правил, использующих в своих условиях указанное определение словаря.
