---
title: Управление соединителями и приложениями API и их мониторинг в службе приложений Azure | Microsoft Docs
...
> [!NOTE]
> Диспетчер локальных подключений необходим только для подключения к локальному ресурсу за брандмауэром. Если не вы подключены к локальной системе, диспетчер гибридного подключения может не отображаться в колонке соединителя.
> 
> 
...
> [!IMPORTANT]
> Открыв свойства приложения API, в разделе **Основные компоненты** вы увидите ссылки **Узел** и **Шлюз**, с помощью которых можно открыть новые колонки.
> 
> ![][host]
> 
...
> 
> [!NOTE]
> Чтобы начать работу с Logic Apps, не создавая учетную запись Azure, перейдите на [эту страницу](https://tryappservice.azure.com/?appservice=logic). Там вы сможете создать простое кратковременное приложение логики. Не требуется никаких кредитных карт и обязательств.
> 
> 
