...
4.	Создание нового приложения логики
5.	Открыть раздел "Триггеры и действия", чтобы открыть конструктор приложения логики и настроить поток. Прослушиватель HTTP присутствует в коллекции. Выберите его.
6.	Теперь можно задать метод HTTP и относительный URL-адрес, в котором прослушиватель должен инициировать поток: ![][4] ![][5]

7.	Чтобы получить полный URI, дважды щелкните прослушиватель HTTP, чтобы просмотреть его параметры конфигурации и скопировать URL-адрес для "узла" вашего приложения API: ![][6]
8.	Теперь вы можете использовать данные, полученные в запросе HTTP, в других действиях в потоке следующим образом: ![][7] ![][8]
