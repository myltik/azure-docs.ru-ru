...
1. Условием отбора для действия **Фильтровать массив** является любой элемент `@result()` с состоянием `Failed`.  Таким образом, массив результатов всех действий в `My_Scope` будет отфильтрован и отобразятся результаты только тех действий, которые завершились сбоем.
1. Затем выполняется действие **For Each** для данных, полученных в результате выполнения действия **Фильтровать массив**.  При этом выполняется действие для результатов *каждого* действия со сбоем, отфильтрованных на предыдущем шаге.
