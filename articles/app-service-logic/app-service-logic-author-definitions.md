...
Затем в дочернем приложении логику вы используете функцию [`triggerBody()`](https://msdn.microsoft.com/library/azure/dn948512.aspx#triggerBody) для получения значений, которые были переданы в дочерний рабочий процесс. Затем вы заполните выходные данные данными, которые необходимо вернуть в родительский поток.
...
Обычно необходима возможность записать *шаги по исправлению* — определенную логику, которая выполняется, если, **и только если**, один или несколько из вызовов не удалось выполнить. В этом примере мы получаем данные из различных мест, но если вызов завершается неудачно, я хочу отправить (POST) сообщение куда-то, чтобы можно было отследить этот сбой позже.
...
## Два (или более) шагов, выполняемых параллельно

Для выполнения нескольких действий одновременно, а не последовательно, необходимо удалить условие `dependsOn`, которое связывает эти два действия. После удаления зависимости действия автоматически будет выполняться параллельно, если только им не требуются данные друг от друга.
...
Как видно в приведенном выше примере, branch1 и branch2 зависят только от содержимого из readData. В результате обе этих ветви будут выполняться одновременно:
...
Ваша стратегия при этом зависит от того, обрабатываете вы один элемент или коллекцию элементов. В случае одного элемента вы захотите использовать функцию [`coalesce()`](https://msdn.microsoft.com/library/azure/dn948512.aspx#coalesce):
...
Кроме того, если первые две ветви работают со списком заказов, например, может потребоваться использовать функцию [`union()`](https://msdn.microsoft.com/library/azure/dn948512.aspx#union) для объединения данных из обеих ветвей.
...
1. Получите [`length()`](https://msdn.microsoft.com/library/azure/dn948512.aspx#length) названия заказа, будет возвращено обще число знаков.
...
В этом примере производится извлечение `startTime` из предыдущего шага. Затем мы получаем текущее время и вычитаем одну секунду: [`addseconds(..., -1)`](https://msdn.microsoft.com/library/azure/dn948512.aspx#addseconds) (например, можно использовать другие единицы времени: `minutes` или `hours`). Наконец, мы можем сравнить эти два значения. Если первое меньше второго, то это значит, что с момента первоначального размещения заказа прошло больше секунды.

Также обратите внимание, что можно использовать модули форматирования строк для форматирования дат: в строке запроса я использую [`utcnow('r')`](https://msdn.microsoft.com/library/azure/dn948512.aspx#utcnow), чтобы получить RFC1123. Все форматирование дат [задокументировано на сайте MSDN](https://msdn.microsoft.com/library/azure/dn948512.aspx#utcnow).
...
Нередко жизненный цикл развертывания включает в себя среду разработки, промежуточную среду и рабочую среду. Во всех них может использоваться одно и то же определение, например, но разные базы данных. Аналогично, вы можете использовать одно определение в разных регионах для обеспечения высокой доступности, но хотите, чтобы каждый экземпляр приложения логики взаимодействовал базой данных своего региона.
...
Затем в каждой среде можно предоставить отличное значение для параметра `connection`. В [документации по REST API](https://msdn.microsoft.com/library/azure/dn948513.aspx) описаны все параметры для создания приложений логики и управления ими.
