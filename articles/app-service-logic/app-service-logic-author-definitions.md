...
Внутри действия можно использовать функцию [`item()`](https://msdn.microsoft.com/library/azure/dn948512.aspx#item). В этом примере мне было необходимо получить заголовок `location`, поэтому мне пришлось снова использовать `@item().outputs.headers` для получения выходных данных выполнения второго действия, итерацию которых мы теперь осуществляем.
