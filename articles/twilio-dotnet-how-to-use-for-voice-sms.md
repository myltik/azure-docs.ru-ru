<properties 
	pageTitle="Использование Twilio для поддержки голосовых вызовов и SMS (.NET) | Microsoft Azure" 
...
Приложение Azure, на котором размещается приложение с поддержкой Twilio, ничем не отличается от любого другого приложения Azure. Добавьте библиотеку Twilio для .NET и настройте роль так, чтобы она могла использовать такие библиотеки. Сведения о создании начального проекта Azure см. в разделе [Создание проекта Azure в Visual Studio][vs_project].
...
В Twilio есть пять библиотек для разработчиков .NET: Библиотека|Описание ---|--- Twilio.API| Основная библиотека Twilio, которая позволяет оформить REST API Twilio в виде библиотеки, доступной в .NET. Эта библиотека доступна для .NET, Silverlight и Windows Phone 7. Twilio.TwiML|Позволяет создавать разметку TwiML так, чтобы ее можно было использовать в .NET. Twilio.MVC|Предназначена для разработчиков, использующих MVC ASP.NET. Включает в себя TwilioController, TwiML ActionResult и атрибут проверки запроса. Twilio.WebMatrix|Предназначена для разработчиков, использующих бесплатное средство разработки WebMatrix корпорации Майкрософт. Содержит вспомогательные приложения по синтаксису Razor для выполнения различных действий Twilio. Twilio.Client.Capability|Содержит генератор маркеров Capability, предназначенный для использования с Twilio Client JavaScript SDK.
