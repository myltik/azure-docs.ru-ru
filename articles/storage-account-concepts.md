<properties 
	pageTitle="Основные понятия для учетных записей хранения | Azure" 
	description="Узнайте об основных понятиях для учетных записей хранения." 
	headerExpose="" 
	footerExpose="" 
	services="storage" 
	authors="tamram" 
...
Ваша учетная запись хранения Azure - это уникальная безопасная учетная запись, которая предоставляет права доступа к службам хранилища Azure: - службам BLOB-объектов, очередей, таблиц и файлов. Ваши данные защищены на уровне учетной записи хранения, и по умолчанию только у вас есть к ним доступ. 
...
3. В поле **URL-адрес** введите имя своей учетной записи хранения. Информацию об использовании имени для адресации объектов, которые содержатся в службе хранилища Azure, см. в разделе [Конечные точки учетной записи хранения](#account-endpoints) ниже.
...
Для своей учетной записи хранения вы можете настроить имя домена. Для получения дополнительной информации см. раздел [Настройка пользовательского имени домена для данных больших двоичных объектов в учетной записи хранения](http://azure.microsoft.com/ru-ru/documentation/articles/storage-custom-domain-name/).
...
1. На [портале управленияh](ttp://manage.windowsazure.com) щелкните **Хранилище**.
