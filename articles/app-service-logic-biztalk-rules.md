...
1. Последовательно выберите в меню пункты Создать -> Azure Marketplace -> Приложения API -> Правила Biz Talk -> Создать
![Alt text][3]
...
Если в качестве типа словаря выбран XML, необходимо указать следующие входные данные.  
  а.	Схема - если щелкнуть это свойство, откроется новая колонка, в которой пользователь может выбрать одну из списка уже отправленных схем или отправить новую схему.   
  б.	XPATH - это свойство разблокируется только после выбора схемы на предыдущем шаге. Если его щелкнуть, откроется выбранная схема, и пользователь может выбрать в ней узел, для которого требуется создать определение словаря.  
  в.	ФАКТ - это свойство определяет, какие объекты данных должны загружаться в обработчик правил для обработки. Это дополнительное свойство, и по умолчанию в нем устанавливается родительский объект выбранного XPATH. Свойство ФАКТ имеет особенно важное значение в сценариях с использованием цепочки и коллекции.
