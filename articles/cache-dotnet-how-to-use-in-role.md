...
Это руководство показывает, как приступить к работе с **кэшем роли для кэша Azure**. Примеры написаны на языке C# и используют интерфейс .NET API. Сценарии включают **настройки кластера кэша**, **настройку клиентов кэша**, **добавления и удаления объектов из кэша, хранение состояния сеанса ASP.NET в кэше** и **включение кэширования вывода страниц ASP.NET с помощью кэша**. За дополнительной информацией об использовании кэша роли обратитесь к разделу [Дальнейшие действия][].
...
-	**Выделенная роль** для кэширования - экземпляры роли используются исключительно для кэширования.
...
Чтобы настроить кластер кэша **Совмещенная роль**, выберите роль, в которой вы хотите разместить кластер кэша. Щелкните правой кнопкой мыши свойства в **Обозревателе решений** и выберите команду **Свойства**.
...
>Если элемент **Кэш роли** не отображается в списке, введите **WindowsAzure.Caching** в текстовом поле **поиск в Интернете** и выберите его из результатов.
...
Пакет NuGet добавляет перечисленные ниже элементы конфигурации в файл web.config или app.config вашей роли. Разделы **dataCacheClients** и **cacheDiagnostics** добавляются под элемент **configSections**. При отсутствии элемента **configSections** он создается как дочерний объект элемента **configuration**.
...
Пакет NuGet также добавляет параметр **ClientDiagnosticLevel** в **ConfigurationSettings** роли клиента кэша в ServiceConfiguration.cscfg. Ниже приведен пример раздела **WebRole1** из файла ServiceConfiguration.cscfg с **ClientDiagnosticLevel**, равным 1, что соответствует значению по умолчанию **ClientDiagnosticLevel**.
...
> ErrorCode:SubStatus: Выполняется попытка создать объект с ключом, который уже существует в кэше. Кэш принимает только уникальные значения ключа для объектов.
...
-   Узнайте о том, как перейти на использование кэша роли: [Перенос в кэш роли][]
-   Ознакомьтесь с примерами: [Примеры кэша роли][]
-	Посмотрите о кэшах роли видео [Максимальная производительность. ускорение работы приложений облачных служб с помощью Windows Azure Caching][] с TechEd 2013
...
[Максимальная производительность. ускорение работы приложений облачных служб с помощью Windows Azure Caching]: http://channel9.msdn.com/Events/TechEd/NorthAmerica/2013/WAD-B326#fbid=kmrzkRxQ6gU
