<properties linkid="Contact - Support" urlDisplayName="Кэширование" pageTitle="Практическое руководство по использованию кэша роли (.NET) — руководство по возможностям Azure" metaKeywords="Кэш Azure, кэширование Azure, кэш Azure, кэширование Azure, состояние сеанса хранилища Azure, кэш Azure для .NET, кэш Azure для C#" description="Сведения об использовании кэша роли Azure. Примеры написаны на коде C# и используют .NET API." metaCanonical="" services="cache" documentationCenter=".NET" title="Практическое руководство по использованию кэша роли для кэша Azure" authors=""  solutions="" writer="" manager="" editor=""  />
...
# Практическое руководство по использованию кэша роли для Azure

В этом руководстве показано, как начать использовать 
**кэш роли для кэша Azure**. Примеры написаны на языке C\# и используют интерфейс .NET API. Рассматриваются разнообразные сценарии, включая **настройку кластера кэша**, **настройку клиентов кэша**, **добавление и удаление
объектов из кэша, хранение состояния сеанса ASP.NET в кэше**
и **включение кэширования вывода страниц ASP.NET с помощью кэша**. Дополнительные
сведения об использовании кэша роли см. в разделе [Дальнейшие действия][].
...
Кэш роли обеспечивает уровень кэширования в приложениях Azure. Кэширование повышает производительность путем временного хранения информации в памяти из других источников — баз данных и может снизить затраты, связанные с транзакциями баз данных в облаке. Кэш роли включает следующие возможности:
...
NuGet-пакет выполняет несколько задач: добавление требуемой конфигурации в файл конфигурации роли, добавление параметра уровня диагностики клиента кэша в файл ServiceConfiguration.cscfg приложения Azure, а также добавление ссылок на требуемые сборки.

>Для веб-ролей ASP.NET пакет NuGet для Caching также добавляет два закомментированных раздела в файл web.config. Первый раздел позволяет сохранить состояние сеанса в кэше, а второй раздел включает кэширование вывода страниц ASP.NET. Дополнительные сведения см. в разделах [Практическое руководство. Хранение состояния сеанса ASP.NET в кэше] и [Практическое руководство. Хранение кэширования выводимых данных страниц ASP.NET в кэше][].

Пакет NuGet добавляет перечисленные ниже элементы конфигурации в файл web.config или app.config вашей роли. Разделы **dataCacheClients** и **cacheDiagnostics** добавляются под элементом **configSections**. При отсутствии элемента **configSections** он создается как дочерний объект элемента **configuration**.
...
>Если **[имя роли кластера кэша]** не заменяется именем роли, в которой размещен кластер кэша, выдается **TargetInvocationException** при обращении к кэшу с внутренним **DatacacheException** с сообщением "Такой роли не существует".
...
Чтобы работать с кэшем программными средствами, требуется ссылка на кэш. Добавьте следующие данные в начало любого файла, из которого планируется использовать кэш роли:

    using Microsoft.ApplicationServer.Caching;

>Если Visual Studio не распознает типы в операторе using даже после установки пакета кэша NuGet для Caching, который добавляет необходимые ссылки, убедитесь, что целевым профилем проекта является платформа .NET Framework 4.0 или более поздней версии, а также не забудьте выбрать один из профилей, в котором не указан **Клиентский профиль**. Инструкции по настройке клиентов кэша см. в разделе [Настройка клиентов кэша][].
...
Если объект с тем же ключом уже находится в кэше, возникает исключение
**DataCacheException** со следующим сообщением:
...
Чтобы просмотреть оставшееся время ожидания для элемента в кэше, можно использовать метод
**GetCacheItem** для получения объекта **DataCacheItem**,
...
Поставщик состояний сеансов для кэша роли представляет собой
...
>Если ваш файл web.config не содержит этот закомментированный раздел после установки пакета кэша NuGet для Caching, убедитесь, что установлена самая новая версия диспетчера пакета (см. раздел [Установка диспетчера пакета NuGet][]), после чего удалите и переустановите этот пакет.
...
Дополнительные сведения об использовании поставщика состояний сеансов для службы Caching см. в разделе [Поставщик состояний сеансов для кэша роли][]

<a name="store-page"></a>
## Практическое руководство. Хранение кэширования выводимых данных страниц ASP.NET в кэше

Поставщик кэша вывода для кэша роли представляет собой механизм внепроцессного хранения для выходных данных кэширования. Эти данные предназначены специально для полных HTTP-ответов
...
Вы изучили основные сведения о кэше роли. Дополнительные сведения о более сложных задачах кэширования можно найти по следующим ниже ссылкам.

-   См. справочник MSDN: [Кэш роли][]
-   Узнайте о том, как выполнить перенос данных в кэш роли: [Перенос в кэш роли][]
-   Ознакомьтесь с примерами: [Примеры для кэша роли][]
-	Посмотрите видео [Максимальная производительность: как ускорить работу приложений облачных служб с помощью кэширования Azure][] от 2013 TechEd в кэше роли
...
[Приступая к работе со службой кэша роли]: #getting-started-cache-service
[Подготовка проекта Visual Studio для использования кэша роли]: #prepare-vs
[Настройка приложения для использования кэширования]: #configure-app
[Приступая к работе со службой кэша (предварительная версия)]: #getting-started-cache-role-instance
[Настройка кластера кэша]: #enable-caching
[Настройка нужного размера кэша]: #cache-size
...
[Примеры для кэша роли]: http://msdn.microsoft.com/ru-ru/library/jj189876.aspx
...
[Максимальная производительность: как ускорить работу приложений облачных служб с помощью кэширования Azure]: http://channel9.msdn.com/Events/TechEd/NorthAmerica/2013/WAD-B326#fbid=kmrzkRxQ6gU
