...
Это руководство показывает, как приступить к работе с **кэшем роли для кэша Azure**. Примеры написаны на C# и используют .NET API. Рассматриваются разнообразные сценарии, включая **настройку кластера кэша**, **настройку клиентов кэша**, **добавление и удаление объектов из кэша, хранение состояния сеанса ASP.NET в кэше** и **включение кэширования вывода страниц ASP.NET с помощью кэша**. За дополнительной информацией по использованию кэша роли обратитесь к разделу[Дальнейшие действия][Дальнейшие действия].
...
-   [Что такое кэш роли][Что такое кэш роли]
-   [Приступая к работе с кэшем роли][Приступая к работе с кэшем роли]
    -   [Настройка кластера кэша][Настройка кластера кэша]
...
Кэш роли обеспечивает уровень кэширования в приложениях Azure. Кэширование повышает производительность за счет временного хранения в памяти информации из других серверных источников и может снизить затраты, связанные с транзакциями баз данных в облаке. Кэш роли включает следующие возможности:
...
Это руководство содержит обзор, посвященный началу работы с кэшем роли. Дополнительные сведения о возможностях, выходящих за рамки данного руководства по началу работы, см. в разделе [Обзор кэша роли][Обзор кэша роли].
...
> Этот раздел содержит упрощенный обзор настройки размера кэша. Дополнительные сведения о размере кэша и других вопросах планирования мощностей см. в разделе [Вопросы планирования мощностей для кэша роли][Вопросы планирования мощностей для кэша роли]
...
Пакет NuGet выполняет несколько задач: добавление требуемой конфигурации в файл конфигурации роли, добавление параметра уровня диагностики клиента кэша в файл ServiceConfiguration.cscfg приложения Azure, а также добавление ссылок на требуемые сборки.

> Для веб-ролей ASP.NET пакет NuGet для Caching также добавляет два закомментированных раздела в файл web.config. Первый раздел позволяет сохранить состояние сеанса в кэше, а второй раздел включает кэширование вывода страниц ASP.NET. Дополнительную информацию см. в разделах [Практическое руководство. Сохранение состояния сеанса ASP.NET в кэше][Практическое руководство. Сохранение состояния сеанса ASP.NET в кэше] и [Практическое руководство. Сохранение кэшированных данных вывода страниц ASP.NET в кэше][Практическое руководство. Сохранение кэшированных данных вывода страниц ASP.NET в кэше]
...
> Кэш роли предоставляет уровень диагностики как сервера, так и клиента кэша. Уровень диагностики — отдельный параметр, который определяет уровень диагностических сведений, собираемых для кэширования. Дополнительные сведения см. в разделе [Устранение неполадок и диагностика для кэша роли][Устранение неполадок и диагностика для кэша роли]
...
> Если Visual Studio не распознает типы в инструкции using даже после установки пакета NuGet для Caching, который добавляет необходимые ссылки, убедитесь, что целевым профилем проекта является платформа .NET Framework 4.0 или более поздней версии, а также не забудьте выбрать один из профилей, в котором не указан **Клиентский профиль**. Инструкции по настройке клиентов кэша см. в разделе [Настройка клиентов кэша][Настройка клиентов кэша].
...
> ErrorCode:SubStatus: предпринимается попытка создать объект с ключом, уже существующим в кэше. Кэш принимает только уникальные значения ключа для объектов.
...
Поставщик состояний сеансов для кэша роли представляет собой механизм внепроцессного хранения для приложений ASP.NET. Этот оставщик позволяет хранить состояние сеанса в кэше Azure, а не в памяти или базе данных SQL Server. Чтобы воспользоваться поставщиком состояний сеансов кэширования, сначала настройте свой кластер кэша, а затем настройте приложение ASP.NET для кэширования с помощью пакета кэша NuGet для Caching, как описано в разделе [Приступая к работе с кэшем роли][Приступая к работе с кэшем роли]. При установке пакета кэша NuGet для Caching он добавляет в файл web.config закомментированный раздел с требуемой конфигурацией, позволяющей приложению ASP.NET использовать поставщик состояний сеансов для кэша роли.
...
Дополнительные сведения об использовании поставщика кэша вывода для кэша роли см. в разделе [Поставщик кэша вывода для кэша роли][Поставщик кэша вывода для кэша роли].
...
-   См. справочник MSDN: [Кэш в роли][Кэш в роли]
-   Узнайте о том, как перейти на использование кэша роли: [Перенос в кэш роли][Перенос в кэш роли]
-   Ознакомьтесь с примерами: [Примеры кэша роли][Примеры кэша роли]
-   Посмотрите о кэшах роли видео [Maximum Performance: Accelerate Your Cloud Services Applications with Azure Caching][Maximum Performance: Accelerate Your Cloud Services Applications with Azure Caching] (Максимальная производительность: как ускорить работу приложений облачных служб с помощью кэширования Azure) с TechEd 2013



  [Дальнейшие действия]: #next-steps
  [Какой кэш Azure подходит мне лучше всего?]: http://msdn.microsoft.com/ru-ru/library/azure/dn766201.aspx
  [Что такое кэш роли]: #what-is
  ...
  [Кэш в роли]: http://www.microsoft.com/ru-ru/showcase/Search.aspx?phrase=azure+caching
