...
**Кэш роли для кэша Azure**. Примеры написаны на языке C# и используют интерфейс .NET API. Сценарии включают **настройки кластера кэша**, **настройку клиентов кэша**, **добавления и удаления объектов из кэша, хранение состояния сеанса ASP.NET в кэше** и **включение кэширования вывода страниц ASP.NET с помощью кэша**. За дополнительной информацией об использовании кэша роли обратитесь к разделу [Дальнейшие действия][].
...
>Если Visual Studio не распознает типы с помощью
инструкции даже после установки пакета кэша NuGet, который добавляет необходимые ссылки, убедитесь, что целевым
профилем для проекта является платформа .NET Framework версии 4.0 или более поздней версии и обязательно выберите один из профилей, которые не определяет **клиентский профиль**. Инструкции по настройке клиентов кэша см. в разделе [Настройка клиентов кэша][].
...
> ErrorCode:SubStatus: Предпринимается попытка создать объект с > ключом, уже существующим в кэше. Кэш принимает только
...
-	Посмотрите о кэшах роли видео [Maximum Performance: Accelerate Your Cloud Services Applications with Azure Caching][] (Максимальная производительность: как ускорить работу приложений облачных служб с помощью кэширования Azure) с TechEd 2013
