<properties linkid="manage-services-hdinsight-develop-hadoop-streaming-programs-for-hdinsight" urlDisplayName="" pageTitle="Разработка программ потоковой передачи Hadoop на C# для HDInsight | Azure" metaKeywords="hdinsight, разработка hdinsight, разработка hadoop, развертывание hinsight, разработка, развертывание, учебник, MapReduce" description="Узнайте, как разрабатывать на C# программы MapReduce для потоковой передачи Hadoop и развертывать их в Azure HDInsight." metaCanonical="" services="hdinsight" documentationCenter="" title="Разработка программ потоковой передачи Hadoop на C# для HDInsight" authors="jgao" solutions="" manager="paulettm" editor="cgronlun" />
...
5. В обозревателе решений щелкните правой кнопкой мыши **Program.cs**, затем щелкните **Переименовать**.
...
1. В Visual Studio 2013 щелкните **ФАЙЛ**, **Добавить**, **Новый проект**.
...
4. В обозревателе решений щелкните правой кнопкой мыши **Program.cs**, затем щелкните **Переименовать**.
...
11. Щелкните **СБОРКА**, а затем щелкните **Построить решение** для компиляции решения.	
...
1. Передача данных в HDFS эмулятора
2. Передача программ Mapper и Reducer в HDFS эмулятора
...
Azure HDInsight использует хранилище BLOB-объектов Azure в качестве файловой системы по умолчанию. Можно настроить кластер HDInsight на использование дополнительного хранилища BLOB-объектов для хранения файлов данных. В этом разделе вы создадите учетную запись хранения и передадите файлы данных в хранилище BLOB-объектов.  В качестве файлов данных используются TXT-файлы в каталоге %hadoop_home%.
...
1. Откройте Блокнот.
2. Скопируйте следующий код и вставьте его в Блокнот:
...
В этом учебнике вы узнали, как разработать задание MapReduce для потоковой передачи Hadoop, протестировать приложение в эмуляторе HDInsight, написать сценарий PowerShell для подготовки кластера HDInsight и выполнения MapReduce в кластере. Для получения дополнительных сведений ознакомьтесь со следующими статьями:
...
[hdinsight-powershell]: /ru-ru/manage/services/hdinsight/install-and-configure-powershell-for-hdinsight/
