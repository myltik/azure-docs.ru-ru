...
Все ресурсы Log Analytics, определенные в шаблоне Resource Manager, имеют свойство **apiVersion**. В нем указывается версия API, которую должен использовать этот ресурс.  Для ресурсов, использующих [прежний и обновленный язык запросов](../log-analytics/log-analytics-log-search-upgrade.md), версии будут разными.  
 ...
 В приведенной ниже таблице перечислены версии API Log Analytics для рабочих областей прежней и обновленной версий. Различия в синтаксисе поясняются на примерах запросов. 

| Версия рабочей области | Версия API | Пример запроса |
|:---|:---|:---|
| v1 (прежняя версия)   | 2015-11-01-preview | Type=Event EventLevelName = Error             |
| v2 (обновленная версия) | 2017-03-15-preview | Event &#124; where EventLevelName == "Error"  |

Важно учитывать следующие аспекты, связанные с поддержкой рабочих областей в разных версиях.

- Шаблоны, которые используют язык запросов прежней версии, можно установить в рабочую область прежней или обновленной версии.  Если используется обновленная рабочая область, запросы автоматически переводятся на новый язык при запуске пользователем.
- Шаблоны, которые используют язык запросов обновленной версии, можно установить только в рабочую область обновленной версии.
...
| Имя элемента | Обязательно | Описание |
|:--|:--|:--|
| Тип | Да | Тип действия.  Для действий оповещения это тип **Alert**. |
...
| Имя элемента | Обязательно | Описание |
|:--|:--|:--|
| Recipients | Да | Разделенный запятыми список электронных адресов, на которые отправляются уведомления при создании оповещения, как показано в приведенном ниже примере.<br><br>**[ "recipient1@contoso.com", "recipient2@contoso.com" ]** |
...
|:--|:--|:--|
| type | Да | Тип действия.  Объект **Webhook** для действий веб-перехватчика. |
