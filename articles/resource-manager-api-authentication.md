...
2. Запрос Resource Manager от имени пользователя (доступ от имени пользователя) для получения списка принадлежащих ему подписок Azure.
...
Azure AD также поддерживает учетные данные сертификата для приложения. При этом вам нужно создать самозаверяющий сертификат, сохранить закрытый ключ и добавить открытый ключ для регистрации приложения Azure AD. Для проверки подлинности приложение отправляет небольшой объем полезных данных в Azure AD, подписанных с помощью закрытого ключа, а Azure AD проверяет подпись, используя зарегистрированный открытый ключ.
...
| nonce | Часть данных, которая связывает запрос на авторизацию с возвращаемым маркером id\_token. Таким образом, ответ на авторизацию запрашивается, а не используется повторно
...
Перед выполнением входа для пользователя приложению необходимо проверить маркер id\_token. Проверка маркера — сложный процесс. Советуем использовать стандартную библиотеку обработчика веб-маркеров JSON для платформы разработки (см. [исходный код обработчика веб-маркеров JSON Azure для .NET](https://github.com/AzureAD/azure-activedirectory-identitymodel-extensions-for-dotnet/blob/master/src/System.IdentityModel.Tokens.Jwt/JwtSecurityTokenHandler.cs)). Тем не менее вы отвечаете за безопасность своего приложения. Поэтому убедитесь, что библиотека, используемая для обработки маркера id\_token, должным образом проверяет следующие свойства:

- **Время создания маркера**. Проверьте утверждения nbf и exp, чтобы убедиться, что маркер не новый или не устарел. Обычно нужно немного подождать (5 минут), чтобы устранить разницу во времени.
- **Издатель**. Проверьте утверждение iss, чтобы убедиться, что издателем маркера является Azure Active Directory: https://sts.windows.net/{tenant_id_of_the_directory}.
- **Аудитория**. Проверьте утверждение aud, чтобы убедиться, что маркер создан для вашего приложения. В качестве значения должен использоваться идентификатор клиента приложения.
- **nonce**. Проверьте утверждение nonce относительно данных nonce, отправленных в запросе на авторизацию, и убедитесь, что приложение затребовало ответ и что маркер не используется повторно.
- **Подпись**. Приложение должно проверить, подписан ли маркер Azure Active Directory. Azure AD часто выполняет подписку ключей, поэтому приложению нужно запрашивать обновленные ключи ежедневно или отклонять их при сбое проверки подписи. Дополнительные сведения см. в статье [Важная информация о смене ключей подписывания в Azure AD](https://msdn.microsoft.com/library/azure/dn641920.aspx).
...
### Запрос маркера (предоставление кода OAuth 2.0)

Теперь, когда приложение получило код авторизации из Azure AD, необходимо получить маркер доступа для Azure Resource Manager. Выполните запрос на маркер предоставления кода OAuth 2.0 к конечной точке маркера Azure AD с помощью метода POST:

    http://login.microsoftonline.com/{directory_domain_name}/OAuth2/Token

Параметры строки запроса, доступные для этого запроса, описаны в статье [Authorization Code Grant Flow](https://msdn.microsoft.com/library/azure/dn645542.aspx) (Предоставление кода авторизации).

В следующем примере показан запрос на маркер предоставления кода с использованием пароля:
...
Пример ответа для маркера предоставления кода:

    HTTP/1.1 200 OK

    {"token_type":"Bearer","expires_in":"3599","expires_on":"1432039858","not_before":"1432035958","resource":"https://management.core.windows.net/","access_token":"eyJ0eXAiOiJKV1Q****M7Cw6JWtfY2lGc5A","refresh_token":"AAABAAAAiL9Kn2Z****55j-sjnyYgAA","scope":"user_impersonation","id_token":"eyJ0eXAiOiJKV*****-drP1J3P-HnHi9Rr46kGZnukEBH4dsg"}

#### Обработка ответа маркера предоставления кода

Успешный ответ маркера будет содержать маркер доступа (при доступе от имени пользователя) для Azure Resource Manager. Приложение использует этот маркер доступа для доступа к Resource Manager от имени пользователя. Срок действия маркеров доступа, выданных Azure AD, составляет один час. Маловероятно, что веб-приложению понадобится обновить маркер доступа (при доступе от имени пользователя). Однако, если обновление потребуется, можно воспользоваться маркером обновления, который приложение получает в ответе маркера. Выполните запрос маркера OAuth 2.0 к конечной точке маркера Azure AD с помощью метода POST:
...
В следующем примере показано, как использовать маркер обновления:
...
Обратите внимание, что хотя с помощью маркеров обновления и можно получать новые маркеры доступа для Azure Resource Manager, они не подходят для автономного доступа приложения. Срок действия маркеров обновления ограничен, и они привязаны к пользователю. Если пользователь покидает организацию, приложение, использующее маркер обновления, потеряет доступ. Этот способ не подходит для приложений, используемых группами для управления ресурсами Azure.

## Предоставление списка доступных подписок

Теперь у приложения есть маркер для доступа к Azure Resource Manager от имени пользователя.
...
Теперь у вас есть список подписок Azure, которые пользователь может подключить к приложению. Следующий шаг — дать пользователю команду создать подключение. Когда пользователь устанавливает **подключение**, приложение выполняет следующее:
...
Чтобы выполнить проверку подлинности приложения и получить маркер для API Graph Azure AD, выполните запрос на маркер предоставления учетных данных клиента OAuth 2.0 к конечной точке маркера Azure AD (**http://login.microsoftonline.com/{directory\_domain\_name}/OAuth2/Token**).

В строках 73–77 метода [GetObjectIdOfServicePrincipalInOrganization](https://github.com/dushyantgill/VipSwapper/blob/master/CloudSense/CloudSense/AzureADGraphAPIUtil.cs#L73) примера приложения ASP.NET MVC получается маркер доступа только для приложений для API Graph с использованием библиотеки проверки подлинности Active Directory для .NET.

Данные запроса на маркер предоставления учетных данных клиента:

| Элемент | Значение
|----|----
| grant\_type | **client\_credentials**
| client\_id | Идентификатор клиента приложения
| resource | Идентификатор с кодировкой URL ресурса, для которого запрашивается маркер доступа. В данном случае используется следующий идентификатор API Graph Azure AD: **https://graph.windows.net/** 
| client\_secret или client\_assertion\_type и client\_assertion | Если приложение использует пароль, используйте client\_secret. Если приложение использует сертификат, используйте client\_assertion.

Пример запроса на маркер предоставления учетных данных клиента:
...
Пример ответа маркера предоставления учетных данных клиента:
...
- Если приложение только выполняет мониторинг подписки и не вносит никаких изменений, ему требуются разрешения только на чтение для подписки. Назначьте роль **модуля чтения**.
- Если приложение управляет подпиской Azure, а также создает, изменяет или удаляет сущности, ему понадобится разрешение участника.
  ...
  - Для управления ресурсами любого типа назначьте роль **участника**.
...
Теперь все готово для назначения соответствующей роли RBAC субъекту-службе приложения для выбранной подписки с помощью API [создания назначения роли Resource Manager](https://msdn.microsoft.com/library/azure/dn906887.aspx).
...
В строках 210–214 метода [ServicePrincipalHasReadAccessToSubscription](https://github.com/dushyantgill/VipSwapper/blob/master/CloudSense/CloudSense/AzureResourceManagerUtil.cs#L203) примера приложения ASP.NET MVC получается маркер доступа только для приложений для Azure Resource Manager с использованием библиотеки проверки подлинности Active Directory для .NET.

#### Получение разрешений приложений для подписки

Чтобы проверить, есть ли у приложения необходимый доступ на подписку Azure, можно также вызвать API [разрешений Resource Manager](https://msdn.microsoft.com/library/azure/dn906889.aspx) (который используется, чтобы определить, есть ли у пользователя права управления доступом для подписки). Однако на этот раз вызовите API разрешений с помощью маркера доступа только для приложений, полученного на предыдущем шаге.
