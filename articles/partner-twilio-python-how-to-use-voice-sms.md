...
Дополнительные сведения см. в разделе [][2]<https://github.com/twilio/twilio-php/blob/master/README.md></a>.
...
-   **Примечание**. Рекомендации по устранению ошибок, связанных с проверкой SSL-сертификата, см. в разделе [][3]<http://readthedocs.org/docs/twilio-php/en/latest/usage/rest.html></a>
...
Когда приложение инициирует вызов API Twilio, Twilio отправляет ваш запрос на URL-адрес, который должен возвратить ответ TwiML. В приведенном выше примере используется URL-адрес, предоставляемый Twilio: [][4]<http://twimlets.com/message></a>. (Хотя TwiML предназначается для использования службой Twilio, его можно также просмотреть в браузере. Например, выберите [][4]<http://twimlets.com/message></a> для просмотра пустого `<Response>` элемента; в качестве другого примера нажмите [][5]<http://twimlets.com/message?Message%5B0%5D=Hello%20World></a> для просмотра элемента `<Response>`, в котором содержится элемент `<Say>`).
...
Дополнительные сведения о TwiML см. на странице [][6]<https://www.twilio.com/docs/api/twiml></a>.

После настройки PHP-страницы на предоставление ответов TwiML используйте URL PHP-страницы как URL, передаваемый в метод `Services_Twilio->account->calls->create`. Например, если у вас есть веб-приложение с именем **MyTwiML**, развернутая в размещенной службе Azure, а PHP-страница использует имя **mytwiml.php**, URL-адрес может быть передан в класс **Services\_Twilio-\>account-\>calls-\>create**, как показано в следующем примере:
...
-   [Обращение в службу поддержки Twilio][Обращение в службу поддержки Twilio]
  ...
  [Осуществление телефонных звонков с использованием Twilio в PHP-приложении Azure]: http://windowsazure.com/ru-ru/documentation/articles/partner-twilio-php-make-phone-call
