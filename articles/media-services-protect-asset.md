<properties urlDisplayName="Encrypt Assets in Media Services" pageTitle="Как шифровать ресурсы в службах мультимедиа - Azure" metaKeywords="" description="Learn how to use Microsoft PlayReady Protection to encrypt an asset in Media Services. Code samples are written in C# and use the Media Services SDK for .NET. Code samples are written in C# and use the Media Services SDK for .NET." metaCanonical="" services="media-services" documentationCenter="" title="How to: Protect an Asset with PlayReady Protection" authors="juliako" solutions="" manager="dwrede" editor="" />
...
Эта статья является частью серии вводных статей о программировании служб мультимедиа в Azure. Предыдущая статья: [Практическое руководство. Проверка хода выполнения задания](../media-services-check-job-progress/).
...
Следующий пример демонстрирует, как создать простое задание для обеспечения защиты с помощью PlayReady Protection.
   ...
   1. Получение данных конфигурации. Пример файла конфигурации можно найти в разделе [Предустановка задачи для Azure Media Encryptor](http://msdn.microsoft.com/ru-ru/library/hh973610.aspx) .
