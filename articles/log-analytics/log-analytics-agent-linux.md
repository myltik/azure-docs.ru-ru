...
----------- | ----------- | --------------
omsagent | 1.4.1 | Агент Operations Management Suite для Linux
...
* Создается непривилегированный пользователь с именем `omsagent` . Управляющая программа omsagent запускается от имени этой учетной записи.
* В каталоге /etc/sudoers.d/omsagent создается файл sudoers с директивами include. Этот файл дает программе omsagent право на перезапуск управляющих программ syslog и omsagent. Если директивы sudo "include" не поддерживаются в установленной версии sudo, эти записи будут записываться в каталог /etc/sudoers.
...
1. Скачайте последнюю версию [агента OMS для Linux (x64)](https://github.com/Microsoft/OMS-Agent-for-Linux/releases/download/OMSAgent_GA_v1.4.1-45/omsagent-1.4.1-45.universal.x64.sh) или [агента OMS для Linux (x86)](https://github.com/Microsoft/OMS-Agent-for-Linux/releases/download/OMSAgent_GA_v1.4.1-45/omsagent-1.4.1-45.universal.x86.sh) с сайта GitHub.  
...
Выполните команду omsadmin.sh, указав идентификатор и ключ рабочей области. Эту команду должен выполнять привилегированный пользователь (sudo):
```
...
1. Повторно подключитесь к службе OMS, используя службу OMS для Linux и следующую команду с включенным параметром `-v`. Это позволяет передать подробные выходные данные агента с помощью подключения к службе OMS через прокси-сервер. 
...
Это известная проблема, которая возникает при первой передаче данных Linux в рабочую область OMS. Эта ошибка не влияет на отправляемые данные и не мешает работе службы.
