...
|---------|---------|---------|
|abs | Возвращает абсолютное значение заданного поля или функции.|<p>abs(x)</p> <p>abs(-5)</p>|
...
|def | def — сокращение от default. Возвращает значение поля field, а если такое поле не существует, возвращает указанное значение по умолчанию или первое по порядку значение, для которого справедливо выражение exists()==true. |<p>div(1,y)</p><p>div(sum(x,100),max(y,1))</p>|
| div | div(x,y) делит x на y. | div(1,y),div(sum(x,100),max(y,1)) |
| dist | Возвращает расстояние между двумя векторами (точками) в многомерном пространстве. Принимает в качестве аргументов степень, а также два или несколько экземпляров ValueSource и вычисляет расстояния между двумя векторами. Каждый экземпляр ValueSource должен иметь числовое значение. Количество экземпляров ValueSource должно быть четным. Метод предполагает, что первая половина этих экземпляров представляет первый вектор, а вторая — второй вектор. | <p>dist(2, x, y, 0, 0): вычисляет евклидово расстояние между (0,0) и (x,y) для каждого документа</p><p>dist(1, x, y, 0, 0): вычисляет манхэттенское расстояние (расстояние городских кварталов) между (0,0) и (x,y) для каждого документа</p><p>dist(2,,x,y,z,0,0,0): евклидово расстояние между (0,0,0) и (x,y,z) для каждого документа.</p><p>dist(1,x,y,z,e,f,g): манхэттенское расстояние между (x,y,z) и (e,f,g), где каждая буква обозначает имя поля.</p> |
| exists (существует) | Возвращает TRUE, если существует хотя бы один элемент поля. | <p>exists(author) возвращает значение TRUE для любого документа, который имеет значение в поле author.</p><p>exists(query(price:5.00)) возвращает значения TRUE в тех строках, где поле price имеет значение 5,00.</p> |
| hsin | Формула гаверсинуса вычисляет расстояние между двумя точками на поверхности сферы при условии передвижения по поверхности сферы. Аргументы должны быть заданы в радианах. Кроме них, hsin принимает логический аргумент, который определяет, нужно ли преобразовывать в радианы выходные данные. | hsin(2, true, x, y, 0, 0) |
...
| map | Преобразует все значения входной функции x, попадающие в диапазон от min до max включительно, в значения, определяемые аргументом target. Аргументы функции min и max должны быть константами. Аргументы target и default могут быть константами или функциями. Если значение x не попадает в диапазон от min до max, возвращается значение x или значение по умолчанию, если указан 5-й аргумент default. | <p>map(x,min,max,target) map(x,0,0,1) — изменяет все значения 0 на 1. Это может быть полезно при обработке значений 0, используемых по умолчанию.</p> <p> map(x,min,max,target,default) map(x,0,100,1,-1) — преобразует все значения от 0 до 100 в значение 1, а все остальные — в -1.</p> </p> map(x,0,100,sum(x,599),docfreq(text,solr)) — прибавляет число 599 ко всем значениям в диапазоне от 0 до 100, а для всех остальных значений выводит частоту термина solr в поле text.</p> |
...
| min | Возвращает минимальное числовое значение нескольких вложенных функций констант, которые переданы в качестве аргументов: max(x,y,…). Функция max может быть полезна для "обрезания" высоких значений другой функции или поля до определенной константы. (Чтобы выбрать минимальное значение из одного поля, содержащего различные значения, используйте синтаксис field(myfield,min).) | min(myfield,myotherfield,0) |
| ms | Возвращает разницу в миллисекундах между заданными аргументами. Даты указываются в формате эпохи времени Unix или POSIX, то есть от полуночи 1 января 1970 г. в формате UTC. В качестве аргументов принимаются имена индексированных полей типа TrieDateField или математические вычисления на основе константы типа date или значения NOW. · ms(): эквивалентно ms(NOW), возвращает время в миллисекундах, прошедшее с начала эпохи. · ms(a): возвращает число миллисекунд с момента времени, заданного аргументом. · ms(a,b): возвращает число миллисекунд, прошедших с момента b до момента a (то есть a - b). | <p> </p><p> ms(NOW/DAY) </p> <p> ms(2000-01-01T00:00:00Z)</p> <p> ms(mydatefield)</p> <p> ms(NOW,mydatefield)</p> <p> ms(mydatefield,2000-01-01T00:00:00Z)</p> <p> ms(datefield1,datefield2)</p> |
| not | Логическое отрицание значения функции, указанной в качестве аргумента. | not(exists(author)): возвращает TRUE только в том случае, когда exists(author) имеет значение FALSE. |
| или | Логическая дизъюнкция. | or(value1,value2): возвращает TRUE, если хотя бы одно из значений value1 и value2 равно TRUE. |
| pow | Возводит указанное основание в указанную степень. pow(x,y): возвращает значение x в степени y. | <p> pow(x,y)</p> <p>pow(x,log(y))</p> <p> pow(x,0.5)</p>: эквивалентно функции sqrt |
| product | Возвращает произведение нескольких значений или функций, которые указываются в виде списка, разделенного запятыми. Для этой функции можно также использовать псевдоним mul(…). | <p> product(x,y,…)</p> <p>product(x,2)</p> <p> product(x,y)</p> <p>mul(x,y)</p> |
| recip | Выполняет обратную функцию, то есть `recip(x,m,a,b)` соответствует `<a/(m*x+b)`, где m, a и b — константы, а x — функция любого уровня сложности. Если a = b и x >= 0, эта функция имеет максимальное значение 1, которое уменьшается по мере увеличения x. Увеличение значений a и b позволяет сместить график функции в более плоскую часть кривой. Эти свойства позволяют использовать функцию для повышения рейтинга более свежих документов, указав в качестве x функцию rord(datefield). | <p>recip(myfield,m,a,b)<p> <p> recip(rord(creationDate),1,1000,1000)</p> |
| scale | Масштабирует значения функции x таким образом, чтобы они располагались в диапазоне от minTarget до maxTarget включительно. Текущая реализация функции перебирает все значения функции, чтобы определить минимальное и максимальное значение для выбора правильного масштаба. Текущая реализация не различает удаленные документы или документы, для которых не указано значение. Для обоих этих случаев используется значение 0,0. Таким образом, даже если все нормальные значения аргумента больше 0,0, минимальным значением для функции масштабирования все равно может оказаться 0,0. Чтобы обойти эту проблему, мы рекомендуем использовать соответствующую функцию map() для замены значения 0,0 на значение из диапазона осмысленных значений, например так: scale(map(x,0,0,5),1,2). | <p>scale(x,minTarget,maxTarget)</p> <p>scale(x,1,2): масштабирует значения x таким образом, чтобы все они оказались между 1 и 2 включительно. </p> |
| sqedist | Вычисляет квадрат евклидова расстояния без извлечения квадратного корня, что позволяет не расходовать ресурсы на довольно дорогую операцию. Зачастую приложениям, которые используют евклидово расстояние, не нужно само значение этого расстояния, и для практических целей можно использовать квадрат расстояния. Количество экземпляров ValueSource должно быть четным. Метод предполагает, что первая половина этих экземпляров представляет первый вектор, а вторая — второй вектор. | sqedist(x\_td, y\_td, 0, 0) |
| sqrt | Возвращает квадратный корень из указанного значения или функции. | <p>sqrt(x)</p><p>sqrt(100)</p><p>sqrt(sum(x,100))</p> |
| strdist | Вычисления расстояние между двумя строками. Использует интерфейс StringDistance средства проверки орфографии Lucene и поддерживает все доступные в этом пакете реализации. Кроме этого, позволяет приложениям подключать свои пакеты через возможности загрузки ресурсов Solr. strdist принимает такие аргументы: string1, string2, distance measure. Возможные значения для меры расстояния: jw: расстояние Яро — Винклера; edit: расстояние Левенштейна или редакторское расстояние; ngram: NGramDistance, также принимает необязательный аргумент размера ngram. По умолчанию используется значение 2. FQN: Полное имя класса для реализации интерфейса StringDistance. Должен использоваться конструктор без аргументов. | strdist("SOLR",id,edit) |
| sub | Возвращает разность значений x-y, переданных в аргументах sub(x,y). | <p>sub(myfield,myfield2)</p> <p> sub(100,sqrt(myfield))</p> |
| sum | Возвращает сумму нескольких значений или функций, которые передаются в виде списка, разделенного запятыми. Для этой функции можно также использовать псевдоним add(…). | <p>sum(x,y,…)</p> <p>sum(x,1)</p> <p> sum(x,y)</p> <p> sum(sqrt(x),log(y),z,0.5)</p> <p> add(x,y)</p> |
| xor | Исключающее ИЛИ. | xor(field1,field2): возвращает TRUE, если любой из аргументов (field1 или field2) имеет значение TRUE; но возвращает FALSE, если оба аргумента имеют значение TRUE. |
