...
> [!NOTE]
> Подобным образом индексируются не все поля, однако наиболее распространенные текстовые поля (например, описания и имена) в общем случае проиндексированы именно так.
>
>
...
> [!NOTE]
> Подстановочные знаки пока нельзя использовать в кавычках. В строке `"*This text*"` все символы (\*) будут трактоваться как символ звездочки (\*).
>
...
**Описание.** Позволяет создавать поля на время выполнения запросов. Вы также можете использовать measure после Extend, если вы хотите выполнить агрегирование.
