<properties 
   pageTitle="Журналы в Log Analytics | Microsoft Azure"
...
Действия Runbook запускают модуль Runbook с помощью [webhook](../automation/automation-webhooks.md). При создании правила генерации оповещений автоматически создается новое действие webhook для модуля Runbook с именем **OMS Alert Remediation**, за которым следует GUID. Правило генерации оповещений не отправляет полезные данные, и нельзя заполнить никакие параметры модуля Runbook, поэтому необходимо использовать Runbook, не требующий параметры.

Можно также запустить Runbook из оповещения с помощью действия webhook, как описано в предыдущем разделе. Преимущество действий webhook заключается в предоставлении полезных данных для отправки данных в Runbook. Преимуществом действий Runbook является отсутствие необходимости создавать webhook самостоятельно или изучать принципы его использования.
...
4.	Щелкните **Сохранить** для завершения работы с правилом генерации оповещений. Оно запускается немедленно.


![Добавление правила генерации оповещений](media/log-analytics-alerts/add-alert-rule.png)

| Свойство | Описание |
|:--|:--|
| **Alert information** (Сведения об оповещении) | |
...
События включают источники данных, такие как журналы событий Windows, Syslog и пользовательские журналы. Вам может потребоваться создать оповещение, когда создается конкретное событие ошибки или при создании нескольких событий ошибки за определенное временное окно.
...
[Данные о производительности](log-analytics-data-sources-performance-counters.md) хранятся в виде записей в репозитории OMS по аналогии с событиями. В каждую запись вносится среднее значение за предыдущие 30 минут. Если вы хотите создать оповещение, когда значение счетчика производительности превышает определенный порог, это пороговое значение следует включить в запрос.

Например, чтобы создать оповещение, когда уровень загрузки ЦП превышает 90 % в течение 30 минут, используйте запрос *Type=Perf ObjectName=Processor CounterName="% Processor Time" CounterValue>90* и пороговое значение для правила генерации оповещений *больше 0*.

 Так как [записи производительности](log-analytics-data-sources-performance-counters.md) агрегируются каждые 30 минут независимо от частоты сбора данных для каждого счетчика, при временном окне менее 30 минут возвращаемые записи могут отсутствовать. Установка временного окна равным 30 минутам обеспечивает получение одной записи для каждого подключенного источника, представляющей усредненное значение за этот период.
