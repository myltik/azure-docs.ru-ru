<properties
    pageTitle="Монитор производительности сети в OMS | Microsoft Azure"
    description="Монитор производительности сети позволяет отслеживать производительность ваших сетей практически в реальном времени для обнаружения в них узких мест производительности."
...
# <a name="network-performance-monitor-(preview)-solution-in-oms"></a>Монитор производительности сети (предварительная версия) в OMS
...
>[AZURE.NOTE] Скрипт EnableRules.ps1 настраивает правила брандмауэра Windows только на том компьютере, где он выполняется. При наличии брандмауэра сети он должен разрешать пропуск трафика к TCP-порту, который используется монитором производительности сети.
...
1. Монитор производительности сети получает данные из компьютеров под управлением Windows Server 2008 с пакетом обновления 1 либо более поздней версии или Windows 7 с пакетом обновления 1 либо более поздней версии. Эти требования применяются и к Microsoft Monitoring Agent.
2. Добавьте решение "Монитор производительности сети" в рабочую область OMS, как описано в статье [Добавление решений Log Analytics из коллекции решений](log-analytics-add-solutions.md).  
...
4. Если не нужно создавать события работоспособности для выбранных элементов, снимите флажок **Включить наблюдение за работоспособностью ссылок, охваченных этим правилом**.
5. Выберите условия мониторинга.
...
В мониторе производительности сети используются пакеты подтверждения TCP SYN-SYNACK-ACK для сбора сведений о потере и задержке. Для получения сведений о топологии используется трассировка маршрута.
...
|---|---|---|---|---|---|---|
| Windows |![Да](./media/log-analytics-network-performance-monitor/oms-bullet-green.png)|![Да](./media/log-analytics-network-performance-monitor/oms-bullet-green.png)|![Нет](./media/log-analytics-network-performance-monitor/oms-bullet-red.png)|            ![Нет](./media/log-analytics-network-performance-monitor/oms-bullet-red.png)|![Нет](./media/log-analytics-network-performance-monitor/oms-bullet-red.png)| Подтверждения TCP выполняются каждые 5 секунд, данные отправляются каждые 3 минуты |

Для оценки работоспособности сети в решении используются искусственные транзакции. Агенты OMS, установленные в различных точках в сети, обмениваются друг с другом пакетами TCP и в процессе определяют значения времени кругового пути и потери пакетов (если таковые имеются). Периодически каждый агент выполняет трассировку маршрута к другим агентам, чтобы найти все различные маршруты в сети, которые необходимо протестировать. Используя эти данные, агенты могут снизить показатели задержки сети и потери пакетов. Тесты повторяются каждые пять секунд, и агенты собирают данные в течение трех минут перед их отправкой в OMS.
...
Если щелкнуть узел или навести на него указатель мыши на карте топологии, отобразятся его свойства, например полное доменное имя и IP-адрес. Нажмите кнопку прыжка, чтобы увидеть IP-адрес. Чтобы выделить определенные маршруты, снимите все флажки, а затем установите их только возле тех маршрутов, которые необходимо выделить на карте. Масштаб карты топологии можно увеличить или уменьшить с помощью колесика мыши.
...
6. Значения потери, задержки и количество прыжков в каждом пути можно просмотреть в области **Подробности о пути**. В этом примере можно увидеть три неработоспособных пути, как указано в области. Используйте полосу прокрутки, чтобы просмотреть сведения об этих неработоспособных путях.  С помощью флажков можно выбрать один из путей, чтобы топология отображалась только для него. С помощью колесика мыши можно увеличить или уменьшить масштаб карты топологии.
