...
При создании сети добавьте в нее подсеть, и эта подсеть удалится из сети Default. Если удалить сеть, все подсети автоматически возвратятся в сеть Default.
Иными словами, сеть Default является контейнером для всех подсетей, которые не содержатся в какой-либо сети, определяемой пользователем. Сеть Default нельзя изменить или удалить. Она всегда остается в системе. Тем не менее вы можете создать любое количество настраиваемых сетей.
...
3. Щелкните **Сохранить**.<br><br> ![Включение мониторинга узла](./media/log-analytics-network-performance-monitor/npm-enable-node-monitor.png)
