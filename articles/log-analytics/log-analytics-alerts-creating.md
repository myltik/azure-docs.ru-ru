---
title: "Создание правил генерации оповещений в OMS Log Analytics | Документация Майкрософт"
...
---
# <a name="create-and-manage-alert-rules-in-log-analytics-with-the-oms-portal"></a>Создание правил генерации оповещений и управление ими в Log Analytics с помощью портала OMS
[Оповещения в Log Analytics](log-analytics-alerts.md) создаются с помощью правил генерации оповещений, которые автоматически выполняют поиск по журналам через регулярные интервалы.  Они создают запись оповещения, если результаты соответствуют определенным условиям.  После этого правило может автоматически запустить одно или несколько действий, чтобы заблаговременно уведомить вас об оповещении либо вызвать другой процесс.   
...
>[!NOTE]
> Сейчас на портале Azure нельзя создавать и изменять правила генерации оповещений. 
...
### <a name="alert-frequency"></a>Alert frequency (Частота оповещений)
...
Правила генерации оповещений всегда создают [запись оповещения](#alert-records) при достижении порогового значения.  Вы также можете определить одно или несколько действий, которые будут выполняться при достижении этого значения, например отправка сообщения электронной почты или запуск модуля runbook.  Сведения о настройке действий см. в статье, посвященной [добавлению действий в правила генерации оповещений в Log Analytics](log-analytics-alerts-actions.md). 
...
| Enter your custom JSON payload (Введите пользовательские полезные данные JSON) |Настраиваемые полезные данные для отправки в webhook.  |
