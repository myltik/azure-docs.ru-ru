...
>[AZURE.NOTE] Для виртуальных машин, запущенных в Azure, установку можно упростить с помощью [расширения виртуальной машины](log-analytics-azure-vm-extension.md).

На компьютерах, подключенных к Интернету, агент будет использовать интернет-подключение для отправки данных в OMS. Для компьютеров, не подключенных к Интернету, можно использовать прокси-сервер или сервер пересылки Log Analytics в OMS.
...
- Каждый компьютер Windows должен иметь доступ к Интернету по протоколу HTTPS. Подключение может быть установлено напрямую, через прокси-сервер или с помощью функции пересылки службы Log Analytics в OMS.
...
3. Щелкните значок копирования справа от **идентификатора рабочей области** и вставьте идентификатор в Блокнот.
4. Нажмите значок копирования справа от **первичного ключа** и вставьте ключ в Блокнот.     
    ![Копирование идентификатора рабочей области и первичного ключа](./media/log-analytics-windows-agents/oms-direct-agent-primary-key.png)
...
- Измените, а затем используйте следующий пример для установки агента с помощью командной строки.
    ...
    >[AZURE.NOTE] Для обновления агента необходимо использовать API сценариев службы Log Analytics. Сведения об обновлении агента см. в следующем разделе.
...
>[AZURE.NOTE] Если ранее вы использовали командную строку или сценарий для установки или настройки агента, теперь вместо `EnableAzureOperationalInsights` используется `AddCloudWorkspace`.

## <a name="install-the-agent-using-dsc-in-azure-automation"></a>Установка агента с помощью DSC в службе автоматизации Azure

>[AZURE.NOTE] В этом примере процедура и сценарий не выполняют обновление существующего агента.
...
## <a name="optionally,-configure-agents-to-use-the-oms-log-analytics-forwarder"></a>Кроме того, вы можете настроить агенты на использование сервера пересылки службы Log Analytics в OMS.
...
Дополнительные сведения о сервере пересылки, включая программу установки и конфигурацию, см. в статье [OMS Log Analytics Forwarder](https://blogs.technet.microsoft.com/msoms/2016/03/17/oms-log-analytics-forwarder) (Сервер пересылки Log Analytics в OMS).

Сведения о настройке агентов для использования прокси-сервера (в данном случае сервера пересылки в OMS) есть в статье [Настройка параметров прокси-сервера и брандмауэра в службе Log Analytics](log-analytics-proxy-firewall.md).
