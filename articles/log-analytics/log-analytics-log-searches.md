...
Общие запросы поиска журналов отображаются на большинстве страниц решений. На портале OMS щелкните плитки или откройте вложенные элементы, чтобы просмотреть сведения о них с помощью функции поиска по журналам.
...
Прежде всего, необходимо знать, что первой частью поискового запроса, расположенной перед символом вертикальной черты "|", всегда является *фильтр*. Его можно считать предложением WHERE в TSQL: он определяет, *какое* подмножество данных следует извлекать из рабочей области Analytics. Поиск в хранилище данных заключается главным образом в указании характеристик данных, которые нужно извлечь, поэтому логично, что запрос будет начинаться с предложения WHERE.
...
Мы начнем с данных о производительности. Обратите внимание, что служба Log Analytics в настоящее время собирает счетчики производительности для машин Windows и Linux.
