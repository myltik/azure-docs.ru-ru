---
title: "Решение &quot;Управление ресурсами&quot; в Log Analytics | Документация Майкрософт"
...
Решение для управления ресурсами в службе Log Analytics позволяет оценить ресурсы серверов Hyper-V. Для работы этого решения требуется System Center Operations Manager и System Center Virtual Machine Manager. Если используются только непосредственно подключенные агенты, решение для планирования ресурсов недоступно. Решение считывает показатели счетчиков производительности на отслеживаемом сервере и отправляет данные об использовании в службу OMS в облаке для обработки. К полученным данным применяется логика и облачная служба записывает данные. Со временем определяются закономерности использования и на основе текущего потребления ресурсов выполняется планирование емкости.
...
В настоящее время для нового решения требуется Hyper-V Server 2012 или более поздней версии. Это решение предоставляет подробные сведения о среде Hyper-V, а также информацию о совокупном использовании (ЦП, памяти и дисков) для узлов и виртуальных машин, запущенных на этих серверах Hyper-V. Кроме того, оно выполняет сбор метрик ЦП, памяти и дисков на всех узлах и виртуальных машинах.

Остальные сведения на этой странице касаются старой версии решения для управления ресурсами. Как только выйдет общедоступная предварительная версия нового решения, эти сведения будут обновлены.
...
* Требуется подключение между Operations Manager и Virtual Machine Manager (VMM). Дополнительные сведения о подключении систем см. в статье [Подключение VMM с Operations Manager](http://technet.microsoft.com/library/hh882396.aspx).
...
С помощью средства прогнозирования можно просматривать динамику использования дискового пространства. Данное средство также позволяет прогнозировать нехватку дискового пространства. Это поможет надлежащим образом спланировать необходимый объем ресурсов хранения и быть в курсе, когда потребуется приобрести дополнительные системы хранения
