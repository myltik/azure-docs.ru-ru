---
title: "Начало работы со службой Log Analytics | Документация Майкрософт"
description: "Подготовиться к работе с Log Analytics можно за считаные минуты."
...
Бесплатную рабочую область OMS можно создать с помощью веб-сайта OMS. Вы также можете использовать подписку на Microsoft Azure для создания бесплатной рабочей области Log Analytics. Обе рабочие области имеют идентичный набор функций. Бесплатные рабочие области могут отправлять в службу OMS не более 500 МБ в день. Для всех рабочих областей необходима подписка Azure. Ее также можно использовать для доступа к другим службам Azure. Независимо от используемого метода создания рабочей области, применяется либо учетная запись Майкрософт, либо учетная запись организации.
...
* Платная подписка Microsoft Azure для полноценного использования службы Log Analytics. Если у вас нет подписки Azure, вы можете создать [бесплатную учетную запись](https://azure.microsoft.com/free/), предоставляющую доступ ко всем службам Azure в течение 30 дней пробного периода. Вы также можете создать бесплатную учетную запись OMS на веб-сайте [Operations Management Suite](http://microsoft.com/oms).
* Необходимо создать рабочую область.
...
* Определите, имеют ли компьютеры прямое подключение к Интернету. Если нет, для доступа к веб-сайтам OMS им потребуется сервер шлюза. Весь доступ осуществляется по протоколу HTTPS. Можно настроить сервер [шлюза OMS](log-analytics-oms-gateway.md) для переадресации трафика с серверов в OMS, если на этих компьютерах отсутствует доступ к Интернету.
...
## <a name="sign-up-in-3-steps-using-oms"></a>Регистрация в 3 этапа с помощью OMS
1. Перейдите на веб-сайт [Operations Management Suite](http://microsoft.com/oms). Войдите с помощью учетной записи Майкрософт, например Outlook.com, или организационной учетной записи, которую предоставила компания или образовательная организация для работы с Office 365 или другими службами Майкрософт.
...
3. Затем создайте новую бесплатную подписку Azure или укажите ссылку на существующую.  
...
Дополнительные сведения о настройке рабочей области и связывании существующих учетных записей Azure с рабочими областями, созданными с помощью Operations Management Suite, см. в статье [Управление рабочими областями](log-analytics-manage-access.md).
...
3. Нажмите кнопку **ОК**, и вы увидите список рабочих областей.
...
5. Щелкните ссылку **портал OMS** , чтобы открыть веб-сайт Operations Management Suite с новой рабочей областью.
...
## <a name="optionally-connect-windows-computers-by-installing-an-agent"></a>Подключение компьютеров с ОС Windows путем установки агента (необязательно)
...
> [!NOTE]
> В настоящий момент решения для управления емкостью и оценки конфигурации не поддерживаются серверами, подключенными непосредственно к Operations Management Suite.
...
2. Разверните узел **Operational Insights** и выберите **Operational Insights Connection** (Подключение к Operational Insights).
   ...
   > [!NOTE]
   > В зависимости от того, какая версия накопительного пакета обновления SCOM используется, отобразится узел *System Center Advisor*, *Operational Insights* или *Operations Management Suite*.
   >
   >
