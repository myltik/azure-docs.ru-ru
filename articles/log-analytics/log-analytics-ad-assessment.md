<properties
	pageTitle="Оптимизация среды с помощью решения оценки Active Directory в Log Analytics | Microsoft Azure"
...
# Оптимизация среды с помощью решения оценки Active Directory в Log Analytics
...
- Решение оценки Active Directory необходимо добавить в рабочую область OMS, как описано в статье [Add Log Analytics solutions from the Solutions Gallery](log-analytics-add-solutions.md) (Добавление решений Log Analytics из коллекции решений). Дополнительная настройка не требуется.
...
## Сведения о сборе данных по оценке
...
**Безопасность и соответствие требованиям**: эта приоритетная область содержит рекомендации в отношении возможных угроз и нарушений безопасности, корпоративные политики, а также технические, юридические и нормативные требования.

**Доступность и непрерывность бизнес-процессов**: эта приоритетная область содержит рекомендации в отношении доступности служб, устойчивости инфраструктуры и защиты бизнеса.

**Производительность и масштабируемость**: эта приоритетная область содержит рекомендации, которые помогут ИТ-инфраструктуре вашей организации расти, обеспечат соответствие ИТ-среды текущим требованиям к производительности и позволят ей реагировать на изменение требований инфраструктуры.

**Обновление, миграция и развертывание**: эта приоритетная область содержит рекомендации, которые помогут вам обновить, перенести или развернуть Active Directory в существующую инфраструктуру.
...
3. На всех страницах интересующей области можно просматривать приоритетные рекомендации для вашей среды. Щелкните рекомендацию в разделе **Затронутые объекты**, чтобы просмотреть сведения о причинах этой рекомендации. ![image of Assessment recommendations](./media/log-analytics-ad-assessment/ad-focus.png)
4. Вы можете выполнить действия по исправлению, указанные в разделе **Предложенные действия**. Когда проблема с этим элементом будет устранена, последующие оценки будут указывать, что рекомендованные действия были выполнены, и тогда оценка соответствия возрастет. Исправленные элементы отображаются как **Прошедшие проверку объекты**.
...
1.	Войдите в рабочую область и откройте поиск по журналам. Выполните следующий запрос, чтобы получить список рекомендаций, не выполненных на компьютерах в вашей среде.
    ...
    Снимок экрана с запросом поиска по журналам: ![невыполненные рекомендации](./media/log-analytics-ad-assessment/ad-failed-recommendations.png)
...
После выполнения следующей плановой оценки (по умолчанию выполняется раз в 7 дней) указанные рекомендации помечаются как *игнорируемые* и больше не отображаются на панели мониторинга оценки.
...
## Часто задаваемые вопросы по решениям оценки AD
