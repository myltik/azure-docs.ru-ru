<properties
   pageTitle="Настраиваемые поля в службе Log Analytics | Microsoft Azure"
...
# Настраиваемые поля в службе Log Analytics
...
>[AZURE.NOTE] В предварительной версии этой функции поддерживаются только 100 настраиваемых полей на рабочую область. С выходом общедоступной версии это число увеличится.
...
2. Выберите функцию **поиска по журналу** и введите [запрос на получение записей](log-analytics-log-searches.md), которые будет содержать настраиваемое поле.
...
4. После этого откроется **мастер извлечения полей**, и выбранная запись будет отображаться в столбце **Main Example** (Главный пример). Настраиваемое поле будет определено для записей, значения в свойствах которых соответствуют выбранным.  
...
1. Выделите текст в примере записи, для которой требуется заполнить настраиваемое поле. Затем появится диалоговое окно, в котором нужно указать имя для поля и запустить выполнение начального извлечения. К имени поля будет автоматически добавлено **\_CF**.
...
### Шаг 3. Проверка правильности извлечения и создание настраиваемого поля
...
2.	Если какие-либо значения, определенные в ходе начального извлечения, неверны, щелкните значок **Изменить** рядом с неправильной записью и выберите команду **Modify this highlight** (Изменить это выделение).
3.	Эта запись скопируется в раздел **Additional examples** (Дополнительные примеры) под колонкой **Main Example** (Главный пример). Здесь можно выделить необходимую часть данных, чтобы указать Log Analytics на правильный выбор.
4.	Нажмите кнопку **Извлечь**, чтобы использовать эту информацию для оценки всех существующих записей. Для других записей могут быть изменены результаты на основе этих новых сведений.
5.	Вносите исправления до тех пор, пока во всех записях в извлечении не будут правильно определены данные для заполнения нового настраиваемого поля.
6. Добившись удовлетворительных результатов, нажмите кнопку **Save Extract** (Сохранить извлечение). Теперь настраиваемое поле определено, но оно еще не добавлено к записям.
7.	Подождите, пока не будут собраны новые записи, соответствующие указанным условиям, и выполните поиск по журналу снова. После этого новые записи будут содержать настраиваемое поле.
8.	Используйте настраиваемое поле, как и другие свойства записи. Его можно применять для объединения и группирования данных, а также для создания новых сведений для анализа.


## Просмотр настраиваемых полей
Вы можете просмотреть список всех настраиваемых полей в группе управления с помощью элемента **Параметры** на панели мониторинга OMS. Выберите вкладку **Данные**, а затем — раздел **Настраиваемые поля**, чтобы просмотреть список всех настраиваемых полей в рабочей области.
...
После этого откроется **мастер извлечения полей**, и в колонке **Main Example** (Главный пример) будут выбраны поля **EventLog** и **EventID**. Это означает, что настраиваемое поле будет определено для событий системного журнала с идентификатором 7036. Этих полей достаточно. Нам больше не нужно ничего выбирать.

![Главный пример](media/log-analytics-custom-fields/main-example.png)

Мы выделим имя службы в свойстве **RenderedDescription** и введем для имени службы **Service**. Настраиваемое поле будет называться **Service\_CF**.
...
Поле **Service\_CF** создано, но еще не добавлено ни к одной записи.

![Начальное количество](media/log-analytics-custom-fields/initial-count.png)

По истечении некоторого времени будут собраны новые события, и поле **Service\_CF** начнет добавляться к записям, соответствующим нашим условиям.

![Окончательные результаты](media/log-analytics-custom-fields/final-results.png)

Теперь мы можем использовать настраиваемое поле, как и другие свойства записи. Для иллюстрации этого мы создадим запрос, который группирует записи по новому полю **Service\_CF**. Таким образом мы сможем проверить, какие службы наиболее активные.
