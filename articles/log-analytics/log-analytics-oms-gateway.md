...
* Компьютеры с Windows с Microsoft Monitoring Agent, который напрямую подключен к рабочей области OMS.
* Компьютеры на базе Linux с агентом OMS для Linux, который напрямую подключен к рабочей области OMS.  
...
![Схема взаимодействия Operations Manager с OMS](./media/log-analytics-oms-gateway/log-analytics-agent-opsmgrconnect.png)
...
* [Добавьте источники данных](log-analytics-data-sources.md), чтобы собирать данные из подключенных источников в рабочую область Log Analytics и сохранять их в репозитории Log Analytics.
