...
Чтобы выполнить мониторинг шлюза OMS и проанализировать производительность и данные событий сервера, где он установлен, агент OMS необходимо установить на компьютере со шлюзом.
...
1. Скачайте последнюю версию шлюза OMS в [Центре загрузки Майкрософт](http://download.microsoft.com/download/2/5/C/25CF992A-0347-4765-BD7D-D45D5B27F92C/OMS%20Gateway.msi).
2. Чтобы начать установку, дважды щелкните файл **OMS Gateway.msi**.
...
4. На странице лицензионного соглашения установите флажок **Я принимаю условия лицензионного соглашения**, а затем нажмите кнопку **Далее**.
5. На странице со сведениями об адресе порта и прокси-сервера:
   ...
   3. Щелкните **Далее**
6. Если Центр обновления Майкрософт отключен, появится страница "Центр обновления Майкрософт", где его можно включить. Сделайте выбор и нажмите кнопку **Далее**. В противном случае перейдите к следующему шагу.
7. На странице "Конечная папка" оставьте папку по умолчанию (**%ProgramFiles%\OMS Gateway**) или укажите расположение, куда необходимо установить шлюз, и нажмите кнопку **Далее**.
8. На странице "Все готово для установки" щелкните **Установить**. После этого служба контроля учетных записей может запросить разрешение на установку. Чтобы продолжить, нажмите кнопку **Да**.
9. После завершения установки нажмите кнопку **Готово**. Проверить, запущена ли служба, можно в оснастке services.msc. Если это так, **шлюз OMS** появится в списке служб.  
...
Если при выполнении шага 3 произошла ошибка, значит модуль не импортирован. Возможно, эта ошибка возникла, так как PowerShell не удалось найди модуль. Его можно найти по указанному пути установки шлюза: C:\Program File\Microsoft OMS Gateway\PowerShell.
...
Мы рекомендуем устанавливать агент OMS на компьютерах с установленным шлюзом. Затем агент можно использовать для сбора событий, регистрируемых шлюзом.
