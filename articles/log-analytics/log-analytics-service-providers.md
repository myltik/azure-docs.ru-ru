<properties
    pageTitle="Функции Log Analytics для поставщиков служб | Microsoft Azure"
...
Log Analytics входит в число служб Azure, доступных в рамках подписки CSP, для партнеров и поставщиков услуг, работающих по программе [поставщика облачных решений (CSP)](https://partner.microsoft.com/Solutions/cloud-reseller-overview). 
...
+ В PowerShell укажите параметр `-Tenant contoso.com` при использовании командлета `Add-AzureRmAccount`.
...
>[AZURE.NOTE] Решения резервного копирования и восстановления сайтов (Site Recovery) для Log Analytics не могут подключаться к хранилищу служб восстановления, поэтому их нельзя настроить в подписке CSP.
