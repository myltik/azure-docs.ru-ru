---
title: "Настройка параметров прокси-сервера и брандмауэра в службе Azure Log Analytics | Документация Майкрософт"
...
Действия, необходимые для настройки параметров прокси-сервера и брандмауэра для службы Log Analytics, отличаются в зависимости от используемых типов агентов. Просмотрите следующие разделы для типа агента, который используете.

## <a name="settings-for-the-oms-gateway"></a>Параметры шлюза OMS

Если у агентов нет доступа к Интернету, они могут отправлять данные в шлюз OMS с помощью собственных сетевых ресурсов. Шлюз собирает данные и отправляет их в службу OMS от имени этих устройств и компьютеров.

Настройте агенты, обменивающиеся данными со шлюзом OMS, используя его полное доменное имя и номер пользовательского порта.

Шлюзу OMS требуется доступ к Интернету. Используйте те же параметры прокси-сервера или брандмауэра для шлюза OMS, что и для имеющихся типов агентов. Дополнительные сведения о шлюзе OMS см. в статье [Подключения компьютеров и устройств к OMS с помощью шлюза OMS](log-analytics-oms-gateway.md).

## <a name="configure-settings-with-the-microsoft-monitoring-agent"></a>Настройка параметров с помощью Microsoft Monitoring Agent
...
## <a name="configure-settings-with-operations-manager"></a>Настройка параметров с помощью Operations Manager
