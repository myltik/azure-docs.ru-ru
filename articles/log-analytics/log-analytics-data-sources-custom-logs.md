<properties 
   pageTitle="Настраиваемые журналы в службе Log Analytics | Microsoft Azure"
...
# Настраиваемые журналы в службе Log Analytics
...
### Шаг 1. Открытие мастера настраиваемого журнала

Мастер настраиваемого журнала на портале OMS позволяет определить новый настраиваемый журнал для сбора данных.

1.	На портале OMS выберите **Параметры**.
2.	Выберите вкладку **Данные**, а затем — раздел **Custom logs** (Настраиваемые журналы).
3.	По умолчанию все изменения конфигурации автоматически отправляются во все агенты. Для агентов Linux файл конфигурации отправляется в сборщик данных Fluentd. Если вы хотите изменить этот файл вручную в каждом агенте Linux, снимите флажок *Apply below configuration to my Linux machines* (Применить конфигурацию ниже к моим компьютерам Linux).
4.	Щелкните **Add+** (Добавить+), чтобы открыть мастер настраиваемого журнала.

### Шаг 2. Загрузка и преобразование примера журнала
...
>[AZURE.NOTE]В настоящее время Log Analytics обрабатывает дату и время, собранные из журнала, используя в качестве разделителя метку времени в формате UTC. Вскоре в агенте для этого будет использоваться часовой пояс.
...
### Шаг 3. Добавление пути к файлам журнала
...
| Все файлы агента Windows в папке */var/log/audits* с расширением .txt | /var/log/audit/*.txt |
| Все файлы агента Windows в папке */var/log/audit* с расширением .txt и именем, начинающимся со слова log | /var/log/audit/log*.txt |
...
### Шаг 4. Указание имени и описания журнала
...
### Шаг 5. Проверка того, собираются ли данные настраиваемых журналов
...
>[AZURE.NOTE] Если в разделе поиска отсутствует свойство RawData, закройте и откройте браузер снова.

### Шаг 6. Преобразование записей настраиваемого журнала
...
| SourceSystem | Тип агента, из которого собрана запись. <br> OpsManager — агент Windows, подключенный напрямую или через SCOM. <br> Linux — все агенты Linux. |
...
## Поиск по журналу с использованием записей настраиваемого журнала
...
| Запрос | Описание |
|:--|:--|
| Type=MyApp\_CL | Все события из настраиваемого журнала с именем MyApp\_CL. |
| Type=MyApp\_CL Severity\_CF=error | Все события из настраиваемого журнала с именем MyApp\_CL и значением *error* в настраиваемом поле с именем *Severity\_CF*. |
...
В этом разделе описывается процедура создания настраиваемого журнала. В каждой строке примера журнала, из которого собираются данные, содержится одна запись, которая начинается с даты и времени, после которых идут поля для кода, состояния и сообщения, разделенные запятыми. Ниже приведены некоторые примеры записей.
...
Файлы журнала будут храниться в каталоге *C:\\MyApp\\Logs*. Каждый день создается файл с именем, которое включает в себя дату, в формате *приложение\_ГГГГММДД.log*. Для этого журнала будет использоваться путь *C:\\MyApp\\Logs\\*.log*.

![Путь к файлам журнала](media/log-analytics-data-sources-custom-logs/collection-path.png)

### Указание имени и описания журнала

Мы введем имя *MyApp\_CL* и текст **описания**.
...
Мы используем запрос *Type=MyApp\_CL*, чтобы вернуть все записи из журнала, данные которого собираются.
...
Мы используем настраиваемые поля, чтобы определить поля *EventTime*, *Code*, *Status* и *Message*. Кроме того, мы сможем увидеть разницу в записях, которые возвращаются запросом.

![Запрос к журналу с использованием настраиваемых полей](media/log-analytics-data-sources-custom-logs/query-02.png)

## Дальнейшие действия

- Используйте [настраиваемые поля](log-analytics-custom-fields.md), чтобы разделить записи в настраиваемом журнале на отдельные поля.
