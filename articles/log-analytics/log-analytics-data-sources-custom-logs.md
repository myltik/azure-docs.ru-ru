---
title: "Сбор пользовательских журналов в службе анализа журналов Azure | Документы Microsoft"
description: "Служба Log Analytics может собирать события из текстовых файлов на компьютерах Windows и Linux.  В этой статье описывается укажите новый пользовательский журнал и сведения для записи, которые они создают в рабочей области аналитики журналов."
...
С помощью настраиваемых журналов, выступающих в качестве источников данных, в службе Log Analytics можно собирать события из текстовых файлов на компьютерах Windows и Linux. Во многих приложениях вместо стандартной службы ведения журнала, например журнала событий Windows или системного журнала, данные журналов записываются в текстовые файлы.  После сбора, можно выполнить синтаксический анализ каждой записи в процессе входа для отдельных полей с помощью [настраиваемые поля](log-analytics-custom-fields.md) функции аналитики журналов.
...
Специальный мастер журнала выполняется на портале Azure и позволяет определить новый пользовательский журнал для сбора.
...
Если используется разделитель отметки времени, затем свойство TimeGenerated каждую запись в службе анализа журналов будет заполняться даты и времени, указанный для этой записи в файл журнала.  Если в качестве разделителя используется новая строка, свойство TimeGenerated заполняется датой и временем сбора этой записи в Log Analytics.
...
## <a name="removing-a-custom-log"></a>Удаление пользовательского журнала
Удаление пользовательских журналов, ранее с помощью процесса на портале Azure.

1. Из **данные** меню в **Дополнительные параметры** рабочей области, выберите **пользовательские журналы** Чтобы перечислить все пользовательские журналы.
2. Нажмите кнопку **удалить** рядом с пользовательский журнал для удаления.
...
* Используйте [настраиваемые поля](log-analytics-custom-fields.md) выполнить синтаксический анализ записи в пользовательских входа для отдельных полей.
