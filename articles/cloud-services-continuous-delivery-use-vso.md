<properties 
	...
	description="В данном разделе приводится информация о том, как настроить командные проекты Visual Studio Online для автоматического создания и развертывания веб-сайтов или облачных служб Azure." 
	services="web-sites" 
...
# Непрерывная доставка в Azure с использованием Visual Studio Online
  ...
  Вы можете настроить командные проекты Visual Studio Online для автоматического построения облачных служб или веб-сайтов Azure.  (Дополнительные сведения о настройке непрерывное построение и развертывание с помощью системы *on-premises* Team Foundation Server, в разделе [непрерывная доставка для облачных служб в Azure](../cloud-services-dotnet-continuous-delivery).)
...
-   [Шаг 4. Внесение необходимых изменений и запуск процессов повторной сборки и развертывания.][]
...
Следуйте инструкциям [здесь](http://go.microsoft.com/fwlink/?LinkId=512980) для создания командного проекта и связать его с Visual Studio. В этом руководстве в качестве системы управления версиями применяется Team Foundation Version Control (TFVC). Если вы хотите использовать Git для системы управления версиями, см. раздел [версии Git в этом пошаговом руководстве](http://go.microsoft.com/fwlink/p/?LinkId=397358).
...
Чтобы создать веб-сайт, выберите шаблон проекта "Веб-приложение ASP.NET" и затем выберите MVC. См. раздел [Начало работы с Azure и ASP.NET](http://azure.microsoft.com/documentation/articles/web-sites-dotnet-get-started/).

2. Откройте контекстное меню для решения и выберите **добавить решение в систему управления версиями**.<br/>
![][5]

3. Примите или измените значения по умолчанию и нажмите кнопку **ОК** кнопки. По завершении обработки в обозревателе решений появится значок системы управления версиями.<br/>
![][6]

4. Откройте контекстное меню для решения и выберите **вернуть**.<br/>
![][7]

5. В области "Ожидающие изменения" Team Explorer введите комментарий для возврата и выберите **вернуть** кнопки.<br/>
![][8]

<br/>
Обратите внимание, параметры для включения или исключения определенных изменений при возврате. При необходимости изменения исключаются выберите **включить все** связи.<br/>
![][9]

<h2> <a name="step3"> </a>Шаг 3. Подключение проекта к Azure</h2>

1. После того как был создан командный проект VSO с определенным исходным кодом, вы можете подключить его к Azure.  В [портала Azure](http://manage.windowsazure.com), выберите облачную службу или веб-сайт или создать новый, выбрав значок внизу слева и выбрав + **облачной службы** или **веб-сайта** и **быстрое создание**. Выберите **Настройка публикации в Visual Studio Online** связи.<br/>
![][10]

2. В мастере, введите имя учетной записи Visual Studio Online в текстовое поле и нажмите **авторизовать сейчас** связи. Может появиться запрос на вход в систему.<br/>
![][11]

3. Выберите в всплывающем окне OAuth **Accept** для авторизации Azure для настройки командного проекта в VSO.<br/>
![][12]
...
5. После привязки проекта появятся инструкции по проверке изменений в командном проекте Visual Studio Online.  При следующей регистрации Visual Studio Online будет строить и развертывать ваш проект в Azure.  Попробуйте реализованное поведение, щелкните **возврата из Visual Studio** ссылку, а затем **запустить Visual Studio** связи (или его эквивалент **Visual Studio** , расположенную в нижней части окна портала).<br/>
![][14]

<h2><a name="step4"> </a>Шаг 4. Запуск повторной сборки и развертывания проекта</h2>

1. В Visual Studio Team Explorer щелкните **обозревателя управления исходным кодом** связи.<br/>
![][15]
...
5. В Team Explorer выберите **ожидающие изменения** связи.<br/>
![][19]

6. Введите комментарий и нажмите кнопку **вернуть** кнопки.<br/>
![][20]

7. Нажмите кнопку "Главная", чтобы вернуться на главную страницу Team Explorer.<br/>
![][21]

8. Выберите **строит** ссылку, чтобы просмотреть выполняющиеся построения.<br/>
![][22]
<br/>
...
![][25]
<br/>
В **триггер** вкладку, вы увидите, определение построения имеет значение по умолчанию для сборки на каждом возврате.<br/>
![][26]
<br/>
...
Если вы используете несколько конфигураций службы (cscfg-файлы), можно указать конфигурацию требуемой службы в **сборки, Дополнительно, Аргументы MSBuild** параметр. Например, чтобы использовать файл ServiceConfiguration.Test.cscfg, задайте аргумент командной строки MSBuild /p:TargetProfile=Test.<br/>
![][38]

11. К этому моменту процесс построения должен быть успешно завершен.<br/>
![][28]

12. Если дважды щелкнуть имя сборки, Visual Studio отобразит **сводку сборки**, включающую все результаты тестирования из соответствующих проектов модульного тестирования.<br/>
![][29]
...
Этот шаг необязателен и применяется к облачным службам. На портале управления выберите предыдущее развертывание и нажмите кнопку **повторно развернуть** кнопку, чтобы восстановить состояние сайта на предыдущих возврата.  Обратите внимание, что это будет активировать новую сборку в TFS и создать новую запись в журнале развертывания.<br/>
![][34]
...
Этот шаг применяется только к веб-сайтам, не облачных служб. Чтобы поместить условия качества вашего развертывания, можно выполнять модульные тесты и, если они не могут остановить развертывание.

1.  В Visual Studio добавьте проект модульного теста.<br/>
![][39]

2.  Добавление ссылки на проект в проект, который требуется проверить.<br/>
![][40]
...
5.  Задайте **завершить построение при ошибке теста** значение True. Это означает, что развертывание не будет происходить, если тест не будет пройден положительно.<br/>
![][41]
...
Дополнительные сведения о модульном тестировании в Visual Studio Online см. в разделе [выполнять модульные тесты в сборке](http://go.microsoft.com/fwlink/p/?LinkId=510474).

Дополнительные сведения см. в разделе [Visual Studio Online](http://go.microsoft.com/fwlink/?LinkId=253861). Если вы используете Git, см. раздел [совместному использованию кода в Git](http://www.visualstudio.com/get-started/share-your-code-in-git-vs.aspx) и [публикация из системы управления версиями на веб-сайтах Azure](http://azure.microsoft.com/documentation/articles/web-sites-publish-source-control).

[Шаг 1. Создание командного проекта.]: #step1
[Шаг 2. Регистрация проекта в системе управления версиями.]: #step2
[Шаг 3. Подключение проекта к Azure.]: #step3
[Шаг 4. Внесение необходимых изменений и запуск процессов повторной сборки и развертывания.]: #step4
