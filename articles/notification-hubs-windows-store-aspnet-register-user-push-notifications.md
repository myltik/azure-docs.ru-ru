<properties linkid="notification-hubs-how-to-guides-howto-register-user-with-aspnet-webapi-windowsphonedotnet" urlDisplayName="Уведомление пользователей приложения Магазина Windows с помощью веб-интерфейса API" pageTitle="Регистрация текущего пользователя для push-уведомлений с помощью веб-интерфейса API — концентраторы уведомлений" metaKeywords="Регистрация приложения Azure, концентраторы уведомлений, push-уведомления Azure, push-уведомления приложения Магазина Windows" description="Узнайте, как запросить регистрацию push-уведомления в приложении Магазина Windows с помощью концентраторов уведомлений Azure, когда регистрация выполняется с помощью веб-интерфейса API ASP.NET." metaCanonical="" services="notification-hubs" documentationCenter="" title="Регистрация текущего пользователя для push-уведомлений с помощью ASP.NET" authors="" solutions="" manager="" editor="" />
...
3. В обозревателе решений щелкните правой кнопкой мыши проект, выберите **Добавить**, **Класс**, введите `LocalStorageManager` и нажмите кнопку **Добавить**.

	![][1] 

	При этом в проект добавляется файл кода для класса **LocalStorageManager**.

4. Откройте новый файл проекта LocalStorageManager.cs и добавьте следующий оператор **using**:

		using Windows.Storage;

5. Замените определение класса **LocalStorageManager** следующим кодом:
...
6. Откройте файл проекта MainPage.xaml и замените корневой элемент **Grid** следующим кодом XAML:
...
7. В файле MainPage.xaml.cs добавьте следующие операторы **using**:
...
8. Добавьте следующее определение в класс **MainPage**:

		private string registerUri = "https://<SERVICE_ROOT_URL>/api/register";
        
	Замените _`<SERVICE_ROOT_URL>`_ корневым URI веб-API, который был создан на шаге **Уведомление пользователей с помощью концентраторов уведомлений** 
